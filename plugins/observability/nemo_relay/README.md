...
The `MOOR_NEMO_RELAY_*` environment variables below only configure an
already-enabled plugin. They do not enable plugin discovery by themselves.

For isolated test homes, enable the plugin in the same `MOOR_HOME` that the
agent run will use:

```bash
env MOOR_HOME=/tmp/moor-nemo-relay-test \
  moor plugins enable observability/nemo_relay
```

Runs started with `--ignore_user_config` skip the enabled-plugin state from
`MOOR_HOME`, so local E2E tests should omit that flag unless the test harness
loads `observability/nemo_relay` explicitly another way.

`MOOR_HOME` is the Moor profile/config home used by both
`moor plugins enable ...` and the later `moor chat ...` run. If unset,
Moor uses the user's default home, usually `~/.moor`. For isolated smoke
tests, choose any writable temporary directory and use the same value for every
command in that test:

```bash
export MOOR_HOME=/tmp/moor-nemo-relay-test
moor plugins enable observability/nemo_relay
moor chat --query 'Reply exactly ok' --provider custom --model qwen3.6:35b
```

For source checkouts, make sure the `moor` command you run is built from the
checkout that contains this plugin. A globally installed older CLI will not see
new bundled plugins from your working tree.

```bash
uv sync
uv run moor plugins enable observability/nemo_relay
...
```bash
uv build --wheel
python -m pip install --force-reinstall dist/moor_agent-*.whl
moor plugins enable observability/nemo_relay
```
...
`moor.session.segment` / `moor.session.segment_reason`
...
The plugin can configure exporters directly from `MOOR_NEMO_RELAY_*`
...
```bash
export MOOR_NEMO_RELAY_ATOF_ENABLED=1
export MOOR_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=.nemo-relay/atof
export MOOR_NEMO_RELAY_ATIF_ENABLED=1
export MOOR_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=.nemo-relay/atif
```

Optional overrides:

- `MOOR_NEMO_RELAY_ATOF_FILENAME`
- `MOOR_NEMO_RELAY_ATOF_MODE` (`append` or `overwrite`)
- `MOOR_NEMO_RELAY_ATIF_FILENAME_TEMPLATE`
- `MOOR_NEMO_RELAY_ATIF_AGENT_NAME`
- `MOOR_NEMO_RELAY_ATIF_AGENT_VERSION`
- `MOOR_NEMO_RELAY_ATIF_MODEL_NAME`
- `MOOR_NEMO_RELAY_ATIF_SUBAGENT_EXPORT_MODE` (`embedded` by default; set `all` to also write standalone child files)
...
```bash
export MOOR_NEMO_RELAY_PLUGINS_TOML=.nemo-relay/plugins.toml
```
...
When `MOOR_NEMO_RELAY_PLUGINS_TOML` is set and initializes successfully, NeMo
Relay owns exporter lifecycle through that config. The direct
`MOOR_NEMO_RELAY_ATOF_*` fallback setup is skipped. If the same
`plugins.toml` observability config enables `atif`, the direct
`MOOR_NEMO_RELAY_ATIF_*` fallback setup is also skipped so Moor does not
...
```bash
export MOOR_HOME=/tmp/moor-nemo-relay-docs/moor-home
mkdir -p "$MOOR_HOME"

cat > "$MOOR_HOME/config.yaml" <<'YAML'
model:
  provider: custom
  default: qwen3.6:35b
...
```bash
export MOOR_NEMO_RELAY_ATOF_ENABLED=1
export MOOR_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=/tmp/moor-nemo-relay-docs/subagent/atof
export MOOR_NEMO_RELAY_ATOF_FILENAME=nested-subagent-atof.jsonl
export MOOR_NEMO_RELAY_ATOF_MODE=overwrite
export MOOR_NEMO_RELAY_ATIF_ENABLED=1
export MOOR_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=/tmp/moor-nemo-relay-docs/subagent/atif
export MOOR_NEMO_RELAY_ATIF_FILENAME_TEMPLATE='nested-subagent-atif-{session_id}.json'
export MOOR_NEMO_RELAY_ATIF_AGENT_NAME='Moor Agent E2E'
export MOOR_NEMO_RELAY_ATIF_AGENT_VERSION=docs-example
export MOOR_NEMO_RELAY_ATIF_SUBAGENT_EXPORT_MODE=all
...
{"kind":"mark","name":"moor.subagent.start","metadata":{"parent_session_id":"docs-parent-session","session_id":"docs-child-session","subagent_id":"sa-0-docs","child_role":"leaf"}}
...
```bash
mkdir -p /tmp/moor-nemo-relay-docs/workdir
printf 'docs_parallel_alpha_function\n' > /tmp/moor-nemo-relay-docs/workdir/alpha.txt
printf 'docs_parallel_beta_function\n' > /tmp/moor-nemo-relay-docs/workdir/beta.txt
cd /tmp/moor-nemo-relay-docs/workdir

export MOOR_NEMO_RELAY_ATOF_ENABLED=1
export MOOR_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=/tmp/moor-nemo-relay-docs/parallel/atof
export MOOR_NEMO_RELAY_ATOF_FILENAME=parallel-tools-atof.jsonl
export MOOR_NEMO_RELAY_ATOF_MODE=overwrite
export MOOR_NEMO_RELAY_ATIF_ENABLED=1
export MOOR_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=/tmp/moor-nemo-relay-docs/parallel/atif
export MOOR_NEMO_RELAY_ATIF_FILENAME_TEMPLATE='parallel-tools-atif-{session_id}.json'
export MOOR_NEMO_RELAY_ATIF_AGENT_NAME='Moor Agent E2E'
export MOOR_NEMO_RELAY_ATIF_AGENT_VERSION=docs-example
...
```bash
export MOOR_NEMO_RELAY_PLUGINS_TOML=/tmp/moor-middleware-test/plugins.toml
```
...
```bash
export MOOR_HOME=/tmp/moor-middleware-test/moor-home
mkdir -p "$MOOR_HOME" /tmp/moor-middleware-test/nemo-relay
...
cat > /tmp/moor-middleware-test/nemo-relay/plugins.toml <<'TOML'
version = 1
...
[components.config.atof]
enabled = true
output_directory = "/tmp/moor-middleware-test/atof"
filename = "middleware-events.jsonl"
mode = "overwrite"

[components.config.atif]
enabled = true
output_directory = "/tmp/moor-middleware-test/atif"
...
export MOOR_NEMO_RELAY_PLUGINS_TOML=/tmp/moor-middleware-test/nemo-relay/plugins.toml
