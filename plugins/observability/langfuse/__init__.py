...
def _debug(message: str) -> None:
    if _env("MOOR_LANGFUSE_DEBUG").lower() in {"1", "true", "yes", "on"}:
...
def _build_client() -> Optional[Langfuse]:
    ...
    public_key, secret_key = (_env(f"MOOR_LANGFUSE_{n}") or _env(f"LANGFUSE_{n}") for n in ("PUBLIC_KEY", "SECRET_KEY"))
    ...
    for key, name, default in (("base_url", "BASE_URL", "https://cloud.langfuse.com"), ("environment", "ENV", ""),
                               ("release", "RELEASE", "")):
        value = _env(f"MOOR_LANGFUSE_{name}") or _env(f"LANGFUSE_{name}") or default
        if value:
            kwargs[key] = value
...
def _canonical_usage_and_cost(canonical: Any, *, provider: str, model: str,
    ...
    # need an explicit total (Moor estimate also includes request pricing).
...
def _start_root_trace(task_key: str, *, task_id: str, session_id: str, platform: str, provider: str, model: str,
    ...
    metadata = {
        "source": "moor", "task_id": task_id, "turn_id": turn_id, "api_request_id": api_request_id,
    ...
    def open_root():
        ctx = client.start_as_current_observation(trace_context=trace_ctx, name="Moor turn", as_type="chain",
    ...
    if propagate_attributes is not None:
        try:
            with propagate_attributes(session_id=session_id or task_key, trace_name="Moor turn",
                                      tags=["moor", "langfuse"]):
...
def register(ctx) -> None:
    # Both hook-name variants so the plugin works across Moor versions:
