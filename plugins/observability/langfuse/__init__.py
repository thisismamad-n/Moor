...
load when listed in ``plugins.enabled`` (via ``moor plugins enable
observability/langfuse`` or ``moor tools → Langfuse Observability``). At
runtime the plugin also requires the ``langfuse`` SDK and credentials; if
either is missing the hooks are inert.

Required env vars (set via ``moor tools`` or ~/.moor/.env):
  MOOR_LANGFUSE_PUBLIC_KEY  - Langfuse project public key (pk-lf-...)
  MOOR_LANGFUSE_SECRET_KEY  - Langfuse project secret key (sk-lf-...)
  MOOR_LANGFUSE_BASE_URL    - Langfuse server URL (default: https://cloud.langfuse.com)

Optional env vars:
  MOOR_LANGFUSE_ENV         - environment tag (e.g. "production", "local")
  MOOR_LANGFUSE_RELEASE     - release/version tag
  MOOR_LANGFUSE_SAMPLE_RATE - sampling rate 0.0–1.0 (default: 1.0)
  MOOR_LANGFUSE_MAX_CHARS   - max chars per field (default: 12000)
  MOOR_LANGFUSE_CAPTURE     - content capture mode (default: "sanitized")
  ...
  MOOR_LANGFUSE_DEBUG       - set to "true" for verbose logging
"""
...
def _debug_enabled() -> bool:
    return _env_bool("MOOR_LANGFUSE_DEBUG")
...
def _capture_mode() -> str:
    ...
    value = _env("MOOR_LANGFUSE_CAPTURE").lower()
    if not value:
    ...
    if not _warned_invalid_capture:
        ...
        logger.warning(
            "Langfuse plugin: invalid MOOR_LANGFUSE_CAPTURE=%r, falling back "
            "to %r (valid: %s)",
    ...
    with _LANGFUSE_CLIENT_LOCK:
        ...
        if Langfuse is None:
            logger.warning(
                ...
                "tracing is disabled. Run `moor tools` and configure Langfuse "
                "Observability to reinstall it."
            )
            _LANGFUSE_CLIENT = _INIT_FAILED
            return None

        public_key = _env("MOOR_LANGFUSE_PUBLIC_KEY") or _env("LANGFUSE_PUBLIC_KEY")
        secret_key = _env("MOOR_LANGFUSE_SECRET_KEY") or _env("LANGFUSE_SECRET_KEY")
        ...
        placeholder_issues = [
            msg
            for msg in (
                _validate_langfuse_key("MOOR_LANGFUSE_PUBLIC_KEY", public_key),
                _validate_langfuse_key("MOOR_LANGFUSE_SECRET_KEY", secret_key),
            )
            if msg
        ]
        if placeholder_issues:
            logger.warning(
                ...
                "or unset MOOR_LANGFUSE_PUBLIC_KEY / MOOR_LANGFUSE_SECRET_KEY to "
        ...
        base_url = _env("MOOR_LANGFUSE_BASE_URL") or _env("LANGFUSE_BASE_URL") or "https://cloud.langfuse.com"
        environment = _env("MOOR_LANGFUSE_ENV") or _env("LANGFUSE_ENV")
        release = _env("MOOR_LANGFUSE_RELEASE") or _env("LANGFUSE_RELEASE")
        sample_rate = _env("MOOR_LANGFUSE_SAMPLE_RATE")
        ...
        if sample_rate:
            try:
                ...
                logger.warning("Invalid MOOR_LANGFUSE_SAMPLE_RATE=%r", sample_rate)
        ...
        # short-lived-process gap (kanban workers / moor chat -q / cron): exit
...
def _finalize_all_traces() -> None:
    ...
    kanban workers, ``moor chat -q`` one-shots, cron jobs — can exit while
