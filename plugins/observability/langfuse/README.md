...
Set these in `~/.moor/.env` (or via `moor tools`):
...
moor chat -q "hello"              # then check Langfuse for a "Moor turn" trace
```
...
child span to inspect `role: system` (truncated via `MOOR_LANGFUSE_MAX_CHARS`).
...
`MOOR_LANGFUSE_CAPTURE` controls how much *content* (prompts, responses,
...
```bash
moor plugins disable observability/langfuse
```
