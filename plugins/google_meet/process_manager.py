...
One active meeting at a time, recorded in ``$MOOR_HOME/workspace/meetings/.active.json``
...
def start(url: str, *, out_dir: Optional[Path] = None, headed: bool = False,
          auth_state: Optional[str] = None, guest_name: str = "Moor Agent", duration: Optional[str] = None,
    ...
    for name in ("transcript.txt", "status.json"):
        with contextlib.suppress(OSError):
            (out / name).unlink()
    env = {**os.environ, "MOOR_MEET_URL": url, "MOOR_MEET_OUT_DIR": str(out),
           "MOOR_MEET_GUEST_NAME": guest_name}
    for value, var in (
        (headed and "1", "MOOR_MEET_HEADED"),
        (auth_state, "MOOR_MEET_AUTH_STATE"),
        (duration, "MOOR_MEET_DURATION"),
        (mode, "MOOR_MEET_MODE"),  # bot defaults to transcribe when unset (v1 behavior)
        (realtime_model, "MOOR_MEET_REALTIME_MODEL"),
        (realtime_voice, "MOOR_MEET_REALTIME_VOICE"),
        (realtime_instructions, "MOOR_MEET_REALTIME_INSTRUCTIONS")):
        if value:
            env[var] = value
    ...
    if not realtime_api_key:
        ...
        realtime_api_key = get_secret("MOOR_MEET_REALTIME_KEY") or get_secret("OPENAI_API_KEY")
    if realtime_api_key:
