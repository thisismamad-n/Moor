...
Standalone subprocess spawned by ``process_manager.py``; configured via ``MOOR_MEET_*`` env,
...
Debug: ``MOOR_MEET_URL=... MOOR_MEET_OUT_DIR=/tmp/x MOOR_MEET_HEADED=1 \\
...
# collects {speaker, text}; ``window.__moorMeetDrain()`` pulls new entries.
...
_ADMISSION_PROBE_JS = r"""
    (() => {
      ...
      if (window.__moorMeetInstalled && document.querySelector(
...
def _start_pcm_pump(rt: dict, bridge_info: dict, pcm_path: Path, state: "_BotState") -> None:
    ...
    if platform_tag == "linux":
        cmd = ["paplay", "--raw", "--rate=24000", "--format=s16le", "--channels=1",
               f"--device={target or 'moor_meet_sink'}", str(pcm_path)]
...
_BotConfig = SimpleNamespace  # everything the bot reads from ``MOOR_MEET_*`` env vars


def _config_from_env() -> _BotConfig:
    env = os.environ.get
    out_raw = env("MOOR_MEET_OUT_DIR", "").strip()
    return _BotConfig(
        url=env("MOOR_MEET_URL", "").strip(),
        out_dir=Path(out_raw) if out_raw else None,
        headed=env("MOOR_MEET_HEADED", "").lower() in {"1", "true", "yes"},
        auth_state=env("MOOR_MEET_AUTH_STATE", "").strip(),
        guest_name=env("MOOR_MEET_GUEST_NAME", "Moor Agent"),
        duration_s=_parse_duration(env("MOOR_MEET_DURATION", "")),
        realtime=env("MOOR_MEET_MODE", "transcribe").strip().lower() == "realtime",
        # MOOR_MEET_REALTIME_KEY is resolved by process_manager.start() via the parent's
        # profile secret scope; OPENAI_API_KEY only serves standalone `python -m` runs.
        realtime_api_key=env("MOOR_MEET_REALTIME_KEY") or env("OPENAI_API_KEY", ""),
        realtime_model=env("MOOR_MEET_REALTIME_MODEL", "gpt-realtime"),
        realtime_voice=env("MOOR_MEET_REALTIME_VOICE", "alloy"),
        realtime_instructions=env("MOOR_MEET_REALTIME_INSTRUCTIONS", ""),
        lobby_timeout=float(env("MOOR_MEET_LOBBY_TIMEOUT", "300")))
...
def run_bot() -> int:
    ...
    if not _is_safe_meet_url(cfg.url):
        sys.stderr.write("google_meet bot: refusing to launch — MOOR_MEET_URL must be a "
