...
Let the moor agent join a Google Meet call, transcribe it, optionally speak
...
```
┌─ gateway (Linux box, where moor runs) ────────────────────────────┐
│                                                                      │
...
│                                                                      │
│   NodeServer (from `moor meet node run`)                           │
│     │                                                                │
...
| `__init__.py` | `register(ctx)` — registers 5 tools + `on_session_end` hook + `moor meet` CLI |
...
| `cli.py` | `moor meet setup / auth / join / status / transcript / say / stop / node ...` |
...
| `node/registry.py` | v3: `$MOOR_HOME/workspace/meetings/nodes.json` |
| `node/server.py` | v3: `NodeServer` (runs on host machine) |
| `node/client.py` | v3: `NodeClient` (used by tool handlers + CLI on gateway) |
| `node/cli.py` | v3: `moor meet node {run,list,approve,remove,status,ping}` |
...
moor meet install                                      # pip + Chromium
moor meet setup                                        # preflight
moor meet auth                                         # optional
moor meet join https://meet.google.com/abc-defg-hij    # transcribe
```
...
```bash
moor meet install --realtime                     # installs pulseaudio-utils
echo 'OPENAI_API_KEY=sk-...' >> ~/.moor/.env
moor meet join https://meet.google.com/abc-defg-hij --mode realtime
# then from the agent or CLI:
moor meet say "Good morning everyone, I'm the note-taker bot."
```

macOS:
```bash
moor meet install --realtime     # runs: brew install blackhole-2ch ffmpeg
# then — manually! — open System Settings → Sound → Input → BlackHole 2ch
echo 'OPENAI_API_KEY=sk-...' >> ~/.moor/.env
moor meet join https://meet.google.com/abc-defg-hij --mode realtime
```

On macOS, moor will **not** switch your system audio input automatically — the
...
On the gateway:
```bash
moor meet node approve my-mac ws://<mac-ip>:18789 <token>
moor meet node ping my-mac
