...
moor meet install                 # pip deps + Chromium (transcribe only)
...
#   Then set OPENAI_API_KEY or MOOR_MEET_REALTIME_KEY in ~/.moor/.env
```
...
moor meet node ping my-mac                   # confirm reachable
```

Run `moor meet setup` to preflight local prereqs.
...
- Guest mode sits in the lobby until a host admits. Warn the user; `moor meet auth` avoids this.
- **Lobby timeout**: if the host doesn't admit the bot within 5 minutes (configurable via `MOOR_MEET_LOBBY_TIMEOUT` env), the bot leaves and `meet_status` reports `leaveReason: "lobby_timeout"`.
...
| `realtimeDevice` | Audio device name the bot is feeding (e.g. `moor_meet_src`). |
...
Local:
```
$MOOR_HOME/workspace/meetings/<meeting-id>/transcript.txt
```
...
- Remote nodes use bearer-token auth; tokens are generated on the node (32 hex chars, persisted in `$MOOR_HOME/workspace/meetings/node_token.json`) and must be copied to the gateway via `moor meet node approve`.
