...
``BROWSER_USE_API_KEY`` (https://browser-use.com) or the managed Moor tool gateway (bills to a
Moor subscription). Direct first, managed second, unless ``tool_gateway.browser: gateway`` flips
...
class BrowserUseBrowserProvider(CloudBrowserProvider):
    ...
    # registered for the Moor gateway path and legacy cloud_provider configs.
    setup_tag = None
    ...
    def _get_config_or_none(self, *, refresh_token: bool = True) -> Optional[Dict[str, Any]]:
        # Lazy: managed_tool_gateway pulls in the Moor auth stack direct-key users never need.
        from tools.managed_tool_gateway import peek_moor_access_token, resolve_managed_tool_gateway
        ...
        def _managed_config() -> Optional[Dict[str, Any]]:
            ...
            managed = resolve_managed_tool_gateway(
                "browser-use", token_reader=None if refresh_token else peek_moor_access_token)
            if managed is None:
                return None
            return {
        ...
        # Strict: "moor" (or legacy use_gateway: true) → managed ONLY; any other stored selection →
    ...
    def _get_config(self) -> Dict[str, Any]:
        from tools.tool_backend_helpers import (
            MOOR_MANAGED_PROVIDER, managed_moor_tools_enabled, read_selection, selection_error)
