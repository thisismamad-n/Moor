...
``BROWSER_USE_API_KEY`` for self-billed users, or the managed Moor tool
gateway (which Moor uses to bill Browser Use sessions to a Moor
...
Auth env vars (one of)::
    ...
    # OR a managed Moor gateway entry (configured via 'moor setup')
"""
...
# Idempotency tracking for managed-mode session creation. The managed Moor
...
class BrowserUseBrowserProvider(BrowserProvider):
    ...
    to the managed Moor tool gateway when ``tool_gateway.browser`` config
    ...
    # ------------------------------------------------------------------
    # Config resolution (direct API key OR managed Moor gateway)
    # ------------------------------------------------------------------

    def _get_config_or_none(self, *, refresh_token: bool = True) -> Optional[Dict[str, Any]]:
        # Import here to avoid a hard dependency at module-import time —
        # managed_tool_gateway pulls in the Moor auth stack which can be
        ...
        def _managed_config() -> Optional[Dict[str, Any]]:
            ...
            managed = resolve_managed_tool_gateway(
                "browser-use",
                token_reader=None if refresh_token else peek_moor_access_token,
            )
            if managed is None:
                return None
            return {
                "api_key": managed.moor_user_token,
        ...
        # Strict selection: "moor" (or legacy use_gateway: true) → managed
    ...
    def create_session(self, task_id: str) -> Dict[str, object]:
        ...
        # default to a long Browser-Use timeout when Moor only needs a task-
    ...
    def get_setup_schema(self) -> Optional[Dict[str, Any]]:
        # Hidden from the moor tools picker: the "Browser Use" row now
        # activates the CLI-based backend (tools/browser_use_cli.py). This
        # provider stays registered for the Moor gateway path and un-migrated
