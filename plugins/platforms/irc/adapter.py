...
class IRCAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config, **kwargs):
        ...
        try:
            ...
            self.port = 6697
        self.nickname = os.getenv("IRC_NICKNAME") or extra.get("nickname", "moor-bot")
    ...
    async def _handle_line(self, raw: str) -> None:
        ...
        if command == "433":
            # Retry with incrementing suffix: moor_, hermes_1, hermes_2...
...
def check_requirements() -> bool:
    ...
    # moor setup / requirements check path.
...
def interactive_setup() -> None:
    """Interactive `moor gateway setup` flow for the IRC platform.

    Lazy-imports ``moor_cli.setup`` helpers so the plugin stays importable
    ...
    channel = prompt(
        "Channel to join (e.g. #moor — comma-separate for multiple)",
...
async def _standalone_send(
    ...
    *,
    ...
    nickname = os.getenv("IRC_NICKNAME") or extra.get("nickname", "moor-bot")
    ...
    nick_base = nickname.rstrip("_0123456789-")[:24] or "moor-bot"
