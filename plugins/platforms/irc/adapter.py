"""IRC Platform Adapter for Moor Agent — stdlib asyncio only, zero external dependencies.

config.yaml ``gateway.platforms.irc.extra`` keys: server, port (6697), nickname (moor-bot), channel,
...
class IRCAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config, **kwargs):
        ...
        self.nickname = _env_or_extra(extra, "IRC_NICKNAME", "nickname", "moor-bot")
    ...
    async def _handle_line(self, raw: str) -> None:
        ...
        elif command == "433":  # ERR_NICKNAMEINUSE — retry: moor_, hermes_1, hermes_2...
...
def interactive_setup() -> None:
    ...
    for label, env, what in (("Bot nickname (e.g. moor-bot)", "IRC_NICKNAME", "Nickname"),
                             ("Channel to join (e.g. #moor — comma-separate for multiple)", "IRC_CHANNEL", "Channel")):
    ...
    print()
    print_success("IRC configuration saved to ~/.moor/.env")
...
async def _sa_register(conn: _StandaloneConn, nick_base: str, server_password: str) -> Optional[Dict[str, Any]]:
    ...
    await conn.raw(f"NICK {standalone_nick}")
    await conn.raw(f"USER {standalone_nick} 0 * :Moor Agent (cron)")
...
async def _standalone_send(pconfig, chat_id: str, message: str, *, thread_id: Optional[str] = None,
    ...
    nick_base = _env_or_extra(extra, "IRC_NICKNAME", "nickname", "moor-bot").rstrip("_0123456789-")[:24] or "moor-bot"
