...
class LineAdapter(BasePlatformAdapter):
    ...
    async def _handle_media(self, request) -> Any:
        ...
        path is rechecked against allowed roots (tempdir, ``/tmp``→``/private/tmp`` on macOS, MOOR_HOME).
        ...
        try:
            ...
            moor_home = Path(get_moor_home()).resolve()
        except Exception:
            moor_home = Path.home().joinpath(".moor").resolve()
        resolved = path.resolve()
        if not any(resolved.is_relative_to(r) for r in (Path(tempfile.gettempdir()).resolve(), Path("/tmp").resolve(), moor_home)):
...
def is_connected(config) -> bool:
    """Surface in ``moor status`` even before the adapter is instantiated."""
    return validate_config(config)


def _env_enablement() -> Optional[Dict[str, Any]]:
    """Seed PlatformConfig.extra from env-only setups so ``moor status`` sees them."""
    if not _env_credentials_present():
        return None
    seeded: Dict[str, Any] = {}
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``moor setup line`` (writes ``~/.moor/.env``)."""
    print("\nLINE Messaging API setup\n------------------------\n"
          "Create a Messaging API channel at https://developers.line.biz/console/\nthen copy the values below.\n")
    try:
        from moor_cli.config import get_env_value as _get_env, save_env_value as _set_env
    except ImportError:
        print("moor_cli.config not available; set LINE_* vars manually in ~/.moor/.env")
        return
