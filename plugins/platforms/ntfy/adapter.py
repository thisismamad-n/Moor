"""ntfy platform adapter (Moor plugin).
...
``plugins/platforms/ntfy/``. The Moor plugin loader scans the
    ...
    platforms:
      ntfy:
        ...
        extra:
          ...
          topic: "moor-in"              # subscribe topic (incoming)
          publish_topic: "moor-out"     # optional — defaults to topic
...
_ECHO_TAG = "moor-agent"  # tag added to outgoing messages for echo-loop prevention
...
async def _standalone_send(
    ...
    *,
    ...
    ``moor cron`` running standalone). Without this hook,
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="pip install httpx   # already a Moor dependency",
        # Env-driven auto-configuration: seeds PlatformConfig.extra so
        # env-only setups show up in `moor gateway status` without
