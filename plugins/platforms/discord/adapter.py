...
_NATIVE_SLASH_COMMANDS: tuple = (
    ...
    ("reset", "Reset your Moor session", (), "/reset", "Session reset~"),
    ...
    ("status", "Show Moor session status", (), "/status", "Status sent~"),
    ("sethome", "Set this chat as the home channel", (), "/sethome", None),
    ("stop", "Stop the running Moor agent", (), "/stop", "Stop requested~"),
    ...
    ("reload-skills", "Re-scan ~/.moor/skills/ for new or removed skills", (), "/reload-skills", None),
    ("voice", "Toggle voice reply mode",
     ...
     "/voice {mode}", None),
    ("update", "Update Moor Agent to the latest version", (), "/update", "Update initiated~"),
    ("restart", "Gracefully restart the Moor gateway", (), "/restart", "Restart requested~"),
    ...
    ("thread", "Create a new thread and start a Moor session in it", (), None, None),
...
def _discord_ready_timeout_seconds() -> float:
    ...
    raw = os.getenv("MOOR_GATEWAY_PLATFORM_CONNECT_TIMEOUT", "").strip()
    if raw:
        try:
            return max(0.0, float(raw))
        except ValueError:
            logger.warning("Ignoring invalid MOOR_GATEWAY_PLATFORM_CONNECT_TIMEOUT=%r", raw)
    return 30.0
...
class DiscordAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._liveness_interval_seconds = self._finite_positive_config_float(
            ...
            env_key="MOOR_DISCORD_LIVENESS_INTERVAL_SECONDS",
        )
        self._liveness_failure_threshold = self._config_int(
            "websocket_liveness_failure_threshold", 2,
            env_key="MOOR_DISCORD_LIVENESS_FAILURE_THRESHOLD",
        )
        ...
        self._discord_recovery_store = DiscordRecoveryStore(get_moor_home())
    ...
    def _command_sync_state_path(self) -> _Path:
        from moor_constants import get_moor_home
        directory = get_moor_home() / _DISCORD_COMMAND_SYNC_STATE_SUBDIR
        try:
    ...
    def _is_down_notice_content(self, content: str) -> bool:
        ...
        subject = r"(?:moor|the agent|agent|the gateway|gateway|bmo)"
    ...
    def _canonicalize_app_command_payload(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        """Reduce command payloads to the semantic fields Moor manages."""
    ...
    async def play_ack_in_voice(self, guild_id: int, phrase: Optional[str] = None) -> bool:
        ...
        audio_path = os.path.join(
            tempfile.gettempdir(), "moor_voice", f"ack_{_uuid.uuid4().hex[:12]}.mp3",
        )
    ...
    def _is_pairing_approved_user(self, user_id: str) -> bool:
        """True when the Discord user has an explicit Moor pairing grant."""
    ...
    def _register_thread_slash(self, tree, name: str, description: str) -> None:
        ...
        @discord.app_commands.describe(
            name="Thread name", message="Optional first message to send to Moor in the thread",
    ...
    def _register_slash_commands(self) -> None:
        ...
        def _auto_register(name: str, description: str, args_hint: str) -> None:
            ...
            try:
                ...
                pass
        try:
            from moor_cli.commands import COMMAND_REGISTRY, _is_gateway_available, _resolve_config_gates
            try:
        ...
        try:
            from moor_cli.commands import _iter_plugin_command_entries
    ...
    def _register_skill_group(self, tree) -> None:
        ...
        try:
            ...
            cmd = discord.app_commands.Command(
                name="skill", description="Run a Moor skill", callback=_skill_handler,
            )
    ...
    def _refresh_skill_catalog_state(self) -> None:
        ...
        from moor_cli.commands_platforms import discord_skill_commands_by_category
    ...
    async def _create_thread(
        ...
        try:
            ...
            try:
                seed_content = starter_message or f"\U0001f9f5 Thread created by Moor: **{name}**"
    ...
    def _derive_auto_thread_name(self, content: str) -> str:
        ...
        Real semantic naming is done after the first agent turn, when Moor has an LLM-generated session
        ...
        thread_name = content[:80] if content else "Moor"
        if len(content) > 80:
    ...
    def _stamp_auto_thread_name(thread: Any, thread_name: str) -> Any:
        ...
        try:
            setattr(thread, "_moor_auto_thread_initial_name", thread_name)
        except Exception:
            pass
        return thread
            ...
            try:
                ...
                try:
                    seed_msg = await message.channel.send(
                        f"\U0001f9f5 Thread created by Moor: **{thread_name}**"
                    )
    ...
    async def rename_thread(
        ...
        if edit is None:
            return False
        try:
            await edit(name=cleaned, reason="Moor semantic session title")
            logger.info(
    ...
    async def create_handoff_thread(self, parent_chat_id: str, name: str) -> Optional[str]:
        ...
        try:
            ...
            if send is None:
                return None
            seed_msg = await send(f"\U0001f9f5 Moor handoff: **{thread_name}**")
            thread = await seed_msg.create_thread(
    ...
    async def send_clarify(
        ...
        def _build(_channel):
            ...
            content = self._self_contained_prompt_content(
                "❓ **Moor needs your input**", str(question or "").strip(), tail=f"\n\n{hint}",
            )
    ...
    async def send_update_prompt(
        ...
        """Yes/No prompt for the gateway ``/update`` watcher when ``moor update --gateway`` needs input."""
        def _build(_channel):
    ...
    async def _handle_message(
        ...
        source = self.build_source(
            ...
            auto_thread_initial_name=(
                getattr(auto_threaded_channel, "_moor_auto_thread_initial_name", None)
...
def _define_discord_view_classes() -> None:
    ...
    class _MoorView(discord.ui.View):
        """Shared plumbing for Moor component views: allowlist auth, single-use
    ...
    class ExecApprovalView(_MoorView):
    ...
    class SlashConfirmView(_MoorView):
    ...
    class UpdatePromptView(_MoorView):
        """Yes/No buttons for ``moor update`` prompts; the answer is written to
        ...
        async def _respond(self, interaction: discord.Interaction, answer: str, color: discord.Color, label: str):
            ...
            try:
                from moor_constants import get_moor_home
                response_path = get_moor_home() / ".update_response"
    ...
    class ModelPickerView(_MoorView):
    ...
    class ChoicePickerView(_MoorView):
    ...
    class ClarifyChoiceView(_MoorView):
...
# process (e.g. standalone ``moor cron``); same forum/thread/multipart logic via Discord REST.
...
_YAML_WEBSOCKET_LIVENESS_KEYS = (
    ("websocket_liveness_interval_seconds", "liveness_interval_seconds", "MOOR_DISCORD_LIVENESS_INTERVAL_SECONDS"),
    ("websocket_liveness_failure_threshold", "liveness_failure_threshold", "MOOR_DISCORD_LIVENESS_FAILURE_THRESHOLD"),
...
def _is_connected(config) -> bool:
    ...
    Looks up ``moor_cli.gateway.get_env_value`` at call time so tests can patch it (ambient env)."""
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="Run `moor setup` to install Discord support.",
