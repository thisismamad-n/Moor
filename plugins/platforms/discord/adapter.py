...
def _discord_ready_timeout_seconds() -> float:
    ...
    if raw:
        try:
            ...
            logger.warning(
                "Ignoring invalid MOOR_GATEWAY_PLATFORM_CONNECT_TIMEOUT=%r",
                raw,
            )
    return 30.0
...
def _read_dm_role_auth_guild() -> Optional[int]:
    ...
    policy, ``~/.moor/.env`` is for secrets only, and this is a
...
class DiscordAdapter(BasePlatformAdapter):
    ...
    def _handle_bot_task_done(self, task: asyncio.Task) -> None:
        ...
        marked running, the Discord websocket is dead while the Moor gateway
    ...
    async def cancel_background_tasks(self) -> None:
        ...
        disconnect budget (``MOOR_GATEWAY_ADAPTER_DISCONNECT_TIMEOUT``, default
    ...
    def _register_slash_commands(self) -> None:
        ...
        @tree.command(name="reset", description="Reset your Moor session")
        ...
        @tree.command(name="status", description="Show Moor session status")
        ...
        @tree.command(name="stop", description="Stop the running Moor agent")
        ...
        @tree.command(name="reload-skills", description="Re-scan ~/.moor/skills/ for new or removed skills")
        ...
        @tree.command(name="update", description="Update Moor Agent to the latest version")
        async def slash_update(interaction: discord.Interaction):
            await self._run_simple_slash(interaction, "/update", "Update initiated~")

        @tree.command(name="restart", description="Gracefully restart the Moor gateway")
        ...
        @tree.command(name="thread", description="Create a new thread and start a Moor session in it")
        @discord.app_commands.describe(
            name="Thread name",
            message="Optional first message to send to Moor in the thread",
        ...
        # moor_cli/commands.py automatically appear as Discord slash
    ...
    def _refresh_skill_catalog_state(self) -> None:
        ...
        """
        from moor_cli.commands import discord_skill_commands_by_category
    ...
    async def _handle_thread_create_slash(
        ...
        # If a message was provided, kick off a new Moor session in the thread
    ...
    def _derive_auto_thread_name(self, content: str) -> str:
        ...
        Moor has an LLM-generated session title and can safely rename
    ...
    async def send_clarify(
        ...
        try:
            ...
            content = self._self_contained_prompt_content(
                "❓ **Moor needs your input**", str(question or "").strip(),
                tail=clarify_tail,
            )
    ...
    async def send_update_prompt(
        ...
        Used by the gateway ``/update`` watcher when ``moor update --gateway``
...
def _component_check_auth(
    ...
    the pairing store (``moor pairing approve``).
    ...
    # so users approved via ``moor pairing approve`` can interact with
...
def _define_discord_view_classes() -> None:
    ...
    class UpdatePromptView(discord.ui.View):
        """Interactive Yes/No buttons for ``moor update`` prompts.
    ...
    class ModelPickerView(discord.ui.View):
        ...
        def _build_model_select(self, provider_slug: str):
            ...
            truncating at 25. This matters for providers like Moor whose
        ...
        async def _on_provider_selected(self, interaction: discord.Interaction):
            ...
            # and silently dropped the tail (e.g. Moor `:free` Portal picks).
...
# is not in this process (e.g. ``moor cron`` running standalone) and no live
...
def _apply_yaml_config(yaml_cfg: dict, discord_cfg: dict) -> dict | None:
    ...
    _websocket_liveness_keys = (
        (
            ...
            "MOOR_DISCORD_LIVENESS_INTERVAL_SECONDS",
        ),
        (
            "websocket_liveness_failure_threshold",
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        # moor_cli/setup.py::_setup_discord function.  Same shape as Teams.
