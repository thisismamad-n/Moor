...
# Uses get_moor_home() so each profile gets its own Matrix store.
from moor_constants import get_moor_dir as _get_moor_dir

_STORE_DIR = _get_moor_dir("platforms/matrix/store", "matrix/store")
...
class MatrixAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._text_batch_delay_seconds = float(
            os.getenv("MOOR_MATRIX_TEXT_BATCH_DELAY_SECONDS", "0.6")
        )
        self._text_batch_split_delay_seconds = float(
            os.getenv("MOOR_MATRIX_TEXT_BATCH_SPLIT_DELAY_SECONDS", "2.0")
        )
...
# + _PLATFORMS["matrix"] static dict in moor_cli/{setup,gateway}.py, and the
...
def interactive_setup() -> None:
    """Configure Matrix credentials. Replaces moor_cli/setup.py::_setup_matrix
...
def _is_connected(config) -> bool:
    ...
    configured. Read via moor_cli.gateway.get_env_value so setup-status
