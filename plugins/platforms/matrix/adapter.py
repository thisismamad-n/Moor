...
from moor_constants import get_moor_dir as _get_moor_dir
...
class MatrixAdapter(BasePlatformAdapter):
    ...
    typed_command_prefix = "!"  # clients reserve typed "/" for local commands; "!command" always reaches Moor
    ...
    def _resolve_store_dir(self) -> Path:
        ...
        self._store_dir = _get_moor_dir("platforms/matrix/store", "matrix/store")
        return self._store_dir

    @property
    def _crypto_db_path(self) -> Path:
        return (self._store_dir or _get_moor_dir("platforms/matrix/store", "matrix/store")) / "crypto.db"
        ...
        if self._access_token:
            ...
            try:
                resp = await client.login(
                    identifier=self._user_id, password=self._password, device_name="Moor Agent",
    ...
    async def _connect_setup_e2ee(self, client: Any, api: Any, state_store: Any) -> bool:
        ...
        try:
            ...
            _acct_id = self._user_id or "moor"
    ...
    def _strip_mention(self, body: str) -> str:
        ...
        words, or "Moor Agent" would become "Agent"."""
        if not body:
            return ""
        if self._user_id:
...
def interactive_setup() -> None:
    ...
    if token or get_env_value("MATRIX_PASSWORD"):
        ...
        for line in ("📬 Home Room: where Moor delivers cron job results and notifications.",
...
def _is_connected(config) -> bool:
    """Connected = homeserver + token (or password). Reads via moor_cli.gateway.get_env_value so
