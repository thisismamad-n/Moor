...
# ``model_tools`` import time, so ``moor status``, ``moor chat``, etc.
...
# ``moor_plugins.platforms__google_chat.adapter`` once the plugin
...
#     fresh "Moor is thinking…" card during the small window between
...
def _mime_for_message_type(mime: str) -> MessageType:
    """Map a MIME string to a moor MessageType.
...
def card_spec_to_cards_v2(card_spec: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {"cardId": str(card_spec.get("card_id") or "moor-card"), "card": card}
...
class GoogleChatAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Persisted to ${MOOR_HOME}/google_chat_thread_counts.json so
        ...
        try:
            ...
            _moor_home = _get_moor_home()
        except (ModuleNotFoundError, ImportError):
            _moor_home = _Path.home() / ".moor"
        self._thread_count_store = _ThreadCountStore(
            _moor_home / "google_chat_thread_counts.json"
        )
        ...
        # they don't sit in the chat forever as "Moor is thinking…".
    ...
    def _bot_id_cache_path(self) -> _Path:
        ...
        base = os.getenv("MOOR_HOME", str(_Path.home() / ".moor"))
    ...
    async def _dispatch_message(self, msg: Dict[str, Any], envelope: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if text.startswith("/setup-files") and event.source is not None:
                ...
                # ${MOOR_HOME}/google_chat_user_tokens/<sanitized>.json
    ...
    async def _build_message_event(
        ...
        """Parse a Chat API message into a moor MessageEvent."""
        space = envelope.get("space") or msg.get("space") or {}
    ...
    async def send_typing(self, chat_id: str, metadata: Any = None) -> None:
        """Post a visible 'Moor is thinking…' marker message.
        ...
        "Moor is thinking…" stuck in chat forever, plus one card that
    ...
    async def stop_typing(self, chat_id: str) -> None:
        ...
        Google Chat has no separate typing API: the "Moor is thinking…"
    ...
    async def on_processing_complete(
        ...
        see "Moor is thinking…" stuck forever.
        """
    ...
    async def _consume_typing_card_with_text(
        ...
        creating a fresh "Moor is thinking…" card during any subsequent
...
def interactive_setup() -> None:
    """Walk the user through Google Chat configuration via ``moor setup``.

    The setup wizard at ``moor_cli/gateway.py`` calls this for plugin
    ...
    prompt for env vars, persist them to ``~/.moor/.env`` so the next
    ...
    print_info("  3. Create a Pub/Sub topic (e.g. moor-chat-events) and a Pull subscription.")
    ...
    print()
    print_success("Google Chat configuration saved to ~/.moor/.env")
    print_info("Restart the gateway: moor gateway restart")
...
async def _standalone_send(
    ...
    *,
    ...
    runner is not in this process (e.g. ``moor cron`` running as a
    separate process from ``moor gateway``).  Without this hook,
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="Run `moor setup` to install Google Chat support.",
        ...
        # the "Moor is thinking..." marker patches and edit overhead.
        ...
        platform_hint=(
            ...
            "are generating a response, a 'Moor is thinking…' marker message "
