...
class GoogleChatAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._thread_count_store = _ThreadCountStore(_moor_home / "google_chat_thread_counts.json")
    ...
    def _bot_id_cache_path(self) -> _Path:
        ...
        from moor_constants import get_moor_home as _get_moor_home
        return _get_moor_home() / "google_chat_bot_id.json"
    ...
    async def send_clarify(
        ...
        def _button(text: str, choice: str) -> Dict[str, Any]:
            return {"text": text, "action": "moor_clarify", "parameters": {"clarify_id": clarify_id, "choice": choice}}
    ...
    async def send_typing(self, chat_id: str, metadata: Any = None) -> None:
        """Post a visible 'Moor is thinking…' marker (Chat has no typing API); ``send()``
        ...
        body = _thread_body(getattr(self.config, "typing_status_text", None) or "Moor is thinking…", thread_id)
...
Walkthrough:
  ...
  3. Create a Pub/Sub topic (e.g. moor-chat-events) and a Pull subscription.
...
async def _standalone_send(
    ...
    gateway runner is not in-process, e.g. ``moor cron``). Needs SA credentials and a
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        platform_hint=(
            ...
            "confirmations instead. While you are generating a response, a 'Moor is thinking…' marker message "
