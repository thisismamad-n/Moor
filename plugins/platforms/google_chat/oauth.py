...
- Per-user tokens (keyed by sender email):
    ``${MOOR_HOME}/google_chat_user_tokens/<sanitized_email>.json``
- Legacy single-user token (fallback, untouched for backward compat):
    ``${MOOR_HOME}/google_chat_user_token.json``
- Per-user pending OAuth state during /setup-files start → exchange:
    ``${MOOR_HOME}/google_chat_user_oauth_pending/<sanitized_email>.json``
- Legacy pending state:
    ``${MOOR_HOME}/google_chat_user_oauth_pending.json``
- OAuth client secret (profile-scoped — each profile registers its own):
    ``${MOOR_HOME}/google_chat_user_client_secret.json``
"""
...
# Use the project's MOOR_HOME helper so the token follows the user's
# profile (e.g. tests can override via MOOR_HOME=/tmp/...).
try:
    from moor_constants import display_moor_home, get_moor_home
except (ModuleNotFoundError, ImportError):
    # Fallback for environments where moor_constants isn't importable
...
def _moor_home() -> Path:
    """Resolve MOOR_HOME at call time (NOT module import).

    Tests and ``MOOR_HOME=...`` env overrides need this to be late-
...
# ``ls ~/.moor/google_chat_user_tokens/`` trivial.
...
def _legacy_token_path() -> Path:
    return _moor_home() / "google_chat_user_token.json"


def _user_tokens_dir() -> Path:
    return _moor_home() / "google_chat_user_tokens"


def _legacy_pending_path() -> Path:
    return _moor_home() / "google_chat_user_oauth_pending.json"


def _user_pending_dir() -> Path:
    return _moor_home() / "google_chat_user_oauth_pending"
...
def _client_secret_path() -> Path:
    return _moor_home() / "google_chat_user_client_secret.json"
...
def load_user_credentials(email: Optional[str] = None) -> Optional[Any]:
    ...
    try:
        ...
        logger.warning(
            ...
            "attachment delivery is disabled. Run `moor setup` to install Google Chat support."
        )
        return None
...
def store_client_secret(path: str) -> None:
    """Validate and copy the user's OAuth client_secret.json into MOOR_HOME."""
...
def exchange_auth_code(code: str, email: Optional[str] = None) -> None:
    ...
    rel_label = (
        f"{display_moor_home()}/google_chat_user_tokens/{_sanitize_email(email)}.json"
        if email
        else f"{display_moor_home()}/google_chat_user_token.json"
    )
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Google Chat user-OAuth setup for Moor (native attachment delivery)"
    )
