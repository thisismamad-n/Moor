...
def _make_activity_event(*, hook_event_name: str, session_id: Any, status: str = "ok", tool_name: Any = None,
    ...
    event: Dict[str, Any] = {"schema": ACTIVITY_EVENT_SCHEMA, "eventId": f"moor-{uuid.uuid4()}",
...
class RaftAdapter(BasePlatformAdapter):
    ...
    async def _handle_wake(self, request: "web.Request") -> "web.Response":
        ...
        # No payload["schema"] gate: the bridge owns schema evolution; Moor only checks content-free.
...
def interactive_setup() -> None:
    ...
    from moor_cli.cli_output import print_header, print_info, print_success, print_warning, prompt, prompt_yes_no
