...
_DEFAULT_DESCRIPTION = "Moor Agent — a general-purpose agent reachable over A2A."
...
class A2AAdapter(BasePlatformAdapter):
    ...
    def _load_served_agents(self, extra: dict) -> dict[str, dict]:
        ...
        for key, val in items:
            ...
            agents[slug] = {
                ...
                "name": str(val.get("name") or f"Moor {slug}"),
                "description": str(val.get("description") or f"Moor profile '{profile or slug}' exposed over A2A."),
    ...
    def _forward_to_profile(self, agent: dict, peer: str, context_id: str, framed_text: str) -> tuple[str, str]:
        """Forward a routed task to another local profile via ``moor chat``. First contact creates a
        ...
        with self._forward_lock(key):
            ...
            cmd = ["moor", "chat", "-q", framed_text, "-Q", "--source", "a2a"] + (["--resume", session_id] if session_id else [])
            env = {**os.environ, "MOOR_A2A_PEER": peer}
