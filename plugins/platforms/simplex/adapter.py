"""SimpleX Chat platform adapter (Moor plugin).
...
This adapter ships as a Moor platform plugin under
``plugins/platforms/simplex/``. The Moor plugin loader scans the
...
discoverable and ``moor setup`` can describe it even when websockets is
...
# Imported at module top because they're stdlib-only inside Moor — no
...
class SimplexAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig, **kwargs):
        ...
        self._text_batch_delay = float(
            os.getenv("MOOR_SIMPLEX_TEXT_BATCH_DELAY", "0.8")
        )
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``moor setup gateway`` → SimpleX.

    Prompts for the WebSocket URL and the optional allowlist / groups /
    auto-accept / home channel. Writes to ``~/.moor/.env`` via
    ``moor_cli.config``.
    """
    print()
    ...
    try:
        ...
        print(
            "moor_cli.config not available; set SIMPLEX_* vars manually in "
            "~/.moor/.env"
        )
        return
