...
def _is_allowed_bridge_path(url: str) -> bool:
    """Absolute bridge path resolves (symlinks included) inside a Moor cache dir — a rogue bridge could hand back /etc/passwd."""
    try:
...
class WhatsAppAdapter(WhatsAppBehaviorMixin, BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._session_path = Path(extra.get("session_path", get_moor_dir("platforms/whatsapp/session", "whatsapp/session")))
    ...
    def _preflight(self) -> bool:
        ...
        checks = (
            (check_whatsapp_requirements, ("[%s] Node.js not found. WhatsApp requires Node.js.", self.name),
             "whatsapp_node_missing", "Node.js is not installed — install Node.js and re-run `moor gateway`."),
            ...
            (creds_path.exists, ("[%s] WhatsApp is enabled but not paired (no creds.json at %s). Pair from the dashboard or run "
                                 "`moor whatsapp`; remove WHATSAPP_ENABLED from your .env to disable.", self.name, creds_path),
             "whatsapp_not_paired", "WhatsApp enabled but not paired — pair from the dashboard or run `moor whatsapp`."),
        )
...
# ── Plugin glue: register(ctx) plus the hooks for gateway/run.py, gateway/config.py, moor_cli/gateway.py, send_message_tool.py.
...
def interactive_setup() -> None:
    ...
    from moor_cli.cli_output import prompt, prompt_yes_no, print_header, print_info, print_success
...
def _is_connected(config) -> bool:
    ...
    if config is not None and getattr(config, "enabled", False) and (getattr(config, "extra", {}) or {}):
        return True
    # Via moor_cli.gateway.get_env_value (not os.getenv) so setup-status callers that patch it observe the same value.
