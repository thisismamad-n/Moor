...
# Moor monkey-patches ``websockets.connect`` on the shared module to inject ping settings.
...
# (``loop``), and Moor also monkey-patches ``websockets.connect`` on the shared ``websockets`` module to
...
class FeishuAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._dedup_state_path = get_moor_home() / "feishu_seen_message_ids.json"
    ...
    def _load_settings(extra: Dict[str, Any]) -> FeishuAdapterSettings:
        ...
        return FeishuAdapterSettings(
            ...
            dedup_cache_size=max(32, env_int("MOOR_FEISHU_DEDUP_CACHE_SIZE", _DEFAULT_DEDUP_CACHE_SIZE)),
            text_batch_delay_seconds=env_float("MOOR_FEISHU_TEXT_BATCH_DELAY_SECONDS", _DEFAULT_TEXT_BATCH_DELAY_SECONDS),
            text_batch_split_delay_seconds=env_float("MOOR_FEISHU_TEXT_BATCH_SPLIT_DELAY_SECONDS", 2.0),
            text_batch_max_messages=max(1, env_int("MOOR_FEISHU_TEXT_BATCH_MAX_MESSAGES", _DEFAULT_TEXT_BATCH_MAX_MESSAGES)),
            text_batch_max_chars=max(1, env_int("MOOR_FEISHU_TEXT_BATCH_MAX_CHARS", _DEFAULT_TEXT_BATCH_MAX_CHARS)),
            media_batch_delay_seconds=env_float("MOOR_FEISHU_MEDIA_BATCH_DELAY_SECONDS", _DEFAULT_MEDIA_BATCH_DELAY_SECONDS),
    ...
    def _get_sdk_executor(self) -> concurrent.futures.ThreadPoolExecutor:
        ...
        with lock:
            ...
            if executor is None or getattr(executor, "_shutdown", False):
                executor = concurrent.futures.ThreadPoolExecutor(max_workers=10, thread_name_prefix="moor-feishu-sdk")
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        try:
            ...
            if not acquired:
                ...
                message = (
                    "Another local Moor gateway is already using this Feishu app_id"
    ...
    async def send_exec_approval(
        ...
        try:
            ...
            def _btn(label: str, action_name: str, btn_type: str = "default") -> dict:
                return _card_button(label, btn_type, {"moor_action": action_name, "approval_id": approval_id})
    ...
    def _build_update_prompt_card(*, prompt: str, default: str, prompt_id: int) -> Dict[str, Any]:
        ...
        def _btn(label: str, answer: str, btn_type: str) -> dict:
            return _card_button(label, btn_type, {"moor_update_prompt_action": answer, "update_prompt_id": prompt_id})
    ...
    def _on_message_read_event(self, data: P2ImMessageMessageReadV1) -> None:
        """Ignore read-receipt events that Moor does not act on."""
    ...
    def _on_card_action_trigger(self, data: Any) -> Any:
        ...
        if isinstance(action_value, dict):
            if action_value.get("moor_action"):
                return self._handle_approval_card_action(event=event, action_value=action_value, loop=loop)
            if action_value.get("moor_update_prompt_action"):
    ...
    def _handle_approval_card_action(self, *, event: Any, action_value: Dict[str, Any], loop: Any) -> Any:
        ...
        if not state:
            ...
            return self._card_response()
        choice = _APPROVAL_CHOICE_MAP.get(action_value.get("moor_action"), "deny")
    ...
    def _handle_update_prompt_card_action(self, *, event: Any, action_value: Dict[str, Any], loop: Any) -> Any:
        ...
        if not state:
            ...
            return self._card_response()
        answer = str(action_value.get("moor_update_prompt_action", "") or "").strip().lower()
    ...
    async def _handle_webhook_request(self, request: Any) -> Any:
        ...
        if payload.get("encrypt"):
            logger.error("[Feishu] Encrypted webhook payloads are not supported by Moor webhook mode")
...
def _begin_registration(domain: str = "feishu") -> dict:
    ...
    qr_url += ("&" if "?" in qr_url else "?") + "from=moor&tp=moor"
    return {
...
# static dict in moor_cli/gateway.py, and the _send_feishu dispatch in tools/send_message_tool.py).
# ──────────────────────────────────────────────────────────────────────────
...
async def _standalone_send(pconfig, chat_id, message, *, thread_id=None, media_files=None, force_document=False):
    ...
    if not await asyncio.to_thread(_load_lark_oapi):
        return {"error": "Feishu dependencies not installed. Run `moor setup` to install Feishu support."}
    try:
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="Run `moor setup` to install Feishu support.", setup_fn=interactive_setup,
