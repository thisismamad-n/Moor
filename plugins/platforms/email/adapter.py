"""
Email platform adapter for the Moor gateway.

Allows users to interact with Moor by sending emails.
...
def _send_imap_id(imap: "imaplib.IMAP4") -> None:
    ...
    try:
        ...
        imap.xatom(
            "ID",
            f'("name" "moor-agent" "version" "{_moor_version}" '
            '"vendor" "Moor inc." '
...
class EmailAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # send-helper, and `moor config show` already read. Without the
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        if missing:
            message = (
                ...
                + ", ".join(missing)
                + ". Set it via `moor gateway setup` (env) or platforms.email "
                "in config.yaml."
            )
...
# moor_cli/gateway.py, and the _send_email dispatch in
