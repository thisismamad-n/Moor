...
# identify MoorAgent traffic — matching other Moor outbound surfaces
# that already set ``MoorAgent/<version>`` for platform-partner attribution.
try:
    from moor_cli import __version__ as _MOOR_VERSION
except Exception:
    _MOOR_VERSION = "unknown"
_MOOR_SLACK_USER_AGENT_PREFIX = f"MoorAgent/{_MOOR_VERSION}"
...
class SlackAdapter(BasePlatformAdapter):
        ...
        try:
            ...
            if "im:history" in granted and "mpim:history" not in granted:
                ...
                logger.warning(
                    ...
                    "Regenerating the app from `moor slack` produces a "
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        if not raw_token:
            logger.error(
                ...
                "error; set SLACK_BOT_TOKEN via `moor gateway setup` "
                "or in the active profile's ~/.moor/.env file, then restart "
                "the gateway.",
            )
            self._set_fatal_error(
                "missing_slack_bot_token",
                "SLACK_BOT_TOKEN not configured. Use `moor gateway setup` "
                "or add it to your active profile's ~/.moor/.env file, "
        ...
        if not app_token:
            logger.error(
                ...
                "error; set SLACK_APP_TOKEN via `moor gateway setup` "
                "or in the active profile's ~/.moor/.env file, then restart "
                "the gateway.",
            )
            self._set_fatal_error(
                "missing_slack_app_token",
                "SLACK_APP_TOKEN not configured. Use `moor gateway setup` "
                "or add it to your active profile's ~/.moor/.env file, "
                "then restart the gateway.",
                retryable=False,
            )
        ...
        try:
            ...
            primary_client = AsyncWebClient(
                token=primary_token,
                user_agent_prefix=_MOOR_SLACK_USER_AGENT_PREFIX,
            )
            ...
            # /model work directly without /moor prefix). A single regex
            ...
            # manifest (see `moor slack manifest`). In Socket Mode, Slack
            ...
            from moor_cli.commands import slack_native_slashes
            import re as _re
            ...
            if _slash_names:
                ...
                _slash_pattern = _re.compile(r"^/moor$")
            ...
            self._app.action(
                _re.compile(r"^moor_clarify_choice_\d+$")
            ...
            if _allow_bots_cfg != "none":
                logger.info(
                    ...
                    "'moor slack manifest' if unsure), and (b) the other bot's "
    ...
    async def _handle_slack_message(
        ...
        if is_dm:
            ...
            if not thread_ts and self._dm_top_level_threads_as_sessions():
                thread_ts = ts
        elif event.get("_moor_no_thread_response"):
        ...
        try:
            ...
            if not smart_denied and allow_session:
                actions.append({
                    ...
                    "action_id": "moor_approve_session",
                    "value": session_key,
                })
                if allow_permanent:
                    actions.append({
                    ...
                    "elements": [
                        {
                            ...
                            "style": "primary",
                            "action_id": "moor_confirm_once",
                            "value": value,
                        },
                        {
                            "type": "button",
                            "text": {"type": "plain_text", "text": "Always Approve"},
                            "action_id": "moor_confirm_always",
                            "value": value,
                        },
                        {
                            "type": "button",
    ...
    async def send_clarify(
        ...
        (unique ``moor_clarify_choice_<idx>`` action_id, ``value`` packs
        ``clarify_id|idx``) plus a final "✏️ Other…" button
        (``moor_clarify_other``).  A choice click resolves the clarify
    ...
    async def _handle_slash_confirm_action(self, ack, body, action) -> None:
        ...
        choice_map = {
            "moor_confirm_once": "once",
            "moor_confirm_always": "always",
    ...
    async def _handle_approval_action(self, ack, body, action) -> None:
        ...
        choice_map = {
            "moor_approve_once": "once",
            "moor_approve_session": "session",
            "moor_approve_always": "always",
            "moor_deny": "deny",
        }
    ...
    async def _handle_slash_command(self, command: dict) -> None:
        ...
        The legacy ``/moor <subcommand> [args]`` form is preserved for
        backward compatibility with older workspace manifests and for users
        who want a single entry point for free-form questions (``/moor
        ...
        if slash_name in {"moor", ""}:
            # Legacy /moor <subcommand> [args] routing + free-form questions.
            ...
            # ``text.split()`` returns ``[]`` (e.g. user sends ``/moor   ``).
        ...
        # "Running /cmd…" ack shown by handle_moor_command).
        # Only stash for COMMAND events (text starts with "/") — free-form
        # questions via "/moor <question>" must produce public replies so
...
# ``moor_cli/{setup,gateway}.py``, and the ``_send_slack`` dispatch in
...
async def _standalone_send(
    ...
    *,
    ...
    try:
        ...
        _tokens_file = get_moor_home() / "slack_tokens.json"
...
def interactive_setup() -> None:
    ...
    offers to set a home channel. Replaces ``moor_cli/setup.py::_setup_slack``.
    """
    ...
    def _write_slack_manifest_and_instruct() -> None:
        """Generate the Slack manifest, write it under MOOR_HOME, and print
        ...
        try:
            ...
            target = Path(get_moor_home()) / "slack-manifest.json"
...
def _is_connected(config) -> bool:
    ...
    Looks up via ``moor_cli.gateway.get_env_value`` at call time (not via the
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        # Interactive setup wizard — replaces moor_cli/setup.py::_setup_slack
        # and the static _PLATFORMS["slack"] dict in moor_cli/gateway.py.
