...
`moor photon setup` does, in order:
...
3. **Provision the project secret** — mint a fresh project secret (the
   dashboard reveals it only once) and persist it to `~/.moor/.env` so the
...
Run `moor photon status` to see what's configured.

## Credentials

Runtime SDK credentials live in `~/.moor/.env` (the same place every other
...
Management metadata lives in `~/.moor/auth.json` under `credential_pool`:
...
| `PHOTON_TELEMETRY`        | false                      | Spectrum SDK telemetry — toggle with `moor photon telemetry on\|off` (restart the gateway to apply) |
...
- **Cron/standalone sends require a running gateway.** Processes outside
  the gateway (cron subprocesses, `moor send`) cannot spawn the sidecar;
  they authenticate to the gateway's live sidecar via the runtime record at
  `<moor-home>/runtime/photon-sidecar.json` (written after the sidecar's
...
6. Verify end-to-end: `moor photon status`, a DM and a group roundtrip,
