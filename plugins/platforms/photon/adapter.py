...
#
# Out-of-process senders (cron subprocesses, `moor send`, the dashboard)
...
def _runtime_record_path() -> Path:
    # get_moor_home() honors profile overrides — never hardcode ~/.moor.
...
# this module (moor status, test collection, plugin discovery).
...
def sidecar_deps_installed() -> bool:
    ...
    _start_sidecar(), and `moor photon status` so all three agree on
...
def check_requirements() -> bool:
    ...
    if not sidecar_deps_installed():
        ...
        # (load_gateway_config, moor status, GET /api/status polling) —
...
def _sidecar_deps_stale() -> bool:
    ...
    `moor update` rewrites ``package-lock.json`` when the spectrum-ts pin is
...
class PhotonAdapter(BasePlatformAdapter):
    ...
    async def _start_sidecar(self) -> None:
        if not sidecar_deps_installed():
            ...
            # `moor photon setup`, so the connect path must be able to
        ...
        # A `moor update` that bumps the spectrum-ts pin rewrites
            ...
            while time.time() < deadline:
                ...
                try:
                    ...
                    if resp.status_code == 200:
                        ...
                        # (cron, `moor send`) can reach this sidecar
...
async def _standalone_send(
    ...
    *,
    ...
    if not token:
        ...
        # the gateway process env otherwise, so cron/`moor send` would be
        ...
        if not token:
            return {
                "error": (
                    ...
                    "records its address under <moor-home>/runtime/"
...
def register(ctx) -> None:
    ...
    # gateway-setup hook and the `moor photon` CLI command below.
    ...
    # Register CLI subcommands — `moor photon ...`
