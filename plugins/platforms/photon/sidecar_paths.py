...
under an immutable ``/opt/moor`` (read-only for the moor user), which
...
4. Source dir read-only and deps missing or stale → mirror the sidecar
   source files to ``$MOOR_HOME/photon/sidecar`` (the durable data volume,
