"""
``moor photon ...`` CLI subcommands — registered by the plugin via
...
# Writable sidecar runtime dir (mirrors to MOOR_HOME on immutable
...
# wiring imports this module for `moor --help`).
...
# when called later (gateway start, moor status). Cleared on success.
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire up `moor photon ...` subcommands."""
...
def _run_device_login(args: argparse.Namespace) -> int:
    ...
    no standalone ``moor photon login`` command; Photon onboards
...
def _cmd_setup(args: argparse.Namespace) -> int:
    ...
    # 3. Provision Spectrum credentials (runtime -> ~/.moor/.env,
    ...
    try:
        from moor_cli.config import write_platform_config_field
    ...
    print("  Start the gateway:  moor gateway start")
    return 0
...
def _cmd_status(_args: argparse.Namespace) -> int:
    ...
    print(f"  sidecar deps        : {'✓ installed' if sidecar_installed else '✗ run `moor photon install-sidecar`'}")
    print(f"  telemetry           : {'on' if _telemetry_enabled() else 'off'} (`moor photon telemetry on|off`)")
    return 0
...
def _telemetry_enabled() -> bool:
    """Read PHOTON_TELEMETRY from the env / ~/.moor/.env.
...
def _cmd_telemetry(args: argparse.Namespace) -> int:
    ...
    if state is None:
        ...
        print("  Toggle with `moor photon telemetry on` / `moor photon telemetry off`.")
        return 0
    try:
        ...
        return 1
    print(f"✓ Spectrum telemetry turned {state} (PHOTON_TELEMETRY in ~/.moor/.env)")
    print("  Restart the gateway for the sidecar to pick it up:  moor gateway restart")
    return 0
...
#
# `moor gateway setup` discovers platforms via the registry and calls each
...
# project + user + sidecar flow as ``moor photon setup`` with interactive
# defaults (phone is prompted when stdin is a TTY).

def gateway_setup() -> None:
    """Run Photon first-time setup from the `moor gateway setup` wizard."""
