...
Small Node helper that bridges Moor Agent to Photon's Spectrum SDK
(`spectrum-ts`).  Moor is Python; Photon has no public HTTP
...
- exposes a loopback-only HTTP control channel for the Python adapter
  to push send/typing requests (auth via `X-Moor-Sidecar-Token`)
...
The Moor plugin's `hermes photon setup` command runs `npm install`
here automatically.
...
- exposes a loopback-only HTTP control channel for the Python adapter
  to push send/typing requests (auth via `X-Moor-Sidecar-Token`)
- drains the inbound message stream so `spectrum-ts` keeps its
  reconnect/heartbeat machinery alive and Moor can receive inbound messages
...
`Space.send(...)` API. Moor is Python, so replies go through this sidecar
