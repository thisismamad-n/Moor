...
// `message.content.text` before Moor can see it. We rewrite the two inbound
...
const MARKER = "Moor patch: Preserve mixed text + attachment iMessage payloads";
...
export function patchSpectrumTs(root = scriptDir()) {
  ...
  for (const file of files) {
    ...
    // Moor to patch; keep the legacy v8 path below for older pinned installs.
    if (
