  ...
  streamRestartTimer = setTimeout(() => {
    ...
    console.error(
      `photon-sidecar: upstream stream degraded for ${degradedForMs}ms; ` +
        "exiting so Moor can restart the Photon adapter"
    );
    process.exit(75);
...
function inboundStreamErrorMessage(e) {
  ...
  // iMessage internal error. Local Moor allowlists cannot cause or fix this:
  ...
  if (
    ...
    out +=
      ...
      "this is upstream of Moor, so inbound iMessages may not be delivered " +
