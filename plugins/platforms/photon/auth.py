...
Credential storage mirrors every other Moor channel:
    ...
    * runtime SDK creds  -> ``~/.moor/.env``  (``PHOTON_PROJECT_ID`` =
      project id, ``PHOTON_PROJECT_SECRET``) via ``save_env_value``
    * management metadata -> ``~/.moor/auth.json`` under
...
try:
    import httpx
except ImportError:  # pragma: no cover - httpx is a moor dependency
...
# ---------------------------------------------------------------------------
# auth.json helpers — share the file with the rest of moor-agent.

def _auth_json_path() -> Path:
    """Resolve ``~/.moor/auth.json`` honouring the active Moor profile."""
    try:
        from moor_constants import get_moor_home
        return Path(get_moor_home()) / "auth.json"
    except Exception:
        return Path(os.path.expanduser("~/.moor")) / "auth.json"
...
def _save_auth(data: Dict[str, Any]) -> None:
    ...
    # moor_cli/auth.py:_save_auth_store (#19673, #21148).
    fd = os.open(
        str(tmp),
...
def store_photon_token(token: str) -> None:
    ...
    from moor_cli.auth import _auth_store_lock
...
def load_project_credentials() -> Tuple[Optional[str], Optional[str]]:
    ...
    Precedence: process env (``~/.moor/.env`` is loaded into the gateway's
...
def store_project_credentials(
    *,
    ...
    The runtime SDK creds land in ``~/.moor/.env`` via the same
...
def _persist_runtime_env(spectrum_project_id: str, project_secret: str) -> None:
    """Write the SDK creds to ``~/.moor/.env`` (canonical runtime store).
    ...
    try:
        ...
        logger.warning("photon: moor_cli.config unavailable — skipping .env write")
        return
    try:
...
def print_credential_summary(emit: Any = print) -> None:
    ...
    labels["device_token"] = (
        ...
        else "✗ missing (run `moor photon setup`)"
    )
    ...
    labels["phone_number"] = (
        phone if phone else "✗ missing (run `moor photon setup --phone ...`)"
    )
    labels["assigned_phone_number"] = (
        assigned if assigned else "✗ missing (run `moor photon setup`)"
    )
...
def credential_summary() -> Dict[str, str]:
    ...
    def _present_phone() -> str:
        ...
        return phone or "✗ missing (run `moor photon setup --phone ...`)"

    def _present_assigned_phone() -> str:
        _phone, assigned = load_user_numbers()
        return assigned or "✗ missing (run `moor photon setup`)"
