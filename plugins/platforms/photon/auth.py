...
``(id, projectSecret)``. Storage: runtime SDK creds -> ``~/.moor/.env``; management
...
# published CLI client until Moor gets its own client_id.
...
def _auth_json_path() -> Path:
    """``~/.moor/auth.json`` honouring the active Moor profile."""
    try:
...
def _persist_runtime_env(spectrum_project_id: str, project_secret: str) -> None:
    """Write the SDK creds to ``~/.moor/.env`` (secret never bound to a printable local
...
def print_credential_summary(emit: Any = print) -> None:
    ...
    rows = [
        ...
        "──────────────────────",
        "  device token        : " + (
            "✓ stored" if load_photon_token() else "✗ missing (run `moor photon setup`)"),
        "  project id          : " + (sid if sid else "✗ missing"),
        "  project secret      : " + ("✓ stored" if sec else "✗ missing"),
        "  my number           : " + (phone if phone else "✗ missing (run `moor photon setup --phone ...`)"),
        "  assigned number     : " + (assigned if assigned else "✗ missing (run `moor photon setup`)")]
    emit("\n".join(rows))
