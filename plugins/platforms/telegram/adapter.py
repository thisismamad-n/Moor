...
class TelegramAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._media_batch_delay_seconds = env_float("MOOR_TELEGRAM_MEDIA_BATCH_DELAY_SECONDS", 0.8)
    ...
    def _dm_topic_fallback(metadata: Optional[Dict[str, Any]]) -> bool:
        """True for Moor private-chat topic lanes (``telegram_dm_topic_reply_fallback``)."""
    ...
    def _thread_kwargs_for_send(
        ...
        metadata; Moor private-chat topic lanes are marked ``telegram_dm_topic_reply_fallback``. Anchor-less synthetic sends
        prefer the Moor topic's ``message_thread_id`` (the native DM-topic id renders in a different chat lane).
        ...
        restart) prefer the Moor topic's ``message_thread_id`` so they stay in the active topic lane
        (#87051); ``direct_messages_topic_id`` is only used when no topic thread resolves, since the native
        DM-topic id does not match the Moor topic lane and can render the message in a different chat
        lane.
        """
        ...
        if fallback and reply_to_mode != "off":
            ...
            if reply_to_message_id is None:
                # Anchor-less synthetic send: prefer the Moor topic thread id (see docstring).
                # Anchor-less synthetic sends (loop wakeups, watch notifications, restart-resumed
                # follow-ups) must stay in the active topic lane: prefer the Moor topic thread id when it
    ...
    def _prune_stale_dm_topic_binding(self, chat_id: Any, thread_id: Any, *, metadata: Optional[Dict[str, Any]] = None) -> None:
        ...
        Rows are namespaced by profile: the send's ``moor_profile`` wins over the adapter's stamp.
        ...
        binding, so the send's ``moor_profile`` metadata wins over the adapter's own profile stamp;
        ...
        if db is None or not hasattr(db, "delete_telegram_topic_binding"):
            return
        try:
            profile_name = (metadata or {}).get("moor_profile") or getattr(self, "_moor_profile_name", None) or "default"
    ...
    async def _handle_polling_conflict(self, error: Exception) -> None:
        ...
        message = (
            ...
            "or another process is using the same bot token. To recover: ensure no other Moor or OpenClaw instance is running "
            "with this token, then restart the gateway with 'moor gateway restart'."
    ...
    async def _build_ptb_requests(self) -> tuple:
        ...
        request_kwargs = {
            "connection_pool_size": env_int("MOOR_TELEGRAM_HTTP_POOL_SIZE", 512),
            "pool_timeout": env_float("MOOR_TELEGRAM_HTTP_POOL_TIMEOUT", 8.0),
            "connect_timeout": env_float("MOOR_TELEGRAM_HTTP_CONNECT_TIMEOUT", 10.0),
            "read_timeout": env_float("MOOR_TELEGRAM_HTTP_READ_TIMEOUT", 20.0),
            "write_timeout": env_float("MOOR_TELEGRAM_HTTP_WRITE_TIMEOUT", 20.0),
        ...
        disable_fallback = os.getenv("MOOR_TELEGRAM_DISABLE_FALLBACK_IPS", "").strip().lower() in {"1", "true", "yes", "on"}
        fallback_ips = [] if disable_fallback else self._fallback_ips()
        if not fallback_ips and not disable_fallback:
            discovery_timeout = self._env_float_clamped("MOOR_TELEGRAM_FALLBACK_DISCOVERY_TIMEOUT", 5.0, min_value=0.0)
    ...
    async def _initialize_app_with_retries(self, builder) -> None:
        ...
        _max_connect = 8
        _init_timeout = env_float("MOOR_TELEGRAM_INIT_TIMEOUT", 30.0)  # per attempt
            ...
            try:
                if asyncio.get_running_loop().time() >= _total_deadline:
                    raise OSError(
                        ...
                        f"Check network connectivity to api.telegram.org or set MOOR_TELEGRAM_HTTP_CONNECT_TIMEOUT / "
                        f"MOOR_TELEGRAM_INIT_TIMEOUT to a lower value.")
                ...
                if _attempt >= _max_connect - 1:
                    raise OSError(
                        ...
                        f"or set MOOR_TELEGRAM_HTTP_CONNECT_TIMEOUT to a lower value.")
    ...
    def _build_provider_keyboard(self, providers: list, page: int = 0) -> tuple:
        ...
        try:
            from moor_cli.models_catalog_static import group_providers
        except Exception:
    ...
    async def _handle_model_picker_callback(self, query, data: str, chat_id: str) -> None:
        ...
        if data.startswith("mp:"):  # provider selected: show model buttons (page 0)
            ...
            group_id = data[4:]
            try:
                from moor_cli.models_catalog_static import PROVIDER_GROUPS
    ...
    # `gt:<verb>` -> (script in ~/.moor/scripts/gmail-triage/, extra-args, success-label, is_state). The callback
    ...
    async def _handle_gmail_triage_callback(self, query, data: str, cb: Dict[str, Any]) -> None:
        ...
        script_path = _Path.home() / ".moor" / "scripts" / "gmail-triage" / script_name
    ...
    def _message_mentions_bot(self, message: Message) -> bool:
        ...
        # Server-side MessageEntity values are authoritative: raw substrings like "foo@moor_bot.example"
    ...
    def _should_process_message(self, message: Message, *, is_command: bool = False) -> bool:
        ...
        # counted as incoming unread in the Moor inbox (#52363). Otherwise a BotFather rename leaves the
    ...
    def _reload_dm_topics_from_config(self) -> None:
        ...
        try:
            from moor_cli.config import load_config_readonly  # canonical loader: managed overlay + ${VAR}
...
# _PLATFORMS["telegram"] static dict in moor_cli/{setup,gateway}.py, and the _send_telegram dispatch in
...
def _resolve_notifications_mode() -> str:
    ...
    mode = os.getenv("MOOR_TELEGRAM_NOTIFICATIONS", "")
    if not mode:
        try:
...
def interactive_setup() -> None:
    ...
    from moor_cli import setup as _setup_mod
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="Run `moor setup` to install Telegram support.", setup_fn=interactive_setup, apply_yaml_config_fn=_apply_yaml_config,
