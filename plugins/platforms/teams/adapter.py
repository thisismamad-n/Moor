...
``<moor-venv>/bin/pip install microsoft-teams-apps aiohttp``) and credentials via env
...
async def _standalone_send(
    ...
    (``moor cron``). ``TEAMS_SERVICE_URL`` is allowlisted and ``chat_id`` charset-checked
...
def _suppress_third_party_dotenv() -> Iterator[None]:
    ...
    cwd-discovered ``.env`` at import, mutating process-global ``os.environ``. Moor owns dotenv loading.
...
class TeamsAdapter(BasePlatformAdapter):
    ...
    async def _on_card_action(
        ...
        choice = _APPROVAL_CHOICES.get(moor_action)
        if not choice:
            return self._invoke_message("Unknown action.")
    ...
    async def send_exec_approval(
        ...
        def _action(title: str, moor_action: str, **kw) -> "ExecuteAction":
            return ExecuteAction(
                title=title, verb="moor_approve", data={**btn_data_base, "moor_action": moor_action}, **kw)
...
_SETUP_INTRO = (  # "" → blank line
    ...
    '  teams app create --name "Moor" --endpoint "https://<tunnel>/api/messages"', "",
...
def _install_hint() -> str:
    ...
    ``venv_pip=True`` targets the real Moor venv, sidestepping PEP 668 on Ubuntu 24.04."""
    try:
