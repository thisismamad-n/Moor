...
``{title, url, description, position}`` rows every other Moor web
provider produces.
...
prefers Moor-managed xAI Grok OAuth (via ``moor auth``) and falls back
to ``XAI_API_KEY`` (resolved through ``~/.moor/.env``, then
``os.environ``).
"""
...
class XAIWebSearchProvider(WebSearchProvider):
    ...
    def is_available(self) -> bool:
        ...
        every ``moor tools`` repaint and at tool-registration time.
    ...
    def search(self, query: str, limit: int = 5) -> Dict[str, Any]:
        ...
        if not api_key:
            return {
                "success": False,
                "error": (
                    "No xAI credentials found. Run `moor auth` to sign in with "
