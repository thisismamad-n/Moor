...
def _use_keyless_ring() -> bool:
    ...
    managed Moor gateway isn't the selected path, and the keyless tier
    ...
    try:
        if read_selection("web") == MOOR_MANAGED_PROVIDER:
            return False
...
def check_firecrawl_api_key() -> bool:
    """Return True when the Firecrawl backend selected via `moor tools`
...
def _get_firecrawl_client() -> Any:
    ...
    - ``"moor"`` (or legacy ``use_gateway: true``) → managed Tool Gateway
    ...
    functions (``resolve_managed_tool_gateway``, ``_read_moor_access_token``,
    ...
    if selected == MOOR_MANAGED_PROVIDER:
        ...
        if managed is None:
            logger.error(
                "Firecrawl client initialization failed: the Moor "
