"""Firecrawl web search + extract provider (direct SDK, keyless cloud, or Moor tool-gateway).
...
def _use_keyless_ring() -> bool:
    """Route via the keyless ring only with no direct credentials, when the managed Moor
    ...
    if _env("FIRECRAWL_API_KEY") or _env("FIRECRAWL_API_URL"):
        return False
    from tools.tool_backend_helpers import MOOR_MANAGED_PROVIDER, read_selection
    from plugins.web.keyless_mcp import use_keyless
    # Both probes are optional layers: a failing probe never blocks the ring.
    for probe in (lambda: read_selection("web") == MOOR_MANAGED_PROVIDER, lambda: _is_tool_gateway_ready() and not _is_explicit_firecrawl_selection()):
        try:
            if probe():
                return False
...
def _is_tool_gateway_ready() -> bool:
    """True when gateway URL + Moor Subscriber token are available."""
    return _gateway.resolve_managed_tool_gateway("firecrawl", token_reader=_gateway.peek_moor_access_token) is not None


def check_firecrawl_api_key() -> bool:
    """True when the route selected via ``moor tools`` (or, on a never-configured
...
def _firecrawl_backend_help_suffix() -> str:
    return ", or use the Moor Tool Gateway via your subscription (FIRECRAWL_GATEWAY_URL or TOOL_GATEWAY_DOMAIN)" if _backend_helpers.managed_moor_tools_enabled() else ""
...
def _get_firecrawl_client() -> Any:
    ...
    ``"moor"`` → managed Tool Gateway ONLY; any other stored backend → direct Firecrawl ONLY (never a silent
    ...
    def _managed():
        gw = _gateway.resolve_managed_tool_gateway("firecrawl", token_reader=_gateway.read_moor_access_token)
        if gw is None:
            return None
        return "sdk", {"api_key": gw.moor_user_token, "api_url": gw.gateway_origin}, ("tool-gateway", gw.gateway_origin, gw.moor_user_token)
    ...
    def _unconfigured_message() -> str:
        ...
        if _backend_helpers.managed_moor_tools_enabled():
            return message + " With your Moor subscription you can also use the Tool Gateway. run `moor tools` and select Moor Subscription as the web provider."
...
class FirecrawlWebSearchProvider(BaseWebSearchProvider):
    ...
    def get_setup_schema(self) -> Dict[str, Any]:
        return setup_schema(
            ...
            "Full search + extract; supports keyless cloud, direct API, and Moor tool-gateway routing.",
