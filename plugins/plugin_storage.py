...
``<moor home>/plugins/<name>/``. That tree is the plugin *install* dir —
``moor plugins remove`` deletes it and ``moor plugins update`` git-pulls
...
``<moor home>/plugin-data/<name>/``, owned by the user, untouched by
...
through ``agent.secret_scope`` / ``.env`` like everywhere else in Moor.
...
# Mirrors the plugin-name shape `moor plugins install` accepts. Anything
...
def plugin_data_dir(name: str) -> Path:
    ...
    ``<moor home>/plugin-data/<name>/`` — survives plugin update and
    removal, and follows the active profile because it resolves through
    :func:`moor_constants.get_moor_home` on every call. Don't cache the
    result across profile switches.
    """
    from moor_constants import get_moor_home

    root = get_moor_home() / "plugin-data" / _validate_name(name)
