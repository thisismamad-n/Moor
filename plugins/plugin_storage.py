"""Per-plugin persistent storage: ``<moor home>/plugin-data/<name>/``.

Plugins must NOT park state in ``<moor home>/plugins/<name>/`` (the install dir, deleted by
...
# Mirrors the plugin-name shape `moor plugins install` accepts (no separators/traversal).
...
def plugin_data_dir(name: str) -> Path:
    """Return (and create) ``<moor home>/plugin-data/<name>/``; resolves ``get_moor_home()`` on
