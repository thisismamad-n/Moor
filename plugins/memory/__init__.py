...
``$MOOR_HOME/plugins/<name>/``, project ``./.moor/plugins/<name>/`` (opt-in via
MOOR_ENABLE_PROJECT_PLUGINS), then ``moor_agent.memory_providers`` entry points.
...
def _get_project_plugins_dir() -> Optional[Path]:
    """``./.moor/plugins/`` or None. Gated on MOOR_ENABLE_PROJECT_PLUGINS like the
...
def find_provider_dir(name: str) -> Optional[Path]:
    ...
    provider silently loses its dashboard panel and ``moor <provider>`` commands."""
    ...
    try:
        ...
        if cli_mod is None:
            if not _is_bundled(plugin_dir):
                # cli.py imports as _moor_user_memory.<name>.cli, usually before the
