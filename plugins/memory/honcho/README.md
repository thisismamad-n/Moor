...
> **Honcho docs:** <https://docs.honcho.dev/v3/guides/integrations/moor>
...
```bash
moor memory setup honcho   # configure Honcho directly (works on a fresh install)
moor memory setup          # generic picker, choose Honcho from the list
```
...
Or manually:
```bash
moor config set memory.provider honcho
echo "HONCHO_API_KEY=***" >> ~/.moor/.env
```

> `moor honcho setup` also works, but only **after** Honcho is the active
> memory provider — the `honcho` subcommand is registered for the active
> provider only. On a fresh install, use `moor memory setup honcho`.
...
cycle (not per pass). Select a fast, inexpensive model under `moor model`
...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `$MOOR_HOME/honcho.json` | Profile-local (isolated Moor instances) |
| 2 | `~/.moor/honcho.json` | Default profile (shared host blocks) |
| 3 | `~/.honcho/config.json` | Global (cross-app interop) |

Host key is derived from the active Moor profile: `moor` (default) or `moor_<profile>`.
...
| 4 | `per-session` strategy | Moor session ID (`20260415_a3f2b1`) |
| 5 | `per-repo` strategy | Git root directory name (`moor-agent`) |
| 6 | `per-directory` strategy | Current directory basename (`src`) |
| 7 | `global` strategy | Workspace name (`moor`) |

Gateway platforms always resolve via priority 3 (per-chat isolation) regardless of `sessionStrategy`. The strategy setting only affects CLI sessions.

If `sessionPeerPrefix` is `true`, the peer name is prepended: `alice-moor-agent`.

#### What each strategy produces

- **`per-directory`** — basename of `$PWD`. Opening moor in `~/code/myapp` and `~/code/other` gives two separate sessions. Same directory = same session across runs.
...
Multiple Moor profiles can share one workspace while maintaining separate AI identities. Config resolution is **host block > root > env var > default** — host blocks inherit from root, so shared settings only need to be declared once:
...
Both profiles see the same user (`yourname`) in the same shared environment (`moor`), but each AI peer builds its own observations, conclusions, and behavior patterns. The coder's memory stays code-oriented; the main agent's stays broad.

Host key is derived from the active Moor profile: `moor` (default) or `moor_<profile>` (e.g. `moor -p coder` -> host key `moor_coder`). Older `moor.<profile>` host blocks are still read for compatibility and are migrated when the CLI writes profile-scoped Honcho config.
...
| `MOOR_HONCHO_HOST` | Host key override |
...
| `HONCHO_OAUTH_CLIENT_ID` | OAuth client (default `moor-agent`) |
...
|---------|-------------|
| `moor memory setup honcho` | Configure Honcho directly — works on a fresh install |
| `moor honcho setup` | Interactive setup wizard (only registered once Honcho is the active provider; redirects to `moor memory setup`) |
| `moor honcho status` | Show resolved config for active profile |
| `moor honcho enable` / `disable` | Toggle Honcho for active profile |
| `moor honcho mode <mode>` | Change recall or observation mode |
| `moor honcho peer --user <name>` | Update user peer name |
| `moor honcho peer --ai <name>` | Update AI peer name |
| `moor honcho tokens --context <N>` | Set context token budget |
| `moor honcho tokens --dialectic <N>` | Set dialectic max chars |
| `moor honcho map <name>` | Map current directory to a session name |
| `moor honcho sync` | Create host blocks for all Moor profiles |
