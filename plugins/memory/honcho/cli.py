"""``moor honcho`` subcommands: setup wizard, status, peers, sessions, identity, migrate."""
...
_RETRY_HINT = "  Re-run 'moor honcho setup' to retry, or choose an API key instead.\n"
...
def _label(host: str) -> str:
    return f"[{host}] " if host != "moor" else ""
...
def _sync_profiles(verbose: bool) -> int:
    ...
    cfg = _read_config()
    if not cfg:
        return say("  No Honcho config found. Run 'moor honcho setup' first.\n") or 0
    default_block, has_key = _default_block_and_key(cfg)
    if not default_block and not has_key:
        return say("  Honcho not configured on default profile. Run 'moor honcho setup' first.\n") or 0
...
def sync_honcho_profiles_quiet() -> int:
    """Sync host blocks for all profiles from `moor update`; no output, no exceptions."""
...
def _resolve_effective_identity_mapping(cfg: dict, moor_host: dict) -> tuple[bool, dict, str, bool, bool]:
...
def _apply_runtime_prefix(moor_host: dict, current_prefix: str, prefix_from_root: bool, label: str) -> None:
...
def _echo_identity_mapping(moor_host: dict) -> None:
    print(f"  resolved →\n    pinUserPeer       = {bool(moor_host.get('pinUserPeer'))}\n"
          f"    userPeerAliases   = {moor_host.get('userPeerAliases') or '{}'}\n"
          f"    runtimePeerPrefix = {moor_host.get('runtimePeerPrefix') or '(none)'}")


def _configure_raw_identity_mapping(moor_host, current_pin, current_aliases, current_prefix,
    ...
    _apply_runtime_prefix(moor_host, current_prefix, prefix_from_root,
                          "runtimePeerPrefix — namespace for unknown IDs (blank for none)")


def _setup_identity_mapping(cfg: dict, moor_host: dict, current_peer: str) -> None:
    ...
    if shape == "raw":
        _configure_raw_identity_mapping(moor_host, current_pin, current_aliases, current_prefix,
        ...
        if shape == "multi" and aliases_from_root:
            prior_aliases = {}
        _scrub_identity_mapping(moor_host)  # each shape starts from a clean slate
        moor_host["pinUserPeer"] = shape == "single"
        if shape == "single":
...
def _setup_local_auth(cfg: dict, moor_host: dict) -> None:
...
def _setup_device_login(moor_host: dict, write_path: Path, *, open_browser: bool) -> bool:
    ...
    try:
        cred = authorize_via_device_code(
            config_path=write_path, source="moor-cli", apply_config=False, display=_show,
        ...
        print("\n  Device code expired before approval.\n  Re-run 'moor honcho setup' to get a new code.\n")
    except AccessDenied:
...
def _setup_browser_login(moor_host: dict, write_path: Path) -> bool:
    ...
    try:
        cred = authorize_via_loopback(config_path=write_path, source="moor-cli", apply_config=False, open_url=_open)
    except Exception as e:
...
def _setup_cloud_auth(cfg: dict, moor_host: dict, write_path: Path) -> bool:
    ...
    if device_available and method in {"device", "d"}:
        return _setup_device_login(moor_host, write_path, open_browser=can_browse and not is_remote)
    if method in {"oauth", "o"}:
        return _setup_browser_login(moor_host, write_path)
...
def _choice_step(moor_host, key, current, label, valid, fallback=None) -> None:
    """Prompt for one of ``valid``; an invalid answer writes ``fallback`` (None = keep current)."""
    new = _prompt(label, default=current)
    if new in valid:
        moor_host[key] = new
    elif fallback is not None:
        moor_host[key] = fallback


def _setup_tuning(cfg: dict, moor_host: dict) -> None:
    ...
    _choice_step(moor_host, "observationMode", _pref(moor_host, cfg, "observationMode", "directional"),
    ...
    new_wf = _prompt("Write frequency", default=str(_pref(moor_host, cfg, "writeFrequency", "async")))
    moor_host["writeFrequency"] = _first_parsed([new_wf], int, new_wf if new_wf in {"async", "turn", "session"} else "async")
    ...
    _menu("Dialectic cadence",
          ...
          "Recommended: 1-5.")
    new_dialectic = _prompt("Dialectic cadence", default=str(_pref(moor_host, cfg, "dialecticCadence") or "2"))
    ...
    _choice_step(moor_host, "dialecticReasoningLevel", _pref(moor_host, cfg, "dialecticReasoningLevel") or "low",
                 "Reasoning level", REASONING_LEVELS, "low")

    _menu("Session strategy", *(f"{s:<13} -- {desc}" for s, desc in _STRATEGIES.items()))
    _choice_step(moor_host, "sessionStrategy", _pref(moor_host, cfg, "sessionStrategy", "per-session"),
...
def cmd_setup(args) -> None:
    ...
    if is_local:
        _setup_local_auth(cfg, moor_host)
    elif not _setup_cloud_auth(cfg, moor_host, write_path):
        return
    ...
    for key, label, default in (
        ...
        ("aiPeer", "AI peer name", _pref(moor_host, cfg, "aiPeer", "moor")),
        ("workspace", "Workspace ID", _pref(moor_host, cfg, "workspace", "moor")),
    ):
        if new := _prompt(label, default=default):
            moor_host[key] = new

    _setup_identity_mapping(cfg, moor_host, current_peer)
  ...
  Other commands:
    moor honcho status     -- show full config
    moor honcho mode       -- change recall/observation mode
    moor honcho tokens     -- tune context and dialectic budgets
    moor honcho peer       -- update peer names
    moor honcho map <name> -- map this directory to a session name
""")
...
def _active_profile_name() -> str:
    """Active Moor profile name (respects --target-profile override)."""
    if _profile_override:
...
def _all_profile_host_configs() -> list[tuple[str, str, dict]]:
    ...
    cfg = _read_config()
    # _host_block (not hosts.get) keeps legacy dot-form keys ("moor.work") readable.
...
Honcho peers
{RULE}
  User peer:   {_pref(moor, cfg, 'peerName') or '(not set)'}
    Your identity in Honcho. Messages you send build this peer's card.
  AI peer:     {_pref(moor, cfg, 'aiPeer') or _host_key()}
    Moor' identity in Honcho. Seed with 'moor honcho identity <file>'.
    Dialectic calls ask this peer questions to warm session context.

  Dialectic reasoning:  {_pref(moor, cfg, 'dialecticReasoningLevel') or 'low'}  ({', '.join(REASONING_LEVELS)})
  Dialectic cap:        {_pref(moor, cfg, 'dialecticMaxChars') or 600} chars
""")
...
def _show_or_set_choice(args, *, attr: str, key: str, noun: str, title: str, choices: dict,
    ...
    if value is None:
        ...
        return print(f"\n  Set with: moor honcho {attr} [{'|'.join(choices)}]\n")
...
{RULE}
  ...
  Context     {_pref(moor, cfg, 'contextTokens') or '(Honcho default)'} tokens
  ...
  Dialectic   {_pref(moor, cfg, 'dialecticMaxChars') or 600} chars, reasoning: {_pref(moor, cfg, 'dialecticReasoningLevel') or 'low'}
    AI-to-AI inference. Moor asks Honcho's AI peer a question
...
def cmd_identity(args) -> None:
    ...
    if not _resolve_api_key(cfg):
        return print("  No API key configured. Run 'moor honcho setup' first.\n")
    ...
    if getattr(args, "show", False):
        ...
        print(ai_rep.get("representation") or ai_rep.get("card")
              or "  No representation built yet.\n  Run 'moor honcho identity <file>' to seed one.")
        print()
        return
...
{RULE}
  ...
  AI peer:   {hcfg.ai_peer}
    ...
    moor honcho identity --show        — show both peer representations
    moor honcho identity <file>        — seed AI peer from SOUL.md or any .md/.txt
""")
        return
...
""")
  ...
  2. Run:  moor honcho setup
     Paste the key when prompted.
""")
        if _yes(_prompt("  Run 'moor honcho setup' now?", default="y")):
            cmd_setup(args)
            cfg = _read_config()
            has_key = bool(cfg.get("apiKey", ""))
        else:
            print("\n  Run 'moor honcho setup' when ready, then re-run this walkthrough.")
    ...
    if user_files or agent_files:
        ...
        print("  No OpenClaw native memory files found in cwd or ~/.openclaw/.\n"
              ...
              "  or seed them manually:  moor honcho identity <path/to/file>")
...
""")
    if user_files:
        ...
        print("""
  These are picked up automatically the first time you run 'moor'
  with Honcho configured and no prior session history.
  (Moor calls migrate_memory_files() on first session init.)
...
Step 4  Seed AI identity files → Honcho AI peer
  ...
  In Moor, they are seeded once into Honcho's AI peer through the
...
""")
    if agent_files:
        ...
        if has_key:
            ...
            print("\n".join(f"    moor honcho identity {f}" for f in agent_files))
    else:
        print("  No agent identity files detected.\n  To seed manually:  moor honcho identity <path/to/SOUL.md>")
...
Step 5  What changes vs. OpenClaw native memory
  ...
  Session naming
    ...
    Moor:   per-session by default — each run gets its own session
              Map a custom name:  moor honcho map <session-name>

Step 6  Next steps
""")
    if not has_key:
        print("  1. moor honcho setup              — configure API key (required)\n"
              "  2. moor honcho migrate            — re-run this walkthrough")
    else:
        print("""  1. moor honcho status             — verify Honcho connection
  2. moor                           — start a session
     (user memory files auto-uploaded on first turn if not done above)
  3. moor honcho identity --show    — verify AI peer representation
  4. moor honcho tokens             — tune context and dialectic budgets
  5. moor honcho mode               — view or change memory mode""")
    print()
...
_SUBCOMMANDS = (
    ("setup", "Initial Honcho setup (redirects to moor memory setup)", None, ()),
    ...
    ("migrate", "Step-by-step migration guide from openclaw-honcho to Moor Honcho", cmd_migrate, ()),
...
def register_cli(subparser) -> None:
    """Build the ``moor honcho`` argparse subcommand tree on the ``moor honcho`` parser."""
