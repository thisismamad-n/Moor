...
Handles: moor honcho setup | status | sessions | map | peer
"""
...
def cmd_enable(args) -> None:
    ...
    cfg = _read_config()
    host = _host_key()
    label = f"[{host}] " if host != "moor" else ""
...
def cmd_sync(args) -> None:
    ...
    Scans all Moor profiles and creates host blocks for any that don't
    ...
    cfg = _read_config()
    if not cfg:
        print("  No Honcho config found. Run 'moor honcho setup' first.\n")
        return
    ...
    if not default_block and not has_key:
        print("  Honcho not configured on default profile. Run 'moor honcho setup' first.\n")
        return
...
def sync_honcho_profiles_quiet() -> int:
    ...
    Called from `moor update` -- no output, no exceptions.
    """
    try:
        from moor_cli.profiles import list_profiles
        profiles = list_profiles()
...
def _host_key() -> str:
    """Return the active Honcho host key, derived from the current Moor profile."""
...
def _local_config_path() -> Path:
    ...
    Always returns $MOOR_HOME/honcho.json so each profile/instance gets
    ...
    """
    return get_moor_home() / "honcho.json"
...
def _resolve_effective_identity_mapping(
    ...
    pin = False
    for val in (
        moor_host.get("pinUserPeer"),
        moor_host.get("pinPeerName"),
        cfg.get("pinUserPeer"),
        cfg.get("pinPeerName"),
    ):
        if val is not None:
    ...
    if "userPeerAliases" in moor_host:
        aliases_src = moor_host.get("userPeerAliases")
    ...
    if "runtimePeerPrefix" in moor_host:
        prefix_src = moor_host.get("runtimePeerPrefix")
...
def _scrub_identity_mapping(moor_host: dict) -> None:
...
def _gateway_platforms() -> list[str] | None:
    ...
    guarded (matching the idiom moor_cli already uses for gateway refs).
    """
    try:
...
def _apply_runtime_prefix(
    moor_host: dict, current_prefix: str, prefix_from_root: bool, label: str
) -> None:
    ...
    if new_prefix and not (prefix_from_root and new_prefix == current_prefix):
        moor_host["runtimePeerPrefix"] = new_prefix


def _echo_identity_mapping(moor_host: dict) -> None:
    """Show the resulting keys so the operator can verify what was written."""
    aliases = moor_host.get("userPeerAliases")
    prefix = moor_host.get("runtimePeerPrefix")
    print("  resolved →")
    print(f"    pinUserPeer       = {bool(moor_host.get('pinUserPeer'))}")
...
def _configure_raw_identity_mapping(
    ...
    _scrub_identity_mapping(moor_host)
    moor_host["pinUserPeer"] = pin
    if pin:
        return
    aliases = (
    ...
    while True:
        ...
        if "=" in entry:
            ...
            if rid and peer:
                aliases[rid] = peer
    if aliases:
        moor_host["userPeerAliases"] = aliases
    _apply_runtime_prefix(
        moor_host, current_prefix, prefix_from_root,
...
def _ensure_sdk_installed() -> bool:
    ...
    # data-volume target instead of writing to /opt/moor (NS-605).
...
def cmd_setup(args) -> None:
    ...
    print("\nHoncho memory setup\n" + "─" * 40)
    print("  Honcho gives Moor persistent cross-session memory.")
    ...
    moor_host = hosts.setdefault(_host_key(), {})
    ...
    if is_local:
        ...
        current_host_key = moor_host.get("apiKey", "")
        masked = (
        ...
        if new_local_key:
            moor_host["apiKey"] = new_local_key
    ...
    if not is_local:
        ...
        if use_device:
            ...
            try:
                ...
                print("\n  Cancelled. Re-run 'moor honcho setup' to try again.\n")
                return
            except (AuthorizationTimeout, DeviceCodeExpired):
                print("\n  Device code expired before approval.")
                print("  Re-run 'moor honcho setup' to get a new code.\n")
                return
            except AccessDenied:
                print("\n  Sign-in was denied on the approval page.")
                print("  Re-run 'moor honcho setup' to retry, or choose an API key instead.\n")
                return
                ...
                return
            print(" approved")
            _apply_grant_to_host(moor_host, cred)
            ...
            if not cfg.get("apiKey"):
                ...
                print("  Run 'moor honcho setup' again once you have a key.\n")
                return

    # --- 3. Identity ---
    current_peer = moor_host.get("peerName") or cfg.get("peerName", "")
    new_peer = _prompt("Your name (user peer)", default=current_peer or os.getenv("USER", "user"))
    if new_peer:
        moor_host["peerName"] = new_peer

    current_ai = moor_host.get("aiPeer") or cfg.get("aiPeer", "moor")
    new_ai = _prompt("AI peer name", default=current_ai)
    if new_ai:
        moor_host["aiPeer"] = new_ai

    current_workspace = moor_host.get("workspace") or cfg.get("workspace", "moor")
    new_workspace = _prompt("Workspace ID", default=current_workspace)
    if new_workspace:
        moor_host["workspace"] = new_workspace

    # --- 3b. Gateway identity mapping ---
    # These keys only affect the Moor GATEWAY (Telegram/Discord/Slack/...),
    ...
    (
        ...
        aliases_from_root,
        prefix_from_root,
    ) = _resolve_effective_identity_mapping(cfg, moor_host)
    ...
    if gw_platforms is None:
        ...
        run_mapping = _prompt(
            "Running the Moor gateway (Telegram/Discord/etc.)? (y/N)",
            default="n",
    ...
    if run_mapping:
        ...
        if shape == "single":
            ...
            _scrub_identity_mapping(moor_host)
            moor_host["pinUserPeer"] = False
            if prior_aliases:
                moor_host["userPeerAliases"] = prior_aliases
            _apply_runtime_prefix(
                moor_host, current_prefix, prefix_from_root,
            ...
            if merged:
                moor_host["userPeerAliases"] = merged
            _apply_runtime_prefix(
                moor_host, current_prefix, prefix_from_root,
            ...
            _configure_raw_identity_mapping(
                moor_host, current_pin, current_aliases, current_prefix,
    ...
    # --- 4. Observation mode ---
    current_obs = moor_host.get("observationMode") or cfg.get("observationMode", "directional")
    print("\n  Observation mode:")
    ...
    if new_obs in {"unified", "directional"}:
        moor_host["observationMode"] = new_obs
    else:
        moor_host["observationMode"] = "directional"

    # --- 5. Write frequency ---
    current_wf = str(moor_host.get("writeFrequency") or cfg.get("writeFrequency", "async"))
    print("\n  Write frequency:")
    ...
    try:
        moor_host["writeFrequency"] = int(new_wf)
    except (ValueError, TypeError):
        moor_host["writeFrequency"] = new_wf if new_wf in {"async", "turn", "session"} else "async"

    # --- 6. Recall mode ---
    _raw_recall = moor_host.get("recallMode") or cfg.get("recallMode", "hybrid")
    ...
    if new_recall in {"hybrid", "context", "tools"}:
        moor_host["recallMode"] = new_recall

    # --- 7. Context token budget ---
    current_ctx_tokens = moor_host.get("contextTokens") or cfg.get("contextTokens")
    ...
    if new_ctx_tokens.strip().lower() in {"none", "uncapped", "no limit"}:
        moor_host.pop("contextTokens", None)
        ...
        try:
            ...
            if val >= 0:
                moor_host["contextTokens"] = val
        except (ValueError, TypeError):
            pass  # keep current

    # --- 7b. Dialectic cadence ---
    current_dialectic = str(moor_host.get("dialecticCadence") or cfg.get("dialecticCadence") or "2")
    print("\n  Dialectic cadence:")
    ...
    try:
        ...
        if val >= 1:
            moor_host["dialecticCadence"] = val
    except (ValueError, TypeError):
        moor_host["dialecticCadence"] = 2

    # --- 7c. Dialectic reasoning level ---
    current_reasoning = (
        moor_host.get("dialecticReasoningLevel")
    ...
    if new_reasoning in {"minimal", "low", "medium", "high", "max"}:
        moor_host["dialecticReasoningLevel"] = new_reasoning
    else:
        moor_host["dialecticReasoningLevel"] = "low"

    # --- 8. Session strategy ---
    current_strat = moor_host.get("sessionStrategy") or cfg.get("sessionStrategy", "per-session")
    print("\n  Session strategy:")
    ...
    if new_strat in {"per-session", "per-repo", "per-directory", "global"}:
        moor_host["sessionStrategy"] = new_strat

    moor_host["enabled"] = True
    moor_host.setdefault("saveMessages", True)
    ...
    try:
        ...
        moor_config = load_config()
        moor_config.setdefault("memory", {})["provider"] = "honcho"
        save_config(moor_config)
        print("  Memory provider set to 'honcho' in config.yaml")
    except Exception as e:
        print(f"  Could not auto-enable in config.yaml: {e}")
        print("  Run: moor config set memory.provider honcho")
    ...
    print("\n  Other commands:")
    print("    moor honcho status     -- show full config")
    print("    moor honcho mode       -- change recall/observation mode")
    print("    moor honcho tokens     -- tune context and dialectic budgets")
    print("    moor honcho peer       -- update peer names")
    print("    moor honcho map <name> -- map this directory to a session name\n")
...
def _headless() -> tuple[bool, bool]:
    """(is_remote, can_open_browser) — degrades safely if moor_cli internals move."""
    try:
        from moor_cli.auth import _can_open_graphical_browser, _is_remote_session
...
def _apply_grant_to_host(moor_host: dict, cred) -> None:
    ...
    if cred.consent_peer_name:
        moor_host["peerName"] = cred.consent_peer_name


def _active_profile_name() -> str:
    """Return the active Moor profile name (respects --target-profile override)."""
    if _profile_override:
...
def _all_profile_host_configs() -> list[tuple[str, str, dict]]:
    ...
    for p in profiles:
        ...
        # ("moor.work") stay readable per the README's back-compat
...
def cmd_status(args) -> None:
    ...
    try:
        ...
        print("  honcho-ai is not installed. Run: moor honcho setup\n")
        return
    ...
    if not cfg:
        ...
        try:
            ...
            if _env_cfg.api_key or _env_cfg.base_url:
                ...
                print("  Run 'moor honcho setup' to configure.\n")
                return
        except Exception:
            print(f"  No Honcho config found at {active_path}")
            print("  Run 'moor honcho setup' to configure.\n")
...
def cmd_sessions(args) -> None:
    ...
    if not sessions:
        ...
        print("  Add one with: moor honcho map <session-name>")
...
def cmd_peer(args) -> None:
    ...
    if user_name is None and ai_name is None and reasoning is None:
        # Show current values
        hosts = cfg.get("hosts", {})
        moor = hosts.get(_host_key(), {})
        user = moor.get('peerName') or cfg.get('peerName') or '(not set)'
        ai = moor.get('aiPeer') or cfg.get('aiPeer') or _host_key()
        lvl = moor.get("dialecticReasoningLevel") or cfg.get("dialecticReasoningLevel") or "low"
        max_chars = moor.get("dialecticMaxChars") or cfg.get("dialecticMaxChars") or 600
        print("\nHoncho peers\n" + "─" * 40)
        print(f"  User peer:   {user}")
        print("    Your identity in Honcho. Messages you send build this peer's card.")
        print(f"  AI peer:     {ai}")
        print("    Moor' identity in Honcho. Seed with 'moor honcho identity <file>'.")
...
def cmd_mode(args) -> None:
    ...
    if mode_arg is None:
        ...
        print("\n  Set with: moor honcho mode [hybrid|context|tools]\n")
        return
...
def cmd_strategy(args) -> None:
    ...
    if strat_arg is None:
        ...
        print("\n  Set with: moor honcho strategy [per-session|per-directory|per-repo|global]\n")
        return
...
def cmd_tokens(args) -> None:
    ...
    if context is None and dialectic is None:
        ctx_tokens = moor.get("contextTokens") or cfg.get("contextTokens") or "(Honcho default)"
        d_chars = moor.get("dialecticMaxChars") or cfg.get("dialecticMaxChars") or 600
        d_level = moor.get("dialecticReasoningLevel") or cfg.get("dialecticReasoningLevel") or "low"
        print("\nHoncho budgets\n" + "─" * 40)
        print()
        ...
        print("    AI-to-AI inference. Moor asks Honcho's AI peer a question")
        ...
        print("\n  Set with: moor honcho tokens [--context N] [--dialectic N]\n")
        return
...
def cmd_identity(args) -> None:
    ...
    if not _resolve_api_key(cfg):
        print("  No API key configured. Run 'moor honcho setup' first.\n")
        return
    ...
    if show:
        ...
        if ai_rep.get("representation"):
            ...
            print("  Run 'moor honcho identity <file>' to seed one.")
        print()
        return
    ...
    if not file_path:
        ...
        print(f"  AI peer:   {hcfg.ai_peer}")
        print()
        print("    moor honcho identity --show        — show both peer representations")
        print("    moor honcho identity <file>        — seed AI peer from SOUL.md or any .md/.txt\n")
        return
...
def cmd_migrate(args) -> None:
    """Step-by-step migration guide: OpenClaw native memory → Moor + Honcho."""
    from pathlib import Path
    ...
    print("\nHoncho migration: OpenClaw native memory → Moor\n" + "─" * 50)
    print()
    ...
    if has_key:
        ...
        print("  Honcho is a cloud memory service that gives Moor persistent memory")
        print("  across sessions. You need an API key to use it.")
        print()
        print("  1. Get your API key at https://app.honcho.dev")
        print("  2. Run:  moor honcho setup")
        print("     Paste the key when prompted.")
        print()
        answer = _prompt("  Run 'moor honcho setup' now?", default="y")
        if answer.lower() in {"y", "yes"}:
            ...
            print()
            print("  Run 'moor honcho setup' when ready, then re-run this walkthrough.")
    ...
    if user_files or agent_files:
        ...
        print("  or seed them manually:  moor honcho identity <path/to/file>")
    ...
    if user_files:
        ...
        print()
        print("  These are picked up automatically the first time you run 'moor'")
        print("  with Honcho configured and no prior session history.")
        print("  (Moor calls migrate_memory_files() on first session init.)")
        print()
        print("  If you want to migrate them now without starting a session:")
        for f in user_files:
            print("    moor honcho migrate  — this step handles it interactively")
        if has_key:
            ...
            print("  Run 'moor honcho setup' first, then re-run this step.")
    else:
    ...
    print()
    print("  In Moor, they are seeded once into Honcho's AI peer through the")
    ...
    if agent_files:
        ...
        if has_key:
            ...
            print("  Run 'moor honcho setup' first, then seed manually:")
            for f in agent_files:
                print(f"    moor honcho identity {f}")
    else:
        print("  No agent identity files detected.")
        print("  To seed manually:  moor honcho identity <path/to/SOUL.md>")
    ...
    print("    Moor:   cloud-backed Honcho peers. Files can stay on disk as source")
    ...
    print("    Moor:   Honcho context fetched async at turn end, injected next turn.")
    ...
    print("    Moor:   Honcho observes every message and updates representations")
    ...
    print("    Moor:   per-session by default — each run gets its own session")
    print("              Map a custom name:  moor honcho map <session-name>")
    ...
    print()
    if not has_key:
        print("  1. moor honcho setup              — configure API key (required)")
        print("  2. moor honcho migrate            — re-run this walkthrough")
    else:
        print("  1. moor honcho status             — verify Honcho connection")
        print("  2. moor                           — start a session")
        print("     (user memory files auto-uploaded on first turn if not done above)")
        print("  3. moor honcho identity --show    — verify AI peer representation")
        print("  4. moor honcho tokens             — tune context and dialectic budgets")
        print("  5. moor honcho mode               — view or change memory mode")
    print()
...
def honcho_command(args) -> None:
    ...
    if sub == "setup":
        ...
        print("  Running 'moor memory setup'...\n")
        from moor_cli.memory_setup import cmd_setup_provider
...
def register_cli(subparser) -> None:
    """Build the ``moor honcho`` argparse subcommand tree.

    Called by the plugin CLI registration system during argparse setup.
    The *subparser* is the parser for ``moor honcho``.
    """
    ...
    subs.add_parser(
        "setup",
        help="Initial Honcho setup (redirects to moor memory setup)",
    )
    ...
    subs.add_parser(
        "migrate",
        help="Step-by-step migration guide from openclaw-honcho to Moor Honcho",
    )
