...
Config chain: $MOOR_HOME/honcho.json -> ~/.honcho/config.json -> env vars.
"""
...
class HonchoMemoryProvider(DialecticMixin, MemoryProvider):
    ...
    def save_config(self, values, moor_home):
        """Merge ``values`` into $MOOR_HOME/honcho.json (Honcho SDK native format)."""
    ...
    def _run_session_init(self, label: str) -> bool:
        ...
        try:
            self._do_session_init(self._config, self._lazy_init_session_id or "moor-default", **dict(init_kwargs))
        except Exception as e:
            self._manager = None
            self._session_initialized = False
            detail: object = e
    ...
    def _do_session_init(self, cfg, session_id: str, **kwargs) -> None:
        ...
        if cfg.session_strategy == "per-session":
            ...
            try:
                ...
                self._manager.migrate_memory_files(self._session_key, str(get_moor_home() / "memories"))
    ...
    def _pop_auth_notice(self) -> str:
        ...
        return ("[Honcho memory status] Authentication with the Honcho memory backend has expired and automatic "
                ...
                "Tell the user (once) that Honcho memory is paused and that running 'moor honcho setup' "
