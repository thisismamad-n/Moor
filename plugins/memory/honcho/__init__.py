...
Config: Uses the existing Honcho config chain:
  1. $MOOR_HOME/honcho.json (profile-scoped)
...
class HonchoMemoryProvider(MemoryProvider):
    ...
    def save_config(self, values, moor_home):
        """Write config to $MOOR_HOME/honcho.json (Honcho SDK native format)."""
        import json
        import os
    ...
    def _start_session_init_background(self, *, wait_timeout: float = 0.0) -> None:
        ...
        with self._init_lock:
            ...
            init_session_id = self._lazy_init_session_id or "moor-default"
    ...
    def _do_session_init(self, cfg, session_id: str, **kwargs) -> None:
        ...
        try:
            if not session.messages and cfg.session_strategy != "per-session":
                ...
                mem_dir = str(get_moor_home() / "memories")
    ...
    def _ensure_session(self) -> bool:
        ...
        try:
            self._do_session_init(
                self._config,
                self._lazy_init_session_id or "moor-default",
    ...
    def _pop_auth_notice(self) -> str:
        ...
        return (
            ...
            "'moor honcho setup' to re-authenticate will restore it."
        )
