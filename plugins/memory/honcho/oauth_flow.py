...
core (the code can arrive via the loopback listener here or a ``moor://``
...
# Loopback redirect registered for the Moor OAuth client. IP-literal so the browser can't resolve the
...
_CALLBACK_HTML = _CALLBACK_PAGE.format(
    title="Honcho connected", body="Connected to Honcho. You can close this tab and return to Moor."
).encode()
...
def start_loopback_flow_background(
    *, config_path: Path | None = None, host: str | None = None, source: str = "moor-desktop",
    timeout: float = 300.0,
) -> dict[str, str]:
