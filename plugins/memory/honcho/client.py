...
def resolve_active_host() -> str:
    ...
    # but applying it before profile resolution makes every named Moor
    # profile share that same host.  Keep named profiles isolated; only the
    # default Moor profile may opt into the config's default host.
...
class HonchoClientConfig:
    ...
    def resolve_session_name(
        ...
        Resolution order:
          ...
          2. per-session strategy — Moor session_id ({timestamp}_{hex}); authoritative,
             so a generated title never remaps a live conversation
          3. Manual directory override from sessions map
          4. Moor session title (from /title command; non-per-session)
