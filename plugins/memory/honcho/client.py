...
Config file resolution: $MOOR_HOME/honcho.json -> ~/.honcho/config.json -> env vars
...
def profile_host_key(profile: str | None) -> str:
    """Return the safe Honcho host key for a Moor profile."""
...
def resolve_active_host() -> str:
    """Honcho host key: MOOR_HONCHO_HOST env, else the active profile. The config's
...
def resolve_config_path() -> Path:
    """Active Honcho config path: $MOOR_HOME/honcho.json -> default profile's honcho.json
...
def _connection_fields(look: _HostLookup, host: str, path: Path) -> dict[str, Any]:
    ...
    # read it before the flat Moor spellings.
...
class HonchoClientConfig:
    ...
    # re-resolving resolve_config_path()/get_moor_home() later — those resolvers read a ContextVar that
    ...
    @classmethod
    def from_env(cls, workspace_id: str = "moor", host: str | None = None) -> HonchoClientConfig:
        ...
        return cls(
            ...
            config_path=resolve_config_path(), moor_home=get_moor_home(),
        )
    ...
    def from_global_config(cls, host: str | None = None, config_path: Path | None = None) -> HonchoClientConfig:
        ...
        derives it from the active Moor profile."""
        resolved_host = host or resolve_active_host()
        ...
        return cls(
            ...
            config_path=path, moor_home=get_moor_home(),
        )
...
def spawn_context_thread(target, *, name: str, daemon: bool = True, args: tuple = ()) -> "_threading.Thread":
    ...
    (set_moor_home_override) and a plain Thread starts EMPTY, so ambient resolution on it
