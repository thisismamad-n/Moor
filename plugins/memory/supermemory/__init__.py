...
_DEFAULT_CONTAINER_TAG = "moor"
_VALID_SEARCH_MODES = ("hybrid", "memories", "documents")
_DEFAULT_BASE_URL = "https://api.supermemory.ai"
_API_KEY_URL = "http://app.supermemory.ai/integrations?connect=moor"
...
def _load_supermemory_config(moor_home: Optional[str] = None) -> dict:
    """Defaults overlaid with $moor_home/supermemory.json (None = defaults only), every key normalized."""
    config = {k: (list(d) if isinstance(d, list) else d) for k, (d, _) in _CONFIG_SPEC.items()}
    if moor_home is not None:
        config.update({k: v for k, v in _read_json_dict(Path(moor_home) / "supermemory.json").items() if v is not None})
    for key, (_, normalize) in _CONFIG_SPEC.items():
        config[key] = normalize(config[key])
    return config


def _save_supermemory_config(values: dict, moor_home: str) -> None:
    from utils import atomic_json_write
    config_path = Path(moor_home) / "supermemory.json"
...
class _SupermemoryClient:
    def __init__(self, api_key: str, timeout: float, container_tag: str,
        ...
        self._client = Supermemory(api_key=api_key, base_url=self._base_url, timeout=timeout, max_retries=0,
                                   default_headers={"x-sm-source": "moor"})

    def _merge_metadata(self, metadata: Optional[dict]) -> dict:
        # sm_source routes Moor writes into the "Moor" Space in the Supermemory app so the user
        # can filter / bulk-manage them per source agent (a routing key for the user, not telemetry).
        merged = {"sm_source": "moor", **(metadata or {})}
...
def _probe_supermemory_connection(api_key: str, moor_home: str, *, identity: str = "default") -> dict:
    config = _load_supermemory_config(moor_home)
...
class SupermemoryMemoryProvider(MemoryProvider):
    def __init__(self):
        self._api_key = self._session_id = self._moor_home = ""
    ...
    def get_config_schema(self):
        # Only the API key is prompted during `moor memory setup`; other options live in supermemory.json / env.
    ...
    def save_config(self, values, moor_home):
        ...
        _save_supermemory_config(sanitized, moor_home)

    def get_status_config(self, provider_config: dict) -> dict:
        from moor_constants import get_moor_home
        return {"summary": _format_connection_summary(_probe_supermemory_connection(get_secret("SUPERMEMORY_API_KEY", "") or "", str(get_moor_home())))}

    def post_setup(self, moor_home: str, config: dict) -> None:
        from moor_cli.config import save_config
        from moor_cli.memory_setup import _prompt, _write_env_vars
        ...
        save_config(config)
        if val:
            _write_env_vars({"SUPERMEMORY_API_KEY": val}, moor_home=moor_home)
        ...
        status = _probe_supermemory_connection(api_key, moor_home)
