"""moor-memory-store — holographic memory plugin (MemoryProvider): structured fact storage with entity
resolution, trust scoring, and HRR-based compositional retrieval. Original plugin by dusterbloom (PR #2351).
Config in $MOOR_HOME/config.yaml under plugins.moor-memory-store: db_path ($MOOR_HOME/memory_store.db),
...
def _load_plugin_config() -> dict:
    try:
        from moor_cli.config import load_config_readonly  # canonical: managed-scope overlay + ${VAR} expansion
        return cfg_get(load_config_readonly(), "plugins", "moor-memory-store", default={}) or {}
    except Exception:
        return {}
...
class HolographicMemoryProvider(MemoryProvider):
    ...
    def save_config(self, values, moor_home):
        ...
        config_path = Path(moor_home) / "config.yaml"
        try:
            import yaml
            from moor_cli.config import read_user_config_raw  # raw read: merged defaults must not be persisted
            existing = read_user_config_raw(config_path)
            existing.setdefault("plugins", {})["moor-memory-store"] = values
    ...
    def get_config_schema(self):
        ...
        return [
            {"key": "db_path", "description": "SQLite database path", "default": f"{display_moor_home()}/memory_store.db"},
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        _moor_home = str(get_moor_home())
        db_path = self._config.get("db_path", _moor_home + "/memory_store.db")
        if isinstance(db_path, str):  # expand $MOOR_HOME so paths resolve to the active profile
            db_path = db_path.replace("$MOOR_HOME", _moor_home).replace("${MOOR_HOME}", _moor_home)
