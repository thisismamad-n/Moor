"""moor-memory-store — holographic memory plugin using MemoryProvider interface.
...
Config in $MOOR_HOME/config.yaml (profile-scoped):
  plugins:
    moor-memory-store:
      db_path: $MOOR_HOME/memory_store.db   # omit to use the default
      auto_extract: false
      default_trust: 0.5
...
def _load_plugin_config() -> dict:
    try:
        ...
        return cfg_get(all_config, "plugins", "moor-memory-store", default={}) or {}
    except Exception:
        return {}
...
class HolographicMemoryProvider(MemoryProvider):
    ...
    def save_config(self, values, moor_home):
        ...
        try:
            ...
            existing["plugins"]["moor-memory-store"] = values
    ...
    def get_config_schema(self):
        ...
        _default_db = f"{display_moor_home()}/memory_store.db"
        return [
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        _default_db = _moor_home + "/memory_store.db"
        db_path = self._config.get("db_path", _default_db)
        # Expand $MOOR_HOME in user-supplied paths so config values like
        # "$MOOR_HOME/memory_store.db" or "~/.moor/memory_store.db" both
        # resolve to the active profile's directory.
        if isinstance(db_path, str):
            db_path = db_path.replace("$MOOR_HOME", _moor_home)
            db_path = db_path.replace("${MOOR_HOME}", _moor_home)
