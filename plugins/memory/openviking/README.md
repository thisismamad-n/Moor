...
default profile that is `~/.moor/.env`; for a named profile use
...
Moor also sends `User-Agent: openviking-memory-moor/<version>` on
OpenViking requests. This standard harness identifier contains the Moor
...
does not ask for a peer ID. Without a configured peer, Moor sends neither
...
order remains environment, linked OpenViking config, then Moor YAML. To use
...
on the old implicit `moor` peer now use user memory for new writes. Without
...
Set `agent: moor` to restore peer-scoped writes. Memories written at user
...
`viking_remember` creates a one-shot `moor-remember-<random>` OpenViking
...
not advertised or used. The one-shot session is separate from the live Moor
...
Use the same OpenViking profile and credentials as Moor for manual recovery.
...
file URIs, such as
`viking://user/default/peers/moor/memories/preferences/mem_abc123.md` (any
