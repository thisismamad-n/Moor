...
or a linked OpenViking CLI config:
  ...
  OPENVIKING_AGENT     — Moor peer ID in OpenViking (default: moor)
...
def _openviking_server_log_path() -> Path:
    try:
        ...
        home = Path(os.environ.get("MOOR_HOME", "")).expanduser() if os.environ.get("MOOR_HOME") else Path.home() / ".moor"
...
def _start_local_openviking_server(endpoint: str) -> tuple[str, str]:
    ...
    try:
        ...
        # aborting `moor update` with access-denied on .pyd files.
...
class OpenVikingMemoryProvider(MemoryProvider):
    ...
    def __init__(self):
        ...
        # against a concurrent increment. See moor-agent#28296 review.
    ...
    def post_setup(self, moor_home: str, config: dict) -> None:
        ...
        from moor_cli.memory_setup import _CANCELLED, _curses_select, _print_cancelled_setup, _prompt

        moor_home_path = Path(moor_home)
        env_path = moor_home_path / ".env"
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        self._turn_count = 0
        moor_home = str(kwargs.get("moor_home") or "").strip()
        if not moor_home:
            try:
                from moor_constants import get_moor_home
                moor_home = str(get_moor_home())
            except Exception:
                moor_home = str(Path.home() / ".moor")
        self._moor_home = moor_home
    ...
    def _ensure_client(self) -> Optional["_VikingClient"]:
        ...
        ``~/.moor/.env`` after startup never reach the live client and tools
        keep running against stale auth until the user restarts moor (#21130).
    ...
    def _pending_session_dir(self) -> Optional[Path]:
        if not self._moor_home:
            return None
        return Path(self._moor_home) / _PENDING_SESSIONS_RELATIVE_DIR
    ...
    def _run_lock_dir(self) -> Optional[Path]:
        if not self._moor_home:
            return None
        return Path(self._moor_home) / _RUN_LOCKS_RELATIVE_DIR
    ...
    def on_session_switch(
        ...
        **kwargs,
        ...
        and memory extraction never fires for it. See moor-agent#28296.
    ...
    def _read_uri_payload(
        ...
        *,
        ...
        # (e.g. viking://user/moor/.overview.md).
