...
_DEFAULT_AGENT = ""
_OPENVIKING_USER_AGENT = f"openviking-memory-moor/{_MOOR_VERSION}"
...
_SYNC_TRACE_ENV = "MOOR_OPENVIKING_SYNC_TRACE"
...
_RETRY_LATER = (
    "OpenViking memory is temporarily unavailable; Moor will retry on a later access or when the config changes."
)
...
def _normalize_openviking_url(url: str) -> str:
    ...
    try:
        ...
        raise _OpenVikingEndpointError("OpenViking endpoint safety validation failed; Moor refused the connection.") from exc
    if blocked:
...
def _load_moor_openviking_config() -> dict:
    try:
...
def _moor_home_path() -> Path:
    try:
        ...
        return Path(env_home).expanduser() if env_home else Path.home() / ".moor"
...
def _start_local_openviking_server(endpoint: str) -> tuple[str, str]:
    ...
    if _local_openviking_port_is_open(host, port):
        return _LOCAL_SERVER_OCCUPIED, (
            f"Port {host}:{port} is occupied by {_describe_local_port_listener(host, port)}. Moor did not start "
    ...
    log_path = _moor_home_path() / _OPENVIKING_SERVER_LOG_RELATIVE_PATH
    try:
        ...
        # the Moor venv's .pyd files, breaking `moor update`).
        # Do not let the server child inherit this process's PYTHONPATH. If inherited, openviking-server
        # would import aiohttp and friends from the Moor venv instead of its own (its venv's site-packages
        # are shadowed because PYTHONPATH precedes them) — and on Windows the loaded DLLs then lock the
        # Moor venv, aborting `moor update` with access-denied on .pyd files. (#78153)
...
class OpenVikingMemoryProvider(MemoryProvider):
    ...
    def __init__(self):
        ...
        self._session_id, self._turn_count, self._moor_home = "", 0, ""
        ...
        # increment. See moor-agent#28296 review.
    ...
    def is_available(self) -> bool:
        ...
        if os.environ.get("OPENVIKING_ENDPOINT"):
            return True
        provider_config = _load_moor_openviking_config()
    ...
    def save_config(self, values: Dict[str, Any], moor_home: str) -> None:
    ...
    def post_setup(self, moor_home: str, config: dict) -> None:
        ...
        _setup.run_setup(moor_home, config)
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        connection_error = ""
        try:
            settings = _resolve_connection_settings(_load_moor_openviking_config())
        except _OpenVikingEndpointError as exc:
        ...
        self._turn_count = 0
        self._moor_home = str(kwargs.get("moor_home") or "").strip() or str(_moor_home_path())
    ...
    def _ensure_client(self) -> Optional["_VikingClient"]:
        ...
        so OPENVIKING_* values added to ``~/.moor/.env`` after startup never reach the live client and
        tools keep running against stale auth until the user restarts moor (#21130).
        """
    ...
    def _ensure_client_locked(self) -> Optional["_VikingClient"]:
        ...
        if health_state == "responded":
            logger.warning(
                "%s OpenViking memory is temporarily unavailable; Moor will retry on a later access (after cooldown) or when the config changes.",
                health_message,
            )
        else:
    ...
    def _extract_current_turn_messages(messages: Optional[List[Dict[str, Any]]], user_content: str, assistant_content: str) -> List[Dict[str, Any]]:
        """Slice the completed turn out of Moor' full canonical transcript: the last
    ...
    def _state_path(self, kind: str, name: str) -> Optional[Path]:
        """Marker/lock file under MOOR_HOME: ``pending`` -> pending_sessions/<sid>.json,
        ``lock`` -> runs/<run_id>.lock; an empty run id maps to the legacy recovery lock."""
        name = str(name or "").strip()
        if not self._moor_home or (not name and kind != "lock"):
            return None
        if kind == "pending":
            return Path(self._moor_home) / _PENDING_SESSIONS_RELATIVE_DIR / f"{quote(name, safe='')}.json"
        return Path(self._moor_home) / _RUN_LOCKS_RELATIVE_DIR / (f"{quote(name, safe='')}.lock" if name else _LEGACY_RECOVERY_LOCK_FILENAME)
    ...
    def _pending_sessions(self) -> List[tuple[str, str]]:
        ...
        directory = Path(self._moor_home) / _PENDING_SESSIONS_RELATIVE_DIR if self._moor_home else None
    ...
    def _tool_remember(self, args: dict) -> str:
        """Submit content through a dedicated session so it never touches the live Moor session."""
        ...
        session_id = f"moor-remember-{uuid.uuid4().hex[:12]}"
        ...
        def failure(message: str, *, stage: str, message_status: str) -> str:
            return tool_error(
                ...
                recovery_note=(
                    ...
                    "Moor. Otherwise, do not resubmit automatically; report the uncertain state to the user."
                ),
            )
        try:
