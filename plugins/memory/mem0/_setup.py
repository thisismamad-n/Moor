...
def _write_env(env_path: Path, env_writes: dict[str, str]) -> None:
    ...
    if env_path.exists():
        ...
        # moor_cli/config.py. read_text() with no encoding falls back to the
...
def _save_mem0_json(moor_home: str, data: dict) -> None:
...
def _setup_platform(moor_home: str, config: dict, flags: dict[str, str]) -> None:
    ...
    schema = [
        ...
        {"key": "user_id", "description": "User identifier", "default": "moor-user"},
        {"key": "agent_id", "description": "Agent identifier", "default": "moor"},
    ...
    # docs tell self-hosted users to put MEM0_HOST in ~/.moor/.env. Warn
    ...
    if os.environ.get("MEM0_HOST", "").strip():
        print(
            ...
            "remove it from ~/.moor/.env (or unset it) or Moor will keep "
    ...
    provider.save_config(provider_config, moor_home)
...
def _setup_selfhosted(moor_home: str, config: dict, flags: dict[str, str]) -> None:
    ...
    user_id = flags.get("user_id") or _prompt(
        "User identifier", default=provider_config.get("user_id") or "moor-user"
    )
...
def _setup_oss(moor_home: str, config: dict, flags: dict[str, str]) -> None:
    ...
    user_id = flags.get("user_id") or os.getenv("USER", "moor-user")
    ...
    if env_writes:
        _write_env(Path(moor_home) / ".env", env_writes)
    _save_mem0_json(moor_home, {"mode": "oss", "user_id": user_id, "agent_id": "moor", "oss": oss_config})
...
def _prompt_api_key(label: str, env_var: str, moor_home: str) -> str:
    ...
    if not existing:
        ...
        if env_path.exists():
            ...
            # moor_cli/config.py; a Notepad BOM on the first line would
...
_PGVECTOR_CONTAINER = "moor-pgvector"
_PGVECTOR_IMAGE = "pgvector/pgvector:pg17"
_PGVECTOR_PASSWORD = "moor"
...
def _setup_oss_interactive(moor_home: str, config: dict) -> None:
    ...
    if llm_def["needs_key"]:
        key = _prompt_api_key(llm_def["label"], llm_def["env_var"], moor_home)
        if key:
    ...
    if embedder_def["needs_key"] and embedder_id != llm_id:
        key = _prompt_api_key(f"{embedder_def['label']} embedder", embedder_def["env_var"], moor_home)
        if key:
    ...
    user_id = input(f"  User ID [{os.getenv('USER', 'moor-user')}]: ").strip()
    user_id = user_id or os.getenv("USER", "moor-user")

    agent_id = input("  Agent ID [moor]: ").strip()
    agent_id = agent_id or "moor"
    ...
    if env_writes:
        _write_env(Path(moor_home) / ".env", env_writes)
    _save_mem0_json(moor_home, {"mode": "oss", "user_id": user_id, "agent_id": agent_id, "oss": oss_config})
...
def post_setup(moor_home: str, config: dict) -> None:
    """Entry point called by moor memory setup framework.
    ...
    if flags["mode"] == "oss":
        ...
        _setup_oss(moor_home, config, flags)
        return

    if flags["mode"] in ("selfhosted", "self-hosted"):
        _setup_selfhosted(moor_home, config, flags)
        return

    if flags["mode"] == "platform":
        _setup_platform(moor_home, config, flags)
        return
