...
from moor_constants import get_moor_home  # noqa: F401 — patched by tests
...
_PGVECTOR_CONTAINER, _PGVECTOR_IMAGE, _PGVECTOR_PASSWORD = "moor-pgvector", "pgvector/pgvector:pg17", "moor"
...
def _prompt_api_key(label: str, env_var: str, moor_home: str) -> str:
    """Prompt for API key, showing masked existing value if found."""
    existing = os.environ.get(env_var, "")
    env_path = Path(moor_home) / ".env"
...
def _persist_provider_config(moor_home: str, config: dict, provider_config: dict, env_writes: dict[str, str], label: str, key_line: str, server: str | None = None) -> None:
    ...
    Mem0MemoryProvider().save_config(provider_config, moor_home)
    if env_writes:
...
def _setup_platform(moor_home: str, config: dict, flags: dict[str, str]) -> None:
    ...
    provider_config = _read_mem0_json(Path(moor_home) / "mem0.json")
    print("\n  Configuring mem0:\n")
    ...
    if os.environ.get("MEM0_HOST", "").strip():
        print(f"\n  ⚠ MEM0_HOST is set in your environment ({os.environ['MEM0_HOST']}). It overrides platform mode — remove it from ~/.moor/.env (or unset it) or Moor will keep routing to the self-hosted server.")
    _persist_provider_config(moor_home, config, provider_config, env_writes, "mem0", "API keys saved to .env")
...
def _setup_selfhosted(moor_home: str, config: dict, flags: dict[str, str]) -> None:
    ...
    user_id = flags.get("user_id") or _prompt("User identifier", default=provider_config.get("user_id") or "moor-user")
    agent_id = _prompt("Agent identifier", default=provider_config.get("agent_id") or "moor")
    if flags.get("dry_run"):
    ...
    _persist_provider_config(moor_home, config, provider_config, env_writes, "mem0 (self-hosted)", "API key saved to .env", server=host)
...
def _finish_oss(moor_home: str, config: dict, oss_config: dict, env_writes: dict[str, str], user_id: str, agent_id: str, pgvector_config: dict | None = None) -> None:
...
def _setup_oss(moor_home: str, config: dict, flags: dict[str, str]) -> None:
    """OSS mode — non-interactive when --mode was given, otherwise curses pickers."""
    if not flags.get("_mode_from_flag"):
        _setup_oss_interactive(moor_home, config)
        return
    ...
    if flags.get("dry_run"):
        ...
        print("  [dry-run] No files written.\n")
        return
    _finish_oss(moor_home, config, oss_config, env_writes, flags.get("user_id") or os.getenv("USER", "moor-user"), "moor")
...
def _configure_model_provider(kind: str, registry: dict, moor_home: str, env_writes: dict[str, str], llm: tuple[str, dict] | None = None) -> tuple[str, dict, str, str | None]:
    ...
    if pdef["needs_key"]:
        if llm is None or pid != llm[0]:
            if key := _prompt_api_key(pdef["label"] if llm is None else f"{pdef['label']} embedder", pdef["env_var"], moor_home):
...
def _setup_oss_interactive(moor_home: str, config: dict) -> None:
    ...
    user_id = _input("User ID", os.getenv("USER", "moor-user"))
    agent_id = _input("Agent ID", "moor")
    flags = {
    ...
    _finish_oss(moor_home, config, oss_config, env_writes, user_id, agent_id, pgvector_config)
...
def _install_provider_deps(llm_id: str, embedder_id: str, vector_id: str) -> None:
    ...
    for dep in sorted(deps):
        ...
        try:
            # Environment-aware install: sealed hosted venvs redirect to the durable data-volume target instead of /opt/moor.
...
def post_setup(moor_home: str, config: dict) -> None:
    """Entry point for `moor memory setup`: routes on --mode (platform / selfhosted / oss), else shows a picker.
    ...
    handler(moor_home, config, flags)
