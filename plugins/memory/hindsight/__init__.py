...
Config via environment variables:
  ...
  HINDSIGHT_BANK_ID                — memory bank identifier (default: moor)
  ...
  HINDSIGHT_RETAIN_SOURCE          — metadata source value attached to retained memories (default: moor)
  HINDSIGHT_RETAIN_USER_PREFIX     — label used before user turns in retained transcripts
  HINDSIGHT_RETAIN_ASSISTANT_PREFIX — label used before assistant turns in retained transcripts

Or via $MOOR_HOME/hindsight/config.json (profile-scoped), falling back to
...
# via the ``retain_source`` config key or HINDSIGHT_RETAIN_SOURCE (e.g. "moor").
...
def _check_local_runtime() -> tuple[bool, str | None]:
    ...
    healthy and ``moor memory status`` would stay green while the daemon
...
def _local_runtime_hint(reason: str | None) -> str:
    ...
    selected local_embedded without going through ``moor memory setup`` — a
    ...
    if "no module named" in text and ("hindsight'" in text or 'hindsight"' in text
        ...
        return (
            ...
            f"{sys.executable} hindsight-all — or run 'moor memory setup'. "
...
def _load_config() -> dict:
    ...
    Resolution order:
      1. $MOOR_HOME/hindsight/config.json  (profile-scoped)
    ...
    profile_path = get_moor_home() / "hindsight" / "config.json"
    ...
    return {
        ...
        "banks": {
            "moor": {
                "bankId": os.environ.get("HINDSIGHT_BANK_ID", "moor"),
...
def _event_timestamp() -> str:
    """Return the configured Moor event time with an explicit UTC offset."""
    event_time = _moor_now()
    # moor_time.now() guarantees an aware datetime. Keep this fallback so a
...
def _embedded_profile_name(config: dict[str, Any]) -> str:
    ...
    profile = config.get("profile", "moor")
    return str(profile or "moor")
...
def _resolve_bank_id_template(template: str, fallback: str, **placeholders: str) -> str:
    ...
    collapsed — e.g. ``moor-{user}`` with no user becomes ``moor``.
...
class HindsightMemoryProvider(MemoryProvider):
    ...
    def save_config(self, values, moor_home):
        """Write config to $MOOR_HOME/hindsight/config.json."""
        import json
        from pathlib import Path
        config_dir = Path(moor_home) / "hindsight"
    ...
    def post_setup(self, moor_home: str, config: dict) -> None:
        ...
        from moor_cli.memory_setup import _CANCELLED, _curses_select, _print_cancelled_setup
        ...
        provider_config.setdefault("bank_id", "moor")
        ...
        self.save_config(provider_config, moor_home)
        ...
        if mode == "local_embedded":
            ...
            if not llm_api_key:
                llm_api_key = _load_simple_env(Path(moor_home) / ".env").get("HINDSIGHT_LLM_API_KEY", "")
            if not llm_api_key:
    ...
    def _offer_starter_template(self, mode: str, provider_config: dict, env_writes: dict) -> None:
        ...
        from moor_cli.memory_setup import _CANCELLED, _curses_select
        ...
        bank_id = provider_config.get("bank_id", "moor")
    ...
    def get_config_schema(self):
        return [
            ...
            {"key": "bank_id", "description": "Memory bank name (static fallback when bank_id_template is unset)", "default": "moor"},
            {"key": "bank_id_template", "description": "Optional template to derive bank_id dynamically. Placeholders: {profile}, {workspace}, {platform}, {user}, {session}. Example: moor-{profile}", "default": ""},
        ...
        if self._client is None:
            if self._mode == "local_embedded":
                ...
                logger.debug("Creating HindsightEmbedded client (profile=%s, provider=%s)",
                             self._config.get("profile", "moor"), llm_provider)
                kwargs = dict(
                    profile=self._config.get("profile", "moor"),
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        try:
            ...
            if Version(installed) < Version(_MIN_CLIENT_VERSION):
                ...
                # durable data-volume target instead of /opt/moor (NS-605).
                from tools.lazy_deps import install_specs
        ...
        banks = cfg_get(self._config, "banks", "moor", default={})
        static_bank_id = self._config.get("bank_id") or banks.get("bankId", "moor")
        ...
        if self._mode == "local_embedded":
            ...
            if hasattr(os, "geteuid") and os.geteuid() == 0:
                msg = (
                    ...
                    "to cloud / local_external mode via 'moor memory setup'."
                )
                logger.warning(msg)
