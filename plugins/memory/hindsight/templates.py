...
the ``moor`` integration, and applies a chosen manifest to the user's bank
...
def fetch_moor_templates(url: str | None = None) -> list[dict]:
    """Return catalog entries tagged for the ``moor`` integration."""
    catalog = _get_json(url or catalog_url())
    entries = catalog.get("templates", []) if isinstance(catalog, dict) else []
    return [e for e in entries if "moor" in (e.get("integrations") or [])]
