...
Rules:
  ...
  - empty dirs    → always delete (under MOOR_HOME)
  - research      → keep 10 newest, prompt for older (deep only)
  - chrome-profile→ prompt after 14 days (deep only)
  - >500 MB files → prompt always (deep only)

Scope: strictly MOOR_HOME and /tmp/moor-*
Never touches: ~/.moor/logs/ or any system directory.
"""
...
try:
    ...
    def get_moor_home() -> Path:  # type: ignore[no-redef]
        val = (os.environ.get("MOOR_HOME") or "").strip()
        return Path(val).resolve() if val else (Path.home() / ".moor").resolve()
...
def get_state_dir() -> Path:
    """State dir — separate from ``$MOOR_HOME/logs/``."""
    return get_moor_home() / "disk-cleanup"
...
def get_log_file() -> Path:
    """Audit log — intentionally NOT under ``$MOOR_HOME/logs/``."""
...
def is_safe_path(path: Path) -> bool:
    """Accept only paths under MOOR_HOME or ``/tmp/moor-*``.

    Rejects Windows mounts (``/mnt/c`` etc.) and any system directory.
    """
    moor_home = get_moor_home()
    try:
        path.resolve().relative_to(moor_home)
        return True
    except (ValueError, OSError):
        pass
    # Allow /tmp/moor-* explicitly
    parts = path.parts
    if len(parts) >= 3 and parts[1] == "tmp" and parts[2].startswith("moor-"):
        return True
    return False
...
# Paths under $MOOR_HOME that must NEVER be deleted by quick(),
...
def _is_protected_cron_path(p: Path) -> bool:
    ...
    history at once.
    """
    # Lazily build the set once per process so MOOR_HOME is resolved
    # exactly once.
    if not _PROTECTED_CRON_PATHS:
        moor_home = get_moor_home()
        for parent in ("cron", "cronjobs"):
            base = moor_home / parent
...
def quick() -> Dict[str, Any]:
    ...
    # Remove empty dirs under MOOR_HOME, but never recurse into known
    # durable state trees.  Some installs place the Moor checkout, venv,
    # and desktop build under MOOR_HOME; a full rglob over that tree can
    # stall the gateway event loop for minutes.
    moor_home = get_moor_home()
    empty_removed = 0
    sweep_stack: List[Tuple[Path, bool]] = []
    try:
        for top in moor_home.iterdir():
            if (
                top.is_dir()
...
def guess_category(path: Path) -> Optional[str]:
    ...
    moor_home = get_moor_home()
    try:
        rel = path.resolve().relative_to(moor_home)
        ...
        # Path isn't under MOOR_HOME (e.g. /tmp/moor-*) — fall through.
        pass
