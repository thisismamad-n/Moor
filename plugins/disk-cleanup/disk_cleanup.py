...
after 14 days; empty dirs under MOOR_HOME always. Prompt-only: research
(keep 10 newest, > 30 days), chrome-profile > 14 days, any file > 500 MB.
Scope: strictly MOOR_HOME and /tmp/moor-*; never ~/.moor/logs/ or system dirs.
"""
...
def _state_file(name: str) -> Path:
    """``$MOOR_HOME/disk-cleanup/<name>`` — deliberately outside ``$MOOR_HOME/logs/``."""
    return get_moor_home() / "disk-cleanup" / name


def is_safe_path(path: Path) -> bool:
    """Accept only paths under MOOR_HOME or ``/tmp/moor-*`` (rejects /mnt/c etc.)."""
    with contextlib.suppress(ValueError, OSError):
        path.resolve().relative_to(get_moor_home())
        return True
    parts = path.parts
    return len(parts) >= 3 and parts[1] == "tmp" and parts[2].startswith("moor-")
...
# Top-level MOOR_HOME dirs whose empty subdirs are never swept (last row: user project trees).
...
# Top-level MOOR_HOME entries guess_category() never auto-tracks: state, logs, memory,
...
@functools.lru_cache(maxsize=1)  # built lazily so MOOR_HOME resolves once
def _protected_cron_paths() -> frozenset:
    ...
    return frozenset(str(x) for parent in ("cron", "cronjobs") for base in (get_moor_home() / parent,)
...
def track(path_str: str, category: str, silent: bool = False) -> bool:
    ...
    if not is_safe_path(path):
        _log(f"REJECT: {path} (outside MOOR_HOME)")
        return False
...
def quick() -> Dict[str, Any]:
    ...
    for item, p, age in _live_items(load_tracked(), datetime.now(timezone.utc), log_stale=True):
        ...
        if err is None:
            ...
            errors.append(err)
            new_tracked.append(item)
    empty_removed = _sweep_empty_dirs(get_moor_home())
...
def _sweep_empty_dirs(moor_home: Path) -> int:
    """Remove empty dirs under MOOR_HOME without recursing into durable/heavy trees (a full
    rglob over a checkout+venv under MOOR_HOME can stall the gateway loop for minutes).
    Iterative post-order so parents emptied by child removal are caught."""
    removed = 0
    stack: List[Tuple[Path, bool]] = [
        (top, False) for top in _subdirs(moor_home, _EMPTY_DIR_PROTECTED_TOP_LEVEL | _EMPTY_DIR_SWEEP_PRUNE_DIRS)]
    while stack:
...
def guess_category(path: Path) -> Optional[str]:
    ...
    if not is_safe_path(path):
        return None
    with contextlib.suppress(ValueError):  # not under MOOR_HOME (/tmp/moor-*) — name rules only
        rel = path.resolve().relative_to(get_moor_home())
