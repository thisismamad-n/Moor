...
Every handler is a thin wrapper around ``moor_cli.kanban_db`` (the same code paths the CLI
...
def _ws_upgrade_authorized(ws: "WebSocket") -> bool:
    ...
    try:
        from moor_cli import web_server_chat as _ws
    except Exception:
        return True
...
def _with_board_pinned(board: Optional[str], fn: Callable[[], Any]) -> Any:
    ...
    ``MOOR_KANBAN_BOARD`` env var (concurrent requests for different boards would cross-write)."""
...
def _run_aux(board: Optional[str], module: str, fn: str, task_id: str, author: Optional[str]) -> Any:
    """Run a slow auxiliary-LLM task helper (``moor_cli.<module>.<fn>``) with the board pinned;
    the module is imported lazily so a missing aux client can't break plugin load."""
    def _run():
        return getattr(importlib.import_module(f"moor_cli.{module}"), fn)(task_id, author=(author or None))
...
def get_board(
    ...
    (``MOOR_KANBAN_BOARD`` env → on-disk ``current`` pointer → ``default``)."""
...
def create_task(payload: CreateTaskBody, board: Optional[str] = Query(None)):
    with _board_conn(board) as (board, conn), _value_error_400():
        ...
        # dashboard backend may run under a different MOOR_HOME than the board's profile.
        if task and task.status == "ready" and task.assignee:
            try:
                from moor_cli.kanban import _check_dispatcher_presence
                from moor_constants import get_moor_home
                running, message = _check_dispatcher_presence(moor_home=get_moor_home())
...
class UpdateTaskBody(BaseModel):
    ...
    # Handoff fields forwarded to complete_task on -> 'done' (parity with ``moor kanban complete``).
...
def list_diagnostics(
    ...
    consumed by ``moor kanban diagnostics`` when the dashboard runs."""
    with _board_conn(board) as (board, conn):
...
def specify_task_endpoint(task_id: str, payload: SpecifyBody, board: Optional[str] = Query(None)):
    """Flesh out a triage task via the auxiliary LLM (``moor kanban specify``). Non-OK is NOT
...
def reassign_task_endpoint(task_id: str, payload: ReassignBody, board: Optional[str] = Query(None)):
    ...
    (``moor kanban reassign <task_id> <profile> [--reclaim]``)."""
    with _board_conn(board) as (board, conn):
...
def _active_profile_name() -> str:
    """Current Moor profile name for notify-sub ownership."""
    try:
...
def model_options():
    ...
    (same substrate as the Models page) so it can't offer a pair Moor rejects. Skips pricing
...
async def export_board_endpoint(slug: str, body: ExportBoardBody):
    ...
    from moor_cli import kanban_transfer
...
def update_profile_description(profile_name: str, payload: DescribeBody):
    ...
    with _errors_to_500("failed to update profile"):
        ...
        if canon == "default":
            ...
            profile_dir = Path(get_moor_home())
        else:
...
def decompose_task_endpoint(task_id: str, payload: DecomposeBody, board: Optional[str] = Query(None)):
    """Fan a triage task out into child tasks via the auxiliary LLM (``moor kanban decompose``).
