...
``moor_cli.kanban_db`` or a direct SQL query. Writes use the same code
...
own pages inject it via ``window.__MOOR_SESSION_TOKEN__`` so logged-in
...
the in-browser PTY bridge in ``moor_cli/web_server.py``.

This means ``moor dashboard --host 0.0.0.0`` is safe to run on a LAN:
...
def _ws_upgrade_authorized(ws: "WebSocket") -> bool:
    ...
    WS auth gate (``moor_cli.web_server._ws_auth_ok``).
    ...
    """
    try:
        from moor_cli import web_server as _ws
    except Exception:
...
def _compute_task_diagnostics(
    ...
    Uses ``moor_cli.kanban_diagnostics`` — see that module for the
    rule definitions.
    """
    from moor_cli import kanban_diagnostics as kd
...
def _warnings_summary_from_diagnostics(
    ...
    if not diagnostics:
        return None
    from moor_cli.kanban_diagnostics import SEVERITY_ORDER
...
def get_board(
    ...
    through to the active board (``MOOR_KANBAN_BOARD`` env → on-disk
...
def create_task(payload: CreateTaskBody, board: Optional[str] = Query(None)):
    ...
    try:
        ...
        if task and task.status == "ready" and task.assignee:
            try:
                from moor_cli.kanban import _check_dispatcher_presence
                from moor_constants import get_moor_home

                # Scope the probe to the request's active home. The dashboard
                # backend can run under a different MOOR_HOME than the
...
from moor_cli.kanban_db import (  # noqa: E402
...
class UpdateTaskBody(BaseModel):
    ...
    # transitions to 'done'. Dashboard parity with ``moor kanban
...
# spawn failures, stuck-blocked). See moor_cli.kanban_diagnostics for
# the rule engine.
# ---------------------------------------------------------------------------
...
def list_diagnostics(
    ...
    AND for ``moor kanban diagnostics`` which shells to this
...
def reclaim_task_endpoint(
    ...
    ``moor kanban reclaim <task_id> --reason ...``.
    """
    board = _resolve_board(board)
    conn = _conn(board=board)
    try:
...
def specify_task_endpoint(
    ...
    it to ``todo``. Maps 1:1 to ``moor kanban specify <task_id>``.
    ...
    # MOOR_KANBAN_BOARD env var — this endpoint runs in FastAPI's
    ...
    with kanban_db.scoped_current_board(board or kanban_db.DEFAULT_BOARD):
        ...
        from moor_cli import kanban_specify  # noqa: WPS433 (intentional)
...
def reassign_task_endpoint(
    ...
    Maps 1:1 to ``moor kanban reassign <task_id> <profile> [--reclaim]``.
    """
    board = _resolve_board(board)
    conn = _conn(board=board)
    try:
...
@router.get("/config")
def get_config():
    """Return kanban dashboard preferences from ~/.moor/config.yaml.
...
def _active_profile_name() -> str:
    """Return the current Moor profile name for notify-sub ownership."""
    try:
...
def get_assignees(board: Optional[str] = Query(None)):
    ...
    Returns the union of ``~/.moor/profiles/*`` on disk and every
...
def model_options():
    ...
    Thin wrapper around ``moor_cli.inventory.build_models_payload`` — the
...
def update_profile_description(profile_name: str, payload: DescribeBody):
    ...
    try:
        ...
        if canon == "default":
            from moor_constants import get_moor_home  # type: ignore
            from pathlib import Path as _Path
            profile_dir = _Path(get_moor_home())
        else:
...
def auto_describe_profile(profile_name: str, payload: DescribeAutoBody):
    ...
    Maps 1:1 to ``moor profile describe <name> --auto``. Non-OK
    ...
    """
    try:
        from moor_cli import profile_describer  # noqa: WPS433 (intentional)
...
def decompose_task_endpoint(
    ...
    1:1 to ``moor kanban decompose <task_id>``.
    ...
    # MOOR_KANBAN_BOARD env var would let concurrent requests for
    # different boards race and cross-write (issue #38323).
    with kanban_db.scoped_current_board(board or kanban_db.DEFAULT_BOARD):
        from moor_cli import kanban_decompose  # noqa: WPS433 (intentional)
...
def set_orchestration_settings(payload: OrchestrationSettingsBody):
    """Update the kanban orchestration knobs in ~/.moor/config.yaml.
