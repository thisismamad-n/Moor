/**
 * Moor Kanban — Dashboard Plugin
 *
 * Board view for the multi-agent collaboration board backed by
 * ~/.moor/kanban.db. Calls the plugin's backend at /api/plugins/kanban/
 * and tails task_events over a WebSocket for live updates.
 *
 * Plain IIFE, no build step. Uses window.__MOOR_PLUGIN_SDK__ for React +
...
(function () {
  ...
  const SDK = window.__MOOR_PLUGIN_SDK__;
  if (!SDK) return;
  ...
  const COLUMN_DOT = {
    triage: "moor-kanban-dot-triage",
    todo: "moor-kanban-dot-todo",
    ready: "moor-kanban-dot-ready",
    running: "moor-kanban-dot-running",
    blocked: "moor-kanban-dot-blocked",
    review: "moor-kanban-dot-review",
    done: "moor-kanban-dot-done",
    archived: "moor-kanban-dot-archived",
  };
  ...
  const MIME_TASK = "text/x-moor-task";
  ...
  const LS_BOARD_KEY = "moor.kanban.selectedBoard";
  ...
  function renderMarkdown(src) {
    ...
    html = html.replace(/\u0000CODE(\d+)\u0000/g, (_m, i) =>
      `<pre class="moor-kanban-md-code"><code>${escapeHtml(blocks[Number(i)])}</code></pre>`,
    );
    return html;
  }
  ...
  function sanitizeMarkdownAttrs(tag, attrs) {
    ...
    if (tag === "pre" && /\sclass=(["'])moor-kanban-md-code\1/i.test(attrs)) {
      return ' class="moor-kanban-md-code"';
    }
    return "";
  }
  ...
  function MarkdownBlock(props) {
    ...
    if (!enabled) {
      return h("pre", { className: "moor-kanban-pre" }, props.source || "");
    }
    return h("div", {
      className: "moor-kanban-md",
  ...
  // standard `drop` event and our `moor-kanban:drop` event.
  // -------------------------------------------------------------------------
    ...
    function onDown(e) {
      ...
      function move(ev) {
        ...
        if (target !== lastTarget) {
          if (lastTarget) lastTarget.classList.remove("moor-kanban-column--drop");
          if (target) target.classList.add("moor-kanban-column--drop");
          lastTarget = target;
        }
      }
      function up() {
        ...
        if (lastTarget) {
          lastTarget.classList.remove("moor-kanban-column--drop");
          const status = lastTarget.getAttribute("data-kanban-column");
          const isTrash = lastTarget.hasAttribute("data-kanban-trash");
          if (isTrash) {
            lastTarget.dispatchEvent(new CustomEvent("moor-kanban:delete", {
              detail: { taskId },
              bubbles: true,
            }));
          } else if (status) {
            lastTarget.dispatchEvent(new CustomEvent("moor-kanban:drop", {
  ...
  function KanbanPage() {
    ...
    useEffect(function () {
      ...
      function openWs() {
        ...
        // loopback. Reading window.__MOOR_SESSION_TOKEN__ directly (the old
   ...
   const deleteTask = useCallback(function (taskId) {
    ...
    return h(ErrorBoundary, null,
      h("div", { className: "moor-kanban flex flex-col gap-4" },
        h(BoardSwitcher, {
          board: board,
          boardList: boardList,
  ...
  function AttentionStrip(props) {
    ...
    return h("div", {
      className: cn(
        "moor-kanban-attention",
        "moor-kanban-attention--" + topSev,
      ),
    },
      h("div", { className: "moor-kanban-attention-bar" },
        h("span", { className: "moor-kanban-attention-icon" },
          topSev === "critical" ? "!!!" : topSev === "error" ? "!!" : "⚠"),
        h("span", { className: "moor-kanban-attention-text" },
          diagTasks.length === 1
      ...
      expanded
        ? h("div", { className: "moor-kanban-attention-list" },
            diagTasks.map(function (task) {
              ...
              return h("div", {
                key: task.id,
                className: cn(
                  "moor-kanban-attention-row",
                  "moor-kanban-attention-row--" + sev,
                ),
              },
                h("span", { className: "moor-kanban-attention-row-sev" },
                  sev === "critical" ? "!!!" : sev === "error" ? "!!" : "⚠"),
                h("span", { className: "moor-kanban-attention-row-id" }, task.id),
                h("span", { className: "moor-kanban-attention-row-title" },
                  task.title || tx(t, "untitled", "(untitled)")),
                h("span", { className: "moor-kanban-attention-row-meta" },
                  task.assignee ? "@" + task.assignee : tx(t, "unassigned", "unassigned"),
                  " \u00b7 ",
                  kinds.length > 0 ? kinds.join(", ") : tx(t, "diagnostic", "diagnostic"),
                ),
                h("button", {
                  className: "moor-kanban-attention-row-btn",
  ...
  function DiagnosticActionButton(props) {
    ...
    const cls = cn(
      "moor-kanban-diag-action-btn",
      action.suggested ? "moor-kanban-diag-action-btn--suggested" : "",
    );
    ...
    return h("span", { className: cls + " moor-kanban-diag-action-btn--unknown" },
      label);
  }
  ...
  function DiagnosticCard(props) {
    ...
    const execAction = function (action) {
      ...
      if (action.kind === "comment") {
        ...
        const ta = document.querySelector(".moor-kanban-drawer-comment-row input, .moor-kanban-drawer-comment-row textarea");
        if (ta) {
    ...
    const sevClass = "moor-kanban-diag--" + (diag.severity || "warning");
    return h("div", { className: cn("moor-kanban-diag", sevClass) },
      h("div", { className: "moor-kanban-diag-header" },
        h("span", { className: "moor-kanban-diag-sev" },
          diag.severity === "critical" ? "!!!" :
          diag.severity === "error" ? "!!" : "\u26a0"),
        h("span", { className: "moor-kanban-diag-title" },
          diag.title),
      ),
      h("div", { className: "moor-kanban-diag-detail" },
        diag.detail),
      diag.data && Object.keys(diag.data).length > 0
        ? h("div", { className: "moor-kanban-diag-data" },
            Object.keys(diag.data).map(function (k) {
              const v = diag.data[k];
              if (Array.isArray(v) && v.length > 0 && typeof v[0] === "string" &&
                  v[0].indexOf("t_") === 0) {
                // Task-id list — render as chips.
                return h("div", { key: k, className: "moor-kanban-diag-data-row" },
                  h("span", { className: "moor-kanban-diag-data-key" }, k + ":"),
                  v.map(function (x) {
                    return h("code", {
                      key: x, className: "moor-kanban-event-phantom-chip",
                    }, x);
                  }),
                );
              }
              return h("div", { key: k, className: "moor-kanban-diag-data-row" },
                h("span", { className: "moor-kanban-diag-data-key" }, k + ":"),
                h("span", { className: "moor-kanban-diag-data-val" },
      ...
      reassignAction
        ? h("div", { className: "moor-kanban-diag-reassign-row" },
            h("span", { className: "moor-kanban-diag-reassign-label" },
              tx(t, "reassignTo", "Reassign to:")),
            h("select", {
              className: "moor-kanban-recovery-select",
        ...
        : null,
      h("div", { className: "moor-kanban-diag-actions" },
      ...
      msg
        ? h("div", {
            className: cn(
              "moor-kanban-diag-msg",
              msg.ok ? "moor-kanban-diag-msg--ok" : "moor-kanban-diag-msg--err",
            ),
          }, msg.text)
        : null,
    );
  ...
  function DiagnosticsSection(props) {
    ...
    return h("div", { className: "moor-kanban-section" },
      h("div", { className: "moor-kanban-section-head-row" },
        h("span", { className: "moor-kanban-section-head" },
          hasOpenDiags
            ? h("span", { className: "moor-kanban-section-head-warning" },
                `\u26a0 ${tx(t, "diagnostics", "Diagnostics")} (${diags.length})`)
            : tx(t, "diagnostics", "Diagnostics"),
        ),
        h("button", {
          className: "moor-kanban-section-toggle",
          onClick: function () { setOpen(function (x) { return !x; }); },
          type: "button",
        }, open ? tx(t, "hide", "Hide") : tx(t, "show", "Show")),
      ),
      open
        ? h("div", { className: "moor-kanban-diag-list" },
  ...
  function DocsLink() {
    return h("a", {
      ...
      className: "moor-kanban-docs-link",
      title: "Open Moor Kanban docs in a new tab",
      "aria-label": "Moor Kanban documentation",
    }, "?");
  }
  ...
  function OrchestrationPanel() {
    ...
    // each triage card (or runs `moor kanban decompose <id>`) and tasks
    ...
    return h(Card, { className: "p-3" },
      h(CardContent, { className: "p-2 flex flex-col gap-3" },
        ...
        msg ? h("div", {
          className: msg.ok ? "moor-kanban-msg-ok" : "moor-kanban-msg-err",
        }, msg.text) : null,
  ...
  function BoardSwitcher(props) {
    ...
    if (!shouldShow) {
      return h("div", {
        className: "moor-kanban-boardswitcher-compact",
    ...
    return h("div", { className: "moor-kanban-boardswitcher" },
      h("div", { className: "moor-kanban-boardswitcher-inner" },
  ...
  function NewBoardDialog(props) {
    ...
    return h("div", {
      className: "moor-kanban-dialog-backdrop",
      onClick: function (e) { if (e.target === e.currentTarget) props.onCancel(); },
    },
      h("form", {
        className: "moor-kanban-dialog",
        onSubmit: onSubmit,
      },
        h("div", { className: "moor-kanban-dialog-title" },
        ...
        h("div", { className: "moor-kanban-dialog-actions" },
          h(Button, {
            type: "button",
  ...
  function BoardToolbar(props) {
    ...
    return h("div", { className: "flex flex-wrap items-end gap-3" },
      ...
      h("div", { className: "flex flex-col gap-1",
                 title: "Filter by assigned Moor profile. Profiles are the named agent identities that claim and work on tasks." },
  ...
  function BulkActionBar(props) {
    ...
    return h("div", { className: "moor-kanban-bulk" },
      h("span", { className: "moor-kanban-bulk-count" },
      ...
      h("div", { className: "moor-kanban-bulk-priority",
      ...
      h("div", { className: "moor-kanban-bulk-reassign",
                 title: "Reassign selected tasks to a different Moor profile. Pick a profile (or unassign) and click Apply." },
        h(Select, Object.assign({
          value: assignee,
          className: "h-7 text-xs",
      ...
      h("label", { className: "moor-kanban-bulk-reclaim-first", title: "Reclaim any active claims before reassigning" },
        h(Checkbox, {
  ...
  function TrashDropZone(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("moor-kanban:delete", onTouchDelete);
      return function () { el.removeEventListener("moor-kanban:delete", onTouchDelete); };
    }, [props.onDelete]);
    ...
    return h("div", {
      ...
      className: cn(
        "moor-kanban-trash",
        dragOver ? "moor-kanban-trash--drop" : "",
        props.draggingTaskId ? "moor-kanban-trash--active" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("span", { className: "moor-kanban-trash-icon" }, "🗑️"),
      h("span", { className: "moor-kanban-trash-label" },
  ...
  function BoardColumns(props) {
    ...
    const isPanBlockedTarget = useCallback(function (target) {
      ...
      if (target.closest && target.closest(".moor-kanban-card")) return true;
      if (target.closest && target.closest(".moor-kanban-column-add")) return true;
      if (target.closest && target.closest(".moor-kanban-col-check")) return true;
    ...
    const stopPan = useCallback(function () {
      ...
      if (el) {
        ...
        el.classList.remove("moor-kanban-columns--panning");
        el.style.userSelect = "";
      }
    ...
    const handleMouseDown = useCallback(function (e) {
      ...
      setIsPanning(true);
      el.classList.add("moor-kanban-columns--panning");
    ...
    const handleDragStart = useCallback(function (e) {
      const card = e.target.closest && e.target.closest(".moor-kanban-card");
      if (!card) return;
    ...
    return h("div", {
      ...
      className: cn(
        "moor-kanban-columns",
        isScrollable ? "moor-kanban-columns--scrollable" : "",
        isPanning ? "moor-kanban-columns--panning" : "",
      ),
      onDragStart: handleDragStart,
      onDragEnd: handleDragEnd,
  ...
  function Column(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("moor-kanban:drop", onTouchDrop);
      return function () { el.removeEventListener("moor-kanban:drop", onTouchDrop); };
    ...
    return h("div", {
      ...
      className: cn(
        "moor-kanban-column",
        dragOver ? "moor-kanban-column--drop" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("div", { className: "moor-kanban-column-header",
                 title: colHelp || "" },
        h(Checkbox, {
          className: "moor-kanban-col-check",
        ...
        h("span", { className: cn("moor-kanban-dot", COLUMN_DOT[props.column.name]) }),
        h("span", { className: "moor-kanban-column-label" },
          colLabel || props.column.name),
        h("span", { className: "moor-kanban-column-count",
                    title: `${props.column.tasks.length} task${props.column.tasks.length === 1 ? "" : "s"} in this column` },
          props.column.tasks.length),
        h("button", {
          type: "button",
          className: "moor-kanban-column-add",
          title: tx(t, "createTask", "Create task in this column"),
          onClick: function () { setShowCreate(function (v) { return !v; }); },
        }, showCreate ? "×" : "+"),
      ),
      h("div", { className: "moor-kanban-column-sub" },
        colHelp || ""),
      ...
      h("div", { className: "moor-kanban-column-body" },
        props.column.tasks.length === 0
          ? h("div", { className: "moor-kanban-empty" }, tx(t, "noTasks", "— no tasks —"))
          : lanes
            ? lanes.map(function (lane) {
                return h("div", { key: lane.assignee, className: "moor-kanban-lane" },
                  h("div", { className: "moor-kanban-lane-head" },
                    h("span", { className: "moor-kanban-lane-name" }, lane.assignee),
                    h("span", { className: "moor-kanban-lane-count" }, lane.tasks.length),
                  ),
  ...
  function stalenessClass(task) {
    ...
    if (age >= tier.red)   return "moor-kanban-card--stale-red";
    if (age >= tier.amber) return "moor-kanban-card--stale-amber";
    return "";
  }
  ...
  function TaskCard(props) {
    ...
    const handleDragStart = function (e) {
      ...
      const selectedCards = document.querySelectorAll(".moor-kanban-card--selected");
      if (selectedCards.length > 1 && props.selected) {
        const ghost = document.createElement("div");
        ghost.className = "moor-kanban-drag-ghost";
    ...
    return h("div", {
      ...
      className: cn(
        "moor-kanban-card",
        props.selected ? "moor-kanban-card--selected" : "",
        props.failed ? "moor-kanban-card--failed" : "",
        props.draggingSource ? "moor-kanban-card--dragging-source" : "",
        stalenessClass(t),
      ),
      draggable: true,
      tabIndex: 0,
      ...
      h(Card, null,
        h(CardContent, { className: "moor-kanban-card-content" },
          h("div", { className: "moor-kanban-card-row" },
            h("label", {
              className: "moor-kanban-card-check-wrap",
              title: tx(i18n, "selectForBulk", "Select for bulk actions"),
              onClick: function (e) { e.stopPropagation(); },
            },
              h(Checkbox, {
                className: "moor-kanban-card-check",
            ...
            h("span", { className: "moor-kanban-card-id",
                        title: `Task id: ${t.id}. Use this id with kanban_show, /kanban show, or moor kanban show.` }, t.id),
            t.warnings && t.warnings.count > 0
              ? h("span", {
                  className: cn(
                    "moor-kanban-warning-badge",
                    "moor-kanban-warning-badge--" + (t.warnings.highest_severity || "warning"),
                  ),
                  title: (
            ...
            needsAssignee
              ? h(Badge, {
                  variant: "outline",
                  className: "moor-kanban-needs-assignee",
          ...
          h("div", { className: "moor-kanban-card-title" },
            t.title || tx(i18n, "untitled", "(untitled)")),
          h("div", { className: "moor-kanban-card-row moor-kanban-card-meta" },
            t.assignee
              ? h("span", { className: "moor-kanban-assignee",
                            title: `Assigned to Moor profile @${t.assignee}` }, "@", t.assignee)
              : h("span", { className: "moor-kanban-unassigned",
                            title: needsAssignee
            ...
            t.comment_count > 0
              ? h("span", { className: "moor-kanban-count",
            ...
            t.link_counts && (t.link_counts.parents + t.link_counts.children) > 0
              ...
              : null,
            h("span", { className: "moor-kanban-ago",
  ...
  // centered modal reusing the moor-kanban-dialog chrome so the form is
  ...
  function InlineCreate(props) {
    ...
    return h("div", {
      ...
      h("form", {
        className: "moor-kanban-dialog moor-kanban-create-dialog",
            ...
            h("div", { className: "flex flex-col gap-1 flex-1" },
              ...
              h(Input, {
                ...
                title: props.columnName === "triage"
                  ? "Moor profile that will spec this task (default: the dispatcher's configured specifier). Leave blank to let the dispatcher pick."
                  : "Moor profile to assign. Leave blank and the dispatcher will pick from available profiles when the task is Ready.",
  ...
  function TaskDrawer(props) {
    ...
    return h("div", { className: "moor-kanban-drawer-shade", onClick: props.onClose },
      h("div", {
        className: "moor-kanban-drawer",
        onClick: function (e) { e.stopPropagation(); },
      },
        h("div", { className: "moor-kanban-drawer-head" },
          h("span", { className: "text-xs text-muted-foreground" }, props.taskId),
          h("button", {
            type: "button",
            onClick: props.onClose,
            className: "moor-kanban-drawer-close",
        ...
        data ? h("div", { className: "moor-kanban-drawer-comment-foot" },
          h("div", {
            className: "moor-kanban-comment-hint text-xs text-muted-foreground",
          ...
          h("div", { className: "moor-kanban-drawer-comment-row" },
            h(Input, {
              value: newComment,
  ...
  function AttachmentsSection(props) {
    ...
    return h("div", { className: "moor-kanban-section" },
      h("div", { className: "moor-kanban-section-head" },
      ...
      atts.length === 0
        ...
        : atts.map(function (a) {
            return h("div", {
              ...
              h("button", {
                type: "button",
                className: "moor-kanban-attachment-link truncate",
                title: a.filename,
  ...
  function TaskDetail(props) {
    ...
    return h("div", { className: "moor-kanban-drawer-body" },
      h("div", { className: "moor-kanban-drawer-title" },
        h("span", { className: cn("moor-kanban-dot", COLUMN_DOT[t.status]) }),
        props.editing
          ? h(TitleEditor, {
              initial: t.title || "",
          ...
          : h("span", {
              className: "moor-kanban-drawer-title-text",
              title: tx(i18n, "clickToEdit", "Click to edit"),
              onClick: function () { props.setEditing(true); },
            }, t.title || tx(i18n, "untitled", "(untitled)")),
      ),
      h("div", { className: "moor-kanban-drawer-meta" },
      ...
      (function () {
        ...
        if (finalResult) {
          ...
          return h("div", { className: "moor-kanban-section" },
            h("div", { className: "moor-kanban-section-head" }, label),
            h(MarkdownBlock, { source: finalResult, enabled: props.renderMarkdown }),
          );
        }
        if (isDone && isParent) {
          return h("div", { className: "moor-kanban-section" },
            h("div", { className: "moor-kanban-section-head" }, tx(i18n, "result", "Result")),
            h("div", { className: "moor-kanban-done-no-result moor-kanban-done-parent-note" },
        ...
        if (isDone) {
          return h("div", { className: "moor-kanban-section" },
            h("div", { className: "moor-kanban-section-head" }, tx(i18n, "result", "Result")),
            h("div", { className: "moor-kanban-done-no-result" },
      ...
      childResults.length > 0 ? h("div", { className: "moor-kanban-section" },
        h("div", { className: "moor-kanban-section-head" },
        ...
        childResults.map(function (child) {
          ...
          return h("div", { key: child.id, className: "moor-kanban-comment" },
            h("div", { className: "moor-kanban-comment-head" },
              h("span", { className: "moor-kanban-comment-author" },
                `${child.id} · ${child.title || tx(i18n, "untitled", "(untitled)")}`),
              h(Badge, { variant: "outline" }, child.status),
              h("button", {
                type: "button",
                className: "moor-kanban-diag-action-btn",
      ...
      h("div", { className: "moor-kanban-section" },
        h("div", { className: "moor-kanban-section-head" },
        ...
        comments.length === 0
          ...
          : comments.map(function (c) {
              return h("div", { key: c.id, className: "moor-kanban-comment" },
                h("div", { className: "moor-kanban-comment-head" },
                  h("span", { className: "moor-kanban-comment-author" }, c.author || "anon"),
                  h("span", { className: "moor-kanban-comment-ago" },
      ...
      h("div", { className: "moor-kanban-section" },
        ...
        events.slice().reverse().slice(0, 20).map(function (e) {
          ...
          return h("div", {
            ...
            className: cn(
              "moor-kanban-event",
              isDiag ? "moor-kanban-event--hallucination" : "",
            ),
          },
            isDiag
              ? h("div", { className: "moor-kanban-event-header" },
                  h("span", { className: "moor-kanban-event-warning-icon" }, "⚠"),
                  h("span", { className: "moor-kanban-event-warning-label" },
                    getDiagnosticEventLabel(i18n, e.kind) || e.kind),
                  h("span", { className: "moor-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                )
              : h("div", { className: "moor-kanban-event-header-plain" },
                  h("span", { className: "moor-kanban-event-kind" }, e.kind),
                  h("span", { className: "moor-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                ),
            isDiag && phantoms.length > 0
              ? h("div", { className: "moor-kanban-event-phantom-row" },
                  h("span", { className: "moor-kanban-event-phantom-label" },
                    tx(i18n, "phantomIds", "Phantom ids:")),
                  phantoms.map(function (pid) {
                    return h("code", {
                      key: pid,
                      className: "moor-kanban-event-phantom-chip",
                    }, pid);
                  }),
                )
              : null,
            e.payload && !isDiag
              ? h("code", { className: "moor-kanban-event-payload" },
    ...
    return h("div", { className: "moor-kanban-section" },
      h("div", { className: "moor-kanban-section-head-row" },
        ...
        !showAll
          ? h("button", {
              type: "button",
              onClick: function () { setExpanded(true); },
              className: "moor-kanban-edit-link",
      ...
      visible.map(function (r) {
        const outcomeClass = r.ended_at
          ? `moor-kanban-run--${r.outcome || r.status || "ended"}`
          : "moor-kanban-run--active";
        return h("div", { key: r.id, className: cn("moor-kanban-run", outcomeClass) },
          h("div", { className: "moor-kanban-run-head" },
            h("span", { className: "moor-kanban-run-outcome" },
              r.ended_at ? (r.outcome || r.status || tx(t, "ended", "ended")) : tx(t, "active", "active")),
            h("span", { className: "moor-kanban-run-profile" },
              r.profile ? `@${r.profile}` : tx(t, "noProfile", "(no profile)")),
            h("span", { className: "moor-kanban-run-elapsed" }, fmtElapsed(r)),
            h("span", { className: "moor-kanban-run-ago" },
              timeAgo ? timeAgo(r.started_at) : ""),
          ),
          r.summary
            ? h("div", { className: "moor-kanban-run-summary" }, r.summary)
            : null,
          r.error
            ? h("div", { className: "moor-kanban-run-error" }, r.error)
            : null,
          (r.metadata && Object.keys(r.metadata).length > 0)
            ? (function () {
                ...
                return h("details", {
                    className: "moor-kanban-run-meta-block",
                    open: !collapsed,
                  },
                  h("summary", { className: "moor-kanban-run-meta-label" }, "Metadata"),
                  h("code", { className: "moor-kanban-run-meta" }, json),
                );
              })()
            : null,
        );
  ...
  function WorkerLogSection(props) {
    ...
    if (state.loading) {
      ...
      body = h("pre", { className: "moor-kanban-pre moor-kanban-log" },
        data.content || "(empty)");
    }
  ...
  function MetaRow(props) {
    return h("div", { className: "moor-kanban-meta-row" },
      h("span", { className: "moor-kanban-meta-label" }, props.label),
      h("span", { className: "moor-kanban-meta-value" }, props.value),
    );
  }
  ...
  function TitleEditor(props) {
    ...
    return h("div", { className: "moor-kanban-edit-row" },
      h(Input, {
  ...
  function AssigneeEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "moor-kanban-meta-row" },
        h("span", { className: "moor-kanban-meta-label" }, tx(t, "assignee", "Assignee")),
        h("span", {
          className: "moor-kanban-meta-value moor-kanban-editable",
  ...
  function PriorityEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "moor-kanban-meta-row" },
        h("span", { className: "moor-kanban-meta-label" }, tx(t, "priority", "Priority")),
        h("span", {
          className: "moor-kanban-meta-value moor-kanban-editable",
          onClick: function () { setEditing(true); },
  ...
  function ModelEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "moor-kanban-meta-row" },
        h("span", { className: "moor-kanban-meta-label" }, tx(t, "model", "Model")),
        h("span", {
          className: cn(
            "moor-kanban-meta-value moor-kanban-editable",
    ...
    return h("div", { className: "moor-kanban-meta-row" },
      h("span", { className: "moor-kanban-meta-label" }, tx(t, "model", "Model")),
      loading
        ? h("span", { className: "moor-kanban-meta-value text-muted-foreground" },
  ...
  function BodyEditor(props) {
    ...
    return h("div", { className: "moor-kanban-section" },
      h("div", { className: "moor-kanban-section-head-row" },
        h("span", { className: "moor-kanban-section-head" }, tx(t, "description", "Description")),
        editing
      ...
      editing
        ? h("textarea", {
            className: "moor-kanban-textarea",
            value: v,
            rows: 8,
            onChange: function (e) { setV(e.target.value); },
          })
  ...
  function DependencyEditor(props) {
    ...
    return h("div", { className: "moor-kanban-section" },
      h("div", { className: "moor-kanban-section-head" }, tx(t, "dependencies", "Dependencies")),
      h("div", { className: "moor-kanban-deps-row" },
        h("span", { className: "moor-kanban-deps-label" }, tx(t, "parents", "Parents:")),
        h("div", { className: "moor-kanban-deps-chips" },
          (links.parents || []).length === 0
            ? h("span", { className: "moor-kanban-deps-empty" }, tx(t, "none", "none"))
            : (links.parents || []).map(function (id) {
                return h("span", { key: id, className: "moor-kanban-dep-chip" },
                  id,
                  h("button", {
                    type: "button",
                    className: "moor-kanban-dep-chip-x",
      ...
      h("div", { className: "moor-kanban-deps-row" },
        h("span", { className: "moor-kanban-deps-label" }, tx(t, "children", "Children:")),
        h("div", { className: "moor-kanban-deps-chips" },
  ...
  function StatusActions(props) {
    ...
    return h("div", null,
      h("div", { className: "moor-kanban-actions" },
        specifyButton,
        decomposeButton,
  ...
  function HomeSubsSection(props) {
    ...
    return h("div", { className: "moor-kanban-section" },
      ...
      h("div", { className: "moor-kanban-home-subs" },
        channels.map(function (hc) {
          ...
          return h(Button, {
            ...
            className: hc.subscribed
              ? "moor-kanban-home-sub moor-kanban-home-sub--on"
              : "moor-kanban-home-sub",
          }, label);
        })
      )
    );
  ...
  if (window.__MOOR_PLUGINS__ && typeof window.__MOOR_PLUGINS__.register === "function") {
    window.__MOOR_PLUGINS__.register("kanban", KanbanPage);
  }
})();
