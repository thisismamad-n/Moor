"""Meta Model API (Muse Spark) provider plugin for Moor Agent.
...
Bundled from https://github.com/albertodepaola/moor-meta-provider. Moor'
...
* **Zero core edits.** Everything rides on ``ProviderProfile`` hooks. No changes
  to moor' ``model_metadata.py`` / ``models.py`` / ``run_agent.py`` are needed:
...
def _resolve_effort(reasoning_config: dict | None) -> str:
    """Map Moor' reasoning_config to a Meta-safe ``reasoning_effort`` value.
...
meta_ai = MetaAIProfile(
    ...
    # in moor_cli/providers.py and the retention hint in
