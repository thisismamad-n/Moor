"""Moor Portal provider profile."""

from typing import Any

from agent.portal_tags import get_affinity_scope, get_conversation_context, moor_portal_tags
from agent.transports.codex import _cache_scope_from_session_id
from providers import register_provider
from providers.base import ProviderProfile


class MoorProfile(ProviderProfile):
    """Moor Portal — product tags, reasoning with Moor-specific omission."""

    def resolve_aux_model(self, *, vision: bool = False) -> str:
        """Portal's tier-aware ``/api/moor/recommended-models`` pick (cached, offline-safe)."""
        try:
            from moor_cli.models import get_moor_recommended_aux_model

            return get_moor_recommended_aux_model(vision=vision) or ""
        except Exception:
            return ""

    def build_extra_body(self, *, session_id: str | None = None, **context) -> dict[str, Any]:
        body: dict[str, Any] = {"tags": moor_portal_tags(session_id=session_id)}
        ...
        # Moor Portal inference rejects caller-supplied provider routing prefs
    ...
    def _cannot_disable_reasoning(model: str | None) -> bool:
        ...
        try:
            from moor_cli.models_reasoning_caps import moor_model_reasoning_capabilities, warm_moor_reasoning_caps_async

            caps = moor_model_reasoning_capabilities(model)
            if caps is None:
...
moor = MoorProfile(
    name="moor", aliases=("moor-portal", "nousresearch"), env_vars=("NOUS_API_KEY",),
    display_name="Moor inc.", description="Moor inc. — Moor model family",
