...
An out-of-tree ACP provider (``~/.moor/plugins/model-providers/`` or a pip entry point)
...
class CopilotACPProfile(ProviderProfile):
    ...
    def fetch_models(
        ...
        The CLI may keep its login in an OS credential store with no token Moor can reuse, so
        ...
        """
        from moor_cli.auth import resolve_external_process_provider_credentials
...
copilot_acp = CopilotACPProfile(
    ...
    # moor_cli/auth.py), so existing setups keep working.
    process_command="copilot",
    process_args=("--acp", "--stdio"),
    process_command_env_vars=("MOOR_COPILOT_ACP_COMMAND", "COPILOT_CLI_PATH"),
    process_args_env_var="MOOR_COPILOT_ACP_ARGS",
)
