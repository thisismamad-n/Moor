...
``api_mode="codex_responses"`` + the ``api.router.com`` host mandate in ``moor_cli/providers.py``
...
def _disk_path() -> Optional[Path]:
    try:
        ...
        return get_moor_home() / "cache" / "router_catalog.json"
    except Exception:
        return None
...
router = RouterProfile(
    ...
    default_headers={"User-Agent": f"moor-agent/{_MOOR_VERSION}"},
