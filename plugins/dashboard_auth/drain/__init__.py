...
What it is
----------
A service-to-service auth provider. ``moor-account-service`` (NAS) provisions a
...
for-secrets-only rule), provisioned by NAS at deploy time (Phase 3):
    ...
    MOOR_DASHBOARD_DRAIN_SECRET   # the per-agent shared secret (>=43 url-safe-b64 chars)
...
When ``MOOR_DASHBOARD_DRAIN_SECRET`` is unset, the plugin is a no-op (records
...
def register(ctx) -> None:
    ...
    No-op (records a skip reason) when ``MOOR_DASHBOARD_DRAIN_SECRET`` is
    ...
    if not secret:
        LAST_SKIP_REASON = (
            "MOOR_DASHBOARD_DRAIN_SECRET is not set. Set a per-agent "
    ...
    if reason is not None:
        LAST_SKIP_REASON = (
            f"MOOR_DASHBOARD_DRAIN_SECRET rejected — {reason}. "
