...
A standards-compliant OpenID Connect Relying Party for the ``moor dashboard``
OAuth gate. Unlike the bundled ``moor`` provider (which encodes Moor Portal's
bespoke contract — ``agent:{instance_id}`` client ids, a custom access-token
JWT, the ``x-moor-refresh-token`` header, an ``oauth_contract_version`` claim),
...
:class:`~moor_cli.dashboard_auth.DashboardAuthProvider` methods and touches
...
``moor_cli/dashboard_auth/routes.py``; this provider only:
  ...
  4. verifies the **ID token** (RS256/ES256) against the discovered
     ...
     onto a :class:`~moor_cli.dashboard_auth.Session`.
...
only choice that is universally correct across self-hosted IDPs. (The ``moor``
provider verifies its *access* token because Moor Portal mints a custom JWT
...
same precedence convention as the ``moor`` plugin)::
    ...
    dashboard:
      oauth:
        ...
        self_hosted:
          ...
          # credential — prefer the env var / ~/.moor/.env over config.yaml.

    # Environment overrides (Docker/Fly secret injection)
    MOOR_DASHBOARD_OIDC_ISSUER
    MOOR_DASHBOARD_OIDC_CLIENT_ID
    MOOR_DASHBOARD_OIDC_SCOPES        # optional; defaults to "openid profile email"
    MOOR_DASHBOARD_OIDC_CLIENT_SECRET # optional; set for a confidential client
...
# JWKS cache (PyJWKClient handles its own caching; this mirrors the moor
# provider's 5-minute lifespan so key rotation is picked up promptly).
_JWKS_CACHE_SECONDS = 300


# ---------------------------------------------------------------------------
# Skip-reason channel (mirrors the moor plugin)
# ---------------------------------------------------------------------------
...
class SelfHostedOIDCProvider(DashboardAuthProvider):
    ...
    def _session_from_tokens(
        ...
        *,
        ...
        OAuth access token is intentionally NOT stored — Moor does not call
    ...
    def _validate_redirect_uri(self, redirect_uri: str) -> None:
        ...
        redirect_uris are permitted. Mirrors the moor provider.
        """
...
def register(ctx) -> None:
    ...
    client_id are configured (via ``MOOR_DASHBOARD_OIDC_*`` env vars or the
    ...
    issuer = _resolve_setting(
        "MOOR_DASHBOARD_OIDC_ISSUER", oidc_cfg.get("issuer")
    )
    client_id = _resolve_setting(
        "MOOR_DASHBOARD_OIDC_CLIENT_ID", oidc_cfg.get("client_id")
    )
    scopes = (
        _resolve_setting("MOOR_DASHBOARD_OIDC_SCOPES", oidc_cfg.get("scopes"))
        or _DEFAULT_SCOPES
    )
    # Optional — set only for a confidential client. A credential, so the
    # canonical home is the env var / ~/.moor/.env; config.yaml is supported
    # for precedence symmetry. Empty ⇒ public client (unchanged behaviour).
    client_secret = _resolve_setting(
        "MOOR_DASHBOARD_OIDC_CLIENT_SECRET", oidc_cfg.get("client_secret")
    )
    ...
    if not issuer or not client_id:
        LAST_SKIP_REASON = (
            ...
            "(MOOR_DASHBOARD_OIDC_ISSUER + MOOR_DASHBOARD_OIDC_CLIENT_ID) "
