...
# JWKS Cache-Control max-age (moor contract C7); self-hosted mirrors it.
...
def pkce_login_start(authorize_url: str, *, client_id: str, scope: str, redirect_uri: str) -> LoginStart:
    ...
    ``cookie_payload["moor_session_pkce"]`` as a flat ``state=…;verifier=…`` string
    ...
    return LoginStart(
        ...
        cookie_payload={"moor_session_pkce": f"state={state};verifier={code_verifier}"})
...
class JwtOAuthProvider(DashboardAuthProvider):
    ...
    (moor: Portal access token; self-hosted: OIDC ID token). Subclasses set ``_client_id`` and
