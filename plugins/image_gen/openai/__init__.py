...
three virtual model IDs so the ``moor tools`` model picker and the
    ...
    def generate(
        ...
        **kwargs: Any,
        ...
        if not api_key:
            return error_response(
                error=(
                    "OPENAI_API_KEY not set. Run `moor tools` → Image "
                    "Generation → OpenAI to configure, or `moor setup` "
                    "to add the key."
                ),
