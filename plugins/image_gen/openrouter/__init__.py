...
The Image API is OpenRouter-only: Moor Portal proxies the chat-completions
protocol and has no ``/images/generations`` route, so the second surface is
enabled per provider (see ``supports_image_api``) and stays off for Moor.
"""
...
class OpenRouterCompatImageProvider(ImageGenProvider):
    ...
    def __init__(
        ...
        *,
        ...
        # OpenRouter only: Moor Portal proxies the chat-completions protocol
    ...
    def list_models(self) -> List[Dict[str, Any]]:
        ...
        after this code shipped — is selectable in ``moor tools``. Moor
