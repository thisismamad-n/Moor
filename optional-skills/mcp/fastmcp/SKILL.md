...
The server-building part may be correct while the Moor config is not. Load the `native-mcp` skill and configure the server in `~/.hermes/config.yaml`, then restart Moor.
