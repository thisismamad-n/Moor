...
curl -fsSL "https://github.com/ast-grep/ast-grep/releases/download/${VERSION}/app-${TRIPLE}.zip" -o ~/.moor/cache/scratch/ast-grep.zip
unzip ~/.moor/cache/scratch/ast-grep.zip -d ~/.moor/cache/scratch/ast-grep
sudo mv ~/.moor/cache/scratch/ast-grep/ast-grep /usr/local/bin/sg
