...
## 5. GODMODE FAST — Moor 4 405B (Zero Refusal)

**Model:** `NousResearch/hermes-4-405b`
**Strategy:** Moor models are already uncensored, so this combo is about speed — instant token streaming with zero refusal checking needed. Uses the same Pliny Love divider format as a formality.
...
## Using with Moor Agent

### As ephemeral system prompt (config.yaml)

Pick a template above and set it in `~/.moor/config.yaml`:
