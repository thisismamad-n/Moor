...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("MOOR_HOME", os.path.expanduser("~/.moor")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("MOOR_HOME", Path.home() / ".moor")) / "skills" / "red-teaming" / "godmode"
...
MOOR_HOME = Path(os.getenv("MOOR_HOME", Path.home() / ".moor"))
CONFIG_PATH = MOOR_HOME / "config.yaml"
PREFILL_PATH = MOOR_HOME / "prefill.json"
...
MODEL_STRATEGIES = {
    ...
    # Moor/Moor models — already uncensored, just needs clean prompt
    "moor": {
        "order": ["prefill_only"],
        "system_templates": {},
    },
...
def _detect_model_family(model: str) -> str:
    ...
    if "grok" in model_lower or "x-ai" in model_lower:
        return "grok"
    if "moor" in model_lower or "moor" in model_lower:
        return "moor"
...
def _get_current_model() -> tuple:
    """Read current model and provider from Moor config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.moor/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart Moor for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart Moor for changes to take effect.")
