...
All per-subject data lives under PDD_DATA_DIR (default: $MOOR_HOME/unbroker),
which is the same trust boundary Moor uses for .env and OAuth tokens.
"""
...
def moor_home() -> Path:
    return Path(os.environ.get("MOOR_HOME") or (Path.home() / ".moor"))


def data_dir() -> Path:
    override = os.environ.get("PDD_DATA_DIR")
    return Path(override) if override else moor_home() / "unbroker"
