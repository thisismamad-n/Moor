...
MOOR_HOME_DIR="${MOOR_HOME:-$HOME/.moor}"
MOOR_CFG="${MOOR_HOME_DIR}/config.yaml"
MCP_PORT=40404
...
# ── 3. Ensure Moor config has twozero_td MCP entry ──
if [[ ! -f "$MOOR_CFG" ]]; then
    echo -e " ${FAIL} Moor config not found at ${MOOR_CFG}"
    manual_steps+=("Create ${MOOR_CFG} with twozero_td MCP server entry")
elif grep -q 'twozero_td' "$MOOR_CFG" 2>/dev/null; then
    echo -e " ${OK} twozero_td MCP entry exists in Moor config"
else
    echo -e " ${WARN} Adding twozero_td MCP entry to Moor config..."
    python3 -c "
...
" 2>/dev/null && echo -e " ${OK} twozero_td MCP entry added to config" \
              || { echo -e " ${FAIL} Could not update config (is PyYAML installed?)"; \
                   manual_steps+=("Add twozero_td MCP entry to ${MOOR_CFG} manually"); }
    manual_steps+=("Restart Moor session to pick up config change")
fi
