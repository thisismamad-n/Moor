...
### 1. Start order: editor first, Moor second

Moor probes MCP servers at session start. If the editor (and its server)
...
`LogModelContextProtocol` with the address), then open a NEW Moor session.
...
restart the editor, restart the Moor session.
...
### 5. GenerateClientConfig is not for Moor
...
explicitly warns against overlapping calls. Moor executes same-turn tool
...
### 9. Timeouts: Moor gives up before Unreal does

Moor' default per-call timeout is 120 s. Asset imports, first-shader
...
`<Project>/Saved/Screenshots/...`). If Moor runs elsewhere (SSH backend,
