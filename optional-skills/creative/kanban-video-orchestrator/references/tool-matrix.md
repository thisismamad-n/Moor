...
toolsets it needs. Only references skills that ship in the public moor-agent
...
### Visual / rendering skills (`moor-agent/skills/creative/`)
...
| `touchdesigner-mcp` (via the `touchdesigner` plugin: `moor plugins install touchdesigner`) | Control a running TouchDesigner instance — real-time visuals, audio-reactive installation art, VJ | Renderer for real-time/audio-reactive content; installation art; live performance |
...
### Design / pre-production skills (`moor-agent/skills/creative/`)
...
### Audio / media skills (`moor-agent/skills/creative/` + `skills/media/`)
...
key is present in `${MOOR_HOME:-~/.moor}/.env` (or macOS Keychain) before firing the kanban.
...
preference: macOS Keychain → `${MOOR_HOME:-~/.moor}/.env` → environment variable.
