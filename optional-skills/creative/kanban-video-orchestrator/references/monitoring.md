...
moor kanban watch --tenant <project-slug>

# Snapshot of the board
moor kanban list --tenant <project-slug>
moor kanban list --tenant <project-slug> --json     # machine-readable

# Per-status counts + oldest-ready age
moor kanban stats --tenant <project-slug>

# Visual dashboard (browser)
moor dashboard

# Inspect a specific task (includes comments + events)
moor kanban show <task-id>

# Follow a single task's event stream
moor kanban tail <task-id>
```

Verify available subcommands with `moor kanban --help` — the kanban CLI
...
|---------|--------------|--------|
| Task RUNNING but no heartbeat in 2+ min | Worker stuck, infinite loop, blocked on input | `moor kanban show <id>` — read the worker's last events. The dispatcher SIGTERMs tasks that exceed their `max-runtime`; if you need to stop one earlier, `moor kanban block <id>` then `moor kanban archive <id>`, and create a re-run task. |
| Same task retried 2+ times | Reproducible failure (missing key, bad spec, broken tool) | `moor kanban show <id>` to read failure events. Fix root cause before re-running. |
| RUNNING longer than max_runtime | Task is slow but progressing OR genuinely stuck | Check heartbeats with `moor kanban tail <id>`. If progressing, the dispatcher will SIGTERM eventually anyway — raise `max-runtime` on a re-created task. |
...
| Tasks created but never picked up | Profile not running, or tenant mismatch, or dispatcher not running | Check `moor profile list` (profile exists?), `hermes status` (gateway/dispatcher up?), and verify tenant. |
...
NEW_TASK_ID=$(moor kanban create "Generate SRT captions from voiceover" \
    --assignee captioner \
    --workspace dir:"$HOME/projects/video-pipeline/<slug>" \
    --tenant <slug> \
...
moor kanban link "$NEW_TASK_ID" <editor-task-id>
```
...
moor kanban block <task-id>
moor kanban archive <task-id>

# Diagnose what happened
moor kanban show <task-id>      # task body, comments, recent events
moor kanban tail <task-id>      # follow the live event stream
moor kanban log <task-id>       # worker process log
```
...
3. Re-fire the initial `moor kanban create` for the director
...
while true; do
    clear
    moor kanban list --tenant <slug>
    echo "---"
    moor kanban stats --tenant <slug>
    sleep 30
done
```

For a live event feed, run `moor kanban watch --tenant <slug>` in a
...
- **Profile process not running.** Tasks queue indefinitely in READY if no
  worker for that profile is online. Check `moor profile list` and start
