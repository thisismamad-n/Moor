...
| Tasks created but never picked up | Profile not running, or tenant mismatch, or dispatcher not running | Check `moor profile list` (profile exists?), `moor status` (gateway/dispatcher up?), and verify tenant. |
