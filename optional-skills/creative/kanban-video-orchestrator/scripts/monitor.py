#!/usr/bin/env python3
"""
Monitor a running video-production kanban. Polls `moor kanban list` and
...
def moor_available() -> bool:
    return shutil.which("moor") is not None


def kanban_list(tenant: str) -> list[dict]:
    """Returns parsed task rows. Falls back to plain stdout parsing if JSON
    output isn't supported by the installed moor CLI."""
    try:
        out = subprocess.run(
            ["moor", "kanban", "list", "--tenant", tenant, "--json"],
        ...
        pass
    # Fallback: textual parse of `moor kanban list`
    out = subprocess.run(
        ["moor", "kanban", "list", "--tenant", tenant],
        capture_output=True, text=True, encoding='utf-8', errors='replace', check=False,
    )
    rows = []
...
def main():
    ...
    if not moor_available():
        print("ERROR: 'moor' CLI not found in PATH", file=sys.stderr)
        sys.exit(1)
