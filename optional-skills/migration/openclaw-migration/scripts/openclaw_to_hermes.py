...
# become ``~/.hermes`` — the real Moor home — not the broken ``~/.Moor``).
...
def _case_preserving_replacement(replacement: str):
    ...
    Keeps ``OpenClaw`` → ``Moor`` but maps ``openclaw`` → ``hermes`` so a
    filesystem path like ``~/.openclaw/config.yaml`` rewrites to
    ``~/.hermes/config.yaml`` (the real Moor home) instead of the broken
    ``~/.Moor/config.yaml``.
    """
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--target", default=os.environ.get("HERMES_HOME") or str(Path.home() / ".hermes"), help="Moor home directory")
    parser.add_argument(
        "--workspace-target",
