#!/usr/bin/env python3
"""OpenClaw -> Moor migration helper.

This script migrates the parts of an OpenClaw user footprint that map cleanly
into Moor Agent, archives selected unmapped docs for manual review, and
...
MIGRATION_OPTION_METADATA: Dict[str, Dict[str, str]] = {
    "soul": {
        "label": "SOUL.md",
        "description": "Import the OpenClaw persona file into Moor.",
    },
    "workspace-agents": {
    ...
    "memory": {
        "label": "MEMORY.md",
        "description": "Import long-term memory entries into Moor memories.",
    },
    "user-profile": {
        "label": "USER.md",
        "description": "Import user profile entries into Moor memories.",
    },
    "messaging-settings": {
    ...
    "command-allowlist": {
        ...
        "description": "Merge OpenClaw exec approval patterns into Moor command_allowlist.",
    },
    "skills": {
        "label": "User skills",
        "description": "Copy OpenClaw skills into ~/.moor/skills/openclaw-imports/.",
    },
    "tts-assets": {
        "label": "TTS assets",
        "description": "Copy compatible workspace TTS assets into ~/.moor/tts/.",
    },
    "discord-settings": {
        "label": "Discord settings",
        "description": "Import Discord bot token and allowlist into Moor .env.",
    },
    "slack-settings": {
        "label": "Slack settings",
        "description": "Import Slack bot/app tokens and allowlist into Moor .env.",
    },
    "whatsapp-settings": {
        "label": "WhatsApp settings",
        "description": "Import WhatsApp allowlist into Moor .env.",
    },
    "signal-settings": {
        "label": "Signal settings",
        "description": "Import Signal account, HTTP URL, and allowlist into Moor .env.",
    },
    "provider-keys": {
        "label": "Provider API keys",
        "description": "Import model provider API keys into Moor .env (requires --migrate-secrets).",
    },
    "model-config": {
        "label": "Default model",
        "description": "Import the default model setting into Moor config.yaml.",
    },
    "tts-config": {
        "label": "TTS configuration",
        "description": "Import TTS provider and voice settings into Moor config.yaml.",
    },
    "shared-skills": {
        "label": "Shared skills",
        "description": "Copy shared OpenClaw skills from ~/.openclaw/skills/ into Moor.",
    },
    "daily-memory": {
        "label": "Daily memory files",
        "description": "Merge daily memory entries from workspace/memory/ into Moor MEMORY.md.",
    },
    "archive": {
    ...
    "mcp-servers": {
        "label": "MCP servers",
        "description": "Import MCP server definitions from OpenClaw into Moor config.yaml.",
    },
    "plugins-config": {
    ...
    "cron-jobs": {
        ...
        "description": "Import cron job definitions. Archive for manual recreation via 'moor cron'.",
    },
    "hooks-config": {
    ...
    "agent-config": {
        ...
        "description": "Import agent defaults (compaction, context, thinking) into Moor config. Archive multi-agent list.",
    },
    "gateway-config": {
    ...
    "session-config": {
        ...
        "description": "Import session reset policies (daily/idle) into Moor session_reset config.",
    },
    "full-providers": {
        "label": "Full model provider definitions",
        "description": "Import custom model providers (baseUrl, apiType, headers) into Moor custom_providers.",
    },
    "deep-channels": {
    ...
    "browser-config": {
        ...
        "description": "Import browser automation settings into Moor config.yaml.",
    },
    "tools-config": {
        "label": "Tools configuration",
        "description": "Import tool settings (exec timeout, sandbox, web search) into Moor config.yaml.",
    },
    "approvals-config": {
        "label": "Approval rules",
        "description": "Import approval mode and rules into Moor config.yaml approvals section.",
    },
    "memory-backend": {
...
def load_yaml_file(path: Path) -> Dict[str, Any]:
    ...
    try:
        ...
        raise ConfigReadError(
            ...
            f"({exc}). Fix it with `moor config edit` (or move it aside), then "
    ...
    if not isinstance(data, dict):
        raise ConfigReadError(
            ...
            f"`moor config edit` (or move it aside), then re-run the migration."
        )
    return data
...
def dump_yaml_file(path: Path, data: Dict[str, Any]) -> None:
    ...
    """
    if yaml is None:
        raise RuntimeError("PyYAML is required to update Moor config.yaml")
    ensure_parent(path)
...
# ── Brand rewriting ─────────────────────────────────────────
# Replace OpenClaw brand names with Moor in migrated text so that
# memory entries, user profiles, SOUL.md, and workspace instructions
# read as self-referential to the new agent identity.
#
# Case-preserving: ``OpenClaw`` → ``Moor`` (prose), but lowercase matches
# like ``openclaw`` → ``moor`` (so filesystem paths like ``~/.openclaw``
# become ``~/.moor`` — the real Moor home — not the broken ``~/.Moor``).
_REBRAND_PATTERNS: List[Tuple[re.Pattern, str]] = [
    (re.compile(r'\bOpen[\s-]?Claw\b', re.IGNORECASE), 'Moor'),
    (re.compile(r'\bClawdBot\b', re.IGNORECASE), 'Moor'),
    (re.compile(r'\bMoltBot\b', re.IGNORECASE), 'Moor'),
]
...
def _case_preserving_replacement(replacement: str):
    ...
    Keeps ``OpenClaw`` → ``Moor`` but maps ``openclaw`` → ``moor`` so a
    filesystem path like ``~/.openclaw/config.yaml`` rewrites to
    ``~/.moor/config.yaml`` (the real Moor home) instead of the broken
    ``~/.Moor/config.yaml``.
    """
...
def rebrand_text(text: str) -> str:
    """Replace OpenClaw / ClawdBot / MoltBot brand names with Moor.
...
def write_report(output_dir: Path, report: Dict[str, Any]) -> None:
    ...
    lines = [
        "# OpenClaw -> Moor Migration Report",
        "",
...
class Migrator:
    def __init__(
        ...
        # OpenClaw's extensions/migrate-moor/apply.ts "blocked by earlier
    ...
    # Option ids that mutate the Moor config.yaml file.  Once any one of
    ...
    def _build_warnings(self, summary: Dict[str, int]) -> List[str]:
        ...
        Modelled on OpenClaw's extensions/migrate-moor/plan.ts warnings[].
    ...
    def _build_next_steps(self, summary: Dict[str, int]) -> List[str]:
        ...
        if summary.get("migrated", 0) > 0:
            ...
            steps.append(
                "Start a new Moor session (or /reset) to pick up the imported config."
            )
        if summary.get("conflict", 0) > 0:
            steps.append(
    ...
    def migrate_command_allowlist(self) -> None:
        ...
        if not destination.exists():
            self.record("command-allowlist", source, destination, "skipped", "Moor config.yaml does not exist yet")
            return
    ...
    def migrate_messaging_settings(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        if isinstance(workspace, str) and workspace.strip():
            ...
            # that path will be stale after migration and would cause the Moor
    ...
    def migrate_provider_keys(self, config: Dict[str, Any]) -> None:
                ...
                if not api_key:
                    ...
                    if isinstance(raw_key, dict) and raw_key.get("source") in {"file", "exec"}:
                        self.record(
                            ...
                            f"that cannot be auto-migrated. Add this key manually via: moor config set",
                        )
                    continue
        ...
        for oc_key, moor_key in env_key_mapping.items():
            val = openclaw_env.get(oc_key, "").strip()
            if val and moor_key not in secret_additions:
                secret_additions[moor_key] = val
        ...
        if isinstance(json_env, dict):
            ...
            for src in sources:
                for oc_key, moor_key in env_key_mapping.items():
                    val = src.get(oc_key)
                    if isinstance(val, str) and val.strip() and moor_key not in secret_additions:
                        secret_additions[moor_key] = val.strip()
    ...
    def migrate_model_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        moor_config = load_yaml_file(destination)
        current_model = moor_config.get("model")
        ...
        if self.execute:
            backup_path = self.maybe_backup(destination)
            existing_model = moor_config.get("model")
            if isinstance(existing_model, dict):
                existing_model["default"] = model_str
            else:
                moor_config["model"] = {"default": model_str}
            dump_yaml_file(destination, moor_config)
    ...
    def migrate_tts_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        if isinstance(provider, str) and provider in {"elevenlabs", "openai", "edge", "microsoft"}:
            # OpenClaw renamed "edge" to "microsoft"; Moor still uses "edge"
        ...
        moor_config = load_yaml_file(destination)
        existing_tts = moor_config.get("tts", {})
    ...
    def migrate_mcp_servers(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        moor_cfg_path = self.target_root / "config.yaml"
        moor_cfg = load_yaml_file(moor_cfg_path)
        existing_mcp = moor_cfg.get("mcp_servers") or {}
        added = 0
        ...
        for name, srv in mcp_raw.items():
            ...
            if name in existing_mcp and not self.overwrite:
                self.record("mcp-servers", f"mcp.servers.{name}", f"mcp_servers.{name}", "conflict",
                            "MCP server already exists in Moor config")
                continue

            moor_srv: Dict[str, Any] = {}
            # STDIO transport
            if srv.get("command"):
                moor_srv["command"] = srv["command"]
                if srv.get("args"):
                    moor_srv["args"] = srv["args"]
                if srv.get("env"):
                    moor_srv["env"] = srv["env"]
                if srv.get("cwd"):
                    moor_srv["cwd"] = srv["cwd"]
            # HTTP/SSE transport
            if srv.get("url"):
                moor_srv["url"] = srv["url"]
                if srv.get("headers"):
                    moor_srv["headers"] = srv["headers"]
                if srv.get("auth"):
                    moor_srv["auth"] = srv["auth"]
            # Common fields
            if srv.get("enabled") is False:
                moor_srv["enabled"] = False
            if srv.get("timeout"):
                moor_srv["timeout"] = srv["timeout"]
            if srv.get("connectTimeout"):
                moor_srv["connect_timeout"] = srv["connectTimeout"]
            # Tool filtering
            tools_cfg = srv.get("tools") or {}
            if tools_cfg.get("include") or tools_cfg.get("exclude"):
                moor_srv["tools"] = {}
                if tools_cfg.get("include"):
                    moor_srv["tools"]["include"] = tools_cfg["include"]
                if tools_cfg.get("exclude"):
                    moor_srv["tools"]["exclude"] = tools_cfg["exclude"]
            # Sampling
        ...
        if added > 0 and self.execute:
            self.maybe_backup(moor_cfg_path)
            moor_cfg["mcp_servers"] = existing_mcp
            dump_yaml_file(moor_cfg_path, moor_cfg)
        ...
        if cron:
            ...
            if self.archive_dir and self.execute:
                ...
                self.record("cron-jobs", "openclaw.json cron.*", str(dest), "archived",
                            "Cron config archived. Use 'moor cron' to recreate jobs manually.")
            else:
    ...
    def migrate_agent_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        # Map agent defaults
        agent_cfg = moor_cfg.get("agent") or {}
        ...
        if defaults.get("thinkingDefault"):
            # Map OpenClaw thinking -> Moor reasoning_effort
        ...
        if compaction:
            compression = moor_cfg.get("compression") or {}
    ...
    def migrate_gateway_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        self.record("gateway-config", "openclaw.json gateway.*", "archive/gateway-config.json",
                    "archived", "Gateway config archived. Use 'moor gateway' to configure.")

        # Extract gateway auth token to .env if present
        auth = gateway.get("auth") or {}
        if auth.get("token") and self.migrate_secrets:
            self._set_env_var("MOOR_GATEWAY_TOKEN", auth["token"], "gateway.auth.token")
    ...
    def migrate_session_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        moor_cfg_path = self.target_root / "config.yaml"
        moor_cfg = load_yaml_file(moor_cfg_path)
        sr = moor_cfg.get("session_reset") or {}
        changes = False
    ...
    def migrate_full_providers(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        moor_cfg_path = self.target_root / "config.yaml"
        moor_cfg = load_yaml_file(moor_cfg_path)
        custom_providers = moor_cfg.get("custom_providers") or []
        added = 0
        ...
        if added > 0 and self.execute:
            self.maybe_backup(moor_cfg_path)
            moor_cfg["custom_providers"] = custom_providers
            dump_yaml_file(moor_cfg_path, moor_cfg)
    ...
    def migrate_deep_channels(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        # Map Discord-specific settings to Moor config
        discord_cfg = channels.get("discord") or {}
        if discord_cfg:
            moor_cfg_path = self.target_root / "config.yaml"
            moor_cfg = load_yaml_file(moor_cfg_path)
            discord_moor = moor_cfg.get("discord") or {}
            changed = False
            if "requireMention" in discord_cfg:
                discord_moor["require_mention"] = discord_cfg["requireMention"]
                changed = True
            if discord_cfg.get("autoThread") is not None:
                discord_moor["auto_thread"] = discord_cfg["autoThread"]
                changed = True
            if changed and self.execute:
                moor_cfg["discord"] = discord_moor
                dump_yaml_file(moor_cfg_path, moor_cfg)
    ...
    def migrate_browser_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        moor_cfg_path = self.target_root / "config.yaml"
        moor_cfg = load_yaml_file(moor_cfg_path)
        browser_moor = moor_cfg.get("browser") or {}
        changed = False

        # Map fields that have Moor equivalents
        if browser.get("cdpUrl"):
            browser_moor["cdp_url"] = browser["cdpUrl"]
            changed = True
        if browser.get("headless") is not None:
            browser_moor["headless"] = browser["headless"]
            changed = True

        if changed:
            moor_cfg["browser"] = browser_moor
            if self.execute:
                self.maybe_backup(moor_cfg_path)
                dump_yaml_file(moor_cfg_path, moor_cfg)
    ...
    def migrate_approvals_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        if mode:
            ...
            moor_mode = mode_map.get(mode, "manual")
            moor_cfg.setdefault("approvals", {})["mode"] = moor_mode
            if self.execute:
                self.maybe_backup(moor_cfg_path)
                dump_yaml_file(moor_cfg_path, moor_cfg)
            self.record("approvals-config", "openclaw.json approvals.mode",
                        "config.yaml approvals.mode", "migrated", f"Mapped '{mode}' -> '{moor_mode}'")
    ...
    def generate_migration_notes(self) -> None:
        ...
        if archived:
            notes.extend([
                ...
                "direct 1:1 mapping in Moor. Review each file and recreate manually:",
                "",
            ])
        ...
        if conflicts:
            notes.extend([
                "## Conflicts (Existing Moor Config Not Overwritten)",
                "",
                "These items already existed in your Moor config. Re-run with",
        ...
        notes.extend([
            ...
            "state files (todo.json, sessions, logs). If the Moor agent discovers these",
            "directories, it may read/write to them instead of the Moor state, causing",
            "confusion (e.g., cron jobs reading a different todo list than interactive sessions).",
            "",
            "**Strongly recommended:** Run `moor claw cleanup` to rename the OpenClaw",
            ...
            "- Run `moor setup` to configure any remaining settings",
            "- Run `moor mcp list` to verify MCP servers were imported correctly",
        ])

        if has_cron_config_archive:
            notes.append("- Run `moor cron` to recreate scheduled tasks (see archive/cron-config.json)")
        elif has_cron_store_archive:
            notes.append("- Run `moor cron` to recreate scheduled tasks (see archived cron-store)")
        ...
        notes.extend([
            "- Run `moor gateway install` if you need the gateway service",
            "- Review `~/.moor/config.yaml` for any adjustments",
            "",
        ])
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Migrate OpenClaw user state into Moor Agent.")
    parser.add_argument("--source", default=str(Path.home() / ".openclaw"), help="OpenClaw home directory")
    parser.add_argument("--target", default=os.environ.get("MOOR_HOME") or str(Path.home() / ".moor"), help="Moor home directory")
    parser.add_argument(
        "--workspace-target",
    ...
    parser.add_argument("--overwrite", action="store_true", help="Overwrite existing Moor targets after backing them up")
    parser.add_argument(
        "--migrate-secrets",
        action="store_true",
...
def main() -> int:
    ...
    print()
    print("  ╔══════════════════════════════════════════════════════╗")
    print(f"  ║   OpenClaw -> Moor Migration   [{mode_label:>8s}]   ║")
    print("  ╠══════════════════════════════════════════════════════╣")
    ...
    if migrated:
        ...
        for item in migrated:
            ...
            if dest.startswith(str(report["target_root"])):
                dest = "~/.moor/" + dest[len(str(report["target_root"])) + 1:]
    ...
    if args.execute:
        print()
        print("  Next steps:")
        print("    1. Review ~/.moor/config.yaml")
        print("    2. Run: moor mcp list")
        if any(i["kind"] == "cron-jobs" and i["status"] == "archived" for i in items):
            print("    3. Recreate cron jobs: moor cron")
