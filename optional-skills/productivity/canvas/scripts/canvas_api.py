#!/usr/bin/env python3
"""Canvas LMS API CLI for Moor Agent.
...
def _check_config():
    ...
    if missing:
        moor_env = os.path.join(
            os.environ.get("MOOR_HOME", os.path.expanduser("~/.moor")), ".env"
        )
        print(
            f"Missing required environment variables: {', '.join(missing)}\n"
            f"Set them in {moor_env} or export them in your shell.\n"
...
def main():
    parser = argparse.ArgumentParser(
        description="Canvas LMS API CLI for Moor Agent"
    )
