"""Centralized logging setup for Moor Agent.

Provides a single ``setup_logging()`` entry point that both the CLI and
gateway call early in their startup path.  All log files live under
``~/.moor/logs/`` (profile-aware via ``get_moor_home()``).
...
Component separation:
    ...
    gui.log receives dashboard-side records from ``moor_cli.web_server``,
    ``moor_cli.pty_bridge``, ``tui_gateway.*``, and ``uvicorn.*``.
...
# essentially always in Moor (TUI, gateway, ``hy_memory`` server, MCP
...
from moor_constants import get_config_path, get_moor_home
...
def _install_session_record_factory() -> None:
    ...
    if getattr(current_factory, "_moor_session_injector", False):
    ...
    _session_record_factory._moor_session_injector = True  # type: ignore[attr-defined]
...
# Used by _ComponentFilter and exposed for ``moor logs --component``.
COMPONENT_PREFIXES = {
    ...
    # ``moor logs --component gateway``.
    "gateway": ("gateway", "moor_plugins", "plugins.platforms"),
...
def setup_logging(
    *,
    ...
    """Configure the Moor logging subsystem.
    ...
    *force* is ``True``.
    ...
    ----------
    moor_home
        Override for the Moor home directory.  Falls back to
        ``get_moor_home()`` (profile-aware).
    log_level
...
def setup_verbose_logging() -> None:
    ...
    for h in root.handlers:
        if isinstance(h, logging.StreamHandler) and not isinstance(h, RotatingFileHandler):
            if getattr(h, "_moor_verbose", False):
                return

    handler = logging.StreamHandler(_safe_stderr())
    handler.setLevel(logging.DEBUG)
    handler.setFormatter(RedactingFormatter(_LOG_FORMAT_VERBOSE, datefmt="%H:%M:%S"))
    handler._moor_verbose = True  # type: ignore[attr-defined]
...
class _ManagedRotatingFileHandler(RotatingFileHandler):
    ...
    1.  In managed mode (NixOS), the stateDir uses setgid (2770) so new files
        inherit the moor group. However, both ``_open()`` (initial creation)
    ...
    def __init__(self, *args, **kwargs):
        from moor_cli.config import is_managed
...
# the module header): when several Moor processes log to the same file, an
    ...
    *root* directly, so emitting threads never block on file I/O or the
    ...
    with _queue_state_lock:
        if _log_queue is None:
            ...
            qh._moor_queue = True  # type: ignore[attr-defined]
...
def drain_log_queue(timeout: float = 1.0) -> None:
    ...
    t = threading.Thread(target=_drain, name="moor-log-drain", daemon=True)
    t.start()
    t.join(timeout)
...
def _reset_queued_handlers() -> None:
    ...
    with _queue_state_lock:
        ...
        for h in list(root.handlers):
            if getattr(h, "_moor_queue", False):
...
def _read_logging_config():
    ...
    try:
        ...
        # reuses the parse moor_cli.main's early bridge already did (one
        # config.yaml parse per process instead of 3-4). Fall back to a
        # direct parse when moor_cli.config isn't importable (bare
        # moor_logging consumers).
        try:
            from moor_cli.config import read_raw_config as _rrc
            cfg = _rrc() or {}
        except Exception:
