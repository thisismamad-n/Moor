...
# in Moor (TUI, gateway, hy_memory, MCP servers, CLI commands all log) —
...
def _install_session_record_factory() -> None:
    ...
    def _session_record_factory(*args, **kwargs):
        ...
        try:
            record.moor_home = str(get_moor_home().resolve())  # type: ignore[attr-defined]
        except Exception:
            record.moor_home = ""  # type: ignore[attr-defined]
        return record
...
# Logger name prefixes per component; used by _ComponentFilter and ``moor logs --component``.
COMPONENT_PREFIXES = {
    ...
    "cli": ("moor_cli", "cli"),
    "cron": ("cron",),
    "gui": ("moor_cli.web_server", "moor_cli.pty_bridge", "tui_gateway", "uvicorn"),
}
...
def setup_logging(
    *,
    ...
    """Configure the Moor logging subsystem; returns the ``logs/`` directory.
...
def setup_verbose_logging() -> None:
    ...
    if any(getattr(h, "_moor_verbose", False) for h in root.handlers):
        return
...
class _ProfileRoutingFileHandler(logging.Handler):
    """Route queued records to the log file for their Moor home.
    ...
    def __init__(self, existing: RotatingFileHandler, profile_homes: Sequence[Path]) -> None:
        ...
        self._moor_routed_log_path = resolved
    ...
    def _home_for_record(self, record: logging.LogRecord) -> Path:
        raw_home = getattr(record, "moor_home", "")
        try:
...
def drain_log_queue(timeout: float = 1.0) -> None:
    ...
    if listener is None:
        return
    t = threading.Thread(target=lambda: _quietly(listener.stop), name="moor-log-drain", daemon=True)
    t.start()
    t.join(timeout)
...
def _read_logging_config():
    ...
    try:
        ...
        # moor_cli.main's early parse (one config.yaml parse per process);
        # fall back to a direct parse for bare moor_logging consumers.
        try:
