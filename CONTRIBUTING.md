# Contributing to Moor Agent

Thank you for contributing to Moor Agent! This guide covers everything you need: setting up your dev environment, understanding the architecture, deciding what to build, and getting your PR merged.
...
2. **Cross-platform compatibility** — macOS, different Linux distros, and WSL2 on Windows. We want Moor to work everywhere.
...
Related: #38284 covers the agent-side analog — Moor itself checking existing issues and PRs before deep self-troubleshooting. This section is the human-contributor complement.
...
Bundled skills (in `skills/`) ship with every Moor install. They should be **broadly useful to most users**:
...
├── cli.py                    # MoorCLI class — interactive CLI orchestrator (~4.6k LOC + moor_cli/cli_*_mixin.py)
├── model_tools.py            # Tool orchestration (thin layer over tools/registry.py)
├── toolsets.py               # Tool groupings and presets (moor-cli, moor-telegram, etc.)
├── moor_state.py           # SessionDB facade (~1.4k LOC); implementation in moor_state_*.py (21 siblings) — FTS5 search, session titles
...
│   ├── cli_*_mixin.py            # MoorCLI mixins (slash commands, display, session, ...)
│   ├── config.py                 # Config management, migration, env var definitions
│   ├── setup.py                  # Interactive setup wizard
│   ├── auth.py                   # Provider resolution, OAuth, Moor Portal (facade + auth_*.py siblings)
...
| `~/.moor/sessions/` | Gateway routing index (`sessions.json`), request-dump breadcrumbs, gateway `*.jsonl` transcripts, and explicit `/save` exports. Automatic per-session JSON snapshots are no longer written; state.db is canonical. |
...
- **Session persistence**: All conversations are stored in SQLite (`moor_state.py`) with full-text search and unique session titles. Automatic per-session JSON snapshots have been removed. Existing files are left untouched; use `/save json` or `moor sessions export` for an explicit export.
...
The user may skip setup and keep loading the skill. Moor only exposes metadata (`stored_as`, `skipped`, `validated`) to the model — never the secret value.
...
2. **Tools referenced in SKILL.md prose must be native Moor tools or MCP servers the skill explicitly expects.** When the skill needs a capability, point at the proper tool by name in backticks: `` `terminal` ``, `` `web_extract` ``, `` `web_search` ``, `` `read_file` ``, `` `write_file` ``, `` `patch` ``, `` `search_files` ``, `` `vision_analyze` ``, `` `browser_navigate` ``, `` `delegate_task` ``, `` `image_generate` ``, `` `text_to_speech` ``, `` `cronjob` ``, `` `memory` ``, `` `skill_view` ``, `` `todo` ``, `` `execute_code` ``.
...
4. **`author` credits the human contributor first.** For external contributions, the contributor's real name + GitHub handle goes first (`Jane Doe (jane-doe)`); "Moor Agent" is the secondary collaborator. If the contributor's commit shows "Moor Agent" as author because they used Moor to draft the skill, replace it with their actual name — credit the human, not the tool.
...
Moor uses a data-driven skin system — no code changes needed to add a new skin.
...
Moor runs on Linux, macOS, and native Windows (plus WSL2). When writing code
...
Moor has terminal access. Security matters.
