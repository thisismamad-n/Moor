...
def _scrub_config_yaml_mirrors(old_value: str, new_value: str | None) -> List[str]:
    ...
    from moor_cli.config import get_config_path, read_user_config_raw, require_readable_config_before_write
...
def purge_env_credential_references(
    ...
    # Make the removal sticky the same way `moor auth remove` does: a lingering shell export (or
    ...
    _for_each_provider(providers, "moor_cli.auth.suppress_credential_source", f"env:{env_var}")
    if clear_models_cache and providers:
        # Best-effort — a cache failure must not block the credential removal itself.
        _for_each_provider(providers, "moor_cli.models.clear_provider_models_cache")
...
def save_provider_env_credential(env_var: str, value: str) -> Dict[str, Any]:
    ...
    kept 401'ing until the user ran ``moor auth add <provider> --type api-key`` separately. This makes the
    Desktop save's effect on disk match what ``moor auth add`` does.
    """
    ...
    _for_each_provider(providers, "moor_cli.auth.unsuppress_credential_source", f"env:{env_var}")
...
def remove_provider_env_credential(env_var: str) -> Dict[str, Any]:
    ...
    from moor_cli.config import load_env, remove_env_value
