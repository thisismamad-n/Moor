...
from moor_cli.model_setup_flows_common import _say
...
def _whatsapp_choose_mode(get_env_value, save_env_value):
    """Step 1 of ``moor whatsapp``: ``"bot"`` / ``"self-chat"``, or None when cancelled."""
    current_mode = get_env_value("WHATSAPP_MODE") or ""
    if current_mode:
        mode_label = "separate bot number" if current_mode == "bot" else "personal number (self-chat)"
        print(f"\n✓ Mode: {mode_label}")
        return current_mode
    _say("", "How will you use WhatsApp with Moor?", "",
...
def _whatsapp_allowed_users(wa_mode: str, get_env_value, save_env_value) -> None:
    """Step 3 of ``moor whatsapp``: show / set WHATSAPP_ALLOWED_USERS."""
...
def _whatsapp_install_bridge(bridge_dir) -> bool:
    """Step 4 of ``moor whatsapp``: ``npm install`` the bridge when needed. False = stop."""
    ...
    try:
        result = subprocess.run(
            ...
            encoding="utf-8", errors="replace", env=with_moor_node_path())
...
def cmd_whatsapp(args):
    ...
    # every `moor gateway` would pay a 30s bridge timeout + indefinite retries. Set only after
    ...
    if (session_dir / "creds.json").exists():
        ...
        if _yes_no("\n  Re-pair? This will clear the existing session. [y/N] "):
            ...
            _say("\n✓ WhatsApp is configured and paired!", "  Start the gateway with: moor gateway")
            return
...
def cmd_whatsapp_cloud(args):
    ...
    ``moor whatsapp`` Baileys bridge wizard. See ``moor_cli/setup_whatsapp_cloud.py``."""
    from moor_cli.main import _require_tty
...
def _sync_status(ssc) -> int:
    ...
    if not status.get("logged_in"):
        _err("\nNot logged into Moor Portal — sync is inert.")
        ...
        _err("\nNo sync base URL configured (config.yaml sync.base_url or MOOR_SYNC_BASE_URL). Sync is inert.")
    return 0
...
_SYNC_GATED = {
    "pull": _sync_pull,
    "push": lambda ssc, identity: ssc.push_skills(identity=identity, message="moor sync push"),
    "now": lambda ssc, identity: {"pull": ssc.pull_skills(identity=identity),
                                  "push": ssc.push_skills(identity=identity, message="moor sync now")}}
...
def cmd_sync(args):
    ...
    if not ssc.resolve_sync_base_url():
        _err("sync inert: no sync base URL configured (config.yaml sync.base_url or MOOR_SYNC_BASE_URL).")
        return 1
...
def cmd_slack(args):
    """``moor slack <subcommand>``; ``manifest`` prints or writes a Slack app manifest with
    every gateway command registered as a first-class slash."""
    sub = getattr(args, "slack_command", None)
    if sub in {None, ""}:
        _err("usage: moor slack <subcommand>\n"
             "\n"
             "subcommands:\n"
