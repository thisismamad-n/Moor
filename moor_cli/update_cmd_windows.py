"""Windows gateway lifecycle for ``moor update``: pause/resume/cold-start the service, sweep venv holders, reap orphaned backends.

Split out of ``update_cmd.py``; names are re-imported there so ``moor_cli.update_cmd.<name>`` still resolves/monkeypatches.
...
logger = logging.getLogger("moor_cli.update_cmd")  # log-record parity with the origin module
...
def _self_and_non_gateway_ancestor_pids(psutil) -> set[int]:
    ...
    with suppress(Exception):
        # Never return ourselves or our own ancestry: a CLI ``moor update`` runs from the venv python and
...
def _detect_venv_python_processes(*, exclude_pids: set[int] | None = None) -> list[tuple[int, str, str]]:
    ...
    The moor.exe shim guard misses the Desktop backend and anything off ``venv\\Scripts\\python(w).exe``;
    ...
    for proc in proc_iter:
        ...
        name_low = name.lower()
        if not (in_venv or name_low.startswith(("python", "pypy")) or name_low in {"uv.exe", "uvx.exe", "moor.exe"}):
            continue
        ...
        # its .pyd files — match cmdline (venv path, or `-m moor_cli.main` + root/cwd).
        if in_venv or venv_prefix in cmdline_low or (
            "moor_cli.main" in cmdline_low and (root_prefix in cmdline_low or _cwd_prefix(proc).startswith(root_prefix))
        ):
...
def _moor_holder_subcommand(cmdline: str) -> str | None:
    """The actual Moor SUBCOMMAND a venv-holder argv runs, or None (callers must NOT guess a label).

    Token-based, never substring (``kanban --preserve-cache`` contains "serve"): find the ``moor_cli.main`` /
    ``moor(.exe)`` entry token, return the first following token that isn't a flag or a flag's value.
    ...
    def _is_entry(i: int, token: str) -> bool:
        ...
        return (low.endswith("moor_cli.main") and i > 0 and tokens[i - 1] == "-m") or (
            low.rsplit("\\", 1)[-1].rsplit("/", 1)[-1] in ("moor", "moor.exe"))
...
def _refuse_gateway_ancestor_tree_kill(pids: list[int], *, gateway_mode: bool) -> bool:
    """Refuse a plain Windows update that would tree-kill its own ancestry (a chat agent's ``moor update`` is
    ...
    if gateway_mode or not pids:
        return False
    def _ancestors():
        from moor_cli.gateway import _is_pid_ancestor_of_current_process
    ...
    print(
        ...
        "  From a chat platform, use `/update` instead.\n  Otherwise, run `moor update` from a separate terminal."
    )
    return True
...
def _serve_relaunch_commands(entries: list[dict]) -> list[list[str]]:
    ...
    with suppress(Exception):
        ...
        if scripts_dir is not None:
            moor = next((str(scripts_dir / n) for n in ("moor.exe", "moor") if (scripts_dir / n).is_file()), moor)
    ...
    for entry in entries:
        ...
        commands.append(
            [moor] + (["--profile", profile] if profile and profile != "default" else [])
...
def _relaunch_stopped_serves(token: dict) -> None:
    ...
    if skipped or failed:
        print("  ⚠ Some stopped backends could not be relaunched automatically; restart them manually (moor serve --host <ip> --port <port>).")
    _record_update_step(
...
def _is_backend_argv(argv_low: str) -> bool:
    """Whether a lower-cased argv is a Desktop backend (``moor_cli.main`` running ``serve``/``dashboard``)."""
    return "moor_cli.main" in argv_low and (" serve" in argv_low or " dashboard" in argv_low)
...
def _orphaned_desktop_backend_pids(matches: list[tuple[int, str, str]]) -> list[tuple[int, int]] | None:
    ...
    would dead-end the update with "Moor is still running" and zero open windows. Qualifies only if cmdline
    is a Moor backend AND the parent is demonstrably gone (PID missing or reused). Tree-aware: holders inside
    ...
    it tree-kills its backends and waits for the venv shim before spawning moor-setup, and the
    ...
    dead-ends the update with "Moor is still running" while the user stares at zero open windows (ryanc's
    ...
    for pid, _name, cmdline in matches:
        ...
        try:
            ...
            # gateway.status.get_process_start_time so pid_is_moor round-trips at kill time.
...
def _ledger_reapable_backend_pids(matches: list[tuple[int, str, str]]) -> list[int]:
    ...
    try:
        from moor_cli.process_identity import REAPABLE_PURPOSES, ledger_entries, spawner_is_dead
...
def _handoff_reapable_backend_pids(matches: list[tuple[int, str, str]]) -> list[int] | None:
    ...
    hand-off. Inside the hand-off gate (marker + ``--gateway`` + no live ``moor.exe`` shim) nothing legitimate
...
def _stop_process_trees(pids: list[int] | list[tuple[int, int]]) -> None:
    ...
    ``taskkill /T /F``: stopping only the parent can leave a ``.moor-runtime`` child holding the install open.

    See #70026.
    """
    from gateway.status import get_process_start_time
    from moor_cli._subprocess_compat import pid_is_moor, windows_hide_flags
    for entry in pids:
        ...
        try:
            if expected_start_time is None:
                ...
                continue
            if not pid_is_moor(pid, expected_start_time=expected_start_time):
                logger.debug("Skipping taskkill of non-Moor or changed PID %s", pid)
                continue
            subprocess.run(
...
def _looks_like_desktop_control_plane(cmdline: str) -> bool:
    """True for this-install ``moor serve`` / ``moor dashboard`` argv (Desktop control plane).
    ...
    """
    return "moor_cli.main" in (cmdline or "").lower() and _moor_holder_subcommand(cmdline) in _BACKEND_PURPOSES
...
def _restore_windows_gateway_service(name: str, *, timeout: float = 60.0) -> None:
    ...
    from moor_cli.update_cmd import _start_windows_gateway_service
    service = _win_service(name)[1]
...
def _pause_windows_gateway_services(service_gateways, token: dict, profiles: dict, unmapped: list) -> dict:
    ...
    from moor_cli.update_cmd import _restore_windows_gateway_service, _stop_windows_gateway_service
    paused_services = []
...
def _discover_windows_gateways():
    ...
    from moor_cli.gateway import find_gateway_pids, find_profile_gateway_processes, find_windows_gateway_services
...
def _pause_windows_gateways_for_update() -> dict | None:
    ...
    Scheduled/startup gateways run via pythonw.exe, invisible to the moor.exe instance guard, yet keep files
...
def _refresh_windows_gateway_launchers() -> None:
    ...
    artifacts written once at install time — ``moor update`` never touched them, so installs created
...
def _refresh_bootstrap_cache_scripts(branch: str = "main") -> None:
    """Overwrite ``$MOOR_HOME/bootstrap-cache/install-<ref>.{ps1,sh}`` for *branch* from the fresh checkout.

    Old ``moor-setup.exe`` builds NEVER re-download a cached branch-ref script, so a stale one runs
    ...
    so the poisoned cache outlives every ``moor update``.
...
def _relaunch_paused_gateways(token: dict, profiles: dict, unmapped: list) -> tuple[list[str], int]:
    ...
    with _abort_on_error("Could not load Windows gateway restart helper"):
        from moor_cli.gateway import launch_detached_gateway_restart_by_cmdline, launch_detached_profile_gateway_restart
...
def _verify_relaunched_gateways_alive(token: dict, profiles: dict, unmapped: list) -> None:
    ...
    if not ready_pids:
        ...
        print(
            ...
            "    Recover with: moor gateway restart"
        )
