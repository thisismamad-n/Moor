...
# Entry delimiter of the Moor memory store (memories/MEMORY.md) and the openclaw script.
ENTRY_DELIMITER = "\n§\n"
...
def load_yaml_file(path: Path) -> Dict[str, Any]:
    ...
    if not path.exists():
        return {}
    fix_hint = "Fix it with `moor config edit` (or move it aside), then re-run the import."
...
def _translate_mcp_server(name: str, srv: Dict[str, Any]) -> Tuple[Dict[str, Any], List[str]]:
    """Map one Claude/Codex MCP server entry to Moor shape; returns (server, stripped secret paths)."""
    ...
    def import_mcp_servers(self, servers: Dict[str, Any], kind: str) -> None:
        ...
        for name, srv in servers.items():
            ...
            if name in existing and not self.overwrite:
                ...
                continue
            moor_srv, stripped = _translate_mcp_server(name, srv)
...
def import_agent_command(args) -> None:
    ...
    from moor_cli.setup import (Colors, color, print_header, print_info, print_success,
    ...
    if not source_dir.is_dir():
        ...
        print_info(f"Specify a custom path: moor import-agent {agent} --source /path/to/{_AGENT_DEFAULT_DIRS[agent]}")
        return
    ...
    def run_import(execute: bool, phase: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            return AgentImporter(agent, source_dir.resolve(), moor_home.resolve(),
