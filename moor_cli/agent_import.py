"""moor import-agent — import Claude Code / Codex CLI setups into Moor.
...
Usage:
    ...
    moor import-agent claude-code --dry-run # preview only, no changes
    moor import-agent codex --source /path/to/.codex

Follows the OpenClaw migration pattern (``moor claw migrate`` /
``optional-skills/migration/openclaw-migration/scripts/openclaw_to_moor.py``):
...
--------
claude-code (~/.claude):
    CLAUDE.md                       → memory entries in MOOR_HOME/memories/MEMORY.md
    ...
    skills/<name>/SKILL.md          → MOOR_HOME/skills/claude-code-imports/<name>/

codex (~/.codex):
    AGENTS.md                       → memory entries in MOOR_HOME/memories/MEMORY.md
    config.toml [mcp_servers.*]     → config.yaml mcp_servers
    memories/*.md                   → memory entries in MOOR_HOME/memories/MEMORY.md
    skills/<name>/SKILL.md          → MOOR_HOME/skills/codex-imports/<name>/
...
deliberately via ``moor setup`` or config.yaml.
"""
...
# Same entry delimiter as the Moor memory store and the openclaw migration
...
def load_yaml_file(path: Path) -> Dict[str, Any]:
    ...
    if not isinstance(data, dict):
        raise ConfigReadError(
            ...
            f"`moor config edit` (or move it aside), then re-run the import."
        )
    return data
...
# ---------------------------------------------------------------------------
# Memory-entry primitives (ported from openclaw_to_moor.py)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Claude Code permission rules → Moor command patterns
# ---------------------------------------------------------------------------
...
def claude_rule_to_command_pattern(rule: str) -> Optional[str]:
    """Convert a Claude Code ``Bash(...)`` permission rule into a Moor glob.
...
class AgentImporter:
    ...
    def _run_claude_code(self) -> None:
        ...
        if commands_dir.is_dir() and any(commands_dir.glob("*.md")):
            self.record(
                ...
                "Claude slash commands have no direct Moor equivalent — "
    ...
    def import_skills(self, source_root: Path) -> None:
        """skills/<name>/SKILL.md dirs → MOOR_HOME/skills/<category>/<name>."""
...
def import_agent_command(args) -> None:
    """Handle ``moor import-agent`` (invoked from moor_cli.main)."""
    from moor_cli.config import get_config_path, load_config, save_config
    ...
    if agent is None:
        ...
        if not detected:
            ...
            print_info("Specify one explicitly: moor import-agent claude-code --source /path")
            return
        if len(detected) > 1 and explicit_source is None:
            print()
            print_info("Multiple agent setups detected: " + ", ".join(detected))
            print_info("Pick one: moor import-agent claude-code   or   moor import-agent codex")
            return
        agent = detected[0]
    ...
    if not source_dir.is_dir():
        ...
        print_info("Specify a custom path: moor import-agent "
    ...
    print_info("Secrets:     never imported — run 'moor setup' for credentials")
    ...
    if not auto_yes:
        if not sys.stdin.isatty():
            ...
            print_info(f"To execute, re-run with: moor import-agent {agent} --yes")
            return
    ...
    print_info("API keys and credentials were NOT imported — run 'moor setup' "
               "to configure providers, or add them to ~/.moor/.env.")


def print_import_report(report: Dict[str, Any], dry_run: bool) -> None:
    """Print a formatted per-item import report (claw-migrate style)."""
    from moor_cli.setup import Colors, color, print_header, print_info
    ...
    if stripped:
        ...
        for name in stripped:
            print(f"      {name}")
        print_info("Re-add credentials deliberately via 'moor setup' or ~/.moor/.env.")
        print()
