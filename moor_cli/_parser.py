"""Top-level argparse construction for the moor CLI.

Only the top-level parser and the ``chat`` subparser live here. Every other subparser (model,
gateway, sessions, …) is built by ``moor_cli/subcommands/<group>.py`` and wired in
...
# (it sets ``MOOR_HOME`` and strips itself from ``sys.argv``), so it isn't on the parser.
...
# live parser fails. The derived path is authoritative; tests/moor_cli/
...
def _cfg_path() -> str:
    ...
    it re-homes the process to the sticky ``active_profile``; ``get_moor_home()`` would emit the
    "[MOOR_HOME fallback] ... wrong profile" warning on every ``moor`` command for that
    throwaway help string. Read the process home directly: after the override it IS the profile home.
    """
    from moor_constants import display_moor_home, get_process_moor_home

    return f"{display_moor_home(get_process_moor_home())}/config.yaml"
...
def top_level_value_flag_sets() -> tuple[frozenset[str], frozenset[str]]:
    ...
    argparse surface — the drift that made ``moor --reasoning high chat …`` misread ``high`` as
...
Examples:
    ...
    moor gateway start          Start the installed gateway service
    moor gateway stop           Stop the gateway service
    moor gateway status         Show gateway status
    moor -p <profile> <cmd>     Run any command against a named profile's
                                  home (also --profile) — e.g. moor -p coder gateway stop
...
def _add_top_level_flags(parser: argparse.ArgumentParser) -> None:
    ...
    inherited(parser, "-m", "--model", default=None, help=(
        ...
        "Applies to -z/--oneshot and --tui. Also settable via MOOR_INFERENCE_MODEL env var."))
    inherited(parser, "--provider", default=None, help=(
        "Provider override for this invocation (e.g. openrouter, anthropic). "
        "Applies to -z/--oneshot and --tui. The persistent provider lives in config.yaml "
        "under model.provider — use `moor setup` or edit the file to change it."))
    ...
    inherited(parser, "--accept-hooks", action="store_true", default=False, help=(
        ...
        "without a TTY prompt.  Equivalent to MOOR_ACCEPT_HOOKS=1 or "
...
def _build_chat_parser(subparsers) -> argparse.ArgumentParser:
    ...
    ``moor -m foo chat`` argparse first sets ``args.model`` from the top-level parser, then
    ...
    unless the flag is actually passed after the subcommand (tests/moor_cli/
    test_argparse_flag_propagation.py).
    """
    chat_parser = subparsers.add_parser(
        "chat", help="Interactive chat with the agent",
        description="Start an interactive chat session with Moor Agent")
    ...
    inherited(chat_parser, "--accept-hooks", action="store_true", default=SUPPRESS, help=(
        "Auto-approve any unseen shell hooks declared in config.yaml "
        "without a TTY prompt (see also MOOR_ACCEPT_HOOKS env var and "
    ...
    inherited(chat_parser, "--safe-mode", action="store_true", default=SUPPRESS,
              help="Troubleshooting mode: disable ALL customizations — user config, AGENTS.md/memory injection, plugins, and MCP servers (implies --ignore-user-config and --ignore-rules). Use to isolate whether a problem comes from your setup or from Moor itself.")
...
class MoorArgumentParser(argparse.ArgumentParser):
    ...
    def _check_value(self, action, value):
        if isinstance(action, argparse._SubParsersAction) and value not in action.choices:
            # ``self.prog`` is "moor" at the top level and "moor gateway" for a nested group
...
def build_top_level_parser():
    ...
    """
    parser = MoorArgumentParser(
        prog="moor", description="Moor Agent - AI assistant with tool-calling capabilities",
        formatter_class=argparse.RawDescriptionHelpFormatter, epilog=_EPILOGUE)
    _add_top_level_flags(parser)
    # metavar keeps the usage line to ``moor [...] <command>`` instead of the brace list of
    # every subcommand name; ``moor --help`` still lists each command with its help row.
