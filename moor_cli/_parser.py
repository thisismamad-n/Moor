...
# argparse runs (it sets ``MOOR_HOME`` and strips itself from ``sys.argv``),
...
# tests/moor_cli/test_top_level_value_flags_parity.py fails CI if the parser
...
def top_level_value_flag_sets() -> tuple[frozenset[str], frozenset[str]]:
    ...
    the exact drift that made ``moor --reasoning high chat …`` misread
...
def _inherited_flag(parser, *args, **kwargs):
    """Register a flag that ``moor_cli.relaunch`` should carry over when
...
def build_top_level_parser():
    ...
    parser = argparse.ArgumentParser(
        prog="moor",
        description="Moor Agent - AI assistant with tool-calling capabilities",
    ...
    # Mirrors `moor chat --model ... --provider ...` semantics.
    _inherited_flag(
        parser,
        "-m",
        "--model",
    ...
    # top-level parser: when the user writes `moor -m foo chat`, argparse
    ...
    # `-w/--worktree`, `--yolo`, etc. (see tests/moor_cli/
