...
configured for "cli" in `moor tools`.
Rules / memory / AGENTS.md / preloaded skills = same as a normal chat turn.
Approvals = auto-bypassed (MOOR_YOLO_MODE=1 is set for the call).
Working directory = the user's CWD (AGENTS.md etc. resolve from there as usual).

Model / provider selection mirrors `moor chat`:
...
def _validate_explicit_toolsets(toolsets: object = None) -> tuple[list[str] | None, str | None]:
    ...
    try:
        ...
        return None, f"moor -z: failed to validate --toolsets: {exc}\n"
    ...
    if any(name in {"all", "*"} for name in built_in):
        ...
        if ignored:
            sys.stderr.write(
                "moor -z: --toolsets all enables every toolset; "
    ...
    if unknown:
        sys.stderr.write(f"moor -z: ignoring unknown --toolsets entries: {', '.join(unknown)}\n")
    if disabled:
        sys.stderr.write(
            "moor -z: ignoring disabled MCP servers (set enabled: true in config.yaml to use): "
            f"{', '.join(disabled)}\n"
        )

    if not valid:
        return None, "moor -z: --toolsets did not contain any valid toolsets.\n"
...
def run_oneshot(
    ...
    Args:
        ...
        model: Optional model override. Falls back to MOOR_INFERENCE_MODEL
    ...
    env_model_early = os.getenv("MOOR_INFERENCE_MODEL", "").strip()
    if provider and not ((model or "").strip() or env_model_early):
        sys.stderr.write(
            "moor -z: --provider requires --model (or MOOR_INFERENCE_MODEL). "
    ...
    if failure is not None:
        ...
        real_stderr.write(f"moor -z: agent failed: {failure}\n")
        real_stderr.flush()
        return 1
...
def _create_session_db_for_oneshot():
    """Best-effort SessionDB for ``moor -z`` / oneshot mode.

    Oneshot bypasses ``MoorCLI._init_agent()``, so it must wire the SQLite
...
def _run_agent(
    ...
    # Imports are local so they don't run when moor is invoked for
    ...
    env_model = os.getenv("MOOR_INFERENCE_MODEL", "").strip()
    ...
    if effective_provider is None and (model or env_model):
        ...
        if explicit_model:
            ...
            if direct is not None:
                ...
                current_provider = (
                    cfg_provider
                    or os.getenv("MOOR_INFERENCE_PROVIDER", "").strip().lower()
                    or "auto"
                )
    ...
    # MoorCLI._init_agent's wait — it builds AIAgent directly here, so the
    ...
    try:
        ...
        agent = AIAgent(
            ...
            #                MOOR_INTERACTIVE which we never set
            #   - shell-hook approval → auto-approved via MOOR_ACCEPT_HOOKS=1
            #                (set above); also falls back to deny on non-tty
            #   - dangerous-command approval → bypassed via MOOR_YOLO_MODE=1
