...
Toolsets = explicit --toolsets, else the user's "cli" toolsets from `moor tools`. Rules /
memory / AGENTS.md / preloaded skills = same as a normal chat turn. Approvals are auto-bypassed
(MOOR_YOLO_MODE=1). Model/provider mirror `moor chat`: both optional; only --model → auto-detect
...
def _run_agent(
    ...
    try:
        agent = AIAgent(
            ...
            # MOOR_INTERACTIVE (never set), hook approval via MOOR_ACCEPT_HOOKS=1, dangerous
            # commands via MOOR_YOLO_MODE=1, skill secret capture degrades gracefully.
