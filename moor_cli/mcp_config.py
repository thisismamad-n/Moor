...
def _resolve_mcp_server_config(config: dict) -> dict:
    ...
    Mirrors ``_load_mcp_config()`` in ``tools/mcp_tool.py``: load ``~/.moor/.env`` into ``os.environ`` and
...
def _configure_http_auth(
    ...
    if needs_auth and (auth_type == "header" or not auth_type):
        ...
        if get_env_value(env_key):
            ...
            from moor_cli.cli_output import prompt
...
def cmd_mcp_add(args):
    ...
    # --command uses dest="mcp_command" (see moor_cli/main.py for why the dest is renamed).
    ...
    if _save_mcp_server(name, server_config):
        print()
        _success(
            f"Saved '{name}' to {display_moor_home()}/config.yaml ({tool_count}/{len(tools)} tools enabled)"
        )
...
def cmd_mcp_remove(args):
    ...
    # an earlier `moor mcp test`) is evicted too.
    try:
        from tools.mcp_oauth_manager import get_manager
...
def _reauth_oauth_server(name: str, server_config: dict) -> bool:
    ...
    # re-auth path in web_server.py. force_interactive_oauth: `moor mcp login` is explicitly
...
_MCP_USAGE = (
    "moor mcp                                    Open the catalog picker (default)",
    "moor mcp catalog                            List Moor-approved MCPs",
    "moor mcp install <name>                     Install a catalog MCP",
    "moor mcp serve                              Run as MCP server",
    "moor mcp add <name> --url <endpoint>        Add a custom MCP server",
    "moor mcp add <name> --command <cmd>         Add a stdio server",
    "moor mcp add <name> --preset <preset>       Add from a known preset",
    "moor mcp remove <name>                      Remove a server",
    "moor mcp list                               List configured servers",
    "moor mcp test <name>                        Test connection",
    "moor mcp configure <name>                   Toggle tools",
    "moor mcp login <name>                       Re-authenticate OAuth",
    "moor mcp reauth <name> | --all              Re-auth one or all OAuth servers",
)
...
def mcp_command(args):
    ...
    if action in ("picker", "catalog", "install"):
        ...
        from moor_cli import mcp_picker
