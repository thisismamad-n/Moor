...
dependency/config-schema warnings. Mixed into :class:`moor_cli.plugins.PluginManager`.

Origin-internal names (``PluginContext``, ``LoadedPlugin``, ``_PLUGINS_DEBUG`` …) are imported lazily
through ``moor_cli.plugins`` so tests that patch them on the origin keep working.
"""
...
from moor_cli.plugins_manifest import PluginManifest, manifest_key, validate_config_schema
from moor_cli.plugins_state import _plugin_settings_entry

if TYPE_CHECKING:  # pragma: no cover
    from moor_cli.plugins import LoadedPlugin
...
class PluginLoaderMixin:
    ...
    def _register_deferred_platform_tools(self, manifest: PluginManifest, loaded: LoadedPlugin) -> None:
        ...
        would miss them in ``moor tools`` / ``platform_toolsets``. Opt-in is explicit via ``provides_tools``;
        ...
        returns ``[]``, the toolset is missing from the ``moor tools`` checklist, and even an explicit
        ...
        """
        from moor_cli.plugins import PluginContext, _PLUGINS_DEBUG
    ...
    def _warn_python_dependencies(self, manifest: PluginManifest) -> None:
        ...
        See #64165.
        python_dependencies is a declaration seam ONLY: Moor validates and prints the requirements with an
    ...
    def _load_plugin_scoped(self, manifest: PluginManifest) -> None:
        ...
        from moor_cli.plugins import LoadedPlugin, PluginContext, _PLUGINS_DEBUG
        ...
        # skipped with a clear reason instead of dying on ImportError mid-register (moor_cli.plugin_compat).
        from moor_cli.plugin_compat import disable_reason
        ...
        try:
            ...
            if register_fn is None:
                ...
                loaded.enabled = True
                from moor_cli.plugins_ledger import _hook_source_of
    ...
    def _attribute_registrations(
        ...
        # Discovery-time tools predate registration_start; credit them back or `moor plugins list`
    ...
    def _load_portable_plugin(self, manifest: PluginManifest, loaded: LoadedPlugin) -> None:
        ...
        try:
            ...
            package = load_agent_plugin(
                Path(manifest.path), get_moor_home() / "plugin-data" / manifest.skill_namespace)
    ...
    def _directory_module_name(self, manifest: PluginManifest) -> str:
        """Profile-safe import namespace for a directory plugin: the bare ``moor_plugins.<slug>`` for the
    ...
    def _load_directory_module(
        ...
        """Import a directory plugin as ``moor_plugins.<slug>`` (slug from ``manifest.key`` so
        ...
        # Evict stale entries for this slug (same slug cached from another Moor home, or an earlier force
