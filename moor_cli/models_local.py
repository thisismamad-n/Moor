...
disk cache). Split out of ``moor_cli.models``; helpers still defined there are looked up on
``moor_cli.models`` at call time so ``patch("moor_cli.models.<name>")`` mocks keep intercepting.
"""
...
def probe_ollama_local_models(
    ...
    from moor_cli.models import _MOOR_USER_AGENT, _get_ollama_base_url, _urlopen_model_catalog_request
    ...
    if failed_at is not None:
        ...
        _OLLAMA_LOCAL_PROBE_FAILURE_CACHE.pop(failure_key, None)
    try:
        request_headers = {"User-Agent": _MOOR_USER_AGENT, **(headers or {})}
...
_NEVER_OLLAMA_PROVIDERS = frozenset({"openrouter", "moor", "anthropic", "openai", "openai-codex", "gemini", "ollama-cloud"})
...
def _ollama_local_catalog(force_refresh: bool) -> list[str]:
    ...
    Cloud)."""
    from moor_cli.models import _get_provider_config_dict, fetch_api_models
    if force_refresh:
