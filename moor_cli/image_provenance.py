"""Image-authored deployment provenance for immutable Moor runtimes.

The image bakes ``/etc/moor/image-provenance.json`` outside ``$MOOR_HOME`` and the checkout, so a
...
IMAGE_PROVENANCE_PATH = Path("/etc/moor/image-provenance.json")
