...
# Logger name parity with the origin module (tests capture "moor_cli.commands").
logger = logging.getLogger("moor_cli.commands")
...
def discord_skill_commands_by_category(
    ...
    parity, external-dir skills are visible via ``moor skills list`` and the agent's ``/skill-name``
...
# Canonical commands deliberately routed through ``/moor <command>`` on Slack only: the registry
...
_SLACK_VIA_MOOR_ONLY = frozenset({
...
def slack_native_slashes() -> list[tuple[str, str, str]]:
    ...
    _SLACK_VIA_MOOR_ONLY are skipped. ``/moor`` is always first for anything dropped."""
...
def slack_app_manifest(
    request_url: str = "https://moor-agent.local/slack/commands") -> dict[str, Any]:
...
def slack_subcommand_map() -> dict[str, str]:
    """name/alias -> "/command" for the Slack ``/moor`` handler, plugin commands included."""
