"""``moor gateway enroll`` — enroll a self-hosted gateway with a relay connector.
...
``docs/connector-gateway-auth-design.md``:
  ...
  1. Resolve a fresh Moor Portal access token from the existing login
     (``~/.moor/auth.json``) — the same path ``moor dashboard register``
     uses (``resolve_moor_access_token``). This proves *which Moor org (tenant)*
  ...
  4. Persist ``GATEWAY_RELAY_ID`` / ``GATEWAY_RELAY_SECRET`` /
     ...
     ``~/.moor/.env``. The per-gateway secret authenticates the WS upgrade;
...
def _resolve_identity_token() -> str:
    """Resolve the caller-identity bearer token (generic-OIDC or Moor Portal).
    ...
    self-hosted-IdP path; otherwise Moor Portal). Raises RuntimeError on failure.
    """
...
def _post_enroll(
    *,
    ...
    try:
        ...
        if exc.code == 401:
            raise RuntimeError(
                "Connector rejected the caller identity (401). Your Moor Portal "
                "token could not be verified — try `moor auth add moor` and retry."
            ) from exc
        if exc.code == 403:
            raise RuntimeError(
                detail
...
def cmd_gateway_enroll(args) -> None:
    ...
    if is_managed():
        print(
            "✗ `moor gateway enroll` is not available in a managed/hosted install.\n"
    ...
    #    gapped / self-hosted-IdP, NO Moor Portal); otherwise the Moor Portal token.
    try:
    ...
    #    save_env_value writes them to ~/.moor/.env (0600 dir) and never logs.
    to_write = {
    ...
    # process is launched under (load_moor_dotenv exports that .env), so
...
def _warn_if_secondary_multiplex_profile() -> bool:
    ...
    in moor_cli/gateway.py. Best-effort: any failure to determine the
    ...
    try:
        ...
        default_root = Path(get_default_moor_root()).resolve()
