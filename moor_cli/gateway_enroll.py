...
def cmd_gateway_enroll(args) -> None:
    ...
    # is set (air-gapped / self-hosted IdP), otherwise Moor Portal.
    try:
        ...
        if getattr(exc, "relogin_required", False):
            _fail(
                "✗ You're not logged into Moor Portal.",
                "  Run `moor setup` (or `moor auth add moor`) first, then retry.",
            )
        _fail(f"✗ Could not resolve a Moor Portal access token: {exc}")
    ...
    # Persist idempotently; save_env_value writes the sensitive values to ~/.moor/.env and never
