...
def _serve_unit_recovery_available() -> bool:
    """Can a fresh process restart ``moor-serve*`` units on this host?"""
...
def _qualified_serve_skips(skip_units) -> list[dict]:
    ...
    ``moor-serve.service`` can exist in BOTH the user and system manager (two processes)."""
    rows: list[dict] = []
...
def _run_fresh_recovery_process(
    ...
    """Spawn ``moor_cli.update_restart_recovery --stdin`` detached from this process; None when it
    could not run (no systemd-run in gateway mode, OSError, timeout) — the caller fails closed."""
    command = [sys.executable, "-m", "moor_cli.update_restart_recovery", "--stdin"]
    env = os.environ.copy()
    env["MOOR_UPDATE_RESTART_RECOVERY"] = "1"
    for marker in ("_MOOR_GATEWAY", "MOOR_GATEWAY", "MOOR_GATEWAY_MODE"):
        env.pop(marker, None)
...
def _recover_gateway_restart_after_abort(
    ...
    part of the identity, not decoration: ``moor-serve.service`` can exist in both the user and the system
    ...
    """
    from moor_cli.update_cmd import _gateway_recovery_partition
...
def _warn_stale_serve_runtimes(rows) -> None:
    """Name the serve/dashboard processes still on pre-update code: ``moor serve`` hosts
    ...
    print(
        "    Restart them before using Moor again, e.g. `systemctl --user restart moor-serve.service`"
        " or by relaunching `moor serve` / the Desktop app.")
