...
_GATEWAY_ENV = (("PYTHONIOENCODING", "utf-8"), ("MOOR_GATEWAY_DETACHED", "1"), ("MOOR_SUPERVISED_CHILD", "1"))
...
def _launch_elevated_gateway_command(command: str, extra_args: list[str] | None = None) -> bool:
    ...
    """
    _assert_windows()
    args = ["-m", "moor_cli.main", *_current_profile_cli_args(), "gateway", command, *(extra_args or [])]
...
def _launch_elevated_install(force: bool = False, *, start_now: bool | None = None, start_on_login: bool | None = None) -> bool:
    ...
    overrides = {"MOOR_GATEWAY_ELEVATED_HANDOFF": "1"}
    extra_args = ["--elevated-handoff"]
    if force:
        extra_args.append("--force")
    for choice, env_key, flag in (
        (start_now, "MOOR_GATEWAY_INSTALL_START_NOW", "start-now"),
        (start_on_login, "MOOR_GATEWAY_INSTALL_START_ON_LOGIN", "start-on-login"),
    ):
...
def get_task_name() -> str:
    ...
    _assert_windows()
    from moor_cli.gateway import _profile_suffix  # local: avoids circular init during boot
...
def get_task_script_path() -> Path:
    """The generated ``gateway.cmd`` wrapper under ``<MOOR_HOME>/gateway-service/`` (per-profile
    installs stay self-contained); the VBS launcher lives beside it."""
    _assert_windows()
    script_dir = _moor_home() / "gateway-service"
...
def _stable_gateway_working_dir(project_root: Path) -> str:
    """Stable cwd for detached/startup runs: anchor at MOOR_HOME when it exists (mirrors the POSIX
...
def _gateway_run_argv(python_exe: str, profile_arg: str) -> list[str]:
    """``python -m moor_cli.main [--profile X] gateway run`` — shared by every launcher renderer."""
...
def _build_gateway_cmd_script(python_path: str, working_dir: str, moor_home: str, profile_arg: str) -> str:
    ...
    lines = [
        ...
        f'set "MOOR_HOME={moor_home}"',
        *[f'set "{k}={v}"' for k, v in _GATEWAY_ENV],
        # VIRTUAL_ENV lets the gateway's own python detection find the venv.
        f'set "VIRTUAL_ENV={_preserve_moor_home_path(venv_dir)}"',
...
def _build_gateway_vbs_script(python_path: str, working_dir: str, moor_home: str, profile_arg: str) -> str:
    ...
    lines = [
        ...
        f"env.Item({q('MOOR_HOME')}) = {q(moor_home)}",
        *[f"env.Item({q(k)}) = {q(v)}" for k, v in _GATEWAY_ENV],
        f"env.Item({q('VIRTUAL_ENV')}) = {q(_preserve_moor_home_path(venv_dir))}",
...
def _build_gateway_argv() -> tuple[list[str], str, dict[str, str]]:
    ...
    python_path, working_dir, moor_home, profile_arg = _launcher_settings()
...
def windowless_gateway_restart_spec(run_argv: list[str]) -> tuple[list[str], str, dict[str, str]]:
    ...
    interpreter via ``_resolve_detached_python`` and supplies the stable cwd + env overlay (MOOR_HOME,
    ...
    try:
        moor_home = str(_moor_home().resolve())
    except Exception:
        moor_home = ""
    env_overlay: dict[str, str] = {"PYTHONIOENCODING": "utf-8", "MOOR_GATEWAY_DETACHED": "1", "VIRTUAL_ENV": str(venv_dir)}
    if moor_home:
...
def _prompt_install_choices(start_now: bool | None = None, start_on_login: bool | None = None) -> tuple[bool, bool]:
    ...
    if start_now is None:
        start_now = _install_choice_from_env("MOOR_GATEWAY_INSTALL_START_NOW")
...
def _start_attestation_path() -> Path:
    return _moor_home().joinpath(*_START_ATTESTATION_RELATIVE)
...
def _attested_pid_exited_cleanly(pid: int) -> bool:
    ...
    try:
        ...
        data = json.loads(get_lifecycle_sentinel_path(_moor_home()).read_text(encoding="utf-8"))
    except Exception:
        return False
...
def check_start_attestation(current_pids: list[int] | None = None) -> str | None:
    ...
    lines = [
        ...
        "shutdown record.",
        "  This usually means the shell that ran `moor gateway start` was inside "
...
def _report_gateway_start(via: str) -> None:
    ...
    if pids:
        ...
        print(f"  Check the log for startup errors:\n    type {_moor_home()}\\logs\\gateway.log\n    type {_moor_home()}\\logs\\gateway-stdio.log")
...
def _print_next_steps() -> None:
    ...
    print(f"  type {_moor_home()}\\logs\\gateway.log       # View logs")
