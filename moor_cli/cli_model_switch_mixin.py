...
Mixin split out of ``cli.py``; bound onto ``MoorCLI`` via the MRO. cli.py-internal
...
module-level functions taking ``cli`` and siblings are called as ``MoorCLI.<name>(self, ...)``.
"""
...
def _print_switch_summary(cli, result, old_model, *, one_turn: bool, strict_context: bool) -> None:
    ...
    from moor_cli.model_switch import format_model_for_display, resolve_display_context_length
    ...
    # Provider-aware context chain: Codex OAuth / Copilot / Moor caps win over the raw
...
def _commit_model_switch(
    ...
    if not one_turn:
        MoorCLI._persist_model_switch_to_session(cli, result)
...
def _persist_global_switch(cli, result) -> None:
    ...
    from cli import MoorCLI, save_config_value
    MoorCLI._clear_persisted_context_for_model_switch(cli, result)
