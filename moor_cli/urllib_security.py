...
_CA_BUNDLE_ENV_VARS = ("MOOR_CA_BUNDLE", "SSL_CERT_FILE", "REQUESTS_CA_BUNDLE", "CURL_CA_BUNDLE")
...
def _secure_opener_from_installed_policy(original_url: str, *, ssl_context=None):
    ...
    (``ssl_ca_cert``/``ssl_verify``) apply; with None a moor-owned opener gets the explicit CA
    ...
    secured._moor_initial_addheaders = list(getattr(installed, "addheaders", ()))
    secured.addheaders = []
    return secured
...
def open_credentialed_url(
    ...
    *,
    ...
    if opener_factory is None:
        ...
        for name, value in getattr(opener, "_moor_initial_addheaders", ()):
