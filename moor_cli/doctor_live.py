"""``moor doctor --live`` — opt-in bounded real-call tool-backend probes.

Opt-in only: these probes make real (cheap, metadata/read-only) network calls and may spend a
trivial amount of quota. They run ONLY when the user passes ``moor doctor --live``.
"""
...
def _browser_available() -> bool:
    ...
    if shutil.which("agent-browser"):
        return True
    try:
        from moor_cli.doctor import MOOR_HOME, PROJECT_ROOT
        if (PROJECT_ROOT / "node_modules" / "agent-browser").exists():
            return True
        for candidate in (MOOR_HOME / "node" / "bin", MOOR_HOME / "node", MOOR_HOME / "node_modules" / ".bin"):
    ...
    # probes above. Mirror the rung moor_cli.doctor uses so this probe can't diverge from it, including
