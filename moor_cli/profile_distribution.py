...
* ``moor profile export/import`` — local backup / restore for a profile
  on your own machine. NOT a distribution format. Stays as-is.
* ``moor skills install <url>`` — the URL install pattern we're mirroring,
  but at the profile granularity.

Subcommands (all live under ``moor profile``, not a parallel tree):

    moor profile install <source> [--name N] [--alias] [--force] [--yes]
...
class DistributionManifest:
    ...
    def from_dict(cls, data: Any) -> "DistributionManifest":
        ...
        return cls(
            ...
            description=str(data.get("description") or ""),
            moor_requires=str(data.get("moor_requires") or ""),
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.moor_requires:
            out["moor_requires"] = self.moor_requires
        if self.author:
...
def _stage_source(source: str, workdir: Path) -> Tuple[Path, str]:
    ...
    installed manifest's ``source:`` field so ``moor profile update`` can
...
def plan_install(
    ...
    check_moor_requires(manifest.moor_requires, moor_version)
    ...
    if canon == "default":
        raise DistributionError(
            ...
            "root profile (~/.moor).  Pass --name <name> to install under a "
            "new profile."
        )
    manifest.name = canon
    ...
    with tempfile.TemporaryDirectory(prefix="moor_dist_install_") as tmp:
        ...
        if plan.existing and not force:
            raise DistributionError(
                ...
                "Use `moor profile update` to upgrade in place, "
