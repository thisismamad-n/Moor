"""Implementation of the ``moor bundles`` CLI subcommand.

Mirrors the structure of ``moor_cli/skills_hub.py`` but for skill
...
def _console() -> Console:
    # Bind to stderr so piping `moor bundles list | grep …` doesn't
...
def _cmd_list(args) -> None:
    ...
    if not bundles:
        c.print(
            ...
            f"  moor bundles create <name> --skill skill1 --skill skill2[/]\n"
...
def register_cli(subparser) -> None:
    """Build the ``moor bundles`` argparse tree.

    Called from ``moor_cli/main.py`` where it owns the top-level
    ...
    p_create.add_argument(
        ...
        help="Human-readable description shown in /help and `moor bundles list`",
    )
    p_create.add_argument(
...
def bundles_command(args) -> None:
    """Dispatch ``moor bundles <subcommand>`` to the right handler."""
