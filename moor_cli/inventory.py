...
- ``list_authenticated_providers`` already populates each row's
  ...
  curation and pulls in non-agentic models (Moor /models returns ~400
...
def build_models_payload(
    ...
    *,
    ...
    - ``pricing``: enrich each row with formatted per-model pricing and,
      for Moor, ``free_tier``/``unavailable_models`` so the GUI picker can
      show $/Mtok columns and gate paid models on free accounts —
      mirroring the ``moor model`` CLI picker. Adds network calls
      (pricing fetch + Moor tier check); only set for interactive pickers.
    ...
    - ``force_fresh_moor_tier``: bypass the short Moor free-tier cache when
      selecting Portal-recommended Moor models and applying tier gating. Keep
    ...
    rows = list_authenticated_providers(
        ...
        force_fresh_moor_tier=force_fresh_moor_tier,
    ...
    try:
        from moor_cli.providers import is_routing_aggregator as _is_routing_aggregator
    except Exception:
    ...
    if pricing:
        _apply_pricing(rows, force_fresh_moor_tier=force_fresh_moor_tier)
    if capabilities:
...
def build_aux_picker_rows(
    *,
    ...
    back. Mirrors the same filter in ``moor_cli/moa_cmd.py``.
...
def _reasoning_catalog_reader(slug: str):
    ...
    if slug == "moor":
        warm_moor_reasoning_caps_async()
        return moor_model_reasoning_capabilities
    if slug == "openrouter":
        warm_openrouter_reasoning_caps_async()
...
def _apply_featured(rows: list[dict]) -> None:
    ...
    Aggregator providers (moor, openrouter) serve dozens of models across many
...
def _apply_custom_aliases(rows: list[dict]) -> None:
    ...
    :func:`moor_cli.providers.custom_provider_aliases` — lets the frontend
...
def _append_unconfigured_rows(
    ...
    *,
    ...
    from moor_cli.models import CANONICAL_PROVIDERS, _PROVIDER_LABELS
...
def _anthropic_oauth_credentials_present() -> bool:
    ...
    model.provider / API-key env vars: Moor' own Anthropic device flow
    ...
    try:
        ...
        moor_creds = read_moor_oauth_credentials() or {}
        if moor_creds.get("accessToken"):
            return True
...
def _filter_explicit_provider_rows(rows: list[dict], ctx: ConfigContext) -> list[dict]:
    ...
    pickers want the narrower subset the user explicitly configured for Moor.
    """
    ...
    for row in rows:
        ...
        if slug == "anthropic" and _anthropic_oauth_credentials_present():
            # Anthropic OAuth logins (Moor device flow / Claude Code) are
...
def _apply_pricing(
    ...
    *,
    ...
    # Resolve Moor free-tier once (cached in models.py for the TTL window).
    moor_free_tier: Optional[bool] = None
    ...
    for row in rows:
        ...
        for mid in models:
            ...
            # Sale chrome is Moor Portal-only. Other providers (OpenRouter,
        ...
        if slug == "moor":
            try:
                if moor_free_tier is None:
                    moor_free_tier = check_moor_free_tier(
                        force_fresh=force_fresh_moor_tier
                    )
                row["free_tier"] = bool(moor_free_tier)
                if moor_free_tier:
                    _selectable, unavailable = partition_moor_models_by_tier(
...
def _moa_provider_row(current_provider: str = "") -> dict | None:
    ...
    picker path (:func:`moor_cli.model_switch.list_picker_providers`) so the
