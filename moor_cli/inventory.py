...
def build_models_payload(
    ...
    capabilities: bool = False, featured: bool = False, force_fresh_moor_tier: bool = False,
    ...
    rows = list_authenticated_providers(
        ...
        custom_providers=ctx.custom_providers, force_fresh_moor_tier=force_fresh_moor_tier,
...
def _reasoning_catalog_reader(slug: str):
    ...
    try:
        from moor_cli.models_reasoning_caps import (
            moor_model_reasoning_capabilities,
            openrouter_model_reasoning_capabilities,
            warm_moor_reasoning_caps_async,
    ...
    readers = {
        "moor": (warm_moor_reasoning_caps_async, moor_model_reasoning_capabilities),
...
def _apply_custom_aliases(rows: list[dict]) -> None:
    ...
    :func:`moor_cli.providers.custom_provider_aliases` — lets the frontend do a membership check instead.
    """
    from moor_cli.providers import custom_provider_aliases
    ...
    for entry in CANONICAL_PROVIDERS:
        ...
        if entry.slug.lower() == cur:
            ...
            warning = (
                ...
                else "Configured provider is not authenticated; run `moor model` to reactivate. "
                "Showing the saved model only."
            )
...
def _anthropic_oauth_credentials_present() -> bool:
    """True when the user explicitly authenticated Anthropic via OAuth (Moor device flow or Claude Code
    ...
    try:
        ...
        readers = (read_moor_oauth_credentials, read_claude_code_credentials)
...
def _external_process_signed_in(slug: str) -> bool:
    ...
    try:
        from moor_cli.auth import PROVIDER_REGISTRY, get_external_process_provider_status
...
def _apply_picker_hints(rows: list[dict]) -> None:
    ...
    for row in rows:
        ...
        row["warning"] = (f"paste {key_env} to activate" if auth_type == "api_key" and key_env
                          else f"run `moor model` to configure ({auth_type})")
...
def _apply_pricing(rows: list[dict], *, force_fresh_moor_tier: bool = False, cached_only: bool = False) -> None:
    """Set ``row["pricing"] = {model_id: {input, output, cache | None, free}}``; for Moor also
    ``free_tier`` (account is free-tier) and ``unavailable_models`` (paid models a free user can't pick).
    ``cached_only`` never hits the network: unknown Moor entitlement fails closed (``free_tier_pending``,
    ...
    from moor_cli.models import (
        ...
        get_cached_moor_free_tier,
        partition_moor_models_by_tier,
    )

    moor_free_tier: Optional[bool] = None  # resolved once (cached in models.py for the TTL window)
    ...
    for row in rows:
        ...
        try:
            ...
            raw_pricing = {}
        cached_moor_tier: Optional[bool] = None
        if slug == "moor" and cached_only:
            cached_moor_tier = get_cached_moor_free_tier()
            if cached_moor_tier is None:
                ...
                if not row.get("warning"):  # say why every model renders locked
                    row["warning"] = ("Checking Moor plan entitlement… models unlock on the "
                                      "next picker open or refresh.")
                continue
        if not raw_pricing:
            if slug == "moor":
                row["free_tier"] = bool(cached_moor_tier)
                row["pricing_pending"] = True
                row["unavailable_models"] = list(models) if cached_moor_tier else []
            continue
        ...
        for mid in models:
            ...
            # Sale chrome is Moor Portal-only (other catalogs' nested pricing.original is ignored); free
        ...
        if slug == "moor":
            try:
                if moor_free_tier is None:
                    moor_free_tier = (cached_moor_tier if cached_only
                                      else check_moor_free_tier(force_fresh=force_fresh_moor_tier))
                row["free_tier"] = bool(moor_free_tier)
                row["unavailable_models"] = (
                    partition_moor_models_by_tier(list(models), raw_pricing, free_tier=True)[1]
                    if moor_free_tier else [])
...
def _prewarm_pricing_async(
    ...
    from moor_cli.models_pricing import pricing_cache_scope
    ...
    endpoint_scope = tuple(sorted(
        ...
        for slug in slugs))
    prewarm_key = (moor_home_key(), endpoint_scope)
    ...
    with _pricing_prewarm_lock:
        ...
        thread = Thread(target=copy_context().run, args=(_worker,),
                        name="moor-picker-pricing-prewarm", daemon=True)
