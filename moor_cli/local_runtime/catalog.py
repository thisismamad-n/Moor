...
_CATALOG_URL = ("https://raw.githubusercontent.com/NousResearch/hermes-agent"
                "/main/moor_cli/local_runtime/catalog.json")
_SCHEMA_VERSION = 1
...
def _packaged_catalog() -> "tuple[CatalogEntry, ...]":
    ...
    raw = files("moor_cli.local_runtime").joinpath("catalog.json").read_text(encoding="utf-8-sig")
...
def refresh_catalog(force: bool = False) -> bool:
    ...
    try:
        req = urllib.request.Request(_CATALOG_URL, headers={"User-Agent": "moor-local-runtime"})
