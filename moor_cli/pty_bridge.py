"""PTY bridge for `moor dashboard` chat tab.
...
``moor_cli.web_server``. POSIX-only: depends on ``fcntl``, ``termios`` and ``ptyprocess`` (native
...
# ui-tui/packages/moor-ink/src/ink/termio/host.ts — keep the two in sync.
PTY_HOST_ENV = "MOOR_PTY_HOST"
        ...
        if not _PTY_AVAILABLE:
            if sys.platform.startswith("win"):
                raise PtyUnavailableError("Pseudo-terminals are unavailable on this platform. "
                                          "Moor Agent supports Windows only via WSL.")
