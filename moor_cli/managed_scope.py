...
DISTINCT from ``moor_cli.config.is_managed()`` / ``MOOR_MANAGED`` (a coarse package-manager
...
def _under_pytest() -> bool:
    """True inside the test suite: ignore the system ``/etc/moor`` so a real managed scope on a
    dev/CI box can't leak policy into the suite. An explicit ``MOOR_MANAGED_DIR`` still wins."""
...
def get_managed_dir() -> Optional[Path]:
    ...
    Priority: ``$MOOR_MANAGED_DIR`` (IT-only bootstrap override; never persisted to any .env;
    honored only when non-empty AND the directory exists), then ``/etc/moor`` when it exists.
