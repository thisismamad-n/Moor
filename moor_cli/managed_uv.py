...
The Python backing the install is shared by every Moor profile because the checkout's ``venv``
is shared. Runtime repair therefore uses an install-scoped store under
``<checkout>/.moor-runtime/python``. A vulnerable interpreter is never reinstalled in place.
"""
...
_RUNTIME_DIR_NAME = ".moor-runtime"
_VENV_NAME = "venv"
_ALT_VENV_NAME = ".venv"
_REPAIR_LOCK_NAME = "runtime-repair.lock"
_MACOS_MANAGED_PYTHON_IDENTIFIER = "com.moorinc.moor.managed-python"

_Provisioned = tuple[Path, Path, SQLiteRuntimeInfo]


def managed_uv_path() -> Path:
    """Path of Moor' own uv binary (``$MOOR_HOME/bin/uv[.exe]``); may not exist yet."""
    return get_moor_home() / "bin" / ("uv.exe" if platform.system() == "Windows" else "uv")
...
def _report_runtime_repair_failure(repair: RuntimeRepairResult) -> None:
    if repair.backup_venv is None:
        ...
        print("    Sessions stay protected meanwhile: Moor keeps databases "
              "out of WAL mode on this SQLite build. The next `moor update` "
              "will retry.")
        return
...
def _uv_self_update_stamp() -> Path:
    from moor_constants import get_moor_home
    return get_moor_home() / "cache" / ".uv_self_update_stamp"


def _uv_self_update_is_fresh(now: float | None = None) -> bool:
    """True when ``uv self update`` ran recently enough to skip.

    uv releases roughly weekly while many users run ``moor update`` daily; a blocking network
...
def _reload_moor_constants():
    """Re-execute ``moor_constants`` from disk (the imported one may predate venv_python_path)."""
    import moor_constants
    return importlib.reload(moor_constants)
...
def _attempt_install_generation(
    ...
    try:
        ...
        return reject("uv resolved Python outside the Moor generation: %s", python)
...
def _install_safe_python_generation(
    ...
    # supported minor (e.g. 3.11 → 3.12) so the user isn't stuck on every `moor update`. The
...
def _windows_runtime_holders() -> tuple[bool, str]:
    if platform.system() != "Windows":
        return False, ""
    main_module = sys.modules.get("moor_cli.main")
...
def _windows_runtime_self_lock(live: Path) -> tuple[bool, str]:
    ...
    (``moor update`` itself runs from the venv python), which is correct for the dependency-sync
    ...
    updater's own ``python.exe`` (or a waiting ``moor.exe`` launcher ancestor) can never be renamed from
    ...
    # Belt-and-braces: the venv\Scripts\moor.exe launcher stays mapped while it waits for this
...
def _repair_windows_preflight(
    ...
    if self_locked:
        ...
        for line in (
            ...
            # See #93032.
            "    Retrying `moor update` from inside this venv cannot help: "
            ...
            f"      cd {root}",
            "      <system Python> -m moor_cli.main update",
...
def _install_uv(target: Path) -> None:
    ...
    into ``$MOOR_HOME/bin/`` instead of ``~/.local/bin/``.
    """
