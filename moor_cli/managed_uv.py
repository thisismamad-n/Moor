...
Moor owns its own uv binary at ``$MOOR_HOME/bin/uv`` (or ``uv.exe`` on
Windows).  Every code path that needs uv resolves it from that single location.
If the binary is missing, ``ensure_uv()`` bootstraps it via the official
standalone installer with ``UV_UNMANAGED_INSTALL`` / ``UV_INSTALL_DIR`` pointed
at ``$MOOR_HOME/bin`` so the installer writes directly there — no PATH
...
uses an install-scoped store under ``<checkout>/.moor-runtime/python``. A
...
from moor_cli.sqlite_runtime import SQLiteRuntimeInfo, probe_sqlite_runtime
...
def managed_uv_path() -> Path:
    ...
    ``$MOOR_HOME/bin/uv`` on POSIX, ``$MOOR_HOME\\bin\\uv.exe`` on
...
class _UvResult(str):
    ...
    ``(path, fresh_bootstrap)`` tuple across releases. ``moor update`` runs
    the call site from the *old*, already-imported ``moor_cli.main`` against
...
def _uv_self_update_is_fresh(now: float | None = None) -> bool:
    ...
    uv releases roughly weekly while many users run ``moor update`` daily;
    re-running a blocking network self-update on every invocation is waste
    and, offline, an unbounded hang risk. A stamp file under MOOR_HOME
    caches the last successful self-update time.
    """
    try:
        from moor_constants import get_moor_home

        stamp = get_moor_home() / "cache" / ".uv_self_update_stamp"
...
def update_managed_uv(
    *,
    ...
    Call this during ``moor update`` so the managed copy stays current.
...
def _reload_moor_constants():
    """Re-execute ``moor_constants`` from disk and return the fresh module.

    ``moor update`` imports ``moor_constants`` from the OLD checkout,
...
def _install_safe_python_generation(
    ...
    *,
    ...
    # user isn't stuck on every `moor update` with no path to a fixed
...
def _default_live_venv(root: Path) -> Path:
    ...
    ``not-applicable`` on every ``moor update`` and stayed on
    journal_mode=DELETE forever — even though the WAL fallback warning
    promises that ``moor update`` repairs the runtime (issue class:
...
def _install_uv(target: Path) -> None:
    ...
    ``$MOOR_HOME/bin/`` instead of ``~/.local/bin/``.
    """
