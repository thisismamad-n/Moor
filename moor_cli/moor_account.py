...
# Portal's TOOL_COVERAGE_CATEGORIES (moor-account-service
...
def moor_portal_topup_url(account_info: Optional[MoorPortalAccountInfo] = None) -> str:
    ...
    """
    base_billing = moor_portal_billing_url(account_info)  # {base}/billing
...
def format_moor_portal_entitlement_message(
    account_info: Optional[MoorPortalAccountInfo],
    *,
    capability: str = "this feature",
    include_refresh_hint: bool = True,
    coverage_category: Optional[str] = None,
) -> Optional[str]:
    """Return user-facing guidance for a missing Moor tool-gateway entitlement.
    ...
    if account_info is not None:
        if coverage_category is not None:
            ...
            if account_info.tool_gateway_entitled:
                ...
                return (
                    f"{capability} isn't included with your current Moor Portal "
    ...
    if account_info is None:
        return (
            f"Moor could not verify your Moor Portal entitlement, so {capability} "
            f"is unavailable. Run `moor model` to refresh your login, or check "
    ...
    if not account_info.logged_in:
        if account_info.inference_credential_present:
            return (
                f"Moor inference credentials are configured, but Moor cannot verify "
                f"your Moor Portal paid access for {capability}. Log in with "
                f"`moor model` to enable Portal-managed features. Billing and "
    ...
    if account_info.paid_service_access is None:
        detail = (
            f"Moor could not verify your Moor Portal paid access, so {capability} "
            f"is unavailable."
        )
    ...
    if reason == "account_missing":
        return (
            f"Moor could not find a Moor Portal account or organisation for this "
            f"login, so {capability} is unavailable. Run `moor model` to "
            f"authenticate again; if the problem persists, contact Moor support."
        )
...
def _no_paid_access_message(
    account_info: MoorPortalAccountInfo,
    capability: str,
    billing_url: str,
) -> str:
    access = account_info.paid_service_access_info
    ...
    if access and access.member_spend_cap_exceeded:
        ...
        return (
            f"Your Moor Portal access is paused because you've exceeded the"
            f" per-member spend cap set by your organisation.{cap_detail}"
            f"{credit_detail} Ask your organisation admin to raise the"
            f" member spend cap at {billing_url}, then run `moor model`"
            f" to refresh."
        )
    ...
    if has_active_subscription and active_subscription_is_paid:
        credit_detail = _credit_detail(total_usable, subscription_credits, purchased_credits)
        return (
            f"Your Moor Portal credits are exhausted{credit_detail}, so {capability} "
    ...
    if has_active_subscription is False:
        credit_detail = _credit_detail(total_usable, subscription_credits, purchased_credits)
        return (
            f"Your Moor Portal account has no active subscription or usable credits"
...
def get_moor_portal_account_info(
    *,
    force_fresh: bool = False,
    min_jwt_ttl_seconds: int = 60,
) -> MoorPortalAccountInfo:
    """Return normalized Moor Portal account entitlement information.
...
def _info_from_oauth_pool(
    *,
    ...
    try:
        payload = _fetch_moor_account_info(access_token, entry_portal_url)
    except Exception as exc:
        return _error_info(
            error=exc,
            logged_in=True,
...
def _tool_access_from_value(value: Any) -> Optional[MoorToolAccessInfo]:
    ...
    API) into :class:`MoorToolAccessInfo`. Fails closed: a non-object value
    ...
    return MoorToolAccessInfo(enabled=enabled, coverage=coverage)


def _subscription_from_payload(value: Any) -> Optional[MoorPortalSubscriptionInfo]:
    if not isinstance(value, dict):
        return None
    return MoorPortalSubscriptionInfo(
...
def _paid_service_access_from_payload(value: Any) -> Optional[MoorPaidServiceAccessInfo]:
    if not isinstance(value, dict):
        return None
    allowed = _coerce_bool(value.get("allowed"))
    paid_access = _coerce_bool(value.get("paid_access"))
    return MoorPaidServiceAccessInfo(
        allowed=allowed,
        paid_access=paid_access,
