"""Normalized Moor Portal account entitlement helpers."""
...
MoorAccountInfoSource = Literal["jwt", "account_api", "inference_key", "none", "error"]
...
_account_info_cache: tuple[str, float, "MoorPortalAccountInfo"] | None = None
_ACCOUNT_INFO_CACHE_LOCK = threading.Lock()


@dataclass(frozen=True)
class MoorPortalSubscriptionInfo:
...
@dataclass(frozen=True)
class MoorPaidServiceAccessInfo:
...
FREE_TIER_NEEDS_ACCOUNT = "This needs a Moor account. Run `moor auth upgrade`."
FREE_TIER_NEEDS_ACCOUNT_CHAT = "This needs a Moor account. Use /login to sign in."


def _is_anonymous_tier(account_info: Optional["MoorPortalAccountInfo"]) -> bool:
...
class MoorPortalAccountInfo:
    ...
    subscription: Optional[MoorPortalSubscriptionInfo] = None
    paid_service_access: Optional[bool] = None
    paid_service_access_info: Optional[MoorPaidServiceAccessInfo] = None
    tool_access: Optional[MoorToolAccessInfo] = None
    raw_claims: Optional[dict[str, Any]] = None
    raw_account: Optional[dict[str, Any]] = None
    error: Optional[str] = None
    # NAS account tier claim; ``"anonymous"`` is the free tier (no Moor account behind it).
    ...
    def is_anonymous_tier(self) -> bool:
        """The free tier: no Moor account, so no billing, credits, or entitlement to speak of."""
...
def moor_portal_billing_url(account_info: Optional[MoorPortalAccountInfo] = None) -> str:
    """Return the billing URL for a normalized Moor account snapshot."""
    try:
    ...
    if not _nonblank(base):
        base = DEFAULT_MOOR_PORTAL_URL
    return f"{base.rstrip('/')}/billing"


def moor_portal_topup_url(account_info: Optional[MoorPortalAccountInfo] = None) -> str:
    ...
    ``/orgs/None/billing``.
    """
    base = moor_portal_billing_url(account_info)[: -len("/billing")]
...
def format_moor_portal_entitlement_message(
    ...
    if account_info is not None:
        if coverage_category is not None:
            ...
            if account_info.tool_gateway_entitled:
                return (
                    f"{capability} isn't included with your current Moor Portal access. "
    ...
    if account_info is None:
        return (
            f"Moor could not verify your Moor Portal entitlement, so {capability} is unavailable. "
            f"Run `moor model` to refresh your login, or check billing at {billing_url}."
        )
    if not account_info.logged_in:
        if account_info.inference_credential_present:
            return (
                f"Moor inference credentials are configured, but Moor cannot verify your Moor Portal "
                f"paid access for {capability}. Log in with `moor model` to enable Portal-managed "
                f"features. Billing and credits are managed at {billing_url}."
            )
        return (
            f"Log in to Moor Portal to use {capability}: run `moor model`. "
            f"Billing and credits are managed at {billing_url}."
        )
    if account_info.paid_service_access is None:
        detail = f"Moor could not verify your Moor Portal paid access, so {capability} is unavailable."
        if account_info.error:
            detail += f" Account lookup failed: {account_info.error}."
        if include_refresh_hint:
            detail += " Run `moor model` to refresh your session."
    ...
    if reason == "account_missing":
        return (
            f"Moor could not find a Moor Portal account or organisation for this login, so {capability} "
    ...
    if reason == "no_usable_credits" or account_info.paid_service_access is False:
        ...
        return message
    return (
        f"Your Moor Portal account does not currently have paid service access, "
        f"so {capability} is unavailable. Add credits or update billing at {billing_url}."
    )


def _no_paid_access_message(
    account_info: MoorPortalAccountInfo, capability: str, billing_url: str, *, in_chat: bool = False,
) -> str:
    if _is_anonymous_tier(account_info):
        return FREE_TIER_NEEDS_ACCOUNT_CHAT if in_chat else FREE_TIER_NEEDS_ACCOUNT
    ...
    if access.member_spend_cap_exceeded:
        ...
        return (
            f"Your Moor Portal access is paused because you've exceeded the per-member spend cap set by "
            f"your organisation.{cap_detail}{credit_detail} Ask your organisation admin to raise the "
            f"member spend cap at {billing_url}, then run `moor model` to refresh."
        )
    if active and paid:
        return (
            f"Your Moor Portal credits are exhausted{credit_detail}, so {capability} is unavailable. "
            f"Top up or renew credits at {billing_url}."
        )
    if active and paid is False:
        return (
            f"Your current Moor Portal plan does not include paid service access, "
            f"so {capability} is unavailable. Upgrade or add credits at {billing_url}."
        )
    if active is False:
        return (
            f"Your Moor Portal account has no active subscription or usable credits{credit_detail}, "
            f"so {capability} is unavailable. Subscribe or add credits at {billing_url}."
        )
    return (
        f"Your Moor Portal account has no usable paid credits{credit_detail}, so "
        f"{capability} is unavailable. Add credits or update billing at {billing_url}."
    )


def reset_moor_portal_account_info_cache() -> None:
    """Clear the short-lived account-info cache used by tests."""
    global _account_info_cache
    _account_info_cache = None


def get_moor_portal_account_info(*, force_fresh: bool = False, min_jwt_ttl_seconds: int = 60) -> MoorPortalAccountInfo:
    """Normalized Moor Portal account entitlement.
    ...
    if not _nonblank(access_token):
        return (
            ...
            or MoorPortalAccountInfo(logged_in=False, source="none", fresh=False, portal_base_url=portal_base_url)
        )
    if not force_fresh:
...
def moor_policy_present() -> Optional[bool]:
    ...
    """
    try:
        from moor_cli.auth import get_provider_auth_state, _decode_jwt_claims

        access_token = (get_provider_auth_state("moor") or {}).get("access_token")
        if not _nonblank(access_token):
            return None
...
def moor_policy_notice(*, removed: bool) -> str:
    ...
    """
    if not removed or moor_policy_present() is not True:
        return ""
    return (
...
def _fresh_account_info(state: dict[str, Any], force_fresh: bool, portal_base_url: Optional[str]) -> MoorPortalAccountInfo:
    global _account_info_cache

    try:
        from moor_cli.auth import get_provider_auth_state, resolve_moor_access_token

        access_token = resolve_moor_access_token()
...
def _info_from_inference_key_pool(portal_base_url: Optional[str]) -> Optional[MoorPortalAccountInfo]:
    """Return an explicit unknown-entitlement snapshot for opaque Moor keys."""
    try:
        entry = _select_moor_pool_entry()
        if entry is None:
            return None
        if not _nonblank(getattr(entry, "runtime_api_key", None) or getattr(entry, "access_token", "")):
            return None
        return MoorPortalAccountInfo(
...
) -> Optional[MoorPortalAccountInfo]:
    try:
        entry = _select_moor_pool_entry()
    except Exception:
        return None
...
def _info_from_fetched_account(
    ...
    payload = _fetch_moor_account_info(access_token, portal_base_url)
    if not payload:
...
def _select_moor_pool_entry() -> Optional[Any]:
...
def _fetch_moor_account_info(access_token: str, portal_base_url: Optional[str] = None) -> dict[str, Any]:
...
def _info_from_valid_jwt(
    ...
    try:
        from moor_cli.auth import _decode_jwt_claims
    except Exception:
        return None
    ...
    access_info = MoorPaidServiceAccessInfo(
    ...
    return MoorPortalAccountInfo(
        ...
        moor_client=_coerce_str(claims.get("moor_client")),
...
def _info_from_account_payload(
    ...
    access = _coerced_dataclass(MoorPaidServiceAccessInfo, payload.get("paid_service_access"))
    paid_access = None
...
def _tool_access_from_value(value: Any) -> Optional[MoorToolAccessInfo]:
    ...
    return MoorToolAccessInfo(enabled=value.get("enabled") is True, coverage=coverage)
...
def _subscription_from_payload(value: Any) -> Optional[MoorPortalSubscriptionInfo]:
    return _coerced_dataclass(MoorPortalSubscriptionInfo, value)
