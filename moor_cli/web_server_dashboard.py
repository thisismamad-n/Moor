...
from moor_cli.config import cfg_get, get_process_moor_home
...
def _normalise_prefix(raw: Optional[str]) -> str:
    ...
    ``moor_cli.dashboard_auth.prefix`` so gate, OAuth, cookies and SPA mount agree)."""
...
def _render_active_theme_bootstrap_css() -> str:
    ...
    target palette instead of flashing the bundle's default Moor Teal until
...
def mount_spa(application: FastAPI):
    ...
    proxy (``X-Forwarded-Prefix: /moor``) the served index.html is rewritten so absolute
    asset URLs and the runtime ``__MOOR_BASE_PATH__`` honour that prefix without a rebuild.
    ...
    """
    from moor_cli.web_server import WEB_DIST, _DASHBOARD_EMBEDDED_CHAT_ENABLED, app
    from moor_cli.web_deps import _server

    # `moor serve` is the headless backend: it must NEVER serve the browser SPA, even if a
    # dist is lying around, so only the JSON-RPC/WS/API surface is reachable.
    if os.environ.get("MOOR_SERVE_HEADLESS") == "1":

        @application.get("/{full_path:path}")
        async def no_frontend(full_path: str):
            # Desktop token handshake: the Electron shell boots by fetching `/` and reading
            # ``window.__MOOR_SESSION_TOKEN__`` for /api/ws auth. When headless 404'd every
            # path, a renderer whose spawn token no longer matched (e.g. after `moor update`)
            ...
            if full_path == "" and not gated:
                return HTMLResponse(
                    ...
                    f"window.__MOOR_SESSION_TOKEN__={json.dumps(_server()._SESSION_TOKEN)};"
                    "window.__MOOR_AUTH_REQUIRED__=false;"
    ...
    # A missing WEB_DIST is deliberately NOT a mount-time terminal state (#82614): a long-lived `moor
    ...
    def _serve_index(prefix: str = ""):
        ...
        cookie auth; ``__MOOR_AUTH_REQUIRED__`` tells it which scheme to use for /api/pty
...
def _normalise_theme_definition(data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    ~/.moor/, the same trust level as config.yaml. Empty asset values are dropped so a
...
def _discover_user_themes() -> list:
    ...
    Uses the process launch home, not ``get_moor_home()``, so a transient profile override
    from embedded chat does not hide themes under the server's own ``MOOR_HOME``.
    """
...
def _dashboard_plugin_search_dirs() -> List[tuple]:
    ...
    When the process itself is profile-scoped (``MOOR_HOME=<root>/profiles/<name>``) the
    launch home has no ``plugins/`` — user plugins live in the moor root — so the default
    ...
    # context-local MOOR_HOME override (e.g. embedded /chat under --open-profile). #87197: when the
    # process itself is profile-scoped (``--profile <name>`` sets ``MOOR_HOME=<root>/profiles/<name>``),
    # the launch home is the profile directory, which has no ``plugins/`` — user plugins are installed in
    # the moor root (``~/.moor/plugins``). Scan the default root as well (``get_default_moor_root()``
    # unwraps ``<root>/profiles/<name>`` → ``<root>`` and returns a custom ``MOOR_HOME`` unchanged when it
    # *is* the root), mirroring how ``moor_cli.plugins`` resolves plugin install locations. The
    ...
    # ``true`` / ``yes`` / ``on``) so the gate matches ``moor_cli/plugins.py`` and the documented user
    # contract.
...
def _discover_dashboard_plugins() -> list:
    ...
    project plugin sources — same three sources as ``moor_cli.plugins``."""
    plugins = []
    seen_names: set = set()
...
def _plugin_api_mount_skip_reason(plugin: Dict[str, Any], enabled_set: set, disabled_set: set) -> Optional[str]:
    ...
    disable; project plugins (``./.moor/plugins/``) ship with the CWD and are
...
def _mount_plugin_api_routes():
    ...
    (``./.moor/plugins/``) ship with the CWD and are therefore attacker-controlled in any threat model
    ...
    """
    from moor_cli.web_server import _get_dashboard_plugins, app
    try:
