...
from moor_cli.web_models import CronJobCreate
...
def _cron_profile_dicts() -> List[Dict[str, Any]]:
    ...
    from moor_cli.web_server_profiles import _fallback_profile_dicts
...
def _cron_default_profile() -> str:
    ...
    profile's job into ~/.moor. ``custom`` (MOOR_HOME outside the profiles tree) has no
...
def _cron_store_scope(home: Path):
    """Point MOOR_HOME and the cron.jobs store at one profile's home for the block.
...
def _gateway_fire_endpoint(profile: str, home: Path) -> str:
    ...
    if multiplex and profile != "default":
        ...
        listener_profile, listener_home = "default", get_default_moor_root()
        _log.info(
    ...
    try:
        ...
        # expansion) — never a raw yaml.safe_load (tests/moor_cli/test_config_read_guard.py).
        from moor_constants import reset_moor_home_override, set_moor_home_override
        token = set_moor_home_override(str(listener_home))
        try:
