"""Dashboard process-hygiene helpers — extracted from ``moor_cli/main.py``.
...
``_detect_concurrent_moor_instances``) are lifted verbatim. References to
helpers that STAY in ``moor_cli.main`` (``_find_stale_dashboard_pids``,
``_respawn_dashboard_processes``, ``_is_windows``, ...) are routed through a
lazy ``_m()`` main reference so existing test monkeypatches on
``moor_cli.main.<name>`` keep reaching this code path, and imports stay
...
patches on ``moor_cli.main`` resolve unchanged.
"""
...
def _m():
    """Lazy ``moor_cli.main`` reference (call-time; keeps patches working)."""
...
def _scan_dashboard_processes(
    *,
    ...
    ``moor-dashboard.service`` systemd unit.  Managed units are restarted
    ...
    *exclude_pids* is an optional set of PIDs that must never be returned.
    ...
    backend child process: when the desktop spawns ``moor serve`` as
    ...
    environment variable ``MOOR_DESKTOP_CHILD_PID`` on the spawned
    ...
    patterns = [
        ...
        # The headless backend (`moor serve`) is the same long-lived server
    ...
    try:
        if sys.platform == "win32":
            ...
            # (rather than `pgrep -f "moor.*dashboard"`) keeps us consistent
            # with `moor_cli.gateway._scan_gateway_pids` and avoids the
...
def _moor_home_for_pid(pid: int) -> str | None:
    """Best-effort ``MOOR_HOME`` from *pid*'s environment."""
    try:
        import psutil

        home = psutil.Process(pid).environ().get("MOOR_HOME")
        if home:
            return home
    except Exception:
        pass
    ...
    for part in raw.split(b"\x00"):
        if part.startswith(b"MOOR_HOME="):
...
def _is_ephemeral_port_zero_backend(argv: list[str]) -> bool:
    ...
    ``moor update`` multiplies listening backends because ``--port 0``
...
def _profile_key_for_respawn(
    ...
    if moor_home:
        try:
            home_path = Path(moor_home).resolve()
        except (OSError, RuntimeError, ValueError):
            home_path = Path(moor_home)
        ...
        try:
            ...
            return f"home:{os.path.normcase(moor_home)}"
...
def _filter_dashboard_respawn_candidates(
    ...
    """Select which killed manual backends to respawn after ``moor update``.

    Each candidate is ``(pid, argv, moor_home)``.

    Rules (#78821):
    1. Never resurrect Desktop ephemeral ``serve|dashboard --port 0``
       backends — Desktop (``MOOR_DESKTOP_CHILD_PID``) owns their
    ...
    3. Cap at most one managed backend per profile / ``MOOR_HOME``.

    Intentionally does **not** blanket-skip every PPID-1 process: a prior
    ``moor update`` respawn detaches with ``start_new_session=True``, so
    ...
    for _pid, argv, moor_home in candidates:
        if not argv:
            continue
        ...
        if norm in seen_cmdlines:
            continue
        profile_key = _profile_key_for_respawn(argv, moor_home)
...
def _kill_stale_dashboard_processes(
    ...
    *,
    ...
    """Kill running ``moor dashboard`` / ``moor serve`` processes.
    ...
    When ``restart_managed`` is true (the ``moor update`` path), a detected
    ``moor-dashboard.service`` is restarted through systemd; any OTHER
    killed PID that was supervised by a systemd unit (custom unit names —
    e.g. a remote backend's ``moor-serve.service``) has its owning unit
    ...
    *already_restarted_units* names units (no ``.service`` suffix) the
    caller already restarted directly — e.g. ``moor update``'s systemd
    fleet-restart loop, which restarts ``moor-serve*`` units before this
    ...
    # backend child, it sets MOOR_DESKTOP_CHILD_PID so that the update
    # path can skip killing the desktop-managed process.  (#37532)
    exclude: set[int] | None = None
    raw_pid = os.environ.get("MOOR_DESKTOP_CHILD_PID")
    if raw_pid:
    ...
    # caller asked for restarts (the `moor update` path) — `--stop` must
    ...
    if restart_managed and sys.platform != "win32":
        for pid in pids:
            ...
            if not pid_service[pid]:
                ...
                # Snapshot MOOR_HOME before the kill so per-profile caps
                # still work after the process is gone (#78821).
                cmdline = _m()._dashboard_cmdline_for_pid(pid)
                if cmdline:
                    pid_cmdline[pid] = cmdline
                    pid_home[pid] = _moor_home_for_pid(pid)

        if already_restarted_units:
            # Already handled directly by the caller (e.g. moor update's
    ...
    #    remote backend (moor serve) under Restart=on-failure never comes
...
def _detect_concurrent_moor_instances(
    ...
    The Moor Desktop Electron app spawns ``moor.EXE`` as a backend child,
    ...
    shims (``moor.exe`` / ``moor-gateway.exe``) and returns ``(pid,
    ...
    launcher (``moor.exe``) is a separate process from the Python
    ...
    for shim in _m()._moor_exe_shims(scripts_dir):
        try:
    ...
    # setuptools-generated moor.exe launcher is a separate native process
    ...
    #      moor.exe sitting *under* a non-Moor parent (e.g. a Moor
...
def _is_desktop_local_serve_cmdline(command: str) -> bool:
    ...
    Desktop primary/pool backends launch as::
        ...
        moor serve --host 127.0.0.1 --port 0
        moor serve --isolated --host 127.0.0.1 --port 0 ...
    ...
    if "serve" not in cmd:
        return False
    if "moor" not in cmd and "moor_cli" not in cmd:
        return False
...
def _exclude_pids_from_env() -> set[int]:
    """PIDs Desktop marks as live backends (MOOR_DESKTOP_CHILD_PID)."""
    raw = os.environ.get("MOOR_DESKTOP_CHILD_PID", "")
    out: set[int] = set()
...
# on the *remote* host for every ``moor serve`` backend it spawns over SSH
# (see apps/desktop/electron/remote-lifecycle.ts). A backend started from
# another client/machine — e.g. a MacBook driving a ``moor serve`` on a Mac
...
def _moor_home_dir() -> Path:
    """Resolved Moor home (MOOR_HOME override or ~/.moor)."""
    override = os.environ.get("MOOR_HOME", "").strip()
    if override:
...
def _valid_lockfile_payload(parsed: object, ownership_id: str) -> bool:
    ...
    *adds* lock-owned PIDs to its spare-set.
    ...
    for field in ("profile", "moorPath", "moorHome", "logPath", "startedAt"):
        value = parsed.get(field)
        if not isinstance(value, str) or len(value) > 1024:
            return False
    # logPath is written as ``{lock_root}/{ownershipId}/{spawnNonce}.log``. We
    # only check the suffix so a relocated MOOR_HOME (different leading path)
...
def _lock_owned_serve_pids(base_dir: Path | None = None) -> set[int]:
    ...
    Scans ``{moor_home}/desktop-ssh/<ownershipId>/backend.lock.json`` (the
    ...
    root = base_dir if base_dir is not None else (
        _moor_home_dir() / _REMOTE_LOCK_SUBDIR
    )
    owned: set[int] = set()
    if not root.is_dir():
        return owned
...
def _reap_orphaned_desktop_local_serves(
    *,
    reason: str = "orphaned desktop-local moor serve",
    signal_term=None,
    signal_kill=None,
    sleep_fn=None,
    ...
    """Kill leftover Desktop-local ``moor serve`` backends with no parent.
    ...
    running under MOOR_PARENT_PID; this helper clears *already* orphaned
    ...
    - never self / never MOOR_DESKTOP_CHILD_PID entries
    ...
    - never a candidate younger than ``_REAP_MIN_AGE_SECONDS`` (or whose age
      ...
      only after the backend reports MOOR_BACKEND_READY, so during
    ...
    for pid, cmd in scanned:
        ...
        # MOOR_BACKEND_READY, so a sibling spawned seconds ago is not yet
