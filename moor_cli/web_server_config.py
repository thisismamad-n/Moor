...
# moor, telegram) currently surface a single schema field each.
...
def _schema_with_dynamic_provider_options() -> Dict[str, Dict[str, Any]]:
    ...
    """
    from moor_cli.web_server_profiles import _plugin_terminal_backend_rows
...
def _normalize_main_model_assignment(provider: str, model: str) -> tuple[str, str]:
    ...
    1. Vendor-name → moor-provider: when the provider is not a known provider/alias but the
    ...
    from moor_cli.models import _AGGREGATOR_PROVIDERS, _KNOWN_PROVIDER_NAMES, normalize_provider
...
def _dashboard_code_skew_guard() -> Optional[str]:
    ...
    Long-lived dashboard / Desktop-owned ``moor serve`` processes freeze ``sys.modules``
    at boot; after ``moor update`` replaces the checkout, a first-time lazy import can
...
def _dashboard_skew_restart_hint() -> str:
    ...
    dashboard and Desktop-owned ``moor serve``; naming a systemd unit would mislead
    ...
    if os.environ.get("MOOR_SERVE_HEADLESS") == "1":
        return (
            ...
            "(use Restart backend in Moor Desktop, or quit and reopen the app)"
        )
    return (
        "restart this Moor process to load the new code "
        "(moor dashboard --port <port>, or the equivalent service restart for this install)"
    )
...
def _apply_moor_gateway_defaults(cfg: dict) -> list:
    """Mirror the CLI's post-model-selection behaviour when switching main to Moor: route
    *unconfigured* tools through the Moor Tool Gateway. Purely additive — tools with a direct
    ...
    try:
        ...
        return sorted(apply_moor_managed_defaults(cfg, enabled_toolsets=enabled, force_fresh=True))
    except Exception:
...
def _register_custom_endpoint(base_url: str, api_key: str, model: str) -> None:
    ...
    ``moor model`` custom flow) so the picker gets a proper ready row instead of a "needs
    setup" dead-end. Dedups by base_url; never blocks the already-persisted assignment."""
    try:
        from moor_cli.main_provider_setup import _auto_provider_name, _save_custom_provider
...
def _apply_main_assignment_sync(cfg: dict, provider: str, model: str, base_url: str, api_key: str) -> dict:
    ...
    new_provider = provider.strip().lower()
    gateway_tools = _apply_moor_gateway_defaults(cfg) if new_provider == "moor" else []
    save_config(cfg)
...
def _infer_provider_on_model_change(model_val: str, prev_provider: str) -> tuple[str, str]:
    ...
    if not name:
        return "", name
    try:
        from moor_cli.models import _AGGREGATOR_PROVIDERS, detect_provider_for_model, normalize_provider
    except Exception:
        return "", name
