"""Post-``moor update`` config-schema migration for the active profile and every sibling.
Names are re-imported by ``update_cmd`` (``moor_cli.update_cmd.<name>`` resolves/monkeypatches);
...
def _reload_config_modules() -> None:
    ...
    for mod_name in (
        "moor_cli.config_defaults", "moor_cli.config", "moor_cli.config_migrations",
...
def _migrate_sibling_profile_configs() -> list[tuple[str, int, int]]:
    ...
    sibling (active skipped): scope via the context-local MOOR_HOME override (never ``os.environ``)
    ...
    with _best_effort('Sibling profile enumeration failed: %s'):
        from moor_constants import (
            get_process_moor_home, reset_moor_home_override, set_moor_home_override)
...
def _restore_snapshot_safety_nets(pre_update_snapshot_id) -> None:
    ...
    *pre_update_snapshot_id*) and every sibling profile (against ITS OWN snapshot)."""
    ...
    with _best_effort('Sibling config auto-restore check failed: %s'):
        from moor_cli.backup import restore_config_model_settings_all_profiles
...
def _ask_configure_new_options(*, assume_yes: bool, gateway_mode: bool) -> str:
    ...
    from moor_cli.update_cmd import _gateway_prompt
    if assume_yes:
...
def _check_and_apply_config_migration(
    *, assume_yes: bool = False, gateway_mode: bool = False, pre_update_snapshot_id: str | None = None
    ...
    try:
        ...
        print("     Run 'moor config migrate' to check manually.")
        return
    ...
    # migration per sibling home via the context-local MOOR_HOME override (never os.environ).
