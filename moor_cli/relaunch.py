"""Unified self-relaunch for Moor CLI: preserves inherited flags (--tui, --dev, --profile, --model…)
across process replacement so ``moor sessions browse`` / post-setup relaunch keep the user's mode."""
...
def resolve_moor_bin() -> Optional[str]:
    """Moor entry point: ``sys.argv[0]`` if a real executable, else ``which moor``, else ``None``
    (caller falls back to ``python -m moor_cli.main``).
    ...
    lose the venv. Falling through to ``sys.executable -m moor_cli.main`` keeps the venv.
    """
    argv0 = sys.argv[0]
    ...
    def _is_unsafe_python_launcher(p: str) -> bool:
        ...
        from moor_cli.linux_desktop_entry import _needs_interpreter
...
def build_relaunch_argv(
    ...
    argv = [bin_path] if bin_path else [sys.executable, "-m", "moor_cli.main"]
...
def relaunch(
    ...
    if sys.platform == "win32":
        ...
        try:
            ...
            # Raw ``[Errno 8] Exec format error`` is cryptic; usual causes are ``moor`` not on
