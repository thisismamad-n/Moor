...
MOOR_OVERLAYS: Dict[str, MoorOverlay] = {
    ...
    "openai-codex": MoorOverlay(
        transport="codex_responses",
        auth_type="oauth_external",
        base_url_override="https://chatgpt.com/backend-api/codex",
    ),
    "openai-api": MoorOverlay(
        transport="codex_responses",
        base_url_override="https://api.openai.com/v1",
        base_url_env_var="OPENAI_BASE_URL",
    ),
    "xai-oauth": MoorOverlay(
        transport="codex_responses",
        auth_type="oauth_external",
        base_url_override="https://api.x.ai/v1",
        base_url_env_var="XAI_BASE_URL",
    ),
    "qwen-oauth": MoorOverlay(
        transport="openai_chat",
        auth_type="oauth_external",
        base_url_override="https://portal.qwen.ai/v1",
        base_url_env_var="MOOR_QWEN_BASE_URL",
    ),
    "lmstudio": MoorOverlay(
        transport="openai_chat",
        auth_type="api_key",
    ...
    "copilot-acp": MoorOverlay(
        transport="codex_responses",
        auth_type="external_process",
        base_url_override="acp://copilot",
        base_url_env_var="COPILOT_ACP_BASE_URL",
    ),
    "github-copilot": MoorOverlay(
        transport="openai_chat",
        extra_env_vars=("COPILOT_GITHUB_TOKEN", "GH_TOKEN"),
    ),
    "anthropic": MoorOverlay(
    ...
    "minimax-oauth": MoorOverlay(
        transport="anthropic_messages",
        auth_type="oauth_external",
        base_url_override="https://api.minimax.io/anthropic",
    ),
    "minimax-cn": MoorOverlay(
        transport="anthropic_messages",
        base_url_env_var="MINIMAX_CN_BASE_URL",
    ),
    "deepseek": MoorOverlay(
        transport="openai_chat",
    ...
    "alibaba-coding-plan": MoorOverlay(
        transport="openai_chat",
    ...
    "opencode": MoorOverlay(
        transport="openai_chat",
        is_aggregator=True,
        base_url_env_var="OPENCODE_ZEN_BASE_URL",
    ),
    "opencode-go": MoorOverlay(
        transport="openai_chat",
        is_aggregator=True,
        base_url_env_var="OPENCODE_GO_BASE_URL",
    ),
    "opencode-free": MoorOverlay(
        transport="openai_chat",
        is_aggregator=True,
...
def get_provider(name: str, *, allow_network: bool = True) -> Optional[ProviderDef]:
    ...
    Resolution order:
      1. Moor overlays (for providers not in models.dev: moor, openai-codex, etc.)
    ...
    if mdev_info is not None:
        ...
        # Combine env vars: models.dev env + moor extra
    ...
    # tencent-tokenhub) are absent from models.dev and MOOR_OVERLAYS, so
...
def moor_api_mode(model: str = "") -> str:
    """Resolve the wire protocol for a Moor Portal model.
    ...
    historical Moor transport — so callers that don't yet know the model
...
def determine_api_mode(provider: str, base_url: str = "", model: str = "") -> str:
    ...
    Resolution order:
      ...
      2. Moor Portal dual-wire (model-derived; overlay alone is openai_chat).
      3. Known provider → transport → TRANSPORT_TO_API_MODE.
      4. Direct provider checks (bedrock).
      5. Default: 'chat_completions'.

    *model* is optional but required for dual-wire providers (Moor) whose
    ...
    # Moor is dual-wire: anthropic/* → Messages, everything else →
    ...
    # Direct provider checks for providers not in MOOR_OVERLAYS
    if provider == "bedrock":
        return "bedrock_converse"
