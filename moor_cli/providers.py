...
# -- Moor overlay: metadata models.dev doesn't provide ----------------------
...
MOOR_OVERLAYS: Dict[str, MoorOverlay] = {
    ...
    "xai-oauth": MoorOverlay(transport="codex_responses", auth_type="oauth_external",
                               base_url_override="https://api.x.ai/v1", base_url_env_var="XAI_BASE_URL"),
    "qwen-oauth": MoorOverlay(auth_type="oauth_external", base_url_override="https://portal.qwen.ai/v1",
                                base_url_env_var="MOOR_QWEN_BASE_URL"),
    "lmstudio": MoorOverlay(extra_env_vars=("LM_API_KEY",), base_url_override="http://127.0.0.1:1234/v1",
                              base_url_env_var="LM_BASE_URL"),
    "copilot-acp": MoorOverlay(transport="codex_responses", auth_type="external_process",
                                 base_url_override="acp://copilot", base_url_env_var="COPILOT_ACP_BASE_URL"),
    "github-copilot": MoorOverlay(extra_env_vars=("COPILOT_GITHUB_TOKEN", "GH_TOKEN")),
    "anthropic": MoorOverlay(transport="anthropic_messages", extra_env_vars=("ANTHROPIC_TOKEN", "CLAUDE_CODE_OAUTH_TOKEN")),
    "zai": MoorOverlay(extra_env_vars=("GLM_API_KEY", "ZAI_API_KEY", "Z_AI_API_KEY"), base_url_env_var="GLM_BASE_URL"),
    "kimi-for-coding": MoorOverlay(base_url_env_var="KIMI_BASE_URL"),
    "stepfun": MoorOverlay(extra_env_vars=("STEPFUN_API_KEY",),
    ...
    "gmi": MoorOverlay(extra_env_vars=("GMI_API_KEY",), base_url_override="https://api.gmi-serving.com/v1",
                         base_url_env_var="GMI_BASE_URL"),
    "fireworks": MoorOverlay(extra_env_vars=("FIREWORKS_API_KEY",),
                               base_url_override="https://api.fireworks.ai/inference/v1"),
    "actual": MoorOverlay(transport="codex_responses", extra_env_vars=("ACTUAL_API_KEY", "ACTUAL_BASE_URL"),
    ...
    "nebius-token-factory": MoorOverlay(extra_env_vars=("NEBIUS_API_KEY", "NEBIUS_TOKEN_FACTORY_API_KEY"),
                                          base_url_override="https://api.tokenfactory.nebius.com/v1",
                                          base_url_env_var="NEBIUS_BASE_URL"),
    "ollama-cloud": MoorOverlay(base_url_override="https://ollama.com/v1", base_url_env_var="OLLAMA_BASE_URL"),
    # Azure Foundry serves OpenAI- and Anthropic-style endpoints; transport comes from model.api_mode.
    "azure-foundry": MoorOverlay(base_url_env_var="AZURE_FOUNDRY_BASE_URL"),
    "bedrock": MoorOverlay(transport="bedrock_converse", auth_type="aws_sdk"),
    ...
    "vertex": MoorOverlay(auth_type="vertex"),
}
...
class ProviderDef:
    ...
    doc: str = ""
    source: str = ""                      # "models.dev", "moor", "user-config"
...
_LABEL_OVERRIDES: Dict[str, str] = {
    "moa": "Mixture of Agents", "moor": "Moor Portal", "openai-codex": "ChatGPT or Codex Subscription",
...
def _overlay_pdef(canonical, ov: MoorOverlay, name, env_vars, base_url, doc, source) -> ProviderDef:
...
def get_provider(name: str, *, allow_network: bool = True) -> Optional[ProviderDef]:
    """Look up a built-in provider by id or alias: models.dev catalog merged with the Moor overlay;
    moor-only overlay (moor, openai-codex, …); plugin provider profiles with a concrete endpoint."""
    canonical = normalize_provider(name)
    mdev_info = _models_dev_info(canonical, allow_network)
    overlay = MOOR_OVERLAYS.get(canonical)
    if mdev_info is not None:
        ov = overlay or MoorOverlay()
...
def moor_api_mode(model: str = "") -> str:
    """Wire protocol for a Moor Portal model. Portal serves its ``anthropic/*`` catalog on a native
    Messages route alongside OpenAI-compatible chat/completions for everything else.

    ``anthropic/*`` rides chat/completions by default for now (``moor.anthropic_wire``). Measured
    ...
    Empty/unknown model defaults to ``chat_completions`` (the historical Moor transport)."""
    if str(model or "").strip().lower().startswith("anthropic/"):
        # ``auto`` starts on chat too: it is safe on every upstream, and ``agent/moor_wire.py``
        # promotes the session to native from the first response when the upstream allows it.
        return "anthropic_messages" if _moor_anthropic_wire() == "native" else "chat_completions"
    return "chat_completions"


def _moor_anthropic_wire() -> str:
    """``moor.anthropic_wire``: ``"chat"`` (default), ``"native"``, or ``"auto"`` (chat, then per-session
    promotion decided from the first response; see ``agent/moor_wire.py``). Anything else reads as ``chat``."""
    try:
        from moor_cli.config import load_config_readonly
        value = str(((load_config_readonly().get("moor") or {}).get("anthropic_wire")) or "chat").strip().lower()
    except Exception:
        return "chat"
...
def determine_api_mode(provider: str, base_url: str = "", model: str = "") -> str:
    """API mode (wire protocol) for a provider/endpoint: host-mandated mode, then Moor dual-wire
    ...
    if mandated is not None:
        return mandated
    if (provider or "").strip().lower() in {"moor", "moor-portal", "nousresearch"}:
        return moor_api_mode(model)
...
def _lossy_alias_registry_pdef(raw: str, canonical: str) -> Optional[ProviderDef]:
    """Exact Moor registry ids win over LOSSY alias collapsing (kimi-coding-cn must stay distinct
    ...
    try:
        from moor_cli.auth import PROVIDER_REGISTRY as _AUTH_PROVIDER_REGISTRY
