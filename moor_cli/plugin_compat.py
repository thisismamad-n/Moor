...
The decomposition (PR #102117) moved most of Moor's internals into ``<stem>_<topic>`` sibling modules.
...
Surfaces that read from here: the CLI banner, ``moor plugins compat``, ``moor doctor``, the post-update
...
class MoorPluginCompatWarning(FutureWarning):
...
def report_file_path() -> Path:
    ...
    return get_moor_home() / REPORT_FILE
...
def disable_reason(manifest, *, today: Optional[_dt.date] = None) -> Optional[str]:
    ...
    if not hits:
        return None
    return (f"uses {len(hits)} import path(s) removed on {COMPAT_REMOVAL}; run `moor plugins compat` "
...
def summary_lines(report: Dict[str, List[Hit]], *, today: Optional[_dt.date] = None) -> List[str]:
    ...
    if removal_in_effect(today) and allow_deprecated_imports():
        ...
        tail = "Update the plugin(s); the old paths no longer exist. Details: moor plugins compat"
    elif removal_in_effect(today):
        head = (f"{n} plugin{'s' if n != 1 else ''} DISABLED: they import paths removed on {COMPAT_REMOVAL}: {names}")
        tail = f"Update the plugin(s) or set plugins.{ALLOW_KEY}: true to force-load. Details: moor plugins compat"
    else:
        ...
        tail = "Check for plugin updates or notify the author before then. Details: moor plugins compat"
    return [head, tail]
...
def warn_once(facade: str, name: str, target_module: str, target_name: str) -> None:
    """Per-name record that a moved name was resolved through its old path: a ``MoorPluginCompatWarning``
    ...
    msg = (f"moor plugin compat: `{facade}.{name}` moved to `{new}`. The old path is kept only for external "
           f"plugins and is removed on {COMPAT_REMOVAL}; update your import.")
    _log.warning(msg)
    warnings.warn(msg, MoorPluginCompatWarning, stacklevel=3)
...
def quiet_for_interactive() -> None:
    ...
    Appends (does not override) so an explicit ``-W error::...MoorPluginCompatWarning`` still wins."""
    if not any(a == "error" and c is not None and issubclass(MoorPluginCompatWarning, c)
               for a, _m, c, _mod, _l in warnings.filters):
        warnings.filterwarnings("ignore", category=MoorPluginCompatWarning, append=True)
