...
``cli.py``'s ``MoorCLI`` class. ``MoorCLI`` inherits ``CLICommandsMixin`` so
...
class CLICommandsMixin:
    ...
    def _handle_rollback_command(self, command: str):
        ...
        if not mgr.enabled:
            ...
            print("  Enable with: moor --checkpoints")
    ...
    def _handle_export_command(self, command: str):
        ...
        """
        from moor_cli.profiles import export_profile, get_active_profile_name
        ...
        try:
            ...
            print("  Share it: the other user runs /import or `moor profile import <archive>`.")
    ...
    def _handle_journey_command(self, cmd_original: str) -> None:
        """Handle /journey — the learning timeline (see `moor journey`).
    ...
    def _handle_image_command(self, cmd_original: str):
        ...
        if _remainder:
            ...
            _cprint(f"  {_DIM}Tip: type your next message, or run moor chat -q --image {_termux_example_image_path(image_path.name)} \"What do you see?\"{_RST}")
    ...
    def _handle_tools_command(self, cmd: str):
        ...
        def _run_capture(ns: Namespace) -> None:
            ...
            # Buffer reports isatty()=True so color() in moor_cli/colors.py
    ...
    def _handle_handoff_command(self, cmd_original: str) -> bool:
        ...
        try:
            ...
            pass
        _cprint("  Timed out waiting for the gateway. Is `moor gateway` running?")
    ...
    def _handle_resume_command(self, cmd_original: str) -> None:
        ...
        if not target:
            ...
            if self._show_recent_sessions(reason="resume"):
                ...
                return
            _cprint("  Tip:   Use /history or `moor sessions list` to find sessions.")
            return
        ...
        if not session_meta:
            ...
            _cprint("  Use /sessions or `moor sessions list` to see available sessions.")
            return
        ...
        # same directory as a startup `moor -c`/`--resume`. The startup resume
    ...
    def _handle_worktree_command(self, cmd_original: str) -> None:
        ...
        commits, same as ``moor -w``).

        ``prune`` is the same attended reclaim as ``moor worktree prune``
        (moor_cli/worktree_gc.py): never deletes tracked changes, unique
        ...
        if sub in {"new", "add", "create"}:
            ...
            # same way `moor -w` and session-resume cwd restore do.
            try:
                os.chdir(wt_info["path"])
            except OSError as e:
                print(f"  ⚠ Created worktree but could not enter it: {e}")
            os.environ["TERMINAL_CWD"] = wt_info["path"]
            # Register for the same keep-if-unpushed cleanup as `moor -w`.
    ...
    def _handle_pet_command(self, cmd: str):
        ...
        from moor_cli.pets import _set_active, _set_enabled, print_pet_gallery, set_pet_scale, toggle_pet_display
    ...
    def _handle_curator_command(self, cmd: str):
        ...
        Delegates to moor_cli.curator so the CLI and the `moor curator`
    ...
    def _handle_skills_command(self, cmd: str):
        """Handle /skills slash command — delegates to moor_cli.skills_hub."""
    ...
    def _handle_bundles_command(self, cmd: str) -> None:
        ...
        Mirrors ``moor bundles list`` but renders inside the running
        ...
        if not bundles:
            ...
            _cprint(
                f"  {_DIM}Create one with: moor bundles create "
        ...
        _cprint(
            ...
            f"Manage with `moor bundles`.{_RST}"
        )
    ...
    def _handle_heartbeat_command(self, cmd: str) -> None:
        ...
        cross-process schedules use `moor cron`.
        """
        ...
        _cprint(
            ...
            f"lives only while this Moor process runs — use `moor cron` for "
    ...
    def _handle_skin_command(self, cmd: str):
        ...
        if len(parts) < 2 or not parts[1].strip():
            ...
            print("\n  Usage: /skin <name>")
            print(f"  Custom skins: drop a YAML file in {display_moor_home()}/skins/\n")
            return
    ...
    def _handle_fast_command(self, cmd: str):
        ...
        try:
            from moor_cli.models import _is_anthropic_fast_model
            agent = getattr(self, "agent", None)
    ...
    def _handle_debug_command(self, cmd_original: str = ""):
        ...
        - ``/debug moor``   → upload to Moor-internal storage (private, staff-only)
        - ``/debug local``  → render the report to stdout, no upload

        ``moor`` and ``local`` are mutually exclusive; if both are given,
        ...
        moor = "moor" in words and not local
        ...
        args = SimpleNamespace(
            lines=200, expire=7, local=local, moor=moor, yes=True
        )
    ...
    def _handle_update_command(self) -> bool:
        ...
        ``moor update`` so the user sees update output directly and gets
        ...
        raw = self._prompt_text_input_modal(
            ...
            detail="This will exit the current session and run `moor update`.",
            choices=choices,
        )
        if raw is None:
