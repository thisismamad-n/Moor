...
def build_mcp_parser(subparsers, *, cmd_mcp: Callable) -> None:
    ...
    mcp_parser = subparsers.add_parser(
        "mcp", help="Manage MCP servers and run Moor as an MCP server",
        description="Manage MCP server connections and run Moor as an MCP server.\n\n"
    ...
    mcp_serve_p = mcp_sub.add_parser(
        "serve", help="Run Moor as an MCP server (expose conversations to other agents)")
    ...
    # Catalog (Moor-approved MCPs shipped with the repo)
    mcp_sub.add_parser(
        "picker", help="Interactive catalog picker (also the default for `moor mcp`)")
    mcp_sub.add_parser("catalog", help="List Moor-approved MCPs available for one-click install")
    mcp_install_p = mcp_sub.add_parser(
        "install", help="Install a catalog MCP by name (e.g. `moor mcp install deepwiki`)")
