"""``moor sync`` subcommand parser — Skill Sync.

Cloned from ``moor_cli/subcommands/cron.py`` — same injected-handler shape
...
Skill Sync covers two surfaces, both under this one command for launch:
  ...
  Personal — your own skills, across your own devices:
    moor sync status                 show gate/opt-in/head state
    moor sync pull                   pull and materialize opted-in skills
    moor sync push                   push opted-in skills
    moor sync now                    reconcile: pull then push
    moor sync enable <skill>         opt a skill into sync
    moor sync disable <skill>        opt a skill out of sync
    moor sync device [--name]        show or set this device's label

  Organisation — skills shared with your team:
    moor sync propose <skill>        share a skill with your organisation

Sync is INERT unless the resolved Moor token carries the access-gate claim
...
def build_sync_parser(subparsers, *, cmd_sync: Callable) -> None:
    ...
    sync_parser = subparsers.add_parser(
        ...
        epilog=(
            "Examples:\n"
            "  moor sync status            what is synced, and from where\n"
            "  moor sync enable my-skill   include a skill in your sync\n"
            "  moor sync now               pull, then push\n"
            "  moor sync propose my-skill  share a skill with your team\n"
        ),
