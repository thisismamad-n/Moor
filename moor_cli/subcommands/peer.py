...
A *peer* is another Moor gateway (any machine: homelab, Spark, Moor
...
bot on THIS machine a transport to message bots on THAT machine:
    ...
    moor peer dm spark/researcher "..."      # named profile (multiplexed peer)
...
``moor -p <bot> chat --in ~ -c "Bot Chat" ...`` bot-messaging command, so
...
- Peer labels/URLs live in config.yaml (``bot_peers``); the peer's
  API_SERVER_KEY is a credential and lives in ``~/.moor/.env`` as
  ``MOOR_PEER_<NAME>_KEY``.
...
def cmd_peer(args) -> int:
    ...
    if action == "dm":
        ...
        if not isinstance(peer, dict) or not peer.get("url"):
            print(f"No peer named '{peer_name}'. Run: moor peer list", file=sys.stderr)
            return 1
...
def build_peer_parser(subparsers) -> None:
    ...
    parser = subparsers.add_parser(
        "peer",
        help="Bot-to-bot DMs across machines (peer Moor gateways)",
        description=(
            "Register other Moor gateways as peers and message their agents. "
