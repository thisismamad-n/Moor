"""``moor peer`` — bot-to-bot DMs across machines/gateways.

A *peer* is another Moor gateway running the ``api_server`` platform; its stock
...
cross-machine twin of ``moor -p <bot> chat --in ~ -c "Bot Chat"``. ``run``/``status``
/``stop`` do the same turn through the async Runs API. Peer labels/URLs live in
config.yaml (``bot_peers``); the key lives in ``~/.moor/.env`` as
``MOOR_PEER_<NAME>_KEY``. ``<peer>/<profile>`` targets the ``/p/<profile>/`` mirror.
"""
...
def _peer_key_env(name: str) -> str:
    return f"MOOR_PEER_{name.upper().replace('-', '_')}_KEY"
...
def _request(
    ...
    request_headers = {
        ...
        "User-Agent": "moor-peer-dm"}
    if headers:
        request_headers.update(headers)
    req = urllib.request.Request(url, data=data, method=method, headers=request_headers)
    # The peer URL is user-registered (``moor peer add``); a redirect to a
    ...
    try:
        ...
        if exc.code == 400 and "title" in detail.lower():
            ...
            raise RuntimeError(
                ...
                f"(HTTP 400: {detail}). Update the peer's moor-agent, or unhide the "
                f"session there: PATCH /api/sessions/<id> {{\"hidden\": false}}.") from exc
        raise
    # Real api_server wraps the row: {"object": "moor.session", "session": {...}}.
...
def _parse_target(target: str) -> tuple[str, str | None]:
    ...
    if not peer:
        raise ValueError("Peer name required (moor peer dm <peer>[/<agent>] ...)")
...
def _resolve_peer_target(target: str) -> tuple[str, str | None, dict, str]:
    ...
    if not isinstance(peer, dict) or not peer.get("url"):
        raise LookupError(f"No peer named '{peer_name}'. Run: moor peer list")
    key = _peer_secret(peer_name)
    if not key:
        raise PermissionError(
            f"No API key for peer '{peer_name}'. Set it: moor peer add {peer_name} "
            f"--url <url> --key <key> (or add {_peer_key_env(peer_name)}=<key> to ~/.moor/.env)")
...
def _peer_add(args) -> int:
    ...
    if key:
        ...
        print(f"Peer '{name}' saved ({url}) — key stored as {_peer_key_env(name)} in ~/.moor/.env")
    else:
        print(
            f"Peer '{name}' saved ({url}). No key given — set the peer's API_SERVER_KEY with:\n"
            f"  moor peer add {name} --url {url} --key <key>\n"
            f"  (or add {_peer_key_env(name)}=<key> to ~/.moor/.env)")
    return 0
...
def _peer_list(args) -> int:
    peers = _load_peers()
    if not peers:
        print("No peers registered. Add one: moor peer add <name> --url http://host:port --key <API_SERVER_KEY>")
        return 0
...
def cmd_peer(args) -> int:
    ...
    if action not in {"dm", "run", "status", "stop"}:
        print("Unknown peer action. See: moor peer --help", file=sys.stderr)
        return 2
    try:
...
def build_peer_parser(subparsers) -> None:
    ...
    parser = subparsers.add_parser(
        "peer", help="Bot-to-bot DMs across machines (peer Moor gateways)",
        description="Register other Moor gateways as peers and message their agents. "
            "'moor peer dm <peer>[/<agent>] \"...\"' delivers into the remote "
            "agent's canonical Bot Chat over the peer's API server and prints "
            "the reply — the cross-machine twin of 'moor -p <bot> chat'. "
            "The peer must run the api_server platform; its API_SERVER_KEY is "
            "stored locally as a credential in ~/.moor/.env.",
        epilog=(
            "Examples:\n"
            "  moor peer add spark --url http://spark.lan:8377 --key <API_SERVER_KEY>\n"
            "  moor peer list\n"
            '  moor peer dm spark "Message from 🤖 dixie (@dixie): disk status?"\n'
            '  moor peer dm spark/researcher "..."   # named profile on a multiplexed peer\n'
            "  moor peer run spark --idempotency-key ticket-123 < long-task.txt\n"
            "  moor peer status spark run_abc123\n"
            "  moor peer stop spark run_abc123\n"
            "  moor peer remove spark\n"
            "\n"
    ...
    add_p.add_argument("--key", default="", help="The peer's API_SERVER_KEY (stored in ~/.moor/.env)")
    ...
    def _remote(name: str, help: str, *, run_id: bool):
        ...
        if run_id:
            sp.add_argument("run_id", help="Run ID returned by 'moor peer run'")
        else:
