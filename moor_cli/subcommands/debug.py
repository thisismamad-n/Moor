...
def build_debug_parser(subparsers, *, cmd_debug: Callable) -> None:
    ...
    debug_parser = subparsers.add_parser(
        ...
        description="Debug utilities for Moor Agent. Use 'moor debug share' to "
        "upload a debug report (system info + recent logs) to a paste "
        "service and get a shareable URL.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    moor debug share              Upload debug report (asks for confirmation)
    moor debug share --yes        Skip confirmation (for scripts/CI)
    moor debug share --lines 500  Include more log lines
    moor debug share --expire 30  Keep paste for 30 days
    moor debug share --local      Print report locally (no upload)
    moor debug share --no-redact  Disable upload-time secret redaction
    moor debug share --moor       Upload to Moor-internal storage (private)
    moor debug delete <url>       Delete a previously uploaded paste
""",
    )
    ...
    share_parser.add_argument(
        ...
        action="store_true",
        help=(
            "Upload the debug bundle to Moor-internal storage (AWS S3) instead "
            "of a public paste service. The bundle is private — viewable only "
            "by Moor staff (and allowlisted Discord mods) via a Google-login-"
    ...
    delete_parser = debug_sub.add_parser(
        "delete",
        help="Delete a paste uploaded by 'moor debug share'",
    )
