"""``moor dashboard`` / ``moor serve`` subcommand parsers.
...
``moor_cli/main.py:main()`` (god-file Phase 2); handler injected to avoid
importing ``main``.
"""
...
def _add_server_runtime_args(parser) -> None:
    ...
    # `moor dashboard` / `moor serve` cmdlines and SIGTERM them directly —
    # the same path `moor update` uses to clean up stale servers.
    parser.add_argument(
        "--stop",
        action="store_true",
        help="Stop all running Moor web server processes and exit",
    )
    parser.add_argument(
        "--status",
        action="store_true",
        help="List running Moor web server processes and exit",
    )
...
def build_dashboard_parser(
    ...
    # backend as `moor dashboard --no-open --tui --host ... --port ...`. The
    ...
    # browser. It exists so the Moor Desktop app (and headless remote
    ...
    serve_parser = subparsers.add_parser(
        "serve",
        help="Start the Moor backend server (headless; powers the desktop app and remote backends)",
        description=(
            "Run the Moor backend server — the JSON-RPC/WebSocket gateway the "
    ...
    # (cmd_dashboard exports MOOR_SERVE_HEADLESS=1). `dashboard` leaves it
    # unset and serves the browser UI as before.
    serve_parser.set_defaults(func=cmd_dashboard, no_open=True, headless_backend=True)

    # `moor dashboard register` — register a self-hosted dashboard OAuth
    # client with Moor Portal and write the client_id into ~/.moor/.env.
    # Nested subparser so bare `moor dashboard` keeps launching the server
    ...
    dashboard_register_parser = dashboard_subparsers.add_parser(
        "register",
        help="Register a self-hosted dashboard with Moor Portal (writes the OAuth client ID to .env)",
        description=(
            "Register this install as a self-hosted dashboard with your Moor "
            "Portal account. Creates an OAuth client, writes "
            "MOOR_DASHBOARD_OAUTH_CLIENT_ID into ~/.moor/.env, and prints "
            "how to engage the login gate. Requires being logged in (moor setup)."
        ),
    )
    ...
    dashboard_register_parser.add_argument(
        ...
        help=(
            ...
            "https://moor.example.com/auth/callback. Omit for localhost-only use."
        ),
    )
    dashboard_register_parser.add_argument(
        "--portal-url",
        ...
        default=None,
        help=(
            "Override the Moor Portal base URL for registration (default: the "
            "portal you logged into). The access token must be valid at this "
            "portal. Also settable via MOOR_DASHBOARD_PORTAL_URL. Mainly for "
