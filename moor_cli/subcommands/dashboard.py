...
def _add_server_runtime_args(parser) -> None:
    ...
    # manager / PID file: they scan the process table for `moor dashboard|serve`
    # cmdlines and SIGTERM them — the same path `moor update` uses.
    parser.add_argument(
        "--stop", action="store_true", help="Stop all running Moor web server processes and exit")
    parser.add_argument(
        "--status", action="store_true", help="List running Moor web server processes and exit")
...
def build_serve_parser(
    *, cmd_dashboard: Callable, add_help: bool = True, exit_on_error: bool = True,
    ...
    parser = argparse.ArgumentParser(
        prog="moor serve",
        description="Run the Moor backend server - the JSON-RPC/WebSocket gateway the "
...
def build_dashboard_parser(
    ...
    # Compat shim: desktop shells <= 0.15.x spawn `moor dashboard --no-open --tui ...`;
    ...
    # `register` is nested so bare `moor dashboard` keeps launching the server.
    ...
    dashboard_register_parser = dashboard_subparsers.add_parser(
        ...
        description="Register this install as a self-hosted dashboard with your Moor "
    ...
    dashboard_register_parser.add_argument(
        ...
        help="Override the Moor Portal base URL for registration (default: the "
