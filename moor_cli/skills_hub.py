...
# serves everything from "moor-index", so that entry MUST exceed the whole catalog or browse
...
_BROWSE_LIMITS = {
    "moor-index": 1000000, "official": 200, "skills-sh": 200, "well-known": 50,
    "github": 200, "clawhub": 500, "lobehub": 500, "browse-sh": 500}
# Programmatic (TUI gateway) browse keeps its own, lower caps.
_BROWSE_API_LIMITS = {
    "moor-index": 5000, "official": 100, "skills-sh": 100, "well-known": 25,
...
def _existing_categories() -> List[str]:
    """Sorted category buckets under ``~/.moor/skills/`` (children without their own SKILL.md)."""
...
def _prompt_for_category(c: Console, existing: List[str]) -> str:
    ...
    if existing:
        ...
        c.print("[bold]Category[/] "
                "[dim](optional — press Enter to install flat at ~/.moor/skills/<name>/)[/]")
...
def _render_browse_page(c: Console, deduped, page_items, page: int, total_pages: int,
    ...
    if official_count > 0 and page == 1:
        c.print(f"[bright_cyan]★ {official_count} official optional skill(s) from Moor inc.[/]")
    c.print()
...
def browse_skills(page: int = 1, page_size: int = 20, source: str = "all") -> dict:
    ...
    # moor-index AND the external APIs at once would double-count every skill.
...
def _resolve_url_bundle_name(c: Console, bundle, meta, identifier: str,
    ...
    if name_override and _is_valid_installed_skill_name(name_override):
        ...
        c.print(f"[bold red]Cannot install from URL:[/] {url}\n"
                ...
                f"  [bold]moor skills install {url} --name <your-name>[/]\n\n"
...
def _announce_blueprint(c: Console, skill_name: str) -> None:
    """Offer an installed skill's ``metadata.moor.blueprint`` via /suggestions — never
...
_INSTALL_PANELS = {
    "official": (
        "[bold bright_cyan]This is an official optional skill maintained by Moor inc..[/]\n\n"
        "It ships with moor-agent but is not activated by default.\n"
...
def _confirm_install(c: Console, bundle, category: str) -> bool:
    ...
    files_at = f"Files will be at: [cyan]{display_moor_home()}/skills/{category + '/' if category else ''}{bundle.name}/[/]"
...
def do_list(source_filter: str = "all", enabled_only: bool = False,
    ...
    profile's config — ``-p`` swaps MOOR_HOME at process start, so no profile flag here."""
    from tools.skills_hub import HubLockFile, ensure_hub_dirs
...
def do_update(name: Optional[str] = None, console: Optional[Console] = None,
    ...
    Mirrors the user-modified protection bundled skills already get from ``moor update`` (ported from
    ...
    if skipped_local:
        ...
        c.print("[dim]Overwrite with: moor skills update <name> --force[/]\n")
...
def do_list_modified(console: Optional[Console] = None, as_json: bool = False) -> None:
    """List bundled skills the user has edited (which `moor update` keeps)."""
    ...
    c.print(f"\n[bold]{len(modified)} user-modified bundled skill(s)[/] "
            "[dim](kept as-is by `moor update`):[/]")
    for entry in modified:
        c.print(f"  [yellow]~[/] {entry['name']}")
    c.print()
    c.print("[dim]See changes:   moor skills diff <name>[/]")
    c.print("[dim]Resume updates: moor skills reset <name>          (keep your copy, re-baseline)[/]")
    c.print("[dim]Revert to stock: moor skills reset <name> --restore[/]\n")
...
def do_diff(name: str, console: Optional[Console] = None) -> None:
    ...
    c.print()
    c.print(f"[dim]Revert with: moor skills reset {name} --restore[/]\n")
...
def do_tap(action: str, repo: str = "", console: Optional[Console] = None) -> None:
    ...
    if action == "list":
        ...
        if not repo:
            _print_error(c, f"Repo required. Usage: moor skills tap {action} owner/repo")
            return
    ...
    if target == "github":
        ...
        if not auth.is_authenticated():
            _print_error(c, "GitHub authentication required.\n"
                            f"Set GITHUB_TOKEN in {display_moor_home()}/.env "
...
def _github_publish(skill_path: Path, skill_name: str, target_repo: str, auth) -> tuple:
    ...
    try:
        resp = call("post", f"{target_repo}/pulls", json={
            ...
            "body": f"Submitting the `{skill_name}` skill via Moor Skills Hub.\n\n"
                    f"This skill was scanned by the Moor Skills Guard before submission.",
...
# `moor skills <action>` -> handler(args). Lambdas late-bind the do_* names so
# tests that patch("moor_cli.skills_hub.do_install") still intercept.
_CLI_ACTIONS = {
...
def skills_command(args) -> None:
    """Router for `moor skills <subcommand>` — called from moor_cli/main.py."""
    handler = _CLI_ACTIONS.get(getattr(args, "skills_action", None))
    if handler is None:
        _console.print("Usage: moor skills [browse|search|install|inspect|list|list-modified|diff|check|update|audit|uninstall|reset|opt-out|opt-in|publish|snapshot|tap]\n")
