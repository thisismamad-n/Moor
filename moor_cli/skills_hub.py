...
def _resolve_source_meta_and_bundle(identifier: str, sources):
    ...
    skill is how ``moor skills inspect owner/repo/skills/foo`` showed
...
def _existing_categories() -> List[str]:
    """Return sorted subdirectory names under ``~/.moor/skills/`` that look
...
def _prompt_for_category(c: Console, existing: List[str]) -> str:
    ...
    if existing:
        ...
        c.print(
            "[bold]Category[/] [dim](optional — press Enter to install flat at ~/.moor/skills/<name>/)[/]"
        )
    from moor_cli.cli_output import line_input
...
def do_browse(page: int = 1, page_size: int = 20, source: str = "all",
    ...
    # skips the external API sources and serves everything from "moor-index".
...
def do_install(identifier: str, category: str = "", force: bool = False,
    ...
    if not force and not skip_confirm:
        ...
        if bundle.source == "official":
            c.print(Panel(
                ...
                f"Files will be at: [cyan]{display_moor_home()}/skills/{category + '/' if category else ''}{bundle.name}/[/]",
    ...
    # metadata.moor.blueprint block, it is a runnable automation. Register it as
...
def browse_skills(page: int = 1, page_size: int = 20, source: str = "all") -> dict:
    ...
    page_size = max(1, min(page_size, 100))
    _TRUST_RANK = {"builtin": 3, "trusted": 2, "community": 1}
    # "moor-index" must carry a high limit: when the index is available the
    ...
    _PER_SOURCE_LIMIT = {"moor-index": 5000, "official": 100, "skills-sh": 100,
    ...
    # source-skip logic — querying moor-index AND the external APIs at once
...
def do_list(source_filter: str = "all",
    ...
    config — ``moor -p <profile> skills list`` reads that profile's
    ``skills.disabled`` list because ``-p`` swaps ``MOOR_HOME`` at process
...
def do_update(name: Optional[str] = None, console: Optional[Console] = None,
    ...
    already get from ``moor update`` (ported from
    ...
    if target == "github":
        ...
        if not auth.is_authenticated():
            c.print("[bold red]Error:[/] GitHub authentication required.\n"
                    f"Set GITHUB_TOKEN in {display_moor_home()}/.env or run 'gh auth login'.\n")
            return
