"""``moor kanban …`` — dispatch (``kanban_command``), task-verb handlers, ``run_slash`` for ``/kanban``.
...
from moor_cli.kanban_output import (
...
from moor_cli.kanban_boards import _dispatch_boards
from moor_cli.kanban_ops import (
    _cmd_daemon, _kanban_config, _cmd_dispatch, _cmd_gc, _cmd_repair, _cmd_tail, _cmd_watch,
)
from moor_cli.kanban_parser import build_parser  # noqa: F401  (re-exported: moor_cli.main, run_slash)
...
def _check_dispatcher_presence(moor_home: Optional[Path] = None) -> tuple[bool, str]:
    """``(running, message)`` for the "will anything dispatch this?" warning: True when a gateway is
    alive for this MOOR_HOME with ``kanban.dispatch_in_gateway`` on, else False + human guidance.
    Fails OPEN (probe/config errors -> ``(True, "")``) — a missed warning beats crying wolf.
    ``moor_home`` scopes the probe to a profile dir (dashboard backend); CLI callers pass None.

    The dashboard plugin API passes it because the dashboard backend process can be running under a
    different MOOR_HOME than the profile the request targets, which otherwise produced a "no gateway is
    ...
    try:
        ...
        liveness = resolve_gateway_liveness(profile_dir=moor_home, use_cache=False)
    except Exception:
...
def kanban_command(args: argparse.Namespace) -> int:
    """Entry point from ``moor kanban …``; returns a shell-style exit code."""
    ...
    if not action:
        ...
        if parser is not None:
            ...
            print("usage: moor kanban <action> [options]\n"
                  "Run 'moor kanban --help' for the full list of actions.", file=sys.stderr)
        return 0
    ...
    # `--board <slug>` pins MOOR_KANBAN_BOARD for the duration of this call so it inherits the
    ...
    with board_scope:
        ...
        # "no such table: tasks" on first use from a fresh MOOR_HOME.
        try:
            kb.init_db()
...
def _cmd_init(args: argparse.Namespace) -> int:
    ...
    if profiles:
        ...
        print("No profiles found under ~/.moor/profiles/.\n"
              "Create one with `moor -p <name> setup` before assigning tasks.")
    print(
