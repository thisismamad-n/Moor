...
Pure logic: walks a Moor config dict, returns issues for any reference
to a retired xAI model. No I/O, no CLI dependencies — testable in isolation
and reusable from both `moor doctor` and a future `moor migrate xai`.
"""
...
class RetirementIssue:
    """A reference to a retired xAI model found in a Moor config."""
...
def find_retired_xai_refs(config: Dict[str, Any]) -> List[RetirementIssue]:
    """Walk all model slots in a Moor config and return retirement issues.
...
def apply_migration(
    ...
    # None``, so the next `moor migrate xai` reports nothing to migrate
