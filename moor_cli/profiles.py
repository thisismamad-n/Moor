"""
Profile management for multiple isolated Moor instances.

Each profile is a fully independent MOOR_HOME directory with its own
config.yaml, .env, memory, sessions, skills, gateway, cron, and logs.
Profiles live under ``~/.moor/profiles/<name>/`` by default.

The "default" profile is ``~/.moor`` itself — backward compatible,
zero migration needed.

Usage::

    moor profile create coder          # fresh profile + bundled skills
    moor profile create coder --clone  # also copy config, .env, SOUL.md, skills
    moor profile create coder --clone-all  # full copy of source profile
    coder chat                           # use via wrapper alias
    moor -p coder chat                 # or via flag
    moor profile use coder             # set as sticky default
    moor profile delete coder          # remove profile + alias + service
"""
...
_PROFILE_DIRS = [
    ...
    # See moor_constants.get_subprocess_home().
    "home",
]
...
# Marker file written by `moor profile create --no-skills`.  When present in
# a profile's root, callers of seed_profile_skills() (fresh-create, `moor
...
# `moor skills install` or drop SKILL.md files into the profile's skills/.
...
def _clone_all_copytree_ignore(source_dir: Path):
    ...
    Three categories:
      ...
      2. Root-level entries in ``_CLONE_ALL_DEFAULT_EXCLUDE_ROOT`` — known
         Moor infrastructure directories that only the default profile
         (``~/.moor``) ever contains.  Gated on ``source_dir`` actually
    ...
    is_default_source = source_resolved == _get_default_moor_home().resolve()
...
_RESERVED_NAMES = frozenset({
    "moor", "default", "test", "tmp", "root", "sudo",
})

# Moor subcommands that cannot be used as profile names/aliases
_MOOR_SUBCOMMANDS = frozenset({
...
def _get_profiles_root() -> Path:
    ...
    Anchored to the moor root, NOT to the current MOOR_HOME
    (which may itself be a profile).  This ensures ``coder profile list``
    can see all profiles.

    In Docker/custom deployments where MOOR_HOME points outside
    ``~/.moor``, profiles live under ``MOOR_HOME/profiles/`` so
    they persist on the mounted volume.
    """
    return _get_default_moor_home() / "profiles"


def _get_default_moor_home() -> Path:
    """Return the default (pre-profile) MOOR_HOME path.

    In standard deployments this is ``~/.moor``.
    In Docker/custom deployments where MOOR_HOME is outside ``~/.moor``
    (e.g. ``/opt/data``), returns MOOR_HOME directly.
    """
...
def _get_active_profile_path() -> Path:
    ...
    return _get_default_moor_home() / "active_profile"
...
def validate_profile_name(name: str) -> None:
    ...
    if name in _RESERVED_NAMES:
        raise ValueError(
            ...
            f"the Moor installation itself or a common system binary.  "
...
def profile_matches_home(name: str, home: "Path | None" = None) -> bool:
    ...
    ``home`` defaults to the process's current Moor home
    (:func:`moor_constants.get_moor_home`).  Used by single-profile
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    Checks: alias-name validity, reserved names, moor subcommands, existing
    binaries in PATH.
    """
    canon = normalize_profile_name(name)
    try:
    ...
    if canon in _MOOR_SUBCOMMANDS:
        return f"'{canon}' conflicts with a moor subcommand"
...
def create_wrapper_script(name: str, target: Optional[str] = None) -> Optional[Path]:
    ...
    if is_windows:
        ...
        try:
            moor_exe = shutil.which("moor") or "moor"
            wrapper_path.write_text(f'#!/bin/sh\nexec {shlex.quote(moor_exe)} -p {profile} "$@"\n', encoding="utf-8")
...
def _migrate_profile_config_if_outdated(profile_dir: Path) -> None:
    ...
    ``_config_version``) or that is simply older than the running Moor. If we
    ...
    try:
        ...
        # not be migrated. The next `moor doctor --fix` can still surface the
...
def find_alias_for_profile(profile_name: str) -> Optional[str]:
    ...
    alias whose body invokes ``moor -p <profile>``. When the alias name equals
    the profile name this is trivial, but a custom alias (``moor profile alias
...
# Real wrappers are a few hundred bytes of shell; the needle (``moor -p X``)
...
def _read_distribution_meta(profile_dir: Path) -> tuple:
    ...
    never break ``moor profile list`` for an unrelated profile.
    """
...
def _check_gateway_running(profile_dir: Path) -> bool:
    ...
    *this* reader — which is exactly the case for a dashboard process that is
    ...
    agree.  Parameterized by ``profile_dir`` so it never mutates ``MOOR_HOME``.
    """
    try:
...
# ``config.yaml``. ``config.yaml`` is the user-facing Moor config
...
def profiles_to_serve(
    ...
    """Return the ``(profile_name, moor_home)`` pairs a gateway should serve.
    ...
    - ``multiplex=True``: returns the default profile plus every valid named
      profile under ``profiles/``, each paired with its own MOOR_HOME. When
    ...
    The returned ``moor_home`` is the path to pass to
    ``set_moor_home_override`` when scoping a turn to that profile.
    """
...
def create_profile(
    ...
    no_skills:
        ...
        a marker file so ``moor update`` skips re-seeding this profile's
    ...
    if canon == "default":
        raise ValueError(
            "Cannot create a profile named 'default' — it is the built-in profile (~/.moor)."
        )
    ...
    if clone_all and source_dir:
        # Full copy of source profile (exclude sibling ~/.moor/profiles/)
        shutil.copytree(
            source_dir,
            profile_dir,
            symlinks=True,
    ...
    # Keys pages, `moor -p <name> auth add`) had no file until first
    ...
    if not env_path.exists():
        try:
            env_path.write_text(
                "# Per-profile secrets for this Moor profile.\n"
    ...
    if not soul_path.exists():
        try:
            from moor_cli.default_soul import DEFAULT_SOUL_MD
            soul_path.write_text(DEFAULT_SOUL_MD, encoding="utf-8")
        except Exception:
            pass  # best-effort — don't fail profile creation over this

    # Write the opt-out marker so seed_profile_skills() and `moor update`'s
    ...
    # Cloned configs can be older than the running Moor (or predate schema
    ...
    if description and description.strip():
        try:
            ...
            pass  # non-fatal — user can describe later with `moor profile describe`

    # Phase 4: when running inside a container under s6, register the
    # new profile's gateway as a runtime s6 service so
    # `moor -p <profile> gateway start` can supervise it via
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Uses subprocess because sync_skills() caches MOOR_HOME at module level.
    Returns the sync result dict, or None on failure.

    Profiles that opted out of bundled skills (via ``moor profile create
    ...
    only the essential skills (e.g. ``moor-agent``), reporting
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "MOOR_HOME": str(profile_dir)},
...
def backfill_profile_envs(quiet: bool = False) -> List[str]:
    ...
    default_env = _get_default_moor_home() / ".env"
...
def _profile_bound_backend_pids(canon: str, profile_dir: Path) -> list[int]:
    """PIDs of running Moor *backends* bound to this profile.
    ...
    names it, so find it by inspection: a Moor backend subcommand
    (``serve``/``dashboard``/``gateway``) that is bound to *this* profile either
    by a ``--profile <canon>`` / ``-p <canon>`` selector or by a ``MOOR_HOME``
    ...
    # Never terminate ourselves or a parent (e.g. `moor -p <canon> profile
    ...
    moor_markers = ("moor_cli.main", "moor-gateway", "tui_gateway")
    ...
    # something (e.g. Electron's `findOnPath('moor')` resolution) spawns the
    ...
    # interpreter, not "moor", so the checks below would otherwise miss it.
    ...
    # interpreter.
    _MOOR_CONSOLE_SCRIPT_NAMES = frozenset({"moor", "moor-agent", "moor-acp"})
    pids: list[int] = []
    ...
    for proc in psutil.process_iter(["pid", "name", "username", "cmdline"]):
        try:
            ...
            # resolved executable named `moor`, or a python interpreter
            # directly exec'ing a `moor`-named console-script shim (argv[0]
            # is the interpreter, argv[1] is the shim's path).
            joined = " ".join(argv)
            exe_name = os.path.basename(argv[0]).lower()
            is_moor = (
                any(marker in joined for marker in moor_markers)
                or exe_name == "moor"
                or exe_name.startswith("moor")
            )
            if not is_moor and len(argv) >= 2 and _python_interpreter_re.match(exe_name):
                # Match against the actual known console-script entry points
                # (pyproject.toml [project.scripts]: moor, moor-agent,
                # moor-acp) rather than a bare `startswith("moor")` --
                ...
                # "moor-notes.py" or "moor-unrelated-tool" as the shim,
                ...
                is_moor = script_stem in _MOOR_CONSOLE_SCRIPT_NAMES
            if not is_moor:
                continue
            ...
            # ...or by MOOR_HOME env pointing at this profile dir.
            if not bound:
                try:
                    env_home = (proc.environ() or {}).get("MOOR_HOME", "")
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if canon == "default":
        raise ValueError(
            "Cannot delete the default profile (~/.moor).\n"
            "To remove everything, use: moor uninstall"
        )
    ...
    # the rmtree below fail with ENOTEMPTY and — before the ensure_moor_home
...
def _maybe_register_gateway_service(profile_name: str) -> None:
    ...
    ``MOOR_HOME`` and binds the port resolved by ``gateway/config.py``
    ...
    try:
        ...
        from moor_cli.service_manager import get_service_manager
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # Temporarily set MOOR_HOME so _profile_suffix resolves correctly
    old_home = os.environ.get("MOOR_HOME")
    try:
        os.environ["MOOR_HOME"] = str(profile_dir)
        from moor_cli.gateway import get_service_name, get_launchd_plist_path
        ...
        elif "MOOR_HOME" in os.environ:
            del os.environ["MOOR_HOME"]
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``~/.moor/active_profile``. Use ``"default"`` to clear.
    """
    canon = normalize_profile_name(name)
    validate_profile_name(canon)
    if canon != "default" and not profile_exists(canon):
        raise FileNotFoundError(
            ...
            f"Create it with: moor profile create {canon}"
        )
...
def get_active_profile_name() -> str:
    """Infer the current profile name from MOOR_HOME.

    Returns ``"default"`` if MOOR_HOME is not set or points to ``~/.moor``.
    Returns the profile name if MOOR_HOME points into ``~/.moor/profiles/<name>``.
    Returns ``"custom"`` if MOOR_HOME is set to an unrecognized path.
    """
    from moor_constants import get_moor_home
    moor_home = get_moor_home()
    resolved = moor_home.resolve()

    default_resolved = _get_default_moor_home().resolve()
...
def _default_export_ignore(root_dir: Path):
    ...
    * **Root-level allow-list** — only entries whose name appears in
      ...
      MOOR_HOME equals the cwd) is excluded. Blacklisting was tried
      first and proved unable to anticipate every non-Moor file the
      user may have lying alongside MOOR_HOME (#58394).
...
def _scrub_export_secrets(staged: Path) -> None:
    ...
    ``moor sessions export --redact``. Runs on the *staged copy only* so the
    live profile is never rewritten. ``force=True`` ignores
    ``security.redact_secrets`` / ``MOOR_REDACT_SECRETS`` — share archives
...
def export_profile(name: str, output_path: str, extra_files: Optional[Dict[str, str]] = None) -> Path:
    ...
    if canon == "default":
        # The default profile IS ~/.moor itself — its parent is ~/ and its
        # directory name is ".moor", not "default".  We stage a clean copy
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    if not inferred_name:
        raise ValueError(
            ...
            "Specify it explicitly: moor profile import <archive> --name <name>"
        )
    ...
    # dir.  Importing as "default" would target ~/.moor itself — disallow
    # that and guide the user toward a named profile.
    canon = normalize_profile_name(inferred_name)
    validate_profile_name(canon)
    if canon == "default":
        raise ValueError(
            "Cannot import as 'default' — that is the built-in root profile (~/.moor). "
            "Specify a different name: moor profile import <archive> --name <name>"
        )
    ...
    with tempfile.TemporaryDirectory(prefix="moor_profile_import_") as tmpdir:
...
def _migrate_honcho_profile_host(old_name: str, new_name: str, new_dir: Path) -> None:
    ...
    old_host = f"moor_{old_name}"
    legacy_old_host = f"moor.{old_name}"
    new_host = f"moor_{new_name}"

    candidates = [
        new_dir / "honcho.json",
        _get_default_moor_home() / "honcho.json",
    ...
    for path in candidates:
        ...
        if isinstance(block, dict) and "aiPeer" not in block:
            if source_host.startswith("moor_"):
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a MOOR_HOME path string.

    Called early in the CLI entry point, before any moor modules
    are imported, to set the MOOR_HOME environment variable.

    When MOOR_HOME is already set, the configured spelling IS the
    ...
    canon = normalize_profile_name(profile_name)
    validate_profile_name(canon)
    env_home = os.environ.get("MOOR_HOME", "").strip()
    if env_home:
        env_path = Path(env_home)
        # A profile-shaped env value means the root is the grandparent
        # (mirrors get_default_moor_root()).
