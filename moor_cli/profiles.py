...
from moor_constants import clear_named_profile_deleted, mark_named_profile_deleted, named_profile_is_deleted
...
# stay visible; containers persist HOME state here). See moor_constants.get_subprocess_home().
...
# (+wal/shm, can reach many GB), session dirs, `moor backup` archives, quick-backup
...
# Marker written by `moor profile create --no-skills`. When present at a profile root,
# seed_profile_skills() callers (fresh-create, `moor update` all-profile sync, the
...
def _clone_all_copytree_ignore(source_dir: Path):
    ...
    if source_resolved == _get_default_moor_home().resolve():
...
# Allow-list for ``export_profile("default")``: when MOOR_HOME equals the cwd
# (Docker/custom deployments) the default home holds arbitrary user files that must NOT
# be bundled. Only known Moor profile artifacts at the root survive; sensitive runtime
...
# when introduced in ``moor_constants``.
# See #58394.
...
_RESERVED_NAMES = frozenset({"moor", "default", "test", "tmp", "root", "sudo"})
...
def _get_profiles_root() -> Path:
    """Named-profiles root, anchored to the moor root (NOT the current MOOR_HOME, which
...
def _get_default_moor_home() -> Path:
    """Default (pre-profile) MOOR_HOME: ``~/.moor``, or MOOR_HOME itself in
...
def _is_our_wrapper(path: Path) -> bool:
    ...
    try:
        return "moor -p" in path.read_text(encoding="utf-8")
    except Exception:
        return False


def _missing_profile_error(canon: str) -> FileNotFoundError:
    return FileNotFoundError(f"Profile '{canon}' does not exist. Create it with: moor profile create {canon}")
...
def _migrate_profile_config_if_outdated(profile_dir: Path) -> None:
    ...
    if not (profile_dir / "config.yaml").exists():
        return
    # Creation must not fail over an unmigratable old config; `moor doctor --fix` surfaces
...
# wrappers are a few hundred bytes with the ``moor -p X`` needle near the top; the wrapper
...
def _check_gateway_running(profile_dir: Path) -> bool:
    """Gateway liveness for a profile dir, never mutating MOOR_HOME.
...
# Deliberately tiny and separate from ``config.yaml`` (user-facing Moor config, ~5000
...
def _clone_all_into(source_dir: Path, profile_dir: Path, canon: str) -> None:
    ...
    from moor_cli.auth import strip_cloned_single_use_oauth_grants
...
def create_profile(
    ...
    canon = _canon_valid(name)
    if canon == "default":
        raise ValueError("Cannot create a profile named 'default' — it is the built-in profile (~/.moor).")
    profile_dir = get_profile_dir(canon)
    ...
    # profile-scoped env writes (dashboard Channels/Keys pages, `moor -p <name> auth add`)
    ...
    # Opt-out marker read by seed_profile_skills() and `moor update`'s all-profile sync
    # (the feature still works via the empty skills/ dir if this fails).
    if no_skills:
        _seed_file_if_missing(
            profile_dir / NO_BUNDLED_SKILLS_MARKER,
            "This profile opted out of bundled-skill seeding (`moor profile create --no-skills`).\n"
    ...
    if description and description.strip():
        with contextlib.suppress(Exception):  # non-fatal — `moor profile describe` works later
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    """Seed bundled skills into a profile via subprocess (sync_skills() caches MOOR_HOME at
...
_MOOR_ARGV_MARKERS = ("moor_cli.main", "moor-gateway", "tui_gateway")
...
# `findOnPath('moor')`) spawns the shim by handing the interpreter its path — then the
# OS-reported argv[0] is the interpreter, not "moor".
...
# argv[1] is matched against exact names, not ``startswith("moor")``: with a bare
# interpreter argv[0], argv[1] can be ANY user script ("moor-notes.py").
_MOOR_CONSOLE_SCRIPT_NAMES = frozenset({"moor", "moor-agent", "moor-acp"})


def _is_moor_argv(argv: list) -> bool:
    """True for a Moor process: entrypoint marker in argv, executable named ``moor*``,
    or a python interpreter directly exec'ing a known ``moor`` console-script shim."""
    joined = " ".join(argv)
    exe_name = os.path.basename(argv[0]).lower()
    if any(marker in joined for marker in _MOOR_ARGV_MARKERS) or exe_name.startswith("moor"):
        return True
    if len(argv) >= 2 and _PYTHON_INTERPRETER_RE.match(exe_name):
        script_name = os.path.basename(str(argv[1])).lower()
        return script_name.rsplit(".", 1)[0] in _MOOR_CONSOLE_SCRIPT_NAMES
    return False
...
def _profile_bound_backend_pids(canon: str, profile_dir: Path) -> list[int]:
    """PIDs of running Moor *backends* bound to this profile (``gateway.pid`` only tracks
    ...
    # Never terminate ourselves or a parent (`moor -p <canon> profile delete` runs under
    ...
    for proc in psutil.process_iter(["pid", "name", "username", "cmdline"]):
        try:
            ...
            if not argv or not _is_moor_argv(argv):
                continue
            if not ({tok.lower() for tok in argv} & _BACKEND_TOKENS):
                continue

            # Bound to THIS profile by selector flag, or by MOOR_HOME pointing at its dir.
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    canon = normalize_profile_name(name)
    if canon == "default":
        raise ValueError("Cannot delete the default profile (~/.moor).\nTo remove everything, use: moor uninstall")
...
def _maybe_register_gateway_service(profile_name: str) -> None:
    ...
    Port selection: each supervised profile gateway loads its own ``MOOR_HOME`` and binds the port
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # MOOR_HOME is set temporarily so _profile_suffix resolves the service name.
...
def _stop_gateway_process(profile_dir: Path) -> None:
    ...
    try:
        ...
        pid = int(data["pid"])
        # Cross-profile kill refusal: the record's moor_home stamp names the gateway's TRUE
        ...
        if recorded_gateway_home_conflicts(data, expected_home=profile_dir):
            print(
                f"✗ Refusing to stop PID {pid}: its recorded MOOR_HOME "
...
def get_active_profile_name() -> str:
    """Profile name inferred from MOOR_HOME: ``"default"`` when unset or ``~/.moor``, the
    name under ``~/.moor/profiles/<name>``, ``"custom"`` for any other path."""
    from moor_constants import get_moor_home
    resolved = get_moor_home().resolve()
    if resolved == _get_default_moor_home().resolve():
        return "default"
...
def _inside_git_checkout(path: Path) -> bool:
    ...
    (not cwd) so the check holds when MOOR_HOME sits in a checkout but the process runs
...
def _profile_export_directory() -> Path:
    ...
    import tempfile
    export_dir = _get_default_moor_home() / "profile-exports"
    if not _inside_git_checkout(export_dir):
        return export_dir

    # A custom deployment may point MOOR_HOME at its source checkout: use a sibling store,
    # falling back to the OS temp dir only when the user's home itself is a checkout (dotfiles
    # repo). Per-uid temp name: a fixed /tmp/moor-profile-exports is a predictable shared
    # path another local user could pre-create (or symlink) first.
    uid_suffix = f"-{os.getuid()}" if hasattr(os, "getuid") else ""
    candidates = (
        Path.home() / ".moor-profile-exports", Path(tempfile.gettempdir()) / f"moor-profile-exports{uid_suffix}"
    )
...
def _scrub_export_secrets(staged: Path) -> None:
    """Force-redact secret-shaped strings in a staged export tree (same pass as ``moor
...
def export_profile(name: str, output_path: str, extra_files: Optional[Dict[str, str]] = None) -> Path:
    ...
    # The default profile IS ~/.moor (dir name ".moor"), so both paths stage a filtered
...
def _migrate_honcho_profile_host(old_name: str, new_name: str, new_dir: Path) -> None:
    ...
    candidates = [
        new_dir / "honcho.json", _get_default_moor_home() / "honcho.json", Path.home() / ".honcho" / "config.json"
    ]
    seen: set[Path] = set()
    for path in candidates:
        ...
        if isinstance(block, dict) and "aiPeer" not in block:
            block["aiPeer"] = old_name  # source_host is ``moor_<old>`` or legacy ``moor.<old>``
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a MOOR_HOME path string. Called early in the CLI entry
    point, before moor modules are imported, to set MOOR_HOME.

    When MOOR_HOME is already set, the configured spelling IS the launch root (it may be a
    junction/symlink alias of the platform default). Keep that spelling so profile re-home does not destroy
    the launcher's lexical provenance -- the subprocess sanitizer needs it to match moor-owned PYTHONPATH
