...
Split out of ``moor_cli.kanban_db``; origin-resident helpers are reached
...
class DispatchResult:
    ...
    Code terminal like ``orion-cc``), not a Moor profile. Expected steady-state
    ...
    if sweep.exited_hook_payloads and _kb._kanban_observer_consumed("on_kanban_worker_exited"):
        ...
        for hook_fields in sweep.exited_hook_payloads:
            ...
            _kb._fire_kanban_lifecycle_hook(
                ...
                # fire in the DISPATCHER process (gateway-embedded dispatcher or ``moor kanban
...
def _profile_exists_fn() -> Optional[Callable[[str], bool]]:
    """``moor_cli.profiles.profile_exists``, or ``None`` when it cannot be
...
def has_spawnable_ready(conn: sqlite3.Connection) -> bool:
    """True iff a ready+assigned+unclaimed task maps to a real Moor profile.
...
def review_dispatch_enabled() -> bool:
    """Whether review tasks dispatch automatically. Default true (Moor ships
...
def count_running_tasks_other_boards(board: Optional[str] = None) -> int:
    ...
    Boards are matched by resolved DB path, so ``MOOR_KANBAN_DB`` (pins every
...
def _dispatch_lane_task(
    ...
    *,
    ...
    # would fail ``moor -p <assignee>`` at startup and loop ready→crash→ready
    ...
    if guard_reason is not None:
        ...
        # Event so ``moor kanban tail`` shows why the task looks stuck.
...
def _module_moor_argv() -> list[str]:
    """Interpreter-bound Moor CLI invocation (``moor_cli.main`` is the
    console-script target — there is no top-level ``moor`` package)."""
...
def _moor_path_argv(path: str) -> list[str]:
    """argv for a resolved Moor executable path. Windows batch shims
...
def _resolve_moor_argv() -> list[str]:
    ...
    same-directory file), then ``which("moor")`` (Windows: safe PATH search,
    batch shims fall back to the module form), then ``sys.executable -m
    moor_cli.main`` for shim-less environments (cron, systemd ``User=``,
    launchd). Mirrors ``gateway.run._resolve_moor_bin``; local because
    ``moor_cli`` sits below ``gateway`` in the dependency order.
    """
    import shutil
...
def _resolve_worker_cli_toolsets(moor_home: Optional[str]) -> Optional[list[str]]:
    ...
    tools when ``MOOR_KANBAN_TASK`` is set.
    """
    if not moor_home:
        return None
    try:
...
def _worker_argv(task: Task, profile_arg: str, moor_home: Optional[str]) -> list[str]:
    """Build the ``moor -p <profile> --cli ... chat -q ...`` worker command."""
    cmd = [
        *_resolve_moor_argv(),
        "-p", profile_arg,
        ...
        "--cli",
        # Workers run under a profile-scoped MOOR_HOME and so see that
    ...
    worker_toolsets = _resolve_worker_cli_toolsets(moor_home)
    if worker_toolsets:
...
def _open_worker_log(task: Task, board: Optional[str]):
    ...
    `moor kanban log` reads its own file and boards sharing task ids don't
    collide."""
...
def _default_spawn(task: Task, workspace: str, *, board: Optional[str] = None) -> Optional[int]:
    ...
    ``MOOR_KANBAN_DB`` / ``MOOR_KANBAN_BOARD`` / workspaces_root to the
    board the task was claimed from, so workers cannot see other boards.
    """
    if not task.assignee:
        raise ValueError(f"task {task.id} has no assignee")

    from moor_cli.profiles import normalize_profile_name, resolve_profile_env
    ...
    # match after `moor -p` rewrites MOOR_HOME (symlink / Docker layouts).
    env["MOOR_KANBAN_DB"] = str(_kb.kanban_db_path(board=board))
    env["MOOR_KANBAN_WORKSPACES_ROOT"] = str(_kb.workspaces_root(board=board))
...
def run_daemon(
    *,
    ...
    the gateway dispatcher and ``moor kanban dispatch`` — the standalone
...
from moor_cli import kanban_db as _kb  # noqa: E402
from moor_cli import kanban_db_connect as _kbc  # noqa: E402
from moor_cli import kanban_db_workspace as _kbw  # noqa: E402
