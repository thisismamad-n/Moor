...
Split out of :mod:`moor_cli.plugins`. Names that tests patch on the origin
...
def discover_entrypoint_manifests() -> List["PluginManifest"]:
    ...
    ``moor_agent.plugin_capabilities`` group (``<plugin-id>.<capability-id>`` entries pointing at the same
...
def collect_directory_manifests() -> List[PluginManifest]:
    ...
    from moor_cli import plugins as _origin  # patched names resolve through the origin
    manifests: List[PluginManifest] = []
    ...
    if _origin._env_enabled("MOOR_ENABLE_PROJECT_PLUGINS"):
        ...
        logger.debug("Project plugins disabled (set MOOR_ENABLE_PROJECT_PLUGINS=1 to enable)")
    return manifests
...
def gate_manifest(
    ...
    if names & LEGACY_RELAY_PLUGIN_KEYS:
        ...
        return _placeholder(error, logging.WARNING, "Refusing to load removed Moor Relay plugin '%s'; %s", error)
    if names & disabled:
    ...
    if manifest.source == "bundled":
        ...
        # Bundled platforms register LAZILY: eagerly importing ~20 heavy SDKs added seconds to every `moor`
    ...
    if enabled is None or not names & enabled:
        return _placeholder(
            f"not enabled in config (run `moor plugins enable {lookup_key}` to activate)", logging.DEBUG,
