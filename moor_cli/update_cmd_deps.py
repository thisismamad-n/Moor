"""Post-``moor update`` dependency sync: venv preflight, editable reinstall, lazy refresh,
npm/Desktop rebuilds, self-lock deferral. Names are re-imported by ``update_cmd`` (so
``moor_cli.update_cmd.<name>`` resolves/monkeypatches); origin helpers are imported lazily."""
...
def _editable_install_is_current(git_cmd, cwd, pre_pull_sha: str | None) -> bool:
    ...
    -e .`` always rewrites console-script shims (Windows: ``moor.exe`` quarantine, ``os error 32``
...
def _critical_module_import_failures(
    ...
    """
    from moor_cli.update_cmd import _UPDATE_CRITICAL_MODULES, _m
...
def _restore_active_tool_dependencies(
    ...
    """Restore allowlisted ``moor tools`` dependencies (from a pre-rebuild probe) into a rebuilt
...
def _refresh_active_memory_provider_dependencies() -> None:
    ...
    ``plugin.yaml`` (not Moor extras / ``LAZY_DEPS``), so the core reinstall can strip them;
...
def _npm_lockfile_changed(moor_root: Path) -> bool:
    ...
    if (web_dir / "package.json").is_file() and not _web_build_toolchain_ready(
        *_web_toolchain_roots(web_dir)):
        return True
    try:
        cache_file = _npm_lock_cache_file(moor_root)
...
def _npm_lock_cache_file(moor_root: Path) -> Path:
...
def _record_npm_lockfile_hash(moor_root: Path) -> None:
    ...
    if digest is None:
        return
    try:
        _npm_lock_cache_file(moor_root).write_text(digest, encoding="utf-8")
    except OSError:
...
def _repair_node_deps_on_current_checkout(
    ...
    *,
    ...
    """Repair Node deps on the ``commit_count == 0`` path: a failed npm install says "re-run moor
...
def _update_node_dependencies() -> list[str]:
    ...
    # Runs before the lockfile-unchanged early return below since that's the common `moor update` case.
    # See #43564.
    ...
    # The chatty npm-deprecation noise during `moor update` comes from the *desktop* build, not this step;
...
def _venv_core_imports_healthy() -> tuple[bool, str]:
    ...
    if not venv_python.exists():
        ...
        managed_markers = (_m().PROJECT_ROOT / ".moor-bootstrap-complete", _m()._update_marker_path())
...
# canonical case: ``moor_cli.main`` used to import it at startup while resolving external secret sources;
...
def _abort_dependency_sync_if_self_locked(gateway_resume=None) -> None:
    ...
    a mapped ``.pyd`` -> exit 2, next launch's marker recovery finishes; the ``moor.exe`` shim
...
def _rebuild_desktop_after_update(
    ...
    # rebuild window), then surface the tail. Put moor-managed Node on PATH: the desktop
...
def _venv_foreign_owned_paths(venv_root, limit: int = 5) -> list:
    ...
    A venv touched by ``sudo pip``/``sudo moor`` dies mid-update with ``venv/bin/moor`` already
    ...
    See #83529.
    A later normal ``moor update`` then dies mid-mutation inside ``uv pip install -e .`` ("Permission
    denied (os error 13)") with ``venv/bin/moor`` already deleted — the CLI is bricked. Same philosophy as
    the contended-venv gate (#87331): a venv we cannot safely mutate is never mutated at all.
    """
    from moor_cli.update_cmd import _path_uid
    try:
...
def _refuse_update_if_venv_foreign_owned(project_root) -> None:
    ...
    print("  Updating now would fail midway (Permission denied) and leave Moor broken.")
    print("  This usually happens after running moor or pip with sudo. Offending paths:")
