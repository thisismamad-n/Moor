...
Catalog policy:
- Entries are added only by merging a PR into moor-agent. Presence in the
  ``optional-mcps/`` directory = Moor approval. No community tier, no trust
...
- Secrets prompted at install time go to ``~/.moor/.env`` (the
...
class TransportSpec:
    ...
    # auth.env so they are prompted for and land in ~/.moor/.env.
...
def _catalog_root() -> Path:
    """Return the optional-mcps/ directory shipped with this Moor install."""
...
def list_catalog() -> List[CatalogEntry]:
    ...
    UIs can tell the user their Moor is out of date.
    """
    root = _catalog_root()
    if not root.exists():
        return []
...
def catalog_diagnostics() -> List[tuple]:
    ...
    is one of:
      - ``future_manifest`` — manifest_version is newer than this Moor
        understands. Update Moor to install this entry.
...
def _do_git_install(entry: CatalogEntry) -> Path:
    """Clone the entry's repo into ``~/.moor/mcp-installs/<name>`` and run
...
def _prompt_env_vars(specs: List[EnvVarSpec]) -> Dict[str, str]:
    ...
    non-secrets alike to ~/.moor/.env via save_env_value()."""
    ...
    if entry.tools.default_excluded and prior_selection is None:
        if prior_exclude is not None:
            ...
            print(color(
                ...
                "config.yaml or run "
                f"`moor mcp configure {entry.name}` to change.",
                Colors.GREEN,
            ))
            return
