"""MCP catalog — curated, Moor-approved MCP servers shipped with the repo.

Entries are added only by merging a PR into moor-agent; presence in ``optional-mcps/`` = Moor
...
class TransportSpec:
    ...
    # go through auth.env so they are prompted for and land in ~/.moor/.env.
...
def _parse_auth(path: Path, raw: Any, name: str, http: bool) -> AuthSpec:
    ...
    if http and a_type == "api_key":
        ...
        from moor_cli.mcp_config import _env_key_for_server
...
def _parse_manifest(path: Path) -> CatalogEntry:
    ...
    if mv != _MANIFEST_VERSION:
        raise CatalogError(
            ...
            f"(this Moor understands version {_MANIFEST_VERSION})"
        )
...
def list_catalog() -> List[CatalogEntry]:
    ...
    skipped too but surfaced via :func:`catalog_diagnostics` so UIs can say "update Moor".
    """
    root = _catalog_root()
    if not root.exists():
        return []
...
def catalog_diagnostics() -> List[tuple]:
    ...
    ``kind`` is ``future_manifest`` (newer than this Moor) or ``invalid`` (malformed)."""
...
def _install_root() -> Path:
    ...
    root = get_moor_home() / "mcp-installs"
...
def _do_git_install(entry: CatalogEntry) -> Path:
    """Clone the entry's repo into ``~/.moor/mcp-installs/<name>`` and run bootstrap. Returns the dir."""
...
def _prompt_env_vars(specs: List[EnvVarSpec]) -> Dict[str, str]:
    """Prompt for each env spec; secrets and non-secrets alike go to ~/.moor/.env."""
...
def _build_server_config(entry: CatalogEntry, install_dir: Optional[Path]) -> dict:
    """Translate a manifest into the ``mcp_servers.<name>`` block format used by moor_cli/mcp_config.py."""
    cfg: dict = {}
    t = entry.transport
    if t.type == "stdio":
        ...
        if entry.auth.type == "oauth":
            ...
            from moor_cli.mcp_config import _bearer_auth_headers
...
def _probe_tools(name: str) -> Optional[List[tuple]]:
    ...
    if not server_cfg:
        return None
    try:
        from moor_cli.mcp_config import _probe_single_server  # lazy: keep this module cheap
...
def _apply_tool_selection(
    ...
    *,
    ...
    else apply ``default_enabled``, else no filter; point the user at ``moor mcp configure``.
    """
    print()
    name = entry.name
    configure_hint = f"`moor mcp configure {name}`"
    ...
    if len(chosen_indices) == len(probed):
        ...
        # re-run `moor mcp configure <name>` and unselect a tool (switches to include-mode).
        _write_tools_filter(name, "include", None)
        _say(
...
def install_entry(entry: CatalogEntry, *, enable: bool = True) -> None:
    ...
    _say(
        ...
        f"Start a new Moor session to load its tools."
    )
