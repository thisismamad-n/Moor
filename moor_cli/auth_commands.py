...
_OAUTH_CAPABLE_PROVIDERS = {"anthropic", "moor", "openai-codex", "xai-oauth", "qwen-oauth", "minimax-oauth"}
...
def auth_add_command(args) -> None:
    ...
    if provider == "moor":
        # Codex-style auto-import: if a shared Moor credential lives at
        # <moor-root>/shared/moor_auth.json (written by any previous
        # successful login), offer to import it instead of running the
        # full device-code flow. This makes `moor --profile <name>
        # auth add moor --type oauth` a one-tap operation for users who
        # run multiple profiles.
        shared = auth_mod._read_shared_moor_state()
        if shared:
            try:
                path = auth_mod._moor_shared_store_path()
            except RuntimeError:
                path = None
            print()
            if path:
                print(f"Found existing Moor OAuth credentials at {path}")
            else:
            ...
            if do_import in {"", "y", "yes"}:
                ...
                rehydrated = auth_mod._try_import_shared_moor_state(
                    timeout_seconds=getattr(args, "timeout", None) or 15.0,
                )
                if rehydrated is not None:
                    custom_label = (getattr(args, "label", None) or "").strip() or None
                    entry = auth_mod.persist_moor_credentials(rehydrated, label=custom_label)
        ...
        # Honor `--label <name>` so moor matches other providers' UX.  The
        # helper embeds this into providers.moor so that label_from_token
        # doesn't overwrite it on every subsequent load_pool("moor").
        custom_label = (getattr(args, "label", None) or "").strip() or None
    ...
    if provider == "openai-codex":
        ...
        # latest login: a second ``moor auth add openai-codex`` overwrote
    ...
    if provider == "xai-oauth":
        ...
        # The singleton round-trip collapsed every added account into the
        # latest login: a second ``moor auth add xai-oauth`` overwrote the
...
def auth_remove_command(args) -> None:
    ...
    # Unified removal dispatch.  Every credential source Moor reads from
