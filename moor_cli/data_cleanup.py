...
def plan_data_removal(home: Path, project: Path, userdata: Path | None = None) -> DataRemovalPlan:
    ...
    machine = get_default_moor_root(home=home).resolve()
    protected = {
    ...
    if is_bundled_payload(project):
        from moor_cli.bundled_app import NotBundledApp, resolve_bundle_layout
    ...
    canonical = home / "moor-agent"
    if (canonical / "moor_cli" / "__init__.py").is_file():
    ...
    if userdata is not None and not userdata.resolve().is_relative_to(home):
        if userdata.resolve() == Path.home().resolve() or home.is_relative_to(userdata.resolve()):
            raise ValueError(f"desktop data directory must not contain the Moor or user home: {userdata}")
