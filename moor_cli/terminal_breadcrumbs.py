"""Per-terminal session breadcrumbs for ``moor -c`` / ``--continue``.

Each CLI session writes a tiny breadcrumb file
``$MOOR_HOME/terminal-sessions/<terminal-id>`` containing
``{"session_id": ..., "cwd": ..., "ts": ...}``.  A bare ``moor -c`` then
...
def _breadcrumbs_dir() -> Path:
    ...
    return get_moor_home() / "terminal-sessions"
