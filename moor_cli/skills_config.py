"""
Skills configuration for Moor Agent.
`moor skills` enters this module.

Toggle individual skills or categories on/off, globally or per-platform.
Config stored in ~/.moor/config.yaml under:
...
from moor_cli.config import cfg_get, load_config, save_config
from moor_cli.colors import Colors, color
from moor_cli.platforms import PLATFORMS as _PLATFORMS
...
def save_disabled_skills(config: dict, disabled: Set[str], platform: Optional[str] = None):
    ...
    Essential skills (e.g. ``moor-agent``) are silently dropped from the
