...
1. **Bundled plugins** – ``<repo>/plugins/<name>/`` (shipped with moor-agent;
   ``memory/`` and ``context_engine/`` subdirs are excluded — they have their
   own discovery paths)
2. **User plugins**   – ``~/.moor/plugins/<name>/``
3. **Project plugins** – ``./.moor/plugins/<name>/`` (opt-in via
   ``MOOR_ENABLE_PROJECT_PLUGINS``)
4. **Pip plugins**     – packages that expose the ``moor_agent.plugins``
   entry-point group.
...
from moor_cli.middleware import OBSERVER_SCHEMA_VERSION, VALID_MIDDLEWARE
from moor_cli.plugin_capabilities import (  # noqa: F401 — re-exported
    CAPABILITY_REGISTRY,
...
# ---------------------------------------------------------------------------
#
# Set ``MOOR_PLUGINS_DEBUG=1`` to surface verbose plugin-discovery logs to
# stderr in addition to ~/.moor/logs/agent.log. Aimed at plugin authors
...
_PLUGINS_DEBUG = os.getenv("MOOR_PLUGINS_DEBUG", "").strip().lower() in {
    "1", "true", "yes", "on",
}
...
def _install_plugin_debug_handler(force: bool = False) -> None:
    """When MOOR_PLUGINS_DEBUG is on, tee plugin logs to stderr at DEBUG.
    ...
    logger.debug(
        "MOOR_PLUGINS_DEBUG=1 — verbose plugin discovery logging enabled"
    )
...
VALID_HOOKS: Set[str] = {
    ...
    "pre_transcription",
    # Kanban task lifecycle hooks. Fired by moor_cli.kanban_db when a task
    ...
    # separate `moor -p <profile> chat -q` subprocesses:
    #   - kanban_task_claimed   -> the DISPATCHER process (gateway-embedded
    #                              dispatcher or `moor kanban dispatch`),
    ...
    # ``moor kanban dispatch``); on_kanban_task_updated fires in whichever
    ...
    #   result: moor_cli.kanban_db.DispatchResult (spawned, reclaimed,
...
ENTRY_POINTS_GROUP = "moor_agent.plugins"
ENTRY_POINT_CAPABILITIES_GROUP = "moor_agent.plugin_capabilities"
...
def discover_entrypoint_manifests() -> List["PluginManifest"]:
    ...
    * **Capability declarations** — read from the companion
      ``moor_agent.plugin_capabilities`` entry-point group (declarations
...
PLUGIN_SECTIONS_START = "<!-- moor-plugin-sections:start -->"
PLUGIN_SECTIONS_END = "<!-- moor-plugin-sections:end -->"
...
def format_system_prompt_section(section_id: str, content: str) -> str:
    ...
    return (
        ...
        f"<!-- moor-plugin-section-chars:{len(content)} -->\n\n"
        f"{content}"
    )
...
# Reserved event namespace prefix — only core may publish ``moor:<event>``.
MOOR_EVENT_NAMESPACE = "moor"
...
_KNOWN_MANIFEST_FIELDS: Set[str] = {
    ...
    "capabilities", "emits", "listens", "moor", "depends",
}
        ...
        for dep in manifests[k].requires_plugins:
            ...
            if resolved is None:
                logger.warning(
                    ...
                    "via ctx.has_plugin). Run `moor plugins enable %s` if "
                    "it is installed.",
                    k, dep_id, dep_id,
                )
                continue
...
class PluginManifest:
    ...
    #              in ~/.moor/plugins/ still gated by ``plugins.enabled``
    #              (untrusted code).
    kind: str = "standalone"
    # Registry key — path-derived, used by ``plugins.enabled``/``disabled``
    # lookups and by ``moor plugins list``. For a flat plugin at
    ...
    # ``moor_cli.plugin_capabilities.CAPABILITY_REGISTRY``. Declaration is
    ...
    # (``moor plugins show``); a plugin may emit/subscribe without declaring.
...
class PluginState:
    ...
    def data_dir(self) -> Path:
        ...
        return get_moor_home() / "plugin-data" / self._data_namespace
...
class PluginContext:
    ...
    def profile_name(self) -> str:
        ...
        Derived from ``MOOR_HOME`` via
        :func:`moor_cli.profiles.get_active_profile_name`, so it works in
        ...
        running under ``~/.moor/profiles/<name>``, or ``"custom"`` when
        ``MOOR_HOME`` points somewhere unrecognized.
        """
        try:
    ...
    def register_cli_command(
        ...
        """Register a CLI subcommand (e.g. ``moor honcho ...``).
    ...
    def register_command(
        ...
        Unlike ``register_cli_command()`` (which creates ``moor <subcommand>``
    ...
    def register_dashboard_auth_provider(self, provider) -> Optional[PluginRegistration]:
        ...
        ``provider`` must be an instance of
        :class:`moor_cli.dashboard_auth.DashboardAuthProvider`. Used by
        ...
        """
        from moor_cli.dashboard_auth import DashboardAuthProvider
        from moor_cli.dashboard_auth.registry import (
    ...
    def register_secret_source(self, source) -> Optional[PluginRegistration]:
        ...
        sources run during ``load_moor_dotenv()`` startup — after
        ``~/.moor/.env`` loads, before Moor reads credentials — when
        ...
        NOTE ON TIMING: ``load_moor_dotenv()`` usually runs at import
        *before* plugin discovery.  After discovery completes, the plugin
        ...
        + ``load_moor_dotenv``) so the first process sees them (#64177).
    ...
    def register_auxiliary_task(
        ...
        *,
        ...
        core files. After registration, the task:
          ...
          - Appears in the ``moor model → Configure auxiliary models`` picker
        ...
        # Lazy import to avoid circular: moor_cli.main imports plugins indirectly
        from moor_cli.main import _AUX_TASKS as _BUILTIN_AUX_TASKS
    ...
    def emit(self, event: str, payload: Optional[dict] = None) -> int:
        ...
        including ``moor:x`` or a foreign ``other:x``) is rejected with a
        ``ValueError`` and a logged warning — fail-closed. The ``moor:``
    ...
    def subscribe(self, event: str, callback: Callable) -> None:
        ...
        *event* is the full ``<plugin_key>:<event>`` name (or ``moor:<event>``
    ...
    def register_skill(
        ...
        ``~/.moor/skills/`` tree and is **not** listed in the system
...
class PluginManager:
    ...
    def __init__(self, scope_key: Optional[str] = None) -> None:
        ...
        # original scope.
        self.scope_key = scope_key or moor_home_key()
        ...
        # by resolved moor home). The ledger is therefore keyed per manager
        # — i.e. per (moor_home, plugin_id) — and every release/restore
        ...
        # contributed, so `moor plugins list` still attributes them once the
        # full plugin loads.
    ...
    def discover_and_load(self, force: bool = False) -> None:
        ...
        with self._discovery_lock, _plugin_home_scope(self.home_path):
            ...
            if env_var_enabled("MOOR_SAFE_MODE"):
                logger.info("MOOR_SAFE_MODE=1 — plugin discovery skipped")
            ...
            try:
                ...
                # load_moor_dotenv() already ran at import time. Re-pull so the
    ...
    def _discover_and_load_inner(self) -> None:
        ...
        if stale_relay_keys:
            logger.warning(
                "Removed Moor plugin %s is still listed in plugins.enabled; "
        ...
        for manifest in winners.values():
            ...
            # Relay lifecycle ownership now lives in the Moor core. Loading
            ...
            if (
                ...
                loaded.error = (
                    "removed — Relay lifecycle is owned by Moor core; configure "
                    f"{RELAY_PLUGINS_CONFIG_ENV} instead"
                )
                self._plugins[lookup_key] = loaded
                logger.warning(
                    "Refusing to load removed Moor Relay plugin '%s'; %s",
                    lookup_key,
                    loaded.error,
                )
                continue
            ...
            # Built-in backends auto-load — they ship with moor and must
            ...
            # all ~20 of them added several seconds to every `moor`
            # invocation — including plain `moor chat`, which never touches a
            ...
            if not is_enabled:
                loaded = LoadedPlugin(manifest=manifest, enabled=False)
                loaded.error = (
                    "not enabled in config (run `moor plugins enable {}` to activate)"
                    .format(lookup_key)
                )
                self._plugins[lookup_key] = loaded
                logger.debug(
    ...
    def register_approval_transport(
        ...
        *,
        ...
        from moor_cli.approval_transport import RegisteredApprovalTransport
        ...
        self._approval_transports[clean] = RegisteredApprovalTransport(
            ...
            present=present_fn,
            plugin_id=plugin_id,
            profile_home=str(get_moor_home().resolve()),
        )
    ...
    def get_approval_transport(self, name: str):
        ...
        if registered is None:
            return None
        if registered.profile_home != str(get_moor_home().resolve()):
            return None
        return registered
    ...
    def _collect_directory_manifests(self) -> List[PluginManifest]:
        ...
        # 3. Project plugins (./.moor/plugins/), only when explicitly opted
        # in. This must match the full discovery gate exactly.
        if _env_enabled("MOOR_ENABLE_PROJECT_PLUGINS"):
            project_dir = Path.cwd() / ".moor" / "plugins"
            ...
            logger.debug(
                "Project plugins disabled (set MOOR_ENABLE_PROJECT_PLUGINS=1 to enable)"
            )
    ...
    def has_enabled_portable_mcp(self, raw_config: Mapping[str, Any]) -> bool:
        ...
        for manifest in winners.values():
            ...
            if lookup_key not in enabled and manifest.name not in enabled:
                continue
            try:
                from moor_cli.agent_plugins import _discover_mcp
    ...
    def _scan_entry_points(self) -> List[PluginManifest]:
        ...
        declarations from the ``moor_agent.plugin_capabilities`` group.
    ...
    def _register_deferred_platform(self, manifest: PluginManifest) -> None:
        ...
        ``moor plugins list`` still shows the platform as available, and we
        hand the registry a loader that runs the normal eager-load path.
        """
        lookup_key = manifest.key or manifest.name
        platform_name = self._platform_name_from_manifest(manifest)

        # Record an enabled placeholder for introspection (`moor plugins
    ...
    def _register_deferred_platform_tools(
        ...
        ``moor tools`` checklist, and even an explicit ``platform_toolsets``
        ...
        try:
            ...
            # `moor plugins list` under-reports what the process is actually
    ...
    def _load_plugin_scoped(self, manifest: PluginManifest) -> None:
        ...
        try:
            ...
            if register_fn is None:
                ...
                # above cannot see them and `moor plugins list` would
    ...
    def _load_portable_plugin(
        ...
        try:
            ...
            package = load_agent_plugin(
                Path(manifest.path),
                get_moor_home() / "plugin-data" / manifest.skill_namespace,
            )
            ctx = PluginContext(manifest, self)
    ...
    def _load_directory_module(
        ...
        *,
        ...
        """Import a directory-based plugin as ``moor_plugins.<slug>``.

        The module slug is derived from ``manifest.key`` so category-namespaced
        plugins (``image_gen/openai``) import as
        ``moor_plugins.image_gen__openai`` without colliding with any
        future ``tts/openai``.
        """
        ...
        # like "moor-lcm") or from an earlier force=True reload in the
...
# multiple profiles via different MOOR_HOME directories, and a single
# long-lived process (gateway multiplexer, test session, embedder) can
# switch between them via ``set_moor_home_override()`` — which is a
# ContextVar and deliberately does NOT touch os.environ (see
# moor_constants.set_moor_home_override). A process-wide single-slot
...
def _plugin_home_key() -> Path:
    ...
    Plugins are discovered from ``get_moor_home() / "plugins"`` and some
    plugins (notably context engines such as moor-lcm) capture that home
    ...
    ``set_moor_home_override()``) while serving another profile, so the
    plugin manager must be scoped to the active Moor home instead of
    being one process-wide singleton.
    """
    try:
        return get_moor_home().expanduser().resolve()
    except Exception:
        return get_moor_home().expanduser()
...
def _clear_plugin_submodules(manager: Optional[PluginManager]) -> None:
    ...
    ``moor_plugins.<slug>`` and registers that top-level module in
    ...
    *relative* import (``from . import foo``, ``from .sub import bar``)
    ...
    ``moor_plugins.<slug>.<submodule>``. When we swap in a fresh manager
...
def get_plugin_manager() -> PluginManager:
    ...
    profile switch — via ``MOOR_HOME`` or the context-local
    ``set_moor_home_override()`` — gets its own manager with its own
...
def _plugin_toolset_keys_cache_path():
    ...
    return get_moor_home() / "cache" / "plugin_toolset_keys.json"
...
def _get_pre_tool_call_directive_details(
    ...
    from moor_cli.lifecycle import invoke_hook as invoke_lifecycle_hook
...
def resolve_plugin_command_result(result: Any) -> Any:
    ...
    thread = threading.Thread(
        target=_runner,
        name="moor-plugin-command-await",
        daemon=True,
    )
    thread.start()
...
def get_plugin_subscriptions() -> Dict[str, List[Callable]]:
    ...
    (``<plugin_key>:<event>`` or ``moor:<event>``) to subscriber callbacks in
...
def get_plugin_toolsets() -> List[tuple]:
    ...
    Used by the ``moor tools`` TUI so plugin-provided toolsets appear
