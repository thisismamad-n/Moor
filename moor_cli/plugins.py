"""Moor Plugin System — discovers, loads, and manages plugins.

Sources, later overriding earlier on key collision: bundled ``<repo>/plugins/<name>/`` (``memory/``
and ``context_engine/`` have their own discovery), user ``~/.moor/plugins/<name>/``, project
``./.moor/plugins/<name>/`` (opt-in via ``MOOR_ENABLE_PROJECT_PLUGINS``), and pip packages in
the ``moor_agent.plugins`` entry-point group. A directory plugin needs a ``plugin.yaml`` manifest
...
from moor_cli.middleware import VALID_MIDDLEWARE
from moor_cli.plugin_capabilities import plugin_capability_granted
from moor_cli.relay_plugin_cutover import RELAY_PLUGINS_CONFIG_ENV, legacy_relay_plugin_keys
# Sibling modules' names are re-exported here (origin) so plugins and tests keep one import path.
from moor_cli.plugins_manifest import (  # noqa: F401 — re-exported
...
from moor_cli.plugins_discovery import (  # noqa: F401 — re-exported
...
from moor_cli.plugins_loader import (
...
from moor_cli.plugins_dispatch import (  # noqa: F401 — re-exported
    DEFAULT_SYSTEM_PROMPT_SECTION_MAX_CHARS, MOOR_EVENT_NAMESPACE, MAX_SYSTEM_PROMPT_SECTION_CHARS,
...
from moor_cli.plugins_ledger import PluginLedgerMixin, PluginRegistration
from moor_cli.plugins_state import (
...
def get_bundled_plugins_dir() -> Path:
    """Bundled ``plugins/`` dir: ``MOOR_BUNDLED_PLUGINS`` (Nix wrapper / packaged installs, read-only
...
# ``MOOR_PLUGINS_DEBUG=1`` tees verbose discovery logs to stderr in addition to agent.log. Read
# once at import; tests flip it mid-process via ``_install_plugin_debug_handler(force=True)``.
_PLUGINS_DEBUG = env_var_enabled("MOOR_PLUGINS_DEBUG")
_DEBUG_HANDLER_INSTALLED = False


def _install_plugin_debug_handler(force: bool = False) -> None:
    """When MOOR_PLUGINS_DEBUG is on, tee plugin logs to stderr at DEBUG (once per process)."""
    ...
    logger.debug("MOOR_PLUGINS_DEBUG=1 — verbose plugin discovery logging enabled")
...
VALID_HOOKS: Set[str] = {
    ...
    "pre_transcription",
    # Kanban task observers (moor_cli.kanban_db), fired AFTER the DB commit so a slow plugin never
    ...
    # | "idle", result: moor_cli.kanban_db.DispatchResult (spawned, reclaimed, promoted,
...
class PluginContext:
    ...
    def profile_name(self) -> str:
        """Active profile name (``"default"``, the ``~/.moor/profiles/<name>`` id, or ``"custom"``),
        derived from ``MOOR_HOME`` — not ``_cli_ref``, which is None outside the interactive CLI —
    ...
    def _tool_override_allowed(self, tool_name: str) -> bool:
        ...
        Bundled plugins (shipped with Moor core) are trusted by default — an override there is a
    ...
    def register_cli_command(
        ...
        """Register a CLI subcommand (``moor <name> ...``). *setup_fn* receives the argparse
    ...
    def register_dashboard_auth_provider(self, provider) -> Optional[PluginRegistration]:
        """Register a :class:`moor_cli.dashboard_auth.DashboardAuthProvider` for the dashboard
    ...
    def register_auxiliary_task(
        ...
        from moor_cli.main_provider_setup import _AUX_TASKS as _BUILTIN_AUX_TASKS
    ...
    def emit(self, event: str, payload: Optional[dict] = None) -> int:
        ...
        the subscriber count scheduled. Any ``':'`` in the name (``moor:x`` is reserved for core,
        ...
        if ":" in event:
            ...
            raise ValueError(f"Plugin '{plugin_key}' may not emit '{event}': emit only the bare event name; "
                             f"the namespace is forced to '{plugin_key}:' and the '{MOOR_EVENT_NAMESPACE}:' "
    ...
    def register_skill(
        ...
        Not in ``~/.moor/skills/`` nor ``<available_skills>`` — explicit loads only. Raises
...
_SCOPED_PROVIDER_REGISTRARS: Tuple[Tuple[str, str, str, str, str, str, Dict[str, Any]], ...] = (
    ...
    ("register_secret_source", "secret_source", "agent.secret_sources.registry",
     ...
     "Register a :class:`agent.secret_sources.base.SecretSource`, run by ``load_moor_dotenv()`` "
     "(after ``~/.moor/.env``, before credentials are read) when ``secrets.<name>`` is enabled. The "
...
class PluginManager(PluginLoaderMixin, PluginDispatchMixin, PluginLedgerMixin):
    ...
    def __init__(self, scope_key: Optional[str] = None) -> None:
        ...
        # is keyed per (moor_home, plugin_id) and every inverse is identity-conditional — one
        ...
        # and contributed tool names (so `moor plugins list` still attributes them).
    ...
    def discover_and_load(self, force: bool = False) -> None:
        ...
        with self._discovery_lock, _plugin_home_scope(self.home_path):
            ...
            try:
                ...
                # load_moor_dotenv() ran at import, before plugin secret sources existed: re-pull.
                # Plugin secret sources register during discover; the initial load_moor_dotenv() already
    ...
    def _discover_and_load_inner(self) -> None:
        ...
        if stale_relay_keys:
            logger.warning("Removed Moor plugin %s is still listed in plugins.enabled; "
    ...
    def _refresh_plugin_compat_report(self, manifests: List[PluginManifest]) -> None:
        """Refresh MOOR_HOME/.plugin-compat-report.json from this discovery pass (moor_cli.plugin_compat).
        ...
        """
        try:
            from moor_cli.plugin_compat import compat_report
    ...
    def register_approval_transport(self, name: str, present_fn: Callable, *, plugin_id: str) -> None:
        ...
        self._approval_transports[clean] = RegisteredApprovalTransport(
            name=clean, present=present_fn, plugin_id=plugin_id, profile_home=str(get_moor_home().resolve()),
        )
    ...
    def get_approval_transport(self, name: str):
        ...
        if registered is None or registered.profile_home != str(get_moor_home().resolve()):
            return None
        return registered
    ...
    def has_enabled_portable_mcp(self, raw_config: Mapping[str, Any]) -> bool:
        ...
        for lookup_key, manifest in {manifest_key(m): m for m in self._collect_directory_manifests()}.items():
            ...
            try:  # lazy: this is a startup probe, keep agent_plugins unimported unless needed
                ...
                if _discover_mcp(Path(manifest.path), get_moor_home() / "plugin-data"
...
# Resolved Moor home -> PluginManager. A process can switch profiles via
# ``set_moor_home_override()``; a single slot would leak one profile's plugin/context-engine state
...
def _plugin_home_key() -> Path:
    """Resolved active Moor home — the key for per-profile plugin managers (plugins capture the
...
def get_plugin_manager() -> PluginManager:
    """Return the plugin manager for the active Moor profile/home (cached per resolved home; a
...
def _reset_plugin_managers_for_tests() -> None:
    ...
    try:
        from moor_cli.dashboard_auth.registry import clear_providers
        clear_providers()
    except Exception:
...
def resolve_plugin_command_result(result: Any) -> Any:
    ...
    threading.Thread(target=_runner, name="moor-plugin-command-await", daemon=True).start()
...
def get_plugin_toolsets() -> List[tuple]:
    """Plugin toolsets as ``(key, label, description)`` tuples for the ``moor tools`` TUI."""
    manager = get_plugin_manager()
...
_PLUGIN_COMPAT_LAZY = {
    'CAPABILITY_REGISTRY': ('moor_cli.plugin_capabilities', 'CAPABILITY_REGISTRY'),
    'ENTRY_POINT_CAPABILITIES_GROUP': ('moor_cli.plugins_discovery', 'ENTRY_POINT_CAPABILITIES_GROUP'),
    'LEGACY_RELAY_PLUGIN_KEYS': ('moor_cli.relay_plugin_cutover', 'LEGACY_RELAY_PLUGIN_KEYS'),
    'MAX_SYSTEM_PROMPT_SECTIONS': ('moor_cli.plugins_dispatch', 'MAX_SYSTEM_PROMPT_SECTIONS'),
    'OBSERVER_SCHEMA_VERSION': ('moor_cli.middleware', 'OBSERVER_SCHEMA_VERSION'),
    'VALID_CAPABILITY_IDS': ('moor_cli.plugin_capabilities', 'VALID_CAPABILITY_IDS'),
    'cfg_get': ('moor_cli.config', 'cfg_get'),
    'fast_safe_load': ('utils', 'fast_safe_load'),
    'format_system_prompt_section': ('moor_cli.plugins_dispatch', 'format_system_prompt_section'),
