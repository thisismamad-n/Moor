...
Mixin on ``MoorCLI``; cli.py symbols are imported lazily inside methods (import cycle)."""
...
class CLILoopsMixin:
    ...
    def _cmd_plugins(self, cmd_original: str):
        ...
        try:
            # Discover from disk (bundled + user) like `moor plugins list`, so
            ...
            # on one line (full catalog behind `moor plugins list`).
    ...
    def _maybe_continue_goal_after_turn(self) -> None:
        ...
        _active_deleg = 0
        try:
            from moor_cli.goals import count_active_delegations, gather_background_processes as _gather_bg
