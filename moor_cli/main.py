...
Usage:
    moor                     # Interactive chat (default)
    moor chat                # Interactive chat
    moor gateway             # Run gateway in foreground
    moor gateway start       # Start gateway as service
    moor gateway stop        # Stop gateway service
    moor gateway status      # Show gateway status
    moor gateway install     # Install gateway service
    moor gateway uninstall   # Uninstall gateway service
    moor setup               # Interactive setup wizard
    moor logout              # Clear stored authentication
    moor status              # Show status of all components
    moor cron                # Manage cron jobs
    moor cron list           # List cron jobs
    moor cron status         # Check if cron scheduler is running
    moor doctor              # Check configuration and dependencies
    moor honcho setup                    # Configure Honcho AI memory integration
    moor honcho status                   # Show Honcho config and connection status
    moor honcho sessions                 # List directory → session name mappings
    moor honcho map <name>               # Map current directory to a session name
    moor honcho peer                     # Show peer names and dialectic settings
    moor honcho peer --user NAME         # Set user peer name
    moor honcho peer --ai NAME           # Set AI peer name
    moor honcho peer --reasoning LEVEL   # Set dialectic reasoning level
    moor honcho mode                     # Show current memory mode
    moor honcho mode [hybrid|honcho|local]  # Set memory mode
    moor honcho tokens                   # Show token budget settings
    moor honcho tokens --context N       # Set session.context() token cap
    moor honcho tokens --dialectic N     # Set dialectic result char cap
    moor honcho identity                 # Show AI peer identity representation
    moor honcho identity <file>          # Seed AI peer identity from a file (SOUL.md etc.)
    moor honcho migrate                  # Step-by-step migration guide: OpenClaw native → Moor + Honcho
    moor --version           Show version and update status
    moor update              Update to latest version
    moor uninstall           Uninstall Moor Agent
    moor acp                 Run as an ACP server for editor integration
    moor sessions browse     Interactive session picker with search

    moor claw migrate --dry-run  # Preview migration without changes
"""

# IMPORTANT: moor_bootstrap must be the very first import — it sets up
# UTF-8 stdio on Windows so print()/subprocess children don't hit
# UnicodeEncodeError with non-ASCII characters.  No-op on POSIX.
#
# Guarded against ModuleNotFoundError because ``moor_bootstrap`` is a
# top-level module registered via pyproject.toml's ``py-modules`` list.
# When the user upgrades code via ``git pull`` (or ``moor update``
# crashes between ``git reset --hard`` and ``uv pip install -e .``), the
# new code references ``moor_bootstrap`` but the editable install's
# ``.pth`` file still points at the old set of top-level modules.  Without
# this guard, moor crashes on import and the user can't run
# ``moor update`` to recover.  Missing the bootstrap means UTF-8 stdio
...
from moor_cli._subprocess_compat import suppress_platform_ver_console
...
# Two lines of inline path math so ``python moor_cli/main.py`` (script
# mode — sys.path[0] is moor_cli/, not the repo root) can import the
# canonical helpers; everything else lives in moor_cli._startup_fast.
...
# a prior ``moor update`` left a recovery marker and a core package's import
# files were wiped (#57828 — failed lazy backend refresh), the module-level
# ``from moor_cli.env_loader import ...`` / ``from moor_cli.config import
...
# package directory, so if IT can't import, nothing else in moor_cli can
...
def _set_process_title() -> None:
    """Set the process title to 'moor' so tools like 'ps', 'top', and
    'htop' show the app name instead of 'python3.xx'.

    Purely cosmetic — non-fatal on any platform.

    Strategy (try in order):
      1. ``setproctitle`` (opt-in dep — installed via ``moor tools`` or
      ...
      4. No-op on Windows (the .exe name is already ``moor.exe``).
    """
    # Strategy 1: setproctitle (best — works on macOS, Linux, BSD)
    try:
        import setproctitle  # type: ignore[import-untyped]

        setproctitle.setproctitle("moor")
        return
    except ImportError:
        pass
    ...
    try:
        ...
        if system == "Linux":
            ...
            libc.prctl(15, b"moor", 0, 0, 0)  # PR_SET_NAME = 15
        elif system == "Darwin":
            libc = ctypes.CDLL("libc.dylib", use_errno=True)
            libc.pthread_setname_np(b"moor")
        # Windows: the .exe name is already ``moor.exe`` — nothing to do.
    except Exception:
        pass
...
# that run *before* moor_cli.config is importable. Mirrors the explicit
...
def _config_default_interface_early() -> str:
    ...
    try:
        ...
        if home:
            ...
            cfg_path = os.path.join(os.path.expanduser("~"), ".moor", "config.yaml")
...
def _wants_tui_early(argv: "list[str] | None" = None) -> bool:
    ...
    explicit ``--tui``/``MOOR_TUI=1``, then a real-TTY gate (a
    ...
    cron jobs, pipes run ``moor … chat -q`` with stdio on a pipe. This
    ...
    if "--cli" in argv:
        return False
    if os.environ.get("MOOR_TUI") == "1" or "--tui" in argv:
        return True
    try:
...
# `entry.tsx`; this is just the earlier cousin. ``MOOR_TUI_NO_EARLY_DISABLE``
# escapes the behaviour for diagnostics.
def _suppress_mouse_residue_early() -> None:
    if os.environ.get("MOOR_TUI_NO_EARLY_DISABLE") == "1":
        return
    if not _wants_tui_early():
        return
    try:
        # Skip when stdout is redirected (`moor --tui … >log`, CI capture):
...
def _active_profile_may_override_home_fast(moor_root: str) -> bool:
    return _startup_fast.active_profile_may_override_home(moor_root)
...
def _try_ultrafast_version() -> bool:
    """Handle ``moor --version`` before config/logging imports."""
...
from moor_cli.subcommands._shared import add_accept_hooks_flag as _add_accept_hooks_flag
from moor_cli.subcommands.cron import build_cron_parser
from moor_cli.subcommands.sync import build_sync_parser
...
from moor_cli.subcommands.whatsapp import build_whatsapp_parser
...
def _require_tty(command_name: str) -> None:
    ...
    Interactive TUI commands (moor tools, moor setup, moor model) use
    curses or input() prompts that spin at 100% CPU when stdin is a pipe.
    This guard prevents accidental non-interactive invocation.
    """
    if not sys.stdin.isatty():
        print(
            f"Error: 'moor {command_name}' requires an interactive terminal.\n"
...
# ---------------------------------------------------------------------------
# Profile override — MUST happen before any moor module import.
#
# Many modules cache MOOR_HOME at import time (module-level constants).
# We intercept --profile/-p from sys.argv here and set the env var so that
# every subsequent ``os.getenv("MOOR_HOME", ...)`` resolves correctly.
# The flag is stripped from sys.argv so argparse never sees it.
# Falls back to ~/.moor/active_profile for sticky default.
# ---------------------------------------------------------------------------
def _apply_profile_override() -> None:
    ...
    def _inside_mcp_add_args(index: int) -> bool:
        """True once argv reaches `moor mcp add ... --args <command argv>`.
    ...
    def _resolve_sudo_user_profile_env(name: str) -> str | None:
        """Resolve `sudo moor -p <name>` against the invoking user's home.
        ...
        candidate = home / ".moor" / "profiles" / name
        try:
            if candidate.is_dir():
    ...
    # after the subcommand (`moor chat -p coder`), so keep scanning broadly.
    ...
    # Mirrors moor_cli.profiles._PROFILE_ID_RE so we never call
    ...
    # 1.5 If MOOR_HOME is already set and no explicit flag was given, trust it
    # only when it already points to a specific profile directory.  The
    # distinguishing heuristic: a profile path has "profiles" as its immediate
    # parent directory name (e.g. ~/.moor/profiles/coder or
    # /opt/data/profiles/coder).  If MOOR_HOME points to the moor root
    # instead (e.g. systemd hardcodes MOOR_HOME=/root/.moor), we must
    # still read active_profile — the user may have switched profiles via
    # `moor profile use` and the gateway should honour that choice.
    # See issue #22502.
    moor_home_env = os.environ.get("MOOR_HOME", "")
    if profile_name is None and moor_home_env:
        if Path(moor_home_env).parent.name == "profiles":
            return

    # 2. If no flag, check active_profile in the moor root.
    #
    # EXCEPTION: a supervised s6 gateway child (exported by the container
    # run-script as MOOR_S6_SUPERVISED_CHILD=1) must NOT follow the sticky
    ...
    # reserved ``gateway-default`` slot runs bare ``moor gateway run`` to mean
    # "the root MOOR_HOME profile". If the reserved default child read
    ...
    if profile_name is None and not os.environ.get("MOOR_S6_SUPERVISED_CHILD"):
        try:
            from moor_constants import get_default_moor_root

            active_path = get_default_moor_root() / "active_profile"
    ...
    # 3. If we found a profile, resolve and set MOOR_HOME
    if profile_name is not None:
        try:
            from moor_cli.profiles import resolve_profile_env

            moor_home = resolve_profile_env(profile_name)
        except FileNotFoundError as exc:
            moor_home = _resolve_sudo_user_profile_env(profile_name)
            if not moor_home:
            ...
            # A bug in profiles.py must NEVER prevent moor from starting
            print(
...
# Windows launcher self-heal — the ``moor`` command users run is a COPY of
# the venv console script, staged into the managed binary dir (the default
# Moor root's ``bin``, next to the managed uv) by install.ps1. That dir
# lives OUTSIDE the git checkout precisely because an earlier layout staged
# the copies at ``<checkout>\bin``, where ``moor update``'s autostash
# (``git stash push --include-untracked``) swept them off disk; with the
# desktop updater's ``--keep-stash`` nothing restored them and ``moor``
...
# ``python -m moor_cli.main``. Costs a few stat calls when healthy; gates
# fail toward inaction so source checkouts are untouched. Sits AFTER the
# profile override on purpose — no moor module may be imported before
# profiles resolve. The launcher dir itself is per-machine (the helper
# anchors on the DEFAULT root, not MOOR_HOME), so profile sessions heal
# the same shared dir.
if sys.platform == "win32":
    try:
        from moor_cli import _install_repair as _install_repair_mod

        _install_repair_mod.ensure_windows_bin_launchers(_bootstrap_root)
    except Exception:
        pass

# Load .env from ~/.moor/.env first, then project root as dev fallback.
...
# Bridge security.redact_secrets from config.yaml → MOOR_REDACT_SECRETS env
# var BEFORE moor_logging imports agent.redact (which snapshots the flag at
...
try:
    ...
    # yaml.load — the SAME parse then serves moor_logging's
        ...
        if "MOOR_REDACT_SECRETS" not in os.environ:
            ...
            if isinstance(_early_sec_cfg, dict):
                ...
                if _early_redact is not None:
                    os.environ["MOOR_REDACT_SECRETS"] = str(_early_redact).lower()
...
# Initialize centralized file logging early — all `moor` subcommands
...
try:
    from moor_logging import setup_logging as _setup_logging
...
if _FORCE_IPV4_EARLY:
    try:
        from moor_constants import apply_ipv4_preference as _apply_ipv4

        _apply_ipv4(force=True)
    except Exception:
        pass  # best-effort — don't crash if moor_constants not importable yet
...
from moor_cli import __version__, __release_date__

# Provider model-selection wizard flows extracted to moor_cli/model_setup_flows.py
# (god-file decomposition Phase 2). Re-imported here so select_provider_and_model and
# existing test monkeypatches (moor_cli.main._model_flow_*) keep resolving unchanged.
...
def _termux_bundled_skills_stamp_path() -> Path:
    return get_moor_home() / "skills" / ".termux_bundled_sync_stamp"


def _termux_bundled_skills_sync_needed() -> bool:
    if not _is_termux_startup_environment():
        return True
    if os.environ.get("MOOR_TERMUX_FORCE_SKILLS_SYNC") == "1":
        return True
    try:
...
def _termux_should_prefetch_update_check() -> bool:
    if not _is_termux_startup_environment():
        return True
    return os.environ.get("MOOR_TERMUX_PREFETCH_UPDATES") == "1"
...
def _relative_time(ts) -> str:
    ...
    in :mod:`moor_cli.timefmt` so lightweight consumers don't have to
    import the whole CLI surface.
    """
    from moor_cli.timefmt import relative_time
...
def _has_any_provider_configured() -> bool:
    ...
    from moor_cli.config import get_env_path, get_moor_home, load_config
    ...
    if isinstance(model_cfg, dict):
        ...
        if isinstance(_default, dict):
            from moor_cli.config import split_model_config_default
        ...
        _model_name = ""
    _has_moor_config = _model_name and _model_name != _DEFAULT_MODEL
    ...
    # Check for Moor Portal OAuth credentials
    auth_file = get_moor_home() / "auth.json"
    if auth_file.exists():
        try:
            import json
...
def _session_browse_picker(sessions: list, session_db=None) -> Optional[str]:
    ...
    def _delete_session(session_id: str) -> bool:
        if session_db is None:
            return False
        try:
            sessions_dir = get_moor_home() / "sessions"
        except Exception:
            sessions_dir = None
        try:
            return bool(
...
def _resolve_last_session(source: str = "cli") -> Optional[str]:
    ...
    ``moor -c`` from repo A continues repo A's last session rather than the
...
def _exec_in_container(container_info: dict, cli_args: list):
    ...
    Args:
        container_info: dict with backend, container_name, exec_user, moor_bin
        cli_args: the original CLI arguments (everything after 'moor')
    """

    backend = container_info["backend"]
    container_name = container_info["container_name"]
    exec_user = container_info["exec_user"]
    moor_bin = container_info["moor_bin"]
        ...
        if sudo_path:
            ...
            if probe2.returncode != 0:
                print(
                    ...
                    f"  }}];\n"
                    f"\n"
                    f"Or run: sudo moor {' '.join(cli_args)}",
                    file=sys.stderr,
                )
                sys.exit(1)
        else:
            print(
                f"Error: container '{container_name}' not found via {backend}.\n"
                f"The container may be running under root. Try: sudo moor {' '.join(cli_args)}",
                file=sys.stderr,
            )
            sys.exit(1)
    ...
    if continue_val and not getattr(args, "resume", None):
        if isinstance(continue_val, str):
            ...
            if resolved:
                ...
                print(
                    "Use 'moor sessions list' to see available sessions, or "
            ...
            try:
                from moor_cli.terminal_breadcrumbs import resolve_breadcrumb_session
...
def _print_tui_exit_summary(
    ...
    print(f"  moor --tui --resume {target}")
    if title:
        print(f'  moor --tui -c "{title}"')
    print()
...
def _tui_need_npm_install(root: Path) -> bool:
    """True when @moor/ink is missing or node_modules is behind package-lock.json.
    ...
    *root* (``ui-tui/dist/entry.js``).
    ...
    ink = ws_root / "node_modules" / "@moor" / "ink" / "package.json"
    if not ink.is_file():
        return True
    if not lock.is_file():
        return False
...
_TUI_BUILD_INPUT_FILES = (
    ...
    "scripts/build.mjs",
    "packages/moor-ink/package.json",
    "packages/moor-ink/index.js",
    "packages/moor-ink/text-input.js",
)
...
def _tui_need_rebuild(root: Path) -> bool:
    ...
    or local edits. Set ``MOOR_TUI_FORCE_BUILD=1`` to force the old behaviour.
    """
...
def _ensure_tui_node() -> None:
    ...
    ``MOOR_SKIP_NODE_BOOTSTRAP=1`` to disable auto-install.
    """
    if shutil.which("node") and shutil.which("npm"):
        return
    if os.environ.get("MOOR_SKIP_NODE_BOOTSTRAP"):
        return
    ...
    try:
        ...
        result = subprocess.run(
            ...
            env={**os.environ, "MOOR_HOME": moor_home},
            capture_output=True,
            text=True,
            encoding="utf-8",
            errors="replace",
    ...
    extras.extend([Path(moor_home) / "node" / "bin", Path.home() / ".local" / "bin"])
...
def _find_bundled_tui(moor_cli_dir: Path | None = None) -> Path | None:
    """Find a pre-built TUI entry.js bundled in the wheel."""
    if moor_cli_dir is None:
        moor_cli_dir = Path(__file__).parent
    bundled = moor_cli_dir / "tui_dist" / "entry.js"
...
def _restore_tui_workspace(tui_dir: Path) -> bool:
    ...
    files deleted in the working tree after ``moor update`` (HEAD stays
...
def _ensure_tui_workspace(tui_dir: Path) -> None:
    ...
    if _restore_tui_workspace(tui_dir):
        if not os.environ.get("MOOR_QUIET"):
    ...
    print(
        ...
        "This usually means `moor update` left tracked ui-tui files deleted.\n"
        "Recovery:\n"
        "  1. From the Moor checkout, run `git restore -- ui-tui`\n"
        "  2. Run `npm install --silent --no-fund --no-audit --progress=false`\n"
        "  3. Retry `moor --tui`\n"
        "If the checkout is still inconsistent, run `moor update --force`.",
        file=sys.stderr,
    )
    sys.exit(1)
...
def _make_tui_argv(tui_dir: Path, tui_dev: bool) -> tuple[list[str], Path]:
    ...
    def _node_bin(bin: str) -> str:
        if bin == "node":
            ...
            if env_node and os.path.isfile(env_node) and os.access(env_node, os.X_OK):
                return env_node
        # find_node_executable() prefers the managed $MOOR_HOME/node tree,
        ...
        from moor_constants import find_node_executable
    ...
    if tui_dev and ext_dir:
        print(
            f"Error: --dev is incompatible with MOOR_TUI_DIR={ext_dir}\n"
            f"The prebuilt TUI has no source code to hot-reload.\n"
            f"Unset MOOR_TUI_DIR (e.g. `unset MOOR_TUI_DIR`) to use --dev from a checkout.",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    # moor_cli/tui_dist/entry.js but never ships ui-tui/ at all (that
    ...
    if (
        ...
        def _run_tui_install() -> subprocess.CompletedProcess:
            ...
            return subprocess.run(
                ...
                encoding="utf-8",
                errors="replace",
                env=_npm_lifecycle_env(with_moor_node_path()),
            )
    ...
    if tui_dev:
        # Keep the local @moor/ink package exports in sync with source.
        # --dev runs src/entry.tsx directly, but @moor/ink resolves through
        # packages/moor-ink/dist/entry-exports.js. If that dist bundle is
        ...
        npm = _node_bin("npm")
        ink_dir = tui_dir / "packages" / "moor-ink"
        result = subprocess.run(
            [npm, "run", "build"],
            cwd=str(ink_dir),
            capture_output=True,
...
def _normalize_tui_toolsets(toolsets: object) -> list[str]:
    ...
    try:
        from moor_cli.oneshot import _normalize_toolsets
...
def _apply_tui_python_env(env: dict) -> None:
    ...
    src_root = str(env.get("MOOR_PYTHON_SRC_ROOT") or "").strip()
    if not src_root or not Path(src_root).is_dir():
        env["MOOR_PYTHON_SRC_ROOT"] = str(PROJECT_ROOT)

    cwd = str(env.get("MOOR_CWD") or "").strip()
    if not cwd or not Path(cwd).is_dir():
        env["MOOR_CWD"] = _safe_tui_cwd(env)

    python = str(env.get("MOOR_PYTHON") or "").strip()
    if os.path.dirname(python):
        python_path = Path(python)
        if not python_path.is_absolute():
            python_path = Path(env["MOOR_CWD"]) / python_path
    ...
    if not python_is_executable:
        env["MOOR_PYTHON"] = sys.executable
...
def _launch_tui(
    ...
    # TUI child is a moor process: propagate the profile-home contract via
    # the single factory; keep secrets (the TUI/agent needs provider creds).
    from tools.environments.local import build_subprocess_env
    env = build_subprocess_env(scrub_secrets=False, inherit_profile_home=True)
    try:
        from moor_cli.config import apply_terminal_config_to_env
        apply_terminal_config_to_env(env=env)
    except Exception:
        logger.debug("Failed to apply terminal config bridge for TUI launch", exc_info=True)
    active_session_fd, active_session_file = tempfile.mkstemp(
        prefix="moor-tui-active-session-", suffix=".json"
    )
    os.close(active_session_fd)
    env["MOOR_TUI_ACTIVE_SESSION_FILE"] = active_session_file
    ...
    if worktree:
        ...
        if not wt_info:
            sys.exit(1)
        env["MOOR_CWD"] = wt_info["path"]
        env["TERMINAL_CWD"] = wt_info["path"]

    _apply_tui_python_env(env)

    if model:
        env["MOOR_MODEL"] = model
        env["MOOR_INFERENCE_MODEL"] = model
    if provider:
        env["MOOR_TUI_PROVIDER"] = provider
        env["MOOR_INFERENCE_PROVIDER"] = provider
    tui_toolsets = _normalize_tui_toolsets(toolsets)
    if tui_toolsets:
        env["MOOR_TUI_TOOLSETS"] = ",".join(tui_toolsets)
    if skills:
        if isinstance(skills, (list, tuple)):
            ...
            if flattened:
                env["MOOR_TUI_SKILLS"] = ",".join(flattened)
        else:
            value = str(skills).strip()
            if value:
                env["MOOR_TUI_SKILLS"] = value
    if query:
        env["MOOR_TUI_QUERY"] = query
    if image:
        env["MOOR_TUI_IMAGE"] = image
    if checkpoints:
        env["MOOR_TUI_CHECKPOINTS"] = "1"
    if pass_session_id:
        env["MOOR_TUI_PASS_SESSION_ID"] = "1"
    if max_turns is not None:
        env["MOOR_TUI_MAX_TURNS"] = str(max_turns)
    if verbose:
        env["MOOR_TUI_TOOL_PROGRESS"] = "verbose"
    elif quiet:
        env["MOOR_TUI_TOOL_PROGRESS"] = "off"
    if accept_hooks:
        env["MOOR_ACCEPT_HOOKS"] = "1"
    ...
    # MOOR_TUI_RESUME is an internal hand-off from the Python wrapper to the
    # Ink app.  Because we start from a full os.environ snapshot (via
    # build_subprocess_env), an exported/stale value
    # in the user's shell would otherwise make a plain `moor --tui` try to
    ...
    # still set MOOR_TUI_RESUME themselves.
    env.pop("MOOR_TUI_RESUME", None)
    if resume_session_id:
        env["MOOR_TUI_RESUME"] = resume_session_id
    ...
    # Exit code 42 = TUI requested an update. Relaunch as `moor update` so
...
def _pin_kanban_board_env() -> None:
    """Pin the active kanban board into ``MOOR_KANBAN_BOARD`` for the chat session.

    Without this, in-process tools (``kanban_*``) and shelled-out CLI calls
    (``moor kanban …``) resolve the board on different paths: the env-pin if
    set, otherwise the global ``<root>/kanban/current`` file. A concurrent
    ``moor kanban boards switch`` from another session can flip the file
    ...
    """
    if os.environ.get("MOOR_KANBAN_BOARD"):
        return
    try:
        from moor_cli.kanban_db import get_current_board

        os.environ["MOOR_KANBAN_BOARD"] = get_current_board()
    except Exception:
        pass


def _sync_bundled_skills_quietly() -> None:
    """Seed ``~/.moor/skills/`` with the bundled skill library on first launch.
...
def _resolve_use_tui(args) -> bool:
    ...
    workers, cron jobs, and pipelines run ``moor … chat -q`` with stdout
...
def cmd_chat(args):
    ...
    if isinstance(_resume_raw, str) and _resume_raw.strip().lower() == "latest":
        ...
        if _last_id:
            ...
            print("Use 'moor sessions list' to see available sessions.")
            sys.exit(1)
    ...
    if isinstance(_resume_foreign, str) and _resume_foreign.strip().lower() in (
        ...
        from moor_cli.foreign_sessions import (
        ...
        print(f"  (later: moor --resume {_imported_id})")
    ...
    try:
        ...
        from moor_cli.config import load_config as _load_config_for_xai_check
        ...
        if _retired_xai_refs:
            ...
            sys.stderr.write("  \033[2mRun 'moor doctor' for details.\033[0m\n\n")
    except Exception:
        pass
    ...
    if not _has_any_provider_configured():
        ...
        if reply in {"", "y", "yes"}:
            ...
            return
        print()
        print("You can run 'moor setup' at any time to configure.")
        sys.exit(1)
    ...
    if _termux_should_prefetch_update_check():
        try:
            from moor_cli.banner import prefetch_banner_data, prefetch_update_check
    ...
    #
    # FIRST RUN is the exception: with an empty ~/.moor/skills the banner
    ...
    def _skills_dir_is_unseeded() -> bool:
        try:
            from moor_cli.config import get_moor_home
            skills_dir = Path(get_moor_home()) / "skills"
    ...
    if _skills_dir_is_unseeded():
        ...
        try:
            import moor_cli.banner as _banner_mod
    ...
    # user's ~/.moor/config.yaml and return built-in defaults. Set BEFORE
    # importing cli (which runs `CLI_CONFIG = load_cli_config()` at module
    # import time). Credentials in .env are still loaded — this flag only
    # ignores behavioral/config settings.
    if getattr(args, "ignore_user_config", False):
        os.environ["MOOR_IGNORE_USER_CONFIG"] = "1"
...
def cmd_gateway(args):
    ...
    from moor_cli.gateway import gateway_command
...
def cmd_proxy(args):
    ...
    from moor_cli.proxy.cli import cmd_proxy as _cmd_proxy
...
def cmd_whatsapp(args):
    ...
    # has no creds.json.  Every subsequent `moor gateway` then paid a 30s
    ...
    session_dir = get_moor_home() / "whatsapp" / "session"
    ...
    if (session_dir / "creds.json").exists():
        ...
        if response.lower() in {"y", "yes"}:
            ...
            print("  Start the gateway with: moor gateway")
            return
    ...
    if (session_dir / "creds.json").exists():
        ...
        # and `moor gateway` skips it cleanly instead of paying a 30s
        ...
        if wa_mode == "bot":
            print("  Next steps:")
            print("    1. Start the gateway:  moor gateway")
        ...
        print()
        print("  Or install as a service: moor gateway install")
    else:
        print("⚠ Pairing may not have completed. Run 'moor whatsapp' to try again.")
...
def cmd_whatsapp_cloud(args):
    ...
    Distinct from ``moor whatsapp`` (the Baileys bridge wizard) — the
    two adapters are complementary, not alternatives. See
    ``moor_cli/setup_whatsapp_cloud.py``.
    """
    _require_tty("whatsapp-cloud")
    from moor_cli.setup_whatsapp_cloud import run_whatsapp_cloud_setup
...
def cmd_model(args):
    ...
    if getattr(args, "refresh", False):
        try:
            ...
            pass
    from moor_cli.setup import run_setup_action_with_navigation
...
def select_provider_and_model(args=None):
    ...
    Shared by ``cmd_model`` (``moor model``) and the setup wizard
    ...
    effective_provider = (
        config_provider or os.getenv("MOOR_INFERENCE_PROVIDER") or "auto"
    )
    ...
    if not active and effective_provider != "auto":
        ...
        if active_def is not None:
            ...
            warning = (
                f"Unknown provider '{effective_provider}'. Check 'moor model' for "
                "available providers, or run 'moor doctor' to diagnose config "
                "issues."
            )
    ...
    # PROVIDER_GROUPS in moor_cli/models.py). A multi-member group shows one
    ...
    # Honor ``model_catalog.excluded_providers`` so the CLI ``moor model``
    ...
    # matching against moor_id / alias / canonical slug.
    _cli_excluded = {
        str(p).strip().lower()
    ...
    elif selected_provider == "moor":
        _model_flow_moor(config, current_model, args=args)
    ...
    # a leftover OPENAI_BASE_URL in ~/.moor/.env can poison auxiliary
...
def _clear_stale_openai_base_url():
    """Remove OPENAI_BASE_URL from ~/.moor/.env if the active provider is not 'custom'.
    ...
    """
    from moor_cli.config import get_env_value, save_env_value, load_config
...
# `moor model` provider picker. It does NOT re-run credential setup — it
# only routes already-authenticated providers to specific aux tasks. Users
# configure new providers through the normal `moor model` flow first.
# ─────────────────────────────────────────────────────────────────────────────
...
def _all_aux_tasks() -> list[tuple[str, str, str]]:
    ...
    :meth:`moor_cli.plugins.PluginContext.register_auxiliary_task`) appear
    in the same surfaces as built-in ones without core knowing about them.
    """
    tasks = list(_AUX_TASKS)
    try:
        from moor_cli.plugins import get_plugin_auxiliary_tasks
...
def _aux_config_menu() -> None:
    ...
    while True:
        ...
        print("  Moor Portal) if the main model is unavailable.  Override a")
...
def _aux_select_for_task(task: str) -> None:
    ...
    up new ones through the normal ``moor model`` flow, then route aux tasks
    to them here.
    """
    from moor_cli.config import load_config
    from moor_cli.inventory import build_aux_picker_rows, format_aux_picker_entries
...
def _aux_flow_provider_model(
    ...
    from moor_cli.models import get_pricing_for_provider
...
def _prompt_provider_choice(choices, *, default=0, title="Select provider:"):
    ...
    """
    try:
        from moor_cli.setup import _curses_prompt_choice
...
# downstream call sites read `moor_cli.main._PROVIDER_MODELS` directly,
# so the symbol needs to be reachable as a module attribute. But importing
# the catalog eagerly costs ~55ms on every `moor` invocation — including
# fast paths like `moor --version` and slash-command dispatch that never
...
# surface so argparse wiring and test monkeypatches on moor_cli.main.<name>
# keep resolving unchanged. Importing those modules eagerly costs ~50ms on
# every `moor` invocation, including fast paths like `moor --version`
...
_LAZY_COMMAND_EXPORTS = {
    "moor_cli.sessions_cmd": (
        "cmd_sessions",
    ),
    "moor_cli.dashboard_procs": (
        "_detect_concurrent_moor_instances",
...
def _self():
    ...
    keeps monkeypatches on moor_cli.main.<name> working, exactly like a
...
def _prompt_api_key(
    ...
    """Shared API-key entry point for ``moor setup`` / ``moor model``.
    ...
    recover from a malformed paste without editing ``~/.moor/.env`` by hand.

    Returns ``(resolved_key, abort)``.  ``abort=True`` means the caller should
    ``return`` immediately — the user cancelled entry, declined to replace, or
    cleared the key and is now unconfigured.
    """
    from moor_cli.auth import LMSTUDIO_NOAUTH_PLACEHOLDER
    ...
    from moor_cli.env_loader import format_secret_source_suffix
    ...
    if choice.startswith("c") and not pool_backed:
        ...
        print(
            f"  API key cleared.  Re-run `moor setup` to configure {pconfig.name} again."
        )
        return "", True
...
def cmd_login(args):
    ...
    from moor_cli.auth import login_command
...
def cmd_logout(args):
    ...
    from moor_cli.auth import logout_command
...
def cmd_auth(args):
    ...
    from moor_cli.auth_commands import auth_command
...
def cmd_status(args):
    ...
    from moor_cli.status import show_status
...
def cmd_sync(args):
    ...
    if sub in {"enable", "disable"}:
        ...
        if not is_curation_eligible(skill):
            print(
                ...
                f"skills under ~/.moor/skills/ can sync.",
                file=sys.stderr,
            )
            return 1
    ...
    if sub == "status":
        ...
        if status.get("org_available"):
            ...
            if modified:
                print(
                    ...
                    f"{', '.join(modified)}\n"
                    f"  Share them back with `moor sync propose <skill>`. "
        ...
        if not status.get("logged_in"):
            print("\nNot logged into Moor Portal — sync is inert.", file=sys.stderr)
        elif not status.get("moor_admin"):
            print(
            ...
            print(
                "\nSync feature is off for this instance (set MOOR_SYNC_ENABLED=1 "
            ...
            print(
                ...
                "MOOR_SYNC_BASE_URL). Sync is inert.",
                file=sys.stderr,
            )
        return 0
    ...
    try:
        if sub == "pull":
            ...
            result = ssc.push_skills(identity=identity, message="moor sync push")
        elif sub == "now":
            pull_res = ssc.pull_skills(identity=identity)
            push_res = ssc.push_skills(identity=identity, message="moor sync now")
...
def cmd_webhook(args):
    ...
    from moor_cli.webhook import webhook_command
...
def cmd_slack(args):
    ...
    Dispatches ``moor slack <subcommand>``. Currently supports:
    ...
    if sub in {None, ""}:
        ...
        print(
            "usage: moor slack <subcommand>\n"
            "\n"
            "subcommands:\n"
            "  manifest   Generate a Slack app manifest with every gateway\n"
            "             command registered as a native slash\n"
            "\n"
            "Run `moor slack manifest -h` for details.",
            file=sys.stderr,
        )
        return 1

    if sub == "manifest":
        from moor_cli.slack_cli import slack_manifest_command
...
def cmd_project(args):
    ...
    from moor_cli.projects_cmd import projects_command
...
def cmd_hooks(args):
    ...
    from moor_cli.hooks import hooks_command
...
def cmd_doctor(args):
    ...
    from moor_cli.doctor import run_doctor
...
def cmd_verify(args):
    ...
    from moor_cli.verify_cmd import run_verify_command

    sys.exit(run_verify_command(args))


def cmd_security(args):
    """Dispatch `moor security <subcmd>`."""
    sub = getattr(args, "security_command", None)
    if sub in ("audit", None):
        from moor_cli.security_audit import cmd_security_audit
...
def cmd_approvals(args):
    """Dispatch `moor approvals <subcmd>`."""
    from moor_cli.approvals_suggest import approvals_command
...
def cmd_dump(args):
    ...
    from moor_cli.dump import run_dump
...
def cmd_debug(args):
    ...
    from moor_cli.debug import run_debug
...
def cmd_config(args):
    ...
    from moor_cli.config import config_command
...
def cmd_skin(args):
    ...
    from moor_cli.skin_cmd import skin_command
...
def cmd_backup(args):
    ...
    if getattr(args, "quick", False):
        from moor_cli.backup import run_quick_backup

        run_quick_backup(args)
    else:
        from moor_cli.backup import run_backup
...
def _print_version_info(*, check_updates: bool = True) -> None:
    ...
    # `moor --version` pre-import fast path (the `version` subcommand
...
def cmd_uninstall(args):
    ...
    if getattr(args, "gui_summary", False):
        from moor_cli.gui_uninstall import gui_install_summary
    ...
    if getattr(args, "gui", False):
        ...
        from moor_cli.uninstall import run_gui_uninstall
    ...
    from moor_cli.uninstall import run_uninstall
...
# Update pipeline lives in moor_cli/update_cmd.py (main.py decomposition,
...
# monkeypatches on moor_cli.main.<name> keep resolving unchanged without
...
# validated against. Lives next to the checkout (NOT in MOOR_HOME) because
...
def _sweep_stale_bytecode_if_checkout_changed() -> None:
    ...
    ``moor update``, a manual ``git pull``, a ZIP update, a file-sync
    ...
    Update-time clears alone can never close this class: ``moor update``
    ...
    ``moor`` entry point compares the checkout fingerprint (cheap file
...
def _web_ui_build_needed(web_dir: Path) -> bool:
    ...
    mtime comparison. ``git checkout`` / ``git pull`` / ``moor update``
    ...
    ``moor_cli/web_dist/`` (per vite.config.ts outDir), NOT ``web/dist/``,
    ...
    dist_dir = project_root / "moor_cli" / "web_dist"
...
def _web_ui_stamp_path() -> Path:
    """Return the path to the web UI build stamp file under $MOOR_HOME."""
    from moor_constants import get_moor_home
    return get_moor_home() / "web-ui-build-stamp.json"
...
def _run_with_idle_timeout(
    ...
    *,
    ...
    ``moor`` launcher present but ``moor_cli`` not importable.
...
def _nixos_build_env() -> dict[str, str] | None:
    ...
    Two-tier resolution:
    1. Fast path — the moor venv's python3 (present in managed installs)
    ...
    # Tier 1: fast path — moor venv python3, no nix-shell overhead
    ...
    # a moor venv (pip / non-managed / bare-git installs).  The resolved
...
def _run_npm_install_deterministic(
    ...
    *,
    ...
    the working tree dirty and causes the next ``moor update`` to stash the
    lockfile — repeatedly.
    ...
    run_env["PATH"] = with_moor_node_path(run_env)["PATH"]
...
def _build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    project_root = web_dir.parent.parent if web_dir.parent.name == "apps" else web_dir.parent
    dist_index = project_root / "moor_cli" / "web_dist" / "index.html"
    try:
...
def _do_build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    Args:
        ...
        fatal: If True, print error guidance and return False on failure
               instead of a soft warning (used by ``moor web``).
    ...
    # build path usable on a stock `py -m moor_cli.main web` invocation.
    ...
    if not npm:
        ...
        return not fatal
    build_env = _npm_lifecycle_env(with_moor_node_path())
    ...
    # as `moor update`'s _update_node_dependencies() (ui-tui + web +
    ...
    if r1.returncode != 0:
        _say(
            ...
            + ("" if fatal else " (moor web will not be available)")
        )
        _relay(r1)
        if fatal:
...
#   - ``moor update`` can unconditionally call ``moor desktop --build-only``
#     and it will skip if nothing actually changed
#   - ``moor desktop`` (interactive launch) skips the build when the
#     stamp matches, making repeated launches fast
#
# Stamp file: $MOOR_HOME/desktop-build-stamp.json
# Schema:
#   {
...
def _desktop_stamp_path() -> Path:
    """Return the path to the desktop build stamp file under $MOOR_HOME."""
    from moor_constants import get_moor_home
    return get_moor_home() / "desktop-build-stamp.json"
...
def _renderer_bundle_torn(dist_dir: Path) -> bool:
    ...
    the intact SOURCE tree, ``moor desktop`` skips the rebuild that would fix
...
def _desktop_build_needed(desktop_dir: Path, project_root: Path, *, source_mode: bool) -> bool:
    ...
    ``moor update`` that pulled new source but hasn't built yet).
    """
...
#
# The desktop self-update chain (Desktop → moor-setup --update →
# `moor update` → `moor desktop --build-only` → relaunch) rebuilds
...
def _windows_native_machine() -> str:
    ...
    emulation: the desktop update chain runs an x64 moor-setup.exe (and thus
...
def _ensure_desktop_exe_launchable(
    ...
    if restored is not None:
        ...
        print("    Your existing version was kept and still works. Run `moor desktop`")
        print("    (or the in-app update) again to retry with a fresh Electron download.")
        return restored, True

    print("  ✗ No usable backup was found to restore.")
    print("    Run `moor desktop --force-build` to rebuild, or re-run the Moor")
...
def _redownload_electron_dist(
    ...
    *,
    ...
    dl_env = with_moor_node_path(env)
    if mirror:
...
def _force_adhoc_macos_signing(env: dict, *, source_mode: bool) -> bool:
    ...
    machine* (``moor desktop --build-only`` → electron-builder ``--dir``).
...
def _detect_linux_password_store() -> str | None:
    ...
    fails under `moor desktop` because the launcher environment doesn't look
...
def _desktop_launch_options() -> tuple[list[str], str, str, str]:
    ...
    is one of "auto"/"1"/"0" (normalized for the MOOR_DESKTOP_DISABLE_GPU
...
def _register_linux_desktop_entry() -> None:
    ...
    shell. ``moor uninstall --gui`` removes it.
    """
    try:
        from moor_cli.linux_desktop_entry import install_desktop_entry, is_supported
...
def cmd_gui(args: argparse.Namespace):
    ...
    try:
        from moor_logging import setup_logging as _setup_logging_gui
    ...
    if getattr(args, "fake_boot", False):
        env["MOOR_DESKTOP_BOOT_FAKE"] = "1"
    if getattr(args, "ignore_existing", False):
        env["MOOR_DESKTOP_IGNORE_EXISTING"] = "1"
    if getattr(args, "moor_root", None):
        env["MOOR_DESKTOP_MOOR_ROOT"] = str(Path(args.moor_root).expanduser().resolve())
    if getattr(args, "cwd", None):
        env["MOOR_DESKTOP_CWD"] = str(Path(args.cwd).expanduser().resolve())
    else:
        env["MOOR_DESKTOP_CWD"] = os.getcwd()
    ...
    # `MOOR_DESKTOP_DISABLE_GPU=... moor desktop` and
    # `ELECTRON_OZONE_PLATFORM_HINT=... moor desktop` keep working.
    config_electron_flags, config_disable_gpu, config_password_store, config_ozone_hint = (
        _desktop_launch_options()
    )
    if config_disable_gpu != "auto" and "MOOR_DESKTOP_DISABLE_GPU" not in os.environ:
        env["MOOR_DESKTOP_DISABLE_GPU"] = config_disable_gpu
    ...
    # `MOOR_DESKTOP_PASSWORD_STORE=... moor desktop` keeps working.
    if sys.platform == "linux" and "MOOR_DESKTOP_PASSWORD_STORE" not in os.environ:
        password_store = (
        ...
        if password_store:
            env["MOOR_DESKTOP_PASSWORD_STORE"] = password_store
    ...
    if source_mode or not skip_build:
        ...
        if not npm:
            ...
            print("Install Node.js, then run:  moor gui")
            sys.exit(1)
    else:
        npm = None
    ...
    if skip_build:
        ...
        if not build_needed:
            ...
            # Put the moor-managed Node on PATH so npm's child scripts (which
            # shell out to bare `node`, e.g. electron-winstaller's
            # select-7z-arch.js) resolve it even when the parent PATH is
            # stripped — the desktop updater chain (Desktop → moor-setup →
            # moor update) loses shell PATH customizations. Wrapping the
            # NixOS build env keeps its PYTHON hint while restoring managed Node
            # ahead of a bare PATH (same idiom as the `moor update` path).
            nixos_env = with_moor_node_path(_nixos_build_env())
            ...
            if build_result.returncode != 0:
                ...
                print("    ELECTRON_MIRROR=<mirror-base-url> moor desktop --force-build")
...
# Dashboard process-hygiene helpers live in moor_cli/dashboard_procs.py
...
# moor_cli.main.<name> keep resolving unchanged.
...
def _parse_dashboard_runtime(command: str) -> tuple[str, str, int] | None:
    ...
    if any(
        ...
        for pattern in (
            "moor dashboard",
            "moor_cli.main dashboard",
            "moor_cli/main.py dashboard",
        )
    ):
        mode = "dashboard"
    elif any(
...
_DASHBOARD_SYSTEMD_UNIT = "moor-dashboard.service"
...
def _get_systemd_service_for_pid(pid: int) -> str | None:
    ...
    ``moor-serve.service``).  Returns ``None`` when the PID is not
    ...
    try:
        ...
        for line in text.splitlines():
            line = line.strip()
            # Format: 0::/system.slice/moor-serve.service
...
def _dashboard_cmdline_for_pid(pid: int) -> list[str] | None:
    ...
    is reconstructed, but moor launch commands don't embed exotic args).
...
def _respawn_dashboard_processes(commands: list[list[str]]) -> list[list[str]]:
    """Best-effort respawn of manually-started dashboards after ``moor update``.
    ...
    log_path = get_moor_home() / "logs" / "dashboard-restart.log"
    try:
...
# =========================================================================
# Fork detection and upstream management for `moor update`
# =========================================================================
...
def _recover_from_interrupted_install() -> None:
    """Finish update work left half-done by a prior ``moor update``.
    ...
    protocol stream (``moor acp`` speaks JSON-RPC on stdout) must never get
...
def _recover_core_update_marker_locked() -> None:
    ...
    """
    print(
        "⚠ A previous `moor update` was interrupted mid-install — "
        "finishing dependency installation now..."
    )

    # Windows: a normal ``moor.exe`` launch always has the launcher as an
    ...
    self_locked = _windows_running_moor_launcher_locked()
    if self_locked:
        install_prefix, install_env = _default_venv_install_target()
        print(
            "  → Running from moor.exe; applying package-only first aid, "
    ...
    try:
        from moor_cli import _install_repair as _ir
...
def _windows_shim_in_process_chain() -> Path | None:
    ...
    ``venv\\Scripts\\moor.exe`` is a launcher that runs the interpreter with
    ...
    ``moor ...`` command holds its own shim, and an editable install run
    ...
    ``moor.exe`` belonging to some other install never matches.
    """
    if not _is_windows():
        return None
    ...
    if scripts_dir is None:
        return None
    shims = {_norm_exe_path(shim): shim for shim in _moor_exe_shims(scripts_dir)}
    if not shims:
        return None
...
def _windows_running_moor_launcher_locked() -> bool:
    """True when a venv ``moor*.exe`` shim is this process or an ancestor.
...
def _reexec_dependency_sync_off_windows_shim() -> bool:
    ...
    The child re-runs ``moor update``, so the whole remaining flow — the
    ...
    cmd = [str(python_exe), "-m", "moor_cli.main", *sys.argv[1:]]
...
def _run_install_with_heartbeat(
    ...
    *,
    ...
    know ``moor update`` is still progressing even if pip/uv itself is silent.
    """
...
def _venv_scripts_dir() -> Path | None:
    ...
    from moor_constants import project_venv_dir, venv_bin_dir
...
def _moor_exe_shims(scripts_dir: Path) -> list[Path]:
...
def _quarantine_running_moor_exe(
    ...
    """Pre-empt Windows file lock on the running ``moor.exe``.
    ...
    when ``moor update`` runs, ``moor.exe`` IS the live process, and uv
    fails with ``Access is denied. (os error 5)``.

    We rename live shims to ``moor.exe.old.<unix-ms>`` first. uv then writes
    fresh shims at the original paths. The ``.old`` files are cleaned up on
    the next moor invocation by ``_cleanup_quarantined_exes``.
    ...
    2. If all retries fail, print a clear warning naming the most likely
       culprit (running Moor Desktop / gateway / REPL).

    The updater's own launcher is no longer one of those culprits: an update
    started from ``moor.exe`` re-runs itself under the venv Python before
    ...
    for shim in _moor_exe_shims(scripts_dir):
        if not shim.exists():
            continue
        ...
        print(
            "    Close Moor Desktop, exit other `moor` REPLs, stop the "
            "gateway, or pause AV scanning, then re-run `moor update`."
        )
...
def _cleanup_pending_shim_renames(scripts_dir: Path) -> int:
    """Drop reboot renames older Moor versions queued for our shims.

    Moor used to fall back to ``MoveFileExW(MOVEFILE_DELAY_UNTIL_REBOOT)``
    ...
    try:
        ...
        with winreg.OpenKey(
            ...
            kept, removed = _filter_pending_shim_renames(
                entries, _moor_exe_shims(scripts_dir)
            )
            if not removed:
                return 0
            if kept:
...
def _restore_quarantined_exes(moved: list[tuple[Path, Path]]) -> None:
    """Roll back ``_quarantine_running_moor_exe`` if uv didn't write replacements.

    This is the safety-critical direction. A failed *quarantine* only aborts an
    update; a failed *restore* leaves the install with no ``moor`` on PATH,
...
class ShimQuarantineError(RuntimeError):
    """A live ``moor*.exe`` shim could not be renamed aside (#87331).
...
def _run_quarantined_install(
    ...
    *,
    ...
    """Run an editable install, quarantining the running ``moor.exe`` first.

    Any ``pip install -e .`` (or ``--reinstall``) rewrites the entry-point
    shims, and on Windows the live ``moor.exe`` is the running process —
    pip can neither delete nor overwrite it, so without quarantine the shim
    is left missing and ``moor`` drops off PATH. This wraps
    ...
    if scripts_dir is not None:
        moved = _quarantine_running_moor_exe(scripts_dir, failed_out=failed)
    ...
    try:
        ...
        # quarantined aside and `moor` missing from PATH after a green
...
def _quarantine_stamp_ms(stale: Path) -> int | None:
    ...
    :func:`_quarantine_running_moor_exe`. We neither rescue nor delete those:
...
def _cleanup_quarantined_exes(scripts_dir: Path | None = None) -> None:
    """Sweep — and where necessary RESCUE — ``moor.exe.old.*`` from updates.

    Called early on every moor invocation. Two cases the old unconditional
    ``unlink()`` got wrong, both ending with ``moor`` gone from PATH:

    1. **Orphan rescue.** If ``moor.exe`` is missing while
       ``moor.exe.old.*`` is present, that .old file is the ONLY surviving
    ...
    2. **Concurrency.** A fresh quarantine file may belong to an update in
       ...
       ``moor update`` does exactly this). Leave anything inside the grace
...
def _run_package_only_install(
    ...
    *,
    ...
    rewrite ``moor.exe``. The editable-install quarantine path would rename
...
def _repair_venv_via_import_probes(
    ...
    *,
    ...
    detected (#57828). Package-only reinstall — never rewrites ``moor.exe``.
    ...
    print("  ⚠ Venv repair incomplete. Run manually, then `moor update`:")
    print(
...
def _install_python_dependencies_with_optional_fallback(
    ...
    *,
    ...
    On Windows, pre-renames live ``moor.exe`` / ``moor-gateway.exe`` shims
    in the venv Scripts dir before each install attempt so uv can write fresh
    copies (Windows blocks REPLACE on a running .exe but allows RENAME). See
    ``_quarantine_running_moor_exe`` for the rationale.
    ...
    if (
        ...
        # Quarantining the wrong dir means the running moor.exe stays locked
    ...
    # hours later inside ``moor update``'s desktop-rebuild or skill-sync
...
def _verify_console_scripts_installed(
    ...
    *,
    ...
    On Windows, ``uv pip install -e .`` can register ``moor.exe`` in the
    wheel RECORD while the file never lands on disk — typically when the live
    ``moor.exe`` shim is locked during ``moor update``, or when uv/distlib
    skips a launcher write. The symptom is ``moor-agent.exe`` and
    ``moor-acp.exe`` present but ``moor.exe`` missing, so ``moor`` drops
    ...
    try:
        ...
        print(
            "  ⚠ Entry point repair failed; try `moor update --force` after "
            "closing other moor processes."
        )
        return
    ...
    if still_missing:
        print(
            ...
            "Workaround: python -m moor_cli.main <command>"
        )
    else:
...
def _verify_core_dependencies_installed(
    ...
    *,
    ...
    # outer Python that drove ``moor update``, not the venv we just wrote
    ...
    #
    # Quarantine the running ``moor.exe`` first: ``--reinstall -e .``
    # rewrites the entry-point shims, and on Windows pip can't overwrite the
    # live launcher, which would leave ``moor`` off PATH.
    scripts_dir = _venv_scripts_dir() if _is_windows() else None
    ...
    try:
        ...
        print("  ⚠ Repair install failed; check `moor update` output above.")
        return
    ...
    try:
        ...
        print(
            ...
            "Run `moor update --force` after closing other moor processes."
        )
        return
...
class _UpdateOutputStream:
    """Stream wrapper used during ``moor update`` to survive terminal loss.

    Wraps the process's original stdout/stderr so that:

    * Every write is also mirrored to an append-only log file
      (``~/.moor/logs/update.log``) that users can inspect after the
    ...
    * Writes to the original stream that fail with ``BrokenPipeError`` /
    ...
    ``_install_hangup_protection``, this makes ``moor update`` safe to
...
def _install_hangup_protection(gateway_mode: bool = False):
    ...
    Users commonly run ``moor update`` in an SSH session or a terminal
    ...
    2. ``sys.stdout`` / ``sys.stderr`` are wrapped to mirror output to
       ``~/.moor/logs/update.log`` and to silently absorb
    ...
    **intentionally left alone** — those are legitimate cancellation
    ...
    In gateway mode (``moor update --gateway``) the update is already
    ...
    try:
        ...
        # moor_cli.config.get_moor_home to simulate setup failure.
        from moor_cli.config import get_moor_home as _get_moor_home

        logs_dir = _get_moor_home() / "logs"
...
def cmd_update(args):
    ...
    if getattr(args, "plan", False):
        ...
        # Moor runtime across profiles, its supervisor, and its running
    ...
    if getattr(args, "check", False):
        ...
        # what a subsequent `moor update --branch=<x>` would actually pull.
    ...
    from moor_cli.update_lock import (
...
def _coalesce_session_name_args(argv: list) -> list:
    ...
    When a user types ``moor -c Pokemon Agent Dev`` without quoting the
...
def cmd_profile(args):
    ...
    if action is None:
        # Bare `moor profile` — show current profile status
        from moor_cli.profiles import format_profile_label

        profile_name = get_active_profile_name()
        dhh = display_moor_home()
        ...
        if current is not None:
            ...
            if p.alias_path:
                ...
                print(f"Alias:          {alias_display} → moor -p {p.name}")
        print()
        return
    ...
    if action == "list":
        ...
        try:
            ...
            if name == "default":
                print("Switched to: default (~/.moor)")
            else:
        ...
        try:
            ...
            if not no_alias:
                ...
                if collision:
                    ...
                    print(
                        f"  Choose a custom alias:  moor profile alias {name} --name <custom>"
                    )
                    print(f"  Or access via flag:     moor -p {name} chat")
                else:
        ...
        from moor_cli import profiles as _profiles_mod
        ...
        if name and not text_value and not auto_flag:
            try:
                if _profiles_mod.normalize_profile_name(name) == "default":
                    from moor_constants import get_moor_home as _hh
        ...
        from moor_cli import profile_describer as _pd
        ...
        if dist_name:
            ...
            print(f"  (run `moor profile info {name}` for full manifest)")
        if alias_name:
            is_windows = sys.platform == "win32"
            wrapper = _get_wrapper_dir() / (f"{alias_name}.bat" if is_windows else alias_name)
            print(f"Alias:   {alias_name} → moor -p {name}  ({wrapper})")
        print()
        ...
        from moor_cli.profiles import profile_exists, validate_alias_name
        ...
        from moor_cli.profiles import normalize_profile_name, rename_profile
        ...
        from moor_cli.profiles import import_profile
        ...
        try:
            ...
            with tempfile.TemporaryDirectory(prefix="moor_dist_preview_") as tmp:
                plan = plan_install(
                    args.source,
                    Path(tmp),
            ...
            if plan.has_cron:
                print(
                    ...
                    f"  Review them with:  moor -p {plan.manifest.name} cron list"
                )
            print(f"\n  Use with:      moor -p {plan.manifest.name} chat")
        ...
        from moor_cli.profiles import get_profile_dir, normalize_profile_name
        ...
        try:
            ...
            if current is None:
                print(
                    ...
                    "Only profiles installed via `moor profile install` can be updated."
                )
                sys.exit(1)
            ...
            if plan.has_cron:
                print(
                    ...
                    f"moor -p {plan.manifest.name} cron list"
                )
        except (DistributionError, ValueError) as e:
            print(f"Error: {e}")
            sys.exit(1)
        ...
        from moor_cli.profile_distribution import describe_distribution, DistributionError
        ...
        if data.get("moor_requires"):
            print(f"Requires:     Moor {data['moor_requires']}")
        if data.get("source"):
...
def _render_distribution_plan(plan) -> None:
    ...
    from moor_cli.profile_distribution import MANIFEST_FILENAME
    mf = plan.manifest
    ...
    if mf.moor_requires:
        print(f"  Requires: Moor {mf.moor_requires}")
...
def _report_dashboard_status() -> int:
    ...
    print(f"{len(live)} moor dashboard process(es) running:")
...
def _maybe_setup_dashboard_auth_interactively(args) -> None:
    ...
    provider on the spot — or point them at ``moor dashboard register`` for
    OAuth.
    ...
    try:
        from moor_cli.web_server import should_require_dashboard_auth
    ...
    try:
        from moor_cli.dashboard_auth import list_providers
        if list_providers():
    ...
    print("    [2] OAuth via Moor Portal (run `moor dashboard register`)")
    print("    [3] Cancel")
    print()
    ...
    try:
        from moor_cli.config import load_config, save_config
        from moor_cli.plugins_cmd import ensure_basic_auth_plugin_enabled_in_config
...
def _read_ssh_session_token_file(path: str) -> str:
    ...
    if sys.platform == "win32":
        from moor_cli.windows_ssh_runtime import read_token
    ...
    # The Desktop client writes the token under $HOME/.moor/desktop-ssh: a
    # literal "~/.moor/desktop-ssh" in apps/desktop/electron/remote-lifecycle.ts
    # expanded against the account's $HOME, independent of MOOR_HOME and the
    # active profile. Anchor validation to that same OS-home path, NOT to
    # get_moor_home(): a non-default sticky profile (or any MOOR_HOME pointing
    # elsewhere, e.g. a Docker /opt/data root) re-homes get_moor_home() and
    # would otherwise reject every token the client legitimately wrote (#69551).
    token_root = _Path.home() / ".moor" / "desktop-ssh"
    try:
...
def _is_electron_packaged_web_dist(path: str) -> bool:
    ...
    Packaged Desktop sets ``MOOR_WEB_DIST`` to ``.../app.asar/dist`` or
    ``.../app.asar.unpacked/dist``. A standalone ``moor dashboard`` that
...
def cmd_dashboard(args):
    ...
    if _token_file and not _headless_backend:
        raise SystemExit("--ssh-session-token-file is only valid with moor serve")

    # ── Sanitize Desktop-inherited env that hijacks a standalone launch ─
    # Desktop Electron spawns its backend with MOOR_DESKTOP=1 plus
    # MOOR_WEB_DIST=<packaged app.asar[/unpacked]/dist> (and often
    # MOOR_SERVE_HEADLESS=1 on the serve path). A shell that inherits
    # those vars then runs `moor dashboard` would otherwise:
    #   - serve the desktop renderer → "Desktop IPC bridge is unavailable"
    #     (issue #52945), or
    #   - disable the SPA via inherited MOOR_SERVE_HEADLESS.
    # Only strip Electron-packaged WEB_DIST contamination — caller-managed
    # MOOR_WEB_DIST overrides (dev / custom builds) must still work.
    # The desktop-spawned backend itself (MOOR_DESKTOP=1) keeps its dist.
    # Intentionally headless `serve` re-sets MOOR_SERVE_HEADLESS below.
    if os.environ.get("MOOR_DESKTOP") != "1":
        _inherited_web_dist = os.environ.get("MOOR_WEB_DIST", "")
    ...
    # launches the dashboard (`worker dashboard` → MOOR_HOME points into
    ...
    if (
        ...
        and os.environ.get("MOOR_DESKTOP") != "1"
    ):
        ...
        reexec_argv = [
            sys.executable, "-m", "moor_cli.main",
            "-p", "default",
        ...
        from tools.environments.local import build_subprocess_env
        # Exact env preservation: MOOR_HOME is explicitly pinned to the
        ...
        # MOOR_HOME.  We must resolve the root explicitly instead of just
        # dropping MOOR_HOME: in the Docker layout the machine root is
        # /opt/data (set via `ENV MOOR_HOME=/opt/data`), so an unset
        # MOOR_HOME falls back to $HOME/.moor = /opt/data/.moor — an
        ...
        # update-button guard also misfires).  get_default_moor_root()
        # returns the root for both layouts: ~/.moor for a standard install
        ...
        try:
            from moor_constants import get_default_moor_root
            env["MOOR_HOME"] = str(get_default_moor_root())
        except Exception:
            # Best-effort: if root resolution fails, fall back to the prior
            # behaviour (drop MOOR_HOME) rather than block the reroute.
    ...
    if _headless_backend:
        ...
        os.environ["MOOR_SERVE_HEADLESS"] = "1"
    elif "MOOR_WEB_DIST" not in os.environ and not getattr(args, "skip_build", False):
        ...
        _dist_root = (
            Path(os.environ["MOOR_WEB_DIST"])
            if "MOOR_WEB_DIST" in os.environ
            else PROJECT_ROOT / "moor_cli" / "web_dist"
        )
        if not (_dist_root / "index.html").exists():
            ...
            # default dist location is recoverable: a custom MOOR_WEB_DIST
            # points at a caller-managed directory the build cannot populate.
            _recoverable = "MOOR_WEB_DIST" not in os.environ
            if _recoverable:
        ...
        # MOOR_WEB_DIST is set without --skip-build: the build is skipped
        ...
        _dist_root = Path(os.environ["MOOR_WEB_DIST"]).expanduser()
        if not (_dist_root / "index.html").exists():
            print(f"✗ MOOR_WEB_DIST is set but no web dist found at: {_dist_root}")
            print("  Pre-build first:  npm install --workspace web && npm run build -w web")
            print("  Or unset MOOR_WEB_DIST to build and use the default web UI dist.")
            sys.exit(1)
        # Write the expanded path back: web_server reads MOOR_WEB_DIST raw
        # at import (no expanduser), so a validated "~/dist" would otherwise
        # pass here and still 404 there.
        os.environ["MOOR_WEB_DIST"] = str(_dist_root)
        print(f"→ Using web dist from MOOR_WEB_DIST: {_dist_root}")

    # Discover and load plugins so any DashboardAuthProvider plugin
    # (e.g. plugins/dashboard_auth/moor) registers BEFORE start_server's
...
def cmd_dashboard_register(args):
    """Register a self-hosted dashboard OAuth client with Moor Portal."""
    from moor_cli.dashboard_register import cmd_dashboard_register as _impl
...
def cmd_gateway_enroll(args):
    ...
    from moor_cli.gateway_enroll import cmd_gateway_enroll as _impl
...
def cmd_completion(args, parser=None):
    ...
    from moor_cli.completion import generate_bash, generate_zsh, generate_fish
...
def cmd_prompt_size(args):
    ...
    from moor_cli.prompt_size import cmd_prompt_size as _impl
...
def cmd_logs(args):
    ...
    from moor_cli.logs import tail_log, list_logs
...
def cmd_console(args):
    """Open the safe Moor command console."""
    from moor_cli.console_engine import run_console_repl
...
def _build_provider_choices() -> list[str]:
    ...
    try:
        from moor_cli.models import CANONICAL_PROVIDERS as _cp
        return ["auto"] + [p.slug for p in _cp]
    except Exception:
        # Fallback: static list guarantees the CLI always works
        return [
            "auto", "openrouter", "moor", "openai-codex", "xai-oauth", "copilot-acp", "copilot",
...
def _first_positional_argv() -> str | None:
    ...
    ``moor -m gpt5 --provider openai chat "msg"`` by skipping the
...
def _plugin_cli_discovery_needed() -> bool:
    ...
    if first is None:
        # Bare ``moor`` or only flags → defaults to ``chat``.
        return False
...
def _resolve_deferred_platform_cli_command(command_name: str | None) -> None:
    ...
    registers a top-level ``moor <name>`` command (e.g. Photon ->
    ...
    import it, so the CLI registration never happens and ``moor photon``
...
def _is_tui_chat_launch(args) -> bool:
    return bool(getattr(args, "tui", False) or os.environ.get("MOOR_TUI") == "1")
...
def _prepare_agent_startup(args) -> None:
    ...
    # --yolo: chokepoint guarantee that MOOR_YOLO_MODE is set before ANY
...
def _try_fast_chat_launch() -> bool:
    ...
    ``moor`` / ``moor -w -s foo --yolo`` / ``moor chat`` don't need the
    ...
    """
    if os.environ.get("MOOR_DISABLE_FAST_CHAT_LAUNCH") == "1":
        return False
    argv = sys.argv[1:]
    ...
    try:
        from moor_cli.config import get_container_exec_info
...
def _try_termux_fast_cli_launch() -> bool:
    ...
    if not _is_termux_startup_environment():
        return False
    if os.environ.get("MOOR_TERMUX_DISABLE_FAST_CLI") == "1":
        return False
    ...
    if args.command in {None, "chat"}:
        ...
        if interactive_prompt:
            ...
            os.environ["MOOR_DEFER_AGENT_STARTUP"] = "1"
            os.environ["MOOR_FAST_STARTUP_BANNER"] = "1"
...
def _try_termux_fast_tui_launch() -> bool:
    ...
    `moor --tui` is the hot path on phones. The full parser setup imports
...
def cmd_memory(args):
    ...
    if sub == "off":
        ...
        if not existing:
            print(
                f"\n  Nothing to reset — no memory files found in {display_moor_home()}/memories/\n"
            )
            return
        ...
        print(f"  Files were in: {display_moor_home()}/memories/\n")
    else:
        from moor_cli.memory_setup import memory_command
...
def cmd_tools(args):
    ...
    if action in {"list", "disable", "enable"}:
        from moor_cli.tools_config import tools_disable_enable_command

        tools_disable_enable_command(args)
    elif action == "post-setup":
        from moor_cli.tools_config import run_post_setup_command
...
def cmd_monitoring(args):
    ...
    if action == "status":
        ...
        print(f"  OTel SDK:       {'installed' if otlp_exporter.is_available() else 'not installed'} "
              f"(optional extra: moor-agent[otlp])")
...
def cmd_skills(args):
    ...
    if getattr(args, "skills_action", None) == "config":
        ...
        from moor_cli.skills_config import skills_command as skills_config_command

        skills_config_command(args)
    elif getattr(args, "skills_action", None) in ("trust", "untrust"):
        _cmd_skills_trust(args)
    else:
        from moor_cli.skills_hub import skills_command

        skills_command(args)


def _cmd_skills_trust(args):
    """``moor skills trust [path]`` / ``moor skills untrust [path]``.
...
def cmd_pairing(args):
    from moor_cli.pairing import pairing_command
...
def cmd_mcp(args):
    from moor_cli.mcp_config import mcp_command

    mcp_command(args)


def cmd_claw(args):
    from moor_cli.claw import claw_command
...
def _advertise_agent_env() -> None:
    ...
    (``moor-agent`` in huggingface.js ``agent-harnesses.ts``): standard-var
    matching is exact, so any other value is counted as "unknown".
    ``MOOR_AGENT`` is the moor-specific marker. setdefault: never
    clobber an outer harness (e.g. Moor running inside another agent's
    terminal).
    """
    os.environ.setdefault("AI_AGENT", "moor-agent")
    os.environ.setdefault("MOOR_AGENT", "true")


def main():
    """Main entry point for moor CLI."""
    # Cosmetic: make the process show up as 'moor' instead of 'python3.11'
    ...
    try:
        from moor_cli.stdio import configure_windows_stdio
        configure_windows_stdio()
    except Exception:
        pass

    # Sweep stale ``moor.exe.old.*`` quarantine files left by previous
    # ``moor update`` runs on Windows. Silent no-op on non-Windows or when
    # there's nothing to clean. See ``_quarantine_running_moor_exe``.
    try:
        _cleanup_quarantined_exes()
    except Exception:
        pass

    # If the checkout changed since the last launch (moor update, manual
    ...
    # Self-heal a venv left half-built by an interrupted ``moor update``
    ...
    # ``moor skills install update``) merely defers recovery one launch;
    # under-matching (missing ``moor -p work update``) would race a recovery
    ...
    # =========================================================================
    # model command  (parser built in moor_cli/subcommands/model.py)
    # =========================================================================
    build_model_parser(subparsers, cmd_model=cmd_model)

    from moor_cli.moa_cmd import cmd_moa
    ...
    # =========================================================================
    from moor_cli.fallback_cmd import cmd_fallback
    ...
    fallback_subparsers.add_parser(
        "add",
        help="Pick a provider + model (same picker as `moor model`) and append to the chain",
    )
    fallback_subparsers.add_parser(
        "remove",
        aliases=["rm"],
    ...
    worktree_parser = subparsers.add_parser(
        ...
        description=(
            "Attended reclaim for the .worktrees/ directory moor -w sessions "
            "accumulate. Never deletes uncommitted tracked changes, unique "
            "unpushed commits, or in-use trees; untracked-only scratch is "
            "archived to ~/.moor/archive/worktree-prune/ before removal. See: "
    ...
    def _dispatch_worktree(_args):
        from moor_cli.worktree_cmd import cmd_worktree
    ...
    secrets_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "instead of storing them in ~/.moor/.env.  Supports Bitwarden "
    ...
    from moor_cli import secrets_cli as _secrets_cli
    from moor_cli import onepassword_secrets_cli as _op_secrets_cli
    ...
    # `moor proxy` (defined elsewhere in this file) is a separate INBOUND
    ...
    from moor_cli import proxy_cli as _proxy_cli
    ...
    def _dispatch_egress(args):  # noqa: ANN001
        ...
        # from the inbound OAuth ``moor proxy`` subparser (dest='proxy_command').
    ...
    # migrate command
    # =========================================================================
    from moor_cli.migrate import cmd_migrate, cmd_migrate_xai
    ...
    # =========================================================================
    # gateway + proxy commands  (parsers built in moor_cli/subcommands/gateway.py)
    # =========================================================================
    ...
    # =========================================================================
    # setup command  (parser built in moor_cli/subcommands/setup.py)
    # =========================================================================
    build_setup_parser(subparsers, cmd_setup=cmd_setup)


    # =========================================================================
    # whatsapp command  (parser built in moor_cli/subcommands/whatsapp.py)
    # =========================================================================
    ...
    whatsapp_cloud_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "required). Distinct from `moor whatsapp` which sets up "
    ...
    # =========================================================================
    # slack command  (parser built in moor_cli/subcommands/slack.py)
    # =========================================================================
    ...
    # =========================================================================
    from moor_cli.send_cmd import register_send_subparser
    register_send_subparser(subparsers)

    # =========================================================================
    # login command  (parser built in moor_cli/subcommands/login.py)
    # =========================================================================
    build_login_parser(subparsers, cmd_login=cmd_login)

    # =========================================================================
    # logout command  (parser built in moor_cli/subcommands/logout.py)
    # =========================================================================
    build_logout_parser(subparsers, cmd_logout=cmd_logout)

    # =========================================================================
    # auth command  (parser built in moor_cli/subcommands/auth.py)
    # =========================================================================
    build_auth_parser(subparsers, cmd_auth=cmd_auth)

    # =========================================================================
    # status command  (parser built in moor_cli/subcommands/status.py)
    # =========================================================================
    build_status_parser(subparsers, cmd_status=cmd_status)

    # =========================================================================
    # pause / resume commands  (parser built in moor_cli/subcommands/pause.py)
    # =========================================================================
    build_pause_parser(subparsers)

    # =========================================================================
    # cron command  (parser built in moor_cli/subcommands/cron.py)
    # =========================================================================
    build_cron_parser(subparsers, cmd_cron=cmd_cron)
    build_sync_parser(subparsers, cmd_sync=cmd_sync)

    # =========================================================================
    # webhook command  (parser built in moor_cli/subcommands/webhook.py)
    # =========================================================================
    build_webhook_parser(subparsers, cmd_webhook=cmd_webhook)

    # =========================================================================
    # peer command — bot-to-bot DMs across machines (peer Moor gateways)
    # =========================================================================
    from moor_cli.subcommands.peer import build_peer_parser

    build_peer_parser(subparsers)

    # =========================================================================
    # portal command — Moor Portal status + Tool Gateway routing
    # =========================================================================
    from moor_cli.portal_cli import add_parser as _add_portal_parser
    _add_portal_parser(subparsers)

    # =========================================================================
    # kanban command — multi-profile collaboration board
    # =========================================================================
    from moor_cli.kanban import build_parser as _build_kanban_parser
    ...
    # =========================================================================
    from moor_cli.projects_cmd import build_parser as _build_project_parser
    ...
    # =========================================================================
    # hooks command  (parser built in moor_cli/subcommands/hooks.py)
    # =========================================================================
    build_hooks_parser(subparsers, cmd_hooks=cmd_hooks)

    # =========================================================================
    # doctor command  (parser built in moor_cli/subcommands/doctor.py)
    # =========================================================================
    build_doctor_parser(subparsers, cmd_doctor=cmd_doctor)

    # =========================================================================
    # verify command  (parser built in moor_cli/subcommands/verify.py)
    # =========================================================================
    ...
    # =========================================================================
    # security command  (parser built in moor_cli/subcommands/security.py)
    # =========================================================================
    build_security_parser(subparsers, cmd_security=cmd_security)

    # =========================================================================
    # approvals command  (parser built in moor_cli/subcommands/approvals.py)
    # =========================================================================
    build_approvals_parser(subparsers, cmd_approvals=cmd_approvals)

    # =========================================================================
    # dump command  (parser built in moor_cli/subcommands/dump.py)
    # =========================================================================
    build_dump_parser(subparsers, cmd_dump=cmd_dump)

    # =========================================================================
    # debug command  (parser built in moor_cli/subcommands/debug.py)
    # =========================================================================
    build_debug_parser(subparsers, cmd_debug=cmd_debug)

    # =========================================================================
    # backup command  (parser built in moor_cli/subcommands/backup.py)
    # =========================================================================
    ...
    checkpoints_parser = subparsers.add_parser(
        "checkpoints",
        help="Inspect / prune / clear ~/.moor/checkpoints/",
    ...
    # =========================================================================
    # import command  (parser built in moor_cli/subcommands/import_cmd.py)
    # =========================================================================
    build_import_cmd_parser(subparsers, cmd_import=cmd_import)

    # =========================================================================
    # import-agent command  (parser: moor_cli/subcommands/import_agent.py)
    # =========================================================================
    def cmd_import_agent(args):
        from moor_cli.agent_import import import_agent_command
        import_agent_command(args)

    build_import_agent_parser(subparsers, cmd_import_agent=cmd_import_agent)

    # =========================================================================
    # config command  (parser built in moor_cli/subcommands/config.py)
    # =========================================================================
    build_config_parser(subparsers, cmd_config=cmd_config)

    # =========================================================================
    # skin command  (parser built in moor_cli/subcommands/skin.py)
    # =========================================================================
    build_skin_parser(subparsers, cmd_skin=cmd_skin)

    # =========================================================================
    # console command  (parser built in moor_cli/subcommands/console.py)
    # =========================================================================
    build_console_parser(subparsers, cmd_console=cmd_console)

    # =========================================================================
    # pairing command  (parser built in moor_cli/subcommands/pairing.py)
    # =========================================================================
    build_pairing_parser(subparsers, cmd_pairing=cmd_pairing)

    # =========================================================================
    # skills command  (parser built in moor_cli/subcommands/skills.py)
    # =========================================================================
    ...
    from moor_cli.bundles import register_cli as _bundles_register, bundles_command
    _bundles_register(bundles_parser)
    bundles_parser.set_defaults(func=bundles_command)

    # =========================================================================
    # plugins command  (parser built in moor_cli/subcommands/plugins.py)
    # =========================================================================
    ...
    # subcommand — ``moor --help``, ``moor logs``,
    ...
    try:
        from moor_cli.curator import register_cli as _register_curator_cli
    ...
    pets_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "and Moor shows it reacting to agent activity across the CLI, "
            "TUI, and desktop app."
        ),
    )
    try:
        from moor_cli.pets import register_cli as _register_pets_cli
    ...
    try:
        from moor_cli.journey import register_cli as _register_journey_cli
    ...
    # =========================================================================
    # memory command  (parser built in moor_cli/subcommands/memory.py)
    # =========================================================================
    build_memory_parser(subparsers, cmd_memory=cmd_memory)

    # =========================================================================
    # tools command  (parser built in moor_cli/subcommands/tools.py)
    # =========================================================================
    ...
    computer_use_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Use `moor computer-use doctor` to run cua-driver's\n"
    ...
    computer_use_perms = computer_use_sub.add_parser(
        ...
        description=(
            ...
            "terminal or the Moor app. `status` reports the driver's grant\n"
    ...
    def cmd_computer_use(args):
        ...
        if action == "status":
            ...
            if path:
                ...
                # to repair an install that `moor computer-use install` will
                ...
                if not contract.get("ready"):
                    ...
                    if override:
                        print(
                            "    Update the binary selected by MOOR_CUA_DRIVER_CMD, or unset "
                            "the override and run: moor computer-use install --upgrade"
                        )
                    else:
        ...
        if action == "permissions":
            ...
            if perms_action == "status":
                ...
                if not st["platform_supported"]:
                    ...
                    sys.exit(1)
                if not st["installed"]:
                    print("cua-driver: not installed. Run: moor computer-use install")
                    sys.exit(1)
    ...
    # =========================================================================
    # mcp command  (parser built in moor_cli/subcommands/mcp.py)
    # =========================================================================
    ...
    sessions_export.add_argument(
        ...
        help=(
            ...
            "md/qmd: output directory (default: <moor home>/session-exports)"
        ),
    )
    sessions_export.add_argument(
        "--format",
    ...
    sessions_import = sessions_subparsers.add_parser(
        ...
        description=(
            ...
            "so it can be resumed with 'moor --resume <id>'. The foreign "
    ...
    # cmd_sessions lives in moor_cli/sessions_cmd.py (main.py decomposition).
    ...
    # and lets monkeypatches on moor_cli.main.cmd_sessions keep working.
    ...
    # =========================================================================
    # insights command  (parser built in moor_cli/subcommands/insights.py)
    # =========================================================================
    build_insights_parser(subparsers, cmd_insights=cmd_insights)
    build_monitoring_parser(subparsers, cmd_monitoring=cmd_monitoring)

    # =========================================================================
    # claw command  (parser built in moor_cli/subcommands/claw.py)
    # =========================================================================
    build_claw_parser(subparsers, cmd_claw=cmd_claw)

    # NOTE: the `moor version` subcommand was removed — `moor --version`
    # / `-V` now carries the full output including update status.

    # =========================================================================
    # update command  (parser built in moor_cli/subcommands/update.py)
    # =========================================================================
    build_update_parser(subparsers, cmd_update=cmd_update)

    # =========================================================================
    # uninstall command  (parser built in moor_cli/subcommands/uninstall.py)
    # =========================================================================
    build_uninstall_parser(subparsers, cmd_uninstall=cmd_uninstall)

    # =========================================================================
    # acp command  (parser built in moor_cli/subcommands/acp.py)
    # =========================================================================
    build_acp_parser(subparsers, cmd_acp=cmd_acp)

    # =========================================================================
    # profile command  (parser built in moor_cli/subcommands/profile.py)
    # =========================================================================
    ...
    # =========================================================================
    # dashboard command  (parser built in moor_cli/subcommands/dashboard.py)
    # =========================================================================
    ...
    # run `moor desktop` from a terminal, so the canonical name needs
    # to be the one that appears in --help (argparse promotes the primary
    # name; aliases stay hidden).
    # =========================================================================
    # gui command  (parser built in moor_cli/subcommands/gui.py)
    # =========================================================================
    build_gui_parser(subparsers, cmd_gui=cmd_gui)

    # =========================================================================
    # logs command  (parser built in moor_cli/subcommands/logs.py)
    # =========================================================================
    build_logs_parser(subparsers, cmd_logs=cmd_logs)

    # =========================================================================
    # prompt-size command  (parser built in moor_cli/subcommands/prompt_size.py)
    # =========================================================================
    ...
    # e.g. ``moor -c Pokemon Agent Dev`` → ``moor -c 'Pokemon Agent Dev'``
    ...
    # fails (e.g. 'moor -c model' where 'model' is consumed as the
    ...
    # --yolo: set MOOR_YOLO_MODE *before* plugin discovery.  The call to
    ...
    # so introspection/management commands (moor hooks list, cron
    ...
    # ``moor egress start`` refusing when credential_source=bitwarden
