...
from moor_cli.subcommands.version import build_version_parser
...
def cmd_chat(args):
    ...
    # rare launch right after `moor update` changed a bundled skill.
...
def cmd_gui(args: argparse.Namespace):
    ...
    # `MOOR_DESKTOP_DISABLE_GPU=... moor desktop` keeps working.
...
def _windows_running_moor_launcher_locked() -> bool:
    ...
    if scripts_dir is None:
        return False
    shims = _moor_exe_shims(scripts_dir)
    if not shims:
        return False
...
def _run_quarantined_install(
    ...
    *,
    ...
    if scripts_dir is not None:
        moved = _quarantine_running_moor_exe(scripts_dir)
    try:
...
def _cleanup_quarantined_exes(scripts_dir: Path | None = None) -> None:
    """Sweep ``moor.exe.old.*`` left by prior updates.

    Called early on every moor invocation. The .old files are unlocked once
...
def cmd_profile(args):
    ...
    if action == "list":
        ...
        from moor_cli.profiles import rename_profile
...
def _maybe_setup_dashboard_auth_interactively(args) -> None:
    ...
    ``moor dashboard register`` for OAuth.
    ...
    try:
        from moor_cli.web_server import should_require_auth
...
# so that in ``moor -m gpt5 chat``, ``gpt5`` is correctly skipped as a
# flag value rather than misclassified as a subcommand. Kept in sync with
# the top-level flags declared in ``moor_cli/_parser.py``.
#
...
def main():
    ...
    # subcommand — ``moor --help``, ``moor version``, ``moor logs``,
    ...
    # =========================================================================
    # version command  (parser built in moor_cli/subcommands/version.py)
    # =========================================================================
