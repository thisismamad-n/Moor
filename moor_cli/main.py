...
Usage:
    ...
    moor chat / gateway / setup / status / cron / doctor / update / ...
    moor --version           # Show version and update status
    moor <cmd> --help        # Per-command help
"""

# moor_bootstrap must be the very first import — it sets up UTF-8 stdio on
...
# Inline path math so ``python moor_cli/main.py`` (script mode: sys.path[0]
# is moor_cli/, not the repo root) can import moor_cli._startup_fast.
...
# ``moor update`` may have left a recovery marker with a core package wiped;
# the moor_cli.config/env_loader imports further down would then crash before
...
# PID" class it exists for. ``moor_startup_watchdog`` is stdlib-only so it
...
if _argv_is_gateway_run(sys.argv[1:]):
    try:
        from moor_startup_watchdog import arm_startup_watchdog as _arm_sw
...
def _warn_if_unsupervised_pid1(pid: "int | None" = None) -> None:
    ...
    children). A Compose service that overrides ``entrypoint:`` to invoke moor directly makes
    moor itself PID 1 — nothing then ``wait()``s on those orphans and they accumulate as
    ...
    if (pid if pid is not None else os.getpid()) != 1:
        return
    print(
        "[moor] WARNING: this process is PID 1 with no init above it "
...
# moor_cli.config is importable. Cached so early callers don't re-parse YAML.
...
def _wants_tui_early(argv: "list[str] | None" = None) -> bool:
    ...
    Precedence: ``--cli`` wins, then ``--tui``/``MOOR_TUI=1``, then a
...
# `resetTerminalModes()` is the later cousin. ``MOOR_TUI_NO_EARLY_DISABLE``
...
# ``moor --version`` is answered before config/logging imports.
...
# Profile override — MUST happen before any moor module import: many modules
# cache MOOR_HOME at import time. --profile/-p is pre-parsed from sys.argv,
# MOOR_HOME set, and the flag stripped so argparse never sees it. Falls back
# to ~/.moor/active_profile for the sticky default.
_PROFILE_NAME_RE = r"^[a-z0-9][a-z0-9_-]{0,63}$"  # mirrors moor_cli.profiles._PROFILE_ID_RE
...
def _looks_like_moor_invocation() -> bool:
    """False when ``sys.argv`` belongs to a test runner rather than a ``moor`` run.

    pytest's own ``-p no:xdist`` reaches ``_scan_profile_flag`` through ``sys.argv`` at import
    time; it must stay a silent skip, while a real ``moor -p 'Work Bot'`` must fail loudly.
    """
    return "pytest" not in (sys.argv[0] or "")


def _exit_invalid_profile_name(value: str) -> None:
    from moor_cli.profiles import _invalid_profile_name_error

    print(f"Error: {_invalid_profile_name_error(value)}", file=sys.stderr)
    print("Run `moor profile list` to see your profiles.", file=sys.stderr)
    sys.exit(2)
...
def _scan_profile_flag(argv: list) -> tuple:
    ...
    ``moor`` run — after a subcommand, ``-p`` may belong to that subcommand or a
    plugin (`moor kanban ... -p 8080`), and option-looking values (``no:xdist``,
    ...
    while i < len(argv):
        ...
        if arg in {"--profile", "-p"} and i + 1 < len(argv):
            ...
            if re.match(_PROFILE_NAME_RE, value):
                return value, 2, i
            if not saw_subcommand and not _looks_like_option_value(raw) and _looks_like_moor_invocation():
...
def _resolve_sudo_user_profile_env(name: str) -> str | None:
    ...
    if name == "default":
        return None
    from moor_constants import sudo_invoker_default_home
...
def _under_gateway_supervisor(argv: list) -> bool:
    ...
    ``-p <name>`` or pin MOOR_HOME to the profile dir; a bare invocation
    means "the root MOOR_HOME profile". If a supervised default-profile
    child read active_profile, switching the active profile (dashboard,
    ``moor profile use``) would silently redirect the default gateway into
    ...
    MOOR_SUPERVISED_CHILD (systemd unit / launchd plist / Windows task),
    MOOR_S6_SUPERVISED_CHILD (legacy s6 container), INVOCATION_ID (systemd
    ...
    self-hosted CI runners), MOOR_GATEWAY_EXTERNAL_SUPERVISOR (explicit
    opt-in). XPC_SERVICE_NAME is deliberately NOT consulted: interactive macOS
    terminals set it too.
    """
    if os.environ.get("MOOR_SUPERVISED_CHILD") or os.environ.get("MOOR_S6_SUPERVISED_CHILD"):
        return True
...
def _apply_profile_override() -> None:
    ...
    # MOOR_HOME already set with no explicit flag: trust it only when it
    # points at a specific profile dir ("profiles" as immediate parent). If it
    # points at the moor root (systemd hardcodes MOOR_HOME=/root/.moor)
    # we must still read active_profile — the user may have run
    # `moor profile use` and the gateway should honour it (#22502).
...
# Windows launcher self-heal — the ``moor`` command is a COPY of the venv
# console script staged into the managed bin dir (outside the checkout, since
# ``moor update``'s autostash once swept ``<checkout>\bin`` copies off disk;
...
# app's ``python -m moor_cli.main`` spawn. Gates fail toward inaction. Sits
# AFTER the profile override on purpose — no moor module may import before
...
# ``<checkout>\bin``, where ``moor update``'s autostash (``git stash push --include-untracked``) swept
# them off disk; with the desktop updater's ``--keep-stash`` nothing restored them and ``moor`` stopped
...
load_moor_dotenv(project_env=PROJECT_ROOT / ".env")
...
try:
    ...
    # serves moor_logging, moor_time and later raw reads: 3-4 config.yaml parses become one.
    # Managed overlay included: administrator-pinned redact_secrets / force_ipv4 win here too.
    from moor_cli.config_effective import load_user_config_effective as _load_effective_early
...
from moor_cli.main_agent_cmds import (
    cmd_acp,
    cmd_insights,
    cmd_memory,
    cmd_monitoring,
...
from moor_cli.main_platform_setup import (
    cmd_slack,
    cmd_sync,
    cmd_whatsapp,
    cmd_whatsapp_cloud,
...
from moor_cli.main_dashboard import (  # frozen updater surface: update_cmd*.py resolve these via _m()
    _respawn_dashboard_processes,
)
from moor_cli.main_provider_setup import (
...
from moor_cli.main_install_repair import (
...
from moor_cli.main_desktop import (
    cmd_gui,
)
from moor_cli.main_desktop import (  # frozen updater surface: update_cmd*.py resolve these via _m()
    _desktop_build_needed,
    _desktop_dist_exists,
    _desktop_macos_relaunchable_fixup,
    _desktop_packaged_executable,
)
from moor_cli.main_web_build import (
    _sweep_stale_bytecode_if_checkout_changed,
)
from moor_cli.main_web_build import (  # frozen updater surface: update_cmd*.py resolve these via _m()
    _build_web_ui,
    _nixos_build_env,
    _record_bytecode_fingerprint,
    _run_npm_install_deterministic,
)
from moor_cli.main_tui_launch import (
    _launch_tui,
...
def _dotenv_has_provider_key(env_file: Path, provider_env_vars: set) -> bool:
    """True if ~/.moor/.env assigns a non-empty value to any provider key."""
...
def _has_any_provider_configured(*, strict_profile_scope: bool = False) -> bool:
    """Check if at least one inference provider is usable. Never creates one: the Moor free tier
    counts only once its identity exists, and the boot bootstrap (``moor_cli.free_tier_bootstrap``)
    ...
    """
    from moor_cli.config import DEFAULT_CONFIG, get_env_path, get_moor_home, load_config
    from moor_cli.auth import PROVIDER_REGISTRY, get_auth_status
    ...
    _has_moor_config = _model_name and _model_name != DEFAULT_CONFIG.get("model", "")
    ...
    if _auth_store_logged_in(get_moor_home() / "auth.json", PROVIDER_REGISTRY, strict_profile_scope):
        return True
    ...
    # Claude Code OAuth credentials count only once Moor is explicitly
    # configured — having Claude Code installed isn't consent to use its tokens.
    if _has_moor_config and not strict_profile_scope:
        try:
    ...
    # Nothing explicit anywhere: an existing Moor free-tier identity counts while the tier is on.
    try:
...
def _session_db():
    """Yield a read-only ``SessionDB`` (lazy import, so test patches on ``moor_state``
            ...
            if _crumb_id:
                ...
                if last_id:
                    ...
                    print(
                        ...
                        "`moor`, or list sessions with `moor sessions list`.",
                        file=sys.stderr,
                    )
                    sys.exit(1)
...
def _import_foreign_resume(args) -> None:
    ...
    if not (isinstance(_resume_foreign, str) and _resume_foreign.strip().lower() in ("@claude", "@codex")):
        return
    from moor_cli.foreign_sessions import import_foreign_session, pick_foreign_session
...
def _start_chat_background_prefetch() -> None:
    ...
    The ONE exception is an unseeded ~/.moor/skills: there the banner
...
def _first_run_setup_guard(args) -> None:
    """No provider configured: offer `moor setup` (TTY) or exit 1 with guidance."""
    print()
    print(
...
def cmd_chat(args):
    ...
    from moor_cli.stream_json import stream_json_requested
    # Structured stdout is a non-interactive protocol: it overrides MOOR_TUI/display.interface too.
    ...
    try:
        ...
        # Mixed-version installs (new cli.py, older moor_cli.config) crash
        # here — e.g. missing resolve_turn_limit / split_model_config_default
        # (#96900). The agent-setup mixin prints this hint too late: MoorCLI
        # construction already failed. Fast-chat launch also goes through
        # cmd_chat, so this one catch covers `moor` / `moor chat`.
...
def _forward_command(name: str, module: str, attr: str, *, forward_return: bool = False, doc: str = ""):
    """A ``moor <cmd>`` handler that hands ``args`` to ``<module>.<attr>``.
...
_PROVIDER_MODEL_FLOWS = {
    ...
    "moor": lambda c, m, a: _model_flow_moor(c, m, args=a),
...
def _resolve_active_provider(config, model_cfg, effective_provider, custom_provider_map):
    ...
    """
    from moor_cli.auth import AuthError, format_auth_error, resolve_provider
    from moor_cli.config import get_compatible_custom_providers, get_env_value
    from moor_cli.providers import custom_provider_aliases, resolve_provider_full
    ...
    if not active and effective_provider != "auto":
        ...
        if active_def is not None:
            ...
            print(
                f"Warning: Unknown provider '{effective_provider}'. Check 'moor model' for "
    ...
    if not active:
        try:
            ...
            if exc.code == "no_provider_configured":
                ...
                # "run `moor model`" from inside `moor model` is circular.
                print("No provider is set up yet — pick one below. (Moor Portal works without an API key.)")
...
def select_provider_and_model(args=None):
    ...
    effective_provider = config_provider or os.getenv("MOOR_INFERENCE_PROVIDER") or "auto"
    ...
    from moor_cli.models import _PROVIDER_LABELS
    ...
    # moor_cli.main keep intercepting.
...
# ``moor_cli/update_cmd*.py`` files resolve these names via ``_m().<name>``
# on moor_cli.main; importing update_cmd eagerly would cost every ``moor``
...
_FROZEN_UPDATER_SURFACE: dict[str, tuple[str, ...]] = {
    "moor_cli.update_cmd": (
        ...
        "_print_parked_branch_skip_warning", "_purge_stale_moor_modules",
...
def _print_version_info(*, check_updates: bool = True) -> None:
    # Shared with the `moor --version` pre-import fast path.
...
def _update_preflight_handled(args) -> bool:
    ...
    if getattr(args, "plan", False):
        # Read-only plan phase (#91277 Phase 2): inventory every running Moor runtime across profiles, its
...
def _coalesce_session_name_args(argv: list) -> list:
    ...
    ``moor -c Pokemon Agent Dev`` → ``['-c', 'Pokemon Agent Dev']``; tokens
...
from moor_cli.profile_cmd import cmd_profile
...
def _dashboard_validate_serve_args(args, headless_backend, token_file):
    ...
    # `moor serve` is headless/non-interactive: fail closed on a corrupt
...
def _dashboard_sanitize_desktop_env(headless_backend) -> None:
    ...
    MOOR_SERVE_HEADLESS=1). A shell inheriting those then running
    `moor dashboard` would serve the desktop renderer ("Desktop IPC bridge
    ...
    (MOOR_DESKTOP=1) keeps its dist. Headless `serve` re-sets
    MOOR_SERVE_HEADLESS itself.
    """
    if os.environ.get("MOOR_DESKTOP") != "1":
        if _is_electron_packaged_web_dist(os.environ.get("MOOR_WEB_DIST", "")):
...
def _dashboard_prepare_runtime(args, headless_backend) -> bool:
    ...
    mcp_discovery_after_bind = headless_backend and os.environ.get("MOOR_DESKTOP") == "1"
...
def cmd_completion(args, parser=None):
    ...
    from moor_cli import completion
...
def _plugin_cli_discovery_needed() -> bool:
    ...
    """
    first = _first_positional_argv()  # None = bare ``moor`` → chat
...
def _resolve_deferred_platform_cli_command(command_name: str | None) -> None:
    ...
    on import; ``discover_plugins()`` alone leaves ``moor photon`` failing
    ...
    not import it, so the CLI registration never happens and ``moor photon`` fails with argparse ``invalid
    choice`` (issue #54678).
    """
    if not command_name:
        return
...
def _is_tui_chat_launch(args) -> bool:
    if getattr(args, "tui", False) or os.environ.get("MOOR_TUI") == "1":
        return True
    # The chat path decides TUI-vs-classic via _resolve_use_tui (--cli/--tui
    # flags, TTY gate, MOOR_TUI env, display.interface config). Bare
    # `moor`/`moor chat` with a TUI display config was previously missed
...
def _prepare_agent_startup(args) -> None:
    ...
    # --yolo chokepoint: MOOR_YOLO_MODE must be set before any discovery
    ...
    try:
        from moor_cli.mcp_startup import set_mcp_server_filter
    ...
    if _run_inline_mcp_discovery:
        try:  # synchronous for entrypoints without a later bounded startup path
            from moor_cli.mcp_startup import get_mcp_server_filter
...
def _try_fast_serve_launch() -> bool:
    ...
    """
    if os.environ.get("MOOR_DISABLE_FAST_SERVE_LAUNCH") == "1":
        return False
...
def _try_termux_fast_tui_launch() -> bool:
    ...
    `moor --tui` is the hot path on phones and the TUI immediately execs
...
def _advertise_agent_env() -> None:
    ...
    (``moor-agent``) — matching is exact. ``MOOR_AGENT`` is the
    moor-specific marker. setdefault: never clobber an outer harness.
    ...
    harness that spawned them. Moor running inside another agent's terminal).
    """
...
def _cmd_sessions_lazy(args, **kwargs):
    """``moor sessions`` handler; sessions_cmd imports only when the subcommand runs."""
...
def _build_cli_parser():
    """Build the full ``moor`` argparse tree -> ``(parser, subparsers)``.

    Registration ORDER is the ``moor --help`` order; keep it stable. Groups
    live in ``moor_cli/subcommands/<group>.py`` with handlers injected so
    ...
    # OUTBOUND egress firewall; ``moor proxy`` (gateway group) is the INBOUND one.
    ...
    # "desktop" is canonical (moor-setup.exe tells users to run it, so it
...
def _parse_cli_args(parser, subparsers, argv):
    ...
    routing; if that fails (``moor -c model`` — 'model' is the session name)
...
def main():
    ...
    # updates (see ``_quarantine_running_moor_exe``). No-op elsewhere.
    try:
        _cleanup_quarantined_exes()
    except Exception:
        pass
    ...
    # Self-heal a venv left half-built by an interrupted ``moor update``, and
    ...
    # substring match is deliberately loose: over-matching (``moor skills
    # install update``) only defers recovery one launch; under-matching
    # (``moor -p work update``) would race. Never raises.
    # See #95294.
...
_PLUGIN_COMPAT_LAZY = {
    'line_input': ('moor_cli.cli_output', 'line_input'),
}
