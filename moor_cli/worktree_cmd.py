"""``moor worktree`` — audit and reclaim accumulated git worktrees/branches.

Attended counterpart of the silent startup pruner (see
``moor_cli/worktree_gc.py`` for the policy and shared invariants). Usage:

    moor worktree list             # audit: verdict + reason per tree
    moor worktree prune            # reap safe trees + merged branches
    moor worktree prune --dry-run  # show the plan, change nothing
    moor worktree prune --trees-only / --branches-only
"""
