"""Dependency-light venv recovery that runs BEFORE moor_cli.main's imports.

The ``moor`` console entry point is ``moor_cli.main:main``.  Importing
``moor_cli.main`` pulls in third-party packages at module level (``dotenv``
via ``moor_cli.env_loader``, ``yaml`` via ``moor_cli.config``, ...).  In
...
a corrupted venv.  ``moor_cli.main`` imports and calls
:func:`recover_if_needed` at the very top of its module body, before any
third-party import.

Scope: this early pass only repairs enough for ``moor_cli.main`` to become
...
#
# ``moor update`` renames the live ``moor*.exe`` shims aside
# (``moor.exe.old.<unix-ms>``) so uv can write replacements. Putting them BACK
# is the safety-critical direction: losing that rename leaves the install with
# no ``moor`` on PATH, and the command that would repair it IS ``moor
# update`` (#75584).
#
...
def restore_quarantined_shims(
    ...
    *,
    ...
    Messages go to stderr by default -- the startup sweep runs on EVERY moor
    invocation, and ``moor acp`` speaks JSON-RPC on stdout.
    """
    if stream is None:
        stream = sys.stderr
...
def _find_uv_binary() -> str | None:
    ...
    (``~/.moor/bin/uv.exe``) or the user has on PATH.  Stdlib-only.
    """
    exe = "uv.exe" if sys.platform == "win32" else "uv"
    candidates = [
        Path.home() / ".moor" / "bin" / exe,
...
def _run_repair_install(specs: list[str], project_root: Path) -> bool:
    ...
    Streams nothing to stdout (``moor acp`` speaks JSON-RPC on stdout);
...
def _pytest_owns_live_checkout(root: Path) -> bool:
    ...
    Lifecycle tests spawn real subprocesses that import ``moor_cli.main``
...
def recover_if_needed(
    ...
    """Repair wiped core packages so ``moor_cli.main`` can import at all.

    Fast path (no marker present) is two ``lstat`` calls.  Only acts when a
    recovery marker from a prior ``moor update`` exists AND an import probe
    ...
    try:
        ...
        # launch is itself `moor update`: CLI and Desktop retries preserve
...
# every `moor` launch: past this many attempts the early pass hands the
...
def _complete_pending_core_install(root: Path, core_marker: Path) -> bool:
    ...
    a prior ``moor update`` (or the self-lock preflight, #83569) left the
    ...
    ``moor_cli.main``'s third-party imports, maps nothing yet, so the
    ...
    failure does not block every launch (``moor acp`` included).
    ...
    """
    try:
        from moor_cli import _install_repair as ir
        ...
        try:
            ...
            print(
                "  The next launch will retry; moor will keep working from "
