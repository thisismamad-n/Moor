...
Deliberately **stdlib-only** so importing it can never fail on a corrupted venv. ``moor_cli.main``
calls :func:`recover_if_needed` at the very top of its module body, before any third-party import.
Scope: repair only enough for ``moor_cli.main`` to become importable again (force-reinstall of
...
# ``moor update`` renames the live ``moor*.exe`` shims aside (``moor.exe.old.<unix-ms>``) so
# uv can write replacements. Putting them BACK is the safety-critical direction: losing that rename
# leaves no ``moor`` on PATH, and the command that would repair it IS ``moor update``. The
...
def _find_uv_binary() -> str | None:
    ...
    fallback refuses to touch them; the only sanctioned installer is then uv itself, which Moor
    vendors (``~/.moor/bin/uv.exe``) or the user has on PATH.
    """
    exe = "uv.exe" if sys.platform == "win32" else "uv"
    for sub in ((".moor", "bin"), (".local", "bin"), (".cargo", "bin")):
...
def _run_repair_install(specs: list[str], project_root: Path) -> bool:
    ...
    Streams nothing to stdout (``moor acp`` speaks JSON-RPC on stdout). uv is preferred when the
...
def _pytest_owns_live_checkout(root: Path) -> bool:
    ...
    Lifecycle tests spawn real subprocesses that import ``moor_cli.main`` with recovery armed and
...
def recover_if_needed(project_root: Path | None = None, argv: list[str] | None = None) -> None:
    ...
    prior ``moor update`` exists AND an import probe confirms a core package is actually broken.
    ...
    try:
        ...
        # recovered even when this launch is itself `moor update`: CLI and Desktop retries keep
...
def _complete_pending_core_install(root: Path, core_marker: Path) -> bool:
    ...
    ``recover_if_needed`` invokes this when ``.update-incomplete`` exists — a prior ``moor update`` (or
    ...
    venv extension mapped; this process, running before ``moor_cli.main``'s third-party imports, maps
    ...
    try:
        ...
        if not _claim_recovery_lock(root):
            return False
        try:
            print("⚠ A previous `moor update` was interrupted mid-install — "
            ...
            print("  The next launch will retry; moor will keep working from "
