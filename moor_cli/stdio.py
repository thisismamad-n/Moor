...
def configure_windows_stdio() -> bool:
    ...
    ``MOOR_DISABLE_WINDOWS_UTF8=1`` to opt out (forces the old cp1252 path for diagnosing
    ...
    if _CONFIGURED:
        return False
    if not is_windows() or os.environ.get("MOOR_DISABLE_WINDOWS_UTF8") in {"1", "true", "True", "yes"}:
...
def _augment_path_with_known_tools() -> None:
    r"""Prepend moor-managed tool directories to ``PATH`` (no-op on POSIX / missing dirs).

    install.ps1 adds entries like ``%LOCALAPPDATA%\moor\git\bin`` to the User PATH via
    ``SetEnvironmentVariable``, but already-running shells never see that broadcast, so a moor
    ...
    # dir hosts moor.exe + pip console scripts; WinGet\Links is where ``winget install`` drops
