...
Entry points (``cli.py`` ``main``, ``moor_cli/main.py`` CLI dispatch,
...
def configure_windows_stdio() -> bool:
    ...
    Set ``MOOR_DISABLE_WINDOWS_UTF8=1`` in the environment to opt out
    ...
    if os.environ.get("MOOR_DISABLE_WINDOWS_UTF8") in {"1", "true", "True", "yes"}:
        _CONFIGURED = True
        return False
    ...
    # already-running shells don't see the change — which means moor
...
def _default_windows_editor() -> str:
    ...
    ``moor config edit`` both honour ``$EDITOR``.  Users who prefer a
...
def _augment_path_with_known_tools() -> None:
    ...
    ``%LOCALAPPDATA%\\moor\\git\\bin`` to the User PATH via
    ...
    *spawned* processes only — already-running shells (including the one the
    user invokes ``moor`` from right after install) retain their old PATH.
    ...
    # see on next launch.
    candidate_dirs = [
        os.path.join(local_appdata, "moor", "git", "cmd"),
        os.path.join(local_appdata, "moor", "git", "bin"),
        os.path.join(local_appdata, "moor", "git", "usr", "bin"),
        # Moor venv Scripts directory — host of the moor.exe shim itself,
        # also where any pip-installed console scripts land.  Usually already
        # on PATH when the user invokes moor, but harmless to include.
        os.path.join(local_appdata, "moor", "moor-agent", "venv", "Scripts"),
