...
Mixin bound onto ``MoorCLI`` via the MRO. cli.py-internal symbols are imported LAZILY
...
class CLISessionMixin:
    ...
    def _show_session_status(self):
        ...
        lines = [
            "Moor CLI Status", "", f"Session ID: {self.session_id}", f"Path: {display_moor_home()}",
        ]
        if title:
    ...
    def _show_recent_sessions(self, *, reason: str = "history", limit: int = 10) -> bool:
        ...
        from moor_cli.timefmt import relative_time as _relative_time
    ...
    def save_conversation(self, cmd: str = "/save"):
        ...
        try:
            ...
            # tools/subprocesses on this thread resolve MOOR_SESSION_ID to the child id after an
    ...
    def _write_terminal_breadcrumb(self) -> None:
        """Record this terminal's live session for bare ``moor -c``. Called whenever
    ...
    def _toggle_yolo(self):
        ...
        ``MOOR_YOLO_MODE``: that env var is frozen into ``tools.approval._YOLO_MODE_FROZEN``
        ...
        if _YOLO_MODE_FROZEN:
            _cprint(
                ...
                " for this process (started with --yolo / MOOR_YOLO_MODE)."
    ...
    def _print_exit_summary(self, clear_screen: bool = True):
        ...
        # the hint ("default"/"custom" use the standard MOOR_HOME).
        try:
