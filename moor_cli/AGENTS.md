...
- **Three loaders — know which you're in:** `load_cli_config()` (CLI, `cli.py`); `load_config()`
  (`moor tools/setup`, most subcommands, `moor_cli/config.py`, merges `DEFAULT_CONFIG`); raw
...
`_apply_profile_override()` in `moor_cli/main.py` sets `MOOR_HOME` before any module import, so
every `get_moor_home()` scopes to the active profile (rules in root). Profiles are independent
