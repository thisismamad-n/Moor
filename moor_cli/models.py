...
``moor_cli.models.<name>`` stays the stable import/monkeypatch surface:
...
from moor_cli.models_reasoning_caps import (
...
# ---------------------------------------------------------------------------
# Moor Portal free-model helpers — the Portal models endpoint is the source of truth for what is
...
def partition_moor_models_by_tier(
    ...
    """Split Moor models into (selectable, unavailable): free-tier users may only select free models
...
def _union_with_portal_recommendations(
    ...
    *, force_refresh: bool, synthesize_free_pricing: bool,
    ...
    """
    try:
        payload = fetch_moor_recommended_models(portal_base_url, force_refresh=force_refresh)
    except Exception:
        payload = None
...
def _pricing_profile_key() -> str:
    ...
    return moor_home_key()


def get_cached_moor_free_tier() -> Optional[bool]:
...
def check_moor_free_tier(*, force_fresh: bool = False, cached_only: bool = False) -> bool:
    """True only when the Moor Portal user is KNOWN to be free-tier (unknown/error → False so this
    ...
    if not force_fresh:
        cached_result = get_cached_moor_free_tier()
    ...
    try:
        ...
        result = get_moor_portal_account_info(force_fresh=force_fresh).is_free_tier
    except Exception:
...
# ---------------------------------------------------------------------------
# Moor Portal recommended models — curated paid/free suggestions plus dedicated compaction (aux)
...
def _read_moor_recommended_disk(base: str) -> dict[str, Any] | None:
    ...
    blob = _read_json_cache(_moor_recommended_disk_path(), errors=(OSError, json.JSONDecodeError))
...
def _write_moor_recommended_disk(base: str, data: dict[str, Any]) -> None:
    ...
    try:
        ...
        logger.debug("moor recommended-models disk cache write failed: %s", exc)
...
def fetch_moor_recommended_models(
    ...
    Cached per portal URL for ``_MOOR_RECOMMENDED_CACHE_TTL`` seconds in process (``force_refresh``
    ...
    try:
        data = _get_json(
            f"{base}{MOOR_RECOMMENDED_MODELS_PATH}", timeout=timeout, headers={"Accept": "application/json"}
        )
    ...
    if data:
        ...
        data = _read_moor_recommended_disk(base) or data
...
def _resolve_moor_portal_url() -> str:
    ...
    try:
        from moor_cli.auth import DEFAULT_MOOR_PORTAL_URL, get_provider_auth_state

        state = get_provider_auth_state("moor") or {}
        portal = str(state.get("portal_base_url") or "").strip()
        return (portal or str(DEFAULT_MOOR_PORTAL_URL)).rstrip("/")
    except Exception:
...
def _openrouter_model_supports_tools(item: Any) -> bool:
    """True when ``supported_parameters`` advertises ``tools`` (moor-agent is tool-calling-first).
    Permissive when the field is absent/malformed: some OpenRouter-compatible gateways (Moor Portal,
...
# Reasoning-capability cache slots, one set per catalog (OpenRouter, Moor Portal). The logic
...
def fetch_openrouter_models(
    ...
    for preferred_id, _ in fallback:
        ...
        if live_item is None or not _openrouter_model_supports_tools(live_item):
            continue
        # Hide models that don't advertise tool-calling support — moor-agent requires it and surfacing
...
def get_curated_moor_model_ids() -> list[str]:
    """Curated Moor Portal model ids: the remote catalog manifest, else the in-repo
    ``_PROVIDER_MODELS["moor"]`` snapshot. Always a list."""
    try:
        from moor_cli.model_catalog import get_curated_moor_models
        remote = get_curated_moor_models()
    except Exception:
        remote = None
    return list(remote or _PROVIDER_MODELS.get("moor", []))
...
def list_available_providers() -> list[dict[str, str]]:
    ...
    derived from :data:`CANONICAL_PROVIDERS` (shared with ``moor model`` and ``/model``)."""
...
def detect_static_provider_for_model(
    ...
    # Step 0: a bare provider name typed as the model (`/model moor`) is a provider switch to that
    ...
    if resolved_provider not in {"custom", "openrouter"}:
        ...
        if resolved_provider in _PROVIDER_LABELS and default_models and resolved_provider not in current_keys:
            ...
            # so [0] would silently escalate `/model moor` to the priciest flagship.
...
def _resolve_provider_prefix(model_name: str) -> Optional[tuple[str, str]]:
    """Route an explicit ``vendor/model`` prefix (``moor/deepseek-v4-pro``, ``ollama/qwen3.5:4b``) to
    a provider the user defined in ``providers:`` (by raw name or alias) instead of the default.

    ``moor/deepseek-v4-pro`` or ``ollama/qwen3.5:4b`` should route to the named provider instead of falling
...
def normalize_provider(provider: Optional[str]) -> str:
    ...
    ``moor_cli.auth.resolve_provider()`` to resolve it from credentials."""
...
def _first_exchangeable_copilot_token(raw_tokens) -> str:
    ...
    from moor_cli.copilot_auth import exchange_copilot_token, validate_copilot_token
...
def _moor_catalog(normalized: str, force_refresh: bool) -> Optional[list[str]]:
    try:
    ...
    # Portal models still surface without a Moor release.
    return get_curated_moor_model_ids() or None
...
def _openai_catalog(normalized: str, force_refresh: bool) -> Optional[list[str]]:
    ...
    # entries, so intersect with the curated agentic catalog so ``/model`` matches ``moor model``.
    ...
    # pure-catalog helper (no extra live fetch) so we only accept models Moor actually ships. (#46850)
...
# every authed provider's /v1/models). One JSON file at $MOOR_HOME/provider_models_cache.json;
...
def clear_provider_models_cache(provider: Optional[str] = None) -> None:
    ...
    ``/model --refresh`` and ``moor model --refresh``."""
    try:
...
def _fetch_ai_gateway_models(timeout: float = 5.0) -> Optional[list[str]]:
    ...
    headers = {"Authorization": f"Bearer {api_key}", "User-Agent": _MOOR_USER_AGENT}
    try:
...
_PLUGIN_COMPAT_LAZY = {
    'LMStudioLoadResult': ('moor_cli.models_local', 'LMStudioLoadResult'),
    'PROVIDER_GROUPS': ('moor_cli.models_catalog_static', 'PROVIDER_GROUPS'),
    'ProviderEntry': ('moor_cli.models_catalog_static', 'ProviderEntry'),
    'atomic_json_write': ('utils', 'atomic_json_write'),
    'base_url_host_matches': ('utils', 'base_url_host_matches'),
    'compute_sale_discount': ('moor_cli.models_pricing', 'compute_sale_discount'),
    'ensure_lmstudio_model_loaded': ('moor_cli.models_local', 'ensure_lmstudio_model_loaded'),
    'fetch_ai_gateway_pricing': ('moor_cli.models_pricing', 'fetch_ai_gateway_pricing'),
    'fetch_lmstudio_models': ('moor_cli.models_local', 'fetch_lmstudio_models'),
    'fetch_models_with_pricing': ('moor_cli.models_pricing', 'fetch_models_with_pricing'),
    'fetch_ollama_local_models': ('moor_cli.models_local', 'fetch_ollama_local_models'),
    'get_cached_moor_inference_base_url': ('moor_cli.models_pricing', 'get_cached_moor_inference_base_url'),
    'get_pricing_for_provider': ('moor_cli.models_pricing', 'get_pricing_for_provider'),
    'group_providers': ('moor_cli.models_catalog_static', 'group_providers'),
    'lmstudio_model_reasoning_options': ('moor_cli.models_local', 'lmstudio_model_reasoning_options'),
    'moor_catalog_url': ('moor_cli.models_reasoning_caps', 'moor_catalog_url'),
    'moor_model_reasoning_capabilities': ('moor_cli.models_reasoning_caps', 'moor_model_reasoning_capabilities'),
    'moor_policy_allowed_ids': ('moor_cli.models_pricing', 'moor_policy_allowed_ids'),
    'ollama_model_supports_thinking': ('moor_cli.models_local', 'ollama_model_supports_thinking'),
    'openrouter_model_reasoning_capabilities': ('moor_cli.models_reasoning_caps', 'openrouter_model_reasoning_capabilities'),
    'parse_openrouter_reasoning_capabilities': ('moor_cli.models_reasoning_caps', 'parse_openrouter_reasoning_capabilities'),
    'peek_cached_pricing': ('moor_cli.models_pricing', 'peek_cached_pricing'),
    'pricing_cache_scope': ('moor_cli.models_pricing', 'pricing_cache_scope'),
    'probe_lmstudio_models': ('moor_cli.models_local', 'probe_lmstudio_models'),
    'probe_ollama_local_models': ('moor_cli.models_local', 'probe_ollama_local_models'),
    'provider_group_for_slug': ('moor_cli.models_catalog_static', 'provider_group_for_slug'),
    'refresh_reasoning_caps_async': ('moor_cli.models_reasoning_caps', 'refresh_reasoning_caps_async'),
    'restrict_to_moor_policy': ('moor_cli.models_pricing', 'restrict_to_moor_policy'),
    'should_use_ollama_native_catalog': ('moor_cli.models_local', 'should_use_ollama_native_catalog'),
    'url_origin': ('moor_cli.urllib_security', 'url_origin'),
    'validate_requested_model': ('moor_cli.models_validate', 'validate_requested_model'),
    'warm_moor_reasoning_caps_async': ('moor_cli.models_reasoning_caps', 'warm_moor_reasoning_caps_async'),
    'warm_openrouter_reasoning_caps_async': ('moor_cli.models_reasoning_caps', 'warm_openrouter_reasoning_caps_async'),
}
