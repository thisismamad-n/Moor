...
Add, remove, or reorder entries here — both `moor setup` and
`moor` provider-selection will pick up the change automatically.
"""
...
from moor_cli.urllib_security import open_credentialed_url, url_origin
...
_MOOR_USER_AGENT = f"moor-cli/{_MOOR_VERSION}"
...
def _codex_curated_models() -> list[str]:
    ...
    This keeps the gateway /model picker in sync with the CLI `moor model`
    flow without maintaining a separate static list.
    """
    from moor_cli.codex_models import DEFAULT_CODEX_MODELS, _finalize_codex_models
...
# $MOOR_HOME/models_dev_cache.json as of 2026-04-28. Whenever xAI renames
...
def _xai_merge_curated_extras(ids: list[str]) -> list[str]:
    """Append moor-curated xAI models that are missing from models.dev."""
    out = list(ids)
...
def _xai_curated_models() -> list[str]:
    ...
    Reads $MOOR_HOME/models_dev_cache.json directly (no network). Falls
...
_PROVIDER_MODELS: dict[str, list[str]] = {
    ...
    # "opencode/latest"; we send honest Moor attribution and don't
    ...
    # endpoint expects (see moor_cli/model_setup_flows.py).
...
# ---------------------------------------------------------------------------
# Moor Portal free-model helper
# ---------------------------------------------------------------------------
# The Moor Portal models endpoint is the source of truth for which models
...
# ---------------------------------------------------------------------------
# Moor Portal account tier detection
# ---------------------------------------------------------------------------
def is_moor_free_tier(account_info: dict[str, Any]) -> bool:
...
def partition_moor_models_by_tier(
    model_ids: list[str],
    pricing: dict[str, dict[str, str]],
    free_tier: bool,
) -> tuple[list[str], list[str]]:
    """Split Moor models into (selectable, unavailable) based on user tier.
...
def union_with_portal_free_recommendations(
    ...
    *,
    ...
    The Portal's ``/api/moor/recommended-models`` endpoint advertises which
    models are free *right now* — independent of what the in-repo
    ``_PROVIDER_MODELS["moor"]`` list happens to contain or whether the
    ...
    older Moor that doesn't ship that model in its hardcoded curated
...
def union_with_portal_paid_recommendations(
    ...
    *,
    ...
    users. The Portal's ``/api/moor/recommended-models`` endpoint advertises
    which paid models are blessed *right now* — independent of what the
    in-repo ``_PROVIDER_MODELS["moor"]`` list happens to contain or whether
    ...
    picker even if the user is running an older Moor that doesn't ship
    ...
    * ``pricing`` is left untouched — we deliberately do NOT synthesize
      ...
      :func:`partition_moor_models_by_tier` keeps free models selectable;
...
def check_moor_free_tier(*, force_fresh: bool = False) -> bool:
    """Check if the current Moor Portal user is on a free (unpaid) tier.
...
# ---------------------------------------------------------------------------
# Moor Portal recommended models
#
...
_moor_recommended_cache: dict[str, tuple[dict[str, Any], float]] = {}
...
def _read_moor_recommended_disk(base: str) -> dict[str, Any] | None:
    ...
    """
    try:
        with open(_moor_recommended_disk_path(), encoding="utf-8") as fh:
            blob = json.load(fh)
...
def _write_moor_recommended_disk(base: str, data: dict[str, Any]) -> None:
    ...
    if not data:
        return
    path = _moor_recommended_disk_path()
    try:
        try:
        ...
        logging.getLogger(__name__).debug(
            "moor recommended-models disk cache write failed: %s", exc
        )


def fetch_moor_recommended_models(
    portal_base_url: str = "",
    timeout: float = 5.0,
    *,
    force_refresh: bool = False,
) -> dict[str, Any]:
    """Fetch the Moor Portal's curated recommended-models payload.

    Hits ``<portal>/api/moor/recommended-models``. The endpoint is public —
    no auth is required. Results are cached per portal URL for
    ``_MOOR_RECOMMENDED_CACHE_TTL`` seconds in process; pass
    ``force_refresh=True`` to bypass the in-process cache.

    A successful live fetch is also persisted to a per-base disk cache
    (``$MOOR_HOME/cache/moor_recommended_cache.json``) as last-known-good.
    ...
    now = time.monotonic()
    cached = _moor_recommended_cache.get(base)
    if not force_refresh and cached is not None:
        payload, cached_at = cached
        if now - cached_at < _MOOR_RECOMMENDED_CACHE_TTL:
            return payload

    url = f"{base}{MOOR_RECOMMENDED_MODELS_PATH}"
    try:
    ...
    if data:
        ...
        _moor_recommended_cache[base] = (data, now)
    ...
    if disk:
        _moor_recommended_cache[base] = (disk, now)
        return disk
...
def _resolve_moor_portal_url() -> str:
    ...
    try:
        ...
        return str(DEFAULT_MOOR_PORTAL_URL).rstrip("/")
    except Exception:
...
def get_moor_recommended_aux_model(
    *,
    vision: bool = False,
    ...
    * ``vision=False`` → ``paidRecommendedCompactionModel`` or
    ...
    via :func:`check_moor_free_tier`. Pass an explicit bool to bypass the
    ...
    ``google/gemini-3-flash-preview``).
    """
    base = portal_base_url or _resolve_moor_portal_url()
    payload = fetch_moor_recommended_models(base, force_refresh=force_refresh)
    if not payload:
        return None

    if free_tier is None:
        try:
            free_tier = check_moor_free_tier()
        except Exception:
...
# this list:  moor model, /model, list_authenticated_providers.
#
# Fields:
#   slug        — internal provider ID (used in config.yaml, --provider flag)
#   label       — short display name
#   tui_desc    — longer description for the `moor model` interactive picker
# ---------------------------------------------------------------------------

class ProviderEntry(NamedTuple):
    slug: str
    label: str
    tui_desc: str   # detailed description for `moor model` TUI

CANONICAL_PROVIDERS: list[ProviderEntry] = [
    ProviderEntry("moor",           "Moor Portal",              "Moor Portal (Everything your agent needs, 300+ models with bundled tool use)"),
...
#
# Some vendors expose several Moor provider slugs (one per endpoint /
# auth method: global API, China API, OAuth coding plan, ...). Listing every
# slug as a top-level row in the interactive `moor model` / setup wizard /
...
def group_providers(slugs):
    ...
    DISPLAY ONLY. Used by every interactive picker (``moor model``, the
...
# In-repo fallback for the model Moor silently lands on when the user never
...
def get_preferred_silent_default_model(provider: str = "openrouter") -> str:
    ...
    """
    try:
        from moor_cli.model_catalog import get_default_model_from_cache
...
# curated-list entry [0]. Metered aggregators (Moor Portal, OpenRouter) order
...
# ``moor model``) uses the richer free/paid-tier-aware resolver — see
# ``get_recommended_default_model`` in moor_cli/web_server.py and
# ``partition_moor_models_by_tier`` — which can hit the Portal.
_SILENT_DEFAULT_PROVIDERS: frozenset[str] = frozenset({"moor", "openrouter"})
...
def get_default_model_for_provider(provider: str) -> str:
    ...
    model was ever selected (e.g. ``moor auth add openai-codex`` without
    ``moor model``, or a profile that sets ``provider`` with no ``model``).

    For most providers this is the first entry in ``_PROVIDER_MODELS`` — the
    same model the ``moor model`` picker offers first. For metered aggregators
...
def _openrouter_model_supports_tools(item: Any) -> bool:
    ...
    moor-agent is tool-calling-first — every provider path assumes the model
    ...
    **Permissive when the field is missing.** Some OpenRouter-compatible gateways
    (Moor Portal, private mirrors, older catalog snapshots) don't populate
...
# copy, `moor -p`, a cron job, or a freshly booted gateway answers
...
# the Moor Portal list different models, and a staging Portal must not answer
# for production.
...
def _reasoning_caps_disk_path() -> Path:
    from moor_constants import get_moor_home
    return get_moor_home() / "cache" / "reasoning_caps.json"
...
def _fetch_reasoning_caps_catalog(
    ...
    (OpenRouter itself, Moor Portal). Returns None when the catalog is
    ...
    """
    headers = {"Accept": "application/json", "User-Agent": _MOOR_USER_AGENT}
    try:
...
# Moor Portal serves OpenRouter's catalog schema, so the same parser and
...
_moor_reasoning_caps_cache: dict[str, Optional[dict[str, Any]]] | None = None
_moor_reasoning_caps_failed_at: float | None = None


def moor_catalog_url() -> str:
    ...
    ``MOOR_INFERENCE_BASE_URL`` → resolved credential base → prod — so a
    ...
    """
    return f"{_resolve_moor_pricing_credentials()[1]}/v1/models"


def _fetch_moor_reasoning_caps(
    timeout: float = 6.0, *, force: bool = False
) -> Optional[dict[str, Optional[dict[str, Any]]]]:
    """Moor Portal counterpart of :func:`_fetch_openrouter_reasoning_caps`."""
    global _moor_reasoning_caps_cache, _moor_reasoning_caps_failed_at
    if _moor_reasoning_caps_cache is not None and not force:
        return _moor_reasoning_caps_cache
    if (
        _moor_reasoning_caps_failed_at is not None
        and (time.monotonic() - _moor_reasoning_caps_failed_at) < 60
    ):
        return None
    caps_by_id = _fetch_reasoning_caps_catalog(moor_catalog_url(), timeout)
    if caps_by_id is None:
        _moor_reasoning_caps_failed_at = time.monotonic()
        return None
    _moor_reasoning_caps_cache = caps_by_id
    return caps_by_id


def _refresh_moor_reasoning_caps() -> None:
    _fetch_moor_reasoning_caps(force=True)


def moor_model_reasoning_capabilities(
    model_id: Optional[str],
    *,
    timeout: float = 6.0,
    allow_fetch: bool = False,
) -> Optional[dict[str, Any]]:
    """Return live-catalog reasoning capabilities for a Moor Portal model.

    Same tri-state contract and cache-only default as
    :func:`openrouter_model_reasoning_capabilities`; warm the cache with
    :func:`warm_moor_reasoning_caps_async` from hot paths.
    """
    model = str(model_id or "").strip()
    if not model:
        return None
    caps_by_id = _moor_caps_cached()
    if caps_by_id is None and allow_fetch:
        caps_by_id = _fetch_moor_reasoning_caps(timeout=timeout)
    if caps_by_id is None:
        return None
    return caps_by_id.get(model)


_moor_caps_disk_checked = False
_moor_caps_warm_started = False


def _moor_caps_cached() -> Optional[dict[str, Optional[dict[str, Any]]]]:
    ...
    """
    global _moor_reasoning_caps_cache, _moor_caps_disk_checked
    if _moor_reasoning_caps_cache is None and not _moor_caps_disk_checked:
        _moor_caps_disk_checked = True
        _moor_reasoning_caps_cache = _hydrate_reasoning_caps_from_disk(
            moor_catalog_url(), _refresh_moor_reasoning_caps
        )
    return _moor_reasoning_caps_cache


def warm_moor_reasoning_caps_async() -> None:
    """Moor Portal counterpart of :func:`warm_openrouter_reasoning_caps_async`."""
    global _moor_caps_warm_started
    if _moor_caps_warm_started or _moor_caps_cached() is not None:
        return
    _moor_caps_warm_started = True
    _warm_reasoning_caps_async(_refresh_moor_reasoning_caps)
...
def fetch_openrouter_models(
    ...
    *,
    ...
    try:
        from moor_cli.model_catalog import get_curated_openrouter_models
    ...
    for preferred_id in preferred_ids:
        ...
        if live_item is None:
            continue
        # Hide models that don't advertise tool-calling support — moor-agent
        ...
        if preferred_id == silent_default:
            ...
            # picker shows which model Moor lands on when none is selected.
            desc = "default"
        else:
...
def get_curated_moor_model_ids() -> list[str]:
    """Return the curated Moor Portal model-id list.

    Prefers the remotely-hosted catalog manifest (published under
    ``website/static/api/model-catalog.json``); falls back to the in-repo
    snapshot in ``_PROVIDER_MODELS["moor"]`` when the manifest is
    unreachable. Always returns a list (never None).
    """
    try:
        from moor_cli.model_catalog import get_curated_moor_models
        remote = get_curated_moor_models()
    except Exception:
        remote = None
    if remote:
        return list(remote)
    return list(_PROVIDER_MODELS.get("moor", []))
...
def fetch_ai_gateway_models(
    ...
    *,
    ...
    from moor_constants import AI_GATEWAY_BASE_URL
...
def compute_sale_discount(
    ...
    Moor Portal-only feature: callers gate on the provider; this helper only
    sees ``original`` because the Moor fetch path opted in via
...
def fetch_models_with_pricing(
    ...
    *,
    ...
    Works with any OpenRouter-compatible endpoint (OpenRouter, Moor Portal).

    When *include_sale_original* is true (Moor Portal only) and the gateway
    ...
    for item in payload.get("data", []):
        ...
        if mid and isinstance(pricing, dict):
            ...
            # Sale chrome is Moor Portal-only. Never copy pricing.original for
...
def fetch_ai_gateway_pricing(
    ...
    *,
    ...
    """Fetch Vercel AI Gateway /v1/models and return moor-shaped pricing.

    Vercel uses ``input`` / ``output`` field names; moor's picker expects
...
_DEFAULT_MOOR_INFERENCE_BASE = "https://inference-api.nousresearch.com"


def _resolve_moor_pricing_credentials() -> tuple[str, str]:
    """Return ``(api_key, base_url)`` for Moor Portal pricing.

    The Moor inference ``/v1/models`` endpoint exposes pricing without
    ...
    1. ``MOOR_INFERENCE_BASE_URL`` env override (staging / preview)
    ...
    try:
        ...
        env_base = _moor_inference_env_override()
    except Exception:
        env_base = None
    ...
    base_url = (env_base or creds_base or _DEFAULT_MOOR_INFERENCE_BASE).rstrip("/")
...
def get_pricing_for_provider(provider: str, *, force_refresh: bool = False) -> dict[str, dict[str, str]]:
    """Return live pricing for providers that support it (openrouter, moor, ai-gateway, novita)."""
    ...
    if normalized == "moor":
        ...
        if base_url:
            return fetch_models_with_pricing(
                ...
                base_url=base_url,
                force_refresh=force_refresh,
                # Sale chrome (pricing.original) is Moor Portal-only.
                include_sale_original=True,
            )
    return {}
...
def list_available_providers() -> list[dict[str, str]]:
    ...
    source of truth shared with ``moor model``, ``/model``, etc.).
    """
...
def probe_ollama_local_models(
    ...
    try:
        ...
        request_headers = {"User-Agent": _MOOR_USER_AGENT}
...
def curated_models_for_provider(
    ...
    *,
    ...
    # Try live API first (Codex, Moor, etc. all support /models)
...
_AGGREGATOR_PROVIDERS = frozenset(
    {"moor", "openrouter", "ai-gateway", "copilot", "kilocode"}
)
...
def _resolve_static_model_alias(
    ...
    try:
        from moor_cli.model_switch import MODEL_ALIASES
    except Exception:
        return None
...
def detect_static_provider_for_model(
    ...
    # If someone types `/model moor` or `/model anthropic`, treat it as a
    ...
    if resolved_provider not in {"custom", "openrouter"}:
        ...
        if (
            ...
            # curated list is ordered most-capable-first (e.g. Moor Portal),
            # entry [0] is the priciest flagship, and typing ``/model moor``
...
def normalize_provider(provider: Optional[str]) -> str:
    """Normalize provider aliases to Moor' canonical provider ids.

    Note: ``"auto"`` passes through unchanged — use
    ``moor_cli.auth.resolve_provider()`` to resolve it to a concrete
...
def _resolve_copilot_catalog_api_key() -> str:
    ...
    Resolution order:
      ...
      2. ``read_credential_pool("copilot")`` — a token (typically a
         ...
         populated by ``moor auth add copilot`` and by ``_seed_from_env``
         when the env var is set in ``~/.moor/.env``.
...
#   - "moor": curated list and Portal /models endpoint are the source of
...
def _model_dedup_key(model_id: str) -> str:
    ...
    try:
        from moor_cli.model_search import model_alias_canonical
...
def provider_model_ids(provider: Optional[str], *, force_refresh: bool = False) -> list[str]:
    ...
    Tries live API endpoints for providers that support them (Codex, Moor),
    falling back to static lists. For providers in ``_MODELS_DEV_PREFERRED``
    (opencode-go/zen, xiaomi, deepseek, smaller inference providers, etc.),
    models.dev entries are merged on top of curated so new models released
    on the platform appear in ``/model`` without a Moor release.
    """
    requested = str(provider or "").strip().lower()
    if requested == "ollama":
        if force_refresh:
    ...
    if normalized == "openai-codex":
        ...
        # a Moor release). Falls back to the hardcoded catalog if no token
    ...
    if normalized == "moor":
        # Try live Moor Portal /models endpoint
        try:
            from moor_cli.auth import fetch_moor_models, resolve_moor_runtime_credentials
            creds = resolve_moor_runtime_credentials()
            if creds:
                live = fetch_moor_models(api_key=creds.get("api_key", ""), inference_base_url=creds.get("base_url", ""))
                if live:
                    return live
        except Exception:
            pass
        # Live failed (or no creds). Fall back to the docs-hosted manifest
        # — NOT the in-repo _PROVIDER_MODELS["moor"] snapshot — so newly
        # added Portal models still surface without a Moor release.
        manifest_ids = get_curated_moor_model_ids()
        if manifest_ids:
            return manifest_ids
    ...
    if normalized in ("openai", "openai-api"):
        ...
        if api_key:
            ...
            # ``moor model`` shows.
            from moor_cli.providers import is_official_openai_host
...
#
# Cache strategy:
#   - One JSON file at $MOOR_HOME/provider_models_cache.json
...
def _provider_models_cache_path() -> Path:
    from moor_constants import get_moor_home
    return get_moor_home() / "provider_models_cache.json"
...
def _credential_fingerprint(provider: str) -> str:
    ...
    (codex, copilot, anthropic-via-claude-code, moor portal), the
    relevant tokens live in ``$MOOR_HOME/auth.json`` and external
    ...
    # env var, so it must change the fingerprint too or `moor config set
...
def _lmstudio_request_headers(api_key: Optional[str] = None) -> dict:
    ...
    headers = {"User-Agent": _MOOR_USER_AGENT}
...
_COPILOT_MODEL_ALIASES = {
    ...
    # Dash-notation fallbacks: Moor' default Claude IDs elsewhere use
...
_DEEPINFRA_MODELS_QUERY = "filter=true&sort_by=moor"
...
def _ollama_cloud_cache_path() -> Path:
    ...
    from moor_constants import get_moor_home
    return get_moor_home() / "ollama_cloud_models_cache.json"
    ...
    *,
    ...
    if normalized == "moa":
        try:
            ...
            return {
                ...
                "message": f"MoA preset `{requested}` was not found. Run `moor moa list`.",
            }
        except Exception as exc:
            return {
                "accepted": False, "persist": False, "recognized": False,
        ...
        if ollama_models is None:
            return {
                ...
                "message": (
                    ...
                    "Moor will save the model name, but local Ollama model discovery could not verify it."
                ),
            }
    ...
    if normalized == "custom" or normalized.startswith("custom:"):
        ...
        message = (
            ...
            f"Moor will still save `{requested}`, but the endpoint should expose `/models` for verification."
        )
        if api_mode == "anthropic_messages":
            message += (
    ...
    if normalized in {"openai-codex", "xai-oauth"}:
        ...
        # BEFORE the hidden-slug soft-accept below: the suffix is a Moor
        ...
        if catalog_models:
            ...
            return {
                ...
                "message": (
                    ...
                    f"{suggestion_text}"
                    "\n  MiniMax does not expose a /models endpoint, so Moor cannot verify the model name."
