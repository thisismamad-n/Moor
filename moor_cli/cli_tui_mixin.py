...
interactive CLI. Mixin bound onto ``MoorCLI`` via the MRO; cli.py-internal symbols are imported
...
class CLITuiMixin:
    ...
    def _render_scroll_list_panel(self, state, title, hint, labels, *, min_width, max_width, indent):
        ...
        """
        from cli import MoorCLI, _panel_box_width, _wrap_panel_text
    ...
    def _tui_handle_ignored_terminal_sequence(self, event):
        ...
        moor_cli.pt_input_extras registers focus reports (CSI I / CSI O) as Keys.Ignore at the
    ...
    def _tui_enter_while_busy(self, text: str, images: list, payload) -> None:
        ...
        """
        from cli import CLI_CONFIG, _ACCENT, _DIM, _RST, _cprint, _moor_home
        ...
        if _effective_mode == "queue":
            ...
            if redirected:
                ...
                try:
                    with open(_moor_home / "interrupt_debug.log", "a", encoding="utf-8") as _f:
                        _f.write(
    ...
    def _tui_collapse_paste(self, text: str, line_count: int, *, fallback: bool) -> str:
        """Save a large paste under ~/.moor/pastes and return the placeholder for the buffer."""
        from cli import _moor_home, datetime, logger
    ...
    def _tui_handle_prompt_stash(self, event):
        ...
        """
        from moor_cli.prompt_stash import ACTION_RESTORED, ACTION_STASHED, resolve_ctrl_s
        buf = event.app.current_buffer
    ...
    def _tui_set_base_style(self):
        ...
        self._tui_style_base = {
            ...
            'hint': '#888888 italic',
            'status-bar': 'bg:#090b10 #f1f5f9',
            'status-bar-strong': 'bg:#090b10 #38bdf8 bold',
            'status-bar-dim': 'bg:#090b10 #64748b',
            'status-bar-good': 'bg:#090b10 #10b981 bold',
            'status-bar-warn': 'bg:#090b10 #f59e0b bold',
            'status-bar-bad': 'bg:#090b10 #ef4444 bold',
            'status-bar-critical': 'bg:#090b10 #dc2626 bold',
            'status-bar-yolo': 'bg:#090b10 #ef4444 bold',
            'status-bar-session-title': 'bg:#2563eb #f1f5f9 bold',
            'input-rule': '#2563eb',
            'image-badge': '#38bdf8 bold',
            'completion-menu': 'bg:#090b10 #f1f5f9',
            'completion-menu.completion': 'bg:#090b10 #f1f5f9',
            'completion-menu.completion.current': 'bg:#1e293b #38bdf8',
            'completion-menu.meta.completion': 'bg:#090b10 #64748b',
            'completion-menu.meta.completion.current': 'bg:#1e293b #3b82f6',
            'clarify-border': '#2563eb',
            'clarify-title': '#38bdf8 bold',
            'clarify-question': '#f1f5f9 bold',
            'clarify-choice': '#94a3b8',
            'clarify-selected': '#38bdf8 bold',
            'clarify-active-other': '#38bdf8 italic',
            'clarify-answer': '#10b981',
            ...
            'voice-status': 'bg:#090b10 #38bdf8',
            'voice-status-recording': 'bg:#090b10 #ef4444 bold'}
