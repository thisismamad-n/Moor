"""
moor fallback — manage the fallback provider chain.
...
Subcommands:
  moor fallback [list]   Show the current fallback chain (default when no subcommand)
  moor fallback add      Pick provider + model via the same picker as `moor model`,
                           then append the selection to the chain
  moor fallback remove   Pick an entry to delete from the chain
  moor fallback clear    Remove all fallback entries

Storage: ``fallback_providers`` in ``~/.moor/config.yaml`` (top-level, list of
...
def _restore_auth_active_provider(value: Any) -> None:
    ...
    try:
        ...
        # `moor model` to fix it.  Don't fail the fallback add.
        pass
...
def cmd_fallback_list(args) -> None:  # noqa: ARG001
    ...
    if not chain:
        ...
        print()
        print("  Add one with:  moor fallback add")
        print()
        return
...
def cmd_fallback_add(args) -> None:
    """Launch the same picker as `moor model`, then append the selection to the chain."""
    from moor_cli.main import _require_tty, select_provider_and_model
    ...
    print("  `moor model` — select the provider + model you want as a fallback.")
    print()
    ...
    print()
    print("  Run `moor fallback list` to view, or `moor fallback remove` to delete.")
...
def cmd_fallback(args) -> None:
    """Top-level dispatcher for ``moor fallback [subcommand]``."""
