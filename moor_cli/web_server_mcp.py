...
Wraps the same config layer the CLI uses (moor_cli.mcp_config); stdio ``env``
...
def _normalize_mcp_server_create(body: MCPServerCreate) -> tuple[str, Dict[str, Any], Optional[str]]:
    ...
    """
    from moor_cli.mcp_config import _bearer_auth_headers, _strip_bearer_prefix
...
def _run_dashboard_mcp_oauth(flow, cfg: dict) -> None:
    ...
    from moor_cli.mcp_config import _oauth_tokens_present, _probe_single_server, _save_mcp_server
    try:
        ...
        try:
            ...
            with transaction, force_interactive_oauth(), dashboard_oauth_flow(flow):
                ...
                previous_entry = None
                try:
                    previous_entry = manager.remove(flow.server_name, moor_home=flow.moor_home)
                    ...
                    manager.restore_entry(flow.server_name, previous_entry, moor_home=flow.moor_home)
                    raise
        finally:
