...
def _strip_moor_path_lines(content: str) -> str:
    """Drop the ``# Moor Agent`` marker (+ its PATH line) and any moor PATH line; squash blank runs."""
    new_lines = []
    skip_next = False
...
def remove_path_from_shell_configs():
    """Remove Moor PATH entries from shell configuration files."""
    removed_from = []
    for config_path in (c for c in (Path.home() / n for n in _SHELL_RC_NAMES) if c.exists()):
        try:
            content = config_path.read_text(encoding="utf-8")
            new_content = _strip_moor_path_lines(content)
...
def remove_wrapper_script():
    ...
    def _unlink_ours(wrapper: Path) -> bool:
        # Only our wrapper (contains a moor_cli / moor-agent reference).
        content = wrapper.read_text(encoding="utf-8")
        return _unlink_if('moor_cli' in content or 'moor-agent' in content, wrapper)

    candidates = (
        bin_dir / name
        for bin_dir in (Path.home() / ".local" / "bin", Path("/usr/local/bin"))
        for name in ("moor", "moor-acp", "moor-agent"))
...
def uninstall_gateway_service():
    ...
    try:
        from moor_cli.gateway import kill_gateway_processes, find_gateway_pids
...
def _remove_systemd_gateway() -> bool:
    ...
    from moor_cli.gateway import _systemctl_cmd, get_service_name, get_systemd_unit_path
...
def _remove_launchd_gateway() -> bool:
    ...
    from moor_cli.gateway import get_launchd_plist_path
...
def _remove_windows_gateway() -> bool:
    ...
    from moor_cli import gateway_windows as gw
...
# MOOR_HOME / MOOR_GIT_BASH_PATH (HKCU\Environment), User-scope PATH entries
# (%LOCALAPPDATA%\moor\git\{cmd,bin,usr\bin}, ...\moor\node), PortableGit + Node copies
...
def _moor_path_markers(moor_home: Path, *, include_managed_bin: bool = False) -> list[str]:
    """Prefixes identifying moor-owned User-PATH entries (prefix match sweeps git\cmd, git\bin,
    ...
    subs = ("moor-agent", "git", "node", "venv") + (("bin",) if include_managed_bin else ())
...
def remove_path_from_windows_registry(moor_home: Path, *, include_managed_bin: bool = False) -> list[str]:
    """Strip moor-owned entries from User-scope PATH in the registry (see ``_moor_path_markers``)."""
    markers = tuple(m.lower() for m in _moor_path_markers(moor_home, include_managed_bin=include_managed_bin))
...
def remove_moor_env_vars_windows() -> list[str]:
    """Delete MOOR_HOME and MOOR_GIT_BASH_PATH from User-scope env vars."""
    def edit(winreg, key, removed):
        for name in ("MOOR_HOME", "MOOR_GIT_BASH_PATH"):
            try:
...
def remove_portable_tooling_windows(moor_home: Path) -> list[Path]:
    """Delete the PortableGit / Node / gateway-service dirs the Windows installer created under
    ``moor_home`` (isolated from any system Git / Node, so nothing else breaks)."""
    targets = (moor_home / sub for sub in ("git", "node", "gateway-service"))
...
def remove_windows_bin_launchers(*, windows: bool | None = None) -> list[Path]:
    """Delete the ``moor`` launchers install.ps1 staged in the managed ``bin`` dir: every mode
...
def _is_default_moor_home(moor_home: Path) -> bool:
    """Return True when ``moor_home`` points at the default (non-profile) root."""
    try:
        from moor_constants import get_default_moor_root
        return moor_home.resolve() == get_default_moor_root().resolve()
    except Exception:
        return False
...
def _uninstall_profile(profile) -> None:
    ...
    out to ``moor -p <name> gateway stop|uninstall`` because service names / unit paths derive
    from the current MOOR_HOME and can't be switched in-process."""
    name = profile.name
    log_info(f"Uninstalling profile '{name}'...")

    # 1. Gateway service, via `python -m moor_cli.main` (the `moor` wrapper may be half-gone).
    moor_invocation = [sys.executable, "-m", "moor_cli.main", "--profile", name]
    for subcmd in ("stop", "uninstall"):
        try:
            subprocess.run(
                moor_invocation + ["gateway", subcmd], capture_output=True, text=True,
    ...
    # 3. Wipe the profile's MOOR_HOME directory.
    _rmtree_step(profile.path, indent="  ", fully=False)


def run_gui_uninstall(args):
    """``moor uninstall --gui``: remove the desktop app's built artifacts, packaged bundle
    (best-effort) and Electron userData — never config/sessions/.env, the agent or its venv."""
    from moor_cli.gui_uninstall import agent_is_installed, gui_install_summary, uninstall_gui
    ...
    if not summary["gui_installed"]:
        print("No Moor Chat GUI installation was found.")
        print(f"  Checked: {moor_home}, and the standard app locations for this OS.")
        return

    print(color("This removes the Chat GUI only. The Moor agent stays installed.", Colors.CYAN))
    print()
    ...
    print()
    if agent_is_installed(moor_home):
        print(color("Kept intact:", Colors.GREEN, Colors.BOLD))
        print(f"  • The Moor agent at {moor_home / 'moor-agent'}")
        print(f"  • Your config, sessions, and secrets under {moor_home}")
        print()
    ...
    print()
    uninstall_gui(moor_home)

    print()
    _print_box("│            ✓ Chat GUI Uninstalled!                      │", Colors.GREEN)
    print()
    print("The Moor agent is still installed. Run 'moor' to use the CLI,")
    print("or 'moor uninstall' to remove the agent too.")
    print()


def run_uninstall(args):
    """Interactive/``--yes`` uninstall: full removes code and ``~/.moor/``; keep-data removes only
    ...
    if bool(getattr(args, "dry_run", False)):
        _print_uninstall_dry_run(
            project_root=project_root, moor_home=moor_home, full_uninstall=full_flag)
        return

    # Named profiles (only when uninstalling from the default root) are offered for cleanup too,
    # instead of leaving zombie MOOR_HOMEs and systemd units behind.
    is_default_profile = _is_default_moor_home(moor_home)
    ...
    print(f"  Config:  {moor_home / 'config.yaml'}")
    print(f"  Secrets: {moor_home / '.env'}")
    print(f"  Data:    {moor_home / 'cron/'}, {moor_home / 'sessions/'}, {moor_home / 'logs/'}")
    print()
    ...
    # services, alias wrappers, MOOR_HOME dirs) — otherwise they leave zombie services behind.
    ...
    print()
    if full_uninstall:
        print(color("⚠️  WARNING: This will permanently delete ALL Moor data!", Colors.RED, Colors.BOLD))
        print(color("   Including: configs, API keys, sessions, scheduled jobs, logs", Colors.RED))
        if remove_profiles:
            print(color(f"   Plus {n_profiles} profile(s): {profile_names}", Colors.RED))
    else:
        print("This will remove the Moor code but keep your configuration and data.")
    ...
    _perform_uninstall(
        project_root=project_root, moor_home=moor_home, full_uninstall=full_uninstall,
        remove_profiles=remove_profiles, named_profiles=named_profiles)


def _print_uninstall_dry_run(*, project_root: Path, moor_home: Path, full_uninstall: bool) -> None:
    ...
    if not full_uninstall:
        print(f"  • Keep Moor config/data: {moor_home}")
    else:
        print(f"  • Moor config/data: {moor_home}")
        profiles = _discover_named_profiles() if _is_default_moor_home(moor_home) else []
        if profiles:
...
def _perform_uninstall(
    *,
    ...
    the checkout -> (Windows) PortableGit/Node -> optionally ``$MOOR_HOME`` and named profiles."""
    print()
    ...
    # 2-3b. PATH entries, wrapper, Windows launchers, node symlinks. Windows: moor_home is
    #    %VAR%-expanded because install.ps1 writes literal C:\Users\<u>\...; moor\bin (launchers +
    ...
    windows = _is_windows()
    sweep_managed_bin = windows and full_uninstall and _is_default_moor_home(moor_home)
    for on_this_platform, label, remove, success_fmt, none_msg in (
        ...
        (windows, "Removing PATH entries from Windows User environment...",
         lambda: remove_path_from_windows_registry(
             Path(os.path.expandvars(str(moor_home))), include_managed_bin=sweep_managed_bin),
         "Removed from User PATH: {}", "No moor-owned PATH entries in User environment"),
        (windows, "Removing MOOR_HOME / MOOR_GIT_BASH_PATH User env vars...",
         remove_moor_env_vars_windows, "Removed User env var: {}", "No moor-set User env vars to remove"),
        (True, "Removing moor command...", remove_wrapper_script, "Removed {}", "No wrapper script found"),
        (windows, "Removing Windows moor launchers...",
         remove_windows_bin_launchers, "Removed {}", "No Windows moor launchers found"),
        (True, "Removing moor-managed node/npm/npx symlinks...",
         lambda: remove_node_symlinks(moor_home), "Removed {}", "No moor-managed node/npm/npx symlinks found"),
    ):
        if on_this_platform:
    ...
    #     .env (safe in keep-data mode); the packaged app + Electron userData live OUTSIDE MOOR_HOME.
    log_info("Removing desktop Chat GUI artifacts...")
    try:
        from moor_cli.gui_uninstall import uninstall_gui
        if not uninstall_gui(moor_home):
    ...
    if windows:
        _remove_step(
            ...
            lambda: remove_portable_tooling_windows(moor_home), "Removed {}",
    ...
    if full_uninstall:
        ...
        _rmtree_step(moor_home)
    else:
        log_info(f"Keeping configuration and data in {moor_home}")
...
def main(argv=None) -> int:
    """``python -m moor_cli.uninstall --mode <gui|lite|full>``. Imports only stdlib +
    ``moor_constants`` + ``moor_cli.colors``, so it runs under a bare system Python (no venv)."""
    import argparse
    parser = argparse.ArgumentParser(prog="python -m moor_cli.uninstall")
    parser.add_argument(
