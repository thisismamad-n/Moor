...
- Keep data: Remove code but keep ~/.moor/ (configs, sessions, logs)
"""
...
def remove_node_symlinks(moor_home: Path) -> list:
    ...
    symlinks node/npm/npx into the same directory as the ``moor`` command:
...
#
#   1. Sets User-scope env vars ``MOOR_HOME`` and ``MOOR_GIT_BASH_PATH``
...
#      like ``%LOCALAPPDATA%\moor\git\cmd``, ``%LOCALAPPDATA%\moor\git\bin``,
#      ``%LOCALAPPDATA%\moor\git\usr\bin``, ``%LOCALAPPDATA%\moor\node``.
#      Again not in any rc file — only accessible via the registry or the
#      .NET [Environment] API.
#   3. Downloads PortableGit to ``%LOCALAPPDATA%\moor\git\`` and Node to
#      ``%LOCALAPPDATA%\moor\node\`` as user-scoped, isolated copies.
#      These are ~200MB combined and serve no purpose after uninstall.
#   4. On the ``moor dashboard`` + gateway paths, drops files into
#      ``%LOCALAPPDATA%\moor\gateway-service\`` and sometimes
...
def _moor_path_markers(moor_home: Path) -> list[str]:
    ...
    # backslash-moor as a word-ish boundary.
    return markers
...
def _uninstall_profile(profile) -> None:
    ...
    remove its alias wrapper, and wipe its MOOR_HOME directory.

    We shell out to ``moor -p <name> gateway stop|uninstall`` because
    service names, unit paths, and plist paths are all derived from the
    current MOOR_HOME and can't be easily switched in-process.
    """
    import sys as _sys
    name = profile.name
    ...
    #    Use `python -m moor_cli.main` so we don't depend on a `moor`
    #    wrapper that may be half-removed mid-uninstall.
    moor_invocation = [_sys.executable, "-m", "moor_cli.main", "--profile", name]
    for subcmd in ("stop", "uninstall"):
        try:
            subprocess.run(
                moor_invocation + ["gateway", subcmd],
                capture_output=True,
...
def run_gui_uninstall(args):
    ...
    print(color("│         ⚕ Moor Chat GUI Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()
...
def run_uninstall(args):
    ...
    # offer to clean them up too instead of leaving zombie MOOR_HOMEs
    ...
    # full wipe (code + ~/.moor data); otherwise keep-data. Named profiles
    ...
    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│            ⚕ Moor Agent Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()
    ...
    # their alias wrappers, and wiping their MOOR_HOME dirs. Otherwise
...
def _print_uninstall_dry_run(*, project_root: Path, moor_home: Path, full_uninstall: bool) -> None:
    ...
    if full_uninstall:
        ...
        if _is_default_moor_home(moor_home):
...
def _perform_uninstall(
    *,
    ...
    ``moor`` wrapper + node symlinks → remove the desktop Chat GUI artifacts →
    delete the code checkout → (Windows) remove PortableGit/Node → optionally
    wipe ``$MOOR_HOME`` data and named profiles on full uninstall.
    """
    print()
    ...
    #     step-5 rmtree(moor_home) would sweep the in-tree artifacts anyway,
    #     but the packaged app + Electron userData live OUTSIDE MOOR_HOME and
    ...
    try:
        ...
        gui_removed = uninstall_gui(moor_home)
        if not gui_removed:
    ...
    try:
        if project_root.exists():
            # If the install is inside ~/.moor/, just remove the moor-agent subdir
            if moor_home in project_root.parents or project_root.parent == moor_home:
    ...
    #     under MOOR_HOME but they're install tooling, not config — safe to
    #     remove even in "keep data" mode.  If we're doing a full uninstall
    #     the step-5 rmtree(moor_home) would sweep them anyway; calling
    ...
    if _is_windows():
        ...
        removed_artifacts = remove_portable_tooling_windows(moor_home)
    ...
    if full_uninstall:
        ...
        #     alias wrapper. The profile MOOR_HOME dirs live under
        ...
        try:
            if moor_home.exists():
                ...
                log_success(f"Removed {moor_home}")
        except Exception as e:
            log_warn(f"Could not fully remove {moor_home}: {e}")
            log_info("You may need to manually remove it")
    else:
...
def main(argv=None) -> int:
    """Module entrypoint: ``python -m moor_cli.uninstall --mode <gui|lite|full>``.
    ...
    This module imports only stdlib + ``moor_constants`` + ``moor_cli.colors``
    (and lazily ``moor_cli.gui_uninstall``), so it runs fine under a bare
