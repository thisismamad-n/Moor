"""moor memory setup|status — configure memory provider plugins."""
...
def _provider_pip_dependencies(provider_name: str, declared: list) -> list:
    ...
    ``moor memory setup`` — if the update-time refresh only reinstalled the declared ``hindsight-client``,
    ...
    if provider_name == "hindsight":
        try:
            import json
            cfg_path = get_moor_home() / "hindsight" / "config.json"
...
def _install_dependencies(provider_name: str, *, force: bool = False) -> None:
    ...
    no-ops when nothing drifted) — how ``moor update`` heals a provider after a venv rebuild.
    ...
    satisfied ranges. This is how ``moor update`` heals the active memory provider after a venv
    ...
    # install_specs routes to the durable target on sealed hosted images (MOOR_LAZY_INSTALL_TARGET)
...
def _post_setup_hook(provider, config: dict) -> bool:
    ...
    if hasattr(provider, "post_setup"):
        provider.post_setup(str(get_moor_home()), config)
        return True
    return False
...
def cmd_setup_provider(provider_name: str) -> None:
    ...
    if not match:
        ...
        print("  Run 'moor memory setup' to see available providers.\n")
        return
...
def cmd_setup(args) -> None:
    ...
    if not providers:
        ...
        print("  Install a plugin to ~/.moor/plugins/ and try again.\n")
        return
    ...
    if provider_config and hasattr(provider, "save_config"):
        try:
            provider.save_config(provider_config, str(get_moor_home()))
        except Exception as e:
...
def _write_env_vars(
    ...
    ``save_env_value`` applies the shared gate (name regex, ``LD_PRELOAD``/``PYTHONPATH``/``MOOR_HOME``
    denylist, CR/LF stripping, atomic 0o600 writes). ``ValueError`` is reported and skipped so one
    bad key doesn't sink the batch; filesystem errors propagate. ``moor_home`` is applied via the
    context-local override, not ``os.environ``.
    """
    from moor_cli.config import save_env_value
    from moor_constants import reset_moor_home_override, set_moor_home_override

    token = set_moor_home_override(moor_home) if moor_home is not None else None
    try:
                ...
                if required_fields:
                    ...
                    for f in required_fields:
                        ...
                        if url and not is_set:
                            line += f"  → {url}"
                        print(line)
                print("  Note: systemd/gateway services do not inherit ~/.moor/.env —")
                print("        set any variables above in the service environment.")
        else:
            print("\n  Plugin:    NOT installed ✗")
            print(f"  Install the '{provider_name}' memory plugin to ~/.moor/plugins/")
