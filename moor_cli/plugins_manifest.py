...
Split out of :mod:`moor_cli.plugins`; validation warns and never fails a load.
"""
...
# and continue loading. ``capabilities``/``emits``/``listens``/``moor``/``depends`` are reserved.
...
def _plugins_debug() -> bool:
    from moor_cli import plugins as _origin
        ...
        for dep in manifests[k].requires_plugins:
            ...
            if resolved is None:
                logger.warning(
                    ...
                    "via ctx.has_plugin). Run `moor plugins enable %s` if it is installed.",
                    k, dep_id, dep_id,
                )
            elif resolved == k:
...
class PluginManifest:
    ...
    # VALIDATED AND SURFACED ONLY — Moor never auto-installs these (isolation design for the install seam
