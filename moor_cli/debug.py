...
Currently supports:
    moor debug share    Upload debug report (system info + logs) to a
                          ...
                          ``~/.moor/logs/*.log`` are not leaked into
                          the public paste service. Pass ``--no-redact``
                          to disable.
                          Pass ``--moor`` to upload instead to Moor-internal
                          storage (AWS S3) via a signed URL minted by the
                          Moor account service: the bundle is private
                          (viewable only by Moor staff / allowlisted mods via
...
def _pending_file() -> Path:
    """Path to ``~/.moor/pastes/pending.json``.
    ...
    """
    return get_moor_home() / "pastes" / "pending.json"
...
def delete_paste(url: str) -> bool:
    ...
    req = urllib.request.Request(
        ...
        headers={"User-Agent": "moor-agent/debug-share"},
    )
...
def _schedule_auto_delete(urls: list[str], delay_seconds: int = _AUTO_DELETE_SECONDS):
    ...
    The replacement is stateless: we append to ``~/.moor/pastes/pending.json``
...
def _upload_paste_rs(content: str) -> str:
    ...
    req = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "moor-agent/debug-share",
        },
    )
    with urllib.request.urlopen(req, timeout=30) as resp:
...
def _primary_log_path(log_name: str) -> Optional[Path]:
    ...
    from moor_cli.logs import LOG_FILES

    filename = LOG_FILES.get(log_name)
    return (get_moor_home() / "logs" / filename) if filename else None


# Logs written by a client process rather than by this backend. When the
# desktop app talks to a remote/docker/SSH backend, `moor debug share` runs
...
# ---------------------------------------------------------------------------
# Shared bundle collection (used by both the paste.rs and Moor-S3 paths)
# ---------------------------------------------------------------------------

# Bundle format identifier embedded in the Moor-S3 JSON envelope. The
# discord-support viewer keys off this string to parse the bundle.
_MOOR_BUNDLE_FORMAT = "moor-debug-share/1"
...
def collect_share_bundle(
    ...
    Moor-S3 path (``--moor``).  Centralising it guarantees the Moor bundle is
    built from the *same* force-redacted snapshots as the public paste path —
    redaction is the safety boundary, so the Moor path must never see raw
    logs.
...
def build_moor_bundle(bundle: dict[str, str], redact: bool = True) -> bytes:
    """Gzip-compress a :func:`collect_share_bundle` mapping into the Moor envelope.

    The JSON shape is what the discord-support viewer (Repo 3) parses::

        {"format": "moor-debug-share/1",
         "redacted": <bool>,
         "created": <iso8601>,
...
def build_debug_share(
    *,
    ...
    # shared collector so the paste.rs and Moor-S3 paths build identical,
...
def run_debug_share(args):
    ...
    if moor:
        _run_debug_share_moor(args, log_lines=log_lines, redact=redact)
        return
    ...
    print("To delete now:  moor debug delete <url>")

    print("\nShare these links with the Moor team for support.")


_MOOR_PRIVACY_NOTICE = """\
⚠️  --moor: This uploads your debug bundle to Moor-INTERNAL storage (AWS S3),
  ...
  • The bundle is viewable only by Moor staff (and allowlisted Discord mods)
...
def _run_debug_share_moor(args, *, log_lines: int, redact: bool) -> None:
    """Handle ``moor debug share --moor``: upload the bundle to Moor-S3.

    Collects the same force-redacted bundle as the paste path, gzips it into
    the Moor envelope, requests a signed URL from NAS, uploads, and prints the
    private viewer link. On any failure falls back to a clear error that
    suggests ``--local``.
    """
    from moor_cli.diagnostics_upload import share_to_moor

    print(_MOOR_PRIVACY_NOTICE)
    if not _confirm_upload(args):
        return
    if not redact:
        print(
    ...
    if redact:
        logger.info(
            "moor debug share --moor: applied force-mode redaction before upload"
        )
    blob = build_moor_bundle(bundle, redact=redact)

    print("Uploading to Moor diagnostics storage...")
    try:
        res = share_to_moor(blob)
    except Exception as exc:
        print(
            f"\nMoor upload failed: {exc}\n"
            "\nThe Moor diagnostics service may be unavailable or not yet "
            "provisioned.\n"
            "Run `moor debug share --local` to print the report instead, "
            "or `moor debug share` to upload to a public paste service.\n",
            file=sys.stderr,
        )
        sys.exit(1)

    view_url = res.get("viewUrl") or res.get("view_url")
    print("\nDebug bundle uploaded to Moor (private):")
    if view_url:
...
def run_debug(args):
    ...
    if subcmd == "share":
        ...
        print("  --moor       Upload to Moor-internal storage (private, staff-only,")
