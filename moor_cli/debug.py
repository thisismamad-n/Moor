...
_USER_AGENT = "moor-agent/debug-share"
...
# ``moor debug`` sweeps on entry (CLI-only users). Replaced a fork-and-sleep subprocess that
...
def _save_pending(entries: list[dict]) -> None:
    try:
        ...
        pass  # non-fatal — worst case the user runs ``moor debug delete`` manually
...
def collect_debug_report(
    *, log_lines: int = 200, dump_text: str = "",
    ...
    ``dump_text`` is pre-captured dump output; when empty, ``moor dump`` is run internally.
    """
    buf = io.StringIO()
...
# Moor-S3 envelope format id; the discord-support viewer keys off it.
...
def build_moor_bundle(bundle: dict[str, str], redact: bool = True) -> bytes:
    """Gzip a :func:`collect_share_bundle` mapping into the Moor envelope (shape parsed by the
    discord-support viewer — keep it stable)."""
    envelope = {"format": _MOOR_BUNDLE_FORMAT, "redacted": bool(redact),
...
def build_debug_share(
    ...
    Shared by ``moor debug share`` and the dashboard ``POST /api/ops/debug-share``. Blocking
...
def run_debug_share(args):
    ...
    if getattr(args, "moor", False):
        return _run_debug_share_moor(args, log_lines=log_lines, redact=redact)
...
def _run_debug_share_moor(args, *, log_lines: int, redact: bool) -> None:
    """``moor debug share --moor``: gzip the same bundle into the Moor envelope → Moor-S3."""
    ...
    bundle = collect_share_bundle(log_lines=log_lines, redact=redact)
    if redact:
        logger.info("moor debug share --moor: applied force-mode redaction before upload")
    print("Uploading to Moor diagnostics storage...")
    try:
        res = share_to_moor(build_moor_bundle(bundle, redact=redact))
    except Exception as exc:
        print(f"\nMoor upload failed: {exc}\n"
              "\nThe Moor diagnostics service may be unavailable or not yet provisioned.\n"
              "Run `moor debug share --local` to print the report instead, "
    ...
    print("\nShare this private link with the Moor team — only Moor staff "
          ...
          "  GitHub Issues        https://github.com/thisismamad-n/Moor/issues\n"
          "  GitHub Discussions   https://github.com/thisismamad-n/Moor/discussions")
...
def run_debug_delete(args):
    ...
    if not urls:
        print("Usage: moor debug delete <url> [<url> ...]\n"
              "  Deletes paste.rs pastes uploaded by 'moor debug share'.")
        return
    for url in urls:
        try:
            if delete_paste(url):
...
Options (share):
  ...
  --moor       Upload to Moor-internal storage (private, staff-only,
