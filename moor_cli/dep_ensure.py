...
from moor_constants import agent_browser_runnable, find_node_executable
from tools.environments.local import moor_subprocess_env

_IS_WINDOWS = platform.system() == "Windows"

_DEP_CHECKS = {
    # find_node_executable() rather than a bare which(): $MOOR_HOME/node is not on PATH, so
    ...
    "browser": lambda: (agent_browser_runnable(shutil.which("agent-browser")) or _has_system_browser()
                        or _has_moor_agent_browser() or _has_npx_agent_browser()),
...
def _has_moor_agent_browser() -> bool:
    from moor_constants import get_moor_home  # late import: tests patch moor_constants.get_moor_home
    home = get_moor_home()
    if _IS_WINDOWS:  # npm -g --prefix puts .cmd shims directly in the prefix dir
        return (home / "node" / "agent-browser.cmd").is_file()
    # install.sh installs into $MOOR_HOME/node/bin/ via npm -g --prefix; legacy git clones used node_modules/.bin/.
...
def ensure_dependency(dep: str, interactive: bool = True) -> bool:
    ...
    if shell == "powershell":
        ...
        if not ps_bin:
            ...
            return False
        cmd = [ps_bin, "-ExecutionPolicy", "Bypass", "-File", str(script), "-Ensure", dep, "-MoorHome", str(get_moor_home())]
    else:
        cmd = ["bash", str(script), "--ensure", dep]
    run_env = {**moor_subprocess_env(inherit_credentials=False), "IS_INTERACTIVE": "false"}
