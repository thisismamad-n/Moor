...
_DEP_CHECKS = {
    # find_node_executable() rather than a bare which(): $MOOR_HOME/node is
...
def _has_moor_agent_browser() -> bool:
    ...
    # install.sh installs globally into $MOOR_HOME/node/bin/ via npm -g --prefix
...
def ensure_dependency(
    ...
    if shell == "powershell":
        ...
        cmd = [
            ...
            "-File", str(script),
            "-Ensure", dep,
            "-MoorHome", str(get_moor_home()),
        ]
    else:
        cmd = ["bash", str(script), "--ensure", dep]

    run_env = moor_subprocess_env(inherit_credentials=False)
