...
`moor tools` and `moor setup tools` both enter this module.
Select a platform → toggle toolsets on/off → for newly enabled tools
that need API keys, run through provider-aware configuration.

Saves per-platform tool configuration to ~/.moor/config.yaml under
...
from moor_cli.moor_subscription import (
    MANAGED_FEATURE_COVERAGE_CATEGORY,
    MoorSubscriptionFeatures,
    apply_moor_managed_defaults,
    get_moor_subscription_features,
)
from moor_cli.moor_account import format_moor_portal_entitlement_message
from tools.tool_backend_helpers import MOOR_MANAGED_PROVIDER, fal_key_is_configured
...
def _post_setup_no_window_flags(*, streams_to_console: bool = False) -> int:
    ...
    ``moor tools post-setup <key>`` child. On Windows, every console child
    ...
    :func:`moor_cli._subprocess_compat.windows_hide_flags`) suppresses it
...
from moor_cli.cli_output import (  # noqa: E402 — late import block
...
# They're still in _MOOR_CORE_TOOLS (available at runtime if enabled),
# but the setup checklist won't pre-select them for first-time users.
#
# Video gen is off by default — it's a niche, paid, slow feature. Users
# who want it opt in via `moor tools` → Video Generation, which walks
...
# `moor tools` → X (Twitter) Search setup walks users through credential
# setup. The tool's check_fn means the schema still won't appear to the
# model if the credential later goes missing or expires.
_DEFAULT_OFF_TOOLSETS = {"homeassistant", "spotify", "discord", "discord_admin", "video", "video_gen", "x_search", "a2a"}


# Config-only capabilities: they appear in `moor tools` for provider/API-key
...
def _xai_credentials_present() -> bool:
    ...
    """
    try:
        from moor_cli.auth import _read_xai_oauth_tokens
...
# Platform-scoped toolsets: only appear in the `moor tools` checklist for
...
def _get_effective_configurable_toolsets():
    ...
    Without the dedupe, ``moor tools`` → "reconfigure existing" would
    list the same toolset twice.
    """
    result = list(CONFIGURABLE_TOOLSETS)
    seen = {ts_key for ts_key, _, _ in result}
    try:
        from moor_cli.plugins import discover_plugins, get_plugin_toolsets
...
def _checklist_toolset_keys(platform: str) -> Set[str]:
    """Return the toolset keys the ``moor tools`` checklist actually offers
    for ``platform``.
    ...
    so ``moor tools`` never claims to add or remove a toolset the user was
...
TOOL_CATEGORIES = {
    "tts": {
        ...
        "providers": [
            ...
            {
                "name": "Moor Subscription",
                "badge": "subscription",
                "tag": "Managed OpenAI TTS billed to your subscription",
                "env_vars": [],
                "tts_provider": "openai",
                "requires_moor_auth": True,
                "managed_moor_feature": "tts",
    ...
    "web": {
        ...
        #   - "Moor Subscription" — managed Firecrawl billed via Moor
        #     subscription (requires_moor_auth + override_env_vars).
    ...
    "image_gen": {
        ...
        #   - "Moor Subscription" — managed FAL billed via the Moor
        #     subscription (requires_moor_auth + override_env_vars).
        ...
        "providers": [
            {
                ...
                "env_vars": [],
                "requires_moor_auth": True,
                "managed_moor_feature": "image_gen",
    ...
    "video_gen": {
        ...
        "icon": "🎬",
        # "Moor Subscription" row mirrors the image_gen pattern — managed
        # FAL video generation billed via the Moor Portal.  Plugin-backed
        ...
        "providers": [
            {
                ...
                "env_vars": [],
                "requires_moor_auth": True,
                "managed_moor_feature": "video_gen",
                "override_env_vars": ["FAL_KEY"],
                # The underlying plugin backend — when the user picks
                # "Moor Subscription" we set video_gen.provider = "fal"
    ...
    "browser": {
        ...
        # backend, never on the paid Moor Subscription gateway row:
        #   - "Local Browser" — non-cloud option, no CloudBrowserProvider.
        #   - "Moor Subscription (Browser Use cloud)" — managed Browser Use
        #     billed via Moor subscription (requires_moor_auth +
        ...
        "providers": [
            ...
            {
                "name": "Moor Subscription (Browser Use cloud)",
                "badge": "subscription",
                "tag": "Managed Browser Use billed to your subscription",
                "env_vars": [],
                "browser_provider": "browser-use",
                "requires_moor_auth": True,
                "managed_moor_feature": "browser",
        ...
        "providers": [
            {
                ...
                "env_vars": [
                    ...
                    # extra keys required. Set MOOR_CUA_DRIVER_CMD to use a
        ...
        "providers": [
            {
                ...
                "env_vars": [
                    {"key": "MOOR_LANGFUSE_PUBLIC_KEY", "prompt": "Langfuse public key (pk-lf-...)", "url": "https://cloud.langfuse.com"},
                    {"key": "MOOR_LANGFUSE_SECRET_KEY", "prompt": "Langfuse secret key (sk-lf-...)", "url": "https://cloud.langfuse.com"},
                ],
                "post_setup": "langfuse",
            },
            {
                ...
                "env_vars": [
                    {"key": "MOOR_LANGFUSE_PUBLIC_KEY", "prompt": "Langfuse public key (pk-lf-...)"},
                    {"key": "MOOR_LANGFUSE_SECRET_KEY", "prompt": "Langfuse secret key (sk-lf-...)"},
                    {"key": "MOOR_LANGFUSE_BASE_URL", "prompt": "Langfuse server URL (e.g. http://localhost:3000)", "default": "http://localhost:3000"},
                ],
                "post_setup": "langfuse",
            },
        ],
...
# provider+model picker like `moor model` — NOT this single-key prompt, so
...
def _cua_version_summary(raw: str, *, limit: int = 120) -> str:
    ...
    A binary selected by ``MOOR_CUA_DRIVER_CMD`` is not obliged to answer
...
def _pip_install(
    ...
    *,
    ...
    # Managed uv first: $MOOR_HOME/bin is never on PATH, so a bare which()
...
def install_cua_driver(
    ...
    * ``upgrade=True`` — always re-run the installer (or call ``cua-driver
      update`` if the binary supports it). Used by ``moor update`` and
      by ``moor computer-use install --upgrade``.
    ...
    falling through to the full upstream installer. ``moor update`` sets
    ...
    a further ~600s wait on Windows). ``moor computer-use install
    ...
    ``moor update`` already prints a contextual line before its update
    ...
    if system not in ("Darwin", "Windows", "Linux"):
        if upgrade:
            # Silent on unsupported platforms — `moor update` calls this
    ...
    if override and not binary:
        _print_warning(
            "    MOOR_CUA_DRIVER_CMD does not resolve to an executable: "
            f"{override}"
        )
        _print_info(
    ...
    # contract landed, with `moor update` declining to refresh).
    ...
    if not _cua_install_target_writable():
        ...
        _print_info(
            "    Run `moor computer-use install --upgrade` from an admin account to update it."
        )
        return bool(binary)
    ...
    # caller: `moor update` (require_confirmed_update=True) keeps the
    ...
    # `moor computer-use install --upgrade` falls through and re-runs the
    # installer as before.
    ...
    if binary and not repair_existing:
        ...
        if _state is None and require_confirmed_update:
            ...
            _print_info(
                "    Force a refresh with: moor computer-use install --upgrade"
            )
            return True
    ...
    if ok and repair_existing:
        ...
        if not repaired.get("ready"):
            ...
            _print_info("    Run: moor computer-use doctor")
            return False
    if ok and before:
        try:
...
def _run_cua_driver_installer(
    ...
    * Windows       → ``powershell -NoProfile -ExecutionPolicy Bypass -Command
    ...
    try:
        # When not verbose (e.g. `moor update`'s refresh), capture the
        ...
        if verbose:
            ...
            # Preserve the full installer output. During `moor update`,
            # sys.stdout is the mirroring _UpdateOutputStream whose `_log`
            # handle is ~/.moor/logs/update.log — write straight to it so
...
def _ensure_browser_use_cli(*, verbose_hints: bool = False) -> None:
    ...
    Firecrawl, and the Moor-managed cloud rows all execute through
    ...
    check — only the moor-managed ``$MOOR_HOME/bin`` copy does.
    ...
    if post_setup_key in {"agent_browser", "browserbase"}:
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                _print_info("    Change via stt.local.model in ~/.moor/config.yaml")
            else:
        ...
        _print_info("    Switch voices by setting tts.piper.voice in ~/.moor/config.yaml")
        ...
        # Run the full `moor auth spotify` flow — if the user has no
        # client_id yet, this drops them into the interactive wizard
        # (opens the Spotify dashboard, prompts for client_id, persists
        # to ~/.moor/.env), then continues straight into PKCE. If they
        # already have an app, it skips the wizard and just does OAuth.
        from types import SimpleNamespace
        try:
            from moor_cli.auth import login_spotify_command
        except Exception as exc:
            _print_warning(f"    Could not load Spotify auth: {exc}")
            _print_info("    Run manually: moor auth spotify")
            return
        ...
        try:
            ...
            # toolset enable; they can retry with `moor auth spotify`.
            _print_warning(f"    Spotify login did not complete: {exc}")
            _print_info("    Run later: moor auth spotify")
        except Exception as exc:
        ...
        try:
            from moor_cli.plugins_cmd import _get_enabled_set, _save_enabled_set
            ...
            _print_info("    Run manually: moor plugins enable observability/langfuse")
        _print_info("    Restart Moor for tracing to take effect.")
        _print_info("    Verify: moor plugins list")
        ...
        try:
            from moor_cli.auth import get_xai_oauth_auth_status
        ...
        try:
            ...
            _print_info("    Run later: moor auth add xai-oauth   (or set XAI_API_KEY)")
            return
        ...
        idx = prompt_choice(
            ...
            choices=[
                ...
                "Skip — configure later via `moor auth add xai-oauth`",
            ],
            default=0,
        )
        if idx == 0:
            if _run_xai_oauth_login_from_setup():
                ...
                _print_warning(
                    ...
                    "Run later: moor auth add xai-oauth"
                )
        elif idx == 1:
            ...
            if api_key:
                ...
                _print_warning(
                    "    No API key provided. Run later: moor auth add xai-oauth"
                )
        else:
...
def valid_post_setup_keys() -> Set[str]:
    ...
    ``post_setup``). This is the allowlist the ``moor tools post-setup``
...
def run_post_setup_command(args) -> int:
    """``moor tools post-setup <key>`` — non-interactive post-setup runner.
    ...
    if not key:
        _print_error("Usage: moor tools post-setup <key>")
        return 2
...
def _exempt_explicit_platform_native(
    ...
    the composite they chose (e.g. ``moor-discord``, which contains those
...
#:
#: Saving ``moor tools`` (or one toggle in the desktop Toolsets UI) replaces
...
#: has touched the picker, while everyone still on ``[moor-cli]`` inherits it
...
def _enable_recently_shipped_toolsets(
    ...
    Either way of saying no outlives this: unchecking in ``moor tools``
    ...
    default_ts = plat_info["default_toolset"] if plat_info else f"moor-{platform}"
    ...
    for ts_key in sorted(_RECENTLY_SHIPPED_TOOLSETS):
        ...
        # narrow composites (moor-acp, moor-webhook) stay narrow.
...
def _get_platform_tools(
    ...
    *,
    ...
    # ``moor-discord``) is an opt-in to the platform's native default-off
    ...
    # explicit config like ``[moor-cli, a2a]`` must survive filtering just
    ...
    # "moor-cli" (which include all _MOOR_CORE_TOOLS) cause disabled
    ...
    if has_explicit_config:
        ...
        # ``[moor-cli, spotify]`` after enabling Spotify via ``moor
        ...
        # (e.g. "moor-cli") to individual tool names and reverse-mapping.
        all_tool_names = set()
        for ts_name in toolset_names:
        ...
        # through ``moor tools`` to flip the toolset on. Only fires when
    ...
    # otherwise saving via `moor tools` (which flips has_explicit_config
    # to True) silently drops them.
    _plat_info = PLATFORMS.get(platform)
    _default_ts = _plat_info["default_toolset"] if _plat_info else f"moor-{platform}"
    ...
    skip |= {k for k in TOOLSETS if k.startswith("moor-")}
    ...
    # shipped as a bundled plugin but user must opt in via `moor tools`
    # so we don't ship 7 Spotify tool schemas to users who don't use it).
    # A plugin toolset is "known" for a platform once `moor tools`
    ...
    if plugin_ts_keys:
        ...
        for pts in plugin_ts_keys:
            if pts in toolset_names:
                ...
                # New plugin not yet seen by moor tools — default enabled
                enabled_toolsets.add(pts)
    ...
    # JSON-array string (e.g. "['memory']") from `moor config set` or a
    ...
    # is invalid (e.g. a migration or hand-edit left `moor` instead of
    # `moor-cli`), resolve_toolset() returns [] for each and the platform ends
    ...
    # at runtime, not only during the next `moor update`/`moor doctor`.
    ...
    if isinstance(_explicit, list) and _explicit:
        ...
        if (
            ...
            logger.warning(
                ...
                "name(s): %s) - tools will be unavailable. Run `moor tools` "
...
def _save_platform_tools(config: dict, platform: str, enabled_toolset_keys: Set[str]):
    ...
    # Also exclude platform default toolsets (moor-cli, moor-telegram, etc.)
    ...
    # Opening `moor tools` is the user's opt-in to reconfigure tools, so treat
...
def _toolset_has_keys(
    ...
    *,
    ...
    features: Optional[MoorSubscriptionFeatures] = None,
) -> bool:
    ...
    if ts_key in {"web", "image_gen", "video_gen", "tts", "stt", "browser"}:
        if features is None:
            features = get_moor_subscription_features(
                config, force_fresh=force_fresh
            )
        feature = features.features.get(ts_key)
        if feature and (feature.available or feature.managed_by_moor):
            return True
...
def _prompt_toolset_checklist(
    ...
    *,
    ...
    from moor_cli.curses_ui import curses_checklist
    from toolsets import resolve_toolset
...
# ("Moor Subscription" managed-gateway entry, "Firecrawl Self-Hosted")
...
# ("Moor Subscription", "Local Browser", "Camofox") — see the comment block
...
def _visible_providers(
    ...
    *,
    ...
    Moor-managed Tool Gateway rows (``managed_moor_feature``) are always
    shown — even to logged-out / unentitled users — so the picker advertises
    that the capability exists.  Selecting one drives an inline Moor Portal
    login + entitlement check (see ``_configure_provider``); the row only
    *activates* the gateway once paid access is confirmed.
    """
    if features is None:
        features = get_moor_subscription_features(config, force_fresh=force_fresh)
    ...
    for provider in cat.get("providers", []):
        # Moor-managed Tool Gateway rows stay visible regardless of auth —
        # selecting one drives an inline Portal login. A `requires_moor_auth`
        # row that is NOT a managed gateway feature (pure pre-auth UX) is
        # still hidden until the user is logged in.
        if (
            provider.get("requires_moor_auth")
            and not provider.get("managed_moor_feature")
            and not features.moor_auth_present
        ):
            continue
        ...
        if (
            pool_only
            and provider.get("managed_moor_feature") == "video_gen"
    ...
    # later) so the picker lists them alongside FAL / Moor Subscription.
    ...
    # remaining hardcoded rows ("Moor Subscription", "Firecrawl
    ...
    # the hardcoded "Moor Subscription" / "Local Browser" / "Camofox" rows
...
def _hidden_moor_gateway_message(
    cat: dict,
    config: dict,
    capability: str,
    *,
    ...
    """Deprecated: Moor Tool Gateway rows are no longer hidden.

    Previously this returned a "log in / upgrade" banner shown above a
    category when its Moor-managed rows were filtered out for unentitled
    ...
    (``ensure_moor_portal_access``). Kept as a no-op so call sites stay simple;
...
_POST_SETUP_INSTALLED: dict = {
    ...
    # who toggles the toolset on via `moor tools` gets a silent no-op
...
# Python dependencies installed explicitly through ``moor tools`` are not
...
def active_restorable_python_tool_dependencies() -> list[str]:
    """Return ``moor tools`` Python dependencies present in this runtime."""
    return [
        name
...
def _agent_browser_installed() -> bool:
    ...
    from moor_cli.moor_subscription import _local_browser_runnable

    # The install hook runs in a spawned ``moor tools post-setup`` process,
...
def _cloud_agent_browser_installed() -> bool:
    ...
    from moor_cli.moor_subscription import _has_agent_browser
...
def provider_readiness_status(
    ...
    *,
    ...
    - ``"needs_auth"``  — needs a sign-in: Moor Portal login/entitlement for
    ...
    zero-env-var row, including logged-out Moor Subscription rows).

    ``features`` (a ``MoorSubscriptionFeatures``) can be passed to avoid
    ...
    managed_feature = provider.get("managed_moor_feature")
    if provider.get("requires_moor_auth") or managed_feature:
        if features is None:
...
def _configure_tool_category(
    ...
    *,
    ...
    hidden_moor_message = _hidden_moor_gateway_message(
        cat,
        config,
        f"the Moor Subscription provider for {name}",
        force_fresh=force_fresh,
    )
    ...
    if len(providers) == 1:
        ...
        if hidden_moor_message:
            for line in hidden_moor_message.splitlines():
        ...
        # When the user is logged into Moor, surface a marker on providers
        # whose access is included in their subscription so it's visually
        # obvious which options cost extra vs. cost nothing on top of Moor.
        try:
            _moor_logged_in = bool(
                get_moor_subscription_features(
                    config,
                    force_fresh=force_fresh,
                ).moor_auth_present
        ...
        for p in providers:
            ...
            # Mark Moor-managed entries. Logged-in paid subscribers get the
            # "included" star; everyone else gets a "via Moor Portal" hint so
            ...
            sub_marker = ""
            if p.get("managed_moor_feature"):
                if _moor_logged_in:
                    sub_marker = "  ★ Included with your Moor subscription"
                else:
                    sub_marker = "  ★ via Moor Portal (login on select)"
...
def _is_provider_active(
    ...
    *,
    ...
    if plugin_name and not provider.get("managed_moor_feature"):
        # Managed (Moor-subscription) entries fall through to the
    ...
    if video_plugin_name and not provider.get("managed_moor_feature"):
    ...
    if managed_feature:
        ...
        if managed_feature == "image_gen":
            ...
            if isinstance(image_cfg, dict):
                ...
                if configured_provider not in {None, "", "fal", MOOR_MANAGED_PROVIDER}:
                    return False
                if (
                    configured_provider != MOOR_MANAGED_PROVIDER
                    and image_cfg.get("use_gateway") is not None
                    and not is_truthy_value(image_cfg.get("use_gateway"), default=False)
                ):
                    return False
            return feature.managed_by_moor
        ...
        if provider.get("tts_provider"):
            return (
                feature.managed_by_moor
                and cfg_get(config, "tts", "provider")
                in {provider["tts_provider"], MOOR_MANAGED_PROVIDER}
            )
        if provider.get("stt_provider"):
            return (
                feature.managed_by_moor
                and cfg_get(config, "stt", "provider")
                in {provider["stt_provider"], MOOR_MANAGED_PROVIDER}
            )
        if "browser_provider" in provider:
            ...
            return feature.managed_by_moor and current in {
                provider["browser_provider"],
                MOOR_MANAGED_PROVIDER,
            }
        if provider.get("web_backend"):
            current = cfg_get(config, "web", "backend")
            return (
                feature.managed_by_moor
                and current in {provider["web_backend"], MOOR_MANAGED_PROVIDER}
...
def _select_plugin_image_gen_provider(plugin_name: str, config: dict, *, use_gateway: bool = False) -> None:
    ...
    ``use_gateway=True`` marks a provider picked through the Moor-managed
    flow: the stored selection becomes ``image_gen.provider: moor`` (the
    ...
    img_cfg["provider"] = MOOR_MANAGED_PROVIDER if use_gateway else plugin_name
...
# Kept in sync with the dashboard selects (moor_cli/web_server.py
...
def _select_plugin_video_gen_provider(plugin_name: str, config: dict, *, use_gateway: bool = False) -> None:
    ...
    ``video_gen.provider: moor``; BYOK picks store the plugin name; any
    ...
    vid_cfg["provider"] = MOOR_MANAGED_PROVIDER if use_gateway else plugin_name
...
def _write_provider_config(provider: dict, config: dict, *, managed_feature) -> None:
    ...
    post-setup hooks, gate on Moor auth, or run interactive model pickers.
    ...
    category. Managed "Moor Subscription" rows write ``moor``; BYOK rows
    ...
    legacy shim (use_gateway: true ⇒ moor) cannot override the new choice.
    """
    def _set_selection(section_key: str, name_key: str, vendor_value) -> None:
        ...
        section[name_key] = (
            MOOR_MANAGED_PROVIDER if managed_feature else vendor_value
        )
    ...
    # image_gen/video_gen "Moor Subscription" rows carry only
    # managed_moor_feature) still persist the "moor" selection.
    if managed_feature and managed_feature not in {"web", "tts", "stt", "browser"}:
        section = config.setdefault(managed_feature, {})
        if isinstance(section, dict):
            section["provider"] = MOOR_MANAGED_PROVIDER
            section.pop("use_gateway", None)
...
def apply_provider_selection(ts_key: str, provider_name: str, config: dict) -> None:
    ...
    post-setup hooks, gate on Moor Portal auth, or run interactive model
    ...
    # flow). Managed picks store the "moor" selection.
    plugin_name = provider.get("image_gen_plugin_name")
    if plugin_name:
        img_cfg = config.setdefault("image_gen", {})
        if not isinstance(img_cfg, dict):
            ...
            config["image_gen"] = img_cfg
        img_cfg["provider"] = (
            MOOR_MANAGED_PROVIDER if managed_feature else plugin_name
        )
        img_cfg.pop("use_gateway", None)
    ...
    if video_plugin:
        ...
        if not isinstance(vid_cfg, dict):
            ...
            config["video_gen"] = vid_cfg
        vid_cfg["provider"] = (
            MOOR_MANAGED_PROVIDER if managed_feature else video_plugin
        )
        vid_cfg.pop("use_gateway", None)
...
def _configure_provider(
    ...
    *,
    ...
    # Moor-managed Tool Gateway backends are always listed (see
    # _visible_providers), but only *activate* once the user has paid Moor
    ...
    # "enable all tools" prompt (that lives in `moor model`).
    if managed_feature:
        from moor_cli.moor_subscription import (
            MANAGED_FEATURE_COVERAGE_CATEGORY,
            ensure_moor_portal_access,
        )

        if not ensure_moor_portal_access(
            capability=f"{provider.get('name', 'the Moor Tool Gateway')}",
            coverage_category=MANAGED_FEATURE_COVERAGE_CATEGORY.get(managed_feature),
        ):
            _print_warning(
                "  Not enabled — Moor Portal access is required for this backend."
            )
            return

    # Pure pre-auth UX rows (requires_moor_auth without a managed gateway
    # feature) keep the old gate. Managed rows are handled by the inline
    # login above, so don't double-check them here.
    if provider.get("requires_moor_auth") and not managed_feature:
        features = get_moor_subscription_features(config, force_fresh=force_fresh)
        entitled = bool(
            features.account_info and features.account_info.paid_service_access is True
        )
        if not features.moor_auth_present or not entitled:
            message = format_moor_portal_entitlement_message(
                features.account_info,
                capability=f"{provider.get('name', 'Moor Subscription')}",
            )
            _print_warning(
                f"  {message or 'Moor Subscription is only available after logging into Moor Portal.'}"
            )
            return
    ...
    if provider.get("tts_provider"):
        ...
        tts_cfg["provider"] = (
            MOOR_MANAGED_PROVIDER if managed_feature else provider["tts_provider"]
        )
    ...
    if not env_vars:
        ...
        if managed_feature:
            _print_info("  Requests for this tool will be billed to your Moor subscription.")
        ...
        if backend:
            ...
            # explicit selection: "moor" for a managed row, "fal" for BYOK.
            img_cfg = config.setdefault("image_gen", {})
            if isinstance(img_cfg, dict):
                img_cfg["provider"] = (
                    MOOR_MANAGED_PROVIDER if managed_feature else "fal"
                )
                img_cfg.pop("use_gateway", None)
    ...
    # Moor-managed sibling, show a single dim hint so users know
    # they can avoid the key entirely via a Portal subscription.
    # Suppressed when the user is already authed to Moor.
    _show_portal_hint = False
    if env_vars and not managed_feature and not provider.get("requires_moor_auth"):
        try:
            ...
            for _cat_key, _cat in TOOL_CATEGORIES.items():
                ...
                if provider in _providers and any(
                    sib.get("managed_moor_feature") for sib in _providers
                ):
                    _has_managed_sibling = True
                    break
            if _has_managed_sibling:
                _features = get_moor_subscription_features(
                    config,
                    force_fresh=force_fresh,
                )
                _show_portal_hint = not _features.moor_auth_present
        except Exception:
            _show_portal_hint = False

    if _show_portal_hint:
        _print_info("  Available through Moor Portal subscription.")
...
def _configure_vision_backend() -> None:
    ...
    provider + model — the same surface as ``moor model`` — or point at a
...
def _configure_vision_provider_model(config: dict, vision_cfg: dict) -> None:
    ...
    substrate — so this picker lists exactly what the ``moor model`` aux-task
    ...
    """
    try:
        from moor_cli.inventory import (
    ...
    if not providers:
        _print_warning(
            ...
            "with `moor model`, then re-run this."
        )
        return
...
def _reconfigure_provider(
    ...
    *,
    ...
    # Same inline Moor Portal login + entitlement gate as _configure_provider:
    ...
    # provider string per category — "moor" for managed rows, the vendor name
    # for BYOK rows — and drops any legacy use_gateway key so the read-time
    # shim (use_gateway: true ⇒ moor) cannot override the fresh pick.
    if provider.get("tts_provider"):
        tts_cfg = config.setdefault("tts", {})
        tts_cfg["provider"] = (
            MOOR_MANAGED_PROVIDER if managed_feature else provider["tts_provider"]
    ...
    if provider.get("stt_provider"):
        stt_cfg = config.setdefault("stt", {})
        stt_cfg["provider"] = (
            MOOR_MANAGED_PROVIDER if managed_feature else provider["stt_provider"]
        )
    ...
    if "browser_provider" in provider:
        ...
        browser_cfg = config.setdefault("browser", {})
        if managed_feature:
            browser_cfg["cloud_provider"] = MOOR_MANAGED_PROVIDER
            _print_success(f"  Browser cloud provider set to: {bp or 'moor'}")
        elif bp == "local":
    ...
    if provider.get("web_backend"):
        ...
        web_cfg["backend"] = (
            MOOR_MANAGED_PROVIDER if managed_feature else provider["web_backend"]
        )
    ...
    if not env_vars:
        ...
        if backend:
            ...
            if backend == "fal":
                img_cfg = config.setdefault("image_gen", {})
                if isinstance(img_cfg, dict):
                    # A managed (Moor Subscription) row also carries
                    # imagegen_backend="fal" — store the "moor" selection
    ...
    if backend:
        ...
        if backend == "fal":
            ...
            if isinstance(img_cfg, dict):
                ...
                # never clobber a Moor-managed pick back onto direct keys.
                img_cfg["provider"] = (
                    MOOR_MANAGED_PROVIDER if managed_feature else "fal"
                )
                img_cfg.pop("use_gateway", None)
...
def _reconfigure_simple_requirements(ts_key: str):
    ...
    if ts_key == "vision":
        ...
        # `moor model`). Run it directly so reconfigure doesn't fall back to
...
def tools_command(args=None, first_install: bool = False, config: dict = None):
    """Entry point for `moor tools` and `moor setup tools`.
    ...
    if first_install:
        for pkey in enabled_platforms:
            ...
            auto_configured = apply_moor_managed_defaults(
                config,
            ...
            for ts_key in sorted(auto_configured):
                ...
                print(color(f"  ✓ {label}: using your Moor subscription defaults", Colors.GREEN))
    ...
    print(color(f"  Tool configuration saved to {display_moor_home()}/config.yaml", Colors.DIM))
    print(color("  Changes take effect on next 'moor' or gateway restart.", Colors.DIM))
    print()
