...
#:
#: Not gated on a Moor sign-in here: the six ``bfl_flux3_*`` tools carry
#: ``check_fn=check_bfl_requirements``, so an enabled toolset still ships zero
#: schemas to a user with no Moor credential — the same split Home Assistant
    ...
    *,
    ...
    if managed_feature:
        ...
        if "browser_provider" in provider:
            ...
            return feature.managed_by_moor and provider["browser_provider"] == current
        if provider.get("web_backend"):
            current = cfg_get(config, "web", "backend")
            return feature.managed_by_moor and current == provider["web_backend"]
        return feature.managed_by_moor
...
def _select_plugin_image_gen_provider(plugin_name: str, config: dict, *, use_gateway: bool = False) -> None:
    ...
    provider picked through the Moor-managed flow must keep routing through
    the gateway. Hardcoding ``False`` here silently flipped Moor-managed FAL
...
def _write_provider_config(provider: dict, config: dict, *, managed_feature) -> None:
    ...
    NOT prompt for env vars, run post-setup hooks, gate on Moor auth, or run
...
def _configure_mcp_tools_interactive(config: dict):
    ...
    for server_name, tools in server_tools.items():
        ...
        # tools.include across the codebase (catalog installs, moor mcp
