...
from moor_cli.config import cfg_get, load_config, save_config, get_env_value
from moor_cli.moor_subscription import (
    MoorSubscriptionFeatures, apply_moor_managed_defaults, get_moor_subscription_features)
...
# OFF by default for new installs (still in _MOOR_CORE_TOOLS; the checklist won't pre-select them). x_search
...
# Config-only capabilities: provider setup in `moor tools` (TOOL_CATEGORIES) but not model toolsets — zero
...
def _checklist_toolset_keys(platform: str) -> Set[str]:
    """Toolset keys the ``moor tools`` checklist offers for ``platform`` (mirrors ``_prompt_toolset_checklist``);
...
def _platform_default_toolset(platform: str) -> str:
    """Composite toolset a platform falls back to (plugin platforms derive ``moor-<platform>``)."""
    return PLATFORMS[platform]["default_toolset"] if platform in PLATFORMS else f"moor-{platform}"
...
def _row(name: str, badge: str = "", tag: str = "", env_vars: list = (), **markers) -> dict:
    """One TOOL_CATEGORIES provider row; ``markers`` are the ``*_provider`` / ``post_setup`` / Moor keys."""
    row = {"name": name}
    if badge:
        row["badge"] = badge
    if tag:
...
_LANGFUSE_PUBLIC = ("MOOR_LANGFUSE_PUBLIC_KEY", "Langfuse public key (pk-lf-...)")
_LANGFUSE_SECRET = ("MOOR_LANGFUSE_SECRET_KEY", "Langfuse secret key (sk-lf-...)")
...
TOOL_CATEGORIES = {
    "tts": {
        ...
        "providers": [
            ...
            _row("Moor Subscription", "subscription", "Managed OpenAI TTS billed to your subscription", tts_provider="openai",
                 **_MOOR, managed_moor_feature="tts", override_env_vars=["VOICE_TOOLS_OPENAI_KEY", "OPENAI_API_KEY"]),
    ...
    "stt": {
        ...
        "providers": [
            ...
            _row("Moor Subscription", "subscription", "Managed OpenAI transcription billed to your subscription",
                 stt_provider="openai", **_MOOR, managed_moor_feature="stt",
    ...
    "web": {
        ...
        "providers": [
            {"name": "Moor Subscription", "badge": "subscription", "tag": "Managed web search and extract billed to your subscription",
    ...
    "image_gen": {
        ...
        # _plugin_image_gen_providers(). Only the managed "Moor Subscription" row lives here: ONE row for the
        # FAL, Krea and Portal gateways, whose union catalog is `imagegen_backend: "moor"`; the stored model id
        # picks the gateway at run time (tools/image_generation_managed.py).
        "providers": [
            _row("Moor Subscription", "subscription",
                 "Managed image generation (FAL, Krea 2, Moor Portal models) billed to your subscription", **_MOOR,
                 managed_moor_feature="image_gen", override_env_vars=["FAL_KEY"], imagegen_backend="moor"),
        ],
    },
    "video_gen": {
        "name": "Video Generation", "icon": "🎬",
        # Mirrors image_gen: managed FAL video billed via the Moor Portal. Plugin-backed rows (FAL BYOK, xAI, …)
        ...
        "providers": [
            _row("Moor Subscription", "subscription", "Managed FAL video generation billed to your subscription", **_MOOR,
                 managed_moor_feature="video_gen", override_env_vars=["FAL_KEY"], video_gen_plugin_name="fal"),
        ],
    },
    "x_search": {
        "name": "X (Twitter) Search", "setup_title": "Select xAI Credential Source",
        "setup_note": (
            "Moor routes X searches through xAI's built-in x_search Responses tool for read-only public X "
    ...
    "browser": {
        ...
        # first so a fresh install's Enter lands on the free local backend (index 0), never on the paid Moor row.
        ...
        "providers": [
            ...
            _row("Lightpanda", "free · local · no Chromium", "Zig headless browser spawned by Moor, text-only (no screenshots)",
            ...
            _row("Moor Subscription (Browser Use cloud)", "subscription", "Managed Browser Use billed to your subscription",
                 browser_provider="browser-use", **_MOOR, managed_moor_feature="browser",
    ...
    "computer_use": {
        ...
        # cua-driver reads HOME/TMPDIR from the process env; MOOR_CUA_DRIVER_CMD selects a specific
        ...
        "providers": [
            ...
            _row("Langfuse Self-Hosted", tag="Self-hosted Langfuse instance", post_setup="langfuse",
                 env_vars=[_key(*_LANGFUSE_PUBLIC), _key(*_LANGFUSE_SECRET),
                           _key("MOOR_LANGFUSE_BASE_URL", "Langfuse server URL (e.g. http://localhost:3000)", default="http://localhost:3000")]),
        ],
    },
}
...
#: "declined": saving ``moor tools`` freezes a platform's composite into an explicit list nothing adds to, so
#: a later toolset stays off forever for picker users while ``[moor-cli]`` users inherit it.
...
def _enable_recently_shipped_toolsets(enabled_toolsets: Set[str], config: dict, platform: str) -> None:
    ...
    for ts_key in sorted(_RECENTLY_SHIPPED_TOOLSETS):
        ...
        # composites (moor-acp, moor-webhook) stay narrow.
...
def _explicit_toolsets(
    ...
    ``moor-cli`` would re-enable disabled toolsets). A mixed list (``[moor-cli, spotify]``) still expands the
...
def _coerce_platform_toolsets_value(value, platform: str):
    ...
    The parser is shared with ``moor doctor`` and ``moor plugins`` (``toolset_validation``
    ...
    if platform not in _warned_invalid_platform_toolsets:
        ...
        logger.warning(
            ...
            "Run `moor tools` to reconfigure.", platform, value)
    return value
...
def _get_platform_tools(config: dict, platform: str, *, include_default_mcp_servers: bool = True) -> Set[str]:
    ...
    # Plugin toolsets are first-class on a saved list: ``[moor-cli, a2a]`` must survive filtering.
    # Plugin-provided toolsets are first-class on a platform-toolsets list — explicit config like
    # ``[moor-cli, a2a]`` must survive filtering just like a built-in configurable toolset would. See
    # issue #81163.
    ...
    # above. It may arrive as a JSON-array string ("['memory']") from `moor config set` or a JSON-mode editor.
...
def _prune_toolsets_stripped_by_disabled(enabled_toolsets: Set[str], disabled_names: List[str]) -> Set[str]:
    ...
    never appear in the list. A name-only subtraction here left inspection surfaces (``moor tools
...
def _recover_platform_native_toolsets(enabled_toolsets: Set[str], platform: str, *, skip: Set[str]) -> None:
    ...
    skip = skip | {k for k in TOOLSETS if k.startswith("moor-")} | (set(_DEFAULT_OFF_TOOLSETS) - {platform})
...
def _warn_all_invalid_platform_toolsets(platform: str, explicit: list) -> None:
    """Warn once when an explicit platform list has only invalid names (``moor`` for ``moor-cli`` → no
...
def _save_platform_tools(config: dict, platform: str, enabled_toolset_keys: Set[str]):
    ...
    # names): platform defaults (moor-cli, ...) resolve to ALL tools and would silently override the user's
...
def _toolset_has_keys(
    ts_key: str, config: dict = None, *, force_fresh: bool = False, features: Optional[MoorSubscriptionFeatures] = None,
) -> bool:
...
def _shared_metrics_menu_label(config: dict) -> str:
    ...
    state = "off" if not enabled else ("collecting + sending to Moor" if send else "collecting locally")
...
def _configure_shared_metrics_interactive(config: dict) -> None:
    ...
    from moor_cli.setup import setup_telemetry
...
def _print_tools_summary(config: dict, enabled_platforms: List[str]) -> None:
    """``moor tools --summary``: enabled toolsets per platform, non-interactive."""
    total = len(_get_effective_configurable_toolsets())
...
def _first_install_flow(config: dict, enabled_platforms: List[str]) -> None:
    ...
    for pkey in enabled_platforms:
        ...
        auto_configured = apply_moor_managed_defaults(config, enabled_toolsets=new_enabled, force_fresh=True)
...
def tools_command(args=None, first_install: bool = False, config: dict = None):
    """Entry point for `moor tools` / `moor setup tools`. ``first_install`` skips the menu (checklist + key
    ...
    print(color("  Guide: https://github.com/thisismamad-n/Moor/tree/master/website/docs/user-guide/features/tools", Colors.DIM))
    print()
    if first_install:
...
_PLUGIN_COMPAT_LAZY = {
    'MANAGED_FEATURE_COVERAGE_CATEGORY': ('moor_cli.moor_subscription', 'MANAGED_FEATURE_COVERAGE_CATEGORY'),
    'MOOR_MANAGED_PROVIDER': ('tools.tool_backend_helpers', 'MOOR_MANAGED_PROVIDER'),
    'base_url_hostname': ('utils', 'base_url_hostname'),
    'fal_key_is_configured': ('tools.tool_backend_helpers', 'fal_key_is_configured'),
    'format_moor_portal_entitlement_message': ('moor_cli.moor_account', 'format_moor_portal_entitlement_message'),
