...
Usage:
    moor pairing list              # Show all pending + approved users
    moor pairing approve <platform> <request-id|code>  # Approve a pairing request
    moor pairing revoke <platform> <user_id> # Revoke user access
    moor pairing clear-pending     # Clear all expired/pending codes
"""

def pairing_command(args):
    """Handle moor pairing subcommands."""
    ...
    if action == "list":
        ...
        print("Usage: moor pairing {list|approve|revoke|clear-pending}")
        print("Run 'moor pairing --help' for details.")
...
def _cmd_list(store):
    ...
    if pending:
        ...
        print("\n  Approve with: moor pairing approve <platform> <request-id>")
...
def _cmd_approve(store, platform: str, code: str):
    ...
    if result:
        ...
        print(
            ...
            "~/.moor/platforms/pairing/_rate_limits.json\n".format(platform)
        )
    else:
        print(f"\n  Pairing request or code '{code}' not found or expired for platform '{platform}'.")
        print("  Run 'moor pairing list' to see pending requests.\n")
