...
def run_setup_wizard(args):
    ...
    if is_existing:
        ...
        if quick_requested:
            _run_quick_setup(config, moor_home)
            return
        ...
        if setup_mode == 0:
            _run_first_time_quick_setup(config, moor_home, is_existing)
            return
        if setup_mode == 2:
            _run_blank_slate_setup(config, moor_home, is_existing)
            return
...
def _run_blank_slate_setup(config: dict, moor_home, is_existing: bool):
    ...
    if path == 0:
        ...
        # `moor update` runs don't re-inject them.
        try:
