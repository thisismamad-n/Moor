"""Interactive setup wizard for Moor Agent (config lives in ~/.moor/).
...
``moor_cli.setup.<name>`` keep working.
"""
...
# Config helpers are re-exported (tests patch them on this module). display_moor_home is
# imported lazily at call sites (stale-module safety during moor update).
from moor_cli.config import (
    cfg_get, DEFAULT_CONFIG, get_moor_home, get_config_path, get_env_path, load_config, save_config,
    save_env_value, remove_env_value, get_env_value, ensure_moor_home,
)
from moor_cli.colors import Colors, color
from moor_cli.cli_output import print_error, print_info, print_success, print_warning
...
_DOCS_BASE = "https://github.com/thisismamad-n/Moor/tree/master/website/docs"
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    _info("The interactive wizard cannot be used here.", None,
          "Configure Moor using environment variables or config commands:",
          "  moor config set model.provider custom",
          "  moor config set model.base_url http://localhost:8080/v1",
          "  moor config set model.default your-model-name", None,
          "Or set OPENROUTER_API_KEY / OPENAI_API_KEY in your environment.",
          "Run 'moor setup' in an interactive terminal to use the full wizard.", None)
...
_SETUP_NAVIGATION: ContextVar[_SetupNavigationState | None] = ContextVar("moor_setup_navigation", default=None)
...
def _setup_navigation_scope():
    ...
    from moor_cli.curses_ui import reset_menu_navigation_handler, set_menu_navigation_handler
...
def run_setup_action_with_navigation(
    ...
    ``moor model`` that use the wizard's pickers outside ``run_setup_wizard``."""
...
def is_noninteractive() -> bool:
    ...
    with ``stdin=DEVNULL`` and ``MOOR_NONINTERACTIVE=1`` (``moor_cli/web_server.py``), where a
    prompt that aborts on EOF would kill the spawned action — callers fall back to their default."""
    return os.environ.get("MOOR_NONINTERACTIVE", "").strip().lower() in {"1", "true", "yes", "on"}


def prompt_yes_no(question: str, default: bool = True) -> bool:
    """Prompt for yes/no. Ctrl+C exits; empty input, ``MOOR_NONINTERACTIVE=1`` or a
...
def _prompt_api_key(var: dict):
    ...
    print()
    _prompt_and_save_env_var(var, "  ✓ Saved", "  Skipped (configure later with 'moor setup')")
...
def setup_model_provider(config: dict, *, quick: bool = False):
    """Configure the inference provider and default model via the ``moor model`` flow (one code
    ...
    # keep safe defaults (configure via `moor auth add` / `moor setup tts`).
    config.clear()
...
_TOOL_PROGRESS_HELP = (
    ...
    "  log     — Silent in chat; write every tool call to ~/.moor/logs/tool_calls.log (gateway only)",
)
def setup_agent_settings(config: dict):
    ...
    if max_iter is None:
        ...
        # config.yaml only; gateway/run.py derives MOOR_MAX_ITERATIONS from agent.max_turns.
...
def setup_tools(config: dict, first_install: bool = False):
    """`moor setup tools` == `moor tools`: platform selection → toolset toggles → provider keys.
...
_SEND_CONSENT_EXPLAINER = (
    "", "Sending uploads each daily package to the Moor telemetry",
...
def setup_telemetry(config: dict):
    ...
    _info("Shared metrics contain only bounded counters and histograms.",
          "Collection is local. Sending them to Moor is a separate opt-in.")
    ...
    shared_metrics["send"] = prompt_yes_no("Send shared metrics to Moor?", default=shared_metrics.get("send") is True)
...
def _record_send_consent_change(*, enabled: bool) -> None:
    ...
    try:
        ...
        from moor_cli.observability.shared_metrics_sender import reconcile_send_consent
...
# moor_cli.setup. They import this module lazily inside bodies, so this is cycle-free.

from moor_cli.setup_tts import setup_tts  # noqa: E402
from moor_cli.setup_terminal import setup_terminal_backend  # noqa: E402
from moor_cli.setup_platforms import setup_gateway  # noqa: E402
from moor_cli.setup_summary import _print_setup_summary  # noqa: E402,F401
from moor_cli.setup_migration import _offer_openclaw_migration, _skip_configured_section  # noqa: E402
from moor_cli.setup_quick import _run_portal_one_shot, _run_quick_setup  # noqa: E402
...
def _run_setup_section(config: dict, section: str) -> None:
    """``moor setup <section>``: run one SETUP_SECTIONS entry under the banner."""
...
def _run_full_setup(config: dict, moor_home, *, is_existing: bool, migration_ran: bool) -> None:
    ...
    _info(f"Config file:  {get_config_path()}", f"Secrets file: {get_env_path()}",
          f"Data folder:  {moor_home}", f"Install dir:  {PROJECT_ROOT}", None,
          "You can edit these files directly or use 'moor config edit'")
    if migration_ran:
...
def _run_setup_wizard_impl(args):
    ...
    ``moor setup <section>`` from SETUP_SECTIONS."""
    ...
    if getattr(args, 'non_interactive', False) or not is_interactive_stdin():
        ...
        return
    if getattr(args, "portal", False):  # one-shot Moor Portal setup; skips the rest
    ...
    if is_existing:
        ...
        if quick_requested:
            _run_setup_steps([("Quick Setup", lambda: _run_quick_setup(config, moor_home))])
            return
        ...
        _info("Running the full wizard — each prompt shows your current value.",
              ...
              "Tip: jump straight to a section with 'moor setup model|terminal|",
        ...
        migration_ran = _offer_openclaw_migration(moor_home)  # before configuration begins
        if migration_ran:
            config = load_config()
        setup_mode = prompt_choice("How would you like to set up Moor?", [label for label, _ in _FIRST_TIME_MODES], 0)
        label, runner = _FIRST_TIME_MODES[setup_mode]
        if runner is not None:
            from moor_cli import setup_quick
            _run_setup_steps([(label, lambda: getattr(setup_quick, runner)(config, moor_home, is_existing))])
            return
    _run_full_setup(config, moor_home, is_existing=is_existing, migration_ran=migration_ran)
...
_PLUGIN_COMPAT_LAZY = {
    'get_moor_subscription_features': ('moor_cli.moor_subscription', 'get_moor_subscription_features'),
    'get_optional_skills_dir': ('moor_constants', 'get_optional_skills_dir'),
