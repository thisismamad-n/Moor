...
from moor_cli.curses_ui import MenuNavigationEvent, MenuNavigationStart
...
from moor_constants import get_optional_skills_dir
...
# display_moor_home imported lazily at call sites (stale-module safety during moor update)
...
from moor_cli.cli_output import (  # noqa: E402
    print_error,
    print_info,
    print_success,
    print_warning,
)
from moor_cli.secret_prompt import masked_secret_prompt  # noqa: E402
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print_info("  moor config set model.provider custom")
    print_info("  moor config set model.base_url http://localhost:8080/v1")
    print_info("  moor config set model.default your-model-name")
    print()
    print_info("Or set OPENROUTER_API_KEY / OPENAI_API_KEY in your environment.")
    print_info("Run 'moor setup' in an interactive terminal to use the full wizard.")
    print()
...
_SETUP_NAVIGATION: ContextVar[_SetupNavigationState | None] = ContextVar(
    "moor_setup_navigation", default=None
)
...
def is_noninteractive() -> bool:
    ...
    ``MOOR_NONINTERACTIVE=1`` (see ``moor_cli/web_server.py``). In that
    ...
    """
    return os.environ.get("MOOR_NONINTERACTIVE", "").strip().lower() in {
        "1",
        "true",
        "yes",
        "on",
...
def prompt_yes_no(question: str, default: bool = True) -> bool:
    ...
    Non-interactive callers (``MOOR_NONINTERACTIVE=1`` or a closed/redirected
...
def _prompt_api_key(var: dict):
    ...
    if value:
        ...
        print_warning("  Skipped (configure later with 'moor setup')")


def _print_setup_summary(config: dict, moor_home):
    ...
    if not _provider_ready:
        ...
        print_info("    moor model            (pick any provider/model)")
        print_info("    moor setup --portal   (Moor Portal OAuth, no API key)")
    ...
    tool_status = []
    subscription_features = get_moor_subscription_features(config)
    ...
    if _vision_backends:
        ...
        tool_status.append(("Vision (image analysis)", False, "run 'moor setup' to configure"))


    # Web tools (Exa, Parallel, Firecrawl, or Tavily)
    if subscription_features.web.managed_by_moor:
        tool_status.append(("Web Search & Extract (Moor subscription)", True, None))
    ...
    if subscription_features.browser.managed_by_moor:
        tool_status.append(("Browser Automation (Moor Browser Use)", True, None))
    ...
    # Image generation — FAL (direct or via Moor), or any plugin-registered
    # provider (OpenAI, etc.)
    if subscription_features.image_gen.managed_by_moor:
        tool_status.append(("Image Generation (Moor subscription)", True, None))
    ...
    # Video generation — opt-in via `moor tools` → Video Generation.
    # Only show the row when a plugin reports available so we don't badger
    # users who don't care about video gen with a "missing" status line.
    if subscription_features.video_gen.managed_by_moor:
        tool_status.append(("Video Generation (FAL via Moor subscription)", True, None))
    else:
        try:
            from agent.video_gen_registry import list_providers as _list_video_providers
            from moor_cli.plugins import _ensure_plugins_discovered as _ensure_plugins
            _ensure_plugins()
            _video_backend = None
    ...
    if subscription_features.tts.managed_by_moor:
        tool_status.append(("Text-to-Speech (OpenAI via Moor subscription)", True, None))
        ...
        if neutts_ok:
            ...
            tool_status.append(("Text-to-Speech (NeuTTS — not installed)", False, "run 'moor setup tts'"))
        ...
        if kittentts_ok:
            ...
            tool_status.append(("Text-to-Speech (KittenTTS — not installed)", False, "run 'moor setup tts'"))
    else:
    ...
    if _stt_feature is not None and _stt_feature.managed_by_moor:
        tool_status.append(("Speech-to-Text (OpenAI via Moor subscription)", True, None))
        ...
        if fw_ok:
            ...
            tool_status.append(
                ("Speech-to-Text (Local Whisper — not installed)", False, "run 'moor tools' → Speech-to-Text")
            )

    if subscription_features.modal.managed_by_moor:
        tool_status.append(("Modal Execution (Moor subscription)", True, None))
    elif cfg_get(config, "terminal", "backend") == "modal":
        if subscription_features.modal.direct_override:
            tool_status.append(("Modal Execution (direct Modal)", True, None))
        else:
            tool_status.append(("Modal Execution", False, "run 'moor setup terminal'"))
    elif managed_moor_tools_enabled() and subscription_features.moor_auth_present:
        tool_status.append(("Modal Execution (optional via Moor subscription)", True, None))

    # Home Assistant
    if get_env_value("HASS_TOKEN"):
        tool_status.append(("Smart Home (Home Assistant)", True, None))

    # Spotify (OAuth via moor auth spotify — check auth.json, not env vars)
    try:
    ...
    if disabled_tools:
        print_warning(
            "Some tools are disabled. Run 'moor setup tools' to configure them,"
        )
    ...
    print(
        f"   {color('Data:', Colors.YELLOW)}      {moor_home}/cron/, sessions/, logs/"
    )
    print()
    ...
    print()
    print(f"   {color('moor setup', Colors.GREEN)}          Re-run the full wizard")
    print(f"   {color('moor setup model', Colors.GREEN)}    Change model/provider")
    print(f"   {color('moor setup terminal', Colors.GREEN)} Change terminal backend")
    print(f"   {color('moor setup gateway', Colors.GREEN)}  Configure messaging")
    print(f"   {color('moor setup tools', Colors.GREEN)}    Configure tool providers")
    print()
    print(f"   {color('moor config', Colors.GREEN)}         View current settings")
    print(
        f"   {color('moor config edit', Colors.GREEN)}    Open config in your editor"
    )
    print(f"   {color('moor config set <key> <value>', Colors.GREEN)}")
    ...
    print()
    print(f"   {color('moor', Colors.GREEN)}              Start chatting")
    print(f"   {color('moor gateway', Colors.GREEN)}      Start messaging gateway")
    print(f"   {color('moor doctor', Colors.GREEN)}       Check for issues")
    print()
...
# between `moor tools` and `moor setup tools`).
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    Delegates to ``cmd_model()`` (the same flow used by ``moor model``)
    ...
    provider added to ``moor model`` is automatically available here.
    ...
    # Delegate to the shared moor model flow — handles provider picker,
    ...
    # on demand via `moor auth add`, `moor setup` vision, and
    # `moor setup tts`. This keeps both quick and full setup thin.


    # Tool Gateway prompt is already shown by _model_flow_moor() above.
    save_config(config)
...
def _xai_oauth_logged_in_for_setup() -> bool:
    ...
    through ``moor model`` -> xAI Grok OAuth (SuperGrok / Premium+).
    """
    try:
...
def _setup_tts_provider(config: dict):
    ...
    choices = []
    providers = []
    if managed_moor_tools_enabled() and subscription_features.moor_auth_present:
        choices.append("Moor Subscription (managed OpenAI TTS, billed to your subscription)")
        providers.append("moor-openai")
    choices.extend(
        [
    ...
    selected_via_moor = selected == "moor-openai"
    if selected == "moor-openai":
        selected = "openai"
        print_info("OpenAI TTS will use the managed Moor gateway and bill to your subscription.")
        if get_env_value("VOICE_TOOLS_OPENAI_KEY") or get_env_value("OPENAI_API_KEY"):
            print_warning(
                "Direct OpenAI credentials are still configured and may take precedence until removed from ~/.moor/.env."
            )
    ...
    elif selected == "openai" and not selected_via_moor:
            ...
            if choice_idx == 0:
                ...
                if api_key:
                    ...
                    print_warning(
                        ...
                        f"via moor setup model or {_dhh()}/.env to use xAI TTS. "
                        "Falling back to Edge TTS."
                    )
                    selected = "edge"
            else:
...
def setup_tts(config: dict):
    """Standalone TTS setup (for 'moor setup tts')."""
...
def setup_terminal_backend(config: dict):
    ...
    # under ~/.moor/plugins/). Fail-soft: a broken plugin must not take the
    # setup wizard down.
    ...
    if selected_backend == "local":
        ...
        # configurable later via `moor setup terminal` / config.yaml.
        ...
        if prompt_yes_no("  Enable egress firewall for Docker sandboxes?", False):
            ...
            print_info(
                "Run `moor egress setup` then `moor egress start` to mint "
                "tokens and launch the proxy."
            )
        else:
            print_info(
                "Skipping egress firewall. You can enable it later with `moor egress setup`."
            )
        ...
        managed_modal_available = bool(
            ...
            and
            get_moor_subscription_features(config).moor_auth_present
        ...
        if use_managed_modal:
            ...
            print_info("Modal execution will use the managed Moor gateway and bill to your subscription.")
        ...
        print_info("Requires the optional SDK: pip install 'moor-agent[vercel]'")
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                print_warning("Install failed — run manually: pip install 'moor-agent[vercel]'")
                if result.stderr:
                    print_info(f"  Error: {result.stderr.strip().splitlines()[-1]}")
...
def _apply_default_agent_settings(config: dict):
    ...
    # bridges it into MOOR_MAX_ITERATIONS at startup. We no longer write
    ...
    print_info("  Run `moor setup agent` later to customize.")
...
def setup_agent_settings(config: dict):
    ...
    try:
        ...
        if max_iter > 0:
            ...
            # MOOR_MAX_ITERATIONS from agent.max_turns at startup.
    ...
    print_info("  log     — Silent in chat; write every tool call to ~/.moor/logs/tool_calls.log (gateway only)")
...
def _setup_telegram_auto_result():
    ...
    try:
        from moor_cli.telegram_managed_bot import auto_setup_telegram_bot_result
    except ImportError:
        return None

    profile_name: str | None = None
    try:
        profile_name = _profile_name_from_moor_home(Path(get_moor_home()))
    except Exception:
        pass
...
def _profile_name_from_moor_home(moor_home) -> str | None:
    """Return the active profile name when MOOR_HOME is a profile dir."""
    if moor_home.parent.name == "profiles":
        return moor_home.name
    return None
...
def _setup_qqbot():
    ...
    from moor_cli.gateway import _setup_qqbot as _gateway_setup_qqbot
...
def setup_gateway(config: dict):
    ...
    from moor_cli.gateway import _all_platforms, _platform_status, _configure_platform
    ...
    if not selected:
        print_info("No platforms selected. Run 'moor setup gateway' later to configure.")
    else:
        for idx in selected:
    ...
    if any_messaging:
        ...
        if missing_home:
            ...
            for plat in missing_home:
                print_info(
                    f"     moor config set {plat.upper()}_HOME_CHANNEL <channel_id>"
                )
    ...
    # via `moor import` or `moor setup gateway`). Gating this on
...
def setup_tools(config: dict, first_install: bool = False):
    ...
    Both `moor setup tools` and `moor tools` use the same flow:
...
def _model_section_has_credentials(config: dict) -> bool:
    ...
    Sources of truth:
      * ``PROVIDER_REGISTRY`` in ``moor_cli.auth`` — lists every supported
        provider along with its ``api_key_env_vars``.
      * ``active_provider`` in the auth store — covers OAuth device-code /
        external-OAuth providers (Moor, Codex, Qwen, Gemini CLI, ...).
...
def _get_section_config_summary(config: dict, section_key: str) -> Optional[str]:
    ...
    ``get_env_value`` is the module-level import from moor_cli.config
    ...
    if section_key == "model":
        ...
        from moor_cli.gateway import _all_platforms, _platform_status
...
def _load_openclaw_migration_module():
    """Load the openclaw_to_moor migration script as a module.
    ...
    spec = importlib.util.spec_from_file_location(
        "openclaw_to_moor", _OPENCLAW_SCRIPT
    )
...
def _offer_openclaw_migration(moor_home: Path) -> bool:
    ...
    if not prompt_yes_no("Would you like to see what can be imported?", default=True):
        print_info(
            "Skipping migration. You can run it later with: moor claw migrate --dry-run"
        )
        return False
    ...
    if not prompt_yes_no("Proceed with migration?", default=False):
        print_info(
            "Migration cancelled. You can run it later with: moor claw migrate"
        )
        print_info(
    ...
    if conflicts:
        print_info(f"Skipped {conflicts} item(s) that already exist in Moor (use moor claw migrate --overwrite to force).")
    if skipped:
...
def _run_portal_one_shot(config: dict) -> None:
    """One-shot Moor Portal setup — OAuth + model pick + provider + Tool Gateway.

    Wired into ``moor setup --portal`` and ``moor portal``. This is the
    Moor-Portal slice of the first-time quick setup, collapsed into a single
    ...
    ``moor setup`` and hunt for the quick-setup option.

    The login + model selection + provider switch + Tool Gateway opt-in are all
    delegated to ``_model_flow_moor`` — the exact same flow quick setup uses
    (``_run_first_time_quick_setup``) and the same one ``moor model`` runs
    when you pick Moor. Routing through it (instead of hand-rolling the auth +
    provider write here) means ``moor portal`` always offers a model picker,
    and there is a single source of truth for the Moor onboarding steps.
    """
    ...
    print(color("│     ⚕ Moor Setup — Moor Portal (one-shot)             │", Colors.MAGENTA))
    print(
        color(
            "└─────────────────────────────────────────────────────────┘",
            Colors.MAGENTA,
    ...
    print_info("    — all routed through your Moor Portal sub.")
    print()
    print_info("  Sign up: https://portal.nousresearch.com/manage-subscription")
    print()

    # _model_flow_moor handles BOTH the logged-out path (device-code OAuth,
    # which selects a model internally) and the already-logged-in path (curated
    # Moor model picker), then offers the Tool Gateway opt-in and sets
    # provider=moor via the login/model save. This is the same routine quick
    # setup calls, so `moor portal` == quick setup's Moor step.
    try:
        from moor_cli.main import _model_flow_moor

        _model_flow_moor(config)
    except (KeyboardInterrupt, EOFError, SystemExit):
        # _login_moor raises SystemExit(130)/(1) on cancel/failure; the
        # logged-out path inside _model_flow_moor catches it, but the
        ...
        logger.debug("_model_flow_moor error during `moor portal`: %s", exc)
        print()
        print_error(f"  Moor Portal setup encountered an error: {exc}")
        print_info("  You can retry later with `moor portal`.")
        return

    # Re-sync the in-memory config from disk — _model_flow_moor (and the
    ...
    print_info("  Run `moor portal info` to inspect routing.")
    print_info("  Run `moor` to start chatting.")


@contextmanager
def _setup_navigation_scope():
    """Install and reliably restore the setup menu navigation context."""
    from moor_cli.curses_ui import (
...
def run_setup_action_with_navigation(
    ...
    *,
    ...
    Shared commands such as ``moor model`` use the same provider/model
...
def _run_setup_wizard_impl(args):
    ...
    Supports full, quick, and section-specific setup:
      moor setup           — full or quick (auto-detected)
      moor setup model     — just model/provider
      moor setup tts       — just text-to-speech
      moor setup terminal  — just terminal backend
      moor setup gateway   — just messaging platforms
      moor setup tools     — just tool configuration
      moor setup telemetry — just local shared metrics
      moor setup agent     — just agent settings
    """
    ...
    # --portal: one-shot Moor Portal setup. Skips the rest of the wizard.
    ...
    if is_existing:
        ...
        if quick_requested:
            _run_setup_steps(
                [("Quick Setup", lambda: _run_quick_setup(config, moor_home))]
            )
            return
        ...
        print_info("")
        print_info("Tip: jump straight to a section with 'moor setup model|terminal|")
        ...
        migration_ran = _offer_openclaw_migration(moor_home)
        if migration_ran:
            config = load_config()
        ...
        setup_mode = prompt_choice(
            ...
            [
                "Quick Setup (Moor Portal) — free OAuth login, no API keys, model + tools (recommended)",
    ...
    print_info(f"Data folder:  {moor_home}")
    print_info(f"Install dir:  {PROJECT_ROOT}")
    print()
    print_info("You can edit these files directly or use 'moor config edit'")
    ...
    # Tune later with `moor setup agent`.
    if not is_existing:
...
def _run_first_time_quick_setup(config: dict, moor_home, is_existing: bool):
    ...
    Routes straight to the Moor Portal provider — runs the device-code OAuth
    login, picks a Moor model, then configures the terminal backend and (optionally)
    a messaging platform. Applies sensible defaults for everything else (agent
    settings, tools); the user can customize later via ``moor setup <section>``
    or switch providers with ``moor model``.
    """
    from moor_cli.config import load_config

    # Step 1: Moor Portal — OAuth login + model selection.
    # _model_flow_moor() handles both the logged-out path (device-code OAuth,
    # which selects a model internally) and the already-logged-in path (curated
    # Moor model picker). Provider is set to "moor" by the login/model save.
    print()
    print_header("Moor Portal")
    print_info("One subscription, 300+ models, plus the Tool Gateway:")
    print_info("  web search, image generation, TTS, browser automation.")
    print_info("Sign up: https://portal.nousresearch.com/manage-subscription")
    print()
    try:
        ...
        print()
        print_info("Moor Portal setup cancelled.")
    except Exception as exc:
        logger.debug("_model_flow_moor error during quick setup: %s", exc)
        print_warning(f"Moor Portal setup encountered an error: {exc}")
        print_info("You can try again later with: moor model")

    # Re-sync the wizard's config dict from disk — _model_flow_moor (and the
    ...
    gateway_choice = prompt_choice(
        ...
        [
            ...
            "Skip — set up later with 'moor setup gateway'",
        ],
        0,
    )
    ...
    if gateway_choice == 0:
        ...
        # (e.g. via `moor import` from another machine).
        from moor_cli.gateway import ensure_gateway_service
        ensure_gateway_service(context="setup")
    ...
    print()
    print_info("  Configure all settings:    moor setup")
    if gateway_choice != 0:
...
def _blank_slate_minimal_toolsets(config: dict):
    ...
    ``moor-agent`` skill (the agent's operating manual for driving,
    configuring, and troubleshooting Moor) is always seeded — without
    ...
    2. ``agent.disabled_toolsets`` — a global hard-suppression list (applied last
       ...
       quirks. The user re-enables any of them later via ``moor tools`` (which
...
def _blank_slate_minimize_config(config: dict):
    ...
    Everything here is opt-in afterwards via ``moor setup agent`` /
    ``moor config set``. We keep only what's needed to run.
    """
...
def _run_blank_slate_setup(config: dict, moor_home, is_existing: bool):
    ...
    print_info("essential `moor-agent` skill is always kept so the agent")
    print_info("can help you drive and configure Moor itself.")
    print()
    ...
    if path == 0:
        ...
        # `moor update` runs don't re-inject them. Essential skills (the
        # `moor-agent` operating manual) are still seeded by the sync.
        try:
        ...
        print_info("  Enable tools:        moor tools")
        print_info("  Seed skills:         moor skills opt-in --sync")
        print_info("  Add MCP servers:     moor mcp add")
        print_info("  Enable plugins:      moor plugins")
        print_info("  Tune agent settings: moor setup agent")
        print()
        _print_setup_summary(config, moor_home)
        return
...
def _blank_slate_walkthrough(config: dict, moor_home):
    ...
    try:
        ...
        if seed_skills:
            ...
            set_bundled_skills_opt_out(True)
            # Essential skills (the `moor-agent` operating manual) are
            # still seeded even for an opted-out profile.
            sync_skills(quiet=True)
            print_info("No skills seeded (except the essential `moor-agent`")
            print_info("skill). A .no-bundled-skills marker keeps future")
            print_info("`moor update` runs from re-injecting them. Opt back in any")
            print_info("time with `moor skills opt-in --sync`.")
    except Exception as exc:
    ...
    if prompt_yes_no("Review and enable built-in plugins now?", default=False):
        print_info("Manage plugins with `moor plugins list` / `moor plugins install`.")
    else:
        print_info("No plugins enabled. Add later with `moor plugins`.")

    # ── MCP servers (off unless chosen) ──
    print()
    print_header("MCP Servers")
    if prompt_yes_no("Add an MCP server now?", default=False):
        print_info("Add servers with `moor mcp add <name> --url ... | --command ...`.")
    else:
        print_info("No MCP servers configured. Add later with `moor mcp add`.")
    ...
    print()
    print_success("Blank Slate setup complete — minimal agent ready.")
    print_info("  Enable more tools:   moor tools")
    print_info("  Seed skills:         moor skills opt-in --sync")
    print_info("  Add MCP servers:     moor mcp add")
    print_info("  Tune agent settings: moor setup agent")
    print()
...
def _run_quick_setup(config: dict, moor_home):
    ...
    if not has_anything_missing:
        ...
        print()
        print_info("Run 'moor setup' and choose 'Full Setup' to reconfigure,")
    ...
    if missing_messaging:
        ...
        print_info("You can configure these later with 'moor setup gateway'.")
