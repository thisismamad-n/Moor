...
Split out of ``moor_cli.models``. OpenRouter and
Moor Portal share one implementation parametrized by :class:`_CapsSource`; the per-source module
globals (``_openrouter_reasoning_caps_cache``, ``_moor_caps_disk_checked``, ...) stay defined on
``moor_cli.models`` — tests reset them there — and are read/written by attribute name.
...
# that must never block on HTTP — so without a disk copy, `moor -p`, a cron job, or a freshly
...
def _read_reasoning_caps_disk() -> dict[str, Any]:
    from moor_cli.models import _read_json_cache
...
def _fetch_reasoning_caps_catalog(url: str, timeout: float) -> Optional[Caps]:
    ...
    m = _origin()
    try:
        req = urllib.request.Request(url, headers={"Accept": "application/json", "User-Agent": m._MOOR_USER_AGENT})
...
# ── Per-source cache (OpenRouter, Moor Portal) ─────────────────────────

@dataclass(frozen=True)
class _CapsSource:
    """One catalog's cache slots on ``moor_cli.models`` plus how to name its URL.
...
def refresh_reasoning_caps_async(provider: Optional[str]) -> None:
    ...
    """
    src = {"moor": _MOOR_CAPS, "moor-portal": _MOOR_CAPS, "nousresearch": _MOOR_CAPS,
...
_MOOR_CAPS = _CapsSource(
    ...
    lambda: moor_catalog_url(),
)


def moor_catalog_url() -> str:
    """The Portal ``/v1/models`` URL for the endpoint we actually talk to (``MOOR_INFERENCE_BASE_URL``
...
def moor_model_reasoning_capabilities(
    ...
    """Moor Portal counterpart of :func:`openrouter_model_reasoning_capabilities`; warm the cache
    with :func:`warm_moor_reasoning_caps_async` from hot paths."""
    return _model_caps(_MOOR_CAPS, model_id, timeout=timeout, allow_fetch=allow_fetch)
