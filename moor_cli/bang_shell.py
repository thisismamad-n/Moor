...
def bang_shell_enabled() -> bool:
    ...
    return not (env_var_enabled("MOOR_GATEWAY_SESSION") or env_var_enabled("MOOR_CRON_SESSION")
                or (os.getenv("MOOR_SESSION_PLATFORM") or "").strip())
...
def _bang_env() -> dict:
    """Environment for a bang command with moor-managed secrets filtered.
...
def run_bang_command(command: str, *, cwd: Optional[str] = None, timeout: int = DEFAULT_TIMEOUT, writer=None) -> int:
    ...
    try:
        ...
        # Ctrl+C interrupts the command, not the Moor session.
        proc.kill()
        emit("!: interrupted")
        return 130
    finally:
