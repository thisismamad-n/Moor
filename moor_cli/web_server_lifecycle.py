...
def _warm_gateway_module() -> None:
    ...
    for mod in (
        "moor_cli.gateway",
        "moor_cli.auth",  # provider auth state → copilot_auth → subprocess
        "moor_cli.copilot_auth",
        "moor_cli.runtime_provider",
        "moor_cli.skin_engine",  # resolve_skin() config + skin engine init
        "moor_cli.inventory",  # provider catalogs + models.dev cache
...
def _write_dashboard_ready_file(actual_port: int) -> None:
    """Publish the port through an atomic ready file when ``MOOR_DESKTOP_READY_FILE`` is set.
...
def _is_serve_orphaned(
    ...
    *,
    ...
    ``MOOR_PARENT_PID`` is the Electron PID, not necessarily our PPID (the
    Windows ``moor.exe`` launcher adds shims), so never compare getppid().
...
def _start_parent_death_watchdog() -> None:
    ...
    start_marker = os.environ.get("MOOR_PARENT_START_MARKER") or None
    nonce = os.environ.get("MOOR_PARENT_NONCE") or None
    ...
    if has_marker and (not _valid_parent_start_marker(start_marker or "") or not nonce or nonce != nonce.strip()):
        ...
        _log.warning(
            "Parent-death watchdog disabled: unusable MOOR_PARENT_START_MARKER=%r / nonce; "
