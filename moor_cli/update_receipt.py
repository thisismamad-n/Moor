...
# ``moor update`` is a single-threaded CLI command; a module singleton lets the 7k-line updater
...
class UpdateReceipt:
    """Collects the observable facts of one ``moor update`` run."""
    ...
    def gateway_restart_result(
        ...
        if fresh_recovery is not None:
            ...
            # ``moor serve`` hosts tui_gateway and is not a gateway profile, so neither the
...
def finalize_pending_update_receipt(exit_code: Optional[int] = None, stop_reason: str = "") -> Optional[Path]:
    ...
    ``moor update`` has many early ``sys.exit`` paths (preflight refusals, venv-holder refusal,
...
def _profile_homes() -> list[tuple[str, Path]]:
    ...
    from moor_cli.profiles import _get_default_moor_home, _get_profiles_root, _PROFILE_ID_RE
...
def print_fleet_version_matrix(fleet: list[dict[str, Any]]) -> bool:
    ...
    if stale_or_down:
        ...
        print("  Run `moor gateway restart` (or `moor -p <profile> gateway restart` for a named")
        print("  profile), then `moor gateway status` to confirm.")
