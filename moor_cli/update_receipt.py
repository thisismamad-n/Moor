...
``moor_cli.build_info.get_code_identity()``: an image build reports
...
# Module-level current receipt. ``moor update`` is a single-threaded CLI
...
def _receipt_dir() -> Path:
    ...
    return get_moor_home() / "logs" / _RECEIPT_DIR_NAME
...
def finalize_pending_update_receipt(
    ...
    ``moor update`` has many early-termination paths (Windows
