"""Runtime-backed validation behind ``moor plugins doctor``.
...
def _doctor_runtime(plugin_path: Path):
    ...
    test framework. Registration code executes under a temporary MOOR_HOME
    with outbound socket connects blocked.
    """
    temporary_home = tempfile.TemporaryDirectory(prefix="moor-plugin-doctor-")
    stack = ExitStack()
    ...
    modules_before = {
        ...
        if name == "moor_plugins" or name.startswith("moor_plugins.")
    }
    ...
    try:
        ...
        if not manifests:
            raise _DoctorLoadError(
                f"Moor discovery found no valid plugin manifest under {copied}"
            )
        if len(manifests) != 1:
            raise _DoctorLoadError(
        ...
        for name in list(sys.modules):
            if (
                ...
                and (name == "moor_plugins" or name.startswith("moor_plugins."))
            ):
...
def resolve_plugin_path(target: str | os.PathLike[str] | None = None) -> Path:
    ...
    user_root = get_moor_home() / "plugins"
...
def _check_manifest_v2(report: "DoctorReport", manifest: Any) -> None:
    ...
    if missing:
        report.warning(
            ...
            + ", ".join(missing)
            + " — Moor never auto-installs plugin dependencies; "
