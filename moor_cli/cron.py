...
def _warn_if_gateway_not_running() -> None:
    ...
    print(color("     Start it with: moor gateway install\n"
...
def cron_list(show_all: bool = False):
    ...
    if not jobs:
        print(color("No scheduled jobs.\nCreate one with 'moor cron create ...' "
...
def cron_tick():
    ...
    try:
        ...
        print("  Check `moor cron status` and the gateway log for details.")
        return 1
    return 0
...
def cron_incidents(args) -> int:
    ...
    if action == "ack":
        ...
        if not incident_id:
            print(color("✗ Incident ID required: moor cron incidents ack <incident_id>", Colors.RED))
            return 1
    ...
    for inc in incidents:
        ...
        print()
    print(color(f"  {len(incidents)} incident(s)  |  ack one with: moor cron incidents ack <id>",
                Colors.DIM))
    return 0


_PERMISSION_HINT = ("  Hint: jobs.json may be owned by another user (e.g. rewritten by a root "
                    "`docker exec moor moor cron ...`). Fix ownership to match the gateway "
...
def _print_ticker_health(pids: list) -> None:
    ...
    if hb_age is None:
        ...
        print("  Cron jobs will NOT fire until the ticker writes its first heartbeat.\n"
              ...
              "  If heartbeat never appears, restart: moor gateway restart")
        ...
        print("  Cron jobs may NOT be firing. Restart: moor gateway restart")
...
def _script_health_issue(script: str) -> Optional[str]:
    ...
    try:
        ...
        return f"script resolves outside MOOR_HOME/scripts: {script!r}"
    if not path.exists():
...
def cron_doctor() -> int:
    ...
    for job, issues in findings:
        ...
        for issue in issues:
            print(f"    - {issue}")
    print()
    print(color("Next: fix the listed job config, then run `moor cron doctor` again.", Colors.DIM))
    return 1
...
def _job_action(action: str, job_id: str, success_verb: str) -> int:
    ...
    if action == "run":
        ...
        # inherits MOOR_SESSION_KEY) would be orphaned mid-LLM-call, leaving the execution row
        ...
        with contextlib.suppress(Exception):
            ...
            # gateway/desktop session env (MOOR_SESSION_KEY); declare the channel stateless so
...
def cron_resume(args) -> int:
    ...
    from cron.jobs import AmbiguousJobReference, _moor_now, rearm_oneshot
    try:
        job = rearm_oneshot(args.job_id, _moor_now().isoformat() if run_now else run_at)
...
def cron_notepad(args) -> int:
    """Handle ``moor cron notepad <job_id> [get|set|delete|list]`` (per-job durable KV).
    ...
    try:
        ...
        if key is None or (action == "set" and value is None):
            print(color(f"Usage: moor cron notepad <job_id> {usage_args}", Colors.RED))
            return 1
        if action == "set":
