...
def active_profile_may_override_home(moor_root: str) -> bool:
    """Cheap probe: does an active non-default profile redirect MOOR_HOME?"""
    active_profile = os.path.join(moor_root, "active_profile")
    try:
...
def _resolved_home() -> str:
    ...
    if moor_home:
        return moor_home
    return os.path.join(os.path.expanduser("~"), ".moor")
...
def container_mode_may_be_active() -> bool:
    ...
    moor_home = os.environ.get("MOOR_HOME", "").strip()
    if moor_home:
        if os.path.exists(os.path.join(moor_home, ".container-mode")):
            return True
        parent_name = os.path.basename(os.path.dirname(os.path.normpath(moor_home)))
        return (
            parent_name != "profiles"
            and active_profile_may_override_home(moor_home)
        )

    default_home = os.path.join(os.path.expanduser("~"), ".moor")
...
def read_install_method() -> str | None:
    ...
    non-default profile might redirect MOOR_HOME.
    """
...
def print_fast_version_info(*, check_updates: bool = True) -> None:
    """THE canonical ``moor --version`` output (also used by /version).
    ...
    ``moor version`` subcommand. Every lazy block degrades gracefully —
...
def try_fast_version(argv: list[str] | None = None) -> bool:
    ...
    container. Termux keeps the MOOR_TERMUX_DISABLE_FAST_CLI escape hatch.
    """
    if argv is None:
        argv = sys.argv[1:]
