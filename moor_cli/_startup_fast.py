...
def active_profile_may_override_home(moor_root: str) -> bool:
    ...
    active = (_read_text(os.path.join(moor_root, "active_profile")) or "").strip()
...
def _resolved_home() -> str:
    return os.environ.get("MOOR_HOME", "").strip() or _default_home()
...
def container_mode_may_be_active() -> bool:
    ...
    be active".
    """
    if os.environ.get("MOOR_DEV") == "1" or is_container_startup_environment():
        return False
    ...
    if moor_home:
        ...
        return parent_name != "profiles" and active_profile_may_override_home(moor_home)
