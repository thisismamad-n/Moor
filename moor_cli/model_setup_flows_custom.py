...
Imports of moor_cli.main / auth / config / models stay lazy (main.py import cycle;
...
def _probe_custom_endpoint(effective_key: str, effective_url: str) -> tuple[dict, str]:
    ...
    if probe.get("used_fallback") and probe.get("resolved_base_url"):
        ...
        print(f"Warning: could not verify this endpoint via {probe.get('probed_url')}. Moor will still save it.")
...
def _model_flow_custom(config):
    ...
    from moor_cli.main_provider_setup import _auto_provider_name, _prompt_custom_api_mode_selection, _save_custom_provider
    from moor_cli.auth import _save_model_choice, deactivate_provider
    from moor_cli.config import custom_endpoint_key_env, get_env_value, save_env_value
...
def _discover_named_custom_models(provider_info: dict, api_key: str, configured_models: list, explicit_catalog: bool):
    ...
    from moor_cli.models import fetch_api_models, _get_ollama_native_headers
...
def _model_flow_named_custom(config, provider_info):
    ...
    from moor_cli.main_provider_setup import _custom_provider_api_key_config_value, _custom_provider_base_url_config_value, _save_custom_provider
    from moor_cli.auth import _save_model_choice
    from moor_cli.config import load_config, save_config
    from moor_cli.model_switch import _entry_models_discovered, _models_config_is_allowlist
