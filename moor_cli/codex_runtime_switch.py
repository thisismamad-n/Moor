...
Toggles `model.openai_runtime` between "auto" (= chat_completions, Moor'
...
The actual runtime resolution happens in moor_cli.runtime_provider's
...
def parse_args(arg_string: str) -> tuple[Optional[str], list[str]]:
    ...
    if raw in {"off", "default", "disable", "moor"}:
        return "auto", []
...
def apply(
    ...
    *,
    ...
    # the world state (managed block in ~/.codex/config.toml, moor-tools
    ...
    if new_value == "codex_app_server":
        ...
        # Auto-migrate Moor' MCP servers + Codex's installed curated
        # plugins into ~/.codex/config.toml so the spawned codex subprocess
        # sees the same tool surface AND can call back into Moor for
        ...
        try:
            ...
            # Tools/MCP servers (excluding the moor-tools callback,
            # which is internal plumbing — surface separately).
            user_servers = [
                s for s in mig_report.migrated if s != "moor-tools"
            ]
            if user_servers:
                msg_lines.append(
            ...
            # Permissions + Moor tool callback are always-on production
            ...
            if "moor-tools" in mig_report.migrated:
                msg_lines.append(
