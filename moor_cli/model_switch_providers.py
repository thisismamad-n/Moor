...
Split out of ``moor_cli/model_switch.py``; every moved name is re-imported there so
``moor_cli.model_switch.<name>`` keeps resolving (and monkeypatching) as before."""
...
logger = logging.getLogger("moor_cli.model_switch")
...
def _save_discovered_models_to_config(
    ...
    from moor_cli.model_switch import _extra_headers_from_config
...
def _discovered_catalog_stale(entry: dict, model_ids: list[str]) -> bool:
    ...
    A mapping Moor itself discovered (entry flag or legacy in-mapping sentinel) is ours to
...
def _fetch_picker_live_models(
    ...
    from moor_cli.models import _get_ollama_native_headers, cached_fetch_api_models
...
def _iter_builtin_candidates(models_dev_data: dict, excluded: set, seen: set):
    """Yield ``(moor_id, mdev_id, pconfig, env_vars)`` for section-1 rows.
    ...
    from moor_cli.auth import PROVIDER_REGISTRY, is_runtime_provider_routable
    from moor_cli.models import _AGGREGATOR_PROVIDERS
    from moor_cli.providers import ALIASES
    for moor_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        alias_target = ALIASES.get(moor_id)
        if alias_target and alias_target != moor_id and alias_target in _AGGREGATOR_PROVIDERS:
            continue
        try:
            from providers import get_provider_profile
            prof = get_provider_profile(moor_id)
            if prof is not None and prof.name != moor_id:
                continue
        except Exception:
            pass
        if moor_id.lower() in seen:
            continue
        if moor_id.lower() in excluded or mdev_id.lower() in excluded:
            continue
        pdata = models_dev_data.get(mdev_id)
        if not isinstance(pdata, dict):
            continue
        pconfig = PROVIDER_REGISTRY.get(moor_id)
        if (pconfig and pconfig.auth_type != "api_key") or not is_runtime_provider_routable(moor_id):
            continue
        env_vars = list(pconfig.api_key_env_vars) if pconfig and pconfig.api_key_env_vars else pdata.get("env", [])
        if isinstance(env_vars, list):
            yield moor_id, mdev_id, pconfig, env_vars
...
def _raw_pool_usable(moor_id: str) -> bool:
    """Section-1 pool check: only consult the pool when auth.json lists a raw entry."""
    try:
        from moor_cli.auth import _load_auth_store
        store = _load_auth_store()
        if store and store.get("credential_pool", {}).get(moor_id):
            return _credential_pool_is_usable(moor_id, raw_pool_present=True)
    except Exception:
        pass
    return False
...
def _overlay_has_env_creds(pid: str, moor_slug: str, overlay, read_env) -> bool:
...
def _moor_picker_model_ids(curated: dict, force_fresh_moor_tier: bool) -> list:
    """Moor serves a huge live catalog; the picker shows ONLY the curated agentic list, augmented
    with the Portal's free/paid recommendations (new models surface without a CLI release) and
    narrowed by org policy. Mirrors ``_model_flow_moor`` so GUI pickers match the CLI. A failed
    ...
    try:
        ...
        try:
            ...
            portal = ""
        if check_moor_free_tier(force_fresh=force_fresh_moor_tier):
    ...
    try:
        ...
        model_ids = restrict_to_moor_policy(model_ids, moor_policy_allowed_ids(), rescue_empty=True)
    except Exception:
        pass
    return model_ids
...
def _absorb_entry_models(grp: dict, entry: dict, active_model: Any) -> None:
    ...
    from moor_cli.model_switch import _declared_model_ids, _entry_models_discovered, _models_config_is_allowlist
...
def _display_prefix(name: str) -> str:
    """Text before the per-model separator Moor's own writer uses ("—" / " - ")."""
...
def _collect_authed_provider_slugs(
    ...
    for moor_id, _mdev_id, _pconfig, env_vars in _iter_builtin_candidates(models_dev_data, excluded_set, seen):
        if _any_env(env_vars, _scoped_key_env) or _raw_pool_usable(moor_id):
            _emit(moor_id, moor_id)

    mdev_to_moor = {v: k for k, v in PROVIDER_TO_MODELS_DEV.items()}
    for pid, overlay in MOOR_OVERLAYS.items():
        moor_slug = mdev_to_moor.get(pid, pid)
        if _skip(seen, excluded_set, pid, moor_slug) or overlay.auth_type == "aws_sdk":
            continue
        if (
            _overlay_has_env_creds(pid, moor_slug, overlay, _scoped_key_env)
            or _auth_store_has_provider(pid, moor_slug) or _pool_usable(moor_slug)):
            _emit(moor_slug, pid, moor_slug)
    ...
    # Moor excluded: its picker branch builds from the curated list and never reads the
    # api_key-only cache entry a prefetch would write.
    return [s for s in slugs if s != "moor"]


@dataclass
class _PickerBuild:
    """State threaded through the ``list_authenticated_providers`` sections: 1 built-ins mapped to
    models.dev, 2 moor-only overlays, 2b canonical providers missed by 1/2, 3 ``providers:``
    entries + 3b the bare active custom endpoint, 4 ``custom_providers:`` entries. Row-builder
    imports of ``moor_cli.auth/models`` stay lazy so tests can patch those modules."""
    current_provider: str
    current_base_url: str
    current_model: str
    max_models: int | None
    for_picker: bool
    force_fresh_moor_tier: bool
...
def _lap_builtin_rows(b: _PickerBuild, data: dict, user_providers: dict) -> None:
    ...
    for moor_id, mdev_id, pconfig, env_vars in _iter_builtin_candidates(data, b.excluded, b.seen_slugs):
        if not (_any_env(env_vars) or _raw_pool_usable(moor_id)):
            continue
        ...
        b.add_builtin_row(
            moor_id, display_name, b.current_provider in (moor_id, mdev_id), model_ids, "built-in")


def _overlay_has_creds(b: _PickerBuild, pid: str, moor_slug: str, overlay) -> bool:
    ...
    if overlay.auth_type == "aws_sdk":
        ...
        has_creds = _overlay_has_env_creds(pid, moor_slug, overlay, os.environ.get)
    ...
    if not has_creds and overlay.auth_type == "external_process":
        try:
            ...
            _ext_status = get_auth_status(moor_slug) or {}
    ...
    has_creds = has_creds or _auth_store_has_provider(pid, moor_slug)
    if not has_creds:
        ...
        try:
            if _credential_pool_is_usable(moor_slug):
                ...
                try:
                    from agent.credential_pool import load_pool
                    has_creds = load_pool(moor_slug).has_credentials()
                except Exception:
                    pass
        except Exception as exc:
    ...
    if not has_creds and moor_slug == "anthropic":
        ...
        try:
            from agent.anthropic_credentials import read_claude_code_credentials, read_moor_oauth_credentials
            moor_creds = read_moor_oauth_credentials()
            cc_creds = read_claude_code_credentials()
            if (moor_creds and moor_creds.get("accessToken")) or (cc_creds and cc_creds.get("accessToken")):
                has_creds = True
        except Exception as exc:
...
def _lap_overlay_rows(b: _PickerBuild, data: dict) -> None:
    ...
    for pid, overlay in MOOR_OVERLAYS.items():
        moor_slug = mdev_to_moor.get(pid, pid)
        if _skip(b.seen_slugs, b.excluded, pid, moor_slug):
            continue
        if not _overlay_has_creds(b, pid, moor_slug, overlay):
            continue
        if moor_slug in {"openai-codex", "copilot", "copilot-acp"}:
            ...
            model_ids = _moor_picker_model_ids(b.curated, b.force_fresh_moor_tier)
        else:
            model_ids = _live_or_curated_ids(moor_slug, b.curated, moor_slug, pid)
        b.add_builtin_row(
            moor_slug, get_label(moor_slug), b.current_provider in (moor_slug, pid), model_ids, "moor")
...
def _lap_user_provider_rows(b: _PickerBuild, user_providers: dict) -> None:
    ...
    from moor_cli.model_switch import _extra_headers_from_config, _scoped_key_env
    from moor_cli.config import coerce_provider_id, is_provider_enabled
...
def _lap_custom_provider_rows(b: _PickerBuild, custom_providers: list) -> None:
    ...
    their own rows."""
    from moor_cli.model_switch import _extra_headers_from_config, _scoped_key_env
    from moor_cli.config import coerce_provider_id
...
def list_authenticated_providers(
    ...
    custom_providers: list | None = None, *, force_fresh_moor_tier: bool = False,
    ...
    ("built-in", "moor", "canonical", "user-config", "model-config").
    ``force_fresh_moor_tier`` bypasses the short Moor tier cache (account-sensitive flows only);
    ...
    from moor_cli.config import coerce_provider_id, stringify_provider_map
    ...
    # as (moor_id / mdev_id / canonical slug).
    b = _PickerBuild(
        current_provider=current_provider, current_base_url=current_base_url, current_model=current_model,
        max_models=max_models, for_picker=for_picker, force_fresh_moor_tier=force_fresh_moor_tier,
...
def list_picker_providers(
    ...
    OpenRouter's list is replaced with :func:`moor_cli.models.fetch_openrouter_models` (curated
