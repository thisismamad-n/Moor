...
def resolve_pack_plugins(pack: PluginPack) -> List[ResolvedPackPlugin]:
    ...
    for entry in pack.plugins:
        if entry.install_identifier is not None:
            ...
            continue
        try:
            from moor_cli.plugin_index import load_index, resolve_name
...
def render_pack_review(console, pack: PluginPack, resolved: List[ResolvedPackPlugin]) -> None:
    ...
    if pack.skills:
        ...
        console.print("[dim]Install them manually, e.g. `moor skills install <id>`.[/dim]")
    console.print(
...
def cmd_pack_install(source: str, *, force: bool = False) -> None:
    """``moor plugins pack install <path-or-url>``: mandatory review screen -> one pack-level
    consent -> pinned fan-out installs -> per-plugin capability consent. Exit 1 if any failed."""
    from moor_cli.plugins_cmd import _ask_yes, _console, _fail, _is_tty
    console = _console()
    pack, resolved = _load_and_review(console, source)
...
def pack_command(args) -> None:
    ...
    if handler is None:
        from moor_cli.plugins_cmd import _console, _fail
        _fail(_console(), "[red]Error:[/red] Usage: moor plugins pack {install|export|show}")
    handler(args)
