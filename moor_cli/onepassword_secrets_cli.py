"""CLI handlers for ``moor secrets onepassword ...``.

Unlike Bitwarden, the ``op`` binary is NOT auto-installed: 1Password publishes the CLI through OS
package managers and signed installers, so Moor expects an already-installed, already-
...
def cmd_setup(args: argparse.Namespace) -> int:
    ...
    console.print(
        Panel.fit(
            ...
            "Moor resolves [cyan]op://vault/item/field[/cyan] references through your\n"
    ...
    console.print(
        "  Map credentials:  [cyan]moor secrets onepassword set OPENAI_API_KEY "
        "\"op://Private/OpenAI/api key\"[/cyan]\n"
        "  Preview:          [cyan]moor secrets onepassword sync[/cyan]\n"
        "  Status:           [cyan]moor secrets onepassword status[/cyan]"
    )
    return 0
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if not enabled:
        console.print("\n  Run [cyan]moor secrets onepassword setup[/cyan] to enable.")
        return 0
    if binary and not token_set:
        ...
        if who:
            ...
            console.print(
                ...
                "Moor will warn and skip 1Password on next startup.[/yellow]"
            )
    if not references:
        console.print(
            "\n  [yellow]No references mapped yet.[/yellow]  Add one: "
            "[cyan]moor secrets onepassword set ENV_VAR \"op://…\"[/cyan]"
        )
    return 0
...
def cmd_set(args: argparse.Namespace) -> int:
    ...
    if not op_cfg.get("enabled"):
        console.print(
            ...
            "[cyan]moor secrets onepassword setup[/cyan] to turn it on.[/yellow]"
        )
    return 0
...
def cmd_token(args: argparse.Namespace) -> int:
    ...
    return rotate_token(
        ...
        disabled_note=None if op_cfg.get("enabled") else (
            ...
            "run `moor secrets onepassword setup` to turn it on.[/yellow]"
        ),
    )
...
def cmd_sync(args: argparse.Namespace) -> int:
    ...
    if not references:
        console.print(
            ...
            "`moor secrets onepassword set ENV_VAR \"op://…\"`.[/yellow]"
        )
        return 0
    ...
    console.print(
        ...
        "[cyan]moor[/cyan] invocation.  Re-run with [cyan]--apply[/cyan] to export "
...
def cmd_disable(args: argparse.Namespace) -> int:
    return disable_secret_source(
        ...
        "next Moor invocation.\n"
        "  Your reference mappings are left in config.yaml — remove them with "
        "[cyan]moor secrets onepassword remove ENV_VAR[/cyan] if you no longer "
        "need them.",
    )
...
_PLUGIN_COMPAT_LAZY = {
    'masked_secret_prompt': ('moor_cli.secret_prompt', 'masked_secret_prompt'),
}
