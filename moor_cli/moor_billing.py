"""Moor Portal Remote Spending HTTP client.

Thin, fail-loud client for the ``/api/billing/*`` endpoints the terminal billing screens drive.
``moor_account.py`` owns read-only entitlement/balance; this module owns the *write* side: buy
...
# Short-lived cache for the resolved (token, base): `resolve_moor_access_token` takes two
...
def _billing_not_logged_in(exc: Optional[BaseException] = None) -> "BillingAuthError":
    ...
    err = BillingAuthError("Not logged into Moor Portal — run `moor portal` to log in.", status=401, error="invalid_token")
    if exc is not None:
        err.__cause__ = exc
    return err
...
def _request(
    ...
    try:
        ...
        raise BillingError(f"Could not reach Moor Portal: {exc.reason}", error="network_error") from exc
        ...
        raise BillingError("Could not reach Moor Portal: timed out", error="network_error") from exc
