"""Shared helpers for attaching Moor to a local Chromium-family CDP port.
...
# the real profile into ``~/.moor/browser-profile/`` and launch the user's real binary on
...
def real_profile_copy_dir(browser: str) -> str:
    """Return the moor-owned snapshot dir for ``browser``'s real profile."""
    return str(get_moor_home() / "browser-profile" / browser)
...
def _secure_snapshot(path: str, *, contents: bool = False) -> None:
    ...
    NixOS group-share carve-out, MOOR_UID/GID) as every Moor secret dir — via ``_secure_dir``/
    ...
    ``MOOR_HOME_MODE`` hatch. Best-effort; never blocks a launch."""
    try:
        from moor_cli.config import _secure_dir, _secure_file
        if not contents:
            _secure_dir(path)
            return
...
_SNAPSHOT_DONE_MARKER = ".moor-snapshot-complete"
...
def _locked_profile_error(browser: str) -> str:
    ...
    if _real_profile_autoclose():
        msg = (
            ...
            "yet. Moor can close it for you (this quits the browser — you'll lose unsaved "
        ...
        msg = (
            ...
            "browser.use_real_profile off. (Enable browser.real_profile_autoclose to let Moor "
...
def snapshot_real_profile(browser: str, src: str | None = None) -> tuple[str | None, str | None]:
    """Snapshot ``browser``'s real ACTIVE profile into the moor copy dir; returns ``(dst, err)``.
...
def cleanup_real_profile_snapshots() -> None:
    ...
    root = str(get_moor_home() / "browser-profile")
...
def chrome_debug_data_dir() -> str:
    return str(get_moor_home() / "chrome-debug")
