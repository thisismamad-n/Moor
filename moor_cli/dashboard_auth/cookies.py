...
Three cookies in play:
  - moor_session_at:   the OAuth access token
                         (HttpOnly, lifetime = token TTL, ~15 min)
  - moor_session_rt:   the OAuth refresh token
                         (HttpOnly, lifetime = 24h, ROTATING + reuse-detected)
                         Moor Portal issues a rotating refresh token for the
                         dashboard auth-code grant (Portal NAS #293 / moor
  ...
  - moor_session_pkce: short-lived PKCE state + CSRF nonce + provider
...
setter wrote ``__Secure-moor_session_at`` would never find the value.
...
SESSION_AT_COOKIE = "moor_session_at"
SESSION_RT_COOKIE = "moor_session_rt"
...
# auth plugins are enabled (for example Basic + Moor OAuth).
SESSION_PROVIDER_COOKIE = "moor_session_provider"
PKCE_COOKIE = "moor_session_pkce"
...
SSO_ATTEMPT_COOKIE = "moor_sso_attempt"
...
def _cookie_path(prefix: str) -> str:
    ...
    Under ``X-Forwarded-Prefix: /moor`` we want ``Path=/moor`` so:
...
def set_session_cookies(
    ...
    *,
    ...
    ``refresh_token`` is written as the RT cookie when non-empty. Moor Portal
    issues a 24h rotating refresh token (moor #37247); a provider that
