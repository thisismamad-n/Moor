...
All HttpOnly, ``SameSite=Lax`` unless noted, Path = proxy prefix or /: ``moor_session_at``
(access token; Max-Age = token TTL), ``moor_session_rt`` (rotating refresh token; written only
when the provider returned one, always cleared on logout/expiry), ``moor_session_provider``
(non-secret routing hint so an RT is not handed to the wrong provider), ``moor_session_pkce``
...
cookies set on such a 302, crbug 40508226), ``moor_sso_attempt`` (auto-SSO loop guard, 60 s).
...
def _cookie_path(prefix: str) -> str:
    """``Path=/moor`` under a proxy prefix (no leak to sibling apps), else ``/``."""
...
def set_session_cookies(
    ...
    Moor Portal issues a 24h rotating refresh token (moor #37247); a provider that omits it returns
