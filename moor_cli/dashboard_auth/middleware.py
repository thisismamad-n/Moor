...
def _unauth_response(request: Request, *, reason: str) -> Response:
    ...
    Under a reverse proxy with ``X-Forwarded-Prefix: /moor``, the
    ``login_url`` is prefixed (``/moor/login?next=...``) so the
...
def _auto_sso_response(request: Request) -> Response | None:
    ...
    from moor_cli.dashboard_auth.cookies import detect_https
    set_sso_attempt_cookie(
...
def _safe_next_target(request: Request) -> str:
    ...
    # the SPA's own ``sessionStorage["moor.lastLocation"]`` fallback
...
async def gated_auth_middleware(
    ...
    # ``moor_session_at``. Verify it with the identical ``verify_session``
    ...
    # sends only ``moor_session_rt``. If we bailed on ``not at`` here we'd
    ...
    if at:
        ...
        # a self-hosted-OIDC session hits the `moor` provider first, which
        # tries to reach Moor Portal's JWKS; if that's unreachable it raises,
    ...
    if session is None:
        ...
        from moor_cli.dashboard_auth.cookies import clear_session_cookies
        from moor_cli.dashboard_auth.prefix import prefix_from_request
