...
Proxies mounting the dashboard at a path inject ``X-Forwarded-Prefix: /moor``
so the backend can build prefixed URLs (Location headers, OAuth redirect_uri,
cookie Path, SPA asset URLs). An operator-declared ``MOOR_DASHBOARD_PUBLIC_URL``
...
def _warn_if_malformed(source: str, raw: str) -> None:
    ...
    _warn_once(
        ...
        source, cleaned, cleaned.split("://")[-1] or "moor.example.com")
...
def normalise_prefix(raw: Optional[str]) -> str:
    """``"/moor"`` form (no trailing slash) or ``""`` when unset/malformed. ``..``, ``//`` and
...
def resolve_public_url() -> str:
    ...
    ``MOOR_DASHBOARD_PUBLIC_URL`` env (blank counts as unset so a provisioned-but-blank secret
