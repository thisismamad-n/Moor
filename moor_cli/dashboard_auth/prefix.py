...
prefix (e.g. ``mission-control.tilos.com/moor/*`` -> dashboard on
...
This module is also the home of the ``MOOR_DASHBOARD_PUBLIC_URL`` /
...
def _warn_if_malformed(source: str, raw: str) -> None:
    ...
    missing scheme (``moor.example.com`` instead of
    ``https://moor.example.com``) — the single most common cause of
    "I set MOOR_DASHBOARD_PUBLIC_URL but the OAuth callback is still
    ...
    _log.warning(
        ...
        source,
        cleaned,
        cleaned.split("://")[-1] or "moor.example.com",
    )
...
# ---------------------------------------------------------------------------
# MOOR_DASHBOARD_PUBLIC_URL / dashboard.public_url
# ---------------------------------------------------------------------------
...
def resolve_public_url() -> str:
    ...
    Precedence (mirrors ``dashboard.oauth.client_id``):
      ...
      1. ``MOOR_DASHBOARD_PUBLIC_URL`` env var (when non-empty after
    ...
    """
    env_raw = os.environ.get("MOOR_DASHBOARD_PUBLIC_URL", "")
    env_clean = _normalise_public_url(env_raw)
    if env_clean:
        return env_clean
    _warn_if_malformed("MOOR_DASHBOARD_PUBLIC_URL env var", env_raw)
