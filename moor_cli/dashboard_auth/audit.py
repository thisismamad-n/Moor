...
from ``moor_constants`` or other moor_cli modules — so it can be
...
def _resolve_log_path() -> Path:
    """``$MOOR_HOME/logs/dashboard-auth.log``.

    Uses ``moor_constants.get_moor_home()`` (a leaf module — no import
    cycle) so profile overrides and the native-Windows ``%LOCALAPPDATA%``
    fallback are honored.
    """
    from moor_constants import get_moor_home

    return get_moor_home() / "logs" / "dashboard-auth.log"
