"""Install and remove the Linux desktop entry (``moor.desktop``).
...
DESKTOP_ENTRY_NAME = "moor.desktop"
...
def _can_import_moor_cli(interpreter: Path) -> bool:
    """Whether *interpreter* can import ``moor_cli.main`` unaided.
...
def _running_interpreter_fallback() -> str:
    """The RUNNING interpreter — it has ``moor_cli`` importable by definition."""
    return os.path.abspath(sys.executable)


def resolve_exec_command(project_root: Optional[Path] = None) -> str:
    """Build the absolute ``Exec=`` command line for ``moor desktop``.

    Prefer the real ``moor`` launcher; fall back to ``<python> -m moor_cli.main desktop``.
    """
    from moor_cli.relaunch import resolve_moor_bin

    bin_path = _resolve_moor_bin_for_desktop_entry(resolve_moor_bin, checkout_root=project_root)
    interpreter = _running_interpreter()
    if not _can_import_moor_cli(Path(interpreter)):
        ...
        # The candidate interpreter cannot actually import moor_cli.main (checked in isolated mode from a
    ...
    argv = [interpreter, "-m", "moor_cli.main", "desktop"]
    if bin_path:
        resolved = Path(bin_path).resolve()
        # A Python launcher whose shebang points OUTSIDE the venv (e.g. the repo's `moor` script
...
def _inside_checkout(candidate: str, checkout_root: Path, original_argv0: str) -> bool:
    ...
    try:
        ...
        return False
    # Anything shipped in the tree (e.g. the repo `moor` script) is checkout-internal. Compare
    ...
    for root in {checkout_root, resolved_root}:
        if root is not None and (path == root or root in path.parents):
            return True
    # The `python -m moor_cli.main` relaunch context surfaces the invoking interpreter as
...
def _resolve_moor_bin_for_desktop_entry(
    resolve_fn=None,
    checkout_root: Optional[Path] = None,
) -> Optional[str]:
    """Resolve the launcher binary for the persisted ``.desktop`` entry.

    Wraps :func:`moor_cli.relaunch.resolve_moor_bin` with one rule: an ``argv[0]`` inside this
    ...
    if resolve_fn is None:
        from moor_cli.relaunch import resolve_moor_bin as resolve_fn
    ...
    # An external primary (another install's /opt/.../bin/moor, a venv console script) wins
    ...
    # /opt/.../bin/moor from another install method, or a venv console script). It must be evaluated
...
def _known_wrapper_candidates():
    ...
    (``/usr/local/bin``), and user (``~/.local/bin``). The wrapper is always named ``moor``.
    """
    candidates = []
    prefix = os.environ.get("PREFIX")
    if prefix:
        candidates.append(Path(prefix) / "bin" / "moor")
    if hasattr(os, "geteuid") and os.geteuid() == 0:
        candidates.append(Path("/usr/local/bin/moor"))
    candidates.append(Path.home() / ".local" / "bin" / "moor")
    return candidates
...
def render_desktop_entry(exec_command: str, icon: str) -> str:
    return (
        ...
        "Type=Application\n"
        "Name=Moor\n"
        "GenericName=Moor Desktop\n"
        "Comment=Launch Moor Desktop\n"
        f"Exec={exec_command}\n"
        f"Icon={icon}\n"
        "Terminal=false\n"
        "Categories=Utility;\n"
...
def _hicolor_icon_dest(subdir: str) -> Path:
    return _xdg_data_home() / "icons" / "hicolor" / subdir / "apps" / "moor.png"
...
def _install_icon_to_hicolor(icon: Path) -> bool:
    """Install the app icon into the user's hicolor tree so ``Icon=moor`` resolves without an
...
def install_desktop_entry(project_root: Path) -> Optional[Path]:
    """Write (or refresh) the Moor desktop entry and return its path.
    ...
    icon_value = str(icon) if icon.is_file() else "moor"
