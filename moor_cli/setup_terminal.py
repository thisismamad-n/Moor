...
setup.py names are resolved through the module object so test patches on ``moor_cli.setup.<name>``
...
def _setup_backend_local(config: dict) -> None:
    ...
    # Gateway cwd defaults to home; sudo stays off. Both configurable via `moor setup terminal`.
...
def _setup_backend_docker(config: dict) -> None:
    ...
    if _setup.prompt_yes_no("  Enable egress firewall for Docker sandboxes?", False):
        ...
        _setup.print_info(
            "Run `moor egress setup` then `moor egress start` to mint tokens and launch the proxy.")
    else:
        _setup.print_info("Skipping egress firewall. You can enable it later with `moor egress setup`.")
...
def _setup_backend_modal(config: dict) -> None:
    ...
    managed_modal_available = bool(
        tool_backend_helpers.managed_moor_tools_enabled()
        and moor_subscription.get_moor_subscription_features(config).moor_auth_present
    ...
    if managed_modal_available:
        ...
        use_managed_modal = _setup.prompt_choice(
            ...
            ["Use my Moor subscription", "Use my own Modal account"], default_idx) == 0
    if use_managed_modal:
        config["terminal"]["modal_mode"] = "managed"
        _setup.print_info("Modal execution will use the managed Moor gateway and bill to your subscription.")
...
def setup_terminal_backend(config: dict):
    ...
    _setup._info("Choose where Moor runs shell commands and code.",
    ...
    # Plugin-registered backends (~/.moor/plugins/). Fail-soft: a broken plugin must not take
    # the wizard down.
