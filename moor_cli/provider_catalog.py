...
The provider list shown by ``moor model`` (CLI/TUI) and the desktop Settings → Providers tabs
...
come from :data:`moor_cli.auth.PROVIDER_REGISTRY` (credential truth); ``display_name`` /
...
def provider_catalog() -> list[ProviderDescriptor]:
    """One descriptor per provider in the ``moor model`` universe (:data:`CANONICAL_PROVIDERS`,
    ...
    PROVIDER_REGISTRY = _safe_import("moor_cli.auth", "PROVIDER_REGISTRY", {})
    OPTIONAL_ENV_VARS = _safe_import("moor_cli.config", "OPTIONAL_ENV_VARS", {})
    ...
    MOOR_OVERLAYS = _safe_import("moor_cli.providers", "MOOR_OVERLAYS", {})
    try:
