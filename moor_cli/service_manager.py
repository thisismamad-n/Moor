...
the existing module-level functions in moor_cli.gateway and
moor_cli.gateway_windows directly. This protocol is a thin facade
...
def detect_service_manager() -> ServiceManagerKind:
    ...
    # `moor gateway start/stop/restart` fell through to host code that spawns
...
def _s6_running() -> bool:
    ...
    Detection has to work for **both** root and the unprivileged moor
    ...
    detection always fails. Since every Moor runtime call inside the
    container drops to moor via ``s6-setuidgid``, that silent failure
...
# in ``moor_cli.gateway`` (systemd/launchd) and ``moor_cli.gateway_windows``
...
# active (set via the ``moor -p <profile>`` flag before the call). This
...
class SystemdServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around the ``systemd_*`` functions in moor_cli.gateway.
    ...
    def start(self, name: str) -> None:
        from moor_cli.gateway import systemd_start
        systemd_start()

    def stop(self, name: str) -> None:
        from moor_cli.gateway import systemd_stop
        systemd_stop()

    def restart(self, name: str) -> None:
        from moor_cli.gateway import systemd_restart
        systemd_restart()

    def is_running(self, name: str) -> bool:
        from moor_cli.gateway import _probe_systemd_service_running
        _, running = _probe_systemd_service_running()
        return running


class LaunchdServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around the ``launchd_*`` functions in moor_cli.gateway."""

    kind: ServiceManagerKind = "launchd"

    def start(self, name: str) -> None:
        from moor_cli.gateway import launchd_start
        launchd_start()

    def stop(self, name: str) -> None:
        from moor_cli.gateway import launchd_stop
        launchd_stop()

    def restart(self, name: str) -> None:
        from moor_cli.gateway import launchd_restart
        launchd_restart()

    def is_running(self, name: str) -> bool:
        from moor_cli.gateway import _probe_launchd_service_running
        return _probe_launchd_service_running()


class WindowsServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around ``moor_cli.gateway_windows`` (Scheduled Task /
...
# runs inside the container (Phase 4). Static services (main-moor, dashboard)
...
def _profile_dir_for_gateway_service(name: str) -> Path:
    ...
    ``gateway stop --all``. Do not write the caller's MOOR_HOME blindly;
    derive the shared profile root from the current MOOR_HOME and map the
...
# UID/GID of the in-image ``moor`` user. Hardcoded to match what
...
def _seed_supervise_skeleton(svc_dir: Path) -> None:
    ...
    inside a service directory, owned by the moor user.
    ...
    (the ``moor`` user — UID 10000 — running every Moor runtime
    ...
    is dispatched as the moor user.
    ...
    with moor ownership before triggering ``s6-svscanctl -a``,
    s6-supervise inherits our layout and never touches it.

    Layout produced
    ---------------
    ``svc_dir/``                           moor:moor, 0755 (parent must already exist)
    ``svc_dir/event/``                     moor:moor, 03730   (setgid + g+rwx + sticky)
    ``svc_dir/supervise/``                 moor:moor, 0755
    ``svc_dir/supervise/event/``           moor:moor, 03730
    ``svc_dir/supervise/control``          moor:moor, 0660    (FIFO)
    ...
    slot's supervise/ was moor-owned.
    ...
    def _mkdir_owned(path: Path, mode: int) -> None:
        ...
        path.chmod(mode)
        try:
            os.chown(path, _MOOR_UID, _MOOR_GID)
        except PermissionError:
            # Running as the moor user already — directory is moor-
    ...
    if not control.exists():
        ...
        control.chmod(0o660)
        try:
            os.chown(control, _MOOR_UID, _MOOR_GID)
        except PermissionError:
            pass
    ...
    # when the parent slot's supervise/ is moor-owned.
    log_dir = svc_dir / "log"
    if log_dir.is_dir():
        ...
        if not log_control.exists():
            ...
            try:
                os.chown(log_control, _MOOR_UID, _MOOR_GID)
            except PermissionError:
                pass
...
class GatewayNotRegisteredError(S6Error):
    ...
    Most commonly: ``moor -p typo gateway start`` when no profile
...
class S6ServiceManager:
    ...
    ``S6_DYNAMIC_SCANDIR``. Static services (main-moor, dashboard)
    ...
    def _render_run_script(
        ...
        The script:
          1. Sources MOOR_HOME (and any extra env) via with-contenv —
             so e.g. ``-e MOOR_HOME=/data/moor`` is honored at run
          ...
          4. Drops to the moor user and exec's
             ``moor -p <profile> gateway run`` (or just ``moor
        ...
        root MOOR_HOME profile" (the implicit profile that exists at
        the top of $MOOR_HOME, not under profiles/). It must be
        spelled this way because ``_profile_suffix()`` returns the
        empty string for the root profile, and the dispatcher in
        ``moor_cli.gateway`` maps that empty string to the
        ``gateway-default`` service slot. Passing ``-p default`` here
        would instead look up ``$MOOR_HOME/profiles/default/`` — a
        ...
        each supervised profile gateway loads its own ``MOOR_HOME``,
        ...
        allocator in ``moor_cli.profiles._allocate_gateway_port``).
        ...
        # guard.
        lines.append("export MOOR_S6_SUPERVISED_CHILD=1")
        # ``--replace`` makes the supervised gateway authoritative for its
        # profile's MOOR_HOME. Without it, a gateway started OUTSIDE s6
        # (a stray ``moor gateway run`` from a shell, an agent action, or
        # the Open WebUI helper) grabs the per-MOOR_HOME PID lock first;
        ...
        # gateway/run.py) so s6 always wins. The MOOR_S6_SUPERVISED_CHILD
        # sentinel above prevents the run→start→run redirect recursion.
        # Each profile is scoped to its own MOOR_HOME and s6 guarantees a
        # single supervised instance per slot, so there is no legitimate
        # supervised sibling for ``--replace`` to clobber.
        if profile == "default":
            gateway_cmd = "moor gateway run --replace"
        else:
            gateway_cmd = f"moor -p {shlex.quote(profile)} gateway run --replace"
        # Skip the drop when already non-root (setgroups() lacks CAP_SETGID →
        # s6 boot-loop).
        lines.append(f'[ "$(id -u)" = 0 ] || exec {gateway_cmd}')
        lines.append(f"exec s6-setuidgid moor {gateway_cmd}")
        return "\n".join(lines) + "\n"
    ...
    def _render_log_run(profile: str) -> str:
        ...
        OQ8-C: persist to ``${MOOR_HOME}/logs/gateways/<profile>/``.
        CRITICAL: the MOOR_HOME path is sourced from the runtime env
        via with-contenv — NOT Python-substituted at registration time
        — so a container started with ``-e MOOR_HOME=/data/moor``
        gets its logs under /data/moor/logs/..., not the build-time
        default.
        ...
        return (
            ...
            f': "${{MOOR_HOME:=/opt/data}}"\n'
            f'log_dir="$MOOR_HOME/logs/gateways/{prof}"\n'
            # Create the leaf and clear a stale s6-log lock as moor when
            # this script starts as root. Never chown or unlink moor-writable
            # volume paths from this restartable root-context script:
            # log/supervise/control is moor-owned, so an unprivileged user
            # can race a pathname op through a symlink swap (CWE-59 /
            # CWE-367). Parent logs/gateways is seeded moor-owned at stage2
            # boot (#45258; tests/docker/test_log_dir_seed.py).
            f'if [ "$(id -u)" = 0 ]; then\n'
            f'  s6-setuidgid moor mkdir -p "$log_dir"\n'
            f'  s6-setuidgid moor rm -f "$log_dir/lock"\n'
            f'else\n'
            f'  mkdir -p "$log_dir"\n'
            f'  rm -f "$log_dir/lock"\n'
            f'fi\n'
            ...
            f'exec s6-setuidgid moor s6-log 1 n10 s1000000 T "$log_dir"\n'
        )
    ...
    def stop(self, name: str) -> None:
        ...
        intent). Without the marker, an intentional ``moor gateway
    ...
    def register_profile_gateway(
        ...
        *,
        ...
        user explicitly runs ``moor -p <profile> gateway start``.
        ...
        try:
            ...
            # Pre-create the supervise/ skeleton with moor ownership
            ...
            # the moor user) won't hit EACCES on root-owned 0700
    ...
    def unregister_profile_gateway(self, profile: str) -> None:
        ...
        **before** ``rmtree`` so s6-svscan reaps the supervise child
        ...
        # is moor-owned (see ``_seed_supervise_skeleton``), and on
