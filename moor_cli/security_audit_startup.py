...
3. Running inside a container with no persistent volume mount over the
   MOOR_HOME data dir (state is ephemeral — lost on container restart).
...
def _running_as_root() -> Optional[str]:
    ...
    return (
        ...
        "full host compromise. Run Moor as an unprivileged user (or in a "
...
def run_security_audit(
    *, moor_home: Optional[Path] = None, config: Optional[dict] = None
) -> list[str]:
    ...
    for check in (
        ...
        try:
            ...
            continue
    try:
        r = _container_no_volume_mount(moor_home)
        if r:
            findings.append(r)
    except Exception:
        pass
