...
def _cmd_list(args) -> int:
    ...
    if args.installed:
        ...
        if not pets:
            print("No pets installed. Try: moor pets install boba")
            return 0
...
def _cmd_install(args) -> int:
    ...
    if args.select or not _has_active_pet():
        ...
        print(f"  Make it active with: moor pets select {slug}")
    return 0
...
def _cmd_doctor(args) -> int:
    ...
    ok = True
    if not pets:
        print("  → no pets installed. Run: moor pets install boba")
        ok = False
    elif active is None:
        print("  → active pet unresolved. Run: moor pets select <slug>")
        ok = False
    elif not enabled:
        print("  → pet display is disabled. Run: moor pets select " + active.slug)
