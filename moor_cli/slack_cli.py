"""``moor slack ...`` CLI subcommands.

Today only ``moor slack manifest`` is implemented — it generates the
...
Typical workflow::
    ...
    $ moor slack manifest > slack-manifest.json
    # or:
...
def _build_full_manifest(
    ...
    """
    from moor_cli.commands import slack_app_manifest
...
def slack_manifest_command(args) -> int:
    ...
    Flags (all parsed in ``moor_cli/main.py``):
      --write [PATH]  Write to file instead of stdout (default path:
                      ``$MOOR_HOME/slack-manifest.json``)
    ...
    if getattr(args, "slashes_only", False) and (
        ...
        print(
            "moor slack manifest: long description options cannot be used "
            "with --slashes-only",
            file=sys.stderr,
        )
        return 2
    if long_description_file is not None:
        ...
        try:
            ...
            print(
                f"moor slack manifest: cannot read long description from "
                f"{source_arg}: {exc}",
                file=sys.stderr,
            )
            return 2
    if (
        ...
        print(
            "moor slack manifest: long description must be at least "
    ...
    if (
        ...
        print(
            "moor slack manifest: long description must be at most "
