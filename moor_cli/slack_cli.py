"""``moor slack manifest`` — generate the Slack app manifest JSON that registers every gateway
...
def _build_full_manifest(
    ...
    Other sections (OAuth scopes, socket mode) are sensible Moor defaults, tweakable in the Slack
    UI after pasting.
    """
    from moor_cli.commands_platforms import slack_app_manifest
    ...
    if messaging_experience == "assistant":
        ...
        features["agent_view"] = {"agent_description": "Chat with Moor in Slack Messages."}
        bot_scopes.append("assistant:write")
...
def slack_manifest_command(args) -> int:
    """Print or write a Slack app manifest JSON (flags documented in ``moor_cli/main.py``)."""
    ...
    def fail(msg: str) -> int:
        print(f"moor slack manifest: {msg}", file=sys.stderr)
        return 2
