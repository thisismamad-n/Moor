"""Host-platform checks for moor doctor: interpreter, SQLite, certificates, macOS TCC, gateway supervision, command install.
Split out of ``moor_cli/doctor.py``, which re-exports every name so ``moor_cli.doctor.<name>`` keeps resolving (and monkeypatching)."""
...
from moor_cli.config import is_nix_install_method, recommended_update_command_for_method
...
def _sqlite_upgrade_hint(install_method: str | None = None) -> str:
    ...
    action = cmd if is_nix_install_method(method) else {  # nix: prose guidance, not a shell command
        "docker": f"run `{cmd}`, then recreate all Moor containers", "apt": f"run `{cmd}`"}.get(method, "run `moor update`")
...
def _moor_database_paths(moor_home: Path) -> list[tuple[str, Path]]:
    """(display name, path) pairs for moor-managed SQLite databases: backup.py's per-profile store list + per-board kanban.db."""
    from moor_cli.backup import _QUICK_STATE_FILES
    entries = [(name, moor_home / name) for name in _QUICK_STATE_FILES if name.endswith(".db")]
...
def _read_journal_mode(db_path: Path) -> tuple[str | None, str | None]:
    ...
    process's POSIX advisory locks (see ``moor_cli.sqlite_safe_read``), and the dashboard console runs
...
def _report_database_journal_modes(moor_home: Path | None = None, version_info: tuple[int, ...] | None = None) -> None:
    ...
    from moor_state_wal import _wal_reset_repair_hint, is_sqlite_wal_reset_vulnerable
    vulnerable = is_sqlite_wal_reset_vulnerable(version_info)
    try:
        databases = _moor_database_paths(moor_home if moor_home is not None else MOOR_HOME)
...
def _check_version_consistency(issues: list[str]) -> None:
    """Detect pyproject.toml vs moor_cli.__version__ drift (a conflict resolution can revert one but not the
    ...
    _fail_and_issue("Version mismatch between source files", f"(pyproject.toml {pyproject_version} != moor_cli/__init__.py {init_version})",
                    "Re-sync version files (e.g. run 'moor update', or set moor_cli/__init__.py __version__ to match pyproject.toml)", issues)
...
def _check_s6_supervision(issues: list[str]) -> None:
    ...
    try:
        from moor_cli.service_manager import S6ServiceManager, detect_service_manager
    except Exception:
        return
    ...
    for static in ("main-moor", "dashboard"):  # s6-rc symlinks under /run/service/, same s6-svstat probe
    ...
    if not profiles:
        return check_info("No per-profile gateways registered yet — create one with `moor profile create <name>`")
...
def check_certificates(should_fix: bool = False, issues: "list | None" = None) -> None:
    ...
    if not should_fix:
        issues.append(f"Repair the CA bundle: run `moor doctor --fix`, or `{pip_cmd}`")
        return
...
def _check_gateway_service_linger(issues: list[str]) -> None:
    ...
    try:
        from moor_cli.gateway import get_systemd_linger_status, get_systemd_unit_path, is_linux
...
_TCC_CDHASH_DETAIL = (
    ...
    "all permission grants. Run `moor update` to get the stable identifier-pinned signing identity, "
    "then re-grant permissions once.")
_TCC_STABLE_DETAIL = {
    True: "(certificate-anchored DR; grants survive rebuilds)",
    False: "(identifier-pinned DR; grants survive rebuilds — for the strongest anchor, see `moor desktop --setup-tcc-identity`)",
}
...
def check_macos_tcc_grants() -> None:
    ...
    check_info("If macOS still re-prompts for permissions (toggle shows ON): the stored grant is stale — run "
               "`tccutil reset ScreenCapture com.moorinc.moor` (repeat per affected service), toggle it ON in "
               "System Settings, then fully quit & relaunch Moor once.")


def _desktop_app_bundle() -> Path | None:
    """Locate the locally-built desktop bundle (``apps/desktop/release/mac-<arch>/Moor.app``), newest first.

    The only layout whose ad-hoc re-signed bundle can invalidate TCC grants. ``/Applications/Moor.app`` is
    deliberately not probed: it is the separately-signed, certificate-anchored moor-setup launcher.
    """
    release_dir = Path(__file__).resolve().parents[1] / "apps" / "desktop" / "release"
    candidates = [p for p in release_dir.glob("mac*/Moor.app") if p.is_dir()]
...
def check_macos_tcc_anchor(should_fix: bool = False) -> None:
    ...
    with warn_on_error("macOS TCC anchor check failed"):
        from moor_cli import macos_tcc_anchor as tcc
...
def check_macos_full_disk_access() -> None:
    ...
    try:
        ...
        check_info("One switch silences all macOS folder prompts: grant your terminal app Full Disk Access and Moor "
                   ...
                   "    then enable your terminal (and Moor.app if you use Desktop), and restart them once. "
                   "With Moor' stable signing identities the grant survives every update.")
    except OSError:
...
def _check_security_advisories(should_fix: bool, f: Finding) -> None:
    ...
    from moor_cli.security_advisories import detect_compromised, filter_unacked, full_remediation_text, get_acked_ids
    ...
    for hit in fresh_hits:
        ...
        _fail_and_issue(f"{hit.advisory.title}", f"({hit.package}=={hit.installed_version})",
                        ...
                        f"and rotate credentials, then run `moor doctor --ack {hit.advisory.id}`.", f.manual_issues)
...
def _check_python_environment(should_fix: bool, f: Finding) -> None:
    ...
    with warn_on_error("SQLite version probe failed: {e}", ""):
        import sqlite3
        from moor_state_wal import is_sqlite_wal_reset_vulnerable, sqlite_source_id
        src = sqlite_source_id()
        # Warn-only: Moor already refuses WAL on fresh DBs and runtime repair is best-effort.
...
def _check_command_installation(should_fix: bool, f: Finding) -> None:
    ...
    venv_bin = next((c for c in (PROJECT_ROOT / n / "bin" / "moor" for n in ("venv", ".venv")) if c.exists()), None)
    if venv_bin is None:
        check_warn("Venv entry point not found", "(moor not in venv/bin/ or .venv/bin/ — reinstall with pip install -e '.[all]')")
    ...
    if link.is_symlink():
        ...
        check_fail(f"{display}/moor not found", "(moor command may not work outside the venv)")
        if not should_fix:
            return f.issues.append(f"Missing {display}/moor symlink — run 'moor doctor --fix'")
