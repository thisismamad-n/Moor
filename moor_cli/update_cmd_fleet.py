"""Gateway fleet restart + post-update verification for ``moor update``.

Split out of ``moor_cli/update_cmd.py``; every name is re-imported there so
``moor_cli.update_cmd.<name>`` keeps resolving/monkeypatching. Origin helpers are
...
from moor_cli.update_inventory import _gateway_service_matches_profile

# Log-record parity with the origin module.
logger = logging.getLogger("moor_cli.update_cmd")

# Under MOOR_HOME (not next to the venv): records the fleet-restart obligation
...
_LIST_GATEWAY_UNITS = ["list-units", "moor-gateway*", "moor-serve*", "--plain", "--no-legend", "--no-pager"]
...
def _fleet_restart_pending_marker_path() -> Path:
    """MOOR_HOME breadcrumb for a pull that has not yet restarted the fleet."""
    from moor_cli.update_cmd import get_moor_home
...
def _receipt_looks_unfinished(receipt: dict) -> bool:
    ...
    successful receipt look unfinished, or the next ``moor update`` retriggers
...
def _warn_pending_fleet_restart(*, startup: bool = False) -> None:
    ...
    print("⚠ A previous `moor update` pulled new code but did not restart running gateways.", file=stream)
    print("  Gateways may still be serving pre-update modules (mixed sys.modules).", file=stream)
    if startup:
        print("  Run `moor update` or `moor gateway restart`.", file=stream)
...
def _run_pending_fleet_restart() -> bool:
    ...
    # Warn if legacy Moor gateway unit files are still installed. When both moor.service (from a
    # pre-rename install) and the current moor-gateway.service are enabled, they SIGTERM-fight for the
    # same bot token (see PR #11909). Flagging here means every `moor update` surfaces the issue until the
    # user migrates.
    try:
...
def _apply_pending_fleet_restart_catchup() -> None:
    ...
    """
    from moor_cli.update_cmd import _run_pending_fleet_restart
    ...
    if _run_pending_fleet_restart():
        ...
        return
    print("  ⚠ Fleet restart incomplete. Recover with: moor gateway restart")
    sys.exit(1)
...
def _for_each_systemd_gateway_unit(list_units_stdout: str, *, process_unit, on_unit_timeout) -> None:
    """Process each moor-gateway*/moor-serve* unit from ``systemctl list-units``.
    ...
    for line in (list_units_stdout or "").strip().splitlines():
        ...
        if not parts:
            continue
        unit = parts[0]
        if not unit.endswith(".service") or not _is_moor_gateway_unit(unit):
            continue
...
def _service_unit_supports_graceful_sigusr1_restart(svc_name: str) -> bool:
    ...
    Only ``moor-gateway*`` runs ``gateway/run.py`` (the handler); SIGUSR1 would just
    kill ``moor-serve*`` and burn the drain budget, so those go straight to the blunt
    restart. Same exact/hyphenated shape as ``_for_each_systemd_gateway_unit`` so a
    near-prefix unit like ``moor-gatewayd`` is never signalled.
    ...
    try:
        ...
        try:
            ...
            print(
                ...
                "Recover manually: moor gateway restart"
            )
...
def _restart_macos_launchd_gateways(restarted_services: list, failed_or_stale_units: list, drain_budget: float) -> None:
    ...
    The pull is shared across profiles, so every ``ai.moor.gateway*`` LaunchAgent
...
def _surviving_gateway_pids_after_failed_restart():
    ...
    ``None`` when undeterminable (notably ``moor_cli.gateway`` no longer importing
...
# NOT a claim that no supervisor exists: a systemd-launched serve sets neither MOOR_SPAWN
# nor MOOR_PARENT_PID ("manual-serve"). Unit-backed serves are recovered by the fresh
...
_SERVE_SKIP_REASON = (
    ...
    " systemd unit pass when it owns a moor-serve* unit, else left running for explicit"
    " operator restart"
)
...
def _gateway_recovery_partition(plan, *, skip_profiles: set[str] | None = None) -> tuple[dict[str, str], list[dict]]:
    ...
    Uses only the pre-checkout inventory: re-importing ``moor_cli.gateway`` in the
    ...
    child's ``moor-serve*`` systemd pass enumerates units from systemd; leftovers are
...
def _warn_gateway_restart_phase_aborted(exc: BaseException, pids) -> None:
    ...
    logged at debug level, so an early failure (e.g. importing ``moor_cli.gateway`` from the freshly
...
def _resolve_manage_cmd(cache: dict, scope_: str, scope_cmd_: list, svc_name_: str):
    ...
    so a least-privilege sudoers entry scoped to moor-gateway* qualifies (idempotent
...
def _restart_one_systemd_gateway_unit(
    ...
    # → drain → exit, Restart=always respawns. moor-serve has no handler → blunt restart below.
    _main_pid = 0
...
def _restart_systemd_gateway_units(restarted_services, failed_or_stale_units, restarted_scoped_units, drain_budget):
    ...
    """
    from moor_cli.gateway import supports_systemd_services, _ensure_user_systemd_env
...
def _force_kill_stuck_gateways(killed_pids) -> None:
    ...
    with _best_effort('Post-restart survivor sweep failed: %s'):
        from moor_cli.gateway import find_gateway_pids, _get_service_pids
...
def _restart_gateway_fleet_after_update(_pre_update_plan, gateway_mode: bool):
    ...
    """
    from moor_cli.update_cmd import _m, _write_gateway_update_exit_code
    ...
    # Scope-qualified twin (``user/moor-serve`` vs ``system/moor-serve`` are different
    ...
    # Purge stale cached Moor modules FIRST: the import below loads new gateway
    ...
    try:
        ...
        from moor_cli.gateway import (  # noqa: F401
            is_macos,
            find_gateway_pids,
        ...
        # macOS: EVERY ai.moor.gateway* LaunchAgent (systemd parity).
        if is_macos():
...
def _print_legacy_units_warning() -> None:
    """Legacy moor.service fights moor-gateway.service over the bot token; warn on
    every update until migrated."""
    from moor_cli.gateway import (has_legacy_moor_units, _find_legacy_moor_units, supports_systemd_services)
    if not (supports_systemd_services() and has_legacy_moor_units()):
        return
    print()
...
def _verify_fleet_after_update(restart, *, _pre_update_plan, _windows_gateway_resume, node_failures, update_complete):
    ...
    # units, so a unit-less `moor serve` keeps stale sys.modules. Runs AFTER
    ...
    # units, so an sshd-spawned `serve --isolated` or a manual `moor serve` (no unit) is left running its
    ...
    with _best_effort('Fleet version verification failed: %s'):
        from moor_cli.update_receipt import print_fleet_version_matrix
    ...
    with _best_effort('Runtime-outcome reconciliation failed: %s'):
        ...
        if _pre_update_plan is not None and _pre_update_plan.runtimes:
            from moor_cli.update_inventory import (match_runtime_outcomes, report_unaccounted_runtimes)
...
def _restart_phase_failure_is_incomplete(surviving, pre_restart_pids) -> bool:
    ...
    * ``surviving is None`` — the survivor probe could not determine state (typically the freshly-pulled
    ``moor_cli.gateway`` no longer imports, one of the ways the phase aborts). That is proof-of-safety
