...
`moor backup` creates a zip archive of the entire ~/.moor/ directory
(excluding the moor-agent repo and transient files).
...
from moor_constants import get_default_moor_root, get_moor_home, display_moor_home
from utils import (
    _preserve_file_mode,
...
# Where ``moor backup --quick`` / ``/snapshot`` / the pre-update safety net
...
# ``moor-agent`` is special-cased to root level only in ``_should_exclude``
# so that skill directories like ``skills/autonomous-ai-agents/moor-agent/``
...
# MOOR_HOME gets walked file-by-file, ballooning a backup to hundreds of
...
_EXCLUDED_DIRS = {
    ...
    "browser-profiles",
    # Python dependency trees (plugin / MCP-server venvs under MOOR_HOME) —
...
# File names that ``moor import`` must never overwrite, matched by basename so
...
#     stuck "starting"/"cooking", disconnecting it from the Moor portal
...
# Reserved archive subtree for provider state that lives OUTSIDE MOOR_HOME
...
def _collect_memory_provider_external_paths() -> List[Path]:
    ...
    outside MOOR_HOME, resolved from config only (no network, no init).
...
def _should_exclude(rel_path: Path) -> bool:
    ...
    for part in parts:
        if part not in _EXCLUDED_DIRS:
            continue
        # ``moor-agent`` only matches at the root level (first component).
        # Nested directories with the same name — e.g.
        # ``skills/autonomous-ai-agents/moor-agent/`` — must be preserved.
        if part == "moor-agent" and part != parts[0]:
            continue
        return True
...
def _should_skip_backup_file(abs_path: Path, rel_path: Path, out_path: Path) -> bool:
    ...
    # write can copy data from outside MOOR_HOME.
...
# to the user as a hung `moor update` (#70553 follow-up). Sessions databases
...
def verify_sqlite_integrity(
    ...
    *,
    ...
    if check_header:
        ...
        # moor_cli.sqlite_safe_read). Verification targets snapshots and
...
def _run_backup_locked(args, moor_root: Path) -> None:
    ...
    try:
        if args.output:
            ...
            if out_path.is_dir():
                ...
                out_path = out_path / f"moor-backup-{stamp}.zip"
        else:
            stamp = datetime.now().strftime("%Y-%m-%d-%H%M%S")
            out_path = Path.home() / f"moor-backup-{stamp}.zip"
    ...
    for dirpath, dirnames, filenames in os.walk(moor_root, followlinks=False):
        dp = Path(dirpath)
        rel_dir = dp.relative_to(moor_root)

        # Prune excluded directories in-place so os.walk doesn't descend
        # ``moor-agent`` is only pruned at the root level; nested dirs
        ...
        for fname in filenames:
            fpath = dp / fname
            rel = fpath.relative_to(moor_root)
    ...
    # outside MOOR_HOME, so the walk above never sees it. Ask the active
    ...
    if external_to_add:
        print(
            ...
            f"stored outside {display_moor_home()}."
        )
...
# Critical state files to include in quick snapshots (relative to MOOR_HOME).
...
_QUICK_STATE_FILES = (
    ...
    # are at $MOOR_HOME/<name> for the default/root profile; on non-root
    # profiles the real path is outside MOOR_HOME and the entry is silently
...
def _create_quick_snapshot_locked(
    ...
    if failed_dbs:
        ...
        print(
            ...
            f"{root} and run: moor snapshot list"
        )
        logger.error(
...
# Relative path of the cron job database inside MOOR_HOME. Kept in sync with
...
def _write_full_zip_backup_locked(out_path: Path, moor_root: Path) -> Optional[Path]:
    """Write a full zip snapshot of ``moor_root`` to ``out_path``.
...
def _pre_update_backup_dir(moor_home: Optional[Path] = None) -> Path:
    home = moor_home or get_moor_home()
...
def create_pre_update_backup(
    ...
    """Create a full zip backup of MOOR_HOME under ``backups/``.

    Mirrors :func:`run_backup` (same exclusion rules, same SQLite safe-copy)
    but writes to ``<MOOR_HOME>/backups/pre-update-<timestamp>.zip`` and
    ...
    backup_dir = _pre_update_backup_dir(moor_root)
    try:
    ...
    result = _write_full_zip_backup(out_path, moor_root)
    if result is None:
        return None
...
def create_pre_migration_backup(
    ...
    """Create a full zip backup of MOOR_HOME under ``backups/`` before a
    ...
    ``<MOOR_HOME>/backups/pre-migration-<timestamp>.zip`` and auto-prunes
