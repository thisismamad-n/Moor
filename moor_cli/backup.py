"""Backup and import commands for moor CLI."""
...
from moor_constants import (
    _get_platform_default_moor_home, get_default_moor_root, get_moor_home, display_moor_home,
)
from utils import (
...
# Where ``moor backup --quick`` / ``/snapshot`` / the pre-update safety net write state
...
_EXCLUDED_DIRS = {
    "moor-agent",     # the codebase repo — re-clone instead
...
# moor-managed runtime downloads (GGUF models, llama.cpp runtimes, managed Node): re-downloaded
# on demand and routinely tens to hundreds of GB. Matched ONLY at the root of MOOR_HOME and at
...
# Files ``moor import`` must never overwrite, matched by basename so root and named profiles are
...
# disconnected from the Moor portal); PID/lock/registry files reference source PIDs. Mirrors
...
# Reserved archive subtree for memory-provider state OUTSIDE MOOR_HOME (e.g. ~/.honcho, via
...
@contextmanager
def _backup_operation_lock(moor_home: Path, timeout_seconds: float = 0.25):
    """Acquire one cross-process backup slot for full and quick snapshots."""
    lock_path = moor_home / ".backup.lock"
...
def _should_exclude(rel_path: Path) -> bool:
    """Return True if *rel_path* (relative to moor root) should be skipped."""
    parts = rel_path.parts
    if _in_excluded_root_dir(rel_path):
        return True
    # ``moor-agent`` only matches at the root level; nested same-named dirs are preserved.
    if any(p in _EXCLUDED_DIRS and (p != "moor-agent" or p == parts[0]) for p in parts):
        return True
    name = rel_path.name
    return name in _EXCLUDED_NAMES or name.startswith(_EXCLUDED_PREFIXES) or name.endswith(_EXCLUDED_SUFFIXES)


def _iter_backup_files(moor_root: Path, out_path: Path, skipped_dirs: Optional[set] = None):
    ...
    shared by ``moor backup`` and the pre-update / pre-migration path so they can never drift.
    """
    for dirpath, dirnames, filenames in os.walk(moor_root, followlinks=False):
        ...
        kept = [
            d for d in dirnames
            if (d not in _EXCLUDED_DIRS or (d == "moor-agent" and not is_root))
...
# 30 GB state.db, reading as a hung ``moor update``) is replaced by the O(1) header+schema probe.
...
def _unlink_move_restore_db(src: Path, dst: Path) -> bool:
    ...
    try:
        ...
        logger.error("Refusing unlink+move restore of %s: %s Close the in-process "
                     "database handles (or restart Moor) and retry.", dst, exc2)
        return False
...
def _resolve_backup_output_path(output: Optional[str]) -> Path:
    ...
    out_path = None
    default_name = f"moor-backup-{datetime.now().strftime('%Y-%m-%d-%H%M%S')}.zip"
    try:
        if output:
...
def run_backup(args) -> None:
    ...
    try:
        with _backup_operation_lock(moor_root):
            _run_backup_locked(args, moor_root)
    except BackupInProgressError as exc:
        print(f"Error: {exc}")
        raise SystemExit(2) from exc


def _run_backup_locked(args, moor_root: Path) -> None:
    ...
    print(f"Scanning {display_moor_home()} ...")
    skipped_dirs: set = set()
    files_to_add: list[tuple[Path, Path]] = list(_iter_backup_files(moor_root, out_path, skipped_dirs))
    ...
    print(f"\nBackup {'incomplete' if errors else 'complete'}: {out_path}\n"
          ...
          f"  Time:        {elapsed:.1f}s")
    if external_to_add:
        print(f"\n  Included {len(external_to_add)} memory-provider file(s) stored outside {display_moor_home()}.")
    if skipped_external:
...
def _validate_backup_zip(zf: zipfile.ZipFile) -> tuple[bool, str]:
    ...
    # Telltale files a moor home has — at the root or one level deep (zipped directory).
    if not any(Path(n).name in {"config.yaml", ".env", "state.db"} for n in names):
        return False, "zip does not appear to be a Moor backup (no config.yaml, .env, or state databases found)"
    return True, ""
...
def _detect_prefix(zf: zipfile.ZipFile) -> str:
    ...
    if len(first_parts) == 1 and first_parts <= {".moor", "moor"}:
...
def _count_session_rows(path: Path) -> Optional[Tuple[int, int]]:
    ...
    ``None`` means "unknown" (missing, not a Moor session store, unreadable) — never "zero":
...
def _confirm_import_overwrite(moor_root: Path) -> bool:
    """Prompt before importing over an existing installation; True when import may proceed."""
    if not any((moor_root / m).exists() for m in ("config.yaml", ".env")):
        return True
    print("\nWarning: Target directory already has Moor configuration.\n"
...
def _import_members(
    zf: zipfile.ZipFile, members: List[str], prefix: str, moor_root: Path, file_count: int
    ...
    for member in members:
        ...
        # NOT under MOOR_HOME; provider configs commonly hold credentials, so tighten to 0600.
        ...
        if external:
            ...
            if rel.endswith(_SQLITE_SIDECAR_SUFFIXES):
                skipped_runtime.append(rel)
                continue
            target = moor_root / rel
            root = moor_root.resolve()
...
def run_import(args) -> None:
    ...
    # ``get_default_moor_root()`` would silently retarget a profile restore at the live root.
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        print(f"Backup contains {file_count} files\nTarget: {display_moor_home()}")
        if prefix:
            print(f"Detected archive prefix: {prefix!r} (will be stripped)")
        if not args.force and not _confirm_import_overwrite(moor_root):
            return
        ...
        restored, restored_external, errors, skipped_runtime, db_shrunk = _import_members(
            zf, members, prefix, moor_root, file_count)
        elapsed = time.monotonic() - t0
...
def _restore_profile_wrappers(moor_root: Path) -> List[str]:
    ...
    except ImportError:  # moor_cli.profiles unavailable (fresh install)
        if any(profiles_dir.iterdir()):
            print("\n  Profiles detected but aliases could not be created.\n"
                  "  Run: moor profile list  (after installing moor)")
    return [n for n, _ in restored_profiles]


def _revive_gateway_after_import(moor_root: Path) -> None:
    ...
    if moor_root != native_default and any(
        ...
        print("\nRestored into a non-default home; leaving the gateway service alone to avoid clashing "
              ...
              "To start a gateway for this home, run:  moor gateway install")
        return
    try:
...
# Critical state files (relative to MOOR_HOME) for quick snapshots; everything else is
...
# outside state.db, so it is listed explicitly — ``moor update`` snapshots this set (#15733).
_QUICK_STATE_FILES = (
    ...
    # schema-init re-creates an empty one (#52889). Skipped when outside MOOR_HOME.
...
def _quick_snapshot_root(moor_home: Optional[Path] = None) -> Path:
    home = moor_home or get_moor_home()
    return home / _QUICK_SNAPSHOTS_DIR


def create_quick_snapshot(
    label: Optional[str] = None, moor_home: Optional[Path] = None, keep: Optional[int] = None,
...
def _create_quick_snapshot_locked(
    ...
    multi-GB ``state.db`` never stalls ``moor update`` while the small files are always captured.
    """
    ...
    if failed_dbs:
        ...
        print(f"  ⚠ CRITICAL: could not snapshot DB file(s): {', '.join(failed_dbs)}\n"
              f"  ⚠ If sessions disappear after update, check {root} and run: moor snapshot list")
...
def list_quick_snapshots(limit: int = 20, moor_home: Optional[Path] = None) -> List[Dict[str, Any]]:
    """List existing quick state snapshots, most recent first."""
    results = []
    for d in _snapshot_dirs(_quick_snapshot_root(moor_home)):
...
def restore_quick_snapshot(snapshot_id: str, moor_home: Optional[Path] = None) -> bool:
...
def restore_cron_jobs_if_emptied(snapshot_id: str, moor_home: Optional[Path] = None) -> Optional[Dict[str, Any]]:
    ...
    for name, profile_home in _sibling_profile_homes(home):
        try:
            snap_id = create_quick_snapshot(
                label="pre-update", moor_home=profile_home, keep=keep, max_file_size=max_file_size)
            if snap_id:
...
def restore_config_model_settings_if_rewritten(
    snapshot_id: str, moor_home: Optional[Path] = None) -> Optional[Dict[str, Any]]:
...
def _restore_all_sibling_profiles(
    ...
    """Run ``restore_fn(snap_id, moor_home=...)`` for every sibling against ITS OWN
    ...
    for name, snap_id in profile_snapshots.items():
        ...
        if profile_home is None:
            continue
        try:
            result = restore_fn(snap_id, moor_home=profile_home)
        except Exception as exc:
...
def prune_quick_snapshots(keep: int = _QUICK_DEFAULT_KEEP, moor_home: Optional[Path] = None) -> int:
    """Remove oldest quick snapshots beyond the keep limit. Returns count deleted."""
    return _prune_oldest(_snapshot_dirs(_quick_snapshot_root(moor_home)), keep, shutil.rmtree, "snapshot")
...
def run_quick_backup(args) -> None:
    ...
    if snap_id:
        print(f"State snapshot created: {snap_id}\n"
              f"  {len(list_quick_snapshots())} snapshot(s) stored in {display_moor_home()}/state-snapshots/\n"
...
def _write_full_zip_backup(out_path: Path, moor_root: Path) -> Optional[Path]:
    """Full zip snapshot of ``moor_root`` to ``out_path`` under the backup slot (same rules as
...
def _create_prefixed_full_backup(
    moor_home: Optional[Path], prefix: str, keep: int, what: str, prune_what: str) -> Optional[Path]:
    """Write ``<MOOR_HOME>/backups/<prefix><timestamp>.zip`` and prune older same-prefix zips.
    Returns the path, or ``None`` if nothing to back up or the write failed. Never raises."""
    moor_root = moor_home or get_default_moor_root()
    if not moor_root.is_dir():
        return None
    backup_dir = moor_root / _PRE_UPDATE_BACKUPS_DIR
    try:
    ...
    if _write_full_zip_backup(out_path, moor_root) is None:
        return None
...
def create_pre_update_backup(
    moor_home: Optional[Path] = None, keep: int = _PRE_UPDATE_DEFAULT_KEEP) -> Optional[Path]:
    """Full zip backup to ``backups/pre-update-<timestamp>.zip``, auto-pruned; ``None`` if nothing
    was found or the backup failed. Never raises — ``moor update`` continues anyway."""
    return _create_prefixed_full_backup(moor_home, _PRE_UPDATE_PREFIX, max(keep, 1), "pre-update", "backup")


def create_pre_migration_backup(
    moor_home: Optional[Path] = None, keep: int = _PRE_MIGRATION_DEFAULT_KEEP) -> Optional[Path]:
    """Full zip backup to ``backups/pre-migration-<timestamp>.zip`` before ``moor claw migrate``
    (same dir as update backups so listings/``moor import`` find it); ``None`` if nothing was
    found or the write failed. Never raises."""
    return _create_prefixed_full_backup(
        moor_home, _PRE_MIGRATION_PREFIX, max(keep, 0), "pre-migration", "pre-migration backup")
