"""
Status command for moor CLI.

Shows the status of all Moor Agent components.
"""
...
def redact_key(key: str) -> str:
    ...
    "(not set)" placeholder in dim color to match ``moor config``'s
...
def _estop_status_line():
    """One-line pause banner for `moor status`, or None when not paused.

    Cheap: a single stat on $MOOR_HOME/ESTOP via agent.estop.
    """
    try:
    ...
    return f"⏸️  PAUSED (global emergency stop{suffix}; `moor resume` to lift)"


def show_status(args):
    """Show status of all Moor Agent components."""
    deep = getattr(args, 'deep', False)

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.CYAN))
    print(color("│                 ⚕ Moor Agent Status                  │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    try:
        ...
        # Read-only display: use the refresh-free snapshot so `moor status`
    ...
    moor_account_info = None
    if (
        moor_status.get("logged_in")
        or moor_status.get("access_token")
        or moor_status.get("portal_base_url")
        or moor_status.get("inference_credential_present")
        or moor_status.get("error_code")
    ):
        try:
            moor_account_info = get_moor_portal_account_info()
        except Exception:
            moor_account_info = None

    moor_logged_in = bool(
        moor_status.get("logged_in")
        or (moor_account_info and moor_account_info.logged_in)
    )
    moor_inference_present = bool(
        moor_status.get("inference_credential_present")
        or (moor_account_info and moor_account_info.inference_credential_present)
    )
    moor_error = moor_status.get("error")
    if moor_logged_in:
        moor_label = "logged in"
    elif moor_inference_present:
        moor_label = "not logged in (Moor inference key configured)"
    else:
        moor_label = "not logged in (run: moor portal)"
    print(
        f"  {'Moor Portal':<12}  {check_mark(moor_logged_in)} "
        f"{moor_label}"
    )
    portal_url = moor_status.get("portal_base_url") or "(unknown)"
    inference_url = (
        moor_status.get("inference_base_url")
        or (moor_account_info.inference_base_url if moor_account_info else None)
    )
    access_exp = _format_iso_timestamp(moor_status.get("access_expires_at"))
    key_exp = _format_iso_timestamp(moor_status.get("agent_key_expires_at"))
    refresh_label = "yes" if moor_status.get("has_refresh_token") else "no"
    if moor_logged_in or portal_url != "(unknown)" or moor_error:
        print(f"    Portal URL: {portal_url}")
    if moor_inference_present and inference_url:
        print(f"    Inference:  {inference_url}")
    if moor_logged_in or moor_status.get("access_expires_at"):
        print(f"    Access exp: {access_exp}")
    if moor_logged_in or moor_inference_present or moor_status.get("agent_key_expires_at"):
        print(f"    Key exp:    {key_exp}")
    if moor_logged_in or moor_status.get("has_refresh_token"):
        print(f"    Refresh:    {refresh_label}")
    if moor_error:
        print(f"    Error:      {moor_error}")

    codex_logged_in = bool(codex_status.get("logged_in"))
    print(
        f"  {'OpenAI Codex':<12}  {check_mark(codex_logged_in)} "
        f"{'logged in' if codex_logged_in else 'not logged in (run: moor model)'}"
    )
    ...
    print(
        ...
        f"{'logged in' if minimax_logged_in else 'not logged in (run: moor auth add minimax-oauth)'}"
    )
    ...
    # disrupt the already-printed Moor/Codex/Qwen/MiniMax rows above.
    try:
    ...
    print(
        ...
        f"{'logged in' if xai_oauth_logged_in else 'not logged in (run: moor auth add xai-oauth)'}"
    )
    ...
    # =========================================================================
    # Moor Subscription Features
    # =========================================================================
    if managed_moor_tools_enabled():
        features = get_moor_subscription_features(config)
        print()
        print(color("◆ Moor Tool Gateway", Colors.CYAN, Colors.BOLD))
        if not features.moor_auth_present:
            print("  Moor Portal   ✗ not logged in")
        else:
            print("  Moor Portal   ✓ managed tools available")
        for feature in features.items():
            if feature.managed_by_moor:
                state = "active via Moor subscription"
            elif feature.active:
                current = feature.current_provider or "configured provider"
                state = f"active via {current}"
            elif feature.included_by_default and features.moor_auth_present:
                state = "included by subscription, not currently selected"
            elif feature.key == "modal" and features.moor_auth_present:
                state = "available via subscription (optional)"
            else:
                state = "not configured"
            print(f"  {feature.label:<15} {check_mark(feature.available or feature.active or feature.managed_by_moor)} {state}")
    elif moor_logged_in or moor_inference_present:
        # Moor OAuth without entitlement, or an opaque inference key without
    ...
    for pname, env_vars in apikey_providers.items():
        ...
        label = "configured" if configured else "not configured (run: moor model)"
    ...
    if _effective_provider_label() == "LM Studio":
        from moor_cli.models import probe_lmstudio_models
        model_cfg = config.get("model")
    ...
    if terminal_env == "ssh":
        ...
        sdk_label = "installed" if sdk_ok else "missing (install: pip install 'moor-agent[vercel]')"
        ...
        # provider's doctor rows (fail-soft — never break `moor status`).
        try:
    ...
    try:
        from moor_cli.gateway import get_gateway_runtime_snapshot, _format_gateway_pids
    ...
    jobs_file = get_moor_home() / "cron" / "jobs.json"
    if jobs_file.exists():
        import json
        try:
    ...
    if _session_count is not None and _session_count > 0:
        ...
        sessions_file = get_moor_home() / "sessions" / "sessions.json"
    ...
    print(color("  Run 'moor doctor' for detailed diagnostics", Colors.DIM))
    print(color("  Run 'moor setup' to configure", Colors.DIM))
    print()
