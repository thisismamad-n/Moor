...
def _estop_status_line():
    ...
    return f"⏸️  PAUSED (global emergency stop{f' — reason: {reason}' if reason else ''}; `moor resume` to lift)"
...
def _render_terminal(ctx):
    ...
    if terminal_env in _TERMINAL_ENV_ROWS:
        ...
        _kv_flag("SDK:", importlib.util.find_spec("vercel") is not None, "installed",
                 "missing (install: pip install 'moor-agent[vercel]')")
        ...
        # (fail-soft — never break `moor status`).
        try:
...
def _render_gateway(ctx):
    ...
    try:
        ...
        from moor_cli.gateway_multiplex_served import multiplexer_served_secondaries
        ...
        if not snapshot.running and named_profile_served_by_running_multiplexer():
            ...
            _kv("Manage with:", "moor gateway status   # from the default profile")
            return
        ...
        if snapshot.running and (served := multiplexer_served_secondaries()):
            ...
            from moor_cli.gateway_multiplex_served import served_profile_ingress_urls
...
def _render_sessions(ctx):
    ...
    elif not (sessions_file := get_moor_home() / "sessions" / "sessions.json").exists():
        _kv("Active:", 0)
    else:
        try:
...
def _render_footer(ctx):
    _banner(("─" * 60, "  Run 'moor doctor' for detailed diagnostics", "  Run 'moor setup' to configure"),
            Colors.DIM)
    print()


# Print order of `moor status`; each renderer takes the shared _StatusContext.
_SECTIONS = (
    _render_header, _render_environment, _render_api_keys, _render_auth_providers, _render_moor_gateway,
...
def show_status(args):
    ...
    # Shared by section renderers: config, --deep, and the Moor login facts Auth Providers derives
    # for the later Moor Tool Gateway section.
    ctx = SimpleNamespace(deep=getattr(args, 'deep', False), config={}, moor_logged_in=False,
                          moor_inference_present=False, moor_account_info=None)
...
_PLUGIN_COMPAT_LAZY = {
    ...
    'get_moor_portal_account_info': ('moor_cli.moor_account', 'get_moor_portal_account_info'),
