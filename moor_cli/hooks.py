"""moor hooks — inspect and manage shell-script hooks.

Usage::

    moor hooks list
    moor hooks test <event> [--for-tool X] [--payload-file F]
    moor hooks revoke <command>
    moor hooks doctor

Consent records live under ``~/.moor/shell-hooks-allowlist.json`` and
hook definitions come from the ``hooks:`` block in ``~/.moor/config.yaml``
...
def hooks_command(args) -> None:
    """Entry point for ``moor hooks`` — dispatches to the requested action."""
    sub = getattr(args, "hooks_action", None)

    if not sub:
        print("Usage: moor hooks {list|test|revoke|doctor}")
        print("Run 'moor hooks --help' for details.")
        return
...
def _cmd_list(_args) -> None:
    ...
    if not specs and not outbound:
        print("No shell hooks or outbound webhooks configured in ~/.moor/config.yaml.")
        print("See `moor hooks --help` or")
        print("    website/docs/user-guide/features/hooks.md")
        print("for the config schema and worked examples.")
        return

    if not specs:
        print("No shell hooks configured in ~/.moor/config.yaml.")
    else:
                    ...
                    if entry and entry.get("approved_at"):
                        ...
                        if mtime_now and mtime_at and mtime_now > mtime_at:
                            print(
                                ...
                                f"run `moor hooks doctor` to re-validate"
                            )
            print()
...
# stdin a script sees under `moor hooks test` and `moor hooks doctor`
...
def _doctor_one(spec, shell_hooks) -> int:
    ...
    if entry and entry.get("script_mtime_at_approval"):
        ...
        if mtime_now and mtime_at and mtime_now > mtime_at:
            ...
            print(f"      ⚠ script modified since approval "
                  ...
                  f"then `moor hooks revoke` + re-approve to refresh")
    ...
    # is already allowlisted.  Otherwise `moor hooks doctor` would execute
