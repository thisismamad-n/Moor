...
from moor_cli._subprocess_compat import windows_detach_flags
...
def _probe_gateway_health() -> tuple[bool, dict | None]:
    ...
    """
    from moor_cli.web_server import _GATEWAY_HEALTH_TIMEOUT, _GATEWAY_HEALTH_URL
...
# ~/.moor/logs/<action>.log which the dashboard tails.
...
def _dashboard_spawn_executable() -> str:
    ...
    """
    from moor_cli.web_server import PROJECT_ROOT
...
def _profile_action_environment(
    ...
    """Environment for a detached ``moor <subcommand>`` action.

    The dashboard loads its own profile's ``.env`` into process-global ``os.environ``. Copying
    that mapping verbatim into ``moor -p <other> ...`` lets the named child see the dashboard
    profile's platform credentials and ports *before* its own dotenv loads (``load_moor_dotenv``
    ...
    Named-profile actions therefore start from Moor' standard scrubbed subprocess env, then drop
    the profile-managed keys plus every key declared by the dashboard/default profile dotenv files
    and their hydrated secret sources, and pin ``MOOR_HOME`` to the target profile. The child's
    ...
    if profile is None:
        ...
        try:
            source_homes = {str(get_default_moor_root()), str(get_moor_home())}
        except Exception:
            source_homes = set()
        ...
        action_env["MOOR_HOME"] = str(target_home)
        apply_subprocess_home_env(action_env)

    action_env["MOOR_NONINTERACTIVE"] = "1"
    ...
    # The dashboard runs inside the gateway process, so os.environ carries _MOOR_GATEWAY=1;
...
def _action_targets_system_gateway(subcommand: List[str]) -> bool:
    ...
    ``moor update`` fleet restart (``update_cmd_fleet._needs_sudo`` / ``_sudo_noninteractive_ok``).
    """
    from moor_cli.update_cmd_fleet import _needs_sudo
    ...
    from moor_cli.gateway import _select_systemd_scope
    from moor_constants import reset_moor_home_override, set_moor_home_override

    profile = _named_profile_from_action(subcommand)
    if profile is None:
        return _select_systemd_scope(False)
    # Unit names are derived from MOOR_HOME, so a selector-bearing action must be resolved
    ...
    from moor_cli.web_server_profiles import _resolve_profile_dir
    token = set_moor_home_override(_resolve_profile_dir(profile))
    try:
        return _select_systemd_scope(False)
    finally:
...
def _spawn_moor_action(
    ...
    """Spawn ``moor <subcommand>`` detached (via ``moor_cli.main``) and record the handle."""
    from moor_cli.web_server import PROJECT_ROOT
    ...
    if _action_targets_system_gateway(subcommand):
        ...
        # MOOR_HOME past sudo's env_reset and reads SUDO_USER for the service identity.
        ...
        # two-step gate as the ``moor update`` fleet restart: a refused blanket probe falls
        # back to ``sudo -l`` on the exact argv, so a command-scoped NOPASSWD entry qualifies.
        from moor_cli.update_cmd_fleet import _sudo_noninteractive_ok
        ...
        if not _sudo_noninteractive_ok(["-l", "--", *cmd]):
            message = (
                ...
                f"'sudo moor {' '.join(subcommand)}' on the host, or grant that user NOPASSWD sudo."
            )
    ...
    # dashboard profile's credentials; see _profile_action_environment (also drops _MOOR_GATEWAY).
...
def _own_profile_selector(profile: Optional[str]) -> Optional[str]:
    ...
    profile (a pooled Desktop ``moor --profile X serve`` answers ``/api/gateway/*`` without
    ...
    own = profile_name_for_home(get_process_moor_home())
    return own if own and own != "default" else None


def _gateway_subcommand(profile: Optional[str], verb: str) -> List[str]:
    """``moor [-p X] gateway <verb>`` argv for a dashboard lifecycle action. A profile served by the
    ...
    pooled ``--profile X serve`` would inherit X's ``MOOR_HOME`` and hit the same exit-78 refusal."""
    from moor_cli.web_server_profiles import _profile_cli_args
    ...
    if profile and verb == "restart" and multiplexed_profile_refusal(profile, verb) is not None:
        from moor_constants import get_process_moor_home, profile_name_for_home
        args = [] if profile_name_for_home(get_process_moor_home()) == "default" else ["-p", "default"]
...
def multiplexed_profile_refusal(profile: Optional[str], verb: str) -> Optional[str]:
    ...
    The spawned ``moor -p X gateway <verb>`` would only print exit-78 / "no gateway running for this
...
def _restart_gateway_after(profile: Optional[str], *, what: str, label: str) -> dict[str, Any]:
    ...
    from moor_cli.web_server import _spawn_gateway_restart
    try:
