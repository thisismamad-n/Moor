"""Moor Portal OAuth: device-code login, refresh, shared-store mirroring, JWT selection, status.

Split out of ``moor_cli/auth.py``; origin helpers are imported lazily inside each function
so ``moor_cli.auth.<name>`` patches still intercept (and no import cycle).
"""
...
from moor_cli.auth_constants import (
    ...
    DEFAULT_MOOR_INFERENCE_URL, DEFAULT_MOOR_PORTAL_URL, DEFAULT_MOOR_SCOPE,
...
_UNUSABLE_JWT_RELOGIN = "Re-authenticate with: moor auth add moor"
...
def _oauth_trace(event: str, *, sequence_id: Optional[str] = None, **fields: Any) -> None:
    if os.getenv("MOOR_OAUTH_TRACE", "").strip().lower() not in {"1", "true", "yes", "on"}:
        return
...
# Moor agent-key slots; a fresh login persists them as None, quarantine strips them.
_MOOR_EMPTY_AGENT_KEY_FIELDS: Dict[str, Any] = {
...
_MOOR_STALE_PORTAL_HOSTS: FrozenSet[str] = frozenset({"api.nousresearch.com"})
...
def _portal_entitlement_message(capability: str) -> str:
    ...
    return format_moor_portal_entitlement_message(account_info, capability=capability) or ""


def _format_moor_entitlement_auth_error(error: AuthError) -> str:
    with suppress(Exception):
        if message := _portal_entitlement_message("Moor model access"):
            return message
...
def _migrate_stale_moor_portal_url(providers: Dict[str, Any]) -> None:
    ...
    if stored and urlparse(stored).hostname in _MOOR_STALE_PORTAL_HOSTS:
        logger.warning(
            "auth: migrating stale moor portal_base_url %s -> %s", stored, DEFAULT_MOOR_PORTAL_URL)
        moor["portal_base_url"] = DEFAULT_MOOR_PORTAL_URL


# Allowlist of hosts the Moor Portal proxy will forward inference JWTs to — a bearer sent anywhere
...
_ALLOWED_MOOR_INFERENCE_HOSTS: FrozenSet[str] = frozenset({"inference-api.nousresearch.com"})
...
def _moor_invoke_jwt_status(
    ...
    min_ttl_seconds: int = MOOR_INVOKE_JWT_MIN_TTL_SECONDS) -> Optional[str]:
    """Return None when the token can be used for inference, else a reason."""
    from moor_cli.auth import _is_expiring
...
def _moor_invoke_jwt_is_usable(
    token: Any, *, scope: Any = None, expires_at: Any = None,
    min_ttl_seconds: int = MOOR_INVOKE_JWT_MIN_TTL_SECONDS) -> bool:
    from moor_cli.auth import _moor_invoke_jwt_status
    return _moor_invoke_jwt_status(
...
def _state_invoke_jwt_status(state: Dict[str, Any], token: Any) -> Optional[str]:
    """``_moor_invoke_jwt_status`` for *token* using *state*'s scope / expires_at (patchable)."""
    from moor_cli.auth import _moor_invoke_jwt_status
    return _moor_invoke_jwt_status(
        token, scope=state.get("scope"), expires_at=state.get("expires_at"))


def _assert_moor_inference_jwt_usable(state: Dict[str, Any], *, access_token: Any = None) -> None:
...
def _remaining_ttl(expires_at: Any, fallback_expires_in: Any) -> int:
    ...
    from moor_cli.auth import _coerce_ttl_seconds, _parse_iso_timestamp
...
def _set_moor_agent_key_from_invoke_jwt(
    ...
    from moor_cli.auth import _nonempty_str
...
def _select_moor_invoke_jwt(
    ...
    _oauth_trace(
        "moor_invoke_jwt_selected", sequence_id=sequence_id,
...
_MOOR_EFFECTIVE_STATE_IGNORED_KEYS = frozenset({"expires_in", "agent_key_expires_in"})


def _moor_effective_provider_state(state: Dict[str, Any]) -> Dict[str, Any]:
    return {k: v for k, v in state.items() if k not in _MOOR_EFFECTIVE_STATE_IGNORED_KEYS}
...
def _moor_shared_auth_dir() -> Path:
    """Directory of the shared Moor token store: ``MOOR_SHARED_AUTH_DIR`` or ``<root>/shared/``.

    Outside any named profile so all profiles share it (``moor --profile X auth add moor --type
...
def _moor_shared_store_path() -> Path:
    ...
    # user's Moor root so a test that forgot MOOR_SHARED_AUTH_DIR fails loudly instead of
    # corrupting cross-profile state.
    if os.environ.get("PYTEST_CURRENT_TEST"):
        from moor_constants import get_default_moor_root
        real_home_shared = (
            get_default_moor_root() / "shared" / MOOR_SHARED_STORE_FILENAME).resolve(strict=False)
        try:
...
def _moor_shared_store_lock(timeout_seconds: float = AUTH_LOCK_TIMEOUT_SECONDS):
    ...
    try:
        ...
        yield  # No MOOR_HOME yet (pre-setup): fall through without locking.
        return
    with _file_lock(
        lock_path, _moor_shared_lock_holder, timeout_seconds,
...
# OAuth fields mirrored between a profile's Moor state and the shared cross-profile store.
_MOOR_SHARED_STATE_KEYS = (
...
def _merge_shared_moor_oauth_state(state: Dict[str, Any]) -> bool:
    ...
    from moor_cli.auth import _nonempty_str, _parse_iso_timestamp, _read_shared_moor_state
    shared = _read_shared_moor_state() or {}
    ...
    if not refresh_changed and not shared_access_exp > local_access_exp:
        return False
    for key in _MOOR_SHARED_STATE_KEYS:
...
def _moor_shared_shape(src: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        ...
        "inference_base_url": src.get("inference_base_url") or DEFAULT_MOOR_INFERENCE_URL,
...
def _write_shared_moor_state(state: Dict[str, Any]) -> None:
    ...
    """
    from moor_cli.auth import _nonempty_str, _write_private_file_atomic
...
def _read_shared_moor_state() -> Optional[Dict[str, Any]]:
    """Shared Moor OAuth state when present and well-formed, else None.
...
def _clear_shared_moor_state(reason: str) -> None:
    ...
    try:
        with _moor_shared_store_lock():
            _moor_shared_store_path().unlink(missing_ok=True)
...
def _quarantine_forensics(state: Dict[str, Any], error: AuthError, reason: str) -> Dict[str, Any]:
    ...
    provenance is client_id + agent_key_id (Moor state has no session_id).
    """
...
def _quarantine_moor_oauth_state(state: Dict[str, Any], error: AuthError, *, reason: str) -> None:
    ...
    from moor_cli.auth import (
        _FLAT_OAUTH_TOKEN_KEYS, _last_auth_error_marker, invalidate_moor_auth_status_cache)
    ...
    for key in (*_FLAT_OAUTH_TOKEN_KEYS, *_MOOR_EMPTY_AGENT_KEY_FIELDS):
        state.pop(key, None)
    state["last_auth_error"] = _last_auth_error_marker("moor", error, reason=reason)
...
def _quarantine_moor_pool_entries(
    ...
    entries = _pool_entries(auth_store, "moor")
    if entries is None:
        return False
    ...
    if removed:
        auth_store["credential_pool"]["moor"] = retained
        _oauth_trace("moor_pool_device_code_quarantined", reason=reason, error_code=error.code)
    return removed


def _try_import_shared_moor_state(*, timeout_seconds: float = 15.0) -> Optional[Dict[str, Any]]:
    """Rehydrate Moor OAuth state from the shared store via a forced refresh.

    Returns auth_state ready for ``persist_moor_credentials()``; None on any failure (expired
    token, portal unreachable) so the caller falls through to device-code.
    """
    from moor_cli.auth import (
        _read_shared_moor_state, _write_shared_moor_state, refresh_moor_oauth_from_state,
        _is_terminal_moor_refresh_error)
    try:
        with _moor_shared_store_lock(timeout_seconds=_shared_lock_timeout(timeout_seconds)):
            shared = _read_shared_moor_state()
            if not shared:
                return None
            # Full state dict so refresh_moor_oauth_from_state has every field it needs.
            state: Dict[str, Any] = {
                **_moor_shared_shape(shared), "agent_key": None, "agent_key_expires_at": None,
                "tls": {"insecure": False, "ca_bundle": None}}
            refreshed = refresh_moor_oauth_from_state(
                state, timeout_seconds=timeout_seconds, force_refresh=True,
                on_state_update=lambda updated, _reason: _write_shared_moor_state(updated))
            _write_shared_moor_state(refreshed)
    except Exception as exc:
        is_auth = isinstance(exc, AuthError)
        _oauth_trace(
            "moor_shared_import_failed", error_type=type(exc).__name__,
            **({"error_code": getattr(exc, "code", None)} if is_auth else {}))
        if is_auth and _is_terminal_moor_refresh_error(exc):
...
def _refresh_access_token(
    *, client: httpx.Client, portal_base_url: str, client_id: str, refresh_token: str,
    ...
    if code == "refresh_token_reused" or "reuse" in description.lower():
        ...
        relogin = True
    raise _moor_err(description, code, relogin=relogin)


def _refresh_moor_or_quarantine(
    *, client: httpx.Client, auth_store: Dict[str, Any], state: Dict[str, Any],
    ...
    from moor_cli.auth import _refresh_access_token, _is_terminal_moor_refresh_error
    try:
        ...
        if _is_terminal_moor_refresh_error(exc):
            _quarantine_moor_oauth_state(state, exc, reason=reason)
            _quarantine_moor_pool_entries(auth_store, exc, reason=reason)
            persist()
        raise


def _apply_moor_refreshed_tokens(
    state: Dict[str, Any], refreshed: Dict[str, Any], refresh_token: str, *,
    inference_base_url: Optional[str] = None) -> None:
    """Write a successful Moor token-refresh payload into *state* (tokens + expiry fields).
    ...
    *inference_base_url*, when given, is the healed network-provenance URL to persist alongside
    ...
    """
    from moor_cli.auth import _coerce_ttl_seconds
...
def _healed_moor_inference_url(refreshed: Dict[str, Any]) -> str:
    ...
    """
    url = _validate_moor_inference_url_from_network(refreshed.get("inference_base_url"))
    return url or DEFAULT_MOOR_INFERENCE_URL


def _moor_http_client(timeout_seconds: float, verify: Any) -> httpx.Client:
    return httpx.Client(
...
def fetch_moor_models(
    *, inference_base_url: str, api_key: str, timeout_seconds: float = 15.0,
    ...
    if response.status_code != 200:
        ...
        raise _moor_err(description, "models_fetch_failed")
    ...
    for item in data:
        ...
        # Moor models aren't reliable for agentic tool-calling
        if _nonempty_str(model_id) and "moor" not in model_id.lower():
...
def _agent_key_is_usable(state: Dict[str, Any], min_ttl_seconds: int) -> bool:
    ...
    return _nonempty_str(key) and _moor_invoke_jwt_is_usable(
...
def refresh_moor_oauth_pure(
    ...
    inference_base_url: str, *, token_type: str = "Bearer", scope: str = DEFAULT_MOOR_SCOPE,
...
def refresh_moor_oauth_from_state(
    ...
    on_state_update: Optional[Callable[[Dict[str, Any], str], None]] = None) -> Dict[str, Any]:
    """Refresh Moor OAuth from a state dict (defaults filled in) without mutating auth.json."""
    from moor_cli.auth import (
        _assert_moor_inference_jwt_usable, _refresh_access_token, _resolve_verify,
    ...
    state: Dict[str, Any] = {
        "access_token": src.get("access_token", ""), "refresh_token": src.get("refresh_token", ""),
        "client_id": src.get("client_id") or DEFAULT_MOOR_CLIENT_ID,
        "portal_base_url": (src.get("portal_base_url") or DEFAULT_MOOR_PORTAL_URL).rstrip("/"),
        "inference_base_url": (
            src.get("inference_base_url") or DEFAULT_MOOR_INFERENCE_URL).rstrip("/"),
        "token_type": src.get("token_type") or "Bearer",
        "scope": src.get("scope") or DEFAULT_MOOR_SCOPE,
        "obtained_at": src.get("obtained_at"), "expires_at": src.get("expires_at"),
    ...
    with _moor_http_client(timeout_seconds or 15.0, verify) as client:
        ...
        if force_refresh or current_invoke_jwt_status is not None:
            ...
            _apply_moor_refreshed_tokens(
                state, refreshed, refresh_token_value,
                inference_base_url=_healed_moor_inference_url(refreshed))
...
def persist_moor_credentials(creds: Dict[str, Any], *, label: Optional[str] = None):
    """Persist Moor OAuth credentials as the singleton provider state.

    Moor credentials are read from ``providers.moor`` (401 recovery, pool seeding) AND
    ``credential_pool.moor`` (runtime ``pool.select()``); a pool-only write broke expiry recovery.
    So: write the singleton, mirror to the shared store, then ``load_pool("moor")`` upserts the
    canonical ``device_code`` entry in place. ``label`` rides in the singleton so re-seeding keeps
    it.
    """
    from moor_cli.auth import _save_active_provider_state, _write_shared_moor_state
    from agent.credential_pool import load_pool
    state = dict(creds)
    if label and str(label).strip():
        state["label"] = str(label).strip()
    _save_active_provider_state("moor", state)
    _write_shared_moor_state(state)
    pool = load_pool("moor")
    return next((e for e in pool.entries() if e.source == MOOR_DEVICE_CODE_SOURCE), None)
...
def _moor_effective_routing(state: Dict[str, Any]) -> tuple[str, str, str, str]:
    ...
    portal_url = (
        _optional_base_url(state.get("portal_base_url")) or os.getenv("MOOR_PORTAL_BASE_URL")
        or os.getenv("MOOR_PORTAL_BASE_URL") or DEFAULT_MOOR_PORTAL_URL).rstrip("/")
    ...
    if env_portal_override:
        ...
        if not portal_host or portal_host not in _MOOR_PORTAL_ALLOWED_HOSTS or not trusted_scheme:
            logger.warning(
...
class _MoorRuntimeResolve:
    """Working set for one ``resolve_moor_runtime_credentials`` call.
    ...
    def _reload_routing(self) -> None:
        (self.portal_base_url, self.stored_inference_base_url, self.inference_base_url,
         self.client_id) = _moor_effective_routing(self.state)

    def persist(self, reason: str) -> None:
        from moor_cli.auth import _save_provider_state_to_source, _write_shared_moor_state
        state = self.state
        persisted = _moor_effective_provider_state(self._persisted_state)
        if _moor_effective_provider_state(state) == persisted:
            _oauth_trace("moor_state_persist_skipped", sequence_id=self.sequence_id, reason=reason)
            return
        try:
            _save_provider_state_to_source(self.auth_store, "moor", state, self._source_path)
        except Exception as exc:
            _oauth_trace(
                "moor_state_persist_failed", sequence_id=self.sequence_id, reason=reason,
                error_type=type(exc).__name__)
            raise
        _oauth_trace(
            "moor_state_persisted", sequence_id=self.sequence_id, reason=reason,
            refresh_token_fp=_token_fingerprint(state.get("refresh_token")),
            access_token_fp=_token_fingerprint(state.get("access_token")))
        self._persisted_state = dict(state)
    ...
    def shared_lock(self):
        return _moor_shared_store_lock(timeout_seconds=_shared_lock_timeout(self.timeout_seconds))
    ...
    def merge_shared(self) -> bool:
        ...
        if not _merge_shared_moor_oauth_state(self.state):
            return False
    ...
    def refresh(self, client: httpx.Client, invoke_jwt_status: Optional[str]) -> None:
        ...
        refreshed = _refresh_moor_or_quarantine(
        ...
        # MOOR_INFERENCE_BASE_URL env override is layered on for the client/return value only.
        self.stored_inference_base_url = _healed_moor_inference_url(refreshed)
...
def resolve_moor_runtime_credentials(
    *, timeout_seconds: float = 15.0, insecure: Optional[bool] = None,
    ...
    """
    from moor_cli.auth import (
        _assert_moor_inference_jwt_usable, _auth_file_path, _provider_state_transaction,
        _resolve_verify, _select_moor_invoke_jwt, _sync_moor_pool_from_auth_store,
    ...
    with _provider_state_transaction("moor") as (auth_store, state, state_source_path):
        ...
        verify = _resolve_verify(insecure=insecure, ca_bundle=ca_bundle, auth_state=state)
        _oauth_trace(
            "moor_runtime_credentials_start", sequence_id=run.sequence_id,
            refresh_token_fp=_token_fingerprint(state.get("refresh_token")))
        with _moor_http_client(timeout_seconds or 15.0, verify) as client:
            run.ensure_usable_access_token(client)
            _assert_moor_inference_jwt_usable(state, access_token=run.access_token)
            _select_moor_invoke_jwt(
        ...
        run.persist("resolve_moor_runtime_credentials_final")
    if run.persisted_any:
        _sync_moor_pool_from_auth_store()
    api_key = state.get("agent_key")
    if not isinstance(api_key, str) or not api_key:
        raise _moor_err("Failed to resolve a Moor inference API key", "server_error")
    expires_at = state.get("agent_key_expires_at")
    return {
        "provider": "moor", "base_url": run.inference_base_url, "api_key": api_key,
...
def _snapshot_moor_pool_status() -> Dict[str, Any]:
    ...
    ``resolve_moor_runtime_credentials()`` refreshes.
    """
    ...
    try:
        ...
        return {
            ...
            "portal_base_url": (
                (attr("portal_base_url") or DEFAULT_MOOR_PORTAL_URL) if is_portal_oauth else None),
            "inference_base_url": (
...
def _compute_moor_auth_status() -> Dict[str, Any]:
    ...
    from moor_cli.auth import get_provider_auth_state, resolve_moor_runtime_credentials
    state = get_provider_auth_state("moor")
    if not state:
        return _snapshot_moor_pool_status()
    base_status = _moor_status_from_state(
...
def get_moor_auth_status_local() -> Dict[str, Any]:
    ...
    NEVER calls ``resolve_moor_runtime_credentials()`` (no refresh POST / single-use token spent);
    ...
    status = _moor_status_from_state(state, logged_in=logged_in, source="auth_store_local")
...
# Enum values reported on the dashboard /api/status as ``moor_session_valid``. NAS's health sweep
...
def _moor_device_code_login(
    *, portal_base_url: Optional[str] = None, inference_base_url: Optional[str] = None,
    ...
    from moor_cli.auth import (
        ...
        _tls_state_from_verify, format_auth_error, refresh_moor_oauth_from_state)
    pconfig = PROVIDER_REGISTRY["moor"]
    portal_base_url = (
        portal_base_url or os.getenv("MOOR_PORTAL_BASE_URL") or os.getenv("MOOR_PORTAL_BASE_URL")
        or pconfig.portal_base_url).rstrip("/")
    requested_inference_url = (
        inference_base_url or os.getenv("MOOR_INFERENCE_BASE_URL")
    ...
    auth_state = {
        ...
        **_MOOR_EMPTY_AGENT_KEY_FIELDS}
    try:
        return refresh_moor_oauth_from_state(
            auth_state, timeout_seconds=timeout_seconds, force_refresh=False)
    except AuthError as exc:
        if exc.code == "subscription_required":
            portal_url = auth_state.get("portal_base_url", DEFAULT_MOOR_PORTAL_URL).rstrip("/")
            print()
...
def _mirror_moor_state_best_effort(auth_state: Dict[str, Any]) -> None:
    """Mirror to the shared store + reseed the pool, swallowing all errors (same as _login_moor)."""
    from moor_cli.auth import _sync_moor_pool_from_auth_store, _write_shared_moor_state
    with suppress(Exception):
...
def step_up_moor_billing_scope(
    *, open_browser: bool = True, timeout_seconds: float = 15.0,
    ...
    server silently downscopes and this returns False. Persists like ``_login_moor`` minus the
    model picker.
    """
    from moor_cli.auth import (
        PROVIDER_REGISTRY, _moor_device_code_login, _save_active_provider_state,
    ...
    _save_active_provider_state("moor", auth_state)
    _mirror_moor_state_best_effort(auth_state)
    granted = auth_state.get("scope")
    return isinstance(granted, str) and MOOR_BILLING_MANAGE_SCOPE in granted.split()


def _pick_moor_model_after_login(
    auth_state: Dict[str, Any], inference_base_url: str) -> Optional[str]:
    """Fetch the curated Moor model list (tier/policy-filtered) and run the interactive picker.
    ...
    if not isinstance(runtime_key, str) or not runtime_key:
        raise _moor_err("No runtime API key available to fetch models", "invalid_token")
    ...
    from moor_cli.models_pricing import (
        get_pricing_for_provider,
        moor_policy_allowed_ids,
        restrict_to_moor_policy,
    )
    ...
    if model_ids:
        ...
        _policy_allowed = moor_policy_allowed_ids()
        if free_tier:
            with suppress(Exception):
                unavailable_message = _portal_entitlement_message("paid Moor models")
        ...
        model_ids = restrict_to_moor_policy(model_ids, _policy_allowed, rescue_empty=True)
        _policy_narrowed = model_ids != _before_policy
        if free_tier:
            model_ids, unavailable_models = partition_moor_models_by_tier(
                model_ids, pricing, free_tier=True)
    if model_ids:
        from moor_cli.moor_account import moor_policy_notice
        _policy_notice = moor_policy_notice(removed=_policy_narrowed)
        if _policy_notice:
        ...
        return _prompt_model_selection(
            ...
            unavailable_message=unavailable_message, confirm_provider="moor",
...
def _offer_shared_moor_import(timeout_seconds: float) -> Optional[Dict[str, Any]]:
    """Codex-style auto-import: offer to rehydrate a Moor credential from another profile.
    ...
    try:
        ...
        shared_path = None
    print()
    print(f"Found existing Moor OAuth credentials at {shared_path}" if shared_path
          else "Found existing shared Moor OAuth credentials")
    if not _prompt_yes_no("Import these credentials? [Y/n]: ", default="y"):
        return None
    print("Rehydrating Moor session from shared credentials...")
    auth_state = _try_import_shared_moor_state(timeout_seconds=timeout_seconds)
...
def _restore_active_provider(prior_active_provider: Any) -> None:
    """Undo the ``active_provider="moor"`` that ``_save_provider_state`` wrote during login."""
...
def _login_moor(args, pconfig: ProviderConfig) -> None:
    ...
    from moor_cli.auth import (
        _auth_store_lock, _load_auth_store, _moor_device_code_login, _save_active_provider_state,
        _save_model_choice, _sync_moor_pool_from_auth_store, _update_config_for_provider,
        _write_shared_moor_state, format_auth_error)
    timeout_seconds = getattr(args, "timeout", None) or 15.0
    ca_bundle = (
        getattr(args, "ca_bundle", None) or os.getenv("MOOR_CA_BUNDLE")
        or os.getenv("SSL_CERT_FILE"))
    try:
        auth_state = _offer_shared_moor_import(timeout_seconds)
        if auth_state is None:
            auth_state = _moor_device_code_login(
        ...
        # Snapshot BEFORE _save_provider_state overwrites active_provider to "moor", so a
        ...
        saved_to = _save_active_provider_state("moor", auth_state)
        ...
        selected_model = None
        try:
            selected_model = _pick_moor_model_after_login(auth_state, inference_base_url)
        except Exception as exc:
        ...
        # switch to Moor with a mismatched model; the Moor tokens stay saved for future use.
