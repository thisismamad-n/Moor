...
# Maps a tools_config provider's ``managed_moor_feature`` to the tool-pool
# coverage category (moor_cli.moor_account.TOOL_COVERAGE_CATEGORIES). Lets the
# `moor tools` picker scope its entitlement gate to the selected backend, so a
# free-tool-pool user is allowed image gen but denied video gen at select time —
# consistent with the per-category feature gates in get_moor_subscription_features.
...
def _selected_provider(section: object, name_key: str = "provider") -> Optional[str]:
    ...
    an in-memory section dict: ``"moor"`` for the managed selection (stored
    ``moor`` value or legacy ``use_gateway: true``), a vendor name for BYOK
    picks, or ``None`` when no selection is stored. Keeping this in lockstep
    with the runtime resolver is what stops ``moor status`` from lying.
    """
    if not isinstance(section, dict):
        return None
...
class MoorSubscriptionFeatures:
    ...
    @property
    def web(self) -> MoorFeatureState:
        return self.features["web"]

    @property
    def image_gen(self) -> MoorFeatureState:
        return self.features["image_gen"]

    @property
    def tts(self) -> MoorFeatureState:
        return self.features["tts"]

    @property
    def stt(self) -> MoorFeatureState:
        return self.features["stt"]

    @property
    def browser(self) -> MoorFeatureState:
        return self.features["browser"]

    @property
    def video_gen(self) -> MoorFeatureState:
        return self.features["video_gen"]

    @property
    def modal(self) -> MoorFeatureState:
...
def _has_agent_browser() -> bool:
    ...
    try:
        ...
        # moor-managed Node dirs (Windows installer / POSIX $MOOR_HOME/node)
        ...
        managed_path = with_moor_node_path().get("PATH", "")
        if managed_path:
...
def _local_stt_backend_available() -> bool:
    ...
    ``apply_moor_managed_defaults`` from flipping a working local setup
...
def get_moor_subscription_features(
    ...
    *,
    ...
    provider_is_moor = str(model_cfg.get("provider") or "").strip().lower() == "moor"
    ...
    subscribed = provider_is_moor or moor_auth_present
    ...
    # requires `pip install faster-whisper`. For Moor subscribers we'd
    # rather route through the managed OpenAI audio gateway — see
    # apply_moor_managed_defaults below.
    ...
    # "moor" (stored value or legacy use_gateway: true) = managed gateway;
    ...
    web_use_gateway = web_selected == "moor"
    tts_use_gateway = tts_selected == "moor"
    stt_use_gateway = stt_selected == "moor"
    browser_use_gateway = browser_selected == "moor"
    image_use_gateway = image_selected == "moor"
    video_use_gateway = video_selected == "moor"

    # The "moor" selection is serviced by a concrete vendor implementation —
    # normalize the current-provider labels so downstream vendor checks hold.
    if web_backend == "moor" or web_use_gateway:
        web_backend = "firecrawl"
    if tts_provider == "moor" or tts_use_gateway:
        tts_provider = "openai"
    if stt_provider == "moor" or stt_use_gateway:
        stt_provider = "openai"
    if browser_provider == "moor" or browser_use_gateway:
    ...
    # Keyless Tavily is opt-in: selecting it in `moor tools` / setup writes
    ...
    try:
        ...
        local_stt_available = bool(_HAS_FASTER_WHISPER) or bool(
            get_env_value("MOOR_LOCAL_STT_COMMAND")
        )
    except Exception:
        local_stt_available = bool(get_env_value("MOOR_LOCAL_STT_COMMAND"))
    ...
    features = {
        "web": MoorFeatureState(
            key="web",
            label="Web tools",
            included_by_default=True,
            available=web_available,
            active=web_active,
            managed_by_moor=web_managed,
        ...
        "image_gen": MoorFeatureState(
            key="image_gen",
            label="Image generation",
            included_by_default=True,
            available=image_available,
            active=image_active,
            managed_by_moor=image_managed,
            direct_override=image_active and not image_managed,
            toolset_enabled=image_tool_enabled,
            current_provider="FAL" if (image_selected not in (None, "moor") or (image_selected is None and direct_fal)) else ("Moor Subscription" if (image_managed or image_use_gateway) else ""),
            explicit_configured=image_selected is not None or direct_fal,
        ),
        "video_gen": MoorFeatureState(
            key="video_gen",
            label="Video generation",
            included_by_default=False,
            available=video_available,
            active=video_active,
            managed_by_moor=video_managed,
            direct_override=video_active and not video_managed,
            toolset_enabled=video_tool_enabled,
            current_provider="FAL" if (video_selected not in (None, "moor") or (video_selected is None and direct_fal_video)) else ("Moor Subscription" if (video_managed or video_use_gateway) else ""),
            explicit_configured=video_selected is not None or direct_fal_video,
        ),
        "tts": MoorFeatureState(
            key="tts",
            label="OpenAI TTS",
            included_by_default=True,
            available=tts_available,
            active=tts_active,
            managed_by_moor=tts_managed,
        ...
        "stt": MoorFeatureState(
            key="stt",
            label="Speech-to-text",
            included_by_default=True,
            available=stt_available,
            active=stt_active,
            managed_by_moor=stt_managed,
        ...
        "browser": MoorFeatureState(
            key="browser",
            label="Browser automation",
            included_by_default=True,
            available=browser_available,
            active=browser_active,
            managed_by_moor=browser_managed,
        ...
        "modal": MoorFeatureState(
            key="modal",
            label="Modal execution",
            included_by_default=False,
            available=modal_available,
            active=modal_active,
            managed_by_moor=modal_managed,
    ...
    return MoorSubscriptionFeatures(
        ...
        moor_auth_present=moor_auth_present,
        provider_is_moor=provider_is_moor,
        features=features,
...
def apply_moor_managed_defaults(
    ...
    *,
    ...
    if not (
        ...
        return set()
    if not features.provider_is_moor:
        return set()
    ...
    if "browser" in selected_toolsets and not features.browser.explicit_configured and not (
        ...
        browser_cfg["cloud_provider"] = "moor"
...
# ``get_moor_subscription_features`` reads (web uses "backend", browser
...
def get_gateway_eligible_tools(
    ...
    *,
    ...
    - unconfigured: tools with no direct credentials and no explicit
      non-moor selection (easy switch, safe to pre-check)
    - has_direct: tools where the user has their own API keys
    - explicit_configured: tools with an explicit non-moor selection stored
    ...
    All lists are empty when the user is not a paid Moor subscriber or
    is not using Moor as their provider.
    """
    ...
    # Quick provider check without the heavy get_moor_subscription_features call
    model_cfg = config.get("model")
    if not isinstance(model_cfg, dict) or str(model_cfg.get("provider") or "").strip().lower() != "moor":
        return [], [], [], []

    direct = _get_gateway_direct_credentials()

    # Check which tools the user has explicitly opted into the gateway for.
    # This is distinct from managed_by_moor which fires implicitly when
    ...
    for key in _ALL_GATEWAY_KEYS:
        ...
        if opted_in.get(key):
            ...
            # An explicit non-moor selection (e.g. a keyless local backend
...
def prompt_enable_tool_gateway(
    ...
    *,
    ...
    # already_managed, just for a non-moor vendor.
    ...
    try:
        account_info = get_moor_portal_account_info(force_fresh=False)
    except Exception:
        account_info = None
    pool_only = bool(
        account_info
    ...
    # pre-checked checklist re-fired on every Moor model swap.
    ...
    if pool_only:
        title = "Your free Moor tool pool — pick the tools to enable:"
    else:
        title = (
            "Your Moor subscription includes the Tool Gateway — "
    ...
    # so the next Moor model swap doesn't pre-check it again. Cancel paths
...
def ensure_moor_portal_access(
    *,
    capability: str = "the Moor Tool Gateway",
    coverage_category: Optional[str] = None,
) -> bool:
    """Make sure the user is entitled to the Moor Tool Gateway, logging in if
    needed.

    Used by ``moor tools`` when a user selects a Moor-managed Tool Gateway
    backend (e.g. "Firecrawl (Moor Portal)").  Unlike ``moor model``'s Moor
    login, this:
    ...
    It only performs the Moor Portal device-code OAuth (when the user isn't
...
def _run_moor_portal_login_only(*, capability: str) -> bool:
    """Run the Moor Portal device-code OAuth and persist credentials only.
    ...
    try:
        proceed = input("  Log in to Moor Portal now? [Y/n]: ").strip().lower()
    ...
    try:
        ...
        # switches the user's inference provider to Moor.
        ...
        if shared:
            try:
                do_import = input(
                    "  Found existing Moor OAuth credentials. Import them? [Y/n]: "
                ).strip().lower()
            except (EOFError, KeyboardInterrupt):
                do_import = "y"
            if do_import in {"", "y", "yes"}:
                auth_state = _try_import_shared_moor_state(timeout_seconds=15.0)
        ...
        # _moor_device_code_login raises SystemExit on subscription_required;
