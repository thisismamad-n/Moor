"""Helpers for Moor subscription managed-tool capabilities."""
...
from moor_cli.config import get_env_value, load_config
from moor_cli.moor_account import (
    MoorPortalAccountInfo, format_moor_portal_entitlement_message, get_moor_portal_account_info,
)
...
_DEFAULT_PLATFORM_TOOLSETS = {"cli": "moor-cli"}
...
class _FeatureSpec:
    ...
    # Tool-pool coverage category (moor_account.TOOL_COVERAGE_CATEGORIES) gating per backend:
    ...
    offer_label: str = ""
    direct_label: str = ""
    # Direct-credential env vars that stop apply_moor_managed_defaults switching the category to
...
def _selected_provider(section: object, name_key: str = "provider") -> Optional[str]:
    """Stored provider for a section (``read_selection`` semantics): ``"moor"`` for the managed
    selection (stored ``moor`` or legacy ``use_gateway: true``), a vendor name for BYOK, else None."""
...
@dataclass(frozen=True)
class MoorFeatureState:
    key: str
    label: str
    included_by_default: bool
    available: bool
    active: bool
    managed_by_moor: bool
...
@dataclass(frozen=True)
class MoorSubscriptionFeatures:
    subscribed: bool
    moor_auth_present: bool
    provider_is_moor: bool
    features: Dict[str, MoorFeatureState]
    account_info: Optional[MoorPortalAccountInfo] = None

    def __getattr__(self, name: str) -> MoorFeatureState:  # ``features.web`` -> per-key state
        if name in _FEATURE_ORDER:
            return self.features[name]
        raise AttributeError(name)

    def items(self) -> Iterable[MoorFeatureState]:
...
def _select_moor(config: Dict[str, object], key: str) -> None:
    """Store the managed ``moor`` selection in the ``key`` section (field per _GATEWAY_SECTION_FIELDS)."""
...
def _provider_is_moor(config: Dict[str, object]) -> bool:
    return _norm(_section(config, "model").get("provider")) == "moor"
...
def _has_agent_browser() -> bool:
    ...
    try:
        ...
        # moor-managed Node dirs ($MOOR_HOME/node, prepended to PATH at runtime but usually absent
        ...
        search_paths = [None, with_moor_node_path().get("PATH", ""), str(local_bin_dir) if local_bin_dir.is_dir() else ""]
        return any(
...
def _local_stt_backend_available() -> bool:
    ...
    if get_env_value("MOOR_LOCAL_STT_COMMAND"):
        return True
    try:
...
def _account_info_or_none(**kwargs) -> Optional[MoorPortalAccountInfo]:
    """``get_moor_portal_account_info(**kwargs)``, failing closed to ``None`` on any error."""
    try:
        return get_moor_portal_account_info(**kwargs)
    except Exception:
        return None


def _state(key: str, **fields) -> MoorFeatureState:
    spec = _FEATURES[key]
    fields.setdefault("direct_override", fields["active"] and not fields["managed_by_moor"])
    return MoorFeatureState(key, spec.label, spec.included_by_default, **fields)


def _web_feature(web_cfg: Dict[str, object], tool_enabled: bool, managed: bool, web_gw: bool, direct_firecrawl: bool) -> MoorFeatureState:
    ...
    # The "moor" selection is serviced by Firecrawl — normalize so downstream vendor checks hold.
    if backend == "moor" or web_gw:
        backend = "firecrawl"
    ...
    return _state(
        ...
        managed_by_moor=web_managed, toolset_enabled=tool_enabled,
...
def _fal_feature(key: str, tool_enabled: bool, direct: bool, managed: bool, selected: Optional[str]) -> MoorFeatureState:
    ...
    if selected not in (None, "moor") or (selected is None and direct):
        label = "FAL"
    else:
        label = "Moor Subscription" if (fal_managed or selected == "moor") else ""
    return _state(
        key, available=bool(managed or direct), active=bool(tool_enabled and (fal_managed or direct)),
        managed_by_moor=fal_managed, toolset_enabled=tool_enabled, current_provider=label,
...
def _audio_provider(cfg: Dict[str, object], default: str, gw: bool) -> str:
    ...
    return "openai" if (provider == "moor" or gw) else (provider or default)
...
def _audio_features(
...
) -> tuple[MoorFeatureState, MoorFeatureState]:
    tts_gw, stt_gw = use_gateway["tts"], use_gateway["stt"]
    # STT default is "local" (faster-whisper, needs a pip install); Moor subscribers are routed to the
    # managed audio gateway by apply_moor_managed_defaults. The "moor" selection is serviced by OpenAI
    ...
    tts = _state(
        ...
        managed_by_moor=tts_tool_enabled and tts_current == "openai" and managed["tts"] and not direct_openai_tts,
    ...
    stt = _state(
        ...
        managed_by_moor=stt_current == "openai" and managed["stt"] and not direct_openai_stt,
...
def _browser_feature(
    ...
    if provider == "moor" or browser_gw:
    ...
    return _state(
        "browser", available=available, active=bool(tool_enabled and available), managed_by_moor=managed_now,
        toolset_enabled=tool_enabled, current_provider=_provider_label("browser", current), explicit_configured=explicit,
    )


def _modal_feature(terminal_cfg: Dict[str, object], tool_enabled: bool, managed: bool, managed_tools_flag: bool) -> MoorFeatureState:
    ...
    return _state(
        "modal", available=available, active=active, managed_by_moor=managed_now, direct_override=direct_override,
...
def get_moor_subscription_features(config: Optional[Dict[str, object]] = None, *, force_fresh: bool = False) -> MoorSubscriptionFeatures:
    if config is None:
        config = load_config() or {}
    provider_is_moor = _provider_is_moor(config)
    ...
    moor_auth_present = bool(account_info and account_info.logged_in)
    managed_tools_flag = moor_auth_present and account_info.tool_gateway_entitled
    enabled = {key: _toolset_enabled(config, key) for key in ("web", "image_gen", "video_gen", "tts", "browser", "terminal")}
    # Stored selections (strict model): "moor" = managed gateway; vendor name = that vendor direct;
    ...
    use_gateway = {key: value == "moor" for key, value in selected.items()}
    ...
    def _fal(key: str) -> MoorFeatureState:
    ...
    return MoorSubscriptionFeatures(
        subscribed=provider_is_moor or moor_auth_present, moor_auth_present=moor_auth_present,
        provider_is_moor=provider_is_moor, features=features, account_info=account_info,
    )
...
def apply_moor_managed_defaults(config: Dict[str, object], *, enabled_toolsets: Optional[Iterable[str]] = None, force_fresh: bool = False) -> set[str]:
    features = get_moor_subscription_features(config, force_fresh=force_fresh)
    account_info = features.account_info
    if not (account_info and account_info.logged_in and account_info.tool_gateway_entitled and features.provider_is_moor):
        return set()
...
def get_gateway_eligible_tools(config: Optional[Dict[str, object]] = None, *, force_fresh: bool = False) -> tuple[list[str], list[str], list[str], list[str]]:
    ...
    and no explicit non-moor selection (safe to pre-check) / own API keys / explicit non-moor selection
    ...
    if config is None:
        config = load_config() or {}
    if not _provider_is_moor(config):
        return [], [], [], []
    ...
    for key in _ALL_GATEWAY_KEYS:
        ...
        elif selected is not None and selected != "moor":
...
def prompt_enable_tool_gateway(config: Dict[str, object], *, force_fresh: bool = True) -> set[str]:
    ...
    if not unconfigured and not has_direct:
        return set()
    try:
        from moor_cli.setup import prompt_checklist
    except Exception:
        return set()
    ...
    source_label = "free tool pool" if pool_only else "Moor subscription"
    ...
    # every Moor model swap. See #92647.
    ...
    title = (
        "Your free Moor tool pool — pick the tools to enable:" if pool_only
        else "Your Moor subscription includes the Tool Gateway — pick the tools to enable:"
    )
    try:
...
# Inline Moor Portal login for the Tool Gateway picker (`moor tools`)


def ensure_moor_portal_access(*, capability: str = "the Moor Tool Gateway", coverage_category: Optional[str] = None) -> bool:
    """Make sure the user is entitled to the Moor Tool Gateway, logging in if needed.
    ...
    if not _entitled(info) and (info is None or not info.logged_in):
        if not _run_moor_portal_login_only(capability=capability):
            return False
        info = _account_info_or_none(force_fresh=True)
    if _entitled(info):
        return True
    # Logged in but not entitled for this capability — neutral billing guidance, do not enable.
    message = format_moor_portal_entitlement_message(info, capability=capability, coverage_category=coverage_category)
...
def _run_moor_portal_login_only(*, capability: str) -> bool:
    """Run the Moor Portal device-code OAuth and persist credentials only (no model selection, no
    ...
    try:
        ...
        print(f"  Could not start Moor Portal login: {exc}")
        return False
    print()
    print(f"  {capability} requires a Moor Portal login.")
    proceed = _confirm("  Log in to Moor Portal now? [Y/n]: ")
    if proceed is None:
        print()
        return False
    if not proceed:
        print("  Skipped Moor Portal login.")
        return False
    try:
        # Snapshot active_provider so a tool-config login never silently switches inference to Moor.
        ...
        if auth._read_shared_moor_state() and _confirm("  Found existing Moor OAuth credentials. Import them? [Y/n]: ") is not False:
            auth_state = auth._try_import_shared_moor_state(timeout_seconds=15.0)
        if auth_state is None:
            auth_state = auth._moor_device_code_login()
        with auth._auth_store_lock():
            auth_store = auth._load_auth_store()
            auth._save_provider_state(auth_store, "moor", auth_state)
            if prior_active_provider:
                auth_store["active_provider"] = prior_active_provider
            else:
                auth_store.pop("active_provider", None)
            auth._save_auth_store(auth_store)
        auth._write_shared_moor_state(auth_state)
        auth._sync_moor_pool_from_auth_store()
        print("  Moor Portal login successful.")
        return True
    except KeyboardInterrupt:
        print("\n  Login cancelled.")
        return False
    except SystemExit:
        # _moor_device_code_login raises SystemExit on subscription_required (guidance already printed).
        return False
    except Exception as exc:
        print(f"  Moor Portal login failed: {exc}")
        return False
...
_PLUGIN_COMPAT_LAZY = {
    'managed_moor_tools_enabled': ('tools.tool_backend_helpers', 'managed_moor_tools_enabled'),
}
