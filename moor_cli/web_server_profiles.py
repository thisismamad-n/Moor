...
from moor_cli.config import DEFAULT_CONFIG, get_process_moor_home
from moor_cli.web_models import MCPServerCreate
from moor_cli.web_server_gateway import _ACTION_LOG_FILES
from moor_cli.web_server_mcp import _normalize_mcp_server_create
...
@contextmanager
def _moor_home_scope(path) -> Any:
    """Scope ``load_config``/``save_config`` (anything resolving ``get_moor_home()`` at call
    time) to ``path`` for the block via the context-local MOOR_HOME override."""
    from moor_constants import set_moor_home_override, reset_moor_home_override
    token = set_moor_home_override(str(path))
    try:
        yield
    finally:
...
def _write_profile_mcp_servers(profile_dir: Path, servers: List["MCPServerCreate"]) -> int:
    """Write MCP server entries into ``profile_dir``'s config.yaml (MOOR_HOME-scoped).
...
def _profile_scope(profile: Optional[str]):
    ...
    Two seams: (1) ``load_config``/``save_config`` resolve ``get_moor_home()`` at call
    ...
    ``get_moor_home()`` so writes land in the live home even when the import-time binding
    is stale (test isolation, late MOOR_HOME override). Yields the profile dir for a named
    ...
    with _config_profile_scope(profile) as scoped:
        profile_dir = get_moor_home() if scoped is None else scoped
        modules = (_skills_tool, _skill_mgr)
        with _SKILLS_PROFILE_LOCK:
            saved = [(m.MOOR_HOME, m.SKILLS_DIR) for m in modules]
            for m in modules:
                m.MOOR_HOME, m.SKILLS_DIR = profile_dir, profile_dir / "skills"
            try:
                yield scoped
            finally:
                for m, (home, skills_dir) in zip(modules, saved):
                    m.MOOR_HOME, m.SKILLS_DIR = home, skills_dir


@contextmanager
def _config_profile_scope(profile: Optional[str]):
    """Await-safe, config-only profile scope: touches ONLY the task-local MOOR_HOME
...
def _profile_cli_args(profile: Optional[str]) -> List[str]:
    ...
    a fresh ``moor`` subprocess whose ``_apply_profile_override()`` reads ``-p`` from argv —
...
def _hub_action_name(verb: str, key: str) -> str:
    """Unique per-skill hub action name (+ registered log file): ``_spawn_moor_action``
