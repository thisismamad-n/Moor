"""Slash command definitions and autocomplete for the Moor CLI.
...
from moor_constants import INDICATOR_STYLES
...
# _moor_auth_status_cache). Falls back to a fresh load when the file cannot
# be stat'ed.
...
class CommandDef:
    ...
    # Names a key in ``moor_cli.slash_exec.EXECUTORS`` — a pure formatter
    # producing the canonical, surface-independent core text.  Surfaces
    # resolve it via ``moor_cli.slash_exec.run_execute`` and apply only
...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("reload-mcp", "Reload MCP servers from config", "Tools & Skills",
               aliases=("reload_mcp",)),
    CommandDef("reload-skills", "Re-scan ~/.moor/skills/ for newly installed or removed skills",
    ...
    CommandDef("subscription", "View your Moor plan and change it in the browser", "Info",
               cli_only=True, aliases=("upgrade",)),
    CommandDef("topup", "Show your Moor balance and manage billing on the portal", "Info"),
...
def _iter_plugin_command_entries() -> list[tuple[str, str, str]]:
    ...
    :func:`moor_cli.plugins.PluginContext.register_command`. They behave
    like ``CommandDef`` entries for gateway surfacing: they appear in the
    Telegram command menu, in Slack's ``/moor`` subcommand mapping, and
...
# Telegram allows up to 100 BotCommands. Moor ships ~50 built-in commands;
...
Telegram only displays a small BotCommand menu in practice.  The full Moor
...
def _collect_gateway_skill_entries(
    ...
    try:
        ...
        # ``moor skills list`` and the agent's ``/skill-name`` dispatch but
...
def telegram_menu_commands(max_commands: int = 100) -> tuple[list[tuple[str, str]], int]:
    ...
    Skills disabled for the ``"telegram"`` platform (via ``moor skills
...
def discord_skill_commands_by_category(
    ...
    *uncategorized*.
    ...
    parity, external-dir skills are visible via ``moor skills list`` and
...
# unrelated command landed. These claim their slots right after /moor,
# ahead of both canonical names and the rest of the aliases. Anything not
# listed here still degrades gracefully (reachable via /moor <command>).
...
# native slot, the alias spelling stays reachable via /moor reset).
...
# ``/moor <command>`` on Slack only. They remain native on every other
# surface (CLI, TUI, Telegram, Discord). Keep this list TIGHT and intentional —
# the telegram-parity test reads it so an entry here is a deliberate
# "Slack-via-/moor" decision, not a silent clamp.
#   - topup: the billing/balance surface; reached via /moor topup on Slack.
#     (the rehaul folded the old /credits + /billing surfaces into /topup.)
#   - moa: high-cost slash mode, available through /moor moa to avoid
#     displacing existing native Slack slash commands at the 50-command cap.
#   - debug: the log/report upload surface; reached via /moor debug on Slack.
#   - egress: Docker-only proxy status; reachable as /moor egress on Slack.
#   - init: repo-scan AGENTS.md bootstrap — a cwd-centric dev command that is
#     rare from Slack; reachable as /moor init. Without this entry, adding
#     /init clamps /version off the native list and breaks Telegram parity.
#   - version: low-frequency info command; reachable as /moor version on
...
#   - diff: git working-tree diff; reached via /moor diff on Slack so it
...
#     /moor update on Slack. Demoted to free the native slot /approvals now
...
#   - heartbeat: session heartbeat management; reached via /moor heartbeat
#     on Slack. Added at the 50-cap — a native slot would clamp /insights.
#   - refine: on-demand memory/skill review; reached via /moor refine on
#     Slack. Added at the 50-cap — a native slot would clamp an existing
#     native slash.
#   - pause: global emergency stop; reached via /moor pause [off] on
#     Slack. Added at the 50-cap — a native slot would clamp /platform.
#   - whoami: one-off identity lookup; reached via /moor whoami on Slack.
...
#     /moor platform on Slack. Demoted when /save became gateway-available
#     (session export is an interactive surface; platform is a rare
#     informational lookup) — without this entry /save tips the registry
#     past the 50-cap and silently clamps /platform, breaking parity.
_SLACK_VIA_MOOR_ONLY = frozenset({"topup", "moa", "debug", "egress", "init", "version", "diff", "update", "heartbeat", "refine", "review", "pause", "whoami", "platform"})
...
def slack_native_slashes() -> list[tuple[str, str, str]]:
    ...
    first-class slash and not a ``/moor <verb>`` subcommand.
    ...
    can still reach them via ``/moor <command>``.

    Results are clamped to Slack's 50-command limit with duplicate-name
    avoidance. ``/moor`` is always reserved as the first entry so the
    legacy ``/moor <subcommand>`` form keeps working for anything that
    ...
    # Reserve /moor as the catch-all top-level command.
    entries.append(("moor", "Talk to Moor or run a subcommand", "[subcommand] [args]"))
    seen.add("moor")
    ...
    def _add(name: str, desc: str, hint: str) -> None:
        ...
        if slack_name in _SLACK_RESERVED_COMMANDS:
            return
        if slack_name in _SLACK_VIA_MOOR_ONLY:
            # Intentionally Slack-via-/moor only (see _SLACK_VIA_MOOR_ONLY).
            return
    ...
    # everything except /moor, so a new canonical command can never silently
...
def slack_app_manifest(request_url: str = "https://moor-agent.local/slack/commands") -> dict[str, Any]:
...
def slack_subcommand_map() -> dict[str, str]:
    """Return subcommand -> /command mapping for Slack /moor handler.

    Maps both canonical names and aliases so /moor bg do stuff works
    the same as /moor background do stuff.

    Plugin-registered slash commands are included so ``/moor <plugin-cmd>``
...
class SlashCommandCompleter(Completer):
    ...
    def _personality_completions(sub_text: str, sub_lower: str):
        """Yield completions for /personality via moor_cli.personality."""
        try:
            ...
            # freshness-correct (same pattern as load_env / _moor_auth_status_cache).
