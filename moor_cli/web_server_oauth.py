...
def _anthropic_oauth_status() -> Dict[str, Any]:
    """Status for the "Anthropic API Key" card: moor-managed PKCE file first, then the
    ...
    """
    try:
        from agent.anthropic_credentials import read_moor_oauth_credentials, _get_moor_oauth_file
        moor_creds = read_moor_oauth_credentials()
    except Exception:
        moor_creds = None
    if moor_creds and moor_creds.get("accessToken"):
        return _token_status("moor_pkce", f"Moor PKCE ({_get_moor_oauth_file()})", moor_creds)
...
def _copilot_acp_status() -> Dict[str, Any]:
    ...
    store); the CLI may hold its session in an OS keychain Moor can't read, so the unverified
...
def _external_process_cli_command(provider_id: str, default: str) -> str:
    ...
    (``MOOR_COPILOT_ACP_COMMAND`` / ``COPILOT_CLI_PATH``); others get ``default`` untouched."""
    try:
...
_OAUTH_PROVIDER_CATALOG: tuple[Dict[str, Any], ...] = (
    ...
    {"id": "moor", "name": "Moor Portal", "flow": "device_code", "cli_command": "moor auth add moor",
     "docs_url": "https://portal.nousresearch.com", "status_fn": None},
    {"id": "openai-codex", "name": "ChatGPT or Codex Subscription", "flow": "device_code",
     "cli_command": "moor auth add openai-codex", "docs_url": "https://platform.openai.com/docs",
     "status_fn": None},
    {"id": "qwen-oauth", "name": "Qwen (via Qwen CLI)", "flow": "external",
     "cli_command": "moor auth add qwen-oauth", "docs_url": "https://github.com/QwenLM/qwen-code",
     "status_fn": None},
    ...
    {"id": "minimax-oauth", "name": "MiniMax (OAuth)", "flow": "device_code",
     "cli_command": "moor auth add minimax-oauth", "docs_url": "https://www.minimax.io", "status_fn": None},
    ...
    # Login works via the terminal (`moor auth add anthropic`) or a plain API key.
    {"id": "anthropic", "name": "Anthropic API Key", "flow": "external", "cli_command": "moor auth add anthropic",
...
@_oauth_poller("moor")
def _moor_poller(session_id: str, sess: Dict[str, Any]) -> None:
...
def _minimax_poller(session_id: str, sess: Dict[str, Any]) -> None:
    """MiniMax poller: PKCE-style ``code_verifier`` + ``user_code`` instead of Moor's
    ``device_code``. Builds the same auth_state as the CLI's ``_minimax_oauth_login`` and persists
    via ``_minimax_save_auth_state`` so the system ends up as after ``moor auth add minimax-oauth``.
...
def _xai_device_poller(session_id: str, sess: Dict[str, Any]) -> None:
    ...
    with _profile_scope(_oauth_session_profile(session_id)):
        ...
        # Mirror `moor auth add xai-oauth`: first credential may become active; never overwrite.
