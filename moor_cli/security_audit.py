"""On-demand supply-chain audit for Moor Agent installs.

Scans three surfaces a Moor user actually controls and we can map to
upstream advisories without auth or extra binaries:

1. The Moor venv (every PyPI dist via ``importlib.metadata``).
2. Python deps declared by user-installed plugins under ``~/.moor/plugins``
...
def _discover_plugins(moor_home: Path) -> list[Component]:
    """Python deps declared by plugins under ``~/.moor/plugins``.
...
def _discover_components(
    *,
    ...
    home = moor_home or Path(get_moor_home())
...
def cmd_security_audit(args: argparse.Namespace) -> int:
    ...
    components = _discover_components(
        skip_venv=skip_venv, skip_plugins=skip_plugins, skip_mcp=skip_mcp, moor_home=home
    )
