...
:mod:`moor_cli.runtime_provider`; origin-internal collaborators
...
def _rp():
    """Origin module, late-bound so test patches on ``moor_cli.runtime_provider.*`` apply."""
    import moor_cli.runtime_provider as origin
    return origin
...
def _shadowed_by_builtin(requested_norm: str) -> bool:
    ...
    canonical provider (``moor``); an entry matching merely an alias (``kimi`` → ``kimi-coding``)
...
def _resolve_named_custom_runtime(*, requested_provider: str, explicit_api_key: Optional[str] = None,
    ...
    if family is not None and not custom_provider.get("api_mode"):
        from moor_cli.models import normalize_opencode_base_url, opencode_model_api_mode
