"""``moor doctor`` — diagnose (and with --fix, repair) a Moor install.
...
from moor_cli.config import get_env_path, get_moor_home, get_project_root
...
from moor_cli.doctor_report import Finding, _section, check_bool, check_info, doctor_check, warn_on_error
from moor_cli.doctor_connectivity import _has_healthy_oauth_fallback_for_apikey_provider, build_probes, run_probes
from moor_cli.doctor_tools import _safe_which

from moor_cli.doctor_config import (
    _check_config_drift,
    _check_config_file,
    _check_env_file,
    _check_mcp_security,
...
from moor_cli.doctor_platform import (
    _check_certificates,
...
from moor_cli.doctor_tools import (
    _check_git_and_rg,
...
def _check_auth_providers(should_fix: bool, f: Finding) -> None:
    ...
    with warn_on_error("Auth provider status", "(could not check: {e})"):
        from moor_cli.auth import get_moor_auth_status_local, get_codex_auth_status, get_minimax_oauth_auth_status
        _login_row("Moor Portal auth", get_moor_auth_status_local())
        # Native OAuth is Moor' own device-code flow; the Codex CLI only imports existing ~/.codex/auth.json
...
def _ack_advisory(ack_target: str) -> None:
    """`moor doctor --ack <id>`: persist the ack and return without running diagnostics."""
    from moor_cli.security_advisories import ADVISORIES, ack_advisory
    ...
    if ack_advisory(ack_target):
        ...
        print(color(f"  ✗ Failed to persist ack for {ack_target}. Check ~/.moor/config.yaml is writable.", Colors.RED))
        sys.exit(1)
...
_PLUGIN_COMPAT_LAZY = {
    ...
    'OPENROUTER_MODELS_URL': ('moor_constants', 'OPENROUTER_MODELS_URL'),
    'STATE_DB_SIZE_WARN_BYTES': ('moor_cli.doctor_state', 'STATE_DB_SIZE_WARN_BYTES'),
    'agent_browser_runnable': ('moor_constants', 'agent_browser_runnable'),
    'base_url_host_matches': ('utils', 'base_url_host_matches'),
    'check_certificates': ('moor_cli.doctor_platform', 'check_certificates'),
    'check_macos_full_disk_access': ('moor_cli.doctor_platform', 'check_macos_full_disk_access'),
    'check_macos_tcc_anchor': ('moor_cli.doctor_platform', 'check_macos_tcc_anchor'),
    'check_macos_tcc_grants': ('moor_cli.doctor_platform', 'check_macos_tcc_grants'),
    'collect_deprecated_config_keys': ('moor_cli.doctor_config', 'collect_deprecated_config_keys'),
    'collect_deprecated_env_vars': ('moor_cli.doctor_config', 'collect_deprecated_env_vars'),
    'collect_relay_plugin_cutover_findings': ('moor_cli.doctor_config', 'collect_relay_plugin_cutover_findings'),
    'describe_vercel_auth': ('moor_cli.vercel_auth', 'describe_vercel_auth'),
    'detect_install_method': ('moor_cli.config', 'detect_install_method'),
    'is_nix_install_method': ('moor_cli.config', 'is_nix_install_method'),
    'managed_scope_check': ('moor_cli.doctor_config', 'managed_scope_check'),
    'recommended_update_command_for_method': ('moor_cli.config', 'recommended_update_command_for_method'),
    'report_deprecated_config_and_env': ('moor_cli.doctor_config', 'report_deprecated_config_and_env'),
}
