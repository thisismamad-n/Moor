"""
Gateway subcommand for moor CLI.

Handles: moor gateway [run|start|stop|restart|status|install|uninstall|setup]
"""
...
# display_moor_home is imported lazily at call sites to avoid ImportError
# when moor_constants is cached from a pre-update version during `moor update`.
...
def _get_service_pids(all_profiles: bool = False) -> set:
    ...
    ``ai.moor.gateway*`` LaunchAgent — the update path needs the whole
    ...
    systemd branch has always been fleet-wide (``moor-gateway*``) and is
    unaffected.
    """
    pids: set = set()

    # --- systemd (Linux): user and system scopes ---
    # systemd always lists every moor-gateway* unit regardless of scope.
    ...
    if is_macos():
        ...
        if all_profiles:
            ...
            # mirrors the systemd branch's ``moor-gateway*`` pattern above.
        ...
        if all_profiles:
            ...
            # ``launchctl list`` prefix scan also catches ai.moor.gateway*
...
def _get_parent_pid(pid: int) -> int | None:
    ...
    "moor CLI that invoked this scan" from "real gateway process".
    """
    if pid <= 1:
        return None
    try:
...
# — restart may fail", and `moor update` can deadlock behind it.  The loop
...
def _get_ancestor_pids() -> set[int]:
    ...
    ``moor gateway status`` from falsely counting the ``moor`` CLI that
...
def _scan_gateway_pids(
    ...
    # scan (e.g. ``moor gateway status``) is never mistaken for a running
    # gateway.  See #13242.
    ...
    # Forward slashes on both sides of the MOOR_HOME= match — see
    ...
    def _matches_current_profile(command: str) -> bool:
        ...
        if current_profile_name:
            return (
                ...
                or f"moor_home={current_home_lc}" in command_lc
            )

        # Default-profile case: no profile flag in argv. Accept as long as
        # the command doesn't advertise *some other* profile. MOOR_HOME
        ...
        # MOOR_HOME= in argv is.
        if "--profile " in command_lc or " -p " in command_lc:
            return False
        if (
            "moor_home=" in command_lc
            and f"moor_home={current_home_lc}" not in command_lc
        ):
            return False
        return True
    ...
    try:
        if is_windows():
            ...
            # wedges the caller forever. ``moor update`` hung exactly there
...
def find_gateway_pids(
    ...
    Args:
        ...
        all_profiles: When ``True``, return gateway PIDs across **all**
            profiles (the pre-7923 global behaviour).  ``moor update``
            needs this because a code update affects every profile.
            When ``False`` (default), only PIDs belonging to the current
            Moor profile are returned.
    """
...
def find_profile_gateway_processes(
    ...
    """Return running gateway PIDs mapped to Moor profiles via PID files."""
    _exclude = set(exclude_pids or set())
...
def _gateway_run_args_for_profile(profile: str) -> list[str]:
    args = [get_python_path(), "-m", "moor_cli.main"]
    if profile != "default":
...
def _capture_gateway_argv(pid: int) -> list[str] | None:
    ...
    Windows Scheduled Task running ``pythonw.exe -m moor_cli.main gateway
...
def _prepare_profile_gateway_update_restart(profile: str, pid: int) -> str | None:
    """Choose who relaunches a profile gateway after ``moor update``.

    A gateway started with ``--external-supervisor`` must exit back to that
    manager. Starting Moor's detached watcher as well would escape the
...
def launch_detached_gateway_restart_by_cmdline(
    ...
    ``gateway run`` whose MOOR_HOME or argv doesn't match a known profile).
...
def _spawn_gateway_restart_watcher(old_pid: int, run_argv: list[str]) -> bool:
    ...
    # when the user closes the terminal, leaving ``moor update`` users
    # with no running gateway until they re-invoke ``moor gateway``
    ...
    from moor_cli._subprocess_compat import (
    ...
    # interpreter and captures the stable cwd + env overlay (MOOR_HOME,
    ...
    watcher = textwrap.dedent(
        ...
        # the env (VIRTUAL_ENV / PYTHONPATH / MOOR_HOME) the windowless
        # base interpreter needs to import moor_cli.  Empty on POSIX, where
...
def _moor_home_from_systemd_unit_file(system: bool = False) -> str | None:
    """Read ``MOOR_HOME`` from the on-disk unit file (not ``systemctl show``).
...
def _sync_moor_home_from_systemd_unit(system: bool) -> None:
    """When acting on a system-scope unit, adopt its ``MOOR_HOME``.

    Under ``sudo``, ``MOOR_HOME`` is stripped and ``HOME=/root``, so
    :func:`get_moor_home` falls back to ``/root/.moor`` — the wrong
    profile. The unit file pins ``MOOR_HOME`` for the actual gateway
    ...
    unit_home = (_moor_home_from_systemd_unit_file(system=True) or "").strip()
    if not unit_home:
        unit_home = _read_systemd_unit_environment(system=True).get("MOOR_HOME", "").strip()
    if not unit_home:
        return
    current = os.environ.get("MOOR_HOME", "").strip()
    if current == unit_home:
        return
    os.environ["MOOR_HOME"] = unit_home
...
def _wait_for_systemd_service_restart(
    *,
    ...
    print(
        ...
        f"  Check status: {'sudo ' if system else ''}moor gateway status\n"
...
def _print_systemd_start_limit_wait(system: bool = False) -> None:
    ...
    print(
        f"  Wait for the start-limit window to expire, then run: {'sudo ' if system else ''}moor gateway restart{scope_flag}"
    )
...
def get_gateway_runtime_snapshot(system: bool = False) -> GatewayRuntimeSnapshot:
    ...
    if is_linux() and is_container():
        ...
        try:
            from moor_cli.service_manager import detect_service_manager, get_service_manager
...
def _print_gateway_process_mismatch(snapshot: GatewayRuntimeSnapshot) -> None:
    ...
    if _launchd_unsupported_marker_exists():
        ...
        print(f"  PID(s): {_format_gateway_pids(snapshot.gateway_pids, limit=None)}")
        print("  This is usually a manual foreground/tmux/nohup run, so `moor gateway`")
...
def _print_other_profiles_gateway_status() -> None:
    ...
    Shown at the bottom of ``moor gateway status`` output so users with
...
def _gateway_list() -> None:
    ...
    """
    try:
        from moor_cli.profiles import list_profiles, get_active_profile_name
    except Exception:
...
def _reap_unsupervised_gateway_orphans(extra_exclude: set | None = None) -> bool:
    ...
    in-process under a ``gateway restart`` argv (moor_cli/gateway.py restart
    ...
    if is_windows():
        try:
            # The install-time task name is profile-aware (Moor_Gateway /
            # Moor_Gateway_<profile>) — never hardcode it, or the guard is
            # dormant on every standard `moor gateway install` deployment.
            from moor_cli.gateway_windows import get_task_name

            _task_name = get_task_name()
        except Exception:
            _task_name = "Moor_Gateway"
    ...
    try:
        ...
        # whole ai.moor.gateway* fleet — not just the current profile's
...
def stop_profile_gateway() -> bool:
    """Stop only the gateway for the current profile (MOOR_HOME-scoped).
...
from moor_constants import is_container, is_termux, is_wsl
...
def _container_systemd_operational() -> bool:
    ...
    This is NOT our Moor Docker image — that one runs s6-overlay as
...
def _windows_gateway_should_absorb_console_controls() -> bool:
    ...
    Foreground ``moor gateway run`` must remain interruptible from
    PowerShell/CMD. Detached service-style launches opt in via
    ``MOOR_GATEWAY_DETACHED=1``; older wrappers without the env marker are
    treated as detached when no interactive stdin is attached.
    """
    if not is_windows():
        return False

    detached = os.getenv("MOOR_GATEWAY_DETACHED", "").strip().lower()
...
def _windows_gateway_breakaway_state() -> bool | None:
    ...
    if not is_windows():
        return None
    from moor_cli._subprocess_compat import _WINDOWS_GATEWAY_BREAKAWAY_ENV
...
_SERVICE_BASE = "moor-gateway"
SERVICE_DESCRIPTION = "Moor Agent Gateway - Messaging Platform Integration"


def _profile_suffix() -> str:
    """Derive a service-name suffix from the current MOOR_HOME.

    Returns ``""`` for the default root, the profile name for
    ``<root>/profiles/<name>``, or a short hash for any other path.
    Works correctly in Docker (MOOR_HOME=/opt/data) and standard deployments.
    """
    import hashlib
    import re
    ...
    default = get_default_moor_root().resolve()
    if home == default:
        return ""
    ...
    try:
        ...
        pass
    # Fallback: short hash for arbitrary MOOR_HOME paths
    return hashlib.sha256(str(home).encode()).hexdigest()[:8]


def _profile_arg(moor_home: str | None = None, default_root: str | Path | None = None) -> str:
    """Return ``--profile <name>`` only when MOOR_HOME is a named profile.

    For ``~/.moor/profiles/<name>``, returns ``"--profile <name>"``.
    For the default profile or hash-based custom paths, returns the empty string.

    Args:
        moor_home: Optional explicit MOOR_HOME path. Defaults to the current
            ``get_moor_home()`` value. Should be passed when generating a
            service definition for a different user (e.g. system service).
        default_root: Optional Moor root to compare against. Used when
            generating a system service for another user from a sudo/root
            process, where ``Path.home()`` and ``get_default_moor_root()``
            refer to root but the target profile lives under the service user.
    """
    import re
    from moor_constants import get_default_moor_root

    home = Path(moor_home or str(get_moor_home())).resolve()
    default = Path(default_root).resolve() if default_root else get_default_moor_root().resolve()
    if home == default:
        return ""
    profiles_root = (default / "profiles").resolve()
    try:
...
def _profile_arg_for_target_user(moor_home: str, target_home_dir: str) -> str:
    """Return the profile arg for a system service running as another user."""
    target_root = Path(target_home_dir) / ".moor"
    try:
        Path(moor_home).resolve().relative_to(target_root.resolve())
        return _profile_arg(moor_home, default_root=target_root)
    except ValueError:
        return _profile_arg(moor_home)


def get_service_name() -> str:
    """Derive a systemd service name scoped to this MOOR_HOME.

    Default ``~/.moor`` returns ``moor-gateway`` (backward compatible).
    Profile ``~/.moor/profiles/coder`` returns ``moor-gateway-coder``.
    Any other MOOR_HOME appends a short hash for uniqueness.
    """
...
# Legacy service names from older Moor installs that predate the
# moor-gateway rename. Kept as an explicit allowlist (NOT a glob) so
# profile units (moor-gateway-*.service) and unrelated third-party
# "moor" units are never matched.
_LEGACY_SERVICE_NAMES: tuple[str, ...] = ("moor.service",)
...
def _find_legacy_moor_units() -> list[tuple[str, Path, bool]]:
    """Return ``[(unit_name, unit_path, is_system)]`` for legacy Moor gateway units.

    Detects unit files installed by older Moor versions that used a
    different service name (e.g. ``moor.service`` before the rename to
    ``moor-gateway.service``). When both a legacy unit and the current
    ``moor-gateway.service`` are active, they fight over the same bot
    ...
    * Explicit allowlist of legacy names (no globbing). Profile units such
      as ``moor-gateway-coder.service`` and unrelated third-party
      ``moor-*`` services are never matched.
    * ExecStart content check — only flag units that invoke our gateway
      entrypoint. A user-created ``moor.service`` running an unrelated
...
def has_legacy_moor_units() -> bool:
    """Return True when any legacy Moor gateway unit files exist."""
    return bool(_find_legacy_moor_units())


def print_legacy_unit_warning() -> None:
    """Warn about legacy Moor gateway unit files if any are installed.

    Idempotent: prints nothing when no legacy units are detected. Safe to
    call from any status/install/setup path.
    """
    legacy = _find_legacy_moor_units()
    if not legacy:
        return
    print_warning("Legacy Moor gateway unit(s) detected from an older install:")
    for name, path, is_system in legacy:
        scope = "system" if is_system else "user"
        print_info(f"    {path}  ({scope} scope)")
    print_info("  These run alongside the current moor-gateway service and")
    print_info("  cause SIGTERM flap loops — both try to use the same bot token.")
    print_info("  Remove them with:")
    print_info("    moor gateway migrate-legacy")


def remove_legacy_moor_units(
    interactive: bool = True,
    dry_run: bool = False,
) -> tuple[int, list[Path]]:
    """Stop, disable, and remove legacy Moor gateway unit files.

    Iterates over whatever ``_find_legacy_moor_units()`` returns — which is
    ...
    legacy = _find_legacy_moor_units()
    if not legacy:
        print("No legacy Moor gateway units found.")
        return 0, []
    ...
    print()
    print("Legacy Moor gateway unit(s) found:")
    for name, path, is_system in legacy:
        scope = "system" if is_system else "user"
    ...
    if interactive and not prompt_yes_no("Remove these legacy units?", True):
        print("Skipped. Run again with: moor gateway migrate-legacy")
        return 0, [p for _, p, _ in legacy]
    ...
    if system_units:
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd removal path, guarded by `if system == "Linux"` / systemd-only branch
            ...
            print_info("  Re-run with: sudo moor gateway migrate-legacy")
...
def print_systemd_scope_conflict_warning() -> None:
    ...
    print_info("    moor gateway uninstall")
    print_info("    sudo moor gateway uninstall --system")
    ...
    if scope == "system":
        ...
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd install wizard, never invoked on Windows
            ...
            print_warning(
                ...
                "root shell, or install a user service instead: moor gateway install"
            )
            return scope, False
...
def ensure_gateway_service(context: str = "setup") -> bool:
    ...
    The zero-decision service path used by ``moor setup`` (end of wizard)
    and ``moor import``: if this host supports a service manager and no
    ...
    ``moor gateway install --system`` explicitly (that path prompts and
    ...
    if is_container():
        ...
        print_info("   moor gateway run          # Run as container main process")
        print_info("")
    ...
    if not (supports_systemd or is_macos() or is_windows()):
        ...
        print_info("  Run the gateway in the foreground with: moor gateway")
        return False
    ...
    try:
        ...
        print_info("  You can retry manually: moor gateway install")
...
def _launchd_user_home() -> Path:
    ...
    Profile-mode Moor often sets ``HOME`` to a profile-scoped directory, but
...
def get_launchd_plist_path() -> Path:
    ...
    Default ``~/.moor`` → ``ai.moor.gateway.plist`` (backward compatible).
    Profile ``~/.moor/profiles/coder`` → ``ai.moor.gateway-coder.plist``.
    """
    suffix = _profile_suffix()
    name = f"ai.moor.gateway-{suffix}" if suffix else "ai.moor.gateway"
...
def launchd_gateway_labels_for_install() -> list[str]:
    ...
    ``get_default_moor_root()``), NOT by globbing ``~/Library/LaunchAgents``:
    the LaunchAgents directory is shared per-user, so a sandboxed
    ``MOOR_HOME`` (tests, capture sandboxes, side-by-side installs) must
    ...
    for profile in list_profiles():
        if profile.is_default:
            root_label.append("ai.moor.gateway")
        elif _re.match(r"^[a-z0-9][a-z0-9_-]{0,63}$", profile.name):
            profile_labels.append(f"ai.moor.gateway-{profile.name}")
...
def get_python_path() -> str:
    ...
    if venv is not None:
        try:
            ...
            # moor_constants cached from before this symbol existed. See
            # _reload_moor_constants() in moor_cli/managed_uv.py.
...
def _remap_path_for_user(path: str, target_home_dir: str) -> str:
    ...
    to *target_home_dir*; otherwise the path is returned unchanged.
      ...
      /root/.moor/moor-agent  -> /home/alice/.moor/moor-agent
      /opt/moor                 -> /opt/moor  (kept as-is)
...
def _moor_home_for_target_user(target_home_dir: str) -> str:
    """Remap the current MOOR_HOME to the equivalent under a target user's home.

    When installing a system service via sudo, get_moor_home() resolves to
    root's home.  This translates it to the target user's equivalent path:
      /root/.moor                    → /home/alice/.moor
      /root/.moor/profiles/coder     → /home/alice/.moor/profiles/coder
      /opt/custom-moor               → /opt/custom-moor  (kept as-is)
    """
    current_moor_raw = os.environ.get("MOOR_HOME", "").strip()
    current_moor = (
        Path(current_moor_raw).expanduser()
        if current_moor_raw
        else get_moor_home()
    )
    ...
    # different MOOR_HOME into the generated service unit.
    current_default = Path.home() / ".moor"
    target_default = Path(target_home_dir) / ".moor"

    # Default ~/.moor → remap to target user's default
    if current_moor == current_default:
        return str(target_default)

    # Profile or subdir of ~/.moor → preserve the relative structure
    try:
        relative = current_moor.relative_to(current_default)
        return str(target_default / relative)
    except ValueError:
        # Completely custom path (not under ~/.moor) — keep as-is
        return str(current_moor)
...
def _build_service_path_dirs(project_root: Path | None = None) -> list[str]:
    ...
    moor_node = moor_home / "node" / "bin"
    if _is_dir(moor_node):
        candidates.append(str(moor_node))
    moor_nm = moor_home / "node_modules" / ".bin"
    if _is_dir(moor_nm):
        candidates.append(str(moor_nm))
...
def _stable_service_working_dir() -> str:
    ...
    uses an absolute python interpreter and ``-m moor_cli.main``, so module
    ...
    dir, or a clone that ``moor update`` later relocates/removes — the path
    ...
    ``MOOR_HOME`` is the stable anchor: it is where config/state/logs live,
    it never moves, and it is guaranteed to exist whenever the gateway is
    meaningfully installed. Fall back to ``PROJECT_ROOT`` only if MOOR_HOME
...
def _systemd_watchdog_seconds(moor_home: str | Path | None = None) -> int:
    ...
    if moor_home is not None:
        ...
        override_token = set_moor_home_override(moor_home)
        reset_home_override = reset_moor_home_override
    try:
...
def _systemd_watchdog_service_fields(
    moor_home: str | Path | None = None,
) -> tuple[str, str]:
    """Return systemd service fields for the effective gateway config."""
    seconds = _systemd_watchdog_seconds(moor_home)
    if seconds <= 0:
        return "simple", ""
...
def _append_node_dir_for_service(
    path_entries: list[str], moor_root: Path | None = None
) -> None:
    """Add the Node directory a generated service unit should use to *path_entries*.

    The moor-managed Node under ``$MOOR_HOME/node`` goes first when it
    ...
    *moor_root* is the Moor home the unit will run against. System units
    ...
    """
    from moor_constants import (
        moor_managed_node_tree_present,
        iter_moor_node_dirs,
    )
...
def generate_systemd_unit(system: bool = False, run_as_user: str | None = None) -> str:
    ...
    if system:
        ...
        moor_home = _moor_home_for_target_user(home_dir)
        systemd_type, systemd_watchdog_directives = _systemd_watchdog_service_fields(
            moor_home
        )
        profile_arg = _profile_arg_for_target_user(moor_home, home_dir)
        # Remap all paths that may resolve under the calling user's home
        # (e.g. /root/) to the target user's home so the service can
        # actually access them.
        python_path = _remap_path_for_user(python_path, home_dir)
        # Anchor cwd to the target user's MOOR_HOME (stable, always exists)
        ...
        # the remapped moor_home, not the calling user's. Prepend — the
        ...
        _append_node_dir_for_service(
            _target_node_entries, Path(moor_home) if moor_home else None
        )
        path_entries = [
...
Group={group_name}
ExecStart={python_path} -m moor_cli.main{f" {profile_arg}" if profile_arg else ""} gateway run
...
"""
    ...
    moor_home = str(get_moor_home().resolve())
    systemd_type, systemd_watchdog_directives = _systemd_watchdog_service_fields(
        moor_home
    )
...
[Service]
Type={systemd_type}
{systemd_watchdog_directives}ExecStart={python_path} -m moor_cli.main{f" {profile_arg}" if profile_arg else ""} gateway run
WorkingDirectory={working_dir}
Environment="PATH={sane_path}"
Environment="VIRTUAL_ENV={venv_dir}"
Environment="MOOR_HOME={moor_home}"
...
def systemd_unit_is_current(system: bool = False) -> bool:
    ...
    # pinned MOOR_HOME is adopted before any compare/regenerate" at a single
    # site, so a future callsite cannot regress it by forgetting to pre-sync.
    #
    # Under ``sudo moor gateway … --system``, MOOR_HOME is often stripped
    # and falls back to ``/root/.moor``. Adopting the unit's pinned home
    # first makes TimeoutStopSec / WorkingDirectory / MOOR_HOME comparisons
    ...
    _sync_moor_home_from_systemd_unit(system=system)
...
def _temp_home_in_service_definition(definition: str) -> str | None:
    """Return the temp-dir MOOR_HOME baked into a service definition, or None.

    A generated systemd unit / launchd plist carries the resolved MOOR_HOME
    ...
    exported a throwaway ``MOOR_HOME=/tmp/...`` — writing it to the real
    ...
    Seen live 2026-06-11: an E2E guard probe ran ``moor gateway restart``
    with ``MOOR_HOME=/tmp/moor-e2e-<pr>`` exported; the restart path's
    ...
    Matches both systemd ``Environment="MOOR_HOME=..."`` lines and launchd
    ``<key>MOOR_HOME</key><string>...</string>`` pairs.
    """
    import re
    import tempfile

    candidates = re.findall(r'MOOR_HOME=([^"\n]+)', definition)
    candidates += re.findall(
        r"<key>MOOR_HOME</key>\s*<string>(.*?)</string>", definition, flags=re.S
    )
    temp_roots = {
...
def _refuse_temp_home_service_write(definition: str, kind: str) -> bool:
    """Refuse (with guidance) when a service definition carries a temp MOOR_HOME."""
    temp_home = _temp_home_in_service_definition(definition)
    if temp_home is None:
        return False
    print(
        f"✗ Refusing to write the gateway {kind}: MOOR_HOME resolves to a "
        f"temporary directory ({temp_home})."
    )
    print(
        "  This usually means a test/E2E environment exported MOOR_HOME. "
...
def refresh_systemd_unit_if_needed(system: bool = False) -> bool:
    ...
    # single MOOR_HOME-sync chokepoint (adopts the unit's pinned home before
    ...
    # sandboxed by the test conftest (only MOOR_HOME is). If a test
    # exercises ``run_gateway()`` with a pytest-tmp MOOR_HOME, the freshly
    # generated unit bakes that ``/tmp/pytest-of-.../moor_test`` path into
    # ``Environment="MOOR_HOME=..."``. Writing that to the developer's
    ...
    if not system and (
        ...
        or '/moor_test"' in new_unit
        or "/moor_test/" in new_unit
    ):
        return False

    # Structural variant of the same belt: refuse to bake ANY temp-dir
    # MOOR_HOME into the unit (manual E2E homes like /tmp/moor-e2e-NNN
    ...
    print(
        f"↻ Updated gateway {_service_scope_label(system)} service definition to match the current Moor install"
    )
    return True
...
def _print_system_scope_remediation(action: str) -> None:
    ...
    print_info("         sudo moor gateway uninstall --system")
    print_info("         moor gateway install")
    print_info("         moor gateway start")


def _get_restart_drain_timeout() -> float:
    """Return the configured gateway restart drain timeout in seconds."""
    raw = os.getenv("MOOR_RESTART_DRAIN_TIMEOUT", "").strip()
    if not raw:
        cfg = read_raw_config()
...
def _get_restart_after_turn_timeout() -> float:
    ...
    env_raw = os.getenv("MOOR_RESTART_AFTER_TURN_TIMEOUT")
...
def systemd_install(
    ...
    # Offer to remove legacy units (moor.service from pre-rename installs)
    # before installing the new moor-gateway.service. If both remain, they
    ...
    if has_legacy_moor_units():
        print()
        print_legacy_unit_warning()
        print()
        if non_interactive or prompt_yes_no("Remove the legacy unit(s) before installing?", True):
            remove_legacy_moor_units(interactive=False)
            print()

    unit_path = get_systemd_unit_path(system=system)
    scope_flag = " --system" if system else ""

    # Existing system units already pin MOOR_HOME; adopt it before any
    ...
    # ``sudo moor gateway install --system --force`` would bake /root/.moor
    ...
    print("Next steps:")
    print(
        f"  {'sudo ' if system else ''}moor gateway start{scope_flag}              # Start the service"
    )
    print(
        f"  {'sudo ' if system else ''}moor gateway status{scope_flag}             # Check status"
    )
    print(
...
def _require_service_installed(action: str, system: bool = False) -> None:
    ...
    if not unit_path.exists():
        ...
        print(f"  Run: {'sudo ' if system else ''}moor gateway install{scope_flag}")
        sys.exit(1)
...
def systemd_start(system: bool = False):
    ...
    # MOOR_HOME sync happens inside refresh_systemd_unit_if_needed's
...
def systemd_stop(system: bool = False):
    ...
    try:
        ...
        print(
            ...
            "check `moor gateway status` or logs for final shutdown state."
        )
        return
    ...
    if pid is not None:
        ...
        try:
            ...
            print(
                ...
                "check `moor gateway status` or logs for final state."
            )
            return
        _wait_for_systemd_service_restart(system=system, previous_pid=pid)
        return
...
def systemd_status(deep: bool = False, system: bool = False, full: bool = False):
    ...
    if not systemd_unit_is_current(system=system):
        ...
        print(
            f"  Run: {'sudo ' if system else ''}moor gateway restart{scope_flag}  # auto-refreshes the unit"
        )
        print()
    ...
    if status == "active":
        ...
        print(f"  Run: {'sudo ' if system else ''}moor gateway start{scope_flag}")
    ...
    if active_state == "activating" and sub_state == "auto-restart":
        ...
        print(
            f"  Run after the start-limit window expires: {'sudo ' if system else ''}moor gateway restart{scope_flag}"
        )
        print(
        ...
        print(
            f"  Run: systemctl {'--user ' if not system else ''}reset-failed {get_service_name()} && {'sudo ' if system else ''}moor gateway start{scope_flag}"
        )
...
# process invocation (each ``moor gateway start/stop/status`` call).
...
#      background process (the `nohup moor gateway run` workaround). See #23387.
...
def _launchd_reload_log_path() -> Path:
    ...
    return get_moor_home() / "logs" / "launchd-reload.log"
...
def _launchd_unsupported_marker_path() -> Path:
    return get_moor_home() / ".gateway-launchd-unsupported"
...
def _gateway_run_command() -> list[str]:
    """Build the `python -m moor_cli.main [--profile X] gateway run --replace` argv.

    Profile-aware: honors the active MOOR_HOME via `_profile_arg()` so the
    detached fallback launches into the same profile as the CLI invocation.
    """
    cmd = [get_python_path(), "-m", "moor_cli.main"]
...
def _timestamped_stderr_gateway_command(
    ...
    *,
    ...
    ``moor update`` sees the flag on the live grandchild argv and hands
...
def _spawn_detached_gateway() -> bool:
    ...
    macOS 26+ (issue #23387). Mirrors the `nohup moor gateway run --replace`
...
def _launchd_fallback_to_detached(reason: str, *, exit_on_failure: bool = True) -> bool:
    ...
    print(
        f"  Try manually: nohup moor gateway run --replace "
...
def generate_launchd_plist() -> str:
    ...
    # without this block every plist rewrite (e.g. `moor gateway start`)
...
def refresh_launchd_plist_if_needed() -> bool:
    ...
    if (
        ...
        # to ~/.moor/logs/launchd-reload.log, which the health watchdog
        # can tail to detect a persistent orphan. See moor-restart
        # rootcause handoff (2026-06-26 incident).
        reload_log_path = get_moor_home() / "logs" / "launchd-reload.log"
        try:
    ...
    print(
        "↻ Updated gateway launchd service definition to match the current Moor install"
    )
    return True
...
def launchd_stop():
    ...
    # `moor gateway start` re-bootstraps when it detects the job is unloaded.
    try:
...
def launchd_restart():
    ...
    try:
        ...
        if pid is not None and probe_gateway_loop_liveness(pid) == GATEWAY_LOOP_WEDGED:
            ...
            # full drain budget only stalls the restart (and `moor update`
...
def launchd_status(deep: bool = False):
    ...
    # Moor PID tracking — may be a detached fallback process spawned when
    ...
    # launchd PID and the Moor PID file).
    ...
    if deep:
        log_file = get_moor_home() / "logs" / "gateway.log"
        if log_file.exists():
            print()
            print("Recent logs:")
...
def _running_under_gateway_supervisor() -> bool:
    ...
    a reliable marker into the child's environment:
      ...
      - the s6-overlay container longrun exports ``MOOR_S6_SUPERVISED_CHILD``.
...
def _guard_named_profile_under_multiplexer(force: bool = False) -> None:
    ...
    named-profile ``moor gateway run`` is always a misconfiguration, so we
...
def _guard_supervised_gateway_conflict(force: bool = False) -> None:
    ...
    Running ``moor gateway run [--replace]`` (or the manual-restart fallback)
...
def _guard_existing_gateway_process_conflict(replace: bool = False) -> None:
    ...
    ``moor gateway run`` can burn memory/CPU just to fail with "already
    ...
    unrelated gateway processes from other MOOR_HOME roots.
    """
    ...
    print("  Use 'moor gateway restart' to replace it,")
    print("  or 'moor gateway stop' first.")
    print("  Or use 'moor gateway run --replace' to auto-replace.")
    sys.exit(1)
...
def _guard_official_docker_root_gateway() -> None:
    ...
    if not hasattr(os, "geteuid") or os.geteuid() != 0:
        return
    if _truthy_env(os.getenv("MOOR_ALLOW_ROOT_GATEWAY")):
        return
    if not _is_official_docker_checkout():
        return

    print_error(
        "Refusing to run the Moor gateway as root inside the official Docker image."
    )
    print(
        "  The image entrypoint normally drops privileges to the 'moor' user. "
        "If you override entrypoint in Docker Compose, include "
        "/opt/moor/docker/entrypoint.sh before the Moor command."
    )
    print(
        "  Running the gateway as root can leave root-owned files in "
        "$MOOR_HOME and break later non-root dashboard/gateway runs."
    )
    print(
        "  Set MOOR_ALLOW_ROOT_GATEWAY=1 only if you intentionally accept this risk."
    )
    sys.exit(1)
...
def run_gateway(verbose: int = 0, quiet: bool = False, replace: bool = False, force: bool = False):
    ...
    # from sibling CLI processes, but foreground `moor gateway run` still
    # needs to obey the banner's "Press Ctrl+C to stop" contract.
    # Service-style launchers set MOOR_GATEWAY_DETACHED=1; older wrappers
    ...
    _gateway_detached = (
        os.getenv("MOOR_GATEWAY_DETACHED", "").strip().lower()
        in {"1", "true", "yes", "on"}
    )
    ...
    # /restart) rather than through `moor gateway restart` which already
    ...
    # `moor gateway start/restart` — leaving the gateway vulnerable to
    ...
    # MOOR_GATEWAY_EXIT_DIAG env var (default: on while we're still
    ...
    def _exit_diag(tag: str, **extra: object) -> None:
        if os.environ.get("MOOR_GATEWAY_EXIT_DIAG", "1") != "1":
            return
        try:
    ...
    # the env vars ``MOOR_GATEWAY_MAX_STARTS`` /
    # ``MOOR_GATEWAY_START_WINDOW_S`` override for escape-hatch use.
    ...
    try:
        ...
        try:
            _env_starts = os.getenv("MOOR_GATEWAY_MAX_STARTS")
            if _env_starts is not None:
                _max_starts = int(_env_starts)
        except ValueError:
            pass
        try:
            _env_win = os.getenv("MOOR_GATEWAY_START_WINDOW_S")
    ...
    def _hard_exit_after_gateway_teardown(code: int) -> None:
        # ``moor gateway run`` enters through this CLI wrapper, not through
    ...
    # plugins/platforms/matrix/adapter.py::register(). #41112.
    {
        ...
        "vars": [
            ...
            },
            {
                ...
                "password": False,
                "help": "Channel ID where Moor delivers cron results and notifications.",
            },
            {
...
def _all_platforms() -> list[dict]:
    ...
    ``moor setup gateway`` without needing the gateway to be running.
    ...
    every host. Currently:
      - Matrix is hidden on Windows. The [matrix] extra pulls
        ...
        picker. Users who want Matrix on Windows can run moor under
        WSL.
    """
    ...
    # User-installed platform plugins under ~/.moor/plugins/ still require
...
def _platform_status(platform: dict) -> str:
    ...
    if token_var == "WHATSAPP_ENABLED":
        if val and val.lower() == "true":
            session_file = get_moor_home() / "whatsapp" / "session" / "creds.json"
        ...
        if var.get("is_allowlist"):
            ...
            if value:
                ...
                if access_idx == 0:
                    ...
                    print_info(
                        "  Approve with: moor pairing approve <platform> <code>"
                    )
                elif is_email:
                    print_success("  Unknown email senders will be ignored.")
                else:
                    print_info(
                        "  Skipped — configure later with 'moor gateway setup'"
                    )
            continue
...
def _setup_weixin():
    ...
    print()
    print_info("  1. Moor will open Tencent iLink QR login in this terminal.")
    ...
    if not check_weixin_requirements():
        ...
        print_info("  Install them, then rerun `moor gateway setup`.")
        return
    ...
    try:
        credentials = asyncio.run(qr_login(str(get_moor_home())))
    except KeyboardInterrupt:
        print()
...
def _setup_qqbot():
    ...
    if access_idx == 0:
        ...
        print_success("  DM pairing enabled.")
        print_info(
            "  Unknown users can request access; approve with `moor pairing approve`."
        )
    elif access_idx == 1:
...
def _setup_signal():
    ...
    if shutil.which("signal-cli"):
        ...
        print_info('    signal-cli link -n "MoorAgent"')
...
def _builtin_setup_fn(key: str):
    ...
    Late-bound to avoid a circular import with ``moor_cli.setup`` (which
...
def _configure_platform(platform: dict) -> None:
    ...
    is needed here. User-installed platform plugins under ~/.moor/plugins/
    ...
    if required:
        print_info(f"  Set these env vars in ~/.moor/.env: {', '.join(required)}")
    else:
        print_info(
...
def gateway_setup():
    ...
    if supports_systemd_services() and has_legacy_moor_units():
        print_legacy_unit_warning()
        print()
    ...
    if any_configured:
            ...
            if supports_systemd_services() and _system_scope_wizard_would_need_root():
                ...
                try:
                    if supports_systemd_services():
                        ...
                        print_info("Start manually: moor gateway")
                except UserSystemdUnavailableError as e:
            ...
            if supports_systemd_services() or is_macos() or is_windows():
                ...
                if start_now or start_on_login:
                    try:
                        ...
                        print_info("  You can try manually: moor gateway install")
                else:
                    print_info("  Skipped start and auto-start setup.")
                    print_info("  You can install later: moor gateway install")
                    if supports_systemd_services():
                        print_info(
                            "  Or as a boot-time service: sudo moor gateway install --system"
                        )
                    print_info("  Or run in foreground:  moor gateway run")
            elif is_wsl():
                print_info("  WSL detected but systemd is not running.")
                print_info("  Run in foreground: moor gateway run")
                print_info(
                ...
                print_info("  Run in foreground: moor gateway run")
                print_info(
                    f"  Or start it manually in the background (best effort): nohup moor gateway run >{_dhh()}/logs/gateway.log 2>&1 &"
                )
            else:
        ...
        print()
        print_info("No platforms configured. Run 'moor gateway setup' when ready.")
...
def _dispatch_via_service_manager_if_s6(
    ...
    :mod:`moor_cli.service_manager` are caught and surfaced as
    ...
    if profile is None:
        ...
        # MOOR_HOME=<root>/profiles/<name>, "" for the default root,
...
def _dispatch_all_via_service_manager_if_s6(action: str) -> bool:
    ...
    Without this, ``moor gateway stop --all`` and ``... restart --all``
...
def gateway_command(args):
    ...
    try:
        ...
        # The direct ``moor gateway install|uninstall|start|stop|restart``
...
def _maybe_redirect_run_to_s6_supervision(args) -> bool:
    ...
    Three gates make this a no-op outside the intended scope:
      ...
      1. ``_dispatch_via_service_manager_if_s6`` returns False unless
         ...
         ``moor gateway run`` are unaffected.
      2. ``MOOR_S6_SUPERVISED_CHILD`` is exported by
         ``S6ServiceManager._render_run_script`` for the supervised
         process itself — i.e. when s6-supervise execs ``moor gateway
         ...
         → ...).
      3. ``--no-supervise`` (or ``MOOR_GATEWAY_NO_SUPERVISE=1``) opts
    ...
    no_supervise = getattr(args, "no_supervise", False) or \
        os.environ.get("MOOR_GATEWAY_NO_SUPERVISE", "").lower() in ("1", "true", "yes")
    if no_supervise:
        return False
    if os.environ.get("MOOR_S6_SUPERVISED_CHILD"):
    ...
    # `docker logs` and ${MOOR_HOME}/logs/gateways/<profile>/current,
    ...
    print(
        ...
        "  dashboard supervised alongside if MOOR_DASHBOARD is set).\n"
        "  This is the recommended setup for the s6 container image — the\n"
        "  gateway will keep running even if it crashes.\n"
        "  Use `--no-supervise` (or MOOR_GATEWAY_NO_SUPERVISE=1) to opt out\n"
    ...
    #
    # Prefer `sleep infinity` (matches the static main-moor service's
    # pattern in docker/s6-rc.d/main-moor/run, and frees the Python
...
def _gateway_command_inner(args):
    ...
    if subcmd == "install":
        ...
        if is_termux():
            ...
            print("Run manually: moor gateway")
            sys.exit(1)
        if supports_systemd_services():
            if is_wsl():
                print_warning(
                ...
                print_info(
                    "  Consider running in foreground instead: moor gateway run"
                )
                print_info(
                    "  Or use tmux/screen for persistence: tmux new -s moor 'moor gateway run'"
                )
                print()
            ...
            print()
            print(
                "  moor gateway run                              # direct foreground"
            )
            print(
                "  tmux new -s moor 'moor gateway run'         # persistent via tmux"
            )
            print(
                "  nohup moor gateway run > ~/.moor/logs/gateway.log 2>&1 &  # background"
            )
            sys.exit(1)
        elif is_container():
            ...
            if detect_service_manager() == "s6":
                ...
                print()
                print("  moor profile create <name>     # creates the s6 service slot")
                print("  moor -p <name> gateway start   # bring it up via s6")
                print("  moor status                    # see currently-supervised gateways")
                return
            ...
            print()
            print("To run the gateway: moor gateway run")
            sys.exit(0)
        else:
            print("Service installation not supported on this platform.")
            print("Run manually: moor gateway run")
            sys.exit(1)
        ...
        if _is_supervised_gateway_process():
            print_error(
                ...
                "Use `moor gateway uninstall` from a shell outside the running gateway."
            )
            sys.exit(1)
        ...
        if is_termux():
            ...
            print("Stop manual runs with: moor gateway stop")
            sys.exit(1)
        if supports_systemd_services():
            ...
            if detect_service_manager() == "s6":
                ...
                print()
                print("  moor profile delete <name>     # tears down the s6 service slot")
                print("  moor -p <name> gateway stop    # stop without deleting the profile")
                return
        ...
        # slot — start them individually via `moor -p <name> gateway
        ...
        if supports_systemd_services():
            ...
            print()
            print("Or run the gateway directly: moor gateway run")
            sys.exit(0)
        else:
            print("Not supported on this platform.")
            sys.exit(1)
        ...
        # The supervised probe also PASSES a plain foreground `moor gateway run`
        ...
        if _is_supervised_gateway_process():
            print_error(
                ...
                "Use `moor gateway stop` from a shell outside the running gateway."
            )
            sys.exit(1)
        ...
        if _is_supervised_gateway_process():
            print_error(
                ...
                "This command was blocked to prevent restart loops.\n"
                "Use `moor gateway restart` from a shell outside the running gateway."
            )
            sys.exit(1)
        ...
        if not service_available:
            ...
            if service_configured:
                ...
                print("  Fix the service, then retry: moor gateway start")
                sys.exit(1)
        ...
        if supports_systemd_services() and (
            ...
            if pids:
                ...
                if is_termux():
                    ...
                    print("  moor gateway install")
                    print("  sudo moor gateway install --system")
            else:
                ...
                print()
                print("To start:")
                print("  moor gateway run      # Run in foreground")
                if is_termux():
                    print(
                        "  nohup moor gateway run > ~/.moor/logs/gateway.log 2>&1 &  # Best-effort background start"
                    )
                elif is_wsl():
                    print(
                        "  tmux new -s moor 'moor gateway run'         # persistent via tmux"
                    ...
                    print(
                        "  moor gateway install  # Install as Windows Scheduled Task (auto-start on login)"
                    )
                else:
                    print("  moor gateway install  # Install as user service")
                    print(
                        "  sudo moor gateway install --system  # Install as boot-time system service"
                    )
        ...
        # Stop, disable, and remove legacy Moor gateway unit files from
        # pre-rename installs (e.g. moor.service). Profile units and
        # unrelated third-party services are never touched.
        ...
        if not supports_systemd_services() and not is_macos():
            ...
            return
        remove_legacy_moor_units(interactive=not yes, dry_run=dry_run)
