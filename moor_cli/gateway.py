...
def _get_service_pids(all_profiles: bool = False) -> set:
    ...
    ``all_profiles`` widens the current profile's unit/label to the whole ``moor-gateway*`` /
    ``ai.moor.gateway*`` fleet so update/reaper never kill a sibling's service gateway as "manual".

    ``all_profiles`` widens the launchd branch to every installed ``ai.moor.gateway*`` LaunchAgent — the
    ...
    the ``moor-gateway*`` fleet glob.
    """
    pids: set = set()

    # --- systemd (Linux): user and system scopes ---
    if supports_systemd_services():
        pattern = "moor-gateway*" if all_profiles else get_service_name()
        for scope_args in [["systemctl", "--user"], ["systemctl"]]:
            try:
                # Belt-and-suspenders for the EXCLUDE use case (#74075): a bare ``launchctl list`` prefix
                # scan also catches ai.moor.gateway* agents the label derivation can't map (renamed
    ...
    if is_macos():
        ...
        if all_profiles:
            # Whole fleet, mirroring the systemd ``moor-gateway*`` glob above.
            # Every gateway LaunchAgent, not just the invoking profile's — mirrors the systemd branch's
            # ``moor-gateway*`` pattern above. The update path restarts the whole fleet, and its
        ...
        for label in sorted(labels):
            ...
            if pid is not None and pid > 0:
                pids.add(pid)
        if all_profiles:
            # Prefix scan also catches ai.moor.gateway* agents the label derivation can't map
            ...
            try:
                ...
                if result.returncode == 0:
                    for line in result.stdout.strip().splitlines():
                        parts = line.split()
                        if len(parts) >= 3 and parts[-1].startswith("ai.moor.gateway"):
                            try:
                                pid = int(parts[0])
                                if pid > 0:
                                    pids.add(pid)
...
# its full budget and `moor update` can deadlock. Two witnesses classify the loop BEFORE any
...
# fail", and `moor update` can deadlock behind it. The loop publishes a liveness signal precisely for this
...
def _get_ancestor_pids() -> set[int]:
    """PIDs of this process and its ancestors, so scans never count the invoking ``moor`` CLI as a gateway.

    Walks from the current PID up to PID 1 (init) so that process-table scans never match the calling CLI
    process or any of its parents. This prevents ``moor gateway status`` from falsely counting the
    ``moor`` CLI that invoked it as a running gateway instance (see #13242).
    """
...
def _scan_gateway_pids(
    ...
    # Exclude the entire ancestor chain so the CLI process that invoked this scan (e.g. ``moor gateway
    ...
    # Forward slashes on both sides of the MOOR_HOME= match (mirrors gateway.status).
    ...
    def _matches_current_profile(command: str) -> bool:
        ...
        # fallback stop could SIGTERM the named gateway). MOOR_HOME may come via env (invisible to
        # wmic/CIM), so only a non-matching explicit MOOR_HOME= disqualifies.
        if profile_flag_value(command_lc) is not None:
            return False
        return not ("moor_home=" in command_lc and f"moor_home={current_home_lc}" not in command_lc)
...
def _windows_process_listing() -> str | None:
    ...
    # failure or timeout (result is None) trips the fallback. ``moor update`` hung exactly there on
...
def find_gateway_pids(exclude_pids: set | None = None, all_profiles: bool = False) -> list:
    """Find running gateway PIDs for the current profile, or every profile with ``all_profiles`` (``moor update``)."""
...
def find_windows_gateway_services(
    *, psutil_module=None, profile_processes: list[ProfileGatewayProcess] | None = None
    ...
    command lines, so identity = Moor's own PID file + a parent chain ending at a running SCM service
...
def _prepare_profile_gateway_update_restart(profile: str, pid: int) -> str | None:
    """Choose who relaunches a profile gateway after ``moor update``: ``--external-supervisor`` gateways
...
def _spawn_gateway_restart_watcher(old_pid: int, run_argv: list[str]) -> bool:
    ...
    if old_pid <= 0 or not run_argv:
        return False
    from moor_cli._subprocess_compat import windows_detach_flags_without_breakaway, windows_detach_popen_kwargs
    ...
    # normalizes the interpreter and captures a stable cwd + env overlay (MOOR_HOME,
    ...
    if sys.platform == "win32":
        try:
            from moor_cli.gateway_windows import windowless_gateway_restart_spec
    ...
    watcher = textwrap.dedent(
        ...
        try:
            ...
            from pathlib import Path
            _log_dir = Path(get_moor_home()) / "logs"
        ...
        # MOOR_HOME) the windowless base interpreter needs to import moor_cli. Empty on POSIX.
        if _respawn_cwd:
...
def _sync_moor_home_from_systemd_unit(system: bool) -> None:
    """Adopt a system-scope unit's ``MOOR_HOME``: under ``sudo`` it is stripped and HOME=/root, so
    get_moor_home() would pick the wrong profile for runtime-status/PID reads."""
    if not system:
        return
    ...
    if not unit_home:
        ...
        unit_home = _parse_kv_pairs(env_line.split()).get("MOOR_HOME", "").strip()
    if unit_home and os.environ.get("MOOR_HOME", "").strip() != unit_home:
...
def _wait_for_systemd_service_restart(
    *,
    ...
    print(
        ...
        f"  Check status: {sudo}moor gateway status\n"
...
def _print_systemd_start_limit_wait(system: bool = False) -> None:
    ...
    print(f"  Wait for the start-limit window to expire, then run: {sudo}moor gateway restart{scope_flag}")
...
def _print_served_ingress_urls(profile: str | None = None) -> None:
    ...
    try:
        from moor_cli.gateway_multiplex_served import format_ingress_url_lines, served_profile_ingress_urls
...
def _print_unserved_shared_ingress(profile: str | None) -> None:
    ...
    try:
        from moor_cli.gateway_multiplex_served import served_profile_unserved_platforms
...
def _print_other_profiles_gateway_status() -> None:
    """Print other profiles' running gateways at the bottom of ``moor gateway status``."""
    try:
...
def _reap_unsupervised_gateway_orphans(extra_exclude: set | None = None) -> bool:
    ...
    if is_windows():
        try:
            from moor_cli.gateway_windows import get_task_name  # profile-aware task name
...
def _reaper_exclusion_pids(extra_exclude: set | None) -> set[int]:
    ...
    with contextlib.suppress(Exception):
        ...
        # cover the whole ai.moor.gateway* fleet — not just the current profile's label — or a sibling
...
def _gw_windows():
    """Lazily import :mod:`moor_cli.gateway_windows` (Windows-only service backend)."""
...
def _gateway_detached_env() -> bool:
    return _truthy_env(os.getenv("MOOR_GATEWAY_DETACHED"))
...
def _windows_gateway_should_absorb_console_controls() -> bool:
    """True for detached Windows gateway runs that should ignore Ctrl+C (``MOOR_GATEWAY_DETACHED=1``
...
def _native_service_homes() -> set[Path]:
    ...
    from moor_constants import _get_platform_default_moor_home, sudo_invoker_default_home

    homes = {_get_platform_default_moor_home().resolve()}
...
def _bare_unit_pinned_home() -> Path | None:
    """Resolved ``MOOR_HOME`` pinned by an installed ``moor-gateway.service``, or None. The unit is the
    ...
    if not is_linux() or os.geteuid() != 0:  # windows-footgun: ok — behind is_linux()
        return None
    pinned = _moor_home_pinned_by_unit(_SYSTEM_UNIT_DIR / f"{_SERVICE_BASE}.service")
    if not pinned:
        return None
    try:
...
def _profile_suffix() -> str:
    """Service-name suffix for MOOR_HOME: "" for a home that owns the bare name, the profile name for
    ``<root>/profiles/<name>``, else a short hash of the path.

    Bare-name owners: this process's platform-native default (``~/.moor``), under sudo the invoking
    user's native default, and the home pinned by an installed ``moor-gateway.service``. Under sudo the
    naming basis moves MID-COMMAND — sudo strips MOOR_HOME and sets HOME=/root, then
    ``_sync_moor_home_from_systemd_unit()`` adopts the unit's own MOOR_HOME into ``os.environ`` — so a
    basis derived from the process alone names one unit before the adoption and another after it. The
    unit-pinned check must precede the profile branch: ``sudo moor gateway install --system`` resolves
    ...
    The bare name is deliberately NOT tied to ``get_default_moor_root()``: that helper treats any
    MOOR_HOME outside ``~/.moor`` (Docker ``/opt/data``, a temp dir) as "the root itself", which let a
    temp-home harness resolve to the default profile's ``moor-gateway`` unit and uninstall the
    ...
    if home in _native_service_homes() or home == _bare_unit_pinned_home():
        return ""
    name = _profile_name_from_home(home, get_default_moor_root().resolve())
...
def _current_profile_name() -> str:
    ...
    return profile_name_for_home(get_moor_home()) or _profile_suffix()


def _profile_arg(moor_home: str | None = None, default_root: str | Path | None = None) -> str:
    """``--profile <name>`` for ``<root>/profiles/<name>``, else "". *moor_home*/*default_root* let a
...
def get_service_name() -> str:
    """Systemd service name: ``moor-gateway`` for default MOOR_HOME, ``moor-gateway-<profile>``
    or ``-<hash>`` otherwise."""
...
def _find_legacy_moor_units() -> list[tuple[str, Path, bool]]:
    """``[(unit_name, unit_path, is_system)]`` for legacy gateway units (e.g. ``moor.service``), which
    ...
    Detects unit files installed by older Moor versions that used a different service name (e.g. When both
    a legacy unit and the current ``moor-gateway.service`` are active, they fight over the same bot token
...
def remove_legacy_moor_units(interactive: bool = True, dry_run: bool = False) -> tuple[int, list[Path]]:
...
def ensure_gateway_service(context: str = "setup") -> bool:
    """Install and start a user-scope gateway service without prompting (``moor setup``/``import``).
...
def get_launchd_plist_path() -> Path:
    """``~/Library/LaunchAgents/ai.moor.gateway[-<profile>].plist`` under the real account home."""
    import pwd
    suffix = _profile_suffix()
...
def launchd_gateway_labels_for_install() -> list[str]:
    ...
    the profile layout, NOT by globbing ``~/Library/LaunchAgents``, so a sandboxed MOOR_HOME never
...
def get_python_path() -> str:
    ...
    if venv is not None:
        try:
            ...
            # Update-boundary: a gateway restarted mid-update can hold a stale moor_constants
            # without this symbol; see _reload_moor_constants() in moor_cli/managed_uv.py.
...
def _moor_home_for_target_user(target_home_dir: str) -> str:
    ...
    try:
        ...
        return str(current_moor)  # Completely custom path (not under ~/.moor) — keep as-is
...
def _build_service_path_dirs(project_root: Path | None = None) -> list[str]:
    ...
    extras = (project_root / "node_modules" / ".bin", moor_home / "node" / "bin", moor_home / "node_modules" / ".bin")
    for extra in extras:
        if _is_dir(extra):
...
def _stable_service_working_dir() -> str:
    """WorkingDirectory that won't disappear under systemd (MOOR_HOME, else PROJECT_ROOT). cwd is
...
def _append_node_dir_for_service(path_entries: list[str], moor_root: Path | None = None) -> None:
    """Append the Node dir a service unit should use: managed ``<moor_root>/node`` (profile-scoped)
...
def generate_systemd_unit(system: bool = False, run_as_user: str | None = None) -> str:
    ...
    if system:
        ...
        # Profile arg relative to the TARGET user's ~/.moor when moor_home lives under it.
        target_root = Path(home_dir) / ".moor"
        try:
            Path(moor_home).resolve().relative_to(target_root.resolve())
            profile_arg = _profile_arg(moor_home, default_root=target_root)
        except ValueError:
        ...
        _append_node_dir_for_service(_target_node_entries, Path(moor_home) if moor_home else None)
    ...
    watchdog_seconds = _systemd_watchdog_seconds(moor_home)
...
[Service]
Type={systemd_type}
{systemd_watchdog_directives}{identity_lines}ExecStart={python_path} -m moor_cli.main{f" {profile_arg}" if profile_arg else ""} gateway run
...
def _normalize_launchd_plist_for_comparison(text: str) -> str:
    ...
    import re
    return re.sub(
        r"(<key>PATH</key>\s*<string>)(.*?)(</string>)", r"\1__MOOR_PATH__\3",
        _normalize_service_definition(text), flags=re.S,
    )


def systemd_unit_is_current(system: bool = False) -> bool:
    # MOOR_HOME sync chokepoint for every compare/regenerate path: under `sudo … --system` it is often
    # stripped to /root/.moor, so refresh would rewrite a correct unit and status warn forever.
...
def _temp_home_in_service_definition(definition: str) -> str | None:
    """Temp-dir MOOR_HOME baked into a systemd unit / launchd plist, or None. A temp home means a
...
def _refuse_temp_home_service_write(definition: str, kind: str) -> bool:
    ...
    if temp_home is None:
        return False
    print(f"✗ Refusing to write the gateway {kind}: MOOR_HOME resolves to a temporary directory ({temp_home}).")
    print(
...
def refresh_systemd_unit_if_needed(system: bool = False) -> bool:
    ...
    # pytest-tmp MOOR_HOME baked into the developer's real unit breaks their gateway on next reboot.
    if not system and any(m in new_unit for m in ("/pytest-of-", '/moor_test"', "/moor_test/")):
        return False

    # Structural variant: refuse ANY temp-dir MOOR_HOME (manual E2E homes lack the pytest markers).
    if _refuse_temp_home_service_write(new_unit, "systemd unit"):
        return False

    unit_path.write_text(new_unit, encoding="utf-8")
    _run_systemctl(["daemon-reload"], system=system, check=True, timeout=30)
    print(f"↻ Updated gateway {_service_scope_label(system)} service definition to match the current Moor install")
    return True
...
def _get_cron_drain_timeout() -> float:
    ...
    See #82161.
    """
    return _agent_timeout_setting("MOOR_CRON_DRAIN_TIMEOUT", "cron_drain_timeout", parse_cron_drain_timeout)
...
def _get_restart_exit_wait_budget() -> float:
    ...
    return resolve_restart_exit_wait_budget(
        ...
        _agent_timeout_setting(
            "MOOR_RESTART_AFTER_TURN_TIMEOUT", "restart_after_turn_timeout", parse_restart_after_turn_timeout
        ),
    )
...
def systemd_install(
    ...
    # Existing system units already pin MOOR_HOME; adopt it before any regenerate.
    if unit_path.exists():
        _sync_moor_home_from_systemd_unit(system=system)
    ...
    print()
    print("Next steps:")
    print(f"  {sudo}moor gateway start{scope_flag}              # Start the service")
    print(f"  {sudo}moor gateway status{scope_flag}             # Check status")
...
def _systemd_unit_belongs_to_current_home(system: bool = False) -> bool:
    """False (with a warning) when the installed unit pins a MOOR_HOME other than this process's: the
    service name then resolved to ANOTHER install's gateway, and stop/disable/unlink would take it down."""
    _sync_moor_home_from_systemd_unit(system=system)  # sudo strips MOOR_HOME; adopt the unit's first
    unit_home = _moor_home_from_systemd_unit_file(system=system)
    if unit_home is None or Path(unit_home).expanduser().resolve() == get_moor_home().resolve():
        return True
    print_warning(
        f"Refusing to remove {get_systemd_unit_path(system=system)}: it runs MOOR_HOME={unit_home}, "
        f"but this process has MOOR_HOME={get_moor_home()}"
    )
    return False
...
def _print_service_not_installed(system: bool) -> None:
    ...
    print(f"  Run: {sudo}moor gateway install{scope_flag}")
...
def systemd_start(system: bool = False):
    ...
    # MOOR_HOME sync happens in refresh's systemd_unit_is_current gate; the unit is guaranteed to exist here.
    refresh_systemd_unit_if_needed(system=system)
...
def systemd_restart(system: bool = False):
    ...
    # MOOR_HOME sync happens in refresh's systemd_unit_is_current gate; its os.environ mutation
    ...
    if pid is not None and probe_gateway_loop_liveness(pid) == GATEWAY_LOOP_WEDGED:
        ...
        print(
            ...
            # graceful shutdown, so waiting the full drain budget only stalls the restart (and `moor
...
def systemd_status(deep: bool = False, system: bool = False, full: bool = False):
    ...
    if not systemd_unit_is_current(system=system):
        ...
        print(f"  Run: {sudo}moor gateway restart{scope_flag}  # auto-refreshes the unit")
        print()
    ...
    if result.stdout.strip() == "active":
        ...
        print(f"  Run: {sudo}moor gateway start{scope_flag}")
    ...
    if active_state == "activating" and unit_props.get("SubState", "") == "auto-restart":
        ...
        print(f"  Run after the start-limit window expires: {sudo}moor gateway restart{scope_flag}")
        ...
        print(f"  Run: systemctl {user_flag}reset-failed {svc} && {sudo}moor gateway start{scope_flag}")
...
def _gateway_run_command() -> list[str]:
    """Build ``python -m moor_cli.main [--profile X] gateway run --replace``, honoring the active profile."""
    return [get_python_path(), "-m", "moor_cli.main", *_profile_arg().split(), "gateway", "run", "--replace"]
...
def _timestamped_stderr_gateway_command(error_log: Path, *, external_supervisor: bool = False) -> list[str]:
    ...
    return [get_python_path(), "-m", "moor_cli.stderr_timestamp", "--error-log", str(error_log), "--", *inner]
...
def _wait_for_gateway_exit(timeout: float = 10.0, force_after: float | None = 5.0) -> bool:
    ...
    MOOR_HOMEs work) to exit; SIGKILL it after ``force_after`` s of graceful waiting."""
...
def _is_official_docker_checkout() -> bool:
    return str(PROJECT_ROOT) == "/opt/moor" and (PROJECT_ROOT / "docker" / "entrypoint.sh").is_file()
...
def _running_under_gateway_supervisor() -> bool:
    ...
    (shells inherit "0"), s6 MOOR_S6_SUPERVISED_CHILD, or ``--external-supervisor``."""
...
def named_profile_served_by_running_multiplexer(profile_name: str | None = None) -> bool:
    ...
    try:
        ...
        # default profile (`moor -p X` loads X's .env) and a config edit after start is not live yet.
...
def _served_profile_needs_no_service() -> bool:
    ...
    Shared by ``moor setup gateway`` / ``moor setup`` / ``moor import`` (``ensure_gateway_service``)
...
def _guard_existing_gateway_process_conflict(replace: bool = False) -> None:
    ...
    "already running". Same user-facing contract; never scans other MOOR_HOME roots."""
    ...
    try:
        ...
        return
    if pid is None:
        # get_running_pid() filters by the current profile's MOOR_HOME; warn if the PID file
        ...
        try:
            ...
            if stale is not None and not _pid_record_belongs_to_current_profile(stale):
                logger.warning(
                    "PID file belongs to another profile (moor_home=%s). "
                    "The old gateway may still be running under that profile.",
                    stale.get("moor_home", "<unknown>"),
                )
        except Exception:
            pass
        return
    ...
    print("  Check with `moor gateway status`.")
    print("  To restart it: `moor gateway restart`. To stop it: `moor gateway stop`.")
    print("  To replace it from here: `moor gateway run --replace`.")
    sys.exit(1)


def _guard_official_docker_root_gateway() -> None:
    """Refuse gateway startup when the official Docker privilege drop was bypassed."""
    if not hasattr(os, "geteuid") or os.geteuid() != 0 or _truthy_env(os.getenv("MOOR_ALLOW_ROOT_GATEWAY")):
        return
    if not _is_official_docker_checkout():
        return

    print_error("Refusing to run the Moor gateway as root inside the official Docker image.")
    print(
    ...
    print("  Set MOOR_ALLOW_ROOT_GATEWAY=1 only if you intentionally accept this risk.")
    sys.exit(1)
...
def _apply_startup_watchdog_config() -> None:
    ...
    try:
        from moor_startup_watchdog import (
        ...
        try:
            from moor_cli.config import load_config as _sw_load_config
...
def _make_exit_diag():
    ...
    ``asyncio.run()`` can return, for chasing silent Windows gateway deaths. MOOR_GATEWAY_EXIT_DIAG=0 opts out."""
...
def _respawn_storm_backoff() -> None:
    ...
    DEFAULT_CONFIG ``gateway.respawn_storm``; MOOR_GATEWAY_MAX_STARTS / MOOR_GATEWAY_START_WINDOW_S
    ...
    try:
        ...
        try:
            ...
            pass
        try:
            _max_starts = int(os.environ["MOOR_GATEWAY_MAX_STARTS"])
        except (KeyError, ValueError):
            pass
        try:
            _win = float(os.environ["MOOR_GATEWAY_START_WINDOW_S"])
        except (KeyError, ValueError):
            pass
        ...
        if _storm is not None:
            ...
            try:
                from moor_startup_watchdog import kick_startup_watchdog
...
def run_gateway(verbose: int = 0, quiet: bool = False, replace: bool = False, force: bool = False):
    ...
    # Detached Windows runs (MOOR_GATEWAY_DETACHED=1, or non-TTY for older wrappers) ignore
    ...
    # exit-code-75 respawn (stale-code or /restart), which bypasses `moor gateway restart`.
    if supports_systemd_services():
        try:
...
def _runtime_health_lines() -> list[str]:
    ...
    if gateway_state == "startup_failed" and exit_reason:
        ...
        if isinstance(work, list) and work:
            from moor_cli.update_cmd_drain_report import describe_active_work_unit
...
def _prompt_unauthorized_access(platform_key: str) -> None:
    ...
    if access_idx == 0:
        ...
        print_info("  Approve with: moor pairing approve <platform> <code>")
    elif access_idx == 2:
        ...
        print_info("  Skipped — configure later with 'moor gateway setup'")
...
def _setup_qqbot():
    ...
    if access_idx == 0:
        ...
        print_info("  Unknown users can request access; approve with `moor pairing approve`.")
    elif access_idx == 1:
...
def _wizard_install_service(backend: str) -> None:
    ...
    if not (start_now or start_on_login):
        ...
        if supports_systemd_services():
            print_info("  Or as a boot-time service: sudo moor gateway install --system")
...
def gateway_command(args):
    ...
    try:
        ...
        from moor_cli.gateway_command_errors import explain_service_failure
...
def _maybe_redirect_run_to_s6_supervision(args) -> bool:
    ...
    ``MOOR_S6_SUPERVISED_CHILD`` (set by ``S6ServiceManager._render_run_script``) marks the supervised
    child, which must run in foreground or we'd recurse run → start → run; ``--no-supervise`` /
    MOOR_GATEWAY_NO_SUPERVISE=1 opts out (CI smoke, debugging)."""
    no_supervise = getattr(args, "no_supervise", False) or \
        os.environ.get("MOOR_GATEWAY_NO_SUPERVISE", "").lower() in ("1", "true", "yes")
    # MOOR_S6_SUPERVISED_CHILD: we ARE the supervised child; fall through so the gateway starts.
    if no_supervise or os.environ.get("MOOR_S6_SUPERVISED_CHILD"):
        return False
    ...
    try:
        ...
        # down the supervised gateway cleanly). Prefer `sleep infinity` (matches the static main-moor
        # service's pattern in docker/s6-rc.d/main-moor/run, and frees the Python interpreter — the
...
def _refuse_from_inside_gateway(verb: str, reason: str) -> None:
    ...
    if _is_supervised_gateway_process():
        print_error(
            ...
            f"Use `moor gateway {verb}` from a shell outside the running gateway."
        )
        sys.exit(1)
...
_NO_BACKEND_MESSAGES = {
    ("install", "termux"): (1,
        "Gateway service installation is not supported on Termux.", "Run manually: moor gateway"),
    ("install", "wsl"): (1,
    ...
    ("install", "s6"): (None,
        ...
        "  moor profile create <name>     # creates the s6 service slot",
        "  moor -p <name> gateway start   # bring it up via s6",
        "  moor status                    # see currently-supervised gateways"),
    ("install", "container"): (0,
        ...
        "To run the gateway: moor gateway run"),
    ("install", "unsupported"): (1,
        "Service installation not supported on this platform.", "Run manually: moor gateway run"),
    ("uninstall", "termux"): (1,
        "Gateway service uninstall is not supported on Termux because there is no managed service to remove.",
        "Stop manual runs with: moor gateway stop"),
    ("uninstall", "s6"): (None,
        "Per-profile gateways are auto-unregistered when you delete the profile.", "",
        "  moor profile delete <name>     # tears down the s6 service slot",
        "  moor -p <name> gateway stop    # stop without deleting the profile"),
    ...
    ("start", "container"): (0,
        ...
        "Or run the gateway directly: moor gateway run"),
    ("start", "unsupported"): (1,
        "Running the gateway as a background service is not available on this platform "
        "(no systemd, launchd or Scheduled Tasks).",
        "Run it directly with: moor gateway run"),
}
...
def _cmd_stop(args):
    ...
    if not stop_all and not find_gateway_pids() and named_profile_served_by_running_multiplexer():
        ...
        print()
        print("    moor gateway stop      # takes every served profile offline")
        print("    moor gateway restart")
        sys.exit(GATEWAY_FATAL_CONFIG_EXIT_CODE)
...
def _cmd_restart(args):
    ...
    # detached run below is the restart; bailing here left `moor gateway restart` a
    ...
    if kind == "systemd" and supports_systemd_services():
        ...
        if linger_ok is not True:
            ...
            _print_lines(
                ...
                "  Then restart the gateway:", "    moor gateway restart",
            )
            return
    ...
    if service_configured:
        _print_lines(
            ...
            "  Fix the service, then retry: moor gateway start",
        )
        sys.exit(1)
    ...
    from gateway.status import get_running_pid
    from moor_cli.gateway_supervised_restart import (
...
# ``moor gateway status`` hints for a manually-run / stopped gateway, keyed by host kind.
_STATUS_RUNNING_HINTS = {
    ...
    "windows": ("To install as a Windows Scheduled Task (auto-start on login):", "  moor gateway install"),
    "other": (
        "To install as a service:", "  moor gateway install", "  sudo moor gateway install --system",
    ),
}
_STATUS_STOPPED_HINTS = {
    ...
    "windows": ("  moor gateway install  # Install as Windows Scheduled Task (auto-start on login)",),
    "other": (
        "  moor gateway install  # Install as user service",
...
def _cmd_status(args):
    ...
    if not snapshot.running and named_profile_served_by_running_multiplexer():
        ...
        print("  Manage it from the default profile: moor gateway status")
...
def _cmd_migrate_legacy(args):
    """Stop, disable, and remove legacy Moor gateway unit files (e.g. moor.service)."""
...
def _cmd_migrate(args):
    from moor_cli.gateway_migrate import cmd_migrate
    cmd_migrate(args)
