"""Configuration management for Moor Agent: config.yaml / .env loading, saving,
validation, migration, and the ``moor config`` command."""
...
_PARSE_FAILURE_FALLBACK_MSG = {
    ...
    "refuse-write": (
        ...
        "Fix the YAML (moor config edit) and retry.")}
...
# interpreter init (PYTHON*, NODE_*: Moor restarts through them), PATH (fix tool lookup
...
# and Moor runtime-location / security-policy flags (config.yaml is the supported surface).
#
# ``MOOR_*`` overall is NOT blocked — many integration credentials use that prefix
# (MOOR_LANGFUSE_PUBLIC_KEY, MOOR_SPOTIFY_CLIENT_ID, ...). The denylist is name-by-name so
...
_ENV_VAR_NAME_DENYLIST: frozenset[str] = frozenset({
    ...
    "MOOR_COPILOT_ACP_COMMAND", "MOOR_COPILOT_ACP_ARGS",
    # Security policy / approval-routing context — set via their dedicated controls only.
    "MOOR_YOLO_MODE", "MOOR_ACCEPT_HOOKS", "MOOR_REDACT_SECRETS",
    "MOOR_INTERACTIVE", "MOOR_EXEC_ASK", "MOOR_GATEWAY_SESSION",
    "MOOR_CRON_SESSION", "MOOR_SINGLE_QUERY_SESSION",
    "MOOR_SESSION_KEY", "MOOR_SESSION_PLATFORM"})
...
def validate_env_var_name_for_write(key: str) -> None:
    ...
    if _env_var_policy_name(key) in _ENV_VAR_NAME_DENYLIST:
        raise ValueError(
            ...
            "or Moor runtime location and security policy (MOOR_HOME, MOOR_YOLO_MODE, ...) "
            "cannot be persisted via the env writer. If you really need this, edit ~/.moor/.env "
            "directly.")
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    # back-compat fallback. The boolean MOOR_TOOL_PROGRESS variant is unsupported (its only
    ...
    "MOOR_LANGFUSE_ENV", "MOOR_LANGFUSE_RELEASE", "MOOR_LANGFUSE_SAMPLE_RATE",
    "MOOR_LANGFUSE_MAX_CHARS", "MOOR_LANGFUSE_CAPTURE", "MOOR_LANGFUSE_DEBUG",
    "LANGFUSE_PUBLIC_KEY", "LANGFUSE_SECRET_KEY", "LANGFUSE_BASE_URL",
    # ACP (Agent Client Protocol) keys — profile-isolable so profiles can use different backends.
    "MOOR_ACP_AUTH_METHOD", "MOOR_ACP_AUTO_APPROVE", "MOOR_COPILOT_ACP_COMMAND",
    "MOOR_COPILOT_ACP_ARGS", "COPILOT_CLI_PATH", "COPILOT_ACP_BASE_URL"})
...
# MOOR_MANAGED). Module-level so tests can patch it without touching /nix/store.
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    """Detect how Moor was installed: apt/docker/nix/nixos/home-manager/git/unknown.
    Order: code-scoped ``<install tree>/.install_method`` stamp (authoritative) -> legacy
    ``$MOOR_HOME/.install_method`` -> managed marker -> /nix/store path -> .git dir -> unknown.
    The stamp lives next to the code because MOOR_HOME is shared data: a container and a host
    install can bind-mount the same home, so a home-scoped ``docker`` stamp would make the host
    ``moor update`` refuse to run. A legacy ``docker`` value is therefore ignored unless we are
    ...
    ``/opt/moor`` at build time. An unsupported manual install dropped into a container (no stamp) falls
    through to the ``.git`` checks and behaves like any off-path install. See issue #34397.
    """
    # The stamp is a property of the running code tree (parent of moor_cli/), NOT of $MOOR_HOME,
    ...
    method = _install_method_stamp(get_moor_home() / ".install_method")
...
def _running_in_container() -> bool:
    """Import-safe wrapper around ``moor_constants.is_container``."""
    try:
...
def recommended_update_command_for_method(method: str) -> str:
    ...
    return _UPDATE_COMMAND_BY_METHOD.get(method, "moor update")
...
# Shared by ``cmd_update`` and ``_cmd_update_check`` (moor_cli/main.py) so the wording never
...
def get_container_exec_info() -> Optional[dict]:
    ...
    already inside the container, or when MOOR_DEV=1 is set. Only FileNotFoundError is
    ...
    try:
        info = {}
        with open(get_moor_home() / ".container-mode", "r", encoding="utf-8") as f:
            for line in f:
                line = line.strip()
...
# ---- Config paths / MOOR_HOME skeleton ----
...
def require_parseable_user_config(*, ignore_user_config: bool = False) -> None:
    ...
    states; ``--ignore-user-config`` / MOOR_IGNORE_USER_CONFIG=1 remain authoritative."""
    if ignore_user_config or os.environ.get("MOOR_IGNORE_USER_CONFIG") == "1":
        return
...
def _chown_to_moor_uid(path) -> None:
    ...
    :func:`ensure_moor_home` on Docker deployments. See #34107.
    """
...
def _secure_dir(path):
    ...
    MOOR_HOME_MODE (e.g. 0701) overrides the mode so a web server can traverse MOOR_HOME to
    ...
    if is_managed():
        return
    try:
        mode = int(os.environ.get("MOOR_HOME_MODE", "").strip() or "700", 8)
    except ValueError:
        mode = 0o700
    try:
        os.chmod(path, mode)
...
def _is_container() -> bool:
    """Detect Docker/Podman/LXC (or MOOR_CONTAINER / MOOR_SKIP_CHMOD opt-out).
...
def ensure_moor_home():
    """Ensure the ~/.moor directory skeleton exists with secure permissions.
    ...
    if is_managed():
        ...
        try:
            if not home.is_dir():
                raise RuntimeError(f"MOOR_HOME {home} does not exist.")
        ...
        _secure_dir(home)
        for subdir in _MOOR_HOME_SUBDIRS:
            d = home / subdir
...
from moor_cli.config_defaults import DEFAULT_CONFIG, OPTIONAL_ENV_VARS  # noqa: E402,F401
from moor_cli.config_providers import (  # noqa: E402,F401  (re-exported; callers/tests use moor_cli.config.<name>)
...
# Back-compat re-exports — :mod:`moor_cli.personality` owns personality/overlay semantics.
from moor_cli.personality import (  # noqa: E402,F401
...
def _split_key_path(key: str) -> list[str]:
    ...
    ``moor config set`` uses ``.`` as the nesting separator, so a key that itself contains a literal dot
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    try:
        ...
        # A malformed SKILL.md must never break `moor update`; this prompting is a nicety.
...
_EXTRA_KNOWN_ROOT_KEYS = {
    ...
    "known_plugin_toolsets", # moor_cli/tools_config.py toolset-save flow
    ...
    for _key in ("backend", "search_backend", "extract_backend"):
        ...
        if note:
            _issue(issues, "warning",
                   ...
                   "Run 'moor tools' and pick a different Web Search & Extract provider")
...
def _env_ref_snapshot(obj, snapshot=None):
    ...
    made against a different environment (load before ``load_moor_dotenv()``, in-process
...
def _normalize_root_model_keys(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    LiteLLM users reach for, and ``moor config set`` blindly accepts any dotted key — so
    ...
    from OpenAI-compatible backends) — while display paths (``moor status``/``dump``) read ``name`` and
...
# the MOOR_MAX_ITERATIONS env bridge, works in every ``<``/``>=``/``max - used`` comparison in
...
def _is_ssh_remote_tilde_cwd(backend: str, cwd: str) -> bool:
    """Whether the remote SSH shell must expand *cwd* itself: ``~`` expanded on the Moor host
...
def _load_config_impl(*, want_deepcopy: bool) -> Dict[str, Any]:
    with _CONFIG_LOCK:
        ...
        if cached is not None and cache_sig is not None and cached[:4] == cache_sig:
            ...
            # load_moor_dotenv() pins unexpanded literals for the process lifetime.
            # Without this, a load_config() that ran before load_moor_dotenv() pins unexpanded literals
...
def load_env() -> Dict[str, str]:
    """Load ~/.moor/.env as a dict (memoised; ``get_env_value()`` runs hundreds of times per
...
def sanitize_env_file() -> int:
    """Rewrite ~/.moor/.env with normalized line formatting; returns the number of changed lines."""
    env_path = get_env_path()
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.moor/.env (also matching ``export KEY=`` lines, so a save
...
def custom_endpoint_key_env(identity: str) -> str:
    ...
    so two endpoints on one host get separate slots. The fixed ``MOOR_CUSTOM_`` prefix keeps the
...
def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.moor/.env and os.environ; True if it was found and removed."""
...
def reload_env() -> int:
    """Re-read ~/.moor/.env into os.environ; returns count of vars changed.
    Removes deleted vars only when known to Moor (OPTIONAL_ENV_VARS and _EXTRA_ENV_KEYS) so
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ``os.environ`` (scope-aware) or ``~/.moor/.env``.
...
def get_env_value_prefer_dotenv(key: str) -> Optional[str]:
    """Resolve a moor-managed credential preferring ``~/.moor/.env`` over ``os.environ``, so a
...
def _show_model_section(config: Dict[str, Any]) -> None:
    ...
    # Read the .env FILE directly so a stale MOOR_MAX_ITERATIONS ghost is caught even when the
    # gateway bridge already overrode os.environ.
    try:
        env_ghost = load_env().get("MOOR_MAX_ITERATIONS")
    except Exception:
        env_ghost = None
    if env_ghost is not None and str(env_ghost).strip() != str(cfg_max_turns).strip():
        print(color(f"                ⚠ .env has stale MOOR_MAX_ITERATIONS={env_ghost} "
                    f"(run 'moor doctor --fix' to remove)", Colors.YELLOW))
...
def resolve_cron_model_drift_defaults(
    ...
    Mirrors the scheduler's precedence: a truthy configured model wins over ``MOOR_MODEL``; the
    ...
    return provider, configured_model or _model_assignment_text(env.get("MOOR_MODEL", ""))
...
def warn_unpinned_cron_jobs_after_model_config_change(
    ...
    print(
        ...
        "model/provider. Inspect with `moor cron list`, then pin the intended values with "
        "`moor cron edit <job_id> --provider <provider> --model <model>`.")
...
def _coerce_config_set_value(key: str, value: str) -> Any:
    """Auto-coerce a ``moor config set`` string to bool/None/int/float/list/dict.
...
def _redirect_platform_display_key(key: str) -> tuple[str, Optional[str]]:
    ...
    Before #71047 a write such as ``moor config set platforms.telegram.streaming false`` landed on a key
...
def _print_unknown_key_notice(key: str, suggestion: Optional[str]) -> None:
    print(color(
        ...
        "but Moor may not read it.", Colors.YELLOW))
    if suggestion:
...
_USAGE_GET = ("Usage: moor config get <key> [--json]", [
...
_USAGE_UNSET = ("Usage: moor config unset <key>", [
    "moor config unset model", "moor config unset terminal.backend",
    "moor config unset OPENROUTER_API_KEY"], None)
...
_CONFIG_USAGE = """Available commands:
  ...
  moor config get <key>          Print a resolved config value
  moor config set <key> <value>   Set a config value
  moor config unset <key>        Remove a config value
  moor config check     Check for missing/outdated config
  moor config migrate   Update config with new options
  moor config path      Show config file path
  moor config env-path  Show .env file path"""
...
def _inject_platform_plugin_env_vars() -> None:
    ...
    Chat etc. are configurable in ``moor config`` UI without the core knowing they exist.
...
_PLUGIN_COMPAT_LAZY = {
    'normalize_route_base_url': ('moor_cli.route_identity', 'normalize_route_base_url'),
}
