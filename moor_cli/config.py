"""
Configuration management for Moor Agent.

Config files are stored in ~/.moor/ for easy access:
- ~/.moor/config.yaml  - All settings (model, toolsets, terminal, etc.)
- ~/.moor/.env         - API keys and secrets

This module provides:
- moor config          - Show current configuration
- moor config edit     - Open config in editor
- moor config get      - Print a resolved configuration value
- moor config set      - Set a specific value
- moor config unset    - Remove a user configuration value
- moor config wizard   - Re-run setup wizard
"""
...
def _backup_corrupt_config(config_path: Path) -> Optional[Path]:
    ...
    they re-run the setup wizard or ``moor config set`` (which rewrites
    ...
    ``config.yaml`` in place: moor never silently mutates the user's config,
...
def _warn_config_parse_failure(
    ...
    A YAML parse error in ``~/.moor/config.yaml`` causes ``load_config()``
    ...
    ``agent.log`` / ``errors.log`` at WARNING level so ``moor logs``
...
#   the next ``subprocess.run([...])`` Moor makes loads attacker code
#   before main().
# * ``PYTHONPATH`` / ``PYTHONHOME`` / ``PYTHONSTARTUP`` /
#   ``PYTHONUSERBASE`` — Python interpreter init. Moor itself starts
#   from one of these on every restart.
# * ``NODE_OPTIONS`` / ``NODE_PATH`` — Node interpreter; affects npm,
#   ``moor update``, the TUI build.
...
#   on every plugin install / ``moor update``.
...
# * ``MOOR_HOME`` / ``MOOR_PROFILE`` / ``MOOR_CONFIG`` /
#   ``MOOR_ENV`` — Moor runtime location flags. Writing these into
...
#
# IMPORTANT: ``MOOR_*`` overall is NOT blocked. Many legitimate
# integration credentials follow that prefix (MOOR_LANGFUSE_PUBLIC_KEY,
# MOOR_SPOTIFY_CLIENT_ID, ...). The
...
_ENV_VAR_NAME_DENYLIST: frozenset[str] = frozenset({
    ...
    # Moor runtime location — never via dashboard env writer.
    # NOT a MOOR_* blanket: integration credentials (MOOR_GEMINI_*,
    # MOOR_LANGFUSE_*, MOOR_SPOTIFY_*, ...) ARE allowed.
...
def _reject_denylisted_env_var(key: str) -> None:
    ...
    if key in _ENV_VAR_NAME_DENYLIST:
        raise ValueError(
            ...
            "PYTHONPATH, PATH, EDITOR, ...) or Moor runtime location "
            "(MOOR_HOME, MOOR_PROFILE, ...) cannot be persisted via "
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    # MOOR_TOOL_PROGRESS_MODE is deprecated (replaced by display.tool_progress
    ...
    # Activation is via plugins.enabled (opt-in through `moor plugins enable
    # observability/langfuse` or `moor tools → Langfuse`); credentials gate
    # the plugin at runtime.
    "MOOR_LANGFUSE_ENV",
    "MOOR_LANGFUSE_RELEASE",
    "MOOR_LANGFUSE_SAMPLE_RATE",
    "MOOR_LANGFUSE_MAX_CHARS",
    "MOOR_LANGFUSE_CAPTURE",
    "MOOR_LANGFUSE_DEBUG",
    "LANGFUSE_PUBLIC_KEY",
    "LANGFUSE_SECRET_KEY",
    "LANGFUSE_BASE_URL",
    ...
    "MOOR_ACP_AUTH_METHOD",
    "MOOR_ACP_AUTO_APPROVE",
    "MOOR_COPILOT_ACP_COMMAND",
    "MOOR_COPILOT_ACP_ARGS",
    "COPILOT_CLI_PATH",
...
# from `nix run` / `nix profile install` (which don't set MOOR_MANAGED).
...
def get_managed_system() -> Optional[str]:
    ...
    raw = os.getenv("MOOR_MANAGED", "").strip()
    marker = None
    if raw:
        marker = raw.lower()
    else:
        managed_marker = get_moor_home() / ".managed"
        # An interactive shell reads the marker, because it does not see the
        # MOOR_MANAGED variable of the service. A marker with content
...
def is_managed() -> bool:
    """Check if Moor is running in package-manager-managed mode.

    Two signals: the MOOR_MANAGED env var (set by the systemd service),
    or a .managed marker file in MOOR_HOME (set by the NixOS activation
...
def _install_method_project_root(project_root: Optional[Path] = None) -> Path:
    ...
    This is the parent of ``moor_cli/`` — i.e. the git checkout for source
    installs, ``/opt/moor`` inside the published image. It is a property of
    the running interpreter, NOT of ``$MOOR_HOME``, which is why a
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    """Detect how Moor was installed: 'apt', 'docker', 'nix', 'nixos',
    ...
    2. Legacy home-scoped stamp ``$MOOR_HOME/.install_method`` — read for
       backward compatibility, but a ``docker`` value is IGNORED when we are
       not actually running inside a container (see below).
    3. MOOR_MANAGED env / .managed marker (NixOS managed mode)
    4. /nix/store/ path detection -> 'nix' (nix run / nix profile install)
    5. .git directory presence -> 'git'
    6. Fallback -> 'unknown'

    Why the stamp is code-scoped, not home-scoped (issue: shared ``~/.moor``)
    --------------------------------------------------------------------------
    The install method describes *the binary that is running*, but
    ``$MOOR_HOME`` is a shared DATA directory — the Docker docs deliberately
    bind-mount it (``~/.moor:/opt/data``) so config/sessions/memory persist
    and can be shared with a host-side Desktop/CLI install. When a
    containerised gateway and a host install share one ``$MOOR_HOME``, a
    ...
    ``docker`` and ``moor update`` refuses to run ("doesn't apply inside the
    ...
    The supported installs self-identify via the code-scoped stamp:
      ...
      - the published ``nousresearch/hermes-agent`` image bakes a ``docker``
        stamp into ``/opt/moor`` at build time.
    ...
    # 1. Code-scoped stamp — authoritative, immune to shared $MOOR_HOME.
    try:
    ...
    #    host install whose shared $MOOR_HOME was stamped by a co-located
    #    container, and honouring it wrongly blocks ``moor update``.
    try:
        method = (
            (get_moor_home() / ".install_method")
    ...
    # detect Nix installs that don't set MOOR_MANAGED (e.g. ``nix run``,
...
def _running_in_container() -> bool:
    """Thin wrapper around ``moor_constants.is_container`` (import-safe)."""
    try:
...
def stamp_install_method(method: str, project_root: Optional[Path] = None) -> None:
    ...
    not in ``$MOOR_HOME``, so that two installs sharing one data directory
    ...
    ``/opt/moor`` in the published image, which instead bakes the stamp at
...
# Long-form text for ``moor update`` / ``--check`` when running inside the
# Docker image.  Surfaced by ``cmd_update`` and ``_cmd_update_check`` in
# moor_cli/main.py; lives here so the wording stays consistent and we
...
#     — that script installs a *new* host-side Moor, it doesn't update
...
def format_docker_update_message() -> str:
    """Return the user-facing message for ``moor update`` inside Docker.
...
def format_managed_message(action: str = "modify this Moor installation") -> str:
    """Build a user-facing error for managed installs."""
    managed_system = get_managed_system() or "a package manager"
    return (
        f"Cannot {action}: this Moor installation is managed by {managed_system}.\n"
        "Use your package manager to upgrade or reinstall Moor."
    )
...
def get_container_exec_info() -> Optional[dict]:
    """Read container mode metadata from MOOR_HOME/.container-mode.

    Returns a dict with keys: backend, container_name, exec_user, moor_bin
    or None if container mode is not active, we're already inside the
    container, or MOOR_DEV=1 is set.

    The .container-mode file is written by the NixOS activation script when
    container.enable = true. It tells the host CLI to exec into the container
    instead of running locally.
    """
    if os.environ.get("MOOR_DEV") == "1":
        return None
    ...
    container_mode_file = get_moor_home() / ".container-mode"
    ...
    container_name = info.get("container_name", "moor-agent")
    exec_user = info.get("exec_user", "moor")
    moor_bin = info.get("moor_bin", "/data/current-package/bin/moor")
...
from moor_constants import get_moor_home, get_process_moor_home  # noqa: F811,E402
from utils import atomic_replace, fast_safe_load

def get_config_path() -> Path:
    """Get the main config file path."""
    return get_moor_home() / "config.yaml"

def get_env_path() -> Path:
    """Get the .env file path (for API keys)."""
    return get_moor_home() / ".env"

def get_project_root() -> Path:
    """Get the project installation directory."""
    return Path(__file__).parent.parent.resolve()

def _resolve_moor_uid_gid() -> tuple[Optional[int], Optional[int]]:
    """Read the MOOR_UID / MOOR_GID env vars set by Docker deployments.

    Docker containers running Moor commonly set these to map the in-container
    user to a host user so volume-mounted state files end up with the right
    ownership. The entrypoint chowns the top-level MOOR_HOME once, but
    subdirectories created at runtime by ``ensure_moor_home()`` (especially
    ...
    if sys.platform == "win32":
        return None, None
    uid_str = os.environ.get("MOOR_UID", "").strip()
    gid_str = os.environ.get("MOOR_GID", "").strip()
    try:
...
def _chown_to_moor_uid(path) -> None:
    """Chown ``path`` to ``MOOR_UID:MOOR_GID`` if those env vars are set.
    ...
    directories created by :func:`ensure_moor_home` on Docker deployments.
    See #34107.
    """
...
def _secure_dir(path):
    ...
    permissions (0750) so interactive users in the moor group can
    share state with the gateway service.

    The mode can be overridden via the MOOR_HOME_MODE environment variable
    (e.g. MOOR_HOME_MODE=0701) for deployments where a web server (nginx,
    caddy, etc.) needs to traverse MOOR_HOME to reach a served subdirectory.
    The execute-only bit on a directory permits cd-through without exposing
    directory listings.

    Also applies ``MOOR_UID``/``MOOR_GID``-based ownership when those env
    ...
    if is_managed():
        return
    try:
        mode_str = os.environ.get("MOOR_HOME_MODE", "").strip()
...
def _is_container() -> bool:
    ...
    When Moor runs in a container with volume-mounted config files, forcing
    ...
    """
    # Explicit opt-out
    if os.environ.get("MOOR_CONTAINER") or os.environ.get("MOOR_SKIP_CHMOD"):
        return True
...
def _secure_file(path):
    ...
    permissions.  Set MOOR_SKIP_CHMOD=1 to force-skip on other systems.
    """
...
def _ensure_default_soul_md(home: Path) -> None:
    """Seed a default SOUL.md into MOOR_HOME, upgrading legacy empty templates.
...
# ensure_moor_home(). Only successful passes are recorded, so a raised
# managed-mode/missing-profile error keeps re-checking on later loads.
_MOOR_HOME_ENSURED: set = set()


def ensure_moor_home():
    """Ensure ~/.moor directory structure exists with secure permissions.
    ...
    After the first successful pass for a given ``MOOR_HOME`` we only re-run
    ...
    ``get_moor_home()`` and therefore re-run for the new path.
    """
    home = get_moor_home()
    key = str(home)
    ...
    if key in _MOOR_HOME_ENSURED and home.is_dir():
        return
    # Named profiles must be created explicitly (e.g. ``moor profile create``).
    # If a stale process keeps running after the profile was renamed/deleted,
    # silently mkdir-ing the old MOOR_HOME would resurrect an empty skeleton
    ...
    if is_managed():
        ...
        try:
            _ensure_moor_home_managed(home)
        finally:
            os.umask(old_umask)
    else:
    ...
    _MOOR_HOME_ENSURED.add(key)


def _ensure_moor_home_managed(home: Path):
    """Managed-mode variant: verify dirs exist (activation creates them), seed SOUL.md."""
    if not home.is_dir():
        raise RuntimeError(
            f"MOOR_HOME {home} does not exist."
        )
...
from moor_cli.config_defaults import DEFAULT_CONFIG, OPTIONAL_ENV_VARS  # noqa: F401
...
# selection step (Moor Portal / OpenRouter / Custom endpoint), so this
...
def _is_env_config_key(key: str) -> bool:
    """Return whether `moor config set` routes this key to .env."""
    if "." in key:
        return False
    key_upper = key.upper()
    api_keys = [
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    Scans all enabled skills for ``metadata.moor.config`` entries, then checks
    ...
    try:
        ...
        # should never break `moor update`.  Skill-config prompting is a
...
def _normalize_custom_provider_entry(
    ...
    *,
    ...
    _KNOWN_KEYS = {
        ...
        # unused here, but Moor' own config writer has historically emitted it
    ...
    # Moor (``_save_discovered_models_to_config``), not hand-curated.
    # Older Moor versions wrote an in-mapping ``__discovered_model_catalog__``
    ...
    if isinstance(models, dict) and models:
        ...
        # Hand-edited configs (and older Moor versions) may write
...
def get_custom_provider_context_length(
    ...
    used by:
      ...
      * ``moor_cli.model_switch.resolve_display_context_length`` (``/model`` confirmation display)
...
_EXTRA_KNOWN_ROOT_KEYS = {
    ...
    "plugins",           # plugin enable/disable lists (moor_cli/plugins_cmd.py)
    "smart_model_routing",   # written by the setup wizard (moor_cli/setup.py)
    "platform_toolsets",     # written by the setup wizard (moor_cli/setup.py)
    "known_plugin_toolsets", # written/read by moor_cli/tools_config.py toolset-save flow
    "known_builtin_toolsets",  # ditto — which builtin toolsets a platform's checklist has offered
    "tool_gateway_declined_tools",  # per-tool Tool Gateway offer declines (moor_cli/moor_subscription.py, #92647)
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if cp and not model_cfg:
        issues.append(ConfigIssue(
            "warning",
            "custom_providers defined but no 'model' section — Moor won't know which provider to use",
            "Add a model section:\n"
            "  model:\n"
            "    provider: custom\n"
    ...
    # scalars are bridged into os.environ (gateway/run.py, moor send) so
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append("  \033[2mRun 'moor doctor' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
...
def _persist_migration(config: Dict[str, Any]) -> None:
    ...
    DEFAULT_CONFIG dumps (the "moor update / moor -p blows up my config"
    reports).
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    if floor_refused:
        ...
        sys.stderr.write(f"⚠ moor config: {msg}\n")
        if not quiet:
            print(f"  ⚠ {msg}")
    else:
        # ── Versioned migration ladder (table-driven) ──
        # The per-version steps live in moor_cli.config_migrations as a
    ...
    if new_var_names and interactive and not quiet:
        ...
        if new_and_unset:
            ...
            if answer in {"y", "yes"}:
                ...
                print("  Set later with: moor config set <key> <value>")
    ...
    # `moor update`, but only the version bump is persisted.
    ...
    # metadata.moor.config in their SKILL.md frontmatter.
    ...
    * ``${env:VAR}`` — Cursor-style SecretRef, same resolution after the
    ...
    if inner.startswith("env:"):
        ...
        if val is not None:
            return val
        logger.warning(
            "Config ref %r: %s is not set (check ~/.moor/.env); "
...
def _env_ref_snapshot(obj, snapshot=None):
    ...
    ``load_moor_dotenv()`` populated the process env, or an env var
...
def split_model_config_default(raw_default: Any) -> tuple[str, str]:
    ...
    together through ``MoorCLI`` construction: the model becomes a plain
...
def _normalize_root_model_keys(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    intuitive name OpenAI-SDK / LiteLLM users reach for, and ``moor config set``
    ...
    backends) — while display paths (``moor status``/``dump``) read ``name``
...
#   - survives the ``str() → int()`` round-trip through the MOOR_MAX_ITERATIONS
...
def cfg_get(cfg: Optional[Dict[str, Any]], *keys: str, default: Any = None) -> Any:
    ...
    ubiquitous ``cfg_path = _moor_home / "config.yaml"`` local variable
...
# Back-compat alias — canonical set lives in moor_cli.personality.
from moor_cli.personality import NEUTRAL_PERSONALITY_NAMES as _NEUTRAL_PERSONALITY_NAMES  # noqa: F401


def _prompt_text(value: Any) -> str:
    """Normalize config prompt values from YAML before handing them to AIAgent.

    Delegates to :mod:`moor_cli.personality` — the single owner of
    personality/overlay semantics. Kept as a re-export for existing importers.
    """
    from moor_cli.personality import prompt_text
...
def render_personality_prompt(value: Any) -> str:
    ...
    from moor_cli.personality import render_personality_prompt as _render
...
def resolve_ephemeral_system_prompt_from_config(cfg: Optional[Dict[str, Any]]) -> str:
    ...
    still prefer ``MOOR_EPHEMERAL_SYSTEM_PROMPT`` when that env var is set.

    Delegates to :mod:`moor_cli.personality` (single owner).
    """
    from moor_cli.personality import resolve_ephemeral_system_prompt

    return resolve_ephemeral_system_prompt(cfg)


def read_raw_config() -> Dict[str, Any]:
    """Read ~/.moor/config.yaml as-is, without merging defaults or migrating.
...
def read_user_config_raw(config_path: Optional[Path] = None) -> Dict[str, Any]:
    ...
    ``_moor_home``, tui profile override, multi-profile probes).
    """
...
def load_config() -> Dict[str, Any]:
    """Load configuration from ~/.moor/config.yaml.
    ...
    (which change ``MOOR_HOME`` and therefore ``get_config_path()``)
    don't collide.
...
def _load_config_impl(*, want_deepcopy: bool) -> Dict[str, Any]:
    with _CONFIG_LOCK:
        ...
        # cache signature so editing /etc/moor/config.yaml invalidates the
        ...
        if cached is not None and cache_sig is not None and cached[:4] == cache_sig:
            ...
            # Without this, a load_config() that ran before load_moor_dotenv()
...
#   openai-codex (OAuth — moor auth) — OpenAI Codex
#   moor         (OAuth — moor auth) — Moor Portal
...
def save_config(
    ...
    *,
    ...
    """Save configuration to ~/.moor/config.yaml.\n
...
def _parse_env_value(raw_value: str) -> str:
    """Parse the small .env value subset Moor writes itself."""
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.moor/.env.
    ...
    interactive menu render (`moor tools`, `moor setup`, status
    panels). Sanitisation is O(lines), so re-parsing the
    same file on every call was burning ~300ms of CPU per `moor tools`
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.moor/.env in place.
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.moor/.env."""
    if is_managed():
...
def custom_endpoint_key_env(identity: str) -> str:
    ...
    - The fixed ``MOOR_CUSTOM_`` prefix keeps the result a valid POSIX name
    ...
    return f"MOOR_CUSTOM_{slug}_API_KEY" if slug else "MOOR_CUSTOM_API_KEY"


def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.moor/.env and os.environ.
...
def reload_env() -> int:
    """Re-read ~/.moor/.env into os.environ. Returns count of vars updated.

    Adds/updates vars that changed and removes vars that were deleted from
    the .env file (but only vars known to Moor — OPTIONAL_ENV_VARS and
    ...
    for key, value in env_vars.items():
        if os.environ.get(key) != value:
            os.environ[key] = value
            count += 1
    # Remove known Moor vars that are no longer in .env
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ``os.environ`` or ``~/.moor/.env``, scope-aware.
...
def get_env_value_prefer_dotenv(key: str) -> Optional[str]:
    """Resolve a credential env value, preferring ``~/.moor/.env`` over ``os.environ``.

    Used for moor-managed credentials where a deliberate edit to ``.env``
...
def show_config():
    ...
    from moor_cli.auth import get_anthropic_key
    ...
    # Warn on stale MOOR_MAX_ITERATIONS ghost in .env that disagrees with
    ...
    try:
        _env_ghost = load_env().get("MOOR_MAX_ITERATIONS")
        if _env_ghost is not None and str(_env_ghost).strip() != str(_cfg_max_turns).strip():
            print(color(
                f"                ⚠ .env has stale MOOR_MAX_ITERATIONS={_env_ghost} "
                f"(run 'moor doctor --fix' to remove)",
                Colors.YELLOW,
            ))
    except Exception:
        pass
    ...
    try:
        from moor_cli.personality import active_personality_name
    ...
    print(color("  moor config edit     # Edit config file", Colors.DIM))
    print(color("  moor config set <key> <value>", Colors.DIM))
    print(color("  moor setup           # Run setup wizard", Colors.DIM))
    print()
...
def resolve_cron_model_drift_defaults(
    ...
    *,
    ...
    wins ``MOOR_MODEL``; the environment is only a fallback. Per-job and cron
    ...
    provider = ""
    model = _model_assignment_text(env.get("MOOR_MODEL", ""))
...
def warn_unpinned_cron_jobs_after_model_config_change(
    ...
    print(
        ...
        "changed model/provider. Inspect with `moor cron list`, then pin the "
        "intended values with `moor cron edit <job_id> --provider <provider> "
        "--model <model>`."
    )
...
_SCHEMA_DEFINED_DICT_KEYS = frozenset({
    ...
    # (moor_cli/plugins_cmd.py, moor_cli/plugin_index.py). Absent from
    # DEFAULT_CONFIG (written only when used), so listed here for
    # `moor config set plugins.index_url ...` validation.
    "plugins",
})
...
def _known_top_level_keys() -> set[str]:
    ...
    to decide whether a ``moor config set`` invocation is targeting a
    known shape.
    """
...
def _suggest_closest_key(key: str, candidates: set[str], cutoff: float = 0.6) -> Optional[str]:
    ...
    similar enough to ``key``, else None.  Used by ``moor config set``
...
def set_config_value(key: str, value: str, force: bool = False):
    ...
    Args:
        ...
        force: When True, skip the unknown-key warning — useful for scripted
            ...
            CLI exposes this via ``moor config set --force``.
    """
    if is_managed():
    ...
    if config_path.exists():
        try:
            ...
            print(
                ...
                f"  (moor config edit will open it in your editor.)",
                file=sys.stderr,
            )
            sys.exit(1)
    ...
    if not isinstance(_default_value_for_key(key), str):
        ...
        if _lower in {'true', 'yes', 'on'}:
            ...
            #   moor config set platform_toolsets.line '["file","web"]'
            # or a multi-line YAML block:
            #   moor config set custom_providers '- name: foo
            #     base_url: https://...'
    ...
    # ``moor config set model.provider openai`` doesn't silently
    ...
    # (e.g. ``moor config set model gpt-5.6-sol`` when model already
    ...
    if "." not in key:
        ...
        if isinstance(_existing, dict):
            if key == "model":
                ...
                print(
                    f"    moor config set {key}.<sub-key> <value>",
                    file=sys.stderr,
                )
                print(
                ...
                print(
                    f"    moor config set --force {key} {value!r}",
                    file=sys.stderr,
                )
                sys.exit(1)
    _set_nested(user_config, key, value)
    # Normalize the api_base → base_url alias at set-time too (issue #8919),
    # so a fresh `moor config set model.api_base ...` lands on the canonical
    ...
    if key == "display.skin" and isinstance(value, str) and value:
        try:
            skin_file = get_moor_home() / "skins" / f"{value}.yaml"
            if skin_file.exists():
                skin_file.touch()
        except Exception:
    ...
    # — e.g. `moor config set model.api_key cfut_...` routes to config.yaml
...
def unset_config_value(key: str):
    ...
    if _is_env_config_key(key):
        ...
        # model-cache rows too, so `moor config unset <KEY>` fully removes
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key:
            print("Usage: moor config get <key> [--json]")
            print()
            print("Examples:")
            print("  moor config get model")
            print("  moor config get terminal.backend")
            print("  moor config get skills.config --json")
            sys.exit(1)
        ...
        if not key or value is None:
            print("Usage: moor config set [--force] <key> <value>")
            print()
            print("Examples:")
            print("  moor config set model anthropic/claude-sonnet-4")
            print("  moor config set terminal.backend docker")
            print("  moor config set OPENROUTER_API_KEY sk-or-...")
            print()
        ...
        key = getattr(args, 'key', None)
        if not key:
            print("Usage: moor config unset <key>")
            print()
            print("Examples:")
            print("  moor config unset model")
            print("  moor config unset terminal.backend")
            print("  moor config unset OPENROUTER_API_KEY")
            sys.exit(1)
        ...
        if missing_config:
            ...
            print("    Run 'moor config migrate' to add them")
        ...
        print("  moor config           Show current configuration")
        print("  moor config edit      Open config in editor")
        print("  moor config get <key>          Print a resolved config value")
        print("  moor config set <key> <value>   Set a config value")
        print("  moor config unset <key>        Remove a config value")
        print("  moor config check     Check for missing/outdated config")
        print("  moor config migrate   Update config with new options")
        print("  moor config path      Show config file path")
        print("  moor config env-path  Show .env file path")
        sys.exit(1)
...
# ``_inject_profile_env_vars`` surfaces them in ``moor config`` UI so users
