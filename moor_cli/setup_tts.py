...
def _xai_oauth_logged_in_for_setup() -> bool:
    ...
    API-key prompt for users who logged in via ``moor model`` -> xAI Grok OAuth."""
    try:
...
def _xai_api_key_path():
    ...
    return ("No xAI API key provided for TTS. Configure XAI_API_KEY via moor setup model "
...
def _setup_tts_provider(config: dict):
    ...
    if tool_backend_helpers.managed_moor_tools_enabled() and moor_subscription.get_moor_subscription_features(config).moor_auth_present:
        options.insert(0, ("moor-openai",
                           "Moor Subscription (managed OpenAI TTS, billed to your subscription)"))
    ...
    if selected == "moor-openai":
        selected = "openai"
        _setup.print_info("OpenAI TTS will use the managed Moor gateway and bill to your subscription.")
