...
def _strip_console_status_footer(text: str) -> str:
    ...
    if not (prev.startswith("Run 'moor doctor'") and last.startswith("Run 'moor setup'")):
        return text.rstrip()
    lines = lines[:-2]
    _drop_trailing_blank(lines)
...
def _clean_summary(text: str | None) -> str:
    ...
    return "" if summary.startswith("Run `moor ") else summary
...
class _CliSurface:
    ...
    ``extracted``: ``builder(subparsers, <handler>=fn)``, fn from moor_cli.main.
    ``registered``: ``register(subparsers.add_parser(root))``, optional module ``handler``.
    ``builder``: ``top = builder(subparsers)``, func from moor_cli.main.
    ``adder``: self-wiring."""
    ...
    def build(self, root: str, *, live: bool) -> _ArgumentParser:
        ...
        main_handler = (
            (lambda: getattr(importlib.import_module("moor_cli.main"), self.handler))
...
def _sub(module: str, builder: str, handler: str) -> _CliSurface:
    return _CliSurface("extracted", f"moor_cli.subcommands.{module}", builder, handler)


def _reg(module: str, handler: str | None = None) -> _CliSurface:
    return _CliSurface("registered", f"moor_cli.{module}", "register_cli", handler)
...
_CLI_FAMILIES: dict[str, tuple[_CliSurface, str]] = {
    ...
    "portal": (_CliSurface("adder", "moor_cli.portal_cli", "add_parser"), "info, tools"),
    "project": (
        _CliSurface("builder", "moor_cli.projects_cmd", "build_parser", "cmd_project"),
        "list, show, *create, *add-folder, *remove-folder, *rename, *set-primary, *use, "
        "*archive, *restore, *bind-board"),
    "kanban": (
        _CliSurface("builder", "moor_cli.kanban", "build_parser", "cmd_kanban"),
...
_SEND_SURFACE = _CliSurface("adder", "moor_cli.send_cmd", "register_send_subparser")


def _register_command_family(
    engine: "MoorConsoleEngine", root: str, surface: _CliSurface, paths: str) -> None:
    ...
    for child_path, mutating in _paths(paths):
        ...
        def handler(_engine: MoorConsoleEngine, args: list[str], fixed=child_path) -> str:
            return _dispatch(surface, root, fixed, args, namespace_update)

        engine.register(
            full_path, usage, summaries.get(full_path) or f"Run `moor {usage}`.", handler,
            mutating=mutating, confirmation=f"Run `moor {usage}`?")
...
_BLOCKED_PAIRS = {
    ...
    ("sessions", "delete"): "`sessions delete` and `sessions prune` are not available in Moor Console.",
    ("sessions", "prune"): "`sessions delete` and `sessions prune` are not available in Moor Console.",
}
...
class MoorConsoleEngine:
    ...
    def help_text(self, subject: str | None = None) -> str:
        ...
        lines = ["Moor Console", "", "Supported commands:"]
...
def _captured(fn):
    ...
    @functools.wraps(fn)
    def wrapper(engine: MoorConsoleEngine, args: list[str]) -> str:
...
def _apply_confirmed_defaults(args: argparse.Namespace) -> None:
    ...
    if getattr(args, "auth_action", None) == "add":
        ...
        if auth_type in {"api-key", "api_key"} and not getattr(args, "api_key", None):
            raise ConsoleCommandError(
                "auth add --type api-key requires --api-key in Moor Console.")
...
_version = _simple_command(
    "version", "moor_cli._startup_fast", "print_fast_version_info", check_updates=True)
...
_doctor = _simple_command(
    "doctor", "moor_cli.doctor", "run_doctor", lambda: (SimpleNamespace(fix=False, ack=None),))
_config_show = _simple_command("config show", "moor_cli.config", "show_config")
_cron_status = _simple_command("cron status", "moor_cli.cron", "cron_status")
...
@_captured
def _config_migrate(_engine: MoorConsoleEngine, args: list[str]) -> None:
...
def _guard_exports(db, session_ids: list[str]) -> None:
    ...
    from moor_state import SessionExportTooLargeError, resolved_max_export_messages
...
@_captured
def _sessions_export(_engine: MoorConsoleEngine, args: list[str]) -> None:
...
@_captured
def _sessions_rename(_engine: MoorConsoleEngine, args: list[str]) -> None:
...
@_captured
def _sessions_repair(_engine: MoorConsoleEngine, args: list[str]) -> None:
    ns = _parse(
        "sessions repair", args, (("--check-only",), dict(action="store_true")),
        (("--no-backup",), dict(action="store_true")))
    from moor_state import DEFAULT_DB_PATH
...
def _cron_pause(_engine: MoorConsoleEngine, args: list[str]) -> str:
    ...
    return _cron_job_action(
        ...
        lambda ref: pause_job(ref, reason="paused from moor console"))
...
def _cron_resume(_engine: MoorConsoleEngine, args: list[str]) -> str:
    ...
    try:
        job = (
            rearm_oneshot(ns.job, _moor_now().isoformat() if ns.run_now else ns.at)
...
_BUILTIN_COMMANDS = (
    (("status",), "status", "Show Moor component status.", _status, ""),
    (("version",), "version", "Show Moor version information.", _version, ""),
    (("doctor",), "doctor", "Run diagnostics without auto-fix.", _doctor, ""),
    (("logs",), "logs [name] [-n N]", "Show recent Moor logs.", _logs, ""),
    ...
    (("config", "migrate"), "config migrate", "Update config with new options.", _config_migrate,
     "Update Moor configuration with missing defaults?"),
...
def run_console_repl(
    *, stdin=None, stdout=None, stderr=None, interactive: bool | None = None) -> int:
    ...
    say("Moor Console. Type `help` for commands, `exit` to quit.")
    while True:
        say("moor> ", end="", flush=True)
        line = stdin.readline()
        if line == "":
            say("")
            return 0
