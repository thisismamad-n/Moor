"""Safe Moor Console command engine.

This module backs ``moor console`` and is intentionally narrower than the
full Moor CLI. It exposes a curated set of native adapters that can later be
...
class ConsoleCommand:
    ...
    usage: str
    summary: str
    handler: Callable[["MoorConsoleEngine", list[str]], str]
    mutating: bool = False
    confirmation: str = ""
...
def _strip_console_status_footer(text: str) -> str:
    ...
    if not (
        prev.startswith("Run 'moor doctor'")
        and last.startswith("Run 'moor setup'")
    ):
        return text.rstrip()
...
def _split_line(line: str) -> list[str]:
    try:
        ...
        from moor_cli._subprocess_compat import split_command_line
...
def _parser_root() -> tuple[_ArgumentParser, argparse._SubParsersAction]:
    parser = _ArgumentParser(prog="moor", add_help=False)
...
def _clean_summary(text: str | None) -> str:
    ...
    if not summary:
        return ""
    if summary.startswith("Run `moor "):
        return ""
    return summary
...
# dashboard opens a fresh MoorConsoleEngine per /api/console connection, so
...
def _dispatch_extracted_subcommand(
    *,
    ...
    parser, subparsers = _parser_root()
    module = importlib.import_module(module_name)
    main_module = importlib.import_module("moor_cli.main")
    builder = getattr(module, builder_name)
...
def _extracted_handler(
    ...
    main_handler_name: str,
    namespace_update: Callable[[argparse.Namespace], None] | None = None,
) -> Callable[["MoorConsoleEngine", list[str]], str]:
    def handler(_engine: MoorConsoleEngine, args: list[str]) -> str:
...
def _register_command_family(
    engine: "MoorConsoleEngine",
    *,
    root: str,
    paths: Iterable[Sequence[str]],
    handler_factory: Callable[[Sequence[str]], Callable[["MoorConsoleEngine", list[str]], str]],
    ...
    for child_path in paths:
        ...
        command_summary = summary or (summaries or {}).get(full_path) or f"Run `moor {usage}`."
        engine.register(
            full_path,
            usage,
            command_summary,
            ...
            confirmation=confirmation or f"Run `moor {usage}`?",
        )


class MoorConsoleEngine:
    """Curated line-command executor for Moor Console."""
        ...
        try:
            ...
            if _contains_shell_syntax(raw_line, tokens):
                raise ConsoleCommandError(
                    "Moor Console does not run shell syntax. Use one supported "
                    "Moor command at a time."
                )
    ...
    def _register_defaults(self) -> None:
        ...
        self.register(
            ...
            _config_set,
            mutating=True,
            confirmation="Update Moor configuration?",
        )
    ...
    def _register_broad_cli_surface(self) -> None:
        ...
        extracted = {
            ...
            "prompt-size": (
                "moor_cli.subcommands.prompt_size",
            ...
            "import": (
                "moor_cli.subcommands.import_cmd",
        ...
        self.register(
            ...
            _config_migrate,
            mutating=True,
            confirmation="Update Moor configuration with missing defaults?",
        )
        self.register(
            ("sessions", "export"),
        ...
        self.register(
            ...
            _adder_handler("send", (), "moor_cli.send_cmd", "register_send_subparser"),
            mutating=True,
        ...
        _register_command_family(
            ...
            root="portal",
            paths=portal_paths,
            summaries=_adder_summaries("moor_cli.portal_cli", "add_parser"),
        ...
        _register_command_family(
            ...
            summaries=_builder_summaries("moor_cli.projects_cmd", "build_parser"),
            mutating=[
                ("create",),
                ("add-folder",),
                ("remove-folder",),
        ...
        _register_command_family(
            ...
            summaries=_builder_summaries("moor_cli.kanban", "build_parser"),
            mutating=[
                ("init",),
                ("boards", "create"),
                ("boards", "rm"),
    ...
    def _resolve_command(self, tokens: Sequence[str]) -> tuple[ConsoleCommand, list[str]]:
        ...
        raise ConsoleCommandError(f"Unsupported Moor Console command: {probe}.{suffix}")

    def _rejection_for(self, tokens: Sequence[str]) -> str:
        first = tokens[0]
        if first.startswith("-"):
            return f"{first} is not available in Moor Console."
        blocked_top = {
            "acp",
            "chat",
            "claw",
        ...
        if first in blocked_top:
            return f"`moor {first}` is not available in Moor Console."
        blocked_pairs = {
            ("config", "edit"): "`config edit` opens an editor and is not available in Moor Console.",
            ("mcp", "serve"): "`mcp serve` starts a server and is not available in Moor Console.",
            ("profile", "alias"): "`profile alias` creates shell wrappers and is not available in Moor Console.",
            ("skills", "config"): "`skills config` is interactive and is not available in Moor Console.",
            ("skills", "publish"): "`skills publish` is not available in Moor Console.",
            ("portal", "login"): "`portal login` is interactive and is not available in Moor Console.",
            ("portal", "open"): "`portal open` opens a browser and is not available in Moor Console.",
            ("kanban", "tail"): "`kanban tail` streams output and is not available in Moor Console.",
            ("kanban", "watch"): "`kanban watch` streams output and is not available in Moor Console.",
            ("kanban", "daemon"): "`kanban daemon` starts a service and is not available in Moor Console.",
            ("kanban", "dispatcher"): "`kanban dispatcher` starts a worker and is not available in Moor Console.",
            ("kanban", "swarm"): "`kanban swarm` starts agent work and is not available in Moor Console.",
            ("kanban", "decompose"): "`kanban decompose` starts agent work and is not available in Moor Console.",
            ("kanban", "specify"): "`kanban specify` starts agent work and is not available in Moor Console.",
            ("kanban", "gc"): "`kanban gc` is not available in Moor Console.",
        }
        if len(tokens) >= 2:
        ...
        if tuple(tokens[:2]) in {("sessions", "delete"), ("sessions", "prune")}:
            return "`sessions delete` and `sessions prune` are not available in Moor Console."
        return ""
...
def _apply_confirmed_defaults(args: argparse.Namespace) -> None:
    ...
    if getattr(args, "auth_action", None) == "add":
        ...
        if auth_type in {"api-key", "api_key"} and not getattr(args, "api_key", None):
            raise ConsoleCommandError("auth add --type api-key requires --api-key in Moor Console.")
...
def _doctor(_engine: MoorConsoleEngine, args: list[str]) -> str:
...
def _logs(_engine: MoorConsoleEngine, args: list[str]) -> str:
    if "-f" in args or "--follow" in args:
        raise ConsoleCommandError("`logs -f` is not available in Moor Console.")
    ...
    from moor_cli.logs import list_logs, tail_log
...
def _sessions_list(_engine: MoorConsoleEngine, args: list[str]) -> str:
...
def _config_show(_engine: MoorConsoleEngine, args: list[str]) -> str:
    _expect_no_args(args, "config show")
    from moor_cli.config import show_config

    return _capture_output(show_config)


def _config_path(_engine: MoorConsoleEngine, args: list[str]) -> str:
...
def _config_set(_engine: MoorConsoleEngine, args: list[str]) -> str:
    if len(args) < 2:
...
def _config_migrate(_engine: MoorConsoleEngine, args: list[str]) -> str:
...
def _sessions_export(_engine: MoorConsoleEngine, args: list[str]) -> str:
...
def _sessions_rename(_engine: MoorConsoleEngine, args: list[str]) -> str:
...
def _sessions_optimize(_engine: MoorConsoleEngine, args: list[str]) -> str:
...
def _sessions_repair(_engine: MoorConsoleEngine, args: list[str]) -> str:
    ...
    def _run() -> None:
        from moor_state import DEFAULT_DB_PATH, _db_opens_cleanly, repair_state_db_schema
...
def _profile_status(_engine: MoorConsoleEngine, args: list[str]) -> str:
    _expect_no_args(args, "profile")
    return _dispatch_extracted_subcommand(
        root="profile",
        fixed=(),
        args=(),
        module_name="moor_cli.subcommands.profile",
        builder_name="build_profile_parser",
        main_handler_name="cmd_profile",
    )


def _cron_list(_engine: MoorConsoleEngine, args: list[str]) -> str:
    parser = _ArgumentParser(prog="cron list", add_help=False)
    parser.add_argument("--all", action="store_true")
    ns = parser.parse_args(args)
    from moor_cli.cron import cron_list

    return _capture_output(lambda: cron_list(show_all=ns.all))


def _cron_status(_engine: MoorConsoleEngine, args: list[str]) -> str:
...
def _cron_pause(_engine: MoorConsoleEngine, args: list[str]) -> str:
    if len(args) != 1:
        raise ConsoleCommandError("Usage: cron pause <job>")
    from cron.jobs import AmbiguousJobReference, pause_job

    try:
        job = pause_job(args[0], reason="paused from moor console")
...
def _cron_run(_engine: MoorConsoleEngine, args: list[str]) -> str:
    if len(args) != 1:
...
def run_console_repl(
    *,
    ...
    """Run the local ``moor console`` REPL."""
    ...
    engine = MoorConsoleEngine()
    if interactive:
        print("Moor Console. Type `help` for commands, `exit` to quit.", file=stdout)

    while True:
        if interactive:
            print("moor> ", end="", file=stdout, flush=True)
        line = stdin.readline()
        if line == "":
            if interactive:
                print(file=stdout)
