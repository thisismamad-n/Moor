"""CLI handlers for ``moor secrets bitwarden ...``."""
...
# module makes the ``moor update`` self-lock preflight defer. This module is registered
# parse-time from ``moor_cli.main``, so the backend import stays lazy (nothing touches ``bw``
...
from moor_cli._secrets_common import (
...
from moor_cli.config import get_env_path, load_config, save_config, save_env_value
...
def __getattr__(name: str):
    ...
    Existing callers (and upstream tests) monkeypatch attributes on ``moor_cli.secrets_cli.bw`` directly.
    ...
    so ``import moor_cli.secrets_cli`` stays crypto-free while ``secrets_cli.bw.find_bws`` still resolves.
    """
    if name == "bw":
        return _load_bw()
...
def cmd_setup(args: argparse.Namespace) -> int:
    ...
    console.print("[green]✓ Bitwarden Secrets Manager is enabled.[/green]  "
                  "Secrets will be pulled at the start of every Moor process.")
    console.print("  Status:  [cyan]moor secrets bitwarden status[/cyan]\n"
                  "  Refresh: [cyan]moor secrets bitwarden sync[/cyan]\n"
                  "  Disable: [cyan]moor secrets bitwarden disable[/cyan]")
    return 0
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if not enabled:
        console.print("\n  Run [cyan]moor secrets bitwarden setup[/cyan] to enable.")
        return 0
    if not token:
        console.print(f"\n  [yellow]Enabled but {token_env} is not set — Moor will skip BSM "
...
def cmd_token(args: argparse.Namespace) -> int:
    ...
    def verify(token: str) -> bool:
        ...
        if project_id and projects and project_id not in {p["id"] for p in projects}:
            console.print(
                ...
                "app or re-run `moor secrets bitwarden setup` to pick a different project.[/yellow]")
        return True
    ...
    return rotate_token(
        ...
        disabled_note=None if bw_cfg.get("enabled") else (
            ...
            "run `moor secrets bitwarden setup` (or set "
...
def cmd_sync(args: argparse.Namespace) -> int:
    ...
    if not args.apply:
        console.print("\n  This was a dry-run — secrets are picked up automatically on the "
                      "next [cyan]moor[/cyan] invocation.  Re-run with [cyan]--apply[/cyan] "
...
_PROJECT_LIST_HINTS = (
    (("invalid_client", "400 bad request"),
     ...
     "[cyan]moor secrets bitwarden setup[/cyan] and pick EU or "
