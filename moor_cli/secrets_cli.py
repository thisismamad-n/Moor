...
# that process is ``moor update``, the self-lock preflight detects the
# loaded native module and defers (#86781).  Keep the backend import lazy:
# this module is registered parse-time from ``moor_cli.main`` and must not
...
def __getattr__(name: str):
    ...
    ``moor_cli.secrets_cli.bw`` directly.  Resolving that attribute at
    ...
    the first actual attribute access, so ``import moor_cli.secrets_cli``
...
def cmd_setup(args: argparse.Namespace) -> int:
    ...
    console.print(
        "  Status:  [cyan]moor secrets bitwarden status[/cyan]\n"
    ...
    if not args.no_verify:
        ...
        if project_id and projects and project_id not in {p["id"] for p in projects}:
            console.print(
                ...
                "app or re-run `moor secrets bitwarden setup` to pick a "
...
def cmd_sync(args: argparse.Namespace) -> int:
    ...
    if not bw_cfg.get("enabled"):
        console.print(
            ...
            "`moor secrets bitwarden setup` first.[/yellow]"
        )
        return 1
