"""Post-update maintenance for ``moor update``: pre-update backup snapshot, state-db verify/restore, curator/FTS notices, FHS path guard, completion summary, stale-module purge.
...
_UPDATE_RUNTIME_RELOAD_MODULES = "moor_constants", "tools.environments.local", "tools.lazy_deps"
...
_STALE_PURGE_PREFIXES = "moor_cli", "gateway", "tools", "tui_gateway", "agent"
...
_STALE_PURGE_PROTECTED = frozenset({"moor_cli", "moor_cli.main", "moor_cli.moor_logging"})
...
_STALE_PURGE_PROTECTED_PREFIX = "moor_cli.update_"
...
def _purge_stale_moor_modules() -> None:
    ...
    with _best_effort('Could not purge stale Moor modules: %s'):
        importlib.invalidate_caches()
        ...
        if purged:
            logger.debug("Purged %d stale Moor module(s) after checkout update", len(purged))
...
def _print_curator_recent_run_notice() -> None:
    ...
    if "\n" in summary:
        ...
        for line in summary.splitlines():
            print(f"  {line}")
        print("  (This message shows once per curator run. View anytime: moor curator status)")
...
def _reload_process_scan_modules() -> None:
    ...
    ``_scan_dashboard_processes`` does a function-level ``from moor_cli._subprocess_compat import
...
def _finish_dashboard_update_cleanup(
    ...
    *already_restarted_units*: systemd unit names (no ``.service``) the fleet-restart loop
    ...
    See #83595.
    """
    from moor_cli.update_cmd import _m, _reload_process_scan_modules
    if node_failures:
        print()
...
def _print_update_completion(message: str) -> None:
    ...
    See #47359, #58764.
    """
    from moor_cli.update_cmd import _branch_head_suffix
...
def _print_verified_update_completion(message: str) -> bool:
    """Print a success completion only after probing the next Moor runtime."""
    from moor_cli.update_cmd import _post_update_sqlite_runtime_status
    ...
    print("  Rebuild the Moor venv with a uv-managed Python, restart Moor, then verify with `moor doctor`.")
    return False
...
def _print_update_summary(*, node_failures: list, desktop_build_ok: bool, pre_update_version: str | None) -> bool:
    ...
    See #88251.
    """
    from moor_cli.update_cmd import _post_update_sqlite_runtime_status, _update_complete_message
    sqlite_runtime_ok, sqlite_info = _post_update_sqlite_runtime_status()
    if sqlite_info is None:
    ...
    if node_failures or not desktop_build_ok or not sqlite_runtime_ok:
        ...
        if not sqlite_runtime_ok:
            print(
                "  The Python runtime remediation did not complete. Run `moor "
                "update` again; if SQLite is unchanged, rebuild the Moor venv "
                "with a uv-managed Python, restart Moor, then verify with "
                "`moor doctor`."
            )
    else:
...
def _restore_state_db_from_snapshot(state_path: Path, snap_state: Path) -> bool:
    ...
    """
    from moor_cli.backup import _foreign_db_holder_pids, verify_sqlite_integrity
    ...
    if holders:
        print(
            ...
            "state.db or its WAL open. Stop them (moor gateway stop), "
    ...
    try:
        ...
        print(
            ...
            "handles (or restart Moor) and retry."
        )
        return False
...
def _verify_and_restore_state_dbs_post_update() -> None:
    ...
    with _best_effort('Sibling-profile state.db guard sweep failed: %s'):
        from moor_cli.backup import _sibling_profile_homes
...
def _print_bundled_skills_sync_report() -> None:
    ...
    if result.get("user_modified"):
        ...
        print("    → see them: moor skills list-modified  (diff/reset to resume updates)")
...
def _ensure_fhs_path_guard() -> None:
    ...
    no-op on non-Linux/non-root/non-FHS or when ``bash -i -c 'command -v moor'`` resolves."""
    from moor_cli.update_cmd import _m
    ...
    try:
        ...
        return
    # Only for FHS-layout installs (link at /usr/local/bin/moor).
    ...
    try:
        probe = subprocess.run(
            ...
            # recovery path in `moor gateway restart` (`systemd_restart()`) as of PR #20949.
            capture_output=True,
...
def _ensure_acp_launcher() -> None:
    r"""Self-heal a ``moor-acp`` launcher next to ``moor`` (mirrors install.sh): ACP hosts
    resolve it on the login-shell PATH but the console script lives in the venv. The shim
    delegates to the sibling ``moor acp``, correct for every layout.

    No-op on Windows (install.ps1 stages launchers into ``$MoorHome\bin``, never
...
def _verify_state_db_after_snapshot(snapshot_id: str) -> None:
    ...
    _src_path = get_moor_home() / "state.db"
    ...
    _snap_state = _quick_snapshot_root(get_moor_home()) / snapshot_id / "state.db"
...
def _run_quick_snapshots() -> Optional[str]:
    ...
    from moor_cli.update_cmd import _record_update_step
    ...
    with _best_effort('Sibling profile snapshots failed: %s'):
        ...
        if _sibling_snaps:
            ...
            import moor_cli.update_cmd_config as _cfg
...
def _run_full_backup() -> None:
    """Zip MOOR_HOME under ``backups/`` (restorable via ``moor import``). Never raises."""
    try:
    ...
    # display_moor_home so the user sees ~/.moor/...
    try:
        from moor_constants import get_moor_home, display_moor_home
        display_path = f"{display_moor_home()}/{out_path.relative_to(get_moor_home())}"
    except Exception:
...
def _run_pre_update_backup(args) -> Optional[str]:
    ...
    ``full`` — quick snapshot PLUS a zip of MOOR_HOME under ``backups/`` (``moor import``).
...
def _sweep_bytecode_after_update(branch: str) -> None:
    ...
    # symbols in moor_constants or lazy_deps. The dependency install above may also have regenerated
...
def _sync_profiles_after_update() -> None:
    ...
    # MOOR_HOME, so sync_skills()'s module-level MOOR_HOME cache can't skew it.
    with suppress(Exception):
        from moor_cli.profiles import list_profiles, seed_profile_skills
...
def _print_plugin_compat_notice() -> None:
    ...
    from moor_cli.plugin_compat import compat_report, removal_in_effect, summary_lines
...
def _print_post_update_notices_and_self_heals() -> None:
    ...
    for message, step in (
        ...
        ('moor-acp launcher self-heal failed: %s', _ensure_acp_launcher),
...
def _run_post_update_maintenance(
    *, assume_yes, gateway_mode, pre_update_snapshot_id, had_desktop_app_before_update, node_failures, desktop_build_ok,
    ...
    if sys.platform == "darwin" and had_desktop_app_before_update:
        print()
        print(
            "  ℹ macOS: if Moor re-prompts for permissions you already "
            "granted (toggle shows ON), the stored grant is stale — run "
            "`tccutil reset ScreenCapture com.moorinc.moor` (repeat "
    ...
    try:
        # See #95596.
        from moor_cli.macos_tcc_anchor import ensure_tcc_anchor
        ensure_tcc_anchor()
    except Exception:
