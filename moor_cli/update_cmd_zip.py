"""ZIP-download fallback for ``moor update`` (Windows with broken git): two-phase stage/commit swap, dirty-tree guard.

Split out of ``update_cmd.py``; every name is re-imported there so ``moor_cli.update_cmd.<name>`` keeps
...
_STASH_HINT = "  Stash or commit your changes, then rerun `moor update`."
...
def _atomic_replace_dir(src: str, dst: str) -> None:
    ...
    Thin alias over the two-phase helpers; retained for the ``moor_cli.main`` re-export surface.
...
def _extracted_root(tmp_dir: str, branch: str) -> str:
    """GitHub ZIPs extract to ``moor-agent-<branch>/``; fall back to the first non-``__MACOSX`` dir."""
...
def _require_staging_space(extracted: str, entries: list[str], project_root: str) -> None:
    ...
    need = sum(
        ...
        # root holds 20 first-party modules (run_agent.py, cli.py, moor_constants.py, ...). Check up front
...
def _stage_entries(extracted: str, entries: list[str], project_root: str) -> list[tuple[str, str]]:
    ...
    try:
        for item in entries:
            ...
            # desktop app, win-unpacked/ Moor.exe) exists only in the LIVE tree. Graft the live release
