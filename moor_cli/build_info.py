...
Source installs report their git revision live via ``git rev-parse`` (``moor_cli/dump.py``,
``moor_cli/banner.py``). Inside the published Docker image ``.dockerignore`` excludes ``.git``,
so those callsites fall back to the ``.moor_build_sha`` file written by the Dockerfile.
"""
...
def get_code_identity(refresh: bool = False) -> dict:
    ...
    ``.moor_build_sha`` for Docker images, else unknown. Cached per process — code identity
...
def get_build_sha(short: int = 8) -> Optional[str]:
    """Baked-in build SHA (Dockerfile ``MOOR_GIT_SHA`` build-arg, full 40-char hash on one
