...
One read-only pass that answers, BEFORE any mutation: what Moor runtimes
...
`detect_install_method`) — so `moor update --plan` can run on a live
...
Deployment kinds (the concept most fleet-update bugs were missing):
    ...
    git      — source checkout; updatable in place via `moor update`
...
Supervisors (how a runtime is restarted after code changes):
    ...
    desktop           — Desktop app supervises `moor serve`; it respawns
...
def describe_restart_mechanism(mechanism: str, profile: str) -> str:
    ...
    if profile != "default":
        return f"moor -p {profile} gateway restart"
    ...
    try:
        ...
        for runtime in plan.runtimes:
            ...
            if r.profile in relaunched or r.profile in external:
                ...
                r.profile in unit or (r.profile == "default" and "moor-gateway" in unit)
                for unit in failed_set
            ):
                outcome = "failed"
            elif any(
                r.profile in svc or (r.profile == "default" and "moor-gateway" in svc)
