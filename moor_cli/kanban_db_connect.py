...
def _resolve_busy_timeout_ms() -> int:
    ...
    return _kb._env_int("MOOR_KANBAN_BUSY_TIMEOUT_MS", DEFAULT_BUSY_TIMEOUT_MS, minimum=1)
...
def _dispatch_tick_lock(db_path: Path):
    ...
    Motivation (issue #35240): a ``moor gateway run --replace`` / ``gateway restart`` invoked from a shell
...
def repair_db(db_path: Optional[Path] = None, *, board: Optional[str] = None) -> RepairResult:
    ...
    ``moor kanban repair`` can pick its exit code. ``OperationalError``
