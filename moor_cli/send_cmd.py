"""CLI subcommand: ``moor send`` — pipe text from shell scripts to any
...
def _read_message_body(
    ...
    if file_path:
        ...
        try:
            ...
            print(
                f"moor send: {file_path} is not a text file. --file reads the "
                ...
                f'  moor send --to telegram "MEDIA:{file_path}"\n'
                f'  moor send --to telegram "optional caption MEDIA:{file_path}"\n'
                "Add [[as_document]] to deliver an image as an uncompressed file:\n"
                f'  moor send --to telegram "[[as_document]] MEDIA:{file_path}"',
                file=sys.stderr,
            )
            sys.exit(_USAGE_EXIT)
        except OSError as exc:
            print(f"moor send: cannot read {file_path}: {exc}", file=sys.stderr)
            sys.exit(_USAGE_EXIT)
    ...
    *,
    ...
    if json_mode:
        ...
        if payload.get("error"):
            print(f"moor send: {payload['error']}", file=sys.stderr)
...
def _list_targets(platform_filter: Optional[str], *, json_mode: bool) -> int:
    ...
    try:
        ...
        print(f"moor send: failed to load channel directory: {exc}", file=sys.stderr)
        return _FAILURE_EXIT

    try:
        raw = load_directory()
    except Exception as exc:
        print(f"moor send: failed to read channel directory: {exc}", file=sys.stderr)
        return _FAILURE_EXIT
    ...
    # setup used only for outbound `moor send`) would otherwise be
    ...
    if platform_filter:
        ...
        if not filtered:
            print(
                f"moor send: no targets found for platform '{platform_filter}'. "
    ...
    if not platforms:
        ...
        print("channel discovery can populate ~/.moor/channel_directory.json.")
        return _SUCCESS_EXIT
...
def _load_moor_env() -> None:
    """Populate ``os.environ`` from ``~/.moor/.env`` AND bridge top-level
    ...
    startup that ``moor send`` must replicate when invoked standalone:

    1. ``load_dotenv(~/.moor/.env)`` — brings bot tokens into the env.
    2. Bridge top-level simple values from ``~/.moor/config.yaml`` into
    ...
    intentionally reimplement the minimum needed here so ``moor send``
...
def cmd_send(args: argparse.Namespace) -> None:
    ...
    # Bridge ~/.moor/.env and ~/.moor/config.yaml into os.environ so the
    ...
    if not target:
        print(
            "moor send: --to PLATFORM[:channel[:thread]] is required\n"
            "Examples:\n"
            "  moor send --to telegram \"hello\"\n"
            "  moor send --to discord:#ops --file report.md\n"
            "  moor send --list      # list available targets",
            file=sys.stderr,
        )
        sys.exit(_USAGE_EXIT)
    ...
    if message is None or not message.strip():
        print(
            "moor send: no message provided. Pass text as a positional "
    ...
    # Import lazily so `moor send --help` stays fast and does not pull in
...
def register_send_subparser(subparsers) -> argparse.ArgumentParser:
    ...
    parser = subparsers.add_parser(
        ...
        description=(
            ...
            "credentials (~/.moor/.env + ~/.moor/config.yaml) — no LLM, "
        ...
        epilog=(
            ...
            "  moor send --to telegram \"MEDIA:/tmp/chart.png\"   # send a media attachment\n"
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="List available targets. Optional positional filter: `moor send --list telegram`.",
    )
