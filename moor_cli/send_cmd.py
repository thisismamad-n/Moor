"""CLI subcommand: ``moor send`` — pipe text from shell scripts to any configured messaging platform
...
def _read_message_body(positional: Optional[str], file_path: Optional[str]) -> Optional[str]:
    ...
    if file_path:
        ...
        try:
            ...
            _fail(f"moor send: cannot read {file_path}: {exc}", _USAGE_EXIT)
...
def _list_targets(platform_filter: Optional[str], *, json_mode: bool) -> int:
    ...
    try:
        ...
        return _fail(f"moor send: failed to load channel directory: {exc}")
    try:
        raw = load_directory()
    except Exception as exc:
        return _fail(f"moor send: failed to read channel directory: {exc}")
...
def _load_moor_env() -> None:
    """Populate ``os.environ`` from ``~/.moor/.env`` AND bridge top-level ``config.yaml`` keys into
...
def cmd_send(args: argparse.Namespace) -> None:
    ...
    _load_moor_env()  # the downstream gateway config loader reads credentials from os.environ
    if getattr(args, "list_targets", False):  # --list short-circuits everything else
        # `moor send --list telegram` lands "telegram" in the `message` positional.
    ...
    # Lazy import keeps `moor send --help` fast (no tool registry / gateway config stack).
