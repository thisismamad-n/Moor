...
_restart_gateway_after_whatsapp_onboarding = late("_restart_gateway_after_whatsapp_onboarding", "moor_cli.web_server_messaging")
_telegram_onboarding_request_sync = late("_telegram_onboarding_request_sync", "moor_cli.web_server_messaging")
_whatsapp_session_path = late("_whatsapp_session_path", "moor_cli.web_server_messaging")
...
remove_env_value = late("remove_env_value", "moor_cli.config")
...
_MESSAGING_ENV_FALLBACKS: dict[str, dict[str, Any]] = {
    ...
    for key, description, prompt, extra in (
        ...
        ("WEIXIN_ACCOUNT_ID", "iLink Bot account ID obtained through QR login in moor gateway setup", "iLink Bot account ID", {}),
        ("WEIXIN_TOKEN", "iLink Bot token obtained through QR login in moor gateway setup", "iLink Bot token", {"password": True}),
...
def _messaging_platform_payload(
    ...
    # and do NOT follow the MOOR_HOME contextvar override, so without it messaging
...
async def apply_telegram_onboarding(pairing_id: str, body: TelegramOnboardingApply, profile: Optional[str] = None):
    ...
    # saved token waiting on a manual restart click reads as "Moor is broken" from the
...
async def get_messaging_platforms(profile: Optional[str] = None):
    ...
    def _run():
        ...
        # credentials/state, not the root install's. load_env() honors the MOOR_HOME contextvar override;
        ...
        with _profile_scope(profile) as scoped_dir:
            return {
                ...
                "gateway_start_command": " ".join(["moor", *_gateway_subcommand(profile, "start")]),
...
def _multiplex_port_binding_conflict(platform_id: str, requested_profile: Optional[str]) -> Optional[str]:
    ...
    if not requested or requested.lower() == "current":
        ...
        # The dashboard's own profile. "custom" (unrecognized MOOR_HOME) is outside
