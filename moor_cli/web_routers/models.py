...
def _moor_recommended_default() -> dict:
    ...
    model_ids = m.get_curated_moor_model_ids()
    pricing = mp.get_pricing_for_provider("moor") or {}
    free_tier = m.check_moor_free_tier(force_fresh=True)
    ...
    if free_tier:
        model_ids, _unavailable = m.partition_moor_models_by_tier(model_ids, pricing, free_tier=True)

    model = m.pick_silent_default_model(model_ids, provider="moor")
...
def get_recommended_default_model(provider: str = ""):
    ...
    ``moor model``'s curation so GUI onboarding lands on a sensible default.
    Moor honors the user's free/paid tier. Any other provider gets the preferred
    ...
    Moor; ``model`` may be empty (caller degrades gracefully)."""
    slug = (provider or "").strip().lower()

    if slug == "moor":
        try:
            return _moor_recommended_default()
        except Exception:
...
async def set_model_assignment(body: ModelAssignment, profile: Optional[str] = None):
    ...
    ``~/.moor/config.yaml`` — applies to **new** sessions only; a running chat
