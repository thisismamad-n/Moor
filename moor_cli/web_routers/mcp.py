...
web_server - reached via the late-binding seam in :mod:`moor_cli.web_deps`
...
async def test_mcp_server(name: str, profile: Optional[str] = None):
    ...
    def _probe_scoped():
        ...
        # The probe touches no skills globals; it only needs the MOOR_HOME
...
async def auth_mcp_server(name: str, request: Request, profile: Optional[str] = None):
    ...
    process_home = str(get_moor_home().expanduser().resolve(strict=False))

    def _read():
        with _profile_scope(profile):
            return _get_mcp_servers(), str(get_moor_home().expanduser().resolve(strict=False))
...
async def list_mcp_catalog(profile: Optional[str] = None):
    """Browse the Moor-approved MCP catalog (the optional-mcps/ manifests).
    ...
    `moor mcp catalog` / `moor mcp install` read.  ``profile`` scopes
...
async def install_mcp_catalog_entry(body: MCPCatalogInstall, profile: Optional[str] = None):
    ...
    # The -p subprocess rebinds MOOR_HOME-derived paths in the child.
