...
_run_dashboard_mcp_oauth = late("_run_dashboard_mcp_oauth", "moor_cli.web_server_mcp")
...
async def add_mcp_server(body: MCPServerCreate, profile: Optional[str] = None):
    from moor_cli.mcp_config import _get_mcp_servers, _save_bearer_auth_token, _save_mcp_server
...
async def replace_mcp_servers(body: MCPServersReplace, profile: Optional[str] = None):
    ...
    from moor_cli.mcp_config import _replace_mcp_servers
...
async def test_mcp_server(name: str, profile: Optional[str] = None):
    ...
    from moor_cli.mcp_config import _get_mcp_servers, _oauth_tokens_present, _probe_single_server
    ...
    def _probe_scoped():
        ...
        # endpoint. The probe only needs MOOR_HOME for .env + token resolution.
...
async def auth_mcp_server(name: str, request: Request, profile: Optional[str] = None):
    ...
    def _home() -> str:
        return str(get_moor_home().expanduser().resolve(strict=False))
    ...
    with _mcp_oauth_flows_lock:
        ...
        if any(f.server_name == name and f.moor_home == flow_home for f in live):
...
async def install_mcp_catalog_entry(body: MCPCatalogInstall, profile: Optional[str] = None):
    ...
    if entry.install is not None:
        ...
        try:
            _spawn_moor_action(_profile_cli_args(effective_profile) + ["mcp", "install", name], action)
        except HTTPException:
            raise
        except Exception as exc:
