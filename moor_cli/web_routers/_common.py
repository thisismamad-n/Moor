...
late-binding seam in :mod:`moor_cli.web_deps` (web_server owns helpers/state;
...
def spawn_profile_action(
    ...
    """Spawn a background ``moor -p <profile> <argv>`` action; a spawn
...
# instead of a 500. The file is never quarantined or renamed from here — that is `moor doctor`'s job.
...
def corrupt_store_as_status(db_path):
    ...
    from moor_state_errors import is_malformed_db_error
