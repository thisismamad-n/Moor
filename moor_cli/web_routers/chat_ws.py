...
_active_session_file_for_channel = late("_active_session_file_for_channel", "moor_cli.web_server_chat")
_profile_scope = late("_profile_scope", "moor_cli.web_server_profiles")
_resolve_chat_argv_async = late("_resolve_chat_argv_async", "moor_cli.web_server_chat")
_resolve_profile_dir = late("_resolve_profile_dir", "moor_cli.web_server_profiles")
_ws_auth_reason = late("_ws_auth_reason", "moor_cli.web_server_chat")
_ws_client_reason = late("_ws_client_reason", "moor_cli.web_server_chat")
_ws_host_origin_reason = late("_ws_host_origin_reason", "moor_cli.web_server_chat")
...
def _ws_auth_mode() -> str:
    ...
    from moor_cli.web_server_chat import _LOOPBACK_HOSTS
        ...
        try:
            ...
            if command_id == command_generation:
                ...
                await out.error_then_complete(
                    "Command timed out. Moor Console returned to the prompt.", line, command_id, "timeout",
                )
        except Exception as exc:
            if command_id == command_generation:
                pending_confirmation = None
...
# (MOOR_TUI_SIDECAR_URL from /api/pty's env) and writes every dispatcher emit
