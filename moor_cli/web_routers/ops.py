...
from moor_cli.web_server_files import _path_is_under
...
from moor_cli.web_routers._common import _CONFIG_MUTATION_LOCK, http_failure, spawn_profile_action
...
def _spawn_action(argv: List[str], name: str, *, log_msg: str, prefix: str) -> dict:
    """Spawn a dashboard-profile ``moor <argv>`` action; spawn failure -> 500."""
...
# --- Webhooks: same JSON store as the CLI (moor_cli.webhook); the adapter
...
# --- Gateway lifecycle: spawn the real `moor gateway <verb>` so behaviour
    ...
    def _run():
        try:
            ...
            pool.add_entry(PooledCredential(
                ...
                # every added account into the latest login: a second ``moor auth add openai-codex``
            ...
            # (mirrors `moor auth add`).
            if not provider.startswith(CUSTOM_POOL_PREFIX):
                try:
                    from moor_cli.auth import _load_auth_store, unsuppress_credential_source
...
async def remove_credential_pool_entry(provider: str, index: int):
    ...
    the same RemovalStep registry as ``moor auth remove``: each source cleans
...
async def run_backup(body: BackupRequest):
    ...
    if not output:
        ...
        archive = _dashboard_backup_dir() / f"moor-backup-{stamp}-{secrets.token_hex(4)}.zip"
        try:
...
async def create_hook(body: HookCreate):
    ...
    valid_hooks = None
    with contextlib.suppress(Exception):
        from moor_cli.plugins import VALID_HOOKS as valid_hooks
