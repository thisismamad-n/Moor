"""Gateway restart/drain, Moor update and background-action status dashboard routes.

Extracted from ``moor_cli.web_server``; helpers/state that tests monkeypatch on
...
def _server_path(name: str) -> Path:
    ...
    import moor_cli.web_server_gateway as gw
...
_UPDATE_ACTION_COMPLETED_RE = re.compile(r"^=== moor-update completed ([0-9a-f]{32}) ===$")

_MANAGED_EXTERNALLY_MESSAGE = "Moor updates are managed outside this dashboard in containerized environments."
...
def _update_refused(error: str, message: str, update_command: str) -> Dict[str, Any]:
    ...
    return {
        "ok": False, "pid": None, "name": "moor-update", "error": error, "message": message,
...
async def update_moor():
    ...
    if existing is not None and existing.poll() is None:
        response = {"ok": True, "pid": existing.pid, "name": "moor-update", "already_running": True}
    ...
    with http_failure("Failed to spawn moor update", 500, "Failed to start update"):
        proc = _spawn_moor_action(["update"], "moor-update", env_overrides={"MOOR_ACTION_ID": action_id})
    return {"ok": True, "pid": proc.pid, "name": "moor-update", "action_id": action_id}
...
_NON_APPLYABLE_MESSAGES = {
    ...
    "apt": lambda: "Moor is managed by Termux APT; run `pkg upgrade moor-agent`.",
}
...
def _latest_update_receipt_summary() -> Optional[Dict[str, Any]]:
    """Compact summary of the latest receipt (written by EVERY ``moor update`` run,
    incl. refused/failed), or None; never raises. Steps/skips stay in the full endpoint.

    Phase-1 bullet 3 (#91277): the receipt (written by EVERY ``moor update`` run since #91283, including
...
@status_router.get("/api/moor/update/receipt")
async def get_update_receipt():
    ...
    receipt = _read_latest_receipt()
    if not receipt:
        raise HTTPException(status_code=404, detail="No update receipt found (no `moor update` run recorded).")
