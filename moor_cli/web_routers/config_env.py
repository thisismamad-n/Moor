...
from moor_cli.config import DEFAULT_CONFIG, OPTIONAL_ENV_VARS, read_raw_config, custom_endpoint_key_env, coerce_provider_id, find_provider_entry, redact_key, _deep_merge
...
_channel_managed_env_keys = late("_channel_managed_env_keys", "moor_cli.web_server_messaging")
...
def _catalog_provider_env_metadata() -> dict:
    ...
    (the ``moor model`` universe), so the desktop Keys tab renders a card even
...
def _get_env_vars_sync(profile: Optional[str] = None):
    ...
    def _row(var_name: str, info: dict, *, custom: bool = False) -> dict:
        ...
        return {
            ...
            # by the SAME provider identity the CLI `moor model` picker uses.
...
def list_custom_endpoints(profile: Optional[str] = None):
    ...
    rather than the process-level MOOR_HOME (mirrors ``/api/config``).
    """
