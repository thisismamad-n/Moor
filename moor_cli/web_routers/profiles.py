...
Shared helpers are reached via the late-binding seam in :mod:`moor_cli.web_deps`
...
from moor_cli.web_server_config import _apply_main_model_assignment, _normalize_main_model_assignment
...
async def create_profile_endpoint(body: ProfileCreate):
    ...
    def _spawn_install(ident: str):
        return _spawn_moor_action(["-p", body.name, "skills", "install", ident, "--yes"],
...
async def get_active_profile_endpoint():
    """``active`` is the sticky default written by ``moor profile use`` (what new CLI
...
async def set_active_profile_endpoint(body: ProfileActiveUpdate):
    """Set the sticky active profile (mirrors ``moor profile use``); does not retarget the
...
async def update_profile_model_endpoint(name: str, body: ProfileModelUpdate):
    ...
    own profile — ``POST /api/model/set`` (main scope) via the MOOR_HOME override."""
    profile_dir = _resolve_profile_dir(name)
    provider = (body.provider or "").strip()
    model = (body.model or "").strip()
...
async def describe_profile_auto_endpoint(name: str, body: ProfileDescribeAuto):
    """Auto-generate a profile's description via the auxiliary LLM (mirrors ``moor profile
...
# ── Export / Import ── wraps moor_cli.profiles.export_profile / import_profile. Paths are
