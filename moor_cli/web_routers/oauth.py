...
from moor_cli.web_server_oauth import (
    _external_process_cli_command, _minimax_poller, _moor_poller, _oauth_profile_name, _oauth_sessions, _oauth_sessions_lock, _truncate_token, _xai_device_poller,
)
...
def _codex_request_user_code(httpx) -> Dict[str, Any]:
    ...
    from moor_cli.auth import CODEX_OAUTH_CLIENT_ID
...
def _codex_exchange_tokens(httpx, code_resp: Dict[str, Any]) -> Dict[str, str]:
    ...
    from moor_cli.auth import CODEX_OAUTH_CLIENT_ID, CODEX_OAUTH_TOKEN_URL
...
# moor: refresh-free local snapshot so listing providers never performs an OAuth
...
_DEVICE_CODE_STARTERS = {
    "moor": _start_moor_device_code, "openai-codex": _start_codex_device_code,
...
def _oauth_provider_disconnect_hint(provider: Dict[str, Any], status: Dict[str, Any]) -> Optional[str]:
    ...
    # "anthropic" is flow == "external" (no in-dashboard login) but Moor still
    # OWNS its credential (the PKCE file ~/.moor/.anthropic_oauth.json and its
    # credential-pool entry, written by `moor auth add anthropic`), so it is
...
def _build_oauth_catalog() -> list[Dict[str, Any]]:
    ...
    in ``moor model`` order, so plugin-added OAuth/external providers appear automatically."""
    ...
    try:
        ...
        for d in provider_catalog():
            ...
            rows.append({
                ...
                "cli_command": f"moor auth add {d.slug}", "docs_url": d.signup_url or "", "status_fn": None,
            })
    except Exception:
        pass
    return rows
...
def _clear_anthropic_auth() -> bool:
    """Clear only the moor-managed PKCE file and auth-store entry (never ~/.claude/*)."""
    cleared = False
    try:
        from agent.anthropic_credentials import _get_moor_oauth_file
