...
late-binding seam in :mod:`moor_cli.web_deps` (``late`` for callables,
...
from moor_cli.web_deps import late, LateState
...
_spawn_moor_action = late("_spawn_moor_action")
...
async def toggle_toolset(name: str, body: ToolsetToggle, profile: Optional[str] = None):
    ...
    # same background install `moor tools` runs interactively. Without this,
    ...
    # dead-end that forced users to discover `moor computer-use install`
            ...
            if cat:
                ...
                for prov in _visible_providers(cat, config, force_fresh=True):
                    ...
                    row = {
                        ...
                        # including logged-out Moor Subscription rows and never-run
...
async def get_toolset_models(
    ...
    The GUI counterpart of the model picker `moor tools` runs after a
...
async def select_toolset_provider(
    ...
    Managed Moor rows (``managed_moor_feature``) additionally report the
    Portal entitlement state: the CLI flow gates these selections on
    ``ensure_moor_portal_access`` (inline login), but the GUI has no inline
    ...
    ``managed_by_moor``). The response now carries an additive
    ``needs_moor_auth: true`` + ``feature`` so the client can drive the
    existing Moor Portal OAuth flow (``POST /api/providers/oauth/moor/start``)
    and refetch.
    """
    from moor_cli.tools_config import (
        TOOL_CATEGORIES,
    ...
    def _run():
        with _profile_scope(body.profile or profile):
            ...
            # Entitlement check for managed Moor rows — mirrors the gate the CLI
            # applies via ensure_moor_portal_access at selection time. This hits
...
async def save_toolset_env(name: str, body: ToolsetEnvUpdate, profile: Optional[str] = None):
    ...
    Writes each ``key: value`` to ``~/.moor/.env`` via ``save_env_value`` —
...
async def run_toolset_post_setup(
    ...
    ``profile`` spawns the hook as ``moor -p <profile> tools post-setup``.
    ...
    write per-profile state must see the same MOOR_HOME the rest of the
...
async def grant_computer_use_permissions(profile: Optional[str] = None):
    """Spawn ``moor computer-use permissions grant`` as a background action.
