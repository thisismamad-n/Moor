...
Extracted from ``moor_cli.web_server``; app state and helpers are late-bound through
:mod:`moor_cli.web_deps` (cycle-safe, monkeypatch-friendly).
"""
...
from moor_cli.web_server_gateway import _display_system_platform
...
from moor_cli.web_routers._common import scoped_to_thread
...
_collect_profile_gateway_topology_cached = late("_collect_profile_gateway_topology_cached", "moor_cli.web_server_gateway")
...
_load_configured_gateway_platforms = late("_load_configured_gateway_platforms", "moor_cli.web_server_gateway")
_probe_gateway_health = late("_probe_gateway_health", "moor_cli.web_server_gateway")
_require_token = late("_require_token")
_resolve_profile_dir = late("_resolve_profile_dir", "moor_cli.web_server_profiles")
_resolve_restart_drain_timeout = late("_resolve_restart_drain_timeout", "moor_cli.web_server_lifecycle")
...
def _count_status_active_sessions() -> int:
    ...
    /api/status never routinely writes to state.db while another Moor process uses it."""
...
async def get_ssh_ownership(request: Request):
    from moor_cli.web_server import _SSH_OWNER_NONCE
...
# status) are written to the per-profile home, not the process-level MOOR_HOME (see issue #69143). Plain
...
async def _resolve_gateway_status(profile_dir: Optional[Path], health_url) -> Dict[str, Any]:
    ...
    if liveness.runtime is not None:
        ...
        # profile is the process's own home (a pooled ``moor --profile X serve``).
        served_name = profile_dir.name if profile_dir is not None else profile_name_for_home(get_process_moor_home())
...
def _moor_session_validity() -> str:
    """Moor bootstrap-session validity for the NAS health sweep: a hosted agent whose Moor
    ...
    try:
        ...
        return get_moor_session_validity()
    except Exception:
        return "unknown"
...
async def _component_health(gateway: Dict[str, Any]) -> Dict[str, Any]:
    ...
    from moor_cli.web_server import DASHBOARD_HEALTH
    ...
    try:
        ...
        storage_check = await run_in_threadpool(_probe_state_db, get_moor_home())
...
async def get_status(profile: Optional[str] = None):
    ...
    """
    from moor_cli.web_server import _GATEWAY_HEALTH_URL
    status_scope = None
    ...
    try:
        ...
        # Off-loop: on a cold Windows install the first import of moor_cli.gateway blocks
        ...
        status = {
            ...
            **auth, "moor_session_valid": _moor_session_validity()}
        ...
        await _advisory_pressure(status, profile_dir if profile_dir else get_moor_home())

        # Profile NAMES and ``gateway_mode`` are low-sensitivity product surface (Moor Cloud
        ...
        if not auth["auth_required"]:
            status.update({
                "moor_home": str(get_moor_home()), "config_path": str(get_config_path()),
...
async def get_system_stats():
    ...
    disk/uptime when available). Non-sensitive: no env values, no paths beyond moor home."""
    ...
    info: Dict[str, Any] = {
        **_display_system_platform(
        ...
        "moor_version": __version__, "cpu_count": os.cpu_count()}
...
def _spawn_action(argv: list, name: str, prefix: str) -> dict:
    """Spawn a background ``moor <argv>`` action; a spawn failure is ``500 "<prefix>: <exc>"``."""
    try:
        proc = _spawn_moor_action(argv, name)
    except Exception as exc:
...
# Portal — Moor Portal auth + Tool Gateway routing status (read-only).
