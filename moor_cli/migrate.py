"""CLI handlers for ``moor migrate ...``.

Currently exposes only ``moor migrate xai`` — diagnoses and (with --apply) rewrites references to
...
def cmd_migrate(args: Any) -> int:
    """Dispatcher for ``moor migrate <subtype>``."""
    if getattr(args, "migrate_type", None) == "xai":
        return cmd_migrate_xai(args)

    print("usage: moor migrate xai [--apply] [--no-backup]", file=sys.stderr)
    return 2
...
def cmd_migrate_xai(args: Any) -> int:
    ...
    if not apply:
        ...
        print(color(
            "Re-run with `moor migrate xai --apply` to rewrite "
    ...
    print()
    print(color("Run `moor doctor` to confirm no retired xAI models remain.", Colors.DIM))
    return 0
