...
``moor_cli.config.migrate_config``. Each step is a function
...
``moor_cli.config`` imports :func:`run_migrations` lazily (inside
``migrate_config``), and every step function here resolves its helpers
(``read_raw_config``, ``_persist_migration``, ``get_env_value``, …) lazily
through the live ``moor_cli.config`` module object at call time via
:func:`_cfg`. There is deliberately NO module-level import of
``moor_cli.config`` here, so no circular import can form — and, just as
importantly, tests that monkeypatch helpers on ``moor_cli.config`` (e.g.
``patch("moor_cli.config.read_raw_config", ...)``) keep working, because
...
#: MOOR_TOOL_PROGRESS*) forever is not worth it. Below-floor configs are
...
def support_floor_message() -> str:
    ...
    return (
        ...
        f"{display_moor_home()}/config.yaml and run `moor setup` to "
...
def _cfg():
    """Return the live ``moor_cli.config`` module (lazy, cycle-free)."""
...
def _migrate_to_21(results: Dict[str, Any], quiet: bool) -> None:
    ...
    read_raw_config = _c.read_raw_config
    _persist_migration = _c._persist_migration
    get_moor_home = _c.get_moor_home
    ...
    if "enabled" not in plugins_cfg:
        ...
        # Scan ``$MOOR_HOME/plugins/`` for currently installed user plugins.
        ...
        if not quiet:
            if grandfathered:
                ...
                print(
                    ...
                    "Use `moor plugins enable <name>` to activate."
                )
...
def _migrate_to_23(results: Dict[str, Any], quiet: bool) -> None:
    ...
    # settings in their `config.yaml`, and `moor curator status` has no
    ...
    #   3. Creates `~/.moor/logs/curator/` if missing (belt-and-suspenders
    #      on top of ensure_moor_home() — old profiles that predate this
    ...
    try:
        curator_dir = get_moor_home() / "logs" / "curator"
        ...
        if added_curator:
            ...
            if not quiet:
                print(
                    ...
                    f"({', '.join(added_curator)}) — edit via `moor config set`"
                )
        if added_aux:
            results["config_added"].append(
            ...
            if not quiet:
                print(
                    ...
                    f"({', '.join(added_aux)}) — edit via `moor config set`"
                )
...
# behavior set it to true explicitly via `moor config set`.
...
def _migrate_to_38(results: Dict[str, Any], quiet: bool) -> None:
    ...
    from moor_cli.relay_plugin_cutover import legacy_relay_plugin_keys
...
def _migrate_to_39(results: Dict[str, Any], quiet: bool) -> None:
    ...
    # period that has since ended server-side, leaving every Moor-signed-in
    ...
    # provider surface (`video_generate`, `moor tools` → Video Generation).
    ...
    if changed:
        ...
        if not quiet:
            print(
                ...
                "lists — video generation now lives under `moor tools` → "
                "Video Generation (Moor Subscription or FAL)."
            )
