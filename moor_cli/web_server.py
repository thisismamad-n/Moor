"""Moor Agent — Web UI server: FastAPI app assembly, auth/host middleware, ``start_server``.
...
Usage: ``python -m moor_cli.main web [--port 8080]``.
"""
...
from moor_cli.web_server_lifecycle import (  # noqa: E402
...
def _start_desktop_cron_ticker(stop_event: "threading.Event", interval: int = 60) -> None:
    ...
    The desktop spawns a ``moor dashboard`` backend, not a gateway, so without
...
async def _lifespan(app: "FastAPI"):
    ...
    # refuse with "restart required" after `moor update` replaced the code
    ...
    def _boot_local_runtime():
        try:
            from moor_cli.config import load_config
            from moor_cli.local_runtime.bootstrap import ensure_local_runtime
    ...
    try:
        ...
        try:
            from moor_cli.local_runtime.bootstrap import shutdown_local_runtime
...
# MOOR_DASHBOARD_SESSION_TOKEN; otherwise fresh per server start. It dies with
...
def _apply_ssh_owner_nonce(nonce: Optional[str]) -> None:
    ...
    if nonce:
        ...
        try:
            marker = os.path.join(purelib, f".moor-ssh-runtime-{nonce}")
...
from moor_cli.dashboard_auth.public_paths import PUBLIC_API_PATHS as _PUBLIC_API_PATHS
...
def _desktop_loopback_auth_exempt(
    ...
    ``MOOR_DESKTOP=1``, and an operator-minted credential (env token, SSH
    session token, or owner nonce).
    """
    return (
        host in _LOOPBACK_HOST_VALUES
        and os.environ.get("MOOR_DESKTOP") == "1"
        and bool(os.environ.get("MOOR_DASHBOARD_SESSION_TOKEN") or ssh_session_token or ssh_owner_nonce)
    )
...
from moor_cli import web_server_gateway as _gateway_mod  # noqa: E402
from moor_cli.web_server_gateway import _ACTION_LOG_FILES, _terminate_desktop_managed_gateway  # noqa: E402
from moor_cli.web_server_sessions import _auto_archive_ticker_loop  # noqa: E402
from moor_cli.web_server_chat import PTY_REGISTRY  # noqa: E402
from moor_cli.web_server_dashboard import (  # noqa: E402
...
# persisted under the ROOT Moor home (not the profile MOOR_HOME) so every
...
def _spawn_gateway_restart(profile: Optional[str] = None) -> Tuple[subprocess.Popen, bool]:
    """Spawn ``moor gateway restart``, reusing an in-flight or recent restart.
    ...
    proc = _gateway_mod._spawn_moor_action(subcommand, "gateway-restart")
...
from moor_cli.web_routers import (  # noqa: E402
...
def _no_auth_provider_message(host: str) -> str:
    ...
    try:
        ...
        if _moor_plugin.LAST_SKIP_REASON:
            skip_reasons.append(f"  • moor: {_moor_plugin.LAST_SKIP_REASON}")
    except Exception:
        pass
...
def _build_uvicorn_server(host: str, port: int, *, ssh_isolated: bool = False):
    ...
    if ssh_isolated:
        from moor_cli.web_server_idle_exit import (
...
def _on_server_started(
    ...
    *,
    ...
    def _reap_desktop_serves() -> None:
        from moor_cli.dashboard_procs import _reap_orphaned_desktop_local_serves

        _reap_orphaned_desktop_local_serves()

    def _reap_mcp_helpers() -> None:
        from moor_cli.process_identity import reap_orphaned_mcp_helpers
    ...
    # No-op for standalone `moor serve` (no MOOR_PARENT_PID).
    _start_parent_death_watchdog()
    ...
    if getattr(app.state, "ssh_isolated_clients", None) is not None:
        from moor_cli.web_server_idle_exit import DEFAULT_IDLE_GRACE_S, start_idle_watchdog
        try:
    ...
    # ACTUAL port — what lets `moor update` relaunch a manually-started serve
    # on its real endpoint (#63206).
    def _register_identity() -> None:
        from moor_cli.process_identity import attach_self_to_kill_on_close_job, register_self
    ...
    if start_mcp_discovery_after_bind:
        ...
        try:
            from moor_cli.mcp_startup import defer_background_mcp_discovery
...
def start_server(
    ...
    JSON-RPC/WS backend, no UI build, no SPA mount (``MOOR_SERVE_HEADLESS``).
...
_PLUGIN_COMPAT_LAZY = {
    'AudioTranscriptionRequest': ('moor_cli.web_models', 'AudioTranscriptionRequest'),
    'AutomationBlueprintInstantiate': ('moor_cli.web_models', 'AutomationBlueprintInstantiate'),
    'BackupRequest': ('moor_cli.web_models', 'BackupRequest'),
    'BulkDeleteSessions': ('moor_cli.web_models', 'BulkDeleteSessions'),
    'CONFIG_SCHEMA': ('moor_cli.web_server_config', 'CONFIG_SCHEMA'),
    'ChatImageUpload': ('moor_cli.web_models', 'ChatImageUpload'),
    'ConfigUpdate': ('moor_cli.web_models', 'ConfigUpdate'),
    'CredentialPoolAdd': ('moor_cli.web_models', 'CredentialPoolAdd'),
    'CronJobCreate': ('moor_cli.web_models', 'CronJobCreate'),
    'CronJobUpdate': ('moor_cli.web_models', 'CronJobUpdate'),
    'CuratorPause': ('moor_cli.web_models', 'CuratorPause'),
    'CustomEndpointUpdate': ('moor_cli.web_models', 'CustomEndpointUpdate'),
    'DEFAULT_CONFIG': ('moor_cli.config', 'DEFAULT_CONFIG'),
    'DebugShareRequest': ('moor_cli.web_models', 'DebugShareRequest'),
    'EnvVarDelete': ('moor_cli.web_models', 'EnvVarDelete'),
    'EnvVarReveal': ('moor_cli.web_models', 'EnvVarReveal'),
    'EnvVarUpdate': ('moor_cli.web_models', 'EnvVarUpdate'),
    'FontSetBody': ('moor_cli.web_models', 'FontSetBody'),
    'FsWriteText': ('moor_cli.web_models', 'FsWriteText'),
    'GitBranchSwitchBody': ('moor_cli.web_models', 'GitBranchSwitchBody'),
    'GitCommitBody': ('moor_cli.web_models', 'GitCommitBody'),
    'GitFileBody': ('moor_cli.web_models', 'GitFileBody'),
    'GitPathBody': ('moor_cli.web_models', 'GitPathBody'),
    'GitWorktreeAddBody': ('moor_cli.web_models', 'GitWorktreeAddBody'),
    'GitWorktreeRemoveBody': ('moor_cli.web_models', 'GitWorktreeRemoveBody'),
    'HookCreate': ('moor_cli.web_models', 'HookCreate'),
    'HookDelete': ('moor_cli.web_models', 'HookDelete'),
    'ImportRequest': ('moor_cli.web_models', 'ImportRequest'),
    'LearningNodeEdit': ('moor_cli.web_models', 'LearningNodeEdit'),
    'LearningNodeRef': ('moor_cli.web_models', 'LearningNodeRef'),
    'MCPCatalogInstall': ('moor_cli.web_models', 'MCPCatalogInstall'),
    'MCPEnabledToggle': ('moor_cli.web_models', 'MCPEnabledToggle'),
    'MCPServerCreate': ('moor_cli.web_models', 'MCPServerCreate'),
    'MCPServersReplace': ('moor_cli.web_models', 'MCPServersReplace'),
    'ManagedDirectoryCreate': ('moor_cli.web_models', 'ManagedDirectoryCreate'),
    'ManagedFileDelete': ('moor_cli.web_models', 'ManagedFileDelete'),
    'ManagedFileUpload': ('moor_cli.web_models', 'ManagedFileUpload'),
    'ManagedFilesPolicy': ('moor_cli.web_server_files', 'ManagedFilesPolicy'),
    'MemoryProviderConfigUpdate': ('moor_cli.web_models', 'MemoryProviderConfigUpdate'),
    'MemoryProviderSelect': ('moor_cli.web_models', 'MemoryProviderSelect'),
    'MemoryProviderSetupRequest': ('moor_cli.web_models', 'MemoryProviderSetupRequest'),
    'MemoryReset': ('moor_cli.web_models', 'MemoryReset'),
    'MessagingPlatformUpdate': ('moor_cli.web_models', 'MessagingPlatformUpdate'),
    'MoaConfigPayload': ('moor_cli.web_models', 'MoaConfigPayload'),
    'MoaModelSlot': ('moor_cli.web_models', 'MoaModelSlot'),
    'MoaPresetPayload': ('moor_cli.web_models', 'MoaPresetPayload'),
    'ModelAssignment': ('moor_cli.web_models', 'ModelAssignment'),
    'OAuthSubmitBody': ('moor_cli.web_models', 'OAuthSubmitBody'),
    'OPTIONAL_ENV_VARS': ('moor_cli.config', 'OPTIONAL_ENV_VARS'),
    'PairingApprove': ('moor_cli.web_models', 'PairingApprove'),
    'PairingRevoke': ('moor_cli.web_models', 'PairingRevoke'),
    'ProfileActiveUpdate': ('moor_cli.web_models', 'ProfileActiveUpdate'),
    'ProfileCreate': ('moor_cli.web_models', 'ProfileCreate'),
    'ProfileDescribeAuto': ('moor_cli.web_models', 'ProfileDescribeAuto'),
    'ProfileDescriptionUpdate': ('moor_cli.web_models', 'ProfileDescriptionUpdate'),
    'ProfileModelUpdate': ('moor_cli.web_models', 'ProfileModelUpdate'),
    'ProfileRename': ('moor_cli.web_models', 'ProfileRename'),
    'ProfileSoulUpdate': ('moor_cli.web_models', 'ProfileSoulUpdate'),
    ...
    'PtyBridge': ('moor_cli.pty_bridge', 'PtyBridge'),
    'PtySessionRegistry': ('moor_cli.pty_session', 'PtySessionRegistry'),
    'PtyUnavailableError': ('moor_cli.pty_bridge', 'PtyUnavailableError'),
    ...
    'detect_install_method': ('moor_cli.config', 'detect_install_method'),
    'disconnect_oauth_provider': ('moor_cli.web_routers.oauth', 'disconnect_oauth_provider'),
    'download_dashboard_backup': ('moor_cli.web_routers.ops', 'download_dashboard_backup'),
    'download_managed_file': ('moor_cli.web_routers.files', 'download_managed_file'),
    'enable_webhooks': ('moor_cli.web_routers.ops', 'enable_webhooks'),
    'env_var_enabled': ('utils', 'env_var_enabled'),
    'events_ws': ('moor_cli.web_routers.chat_ws', 'events_ws'),
    'export_session_endpoint': ('moor_cli.web_routers.sessions', 'export_session_endpoint'),
    'find_provider_entry': ('moor_cli.config', 'find_provider_entry'),
    'format_docker_update_message': ('moor_cli.config', 'format_docker_update_message'),
    'fs_default_cwd': ('moor_cli.web_routers.files', 'fs_default_cwd'),
    'fs_download': ('moor_cli.web_routers.files', 'fs_download'),
    'fs_git_root': ('moor_cli.web_routers.files', 'fs_git_root'),
    'fs_list': ('moor_cli.web_routers.files', 'fs_list'),
    'fs_read_data_url': ('moor_cli.web_routers.files', 'fs_read_data_url'),
    'fs_read_text': ('moor_cli.web_routers.files', 'fs_read_text'),
    'fs_write_text': ('moor_cli.web_routers.files', 'fs_write_text'),
    'gateway_drain': ('moor_cli.web_routers.actions', 'gateway_drain'),
    'gateway_ws': ('moor_cli.web_routers.chat_ws', 'gateway_ws'),
    'get_action_status': ('moor_cli.web_routers.actions', 'get_action_status'),
    'get_active_profile_endpoint': ('moor_cli.web_routers.profiles', 'get_active_profile_endpoint'),
    'get_auxiliary_models': ('moor_cli.web_routers.models', 'get_auxiliary_models'),
    'get_client_voice_config': ('moor_cli.web_routers.audio', 'get_client_voice_config'),
    'get_computer_use_status': ('moor_cli.web_routers.tools', 'get_computer_use_status'),
    'get_config': ('moor_cli.web_routers.config_env', 'get_config'),
    'get_config_path': ('moor_cli.config', 'get_config_path'),
    'get_config_raw': ('moor_cli.web_routers.analytics', 'get_config_raw'),
    'get_cron_delivery_targets': ('moor_cli.web_routers.cron', 'get_cron_delivery_targets'),
    'get_cron_job': ('moor_cli.web_routers.cron', 'get_cron_job'),
    'get_curator_status': ('moor_cli.web_routers.status', 'get_curator_status'),
    'get_dashboard_font': ('moor_cli.web_routers.dashboard_ui', 'get_dashboard_font'),
    'get_dashboard_plugins': ('moor_cli.web_routers.dashboard_ui', 'get_dashboard_plugins'),
    'get_dashboard_themes': ('moor_cli.web_routers.dashboard_ui', 'get_dashboard_themes'),
    'get_defaults': ('moor_cli.web_routers.config_env', 'get_defaults'),
    'get_egress_status': ('moor_cli.web_routers.config_env', 'get_egress_status'),
    'get_elevenlabs_voices': ('moor_cli.web_routers.audio', 'get_elevenlabs_voices'),
    'get_env_path': ('moor_cli.config', 'get_env_path'),
    'get_env_vars': ('moor_cli.web_routers.config_env', 'get_env_vars'),
    'get_health': ('moor_cli.web_routers.status', 'get_health'),
    'get_moor_home': ('moor_cli.config', 'get_moor_home'),
    'get_learning_graph': ('moor_cli.web_routers.status', 'get_learning_graph'),
    'get_learning_node': ('moor_cli.web_routers.status', 'get_learning_node'),
    'get_logs': ('moor_cli.web_routers.status', 'get_logs'),
    'get_media': ('moor_cli.web_routers.files', 'get_media'),
    'get_memory_provider_config': ('moor_cli.web_routers.memory_providers', 'get_memory_provider_config'),
    'get_memory_status': ('moor_cli.web_routers.ops', 'get_memory_status'),
    'get_messaging_platforms': ('moor_cli.web_routers.messaging', 'get_messaging_platforms'),
    'get_moa_models': ('moor_cli.web_routers.models', 'get_moa_models'),
    'get_model_info': ('moor_cli.web_routers.models', 'get_model_info'),
    'get_model_options': ('moor_cli.web_routers.models', 'get_model_options'),
    'get_models_analytics': ('moor_cli.web_routers.analytics', 'get_models_analytics'),
    'get_plugins_hub': ('moor_cli.web_routers.dashboard_ui', 'get_plugins_hub'),
    'get_portal_status': ('moor_cli.web_routers.status', 'get_portal_status'),
    'get_process_moor_home': ('moor_cli.config', 'get_process_moor_home'),
    'get_profile_setup_command': ('moor_cli.web_routers.profiles', 'get_profile_setup_command'),
    'get_profile_soul': ('moor_cli.web_routers.profiles', 'get_profile_soul'),
    'get_profiles_sessions': ('moor_cli.web_routers.profiles', 'get_profiles_sessions'),
    'get_profiles_sessions_sidebar': ('moor_cli.web_routers.profiles', 'get_profiles_sessions_sidebar'),
    'get_provider_config_schema': ('plugins.memory.config_schema', 'get_provider_config_schema'),
    'get_recommended_default_model': ('moor_cli.web_routers.models', 'get_recommended_default_model'),
    ...
    'get_schema': ('moor_cli.web_routers.config_env', 'get_schema'),
    'get_session_detail': ('moor_cli.web_routers.sessions', 'get_session_detail'),
    'get_session_latest_descendant': ('moor_cli.web_routers.sessions', 'get_session_latest_descendant'),
    'get_session_messages': ('moor_cli.web_routers.sessions', 'get_session_messages'),
    'get_session_stats': ('moor_cli.web_routers.sessions', 'get_session_stats'),
    'get_sessions': ('moor_cli.web_routers.sessions', 'get_sessions'),
    'get_skill_content': ('moor_cli.web_routers.skills', 'get_skill_content'),
    'get_skills': ('moor_cli.web_routers.skills', 'get_skills'),
    'get_ssh_ownership': ('moor_cli.web_routers.status', 'get_ssh_ownership'),
    'get_status': ('moor_cli.web_routers.status', 'get_status'),
    'get_system_stats': ('moor_cli.web_routers.status', 'get_system_stats'),
    'get_telegram_onboarding_status': ('moor_cli.web_routers.messaging', 'get_telegram_onboarding_status'),
    'get_terminal_backends': ('moor_cli.web_routers.tools', 'get_terminal_backends'),
    'get_toolset_config': ('moor_cli.web_routers.tools', 'get_toolset_config'),
    'get_toolset_models': ('moor_cli.web_routers.tools', 'get_toolset_models'),
    'get_toolsets': ('moor_cli.web_routers.tools', 'get_toolsets'),
    'get_update_receipt': ('moor_cli.web_routers.actions', 'get_update_receipt'),
    'get_usage_analytics': ('moor_cli.web_routers.analytics', 'get_usage_analytics'),
    'get_whatsapp_onboarding_status': ('moor_cli.web_routers.messaging', 'get_whatsapp_onboarding_status'),
    'git_base_branches_route': ('moor_cli.web_routers.git', 'git_base_branches_route'),
    'git_branch_switch_route': ('moor_cli.web_routers.git', 'git_branch_switch_route'),
    'git_branches_route': ('moor_cli.web_routers.git', 'git_branches_route'),
    'git_commit_context_route': ('moor_cli.web_routers.git', 'git_commit_context_route'),
    'git_commit_route': ('moor_cli.web_routers.git', 'git_commit_route'),
    'git_create_pr_route': ('moor_cli.web_routers.git', 'git_create_pr_route'),
    'git_file_diff_route': ('moor_cli.web_routers.git', 'git_file_diff_route'),
    'git_push_route': ('moor_cli.web_routers.git', 'git_push_route'),
    'git_rev_parse_route': ('moor_cli.web_routers.git', 'git_rev_parse_route'),
    'git_revert_route': ('moor_cli.web_routers.git', 'git_revert_route'),
    'git_review_diff_route': ('moor_cli.web_routers.git', 'git_review_diff_route'),
    'git_review_list_route': ('moor_cli.web_routers.git', 'git_review_list_route'),
    'git_ship_info_route': ('moor_cli.web_routers.git', 'git_ship_info_route'),
    'git_stage_route': ('moor_cli.web_routers.git', 'git_stage_route'),
    'git_status_route': ('moor_cli.web_routers.git', 'git_status_route'),
    'git_unstage_route': ('moor_cli.web_routers.git', 'git_unstage_route'),
    'git_worktree_add_route': ('moor_cli.web_routers.git', 'git_worktree_add_route'),
    'git_worktree_remove_route': ('moor_cli.web_routers.git', 'git_worktree_remove_route'),
    'git_worktrees_route': ('moor_cli.web_routers.git', 'git_worktrees_route'),
    'grant_computer_use_permissions': ('moor_cli.web_routers.tools', 'grant_computer_use_permissions'),
    'import_sessions_endpoint': ('moor_cli.web_routers.sessions', 'import_sessions_endpoint'),
    'install_mcp_catalog_entry': ('moor_cli.web_routers.mcp', 'install_mcp_catalog_entry'),
    'install_skill_hub': ('moor_cli.web_routers.skills', 'install_skill_hub'),
    'instantiate_blueprint': ('moor_cli.web_routers.cron', 'instantiate_blueprint'),
    'is_nix_install_method': ('moor_cli.config', 'is_nix_install_method'),
    'list_checkpoints': ('moor_cli.web_routers.ops', 'list_checkpoints'),
    'list_credential_pool': ('moor_cli.web_routers.ops', 'list_credential_pool'),
    'list_cron_blueprints': ('moor_cli.web_routers.cron', 'list_cron_blueprints'),
    'list_cron_job_runs': ('moor_cli.web_routers.cron', 'list_cron_job_runs'),
    'list_cron_jobs': ('moor_cli.web_routers.cron', 'list_cron_jobs'),
    'list_custom_endpoints': ('moor_cli.web_routers.config_env', 'list_custom_endpoints'),
    'list_hooks': ('moor_cli.web_routers.ops', 'list_hooks'),
    'list_managed_files': ('moor_cli.web_routers.files', 'list_managed_files'),
    'list_mcp_catalog': ('moor_cli.web_routers.mcp', 'list_mcp_catalog'),
    'list_mcp_servers': ('moor_cli.web_routers.mcp', 'list_mcp_servers'),
    'list_oauth_providers': ('moor_cli.web_routers.oauth', 'list_oauth_providers'),
    'list_pairing': ('moor_cli.web_routers.ops', 'list_pairing'),
    'list_profiles_endpoint': ('moor_cli.web_routers.profiles', 'list_profiles_endpoint'),
    'list_skills_hub_sources': ('moor_cli.web_routers.skills', 'list_skills_hub_sources'),
    'list_webhooks': ('moor_cli.web_routers.ops', 'list_webhooks'),
    'load_env': ('moor_cli.config', 'load_env'),
    'mcp_oauth_callback': ('moor_cli.web_routers.mcp', 'mcp_oauth_callback'),
    'mcp_oauth_flow_status': ('moor_cli.web_routers.mcp', 'mcp_oauth_flow_status'),
    'normalize_updated_at': ('gateway.status', 'normalize_updated_at'),
    'open_profile_terminal_endpoint': ('moor_cli.web_routers.profiles', 'open_profile_terminal_endpoint'),
    'parse_active_agents': ('gateway.status', 'parse_active_agents'),
    'pause_cron_job': ('moor_cli.web_routers.cron', 'pause_cron_job'),
    'poll_oauth_session': ('moor_cli.web_routers.oauth', 'poll_oauth_session'),
    'post_agent_plugin_disable': ('moor_cli.web_routers.dashboard_ui', 'post_agent_plugin_disable'),
    'post_agent_plugin_enable': ('moor_cli.web_routers.dashboard_ui', 'post_agent_plugin_enable'),
    'post_agent_plugin_install': ('moor_cli.web_routers.dashboard_ui', 'post_agent_plugin_install'),
    'post_agent_plugin_update': ('moor_cli.web_routers.dashboard_ui', 'post_agent_plugin_update'),
    'post_plugin_visibility': ('moor_cli.web_routers.dashboard_ui', 'post_plugin_visibility'),
    'preview_skill_hub': ('moor_cli.web_routers.skills', 'preview_skill_hub'),
    'prune_checkpoints': ('moor_cli.web_routers.ops', 'prune_checkpoints'),
    'prune_sessions_endpoint': ('moor_cli.web_routers.sessions', 'prune_sessions_endpoint'),
    'pty_ws': ('moor_cli.web_routers.chat_ws', 'pty_ws'),
    'pub_ws': ('moor_cli.web_routers.chat_ws', 'pub_ws'),
    'put_plugin_providers': ('moor_cli.web_routers.dashboard_ui', 'put_plugin_providers'),
    ...
    'reveal_env_var': ('moor_cli.web_routers.config_env', 'reveal_env_var'),
    'revoke_pairing': ('moor_cli.web_routers.ops', 'revoke_pairing'),
    'run_backup': ('moor_cli.web_routers.ops', 'run_backup'),
    'run_config_migrate': ('moor_cli.web_routers.status', 'run_config_migrate'),
    'run_curator': ('moor_cli.web_routers.status', 'run_curator'),
    'run_debug_share_endpoint': ('moor_cli.web_routers.status', 'run_debug_share_endpoint'),
    'run_doctor': ('moor_cli.doctor', 'run_doctor'),
    'run_dump': ('moor_cli.dump', 'run_dump'),
    'run_import': ('moor_cli.web_routers.ops', 'run_import'),
    'run_import_upload': ('moor_cli.web_routers.ops', 'run_import_upload'),
    'run_prompt_size': ('moor_cli.web_routers.status', 'run_prompt_size'),
    'run_security_audit': ('moor_cli.web_routers.ops', 'run_security_audit'),
    'run_toolset_post_setup': ('moor_cli.web_routers.tools', 'run_toolset_post_setup'),
    'save_config': ('moor_cli.config', 'save_config'),
    'save_env_value': ('moor_cli.config', 'save_env_value'),
    'save_toolset_env': ('moor_cli.web_routers.tools', 'save_toolset_env'),
    'scan_skill_hub': ('moor_cli.web_routers.skills', 'scan_skill_hub'),
    'search_sessions': ('moor_cli.web_routers.sessions', 'search_sessions'),
    'search_skills_hub': ('moor_cli.web_routers.skills', 'search_skills_hub'),
    'select_terminal_backend': ('moor_cli.web_routers.tools', 'select_terminal_backend'),
    'select_toolset_model': ('moor_cli.web_routers.tools', 'select_toolset_model'),
    'select_toolset_provider': ('moor_cli.web_routers.tools', 'select_toolset_provider'),
    'serve_plugin_asset': ('moor_cli.web_routers.dashboard_ui', 'serve_plugin_asset'),
    'set_active_profile_endpoint': ('moor_cli.web_routers.profiles', 'set_active_profile_endpoint'),
    'set_curator_paused': ('moor_cli.web_routers.status', 'set_curator_paused'),
    'set_dashboard_font': ('moor_cli.web_routers.dashboard_ui', 'set_dashboard_font'),
    'set_dashboard_theme': ('moor_cli.web_routers.dashboard_ui', 'set_dashboard_theme'),
    'set_env_var': ('moor_cli.web_routers.config_env', 'set_env_var'),
    'set_mcp_server_enabled': ('moor_cli.web_routers.mcp', 'set_mcp_server_enabled'),
    'set_memory_provider': ('moor_cli.web_routers.ops', 'set_memory_provider'),
    'set_moa_models': ('moor_cli.web_routers.models', 'set_moa_models'),
    'set_model_assignment': ('moor_cli.web_routers.models', 'set_model_assignment'),
    'set_webhook_enabled': ('moor_cli.web_routers.ops', 'set_webhook_enabled'),
    'setup_memory_provider': ('moor_cli.web_routers.memory_providers', 'setup_memory_provider'),
    'speak_stream_ws': ('moor_cli.web_routers.audio', 'speak_stream_ws'),
    'speak_text': ('moor_cli.web_routers.audio', 'speak_text'),
    'start_gateway': ('moor_cli.web_routers.ops', 'start_gateway'),
    'start_oauth_login': ('moor_cli.web_routers.oauth', 'start_oauth_login'),
    'start_telegram_onboarding': ('moor_cli.web_routers.messaging', 'start_telegram_onboarding'),
    'start_whatsapp_onboarding': ('moor_cli.web_routers.messaging', 'start_whatsapp_onboarding'),
    'stop_gateway': ('moor_cli.web_routers.ops', 'stop_gateway'),
    'stream_managed_file': ('moor_cli.web_routers.files', 'stream_managed_file'),
    'submit_oauth_code': ('moor_cli.web_routers.oauth', 'submit_oauth_code'),
    'test_mcp_server': ('moor_cli.web_routers.mcp', 'test_mcp_server'),
    'test_messaging_platform': ('moor_cli.web_routers.messaging', 'test_messaging_platform'),
    'toggle_skill': ('moor_cli.web_routers.skills', 'toggle_skill'),
    'toggle_toolset': ('moor_cli.web_routers.tools', 'toggle_toolset'),
    'transcribe_audio_upload': ('moor_cli.web_routers.audio', 'transcribe_audio_upload'),
    'trigger_cron_job': ('moor_cli.web_routers.cron', 'trigger_cron_job'),
    'tts_lease': ('moor_cli.web_routers.audio', 'tts_lease'),
    'uninstall_skill_hub': ('moor_cli.web_routers.skills', 'uninstall_skill_hub'),
    'update_config': ('moor_cli.web_routers.config_env', 'update_config'),
    'update_config_raw': ('moor_cli.web_routers.analytics', 'update_config_raw'),
    'update_cron_job': ('moor_cli.web_routers.cron', 'update_cron_job'),
    'update_moor': ('moor_cli.web_routers.actions', 'update_moor'),
    'update_learning_node': ('moor_cli.web_routers.status', 'update_learning_node'),
    'update_memory_provider_config': ('moor_cli.web_routers.memory_providers', 'update_memory_provider_config'),
    'update_messaging_platform': ('moor_cli.web_routers.messaging', 'update_messaging_platform'),
    'update_profile_description_endpoint': ('moor_cli.web_routers.profiles', 'update_profile_description_endpoint'),
    'update_profile_model_endpoint': ('moor_cli.web_routers.profiles', 'update_profile_model_endpoint'),
    'update_profile_soul': ('moor_cli.web_routers.profiles', 'update_profile_soul'),
    'update_skill_content': ('moor_cli.web_routers.skills', 'update_skill_content'),
    'update_skills_hub': ('moor_cli.web_routers.skills', 'update_skills_hub'),
    'upload_chat_image': ('moor_cli.web_routers.files', 'upload_chat_image'),
    'upload_managed_file': ('moor_cli.web_routers.files', 'upload_managed_file'),
    'upload_managed_file_stream': ('moor_cli.web_routers.files', 'upload_managed_file_stream'),
    'upsert_custom_endpoint': ('moor_cli.web_routers.config_env', 'upsert_custom_endpoint'),
    'validate_custom_endpoint': ('moor_cli.web_routers.config_env', 'validate_custom_endpoint'),
    'validate_provider_credential': ('moor_cli.web_routers.config_env', 'validate_provider_credential'),
    'windows_detach_flags': ('moor_cli._subprocess_compat', 'windows_detach_flags'),
    'windows_hide_flags': ('moor_cli._subprocess_compat', 'windows_hide_flags'),
    'write_platform_config_field': ('moor_cli.config', 'write_platform_config_field'),
}
