...
Usage:
    python -m moor_cli.main web          # Start on http://127.0.0.1:9119
    python -m moor_cli.main web --port 8080
"""
...
from moor_cli._subprocess_compat import windows_detach_flags, windows_hide_flags
...
def _start_desktop_cron_ticker(stop_event: "threading.Event", interval: int = 60) -> None:
    ...
    The scheduler tick loop normally lives in ``moor gateway run`` — but the
    desktop app spawns a ``moor dashboard`` backend, not a gateway, so a cron
...
def _warm_gateway_module() -> None:
    ...
    ``moor_cli.gateway``. But the first WS connection and its initial
...
def _resolve_restart_drain_timeout() -> float:
    try:
        from moor_cli.gateway import _get_restart_drain_timeout
...
def _eager_reconcile_own_session_db() -> None:
    ...
    bringing a store left behind by `moor update` current before the
...
async def _lifespan(app: "FastAPI"):
    ...
    # so a store left behind by `moor update` kept 500ing every
    ...
    # Import moor_cli.gateway eagerly *before* the lifespan yield so the
    ...
    # model picker) can detect when `moor update` replaced the code
    ...
    if os.getenv("MOOR_DESKTOP") == "1":
        ...
        try:
            from moor_cli.gateway import _reap_unsupervised_gateway_orphans
...
from moor_cli.memory_oauth import router as _memory_oauth_router  # noqa: E402
...
# MOOR_DASHBOARD_SESSION_TOKEN so its main process can authenticate the
...
def _resolve_session_token() -> str:
    return os.environ.get("MOOR_DASHBOARD_SESSION_TOKEN") or secrets.token_urlsafe(32)
...
#
# This list is defined in ``moor_cli.dashboard_auth.public_paths`` so the
...
# ---------------------------------------------------------------------------
from moor_cli.dashboard_auth.public_paths import (
...
def _dashboard_public_hosts() -> frozenset[str]:
    ...
    ``public_url`` is already Moor' canonical browser-facing URL behind a
    ...
    """
    from moor_cli.dashboard_auth.prefix import resolve_public_url
...
async def _dashboard_auth_gate(request: Request, call_next):
    from moor_cli.dashboard_auth.middleware import gated_auth_middleware
...
async def _token_auth_seam(request: Request, call_next):
    ...
    through untouched.
    """
    from moor_cli.dashboard_auth.token_auth import token_auth_middleware
...
_SCHEMA_OVERRIDES: Dict[str, Dict[str, Any]] = {
    ...
    "proxy.enabled": {
        "type": "boolean",
        "description": (
            "Docker-only egress credential firewall. Requires `moor egress setup` "
            "and `moor egress start`; Modal/SSH/Daytona are not wired yet."
        ),
        "category": "security",
    },
    ...
    "updates.refresh_cua_driver": {
        "type": "boolean",
        "description": (
            "Refresh an already-installed cua-driver during moor update. "
...
from moor_cli.web_models import (  # noqa: F401
    ConfigUpdate,
    EnvVarUpdate,
    EnvVarDelete,
    EnvVarReveal,
...
def _normalize_main_model_assignment(provider: str, model: str) -> tuple[str, str]:
    ...
    from moor_cli.models import _KNOWN_PROVIDER_NAMES, normalize_provider
    from moor_cli.model_normalize import normalize_model_for_provider
    from moor_cli.providers import resolve_custom_provider, resolve_user_provider
    ...
    if (
        ...
        try:
            ...
            cur_provider = ""
        from moor_cli.models import _AGGREGATOR_PROVIDERS
...
_MANAGED_FILES_ROOT_ENV = "MOOR_DASHBOARD_FILES_ROOT"
...
# MOOR_HOME itself, at which point every one of these basenames is a live
...
# without needing to resolve them relative to MOOR_HOME.
...
def _is_sensitive_path(path: Path) -> bool:
    ...
    and needs no MOOR_HOME resolution, so it blocks these trees wherever
...
def _default_moor_root_is_opt_data() -> bool:
    raw = os.environ.get("MOOR_HOME", "").strip()
    if not raw:
        return False
    try:
        from moor_constants import get_default_moor_root

        root = get_default_moor_root().expanduser().resolve(strict=False)
    except (OSError, RuntimeError):
...
def _dashboard_local_update_managed_externally() -> bool:
    """Return true when the dashboard should not offer ``moor update``.
    ...
    a container — e.g. the moor-webui image sharing the Moor source tree),
    the dashboard's ``moor update`` button is the correct update path and
    ...
    """
    if _default_moor_root_is_opt_data():
        return True
    try:
        from moor_constants import is_container
...
def _managed_files_policy(request: Request, *, create_root: bool = True) -> ManagedFilesPolicy:
    ...
    # (the container/hosted layout) or when MOOR_DASHBOARD_FILES_ROOT is set.
    if _default_moor_root_is_opt_data():
...
async def fs_write_text(payload: FsWriteText):
    ...
    Mirrors the local Electron ``moor:fs:writeText`` hardening: the path is
    ...
    tmp = target.with_name(f".{target.name}.moor-tmp-{os.getpid()}")
    try:
...
# gate + path hardening as /api/fs). Logic lives in ``moor_cli.web_git``;
# these are thin, executor-offloaded wrappers (git/gh can block).
# ---------------------------------------------------------------------------

from moor_cli import web_git as _web_git  # noqa: E402
...
from moor_cli.web_routers import git as _git_routes  # noqa: E402

app.include_router(_git_routes.router)
from moor_cli.web_routers.git import (  # noqa: E402,F401 — legacy re-exports; tests call these via web_server.<name>
    git_status_route,
    git_worktrees_route,
    git_branches_route,
...
def _collect_profile_gateway_topology() -> Dict[str, Any]:
    ...
    * ``profile_platforms`` — ``{profile: platforms}`` runtime platform maps
    ...
    """
    try:
        from moor_cli.profiles import _check_gateway_running, profiles_to_serve
...
# install, minted on first read and persisted under the ROOT Moor home
# (get_default_moor_root()) — NOT the profile-scoped MOOR_HOME — so every
...
def _read_or_create_install_id() -> Optional[str]:
    """Read (or mint + persist) the install id under the root Moor home.
...
_PROFILE_PLATFORM_STATUS_KEY_RE = re.compile(
    # Profile segment mirrors moor_cli.profiles._PROFILE_ID_RE.  Platform
...
async def get_status(profile: Optional[str] = None):
    ...
    # across that await. Status only resolves get_moor_home() at call time
    ...
    try:
        ...
        # MOOR_HOME (see issue #69143). Plain /api/status keeps the exact
        ...
        # Windows install the first import of moor_cli.gateway blocks the
        ...
        # and which providers are registered so ``moor status`` and the
        # SPA's StatusPage can show "OAuth gate ON via Moor inc." or
        ...
        # Moor bootstrap-session validity for the NAS health sweep. A hosted
        # agent whose Moor auth dies terminally (invalid_grant / quarantine)
        ...
        moor_session_valid = "unknown"
        try:
            from moor_cli.auth import get_moor_session_validity
            moor_session_valid = get_moor_session_validity()
        except Exception:
            moor_session_valid = "unknown"
        ...
        status = {
            ...
            "can_update_moor": not _dashboard_local_update_managed_externally(),
            "gateway_running": gateway_running,
            "gateway_state": gateway_state,
            ...
            "moor_session_valid": moor_session_valid,
        }
        ...
        try:
            ...
            storage_check = await asyncio.get_running_loop().run_in_executor(
                None, functools.partial(_probe_state_db, get_moor_home())
            )
        ...
        # moor_session_valid above. Deliberately NOT folded into
        ...
        try:
            ...
            status["memory"] = await asyncio.get_running_loop().run_in_executor(
                ...
                functools.partial(
                    ...
                    profile_dir if profile_dir else get_moor_home(),
                ),
            )
        except Exception:
        ...
        # disk-exhaustion incidents). One statvfs call on MOOR_HOME's
        ...
        try:
            from moor_state import SessionDB as _SDB
            from moor_constants import get_moor_home as _ghh
...
async def get_system_stats():
    ...
    and non-sensitive (no env values, no paths beyond the moor home root).
    """
...
async def run_curator():
    ...
    try:
        proc = _spawn_moor_action(["curator", "run"], "curator-run")
    except Exception as exc:
...
# ---------------------------------------------------------------------------
# Portal endpoint — Moor Portal auth + Tool Gateway routing status (read-only).
# ---------------------------------------------------------------------------
...
def _get_portal_status_sync():
    ...
    try:
        ...
        feats = get_moor_subscription_features(cfg)
        if feats is not None:
            for feat in feats.items():
                if getattr(feat, "managed_by_moor", False):
                    state = "via Moor Portal"
...
async def run_prompt_size():
    try:
        proc = _spawn_moor_action(["prompt-size"], "prompt-size")
    except Exception as exc:
...
async def run_dump():
    try:
        proc = _spawn_moor_action(["dump"], "dump")
    except Exception as exc:
        raise HTTPException(status_code=500, detail=f"Failed: {exc}")
...
async def run_config_migrate():
    try:
        proc = _spawn_moor_action(["config", "migrate"], "config-migrate")
    except Exception as exc:
        raise HTTPException(status_code=500, detail=f"Failed: {exc}")
...
async def run_debug_share_endpoint(body: DebugShareRequest | None = None):
    ...
    *synchronous*: the whole point of ``debug share`` is the set of shareable
    ...
    """
    from moor_cli.debug import build_debug_share
...
# streamed to a per-action log file under ``~/.moor/logs/<action>.log`` so
# the dashboard can tail them back to the user.
# ---------------------------------------------------------------------------

_ACTION_LOG_DIR: Path = get_moor_home() / "logs"
...
_UPDATE_ACTION_COMPLETED_RE = re.compile(
    r"^=== moor-update completed ([0-9a-f]{32}) ===$"
)
...
def _spawn_moor_action(
    subcommand: List[str],
    name: str,
    *,
    env_overrides: Optional[Dict[str, str]] = None,
) -> subprocess.Popen:
    """Spawn ``moor <subcommand>`` detached and record the Popen handle.

    Uses the running interpreter's ``moor_cli.main`` module so the action
    ...
    cmd = [_dashboard_spawn_executable(), "-m", "moor_cli.main", *subcommand]

    # The dashboard runs *inside* the gateway process, so os.environ carries
    # _MOOR_GATEWAY=1. Inheriting it makes a spawned `moor gateway restart`
    ...
    action_env = {**os.environ, "MOOR_NONINTERACTIVE": "1"}
    action_env.pop("_MOOR_GATEWAY", None)
    ...
    action_id = (env_overrides or {}).get("MOOR_ACTION_ID")
    if action_id:
...
def _durable_completed_update_action_id(lines: List[str]) -> Optional[str]:
    ...
    for index, line in enumerate(lines):
        if line.startswith("=== moor update started "):
            last_start = index
...
def _gateway_display_command(profile: Optional[str], verb: str) -> str:
    return " ".join(["moor", *_gateway_subcommand(profile, verb)])
...
def _spawn_gateway_restart(profile: Optional[str] = None) -> Tuple[subprocess.Popen, bool]:
    """Spawn ``moor gateway restart``, reusing an in-flight restart.
    ...
    concurrent ``moor gateway restart`` children race each other on the
    ...
    proc = _spawn_moor_action(subcommand, "gateway-restart")
...
async def restart_gateway(profile: Optional[str] = None):
    """Kick off a ``moor gateway restart`` in the background."""
    try:
...
async def gateway_drain(request: Request):
    ...
    active (no ``MOOR_DASHBOARD_DRAIN_SECRET``), the route is NOT a token
...
async def update_moor():
    ...
    if _dashboard_local_update_managed_externally():
        ...
        _record_completed_action("moor-update", message, exit_code=1)
        return {
            "ok": False,
            "pid": None,
            "name": "moor-update",
    ...
    existing = _ACTION_PROCS.get("moor-update")
    if existing is not None and existing.poll() is None:
        response = {
            "ok": True,
            "pid": existing.pid,
        ...
        action_id = _ACTION_IDS.get("moor-update")
        if action_id:
    ...
    try:
        proc = _spawn_moor_action(
            ["update"],
            "moor-update",
            env_overrides={"MOOR_ACTION_ID": action_id},
        )
    except Exception as exc:
        _log.exception("Failed to spawn moor update")
...
@app.get("/api/moor/update/check")
async def check_moor_update(force: bool = False):
    """Report whether a Moor update is available, without applying it.

    Powers the dashboard's "check before you update" flow: the System page
    shows the commit-behind count and asks the user to confirm before
    ``POST /api/moor/update`` actually runs ``moor update``.
    ...
    if install_method == "apt":
        payload["message"] = (
            "Moor is managed by Termux APT; run `pkg upgrade moor-agent`."
        )
        return payload
    ...
    try:
        from moor_cli.banner import check_for_updates

        if force:
            try:
                (get_moor_home() / ".update_check").unlink()
            except OSError:
                pass
...
async def speak_text(payload: TTSSpeakRequest, profile: Optional[str] = None):
    ...
    configured in ``~/.moor/config.yaml`` under ``tts.``.
    """
...
async def get_action_status(name: str, lines: int = 200):
    ...
    if name == "moor-update":
        ...
        if durable_update_action_id:
            marker = f"=== moor-update completed {durable_update_action_id} ==="
...
def _latest_update_receipt_summary() -> Optional[Dict[str, Any]]:
    ...
    Phase-1 bullet 3 (#91277): the receipt (written by EVERY ``moor
...
@app.get("/api/moor/update/receipt")
async def get_update_receipt():
    ...
    if not receipt:
        raise HTTPException(
            status_code=404,
            detail="No update receipt found (no `moor update` run recorded).",
        )
...
from moor_cli.web_routers import sessions as _sessions_routes  # noqa: E402

app.include_router(_sessions_routes.list_router)
from moor_cli.web_routers.sessions import (  # noqa: E402,F401 — legacy re-exports; tests call these via web_server.<name>
    get_sessions,
)


from moor_cli.web_routers import profiles as _profiles_routes  # noqa: E402

app.include_router(_profiles_routes.sessions_router)
from moor_cli.web_routers.profiles import (  # noqa: E402,F401 — legacy re-exports; tests call these via web_server.<name>
...
def _flat_json_path(provider: ProviderConfigSchema) -> Path:
    return get_moor_home() / provider.name / "config.json"
...
def _install_memory_provider_pip_dependencies(dependencies: List[str]) -> List[Dict[str, Any]]:
    ...
    # under /opt/moor is sealed read-only, and installs must be redirected
    # to the writable durable target on the data volume
    # (MOOR_LAZY_INSTALL_TARGET, e.g. /opt/data/lazy-packages) — the same
...
def _read_memory_provider_existing_values(name: str) -> Dict[str, Any]:
    ...
    for path in (
        moor_home / f"{name}.json",
        moor_home / name / "config.json",
    ):
...
def _save_memory_provider_native_config(name: str, provider: Any, values: Dict[str, Any]) -> None:
    if provider is not None and hasattr(provider, "save_config"):
        try:
            ...
            provider.save_config(values, str(get_moor_home()))
            return
...
# in moor_cli/config.py — listed here for deterministic ordering in the UI.
...
def _dashboard_code_skew_guard() -> Optional[str]:
    ...
    boot.  When ``moor update`` (or a manual ``git pull``) replaces the
    ...
    return (
        ...
        f"(systemctl --user restart moor-dashboard, or moor dashboard --port <port>)"
    )
...
async def get_model_options(
    ...
    try:
        ...
        from moor_cli.inventory import build_model_options_payload, load_picker_context
...
def get_recommended_default_model(provider: str = ""):
    ...
    Mirrors the model-curation `moor model` does so GUI onboarding lands on a
    sensible default instead of blindly taking the first curated entry. For
    Moor this honors the user's free/paid tier: free users get a free model,
    ...
    where free_tier is True/False for Moor and None otherwise. `model` may be
    ...
    if slug == "moor":
        try:
            from moor_cli.models import (
                get_curated_moor_model_ids,
                get_pricing_for_provider,
                check_moor_free_tier,
            ...
            pricing = get_pricing_for_provider("moor") or {}
            ...
            if free_tier:
                ...
                model_ids, _unavailable = partition_moor_models_by_tier(
            ...
            model = pick_silent_default_model(model_ids, provider="moor")
            return {"provider": "moor", "model": model, "free_tier": bool(free_tier)}
        except Exception:
            _log.exception("GET /api/model/recommended-default (moor) failed")
            return {"provider": "moor", "model": "", "free_tier": None}

    # Non-Moor: preferred silent default when the provider's curated list
    ...
    try:
        from moor_cli.inventory import build_models_payload, load_picker_context
        from moor_cli.models import pick_silent_default_model
...
def set_moa_models(body: MoaConfigPayload, profile: Optional[str] = None):
    ...
    try:
        from moor_cli.moa_config import normalize_moa_config, validate_moa_payload
...
async def set_model_assignment(body: ModelAssignment, profile: Optional[str] = None):
    ...
    Writes to ``~/.moor/config.yaml`` — applies to **new** sessions only.
...
def _apply_model_assignment_sync(
    ...
    if scope == "main":
        ...
        # When switching the main provider to Moor, mirror the CLI's
        # post-model-selection behaviour (moor_cli/main.py
        # prompt_enable_tool_gateway / tools_config apply_moor_managed_defaults):
        # auto-route any *unconfigured* tools through the Moor Tool Gateway.
        # This is purely additive — apply_moor_managed_defaults skips every
        ...
        if provider.strip().lower() == "moor":
            try:
                from moor_cli.moor_subscription import apply_moor_managed_defaults
                from moor_cli.tools_config import _get_platform_tools

                enabled = _get_platform_tools(
                    cfg, "cli", include_default_mcp_servers=False
                )
                changed = apply_moor_managed_defaults(
                    cfg,
                ...
                # Portal lookup hiccups / non-subscriber / non-moor gating
                # must never block saving the model assignment.
                _log.debug("apply_moor_managed_defaults skipped", exc_info=True)

        save_config(cfg)

        # Register a named ``custom_providers`` entry for a custom/local
        # endpoint, mirroring the ``moor model`` custom flow
        ...
        if provider.strip().lower() in {"custom", "local"} and base_url:
            try:
                from moor_cli.main import _auto_provider_name, _save_custom_provider
        ...
        # a now-unpaid provider (e.g. moor with $0 balance) keeps paying 402s
...
async def update_config(body: ConfigUpdate, profile: Optional[str] = None):
    def _run():
        ...
        # different MOOR_HOME than this process's gateway sessions.
...
def _is_other_profile(profile: Optional[str]) -> bool:
    ...
    try:
        ...
        return True
    return target.resolve() != get_process_moor_home().resolve()
...
def _catalog_provider_env_metadata() -> dict:
    ...
    (i.e. the ``moor model`` universe). This is what lets the desktop Keys tab
    ...
    """
    try:
        from moor_cli.provider_catalog import provider_catalog
    except Exception:
        return {}
    ...
    try:
        from moor_cli.config import OPTIONAL_ENV_VARS as _OPT
    except Exception:
        _OPT = {}
    _non_provider_keys = {
    ...
    for d in provider_catalog():
        ...
        # Keys tab (otherwise Bedrock — a `moor model` provider — would be
        ...
        # Vertex — a `moor model` provider — would be invisible in the desktop
...
def _get_env_vars_sync(profile: Optional[str] = None):
    ...
    def _row(var_name: str, info: dict, *, custom: bool = False) -> dict:
        ...
        return {
            ...
            # CLI `moor model` picker uses (not desktop-only prefix guesses).
...
def list_custom_endpoints(profile: Optional[str] = None):
    ...
    the process-level MOOR_HOME. Mirrors ``/api/config``'s profile scoping.
    """
    try:
        with _config_profile_scope(profile):
...
async def remove_env_var(body: EnvVarDelete, profile: Optional[str] = None):
    def _run():
        with _profile_scope(body.profile or profile):
            ...
            from moor_cli.credential_lifecycle import remove_provider_env_credential
...
_MESSAGING_ENV_FALLBACKS: dict[str, dict[str, Any]] = {
    ...
    "WEIXIN_ACCOUNT_ID": {
        "description": "iLink Bot account ID obtained through QR login in moor gateway setup",
        "prompt": "iLink Bot account ID",
    },
    "WEIXIN_TOKEN": {
        "description": "iLink Bot token obtained through QR login in moor gateway setup",
...
# moor_cli/setup_hidden_env.py. Shared with the `moor setup gateway`
# wizard so the surfaces ask for the same things.
from moor_cli.setup_hidden_env import (  # noqa: E402
...
def _messaging_platform_payload(
    ...
    # MOOR_HOME contextvar override (#56986 / #69143), so the profile's
...
def _whatsapp_session_path() -> Path:
    ...
    return get_moor_dir("platforms/whatsapp/session", "whatsapp/session")
...
_TELEGRAM_ONBOARDING_USER_AGENT = f"MoorDashboard/{__version__}"
@dataclass
...
async def get_messaging_platforms(profile: Optional[str] = None):
    ...
    # load_env() honors the MOOR_HOME contextvar override; the gateway
...
def _multiplex_port_binding_conflict(
    ...
    *enabling* is blocked; disabling/clearing stays allowed so users can
    ...
    if not requested or requested.lower() == "current":
        ...
        # The dashboard's own profile. "custom" (an unrecognized MOOR_HOME)
...
# Anthropic, device-code for Moor/Codex) still runs in the CLI for now;
# Phase 2 will add in-browser flows. For unconnected providers we return
# the canonical ``moor auth add <provider>`` command so the dashboard
...
def _anthropic_oauth_status() -> Dict[str, Any]:
    ...
    1. ``~/.moor/.anthropic_oauth.json`` — moor-managed PKCE flow (what
    ...
    2. ``ANTHROPIC_API_KEY`` → ``ANTHROPIC_TOKEN`` → ``CLAUDE_CODE_OAUTH_TOKEN``
       ...
       env during ``load_moor_dotenv()``, so the same check covers them)
    ...
    try:
        from agent.anthropic_adapter import (
            read_moor_oauth_credentials,
            _get_moor_oauth_file,
        )
    except ImportError:
        read_moor_oauth_credentials = None  # type: ignore
        _get_moor_oauth_file = None  # type: ignore

    moor_creds = None
    if read_moor_oauth_credentials:
        try:
            moor_creds = read_moor_oauth_credentials()
        except Exception:
            moor_creds = None
    if moor_creds and moor_creds.get("accessToken"):
        return {
            "logged_in": True,
            "source": "moor_pkce",
            "source_label": f"Moor PKCE ({_get_moor_oauth_file() if _get_moor_oauth_file else None})",
            "token_preview": _truncate_token(moor_creds.get("accessToken")),
            "expires_at": moor_creds.get("expiresAt"),
            "has_refresh_token": bool(moor_creds.get("refreshToken")),
        }
...
_OAUTH_PROVIDER_CATALOG: tuple[Dict[str, Any], ...] = (
    {
        ...
        "name": "Moor Portal",
        "flow": "device_code",
        "cli_command": "moor auth add moor",
        "docs_url": "https://portal.nousresearch.com",
        "status_fn": None,  # dispatched via auth.get_moor_auth_status
    },
    {
        "id": "openai-codex",
        "name": "ChatGPT or Codex Subscription",
        "flow": "device_code",
        "cli_command": "moor auth add openai-codex",
    ...
    },
    {
        ...
        "flow": "external",
        "cli_command": "moor auth add qwen-oauth",
    ...
    },
    {
        ...
        # as Moor's device-code flow; the PKCE bit is a security
        # extension that doesn't change the operator experience.
        "flow": "device_code",
        "cli_command": "moor auth add minimax-oauth",
    ...
    },
    {
        ...
        # 127.0.0.1 callback.
        "flow": "device_code",
        "cli_command": "moor auth add xai-oauth",
    ...
    # usage credits on top of a Claude Max plan — see disclaimer in name).
    {
        ...
        "flow": "pkce",
        "cli_command": "moor auth add anthropic",
...
def _resolve_provider_status(provider_id: str, status_fn) -> Dict[str, Any]:
    ...
    try:
        ...
        if provider_id == "moor":
            ...
            raw = hauth.get_moor_auth_status_local()
            return {
                "logged_in": bool(raw.get("logged_in")),
                "source": "moor_portal",
                "source_label": raw.get("portal_base_url") or "Moor Portal",
...
def _build_oauth_catalog() -> list[Dict[str, Any]]:
    ...
    MEMBERSHIP is the union of:
      ...
      2. every accounts-tab provider in the unified ``provider_catalog()`` (the
         ``moor model`` universe) — so any OAuth/external provider added as a
    ...
    #    in lockstep with the `moor model` universe (zero-edit for new plugins).
    try:
        from moor_cli.provider_catalog import provider_catalog
        for d in provider_catalog():
            ...
            rows.append({
                ...
                "name": d.label,
                "flow": "external",
                "cli_command": f"moor auth add {d.slug}",
...
async def list_oauth_providers(profile: Optional[str] = None):
    ...
    Response shape (per provider):
        ...
        status:
          ...
          source           short slug ("moor_pkce", "claude_code", ...)
    ...
    sync with the `moor model` picker; _OAUTH_OVERRIDES supplies per-provider
    ...
    def _run():
        with _profile_scope(profile):
            ...
            if provider_id == "anthropic":
                cleared = False
                try:
                    from agent.anthropic_adapter import _get_moor_oauth_file
                    oauth_file = _get_moor_oauth_file()
                ...
                try:
                    from moor_cli.auth import clear_provider_auth
            ...
            try:
                from moor_cli.auth import clear_provider_auth, invalidate_moor_auth_status_cache
...
#          → persists to ~/.moor/.anthropic_oauth.json AND credential pool
#          → returns { ok: true, status: "approved" }
#
#   Device code (Moor, OpenAI Codex):
#     1. POST /api/providers/oauth/{moor|openai-codex}/start
...
# Guarded so moor web still starts if anthropic_adapter is unavailable;
...
def _save_anthropic_oauth_creds(access_token: str, refresh_token: str, expires_at_ms: int) -> None:
    ...
    the system in the same state as ``moor auth add anthropic``.
    """
    from agent.anthropic_adapter import _get_moor_oauth_file
    oauth_file = _get_moor_oauth_file()
    payload = {
...
async def _start_device_code_flow(
    ...
    """Initiate a device-code flow (Moor, OpenAI Codex, MiniMax, or xAI).
    ...
    if provider_id == "moor":
        ...
        portal_base_url = (
            os.getenv("MOOR_PORTAL_BASE_URL")
        ...
        threading.Thread(
            target=_moor_poller, args=(sid,), daemon=True, name=f"oauth-poll-{sid[:6]}"
        ).start()
        return {
            "session_id": sid,
            "flow": "device_code",
    ...
    if provider_id == "minimax-oauth":
        ...
        # operator's perspective it's identical to Moor's device-code
    ...
    if provider_id == "xai-oauth":
        from moor_cli.auth import _xai_oauth_request_device_code
        import httpx
...
def _moor_poller(session_id: str) -> None:
    """Background poller that drives a Moor device-code flow to completion."""
    from moor_cli.auth import (
        _poll_for_token,
        refresh_moor_oauth_from_state,
    )
    from datetime import datetime, timezone
    import httpx
    with _oauth_sessions_lock:
    ...
    try:
        ...
        with _profile_scope(_oauth_session_profile(session_id)):
            full_state = refresh_moor_oauth_from_state(
                auth_state,
        ...
        with _oauth_sessions_lock:
            sess["status"] = "approved"
        _log.info("oauth/device: moor login completed (session=%s)", session_id)
    except Exception as e:
        _log.warning("moor device-code poll failed (session=%s): %s", session_id, e)
        with _oauth_sessions_lock:
            sess["status"] = "error"
...
def _minimax_poller(session_id: str) -> None:
    ...
    Mirrors `_moor_poller` but calls the MiniMax-specific token endpoint,
    which uses a PKCE-style ``code_verifier`` + ``user_code`` rather than
    the ``device_code`` field used by Moor. On success, builds the same
...
def _xai_device_poller(session_id: str) -> None:
    ...
    try:
        ...
        with _profile_scope(_oauth_session_profile(session_id)):
            ...
            # Mirror `moor auth add xai-oauth`: first credential may become
            ...
            # prior ``moor auth remove xai-oauth`` (mirrors auth_add_command
            # and the ``moor model`` re-login path in _login_xai_oauth).
...
def _codex_full_login_worker(session_id: str) -> None:
    ...
    try:
        ...
        from moor_cli.auth import _save_codex_tokens
...
async def poll_oauth_session(
    ...
    Shared by the device-code flows (Moor, OpenAI Codex, MiniMax, xAI).
...
def _session_db_read_probe_statements() -> tuple:
    ...
    after `moor update` until the first message forced a writable open.
    """
    from moor_state_schema import schema_read_probe_statements
...
def _open_session_db_at_path(db_path: Path, *, read_only: bool):
    ...
    from moor_state import SessionDB, is_malformed_schema_error
...
def _maybe_auto_archive_for_profile(profile: Optional[str]) -> None:
    ...
    The Desktop backend is spawned as ``moor serve`` — it runs neither the
...
def _prune_sessions(body: SessionPrune):
    """Delete ended sessions matching filters (mirrors `moor sessions prune`)."""
    has_window = (
    ...
    profile_home = _cron_profile_home(body.profile)[1] if body.profile else get_moor_home()
...
async def get_logs(
    ...
    try:
        from moor_logging import COMPONENT_PREFIXES
    except ImportError:
        COMPONENT_PREFIXES = {}
...
def _cron_default_profile() -> str:
    ...
    A desktop pool backend runs one process per profile (MOOR_HOME already
    ...
    fallback would write a non-default profile's job into ``~/.moor``.
    Resolve the process's own profile instead. ``custom`` (an unrecognized
    MOOR_HOME outside the profiles tree) has no profile-dir equivalent, so
...
def _cron_profile_home(profile: Optional[str]) -> Tuple[str, Path]:
    """Resolve a profile query value to (profile_name, MOOR_HOME)."""
    from moor_cli import profiles as profiles_mod
...
from moor_cli.web_routers import cron as _cron_routes  # noqa: E402

app.include_router(_cron_routes.router)
from moor_cli.web_routers.cron import (  # noqa: E402,F401 — legacy re-exports; tests call these via web_server.<name>
    list_cron_jobs,
    get_cron_job,
    list_cron_job_runs,
    create_cron_job,
...
def _gateway_fire_endpoint(profile: str, home: Path) -> str:
    ...
    try:
        ...
        # yaml.safe_load of config.yaml (tests/moor_cli/
        # test_config_read_guard.py). The MOOR_HOME override scopes
...
def _gateway_intentionally_stopped(profile: Optional[str]) -> bool:
    ...
    commands (``moor gateway stop`` persists ``"stopped"``; start and
...
#
# Wraps the same config data layer the CLI uses (moor_cli.mcp_config), so
# servers managed here show up under `moor mcp list` and vice versa.  Secrets
...
def _normalize_mcp_server_create(
    ...
    from moor_cli.mcp_security import validate_mcp_server_entry
...
from moor_cli.web_routers import mcp as _mcp_routes  # noqa: E402

app.include_router(_mcp_routes.router)
from moor_cli.web_routers.mcp import (  # noqa: E402,F401 — legacy re-exports; tests call these via web_server.<name>
    list_mcp_servers,
    add_mcp_server,
    replace_mcp_servers,
    remove_mcp_server,
...
def _mcp_oauth_callback_url(request: Request, server_name: str) -> str:
    ...
    from moor_cli.dashboard_auth.prefix import prefix_from_request, resolve_public_url
...
def _mcp_oauth_transaction(flow) -> threading.Lock:
    key = (flow.moor_home, flow.server_name)
...
def _run_dashboard_mcp_oauth(flow, cfg: dict) -> None:
    ...
    try:
        ...
        from tools.mcp_oauth import MoorTokenStorage, force_interactive_oauth
        from tools.mcp_oauth_manager import get_manager

        home_token = set_moor_home_override(flow.moor_home)
        secret_token = set_secret_scope(build_profile_secret_scope(Path(flow.moor_home)))
        try:
            transaction = _mcp_oauth_transaction(flow)
            with transaction, force_interactive_oauth(), dashboard_oauth_flow(flow):
                manager = get_manager()
                storage = MoorTokenStorage(flow.server_name)
...
#
# Wraps the same JSON store the CLI uses (moor_cli.webhook); the webhook
...
# spawn the real `moor gateway <verb>` so behaviour matches the CLI exactly.
...
async def start_gateway(profile: Optional[str] = None):
    try:
        proc = _spawn_moor_action(_gateway_subcommand(profile, "start"), "gateway-start")
    except HTTPException:
        raise
    except Exception as exc:
...
async def stop_gateway(profile: Optional[str] = None):
    try:
        proc = _spawn_moor_action(_gateway_subcommand(profile, "stop"), "gateway-stop")
    except HTTPException:
        raise
    except Exception as exc:
...
async def add_credential_pool_entry(body: CredentialPoolAdd):
    ...
    try:
        ...
        # (via DELETE below or `moor auth remove`) can seed again.
        # Mirrors the `moor auth add` behaviour in auth_commands.py.
...
async def remove_credential_pool_entry(provider: str, index: int):
    ...
    registry the CLI ``moor auth remove`` uses: each source cleans up its
    ...
    from moor_cli.auth import suppress_credential_source
...
async def run_doctor():
    try:
        proc = _spawn_moor_action(["doctor"], "doctor")
    except Exception as exc:
...
async def run_security_audit():
    try:
        proc = _spawn_moor_action(["security", "audit"], "security-audit")
    except Exception as exc:
...
def _new_dashboard_backup_path() -> Path:
    ...
    return _dashboard_backup_dir() / f"moor-backup-{stamp}-{secrets.token_hex(4)}.zip"
...
async def run_backup(body: BackupRequest):
    ...
    try:
        proc = _spawn_moor_action(args, "backup")
    except Exception as exc:
...
async def run_import(body: ImportRequest):
    ...
    if body.force:
        args.append("--force")
    try:
        proc = _spawn_moor_action(args, "import")
    except Exception as exc:
...
async def list_checkpoints():
    ...
    # Checkpoints live under <moor_home>/checkpoints/.  Surface a count +
...
async def prune_checkpoints():
    try:
        proc = _spawn_moor_action(["checkpoints", "prune"], "checkpoints-prune")
    except Exception as exc:
...
def _profile_cli_args(profile: Optional[str]) -> List[str]:
    ...
    Hub install/uninstall/update run in a fresh ``moor`` subprocess, and
...
def _hub_action_name(verb: str, key: str) -> str:
    ...
    ``_spawn_moor_action`` tracks one process/log per name, so a shared
...
from moor_cli.web_routers import skills as _skills_routes  # noqa: E402

app.include_router(_skills_routes.hub_router)
from moor_cli.web_routers.skills import (  # noqa: E402,F401 — legacy re-exports; tests call these via web_server.<name>
    install_skill_hub,
    uninstall_skill_hub,
    update_skills_hub,
...
# Human-readable labels for each hub source id (matches `moor skills search`
# provenance).  Keep in sync with create_source_router()'s source list.
_SKILL_HUB_SOURCE_LABELS = {
    "official": "Official (Moor)",
    "moor-index": "Moor Index",
    "skills-sh": "skills.sh",
...
def _fallback_profile_dicts(profiles_mod) -> List[Dict[str, Any]]:
    ...
    default_home = profiles_mod._get_default_moor_home()
...
def _profile_setup_command(name: str) -> str:
    ...
    return "moor setup" if name == "default" else f"{name} setup"
...
def _write_profile_model(profile_dir: Path, provider: str, model: str) -> None:
    ...
    context-local MOOR_HOME override so the write lands in the target
    ...
    token = set_moor_home_override(str(profile_dir))
    try:
        provider, model = _normalize_main_model_assignment(provider, model)
        cfg = load_config()
...
def _write_profile_mcp_servers(profile_dir: Path, servers: List["MCPServerCreate"]) -> int:
    ...
    Scopes ``load_config``/``save_config`` to ``profile_dir`` via the
    context-local MOOR_HOME override (same mechanism as
    ...
    """
    from moor_constants import set_moor_home_override, reset_moor_home_override
    from moor_cli.mcp_config import _save_bearer_auth_token
...
def _disable_unselected_skills(profile_dir: Path, keep: List[str]) -> int:
    ...
    install.) Scoped to the profile via the MOOR_HOME override. Returns the
...
def _profile_scope(profile: Optional[str]):
    ...
    1. ``load_config``/``save_config`` resolve ``get_moor_home()`` at call
       time — the context-local override from ``set_moor_home_override``
    ...
    retargeted to the *current* ``get_moor_home()`` so writes land in the
    live home even when the import-time binding is stale (e.g. the process
    imported the modules before a MOOR_HOME override, or under test
    isolation).
    """
    requested = (profile or "").strip()
...
def _config_profile_scope(profile: Optional[str]):
    ...
    ``set_moor_home_override`` contextvar — it does NOT swap the
    ...
    which is all endpoints that resolve ``get_moor_home()`` at call time
...
from moor_cli.web_routers import tools as _tools_routes  # noqa: E402

app.include_router(_tools_routes.router)
from moor_cli.web_routers.tools import (  # noqa: E402,F401 — legacy re-exports; tests call these via web_server.<name>
    get_toolsets,
    toggle_toolset,
    get_toolset_config,
    get_toolset_models,
...
def _resolve_toolset_model_plugin(ts_key: str, provider_row: dict) -> Optional[str]:
    ...
    the managed "Moor Subscription" image row instead carries the legacy
...
#
# The endpoint spawns the same ``moor --tui`` binary the CLI uses, behind
...
if sys.platform.startswith("win"):
    try:
        from moor_cli.win_pty_bridge import WinPtyBridge as PtyBridge, PtyUnavailableError
    ...
    try:
        from moor_cli.pty_bridge import PtyBridge, PtyUnavailableError
        _PTY_BRIDGE_AVAILABLE = True
...
from moor_cli.pty_session import PtySessionRegistry, RegistryFull, run_reaper  # noqa: E402
...
_GATEWAY_WS_PROTOCOL = "moor-gateway-v1"
_GATEWAY_WS_TICKET_PROTOCOL_PREFIX = "moor-gateway-ticket."
...
def _ws_auth_reason(ws: "WebSocket") -> tuple[Optional[str], str]:
    ...
    * ``?internal=<process-credential>`` — the process-lifetime internal
    ...
    if auth_required:
        ...
        from moor_cli.dashboard_auth.ws_tickets import (
            TicketInvalid,
        ...
        try:
            ...
            if protocol_ticket:
                ...
                ws._moor_ws_subprotocol = _GATEWAY_WS_PROTOCOL
...
def _resolve_chat_argv(
    ...
    Default: whatever ``moor --tui`` would run.  Tests monkeypatch this
    function to inject a tiny fake command (``cat``, ``sh -c 'printf …'``)
    so nothing has to build Node or the TUI bundle.

    Session resume is propagated via the ``MOOR_TUI_RESUME`` env var —
    matching what ``moor_cli.main._launch_tui`` does for the CLI path.
    Appending ``--resume <id>`` to argv doesn't work because ``ui-tui`` does
    not parse its argv.

    ``MOOR_TUI_GATEWAY_URL`` is injected so the PTY child can attach to
    this process's in-memory ``tui_gateway`` instance instead of spawning
    its own Python gateway subprocess.

    `sidecar_url` (when set) is forwarded as ``MOOR_TUI_SIDECAR_URL`` so
    the spawned ``tui_gateway.entry`` can mirror dispatcher emits to the
    dashboard's ``/api/pub`` endpoint (see :func:`pub_ws`).

    `active_session_file` (when set) is forwarded as
    ``MOOR_TUI_ACTIVE_SESSION_FILE``. The TUI writes the current session id
    ...
    ``MOOR_HOME`` at the profile dir in the child env. Every spawned
    process (the TUI and the ``tui_gateway.entry`` it launches) resolves
    ``get_moor_home()`` from that env var at its own import, so the child
    binds the profile's config, skills, memory, and state.db from the start
    — the same propagation ``moor -p <name>`` performs. The in-process
    ``MOOR_TUI_GATEWAY_URL`` attach is SKIPPED for scoped chats: the
    ...
    """
    from moor_cli.main import PROJECT_ROOT, _apply_tui_python_env, _make_tui_argv
    ...
    # An explicit profile scope still overrides MOOR_HOME before config is
    # bridged into the child environment.
    from tools.environments.local import build_subprocess_env
    env = build_subprocess_env(scrub_secrets=False, inherit_profile_home=True)
    if profile_dir is not None:
        env["MOOR_HOME"] = str(profile_dir)
    try:
        from moor_cli.config import (
    ...
    env.setdefault("MOOR_TUI_DISABLE_MOUSE", "1")
    env.setdefault("MOOR_TUI_INLINE", "1")
    ...
    if resume:
        ...
        env["MOOR_TUI_RESUME"] = resume

    if sidecar_url:
        env["MOOR_TUI_SIDECAR_URL"] = sidecar_url
    ...
    # inherits the profile MOOR_HOME set above.
    if profile_dir is None:
        if gateway_ws_url := _build_gateway_ws_url():
            env["MOOR_TUI_GATEWAY_URL"] = gateway_ws_url
...
def _resolve_client_ws_host() -> Optional[str]:
    ...
    1. Explicit ``MOOR_DASHBOARD_WS_HOST`` env var — wins always. Operators
    ...
    """
    explicit = os.environ.get("MOOR_DASHBOARD_WS_HOST", "").strip()
    if explicit:
        return explicit
...
def _build_gateway_ws_url() -> Optional[str]:
    ...
    if getattr(app.state, "auth_required", False):
        from moor_cli.dashboard_auth.ws_tickets import internal_ws_credential
...
def _active_session_file_for_channel(app: "FastAPI", channel: str) -> Path:
    ...
    fd, raw_path = tempfile.mkstemp(prefix="moor-pty-active-", suffix=".json")
    os.close(fd)
    path = Path(raw_path)
    files[channel] = path
    return path
        ...
        with _console_executor_lock:
            if _console_executor is None:
                _console_executor = concurrent.futures.ThreadPoolExecutor(
                    ...
                    thread_name_prefix="moor-console",
                )
...
async def console_ws(ws: WebSocket) -> None:
    ...
    try:
        from moor_cli.console_engine import MoorConsoleEngine

        engine = MoorConsoleEngine(output_limit=_CONSOLE_OUTPUT_LIMIT)
...
async def pty_ws(ws: WebSocket) -> None:
    ...
    if raw_resume and env:
        registry_resume = env.get("MOOR_TUI_RESUME") or raw_resume
...
async def gateway_ws(ws: WebSocket) -> None:
    ...
    await handle_ws(
        ws,
        auth_identity=getattr(ws, "_moor_auth_identity", None),
        subprotocol=getattr(ws, "_moor_ws_subprotocol", None),
    )
...
# MOOR_TUI_SIDECAR_URL set in /api/pty's PTY env) and writes every
...
def _normalise_prefix(raw: Optional[str]) -> str:
    ...
    Thin re-export of :func:`moor_cli.dashboard_auth.prefix.normalise_prefix`
    ...
    """
    from moor_cli.dashboard_auth.prefix import normalise_prefix
...
def mount_spa(application: FastAPI):
    ...
    ``mission-control.tilos.com/moor/*`` -> local Caddy -> :9119), the
    proxy injects ``X-Forwarded-Prefix: /moor`` on every request. We
    rewrite the served ``index.html`` so absolute asset URLs (``/assets/...``)
    and the SPA's runtime ``__MOOR_BASE_PATH__`` honour that prefix
    without rebuilding the bundle.
    """
    # `moor serve` is the headless backend: it must NEVER serve the browser
    ...
    _headless = os.environ.get("MOOR_SERVE_HEADLESS") == "1"
    if _headless or not WEB_DIST.exists():
        _msg = (
            "Headless backend (moor serve): web UI disabled — use "
            "`moor dashboard` for the browser UI."
            if _headless
    ...
    def _serve_index(prefix: str = ""):
        ...
        ``prefix`` is the normalised ``X-Forwarded-Prefix`` (e.g. ``/moor``)
        ...
        ``__MOOR_AUTH_REQUIRED__`` flag lets the SPA pick the right
        ...
        if gated:
            bootstrap_script = (
                ...
                f"window.__MOOR_AUTH_REQUIRED__={gated_js};"
                f"</script>"
            )
        else:
            bootstrap_script = (
                f'<script>window.__MOOR_SESSION_TOKEN__="{_SESSION_TOKEN}";'
                f"window.__MOOR_DASHBOARD_EMBEDDED_CHAT__={chat_js};"
                f'window.__MOOR_BASE_PATH__="{prefix}";'
                f"window.__MOOR_AUTH_REQUIRED__={gated_js};"
                f"</script>"
        ...
        # (``MOOR_HOME/dashboard-themes/<name>.yaml``), inject a minimal
    ...
    # under ``/moor`` they'd hit ``mission-control.tilos.com/fonts/...``
...
_BUILTIN_DASHBOARD_THEMES = [
    ...
    {"name": "moor-blue",     "label": "Moor Blue",           "description": "Light mode — vivid Moor-blue accents on cream canvas"},
...
def _normalise_theme_definition(data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    # YAML in ~/.moor/, same trust level as the config file itself.
...
def _discover_user_themes() -> list:
    """Scan ~/.moor/dashboard-themes/*.yaml for user-created themes.

    Returns a list of fully-normalised theme definitions ready to ship
    to the frontend, so the client can apply them without a secondary
    round-trip or a built-in stub.

    Uses the dashboard process launch home, not ``get_moor_home()``, so a
    transient profile override from embedded chat does not hide themes that
    live under the server's own ``MOOR_HOME``.
    """
    themes_dir = get_process_moor_home() / "dashboard-themes"
...
async def get_dashboard_themes():
    ...
    from `~/.moor/dashboard-themes/*.yaml` ship with their full
...
def _discover_dashboard_plugins() -> list:
    ...
    Checks three plugin sources (same as moor_cli.plugins):
    1. User plugins:    ~/.moor/plugins/<name>/dashboard/manifest.json
    2. Bundled plugins: <repo>/plugins/<name>/dashboard/manifest.json  (memory/, etc.)
    3. Project plugins: ./.moor/plugins/  (only if MOOR_ENABLE_PROJECT_PLUGINS)
    """
    plugins = []
    seen_names: set = set()

    from moor_cli.plugins import get_bundled_plugins_dir
    ...
    # MOOR_HOME override (e.g. embedded /chat under --open-profile).
    #
    # #87197: when the process itself is profile-scoped (``--profile <name>``
    # sets ``MOOR_HOME=<root>/profiles/<name>``), the launch home is the
    # profile directory, which has no ``plugins/`` — user plugins are
    # installed in the moor root (``~/.moor/plugins``). Scan the default
    # root as well (``get_default_moor_root()`` unwraps
    # ``<root>/profiles/<name>`` → ``<root>`` and returns a custom
    # ``MOOR_HOME`` unchanged when it *is* the root), mirroring how
    # ``moor_cli.plugins`` resolves plugin install locations. The
    # ``seen_names`` dedupe below keeps profile-local plugins (if any)
    # authoritative over same-named root plugins.
    from moor_constants import get_default_moor_root

    user_plugin_roots = [get_process_moor_home() / "plugins"]
    root_plugins = get_default_moor_root() / "plugins"
    ...
    # ``moor_cli/plugins.py`` and the documented user contract.
    if env_var_enabled("MOOR_ENABLE_PROJECT_PLUGINS"):
        search_dirs.append((Path.cwd() / ".moor" / "plugins", "project"))
...
async def get_dashboard_plugins():
    ...
    def _run():
        ...
        try:
            from moor_cli.plugins_cmd import _get_enabled_set, _get_disabled_set
            enabled_set = _get_enabled_set()
            disabled_set = _get_disabled_set()
        except Exception:
            enabled_set = set()
...
def _merged_plugins_hub(force_refresh: bool = False) -> Dict[str, Any]:
    ...
    plugins_root_resolved = (get_moor_home() / "plugins").resolve()
    rows: List[Dict[str, Any]] = []
...
def _mount_plugin_api_routes():
    ...
    Project plugins (``./.moor/plugins/``) ship with the CWD and are
    ...
    for plugin in _get_dashboard_plugins():
        ...
        if not api_path.exists():
            ...
            continue
        try:
            module_name = f"moor_dashboard_plugin_{plugin['name']}"
...
from moor_cli.dashboard_auth.routes import router as _dashboard_auth_router  # noqa: E402
...
def _write_dashboard_ready_file(actual_port: int) -> None:
    ...
    so Electron passes ``MOOR_DESKTOP_READY_FILE`` and waits for this JSON.
    Normal CLI/dashboard launches still use the stdout READY line below.
    """
    target = os.environ.get("MOOR_DESKTOP_READY_FILE")
    if not target:
        return
...
def _is_serve_orphaned(
    ...
    *,
    ...
    ``MOOR_PARENT_PID`` is the Electron Desktop PID, not necessarily this
    Python process's immediate PPID. On Windows the venv ``moor.exe`` launcher
...
def _start_parent_death_watchdog() -> None:
    ...
    Desktop versions that provide only ``MOOR_PARENT_PID`` retain PID-only
    tracking.
    """
    raw_pid = os.environ.get("MOOR_PARENT_PID")
    start_marker = os.environ.get("MOOR_PARENT_START_MARKER")
    nonce = os.environ.get("MOOR_PARENT_NONCE")
    ...
    try:
        poll = max(0.5, float(os.environ.get("MOOR_SERVE_WATCHDOG_POLL_S", "2.0")))
    except (TypeError, ValueError):
        poll = 2.0
...
# spawn (and any script wrapping ``moor serve``) cannot tell "port occupied"
...
# MOOR_BACKEND_READY sentinel (which is NOT changed by any of this).
...
def _report_port_in_use(host: str, port: int) -> None:
    ...
    print(
        ...
        "'moor serve' / 'moor dashboard' backend or the Moor gateway. "
...
def start_server(
    ...
    build and no SPA mount (mount_spa() honours ``MOOR_SERVE_HEADLESS``), so
    ...
    try:
        from moor_cli.moor_auth_keepalive import start_moor_auth_keepalive

        start_moor_auth_keepalive()
    except Exception as exc:
        _log.debug("Moor auth keepalive did not start: %s", exc)
    ...
    if app.state.auth_required:
        ...
        if not list_providers():
            ...
            # to register (e.g. missing MOOR_DASHBOARD_OAUTH_CLIENT_ID).
            ...
            try:
                from plugins.dashboard_auth import moor as _moor_plugin

                if _moor_plugin.LAST_SKIP_REASON:
                    skip_reasons.append(
                        f"  • moor: {_moor_plugin.LAST_SKIP_REASON}"
                    )
            except Exception:
                pass
            ...
            if host in _LOOPBACK_HOST_VALUES:
                _public_url_for_msg = ""
                try:
                    from moor_cli.dashboard_auth.prefix import (
                ...
                _local_only_hint = (
                    ...
                    "(and unset MOOR_DASHBOARD_PUBLIC_URL) to restore the "
            ...
            _fix_hint = (
                ...
                "  • OAuth: run `moor dashboard register` (Moor Portal) or "
            ...
            try:
                ...
                from moor_cli.plugins_cmd import _BASIC_AUTH_PLUGIN_KEYS
                ...
                if _has_creds and (set(_disabled) & _BASIC_AUTH_PLUGIN_KEYS):
                    _fix_hint = (
                        ...
                        "`moor plugins enable basic`), then restart the "
                        "dashboard.\n\n"
                    ) + _fix_hint
            except Exception:
                pass
    ...
    # MOOR_DASHBOARD_READY, open the browser, *then* serve.
    #
    ...
    async def _serve():
        ...
        with server.capture_signals():
            ...
            # for standalone `moor serve` (no MOOR_PARENT_PID env).
            ...
            try:
                from moor_cli.process_identity import (
            ...
            ready_token = "MOOR_BACKEND_READY" if headless else "MOOR_DASHBOARD_READY"
            ...
            if headless:
                ...
                print(f"  Moor backend listening on {host}:{actual_port}", flush=True)
            else:
