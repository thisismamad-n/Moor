...
``moor-cli`` to the non-existent ``moor``. ``resolve_toolset('moor')``
...
def validate_platform_toolsets(
    ...
    1. A toolset name that ``is_valid_toolset`` rejects — usually a corrupted or
       renamed entry. When ``moor-<platform>`` would have been valid (the exact
       #38798 shape, where ``cli`` held ``moor`` instead of ``moor-cli``),
