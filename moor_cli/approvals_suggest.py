...
is the session DB (``~/.moor/state.db``): every assistant ``terminal`` tool
...
* **Destructive / privilege / credential / obfuscation classes are never
  ...
  restarts, moor self-management, …) are eligible.
...
def normalize_command(command: str) -> str:
    ...
    from tools.approval import (
        _rewrite_resolved_moor_home,
        _rewrite_resolved_user_home,
    )

    folded = _rewrite_resolved_user_home(_rewrite_resolved_moor_home(command))
