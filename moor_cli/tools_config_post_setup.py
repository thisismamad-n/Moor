"""Post-setup install hooks and installed-state predicates for `moor tools` provider rows."""
...
from moor_cli.tools_config_cua import (
...
def _ensure_browser_use_cli(*, verbose_hints: bool = False) -> None:
    ...
    moor-managed ``$MOOR_HOME/bin`` copy does."""
...
def _post_setup_lightpanda() -> None:
    ...
    if lightpanda_bin:
        ...
        if os.name == "nt":
            _print_info("    Lightpanda has no native Windows build; run Moor under WSL2.")
...
def _post_setup_agent_browser(post_setup_key: str) -> None:
    ...
    global/moor-managed install), so there is no ``npm install`` step here."""
    ...
    try:
        ...
        # PATH/node_modules probes above. Mirror the rung moor_cli.doctor uses so this probe can't diverge
        ...
        # agent-browser is no longer a root package.json dependency (#43564) — it resolves lazily via npx
        # (or a global/moor-managed install) instead of a local `npm install`, so there's no node_modules/
    ...
    # Reuse the runtime resolution cascade (PATH -> Homebrew/moor-managed node -> npx) rather than
    # a bare shutil.which — moor-managed-Node-only setups resolve agent-browser/npx only that way.
    try:
...
_PIP_POST_SETUP_HOOKS: dict = {
    "faster_whisper": _pip_hook(
        ...
        on_install=("Model sizes: tiny, base (default), small, medium, large-v3",
                    "Change via stt.local.model in ~/.moor/config.yaml")),
    "kittentts": _pip_hook(
    ...
    "piper": _pip_hook(
        ...
        always=("Default voice: en_US-lessac-medium (downloaded on first TTS call)",
                ...
                "Switch voices by setting tts.piper.voice in ~/.moor/config.yaml")),
    "ddgs": _pip_hook(
...
def _post_setup_spotify() -> None:
    # Full `moor auth spotify` flow: no client_id yet → interactive wizard (persists to ~/.moor/.env)
    ...
    try:
        ...
        _info_lines("Run manually: moor auth spotify")
        return
    ...
    try:
        ...
        _info_lines("Run later: moor auth spotify")
    except Exception as exc:
...
def _post_setup_xai_grok() -> None:
    ...
    try:
        from moor_cli.setup import prompt_choice, prompt as _setup_prompt
        ...
        _info_lines("Run later: moor auth add xai-oauth   (or set XAI_API_KEY)")
        return
    ...
    if idx == 0:
        if _run_xai_oauth_login_from_setup():
            ...
            _print_warning("    xAI Grok OAuth login did not complete. Run later: moor auth add xai-oauth")
    elif idx == 1:
        ...
        if api_key:
            ...
            _print_warning("    No API key provided. Run later: moor auth add xai-oauth")
    else:
...
def run_post_setup_command(args) -> int:
    """``moor tools post-setup <key>`` — non-interactive runner the dashboard spawns so the GUI can drive
...
# Python deps installed via ``moor tools`` aren't in the managed runtime's locked ``all`` sync, so a
