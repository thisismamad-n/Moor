"""``moor sessions`` command — extracted from ``moor_cli/main.py``.
...
Helpers that stay in ``moor_cli.main`` (``get_moor_home``,
...
``moor_cli.main.<name>`` keep reaching this code path, and so imports stay
...
#: Default age floor for `moor sessions prune --never-active`.  Deliberately
...
def _prune_never_active_keyed(db, args):
    """`moor sessions prune --never-active` — drop leaked/dead keyed rows.
...
def cmd_sessions(args, sessions_parser=None):
    ...
    if action == "repair":
        ...
        if report.get("repaired"):
            ...
            print(f"    moor sessions recover --source {source_hint} \\")
            print("        --inspect-only")
    ...
    if action == "list":
        from moor_state import workspace_key as _ws_key
        ...
        from moor_cli.session_filters import (
            build_prune_filters,
            describe_filters,
        )
            ...
            try:
                if len(ids) == 1:
                    ...
                    out_dir = (
                        ...
                        else get_moor_home() / "session-exports"
                    )
        ...
        # Markdown / QMD export
        from moor_cli.session_export_md import (
        ...
        if args.output == "-":
            ...
            db.close()
            return
        output_dir = Path(args.output).expanduser() if args.output else get_moor_home() / "session-exports"
        ...
        # `moor sessions prune`: no time window and no filters at all
        ...
        if not _include_pinned and callable(_count_matches):
            ...
            if _pinned_skipped:
                ...
                _optin = (
                    ...
                    "first with `moor sessions unpin <id>`."
                    if action == "prune"
                    else "Unpin first with `moor sessions unpin <id>` to include them."
                )
                print(
        ...
        if skipped_open:
            ...
            print(
                ...
                "sessions. Use `moor sessions delete <id>` "
        ...
        if not pinned_rows:
            print(
                "No pinned sessions. Pin one with: moor sessions pin <session_id>"
            )
            return
        ...
        # Launch moor --resume <id> by replacing the current process
        ...
        if result.get("vacuumed") is False:
            print("  (VACUUM was skipped or failed — run "
                  "`moor sessions optimize` later to reclaim freed space.)")
