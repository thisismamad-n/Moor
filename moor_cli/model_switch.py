...
def _declared_model_ids(value: Any) -> list[str]:
    ...
    if isinstance(value, str):
        ...
        # Pre-fix Moor wrote sentinel keys inside the user-facing ``models`` mapping.
...
def _models_config_is_allowlist(value: Any, discovered: bool = False) -> bool:
    ...
    ``discover_models: false``. A catalog Moor itself persisted (``discovered``) is never a pin."""
    if discovered:
        return False
    if isinstance(value, str):
...
def _direct_alias_source_identity() -> Optional[tuple]:
    ...
    try:
        from moor_constants import get_config_path
...
def resolve_startup_model_route(
    ...
    ``MoorCLI`` is constructed before the interactive ``/model`` pipeline runs; resolving here
    ...
    if current_provider:
        try:
            from moor_cli.providers import is_routing_aggregator, normalize_provider as _norm_prov
...
def _resolve_alias_fallback(
    ...
    results = (resolve_alias(raw_input, p) for p in authenticated_providers or ("openrouter", "moor"))
...
def _ollama_configured_base() -> tuple[dict, str]:
    from moor_cli.models import _get_provider_config_dict
...
def _unknown_provider_message(explicit_provider: str) -> str:
    msg = (
        f"Unknown provider '{explicit_provider}'. Check 'moor model' for available "
    ...
    try:  # Surface common config issues that cause provider resolution failures
        ...
        if issues:
            msg += "\n\nRun 'moor doctor' — config issues detected:" + "".join(f"\n  • {ci.message}" for ci in issues[:3])
    except Exception:
        pass
    return msg
...
def _route_from_model_input(st: _Switch) -> Optional[ModelSwitchResult]:
    ...
    from moor_cli.models import detect_provider_for_model
...
def _creds_for_switched_provider(st: _Switch) -> Optional[ModelSwitchResult]:
    ...
    if st.explicit_provider and st.user_providers:
        from moor_cli.providers import resolve_user_provider
...
def _creds_for_current_provider(st: _Switch) -> None:
    ...
    from moor_cli.models_local import _get_ollama_request_headers, _same_ollama_native_root
...
def _moor_api_mode(provider: str, model: str, api_key: str) -> str:
...
_PROVIDER_API_MODE_OVERRIDES: dict[str, Any] = {
    ...
    **dict.fromkeys(("moor", "moor-portal", "nousresearch"), _moor_api_mode)}
...
def _build_switch_result(st: _Switch) -> ModelSwitchResult:
    ...
    warnings = [w for w in (st.validation.get("message"), _check_moor_model_warning(st.new_model)) if w]
    ...
    try:
        from moor_cli.runtime_provider import _get_named_custom_provider, _custom_provider_request_overrides
...
_PLUGIN_COMPAT_LAZY = {
    ...
    'list_picker_providers': ('moor_cli.model_switch_providers', 'list_picker_providers'),
    'prewarm_picker_cache_async': ('moor_cli.model_switch_providers', 'prewarm_picker_cache_async'),
}
