...
- ``moor_cli.providers``        -- canonical provider identity + overlays
- ``moor_cli.model_normalize``  -- per-provider name formatting
...
def _declared_model_ids(value: Any) -> list[str]:
    ...
    if isinstance(value, dict):
        for model_id in value:
            # Backward compat: pre-fix Moor wrote sentinel keys inside the
...
def _entry_models_discovered(entry: Any) -> bool:
    """True when the entry's ``models`` mapping was auto-discovered by Moor.

    The current shape is an entry-level ``models_discovered: true`` sibling of
    ``models``. Older Moor versions wrote an in-mapping
...
def _models_config_is_allowlist(value: Any, discovered: bool = False) -> bool:
    ...
    written by ``_save_custom_provider`` / the ``moor model`` wizard — not a
    ...
    while ``moor model`` still live-probed the full ``/v1/models`` list.
    ...
    ``_entry_models_discovered``): a catalog Moor itself persisted after a
    ...
    *,
    ...
    try:
        ...
        for entry in providers:
            ...
            # A mapping Moor itself discovered (``models_discovered: true``
...
# Match only the real Moor inc. Hermes 3 / Hermes 4 chat families.
# The previous substring check (`"moor" in name.lower()`) false-positived on
# unrelated local Modelfiles like ``moor-brain:qwen3-14b-ctx16k`` that just
# happen to carry "moor" in their tag but are fully tool-capable.
#
# Positive examples the regex must match:
#   NousResearch/Hermes-3-Llama-3.1-70B, hermes-4-405b, openrouter/hermes3:70b
# Negative examples it must NOT match:
#   moor-brain:qwen3-14b-ctx16k, qwen3:14b, claude-opus-4-6
_MOOR_MOOR_NON_AGENTIC_RE = re.compile(
    r"(?:^|[/:])moor[-_ ]?[34](?:[-_.:]|$)",
    re.IGNORECASE,
)
...
def is_moor_moor_non_agentic(model_name: str) -> bool:
    ...
    if not model_name:
        return False
    return bool(_MOOR_MOOR_NON_AGENTIC_RE.search(model_name))
...
def _load_direct_aliases() -> dict[str, DirectAlias]:
    ...
    Also reads ``model.aliases`` (set by ``moor config set model.aliases.xxx``)
...
def _ensure_direct_aliases() -> None:
    ...
    the module attribute. This keeps `from moor_cli.model_switch import
...
def _resolve_alias_fallback(
    ...
    Falls back to ``("openrouter", "moor")`` only when no authenticated
    providers are supplied (backwards compat for non-interactive callers).
    """
    providers = authenticated_providers or ("openrouter", "moor")
...
def resolve_display_context_length(
    ...
    about Codex OAuth, Copilot, Moor, and falls back to models.dev for the
    rest.
...
async def resolve_display_context_length_async(
    ...
    ``/v1/models``, Copilot, Moor, Codex, GMI, Ollama, models.dev and
    ...
    ``moor_cli.route_identity.should_clear_context_pin_async``, which
...
def switch_model(
    ...
    if explicit_provider:
        ...
        if pdef is None:
            _switch_err = (
                ...
                f"Check 'moor model' for available providers, or define it "
            ...
            try:
                ...
                if _cfg_issues:
                    _switch_err += "\n\nRun 'moor doctor' — config issues detected:"
        ...
        from moor_cli.models import _AGGREGATOR_PROVIDERS as _AGG_PROVIDERS
        from moor_cli.providers import ALIASES as _PROVIDER_ALIAS_TABLE
    ...
    # =================================================================
        ...
        try:
            from moor_cli.config import load_config
            from moor_cli.moa_config import exact_moa_preset_name, normalize_moa_config
    ...
    if provider_changed or explicit_provider:
        ...
        if explicit_provider and user_providers:
            from moor_cli.providers import resolve_user_provider as _ruser
        ...
        if current_provider == "custom" and current_base_url:
            try:
                from moor_cli.models import should_use_ollama_native_catalog
    ...
    # --- Moor Portal dual-wire override ---
    ...
    if target_provider in {"moor", "moor-portal", "nousresearch"}:
        from moor_cli.providers import moor_api_mode

        api_mode = moor_api_mode(new_model)
    ...
    # moor_cli.runtime_provider.resolve_runtime_provider; without the strip,
    ...
    # kimi) the same way.
    from moor_cli.models import opencode_provider_family as _oc_family_fn
    ...
    moor_warn = _check_moor_model_warning(new_model)
    if moor_warn:
        warnings.append(moor_warn)
...
def _prefetch_provider_models_parallel(provider_slugs: list[str]) -> None:
    ...
    def _fetch_one(slug: str) -> None:
        try:
            ...
            if models:
                from moor_cli.models import update_provider_cache_entry
...
def _collect_authed_provider_slugs(
    ...
    from moor_cli.auth import PROVIDER_REGISTRY, _load_auth_store
    from moor_cli.providers import MOOR_OVERLAYS, ALIASES as _PROVIDER_ALIAS_TABLE
    from moor_cli.models import _AGGREGATOR_PROVIDERS as _AGG_PROVIDERS, CANONICAL_PROVIDERS
    ...
    for moor_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        _alias_target = _PROVIDER_ALIAS_TABLE.get(moor_id)
        if (
            _alias_target
            and _alias_target != moor_id
        ...
        try:
            ...
            pass
        if _canonical != moor_id:
            continue
        if moor_id.lower() in seen:
            continue
        if moor_id.lower() in _excluded_set or mdev_id.lower() in _excluded_set:
            continue
        pdata = models_dev_data.get(mdev_id)
        if not isinstance(pdata, dict):
            continue
        pconfig = PROVIDER_REGISTRY.get(moor_id)
        if pconfig and pconfig.auth_type != "api_key":
            continue
        from moor_cli.auth import is_runtime_provider_routable
        if not is_runtime_provider_routable(moor_id):
            continue
        ...
        if not has_creds:
            try:
                ...
                raw_pool_present = bool(
                    store and store.get("credential_pool", {}).get(moor_id)
                )
                if raw_pool_present:
                    has_creds = _credential_pool_is_usable(
                        moor_id, raw_pool_present=True
                    )
            except Exception:
                pass
        if has_creds:
            slugs.append(moor_id)
            seen.add(moor_id.lower())

    # --- Section 2: moor-only providers (MOOR_OVERLAYS) ---
    _mdev_to_moor = {v: k for k, v in PROVIDER_TO_MODELS_DEV.items()}
    for pid, overlay in MOOR_OVERLAYS.items():
        if pid.lower() in seen:
            continue
        moor_slug = _mdev_to_moor.get(pid, pid)
        if moor_slug.lower() in seen:
            continue
        if pid.lower() in _excluded_set or moor_slug.lower() in _excluded_set:
            continue
        has_creds = False
        ...
        if not has_creds:
            try:
                ...
                if pid in providers_store or moor_slug in providers_store:
                    has_creds = True
            except Exception:
                pass
        if not has_creds:
            try:
                if _credential_pool_is_usable(moor_slug):
                    has_creds = True
            except Exception:
                pass
        if has_creds:
            slugs.append(moor_slug)
            seen.add(pid.lower())
            seen.add(moor_slug.lower())
...
def list_authenticated_providers(
    ...
    custom_providers: list | None = None,
    *,
    force_fresh_moor_tier: bool = False,
    ...
    Uses the curated model lists from moor_cli/models.py (OPENROUTER_MODELS,
    ...
    ``force_fresh_moor_tier`` bypasses the short Moor tier cache for explicit
    ...
    from moor_cli.models import (
        ...
        clear_provider_models_cache, get_curated_moor_model_ids,
    )
    ...
    # Compared against moor_id / mdev_id (section 1), pid / moor_slug
    ...
    def _record_builtin_endpoint(slug: str) -> None:
        ...
        try:
            from moor_cli.auth import PROVIDER_REGISTRY as _reg
        except Exception:
            return
        pcfg = _reg.get(slug)
        if not pcfg:
    ...
    # Build curated model lists keyed by moor provider ID
    curated: dict[str, list[str]] = dict(_PROVIDER_MODELS)
    curated["openrouter"] = [mid for mid, _ in OPENROUTER_MODELS]
    # "moor" pulls from the remote model-catalog manifest published at
    # https://hermes-agent.nousresearch.com/docs/api/model-catalog.json so
    # newly added Portal models surface in the /model picker without
    # requiring a Moor release. Falls back to the in-repo
    # _PROVIDER_MODELS["moor"] snapshot when the manifest is unreachable.
    ...
    if "lmstudio" not in curated and (
        ...
        from moor_cli.models import fetch_lmstudio_models
    ...
    for moor_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        if (
            ...
            continue
        # Resolve the canonical provider profile name.  Skip moor_ids
        # that are mere aliases resolving to a different canonical profile
        # (e.g. "kimi" and "moonshot" both → "kimi-coding").  Only process
        # entries whose moor_id matches the canonical profile name so
        ...
        # same moor_id).  Distinct canonical profiles that share a
        ...
        if slug.lower() in seen_slugs:
            continue
        if moor_id.lower() in _excluded or mdev_id.lower() in _excluded:
            continue
        ...
        # section 2 (MOOR_OVERLAYS) with proper auth store checking.
        if pconfig and pconfig.auth_type != "api_key":
            continue
        # models.dev catalogs include providers Moor may not route yet.
        ...
        # /model picker sees the SAME list `moor model` would build, with
        # disk caching to keep the picker open snappy. Falls back to the
        # curated static list when the live fetcher returns nothing.
        model_ids = cached_provider_model_ids(moor_id)
        if not model_ids:
            model_ids = curated.get(moor_id, [])
            if moor_id in _MODELS_DEV_PREFERRED:
                model_ids = _merge_with_models_dev(moor_id, model_ids)
    ...
    # --- 2. Check moor-only providers (moor, openai-codex, copilot, opencode-go) ---
    from moor_cli.providers import MOOR_OVERLAYS
    from moor_cli.auth import PROVIDER_REGISTRY as _auth_registry

    # Build reverse mapping: models.dev ID → Moor provider ID.
    # MOOR_OVERLAYS keys may be models.dev IDs (e.g. "github-copilot")
    # while _PROVIDER_MODELS and config.yaml use Moor IDs ("copilot").
    _mdev_to_moor = {v: k for k, v in PROVIDER_TO_MODELS_DEV.items()}
    ...
    for pid, overlay in MOOR_OVERLAYS.items():
        ...
        # Resolve Moor slug — e.g. "github-copilot" → "copilot"
        moor_slug = _mdev_to_moor.get(pid, pid)
        if moor_slug.lower() in seen_slugs:
            continue
        if pid.lower() in _excluded or moor_slug.lower() in _excluded:
            continue
        ...
        if getattr(overlay, "keyless", False):
            ...
            has_creds = _has_aws_sdk_creds_for_listing(moor_slug)
        elif overlay.auth_type == "vertex":
        ...
        if not has_creds:
            try:
                ...
                if store and (pid in providers_store or moor_slug in providers_store):
                    has_creds = True
            except Exception as exc:
        ...
        if not has_creds:
            try:
                if _credential_pool_is_usable(moor_slug):
                    ...
                    try:
                        from agent.credential_pool import load_pool
                        _pool = load_pool(moor_slug)
                        if _pool.has_credentials():
                            has_creds = True
                    except Exception:
                        pass
            except Exception as exc:
                logger.debug("Credential pool check failed for %s: %s", moor_slug, exc)
        ...
        # configured.
        if not has_creds and moor_slug == "anthropic":
            try:
                ...
                if (moor_creds and moor_creds.get("accessToken")) or \
        ...
        # reflects the active region (eu.*, ap.*) not the static us.* list.
            ...
            try:
                _ids = cached_provider_model_ids(moor_slug)
                model_ids = _ids if _ids else (curated.get(moor_slug, []) or curated.get(pid, []))
            except Exception:
                model_ids = curated.get(moor_slug, []) or curated.get(pid, [])
        elif moor_slug == "moor":
            # Moor serves a large live /v1/models catalog (vendor-prefixed
            # models from many providers, returned alphabetically). The
            # `moor model` picker deliberately shows ONLY the curated agentic
            ...
            # order. Mirror that exactly (see _model_flow_moor in main.py) so
            ...
            model_ids = curated.get("moor", [])
            try:
                from moor_cli.models import (
                    get_pricing_for_provider as _moor_pricing,
                    check_moor_free_tier as _moor_free,
                    union_with_portal_free_recommendations as _union_free,
                    union_with_portal_paid_recommendations as _union_paid,
                )
                from moor_cli.auth import get_provider_auth_state as _moor_state

                _pricing = _moor_pricing("moor") or {}
                _portal = ""
                try:
                    _st = _moor_state("moor") or {}
                    _portal = _st.get("portal_base_url", "") or ""
                except Exception:
                    _portal = ""
                if _moor_free(force_fresh=force_fresh_moor_tier):
            ...
            if not model_ids:
                model_ids = curated.get(moor_slug, []) or curated.get(pid, [])
                if moor_slug in _MODELS_DEV_PREFERRED:
                    model_ids = _merge_with_models_dev(moor_slug, model_ids)
        total = len(model_ids)
        if moor_slug in _UNCAPPED_PICKER_PROVIDERS:
            top = model_ids  # Aggregator: show full catalog regardless of max_models
        else:
            top = model_ids[:max_models] if max_models is not None else model_ids
        ...
        results.append({
            "slug": moor_slug,
            "name": get_label(moor_slug),
            "is_current": moor_slug == current_provider or pid == current_provider,
            "is_user_defined": False,
            "models": top,
            "total_models": total,
            "source": "moor",
        ...
        seen_slugs.add(moor_slug.lower())
        _record_builtin_endpoint(moor_slug)

    # --- 2b. Cross-check canonical provider list ---
    # Catches providers that are in CANONICAL_PROVIDERS but weren't found
    # in PROVIDER_TO_MODELS_DEV or MOOR_OVERLAYS (keeps /model in sync
    # with `moor model`).
    try:
        from moor_cli.models import CANONICAL_PROVIDERS as _canon_provs
    except ImportError:
        _canon_provs = []
    ...
    if user_providers and isinstance(user_providers, dict):
        ...
        for ep_name, ep_cfg in user_providers.items():
            ...
            # ``moor model``, not an allowlist — see
        ...
        for grp in ep_groups.values():
            ...
            #   Ollama/llama.cpp match ``moor model``. Pin with
    ...
    if custom_providers and isinstance(custom_providers, list):
        ...
        for entry in custom_providers:
            ...
            # active model. Moor's own writer (main.py::_save_custom_provider)
            # stores every configured model as a dict under ``models:``;
            # downstream readers (agent/models_dev.py, gateway/run.py,
            # run_agent.py, moor_cli/config.py) already consume that dict.
        ...
        for grp in groups.values():
            ...
            #   ``moor model``. Pin a dict catalog with
...
def _prepend_moa_picker_provider(providers: List[dict], current_provider: str = "") -> List[dict]:
    ...
    """
    try:
        from moor_cli.inventory import _moa_provider_row
...
def list_picker_providers(
    ...
    - OpenRouter's model list is replaced with the output of
      :func:`moor_cli.models.fetch_openrouter_models`, which filters the
