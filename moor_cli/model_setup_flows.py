...
Extracted from ``moor_cli/main.py`` as part of the god-file decomposition
campaign (``~/.moor/plans/god-file-decomposition.md``, Phase 2 — splitting
...
in main.py re-imports them (``from moor_cli.model_setup_flows import *``-style
explicit import) so existing call sites — and test monkeypatches that target
``moor_cli.main._model_flow_*`` — keep resolving against main.py's namespace.
...
imported lazily inside the flows (``from moor_cli.main import ...`` resolves at
call time, when main.py is fully loaded) so this module never imports
``moor_cli.main`` at import time -> no import cycle.
"""
...
def _existing_api_key_for_model_flow(provider_id: str, pconfig) -> tuple[str, str]:
    ...
    from moor_cli.auth import _resolve_api_key_provider_secret
...
def _model_flow_openrouter(config, current_model=""):
    ...
    from moor_cli.main import _prompt_api_key
    ...
    # in-flow (K/R/C) instead of having to edit ~/.moor/.env by hand. The
    # previous bypass-when-key-exists branch left no way to recover from a
    # bad paste short of re-running `moor setup` from scratch. OpenRouter
    ...
    from moor_cli.models import model_ids, get_pricing_for_provider
...
def _model_flow_ai_gateway(config, current_model=""):
    ...
    # Route through _prompt_api_key so users can replace a stale/broken key
    # in-flow (K/R/C) instead of having to edit ~/.moor/.env by hand.
    ...
    from moor_cli.models import ai_gateway_model_ids, get_pricing_for_provider
...
def _model_flow_moa(config, current_model=""):
    ...
    """
    from moor_cli.auth import _save_model_choice, deactivate_provider
    from moor_cli.config import load_config, save_config
...
def _model_flow_moor(config, current_model="", args=None):
    ...
    if not state or not state.get("access_token"):
        print("Not logged into Moor Portal. Starting login...")
        print()
        try:
            ...
            _login_moor(mock_args, PROVIDER_REGISTRY["moor"])
    ...
    try:
        ...
        if relogin:
            ...
            print("Re-authenticating with Moor Portal...\n")
            try:
                mock_args = argparse.Namespace(
                    portal_url=None,
                    inference_url=None,
    ...
    if not free_tier:
        try:
            refreshed_creds = resolve_moor_runtime_credentials(
                force_refresh=True,
            )
            if refreshed_creds:
    ...
    _moor_portal_url = ""
    try:
        _moor_state = get_provider_auth_state("moor")
        if _moor_state:
            _moor_portal_url = _moor_state.get("portal_base_url", "")
    except Exception:
        pass
    ...
    if free_tier:
        ...
        model_ids, pricing = union_with_portal_free_recommendations(
            model_ids, pricing, _moor_portal_url,
        )
    ...
    if free_tier and not model_ids:
        ...
        if unavailable_models:
            ...
            _url = (_moor_portal_url or DEFAULT_MOOR_PORTAL_URL).rstrip("/")
    ...
    if selected:
        _save_model_choice(selected)
        # Reactivate Moor as the provider and update config
...
def _model_flow_openai_codex(config, current_model=""):
    ...
    _codex_token = None
    # Prefer credential pool (where `moor auth` stores device_code tokens),
...
def _model_flow_xai_oauth(_config, current_model="", *, args=None):
    ...
    # live only in the pool (e.g. after ``moor auth add xai-oauth``).  Fall
...
def _model_flow_qwen_oauth(_config, current_model=""):
    ...
    from moor_cli.main import _DEFAULT_QWEN_PORTAL_MODELS
    from moor_cli.auth import (
...
def _model_flow_custom(config):
    ...
    """
    from moor_cli.main import _auto_provider_name, _prompt_custom_api_mode_selection, _save_custom_provider
    from moor_cli.auth import _save_model_choice, deactivate_provider
    from moor_cli.config import (
    ...
    from moor_cli.models import probe_api_models
    ...
    if model_name:
        ...
        print("Endpoint saved. Use `/model` in chat or `moor model` to set a model.")
...
def _model_flow_azure_foundry(config, current_model=""):
    ...
    * Manual entry when every probe fails (private endpoints, etc.).
    ...
    """
    from moor_cli.auth import _save_model_choice, deactivate_provider  # noqa: F401
    from moor_cli.config import (
        get_env_value,
        save_env_value,
        load_config,
    ...
    from moor_cli import azure_detect
    ...
    print("Anthropic-style API endpoints.  Moor will probe your")
    ...
    if use_entra:
        ...
        if not has_azure_identity_installed():
            ...
            print(
                "  Moor will install it now (the preflight below "
...
def _model_flow_named_custom(config, provider_info):
    ...
    """
    from moor_cli.main import _custom_provider_api_key_config_value, _custom_provider_base_url_config_value, _save_custom_provider
    from moor_cli.auth import _save_model_choice, deactivate_provider
    from moor_cli.config import load_config, normalize_extra_headers, save_config
...
def _model_flow_copilot(config, current_model=""):
    ...
    from moor_cli.main import _current_reasoning_effort, _prompt_reasoning_effort_selection, _set_reasoning_effort
    from moor_cli.auth import (
        PROVIDER_REGISTRY,
        _prompt_model_selection,
        _save_model_choice,
    ...
    from moor_cli.config import save_env_value, load_config, save_config
    from moor_cli.models import (
        _PROVIDER_MODELS,
        fetch_api_models,
...
def _model_flow_copilot_acp(config, current_model=""):
    ...
    print("  GitHub Copilot ACP delegates Moor turns to `copilot --acp`.")
    print("  Moor currently starts its own ACP subprocess for each request.")
    print("  Moor uses your selected model as a hint for the Copilot ACP session.")
    ...
    try:
        ...
        print(f"  ⚠ {exc}")
        print(
            "  Set MOOR_COPILOT_ACP_COMMAND or COPILOT_CLI_PATH if Copilot CLI is installed elsewhere."
        )
        return
...
def _model_flow_stepfun(config, current_model=""):
    ...
    from moor_cli.main import _infer_stepfun_region, _prompt_api_key, _prompt_provider_choice, _stepfun_base_url_for_region
    from moor_cli.auth import (
        PROVIDER_REGISTRY,
        _prompt_model_selection,
        _save_model_choice,
...
def _model_flow_bedrock_api_key(config, region, current_model=""):
    ...
    # Check env var and credential pool (keys added via `moor auth`)
    from moor_cli.auth import _resolve_api_key_provider_secret, ProviderConfig
...
def _model_flow_vertex(config, current_model=""):
    ...
    *path* lives in .env (VERTEX_CREDENTIALS_PATH / GOOGLE_APPLICATION_CREDENTIALS);
    ...
    from moor_cli.config import load_config, save_config, get_env_value
    from moor_cli.models import _PROVIDER_MODELS
    ...
    if sa_path:
        ...
        print("      • set VERTEX_CREDENTIALS_PATH in ~/.moor/.env to a service account JSON")
    print()
...
def _select_zai_endpoint(current_base: str) -> str:
    ...
    sourced from ``ZAI_ENDPOINTS`` in ``moor_cli.auth`` so it stays in
    sync with the probe list.

    Returns the selected base URL.  Falls back to *current_base* on cancel
    or error.
    """
    from moor_cli.main import _prompt_provider_choice
    ...
    if provider_id == "gemini" and existing_key:
        ...
        if probe_gemini_tier is not None:
            ...
            if tier == "free":
                ...
                print(
                    "   Moor typically makes 3-10 API calls per user turn "
                ...
                print()
                print(
                    "   To use Gemini with Moor, enable billing on your "
                ...
                print(
                    ...
                    "OpenRouter (free models), Groq, Moor."
                )
                print()
...
def _model_flow_anthropic(config, current_model=""):
    ...
    from moor_cli.main import _run_anthropic_oauth_flow
    from moor_cli.auth import (
        _prompt_model_selection,
        _save_model_choice,
        deactivate_provider,
    ...
    if selected:
        ...
        # providers if the user switches without running 'moor model'.
        cfg = load_config()
        model = cfg.get("model")
        if not isinstance(model, dict):
            model = {"default": model} if model else {}
