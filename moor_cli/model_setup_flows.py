"""Per-provider model-selection wizard flows for ``moor setup`` / ``moor model``.

main / config / auth / models helpers are imported lazily inside bodies: avoids the main.py import
cycle and lets tests patch ``moor_cli.config.load_config`` etc. at call time. The shared skeleton
lives in :mod:`moor_cli.model_setup_flows_common`; the custom / Azure / Bedrock flows live in their
...
from moor_cli.model_setup_flows_common import (
...
from moor_cli.model_setup_flows_custom import _model_flow_custom, _model_flow_named_custom
from moor_cli.model_setup_flows_azure import _model_flow_azure_foundry
from moor_cli.model_setup_flows_bedrock import _model_flow_bedrock
...
def _model_flow_openrouter(config, current_model=""):
    ...
    from moor_cli.auth import ProviderConfig, _prompt_model_selection
    ...
    from moor_cli.models import model_ids
...
def _model_flow_ai_gateway(config, current_model=""):
    ...
    from moor_cli.main_provider_setup import _prompt_api_key
    from moor_cli.auth import PROVIDER_REGISTRY, _prompt_model_selection
    from moor_cli.config import get_env_value
    ...
    from moor_cli.models import ai_gateway_model_ids
    from moor_cli.models_pricing import get_pricing_for_provider
...
def _model_flow_moa(config, current_model=""):
    ...
    from moor_cli.auth import _save_model_choice
    ...
    if not presets:
        print("No MoA presets configured. Run `moor moa configure <name>` first.")
        return
...
def _moor_login_args(args) -> argparse.Namespace:
...
def _moor_model_catalog(free_tier: bool, portal_url: str, model_ids: list, pricing: dict):
    """Free/paid-tier catalog for the Moor picker: ``(model_ids, pricing, unavailable_models,
    ...
    if free_tier:
        try:
            from moor_cli.moor_account import format_moor_portal_entitlement_message, get_moor_portal_account_info
            _account_info = get_moor_portal_account_info(force_fresh=True)
            unavailable_message = format_moor_portal_entitlement_message(_account_info, capability="paid Moor models") or ""
        except Exception:
            unavailable_message = ""
    ...
    if free_tier:
        model_ids, unavailable_models = partition_moor_models_by_tier(model_ids, pricing, free_tier=True)

    if not model_ids and not unavailable_models:
        print("No models available for Moor Portal after filtering.")
        return None
    if free_tier and not model_ids:
        print("No free models currently available.")
        if unavailable_models:
            from moor_cli.auth import DEFAULT_MOOR_PORTAL_URL
            _url = (portal_url or DEFAULT_MOOR_PORTAL_URL).rstrip("/")
            print(unavailable_message or f"Upgrade at {_url} to access paid models.")
        return None
    return model_ids, pricing, unavailable_models, unavailable_message, _policy_narrowed


def _moor_verified_credentials(creds_or_none=None):
    """Resolve Moor runtime credentials; on failure print the diagnosis (re-login when the
    session expired) and return None."""
    from moor_cli.auth import (
        AuthError, PROVIDER_REGISTRY, _login_moor, format_auth_error, resolve_moor_runtime_credentials)

    try:
        return resolve_moor_runtime_credentials()
    except Exception as exc:
        relogin = isinstance(exc, AuthError) and exc.relogin_required
        msg = format_auth_error(exc) if isinstance(exc, AuthError) else str(exc)
        if relogin:
            _say(f"Session expired: {msg}", "Re-authenticating with Moor Portal...\n")
            try:
                _login_moor(_moor_login_args(None), PROVIDER_REGISTRY["moor"])
...
def _moor_persist_selection(selected: str, creds: dict) -> dict:
    """Moor persist step: model choice + provider state, then rewrite ``model`` on a fresh
    ...
    from moor_cli.auth import _save_model_choice, _update_config_for_provider
    from moor_cli.config import get_env_value, load_config, save_config, save_env_value
    _save_model_choice(selected)
    inference_url = creds.get("base_url", "")
    _update_config_for_provider("moor", inference_url)
    config = load_config()
...
def _model_flow_moor(config, current_model="", args=None):
    """Moor Portal provider: ensure logged in, then pick model."""
    from moor_cli.auth import get_provider_auth_state, _prompt_model_selection, _login_moor, PROVIDER_REGISTRY
    from moor_cli.config import load_config
    from moor_cli.moor_subscription import prompt_enable_tool_gateway
    state = get_provider_auth_state("moor")
    if not state or not state.get("access_token"):
        _say("Not logged into Moor Portal. Starting login...", "")

        def _login_then_offer_gateway(login_args, pconfig):
            _login_moor(login_args, pconfig)
            # Offer Tool Gateway enablement for paid subscribers
            with contextlib.suppress(Exception):
                prompt_enable_tool_gateway(load_config() or {})

        # login_moor already handles model selection + config update
        _run_login(_login_then_offer_gateway, _moor_login_args(args), PROVIDER_REGISTRY["moor"])
        return
    ...
    if not free_tier:
        ...
        try:
            creds = resolve_moor_runtime_credentials(force_refresh=True) or creds
        except Exception:
    ...
    _moor_portal_url = ""
    with contextlib.suppress(Exception):
        _moor_portal_url = (get_provider_auth_state("moor") or {}).get("portal_base_url", "")

    catalog = _moor_model_catalog(free_tier, _moor_portal_url, model_ids, pricing)
    if catalog is None:
        return
    ...
    selected = _prompt_model_selection(
        ...
        portal_url=_moor_portal_url, unavailable_message=unavailable_message, confirm_provider="moor",
        confirm_base_url=creds.get("base_url", ""), confirm_api_key=creds.get("api_key", ""))
    if not selected:
        print("No change.")
        return
    config = _moor_persist_selection(selected, creds)
    print(f"Default model set to: {selected} (via Moor Portal)")
    # Offer Tool Gateway enablement for paid subscribers
...
def _model_flow_openai_codex(config, current_model=""):
    ...
    # Prefer the credential pool (where `moor auth` stores device_code tokens),
...
def _model_flow_xai_oauth(_config, current_model="", *, args=None):
    ...
    # credentials may live only in the pool (``moor auth add xai-oauth``) — fall back to
...
def _model_flow_qwen_oauth(_config, current_model=""):
    ...
    from moor_cli.main_provider_setup import _DEFAULT_QWEN_PORTAL_MODELS
    from moor_cli.auth import (
...
def _copilot_catalog(api_key: str):
    ...
    from moor_cli.models import fetch_github_model_catalog, normalize_copilot_model_id
...
def _copilot_obtain_token() -> bool:
    ...
    if choice == "1":
        try:
            from moor_cli.copilot_auth import copilot_device_code_login
    ...
    if choice == "2":
        ...
        with contextlib.suppress(ImportError):
            from moor_cli.copilot_auth import validate_copilot_token
...
def _model_flow_copilot(config, current_model=""):
    ...
    from moor_cli.config import load_config
    from moor_cli.models import fetch_api_models, github_model_reasoning_efforts, copilot_model_api_mode
    provider_id = "copilot"
...
def _model_flow_copilot_acp(config, current_model=""):
    ...
    _say("  GitHub Copilot ACP delegates Moor turns to `copilot --acp`.",
         "  Moor currently starts its own ACP subprocess for each request.",
         "  Moor uses your selected model as a hint for the Copilot ACP session.",
         f"  Command: {resolved_command}", f"  Backend marker: {effective_base}", "")
    try:
        creds = resolve_external_process_provider_credentials(provider_id)
    except Exception as exc:
        _say(f"  ⚠ {exc}", "  Set MOOR_COPILOT_ACP_COMMAND or COPILOT_CLI_PATH if Copilot CLI is installed elsewhere.")
        return
...
def _model_flow_kimi(config, current_model=""):
    ...
    from moor_cli.auth import PROVIDER_REGISTRY, KIMI_CODE_BASE_URL
...
def _model_flow_stepfun(config, current_model=""):
    ...
    from moor_cli.main_provider_setup import _infer_stepfun_region, _prompt_provider_choice, _stepfun_base_url_for_region
    from moor_cli.auth import PROVIDER_REGISTRY
    from moor_cli.config import save_env_value
    from moor_cli.models import _PROVIDER_MODELS, fetch_api_models
    provider_id = "stepfun"
    pconfig = PROVIDER_REGISTRY[provider_id]
    base_url_env = pconfig.base_url_env_var or ""
...
def _model_flow_vertex(config, current_model=""):
    ...
    if sa_path:
        ...
        _say("  Vertex credentials: Application Default Credentials (ADC)",
             ...
             "      • set VERTEX_CREDENTIALS_PATH in ~/.moor/.env to a service account JSON")
    print()
...
def _select_zai_endpoint(current_base: str) -> str:
    """Picker for the official Z.AI endpoints (``ZAI_ENDPOINTS`` in ``moor_cli.auth``, kept in
    sync with the probe list) plus a custom-proxy option. Returns the selected base URL;
    *current_base* on cancel/error."""
    from moor_cli.main_provider_setup import _prompt_provider_choice
...
_GEMINI_FREE_TIER_NOTICE = (
    ...
    "   Moor typically makes 3-10 API calls per user turn (tool iterations + auxiliary tasks),",
    "   so the free tier is exhausted after a handful of messages and cannot sustain",
    "   an agent session.", "",
    "   To use Gemini with Moor, enable billing on your Google Cloud project and regenerate",
    "   the key in a billing-enabled project: https://aistudio.google.com/apikey", "",
    "   Alternatives with workable free usage: DeepSeek, OpenRouter (free models), Groq, Moor.", "",
...
def _model_flow_api_key_provider(config, provider_id, current_model=""):
    ...
    from moor_cli.auth import PROVIDER_REGISTRY
    from moor_cli.config import save_env_value, load_config
    from moor_cli.models import opencode_model_api_mode, normalize_opencode_model_id
    pconfig = PROVIDER_REGISTRY[provider_id]
...
def _anthropic_authenticate() -> bool:
    ...
    from moor_cli.main_provider_setup import _run_anthropic_oauth_flow
    from moor_cli.config import save_env_value, save_anthropic_api_key
...
_PLUGIN_COMPAT_LAZY = {
    'BEDROCK_GEO_PREFIXES': ('moor_cli.model_setup_flows_bedrock', 'BEDROCK_GEO_PREFIXES'),
    'bedrock_model_routable_from_region': ('moor_cli.model_setup_flows_bedrock', 'bedrock_model_routable_from_region'),
    'bedrock_region_geo_prefix': ('moor_cli.model_setup_flows_bedrock', 'bedrock_region_geo_prefix'),
    'custom_provider_slug': ('moor_cli.providers', 'custom_provider_slug'),
