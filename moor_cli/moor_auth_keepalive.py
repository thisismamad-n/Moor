...
MOOR_AUTH_KEEPALIVE_INTERVAL_SECONDS = 6 * 60 * 60
...
def _refresh_selected_pool_entry(
    *,
    ...
    """Refresh the current Moor credential pool entry when it is stale.

    Returns True when a pool entry exists and is usable/refreshed, False when a
    pool exists but no entry can be used, and None when no Moor pool exists.
    """
    try:
...
def refresh_moor_auth_keepalive_once(
    *,
    ...
    """Refresh Moor auth once if credentials are configured."""
...
def start_moor_auth_keepalive(
    *,
    interval_seconds: int = MOOR_AUTH_KEEPALIVE_INTERVAL_SECONDS,
    initial_delay_seconds: int = MOOR_AUTH_KEEPALIVE_INITIAL_DELAY_SECONDS,
    min_key_ttl_seconds: int = MOOR_INVOKE_JWT_MIN_TTL_SECONDS,
    timeout_seconds: Optional[float] = None,
) -> Optional[threading.Thread]:
    """Start the process-wide Moor auth keepalive thread."""
