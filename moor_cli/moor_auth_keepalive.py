"""Background keepalive for long-lived Moor Portal sessions."""
...
from moor_cli.auth import (
    ...
    MOOR_INVOKE_JWT_MIN_TTL_SECONDS,
    AuthError,
...
MOOR_AUTH_KEEPALIVE_INTERVAL_SECONDS = 15 * 60
MOOR_AUTH_KEEPALIVE_MIN_INTERVAL_SECONDS = 60
# Ticks per credential lifetime: four keeps refresh comfortably ahead of expiry without chatter.
MOOR_AUTH_KEEPALIVE_TICKS_PER_LIFETIME = 4
MOOR_AUTH_KEEPALIVE_INITIAL_DELAY_SECONDS = 60
MOOR_AUTH_KEEPALIVE_INTERVAL_CONFIG_KEY = "keepalive_interval_seconds"
...
def _timeout_seconds(value: Optional[float]) -> float:
    if value is not None:
        return float(value)
    try:
        return float(os.getenv("MOOR_MOOR_TIMEOUT_SECONDS", "15"))
    except (TypeError, ValueError):
        return 15.0


def _moor_config() -> dict:
    """The ``moor:`` section of config.yaml, or {} on any failure (config loader imported lazily)."""
    try:
        from moor_cli.config import load_config

        section = load_config().get("moor")
...
def _interval_seconds(value: Optional[int]) -> int:
    """Tick interval: explicit argument, then ``moor.keepalive_interval_seconds`` in config.yaml,
    ...
    if value is not None:
        try:
            ...
            return MOOR_AUTH_KEEPALIVE_INTERVAL_SECONDS
    raw = _moor_config().get(MOOR_AUTH_KEEPALIVE_INTERVAL_CONFIG_KEY)
    ...
    try:
        ...
        logger.warning(
            "Ignoring invalid moor.%s=%r; using %ds",
            MOOR_AUTH_KEEPALIVE_INTERVAL_CONFIG_KEY, raw, MOOR_AUTH_KEEPALIVE_INTERVAL_SECONDS,
        )
        return MOOR_AUTH_KEEPALIVE_INTERVAL_SECONDS


def _observed_lifetime_seconds() -> Optional[int]:
    """Server-issued lifetime (seconds) of the current Moor credentials; the shorter of the access
...
def _tick_seconds(configured_interval: int, lifetime: Optional[int]) -> int:
    ...
    derived = lifetime // MOOR_AUTH_KEEPALIVE_TICKS_PER_LIFETIME
    return max(MOOR_AUTH_KEEPALIVE_MIN_INTERVAL_SECONDS, min(configured_interval, derived))
...
def _refresh_selected_pool_entry(*, min_key_ttl_seconds: int, min_access_ttl_seconds: Optional[int] = None) -> Optional[bool]:
    ...
    no usable entry; None = no Moor pool.
    """
    try:
        ...
        logger.debug("Moor auth keepalive: credential pool unavailable: %s", exc)
        return None
    ...
    try:
        ...
        logger.debug("Moor auth keepalive: credential pool selection failed: %s", exc)
        return False
    if entry is None:
        return False
    ...
    if access_expiring or not key_usable:
        if pool.try_refresh_current() is None:
            return False
        logger.debug("Moor auth keepalive: refreshed credential pool entry")
    return True


def refresh_moor_auth_keepalive_once(
...
def _keepalive_loop(
    ...
    while not stop_event.is_set():
        ...
        refresh_moor_auth_keepalive_once(
            min_key_ttl_seconds=horizon, min_access_ttl_seconds=horizon, timeout_seconds=timeout_seconds
        )
        stop_event.wait(tick)


def start_moor_auth_keepalive(
    *, interval_seconds: Optional[int] = None,
    initial_delay_seconds: int = MOOR_AUTH_KEEPALIVE_INITIAL_DELAY_SECONDS,
    min_key_ttl_seconds: int = MOOR_INVOKE_JWT_MIN_TTL_SECONDS, timeout_seconds: Optional[float] = None,
) -> Optional[threading.Thread]:
    """Start the process-wide Moor auth keepalive thread (idempotent; None when disabled)."""
    ...
    with _keepalive_lock:
        ...
        _keepalive_thread = threading.Thread(
            target=_keepalive_loop, args=(_keepalive_stop,), daemon=True, name="moor-auth-keepalive",
            kwargs={
        ...
        logger.debug("Moor auth keepalive started")
        return _keepalive_thread


def stop_moor_auth_keepalive(timeout: float = 5.0) -> None:
