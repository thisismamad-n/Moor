...
# Once-per-process guards: load_moor_dotenv() runs repeatedly (user + project env, gateway hot-reload,
...
# env-var name → source label ("bitwarden", …) for externally injected credentials; setup / `moor
...
def _hydrate_profile_secret_sources(home: Path) -> dict[str, str]:
    ...
    try:
        ...
        # Mirror load_moor_dotenv()'s .op.env bootstrap (1Password token lives in gitignored .op.env)
...
def load_moor_dotenv(
    *,
    ...
    """Load Moor env files: ``~/.moor/.env`` overrides stale shell exports; project ``.env`` is a dev
    ...
    if is_multiplex_active() and get_moor_home_override() is not None:
    ...
    # Windows (#73381, #86735). 2. A fresh ``moor update`` retry just completed a deferred dependency
    ...
    # ~/.moor/.env win on every reload and flip the backend mid-session in long-lived processes.
    ...
    # with override=True — so a stale TERMINAL_ENV=docker left in ~/.moor/.env (e.g. written by an older
    # `moor setup` before the user switched terminal.backend in config.yaml) silently wins again on every
    # reload. Startup launchers bridge config→env once, but long-lived processes (gateway per-turn reload,
    # cron standalone runs) call load_moor_dotenv() repeatedly and used to flip the effective backend back
...
def _apply_external_secret_sources(home_path: Path) -> None:
    ...
    tokens), BEFORE Moor reads credentials; failures never block startup. Precedence/conflicts/provenance
    ...
    # load_moor_dotenv() calls per startup don't re-fetch / re-print — error retries within one process
...
def _load_secrets_config(home_path: Path) -> dict:
    ...
    # populating it lets main.py's early bridge and moor_logging reuse one parse instead of 3-4.
...
def _process_moor_home() -> Path:
    """The MOOR_HOME the running process was launched under.

    Must be the *true* process home, ignoring any context-local
    ``set_moor_home_override`` a per-request task has installed. Both
    ...
    * ``_load_secrets_config`` uses it to decide whether the shared
    ...
    ``moor_constants.get_process_moor_home()`` is the override-immune
