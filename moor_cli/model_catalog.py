...
The Moor docs site hosts a JSON manifest of curated models for providers
we want to update without shipping a release (currently OpenRouter and
Moor Portal). This module fetches, validates, and caches that manifest,
...
def get_catalog(*, force_refresh: bool = False) -> dict[str, Any]:
    ...
    # this via get_curated_moor_model_ids on every open) never block on the
...
def get_curated_moor_models() -> list[str] | None:
    """Return Moor Portal's curated list of model ids from the manifest.

    Returns ``None`` when the manifest is unavailable.
    """
    block = _get_provider_block("moor")
    if not block:
        return None
    out: list[str] = []
    for m in block.get("models", []):
...
def get_default_model_from_cache(provider: str) -> str | None:
    ...
    ``"default": true``; that entry is the model Moor silently lands on when
    ...
    network-free. The cache is kept fresh by the picker/`moor update` paths;
...
def seed_cache_from_checkout(project_root: "Path | str") -> bool:
    ...
    ``moor update`` pulls the latest repo, so the freshly-pulled
    ...
    to ``~/.moor/cache/model_catalog.json`` via the same atomic writer the
