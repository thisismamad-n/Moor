...
``~/.moor/cache/model_catalog.json`` → master URL fetch; any fetch failure keeps the stale copy
(or ``{}``). ``get_curated_openrouter_models()`` / ``get_curated_moor_models()`` are thin accessors
...
# Fallback to upstream mirrors if raw GitHub is unreachable or delayed.
...
def _cache_path() -> Path:
    ...
    return get_moor_home() / "cache" / "model_catalog.json"
...
def _fetch_manifest(url: str, timeout: float) -> dict[str, Any] | None:
    ...
    try:
        req = urllib.request.Request(url, headers={"Accept": "application/json", "User-Agent": _MOOR_USER_AGENT})
...
def refresh_catalogs() -> bool:
    """Force-refresh every remote catalog the picker reads (manifest, OpenRouter live list, Moor Portal
    ...
    try:
        from moor_cli.models import fetch_moor_recommended_models, fetch_openrouter_models

        fetch_openrouter_models(force_refresh=True)
        fetch_moor_recommended_models(force_refresh=True)
    except Exception:
...
def get_curated_moor_models() -> list[str] | None:
    """Moor Portal's curated model ids from the manifest."""
    return [mid for mid, _ in _block_ids(_get_provider_block("moor"))] or None
...
def get_default_model_from_cache(provider: str) -> str | None:
    """The manifest's labeled default for ``provider`` (the model Moor silently lands on when the
...
def seed_cache_from_checkout(project_root: "Path | str") -> bool:
    ...
    After ``moor update`` that file IS the newest catalog, so the picker stays current even when
