"""``moor profile`` command — one handler per action, dispatched by ``PROFILE_ACTIONS``.

Imports from ``moor_cli.profiles`` stay lazy (inside each handler) so tests can monkeypatch
...
def _profile_status(args):
    ...
    from moor_cli.profiles import format_profile_label, get_active_profile_name, list_profiles
    ...
    if current is not None:
        ...
        if p.alias_path:
            print(f"Alias:          {p.alias_name or p.name} → moor -p {p.name}")
    print()
...
def _profile_use(args):
    from moor_cli.profiles import set_active_profile
    name = args.profile_name
    try:
        set_active_profile(name)
        print("Switched to: default (~/.moor)" if name == "default" else f"Switched to: {name}")
...
def _profile_create(args):
    ...
    if cloned:
        if clone_all:
            ...
            from moor_cli.profile_memory_config import cloned_memory_provider
    ...
    if not no_alias:
        ...
        if collision:
            ...
            print(f"  Choose a custom alias:  moor profile alias {name} --name <custom>")
...
def _profile_delete(args):
    from moor_cli.profiles import delete_profile
    try:
...
def _describe_target_dir(name: str) -> Path:
    """Profile dir for ``describe``: ``default`` maps to the CURRENT home (get_moor_home),
...
def _profile_show(args):
    ...
    if alias_name:
        print(f"Alias:   {alias_name} → moor -p {name}  ({_wrapper_path(alias_name)})")
    print()
...
def _profile_purge_identity(args):
    ...
    if not purged:
        _die(f"Error: session identity was not purged. Restart or stop the gateway, then run:\n"
             f"    moor profile purge-identity {args.profile_name}", err=True)
    print(f"✓ Session/routing identity purged: {args.profile_name}")


def _profile_export(args):
    from moor_cli.profiles import export_profile, get_profile_export_path
    name = args.profile_name
    try:
...
def _profile_install(args):
    import tempfile
    from moor_cli.profile_distribution import DistributionError, install_distribution, plan_install
    try:
...
def _profile_update(args):
    from moor_cli.profile_distribution import DistributionError, read_manifest, update_distribution
    ...
    try:
        ...
        if plan.has_cron:
            print(f"  Cron files were refreshed.  Review with:  moor -p {plan.manifest.name} cron list")
    except (DistributionError, ValueError) as e:
        _die(f"Error: {e}")
...
# Order mirrors the original if/elif chain; None = bare ``moor profile``.
PROFILE_ACTIONS = {
