...
- **Aggregators** (OpenRouter, Moor, AI Gateway, Kilo Code) need
...
- **DeepSeek** accepts only the first-class V-series IDs
  ...
  Moor revisions folded every non-reasoner input into
...
# slug used by aggregator APIs (OpenRouter, Moor, etc.).
#
...
def _normalize_provider_alias(provider_name: str) -> str:
    """Resolve provider aliases to Moor' canonical ids."""
...
def normalize_model_for_provider(model_input: str, target_provider: str) -> str:
    ...
    Args:
        ...
        target_provider: The canonical Moor provider id, e.g.
            ``"openrouter"``, ``"anthropic"``, ``"copilot"``,
            ``"deepseek"``, ``"custom"``.  Should already be normalised
            via ``moor_cli.models.normalize_provider()``.
