"""Cross-process mutual exclusion for in-flight Moor updates.

Three different surfaces can start an update of the same install tree:

* ``moor update`` from a terminal,
* the dashboard's Update button (``POST /api/moor/update`` →
  ``_spawn_moor_action(["update"])``, detached),
* the desktop's Update button, which hands off to the Tauri
  ``moor-setup --update`` and, on its failure screen, to install-mode
...
once — so a dashboard-spawned ``moor update`` and an installer-driven
...
unchanged and remain byte-compatible with the Rust and Electron readers:
    ...
    <MOOR_HOME>/.moor-update-in-progress   body: "<pid>\\n<started_at_unix>"
...
then spawns ``moor update`` as a child stage. Without a handoff the child
sees its own parent's live marker and refuses — the GUI update deadlocks
against itself on every attempt ("Moor is still running", retry forever).
...
* A live holder that is a *process ancestor* of ours is likewise our own
  ...
  ``moor-setup`` binary under ``~/.moor`` is only refreshed by a full
  installer run (``copy_self_to_moor_home`` deliberately no-ops during
  ...
  ancestry check those users get exit 2 ("Moor is still running") on every
  GUI update forever, with no Moor process actually running.
"""
...
# Set by an orchestrating updater (the Tauri `moor-setup --update` flow) to
# its own pid before spawning `moor update` as a child stage. The parent
...
HANDOFF_PID_ENV = "MOOR_UPDATE_HANDOFF_PID"
...
# to show "Moor is still running" instead of a generic failure. Naming it
...
def update_marker_path() -> Path:
    ...
    Uses the *process* Moor home (never the context-local profile override):
    the Rust updater resolves ``$MOOR_HOME`` or the platform default, and the
    ...
    return get_process_moor_home() / MARKER_NAME
...
def _is_ancestor_pid(pid: int) -> bool:
    ...
    The orchestrating updater spawns ``moor update`` as a (grand)child, so a
    ...
    parent chain. This heals the fleet of staged ``moor-setup`` binaries
...
def describe_holder(holder: UpdateHolder) -> str:
    ...
    return (
        f"✗ Another Moor update is already running (PID {holder.pid}, "
...
class UpdateLock:
    ...
    def acquire(self) -> bool:
        ...
        updater spawning `moor update` as a stage): we run under ITS claim
