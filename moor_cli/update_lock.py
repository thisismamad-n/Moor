...
# Set by an orchestrating updater (Tauri `moor-setup --update`) to its own pid before
# spawning `moor update` as a child stage; the parent holds the marker for its whole run,
...
# Tauri updater (UPDATE_EXIT_CONCURRENT in update.rs) to show "Moor is still running".
...
def update_marker_path() -> Path:
    ...
    Uses the *process* Moor home (never the context-local profile override): the Rust
    updater resolves ``$MOOR_HOME`` or the platform default and the desktop pins that same
...
def _is_ancestor_pid(pid: int) -> bool:
    ...
    The orchestrating updater spawns ``moor update`` as a (grand)child, so a live marker
...
def describe_holder(holder: UpdateHolder) -> str:
    ...
    return (
        f"✗ Another Moor update is already running (started {elapsed} ago, "
        ...
        "  started it, then run `moor update` again."
    )
