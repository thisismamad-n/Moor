"""Migrate Moor' MCP server config and Codex's installed curated plugins
...
module:
  ...
  1. Reads Moor' YAML and writes equivalent [mcp_servers.<name>]
...
What translates (MCP servers):
  Moor mcp_servers.<n>.command/args/env  → codex stdio transport
  Moor mcp_servers.<n>.url/headers       → codex streamable_http transport
  Moor mcp_servers.<n>.timeout           → codex tool_timeout_sec
  Moor mcp_servers.<n>.connect_timeout   → codex startup_timeout_sec

What does NOT translate (warned + skipped):
  Moor-specific keys (sampling, etc.) — codex's MCP client has no
  equivalent. Listed in the per-server skipped[] field of the report.

What's NOT migrated (intentional):
  AGENTS.md — codex respects this file natively in its cwd. Moor' own
...
class MigrationReport:
    ...
    def summary(self) -> str:
        ...
        if self.migrated:
            ...
            lines.append("No MCP servers found in Moor config.")
...
# Moor keys that codex's MCP schema doesn't support — dropped during
...
_KEYS_DROPPED_WITH_WARNING = {
    # Moor' sampling subsection — codex MCP has no equivalent
    "sampling",
}


def _translate_one_server(
    name: str, moor_cfg: dict
) -> tuple[Optional[dict], list[str]]:
    """Translate one Moor MCP server config to the codex inline-table dict
    ...
    has_command = bool(moor_cfg.get("command"))
    has_url = bool(moor_cfg.get("url"))
    ...
    for key in moor_cfg:
        ...
        elif key not in _KNOWN_MOOR_KEYS:
            skipped.append(f"{key} (unknown Moor key)")
...
def _format_toml_value(value: Any) -> str:
    ...
    if isinstance(value, str):
        ...
        # chars but env-var passthrough (MOOR_HOME, PYTHONPATH) could
...
def render_codex_toml_section(
    ...
    if not servers and not plugins and not default_permission_profile:
        out.append("# (no MCP servers, plugins, or permissions configured by Moor)")
...
def _insert_managed_block_at_top_level(user_text: str, managed_block: str) -> str:
    """Insert Moor' managed Codex TOML block while keeping root keys root-scoped.
...
def _strip_unmanaged_plugin_tables(toml_text: str) -> str:
    ...
    directly (i.e. before Moor' migrate has ever touched the file). When we
...
def _strip_existing_managed_block(toml_text: str) -> str:
    ...
    this code wrote so re-runs don't break configs from prior Moor
    versions."""
    lines = toml_text.splitlines(keepends=True)
    out: list[str] = []
    in_managed = False
...
def _looks_like_test_tempdir(path: str) -> bool:
    ...
    what pytest's tempdir factory uses by default. If a MOOR_HOME pointing
    at one of those paths is burned into ``~/.codex/config.toml``, every
    codex-routed moor-tools call fails silently once the directory is GC'd.

    We err on the side of refusing — losing a (very unlikely) real
    ``~/.moor`` symlink that happens to live under ``/private/var/folders``
...
def _build_moor_tools_mcp_entry() -> dict:
    """Build the codex stdio-transport entry that launches Moor' own
    ...
    so a moor installed under /opt/, /usr/local/, or a venv all work.
    MOOR_HOME and PYTHONPATH are passed through so the spawned process
    sees the same config + module layout the user is running."""
    import sys

    env: dict[str, str] = {}
    # MOOR_HOME passes through IF SET so the MCP subprocess sees the same
    # config / auth / sessions DB as the parent CLI. Read from os.environ
    # (not get_moor_home()) on purpose: when the env var is unset we want
    # codex's subprocess to inherit whatever MOOR_HOME its launcher sets
    ...
    # — that would override the launcher's MOOR_HOME and pin the subprocess
    # to the wrong profile.
    #
    # The pytest-tempdir guard below catches the issue #26250 Bug C scenario:
    # a sibling test's monkeypatch.setenv("MOOR_HOME", tmp_path) would
    ...
    moor_home = os.environ.get("MOOR_HOME") or ""
    if moor_home and _looks_like_test_tempdir(moor_home):
        moor_home = ""
    if moor_home:
        env["MOOR_HOME"] = moor_home
    # PYTHONPATH passes through so a worktree-launched moor finds the
    ...
    env["MOOR_QUIET"] = "1"
    env["MOOR_REDACT_SECRETS"] = env.get("MOOR_REDACT_SECRETS", "true")
...
def migrate(
    ...
    *,
    ...
    expose_moor_tools: bool = True,
) -> MigrationReport:
    """Translate Moor mcp_servers config + Codex curated plugins into
    ~/.codex/config.toml.

    Args:
        moor_config: full ~/.moor/config.yaml dict
        ...
        expose_moor_tools: when True (default), register Moor' own
            tool surface (web_search, browser_*, delegate_task, vision,
            memory, skills, etc.) as an MCP server in ~/.codex/config.toml
            so the codex subprocess can call back into Moor for tools
    ...
    moor_servers = (moor_config or {}).get("mcp_servers") or {}
    if not isinstance(moor_servers, dict):
        report.errors.append(
            "mcp_servers in Moor config is not a dict; cannot migrate."
        )
        return report

    translated: dict[str, dict] = {}
    for name, cfg in moor_servers.items():
    ...
    # Inject Moor' own tool surface as an MCP server so the spawned
    # codex subprocess can call back into Moor for the tools codex
    # doesn't ship with — web_search, browser_*, delegate_task, vision,
    # memory, skills, session_search, image_generate, text_to_speech.
    # The server itself is agent/transports/moor_tools_mcp_server.py
    # and is launched on demand by codex (stdio MCP).
    if expose_moor_tools:
        translated["moor-tools"] = _build_moor_tools_mcp_entry()
        if "moor-tools" not in report.migrated:
            report.migrated.append("moor-tools")
