"""Migrate Moor MCP server config and Codex's installed curated plugins into ~/.codex/config.toml.
"""
...
MIGRATION_END_MARKER = "# end moor-agent managed section"
...
# Moor MCP keys codex understands (transport stdio/http, timeouts, general). Any other key is
# dropped with a warning: ``sampling`` has no codex equivalent; the rest are unknown Moor keys.
_KNOWN_MOOR_KEYS = {
...
# (moor key, codex key, skip note) — timeouts are emitted as floats or skipped when non-numeric.
_TIMEOUT_KEYS = (
...
def _translate_one_server(name: str, moor_cfg: dict) -> tuple[Optional[dict], list[str]]:
    """Translate one Moor MCP server config to codex's inline-table dict.

    Returns ``(codex_entry, skipped_keys)``; ``codex_entry`` is None when the config is unusable.
    stdio (``command``) wins over ``url`` when both are set. Moor' ``transport: sse`` hint is
    ...
    skipped: list[str] = []
    out: dict[str, Any] = {}
    if moor_cfg.get("command"):
        if moor_cfg.get("url"):
            skipped.append("url (both command and url set; preferring stdio)")
        out["command"] = str(moor_cfg["command"])
        if moor_cfg.get("args"):
            out["args"] = [str(a) for a in moor_cfg["args"]]
        if moor_cfg.get("env"):
            out["env"] = _str_map(moor_cfg["env"])
        if moor_cfg.get("cwd"):
            out["cwd"] = str(moor_cfg["cwd"])
    elif moor_cfg.get("url"):
        out["url"] = str(moor_cfg["url"])
        if moor_cfg.get("headers"):
            out["http_headers"] = _str_map(moor_cfg["headers"])
        if moor_cfg.get("transport") == "sse":
            skipped.append("transport=sse (codex auto-negotiates)")
    else:
        return None, ["no command or url field"]
    for moor_key, codex_key, note in _TIMEOUT_KEYS:
        if moor_key in moor_cfg:
            try:
                out[codex_key] = float(moor_cfg[moor_key])
...
# env-var passthrough (MOOR_HOME, PYTHONPATH) could carry one in pathological cases.
_TOML_ESCAPES = (
    ("\\", "\\\\"), ('"', '\\"'), ("\b", "\\b"), ("\t", "\\t"),
    ("\n", "\\n"), ("\f", "\\f"), ("\r", "\\r"))
...
def render_codex_toml_section(
    ...
    if not servers and not plugins and not default_permission_profile:
        out += ["# (no MCP servers, plugins, or permissions configured by Moor)", MIGRATION_END_MARKER]
        return "\n".join(out) + "\n"
...
def _looks_like_test_tempdir(path: str) -> bool:
    ...
    Such dirs are reaped between sessions; a MOOR_HOME pointing there burned into
...
def _build_moor_tools_mcp_entry() -> dict:
    """Codex stdio entry launching Moor' own tool surface as an MCP server (browser/web/
    delegate_task/vision/memory/skills call-backs).

    MOOR_HOME passes through only IF SET, read from os.environ (not get_moor_home()): when
    unset the codex subprocess must inherit its launcher's runtime MOOR_HOME (systemd, gateway,
    kanban), not a migrate-time default burned into config.toml that pins the wrong profile. The
    pytest-tempdir guard keeps a sibling test's monkeypatched MOOR_HOME out of the user's real
    config. PYTHONPATH passes through so a worktree-launched moor finds the branch's modules.
    """
    import sys
    env: dict[str, str] = {}
    # MOOR_HOME passes through IF SET so the MCP subprocess sees the same config / auth / sessions DB as
    # the parent CLI. Read from os.environ (not get_moor_home()) on purpose: when the env var is unset we
    # want codex's subprocess to inherit whatever MOOR_HOME its launcher sets at runtime (systemd unit,
    # gateway, kanban dispatcher, custom shell), rather than burning the migrate-time resolved default into
    # config.toml — that would override the launcher's MOOR_HOME and pin the subprocess to the wrong
    # profile. The pytest-tempdir guard below catches the issue #26250 Bug C scenario: a sibling test's
    # monkeypatch.setenv("MOOR_HOME", tmp_path) would otherwise leak a transient pytest tempdir into the
    # user's real ~/.codex/config.toml and silently brick codex once the tempdir is GC'd.
    moor_home = os.environ.get("MOOR_HOME") or ""
    if moor_home and not _looks_like_test_tempdir(moor_home):
...
def migrate(
    moor_config: dict, *, codex_home: Optional[Path] = None, dry_run: bool = False,
    discover_plugins: bool = True, default_permission_profile: Optional[str] = ":workspace",
    expose_moor_tools: bool = True) -> MigrationReport:
    """Translate Moor mcp_servers config + Codex curated plugins into ~/.codex/config.toml.
    ...
    approval prompt on every write. ``expose_moor_tools`` registers Moor' own tool surface
    (agent/transports/moor_tools_mcp_server.py, launched on demand by codex over stdio) as an MCP
    ...
    if not isinstance(moor_servers, dict):
        report.errors.append("mcp_servers in Moor config is not a dict; cannot migrate.")
        return report
    translated: dict[str, dict] = {}
    for raw_name, cfg in moor_servers.items():
        name = str(raw_name)
