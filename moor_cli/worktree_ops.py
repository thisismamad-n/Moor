"""Git worktree isolation for ``moor -w`` sessions: create, classify, prune.
...
def _setup_worktree(repo_root: str = None, sync_base: bool = True,
    ...
    *sync_base* branches from the fetched remote tip (``_resolve_worktree_base``), else local
    HEAD. *name* replaces the random ``moor-<id>``; named trees lack the ``moor-`` prefix so
    ...
    try:
        _git(["worktree", "lock", "--reason", f"moor pid={os.getpid()}", str(wt_path)], repo_root)
...
def _worktree_lock_is_live(repo_root: str, worktree_path: str, timeout: int = 10):
    """Lock state: ``"live"`` (owning pid runs), ``"dead"`` (pid gone / non-moor reason), None (unlocked).

    ``moor -w`` locks with reason ``moor pid=<pid>``; ``worktree remove --force`` refuses
...
def _prune_candidates(worktrees_dir: Path, max_age_hours: int, now: float) -> list:
    ...
    Kanban trees (``t_<hex>``) belong to the kanban gc. ``moor-*`` trees age on *max_age_hours*,
    ...
    for entry in sorted(worktrees_dir.iterdir()):
        if not entry.is_dir() or kanban_re.match(entry.name):
            continue
        tier_hours = max_age_hours if entry.name.startswith("moor-") else max_age_hours * 3
        try:
...
def _classify_prune_candidates(repo_root: str, candidates: list) -> list:
    ...
    def _classify(item):
        ...
        # Live lock = running moor; a dead lock is unlocked in phase 3.
...
def _prune_stale_worktrees(repo_root: str, max_age_hours: int = 24) -> None:
    ...
    if preserved_stale:
        logger.warning("Preserving %d worktree(s) older than 7 days with unmerged work "
                       "(run `moor worktree prune` to review and reclaim): %s",
    ...
    try:
        from moor_cli.worktree_gc import worktrees_summary
        ...
        if count >= 10 or (size_mb or 0) >= 5120:
            ...
            logger.warning(".worktrees/ holds %d tree(s) (%s) — run `moor worktree list` "
                           "to audit and `moor worktree prune` to reclaim safely.", count, size_txt)
    except Exception:
        pass


def _prune_orphaned_branches(repo_root: str, protect: Optional[set] = None) -> None:
    """Delete local ``moor/moor-*`` and ``pr-*`` branches with no worktree, except *protect*."""
    try:
