...
Custom-provider lookup lives in :mod:`moor_cli.runtime_provider_custom`; Azure Foundry,
OpenRouter/bare-custom, Bedrock and external-process builders in
:mod:`moor_cli.runtime_provider_backends` — both re-exported here so
``moor_cli.runtime_provider.<name>`` imports and test patches keep working."""
...
from moor_cli.providers import determine_api_mode, is_official_openai_host, moor_api_mode
from utils import base_url_host_matches, base_url_hostname, env_int


# Late-bound delegates, deliberately NOT module-level from-imports: this module is often imported
# lazily, so its first import can happen while a test has ``moor_cli.config.load_config`` patched
...
# Hosts that only speak one wire protocol. Mirrors host_mandated_api_mode in moor_cli/providers.py
...
def _api_key_provider_api_mode(provider: str, model_cfg: Dict[str, Any], api_key: str, base_url: str, effective_model: Any, *,
    ...
    if provider in ("xai", "actual"):
        ...
        # the host_mandated_api_mode clause in moor_cli/providers.py so the runtime resolver stays in
...
def resolve_requested_provider(requested: Optional[str] = None) -> str:
    """Provider request from explicit arg, then config, then ``MOOR_INFERENCE_PROVIDER``, else
    ...
    return _getenv("MOOR_INFERENCE_PROVIDER", "").strip().lower() or "auto"
...
from moor_cli.runtime_provider_backends import (  # noqa: E402,F401
...
def _pool_entry_mode_and_url(provider, entry, model_cfg, effective_model, base_url) -> tuple:
    ...
    if provider == "moor":
        return moor_api_mode(effective_model), (_moor_inference_env_override() or "") or base_url
    if provider == "copilot":
...
def _refresh_moor_pool_entry(pool: CredentialPool, entry: Any, pool_api_key: str):
    """Moor pool entries carry the agent_key (an invoke JWT) which the pool does not refresh on
    selection (avoids network calls in `moor auth list`); refresh here before falling back to
    singleton auth resolution. Returns (entry, pool_api_key) — key "" when still unusable."""
    min_ttl = _moor_min_key_ttl()
    if _moor_entry_key_usable(entry, min_ttl):
        return entry, pool_api_key
    logger.debug("Moor pool entry agent_key expired/missing, refreshing selected pool entry")
    try:
        refreshed = pool.try_refresh_current()
    except Exception as exc:
        logger.debug("Moor pool entry refresh failed: %s", exc)
        refreshed = None
    if refreshed is not None:
        entry, pool_api_key = refreshed, _pool_entry_api_key(refreshed)
    if not pool_api_key or not _moor_entry_key_usable(entry, min_ttl):
        logger.debug("Moor pool entry agent_key still unavailable, falling through to runtime resolution")
        pool_api_key = ""
    return entry, pool_api_key
...
def _explicit_moor(requested_provider, model_cfg, api_key, explicit_base_url, target_model):
    state = auth_mod.get_provider_auth_state("moor") or {}
    base_url = (explicit_base_url or _moor_inference_env_override()
                or str(state.get("inference_base_url") or auth_mod.DEFAULT_MOOR_INFERENCE_URL).strip().rstrip("/"))
    # The agent_key compatibility field is used for inference only when it holds a NAS invoke JWT;
    # raw OAuth access_token fallback is handled by resolve_moor_runtime_credentials().
    api_key = api_key or (str(state.get("agent_key") or "").strip() if _agent_key_is_usable(state, _moor_min_key_ttl()) else "")
    api_key, base_url, expires_at = _creds_fallback(api_key, explicit_base_url, base_url,
                                                    ...
                                                    _resolve_moor_creds)
    return _runtime("moor", moor_api_mode(_effective_model(model_cfg, target_model)), base_url, api_key, source="explicit",
...
_EXPLICIT_RESOLVERS: Dict[str, Callable[..., Dict[str, Any]]] = {
    "anthropic": _explicit_anthropic, "openai-codex": _explicit_codex, "moor": _explicit_moor,
...
_OAUTH_RUNTIME_PROVIDERS: Dict[str, _OAuthRuntimeSpec] = {
    "moor": _OAuthRuntimeSpec(_resolve_moor_creds, moor_api_mode, "portal", "expires_at",
                              "Auto-detected Moor provider but credentials failed"),
    "openai-codex": _OAuthRuntimeSpec(lambda: resolve_codex_runtime_credentials(), "codex_responses", "moor-auth-store",
                                      "last_refresh", "Auto-detected Codex provider but credentials failed"),
    "xai-oauth": _OAuthRuntimeSpec(lambda: resolve_xai_oauth_runtime_credentials(), "codex_responses", "moor-auth-store",
...
def _resolve_vertex_runtime(requested_provider: str) -> Dict[str, Any]:
    ...
    if not token or not base_url:
        raise AuthError("Vertex AI credentials could not be resolved. Vertex uses OAuth2 (not a static API key): provide a "
                        "service-account JSON via GOOGLE_APPLICATION_CREDENTIALS (or VERTEX_CREDENTIALS_PATH) in ~/.moor/.env, "
                        "or run 'gcloud auth application-default login' for ADC. Set the GCP project/region under vertex: in "
                        "config.yaml if they aren't embedded in the credentials. Run `moor setup` to install Vertex support.")
...
def resolve_runtime_provider(*, requested: Optional[str] = None, explicit_api_key: Optional[str] = None,
    ...
    rung returns or raises, else falls to the next):
      ...
      7. OAuth specs (moor/codex/xai/qwen; "auto" swallows AuthError and logs) → minimax-oauth
...
_PLUGIN_COMPAT_LAZY = {
    'custom_provider_aliases': ('moor_cli.providers', 'custom_provider_aliases'),
