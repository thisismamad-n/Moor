...
def _config_base_url_trustworthy_for_bare_custom(cfg_base_url: str, cfg_provider: str) -> bool:
    ...
    try:
        from moor_cli.auth import resolve_provider as _resolve_provider
...
def _copilot_runtime_api_mode(
    ...
    *,
    ...
    try:
        from moor_cli.models import copilot_model_api_mode
...
_VALID_API_MODES = {
    ...
    # instead of Moor' tool dispatch. Gated behind config key
...
def _parse_api_mode(raw: Any) -> Optional[str]:
    ...
    if isinstance(raw, str):
        from moor_cli.config import _canonical_api_mode
...
def _moor_inference_base_url_override() -> str:
    """Return the trusted Moor runtime base URL override, if configured.

    Delegates to ``auth._moor_inference_env_override`` so every
    ``MOOR_INFERENCE_BASE_URL`` read shares one normalization path
    ...
    """
    return _moor_inference_env_override() or ""
...
def _resolve_runtime_from_pool_entry(
    *,
    ...
    if provider == "openai-codex":
        ...
        api_mode = moor_api_mode(effective_model)
        base_url = _moor_inference_base_url_override() or base_url
...
def resolve_requested_provider(requested: Optional[str] = None) -> str:
    ...
    env_provider = _getenv("MOOR_INFERENCE_PROVIDER", "").strip().lower()
    if env_provider:
        return env_provider
...
def _get_named_custom_provider(requested_provider: str) -> Optional[Dict[str, Any]]:
    ...
    if requested_norm != "custom" and not requested_norm.startswith("custom:"):
        try:
            ...
            # the canonical provider itself (``moor``, ``openrouter``, …) so
            # accidentally shadowing a canonical provider still resolves to
            # the built-in. See tests/moor_cli/test_runtime_provider_resolution.py
...
def canonical_custom_identity(
    *,
    ...
    3. ``config_provider`` — the active ``config.model.provider`` (or its
       ``provider``/``MOOR_INFERENCE_PROVIDER`` equivalent). When neither
    ...
    if not candidate:
        try:
            ...
            candidate = ""
    if not candidate:
        candidate = os.environ.get("MOOR_INFERENCE_PROVIDER", "").strip()
...
def _resolve_azure_foundry_runtime(
    *,
    ...
    if not base_url:
        raise AuthError(
            "Azure Foundry requires a base URL. Set it via 'moor model' or "
...
def _resolve_explicit_runtime(
    *,
    ...
    if provider == "moor":
        ...
        base_url = (
            explicit_base_url
            or _moor_inference_base_url_override()
        ...
        # by resolve_moor_runtime_credentials().
        api_key = explicit_api_key or (
            str(state.get("agent_key") or "").strip()
            if _agent_key_is_usable(
                state,
                max(60, env_int("MOOR_MOOR_MIN_KEY_TTL_SECONDS", 1800)),
            )
            else ""
        )
        ...
        if not api_key:
            creds = resolve_moor_runtime_credentials(
                timeout_seconds=float(_getenv("MOOR_MOOR_TIMEOUT_SECONDS", "15")),
            )
            api_key = creds.get("api_key", "")
        ...
        return {
            "provider": "moor",
            "api_mode": moor_api_mode(target_model or model_cfg.get("default") or ""),
            "base_url": base_url,
            "api_key": api_key,
            "source": "explicit",
...
def resolve_runtime_provider(
    *,
    ...
    from moor_cli.config import is_provider_enabled, load_config
    ...
    if pool and pool.has_credentials():
        ...
        # For Moor, the pool entry's runtime_api_key is the agent_key
        # compatibility field. It must be an invoke JWT. The pool doesn't
        # refresh it during selection (that would trigger network calls in
        # non-runtime contexts like `moor auth list`). If the key is
        # expired/missing, refresh the selected pool entry before falling back
        # to singleton auth resolution.
        if provider == "moor" and entry is not None:
            min_ttl = max(60, env_int("MOOR_MOOR_MIN_KEY_TTL_SECONDS", 1800))
            moor_state = {
                "agent_key": getattr(entry, "agent_key", None),
                "agent_key_expires_at": getattr(entry, "agent_key_expires_at", None),
                "scope": getattr(entry, "scope", None),
            }
            if not _agent_key_is_usable(moor_state, min_ttl):
                ...
                if not pool_api_key or not _agent_key_is_usable(moor_state, min_ttl):
    ...
    if provider == "moor":
        try:
            ...
            if requested_provider != "auto":
                raise
            # Auto-detected Moor but credentials are stale/revoked —
            # fall through to env-var providers (e.g. OpenRouter).
            logger.info("Auto-detected Moor provider but credentials failed; "
    ...
    if provider == "openai-codex":
        try:
            ...
            return {
                ...
                "base_url": creds.get("base_url", "").rstrip("/"),
                "api_key": creds.get("api_key", ""),
                "source": creds.get("source", "moor-auth-store"),
                "last_refresh": creds.get("last_refresh"),
                "requested_provider": requested_provider,
            }
        except AuthError:
    ...
    if provider == "minimax-oauth":
        ...
        if pconfig and pconfig.auth_type == "oauth_minimax":
            from moor_cli.auth import resolve_minimax_oauth_runtime_credentials
    ...
    if provider == "anthropic":
        ...
        if _is_azure_endpoint:
            ...
            # chain.  Accept both `key_env` (Moor canonical — matches the
            # custom_providers field name) and `api_key_env` (documented in the
            # Azure Foundry guide and read by most Moor-compatible importers).
