"""Client for uploading ``moor debug share`` bundles to Moor-internal S3.

This is the opt-in (``--moor``) destination for ``moor debug share``.
Unlike the public paste.rs path, bundles uploaded here go to a Moor-owned
S3 bucket via a short-lived signed URL minted by the Moor account service
(NAS).  The bucket auto-expires objects after 14 days, and the contents are
only viewable by Moor staff (and allowlisted Discord mods) through a
...
# Base URL of the Moor account service that mints the signed upload URL.
...
NAS_BASE = os.environ.get(
    "MOOR_DIAGNOSTICS_BASE_URL", "https://portal.nousresearch.com"
)
...
def share_to_moor(report_bundle: bytes) -> dict:
    """Orchestrate the full Moor-S3 upload of a gzipped *report_bundle*.
