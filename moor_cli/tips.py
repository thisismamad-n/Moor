...
TIPS = [
    ...
    "/title \"my project\" names your session — resume it later with /resume or moor -c.",
    ...
    "Ctrl+Z suspends Moor to the background — run fg in your shell to resume.",
    ...
    # --- CLI Flags ---
    "moor -c resumes your most recent CLI session. moor -c \"project name\" resumes by title.",
    "moor -w creates an isolated git worktree — perfect for parallel agent workflows.",
    "moor -w -q \"Fix issue #42\" combines worktree isolation with a one-shot query.",
    "moor chat -t web,terminal enables only specific toolsets for a focused session.",
    "moor chat -s github-pr-workflow preloads a skill at launch.",
    "moor chat -q \"query\" runs a single non-interactive query and exits.",
    "moor chat --max-turns 1000 overrides the default 500-iteration limit per turn.",
    "moor chat --checkpoints enables filesystem snapshots before every destructive file change.",
    "moor --yolo bypasses all dangerous command approval prompts for the entire session.",
    "moor chat --source telegram tags the session for filtering in moor sessions list.",
    "moor -p work chat runs under a specific profile without changing your default.",

    # --- CLI Subcommands ---
    "moor doctor --fix diagnoses and auto-repairs config and dependency issues.",
    "moor dump outputs a compact setup summary — great for bug reports.",
    "moor config set KEY VALUE auto-routes secrets to .env and everything else to config.yaml.",
    "moor config edit opens config.yaml in your default editor.",
    "moor config check scans for missing or stale configuration options.",
    "moor sessions browse opens an interactive session picker with search.",
    "moor sessions stats shows session counts by platform and database size.",
    "moor sessions prune --older-than 30 cleans up old sessions.",
    "moor skills search react --source skills-sh searches the skills.sh public directory.",
    "moor skills check scans installed hub skills for upstream updates.",
    "moor skills tap add myorg/skills-repo adds a custom GitHub skill source.",
    "moor skills snapshot export setup.json exports your skill configuration for backup or sharing.",
    "moor mcp add github --command npx adds MCP servers from the command line.",
    "moor mcp serve runs Moor itself as an MCP server for other agents.",
    "moor auth add lets you add multiple API keys for credential pool rotation.",
    "moor completion bash >> ~/.bashrc enables tab completion for all commands and profiles.",
    "moor logs -f follows agent.log in real time. --level WARNING --since 1h filters output.",
    "moor backup creates a zip backup of your entire Moor home directory.",
    "moor profile create coder creates an isolated profile that becomes its own command.",
    "moor profile create work --clone copies your current config and keys to a new profile.",
    "moor update syncs new bundled skills to ALL profiles automatically.",
    "moor gateway install sets up Moor as a system service (systemd/launchd).",
    "moor memory setup lets you configure an external memory provider (Honcho, Mem0, etc.).",
    "moor webhook subscribe creates event-driven webhook routes with HMAC validation.",
    "Save money: moor tools disables unused tools, moor skills config trims skills down.",
    "/reasoning low or /reasoning minimal cuts thinking depth below the default (medium) — faster, cheaper responses.",
    "moor models routes vision, compression, and aux tasks to cheaper models — cuts background token cost 85%+ without downgrading your main chat model.",
    ...
    "Set worktree: true in config.yaml to always create a git worktree (same as moor -w).",
    ...
    "MOOR_TIMEZONE overrides the server timezone with any IANA timezone string.",
    ...
    "Profile names become shell commands — 'moor profile create coder' creates the 'coder' command.",
    "moor profile export coder -o backup.tar.gz creates a portable profile archive.",
    ...
    "When exiting, Moor prints a resume command with session ID and stats.",
    "moor sessions export backup.jsonl exports all sessions for backup or analysis.",
    "moor -r SESSION_ID resumes any specific past session by its ID.",
    ...
    "moor skills install official/security/1password installs optional skills from the repo.",
    ...
    "The plan skill saves markdown plans under .moor/plans/ in the active workspace.",

    # --- Cron & Scheduling ---
    "Cron jobs can attach skills: moor cron add --skill blogwatcher \"Check for new posts\".",
    ...
    # --- Gateway & Messaging ---
    "Moor runs on 21 messaging platforms: Telegram, Discord, Slack, WhatsApp, Signal, Matrix, IRC, Microsoft Teams, email, and more.",
    "moor gateway install sets it up as a system service that starts on boot.",
    "DingTalk uses Stream Mode — no webhooks or public URL needed.",
    "BlueBubbles brings iMessage to Moor via a local macOS server.",
    ...
    "Context files (.moor.md, AGENTS.md) are security-scanned for prompt injection before loading.",
    ...
    "Moor loads project context from .moor.md, AGENTS.md, CLAUDE.md, or .cursorrules (first match).",
    ...
    # --- MCP ---
    "moor mcp opens an interactive picker of Moor-approved MCPs you can install in one keystroke.",
    "moor mcp catalog lists Moor-approved MCP servers shipped with the repo.",
    "moor mcp install <name> installs a catalog entry, prompts for credentials, and lets you pick which of its tools to enable.",
    ...
    "MCP servers auto-generate toolsets at runtime — moor tools can toggle them per platform.",
    ...
    "Checkpoints use shadow repos in ~/.moor/checkpoints/ — your project's .git is never touched.",

    # --- Batch & Data ---
    "batch_runner.py processes hundreds of prompts in parallel for training data generation.",
    "moor chat -Q enables quiet mode for programmatic use — suppresses banner and spinner.",
    ...
    "moor acp runs Moor as an ACP server for VS Code, Zed, and JetBrains integration.",
    "Custom providers: save named endpoints in config.yaml under custom_providers.",
    "MOOR_EPHEMERAL_SYSTEM_PROMPT injects a system prompt that's never persisted to history.",
    "credential_pool_strategies supports fill_first, round_robin, least_used, and random rotation.",
    "moor auth add moor or moor auth add openai-codex sets up OAuth-based providers.",
    ...
    "moor status --deep runs deeper diagnostic checks across all components.",
    ...
    "Cron scripts live in ~/.moor/scripts/ and run before the agent — perfect for data collection pipelines.",
    "prefill_messages_file in config.yaml injects few-shot examples into every API call, never saved to history.",
    "SOUL.md completely replaces the agent's default identity — rewrite it to make Moor your own.",
    ...
    "Before a gateway session reset, Moor auto-flushes important facts to memory in the background.",
    ...
    "The skills audit log at ~/.moor/skills/.hub/audit.log tracks every install and removal operation.",
    ...
    "Profiles scope Moor state via MOOR_HOME; host tool subprocesses keep your real HOME unless terminal.home_mode is profile.",
    "MOOR_HOME_MODE env var (octal, e.g. 0701) sets custom directory permissions for web server traversal.",
    "Container mode: place .container-mode in MOOR_HOME and the host CLI auto-execs into the container.",
    "Ctrl+C has 5 priority tiers: cancel recording → cancel prompts → cancel picker → interrupt agent → exit.",
    "Every interrupt during an agent run is logged to ~/.moor/interrupt_debug.log with timestamps.",
    ...
    "Profile names cannot collide with existing PATH binaries — 'moor profile create ls' would be rejected.",
    "moor profile create backup --clone-all copies everything (config, keys, SOUL.md, memories, skills, sessions).",
    ...
    "moor claw migrate --dry-run previews OpenClaw migration without writing anything.",
    ...
    "MOOR_DEV=1 bypasses container mode detection for local development.",
    "Each MCP server gets its own toolset (mcp-servername) that can be toggled independently via moor tools.",
    "MCP ${ENV_VAR} placeholders in config are resolved at server spawn — including vars from ~/.moor/.env.",
    "Skills from trusted repos (Moor inc.) get a 'trusted' security level; community skills get extra scanning.",
    "The skills quarantine at ~/.moor/skills/.hub/quarantine/ holds skills pending security review.",

    # --- Advanced Slash Commands ---
    '/steer <prompt> injects a note after the next tool call — nudge direction mid-task without interrupting.',
    '/goal <text> sets a standing Ralph-loop objective — Moor auto-continues turn after turn until a judge says done.',
    '/snapshot create [label] saves a full state snapshot of Moor config; /snapshot restore <id> reverts later.',
    ...
    '/busy queue|steer|interrupt controls what pressing Enter does while Moor is working.',
    ...
    '/reload reloads ~/.moor/.env into the running session — pick up new API keys without restarting.',
    ...
    "MOOR_CRON_MAX_PARALLEL (default 4) caps how many cron jobs run per tick so bursts don't saturate your keys.",

    # --- Gateway Hooks ---
    'Gateway hooks live under ~/.moor/hooks/<name>/ with HOOK.yaml + handler.py — handler must be named `handle`.',
    'Hook events include gateway:startup, session:start, agent:step, and command:* wildcard subscriptions.',
    'Drop a ~/.moor/BOOT.md checklist and a gateway:startup hook runs it as a one-shot agent every boot.',

    # --- Curator ---
    'moor curator run --dry-run previews what the curator would archive or consolidate without mutating anything.',
    ...
    'moor auth reset <provider> clears all cooldowns and exhaustion flags on a credential pool.',
    ...
    # --- TUI & Dashboard ---
    'MOOR_TUI_RESUME=1 auto-re-attaches to the most recent TUI session on launch — handy after SSH drops.',
    "MOOR_TUI_THEME=light|dark|<hex> forces the TUI theme on terminals that don't set COLORFGBG.",
    ...
    'moor dashboard launches a local web UI at 127.0.0.1:9119 — zero data leaves localhost.',
    'moor dashboard embeds the full Moor TUI in your browser via xterm.js and a WebSocket PTY.',
    'Drop a YAML in ~/.moor/dashboard-themes/ with two palette colors to reskin the entire dashboard.',
    'Dashboard plugins are drop-in: manifest.json + JS bundle in ~/.moor/dashboard-plugins/ — no npm build required.',
    ...
    'MOOR_BACKGROUND_NOTIFICATIONS=result only pings when background tasks finish (vs concise/all/error/off).',
    'MOOR_WRITE_SAFE_ROOT restricts write_file/patch to directory prefixes; multiple paths via os.pathsep (: or ;).',
    'MOOR_IGNORE_RULES skips auto-injection of AGENTS.md, SOUL.md, .cursorrules, memory, and preloaded skills.',
    'MOOR_ACCEPT_HOOKS auto-approves unseen shell hooks declared in config.yaml without a TTY prompt.',
    ...
    # --- TTS ---
    'tts.provider: piper runs 44-language local TTS on CPU — voices auto-download to ~/.moor/cache/piper-voices/.',
    ...
    '/reload-skills re-scans ~/.moor/skills/ so drop-in skills appear without restarting the session.',
    ...
    'moor -z "<prompt>" is the purest one-shot: final answer on stdout, nothing else — ideal for piping in scripts.',
    'moor chat --pass-session-id injects the session ID into the system prompt so the agent can self-reference it.',
    'moor chat --image path/to/pic.png attaches a local image to a single -q query without a separate upload step.',
    'moor chat --ignore-user-config skips the active user config — reproducible bug reports and CI runs.',
    "moor chat --source tool tags programmatic chats so they don't clutter moor sessions list.",
    'moor dump --show-keys includes redacted API key fingerprints for deeper support debugging.',
    'moor sessions rename <ID> "new title" renames any past session; moor sessions delete <ID> removes one.',
    'moor import restores a full Moor backup zip; session JSON/JSONL exports import from the dashboard Sessions page.',
    'moor fallback manages the fallback_model chain interactively — no hand-editing config.yaml.',
    'moor pairing rotates the DM pairing token — the first messager after rotation claims access to the bot.',
    'moor setup walks first-time users through provider, keys, and platform wiring in one interactive flow.',
    'moor status --deep runs the full health sweep across every component; plain moor status is the quick view.',

    # --- Agent Behavior Env Vars ---
    'MOOR_AGENT_TIMEOUT=0 disables the gateway inactivity kill for a running agent — use for long research runs.',
    'MOOR_ENABLE_PROJECT_PLUGINS=1 auto-loads repo-local plugins from ./.moor/plugins/ — trust-gated by design.',
    "MOOR_DISABLE_FILE_STATE_GUARD=1 turns off the 'file changed since you read it' guard on patch and write_file.",
    'MOOR_ALLOW_PRIVATE_URLS=true lets web tools hit localhost and private networks — off by default in gateway mode.',
    'MOOR_OPTIONAL_SKILLS=name1,name2 auto-installs extra optional-catalog skills on first run per profile.',
    'MOOR_BUNDLED_SKILLS points at a custom bundled-skill tree — used by Homebrew and Nix packaging.',
    'MOOR_DUMP_REQUEST_STDOUT=1 dumps every API request payload to stdout instead of log files.',
    'MOOR_OAUTH_TRACE=1 logs redacted OAuth token exchange and refresh attempts for debugging provider auth.',
    'MOOR_STREAM_RETRIES (default 3) controls mid-stream reconnect attempts on transient network errors.',

    # --- Gateway Behavior Env Vars ---
    'MOOR_GATEWAY_BUSY_ACK_ENABLED=false silences the ⚡/⏳/⏩ ack messages when a user messages a busy agent.',
    'MOOR_AGENT_NOTIFY_INTERVAL (default 180s) sets how often the gateway pings with progress on long turns.',
    'MOOR_RESTART_DRAIN_TIMEOUT (default 900s) caps how long /restart waits for in-flight runs before forcing.',
    'MOOR_CHECKPOINT_TIMEOUT (default 30s) caps filesystem checkpoint creation — raise it on huge monorepos.',
    ...
    # --- Security ---
    'security.tirith_fail_open: false makes Moor block commands when the tirith scanner itself errors out.',
    ...
    '--source tool chats are excluded from moor sessions list by default — set --source explicitly to see them.',
    ...
    'Dashboard plugins are served from /dashboard-plugins/<name>/ — drop files into ~/.moor/dashboard-plugins/.',
]
...
# any project or none — Moor is not a coding-only agent.
# ---------------------------------------------------------------------------
