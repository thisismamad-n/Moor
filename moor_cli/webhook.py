"""moor webhook — manage dynamic webhook subscriptions from the CLI.
...
Subscriptions persist to ~/.moor/webhook_subscriptions.json and are
...
def _subscriptions_path() -> Path:
    return _moor_home() / _SUBSCRIPTIONS_FILENAME
...
def _setup_hint() -> str:
  ...
  Then start the gateway: moor gateway run
"""
...
def webhook_command(args):
    """Entry point for 'moor webhook' subcommand."""
    sub = getattr(args, "webhook_action", None)

    if not sub:
        print("Usage: moor webhook {subscribe|list|remove|test}")
        print("Run 'moor webhook --help' for details.")
        return
...
def _cmd_subscribe(args):
    ...
    print("  The gateway must be running to receive events (moor gateway run).\n")
...
def _cmd_list(args):
    ...
    if not subs:
        ...
        print("  Create one with: moor webhook subscribe <name>")
        return
...
def _cmd_test(args):
    ...
    try:
        ...
        print(f"  Error: {e}")
        print("  Is the gateway running? (moor gateway run)")
