...
def register_cli(parent_parser: argparse.ArgumentParser) -> None:
    ...
    # dest='egress_command' keeps this tree disjoint from the inbound OAuth ``moor proxy``
...
def _setup_mint_tokens(console: Console, args: argparse.Namespace):
    ...
    if args.from_bitwarden:
        ...
        # Operators commonly keep provider keys only in ~/.moor/.env (loaded when the agent
        # runs, NOT exported into an interactive shell); backfill so discovery sees them.
        loaded = _load_env_file_into_environ()
        if loaded:
            console.print(f"  [dim]Loaded {loaded} provider key name(s) from ~/.moor/.env for discovery.[/dim]")
    ...
    if rotate and existing:
        ...
        if sys.stdin.isatty():
            console.print(
                ...
                "tokens in every running Moor sandbox.  They will start 401-ing against upstreams until restarted."
            )
...
def _setup_restart_daemon(console: Console, args: argparse.Namespace, proxy_cfg: dict) -> None:
    ...
    if do_restart:
        try:
            ...
            console.print("  Run [cyan]moor egress start[/cyan] manually before launching new Docker sandboxes.")
        else:
...
def cmd_start(args: argparse.Namespace) -> int:
    ...
    if not proxy_cfg.get("enabled"):
        console.print("[yellow]proxy.enabled is false — run `moor egress setup` first.[/yellow]")
        return 1
    ...
    if refresh_bw:
        ...
        if not os.environ.get(bw_access_env, "").strip():
            return _refuse(
                ...
                "`moor egress setup --no-bitwarden` to switch back to env-based credentials.",
            )
        if not (bw_cfg or {}).get("project_id"):
            return _refuse(
                ...
                "project, or switch back via `moor egress setup --no-bitwarden`.",
            )
    try:
...
def cmd_reload(args: argparse.Namespace) -> int:
    ...
    New upstream SECRETS still need ``moor egress restart``: the daemon reads credentials from
...
def cmd_disable(args: argparse.Namespace) -> int:
    ...
    if ip.get_status().pid is not None:
        console.print(
            "  iron-proxy is still running — stop it with [cyan]moor egress stop[/cyan] if you want it down too."
        )
    return 0
...
def cmd_config(args: argparse.Namespace) -> int:
    ...
    if status.config_path is None:
        console.print("[yellow](no config generated — run `moor egress setup`)[/yellow]")
        return 1
...
def _bitwarden_env_names(console: Console) -> Optional[List[str]]:
    ...
    if not bw_cfg.get("enabled"):
        ...
        console.print("  Run `moor secrets bitwarden setup` first, or omit --from-bitwarden.")
        return None
    try:
...
def _load_env_file_into_environ() -> int:
    """Backfill known provider keys from ``~/.moor/.env`` into ``os.environ``; returns the count.
