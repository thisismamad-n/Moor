...
def _missing_env_specs(manifest: dict) -> list[dict]:
    ...
    normalised to dicts; nameless entries dropped) whose variable is unset in ``~/.moor/.env``."""
    env_specs = [
...
def _print_python_dependencies(manifest: dict, console) -> None:
    """Print declared ``python_dependencies`` with an install hint — Moor never auto-installs
...
def ensure_basic_auth_plugin_enabled_in_config(cfg: dict) -> bool:
    ...
    ``moor setup`` / ``moor plugins disable basic`` can park it there while
...
def cmd_enable(name: str, allow_tool_override: Optional[bool] = None) -> None:
    ...
    """
    from moor_cli.relay_plugin_cutover import LEGACY_RELAY_PLUGIN_KEYS, RELAY_PLUGINS_CONFIG_ENV
    console = _console()
...
def _discover_all_plugins() -> list:
    ...
    from moor_cli.plugins import discover_entrypoint_manifests, get_bundled_plugins_dir
...
def cmd_show(name: str) -> None:
    ...
    if match is None:
        ...
        _fail(console, "[dim]List installed plugins:[/dim] moor plugins list")
...
def cmd_search(
    ...
    *,
    ...
    from moor_cli.plugin_index import SECURITY_FOOTER, load_index, search_index
    console = _console()
