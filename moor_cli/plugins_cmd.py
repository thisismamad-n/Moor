...
Plugins are installed from Git repositories into ``~/.moor/plugins/``.
...
def _missing_requires_env_names(manifest: dict) -> list[str]:
    """Return declared ``requires_env`` names that are unset in ``~/.moor/.env``."""
...
def _prompt_plugin_env_vars(manifest: dict, console) -> None:
    ...
    from moor_cli.config import get_env_value, save_env_value  # noqa: F811
...
def _install_metadata_path() -> Path:
    return get_moor_home() / "plugins" / _INSTALL_METADATA_FILE
...
def _resolve_index_name(identifier: str, console) -> tuple[str, Optional[str]]:
    ...
    """
    from moor_cli.plugin_index import SECURITY_FOOTER, load_index, resolve_name
    ...
    if entry is None:
        if len(candidates) > 1:
            ...
            console.print(
                ...
                f"community index ({source}). Use `moor plugins search <term>` to "
...
def cmd_install(
    ...
    console.print("[dim]  moor gateway restart[/dim]")
    console.print()
...
def cmd_update(name: str) -> None:
    ...
    if install_record.get("pinned") is True:
        ...
        console.print(
            ...
            f"`moor plugins install {recorded_source} --force "
        ...
        if allowed is not True:
            ...
            if scan_result.verdict == "dangerous":
                ...
                console.print(
                    ...
                    f"findings, then re-enable with `moor plugins enable {name}` "
                    f"if you trust them.",
                )
...
def ensure_basic_auth_plugin_enabled_in_config(cfg: dict) -> bool:
    ...
    ``moor setup`` / ``moor plugins disable basic`` can park the plugin
...
def _resolve_plugin_key(name: str) -> Optional[str]:
    ...
    This is the single normalization point so ``moor plugins enable`` /
...
def cmd_enable(name: str, allow_tool_override: Optional[bool] = None) -> None:
    ...
    if name in LEGACY_RELAY_PLUGIN_KEYS:
        console.print(
            ...
            f"by Moor core; configure {RELAY_PLUGINS_CONFIG_ENV} instead."
        )
        sys.exit(1)
...
def _print_capability_list(console, capabilities: list) -> None:
    ...
    from moor_cli.plugin_capabilities import CAPABILITY_REGISTRY
...
def cmd_capabilities(name: Optional[str] = None) -> None:
    ...
    from moor_cli.plugin_capabilities import granted_capabilities
...
def _discover_entrypoint_plugins() -> list[tuple[str, str, str, str]]:
    """Return plugin entries advertised through ``moor_agent.plugins``.

    Entry-point plugins are installed as Python packages, so they do not have a
    plugin directory under ``~/.moor/plugins``. Include package metadata here
    so ``moor plugins list`` can show and enable them.
    """
    from moor_cli.plugins import ENTRY_POINTS_GROUP
...
def cmd_show(name: str) -> None:
    ...
    if match is None:
        ...
        console.print("[dim]List installed plugins:[/dim] moor plugins list")
        sys.exit(1)
...
def cmd_toggle() -> None:
    ...
    # plugins.disabled, but ``moor plugins enable web/firecrawl`` cleared
...
def _user_installed_plugin_dir(name: str) -> Optional[Path]:
    """Resolved path under ``~/.moor/plugins/<name>`` if it exists."""
    plugins_dir = _plugins_dir()
    try:
        target = _sanitize_plugin_name(name, plugins_dir, allow_subdir=True)
    except ValueError:
...
def dashboard_update_user_plugin(name: str) -> dict[str, Any]:
    ...
    if install_record.get("pinned") is True:
        ...
        return {
            ...
            "error": (
                ...
                f"run `moor plugins install {recorded_source} --force "
    ...
    # Sibling of the CLI ``moor plugins update`` path: drop bytecode
...
def _clear_plugin_bytecode(target: Path) -> int:
    ...
    fingerprint sweep in ``moor_cli.main`` never covers them. After a
...
def _git_pull_plugin_dir(target: Path) -> tuple[bool, str]:
    ...
    has local changes"), and the same autostash approach ``moor update``
    ...
    try:
        ...
        if dirty:
            ...
            push = _run_plugin_git(
                ...
                "-m", "moor-plugin-update-autostash",
            )
...
def cmd_search(
    ...
    *,
    ...
    console.print(table)
    console.print(f"[dim]Index source: {source}. Install: moor plugins install <name>[/dim]")
    console.print(f"[dim]{SECURITY_FOOTER}[/dim]")


def plugins_command(args) -> None:
    """Dispatch moor plugins subcommands."""
    ...
    if action == "install":
        ...
        from moor_cli.plugin_packs import pack_command
