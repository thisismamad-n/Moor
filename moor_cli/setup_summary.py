...
resolved through the module object so test patches on ``moor_cli.setup.<name>`` take effect."""
...
_TTS_SUMMARY_ROWS = {
    ...
    "neutts": ("NeuTTS", "neutts", "run 'moor setup tts'"),
    "kittentts": ("KittenTTS", "kittentts", "run 'moor setup tts'")}
...
_STT_SUMMARY_DEFAULT = ("Local Whisper", "faster_whisper", "run 'moor tools' → Speech-to-Text")
...
_BROWSER_MISSING_HINTS = {
    ...
    "Camofox": "CAMOFOX_URL",
    "Local browser": "moor pm install agent-browser"}
...
_EDIT_WIZARD_ROWS = (
    ("moor setup", "          Re-run the full wizard"), ("moor setup model", "    Change model/provider"),
    ("moor setup terminal", " Change terminal backend"), ("moor setup gateway", "  Configure messaging"),
    ("moor setup tools", "    Configure tool providers"))
_EDIT_CONFIG_ROWS = (
    ("moor config", "         View current settings"), ("moor config edit", "    Open config in your editor"),
    ("moor config set <key> <value>", ""))
_READY_ROWS = (
    ("moor", "              Start chatting"), ("moor gateway", "      Start messaging gateway"),
    ("moor doctor", "       Check for issues"))
...
def _vision_row(config, feats):
    ...
    try:
        ...
        ok = False
    return ("Vision (image analysis)", ok, None if ok else "run 'moor setup' to configure")


def _managed_or_provider_row(feature, name: str, managed_label: str, missing_hint: str):
    """Row for a Moor-manageable feature: managed > available (with provider) > missing hint."""
...
def _web_row(config, feats):
    ...
    return _managed_or_provider_row(feats.web, "Web Search & Extract", "Moor subscription", _WEB_MISSING)
...
def _browser_row(config, feats):
    ...
    return _managed_or_provider_row(feats.browser, "Browser Automation", "Moor Browser Use", hint)


def _image_gen_row(config, feats):
    # FAL (direct or via Moor), or any plugin-registered provider (OpenAI, etc.)
    if feats.image_gen.managed_by_moor:
        return ("Image Generation (Moor subscription)", True, None)
...
def _video_gen_row(config, feats):
    # Opt-in via `moor tools` → Video Generation. Only show the row when a plugin reports
    # available so we don't badger users who don't care about video gen with a "missing" line.
    if feats.video_gen.managed_by_moor:
        return ("Video Generation (FAL via Moor subscription)", True, None)
...
def _tts_row(config, feats):
    ...
    if feats.tts.managed_by_moor:
        return ("Text-to-Speech (OpenAI via Moor subscription)", True, None)
...
def _stt_row(config, feats):
    ...
    if stt_feature is not None and stt_feature.managed_by_moor:
        return ("Speech-to-Text (OpenAI via Moor subscription)", True, None)
...
def _modal_row(config, feats):
    if feats.modal.managed_by_moor:
        return ("Modal Execution (Moor subscription)", True, None)
    if _setup.cfg_get(config, "terminal", "backend") == "modal":
        ...
        return ("Modal Execution", False, "run 'moor setup terminal'")
    if tool_backend_helpers.managed_moor_tools_enabled() and feats.moor_auth_present:
        return ("Modal Execution (optional via Moor subscription)", True, None)
    return None
...
def _spotify_row(config, feats):
    # OAuth via moor auth spotify — check auth.json, not env vars
    try:
...
def _print_setup_summary(config: dict, moor_home):
    ...
    try:
        ...
        print()
        _setup.print_warning("No inference provider is configured — Moor cannot chat yet.")
        _setup._info("  Finish this one step with either of:",
              "    moor model            (pick any provider/model)",
              "    moor setup --portal   (Moor Portal OAuth, no API key)")

    print()
    _setup.print_header("Tool Availability Summary")

    tool_status = []
    subscription_features = moor_subscription.get_moor_subscription_features(config)
    ...
    if available_count < len(tool_status):
        _setup.print_warning("Some tools are disabled. Run 'moor setup tools' to configure them,")
    ...
    for label, value in (("Settings:", f"  {_setup.get_config_path()}"), ("API Keys:", f"  {_setup.get_env_path()}"),
                         ("Data:", f"      {moor_home}/cron/, sessions/, logs/")):
...
import moor_cli.setup as _setup  # noqa: E402  (bottom: moor_cli.setup imports this module)
