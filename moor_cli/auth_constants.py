...
Pure leaf: imports nothing from ``moor_cli.auth`` so the per-provider modules
(``auth_moor``, ``auth_codex``, ...) can import it at module scope without cycles."""
...
# httpx is imported lazily (~30ms) because moor_cli.auth is on the interactive-CLI startup path
...
if TYPE_CHECKING:
    ...
    class _LazyHttpx:
        ...
        # set/del forward to the real module so monkeypatch.setattr("moor_cli.auth.httpx.Client")
...
SPOTIFY_DOCS_URL = "https://github.com/thisismamad-n/Moor/tree/master/website/docs/user-guide/features/spotify"
...
OAUTH_OVER_SSH_DOCS_URL = "https://github.com/thisismamad-n/Moor/tree/master/website/docs/guides/oauth-over-ssh"
...
_moor_err = _provider_error_factory("moor")
