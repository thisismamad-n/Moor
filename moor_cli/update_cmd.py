"""Moor update pipeline — extracted from ``moor_cli/main.py``.
...
change is that references to helpers/constants that STAY in ``moor_cli.main``
(and to moved-but-test-patched siblings) are routed through ``_m()`` — a lazy
``moor_cli.main`` reference — so existing call sites and test monkeypatches
that target ``moor_cli.main.<name>`` (``PROJECT_ROOT``, ``_is_windows``,
...
the test-patch surface still resolve on ``moor_cli.main``.
...
Imports are one-way: ``moor_cli.main`` imports this module, never the reverse
...
#: ``_purge_stale_moor_modules`` so any LATER import chain resolves against
...
_STALE_PURGE_PROTECTED = frozenset(
    {
        ...
        "moor_cli.main",
        "moor_cli.update_cmd",
        "moor_cli.moor_logging",
    }
)


def _purge_stale_moor_modules() -> None:
    """Evict every cached Moor module after the checkout changed in-place.

    ``moor update`` keeps running in the pre-pull Python process. The
    gateway auto-restart phase that follows does function-level
    ``from moor_cli.gateway import ...`` — executing NEW source inside an
    ...
    field failure: freshly-pulled ``moor_cli.gateway`` does
    ``from moor_cli.cli_output import line_input``, but ``cli_output`` was
    ...
    export. This is the class fix: drop EVERY cached module under the Moor
    ...
    try:
        ...
        if purged:
            logger.debug(
                "Purged %d stale Moor module(s) after checkout update", len(purged)
            )
    except Exception as exc:
        logger.debug("Could not purge stale Moor modules: %s", exc)


def _reload_updated_runtime_modules() -> None:
    """Reload update-sensitive modules after the checkout changes in-place.

    ``moor update`` keeps running in the pre-pull Python process. After a
...
def _reload_config_modules() -> None:
    ...
    ``moor update`` runs in the PRE-pull Python process. After ``git pull``
    ...
    This function force-reloads ``moor_cli.config_defaults``,
    ``moor_cli.config``, and ``moor_cli.config_migrations`` from disk
    so subsequent imports read the UPDATED code.

    It also reloads ``moor_cli._subprocess_compat`` and
    ``moor_cli.dashboard_procs`` so that post-update dashboard cleanup
    ...
    importlib.invalidate_caches()
    for mod_name in (
        "moor_cli.config_defaults",
        "moor_cli.config",
        "moor_cli.config_migrations",
        "moor_cli._subprocess_compat",
...
def _migrate_sibling_profile_configs() -> list[tuple[str, int, int]]:
    ...
    shared checkout serves every profile, but ``moor update`` historically
    ...
    caller): scope config reads/writes via the context-local MOOR_HOME
    ...
    try:
        ...
        from moor_cli.profiles import _get_profiles_root, _PROFILE_ID_RE

        active_home = get_process_moor_home()
...
def _editable_install_is_current(git_cmd, cwd, pre_pull_sha: str | None) -> bool:
    ...
    On Windows that rewrite is the only reason the running ``moor.exe`` has
    ...
    Skipping is safe because Moor pins its editable finder to a *static*
...
def _validate_critical_files_syntax(root) -> tuple[bool, str | None, str | None]:
    ...
    root = Path(root)
    with tempfile.TemporaryDirectory(prefix="moor-syntax-check-") as tmpdir:
...
def _validate_critical_modules_import(root) -> tuple[bool, str | None, str | None]:
    ...
    ``_venv_core_imports_healthy``): ``moor update`` can be driven by a
    ...
    probe = (
        ...
        # The root set is injected from moor_constants so this can't drift
        # from the hint the user is shown (they disagreed once already).
        "        missing = (getattr(exc, 'name', '') or '').split('.')[0]\n"
        "        if missing in %r or missing.startswith('moor_'):\n"
...
def _print_curator_first_run_notice() -> None:
    """Print a short heads-up about the skill curator after `moor update`.
...
def _print_fts_optimize_available_notice() -> None:
    """Advertise the opt-in v23 search-index optimization after `moor update`.
    ...
    if interrupted:
        ...
        print(
            "  A previous `moor sessions optimize-storage` run was "
            "interrupted. Search still works; re-run the command to resume "
            "and finish reclaiming disk:"
        )
        print("    moor sessions optimize-storage")
        return
    ...
    print("  Run when convenient:  moor sessions optimize-storage")
    print(
...
def _print_curator_recent_run_notice() -> None:
    ...
    rename. ``moor update`` is a high-attention surface — surface the
    most recent run's rename map here, once.

    Show-once: state stamps ``last_run_summary_shown_at`` after printing.
    Subsequent ``moor update`` invocations skip the block until a newer
    ...
    print(
        ...
        "View anytime: moor curator status)"
    )
...
def _reload_process_scan_modules() -> None:
    ...
    ``from moor_cli._subprocess_compat import bounded_probe_run``. If the
...
def _finish_dashboard_update_cleanup(
    ...
    *already_restarted_units* forwards the systemd unit names (no
    ...
    print("    moor dashboard --port <port>")
...
def _atomic_replace_dir(src: str, dst: str) -> None:
    ...
    ``moor_cli.main`` re-export surface and guards the #49145 regression.
    """
...
def _stage_replacement(src: str, dst: str) -> str:
    ...
    untouched.
    """
    staging = f"{dst}.moor-update-staging"
    backup = f"{dst}.moor-update-old"
...
def _discard_staged(staged) -> None:
    ...
    the tree. The user then follows the "re-run `moor update`" advice with
...
def _commit_staged_replacements(staged) -> None:
    ...
    first-party modules (``run_agent.py``, ``cli.py``, ``moor_constants.py``
...
def _assess_parked_branch_switch(
    ...
    stale feature branch left behind by earlier tooling; ``moor update``
...
def _print_parked_branch_skip_warning(
    ...
    print(f"    git -C {cwd} checkout {target_branch} && moor update")
    print(
...
def _print_update_completion(message: str) -> None:
    ...
    action_id = os.environ.get("MOOR_ACTION_ID", "")
    if len(action_id) == 32 and all(char in "0123456789abcdef" for char in action_id):
        print(f"=== moor-update completed {action_id} ===")
...
def _refuse_update_for_contended_shims(exc: BaseException) -> None:
    ...
    """
    print("✗ Cannot continue the update: live Moor launcher(s) could not be")
    print("  moved aside:")
    for name in getattr(exc, "failed_shims", []) or ["moor.exe"]:
        print(f"    {name}")
    print("  Another process is holding this install's venv — typically Moor")
    print("  Desktop, a gateway, or another moor REPL — and mutating the venv")
    print("  now would strand it half-updated.")
    print("  The dependency install has been deferred: close the process(es)")
    print("  above, then run any `moor` command to finish it automatically.")
...
def _zip_overlay_block_reason(
    ...
    two-phase replace creates ``*.moor-update-staging`` siblings inside the
...
_ZIP_STAGING_ARTIFACT_SUFFIXES = (".moor-update-staging", ".moor-update-old")
...
def _abort_zip_update_if_dirty_tree() -> None:
    ...
    print("  Stash or commit your changes, then rerun `moor update`.")
...
def _update_via_zip(args, *, had_desktop_app_before_update: bool = False) -> bool:
    ...
    try:
        ...
        with zipfile.ZipFile(zip_path, "r") as zf:
            ...
            # symlink members. A GitHub source ZIP for moor-agent itself
        ...
        # GitHub ZIPs extract to moor-agent-<branch>/
        extracted = os.path.join(tmp_dir, f"moor-agent-{branch}")
        ...
        try:
            for item in entries:
                ...
                # Moor.exe) exists only in the LIVE tree. Swapping `apps`
        ...
        print(
            "  Re-run `moor update` to retry; if the agent won't start, "
    ...
    from moor_cli.managed_uv import ensure_uv, update_managed_uv
    ...
    if uv_bin:
        ...
        from moor_cli.managed_uv import managed_python_env
    ...
    if not import_ok:
        ...
        print("  Re-run `moor update` to complete it.")
        _m().sys.exit(1)
    ...
    try:
        ...
        if result.get("user_modified"):
            ...
            print(
                "    → see them: moor skills list-modified  "
    ...
    try:
        from moor_cli.model_catalog import seed_cache_from_checkout
    ...
    try:
        from moor_cli.backup import _quick_snapshot_root, verify_sqlite_integrity

        _state_path = get_moor_home() / "state.db"
        if _state_path.exists():
            ...
            if not _state_ok.get("valid"):
                ...
                _snap_root = _quick_snapshot_root(get_moor_home())
...
def _stash_local_changes_if_needed(git_cmd: list[str], cwd: Path) -> Optional[str]:
    ...
    stash_name = datetime.now(timezone.utc).strftime(
        "moor-update-autostash-%Y%m%d-%H%M%S"
    )
...
def _park_stashed_changes(stash_ref: str) -> None:
    ...
    Used by ``moor update --keep-stash`` (the desktop updater's mode): the
...
def _restore_stashed_changes(
    ...
    if restore.returncode != 0 and not has_conflicts and (
        ...
        # files makes moor completely unrunnable (SyntaxError on import).
...
def _should_skip_upstream_prompt() -> bool:
    ...
    return (get_moor_home() / SKIP_UPSTREAM_PROMPT_FILE).exists()
...
def _mark_skip_upstream_prompt():
    ...
    try:
        ...
        (get_moor_home() / SKIP_UPSTREAM_PROMPT_FILE).touch()
    except Exception:
        pass
...
def _invalidate_update_cache():
    ...
    ``moor update``, every profile is now current.
    """
    homes = []
    # Default profile home (Docker-aware — uses /opt/data in Docker)
    from moor_constants import get_default_moor_root

    default_home = get_default_moor_root()
...
def _format_concurrent_instances_message(
    ...
    shim = scripts_dir / "moor.exe"
    lines = ["✗ Another moor.exe is running:"]
    ...
    lines.append("")
    lines.append("  Close Moor Desktop, exit any open `moor` REPLs, and")
    lines.append("  stop the gateway (`moor gateway stop`) before retrying.")
    lines.append("")
    if matches:
        ...
        lines.append("")
    lines.append("  Override with `moor update --force` if you've already")
...
def _capture_active_tool_dependencies() -> list[str]:
    """Snapshot Python dependencies installed explicitly through ``moor tools``."""
    try:
...
def _restore_active_tool_dependencies(
    ...
    *,
    ...
    """Restore allowlisted ``moor tools`` dependencies into a rebuilt venv.
...
def _refresh_active_lazy_features(
    ...
    *,
    ...
    first use. ``moor update`` runs ``uv pip install -e .[all]`` which
    ...
    if install_cmd_prefix is None:
        print("  ⚠ Lazy refresh failed; rerun `moor update` once resolved.")
        return False
    ...
    if status == "healthy":
        ...
        print("  Rerun `moor update` once the upstream issue is resolved.")
        return True
...
def _refresh_active_memory_provider_dependencies() -> None:
    ...
    try:
        from moor_cli.memory_setup import _install_dependencies
    except Exception as exc:
...
def _install_psutil_android_compat(
    ...
    *,
    ...
    from moor_cli.psutil_android import PSUTIL_URL, prepare_patched_psutil_sdist
...
def _ensure_uv_for_termux(pip_cmd: list[str]) -> str | None:
    ...
    standalone uv into ``$MOOR_HOME/bin/uv``, but on Termux the official
...
def _npm_manifest_paths() -> tuple[Path, ...]:
    ...
    lockfile is then unchanged but `moor update` is exactly the step
...
def _npm_lockfile_changed(moor_root: Path) -> bool:
    ...
    # landed must NOT skip the reinstall — otherwise every later `moor
    ...
    try:
        ...
        cache_file = moor_root / f".npm_lock_hash_{cache_key}"
...
def _record_npm_lockfile_hash(moor_root: Path) -> None:
...
def _repair_node_deps_on_current_checkout(print_completion) -> None:
    ...
    moor update" — but the early return never reached the Node refresh,
    ...
    if node_failures:
        ...
        print("    Fix npm and re-run `moor update`.")
        print_completion(
...
def _update_node_dependencies() -> list[str]:
    ...
    if not npm:
        ...
        if is_wsl() and path_npm and _m()._is_windows_npm_path(path_npm):
            ...
            print("    package manager), then re-run `moor update`.")
            failed = []
            if any(
    ...
    shared_moor_root = get_default_moor_root()
    ...
    # `moor update` case. Synchronous and can block ~11s on a true cold
    ...
    if not _m()._npm_lockfile_changed(shared_moor_root):
    ...
    def _partial_update_failure(*labels: str) -> list[str]:
        ...
        print("    deps). Fix npm and re-run `moor update`.")
        return list(labels)
    ...
    nixos_env = with_moor_node_path(_m()._nixos_build_env())
    ...
    # `moor update` comes from the *desktop* build, not this step; that
    ...
    if result.returncode == 0:
        _record_npm_lockfile_hash(shared_moor_root)
...
def _cmd_update_check(branch: str = "main", *, branch_explicit: bool = False):
    ...
    from moor_cli.gitlock import clear_stale_git_locks
    ...
    if is_shallow:
        ...
        if head_sha and target_sha and head_sha == target_sha:
            ...
            from moor_cli.banner import _github_compare_behind
...
def _ensure_fhs_path_guard() -> None:
    ...
    repaired on ``moor update`` without requiring a reinstall.  The
    ...
    ``moor`` prints ``command not found`` even though the symlink lives
    at /usr/local/bin/moor.

    Silent no-op on: non-Linux, non-root, non-FHS installs, and any system
    where ``bash -i -c 'command -v moor'`` already resolves.  Idempotent.
    """
    ...
    # /usr/local/bin/moor, code at /usr/local/lib/moor-agent).
    fhs_link = Path("/usr/local/bin/moor")
...
def _ensure_acp_launcher() -> None:
    r"""Self-heal: install a ``moor-acp`` launcher next to the ``moor`` one.

    Mirrors the launcher block in ``scripts/install.sh`` so existing installs
    gain the ACP command on ``moor update`` without a reinstall.  ACP hosts
    (Zed, JetBrains, Buzz Desktop) spawn the agent by resolving the
    ``moor-acp`` command name against the login-shell PATH; the console
    ...
    The shim simply delegates to the sibling ``moor`` launcher with the
    ...
    No-op on Windows (install.ps1 stages the ``moor`` / ``moor-acp``
    launchers into the managed binary dir ``$MoorHome\bin`` and puts THAT
    ...
    ``moor_cli._install_repair.ensure_windows_bin_launchers`` re-stages
    them) and wherever a ``moor-acp`` is already present next to the
    ``moor`` command.  Unwritable directories (e.g. ``/usr/local/bin`` as
    ...
    for bin_dir in (Path.home() / ".local" / "bin", Path("/usr/local/bin")):
        moor_cmd = bin_dir / "moor"
        acp_cmd = bin_dir / "moor-acp"
        try:
            if not (moor_cmd.is_file() or moor_cmd.is_symlink()):
                continue
            ...
            continue
        print(f"  ✓ Installed moor-acp launcher → {acp_cmd}")
...
def _run_pre_update_backup(args) -> Optional[str]:
    ...
    - ``full``  — the quick snapshot PLUS a full zip of MOOR_HOME under
      ``backups/`` (restorable via ``moor import``; the #48200 wrong-path
    ...
    try:
        ...
        # NOTE: this function later does `from moor_constants import
        # get_moor_home`, which makes the name function-local — the
        # module-level import is shadowed and unbound here. Alias explicitly.
        from moor_cli.config import get_moor_home as _get_home
        ...
        try:
            from moor_cli.backup import create_pre_update_snapshots_all_profiles
    ...
    try:
        from moor_cli.backup import create_pre_update_backup
    except Exception as exc:
        print(
    ...
    # Render path using display_moor_home so the user sees ~/.moor/...
    try:
        from moor_constants import get_moor_home, display_moor_home

        home = get_moor_home()
        try:
            display_path = f"{display_moor_home()}/{out_path.relative_to(home)}"
        except ValueError:
    ...
    print(f"  Restore:  moor import {out_path}")
...
def _venv_core_imports_healthy() -> tuple[bool, str]:
    ...
    ``moor update`` may be driven by a different Python). Catches the
    ...
    Without this probe, ``moor update`` on a current checkout prints
    ...
    if not venv_python.exists():
        ...
        # dev may run moor from any interpreter), so report healthy to
        # avoid forcing reinstalls. But on a MANAGED install (the Windows
        # installer / desktop bootstrap stamps `.moor-bootstrap-complete`,
        ...
        managed_markers = (
            _m().PROJECT_ROOT / ".moor-bootstrap-complete",
...
def _detect_venv_python_processes(
    *, exclude_pids: set[int] | None = None
    ...
    The moor.exe shim guard misses the biggest lock-holder class on
    Windows: the Desktop app's backend (``python.exe -m moor_cli.main
    ...
    calling process and its ancestors are always excluded (a CLI ``moor
    ...
    for proc in proc_iter:
        ...
        # the `-m moor_cli.main gateway run` argv, so autostarted gateways
...
# ``cryptography`` is the canonical case: ``moor_cli.main`` used to import
...
def _abort_dependency_sync_if_self_locked(gateway_resume=None) -> None:
    ...
    - The ``moor.exe`` console shim we were launched from (#88838, #89599).
      The marker cannot help here — every future ``moor`` launch is also the
...
def _defer_update_for_self_lock(loaded: list[str]) -> None:
    ...
    print("  sync has been deferred: the next `moor` launch will complete it")
...
def _moor_holder_subcommand(cmdline: str) -> str | None:
    """The actual Moor SUBCOMMAND a venv-holder argv runs, or None.

    Token-based, never substring (#90778: ``kanban --preserve-cache``
    contained \"serve\" and got labeled as the Desktop backend). Finds the
    ``moor_cli.main`` / ``moor(.exe)`` entry token, then returns the
    ...
    for i, token in enumerate(tokens):
        ...
        if low.endswith("moor_cli.main") and i > 0 and tokens[i - 1] == "-m":
            entry_idx = i
            break
        base = low.rsplit("\\", 1)[-1].rsplit("/", 1)[-1]
        if base in ("moor", "moor.exe"):
            entry_idx = i
            break
...
def _format_venv_python_holders_message(matches: list[tuple[int, str, str]]) -> str:
    ...
    (#90778): a standalone ``moor dashboard`` must not be labeled as the
    ...
    for pid, name, cmdline in matches[:6]:
        sub = _moor_holder_subcommand(cmdline)
    ...
    lines.append("    moor update")
    lines.append("  (or use `moor update --force-venv` to proceed anyway at your own risk)")
    return "\n".join(lines)
...
def _venv_launcher_ancestors(pids: list[int]) -> list[int]:
    ...
    # Never return ourselves or our own ancestry: a CLI ``moor update``
...
def _orphaned_desktop_backend_pids(
    ...
    shim before spawning moor-setup, and the update-in-progress marker
    ...
    - its cmdline is a Moor backend (``moor_cli.main`` + ``serve`` /
      ``dashboard``), and
    ...
    managed-runtime descendants (the ``.moor-runtime`` interpreter child)
    ...
    def _is_backend(argv_low: str) -> bool:
        return "moor_cli.main" in argv_low and (
...
def _ledger_reapable_backend_pids(
    ...
    (``moor_cli.process_identity``). A holder qualifies when ALL of:
...
def _handoff_reapable_backend_pids(
    ...
    """PIDs of Moor ``serve``/``dashboard`` backends safe to reap during a
    ...
    run AND no live Desktop shim (``moor.exe``) is open, NOTHING legitimate
    ...
    - Only Moor backends (``moor_cli.main`` + ``serve``/``dashboard``)
    ...
    - Only runs when the CALLER has confirmed the hand-off context
      ...
      ``moor.exe`` shim) — outside that gate this function is never called
...
def _stop_process_trees(pids: list[int]) -> None:
    ...
    ``.moor-runtime`` interpreter child alive and holding the install open
...
def _looks_like_desktop_control_plane(cmdline: str) -> bool:
    """True for this-install ``moor serve`` / ``moor dashboard`` argv.
    ...
    """
    if "moor_cli.main" not in (cmdline or "").lower():
        return False
    return _moor_holder_subcommand(cmdline) in ("serve", "dashboard")
...
def _desktop_owns_gateway_lifecycle() -> bool:
    ...
    """
    try:
        from moor_cli.process_identity import ledger_entries, spawner_is_dead
...
def _pause_windows_gateways_for_update() -> dict | None:
    ...
    moor.exe concurrent-instance guard does not see them. They still import
    ...
    # — e.g. a Windows Scheduled Task running ``pythonw.exe -m moor_cli.main
    ...
    if unmapped_pids:
        ...
        if respawnable < len(unmapped_pids):
            ...
            print("    Restart manually after update: moor gateway run")
...
def _cold_start_windows_gateway_after_update() -> None:
    ...
    ``moor gateway start`` uses (``gateway_windows._spawn_detached``).
...
def _for_each_systemd_gateway_unit(
    ...
    *,
    ...
    """Process each ``moor-gateway*.service``/``moor-serve*.service`` unit
    ...
    for line in (list_units_stdout or "").strip().splitlines():
        ...
        # ``unit.startswith("moor-serve")`` alone would also accept the
        # unrelated ``moor-server.service`` — require the exact base unit
        # or the hyphenated profile family instead (review on #83595).
        if not (
            unit == "moor-gateway.service"
            or unit.startswith("moor-gateway-")
            or unit == "moor-serve.service"
            or unit.startswith("moor-serve-")
        ):
            continue
...
def _service_unit_supports_graceful_sigusr1_restart(svc_name: str) -> bool:
    ...
    Only ``moor-gateway*`` units run ``gateway/run.py``, which installs the
    SIGUSR1 handler. ``moor-serve*`` units (#83438) don't, so sending them
    ...
    (``moor-gateway-helper`` is fine — profile units are
    ``moor-gateway-<profile>`` — but ``moor-gatewayd``-style names are
    not) can't be sent a SIGUSR1 it doesn't handle.
    """
    return svc_name == "moor-gateway" or svc_name.startswith("moor-gateway-")
...
def _warn_incomplete_gateway_fleet_restart(failed_units: list) -> None:
    ...
    print("    moor gateway status")
    if any(not name.startswith("ai.moor.") for name in ordered):
        print("    systemctl --user restart <unit>   # user-scope")
        print("    sudo systemctl restart <unit>     # system-scope")
    if any(name.startswith("ai.moor.") for name in ordered):
...
def _restart_macos_launchd_gateways(
    ...
    ``ai.moor.gateway*`` LaunchAgent must reload it — restarting only the
...
def _surviving_gateway_pids_after_failed_restart():
    ...
    when ``moor_cli.gateway`` itself no longer imports, which is one of the
...
def _warn_gateway_restart_phase_aborted(exc: BaseException, pids) -> None:
    ...
    (e.g. importing ``moor_cli.gateway`` from the freshly pulled checkout)
...
def _refresh_windows_gateway_launchers() -> None:
    ...
    ``moor update`` never touched them, so installs created before the
...
def _refresh_bootstrap_cache_scripts(branch: str = "main") -> None:
    ...
    The Desktop GUI updater (``moor-setup.exe``) executes
    ``$MOOR_HOME/bootstrap-cache/install-<ref>.ps1`` (or ``.sh``) for its
    ...
    try:
        ...
        cache_dir = Path(_m().get_moor_home()) / "bootstrap-cache"
...
def _discard_lockfile_churn(git_cmd, repo_root):
    ...
    ``moor update`` sees a clean tree instead of autostashing every run.
...
def _normalize_managed_eol(git_cmd, repo_root):
    ...
    ``install.ps1`` forever — so ``moor update``, which ships with the checkout
...
def _rebuild_desktop_after_update(
    ...
    # `moor desktop --build-only` subprocess re-imports the whole CLI stack
    ...
    desktop_build_cmd = [sys.executable, "-m", "moor_cli.main", "desktop", "--build-only"]
    ...
    # `moor update` runs inside the desktop updater chain (Desktop →
    # moor-setup → moor update), the shell PATH customizations are lost,
    # so a bare-PATH child would fail with `node: not found` before cmd_gui can
    # self-heal.
    from moor_constants import with_moor_node_path

    build_env = with_moor_node_path()
    ...
    if build_result.returncode != 0:
        print("  ⚠ Desktop build failed (run `moor desktop` to retry)")
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    try:
        from moor_cli.update_receipt import begin_update_receipt
    ...
    # running Moor runtime, its supervisor, and its running code version —
    ...
    # On Windows, abort early if another moor.exe is holding the venv shim
    ...
    if _m()._is_windows() and not getattr(args, "force", False):
        ...
        if scripts_dir is not None:
            concurrent = _m()._detect_concurrent_moor_instances(scripts_dir)
            if concurrent:
    ...
    # (most commonly the Desktop app's `moor serve` backend) will keep .pyd
    ...
    # the moor.exe shim guard above, but its lock probe only checks the shim
    ...
    if _m()._is_windows() and not getattr(args, "force_venv", False):
        ...
        if _venv_holders:
            ...
            # the spawn ledger proves are orphaned Moor backends — the
            ...
            if _ledger_backends:
                print(
                    ...
                    "Moor backend process(es) hold the venv; stopping their trees"
                )
        ...
        if _venv_holders:
            ...
            if _orphan_backends:
                ...
                # its backend (and any .moor-runtime child) holding the
        ...
        if _venv_holders:
            ...
            # the hand-off context those surviving Moor backends are leaks,
            ...
            try:
                ...
                if _scripts_dir is not None:
                    _no_live_shim = not _m()._detect_concurrent_moor_instances(_scripts_dir)
            except Exception:
                _no_live_shim = False
            if _handoff and _no_live_shim:
                ...
                if _handoff_backends:
                    print(
                        f"  ⚠ {len(_handoff_backends)} Moor backend process(es) "
    ...
    # process by design (a CLI `moor update` IS the venv python), and an
    ...
    try:
        ...
        if commit_count == 0:
            ...
            if handed_off_sync:
                print("→ Finishing the dependency install handed off by moor.exe...")
            elif not healthy:
            ...
            if handed_off_sync or not healthy:
                ...
                if healthy_after:
                    ...
                    print("  Close all Moor windows/gateways and re-run: moor update")
            else:
        ...
        # orphan merge-conflict markers in moor_cli/config.py bricked
        # every user who ran ``moor update`` for the 7 minutes between
        ...
        try:
            ...
            # so the CLI stays bootable. The user can then retry ``moor
            ...
            if not syntax_ok:
                ...
                if pre_pull_sha:
                    ...
                    if rollback_result.returncode == 0:
                        ...
                        print("  Try ``moor update`` again later once a fix lands.")
                    else:
        ...
        # Before this guard, ``moor update`` printed "✓ Code updated!" and
        # reinstalled deps + rebuilt the desktop app against the stale tree —
        # no error, no warning, ``moor doctor`` healthy. Compare pre-pull
        ...
        if pre_pull_sha and post_pull_sha == pre_pull_sha:
            ...
            print(
                ...
                f"git -C {_m().PROJECT_ROOT} checkout {branch} && moor update"
            )
            _m()._resume_windows_gateways_after_update(_windows_gateway_resume)
            sys.exit(1)
        ...
        # the old bytecode (e.g. get_moor_home added to moor_constants).
        removed = _m()._clear_bytecode_cache(_m().PROJECT_ROOT)
        if removed:
            print(
                f"  ✓ Cleared {removed} stale __pycache__ director{'y' if removed == 1 else 'ies'}"
        ...
        # marker survives and the next ``moor`` launch finishes the install
        ...
        # symbols in moor_constants or lazy_deps. The dependency install
        ...
        if lazy_ok:
            ...
            print(
                "  ⚠ Lazy-refresh recovery incomplete — run `moor` again "
        ...
        try:
            ...
            if _state_path.exists():
                ...
                if _state_ok.get("valid"):
                    ...
                    if _pre_snap_id:
                        _snap_state = (
                            _quick_snapshot_root(get_moor_home())
                            / _pre_snap_id
                            / "state.db"
                        )
                        if _snap_state.exists():
        ...
        # current — so copy it straight over ~/.moor/cache/model_catalog.json
        ...
        # seed_profile_skills() uses subprocess with an explicit MOOR_HOME so
        # it is not affected by sync_skills()'s module-level MOOR_HOME cache,
        # which means the active profile is reliably synced regardless of whether
        # the caller's MOOR_HOME env var points at the default or a named profile.
        try:
        ...
        try:
            from moor_cli.profiles import backfill_profile_envs
        ...
        # ``moor update`` process is the PRE-pull Python process — its
        # ``sys.modules`` cache holds the OLD ``moor_cli.config`` and
        # ``moor_cli.config_migrations`` from before ``git pull`` updated
        # the source files. A function-level ``from moor_cli.config import
        ...
        if version_bump_only:
            ...
            try:
                ...
                print("     Run 'moor config migrate' to retry.")
            ...
            if assume_yes:
                ...
                try:
                    ...
                    print(
                        ...
                        "Run 'moor config migrate' manually to configure."
                    )
                    response = "n"
            ...
            if response in {"", "y", "yes", "auto"}:
                ...
                if (gateway_mode or assume_yes or response == "auto") and missing_env:
                    print("  ℹ API keys require manual entry: moor config migrate")
            else:
                print()
                print("Skipped. Run 'moor config migrate' later to configure.")
        else:
        ...
        # context-local MOOR_HOME override (never os.environ — other
        ...
        # it and warn loudly.
        try:
            from moor_cli.backup import restore_cron_jobs_if_emptied
        ...
        try:
            from moor_cli.backup import restore_cron_jobs_all_profiles
        ...
        # check `moor curator status`. Self-stamps after printing so it
        ...
        # Self-heal the moor-acp launcher for installs that predate it, so
        ...
        try:
            ...
            logger.debug("moor-acp launcher self-heal failed: %s", e)

        # Migrate the Windows moor launchers to the managed binary dir
        # (the default Moor root's bin, next to the managed uv) and repair
        # them if they are missing. Earlier layouts put them inside the git
        # checkout (moor-agent\bin) or put venv\Scripts itself on PATH; the
        # in-checkout copies were swept by this command's own pre-update
        # autostash (git stash push --include-untracked) and, with
        # --keep-stash (the desktop updater), never restored — `moor`
        ...
        # the managed clone under the default Moor root).
        try:
            from moor_cli._install_repair import migrate_windows_bin_path
        ...
        # don't have it. Tying the refresh to ``moor update`` gives users a
        ...
        try:
            ...
            if (
                ...
                from moor_cli.tools_config import install_cua_driver
                ...
                # driver) keeps the installed version — `moor update`
                # must stay fast; `moor computer-use install --upgrade`
        ...
        # When running as ``moor update --gateway`` (spawned by the gateway's
        ...
        #
        # Purge stale cached Moor modules FIRST: the import below pulls
        ...
        try:
            ...
            def _resolve_manage_cmd(scope_: str, scope_cmd_: list, svc_name_: str):
                ...
                ``systemctl ... moor-gateway*`` also qualifies
                ...
                if (
                    ...
                    try:
                        ...
                        if not sudo_ok:
                            ...
                            # (NOPASSWD for systemctl ... moor-gateway*)
            ...
            # Discover all moor-gateway* units (default + profiles) plus
                ...
                for scope, scope_cmd in [
                    ...
                    try:
                        ...
                        print(
                            ...
                            f"Check the gateway with: moor gateway status"
                        )
                        continue
                    ...
                    def _restart_one_systemd_gateway_unit(svc_name: str) -> None:
                        ...
                        # moor-serve has no such handler (it isn't
                        ...
                        if _manage_cmd is None:
                            ...
                            print(
                                ...
                                f"    To let `moor update` restart it automatically, allow\n"
            ...
            # Restart EVERY ai.moor.gateway* LaunchAgent, not only the
            ...
            if restarted_services or killed_pids:
                ...
                if unmapped_count:
                    ...
                    if unmapped_count > 1:
                        print(
                            "    (or: moor -p <profile> gateway run  for each profile)"
                        )

            if failed_or_stale_units:
                gateway_fleet_restart_incomplete = True
                if gateway_mode:
                    _exit_code_path = get_moor_home() / ".update_exit_code"
                    try:
            ...
            try:
                from moor_cli.update_receipt import record_gateway_restart
        ...
        # When both moor.service (from a pre-rename install) and the
        # current moor-gateway.service are enabled, they SIGTERM-fight
        # for the same bot token (see PR #11909). Flagging here means
        # every `moor update` surfaces the issue until the user migrates.
        try:
            from moor_cli.gateway import (
                has_legacy_moor_units,
                _find_legacy_moor_units,
                supports_systemd_services,
            )
            ...
            if supports_systemd_services() and has_legacy_moor_units():
                ...
                for name, path, is_sys in _find_legacy_moor_units():
                    scope = "system" if is_sys else "user"
                    print(f"    {path}  ({scope} scope)")
                print()
                print("  These pre-rename units (moor.service) fight the current")
                print("  moor-gateway.service for the bot token and cause SIGTERM")
                print("  flap loops. Remove them with:")
                print()
                print("    moor gateway migrate-legacy")
                print()
        ...
        #
        # Forward the systemd units restarted above (includes moor-serve*,
        ...
        print("  moor model              # Select provider and model")
        ...
        try:
            from moor_cli.update_receipt import (
...
def _restart_phase_failure_is_incomplete(surviving, pre_restart_pids) -> bool:
    ...
    * ``surviving is None`` — the survivor probe could not determine state
      (typically the freshly-pulled ``moor_cli.gateway`` no longer imports,
