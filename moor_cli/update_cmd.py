...
def _cmd_update_check(branch: str = "main", *, branch_explicit: bool = False):
    ...
    """
    from moor_cli.config import detect_install_method, recommended_update_command_for_method
...
def _ensure_acp_launcher() -> None:
    """Self-heal: install a ``moor-acp`` launcher next to the ``moor`` one.
    ...
    No-op on Windows (install.ps1 copies ``moor.exe`` + ``moor-acp.exe``
    ...
    so ``moor-acp.exe`` already resolves) and wherever a ``moor-acp`` is
    already present next to the ``moor`` command.  Unwritable directories
...
def _rebuild_desktop_after_update(
    ...
    if build_result.returncode != 0:
        print("  ⚠ Desktop build failed (non-fatal; run `moor desktop` to retry)")
