"""Moor update pipeline: dispatchers (``_cmd_update_impl``/``_cmd_update_check``) + git plumbing.

Each concern lives in ``update_cmd_<concern>.py`` and is re-imported here so
``moor_cli.update_cmd.<name>`` keeps resolving (and stays monkeypatchable). Imports are one-way:
main -> update_cmd -> update_cmd_*; ``_m()`` resolves ``moor_cli.main`` at call time.
"""
...
from moor_cli.update_cmd_windows import (  # noqa: F401
    ...
    _handoff_reapable_backend_pids, _moor_holder_subcommand, _holder_value_flags,
...
from moor_cli.update_cmd_fleet import (  # noqa: F401
...
from moor_cli.update_cmd_zip import (  # noqa: F401
...
from moor_cli.update_cmd_stash import (  # noqa: F401
...
from moor_cli.update_cmd_deps import (  # noqa: F401
...
def _m():
    """Lazy ``moor_cli.main`` handle: keeps main-side test patches effective, import one-way."""
...
_UPDATE_CRITICAL_FILES = (
    "moor_cli/main.py", "moor_cli/config.py", "moor_cli/__init__.py",
    "moor_cli/web_server.py", "cli.py", "run_agent.py", "model_tools.py", "toolsets.py",
    "moor_constants.py")
...
def _gateway_prompt(prompt_text: str, default: str = "", timeout: float = 300.0) -> str:
    ...
    from moor_constants import get_moor_home  # noqa: F811  (deliberate: constants variant)
    home = get_moor_home()
...
def _format_concurrent_instances_message(matches: list[tuple[int, str]], scripts_dir: Path) -> str:
    """Explanation + remediation hint for the Windows concurrent-moor.exe gate."""
...
def _classify_concurrent_instance(pid: int) -> str:
    ...
    from moor_cli._scan_venv_blockers import _is_pausable_gateway  # noqa: PLC0415
...
def _cmd_update_check(branch: str = "main", *, branch_explicit: bool = False):
    ...
    from moor_cli.update_contract import evaluate_update_admission, record_refusal_receipt
...
def _reconcile_diverged_checkout(git_cmd, branch: str, pre_pull_sha) -> None:
    ...
    if not has_common_ancestor and pre_pull_sha:
        ...
        rescue_ref = (
            f"refs/moor-update-backups/orphan-{branch}-"
...
def _pull_updates(
    ...
    # critical-path file (PR #28452 incident: orphan merge-conflict markers in moor_cli/config.py bricked
    # every user who ran ``moor update`` for the 7 minutes between the bad commit and the fix landing).
...
class _UpdateOptions:
    """Resolved ``moor update`` inputs (flags, config, pre-update snapshots)."""
...
def _begin_update_receipt_and_plan(args):
    ...
    pre-update plan (None if the probe failed); ``sys.exit(2)`` when a non-gateway moor.exe
    holds the venv shim."""
    ...
    # Plan phase (#91277 Phase 2): snapshot the pre-update fleet — every running Moor runtime, its
    ...
    with _best_effort('Update plan phase failed: %s'):
        from moor_cli.update_inventory import collect_runtime_inventory, record_plan_in_receipt
    ...
    # Windows: another moor.exe holding the venv shim means WinError 32 spam and a
    ...
    if _m()._is_windows() and not getattr(args, "force", False):
        ...
        concurrent = _m()._detect_concurrent_moor_instances(scripts_dir) if scripts_dir is not None else []
...
def _verify_head_after_pull(
    ...
    # step re-detaches to the SHA). Before this guard, ``moor update`` printed "✓ Code updated!" and
    # reinstalled deps + rebuilt the desktop app against the stale tree — no error, no warning, ``moor
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    # Any venv python still running (typically the Desktop `moor serve` backend) keeps .pyd
    ...
    # Self-lock deferral moved: the venv-holder sweep above excludes this process by design (a CLI `moor
