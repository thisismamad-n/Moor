...
cycle) and tests patch ``moor_cli.config.load_config`` etc. at call time.
"""
...
def _ask(prompt: str, *, secret: bool = False, raw: bool = False, cancel_msg: str | None = "\nCancelled.",
    ...
    if secret:
        from moor_cli.secret_prompt import masked_secret_prompt as fn
    else:
...
def _commit_model_config(cfg: dict) -> None:
    ...
    from moor_cli.auth import deactivate_provider
...
def _login_retry_context(args) -> tuple[str, str]:
    ...
    among its positional args. Moor keeps ``moor portal``; every other OAuth provider is retried
    with ``moor auth add <provider>`` and named by its own portal host (``moor login`` no longer
    exists). Falls back to ``moor model`` when no provider config is in play."""
    pconfig = next((a for a in args if hasattr(a, "id") and hasattr(a, "portal_base_url")), None)
    if pconfig is None:
        return "moor model", "the sign-in service"
    ...
    if provider_id == "moor":
        return "moor portal", host
    return (f"moor auth add {provider_id}" if provider_id else "moor model"), host
...
def _run_login(login_fn, *args, **kwargs) -> bool:
    ...
    provider config passed to the helper, so a MiniMax failure never says ``moor portal`` /
