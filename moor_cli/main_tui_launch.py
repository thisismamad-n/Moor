...
Split out of ``moor_cli/main.py``. Names that still live in main (``PROJECT_ROOT``, ...)
...
def _print_tui_exit_summary(session_id: Optional[str], active_session_file: Optional[str] = None) -> None:
    ...
    from moor_cli.main import _resolve_last_session
    target = (
...
def _tui_selected_workspace_keys(tui_dir: Path, ws_root: Path) -> set:
    ...
    from moor_cli.main import _is_termux_startup_environment
    try:
...
def _tui_need_npm_install(root: Path) -> bool:
    ...
    if not (ws_root / "node_modules" / "@moor" / "ink" / "package.json").is_file():
        return True
    if not lock.is_file():
        return False
...
_TUI_BUILD_INPUT_DIRS = ("src", "packages/moor-ink/src")
...
def _tui_need_rebuild(root: Path) -> bool:
    ...
    ``MOOR_TUI_FORCE_BUILD=1`` forces a rebuild."""
...
def _ensure_tui_node() -> None:
    ...
    the resolved node dir to PATH. ``MOOR_SKIP_NODE_BOOTSTRAP=1`` disables auto-install."""
    ...
    extras += [Path(moor_home) / "node" / "bin", Path.home() / ".local" / "bin"]
    for extra in extras:
        s = str(extra)
...
def _restore_tui_workspace(tui_dir: Path) -> bool:
    ...
    tracked files after ``moor update``); True when the directory exists afterwards.

    On Windows an antivirus / NTFS filter driver can leave tracked ``ui-tui/`` files deleted in the working
    tree after ``moor update`` (HEAD stays intact; the files just vanish — see issue #49145). Those files
...
def _tui_node_bin(bin: str) -> str:
    """Resolve ``node``/``npm`` for the TUI launch, or exit with a hint. ``MOOR_NODE`` wins for node;
    ``find_node_executable()`` sees the managed ``$MOOR_HOME/node`` tree a bare which() misses."""
    if bin == "node":
...
def _make_tui_argv(tui_dir: Path, tui_dev: bool) -> tuple[list[str], Path]:
    ...
    # moor_cli/tui_dist/entry.js but never ui-tui/ (git checkouts only).
    # 1. A prebuilt install (Docker image, Nix build, or prior `npm run build`) ships
    #   moor_cli/tui_dist/entry.js but never ships ui-tui/ at all (that directory only exists in a git
    ...
    if tui_dev:
        ...
        # packages/moor-ink/dist/entry-exports.js; a stale dist after a pull
        # leaves newer hooks/components missing at runtime. Prebuild it here.
        npm = _tui_node_bin("npm")
        _run_tui_npm_build(npm, tui_dir / "packages" / "moor-ink", "TUI dev prebuild failed.")
...
def _launch_tui(
    ...
    for key, value in (
        ("MOOR_MODEL", model), ("MOOR_INFERENCE_MODEL", model),
        ("MOOR_TUI_PROVIDER", provider), ("MOOR_INFERENCE_PROVIDER", provider),
        ("MOOR_TUI_TOOLSETS", ",".join(_normalize_tui_toolsets(toolsets))),
        ("MOOR_TUI_SKILLS", skills_value),
        ("MOOR_TUI_QUERY", query), ("MOOR_TUI_IMAGE", image),
        ("MOOR_TUI_CHECKPOINTS", "1" if checkpoints else None),
        ("MOOR_TUI_PASS_SESSION_ID", "1" if pass_session_id else None),
        ("MOOR_TUI_MAX_TURNS", str(max_turns) if max_turns is not None else None),
        ("MOOR_TUI_TOOL_PROGRESS", "verbose" if verbose else "off" if quiet else None),
        ("MOOR_ACCEPT_HOOKS", "1" if accept_hooks else None)):
        if value:
            env[key] = value
    ...
    # MOOR_TUI_RESUME is an internal hand-off to the Ink app. We start from a
    # full os.environ snapshot, so a stale exported value would make a plain
    # `moor --tui` try to resume a non-existent session; only forward the id
...
def _pin_kanban_board_env() -> None:
    """Pin the active kanban board into ``MOOR_KANBAN_BOARD`` so in-process tools and shelled-out
    ``moor kanban`` calls agree even if a concurrent ``boards switch`` flips the file mid-turn.

    Without this, in-process tools (``kanban_*``) and shelled-out CLI calls (``moor kanban …``) resolve
    the board on different paths: the env-pin if set, otherwise the global ``<root>/kanban/current`` file. A
    concurrent ``moor kanban boards switch`` from another session can flip the file mid-turn, so the same
...
def _sync_bundled_skills_quietly() -> None:
    """Seed ``~/.moor/skills/`` with the bundled library (idempotent, milliseconds when synced).
...
def _resolve_use_tui(args) -> bool:
    ...
    ``MOOR_TUI=1`` → TUI; ``display.interface`` config; default classic.

    The TTY gate is load-bearing: ambient preferences must never hijack a piped
    ``moor chat -q`` (kanban workers, cron) — the Ink no-TTY bail-out exits 0 and
