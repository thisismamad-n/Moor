...
from moor_cli.setup import (Colors, color, print_header, print_info, print_success, print_error,
                              prompt_yes_no)
...
_SCRIPT_REL = Path("migration", "openclaw-migration", "scripts", "openclaw_to_moor.py")
...
_OPENCLAW_SCRIPT_INSTALLED = get_moor_home() / "skills" / _SCRIPT_REL

# Known OpenClaw directory names (current + legacy)
_OPENCLAW_DIR_NAMES = (".openclaw", ".clawdbot", ".moltbot")

# `moor claw migrate` flags/defaults. Secrets are never included implicitly: --migrate-secrets
...
def _print_banner(title: str) -> None:
    ...
    print()
    rule = "─" * 57
    for line in (f"┌{rule}┐", f"│          ⚕ Moor — {title:<35s}│", f"└{rule}┘"):
...
def _warn_if_gateway_running(auto_yes: bool) -> None:
    """Warn if a Moor gateway has connected platforms (token conflicts, e.g. Telegram 409)."""
    ...
    if connected and _warn_running(
        auto_yes, "Moor gateway is running with active connections: " + ", ".join(connected), [],
        ("Migrating bot tokens while the gateway is active will cause "
         "conflicts (Telegram, Discord, and Slack only allow one active "
         "session per token).",
         "Recommendation: stop the gateway first with 'moor gateway stop'."),
...
def claw_command(args):
    ...
    if action == "migrate":
        ...
        print("Usage: moor claw <command> [options]\n\nCommands:\n"
              "  migrate          Migrate settings from OpenClaw to Moor\n"
              "  cleanup          Archive leftover OpenClaw directories after migration\n\n"
              "Run 'moor claw <command> --help' for options.")


def _cmd_migrate(args):
    """Run the OpenClaw → Moor migration: preflight, preview, confirm, back up, apply."""
    ...
    if not opts.source_dir.is_dir():
        return _error_block(
            ...
            "You can specify a custom path: moor claw migrate --source /path/to/.openclaw")
    ...
    opts.moor_home = get_moor_home()
    print()
    print_header("Migration Settings")
    _info(f"Source:      {opts.source_dir}", f"Target:      {opts.moor_home}",
    ...
    if _confirm(opts.yes, "Proceed with migration?", default=True, declined="Migration cancelled.",
                non_tty=("Non-interactive session — preview only.",
                         "To execute, re-run with: moor claw migrate --yes")):
        _apply_migration(run_migrator, opts)
    # Source directory is left untouched — archiving is `moor claw cleanup`'s job.
...
def _load_migrator(script_path: Path, opts: SimpleNamespace) -> Optional[Callable[[bool], dict]]:
    ...
    return lambda execute: mod.Migrator(
        source_root=opts.source_dir.resolve(), target_root=opts.moor_home.resolve(),
...
def _apply_migration(run_migrator: Callable[[bool], dict], opts: SimpleNamespace) -> None:
    ...
    restorable with `moor import`: one restore point before any mutation, pruned to the last 5."""
    backup_archive: Optional[Path] = None
    if not opts.no_backup:
        try:
            from moor_cli.backup import create_pre_migration_backup
            from moor_cli.sizefmt import format_bytes as _format_size
            backup_archive = create_pre_migration_backup(moor_home=opts.moor_home)
            if backup_archive:
                print()
    ...
    try:
        ...
        if backup_archive:
            _info(f"A pre-migration backup is available at: {backup_archive}",
                  f"Restore with: moor import {backup_archive.name}")
        return
...
def _cmd_cleanup(args):
    ...
    if running and not _warn_running(
        ...
        "Proceed anyway?",
        declined="Aborted. Stop OpenClaw first, then re-run: moor claw cleanup",
        ...
        if dry_run:
            ...
            print_info(f"Would archive: {source_dir} → {archive_path}")
                      ...
                      non_tty=(f"Non-interactive session — would archive: {source_dir}",
                               "To execute, re-run with: moor claw cleanup --yes")):
            try:
...
def _print_migration_report(report: dict, dry_run: bool):
    ...
    if dry_run:
        _info("", "To execute the migration, run without --dry-run:",
              f"  moor claw migrate --preset {report.get('preset', 'full')}")
    elif migrated:
        print()
        ...
        if any(i.get("kind") == "provider-keys" and i.get("status") == "skipped" for i in items):
            ...
            _info("", "To migrate API keys, re-run with:",
                  "  moor claw migrate --migrate-secrets", "", "Or add your key manually:",
                  "  moor config set OPENROUTER_API_KEY sk-or-v1-...")
