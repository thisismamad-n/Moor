...
Split out of ``moor_cli/config.py``; every name is re-imported there, so
``moor_cli.config.<name>`` keeps resolving (and monkeypatching) as before. Functions that call
``get_compatible_custom_providers`` / ``load_config*`` import them lazily from ``moor_cli.config``
...
logger = logging.getLogger("moor_cli.config")
...
_KNOWN_PROVIDER_KEYS = {
    # ``provider`` duplicates the ``providers.<name>`` mapping key and is unused here, but Moor'
...
def _normalize_custom_provider_entry(
    ...
    # ``models_discovered`` marks a mapping auto-discovered by Moor, not hand-curated.
