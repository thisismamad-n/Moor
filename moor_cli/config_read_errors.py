"""What Moor does when an existing ``config.yaml`` cannot be read or parsed.
...
def _warn_config_parse_failure(
    ...
    in ``moor_cli.config._load_config_impl``).
    """
    try:
        st = config_path.stat()
...
def _read_error_fix(exc: OSError) -> str:
    return _FIX_PERMS if isinstance(exc, PermissionError) else (
        "Try again; run `moor config check` if it keeps failing.")
