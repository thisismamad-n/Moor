"""External-tool checks for moor doctor: terminal backends, git/rg, Node + agent-browser, npm audit, tool availability.
...
def _doctor_tool(name: str) -> tuple[str | None, str]:
    """Resolve the tool Moor would actually run: the pm store first
...
_TERMUX_INSTALL_ALL_FALLBACK_NOTES = (
    "Termux uses the Moor APT package: pkg install moor-agent.",
...
def _is_kanban_worker_env_gate(item: dict) -> bool:
    ...
    return (item.get("name") == "kanban" and not os.environ.get("MOOR_KANBAN_TASK")
...
def _apply_doctor_tool_availability_overrides(available: list[str], unavailable: list[dict]) -> tuple[list[str], list[dict]]:
    ...
    from moor_cli.doctor_state import _honcho_is_configured_for_doctor
...
_TOOLSET_SETUP_HINTS: dict[str, str] = {
    "image_gen": "(image generation unavailable — check the provider selection and its key or SDK with 'moor tools')",
}
    ...
    if terminal_env == "docker":
        if not docker_exe:
            _fail_and_issue("Docker or Podman not installed", "(needed for the 'docker' terminal backend)",
                            "Install Docker or Podman, or run `moor setup terminal` to switch backend.", issues)
        else:
            ...
            _require(_run_ok([docker_exe, "version"], timeout=10),
                     ...
                     f"{hint[0].upper()}{hint[1:]}, or run `moor setup terminal` to switch backend.", issues)
    elif docker_exe:
...
def _check_ssh_backend(issues: list[str]) -> None:
    ...
    if not ssh_host:
        return _fail_and_issue("SSH host not configured", "(needed for the 'ssh' terminal backend)",
                               "run `moor setup terminal` and enter the SSH host and user.", issues)
...
def _check_daytona_backend(issues: list[str]) -> None:
    _require(os.getenv("DAYTONA_API_KEY"), ("Daytona API key", "(configured)"),
             ...
             "run `moor setup terminal` (Daytona) to enter it.", issues)
    try:
        from daytona import Daytona  # noqa: F401 — SDK presence check
        check_ok("daytona SDK", "(installed)")
    except ImportError:
        _fail_and_issue("daytona SDK not installed", "(run moor setup terminal)", "Run moor setup terminal and select Daytona, then restart Moor", issues)
...
def _check_vercel_backend(issues: list[str]) -> None:
    ...
    _require(importlib.util.find_spec("vercel") is not None, ("vercel SDK", "(installed)"),
             ("vercel SDK not installed", "(run moor setup terminal)"),
             "Run moor setup terminal and select Vercel Sandbox, then restart Moor", issues)
...
def _check_agent_browser(should_fix: bool) -> bool:
    ...
    if _is_termux():
        ...
        check_warn("agent-browser not installed", "(run: moor pm install agent-browser)")
    return False
...
def _check_chromium() -> None:
    ...
    if not check_bool(_chromium_installed(), ("Playwright Chromium", "(browser engine)"),
        ...
        check_info("Install with: moor pm install chromium")
...
def _check_lightpanda() -> None:
    ...
    if not used:
        ...
        check_info("Fix: pick Lightpanda in `moor tools` → Browser Automation, or set browser.engine: auto")
...
def _audit_one(npm_bin: str, npm_dir, label: str, audit_extra: list[str], issues: list[str]) -> None:
    ...
    every `moor update`), so a local `npm audit fix` never persists — the next
    ...
    try:
        ...
        if total == 0:
            ...
            remedy = ("fix is an upstream lockfile bump — a local manual fix does not persist"
                      " (the next `moor update` reinstalls from the committed lockfile)")
...
def _check_npm_audit(should_fix: bool, f: Finding) -> None:
    ...
    MOOR_HOME mirror rather than the (possibly read-only) Docker install tree, hence the shared resolver.
    """
    ...
    if npm_bin:
        try:
            ...
            # See #38772. The WhatsApp bridge may live under a writable MOOR_HOME mirror instead of the
...
def _check_tool_availability(should_fix: bool, f: Finding) -> None:
    ...
    if api_disabled or any(status != "ok" for status, _, _ in web_rows):
        f.issues.append("Run 'moor setup' to configure missing API keys for full tool access")
