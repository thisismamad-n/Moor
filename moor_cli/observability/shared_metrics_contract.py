...
def client_resource(
    ...
    *,
    ...
    normalized_version = str(moor_version or "").strip()
    ...
    return {
        ...
        "moor_version": normalized_version,
...
def _auxiliary_model_call_dimensions(event: Any) -> dict[str, str] | None:
    ...
    if relay_metadata - {"otel.status_code"} or metadata.get(
        ...
        return None
    call_role = metadata.get("moor.call_role")
...
def tool_category(kwargs: dict[str, Any]) -> str:
    ...
    if toolset.startswith(
        ("discord", "email", "feishu", "moor-yuanbao", "slack", "sms")
    ):
