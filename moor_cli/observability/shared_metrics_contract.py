...
SCHEMA_KEY = "moor.metrics.schema_version"
SCHEMA_VERSION = "moor.metrics.event.v2"
MODEL_CALL_SCOPE = "moor.model_call"
MODEL_CALL_PROFILE_MODEL = "unknown"
TASK_SCOPE = "moor.task_run"
TOOL_CALL_SCOPE = "moor.tool_call"
CLIENT_ACTIVE_MARK = "moor.client.active"
TOOL_APPROVAL_MARK = "moor.tool_approval"
SKILL_LIFECYCLE_MARK = "moor.skill.lifecycle"
SKILL_LOAD_MARK = "moor.skill.load"
SUBSCRIBER_NAME = "moor.nemo_relay.shared_metrics"
CLIENT_ACTIVE_METRIC = "moor.client.active"
LEGACY_MODEL_CALL_METRIC = "moor.model_call.count"
MODEL_ROUTE_METRIC = "moor.model_route.count"
TASK_STARTED_METRIC = "moor.task_run.started"
TASK_FINISHED_METRIC = "moor.task_run.finished"
TOOL_CALL_METRIC = "moor.tool_call.count"
TOOL_APPROVAL_METRIC = "moor.tool_approval.count"
SKILL_LIFECYCLE_METRIC = "moor.skill.lifecycle.count"
SKILL_LOAD_METRIC = "moor.skill.load.count"
...
CLIENT_RESOURCE_KEYS = frozenset({"architecture", "moor_version", "install_method", "os_family"})
...
def client_resource(
    moor_version: Any, *, os_name: Any, architecture: Any, install_method: Any
) -> dict[str, str]:
    """Build the bounded client resource attached to aggregate packages."""
    version = str(moor_version or "").strip()
    return {
        "architecture": client_architecture(architecture),
        "moor_version": version if 0 < len(version) <= 64 else "unknown",
...
def client_resource_is_valid(resource: Any) -> bool:
    ...
    if not isinstance(resource, dict) or set(resource) != CLIENT_RESOURCE_KEYS:
        return False
    version = resource.get("moor_version")
    return (
...
def _auxiliary_model_call_dimensions(event: Any) -> dict[str, str] | None:
    ...
    metadata = _relay_metadata(
        event, RUNTIME_SCHEMA_KEY, RUNTIME_SCHEMA_VERSION, "moor.call_role"
    )
    call_role = (metadata or {}).get("moor.call_role")
...
def execution_surface(kwargs: dict[str, Any]) -> str:
    ...
    try:
        from moor_cli.platforms import get_all_platforms
...
def task_terminal_state(kwargs: dict[str, Any]) -> tuple[str, str, str]:
    """Map Moor terminal state to bounded (outcome, end_reason, termination)."""
...
_TOOL_CATEGORY_PREFIXES = (
    ...
    (("discord", "email", "feishu", "moor-yuanbao", "slack", "sms"), "communication"),
)
