"""Direct NeMo Relay integration for Moor shared client metrics."""
...
def _retry_ordinal(event: dict[str, Any]) -> int:
    """Moor's provider-local retry ordinal; 0 when absent or malformed."""
...
class _Runtime:
    """Own shared-metrics state layered on the Moor core Relay host."""
    ...
    def start_task(self, event: dict[str, Any]) -> _TaskRun | None:
        """Open one Relay function scope for a Moor task run."""
        ...
        with session.lock:
            ...
            if existing is not None:
                ...
                if task is not None:
                    ...
                    # attempt. Provider fallback resets Moor's provider-local retry
            ...
            if task is not None:
                ...
                if _retry_ordinal(event) > 0:
                    # A real Moor retry can advance api_request_id while carrying the
    ...
    def finish_task(self, event: dict[str, Any]) -> None:
        ...
        if finished:
            self._flush_and_export("Moor shared-metrics task flush failed")
    ...
    def close_session(self, event: dict[str, Any]) -> None:
        ...
        try:
            ...
            logger.warning(
                "Moor shared-metrics session %s closed with errors: subscriber flush failed: %s",
                session.session_id,
                exc,
            )
        else:
    ...
    def shutdown(self) -> None:
        ...
        if not self._registered:
            return
        self._flush_and_export("Moor shared-metrics shutdown flush failed")
        self._deregister()
        self._release()
    ...
    def _finish_tool_call(
        ...
        self._guarded(
            "Moor shared-metrics tool call close failed",
    ...
    def _finish_model_call(self, session: _MetricsSession, model_call_key: tuple[str, str]) -> None:
        ...
        if model_call is None:
            return
        self._guarded(
            "Moor shared-metrics model call close failed",
        ...
        with self._send_lock:
            ...
            if self._send_thread is None or not self._send_thread.is_alive():
                self._send_thread = threading.Thread(
                    ...
                    name="moor-shared-metrics-send", daemon=True,
                )
    ...
    def _safe(cls, callback: Callable[..., Any], *args: Any, **kwargs: Any) -> Any:
        return cls._guarded("Moor shared metrics operation failed", callback, *args, **kwargs)


def _raw_config() -> dict[str, Any]:
    """Read-only config snapshot (lazy import: tests patch ``moor_cli.config``).
    ...
    """
    from moor_cli.config import read_raw_config_readonly
...
def _resolved_send_config():
    ...
    from moor_cli.observability.shared_metrics_send_config import resolve_send_config
...
def enabled() -> bool:
    """Return the shared-metrics policy for the active Moor profile."""
    profile_key = relay_runtime.current_profile_key()
    try:
        config: Any = _raw_config()
    except Exception:
        logger.debug("Unable to read Moor shared-metrics policy", exc_info=True)
        config = None
...
def _reconcile_send_consent_once() -> None:
    ...
    AND consent is off: nothing to protect, and creating ``~/.moor/telemetry`` for every
    ...
    try:
        ...
        default_path = get_moor_home() / "telemetry" / "shared_metrics" / "metrics.sqlite3"
...
def observe_lifecycle(hook_name: str, **kwargs: Any) -> None:
    """Project one Moor lifecycle event into the core Relay integration."""
    ...
    try:
        ...
        logger.warning("Moor shared metrics hook failed: %s", hook_name, exc_info=True)


def _with_runtime_toolset(event: dict[str, Any]) -> dict[str, Any]:
    """Attach the toolset already declared by Moor's runtime registry."""
...
def start_task_run(
    *, session_id: str, task_id: str, platform: str, parent_session_id: str = ""
    ...
    """Start task metrics at the outer Moor execution boundary."""
    _run_task_hook(
    ...
    *, retry_failed: bool = False, host: relay_runtime.RelayRuntime | None = None
    ...
    with _RUNTIME_LOCK:
        ...
        try:
            ...
            logger.warning("Moor shared metrics initialization failed", exc_info=True)
...
_PLUGIN_COMPAT_LAZY = {
    'CLIENT_ACTIVE_MARK': ('moor_cli.observability.shared_metrics_contract', 'CLIENT_ACTIVE_MARK'),
    'MODEL_CALL_PROFILE_MODEL': ('moor_cli.observability.shared_metrics_contract', 'MODEL_CALL_PROFILE_MODEL'),
    'SCHEMA_KEY': ('moor_cli.observability.shared_metrics_contract', 'SCHEMA_KEY'),
    'SCHEMA_VERSION': ('moor_cli.observability.shared_metrics_contract', 'SCHEMA_VERSION'),
    'SKILL_LIFECYCLE_MARK': ('moor_cli.observability.shared_metrics_contract', 'SKILL_LIFECYCLE_MARK'),
    'SKILL_LOAD_MARK': ('moor_cli.observability.shared_metrics_contract', 'SKILL_LOAD_MARK'),
    'TOOL_APPROVAL_MARK': ('moor_cli.observability.shared_metrics_contract', 'TOOL_APPROVAL_MARK'),
    'TOOL_CALL_SCOPE': ('moor_cli.observability.shared_metrics_contract', 'TOOL_CALL_SCOPE'),
    'model_call_fields': ('moor_cli.observability.shared_metrics_contract', 'model_call_fields'),
    'skill_lifecycle_fields': ('moor_cli.observability.shared_metrics_contract', 'skill_lifecycle_fields'),
    'skill_load_fields': ('moor_cli.observability.shared_metrics_contract', 'skill_load_fields'),
    'task_start_fields': ('moor_cli.observability.shared_metrics_contract', 'task_start_fields'),
    'task_terminal_fields': ('moor_cli.observability.shared_metrics_contract', 'task_terminal_fields'),
    'task_terminal_state': ('moor_cli.observability.shared_metrics_contract', 'task_terminal_state'),
    'tool_approval_outcome': ('moor_cli.observability.shared_metrics_contract', 'tool_approval_outcome'),
    'tool_category': ('moor_cli.observability.shared_metrics_contract', 'tool_category'),
    'tool_terminal_fields': ('moor_cli.observability.shared_metrics_contract', 'tool_terminal_fields'),
}
