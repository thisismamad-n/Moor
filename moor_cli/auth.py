"""
Multi-provider authentication system for Moor Agent.

Supports OAuth device code flows (Moor Portal, future: OpenAI Codex) and
traditional API key providers (OpenRouter, custom endpoints). Auth state
is persisted in ~/.moor/auth.json with cross-process file locking.
...
# httpx is imported lazily: it costs ~30ms at import time and moor_cli.auth
...
if TYPE_CHECKING:
    ...
    class _LazyHttpx:
        ...
        # ("moor_cli.auth.httpx.Client", ...) keeps working in tests.
...
from moor_constants import OPENROUTER_BASE_URL, secure_parent_dir
...
try:  # Version tag for the Codex token-endpoint User-Agent; fall back if unavailable.
    from moor_cli import __version__ as _MOOR_CLI_VERSION
except Exception:  # pragma: no cover - version import should always succeed
    _MOOR_CLI_VERSION = "unknown"
CODEX_OAUTH_USER_AGENT = f"moor-cli/{_MOOR_CLI_VERSION}"
...
PROVIDER_REGISTRY: Dict[str, ProviderConfig] = {
    "moor": ProviderConfig(
        ...
        portal_base_url=DEFAULT_MOOR_PORTAL_URL,
        inference_base_url=DEFAULT_MOOR_INFERENCE_URL,
        client_id=DEFAULT_MOOR_CLIENT_ID,
        scope=DEFAULT_MOOR_SCOPE,
    ),
    ...
    "opencode-free": ProviderConfig(
        ...
        # secret to configure. Select via `moor model` / `/model free`.
        api_key_env_vars=(),
    ),
...
def get_anthropic_key() -> str:
    ...
    ``~/.moor/.env`` so a deliberate key rotation isn't shadowed by a stale
...
def _resolve_api_key_provider_secret(
    ...
    if provider_id == "copilot":
        ...
        try:
            from moor_cli.copilot_auth import resolve_copilot_token, get_copilot_api_token
    ...
    for env_var in pconfig.api_key_env_vars:
        # Prefer ~/.moor/.env over os.environ so a deliberate key rotation
...
def is_rate_limited_auth_error(error: Exception) -> bool:
    ...
    instead of prompting the operator to run ``moor auth``.
    """
    return (
...
def format_auth_error(error: Exception) -> str:
    ...
    if error.relogin_required:
        return f"{error} Run `moor model` to re-authenticate."
...
def _format_moor_entitlement_auth_error(error: AuthError) -> str:
    try:
        ...
        message = format_moor_portal_entitlement_message(
            account_info,
            capability="Moor model access",
        )
        if message:
            return message
    except Exception:
        pass
    return f"{error} Check credits or billing in Moor Portal, then retry."
...
def _oauth_trace_enabled() -> bool:
    raw = os.getenv("MOOR_OAUTH_TRACE", "").strip().lower()
...
# =============================================================================
# Auth Store — persistence layer for ~/.moor/auth.json
# =============================================================================

def _auth_file_path() -> Path:
    path = get_moor_home() / "auth.json"
    # Seat belt: if pytest is running and MOOR_HOME resolves to the real
    # user's auth store, refuse rather than silently corrupt it. This catches
    # tests that forgot to monkeypatch MOOR_HOME, tests invoked without the
    # hermetic conftest, or sandbox escapes via threads/subprocesses. In
    # production (no PYTEST_CURRENT_TEST) this is a single dict lookup.
    if os.environ.get("PYTEST_CURRENT_TEST"):
        real_home_auth = (Path.home() / ".moor" / "auth.json").resolve(strict=False)
        try:
        ...
        if resolved == real_home_auth:
            raise RuntimeError(
                ...
                "Set MOOR_HOME to a tmp_path in your test fixture, or run "
...
def _global_auth_file_path() -> Optional[Path]:
    ...
    directory (classic mode, or custom MOOR_HOME that is not a profile).
    ...
    try:
        ...
        global_root = get_default_moor_root()
    except Exception:
        return None
    profile_home = get_moor_home()
    try:
...
def _load_global_auth_store() -> Dict[str, Any]:
    ...
    ``_moor_auth_status_cache``): read_credential_pool() -> load_pool() runs
    this once per provider row in the /model picker, and the path resolution
    (``_global_auth_file_path()`` -> ``get_default_moor_root()``) + JSON
    ...
    if os.environ.get("PYTEST_CURRENT_TEST"):
        ...
        if real_home_env:
            real_root = Path(real_home_env) / ".moor" / "auth.json"
            try:
...
def _file_lock(
    ...
    (e.g. profile auth.json vs shared Moor store) don't share reentrancy
...
def _auth_store_lock(
    ...
    *,
    ...
    ``_moor_shared_store_lock``, acquire ``_auth_store_lock`` FIRST
    (outer) and the shared Moor lock SECOND (inner). All runtime
...
def _load_auth_store(auth_file: Optional[Path] = None) -> Dict[str, Any]:
    ...
    if isinstance(raw, dict) and (
        ...
        if isinstance(raw.get("providers"), dict):
            _migrate_stale_moor_portal_url(raw["providers"])
        return raw
    ...
    if isinstance(raw, dict) and isinstance(raw.get("systems"), dict):
        systems = raw["systems"]
        providers = {}
        if "moor_portal" in systems:
            providers["moor"] = systems["moor_portal"]
        return {"version": AUTH_STORE_VERSION, "providers": providers,
                "active_provider": "moor" if providers else None}
...
def _save_auth_store(auth_store: Dict[str, Any], target_path: Optional[Path] = None) -> Path:
    ...
    # moor-agent install tree (#25821, #93050).
...
def _load_provider_state_with_source(
    ...
    global-root fallback state; persisting a rotated Moor refresh token only to
...
def _load_provider_state(auth_store: Dict[str, Any], provider_id: str) -> Optional[Dict[str, Any]]:
    ...
    profile can see providers (e.g. ``moor``) that were only authenticated at
    global scope. Once the user runs ``moor auth login <provider>`` inside
...
def mark_provider_active_if_unset(provider_id: str) -> None:
    ...
    Used by ``moor auth add`` OAuth paths that create credential-pool
...
def read_credential_pool(provider_id: Optional[str] = None) -> Dict[str, Any]:
    ...
    ``moor auth add <provider>`` inside the profile, profile entries
...
def get_provider_auth_state(provider_id: str) -> Optional[Dict[str, Any]]:
    ...
    OAuth or Moor device-code session). See issue #18594 follow-up.
    """
    auth_store = _load_auth_store()
...
def is_provider_explicitly_configured(provider_id: str) -> bool:
    ...
    # not by the user explicitly configuring anthropic in Moor.
    ...
    # the user initiated inside Moor (manual add / device-code / PKCE), plus
    ...
    try:
        for entry in read_credential_pool(normalized):
            ...
            if source.startswith("env:"):
                ...
                continue
            if (
                source in {"device_code", "loopback_pkce", "moor_pkce", "manual"}
    ...
    # explicit for users who never set Moor up for it. Only moor-scoped
...
def clear_provider_auth(provider_id: Optional[str] = None) -> bool:
    """
    Clear auth state for a provider. Used by `moor logout`.
...
def _get_config_hint_for_unknown_provider(provider_name: str) -> str:
    ...
    try:
        ...
        lines = ["Config issue detected — run 'moor doctor' for full diagnostics:"]
        for ci in issues:
    ...
    *,
    ...
    if normalized != "auto":
        ...
        if _config_hint:
            ...
            msg += " Check 'moor model' for available providers, or run 'moor doctor' to diagnose config issues."
    ...
    # Auto-detect an OpenRouter credential added via `moor auth add openrouter`
    ...
    # `moor auth list` showing the credential while requests go out with no
    ...
    for pid, pconfig in PROVIDER_REGISTRY.items():
        ...
        for env_var in pconfig.api_key_env_vars:
            if has_usable_secret(_scoped_key_env(env_var)):
                ...
                # OAuth but has a stale key in ~/.moor/.env isn't silently
    ...
    raise AuthError(
        "No inference provider configured. Run 'moor model' to choose a "
        "provider and model, or set an API key (OPENROUTER_API_KEY, "
        "OPENAI_API_KEY, etc.) in ~/.moor/.env.",
...
_MOOR_STALE_PORTAL_HOSTS: FrozenSet[str] = frozenset({
    "api.nousresearch.com",
})

# Allowlist of valid Moor Portal hosts. A portal_base_url outside this
...
_MOOR_PORTAL_ALLOWED_HOSTS: FrozenSet[str] = frozenset({
...
def _migrate_stale_moor_portal_url(providers: Dict[str, Any]) -> None:
    moor = providers.get("moor")
    if not isinstance(moor, dict):
        return
    stored = (moor.get("portal_base_url") or "").strip()
    if stored:
        parsed = urlparse(stored)
        if parsed.hostname in _MOOR_STALE_PORTAL_HOSTS:
            logger.warning(
                "auth: migrating stale moor portal_base_url %s -> %s",
                stored, DEFAULT_MOOR_PORTAL_URL,
            )
            moor["portal_base_url"] = DEFAULT_MOOR_PORTAL_URL


# Allowlist of hosts the Moor Portal proxy is willing to forward inference
...
# (MOOR_INFERENCE_BASE_URL) bypass validation — that's the documented
...
def _validate_moor_inference_url_from_network(url: Optional[str]) -> Optional[str]:
    ...
    The env-var override path (``MOOR_INFERENCE_BASE_URL``) bypasses
    ...
    if parsed.scheme != "https":
        logger.warning(
            "moor: refusing non-https inference URL scheme %r from Portal response",
            parsed.scheme,
        )
        return None
    if parsed.hostname not in _ALLOWED_MOOR_INFERENCE_HOSTS:
        logger.warning(
...
def _moor_inference_env_override() -> Optional[str]:
    """Return the user-set ``MOOR_INFERENCE_BASE_URL`` override, if any.
    ...
    """
    return _optional_base_url(os.getenv("MOOR_INFERENCE_BASE_URL"))


def _moor_portal_env_override() -> Optional[str]:
    """Return the user/deployment-set Portal base URL override, if any.

    Mirrors ``_moor_inference_env_override()``: ``MOOR_PORTAL_BASE_URL`` /
    ``MOOR_PORTAL_BASE_URL`` are the documented dev/staging escape hatch for
    pointing Moor at a non-production Moor Portal (e.g. a hosted agent
    provisioned on moor-account-service's `staging` environment, which stamps
    ``MOOR_PORTAL_BASE_URL=https://portal.staging-nousresearch.com`` into
    ...
    gated by ``_MOOR_PORTAL_ALLOWED_HOSTS``: that allowlist exists to reject
    ...
    return _optional_base_url(
        os.getenv("MOOR_PORTAL_BASE_URL") or os.getenv("MOOR_PORTAL_BASE_URL")
    )
...
def _moor_invoke_jwt_status(
    token: Any,
    *,
    scope: Any = None,
    expires_at: Any = None,
    min_ttl_seconds: int = MOOR_INVOKE_JWT_MIN_TTL_SECONDS,
) -> Optional[str]:
    ...
    if MOOR_INFERENCE_INVOKE_SCOPE not in scopes:
...
def _moor_invoke_jwt_is_usable(
    token: Any,
    *,
    scope: Any = None,
    expires_at: Any = None,
...
def _assert_moor_inference_jwt_usable(
    ...
    *,
    ...
    if reason is None:
        return
    raise AuthError(
        "Moor Portal access token is not a usable inference JWT "
        f"({reason}). Re-authenticate with: moor auth add moor",
        provider="moor",
        code=reason,
...
def _log_moor_invoke_jwt_selected(
    *,
    access_token: Any,
    sequence_id: Optional[str] = None,
) -> None:
    logger.debug("Moor inference auth: using NAS invoke JWT")
    _oauth_trace(
        "moor_invoke_jwt_selected",
        sequence_id=sequence_id,
        access_token_fp=_token_fingerprint(access_token),
    )


def _moor_jwt_expires_at(token: Any, fallback_expires_at: Any = None) -> Optional[str]:
    claims = _decode_jwt_claims(token)
    exp = claims.get("exp")
    if isinstance(exp, (int, float)):
        try:
...
def _set_moor_agent_key_from_invoke_jwt(
    state: Dict[str, Any],
    *,
    ...
    expires_at = _moor_jwt_expires_at(access_token, state.get("expires_at"))
...
def _select_moor_invoke_jwt(
    ...
    *,
    ...
    _set_moor_agent_key_from_invoke_jwt(state)
    _log_moor_invoke_jwt_selected(
        access_token=state.get("access_token"),
        sequence_id=sequence_id,
    )


_MOOR_EFFECTIVE_STATE_IGNORED_KEYS = frozenset({
...
def _moor_effective_provider_state(state: Dict[str, Any]) -> Dict[str, Any]:
    return {
        key: value
        for key, value in state.items()
        if key not in _MOOR_EFFECTIVE_STATE_IGNORED_KEYS
    }
...
def resolve_qwen_runtime_credentials(
    *,
    ...
    base_url = os.getenv("MOOR_QWEN_BASE_URL", "").strip().rstrip("/") or DEFAULT_QWEN_BASE_URL
    return {
        "provider": "qwen-oauth",
        "base_url": base_url,
...
def get_qwen_auth_status() -> Dict[str, Any]:
    ...
    try:
        ...
        # and `moor model` walks users into a broken Qwen setup flow.
...
# =============================================================================
# Spotify auth — PKCE tokens stored in ~/.moor/auth.json
# =============================================================================
...
def _spotify_client_id(
    ...
    candidates = (
        explicit,
        get_env_value("MOOR_SPOTIFY_CLIENT_ID"),
    ...
    for candidate in candidates:
        ...
        if cleaned:
            return cleaned
    raise AuthError(
        "Spotify client_id is required. Set MOOR_SPOTIFY_CLIENT_ID or pass --client-id.",
        provider="spotify",
...
def _spotify_redirect_uri(
    ...
    candidates = (
        explicit,
        get_env_value("MOOR_SPOTIFY_REDIRECT_URI"),
...
def _spotify_api_base_url(state: Optional[Dict[str, Any]] = None) -> str:
    ...
    candidates = (
        get_env_value("MOOR_SPOTIFY_API_BASE_URL"),
...
def _spotify_accounts_base_url(state: Optional[Dict[str, Any]] = None) -> str:
    ...
    candidates = (
        get_env_value("MOOR_SPOTIFY_ACCOUNTS_BASE_URL"),
...
def _refresh_spotify_oauth_state(
    ...
    *,
    ...
    if not refresh_token:
        raise AuthError(
            "Spotify refresh token missing. Run `moor auth spotify` again.",
            provider="spotify",
    ...
    if response.status_code >= 400:
        detail = response.text.strip()
        raise AuthError(
            "Spotify token refresh failed. Run `moor auth spotify` again."
            + (f" Response: {detail}" if detail else ""),
            provider="spotify",
            code="spotify_refresh_failed",
            relogin_required=True,
...
def resolve_spotify_runtime_credentials(
    *,
    ...
    with _auth_store_lock():
        ...
        if not state:
            raise AuthError(
                "Spotify is not authenticated. Run `moor auth spotify` first.",
                provider="spotify",
        ...
        if should_refresh:
            try:
                ...
                if exc.relogin_required and state.get("refresh_token"):
                    ...
                    # Mirrors the Moor / xAI-OAuth / Codex-OAuth / MiniMax pattern.
    ...
    if not access_token:
        raise AuthError(
            "Spotify access token missing. Run `moor auth spotify` again.",
            provider="spotify",
...
def _spotify_interactive_setup(redirect_uri_hint: str) -> str:
    ...
    resulting client_id to ~/.moor/.env, and return it.
    ...
    print("       App name:     anything (e.g. moor-agent)")
    ...
    # Persist so subsequent `moor auth spotify` runs skip the wizard.
    save_env_value("MOOR_SPOTIFY_CLIENT_ID", raw)
    ...
    if redirect_uri_hint and redirect_uri_hint != DEFAULT_SPOTIFY_REDIRECT_URI:
        save_env_value("MOOR_SPOTIFY_REDIRECT_URI", redirect_uri_hint)

    print()
    print("Saved MOOR_SPOTIFY_CLIENT_ID to ~/.moor/.env")
    print()
    return raw
...
def login_spotify_command(args) -> None:
    ...
    # with "MOOR_SPOTIFY_CLIENT_ID is required".
    ...
    print()
    print("Open this URL to authorize Moor:")
    print(authorize_url)
    print()
...
def _print_loopback_ssh_hint(redirect_uri: str, *, docs_url: str | None = None) -> None:
    ...
    print(divider)
    print(f"Moor is waiting for the OAuth callback on {redirect_uri}")
...
# =============================================================================
# OpenAI Codex auth — tokens stored in ~/.moor/auth.json (not ~/.codex/)
#
# Moor maintains its own Codex OAuth session separate from the Codex CLI
# and VS Code extension. This prevents refresh token rotation conflicts
# where one app's refresh invalidates the other's session.
# =============================================================================

def _read_codex_tokens(*, _lock: bool = True) -> Dict[str, Any]:
    """Read Codex OAuth tokens from Moor auth store (~/.moor/auth.json).
    ...
    if not state:
        raise AuthError(
            "No Codex credentials stored. Run `moor auth` to authenticate.",
    ...
    if not isinstance(tokens, dict):
        raise AuthError(
            "Codex auth state is missing tokens. Run `moor auth` to re-authenticate.",
            provider="openai-codex",
    ...
    if not isinstance(access_token, str) or not access_token.strip():
        raise AuthError(
            "Codex auth is missing access_token. Run `moor auth` to re-authenticate.",
            provider="openai-codex",
    ...
    if not isinstance(refresh_token, str) or not refresh_token.strip():
        raise AuthError(
            "Codex auth is missing refresh_token. Run `moor auth` to re-authenticate.",
            provider="openai-codex",
...
def _sync_codex_pool_entries(
    ...
    * ``device_code`` — the singleton-seeded entry written by the device-code
      OAuth flow when the user logged in via ``moor setup`` / the model
      picker.  Always synced with the fresh tokens.
    * ``manual:device_code`` — entries created by ``moor auth add openai-codex``
      ...
      ``moor auth add openai-codex`` and must not be overwritten by a
...
def _save_codex_tokens(tokens: Dict[str, str], last_refresh: str = None, label: str = None) -> None:
    """Save Codex OAuth tokens to Moor auth store (~/.moor/auth.json)."""
    ...
    with _auth_store_lock():
        ...
        # ``moor auth add openai-codex`` created (which must not be
...
def refresh_codex_oauth_pure(
    ...
    *,
    ...
    """Refresh Codex OAuth tokens without mutating Moor auth state."""
    ...
    if response.status_code == 429:
        ...
        # "run moor auth" prompt (see issue #32790).
    ...
    if response.status_code != 200:
        ...
        if code == "refresh_token_reused":
            message = (
                ...
                "then run `moor auth` to re-authenticate."
            )
            relogin_required = True
...
def _refresh_codex_auth_tokens(
    ...
    Saves the new tokens to Moor auth store automatically.
    """
    try:
        ...
        # Self-heal cross-store refresh_token rotation. Moor keeps its OWN
        ...
        # the Codex CLI (or another Moor process) rotates the shared token,
...
def resolve_codex_runtime_credentials(
    *,
    ...
    if data is None:
        ...
        if pool_token:
            base_url = (
                os.getenv("MOOR_CODEX_BASE_URL", "").strip().rstrip("/")
    ...
    access_token = str(tokens.get("access_token", "") or "").strip()
    refresh_timeout_seconds = env_float("MOOR_CODEX_REFRESH_TIMEOUT_SECONDS", 20)

    should_refresh = bool(force_refresh)
    if (not should_refresh) and refresh_if_expiring:
        should_refresh = _codex_access_token_is_expiring(access_token, refresh_skew_seconds)
    if should_refresh:
        # Re-read under lock to avoid racing with other Moor processes
...
# =============================================================================
# xAI Grok OAuth — tokens stored in ~/.moor/auth.json
# =============================================================================
...
def _read_xai_oauth_tokens(*, _lock: bool = True) -> Dict[str, Any]:
    ...
    if not state:
        raise AuthError(
            "No xAI OAuth credentials stored. Select xAI Grok OAuth (SuperGrok / Premium+) in `moor model`.",
            provider="xai-oauth",
    ...
    if not isinstance(tokens, dict):
        raise AuthError(
            "xAI OAuth state is missing tokens. Re-authenticate with `moor model`.",
            provider="xai-oauth",
    ...
    if not access_token:
        raise AuthError(
            "xAI OAuth state is missing access_token. Re-authenticate with `moor model`.",
            provider="xai-oauth",
    ...
    if not refresh_token:
        raise AuthError(
            "xAI OAuth state is missing refresh_token. Re-authenticate with `moor model`.",
            provider="xai-oauth",
...
def _write_through_xai_oauth_to_global_root(state: Dict[str, Any]) -> None:
    ...
    # ~/.moor/auth.json even when MOOR_HOME points at a profile path
...
def _xai_proactive_refresh_skew_seconds(access_token: str) -> int:
    ...
    resolution (chat turn, Imagine tool call, ``moor auth status``, …),
...
def _xai_validate_oauth_endpoint(url: str, *, field: str) -> str:
    ...
    output is cached in ``~/.moor/auth.json``. A single MITM during initial
    ...
    if host != "x.ai" and not host.endswith(".x.ai"):
        raise AuthError(
            ...
            f"discovery; re-authenticate with `moor model` to re-fetch.",
            provider="xai-oauth",
            code="xai_discovery_invalid",
        )
    return url
...
def _xai_validate_inference_base_url(value: str, *, fallback: str) -> str:
    ...
    the user's SuperGrok subscription. ``XAI_BASE_URL`` / ``MOOR_XAI_BASE_URL``
...
def refresh_xai_oauth_pure(
    ...
    *,
    ...
    if not isinstance(refresh_token, str) or not refresh_token.strip():
        raise AuthError(
            "xAI OAuth is missing refresh_token. Re-authenticate with `moor model`.",
            provider="xai-oauth",
            code="xai_auth_missing_refresh_token",
            relogin_required=True,
        )
    ...
    # written by an older Moor (or hand-edited) may carry a non-xAI
    ...
    # with a clear error so the user can re-run `moor model` to refetch.
    ...
    if response.status_code != 200:
        ...
        # on the allowlist for API access).  Re-running ``moor model``
...
def resolve_xai_oauth_runtime_credentials(
    *,
    ...
    tokens = dict(data["tokens"])
    access_token = str(tokens.get("access_token", "") or "").strip()
    refresh_timeout_seconds = env_float("MOOR_XAI_REFRESH_TIMEOUT_SECONDS", 20)
    ...
    base_url = _xai_validate_inference_base_url(
        os.getenv("MOOR_XAI_BASE_URL", "").strip().rstrip("/")
...
def _moor_device_auth_timeout_message(portal_base_url: str) -> str:
    """Actionable timeout text for Moor device-code login failures.
    ...
    """
    portal = (portal_base_url or DEFAULT_MOOR_PORTAL_URL).rstrip("/")
    return (
...
def _poll_for_token(
    ...
    # the CLI login (_moor_device_code_login) and the dashboard/desktop
    # poller (web_server._moor_poller, which surfaces str(e) to the UI).
    raise TimeoutError(_moor_device_auth_timeout_message(portal_base_url))


# =============================================================================
# Moor Portal — token refresh and model discovery
# =============================================================================

# -----------------------------------------------------------------------------
# Shared Moor token store — lets OAuth credentials persist across profiles
# so a new `moor --profile <name> auth add moor --type oauth` can one-tap
# import instead of running the full device-code flow every time.
#
# File lives at ${MOOR_SHARED_AUTH_DIR}/moor_auth.json, defaulting to
# ``<moor-root>/shared/moor_auth.json`` where ``<moor-root>`` is what
# ``get_default_moor_root()`` returns — ``~/.moor`` on Linux/macOS,
# ``%LOCALAPPDATA%\moor`` on native Windows, or the Docker/custom root.
# It is OUTSIDE any named profile's MOOR_HOME so named profiles (which
# typically live under ``<moor-root>/profiles/<name>/``) all see the
# same file.
#
...
MOOR_SHARED_STORE_FILENAME = "moor_auth.json"
_moor_shared_lock_holder = threading.local()


def _moor_shared_auth_dir() -> Path:
    """Resolve the directory that holds the shared Moor token store.

    Honors ``MOOR_SHARED_AUTH_DIR`` so tests can redirect it to a tmp
    path without touching the real user's home. Defaults to
    ``<moor-root>/shared/``, where ``<moor-root>`` is what
    :func:`moor_constants.get_default_moor_root` returns — so
    Linux/macOS classic installs land at ``~/.moor/shared/``, native
    Windows installs at ``%LOCALAPPDATA%\\moor\\shared\\``, and
    Docker / custom ``MOOR_HOME`` deployments at
    ``<MOOR_HOME>/shared/``. Sits outside any named profile so all
    profiles under the same root share the store.
    """
    override = os.getenv("MOOR_SHARED_AUTH_DIR", "").strip()
    if override:
        return Path(override).expanduser()
    from moor_constants import get_default_moor_root
    return get_default_moor_root() / "shared"


def _moor_shared_store_path() -> Path:
    path = _moor_shared_auth_dir() / MOOR_SHARED_STORE_FILENAME
    # Seat belt: if pytest is running and this resolves to a path under the
    # real user's Moor root, refuse rather than silently corrupt cross-profile
    # state. Tests must set MOOR_SHARED_AUTH_DIR to a tmp_path (conftest
    ...
    if os.environ.get("PYTEST_CURRENT_TEST"):
        from moor_constants import get_default_moor_root
        real_home_shared = (
            get_default_moor_root() / "shared" / MOOR_SHARED_STORE_FILENAME
        ...
        if resolved == real_home_shared:
            raise RuntimeError(
                f"Refusing to touch real user shared Moor auth store during test run: "
                f"{path}. Set MOOR_SHARED_AUTH_DIR to a tmp_path in your test fixture."
            )
    return path


@contextmanager
def _moor_shared_store_lock(timeout_seconds: float = AUTH_LOCK_TIMEOUT_SECONDS):
    """Cross-profile lock for the shared Moor OAuth store.
    ...
    ``_try_import_shared_moor_state``, which holds this lock alone for
    ...
    """
    try:
        lock_path = _moor_shared_store_path().with_suffix(".lock")
    except RuntimeError:
        # No MOOR_HOME yet (pre-setup): fall through without locking.
        yield
        return

    with _file_lock(
        lock_path,
        _moor_shared_lock_holder,
        timeout_seconds,
        "Timed out waiting for shared Moor auth lock",
    ):
        yield


def _merge_shared_moor_oauth_state(state: Dict[str, Any]) -> bool:
    """Copy fresher shared OAuth tokens into a profile-local Moor state."""
...
def _write_shared_moor_state(state: Dict[str, Any]) -> None:
    """Persist a minimal copy of the Moor OAuth state to the shared store.
    ...
    shared = {
        ...
        "scope": state.get("scope") or DEFAULT_MOOR_SCOPE,
        "client_id": state.get("client_id") or DEFAULT_MOOR_CLIENT_ID,
        "portal_base_url": state.get("portal_base_url") or DEFAULT_MOOR_PORTAL_URL,
        "inference_base_url": state.get("inference_base_url") or DEFAULT_MOOR_INFERENCE_URL,
    ...
    try:
        with _moor_shared_store_lock():
            ...
            # Create with 0o600 atomically via os.open(O_EXCL) — closes the TOCTOU
            # window where write_text() + post-write chmod briefly exposed Moor
            ...
            try:
                ...
                try:
                    ...
                    pass
        _oauth_trace(
            "moor_shared_store_written",
            path=str(path),
            refresh_token_fp=_token_fingerprint(refresh_token),
        )
    except Exception as exc:
        logger.debug("Failed to write shared Moor auth store: %s", exc)


def _read_shared_moor_state() -> Optional[Dict[str, Any]]:
    """Return the shared Moor OAuth state if present and well-formed.
    ...
    try:
        ...
        logger.debug("Shared Moor auth store at %s is unreadable: %s", path, exc)
        return None
    if not isinstance(payload, dict):
        return None
...
def _clear_shared_moor_state(reason: str) -> None:
    """Remove the shared Moor OAuth store after a terminal token failure."""
    try:
        with _moor_shared_store_lock():
            path = _moor_shared_store_path()
            try:
                ...
                pass
        _oauth_trace("moor_shared_store_cleared", reason=reason)
    except Exception as exc:
        logger.debug("Failed to clear shared Moor auth store: %s", exc)


def _is_terminal_moor_refresh_error(exc: Exception) -> bool:
    """True when retrying the same Moor refresh token cannot succeed."""
    return (
        isinstance(exc, AuthError)
        and exc.provider == "moor"
...
def _quarantine_moor_oauth_state(
    state: Dict[str, Any],
    error: AuthError,
    *,
    reason: str,
    ...
    # sizes/booleans. NEVER pass a raw token/agent_key into the log call — Moor
    # has a known bug class where credential-shaped literals get corrupted in logs.
    forensic: Dict[str, Any] = {
        "reason": reason,
        "error_code": error.code,
        # No session_id field exists on Moor state; provenance is client_id +
    ...
    logger.warning(
        "Moor OAuth state quarantined (terminal auth death): %s",
    ...
    invalidate_moor_auth_status_cache()


def _quarantine_moor_pool_entries(
    auth_store: Dict[str, Any],
    error: AuthError,
    *,
    reason: str,
) -> bool:
    """Remove singleton-seeded Moor pool entries that contain dead OAuth state."""
    pool = auth_store.get("credential_pool")
    if not isinstance(pool, dict):
        return False
    entries = pool.get("moor")
    if not isinstance(entries, list):
        return False

    retained = []
    removed = False
    singleton_sources = {MOOR_DEVICE_CODE_SOURCE, f"manual:{MOOR_DEVICE_CODE_SOURCE}"}
    for entry in entries:
    ...
    if removed:
        pool["moor"] = retained
        _oauth_trace(
            "moor_pool_device_code_quarantined",
            reason=reason,
            error_code=error.code,
        )
    return removed
...
def _try_import_shared_moor_state(
    *,
    timeout_seconds: float = 15.0,
) -> Optional[Dict[str, Any]]:
    """Attempt to rehydrate Moor OAuth state from the shared store.
    ...
    """
    try:
        with _moor_shared_store_lock(timeout_seconds=max(timeout_seconds + 5.0, AUTH_LOCK_TIMEOUT_SECONDS)):
            shared = _read_shared_moor_state()
            if not shared:
                return None

            # Build a full state dict so refresh_moor_oauth_from_state has every
            ...
            state: Dict[str, Any] = {
                ...
                "client_id": shared.get("client_id") or DEFAULT_MOOR_CLIENT_ID,
                "portal_base_url": shared.get("portal_base_url") or DEFAULT_MOOR_PORTAL_URL,
                "inference_base_url": shared.get("inference_base_url") or DEFAULT_MOOR_INFERENCE_URL,
                "token_type": shared.get("token_type") or "Bearer",
                "scope": shared.get("scope") or DEFAULT_MOOR_SCOPE,
            ...
            def _persist_shared_refresh(updated_state: Dict[str, Any], _reason: str) -> None:
                _write_shared_moor_state(updated_state)

            refreshed = refresh_moor_oauth_from_state(
                state,
                timeout_seconds=timeout_seconds,
                force_refresh=True,
                on_state_update=_persist_shared_refresh,
            )
            _write_shared_moor_state(refreshed)
    except AuthError as exc:
        _oauth_trace(
            "moor_shared_import_failed",
            error_type=type(exc).__name__,
            error_code=getattr(exc, "code", None),
        )
        if _is_terminal_moor_refresh_error(exc):
            _clear_shared_moor_state("shared_import_terminal_refresh_failure")
        logger.debug("Shared Moor import failed: %s", exc)
        return None
    except Exception as exc:
        _oauth_trace(
            "moor_shared_import_failed",
...
def _refresh_access_token(
    *,
    ...
    response = client.post(
        f"{portal_base_url}/api/oauth/token",
        headers={"x-moor-refresh-token": refresh_token},
        data={
            "grant_type": "refresh_token",
            "client_id": client_id,
        },
    ...
    if response.status_code == 200:
        ...
        if "access_token" not in payload:
            raise AuthError("Refresh response missing access_token",
                            provider="moor", code="invalid_token", relogin_required=True)
        return payload
    ...
    try:
        ...
        raise AuthError("Refresh token exchange failed",
                        provider="moor", relogin_required=True) from exc

    code = str(error_payload.get("error", "invalid_grant"))
    description = str(error_payload.get("error_description") or "Refresh token exchange failed")
    relogin = code in {"invalid_grant", "invalid_token", "refresh_token_reused"}

    # Detect the OAuth 2.1 "refresh token reuse" signal from the Moor portal
    ...
    # called POST /api/oauth/token with Moor's refresh_token without
    # persisting the rotated token back to auth.json — the server then
    # retires the original RT, Moor's next refresh uses it, and the whole
    ...
    if code == "refresh_token_reused" or "reuse" in lowered or "reuse detected" in lowered:
        description = (
            "Moor Portal detected refresh-token reuse and revoked this session.\n"
            "This usually means an external process (monitoring script, "
            "custom self-heal hook, or another Moor install sharing "
            "~/.moor/auth.json) called POST /api/oauth/token with Moor's "
            "refresh token without persisting the rotated token back.\n"
            "Moor refresh tokens are single-use — only Moor may call the "
            "refresh endpoint. For health checks, use `moor auth status` "
            "instead.\n"
            "Re-authenticate with: moor auth add moor"
        )
        relogin = True

    raise AuthError(description, provider="moor", code=code, relogin_required=relogin)


def fetch_moor_models(
    *,
    inference_base_url: str,
    api_key: str,
    timeout_seconds: float = 15.0,
    ...
    """Fetch available model IDs from the Moor inference API."""
    ...
    if response.status_code != 200:
        ...
        raise AuthError(description, provider="moor", code="models_fetch_failed")
    ...
    for item in data:
        ...
        if isinstance(model_id, str) and model_id.strip():
            mid = model_id.strip()
            # Skip Moor models — they're not reliable for agentic tool-calling
            if "moor" in mid.lower():
                continue
...
def _agent_key_is_usable(state: Dict[str, Any], min_ttl_seconds: int) -> bool:
    ...
    if not isinstance(key, str) or not key.strip():
        return False
    return _moor_invoke_jwt_is_usable(
        key,
        scope=state.get("scope"),
...
# Per-process memo for resolve_moor_access_token. Startup runs
        ...
        with _RESOLVE_TOKEN_CACHE_LOCK:
            if _RESOLVE_TOKEN_CACHE is not None:
                ...
                if (time.monotonic() - cached_at) < _RESOLVE_TOKEN_CACHE_TTL_S:
                    return cached_token
    with _provider_state_transaction("moor") as (
        auth_store,
        state,
        state_source_path,
    ):
        ...
        if not state:
            raise AuthError(
                "Moor is not logged into Moor Portal.",
                provider="moor",
                relogin_required=True,
            )

        # MOOR_PORTAL_BASE_URL / MOOR_PORTAL_BASE_URL is the trusted
        # operator/deployment override (mirrors MOOR_INFERENCE_BASE_URL) and
        ...
        # no override is set.
        env_portal_override = _moor_portal_env_override()
        if env_portal_override:
            portal_base_url = env_portal_override.rstrip("/")
        else:
            portal_base_url = (
                _optional_base_url(state.get("portal_base_url"))
                or DEFAULT_MOOR_PORTAL_URL
            ).rstrip("/")

            parsed_portal_url = urlparse(portal_base_url)
            if parsed_portal_url.hostname and parsed_portal_url.hostname not in _MOOR_PORTAL_ALLOWED_HOSTS:
                logger.warning(
        ...
        with _moor_shared_store_lock(timeout_seconds=max(timeout_seconds + 5.0, AUTH_LOCK_TIMEOUT_SECONDS)):
            ...
            if not isinstance(access_token, str) or not access_token:
                raise AuthError(
                    "No access token found for Moor Portal login.",
                    provider="moor",
                    relogin_required=True,
                )
            ...
            if not _is_expiring(state.get("expires_at"), refresh_skew_seconds):
                if merged_shared:
                    _save_provider_state_to_source(auth_store, "moor", state, state_source_path)
...
def refresh_moor_oauth_pure(
    ...
    *,
    ...
    """Refresh Moor OAuth state without mutating auth.json directly.
    ...
    state: Dict[str, Any] = {
        "access_token": access_token,
        "refresh_token": refresh_token,
        "client_id": client_id or DEFAULT_MOOR_CLIENT_ID,
        "portal_base_url": (portal_base_url or DEFAULT_MOOR_PORTAL_URL).rstrip("/"),
        "inference_base_url": (inference_base_url or DEFAULT_MOOR_INFERENCE_URL).rstrip("/"),
        "token_type": token_type or "Bearer",
        "scope": scope or DEFAULT_MOOR_SCOPE,
    ...
    with httpx.Client(timeout=timeout, headers={"Accept": "application/json"}, verify=verify) as client:
        current_invoke_jwt_status = _moor_invoke_jwt_status(
        ...
        if force_refresh or current_invoke_jwt_status is not None:
            ...
            if not isinstance(refresh_token_value, str) or not refresh_token_value:
                ...
                raise AuthError(
                    "No refresh token is available for Moor Portal.",
                    provider="moor",
                    relogin_required=True,
                )
            refreshed = _refresh_access_token(
            ...
            refreshed_url = _validate_moor_inference_url_from_network(refreshed.get("inference_base_url"))
            state["inference_base_url"] = refreshed_url or DEFAULT_MOOR_INFERENCE_URL
            state["obtained_at"] = now.isoformat()
            state["expires_in"] = access_ttl
            state["expires_at"] = datetime.fromtimestamp(
        ...
        _assert_moor_inference_jwt_usable(state)
        _select_moor_invoke_jwt(state)

    return state


def refresh_moor_oauth_from_state(
    state: Dict[str, Any],
    *,
    timeout_seconds: float = 15.0,
    force_refresh: bool = False,
    ...
    """Refresh Moor OAuth from a state dict. Thin wrapper around refresh_moor_oauth_pure."""
    tls = state.get("tls") or {}
    return refresh_moor_oauth_pure(
        state.get("access_token", ""),
        state.get("refresh_token", ""),
        state.get("client_id", "moor-cli"),
        state.get("portal_base_url", DEFAULT_MOOR_PORTAL_URL),
        state.get("inference_base_url", DEFAULT_MOOR_INFERENCE_URL),
        token_type=state.get("token_type", "Bearer"),
        scope=state.get("scope", DEFAULT_MOOR_SCOPE),
        obtained_at=state.get("obtained_at"),
        expires_at=state.get("expires_at"),
...
def persist_moor_credentials(
    creds: Dict[str, Any],
    *,
    label: Optional[str] = None,
):
    """Persist Moor OAuth credentials as the singleton provider state
    and ensure the credential pool is in sync.

    Moor credentials are read at runtime from two independent locations:

    - ``providers.moor``: singleton state read by
      ``resolve_moor_runtime_credentials()`` during 401 recovery and by
      ``_seed_from_singletons()`` during pool load.
    - ``credential_pool.moor``: used by the runtime ``pool.select()`` path.

    Historically ``moor auth add moor`` wrote a ``manual:device_code`` pool
    entry only, skipping ``providers.moor``. When the runtime credential
    expired, the recovery path read the empty singleton state and raised
    ``AuthError`` silently (``logger.debug`` at INFO level).

    This helper writes ``providers.moor`` then calls ``load_pool("moor")`` so
    ...
    ``moor auth add moor --label <name>``).  It gets embedded in the
    singleton state so that ``_seed_from_singletons`` uses it as the pool
    entry's label on every subsequent ``load_pool("moor")`` instead of the
    ...
    with _auth_store_lock():
        auth_store = _load_auth_store()
        _save_provider_state(auth_store, "moor", state)
        _save_auth_store(auth_store)

    # Mirror to the shared store so a new profile can one-tap import
    # these credentials via `moor auth add moor --type oauth`. Best-
    ...
    pool = load_pool("moor")
    return next(
        (e for e in pool.entries() if e.source == MOOR_DEVICE_CODE_SOURCE),
        None,
    )


def _sync_moor_pool_from_auth_store() -> None:
    """Best-effort pool reseed after providers.moor changes; never fail login."""
    try:
        from agent.credential_pool import load_pool

        load_pool("moor")
    except Exception as exc:
        logger.debug("Failed to sync Moor credential pool from auth store: %s", exc)


def resolve_moor_runtime_credentials(
    *,
    timeout_seconds: float = 15.0,
    insecure: Optional[bool] = None,
    ca_bundle: Optional[str] = None,
    ...
    """
    Resolve Moor inference credentials for runtime use.
    ...
    with _provider_state_transaction("moor") as (
        ...
        if not state:
            raise AuthError("Moor is not logged into Moor Portal.",
                            provider="moor", relogin_required=True)
        ...
        def _resolve_effective_routing_metadata() -> tuple[str, str, str, str]:
            ...
            portal_url = (
                _optional_base_url(state.get("portal_base_url"))
                or os.getenv("MOOR_PORTAL_BASE_URL")
                or os.getenv("MOOR_PORTAL_BASE_URL")
                or DEFAULT_MOOR_PORTAL_URL
            ).rstrip("/")
            ...
            if env_portal_override:
                ...
                if (
                    not portal_host
                    or portal_host not in _MOOR_PORTAL_ALLOWED_HOSTS
            ...
            effective_inference_url = (
                _moor_inference_env_override() or stored_inference_url
            )
            effective_client_id = str(
                state.get("client_id") or DEFAULT_MOOR_CLIENT_ID
            )
            return (
                portal_url,
                stored_inference_url,
        ...
        def _persist_state(reason: str) -> None:
            ...
            # the mtime-keyed Moor auth-status cache warm during read paths.
            if (
                _moor_effective_provider_state(state)
                == _moor_effective_provider_state(persisted_state)
            ):
                _oauth_trace(
                    "moor_state_persist_skipped",
                    sequence_id=sequence_id,
                    reason=reason,
                )
                return
        ...
        timeout = httpx.Timeout(timeout_seconds if timeout_seconds else 15.0)
        _oauth_trace(
            "moor_runtime_credentials_start",
            sequence_id=sequence_id,
            refresh_token_fp=_token_fingerprint(state.get("refresh_token")),
        )
        ...
        with httpx.Client(timeout=timeout, headers={"Accept": "application/json"}, verify=verify) as client:
            ...
            if not isinstance(access_token, str) or not access_token:
                with _moor_shared_store_lock(
                    ...
                    if _merge_shared_moor_oauth_state(state):
                        access_token = state.get("access_token")
                        refresh_token = state.get("refresh_token")
                        (
                            portal_base_url,
            ...
            if not isinstance(access_token, str) or not access_token:
                raise AuthError("No access token found for Moor Portal login.",
                                provider="moor", relogin_required=True)

            invoke_jwt_status = _moor_invoke_jwt_status(
                access_token,
                scope=state.get("scope"),
                expires_at=state.get("expires_at"),
            )
            if force_refresh or invoke_jwt_status is not None:
                with _moor_shared_store_lock(timeout_seconds=max(timeout_seconds + 5.0, AUTH_LOCK_TIMEOUT_SECONDS)):
                    ...
                    if force_refresh or invoke_jwt_status is not None:
                        ...
                        state["token_type"] = refreshed.get("token_type") or state.get("token_type") or "Bearer"
                        state["scope"] = refreshed.get("scope") or state.get("scope")
                        # Heal a poisoned stored value (see refresh_moor_oauth_pure):
                        ...
                        # persisted to auth.json below. The MOOR_INFERENCE_BASE_URL
                        # env override is layered on for the client/return value
                        # only (see below) — it is never persisted.
                        refreshed_url = _validate_moor_inference_url_from_network(refreshed.get("inference_base_url"))
                        stored_inference_base_url = refreshed_url or DEFAULT_MOOR_INFERENCE_URL
                        inference_base_url = (
                            _moor_inference_env_override() or stored_inference_base_url
                        )
            ...
            _assert_moor_inference_jwt_usable(
                state,
                access_token=access_token,
            )
            _select_moor_invoke_jwt(
                state,
                access_token=access_token,
                sequence_id=sequence_id,
            )
        ...
        _persist_state("resolve_moor_runtime_credentials_final")
    ...
    if not isinstance(api_key, str) or not api_key:
        raise AuthError("Failed to resolve a Moor inference API key",
                        provider="moor", code="server_error")
    ...
    return {
        ...
        "expires_at": expires_at,
        "expires_in": expires_in,
        "source": MOOR_AUTH_PATH_INVOKE_JWT,
        ...
        "auth_path": MOOR_AUTH_PATH_INVOKE_JWT,
...
def _empty_moor_auth_status() -> Dict[str, Any]:
    return {
        "logged_in": False,
...
def _snapshot_moor_pool_status() -> Dict[str, Any]:
    ...
    try:
        ...
        if not pool or not pool.has_credentials():
            return _empty_moor_auth_status()
...
# ── Process-level memo for get_moor_auth_status() ──
# get_moor_auth_status() validates state by calling resolve_moor_runtime_credentials(),
# which does a synchronous OAuth refresh POST to portal.nousresearch.com. That can take
# ~350ms even on the failure path, and read-only UI surfaces (`moor tools`, status panels,
# subscription-feature checks) call it many times per render — `moor tools` → "All Platforms"
...
# `moor auth login/logout/add/remove` invalidate naturally on the next call.
_MOOR_AUTH_STATUS_CACHE_TTL = 15.0  # seconds
_moor_auth_status_cache: Optional[Tuple[float, str, Optional[float], Dict[str, Any]]] = None

# mtime-keyed memo for _load_global_auth_store(): (path, mtime_ns, store).
# Same invalidation contract as _moor_auth_status_cache — the global auth
...
def invalidate_moor_auth_status_cache() -> None:
    """Clear the get_moor_auth_status() process-level memo.

    Call this from any code path that mutates Moor auth state without going
    through resolve_moor_runtime_credentials() (e.g. tests). Login/logout
    ...
    """
    global _moor_auth_status_cache
    _moor_auth_status_cache = None


def get_moor_auth_status() -> Dict[str, Any]:
    """Status snapshot for Moor auth.
    ...
    ``invalidate_moor_auth_status_cache()`` explicitly.
    """
    global _moor_auth_status_cache
    now = time.monotonic()
    auth_file_key, mtime = _auth_file_cache_key()
    cached = _moor_auth_status_cache
    if cached is not None:
        ...
        if (
            ...
            and (now - cached_at) < _MOOR_AUTH_STATUS_CACHE_TTL
        ):
            return dict(cached_status)

    status = _compute_moor_auth_status()
    _moor_auth_status_cache = (now, auth_file_key, mtime, dict(status))
    return status


def _compute_moor_auth_status() -> Dict[str, Any]:
    """Uncached implementation of get_moor_auth_status(). See that function."""
    state = get_provider_auth_state("moor")
    if state:
        base_status = {
        ...
        try:
            ...
            refreshed_state = get_provider_auth_state("moor") or state
            base_status.update(
                {
                    "logged_in": True,
    ...
    return _snapshot_moor_pool_status()


def get_moor_auth_status_local() -> Dict[str, Any]:
    """Refresh-free Moor auth snapshot for read-only display surfaces.

    Unlike :func:`get_moor_auth_status`, this NEVER calls
    ``resolve_moor_runtime_credentials()`` and therefore never performs an
    ...
    need a live credential) should keep using ``get_moor_auth_status()``.
    ...
    access_token = state.get("access_token")
    jwt_reason = _moor_invoke_jwt_status(
        access_token,
        scope=state.get("scope"),
        expires_at=state.get("expires_at"),
    )
...
# Enum values reported on the dashboard /api/status as ``moor_session_valid``.
...
MOOR_SESSION_VALID = "valid"
MOOR_SESSION_TERMINAL = "terminal"
MOOR_SESSION_UNKNOWN = "unknown"


def get_moor_session_validity() -> str:
    """Classify the Moor bootstrap session for the dashboard /api/status probe.

    Returns one of:
      - ``"valid"``    — a usable Moor credential is present (login healthy).
      - ``"terminal"`` — the Moor session has taken a terminal auth failure
      ...
      - ``"unknown"``  — indeterminate (no Moor provider state, or a transient/
    ...
    # into the Moor provider state when it clears dead tokens (the exact path
    ...
    if isinstance(last_err, dict) and last_err.get("relogin_required"):
        ...
        if not (state.get("access_token") or state.get("refresh_token")):
            return MOOR_SESSION_TERMINAL

    if _moor_invoke_jwt_status(
        state.get("access_token"),
        scope=state.get("scope"),
        expires_at=state.get("expires_at"),
    ) is None:
        return MOOR_SESSION_VALID
...
def get_codex_auth_status() -> Dict[str, Any]:
    ...
    Checks the credential pool first (where `moor auth` stores credentials),
    then falls back to the legacy provider state.
    """
    # Check credential pool first — this is where `moor auth` and
    # `moor model` store device_code tokens.
    try:
        from agent.credential_pool import load_pool
...
def get_api_key_provider_status(provider_id: str) -> Dict[str, Any]:
    ...
    # the MoorOverlay keyless flag — the same source the provider catalog
    # and GUI contract tests use.
    try:
        from moor_cli.providers import MOOR_OVERLAYS
        _overlay = MOOR_OVERLAYS.get(provider_id)
    except Exception:
        _overlay = None
...
def get_external_process_provider_status(provider_id: str) -> Dict[str, Any]:
    ...
    command = (
        os.getenv("MOOR_COPILOT_ACP_COMMAND", "").strip()
        or os.getenv("COPILOT_CLI_PATH", "").strip()
        or "copilot"
    )
    raw_args = os.getenv("MOOR_COPILOT_ACP_ARGS", "").strip()
...
def _get_azure_foundry_auth_status() -> Dict[str, Any]:
    ...
    checks:
      ...
      * ``auth_mode == "entra_id"`` AND ``azure-identity`` is importable
        (we do NOT mint a token here; ``moor doctor`` runs the live
    ...
    try:
        from moor_cli.config import load_config, get_env_value_prefer_dotenv
        cfg = load_config()
    except Exception:
        cfg = {}
        ...
        try:
            ...
            if not installed:
                info["hint"] = (
                    ...
                    "pip install azure-identity  (or rely on Moor' "
...
def resolve_api_key_provider_credentials(provider_id: str) -> Dict[str, Any]:
    ...
    if provider_id in {"kimi-coding", "kimi-coding-cn"}:
        ...
        try:
            from moor_cli.copilot_auth import (
...
def resolve_external_process_provider_credentials(provider_id: str) -> Dict[str, Any]:
    ...
    if not resolved_command and not base_url.startswith("acp+tcp://"):
        raise AuthError(
            ...
            "Install GitHub Copilot CLI or set MOOR_COPILOT_ACP_COMMAND/COPILOT_CLI_PATH.",
            provider=provider_id,
...
def _update_config_for_provider(
    ...
    # model.api_key.
    from moor_cli.config import clear_model_endpoint_credentials
...
def _logout_default_provider_from_config() -> Optional[str]:
    ...
    `moor logout` historically keyed off auth.json.active_provider only.
    ...
    if provider in {"moor", "openai-codex", "xai-oauth"}:
        return provider
    return None
...
def _confirm_selection_guards(
    ...
    *,
    ...
    :mod:`moor_cli.model_selection_guards` and shows one [y/N] confirm with
...
def _prompt_model_selection(
    ...
    # Sale chrome (★ / -N% / was) is Moor Portal-only — never for OpenRouter
    # or other providers even if pricing.original is somehow present.
    sale_chrome = (confirm_provider or "").strip().lower() == "moor"

    def _confirmed_selection(mid: str) -> Optional[str]:
        if not mid:
            return None
        # Unified guard registry (moor_cli.model_selection_guards): the cost
    ...
    # Leave room for a leading "★ " on sale rows (Moor only).
    ...
    try:
        ...
        _upgrade_url = (portal_url or DEFAULT_MOOR_PORTAL_URL).rstrip("/")
        ...
        from moor_cli.model_search import model_search_text
    ...
    from moor_cli.curses_ui import format_radio_item_ansi
...
def _save_model_choice(model_id: str) -> None:
    ...
    """
    from moor_cli.config import save_config, load_config
...
def login_command(args) -> None:
    """Deprecated: use 'moor model' or 'moor setup' instead."""
    print("The 'moor login' command has been removed.")
    print("Use 'moor auth' to manage credentials,")
    print("'moor model' to select a provider, or 'moor setup' for full setup.")
    raise SystemExit(0)
...
def _login_openai_codex(
    ...
    *,
    ...
    """OpenAI Codex login via device code flow. Tokens stored in ~/.moor/auth.json."""

    del args, pconfig  # kept for parity with other provider login helpers

    # Check for existing moor-owned credentials
    if not force_new_login:
        try:
            ...
            if isinstance(_resolved_key, str) and _resolved_key and not _codex_access_token_is_expiring(_resolved_key, 60):
                print("Existing Codex credentials found in Moor auth store.")
                try:
    ...
    if not force_new_login:
        ...
        if cli_tokens:
            ...
            print("Moor will create its own session to avoid conflicts with Codex CLI / VS Code.")
            try:
            ...
            if do_import in {"y", "yes"}:
                ...
                base_url = os.getenv("MOOR_CODEX_BASE_URL", "").strip().rstrip("/") or DEFAULT_CODEX_BASE_URL
                config_path = _update_config_for_provider("openai-codex", base_url)
                print()
                print("Credentials imported. Note: if Codex CLI refreshes its token,")
                print("Moor will keep working independently with its own session.")
                print(f"  Config updated: {config_path} (model.provider=openai-codex)")
                return

    # Run a fresh device code flow — Moor gets its own OAuth session
    print()
    ...
    # Save tokens to Moor auth store
...
def _login_xai_oauth(
    ...
    *,
    ...
    if not force_new_login:
        try:
            ...
            if isinstance(api_key, str) and api_key and not _xai_access_token_is_expiring(api_key, 60):
                print("Existing xAI OAuth credentials found in Moor auth store.")
                try:
                    reuse = input("Use existing credentials? [Y/n]: ").strip().lower()
                except (EOFError, KeyboardInterrupt):
                    reuse = "y"
    ...
    print("(Moor creates its own local OAuth session)")
    print()
    ...
    # xAI credential re-enabled. ``moor auth remove xai-oauth`` leaves a
    # ``device_code`` suppression marker that otherwise stops the singleton
    # seed from re-creating the pool entry, so ``moor auth list`` would show
...
def _minimax_save_auth_state(auth_state: Dict[str, Any]) -> None:
    """Persist MiniMax OAuth state to Moor auth store (~/.moor/auth.json)."""
    with _auth_store_lock():
        auth_store = _load_auth_store()
...
def _minimax_oauth_login(
    *, region: str = "global", open_browser: bool = True,
    ...
    print(f"Starting Moor login via MiniMax ({region}) OAuth...")
...
def _minimax_oauth_quarantine_on_terminal_refresh(state: Dict[str, Any], exc: AuthError) -> None:
    ...
    provider. Mirrors the Moor / xAI-OAuth / Codex-OAuth quarantine pattern
    so subsequent calls fail fast without a network retry.
    """
...
def build_minimax_oauth_token_provider() -> Callable[[], str]:
    ...
    def _provide() -> str:
        ...
        if not state or not state.get("access_token"):
            raise AuthError(
                "Not logged into MiniMax OAuth. Run `moor model` and select "
                "MiniMax (OAuth).",
...
def resolve_minimax_oauth_runtime_credentials(
    *, min_token_ttl_seconds: int = MINIMAX_OAUTH_REFRESH_SKEW_SECONDS,
    ...
    diagnostic call sites like ``moor status`` that just want to know
...
def _moor_device_code_login(
    *,
    ...
    """Run the Moor device-code flow and return full OAuth state without persisting."""
    ...
    requested_inference_url = (
        inference_base_url
        or os.getenv("MOOR_INFERENCE_BASE_URL")
    ...
    print(f"Starting Moor login via {pconfig.name}...")
    print(f"Portal: {portal_base_url}")
    if insecure:
    ...
    try:
        return refresh_moor_oauth_from_state(
            auth_state,
            timeout_seconds=timeout_seconds,
            force_refresh=False,
        )
        ...
        if exc.code == "subscription_required":
            ...
            print()
            print("After subscribing, run `moor model` again to finish setup.")
            raise SystemExit(1)
        raise


def moor_token_has_billing_scope() -> bool:
    """Return True if the currently-held Moor token carries ``billing:manage``.
    ...
    if not isinstance(scope, str):
        return False
    return MOOR_BILLING_MANAGE_SCOPE in scope.split()


def step_up_moor_billing_scope(
    *,
    open_browser: bool = True,
    timeout_seconds: float = 15.0,
    on_verification: Optional[Callable[[str, str], None]] = None,
    ...
    targets the same deployment (incl. a preview via ``MOOR_PORTAL_BASE_URL`` set
    at the original login). Persists to the auth store + shared store + pool, exactly
    like ``_login_moor`` — but WITHOUT the model picker (this is a scope upgrade, not
    a fresh login).
    ...
    requested: list[str] = []
    for tok in (prior_scope.split() or [MOOR_INFERENCE_INVOKE_SCOPE, "tool:invoke"]):
        if tok and tok not in requested:
            requested.append(tok)
    if MOOR_BILLING_MANAGE_SCOPE not in requested:
        requested.append(MOOR_BILLING_MANAGE_SCOPE)
    ...
    with _auth_store_lock():
        auth_store = _load_auth_store()
        _save_provider_state(auth_store, "moor", auth_state)
        _save_auth_store(auth_store)

    # Mirror to shared store + reseed the pool (best-effort), same as _login_moor.
    try:
        _write_shared_moor_state(auth_state)
    except Exception:
        pass
    try:
        _sync_moor_pool_from_auth_store()
    ...
    return isinstance(granted, str) and MOOR_BILLING_MANAGE_SCOPE in granted.split()


def _login_moor(args, pconfig: ProviderConfig) -> None:
    """Moor Portal device authorization flow."""
    ...
    try:
        ...
        # flow, check the shared store for an existing Moor credential
        ...
        if shared:
            ...
            print()
            if shared_path:
                print(f"Found existing Moor OAuth credentials at {shared_path}")
            else:
                print("Found existing shared Moor OAuth credentials")
            try:
                do_import = input("Import these credentials? [Y/n]: ").strip().lower()
            except (EOFError, KeyboardInterrupt):
                do_import = "y"
            if do_import in {"", "y", "yes"}:
                print("Rehydrating Moor session from shared credentials...")
                auth_state = _try_import_shared_moor_state(
                    timeout_seconds=timeout_seconds,
                )
        ...
        # overwrites it to "moor".  If the user picks "Skip (keep current)"
        ...
        # leave the config in a half-updated state (provider=moor but model
        ...
        try:
            ...
            from moor_cli.models import (
                get_curated_moor_model_ids, get_pricing_for_provider,
                check_moor_free_tier, partition_moor_models_by_tier,
            ...
            unavailable_message = ""
            if model_ids:
                pricing = get_pricing_for_provider("moor")
                ...
                if free_tier:
                    try:
                        ...
                        _account_info = get_moor_portal_account_info(force_fresh=True)
                        unavailable_message = (
                            format_moor_portal_entitlement_message(
                                _account_info,
                                capability="paid Moor models",
                            )
                            or ""
                        )
                    except Exception:
                    ...
                    # as free so users on older Moor builds still see
                    # newly-launched free models without a CLI release.
                    model_ids, pricing = union_with_portal_free_recommendations(
                        model_ids, pricing, _portal_for_recs,
                    )
                    model_ids, unavailable_models = partition_moor_models_by_tier(
            ...
            if model_ids:
                ...
                selected_model = _prompt_model_selection(
                    ...
                    confirm_provider="moor",
                    confirm_base_url=inference_base_url,
                    confirm_api_key=runtime_key,
                )
            elif unavailable_models:
                _url = (_portal or DEFAULT_MOOR_PORTAL_URL).rstrip("/")
        ...
        # them to Moor with a mismatched model.  The Moor OAuth tokens
        # stay saved for future use.
        if not selected_model:
            # Restore the prior active_provider that _save_provider_state
            # overwrote to "moor".  config.yaml model.provider is left
            ...
            with _auth_store_lock():
                ...
                _save_auth_store(auth_store)
            print()
            print("No provider change. Moor credentials saved for future use.")
            print("  Run `moor model` again to switch to Moor Portal.")
            return

        config_path = _update_config_for_provider(
            "moor", inference_base_url, default_model=selected_model,
        )
        if selected_model:
            _save_model_choice(selected_model)
            print(f"Default model set to: {selected_model}")
        print(f"  Config updated: {config_path} (model.provider=moor)")
...
def logout_command(args) -> None:
    ...
    if clear_provider_auth(target) or should_reset_config:
        ...
        if should_reset_config and os.getenv("OPENROUTER_API_KEY"):
            print("Moor will use OpenRouter for inference.")
        elif should_reset_config:
            print("Run `moor model` or configure an API key to use Moor.")
        else:
