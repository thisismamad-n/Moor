...
- The auth store (``~/.moor/auth.json``) holds per-provider state, the credential pool and
...
- ``OAUTH_PROVIDER_FLOWS`` maps each OAuth provider to its resolver/status builder; the flows live in
  ``auth_moor``/``auth_codex``/``auth_xai``/``auth_qwen``/``auth_minimax``/``auth_spotify`` and are
...
from moor_cli.auth_device_flow import (  # noqa: F401  re-exported
    _can_open_graphical_browser, _default_verify, _is_remote_session,
    _moor_device_auth_timeout_message, _offer_existing_oauth_credentials,
...
from moor_cli.auth_oauth_grants import (  # noqa: F401  re-exported
...
from moor_cli.auth_moor import (  # noqa: F401  re-exported
    MOOR_SESSION_TERMINAL, MOOR_SESSION_UNKNOWN, MOOR_SESSION_VALID, _ALLOWED_MOOR_INFERENCE_HOSTS,
    _agent_key_is_usable, _apply_moor_refreshed_tokens, _assert_moor_inference_jwt_usable,
    _compute_moor_auth_status, _format_moor_entitlement_auth_error, _healed_moor_inference_url,
    _login_moor, _merge_shared_moor_oauth_state, _migrate_stale_moor_portal_url,
    _moor_device_code_login, _moor_inference_env_override, _moor_invoke_jwt_is_usable,
    _moor_invoke_jwt_status, _moor_portal_env_override, _moor_shared_store_lock,
    _moor_shared_store_path, _pool_first_oauth_status, _quarantine_moor_oauth_state,
    _quarantine_moor_pool_entries, _read_shared_moor_state, _refresh_access_token,
    _refresh_moor_or_quarantine, _select_moor_invoke_jwt, _sync_moor_pool_from_auth_store,
    _token_fingerprint, _try_import_shared_moor_state, _validate_moor_inference_url_from_network,
    _write_shared_moor_state, fetch_moor_models, get_moor_auth_status_local,
    get_moor_session_validity, persist_moor_credentials, refresh_moor_oauth_from_state,
    resolve_moor_runtime_credentials, step_up_moor_billing_scope)
from moor_cli.auth_minimax import (  # noqa: F401  re-exported
...
from moor_cli.auth_xai import (  # noqa: F401  re-exported
...
from moor_cli.auth_codex import (  # noqa: F401  re-exported
...
from moor_cli.auth_constants import (  # noqa: F401  re-exported
    _decode_jwt_claims, AUTH_STORE_VERSION, AUTH_LOCK_TIMEOUT_SECONDS, DEFAULT_MOOR_PORTAL_URL,
    DEFAULT_MOOR_INFERENCE_URL, DEFAULT_MOOR_CLIENT_ID, MOOR_BILLING_MANAGE_SCOPE,
    DEFAULT_MOOR_SCOPE, MOOR_DEVICE_CODE_SOURCE, MOOR_AUTH_PATH_INVOKE_JWT,
    ACCESS_TOKEN_REFRESH_SKEW_SECONDS, MOOR_INVOKE_JWT_MIN_TTL_SECONDS, DEFAULT_CODEX_BASE_URL,
    ...
    ACTUAL_LOCAL_NOAUTH_PLACEHOLDER, CODEX_RATE_LIMITED_CODE, AuthError, _moor_err, httpx)
...
_REGISTRY_ROWS: Tuple[Any, ...] = (
    ProviderConfig(
        "moor", "Moor Portal", "oauth_device_code", portal_base_url=DEFAULT_MOOR_PORTAL_URL,
        inference_base_url=DEFAULT_MOOR_INFERENCE_URL, client_id=DEFAULT_MOOR_CLIENT_ID,
    ...
    # is a 401), so there is no secret to configure. Select via `moor model` / `/model free`.
...
def get_anthropic_key() -> str:
    ...
    Checks both the ``.env`` file and the process environment, preferring ``~/.moor/.env`` so a deliberate
...
def is_rate_limited_auth_error(error: Exception) -> bool:
    ...
    re-authenticating cannot fix it, so callers should say "retry later", not ``moor auth``."""
...
def _auth_file_path() -> Path:
    ...
    # monkeypatch MOOR_HOME or escaped the hermetic conftest). In production: one dict lookup.
    if (os.environ.get("PYTEST_CURRENT_TEST")
            and _same_path(path, Path.home() / ".moor" / "auth.json")):
        raise RuntimeError(
...
def _global_auth_file_path() -> Optional[Path]:
    ...
    try:
        ...
        return None
    return None if _same_path(get_moor_home(), global_root) else global_root / "auth.json"
...
def _load_global_auth_store() -> Dict[str, Any]:
    ...
    if os.environ.get("PYTEST_CURRENT_TEST") and os.environ.get("HOME"):
        real_root = Path(os.environ["HOME"]) / ".moor" / "auth.json"
        try:
...
def _file_lock(
    ...
    shared Moor store) track reentrancy separately."""
...
def _auth_store_lock(
    ...
    ``_moor_shared_store_lock`` SECOND (inner), else deadlock against a concurrent shared import."""
...
def _load_auth_store(auth_file: Optional[Path] = None) -> Dict[str, Any]:
    ...
    if isinstance(raw, dict) and isinstance(raw.get("systems"), dict):  # legacy "systems" format
        systems = raw["systems"]
        providers = {"moor": systems["moor_portal"]} if "moor_portal" in systems else {}
...
def _save_auth_store(auth_store: Dict[str, Any], target_path: Optional[Path] = None) -> Path:
    ...
    # enforced); ignore failures. secure_parent_dir refuses to chmod /, top-level dirs, or the moor-agent
...
def mark_provider_active_if_unset(provider_id: str) -> None:
    """Set ``active_provider`` only when none is set yet: the first ``moor auth add`` credential must
...
def read_credential_pool(provider_id: Optional[str] = None) -> Dict[str, Any]:
    ...
    when the profile has zero entries for it (``moor auth add`` in the profile shadows global)."""
...
def write_credential_pool(
    ...
    *status_cleared_ids* were cleared deliberately (``moor auth reset``) and skip the
...
def _explicit_pool_entry_present(normalized: str) -> bool:
    """Pool rows from EXPLICIT Moor flows (manual add / device-code / PKCE) or live env keys;
...
# Set by Claude Code itself, not by the user explicitly configuring anthropic in Moor.
_IMPLICIT_ENV_VARS = frozenset({"CLAUDE_CODE_OAUTH_TOKEN"})
_EXPLICIT_POOL_SOURCES = frozenset({"device_code", "loopback_pkce", "moor_pkce", "manual"})
...
def _pool_entry_is_explicit(entry: Any) -> bool:
    """True for pool rows the user created via an explicit Moor flow (or a still-live env key)."""
...
def _keyless_provider_has_explicit_config(normalized: str) -> bool:
    """Vertex / Bedrock count as explicit when moor-scoped routing config is present.
    ...
    moor-scoped signals (VERTEX_PROJECT_ID / vertex.project_id / VERTEX_CREDENTIALS_PATH) count
    here."""
...
def is_provider_explicitly_configured(provider_id: str) -> bool:
    ...
    moor-initiated flow, or moor-scoped routing config for keyless cloud-SDK providers. Ambient
...
def clear_provider_auth(provider_id: Optional[str] = None) -> bool:
    ...
    ``moor logout``. Returns True if something was cleared."""
    with _auth_store_lock():
        auth_store = _load_auth_store()
...
def _refuse_env_adoption_if_config_corrupt() -> None:
    ...
    try:
        from moor_cli.config import get_active_config_parse_failure
    ...
    raise AuthError(
        ...
        f"was saved next to it) or run moor setup.",
        code="corrupt_config")
...
def _openrouter_auto_detected(scoped_key_env: Callable[[str], str]) -> bool:
    ...
    `moor auth add openrouter` has no env var; without the pool check it is invisible to
    ...
    if any(has_usable_secret(scoped_key_env(v)) for v in ("OPENAI_API_KEY", "OPENROUTER_API_KEY")):
        return True
    try:
        # Auto-detect an OpenRouter credential added via `moor auth add openrouter` (manual pool entry, no
        # env var). Without this, a key that only lives in the credential pool is invisible to
        # auto-detection — the user sees `moor auth list` showing the credential while requests go out
...
def _env_key_auto_detected(
    ...
    preempts a logged-in OAuth provider so a stale key in ~/.moor/.env never switches silently."""
            ...
            if has_usable_secret(scoped_key_env(env_var)):
                if oauth_active and oauth_active != pid:
                    logger.warning(
                        ...
                        # ~/.moor/.env isn't silently switched without knowing why.
...
def resolve_provider(
    ...
    *,
    ...
    if normalized != "auto":
        ...
        tail = (f"\n\n{hint}" if hint else " Check 'moor model' for available providers, "
                "or run 'moor doctor' to diagnose config issues.")
...
def _quarantine_flat_oauth_state(state: Dict[str, Any], provider: str, exc: "AuthError") -> None:
    ...
    subsequent calls fail fast without a network retry (mirrors the Moor / xAI / Codex pattern)."""
...
# Valid Moor Portal hosts; a stored portal_base_url outside this set is a misconfiguration and falls
...
def _moor_portal_base_url(state: Dict[str, Any]) -> str:
    """MOOR_PORTAL_BASE_URL / MOOR_PORTAL_BASE_URL is the trusted operator override and wins
    ...
    with _provider_state_transaction("moor") as (auth_store, state, state_source_path):
        ...
        with _moor_shared_store_lock(timeout_seconds=lock_timeout):
            ...
            if not isinstance(refresh_token, str) or not refresh_token:
                raise _moor_err("Session expired and no refresh token is available.", relogin=True)
            ...
            _apply_moor_refreshed_tokens(state, refreshed, refresh_token)
...
# Process-level memo for get_moor_auth_status(): it validates via a synchronous refresh POST
...
def invalidate_moor_auth_status_cache() -> None:
    """Clear the get_moor_auth_status() memo (for code paths that mutate Moor auth state without
...
def get_moor_auth_status() -> Dict[str, Any]:
    """Status snapshot for Moor auth, memoised ~15s keyed on the auth.json mtime.
    ...
    if (cached is not None and cached[1:3] == (auth_file_key, mtime)
            and (now - cached[0]) < _MOOR_AUTH_STATUS_CACHE_TTL):
...
class OAuthProviderFlow:
    ...
    ``monkeypatch.setattr("moor_cli.auth.resolve_codex_runtime_credentials", ...)`` applies."""
    provider_id: str
    resolve_fn: str
    status_fn: str
    terminal_refresh_codes: FrozenSet[str] = frozenset()  # retrying the same refresh token cannot succeed
    # ``moor logout`` with no active provider falls back to config.yaml ``model.provider`` only
...
_is_terminal_moor_refresh_error = partial(_is_terminal_refresh_error, provider="moor")
...
def _external_process_auth_evidence(provider_id: str) -> tuple[bool, Optional[str]]:
    ...
    Moor can't read). Deliberately subprocess-free: spawning ``gh auth token`` from status
    ...
    try:
        from moor_cli.copilot_auth import COPILOT_ENV_VARS, validate_copilot_token
...
def _external_process_spec(
    ...
    Launch details come from the provider's own profile (copilot-acp: MOOR_COPILOT_ACP_COMMAND /
    COPILOT_CLI_PATH / MOOR_COPILOT_ACP_ARGS), so out-of-tree providers describe their binary."""
...
def get_auth_status(provider_id: Optional[str] = None) -> Dict[str, Any]:
    ...
    tests that patch ``moor_cli.auth.get_*_auth_status`` still apply."""
...
def _get_azure_foundry_auth_status() -> Dict[str, Any]:
    ...
    CLI startup flat; ``moor doctor`` runs the live probe). ``api_key`` (default): usable
...
def login_command(args) -> None:
    ...
    print("The 'moor login' command has been removed.\nUse 'moor auth' to manage credentials,\n"
          "'moor model' to select a provider, or 'moor setup' for full setup.")
    raise SystemExit(0)
...
_PLUGIN_COMPAT_LAZY = {
    'CODEX_OAUTH_USER_AGENT': ('moor_cli.auth_constants', 'CODEX_OAUTH_USER_AGENT'),
    'CODEX_QUOTA_PROBE_MIN_INTERVAL_SECONDS': ('moor_cli.auth_codex', 'CODEX_QUOTA_PROBE_MIN_INTERVAL_SECONDS'),
    'DEFAULT_SPOTIFY_REDIRECT_URI': ('moor_cli.auth_constants', 'DEFAULT_SPOTIFY_REDIRECT_URI'),
    'DEVICE_AUTH_POLL_INTERVAL_CAP_SECONDS': ('moor_cli.auth_constants', 'DEVICE_AUTH_POLL_INTERVAL_CAP_SECONDS'),
    'MINIMAX_OAUTH_GRANT_TYPE': ('moor_cli.auth_constants', 'MINIMAX_OAUTH_GRANT_TYPE'),
    'MOOR_INFERENCE_INVOKE_SCOPE': ('moor_cli.auth_constants', 'MOOR_INFERENCE_INVOKE_SCOPE'),
    'MOOR_SHARED_STORE_FILENAME': ('moor_cli.auth_moor', 'MOOR_SHARED_STORE_FILENAME'),
    'OAUTH_OVER_SSH_DOCS_URL': ('moor_cli.auth_constants', 'OAUTH_OVER_SSH_DOCS_URL'),
    'QWEN_OAUTH_CLIENT_ID': ('moor_cli.auth_constants', 'QWEN_OAUTH_CLIENT_ID'),
    'QWEN_OAUTH_TOKEN_URL': ('moor_cli.auth_constants', 'QWEN_OAUTH_TOKEN_URL'),
    'SINGLE_USE_OAUTH_SINGLETON_FILES': ('moor_cli.auth_oauth_grants', 'SINGLE_USE_OAUTH_SINGLETON_FILES'),
    'SPOTIFY_ACCESS_TOKEN_REFRESH_SKEW_SECONDS': ('moor_cli.auth_constants', 'SPOTIFY_ACCESS_TOKEN_REFRESH_SKEW_SECONDS'),
    'SPOTIFY_DASHBOARD_URL': ('moor_cli.auth_constants', 'SPOTIFY_DASHBOARD_URL'),
    'XAI_OAUTH_DEVICE_CODE_URL': ('moor_cli.auth_constants', 'XAI_OAUTH_DEVICE_CODE_URL'),
    'XAI_OAUTH_DISCOVERY_URL': ('moor_cli.auth_constants', 'XAI_OAUTH_DISCOVERY_URL'),
    'XAI_OAUTH_ISSUER': ('moor_cli.auth_constants', 'XAI_OAUTH_ISSUER'),
    'refresh_moor_oauth_pure': ('moor_cli.auth_moor', 'refresh_moor_oauth_pure'),
}
