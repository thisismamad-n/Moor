"""Desktop (Electron) app: build/stamp, stage-and-swap pack, exe integrity gate, macOS signing/TCC, Linux sandbox, launch (moor gui/desktop).
...
from moor_cli.main_tui_launch import _npm_lifecycle_env
...
def _desktop_stamp_path() -> Path:
    """Path of the desktop build stamp under $MOOR_HOME."""
...
def _desktop_packaged_executable_in(release_dir: Path) -> Optional[Path]:
    ...
    *release_dir* is electron-builder's ``directories.output`` — the live ``apps/desktop/release`` or a
    ...
    if sys.platform == "darwin":
        ...
        candidates = [
            release_dir / d / "Moor.exe" for d in ("win-unpacked", "win-ia32-unpacked", "win-arm64-unpacked")
        ]
    else:
        candidates = [
            release_dir / d / n for d in ("linux-unpacked", "linux-arm64-unpacked") for n in ("moor", "Moor")
        ]
    ...
    if sys.platform == "win32" and len(existing) > 1:
        ...
        # mtime can hand a wrong-architecture Moor.exe to the launcher. Prefer
        ...
        # wrong-architecture Moor.exe to the launcher, which Windows rejects with "This app can't run on
        # your computer" (#69179).
...
# PLACE: before-pack.mjs wipes ``release/<platform>- unpacked`` (or the mac ``Moor.app``) and the Electron
# unpack + asar + rename then rebuild it. Any failure after that wipe — corrupt cached zip, blocked
# download, missing dep, disk full — leaves the user with NO app, and ``moor update`` used to report
...
# (Desktop → moor-setup --update → `moor update` → `moor desktop --build-only` → relaunch) rebuilds
# Moor.exe on the end user's machine and used to verify only that the file EXISTS before declaring
...
# Moor.exe is caught BEFORE the updater replaces the working app, and the previous build can be restored
...
def _windows_native_machine_from_iswow64() -> Optional[str]:
    ...
    emulated process arch) and rejected a correctly-built ARM64 ``Moor.exe``. Binding
...
def _windows_native_machine() -> str:
    ...
    chain runs an x64 moor-setup.exe (and thus x64 Python) on Windows-on-ARM devices, where
...
def _desktop_macos_relaunchable_fixup(
    ...
    "Moor is damaged" and loses every grant. Clear quarantine xattrs, then sign
...
def _macos_create_signing_identity(
    ...
    tmp_dir = Path(tempfile.mkdtemp(prefix="moor-tcc-"))
    try:
        ...
        def _export_p12(extra_args: list) -> None:
            subprocess.run(
                [
                    ...
                    "-out", str(p12), "-passout", "pass:moorlocal",
                ],
                capture_output=True, check=True)
...
def _desktop_macos_setup_tcc_identity(identity: str = "Moor Local Signing") -> bool:
    ...
    print(
        ...
        "stuck, reset it with:  tccutil reset All com.moorinc.moor"
    )
    return True
...
def _install_desktop_workspace_deps(npm: str, env: dict) -> None:
    ...
    from moor_cli.main import PROJECT_ROOT
    from moor_cli.main_web_build import _run_npm_install_deterministic
...
def _run_desktop_pack_with_recovery(
    ...
    if build_result.returncode != 0 and staging_dir is not None and _staged_exe() is None:
        ...
        if restored:
            ...
            for p in purged:
                print(f"    - {p}")
            # The purge can't remove a win-unpacked tree whose Moor.exe is
...
def _promote_staged_desktop_app(desktop_dir: Path, staging_dir: Path) -> Path:
    ...
    # Moor.exe Windows cannot load. Verified on the STAGED exe, so a failure
...
def _build_desktop_app(desktop_dir: Path, *, source_mode: bool, npm: str, env: dict) -> Optional[Path]:
    ...
    if not source_mode:
        ...
        # A running desktop instance holds Moor.exe locked on Windows, so the
...
def _desktop_launch_env(args: argparse.Namespace) -> tuple[dict, list[str]]:
    ...
    env["MOOR_DESKTOP_CWD"] = str(Path(cwd).expanduser().resolve()) if cwd else os.getcwd()
...
def cmd_gui(args: argparse.Namespace):
    ...
    from moor_cli.main import PROJECT_ROOT
    from moor_cli.main_install_repair import _resolve_node_runtime_npm
    desktop_dir = PROJECT_ROOT / "apps" / "desktop"
    ...
    if getattr(args, "setup_tcc_identity", False):
        identity = getattr(args, "identity", None) or "Moor Local Signing"
