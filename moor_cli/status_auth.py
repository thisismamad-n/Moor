"""Credential sections of `moor status`, run through ``status._SECTIONS`` with its shared context.
Origin helpers (``_row``, ``_first_env_value``, ...) are resolved through the ``moor_cli.status``
...
_OAUTH_BLOCKS = (
    ...
    ("OpenAI Codex", "get_codex_auth_status", "moor model", _FILE_REFRESH_ROWS),
    ...
    ("MiniMax OAuth", "get_minimax_oauth_auth_status", "moor auth add minimax-oauth", (
        ("Region:", "region", None, True),
        ("Access exp:", "expires_at", None, None),
        ("Error:", "error", None, False))),
    ("xAI OAuth", "get_xai_oauth_auth_status", "moor auth add xai-oauth", _FILE_REFRESH_ROWS))
...
# Moor Tool Gateway per-feature state: first matching (predicate(feature, moor_auth), text(feature)).
_FEATURE_STATES = (
    (lambda f, _: f.managed_by_moor, lambda f: "active via Moor subscription"),
...
def _render_auth_providers(ctx):
    ...
    try:
        # Read-only display: the refresh-free snapshot, so `moor status` never performs an OAuth
        # refresh or burns a single-use refresh token.
        moor_status = auth.get_moor_auth_status_local()
        statuses = {getter: getattr(auth, getter)() for _, getter, _, _ in _OAUTH_BLOCKS[:3]}
    except Exception:
        moor_status, statuses = {}, {}
    ...
    info = None
    if any(moor_status.get(k) for k in ("logged_in", "access_token", "portal_base_url",
                                        "inference_credential_present", "error_code")):
        try:
            info = get_moor_portal_account_info()
        except Exception:
            pass
    ctx.moor_account_info = info
    ctx.moor_logged_in = logged_in = bool(moor_status.get("logged_in") or (info and info.logged_in))
    ...
    for label, value, show in (
        ...
        ("Access exp:", _format_iso_timestamp(moor_status.get("access_expires_at")),
         logged_in or moor_status.get("access_expires_at")),
        ("Key exp:", _format_iso_timestamp(moor_status.get("agent_key_expires_at")),
         logged_in or inference or moor_status.get("agent_key_expires_at")),
        ("Refresh:", "yes" if moor_status.get("has_refresh_token") else "no",
         logged_in or moor_status.get("has_refresh_token")),
        ("Error:", moor_error, moor_error)):
        if show:
...
def _render_moor_gateway(ctx):
    if managed_moor_tools_enabled():
        features = get_moor_subscription_features(ctx.config)
        _status._section("Moor Tool Gateway")
        print("  Moor Portal   ✓ managed tools available" if features.moor_auth_present
              else "  Moor Portal   ✗ not logged in")
        for f in features.items():
            state = next((text(f) for match, text in _FEATURE_STATES if match(f, features.moor_auth_present)),
                         "not configured")
            _status._row(f.label, f.available or f.active or f.managed_by_moor, state, 15, " ")
    elif ctx.moor_logged_in or ctx.moor_inference_present:
        # Moor OAuth without entitlement, or an opaque inference key without Portal account
        # information, cannot enable the Tool Gateway.
        _status._section("Moor Tool Gateway")
        message = format_moor_portal_entitlement_message(
            ctx.moor_account_info, capability="managed web, image, TTS, STT, browser, and Modal tools"
        )
...
def _render_apikey_providers(ctx):
    ...
    for pname, env_vars in _APIKEY_PROVIDERS.items():
        ...
        _status._row(pname, configured, "configured" if configured else "not configured (run: moor model)", 16, " ")
    ...
    if _status._effective_provider_label() == "LM Studio":
        from moor_cli.models_local import probe_lmstudio_models
...
import moor_cli.status as _status  # noqa: E402  (bottom: moor_cli.status imports this module)
