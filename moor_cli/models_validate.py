...
Split out of ``moor_cli.models``. Catalog fetchers defined in ``moor_cli.models`` are looked up
there at call time (``_m.<name>``) so ``patch("moor_cli.models.<name>")`` mocks keep intercepting;
local-server probes are looked up on ``moor_cli.models_local`` (``_ml.<name>``) the same way.
...
def _validate_moa(req: _Request) -> dict[str, Any]:
    try:
        ...
        if req.requested in cfg["presets"]:
            return _accept()
        return _reject(f"MoA preset `{req.requested}` was not found. Run `moor moa list`.")
...
def _validate_lmstudio(req: _Request) -> dict[str, Any]:
    from moor_cli import models_local as _ml
...
def _ollama_probe_headers(req: _Request) -> dict[str, str]:
    ...
    from moor_cli import models_local as _ml
    from moor_cli.models_local import _configured_ollama_base_url, _drop_authorization
...
def _validate_custom(req: _Request) -> dict[str, Any]:
    ...
    message = (
        ...
        f"Moor will still save `{req.requested}`, but the endpoint should expose `/models` for verification."
    )
    if anthropic_style:
...
def _validate_static_catalog(req: _Request) -> Optional[dict[str, Any]]:
    ...
    if req.normalized == "openai-codex":
        ...
        # the suffix is a Moor picker convention, so an unknown `*-900k` can never be a real
...
def _moor_portal_recommended_names() -> set[str]:
    """Lower-cased ids from the Portal's live recommended-models feed (empty on any failure)."""
    from moor_cli import models as _m

    try:
        payload = _m.fetch_moor_recommended_models(_m._resolve_moor_portal_url())
        return {
            name.lower()
...
def _validate_live_listing(req: _Request) -> Optional[dict[str, Any]]:
    ...
    # Moor: the Portal's recommended-models feed can list a model before the curated list or the
    # docs-hosted manifest catches up; `moor chat` already accepts those at model-list build
    # time, so mirror that source of truth for per-message /model validation.
    if req.normalized == "moor" and req.lookup.lower() in _moor_portal_recommended_names():
        return _accept_with_note(f"Note: `{req.requested}` was not found in the live /v1/models listing "
                                 "but is a current Moor Portal recommendation — accepted.")
