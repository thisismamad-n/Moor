"""Windows ConPTY bridge for the `moor dashboard` chat tab.

Drop-in counterpart to ``moor_cli.pty_bridge.PtyBridge`` for native
Windows. Mirrors the exact public surface the ``/api/pty`` WebSocket
handler in ``moor_cli.web_server`` consumes: ``spawn``, ``read``,
