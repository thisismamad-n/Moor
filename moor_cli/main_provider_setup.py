...
Split out of ``moor_cli/main.py``. Names that still live in main are imported lazily at call time.
"""

import contextlib

from typing import Optional
from moor_cli.model_setup_flows_common import _ask, _ensure_dict_section, _print_numbered, _radiolist, _say
...
def _clear_stale_openai_base_url():
    """Remove OPENAI_BASE_URL from ~/.moor/.env unless the active provider is 'custom' — a
...
def _all_aux_tasks() -> list[tuple[str, str, str]]:
    ...
    (:meth:`moor_cli.plugins.PluginContext.register_auxiliary_task`)."""
...
def _aux_config_menu() -> None:
    ...
    while True:
        ...
        _say("", "  Auxiliary models — side-task routing", "",
             ...
             "  Moor only falls back to a lightweight backend (OpenRouter,",
             "  Moor Portal) if the main model is unavailable.  Override a",
...
_CUSTOM_API_MODES = (
    ("", "Auto-detect", "Use Moor URL heuristics; best for standard OpenAI-compatible endpoints.",
...
def _prompt_api_key(pconfig, existing_key: str, provider_id: str = "", existing_source: str = "") -> tuple:
    """API-key entry for ``moor setup`` / ``moor model``: first-time entry, or [K]eep / [R]eplace /
    ...
    if choice.startswith("c") and not pool_backed:
        ...
        print(f"  API key cleared.  Re-run `moor setup` to configure {pconfig.name} again.")
        return "", True
...
def _stepfun_base_url_for_region(region: str) -> str:
    from moor_cli.auth import STEPFUN_STEP_PLAN_CN_BASE_URL, STEPFUN_STEP_PLAN_INTL_BASE_URL
...
def _run_anthropic_oauth_flow(save_env_value):
    ...
    from moor_cli.config import save_anthropic_oauth_token, use_anthropic_claude_code_credentials
    ...
    def _activate_claude_code_credentials_if_available() -> bool:
        ...
        if creds and (is_claude_code_token_valid(creds) or bool(creds.get("refreshToken"))):
            ...
            print(f"    Moor will use Claude's credential store directly instead of copying a setup-token into {_dhh_fn()}/.env.")
            return True
        return False
...
def _named_custom_provider_map(cfg) -> dict[str, dict[str, str]]:
    ...
    from moor_cli.config import get_compatible_custom_providers, read_raw_config
...
def _build_provider_picker_rows(config: dict, active: str, provider_labels: dict[str, str],
    ...
    """Rows for the ``moor model`` provider picker plus the pre-selected index. Canonical providers
    ...
    from moor_cli.models import CANONICAL_PROVIDERS, _PROVIDER_ALIASES
    from moor_cli.models_catalog_static import group_providers, provider_group_for_slug
