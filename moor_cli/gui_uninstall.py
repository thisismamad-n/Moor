"""
Moor Desktop (Chat GUI) uninstaller.
...
the Python agent or the user's config/data:
  ...
  1. Source-built GUI (``moor desktop`` / ``moor gui``)
     Built inside the agent checkout under ``$MOOR_HOME/moor-agent/``:
       ...
       - ``$MOOR_HOME/desktop-build-stamp.json`` (the build freshness stamp)
  ...
  2. Packaged distributable (DMG / NSIS / AppImage / deb / rpm)
     ...
     own bundled Electron + a per-user Electron ``userData`` directory:
       - macOS:   ``/Applications/Moor.app`` or ``~/Applications/Moor.app``
       - Windows: ``%LOCALAPPDATA%\\Programs\\Moor`` (NSIS per-user)
...
the app name ("Moor"), separate from ``$MOOR_HOME``:
  - macOS:   ``~/Library/Application Support/Moor``
  - Windows: ``%APPDATA%\\Moor``
  - Linux:   ``$XDG_CONFIG_HOME/Moor`` (default ``~/.config/Moor``)
...
``moor uninstall --gui`` (and friends) without paying for the full CLI.
"""
...
def source_built_gui_artifacts(moor_home: Path) -> "list[Path]":
    """GUI build artifacts produced by ``moor desktop`` inside the checkout.

    These are removable on a GUI uninstall without harming the agent: the
    Python agent runs from ``moor-agent/`` source + ``venv/`` and never
    needs the Electron build output or node_modules.
    """
    agent_root = _agent_root(moor_home)
    ...
    return [
        ...
        moor_home / "desktop-build-stamp.json",
    ]
...
def packaged_gui_app_paths() -> "list[Path]":
    ...
    if sys.platform == "darwin":
        paths += [
            Path("/Applications/Moor.app"),
            home / "Applications" / "Moor.app",
        ]
    elif sys.platform == "win32":
        local = os.environ.get("LOCALAPPDATA")
        local_base = Path(local) if local else (home / "AppData" / "Local")
        paths += [
            # NSIS per-user install (perMachine=false → Programs\Moor).
            local_base / "Programs" / "Moor",
            # Older / alternate layout some builds used.
            local_base / "moor-desktop",
        ]
        program_files = os.environ.get("ProgramFiles")
        if program_files:
            # NSIS per-machine fallback (needs admin to remove).
            paths.append(Path(program_files) / "Moor")
    else:
        ...
        paths += [
            # The launcher entry `moor desktop` installs. Its icon lives
            # in the checkout, not in the installed app.
            desktop_entry_path(),
            # Some packaged builds emit this casing.
            data_base / "applications" / "Moor.desktop",
        ]
    return paths


def agent_is_installed(moor_home: Path) -> bool:
    """Return True when a usable Python agent install exists under MOOR_HOME.
...
def gui_is_installed(moor_home: Path) -> bool:
    ...
    for p in source_built_gui_artifacts(moor_home):
        if p.exists():
            return True
...
def uninstall_gui(moor_home: "Path | None" = None, *, remove_userdata: bool = True) -> "list[Path]":
    ...
    Never touches ``moor-agent/moor_cli`` (agent source), ``venv/``, or any
    config / sessions / .env under ``$MOOR_HOME``.
    ...
    if sys.platform.startswith("linux"):
        ...
        # Reindex so Moor disappears from the launcher.
        try:
            from moor_cli.linux_desktop_entry import (
                desktop_entry_path,
        ...
        log_info(
            ...
            "with your package manager (e.g. 'sudo apt remove moor' or "
            "'sudo dnf remove moor'). AppImage builds are a single file you "
