...
01-moor-setup (the stage2 hook) has chowned the volume and seeded $MOOR_HOME, but
...
def reconcile_profile_gateways(
    *, moor_home: Path, scandir: Path, dry_run: bool = False,
    ...
    the top of ``$MOOR_HOME``): ``moor_cli.gateway`` maps an empty profile suffix to it,
    so it is what ``moor gateway start`` (no ``-p``) targets.

    Without it, bare ``moor gateway start`` inside the container would land on ``s6-svc -u
    ...
    if profiles_root.is_dir():
        for entry in sorted(profiles_root.iterdir()):
            # SOUL.md (seeded by `moor profile create`) is the "real profile" marker.
...
def _maybe_migrate_legacy_gateway_run_state(
    moor_home: Path, *, container_argv: Sequence[str] | None, dry_run: bool) -> str | None:
...
def _strip_container_argv_prefix(argv: Sequence[str]) -> list[str]:
    ...
    if args and args[0].endswith("entrypoint-dispatch.sh"):  # non-PID-1 dispatch shim
        args = args[1:]
    if args and Path(args[0]).name == "moor":  # the wrapper re-execs `moor <subcommand>`
        args = args[1:]
    return args
...
def _register_service(scandir: Path, profile: str, *, start: bool) -> None:
    ...
    try:
        ...
        if not start:  # `moor -p <profile> gateway start` brings it up later (s6-svc -u)
            (tmp_dir / "down").touch()
        # Pre-create supervise/ with moor ownership BEFORE publishing so s6-supervise inherits
        # it and runtime s6-svc calls as the moor user won't EACCES.
...
def _write_reconcile_log(moor_home: Path, actions: list[ReconcileAction]) -> None:
    """Append one line per profile to $MOOR_HOME/logs/container-boot.log (rotated to ``.1``) —
...
def main() -> int:
    ...
    # A dashboard-only container must not reconcile: with a shared bind-mounted MOOR_HOME both
