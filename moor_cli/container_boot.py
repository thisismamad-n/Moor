...
``$MOOR_HOME/profiles/<name>/`` live on the persistent VOLUME, and
...
Dockerfile (Phase 4 Task 4.0). Runs as root after 01-moor-setup
(the stage2 hook) has chowned the volume and seeded $MOOR_HOME, but
...
def reconcile_profile_gateways(
    *,
    ...
    (the implicit profile that lives at the top of ``$MOOR_HOME``,
    not under ``profiles/``). The dispatcher in ``moor_cli.gateway``
    maps an empty profile suffix to ``gateway-default``, so this slot
    is what ``moor gateway start`` (no ``-p``) targets. Without it,
    bare ``moor gateway start`` inside the container would land on
    ...
    ``$MOOR_HOME/gateway_state.json`` (sibling to the profile root,
    not under ``profiles/``); stale runtime files there are swept the
    same way as for named profiles.

    Args:
        moor_home: The container's MOOR_HOME (typically /opt/data).
            Profiles live under ``<moor_home>/profiles/<name>/``;
            the default profile lives at ``<moor_home>`` itself.
    ...
    # ``moor gateway start`` (no ``-p``) has somewhere to land;
    ...
    actions.append(ReconcileAction(
        ...
        prior_exit=_read_prior_exit_label(moor_home),
    ))
    ...
    if profiles_root.is_dir():
        for entry in sorted(profiles_root.iterdir()):
            if not entry.is_dir():
                continue
            # SOUL.md is always seeded by `moor profile create` (config.yaml
            # is not — that comes later via `moor setup`). Use it as the
            ...
            # via ``moor -p default-named gateway start`` if they
    ...
    if not dry_run:
        _write_reconcile_log(moor_home, actions)
    return actions
...
def _maybe_migrate_legacy_gateway_run_state(
    ...
    *,
    ...
    """
    state_file = moor_home / "gateway_state.json"
...
def _strip_container_argv_prefix(argv: Sequence[str]) -> list[str]:
    """Strip the s6/wrapper prefix off the container argv, leaving the moor args.

    Two container-command argv shapes are handled:

    * **s6-overlay v2 / tini:** PID 1 argv is
      ``/init /opt/moor/docker/main-wrapper.sh <subcommand> [args...]``.
    * **s6-overlay v3:** PID 1 is ``s6-svscan`` and the command lives on the
      rc.init-launched process as ``/bin/sh -e
      /run/s6/basedir/scripts/rc.init top /opt/moor/docker/main-wrapper.sh
    ...
    ``moor`` invocations carry no wrapper, so fall back to peeling a bare
    ``init`` prefix. The wrapper re-execs ``moor <subcommand>``, so an
    explicit leading ``moor`` is peeled too. Shared by the legacy-gateway
    ...
    # The wrapper re-execs `moor <subcommand>`; peel an explicit moor.
    if args and Path(args[0]).name == "moor":
        args = args[1:]
    return args
...
def _is_dashboard_container(argv: Sequence[str]) -> bool:
    ...
    A dashboard-only container (``moor dashboard ...``) never spawns or
    ...
    the gateway and dashboard containers share a bind-mounted MOOR_HOME,
...
def _register_service(scandir: Path, profile: str, *, start: bool) -> None:
    ...
    try:
        ...
        # it up explicitly with `moor -p <profile> gateway start`
        ...
        # spawns will EEXIST our dirs/FIFOs and inherit moor
        # ownership, so runtime s6-svc / s6-svstat / s6-svwait calls
        # (all dispatched as the moor user) won't hit EACCES. See
...
def _write_reconcile_log(
    moor_home: Path, actions: list[ReconcileAction],
) -> None:
    """Append one line per profile to $MOOR_HOME/logs/container-boot.log.
...
def main() -> int:
    ...
    # bind-mounted MOOR_HOME, both race to flock() the same s6-log lock
    ...
    actions = reconcile_profile_gateways(
        moor_home=moor_home, scandir=scandir,
    )
    for a in actions:
        print(
