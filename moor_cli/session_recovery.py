...
def _copy_source_bundle(source: Path, snapshot_dir: Path) -> tuple[Path, list[str]]:
    ...
    locks -- the failure class ``moor_cli.sqlite_safe_read`` exists to
    ...
    keeps this path consistent with ``moor_state._backup_db_file``.
    """
    ...
    try:
        ...
        if before != after:
            raise SessionRecoverySafetyError(
                ...
                "This includes the interactive `moor` CLI session this "
                ...
                "recovery from a fresh shell with no `moor` session open, "
