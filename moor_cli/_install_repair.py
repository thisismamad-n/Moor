...
- ``moor_cli._early_recovery.recover_if_needed`` — stdlib-only, runs BEFORE
  ``moor_cli.main``'s third-party imports, so it can complete a pending
  update while no native extension is mapped yet (#83569).
- ``moor_cli.main._recover_core_update_marker_locked`` — the historical
...
the corrupted-venv state it exists to repair. ``moor_cli.main`` imports
``managed_uv``, ``moor_constants``, and friends only in its late path; the
...
:func:`moor_cli._early_recovery._find_uv_binary` lookup and falls back to
...
def _stdout_to_stderr():
    ...
    ``moor acp`` speaks JSON-RPC on stdout; an inherited-fd install child
...
def _venv_scripts_dir(root: Path) -> Path | None:
    ...
    # moor_constants is stdlib-only, so the canonical layout helpers are safe
...
def _venv_is_relocatable(venv_dir: Path) -> bool:
    ...
    uv writes the flag; ``moor_cli.managed_uv`` builds its replacement
...
def ensure_windows_bin_launchers(
    ...
    *,
    ...
    """Re-stage the Windows ``moor`` launchers when they vanish.

    On Windows, ``moor`` resolves through launchers derived from the venv
...
def migrate_windows_bin_path(
    ...
    *read_user_path*/*write_user_path* are injectable for tests.
    ...
    from moor_constants import get_default_moor_root, venv_bin_dir
...
def _restore_quarantined_exes(moved: list[tuple[Path, Path]]) -> None:
    ...
    ``moor acp`` speaks JSON-RPC on stdout.
    """
...
def _run_install_cmd(cmd: list[str], *, env: dict | None, root: Path) -> None:
    ...
    failed: list[str] = []
    moved = (
        _quarantine_running_moor_exe(scripts_dir, failed_out=failed)
        if scripts_dir
        else []
    )
    if failed:
    ...
    try:
        ...
        # the quarantined shims renamed aside and `moor` gone from PATH
...
def run_core_install(root: Path) -> None:
    ...
    - quarantine live ``moor*.exe`` shims on Windows so they can be replaced
