...
def _venv_scripts_dir(root: Path) -> Path | None:
    ...
    if not venv_dir.is_dir():
        return None
    # moor_constants is stdlib-only, so the canonical layout helper is safe
...
def _quarantine_running_moor_exe(scripts_dir: Path) -> list[tuple[Path, Path]]:
...
def _run_install_cmd(cmd: list[str], *, env: dict | None, root: Path) -> None:
    ...
    moved = _quarantine_running_moor_exe(scripts_dir) if scripts_dir else []
    try:
