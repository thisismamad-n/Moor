...
Callers: ``_early_recovery.recover_if_needed`` (stdlib-only, runs BEFORE ``moor_cli.main``'s
third-party imports so a pending update completes while no native extension is mapped yet) and
``moor_cli.main_install_repair._recover_core_update_marker_locked`` (the post-import recovery path).
...
from moor_cli import _early_recovery as _er
...
def _is_termux_env(env: dict | None = None) -> bool:
    """Stdlib Termux probe (moor_cli.main's version lives behind imports)."""
...
def _stdout_to_stderr():
    ...
    ``moor acp`` speaks JSON-RPC on stdout; an inherited-fd install child writing there would
...
def _venv_scripts_dir(root: Path) -> Path | None:
    """Project venv Scripts/bin dir, when present (moor_constants is stdlib-only)."""
    # moor_constants is stdlib-only, so the canonical layout helpers are safe to use from this
...
#: Moor root's ``bin``, next to uv.exe). Keep in lockstep with scripts/install.ps1.
...
def _default_moor_root() -> Path | None:
    """The DEFAULT Moor root (not ``get_moor_home()``, which under ``moor -p <name>`` is
    ...
    try:
        return Path(get_default_moor_root())
    except Exception:
        return None
...
def ensure_windows_bin_launchers(
    ...
    r"""Re-stage the Windows ``moor`` launchers when they vanish.

    On Windows, ``moor`` resolves through launchers derived from the venv console scripts — never
    ...
    The canonical launcher home is the managed binary dir — the default Moor root's ``bin``
    (``%LOCALAPPDATA%\\moor\\bin``, next to the managed uv) — which lives OUTSIDE the git checkout so no
    git operation can ever touch it. It is a per-machine dir shared by every profile: ``get_moor_home()``
    would point inside ``profiles\\<name>`` under ``moor -p``, so the anchor here is
    :func:`moor_constants.get_default_moor_root`. See #83797.
    """
    if windows is None:
        windows = _is_windows()
    if not windows:
        return []
    root = Path(root)
    home = _default_moor_root()
    if home is None:
        return []

    targets: list[Path] = []
    # Runs at every moor_cli.main process start, so the healthy path must stay a few stat calls.
    ...
    if restored:
        ...
        with contextlib.suppress(OSError, ValueError):
            print("  ✓ Restored moor launcher(s): " + ", ".join(restored), file=sys.stderr)
    return restored
...
def migrate_windows_bin_path(
    ...
    """One-time PATH migration to the ``MOOR_HOME\\bin`` launcher layout (``moor update`` tail).
    ...
    *read_user_path*/*write_user_path* are injectable for tests.
    ...
    if kept != entries:
        try:
            ...
            return False
        with contextlib.suppress(OSError, ValueError):
            print(f"  ✓ moor launchers now resolve from {home_bin} "
...
def _quarantine_running_moor_exe(
    ...
    """Rename live moor*.exe shims aside so the installer can rewrite them.
...
def _restore_quarantined_exes(moved: list[tuple[Path, Path]]) -> None:
    ...
    (#75584). Warnings land on stderr — this module runs in the early-recovery path and ``moor acp``
...
def _run_install_cmd(cmd: list[str], *, env: dict | None, root: Path) -> None:
    ...
    failed: list[str] = []
    moved = _quarantine_running_moor_exe(scripts_dir, failed_out=failed) if scripts_dir else []
    if failed:
    ...
    try:
        ...
        # the shims renamed aside and `moor` gone from PATH. Restore only renames back when the
