"""Autostash handling for ``moor update``: stash before the pull, restore/park/discard afterwards, warn about orphans.
...
_AUTOSTASH_NAME_PREFIX = "moor-update-autostash-"
...
def _warn_orphaned_update_autostashes(git_cmd: list[str], cwd: Path) -> int:
    ...
    Deliberately NOT a GC: a stash may be the only copy of the user's work, so Moor never drops one.
...
def _restored_python_paths(git_cmd: list[str], cwd: Path) -> tuple[str, ...] | None:
    ...
    from moor_cli.update_cmd import _git_untracked_paths
...
def _reject_unsafe_stash_restore(
    ...
    from moor_cli.update_cmd import _git_untracked_paths
    print()
    print("✗ Restored local changes made the Moor agent unexecutable.")
...
def _apply_stash(git_cmd: list[str], cwd: Path, stash_ref: str) -> bool:
    ...
    print(f"  Stash ref: {stash_ref}")
    _reset_hard(git_cmd, cwd)  # conflict markers make moor unrunnable; changes stay in the stash
...
def _drop_restored_stash(git_cmd: list[str], cwd: Path, stash_ref: str) -> None:
    ...
    if drop.returncode != 0:
        print("⚠ Local changes were restored, but Moor couldn't drop the saved stash entry.")
...
def _discard_stashed_changes(git_cmd: list[str], cwd: Path, stash_ref: str) -> bool:
    ...
    drop = _git_run(git_cmd, ["stash", "drop", stash_selector], cwd)
    if drop.returncode != 0:
        print("⚠ Configured to discard local changes, but Moor couldn't drop the saved stash entry.")
