"""CLI handlers for the ``moor proxy`` subcommand."""
...
from moor_cli.proxy.server import (
...
def cmd_proxy_start(args: Any) -> int:
    ...
    if not AIOHTTP_AVAILABLE:
        _err("moor proxy requires aiohttp. Run `moor setup` to install it.")
        return 1
    provider = getattr(args, "provider", None) or "moor"
    try:
    ...
    if not adapter.is_authenticated():
        auth_hint = getattr(adapter, "auth_hint", f"moor auth add {adapter.name}")
        _err(f"Not logged into {adapter.display_name}. Run `{auth_hint}` first.")
        return 2
    host = getattr(args, "host", None) or DEFAULT_HOST
    port = getattr(args, "port", None) or DEFAULT_PORT
    _err(
        f"Starting Moor proxy for {adapter.display_name}\n"
...
def cmd_proxy_status(args: Any) -> int:
    ...
    print("Moor proxy upstream adapters\n")
    ...
    print("\nStart the proxy with: moor proxy start [--provider <name>]")
    return 0
...
def cmd_proxy(args: Any) -> int:
    """Dispatch ``moor proxy <subcommand>``; no/unknown subcommand prints the short help."""
    handler = _SUBCOMMANDS.get(getattr(args, "proxy_command", None))
    if handler is not None:
        return handler(args)
    _err(
        "moor proxy — local OpenAI-compatible proxy that attaches your\n"
        "OAuth-authenticated provider credentials to outbound requests.\n"
        "\n"
        "Subcommands:\n"
        "  moor proxy start [--provider moor|xai] [--host 127.0.0.1] [--port 8645]\n"
