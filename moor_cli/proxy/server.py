...
from moor_cli.proxy.sse_done import DONE_SSE_FRAME, SseDoneTracker, content_type_is_sse
...
def _require_aiohttp() -> None:
    if not AIOHTTP_AVAILABLE:
        raise RuntimeError("aiohttp is required for `moor proxy`. Run `moor setup` to install it.")
...
def create_app(adapter: UpstreamAdapter) -> "web.Application":
    ...
    Every adapter method is synchronous and blocking (the Moor adapter takes the 15s cross-process
    ...
    async def handle_proxy(request: "web.Request") -> "web.StreamResponse":
        ...
        if upstream_resp.status in {401, 429}:
            # One-shot retry with a refreshed/rotated credential (Moor: unconditional refresh
