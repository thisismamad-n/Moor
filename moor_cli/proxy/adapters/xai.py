    ...
    def get_credential(self) -> UpstreamCredential:
        with self._lock:
            ...
            if pool is None or not pool.has_credentials():
                raise RuntimeError(
                    "No xAI OAuth credentials found. Run `moor auth add xai-oauth --type oauth` first."
                )
            entry = pool.select()
            if entry is None:
                raise RuntimeError(
                    "No available xAI OAuth credentials found. Run "
                    "`moor auth reset xai-oauth` or re-authenticate with `moor auth add xai-oauth --type oauth`."
                )
            self._pool = pool
