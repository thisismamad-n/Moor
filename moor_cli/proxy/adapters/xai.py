...
# xAI's public API is OpenAI-compatible for the endpoints Moor commonly
# uses. The Responses endpoint is included because Moor' native xAI runtime
...
class XAIGrokAdapter(UpstreamAdapter):
    """Proxy upstream for xAI Grok via Moor-managed OAuth credentials."""

    auth_hint = "moor auth add xai-oauth --type oauth"
        ...
        with self._lock:
            ...
            if pool is None or not pool.has_credentials():
                raise RuntimeError(
                    ...
                    "`moor auth add xai-oauth --type oauth` first."
                )
            ...
            if entry is None:
                raise RuntimeError(
                    ...
                    "`moor auth reset xai-oauth` or re-authenticate with "
                    "`moor auth add xai-oauth --type oauth`."
                )
    ...
    def _credential_from_entry(self, entry: PooledCredential) -> UpstreamCredential:
        ...
        if not bearer:
            raise RuntimeError(
                ...
                "Re-authenticate with `moor auth add xai-oauth --type oauth`."
            )
