...
Reads the user's Moor OAuth state from ``~/.moor/auth.json`` through the
...
class MoorPortalAdapter(UpstreamAdapter):
    ...
    def __init__(self) -> None:
        ...
        # and persistence are handled by resolve_moor_runtime_credentials().
    ...
    def _get_credential(
        ...
        *,
        ...
        with self._lock:
            ...
            if state is None:
                raise RuntimeError(
                    "Not logged into Moor Portal. Run `moor auth add moor` first."
                )
            ...
            try:
                ...
                raise RuntimeError(
                    f"Failed to refresh Moor Portal credentials: {exc}"
                ) from exc
            ...
            # base_url returned by resolve_moor_runtime_credentials() already
            # honors the MOOR_INFERENCE_BASE_URL env override (the documented
    ...
    # to moor_cli.auth to avoid expanding that module's public surface.
    # ------------------------------------------------------------------
