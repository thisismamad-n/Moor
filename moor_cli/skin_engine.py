...
file. Skins are defined as YAML files in ~/.moor/skins/ or as built-in presets.
...
``resolve_skin`` / ``skin.changed``). A skin dropped in ~/.moor/skins/ therefore
...
.. code-block:: python
    ...
    from moor_cli.skin_engine import get_active_skin, list_skins, set_active_skin
    ...
    set_active_skin("mytheme")            # Switch to user skin from ~/.moor/skins/
...
Drop a YAML file in ``~/.moor/skins/<name>.yaml`` following the schema above.
...
class SkinConfig:
    ...
    banner_logo: str = ""    # Rich-markup ASCII art logo (replaces MOOR_AGENT_LOGO)
    banner_hero: str = ""    # Rich-markup hero art (replaces MOOR_CADUCEUS)
