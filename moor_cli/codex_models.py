...
DEFAULT_CODEX_MODELS: List[str] = [
    ...
    # not in the public API, so it stays out of the "openai" catalog in moor_cli/models.py.
...
# OpenAI API, so it intentionally stays out of the "openai" provider catalog in moor_cli/models.py — only
...
def _add_context_variants(model_ids: List[str]) -> List[str]:
    ...
    The suffix is moor-side only — stripped before the id hits the wire (agent/transports/codex.py,
