...
Moor is developed on Linux / macOS and tested natively on Windows too.
...
# Electron exits, so the gateway never gets respawned after a `moor
...
def suppress_platform_ver_console() -> None:
    ...
    *visible* console: one flashing ``cmd`` window per process, triggered by
    ...
    during a dependency's import, not from Moor' own code.
    """
    if not IS_WINDOWS:
        return
    try:
...
def noninteractive_git_env(
    ...
    Moor shells out to git from many non-interactive contexts — MCP catalog
...
def bounded_probe_run(
    ...
    *,
    ...
    scan hit exactly this during ``moor update`` on slow-WMI machines
