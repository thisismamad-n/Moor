...
def _find_stale_dashboard_pids(*, exclude_pids: set[int] | None = None) -> list[int]:
    ...
    from moor_cli.dashboard_procs import _scan_dashboard_processes
...
def _parse_dashboard_runtime(command: str) -> tuple[str, str, int] | None:
    ...
    for candidate in ("dashboard", "serve"):
        patterns = (f"moor {candidate}", f"moor_cli.main {candidate}", f"moor_cli/main.py {candidate}")
...
def _restart_managed_dashboard_service(reason: str, unit: str = _DASHBOARD_SYSTEMD_UNIT) -> bool:
    ...
    # User manager first (Moor installs Linux services in the user scope by
...
def _get_systemd_service_for_pid(pid: int) -> str | None:
    """The systemd service unit name *pid* belongs to (``moor-serve.service``), or None.
...
def _respawn_dashboard_processes(commands: list[list[str]]) -> list[list[str]]:
    """Respawn manually-started dashboards after ``moor update``, detached, logging to
...
class _UpdateOutputStream:
    """stdout/stderr wrapper for ``moor update``: mirrors to ``logs/update.log`` and, once the
...
def _install_hangup_protection(gateway_mode: bool = False):
    ...
    try:
        ...
        log_file.write(f"\n=== moor update started {_dt.datetime.now().isoformat(timespec='seconds')} ===\n")
...
def _report_dashboard_status() -> int:
    ...
    if not live:
        print("No moor dashboard or serve processes running.")
        return 0

    print(f"{len(live)} moor dashboard/serve process(es) running:")
...
def _read_ssh_session_token_file(path: str) -> str:
    ...
    # The Desktop client writes the token under the account's $HOME/.moor/
    # desktop-ssh, independent of MOOR_HOME and the active profile. Anchor
    # validation there, NOT get_moor_home(): a non-default profile or a Docker
    # /opt/data root re-homes get_moor_home() and would reject every token.
    # See #69551.
    token_root = Path.home() / ".moor" / "desktop-ssh"
    try:
...
def _is_electron_packaged_web_dist(path: str) -> bool:
    ...
    A standalone ``moor dashboard`` inheriting that ``MOOR_WEB_DIST`` would
...
def _route_named_profile_dashboard(
    ...
    (MOOR_DESKTOP=1) stay per-profile. Returns normally when no routing applies.
    """
    try:
    ...
    if (
        ...
        or os.environ.get("MOOR_DESKTOP") == "1"
    ):
        return
    ...
    # MOOR_HOME is pinned to the machine root below — the factory must not
    # re-inject a profile home.
    env = build_subprocess_env(scrub_secrets=False, inherit_profile_home=False)
    # Pin the child to the machine ROOT, resolved explicitly rather than by
    # dropping MOOR_HOME: in the Docker layout the root is /opt/data, and an
    # unset MOOR_HOME would fall back to $HOME/.moor = /opt/data/.moor — an
    # empty auto-seeded home with only the default profile and no install stamp.
    # get_default_moor_root() strips a trailing profiles/<name> for both layouts.
    try:
        from moor_constants import get_default_moor_root
        env["MOOR_HOME"] = str(get_default_moor_root())
    except Exception:
        env.pop("MOOR_HOME", None)  # prior behaviour rather than blocking the reroute
...
def _resolve_dashboard_web_dist(args, _headless_backend: bool) -> None:
    ...
    ``serve`` sets MOOR_SERVE_HEADLESS so mount_spa() stays off. Otherwise build
    unless MOOR_WEB_DIST / --skip-build promise a dist — then verify index.html
    (else the server serves 404s). --skip-build on the default location gets ONE
    recovery build; a caller-managed MOOR_WEB_DIST can't be populated.
    """
    from moor_cli.main import PROJECT_ROOT
    from moor_cli.main_web_build import _build_web_ui
    skip_build = getattr(args, "skip_build", False)
    if _headless_backend:
        os.environ["MOOR_SERVE_HEADLESS"] = "1"  # set before web_server import
    elif "MOOR_WEB_DIST" not in os.environ and not skip_build:
        ...
        if not (_dist_root / "index.html").exists():
            ...
            # --build-mode skip after a wipe of web_dist); a custom MOOR_WEB_DIST
        ...
        # MOOR_WEB_DIST without --skip-build: the env var points at a
        # caller-managed dist, so validate it like the --skip-build branch.
        # MOOR_WEB_DIST is set without --skip-build: the build is skipped (the env var points at a
        ...
        if not (_dist_root / "index.html").exists():
            ...
            sys.exit(1)
        # web_server reads MOOR_WEB_DIST raw at import (no expanduser), so a
