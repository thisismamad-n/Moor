"""Provider rows, active-provider detection, model pickers and provider-selection persistence for `moor tools`."""
...
from moor_cli.config import cfg_get, get_env_value, load_config, save_config, save_env_value
from moor_cli.moor_account import format_moor_portal_entitlement_message
from moor_cli.moor_subscription import MANAGED_FEATURE_COVERAGE_CATEGORY, MoorSubscriptionFeatures
from tools.tool_backend_helpers import MOOR_MANAGED_PROVIDER, fal_key_is_configured
from utils import base_url_hostname, is_truthy_value

logger = logging.getLogger("moor_cli.tools_config")
...
def _visible_providers(
    cat: dict, config: dict, *, force_fresh: bool = False, features: Optional[MoorSubscriptionFeatures] = None,
) -> list[dict]:
    """Provider entries visible for the current auth/config state.
    Moor-managed rows (``managed_moor_feature``) are always shown, even logged-out/unentitled, to
    ...
    for provider in cat.get("providers", []):
        managed = provider.get("managed_moor_feature")
        # Managed rows stay visible regardless of auth (selecting one drives an inline Portal login); a
        # `requires_moor_auth` row without a managed feature hides until logged in.
        if provider.get("requires_moor_auth") and not managed and not features.moor_auth_present:
            continue
...
def provider_readiness_status(provider: dict, config: dict, *, features=None, is_active: Optional[bool] = None) -> str:
    ...
    from moor_cli.tools_config import (
        _POST_SETUP_READY, _provider_env_ready, _xai_credentials_present, get_moor_subscription_features,
    )
...
def _toolset_needs_configuration_prompt(ts_key: str, config: dict, *, force_fresh: bool = False) -> bool:
    ...
    from moor_cli.tools_config import TOOL_CATEGORIES, _post_setup_already_installed, _toolset_has_keys
...
def _configure_tool_category(ts_key: str, cat: dict, config: dict, *, force_fresh: bool = True, reconfigure: bool = False):
    ...
    ``reconfigure`` ("Reconfigure an existing tool"): no setup note / skip row / Moor marker, and the
    chosen provider goes through the key-update prompts instead of the new-enable prompts."""
    from moor_cli.tools_config import _prompt_choice, _provider_env_ready, get_moor_subscription_features
    ...
    # Logged-in Moor users get a marker on rows included in their subscription (cost-extra vs. included).
    _moor_logged_in = False
    if not reconfigure:
        try:
            _moor_logged_in = bool(get_moor_subscription_features(config, force_fresh=force_fresh).moor_auth_present)
        except Exception:
            pass
    ...
    for p in providers:
        ...
        sub_marker = ""
        if not reconfigure and p.get("managed_moor_feature"):
            sub_marker = "  ★ Included with your Moor subscription" if _moor_logged_in else "  ★ via Moor Portal (login on select)"
...
def _managed_provider_active(provider: dict, config: dict, managed_feature: str, force_fresh: bool) -> bool:
    """Active check for a Moor-managed row: the feature must be managed AND the category's selected
    provider must be the row's vendor or ``moor``."""
    from moor_cli.tools_config import get_moor_subscription_features

    feature = get_moor_subscription_features(config, force_fresh=force_fresh).features.get(managed_feature)
    if feature is None:
        return False
    ...
    for marker, section, key in _MANAGED_SELECTION_KEYS:
        if _has_marker(provider, marker):
            ...
            selected = current in {provider[marker], MOOR_MANAGED_PROVIDER}
            if marker == "web_backend":
                selected = selected and _web_tier_matches(provider, config)
            return feature.managed_by_moor and selected
    return feature.managed_by_moor
...
def _detect_active_provider_index(providers: list, config: dict, *, force_fresh: bool = False) -> int:
    ...
    from moor_cli.tools_config import _provider_env_ready
...
def _pick_model_from_catalog(
    ...
    selection."""
    from moor_cli.tools_config import _cfg_section, _prompt_choice
...
def _select_into(config: dict, section: str, key: str, vendor, managed) -> dict:
    """Write ``config[section][key] = vendor`` (``moor`` for a managed pick) and drop any legacy ``use_gateway``
    key so the old read-time shim cannot override the new choice. Returns the section dict."""
    from moor_cli.tools_config import _cfg_section

    cfg = _cfg_section(config, section)
    cfg[key] = MOOR_MANAGED_PROVIDER if managed else vendor
...
def _select_plugin_gen_provider(section: str, plugin_name: str, config: dict, *, use_gateway: bool = False) -> None:
    """Persist a plugin-backed image/video gen provider selection (``moor`` for a Moor-managed pick, else the
...
def _write_provider_config(provider: dict, config: dict, *, managed_feature) -> None:
    ...
    Pure, non-interactive core of :func:`_configure_provider` (no env prompts, post-setup hooks, Moor
    auth gating or model pickers) shared by the CLI and the GUI ``PUT .../provider`` endpoint. Each pick
    writes exactly ONE provider string per category (``moor`` for managed rows) and removes any legacy
    ``use_gateway`` key so the read-time shim cannot override the new choice."""
    from moor_cli.tools_config import TOOL_CATEGORIES
    ...
    if managed_feature and managed_feature not in {"web", "tts", "stt", "browser"}:
        # Managed rows without a marker above (image_gen/video_gen "Moor Subscription" rows carry only
...
def apply_provider_selection(ts_key: str, provider_name: str, config: dict) -> None:
    ...
    # GUI flow); managed picks store "moor". The in-tree FAL BYOK row always persists an explicit
...
def _moor_provider_gate(provider: dict, config: dict, managed_feature, *, force_fresh: bool) -> bool:
    """Return False (after printing why) when a Moor-gated row cannot be selected.
    Managed Tool Gateway rows are always listed but only *activate* with paid Moor Portal access —
    selecting one runs an inline Portal login (auth + entitlement only, no inference-provider switch).
    Pure pre-auth UX rows (``requires_moor_auth`` without a managed feature) keep the older logged-in +
    entitled gate."""
    from moor_cli.tools_config import get_moor_subscription_features

    if managed_feature:
        from moor_cli.moor_subscription import ensure_moor_portal_access
        ...
        if not ensure_moor_portal_access(
            ...
            _print_warning("  Not enabled — Moor Portal access is required for this backend.")
            return False
        return True

    if provider.get("requires_moor_auth"):
        features = get_moor_subscription_features(config, force_fresh=force_fresh)
        ...
        if not features.moor_auth_present or not entitled:
            message = format_moor_portal_entitlement_message(
                features.account_info, capability=f"{provider.get('name', 'Moor Subscription')}")
            _print_warning(f"  {message or 'Moor Subscription is only available after logging into Moor Portal.'}")
            return False
    return True
...
def _finish_provider_selection(provider: dict, config: dict, managed_feature) -> None:
    ...
    if backend:
        ...
        # In-tree FAL is the only non-plugin backend: "moor" for a managed row, "fal" for BYOK, drop legacy
...
def _show_portal_hint(provider: dict, config: dict, managed_feature, force_fresh: bool) -> bool:
    """True when a BYOK row shares its category with a Moor-managed sibling and the user is not authed to
    Moor — a single dim hint tells them the key is avoidable via a Portal subscription."""
    from moor_cli.tools_config import TOOL_CATEGORIES, get_moor_subscription_features

    if managed_feature or provider.get("requires_moor_auth"):
        return False
    try:
        for _cat in TOOL_CATEGORIES.values():
            _providers = _cat.get("providers", [])
            if provider in _providers and any(sib.get("managed_moor_feature") for sib in _providers):
                return not get_moor_subscription_features(config, force_fresh=force_fresh).moor_auth_present
    except Exception:
        pass
    return False
...
def _configure_provider(provider: dict, config: dict, *, force_fresh: bool = True, reconfigure: bool = False):
    ...
    hook."""
    from moor_cli.tools_config import _run_post_setup

    env_vars = provider.get("env_vars", [])
    managed_feature = provider.get("managed_moor_feature")

    if not _moor_provider_gate(provider, config, managed_feature, force_fresh=force_fresh):
        return
...
def _configure_vision_backend() -> None:
    ...
    Offers any authenticated provider + model (same surface as ``moor model``) or a custom endpoint
...
def _configure_vision_provider_model(config: dict, vision_cfg: dict) -> None:
    ...
    Rows come from ``build_aux_picker_rows()`` so this lists exactly what the ``moor model`` aux-task
    picker lists, including user-defined ``providers:`` / ``custom_providers:`` endpoints. Persists
    ``auxiliary.vision.provider`` + ``.model``."""
    from moor_cli.tools_config import _prompt_choice
...
def _configure_simple_requirements(ts_key: str, *, reconfigure: bool = False):
    ...
    from moor_cli.tools_config import TOOLSET_ENV_REQUIREMENTS, _toolset_has_keys, _toolset_label
