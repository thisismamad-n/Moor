"""Default configuration data for Moor Agent.

Pure-data leaf module: DEFAULT_CONFIG and OPTIONAL_ENV_VARS, extracted
verbatim from moor_cli/config.py. Must not import from moor_cli.config.
"""
...
DEFAULT_CONFIG = {
    ...
    "toolsets": ["moor-cli"],
    # SQLite journal mode used by every Moor database opener. WAL is the
    ...
    # Soft file-descriptor limit for long-running Moor server processes.
    ...
    "session": {
        # Per-terminal `moor -c`: each CLI session drops a breadcrumb file
        # under $MOOR_HOME/terminal-sessions/<terminal-id>, and a bare
    ...
    "agent": {
        ...
        # all normalized by moor_cli.config.resolve_turn_limit.
        "max_turns": None,
        ...
        # budget. CLI one-shot equivalent: `moor chat --run-budget N`.
        ...
        # an interactive `moor gateway restart` must never block for hours
        ...
        # the moor-level retry loop that wraps the whole call.  Lower
        ...
        # prompt's environment-hints block. Lets a host that wraps Moor
        ...
        # identity slot (SOUL.md). Empty by default. The MOOR_ENVIRONMENT_HINT
        # env var overrides this (build-time/container mechanism).
        "environment_hint": "",
        # Coding posture — on interactive coding surfaces (CLI, TUI, desktop
        # app, ACP) in a code workspace, Moor adds a coding operating brief
        ...
        # `moor status` / fleet monitoring). Retries never stop — this is a
        ...
        # ``MOOR_LOCAL_STREAM_STALE_TIMEOUT`` overrides for escape-hatch use.
    ...
    "terminal": {
        ...
        # bot_relay from a short-lived `moor -p <bot> chat -Q` recipient
        ...
        #             MOOR_HOME/home for persistent state (default)
        #   real    — force the real OS-user HOME
        #   profile — force MOOR_HOME/home when it exists (old strict
        ...
        # skipped. When empty, Moor auto-sources ``~/.profile``,
        # ``~/.bash_profile``, and ``~/.bashrc`` (in that order) if the
        # snapshot shell is bash (this is the ``auto_source_bashrc``
        # behaviour — disable with that key if you want strict login-only
        # semantics).
        "shell_init_files": [],
        # When true (default), Moor sources the user's shell rc files
        ...
        # docker_env lets you specify exact key-value pairs — useful when Moor
        ...
        # bundled Moor image, which drops to the `moor` user via
    ...
    "web": {
        ...
        # firecrawl, keenable). Set by the `moor tools` picker's
    ...
    "browser": {
        ...
        "camofox": {
            # When true, Moor sends a stable profile-scoped userId to Camofox
            # so the server maps it to a persistent Firefox profile automatically.
            # When false (default), each session gets a random userId (ephemeral).
            "managed_persistence": False,
            # Optional externally managed Camofox identity. Useful when another
            # app owns the visible browser and Moor should operate in it.
            "user_id": "",
            "session_key": "",
    ...
    # Opt in via ``moor chat --checkpoints`` or set enabled=True here.
    "checkpoints": {
        "enabled": False,
        ...
        "max_snapshots": 20,
        # Hard ceiling on total ``~/.moor/checkpoints/`` size (MB).  When
        ...
        "max_file_size_mb": 10,
        # Auto-maintenance: moor sweeps the checkpoint base at startup
        ...
        # ``moor checkpoints prune`` command (add ``--keep-orphans`` to
    ...
    # AGENTS.md, CLAUDE.md, .moor.md, or .cursorrules before Moor applies
    ...
    # Single-query (``moor -q/-z "..."``) variant of mcp_discovery_timeout.
    ...
    # single read_file page exceeds these limits, Moor truncates the
    ...
    "compression": {
        ...
        "codex_app_server_auto": "native",  # Codex app-server (codex CLI runtime) thread
                                      ...
                                      # thread context, so Moor' summarizer cannot
                                      # shrink it (#36801). native = codex decides when
                                      # to compact its own thread (default); moor =
                                      # Moor' compression threshold triggers
        ...
        "codex_responses_native": False,  # Opt in to OpenAI's server-side compaction
                                      ...
                                      # route/model is unaffected. Moor' local
        ...
        "codex_responses_compact_threshold": 200000,  # Server-side compaction trigger
                                      ...
                                      # the server compacts before Moor does.
    ...
    "auxiliary": {
        ...
        "vision": {
            "provider": "auto",    # auto | openrouter | moor | codex | custom
        ...
        "review": {
            "provider": "auto",    # auto (= inherit main model) | openrouter | moor | anthropic | ...
        ...
        # Invoked by ``moor kanban specify`` (single id or --all). Set a
        ...
        # Invoked by ``moor kanban decompose`` and the kanban
        ...
        # ``moor profile describe <name> --auto`` and the dashboard's
        ...
        # model; override via `moor model` → auxiliary → Curator to route
    ...
    "display": {
        ...
        # Which interface bare `moor` (and `moor chat`) launches by default:
        ...
        # REPL and `--tui` (or MOOR_TUI=1) forces the TUI regardless of config.
        "interface": "cli",
        # When true, `moor --tui` auto-resumes the most recent human-
        # facing session on launch instead of forging a fresh one.
        # Mirrors `moor -c` muscle memory.  Default off so existing
        # users aren't surprised.  MOOR_TUI_RESUME=<id> always wins.
        "tui_auto_resume_recent": False,
        # When true (default), `moor --tui` drops a one-time hint
        ...
        # Moor credits status-bar notices (usage bands, grant-spent, depleted /
        ...
        # model — see moor_cli/focus_view.py.
        "focus_view": False,
        ...
        # can set `path`/`paths` to MOOR_HOME-relative YAML files/directories
        ...
        # e.g. `model · 68% · ~/projects/moor`. Per-platform overrides go under
        ...
        # CLI, TUI, and desktop app. Manage with `moor pets`. Disabled until
        ...
        "pet": {
            ...
            # get_moor_home()/pets/. Empty → first installed pet.
            "slug": "",
    ...
    "dashboard": {
        ...
        # protocol ping (see moor_cli/web_server.py — an event-loop stall
        ...
        # MOOR_TUI_WS_ORPHAN_REAP_GRACE_S env var remains an internal
        ...
        # (MOOR_TUI_SESSION_TTL_S, default 6h) are closed with
        ...
        # ``--insecure`` is not). The bundled Moor Portal plugin reads
        # both keys at startup; they are the canonical surface for these
        # settings. Each can be overridden by an environment variable —
        # ``MOOR_DASHBOARD_OAUTH_CLIENT_ID`` and
        # ``MOOR_DASHBOARD_PORTAL_URL`` respectively — and the env var
        ...
        # (``MOOR_DASHBOARD_BASIC_AUTH_USERNAME`` /
        ...
        # moor-account-service at deploy time via the
        # ``MOOR_DASHBOARD_DRAIN_SECRET`` env var (the .env-is-for-secrets
        ...
        # Public URL override (env: ``MOOR_DASHBOARD_PUBLIC_URL``).
        # When set, this is the complete authority — scheme + host +
        # optional path prefix (e.g. ``https://example.com/moor``) —
    ...
    "tts": {
        ...
        "gemini": {
            ...
            # transcript section; Moor appends the live transcript when absent.
            "persona_prompt_file": "",
        },
        "xai": {
        ...
        "piper": {
            ...
            # "voices_dir": "",        # Override voice cache dir; default = ~/.moor/cache/piper-voices/
    ...
    # "Hey Moor" hands-free wake word. Always-on, on-device hotword
    ...
    "wake_word": {
        ...
        "phrase": "hey moor",       # for "sherpa" this IS the detected phrase (any text works); for other engines it's a cosmetic label — detection is keyed by the model/keyword below
        ...
        "openwakeword": {
            # "hey_moor" (the bundled, works-out-of-the-box default) OR a
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.moor/plugins/.
    "context": {
        "engine": "compressor",
    ...
    # configured providers (OpenRouter, Moor, Z.ai, Kimi, etc.) are supported.
    "delegation": {
        ...
        # trim, the full text is spilled to ~/.moor/cache/delegation/
    ...
    # response. If not, Moor feeds a continuation prompt back into the
    ...
    "goals": {
        # Max continuation turns before Moor auto-pauses the goal and
    ...
    "moa": {
        ...
        # <moor_home>/moa-traces/<session_id>.jsonl. Off by default — turn it
    ...
    # always goes to ~/.moor/skills/.
    "skills": {
        "external_dirs": [],   # e.g. ["~/.agents/skills", "/shared/team-skills"]
        # Project-local skill discovery: when a session starts inside a git
        # checkout, ``<root>/.moor/skills/`` and ``<root>/.agents/skills/``
        ...
        # with ``moor skills trust`` (run from inside it). Set to false to
        ...
        # Managed by ``moor skills trust`` / ``moor skills untrust``.
        "trusted_project_dirs": [],
        # Substitute ${MOOR_SKILL_DIR} and ${MOOR_SESSION_ID} in SKILL.md
        ...
        # (`moor skills install`). Runs ALONGSIDE the built-in skills
        ...
        # ~/.moor/skills/.curator_ledger.jsonl with before/after file
        # hashes; file contents are stored content-addressed (deduped) under
        # ~/.moor/.curator_backups/blobs/. Enables `moor curator ledger`
        # and single-mutation `moor curator rollback <entry-id>`.
    ...
    # cron daemon.
    #
    # See `moor curator status` for the last run summary.
    "curator": {
        "enabled": True,
        ...
        # class-level umbrellas. `moor curator run --consolidate` overrides
        ...
        # normally restored on every `moor update`, so pruning them only
        ...
        # skills are kept forever. When > 0, `moor curator purge` deletes
        ...
        # skipped), snapshot ~/.moor/skills/ into
        # ~/.moor/skills/.curator_backups/<utc-iso>/skills.tar.gz so the
        # user can roll back with `moor curator rollback`.
        "backup": {
            "enabled": True,
    ...
    # This section is only needed for moor-specific overrides; everything else
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "⚕ *Moor Agent*" header.
    ...
    # dangerous command. -q runs export MOOR_INTERACTIVE=1 (for interactive
    ...
    "approvals": {
        ...
        # modal; MOOR_TUI_NO_CONFIRM=1 force-skips that modal regardless of
    ...
    # replace Moor' built-in platform hint for a single messaging platform
    ...
    # stored approval from ~/.moor/shell-hooks-allowlist.json.
    # See `website/docs/user-guide/features/hooks.md` for schema + examples.
    "hooks": {},

    # Auto-accept shell-hook registrations without a TTY prompt.  Also
    # toggleable per-invocation via --accept-hooks or MOOR_ACCEPT_HOOKS=1.
    ...
    "security": {
        ...
        # .cursorrules, project-local .moor config) always require human
        ...
        # longer trigger the startup banner. Add via `moor doctor --ack
        # <id>`; remove by editing the list directly. See
        # ``moor_cli/security_advisories.py`` for the catalog.
        "acked_advisories": [],
        # Allow Moor to lazy-install opt-in backend packages from PyPI
    ...
    "cron": {
        ...
        # $MOOR_HOME/plugins/<name>/) to relocate the trigger — e.g. "chronos",
        ...
        # agent's existing Moor Portal token — there is no token key here.
        "chronos": {
        ...
        # Also overridable via MOOR_CRON_MAX_PARALLEL env var.
        ...
        # MOOR_CRON_SCRIPT_TIMEOUT. Keep this in sync with
        ...
        # MOOR_CRON_SESSION_DB_TIMEOUT env var. 0 = unlimited (skip the bound).
        ...
        # overridable via MOOR_CRON_MEDIA_SEND_TIMEOUT env var. Keep in
    ...
    # todos to ready, and fires `moor -p <assignee> chat -q ...` for
    ...
    "kanban": {
        ...
        # one `moor` launches with no -p flag). This does not control the
        ...
        # decomposition is manual via `moor kanban decompose <id>` or
    ...
    "code_execution": {
        ...
        #     moor-agent's own python (sys.executable). Maximum isolation
    ...
    #
    # Core Moor tools (terminal, read_file, write_file, patch,
    ...
    # Logging — controls file logging to ~/.moor/logs/.
    ...
    # curated model lists for OpenRouter and Moor Portal from this URL,
    # falling back to the in-repo snapshot on network failure.  Lets us
    # update model picker lists without shipping a moor-agent release.
    # The default URL is served by the docs site GitHub Pages deploy.
    "model_catalog": {
        "enabled": True,
        "url": "https://hermes-agent.nousresearch.com/docs/api/model-catalog.json",
        # Disk cache TTL in hours.  Beyond this, the CLI refetches on the
        # next /model or `moor model` invocation; network failures
    ...
    "gateway": {
        ...
        # internal MOOR_GATEWAY_PLATFORM_CONNECT_TIMEOUT env var, which still
        ...
        # producing ~/.moor/sessions/sessions.json entirely.
        ...
        # the MOOR_SCALE_TO_ZERO env stamp) AND messaging is relay-only/absent
        ...
        # `launchctl kickstart ai.moor.gateway` that defenses 1-2 don't
        ...
        # `moor update` hang because it can never drain the gateway.
        "restart_loop_guard": {
            "max_restarts": 3,
            "window_seconds": 60,
            "max_gap_seconds": 300,
        ...
        # ``MOOR_GATEWAY_MAX_STARTS`` / ``MOOR_GATEWAY_START_WINDOW_S``
        ...
        # system-path denylist (/etc, /proc, ~/.ssh, ~/.aws, ~/.moor/.env,
        ...
        # behavior: files must live under the Moor cache, under
        ...
        # user. Bridged to MOOR_MEDIA_DELIVERY_STRICT.
        "strict": False,
        # Extra directories from which model-emitted bare file paths may be
        # uploaded as native gateway attachments. Files inside the Moor
        # cache (~/.moor/cache/{documents,images,audio,video,screenshots})
        ...
        # to MOOR_MEDIA_ALLOW_DIRS at gateway startup. Tilde paths are
        ...
        # MOOR_MEDIA_TRUST_RECENT_FILES. Only consulted when ``strict``
        # is true; in default mode the denylist alone gates delivery.
        "trust_recent_files": True,
        # Recency window in seconds. 600 (10 min) comfortably covers a
        # multi-tool agent turn. Bridged to MOOR_MEDIA_TRUST_RECENT_SECONDS.
    ...
    # Session storage — controls automatic cleanup of ~/.moor/state.db.
    ...
    "sessions": {
        ...
        # the default of ``moor sessions prune``.
        "retention_days": 90,
        ...
        # rewrites ``~/.moor/sessions/session_{sid}.json`` on every turn
        ...
        # user runs `moor sessions optimize-storage`, because the rebuild is
        # disk-heavy and long on large DBs (see that command's disk preflight).
        #
        #   "advise" (default): `moor update` prints a one-line notice with
        ...
        # ~/.moor/lib/libfts5_cjk.so), 1-2 char CJK terms (일본, 项目, ...)
        ...
        # serve the cjk index. Bridged to MOOR_CJK_FTS (internal carrier).
        "cjk_fts": True,
        ...
        # MOOR_SEARCH_SLOW_MS (internal carrier).
        "search_slow_ms": 1000,
        ...
        # (non-streaming) export such as `moor sessions export`. Checked
    ...
    # ``moor doctor`` behaviour.
    "doctor": {
        # Per-probe timeout (seconds) for the opt-in `moor doctor --live`
        # real-call backend probes (Firecrawl/FAL/browser/MCP/TTS/STT).
        "live_probe_timeout": 10,
    },

    # ``moor update`` behaviour.
    "updates": {
        ...
        #     DBs) into <MOOR_HOME>/state-snapshots/ before the update.
        ...
        #     jobs) safety net.
        #   full — the quick snapshot PLUS a full ``moor backup``-style zip
        #     of MOOR_HOME into <MOOR_HOME>/backups/, restorable with
        #     ``moor import``. Can add minutes on large homes. This is the
        ...
        "backup_keep": 5,
        # What `moor update` does with uncommitted local changes to the
        ...
        # When `moor update` finds the source checkout parked on a feature
        ...
        #                         merge. `moor update --switch-branch`
    ...
    "lsp": {
        ...
        #              ``<MOOR_HOME>/lsp/bin/`` on first use.
    ...
    # `moor tools`. These settings tune the backing Responses API call.
    "x_search": {
    ...
    # rather than storing them in ~/.moor/.env.
    "secrets": {
        ...
        "bitwarden": {
            ...
            # in ~/.moor/.env (or your shell) and never in config.yaml.
            ...
            # material under ~/.moor/cache/. If a later startup cannot reach
            # Bitwarden due to NETWORK/TIMEOUT, Moor may use this encrypted
            ...
            # ~/.moor/bin/ on first use.  When False you must install
        ...
        "onepassword": {
            ...
            # for headless auth.  Sourced from ~/.moor/.env (or the shell)
    ...
    "computer_use": {
        ...
        # by default upstream. Moor disables it for our users unless they
        # explicitly opt in here. When false (default), Moor sets
        ...
        # cua-driver ≥ 0.6.x supports --no-overlay; Moor also calls
        ...
        "no_overlay": None,
        # cua-driver permission mode for each Moor computer-use runtime.
        ...
        # manifest used when permission_mode is "bounded". Moor passes the
    ...
    #
    # Configure with `moor egress setup`.  Disabled by default — the rest of
    # Moor works exactly as before with `enabled: false`.
    "proxy": {
        ...
        "tunnel_port": 9090,
        # Auto-download the pinned iron-proxy binary into ~/.moor/bin/ on
        ...
        # Together, DeepSeek, Moor).  Wildcards (`*.foo.com`) are supported.
        "extra_allowed_hosts": [],
    },

    # Moor Desktop (Electron app) launch options. These only affect
    # `moor desktop`; they do not touch the CLI/gateway.
    "desktop": {
        ...
        # Bridged to the MOOR_DESKTOP_DISABLE_GPU env var the Electron app reads.
        "disable_gpu": "auto",
        ...
        # Ignored on macOS/Windows. Bridged to the MOOR_DESKTOP_PASSWORD_STORE
...
OPTIONAL_ENV_VARS = {
    ...
    "MOOR_BASE_URL": {
        "description": "Moor Portal base URL override",
        "prompt": "Moor Portal base URL (leave empty for default)",
        "url": None,
        "password": False,
        "category": "provider",
        "advanced": True,
    ...
    "VERTEX_CREDENTIALS_PATH": {
        "description": "Path to a Google Cloud service account JSON for Vertex AI (Gemini). "
                       ...
                       "credentials Moor mints short-lived tokens from. Falls back to "
    ...
    "AZURE_FOUNDRY_BASE_URL": {
        "description": "Azure Foundry base URL (set via 'moor model' for endpoint-specific config)",
    ...
    "FIRECRAWL_GATEWAY_URL": {
        "description": "Exact Firecrawl tool-gateway origin override for Moor Subscribers only (optional)",
    ...
    "TOOL_GATEWAY_DOMAIN": {
        "description": "Shared tool-gateway domain suffix for Moor Subscribers only, used to derive vendor hosts, e.g. nousresearch.com -> firecrawl-gateway.nousresearch.com",
    ...
    "TOOL_GATEWAY_SCHEME": {
        "description": "Shared tool-gateway URL scheme for Moor Subscribers only, used to derive vendor hosts (`https` by default, set `http` for local gateway testing)",
    ...
    "TOOL_GATEWAY_USER_TOKEN": {
        "description": "Explicit Moor Subscriber access token for tool-gateway requests (optional; otherwise read from the Moor auth store)",
    ...
    "PORCUPINE_ACCESS_KEY": {
        "description": "Picovoice access key for the Porcupine 'Hey Moor' wake word engine (optional; openWakeWord is the free default)",
    ...
    "SLACK_ALLOWED_USERS": {
        "description": "Comma-separated Slack member IDs allowed to use Moor, e.g. U01ABC2DEF3. Without this, Slack may connect but deny messages by default.",
    ...
    "MATRIX_USER_ID": {
        "description": "Matrix user ID (e.g. @moor:example.org)",
    ...
    "MATRIX_DEVICE_ID": {
        "description": "Stable Matrix device ID for E2EE persistence across restarts (e.g. MOOR_BOT)",
    ...
    "IRC_CHANNEL": {
        "description": "IRC channel to join (e.g. #moor)",
        "prompt": "IRC channel",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "IRC_NICKNAME": {
        "description": "Bot nickname on IRC (default: moor-bot)",
        "prompt": "IRC nickname",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "API_SERVER_MODEL_NAME": {
        "description": "Model name advertised on /v1/models. Defaults to the profile name (or 'moor-agent' for the default profile). Useful for multi-user setups with OpenWebUI.",
    ...
    "GATEWAY_PROXY_URL": {
        "description": "URL of a remote Moor API server to forward messages to (proxy mode). When set, the gateway handles platform I/O only — all agent work is delegated to the remote server. Use for Docker E2EE containers that relay to a host agent. Also configurable via gateway.proxy_url in config.yaml.",
        "prompt": "Remote Moor API server URL (e.g. http://192.168.1.100:8642)",
        "url": None,
        "password": False,
        "category": "messaging",
        "advanced": True,
    ...
    "GATEWAY_PROXY_KEY": {
        "description": "Bearer token for authenticating with the remote Moor API server (proxy mode). Must match the API_SERVER_KEY on the remote host.",
    ...
    # MOOR_TOOL_PROGRESS_MODE is deprecated — tool progress is configured via
    # display.tool_progress in config.yaml (off|new|all|verbose|log). The
    # gateway still falls back to MOOR_TOOL_PROGRESS_MODE for backward
    ...
    # MOOR_TOOL_PROGRESS is fully unsupported since the v12 config support
    # floor retired its only consumer (the v3→4 migration).
    "MOOR_PREFILL_MESSAGES_FILE": {
