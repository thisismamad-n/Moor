"""Default configuration data for Moor Agent: DEFAULT_CONFIG and OPTIONAL_ENV_VARS.

Pure-data leaf module — must not import from moor_cli.config. Comments are the user-facing
docs of config.yaml.
"""
...
DEFAULT_CONFIG = {
    ...
    "toolsets": ["moor-cli"],
    # journal_mode: SQLite journal mode for every Moor DB. "wal" default; use "delete" on
    ...
    "session": {
        # Per-terminal `moor -c`: each CLI session writes a breadcrumb under
        # $MOOR_HOME/terminal-sessions/<terminal-id>, so bare -c/--continue resumes THIS
    ...
    "agent": {
        ...
        # Default 30 min is a safety valve for wedged agents, not a target latency — an interactive `moor
        ...
        # moor-level retry attempts for API errors (connection drops, timeouts, 5xx) wrapping the
        ...
        # wrapping Moor (sandbox runner, managed platform) can describe proxy/credential/ mount
        # layout without editing SOUL.md. Env MOOR_ENVIRONMENT_HINT overrides it.
        "environment_hint": "",
        ...
        # share or `moor doctor` (status channel only; prompt cache untouched). 0 = no escalation
        ...
        # gateway status (`moor status` / fleet monitoring). Retries never stop — a signal, not a
        ...
        # MOOR_LOCAL_STREAM_STALE_TIMEOUT overrides.
    ...
    "terminal": {
        ...
        # sandboxes). Empty = TMPDIR/TMP/TEMP if set, else MOOR_HOME/cache/terminal (auto-pruned
        ...
        # ["/home/user/.moor/cache/documents:/output"]. For gateway MEDIA delivery, write to
        ...
        # default for images whose entrypoints must start as root (e.g. the bundled Moor image,
        # which drops to `moor` via s6-setuidgid). When on, SETUID/SETGID caps are omitted.
    ...
    "web": {
        ...
        # parallel, firecrawl, keenable; tavily is opt-in keyless via `moor tools`, not a ring
        # member). Set by the `moor tools` picker. "free" = always anonymous endpoint even with a
    ...
    "browser": {
        ...
        # Consent to browse with the user's REAL logins locally: runs on a moor-managed SNAPSHOT
        # of the ACTIVE default-Chromium profile (Local State -> profile.last_used; cookies, logins,
        # prefs copied and re-synced per fresh session) driven by Moor' packaged Chromium. The
        # snapshot dir sidesteps Chrome 136+'s default-profile debugging block and never contends
        # with the running browser. Turning off deletes ~/.moor/browser-profile/ so credentials
    ...
    # write_file/patch call); restore with /rollback. Opt-in via `moor chat --checkpoints` or
    ...
    "checkpoints": {
        ...
        "max_snapshots": 20,
        # Hard ceiling on total ~/.moor/checkpoints/ size (MB); the oldest checkpoint per project
        ...
        # must not guess. Orphans: `moor checkpoints prune` (`--keep-orphans` to skip).
        "auto_prune": True,
        "retention_days": 7,
        "min_interval_hours": 24,
    },
    # Hard cap (chars) for one auto-loaded context file (SOUL.md, AGENTS.md, CLAUDE.md, .moor.md,
    ...
    # Same bound for single-query mode (``moor -q/-z``). With only ONE turn there is no
    ...
    "compression": {
        ...
        # Moor' summarizer cannot shrink it. native = codex decides; moor = Moor' threshold
    ...
    "auxiliary": {
        ...
        # of skills); route cheaper via `moor model` → auxiliary → Curator.
        "curator": _aux(600),
    ...
    "display": {
        ...
        # Interface bare `moor`/`moor chat` launches: "cli" (prompt_toolkit REPL) | "tui" (Ink).
        # Flags win: `--cli` forces the REPL, `--tui` / MOOR_TUI=1 forces the TUI.
        "interface": "cli",
        # `moor --tui` auto-resumes the most recent human-facing session (like `moor -c`).
        # MOOR_TUI_RESUME=<id> always wins.
        ...
        # Moor credits status-bar notices (usage bands, grant-spent, depleted/restored). False mutes
        ...
        # `path`/`paths` = MOOR_HOME-relative YAML files/dirs (or conventional status_phrases.yaml
        ...
        # Gateway runtime footer on the FINAL message, e.g. `model · 68% · ~/projects/moor`.
        ...
        # CLI/TUI/desktop, managed with `moor pets`. No effect on prompt caching.
        "pet": {
            "enabled": False,
            "slug": "",   # active pet slug in get_moor_home()/pets/; empty → first installed
    ...
    "dashboard": {
        ...
        # client disconnects. 0 = park forever. Env: MOOR_TUI_WS_ORPHAN_REAP_GRACE_S.
        ...
        # newest message older than MOOR_TUI_SESSION_TTL_S, default 6h) with
        ...
        # OAuth gate (engaged when --host is set and --insecure is not), read by the Moor Portal
        # plugin. Env MOOR_DASHBOARD_OAUTH_CLIENT_ID / MOOR_DASHBOARD_PORTAL_URL win when
        ...
        # no-op. Env MOOR_DASHBOARD_BASIC_AUTH_USERNAME / _PASSWORD_HASH / _PASSWORD / _SECRET /
        ...
        # MOOR_DASHBOARD_DRAIN_SECRET; no-op unless >=256-bit, weak secrets rejected
        # (fail-closed). scope = capability label; min_secret_chars in url-safe-b64 chars.
        "drain_auth": {"scope": "drain", "min_secret_chars": 43},
        # Public URL (env MOOR_DASHBOARD_PUBLIC_URL): full authority (scheme + host + optional
        # prefix, e.g. https://example.com/moor) for the OAuth redirect_uri; its hostname is
    ...
    "tts": {
        ...
        "piper": {
            ...
            # (~/.moor/cache/piper-voices/), use_cuda, length_scale (2.0 = twice as slow),
    ...
    # plugins/context_engine/<name>/ or ~/.moor/plugins/.
    "context": {
        "engine": "compressor",
    ...
    "delegation": {
        ...
        # ~/.moor/cache/delegation/ with a head+tail window + read_file offset footer, nothing
    ...
    "moa": {
        ...
        # <moor_home>/moa-traces/<session_id>.jsonl (or trace_dir) for auditing.
        "save_traces": False,
        "trace_dir": "",
    ...
    # and resolved; read-only — creation goes to ~/.moor/skills/ unless create_dir redirects it.
    "skills": {
        ...
        # MOOR_HOME, scanned alongside the local dir.
        "create_dir": "",
        # In a git checkout, <root>/.moor/skills/ and <root>/.agents/skills/ load as the
        # highest-precedence tier — ONLY if the root is in trusted_project_dirs. false = no scan, no
        # untrusted-skills notice.
        "project_discovery": True,
        # Trusted project roots; managed by `moor skills trust` / `untrust`.
        ...
        # Advisory NVIDIA SkillEvaluator Tier 1 scan on `moor skills install` (alongside the
        ...
        "write_approval": False,
        # Audit ledger: every skill mutation appends to ~/.moor/skills/.curator_ledger.jsonl with
        # before/after hashes (blobs under ~/.moor/.curator_backups/blobs/); powers `moor
    ...
    # daemon. `moor curator status` shows the last run.
    "curator": {
        "enabled": True,
        ...
        "archive_ttl_days": 0,
        # Before every real (non-dry-run) pass, snapshot ~/.moor/skills/ to
        # ~/.moor/skills/.curator_backups/<utc-iso>/skills.tar.gz (`moor curator rollback`).
        "backup": {
            "enabled": True,
    ...
    # peerName, sessions, enabled); moor-specific overrides only here.
    "honcho": {},
    ...
    #   cron job, a -q session (MOOR_INTERACTIVE=1 but nobody to answer), or an unattended
    ...
    "approvals": {
        ...
        # export MOOR_INTERACTIVE=1 (for interactive sudo prompts) but have NO user waiting to answer
        ...
        # Approve" → false. MOOR_TUI_NO_CONFIRM=1 skips the TUI modal.
    ...
    # are given.
    "platform_hints": {},
    # Plugin system. `enabled`/`disabled` lists are written by `moor plugins enable|disable` and
    ...
    "plugins": {
        ...
        # 2026-09-14 removal date (see COMPAT_MANIFEST.md, `moor plugins compat`). Stopgap only: the
    ...
    # approvals persist in ~/.moor/shell-hooks-allowlist.json. Schema + examples:
    # website/docs/user-guide/features/hooks.md.
    "hooks": {},
    # Auto-accept shell-hook registrations without a TTY prompt (also --accept-hooks or
    # MOOR_ACCEPT_HOOKS=1). Gateway/cron/non-interactive runs need one of these to pick up
    # newly-added hooks.
    ...
    "security": {  # Security: pre-exec scanning via tirith plus related guards.
        ...
        # .moor config) always need human approval, even under yolo. Extra patterns are fnmatch
        ...
        # banner. Add via `moor doctor --ack <id>`; remove by editing the list. Catalog:
        # moor_cli/security_advisories.py.
        "acked_advisories": [],
    ...
    "cron": {
        ...
        # an installed provider (plugins/cron_providers/<name>/ or $MOOR_HOME/plugins/ <name>/),
        ...
        # holds NO scheduler credentials (provision reuses the Moor Portal token).
        "chronos": {
        ...
        # Env override: MOOR_CRON_MAX_PARALLEL.
        ...
        "output_retention": 50,
        # Timeout (seconds) for a no-agent cron script. Env: MOOR_CRON_SCRIPT_TIMEOUT. Keep in
        ...
        # job's dispatch guard forever. Env: MOOR_CRON_SESSION_DB_TIMEOUT. 0 = unlimited.
        "session_db_timeout_seconds": 10,
        # Timeout (seconds) per media attachment send during gateway delivery; large attachments
        # (long TTS audio, big exports) need more than 30s. Env: MOOR_CRON_MEDIA_SEND_TIMEOUT.
    ...
    # promotes dependency-satisfied todos to ready, and fires `moor -p <assignee> chat -q ...` per
    ...
    "code_execution": {  # execute_code settings (programmatic tool calls).
        ...
        # relative paths resolve. strict = isolated temp dir with moor-agent's own python
    ...
    # by tool_search / tool_describe / tool_call bridges and surfaced on demand. Core Moor tools
    ...
    "logging": {  # File logging to ~/.moor/logs/: agent.log captures INFO+, errors.log WARNING+.
        "level": "INFO",       # minimum level for agent.log: DEBUG, INFO, WARNING
        "max_size_mb": 5,      # max size per log file before rotation
        "backup_count": 3,     # rotated backups to keep
    },
    # Remote model-catalog manifest: curated OpenRouter / Moor Portal model lists fetched from this
    ...
    "model_catalog": {
        "enabled": True,
        "url": "https://raw.githubusercontent.com/thisismamad-n/Moor/master/website/static/api/model-catalog.json",
        # Disk cache TTL in minutes. The gateway refreshes in the background on this cadence; the
        # CLI refetches on the next /model or `moor model` once the cache is older. Network
    ...
    # defaults (200K context, tools on, vision/reasoning off) and get patched. Provider keys: Moor
    ...
    "monitoring": {
        ...
        "gateway_health_export": {  # Gateway health & diagnostics export.
            ...
            "resource_attributes": {
                "service.name": "moor-gateway", "deployment.environment.name": "production"
            },
        },
    ...
    "gateway": {  # Gateway settings (messaging platforms: Telegram, Discord, Slack, ...).
        ...
        # to MOOR_GATEWAY_PLATFORM_CONNECT_TIMEOUT, which wins if set explicitly.
        ...
        # run_gateway() bridges these to MOOR_STARTUP_WATCHDOG / MOOR_STARTUP_WATCHDOG_TIMEOUT_S
        # and re-arms the live handle; explicit env wins.
        "startup_watchdog": True,
        "startup_watchdog_timeout_seconds": 300,
        # Keep writing the legacy ~/.moor/sessions/sessions.json mirror of the routing index
        ...
        # (MOOR_SCALE_TO_ZERO env stamp) AND messaging is relay-only/absent AND a wakeUrl is
        ...
        # credential/system denylist (/etc, /proc, ~/.ssh, ~/.aws, ~/.moor/.env, auth.json). True:
        # files must be under the Moor cache, media_delivery_allow_dirs, or fresher than
        # trust_recent_files_seconds — recommended for public-facing gateways so prompt injection
        # can't exfiltrate host secrets. Bridged to MOOR_MEDIA_DELIVERY_STRICT.
        "strict": False,
        # Extra roots (project/scratch dirs, mounted shares) from which bare file paths may be
        # uploaded; the Moor cache is always trusted. List of absolute paths or one
        # os.pathsep-separated string; tildes expanded. Bridged to MOOR_MEDIA_ALLOW_DIRS. Honored
        # in both modes.
        ...
        # pure-allowlist mode. Bridged to MOOR_MEDIA_TRUST_RECENT_FILES. Only consulted when
        # strict is true.
        "trust_recent_files": True,
        # Recency window in seconds; 600 covers a multi-tool turn. Bridged to
        # MOOR_MEDIA_TRUST_RECENT_SECONDS. Only consulted when strict is true.
    ...
    # Automatic cleanup of ~/.moor/state.db, which otherwise grows without bound and slows FTS5
    ...
    "sessions": {
        ...
        "auto_prune": True,
        # Inactive days of ended-session history to keep (= `moor sessions prune`).
        ...
        # stay until `moor sessions optimize-storage` runs, since the rebuild is disk-heavy on
        # large DBs. advise = `moor update` prints a one-line notice with reclaimable size when a
        ...
        # (native/fts5_cjk/build.sh → ~/.moor/lib/libfts5_cjk.so), 1-2 char CJK terms get exact
        # index matches instead of LIKE scans. True = use when present (inert otherwise); False =
        # never load/serve it. Bridged to MOOR_CJK_FTS.
        "cjk_fts": True,
        ...
        # Max active messages per session for in-memory export (`moor sessions export`); checked
    ...
    "doctor": {
        # Per-probe timeout (seconds) for `moor doctor --live` real-call probes.
    ...
    "updates": {
        ...
        # files >1 GiB; restore via ``/snapshot``. full = quick PLUS a ``moor backup`` zip in
        # <MOOR_HOME>/backups/ (``moor import`` restores; slow on large homes; ``--backup``
        ...
        # update cleanly. `moor update --switch-branch` overrides to switch for one run.
        "parked_branch_strategy": "switch",
        # Refresh an installed cua-driver during `moor update` (best-effort, macOS only). Turn off
    ...
    # (SuperGrok OAuth or XAI_API_KEY) AND the toolset is enabled in `moor tools`.
    "x_search": {
    ...
    "secrets": {
        ...
        "bitwarden": {
            ...
            # ~/.moor/.env (or the shell), never in config.yaml.
            ...
            # Last-good fallback for NETWORK/TIMEOUT outages: AES-GCM cache under ~/.moor/cache/,
            ...
            # Auto-download bws into ~/.moor/bin/ on first use; False = bws must be on PATH.
            "auto_install": True,
    ...
    "computer_use": {
        # cua-driver's upstream PostHog telemetry defaults ON; Moor sets
        ...
        # #47032; Linux/WSL2 idle spin #28152). cua-driver ≥ 0.6.x supports --no-overlay; Moor also calls
    ...
    "proxy": {
        ...
        "tunnel_port": 9090,
        # Auto-download the pinned binary into ~/.moor/bin/; False = iron-proxy on PATH.
        "auto_install": True,
    ...
    "desktop": {  # Moor Desktop (Electron) launch options; only affect `moor desktop`.
        ...
        # Bridged to MOOR_DESKTOP_DISABLE_GPU: auto = disable GPU only on remote displays
        ...
        # to MOOR_DESKTOP_PASSWORD_STORE; ignored off-Linux.
        "password_store": "auto",
    ...
    "moor": {
        # Upper bound (seconds) on the Moor auth keepalive tick, which derives from the
...
OPTIONAL_ENV_VARS = {
    ...
    "VERTEX_CREDENTIALS_PATH": _prov(
        ...
        "OAuth2, not a static API key — this points at the credentials Moor mints short-lived "
    ...
    "AZURE_FOUNDRY_BASE_URL": _prov(
        "Azure Foundry base URL (set via 'moor model' for endpoint-specific config)",
    ...
    "FIRECRAWL_GATEWAY_URL": _tool(
        "Exact Firecrawl tool-gateway origin override for Moor Subscribers only (optional)",
    ...
    "TOOL_GATEWAY_DOMAIN": _tool(
        "Shared tool-gateway domain suffix for Moor Subscribers only, used to derive vendor "
    ...
    "TOOL_GATEWAY_SCHEME": _tool(
        "Shared tool-gateway URL scheme for Moor Subscribers only, used to derive vendor hosts "
    ...
    "TOOL_GATEWAY_USER_TOKEN": _tool(
        "Explicit Moor Subscriber access token for tool-gateway requests (optional; otherwise "
        "read from the Moor auth store)", "Tool-gateway user token", None, advanced=True),
    ...
    "PORCUPINE_ACCESS_KEY": _tool(
        "Picovoice access key for the Porcupine 'Hey Moor' wake word engine (optional; "
    ...
    "MOOR_LANGFUSE_PUBLIC_KEY": _tool("Langfuse project public key (pk-lf-...)",
        "Langfuse public key", "https://cloud.langfuse.com", password=False),
    "MOOR_LANGFUSE_SECRET_KEY": _tool("Langfuse project secret key (sk-lf-...)",
        "Langfuse secret key", "https://cloud.langfuse.com"),
    "MOOR_LANGFUSE_BASE_URL": _tool("Langfuse server URL (default: https://cloud.langfuse.com)",
    ...
    "SLACK_ALLOWED_USERS": _msg(
        "Comma-separated Slack member IDs allowed to use Moor, e.g. U01ABC2DEF3. Without "
    ...
    "MATRIX_USER_ID": _msg("Matrix user ID (e.g. @moor:example.org)",
    ...
    "MATRIX_DEVICE_ID": _msg(
        "Stable Matrix device ID for E2EE persistence across restarts (e.g. MOOR_BOT)",
    ...
    "IRC_CHANNEL": _msg("IRC channel to join (e.g. #moor)", "IRC channel", None),
    "IRC_NICKNAME": _msg("Bot nickname on IRC (default: moor-bot)", "IRC nickname", None),
    ...
    "API_SERVER_MODEL_NAME": _msg(
        "Model name advertised on /v1/models. Defaults to the profile name (or 'moor-agent' "
    ...
    "GATEWAY_PROXY_URL": _msg(
        "URL of a remote Moor API server to forward messages to (proxy mode). When set, the "
        ...
        "Remote Moor API server URL (e.g. http://192.168.1.100:8642)", None, advanced=True),
    "GATEWAY_PROXY_KEY": _msg(
        "Bearer token for authenticating with the remote Moor API server (proxy mode). Must "
    ...
    # MOOR_TOOL_PROGRESS_MODE (deprecated; use display.tool_progress) is intentionally NOT listed:
    # this dict feeds user-facing surfaces (dashboard keys page, setup checklists), so deprecated
    # knobs stay in config._EXTRA_ENV_KEYS only. MOOR_TOOL_PROGRESS is unsupported.
    "MOOR_PREFILL_MESSAGES_FILE": _setting(
        "Path to JSON file with ephemeral prefill messages for few-shot priming",
        "Prefill messages file path", None),
    "MOOR_EPHEMERAL_SYSTEM_PROMPT": _setting(
