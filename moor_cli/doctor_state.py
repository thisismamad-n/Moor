"""MOOR_HOME state checks for moor doctor: directories, memory files, state.db health, skills hub, memory provider, profiles.
...
def _doctor_memory_config(moor_home: Path | None = None) -> dict:
...
def _render_state_db_stats(stats: dict, holders=None) -> list:
    ...
    if isinstance(deferral, dict):
        lines.append(("warn", f"state.db FTS repair is blocked after {deferral.get('attempts') or '?'} deferral(s) "
                      ...
                      "(stop the listed processes, then run 'moor sessions optimize-storage' with the gateway stopped)"))
    ...
    if logical is not None and logical > STATE_DB_SIZE_WARN_BYTES:
        ...
        if stats.get("fts_rebuild_pending") or stale_trigram:
            detail += "; run 'moor sessions optimize-storage' offline (with the gateway stopped) to compact FTS storage"
...
def _memory_store_flags(moor_home: Path) -> tuple:
    from tools.memory_tool import get_builtin_memory_store_flags
    return get_builtin_memory_store_flags({"memory": _doctor_memory_config(moor_home)})
...
_STATE_DB_REPAIRS = {
    "fts": ("Repaired state.db FTS write health",
            ...
            "state.db FTS write corruption — run 'moor doctor --fix' (or 'moor sessions repair') to rebuild the FTS index"),
    "schema": ("Repaired state.db schema ({count} sessions recovered)",
               ...
               "state.db schema malformed — run 'moor doctor --fix' (or 'moor sessions repair') to recover hidden sessions"),
}
...
def _state_db_health(f: Finding, should_fix: bool, state_db_path: Path, _DHH: str) -> None:
    ...
    try:
        ...
        from moor_state_repair import _db_opens_cleanly
        ...
        for _kind, _text, _detail in rows:
            ...
            if "auto_prune" in _detail:
                issues.append("state.db is large — enable sessions.auto_prune in config.yaml"
                              + (" and run 'moor sessions optimize-storage' offline (gateway stopped)" if "optimize-storage" in _detail else ""))
...
def _check_skills_hub(should_fix: bool, f: Finding) -> None:
    ...
    if check_bool(hub_dir.exists(), "Skills Hub directory exists", ("Skills Hub directory not initialized", "(run: moor skills list)")):
...
def _memory_provider_honcho(issues: list) -> None:
    ...
    if not cfg_path.exists():
        ...
        check_bool(hcfg.api_key or hcfg.base_url,
                   ...
                   ("Honcho config not found", "run: moor memory setup"))
    elif not hcfg.enabled:
        check_info(f"Honcho disabled (set enabled: true in {cfg_path} to activate)")
    elif not (hcfg.api_key or hcfg.base_url):
        _fail_and_issue("Honcho API key or base URL not set", "run: moor memory setup",
                        "No Honcho API key — run 'moor memory setup'", issues)
    else:
...
def _memory_provider_mem0(issues: list) -> None:
    ...
    if mem0_cfg.get("api_key", ""):
        ...
        _fail_and_issue("Mem0 API key not set", "(set MEM0_API_KEY in .env or run moor memory setup)",
...
def _memory_provider_generic(name: str) -> None:
    ...
    if _provider and _provider.is_available():
        ...
        check_warn(f"{name} configured but not available", "run: moor memory status")
    else:
        check_warn(f"{name} plugin not found", "run: moor memory setup")
    ...
    if wrapper_dir.is_dir():
        for wrapper in wrapper_dir.iterdir():
            if not wrapper.is_file():
                continue
            with warn_on_error(""):
                _m = _re.search(r"moor -p (\S+)", wrapper.read_text(encoding="utf-8"))
