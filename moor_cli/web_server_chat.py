...
# /api/pty spawns ``moor --tui`` behind a pseudo-terminal and forwards bytes +
...
def _ws_host_origin_reason(ws: "WebSocket") -> Optional[str]:
    ...
    """
    from moor_cli.web_server import _is_accepted_host, app
...
def _resolve_chat_argv(
    ...
    dir, the same propagation ``moor -p <name>`` performs.
    """
    from moor_cli.web_server_profiles import _config_profile_scope, _resolve_profile_dir
    from moor_cli.web_server_sessions import _open_session_db_for_profile, _session_latest_descendant
    from moor_cli.main import PROJECT_ROOT
    from moor_cli.main_tui_launch import _apply_tui_python_env, _make_tui_argv
    ...
    # scope overrides MOOR_HOME before config is bridged into the env.
    ...
    # which inherits the profile MOOR_HOME set above.
...
def _resolve_client_ws_host() -> Optional[str]:
    """Host the in-container WS client should dial: ``MOOR_DASHBOARD_WS_HOST``
...
async def _resolve_chat_argv_async(
    ...
    from moor_cli.web_server import _get_chat_argv_lock, app
...
def _active_session_file_for_channel(app: "FastAPI", channel: str) -> Path:
    ...
    from moor_cli.web_server import _get_pty_active_session_files
        ...
        with _console_executor_lock:
            if _console_executor is None:
                _console_executor = concurrent.futures.ThreadPoolExecutor(
                    max_workers=_CONSOLE_EXECUTOR_MAX_WORKERS, thread_name_prefix="moor-console")
