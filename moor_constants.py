...
_UNSET = object()
_MOOR_HOME_OVERRIDE: ContextVar[str | object] = ContextVar("_MOOR_HOME_OVERRIDE", default=_UNSET)
...
def set_moor_home_override(path: str | Path | None) -> Token:
    ...
    return _MOOR_HOME_OVERRIDE.set(value)


def reset_moor_home_override(token: Token) -> None:
    """Restore the previous context-local Moor home override."""
    _MOOR_HOME_OVERRIDE.reset(token)


def get_moor_home_override() -> str | None:
    """Return the active context-local Moor home override, if any."""
    override = _MOOR_HOME_OVERRIDE.get()
    return str(override) if override is not _UNSET and override else None


def _expand_moor_home(path: str) -> Path:
    """Expand environment and user-home syntax in a Moor home path."""
    return Path(os.path.expanduser(os.path.expandvars(path)))


def _get_platform_default_moor_home() -> Path:
    """Return the platform default with the literal data-directory suffix."""
    suffix = os.environ.get("MOOR_DATA_DIR_SUFFIX", "")
    if sys.platform == "win32":
        local_appdata = os.environ.get("LOCALAPPDATA", "").strip()
        base = Path(local_appdata) if local_appdata else Path.home() / "AppData" / "Local"
        return base / ("moor" + suffix)
    return Path.home() / (".moor" + suffix)
...
def _warn_profile_fallback_once() -> None:
    """Warn once when MOOR_HOME is unset but a non-default profile is sticky-active (wrong fallback)."""
    ...
    # read re-ran on every get_moor_home() call (#90065).
    _profile_fallback_warned = True
    try:
        fallback_home = _get_platform_default_moor_home()
    ...
    if active and active != "default":
        ...
        msg = (
            f"[MOOR_HOME fallback] MOOR_HOME is unset but active "
            f"profile is {active!r}. Falling back to {fallback_home}, which "
            f"is the DEFAULT profile — not {active!r}. Any data this "
            f"process writes will land in the wrong profile. The "
            f"subprocess spawner should pass MOOR_HOME explicitly "
            f"(see issue #18594)."
        )
...
def get_moor_home() -> Path:
    """Moor home: context-local override → ``MOOR_HOME`` env var → platform default."""
    override = get_moor_home_override()
    if override:
        return _expand_moor_home(override)
    if not os.environ.get("MOOR_HOME", "").strip():
...
def moor_home_key(path: str | Path | None = None) -> str:
    """Stable registry key for a Moor home/profile dir.
    ...
    """
    candidate = Path(path) if path is not None else get_moor_home()
    raw = str(candidate)
...
def reset_moor_home_key_cache() -> None:
    """Forget every remembered home key (for tests that move a home dir on disk)."""
    _HOME_KEY_CACHE.clear()


def get_process_moor_home() -> Path:
    """Moor home of the running process, ignoring task overrides.
    ...
    ``MOOR_HOME`` live on purpose: routed-profile DECISIONS compare against
    :func:`get_routing_process_moor_home` instead (#119242).
    """
    val = os.environ.get("MOOR_HOME", "").strip()
    return _expand_moor_home(val) if val else _get_platform_default_moor_home()


# Host-pinned identity of the profile this process serves as its own (None: follow MOOR_HOME).
_PINNED_PROCESS_MOOR_HOME: str | None = None


def pin_process_moor_home(path: str | Path | None) -> None:
    ...
    ``os.environ["MOOR_HOME"]`` for legacy readers (Moor WebUI) otherwise makes every turn's own
    ...
    ``moor_cli.env_loader._process_moor_home``) misjudge the same way. ``None`` clears the pin.

    Process-global on purpose: it names the process's own identity, not a per-task value. It is NOT
    folded into :func:`get_process_moor_home`: :func:`get_moor_home` falls back to that for
    tasks carrying no override, and the host's mirror exists precisely so those readers see the
    served profile. Hosts that never mutate ``MOOR_HOME`` need not call this (no-op).
    """
    global _PINNED_PROCESS_MOOR_HOME
    _PINNED_PROCESS_MOOR_HOME = None if path is None else str(path)


def process_moor_home_is_pinned() -> bool:
    return _PINNED_PROCESS_MOOR_HOME is not None


def get_routing_process_moor_home() -> Path:
    """Launch home for routed-profile decisions: the pinned home, else :func:`get_process_moor_home`."""
    pinned = _PINNED_PROCESS_MOOR_HOME
    return _expand_moor_home(pinned) if pinned else get_process_moor_home()
...
# get_default_moor_root() memo keyed on (native home, expanded MOOR_HOME) so it stays
...
def get_default_moor_root(*, home: str | Path | None = None) -> Path:
    """Root of an explicit home, or the process home when none is supplied."""
    global _default_moor_root_memo
    native_home = _get_platform_default_moor_home()
    env_home = str(home).strip() if home is not None else os.environ.get("MOOR_HOME", "").strip()
    env_path = _expand_moor_home(env_home) if env_home else None
    memo_key = (str(native_home), str(env_path) if env_path is not None else "")
    memo = _default_moor_root_memo
    if memo is not None and memo[:2] == memo_key:
        return memo[2]
    result = native_home
    if env_path is not None:
        try:
            env_path.resolve().relative_to(native_home.resolve())  # under ~/.moor (normal or profile mode)
        except ValueError:  # Docker/custom root: <root>/profiles/<name> -> <root>, else MOOR_HOME itself
            result = env_path.parent.parent if env_path.parent.name == "profiles" else env_path
    _default_moor_root_memo = (*memo_key, result)
    return result
...
# Files marking a real Moor home; arbitrary dirs with a ``profiles`` segment lack them.
_MOOR_HOME_MARKERS = ("config.yaml", ".env", "state.db")


def _is_moor_profiles_root(profiles_dir: Path) -> bool:
    """True when *profiles_dir* is provably ``<moor-home>/profiles``.

    Accepts the classic ``~/.moor`` layout, a root carrying moor-home marker files, a
    ...
    try:
        if (profiles_dir / _DELETED_PROFILES_DIR).is_dir() or any(
            (root / marker).exists() for marker in _MOOR_HOME_MARKERS
        ):
            return True
    except OSError:
        pass
    try:
        return root.resolve(strict=False) == get_default_moor_root().resolve(strict=False)
    except OSError:
        return False
...
def named_profile_home(path: str | Path) -> Path | None:
    ...
    Requires ``<name>`` not to start with ``.`` and the ``profiles`` parent to be a real Moor home;
    ...
    for candidate in (current, *current.parents):
        if (candidate.parent.name == "profiles" and not candidate.name.startswith(".")
                and _is_moor_profiles_root(candidate.parent)):
            return candidate
        if candidate.name == ".moor":  # default home: a coincidental profiles/ ancestor is not a root
            return None
    return None
...
def named_profile_has_servable_identity(profile_home: str | Path) -> bool:
    ...
    An EMPTY ``.env`` is all a crashed ``moor profile create`` leaves behind, and it is enough for
...
def named_profile_is_live(profile_home: str | Path) -> bool:
    ...
    never be started as a backend (whose ``ensure_moor_home`` would rebuild the full tree)."""
    home = Path(profile_home)
...
def mkdir_under_moor_home(path: str | Path) -> Path:
...
def _packaged_dir(env_var: str, default: Path | None, subdir: str) -> Path:
    ...
    return Path(override) if override else default if default is not None else get_moor_home() / subdir


def get_optional_skills_dir(default: Path | None = None) -> Path:
    """Return the optional-skills directory, honoring package-manager wrappers."""
    return _packaged_dir("MOOR_OPTIONAL_SKILLS", default, "optional-skills")


def get_optional_mcps_dir(default: Path | None = None) -> Path:
    """Return the optional-mcps directory, honoring package-manager wrappers (``MOOR_OPTIONAL_MCPS``)."""
    return _packaged_dir("MOOR_OPTIONAL_MCPS", default, "optional-mcps")


def get_bundled_skills_dir(default: Path | None = None) -> Path:
    """Return the bundled skills directory, honoring package-manager wrappers (``MOOR_BUNDLED_SKILLS``)."""
    return _packaged_dir("MOOR_BUNDLED_SKILLS", default, "skills")


def get_moor_dir(new_subpath: str, old_name: str, *, home: Path | None = None) -> Path:
    """Resolve a Moor subdirectory, honouring a populated legacy ``<old_name>/`` (no migration).
...
def _version_probe_ok(path: str) -> bool:
    """True when ``<path> --version`` exits 0 under the moor-managed Node PATH."""
    result = _run_version_probe([path, "--version"], env=with_moor_node_path())
    return result is not None and result.returncode == 0


# Install tree root (this file lives at <install_root>/moor_constants.py). Used by secure_parent_dir() to
# skip chmod on the install dir — chmodding it 0700 breaks moor-user traversal in Docker (UID 10000). See
# #25821, #93050.
...
def moor_managed_node_tree_present(home: Path | None = None) -> bool:
    ...
    return any((directory / name).is_file() for directory in iter_moor_node_dirs(home) for name in names)
...
def find_node_executable(command: str) -> str | None:
    ...
    probes, repairs, or activates the retired ``MOOR_HOME/node`` layout.
    """
    command = str(command)
...
def with_moor_node_path(env: dict[str, str] | None = None) -> dict[str, str]:
...
def agent_browser_runnable(path: str | None) -> bool:
    ...
    local ``node_modules/agent-browser/bin/...`` binary, which then disappears on the next ``moor update``
...
def display_moor_home(home: Path | None = None) -> str:
    """User-facing ``~/`` display string for MOOR_HOME (``~/.moor/profiles/coder``).

    ``home`` overrides the lookup for callers that run before the CLI has applied the sticky
    ``active_profile`` (``get_moor_home()`` would emit the wrong-profile fallback warning there).
    """
    if home is None:
        home = get_moor_home()
    try:  # as_posix(): str() on Windows yields chimeras like ~/AppData\Local\moor/skills/
...
def secure_parent_dir(path: Path) -> None:
    """Chmod ``0o700`` on *path*'s parent, refusing ``/`` and top-level dirs (misresolved MOOR_HOME)."""
    parent = path.parent.resolve()
    if parent == Path("/") or len(parent.parts) < 3:
        return
    # Refuse the install tree: chmod 0700 breaks moor-user traversal in Docker (UID 10000).
    # A credential file here means MOOR_HOME misresolved; surface it (caused production lockouts).
    # See #25821, #93050.
    if parent == _INSTALL_ROOT or _INSTALL_ROOT in parent.parents:
        ...
        logging.getLogger(__name__).warning(
            ...
            "moor-agent install directory (%s). Credential files are "
            "normally stored under the moor home directory instead.", parent, _INSTALL_ROOT,
        )
        return
...
def _profile_home_path(env: dict[str, str] | None = None) -> str | None:
    """Return ``{MOOR_HOME}/home`` when the profile-home directory exists."""
    moor_home = get_moor_home_override() or (env or {}).get("MOOR_HOME") or os.getenv("MOOR_HOME")
    if not moor_home:
        return None
    profile_home = str(_expand_moor_home(moor_home) / "home")
...
def _iter_real_home_candidates(env: dict[str, str] | None = None) -> list[str]:
    ...
    env = env or {}
    candidates = [_env_get(env, "MOOR_REAL_HOME"), _env_get(env, "HOME")]
    with contextlib.suppress(Exception):
        import pwd
...
def get_real_home(env: dict[str, str] | None = None) -> str:
    """The OS user's real home, avoiding the Moor profile HOME.

    ``HOME`` belongs to the OS account and external CLIs keeping credentials under ``~``; a parent
    already running with ``HOME={MOOR_HOME}/home`` is repaired back when possible.
    """
...
def get_subprocess_home(env: dict[str, str] | None = None) -> str | None:
    ...
    ``{MOOR_HOME}/home``; ``real``: always real HOME; ``profile``: always the profile home.
    """
    env = env or {}
    profile_home = _profile_home_path(env)
...
def apply_subprocess_home_env(env: MutableMapping[str, str]) -> None:
    """Apply Moor' subprocess HOME contract to *env* in-place: ``HOME``/``MOOR_REAL_HOME``
    per the home mode, and the temp vars re-pointed at ``env["MOOR_HOME"]``'s scratch dir."""
    real_home = get_real_home(env)
    if real_home:
        env["MOOR_REAL_HOME"] = real_home
...
# --- Scratch dir: Moor' own temp space, never the system /tmp ---
# System temp is tmpfs on most Linux distros and containers, so browser profiles, PTY probes,
# download spools and every ``tempfile.mkdtemp()`` a moor-launched script performs eat RAM
# and vanish on reboot. ``MOOR_HOME/cache/scratch`` is real storage with an IDLE retention:
...
SCRATCH_DIR_MARKER_ENV = "MOOR_SCRATCH_DIR"
...
# ``moor_rpc_<32 hex>.sock`` (~49) to the temp root, so a root longer than this budget
...
# Canonical home of "is this install package-manager managed": ``moor_cli.config`` re-exports
...
def get_managed_system(home: str | Path | None = None) -> str | None:
    ...
    Signals: MOOR_MANAGED env var (systemd service) or a ``.managed`` marker file in
    MOOR_HOME (NixOS activation script — interactive shells don't see the service env).
    ...
    marker = os.getenv("MOOR_MANAGED", "").strip().lower() or None
    managed_marker = (Path(home) if home is not None else get_moor_home()) / ".managed"
...
def _container_or_chmod_skipped() -> bool:
    """Container/chmod-skip detection: the ``MOOR_CONTAINER``/``MOOR_SKIP_CHMOD`` operator
...
def _resolve_moor_uid_gid() -> tuple[int | None, int | None]:
    """Read MOOR_UID / MOOR_GID (set by Docker deployments); (None, None) if unset/invalid/Windows.
    The entrypoint chowns MOOR_HOME once, but subdirs created at runtime (``profiles/<name>/``)
    need the same chown or they land root:root and block later uid-mapped workers.

    Docker containers running Moor commonly set these to map the in-container user to a host user so
    ...
    return _env_int("MOOR_UID"), _env_int("MOOR_GID")


def _chown_to_moor_uid(path) -> None:
    """Chown ``path`` to ``MOOR_UID:MOOR_GID`` when set; EPERM/ENOENT are non-fatal (the
    ...
    created by ``ensure_moor_home`` on Docker deployments. See #34107.
    """
...
def apply_secure_dir_policy(path, *, home: str | Path | None = None) -> None:
    """Apply the canonical Moor home-directory permission policy to *path*.
    ...
    script set them (#77579); in a container only an explicit ``MOOR_HOME_MODE`` is applied
    (a bind-mounted data dir is often shared with sibling containers, #10757); elsewhere
    ``MOOR_HOME_MODE`` (e.g. ``0701``, ``2770``) is passed to the host's ``chmod``. Special
    bits may be cleared by the host filesystem (macOS commonly clears setgid on directories
    whose group the caller does not belong to). ``MOOR_UID`` / ``MOOR_GID`` ownership is
    ...
    Import-safe twin of ``moor_cli.config._secure_dir`` (which delegates here), so callers
...
def get_scratch_dir(home: str | Path | None = None, *, prune: bool = True) -> Path:
    """``<home>/cache/scratch`` (created, owner-only); *home* defaults to the active Moor home.

    Every Moor process and child gets ``TMPDIR``/``TMP``/``TEMP`` pointed here at boot (see
    ...
    Permissions follow :func:`apply_secure_dir_policy`, so an explicit ``MOOR_HOME_MODE`` or
    a managed/shared home is honored instead of a blanket ``0700`` (#117347).
    """
    base = Path(home) if home is not None else get_moor_home()
...
def prune_scratch_dir(scratch: Path | None = None, max_idle_hours: float = SCRATCH_MAX_IDLE_HOURS) -> int:
    ...
    *max_idle_hours*, reaping processes and git worktree registrations rooted in them
    first (``moor_constants_scratch``); return the count removed."""
...
def scratch_dir_usage_bytes(scratch: Path | None = None) -> int:
    """Total bytes under the scratch dir (for ``moor doctor``); 0 when unreadable."""
    root = scratch if scratch is not None else get_scratch_dir(prune=False)
    total = 0
...
def apply_scratch_tmp_env(env: MutableMapping[str, str]) -> bool:
    """Point ``TMPDIR``/``TMP``/``TEMP`` in *env* at the scratch dir of ``env["MOOR_HOME"]``.

    A temp var the user (or the OS: macOS ``/var/folders``, Windows ``%TEMP%``) set is
    respected and nothing changes. A value Moor itself exported earlier — recognisable
    because it equals ``MOOR_SCRATCH_DIR`` — is re-derived, so a child running under another
    ...
    for key in SCRATCH_TMP_ENV_VARS:
        ...
        if value and value != ours:
            return False
    home = env.get("MOOR_HOME", "").strip()
    try:
        scratch = str(get_scratch_dir(_expand_moor_home(home) if home else get_process_moor_home()))
    except (RuntimeError, OSError):
        # No MOOR_HOME and no resolvable user home (a child env built from nothing on
...
def is_termux() -> bool:
    ...
    from moor_platform.host.runtime import is_termux as detect
...
def is_wsl() -> bool:
    ...
    from moor_platform.host.runtime import is_wsl as detect
...
def is_container() -> bool:
    ...
    from moor_platform.host.runtime import is_container as detect
...
def _detect_container() -> bool:
    ...
    from moor_platform.host.runtime import _detect_container as detect
...
def translate_cwd_for_wsl_backend(cwd: str) -> str:
    """Map a Windows-host cwd (drive path or ``\\\\wsl.localhost\\`` UNC) to POSIX when Moor runs in WSL.
...
def get_config_path() -> Path:
    """Return the path to ``config.yaml`` under MOOR_HOME."""
    return get_moor_home() / "config.yaml"


def get_skills_dir() -> Path:
    """Return the path to the skills directory under MOOR_HOME."""
    return get_moor_home() / "skills"


def get_env_path() -> Path:
    """Return the path to the ``.env`` file under MOOR_HOME."""
...
def apply_ipv4_preference(force: bool = False) -> None:
    ...
    if getattr(socket.getaddrinfo, "_moor_ipv4_patched", False):
        return
    ...
    def _ipv4_getaddrinfo(host, port, family=0, type=0, proto=0, flags=0):
        ...
        return _original_getaddrinfo(host, port, family, type, proto, flags)
    _ipv4_getaddrinfo._moor_ipv4_patched = True  # type: ignore[attr-defined]
...
def project_venv_dir(project_root) -> Path | None:
    ...
    Installers that keep the interpreter out of the checkout (``$MOOR_HOME/venvs/<name>``, the layout the
    ...
    path skipped the import probe, reclassified every ``moor tools`` dependency as missing and failed the
...
def _venv_installs_checkout(venv: Path, root: Path) -> bool:
    """Is *venv*'s own ``moor-agent`` installed from *root*?
    ...
    for dist in distributions(name="moor-agent", path=[str(site_packages(venv))]):
        try:
...
def is_first_party_module(name: str | None) -> bool:
    """True when *name* ships with Moor (exact first segment; ``startswith`` would claim ``agentops``)."""
    root = str(name).split(".")[0] if name else ""
    return bool(root) and (root in FIRST_PARTY_MODULE_ROOTS or root.startswith("moor_"))
...
def partial_update_hint(exc: BaseException) -> list[str]:
    ...
    return [
        ...
        "    moor update",
        "If that also fails, reinstall: https://github.com/thisismamad-n/Moor",
    ]
...
def normalize_scope(scope: str | Path | None) -> str | None:
    ...
    * **Read paths** (``list_providers``, ``get_provider``): ``None`` means
      "the active home's scope" and must go through :func:`moor_home_key`
    ...
    """
    return moor_home_key(scope) if scope is not None else None
