...
def get_default_moor_root() -> Path:
    """Root Moor dir for profile-level ops: ``<root>`` when ``MOOR_HOME=<root>/profiles/<name>``."""
...
def iter_moor_node_dirs(home: Path | None = None) -> list[Path]:
    """moor-managed Node dirs in lookup order; both Windows and POSIX shapes so migrated installs work.

    Keep in sync with moorManagedNodePathEntries() in apps/desktop/electron/backend-env.ts.
    """
...
def _print_managed_node_in_use_notice() -> None:
    ...
    print(
        ...
        "upgrade until the app is closed (re-run `moor update` afterwards).", flush=True,
    )
...
def _stage_windows_node_zip(home: Path, node_arch: str) -> Path | None:
    ...
    if index_bytes is None:
        return None
    pattern = rf"node-v{_MOOR_NODE_TARGET_MAJOR}\.\d+\.\d+-win-{node_arch}\.zip"
...
def _heal_managed_node_windows(home: Path | None = None) -> bool | None:
    ...
    (images are mapped with ``FILE_SHARE_DELETE`` — the same mechanism as the moor.exe quarantine); when
...
def _run_node_bootstrap(func: str, *, timeout: int, **extra_env: str) -> bool:
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "MOOR_HOME": str(get_moor_home()), **extra_env},
...
def bootstrap_moor_managed_node() -> str | None:
    """Install a moor-managed Node tree under ``$MOOR_HOME/node`` and return its npm path.

    Moor never modifies a user-owned toolchain (system, nvm, brew, Nix) that fails ``engines``.
    """
    ...
    if sys.platform == "win32":
        ...
        # MOOR_NODE_SKIP_LINKS=1 keeps node/npm/npx out of ~/.local/bin: never shadow the user toolchain.
        ok = _run_node_bootstrap("_nb_install_bundled_node", timeout=600, MOOR_NODE_SKIP_LINKS="1")
    if not ok:
        return None
...
def heal_moor_managed_node() -> bool:
    """Redownload moor-managed Node when the tree exists but is broken; at most once per process.
    ...
    if _managed_node_heal_attempted or not moor_managed_node_tree_present():
        return False
    if sys.platform == "win32":
...
def find_moor_node_executable(command: str) -> str | None:
    """moor-managed Node/npm path, healing broken/outdated trees; heal failure still returns old Node."""
...
def with_moor_node_path(env: dict[str, str] | None = None) -> dict[str, str]:
    ...
    for entry in reversed([str(path) for path in iter_moor_node_dirs() if path.is_dir()]):
...
def venv_bin_dir(venv_dir, *, windows: bool | None = None) -> Path:
    ...
    Canonical helper for venv layout. This was open-coded in seven places across four ``moor_cli`` modules
    ...
    lived 2400 lines away in another function. A few sites outside ``moor_cli``
