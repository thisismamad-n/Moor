...
_UNSET = object()
_MOOR_HOME_OVERRIDE: ContextVar[str | object] = ContextVar(
    "_MOOR_HOME_OVERRIDE", default=_UNSET
)
...
def _moor_home_from_env() -> Path:
    """Resolve MOOR_HOME from the process environment only.

    Reads the ``MOOR_HOME`` env var, falling back to the platform-native
    default.  Deliberately ignores the context-local override installed by
    :func:`set_moor_home_override`, so this reflects the process/launch
    scope rather than a per-task profile.  Shared by :func:`get_moor_home`
    and :func:`get_process_moor_home` so the two never drift.
    """
    val = os.environ.get("MOOR_HOME", "").strip()
    if val:
        return Path(val)
    return _get_platform_default_moor_home()


def _warn_profile_fallback_once() -> None:
    """Warn once when falling back to the default home while a profile is active.

    Guard: if a non-default profile is sticky-active but ``MOOR_HOME`` is
...
def get_moor_home() -> Path:
    ...
    :func:`set_moor_home_override`) → ``MOOR_HOME`` env var → the
    platform-native default.  This is the single source of truth — all other
    copies should import this.

    When ``MOOR_HOME`` is unset but an ``active_profile`` file indicates
    ...
    expected to propagate ``MOOR_HOME`` explicitly (see the systemd
    template in ``moor_cli/gateway.py`` and the kanban dispatcher in
    ``moor_cli/kanban_db.py``).  See https://github.com/NousResearch/hermes-agent/issues/18594.
    """
...
def get_process_moor_home() -> Path:
    ...
    Unlike :func:`get_moor_home`, this never follows the context-local
    override set by :func:`set_moor_home_override`.  It resolves only the
    process ``MOOR_HOME`` env var (falling back to the platform default),
...
# Process-level memo for get_default_moor_root(). The function resolves
# MOOR_HOME against the native home on every call (~80us of path
...
# Its result depends only on (MOOR_HOME, platform native home), which are
# compared for free on each call, so the memo is freshness-correct even if a
# test or plugin mutates MOOR_HOME mid-process.
...
def get_default_moor_root() -> Path:
    ...
    (``~/.moor`` on POSIX, ``%LOCALAPPDATA%\\moor`` on native Windows).

    In Docker or custom deployments where ``MOOR_HOME`` points outside
    ``~/.moor`` (e.g. ``/opt/data``), returns ``MOOR_HOME`` directly
    — that IS the root.

    In profile mode where ``MOOR_HOME`` is ``<root>/profiles/<name>``,
    returns ``<root>`` so that ``profile list`` can see all profiles.
    Works both for standard (``~/.moor/profiles/coder``) and Docker
    ...
    env_home = os.environ.get("MOOR_HOME", "")
    if _default_moor_root_memo is not None:
        memo_native, memo_env, memo_result = _default_moor_root_memo
    ...
    if not env_home:
        ...
        try:
            ...
            # MOOR_HOME is under ~/.moor (normal or profile mode)
            result = native_home
        except ValueError:
            ...
            if env_path.parent.name == "profiles":
                ...
                # Not a profile path — MOOR_HOME itself is the root
                result = env_path
    _default_moor_root_memo = (str(native_home), env_home, result)
    return result
...
def get_optional_skills_dir(default: Path | None = None) -> Path:
    ...
    tree and expose it via ``MOOR_OPTIONAL_SKILLS``.
    """
    override = os.getenv("MOOR_OPTIONAL_SKILLS", "").strip()
    if override:
        return Path(override)
    if default is not None:
        return default
    return get_moor_home() / "optional-skills"


def get_optional_mcps_dir(default: Path | None = None) -> Path:
    """Return the optional-mcps directory, honoring package-manager wrappers.

    Mirrors :func:`get_optional_skills_dir` for the MCP catalog (Moor-approved
    Model Context Protocol servers shipped with the repo but disabled by
    default). Packaged installs may ship ``optional-mcps`` outside the Python
    package tree and expose it via ``MOOR_OPTIONAL_MCPS``.
    """
    override = os.getenv("MOOR_OPTIONAL_MCPS", "").strip()
    if override:
        return Path(override)
    if default is not None:
        return default
    return get_moor_home() / "optional-mcps"
...
def get_bundled_skills_dir(default: Path | None = None) -> Path:
    ...
    Resolution order:
        1. ``MOOR_BUNDLED_SKILLS`` env var (Nix wrapper / explicit override)
        2. Caller-supplied ``default`` (typically the source-checkout path)
        3. ``<MOOR_HOME>/skills`` last-resort
    """
    override = os.getenv("MOOR_BUNDLED_SKILLS", "").strip()
    if override:
        return Path(override)
    if default is not None:
        return default
...
def get_moor_dir(
    ...
    *,
    ...
    Args:
        new_subpath: Preferred path relative to MOOR_HOME (e.g. ``"cache/images"``).
        old_name: Legacy path relative to MOOR_HOME (e.g. ``"image_cache"``).
        home: Optional explicit Moor home. Profile-aware callers that manage
            ...
            temporarily mutating the process or context-local MOOR_HOME.
...
def iter_moor_node_dirs(home: Path | None = None) -> list[Path]:
    ...
    into ``%LOCALAPPDATA%\\moor\\node``. POSIX installs use
    ``$MOOR_HOME/node/bin``. Include both shapes on every platform so mixed
    or migrated installs still work.
    """
    root = home or get_moor_home()
    dirs = [root / "node"]
    bin_dir = root / "node" / "bin"
    # NOTE: keep this ordering in sync with moorManagedNodePathEntries() in
...
_MOOR_NODE_TARGET_MAJOR = int(os.environ.get("MOOR_NODE_TARGET_MAJOR", "22"))
_managed_node_heal_attempted = False
_NODE_BOOTSTRAP_SCRIPT = Path(__file__).resolve().parent / "scripts" / "lib" / "node-bootstrap.sh"

# Install tree root (this file lives at <install_root>/moor_constants.py).
# Used by secure_parent_dir() to skip chmod on the install dir — chmodding it
# 0700 breaks moor-user traversal in Docker (UID 10000). See #25821, #93050.
...
def node_tool_runnable(path: str | None) -> bool:
    ...
    Moor-managed Node trees live under ``$MOOR_HOME/node`` (or a profile's
    ``MOOR_HOME``). A partial upgrade or interrupted install can leave
    ``bin/npm`` behind while ``lib/cli.js`` is missing — the wrapper exists but
    immediately throws ``MODULE_NOT_FOUND``. ``find_moor_node_executable``
    used to trust file presence alone, so ``moor update`` would pick that
...
def moor_managed_node_tree_present(home: Path | None = None) -> bool:
    ...
    for directory in iter_moor_node_dirs(home):
        for name in names:
...
def _print_managed_node_in_use_notice() -> None:
    ...
    print(
        ...
        "upgrade until the app is closed (re-run `moor update` afterwards).",
        flush=True,
    )


def _heal_managed_node_windows(home: Path | None = None) -> bool | None:
    """Redownload the portable Node zip into ``%MOOR_HOME%\\node`` on Windows.
    ...
    ``FILE_SHARE_DELETE`` — the same mechanism as the moor.exe quarantine);
    ...
    index_url = f"https://nodejs.org/dist/latest-v{_MOOR_NODE_TARGET_MAJOR}.x/"
    try:
    ...
    match = re.search(
        rf"node-v{_MOOR_NODE_TARGET_MAJOR}\.\d+\.\d+-win-{node_arch}\.zip",
        index_html,
    )
    if not match:
        return False
...
def _bootstrap_managed_node_posix() -> bool:
    """Install a fresh managed Node under ``$MOOR_HOME/node`` on POSIX.
    ...
    ``MOOR_NODE_SKIP_LINKS=1`` so the user's own node/npm on PATH is not
...
def bootstrap_moor_managed_node() -> str | None:
    ...
    its own tree under ``$MOOR_HOME/node`` (the same tree a fresh install
    ...
    present.
    """
    existing = find_moor_node_executable("npm")
    if existing:
        return existing
    ...
    for directory in iter_moor_node_dirs():
...
def heal_moor_managed_node() -> bool:
    ...
    if _managed_node_heal_attempted:
        return False
    if not moor_managed_node_tree_present():
        return False
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "MOOR_HOME": str(get_moor_home())},
            capture_output=True,
            timeout=300,
            check=False,
        )
...
def _managed_node_tree_outdated(home: Path | None = None) -> bool:
    ...
    same way a broken one does: :func:`find_moor_node_executable` triggers
    the once-per-process heal, which redownloads
    ``latest-v{_MOOR_NODE_TARGET_MAJOR}.x`` — so existing users are upgraded
    ...
    for directory in iter_moor_node_dirs(home):
        for name in _candidate_node_command_names("node"):
            ...
            return major < _MOOR_NODE_TARGET_MAJOR
    return False


def find_moor_node_executable(command: str) -> str | None:
    """Return a Moor-managed Node/npm executable path, healing broken trees.

    Outdated trees (node major below ``_MOOR_NODE_TARGET_MAJOR``) heal the
    ...
    if needs_heal and heal_moor_managed_node():
...
def find_node_executable(command: str) -> str | None:
    ...
    """
    managed = find_moor_node_executable(command)
    if managed:
        return managed
    if moor_managed_node_tree_present():
        return None
...
def with_moor_node_path(env: dict[str, str] | None = None) -> dict[str, str]:
    ...
    managed = [str(path) for path in iter_moor_node_dirs() if path.is_dir()]
...
def agent_browser_runnable(path: str | None) -> bool:
    ...
    next ``moor update`` — leaving a **dangling symlink** that ``which`` still
...
def display_moor_home() -> str:
    """Return a user-friendly display string for the current MOOR_HOME.

    Uses ``~/`` shorthand for readability::

        default:  ``~/.moor``
        profile:  ``~/.moor/profiles/coder``
        custom:   ``/opt/moor-custom``

    Use this in **user-facing** print/log messages instead of hardcoding
    ``~/.moor``.  For code that needs a real ``Path``, use
    :func:`get_moor_home` instead.
    """
    home = get_moor_home()
    try:
...
def secure_parent_dir(path: Path) -> None:
    ...
    prevent catastrophic host bricking when ``MOOR_HOME`` or other path
    env vars resolve to an unexpected location.

    Also refuses to chmod the moor-agent install tree (the directory this
    ...
    if parent == Path("/") or len(parent.parts) < 3:
        return
    # Refuse the install tree root. chmodding it 0700 breaks moor-user
    ...
    if parent == _INSTALL_ROOT or _INSTALL_ROOT in parent.parents:
        # A credential file inside the install tree usually means MOOR_HOME
        ...
        logging.getLogger(__name__).warning(
            ...
            "normally stored under the moor home directory instead.",
            parent,
            _INSTALL_ROOT,
        )
        return
...
def _profile_home_path(env: dict[str, str] | None = None) -> str | None:
    ...
    if not moor_home:
        return None
    profile_home = os.path.join(moor_home, "home")
...
def _iter_real_home_candidates(env: dict[str, str] | None = None) -> list[str]:
    ...
    env = env or {}
    candidates: list[str] = []
    explicit = str(env.get("MOOR_REAL_HOME") or os.getenv("MOOR_REAL_HOME", "")).strip()
    if explicit:
...
def get_real_home(env: dict[str, str] | None = None) -> str:
    ...
    ``MOOR_HOME`` scopes Moor state. ``HOME`` is reserved for the OS/user
    account and the many external CLIs that store credentials under ``~``.
    If a parent process is already running with ``HOME={MOOR_HOME}/home``,
...
def get_subprocess_home(env: dict[str, str] | None = None) -> str | None:
    ...
    * ``auto`` (default): host installs keep the real user HOME; containers use
      ``{MOOR_HOME}/home`` for persistent state. If a host parent already has
      HOME pointed at the profile home, repair subprocesses back to real HOME.
    * ``real``: always prefer the real OS-user HOME.
    * ``profile``: use ``{MOOR_HOME}/home`` when it exists, preserving the
...
def get_config_path() -> Path:
    ...
    Replaces the ``get_moor_home() / "config.yaml"`` pattern repeated
    in 7+ files (skill_utils.py, moor_logging.py, moor_time.py, etc.).
    """
...
def venv_bin_dir(venv_dir, *, windows: bool | None = None) -> Path:
    ...
    across four ``moor_cli`` modules using three different Windows
    ...
    A few sites outside ``moor_cli`` (``tools/code_execution_tool.py``,
    ...
    *windows* lets a caller pass its own platform verdict. Several callers
    ...
    Windows paths on Linux CI (``moor_cli.main._is_windows`` and friends);
...
# `moor_cli.update_cmd`'s post-update probe consumes this same set so the
...
def is_first_party_module(name: str | None) -> bool:
    ...
    if not root:
        return False
    return root in FIRST_PARTY_MODULE_ROOTS or root.startswith("moor_")
...
def partial_update_hint(exc: BaseException) -> list[str]:
    ...
    rather than their config, is the problem — and `moor update` is exactly
