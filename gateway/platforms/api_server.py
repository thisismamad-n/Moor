...
- GET  /v1/models                  — lists moor-agent and any configured model_routes aliases
...
AnythingLLM, NextChat, ChatBox, etc.) can connect to moor-agent
...
def _moor_version() -> str:
    ...
    ``moor_cli.__version__`` is the runtime source of truth used by the CLI,
    ...
    ``moor_agent-*.dist-info`` after a source update until the environment is
...
# config.yaml accept (moor_constants.VALID_REASONING_EFFORTS); wire-level
...
def _request_agent_overrides(
    ...
    *,
    ...
    ``/v1/models`` advertises a stable virtual model (usually ``moor-agent``)
...
class ResponseStore:
    ...
    def __init__(self, max_size: int = MAX_STORED_RESPONSES, db_path: str = None):
        ...
        # gracefully on NFS/SMB/FUSE-mounted MOOR_HOME (same filesystem
        # issue addressed for state.db/kanban.db — see
        # moor_state._WAL_INCOMPAT_MARKERS).
...
class APIServerAdapter(BasePlatformAdapter):
    ...
    and routes them through moor-agent's AIAgent.
    """
    ...
    def _resolve_model_name(explicit: str) -> str:
        ...
        3. Fallback: "moor-agent"

        Delegates the tiered fallthrough to
        :func:`moor_cli.model_switch.resolve_effective_model` (the shared
    ...
    def _resolve_request_profile(self, request: "web.Request"):
        ...
        if not getattr(cfg, "multiplex_profiles", False):
            ...
            # `moor peer dm mini/researcher` answered by the mini's default
    ...
    def _ensure_session_db(self):
        ...
        Sessions are persisted to ``state.db`` so that ``moor sessions list``
        shows API-server conversations alongside CLI and gateway ones.

        Under multiplex ``/p/<profile>/`` requests the profile runtime scope
        redirects ``get_moor_home()``, so each profile gets its own DB —
    ...
    def _stored_session_model(self, session: Any) -> Optional[str]:
        ...
        The advertised virtual model (usually ``moor-agent``) means "use
        ...
        400s ("moor-agent is not a valid model ID") — the same filter
    ...
    def _session_runtime_request_from_body(self, body: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # The virtual model alias (self._model_name, e.g. "moor-agent") is
    ...
    def _create_agent(
        ...
        gateway platforms), falling back to the moor-api-server default.
        ...
        # moor_cli.model_switch.resolve_effective_model (session /model
        ...
        # (e.g. user ran `moor auth add openai-codex` without `moor model`),
    ...
    async def _handle_health(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response(
            {"status": "ok", "platform": "moor-agent", "version": _moor_version()}
        )
    ...
    async def _handle_models(self, request: "web.Request") -> "web.Response":
        """GET /v1/models — list moor-agent and any configured model_routes aliases.
    ...
    def _artifact_store_for(self, profile: str) -> ArtifactStore:
        ...
        (``<MOOR_HOME>/plugin-data/.../artifacts``-style controlled root),
    ...
    async def _handle_list_sessions(self, request: "web.Request") -> "web.Response":
        ...
        # Exact-title lookup, used by `moor peer dm` to resolve a peer's
        ...
        if title_filter:
            ...
            if not sessions:
                ...
                # which would fail `moor peer dm` resolution and mint
    ...
    async def _handle_create_session(self, request: "web.Request") -> "web.Response":
        ...
        # (e.g. "provider::moor-agent") are split, and the virtual model
        # alias (self._model_name, e.g. "moor-agent") is nulled out
        # there — a bare "moor-agent" is not a model_routes alias, so a
    ...
    async def _handle_session_model_lock(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response({
            "object": "moor.session.model_lock",
            "session_id": session_id,
            "runtime": runtime,
        })
    @_admit_api_agent_request
    async def _handle_chat_completions(self, request: "web.Request") -> "web.Response":
        ...
        if stream:
            ...
            def _on_tool_start(tool_call_id, function_name, function_args):
                """Emit ``moor.tool.progress`` with ``status: running``.
    ...
    async def _write_sse_chat_completion(
        ...
        try:
            ...
            async def _emit(item):
                ...
                as a custom ``event: moor.tool.progress`` SSE event so
                frontends can display them without storing the markers in
                conversation history.  See #6972 for the original event,
                #16588 for the ``toolCallId``/``status`` lifecycle fields.
                """
                if isinstance(item, tuple) and len(item) == 2 and item[0] == "__tool_progress__":
                    await response.write(_sse_frame(item[1], event="moor.tool.progress"))
                else:
                    content_chunk = {
                        "id": completion_id, "object": "chat.completion.chunk",
                        "created": created, "model": model,
                    ...
                    with self._profile_scope(request_profile):
                        try:
                            ...
                            session_tokens = self._bind_api_server_session(
                                ...
                                # MOOR_SESSION_CHAT_ID on /v1/runs and
    ...
    async def _handle_run_approval(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response({
            "object": "moor.run.approval_response",
            "run_id": run_id,
            "choice": choice,
            "resolved": resolved,
        })
    ...
    async def _handle_steer_run(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response({"object": "moor.run.steer", "run_id": run_id, "accepted": True})
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        try:
            ...
            # ~/.moor/config.yaml and plant persistence. Sandboxing (Docker /
