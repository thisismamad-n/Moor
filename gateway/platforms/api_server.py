...
def _prefix_names_served_profile(profile: str) -> bool:
    ...
    try:
        from moor_cli.profiles import profile_matches_home
...
_BROWSER_CONTROL_WS_PROTOCOL = "moor-browser-control-v1"
_BROWSER_CONTROL_TICKET_PROTOCOL_PREFIX = "moor-browser-control-ticket."
...
def _moor_version() -> str:
    """Canonical Moor version: ``moor_cli.__version__`` (dist-info can be stale on
...
def _resolve_request_runtime_agent_kwargs(provider: str, target_model: Optional[str] = None) -> Dict[str, Any]:
    ...
    from moor_cli.runtime_provider import resolve_runtime_provider, format_runtime_provider_error, _get_model_config
    try:
    ...
    max_tokens = None
    env_max_tokens = os.environ.get("MOOR_MAX_TOKENS")
    if env_max_tokens:
...
def _request_agent_overrides(
    ...
    The virtual model (``moor-agent``) means "gateway default". A bare ``model`` without
    ``provider`` is honored only when ``allow_bare_model`` (generic clients hardcode "gpt-4o";
    OpenAI-compatible handlers pass the ``direct_model_requests`` opt-in, moor-native
    ...
    def __init__(self, max_size: int = MAX_STORED_RESPONSES, db_path: str = None):
        ...
        if db_path is None:
            ...
            with suppress(Exception):
                ...
                db_path = str(get_moor_home() / "response_store.db")
...
def _derive_chat_session_id(system_prompt: Optional[str], first_user_message: str) -> str:
    ...
    turns of an Open WebUI-style conversation), so one Moor session/sandbox is reused."""
...
class APIServerAdapter(OpenAICompatRoutesMixin, BasePlatformAdapter):
    """aiohttp server routing OpenAI-format requests through moor-agent's AIAgent."""
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # executing model. Requests that send an explicit ``provider`` — and the moor-native session-chat
    ...
    def _resolve_model_name(explicit: str) -> str:
        """Advertised /v1/models name: explicit override > active profile name > "moor-agent"
        (precedence owned by ``moor_cli.model_switch.resolve_effective_model``)."""
        ...
        return resolve_effective_model(explicit, profile_name, "moor-agent")
    ...
    def _expected_api_key(self) -> str:
        ...
        try:
            ...
            from moor_cli.auth import has_usable_secret
        ...
        if not profile:
            with suppress(Exception):
                ...
                if is_multiplex_active():
                    ...
                    return _profile_runtime_scope(get_moor_home())
            return nullcontext()
        from gateway.run import _profile_runtime_scope
    ...
    def _declared_conversation_session(self, gateway_session_key: Optional[str]) -> Optional[str]:
        """Resolve the live session a client declared with ``X-moor-session-Key`` (the key
    ...
    def _parse_session_key_header(
        ...
        """Validate ``X-moor-session-Key`` (per-channel memory scope) -> ``(key_or_None, None)``
    ...
    def _ensure_session_db(self):
        """SessionDB for the active profile home (the runtime scope redirects ``get_moor_home()``
        per profile). Sync, for ``_create_agent``; handlers use ``_ensure_session_db_async``."""
        if self._session_db is not None:
            return self._session_db
        try:
            from moor_constants import get_moor_home
            return self._open_and_cache_session_db(get_moor_home())
    ...
    def _stored_session_model(self, session: Any) -> Optional[str]:
        ...
        "moor-agent" upstream as a provider model id 400s)."""
    ...
    def _normalize_session_source(value: Any) -> str:
        ...
        allowed = {"api_server", "moor_browser", "browser", "cli", "telegram", "discord", "slack", "desktop", "dashboard"}
        if text not in allowed:
            return "api_server"
        return "moor_browser" if text == "browser" else text
    ...
    def _recover_or_record_model(self, model: str, runtime_kwargs: Dict[str, Any], gateway_session_key) -> str:
        ...
        # No model.default but a provider resolved (e.g. `moor auth add` without `moor model`).
    ...
    def _select_agent_runtime(
        ...
        # moor_cli.model_switch.resolve_effective_model.
        from moor_cli.model_switch import resolve_effective_model
        if session_override:
    ...
    async def _handle_health(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response({"status": "ok", "platform": "moor-agent", "version": _moor_version()})
    ...
    async def _handle_health_detailed(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response({
            "status": readiness["status"], "readiness": readiness, "platform": "moor-agent",
            "version": _moor_version(), "gateway_state": gw_state,
    ...
    async def _handle_models(self, request: "web.Request") -> "web.Response":
        """GET /v1/models — moor-agent plus configured model_routes aliases (alias + resolved
        ...
        def _model(mid: str, root: str, parent) -> Dict[str, Any]:
            return {"id": mid, "object": "model", "created": now, "owned_by": "moor", "permission": [],
    ...
    async def _handle_capabilities(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response({
            "object": "moor.api_server.capabilities", "platform": "moor-agent",
    ...
    def _artifact_store_for(self, profile: str) -> ArtifactStore:
        ...
        (``<MOOR_HOME>/plugin-data/.../artifacts``-style controlled root), so artifacts never escape the
        ...
        try:
            ...
            # Unscoped fallback (tests/manual wiring): controlled root under the Moor home.
            try:
                from moor_state import get_moor_home
                root = Path(get_moor_home()) / "artifacts" / "browser-control"
            except Exception:
    ...
    async def _handle_list_sessions(self, request: "web.Request") -> "web.Response":
        ...
        # Exact-title lookup (`moor peer dm` -> canonical "Bot Chat"). include_hidden is honored
        ...
        if title_filter and not sessions:
            # A canonical Bot Chat auto-archived by the orphan reaper would make `moor peer dm`
            ...
            try:
                ...
                # which would fail `moor peer dm` resolution and mint transient sessions — same accident
    ...
    async def _handle_create_session(self, request: "web.Request") -> "web.Response":
        """POST /api/sessions -- create an empty Moor session row. Existence check, insert and
        ...
        # would persist "moor-agent" and later send it to the provider literally.
        ...
        return web.json_response({"object": "moor.session", "session": self._session_response(session)}, status=201)
    ...
    async def _handle_get_session(self, request: "web.Request") -> "web.Response":
        ...
        if err:
            return err
        return web.json_response({"object": "moor.session", "session": self._session_response(session)})
    ...
    async def _handle_delete_session(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response({"object": "moor.session.deleted", "id": session_id, "deleted": bool(deleted)})
    ...
    async def _handle_fork_session(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response({"object": "moor.session", "session": self._session_response(fork)}, status=201)
    ...
    def _session_headers(session_id: str, gateway_session_key: Optional[str]) -> Dict[str, str]:
        """``X-moor-session-Id`` (+ ``X-moor-session-Key`` when declared) response headers."""
    ...
    def _effective_turn_runtime(self, runtime_request: Dict[str, Any], result: Any, usage: Any) -> Dict[str, Any]:
        ...
        return self._sanitize_runtime_metadata(
            ...
            # gateway/status readers resolve process-level paths and do NOT follow the MOOR_HOME
    ...
    async def _handle_session_chat(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response(
            {"object": "moor.session.chat.completion",
    ...
    async def _handle_session_model_lock(self, request: "web.Request") -> "web.Response":
        ...
        runtime = self._sanitize_runtime_metadata(
            ...
            model_lock="accepted")
        return web.json_response(
            {"object": "moor.session.model_lock", "session_id": session_id, "runtime": runtime})
    ...
    async def _handle_run_job(self, request: "web.Request") -> "web.Response":
        ...
        if err:
            return err
        # Optional transient per-run context (standalone `moor cron run` /
    ...
    def _turn_runtime_metadata(
        ...
        runtime = dict(getattr(agent, "_moor_api_runtime", {}) or {})
