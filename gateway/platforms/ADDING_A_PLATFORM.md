...
There are two ways to add a platform to the Moor gateway:

## Plugin Path (Recommended for Community/Third-Party)

Create a plugin directory in `~/.moor/plugins/` (or under `plugins/platforms/`
...
**zero changes to core Moor code**.
...
- `plugin.yaml` `requires_env` / `optional_env` rich-dict entries —
  auto-populate `OPTIONAL_ENV_VARS` in `moor_cli/config.py` so the setup
...
Checklist for integrating a platform directly into the Moor core.
...
And add it to the `moor-gateway` composite:

```python
"moor-gateway": {
    "includes": [..., "moor-your-platform"]
}
```
...
## 12. Status Display (`moor_cli/status.py`)
...
## 13. Gateway Setup Wizard (`moor_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ moor_cli/ toolsets.py \
  --include="*.py" -l | sort -u
