...
def _moor_extras(completed, is_partial, is_failed, err_msg, finish_reason: str) -> Dict[str, Any]:
    return {
...
class OpenAICompatRoutesMixin:
    ...
    async def _handle_chat_completions(self, request: "web.Request") -> "web.Response":
        ...
        # X-moor-session-Key scopes long-term memory per channel; independent of
        # X-moor-session-Id (the key persists across transcripts, the id rotates on /new).
        gateway_session_key, key_err = self._parse_session_key_header(request)
        if key_err is not None:
            return key_err
        # X-moor-session-Id continues an existing session (history from state.db, not the body);
        ...
        if stream:
            ...
            def _on_tool_start(tool_call_id, function_name, function_args):
                """``moor.tool.progress`` status=running; ``_``-prefixed tools stay off the wire."""
        ...
        response_headers = {"X-moor-session-Id": result.get("session_id", session_id)}
        ...
        # Soft partial (some text, run incomplete): 200 + finish_reason="length"/Moor extras.
        response_data = {
    ...
    async def _handle_responses(self, request: "web.Request") -> "web.Response":
        ...
        # Session precedence: previous_response_id chain > declared X-moor-session-Key > fresh
