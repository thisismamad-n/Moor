...
def _account_file(moor_home: str, account_id: str) -> Path:
    return _account_dir(moor_home) / f"{account_id}.json"
...
def save_weixin_account(
    ...
    *,
    ...
    path = _account_file(moor_home, account_id)
...
def _sync_buf_path(moor_home: str, account_id: str) -> Path:
    return _account_dir(moor_home) / f"{account_id}.sync.json"


def _load_sync_buf(moor_home: str, account_id: str) -> str:
    path = _sync_buf_path(moor_home, account_id)
    if not path.exists():
        return ""
    try:
...
class WeixinAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        if self._account_id and not self._token:
            persisted = load_weixin_account(moor_home, self._account_id)
            if persisted:
