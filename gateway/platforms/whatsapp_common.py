...
def resolve_whatsapp_bridge_dir() -> Path:
    """Resolve the WhatsApp bridge directory, mirroring to MOOR_HOME if needed.

    When the install tree is read-only (e.g., Docker /opt/moor), this function
    mirrors the bridge source to a writable MOOR_HOME location and returns that
    ...
    # Try MOOR_HOME location first
    moor_home = get_moor_home()
    moor_home_bridge = moor_home / "scripts" / "whatsapp-bridge"
    ...
    # Install dir is read-only, mirror to MOOR_HOME if needed
    if moor_home_bridge.exists():
        return moor_home_bridge

    # Mirror the bridge source to MOOR_HOME
    try:
        moor_home_bridge.parent.mkdir(parents=True, exist_ok=True)
        shutil.copytree(
            install_bridge,
            moor_home_bridge,
            dirs_exist_ok=False,
