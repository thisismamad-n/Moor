...
``gateway/assets/status_phrases.yaml``; users add profile-relative catalogs under ``MOOR_HOME``
via ``status_phrases.yaml`` / ``status_phrases/*.yaml`` or ``display.status_phrases: {path:
<MOOR_HOME-relative>, mode: append|replace}``. Absolute paths and ``..`` escapes are ignored on
...
# Moor UI surfaces, not app/vendor buckets.  Long-running-only: regular tool/thinking/interim
...
def resolve_status_phrase_catalog(user_config: Mapping[str, Any] | None,
    ...
    for section in sections:
        for key in _CONFIG_KEYS:
            _merge_phrase_config(catalog, section.get(key), base_dir=moor_home)
    return catalog


def classify_status_context(kind: str, *, tool_name: str | None = None, preview: str | None = None,
                            args: Any = None) -> str:
    """Classify an internal gateway event into a Moor UI-surface bucket."""
