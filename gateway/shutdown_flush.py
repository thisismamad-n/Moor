...
All hooks write atomic JSON payloads under ``<moor_home>/pending_messages/``:
...
def spool_dropped_transcript_message(session_id: str, message: Dict[str, Any]) -> Optional[Path]:
    ...
    message under ``<moor_home>/pending_messages/``), so a runtime cap rotation no longer silently
