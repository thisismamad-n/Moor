...
class GatewayGoalsMixin:
    ...
    def _goal_max_turns_from_config(self) -> int:
        ...
        top-level ``goals`` block is only reachable via moor_cli.config.load_config()."""
        try:
            goals_cfg = (
    ...
    async def _warm_goals_session_db(self, label: str) -> None:
        ...
        try:
            from moor_cli.goals import _get_session_db as _warm_goals_db
    ...
    async def _defer_goal_status_notice_after_delivery(self, source: Any, message: str) -> None:
        ...
        if session_key and hasattr(adapter, "register_post_delivery_callback"):
            try:
                ...
                generation = getattr(active, "_moor_run_generation", None) if active is not None else None
    ...
    async def _loop_wakeup_fire_one(self, sid: str, state: Any, now: float, warned_no_route: set) -> None:
        ...
        # fired. The context-preserving executor keeps the profile MOOR_HOME override under multiplex.
