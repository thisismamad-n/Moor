...
Replaces the old ``os.environ``-based ``MOOR_SESSION_*`` state with task-local ``ContextVar``s
...
_SESSION_VARS = (
    ...
    "MOOR_SESSION_PLATFORM", "MOOR_SESSION_SOURCE", "MOOR_SESSION_CHAT_ID",
    "MOOR_SESSION_CHAT_TYPE", "MOOR_SESSION_CHAT_NAME", "MOOR_SESSION_THREAD_ID",
    "MOOR_SESSION_USER_ID", "MOOR_SESSION_USER_ID_ALT", "MOOR_SESSION_USER_NAME",
    "MOOR_SESSION_SCOPE_ID", "MOOR_SESSION_KEY", "MOOR_SESSION_ID",
    "MOOR_UI_SESSION_ID", "MOOR_SESSION_MESSAGE_ID", "MOOR_SESSION_PROFILE",
    "MOOR_BROWSER_CONTROL_PRINCIPAL", "MOOR_BROWSER_CONTROL_TRANSPORT_FAMILY",
    "MOOR_CRON_SESSION", "MOOR_SESSION_PARENT_CHAT_ID",
))
...
_SESSION_ASYNC_DELIVERY = ContextVar("MOOR_SESSION_ASYNC_DELIVERY", default=_UNSET)
...
_SESSION_HISTORY_DELIVERY = ContextVar("MOOR_SESSION_HISTORY_DELIVERY", default=_UNSET)

# Cron auto-delivery vars, set per-job in run_job() so concurrent jobs don't clobber.
_CRON_AUTO_DELIVER_PLATFORM = ContextVar("MOOR_CRON_AUTO_DELIVER_PLATFORM", default=_UNSET)
_CRON_AUTO_DELIVER_CHAT_ID = ContextVar("MOOR_CRON_AUTO_DELIVER_CHAT_ID", default=_UNSET)
_CRON_AUTO_DELIVER_THREAD_ID = ContextVar("MOOR_CRON_AUTO_DELIVER_THREAD_ID", default=_UNSET)
...
def set_current_session_id(session_id: str) -> None:
    """Synchronize ``MOOR_SESSION_ID`` across ContextVar and ``os.environ`` (tools read it
...
def get_session_env(name: str, default: str = "") -> str:
    """Read a session var by legacy ``MOOR_SESSION_*`` name; drop-in for os.getenv.  The
...
# Surfaces that are not a human chat channel (gateway binds MOOR_SESSION_PLATFORM, CLI/TUI/
# desktop bind MOOR_SESSION_SOURCE, so both are consulted).  Default-deny: an unrecognized
...
def session_is_messaging_surface() -> bool:
    ...
    ``MOOR_PLATFORM``, then the session platform, then the session source)."""
    platform = os.getenv("MOOR_PLATFORM") or get_session_env("MOOR_SESSION_PLATFORM", "")
    idents = (platform, get_session_env("MOOR_SESSION_SOURCE", ""))
...
def async_delivery_supported() -> bool:
    ...
    (``MOOR_KANBAN_TASK``: one-shot subprocesses whose parent disappears after the turn)."""
