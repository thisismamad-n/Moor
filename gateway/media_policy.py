...
``validate_media_delivery_path`` reads ``MOOR_MEDIA_DELIVERY_STRICT`` (gateway.strict),
``MOOR_MEDIA_ALLOW_DIRS`` (gateway.media_delivery_allow_dirs) and
``MOOR_MEDIA_TRUST_RECENT_FILES`` (gateway.trust_recent_files).  Every delivery
entrypoint (gateway startup, ``moor cron run``, ``moor send``) calls
