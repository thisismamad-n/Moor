...
Defenses 1 and 2 (the ``_MOOR_GATEWAY`` guard on ``moor gateway
...
raw ``terminal("launchctl kickstart -k gui/<uid>/ai.moor.gateway")``,
...
State lives in ``<MOOR_HOME>/gateway/restart_loop.json`` so it is
...
def _state_path():
    return get_moor_home() / "gateway" / "restart_loop.json"
