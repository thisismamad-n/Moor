...
try:
    ...
    pass  # a partial ``moor update`` can leave the bootstrap unregistered; only Windows UTF-8 stdio suffers
...
async def run_codex_hygiene_compaction(
    ...
    * Evicting the cached live agent afterwards destroys the only real context: the next turn spawns an
    EMPTY thread and the model starts blank while Moor still mirrors a full history (abrupt amnesia — the
    user-facing damage documented on #73503).
    """
    mode = str(auto_mode or "native").lower()
    if mode not in {"native", "moor", "off"}:
        mode = "native"
    if mode != "moor":
    ...
    # copy_context carries profile secret scope / MOOR_HOME override (executors don't propagate ContextVars).
...
def _telegramize_command_mentions(text: str, platform: Any) -> str:
    ...
    from moor_cli.commands_platforms import _sanitize_telegram_name
...
def _startup_restore_drain_timeout_secs() -> float:
    ...
    """
    return _float_env("MOOR_STARTUP_RESTORE_DRAIN_TIMEOUT", _STARTUP_RESTORE_DRAIN_TIMEOUT_SECS_DEFAULT)
...
def _startup_warmup_timeout_secs() -> float:
    ...
    return _float_env("MOOR_STARTUP_WARMUP_TIMEOUT", _STARTUP_WARMUP_TIMEOUT_SECS_DEFAULT)
...
def _build_gateway_agent_history(
    ...
    from moor_time import get_timezone as _get_msg_tz
...
def _restart_notification_pending() -> bool:
    ...
    return (_moor_home / ".restart_notify.json").exists()


def _planned_restart_notification_path() -> Path:
    return _moor_home / ".restart_pending.json"
...
_env_path = _moor_home / '.env'
load_moor_dotenv(moor_home=_moor_home, project_env=Path(__file__).resolve().parents[1] / '.env')


def _reload_runtime_env_preserving_config_authority() -> None:
    """Reload .env per turn for rotated keys while config.yaml stays authoritative for budgets (else a
    stale MOOR_MAX_ITERATIONS wins). Multiplex never reloads .env globally: secrets come from the
    ...
    if not is_multiplex_active():
        load_moor_dotenv(
            moor_home=_moor_home, project_env=Path(__file__).resolve().parents[1] / '.env')
    _bridge_max_turns_from_config(_moor_home)
...
def _profile_runtime_scope(
    ...
    ``set_moor_home_override`` is a contextvar (reaches the agent worker via ``copy_context()``);
...
async def _discover_gateway_mcp_tools(config: object) -> None:
    ...
    ``mcp_servers`` from ``get_moor_home()``'s config, so an unscoped call only connects the launch
...
def _platform_has_bot_credential(platform: "Platform", platform_config: "PlatformConfig") -> bool:
    ...
    # ~/.moor/.env, so an env fallback would report "has credential" for every Matrix config on the box —
...
from moor_cli.config_defaults import DEFAULT_CONFIG as _DEFAULT_CONFIG
os.environ["MOOR_TURN_LEASE_TIMEOUT"] = str(_DEFAULT_CONFIG["agent"]["gateway_turn_lease_timeout"])
...
_AGENT_ENV_BRIDGE = {
    ...
    "reconnect_attention_after": "MOOR_RECONNECT_ATTENTION_AFTER_SECONDS",
...
_DISPLAY_ENV_BRIDGE = {
    "busy_input_mode": "MOOR_GATEWAY_BUSY_INPUT_MODE",
    "busy_text_mode": "MOOR_GATEWAY_BUSY_TEXT_MODE",
    "busy_ack_enabled": "MOOR_GATEWAY_BUSY_ACK_ENABLED"}
...
def _bridge_max_turns_to_env(agent_cfg: Any) -> None:
    ...
    if raw is not None:
        os.environ["MOOR_MAX_ITERATIONS"] = str(raw)
    elif "MOOR_MAX_ITERATIONS" in os.environ:
        del os.environ["MOOR_MAX_ITERATIONS"]
...
def _bridge_config_to_env(_cfg: dict) -> None:
    ...
    # (e.g. MOOR_MAX_ITERATIONS=60 written by an old `moor setup` run) silently shadow the user's
    ...
    if (isinstance(_display_cfg, dict) and "busy_steer_ack_enabled" in _display_cfg
            and "MOOR_GATEWAY_BUSY_STEER_ACK_ENABLED" not in os.environ):
        os.environ["MOOR_GATEWAY_BUSY_STEER_ACK_ENABLED"] = str(_display_cfg["busy_steer_ack_enabled"])
    _tz_cfg = _cfg.get("timezone", "")
    if _tz_cfg and isinstance(_tz_cfg, str):
        os.environ["MOOR_TIMEZONE"] = _tz_cfg.strip()
    _security_cfg = _cfg.get("security", {})
    if isinstance(_security_cfg, dict) and _security_cfg.get("redact_secrets") is not None:
        os.environ["MOOR_REDACT_SECRETS"] = str(_security_cfg["redact_secrets"]).lower()
    # Media policy uses the shared bridge so standalone entrypoints (`moor cron run`) match.
    ...
    if isinstance(_gateway_cfg, dict):
        ...
        if _trust_recent_seconds is not None:
            os.environ["MOOR_MEDIA_TRUST_RECENT_SECONDS"] = str(_trust_recent_seconds)
        ...
        if ("platform_connect_timeout" in _gateway_cfg
                and not os.environ.get("MOOR_GATEWAY_PLATFORM_CONNECT_TIMEOUT", "").strip()):
            os.environ["MOOR_GATEWAY_PLATFORM_CONNECT_TIMEOUT"] = str(_gateway_cfg["platform_connect_timeout"])
...
_config_path = _moor_home / 'config.yaml'
_cfg: dict = {}
if _config_path.exists():
    try:
        ...
        print(
            ...
            "your current config.yaml. Run `moor doctor` to investigate.",
            file=sys.stderr)
...
try:
    from moor_cli.config import warn_deprecated_cwd_env_vars
    warn_deprecated_cwd_env_vars()
except Exception as _bootstrap_exc:
    print(f"  Warning: deprecation check failed: {_bootstrap_exc}", file=sys.stderr)

os.environ["MOOR_QUIET"] = "1"  # gateway runs quiet: no debug output, cwd used directly

# MOOR_EXEC_ASK is set in start_gateway(), NOT at import: CLI tools importing this module must not
...
def _resolve_gateway_model_context(model: Optional[str] = None) -> _GatewayModelContext:
    ...
    def _custom_ctx() -> Optional[int]:
        from moor_cli.config import get_custom_provider_context_length
...
def _deep_merge_request_overrides(base: Optional[dict], override: Optional[dict]) -> dict:
    ...
    from moor_cli.config import _deep_merge
            ...
            for skill_md in skills_dir.rglob("SKILL.md"):
                ...
                if slug == normalized and declared_name in disabled:
                    return (
                        ...
                        f"Enable it with: `moor skills config`")
        ...
        if optional_dir.exists():
            for skill_md in optional_dir.rglob("SKILL.md"):
                ...
                return (
                    ...
                    f"Install it with: `moor skills install {install_path}`")
    except Exception:
        pass
    return None
...
def _gateway_config_home() -> Path:
    ...
    return Path(override) if override else _moor_home
...
def _load_gateway_config(config_path: "Path | None" = None) -> dict:
    ...
    Defaults to the active gateway home (``_moor_home`` monkeypatches apply); multiplexers pass a path.
    """
...
def _resolve_moor_bin() -> Optional[list[str]]:
    """Moor update command argv: ``moor`` on PATH, else ``python -m moor_cli.main``, else None."""
    import shutil
...
_RECONNECT_ATTENTION_AFTER_SECONDS = _float_env("MOOR_RECONNECT_ATTENTION_AFTER_SECONDS", 7200)
...
_BUILTIN_ADAPTERS: dict[Platform, tuple[str, str, str, str]] = {
    Platform.WHATSAPP_CLOUD: ("whatsapp_cloud", "WhatsAppCloudAdapter", "check_whatsapp_cloud_requirements",
                              "WhatsApp Cloud: aiohttp/httpx missing — reinstall moor-agent"),
...
class GatewayRunner(
    ...
    def _init_session_db(self) -> None:
        ...
        try:
            ...
            # WARNING (not DEBUG) so it lands in errors.log; else an NFS MOOR_HOME silently loses /resume etc.
        ...
        try:
            ...
            if _ckpt_cfg.get("auto_prune", False):
                ...
                # unmounted share); orphan cleanup is only via explicit `moor checkpoints prune`.
    ...
    def _open_session_db_for_active_scope(self, raise_on_error: bool = False) -> Any:
        ...
        ``SessionDB()`` reads the context-local MOOR_HOME; one handle cached per path. Construction
        ...
        resolves ``_default_db_path()`` at call time through the context-local MOOR_HOME override
        ...
        #88235 broadcast.
        """
        from moor_state import AsyncSessionDB, _default_db_path
        ...
        def _open():
            ...
            if borrowed is not None:
                ...
                wrapper.__dict__["_moor_borrowed_handle"] = True
                return wrapper
    ...
    def close_all_session_db_handles(self) -> None:
        ...
        """
        def _close(db) -> None:
            if getattr(db, "__dict__", {}).get("_moor_borrowed_handle"):
                return
    ...
    def _warn_if_docker_media_delivery_is_risky(self) -> None:
        ...
        logger.warning(
            ...
            "output mount (for example '/home/user/.moor/cache/documents:/output') is configured. "
            "This is fine if the model already emits host-visible paths, but MEDIA file delivery can fail "
            "for container-local paths like '/workspace/...' or '/output/...'.")

    _VOICE_MODE_PATH = _moor_home / "gateway_voice_mode.json"
        ...
        with lock:
            ...
            if executor is None or getattr(executor, "_shutdown", False):
                executor = concurrent.futures.ThreadPoolExecutor(
                    max_workers=10, thread_name_prefix="moor-gateway")
    ...
    def _resolve_profile_home_for_source(self, source: SessionSource) -> "Path":
        """Resolve which profile's MOOR_HOME serves this source: ``source.profile``, then
        ...
        try:
            ...
            if explicit_profile and not profile_exists(name):
                logger.warning(
                    ...
                    "falling back to global MOOR_HOME",
            ...
            logger.warning(
                ...
                "falling back to global MOOR_HOME: %s",
...
def _run_planned_stop_watcher(
    ...
    ``add_signal_handler``, so ``moor gateway stop`` would never drain). Runs everywhere; on POSIX
    ...
    standard signal-driven shutdown path never runs when ``moor gateway stop`` signals the gateway. The
...
def _housekeeping_paste_sweep() -> None:
    from moor_cli.debug import _sweep_expired_pastes
...
def _housekeeping_deferred_fts_retry() -> None:
    ...
    from moor_state_registry import borrow_live_shared_session_dbs
...
def _drain_restart_safe_cron_deliveries(adapters, loop, runner=None) -> None:
    ...
    for profile_name, profile_home in _handoff_watch_scopes(runner):
        ...
        if profile_adapters is None:
            continue
        with _profile_runtime_scope(profile_home or get_moor_home()):
...
def _replace_target_belongs_to_other_profile(existing_pid: int) -> bool:
    ...
    # (no service supervisor — e.g. `moor gateway run` on Windows) is at risk: its PID never joins the
    ...
    try:
        from gateway.status import (
            _get_pid_path, _get_process_moor_home, _get_process_start_time, _pid_from_record,
            _read_pid_record, _record_looks_like_gateway, _read_process_cmdline, _same_moor_home)
        our_home = _get_process_moor_home()
        ...
        if _get_process_start_time(existing_pid) != recorded_start:
            return refuse("pid record start-time does not match the live process %s (stale/PID-reuse record).",
                          existing_pid)
        recorded_home = record.get("moor_home")
        if not isinstance(recorded_home, str) or not recorded_home.strip():
            return refuse("pid record predates moor_home stampings; ownership of PID %s unprovable.",
                          existing_pid)
        if not _same_moor_home(recorded_home, our_home):
            return refuse("pid record belongs to a different MOOR_HOME (%s, ours %s). Remove the stale PID "
                          "record or stop the owning profile explicitly.", recorded_home, our_home,
                          level=logging.ERROR)
        # Argv never proves ownership; an explicit contradicting --profile / MOOR_HOME= still refuses.
        ...
        if live_cmdline and _looks_like_profile_conflict_from_cmdline(live_cmdline, our_home):
            return refuse("target PID %s command line explicitly advertises a different profile than "
                          "MOOR_HOME %s.", existing_pid, our_home, level=logging.ERROR)
        return False
    except Exception:
...
def _looks_like_profile_conflict_from_cmdline(command: str, our_home) -> bool:
    ...
    def _env_home_value() -> Optional[str]:
        """MOOR_HOME=<path> env-style assignment on the argv, token-exact."""
        prefix = "MOOR_HOME="
    ...
    for flag in ("--profile", "-p"):
        ...
        if profile_name is None or profile_name == "default" or value != profile_name:
            return True
    home_value = _flag_value("--moor-home") or _env_home_value()
...
async def _start_gateway_replace_existing_instance(existing_pid: int, replace: bool) -> bool:
    """Handle a live gateway PID under this MOOR_HOME: replace it (``--replace``) or refuse.
    ...
    if not replace:
        ...
        logger.error(
            "Another gateway instance is already running (PID %d, MOOR_HOME=%s). "
            "Use 'moor gateway restart' to replace it, or 'moor gateway stop' first.",
            existing_pid, moor_home)
        print(
            f"\n❌ Gateway already running (PID {existing_pid}).\n"
            f"   Use 'moor gateway restart' to replace it,\n"
            f"   or 'moor gateway stop' to kill it first.\n"
            f"   Or use 'moor gateway run --replace' to auto-replace.\n")
        return False
    ...
    if _replace_target_belongs_to_other_profile(existing_pid):
        from gateway.status import _get_process_moor_home
        logger.error(
            "Refusing --replace: PID %d cannot be proven to belong "
            "to this profile's gateway (MOOR_HOME %s). Remove the "
            "stale PID record or stop the owning profile explicitly.",
            existing_pid, _get_process_moor_home())
        return False
    ...
    with suppress(Exception):
        (get_moor_home() / "gateway.pid").unlink(missing_ok=True)
...
def _start_gateway_configure_logging(verbosity: Optional[int]) -> None:
    ...
    from moor_logging import setup_logging, _safe_stderr
    setup_logging(moor_home=_moor_home, mode="gateway")

    def _security_audit() -> None:
        # Warn-on-load, never blocks: surfaces root / weak-SSH / unauthenticated-listener exposure.
        from moor_cli.security_audit_startup import log_startup_security_warnings

        def _raw_cfg():
            from moor_cli.config import read_raw_config
            return read_raw_config()

        log_startup_security_warnings(moor_home=_moor_home, config=_best_effort(_raw_cfg))
        ...
        if _shutdown_ctx is not None:
            ...
            def _diagnostic() -> None:
                ...
                spawn_async_diagnostic(
                    _moor_home / "logs" / "gateway-shutdown-diag.log", _shutdown_ctx["signal"], timeout_seconds=5.0)
...
async def _start_gateway_start_control_socket(runner):
    ...
    try:
        ...
        # becomes the authoritative gateway for its MOOR_HOME, so from here on "does a socket answer?" is
    ...
    if not isinstance(cron_provider, InProcessCronScheduler):
        ...
        if not _has_api_server:
            logger.warning(
                ...
                "the gateway through its supervisor (`moor gateway "
...
async def _start_gateway_shutdown_tail(
    ...
    def _stop_keepalive() -> None:
        from moor_cli.moor_auth_keepalive import stop_moor_auth_keepalive
        stop_moor_auth_keepalive()
...
async def start_gateway(config: Optional[GatewayConfig] = None, replace: bool = False, verbosity: Optional[int] = 0) -> bool:
    ...
    # Duplicate-instance guard scoped to MOOR_HOME; distinct-home multi-profile setups coexist.
    ...
    # Windows has no add_signal_handler, so `moor gateway stop`'s SIGTERM would never drain; poll the
    # planned-stop marker (written BEFORE the kill) instead. Runs everywhere so masked-SIGTERM drains.
    # Windows fallback: asyncio.add_signal_handler raises NotImplementedError on Windows, so `moor gateway
    ...
    # sessions are silently lost across restarts (issue #33778). The fix is a marker-polling thread: `moor
    ...
    _best_effort(_start_keepalive, "Moor auth keepalive did not start: %s")
...
def _guard_corrupt_user_config() -> None:
    ...
    named. Same policy and escape hatch (``MOOR_IGNORE_USER_CONFIG=1``) as ``moor_cli/main.py``."""
    from moor_cli.config import InvalidUserConfigError, require_parseable_user_config
...
def main():
    ...
    # Advertise the harness to children (mirrors _advertise_agent_env in moor_cli/main.py, inlined to
    # avoid its startup side effects). Value must equal registry id ``moor-agent`` exactly.
    ...
    def _arm_watchdog() -> None:
        ...
        from moor_startup_watchdog import arm_startup_watchdog
...
def _exit_after_graceful_shutdown(exit_code: int) -> None:
    ...
    def _drain_logs() -> None:
        ...
        from moor_logging import drain_log_queue
