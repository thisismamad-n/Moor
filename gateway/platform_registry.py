...
class PlatformEntry:
    ...
    required_env: list = field(default_factory=list)  # ``moor setup`` display
    ...
    plugin_name: str = ""  # owning manifest so ``moor gateway setup`` can auto-enable it
...
class PlatformRegistry:
    ...
    def __init__(self) -> None:
        ...
        # `moor` invocation, so the import happens only when a lookup asks for it.
