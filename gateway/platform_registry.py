...
class PlatformEntry:
    ...
    # status displays (``moor setup``, ``moor status``, the dashboard
    ...
    # Env vars this platform needs (for ``moor setup`` display).
    ...
    # built-ins).  Used by ``moor gateway setup`` to auto-enable the
...
class PlatformRegistry:
    ...
    def __init__(self) -> None:
        ...
        # Plugin adapters are isolated per resolved MOOR_HOME and overlay the
        ...
        # plugin-discovery time added several seconds to *every* `moor`
        # invocation -- including plain `moor chat`, which never touches any
        ...
        # `moor setup`/`gateway status`, send_message).
    ...
    def _resolve_all(self) -> None:
        ...
        gateway startup, ``moor setup``/``gateway status``, channel
