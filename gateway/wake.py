...
* Stateless request/response adapters (the API server,
  ...
  ``X-Moor-Session-Id`` key real gateway/HQ turns run under
  ...
  server with the raw session id in the ``X-Moor-Session-Id`` header — the
...
async def deliver_wake(
    ...
    *,
    ...
    ``session_id`` is the RAW session id (the ``X-Moor-Session-Id`` value /
...
async def _self_post_chat_completion(
    ...
    Session continuation via ``X-Moor-Session-Id`` is 403-gated on
