...
matches the raw ``X-moor-session-Id`` real turns use (invisible parallel session), so we self-POST
...
async def _self_post_chat_completion(adapter: Any, *, text: str, session_id: str) -> None:
    ...
    own bind host/port/key. Session continuation via ``X-moor-session-Id`` is 403-gated on
    ...
    if not api_key:
        raise RuntimeError("wake self-post requires API_SERVER_KEY: session continuation via "
                           "X-moor-session-Id is rejected (403) on an unauthenticated API "
                           "server, so the wake cannot reach the target session")
    if ":" in host and not host.startswith("["):
        host = f"[{host}]"  # bare IPv6 literal
    url = f"http://{host}:{port}/v1/chat/completions"
    headers = {"Authorization": f"Bearer {api_key}", "X-moor-session-Id": session_id}
