...
_SESSIONS_JSON_README = (
    ...
    "table in ~/.moor/state.db). Maps messaging session keys (agent:main:<platform>:...) to "
    "active session IDs. This is NOT the session list. ALL sessions (CLI, TUI, and gateway) live "
    "in ~/.moor/state.db and are shown by `moor sessions list` and `/sessions`. Disable this "
...
class SessionPersistenceMixin:
    ...
    def _open_session_db_for_active_scope(self, db_path: Optional[Path] = None):
        ...
        ``_default_db_path()`` follows the context-local MOOR_HOME (resolved per call so
    ...
    def _profile_home_for_key(self, session_key: Optional[str]) -> Optional[Path]:
        """MOOR_HOME of the profile owning *session_key*, or None (no named owner or
        unresolvable)."""
        ...
        try:
            from moor_cli.profiles import get_profile_dir, profile_exists
    ...
    def _db_for_key(self, session_key: Optional[str]):
        ...
        is encoded in the key). ``_db`` follows the ambient MOOR_HOME that only the inbound message
    ...
    def close_all_db_handles(self) -> None:
        ...
        def _close(db) -> None:
            from moor_state_registry import release_or_close  # shared instances no-op on close()
            try:
                release_or_close(db)
            except Exception as exc:
