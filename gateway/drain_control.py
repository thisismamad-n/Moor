...
Contract (presence-based, mirroring ``.restart_notify.json``):
  ...
  * begin-drain  → write ``{MOOR_HOME}/.drain_request.json`` with
...
Why the epoch (NS-570). ``MOOR_HOME`` is a **durable** store — on Moor
...
def current_instantiation_epoch() -> str:
    ...
    Together they discriminate every restart mode that matters:
      ...
      | host ``moor gateway restart``| same    | same(init) | SAME   | honour |
...
def drain_request_path(home: Optional[Path] = None) -> Path:
    ...
    base = home if home is not None else get_moor_home()
...
def write_drain_request(
    *,
    ...
    later survives a machine restart on the durable MOOR_HOME volume can be
...
def drain_requested(*, home: Optional[Path] = None) -> bool:
    ...
    treated as absent: it survived a container/VM restart (MOOR_HOME is a
    durable Fly volume on Moor Cloud) and the lifecycle action that triggered
...
def drain_notification_suppressed(*, home: Optional[Path] = None) -> bool:
    ...
    MOOR_HOME volume, or an expired same-epoch orphan (#85433), is
