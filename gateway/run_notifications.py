...
class GatewayNotificationsMixin:
    ...
    class _UpdatePaths:
        """Marker files ``moor update --gateway`` and its watcher exchange under MOOR_HOME."""
    ...
    def _update_paths(cls) -> "GatewayNotificationsMixin._UpdatePaths":
        ...
        return cls._UpdatePaths(
            pending=_moor_home / ".update_pending.json",
            claimed=_moor_home / ".update_pending.claimed.json", output=_moor_home / ".update_output.txt",
            exit_code=_moor_home / ".update_exit_code",
            prompt=_moor_home / ".update_prompt.json", response=_moor_home / ".update_response",
        )
    ...
    async def _watch_update_progress(
        ...
        while loop.time() < deadline:
            if paths.exit_code.exists():
                ...
                with _log_suppressed(logging.WARNING, "Update final notification failed: %s"):
                    ...
                    await target.send(
                        "✅ Moor update finished." if exit_code == 0
                        else "❌ Moor update failed (exit code {}).".format(exit_code)
                    )
        ...
        if not paths.exit_code.exists():
            ...
            await _flush_buffer()
            with suppress(Exception):
                await target.send("❌ Moor update timed out after 30 minutes.")
            self._clear_update_markers(paths, session_key)
        ...
        try:
            ...
            if chat_id:
                ...
                if output:
                    if len(output) > 3500:
                        output = "…" + output[-3500:]
                    status = "✅ Moor update finished." if exit_code == 0 else "❌ Moor update failed."
                    msg = f"{status}\n\n```\n{output}\n```"
                else:
                    msg = (
                        "✅ Moor update finished successfully." if exit_code == 0 else
                        "❌ Moor update failed. Check the gateway logs or run `moor update` manually for details."
                    )
    ...
    async def _send_restart_notification(self) -> Optional[tuple[str, str, Optional[str]]]:
        ...
        from gateway.run import _moor_home, _non_conversational_metadata
        notify_path = _moor_home / ".restart_notify.json"
    ...
    async def _send_session_db_warning_notifications(self) -> None:
        ...
        if not error:
            return
        from moor_state import _default_db_path, classify_persistence_error, format_session_db_unavailable
        if classify_persistence_error(error) == "corrupt":
            # Copy-pasteable, so name the real store (profiles / MOOR_HOME do not live under ~/.moor).
            ...
            message = (
                ...
                f"   moor sessions recover --source {db_path} "
                "--inspect-only\n"
                "   (if it reports recoverable) moor sessions recover "
