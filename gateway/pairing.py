...
failed approvals, chmod 0600 data files, codes never logged. Storage: ~/.moor/pairing/
"""
...
# path would freeze the MOOR_HOME/profile context at gateway boot, ignoring later
# context-local overrides, so the gateway and ``moor pairing`` CLI wrote different
...
# computing the path eagerly freezes it to whatever MOOR_HOME/profile context existed at that exact import
# moment for the rest of the process's lifetime -- even if a context-local override (see
# moor_constants.set_moor_home_override) is established afterward. A freshly-started, short-lived
# process (e.g. the ``moor pairing`` CLI) re-imports this module later with the final environment already
...
def _default_pairing_dir() -> Path:
    return PAIRING_DIR if PAIRING_DIR is not None else get_moor_dir("platforms/pairing", "pairing")
...
def _load_json_file(path: Path) -> dict:
    ...
    try:
        ...
        try:
            ...
            # exec` runs as root and writes the file, then the gateway process — running as `moor` after
...
class PairingStore:
    ...
    profile's MOOR_HOME exactly as ``moor -p <name> pairing ...`` does, so multiplex
    ...
    def __init__(self, profile: Optional[str] = None):
        ...
        self._dir = get_moor_dir("platforms/pairing", "pairing", home=profile_home) if profile else _default_pairing_dir()
