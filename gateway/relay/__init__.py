"""Relay/connector support package for the Moor gateway.
...
def relay_display_name() -> Optional[str]:
    ...
    if not value:
        ...
        # prefix every reply "**Moor Agent:**" and shadow the connector's
...
def _resolve_relay_identity_token() -> str:
    ...
    Canonical resolver shared by runtime self-provision and ``moor gateway enroll``.
    Modes, in precedence order:
      ...
      2.  Moor Portal (default): ``resolve_moor_access_token()``.
...
def self_provision_relay() -> bool:
    ...
    ``~/.moor/.env``), so a hosted container re-provisions every boot; the
