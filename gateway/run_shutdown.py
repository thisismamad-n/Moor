...
# exit (bounded), then spawns ``moor gateway restart``.
...
class GatewayShutdownMixin:
    ...
    def _scale_to_zero_has_live_background_work(self) -> bool:
        ...
        PERMANENT supervised watchers (_moor_supervised_watcher, incl. the scale-to-zero watcher
        itself) are excluded, else this would be True forever and the gateway could never go dormant.
        """
        if any(
            not t.done() and not getattr(t, "_moor_supervised_watcher", False)
    ...
    def _pause_failed_platform(self, platform, *, reason: str = "") -> None:
        ...
        logger.warning(
            ...
            "resume %s` to retry, or `moor gateway restart` to restart the gateway.",
    ...
    def _stuck_loop_counts_path(self) -> Path:
        ...
        return _moor_home / self._STUCK_LOOP_FILE
    ...
    @staticmethod
    def _spawn_windows_restart_watcher(moor_cmd: list, current_pid: int, restart_after_s: float) -> None:
    ...
    async def _launch_detached_restart_command(self) -> None:
        ...
        if not moor_cmd:
            logger.error("Could not locate moor binary for detached /restart")
            return
        ...
        if sys.platform == "win32":
            GatewayShutdownMixin._spawn_windows_restart_watcher(moor_cmd, current_pid, restart_after_s)
            return
        cmd = " ".join(shlex.quote(part) for part in moor_cmd)
        shell_cmd = (
    ...
    def _wedged_agent_count(self) -> int:
        ...
        timeout = _float_env("MOOR_AGENT_TIMEOUT", 1800)
        if timeout <= 0:
            return 0
    ...
    def _stop_quiesce_and_close_session_dbs(self, timeout: float, ctx: "GatewayShutdownMixin._StopContext") -> None:
        ...
        def _close_shared() -> None:
            ...
            # (#90837).
            from moor_state_registry import close_all
            closed = close_all()
            if closed:
    ...
    def _stop_persist_exit_state(self, ctx: "GatewayShutdownMixin._StopContext") -> None:
        ...
        from gateway.run import _moor_home, _planned_restart_notification_path, _shutdown_gateway_health_export
        from utils import atomic_json_write
        ...
        if not ctx.timed_out:
            with suppress(Exception):
                (_moor_home / ".clean_shutdown").touch()
        else:
            logger.info(
