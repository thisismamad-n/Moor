...
class GatewayStatusCommandsMixin:
    ...
    async def _handle_topup_command(self, event: MessageEvent) -> str:
        """Handle /topup -- show the Moor balance and hand off to the portal. Does NOT charge, confirm,
        ...
        lines = ["💳 **Moor balance**"] + [ln for ln in view.balance_lines if not ln.lstrip().startswith("📈")]
    ...
    async def _handle_usage_command(self, event: MessageEvent) -> str:
        ...
        # Moor credits + monthly-grant gauge (shared with CLI/TUI). Gates on "a Moor account is
        # logged in" — NOT the inference provider — so a Moor user inferring elsewhere still sees
        # a balance. Fail-open: never break /usage.
        from agent.account_usage import moor_credits_lines
        credits_lines = await _quiet(lambda: asyncio.to_thread(moor_credits_lines, markdown=True), [])
    ...
    async def _handle_insights_command(self, event: MessageEvent) -> str:
        ...
        try:
            ...
            # Not a bare hop: ``SessionDB()`` resolves ``get_moor_home()`` at call time, a
