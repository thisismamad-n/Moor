...
(the updater, `moor serve`/dashboard, the Desktop app) currently discovers
...
- ``identify`` → pid, profile label, moor_home, code_sha/code_version
...
- POSIX: Unix domain socket at ``$MOOR_HOME/gateway.sock``. When the home
  path is too long for ``sun_path`` (~104 bytes on macOS/BSD), the socket is
  bound in the system temp dir and a pointer file
  ``$MOOR_HOME/gateway.sock.path`` records the real location; clients
  follow the pointer transparently.
- Windows: named pipe ``\\\\.\\pipe\\moor-gateway-<home-hash>`` served via
...
Consumers (``moor update --plan`` inventory, the post-update fleet version
...
def windows_pipe_name(home: Path) -> str:
    """Per-MOOR_HOME named pipe path (Windows transport)."""
    return rf"\\.\pipe\moor-gateway-{_home_hash(home)}"
...
def _fallback_socket_path(home: Path) -> Path:
    ...
    """
    name = f"moor-gw-{_home_hash(home)}.sock"
...
def _detect_supervisor() -> str:
    ...
    if sys.platform == "darwin" and (
        env.get("XPC_SERVICE_NAME", "").startswith("ai.moor")
        or env.get("LAUNCHD_SOCKET")
    ):
        return "launchd"
    if env.get("MOOR_DESKTOP_MANAGED"):
        return "desktop"
...
def build_identify_payload() -> dict[str, Any]:
    ...
    payload: dict[str, Any] = {
        ...
        "moor_home": record.get("moor_home"),
        "profile": _profile_label_for_home(record.get("moor_home") or ""),
...
class GatewayControlServer:
    ...
    its MOOR_HOME), ``stop()`` on shutdown. All failures are non-fatal —
