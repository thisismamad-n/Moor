...
# 5. Symlinks the 'moor' CLI command into a user-facing bin dir
...
echo ""
echo -e "${CYAN}⚕ Moor Agent Setup${NC}"
echo ""
...
if is_termux; then
    ...
    if [ -n "$UV_CMD" ]; then
        ...
        _uv_log="$(mktemp 2>/dev/null || echo "/tmp/moor-uv-install.$$.log")"
...
if is_termux; then
    # Moor currently declares requires-python >=3.11,<3.14. Termux can expose
    ...
    if [ -z "${PYTHON_PATH:-}" ]; then
        if command -v python >/dev/null 2>&1; then
            ...
            echo -e "${RED}✗${NC} Termux Python $PYTHON_FOUND_VERSION is not supported; Moor requires Python >=3.11,<3.14"
...
# ============================================================================
# PATH setup — symlink moor into a user-facing bin dir
# ============================================================================

echo -e "${CYAN}→${NC} Setting up moor command..."

MOOR_BIN="$SCRIPT_DIR/venv/bin/moor"
COMMAND_LINK_DIR="$(get_command_link_dir)"
COMMAND_LINK_DISPLAY_DIR="$(get_command_link_display_dir)"
mkdir -p "$COMMAND_LINK_DIR"
ln -sf "$MOOR_BIN" "$COMMAND_LINK_DIR/moor"
echo -e "${GREEN}✓${NC} Symlinked moor → $COMMAND_LINK_DISPLAY_DIR/moor"
...
# ============================================================================
# Seed bundled skills into ~/.moor/skills/
# ============================================================================

MOOR_SKILLS_DIR="${MOOR_HOME:-$HOME/.moor}/skills"
...
if "$SCRIPT_DIR/venv/bin/python" "$SCRIPT_DIR/tools/skills_sync.py" 2>/dev/null; then
    ...
    if [ -d "$SCRIPT_DIR/skills" ]; then
        cp -rn "$SCRIPT_DIR/skills/"* "$MOOR_SKILLS_DIR/" 2>/dev/null || true
...
if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
    ...
    "$SCRIPT_DIR/venv/bin/python" -m moor_cli.main setup
fi
