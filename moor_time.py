...
``MOOR_TIMEZONE`` env var, then ``timezone`` in ``~/.moor/config.yaml``, else server-local
...
# MOOR_HOME, so one unkeyed global would leak the first profile's timezone into later
...
def _timezone_cache_identity() -> Tuple[str, str]:
    tz_env = os.getenv("MOOR_TIMEZONE", "").strip()
