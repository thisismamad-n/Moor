...
Each probe runs in a FRESH subprocess with an isolated MOOR_HOME so nothing is cached across reps.
...
ENV = {**os.environ, "MOOR_HOME": HOME, "PYTHONPATH": TREE, "PYTHONDONTWRITEBYTECODE": "0", "OPENROUTER_API_KEY": "sk-bench-placeholder",
       "MOOR_SKIP_UPDATE_CHECK": "1", "NO_COLOR": "1", "TERM": "dumb", "COLUMNS": "120"}
for k in list(ENV):
    if k.startswith(("MOOR_SESSION", "MOOR_PROFILE")): ENV.pop(k)
...
IMPORT_TARGETS = ["run_agent", "cli", "moor_cli.main", "gateway.run", "tools.registry", "moor_state", "tui_gateway.server", "moor_cli.web_server", "model_tools", "agent.prompt_builder"]
probe = r'''
...
tree=os.getcwd(); foreign=[m for m,mod in list(sys.modules.items()) if getattr(mod,"__file__",None) and "moor-agent" in mod.__file__ and not mod.__file__.startswith(tree) and "site-packages" not in mod.__file__]
...
# 2. CLI end-to-end startup: `moor --version`, `moor --help`, `moor doctor --help`, `moor config get model` (no network)
CLI = {"version": ["moor_cli/main.py", "--version"], "help": ["moor_cli/main.py", "--help"], "config_get": ["moor_cli/main.py", "config", "get", "model"], "tools_list": ["moor_cli/main.py", "tools", "--help"], "skills_help": ["moor_cli/main.py", "skills", "--help"]}
cli = {}
...
try:
    ...
    T("resolve_toolset_moor_default_ms", lambda: resolve_toolset("moor-default"), 200)
...
try:
    import moor_state, tempfile, uuid
    from pathlib import Path
    db=moor_state.SessionDB(Path(tempfile.mkdtemp())/"s.db") if hasattr(moor_state,"SessionDB") else None
    if db:
        sid=str(uuid.uuid4())
