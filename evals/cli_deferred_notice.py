...
def run_case(root, output, name, behind, early=False, cancel=False):
    with tempfile.TemporaryDirectory(prefix="moor_test_notice_") as home:
        hh = Path(home) / ".moor"
        hh.mkdir()
        ...
        from moor_cli.banner import VERSION
        ...
        bootstrap = ("import moor_cli.main as m; import moor_cli.banner as b; "
        ...
        try:
            ...
            result = {"case": name, "ready": ready, "exited": exited,
                      ...
                      "loaded_worktree": str(root / "moor_cli/banner.py") in text,
