...
sys.path.insert(0,sys.argv[1] if len(sys.argv)>1 else os.getcwd());os.environ['MOOR_HOME']=tempfile.mkdtemp(prefix='cache-boundary-')  # usage: <repo_root>
...
cc=ContextCompressor(model=model,provider='moor',base_url=url,api_mode='anthropic_messages',config_context_length=1000000,threshold_tokens_cap=200000)
...
a=types.SimpleNamespace(api_mode='anthropic_messages',provider='moor',model=model,base_url=url,tools=[],_usage_anchor=None)
