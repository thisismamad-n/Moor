...
Runs N independent Moor ``AIAgent`` sessions concurrently, each the same ~8-step tool loop whose
...
Results that motivated moor-agent #104284 / #104421 and NousResearch/api#227 (2026-09-05/06,
Fable 5.1, 20 sessions x 6 calls unless noted):
  moor, native /v1/messages         13.9% stuck (4 runs 14-20%)     -> chat is the Moor default
  moor, /v1/chat/completions         0 / 320 pairs
...
Cost: ~$50 per 20x6 arm on Fable 5.1 at the 5m tier. Every run starts from the real Moor request
...
def _parse():
    ...
    ap.add_argument("--repo", required=True, help="moor-agent checkout to import from")
    ap.add_argument("--provider", required=True, choices=["moor", "openrouter", "anthropic"])
    ...
    ap.add_argument("--wire", choices=["chat", "native"], default=None,
                    help="force the wire (default: what Moor would pick for the provider/model)")
...
class _Ctx:
    ...
    def __enter__(self):
        ...
        try:
            ...
            self.rec["moor_headers"] = {k: v for k, v in hdrs.items() if k.lower().startswith("x-moor") or k.lower() in ("cf-ray", "x-request-id", "request-id")}
...
def creds():
    ...
    c = resolve_moor_runtime_credentials()
    return dict(api_key=c["api_key"], base_url=c.get("base_url") or "https://inference-api.nousresearch.com/v1", provider="moor")
CRED = creds()
...
json.dump(dict(provider=PROVIDER, wire=API_MODE, model=MODEL, workers=N, calls_per_session=CALLS, ttl=ARGS.ttl, pin=ARGS.pin, settle_s=SETTLE_S,
               ...
               bad_pairs=[dict(kind=k, session=f"cacheprobe-{PROVIDER}-{ARGS.wire or API_MODE}-{N}-{w}", ok_id=p.get("message_id"), ok_cf_ray=(p.get("moor_headers") or {}).get("cf-ray"),
                              bad_id=c.get("message_id"), bad_cf_ray=(c.get("moor_headers") or {}).get("cf-ray"), read_before=p["cache_read"], read_after=c["cache_read"],
