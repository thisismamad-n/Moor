...
message prefix between call N and N+1. If Moor strips prior-turn thinking, call N+1's messages[:k]
...
#   python cache_prefix_wire.py <repo_root> <A|B> [--moor-home DIR]   (default MOOR_HOME: the real one, for credentials)
sys.path.insert(0, sys.argv[1])
os.environ.setdefault("MOOR_HOME", os.path.expanduser("~/.moor"))
if "--moor-home" in sys.argv:
    os.environ["MOOR_HOME"] = sys.argv[sys.argv.index("--moor-home") + 1]
...
rt = resolve_runtime_provider(requested="moor", target_model="anthropic/claude-fable-5.1")
sid = f"f0wire_{arm}_{int(time.time())}"
ag = AIAgent(model="anthropic/claude-fable-5.1", provider="moor", base_url=rt.get("base_url"), api_key=rt.get("api_key"),
...
time.sleep(1)
log = subprocess.run(f"grep -h '\\[{sid}\\]' ~/.moor/logs/agent.log | grep 'API call #'", shell=True, capture_output=True).stdout.decode("utf-8", "replace")
