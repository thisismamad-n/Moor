...
real result. Uses glm-5.3 via Moor for cost. Deadline shortened via config to keep the run short."""
...
# Temp MOOR_HOME with the real auth + a config that shortens the generic sequential deadline to 40 s, so the
...
rt = resolve_runtime_provider(requested="moor", target_model=MODEL)
sid = f"dl_{arm}_{int(time.time())}"
ag = AIAgent(model=MODEL, provider="moor", base_url=rt.get("base_url"), api_key=rt.get("api_key"), api_mode=rt.get("api_mode"),
