...
env = {k: v for k, v in os.environ.items() if not (k.startswith('MOOR_') or k.endswith(('_API_KEY', '_TOKEN')))}
env.update(HOME=str(out / 'os-home'), MOOR_HOME=str(home), MOOR_DASHBOARD_SESSION_TOKEN='hooks-fixture-token', MOOR_IGNORE_RULES='1', PYTHONPATH=str(a.repo))
cmd = [sys.executable, '-m', 'moor_cli.main', 'serve', '--host', '127.0.0.1', '--port', str(a.port), '--skip-build']
