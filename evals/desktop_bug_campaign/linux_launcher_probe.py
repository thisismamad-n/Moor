...
def main():
    ...
    env = {"PATH": "/usr/bin:/bin", "HOME": str(root / "home"),
           "MOOR_HOME": str(root / "home/.moor"),
           "XDG_DATA_HOME": str(root / "xdg"), "LANG": "C.UTF-8",
           "MOOR_NONINTERACTIVE": "1"}
    Path(env["MOOR_HOME"]).mkdir(parents=True)
    venv_dir = root / "venv"
    ...
    rows["venv_import"] = run([str(python), "-I", "-c", "import ruamel.yaml,moor_cli.main; print(ruamel.yaml.__version__)"], env, "/")
    rows["base_import_negative"] = run([str(python.resolve()), "-I", "-c", "import ruamel.yaml,moor_cli.main"], env, "/")
    ...
    custom = original.replace(b"Name=Moor\n", b"Name=Moor custom\n").replace(b"Terminal=false", b"Terminal=true")
    entry.write_bytes(custom)
    config = Path(env["MOOR_HOME"]) / "config.yaml"
