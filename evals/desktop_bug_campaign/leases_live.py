...
''')
    ...
    (home / 'user').mkdir()
    env.update(MOOR_HOME=str(home), MOOR_DASHBOARD_SESSION_TOKEN='lease-probe',
    ...
    def control(port, action, sid):
        req = urllib.request.Request(f'http://127.0.0.1:{port}/lease-probe/{action}/{sid}', data=b'', method='POST', headers={'X-moor-session-Token': 'lease-probe'})
