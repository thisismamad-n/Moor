...
The run gets a throwaway MOOR_HOME so no local config leaks in, and the
...
# Moor-subscription path on boxes without an OpenRouter key. Credentials are
...
if _or_key:
    ...
    # Resolved by the orchestrator BEFORE MOOR_HOME is redirected to the
    # throwaway home (auth state lives in the real profile). Never printed.
    _tok = os.environ.get("BUBENCH_MOOR_TOKEN", "").strip()
    if not _tok:
        raise SystemExit("no OPENROUTER_API_KEY and no Moor auth available")
    _agent_auth = dict(
        base_url=os.environ.get("BUBENCH_MOOR_BASE_URL", "https://inference-api.nousresearch.com/v1"),
        api_key=_tok,
        provider="moor",
    )
