...
test.beforeAll(async () => {
  ...
  env = buildAppEnv(sandbox, { HOME: sandbox.root, MOOR_DESKTOP_PYTHON: python,
    ...
    PYTHONPATH: repo, MOOR_SINGLE_QUERY_LINGER_SECONDS: '1' })
...
test('ordinary cron pins an unowned named target under a custom root', async () => {
  ...
  const page = fixture.page
  const composer = await openBot('Moor')
  ...
  const discovery = 'from pathlib import Path; import json,os; from tools.bot_live_delivery import find_canonical_owner; h=Path(os.environ["MOOR_HOME"]); print(json.dumps({"default":find_canonical_owner(h),"alpha":find_canonical_owner(h/"profiles"/"alpha")}))'
  ...
  try {
    ...
    expect(launched).toContain(path.join(fixture.sandbox.moorHome, 'profiles', 'alpha'))
    expect(held.default.lease_id).toBe(before.default.lease_id)
    expect(fs.existsSync(path.join(fixture.sandbox.moorHome, 'cron', 'bot_chat_pending'))).toBe(false)
    ...
    const rows = probe('import sqlite3,json,os; from pathlib import Path; h=Path(os.environ["MOOR_HOME"]); print(json.dumps({n:sqlite3.connect(h/"state.db" if n=="default" else h/"profiles"/n/"state.db").execute("select session_id,role,content from messages").fetchall() for n in ["default","alpha"]}))')
    ...
    const missing = probe('import json,os; from pathlib import Path; from cron.scheduler_delivery import _deliver_to_bot_chat; h=Path(os.environ["MOOR_HOME"]); p=h/"profiles"/"beta"; p.rename(h/"profiles"/"beta-removed"); result=_deliver_to_bot_chat({"id":"removed","execution_id":"missing"},"MUST_NOT_RUN","beta"); print(json.dumps({"result":result,"recreated":p.exists(),"wrong_root":(Path.home()/".moor").exists()}))')
