...
_profile_runtime_scope(profile_home)           (gateway/run.py)
   ├── set_moor_home_override(home)          config / state.db / skills /
...
- `get_secret(name)` resolves: global allowlist → active scope → fallback.
  ...
  - multiplexing **on**, no scope installed: **raises `UnscopedSecretError`**
    ...
    value.
- A small allowlist (`MOOR_HOME`, `MOOR_PROFILE`, proxy settings,
...
reload helper: `moor_cli.env_loader.load_moor_dotenv` skips the
...
## The MOOR_HOME override

`moor_constants.py` holds a context-local override consulted by
`get_moor_home()` before the `MOOR_HOME` env var. Everything that resolves
...
automatically. `get_process_moor_home()` exists for the few machine-level
assets that must not follow the override. `moor_home_key()` gives
...
`profiles_to_serve(multiplex, profile_allowlist)` in `moor_cli/profiles.py`
...
DB handles are resolved at call time through the active MOOR_HOME override —
...
MOOR_HOME override. Asset writes are atomic, type- and size-capped.
...
| Built-in tool registry | Built-ins are process-global; plugin-registered tools are overlaid per profile via `moor_home_key()`. |
