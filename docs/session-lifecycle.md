...
The canonical transcript store is SQLite via `SessionDB` (from `moor_state`). The
...
Counts consecutive restarts via a JSON file (`{MOOR_HOME}/restart_counts.json`). If a
