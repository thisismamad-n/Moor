...
`moor config set gateway.profile_routes ...` writes).
...
| `profile` | yes | Target profile name (must exist under `~/.moor/profiles/<name>`). |
...
activates the per-profile runtime scope (per-profile `MOOR_HOME`, secret scope, and
