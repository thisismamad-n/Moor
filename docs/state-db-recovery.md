...
`moor doctor` reports the explicit repair command.
...
repaired or restored file. Do not run `moor doctor --fix` while the gateway
is still up. Next steps:

```bash
moor gateway stop
MOOR_HOME="$HOME/.moor" moor sessions recover --source "$HOME/.moor/state.db" --inspect-only
# if recoverable:
MOOR_HOME="$HOME/.moor" moor sessions recover --source "$HOME/.moor/state.db" --output "$HOME/recovered-state.db"
```
...
```bash
moor gateway stop
MOOR_HOME="$HOME/.moor" moor sessions repair --check-only
MOOR_HOME="$HOME/.moor" moor sessions repair
```
