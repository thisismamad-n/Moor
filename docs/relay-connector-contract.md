...
- **Registration.** The gateway registers a **wake URL** at enroll/provision —
  ...
  a dashboard host). Self-hosted: `moor gateway enroll --wake-url <url>` (or
...
**per-gateway secret** (`moor gateway enroll` → connector `/relay/enroll`, or
...
(`Moor inc./gateway-gateway`); this section is the gateway-facing summary.
...
driven by the managed Portal / `moor` CLI.
