...
They do not replace Moor' planner, model providers, memory, tool registry,
...
Moor also has a first-party NeMo Relay shared-metrics path. It uses these
...
```text
telemetry_schema_version = "moor.observer.v1"
```

Hook callbacks are fail-open. Moor catches callback exceptions, logs a
...
payload construction is gated behind `has_hook(...)`, so Moor only builds
...
The native NeMo Relay SDK integration maps Moor session, turn, LLM, and tool
