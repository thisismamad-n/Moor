...
into Moor and does not require a Moor observability plugin. Moor remains
...
> [!WARNING]
> This removes the Moor `observability/nemo_relay` plugin. Existing users
...
> `plugins.toml` selected with `MOOR_NEMO_RELAY_PLUGINS_TOML`. The legacy
> `MOOR_NEMO_RELAY_ATOF_*` and `MOOR_NEMO_RELAY_ATIF_*` variables no
> longer activate exporters. Without the new variable, Moor does not run
> Relay plugin discovery, configuration layering, middleware, or exporters.

Moor requires NeMo Relay 0.8.3 or later within the 0.8 release line. That
line provides the provider-codec and canonical tool-result contracts Moor
uses for managed provider and tool calls.

## Runtime Dependency and Data Boundary

Moor installs the platform-specific `nemo-relay` native wheel from the
...
through that native module in the Moor process so configured interceptors can
...
Collection remains off unless Moor policy enables it:
...
opt-in. Set `MOOR_NEMO_RELAY_PLUGINS_TOML` to a selected `plugins.toml` to
activate configured middleware, exporters, or dynamic plugins. When the
variable is unset, Moor does not invoke Relay's plugin initializer, so Relay
...
selected file cannot be loaded, Moor reports the error and does not invoke
...
retain the same `session_id` and add `moor.session.segment` plus
`moor.session.segment_reason` (`compaction` or `max_turns`).

## Process-Wide Plugin Policy and Profile Isolation

Relay plugin configuration is a process-level deployment choice, not a Moor
profile setting. The first hosted profile triggers lazy initialization, and
every additional profile hosted by that Moor process shares the resulting
static middleware, dynamic plugins, subscribers, exporters, and guardrail
policy. After initialization succeeds, Moor logs:
...
profile's Relay scope stack. Native dynamic plugins are loaded into the Moor
process and share the same policy boundary.

Run profiles in separate Moor processes when they require different trust
...
```text
Moor turn, API, tool, and approval hooks
  -> Relay session, task, LLM, tool, and mark lifecycle
  -> Moor shared-metrics subscriber
  -> SQLite counters
  -> immutable JSON delta package
```

Moor sends an empty `LLMRequest` into the metrics-owned lifecycle. This does
...
provider route that Moor used for the logical call, such as
...
resources. Concurrent Moor processes share the SQLite latch, so simultaneous
...
provider attempts for the same Moor API request ID; they do not inflate the
logical model-call count. Tool calls are deduplicated by their Moor tool-call
...
cancellations. Active task ownership follows the task ID if Moor rotates its
...
are written with atomic replacement. Each package records the Moor version,
...
Run a real Moor CLI turn against the deterministic local model server:
...
`$MOOR_HOME/telemetry/shared_metrics/outbox/` to the Moor telemetry ingest
...
telemetry:
  shared_metrics:
    ...
    send: false      # NEW: transmit to the Moor telemetry service
```
...
Removing `$MOOR_HOME/telemetry/shared_metrics` still resets local identity,
