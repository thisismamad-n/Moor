...
Homes are switched two ways in a running process: the `MOOR_HOME`
environment variable (single-profile CLI/gateway processes), and the
context-local `set_moor_home_override()` (`moor_constants.py`), which
...
`get_moor_home() / "plugins"`, and context-engine plugins (e.g.
`moor-lcm`) capture profile-scoped state — such as the LCM database
path — at registration time. A single-slot cache meant:

1. Switching homes via `set_moor_home_override()` was invisible to a
   naive "did `MOOR_HOME` change" check, so the singleton silently kept
...
2. Even when a fresh `PluginManager` *was* created for a new home, plugin
   modules are imported into `sys.modules` as `moor_plugins.<slug>` by
   ...
   `moor_plugins.<slug>.<submodule>`, and Python's import machinery
...
- Replace the single-slot singleton with a cache keyed on the *resolved*
  ...
  `get_plugin_manager()` resolves the current home via `get_moor_home()`
  (which itself already consults `get_moor_home_override()` before
...
- Per-profile LCM instances (and any other context-engine plugin) use
  ...
  through `MOOR_HOME` or `set_moor_home_override()`.
...
- Regression coverage exercises the real production path
  (`set_moor_home_override()`) rather than only the env-var path, and
