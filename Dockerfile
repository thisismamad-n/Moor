...
# Do not write .pyc files at runtime: /opt/moor is immutable in the
...
# /opt/moor/tools — outside the /opt/data volume mount, so the
...
# composes tool env.
ENV PLAYWRIGHT_BROWSERS_PATH=/opt/moor/tools

# Install system dependencies in one layer, clear APT cache.
# tini was previously PID 1 to reap orphaned zombie processes (MCP stdio
# subprocesses, git, bun, etc.) that would otherwise accumulate when moor
# ran as PID 1. See #15012. Phase 2 of the s6-overlay supervision plan
# replaces tini with s6-overlay's /init (PID 1 = s6-svscan), which reaps
# zombies non-blockingly on SIGCHLD and additionally supervises the main
# moor process, the dashboard, and per-profile gateways.
...
# names (see .moor/plans/termux-removal-commit-spec.md).
RUN apt-get -o Acquire::Retries=3 update && \
    apt-get -o Acquire::Retries=3 install -y --no-install-recommends \
...
#   docker build --build-arg MOOR_BOT_DESKTOP=1 .
ARG MOOR_BOT_DESKTOP=0
RUN if [ "$MOOR_BOT_DESKTOP" = "1" ]; then \
...
db.execute(\"INSERT INTO docs VALUES ('moor')\"); \
...
# ---------- s6-overlay install ----------
# s6-overlay provides supervision for the main moor process, the dashboard,
...
# Non-root user for runtime; UID can be overridden via MOOR_UID at runtime
RUN useradd -u 10000 -m -d /opt/data moor
...
# /opt/moor, outside the /opt/data volume so it survives the overlay.
...
# baked to /etc/moor/agent-browser-executable-path for stage2-hook.sh:
...
ENV MOOR_RUNTIME_DIR=/opt/moor/tools
COPY pm/ pm/
# pm's lazy imports resolve get_default_moor_root()/project_venv_dir()
# from moor_constants (stdlib-only) at install time — a sealed-stage
# `python3 -m pm.cli install` fails with "No module named
# 'moor_constants'" without it on the path. Copy the module next to pm.
COPY moor_constants.py moor_constants.py
# PM imports the shared stdlib locking owner before deps exist.
COPY moor_cli/__init__.py moor_cli/runtime_state.py moor_cli/
...
RUN set -eu; \
    ...
    "$browser_bin" --version; \
    mkdir -p /etc/moor; \
    printf '%s' "$browser_bin" > /etc/moor/agent-browser-executable-path
...
RUN python3 -c 'from pathlib import Path; from pm import stage_manager_runtime; from scripts.bundles.payload import seal_pm_runtime; root = Path("/opt/moor"); python = Path("/usr/local/bin/python3").resolve(); stage_manager_runtime(python=python, destination=root / "pm-runtime", project=root / "pm"); seal_pm_runtime(root, python)'

# JS build helpers use the prepared interpreter without an installer parent.
ENV MOOR_PYTHON=/usr/local/bin/python3
...
RUN touch ./README.md
RUN python3 -m pm.build_env --source /opt/moor --python /usr/local/bin/python3 \
    --out /opt/moor/.venv --no-install-project --sealed \
...
COPY assets/ assets/
RUN /opt/moor/.venv/bin/python -I scripts/generate_icons.py --source /opt/moor --out /tmp/moor-icons
...
COPY ui-tui/packages/moor-ink/ ui-tui/packages/moor-ink/
COPY apps/shared/ apps/shared/
COPY scripts/build/node-deps.mjs scripts/build/node-deps.mjs
ENV npm_config_install_links=false
RUN node scripts/build/node-deps.mjs --source /opt/moor --workspace ui-tui --workspace web
...
COPY assets/ assets/
COPY --from=icons /tmp/moor-icons /tmp/moor-icons
RUN node scripts/build/tui.mjs --source /opt/moor --out /opt/products/tui && \
    node scripts/build/web.mjs --source /opt/moor --icons /tmp/moor-icons --out /opt/products/web

FROM python_deps AS runtime
# Standalone TypeScript linting is a runtime feature; Vite/esbuild are not.
COPY --from=frontend_build /opt/moor/node_modules/typescript /opt/moor/node_modules/typescript
RUN mkdir -p /opt/moor/node_modules/.bin && \
    ln -s ../typescript/bin/tsc /opt/moor/node_modules/.bin/tsc
...
COPY --from=frontend_build /opt/products/tui /opt/moor/ui-tui
COPY --from=frontend_build /opt/products/web /opt/moor/moor_cli/web_dist
# ---------- Bot Screen X socket directory ----------
# Xvnc would create this itself (/tmp is 1777); pre-creating it keeps ownership
# deterministic when MOOR_UID is remapped between boots.
...
RUN mkdir -p /tmp/moor-runtime && chmod 0700 /tmp/moor-runtime
...
# gives the non-root moor user read + traverse but no write; root retains
# write so the build steps below don't need chmod u+w dances.
COPY --link --chmod=a+rX,go-w . .

# The shared assembler binds the prepared environment and frontend products.
RUN /opt/moor/.venv/bin/python -m docker.build_agent

# Wire the exec shim and install-method stamp.  Files under /opt/moor are
# already root-owned (COPY, dep assembly, npm install all run as root) and
# read-only for the moor user (go-w from the --chmod above).
...
# host install's marker and wrongly block its ``moor update``. A code-scoped
# stamp is read first by detect_install_method() and is immune to the share.
# Start as root so the s6-overlay stage2 hook can usermod/groupmod and chown
# the data volume. Each supervised service then drops to the moor user via
...
# full-provenance /opt/moor/install-stamp.json next to the code.
# .dockerignore excludes .git, so the stamp is the only commit channel the
# image carries: moor_cli/version_info.py reads it at runtime (stamp
# first, live git second, unknown third), and both `moor dump` and
...
# checkout) and /opt/data (the mutable MOOR_HOME volume).  Its `revision`
# is read from the install stamp; the fallback stamp's all-zero commit maps
# to null.
RUN set -eu; \
    if [ ! -f /opt/moor/install-stamp.json ]; then \
    ...
    python3 -c 'import json; from pathlib import Path; path = Path("/opt/moor/install-stamp.json"); stamp = json.loads(path.read_text()); stamp["pmRuntime"] = "/opt/moor/pm-runtime"; path.write_text(json.dumps(stamp) + "\n")'; \
    mkdir -p /etc/moor; \
    python3 -c 'import json, pathlib, tomllib; project = tomllib.loads(pathlib.Path("/opt/moor/pyproject.toml").read_text(encoding="utf-8"))["project"]; stamp = json.loads(pathlib.Path("/opt/moor/install-stamp.json").read_text(encoding="utf-8")); commit = stamp.get("commit"); revision = commit if commit and set(commit) != {"0"} else None; marker = pathlib.Path("/etc/moor/image-provenance.json"); marker.write_text(json.dumps({"schema": 1, "deployment_kind": "image", "manager": "docker", "image": "nousresearch/hermes-agent", "version": project["version"], "revision": revision}, sort_keys=True, separators=(",", ":")) + "\n", encoding="utf-8"); marker.chmod(0o444)'
...
# `exec moor`. Wired in as cont-init.d/01- so it
# runs before user services start.
#
# 02-reconcile-profiles re-creates per-profile gateway s6 service
# slots from $MOOR_HOME/profiles/<name>/ after a container restart
# (the /run/service/ scandir is tmpfs and wiped on restart). Phase 4.
RUN mkdir -p /etc/cont-init.d && \
    printf '#!/command/with-contenv sh\nexec /opt/moor/docker/stage2-hook.sh\n' \
        > /etc/cont-init.d/01-moor-setup && \
    chmod +x /etc/cont-init.d/01-moor-setup
COPY --chmod=0755 docker/cont-init.d/015-supervise-perms /etc/cont-init.d/015-supervise-perms
COPY --chmod=0755 docker/cont-init.d/02-reconcile-profiles /etc/cont-init.d/02-reconcile-profiles

# ---------- Runtime ----------
ENV MOOR_WEB_DIST=/opt/moor/moor_cli/web_dist
# Point the TUI launcher at the prebuilt bundle baked at build time (Layer 8:
# `ui-tui && npm run build`). This makes _make_tui_argv take the prebuilt-bundle
# fast path (`node --expose-gc /opt/moor/ui-tui/dist/entry.js`) and skip the
...
ENV MOOR_TUI_DIR=/opt/moor/ui-tui
ENV MOOR_HOME=/opt/data
ENV MOOR_WRITE_SAFE_ROOT=/opt/data
# Opt-in backend SDKs install on first use into PM dependency generations under
# /opt/data/installs (the sealed /opt/moor/.venv is never written); stage2
...
ENV XDG_RUNTIME_DIR=/tmp/moor-runtime

# `docker exec` privilege-drop shim. When operators run
# `docker exec <c> moor ...` they default to root, and any file the
# command writes under $MOOR_HOME (auth.json, .env, config.yaml) ends
# up root-owned and unreadable to the supervised gateway (UID 10000).
# The shim lives at /opt/moor/bin/moor, sits earliest on PATH, and
# transparently re-exec's the real venv binary via `s6-setuidgid moor`
# when invoked as root. Non-root callers (supervised processes,
# `--user moor`, etc.) hit the short-circuit path with no overhead.
# Recursion is impossible because the shim exec's the venv binary by
# absolute path (/opt/moor/.venv/bin/moor). See the shim source for
# the opt-out env var (MOOR_DOCKER_EXEC_AS_ROOT=1).
COPY --chmod=0755 docker/moor-exec-shim.sh /opt/moor/bin/moor
COPY --chmod=0755 docker/entrypoint-dispatch.sh /opt/moor/docker/entrypoint-dispatch.sh

# Pre-s6 entrypoint.sh did `source .venv/bin/activate` which exported
# the venv bin onto PATH; Architecture B's main-wrapper.sh does the
# same for the container's main process, but `docker exec` and our
# cont-init.d scripts don't pass through the wrapper. Expose the venv
# bin globally so `docker exec <container> moor ...` and any
# subprocess that doesn't activate the venv first still find moor.
#
# /opt/moor/bin is prepended ahead of the venv so the privilege-drop
# shim wins PATH resolution. The shim's last act is to exec the venv
# binary by absolute path, so this PATH ordering is transparent to
# every other consumer.
ENV PATH="/opt/moor/bin:/opt/moor/.venv/bin:/opt/data/.local/bin:${PATH}"
...
RUN mkdir -p /opt/data && chmod 0644 /opt/moor/tools/facts.json && \
    rm -f /opt/moor/.venv/.lock /opt/moor/pm-runtime/.lock
VOLUME [ "/opt/data" ]
...
#
# main-wrapper.sh handles arg routing (bare-exec vs. moor
# subcommand vs. no-args), drops to the moor user via s6-setuidgid,
...
ENTRYPOINT [ "/opt/moor/docker/entrypoint-dispatch.sh" ]
CMD [ ]
