...
ENV PLAYWRIGHT_BROWSERS_PATH=/opt/moor/.playwright
...
#
# ui-tui/packages/moor-ink/ is copied IN FULL (not just its manifests)
...
# `file:` workspace dependency (same pattern as moor-ink above).
...
# lock on the @moor/ink entry, tripped the TUI launcher's
...
# for the same reason: it lazy-installs into /opt/moor/.venv at first
...
# ---------- Permissions ----------
# Link moor-agent itself (editable). Deps are already installed in the
...
#
# Fix: write the commit SHA passed via the MOOR_GIT_SHA build-arg to
# /opt/moor/.moor_build_sha at build time, and have
# moor_cli/build_info.py read it at runtime.  Both `moor dump` and
...
# The published image seals /opt/moor (root-owned, read-only) so a runtime
...
# is seeded + chowned to the moor user by docker/stage2-hook.sh and lives
...
ENV MOOR_LAZY_INSTALL_TARGET=/opt/data/lazy-packages
