...
def _run_async(coro):
    ...
    if loop and loop.is_running():
        ...
        # Carry profile + approval/sudo context so get_moor_home() resolves correctly.
...
def _tool_defs_cache_key(
    ...
    return (
        ...
        bool(os.environ.get("MOOR_KANBAN_TASK")), bool(skip_tool_search_assembly),
...
def _apply_toolset_selection(tools: set, names: List[str], quiet_mode: bool, *, disable: bool) -> None:
    ...
    for name in names:
        if validate_toolset(name):
            ...
            if disable and (name.startswith("moor-") or (get_toolset(name) or {}).get("posture")):
                ...
                if not quiet_mode and name.startswith("moor-") and name not in _WARNED_DISABLED_BUNDLES:
...
def _select_tool_names(enabled_toolsets: Optional[List[str]], disabled_toolsets: Optional[List[str]], quiet_mode: bool) -> set:
    ...
    if enabled_toolsets is not None:
        ...
        if (os.environ.get("MOOR_KANBAN_TASK") and not _is_delegated_child_context()
    ...
    # toolset is stripped even when a composite (moor-cli) re-enables it.
    # This ensures that even if a composite toolset (like moor-cli) is enabled, any tools belonging to a
