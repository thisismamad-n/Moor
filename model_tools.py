...
def _is_dispatcher_owned_worker() -> bool:
    """False when MOOR_KANBAN_* is present but this execution does not own it
...
def _run_async(coro):
    ...
    if loop and loop.is_running():
        ...
        # async tools resolve get_moor_home() under the active profile.
...
#   - cli.py, moor_cli/*      -> inline on startup (no event loop)
...
def _compute_tool_definitions(
    ...
    if enabled_toolsets is not None:
        ...
        if (
            ...
            # Dispatcher-spawned workers are scoped by MOOR_KANBAN_TASK and
    ...
    # This ensures that even if a composite toolset (like moor-cli)
    ...
    if disabled_toolsets:
        for toolset_name in disabled_toolsets:
            if validate_toolset(toolset_name):
                ...
                if toolset_name.startswith("moor-") or (get_toolset(toolset_name) or {}).get("posture"):
                    # Platform bundles (moor-*) include _MOOR_CORE_TOOLS, and
                    ...
                    if (not quiet_mode and toolset_name.startswith("moor-")
    ...
    # window). Core Moor tools (toolsets._MOOR_CORE_TOOLS) are NEVER
...
def _resolve_active_context_length() -> int:
    ...
    """
    try:
        from moor_cli.config import load_config as _load
        cfg = _load() or {}
        ...
        # bypasses the persistent cache for some providers (Moor portal,
...
def _normalize_json_strings_for_schema(value: Any, schema: Any) -> Any:
    ...
    Ported from cline/cline#11803, adapted to moor-agent's coercion layer.
...
def _emit_post_tool_call_hook(
    *,
    ...
    if _post_tool_call_hook_suppressed.get():
        return
    try:
        from moor_cli.lifecycle import has_hook, invoke_hook
