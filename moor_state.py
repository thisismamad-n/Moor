#!/usr/bin/env python3
"""SQLite state store for Moor Agent: session metadata, message history, model
...
from moor_state_common import escape_like as _escape_like, stat_db_file_identity as _stat_db_file_identity
...
from moor_state_wal import _WAL_INCOMPAT_MARKERS, apply_database_pragmas, apply_wal_with_fallback
...
DEFAULT_DB_PATH = _IMPORT_DEFAULT_DB_PATH = get_moor_home() / "state.db"
...
def _default_db_path() -> Path:
    ...
    ``get_moor_home()`` is resolved fresh (a runtime MOOR_HOME redirect works regardless of import)."""
    return DEFAULT_DB_PATH if DEFAULT_DB_PATH != _IMPORT_DEFAULT_DB_PATH else get_moor_home() / "state.db"


# Live-DB guard knobs live HERE (not in moor_state_guard): the hermetic conftest monkeypatches
# ``moor_state._STATE_DB_GUARD_BYPASS`` (``@pytest.mark.live_system_guard_bypass`` escape hatch)
# and ``_EXTRA_DENY_ROOTS`` (the pre-sandbox root, so custom-MOOR_HOME deployments are covered).
...
def _ensure_test_isolation(db_path: Path) -> None:
    ...
    ``MOOR_HOME`` together, which is precisely the state in which it writes to production (#82770).
    """
    ...
    for root in roots:
        if _is_production_state_db(resolved, root):
            raise RuntimeError(
                ...
                "MOOR_HOME. If this test genuinely needs the live database, mark it with "
...
def divert_session_transcript_jsonl(session_id: str, messages) -> "Optional[Path]":
    """Append pending messages to MOOR_HOME/sessions/<id>.jsonl (state.db was replaced under a
...
# connection per resolved path via moor_state_registry.acquire(); one-shots use SessionDB() + close().
...
# ── Process-wide shared SessionDB registry (#90837) ── lives in moor_state_registry.py (acquire /
# release / close_all / release_or_close). Long-lived in-process callers (gateway, tui_gateway, cron,
# in-process tools) share ONE writer connection per resolved path via moor_state_registry.acquire(); CLI
...
class SessionDB(
    ...
    # SQLite's deterministic busy handler convoys under many moor processes: keep its
    ...
    def __init__(self, db_path: Path = None, read_only: bool = False):
        ...
        # Opened via moor_state_registry.acquire(): close() releases a refcount instead.
        # Set True when this instance is opened via moor_state_registry.acquire(). Makes close() a no-op so the
    ...
    def _open_writer(self) -> None:
        ...
        try:
            ...
            if not repair_state_db_schema(self.db_path).get("repaired"):
                raise
            self._connect_and_init_with_lock_patience()
        # FTS optimization is OPT-IN (`moor db optimize`); no background worker races session lifecycle.
    ...
    def _handle_quarantine_if_zeroed(self, already_locked: bool = False) -> None:
        ...
        msg = (
            ...
            f"Restore from {self.db_path.parent / 'state-snapshots'} via `moor snapshot list` / "
    ...
    def _halt_db_corrupt(self, exc: BaseException) -> None:
        ...
        logger.error(
            ...
            "gateway and run `moor sessions recover --source %s --inspect-only`.", self.db_path, exc,
            self.db_path,
        )
    ...
    def _foreign_state_db_holders(self) -> List[Tuple[int, str]]:
        """Foreign processes holding this DB or its WAL sidecars (see moor_state_holders)."""
    ...
    def close(self):
        ...
        When this instance is shared (opened via ``moor_state_registry.acquire``), ``close()`` RELEASES one
        ...
        if self._shared_registry_owned:
            from moor_state_registry import release
            release(self)
            return
            ...
            if self._conn:
                if self._db_corrupt:  # quarantined: no checkpoint over a damaged image
                    logger.warning(
                        ...
                        "then run `moor sessions recover --source %s --inspect-only`.", self.db_path,
...
_PLUGIN_COMPAT_LAZY = {
    'AUTO_VACUUM_MIN_FREELIST_RATIO': ('moor_state_common', 'AUTO_VACUUM_MIN_FREELIST_RATIO'),
    'ActivityProvenance': ('agent.session_activity', 'ActivityProvenance'),
    'CompressionSessionBusyError': ('moor_state_errors', 'CompressionSessionBusyError'),
    'CompressionSessionClosedError': ('moor_state_errors', 'CompressionSessionClosedError'),
    'DEFERRED_INDEX_SQL': ('moor_state_common', 'DEFERRED_INDEX_SQL'),
    'FTS_CJK_STALE_KEY': ('moor_state_common', 'FTS_CJK_STALE_KEY'),
    'FTS_CJK_TABLE_SQL': ('moor_state_fts', 'FTS_CJK_TABLE_SQL'),
    'FTS_CJK_TRIGGER_SQL': ('moor_state_fts', 'FTS_CJK_TRIGGER_SQL'),
    'FTS_REBUILD_DEFERRAL_KEY': ('moor_state_common', 'FTS_REBUILD_DEFERRAL_KEY'),
    'FTS_SQL': ('moor_state_common', 'FTS_SQL'),
    'FTS_STALE_KEY': ('moor_state_common', 'FTS_STALE_KEY'),
    'FTS_STORAGE_VERSION': ('moor_state_common', 'FTS_STORAGE_VERSION'),
    'FTS_TRIGRAM_SQL': ('moor_state_common', 'FTS_TRIGRAM_SQL'),
    'LEGACY_FTS_SQL': ('moor_state_common', 'LEGACY_FTS_SQL'),
    'LEGACY_FTS_TRIGRAM_SQL': ('moor_state_common', 'LEGACY_FTS_TRIGRAM_SQL'),
    'MAX_FTS5_QUERY_CHARS': ('moor_state_common', 'MAX_FTS5_QUERY_CHARS'),
    'PERSISTENCE_ERROR_CAUSES': ('moor_state_errors', 'PERSISTENCE_ERROR_CAUSES'),
    'SCHEMA_SQL': ('moor_state_common', 'SCHEMA_SQL'),
    'SCHEMA_VERSION': ('moor_state_common', 'SCHEMA_VERSION'),
    'SESSION_STATUS_COMPLETE': ('moor_state_sessions', 'SESSION_STATUS_COMPLETE'),
    'SESSION_STATUS_EMPTY': ('moor_state_sessions', 'SESSION_STATUS_EMPTY'),
    'SESSION_STATUS_ERROR': ('moor_state_sessions', 'SESSION_STATUS_ERROR'),
    'SESSION_STATUS_INTERRUPTED': ('moor_state_sessions', 'SESSION_STATUS_INTERRUPTED'),
    ...
    'SessionTurnLeaseLostError': ('moor_state_errors', 'SessionTurnLeaseLostError'),
    'WalUnsupportedError': ('moor_state_wal', 'WalUnsupportedError'),
    'apply_durability_barriers': ('moor_state_repair', 'apply_durability_barriers'),
    'classify_session_status': ('moor_state_sessions', 'classify_session_status'),
    'collect_state_db_stats': ('moor_state_dbfile', 'collect_state_db_stats'),
    'count_db_holders': ('moor_state_dbfile', 'count_db_holders'),
    'describe_skill_invocation': ('agent.skill_commands', 'describe_skill_invocation'),
    'fts5_cjk_so_path': ('moor_state_fts', 'fts5_cjk_so_path'),
    'is_advisory_lock_contention': ('moor_state_common', 'is_advisory_lock_contention'),
    'is_automatic_end_reason': ('moor_state_common', 'is_automatic_end_reason'),
    'is_disk_full_error': ('moor_state_errors', 'is_disk_full_error'),
    'is_sqlite_wal_reset_vulnerable': ('moor_state_wal', 'is_sqlite_wal_reset_vulnerable'),
    'is_transient_sqlite_error': ('moor_state_errors', 'is_transient_sqlite_error'),
    'iter_deleted_sqlite_sidecar_holders': ('moor_state_dbfile', 'iter_deleted_sqlite_sidecar_holders'),
    'release_or_close': ('moor_state_registry', 'release_or_close'),
    'report_startup_progress': ('moor_startup_watchdog', 'report_startup_progress'),
    'resolve_journal_mode': ('moor_state_wal', 'resolve_journal_mode'),
    'resolve_synchromoor_level': ('moor_state_wal', 'resolve_synchromoor_level'),
    'sanitize_context': ('agent.memory_manager', 'sanitize_context'),
    'sqlite_source_id': ('moor_state_wal', 'sqlite_source_id'),
    'workspace_key': ('moor_state_sessions', 'workspace_key'),
}
