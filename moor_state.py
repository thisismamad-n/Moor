...
# transitive dependency via moor_state_common). run_agent keeps its own
# predating copy — moor_state cannot import run_agent (circular) — guarded
...
from moor_cli.sqlite_runtime import (
    is_sqlite_wal_reset_vulnerable as _is_sqlite_wal_reset_vulnerable,
)
from typing import Any, Callable, Dict, List, Optional, Set, Tuple, TypeVar

from moor_state_common import (  # noqa: F401  (re-exported for back-compat)
    _BRANCH_CHILD_SQL,
...
def _workspace_key_clause(key: str) -> Tuple[str, List[str]]:
    ...
    Used by ``moor -c``/``--resume`` to continue the most recent session in
...
DEFAULT_DB_PATH = get_moor_home() / "state.db"
...
def _default_db_path() -> Path:
    ...
    freezes the developer's real ``~/.moor`` even when a test fixture later
    redirects ``MOOR_HOME`` — importing this module during collection was
    ...
    2. Otherwise resolve ``get_moor_home()`` fresh so a runtime
       ``MOOR_HOME`` redirect takes effect regardless of import order.
    """
...
# ~/.moor/state.db accumulated pytest fixture rows — sessions with
...
# FTS write corruption").  The hermetic conftest redirects MOOR_HOME per
# test, but any escape (a session-scoped fixture running before the autouse
# fixture, a subprocess child launched without MOOR_HOME, a stale worktree
# without the re-pin, or a developer shell that exports MOOR_HOME to the
...
_STATE_DB_GUARD_BYPASS_ENV = "MOOR_STATE_DB_GUARD_BYPASS"

#: Additional production roots to refuse (beyond the platform default
#: ``~/.moor``).  The test conftest injects the pre-sandbox production
#: root here so custom-``MOOR_HOME`` deployments are covered too.
...
def _real_platform_state_root() -> Optional[Path]:
    ...
    Deliberately avoids ``Path.home()`` / ``moor_constants``: tests
    routinely monkeypatch ``Path.home`` to a tempdir, and ``moor_state``
    ...
    try:
        if sys.platform == "win32":
            ...
            root = (
                Path(base) / "moor"
                if base
                else Path(os.path.expanduser("~")) / "AppData" / "Local" / "moor"
            )
        else:
            root = Path(os.path.expanduser("~")) / ".moor"
        return root.resolve()
    except Exception:
        return None
...
#: redirects ``MOOR_HOME`` to the per-session tmp isolation root.  Its
...
#: child that received the patched ``MOOR_HOME`` also received the marker,
#: and a child that resolves a production DB while carrying it is, by
#: definition, an isolation escape (#82770).
_TEST_ISOLATION_MARKER_ENV = "MOOR_TEST_ISOLATION"
...
def _has_pytest_ancestor() -> bool:
    ...
    ``MOOR_HOME`` redirect: that child aims at the production DB *and*
...
def _in_test_context() -> bool:
    ...
    so a real ``moor`` invocation pays for at most one walk.
    """
...
def _is_production_state_db(resolved: Path, root: Path) -> bool:
    ...
    under ``~/.moor/moor-agent/...``) so hermetic tests using unusual
...
def _ensure_test_isolation(db_path: Path) -> None:
    ...
    pytest and for hermetic (tmp ``MOOR_HOME``) paths.
    ...
    a rebuilt environment loses ``PYTEST_*`` and ``MOOR_HOME`` together,
    ...
    for root in _production_state_roots():
        if _is_production_state_db(resolved, root):
            raise RuntimeError(
                ...
                "Tests must run against a temporary MOOR_HOME — pass an "
                "explicit tmp db_path or let the hermetic conftest redirect "
                "MOOR_HOME. If this test genuinely needs the live "
...
# moor_cli/kanban_db.py for ~30 call sites) would re-log the same
...
def _apply_wal_size_limit(conn: sqlite3.Connection) -> None:
    ...
    3.0 GB ``state.db``: ``moor sessions optimize`` (FTS merge + VACUUM)
    ...
    ``moor_cli/kanban_db.py`` already bounds its WAL growth with
...
def _enforce_macos_synchromoor_full(conn: sqlite3.Connection) -> None:
...
def apply_wal_with_fallback(
    ...
    *,
    ...
    Shared by :class:`SessionDB` and ``moor_cli.kanban_db.connect`` so
    ...
    if current_mode == "wal":
        ...
        _enforce_macos_synchromoor_full(conn)
        return "wal"
...
def _wal_reset_repair_hint() -> str:
    ...
    ``moor update`` can actually do for this install (#75153).
    """
    try:
...
def _log_wal_reset_bug_once(
    ...
    *,
    ...
    logger.warning(
        ...
        "%s. See `moor doctor`. This warning fires once per "
...
def _log_wal_fallback_once(db_label: str, exc: Exception) -> None:
    ...
    connection on every operation — see moor_cli/kanban_db.py) would
...
def apply_database_pragmas(
    ...
    * ``journal_size_limit`` — max journal/WAL size in bytes
    ...
    """
    try:
        # Local import avoids a circular import with moor_cli.config.
...
def classify_persistence_error(exc_or_str) -> str:
    ...
    * ``"corrupt"`` — the database file itself is structurally damaged
      ...
      (``moor doctor`` / automatic schema surgery).
...
# the Desktop app's own ``moor serve`` backend, interactive CLI sessions,
...
# included because rollback-journal (DELETE) mode — Moor's fallback on
...
def _db_fingerprint(db_path: Path) -> "Optional[str]":
    ...
    RESERVED lock (see ``moor_cli.sqlite_safe_read`` rule 1). This function
    ...
    try:
        ...
        try:
            ...
            # Scaffold/embed installs ship moor_state without moor_cli. No
...
def _backup_db_file(db_path: Path) -> "Tuple[Optional[Path], Optional[str]]":
    ...
    connection's POSIX advisory locks (see ``moor_cli.sqlite_safe_read``).
...
def preflight_db_writability(
    ...
    *,
    ...
    - **Repairs** permissions with ``chmod u+rw`` when the file lives inside
      the Moor home tree (``get_moor_home()``) — the safe repair scope:
    ...
    to check). Shared by :class:`SessionDB` and ``moor_cli.kanban_db``.
    """
    raw = str(db_path)
    ...
    try:
        home: Optional[Path] = Path(get_moor_home()).resolve()
...
def _connect_repair_durable(
    ...
    On Darwin that is exactly the combination :func:`_enforce_macos_synchromoor_full`
    ...
    try:
        ...
        for fts_table in ("messages_fts", "messages_fts_trigram", "messages_fts_cjk"):
            try:
                ...
                # (moor_state.py:645-723). The supported degraded-runtime
                # path (SessionDB._is_fts5_unavailable_error + the
                # regression suite in tests/test_moor_state.py:600-632)
...
def _live_writer_holds_db(db_path: Path) -> bool:
    ...
    bug and on NFS/SMB, Moor deliberately runs ``state.db`` in
        ...
        if not holding_lock:
            ...
            # later BEGIN EXCLUSIVE acquisition.
                ...
                report["error"] = (
                    ...
                    "Stop the gateway (moor gateway stop) and retry."
                )
                logger.error("state.db repair skipped: %s", report["error"])
            else:
...
# (``~/.moor/lib/libfts5_cjk.so``, built by ``native/fts5_cjk/build.sh``).
...
# next ``moor sessions optimize-storage`` on a capable host).
#
...
def fts5_cjk_so_path() -> Path:
    ...
    env = os.getenv("MOOR_FTS5_CJK_SO")
    if env:
        return Path(env).expanduser()
    return get_moor_home() / "lib" / "libfts5_cjk.so"


def _cjk_fts_config_enabled() -> bool:
    """config.yaml ``sessions.cjk_fts`` (default on), via its env bridge."""
    return os.getenv("MOOR_CJK_FTS", "1").strip().lower() not in (
        "0", "false", "off", "no",
    )
...
def _connect_tracked_db(path, tracking_path=None, **kwargs):
    ...
    (scaffold/embed installs that ship moor_state without moor_cli). A
    ...
    try:
        ...
        logger.debug(
            "moor_cli.sqlite_safe_read unavailable; opening %s untracked "
    ...
    # patch moor_state.sqlite3.connect keep control of connection creation;
...
def is_zeroed_state_db(
    ...
    Prefer ``moor_cli.backup.is_zeroed_sqlite_file`` when available; this
    ...
    try:
        ...
        if not acquired:
            ...
            logger.error(
                ...
                "state-snapshots via `moor snapshot list` / "
                "`moor snapshot restore <id>`.",
                path,
            )
            return None
...
# ── Read-only health/stats probes (moor doctor, dashboards) ──────────
...
_MOOR_CMDLINE_MARKERS = ("moor_cli.main", "moor_cli/main", "moor serve",
                           "moor-agent", "moor gateway", "moor chat")


def _looks_like_moor(cmdline: str) -> bool:
    """Heuristic: does this cmdline look like a Moor process?
    ...
    We only flag processes that look like Moor, not every system daemon.
    """
    lower = cmdline.lower()
    return any(marker in lower for marker in _MOOR_CMDLINE_MARKERS)
...
class SessionDB(SessionSearchMixin, SessionSchemaMixin, SessionPortabilityMixin):
    ...
    # With multiple moor processes (gateway + CLI sessions + worktree agents)
    ...
    # `moor update` leaves mixed-version processes sharing the DB until
        ...
        try:
            ...
            if (
                ...
                msg = (
                    ...
                    f"Restore from {snaps} via `moor snapshot list` / "
                    f"`moor snapshot restore <id>` if available. "
            ...
            # NOTE: the v23 FTS optimization is OPT-IN (`moor db optimize`),
            ...
            # ``moor_state._set_last_init_error(None)`` explicitly.
    ...
    def _warn_fts5_unavailable(self, exc: sqlite3.OperationalError) -> None:
        ...
        logger.warning(
            ...
            "disabled. Run `moor update` to rebuild the venv with a "
            ...
            if cjk_present:
                ...
                if live:
                    ...
                    logger.warning(
                        ...
                        "run `moor sessions optimize-storage` on a host "
            ...
            try:
                ...
                for pid_str in os.listdir("/proc"):
                    ...
                    try:
                        ...
                        # so check whether this is a Moor process —
                        # only flag uninspectable holders that look like
                        # another Moor instance, not every system daemon.
                        cmdline = _read_proc_cmdline(pid)
                        if cmdline is not None and _looks_like_moor(cmdline):
    ...
    def _reap_inactive_orphan_desktop_holders(
        ...
        if not sys.platform.startswith("linux") or psutil is None:
            return []
        try:
            from moor_cli.dashboard_procs import _is_ephemeral_port_zero_backend
        except Exception:
            return []
    ...
    #
    # `optimize_fts_storage()` (the `moor sessions optimize-storage`
    ...
    # ── Opt-in v23 FTS storage optimization (`moor sessions optimize-storage`) ──
    #
    ...
    # offline repair path behind ``moor sessions repair-routing``.
    ...
    def set_session_yolo(self, session_id: str, enabled: bool) -> None:
        ...
        ``--yolo`` launch — survives ``moor --resume`` into a fresh
    ...
    # of truth: moor_state_common._RECOVERABLE_END_REASONS, interpolated
    ...
    def _rows_to_conversation(
        ...
        *,
        ...
        for row in rows:
            ...
            # that replay reasoning (OpenRouter, OpenAI, Moor) receive
        ...
        if repair_alternation and messages:
            # Lazy import: moor_state already depends on agent.* (see
    ...
    def get_conversation_root(self, session_id: str) -> str:
        ...
        identifier — used for Moor Portal ``conversation=`` usage tagging.
    ...
    def search_sessions(
        ...
        ``moor -c``/``--resume`` so the "last" session is the last one in
    ...
    def delete_session_if_empty(
        ...
        and ``moor sessions list`` output. (Pattern ported from
    ...
    def retag_kanban_worker_sessions(self, workspaces_root: str) -> int:
        ...
        Workers used to spawn without ``MOOR_SESSION_SOURCE``, so their runs
    ...
    def vacuum(self) -> int:
        ...
        with self._lock:
            ...
            # TRUNCATE: a manual `moor sessions vacuum` runs in a transient
