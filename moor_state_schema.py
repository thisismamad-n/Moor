...
Plain mixin for ``moor_state.SessionDB`` (no ``__init__``/state of its own).
Must never import moor_state (cycle); shared constants live in moor_state_common.
"""
...
class SessionSchemaMixin:
        ...
        if attempts >= _FTS_HOLDER_ESCALATE_ATTEMPTS and now - first_seen >= _FTS_HOLDER_ESCALATE_SECONDS:
            ...
            if foreign_holders:
                logger.error(
                    ...
                    "`moor doctor` reports this degraded state.", attempts, foreign_holders,
                )
    ...
    def _parse_schema_columns(schema_sql: str) -> Dict[str, Dict[str, str]]:
        ...
        with contextlib.suppress(Exception):  # missing/corrupt cache → recompute below
            # Late import: resolves a test-patched moor_constants.get_moor_home.
            from moor_constants import get_moor_home as _home
    ...
    def _run_data_migrations(self, cursor: sqlite3.Cursor, current_version: int, fts5_available: bool) -> None:
        ...
        # install only gets a flag; `moor sessions optimize-storage` performs it. The FTS
        ...
        # as before, and we only record a flag advertising that the optimization is available. `moor
