...
def prepare_iteration(agent: Any,*, messages: Any, api_call_count: Any) -> IterationPrep:
    ...
    # moor.anthropic_wire=auto: a wire switch decided from the previous response lands here,
    # before this iteration's request is built and with nothing in flight.
    if getattr(agent, "_moor_wire_pending", None):
        from agent.moor_wire import apply_pending_wire_switch
    ...
    # Moor agent keys live ~1 h and a single turn can run for hours: adopt the keepalive's fresh
    ...
    try:
        agent._adopt_moor_key_before_expiry()
    except Exception:
        logger.debug("Moor key pre-expiry adoption failed", exc_info=True)
