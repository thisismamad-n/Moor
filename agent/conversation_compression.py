...
# Defaults for the in-agent progress-aware wrap; mirror moor_cli.config.DEFAULT_CONFIG["compression"] keys.
...
def _lock_api_is_absent_on_session_db(lock_db: Any) -> bool:
    ...
    Only the exact old ``moor_state.SessionDB`` class (hot-reload skew) may fail open; proxies, lookalikes,
...
def _lower_threshold_to_aux_context(
    ...
    if threshold_suggestion_viable:
        ...
        msg += (
            ...
            f"Moor's small-context floor and output reservation would recompute the trigger to "
        ...
        if _lock_lookup_error is not None:
            ...
            if getattr(agent, "_last_compression_lock_error_sid", None) != _lock_sid:
                ...
                logger.warning(
                    ...
                    "in-memory module after an update; restart the process (or `moor update`) to resync.",
                    _lock_sid,
                )
...
def _parent_deliberately_ended(session_db: Any, session_id: str) -> bool:
    ...
    if not callable(reader):
        return False
    try:
        from moor_state_common import is_automatic_end_reason
...
def _publish_rotated_compaction(
    ...
    # publish also COALESCEs from the parent row for threads lacking MOOR_HOME.
    ...
    agent._session_db.publish_compression_child(
        ...
        source=agent.platform or os.environ.get("MOOR_SESSION_SOURCE", "cli"), model=agent.model,
...
def compress_context(
    ...
    # compression.codex_app_server_auto). Memory handoff is moor-only: no native
...
def _compress_context_via_codex_app_server(
    ...
    Moor' transcript is left unchanged."""
...
def _shrink_data_url(url: str, *, max_dimension: int, resize_fn: Any) -> tuple:
    ...
    try:
        ...
        tmp = tempfile.NamedTemporaryFile(
            prefix="moor_shrink_", suffix=_IMAGE_SUFFIX_BY_MIME.get(mime, ".jpg"), delete=False
        )
        try:
            tmp.write(raw)
            tmp.close()
