...
def _resolve_concurrent_tool_timeout() -> float | None:
    ...
    in config.yaml wins, the legacy ``MOOR_CONCURRENT_TOOL_TIMEOUT_S`` env var
...
def _flush_session_db_after_tool_progress(
    ...
    *,
    ...
    current Moor process before the normal turn-end persistence path runs.
...
def _resolve_sequential_tool_timeout() -> float | None:
    ...
    ``MOOR_CONCURRENT_TOOL_TIMEOUT_S`` legacy bridge) so the two executor
