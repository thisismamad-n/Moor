...
thread refreshes) → disk cache (~/.moor/models_dev_cache.json, any age) →
...
PROVIDER_TO_MODELS_DEV: Dict[str, str] = {
    ...
    # Moor provider is "meta-ai"; both aliases are needed or muse-spark-*
...
def _models_dev_to_moor_ids(mdev_id: str) -> List[str]:
    ...
    if _MODELS_DEV_TO_PROVIDER is None:
        ...
        for moor_id, mapped in PROVIDER_TO_MODELS_DEV.items():
            _MODELS_DEV_TO_PROVIDER.setdefault(mapped, []).append(moor_id)
...
def _models_dev_id(
    ...
    """models.dev provider id for a Moor provider id, or None. A custom provider reaches the
    ...
    if mdev_id is None:
        ...
        if mdev_id is not None and mdev_id not in PROVIDER_TO_MODELS_DEV.values() \
            ...
            if (key, alias) not in _UNKNOWN_CATALOG_PROVIDER_WARNED:
                ...
                logger.warning("providers.%s: catalog_provider %r is neither a Moor provider id nor a "
...
def _moor_path(name: str) -> Path:
    from moor_constants import get_moor_home
    return get_moor_home() / name


def _get_cache_path() -> Path:
    return _moor_path("models_dev_cache.json")


def _get_etag_path() -> Path:
    return _moor_path("models_dev_cache.etag")
...
def _get_provider_models(
    ...
    """Resolve a Moor provider ID to its models dict, or None if unknown.
...
# models the catalog does not know and never displace catalog data. Provider keys accept the Moor
...
def _provider_override_section(provider: str, *, config: Optional[Dict[str, Any]] = None) -> Optional[Dict[str, Any]]:
    """Override section for *provider* (keyed by Moor OR models.dev id), or None."""
    overrides = (
    ...
    if not overrides or not provider_key:
        return None
    # Forward (Moor → models.dev id) and reverse (caller passed a models.dev id, config keyed by Moor id) aliases.
    candidates = [provider_key, PROVIDER_TO_MODELS_DEV.get(provider_key), *_models_dev_to_moor_ids(provider_key)]
...
def _builtin_model_metadata(
    ...
    """Built-in metadata for a provider/model pair, if Moor has a vendor-specific entry."""
...
def get_provider_info(
    ...
    """Provider metadata by Moor or models.dev ID, or None if not cataloged. ``allow_network`` defaults to True (interactive setup)."""
...
def get_model_info(
    ...
    """Full model metadata by Moor or models.dev provider ID (exact match, then case-insensitive), or
