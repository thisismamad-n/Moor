...
Data resolution order:
  ...
  2. Disk cache (~/.moor/models_dev_cache.json — any age; stale data is
...
def _models_dev_to_moor_ids(mdev_id: str) -> List[str]:
    ...
    if _MODELS_DEV_TO_PROVIDER is None:
        ...
        for moor_id, mapped in PROVIDER_TO_MODELS_DEV.items():
            reverse.setdefault(mapped, []).append(moor_id)
...
def _get_cache_path() -> Path:
    ...
    return get_moor_home() / "models_dev_cache.json"


def _get_etag_path() -> Path:
    """Return path to the ETag sidecar file for conditional GET."""
    from moor_constants import get_moor_home
    return get_moor_home() / "models_dev_cache.etag"
...
def fetch_models_dev(
    ...
    When ``force_refresh=True`` (used by ``moor config refresh``, the
...
def _provider_override_section(provider: str) -> Optional[Dict[str, Any]]:
    ...
    for moor_id in _models_dev_to_moor_ids(provider_key):
        if moor_id != provider_key:
            candidates.append(moor_id)
...
def list_provider_models(
    ...
    picker (``moor model``), which is an interactive user-facing flow
