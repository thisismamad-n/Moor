...
unreadable at terminal widths). Source: ``MoorPortalAccountInfo.paid_service_access_info``
...
def moor_logged_in() -> bool:
    """Cheap local auth-state check: a Moor access token is present. Fail-closed."""
    try:
...
def usage_model_from_account(account_info: Any) -> UsageModel:
    """Build a :class:`UsageModel` from a ``MoorPortalAccountInfo``. Never raises."""
    try:
...
def build_usage_model(*, timeout: float = 10.0) -> UsageModel:
    """Fetch account-info and build the usage model; fail-open. ``MOOR_DEV_CREDITS_FIXTURE`` short-circuits to a fixture."""
    fixture = _dev_fixture_usage_model()
    if fixture is not None:
        return fixture
    if not moor_logged_in():
        return UsageModel(available=False)
    try:
        return usage_model_from_account(fetch_moor_account(timeout))
    except Exception:
...
def _dev_fixture_usage_model() -> Optional[UsageModel]:
    """``MOOR_DEV_CREDITS_FIXTURE`` -> fixture model (``free|healthy|low|topup|depleted``), else None."""
