...
def _read_config_section(*path: str, label: str, log: logging.Logger = logger) -> Dict[str, Any]:
    """Read a nested section of ~/.moor/config.yaml. Tolerates missing file."""
    try:
...
def get_prune_builtins() -> bool:
    """Bundled built-ins are curation candidates (ON by default); a suppression list keeps them archived across `moor update` re-seeds. Hub skills are never pruned."""
    return bool(_load_config().get("prune_builtins", True))


def get_consolidate() -> bool:
    """LLM consolidation pass — OFF by default (prune only, no aux-model fork); ``moor curator run --consolidate`` overrides per invocation."""
...
def should_run_now(now: Optional[datetime] = None) -> bool:
    ...
    ``moor curator run`` bypasses this; the idle check is the caller's."""
    ...
    if last is None:
        try:
            ...
            state["last_run_summary"] = "deferred first run — curator seeded, will run after one interval; use `moor curator run --dry-run` to preview now"
            save_state(state)
...
CURATOR_REVIEW_PROMPT = (
    ...
    "`moor curator rollback` restores a hollow skill (issue #96962).\n\n"
...
def _reports_root() -> Path:
    """``~/.moor/logs/curator/`` (telemetry next to agent.log, not under skills/). mkdir'd here too so gateway-only / bare-library entry paths work."""
...
def _build_rename_summary(*, before_names: Set[str], after_report: List[Dict[str, Any]], tool_calls: List[Dict[str, Any]], model_final: str) -> str:
    ...
    if umbrellas:
        lines.append(f"keep an umbrella stable: moor curator pin {umbrellas[0]}")
    return "\n".join(lines)
...
_REPORT_SECTIONS = (
    ("consolidated", "Consolidated into umbrella skills",
     ...
     "The original directory was moved to `~/.moor/skills/.archive/` for safety and can be restored via "
     "`moor curator restore <name>` if the consolidation was wrong._\n", _consolidated_lines, 50, "see `run.json`"),
    ("pruned", "Pruned — archived for staleness",
     ...
     "Directories live under `~/.moor/skills/.archive/`. Restore any via `moor curator restore <name>`._\n",
...
def _render_report_markdown(p: Dict[str, Any]) -> str:
    ...
    if final:
        ...
        lines += ["## LLM summary\n", p.get("llm_summary"), ""]
    lines += ["## Recovery\n", "- Restore an archived skill: `moor curator restore <name>`",
              "- All archives live under `~/.moor/skills/.archive/` and are recoverable by `mv`",
...
def run_curator_review(
    ...
    *dry_run* SKIPS the stale/archive transitions and instructs the fork to report only; REPORT.md is still written and
    ...
    # next real pass out) but still records a summary for `moor curator status`.
    ...
    def _llm_pass():
        ...
        # Per-run report, best-effort; path recorded for `moor curator status`.
        try:
