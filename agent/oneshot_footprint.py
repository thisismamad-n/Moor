"""What a finite one-shot session (``moor chat -q`` / ``--oneshot``, ``moor -z``) does NOT do.

A one-shot run has no later session in its MOOR_HOME to learn for: the process answers one query and
...
delegation dispatcher already read (``MOOR_SINGLE_QUERY_SESSION``), so interactive sessions are untouched:
...
def is_single_query_session() -> bool:
    ...
    return str(get_session_env("MOOR_SINGLE_QUERY_SESSION", "") or "") == "1"
