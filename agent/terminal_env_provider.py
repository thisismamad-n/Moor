...
Providers live in ``~/.moor/plugins/<name>/`` (user, opt-in via
...
* ``cache_path_base`` — where auto-synced ``~/.moor/cache`` files land
  inside the backend (e.g. ``"~/.moor"`` for home-synced backends,
  ``"/root/.moor"`` for root-homed containers), or ``None`` when host
...
The factory stamps ``_moor_backend_name`` on the returned object so
...
class TerminalEnvironmentProvider(abc.ABC):
    ...
    def cache_path_base(self) -> Optional[str]:
        """Base dir for synced Moor cache files inside the backend."""
        return None
    ...
    def setup_instructions(self) -> List[str]:
        """Lines printed by ``moor setup`` after this backend is selected.
