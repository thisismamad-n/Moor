...
def _try_refresh_moor_paid_entitlement_credentials(agent) -> bool:
    ...
    try:
        ...
        if get_moor_portal_account_info(force_fresh=True).paid_service_access is not True:
            return False
        return agent._try_refresh_moor_client_credentials(force=True)
    except Exception:
        return False
...
def _print_moor_401_diagnostics(agent: Any, api_error: Exception) -> None:
    """Moor 401 that survived a credential refresh: likely Portal OAuth expired/revoked,
    no credits, or agent key blocked."""
    from agent.conversation_loop import _print_moor_entitlement_guidance
    ...
    try:
        ...
        pass
    _plines(agent, "🔐 Moor 401 — Portal authentication failed.")
    if _body_text:
    ...
    _plines(
        agent,
        "   Troubleshooting:",
        "     • Re-authenticate: moor auth add moor",
        "     • Check credits / billing: https://portal.nousresearch.com",
        f"     • Verify stored credentials: {display_moor_home()}/auth.json",
...
def _print_anthropic_401_diagnostics(agent: Any, key: Any) -> None:
    ...
    if is_token_provider(key):
        ...
        _plines(
            ...
            "   Run `moor doctor` for credential-chain diagnostics, or",
...
def _refresh_credentials_after_401(
    ...
    """Per-provider one-shot credential refresh on 401 (codex/xai, vertex, moor, copilot,
    anthropic), printing user-facing diagnostics when the moor/anthropic refresh fails.
    ...
    if (
        ...
        if agent._try_refresh_moor_client_credentials(force=True):
            agent._buffer_vprint("🔐 Moor agent key refreshed after 401. Retrying request...")
            return True
        _print_moor_401_diagnostics(agent, api_error)
...
def recover_after_classification(
    ...
    Moor paid-entitlement refresh → credential-pool rotation → image shrink →
    ...
    if (
        ...
        if _try_refresh_moor_paid_entitlement_credentials(agent):
            _vlines(agent, "🔐 Moor paid access verified — refreshed runtime credentials and retrying request...")
            return True, False
    ...
    # Reasoning-mandatory route (Moor Portal / OpenRouter, e.g. GLM-5.3) 400s on
    ...
    if (
        ...
        try:
            from moor_cli.models_reasoning_caps import refresh_reasoning_caps_async
...
def _print_nonretryable_auth_guidance(
    ...
    from agent.conversation_loop import _print_billing_or_entitlement_guidance, _print_moor_entitlement_guidance
    ...
    if classified.reason == FailoverReason.billing and _print_billing_or_entitlement_guidance(
        ...
        return
    if provider == "moor" and _print_moor_entitlement_guidance(agent, "Moor model access"):
        return
    if provider in {"openai-codex", "xai-oauth", "moor"} and status_code == 401:
        if provider == "openai-codex":
            _vlines(
                ...
                "      2. Then run `moor auth` to re-authenticate.",
            )
        elif provider == "xai-oauth":
            _vlines(
                agent,
                "   💡 xAI OAuth token was rejected (HTTP 401). To fix:",
                "      re-authenticate with xAI Grok OAuth (SuperGrok / Premium+) from `moor model`.",
            )
        else:  # moor
            _vlines(
                agent,
                "   💡 Moor Portal OAuth token was rejected (HTTP 401). Your token may be",
                "      expired, revoked, or your account may be out of credits. To fix:",
                "      1. Re-authenticate: moor portal",
                "      2. Check your portal account: https://portal.nousresearch.com",
            )
            # ``:free`` is OpenRouter slug syntax; Moor Portal will reject the model
            ...
            if isinstance(model, str) and model.endswith(":free"):
                _vlines(
                    ...
                    "         Moor Portal won't recognize that model name. Either switch to a",
                    f"         Moor catalog model, or run `/model openrouter:{model}` to use OpenRouter.",
                )
        return
    _vlines(
        agent,
        "   💡 Your API key was rejected by the provider. Check:",
        "      • Is the key valid? Run: moor setup",
...
def nonretryable_client_error_result(
    ...
    if classified.reason == FailoverReason.content_policy_blocked:
        _vlines(
            ...
            "        moor fallback add   (interactive picker — same as `moor model`)",
        )
...
def max_retries_exhausted_result(
    ...
    if _is_thinking_timeout:
        _vlines(
            ...
            "in `~/.moor/config.yaml` to extend the per-call timeout. (Moor's built-in floor is 600s for "
    ...
    result.update({
        ...
        # Present only for billing walls: (provider, billing_url, is_moor, message).
...
class ClassifiedErrorVerdict:
    ...
    ``"break"`` (restart armed on ``_retry``), ``"continue"`` (re-enter the retry loop; Moor
...
def _is_genuine_moor_rate_limit(agent: Any, api_error: Exception, error_context: Any) -> bool:
    ...
    _genuine = False
    try:
        from agent.moor_rate_guard import is_genuine_moor_rate_limit, record_moor_rate_limit
        _err_resp = getattr(api_error, "response", None)
        _err_hdrs = getattr(_err_resp, "headers", None) if _err_resp else None
        _genuine = is_genuine_moor_rate_limit(headers=_err_hdrs, last_known_state=agent._rate_limit_state)
        if _genuine:
            record_moor_rate_limit(headers=_err_hdrs, error_context=error_context)
        else:
            logger.info(
...
def route_classified_error(
    ...
    chain once; genuine Moor 429 → cross-session breaker + re-enter the loop exactly once."""
    ...
    # Moor Portal: a genuine account-level 429 is recorded to a shared file so ALL
    # sessions back off; is_genuine_moor_rate_limit excludes upstream 429s.
    if (
        is_rate_limited
        and agent.provider == "moor"
        and classified.reason == FailoverReason.rate_limit
        and not recovered_with_pool
        and _is_genuine_moor_rate_limit(agent, api_error, error_context)
    ):
