...
from agent.prompt_builder import (
    ...
    MOOR_AGENT_HELP_GUIDANCE, MOOR_AGENT_HELP_GUIDANCE_NO_SKILLS, KANBAN_GUIDANCE, MEMORY_GUIDANCE,
...
def _tui_embedded_pane_clarifier(hint: str) -> str:
    """Append the desktop embedded-terminal clarifier when ``MOOR_DESKTOP_TERMINAL``
    is set (only the desktop's TUI PTY, never the chat backend). Idempotent."""
    if not hint or _TUI_EMBEDDED_PANE_CLARIFIER in hint or not is_truthy_value(os.getenv("MOOR_DESKTOP_TERMINAL")):
        return hint
...
def _active_profile_name(agent: Any, ambient) -> str:
    ...
    failure. Ambient resolution misreports on threads that lost the MOOR_HOME
...
def _agent_home(agent: Any) -> Optional[Path]:
    ...
    A bound MOOR_HOME ContextVar override wins (the gateway multiplexes
...
def _profile_name_for_home(home: Path) -> str:
    ...
    Uses ``get_default_moor_root()`` (NOT ``get_moor_home()``): on a bound
    ...
    try:
        ...
        rel = home.resolve().relative_to((get_default_moor_root() / "profiles").resolve())
...
def _active_profile_line(agent: Any) -> str:
    """Name the running profile so the agent doesn't conflate ``~/.moor/skills``
    (default) with ``~/.moor/profiles/<active>/skills``.  Resolved from the
    ...
    if active_profile == "default":
        ...
        # ROOT (get_moor_home() on a bound profile session is the PROFILE dir).
        # Without one, keep the ambient (patchable) resolution byte-identical.
        _root_str = str(get_default_moor_root() if _agent_home_path is not None else get_moor_home())
        return (
    ...
    profile_home = str(_agent_home_path) if _agent_home_path is not None else str(get_moor_home())
    ...
    # default profile's data sits at the ROOT (get_default_moor_root()), which in ambient profile mode is
    # NOT get_moor_home().
...
def _context_files_part(agent: Any, ctx_len: Optional[int], soul_loaded: bool) -> List[str]:
    ...
    fallback they really are so the guard can reject Moor's bundled AGENTS.md."""
...
def build_system_prompt_parts(agent: Any, system_message: Optional[str] = None) -> Dict[str, str]:
    ...
    # moor-agent skill installed, so the variant is chosen after the skills
    # index is built; this slot holds its position.
    _help_guidance_slot = len(stable_parts)
    stable_parts.append(MOOR_AGENT_HELP_GUIDANCE_NO_SKILLS)
    stable_parts.extend(_guidance_parts(agent))
    skills_prompt = _skills_prompt(agent)
    # Skill-pointer variant requires BOTH skill_view AND the moor-agent skill
    # in the rendered index (pure string check — inherits the index's stability).
    if "skill_view" in (agent.valid_tool_names or set()) and "- moor-agent:" in skills_prompt:
        stable_parts[_help_guidance_slot] = MOOR_AGENT_HELP_GUIDANCE
