...
upstream prefix cache warm.  See ``moor-agent-dev``'s
...
* ``stable``   — identity (SOUL.md or DEFAULT_AGENT_IDENTITY), tool
  guidance, computer-use guidance, moor subscription block, tool-use
...
_PLUGIN_SECTION_FRAME_RE = re.compile(
    ...
    r"<!-- moor-plugin-section-chars:(?P<chars>[0-9]{1,4}) -->\n\n",
    re.MULTILINE,
)
...
def _ra():
    ...
    ``build_context_files_prompt``, ``build_moor_subscription_prompt``,
...
def _tui_embedded_pane_clarifier(hint: str) -> str:
    ...
    Triggered by ``MOOR_DESKTOP_TERMINAL=1`` (set by ``main.cjs`` only on the
    ...
    if _TUI_EMBEDDED_PANE_CLARIFIER in hint:
        return hint
    if not is_truthy_value(os.getenv("MOOR_DESKTOP_TERMINAL")):
        return hint
...
def _plugin_session_info(agent: Any) -> Dict[str, str]:
    ...
    try:
        ...
        # the MOOR_HOME ContextVar (#86313 class; plugin half per @helix4u).
...
def _frozen_plugin_prompt_sections(agent: Any) -> tuple:
    ...
    if isinstance(stored_prompt, str) and stored_prompt:
        ...
        return rendered
    try:
        from moor_cli.plugins import render_system_prompt_sections
...
def _plugin_section_blocks(sections: tuple, position: str) -> List[str]:
    from moor_cli.plugins import format_system_prompt_sections
...
def _agent_home(agent: Any) -> Optional[Path]:
    ...
    1. A bound MOOR_HOME ContextVar override wins. Surfaces that multiplex
...
def _profile_name_for_home(home: Path) -> str:
    ...
    Uses :func:`get_default_moor_root` (NOT ``get_moor_home()``): on a
    correctly bound profile session the ambient home IS the profile dir, so
    ``get_moor_home()/profiles`` would never contain ``home`` and every
...
def build_system_prompt_parts(agent: Any, system_message: Optional[str] = None) -> Dict[str, str]:
    ...
    # Some execution modes (cron) still want MOOR_HOME persona while keeping
    # cwd project instructions disabled.
    _soul_loaded = False
    if agent.load_soul_identity or not agent.skip_context_files:
        # Scope the SOUL.md read to the agent's OWN home (see _agent_home) —
        # ambient resolution on a thread that lost the MOOR_HOME ContextVar
    ...
    # Pointer to the moor-agent skill + docs for user questions about Moor
    ...
    stable_parts.append(
        MOOR_AGENT_HELP_GUIDANCE if _has_skill_view
        else MOOR_AGENT_HELP_GUIDANCE_NO_SKILLS
    )
    ...
    # MOOR_KANBAN_TASK env var). Normal chat sessions never see
    ...
    moor_subscription_prompt = _r.build_moor_subscription_prompt(agent.valid_tool_names)
    if moor_subscription_prompt:
        stable_parts.append(moor_subscription_prompt)
    ...
    # `moor -p <bot> chat --in ~ -c "Bot Chat"` and the desktop pins), on
    # installs where Bot Mode manages profiles (ui_meta['moor-bots']).
    ...
    # under so it doesn't conflate ~/.moor/skills/ (default profile) with
    # ~/.moor/profiles/<active>/skills/ (this profile's). Deterministic
    ...
    # ambient MOOR_HOME: on a build thread that lost the ContextVar this
    ...
    # from get_default_moor_root(): get_moor_home() on a bound profile
    ...
    # to the legacy behavior (and patchable via this module's get_moor_home).
    if _agent_home_path is not None:
        _home_str = str(_agent_home_path)
        _root_str = str(get_default_moor_root())
    else:
        _home_str = _root_str = str(get_moor_home())
    if active_profile == "default":
        ...
        # data sits at the ROOT (get_default_moor_root()), which in ambient
        # profile mode is NOT get_moor_home().
    ...
    from moor_time import get_timezone as _moor_tz, now as _moor_now
    now = _moor_now()
