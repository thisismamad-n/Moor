...
RELAY_PLUGINS_EXECUTION_CONSUMER = "moor.nemo_relay.plugins"
...
def runtime_metadata(runtime_id: str, **extra: Any) -> dict[str, Any]:
    """Return the scope metadata that stamps every moor-owned Relay scope."""
    ...
    def run_in_session(self, session: RelaySession, callback: Callable[..., Any], *args: Any, **kwargs: Any) -> Any:
        ...
        with self._lock:
            if self._runtime is None:
                raise RuntimeError("Moor Relay operation lease is released")
...
class _ProcessRelayPluginConfiguration:
    ...
    def __init__(self) -> None:
        ...
        self._relay: Any = None  # set while a moor-owned configuration is active
        ...
        with self._lock:
            if not self._owners:
                ...
                if self._state is _RelayPluginConfigurationState.ACTIVE:
                    logger.info(
                        "Relay plugins are active process-wide and apply to all profiles hosted by this Moor process."
                    )
    ...
    def _activate(self, relay: Any) -> _RelayPluginConfigurationState:
        try:
            ...
            logger.warning("Moor Relay plugin initialization failed: %s", exc, exc_info=True)
    ...
    def _preflight(self, relay: Any) -> _RelayPluginConfigurationState | None:
        ...
        if self._relay is not None and not self._clear_active():
            logger.warning(
                "Moor Relay plugin cleanup is still pending; refusing to replace the process-global configuration"
            )
            return _RelayPluginConfigurationState.FAILED
        try:
            ...
            logger.warning(
                "Moor could not determine whether a process-global Relay plugin configuration is already "
                "active; refusing to replace it", exc_info=True,
            )
            return _RelayPluginConfigurationState.FAILED
        if existing_report is not None:
            logger.warning(
                "A process-global Relay plugin configuration is already active outside Moor native ownership; "
                "leaving it unchanged and disabling moor-managed Relay middleware for this process"
            )
    ...
    def _initialize(self, relay: Any) -> bool:
        ...
        if dynamic_plugins:
            try:
                ...
                raise RuntimeError("Moor Relay dynamic plugin activation failed") from exc
    ...
    def _clear_active(self) -> bool:
        ...
        for what, step in (
            ...
            try:
                ...
                logger.warning("Moor Relay plugin %s failed", what, exc_info=True)
                return False
...
class RelayRuntime:
    ...
    def rotate_session_scope(self, session: RelaySession, *, reason: str) -> None:
        ...
        with session.lock:
            ...
            try:
                self.run_in_session(
                    session, self.relay.scope.pop, old_handle, output={"moor.session.segment_reason": reason},
                    metadata=runtime_metadata(self.runtime_id), timeout=_SCOPE_OP_TIMEOUT,
                )
            except Exception:
                logger.warning(
                    "Moor Relay segment close failed (session=%s segment=%d); abandoning the old segment span",
                    session.session_id, session.segment - 1, exc_info=True,
                )
            scope_metadata = runtime_metadata(
                self.runtime_id, **{"moor.session.segment": session.segment, "moor.session.segment_reason": reason},
            )
            try:
                self._open_session_scope(session, scope_metadata, resolve_parent=False)
            except Exception:
                logger.warning(
                    "Moor Relay segment open failed (session=%s segment=%d); keeping the prior scope handle",
    ...
    def _begin_operation(self) -> None:
        ...
        with self._sessions_lock:
            if self._closing:
                raise RuntimeError("Moor Relay runtime is shutting down")
    ...
    def _pop_with_drain(
        ...
        for _ in range(drain_limit):
            ...
            if session_root is not None and _same_handle(top, session_root) and handle is not session_root:
                break
            try:
                orphan_output = {"outcome": "cancelled", "moor.orphan_drain": True}
                pop_relay_scope(self.relay, top, output=orphan_output, metadata=metadata)
                drained += 1
            except Exception:
                logger.warning("Moor Relay orphaned scope drain failed", exc_info=True)
                break
        if drained:
            logger.warning("Moor Relay drained %d orphaned scope(s) before closing %s", drained, handle)
        try:
            pop_relay_scope(self.relay, handle, output=output, metadata=metadata)
            return None
    ...
    def _close_session(self, event: dict[str, Any]) -> None:
        ...
        with self._sessions_lock:
            ...
            self._forget_subagent(session_id)
        if failure:
            logger.warning("Moor Relay session %s closed with errors: %s", session_id, failure)
    ...
    def shutdown(self) -> None:
        ...
        thread = threading.Thread(
            ...
            name=f"moor-nemo-relay-shutdown-{self.runtime_id[:8]}", daemon=True,
        )
        try:
            thread.start()
        except Exception:
            ...
            logger.warning("Moor Relay deferred shutdown could not start", exc_info=True)
    ...
    def _finish_shutdown(self) -> None:
        try:
            ...
            with self._sessions_lock:
                self._shutdown_started = False
            logger.warning("Moor Relay shutdown failed", exc_info=True)
            return
        with self._sessions_lock:
    ...
    def for_profile(self, profile_key: str | None = None, *, create: bool = True) -> RelayHost | None:
        ...
        with self._lock:
            ...
            try:
                ...
                logger.warning("Moor Relay runtime initialization failed", exc_info=True)
...
# >0 while the native pipeline is mid-dispatch of a Moor tool/LLM callback. Nested managed
...
def _warn_on_error(what: str, callback: Callable[..., Any], *args: Any, **kwargs: Any) -> Any:
    """Run fail-open telemetry work: log ``Moor Relay <what> failed`` and return None on error."""
    try:
        return callback(*args, **kwargs)
    except Exception:
        logger.warning("Moor Relay %s failed", what, exc_info=True)
        return None
...
class RelaySessionCoordinator:
    ...
    def _prepare_session(self, host: RelayRuntime, context: dict[str, Any]) -> None:
        ...
        for name, callback in initializers:
            try:
                ...
                logger.warning("Moor Relay session initializer failed: %s", name, exc_info=True)
        ...
        *,
        ...
        with self._active_turns_lock:
            if self._active_turns.get(key):
                ...
                logger.warning(
                    "Skipping Relay instrumentation for concurrent Moor turn %s in session %s",
    ...
    def _close_turn_scope(self, host: RelayRuntime, turn: RelayTurnContext, *, outcome: str) -> None:
        ...
        if failure:
            logger.warning("Moor Relay turn finalization failed: %s", failure)
...
def resolve_execution_context(session_id: str) -> tuple[RelayRuntime | None, RelaySession | None, Any]:
    ...
    if _MANAGED_CALLBACK_DEPTH.get() > 0 or not relay_instrumentation_enabled():
        ...
        # ``tools.execute`` pipeline is mid-dispatch of a Moor tool). Nested managed execution here is
...
def _configured_plugin_inputs(relay: Any) -> tuple[dict[str, Any], list[Any]] | None:
    ...
    try:
        ...
        if "dynamic_plugins" in config:
            raise ValueError("Moor [[dynamic_plugins]] records are unsupported; use Relay [[plugins.dynamic]] records")
        ...
        raise _RelayPluginConfigurationLoadError(
            f"Moor Relay plugin configuration could not be loaded from {config_path}; continuing without Relay plugins"
        ) from exc
...
def _resolve_plugin_awaitable(value: Any) -> Any:
    ...
    return _run_on_daemon_thread(lambda: asyncio.run(value), name="moor-nemo-relay-plugin-lifecycle")
