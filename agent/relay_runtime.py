...
class _ProcessRelayPluginConfiguration:
    ...
    def acquire(
        ...
        with self._lock:
            ...
            if self._active and not self._clear_active():
                logger.warning(
                    "Moor Relay plugin cleanup is still pending; refusing to "
            ...
            try:
                ...
                logger.warning(
                    "Moor could not determine whether a process-global Relay "
            ...
            if existing_report is not None:
                logger.warning(
                    ...
                    "outside Moor native ownership; leaving it unchanged and "
                    "disabling moor-managed Relay middleware for this process"
                )
                return self._remember(
                    owner_id,
            ...
            try:
                ...
                if dynamic_plugins:
                    try:
                        ...
                        raise RuntimeError(
                            "Moor Relay dynamic plugin activation failed"
                        ) from exc

                if self._activation is None:
                    # Moor only enters Relay's initialization path after an
                ...
                logger.warning(
                    "Moor Relay plugin initialization failed: %s",
                    exc,
                    exc_info=True,
                )
                return self._remember(
    ...
    def _clear_active(self) -> bool:
        ...
        try:
            ...
            logger.warning(
                "Moor Relay plugin subscriber flush failed",
                exc_info=True,
            )
            return False
        try:
            ...
            logger.warning(
                "Moor Relay plugin configuration cleanup failed",
                exc_info=True,
            )
            return False
        self._active = False
    ...
    def shutdown(self) -> None:
        ...
        if has_active_operations:
            thread = threading.Thread(
                ...
                name=f"moor-nemo-relay-shutdown-{self.runtime_id[:8]}",
                daemon=True,
            )
            try:
                thread.start()
                ...
                with self._sessions_lock:
                    self._shutdown_started = False
                logger.warning(
                    "Moor Relay deferred shutdown could not start",
                    exc_info=True,
                )
            return
...
def _configured_plugin_inputs(
    ...
    try:
        ...
        if "dynamic_plugins" in config:
            raise ValueError(
                "Moor [[dynamic_plugins]] records are unsupported; use Relay "
        ...
        raise _RelayPluginConfigurationLoadError(
            "Moor Relay plugin configuration could not be loaded from "
...
def _resolve_plugin_awaitable(value: Any) -> Any:
    ...
    thread = threading.Thread(
        target=_runner,
        name="moor-nemo-relay-plugin-lifecycle",
        daemon=True,
    )
    thread.start()
    thread.join()
