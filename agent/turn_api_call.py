"""The provider call for the conversation turn's retry loop: ``moor_rate_limit_guard`` (skip
the attempt while another session's Moor Portal rate limit is active), ``perform_api_call``
...
def moor_rate_limit_guard(
...
) -> MoorRateGuardVerdict:
    """Skip the call if another session recorded a Moor Portal rate limit: every attempt (incl.
    ...
    def _verdict(action: str, result: Optional[Dict[str, Any]] = None) -> MoorRateGuardVerdict:
        return MoorRateGuardVerdict(
    ...
    if agent.provider == "moor":
        ...
        try:
            ...
            _moor_remaining = moor_rate_limit_remaining(anonymous=_anonymous)
            if _moor_remaining is not None and _moor_remaining > 0:
                ...
                agent._buffer_diagnostic_status(f"⏳ {_moor_msg}")
                ...
                return _verdict("return", stamp_failure({
                    "final_response": (f"⏳ {_moor_msg}" if _anonymous
                    ...
                    **({"free_tier": {"kind": "rate_limited", "message": anon_auth.FREE_TIER_RATE_LIMIT_CARD.format(
                        reset=anon_auth.friendly_wait(_moor_remaining))}} if _anonymous else {}),
