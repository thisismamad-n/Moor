...
def _constants_path(getter_name: str) -> Path:
    """Call ``moor_constants.<getter_name>()`` (local import avoids cycles); ``~/.moor`` on any failure."""
    try:
        import moor_constants

        return getattr(moor_constants, getter_name)()
    except Exception:
        return Path(os.path.expanduser("~/.moor"))


def _moor_home_path() -> Path:
    """Active MOOR_HOME (profile-aware). Tests monkeypatch this name."""
    return _constants_path("get_moor_home")


def _moor_root_path() -> Path:
    """Moor root dir (parent of any profile, never per-profile)."""
    return _constants_path("get_default_moor_root")


def _moor_dirs() -> list[Path]:
    """Resolved active MOOR_HOME and global root, deduplicated.

    Both are checked so credential stores at <root>/... stay guarded when
    running under a profile (MOOR_HOME = <root>/profiles/<name>).
    """
    return list(dict.fromkeys(_resolve_each((_moor_home_path(), _moor_root_path()))))
...
def build_write_denied_paths(home: str) -> set[str]:
    ...
    moor_files = (".env", ".anthropic_oauth.json", os.path.join("cache", "bws_cache.enc.json"))
    paths = [
        *(os.path.join(home, *f) for f in home_files),
        *(str(base / f) for f in moor_files for base in (_moor_home_path(), _moor_root_path())),
...
def get_safe_write_roots() -> set[str]:
    """Resolved MOOR_WRITE_SAFE_ROOT paths (``os.pathsep``-separated list)."""
    roots: set[str] = set()
    for path in filter(None, os.getenv("MOOR_WRITE_SAFE_ROOT", "").split(os.pathsep)):
...
# MOOR_HOME / root subpaths that the agent's generic file tools must not
...
_MOOR_PROTECTED_SUBPATHS = ("state.db", "sessions", "mcp-tokens", "pairing")
...
def _classify_write_denial(path: str) -> Optional[str]:
    ...
    for base in _moor_dirs():
        for sub in _MOOR_PROTECTED_SUBPATHS:
            with suppress(Exception):
...
# Exact-file credential stores under MOOR_HOME / <root>. The agent never
...
# Directory-prefix read denies under MOOR_HOME / <root>: (subdir, message for
...
def get_read_block_error(path: str) -> Optional[str]:
    ...
    stores under MOOR_HOME and the global root (exact files, plus anything
    ...
    moor_dirs = _moor_dirs()
    reason = None
    if any(_is_under(resolved, hd / "skills" / ".hub") for hd in moor_dirs):
        reason = (
            "is an internal Moor cache file and cannot be read directly to prevent "
            "prompt injection. Use the skills_list or skill_view tools instead."
        )
    elif any(resolved in _resolve_each(hd / name for hd in moor_dirs) for name in _CREDENTIAL_FILE_NAMES):
        reason = (
            "is a Moor credential store and cannot be read directly. Provider tools "
            "consume these credentials through internal channels." + _DID_SUFFIX
        )
    else:
        for subdir, dir_msg, file_msg in _READ_DENIED_DIRS:
            for blocked_dir in _resolve_each(hd / subdir for hd in moor_dirs):
...
def raise_if_read_blocked(path: str) -> None:
    """Raise ``ValueError`` if ``path`` is a denied Moor read (see ``get_read_block_error``).
...
def _resolve_active_profile_name() -> str:
    """Active profile name from MOOR_HOME: ``~/.moor`` -> ``"default"``,
    ``~/.moor/profiles/X`` -> ``"X"``; ``"default"`` on any resolution failure."""
    try:
        parts = _moor_home_path().resolve().relative_to(_moor_root_path().resolve() / "profiles").parts
...
def classify_sandbox_mirror_target(path: str) -> Optional[dict]:
    """Classify a write target as a sandbox-mirror of authoritative Moor state: ``None``
    for non-mirror paths, else ``target_path`` (resolved), ``mirror_root`` (the
    ``…/home/.moor`` prefix) and ``inner_path`` (what the agent meant on the host)."""
    target = _resolve_target(path)
    parts = target.parts if target is not None else ()
    # Need at least: sandboxes / <backend> / <task> / home / .moor / <thing>; inner_idx = the .moor part.
    inner_idx = next(
        (i + 4 for i, part in enumerate(parts)
         if part == "sandboxes" and i + 5 < len(parts) and parts[i + 3] == "home" and parts[i + 4] == ".moor"),
        None,
    )
...
def get_sandbox_mirror_warning(path: str) -> Optional[str]:
    ...
    return _mirror_warning(
        ...
        "Writes here land on a copy that the host Moor process never reads — the "
        "authoritative file is likely {inner_path!r} under the real MOOR_HOME.",
...
def classify_container_mirror_target(path: str, mirror_prefix: str | None = None) -> Optional[dict]:
    ...
    the bind mount strips the ``sandboxes/`` prefix (the agent sees plain ``/root/.moor/…``),
...
def get_container_mirror_warning(path: str, mirror_prefix: str | None = None) -> Optional[str]:
    ...
    return _mirror_warning(
        ...
        "the container's bind-mounted home — a per-task mirror that the host Moor "
