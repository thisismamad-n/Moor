"""Cross-session rate limit guard for Moor Portal.

Writes rate limit state to a shared file so all sessions (CLI, gateway,
cron, auxiliary) can check whether Moor Portal is currently rate-limited
before making requests.  Prevents retry amplification when RPH is tapped.

Each 429 from Moor triggers up to 9 API calls per conversation turn
(3 SDK retries x 3 Moor retries), and every one of those calls counts
...
_STATE_FILENAME = "moor.json"


def _state_path() -> str:
    """Return the path to the Moor rate limit state file."""
    try:
        from moor_constants import get_moor_home
        base = get_moor_home()
    except ImportError:
        base = os.path.join(os.path.expanduser("~"), ".moor")
...
def record_moor_rate_limit(
    *,
    headers: Optional[Mapping[str, str]] = None,
    error_context: Optional[dict[str, Any]] = None,
    default_cooldown: float = 300.0,
) -> None:
    """Record that Moor Portal is rate-limited.
    ...
    try:
        ...
        logger.info(
            "Moor rate limit recorded: resets in %.0fs (at %.0f)",
            reset_at - now, reset_at,
        )
    except Exception as exc:
        logger.debug("Failed to write Moor rate limit state: %s", exc)


def moor_rate_limit_remaining() -> Optional[float]:
    """Check if Moor Portal is currently rate-limited.
...
def clear_moor_rate_limit() -> None:
    """Clear the rate limit state (e.g., after a successful Moor request)."""
    try:
        os.unlink(_state_path())
    except FileNotFoundError:
        pass
    except OSError as exc:
        logger.debug("Failed to clear Moor rate limit state: %s", exc)
...
def is_genuine_moor_rate_limit(
    *,
    headers: Optional[Mapping[str, str]] = None,
    last_known_state: Optional[Any] = None,
) -> bool:
    """Decide whether a 429 from Moor Portal is a real account rate limit.

    Moor Portal multiplexes multiple upstream providers (DeepSeek, Kimi,
    MiMo, Moor, ...) behind one endpoint.  A 429 can mean either:

      (a) The caller's own RPM / RPH / TPM / TPH bucket on Moor is
      ...
      (b) The upstream provider is out of capacity for a specific model
          ...
          the caller's quota on Moor.

    Tripping the cross-session breaker on (b) blocks ALL Moor requests
    (and all models, since Moor is one provider key) for minutes even
    ...
    We tell the two apart by looking at:
      ...
      1. The 429 response's own ``x-ratelimit-*`` headers.  Moor emits
