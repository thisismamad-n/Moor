...
auxiliary) can check whether Moor Portal is currently rate-limited before making
requests. Without it each 429 fans out into up to 9 calls per turn (3 SDK
retries x 3 Moor retries), all counted against RPH.
"""
...
def _state_path(*, anonymous: bool = False) -> str:
    """Path to the Moor rate limit state file."""
    try:
    ...
    return os.path.join(base, "rate_limits", "moor-anonymous.json" if anonymous else "moor.json")
...
def record_moor_rate_limit(
    *, headers: Optional[Mapping[str, str]] = None, error_context: Optional[dict[str, Any]] = None,
    ...
    """Record that Moor Portal is rate-limited in the shared state file.
    ...
    try:
        ...
        logger.info("Moor rate limit recorded: resets in %.0fs (at %.0f)", reset_at - now, reset_at)
    except Exception as exc:
        logger.debug("Failed to write Moor rate limit state: %s", exc)


def moor_rate_limit_remaining(*, anonymous: bool = False) -> Optional[float]:
...
def clear_moor_rate_limit(*, anonymous: bool = False) -> None:
...
def is_genuine_moor_rate_limit(
    *, headers: Optional[Mapping[str, str]] = None, last_known_state: Optional[Any] = None,
    ...
    Moor multiplexes upstream providers behind one key, so a 429 may be one
    upstream model out of capacity (clears in seconds) rather than our quota;
    tripping the breaker on that would block every Moor model for minutes.
