...
def _getenv(name: str, default: str = "") -> str:
    ...
    read) when on. Mirrors the same wrapper in moor_cli/runtime_provider.py.
    """
...
def _is_moor_portal_endpoint(base_url: str | None) -> bool:
    """Return True for Moor Portal's Anthropic Messages route.
    ...
    2. The operator-set ``MOOR_INFERENCE_BASE_URL`` hostname (staging/preview)
...
def _requires_bearer_auth(base_url: str | None) -> bool:
    ...
    Foundry's Anthropic-style endpoint, Palantir Foundry's LLM proxy, and Moor
    Portal's Messages route follow this pattern.
    """
    if _is_moor_portal_endpoint(base_url):
        return True
    normalized = _normalize_base_url_text(base_url)
    if not normalized:
        return False
...
def _build_anthropic_client_with_bearer_hook(
    ...
    *,
    ...
    kwargs = {
        ...
        # Delegate retry to moor's outer loop (honors Retry-After); the SDK
...
def build_anthropic_client(
    ...
    * a ``Callable[[], str]`` — an Entra ID bearer token provider from
    ...
    kwargs = {
        ...
        # Delegate all rate-limit / 5xx retry to moor's outer conversation
    ...
    if _is_kimi_coding_endpoint(base_url):
        ...
        # HTTP-Referer + X-Title + MoorAgent User-Agent.
    ...
    if _is_opencode_endpoint(base_url):
        ...
        headers.setdefault("User-Agent", f"MoorAgent/{_MOOR_VERSION}")
    ...
    # ``~/.moor/.env``). The result is dual auth —
...
def _resolve_anthropic_pool_token() -> Optional[str]:
    ...
    ``account_usage`` and ``moor models``) never mutates ``~/.moor/auth.json``
...
def resolve_anthropic_token() -> Optional[str]:
    ...
    Priority:
      ...
      5. Anthropic credential_pool OAuth entry (~/.moor/auth.json)
...
# Stores credentials in ~/.moor/.anthropic_oauth.json (our own file).
...
def _get_moor_oauth_file() -> Path:
    return get_moor_home() / ".anthropic_oauth.json"
...
def run_moor_oauth_login_pure() -> Optional[Dict[str, Any]]:
    ...
    try:
        from moor_cli.auth import _can_open_graphical_browser as _can_open_gui
    except Exception:
...
def read_moor_oauth_credentials() -> Optional[Dict[str, Any]]:
    """Read Moor-managed OAuth credentials from ~/.moor/.anthropic_oauth.json."""
...
def _convert_assistant_message(m: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # can validate the message history.  See moor-agent#13848.
    #
...
def _strip_orphaned_tool_blocks(result: List[Dict[str, Any]]) -> None:
    ...
    for i, m in enumerate(result):
        ...
        # replaying it verbatim.  See moor-agent: extended-thinking + parallel
...
def _manage_thinking_signatures(
    ...
    replayed assistant tool-call messages.  See moor-agent#13848 (Kimi) and
    moor-agent#16748 (DeepSeek).

    Moor Portal's ``/v1/messages`` route is the exception among third-party
    ...
    _is_third_party = (
        ...
        and not _is_moor_portal_endpoint(base_url)
    )
...
def build_anthropic_kwargs(
    ...
    # Moor Portal routes on its own catalog ids (``anthropic/claude-opus-4.8``);
    ...
    if not _is_moor_portal_endpoint(base_url):
    ...
    if is_oauth:
        ...
        for block in system:
            if isinstance(block, dict) and block.get("type") == "text":
                ...
                text = text.replace("Moor agent", "Claude Code")
                text = text.replace("moor-agent", "claude-code")
                text = text.replace("Moor inc.", "Anthropic")
                block["text"] = text
...
def create_anthropic_message(
    ...
    *,
    ...
    parsed ``Message`` drops — Moor Portal's ``x-moor-credits-*`` balance family
