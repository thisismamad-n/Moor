"""Anthropic Messages API adapter: client construction + the Messages call for Moor's
...
from agent.anthropic_endpoints import (
    ...
    _is_minimax_anthropic_endpoint, _is_moor_portal_endpoint, _is_opencode_endpoint,
...
# Moor effort -> Anthropic adaptive-thinking effort (output_config.effort). 4.7+ exposes
...
# Anthropic's OAuth billing classifier fingerprints certain Moor tool schemas/prose as a
...
def _base_client_kwargs(base_url, timeout) -> tuple[str, Dict[str, Any]]:
    ...
    moor's outer loop (``max_retries=0``): the SDK default of 2 uses its own backoff that ignores
...
def _new_sdk_client(sdk, kwargs: Dict[str, Any], headers: Dict[str, str]):
    ...
    ``api_key`` unset, so the SDK fills it from ANTHROPIC_API_KEY (loaded from ~/.moor/.env) and
...
def _normalize_to_mcp_wire(name: str) -> str:
    ...
    Moor tools (``read_file``) and native MCP tools registered as ``mcp_<server>_<tool>`` must
...
def _thinking_kwargs(reasoning_config: Dict[str, Any], model: str, effective_max_tokens: int) -> Dict[str, Any]:
    ...
    4.7+ ``thinking.display`` defaults to "omitted", hiding the reasoning Moor shows in its CLI,
...
def build_anthropic_kwargs(
    ...
    # Moor Portal routes on its own catalog ids (``anthropic/claude-opus-4.8``); normalizing would
    ...
    # which silently hides reasoning text that Moor surfaces in its CLI. We request "summarized" so the
...
def create_anthropic_message(
    ...
    ``on_response(httpx_response)`` exposes headers the parsed Message drops (Moor Portal's
    ``x-moor-credits-*`` balance family)."""
...
_PLUGIN_COMPAT_LAZY = {
    ...
    'get_moor_home': ('moor_constants', 'get_moor_home'),
    ...
    'read_moor_oauth_credentials': ('agent.anthropic_credentials', 'read_moor_oauth_credentials'),
    ...
    'run_moor_oauth_login_pure': ('agent.anthropic_credentials', 'run_moor_oauth_login_pure'),
