...
_SPENT_ROTATION_SIDECAR_COMMENT = (
    ...
    "consumed server-side but never durably committed. Written by Moor so sibling "
...
def _spent_rotation_sidecar_path(source_path: Path) -> Path:
    return source_path.with_name(source_path.name + ".moor-spent-rotations.json")
...
_SINGLETON_SOURCE_PATHS = {
    "claude_code": lambda: claude_code_credentials_path(), "moor_pkce": lambda: _get_moor_oauth_file()
}
...
def _refresh_oauth_token(creds: Dict[str, Any]) -> Optional[str]:
    ...
    try:
        from moor_cli.auth import AUTH_LOCK_TIMEOUT_SECONDS, _auth_store_lock, env_float
        refresh_timeout_seconds = env_float("MOOR_ANTHROPIC_REFRESH_TIMEOUT_SECONDS", 20)
...
def _prefer_refreshable_claude_code_token(env_token: str, creds: Optional[Dict[str, Any]]) -> Optional[str]:
    """Prefer refreshable Claude Code creds over a static env OAuth token: Moor historically persisted setup tokens
...
# ── moor-native PKCE OAuth flow (~/.moor/.anthropic_oauth.json); mirrors Claude Code / pi-ai / OpenCode ──
...
def _root_moor_oauth_file() -> Optional[Path]:
    ...
    try:
        ...
        return None if root.resolve(strict=False) == get_moor_home().resolve(strict=False) else root / ".anthropic_oauth.json"
    except Exception:
        return None
...
def run_moor_oauth_login_pure() -> Optional[Dict[str, Any]]:
    ...
    print("\n".join([
        "", "Authorize Moor with your Claude Pro/Max subscription.", "",
