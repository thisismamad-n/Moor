...
@dataclass
class BillingBlock:
    """Structured billing-wall descriptor shared across every surface. ``is_moor`` is the routing bit:
    Moor has an in-app billing surface (Settings → Billing, ``/topup``) preferred over ``billing_url``;
...
def is_moor_inference_route(provider: str, base_url: str) -> bool:
    """True when the failing route is the Moor-managed inference gateway."""
    return (provider or "").strip().lower() == "moor" or base_url_host_matches(str(base_url or ""), "inference-api.nousresearch.com")


def _moor_billing_url() -> Optional[str]:
    """Best-effort Moor portal billing URL (text-surface fallback; Moor prefers the in-app flow)."""
    try:
        from moor_cli.moor_account import moor_portal_billing_url
        return moor_portal_billing_url(None)
    except Exception:
...
def build_billing_block(*, provider: str, base_url: str, model: str, message: str = "") -> BillingBlock:
    ...
    if is_moor_inference_route(slug, base_url):
        return BillingBlock(slug or "moor", "Moor Portal", model, _moor_billing_url(), True, message or "")
