...
``auth.json``.  They intentionally have no dependency on ``moor_cli.auth`` so
...
# Sources Moor owns and can intentionally persist in auth.json.  Everything
...
def sanitize_borrowed_credential_payload(
    ...
    Owned sources (manual entries and Moor-owned OAuth/device-code state)
