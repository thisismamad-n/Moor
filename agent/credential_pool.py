...
def _is_source_suppressed_fn() -> Callable[[str, str], bool]:
    """``moor_cli.auth.is_source_suppressed`` (late-bound), or an always-False stub."""
    try:
...
# singleton to re-seed from and the user can re-add via ``moor auth add``.
...
# selection. On Windows several Moor processes share one rotating log guarded by concurrent-log-handler's
...
# readiness handshake ("Timed out connecting to Moor backend after 15000ms"). Logging the condition at
...
# Moor singleton metadata mirrored between auth.json state and ``entry.extra``.
...
class PooledCredential:
    ...
    def runtime_api_key(self) -> str:
        if self.provider == "moor":
            # Moor stores the runtime inference credential in agent_key for
            ...
            for token, expires_at in (
                ...
                if (
                    ...
                    and token.strip()
                    and auth_mod._moor_invoke_jwt_is_usable(
...
def custom_provider_pool_key_candidates(
    ...
    ``moor auth add <key>`` stores ``providers.<key>`` credentials under the
...
def _guarded_global_root(global_path: Optional[Path]) -> Optional[Path]:
    ...
    never write the real user's ``~/.moor/auth.json`` even when MOOR_HOME
...
def _write_through_provider_state_to_global_root(
    ...
    Best-effort write-through for the multi-profile rotation hazard: moor,
    ...
    ``moor_cli.auth._write_through_xai_oauth_to_global_root``.
...
def _singleton_target_for_entry(pool: "CredentialPool", entry: "PooledCredential") -> Optional[Path]:
    """Root ``.anthropic_oauth.json`` when *entry* is a borrowed moor_pkce row, else None."""
    if entry.source != "moor_pkce" or entry.id not in getattr(pool, "_borrowed_root_ids", ()):
        return None
    try:
        from agent.anthropic_credentials import _root_moor_oauth_file
        return _root_moor_oauth_file()
    except Exception:
        return None
...
# ``moor_cli.auth`` refresh function and terminal-error predicate (looked
...
_REFRESH_TIMEOUT_ENV_VARS = {
    "openai-codex": "MOOR_CODEX_REFRESH_TIMEOUT_SECONDS",
    "xai-oauth": "MOOR_XAI_REFRESH_TIMEOUT_SECONDS",
}
...
class CredentialPool:
    ...
    def _sync_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        ...
        A fresh ``moor model`` / ``moor auth`` login writes new tokens
    ...
    def _sync_moor_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        """Sync a Moor device_code entry from auth.json ``providers.moor`` if state differs.

        Another process refreshing via ``resolve_moor_runtime_credentials``
        writes fresh tokens under ``_auth_store_lock``; adopting them avoids a
        "refresh token reuse" revocation on the Moor Portal.
        """
        if self.provider != "moor" or entry.source != "device_code":
            return entry
        try:
            with _auth_store_lock():
                state = _load_provider_state(_load_auth_store(), "moor")
            if not state:
                return entry
            comparable = {
                key: state.get(key)
            ...
            if not any(v not in (None, "") and getattr(entry, k, None) != v for k, v in comparable.items()):
                return entry
            logger.debug("Pool entry %s: syncing Moor state from auth.json", entry.id)
            ...
            extra_updates.update(
                {k: state[k] for k in _MOOR_EXTRA_STATE_KEYS if state.get(k) is not None}
            )
            return self._adopt(entry, extra=extra_updates, **field_updates)
        except Exception as exc:
            logger.debug("Failed to sync Moor entry from auth.json: %s", exc)
        return entry
    ...
    def _sync_device_code_entry_to_auth_store(self, entry: PooledCredential) -> None:
        ...
        token. Applies to Moor, OpenAI Codex and xAI OAuth singletons.
        ...
        if entry.source != "device_code" or self.provider not in ("moor", *_TOKENS_SINGLETON_PROVIDERS):
            return
        try:
            with _auth_store_lock():
    ...
    def _apply_entry_to_singleton_state(self, entry: PooledCredential, state: Dict[str, Any]) -> bool:
        ...
        if self.provider == "moor":
            ...
            for extra_key in _MOOR_EXTRA_STATE_KEYS:
    ...
    def _refresh_entry(self, entry: PooledCredential, *, force: bool) -> Optional[PooledCredential]:
        ...
        # across Moor processes, or two processes adopt the same on-disk
    ...
    def _fail_closed_unpersisted_rotation(
        ...
        *,
        ...
        For ``claude_code`` / ``moor_pkce`` the singleton file — not
    ...
    def _single_use_refresh_lock_timeout(self) -> float:
        ...
        env_var = _REFRESH_TIMEOUT_ENV_VARS.get(self.provider, "MOOR_ANTHROPIC_REFRESH_TIMEOUT_SECONDS")
    ...
    def _commit_anthropic_rotation(
        ...
        and other profiles see it). moor_pkce -> ~/.moor/.anthropic_oauth.json
        ...
        ``endswith``: manual:moor_pkce is pool-owned and a singleton for it
        would be a second authority for the same refresh-token family.
        """
        if entry.source == "claude_code":
            store = "~/.claude/.credentials.json"
        elif entry.source == "moor_pkce":
            store = "~/.moor/.anthropic_oauth.json"
        else:
            return
        try:
            ...
            if entry.source == "claude_code":
                ...
                ac._write_moor_oauth_credentials(*args, target=_singleton_target_for_entry(self, entry))
    ...
    def _refresh_anthropic(self, entry: PooledCredential) -> PooledCredential:
        ...
        refreshed = refresh_anthropic_oauth_pure(entry.refresh_token, use_json=entry.source.endswith("moor_pkce"))
        updated = replace(
            entry,
            ...
            if self.provider == "anthropic":
                ...
                if synced is not entry:
                    ...
                    if force and entry.runtime_api_key and entry.runtime_api_key != stale_key:
                        logger.debug("Moor entry %s: adopting peer-rotated token, skipping refresh", entry.id)
                        return entry
                auth_mod.resolve_moor_runtime_credentials(force_refresh=force, stale_access_token=stale_key or None)
                updated = self._sync_moor_entry_from_auth_store(entry)
            else:
                return entry
    ...
    def _recover_failed_refresh(self, entry: PooledCredential, exc: Exception) -> Optional[PooledCredential]:
        ...
        if self.provider == "anthropic":
            if entry.source == "claude_code":
                ...
                if synced.refresh_token != entry.refresh_token:
                    ...
                    try:
                        ...
                        refreshed = refresh_anthropic_oauth_pure(
                            synced.refresh_token, use_json=synced.source.endswith("moor_pkce"),
                        )
                ...
                # Backstop for pool-owned sources (moor_pkce, manual:dashboard_pkce):
            ...
            synced = self._sync_moor_entry_from_auth_store(entry)
            if synced.refresh_token != entry.refresh_token:
                logger.debug("Moor refresh failed but auth.json has newer tokens — adopting")
            ...
            if isinstance(exc, TimeoutError):
                ...
                # pool for ~120 sessions ("matched no moor entry ... pool size
            ...
            if auth_mod._is_terminal_moor_refresh_error(exc):
                logger.debug("Moor refresh token is terminally invalid; clearing local token state")
    ...
    def _clear_terminal_moor_state(self, entry: PooledCredential, exc: Exception) -> None:
        try:
            with _auth_store_lock():
                auth_store = _load_auth_store()
                state = _load_provider_state(auth_store, "moor") or {
                ...
                if not store_refresh or store_refresh == str(entry.refresh_token or "").strip():
                    auth_mod._quarantine_moor_oauth_state(state, exc, reason="credential_pool_refresh_failure")
                    auth_mod._quarantine_moor_pool_entries(auth_store, exc, reason="credential_pool_refresh_failure")
                    _save_provider_state(auth_store, "moor", state)
                    _save_auth_store(auth_store)
        except Exception as clear_exc:
            logger.debug("Failed to clear terminal Moor OAuth state: %s", clear_exc)
    ...
    def _entry_needs_refresh(self, entry: PooledCredential) -> bool:
        ...
        # Moor refresh can require network access and happens when runtime
    ...
    def _resync_stale_entry(self, entry: PooledCredential) -> PooledCredential:
        ...
        if self.provider == "moor":
            return self._sync_moor_entry_from_auth_store(entry)
                ...
                if _is_manual_source(entry.source):
                    ...
                    if dead_at and now - dead_at > DEAD_MANUAL_PRUNE_TTL_SECONDS:
                        logger.warning(
                            ...
                            "(reason=%s, age=%.1fh) — re-add via `moor auth add %s`",
...
class _Seeder:
    ...
    def upsert(self, source: str, payload: Dict[str, Any]) -> bool:
        """Upsert unless suppressed (``moor auth remove`` must stay stable across loads)."""
...
def _seed_anthropic_singletons(seed: _Seeder) -> None:
    # Only auto-discover external credentials (Claude Code, Moor PKCE) when
    ...
    # API-key vs OAuth is a user-visible choice at `moor setup`. The API-key
    ...
    # ~/.moor/.env over os.environ, as `_seed_from_env` does.
    _env_file = load_env()
    ...
    if _env_val("ANTHROPIC_API_KEY") and not anthropic_oauth_env:
        ...
        seed.changed |= _retain_sources_not_in(seed.entries, {"moor_pkce", "claude_code"})
        return
    ...
    for source_name, creds in (
        ("moor_pkce", read_moor_oauth_credentials()),
...
def _seed_moor_singleton(seed: _Seeder, auth_store: Dict[str, Any]) -> None:
    ...
    if not (state and has_runtime_material):
        return
    # Prefer a user-supplied label embedded in the singleton state (``moor
    # auth add moor --label <name>``) over the token-derived fingerprint.
    ...
    seed.upsert("device_code", {
        ...
        # ones (#15099).
        **{key: state.get(key) for key in _MOOR_EXTRA_STATE_KEYS},
...
def _seed_qwen_singleton(seed: _Seeder) -> None:
    ...
    try:
        from moor_cli.auth import resolve_qwen_runtime_credentials
...
def _seed_tokens_singleton(seed: _Seeder, auth_store: Dict[str, Any]) -> None:
    ...
    Moor owns its own Codex auth state and does NOT auto-import
    ...
    explicit one-time prompt via `moor auth openai-codex`.
    """
...
def _seed_from_singletons(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    if provider == "anthropic":
        ...
        _seed_moor_singleton(seed, auth_store)
        ...
        # `moor auth remove openai-codex` suppresses device_code; without
...
def get_env_prefer_dotenv(key: str) -> str:
    """Resolve a credential env var, preferring ~/.moor/.env over os.environ.
...
def _warn_env_ingestion_once(provider: str, env_var: str) -> None:
    ...
    logger.warning(
        ...
        "moor auth remove %s <n> to suppress.",
        env_var,
        provider,
...
def _prune_stale_seeded_entries(
    ...
    *,
    ...
    def _is_prunable(entry: PooledCredential) -> bool:
        ...
        # requested (an `moor auth` command that confirmed the source is gone).
        if entry.source.startswith("env:"):
            return prune_env_sources
        # File-backed singletons and Moor PKCE disappear when their backing file is gone.
        return is_borrowed_credential_source(entry.source, entry.provider) or entry.source == "moor_pkce"
