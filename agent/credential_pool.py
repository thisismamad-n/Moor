...
def _write_through_provider_state_to_global_root(
    ...
    (#48415 / #43589): moor, openai-codex, and xai-oauth rotate the
    ...
    ``moor_cli.auth._write_through_xai_oauth_to_global_root`` (which covers
...
class CredentialPool:
    ...
    def _sync_codex_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        ...
        Meanwhile the user may run ``moor model`` / ``moor auth`` which
        ...
        Mirrors the Moor/Anthropic resync paths above.  Only applies to
    ...
    def _sync_moor_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        """Sync a Moor pool entry from auth.json if tokens differ.

        Moor OAuth refresh tokens are single-use.  When another process
        (e.g. a concurrent cron) refreshes the token via
        ``resolve_moor_runtime_credentials``, it writes fresh tokens to
        auth.json under ``_auth_store_lock``.  The pool entry's tokens
        become stale.  This method detects that and adopts the newer pair,
        avoiding a "refresh token reuse" revocation on the Moor Portal.
        """
        ...
        try:
            ...
            if should_sync:
                logger.debug(
                    "Pool entry %s: syncing Moor state from auth.json",
                    entry.id,
                )
                field_updates: Dict[str, Any] = {
                    "last_status": None,
    ...
    def _sync_device_code_entry_to_auth_store(self, entry: PooledCredential) -> None:
        ...
        (currently Moor, OpenAI Codex, and xAI Grok OAuth).
        ...
        here would mean every Moor/Codex/xAI refresh in a multi-provider
        setup silently flips the ``active_provider`` flag — the next
        ``moor`` invocation that defaults to the active provider
        (e.g. setup wizard, ``moor auth status``) would land on
        ...
        # device-code sources (moor, openai-codex, xAI) use ``device_code``.
    ...
    def _single_use_refresh_lock_timeout(self) -> float:
        ...
        resolves.  Reads the provider's ``MOOR_*_REFRESH_TIMEOUT_SECONDS``
        override.
        """
        env_var = (
            "MOOR_CODEX_REFRESH_TIMEOUT_SECONDS"
            if self.provider == "openai-codex"
            else "MOOR_XAI_REFRESH_TIMEOUT_SECONDS"
        )
        ...
        try:
            if self.provider == "anthropic":
                ...
                refreshed = refresh_anthropic_oauth_pure(
                    entry.refresh_token,
                    use_json=entry.source.endswith("moor_pkce"),
                )
                updated = replace(
                    entry,
                ...
                if synced.refresh_token != entry.refresh_token:
                    ...
                    try:
                        ...
                        refreshed = refresh_anthropic_oauth_pure(
                            ...
                            use_json=synced.source.endswith("moor_pkce"),
                        )
                        updated = replace(
                            synced,
                            access_token=refreshed["access_token"],
                    ...
                    return synced
            # For xai-oauth: same race as moor — another process may have
            ...
            if self.provider == "xai-oauth":
                ...
                # pool. Mirrors the Moor quarantine path above.
                if auth_mod._is_terminal_xai_oauth_refresh_error(exc):
                    ...
                    return None
            # For openai-codex: same race as xAI/moor — another Moor process
            ...
            if self.provider == "openai-codex":
                ...
                # in-memory pool.  Mirrors the xAI and Moor quarantine paths.
                if auth_mod._is_terminal_codex_oauth_refresh_error(exc):
                    ...
                    with self._lock:
                        ...
                        self._persist(removed_ids=removed_ids)
                    return None
            # For moor: another process may have consumed the refresh token
                ...
                if auth_mod._is_terminal_moor_refresh_error(exc):
                    ...
                    try:
                        with _auth_store_lock():
                            ...
                            entry_refresh = str(entry.refresh_token or "").strip()
                            if not store_refresh or store_refresh == entry_refresh:
                                auth_mod._quarantine_moor_oauth_state(
                                    state,
                                    exc,
                                    reason="credential_pool_refresh_failure",
                                )
                                auth_mod._quarantine_moor_pool_entries(
                                    auth_store,
                                    exc,
                                    reason="credential_pool_refresh_failure",
                                )
                    ...
                    singleton_sources = {
                        auth_mod.MOOR_DEVICE_CODE_SOURCE,
                        f"manual:{auth_mod.MOOR_DEVICE_CODE_SOURCE}",
                    }
    ...
    def _entry_needs_refresh(self, entry: PooledCredential) -> bool:
        ...
        if self.provider == "moor":
            # Moor refresh can require network access and should happen when
        ...
        for entry in self._entries:
            ...
            if (self.provider == "anthropic" and entry.source == "claude_code"
                ...
                if synced is not entry:
                    entry = synced
                    cleared_any = True
            # For moor entries, sync from auth.json before status checks.
            # Another process may have successfully refreshed via
            # resolve_moor_runtime_credentials(), making this entry's
            ...
            # re-authed via `moor model` / `moor auth` after a 429/401,
            ...
            # tokens that another process (or a fresh `moor model` ->
            ...
            if entry.last_status == STATUS_DEAD:
                ...
                # user can always re-add via ``moor auth add``.  Singleton-
...
def _seed_from_singletons(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    # `moor auth remove <provider> <N>` is stable across all source types.
    try:
        from moor_cli.auth import is_source_suppressed as _is_suppressed
    except ImportError:
    ...
    if provider == "anthropic":
        ...
        # API-key vs OAuth is a user-visible choice at `moor setup` ("Claude
        ...
        # Claude Code CLI, moor_pkce creds from a previous OAuth login)
        ...
        # that masquerade.  Prefer ~/.moor/.env over os.environ for the
        # same reason `_seed_from_env` does — that's the authoritative file
        # that `moor setup` writes.
        _env_file = load_env()
        ...
        if api_key_path_explicit:
            ...
            retained = [
                ...
                if entry.source not in {"moor_pkce", "claude_code"}
            ]
        ...
        from agent.anthropic_adapter import read_claude_code_credentials, read_moor_oauth_credentials
        ...
        if state and has_runtime_material and not _is_suppressed(provider, "device_code"):
            ...
            # (set by persist_moor_credentials(label=...) when the user ran
            # `moor auth add moor --label <name>`).  Fall back to the
        ...
        try:
            ...
            # (moor auth remove copilot gh_cli) must not pay either on
        ...
        # MiniMax OAuth tokens live in ~/.moor/auth.json providers.minimax-oauth.
        # Seed the pool so `/auth list` reflects the logged-in state and the
        # standard `moor auth remove minimax-oauth <N>` flow works.
        ...
        # Respect user suppression — `moor auth remove openai-codex` marks
        ...
        # When the user logs in via ``moor model`` -> xAI Grok OAuth,
        # tokens are written to the auth.json singleton
        # (``providers["xai-oauth"]``).  Surface them in the pool too so
        # ``moor auth list`` reflects the logged-in state and so the pool
        ...
        if isinstance(tokens, dict) and tokens.get("access_token"):
            ...
            # always surfaced as ``device_code`` (consistent with moor/codex).
            source = "device_code"
            if _is_suppressed(provider, source):
                return changed, active_sources
            active_sources.add(source)
            from moor_cli.auth import DEFAULT_XAI_OAUTH_BASE_URL
...
# Prefer ~/.moor/.env over os.environ — the user's config file is the
...
def get_env_prefer_dotenv(key: str) -> str:
    ...
    # load_moor_dotenv() -> apply_onepassword_secrets()).  The raw
...
def _seed_from_env(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    # Honour user suppression — `moor auth remove <provider> <N>` for an
    # env-seeded credential marks the env:<VAR> source as suppressed so it
    # won't be re-seeded from the user's shell environment or ~/.moor/.env.
    ...
    try:
        from moor_cli.auth import is_source_suppressed as _is_source_suppressed
    except ImportError:
    ...
    if provider == "openrouter":
        # Prefer ~/.moor/.env over os.environ
...
def _prune_stale_seeded_entries(
    ...
    *,
    ...
    def _is_prunable(entry: PooledCredential) -> bool:
        ...
        # (e.g. an `moor auth` command that confirmed the source is gone).
