...
Text auto chain: main provider+model → OpenRouter → Moor Portal → custom endpoint →
native Anthropic → direct API-key providers → None. Vision auto chain: main
provider (if a supported vision backend) → OpenRouter → Moor → Anthropic → custom.
...
def _resolve_aux_verify(base_url: Optional[str]) -> Any:
    ...
    ``ssl_verify``, ``MOOR_CA_BUNDLE`` / ``SSL_CERT_FILE``); any failure → httpx default (``True``)."""
    try:
        from agent.ssl_verify import resolve_httpx_verify
        from moor_cli.config import get_custom_provider_tls_settings, load_config_readonly
    ...
    try:
        ...
        if not _WARNED_KEEPALIVE_IMPORT_SKEW:
            ...
            logger.warning(
                ...
                "back to the SDK default HTTP client. Run `moor update` (or "
...
def _create_openai_client(*, api_key: str, base_url: str, **kwargs: Any) -> Any:
    ...
    # Moor owns aux retry/fallback policy; the SDK default (max_retries=2) would triple
    # wall time on a hung endpoint before Moor sees one failure.
    # Moor owns auxiliary retry + provider/model fallback policy (the same-provider transient retry in
    ...
    # so a 120s timeout can stall ~360s before Moor sees a single failure (issue #54465). Disable
    # SDK-internal retries by default and let Moor control the budget; explicit callers can still override
    # via kwargs.
...
def _run_protected_sync_provider_call(callback: Callable[[dict[str, Any]], Any], kwargs: dict[str, Any]) -> Any:
    ...
    threading.Thread(
        target=provider_context.run, args=(_provider_worker,), name="moor-protected-aux-provider",
        daemon=True).start()
    while True:
...
def _client_declares(client_obj: Any, flag: str) -> bool:
    ...
    not isinstance, so out-of-tree clients can opt out of wrappers unimported (cf. SUPPORTS_MOOR_TOOL_CALLS)."""
    try:
...
def _fast_model_from_catalog(provider_id: str) -> str:
    ...
    """
    is_moor = provider_id.strip().lower() == "moor"
    try:
        from moor_cli.auth import resolve_api_key_provider_credentials
        from moor_cli.models_pricing import fetch_models_with_pricing
        ...
        if not api_key and is_moor:
            # Moor is OAuth (resolver raises); anonymous reads return the full catalog.
            try:
                from moor_cli.models_pricing import _resolve_moor_pricing_credentials
                api_key, base_url = _resolve_moor_pricing_credentials()
            except Exception:
                logger.debug("No Moor credentials for catalog", exc_info=True)
        if not base_url:
        ...
        if base_url.endswith("/v1"):  # fetch_models_with_pricing appends /v1/models
            base_url = base_url[:-3]
        # Moor-only args must match the pickers' or the seeded cache loses sale chrome and
        # policy-catalog expiry.
        _moor_kwargs = {}
        if is_moor:
            from moor_cli.models_pricing import _MOOR_CATALOG_TTL_SECONDS
            _moor_kwargs = {"include_sale_original": True, "cache_ttl_seconds": _MOOR_CATALOG_TTL_SECONDS}
        catalog = fetch_models_with_pricing(
            api_key=api_key or None, base_url=base_url, timeout=3.0, **_moor_kwargs) or {}
    except Exception:
    ...
    if is_moor:
        ...
        try:
            from moor_cli.models_pricing import moor_policy_allowed_ids, restrict_to_moor_policy
            ids = restrict_to_moor_policy(ids, moor_policy_allowed_ids())
        except Exception:
            logger.debug("Moor policy filter unavailable", exc_info=True)
...
def _get_aux_model_for_provider(provider_id: str, *, prefer_fast: bool = False) -> str:
    ...
    if picked and provider_id.strip().lower() == "moor":
        try:
            from moor_cli.models_pricing import moor_policy_allowed_ids, restrict_to_moor_policy
            allowed = moor_policy_allowed_ids()
            if allowed and not restrict_to_moor_policy([picked], allowed):
                return ""
        except Exception:
            logger.debug("Moor policy check unavailable", exc_info=True)
    return picked
...
def build_or_headers(or_config: dict | None = None) -> dict:
    ...
    Precedence env > config > default: ``MOOR_OPENROUTER_CACHE`` overrides
    ``openrouter.response_cache``; ``MOOR_OPENROUTER_CACHE_TTL`` (1-86400 s) overrides
    ...
    if or_config is None:
        try:
            ...
            or_config = {}
    env_cache = os.environ.get("MOOR_OPENROUTER_CACHE", "").strip().lower()
    if not (env_cache in {"1", "true", "yes", "on"} if env_cache else or_config.get("response_cache", False)):
        return headers
    headers["X-OpenRouter-Cache"] = "true"
    env_ttl = os.environ.get("MOOR_OPENROUTER_CACHE_TTL", "").strip()
    if env_ttl:
...
_NVIDIA_NIM_CLOUD_HEADERS = {"X-BILLING-INVOKE-ORIGIN": "MoorAgent"}
...
# tracks moor_cli.__version__ — never inline a literal here.
from agent.portal_tags import moor_portal_tags as _moor_portal_tags


def _moor_extra_body() -> dict:
    """Fresh Moor Portal ``extra_body`` (per call, so a hot-reloaded version is reflected)."""
    return {"tags": _moor_portal_tags()}


# Set at resolve time — True if the auxiliary client points to Moor Portal
auxiliary_is_moor: bool = False
...
def _pool_runtime_api_key(entry: Any) -> str:
    # runtime_api_key handles provider-specific fallback (e.g. agent_key for moor); None entry → "".
...
def _pool_runtime_base_url(entry: Any, fallback: str = "") -> str:
    ...
    if getattr(entry, "provider", None) == "moor":
        # Canonical auth-layer reader so the env override shares one normalization path.
        from moor_cli.auth import _moor_inference_env_override
        env_url = _moor_inference_env_override()
        if env_url:
            return env_url
...
def _moor_min_key_ttl_seconds() -> int:
    try:
        return max(60, int(os.getenv("MOOR_MOOR_MIN_KEY_TTL_SECONDS", "1800")))
...
class _AnthropicCompletionsAdapter:
    ...
    def __init__(self, real_client: Any, model: str, is_oauth: bool = False, base_url: str | None = None):
        ...
        # Caller URL first; fall back to the SDK client's host only for Moor Portal — a blanket
        ...
        if not self._base_url:
            ...
            if candidate:
                with contextlib.suppress(Exception):
                    from agent.anthropic_endpoints import _is_moor_portal_endpoint
                    if _is_moor_portal_endpoint(candidate):
    ...
    def create(self, **kwargs) -> Any:
        ...
        # (already TRANSLATED to native fields — raw would 400 on strict gateways) and ``_`` Moor plumbing.
...
def _endpoint_speaks_anthropic_messages(base_url: str) -> bool:
    ...
    Mirrors ``moor_cli.runtime_provider._detect_api_mode_for_url`` so aux and main agree: any
...
def _maybe_wrap_anthropic(
    ...
    """
    # Anthropic/Bedrock/Codex wrappers, plus any client declaring MOOR_SKIP_TRANSPORT_WRAP
    ...
    if (
        ...
        or _client_declares(client_obj, "MOOR_SKIP_TRANSPORT_WRAP")
    ):
        return client_obj
...
def _read_moor_auth() -> Optional[dict]:
    """Moor provider state dict from the credential pool or ~/.moor/auth.json; None when not active with tokens."""
    pool_present, entry = _select_pool_entry("moor")
    if pool_present:
        if entry is None:
            return None
        return {
            ...
            "inference_base_url": _pool_runtime_base_url(entry, _MOOR_DEFAULT_BASE_URL),
    ...
    try:
        ...
        logger.debug("Could not read Moor auth: %s", exc)
        return None


def _moor_api_key(provider: dict) -> str:
    """Extract a usable Moor inference JWT from stored auth state."""
    from moor_cli.auth import _moor_invoke_jwt_is_usable
    for token_key, expiry_key in (("agent_key", "agent_key_expires_at"), ("access_token", "expires_at")):
        token = provider.get(token_key)
        if not isinstance(token, str) or not token.strip():
            continue
        if _moor_invoke_jwt_is_usable(token, scope=provider.get("scope"), expires_at=provider.get(expiry_key)):
            return token
    return ""


def _resolve_moor_pool_runtime_api(*, force_refresh: bool = False) -> Optional[tuple[str, str]]:
    """Resolve Moor auxiliary credentials from the selected pool entry."""
    try:
        from moor_cli.auth import _agent_key_is_usable
        pool = load_pool("moor")
    except Exception as exc:
        logger.debug("Auxiliary Moor pool credential resolution failed: %s", exc)
        return None
    ...
    try:
        ...
        logger.debug("Auxiliary Moor pool selection failed: %s", exc)
        return None
    if entry is None:
        return None
    ...
    if force_refresh or not _agent_key_is_usable(_entry_state(entry), _moor_min_key_ttl_seconds()):
        try:
            refreshed = pool.try_refresh_current()
        except Exception as exc:
            logger.debug("Auxiliary Moor pool refresh failed: %s", exc)
            refreshed = None
        if refreshed is None:
            return None
        entry = refreshed
    api_key = _moor_api_key(_entry_state(entry))
    base_url = _pool_runtime_base_url(entry, _MOOR_DEFAULT_BASE_URL)
    if not api_key or not base_url:
        return None
    return api_key, base_url


def _resolve_moor_runtime_api(
    *, force_refresh: bool = False, stale_access_token: Optional[str] = None
) -> Optional[tuple[str, str]]:
    """Fresh Moor runtime credentials (pool first, then auth store + JWT refresh) — mirrors the main
    ...
    pooled = _resolve_moor_pool_runtime_api(force_refresh=force_refresh)
    if pooled is not None:
        return pooled
    try:
        from moor_cli.auth import resolve_moor_runtime_credentials
        creds = resolve_moor_runtime_credentials(
            timeout_seconds=env_float("MOOR_MOOR_TIMEOUT_SECONDS", 15),
            force_refresh=force_refresh,
            stale_access_token=stale_access_token or None,
        )
    except Exception as exc:
        logger.debug("Auxiliary Moor runtime credential resolution failed: %s", exc)
        return None
...
def _resolve_xai_oauth_for_aux() -> Optional[Tuple[str, str]]:
    ...
    """
    try:
        from moor_cli.auth import DEFAULT_XAI_OAUTH_BASE_URL, _xai_validate_inference_base_url
        ...
        if pool and pool.has_credentials():
            ...
            if entry is not None:
                ...
                base_url = _xai_validate_inference_base_url(
                    _url(os.getenv("MOOR_XAI_BASE_URL", ""))
    ...
    try:
        from moor_cli.auth import resolve_xai_oauth_runtime_credentials
...
def _read_codex_access_token() -> Optional[str]:
    ...
    if pool_present:
        ...
        if token:
            return token
    try:
        from moor_cli.auth import _read_codex_tokens
...
def _resolve_api_key_provider() -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    for provider_id, pconfig in PROVIDER_REGISTRY.items():
        ...
        if base_url_host_matches(base_url, "api.kimi.com"):
            ...
            from moor_cli.models import copilot_default_headers
...
def _endpoint_default_headers(
    ...
    if base_url_host_matches(base_url, "api.kimi.com"):
        ...
        from moor_cli.copilot_auth import copilot_request_headers
        ...
        from tools.xai_http import moor_xai_default_headers
        headers = dict(moor_xai_default_headers())
    else:
...
def _try_moor(vision: bool = False) -> Tuple[Optional[OpenAI], Optional[str]]:
    # Cross-session rate guard: another session's 429 means skip Moor rather than pile onto the tapped RPH bucket.
    with contextlib.suppress(Exception):
        from agent.moor_rate_guard import moor_rate_limit_remaining
        _remaining = moor_rate_limit_remaining()
        if _remaining is not None and _remaining > 0:
            logger.debug("Auxiliary: skipping Moor Portal (rate-limited, resets in %.0fs)", _remaining)
            _mark_provider_unhealthy("moor", ttl=_remaining)
            return None, None
    ...
    if not _aux_probe_active():
        try:
            ...
            recommended = get_moor_recommended_aux_model(vision=vision)
            if recommended:
                model = recommended
    ...
    if runtime is not None:
        ...
        if not api_key:
            ...
            return None, None
        base_url = str(
            (moor or {}).get("inference_base_url") or os.getenv("MOOR_INFERENCE_BASE_URL", _MOOR_DEFAULT_BASE_URL)
        ).rstrip("/")
    return _create_openai_client(api_key=api_key, base_url=base_url), model


def _refresh_moor_recommended_model(*, vision: bool, stale_model: Optional[str]) -> Optional[str]:
    """Fresh Portal recommended model after a stale-model 404 (long-lived processes pin dropped models).

    Returns the fresh recommendation, else ``_MOOR_MODEL``, whichever differs from ``stale_model``; None if neither.
    """
    stale = (stale_model or "").strip().lower()
    fresh: Optional[str] = None
    try:
        from moor_cli.models import get_moor_recommended_aux_model
        fresh = get_moor_recommended_aux_model(vision=vision, force_refresh=True)
    except Exception as exc:
        logger.debug("Moor recommended-model refresh failed (%s); using default %s", exc, _MOOR_MODEL)
    if fresh and fresh.strip().lower() != stale:
        return fresh
    return _MOOR_MODEL if _MOOR_MODEL.strip().lower() != stale else None
...
def _read_main_field(field: str, *, readonly: bool, lower: bool = False) -> str:
    ...
    with contextlib.suppress(Exception):
        from moor_cli import config as _cfg_mod
...
def _validate_base_url(base_url: str) -> None:
    ...
    try:
        ...
        raise RuntimeError(
            ...
            "Run `moor setup` or `moor model` and enter a valid http(s) base URL."
        ) from exc
...
def _build_xai_oauth_aux_client(model: str) -> Tuple[Optional[Any], Optional[str]]:
    ...
    real_client = _create_openai_client(
        api_key=api_key, base_url=base_url, default_headers=moor_xai_default_headers()
    )
    return CodexAuxiliaryClient(real_client, model), model
...
def _get_provider_chain() -> List[tuple]:
    ...
    """
    return [
        ("openrouter", _try_openrouter), ("moor", _try_moor),
...
_AUX_UNHEALTHY_LABEL_ALIASES = {
    "openrouter": "openrouter", "moor": "moor", "custom": "local/custom",
...
def _moor_portal_account_has_fresh_paid_access() -> bool:
    """Return True only when the fresh Moor account API says paid access is allowed."""
    try:
        from moor_cli.moor_account import get_moor_portal_account_info
        return get_moor_portal_account_info(force_fresh=True).paid_service_access is True
    except Exception as exc:
        logger.debug("Auxiliary Moor paid-entitlement refresh check failed: %s", exc)
        return False
...
_POOL_PROVIDER_BY_HOST = (
    ...
    ("inference-api.nousresearch.com", "moor"), ("api.anthropic.com", "anthropic"),
...
def _refresh_copilot_credentials() -> bool:
    from moor_cli.copilot_auth import _jwt_cache, _token_fingerprint, exchange_copilot_token, resolve_copilot_token
...
def _refresh_moor_credentials() -> bool:
    from moor_cli.auth import resolve_moor_runtime_credentials
    return _creds_have_api_key(resolve_moor_runtime_credentials(
        timeout_seconds=env_float("MOOR_MOOR_TIMEOUT_SECONDS", 15), force_refresh=True
    ))
...
_CREDENTIAL_REFRESHERS: Dict[str, Callable[[], bool]] = {
    ...
    "moor": _refresh_moor_credentials, "anthropic": _refresh_anthropic_credentials,
...
def _fallback_destination(
    ...
    attached = getattr(fb_client, "_moor_fallback_destination", None)
...
def _replan_synchromoor_cache_sections(
...
def _fallback_request_kwargs(
    ...
    fallback_messages, fallback_tools = _replan_synchromoor_cache_sections(messages, tools, destination=destination)
...
def _resolve_fallback_entry(entry: Dict[str, Any]) -> Tuple[Optional[Any], Optional[str]]:
    ...
    if client is not None:
        with contextlib.suppress(Exception):
            client._moor_fallback_destination = _fallback_destination_from_entry(entry, client, resolved_model)
...
def _warn_stale_openai_base_url(runtime_provider: str) -> None:
    ...
    ~/.moor/.env value after `moor model` poisons routing)."""
    ...
    if (_env_base and _cfg_provider and _cfg_provider != "custom" and not _cfg_provider.startswith("custom:")):
        logger.warning(
            ...
            "Run: moor model to reconfigure, or remove "
            "OPENAI_BASE_URL from ~/.moor/.env",
...
def _resolve_auto_route(
    ...
    top-level chain; (3) OpenRouter → Moor → custom → Codex → API-key providers, only with no policy
    and no working main client."""
    global auxiliary_is_moor
    auxiliary_is_moor = False  # Reset — _try_moor() will set True if it wins
    runtime = _normalize_main_runtime(main_runtime)
...
def _effective_provider_for_client(client: Any, fallback: str) -> str:
    ...
    effective_provider = getattr(client, "_moor_aux_effective_provider", "")
...
def _to_async_client(sync_client, model: str, is_vision: bool = False):
    ...
    if _client_declares(sync_client, "MOOR_SKIP_ASYNC_WRAP"):
        return sync_client, model
    ...
    # Moor owns the auxiliary retry/timeout budget; disable SDK-internal retries.
    # See #54465.
...
def _resolve_auto_branch(req: _ResolveRequest) -> _ResolveResult:
    ...
    if routed_client is not None and effective_provider:
        try:
            setattr(routed_client, "_moor_aux_effective_provider", effective_provider)
...
def _resolve_moor_branch(req: _ResolveRequest) -> _ResolveResult:
    """Moor Portal (OAuth)."""
    model = req.model
    # Vision: caller flag, _PROVIDER_VISION_MODELS override, or a known vision id.
    client, default = _try_moor(vision=(req.is_vision or model in _PROVIDER_VISION_MODELS.values()
                                        or (model or "").strip().lower() == "mimo-v2-omni"))
    if client is None:
        logger.warning("resolve_provider_client: moor requested but Moor Portal not configured (run: moor auth)")
        return None, None
    ...
    client = _maybe_wrap_anthropic(
        ...
        str(getattr(client, "base_url", "") or ""), moor_api_mode(final_model),
    )
...
def _resolve_openai_codex_branch(req: _ResolveRequest) -> _ResolveResult:
    ...
    if not model:
        ...
        return None, None
    no_token_msg = "resolve_provider_client: openai-codex requested but no Codex OAuth token found (run: moor model)"
    if req.raw_codex:
...
def _resolve_xai_oauth_branch(req: _ResolveRequest) -> _ResolveResult:
    ...
    return _route_or_warn(req, client, default,
                          ...
                          "OAuth token found (run: moor model -> xAI Grok OAuth — SuperGrok / Premium+)")
...
def _resolve_named_custom_branch(req: _ResolveRequest) -> Optional[_ResolveResult]:
    ...
    # canonical name (e.g. ``moor``) still defer to the built-in.
    custom_entry = None
...
def _resolve_azure_foundry_branch(req: _ResolveRequest) -> _ResolveResult:
    ...
    return _route_or_warn(req, client, default_model,
                          ...
                          "runtime resolution failed (run: moor doctor for diagnostics)")
...
def _resolve_api_key_branch(req: _ResolveRequest, pconfig: Any, resolve_creds: Callable) -> _ResolveResult:
    ...
    try:
        from moor_cli.models import opencode_zen_free_runtime as _oc_free_rt
    ...
    if provider == "copilot" and final_model and not req.raw_codex:
        with contextlib.suppress(ImportError):
            from moor_cli.models import _should_use_copilot_responses_api
...
def _resolve_registry_branch(req: _ResolveRequest) -> _ResolveResult:
    ...
    try:
        ...
        logger.debug("moor_cli.auth not available for provider %s", provider)
        return None, None
    ...
    if auth_type == "vertex":
        ...
        # moor / openai-codex / xai-oauth already returned from their explicit branches.
...
def resolve_provider_client(
    ...
    # Excluded: ``auto`` (a stale main slug could pair with any picked provider) and Moor + vision (the
    # Portal's tier-aware vision recommendation must win over a text-only model).
    if not model and provider != "auto" and not (provider == "moor" and is_vision):
        ...
        # not explicitly request one. (# compression-current-model) Moor + vision is the one carve-out: the
        # branch below resolves its model from the Portal's tier-aware vision recommendation
        # (``_try_moor(vision= True)``), and ``final_model = model or default`` means anything pre-filled
...
_VISION_AUTO_PROVIDER_ORDER = ("openrouter", "moor", "deepinfra")
...
# Strict (explicitly requested) vision backends by normalized provider name. moor MUST go
# through resolve_provider_client so anthropic/* picks wrap onto /v1/messages (a bare _try_moor
...
_STRICT_VISION_BACKENDS: Dict[str, Callable[[Optional[str]], Tuple[Optional[Any], Optional[str]]]] = {
    ...
    "moor": lambda model: resolve_provider_client("moor", model, is_vision=True),
...
def get_available_vision_backends() -> List[str]:
    """Available vision backends in auto-selection order (active provider → OpenRouter → Moor → DeepInfra).
...
def _vision_main_provider_client(
    ...
    if main_provider == "moor":
        # Moor picks its vision model from Portal tier-aware slots inside _try_moor(vision=True);
...
def _vision_auto_route(
    ...
    """Auto-detect order: 1. main provider + model, 2. OpenRouter, 3. Moor Portal, 4. DeepInfra, 5. stop."""
...
def get_auxiliary_extra_body() -> dict:
    """Return extra_body kwargs (Moor Portal product tags when Moor-backed, else {})."""
    return _moor_extra_body() if auxiliary_is_moor else {}
...
def auxiliary_max_tokens_param(value: int, *, model: Optional[str] = None) -> dict:
    ...
    direct_openai_family = (
        not _scoped_key_env("OPENROUTER_API_KEY") and _read_moor_auth() is None
...
def _refresh_moor_auxiliary_client(
    *, cache_provider: str, model: Optional[str], async_mode: bool, base_url: Optional[str] = None,
    ...
    """Refresh Moor runtime creds, rebuild the client, and replace the cache entry.
    ...
    """
    runtime = _resolve_moor_runtime_api(force_refresh=True, stale_access_token=api_key)
    if runtime is None:
        return None, model
...
def _preserve_provider_with_base_url(prov: Optional[str]) -> bool:
    ...
    try:
        ...
        return normalized in {
            "anthropic", "copilot", "copilot-acp", "minimax-oauth", "moor", "openai-codex", "qwen-oauth", "xai-oauth",
        }
...
def _moor_on_messages_wire(provider_norm: str, model: str) -> bool:
    """True when a Moor Portal route serves ``model`` over /v1/messages (dual-wire catalog)."""
    if provider_norm not in _MOOR_PROVIDER_NAMES:
        return False
    from moor_cli.providers import moor_api_mode
    return moor_api_mode(model) == "anthropic_messages"
...
def _forwards_max_tokens(provider: str, provider_norm: str, model: str, effective_base: str, task: Optional[str]) -> bool:
    ...
    return (
        ...
        or _moor_on_messages_wire(provider_norm, model)
...
def _merge_aux_extra_body(
    ...
    """Caller extra_body + profile body/reasoning + generic reasoning fallback + Moor tags."""
    ...
    if provider_norm in _MOOR_PROVIDER_NAMES:
        if "tags" not in merged_extra:
            merged_extra["tags"] = _moor_portal_tags()
...
def _build_call_kwargs(
    ...
    if reasoning_config and isinstance(reasoning_config, dict):
        ...
        if (
            provider_norm == "anthropic" or _moor_on_messages_wire(provider_norm, model)
...
def _is_managed_local_endpoint(base_url: Optional[str]) -> bool:
    """True when *base_url* targets the llama-server this Moor manages."""
    if not base_url:
        return False
            ...
            if _explicit and _explicit not in {"auto", "openrouter", "custom"}:
                ...
                if fb_client is None:
                    raise RuntimeError(
                        ...
                        f"a different provider with `moor model`.")
...
def _refreshed_moor_step(route: _LadderRoute, kwargs: Dict[str, Any], message: str) -> Optional[_LadderStep]:
    """Rebuild the Moor client after a credential event; None when nothing refreshed."""
    refreshed_client, refreshed_model = _refresh_moor_auxiliary_client(
        cache_provider=route.resolved_provider or "moor", model=route.final_model,
...
def _ladder_moor_rungs(
    first_err: Exception, route: _LadderRoute, kwargs: Dict[str, Any], client_is_moor: bool,
):
    """Moor-only rungs: stale-model self-heal, paid-account refresh, 401 refresh.
    ...
    if _is_model_not_found_error(first_err) and client_is_moor:
        healed_model = _refresh_moor_recommended_model(
            vision=(task == "vision"), stale_model=kwargs.get("model"))
        if healed_model and healed_model != kwargs.get("model"):
            logger.warning("Auxiliary %s%s: model %r no longer in Moor catalog; "
    ...
    if _is_payment_error(first_err) and client_is_moor and _moor_portal_account_has_fresh_paid_access():
        step = _refreshed_moor_step(
            route, kwargs,
            "Auxiliary %s%s: refreshed Moor runtime credentials after paid account check, retrying")
        if step is not None:
            resp, first_err = yield from _rung(
    ...
    if _is_auth_error(first_err) and client_is_moor:
        step = _refreshed_moor_step(
            route, kwargs, "Auxiliary %s%s: refreshed Moor runtime credentials after 401, retrying")
        if step is not None:
...
def _aux_recovery_ladder(
    ...
    strips → Moor heal/refresh → credential refresh/pool rotation → provider fallback.
    ...
    if first_err is None:
        return resp
    client_is_moor = (resolved_provider == "moor"
                      or base_url_host_matches(base_info, "inference-api.nousresearch.com"))
    resp, first_err = yield from _ladder_moor_rungs(first_err, route, kwargs, client_is_moor)
    if first_err is None:
        return resp
    resp, first_err = yield from _ladder_credential_rungs(first_err, route, kwargs, client_is_moor)
    if first_err is None:
        return resp
...
MOOR_EXTRA_BODY = _moor_extra_body()
