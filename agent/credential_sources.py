...
``moor auth remove <provider> <N>`` must make the entry stay gone across
...
def _remove_env_source(provider: str, removed) -> RemovalResult:
    """env:<VAR> — clear from ~/.moor/.env; hint when the shell exports it."""
    ...
    # .env as utf-8-sig like moor_cli/config.py: a BOM-sensitive read would
...
def _remove_auth_store_oauth(provider: str, removed) -> RemovalResult:
    """Clear auth.json ``providers.<provider>`` (moor, minimax-oauth, xai-oauth, openai-codex).
