...
Moor seeds its credential pool from many places:
    ...
    env:<VAR>     — os.environ / ~/.moor/.env
    claude_code   — ~/.claude/.credentials.json
    moor_pkce   — ~/.moor/.anthropic_oauth.json
    device_code   — auth.json providers.<provider> (moor, openai-codex, ...)
    qwen-cli      — ~/.qwen/oauth_creds.json
    gh_cli        — gh auth token
    config:<name> — custom_providers config entry
    model_config  — model.api_key when model.provider == "custom"
    manual        — user ran `moor auth add`
...
unify here is **removal**:
    ...
    ``moor auth remove <provider> <N>`` must make the pool entry stay gone.

Before this module, every source had an ad-hoc removal branch in
``auth_remove_command``, and several sources had no branch at all — so
``auth remove`` silently reverted on the next ``load_pool()`` call for
qwen-cli, moor device_code (partial), moor_pkce, copilot gh_cli, and
...
class RemovalStep:
    ...
    Attributes:
        provider: Provider pool key (``"xai"``, ``"anthropic"``, ``"moor"``, ...).
...
def _remove_env_source(provider: str, removed) -> RemovalResult:
    ...
    Handles three user situations:
      1. Var lives only in ~/.moor/.env  → clear it
    ...
    """
    from moor_cli.config import get_env_path, remove_env_value
    ...
    try:
        ...
        if env_path.exists():
            ...
            # canonical reader in moor_cli/config.py. read_text() with no
    ...
    if shell_exported:
        result.hints.extend([
            ...
            f"{env_var} until you run `moor auth add {provider}`.",
        ])
    else:
        result.hints.append(
...
def _remove_claude_code(provider: str, removed) -> RemovalResult:
    ...
    return RemovalResult(hints=[
        ...
        "Run `moor auth add anthropic` to re-enable if needed.",
    ])


def _remove_moor_pkce(provider: str, removed) -> RemovalResult:
    """~/.moor/.anthropic_oauth.json is ours — delete it outright."""
    from moor_constants import get_moor_home

    result = RemovalResult()
    oauth_file = get_moor_home() / ".anthropic_oauth.json"
...
def _remove_moor_device_code(provider: str, removed) -> RemovalResult:
    """Moor OAuth lives in auth.json providers.moor — clear it and suppress.

    We suppress in addition to clearing because nothing else stops a future
    `moor auth add moor` (or any other path that writes providers.moor)
    from re-seeding before the user has decided to.  Suppression forces
    them to go through `moor auth add moor` to re-engage, which is the
...
def _remove_minimax_oauth(provider: str, removed) -> RemovalResult:
    ...
    Same pattern as Moor: single-source OAuth state with refresh tokens.
...
def _remove_xai_oauth_device_code(provider: str, removed) -> RemovalResult:
    ...
    Without this step, ``moor auth remove xai-oauth <N>`` silently undoes
    ...
    if _clear_auth_store_provider(provider):
        result.cleaned.append(f"Cleared {provider} OAuth tokens from auth store")
    result.hints.append(
        "Run `moor model` → xAI Grok OAuth (SuperGrok / Premium+) to re-authenticate if needed."
    )
    return result
...
def _remove_codex_device_code(provider: str, removed) -> RemovalResult:
    ...
    via ``moor auth add openai-codex``), but in both cases the re-seed
    ...
    result.hints.extend([
        ...
        "Run `moor auth add openai-codex` to re-enable if needed.",
    ])
    return result
...
def _remove_qwen_cli(provider: str, removed) -> RemovalResult:
    ...
    return RemovalResult(hints=[
        ...
        "Run `moor auth add qwen-oauth` to re-enable if needed.",
    ])
...
def _remove_copilot_gh(provider: str, removed) -> RemovalResult:
    ...
    return RemovalResult(hints=[
        ...
        "Run `moor auth add copilot` to re-enable if needed.",
    ])
...
def _register_all_sources() -> None:
    ...
    register(RemovalStep(
        provider="anthropic", source_id="moor_pkce",
        remove_fn=_remove_moor_pkce,
        description="~/.moor/.anthropic_oauth.json",
    ))
    register(RemovalStep(
        provider="moor", source_id="device_code",
        remove_fn=_remove_moor_device_code,
        description="auth.json providers.moor",
    ))
    register(RemovalStep(
