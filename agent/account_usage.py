...
def _moor_snapshot(windows: list, details: list, tail: list, *, source: str, plan: Optional[str] = None) -> Optional[AccountUsageSnapshot]:
    """Moor snapshot with *tail* lines appended, or None when there is nothing to show."""
    if not windows and not details:
        return None
    return _snapshot("moor", source, windows, details + tail, title="Moor credits", plan=plan)


def build_moor_credits_snapshot(account_info) -> Optional[AccountUsageSnapshot]:
    """MoorPortalAccountInfo → /usage snapshot: dollar magnitudes + renewal date + portal CTA, plus a ``% used``
    ...
    try:
        ...
        return _moor_snapshot(windows, details, [f"Top up: {moor_portal_topup_url(account_info)}", "(or run /topup)"],
                              source="portal-account", plan=getattr(sub, "plan", None) if sub is not None else None)
    except (AttributeError, TypeError):
        return None


def _moor_logged_in() -> bool:
    """Cheap local auth-state check: a Moor access token is present. Fail-open False."""
    try:
...
def _fetch_portal_account(timeout: float):
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=1) as pool:
        return pool.submit(get_moor_portal_account_info, force_fresh=True).result(timeout=timeout)


def moor_credits_lines(*, markdown: bool = False, timeout: float = 10.0) -> list[str]:
    """Rendered Moor-credits /usage lines, or [] when there's nothing to show. Independent of any live agent
    (logged-in gate, then a bounded portal fetch); shared by CLI ``_show_usage`` and the TUI ``session.usage`` RPC.
    Fail-open: any hiccup or timeout → []. MOOR_DEV_CREDITS_FIXTURE renders from the fixture instead of the portal."""
    try:
    ...
    if not _moor_logged_in():
        return []
    try:
        snapshot = build_moor_credits_snapshot(_fetch_portal_account(timeout))
...
def _snapshot_from_credits_state(state) -> Optional[AccountUsageSnapshot]:
    ...
    try:
        ...
        if getattr(state, "paid_access", True) is False:
            details.append(_DEPLETED_LINE)
        return _moor_snapshot(windows, details, ["(dev fixture — MOOR_DEV_CREDITS_FIXTURE)"], source="dev-fixture")
    except (AttributeError, TypeError):
        return None
...
def build_credits_view(*, markdown: bool = False, timeout: float = 10.0) -> CreditsView:
    ...
    balance_lines = [
        line
        for line in render_account_usage_lines(build_moor_credits_snapshot(account), markdown=markdown)
    ...
    return CreditsView(
        ...
        identity_line=("Topping up as " + " / ".join(who)) if who else None, topup_url=moor_portal_topup_url(account),
    ...
    try:
        ...
        if code in (401, 403):
            return _unavailable(f"Codex backend rejected the request (HTTP {code}). Reset credits require ChatGPT-account "
                                "(OAuth) auth — run `moor auth` and sign in with your ChatGPT account.")
