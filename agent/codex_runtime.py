...
def _record_codex_app_server_usage(agent, turn, messages=None) -> dict[str, Any]:
    """Translate Codex app-server token usage into Moor accounting. Prompt bucket = uncached + cached
    ...
    estimate grows monotonically and moor-mode fires thread compaction on tiny threads (#100381)."""
...
def _record_codex_app_server_compaction(agent, turn, *, approx_tokens: int | None = None, force: bool = False) -> bool:
    ...
    if compressor is not None:
        ...
        # Codex owns this summary: a prior Moor deterministic-fallback flag must not leak into it.
...
# --- Codex app-server → Moor UI bridge -------------------------------------
# The app-server bypasses the Moor tool loop, so the bridge translates JSON-RPC notifications
...
def _codex_item_to_tool_name(item: dict) -> str:
    """Synthetic Moor tool name for a codex item (mirrors CodexEventProjector)."""
...
def _ensure_codex_session(agent) -> None:
    ...
    # explicit approval bypass (approvals.mode: off, /yolo, --yolo, MOOR_YOLO_MODE) hands policy to codex's sandbox.
    ...
    # Moor' gateway UI callbacks (tool_progress_callback, _fire_stream_delta,
...
def _bypass_sdk_request_transform(stream_kwargs: dict) -> dict:
    ...
    request without the walk. MOOR_CODEX_SDK_TRANSFORM=1 disables."""
    if os.environ.get("MOOR_CODEX_SDK_TRANSFORM", "").strip().lower() in {"1", "true", "yes", "on"}:
