"""OpenAI-shape bridge shared by Moor' ACP clients.

ACP has no OpenAI-style ``tools``/``tool_calls`` channel, so Moor' tool schemas travel INTO the
...
read/edit/execute tools forwards only Moor' agent-level tools, since re-offering overlapping ones
makes Moor redo finished work.
"""
...
class StreamChunks(list):
    """Chunk list that also carries response-level attributes (e.g. ``moor_projected_messages``)
    Moor reads off the ``create`` result; a plain list would drop them on the stream path."""
