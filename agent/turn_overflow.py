...
_GITHUB_MODELS_HINT = (
    ...
    "      request at ~8K tokens. Moor' system prompt + tool schemas baseline",
    "      exceeds that floor, so this endpoint cannot run an agentic loop.",
    "      Use the `copilot` provider with a Copilot subscription token (`moor",
...
def _recover_context_length(st: _Recovery, _retry: TurnRetryState, error_msg: str) -> OverflowVerdict:
    ...
    if is_output_cap_error(error_msg):
        return st.fail_turn(
            ...
            notices=(
                ...
                "   💡 Moor has no user setting for the output cap — check the endpoint's default max output "
    ...
    # Moor knows for this model (after adopting any limit the server reported), so compressing
    ...
    # Moor assumes, and "wait and /retry" would fail identically forever; compress instead.
