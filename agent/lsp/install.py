...
Installs go to a moor-owned staging dir, ``<MOOR_HOME>/lsp/bin/``, so the
...
missing binary skips the server and ``moor lsp status`` reports it).
...
# ``bin`` in ``<MOOR_HOME>/lsp/bin/`` first, then on PATH.  ``extra_pkgs``
...
INSTALL_RECIPES: Dict[str, Dict[str, Any]] = {
    ...
    # the host so `moor lsp status` reports its presence.
...
def _existing_binary(name: str) -> Optional[str]:
    ...
    for staged in _native_binary_candidates(moor_lsp_bin_dir() / name):
...
def _link_into_bin(target: Path) -> str:
    ...
    link = moor_lsp_bin_dir() / target.name
    if not link.exists():
        try:
...
def _install_npm(pkg: str, bin_name: str, extra_pkgs: Optional[list] = None) -> Optional[str]:
    ...
    # Managed npm first: $MOOR_HOME/node isn't on an arbitrary process's
    ...
    if npm is None:
        ...
        return None
    staging = moor_lsp_bin_dir().parent  # <MOOR_HOME>/lsp/
...
def _install_pip(pkg: str, bin_name: str) -> Optional[str]:
    ...
    pip_target = moor_lsp_bin_dir().parent / "python-packages"
...
def detect_status(pkg: str) -> str:
    """Return ``installed``, ``missing``, or ``manual-only`` (for ``moor lsp status``; spawns nothing)."""
    recipe = INSTALL_RECIPES.get(pkg)
...
__all__ = ["INSTALL_RECIPES", "try_install", "detect_status", "moor_lsp_bin_dir"]
