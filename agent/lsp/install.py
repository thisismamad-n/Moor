...
``<MOOR_HOME>/lsp/bin/``, so we don't pollute the user's global
toolchain.
...
- ``manual`` — never install; if a binary is missing, the server is
  silently skipped and the user is told about it via ``moor lsp
  status``.
...
# ``<MOOR_HOME>/lsp/bin/`` first, then on PATH.
#
# Optional fields:
...
INSTALL_RECIPES: Dict[str, Dict[str, Any]] = {
    ...
    # `moor lsp status` reports the host's presence.
...
def _install_npm(
    ...
    """
    # Managed npm first: $MOOR_HOME/node is not on an arbitrary process's
...
def _install_pip(pkg: str, bin_name: str) -> Optional[str]:
    """Install a Python package into a moor-owned target dir.
    ...
    for script_dir in script_dirs:
        for bin_path in _native_binary_candidates(script_dir / bin_name):
            if bin_path.exists():
                link = moor_lsp_bin_dir() / bin_path.name
                if not link.exists():
                    try:
...
def detect_status(pkg: str) -> str:
    ...
    Used by the ``moor lsp status`` CLI to give users a quick
