...
* The ``iron-proxy`` binary is auto-installed into ``<moor_home>/bin/iron-proxy``
...
* A long-lived CA at ``<moor_home>/proxy/ca.{crt,key}`` is generated on
  first ``moor egress setup``.  Sandboxes trust this CA so iron-proxy can
  terminate TLS and rewrite headers.

* The proxy config lives at ``<moor_home>/proxy/proxy.yaml``.  It enumerates
...
* The proxy runs as a managed subprocess (``moor egress start``), pidfile
  at ``<moor_home>/proxy/iron-proxy.pid``.  Daemon output (including
  per-request records on v0.39) goes to ``<moor_home>/proxy/iron-proxy.log``;
...
# request needs the bearer key below.  ``moor egress reload`` is the
# client.
#
# The key is minted at setup time, stored at
# ``<moor_home>/proxy/management.token`` (0600), and injected into the
...
_MGMT_API_KEY_ENV = "MOOR_IRON_PROXY_MGMT_KEY"
...
def _moor_bin_dir() -> Path:
    from moor_constants import get_moor_home

    return get_moor_home() / "bin"
...
def _proxy_state_dir_ro() -> Path:
    ...
    this — there's no reason to materialize ``~/.moor/proxy/`` just to
    check whether a pidfile exists.
    """
    from moor_constants import get_moor_home

    return get_moor_home() / "proxy"
...
def find_iron_proxy(*, install_if_missing: bool = False) -> Optional[Path]:
    ...
    Resolution order:
      1. ``<moor_home>/bin/iron-proxy``  (our managed copy — preferred)
    ...
    managed = _moor_bin_dir() / _platform_binary_name()
...
def install_iron_proxy(*, force: bool = False) -> Path:
    ...
    these; the user-facing ``moor proxy install`` surface lets them
    propagate so the wizard can show a clear error.
    """

    bin_dir = _moor_bin_dir()
    ...
    with tempfile.TemporaryDirectory(prefix="moor-iron-proxy-") as tmpdir:
        tmp = Path(tmpdir)
...
def _http_download(url: str, dest: Path) -> None:
    req = urllib.request.Request(url, headers={"User-Agent": "moor-agent"})
    try:
...
def iron_proxy_version(binary: Path) -> str:
    ...
    # Don't cache empty output — that would poison ``moor egress
...
def ensure_ca_cert(*, force: bool = False) -> Tuple[Path, Path]:
    ...
    with tempfile.TemporaryDirectory(prefix="moor-proxy-ca-") as tmpdir:
        tmp = Path(tmpdir)
        tmp_key = tmp / "ca.key"
        tmp_crt = tmp / "ca.crt"
...
def mint_proxy_token(prefix: str = "moor-proxy") -> str:
...
def ensure_management_token(*, force: bool = False) -> str:
    ...
    Stored at ``<moor_home>/proxy/management.token`` with 0600 perms.
    ...
    if not force and p.exists():
        try:
            ...
            pass
    token = mint_proxy_token(prefix="moor-mgmt")
    fd = os.open(
        str(p),
...
def reload_proxy() -> bool:
    ...
    if not pid or not _pid_alive(pid):
        raise RuntimeError(
            ...
            "Run `moor egress start`."
        )
    mgmt = _read_management_listen_from_config()
    if mgmt is None:
        raise RuntimeError(
            "The generated proxy.yaml has no management listener (written "
            "before reload support).  Re-run `moor egress setup` and use "
            "`moor egress restart` this one time."
        )
    token = _read_management_token()
    if not token:
        raise RuntimeError(
            "management.token is missing — re-run `moor egress setup`, "
            "then `moor egress restart`."
        )
...
def build_proxy_config(
    ...
    * ``secrets`` transform takes ``secrets:`` (plural), each with a
    ...
    return {
        ...
        # in ``ProxyStatus`` / ``moor egress status``.
        "metrics": {
            "listen": "127.0.0.1:0",
        },
        ...
        # pipeline — `moor egress reload` applies allowlist/token/mapping
...
def write_proxy_config(config: Dict) -> Path:
    """Serialize the config dict to ``<moor_home>/proxy/proxy.yaml``.
...
def discover_uncovered_providers(
    *,
    ...
    The wizard and ``moor egress status`` use this to print a warning.
...
def merge_mappings(
    *,
    ...
    re-running ``moor egress setup`` should not invalidate the tokens
...
# arbitrary env value).
_MOOR_IRON_PROXY_NONCE_ENV = "MOOR_IRON_PROXY_NONCE"
    ...
    if nonce_candidates:
        try:
            ...
            for nonce in nonce_candidates:
                needle = f"{_MOOR_IRON_PROXY_NONCE_ENV}={nonce}".encode()
...
def start_proxy(
    *,
    ...
    if bin_path is None:
        raise RuntimeError(
            "iron-proxy binary not available — run `moor egress install`."
        )
    ...
    env[_MOOR_IRON_PROXY_NONCE_ENV] = _proxy_nonce
    ...
    # ``moor egress stop`` can clean up the orphan.  Failure paths
    ...
    # hits Ctrl-C while waiting for ``moor egress start`` doesn't leak
    ...
    try:
        ...
        if existing_pid and _pid_alive(existing_pid):
            raise RuntimeError(
                ...
                f"Run `moor egress stop` if that proxy is stuck."
            )
            ...
            if access_token and project_id:
                ...
                if missing:
                    ...
                    if not (bitwarden_config or {}).get("allow_env_fallback"):
                        raise RuntimeError(
                            ...
                            f"`moor egress setup --no-bitwarden`, or set "
                ...
                if warnings:
                    logger.warning(
                        ...
                        "run `moor secrets bitwarden status` for detail.",
                        len(warnings),
                    )
            else:
