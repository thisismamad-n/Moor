...
into ``<moor_home>/bin``; CA, ``proxy.yaml``, ``mappings.json``, pidfile and logs live in
...
# interchangeable env-var names for the SAME upstream credential (Moor' auth.py keys Google on both
...
def find_iron_proxy(*, install_if_missing: bool = False) -> Optional[Path]:
    """Managed ``<moor_home>/bin`` copy first, then PATH; optionally auto-install."""
...
def install_iron_proxy(*, force: bool = False) -> Path:
    ...
    (bin_dir := _moor_bin_dir()).mkdir(parents=True, exist_ok=True)
...
def ensure_ca_cert(*, force: bool = False) -> Tuple[Path, Path]:
    ...
    with tempfile.TemporaryDirectory(prefix="moor-proxy-ca-") as tmpdir:
        ...
        _run(["openssl", "req", "-x509", "-new", "-nodes", "-key", str(tmp_key), "-sha256", "-days", "3650", "-subj", "/CN=moor iron-proxy CA",
...
def _yaml():
    """PyYAML module or None (it is a Moor dep, but never a hard requirement here)."""
    try:
        import yaml
        return yaml
    except ImportError:
...
_RELOAD_HTTP_ERRORS = {
    ...
    401: "management API rejected our key (401).  The running daemon was started with a different management.token — run `moor egress restart`.",
}
...
def reload_proxy() -> bool:
    ...
    if not (pid := _read_pid()) or not _pid_alive(pid):
        raise RuntimeError("iron-proxy is not running — nothing to reload.  Run `moor egress start`.")
    ...
    try:
        ...
        raise RuntimeError(
            f"could not reach the management API at {host}:{port} ({exc}).  If the daemon was started before reload support, run `moor egress restart` once."
        ) from exc
...
def _pid_alive(pid: int) -> bool:
    ...
    if nonce_candidates:
        with suppress(OSError):
            ...
            if any(f"{_MOOR_IRON_PROXY_NONCE_ENV}={n}".encode() in env_bytes for n in nonce_candidates):
                return True
    with suppress(OSError):
...
def start_proxy(
    *, binary: Optional[Path] = None, config_path: Optional[Path] = None, extra_env: Optional[Dict[str, str]] = None,
    ...
    if (bin_path := binary or find_iron_proxy(install_if_missing=install_if_missing)) is None:
        raise RuntimeError("iron-proxy binary not available — run `moor egress install`.")
    if not (cfg := config_path or (_proxy_state_dir() / "proxy.yaml")).exists():
        raise RuntimeError(f"iron-proxy config not found at {cfg}. Run `moor egress setup` first.")
    ...
    # Pidfile BEFORE the listening poll so `moor egress stop` can clean an orphan if the parent dies mid-poll.
    pidfile = _pidfile()
    try:
...
def _write_pidfile_safely(pidfile: Path, pid: int) -> None:
    ...
    try:
        ...
        if (existing_pid := _read_pid()) and _pid_alive(existing_pid):
            raise RuntimeError(
                f"Another iron-proxy start appears to be in progress (pidfile {pidfile} -> pid {existing_pid}).  Run `moor egress stop` if that proxy is stuck."
            )
        pidfile.unlink(missing_ok=True)
        fd = os.open(str(pidfile), open_flags, 0o600)
    except OSError as exc:
...
def _refresh_secrets_from_bitwarden(env: Dict[str, str], needed: set, bitwarden_config: Dict, allow_env_fallback: bool) -> None:
    ...
    if missing:
        _bitwarden_shortfall(
            ...
            f"credential_source: env via `moor egress setup --no-bitwarden`, or set `proxy.allow_env_fallback: true` in "
    ...
    if warnings:  # log only the count: the taint analyzer can't tell bws status text is non-secret
        logger.warning("Bitwarden refresh produced %d warning(s); run `moor secrets bitwarden status` for detail.", len(warnings))
