...
2. **Command-type providers** declared under ``tts.providers.<name>:
   ...
   CLI into Moor with shell-template placeholders. **Wins over a
...
shipped today) or ``~/.moor/plugins/tts/<name>/`` (user-installed).
...
``{success: False, error: …}`` JSON envelope the rest of Moor
expects.
"""
...
class TTSProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``moor tools``.
    ...
    def is_available(self) -> bool:
        ...
        Must NOT raise — used by the picker and ``moor setup`` for
