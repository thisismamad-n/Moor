...
``~/.moor/plugins/video_gen/<name>/``; mirrors ``agent/image_gen_provider.py``.
...
def save_b64_video(b64_data: str,*, prefix: str="video", extension: str="mp4") -> Path:
    """Decode base64 video data into ``$MOOR_HOME/cache/videos/``; return the path."""
...
def save_url_video(
    ...
    *,
    ...
    """Download an (often ephemeral) video URL into ``$MOOR_HOME/cache/videos/``;
...
class OpenAICompatibleVideoGenProvider(VideoGenProvider):
    ...
    def generate(
        ...
        try:
            ...
            return error_response(
                error="openai Python package not installed. Run: moor pm repair",
