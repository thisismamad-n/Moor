...
as ``kind: backend``) or ``~/.moor/plugins/video_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
class VideoGenProvider(abc.ABC):
    ...
    def capabilities(self) -> Dict[str, Any]:
        ...
        Used by the tool layer for soft validation and by ``moor tools``
...
def _videos_cache_dir() -> Path:
    """Return ``$MOOR_HOME/cache/videos/``, creating parents as needed."""
    from moor_constants import get_moor_home

    path = get_moor_home() / "cache" / "videos"
...
def save_b64_video(
    ...
    *,
    ...
    """Decode base64 video data and write under ``$MOOR_HOME/cache/videos/``.
...
def save_url_video(
    ...
    *,
    ...
    """Download a video URL and write it under ``$MOOR_HOME/cache/videos/``.
