...
def execute_current(
    ...
    *,
    ...
    """Run a provider attempt under the inherited Moor turn when present."""
...
class ManagedLlmStream(Iterator[Any]):
    """Drive Relay's async stream from Moor's provider worker thread."""
    ...
    def __init__(
        ...
        *,
        ...
        def run_callback(callback: Callable[..., Any], *args: Any) -> Any:
            ...
            def guarded() -> Any:
                # Moor-side callbacks run while the native pipeline drives
    ...
    *,
    ...
    with turn.finalize_lock:
        ...
        try:
            ...
            logger.warning(
                "Moor Relay logical LLM finalization failed",
                exc_info=True,
            )
            return
        with turn.logical_llm_lock:
