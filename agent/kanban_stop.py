...
tool calls. Moor treats that as a clean exit → ``rc=0`` → dispatcher
``protocol_violation``.
...
def kanban_stop_nudge_enabled() -> bool:
    ...
    On when ``MOOR_KANBAN_TASK`` is set (dispatcher-spawned worker), unless
    ``MOOR_KANBAN_STOP_NUDGE`` explicitly disables it.
    """
    env = os.environ.get("MOOR_KANBAN_STOP_NUDGE")
    if env is not None and env.strip().lower() in {"0", "false", "no", "off"}:
        return False
    task = (os.environ.get("MOOR_KANBAN_TASK") or "").strip()
    return bool(task)
...
def build_kanban_stop_nudge(
    *,
    ...
    return (
        "[System: You are a Moor kanban worker. A plain-text reply is NOT a "
