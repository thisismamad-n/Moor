...
# Moor' tools.terminal.security_mode -> Codex permissions profile id.
# Missing config -> workspace-write (Codex's own default).
_MOOR_TO_CODEX_PERMISSION_PROFILE = {
...
class CodexAppServerSession:
    """One Codex thread per Moor session, lifetime owned by AIAgent. Not thread-safe: one caller at a time."""
    ...
    def _handle_server_request(self, req: dict) -> None:
        """Answer a codex server request (approval / elicitation) via Moor' approval flow.
    ...
    def _respond_elicitation(self, params: dict) -> dict:
        ...
        action = "accept" if (params.get("serverName") or "") == MOOR_TOOLS_MCP_SERVER_NAME else "decline"
...
# Moor approval choice -> codex decision (app-server-protocol v2). "deny" and
...
def _approval_choice_to_codex_decision(choice: str) -> str:
    """Map a Moor approval choice onto codex's approval decision wire value."""
