...
MCP server (per `~/.codex/config.toml [mcp_servers.moor-tools]`) and
...
Scope (what we expose):
  ...
  - kanban_* (complete/block/comment/    — kanban worker + orchestrator
    ...
    unblock/link)                          write ~/.moor/kanban.db)
...
Run with: python -m agent.transports.moor_tools_mcp_server
...
EXPOSED_TOOLS: tuple[str, ...] = (
    ...
    "skills_list",
    "text_to_speech",
    # Kanban worker handoff tools — gated on MOOR_KANBAN_TASK env var
    # (set by the kanban dispatcher when spawning a worker). Without these
    # in the callback, a worker spawned with openai_runtime=codex_app_server
    # could do the work but couldn't report completion back to the kernel,
    # making it hang until timeout. Stateless dispatch — they just read
    # the env var and write to ~/.moor/kanban.db.
    "kanban_complete",
    "kanban_block",
    ...
    # only — the kanban tool gates them on MOOR_KANBAN_TASK being unset.
...
def _build_server() -> Any:
    ...
    try:
        ...
        raise ImportError(
            f"moor-tools MCP server requires the 'mcp' package: {exc}"
        ) from exc
    ...
    logger.info(
        "moor-tools MCP server registered %d/%d tools",
        exposed_count,
        len(EXPOSED_TOOLS),
    )
    return mcp
