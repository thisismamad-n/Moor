...
Codex owns the loop and tool list there, so a curated subset of Moor tools is
exposed over stdio MCP; codex registers it via ``~/.codex/config.toml
[mcp_servers.moor-tools]``. Run: ``python -m agent.transports.moor_tools_mcp_server``.
"""
...
if __name__ == "__main__":
    try:
        ...
        pass  # a partial ``moor update`` can leave the bootstrap unregistered
...
MOOR_TOOLS_MCP_SERVER_NAME = "moor-tools"
...
# Each name MUST match a registered Moor tool ``model_tools.handle_function_call()`` can dispatch.
...
EXPOSED_TOOLS: tuple[str, ...] = (
    ...
    # Kanban handoff tools: stateless (read MOOR_KANBAN_TASK, write kanban.db).
    ...
    # Orchestrator-only (the kanban tool gates them on MOOR_KANBAN_TASK unset).
    "kanban_create", "kanban_unblock", "kanban_link",
)


def _build_server() -> Any:
    """Create the MCP server with Moor tools attached (lazy imports: importable without ``mcp``)."""
    try:
        # mcp 2.0 renamed `mcp.server.fastmcp` to `mcp.server.MCPServer` (same surface).
        from mcp.server import MCPServer
    except ImportError as exc:  # pragma: no cover - install hint
        raise ImportError(f"moor-tools MCP server requires the 'mcp' package: {exc}") from exc

    from model_tools import get_tool_definitions, handle_function_call

    mcp = MCPServer(
        MOOR_TOOLS_MCP_SERVER_NAME,
        instructions=(
    ...
    # Authoritative Moor schemas so MCP clients see the same parameter docs the model does.
    all_defs = {
    ...
    for name in EXPOSED_TOOLS:
        ...
        if spec is None:
            logger.debug("skipping %s — not registered in this Moor process", name)
            continue
    ...
    logger.info("moor-tools MCP server registered %d/%d tools", exposed_count, len(EXPOSED_TOOLS))
    return mcp


def main(argv: Optional[list[str]] = None) -> int:
    """Entry point for `python -m agent.transports.moor_tools_mcp_server`."""
    ...
    # Keep Moor' own banners off stdout (the MCP wire).
    os.environ.setdefault("MOOR_QUIET", "1")
    os.environ.setdefault("MOOR_REDACT_SECRETS", "true")

    try:
        server = _build_server()
    except ImportError as exc:
        sys.stderr.write(f"moor-tools MCP server cannot start: {exc}\n")
        return 2
    try:
        ...
        logger.exception("moor-tools MCP server crashed")
        sys.stderr.write(f"moor-tools MCP server error: {exc}\n")
        return 1
    return 0
