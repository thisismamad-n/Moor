...
_XAI_CLIENT_WEB_SEARCH_ALIAS = "moor_web_search"
...
# Grok's native Tool Search. Aliased as moor_<name>.
...
# (moor_<name>), map back in normalize_response so Moor dispatch is unaffected. See #85589.
...
def _alias_wire_tools(response_tools: Any, params: dict[str, Any], is_xai_responses: bool) -> tuple[Any, dict[str, str]]:
    ...
    # or resolved): keep Moor dispatch so ``web.backend`` / ``web.search_backend`` is honored, but rename
    # the wire tool to ``moor_web_search`` so Grok cannot hijack the name. The alias is mapped back to
