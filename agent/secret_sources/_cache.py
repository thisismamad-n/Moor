...
a cache problem must never block Moor startup.
"""
...
def resolve_cache_home(home_path: Optional[Path] = None) -> Path:
    """``home_path`` as resolved by ``load_moor_dotenv()``, else ``$MOOR_HOME``/``~/.moor``."""
...
class DiskCache(Generic[K]):
    ...
    One JSON object per backend at ``<moor_home>/cache/<basename>``::
