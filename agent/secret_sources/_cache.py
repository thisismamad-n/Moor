...
cache problem must never block Moor startup.
"""
...
def resolve_cache_home(home_path: Optional[Path] = None) -> Path:
    """Resolve the Moor home used for cache paths.

    ``home_path`` is whatever ``load_moor_dotenv()`` already resolved;
    falling back to ``$MOOR_HOME`` / ``~/.moor`` keeps direct callers
    (and tests that don't thread a home through) working.
    """
    if home_path is None:
        from moor_constants import get_moor_home

        home_path = get_moor_home()
    return home_path
...
class DiskCache(Generic[K]):
    ...
    One JSON object per backend lives at ``<moor_home>/cache/<basename>``::
    ...
    def write(
        ...
        try:
            ...
            # metadata isn't exposed if MOOR_HOME is ever made traversable.
            try:
