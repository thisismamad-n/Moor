...
env-var-shaped values at process startup, AFTER ``~/.moor/.env`` has loaded
and BEFORE the rest of Moor reads ``os.environ``. The contract is deliberately
narrow: read-only; startup-time and synchronous (one ``fetch()`` per process per
MOOR_HOME, under a registry-enforced wall-clock timeout, no background
...
_GENERIC_REMEDIATION = {
    ErrorKind.NOT_CONFIGURED: "Run `moor secrets {name} setup` to finish configuration.",
    ErrorKind.BINARY_MISSING: "Run `moor secrets {name} setup` to install the helper CLI.",
    ErrorKind.AUTH_FAILED: "Credentials rejected — run `moor secrets {name} setup` to re-authenticate.",
    ErrorKind.AUTH_EXPIRED: "Credentials expired — run `moor secrets {name} setup` to re-authenticate.",
