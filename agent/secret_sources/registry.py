...
:func:`apply_all` via ``moor_cli.env_loader``; plugins register through
...
def _validate_source(source: SecretSource) -> Optional[str]:
    ...
    if source.api_version != SECRET_SOURCE_API_VERSION:
        return (f"Ignoring secret source '{name}': built against secret-source API "
                f"v{source.api_version}, this Moor speaks v{SECRET_SOURCE_API_VERSION}")
...
def _active_profile_name(home_path: Optional[Path]) -> str:
    """Active profile name (``~/.moor/profiles/<name>``); "" for the default profile."""
...
def apply_all(secrets_cfg: dict, home_path: Path,
    ...
    enabled = _ordered_enabled_sources(secrets_cfg, scope=moor_home_key(home_path))
    if not enabled:
        return report
