...
try:
    ...
    _MOOR_VERSION = "0.0.0"
_API_CLIENT = f"moor-agent/{_MOOR_VERSION}"  # client context per Gemini's partner-integration guidance
...
_FREE_TIER_GUIDANCE = (
    ...
    "Moor typically makes 3-10 API calls per user turn, so the free tier is exhausted in a handful of "
...
_STANDARD_KEY_GUIDANCE = (
    ...
    "generativelanguage.googleapis.com). Then update GEMINI_API_KEY / GOOGLE_API_KEY in ~/.moor/.env and "
...
_MISSING_KEY_ERROR = (
    ...
    "environment / ~/.moor/.env (get one at https://aistudio.google.com/app/apikey), or run `moor setup` to "
...
class GeminiNativeClient:
    ...
    # (No MOOR_SKIP_ASYNC_WRAP — the async path has a real conversion, AsyncGeminiNativeClient.)
