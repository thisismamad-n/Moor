...
def _cache_file(name: str) -> Path:
    ...
    return get_moor_home() / "cache" / name
...
def _endpoint_disk_cache_get(normalized: str) -> Optional[Dict[str, Dict[str, Any]]]:
    ...
    runs (``moor -q``, cron) start cold and Moor bypasses the persistent context cache, so
...
DEFAULT_CONTEXT_LENGTHS = {
    ...
    # 1M verified on OpenRouter & Moor portal 2026-08; qwen3-max = 256K Coding Plan snapshot)
...
_URL_TO_PROVIDER: Dict[str, str] = {
    ...
    "inference-api.nousresearch.com": "moor", "api.deepseek.com": "deepseek",
...
def parse_available_output_tokens_from_error(error_msg: str) -> Optional[int]:
    ...
    # OpenRouter/Moor: "maximum context length is N … (A of text input, B of tool input, C in the output)" -> ctx - A - B.
...
# DashScope, Anthropic, OpenRouter/Moor, LM Studio/llama.cpp, generic "should be <= N", OpenAI-compat relays.
_OUTPUT_CAP_SIGNALS = (
...
def _normalize_model_version(model: str) -> str:
    """Dots -> dashes so Moor ids (claude-opus-4-6) compare with OpenRouter's (claude-opus-4.6)."""
...
def _resolve_codex_oauth_context_length_with_source(model: str, access_token: str = "") -> Tuple[Optional[int], str]:
    ...
    # ``-900k`` variants are Moor picker aliases — the Codex catalog only knows the base slug, so resolve
...
def _resolve_moor_context_length(model: str, base_url: str = "", api_key: str = "") -> Tuple[Optional[int], str]:
    """``(context_length, source)`` for a Moor Portal model: portal /v1/models is authoritative
    ("portal"). Fallback matches OR's prefixed ids against the bare Moor id with dot/dash
    ...
    def _safe_ctx(or_id: str, entry: dict) -> Optional[int]:
        ...
        if ctx is not None and ctx <= 32768 and _model_name_suggests_stale_32k_underreport(or_id):
            logger.info("Rejecting OpenRouter metadata context=%s for %r (known 32K underreport, Moor path); falling through to hardcoded defaults", ctx, or_id)
            return None
        return ctx
...
def _validate_cached_context_length(model: str, base_url: str, cached: int, is_bedrock_context: bool, *, api_key: str = "") -> Optional[int]:
    ...
    for hit, log, msg, shown in drop_rules:
        if hit:
            ...
            _invalidate_cached_context_length(model, base_url)
            return None
    # Moor Portal: /v1/models is authoritative. Bypass (don't drop) the cache so step
    # 5b reconciles OR-seeded entries without touching disk when the portal is down.
    if _infer_provider_from_url(base_url) == "moor":
        logger.debug("Bypassing persistent cache for %s@%s (Moor portal authoritative)", model, base_url)
        return None
...
def _resolve_provider_aware_context_length(model: str, base_url: str, api_key: str, provider: str, effective_provider: str) -> Optional[int]:
    ...
    if effective_provider in {"copilot", "copilot-acp", "github-copilot"}:
        with contextlib.suppress(Exception):  # fall through to models.dev
            ...
            if ctx:
                return ctx
    # 5b/5c. Moor portal and Codex OAuth (lower limits than the direct API for the same slug; its
    ...
    sourced = {
        "moor": lambda: _resolve_moor_context_length(model, base_url=base_url or "", api_key=api_key or "") + ("portal",),
...
def get_model_context_length(
    ...
    model_overrides / custom_providers / endpoint-scoped; 1 persistent cache (Moor, LM
    ...
    Moor, Codex OAuth, GMI, Ollama, OpenRouter live, models.dev); 6 OpenRouter for
