...
Optional hooks (override to opt in):
  ...
  backup_paths() -> list[str]            — extra on-disk paths to include in `moor backup`
"""
...
class MemoryProvider(ABC):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs always include:
          - moor_home (str): The active MOOR_HOME directory path. Use this
            for profile-scoped storage instead of hardcoding ``~/.moor``.
        ...
        kwargs may also include:
          ...
          - agent_workspace (str): Shared workspace name (e.g. "moor").
    ...
    def save_config(self, values: Dict[str, Any], moor_home: str) -> None:
        ...
        ``moor_home`` is the active MOOR_HOME directory path.
    ...
    def backup_paths(self) -> List[str]:
        """Return extra on-disk paths this provider stores OUTSIDE MOOR_HOME.

        ``moor backup`` only walks MOOR_HOME, so any provider state kept
        ...
        of the archive, and ``moor import`` restores them to their original
