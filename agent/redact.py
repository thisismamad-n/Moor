...
# `export MOOR_REDACT_SECRETS=false`) cannot disable redaction mid-session.
...
# to this env var in moor_cli/main.py, gateway/run.py, and cli.py) or `MOOR_REDACT_SECRETS=false` in
# ~/.moor/.env. An opt-out warning is logged at gateway and CLI startup so operators see the downgrade —
...
def mask_secret(value: str, *, head: int = 4, tail: int = 4, floor: int = 12,
    ...
    """Mask a secret for display (``moor config`` / ``status`` / ``dump``):
