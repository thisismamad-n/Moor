...
# `export MOOR_REDACT_SECRETS=false`) cannot disable redaction
# mid-session.  ON by default — secure default per issue #17691. Users who
# need raw credential values in tool output (e.g. working on the redactor
# itself) can opt out via `security.redact_secrets: false` in config.yaml
# (bridged to this env var in moor_cli/main.py, gateway/run.py, and
# cli.py) or `MOOR_REDACT_SECRETS=false` in ~/.moor/.env. An opt-out
...
def mask_secret(
    ...
    *,
    ...
    ``moor config``, ``moor status``, ``moor dump``, and anywhere
...
def redact_sensitive_text(
    ...
    *,
    ...
    for URLs, ``"eyJ" in text`` for JWTs). On a typical moor log line
...
# (``security.redact_secrets: false`` / MOOR_REDACT_SECRETS) applies to
