...
def _load_raw_config() -> Dict[str, Any]:
    """Read config.yaml with an mtime+size keyed cache (no moor_cli.config import)."""
...
def _home_relative(p: Path) -> Path:
    """Anchor a relative config path at MOOR_HOME; absolute paths pass through."""
    from moor_constants import get_moor_home
    return p if p.is_absolute() else get_moor_home() / p


# Never disableable: `moor-agent` is the agent's own operating manual and the
...
def get_disabled_skill_names(platform: str | None = None) -> Set[str]:
    ...
    (*platform* defaults to ``MOOR_PLATFORM`` / ``MOOR_SESSION_PLATFORM``)."""
    skills_cfg = _skills_cfg()
    if skills_cfg is None:
        return set()
    from gateway.session_context import get_session_env
    resolved_platform = platform or os.getenv("MOOR_PLATFORM") or get_session_env("MOOR_SESSION_PLATFORM")
...
def parse_config_string_list(value) -> List[str]:
    ...
    ``moor config set`` stores lists as quoted JSON/Python-literal strings;
...
def get_external_skills_dirs() -> List[Path]:
    ...
    are ``~``/``${VAR}`` expanded, relative to MOOR_HOME; the local skills dir is skipped."""
    config_path = get_config_path()
    if not config_path.exists():
        return []
...
def get_skill_create_dir() -> Optional[Path]:
    ...
    relative to MOOR_HOME; a value equal to the local skills dir counts as unset."""
...
def display_skill_create_dir() -> str:
    ...
    if create_dir is None:
        return f"{display_moor_home()}/skills/"
...
def get_all_skills_dirs() -> List[Path]:
    """Skill dirs: local ``~/.moor/skills/`` first, then create_dir, then external.
...
# Project-local skills (<root>/.moor/skills, <root>/.agents/skills; root = nearest
...
PROJECT_SKILLS_SUBDIRS = (os.path.join(".moor", "skills"), os.path.join(".agents", "skills"))
...
def _candidate_project_skills_dirs(root: Path) -> List[Path]:
    ...
    (MOOR_HOME itself may live inside a git checkout)."""
    local_skills = get_skills_dir().resolve()
    dirs: List[Path] = []
...
# cached under MOOR_HOME, never inside the repo); "dangerous" excludes the
# skill from index, list, view and slash commands ("caution" loads, as on the hub).

# ── Project skill quarantine (scan-time injection defense) ──────────────── Trust (`moor skills trust`)
...
# high-confidence findings only. The scan cache lives under MOOR_HOME, never inside the repo (we don't
...
def is_quarantined_project_skill(skill_md) -> bool:
    ...
    try:
        ...
        from moor_constants import get_moor_home
        cache_dir = get_moor_home() / "cache" / "project_skill_scans"
...
def normalize_skill_lookup_name(identifier: str) -> str:
    ...
    # (which follows the live profile-scoped MOOR_HOME), so normalization
    ...
    # ~/.moor/skills/<name> may be a symlink to a checkout elsewhere, and
...
def _moor_metadata(frontmatter: Dict[str, Any]) -> Dict[str, Any]:
    """``metadata.moor`` mapping from frontmatter, or ``{}`` when malformed."""
    metadata = frontmatter.get("metadata")
    moor = metadata.get("moor") if isinstance(metadata, dict) else None
    return moor if isinstance(moor, dict) else {}
...
def extract_skill_conditions(frontmatter: Dict[str, Any]) -> Dict[str, List]:
    ...
    moor = _moor_metadata(frontmatter)
    return {key: moor.get(key, []) for key in _CONDITION_KEYS}


def extract_skill_config_vars(frontmatter: Dict[str, Any]) -> List[Dict[str, Any]]:
    """Extract ``metadata.moor.config`` declarations (key/description/default/prompt).
    Entries missing ``key`` or ``description`` are skipped; ``prompt`` defaults to the description."""
    raw = _moor_metadata(frontmatter).get("config")
