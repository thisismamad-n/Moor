...
from moor_constants import get_config_path, get_skills_dir, is_termux
...
# Org mirrors live under ~/.moor/skills/_org/<org_id>/. Resolution is
...
def _detect_environment(env: str) -> bool:
    ...
    MOOR_KANBAN_* vars without owning them), so caching it process-wide would
    ...
    if env == "kanban":
        ...
        # dispatcher sets ``MOOR_KANBAN_TASK`` / ``MOOR_KANBAN_BOARD`` in the
        ...
        # tool availability.
        if os.getenv("MOOR_KANBAN_TASK") or os.getenv("MOOR_KANBAN_BOARD"):
        ...
        # The Moor Docker image runs s6-overlay as PID 1 (/init). s6 plants
...
def _load_raw_config() -> Dict[str, Any]:
    ...
    This module intentionally avoids importing ``moor_cli.config`` on the
...
# `moor-agent` skill is the agent's own operating manual — it drives
# configuring, extending, and troubleshooting Moor itself, and the system
# prompt unconditionally points at it. Disabling it leaves the agent unable
# to help with Moor, so disable requests for these names are ignored
# everywhere the disabled list is consulted.
ESSENTIAL_SKILLS: frozenset = frozenset({"moor-agent"})
...
def get_disabled_skill_names(platform: str | None = None) -> Set[str]:
    ...
    Args:
        platform: Explicit platform name (e.g. ``"telegram"``).  When
            *None*, resolves from ``MOOR_PLATFORM`` or
            ``MOOR_SESSION_PLATFORM`` env vars.  Returns the global
    ...
    resolved_platform = (
        platform
        or os.getenv("MOOR_PLATFORM")
        or get_session_env("MOOR_SESSION_PLATFORM")
    )
...
def parse_config_string_list(value) -> List[str]:
    ...
    ``moor config set`` and JSON-mode editor saves store lists as quoted
...
# which becomes the dominant cost of ``moor`` startup when ~120 skills
...
def get_external_skills_dirs() -> List[Path]:
    ...
    paths that resolve to the local ``~/.moor/skills/`` are silently skipped.

    Cached in-process, keyed on ``config.yaml`` mtime — the function is
    called once per skill during banner / tool-registry scans, and YAML
    parsing a non-trivial config dominates ``moor`` cold-start time
    ...
    for entry in raw_dirs:
        ...
        p = Path(expanded)
        # Resolve relative paths against MOOR_HOME, not cwd
        if not p.is_absolute():
            p = (moor_home / p).resolve()
        else:
            p = p.resolve()
        if p == local_skills:
            continue
...
def get_all_skills_dirs() -> List[Path]:
    """Return all skill directories: local ``~/.moor/skills/`` first, then external.

    The local dir is always first (and always included even if it doesn't exist
    yet — callers handle that).  External dirs follow in config order.

    NOTE: trusted project-local dirs (``./.moor/skills`` at the git root) are
...
#   <root>/.moor/skills/   — moor-native location
...
# a one-line "run `moor skills trust`" notice.
#
...
def _candidate_project_skills_dirs(root: Path) -> List[Path]:
    ...
    The exclusion matters when MOOR_HOME itself lives inside a git checkout:
    ``<root>/.moor/skills`` would otherwise double as both the profile-local
...
def get_untrusted_project_skills_root() -> Optional[Tuple[Path, int]]:
    ...
    ``moor skills trust``. Returns None when there is nothing to notify
...
#
# Trust (`moor skills trust`) is a REPO-level decision made once; the repo's
...
#
# The scan cache lives under MOOR_HOME, never inside the repo (we don't
...
def _project_scan_cache_dir() -> Path:
    ...
    return get_moor_home() / "cache" / "project_skill_scans"
...
def normalize_skill_lookup_name(identifier: str) -> str:
    ...
    under ``~/.moor/skills/`` (including via symlinks) or configured
    ...
    # ~/.moor/skills/<name> where <name> is a symlink to a checked-out
...
def is_external_skill_path(path) -> bool:
    ...
    ``skills.external_dirs`` are externally owned: Moor can discover and view
...
def extract_skill_conditions(frontmatter: Dict[str, Any]) -> Dict[str, List]:
    ...
    if not isinstance(metadata, dict):
        metadata = {}
    moor = metadata.get("moor") or {}
    if not isinstance(moor, dict):
        moor = {}
    return {
        "fallback_for_toolsets": moor.get("fallback_for_toolsets", []),
        "requires_toolsets": moor.get("requires_toolsets", []),
        "fallback_for_tools": moor.get("fallback_for_tools", []),
        "requires_tools": moor.get("requires_tools", []),
    }
...
def iter_skill_index_files(skills_dir: Path, filename: str):
    ...
    Excludes Moor metadata, VCS, virtualenv/dependency, cache, and skill
