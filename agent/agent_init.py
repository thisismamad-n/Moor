...
def _codex_gpt55_autoraise_notice_marker():
    """Per-profile marker path (``$MOOR_HOME`` is profile-scoped; not a config key)."""
...
def _finalize_routing(agent, api_mode, credential_pool):
    ...
    # The Moor agent key lives ~1 h. Without the proactive refresher every agent in the process
...
_TURN_STATE: Dict[str, Any] = {
    ...
    # Credits tracking (dev-only, MOOR_DEV_CREDITS) from x-moor-credits-* headers; session
...
def _setup_logging(agent):
    ...
    # would starve the root file handlers. Noise reduction belongs in moor_logging.
...
_HOST_DEFAULT_HEADERS: List[tuple[str, Callable[[Any, str], Dict[str, str]]]] = [
    ...
    ("githubcopilot.com", _lazy_headers("moor_cli.models", "copilot_default_headers")),
    ...
    ("x.ai", _lazy_headers("tools.xai_http", "moor_xai_default_headers")),
]
...
def _load_tools(agent, enabled_toolsets, disabled_toolsets):
    # A multiplexed gateway may have switched MOOR_HOME since model_tools was imported;
    ...
    # Kanban guidance is session-static (kanban_show iff MOOR_KANBAN_TASK); resolve once.
...
def _init_session_state(agent, session_id, session_db, parent_session_id, reasoning_config, max_tokens,
    ...
    # ~/.moor/sessions/ — kept unconditionally for request_dump_*.json debug breadcrumbs.
    agent.logs_dir = get_moor_home() / "sessions"
    ...
    # Process-scoped --yolo is persisted so `moor --resume` restores the bypass
...
def _compression_codex_settings(cfg: Dict[str, Any]) -> tuple[str, bool, Optional[int]]:
    ...
    if app_server_auto not in {"native", "moor", "off"}:
        _ra().logger.warning(
...
def _warn_nonagentic_moor_model(agent):
    ...
    with suppress(Exception):
        ...
        if _moor_warn:
            _user_msg = (
                "⚠ Moor inc. Hermes 3 & 4 models are NOT agentic — they "
    ...
    if agent.context_compressor:
        try:
            agent.context_compressor.on_session_start(
                agent.session_id, moor_home=str(get_moor_home()),
...
def init_agent(
    ...
    Non-obvious parameters:
      ...
      skip_context_files: skip SOUL.md/.moor.md/AGENTS.md/CLAUDE.md/.cursorrules injection;
        load_soul_identity keeps ~/.moor/SOUL.md as identity regardless.
    """
    _install_safe_stdio()
    ...
    _warn_nonagentic_moor_model(agent)
