...
def _warn_memory_provider_unavailable(name: str, reason: str = "") -> None:
    ...
    not inheriting ``~/.moor/.env``. See NousResearch/hermes-agent#2765.
    ...
    logger.warning(
        ...
        "provider's credentials/config with 'moor memory status'. Note: "
        "systemd/gateway services do not inherit ~/.moor/.env automatically; set "
...
def _provider_default_routes(provider: str) -> set[str]:
    ...
    routes: set[str] = set()
    try:
        from moor_cli.providers import MOOR_OVERLAYS, get_provider

        overlay = MOOR_OVERLAYS.get(provider)
    ...
    try:
        ...
        from moor_cli.providers import normalize_provider as normalize_registry_provider
...
def _build_codex_gpt5_autoraise_notice(
    ...
    return (
        ...
        f"summarizing.\n"
        f"  Opt back out: moor config set compression.codex_gpt55_autoraise false"
    )
...
def _codex_gpt55_autoraise_notice_marker():
    ...
    Lives under ``$MOOR_HOME`` (which is profile-scoped) alongside the other
    ...
    """
    return get_moor_home() / ".codex_gpt55_autoraise_notice"
...
def _record_codex_gpt55_autoraise_notice(autoraise: Dict[str, Any]) -> None:
    ...
    Best-effort: a read-only or missing ``$MOOR_HOME`` just means the notice
...
def _refuse_checkpoint_required_on_codex_app_server(
    ...
    the default — Moor never even initiates the compaction), so no
...
def init_agent(
    ...
    Args:
        ...
        skip_context_files (bool): If True, skip auto-injection of project context files
            (SOUL.md, .moor.md, AGENTS.md, CLAUDE.md, .cursorrules) from the cwd / MOOR_HOME
            into the system prompt. Use this for batch processing and data generation to avoid
            polluting trajectories with user-specific persona or project instructions.
        load_soul_identity (bool): If True, still use ~/.moor/SOUL.md as the primary
    ...
    if api_mode in {"chat_completions", "codex_responses", "anthropic_messages", "bedrock_converse", "codex_app_server"}:
    ...
    elif agent.provider in {"moor", "moor-portal", "Moor inc."}:
        ...
        agent.api_mode = moor_api_mode(agent.model)
    else:
        ...
        try:
            from moor_cli.providers import host_mandated_api_mode as _host_mandated_api_mode
    ...
    agent._cache_ttl = "5m"
    try:
        from moor_cli.config import load_config_readonly as _load_pc_cfg
    ...
    # x-moor-credits-* response headers after each API call.  Session-start
    # remaining is latched the first time a header is ever seen so we can
    # report cumulative micros spent.  Surfaced behind MOOR_DEV_CREDITS.
    ...
    # both live under ~/.moor/logs/.  Idempotent, so gateway mode
    # (which creates a new AIAgent per message) won't duplicate handlers.
    from moor_logging import setup_logging, setup_verbose_logging
    setup_logging(moor_home=_ra()._moor_home)
    ...
    if agent.verbose_logging:
        ...
        # on the fact that moor_logging.setup_logging() does not
        ...
        # handler level inside moor_logging.py, not here.
        pass
    ...
    if agent.api_mode == "anthropic_messages":
        ...
        if _is_bedrock_anthropic:
            ...
            if agent.provider == "minimax-oauth" and isinstance(effective_key, str) and effective_key:
                try:
                    from moor_cli.auth import build_minimax_oauth_token_provider
        ...
        try:
            from moor_cli.config import load_config_readonly as _load_br_cfg
        ...
        if api_key and base_url:
            ...
            if base_url_host_matches(effective_base, "openrouter.ai"):
                ...
                client_kwargs["default_headers"] = moor_xai_default_headers()
            ...
            if _routed_client is not None:
                ...
                if not getattr(agent, "_fallback_activated", False):
                    ...
                    raise RuntimeError(
                        ...
                        "select a provider, or run `moor setup` for first-time "
                        "configuration."
                    )
    ...
    # A multiplexed gateway may enter a different MOOR_HOME after
    ...
    # worker (kanban_show tool is present iff MOOR_KANBAN_TASK is set).
    ...
    try:
        ...
        if not delegated_child:
            os.environ["MOOR_SESSION_ID"] = agent.session_id

    # Session logs go into ~/.moor/sessions/ alongside gateway sessions
    moor_home = get_moor_home()
    agent.logs_dir = moor_home / "sessions"
    agent.logs_dir.mkdir(parents=True, exist_ok=True)
    # Per-session JSON snapshot writer (~/.moor/sessions/session_{sid}.json)
    ...
    try:
        from moor_cli.config import load_config_readonly as _load_sess_cfg
    ...
    # `moor --resume <id>` can restore the bypass (CLI resume paths read
    ...
    try:
        from moor_cli.config import load_config_readonly as _load_agent_config
    ...
    # management API (the historical Moor behavior) or left to LM Studio's
            ...
            if _mem_provider_name and _mem_provider_name.strip():
                ...
                if agent._memory_manager.providers:
                    ...
                    try:
                        ...
                        _init_kwargs["agent_workspace"] = "moor"
                    except Exception:
                        pass
    ...
    if codex_app_server_auto_compaction not in {"native", "moor", "off"}:
        _ra().logger.warning(
            "Invalid compression.codex_app_server_auto=%r; using 'native'. "
            "Valid values are: native, moor, off.",
    ...
    if _config_context_length is not None and isinstance(_model_cfg, dict):
        ...
        if _configured_provider_norm in _runtime_first_provider_ids:
            ...
            try:
                from moor_cli.auth import resolve_provider as resolve_auth_provider
    ...
    if _engine_name != "compressor":
        ...
        if _selected_engine is None:
            _candidate = None
            try:
                from moor_cli.plugins import get_plugin_context_engine
    ...
    # Moor Moor 3/4 are chat models, not tool-call-tuned. The interactive
    ...
    if not agent.quiet_mode and (agent.platform or "cli") != "cli":
        try:
            from moor_cli.model_switch import _check_moor_model_warning

            _moor_warn = _check_moor_model_warning(agent.model or "")
            if _moor_warn:
                _user_msg = (
                    "⚠ Moor inc. Moor 3 & 4 models are NOT agentic — they "
