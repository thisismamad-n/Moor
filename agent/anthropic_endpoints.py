...
headers, and request quirks (MiniMax, Kimi/Moonshot, DeepSeek, OpenCode, Azure AI Foundry, Moor
...
def _is_deepseek_anthropic_endpoint(base_url: str | None) -> bool:
    ...
    keep-unsigned policy used for Kimi's ``/coding`` endpoint. See moor-agent#16748.
    """
...
def _is_moor_portal_endpoint(base_url: str | None) -> bool:
    """Moor Portal's Anthropic Messages route (Bearer JWT, verbatim catalog ids, native
    thinking-signature replay). Trusted hosts only: prod ``inference-api.nousresearch.com`` or the
    operator-set ``MOOR_INFERENCE_BASE_URL`` host (exact hostname equality, so neither lookalike
...
def _requires_bearer_auth(base_url: str | None) -> bool:
    ...
    Foundry, Palantir Foundry's LLM proxy, CommandCode, Moor Portal. Palantir/CommandCode use
    hostname matching (not substring) so ``evil.com/palantirfoundry`` paths don't trigger it."""
    normalized = _normalized_lower(base_url)
    return (
        _is_moor_portal_endpoint(base_url)
