...
YAML files in ``<MOOR_HOME>/skill-bundles/`` (``name``, ``description``,
...
def _bundles_dir() -> Path:
    """Bundles directory: ``MOOR_BUNDLES_DIR`` override (tests) or ``<MOOR_HOME>/skill-bundles``."""
    override = os.environ.get("MOOR_BUNDLES_DIR")
    return Path(override).expanduser() if override else get_moor_home() / "skill-bundles"
...
# File-level CRUD — used by `moor bundles`.
