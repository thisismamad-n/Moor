...
Storage
-------
Bundles live in ``~/.moor/skill-bundles/*.yaml`` (and the equivalent
profile-aware directory under ``MOOR_HOME``). Each file looks like::
...
- :func:`list_bundles` — return rich info for display (``moor bundles``)
...
def _bundles_dir() -> Path:
    """Return the canonical bundles directory under MOOR_HOME.

    Honors ``MOOR_BUNDLES_DIR`` for tests; falls back to
    ``<MOOR_HOME>/skill-bundles``.
    """
    override = os.environ.get("MOOR_BUNDLES_DIR")
    if override:
        return Path(override).expanduser()
    return get_moor_home() / "skill-bundles"
...
# ---------------------------------------------------------------------------
# File-level CRUD helpers — used by `moor bundles` CLI subcommand.
# ---------------------------------------------------------------------------
