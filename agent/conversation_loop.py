...
# Must mirror _STALE_TOOL_CALL_MARKER_RE in moor_state.py; kept local so importing
# moor_state (module-level DEFAULT_DB_PATH) is not forced at load time.
...
def _ollama_context_limit_error(agent: Any, request_tokens: int) -> Optional[str]:
    ...
    logger.warning(
        "Ollama runtime context too small for Moor tool use: model=%s provider=%s base_url=%s "
    ...
    return (
        f"Ollama loaded `{model}` with only {runtime_ctx:,} tokens of runtime context, but Moor "
        ...
        "again. A known-good starting point is 65,536 tokens. In Moor config, set "
...
def _maybe_grow_local_window(agent: Any, compressor: Any,
    ...
    if provider not in ("llamacpp", "llama.cpp", "llama-cpp", "custom") or not (
        ...
        return None
    try:
        from moor_cli.local_runtime.growth import maybe_grow_window
...
def _print_moor_entitlement_guidance(agent, capability: str) -> bool:
    return _print_guidance(agent, _moor_entitlement_message(capability))
...
def _is_moor_inference_route(provider: str, base_url: str) -> bool:
    return (provider or "").strip().lower() == "moor" or base_url_host_matches(
    ...
    if (provider or "").strip().lower() == "anthropic":
        ...
        if unverified:
            return "\n".join([
                ...
                "Retry with a fresh credential state: `moor auth reset anthropic`. Until that "
...
def _run_api_retry_loop(agent, s: _LoopState) -> Optional[Dict[str, Any]]:
    ...
    while s.retry_count < s.max_retries:
        _ng = _run_phase(moor_rate_limit_guard, agent, s)
...
def run_conversation(
    ...
    # Adopt ~/.moor/.env credential/base-url edits made since the last turn — a
...
_PLUGIN_COMPAT_LAZY = {
    ...
    'PARTIAL_STREAM_STUB_ID': ('moor_constants', 'PARTIAL_STREAM_STUB_ID'),
