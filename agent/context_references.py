...
def _ensure_reference_path_allowed(path: Path) -> None:
    ...
    blocked_exact.add(moor_home / ".env")
    blocked_dirs = [home / rel for rel in _SENSITIVE_HOME_DIRS]
    blocked_dirs.extend(moor_home / rel for rel in _SENSITIVE_MOOR_DIRS)
    ...
    # message text into reference expansion, so `@file:~/.moor/auth.json` from a
