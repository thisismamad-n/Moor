...
row taxonomy is inferred from the atlas shape so Moor can render both legacy
...
# rows (1536x1872), while older Moor/petdex sheets used 9 columns x 8 rows
...
class PetState(str, Enum):
    ...
    These are Moor' activity state names. They are not always identical to the
...
# Legacy Moor/petdex row order (top -> bottom) used by the older 8-row,
...
# Canonical Moor activity names -> accepted row-name aliases in descending
