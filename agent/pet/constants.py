...
inferred from the atlas shape so Moor renders both legacy 8-row sheets and
...
class PetState(str, Enum):
    """Animation state a pet can be shown in (Moor names; Codex rows say ``jumping``/``running`` for ``jump``/``run``)."""
...
# Legacy Moor/petdex row order (top -> bottom) for the older 8-row, 9-column atlas.
...
# Canonical Moor names -> accepted row-name aliases in descending preference.
