...
_OBSERVABLE_METRIC_NAMES = (
    "moor.gateway.up", "moor.gateway.state", "moor.gateway.active_agents", "moor.gateway.busy",
    "moor.gateway.drainable", "moor.gateway.restart_requested", "moor.gateway.background_work",
    "moor.gateway.background_delegations", "moor.platform.up", "moor.platform.degraded",
    "moor.cron.scheduler.heartbeat_age_seconds", "moor.cron.scheduler.last_success_age_seconds",
    "moor.cron.scheduler.catch_up_occurrences", "moor.cron.jobs.enabled", "moor.cron.jobs.running",
    "moor.cron.jobs.overdue",
)
...
class GatewayHealthExportRuntime:
    ...
    def shutdown(self) -> None:
        ...
        if closeables:
            worker = threading.Thread(target=_close, name="moor-gateway-health-export-shutdown", daemon=True)
            worker.start()
...
def _read_runtime_snapshot(config: Dict[str, Any]):
    ...
    try:
        ...
        for name, read in (
            ("moor.gateway.background_work", _read_background_work_count),
            ("moor.gateway.background_delegations", _read_background_delegations_count),
        ):
...
def _start_metric_provider(config: Dict[str, Any], sdk: Dict[str, Any]) -> Any:
    ...
    meter = provider.get_meter("moor.gateway.health")
...
class GatewayDiagnosticLogStreamer(EmitterStreamer):
    ...
    def __init__(self, config: Dict[str, Any], sdk: Dict[str, Any]):
        ...
        self._logger = self._provider.get_logger("moor.gateway.diagnostics")
        self._sdk = sdk
        self.exported = 0
...
def start_gateway_health_export(config: Dict[str, Any]) -> GatewayHealthExportRuntime:
    ...
    if metrics_on or diagnostics_on:
        try:
            ...
            logger.warning("monitoring.gateway_health_export.enabled but OTLP SDK is unavailable; install 'moor-agent[otlp]'", exc_info=True)
    ...
    if diagnostics_on:
        try:
            ...
            thread = threading.Thread(target=_run, name="moor-gateway-health-export", daemon=True)
            thread.start()
