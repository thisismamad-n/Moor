...
def build_gateway_health_snapshot(
    ...
    base = {
        ...
        "moor.supervision_mode": mode if mode in _SUPERVISION_MODES else "unknown",
    }
    ...
    metrics: list[GatewayMetric] = [
        metric("moor.gateway.up", int(bool(gateway_running))),
        metric("moor.gateway.active_agents", active_agents),
        metric("moor.gateway.busy", int(bool(busy))),
        metric("moor.gateway.drainable", int(bool(drainable))),
        metric("moor.gateway.restart_requested", int(bool(runtime.get("restart_requested")))),
        metric("moor.gateway.state", 1, **{"moor.gateway.state": gateway_state}),
    ]
    fatal_count = 0
    events: list[GatewayHealthEvent | GatewayDiagnosticEvent] = []
    ...
    for platform, pdata in platforms.items():
        ...
        pattrs = {"moor.platform": str(platform), "moor.platform.state": state}
        metrics.append(metric("moor.platform.up", int(state in _RUNNING_PLATFORM_STATES), **pattrs))
        metrics.append(metric("moor.platform.degraded", int(is_degraded), **pattrs, **{"moor.error_code": error_code}))
        if is_degraded:
