...
``moor pause`` writes a sentinel at ``$MOOR_HOME/ESTOP``; ``moor resume``
...
(fail safe, e.g. ``touch ~/.moor/ESTOP``). Ported from gastownhall/gastown estop.go (MIT).
"""
...
# Same profile-aware / fleet-root resolvers the file-safety guards use (fail-open to ~/.moor).
from agent.file_safety import _moor_home_path as _moor_home, _moor_root_path as _canonical_root
...
def sentinel_path() -> Path:
    """Path of the ESTOP sentinel this process would write on `moor pause`."""
    return _moor_home() / SENTINEL_NAME


def _candidate_sentinel_paths() -> list:
    """Profile home first, then the fleet root if it is a different directory: a profile
    gateway (MOOR_HOME=~/.moor/profiles/<n>) must still honor an operator's ~/.moor/ESTOP."""
...
def paused_reply() -> Optional[str]:
    ...
    return f"⏸️ Moor is paused{tag}. New work is on hold; run `moor resume` to pick things back up."
...
def check_paused(component: str, logger: logging.Logger) -> bool:
    ...
    if first:
        ...
        logger.info(
            "%s dispatch paused by global emergency stop%s — remove with `moor resume` (%s)",
