"""Centralized Moor Portal request tags.

Every Moor request that hits the Moor Portal — main agent loop, auxiliary
client (compression / titles / vision / web_extract / session_search / etc.),
and any future code path — must carry the same product-attribution tags so
Moor can attribute usage to Moor Agent and bucket it by client release.

Tag shape (sent in OpenAI-compatible ``extra_body['tags']``):

    [
        "product=moor-agent",
        "client=moor-client-v<__version__>",
    ]

The version is sourced live from ``moor_cli.__version__`` so it auto-aligns
...
``moor_cli.__version__`` is the canonical source of truth.
"""
...
# active conversation id here and ``moor_portal_tags()`` picks it up as a
...
_conversation_id: ContextVar[Optional[str]] = ContextVar(
    "moor_portal_conversation_id", default=None
)
...
def _moor_version() -> str:
    """Return the current Moor release version, e.g. ``"0.13.0"``.

    Falls back to ``"unknown"`` if ``moor_cli`` cannot be imported (should
...
def moor_client_tag() -> str:
    """Return the ``client=...`` tag for Moor Portal requests.

    Format: ``client=moor-client-v<MAJOR>.<MINOR>.<PATCH>``.
    """
    return f"client=moor-client-v{_moor_version()}"


def conversation_tag(session_id: str) -> str:
    """Return the ``conversation=...`` tag for a Moor session/conversation.

    Format: ``conversation=<session_id>``. ``session_id`` is the canonical
    Moor conversation identifier (``AIAgent.session_id``) — the same value
    used for ``~/.moor/sessions/`` storage, session logs, and lineage.
...
def moor_portal_tags(session_id: str | None = None) -> List[str]:
    """Return the canonical list of Moor Portal product tags.

    Always returns a fresh list so callers can mutate it freely
    (e.g. ``merged_extra.setdefault("tags", []).extend(moor_portal_tags())``).

    When ``session_id`` is provided, a ``conversation=<session_id>`` tag is
    appended so Portal usage can be attributed to a specific Moor
    ...
    """
    tags = ["product=moor-agent", moor_client_tag()]
...
nous_portal_tags = moor_portal_tags  # LEGACY-PORTAL-TAGS: legacy alias
