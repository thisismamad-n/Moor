...
``extra_body['tags']``: ``["product=moor-agent", "client=moor-client-v<base_version>"]``.
...
# ``moor_portal_tags()``. A ContextVar so concurrent agents in one process never
# see each other's id; ``propagate_context_to_thread`` workers inherit it.
_conversation_id: ContextVar[Optional[str]] = ContextVar("moor_portal_conversation_id", default=None)

# Ambient affinity scope (ROUTING value): OpenRouter's sticky ``session_id``, Moor
...
_affinity_scope: ContextVar[Optional[str]] = ContextVar("moor_affinity_scope", default=None)
...
def moor_client_tag() -> str:
    """``client=moor-client-v<MAJOR>.<MINOR>.<PATCH>`` from canonical runtime identity."""
    from moor_cli.version_info import get_version_info
    return f"client=moor-client-v{get_version_info().base_version}"
...
def moor_portal_tags(session_id: str | None = None) -> List[str]:
    """Fresh list of the canonical Moor Portal tags.
