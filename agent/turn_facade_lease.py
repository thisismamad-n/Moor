...
def admit_durable_turn_lease(
    ...
    def _on_wait(elapsed: float) -> None:
        ...
        agent._emit_status(
            ...
            if elapsed < 1.0 else
            f"⏳ Still waiting for the other Moor process on this session ({int(elapsed)}s)..."
        )
...
def _lease_not_acquired_result(agent, session_id: str, conversation_history) -> Dict[str, Any]:
    ...
    if getattr(agent, "_interrupt_requested", False):
        ...
        result = {
            "final_response": (
                "Stopped waiting for another Moor process on this session. "
    ...
    timeout_msg = (
        "⏳ Another Moor process kept this session busy too long. Your message was not "
