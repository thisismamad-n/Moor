...
- ``set_secret_scope(mapping)`` installs the active profile's secrets for the
  ...
  via ``copy_context()`` exactly like the MOOR_HOME override).
...
_GLOBAL_ENV_EXACT = frozenset({
    ...
    "MOOR_MAX_ITERATIONS", "MOOR_MAX_TOKENS", "MOOR_API_TIMEOUT",
    ...
    "MOOR_KANBAN_DB", "MOOR_KANBAN_WORKSPACES_ROOT", "MOOR_KANBAN_BOARD",
...
_GLOBAL_ENV_PREFIXES = (
    "MOOR_KANBAN_",
    "MOOR_TELEGRAM_",   # tuning knobs (batch delays, fallback toggles) — NOT the token
...
def load_env_file(env_path: Path) -> Dict[str, str]:
    ...
    ``set_secret_scope``. Parses the small KEY=VALUE subset Moor writes
    ...
    ``moor_cli.config._parse_env_value``) but never mutates the process
    ...
    # Parse values with the canonical Moor parser: save_env_value
    ...
    from moor_cli.config import _parse_env_value
...
def build_profile_secret_scope(moor_home: Path) -> Dict[str, str]:
    ...
    try:
        from moor_cli.env_loader import get_secret_source_values
