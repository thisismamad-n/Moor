...
The parent Moor process may itself be a Kanban dispatcher worker with
MOOR_KANBAN_* variables in process env. delegate_task children run inside the
...
# though the worker's MOOR_KANBAN_* vars are legitimately in os.environ (cron
...
_NON_DISPATCHER_OWNED_CONTEXT: ContextVar[bool] = ContextVar(
    "moor_non_dispatcher_owned_context",
    default=False,
)
...
def non_dispatcher_owned_context() -> Iterator[None]:
    ...
    own process, where ``MOOR_KANBAN_TASK`` is legitimately set.  Without this
    ...
    ``kanban_complete`` defaults ``task_id`` to ``$MOOR_KANBAN_TASK`` — letting
...
def delegated_child_subprocess_env(
    ...
    parent dispatcher ``MOOR_KANBAN_*`` vars while losing the ContextVar in
