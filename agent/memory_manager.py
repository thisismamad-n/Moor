...
class MemoryManager:
    ...
    def add_provider(self, provider: MemoryProvider) -> None:
        ...
        _core_tool_names = set(_MOOR_CORE_TOOLS)
    ...
    def _strip_skill_scaffolding(text: str) -> Optional[str]:
        ...
        When a user invokes a /skill or /bundle, Moor expands the turn into
    ...
    def _prefetch_provider(
        ...
        # Propagate the caller's contextvars (profile MOOR_HOME override)
    ...
    def _submit_background(self, fn, *, kind: str = "write") -> None:
        ...
        dashboard, cron) is a ContextVar-scoped MOOR_HOME override, and
    ...
    def initialize_all(self, session_id: str, **kwargs) -> None:
        ...
        Automatically injects ``moor_home`` into *kwargs* so that every
        provider can resolve profile-scoped storage paths without importing
        ``get_moor_home()`` themselves.
        """
        if "moor_home" not in kwargs:
            from moor_constants import get_moor_home
            kwargs["moor_home"] = str(get_moor_home())
        for provider in self._providers:
            try:
