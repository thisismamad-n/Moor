...
Moor stores sessions in its own SQLite store (``moor_state.SessionDB``),
...
* **Zero LLM turn.** This is a deterministic export — it never spends a
  model call. The ``moor trace upload`` subcommand calls
...
def upload_session_trace(
    ...
    *,
    ...
    the user's private ``{user}/moor-traces`` dataset. Returns a
