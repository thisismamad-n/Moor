...
def try_recover_primary_transport(
    ...
    transport error. Skipped for aggregators (OpenRouter, Moor) that manage retries server-side."""
    ...
    if (
        (agent.provider or "").strip().lower() in {"moor", "moor-portal", "nousresearch"}
...
def dump_api_request_debug(
    ...
    try:
        ...
        # Sanitize the session ID (may come from an untrusted X-moor-session-Id header) so a
        ...
        if env_var_enabled("MOOR_DUMP_REQUEST_STDOUT"):
...
def _moa_aggregator_cache_policy(agent, eff_model: str) -> tuple[bool, bool]:
    ...
    try:
        from moor_cli.config import load_config as _load_moa_cfg
...
def anthropic_prompt_cache_policy(
    ...
    # Moor Portal proxies to OpenRouter; treat as OpenRouter-equivalent for cache layout.
    is_moor_portal = base_url_host_matches(eff_base_url, "nousresearch.com")
    ...
    if supports_cache_markers and (
        ...
        try:
            from moor_cli.config import get_custom_provider_model_capability
    ...
    if (is_openrouter or is_moor_portal) and (is_claude or is_kimi) and not is_anthropic_wire:
        return True, False
    # Moor Portal Qwen takes the envelope path too; the alibaba-family check below only matches
    # provider=opencode/alibaba and would leave Portal traffic uncached.
    if is_moor_portal and "qwen" in model_lower:
        return True, False
...
def create_openai_client(agent, client_kwargs: dict, *, reason: str, shared: bool) -> Any:
    ...
    # before the built-in ladder so a profile registered from ~/.moor/plugins/ or a pip entry
    ...
    # Delegate all rate-limit / 5xx retry to moor's outer conversation loop, which honors Retry-After and
...
def _apply_switched_provider_request_overrides(agent, new_provider):
    ...
    if custom_providers is None:
        try:
            from moor_cli.config import load_config, get_compatible_custom_providers
...
def switch_model(
    ...
    try:
        ...
        from moor_cli.config import load_config as _sm_load_config
...
def _pre_tool_block_message(agent, function_name, function_args, effective_task_id, tool_call_id, middleware_trace):
    ...
    try:
        from moor_cli.plugins import _dispatch_pre_tool_call_hooks
...
def invoke_tool(agent, function_name: str, function_args: dict, effective_task_id: str,
    ...
    try:
        from moor_cli.middleware import apply_tool_request_middleware
    ...
    from moor_cli.middleware import run_tool_execution_middleware
...
def _session_id_for_heal_log() -> str:
    try:
        from moor_logging import _session_context
        ...
        if escalate:
            ...
            if key not in _empty_heal_user_notified:
                ...
                _empty_heal_pending_notice[key] = (
                    ...
                    "session's history — run /debug share or `moor "
...
def _realign_tool_result_names(messages: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    #   the OpenAI-compat serializer keeps it provider-agnostic: Gemini reaches Moor under many model
...
def _iter_pool_sockets(client: Any):
    ...
    if not pools:
        return
    from agent.process_bootstrap import MOOR_TRANSPORT_OWNER_EXT
    seen: set[int] = set()
    for pool, owner in pools:
        ...
        for pool_req in list(getattr(pool, "_requests", None) or []):
            if owner is not None:
                ...
                if exts.get(MOOR_TRANSPORT_OWNER_EXT) != owner:
                    continue
