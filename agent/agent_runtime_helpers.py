...
def try_recover_primary_transport(
    ...
    Skipped for proxy/aggregator providers (OpenRouter, Moor) which
    ...
    if (
        provider_lower in {"moor", "moor-portal", "nousresearch"}
...
def dump_api_request_debug(
    ...
    *,
    ...
    try:
        ...
        # originate from untrusted input (X-moor-session-Id header), and an
...
def anthropic_prompt_cache_policy(
    ...
    *,
    ...
    # Moor Portal proxies to OpenRouter behind the scenes — identical
    ...
    # that are canonicalized only after Moor sends the request. Honor its
    ...
    if (
        (is_openrouter or is_moor_portal)
        and (is_claude or is_kimi)
        and not is_anthropic_wire
    ):
        return True, False
    # Moor Portal Qwen (e.g. qwen3.6-plus) takes the same envelope-layout
...
def create_openai_client(agent, client_kwargs: dict, *, reason: str, shared: bool) -> Any:
    ...
    # Delegate all rate-limit / 5xx retry to moor's outer conversation loop,
    ...
    # All primary construction and recovery paths must identify Moor to the
...
def switch_model(agent, new_model, new_provider, api_key='', base_url='', api_mode=''):
    ...
    # Pass model so dual-wire providers (Moor Portal anthropic/* → Messages)
...
def sanitize_api_messages(messages: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    #
    # We do NOT drop the call: moor' own dispatch loop intentionally keeps an
