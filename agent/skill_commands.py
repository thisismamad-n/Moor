...
def _resolve_skill_commands_platform() -> Optional[str]:
    ...
    try:
        ...
        resolved_platform = os.getenv("MOOR_PLATFORM") or get_session_env("MOOR_SESSION_PLATFORM")
    except Exception:
...
def _resolve_skill_commands_home() -> str:
    """Effective Moor home the scan is scoped to (profiles carry their own
    ...
    ``set_moor_home_override``), but the module-level scan only tracked
...
def _inject_skill_config(loaded_skill: dict[str, Any], parts: list[str]) -> None:
    """Append a ``[Skill config: ...]`` block with resolved ``metadata.moor.config``
    ...
    try:
        ...
        if not resolved:
            return
        parts.append("")
        parts.append(f"[Skill config (from {display_moor_home()}/config.yaml):")
