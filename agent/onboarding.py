...
def openclaw_residue_hint_cli() -> str:
    """Banner shown the first time Moor starts and finds ``~/.openclaw/``.

    Points users at ``moor claw migrate`` (non-destructive port of config,
    memory, and skills) first. ``moor claw cleanup`` is mentioned as the
    ...
    return (
        ...
        "To port your config, memory, and skills over to Moor, run "
        "`moor claw migrate`.\n"
        "If you've already migrated and want to archive the old directory, "
        "run `moor claw cleanup` (renames it to ~/.openclaw.pre-migration — "
