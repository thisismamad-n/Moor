...
DEFAULT_AGENT_IDENTITY = (
    ...
    "You are Moor Agent, built by Moor inc.. Be direct: match the length of your reply to the weight of the ask "
...
MOOR_AGENT_HELP_GUIDANCE = (
    # Injected only when skill_view exists AND the moor-agent skill is installed (system_prompt.py slot
    # resolution). No "when the two differ" clause: docs-are-authoritative already carries the precedence.
    "You run on Moor Agent (by Moor inc.). When the user needs help with Moor itself — configuring, "
    ...
    "authoritative reference and always holds the latest, most up-to-date information. The `moor-agent` "
    "skill has the actual commands and proven workflows — load it with skill_view(name='moor-agent') "
    "before configuring, modifying, or troubleshooting Moor so you don't guess or invent workarounds."
)
...
KANBAN_GUIDANCE = (
    ...
    "You have been assigned ONE task from the shared board at `~/.moor/kanban.db`. Your task id is in "
    "`$MOOR_KANBAN_TASK`; your workspace is `$MOOR_KANBAN_WORKSPACE`. The `kanban_*` tools in your schema are your "
    ...
    "2. **Work inside the workspace.** `cd $MOOR_KANBAN_WORKSPACE` before any file operations. The workspace is "
    ...
    "- **Workspace.** `cd $MOOR_KANBAN_WORKSPACE` first. For a `worktree` kind with no `.git`, `git worktree add "
    "<path> ${MOOR_KANBAN_BRANCH:-wt/$MOOR_KANBAN_TASK}` from the main repo, then cd there. For a project-linked "
    "task the workspace is a fresh `<repo>/.worktrees/<task-id>` and `$MOOR_KANBAN_BRANCH` a deterministic "
    ...
    "sits in `ready` forever). Ground every assignee in a real profile (`moor profile list`, or ask the user), and "
    "express dependencies via `parents=[...]` on `kanban_create`, not prose.\n"
    "\n"
    "## Do NOT\n\n"
    "- Do not shell out to `moor kanban <verb>` for board operations. Use the `kanban_*` tools — they work across "
...
# cost that compounds over a long conversation. The moor-agent runtime already executes a batch of tool
...
# to moor-agent's Python prompt-assembly architecture.
...
STEER_CHANNEL_NOTE = (
    ...
    "Mid-turn, the user can steer you: Moor appends their message to the end of a tool result, wrapped exactly as:\n"
    f"{STEER_MARKER_OPEN}\n<their message>\n{STEER_MARKER_CLOSE}\n"
...
def hud_surface_note(valid_tool_names: "set[str] | None" = None) -> str:
    ...
    gated = (
        (True,
         ...
         "rather than anything inside Moor. read_window_below identifies that app."),
        (True,
...
PLATFORM_HINTS = {
    ...
    "tui": (
        ...
        "You are in the Moor terminal UI (TUI). Files: there is no attachment channel and MEDIA:/path tags "
    ...
    "desktop": (
        ...
        "You are chatting inside the Moor desktop app, a graphical chat surface. Markdown renders with full GitHub "
...
def _run_backend_probe(env_type: str, terminal_tool) -> str:
    ...
    env = _create_environment(
        ...
        # snapshot. A normal SSHEnvironment would upload the whole ~/.moor tree just to run `uname`,
...
def _embedder_environment_hint() -> str:
    """Embedder-supplied environment description: MOOR_ENVIRONMENT_HINT (container ENV)
    wins over config.yaml ``agent.environment_hint``. Read once at prompt-build time."""
    return (os.getenv("MOOR_ENVIRONMENT_HINT") or "").strip() or str(
...
def _current_session_platform_hint() -> str:
    ...
    try:
        return (get_session_env("MOOR_SESSION_PLATFORM") if get_session_env else "") or ""
    except Exception:
        return ""
...
def build_skills_system_prompt(
    ...
    ``skills_dir_override`` makes home resolution EXPLICIT: a build thread that never bound the MOOR_HOME
...
def load_soul_md(context_length: Optional[int] = None, home_override: "Path | None" = None) -> Optional[str]:
    """SOUL.md from MOOR_HOME (identity slot #1), or None.

    Callers must pass ``skip_soul=True`` to ``build_context_files_prompt`` so it isn't injected twice.
    ``home_override`` pins the profile home (a thread that lost the MOOR_HOME ContextVar reads the wrong one).

    ``home_override`` scopes the read to an explicit profile home (the agent knows its own home from its
    session_db path). Without it, resolution is ambient — which on a thread that lost the MOOR_HOME
    ...
    soul_path = (Path(home_override) if home_override is not None else get_moor_home()) / "SOUL.md"
...
def _load_moor_md(cwd_path: Path, context_length: Optional[int] = None) -> str:
    ...
    content = _read_context_file(moor_md_path) if moor_md_path else ""
    if not content:
        return ""
    label = str(moor_md_path.relative_to(cwd_path)) if moor_md_path.is_relative_to(cwd_path) else moor_md_path.name
    return _context_section(_strip_yaml_frontmatter(content), label, ".moor.md", moor_md_path, context_length)
...
def build_context_files_prompt(
    ...
    Only ONE project context type loads, first found wins: .moor.md/MOOR.md (walk to git root) →
    AGENTS.md chain (git root → cwd) → CLAUDE.md (cwd) → .cursorrules + .cursor/rules/*.mdc (cwd). SOUL.md
    from MOOR_HOME is independent and always included unless *skip_soul* (already the identity slot).
    """
    ...
    # An explicitly configured cwd is honored verbatim — the Moor tree is a legitimate workspace when the
    # user deliberately points a session at it — and CLI-style surfaces pass
    # allow_install_tree_fallback=True because their launch dir IS the user's shell cwd (developing Moor
    # in-tree). See #64590.
    from agent.runtime_cwd import _is_install_tree
    if cwd is None and not allow_install_tree_fallback and _is_install_tree(cwd_path):
        logger.warning(
            "skipping project-context discovery: working-directory resolution fell back to the Moor "
            "install tree (%s) — set terminal.cwd to your project directory", cwd_path,
        )
        sections = []
    else:
        sections = [_load_moor_md(cwd_path, context_length) or _load_agents_md(cwd_path, context_length)
