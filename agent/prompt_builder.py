...
_MOOR_MD_NAMES = (".moor.md", "MOOR.md")


def _find_moor_md(cwd: Path) -> Optional[Path]:
    """Discover the nearest ``.moor.md`` or ``MOOR.md``.
    ...
    # could pick up a .moor.md planted in /tmp, /home, etc.
    search_dirs = [current, *current.parents] if stop_at else [current]

    for directory in search_dirs:
        for name in _MOOR_MD_NAMES:
...
DEFAULT_AGENT_IDENTITY = (
    "You are Moor Agent, an intelligent AI assistant created by Moor inc.. "
...
MOOR_AGENT_HELP_GUIDANCE = (
    "You run on Moor Agent (by Moor inc.). When the user needs help with "
    "Moor itself — configuring, setting up, using, extending, or troubleshooting "
    "it — or when you need to understand your own features, tools, or capabilities, "
    "the documentation at https://hermes-agent.nousresearch.com/docs is your "
    "authoritative reference and always holds the latest, most up-to-date "
    "information. Load the `moor-agent` skill with skill_view(name='moor-agent') "
...
MOOR_AGENT_HELP_GUIDANCE_NO_SKILLS = (
    "You run on Moor Agent (by Moor inc.). When the user needs help with "
    "Moor itself — configuring, setting up, using, extending, or troubleshooting "
    "it — or when you need to understand your own features, tools, or capabilities, "
...
KANBAN_GUIDANCE = (
    ...
    "the shared board at `~/.moor/kanban.db`. Your task id is in "
    "`$MOOR_KANBAN_TASK`; your workspace is `$MOOR_KANBAN_WORKSPACE`. "
    ...
    "ground truth.\n"
    "2. **Work inside the workspace.** `cd $MOOR_KANBAN_WORKSPACE` before "
    ...
    "\n"
    "- **Workspace.** `cd $MOOR_KANBAN_WORKSPACE` first. For a `worktree` kind "
    "with no `.git`, `git worktree add <path> "
    "${MOOR_KANBAN_BRANCH:-wt/$MOOR_KANBAN_TASK}` from the main repo, then "
    "cd there. For a project-linked task the workspace is a fresh "
    "`<repo>/.worktrees/<task-id>` and `$MOOR_KANBAN_BRANCH` a deterministic "
    ...
    "every assignee in a real profile (`moor profile list`, or ask the user), "
    "and express dependencies via `parents=[...]` on `kanban_create`, not prose.\n"
    "\n"
    "## Do NOT\n"
    "\n"
    "- Do not shell out to `moor kanban <verb>` for board operations. Use "
...
#
# The moor-agent runtime already executes a batch of tool calls
...
# from Cline's TypeScript tool-surface guidance to moor-agent's Python
...
def computer_use_guidance(platform_name: Optional[str] = None) -> str:
    ...
    return (
        ...
        "run `moor computer-use doctor` and share the output. That command "
...
PLATFORM_HINTS = {
    ...
    "tui": (
        "You are running in the Moor terminal UI (TUI). "
        "Cron jobs scheduled from this session are LOCAL-ONLY: their output is "
        "saved (viewable via cronjob action='list') but is NOT delivered back "
    ...
    "desktop": (
        "You are chatting inside the Moor desktop app — a graphical chat "
        ...
        "Widgets can talk back: window.moor.send(\"prompt\") — or a "
        "data-moor-send=\"prompt\" attribute on any clickable element — sends "
...
def build_environment_hints() -> str:
    ...
    extra = (os.getenv("MOOR_ENVIRONMENT_HINT") or "").strip()
    if not extra:
        try:
...
def _skills_prompt_snapshot_path() -> Path:
    return get_moor_home() / ".skills_prompt_snapshot.json"
...
def _current_session_platform_hint() -> str:
    ...
    platform = os.environ.get("MOOR_PLATFORM") or os.environ.get("MOOR_SESSION_PLATFORM")
    if platform:
        return platform
...
def build_skills_system_prompt(
    ...
    scanned alongside the local ``~/.moor/skills/`` directory.  External dirs
    ...
    # MOOR_HOME would otherwise fall back to the launch (default) home and
    ...
    if skills_dir_override is not None:
        ...
        _home_token = set_moor_home_override(str(skills_dir.parent))
    else:
        skills_dir = get_skills_dir()
        _home_token = None
    try:
        external_dirs = get_all_skills_dirs()[1:]  # skip local (index 0)
        # Trusted project-local dirs (./.moor/skills, ./.agents/skills at
...
def _build_skills_system_prompt_inner(
    ...
    if not skills_by_category:
        ...
        result = (
            ...
            "skills, voice, gateway, plugins, or any feature — load the `moor-agent` skill "
            "first. It has the actual commands (e.g. `moor config set …`, `moor tools`, "
            "`moor setup`) so you don't have to guess or invent workarounds.\n"
...
def build_moor_subscription_prompt(valid_tool_names: "set[str] | None" = None) -> str:
    """Build a compact Moor subscription capability block for the system prompt."""
    try:
        ...
        logger.debug("Failed to import Moor subscription helper: %s", exc)
        return ""
    ...
    def _status_line(feature) -> str:
        if feature.managed_by_moor:
            return f"- {feature.label}: active via Moor subscription"
        if feature.active:
            current = feature.current_provider or "configured provider"
            return f"- {feature.label}: currently using {current}"
        if feature.included_by_default and features.moor_auth_present:
            return f"- {feature.label}: included with Moor subscription, not currently selected"
        if feature.key == "modal" and features.moor_auth_present:
            return f"- {feature.label}: optional via Moor subscription"
        return f"- {feature.label}: not currently available"

    lines = [
        "# Moor Subscription",
        "Moor subscription includes managed web tools (Firecrawl), image generation (FAL), OpenAI TTS, OpenAI Whisper STT, and browser automation (Browser Use) by default. Modal execution is optional.",
        "Current capability status:",
    ]
    lines.extend(_status_line(feature) for feature in features.items())
    lines.extend(
        [
            "When a Moor-managed feature is active, do not ask the user for Firecrawl, FAL, OpenAI TTS, OpenAI Whisper, or Browser-Use API keys.",
            "If the user is not subscribed and asks for a capability that Moor subscription would unlock or simplify, suggest Moor subscription as one option alongside direct setup or local alternatives.",
            "Do not mention subscription unless the user asks about it or it directly solves the current missing capability.",
            "Useful commands: moor setup, moor setup tools, moor setup terminal, moor status.",
        ]
    )
    return "\n".join(lines)
...
def load_soul_md(
    ...
    """Load SOUL.md from MOOR_HOME and return its content, or None.
    ...
    ambient — which on a thread that lost the MOOR_HOME ContextVar falls
    ...
    try:
        ...
        logger.debug("Could not ensure MOOR_HOME before loading SOUL.md: %s", e)

    _home = Path(home_override) if home_override is not None else get_moor_home()
...
def _load_moor_md(cwd_path: Path, context_length: Optional[int] = None) -> str:
    """.moor.md / MOOR.md — walk to git root."""
    moor_md_path = _find_moor_md(cwd_path)
    if not moor_md_path:
        return ""
    try:
        content = moor_md_path.read_text(encoding="utf-8").strip()
        if not content:
            return ""
        content = _strip_yaml_frontmatter(content)
        rel = moor_md_path.name
        try:
            rel = str(moor_md_path.relative_to(cwd_path))
        except ValueError:
            pass
        content = _scan_context_content(content, rel)
        result = f"## {rel}\n\n{content}"
        return _truncate_content(
            result, ".moor.md", context_length=context_length,
            read_path=str(moor_md_path),
        )
    except Exception as e:
        logger.debug("Could not read %s: %s", moor_md_path, e)
        return ""
...
def build_context_files_prompt(
    ...
    Priority (first found wins — only ONE project context type is loaded):
      1. .moor.md / MOOR.md  (walk to git root)
      2. AGENTS.md / agents.md   (merged chain: git root → cwd)
      3. CLAUDE.md / claude.md   (cwd only)
      4. .cursorrules / .cursor/rules/*.mdc  (cwd only)

    SOUL.md from MOOR_HOME is independent and always included when present.
    ...
    # Never let a FALLBACK-picked directory inside the Moor install/source
    ...
    # explicitly configured cwd is honored verbatim — the Moor tree is a
    ...
    # their launch dir IS the user's shell cwd (developing Moor in-tree).
    ...
    if (
        ...
        logger.warning(
            ...
            "fell back to the Moor install tree (%s) — set terminal.cwd to "
        ...
        project_context = (
            _load_moor_md(cwd_path, context_length)
    ...
    # SOUL.md from MOOR_HOME only — skip when already loaded as identity
    if not skip_soul:
