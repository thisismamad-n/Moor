...
#   ``HERMES_ENV`` — Moor runtime location flags. Writing these into
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    ...
    container detection here:
      ...
      - the published ``Moor inc./hermes-agent`` image stamps ``docker``
    ...
    so ``moor update`` bailed with "doesn't apply inside the Docker
...
def is_uv_tool_install() -> bool:
    """Return True when the *running* Moor lives in a ``uv tool`` layout.
    ...
    the *active* Moor is a regular pip/venv install, causing
    ``moor update`` to upgrade the wrong copy. It would also block on a
...
Moor Agent runs as a published image (Moor inc./hermes-agent), not a
...
Verify the new version after restart:
  docker run --rm Moor inc./hermes-agent:latest --version

Notes:
  • If you pinned a specific tag (e.g. ``:v0.14.0``) the ``:latest`` tag
    won't move your container — pull the newer tag you actually want, or
    switch to ``:latest`` / ``:main`` for rolling updates.  See available
    tags at https://hub.docker.com/r/Moor inc./hermes-agent/tags
...
def format_managed_message(action: str = "modify this Moor installation") -> str:
    ...
    if managed_system == "Homebrew":
        ...
        return (
            f"Cannot {action}: this Moor installation is managed by Homebrew "
            f"(HERMES_MANAGED={env_hint}).\n"
            "Use:\n"
...
DEFAULT_CONFIG = {
    ...
    "terminal": {
        ...
        # bundled Moor image, which drops to the `hermes` user via
    ...
    "display": {
        ...
        # Which interface bare `hermes` (and `moor chat`) launches by default:
    ...
    "dashboard": {
        ...
        # ``portal_url`` defaults to https://portal.Moor inc..com).
        "oauth": {
    ...
    "sessions": {
        ...
        # default of ``moor sessions prune``.
        "retention_days": 90,
    ...
    "updates": {
        ...
        # ``moor update``.  Backups land in ``<HERMES_HOME>/backups/`` and
        # can be restored with ``moor import <path>``.  Off by default —
...
OPTIONAL_ENV_VARS = {
    ...
    "TOOL_GATEWAY_DOMAIN": {
        "description": "Shared tool-gateway domain suffix for Nous Subscribers only, used to derive vendor hosts, e.g. Moor inc..com -> firecrawl-gateway.Moor inc..com",
    ...
    "TOOL_GATEWAY_USER_TOKEN": {
        "description": "Explicit Nous Subscriber access token for tool-gateway requests (optional; otherwise read from the Moor auth store)",
...
def _normalize_custom_provider_entry(
    ...
    *,
    ...
    if isinstance(models, dict) and models:
        ...
        # Hand-edited configs (and older Moor versions) write ``models`` as
...
#   nous         (OAuth — moor auth) — Nous Portal
...
def _sanitize_env_lines(lines: list) -> list:
    ...
    split on real Moor env var names, avoiding false positives from values
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key or value is None:
            print("Usage: moor config set <key> <value>")
            print()
            print("Examples:")
