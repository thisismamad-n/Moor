...
# when hermes_constants is cached from a pre-update version during `moor update`.
...
def _get_ancestor_pids() -> set[int]:
    ...
    ``moor gateway status`` from falsely counting the ``hermes`` CLI that
...
def install_linux_gateway_from_setup(force: bool = False, enable_on_startup: bool = True) -> tuple[str | None, bool]:
    ...
    if scope == "system":
        ...
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd install wizard, never invoked on Windows
            print_warning(
                "  System service install requires sudo, so Moor can't create it from this user session."
            )
            if run_as_user:
                print_info(
                    f"  After setup, run: sudo moor gateway install --system --run-as-user {run_as_user}"
                )
            else:
                print_info(
                    "  After setup, run: sudo moor gateway install --system --run-as-user <your-user>"
                )
            print_info("  Then start it with: sudo moor gateway start --system")
            return scope, False
...
# (the documented `nohup moor gateway run` workaround). See #23387.
...
def _guard_official_docker_root_gateway() -> None:
    ...
    print(
        ...
        "/opt/hermes/docker/entrypoint.sh before the Moor command."
    )
    print(
...
_PLATFORMS = [
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "4. Alternatively, provide user ID + password and Moor will log in directly",
    ...
    },
    {
        ...
        "setup_instructions": [
            "1. Use a dedicated email account for your Moor agent",
        ...
        "vars": [
            {
                ...
                "password": False,
                "help": "The email address Moor will use (e.g., hermes@gmail.com).",
            },
            {
                "name": "EMAIL_PASSWORD",
...
def _setup_weixin():
    ...
    print_info(
        "  3. Moor will store the returned account_id/token in ~/.hermes/.env."
    )
    print_info(
    ...
    if any_configured:
        ...
        if service_running:
            ...
            if supports_systemd_services() or is_macos() or is_windows():
                ...
                print_info(
                    "  For persistence:   tmux new -s hermes 'moor gateway run'"
                )
                print_info(
    ...
    if subcmd == "install":
        ...
        if supports_systemd_services():
            if is_wsl():
                ...
                print_info(
                    "  Or use tmux/screen for persistence: tmux new -s hermes 'moor gateway run'"
                )
                print()
            ...
            print(
                "  tmux new -s hermes 'moor gateway run'         # persistent via tmux"
            )
            print(
                "  nohup moor gateway run > ~/.hermes/logs/gateway.log 2>&1 &  # background"
            )
            sys.exit(1)
        elif is_container():
        ...
        if supports_systemd_services() and (
            ...
            if pids:
                ...
                if is_termux():
                    print(
                        "  nohup moor gateway run > ~/.hermes/logs/gateway.log 2>&1 &  # Best-effort background start"
                    )
                elif is_wsl():
                    print(
                        "  tmux new -s hermes 'moor gateway run'         # persistent via tmux"
