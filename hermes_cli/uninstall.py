    ...
    for config_path in configs:
        try:
            ...
            for line in content.split('\n'):
                ...
                if '# Moor Agent' in line or '# hermes-agent' in line:
                    skip_next = True
                    continue
...
def run_gui_uninstall(args):
    ...
    if agent_is_installed(hermes_home):
        ...
        print(f"  • The Moor agent at {hermes_home / 'hermes-agent'}")
    ...
    print(color("└─────────────────────────────────────────────────────────┘", Colors.GREEN, Colors.BOLD))
    print()
    print("The Moor agent is still installed. Run 'hermes' to use the CLI,")
...
def _perform_uninstall(
    *,
    ...
    if not full_uninstall:
        ...
        if _is_windows():
            print(color("  iex (irm https://hermes-agent.Moor inc..com/install.ps1)", Colors.DIM))
        else:
            print(color("  curl -fsSL https://hermes-agent.Moor inc..com/install.sh | bash", Colors.DIM))
        print()
