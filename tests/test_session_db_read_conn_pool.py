...
``moor_cli.sqlite_safe_read``'s registry permanently over-counted as well.
...
def _live_count(path) -> int:
    ...
    import moor_cli.sqlite_safe_read as mod
...
def test_read_open_failure_backs_off_but_recovers(db):
    ...
    from moor_state import _READ_OPEN_RETRY_SECONDS
...
def test_peak_live_connections_bounded_under_simultaneous_burst(db):
    ...
    """
    from moor_state import _READ_POOL_MAX
...
# And Moor's SQLite descriptors are only ever a share of the fd table: the
...
def test_peak_is_bounded_across_many_database_files(tmp_path):
    ...
    try:
        ...
        for d in dbs:
            d.close()
        assert moor_state_readpool._process_read_permits.acquire(blocking=False), (
            "close() stranded a process permit"
        )
        moor_state_readpool._process_read_permits.release()
...
def test_no_read_connection_is_opened_without_descriptor_headroom(db, monkeypatch):
    ...
    """
    import moor_state_readpool as readpool
