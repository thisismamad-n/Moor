...
class TestOpenVikingSummaryUriNormalization:
    def test_normalize_summary_uri_maps_pseudo_files_to_parent_directory(self):
        assert OpenVikingMemoryProvider._normalize_summary_uri("viking://user/moor/.overview.md") == "viking://user/moor"
        ...
        assert OpenVikingMemoryProvider._normalize_summary_uri("viking://user/moor/memories/profile.md") == "viking://user/moor/memories/profile.md"
...
class TestOpenVikingSkillQuerySafety:
    ...
    def test_skill_markers_match_moor_scaffolding(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("MOOR_BUNDLES_DIR", str(bundles_dir))
...
class TestOpenVikingConfigSchema:
    ...
    def test_recall_config_reads_from_config_yaml(self, monkeypatch, tmp_path):
        ...
        # Populate config.yaml in the temp MOOR_HOME
        moor_home = tmp_path / "moor_test"
        moor_home.mkdir(exist_ok=True)
        config_yaml = moor_home / "config.yaml"
...
class TestOpenVikingRead:
        ...
        provider._client = FakeVikingClient(
            {
                (
                    ...
                    (("uri", "viking://user/moor"),),
                ): {"result": {"content": "overview text"}},
            }
        )

        result = json.loads(provider._tool_read({"uri": "viking://user/moor/.overview.md", "level": "overview"}))

        assert result["uri"] == "viking://user/moor/.overview.md"
        assert result["resolved_uri"] == "viking://user/moor"
    ...
    def test_read_accepts_uri_batch_and_caps_batch_full_content(self):
        provider = OpenVikingMemoryProvider()
        uris = [
            "viking://user/moor/memories/a.md",
            "viking://user/moor/memories/b.md",
            "viking://user/moor/memories/c.md",
            "viking://user/moor/memories/d.md",
        ]
        provider._client = FakeVikingClient(
            {
                (
    ...
    def test_summary_uri_error_does_not_fallback_and_raises(self):
        ...
        try:
            provider._tool_read({"uri": "viking://user/moor/.overview.md", "level": "overview"})
            assert False, "Expected summary endpoint error to be raised"
        except RuntimeError:
            pass

        assert provider._client.calls == [
            ("/api/v1/content/overview", {"uri": "viking://user/moor"}),
        ]


class TestOpenVikingAutoRecallPrefetch:
    @pytest.mark.parametrize("peer", ["", "moor"])
    def test_prefetch_e2e_sends_limit_and_reads_l2_content(self, monkeypatch, peer):
                    ...
                    if payload.get("context_type") == "memory":
                        self._send_json({
                            "result": {
                                "memories": [
                                    {
                                        "uri": "viking://user/user/peers/moor/memories/e2e-full.md",
                                        "score": 0.9,
                                        "level": 2,
                                        "category": "events",
        ...
        assert records["reads"] == ["viking://user/user/peers/moor/memories/e2e-full.md"]
        ...
        assert all(
            headers.get("user-agent") == f"openviking-memory-moor/{_MOOR_VERSION}"
...
class TestOpenVikingBrowse:
    def test_list_browse_unwraps_and_normalizes_entry_shapes(self):
        ...
        provider._client = FakeVikingClient(
            {
                (
                    ...
                    "result": {
                        "entries": [
                            {"name": "memories", "uri": "viking://user/moor/memories", "type": "dir"},
                            {"rel_path": "profile.md", "uri": "viking://user/moor/memories/profile.md", "isDir": False, "abstract": "Profile"},
                        ]
                    }
                },
            }
        ...
        result = json.loads(provider._tool_browse({"action": "list", "path": "viking://user/moor"}))

        assert result["path"] == "viking://user/moor"
        assert result["entries"] == [
            {"name": "memories", "uri": "viking://user/moor/memories", "type": "dir", "abstract": ""},
            {"name": "profile.md", "uri": "viking://user/moor/memories/profile.md", "type": "file", "abstract": "Profile"},
        ]
        assert provider._client.calls == [(
            "/api/v1/fs/ls",
            {"uri": "viking://user/moor"},
...
class TestEnsureClientReloadsEnv:
    ...
    def test_ensure_client_rebuilds_when_api_key_changes(self, monkeypatch):
        ...
        class _StubClient:
            def __init__(self, endpoint, api_key, account="", user="", agent="moor"):
    ...
    def test_rebuilt_client_resolves_its_own_user_space(self, monkeypatch):
        ...
        monkeypatch.setenv("OPENVIKING_AGENT", "moor")
        ...
        assert alice_uri.startswith("viking://user/alice/peers/moor/")
        assert bob_uri.startswith("viking://user/bob/peers/moor/")
    ...
    def test_handle_tool_call_reconnects_after_startup_health_failure(self, monkeypatch):
        ...
        assert out["session_id"].startswith("moor-remember-")
    ...
    def test_remember_uses_a_distinct_one_shot_session_for_each_call(self, monkeypatch):
        ...
        assert first["session_id"].startswith("moor-remember-")
        assert second["session_id"].startswith("moor-remember-")
    ...
    def test_remember_reports_commit_failure_with_recovery_command(self, monkeypatch):
        ...
        assert out["error"].startswith(
            "Memory message was accepted, but commit failed for session moor-remember-"
        )
        ...
        assert out["session_uri"].endswith(f"/sessions/{out['session_id']}")
        assert out["recovery_command"] == f"ov session commit {out['session_id']}"
        assert "same OpenViking profile and credentials as Moor" in out["recovery_note"]
        assert len(posts) == 2
        assert posts[0][0].endswith("/messages")
...
class TestUnavailableWarningsPromiseRetry:
    ...
    reads as "it never recovers" and sends people restarting moor for
    nothing (#5721).
    """
