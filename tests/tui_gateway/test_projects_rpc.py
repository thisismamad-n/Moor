...
def test_record_repos_persists_and_shows_zero_session_repo(tmp_path):
    ...
    # Repo-first: a scanned repo with no moor sessions still surfaces.
...
def test_terminal_session_persists_its_launch_cwd():
    ...
    The user cd'd into that directory before running moor. Dropping it left
...
def test_desktop_agent_rebuild_preserves_workspace_provenance(
    ...
    session = {
        "agent": object(),
        "attached_images": [],
        "cwd": "/picked/repo" if explicit_cwd else "/opt/moor",
        "edit_snapshots": {},
...
def _bind_profiles(monkeypatch, tmp_path: Path, homes: dict[str, Path]) -> None:
    ...
    """
    monkeypatch.setattr(
        "moor_cli.profiles.get_profile_dir",
...
def test_projects_without_a_profile_stay_on_the_launch_home(monkeypatch, tmp_path):
    ...
    assert not (Path(os.environ["MOOR_HOME"]) / "projects.db").exists()
