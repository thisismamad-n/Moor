...
Moor's own top-level modules and crashed the backend on import
(``ImportError: cannot import name ... from 'utils'``). entry.py must run
``moor_bootstrap.harden_import_path()`` before its first non-stdlib import.
...
def test_entry_imports_from_cwd_with_colliding_packages(tmp_path):
    ...
    (and friends) must succeed — the guard strips CWD so Moor's modules win."""
    ...
    env["MOOR_HOME"] = str(tmp_path / "moor_home")
