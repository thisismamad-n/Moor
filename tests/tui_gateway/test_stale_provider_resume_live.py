...
def live_home(monkeypatch):
    """A REAL isolated MOOR_HOME with a config.yaml + state.db on disk."""
    tmp = Path(tempfile.mkdtemp(prefix="moor-live-staleprov-"))
    home = tmp / ".moor"
    ...
    # moor_constants caches the resolved home at first read — the env var
    ...
    for var in list(os.environ):
        if var.endswith("_API_KEY") or var in ("OPENROUTER_KEY", "MOOR_KEY"):
            monkeypatch.delenv(var, raising=False)

    import moor_cli.config as hconfig
    ...
    # conftest also re-pins moor_state.DEFAULT_DB_PATH per test; pin it to
    ...
    monkeypatch.setattr(server, "_moor_home", str(home), raising=False)
    yield home, server
