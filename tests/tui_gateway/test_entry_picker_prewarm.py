...
guard is covered in ``tests/moor_cli/test_picker_prewarm.py``):

- ``main()`` invokes ``moor_cli.model_switch_providers.prewarm_picker_cache_async``
...
import moor_cli.model_switch_providers
...
def _run_main(monkeypatch, events, *, prewarm=None):
    ...
    # entry.main() imports the helper lazily from moor_cli.model_switch,
