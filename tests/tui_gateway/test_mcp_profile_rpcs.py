...
``MOOR_HOME`` with named profile dirs — no mocks of the config/mcp layer — and
...
@pytest.fixture
def moor_root(tmp_path, monkeypatch):
    """A temp MOOR_HOME root with two named profiles: 'work' and 'other'.

    Pointing MOOR_HOME at a dir outside ~/.moor makes it the profile ROOT
    (get_default_moor_root's Docker/custom branch), so named profiles live at
...
def test_add_lands_in_named_profile_only(moor_root):
    root = moor_root
    resp = _call(
        "mcp.servers.add",
        {
...
def test_list_reflects_the_scoped_profile(moor_root):
    _result(
        _call(
            "mcp.servers.add",
...
def test_status_is_profile_scoped_and_credential_safe(moor_root):
    _result(
        _call(
            "mcp.servers.add",
            {"profile": "work", "name": "svc-a", "config": {"command": "svc-a-bin"}},
...
def test_status_does_not_mix_launch_runtime_into_another_profile(moor_root):
...
def test_status_includes_named_profile_runtime_in_multiplex(moor_root):
    ...
    work_token = set_moor_home_override(moor_root / "profiles" / "work")
    try:
        work_scope = moor_home_key()
    finally:
        reset_moor_home_override(work_token)
...
def test_set_api_key_writes_env_and_header_to_right_profile(moor_root):
    root = moor_root
    _result(
        _call(
            "mcp.servers.add",
...
def test_set_api_key_stdio_references_env_block(moor_root):
    root = moor_root
    _result(
        _call(
            "mcp.servers.add",
...
def test_remove_scoped_to_profile(moor_root):
    root = moor_root
    _result(
        _call(
            "mcp.servers.add",
...
def test_add_duplicate_and_missing_errors(moor_root):
    _result(
        _call(
            "mcp.servers.add",
...
def test_add_requires_transport(moor_root):
...
def test_default_profile_add_when_profile_omitted(moor_root):
    ...
    # Omitted profile → launch/default profile == MOOR_HOME root config.yaml.
    default_cfg = _read_yaml(root / "config.yaml")
