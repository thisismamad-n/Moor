...
def moor_home(tmp_path, monkeypatch):
    ...
    # Bust the goal-module DB cache so it re-resolves MOOR_HOME.
...
def server(moor_home, monkeypatch):
    ...
    # Pin config resolution to the isolated MOOR_HOME. Sibling test
    ...
    # module-level ``_moor_home = get_moor_home()`` snapshot freezes
...
def test_moa_bare_returns_usage(server, session, moor_home):
    _write_moa_config(moor_home, """
moa:
