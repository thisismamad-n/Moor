...
Regression for the silent cross-profile secret leak class: ``moor serve`` hosted many profile
...
only MOOR_HOME. And the launch-profile asymmetry: a default-member hosted-room turn in a
...
def test_rpc_scope_reaches_llm_oneshot_and_model_options(two_homes, monkeypatch):
    ...
    def fake_oneshot(**kwargs):
        seen["oneshot"] = (Path(os.environ.get("MOOR_HOME", "")), get_secret("B_ONLY_TOKEN"), get_secret("A_ONLY_TOKEN"))
        from moor_constants import get_moor_home
        seen["oneshot_home"] = Path(get_moor_home())
        return "t"
    ...
    def build_payload(ctx, **kwargs):
        from moor_constants import get_moor_home
        seen["options"] = (Path(get_moor_home()), get_secret("B_ONLY_TOKEN"), get_secret("A_ONLY_TOKEN"))
        return {"providers": []}

    monkeypatch.setattr("moor_cli.inventory.build_model_options_payload", build_payload)
...
def test_launch_profile_agent_build_is_scoped_once_multiplexing(two_homes, monkeypatch):
    ...
    try:
        ...
        assert Path(get_moor_home()) == root
    finally:
