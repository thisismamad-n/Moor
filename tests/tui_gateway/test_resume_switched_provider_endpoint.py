...
with the Moor Portal route, so resuming a chat switched to openai-codex sent the Codex slug to the
...
MOOR_ROUTE = {"base_url": "https://inference-api.nousresearch.com/v1", "api_mode": "chat_completions"}
...
def test_resume_drops_previous_providers_endpoint(tmp_path, row_origin):
    ...
    if row_origin == "gateway_model_switch":
        db.update_session_meta("s1", json.dumps({"model": "openai/gpt-6-luna", "provider": "moor", **MOOR_ROUTE}),
                               "openai/gpt-6-luna")
        db.update_session_model("s1", "gpt-6-luna-900k", provider="openai-codex")
    else:
        db.update_session_meta("s1", json.dumps({"model": "gpt-6-luna-900k", "provider": "openai-codex", **MOOR_ROUTE}),
                               "gpt-6-luna-900k")
    ...
    assert stored_session_route(row, current_model="openai/gpt-6-luna", current_provider="moor") == (
