...
before ``prompt.submit`` installs the session's profile MOOR_HOME override, so
...
def test_profile_home_session_writes_under_profile(tmp_path):
    ...
    profile_home = tmp_path / ".moor" / "profiles" / "coder"
...
def test_launch_home_fallback_when_no_profile(tmp_path):
    ...
    with patch("tui_gateway.server._moor_home", launch_home):
...
def test_two_profiles_are_isolated(tmp_path):
    ...
    home_a = tmp_path / ".moor" / "profiles" / "a"
    home_b = tmp_path / ".moor" / "profiles" / "b"
