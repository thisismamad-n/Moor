...
empty ``.env``, which is all a crashed ``moor profile create`` leaves behind. Both made a host
...
def two_profile_host(tmp_path, monkeypatch):
    """Fake HOME so ``profiles/`` never resolves to the live install (see moor-agent-dev)."""
    home = tmp_path / "fakehome" / ".moor"
    ...
    from moor_cli.profiles import _get_profiles_root
