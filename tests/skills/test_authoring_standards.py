...
def test_required_frontmatter_fields(p):
    ...
    moor = (fm.get("metadata") or {}).get("moor") or {}
    if not (moor.get("tags") or fm.get("tags")) and not _grandfathered(p, "tags"):
        pytest.fail(f"{_rel(p)}: no tags (metadata.moor.tags or top-level tags)")
...
def test_related_skills_resolve(p):
    ...
    moor = (fm.get("metadata") or {}).get("moor") or {}
    dangling = [
        rs for rs in (moor.get("related_skills") or []) if rs not in _all_names()
    ]
