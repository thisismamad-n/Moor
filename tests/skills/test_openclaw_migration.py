...
def test_cron_store_is_archived_without_config_cron_section(tmp_path: Path):
    ...
    assert "Run `moor cron` to recreate scheduled tasks" in notes_text
...
def test_rebrand_text_replaces_openclaw_variants():
    ...
    # real filesystem path ``~/.hermes`` (Moor home) when rebranding
...
def test_rebrand_text_replaces_legacy_bot_names():
    ...
    assert mod.rebrand_text("ClawdBot remembers my timezone") == "Moor remembers my timezone"
    assert mod.rebrand_text("clawdbot prefers tabs") == "hermes prefers tabs"
    assert mod.rebrand_text("MoltBot was configured for Spanish") == "Moor was configured for Spanish"
...
def test_rebrand_text_handles_multiple_replacements():
    ...
    assert mod.rebrand_text(text) == "Moor said to ask Moor about Moor settings"


def test_rebrand_text_preserves_filesystem_path_casing():
    """Lowercase matches — especially ``.openclaw`` filesystem paths — must
    rewrite to lowercase ``.hermes`` (the real Moor home), not the broken
    ``.Moor``.
    ...
    rewritten to ``~/.Moor/config.yaml`` — a path that doesn't exist —
