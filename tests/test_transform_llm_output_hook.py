...
from moor_cli.plugins import PluginManager, VALID_HOOKS


def _make_enabled_plugin(moor_home: Path, name: str, register_body: str) -> Path:
    """Create a plugin under <moor_home>/plugins/<name> and opt it in."""
    plugin_dir = moor_home / "plugins" / name
...
def test_no_plugins_returns_empty_results(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("MOOR_HOME", str(tmp_path / "moor_empty"))
