"""Moor Portal ``anthropic/*`` models route on the native Messages wire.
...
def _native_wire_selected(monkeypatch):
    """These contracts describe the native wire, which is now opt-in (``moor.anthropic_wire:
    native``; default ``chat`` since 2026-09-06, see ``moor_api_mode``). Select it here so the
    wire keeps working for the flip-back; the default's own contract is in
    ``test_moor_anthropic_wire_default.py``."""
    monkeypatch.setattr(_providers, "_moor_anthropic_wire", lambda: "native")

PORTAL_URL = "https://inference-api.nousresearch.com/v1"
# Staging / preview hosts used via MOOR_INFERENCE_BASE_URL — not the prod
# hostname, so Portal behaviour must key off provider=moor.
...
class TestApiModeRouting:
    """``moor_api_mode`` / ``determine_api_mode`` split the Portal catalog."""
    ...
    def test_anthropic_prefixed_models_use_the_messages_wire(self, model):
        assert moor_api_mode(model) == "anthropic_messages"
    ...
    def test_a_claude_model_without_the_vendor_prefix_is_not_rerouted(self):
        ...
        assert moor_api_mode("claude-opus-4.8") == "chat_completions"

    def test_determine_api_mode_honors_the_model_for_moor(self):
        ...
        overlay alone advertises openai_chat for every Moor model."""
        ...
        assert (
            determine_api_mode("moor", PORTAL_URL, model="hermes-4-405b")
            == "chat_completions"
        )
        # No model → historical OpenAI-wire default (safer than guessing).
        assert determine_api_mode("moor", PORTAL_URL) == "chat_completions"
...
class TestRuntimeResolution:
    ...
    def _stub_portal_credentials(self, monkeypatch):
        ...
        monkeypatch.setattr(rp, "resolve_provider", lambda *a, **k: "moor")
    ...
    def test_anthropic_model_resolves_to_the_messages_wire(self, monkeypatch):
        monkeypatch.setattr(rp, "_get_model_config", lambda: {"provider": "moor"})

        resolved = rp.resolve_runtime_provider(
            requested="moor", target_model="anthropic/claude-opus-5"
        )

        assert resolved["provider"] == "moor"
    ...
    def test_target_model_wins_over_the_persisted_default(self, monkeypatch):
        ...
        monkeypatch.setattr(
            rp,
            "_get_model_config",
            lambda: {"provider": "moor", "default": "hermes-4-405b"},
        )
...
class TestPoolRuntimeResolution:
    ...
    def test_pool_entry_honors_the_model_derived_mode(self, monkeypatch, portal_entry):
        ...
        resolved = rp.resolve_runtime_provider(
            requested="moor", target_model="anthropic/claude-opus-4.8"
        )
...
class TestClientShape:
    ...
    def test_lookalike_host_does_not_get_portal_treatment(self):
        ...
        from agent.anthropic_endpoints import _is_moor_portal_endpoint, _requires_bearer_auth

        spoofed = "https://inference-api.nousresearch.com.attacker.test/v1"
        assert not _is_moor_portal_endpoint(spoofed)
    ...
    def test_portal_bearer_does_not_also_send_env_anthropic_api_key(
        ...
        constructor omits it. Moor loads that env from ~/.moor/.env, so
...
class TestPortalBodyFields:
    ...
    They are produced by the Moor provider profile, which only the OpenAI-wire
    transport consults — so the Anthropic branch has to merge them in itself.
    """

    def _build(self, provider="moor", session_id="sess-abc123"):
    ...
    def test_portal_tags_reach_the_messages_request(self):
        from agent.portal_tags import moor_client_tag
    ...
    def test_helper_merge_is_a_no_op_for_non_moor(self):
        from agent.chat_completion_helpers import (
            _merge_moor_portal_messages_extra_body,
        )

        kwargs = {"model": "claude-opus-4-8", "messages": []}
        agent = SimpleNamespace(provider="anthropic", session_id="s")
        assert _merge_moor_portal_messages_extra_body(agent, kwargs) is kwargs
...
class TestPortalThinkingReplay:
    ...
    def test_staging_host_with_env_override_keeps_signed_thinking(
        ...
        monkeypatch.setenv("MOOR_INFERENCE_BASE_URL", STAGING_URL)
...
class TestAuxiliaryDualWire:
    """``resolve_provider_client('moor', …)`` must wrap Claude onto Messages."""
    ...
    def test_non_anthropic_catalog_model_stays_on_chat_completions(self):
        ...
        with (
            patch(
                "agent.auxiliary_client._try_moor",
            ...
            client, model = resolve_provider_client("moor", "hermes-4-405b")
