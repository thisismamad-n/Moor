...
def moor_home(tmp_path, monkeypatch):
    """Point MOOR_HOME at a temp dir so install paths don't touch the real $HOME."""
...
def test_verify_checksums_signature_skips_without_gpg(moor_home, monkeypatch, tmp_path):
...
def test_subprocess_env_strips_unrelated_secrets(moor_home, monkeypatch):
...
def test_ensure_management_token_persists_and_is_stable(moor_home):
    t1 = ip.ensure_management_token()
    t2 = ip.ensure_management_token()
    assert t1 == t2
    assert t1.startswith("moor-mgmt-")
...
def test_reload_proxy_refuses_when_not_running(moor_home, monkeypatch):
...
def test_start_proxy_injects_management_key_env(moor_home, monkeypatch):
    ...
    cfg = ip.build_proxy_config(
        mappings=[_sample_mapping()],
        ca_cert=moor_home / "ca.crt",
        ca_key=moor_home / "ca.key",
        http_listen=["127.0.0.1:9090"],
    )
    ip.write_proxy_config(cfg)
    (moor_home / "bin").mkdir(parents=True, exist_ok=True)
    fake_bin = moor_home / "bin" / "iron-proxy"
    ...
    assert captured_env[ip._MGMT_API_KEY_ENV].startswith("moor-mgmt-")
...
def test_docker_egress_node_options_uses_sentinel(moor_home, monkeypatch):
    """``_egress_proxy_args_for_docker`` should NOT put NODE_OPTIONS in
    env_overrides directly; it uses a sentinel key
    ``_MOOR_EGRESS_NODE_OPTIONS_APPEND`` so DockerEnvironment can
    ...
    assert env.get("_MOOR_EGRESS_NODE_OPTIONS_APPEND") == "--use-openssl-ca"
...
def test_persisted_nonce_roundtrip(moor_home, monkeypatch):
