"""``reasoning_details`` replay is route-scoped: OpenRouter/Moor read it, every other
chat-completions route gets a wire copy without it (strict schemas 400/422 on the field,
wedging the session after an in-session model switch — moor-agent#70233)."""
...
def test_openrouter_and_moor_routes_keep_reasoning_details():
