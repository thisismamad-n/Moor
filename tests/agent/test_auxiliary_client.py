...
def _clean_env(monkeypatch):
    ...
    for key in (
        ...
        "OPENAI_MODEL", "LLM_MODEL", "MOOR_INFERENCE_BASE_URL",
...
class TestResolveTaskProviderModel:
    ...
    def test_explicit_provider_moa_unwraps_to_aggregator(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "moor_cli.moa_config.resolve_moa_preset",
            lambda cfg, name: preset,
        )
        monkeypatch.setattr("moor_cli.config.load_config", lambda: {"moa": {}})
        monkeypatch.setattr("moor_cli.config.load_config_readonly", lambda: {"moa": {}})
...
class TestMoaAggregatorSharedResolution:
    ...
    Real-config tests: write an actual config.yaml under a temp MOOR_HOME
                    ...
                    "moa": {
                        ...
                        "presets": {
                            ...
                            "moor-mix": {
                                "enabled": True,
                                "reference_models": [
                                    {"provider": "moor", "model": "hermes-4-70b"}
                                ],
                                "aggregator": {
                                    "provider": "moor",
                                    "model": "hermes-4-405b",
...
class TestBuildCallKwargsMaxTokens:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            ("moor", "hermes-4", "https://inference-api.nousresearch.com/v1", "max_tokens"),
        ],
    )
...
class TestMoorTagsScoping:
    def test_tags_injected_when_provider_is_moor(self, monkeypatch):
        ...
        assert kwargs["extra_body"]["tags"] == aux._moor_portal_tags()

    def test_tags_not_injected_for_gemini_when_main_is_moor(self, monkeypatch):
        import agent.auxiliary_client as aux

        monkeypatch.setattr(aux, "auxiliary_is_moor", True)
...
class TestResolveXaiOAuthForAux:
    def test_uses_pool_backed_credentials_without_singleton(self, tmp_path, monkeypatch):
        ...
        ``moor auth status`` already reports these as logged in; compression
        ...
        monkeypatch.setenv("MOOR_HOME", str(moor_home))
        monkeypatch.delenv("MOOR_XAI_BASE_URL", raising=False)
    ...
    def test_pool_backed_credentials_honor_base_url_env_override(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("MOOR_HOME", str(moor_home))
        monkeypatch.setenv("MOOR_XAI_BASE_URL", "https://example.x.ai/v1/")
...
class TestResolveProviderClientUniversalModelFallback:
    ...
    provider was picked via ``moor model``, no per-task override is
    ...
    fallback chain (OpenRouter / Moor / etc.), so aux tasks billed
...
class TestOpenRouterPaidLaneGuard:
    ...
    def test_free_only_allows_builtin_default_model(self, monkeypatch):
        ...
        with patch("agent.auxiliary_client._select_pool_entry", return_value=(False, None)), \
             patch("moor_cli.config.load_config_readonly", return_value={"auxiliary": {"free_only": True}}), \
             patch("agent.auxiliary_client.OpenAI") as mock_openai:
            mock_client = MagicMock(name="openrouter_client")
            mock_openai.return_value = mock_client
            client, model = _try_openrouter()
    ...
    def test_free_only_skips_paid_configured_model(self, monkeypatch):
        ...
        monkeypatch.setenv("OPENROUTER_API_KEY", "or-key")
        with patch("agent.auxiliary_client._select_pool_entry", return_value=(False, None)), \
             patch("moor_cli.config.load_config_readonly",
    ...
    def test_paid_lane_warns_once(self, monkeypatch, caplog):
        ...
        monkeypatch.setenv("OPENROUTER_API_KEY", "or-key")
        with patch("agent.auxiliary_client._select_pool_entry", return_value=(False, None)), \
             patch("moor_cli.config.load_config_readonly", return_value=_paid_cfg), \
             patch("agent.auxiliary_client.OpenAI") as mock_openai:
            mock_client = MagicMock(name="openrouter_client")
            mock_openai.return_value = mock_client
...
class TestGetTextAuxiliaryClient:
    ...
    def test_codex_pool_entry_takes_priority_over_auth_store(self):
        ...
        with (
            ...
            patch("moor_cli.auth._read_codex_tokens", side_effect=AssertionError("legacy codex store should not run")),
        ):
            from agent.auxiliary_client import _build_codex_client
    ...
    def test_returns_none_when_nothing_available(self, monkeypatch):
        ...
        with patch("agent.auxiliary_client._read_moor_auth", return_value=None), \
    ...
    def test_custom_endpoint_uses_codex_wrapper_when_runtime_requests_responses_api(self):
        with patch("agent.auxiliary_client._resolve_custom_runtime",
             ...
             patch("agent.auxiliary_client._read_moor_auth", return_value=None), \
             patch("agent.auxiliary_client._resolve_moor_runtime_api", return_value=None), \
...
class TestAuxiliaryPoolAwareness:
    ...
    def test_try_moor_refreshes_stale_pool_entry(self):
        ...
        with (
            patch("agent.auxiliary_client.load_pool", return_value=pool),
            patch("agent.auxiliary_client.OpenAI") as mock_openai,
            patch("moor_cli.models.get_moor_recommended_aux_model", return_value=None),
        ):
            from agent.auxiliary_client import _try_moor

            client, model = _try_moor()

        assert pool.refreshed is True
        assert client is not None
        assert model == _MOOR_MODEL
    ...
    def test_call_llm_retries_moor_after_401(self):
        class _Auth401(Exception):
            status_code = 401

        stale_client = MagicMock()
        stale_client.base_url = "https://inference-api.nousresearch.com/v1"
        stale_client.chat.completions.create.side_effect = _Auth401("stale moor key")
        ...
        with (
            patch("agent.auxiliary_client._resolve_task_provider_model", return_value=("moor", "moor-model", None, None, None)),
            patch("agent.auxiliary_client._get_cached_client", return_value=(stale_client, "moor-model")),
            ...
            patch("agent.auxiliary_client._resolve_moor_runtime_api", return_value=("fresh-agent-key", "https://inference-api.nousresearch.com/v1")),
        ):
            result = call_llm(
                task="compression",
                messages=[{"role": "user", "content": "hi"}],
...
class TestIsModelNotFoundError:
    ...
    def test_moor_openrouter_catalog_404(self):
        """The exact incident error: a Portal-recommended model dropped from
        the Moor → OpenRouter catalog."""
        exc = Exception(
...
class TestRefreshMoorRecommendedModel:
    ...
    def test_falls_back_to_default_when_portal_unavailable(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "moor_cli.models.get_moor_recommended_aux_model", _boom)
        out = _refresh_moor_recommended_model(
            vision=False, stale_model="some/dead-model")
        assert out == _MOOR_MODEL
    ...
    def test_returns_none_when_no_distinct_alternative(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "moor_cli.models.get_moor_recommended_aux_model",
            lambda **kw: _MOOR_MODEL,
        )
        out = _refresh_moor_recommended_model(
            vision=False, stale_model=_MOOR_MODEL)
        assert out is None
...
class TestGetProviderChain:
    ...
    def test_returns_four_entries(self):
        ...
        assert labels == ["openrouter", "moor", "local/custom", "api-key"]
...
class TestTryPaymentFallback:
    ...
    def test_codex_not_in_fallback_chain(self):
        ...
        When OR/Moor/custom/api-key all fail, payment-fallback returns None —
        Codex is never tried with a guessed model.
        """
        with patch("agent.auxiliary_client._try_openrouter", return_value=(None, None)), \
             patch("agent.auxiliary_client._try_moor", return_value=(None, None)), \
...
class TestAuxiliaryTaskExtraBody:
    def test_sync_call_merges_task_extra_body_from_config(self):
        ...
        with patch("moor_cli.config.load_config", return_value=config), patch("moor_cli.config.load_config_readonly", return_value=config), patch(
            "agent.auxiliary_client._get_cached_client",
    ...
    def test_moa_tasks_reject_task_level_reasoning_effort(self, moa_task, caplog):
        ...
        with patch("moor_cli.config.load_config", return_value=config), patch("moor_cli.config.load_config_readonly", return_value=config), \
             caplog.at_level(logging.WARNING, logger="agent.auxiliary_client"):
...
class TestNvidiaBillingHeaders:
    ...
    def test_resolve_provider_client_cloud_adds_billing_origin_header(self, monkeypatch):
        ...
        assert headers["X-BILLING-INVOKE-ORIGIN"] == "MoorAgent"
...
def test_pool_runtime_base_url_uses_moor_env_override(monkeypatch):
    ...
    monkeypatch.setenv("MOOR_INFERENCE_BASE_URL", "https://ai.wildebeest-newton.ts.net/v1")
...
class TestAuxUnhealthyCache:
    ...
    def test_payment_fallback_skips_unhealthy(self):
        ...
        # unhealthy. The chain should still find moor.
        _mark_provider_unhealthy("local/custom")
        with patch("agent.auxiliary_client._read_main_provider", return_value="openrouter"), \
             patch("agent.auxiliary_client._try_openrouter") as or_try, \
             patch("agent.auxiliary_client._try_moor", return_value=(moor_client, "n-model")), \
    ...
    def test_call_llm_marks_provider_unhealthy_on_402(self, monkeypatch):
        ...
        moor_client = MagicMock()
        moor_resp = MagicMock()
        moor_resp.choices = [MagicMock(message=MagicMock(content="ok"))]
        moor_client.chat.completions.create.return_value = moor_resp
        ...
        with patch("agent.auxiliary_client._get_cached_client",
             ...
             patch("agent.auxiliary_client._try_payment_fallback",
                    return_value=(moor_client, "n-model", "moor")), \
...
class TestCustomEndpointApiKeyInheritance:
    ...
    def test_inherits_main_api_key_when_aux_key_empty(self, monkeypatch):
        ...
        with patch("moor_cli.config.load_config", return_value=fake_config), patch("moor_cli.config.load_config_readonly", return_value=fake_config), \
    ...
    def test_runtime_override_key_is_used(self, monkeypatch):
        ...
        with patch.object(ac, "_RUNTIME_MAIN_API_KEY", "sk-runtime-key"), \
             ...
             patch("moor_cli.config.load_config", return_value={"model": {}}), patch("moor_cli.config.load_config_readonly", return_value={"model": {}}), \
             patch.object(ac, "_create_openai_client", side_effect=_capture_create):
            client, model = resolve_provider_client(
                "custom",
                model="test-model",
...
class TestFastModelTier:
    ...
    def test_catalog_match_prefers_rolling_alias_over_pinned_id(self):
        ...
        with patch("moor_cli.models_pricing.fetch_models_with_pricing", return_value=catalog):
            assert ac._fast_model_from_catalog("moor") == "~openai/gpt-mini-latest"
    ...
    def test_catalog_match_skips_reasoning_batch_and_embedding_lookalikes(self):
        ...
        with patch("moor_cli.models_pricing.fetch_models_with_pricing", return_value=catalog):
            assert ac._fast_model_from_catalog("moor") == "google/gemini-3.6-flash"
    ...
    def test_catalog_match_skips_the_non_chat_siblings_of_a_chat_model(self):
        ...
        with patch("moor_cli.models_pricing.fetch_models_with_pricing", return_value=catalog):
            assert ac._fast_model_from_catalog("moor") == "openai/gpt-4o-mini"
    ...
    def test_catalog_match_takes_the_newest_of_a_family(self):
        ...
        with patch("moor_cli.models_pricing.fetch_models_with_pricing", return_value=catalog):
            assert ac._fast_model_from_catalog("moor") == "openai/gpt-10-mini"
    ...
    def test_catalog_fetch_is_authenticated(self):
        ...
        with patch(
            ...
            "moor_cli.models_pricing.fetch_models_with_pricing", return_value={}
        ) as fetch:
    ...
    def test_fast_tier_is_opt_in(self):
        ...
        with patch.object(ac, "_fast_model_from_catalog") as spy:
            ac._get_aux_model_for_provider("moor")
