...
class TestNamedProfileHintIntegration:
    ...
    def test_real_moor_home_under_profiles_renders_correct_paths(
        ...
        assert "Active Moor profile: coder." in prompt
    ...
    def test_real_default_home_renders_default_branch(self, tmp_path, monkeypatch):
        ...
        assert "Active Moor profile: default." in prompt
...
def test_coding_prompt_preserves_legacy_workspace_order(monkeypatch):
    ...
    monkeypatch.setattr(system_prompt, "MOOR_AGENT_HELP_GUIDANCE_NO_SKILLS", "HELP")
    monkeypatch.setattr(system_prompt, "STEER_CHANNEL_NOTE", "STEER")
    monkeypatch.setattr(system_prompt, "get_moor_home", lambda: Path("/moor"))

    # Production renders this as str(get_moor_home()) + "/profiles/<name>/",
    # and str(Path("/moor")) is platform-dependent (backslash on Windows) —
    # build the expectation the same way instead of hardcoding "/moor".
    _home_str = str(Path("/moor"))
    expected_profile = (
...
def test_conversation_start_uses_session_start_not_build_time(monkeypatch):
    ...
    with (
        ...
        patch("moor_time.now", return_value=datetime(2026, 1, 2, 9, 0)),
    ):
        prompt = build_system_prompt(agent, system_message="SYSTEM_MESSAGE")
...
class TestConversationStartedTwoLine:
    ...
    def test_same_day_session_keeps_single_line(self):
        from moor_time import now as moor_now
        sid = moor_now().strftime("%Y%m%d_%H%M%S_fresh")
