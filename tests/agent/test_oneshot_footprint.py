"""One-shot sessions (``moor chat -q``) drop the self-improvement footprint.
...
``MOOR_SINGLE_QUERY_SESSION`` the approval gate and delegation dispatcher read, so an interactive
...
def test_oneshot_delegation_budget_charges_total_children_then_refuses(oneshot, monkeypatch):
    ...
    monkeypatch.delenv("MOOR_SINGLE_QUERY_SESSION")
