...
def isolated_home(monkeypatch):
    """Temp MOOR_HOME with config + clean credential env vars."""
    test_home = tempfile.mkdtemp(prefix="moor_test_31179_")
    moor_home = os.path.join(test_home, ".moor")
...
def _fresh_modules():
    """Drop cached moor modules so each test reloads against current env."""
    _drop_reload_targets()
