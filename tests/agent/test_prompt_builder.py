...
class TestTruncateContent:
    ...
    def _reset_truncation_state(self, monkeypatch):
        ...
        monkeypatch.setattr("moor_cli.config.load_config", default_load_config)
        monkeypatch.setattr("moor_cli.config.load_config_readonly", default_load_config)
    ...
    def test_truncation_warning_points_to_config_key(self, monkeypatch):
        ...
        monkeypatch.setattr("moor_cli.config.load_config", fake_load_config)
        monkeypatch.setattr("moor_cli.config.load_config_readonly", fake_load_config)
...
class TestDynamicContextFileCap:
    ...
    def _no_explicit_config(self, monkeypatch):
        ...
        monkeypatch.setattr("moor_cli.config.load_config_readonly", lambda: {})
...
class TestBuildMoorSubscriptionPrompt:
    def test_includes_active_subscription_features(self, monkeypatch):
        ...
        monkeypatch.setattr(
            ...
            lambda config=None: MoorSubscriptionFeatures(
                subscribed=True,
                moor_auth_present=True,
                provider_is_moor=True,
                features={
                    "web": MoorFeatureState("web", "Web tools", True, True, True, True, False, True, "firecrawl"),
                    "image_gen": MoorFeatureState("image_gen", "Image generation", True, True, True, True, False, True, "Moor Subscription"),
                    "video_gen": MoorFeatureState("video_gen", "Video generation", False, False, False, False, False, False, ""),
                    "tts": MoorFeatureState("tts", "OpenAI TTS", True, True, True, True, False, True, "OpenAI TTS"),
                    "stt": MoorFeatureState("stt", "Speech-to-text", True, True, True, True, False, True, "OpenAI Whisper"),
                    "browser": MoorFeatureState("browser", "Browser automation", True, True, True, True, False, True, "Browser Use"),
                    "modal": MoorFeatureState("modal", "Modal execution", False, True, False, False, False, True, "local"),
                },
            ),
        )

        prompt = build_moor_subscription_prompt({"web_search", "browser_navigate"})
    ...
    def test_non_subscriber_prompt_includes_relevant_upgrade_guidance(self, monkeypatch):
        ...
        monkeypatch.setattr(
            ...
            lambda config=None: MoorSubscriptionFeatures(
                ...
                features={
                    "web": MoorFeatureState("web", "Web tools", True, False, False, False, False, True, ""),
                    "image_gen": MoorFeatureState("image_gen", "Image generation", True, False, False, False, False, True, ""),
                    "video_gen": MoorFeatureState("video_gen", "Video generation", False, False, False, False, False, False, ""),
                    "tts": MoorFeatureState("tts", "OpenAI TTS", True, False, False, False, False, True, ""),
                    "stt": MoorFeatureState("stt", "Speech-to-text", True, False, False, False, False, True, ""),
                    "browser": MoorFeatureState("browser", "Browser automation", True, False, False, False, False, True, ""),
                    "modal": MoorFeatureState("modal", "Modal execution", False, False, False, False, False, True, ""),
                },
            ),
        )

        prompt = build_moor_subscription_prompt({"image_generate"})

        assert "suggest Moor subscription as one option" in prompt
        assert "Do not mention subscription unless" in prompt

    def test_feature_flag_off_returns_empty_prompt(self, monkeypatch):
        monkeypatch.setattr("tools.tool_backend_helpers.managed_moor_tools_enabled", lambda: False)

        prompt = build_moor_subscription_prompt({"web_search"})
...
class TestBuildContextFilesPrompt:
    ...
    def test_moor_md_still_wins_over_agents_override(self, tmp_path):
        (tmp_path / ".moor.md").write_text("Moor-first context.")
...
# =========================================================================
# .moor.md helper functions
# =========================================================================


class TestFindMoorMd:
    def test_finds_in_cwd(self, tmp_path):
        (tmp_path / ".moor.md").write_text("rules")
        assert _find_moor_md(tmp_path) == tmp_path / ".moor.md"



    def test_walks_to_git_root(self, tmp_path):
        (tmp_path / ".git").mkdir()
        (tmp_path / ".moor.md").write_text("root rules")
        sub = tmp_path / "a" / "b"
        sub.mkdir(parents=True)
        assert _find_moor_md(sub) == tmp_path / ".moor.md"
    ...
    def test_no_git_root_checks_cwd_only(self, tmp_path):
        ...
        the way to / and pick up a .moor.md planted in /tmp, /home, or /
...
class TestEnvironmentHints:
    ...
    def test_environment_hint_from_env_var_is_appended(self, monkeypatch):
        """MOOR_ENVIRONMENT_HINT lets an embedder describe the runtime env."""
        import agent.prompt_builder as _pb
        monkeypatch.setattr(_pb, "is_wsl", lambda: False)
        monkeypatch.delenv("TERMINAL_ENV", raising=False)
        monkeypatch.setenv("MOOR_ENVIRONMENT_HINT", "Running inside an OpenShell sandbox.")
        _pb._clear_backend_probe_cache()
        result = _pb.build_environment_hints()
