...
class TestEnvironmentHints:
    ...
    def test_probe_remote_backend_ssh_is_probe_only_and_torn_down(self, monkeypatch):
        ...
        ~/.moor and snapshot a session just to run `uname`, and its __del__ would
...
class TestContextFileReadTimeout:
    def test_slow_moor_md_is_skipped_and_agents_md_still_loads(self, tmp_path, monkeypatch, caplog):
        (tmp_path / ".git").mkdir()
        (tmp_path / ".moor.md").write_text("Moor project rules.")
        ...
        assert "Moor project rules" not in result
