"""Regression guard: a genuine Moor 429 must re-enter the retry loop so the
top-of-loop Moor rate-limit guard can activate the fallback chain.
...
``record_moor_rate_limit()`` moments earlier and either activates a fallback
