...
def curator_env(tmp_path, monkeypatch):
    """Isolated MOOR_HOME + freshly reloaded curator + skill_usage modules."""
    home = tmp_path / ".moor"
    ...
    # save_state() resolves the state path from MOOR_HOME at write time, so a
    # straggler thread that outlives this test would write into whatever home
    # the *next* test has configured (or the default ~/.moor once monkeypatch
    ...
    # thread here, while MOOR_HOME is still pinned to this test's tmp home
...
def test_run_review_synchromoor_invokes_llm_stub(curator_env, monkeypatch):
    c = curator_env["curator"]
    u = curator_env["usage"]
    skills_dir = curator_env["home"] / "skills"
    _write_skill(skills_dir, "a")
...
# `moor model` → auxiliary picker, the dashboard Models tab, and the full
...
def test_curator_slot_is_canonical_aux_task():
    ...
    from moor_cli.web_server_config import _AUX_TASK_SLOTS
    ...
    # 2. moor_cli/main.py _AUX_TASKS — CLI picker
    aux_keys = {k for k, _name, _desc in _AUX_TASKS}
    assert "curator" in aux_keys, "curator missing from _AUX_TASKS (CLI picker)"

    # 3. moor_cli/web_server.py _AUX_TASK_SLOTS — REST API allowlist
...
def test_review_fork_restricts_toolsets_to_skills_only(curator_env, monkeypatch):
    ...
    ``moor curator rollback`` restored a hollow skill. Removing the toolset
...
def test_review_fork_toolset_surface_excludes_execution_tools():
    ...
    already-stripped package, and ``moor curator rollback`` restores a
...
def test_review_prompt_does_not_steer_terminal_writes():
    ...
    ``mkdir -p ~/.moor/skills/<umbrella>/references/ && mv ...`` its
