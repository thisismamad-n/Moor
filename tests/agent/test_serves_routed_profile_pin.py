"""A host that mirrors the served profile into MOOR_HOME must not flip launch-home identity.

Moor WebUI serves several profiles from one process and, for legacy readers, mirrors the active
turn's profile into ``os.environ["MOOR_HOME"]`` while also installing the context-local override.
...
``moor_constants.pin_process_moor_home`` gives such hosts one stable anchor for all of them.
"""
...
from moor_cli.env_loader import _process_moor_home
from tools.environments.local import _is_routed_home
from tools.mcp_tool_scope import _server_key


def _under(home, fn):
    """Run *fn* with *home* installed as the task's moor-home override."""
    token = moor_constants.set_moor_home_override(home)
    try:
        return fn()
    finally:
        moor_constants.reset_moor_home_override(token)
...
ROUTED = {
    ...
    "env_loader._process_moor_home": lambda home: _process_moor_home().resolve() != Path(home).resolve(),
...
def homes(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(moor_constants, "_PINNED_PROCESS_MOOR_HOME", None)
    return launch, served


@pytest.mark.parametrize("decision", sorted(ROUTED))
def test_pinned_launch_home_survives_a_mirrored_moor_home(homes, monkeypatch, decision):
    launch, served = homes
    routed = ROUTED[decision]
    moor_constants.pin_process_moor_home(launch)
    monkeypatch.setenv("MOOR_HOME", str(served))  # the host's per-turn mirror
    ...
    assert _under(served, lambda: _server_key("atlassian")) == (moor_constants.moor_home_key(served), "atlassian")
    # Process-asset readers keep following the env var: only routing decisions use the pin.
    assert moor_constants.get_process_moor_home() == served
    assert moor_constants.get_routing_process_moor_home() == launch


@pytest.mark.parametrize("decision", sorted(ROUTED))
def test_unpinned_or_cleared_pin_keeps_moor_home_semantics(homes, monkeypatch, decision):
    launch, served = homes
    routed = ROUTED[decision]
    for _ in ("never pinned", "pinned then cleared"):
        ...
        assert routed(served) is True
        assert routed(launch) is False
        monkeypatch.setenv("MOOR_HOME", str(served))  # mirrored: the env var IS the launch home
        assert routed(served) is False
        moor_constants.pin_process_moor_home(launch)
        moor_constants.pin_process_moor_home(None)
