...
``moor_constants.pin_process_moor_home`` (salvaged from #119129) gives the four routed-profile
...
* ``get_process_moor_home()`` / ``get_moor_home()`` keep following ``MOOR_HOME`` while the
...
def homes(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(moor_constants, "_PINNED_PROCESS_MOOR_HOME", None, raising=False)
...
def test_multiplex_activation_pins_the_launch_home_but_env_readers_still_follow_the_env(homes, monkeypatch):
    ...
    assert get_routing_process_moor_home() == launch
    # Only routing DECISIONS are frozen; override-less readers see what the env names.
    assert get_process_moor_home() == served
    assert get_moor_home() == served
    set_multiplex_active(False)
    # The auto-pin is released with the mode: a standalone process follows the env again.
    assert get_routing_process_moor_home() == served
    token = set_moor_home_override(served)
    try:
...
def test_an_explicit_host_pin_survives_a_transient_multiplex_toggle(homes, monkeypatch):
    ...
    monkeypatch.setenv("MOOR_HOME", str(served))
    token = set_moor_home_override(served)
    try:
        set_multiplex_active(True)
        set_multiplex_active(False)
