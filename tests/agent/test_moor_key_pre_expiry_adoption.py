"""Moor agent keys live ~1 h. Every agent in a process must not learn about expiry from its own 401.
...
class _Agent(ClientLifecycleMixin):
    def __init__(self, key):
        self.provider, self.api_mode, self.api_key, self.base_url = "moor", "chat_completions", key, "https://inference-api.nousresearch.com/v1"
...
def test_key_far_from_expiry_is_left_alone_without_touching_the_store():
    ...
    with patch("moor_cli.auth.resolve_moor_runtime_credentials", side_effect=AssertionError("must not hit the store")):
        assert agent._adopt_moor_key_before_expiry() is False
...
def test_key_inside_the_skew_adopts_the_stores_fresh_key_without_forcing_a_refresh():
    ...
    with patch("moor_cli.auth.resolve_moor_runtime_credentials", side_effect=resolve):
        assert agent._adopt_moor_key_before_expiry() is True
    assert calls[0]["force_refresh"] is False  # the keepalive/peer refresh is adopted, never re-minted
    assert agent.adopted == [(fresh, "moor_credential_refresh")]
...
def test_a_fresh_key_for_a_different_account_is_never_adopted():
    ...
    with patch("moor_cli.auth.resolve_moor_runtime_credentials", return_value={"api_key": other, "base_url": agent.base_url}):
        assert agent._adopt_moor_key_before_expiry() is False
...
def test_same_key_back_from_the_store_is_not_readopted():
    ...
    agent = _Agent(key)
    with patch("moor_cli.auth.resolve_moor_runtime_credentials", return_value={"api_key": key, "base_url": agent.base_url}):
        assert agent._adopt_moor_key_before_expiry() is False
    assert agent.adopted == []


def test_keepalive_thread_starts_when_an_agent_routes_to_moor(monkeypatch, tmp_path):
    """Real construction path: the CLI process builds agents through AIAgent, never through the gateway boot."""
    from run_agent import AIAgent

    monkeypatch.setenv("MOOR_HOME", str(tmp_path / "hh"))
    started = []
    monkeypatch.setattr("moor_cli.moor_auth_keepalive.start_moor_auth_keepalive", lambda: started.append(1))
    AIAgent(api_key="k", base_url="https://inference-api.nousresearch.com/v1", provider="moor",
