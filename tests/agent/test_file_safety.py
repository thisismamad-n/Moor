...
class TestCacheFileReadBlocking:
    ...
    def test_hub_index_cache_blocked(self, tmp_path):
        ...
        cache = moor_home / "skills" / ".hub" / "index-cache" / "data.json"
        cache.parent.mkdir(parents=True)
        cache.write_text("{}")

        with patch("agent.file_safety._moor_home_path", return_value=moor_home):
    ...
    def test_hub_directory_blocked(self, tmp_path):
        ...
        moor_home = tmp_path / ".moor"
        hub = moor_home / "skills" / ".hub" / "metadata.json"
...
class TestCombinedGuards:
    ...
    def test_env_guard_works_regardless_of_moor_home(self, tmp_path):
        """The env basename guard does not depend on MOOR_HOME resolution."""
        moor_home = tmp_path / ".moor"
        moor_home.mkdir()
    ...
    def test_cache_guard_still_works_with_env_guard(self, tmp_path):
        ...
        moor_home = tmp_path / ".moor"
        cache = moor_home / "skills" / ".hub" / "index-cache" / "x"
        cache.parent.mkdir(parents=True)
        cache.write_text("")
