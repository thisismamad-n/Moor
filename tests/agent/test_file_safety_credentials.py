"""Tests for MOOR_HOME credential-file read blocking in file_safety.

Regression for https://github.com/thisismamad-n/Moor/issues/17656 —
``read_file`` was previously only sandboxed against ``MOOR_HOME`` itself,
...
for the credential stores while leaving arbitrary ``MOOR_HOME`` files
...
def fake_home(tmp_path, monkeypatch):
    """Point ``_moor_home_path()`` at a tmp dir for isolated checks."""
...
def test_arbitrary_moor_home_file_not_blocked(fake_home):
    """Non-credential files inside MOOR_HOME stay readable."""
...
def test_identically_named_moor_files_outside_home_not_blocked(
    ...
    project.mkdir()
    # auth.json outside MOOR_HOME — readable (per-location gate).
    p = project / "auth.json"
    p.write_text("not secret here", encoding="utf-8")
    assert get_read_block_error(str(p)) is None, (
        "auth.json outside MOOR_HOME should NOT be blocked"
    )
...
def test_profile_mode_blocks_root_credentials(tmp_path, monkeypatch):
    """Under a profile, MOOR_HOME = <root>/profiles/<name>, but
