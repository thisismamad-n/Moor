...
path via ``moor chat --query``) couldn't see what the aggregator actually
produced without joining to the session DB by hand.

These exercise the real ``consume_and_save_trace`` → ``save_moa_turn`` path
with real file I/O against a temp MOOR_HOME — no mocks on the write path.
"""
...
def _enable_traces(tmp_path, monkeypatch):
    """Point MOOR_HOME at a temp dir and turn moa.save_traces on."""
    moor_home = tmp_path / ".moor"
    moor_home.mkdir()
    monkeypatch.setenv("MOOR_HOME", str(moor_home))

    # save_moa_turn reads config via moor_cli.config.load_config; stub it to
