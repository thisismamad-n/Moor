...
Reference: May 2026 incident — a moor-security profile session
accidentally edited skills under both ~/.moor/profiles/moor-security/skills/
AND ~/.moor/skills/ (the default profile's skills), realizing only
...
class TestClassifyCrossProfileTarget:
    ...
    def test_security_writing_default_skill(self, fake_moor, monkeypatch):
        """The exact incident from May 2026."""
        _set_active_home(monkeypatch, fake_moor["security_home"])
        from agent.file_safety import classify_cross_profile_target
        result = classify_cross_profile_target(
            str(fake_moor["default_home"] / "skills" / "foo" / "SKILL.md")
        )
        assert result is not None
        assert result["active_profile"] == "moor-security"
        assert result["target_profile"] == "default"
        assert result["area"] == "skills"

    def test_default_writing_security_skill(self, fake_moor, monkeypatch):
        """Inverse direction — default-profile session reaching into a named profile."""
        _set_active_home(monkeypatch, fake_moor["default_home"])
        from agent.file_safety import classify_cross_profile_target
        result = classify_cross_profile_target(
            str(fake_moor["security_home"] / "skills" / "foo" / "SKILL.md")
        )
        assert result is not None
        assert result["active_profile"] == "default"
        assert result["target_profile"] == "moor-security"


    @pytest.mark.parametrize("area", ["skills", "plugins", "cron", "memories"])
    def test_all_profile_scoped_areas_classified(self, fake_moor, monkeypatch, area):
        _set_active_home(monkeypatch, fake_moor["security_home"])
        from agent.file_safety import classify_cross_profile_target
        target = fake_moor["default_home"] / area / "foo.txt"
...
class TestGetCrossProfileWarning:
    def test_in_profile_returns_none(self, fake_moor, monkeypatch):
        _set_active_home(monkeypatch, fake_moor["security_home"])
    ...
    def test_cross_profile_warning_names_both_profiles(self, fake_moor, monkeypatch):
        _set_active_home(monkeypatch, fake_moor["security_home"])
        from agent.file_safety import get_cross_profile_warning
        ...
        assert "moor-security" in warn
        # Must name the bypass kwarg.
        assert "cross_profile=True" in warn
        # Must reference the area.
        assert "skills" in warn

    def test_warning_is_defense_in_depth_not_boundary(self, fake_moor, monkeypatch):
        _set_active_home(monkeypatch, fake_moor["security_home"])
        from agent.file_safety import get_cross_profile_warning
        warn = get_cross_profile_warning(
            str(fake_moor["default_home"] / "skills" / "foo" / "SKILL.md")
