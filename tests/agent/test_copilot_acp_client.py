...
class CopilotACPClientSafetyTests(unittest.TestCase):
    ...
    def test_write_text_file_respects_safe_root(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with patch.dict(os.environ, {"MOOR_WRITE_SAFE_ROOT": str(safe_root)}, clear=False):
                response = self._dispatch(
                    {
                        "jsonrpc": "2.0",
                        "id": 5,
        ...
        self.assertIn("MOOR_WRITE_SAFE_ROOT", str(response["error"]))
...
def test_run_prompt_preserves_real_home_when_profile_home_available(monkeypatch, tmp_path):
    ...
    (moor_home / "home").mkdir(parents=True)
    real_home = tmp_path / "real-home"
    real_home.mkdir()

    monkeypatch.setenv("HOME", str(real_home))
    monkeypatch.setenv("MOOR_HOME", str(moor_home))
    # Hermeticity: an ambient MOOR_REAL_HOME (exported by Moor' own
    ...
    assert captured["kwargs"]["env"]["MOOR_REAL_HOME"] == str(real_home)
...
# `session/set_model` call. Without it, picking gpt-5.6-terra in Moor
