...
thread that did not bind the MOOR_HOME ContextVar.
...
active-profile line resolved the home via the ambient ``get_moor_home()``,
so an unbound build thread fell back to ``~/.moor`` (default) and leaked
...
def test_skills_prompt_scoped_to_override_not_ambient_home(tmp_path, monkeypatch):
    """An explicit skills_dir_override wins over ambient MOOR_HOME, on a
    ...
    def build():
        # No set_moor_home_override on THIS thread — ambient resolves to
...
def test_profile_name_correct_on_bound_profile_session(tmp_path, monkeypatch):
    ...
    ``get_moor_home()/profiles`` as the root would never match and every
    profile would misreport as \"default\". The name must derive from the
    moor ROOT (get_default_moor_root)."""
    from agent import system_prompt

    bot_home = tmp_path / "profiles" / "mybot"
    bot_home.mkdir(parents=True)

    # Bound session: MOOR_HOME env points at the profile dir itself.
    monkeypatch.setenv("MOOR_HOME", str(bot_home))
