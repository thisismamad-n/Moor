...
def test_bounded_fast_window_policy(monkeypatch):
    ...
    for provider, base_url in (
        ...
        ("moor", "https://inference-api.nousresearch.com/v1"),
...
def test_fast_auto_and_cold_parse_and_slash_command(monkeypatch):
    ...
    cli_mod.MoorCLI._handle_fast_command(stub, "/fast auto")
    assert stub.service_tier == "auto"
    assert stub.agent is None
    cli_mod.MoorCLI._handle_fast_command(stub, "/fast status")
    assert any("auto" in line for line in printed)
    cli_mod.MoorCLI._handle_fast_command(stub, "/fast cold")
    ...
    assert cli_mod.MoorCLI._resolve_turn_agent_config(route_stub, "hi")["request_overrides"] == {
        "service_tier": "priority"
    }
