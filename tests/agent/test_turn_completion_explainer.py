...
def _make_agent(max_iterations: int = 10, config: dict | None = None) -> AIAgent:
    with (
        ...
        patch("moor_cli.config.load_config", return_value=config or {}),
...
def test_explanation_persistence_turn_lease_cause_is_specific():
    ...
    assert "moor doctor" not in lower
...
def test_persistence_commands_are_pinned_to_the_failing_profile(monkeypatch, tmp_path, cause):
    """Every copy-pasteable ``moor`` command in a persistence explanation names the profile
    whose store failed — a multi-profile backend serves sessions whose state.db is not the
    process default, and a bare ``moor`` follows the sticky active_profile (#105887). The
    ...
    assert f"`moor {selector}doctor" in out
    assert "`moor doctor" not in out and "`moor gateway" not in out
...
def test_explanation_persistence_one_arg_backward_compat():
    ...
    assert out.strip() != ""
    assert "moor doctor" in out.lower()
...
def test_classify_persistence_error_reuses_disk_full_markers():
    """The disk bucket delegates to moor_state_errors.is_disk_full_error, so
...
def test_classify_persistence_error_compression_busy_is_distinct():
    ...
    from moor_state import SessionCompressionInProgressError
...
def test_explainer_enabled_by_default():
    ...
    with patch.dict(os.environ, {}, clear=False):
        os.environ.pop("MOOR_TURN_COMPLETION_EXPLAINER", None)
...
def test_explainer_disabled_via_env():
    agent = _make_agent()
    with patch.dict(
        os.environ, {"MOOR_TURN_COMPLETION_EXPLAINER": "0"}, clear=False
    ):
...
def test_classify_persistence_error_quarantined_handle_is_corrupt() -> None:
    ...
    from moor_state import StateDbCorruptError, classify_persistence_error
