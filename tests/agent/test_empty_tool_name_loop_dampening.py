...
@pytest.fixture()
def agent_env():
    """Spin up the mock provider + an isolated MOOR_HOME, yield (agent, helpers)."""
    ...
    test_home = tempfile.mkdtemp(prefix="moor_e2e_47967_")
    os.makedirs(os.path.join(test_home, ".moor"))
    prev_home = os.environ.get("MOOR_HOME")
    os.environ["MOOR_HOME"] = os.path.join(test_home, ".moor")
    ...
    for mod in list(sys.modules):
        if mod == "run_agent" or mod.startswith("agent.") or mod.startswith("tools.") or mod.startswith("moor_"):
            del sys.modules[mod]
    ...
    try:
        ...
        # references into moor_cli.*/tools.* and their monkeypatches would otherwise
        ...
        if prev_home is None:
            ...
            os.environ["MOOR_HOME"] = prev_home
