...
def test_codex_pool_refresh_holds_auth_store_lock_across_post(monkeypatch, tmp_path):
    ...
    Codex refresh tokens are single-use. If two Moor processes both read the
...
def test_moor_pkce_refresh_writes_back_to_singleton(tmp_path, monkeypatch):
    """A successful moor_pkce refresh must update
    ~/.moor/.anthropic_oauth.json, or ``_seed_from_singletons()`` on the
    ...
    oauth_file = moor_home / ".anthropic_oauth.json"
    oauth_file.write_text(
        json.dumps({"accessToken": "sk-ant-oat-rt0", "refreshToken": "rt0", "expiresAt": 0}),
        encoding="utf-8",
    )
    _write_store(moor_home / "auth.json", {"version": 1, "providers": {}})
    ...
    assert on_disk["refreshToken"] == "rt1", (
        "successful moor_pkce refresh must write back to "
        "~/.moor/.anthropic_oauth.json, or _seed_from_singletons() will "
        "revert the pool entry to the pre-refresh (spent) token on next load"
    )

    reloaded = load_pool("anthropic")
    reloaded_entries = [e for e in reloaded.entries() if e.source.endswith("moor_pkce")]
    assert reloaded_entries, "moor_pkce entry should still be present after reload"
...
def test_manual_moor_pkce_refresh_does_not_create_duplicate_singleton(
    tmp_path, monkeypatch
):
    """A pool-owned manual:moor_pkce entry must not create a second source."""
    moor_home = tmp_path / "moor"
    moor_home.mkdir(parents=True, exist_ok=True)
    monkeypatch.setenv("MOOR_HOME", str(moor_home))
    monkeypatch.setattr("moor_cli.auth.is_provider_explicitly_configured", lambda pid: True)
    ...
    assert not oauth_file.exists(), (
        "manual:moor_pkce is already pool-owned; refreshing it must not "
        "create a second moor_pkce singleton source"
    )

    reloaded = load_pool("anthropic")
    matching = [e for e in reloaded.entries() if e.id == "manual-entry"]
    assert len(matching) == 1
    assert matching[0].source == "manual:moor_pkce"
