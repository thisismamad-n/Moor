"""A Moor 401 refresh must replace the auxiliary client under the SAME cache key
...
MOOR_BASE_URL = "https://inference-api.nousresearch.com/v1"
...
def _moor_mock_client(*, async_mode, raises=None, returns=None):
    """A stand-in OpenAI client whose ``chat.completions.create`` 401s or returns."""
    client = MagicMock()
    client.base_url = MOOR_BASE_URL
...
def test_call_llm_auto_provider_evicts_stale_client_end_to_end(monkeypatch):
    ...
    """
    task = "compression"
    stale = _moor_mock_client(async_mode=False, raises=_Auth401("stale creds"))
    fresh = _moor_mock_client(async_mode=False, returns={"ok": True})
    ...
    monkeypatch.setattr(
        ...
        lambda *a, **k: (stale, "moor-model"),
    )
    # The 401 refresh rebuilds a fresh client from refreshed runtime creds.
    monkeypatch.setattr(
        ac, "_resolve_moor_runtime_api",
        lambda *, force_refresh=False, stale_access_token=None: ("fresh-key", MOOR_BASE_URL),
    )
    monkeypatch.setattr(
...
async def test_async_call_llm_auto_provider_evicts_stale_client_end_to_end(monkeypatch):
    ...
    """
    task = "session_search"
    main_runtime = {"provider": "moor", "model": "Hermes-4-405B"}
    stale = _moor_mock_client(async_mode=True, raises=_Auth401("stale creds"))
    fresh = _moor_mock_client(async_mode=True, returns={"ok": True})
    ...
    monkeypatch.setattr(ac, "_to_async_client", lambda *a, **k: (fresh, "moor-model"))
    monkeypatch.setattr(ac, "_validate_llm_response", lambda resp, _task: resp)
