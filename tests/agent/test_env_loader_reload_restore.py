"""load_moor_dotenv() reloads must not clobber values an external secret source resolved (#74265).

The gateway module import, per-turn reloads and cron fires all call ``load_moor_dotenv()`` again;
...
def test_source_secret_survives_second_load_moor_dotenv(tmp_path, monkeypatch):
    ...
    env_loader.load_moor_dotenv(moor_home=home)
    assert os.environ["GLM_API_KEY"] == "vault-value", (
        "second load_moor_dotenv() wrote the .env placeholder back over the source-resolved value"
    )
