...
The official Codex endpoint must receive Moor' own harness identity, rather
...
These tests pin:
- the required Moor originator
- the versioned Moor User-Agent
...
class TestCodexCloudflareHeaders:
    ...
    def test_user_agent_advertises_moor_version(self):
...
class TestPrimaryClientWiring:
    ...
    def test_apply_client_headers_on_base_url_change(self):
        ...
        with patch("agent.process_bootstrap.OpenAI") as mock_openai:
            ...
            assert headers.get("originator") == "moor-agent"
