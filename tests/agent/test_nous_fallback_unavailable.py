"""Tests for Moor fallback local-availability suppression.

Blocker if Moor token material is missing locally: the fallback chain
should not repeatedly attempt Moor resolution; it must skip and continue
...
class TestMoorFallbackLocalAvailability:
    def test_missing_moor_token_is_skipped_once(self):
        """Moor fallback is skipped when no access/refresh token is stored."""
        agent = _make_agent(
            fallback_model=[
