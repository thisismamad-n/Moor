"""Tests for agent/moor_rate_guard.py — cross-session Moor Portal rate limit guard."""
...
class TestMoorRateLimitRemaining:
    """Test checking remaining rate limit time."""


    def test_returns_remaining_seconds_when_active(self, rate_guard_env):
        from agent.moor_rate_guard import record_moor_rate_limit, moor_rate_limit_remaining

        record_moor_rate_limit(headers={"x-ratelimit-reset-requests-1h": "600"})
        remaining = moor_rate_limit_remaining()
...
class TestClearMoorRateLimit:
    ...
    def test_clears_existing_file(self, rate_guard_env):
        ...
        record_moor_rate_limit(headers={"retry-after": "600"})
        assert moor_rate_limit_remaining() is not None
...
class TestAuxiliaryClientIntegration:
    ...
    def test_try_moor_skips_when_rate_limited(self, rate_guard_env, monkeypatch):
        from agent.moor_rate_guard import record_moor_rate_limit

        # Record a rate limit
        record_moor_rate_limit(headers={"retry-after": "600"})

        # Mock _read_moor_auth to return valid creds (would normally succeed)
...
class TestIsGenuineMoorRateLimit:
    ...
    Moor Portal multiplexes upstreams (DeepSeek, Kimi, MiMo, Moor).
    ...
    def test_exhausted_hourly_bucket_in_429_headers_is_genuine(self):
        from agent.moor_rate_guard import is_genuine_moor_rate_limit
    ...
    def test_bare_429_with_no_headers_is_upstream(self):
        ...
        assert is_genuine_moor_rate_limit(headers=None) is False
        assert is_genuine_moor_rate_limit(headers={}) is False
        assert is_genuine_moor_rate_limit(
    ...
    def test_last_known_state_all_healthy_stays_upstream(self):
        ...
        last_state = parse_rate_limit_headers(prior_headers, provider="moor")
        assert is_genuine_moor_rate_limit(
