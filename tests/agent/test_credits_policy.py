...
class TestIsFreeTierModel:
    ...
    def test_moor_welcome_host_is_free_without_pricing(self, monkeypatch):
        ...
        monkeypatch.setattr(models_pricing, "_pricing_cache", {})
        assert is_free_tier_model("moor/welcome", "https://welcome-api.nousresearch.com/v1") is True
        assert is_free_tier_model("some/other", "https://welcome-api.nousresearch.com") is True

    def test_paid_moor_host_still_needs_pricing_evidence(self, monkeypatch):
        """The free-tier rule is the host, not the model name: the paid inference host can serve
        moor/welcome to a named account, and a depleted named account still sees the notice."""
        from agent.credits_tracker import is_free_tier_model
        from moor_cli import models_pricing

        monkeypatch.setattr(models_pricing, "_pricing_cache", {})
        assert is_free_tier_model("moor/welcome", "https://inference-api.nousresearch.com/v1") is False
        assert is_free_tier_model("some/paid", "https://inference-api.nousresearch.com/v1") is False
        assert is_free_tier_model("moor/welcome", "") is False
