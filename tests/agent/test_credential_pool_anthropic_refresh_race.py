...
Moor *processes* via the cross-process ``_auth_store_lock`` flock,
...
Moor's own PKCE login (``manual:moor_pkce`` / ``moor_pkce``) got no
...
def _fake_pool_store(monkeypatch):
    ...
    This stands in for ``~/.moor/auth.json`` so the two "process-local"
...
def test_anthropic_refresh_is_protected_by_cross_process_lock(monkeypatch):
    ...
    pool = CredentialPool(
        "anthropic",
        [_entry(id="a1", access_token="stale-at", refresh_token="stale-rt", source="manual:moor_pkce")],
    )
...
def test_concurrent_moor_pkce_refresh_loses_credential_despite_valid_token_on_disk(monkeypatch):
    """Two 'Moor processes' race to refresh the same stale moor_pkce
    ...
    assert loser_result is not None, (
        ...
        "disk. moor_pkce-sourced entries must adopt the winner's token "
    ...
    assert loser_entry_after.last_status != STATUS_EXHAUSTED, (
        "regression: the losing process marked its Anthropic moor_pkce "
        "credential STATUS_EXHAUSTED after a lost refresh race, even "
        "though the account is not actually exhausted -- a sibling Moor "
        "process holds a perfectly valid rotated token. This causes "
        "spurious 're-authenticate with Anthropic' failures under "
        "ordinary concurrent Moor usage (fleet workers, cron jobs, "
...
def test_concurrent_claude_code_refresh_recovers_via_credentials_file(monkeypatch):
    ...
    evidence that moor_pkce/dashboard-sourced credentials were simply
