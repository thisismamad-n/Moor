...
_EXPECTED_USER_AGENT = f"openviking-memory-moor/{get_version_info().base_version}"
...
""",
    ...
    settings = openviking_module._resolve_connection_settings(
        openviking_module._load_moor_openviking_config()
    )
...
def test_post_setup_existing_profile_picker_validates_and_links_saved_profile(
    ...
    OpenVikingMemoryProvider().post_setup(str(moor_home), config)
...
def test_start_local_openviking_server_strips_pythonpath_from_child_env(monkeypatch):
    ...
    venv cannot be rebuilt during `moor update`.
    """
    popen_calls = []
    ...
    monkeypatch.setattr(openviking_module.shutil, "which", lambda name: "/usr/local/bin/openviking-server")
    monkeypatch.setattr(openviking_module.subprocess, "Popen", fake_popen)
    monkeypatch.setenv("PYTHONPATH", "/opt/moor/.venv/Lib/site-packages")
    monkeypatch.setenv("MOOR_PROFILE", "test-profile")
    ...
    assert child_env.get("MOOR_PROFILE") == "test-profile"
...
def test_tool_add_resource_rejects_moor_credential_file_upload(tmp_path, monkeypatch):
...
# ---------------------------------------------------------------------------
# on_session_switch — flush + commit + rotate behavior (moor-agent#28296)
# ---------------------------------------------------------------------------
...
def test_sync_turn_captures_session_id_before_worker_runs():
    ...
    assert captured_payloads == [{
        "messages": [
            ...
            {"role": "assistant", "parts": [{"type": "text", "text": "a"}], "peer_id": "moor"},
        ]
    }]
...
def test_session_needs_commit_guard_wins_over_stale_turn_count():
    """Regression for moor-agent#28296 review (M3): once a session is marked
...
def test_concurrent_providers_claim_unlocked_pending_owner_once(
    ...
    for provider in providers:
        ...
        provider._moor_home = str(tmp_path)
...
def test_is_available_false_without_any_endpoint(monkeypatch):
    _clear_openviking_env(monkeypatch)
    monkeypatch.setattr(
        openviking_module, "_load_moor_openviking_config", lambda: {}
    )
