"""Unit tests for the Moor Portal profile's reasoning wiring.
...
@pytest.fixture
def moor_profile():
    """Resolve the registered Moor profile through the real discovery path."""
    # ``model_tools`` triggers plugin discovery on import, which is what
    # registers the Moor profile in the global provider registry.
    import model_tools  # noqa: F401
    import providers

    profile = providers.get_provider_profile("moor")
    assert profile is not None, "moor provider profile must be registered"
    return profile
...
def portal_catalog(monkeypatch):
    ...
    monkeypatch.setattr(models_mod, "_moor_reasoning_caps_cache", {
...
class TestMoorReasoningWireShape:
    """``build_api_kwargs_extras`` produces the Portal's wire format."""

    def test_disable_reaches_optional_reasoning_model(self, moor_profile, portal_catalog):
        """The knob the user set is the knob the Portal receives."""
        extra_body, top_level = moor_profile.build_api_kwargs_extras(
    ...
    def test_disable_dropped_for_mandatory_reasoning_model(self, moor_profile, portal_catalog):
        """Mandatory routes 400 on a disable — send nothing instead."""
        extra_body, _ = moor_profile.build_api_kwargs_extras(
            reasoning_config={"enabled": False},
            supports_reasoning=True,
    ...
    def test_disable_dropped_for_unknown_model(self, moor_profile, portal_catalog):
    ...
    def test_disable_dropped_for_non_reasoning_route(self, moor_profile, portal_catalog):
        """A route the catalog says takes no reasoning parameter gets none.

        Moor' own ``supports_reasoning`` can disagree with the Portal about a
    ...
    def test_enabled_config_always_forwarded(self, moor_profile, portal_catalog, model):
    ...
    def test_no_config_defaults_to_medium(self, moor_profile, portal_catalog):
        extra_body, _ = moor_profile.build_api_kwargs_extras(
    ...
    def test_nothing_emitted_without_reasoning_support(self, moor_profile, portal_catalog):
        extra_body, top_level = moor_profile.build_api_kwargs_extras(
            reasoning_config={"enabled": False},
    ...
    def test_caller_config_not_mutated(self, moor_profile, portal_catalog):
        cfg = {"enabled": False}
        moor_profile.build_api_kwargs_extras(
