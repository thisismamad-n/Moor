"""Tests for the bundled moor-achievements dashboard plugin.
...
``plugins/moor-achievements/tests/`` covering the achievement engine
internals (tier math, secret-state handling, catalog invariants). These
tests live at the moor-agent level and focus on the integration
...
def plugin_api(tmp_path, monkeypatch):
    """Load plugin_api with isolated ~/.moor so state/snapshot files don't collide.
...
class _FakeSessionDB:
    """Stand-in for moor_state.SessionDB that records scan calls."""
