...
@pytest.fixture
def tmp_moor_home(tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
    home = tmp_path / "moor"
    home.mkdir()
...
def test_store_and_load_photon_token(tmp_moor_home: Path) -> None:
    photon_auth.store_photon_token("abc123def456")
    assert photon_auth.load_photon_token() == "abc123def456"

    auth_json = json.loads((tmp_moor_home / "auth.json").read_text())
...
def test_store_project_credentials_round_trip(
    tmp_moor_home: Path, monkeypatch: pytest.MonkeyPatch,
) -> None:
...
# cross-process lock moor_cli/auth.py's ~15 other writers all use, so a
...
def test_list_projects_unwraps_list(monkeypatch: pytest.MonkeyPatch) -> None:
    def fake_get(url: str, **kwargs: Any) -> _FakeResponse:
        return _FakeResponse(json_body=[{"id": "p1", "name": "Moor Agent"}])
...
def test_find_project_by_name_case_insensitive(monkeypatch: pytest.MonkeyPatch) -> None:
    def fake_get(url: str, **kwargs: Any) -> _FakeResponse:
        return _FakeResponse(json_body={"data": [
            {"id": "p1", "name": "Other"},
            {"id": "p2", "name": "moor agent"},
        ]})

    monkeypatch.setattr(photon_auth.httpx, "get", fake_get)
    proj = photon_auth.find_project_by_name("tok", "Moor Agent")
...
def test_create_project_omits_spectrum_flag(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(photon_auth.httpx, "post", fake_post)
    data = photon_auth.create_project("tok", name="Moor Agent")
    ...
    assert captured["body"]["name"] == "Moor Agent"
