...
Run from the PR worktree:
    ...
    cd ~/.moor/moor-agent/.worktrees/browser-providers-plugin
...
# checkout lives two levels up at ``~/.moor/moor-agent``.
MAIN_DIR = REPO_ROOT.parent.parent  # ~/.moor/moor-agent
PR_DIR = REPO_ROOT  # the worktree we're in
assert (MAIN_DIR / "tools" / "browser_tool.py").exists(), (
    f"MAIN_DIR={MAIN_DIR} doesn't look like a moor-agent checkout"
)
assert (PR_DIR / "tools" / "browser_tool.py").exists(), (
    f"PR_DIR={PR_DIR} doesn't look like a moor-agent checkout"
)
...
# Isolated MOOR_HOME for the config write.
