...
# worktree), ``MAIN_DIR`` falls back to a sibling ``moor-agent-main``
...
def _resolve_main_dir() -> Path:
    ...
    if (candidate / "tools" / "image_generation_tool.py").exists() and candidate != REPO_ROOT:
        return candidate
    sibling = REPO_ROOT.parent / "moor-agent-main"
...
# Isolated MOOR_HOME so the config write is hermetic.
...
SCENARIOS: list[tuple[str, str, dict[str, str]]] = [
    ...
    (
        ...
        "",
        {
            ...
            "TOOL_GATEWAY_USER_TOKEN": "moor-token",
        },
    ),
]
...
def main() -> int:
    ...
    if MAIN_DIR == PR_DIR:
        print(
            ...
            "      Set up a sibling 'moor-agent-main' checkout pinned to "
