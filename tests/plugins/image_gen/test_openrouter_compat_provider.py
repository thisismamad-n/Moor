#!/usr/bin/env python3
"""Tests for the OpenRouter-compatible image gen provider (OpenRouter + Moor)."""
...
_RUNTIME = "moor_cli.runtime_provider.resolve_runtime_provider"
...
class TestProviderClass:
    ...
    def test_display_names(self):
        ...
        assert by_name["moor"].display_name == "Moor Portal"
    ...
    def test_moor_honors_top_level_model(self):
        from plugins.image_gen.openrouter import _build_providers

        cfg = {"model": "openai/gpt-image-2"}
        moor = {p.name: p for p in _build_providers()}["moor"]
        with patch("plugins.image_gen.openrouter._load_image_gen_config", return_value=cfg):
            assert moor._resolve_model_chain() == ["openai/gpt-image-2"]
...
class TestLiveCatalog:
    ...
    def test_moor_portal_picker_excludes_image_api_catalog(self):
        """Moor Portal has no /images route; its picker must not offer
        Image-API-only models it cannot serve."""
        from plugins.image_gen.openrouter import _build_providers

        moor = {p.name: p for p in _build_providers()}["moor"]
        with patch(_RUNTIME, side_effect=RuntimeError("no creds")):
            ids = [m["id"] for m in moor.list_models()]
        from plugins.image_gen.openrouter import DEFAULT_MODEL, _FALLBACK_MODEL
...
class TestGenerate:
    ...
    def test_posts_to_resolved_base_url(self):
        """Moor routes to its own base URL — proves the same code serves both."""
        moor_runtime = _runtime_ok(
            provider="moor", base_url="https://inference.nousresearch.com/v1", api_key="moor-tok"
        )
        with patch(_RUNTIME, return_value=moor_runtime), \
             patch("requests.post", return_value=_mock_chat_response([_PNG_DATA_URI])) as mock_post, \
             patch("plugins.image_gen.openrouter.save_b64_image", return_value=Path("/tmp/x.png")):
            from plugins.image_gen.openrouter import _build_providers

            moor = {p.name: p for p in _build_providers()}["moor"]
            result = moor.generate(prompt="a pet")

        assert result["success"] is True
        assert result["provider"] == "moor"
...
class TestImageApiSurface:
    ...
    def test_moor_never_uses_the_image_api(self):
        """Moor Portal proxies chat-completions and has no /images route."""
        from plugins.image_gen.openrouter import _build_providers
        ...
        with patch(_RUNTIME, return_value=moor_runtime), \
             ...
             patch("plugins.image_gen.openrouter.save_b64_image", return_value=Path("/tmp/x.png")):
            moor = {p.name: p for p in _build_providers()}["moor"]
            result = moor.generate(prompt="a pet", model="openai/gpt-image-2")
...
class TestRegistration:
    def test_register_both(self):
        ...
        assert set(registered) == {"openrouter", "moor"}
