...
def test_root_session_no_compression(db):
    ...
    assert prov["currentMoorSessionId"] == "root1"
    assert prov["rootMoorSessionId"] == "root1"
    assert prov["parentMoorSessionId"] is None
...
def test_compression_split_continuation(db):
    ...
    prov = build_session_provenance(
        db, "acp-1", "new", previous_moor_session_id="old"
    )
    assert prov["sessionKind"] == "continuation"
    assert prov["parentMoorSessionId"] == "old"
    assert prov["rootMoorSessionId"] == "old"
    assert prov["compressionDepth"] == 1
    assert prov["previousMoorSessionId"] == "old"
...
def test_non_compression_parent_is_root_not_continuation(db):
    ...
    assert prov["sessionKind"] == "root"
    assert prov["compressionDepth"] == 0
    assert prov["rootMoorSessionId"] == "p"  # lineage root still walked
...
def test_meta_wrapper_shape(db):
    ...
    assert set(meta.keys()) == {"moor"}
    assert "sessionProvenance" in meta["moor"]
    assert meta["moor"]["sessionProvenance"]["currentMoorSessionId"] == "root1"
