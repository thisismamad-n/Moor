...
import moor_logging
# Use whatever RotatingFileHandler class moor_logging actually resolved so
# the autouse fixture's isinstance checks (which strip rotating handlers
# between tests) match the real handlers on every platform. moor_logging
...
from moor_logging import RotatingFileHandler
...
def _reset_logging_state():
    ...
    moor_logging._install_session_record_factory()
    yield
    ...
    moor_logging._logging_initialized = False
    moor_logging.clear_session_context()


@pytest.fixture
def moor_home(tmp_path, monkeypatch):
    """Provide an isolated MOOR_HOME for logging tests.

    Uses the same tmp_path as the autouse _isolate_moor_home from conftest,
...
class TestSetupLogging:
    ...
    def test_creates_log_directory(self, moor_home):
        log_dir = moor_logging.setup_logging(moor_home=moor_home)
        assert log_dir == moor_home / "logs"
        assert log_dir.is_dir()

    def test_creates_agent_log_handler(self, moor_home):
        moor_logging.setup_logging(moor_home=moor_home)
        root = logging.getLogger()

        agent_handlers = [
            h for h in moor_logging._queued_file_handlers
    ...
    def test_idempotent_no_duplicate_handlers(self, moor_home):
        moor_logging.setup_logging(moor_home=moor_home)
        moor_logging.setup_logging(moor_home=moor_home)  # second call — should be no-op
    ...
    def test_writes_to_agent_log(self, moor_home):
        moor_logging.setup_logging(moor_home=moor_home)

        test_logger = logging.getLogger("test_moor_logging.write_test")
        test_logger.info("test message for agent.log")

        # Flush handlers
        moor_logging.flush_log_queue()

        agent_log = moor_home / "logs" / "agent.log"
    ...
    def test_profile_routing_follows_context_home(self, moor_home, tmp_path):
        ...
        assert "profile-routed cron record" not in (
            moor_home / "logs" / "agent.log"
        ).read_text()

    def test_a_second_home_routes_instead_of_stacking_an_unfiltered_handler(self, moor_home, tmp_path):
        ...
        profile_home.mkdir()
        moor_logging.setup_logging(moor_home=moor_home)
        moor_logging.setup_logging(moor_home=profile_home)

        assert not [h for h in moor_logging._queued_file_handlers if isinstance(h, RotatingFileHandler)], (
        ...
        a_log = (moor_home / "logs" / "agent.log").read_text()
    ...
    def test_setup_for_an_already_routed_home_adds_no_duplicate_writer(self, moor_home, tmp_path):
        ...
        profile_home.mkdir()
        moor_logging.setup_logging(moor_home=moor_home)
        assert moor_logging.enable_profile_log_routing([moor_home, profile_home]) is True
        moor_logging.setup_logging(moor_home=profile_home)
        ...
        assert "once please" not in (moor_home / "logs" / "agent.log").read_text()

    def test_a_component_log_added_after_routing_is_routed_too(self, moor_home, tmp_path):
        ...
        moor_logging.setup_logging(moor_home=moor_home)
        moor_logging.setup_logging(moor_home=profile_home)
        moor_logging.setup_logging(moor_home=moor_home, mode="gateway")
        ...
        a_log = (moor_home / "logs" / "gateway.log").read_text()
    ...
    def test_explicit_params_override_config(self, moor_home):
        ...
        (moor_home / "config.yaml").write_text(yaml.dump(config))
...
class TestGatewayMode:
    ...
    def test_gateway_log_created(self, moor_home):
        moor_logging.setup_logging(moor_home=moor_home, mode="gateway")
        root = logging.getLogger()
    ...
    def test_gateway_log_not_created_in_cli_mode(self, moor_home):
        moor_logging.setup_logging(moor_home=moor_home, mode="cli")
        root = logging.getLogger()
    ...
    def test_gateway_log_receives_gateway_records(self, moor_home):
        ...
        gw_log = moor_home / "logs" / "gateway.log"
        assert gw_log.exists()
        assert "telegram connected" in gw_log.read_text()

    def test_gateway_log_rejects_non_gateway_records(self, moor_home):
...
class TestGuiMode:
    ...
    def test_gui_log_created(self, moor_home):
        moor_logging.setup_logging(moor_home=moor_home, mode="gui")
        root = logging.getLogger()
    ...
    def test_gui_log_receives_only_gui_components(self, moor_home):
        moor_logging.setup_logging(moor_home=moor_home, mode="gui")

        logging.getLogger("moor_cli.web_server").info("dashboard online")
        logging.getLogger("tui_gateway.ws").info("ws connected")
        logging.getLogger("gateway.run").info("gateway event")

        moor_logging.flush_log_queue()

        gui_log = moor_home / "logs" / "gui.log"
...
class TestSessionContext:
    ...
    def test_session_tag_in_log_output(self, moor_home):
        """When session context is set, log lines include [session_id]."""
        moor_logging.setup_logging(moor_home=moor_home)
        moor_logging.set_session_context("abc123")
...
class TestComponentFilter:
    ...
    def test_passes_matching_prefix(self):
        f = moor_logging._ComponentFilter(("gateway",))
...
class TestSetupVerboseLogging:
    ...
    def test_adds_stream_handler(self, moor_home):
        moor_logging.setup_logging(moor_home=moor_home)
        moor_logging.setup_verbose_logging()
        ...
        verbose_handlers = [
            ...
            and getattr(h, "_moor_verbose", False)
        ]
...
class TestAddRotatingHandler:
    ...
    def test_no_duplicate_for_same_path(self, tmp_path):
        ...
        moor_logging._add_rotating_handler(
            log_path,
    ...
    def test_no_session_filter_on_handler(self, tmp_path):
        ...
        handlers = [h for h in moor_logging._queued_file_handlers if isinstance(h, RotatingFileHandler)]
        ...
        moor_logging.set_session_context("factory_test")
    ...
    def test_managed_mode_initial_open_sets_group_writable(self, tmp_path):
        ...
        try:
            with patch("moor_cli.config.is_managed", return_value=True):
                moor_logging._add_rotating_handler(
                    log_path,
                    level=logging.INFO, max_bytes=1024, backup_count=1,
                    formatter=formatter,
...
class TestWindowsConcurrentLogLockTimeout:
    ...
    def _make_logger_and_handler(self, log_path: Path):
        ...
        handler = moor_logging._ManagedRotatingFileHandler(
    ...
    def test_helper_only_matches_windows_concurrent_lock_timeout(self):
        ...
        assert moor_logging._is_windows_concurrent_log_lock_timeout(
            RuntimeError("Cannot acquire lock after 20 attempts")
        )
        assert not moor_logging._is_windows_concurrent_log_lock_timeout(
...
class TestReadLoggingConfig:
    ...
    def test_returns_none_when_no_config(self, moor_home):
...
class TestExternalRotationRecovery:
    ...
    def _make_handler(self, log_path: Path) -> moor_logging._ManagedRotatingFileHandler:
        handler = moor_logging._ManagedRotatingFileHandler(
    ...
    def _emit(self, handler: logging.Handler, msg: str) -> None:
        ...
        # Match the record factory that moor_logging installs at import time.
        record.session_tag = ""
        handler.emit(record)
        moor_logging.flush_log_queue()
    ...
    def test_gateway_log_attached_after_external_rotation_then_re_setup(
        ...
        """
        moor_logging.setup_logging(moor_home=moor_home, mode="gateway")
        gw_path = moor_home / "logs" / "gateway.log"
...
class TestSafeStderr:
    ...
    def test_wraps_non_utf8_stderr(self, monkeypatch):
        ...
        result = moor_logging._safe_stderr()
...
class TestAsyncQueueLogging:
    ...
    def test_file_handlers_not_on_root(self, moor_home):
        moor_logging.setup_logging(moor_home=moor_home)
        root = logging.getLogger()
        ...
        queue_handlers = [
            h for h in root.handlers if getattr(h, "_moor_queue", False)
        ]
        assert len(queue_handlers) == 1
        # The real file handlers are discoverable via the accessor.
        assert any(
            "agent.log" in getattr(h, "baseFilename", "")
            for h in moor_logging._queued_file_handlers
        )
