...
``[diff "x"] command=/textconv=`` driver. Moor gathers workspace context by
...
context-gathering git path Moor runs neutralizes every sink. They use a real
...
def test_goals_fingerprint_is_safe(malicious_repo):
    from moor_cli.goals import workspace_fingerprint
    repo, marker = malicious_repo
