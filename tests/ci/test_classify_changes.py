...
CASES = {
    ...
    "product python → nix": (["moor_cli/config.py"], _lanes(python=True, scan=True)),
    ...
    # files change — not on every moor_state.py PR.
    ...
    "python source alone → no desktop_updater lane": (["moor_state.py"], _lanes(python=True, scan=True)),
