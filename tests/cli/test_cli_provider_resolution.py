...
from moor_cli import model_setup_flows
...
def test_provider_flag_uses_named_custom_default_model(monkeypatch):
    ...
    monkeypatch.setitem(
        cli.CLI_CONFIG,
        "model",
        {"default": "tencent/hy3:free", "provider": "moor"},
    )
    config = {
        "model": {"default": "tencent/hy3:free", "provider": "moor"},
        "providers": {
            "gmk-lan": {
                "name": "GMK Local",
    ...
    monkeypatch.setattr("moor_cli.runtime_provider.load_config", lambda: config)

    shell = cli.MoorCLI(provider="gmk-lan", compact=True, max_turns=1)
...
def test_moor_cli_init_does_not_eagerly_resolve_runtime_provider(monkeypatch):
    cli = _import_cli()
    calls = {"count": 0}

    def _unexpected_runtime_resolve(**kwargs):
        calls["count"] += 1
        raise AssertionError("resolve_runtime_provider should not be called in MoorCLI.__init__")

    monkeypatch.setattr("moor_cli.runtime_provider.resolve_runtime_provider", _unexpected_runtime_resolve)
    monkeypatch.setattr("moor_cli.runtime_provider.format_runtime_provider_error", lambda exc: str(exc))

    shell = cli.MoorCLI(model="gpt-5", compact=True, max_turns=1)
...
def test_model_flow_moor_does_not_restore_stale_custom_api_key(tmp_path, monkeypatch):
    ...
    config_home.mkdir()
    monkeypatch.setenv("MOOR_HOME", str(config_home))
    ...
    monkeypatch.setattr(
        ...
        lambda provider: {
            "access_token": "moor-token",
            "portal_base_url": "https://portal.example.com",
        },
    )
    monkeypatch.setattr(
        "moor_cli.auth.resolve_moor_runtime_credentials",
        lambda *args, **kwargs: {
    ...
    monkeypatch.setattr(
        "moor_cli.models.get_curated_moor_model_ids",
        lambda: [selected_model],
    )
    monkeypatch.setattr("moor_cli.models_pricing.get_pricing_for_provider", lambda provider: {})
    monkeypatch.setattr("moor_cli.models.check_moor_free_tier", lambda **kwargs: False)
    monkeypatch.setattr(
        "moor_cli.models.union_with_portal_paid_recommendations",
        lambda model_ids, pricing, portal_url: (model_ids, pricing),
    )
    monkeypatch.setattr(
        "moor_cli.auth._prompt_model_selection",
        lambda *args, **kwargs: selected_model,
    )
    monkeypatch.setattr(
        "moor_cli.moor_subscription.prompt_enable_tool_gateway",
        lambda config: None,
    )

    moor_main._model_flow_moor(stale_config, current_model="glm-5.2")

    config = yaml.safe_load(config_path.read_text()) or {}
    model = config.get("model")
    assert model["provider"] == "moor"
...
def test_model_flow_custom_saves_verified_v1_base_url(monkeypatch, capsys):
    ...
    saved_env = {}
    monkeypatch.setattr("moor_cli.config.save_env_value", lambda key, value: saved_env.__setitem__(key, value))
    monkeypatch.setattr("moor_cli.auth._save_model_choice", lambda model: saved_env.__setitem__("MODEL", model))
    monkeypatch.setattr("moor_cli.auth.deactivate_provider", lambda: None)
    monkeypatch.setattr("moor_cli.main_provider_setup._save_custom_provider", lambda *args, **kwargs: None)
    monkeypatch.setattr(
    ...
    monkeypatch.setattr("moor_cli.config.save_config", lambda cfg: None)
...
def test_model_flow_custom_persists_selected_api_mode(monkeypatch):
    ...
    monkeypatch.setattr("moor_cli.auth._save_model_choice", lambda model: None)
    monkeypatch.setattr("moor_cli.auth.deactivate_provider", lambda: None)
    monkeypatch.setattr(
        "moor_cli.models.probe_api_models",
        lambda api_key, base_url: {
    ...
    saved_env = {}
    monkeypatch.setattr("moor_cli.config.load_config", lambda: saved_cfg)
    monkeypatch.setattr("moor_cli.config.save_config", lambda cfg: saved_cfg.update(cfg))
    monkeypatch.setattr(
        "moor_cli.config.save_env_value",
        lambda key, value: saved_env.__setitem__(key, value),
    )
    monkeypatch.setattr(
        "moor_cli.main_provider_setup._save_custom_provider",
    ...
    monkeypatch.setattr("builtins.input", lambda _prompt="": next(answers))
    monkeypatch.setattr("moor_cli.secret_prompt.masked_secret_prompt", lambda _prompt="": "test-key")

    moor_main._model_flow_custom({"model": {"provider": "custom"}})
...
def test_cmd_model_forwards_moor_login_tls_options(monkeypatch):
    monkeypatch.setattr(moor_main, "_require_tty", lambda *a: None)
    monkeypatch.setattr(
        "moor_cli.config.load_config",
        lambda: {"model": {"default": "gpt-5", "provider": "moor"}},
    )
    monkeypatch.setattr("moor_cli.config.save_config", lambda cfg: None)
    monkeypatch.setattr("moor_cli.config.get_env_value", lambda key: "")
    monkeypatch.setattr("moor_cli.config.save_env_value", lambda key, value: None)
    monkeypatch.setattr("moor_cli.auth.resolve_provider", lambda requested, **kwargs: "moor")
    monkeypatch.setattr("moor_cli.auth.get_provider_auth_state", lambda provider_id: None)
    monkeypatch.setattr(moor_main, "_prompt_provider_choice", lambda choices, **kwargs: 0)
    monkeypatch.setattr(moor_cli_main_provider_setup, "_prompt_provider_choice", lambda choices, **kwargs: 0)
    ...
    monkeypatch.setattr("moor_cli.auth._login_moor", _fake_login)

    moor_main.cmd_model(
        SimpleNamespace(
            portal_url="https://portal.nousresearch.com",
            inference_url="https://inference.nousresearch.com/v1",
            client_id="moor-local",
...
def test_auto_provider_name_localhost():
    from moor_cli.main_provider_setup import _auto_provider_name
...
def test_save_custom_provider_uses_provided_name(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        "moor_cli.config.load_config", lambda: yaml.safe_load(cfg_path.read_text()) or {},
    )
    saved = {}
    def _save(cfg):
        saved.update(cfg)
    monkeypatch.setattr("moor_cli.config.save_config", _save)
...
def test_save_custom_provider_references_the_key_instead_of_inlining_it(monkeypatch, tmp_path):
    ...
    saved = {}
    monkeypatch.setattr("moor_cli.config.save_config", lambda cfg: saved.update(cfg))

    _save_custom_provider(
        "http://localhost:11434/v1",
        api_key="sk-secret",
        name="Ollama",
        key_env="MOOR_CUSTOM_LOCALHOST_11434_API_KEY",
    )

    entry = saved["custom_providers"][0]
    assert entry["key_env"] == "MOOR_CUSTOM_LOCALHOST_11434_API_KEY"
...
def test_custom_endpoint_key_env_is_a_valid_posix_name_for_ip_endpoints():
    ...
    from moor_cli.config import _ENV_VAR_NAME_RE, custom_endpoint_key_env
