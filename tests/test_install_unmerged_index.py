...
The ``moor update`` Python path already clears the conflict with ``git reset``
...
def test_install_ps1_stops_venv_resident_processes_before_parking_venv() -> None:
    ...
    ``venv\\Scripts\\pythonw.exe -m moor_cli.main gateway run`` — image name
    ``pythonw``, not ``moor.exe`` — so the ``taskkill /IM moor.exe`` guard
    ...
    # The moor.exe tree-kill is preserved (kills spawned child processes too).
    assert 'taskkill /F /T /IM moor.exe' in text
