...
#   mock_start <workroot>      start, write config into $MOOR_HOME
#   mock_stop                  kill the background server
#
# Requires: ASSETS (e2e-assets dir), LOG_DIR, MOOR_HOME, ok/fail helpers.
...
EOF
  ...
  ok "provider 'mock' configured in $MOOR_HOME (api $url/v1)"
}
