...
#
#   moor chat -q ...        a real turn  -> sessions/ transcripts + state.db rows
#   moor auth add ...       a pooled credential -> auth.json
#   moor profile create ..  a second profile    -> profiles/<name>/**
#
...
#
# Requires: moor command, MOOR_HOME, ok/fail helpers. Sourced by the driver.
...
_user_state_help_has() {
  local moor="$1" flag="$2"
  "$moor" chat --help 2>&1 | grep -qF -- "$flag"
}

_user_state_db_sessions() {
  local py out
  py="$(_user_state_python)"
  out="$("$py" - "$MOOR_HOME/state.db" <<'PY' 2>/dev/null || true
import sqlite3, sys
try:
...
user_state_produce() {
  ...
  if _user_state_help_has "$moor" "--quiet" || _user_state_help_has "$moor" "-q"; then
    local rc=0
    MOOR_DISABLE_LAZY_INSTALLS=1 "$moor" chat -q "Reply with the single word: ok" \
  ...
  if ! "$moor" auth add --help >/dev/null 2>&1; then
    printf '  SKIP moor auth add does not exist on this ref; auth.json is not covered by this leg\n' >&2
  elif [ ! -f "$MOOR_HOME/auth.json" ]; then
    ...
    for provider in openrouter anthropic; do
      ...
      if [ -s "$MOOR_HOME/auth.json" ]; then
        added=true
        break
      fi
    done
    [ "$added" = true ] || fail "moor auth add failed for openrouter and anthropic; see $LOG_DIR/user-state-auth.log"
  ...
  # --- a second profile ----------------------------------------------------
  if ! "$moor" profile create --help >/dev/null 2>&1; then
    printf '  SKIP moor profile create does not exist on this ref; profiles/ is not covered by this leg\n' >&2
  elif [ ! -d "$MOOR_HOME/profiles/e2e-second" ]; then
    MOOR_DISABLE_LAZY_INSTALLS=1 \
      "$moor" profile create e2e-second > "$LOG_DIR/user-state-profile.log" 2>&1 \
      || fail "moor profile create failed; see $LOG_DIR/user-state-profile.log"
    [ -d "$MOOR_HOME/profiles/e2e-second" ] || fail "moor profile create produced no profile dir"
    ...
    printf '\nUser preference: preserve my e2e-second profile identity across upgrades.\n' \
      >> "$MOOR_HOME/profiles/e2e-second/SOUL.md"
