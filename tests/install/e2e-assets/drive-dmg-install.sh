#!/usr/bin/env bash
# Drive the moor-setup dmg bootstrap through its first-run GUI.
#
...
tell application "System Events"
  set procs to (every process whose name contains "Moor")
...
MOOR_BIN="$INSTALL_DIR/venv/bin/moor"
...
install_complete() {
  [ -d "$INSTALL_DIR/.git" ] && [ -x "$MOOR_BIN" ] && installed_app
}
...
while :; do
  if install_complete; then
    log "install landed: checkout + venv console script + Moor.app present"
  ...
  if ! kill -0 "$SETUP_PID" 2>/dev/null; then
    ...
    log "moor-setup exited (pid $SETUP_PID) before the install landed"
    exit 1
  fi
  err="$(bootstrap_error)"
  if [ -n "$err" ]; then
