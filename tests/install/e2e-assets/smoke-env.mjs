 ...
 * variables minus credentials, driver activation, and every MOOR_* override.
 * @param {NodeJS.ProcessEnv} inherited
 * @param {string} home
 * @param {string} userData
 * @returns {Record<string, string> & {HOME: string, USERPROFILE: string, MOOR_HOME: string,
 *   MOOR_DESKTOP_USER_DATA_DIR: string, XDG_CONFIG_HOME: string, XDG_DATA_HOME: string, XDG_CACHE_HOME: string}}
 */
export function smokeEnvironment(inherited, home, userData) {
  /** @type {Record<string, string>} */
  const clean = {};
  const keepMoor = new Set(['MOOR_TEST_INSTALL_REF', 'MOOR_TEST_INSTALL_REPO']);
  for (const [key, value] of Object.entries(inherited)) {
    ...
    if (!value || /(?:KEY|TOKEN|SECRET|PASSWORD|CREDENTIAL)/.test(name)
        ...
        || (name.startsWith('MOOR_') && !keepMoor.has(name))
  ...
  return {
    ...
    MOOR_HOME: home, MOOR_DESKTOP_USER_DATA_DIR: userData,
 ...
 * Copy only moor-owned connection contracts. Cloning Chromium's profile
 * carries browser locks and process state from the prior app into a supposedly
 * isolated launch. MOOR_HOME remains shared so the app updates the actual
 * installed runtime.
 *
 * @template {Record<string, string>} T
 * @param {T & {MOOR_DESKTOP_USER_DATA_DIR: string}} env
 * @returns {T & {MOOR_DESKTOP_USER_DATA_DIR: string}}
 */
export function isolateUpdateWindowEnvironment(env) {
  const source = env.MOOR_DESKTOP_USER_DATA_DIR;
  ...
  return { ...env, MOOR_DESKTOP_USER_DATA_DIR: isolated };
}
...
export function updateWindowEnvironment(inherited, root, origin) {
  const home = inherited.MOOR_HOME;
  const userData = inherited.MOOR_DESKTOP_USER_DATA_DIR;
  if (!home || !userData) throw new Error('Update chat requires isolated MOOR_HOME and MOOR_DESKTOP_USER_DATA_DIR');
  ...
  if (origin === 'source') {
    const editableRoot = inherited.MOOR_PYTHON_SRC_ROOT;
    if (editableRoot) {
      if (!path.isAbsolute(editableRoot) || fs.realpathSync(editableRoot) !== fs.realpathSync(root)) {
        throw new Error('Captured MOOR_PYTHON_SRC_ROOT differs from the installed source');
      }
      env.MOOR_PYTHON_SRC_ROOT = editableRoot;
    }
    for (const key of ['MOOR_DESKTOP_PYTHON', 'MOOR_DESKTOP_MOOR', 'MOOR_DESKTOP_MOOR_ROOT']) {
      if (inherited[key]) {
        if (!path.isAbsolute(inherited[key]) || (key !== 'MOOR_DESKTOP_PYTHON' && !within(root, inherited[key]))) throw new Error(`Captured ${key} escapes the installed source`);
