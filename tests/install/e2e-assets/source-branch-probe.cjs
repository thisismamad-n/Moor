...
function sourceProbeGit(userData, realGit, stagedUrl, platform = process.platform) {
  ...
  const args = [
    `url.${stagedUrl}.insteadOf=https://github.com/thisismamad-n/Moor.git`,
...
function prepareSourceBranchEnvironment(root, expectedSha, realGit, capturedEnv, launchEnv) {
  if (!realGit || !path.isAbsolute(realGit) || !fs.existsSync(realGit)) {
    throw new Error('source app-update requires MOOR_E2E_REAL_GIT for staged main')
  }
  ...
  launchEnv.MOOR_E2E_SOURCE_ROOT = install
  launchEnv.MOOR_E2E_SOURCE_URL = staged
  launchEnv.MOOR_E2E_SOURCE_REAL_GIT = realGit
  launchEnv.MOOR_E2E_SOURCE_GIT = sourceProbeGit(launchEnv.MOOR_DESKTOP_USER_DATA_DIR, realGit, staged)
  ...
  if (process.platform !== 'win32') {
    ...
    const launcher = path.join(install, '.moor', 'bin', 'moor')
    ...
    if (!launcherStat) {
      if (!fs.existsSync(path.join(install, 'pm', 'lock.json'))
          && fs.existsSync(path.join(install, 'venv', 'bin', 'moor'))
...
if [ "$1" = '--run-module' ] && [ "$2" = 'moor_cli.source_check' ] && check_root "$@"; then
  exec ${quote(original)} "$@" --git ${quote(launchEnv.MOOR_E2E_SOURCE_GIT)} --branch main
fi
exec ${quote(original)} "$@"
`, { mode: 0o700 })
    return
...
function branchProbeArgs(args, root, realGit) {
  ...
  if (args.length === 5 && args.slice(0, 4).join(' ') === '/d /v:off /s /c') {
    ...
    if (
      ...
      /["%&|<>^\r\n]/.test(realGit) ||
      !/^""[^"\r\n]+\.cmd" "--run-module" "moor_cli\.source_check" /i.test(command) ||
  ...
  const legacy = args[0] === '-c' && args[1]?.includes('runpy.run_path(str(p))')
    && args[1]?.includes('moor_cli/source_check.py')
  const managed = args[0] === '--run-module' && args[1] === 'moor_cli.source_check'
  if (!(legacy || managed)
...
if (process.env.MOOR_E2E_SOURCE_ROOT && process.env.MOOR_E2E_SOURCE_GIT) {
  const original = childProcess.execFile
  const select = args => {
    const selected = branchProbeArgs(args, process.env.MOOR_E2E_SOURCE_ROOT, process.env.MOOR_E2E_SOURCE_GIT)
  ...
  if (process.env.MOOR_E2E_SOURCE_URL) {
    const spawn = childProcess.spawn
    const staged = process.env.MOOR_E2E_SOURCE_URL
    childProcess.spawn = function (file, args, options) {
      ...
      if (/^git(?:\.exe)?$/i.test(path.basename(file)) &&
          path.resolve(options?.cwd || '') === process.env.MOOR_E2E_SOURCE_ROOT &&
        ...
        return spawn.call(this, process.env.MOOR_E2E_SOURCE_REAL_GIT, [
          '-c', `url.${staged}.insteadOf=https://github.com/thisismamad-n/Moor.git`,
