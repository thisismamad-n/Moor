// drive-update.cjs — launch the INSTALLED Moor.exe (real Electron desktop
...
//
//   node <this file> <path-to-Moor.exe> <proof-dir>
//
...
if (!exePath || !proofDir) {
  console.error('usage: node drive-update.cjs <Moor.exe> <proof-dir>')
  process.exit(1)
}
...
async function main() {
  ...
  // The real backend (`moor serve`) is booting underneath; give it time.
  ...
  // buttonless boot-progress card ("Starting Moor... 86%"), then the
  ...
  // The "Updating Moor — this window will close" overlay should appear,
  ...
  // the staged binary), which claims MOOR_HOME/.moor-update-in-progress
  ...
  // proved the desktop had exited and `moor update` was already running).
  //
  ...
  const moorHome = process.env.MOOR_HOME
  const markerPath = moorHome ? path.join(moorHome, '.moor-update-in-progress') : null
  const resultPath = moorHome ? path.join(moorHome, '.moor-update-result.json') : null
