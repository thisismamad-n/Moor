...
# and requires the checkout to land on this commit with a working `moor`.
#
...
# Keep sandbox state out of the default .moor-sandbox so a run never clobbers
...
SANDBOX_DIR_NAME=".moor-sandbox-e2e-$ROUTE"
export MOOR_DEV_SANDBOX_DIR="$SANDBOX_DIR_NAME"

SANDBOX_ROOT="$REPO_ROOT/$SANDBOX_DIR_NAME"
INSTALL_DIR="/home/moor/.moor/moor-agent"   # user-level layout (sandbox default)
FAKE_REMOTE="/work/repos/moor-agent.git"
...
# both together.
UPSTREAM_URL="${MOOR_DEV_SANDBOX_UPSTREAM:-https://github.com/NousResearch/hermes-agent.git}"
...
LOG_DIR="${MOOR_E2E_LOG_DIR:-$(mktemp -d -t moor-install-e2e-logs.XXXXXX)}"
...
# ── helpers ────────────────────────────────────────────────────────────────
# Does the INSTALLED moor accept FLAG on `moor update`?
#
...
# that silently rots. `moor update --help` is the same surface a user meets,
# and argparse prints every option it accepts.
update_supports() {
  local flag="$1"
  in_sandbox "moor update --help 2>&1" | grep -qF -- "$flag"
}
...
require_moor_works() {
  local when="$1" out
  out="$(in_sandbox "moor --version" 2>&1)" \
    || { printf '%s\n' "$out" >&2; fail "moor --version failed $when"; }
  printf '%s\n' "$out" | sed 's/^/    /'
  ok "moor runs $when"
}
...
require_moor_works 'after install'

# ── apply exactly one update route ─────────────────────────────────────────
case "$ROUTE" in
  update)
    step 'ROUTE: moor update'
    ...
    # moor which it accepts; older ones read the prompt from stdin, so close it.
    if update_supports --yes; then
      update_cmd="moor update --yes"
    else
      update_cmd="moor update </dev/null"
    fi
    if ! in_sandbox "cd $INSTALL_DIR && $update_cmd"; then
      collect_sandbox_logs update
      fail "moor update failed ($update_cmd)"
    fi
    require_landed_on_target 'moor update'
    require_moor_works 'after moor update'
    ;;
  installer)
    ...
    require_moor_works 'after installer re-run'
    ;;
esac
