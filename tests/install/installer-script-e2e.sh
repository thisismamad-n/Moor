...
#              install landed on OLD with a working `moor`
#   update     advance served main to HEAD, apply ONE update method, assert
#              the checkout landed on HEAD with a working `moor`
#
# Usage:
#   tests/install/installer-script-e2e.sh --update-method moor-update|installer-script|installer-script+desktop
...
case "$UPDATE_METHOD" in
  moor-update|installer-script|installer-script+desktop|moor-desktop-app-update) ;;
  *) echo "error: --update-method must be moor-update, installer-script, installer-script+desktop or moor-desktop-app-update, got '$UPDATE_METHOD'" >&2; exit 1 ;;
esac
...
WORK_ROOT="${RUNNER_TEMP:-${TMPDIR:-/tmp}}/moor-installer-script-e2e"
...
# Isolated HOME: the runner's real one may carry a preinstalled moor or a
# developer config, and old installer scripts hardcode $HOME/.moor (the
# MOOR_HOME env override is newer than tags we sample). GIT_CONFIG_GLOBAL
...
assert_desktop_artifact() {
  ...
  for cand in \
    ...
    "$release_dir/linux-unpacked/moor" \
    "$release_dir/mac-arm64/Moor.app" \
    "$release_dir/mac/Moor.app"; do
...
assert_checkout() {
  ...
  "$moor" --version 2>&1 | ts_prefix > "$LOG_DIR/version-$2.log" \
...
smoke_desktop() {
  ...
  # app: `moor desktop --build-only` runs the full desktop pipeline
  # (workspace install, renderer build, stamp write) and stops before the
  # launch -- the same call `moor update` itself makes. Probe the
  # INSTALLED moor for the flag rather than assuming this checkout's
  # surface: sampled OLD releases may predate `moor desktop` or
  # --build-only entirely, and for them the phase skips, loudly.
  local moor="$INSTALL_DIR/venv/bin/moor"
  if ! "$moor" desktop --help 2>/dev/null | grep -qF -- --build-only; then
    ok "moor desktop --build-only not supported at $1; skipping desktop smoke"
    return 0
  fi
  local rc=0
  (cd "$INSTALL_DIR" && "$moor" desktop --build-only < /dev/null 2>&1 \
    | ts_prefix > "$LOG_DIR/desktop-smoke-$1.log") || rc=$?
  log_group "moor desktop --build-only ($1) transcript" "$LOG_DIR/desktop-smoke-$1.log"
  [ "$rc" -eq 0 ] || fail "moor desktop --build-only ($1) exited $rc; transcript above"
  ok "moor desktop --build-only works at $1"
...
case "$UPDATE_METHOD" in
  moor-update)
    ...
    MOOR="$INSTALL_DIR/venv/bin/moor"
  ...
  moor-desktop-app-update)
    # The real user surface: `moor desktop` launches the app, the user
    ...
    # suite's own) is configured into MOOR_HOME exactly like the dev:mock
