...
def test_payload_sys_paths_order_repo_first(tmp_path):
    ...
    # Repo first — its moor_cli wins over anything stale in the venv
...
def test_configure_processes_site_pth_files(tmp_path):
    ...
    concurrent-log-handler → moor_logging.py's file handlers). The old
...
def test_configure_defaults_pycache_prefix_to_localappdata(tmp_path, monkeypatch):
    ...
    try:
        ns["configure"](str(tmp_path), environ=environ)
        expected = os.path.join(str(tmp_path / "lad"), "moor", "pycache")
...
def test_configure_defaults_pycache_prefix_to_home_cache(tmp_path, monkeypatch):
    """POSIX counterpart of the LOCALAPPDATA default: ~/.cache/moor-pycache
    ...
    try:
        ns["configure"](str(tmp_path), environ=environ)
        expected = os.path.join(str(tmp_path / "home"), ".cache", "moor-pycache")
        assert environ["PYTHONPYCACHEPREFIX"] == expected
        assert str(sys.pycache_prefix) == expected
    finally:
        sys.pycache_prefix = original
...
def _render(tmp_path: Path) -> Path:
    ...
    for placeholder, value in {
        "__MOOR_ENTRY_MODULE__": "moor_cli.main",
        "__MOOR_ENTRY_FUNC__": "main",
        "__MOOR_REPO_REL__": "../repo",
        "__MOOR_SITE_REL__": "../venv/Lib/site-packages",
    }.items():
        text = text.replace(placeholder, value)
    bin_dir = tmp_path / "bin"
    bin_dir.mkdir(parents=True, exist_ok=True)
    out = bin_dir / "moor.exe"
...
def test_rendered_wrapper_dispatches_to_the_entry_module(tmp_path):
    ...
    imports moor_cli.main off the payload paths, and returns main()'s
    exit code."""
    bin_dir = tmp_path / "bin"
    (bin_dir / ".." / "repo" / "moor_cli").mkdir(parents=True, exist_ok=True)
    (bin_dir / ".." / "venv" / "Lib" / "site-packages").mkdir(parents=True, exist_ok=True)
    (bin_dir / ".." / "repo" / "moor_cli" / "__init__.py").write_text("")
    (bin_dir / ".." / "repo" / "moor_bootstrap.py").write_text("READY = True\n")
    (bin_dir / ".." / "repo" / "moor_cli" / "main.py").write_text(
        "import sys\n"
        "assert sys.modules['moor_bootstrap'].READY\n"
        "def main():\n"
        "    assert sys.argv[0].endswith('moor.exe'), sys.argv[0]\n"
