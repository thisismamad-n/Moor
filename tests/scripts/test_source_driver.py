...
command=$(source_moor "$INSTALL_DIR")
"$command" 'literal argument'
rm "$INSTALL_DIR/venv/bin/moor"
test "$(source_moor "$INSTALL_DIR")" = "$command"
printf '#!/bin/sh\nexit 23\n' > "$command"
"$command" || test "$?" = 23
rm "$command"
if source_moor "$INSTALL_DIR"; then exit 91; fi
printf '#!/bin/sh\nprintf legacy\n' > "$INSTALL_DIR/venv/bin/moor"
chmod +x "$INSTALL_DIR/venv/bin/moor"
"$(source_moor "$INSTALL_DIR")"
# A PM checkout cannot hide missing publication behind its old venv.
mkdir -p "$INSTALL_DIR/pm"
printf '{}' > "$INSTALL_DIR/pm/lock.json"
if source_moor "$INSTALL_DIR"; then exit 92; fi
...
$exe = Join-Path $root '.moor/bin/moor.exe'
$cmd = Join-Path $root '.moor/bin/moor.cmd'
New-Item -ItemType Directory -Path (Split-Path $legacy), (Split-Path $exe) -Force | Out-Null
Set-Content $legacy 'legacy'
if ((Get-SourceMoor $root) -ne $legacy) { throw 'legacy fallback' }
Set-Content $cmd 'cmd'
if ((Get-SourceMoor $root) -ne $cmd) { throw 'published cmd' }
Set-Content $exe 'exe'
if ((Get-SourceMoor $root) -ne $exe) { throw 'exe precedence' }
Remove-Item $legacy
if ((Get-SourceMoor $root) -ne $exe) { throw 'requires legacy venv' }
Remove-Item $exe, $cmd
$refused = $false
try { Get-SourceMoor $root } catch { $refused = $true }
...
def test_observer_preserves_no_desktop_and_refuses_incomplete_app(tmp_path):
    ...
    (release / "moor").write_bytes(b"incomplete fixture")
    with pytest.raises(RuntimeError, match="desktop"):
        verify(root, "absent")
    with pytest.raises(RuntimeError, match="desktop"):
        verify(root, "present")
    assert (release / "moor").read_bytes() == b"incomplete fixture"
...
def test_pm_observer_accepts_ready_fixture_and_leaves_failed_fixture_untouched(tmp_path, fault, error):
    ...
    root = tmp_path / "source"
    for directory in ("pm", "moor_cli"):
        shutil.copytree(repo / directory, root / directory, ignore=shutil.ignore_patterns("__pycache__"))
    shutil.copy2(repo / "moor_constants.py", root / "moor_constants.py")
    (root / "moor_bootstrap.py").write_text("raise RuntimeError('bootstrap must not run')", encoding="utf-8")
    (root / "uv.lock").write_text("fixture locked graph", encoding="utf-8")
    home = tmp_path / "home"
    store = tmp_path / "store"
    env = dict(os.environ, HOME=str(home), MOOR_HOME=str(home), MOOR_RUNTIME_DIR=str(store),
               PYTHONDONTWRITEBYTECODE="1", MOOR_DISABLE_LAZY_INSTALLS="1")
    env.pop("MOOR_INSTALL_ROOT", None)
    path = os.pathsep.join(p for p in os.get_exec_path() if ".moor" not in Path(p).parts)
...
from moor_cli._launchers import ensure_install_launchers
...
ensure_install_launchers(root, root / '.moor/bin')
'''
...
for (const [product, out] of [['tui', 'ui-tui/dist'], ['web', 'moor_cli/web_dist'], ['desktop', 'apps/desktop/release/linux-unpacked/resources/app.asar.unpacked/dist']]) {
...
'''
    ...
    (root / "apps/desktop/release/linux-unpacked/moor").write_bytes(b"fixture executable")
    command = [sys.executable, "-B", str(ASSETS / "source_driver.py"), "--root", str(root),
               "--launcher", str(root / ".moor/bin/moor"), "--desktop", "present"]
    if fault == "no-desktop":
    ...
    if fault in {"uv-lock", "no-desktop"}:
        ...
        launcher = mint_launcher("moor", foreign, root / ".moor/bin", Path(sys.executable), None)
        assert launcher is not None
    elif fault == "missing-launcher":
        (root / ".moor/bin/moor").unlink()
    elif fault == "incomplete":
        (root / ".update-incomplete").write_text("incomplete", encoding="utf-8")
    else:
        damaged = root / "moor_cli/web_dist/index.html"
