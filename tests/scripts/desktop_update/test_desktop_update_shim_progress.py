...
def progress(tmp_path):
    ...
    status = tmp_path / "moor-update-status"
...
# Stands in for `moor update`, and reports the stage that was on screen
...
n="$(cat "$MOOR_TEST_CALLS" 2>/dev/null || echo 0)"; n=$((n + 1))
printf '%s' "$n" > "$MOOR_TEST_CALLS"
for f in "$TMPDIR"/moor-update-status.[0-9]*; do
  case "$f" in *.tmp) continue ;; esac
  cp "$f" "$MOOR_TEST_CAPTURE.$n" 2>/dev/null
done
exit "$(cat "$MOOR_TEST_EXITS.$n" 2>/dev/null || echo 0)"
"""
...
def _run_handoff(tmp_path, exits: dict[int, int]) -> list[dict]:
    ...
    env = {
        **os.environ,
        "TMPDIR": str(tmp_path),
        "MOOR_TEST_CAPTURE": str(capture),
        "MOOR_TEST_CALLS": str(calls),
        "MOOR_TEST_EXITS": str(tmp_path / "exits"),
    }
    env.pop("MOOR_HOME", None)  # Exercise the legacy install-parent fallback.
...
def test_update_gate_publishes_its_stage_before_running(tmp_path):
    """`moor update` is the longest wait in the hand-off and the one the
