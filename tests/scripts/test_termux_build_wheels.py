...
def test_source_pin_survives_resolution_build_and_offline_install(tmp_path):
    ...
    export.write_text(
        'moor-source-fixture==7.2.2 ; python_version < "3.11"\n'
        f'moor-source-fixture @ {source} ; python_version >= "3.11"\n', encoding="utf-8")
    resolved = tmp_path / "resolved.txt"
    builder.normalize_reqs(export, lock, resolved)
    specs = builder.load_entries(resolved)
    assert specs == {"moor-source-fixture": f" @ {source}"}
    reqs = tmp_path / "offline.txt"
    builder.write_reqs_file(resolved, reqs)
    assert "@" not in reqs.read_text(encoding="utf-8")
    assert 'moor-source-fixture==8.0.0 ; python_version >= "3.11"' in reqs.read_text(encoding="utf-8")
    ...
    assert len(list(wheels.glob("moor_source_fixture-8.0.0-*.whl"))) == 1
