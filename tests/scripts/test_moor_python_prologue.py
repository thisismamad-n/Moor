...
``scripts/_moor-python`` is the POSIX shebang target. Its one decision worth
pinning is staleness: pm stamps each dependency input's mtime beside the
installed-state file ``__MOOR_ACTIVATED`` names, and any input whose mtime
...
PROLOGUE = REPO_ROOT / "scripts" / "_moor-python"
...
def checkout(tmp_path: Path) -> Path:
    ...
    (root / "scripts" / "probe.py").write_text(
        "import os, sys\n"
        "print('target ran; sentinel =', os.environ.get('__MOOR_ACTIVATED'))\n",
        encoding="utf-8",
    )
    (root / "activate").write_text(
        "echo 'ACTIVATED' >&2\n"
        "export __MOOR_ACTIVATED=\"%s/state/facts.json\"\n"
...
def _run(root: Path, sentinel: str | None = "{root}/state/facts.json") -> str:
    ...
    env.pop("__MOOR_ACTIVATED", None)
    if sentinel is not None:
        env["__MOOR_ACTIVATED"] = sentinel.replace("{root}", _posix(root))
