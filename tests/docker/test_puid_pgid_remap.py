...
Build the real image and verify the actual runtime behavior:
  ...
  1. PUID/PGID env vars remap the moor user UID/GID at boot
  2. MOOR_UID/MOOR_GID take precedence over PUID/PGID aliases
...
def test_puid_pgid_remaps_moor_user(
    ...
    assert r.stdout.strip() == "1000", (
        f"expected moor UID 1000 after PUID remap, got: {r.stdout.strip()}"
    )
    ...
    assert r.stdout.strip() == "1000", (
        f"expected moor GID 1000 after PGID remap, got: {r.stdout.strip()}"
    )
...
def test_nas_low_uid_accepted(
    ...
    r = docker_exec_sh(container_name, "id -u moor", timeout=10)
    assert r.stdout.strip() == "99", (
        f"expected moor UID 99, got: {r.stdout.strip()}"
    )

    r = docker_exec_sh(container_name, "id -g moor", timeout=10)
    assert r.stdout.strip() == "100", (
        f"expected moor GID 100, got: {r.stdout.strip()}"
    )
...
def test_remap_enables_data_volume_writes(
    ...
    """After remap, the moor user must be able to write to /opt/data."""
    start_container(built_image, container_name, "PUID=1000", "PGID=1000")
