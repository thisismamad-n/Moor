...
Build the real image and verify the actual runtime behavior:
  ...
  3. --user <moor-uid> (10000) is allowed (supported non-root start)
"""
...
def test_arbitrary_user_uid_rejected(
    ...
    assert "MOOR_UID" in combined or "PUID" in combined, (
...
def test_user_pinned_to_moor_uid_works(
    built_image: str,
) -> None:
    """docker run --user 10000:10000 (the moor UID) must be allowed.
    ...
    assert r.returncode == 0, (
        f"--user 10000:10000 (moor UID) was rejected: {r.stderr[-500:]}"
    )
    assert "OK" in r.stdout