...
Override the image with ``MOOR_TEST_IMAGE`` env var to point at a pre-built
...
IMAGE_TAG = os.environ.get("MOOR_TEST_IMAGE", "moor-agent-harness:latest")
...
def built_image() -> str:
    ...
    Override with ``MOOR_TEST_IMAGE`` env var to point at a pre-built
...
# ---------------------------------------------------------------------------
# docker_exec — default to the unprivileged moor user
# ---------------------------------------------------------------------------
#
# Background: every Moor runtime path inside the container drops to UID
# 10000 (the ``moor`` user) via ``s6-setuidgid moor``. ``docker exec``
...
# fails (PermissionError swallowed) for moor, so a test that ran as root
...
# helpers below run every probe under ``-u moor`` unless a specific
...
def docker_exec(
    ...
    *args: str,
    ...
    """Run a command inside ``container`` as ``user`` (default: moor).
