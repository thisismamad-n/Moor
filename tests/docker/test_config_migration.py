...
Build the real image and verify: a config.yaml present in $MOOR_HOME
is migrated by docker_config_migrate.py on boot, running as the moor
user.
"""
...
def test_config_migration_runs_on_boot(
    ...
    """A config.yaml in $MOOR_HOME must be migrated on boot by
    docker_config_migrate.py, running as the moor user."""
    # Start container
    ...
    assert r.stdout.strip() == "moor", (
        f"config.yaml not owned by moor (migration may have run as root): "
        f"{r.stdout.strip()}"
    )
