...
print('PM interpreter and application dependencies load as moor')
"""
    result = subprocess.run(
        ["docker", "run", "--rm", "--network", "none", "--user", "moor",
         "--entrypoint", "/opt/moor/.venv/bin/python", built_image, "-c", probe],
...
with Image.open('/opt/moor/moor_cli/web_dist/favicon.ico') as image:
    image.load()
    assert image.width > 0 and image.height > 0
assert not Path('/opt/moor/node_modules/vite').exists()
assert not Path('/opt/moor/node_modules/esbuild').exists()
assert Path('/opt/moor/node_modules/typescript/bin/tsc').is_file()
"""
    result = subprocess.run(
        ["docker", "run", "--rm", "--network", "none", "--user", "moor",
         "--entrypoint", "/opt/moor/.venv/bin/python", built_image, "-c", probe],
