...
crash and a supervised dashboard alongside (when ``MOOR_DASHBOARD=1``).
...
These tests verify the three load-bearing properties of that redirect:
  ...
  2. ``--no-supervise`` / ``MOOR_GATEWAY_NO_SUPERVISE=1`` opts out.
  3. The supervised process itself does NOT recurse — the
     ``MOOR_S6_SUPERVISED_CHILD`` sentinel breaks the loop.

Every ``docker exec`` runs as ``moor`` per the conftest module
...
def _wait_for_gateway_or_exit(
    ...
    *,
    ...
    ``pgrep -f 'moor.*gateway'`` (the gateway is running) or
    ...
    while time.monotonic() < end:
        ...
        if status == "running":
            ...
            pgrep = docker_exec_sh(
                container, "pgrep -f 'moor.*gateway' >/dev/null 2>&1",
            )
...
def test_gateway_run_redirects_to_supervised(
    ...
    # process (main-wrapper.sh → python → moor gateway run) can take
    ...
    # (main-wrapper.sh → python → moor gateway run) can take well over
    ...
    # top), NOT the static main-moor service's sleep — a bare grep
    # for `sleep infinity` would false-positive on the main-moor
...
def test_supervised_gateway_does_not_recurse(
    ...
    """The MOOR_S6_SUPERVISED_CHILD sentinel must prevent the
    supervised ``moor gateway run`` from re-entering the redirect.
    ...
    gateway slot would never actually run a python ``moor gateway
    ...
    # (main-wrapper.sh → python → moor gateway run) can take well
    ...
    # started either — so we'd see the CMD's `moor gateway run`
    ...
    # `moor gateway run`. If the recursion guard fails, s6 would
    # respawn fresh `gateway run` processes on every cycle, leaving
    # multiple Python-process descendants under the gateway-default
    # supervise tree.
    r = docker_exec_sh(container_name, "ps -eo pid,cmd | grep -v grep | grep -E 'python.*moor.*gateway run' | wc -l")
    assert r.returncode == 0
    n = int(r.stdout.strip() or 0)
    assert n <= 1, (
        f"expected at most one supervised python `moor gateway run` "
    ...
    # CMD process (PID 17 typically). The static `main-moor`
...
def test_dashboard_supervised_when_env_set(
    ...
    """When ``MOOR_DASHBOARD=1`` is set, ``docker run <image> gateway
