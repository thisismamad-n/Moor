"""Runtime smoke test for Docker $MOOR_HOME/logs/gateways seeding.

Build the real image and verify logs/ and logs/gateways/ exist and are
owned by the moor user after container boot.
...
registered later runs its log service as the dropped moor user and
...
def test_logs_gateways_seeded_and_moor_owned(
    built_image: str, container_name: str,
) -> None:
    """logs/ and logs/gateways/ must exist and be owned by moor after boot."""
    ...
    assert "logs=moor" in r.stdout, (
        f"logs/ not owned by moor: {r.stdout}"
    )
    assert "gateways=moor" in r.stdout, (
        f"logs/gateways/ not owned by moor: {r.stdout}"
    )
