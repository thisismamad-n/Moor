...
Build the real image and verify at runtime:
  ...
  1. /opt/moor is not writable by the moor user (immutable install tree)
  2. PYTHONDONTWRITEBYTECODE and MOOR_DISABLE_LAZY_INSTALLS are set
  3. /opt/moor/.install_method contains "docker" (code-scoped stamp)
  4. $MOOR_HOME/.install_method is NOT stamped as "docker" by stage2
  5. A stale "docker" stamp in $MOOR_HOME is healed (removed) on boot
"""
...
def test_install_tree_not_writable_by_moor(
    built_image: str, container_name: str,
) -> None:
    """The moor user must not be able to modify /opt/moor.
    ...
    r = docker_exec_sh(
        container_name,
        # Try to create a file under /opt/moor as the moor user
        "touch /opt/moor/test_write 2>&1 && "
        "echo WRITE_SUCCEEDED || echo WRITE_FAILED",
        timeout=10,
    )
    assert "WRITE_FAILED" in r.stdout, (
        f"moor user can write to /opt/moor (install tree not immutable): "
        f"{r.stdout}"
    )
    ...
    r = docker_exec_sh(
        container_name,
        "touch /opt/moor/.venv/test_write 2>&1 && "
        "echo WRITE_SUCCEEDED || echo WRITE_FAILED",
        timeout=10,
    )
    assert "WRITE_FAILED" in r.stdout, (
        f"moor user can write to /opt/moor/.venv: {r.stdout}"
    )


def test_moor_disable_lazy_installs_and_dont_write_bytecode(
    built_image: str, container_name: str,
) -> None:
    """The container must set PYTHONDONTWRITEBYTECODE and
    MOOR_DISABLE_LAZY_INSTALLS=1 so no .pyc files are written to the
...
def test_stale_docker_stamp_in_home_is_healed_on_boot(
    ...
    """A stale 'docker' stamp left in $MOOR_HOME by an older image
    ...
    assert "HEALED" in r.stdout or r.stdout.strip() != "docker", (
        f"stale 'docker' stamp in $MOOR_HOME was not healed on boot: "
        f"{r.stdout}"
    )
