"""Tests for acp_adapter.server — MoorACPAgent ACP server."""
...
def agent(mock_manager):
    """MoorACPAgent backed by a mock session manager."""
    return MoorACPAgent(session_manager=mock_manager)
...
class TestSessionOps:
    ...
    async def test_new_session_returns_authenticated_cross_provider_model_state(self):
        ...
        with (
            ...
            patch("moor_cli.inventory.build_models_payload", return_value=payload) as build_payload,
        ):
...
class TestPrompt:
    ...
    async def test_prompt_binds_session_id_into_subprocess_env(self, agent, mock_manager):
        ...
        leaving the ``MOOR_SESSION_ID`` ContextVar bound to the explicit ""
        ...
        empty ``MOOR_SESSION_ID`` instead of the session's own id.
        """
        ...
        def _run(*args, **kwargs):
            ...
            captured["child"] = _make_run_env({}).get("MOOR_SESSION_ID")
...
class TestRegisterSessionMcpServers:
    ...
    async def test_refreshes_agent_tool_surface(self, agent, mock_manager):
        ...
        mock_defs.assert_called_once_with(
            enabled_toolsets=["moor-acp", "mcp-srv"],
            disabled_toolsets=None,
            quiet_mode=True,
        )
        assert state.agent.enabled_toolsets == ["moor-acp", "mcp-srv"]
