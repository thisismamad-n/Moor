"""Behavior tests for moor_state._wal_reset_repair_hint.
...
import moor_state_wal as moor_state


def _hint_for(method):
    with patch("moor_cli.config.detect_install_method", return_value=method), \
         patch("moor_cli.config.recommended_update_command_for_method",
               return_value=f"CMD-{method}"):
        return moor_state._wal_reset_repair_hint()
...
def test_git_install_gets_moor_managed_hint():
    assert _hint_for("git") == (
        "moor-managed installs can repair the embedded runtime with `CMD-git`"
    )
...
def test_hint_falls_back_when_detection_fails():
    with patch("moor_cli.config.detect_install_method",
               side_effect=RuntimeError("boom")):
        hint = moor_state._wal_reset_repair_hint()
