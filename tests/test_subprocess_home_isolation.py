...
Moor state stays profile-scoped through MOOR_HOME. Host subprocesses should
...
class TestGetSubprocessHome:
    """Unit tests for moor_constants.get_subprocess_home()."""

    def _host_mode(self, monkeypatch):
        monkeypatch.setattr(moor_constants, "is_container", lambda: False)
        monkeypatch.delenv("TERMINAL_HOME_MODE", raising=False)
        monkeypatch.delenv("MOOR_REAL_HOME", raising=False)
    ...
    def test_real_mode_repairs_parent_home_already_pointing_at_profile(self, tmp_path, monkeypatch):
        ...
        from moor_constants import get_subprocess_home, get_real_home
    ...
    def test_two_profiles_get_different_homes(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("MOOR_HOME", str(base / "alpha"))
        home_a = get_subprocess_home()

        monkeypatch.setenv("MOOR_HOME", str(base / "beta"))
...
class TestMakeRunEnvHomeInjection:
    ...
    def test_host_auto_preserves_real_home_when_profile_home_exists(self, tmp_path, monkeypatch):
        ...
        assert result["MOOR_REAL_HOME"] == str(real_home)
    ...
    def test_profile_mode_injects_profile_home_when_profile_home_exists(self, tmp_path, monkeypatch):
        ...
        assert result["HOME"] == str(moor_home / "home")
        assert result["MOOR_REAL_HOME"] == str(real_home)
    ...
    def test_no_injection_when_moor_home_unset(self, monkeypatch):
