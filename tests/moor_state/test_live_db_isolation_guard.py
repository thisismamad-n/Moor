...
developer's REAL ~/.moor/state.db, and a pytest-spawned process flipped
...
REAL_ROOT = moor_state._real_platform_state_root()
...
class TestProductionPathRefused:
    def test_explicit_production_db_path_raises(self):
        """SessionDB pointed at the real ~/.moor/state.db must fail hard."""
...
class TestHermeticPathsAllowed:
    ...
    def test_tmp_moor_home_default_resolution_works(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("MOOR_HOME", str(tmp_path / "hermetic-home"))
        monkeypatch.setattr(
            moor_state, "DEFAULT_DB_PATH", moor_state._IMPORT_DEFAULT_DB_PATH
        )
        db = SessionDB()
...
class TestBypassMarker:
    ...
    def test_bypass_marker_disables_state_db_guard(self):
        ...
        """
        moor_state._ensure_test_isolation(REAL_ROOT / "state.db")
...
class TestSubprocessChildCovered:
    def test_child_without_moor_home_is_refused(self, tmp_path):
        """A subprocess child of a test (no MOOR_HOME) must be blocked.
        ...
        env = {
            ...
            if k not in ("MOOR_HOME", "PYTEST_PLUGINS", "PYTHONPATH")
        }
    ...
    def test_child_with_tmp_moor_home_succeeds(self, tmp_path):
        """Same child, hermetic MOOR_HOME: must work — no false positive."""
        env = {
            k: v
            for k, v in os.environ.items()
        ...
        env["PYTEST_CURRENT_TEST"] = "tests/fake.py::test_child (call)"
        env["MOOR_HOME"] = str(tmp_path / "child-home")
        env["PYTHONPATH"] = str(Path(__file__).resolve().parents[2])
        code = (
            "from moor_state import SessionDB\n"
            "db = SessionDB()\n"
