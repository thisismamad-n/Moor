...
child at the same moment the child lost the ``MOOR_HOME`` redirect. That
pairing is exactly how fixture rows (dm:123 / chat-1 / wx-chat) landed in a
developer's production state.db.

``MOOR_TEST_ISOLATION`` is Moor's own marker: the hermetic conftest
exports it (value = the isolation root) before any test module imports, it
inherits into children by default, and ``moor_state`` honors it as a
...
try:
    ...
    fired = True
print(json.dumps({{
    "armed": moor_state_guard._running_under_pytest(),
    "fired": fired,
}}))
"""
...
def test_marker_alone_reports_test_context(monkeypatch):
    ...
    monkeypatch.setenv("MOOR_TEST_ISOLATION", "/tmp/some-isolation-root")
    assert moor_state_guard._running_under_pytest() is True
...
def test_no_signals_reports_production(monkeypatch):
    ...
    monkeypatch.delenv("PYTEST_CURRENT_TEST", raising=False)
    monkeypatch.delenv("PYTEST_VERSION", raising=False)
    monkeypatch.delenv("MOOR_TEST_ISOLATION", raising=False)
    assert moor_state_guard._running_under_pytest() is False
...
def test_child_with_rebuilt_env_keeping_marker_refuses_production_db():
    ...
    stripped, MOOR_HOME lost) but which keeps the marker must still refuse
    to open the production state.db."""
    env = _minimal_env(MOOR_TEST_ISOLATION="/tmp/pytest-isolation-root")
...
def test_child_bypass_env_disarms_guard_even_with_marker():
    ...
    DB: MOOR_STATE_DB_GUARD_BYPASS=1, not marker-stripping."""
    env = _minimal_env(
        MOOR_TEST_ISOLATION="/tmp/pytest-isolation-root",
