...
entirely on the process environment: ``MOOR_HOME`` says *where* to write and
...
# hardcoded ~/.moor — that root is %LOCALAPPDATA%\moor on Windows) and
...
root = moor_state._real_platform_state_root()
if root is None:
    print("NO-ROOT")
else:
    try:
        moor_state._ensure_test_isolation(root / "state.db")
    except RuntimeError:
        print("REFUSED")
    else:
        print("ALLOWED")
...
def _scrubbed_env(**overrides):
    ...
    Also strips ``MOOR_TEST_ISOLATION`` — the conftest-exported marker
    ...
    env = {
        ...
        and k not in ("MOOR_HOME", "MOOR_TEST_ISOLATION")
    }
...
class TestScrubbedChildEnvironment:
    def test_child_without_pytest_env_still_refuses_production_db(self):
        """The #82770 escape: no PYTEST_* and no MOOR_HOME, yet still a test.

        This is the exact shape of the leak — the child resolves the real
        ``state.db`` because ``MOOR_HOME`` is gone, and the env-only guard
    ...
    def test_env_bypass_lets_a_deliberate_child_through(self):
        ...
        """
        env = _scrubbed_env(**{moor_state._STATE_DB_GUARD_BYPASS_ENV: "1"})
