...
The defect: moor_state already knows macOS `fsync()` does not guarantee
write ordering, and mitigates it with `synchronous=FULL` +
`checkpoint_fullfsync=1` (see `_enforce_macos_synchromoor_full`, whose
...
def test_repair_path_has_no_bare_connects() -> None:
    ...
    """
    # The repair/probe helpers live in moor_state_repair; moor_state only
    # re-imports them.
    import moor_state_repair

    source = Path(moor_state_repair.__file__).read_text(encoding="utf-8")
    tree = ast.parse(source, filename=str(moor_state_repair.__file__))
