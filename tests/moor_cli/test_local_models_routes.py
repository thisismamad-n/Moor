...
underneath are exercised against temp MOOR_HOME (autouse fixture). Network
...
def test_download_already_downloaded_short_circuits(client, monkeypatch):
    from moor_cli.local_runtime.bootstrap import models_dir
    from moor_cli.local_runtime.catalog import CATALOG, select_variant
    from moor_cli.local_runtime.estimator import HardwareBudget
...
def test_runtime_install_rejects_impossible_combo(client, monkeypatch):
    ...
    monkeypatch.setattr(
        "moor_cli.local_runtime.binaries._host_os_arch", lambda: ("win", "arm64"))
...
def test_eject_without_supervisor_is_not_a_500(client, monkeypatch):
    ...
    server session."""
    monkeypatch.setattr(
        "moor_cli.local_runtime.bootstrap.get_supervisor", lambda: None)
    # No running server either: the route must answer 409 (no server),
    # never a NameError 500.
    monkeypatch.setattr(
        "moor_cli.web_routers.local_models._state_endpoint", lambda: None)
