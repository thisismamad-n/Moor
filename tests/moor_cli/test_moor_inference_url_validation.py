"""Regression tests for Moor Portal inference_base_url host-allowlist validation.
...
client. ``_validate_moor_inference_url_from_network()`` blocks any URL
...
4. The env-var override path (``MOOR_INFERENCE_BASE_URL``) is NOT
...
class TestValidatorRules:
    ...
    def test_attacker_host_rejected(self, caplog):
        with caplog.at_level(logging.WARNING, logger="moor_cli.auth"):
            assert (
                _validate_moor_inference_url_from_network("https://attacker.com/v1")
                is None
            )
    ...
    def test_default_inference_url_is_in_allowlist(self):
        """Sanity check: DEFAULT_MOOR_INFERENCE_URL must itself validate.
        ...
        """
        assert (
            _validate_moor_inference_url_from_network(DEFAULT_MOOR_INFERENCE_URL)
            == DEFAULT_MOOR_INFERENCE_URL.rstrip("/")
        )
...
class TestCallSiteWiring:
    ...
    ``_validate_moor_inference_url_from_network`` with the un-validated
    ``_optional_base_url`` again, the test catches it.

    Each site lives inside ``resolve_moor_runtime_credentials`` and one
    ...
    def _read_auth_source(self):
        # The Moor refresh sites live in auth_moor.py (split out of auth.py);
        # read both so the guard tolerates relocation but still fires on deletion.
        import moor_cli.auth as _auth_mod
        import moor_cli.auth_moor as _moor_mod
        from pathlib import Path
        return "".join(
            Path(m.__file__).read_text(encoding="utf-8") for m in (_auth_mod, _moor_mod)
        )
    ...
    def test_no_unvalidated_inference_base_url_assignments_remain(self):
        ...
        for needle in (
            ...
            assert needle not in source, (
                ...
                f"Use _validate_moor_inference_url_from_network() instead."
            )

    def test_validator_wired_at_all_known_call_sites(self):
        """All 2 known auth.py NETWORK refresh sites route the Portal-returned
        inference URL through ``_healed_moor_inference_url`` (which applies the
        ...
        source = self._read_auth_source()
        assert (
            source.count('_validate_moor_inference_url_from_network(refreshed.get("inference_base_url"))')
            == 1
        ), "the validator must be applied exactly once, inside _healed_moor_inference_url"
        refresh_count = source.count("_healed_moor_inference_url(refreshed)")
        mint_count = source.count(
            '_validate_moor_inference_url_from_network(mint_payload.get("inference_base_url"))'
        )
    ...
    def test_proxy_adapter_also_validates(self):
        """The Moor proxy adapter applies the validator as defense-in-depth
        ...
        boundary."""
        from pathlib import Path
        import moor_cli.proxy.adapters.moor_portal as _moor_adapter
        source = Path(_moor_adapter.__file__).read_text(encoding="utf-8")
        assert "_validate_moor_inference_url_from_network" in source


class TestEnvOverrideNotGated:
    """The documented dev/staging env-var override must keep working.

    ``MOOR_INFERENCE_BASE_URL`` is read by ``resolve_moor_runtime_credentials``
    ...
    def test_env_override_path_does_not_call_validator(self):
        """In resolve_moor_runtime_credentials, the env override is
        ...
        for line in source.splitlines():
            if "MOOR_INFERENCE_BASE_URL" in line and "os.getenv" in line:
                assert "_validate_moor_inference_url_from_network" not in line, (
...
class TestHealsPoisonedStoredValue:
    ...
    staging endpoint (real incident: opus-4.8 routed to moor, moor pinned
    ...
    def test_refresh_resets_rejected_url_to_default(self, monkeypatch):
        ...
        import moor_cli.auth_moor as moor_cli_auth_moor
        ...
        state = {
            ...
            "refresh_token": "rtok",
            "client_id": "moor-cli",
            "portal_base_url": auth.DEFAULT_MOOR_PORTAL_URL,
        ...
        monkeypatch.setattr(auth, "_moor_invoke_jwt_status", lambda *a, **k: "needs_refresh")
        monkeypatch.setattr(moor_cli_auth_moor, "_moor_invoke_jwt_status", lambda *a, **k: "needs_refresh")
        monkeypatch.setattr(
            auth,
        ...
        monkeypatch.setattr(auth, "_assert_moor_inference_jwt_usable", lambda *a, **k: None)
        monkeypatch.setattr(moor_cli_auth_moor, "_assert_moor_inference_jwt_usable", lambda *a, **k: None)
        monkeypatch.setattr(auth, "_select_moor_invoke_jwt", lambda *a, **k: None)
        monkeypatch.setattr(moor_cli_auth_moor, "_select_moor_invoke_jwt", lambda *a, **k: None)

        result = auth.refresh_moor_oauth_from_state(state, force_refresh=True)

        assert result["inference_base_url"] == auth.DEFAULT_MOOR_INFERENCE_URL, (
...
class TestEnvOverrideWins:
    """``MOOR_INFERENCE_BASE_URL`` must win over the stored value for the
    ...
    def _patch_no_refresh(self, monkeypatch, auth, state):
        ...
        monkeypatch.setattr(auth, "_moor_invoke_jwt_status", lambda *a, **k: None)
        monkeypatch.setattr(moor_cli_auth_moor, "_moor_invoke_jwt_status", lambda *a, **k: None)
        monkeypatch.setattr(
        ...
        monkeypatch.setattr(auth, "_write_shared_moor_state", lambda *a, **k: None)
        monkeypatch.setattr(moor_cli_auth_moor, "_write_shared_moor_state", lambda *a, **k: None)
        monkeypatch.setattr(auth, "_sync_moor_pool_from_auth_store", lambda *a, **k: None)
        monkeypatch.setattr(moor_cli_auth_moor, "_sync_moor_pool_from_auth_store", lambda *a, **k: None)
    ...
    def test_no_refresh_env_override_not_persisted(self, monkeypatch):
        ...
        state = self._base_state(auth, auth.DEFAULT_MOOR_INFERENCE_URL)
        self._patch_no_refresh(monkeypatch, auth, state)
        monkeypatch.setenv("MOOR_INFERENCE_BASE_URL", self.STAGING)

        auth.resolve_moor_runtime_credentials()

        assert state["inference_base_url"] == auth.DEFAULT_MOOR_INFERENCE_URL, (
    ...
    def test_no_refresh_heals_poisoned_stored_without_env(self, monkeypatch):
        ...
        self._patch_no_refresh(monkeypatch, auth, state)
        monkeypatch.delenv("MOOR_INFERENCE_BASE_URL", raising=False)

        result = auth.resolve_moor_runtime_credentials()
        assert result["base_url"] == auth.DEFAULT_MOOR_INFERENCE_URL, (
...
class TestProxyAdapterEnvOverride:
    """The Moor proxy adapter is the second chokepoint: it re-validates the
    base_url returned by resolve_moor_runtime_credentials() against the prod
    allowlist. That re-validation must not clobber a legitimate
    MOOR_INFERENCE_BASE_URL staging override.
    """
    ...
    def test_proxy_adapter_consults_env_override(self):
        ...
        source = Path(_moor_adapter.__file__).read_text(encoding="utf-8")
        assert "_moor_inference_env_override()" in source, (
