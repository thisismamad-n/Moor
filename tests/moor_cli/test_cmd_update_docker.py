"""Tests for ``moor update`` / ``--check`` inside the Docker container.
...
fix, ``moor update`` would fall through to ``"✗ Not a git repository.
...
def test_cmd_update_in_docker_prints_guidance_and_exits(
    ...
    """``moor update`` inside Docker → friendly message + exit 1, no git calls."""
...
def test_format_docker_update_message_contents():
    ...
    assert "MOOR_HOME" in msg or "/opt/data" in msg, (
