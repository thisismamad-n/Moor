...
Scheduled Task named ``Moor_Gateway`` (harmless action) plus a pre-suffix Startup entry under a
...
def test_status_warns_and_uninstall_removes_pre_suffix_launchers(tmp_path, monkeypatch, capsys):
    # The shared isolation fixture uses tmp_path / "moor" as the Windows native default.
    home = tmp_path / "custom-moor"
    ...
    assert current != "Moor_Gateway", "a temp home must carry a profile suffix"
    legacy_pair = home / "gateway-service" / "Moor_Gateway.vbs"
    ...
    create = subprocess.run(
        [schtasks, "/Create", "/F", "/TN", "Moor_Gateway", "/SC", "ONLOGON", "/TR", f"wscript.exe //B {legacy_pair}"],
    ...
    try:
        ...
        assert "moor gateway uninstall" in out
        ...
        query = subprocess.run([schtasks, "/Query", "/TN", "Moor_Gateway"], capture_output=True, text=True, timeout=60)
        ...
        subprocess.run([schtasks, "/Delete", "/F", "/TN", "Moor_Gateway"], capture_output=True, text=True, timeout=60)
