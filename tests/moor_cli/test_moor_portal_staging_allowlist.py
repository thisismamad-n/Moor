"""Regression tests for the Moor Portal env-override bypassing the host
allowlist, mirroring the existing MOOR_INFERENCE_BASE_URL /
_ALLOWED_MOOR_INFERENCE_HOSTS treatment.

Real incident (2026-07): a hosted agent provisioned by moor-account-service
on the `staging` Vercel environment is stamped with
``MOOR_PORTAL_BASE_URL=https://portal.staging-nousresearch.com`` in its
...
Before this fix, ``resolve_moor_access_token`` / ``resolve_moor_runtime_
...
``_MOOR_PORTAL_ALLOWED_HOSTS``, which only recognised the production host.
...
``_quarantine_moor_oauth_state`` and wiped the entire credential pool.

The correct fix (mirroring ``_moor_inference_env_override()``): the env
...
class TestResolveAccessTokenEnvOverrideWins:
    """End-to-end: resolve_moor_access_token must use the env override for
    ...
    def test_env_override_wins_even_with_staging_state_stored(
        ...
        a prior MOOR_AUTH_JSON_BOOTSTRAP seed), and the env var is set to
        ...
        monkeypatch.setenv("MOOR_PORTAL_BASE_URL", staging_portal)
    ...
    def test_no_env_no_staging_state_prod_url_used_unmodified(
        ...
        monkeypatch.delenv("MOOR_PORTAL_BASE_URL", raising=False)
        monkeypatch.delenv("MOOR_PORTAL_BASE_URL", raising=False)
        self._write_auth_file(tmp_path, stored_portal_url=DEFAULT_MOOR_PORTAL_URL)

        seen_portal_urls, records = self._run_and_capture(monkeypatch, auth)

        assert seen_portal_urls == [DEFAULT_MOOR_PORTAL_URL]
