...
def moor_home(tmp_path, monkeypatch):
    ...
    # /etc/moor on the dev/CI box can't influence the test.
    monkeypatch.setenv("MOOR_MANAGED_DIR", str(tmp_path / "no_such_managed_dir"))
...
def test_user_config_overrides_default(moor_home, monkeypatch):
...
def test_env_expansion_in_user_config(moor_home, monkeypatch):
    from moor_cli.config import load_config, cfg_get
...
def test_user_env_overrides_shell(tmp_path, monkeypatch):
    ...
    load_moor_dotenv(moor_home=str(home))
