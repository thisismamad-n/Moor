...
@pytest.fixture
def catalog_env(tmp_path: Path, monkeypatch: pytest.MonkeyPatch, _isolate_moor_home):
    ...
    monkeypatch.setenv("MOOR_OPTIONAL_MCPS", str(catalog))
...
def test_catalog_cannot_declare_reserved_control_key(
    ...
    response = client.post(
        "/api/mcp/catalog/install",
        headers=HEADERS,
        json={"name": "demo", "env": {"MOOR_YOLO_MODE": "1"}},
    )
    ...
    assert installs == []
    env_path = catalog_env / ".env"
    assert not env_path.exists() or "MOOR_YOLO_MODE" not in env_path.read_text(
        encoding="utf-8"
    )
...
def test_rejected_copilot_controls_do_not_change_live_resolvers(
    ...
    monkeypatch.setenv("MOOR_COPILOT_ACP_COMMAND", "/opt/trusted/copilot")
    monkeypatch.setenv("MOOR_COPILOT_ACP_ARGS", "--acp --stdio")
    expected_command = _resolve_command()
    expected_args = _resolve_args()

    attempts = {
        "MOOR_COPILOT_ACP_COMMAND": "/tmp/attacker-command",
        "MOOR_COPILOT_ACP_ARGS": "--malicious-transport",
    }
...
def test_preexisting_copilot_controls_remain_usable(
    ...
    monkeypatch.setenv("MOOR_COPILOT_ACP_COMMAND", "parent-placeholder")
    monkeypatch.setenv("MOOR_COPILOT_ACP_ARGS", "--parent-placeholder")
    (catalog_env / ".env").write_text(
        "MOOR_COPILOT_ACP_COMMAND=/opt/operator/copilot\n"
        "MOOR_COPILOT_ACP_ARGS=--acp --stdio --operator-mode\n",
        encoding="utf-8",
    )

    load_moor_dotenv(
        moor_home=catalog_env,
