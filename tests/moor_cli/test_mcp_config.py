"""
Tests for moor_cli.mcp_config — ``moor mcp`` subcommands.
...
def _isolate_config(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "moor_cli.config.get_moor_home", lambda: tmp_path
    )
    config_path = tmp_path / "config.yaml"
    env_path = tmp_path / ".env"
    monkeypatch.setattr(
        "moor_cli.config.get_config_path", lambda: config_path
    )
    monkeypatch.setattr(
        "moor_cli.config.get_env_path", lambda: env_path
    )
    return tmp_path
...
class TestMcpList:
    ...
    def test_list_with_servers(self, tmp_path, capsys):
        ...
        from moor_cli.mcp_config import cmd_mcp_list
...
class TestMcpRemove:
    def test_remove_existing_server(self, tmp_path, capsys, monkeypatch):
        ...
        from moor_cli.mcp_config import cmd_mcp_remove
    ...
    def test_remove_cleans_oauth_tokens(self, tmp_path, capsys, monkeypatch):
        ...
        monkeypatch.setattr("builtins.input", lambda _: "y")
        # Also patch get_moor_home in the mcp_config module namespace
        monkeypatch.setattr(
            "moor_cli.mcp_config.get_moor_home", lambda: tmp_path
        )
...
class TestMcpAdd:
    ...
    def test_add_http_server_all_tools(self, tmp_path, capsys, monkeypatch):
        ...
        monkeypatch.setattr(
            "moor_cli.mcp_config._probe_single_server", mock_probe
        )
    ...
    def test_add_preset_fills_transport(self, tmp_path, capsys, monkeypatch):
        ...
        monkeypatch.setattr(
            "moor_cli.mcp_config._MCP_PRESETS",
...
class TestProbeEnvResolution:
    ...
    where `moor mcp add --auth header` sent a literal
    ``Authorization: Bearer ${MCP_X_API_KEY}`` and got 401."""

    def test_resolve_interpolates_header(self, monkeypatch):
        from moor_cli.mcp_config import _resolve_mcp_server_config
...
class TestStripBearerPrefix:
    ...
    def test_bare_token_unchanged(self):
        from moor_cli.mcp_config import _strip_bearer_prefix
...
class TestConfigHelpers:
    def test_save_and_load_mcp_server(self, tmp_path):
        from moor_cli.mcp_config import _save_mcp_server, _get_mcp_servers
...
class TestMcpReauth:
    def test_reauth_all_visits_only_oauth_servers_in_order(
        ...
        visited = []
        monkeypatch.setattr(
            "moor_cli.mcp_config._reauth_oauth_server",
            lambda name, cfg: visited.append(name) or True,
        )
        from moor_cli.mcp_config import cmd_mcp_reauth
...
def test_tool_filters_keeps_explicit_empty_include():
    ...
    from moor_cli.mcp_config import _tool_filters
