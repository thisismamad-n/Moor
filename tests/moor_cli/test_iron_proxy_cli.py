"""Unit tests for ``moor_cli.proxy_cli`` command handlers.
...
def moor_home(tmp_path, monkeypatch):
    """Point MOOR_HOME at a temp dir so the wizard doesn't touch the
...
def test_cmd_setup_from_bitwarden_refuses_on_empty_vault(hermes_home, monkeypatch):
    ...
    monkeypatch.setattr(ip, "find_iron_proxy", lambda **kw: moor_home / "iron-proxy")
    monkeypatch.setattr(ip, "iron_proxy_version", lambda b: "test")
    monkeypatch.setattr(
        ip, "ensure_ca_cert",
        lambda **kw: (moor_home / "ca.crt", moor_home / "ca.key"),
    )
...
def test_cmd_disable_uses_public_status_pid_not_private_read_pid(
    ...
    # mutation only.
    from moor_cli.config import load_config as _lc
    cfg2 = _lc()
...
def test_cmd_start_refuses_when_bitwarden_mode_but_disabled(moor_home, monkeypatch):
...
def test_cmd_setup_audit_log_failure_is_warning_not_abort(hermes_home, monkeypatch):
    ...
    monkeypatch.setattr(ip, "discover_provider_mappings", lambda **kw: [
        ip.TokenMapping(
            proxy_token="moor-proxy-deadbeef",
