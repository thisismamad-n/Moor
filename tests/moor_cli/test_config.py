"""Tests for moor_cli configuration management."""
    ...
    def test_default_path(self):
        with patch.dict(os.environ, {}, clear=False):
            ...
            if sys.platform == "win32":
                # Windows default is %LOCALAPPDATA%\moor — see
                # moor_constants._get_platform_default_moor_home.
                ...
                assert home == base / "moor"
            else:
                assert home == Path.home() / ".moor"
...
class TestEnsureMoorHome:
    ...
    def test_upgrades_legacy_template_soul_md(self, tmp_path):
        ...
        from moor_cli.default_soul import DEFAULT_SOUL_MD, _LEGACY_TEMPLATE_SOULS
    ...
    _PRE_REWRITE_DEFAULT_SOUL = (
        "You are Moor Agent, an intelligent AI assistant created by Moor "
...
class TestLoadConfigParseFailure:
    ...
    their config.yaml was being ignored. The helper must:
      * log at WARNING (so ``moor logs`` surfaces it)
...
class TestSaveEnvValueSecure:
    ...
    def test_save_env_value_quotes_values_containing_hash(self, tmp_path):
        ...
        with patch.dict(os.environ, {"MOOR_HOME": str(tmp_path)}, clear=False):
...
class TestRemoveEnvValue:
    def test_removes_key_from_env_file(self, tmp_path):
        ...
        with patch.dict(os.environ, {"MOOR_HOME": str(tmp_path), "KEY_B": "value_b"}):
    ...
    def test_clears_os_environ_even_when_not_in_file(self, tmp_path):
        ...
        with patch.dict(os.environ, {"MOOR_HOME": str(tmp_path), "ORPHAN_KEY": "orphan"}):
    ...
    def test_remove_env_value_preserves_existing_file_mode_on_posix(self, tmp_path):
        ...
        with patch.dict(os.environ, {"MOOR_HOME": str(tmp_path), "DROP": "gone"}):
...
class TestSanitizeEnvLines:
    ...
    def test_migrate_reports_normalized_line_formatting(self, capsys):
        ...
        with (
            patch("moor_cli.config.sanitize_env_file", return_value=2),
            patch(
                "moor_cli.config.check_config_version",
                return_value=(latest_version, latest_version),
            ),
            patch("moor_cli.config.read_raw_config", return_value={}),
            patch("moor_cli.config.get_missing_env_vars", return_value=[]),
            patch("moor_cli.config.get_missing_config_fields", return_value=[]),
            patch("moor_cli.config.get_missing_skill_config_vars", return_value=[]),
        ):
...
class TestOptionalEnvVarsRegistry:
    ...
    def test_tavily_in_env_vars_by_version(self):
        ...
        from moor_cli.config import ENV_VARS_BY_VERSION
        all_vars = []
    ...
    def test_max_iterations_not_offered_as_env_var(self):
        """MOOR_MAX_ITERATIONS must NOT be in OPTIONAL_ENV_VARS (issue #17534).

        Offering it as an editable env var (dashboard, `moor setup`) lets a
        ...
        via config.yaml; MOOR_MAX_ITERATIONS remains a read-only backward-compat
        fallback in the gateway/CLI, never a promoted write target.
        """
        from moor_cli.config import OPTIONAL_ENV_VARS
        assert "MOOR_MAX_ITERATIONS" not in OPTIONAL_ENV_VARS
...
class TestMemoryProviderEnvVarsRegistry:
    ...
    and `moor setup` surface it (previously only Honcho was listed, leaving
...
class TestConfigSupportFloor:
    ...
    def test_v11_config_is_refused_and_untouched(self, tmp_path, capsys):
        ...
        assert "run `moor setup` to regenerate" in captured.out
    ...
    def test_floor_message_uses_display_moor_home(self):
        from moor_cli.config_migrations import support_floor_message
...
class TestEnvWriteDenylist:
    ...
    ``PATH``, ``EDITOR``, etc. — or selected Moor runtime/security controls.
    ...
    ``LD_PRELOAD=/tmp/evil.so`` in ``.env`` and own the next Moor
    process on next startup via the dotenv → ``os.environ`` chain in
    ``moor_cli/env_loader.py``.
    ...
    def _moor_home(self, tmp_path, monkeypatch):
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            "MOOR_SPOTIFY_CLIENT_ID",
            "MOOR_QWEN_BASE_URL",
            "MOOR_MAX_ITERATIONS",
        ],
    )
    def test_moor_integration_keys_still_writable(self, allowed_key):
        """``MOOR_*`` overall is NOT blocked.
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            "MOOR_CRON_SESSION",
            "MOOR_SINGLE_QUERY_SESSION",
            "MOOR_SESSION_KEY",
            "MOOR_SESSION_PLATFORM",
        ],
    )
    def test_moor_security_control_keys_are_not_writable(self, protected_key):
    ...
    def test_preexisting_optional_mcps_override_still_loads(self, tmp_path):
        ...
        (tmp_path / ".env").write_text(
            f"MOOR_OPTIONAL_MCPS={catalog}\n",
            encoding="utf-8",
        )
        invalidate_env_cache()

        assert load_env()["MOOR_OPTIONAL_MCPS"] == str(catalog)

    @pytest.mark.parametrize(
        ("key", "expected"),
        [
            ("Path", "PATH"),
            ("Moor_Yolo_Mode", "MOOR_YOLO_MODE"),
            ("Moor_Optional_Mcps", "MOOR_OPTIONAL_MCPS"),
            ("Moor_Copilot_Acp_Command", "MOOR_COPILOT_ACP_COMMAND"),
            ("Moor_Copilot_Acp_Args", "MOOR_COPILOT_ACP_ARGS"),
        ],
    )
    def test_windows_policy_names_are_case_insensitive(self, key, expected):
        from moor_cli.config import _env_var_policy_name
...
"""
    ...
    def test_persist_migration_writes_full_read_raw_config(self, tmp_path):
        from moor_cli.config import _persist_migration, read_raw_config
    ...
    def test_preserves_existing_codex_app_server_auto_value(self, tmp_path):
        with patch.dict(os.environ, {"MOOR_HOME": str(tmp_path)}):
            self._write(
                ...
                "_config_version: 31\n"
                "compression:\n"
                "  codex_app_server_auto: moor\n",
            )

            migrate_config(interactive=False, quiet=True)

            raw = yaml.safe_load((tmp_path / "config.yaml").read_text())
            assert raw["compression"]["codex_app_server_auto"] == "moor"
...
class TestConfigCommandFailClosedSurface:
    """`moor config set/unset` must exit cleanly (no traceback) when the
