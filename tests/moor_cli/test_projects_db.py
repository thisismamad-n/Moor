"""Tests for the per-profile Projects store (moor_cli/projects_db)."""
...
def test_create_get_list(conn):
    pid = pdb.create_project(conn, name="Moor Agent", folders=["/tmp/moor"])
    ...
    assert proj.primary_path == "/tmp/moor"
    assert [f.path for f in proj.folders] == ["/tmp/moor"]
    assert proj.folders[0].is_primary is True

    # Lookup by slug too.
    assert pdb.get_project(conn, "moor-agent").id == pid
...
def test_per_profile_isolation(tmp_path):
    # Two distinct DB paths stand in for two profiles' MOOR_HOME.
