...
second Moor holder the updater's drain did not stop — survives the copy and is
...
def _isolate_database_holders(monkeypatch):
    ...
    monkeypatch.setattr("moor_cli.backup_restore._foreign_db_holder_pids", lambda path: [])
...
def test_post_update_guard_covers_sibling_profiles(tmp_path, monkeypatch, capsys):
    ...
    monkeypatch.setattr(update_cmd, "get_moor_home", lambda: root_home)
    monkeypatch.setattr(
        "moor_cli.backup._sibling_profile_homes",
