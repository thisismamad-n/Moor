...
`moor update` migrated only the active profile's config.yaml; sibling
...
context-local MOOR_HOME override.
...
import moor_cli.update_cmd_config as update_cmd_config
...
def _write_profile(root: Path, name: str, version: int) -> Path:
    ...
    (home / "config.yaml").write_text(
        yaml.safe_dump({"_config_version": version, "model": {"provider": "moor"}}),
        encoding="utf-8",
    )
    return home
...
def test_sibling_behind_is_migrated_on_disk(monkeypatch, tmp_path):
    ...
    assert on_disk["model"]["provider"] == "moor"
...
def test_override_is_reset_after_run(monkeypatch, tmp_path):
    ...
    before = get_moor_home_override()
    update_cmd._migrate_sibling_profile_configs()
    assert get_moor_home_override() == before
