"""Tests for moor_cli/terminal_breadcrumbs.py — per-terminal ``moor -c``.

Covers terminal id derivation (tty vs env vars vs none), breadcrumb
write/read roundtrip under a temp MOOR_HOME, stale-session fallback
...
from moor_cli import terminal_breadcrumbs as tb
...
def test_breadcrumb_roundtrip(moor_home, monkeypatch, no_terminal_env):
...
def test_config_gate_off_disables_writes_and_resolution(
    moor_home, monkeypatch, no_terminal_env
):
