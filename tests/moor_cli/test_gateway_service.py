...
class TestSystemdServiceRefresh:
    ...
    def test_systemd_restart_timeout_prints_status_guidance(self, monkeypatch, capsys):
        """`moor gateway restart` must not surface a raw TimeoutExpired traceback.

        The dashboard spawns `moor gateway restart` in the background; when a
        ...
        assert "moor gateway status" in output
    ...
    def test_refresh_refuses_to_bake_pytest_tmpdir_into_real_user_unit(
        ...
        ``generate_systemd_unit()`` bakes ``MOOR_HOME`` into the unit's
        ...
        ``/tmp/pytest-of-.../moor_test`` MOOR_HOME — silently breaking
        ...
        # Realistic generated unit referencing a pytest tmpdir MOOR_HOME
        polluted_unit = (
            "[Service]\n"
            'Environment="MOOR_HOME=/tmp/pytest-of-alice/pytest-42/'
            'popen-gw0/test_x/moor_test"\n'
        )
        monkeypatch.setattr(
            gateway_cli,
...
class TestTempHomeServiceDefinitionGuard:
    ...
    def test_detects_tmp_home_in_systemd_unit(self):
        unit = '[Service]\nEnvironment="MOOR_HOME=/tmp/moor-e2e-41264"\n'
        assert (
    ...
    def test_detects_tempdir_env_home(self, monkeypatch, tmp_path):
        ...
        unit = f'[Service]\nEnvironment="MOOR_HOME={tmp_path}/moor-home"\n'
...
class TestGetCronDrainTimeout:
    ...
    def test_env_overrides_config(self, monkeypatch):
        monkeypatch.setenv("MOOR_CRON_DRAIN_TIMEOUT", "45")
        monkeypatch.setattr(
            gateway_cli,
            "read_raw_config",
...
class TestGeneratedSystemdUnits:
    ...
    def test_user_unit_does_not_leak_profile_node_symlink_target(self, tmp_path, monkeypatch):
        ...
        profile_node_bin = tmp_path / ".moor" / "profiles" / "jarvis" / "node" / "bin"
    ...
    def test_launchd_plist_persists_configured_nofile_soft_limit(self, monkeypatch):
        ...
        plist rewrite by `moor gateway start` cannot strip the FD floor and
        reintroduce EMFILE crashes (launchd default soft limit is 256)."""
        import moor_cli.resource_limits as resource_limits
...
class TestLaunchdServiceRecovery:
    ...
    def test_refresh_defers_reload_when_running_inside_gateway_tree(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            ...
            lambda: (
                "<plist>--replace\n<key>MOOR_HOME</key>"
                "<string>/Users/alice/.moor</string></plist>"
            ),
        )
    ...
    def test_launchd_status_reports_fallback_when_unsupported_and_pid_running(self, tmp_path, monkeypatch, capsys):
        ...
        monkeypatch.setattr(gateway_cli, "get_moor_home", lambda: tmp_path)
...
class TestGatewaySystemServiceRouting:
    ...
    def test_launchd_restart_uses_sigusr1_and_exit_wait_budget(self, monkeypatch, capsys):
        ...
        assert ("observe", "ai.moor.gateway", 654, "gui/501") in calls
        out = capsys.readouterr().out
    ...
    def test_gateway_restart_does_not_fallback_to_foreground_when_launchd_restart_fails(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            ...
            lambda: (_ for _ in ()).throw(
                gateway_cli.subprocess.CalledProcessError(5, ["launchctl", "kickstart", "-k", "gui/501/ai.moor.gateway"])
            ),
        )
...
class TestSystemUnitMoorHome:
    ...
    def test_managed_node_makes_system_unit_independent_of_callers_path(
        ...
        monkeypatch.setattr(gateway_cli, "get_moor_home", lambda: root_moor)
    ...
    def test_user_unit_unaffected_by_change(self):
        ...
        moor_home = str(gateway_cli.get_moor_home().resolve())
        assert f'MOOR_HOME={moor_home}' in unit


class TestSystemUnitRefreshSyncsMoorHome:
    """sudo system refresh must not flip TimeoutStopSec via /root/.moor."""

    def test_refresh_adopts_unit_moor_home_before_rewriting(self, tmp_path, monkeypatch):
        root_home = tmp_path / "root"
        alice_home = tmp_path / "alice"
        root_moor = root_home / ".moor"
        alice_moor = alice_home / ".moor"
        root_moor.mkdir(parents=True)
        alice_moor.mkdir(parents=True)
        (root_moor / "config.yaml").write_text(
            "agent:\n  restart_drain_timeout: 60\n", encoding="utf-8"
        )
        (alice_moor / "config.yaml").write_text(
...
class TestMoorHomeForTargetUser:
    """Unit tests for _moor_home_for_target_user()."""

    def test_remaps_default_home(self, monkeypatch):
        monkeypatch.setattr(Path, "home", staticmethod(lambda: Path("/root")))
        monkeypatch.delenv("MOOR_HOME", raising=False)

        result = gateway_cli._moor_home_for_target_user("/home/alice")
...
class TestPreflightUserSystemd:
    ...
    def test_raises_when_linger_disabled_and_loginctl_denied(self, monkeypatch):
        ...
        assert "moor gateway run" in msg  # foreground fallback mentioned
...
class TestProfileArg:
    ...
    def test_systemd_unit_for_target_user_includes_named_profile(self, tmp_path, monkeypatch):
        ...
        root_home = tmp_path / "root"
        target_home = tmp_path / "home" / "alice"
        root_profile = root_home / ".moor" / "profiles" / "mybot"
        root_profile.mkdir(parents=True)

        monkeypatch.setattr(Path, "home", lambda: root_home)
        monkeypatch.setenv("MOOR_HOME", str(root_profile))
        monkeypatch.setattr(gateway_cli, "get_moor_home", lambda: root_profile)
        monkeypatch.setattr(
            gateway_cli,
            "_system_service_identity",
            lambda run_as_user=None: ("alice", "alice", str(target_home)),
    ...
    def test_launchd_plist_path_uses_real_user_home_not_profile_home(self, tmp_path, monkeypatch):
        ...
        assert plist_path == machine_home / "Library" / "LaunchAgents" / "ai.moor.gateway-orcha.plist"
...
class TestRemapPathForUser:
    ...
    def test_remaps_path_under_current_home(self, monkeypatch, tmp_path):
        ...
        result = gateway_cli._remap_path_for_user(
            str(tmp_path / "root" / ".moor" / "moor-agent"),
            str(tmp_path / "alice"),
        )
        assert result == str(tmp_path / "alice" / ".moor" / "moor-agent")
...
class TestSystemUnitPathRemapping:
    ...
    def test_system_unit_has_no_root_paths(self, monkeypatch, tmp_path):
        root_home = tmp_path / "root"
        root_home.mkdir()
        project = root_home / ".moor" / "moor-agent"
        ...
        # WorkingDirectory is anchored at the target user's MOOR_HOME (stable,
        ...
        assert "WorkingDirectory=/home/alice/.moor" in unit
        assert "WorkingDirectory=/home/alice/.moor/moor-agent" not in unit
...
class TestLegacyMoorUnitDetection:
    """Tests for _find_legacy_moor_units / has_legacy_moor_units.

    These guard against the scenario that tripped Luis in April 2026: an
    older install left a ``moor.service`` unit behind when the service was
    renamed to ``moor-gateway.service``. After PR #5646 (signal recovery
    via systemd), the two services began SIGTERM-flapping over the same
    Telegram bot token in a 30-second cycle.

    The detector must flag ``moor.service`` ONLY when it actually runs our
    gateway, and must NEVER flag profile units
    (``moor-gateway-<profile>.service``) or unrelated third-party services.
    """
    ...
    def test_detects_both_scopes_simultaneously(self, tmp_path, monkeypatch):
        ...
        (system_dir / "moor.service").write_text(self._OUR_UNIT_TEXT, encoding="utf-8")
    ...
    def test_accepts_alternate_execstart_formats(self, tmp_path, monkeypatch):
        ...
        ExecStart variants we've seen in the wild:
          - python -m moor_cli.main gateway run
          - python path/to/moor_cli/main.py gateway run
          - moor gateway run   (direct binary)
          - python path/to/gateway/run.py
        """
        user_dir, _ = self._setup_search_paths(tmp_path, monkeypatch)
        variants = [
            "ExecStart=/venv/bin/python -m moor_cli.main gateway run --replace",
            "ExecStart=/venv/bin/python /opt/moor/moor_cli/main.py gateway run",
            "ExecStart=/usr/local/bin/moor gateway run --replace",
            "ExecStart=/venv/bin/python /opt/moor/gateway/run.py",
        ]
        for i, execstart in enumerate(variants):
            name = "moor.service" if i == 0 else "moor.service"  # same name
            # Test each variant fresh
            (user_dir / "moor.service").write_text(
...
class TestRemoveLegacyMoorUnits:
    ...
    def test_removes_user_scope_legacy_unit(self, tmp_path, monkeypatch, capsys):
        ...
        legacy = user_dir / "moor.service"
        legacy.write_text(self._OUR_UNIT_TEXT, encoding="utf-8")

        removed, remaining = gateway_cli.remove_legacy_moor_units(interactive=False)
        ...
        assert any("--user stop moor.service" in c for c in cmds_joined)
        assert any("--user disable moor.service" in c for c in cmds_joined)
    ...
    def test_does_not_touch_profile_units_during_migration(
        ...
        """Teknium's constraint: profile units (moor-gateway-coder.service)
        ...
        profile_unit = user_dir / "moor-gateway-coder.service"
        profile_unit.write_text(self._OUR_UNIT_TEXT, encoding="utf-8")
        default_unit = user_dir / "moor-gateway.service"
...
class TestMigrateLegacyCommand:
    ...
    def test_gateway_command_migrate_legacy_dispatches(
        ...
        monkeypatch.setattr(gateway_cli, "remove_legacy_moor_units", fake_remove)
        monkeypatch.setattr(gateway_cli, "supports_systemd_services", lambda: True)
        monkeypatch.setattr(gateway_cli, "is_macos", lambda: False)
...
class TestSystemdInstallOffersLegacyRemoval:
    ...
    def test_install_offers_removal_when_legacy_detected(
        ...
        # has_legacy_moor_units must return True
        monkeypatch.setattr(gateway_cli, "has_legacy_moor_units", lambda: True)
        monkeypatch.setattr(gateway_cli, "remove_legacy_moor_units", fake_remove)
    ...
    def test_install_skips_legacy_check_when_none_present(
        ...
        monkeypatch.setattr(gateway_cli, "has_legacy_moor_units", lambda: False)
        monkeypatch.setattr(gateway_cli, "remove_legacy_moor_units", fake_remove)
...
class TestSystemScopeWizardPreCheck:
    ...
    def _setup_units(tmp_path, monkeypatch, system_present: bool, user_present: bool):
        ...
        usr_dir.mkdir()
        if system_present:
            (sys_dir / "moor-gateway.service").write_text("[Unit]\n")
        if user_present:
            (usr_dir / "moor-gateway.service").write_text("[Unit]\n")
        monkeypatch.setattr(
            gateway_cli,
            "get_systemd_unit_path",
            lambda system=False: (sys_dir if system else usr_dir) / "moor-gateway.service",
        )
    ...
    def test_non_root_with_explicit_system_arg_returns_true(self, tmp_path, monkeypatch):
        # Caller passed system=True explicitly (e.g. ``moor gateway start --system``).
...
class TestServiceWorkingDirIsStable:
    ...
    (MOOR_HOME), never the source checkout / worktree, so a relocated or
    ...
    def test_launchd_workingdirectory_is_moor_home(self, tmp_path, monkeypatch):
        import re
...
class TestLaunchctlBootstrapEioRetry:
    ...
    PLIST = "/tmp/ai.moor.gateway.plist"
    DOMAIN = "gui/501"
    LABEL = "ai.moor.gateway"
...
class TestTimeoutStopSecCoversCronFloor:
    ...
    def _unit_with_config(self, tmp_path, monkeypatch, config_yaml, env=None):
        moor = tmp_path / "home" / ".moor"
        moor.mkdir(parents=True)
        (moor / "config.yaml").write_text(config_yaml, encoding="utf-8")
        monkeypatch.setenv("MOOR_HOME", str(moor))
        monkeypatch.delenv("MOOR_RESTART_DRAIN_TIMEOUT", raising=False)
        monkeypatch.delenv("MOOR_CRON_DRAIN_TIMEOUT", raising=False)
        monkeypatch.setattr(gateway_cli.shutil, "which", lambda cmd: None)
        monkeypatch.setattr(
