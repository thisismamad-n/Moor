...
Covers:
- Defense 1: gateway stop/restart refuse when _MOOR_GATEWAY=1
...
class TestGatewayLifecyclePattern:
    ...
    @pytest.mark.parametrize("text", [
        ...
        "moor gateway stop",
        "moor gateway uninstall",
        "moor  gateway  restart",         # double spaces
        "Hermez Gateway Restart".lower().replace("z", "s"),  # case handled
        "MOOR GATEWAY RESTART",           # uppercase
    ])
    def test_moor_gateway_commands(self, text):
    ...
    @pytest.mark.parametrize("text", [
        ...
        "launchctl submit -l ai.moor.gateway-hard-restart-no-photon-notice -- /bin/sh ~/.moor/scripts/hard_restart_gateway_no_photon_notice.sh",
        "launchctl submit -l moor-gateway-restart-helper -- /bin/sh helper.sh",
        # bootstrap loads an arbitrary plist — same laundering shape.
        "launchctl bootstrap gui/501 ~/Library/LaunchAgents/ai.moor.gateway.restart-once.plist",
        "launchctl bootout gui/501/ai.moor.gateway",
        ...
        (
            "launchctl submit \\\n"
            "  -l ai.moor.gateway-hard-restart-no-photon-notice \\\n"
            "  -- /bin/sh ~/.moor/scripts/hard_restart_gateway_no_photon_notice.sh"
        ),
    ])
    ...
    def test_label_defined_before_verb_is_caught(self):
        ...
        cmd = (
            "uid=$(id -u); for item in 'ai.moor.gateway-apollo:/a.plist' "
            "'ai.moor.gateway-cronus:/c.plist' 'ai.moor.gateway-plutus:/p.plist' "
            "'ai.moor.gateway:/Users/botuser/Library/LaunchAgents/ai.moor.gateway.plist'; "
    ...
    def test_line_continuation_does_not_bridge_unrelated_lines(self):
        ...
        text = (
            ...
            "unrelated moor note on the next line"
        )
    ...
    @pytest.mark.parametrize("text", [
        ...
        # `launchctl kick"start" ... ai.moor.gateway` executes exactly as
        ...
        'launchctl kick"start" -k gui/501/ai.moor.gateway',
        "launchctl kick'start' -k gui/501/ai.moor.gateway",
        "launchctl kick\\start -k gui/501/ai.moor.gateway",
        'launchctl "kickstart" -k gui/501/ai.moor.gateway',
        # Splices on the newer/legacy unload spellings this PR added.
        'launchctl boot"out" gui/501/ai.moor.gateway',
        "launchctl dis\\able gui/501/ai.moor.gateway",
        # The gateway identifier itself can be spliced just as easily.
        'launchctl bootout gui/501/ai.moor."gateway"',
        # Same class on the systemctl and moor-CLI branches.
        'systemctl re"start" moor-gateway',
        'moor gateway re"start"',
    ])
    ...
    def test_spliced_verb_inside_shell_c_payload_is_blocked(self):
        ...
        command = 'sh -c \'launchctl kick"start" -k gui/501/ai.moor.gateway\''
    ...
    @pytest.mark.parametrize("text", [
        ...
        'launchctl kick"start" -k gui/501/ai.moor.update-checker',
        'systemctl re"start" moor-meta.service',
    ...
    @pytest.mark.parametrize("text", [
        ...
        "moor cron list",
        "moor update",
        "moor config set model claude",
        "echo 'just a normal cron job'",
        "run the backup script",
        "gateway is running fine",
        # `moor gateway start` is benign — starting a gateway from inside a
        ...
        "moor gateway start",
        "moor gateway start --all",
        # Tightened launchctl/systemctl branches: ops on NON-gateway moor
        # services must not be falsely blocked (the old `.*moor` matched any
        # moor token).
        "launchctl unload ai.moor.update-checker.plist",
        "launchctl restart ai.moor.daemon",
        ...
        "systemctl restart moor-meta.service",
        "systemctl restart moor-cron-helper",
        ...
        "echo after the moor gateway restarted cleanly",
        "the moor gateway stopped responding, please investigate",
        # #92372 Branch D: `p?kill` without a leading \b matched the "kill"
        # tail of "skill".
        "moor skill view gateway-notes && echo moor gateway docs",
        # #77173/#77536: a file path with embedded spaces containing the
        # lifecycle words must not match — `moor` is a path component
        # there, not a command.
        "cat '/docs/moor gateway restart-notes.md'",
        "less /home/user/notes/moor gateway restart runbook.txt",
    ])
    ...
    @pytest.mark.parametrize("text", [
        ...
        "moor gateway restart",
        "moor gateway restart; echo done",
        "moor gateway stop && echo stopped",
        # #77173 command-position anchor must not weaken separator/subshell
        # forms either.
        "true;moor gateway restart",
        "true && moor gateway stop",
        "echo $(moor gateway restart)",
        "echo `moor gateway restart`",
    ])
    ...
    @pytest.mark.parametrize("text", [
        ...
        'import subprocess\nsubprocess.run(["launchctl", "bootout", "gui/501/ai.moor.gateway"])',
        'subprocess.run(["moor", "gateway", "restart"])',
        'os.system("launchctl kickstart -k gui/501/ai.moor.gateway")',
    ])
    ...
    @pytest.mark.parametrize("text", [
        ...
        'data = ["moor", "notes"]  # unrelated list',
    ])
    ...
    def test_inert_heredoc_body_prose_not_blocked(self):
        ...
        text = (
            ...
            "If the box is wedged, a human can run: moor gateway restart\n"
            "EOF"
        )
        assert not _contains_gateway_lifecycle_command(text), f"Should NOT match: {text!r}"
...
class TestProfileFlagGatewayLifecycle:
    """#78028: `moor -p <profile> gateway restart|stop` bypasses Branch A's
    ...
    def _pin_profile_identity(self, monkeypatch):
        # The ambient test env may carry MOOR_HOME/MOOR_PROFILE; pin the
        # profile identity explicitly so every assertion is deterministic.
        monkeypatch.setenv("MOOR_PROFILE", "zeus")
        monkeypatch.delenv("MOOR_PROFILE_NAME", raising=False)

    @pytest.mark.parametrize("text", [
        "moor -p zeus gateway stop",
        "moor -p zeus gateway restart",
        "moor --profile zeus gateway restart",
        "moor --profile zeus gateway stop",
        "moor --profile=zeus gateway restart",
        # Global flags before/after the selector must not hide the shape.
        "moor -v -p zeus gateway restart",
        "moor -p zeus -v gateway restart",
        "moor --debug --profile zeus gateway stop",
    ...
    @pytest.mark.parametrize("text", [
        "moor -p venus gateway stop",
        "moor -p venus gateway restart",
        "moor --profile venus gateway restart",
        "moor --profile=venus gateway stop",
        "moor -p venus -v gateway restart",
    ])
    def test_sibling_allowed(self, text):
        assert not _contains_gateway_lifecycle_command(text), f"Should allow: {text!r}"

    @pytest.mark.parametrize("text", [
        "moor -p zeus gateway start",
        "moor -p zeus gateway start --all",
    ])
    ...
    def test_adjacent_form_still_blocked(self):
        ...
        assert _contains_gateway_lifecycle_command("moor gateway restart")
        assert _contains_gateway_lifecycle_command("moor gateway stop")

    def test_moor_home_derived_profile(self, monkeypatch):
        # Without MOOR_PROFILE the guard falls back to the MOOR_HOME-
        ...
        assert _contains_gateway_lifecycle_command("moor -p zeus gateway restart")
        assert not _contains_gateway_lifecycle_command("moor -p venus gateway restart")
    ...
    def test_no_profile_context_conservative_allow(self, monkeypatch):
        ...
        assert not _contains_gateway_lifecycle_command("moor -p zeus gateway restart")
        assert _contains_gateway_lifecycle_command("moor gateway restart")
...
class TestCronCreateLifecycleBlock:
    ...
    def test_block_moor_gateway_restart(self, capsys):
        args = Namespace(
            cron_command="create",
            schedule="30m",
            prompt="Upgrade moor then run moor gateway restart",
            name=None,
            deliver=None,
            repeat=None,
            skill=None,
    ...
    def test_block_script_with_lifecycle_command(self, tmp_path, capsys, monkeypatch):
        ...
        # restart_moor_gateway_once.sh). The script must live under
        # MOOR_HOME/scripts so the scheduler — and the guard — resolve it.
...
class TestGatewaySelfTargetingGuard:
    ...
    def test_stop_allows_outside_gateway(self, monkeypatch):
        ...
        monkeypatch.delenv("_MOOR_GATEWAY", raising=False)
...
class TestTerminalToolGatewayLifecycleGuard:
    """terminal_tool must refuse gateway lifecycle commands when _MOOR_GATEWAY=1.

    Issue #37453: systemctl --user restart moor-gateway runs as a child of the
    ...
    @pytest.mark.parametrize("cmd", [
        "systemctl restart moor-gateway",
        "systemctl --user restart moor-gateway",
        "systemctl stop moor-gateway.service",
        "moor gateway restart",
        "moor gateway uninstall",
        "launchctl kickstart gui/501/ai.moor.gateway",
        "launchctl bootout gui/501/ai.moor.gateway",
        # #62891 exact reported shape and its bootstrap sibling.
        "launchctl submit -l ai.moor.gateway-hard-restart-no-photon-notice -- /bin/sh ~/.moor/scripts/hard_restart_gateway_no_photon_notice.sh",
        "launchctl submit -l com.foo -- /path/gateway",
        "launchctl bootstrap gui/501 ~/Library/LaunchAgents/ai.moor.gateway.restart-once.plist",
        "pkill -f moor.*gateway",
    ])
    ...
    def test_force_true_cannot_bypass_block(self, monkeypatch):
        ...
        result = json.loads(tt.terminal_tool(
            command="systemctl restart moor-gateway", force=True
        ))
    ...
    def test_blocks_launchctl_submit_inside_gateway(self, monkeypatch, tmp_path):
        ...
        result = json.loads(tt.terminal_tool(
            command=(
                "launchctl submit -l ai.moor.delayed-ops -- "
                f"/bin/bash {script}"
            )
        ))
    ...
    @pytest.mark.parametrize("command", [
        # Neutral, non-moor label: label-independent detection is the point
        # (#62891 second reproduction used `ai.moor.svc-reload-tmp`).
        "launchctl submit -l com.foo -- /path/gateway",
        "launchctl submit -l ai.moor.svc-reload-tmp -- /bin/sh /tmp/h-svc-reload.sh",
    ...
    def test_cli_agent_session_not_blocked_by_inherited_env(
        ...
        """#92560: CLI/TUI agent sessions inherit _MOOR_GATEWAY=1 from the
        ...
        # Simulate a CLI agent session: _MOOR_GATEWAY=1 is in the
        ...
        result = json.loads(tt.terminal_tool(command="moor gateway restart"))

        assert result["exit_code"] == 0
        assert calls == ["moor gateway restart"]
    ...
    def test_launchctl_submit_parser_handles_shell_quoting(self, monkeypatch):
        ...
        self._patch_env(monkeypatch, self._make_fake_env(), inside_gateway=True)
        result = json.loads(tt.terminal_tool(
            command="launchctl sub\"\"mit -l ai.moor.loop -- /bin/true"
        ))
    ...
    def test_safe_systemctl_commands_pass_through(self, monkeypatch):
        """Non-moor systemctl commands must not be blocked by this guard."""
        import tools.terminal_tool as tt
...
class TestLifecycleGuardModule:
    ...
    def test_split_across_prompt_and_script_still_blocks(self, tmp_path):
        ...
        script.write_text("moor gateway stop\n", encoding="utf-8")
        with pytest.raises(GatewayLifecycleBlocked):
    ...
    def test_binary_script_does_not_silently_bypass(self, tmp_path):
        ...
        script.write_bytes(b"\xfemoor gateway restart\xff")
        with pytest.raises(GatewayLifecycleBlocked):
            check_gateway_lifecycle("", str(script))


    def test_relative_script_resolved_under_scripts_dir(self, tmp_path, monkeypatch):
        """A bare/relative script name resolves under MOOR_HOME/scripts (the
    ...
    def test_python_script_with_literal_lifecycle_command_still_blocked(
        ...
        script.write_text('import os\nos.system("moor gateway restart")\n', encoding="utf-8")
        with pytest.raises(GatewayLifecycleBlocked):
            check_gateway_lifecycle("clean prompt", str(script))
    ...
    def test_read_referenced_script_tolerates_nul_in_path(self):
        ...
        text, unsafe = _read_referenced_script(Path("/tmp/moor\x00binary"))
        assert text is None
    ...
    def test_binary_from_remote_callback_never_false_positives(self):
        ...
        def _remote_read(_path: str):
            return "MZ\x00\x00\x90\x00 moor gateway restart \x00\x00junk"
    ...
    def test_cron_guard_total_when_home_unresolvable(self, monkeypatch):
        """`get_moor_home()` falls back to Path.home(), which raises
        RuntimeError when neither MOOR_HOME nor HOME resolves
...
class TestRelativePathDoesNotDisableDataExemption:
    ...
    @pytest.mark.parametrize("command", [
        "grep -r 'systemctl restart moor-gateway' .",
        "grep -rn 'moor gateway restart' ./logs",
        "rg 'moor gateway restart' ../archive",
        "grep -c 'systemctl stop moor-gateway' ./var/log/syslog",
        "sqlite3 ./stats.db \"SELECT restart_reason FROM moor_gateway_restarts\"",
    ])
    ...
    @pytest.mark.parametrize("command", [
        ...
        'sqlite3 ./db ".shell moor gateway restart"',
        'sqlite3 ./db ".system systemctl restart moor-gateway"',
        'psql ./x -c "\\! systemctl restart moor-gateway"',
        "grep -r 'moor gateway restart' . | sh",
        "grep -r 'moor gateway restart' ./logs | bash",
        "grep -r 'moor gateway restart' . | sudo sh",
        "grep -r 'x' . ; moor gateway restart",
        "grep -r 'x' . && systemctl restart moor-gateway",
        'grep -r "$(moor gateway restart)" .',
        "rg 'x' ./logs | xargs systemctl restart moor-gateway",
    ])
    ...
    @pytest.mark.parametrize("command", [
        ...
        'sqlite3 db ".shell moor gateway restart"',
        'sqlite3 db ".system systemctl restart moor-gateway"',
        'psql -c "\\! systemctl restart moor-gateway"',
    ])
...
class TestCreateJobBlocksLifecycleCommands:
    ...
    moor_cli.cron.cron_create. Enforcing at create_job covers both."""
    ...
    def test_create_job_blocks_prompt_command(self):
        ...
        with pytest.raises(GatewayLifecycleBlocked):
            create_job(prompt="then run moor gateway restart", schedule="30m")
    ...
    def test_cronjob_tool_surfaces_block_as_error(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("MOOR_HOME", str(tmp_path / ".moor"))
        (tmp_path / ".moor").mkdir(parents=True)
        from tools.cronjob_tools import cronjob
        result = json.loads(cronjob(
            action="create", schedule="0 9 * * *",
            prompt="please run moor gateway restart nightly",
        ))
...
class TestLifecycleGuardDataArgumentExemption:
    ...
    @pytest.mark.parametrize("command", [
        ...
        "'systemctl stop moor-gateway'\"",
        # grep/rg pattern arguments hunting for the lifecycle string.
        "grep -c 'systemctl restart moor-gateway' /var/log/syslog",
        "rg 'moor gateway restart' /home/user/.moor/logs/",
        "journalctl -u moor-gateway --grep 'systemctl restart moor-gateway'",
        # SQL with stop/restart column/value words but no command shape.
        'sqlite3 stats.db "SELECT stop_time, restart_reason FROM '
        'moor_gateway_restarts"',
    ...
    @pytest.mark.parametrize("command", [
        ...
        'sqlite3 db ".shell moor gateway restart"',
        'psql -c "\\! systemctl restart moor-gateway"',
        "grep 'systemctl restart moor-gateway' cmds.txt | sh",
        "grep gateway f | xargs systemctl restart moor-gateway",
        'grep "$(systemctl restart moor-gateway)" f',
        "grep 'restart' log; systemctl restart moor-gateway",
        'sqlite3 db "SELECT 1"; moor gateway stop',
        # Plain lifecycle commands are unaffected by the exemption.
        "moor gateway restart",
        "sudo systemctl stop moor-gateway",
    ])
    ...
    def test_python_script_branch_gets_the_same_exemption(self, tmp_path):
        ...
        prompt = (
            ...
            "WHERE msg LIKE '%systemctl restart moor-gateway%'\""
        )
