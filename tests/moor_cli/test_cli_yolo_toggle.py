...
Pre-fix bug (issue #33925): ``cli.MoorCLI._toggle_yolo`` mutated only
``os.environ["MOOR_YOLO_MODE"]``. That env var is captured once at
...
prompt. Only ``moor --yolo`` (process-start env), ``MOOR_YOLO_MODE=1``,
and ``moor config set approvals.mode off`` actually bypassed.
...
read (``session_id``). This avoids the heavy ``MoorCLI`` construction
path used in ``test_cli_init.py``, which is incompatible with this test
file's path layout — ``MoorCLI.__init__`` imports a lot of optional
...
def _make_stand_in(session_id: str = SESSION_KEY) -> SimpleNamespace:
    ...
    to invoking them on a fully-constructed ``MoorCLI`` for the
    behaviour under test, and avoids the brittle prompt_toolkit / config
    stubbing required to instantiate ``MoorCLI`` from this test file.
    """
...
class TestToggleYoloIsSessionScoped:
    ...
    def test_toggle_yolo_disables_session_bypass_on_second_call(self):
        stand_in = _make_stand_in()
        with patch("cli._cprint"):
            MoorCLI._toggle_yolo(stand_in)  # ON
            assert approval_module.is_session_yolo_enabled(SESSION_KEY) is True
            MoorCLI._toggle_yolo(stand_in)  # OFF
            assert approval_module.is_session_yolo_enabled(SESSION_KEY) is False
...
class TestIsSessionYoloActiveHelper:
    ...
    def test_helper_reflects_toggle(self):
        ...
        assert MoorCLI._is_session_yolo_active(stand_in) is False

        with patch("cli._cprint"):
            MoorCLI._toggle_yolo(stand_in)

        assert MoorCLI._is_session_yolo_active(stand_in) is True
    ...
    def test_helper_honors_frozen_yolo_mode(self):
        """``moor --yolo`` sets ``MOOR_YOLO_MODE`` before tool imports, so
...
class TestSessionRotationTransfersYolo:
    ...
    def test_transfer_moves_yolo_to_new_session(self):
        ...
        try:
            ...
            MoorCLI._transfer_session_yolo(stand_in, "old-id", "new-id")
    ...
    def test_transfer_handles_empty_inputs_safely(self):
        ...
        # to transfer from "" / to "".
        MoorCLI._transfer_session_yolo(stand_in, "", "new")
        MoorCLI._transfer_session_yolo(stand_in, "old", "")
