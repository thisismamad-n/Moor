...
from moor_cli import moor_auth_keepalive as keepalive
...
def test_refresh_always_fires_before_expiry_for_observed_lifetimes():
    ...
    for lifetime in OBSERVED_LIFETIMES_SECONDS:
        tick = keepalive._tick_seconds(
            keepalive.MOOR_AUTH_KEEPALIVE_INTERVAL_SECONDS, lifetime
        )
        horizon = keepalive._refresh_horizon_seconds(
            tick, keepalive.MOOR_INVOKE_JWT_MIN_TTL_SECONDS
        )
...
def test_interval_precedence_and_disable(monkeypatch):
    def _config(section):
        monkeypatch.setattr(keepalive, "_moor_config", lambda: section)

    # An absent section leaves the module default in place.
    _config({})
    assert (
        keepalive._interval_seconds(None)
        == keepalive.MOOR_AUTH_KEEPALIVE_INTERVAL_SECONDS
    )

    _config({keepalive.MOOR_AUTH_KEEPALIVE_INTERVAL_CONFIG_KEY: 600})
    ...
    _config({keepalive.MOOR_AUTH_KEEPALIVE_INTERVAL_CONFIG_KEY: "not-a-number"})
    assert (
        keepalive._interval_seconds(None)
        == keepalive.MOOR_AUTH_KEEPALIVE_INTERVAL_SECONDS
    )
    ...
    _config({keepalive.MOOR_AUTH_KEEPALIVE_INTERVAL_CONFIG_KEY: 0})
    assert keepalive._interval_seconds(None) == 0
    assert keepalive.start_moor_auth_keepalive() is None
...
def test_keepalive_refreshes_stale_pool_entry(monkeypatch):
    ...
    assert keepalive.refresh_moor_auth_keepalive_once() is True
...
def test_keepalive_falls_back_to_singleton_state(monkeypatch):
    ...
    def _resolve_moor_runtime_credentials(**kwargs):
        calls.append(kwargs)
        return {
            "provider": "moor",
    ...
    monkeypatch.setattr(
        ...
        _resolve_moor_runtime_credentials,
    )

    assert keepalive.refresh_moor_auth_keepalive_once(timeout_seconds=15.0) is True
...
def test_keepalive_binds_launch_scope_for_multiplexed_singleton_refresh(tmp_path, monkeypatch, caplog):
    ...
    from moor_cli.auth_moor import _moor_inference_env_override, _moor_portal_env_override

    launch_home = tmp_path / ".moor"
    launch_home.mkdir()
    (launch_home / ".env").write_text(
        "MOOR_INFERENCE_BASE_URL=https://inference.example/v1\n"
        "MOOR_PORTAL_BASE_URL=https://portal.example\n",
        encoding="utf-8",
    )
    ...
    def _resolve(**kwargs):
        seen.append((_moor_inference_env_override(), _moor_portal_env_override(), kwargs))
        return {"provider": "moor"}

    monkeypatch.setattr(keepalive, "resolve_moor_runtime_credentials", _resolve)
...
def test_keepalive_binds_launch_scope_for_multiplexed_pool_refresh(tmp_path, monkeypatch):
    ...
    class _Pool:
        ...
        def select(self):
            seen.append((current_secret_scope() or {}).get("MOOR_INFERENCE_BASE_URL"))
            return _Entry()
