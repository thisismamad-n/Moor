"""Tests for the Kanban DB layer (moor_cli.kanban_db)."""
...
# spawned with `moor -p <profile>` must read/write the same kanban.db
...
# where `kanban_db_path()` resolved to the active profile's MOOR_HOME.
# ---------------------------------------------------------------------------

class TestSharedBoardPaths:
    """`kanban_home`/`kanban_db_path`/`workspaces_root`/`worker_log_path`
    must anchor at the **shared root**, not the active profile's MOOR_HOME."""

    def _set_home(self, monkeypatch, tmp_path, moor_home):
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("MOOR_HOME", str(moor_home))
        monkeypatch.delenv("MOOR_KANBAN_HOME", raising=False)


    def test_profile_worker_resolves_to_shared_root(
        self, tmp_path, monkeypatch
    ):
        # Reproduces the bug: dispatcher uses ~/.moor/kanban.db,
        # worker spawned with -p <profile> previously resolved to
        # ~/.moor/profiles/<profile>/kanban.db. After the fix both
        # converge on ~/.moor/kanban.db.
    ...
    def test_dispatcher_and_worker_share_a_real_database(
        ...
        # MOOR_HOME perspectives via a real SQLite file. Without the
        ...
        # Worker switches to the profile MOOR_HOME and reads.
    ...
    def test_dispatcher_spawn_injects_kanban_paths_without_stale_session(
        ...
        # MOOR_SESSION_* identity inherited from the long-lived gateway.
        # The one exception is MOOR_SESSION_SOURCE, which the dispatcher
        ...
        env = captured["env"]
        assert env["MOOR_KANBAN_DB"] == str(default_home / "kanban.db")
        assert env["MOOR_KANBAN_WORKSPACES_ROOT"] == str(
            default_home / "kanban" / "workspaces"
        )
        assert env["MOOR_KANBAN_TASK"] == "t_dispatch_env"
        assert env["MOOR_KANBAN_BRANCH"] == "wt/t_dispatch_env"
        for key in sc._VAR_MAP:
            if key == "MOOR_SESSION_SOURCE":
...
# ---------------------------------------------------------------------------
# NFS / network-filesystem fallback (see moor_state_wal.apply_wal_with_fallback)
# ---------------------------------------------------------------------------
...
def test_connect_falls_back_to_delete_on_locking_protocol(tmp_path, monkeypatch, caplog):
    ...
    import moor_state_wal as _moor_state_wal
    monkeypatch.setattr(
        _moor_state_wal, "is_sqlite_wal_reset_vulnerable",
        lambda version_info=None: False,
    )
    _moor_state_wal._wal_fallback_warned_paths.clear()

    # Clear module cache so a fresh connect() is attempted
    kb._INITIALIZED_PATHS.clear()
    moor_state_wal._wal_fallback_warned_paths.clear()
    ...
    with _patch("moor_cli.kanban_db.sqlite3.connect", side_effect=wal_blocking_connect):
        with caplog.at_level("ERROR", logger="moor_state"):
            conn = kbc.connect()
...
def test_connect_works_when_wal_is_silently_refused(tmp_path, monkeypatch, caplog):
    ...
    with _patch(
        "moor_cli.kanban_db.sqlite3.connect",
...
def test_unlink_tasks_triggers_recompute_ready(kanban_home):
    ...
    next dispatcher tick or a manual 'moor kanban recompute' would promote it.
    """
    with kbc.connect() as conn:
        # A is done.
...
# ---------------------------------------------------------------------------
# Dispatcher spawn invocation — _resolve_moor_argv()
#
# Workers spawned by the dispatcher must use a `moor` invocation that does
...
# $PATH that doesn't include the venv's bin/, so a bare `["moor", ...]`
...
def test_resolve_moor_argv_falls_back_to_module_form_when_no_path_shim(monkeypatch):
    ...
    `python -m moor` which fails with `No module named moor` on every
    invocation.
    """
    import shutil
    import sys
    ...
    argv = kbd._resolve_moor_argv()
    assert argv == [sys.executable, "-m", "moor_cli.main"]


def test_resolve_moor_argv_module_actually_runs():
    """The fallback module name must be importable + runnable.

    A unit test that pins the literal string is necessary but not
    sufficient — if `moor_cli.main` ever loses `if __name__ == "__main__"`
    handling or its argparse setup, `python -m moor_cli.main --version`
...
def test_maybe_emit_scratch_tip_fires_once_per_install(kanban_home, caplog):
    ...
    with caplog.at_level(logging.WARNING, logger="moor_cli.kanban_db"):
        with kbc.connect() as conn:
