"""Regression tests for _apply_profile_override MOOR_HOME guard (issue #22502).

When MOOR_HOME is set to the moor root (e.g. systemd hardcodes
MOOR_HOME=/root/.moor), _apply_profile_override must still read
active_profile and update MOOR_HOME to the profile directory.

When MOOR_HOME is already a profile directory (.../profiles/<name>),
...
def _platform_home(tmp_path, monkeypatch):
    monkeypatch.setattr("moor_constants._get_platform_default_moor_home", lambda: tmp_path / ".moor")
...
def _run_apply_profile_override(
    ...
    Returns the value of os.environ["MOOR_HOME"] after the call,
    or None if unset.
    """
    ...
    if active_profile and active_profile != "default":
        ...
        (moor_root / "profiles" / active_profile / "config.yaml").write_text("{}\n")  # identity marker

    monkeypatch.setattr(Path, "home", lambda: tmp_path)
    if moor_home is not None:
        monkeypatch.setenv("MOOR_HOME", moor_home)
    else:
    ...
    return os.environ.get("MOOR_HOME")


class TestApplyProfileOverrideMoorHomeGuard:
    """Regression guard for issue #22502.

    Verifies that MOOR_HOME pointing to the moor root does NOT suppress
    the active_profile check, while MOOR_HOME already pointing to a
    profile directory IS trusted as-is.
    """

    def test_moor_home_at_root_with_active_profile_is_redirected(
        self, tmp_path, monkeypatch
    ):
        """MOOR_HOME=/root/.moor + active_profile=coder must redirect
        MOOR_HOME to .../profiles/coder.

        Bug scenario from #22502: systemd sets MOOR_HOME to the moor root
        ...
        assert result is not None, "MOOR_HOME must be set after profile redirect"
        assert "profiles" in result, (
            f"Expected MOOR_HOME to point into profiles/ dir, got: {result!r}"
        )
        assert result.endswith("coder"), (
            f"Expected MOOR_HOME to end with 'coder', got: {result!r}"
        )
    ...
    def test_sudo_explicit_profile_resolves_invoking_users_profile(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(sys, "argv", ["moor", "-p", "elias", "gateway", "install", "--system"])

        import pwd

        monkeypatch.setattr(pwd, "getpwnam", lambda name: SimpleNamespace(pw_dir=str(user_home)))

        from moor_cli.main import _apply_profile_override, _resolve_sudo_user_profile_env
        _apply_profile_override()
        ...
        (user_home / ".moor" / "profiles" / "ghost" / "cron").mkdir(parents=True)
...
class TestSupervisedChildIgnoresStickyProfile:
    ...
    bare ``moor gateway run`` (no ``-p``) to mean "the root MOOR_HOME
    profile". The run-script exports ``MOOR_S6_SUPERVISED_CHILD=1``.
    ...
    def test_non_supervised_run_still_follows_active_profile(
        ...
        """Without the sentinel, a normal `moor gateway run` still honors
    ...
    def test_supervised_named_profile_flag_still_wins(self, tmp_path, monkeypatch):
        ...
        for name in ("briefer", "coder"):
            (moor_root / "profiles" / name).mkdir(parents=True, exist_ok=True)
            (moor_root / "profiles" / name / "config.yaml").write_text("{}\n")  # identity marker

        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.delenv("MOOR_HOME", raising=False)
        monkeypatch.setenv("MOOR_S6_SUPERVISED_CHILD", "1")
        monkeypatch.setattr(sys, "argv", ["moor", "-p", "coder", "gateway", "run"])

        from moor_cli.main import _apply_profile_override
        _apply_profile_override()

        result = os.environ.get("MOOR_HOME")
        assert result is not None
        assert result.endswith("coder")
...
class TestS6ContainerGatewayRun:
    ...
    def test_the_redirected_run_keeps_the_root_home_despite_the_active_profile(
        ...
        monkeypatch.setattr("moor_cli.service_manager._s6_running", lambda: True)
        root = tmp_path / ".moor"
        result = _run_apply_profile_override(
            tmp_path, monkeypatch, moor_home=str(root), active_profile="coder",
            argv=["moor", "gateway", "run"],
        )
    ...
    def test_a_foreground_run_and_other_verbs_still_follow_the_active_profile(
        ...
        monkeypatch.setattr("moor_cli.service_manager._s6_running", lambda: True)
        root = tmp_path / ".moor"
        for argv in (["moor", "gateway", "run", "--no-supervise"], ["moor", "chat"]):
            result = _run_apply_profile_override(
                tmp_path, monkeypatch, moor_home=str(root), active_profile="coder", argv=argv,
            )
