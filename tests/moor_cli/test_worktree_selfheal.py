...
Regression for the Aug 2026 `moor -w` timeout incident: 39 accumulated packs
...
(lock pid = the live moor process), poisoning every retry.
...
class TestCleanupFailedWorktreeAdd:
    def _simulate_timed_out_add(self, repo):
        ...
        wt = repo / ".worktrees" / "moor-dead00"
        _git(repo, "worktree", "add", str(wt), "-b", "moor/moor-dead00")
        # Live-pid lock, exactly what `moor -w` writes before the checkout.
        _git(repo, "worktree", "lock", str(wt), "--reason", "moor pid=999999")
    ...
    def test_sweeps_dir_admin_entry_and_branch(self, repo):
        from moor_cli.worktree_ops import _cleanup_failed_worktree_add

        wt = self._simulate_timed_out_add(repo)
        admin = repo / ".git" / "worktrees" / "moor-dead00"
        assert admin.exists() and (admin / "locked").exists()

        _cleanup_failed_worktree_add(str(repo), wt, "moor/moor-dead00")

        assert not wt.exists(), "partial worktree dir must be removed"
        assert not admin.exists(), "LOCKED admin entry must be removed"
        branches = _git(repo, "branch", "--list", "moor/moor-dead00").stdout
    ...
    def test_retry_succeeds_after_cleanup(self, repo):
        ...
        result = _git(
            repo, "worktree", "add", str(wt), "-b", "moor/moor-dead00", check=False
        )
    ...
    def test_noop_when_nothing_exists(self, repo):
        ...
        _cleanup_failed_worktree_add(
            str(repo), repo / ".worktrees" / "never-existed", "moor/never-existed"
        )  # must not raise
