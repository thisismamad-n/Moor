"""Tests for moor_cli.update_receipt — Phase 1 of the fleet-update plan (#91277).
...
class TestReceiptLifecycle:
    def test_begin_record_finalize_roundtrip(self, receipt_home):
        ...
        ur.record_gateway_restart(
            restarted_services=["moor-gateway"],
        ...
        assert gr["restarted_services"] == ["moor-gateway"]
...
class TestCommandBoundaryFinalization:
    ...
    def test_pending_receipt_persisted_on_exit_2_refusal(self, receipt_home):
        ur.begin_update_receipt()
        ur.record_step("windows_preflight", False, "another moor.exe running")
    ...
    def test_cmd_update_boundary_finalizes_on_early_exit(
        ...
        def _fake_impl(args, gateway_mode):
            ur.begin_update_receipt()
            ur.record_step("windows_preflight", False, "moor.exe holds venv")
            sys.exit(2)

        monkeypatch.setattr(moor_main, "_cmd_update_impl", _fake_impl)
        monkeypatch.setattr(
            moor_main, "detect_install_method", lambda *a, **k: "git", raising=False
        )
        monkeypatch.setattr(
            moor_main,
        ...
        monkeypatch.setattr(
            moor_main, "_finalize_update_output", lambda state: None, raising=False
        )
        ...
        import moor_cli.update_lock as update_lock_mod
...
class TestGatewayStatusStamping:
    ...
    def test_code_identity_failure_degrades_to_absent(self, monkeypatch):
        ...
        monkeypatch.setattr("moor_cli.build_info.get_code_identity", _boom)
        record = gs._build_runtime_status_record()
