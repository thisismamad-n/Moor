...
When ``MOOR_HOME`` points to a named profile, ``read_credential_pool()``
...
def profile_env(tmp_path, monkeypatch):
    """Set up a global root + an active profile under Path.home()/.moor/profiles/coder.

    * Path.home() -> tmp_path
    * Global root -> tmp_path/.moor            (has its own auth.json fixture)
    * Profile     -> tmp_path/.moor/profiles/coder   (active, MOOR_HOME points here)
...
def test_provider_auth_state_falls_back_to_global_when_profile_has_none(profile_env):
    ...
    _write(profile_env["global"] / "auth.json", _make_auth_store(providers={
        "moor": {"access_token": "moor-global", "refresh_token": "rt-global"},
    }))
    _write(profile_env["profile"] / "auth.json", _make_auth_store(providers={}))

    state = get_provider_auth_state("moor")
    assert state is not None
    assert state["access_token"] == "moor-global"
...
def test_provider_auth_state_returns_none_when_neither_has_it(profile_env):
    ...
    assert get_provider_auth_state("moor") is None


# ---------------------------------------------------------------------------
# _load_provider_state — internal global fallback (issue #18594 follow-up)
#
# Several runtime helpers (notably ``resolve_moor_runtime_credentials`` and
# ``resolve_moor_access_token``) call ``_load_provider_state`` directly with
...
# logged into Moor Portal"`` even though the user has a valid global Moor
...
def classic_env(tmp_path, monkeypatch):
    """Classic single-root layout (MOOR_HOME != ~/.moor, no profiles)."""
    fake_home = tmp_path / "home"
    fake_home.mkdir()
    monkeypatch.setattr(Path, "home", lambda: fake_home)
    moor_home = tmp_path / "classic"
    moor_home.mkdir()
