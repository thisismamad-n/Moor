"""Tests for moor_cli.gui_uninstall — GUI-only uninstall + install discovery.
...
def _make_agent(moor_home: Path) -> Path:
    """Create a fake agent install: source package + venv."""
    agent_root = moor_home / "moor-agent"
    (agent_root / "moor_cli").mkdir(parents=True)
    (agent_root / "moor_cli" / "__init__.py").write_text("")
    (agent_root / "venv" / "bin").mkdir(parents=True)
    return agent_root


def _make_gui_build(moor_home: Path) -> None:
    """Create the source-built GUI artifacts a `moor desktop` run produces."""
    desktop = moor_home / "moor-agent" / "apps" / "desktop"
    ...
    (moor_home / "moor-agent" / "node_modules").mkdir(parents=True)
    (moor_home / "desktop-build-stamp.json").write_text("{}")


def _make_user_data(moor_home: Path) -> None:
    (moor_home / "config.yaml").write_text("x: 1\n")
    (moor_home / ".env").write_text("KEY=secret\n")
    (moor_home / "sessions").mkdir()
...
def test_gui_install_summary_shape(tmp_path, monkeypatch):
    ...
    _make_agent(moor_home)
    _make_gui_build(moor_home)
    monkeypatch.setattr(gu, "packaged_gui_app_paths", lambda: [])
    monkeypatch.setattr(gu, "desktop_userdata_dir", lambda: tmp_path / "none")

    summary = gu.gui_install_summary(moor_home)
    ...
    assert summary["moor_home"] == str(moor_home)
...
def test_linux_discovery_includes_launcher_entry(tmp_path, monkeypatch):
    """The launcher entry that `moor desktop` installs is removable."""
...
def test_uninstall_removes_launcher_entry_and_refreshes_cache(tmp_path, monkeypatch):
    ...
    moor_home = tmp_path / ".moor"
    _make_agent(moor_home)
    icon = lde.icon_path(moor_home / "moor-agent")
    icon.parent.mkdir(parents=True, exist_ok=True)
    icon.write_bytes(b"\x89PNG")
    monkeypatch.setattr(gu, "desktop_userdata_dir", lambda: tmp_path / "none")

    removed = gu.uninstall_gui(moor_home)
    ...
    assert lde.icon_path(moor_home / "moor-agent").exists()
    # The agent itself survives a GUI uninstall.
    assert (moor_home / "moor-agent" / "moor_cli").is_dir()
...
def test_uninstall_skips_cache_refresh_when_no_launcher_entry(tmp_path, monkeypatch):
    ...
    gu.uninstall_gui(tmp_path / ".moor")
