"""Tests for npm ``EBADENGINE`` recovery (``moor_cli/npm_engine.py``).
...
import moor_cli.npm_engine as npm_engine
from moor_cli.npm_engine import (
    actual_npm_version,
    is_ebadengine,
    managed_npm_prefix,
...
npm error engine Not compatible with your version of node/npm: moor-agent@1.0.0
npm error notsup Not compatible with your version of node/npm: moor-agent@1.0.0
...
class TestRepairDecision:
    ...
    def test_foreign_npm_provisions_managed_runtime_instead(
        ...
        upgrades = []
        monkeypatch.setattr(
            npm_engine, "bootstrap_moor_managed_node", fake_bootstrap
        )
        monkeypatch.setattr(
            npm_engine,
            "upgrade_managed_npm",
    ...
    def test_foreign_npm_failed_bootstrap_prints_manual_fix(
        ...
        monkeypatch.setattr(
            npm_engine, "bootstrap_moor_managed_node", lambda: None
        )
