"""Tests for `moor fallback` — chain reading, add/remove/clear, legacy migration."""
...
def _write_config(home: Path, data: dict) -> None:
    config_path = home / ".moor" / "config.yaml"
...
class TestReadChain:
    def test_returns_empty_list_when_unset(self):
        from moor_cli.fallback_cmd import _read_chain
        assert _read_chain({}) == []
    ...
    def test_reads_new_list_format(self):
        ...
        cfg = {
            "fallback_providers": [
                ...
                {"provider": "moor", "model": "Hermes-4-Llama-3.1-405B"},
            ]
        }
    ...
    def test_returns_copies_not_aliases(self):
        from moor_cli.fallback_cmd import _read_chain
        cfg = {"fallback_providers": [{"provider": "moor", "model": "foo"}]}
        result = _read_chain(cfg)
        result[0]["provider"] = "mutated"
        assert cfg["fallback_providers"][0]["provider"] == "moor"
...
class TestExtractFallback:
    def test_extracts_from_default_field(self):
        from moor_cli.fallback_cmd import _extract_fallback_from_model_cfg
...
class TestAddCommand:
    def test_add_appends_new_entry(self, isolated_home, capsys):
        ...
        with patch("moor_cli.main.select_provider_and_model", side_effect=fake_picker), \
                patch("moor_cli.main._require_tty"):
            from moor_cli.fallback_cmd import cmd_fallback_add
    ...
    def test_restore_preserves_absent_active_provider(self):
        ...
        from moor_cli import auth, fallback_cmd
    ...
    def test_picker_failure_restores_persisted_primary_without_masking_error(
        ...
        from moor_cli import fallback_cmd
        ...
        with patch(
            "moor_cli.main.select_provider_and_model",
...
class TestRemoveCommand:
    ...
    def test_remove_selected_entry(self, isolated_home, capsys):
        ...
        # Picker returns index 1 (the middle entry, "moor / Hermes-4")
        with patch("moor_cli.setup._curses_prompt_choice", return_value=1):
            from moor_cli.fallback_cmd import cmd_fallback_remove
...
class TestClearCommand:
    ...
    def test_clear_with_confirmation(self, isolated_home, capsys, monkeypatch):
        ...
        from moor_cli.fallback_cmd import cmd_fallback_clear
...
class TestArgparseWiring:
    """Verify `moor fallback` is wired into main.py's argparse tree.
    ...
    def test_fallback_help_lists_subcommands(self):
        ...
        result = subprocess.run(
            [sys.executable, "-m", "moor_cli.main", "fallback", "--help"],
            capture_output=True,
            text=True,
            timeout=30,
        )
