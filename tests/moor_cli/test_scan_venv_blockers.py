"""Tests for moor_cli/_scan_venv_blockers.py.
...
from moor_cli._scan_venv_blockers import (
...
def test_redact_long_flag_value_space_separated() -> None:
    ...
    raw = "python.exe -m moor_cli.main serve --token ghp_abc123 --host 10.0.0.1"
    result = _redact_sensitive_cmdline(raw)
    assert result == "python.exe -m moor_cli.main serve --token <redacted>"
...
def test_redact_normal_host_port_profile_remain() -> None:
    raw = "python.exe -m moor_cli.main serve --host 10.0.0.1 --port 9119 --profile work"
    result = _redact_sensitive_cmdline(raw)
...
def test_redact_no_sensitive_flags_is_noop() -> None:
    raw = "python.exe -m moor_cli.main serve --host 127.0.0.1"
...
def test_redact_short_flags_not_redacted() -> None:
    ...
    raw = "python.exe -m moor_cli.main serve -t web -p default -k somearg"
    result = _redact_sensitive_cmdline(raw)
...
def test_classify_local_preview_args_preserves_full_directory_label_and_port() -> None:
    args = [
        r"C:\Moor\venv\Scripts\python.exe",
        "-m",
        "http.server",
        "8766",
        "--bind",
...
#
# `moor-setup` always invokes `moor update --yes --gateway`, whose
...
@pytest.mark.parametrize(
    ...
    [
        ...
        r"C:\Users\u\AppData\Local\moor\moor-agent\venv\Scripts\python.exe"
...
@pytest.mark.parametrize(
    ...
    [
        ...
        "python.exe",
        "python.exe myscript.py gateway run",  # not a moor_cli.main invocation
        "",
    ],
)
...
def test_main_exempts_gateway_chain_but_keeps_other_holders(monkeypatch, capsys):
    ...
    gateway_launcher = (
        12,
        "python.exe",
        r"C:\x\venv\Scripts\python.exe -m moor_cli.main gateway run --replace",
    )
    gateway_worker = (
        34,
        "python.exe",
        r'"C:\u\uv\python\python.exe"  -m moor_cli.main gateway run --replace',
    )
...
def test_main_desktop_serve_backend_still_blocks(monkeypatch, capsys):
    ...
    serve = (
        78,
        "python.exe",
        r"C:\x\venv\Scripts\python.exe -m moor_cli.main serve --host 127.0.0.1",
    )
...
_SERVE_CMD = r"C:\x\venv\Scripts\python.exe -m moor_cli.main serve --host 127.0.0.1"
...
def test_updater_owned_backend_live_foreign_spawner_still_blocks(monkeypatch, capsys):
    ...
    from moor_cli import _scan_venv_blockers as scan_mod
...
def test_updater_owned_backend_non_backend_argv_never_deferred(monkeypatch, capsys):
    ...
    kanban = (
        78,
        "python.exe",
        r"C:\x\venv\Scripts\python.exe -m moor_cli.main kanban --preserve-cache",
    )
...
def test_main_gateway_with_long_managed_runtime_path_is_exempt(monkeypatch, capsys):
    ...
    exe path (`.moor-runtime\python\generation-...\cpython-3.11-...`).
    The old `cmdline_raw[:120]` truncation in the detector cut the cmdline
    before `-m moor_cli.main gateway run`, so the exemption never matched
    ...
    """
    long_exe = (
        r'"C:\Users\u\AppData\Local\moor\moor-agent\.moor-runtime\python'
    ...
    gateway = (91, "python.exe", long_exe + "  -m moor_cli.main gateway run --replace")
