"""Tests for moor_cli/skills_config.py and skills_tool disabled filtering."""
...
class TestSaveDisabledSkills:
    @patch("moor_cli.skills_config.save_config")
...
class TestIsSkillDisabled:
    ...
    @patch("moor_cli.config.load_config")
    ...
    @patch("moor_cli.config.load_config")
    @patch.dict("os.environ", {"MOOR_PLATFORM": "discord"})
...
class TestGetDisabledSkillNames:
    ...
    def test_session_platform_env_var(self, tmp_path, monkeypatch):
        """MOOR_SESSION_PLATFORM should be used when MOOR_PLATFORM is unset."""
        config = tmp_path / "config.yaml"
        config.write_text(
            "skills:\n"
            "  disabled:\n"
    ...
    def test_moor_platform_takes_precedence(self, tmp_path, monkeypatch):
        """MOOR_PLATFORM should win over MOOR_SESSION_PLATFORM."""
        config = tmp_path / "config.yaml"
        config.write_text(
            "skills:\n"
            "  platform_disabled:\n"
...
class TestGetCategories:
    def test_extracts_unique_categories(self):
        from moor_cli.skills_config import _get_categories
        skills = [
