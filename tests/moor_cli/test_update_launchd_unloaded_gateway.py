"""Regression for #74973 — `moor update` must not leave the gateway down.
...
cron stayed dark until someone manually ran ``moor gateway restart``.
...
def launchd(monkeypatch):
    """Stub moor_cli.gateway so no real launchctl call is made."""
    calls: list[str] = []
    ...
    monkeypatch.setattr(gateway_mod, "get_launchd_label", lambda: "ai.moor.gateway", raising=False)
...
ai.moor.gateway = {
\tactive count = 1
\tpath = /Users/u/Library/LaunchAgents/ai.moor.gateway.plist
\tstate = running
\tpid = 59038
\tprogram = /Users/u/.moor/bin/moor
}
"""
...
class TestParseLaunchdPidFromPrintOutput:
    def test_running_service(self):
        from moor_cli.gateway import _parse_launchd_pid_from_print_output
