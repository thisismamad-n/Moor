"""Tests for moor backup and import commands."""
...
def _advance_backup_clock(seconds: float = 1.1) -> None:
    """Skew moor_cli.backup's datetime forward instead of sleeping.
    ...
    import moor_cli.backup as _backup

    shim = getattr(_backup.datetime, "_moor_test_shim", None)
    if shim is None:
        class _ShimDatetime(_dt.datetime):
            _moor_test_shim = True
...
def _make_moor_tree(root: Path) -> None:
    """Create a realistic ~/.moor directory structure for testing."""
    (root / "config.yaml").write_text("model:\n  provider: openrouter\n")
    (root / ".env").write_text("OPENROUTER_API_KEY=sk-test-123\n")
    for db_name in ("memory_store.db", "moor_state.db"):
    ...
    # moor-agent repo (should be EXCLUDED)
    (root / "moor-agent").mkdir(exist_ok=True)
    (root / "moor-agent" / "run_agent.py").write_text("# big file\n")
    (root / "moor-agent" / ".git").mkdir()
    (root / "moor-agent" / ".git" / "HEAD").write_text("ref: refs/heads/main\n")
...
class TestShouldExclude:
    def test_excludes_moor_agent(self):
        from moor_cli.backup import _should_exclude
        assert _should_exclude(Path("moor-agent/run_agent.py"))
        assert _should_exclude(Path("moor-agent/.git/HEAD"))
    ...
    def test_excludes_state_snapshots_dir(self):
        ...
        from moor_cli.backup import _EXCLUDED_DIRS, _QUICK_SNAPSHOTS_DIR, _should_exclude
    ...
    def test_excludes_desktop_emergency_state_db_baks(self):
        ...
        state.db.pre-update-emergency-*.bak files at the MOOR_HOME root —
...
class TestIterBackupFiles:
    def test_manual_and_automatic_paths_share_one_walk(self, tmp_path):
        ...
        ``moor-agent`` at ANY depth, silently dropping nested skill dirs
        like ``skills/autonomous-ai-agents/moor-agent/`` that the manual
        ...
        root = tmp_path / ".moor"
        root.mkdir()
        _make_moor_tree(root)

        # The case the old automatic walk got wrong: a nested dir named
        # moor-agent holding real skill content.
        nested = root / "skills" / "autonomous-ai-agents" / "moor-agent"
        ...
        rel_nested = str(Path("skills/autonomous-ai-agents/moor-agent/SKILL.md"))
...
class TestBackup:
    ...
    def test_pre_update_db_snapshots_staged_beside_output_zip(self, tmp_path, monkeypatch):
        ...
        out_zip = moor_home / "backups" / "pre-update-test.zip"
        ...
        monkeypatch.setattr(backup_mod.tempfile, "NamedTemporaryFile", _spy)
        result = backup_mod._write_full_zip_backup(out_zip, moor_home)
    ...
    def test_skips_symlinked_files(self, tmp_path, monkeypatch):
        """Backup must not dereference symlinks and leak files outside MOOR_HOME."""
        moor_home = tmp_path / ".moor"
        moor_home.mkdir()
        _make_moor_tree(moor_home)
        outside = tmp_path / "outside-secret.txt"
        outside.write_text("outside secret\n")
        _symlink_file_or_skip(moor_home / "skills" / "outside-link.txt", outside)
    ...
    def test_state_snapshots_not_nested_into_backup(self, tmp_path, monkeypatch):
        ...
        moor_home.mkdir()
        _make_moor_tree(moor_home)
        with sqlite3.connect(moor_home / "state.db") as conn:
            conn.execute("CREATE TABLE sessions (id TEXT PRIMARY KEY)")
            conn.execute("INSERT INTO sessions VALUES ('s1')")

        from moor_cli.backup import _QUICK_SNAPSHOTS_DIR, create_quick_snapshot, run_backup

        # Real producer, so the layout under state-snapshots/ is whatever the
        # code actually writes (manifest.json + state.db copy + ...).
        snap_id = create_quick_snapshot(moor_home=moor_home)
        assert snap_id and (moor_home / _QUICK_SNAPSHOTS_DIR / snap_id / "state.db").exists()
...
class TestValidateBackupZip:
    ...
    def test_state_db_passes(self, tmp_path):
        ...
        from moor_cli.backup import _validate_backup_zip
...
class TestImport:
    ...
    def test_preserves_per_profile_gateway_state(self, tmp_path, monkeypatch):
        ...
        moor_home = tmp_path / ".moor"
        (moor_home / "profiles" / "coder").mkdir(parents=True)
        monkeypatch.setenv("MOOR_HOME", str(moor_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)

        live_state = '{"gateway_state": "running"}'
        (moor_home / "profiles" / "coder" / "gateway_state.json").write_text(live_state)
        ...
        assert (moor_home / "profiles" / "coder" / "config.yaml").read_text() == "model: anthropic\n"
        assert (
            moor_home / "profiles" / "coder" / "gateway_state.json"
    ...
    def test_preserves_runtime_pid_and_process_files(self, tmp_path, monkeypatch):
        ...
        (moor_home / "gateway.pid").write_text("4242")
        (moor_home / "processes.json").write_text('{"live": true}')
        ...
        assert (moor_home / "gateway.pid").read_text() == "4242"
        assert (moor_home / "processes.json").read_text() == '{"live": true}'
        # cron.pid / gateway.lock had no live copy and were not seeded.
        assert not (moor_home / "cron.pid").exists()
        assert not (moor_home / "gateway.lock").exists()
    ...
    def test_restores_secret_files_with_0600_perms(self, tmp_path, monkeypatch):
        ...
        self._make_backup_zip(zip_path, {
            ...
            "auth.json": '{"providers": {"moor": "token"}}',
        ...
        for rel in (".env", "auth.json", "state.db", "profiles/coder/.env"):
            mode = (moor_home / rel).stat().st_mode & 0o777
...
class TestRoundTrip:
    def test_backup_then_import(self, tmp_path, monkeypatch):
        ...
        # Source
        src_home = tmp_path / "source" / ".moor"
        src_home.mkdir(parents=True)
        _make_moor_tree(src_home)

        monkeypatch.setenv("MOOR_HOME", str(src_home))
        ...
        dst_home = tmp_path / "dest" / ".moor"
        dst_home.mkdir(parents=True)
        monkeypatch.setenv("MOOR_HOME", str(dst_home))
        ...
        # moor-agent should NOT be present
        assert not (dst_home / "moor-agent").exists()
...
class TestValidation:
    ...
    def test_detect_prefix_only_dirs(self):
        ...
        import io
        from moor_cli.backup import _detect_prefix

        buf = io.BytesIO()
        with zipfile.ZipFile(buf, "w") as zf:
            # Only directory entries (trailing slash)
            zf.writestr(".moor/", "")
            zf.writestr(".moor/skills/", "")
        buf.seek(0)
        with zipfile.ZipFile(buf, "r") as zf:
...
class TestBackupEdgeCases:
    ...
    def test_empty_moor_home(self, tmp_path, monkeypatch):
        """Backup handles empty moor home (no files to back up)."""
        moor_home = tmp_path / ".moor"
        moor_home.mkdir()
        # Only excluded dirs, no actual files
        (moor_home / "__pycache__").mkdir()
        (moor_home / "__pycache__" / "foo.pyc").write_bytes(b"\x00")
    ...
    def test_pre1980_timestamp_skipped(self, tmp_path, monkeypatch):
        ...
        moor_home = tmp_path / ".moor"
        moor_home.mkdir()
        (moor_home / "config.yaml").write_text("model: test\n")

        # Create a file with epoch timestamp (1970-01-01)
        old_file = moor_home / "ancient.txt"
...
class TestImportEdgeCases:
    ...
    def test_eof_during_confirmation(self, tmp_path, monkeypatch):
        ...
        moor_home = tmp_path / ".moor"
        moor_home.mkdir()
        (moor_home / "config.yaml").write_text("existing\n")
        monkeypatch.setenv("MOOR_HOME", str(moor_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
    ...
    def test_progress_with_many_files(self, tmp_path, monkeypatch):
        ...
        assert (moor_home / "config.yaml").exists()
        assert (moor_home / "sessions" / "s0599.json").exists()
...
class TestImportAtomicWrites:
    """`moor import` must never leave a user's file truncated.
    ...
    tests pin the invariant for both restore branches: the MOOR_HOME branch
    ...
    def test_failed_member_leaves_existing_file_intact(self, tmp_path, monkeypatch):
        ...
        (moor_home / "config.yaml").write_text(original)
        monkeypatch.setenv("MOOR_HOME", str(moor_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        ...
        assert (moor_home / "config.yaml").read_text() == original
        # And the aborted write must not litter the directory it staged in.
        assert list(moor_home.glob(".config.yaml.*")) == []

    def test_failed_external_member_leaves_existing_file_intact(self, tmp_path, monkeypatch):
        """Same invariant on the `_external/` branch, which writes outside MOOR_HOME."""
        dst_home = tmp_path / "dst"
        dst_home.mkdir()
        moor_home = dst_home / ".moor"
        moor_home.mkdir()
    ...
    def test_restore_preserves_existing_file_owner(self, tmp_path, monkeypatch):
        ...
        ``sudo moor import`` onto a user-owned (or Docker/NAS volume-owned)
        MOOR_HOME would hand every restored file to root. The uid/gid is
        ...
        monkeypatch.setattr(
            "moor_cli.backup._preserve_file_owner",
    ...
    def test_restore_does_not_carry_setuid_onto_archive_content(
        ...
        re-serialize content this process produced; ``moor import`` is the
...
class TestSafeCopyDb:
    def test_copies_valid_database(self, tmp_path):
        from moor_cli.backup import _safe_copy_db
...
class TestQuickSnapshot:
    @pytest.fixture
    def moor_home(self, tmp_path):
        """Create a fake MOOR_HOME with critical state files."""
        home = tmp_path / ".moor"
        home.mkdir()
    ...
    def test_state_db_safely_copied(self, moor_home):
        from moor_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(moor_home=moor_home)
        db_copy = moor_home / "state-snapshots" / snap_id / "state.db"
    ...
    def test_failed_state_db_copy_is_loud(self, moor_home, monkeypatch, capsys):
    ...
    def test_restore_refused_db_is_not_counted(self, moor_home, monkeypatch):
        """A refused live-safe restore (holder detected, backup leg failed) must
        not be counted as a restored file — `moor import` reports it, and
        /snapshot restore must not claim success for that file either."""
        import moor_cli.backup as backup_mod
        from moor_cli.backup import create_quick_snapshot, restore_quick_snapshot
        ...
        restore_quick_snapshot(snap_id, moor_home=moor_home)

        manifest = json.loads(
            (backup_mod._quick_snapshot_root(moor_home) / snap_id / "manifest.json").read_text()
        )
    ...
    def test_restore_state_db_live_connection(self, moor_home):
        ...
        live_conn = sqlite3.connect(str(moor_home / "state.db"))
        ...
        result = restore_quick_snapshot(snap_id, moor_home=moor_home)
        assert result is True
    ...
    def test_snapshot_includes_pairing_directories(self, moor_home):
        ...
        (moor_home / "platforms" / "pairing").mkdir(parents=True)
        (moor_home / "platforms" / "pairing" / "telegram-approved.json").write_text(
            '{"12345": {"user_name": "alice"}}'
        )
        (moor_home / "platforms" / "pairing" / "discord-approved.json").write_text(
            '{"67890": {"user_name": "bob"}}'
        )
        # Legacy pairing store (old location)
        (moor_home / "pairing").mkdir()
        (moor_home / "pairing" / "matrix-approved.json").write_text(
            '{"@charlie:server": {"user_name": "charlie"}}'
        )
        # Feishu's separate JSON
        (moor_home / "feishu_comment_pairing.json").write_text(
        ...
        snap_dir = moor_home / "state-snapshots" / snap_id
...
# ---------------------------------------------------------------------------
# Pre-update backup (moor update safety net)
# ---------------------------------------------------------------------------
    ...
    def test_oversized_db_suppresses_pruning(self, moor_home, capsys):
        ...
        """
        import json
        from moor_cli.backup import create_quick_snapshot, list_quick_snapshots

        # First snapshot: complete (state.db is small, under any cap)
        first_id = create_quick_snapshot(label="complete", moor_home=moor_home)
        assert first_id is not None
        first_dir = moor_home / "state-snapshots" / first_id
        ...
        second_id = create_quick_snapshot(
            label="oversized", moor_home=moor_home, max_file_size=1024, keep=1
        )
        assert second_id is not None
        second_dir = moor_home / "state-snapshots" / second_id
        ...
        all_snaps = list_quick_snapshots(limit=100, moor_home=moor_home)
...
class TestQuickSnapshotProjectsKanban:
    ...
    def test_non_default_kanban_board_snapshotted(self, moor_home):
        ...
        board_dir = moor_home / "kanban" / "boards" / "work"
        ...
        snap_id = create_quick_snapshot(moor_home=moor_home)
        copy = (
            moor_home / "state-snapshots" / snap_id
        ...
        assert restore_quick_snapshot(snap_id, moor_home=moor_home) is True
        conn = sqlite3.connect(str(board_dir / "kanban.db"))
    ...
    def test_board_db_copied_wal_safely(self, moor_home, monkeypatch):
        ...
        board = moor_home / "kanban" / "boards" / "work"
        ...
        copy = moor_home / "state-snapshots" / snap_id / "kanban" / "boards" / "work" / "kanban.db"
        rows = sqlite3.connect(str(copy)).execute("SELECT * FROM tasks").fetchall()
        assert rows == [("w1", "ship")]


class TestPreUpdateBackup:
    """Tests for create_pre_update_backup — the auto-backup ``moor update``
    ...
    def test_backup_contents_match_full_backup(self, moor_home):
        """Pre-update backup should include the same user data that
        ``moor backup`` would, and should exclude the same directories."""
        from moor_cli.backup import create_pre_update_backup
        out = create_pre_update_backup(moor_home=moor_home)
        ...
        # moor-agent repo excluded
        assert not any(n.startswith("moor-agent/") for n in names)
        # __pycache__ excluded
        assert not any("__pycache__" in n for n in names)
        # pid files excluded
        assert "gateway.pid" not in names

    def test_pre_update_zip_does_not_nest_the_pre_update_snapshot(self, moor_home):
        """``moor update`` in ``full`` mode takes the quick snapshot *before*
        ...
        snap_id = create_quick_snapshot(label="pre-update", moor_home=moor_home)
        assert snap_id and (moor_home / _QUICK_SNAPSHOTS_DIR / snap_id / "state.db").exists()
    ...
    def test_rotation_keeps_only_n(self, moor_home):
        ...
        created = []
        for _ in range(5):
            out = create_pre_update_backup(moor_home=moor_home, keep=3)
            created.append(out)
            _advance_backup_clock()

        remaining = sorted(
            p.name for p in (moor_home / "backups").iterdir()
    ...
    def test_skips_symlinked_files(self, moor_home, tmp_path):
        """Pre-update backups must not dereference symlinks outside MOOR_HOME."""
        from moor_cli.backup import create_pre_update_backup
...
class TestRunPreUpdateBackup:
    ...
    def moor_home(self, tmp_path, monkeypatch):
        ...
        root.mkdir()
        _make_moor_tree(root)
        # Point MOOR_HOME at the temp dir so config + backup paths resolve here
        monkeypatch.setenv("MOOR_HOME", str(root))
        # Make Path.home() point at tmp_path for anything that uses it
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        # Config reads resolve MOOR_HOME dynamically and their caches are
    ...
    @staticmethod
    def _snaps(moor_home):
        d = moor_home / "state-snapshots"
        return [p for p in d.iterdir() if p.is_dir()] if d.exists() else []




    def test_config_off_disables_everything_silently(self, moor_home, capsys):
        ...
        assert out == ""
        assert not self._snaps(moor_home)
        assert not self._zips(moor_home)



    def test_config_full_mode(self, moor_home, capsys):
        self._set_mode(moor_home, "full")
        from moor_cli.update_cmd import _run_pre_update_backup
        snap_id = _run_pre_update_backup(Namespace(no_backup=False, backup=False))
        out = capsys.readouterr().out
        assert snap_id is not None
...
# ---------------------------------------------------------------------------
# Pre-migration backup (moor claw migrate safety net)
# ---------------------------------------------------------------------------

class TestPreMigrationBackup:
    """Tests for create_pre_migration_backup — the auto-backup
    ``moor claw migrate`` runs before mutating ~/.moor/."""
    ...
    def test_restorable_with_moor_import(self, moor_home, tmp_path):
        """The zip produced by pre-migration backup must be a valid Moor
        backup — `moor import` should accept it."""
        from moor_cli.backup import create_pre_migration_backup, _validate_backup_zip
        out = create_pre_migration_backup(moor_home=moor_home)
        assert out is not None
        with zipfile.ZipFile(out) as zf:
    ...
    def test_does_not_touch_pre_update_backups(self, moor_home):
        """Pre-migration rotation must only prune pre-migration-*.zip files,
        leaving pre-update-*.zip backups untouched."""
        from moor_cli.backup import create_pre_update_backup, create_pre_migration_backup
        update_backup = create_pre_update_backup(moor_home=moor_home, keep=5)
        assert update_backup is not None and update_backup.exists()
        # Spin up a lot of migration backups with keep=1
        for _ in range(3):
            out = create_pre_migration_backup(moor_home=moor_home, keep=1)
            assert out is not None
            _advance_backup_clock()
...
class TestRestoreCronJobsIfEmptied:
    """`moor update` config migration can leave cron/jobs.json valid-but-empty,
    ...
    def _make_snapshot(self, moor_home: Path, label="pre-update"):
        from moor_cli.backup import create_quick_snapshot
        return create_quick_snapshot(label=label, moor_home=moor_home, keep=5)

    def test_restores_when_emptied_after_migration(self, tmp_path):
        from moor_cli.backup import restore_cron_jobs_if_emptied
        moor_home = tmp_path / ".moor"
        jobs_path = moor_home / "cron" / "jobs.json"
        # Pre-update: 3 real jobs.
        self._seed_jobs(jobs_path, [{"id": "a"}, {"id": "b"}, {"id": "c"}])
        snap_id = self._make_snapshot(moor_home)
        assert snap_id

        # Migration silently empties the file (valid JSON, zero jobs).
        jobs_path.write_text(json.dumps({"jobs": []}))

        result = restore_cron_jobs_if_emptied(snap_id, moor_home=moor_home)
        assert result is not None
...
class TestRestoreConfigModelSettingsIfRewritten:
    ...
    def _seed(self, moor_home: Path) -> Path:
        moor_home.mkdir(parents=True, exist_ok=True)
        cfg = moor_home / "config.yaml"
...
# location, NOT under MOOR_HOME. (backup/import cycle data-loss fix)
# ---------------------------------------------------------------------------

class TestMemoryProviderExternalPaths:
    def _make_min_tree(self, moor_home: Path) -> None:
        moor_home.mkdir(parents=True, exist_ok=True)
        (moor_home / "config.yaml").write_text("model:\n  provider: openrouter\n")
        (moor_home / ".env").write_text("OPENROUTER_API_KEY=sk-test\n")
        (moor_home / "state.db").write_bytes(b"x")
    ...
    def test_import_restores_external_to_home_relative_location(self, tmp_path, monkeypatch):
        """_external/ members restore to ~/<relpath>, not under MOOR_HOME,
        ...
        # External state did NOT leak into MOOR_HOME.
        assert not (moor_home / "_external").exists()


# ---------------------------------------------------------------------------
# run_import: MOOR_HOME override handling (issue #99839)
# ---------------------------------------------------------------------------


class TestImportHonorsMoorHomeOverride:
    """`moor import` must restore into the home the command runs under.

    Resolving the target through get_default_moor_root() maps a profile
    ...
    def test_import_targets_named_profile_home(self, tmp_path, monkeypatch):
        """MOOR_HOME=<root>/profiles/<name> must restore INTO the profile,
    ...
    def test_import_skips_gateway_install_for_non_default_home(
        ...
        monkeypatch.setenv("MOOR_HOME", str(sandbox))
        ...
        monkeypatch.setattr(
            backup_mod,
            "_get_platform_default_moor_home",
            lambda: native_default,
        )

        calls = []
        monkeypatch.setattr(
            "moor_cli.gateway.ensure_gateway_service",
            lambda *a, **kw: calls.append(kw),
        )
        monkeypatch.setattr(
            "moor_cli.gateway._is_service_running",
            lambda: False,
        )
    ...
    def test_import_installs_gateway_when_default_home_is_target(
        ...
        native_default = tmp_path / "native-default"
        native_default.mkdir()
        monkeypatch.setenv("MOOR_HOME", str(native_default))
...
def _write_session_db(path: Path, sessions: int, messages_per_session: int) -> None:
    """Create a minimal moor-shaped session database at *path*."""
...
class TestImportLiveSessionDatabase:
    """`moor import` must not swap the inode of a database Moor holds open.
