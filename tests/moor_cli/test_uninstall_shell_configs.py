...
def fake_home(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    """Point both ``Path.home()`` and ``MOOR_HOME`` at a throwaway dir."""
    home = tmp_path / "home"
    home.mkdir()
...
class TestHappyPath:
    def test_moor_path_block_is_removed(self, fake_home: Path):
        rc = fake_home / ".zshrc"
...
class TestCrashDurability:
    def test_shell_config_survives_an_interrupted_rewrite(self, fake_home: Path):
        ...
        # Path.home()/MOOR_HOME patches the fake_home fixture installed.
