...
class TestUnifiedDashboardRouting:
    ...
    def test_profile_launch_reexecs_machine_dashboard(self, main_mod, monkeypatch):
        ...
        # MOOR_HOME.  For a standard install (MOOR_HOME unset) that root is
        # the platform-native default (~/.moor), NOT dropped — see the Docker
        # test below for why we resolve explicitly instead of popping.
        from moor_constants import get_default_moor_root
        assert env.get("MOOR_HOME") == str(get_default_moor_root())


    def test_desktop_profile_backend_skips_machine_dashboard_reroute(self, main_mod, monkeypatch):
        """A desktop-spawned named-profile backend (MOOR_DESKTOP=1) must NOT
