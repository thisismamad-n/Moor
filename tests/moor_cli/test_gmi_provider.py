...
class TestGmiAliases:
    ...
    def test_providers_normalize_provider(self):
        from moor_cli.providers import normalize_provider as normalize_provider_in_providers
...
class TestGmiConfigRegistry:
    def test_optional_env_vars_include_gmi(self):
        ...
        # moor model, not via upgrade prompts.
...
class TestGmiProvidersModule:
    def test_overlay_exists(self):
        ...
        assert "gmi" in MOOR_OVERLAYS
        overlay = MOOR_OVERLAYS["gmi"]
...
class TestGmiAuxiliary:
    def test_resolve_provider_client_uses_gmi_aux_default(self, monkeypatch):
        ...
        # GMI profile declares default_headers with a MoorAgent User-Agent
...
class TestGmiMainFlow:
    def test_chat_parser_accepts_gmi_provider(self, monkeypatch):
        ...
        monkeypatch.setattr("moor_cli.config.get_container_exec_info", lambda: None)
        monkeypatch.setattr(
            "moor_cli.main.cmd_chat",
            lambda args: recorded.setdefault("provider", args.provider),
        )
        monkeypatch.setattr(sys, "argv", ["moor", "chat", "--provider", "gmi"])
    ...
    def test_select_provider_and_model_routes_gmi_to_generic_flow(self, monkeypatch):
        ...
        monkeypatch.setattr("moor_cli.auth.resolve_provider", lambda *args, **kwargs: None)
        ...
        monkeypatch.setattr("moor_cli.main._prompt_provider_choice", fake_prompt_provider_choice)
        monkeypatch.setattr("moor_cli.main._model_flow_api_key_provider", fake_model_flow_api_key_provider)
