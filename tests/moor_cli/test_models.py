"""Tests for the moor_cli models module."""
...
from moor_cli.models import (
    ...
    is_moor_free_tier, partition_moor_models_by_tier,
    check_moor_free_tier, _FREE_TIER_CACHE_TTL,
...
class TestModelIds:
    def test_returns_non_empty_list(self):
        with patch("moor_cli.models.fetch_openrouter_models", return_value=LIVE_OPENROUTER_MODELS):
            ids = model_ids()
...
class TestFetchOpenRouterModels:
    ...
    def test_falls_back_to_static_snapshot_on_fetch_failure(self, monkeypatch):
        ...
        with patch("moor_cli.model_catalog.get_curated_openrouter_models", return_value=None), \
             patch("moor_cli.models._urlopen_model_catalog_request", side_effect=OSError("boom")):
    ...
    def test_filters_out_models_without_tool_support(self, monkeypatch):
        ...
        moor-agent is tool-calling-first — surfacing a non-tool model leads to
        ...
        monkeypatch.setattr(_models_mod, "_openrouter_catalog_cache", None)
        with (
            patch("moor_cli.model_catalog.get_curated_openrouter_models", return_value=[]),
            patch("moor_cli.models._urlopen_model_catalog_request", return_value=_Resp()),
        ):
            models = fetch_openrouter_models(force_refresh=True)
...
class TestOpenRouterToolSupportHelper:
    ...
    def test_tools_in_supported_parameters(self):
        from moor_cli.models import _openrouter_model_supports_tools
...
class TestFindOpenrouterSlug:
    def test_exact_match(self):
        from moor_cli.models import _find_openrouter_slug
        with patch("moor_cli.models.fetch_openrouter_models", return_value=LIVE_OPENROUTER_MODELS):
...
class TestPartitionMoorModelsByTier:
    ...
    def test_paid_tier_all_selectable(self):
        ...
        sel, unav = partition_moor_models_by_tier(models, pricing, free_tier=False)
        assert sel == models
        assert unav == []
...
class TestUnionWithPortalFreeRecommendations:
    ...
    def test_adds_portal_free_model_missing_from_curated(self):
        ...
        with patch(
            "moor_cli.models.fetch_moor_recommended_models",
...
class TestCheckMoorFreeTierCache:
    """Tests for the TTL cache on check_moor_free_tier()."""
    ...
    @patch("moor_cli.moor_account.get_moor_portal_account_info")
    def test_result_is_cached(self, mock_account):
        """Second call within TTL returns cached result without account lookup."""
        mock_account.return_value = MoorPortalAccountInfo(
            logged_in=True,
            source="jwt",
            fresh=False,
            paid_service_access=False,
        )
        result1 = check_moor_free_tier()
        result2 = check_moor_free_tier()
    ...
    def test_force_fresh_bypasses_cache(self, mock_account):
        ...
        assert check_moor_free_tier() is False
        assert check_moor_free_tier(force_fresh=True) is False
...
class TestMoorRecommendedModels:
    ...
    def setup_method(self):
        _models_mod._moor_recommended_cache.clear()
    ...
    def test_fetch_caches_per_portal_url(self):
        from moor_cli.models import fetch_moor_recommended_models
        mock_cm = self._mock_urlopen(self._SAMPLE_PAYLOAD)
        with patch("moor_cli.models._urlopen_model_catalog_request", return_value=mock_cm) as mock_urlopen:
            a = fetch_moor_recommended_models("https://portal.example.com")
            b = fetch_moor_recommended_models("https://portal.example.com")
    ...
    def test_paid_tier_prefers_paid_recommendation(self):
        ...
        with patch("moor_cli.models.fetch_moor_recommended_models", return_value=payload):
            text = get_moor_recommended_aux_model(vision=False, free_tier=False)
            vision = get_moor_recommended_aux_model(vision=True, free_tier=False)
    ...
    def test_tier_detection_error_defaults_to_paid(self):
        ...
        with (
            patch("moor_cli.models.fetch_moor_recommended_models", return_value=payload),
            patch("moor_cli.models.check_moor_free_tier", side_effect=RuntimeError("boom")),
        ):
            assert get_moor_recommended_aux_model(vision=False) == "paid-model"
...
class TestFormatPricePerMtok:
    ...
    def test_standard_prices_keep_two_decimals(self):
        from moor_cli.models import _format_price_per_mtok
    ...
    def test_moor_list_includes_sonnet_5(self):
        from moor_cli.models import _PROVIDER_MODELS
        assert "anthropic/claude-sonnet-5" in _PROVIDER_MODELS["moor"]
...
class TestLocalOllamaModelDiscovery:
    ...
    def test_native_tags_cache_expires(self, monkeypatch):
        from moor_cli.models import fetch_ollama_local_models
        ...
        try:
            ...
            monkeypatch.setattr("moor_cli.models.time.monotonic", lambda: 301.0)
    ...
    def test_runtime_error_from_config_load_does_not_escape_ollama_helpers(self):
        ...
        from moor_cli.models import _get_ollama_base_url, should_use_ollama_native_catalog

        with patch("moor_cli.config.load_config", side_effect=RuntimeError("bad home")), patch(
            "moor_cli.models.probe_ollama_local_models",
            return_value=None,
        ):
    ...
    def test_probe_ollama_models_malformed_base_url_returns_none(self):
        ...
        from moor_cli.models import probe_ollama_local_models
    ...
    def test_fetch_ollama_models_preserves_probe_failure(self):
        ...
        with patch("moor_cli.models.probe_ollama_local_models", return_value=None):
    ...
    def test_ollama_port_detection_requires_working_api_tags(self):
        ...
        with patch("moor_cli.models.probe_ollama_local_models", return_value=["qwen3:1.7b"]):
    ...
    def test_provider_model_ids_ollama_cloud_config_uses_generic_catalog(self):
        ...
        ), patch("moor_cli.models.fetch_ollama_local_models") as fetch_local, patch(
            "moor_cli.models.fetch_api_models",
    ...
    def test_native_ollama_catalog_uses_configured_key_env(self, monkeypatch):
        from moor_cli.models import _get_ollama_request_headers
    ...
    def test_provider_model_ids_ignores_active_non_ollama_custom_endpoint(self):
        ...
        with patch(
            ...
            "moor_cli.models.fetch_ollama_local_models",
            return_value=["qwen3:1.7b"],
        ) as fetch_local:
            assert provider_model_ids("ollama", force_refresh=True) == ["qwen3:1.7b"]
    ...
    def test_ollama_cache_fingerprint_does_not_probe_custom_endpoint(self):
        from moor_cli.models import _credential_fingerprint
        ...
        ), patch("moor_cli.models.probe_ollama_local_models") as probe_ollama:
    ...
    def test_picker_user_provider_verifies_ambiguous_ollama_port(self):
        ...
        with patch("moor_cli.config.load_config", return_value={"providers": {}}), patch(
            "moor_cli.models.should_use_ollama_native_catalog",
            return_value=True,
        ), patch(
            "moor_cli.models.fetch_ollama_local_models",
            return_value=["qwen3:1.7b"],
        ), patch("moor_cli.models.fetch_api_models", return_value=[]) as fetch_api:
            rows = list_authenticated_providers(
    ...
    def test_named_custom_model_flow_discovers_ollama_api_tags(self):
        ...
        from moor_cli.main import _model_flow_named_custom
        ...
        try:
            with patch("moor_cli.config.load_config", return_value=config), patch(
                "moor_cli.config.save_config"
            ), patch("moor_cli.auth._save_model_choice"), patch(
                "moor_cli.auth.deactivate_provider"
            ), patch("moor_cli.main._save_custom_provider"), patch(
                "moor_cli.curses_ui.curses_radiolist",
    ...
    def test_model_validation_ollama_cloud_config_does_not_use_local_tags(self):
        ...
        ), patch("moor_cli.models.probe_ollama_local_models") as probe_ollama, patch(
    ...
    def test_switch_model_on_current_ollama_custom_endpoint_keeps_base_url(self):
        ...
        try:
            with patch(
                ...
                "moor_cli.model_switch.get_model_info",
                return_value=None,
            ):
                result = switch_model(
    ...
    def test_ollama_root_matching_is_case_insensitive_for_hostnames(self):
        from moor_cli.models import _same_ollama_native_root
    ...
    def test_ollama_host_environment_forms_are_normalized(self, monkeypatch):
        from moor_cli.models import _get_ollama_base_url, _root_for_ollama_native_api
    ...
    def test_ollama_failed_probe_is_cached_briefly(self):
        ...
        with patch(
            "moor_cli.models._urlopen_model_catalog_request",
    ...
    def test_switch_model_direct_ollama_alias_preserves_matching_origin_api_key(self):
        import moor_cli.model_switch as model_switch
        ...
        try:
            with patch.object(model_switch, "get_model_info", return_value=None), patch(
                "moor_cli.models._get_provider_config_dict",
