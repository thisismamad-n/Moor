...
def _run(monkeypatch, capsys, argv_tail, db):
    ...
    monkeypatch.setattr(moor_state, "SessionDB", lambda *args, **kwargs: db)
    monkeypatch.setattr(sys, "argv", ["moor", "sessions", *argv_tail])
    try:
        main_mod.main()
        code = 0
...
def test_pin_multiple_ids_one_missing(monkeypatch, capsys):
    ...
    assert "No session 'nope'" in out and "moor sessions list" in out
    assert code == 1
...
def test_pinned_empty_hint(monkeypatch, capsys):
    ...
    assert "moor sessions pin" in out
