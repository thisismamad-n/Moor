...
venv-holder guard, and never relaunched after `moor update` killed it. The
...
def _ledger_entry(**over):
    entry = {
        ...
        "spawner_create": None,
        "registered_at": 222.0,
        "argv": "moor serve --host 100.94.65.93 --port 9119",
        "host": "100.94.65.93",
        "port": 9119,
        "profile": "",
    }
...
def test_serve_relaunch_commands_built_from_structured_identity(monkeypatch):
    ...
    assert ["moor", "serve", "--host", "100.94.65.93", "--port", "9119"] in cmds
    assert ["moor", "--profile", "work", "serve", "--port", "9200"] in cmds
    assert ["moor", "dashboard", "--host", "0.0.0.0", "--port", "9300"] in cmds
...
def test_scan_dashboard_processes_includes_ledger_only_serves(monkeypatch):
    """A profiled serve (`moor --profile p serve ...`) matches no scan
    pattern; the ledger row must still surface it."""
    import moor_cli.dashboard_procs as dp

    profiled = _ledger_entry(
        pid=8123,
        argv="moor --profile work serve --host 100.94.65.93 --port 9119",
        profile="work",
    )
