"""Tests for moor_cli.copilot_auth — Copilot token validation and resolution."""
...
class TestResolveToken:
    ...
    def test_gh_token_second_priority(self, monkeypatch):
        from moor_cli.copilot_auth import resolve_copilot_token
    ...
    def test_invalid_env_var_skips_gh_cli_fallback(self, monkeypatch):
        ...
        with patch("moor_cli.copilot_auth._try_gh_cli_token") as mock_cli:
            token, source = resolve_copilot_token()
        assert token == ""
        assert source == ""
...
class TestGhCliTokenCache:
    ...
    def test_miss_is_cached_and_probe_runs_once(self):
        from moor_cli import copilot_auth
        self._reset()
...
class TestRequestHeaders:
    ...
    def test_default_headers_include_openai_intent(self):
        ...
        assert headers["User-Agent"] == "MoorAgent/1.0"
