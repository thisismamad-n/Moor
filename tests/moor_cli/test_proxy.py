"""Tests for the `moor proxy` subcommand and its upstream adapters."""
...
from moor_cli.proxy.adapters import ADAPTERS, get_adapter
...
def _write_auth_store(moor_home: Path, moor_state: Dict[str, Any]) -> Path:
    """Write an auth.json with the given moor state into a hermetic MOOR_HOME."""
    auth_path = moor_home / "auth.json"
    auth_path.write_text(json.dumps({
        "version": 1,
        "providers": {"moor": moor_state},
    }))
    return auth_path
...
def test_moor_adapter_concurrent_refresh_serialized(tmp_path, monkeypatch):
    ...
    with patch(
        "moor_cli.proxy.adapters.moor_portal.resolve_moor_runtime_credentials",
...
def _write_xai_pool_entry(
    ...
    *,
    ...
    """Write an xai-oauth pool entry into a hermetic MOOR_HOME."""
    auth_path = moor_home / "auth.json"
    auth_path.write_text(json.dumps({
        "version": 1,
        "providers": {},
...
def test_xai_adapter_retry_rotates_pool_entry_on_429(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("moor_cli.auth.refresh_xai_oauth_pure", _refresh_must_not_run)
...
from moor_cli.proxy.server import create_app  # noqa: E402
