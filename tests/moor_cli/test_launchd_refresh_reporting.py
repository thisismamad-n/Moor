...
def _stale_plist(tmp_path, monkeypatch, *, registered: bool):
    plist_path = tmp_path / "com.moor.plist"
    ...
    monkeypatch.setattr(gw, "get_launchd_label", lambda: "com.moor.agent")
...
def test_install_repair_warns_instead_of_claiming_success(tmp_path, monkeypatch, capsys):
    ...
    monkeypatch.setattr("moor_constants.display_moor_home", lambda: "~/.moor-work")
    ...
    assert "~/.moor-work/logs/launchd-reload.log" in out
