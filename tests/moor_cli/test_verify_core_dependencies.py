...
class TestVerifyCoreDependencies:
    ...
    def test_skips_deps_excluded_by_environment_markers(self, temp_pyproject, fake_venv_python):
        ...
        with patch("moor_cli.main._resolve_install_target_python", return_value=py), \
             patch("moor_cli.main.subprocess.run", side_effect=fake_subprocess_run), \
             patch("moor_cli.main._run_install_with_heartbeat"):

            from moor_cli.main import _verify_core_dependencies_installed
    ...
    def test_no_pyproject_is_noop(self, tmp_path, monkeypatch):
        ...
        with patch("moor_cli.main._resolve_install_target_python") as mock_resolve, \
             patch("moor_cli.main._run_install_with_heartbeat") as mock_install:
            from moor_cli.main import _verify_core_dependencies_installed
...
class TestResolveInstallTargetPython:
    def test_uses_virtual_env_from_environment(self, tmp_path):
        ...
        with patch("moor_cli.main._is_windows", return_value=True):
            from moor_cli.main import _resolve_install_target_python
