"""Tests for cli.MoorCLI._confirm_destructive_slash.

Drives the helper directly via __get__ on a SimpleNamespace stand-in so we
don't have to construct a full MoorCLI (which requires extensive setup).
"""
...
def _make_self(prompt_response):
    ...
    self_._normalize_slash_confirm_choice = _bound(
        MoorCLI._normalize_slash_confirm_choice, self_,
    )
    return self_
...
def test_gate_on_choice_once_returns_once():
    ...
    with patch(
        ...
        result = _bound(MoorCLI._confirm_destructive_slash, self_)(
            "clear", "detail",
        )
...
def test_split_destructive_skip_recognized_tokens():
    ...
    assert MoorCLI._split_destructive_skip("/reset now") == ("", True)
    assert MoorCLI._split_destructive_skip("/clear --yes") == ("", True)
    assert MoorCLI._split_destructive_skip("/undo -y") == ("", True)
...
def test_split_destructive_skip_handles_empty_and_none():
    ...
    assert MoorCLI._split_destructive_skip(None) == ("", False)
    assert MoorCLI._split_destructive_skip("") == ("", False)
    assert MoorCLI._split_destructive_skip("   ") == ("", False)
...
def test_confirm_destructive_slash_now_skips_modal():
    ...
    self_._split_destructive_skip = MoorCLI._split_destructive_skip  # classmethod
...
def test_confirm_destructive_slash_no_skip_token_still_prompts():
    ...
    self_._split_destructive_skip = MoorCLI._split_destructive_skip
