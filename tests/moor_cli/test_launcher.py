"""Tests for the top-level `./moor` launcher script."""
...
def test_launcher_delegates_to_argparse_entrypoint(monkeypatch):
    """`./moor` should use `moor_cli.main`, not the legacy Fire wrapper."""
    launcher_path = Path(__file__).resolve().parents[2] / "moor"
    called = []

    fake_main_module = types.ModuleType("moor_cli.main")

    def fake_main():
        called.append("moor_cli.main")

    fake_main_module.main = fake_main
    monkeypatch.setitem(sys.modules, "moor_cli.main", fake_main_module)
