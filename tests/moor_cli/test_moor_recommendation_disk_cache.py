...
def fetch_in_new_process(home, base, *, force=False):
    result = subprocess.run(
        [sys.executable, "-c", "from moor_cli.models import fetch_moor_recommended_models; "
         "import json, sys; print(json.dumps(fetch_moor_recommended_models(sys.argv[1], force_refresh=sys.argv[2] == 'True')))",
         base, str(force)],
...
def test_process_restarts_reuse_fresh_disk_but_force_and_expiry_fetch(tmp_path, portal):
    ...
    path = home / "cache" / "moor_recommended_cache.json"
