"""Tests for ``moor gui`` desktop launcher wiring."""
...
def _isolate_xdg_data_home(tmp_path, monkeypatch):
    ...
    call live, and on a Linux dev box it wrote a ``moor.desktop`` pointing
...
def _stable_keychain_detection(monkeypatch):
    ...
    monkeypatch.delenv("MOOR_DESKTOP_PASSWORD_STORE", raising=False)
...
def _make_packaged_executable(root: Path, monkeypatch) -> Path:
    ...
    if sys.platform == "darwin":
        ...
        exe = desktop_dir / "release" / "linux-unpacked" / "moor"
...
def test_gui_installs_packages_and_launches_desktop_app(tmp_path, monkeypatch):
    ...
    with patch("moor_cli.main.shutil.which", return_value="/usr/bin/npm"), \
         patch("moor_cli.main_web_build._run_npm_install_deterministic", return_value=install_ok) as mock_install, \
         patch("moor_cli.main_desktop._desktop_build_needed", return_value=True), \
         patch("moor_cli.main_desktop._write_desktop_build_stamp"), \
...
def test_gui_install_env_prepends_managed_node_on_bare_path(tmp_path, monkeypatch):
    ...
    from moor_constants import iter_moor_node_dirs

    root = _make_desktop_tree(tmp_path)
    monkeypatch.setattr(cli_main, "PROJECT_ROOT", root)
    _make_packaged_executable(root, monkeypatch)

    # A managed Node tree on disk so with_moor_node_path() actually prepends it.
    ...
    install_ok = subprocess.CompletedProcess(["npm", "ci"], 0)
    launch_ok = subprocess.CompletedProcess(["moor"], 0)
    ...
    with patch("moor_cli.main_install_repair._resolve_node_runtime_npm", return_value="/usr/bin/npm"), \
         patch("moor_cli.main_web_build._run_npm_install_deterministic", return_value=install_ok) as mock_install, \
         patch("moor_cli.main_desktop._desktop_build_needed", return_value=True), \
         patch("moor_cli.main_desktop._write_desktop_build_stamp"), \
         patch("moor_cli.main_desktop._desktop_macos_relaunchable_fixup"), \
    ...
    managed_dirs = [str(p) for p in iter_moor_node_dirs() if p.is_dir()]
...
def test_gui_does_not_retry_after_packaged_executable_exists(tmp_path, monkeypatch, capsys):
    ...
    with patch("moor_cli.main.shutil.which", return_value="/usr/bin/npm"), \
         patch("moor_cli.main_web_build._run_npm_install_deterministic", return_value=install_ok), \
         patch("moor_cli.main_desktop._desktop_macos_relaunchable_fixup"), \
         patch("moor_cli.main_desktop._purge_electron_build_cache", return_value=[Path("/c/electron.zip")]) as mock_purge, \
         patch("moor_cli.main_desktop._redownload_electron_dist", return_value=True) as mock_dl, \
         patch("moor_cli.main.subprocess.run", side_effect=pack_fail) as mock_run, \
         pytest.raises(SystemExit) as exc:
        cli_main.cmd_gui(_ns())
...
def test_setup_tcc_identity_creates_cert_imports_trusts_and_configures(tmp_path, monkeypatch, capsys):
    ...
    monkeypatch.setattr("moor_cli.config.set_config_value", lambda key, value: None)
    ...
    assert not list(tmp_path.glob("moor-tcc-*"))
...
def test_setup_tcc_identity_skips_generation_when_already_valid(tmp_path, monkeypatch, capsys):
    ...
    def fake_run(cmd, **kwargs):
        calls.append(list(cmd))
        if cmd[:4] == ["/usr/bin/security", "find-identity", "-v", "-p"]:
            return _fake_proc(cmd, stdout='  1) ABCD "Moor Local Signing"\n     1 valid identities found')
        return _fake_proc(cmd)
    ...
    assert main_desktop._desktop_macos_setup_tcc_identity("Moor Local Signing") is True
...
def test_cmd_gui_setup_tcc_identity_exits_before_build(tmp_path, monkeypatch):
    """`moor desktop --setup-tcc-identity` calls the setup and exits 0/1
    ...
    with patch("moor_cli.main_desktop._desktop_macos_setup_tcc_identity", return_value=True) as mock_setup, \
         patch("moor_cli.main_web_build._run_npm_install_deterministic") as mock_install, \
         pytest.raises(SystemExit) as exc:
        cli_main.cmd_gui(_ns(setup_tcc_identity=True, identity="Moor Local Signing"))

    assert exc.value.code == 0
    mock_setup.assert_called_once_with("Moor Local Signing")
...
def test_gui_registers_linux_desktop_entry_before_launch(tmp_path, monkeypatch):
    """`moor desktop` gives the app a launcher presence on Linux."""
    root = _make_desktop_tree(tmp_path)
    monkeypatch.setattr(cli_main, "PROJECT_ROOT", root)
    packaged_exe = _make_packaged_executable(root, monkeypatch)
    ...
    monkeypatch.setattr(
        ...
        lambda project_root: registered.append(project_root) or (tmp_path / "moor.desktop"),
    )
...
def test_gui_launches_even_when_desktop_entry_install_fails(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("moor_cli.linux_desktop_entry.is_supported", lambda: True)
    monkeypatch.setattr("moor_cli.linux_desktop_entry.install_desktop_entry", boom)
...
def test_gui_skips_desktop_entry_off_linux(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("moor_cli.linux_desktop_entry.is_supported", lambda: False)

    def fail(_project_root):
        raise AssertionError("must not install a desktop entry off Linux")

    monkeypatch.setattr("moor_cli.linux_desktop_entry.install_desktop_entry", fail)
...
def test_gui_bridges_ozone_hint_to_launch_env(tmp_path, monkeypatch):
    ...
    with patch("moor_cli.main.shutil.which", return_value="/usr/bin/npm"), \
         patch("moor_cli.main_web_build._run_npm_install_deterministic", return_value=ok), \
         patch("moor_cli.main_desktop._desktop_build_needed", return_value=True), \
         patch("moor_cli.main_desktop._write_desktop_build_stamp"), \
         patch("moor_cli.main_desktop._desktop_macos_relaunchable_fixup"), \
         patch("moor_cli.main_desktop._desktop_linux_sandbox_fixup", return_value=True), \
    ...
    with patch("moor_cli.main.shutil.which", return_value="/usr/bin/npm"), \
         ...
         patch("moor_cli.config.load_config", return_value=cfg), \
         patch("moor_cli.linux_desktop_entry.install_desktop_entry", return_value=None), \
         patch("moor_cli.main.subprocess.run", side_effect=_pack_into_staging(root)) as mock_run2, \
         pytest.raises(SystemExit):
        cli_main.cmd_gui(_ns())
...
def test_detect_linux_password_store_via_dbus_secret_service(monkeypatch):
    ...
    with patch("moor_cli.main.subprocess.run", return_value=ping_ok) as mock_run:
        assert main_desktop._detect_linux_password_store() == "gnome-libsecret"
...
def test_detect_linux_password_store_none_when_no_keychain(monkeypatch):
    ...
    with patch("moor_cli.main.subprocess.run", return_value=ping_fail):
        assert main_desktop._detect_linux_password_store() is None
    with patch("moor_cli.main.subprocess.run", side_effect=FileNotFoundError):
        assert main_desktop._detect_linux_password_store() is None
...
def test_gui_explicit_password_store_env_wins_over_config_and_detection(tmp_path, monkeypatch):
    _clear_keychain_env(monkeypatch)
    monkeypatch.setenv("MOOR_DESKTOP_PASSWORD_STORE", "basic")
    root = _make_desktop_tree(tmp_path)
    monkeypatch.setattr(cli_main, "PROJECT_ROOT", root)
    _make_packaged_executable(root, monkeypatch)
    ...
    mock_detect.assert_not_called()
    launch_env = mock_run.call_args_list[1].kwargs["env"]
    assert launch_env["MOOR_DESKTOP_PASSWORD_STORE"] == "basic"
...
def _gui_build_patches(root: Path, run_side_effect):
    return [
        ...
        patch("moor_cli.main_web_build._run_npm_install_deterministic",
        ...
        patch("moor_cli.main_desktop._purge_electron_build_cache", return_value=[]),
        patch("moor_cli.main_desktop._redownload_electron_dist", return_value=False),
