...
Canonical owners:
  ...
  * ``moor_cli/config.py`` — ``load_config()`` / ``load_config_readonly()``
...
ALLOWLIST = {
    ...
    # gateway.run._moor_home (delegates to read_raw_config otherwise).
    "gateway/run.py",
    # Reads the MANAGED-scope config.yaml (/etc/moor/...), not the user's —
    # it IS the overlay source; the canonical loaders call into it.
    "moor_cli/managed_scope.py",
    ...
    # moor_cli.config is importable; it reads one display key best-effort and
