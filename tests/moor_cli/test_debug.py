"""Tests for ``moor debug`` CLI command and debug utilities."""
...
def moor_home(tmp_path, monkeypatch):
    """Set up an isolated MOOR_HOME with minimal logs."""
    home = tmp_path / ".moor"
    home.mkdir()
    ...
    (logs_dir / "gui.log").write_text(
        "2026-04-12 17:00:12 INFO moor_cli.web_server: dashboard request\n"
    )
...
class TestUploadPasteRs:
    ...
    def test_upload_paste_rs_success(self):
        from moor_cli.debug import _upload_paste_rs
        ...
        with patch("moor_cli.debug.urllib.request.urlopen", return_value=mock_resp):
    ...
    def test_upload_paste_rs_network_error(self):
        ...
        with patch(
            "moor_cli.debug.urllib.request.urlopen",
...
class TestUploadToPastebin:
    ...
    def test_falls_back_to_dpaste_com(self):
        from moor_cli.debug import upload_to_pastebin

        with patch("moor_cli.debug._upload_paste_rs",
                    side_effect=Exception("down")), \
             patch("moor_cli.debug._upload_dpaste_com",
...
class TestCaptureLogSnapshot:
    ...
    def test_race_truncate_after_resolve_reports_empty(self, moor_home, monkeypatch):
        """If the log is truncated between resolve and stat, say 'empty', not 'missing'."""
        log_path = moor_home / "logs" / "agent.log"
        from moor_cli import debug
    ...
    def test_keeps_first_line_when_truncation_on_boundary(self, moor_home):
        """When truncation lands on a line boundary, keep the first full line."""
        from moor_cli.debug import _capture_log_snapshot
        ...
        (moor_home / "logs" / "agent.log").write_text(line * num_lines)
...
class TestMissingLogNote:
    ...
    `moor debug share` runs on the backend, so a desktop connected to a
    ...
    def test_backend_written_log_reports_plain_absence(self, moor_home):
        from moor_cli.debug import _capture_log_snapshot

        (moor_home / "logs" / "agent.log").unlink()

        snap = _capture_log_snapshot("agent", tail_lines=10)
        assert snap.full_text is None
        assert snap.tail_text == "(file not found)"

    def test_client_written_log_names_its_writer_and_path(self, moor_home):
        from moor_cli.debug import _capture_log_snapshot

        (moor_home / "logs" / "desktop.log").unlink()

        snap = _capture_log_snapshot("desktop", tail_lines=10)
        assert snap.full_text is None
        assert "not on this host" in snap.tail_text
        assert "Moor Desktop" in snap.tail_text
        # The reader needs the path to collect by hand on the client machine.
        assert str(moor_home / "logs" / "desktop.log") in snap.tail_text

    def test_present_client_log_is_captured_normally(self, moor_home):
    ...
    def test_empty_client_log_is_empty_not_absent(self, moor_home):
        """An empty file means the app ran and logged nothing — a different fact."""
        from moor_cli.debug import _capture_log_snapshot

        (moor_home / "logs" / "desktop.log").write_text("")

        snap = _capture_log_snapshot("desktop", tail_lines=10)
        assert snap.tail_text == "(file empty)"

    def test_report_carries_the_note_for_a_remote_backend(self, moor_home):
        """The uploaded report — what people paste into support — must explain it."""
        from moor_cli.debug import collect_debug_report
...
# ---------------------------------------------------------------------------
# Capture log redaction (force=True applies regardless of MOOR_REDACT_SECRETS)
# ---------------------------------------------------------------------------
...
class TestCaptureLogSnapshotRedaction:
    ...
    @pytest.fixture
    def moor_home_with_secret(self, tmp_path, monkeypatch):
        """Isolated MOOR_HOME whose agent.log contains a vendor-prefixed token."""
        home = tmp_path / ".moor"
        home.mkdir()
        monkeypatch.setenv("MOOR_HOME", str(home))
    ...
    def test_default_redacts_tail_and_full_text(self, moor_home_with_secret):
        from moor_cli.debug import _capture_log_snapshot
    ...
    def test_redact_false_passes_through(self, moor_home_with_secret):
        from moor_cli.debug import _capture_log_snapshot
    ...
    def test_force_true_works_when_redaction_disabled(
        self, moor_home_with_secret, monkeypatch
    ):
        ...
        input unchanged when MOOR_REDACT_SECRETS=false, and the share-time
        ...
        monkeypatch.setenv("MOOR_REDACT_SECRETS", "false")

        from moor_cli.debug import _capture_log_snapshot

        assert os.environ.get("MOOR_REDACT_SECRETS", "") == "false"
    ...
    def test_default_redacts_email_addresses_for_public_share(
        self, moor_home_with_secret
    ):
        from moor_cli.debug import _capture_log_snapshot

        log_path = moor_home_with_secret / "logs" / "agent.log"
        log_path.write_text(
    ...
    def test_no_redact_preserves_email_addresses(self, moor_home_with_secret):
        from moor_cli.debug import _capture_log_snapshot
    ...
    def test_capture_default_log_snapshots_threads_redact(
        ...
        from moor_cli.debug import _capture_default_log_snapshots
...
class TestCollectDebugReport:
    ...
    def test_report_includes_dump_output(self, moor_home):
        from moor_cli.debug import collect_debug_report

        with patch("moor_cli.dump.run_dump") as mock_dump:
            mock_dump.side_effect = lambda args: print(
                "--- moor dump ---\nversion: 0.8.0\n--- end dump ---"
            )
            report = collect_debug_report(log_lines=50)

        assert "--- moor dump ---" in report
...
class TestRunDebugShare:
    ...
    def test_share_sweeps_expired_pastes(self, moor_home, capsys):
        ...
        with patch("moor_cli.dump.run_dump"), \
             patch("moor_cli.debug._sweep_expired_pastes", return_value=(0, 0)) as mock_sweep, \
    ...
    def test_share_uploads_five_pastes(self, moor_home, capsys):
        ...
        with patch("moor_cli.dump.run_dump") as mock_dump, \
            ...
            mock_dump.side_effect = lambda a: print("--- moor dump ---\nversion: test\n--- end dump ---")
            run_debug_share(args)
...
class TestRunDebugShareRedaction:
    ...
    def test_default_share_redacts_uploaded_content(
        self, moor_home_with_secret, capsys
    ):
        ...
        with patch("moor_cli.dump.run_dump"), \
             patch("moor_cli.debug._sweep_expired_pastes", return_value=(0, 0)), \
             patch("moor_cli.debug.upload_to_pastebin", side_effect=fake_upload):
            run_debug_share(args)
...
class TestRunDebug:
    def test_no_subcommand_shows_usage(self, capsys):
        ...
        out = capsys.readouterr().out
        assert "moor debug" in out
        assert "share" in out
        assert "delete" in out

    def test_share_subcommand_routes(self, moor_home):
        from moor_cli.debug import run_debug
...
class TestExtractPasteId:
    def test_paste_rs_url(self):
        from moor_cli.debug import _extract_paste_id
...
class TestDeletePaste:
    def test_delete_sends_delete_request(self):
        ...
        with patch("moor_cli.debug.urllib.request.urlopen",
...
class TestScheduleAutoDelete:
    ...
    ``~/.moor/pastes/pending.json`` and lets ``_sweep_expired_pastes``
    handle the DELETE requests synchronously on the next ``moor debug``
    invocation.
    """


    def test_records_pending_to_json(self, moor_home):
        """Scheduled URLs are persisted to pending.json with expiration."""
        from moor_cli.debug import _schedule_auto_delete, _pending_file
        import json
    ...
    def test_dedupes_same_url(self, moor_home):
        """Same URL recorded twice → one entry with the later expire_at."""
        from moor_cli.debug import _schedule_auto_delete, _load_pending
...
class TestSweepExpiredPastes:
    ...
    def test_sweep_deletes_expired_entries(self, moor_home):
        ...
        with patch("moor_cli.debug.delete_paste", side_effect=fake_delete):
    ...
    def test_sweep_leaves_future_entries_alone(self, moor_home):
        from moor_cli.debug import _sweep_expired_pastes, _save_pending
        import time
        ...
        with patch("moor_cli.debug.delete_paste") as mock_delete:
            deleted, remaining = _sweep_expired_pastes()
    ...
    def test_sweep_survives_network_failure(self, moor_home):
...
class TestRunDebugSweepsOnInvocation:
    ...
    def test_run_debug_calls_sweep(self, moor_home):
        from moor_cli.debug import run_debug

        args = MagicMock()
        args.debug_command = None  # default → prints help

        with patch("moor_cli.debug._sweep_expired_pastes") as mock_sweep:
            run_debug(args)
...
class TestRunDebugDelete:
    ...
    def test_handles_delete_failure(self, capsys):
        from moor_cli.debug import run_debug_delete

        args = MagicMock()
        args.urls = ["https://paste.rs/abc"]

        with patch("moor_cli.debug.delete_paste",
...
class TestShareIncludesAutoDelete:
    ...
    def test_share_shows_privacy_notice(self, moor_home, capsys):
        from moor_cli.debug import run_debug_share
...
class TestBuildDebugShare:
    ...
    Backs both ``moor debug share`` (CLI) and ``POST /api/ops/debug-share``
    ...
    def test_redaction_keeps_secrets_out_of_payload(self, moor_home):
        from moor_cli.debug import build_debug_share

        secret = "sk-proj-SUPERSECRETtoken1234567890"
        (moor_home / "logs" / "agent.log").write_text(
        ...
        with patch("moor_cli.dump.run_dump"), patch(
            "moor_cli.debug.upload_to_pastebin", side_effect=_upload
        ), patch("moor_cli.debug._schedule_auto_delete"):
    ...
    def test_optional_log_failure_is_collected_not_raised(self, moor_home):
        from moor_cli.debug import build_debug_share
...
# ---------------------------------------------------------------------------
# Shared bundle collection + Moor-S3 path
# ---------------------------------------------------------------------------

class TestCollectShareBundle:

    def test_no_redact_omits_banner(self, moor_home):
        from moor_cli.debug import collect_share_bundle
    ...
    def test_redaction_keeps_secrets_out(self, moor_home):
        from moor_cli.debug import collect_share_bundle
...
class TestBuildMoorBundle:
    def test_envelope_shape_and_gzip(self, moor_home):
        import gzip
        import json as _json

        from moor_cli.debug import build_moor_bundle

        files = {"report": "hello", "agent.log": "log line"}
        blob = build_moor_bundle(files, redact=True)

        # It's gzip — magic bytes.
        assert blob[:2] == b"\x1f\x8b"
        envelope = _json.loads(gzip.decompress(blob).decode())
        assert envelope["format"] == "moor-debug-share/1"
    ...
    def test_redacted_false_recorded(self):
        ...
        blob = build_moor_bundle({"report": "x"}, redact=False)
        envelope = _json.loads(gzip.decompress(blob).decode())
        assert envelope["redacted"] is False


class TestRunDebugShareMoor:
    def _args(self, **over):
        class _A:
            lines = 50
            expire = 7
    ...
    def test_moor_success_prints_view_url(self, moor_home, capsys):
        from moor_cli.debug import run_debug_share
        ...
        with patch("moor_cli.dump.run_dump"), patch(
            "moor_cli.diagnostics_upload.share_to_moor", return_value=res
        ) as share:
            run_debug_share(self._args())

        out = capsys.readouterr().out
        assert "Moor-INTERNAL" in out
        assert "https://support.example.com/diagnostics/id-1" in out
        assert "2026-06-20T00:00:00Z" in out
        # The blob passed to share_to_moor must be gzip bytes.
        blob = share.call_args[0][0]
        assert isinstance(blob, (bytes, bytearray)) and blob[:2] == b"\x1f\x8b"

    def test_moor_failure_suggests_local(self, moor_home, capsys):
        from moor_cli.debug import run_debug_share

        with patch("moor_cli.dump.run_dump"), patch(
            "moor_cli.diagnostics_upload.share_to_moor",
        ...
        assert "Moor upload failed" in err
        assert "--local" in err

    def test_moor_does_not_touch_pastebin(self, moor_home):
        from moor_cli.debug import run_debug_share

        res = {"id": "id-1", "viewUrl": "https://v"}
        with patch("moor_cli.dump.run_dump"), patch(
            "moor_cli.diagnostics_upload.share_to_moor", return_value=res
        ), patch("moor_cli.debug.upload_to_pastebin") as paste:
            run_debug_share(self._args())
        paste.assert_not_called()


class TestDebugSlashCommand:
    """`/debug [moor|local]` parsing in the CLI/TUI handler.

    The classic CLI and the TUI slash worker both dispatch through
    ``MoorCLI.process_command`` → ``_handle_debug_command(cmd_original)``,
    ...
    def _captured(self, cmd_original):
        ...
        with patch("moor_cli.debug.run_debug_share", _fake_run):
    ...
    def test_word_parsing_is_case_insensitive(self):
        c = self._captured("/debug MOOR")
        assert c["moor"] is True
...
class TestShareConsentGate:
    """`moor debug share` requires explicit consent before uploading.
    ...
    def _args(self, **over):
        ...
        base = dict(lines=50, expire=7, local=False, moor=False,
    ...
    def test_non_interactive_requires_yes(self, moor_home, capsys, monkeypatch):
        ...
        with patch("moor_cli.dump.run_dump"), \
             patch("moor_cli.debug.upload_to_pastebin") as mock_upload:
            with pytest.raises(SystemExit) as exc:
                run_debug_share(self._args())
    ...
    def test_local_never_prompts(self, moor_home, capsys, monkeypatch):
