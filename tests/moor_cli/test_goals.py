"""Tests for moor_cli/goals.py — persistent cross-turn goals."""
...
def moor_home(tmp_path, monkeypatch):
    ...
    # Bust the goal-module's DB cache for each test so it re-resolves MOOR_HOME.
...
class TestParseJudgeResponse:
    def test_clean_json_done(self):
        from moor_cli.goals import _parse_judge_response
...
class TestGoalManager:
    ...
    def test_set_then_status(self, moor_home):
    ...
    def test_continuation_prompt_shape(self, moor_home):
        ...
        mgr.set("port goal command to moor")
        prompt = mgr.next_continuation_prompt()
        assert prompt is not None
        assert "port goal command to moor" in prompt
...
class TestGoalStateSubgoalsBackcompat:
    def test_old_state_meta_row_loads_without_subgoals(self):
        ...
        from moor_cli.goals import GoalState
...
class TestMigrateGoalToSession:
    ...
    def test_migrates_active_goal_to_child(self, moor_home):
        from moor_cli.goals import save_goal, load_goal, migrate_goal_to_session, GoalState
    ...
    def test_does_not_clobber_existing_child_goal(self, moor_home):
        from moor_cli.goals import save_goal, load_goal, migrate_goal_to_session, GoalState
...
class TestGoalManagerSubgoals:
    def test_add_subgoal(self, moor_home):
        from moor_cli.goals import GoalManager
    ...
    def test_remove_subgoal_out_of_range(self, moor_home):
        import pytest
        from moor_cli.goals import GoalManager
...
class TestJudgeGoalWithSubgoals:
    def test_judge_uses_subgoals_template_when_provided(self, moor_home):
...
class TestWaitBarrier:
    ...
    def test_parked_on_live_pid_does_not_continue_or_judge(self, moor_home):
        from moor_cli import goals
        from moor_cli.goals import GoalManager
    ...
    def test_stop_waiting_clears_barrier(self, moor_home):
        from moor_cli.goals import GoalManager
    ...
    def test_barrier_on_a_process_that_never_exits_expires(self, moor_home):
...
class TestJudgeDrivenWait:
    ...
    def test_time_barrier_clears_after_deadline(self, moor_home):
        from moor_cli.goals import GoalManager
    ...
    def test_continue_verdict_still_continues_with_background(self, moor_home):
...
class TestSessionTriggerBarrier:
    ...
    def test_registry_releases_on_watch_match_while_alive(self, moor_home):
    ...
    def test_wait_on_session_validation(self, moor_home):
        from moor_cli.goals import GoalManager
...
class TestGoalContractSerialization:
    def test_roundtrip_with_contract(self):
        from moor_cli.goals import GoalState, GoalContract
    ...
    def test_render_block_omits_empty_fields(self):
        from moor_cli.goals import GoalContract
...
class TestGoalManagerContract:
    ...
    def test_set_contract_after_the_fact(self, moor_home):
        from moor_cli.goals import GoalManager, GoalContract
        ...
        # Survives reload.
        from moor_cli.goals import GoalManager as GM2
        assert GM2(session_id="c-after").has_contract()

    def test_persistence_roundtrip(self, moor_home):
        from moor_cli.goals import GoalManager, GoalContract
...
class TestJudgeWithContract:
    ...
    def test_judge_uses_contract_template(self, moor_home):
        from unittest.mock import patch
        from moor_cli import goals
        from moor_cli.goals import GoalContract
...
class TestDraftContract:
    def test_draft_parses_json(self, moor_home):
        from unittest.mock import patch
        from moor_cli import goals
    ...
    def test_draft_returns_none_when_no_client(self, moor_home):
        from unittest.mock import patch
        from moor_cli import goals
...
class TestContractAndBackgroundCompose:
    ...
    def test_judge_prompt_carries_contract_and_background(self, moor_home):
        from unittest.mock import patch
        from moor_cli import goals
        from moor_cli.goals import GoalContract
...
class TestBlockedVerdict:
    ...
    def test_blocked_verdict_pauses_goal_instead_of_done(self, moor_home):
        from unittest.mock import patch
        from moor_cli.goals import GoalManager
