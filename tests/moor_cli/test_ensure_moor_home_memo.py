"""ensure_moor_home is memoized per home path (perf: it runs on every
...
def test_distinct_home_paths_each_get_the_skeleton(tmp_path, monkeypatch):
    first = tmp_path / "a" / ".moor"
    second = tmp_path / "b" / ".moor"

    monkeypatch.setenv("MOOR_HOME", str(first))
    cfg.ensure_moor_home()

    # Profile switch: MOOR_HOME moves → the new path is ensured too.
