...
def _touch_ink(root: Path) -> None:
    ink = root / "node_modules" / "@moor" / "ink" / "package.json"
...
def test_make_tui_argv_uses_bundled_tui_when_workspace_missing(
    ...
    moor_cli/tui_dist/entry.js but never ships ui-tui/ (that directory only
...
def _write_ws(root: Path, ws_lock: str, hidden_lock: str) -> Path:
    ...
    ``@moor/ink`` and the marker live at the workspace root (hoisted);
    ...
    (tui_dir / "package.json").write_text('{"name":"moor-tui"}')
    return tui_dir
...
def test_need_install_when_linked_workspace_dep_missing(tmp_path: Path, main_mod) -> None:
    """The closure follows workspace symlinks (@moor/ink → ui-tui/packages/…)
    so a linked workspace's own missing dep triggers a reinstall."""
    tui_dir = _write_ws(
        tmp_path,
        '{"packages":{'
        '"ui-tui":{"dependencies":{"@moor/ink":"*"}},'
        '"node_modules/@moor/ink":{"link":true,"resolved":"ui-tui/packages/moor-ink"},'
        '"ui-tui/packages/moor-ink":{"dependencies":{"inkdep":"1.0.0"}},'
...
def test_workspace_closure_includes_dev_deps_of_selected_child_workspace(main_mod) -> None:
    ...
    packages = {
        "ui-tui": {"dependencies": {"@moor/ink": "*"}},
        "node_modules/@moor/ink": {
            "link": True,
            "resolved": "ui-tui/packages/moor-ink",
        },
        "ui-tui/packages/moor-ink": {"devDependencies": {"child-dev-only": "1"}},
    ...
    termux = main_tui_launch._npm_lock_workspace_closure(
        packages, {"ui-tui", "ui-tui/packages/moor-ink"}
    )
...
def test_termux_install_catches_missing_child_workspace_dev_dep(
    ...
    ws_lock = (
        ...
        '"ui-tui":{"dependencies":{"@moor/ink":"*"}},'
        '"node_modules/@moor/ink":{"link":true,"resolved":"ui-tui/packages/moor-ink"},'
        '"ui-tui/packages/moor-ink":{"devDependencies":{"child-dev-only":"1.0.0"}},'
    ...
    child = tui_dir / "packages" / "moor-ink"
    child.mkdir(parents=True, exist_ok=True)
    (child / "package.json").write_text('{"name":"@moor/ink"}')
...
def test_make_tui_argv_decodes_dev_prebuild_with_utf8_replace(
    ...
    ink_dir = tmp_path / "packages" / "moor-ink"
    ink_dir.mkdir(parents=True)
...
def test_no_stray_lockfiles_in_workspace_subdirs(main_mod) -> None:
    ...
    # Also sweep ui-tui/packages/* (moor-ink etc.)
...
class TestPersistentNpmUserconfig:
    """$MOOR_HOME/npmrc must reach every npm lifecycle child process (#106373)."""

    def test_moor_home_npmrc_sets_userconfig(self, tmp_path, monkeypatch):
        home = tmp_path / "home"
        home.mkdir()
