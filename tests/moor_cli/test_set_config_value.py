...
def _isolated_moor_home(tmp_path):
    """Point MOOR_HOME at a temp dir so tests never touch real config."""
...
class TestExplicitAllowlist:
    ...
    def test_explicit_key_routes_to_env(self, key, _isolated_moor_home):
        set_config_value(key, "test-value-123")
        env_content = _read_env(_isolated_moor_home)
        assert f"{key}=test-value-123" in env_content
        # Must NOT appear in config.yaml
        assert key not in _read_config(_isolated_moor_home)
...
class TestCatchAllPatterns:
    ...
    def test_api_key_suffix_routes_to_env(self, key, _isolated_moor_home):
...
class TestConfigYamlRouting:
    ...
    def test_simple_key(self, _isolated_moor_home):
        set_config_value("model", "gpt-4o")
        config = _read_config(_isolated_moor_home)
        assert "gpt-4o" in config
        assert "model" not in _read_env(_isolated_moor_home)


    def test_terminal_image_goes_to_config(self, _isolated_moor_home):
    ...
    def test_cron_script_timeout_is_recognized(self, _isolated_moor_home, capsys):
        ...
        assert "script_timeout_seconds: 600" in _read_config(_isolated_moor_home)

    def test_memory_nudge_interval_is_recognized(self, _isolated_moor_home, capsys):
        """The documented background-memory review interval is runtime config."""
        set_config_value("memory.nudge_interval", "0")

        assert "not a recognized config key" not in capsys.readouterr().out
        assert "nudge_interval: 0" in _read_config(_isolated_moor_home)

    def test_terminal_docker_cwd_mount_flag_goes_to_config_and_env(self, _isolated_moor_home):
    ...
    def test_terminal_vercel_runtime_goes_to_config_and_env(self, _isolated_moor_home):
...
class TestFalsyValues:
    ...
    def test_config_command_accepts_empty_string(self, _isolated_moor_home):
...
class TestConfigGetUnset:
    ...
    def test_config_get_prints_resolved_nested_value(self, _isolated_moor_home, capsys):
    ...
    def test_config_unset_removes_yaml_key_and_synced_env(self, _isolated_moor_home, capsys):
        set_config_value("terminal.backend", "docker")
        assert "TERMINAL_ENV=docker" in _read_env(_isolated_moor_home)
        capsys.readouterr()
    ...
    def test_config_unset_removes_dotted_token_yaml_key(self, _isolated_moor_home, capsys):
...
class TestListNavigation:
    """moor config set must preserve YAML list fields when using numeric
    ...
    def test_indexed_set_preserves_sibling_list_entries(self, _isolated_moor_home):
        """Setting custom_providers.0.api_key must not destroy entry 1."""
        self._write_config(_isolated_moor_home, (
            "custom_providers:\n"
            "- name: provider-a\n"
            "  api_key: old-a\n"
    ...
    def test_indexed_set_preserves_non_targeted_fields(self, _isolated_moor_home):
    ...
    def test_deeper_nesting_through_list(self, _isolated_moor_home):
...
class TestCronModelDriftConfigWarning:
    ...
    def test_warning_names_the_user_owned_cli_pin_path(
        ...
        assert "moor cron edit <job_id> --provider <provider> --model <model>" in warning
...
class TestStringTypedConfigValues:
    ...
    def test_non_string_defaults_keep_existing_coercion(
        self, _isolated_moor_home, key, value, expected
    ):
    ...
    def test_unknown_keys_keep_existing_coercion(self, _isolated_moor_home):
...
class TestSecretRedactionInDisplay:
    ...
    def test_redact_config_value_masks_nested_api_key(self):
        from moor_cli.config import redact_config_value
    ...
    def test_set_echo_masks_secret_value(self, _isolated_moor_home, capsys):
    ...
    def test_set_echo_keeps_nonsecret_value(self, _isolated_moor_home, capsys):
...
class TestSchemaValidation:
    """#34067: ``moor config set`` must not report bare success for
    ...
    def test_desktop_macos_signing_identity_is_accepted(self, _isolated_moor_home, capsys):
    ...
    def test_force_suppresses_notice(self, _isolated_moor_home, capsys):
        ...
        content = _read_config(_isolated_moor_home)
...
class TestDisplaySkinTouch:
    ...
    (`moor config set display.skin X` while it is already X — the recovery
    ...
    def test_reaffirming_same_skin_moves_the_watcher_signature(self, _isolated_moor_home):
        import os as _os
        skins = _isolated_moor_home / "skins"
        skins.mkdir()
    ...
    def test_builtin_or_missing_skin_file_is_fine(self, _isolated_moor_home):
        """Built-ins have no user file — the set must still succeed cleanly."""
        set_config_value("display.skin", "mono")
        assert "skin: mono" in _read_config(_isolated_moor_home)

    def test_touch_preserves_skin_file_contents(self, _isolated_moor_home):
        skins = _isolated_moor_home / "skins"
        skins.mkdir()
        body = "name: neon\ncolors:\n  ui_accent: '#ff33aa'\n"
...
class TestMappingGuard:
    """``moor config set <section> <scalar>`` must not silently destroy an
    ...
    def test_bare_model_shorthand_preserves_siblings(self, _isolated_moor_home):
        """moor config set model <id> → model.default, siblings survive."""
        self._write_config(_isolated_moor_home, {
            "model": {
                "default": "gpt-4o",
                "provider": "openai-api",
    ...
    def test_bare_model_shorthand_creates_default_when_none(self, _isolated_moor_home):
        """Bare model shorthand still works when config is empty (legacy behaviour)."""
        set_config_value("model", "gpt-5.6-sol")
        assert "gpt-5.6-sol" in _read_config(_isolated_moor_home)

    def test_non_model_mapping_is_refused(self, _isolated_moor_home):
        """moor config set terminal bash → refuse, terminal has sub-keys."""
        self._write_config(_isolated_moor_home, {
            "terminal": {
                "backend": "docker",
    ...
    def test_non_model_mapping_force_overwrites(self, _isolated_moor_home):
        """moor config set --force terminal bash → proceed, section wiped."""
        self._write_config(_isolated_moor_home, {
            "terminal": {
                "backend": "docker",
                "shell": "bash",
    ...
    def test_model_force_overwrites_entire_section(self, _isolated_moor_home):
        """moor config set --force model <id> → overwrite entire section."""
        self._write_config(_isolated_moor_home, {
            "model": {
                "default": "gpt-4o",
                "provider": "openai-api",
...
class TestMalformedYAMLConfigPreservation:
    ...
    def test_set_config_value_refuses_broken_yaml(self, _isolated_moor_home, capsys):
        """set_config_value must raise, not overwrite the broken config."""
        self._write_broken_config(_isolated_moor_home)
    ...
    def test_unset_config_value_refuses_broken_yaml(self, _isolated_moor_home, capsys):
...
class TestLiteralDotKeyEscaping:
    """``moor config set/unset/get`` must not split a key segment on a
    ...
    def test_split_key_path_escaped_dot(self):
        from moor_cli.config import _split_key_path
    ...
    def test_set_preserves_literal_dot_in_provider_key(self, _isolated_moor_home, capsys):
        self._write_config(_isolated_moor_home, {
            "providers": {
    ...
    def test_unset_removes_literal_dot_provider_key(self, _isolated_moor_home, capsys):
        self._write_config(_isolated_moor_home, {
            "providers": {
                "qwen3.5-397b-wafer-non-zdr": {"api": "https://pass.wafer.ai/v1"},
                "openrouter": {"api_key": "or-keep"},
    ...
    def test_unset_nested_field_under_literal_dot_key(self, _isolated_moor_home, capsys):
        self._write_config(_isolated_moor_home, {
            "providers": {
    ...
    def test_get_reads_literal_dot_provider_key(self, _isolated_moor_home, capsys):
        self._write_config(_isolated_moor_home, {
    ...
    def test_unescaped_dotted_path_unchanged(self, _isolated_moor_home):
