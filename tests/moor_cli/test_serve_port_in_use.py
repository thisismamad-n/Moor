...
``moor serve``, the gateway, anything) → ``moor serve`` printed only
...
* conflict → single stdout sentinel ``BACKEND_PORT_IN_USE port=<port>`` +
  ...
  transient-condition convention) — and NO ``MOOR_BACKEND_READY``.
* free explicit port → boots and announces ``MOOR_BACKEND_READY`` exactly
...
def test_probe_detects_held_socket():
    from moor_cli.web_server_lifecycle import _port_bind_conflict
...
def test_addr_in_use_error_classification():
    ...
    from moor_cli.web_server_lifecycle import _is_addr_in_use_error
...
def test_exit_code_is_distinct_tempfail():
    from moor_cli.web_server_lifecycle import PORT_IN_USE_EXIT_CODE
...
# ---------------------------------------------------------------------------
# E2E: real start_server in a subprocess (temp MOOR_HOME)
# ---------------------------------------------------------------------------
...
def _spawn_serve(port: int, tmp_path: Path, merge_stderr: bool = True) -> subprocess.Popen:
    ...
    for k in ("MOOR_DESKTOP", "MOOR_PARENT_PID", "MOOR_PARENT_START_MARKER",
              "MOOR_PARENT_NONCE"):
        env.pop(k, None)
    code = (
...
def test_conflict_emits_sentinel_and_exit_75(tmp_path):
    ...
    assert "MOOR_BACKEND_READY" not in out  # never claimed ready
...
def test_free_port_boots_and_announces_ready(tmp_path):
    ...
    try:
        ...
        assert f"MOOR_BACKEND_READY port={port}" in out
