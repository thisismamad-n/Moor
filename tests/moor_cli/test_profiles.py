"""Comprehensive tests for moor_cli.profiles module.
...
# ---------------------------------------------------------------------------
# Shared fixture: redirect Path.home() and MOOR_HOME for profile tests
# ---------------------------------------------------------------------------
...
def profile_env(tmp_path, monkeypatch):
    ...
    * Path.home() -> tmp_path  (so _get_profiles_root() = tmp_path/.moor/profiles)
    * MOOR_HOME  -> tmp_path/.moor  (so get_moor_home() agrees)
    * Creates the bare-minimum ~/.moor directory.
    """
...
class TestGetProfileDir:
    ...
    def test_default_returns_moor_home(self, profile_env):
...
class TestNoSkillsOptOut:
    """Tests for `moor profile create --no-skills` and the opt-out marker."""
    ...
    def test_delete_marker_re_enables_seeding(self, profile_env, monkeypatch):
        ...
        called = []
        stdout_by_call = [
            '{"copied": ["moor-agent"], "skipped_opt_out": true}',
            '{"copied": []}',
        ]
        monkeypatch.setattr(
            "subprocess.run",
        ...
        assert r1.get("copied") == ["moor-agent"]
        assert len(called) == 1
...
class TestBackfillProfileEnvs:
    """Tests for backfill_profile_envs() — the `moor update` pass that
...
class TestDeleteProfile:
    ...
    def test_rmtree_failure_raises(self, profile_env):
        ...
        with patch("moor_cli.profiles._cleanup_gateway_service"), \
             patch("moor_cli.profiles.time.sleep"), \
             patch("moor_cli.profiles.shutil.rmtree", side_effect=PermissionError("locked")):
    ...
    def test_backend_scan_only_matches_this_profile(self, profile_env, monkeypatch):
        ...
        procs = [
            ...
            FakeProc(101, ["python", "-m", "moor_cli.main", "--profile", "coder", "serve"]),
            # Interactive chat for coder → NOT a backend subcommand, skipped.
            FakeProc(102, ["python", "-m", "moor_cli.main", "--profile", "coder", "chat"]),
            # Backend for a different profile → skipped.
            FakeProc(103, ["python", "-m", "moor_cli.main", "--profile", "other", "serve"]),
            # This very process → skipped even if it matched.
            FakeProc(self_pid, ["python", "-m", "moor_cli.main", "--profile", "coder", "serve"]),
        ]
    ...
    def test_backend_scan_matches_shebang_exec_of_moor_shim(self, profile_env, monkeypatch):
        """A `moor` console-script shim spawned directly (e.g. Electron's
        findOnPath('moor') resolution) reports argv[0] as the interpreter
        (python3) and argv[1] as the shim's path -- not "moor" -- because
        ...
        procs = [
            ...
            # being the python interpreter, not "moor".
            FakeProc(201, ["/usr/bin/python3", "/Users/x/.local/bin/moor", "--profile", "coder", "serve",
                            "--host", "127.0.0.1", "--port", "0"]),
            # Same shape but a different profile → skipped.
            FakeProc(202, ["/usr/bin/python3", "/Users/x/.local/bin/moor", "--profile", "other", "serve"]),
            # Non-moor script run by python3 → skipped.
    ...
    def test_backend_scan_rejects_unrelated_moor_prefixed_script(self, profile_env, monkeypatch):
        """A user's own script that happens to start with "moor" (e.g.
        moor-notes.py, moor-unrelated-tool) must NOT be misidentified as
        the console-script shim just because argv[0] is a python interpreter
        and argv[1]'s basename starts with "moor" -- only the actual known
        console-script entry points (moor, moor-agent, moor-acp) count.
        """
        create_profile("coder", no_alias=True)
        profile_dir = get_profile_dir("coder")
        ...
        procs = [
            ...
            FakeProc(301, ["/usr/bin/python3", "/Users/x/scripts/moor-notes.py",
                            "--profile", "coder", "serve"]),
            FakeProc(302, ["/usr/bin/python3", "/Users/x/scripts/moor-unrelated-tool",
                            "--profile", "coder", "serve"]),
        ]
    ...
    def test_backend_scan_matches_all_known_console_script_shims(self, profile_env, monkeypatch):
        """The other two real console-script entry points (moor-agent,
        moor-acp -- see pyproject.toml [project.scripts]) must also be
        recognized via the shebang-exec path, not just the primary "moor"
        shim.
        """
        create_profile("coder", no_alias=True)
        profile_dir = get_profile_dir("coder")
        ...
        self_pid = os.getpid()
        procs = [
            FakeProc(401, ["/usr/bin/python3", "/Users/x/.local/bin/moor-agent",
                            "--profile", "coder", "serve"]),
            FakeProc(402, ["/usr/bin/python3", "/Users/x/.local/bin/moor-acp",
                            "--profile", "coder", "serve"]),
        ]
...
class TestActiveProfile:
    ...
    def test_set_to_default_removes_file(self, profile_env):
        ...
        create_profile("coder", no_alias=True)
        set_active_profile("coder")
        active_path = tmp_path / ".moor" / "active_profile"
...
class TestGetActiveProfileName:
    ...
    def test_custom_path_returns_default(self, profile_env, monkeypatch):
        """A custom MOOR_HOME (Docker, etc.) IS the default root."""
        tmp_path = profile_env
        custom = tmp_path / "some" / "other" / "path"
        custom.mkdir(parents=True)
        monkeypatch.setenv("MOOR_HOME", str(custom))
        # With Docker-aware roots, a custom MOOR_HOME is the default —
...
class TestWrapperScript:
    ...
    def test_creates_sh_on_posix(self, profile_env, monkeypatch):
        monkeypatch.setattr("moor_cli.profiles.shutil.which", lambda name: "/opt/moor/bin/moor")
        ...
        assert "exec /opt/moor/bin/moor -p mybot" in content


    @pytest.mark.windows_only
    def test_remove_finds_bat_on_windows(self, profile_env):
        from moor_cli.profiles import create_wrapper_script, remove_wrapper_script
        wrapper = create_wrapper_script("mybot")
        assert wrapper is not None
...
class TestFindAliasForProfile:
    ...
    def test_profile_named_alias(self, profile_env):
        from moor_cli.profiles import create_wrapper_script, find_alias_for_profile
    ...
    def test_ignores_unrelated_files(self, profile_env):
        ...
        from moor_cli.profiles import _get_wrapper_dir, find_alias_for_profile
...
class TestRenameProfile:
    ...
    def test_renames_directory(self, profile_env):
        ...
        old_dir = tmp_path / ".moor" / "profiles" / "oldname"
    ...
    def test_multiplexed_rename_unroutes_old_then_hot_serves_new(self, profile_env):
        ...
        create_profile("oldname", no_alias=True)
        old_dir = tmp_path / ".moor" / "profiles" / "oldname"
        new_dir = tmp_path / ".moor" / "profiles" / "newname"
        ...
        def _record_notify(name):
            ...
            if name == "oldname" and old_dir.exists():
                ...
                with pytest.raises(FileNotFoundError):
                    mkdir_under_moor_home(old_dir / "logs")

        with patch("moor_cli.profiles.check_alias_collision", return_value="skip"), \
             patch("moor_cli.profiles._live_default_multiplexer", return_value=True), \
             patch("moor_cli.profiles._notify_multiplexer", side_effect=_record_notify):
    ...
    def test_unmultiplexed_rename_does_not_signal_multiplexer(self, profile_env):
        ...
        with patch("moor_cli.profiles.check_alias_collision", return_value="skip"), \
             ...
             patch("moor_cli.profiles._notify_multiplexer") as notify:
            new_dir = rename_profile("oldname", "newname")

        notify.assert_not_called()
        assert not (tmp_path / ".moor" / "profiles" / ".deleted").exists()
        assert not old_dir.exists() and new_dir.is_dir()
    ...
    def test_rename_migrates_session_identity_without_live_gateway(self, profile_env):
        ...
        pdb.close()
        root_db = SessionDB(tmp_path / ".moor" / "state.db")
        ...
        moved_db.close()
        root_db2 = SessionDB(tmp_path / ".moor" / "state.db")
    ...
    def test_migrate_identity_command_repairs_a_failed_live_migration(self, profile_env, capsys):
        """The failed-live-migration end state must be recoverable: `moor profile
        ...
        with patch("moor_cli.profiles.check_alias_collision", return_value="skip"), \
             ...
             patch("gateway.control_socket.migrate_gateway_profile_identity", return_value=None):
            rename_profile("oldname", "newname")
        assert "moor profile migrate-identity oldname newname" in capsys.readouterr().err

        # Gateway restarted/stopped → the retry command repairs both stores.
        with patch("moor_cli.profiles._live_default_multiplexer", return_value=False):
        ...
        moved_db = SessionDB(tmp_path / ".moor" / "profiles" / "newname" / "state.db")
        row = moved_db._read_one(
            "SELECT session_key, profile_name FROM sessions WHERE id = ?", ("sess1",))
...
# ===================================================================
# TestGetProfilesRoot / TestGetDefaultMoorHome (internal helpers)
# ===================================================================

class TestInternalHelpers:
    """Tests for _get_profiles_root() and _get_default_moor_home()."""



    def test_default_moor_home_docker(self, tmp_path, monkeypatch):
        """In Docker, _get_default_moor_home() returns MOOR_HOME itself."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("MOOR_HOME", str(docker_home))
        home = _get_default_moor_home()
        assert home == docker_home



    def test_create_profile_docker(self, tmp_path, monkeypatch):
        """Profile created in Docker lands under MOOR_HOME/profiles/."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("MOOR_HOME", str(docker_home))
...
class TestEdgeCases:
    ...
    def test_gateway_running_check_falls_back_to_runtime_state(self, profile_env):
        ...
        with patch("gateway.status.get_running_pid", return_value=None), patch(
            ...
            return_value="moor gateway run --replace",
        ):
...
class TestProfilesToServe:
    ...
    def test_off_returns_only_active_named(self, profile_env, monkeypatch):
        # A named profile's gateway runs with MOOR_HOME pointing at the
        # profile dir; get_active_profile_name() infers the name from there.
        create_profile("coder", no_alias=True)
        monkeypatch.setenv("MOOR_HOME", str(get_profile_dir("coder")))
    ...
    def test_on_returns_default_plus_all_named(self, profile_env):
        ...
        assert serve["default"] == _get_default_moor_home()
...
class TestResolveProfileEnvSpelling:
    """resolve_profile_env() keeps the configured MOOR_HOME spelling as
    ...
    def test_resolution_matrix_preserves_configured_spelling(self, monkeypatch, tmp_path):
        ...
        for env_home, profile, expected in cases:
            monkeypatch.setenv("MOOR_HOME", str(env_home))
    ...
    def test_unset_env_falls_back_to_default_root(self, monkeypatch):
        # No MOOR_HOME: the platform default root applies (existing contract).
        monkeypatch.delenv("MOOR_HOME", raising=False)
        assert Path(resolve_profile_env("default")) == _get_default_moor_home()
...
class TestCloneAllExcludesRuntimeTrees:
    ...
    runtime trees the local-models flow puts under ``~/.moor``: ``models/``
    ...
    def test_runtime_trio_is_one_constant_shared_with_backup(self):
        ...
        from moor_cli import backup, profiles
        from moor_constants import LOCAL_RUNTIME_ROOT_DIRS
