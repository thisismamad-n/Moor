...
from moor_cli.dashboard_auth import clear_providers, get_provider
...
def _real_ctx() -> tuple[PluginManager, PluginContext]:
    manager = PluginManager(scope_key=moor_home_key())
...
def test_auth_provider_kept_out_of_manager_teardown_order():
    ...
    # Still attributed to the plugin for `moor plugins list`.
...
def test_targeted_unload_disposes_persistent_auth_provider():
    ...
    # `moor plugins disable basic` drives a targeted unload of that plugin.
