...
def container_env(tmp_path, monkeypatch):
    """Set up a fake MOOR_HOME with .container-mode file."""
    moor_home = tmp_path / ".moor"
    moor_home.mkdir()
    monkeypatch.setenv("MOOR_HOME", str(moor_home))
    monkeypatch.delenv("MOOR_DEV", raising=False)

    container_mode = moor_home / ".container-mode"
    container_mode.write_text(
        "# Written by NixOS activation script. Do not edit manually.\n"
        "backend=podman\n"
        "container_name=moor-agent\n"
        "exec_user=moor\n"
        "moor_bin=/data/current-package/bin/moor\n"
    )
    return moor_home
...
def test_get_container_exec_info_returns_metadata(container_env):
    ...
    with patch("moor_constants.is_container", return_value=False):
        info = get_container_exec_info()

    assert info is not None
    assert info["backend"] == "podman"
    assert info["container_name"] == "moor-agent"
    assert info["exec_user"] == "moor"
    assert info["moor_bin"] == "/data/current-package/bin/moor"
...
def test_exec_in_container_calls_execvp(docker_container_info):
    ...
    from moor_cli.main import _exec_in_container
    ...
    assert "-it" in cmd
    idx_u = cmd.index("-u")
    assert cmd[idx_u + 1] == "moor"
    e_indices = [i for i, v in enumerate(cmd) if v == "-e"]
    e_values = [cmd[i + 1] for i in e_indices]
    assert "TERM=xterm-256color" in e_values
    assert "LANG=en_US.UTF-8" in e_values
    assert "moor-agent" in cmd
    assert "/data/current-package/bin/moor" in cmd
    assert "chat" in cmd
