...
line merely mentions ``moor dashboard`` / ``moor serve`` — including the shell the
``--stop`` was typed into (``bash -c 'moor dashboard --stop'``). Killing it takes down
...
from moor_cli.dashboard_procs import _is_caller_wrapper_shell
...
def test_scan_spares_only_the_callers_wrapper_shell(capsys):
    ...
    wrapper with the same argv shape (another user's ``bash -c 'moor serve'``) stay targets."""
    processes = [
        (111, "/opt/moor/bin/python moor dashboard --port 9119"),
        (222, "bash -c moor dashboard --stop"),
        (333, "bash -c moor serve"),
    ]
    with (
        patch("moor_cli.dashboard_procs._scan_dashboard_processes", return_value=processes),
        patch("moor_cli.dashboard_procs._caller_ancestor_pids", return_value={222}),
        patch("moor_cli.dashboard_procs._argv_head_command",
...
def test_python_headed_or_unreadable_ancestor_stays_killed():
    ...
    with patch("moor_cli.dashboard_procs._argv_head_command", return_value="python3.12"):
        assert _is_caller_wrapper_shell(4242, {4242}) is False
    with patch("moor_cli.dashboard_procs._argv_head_command", return_value=None):
        assert _is_caller_wrapper_shell(4242, {4242}) is False
    with patch("moor_cli.dashboard_procs._argv_head_command", return_value="bash"):
