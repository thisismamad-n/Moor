...
def _validate(model, provider="openrouter", api_models=FAKE_API_MODELS, **kw):
    ...
    with patch("moor_cli.models.fetch_api_models", return_value=api_models), \
         patch("moor_cli.models.probe_api_models", return_value=probe_payload):
...
class TestCuratedModelsForProvider:
    ...
    def test_live_catalog_projected_to_tuples_else_static_fallback(self):
        with patch("moor_cli.models.provider_model_ids", return_value=["m-live"]):
            assert curated_models_for_provider("moor") == [("m-live", "")]
        with patch("moor_cli.models.provider_model_ids", return_value=[]), patch.dict(
            "moor_cli.models._PROVIDER_MODELS", {"moor": ["m-static"]}
        ):
            assert curated_models_for_provider("moor") == [("m-static", "")]
...
class TestFetchApiModels:
    ...
    def test_probe_api_models_tries_v1_fallback(self):
        ...
        with patch("moor_cli.models._urlopen_model_catalog_request", side_effect=_fake_urlopen):
    ...
    def test_probe_api_models_uses_copilot_catalog(self):
        ...
        with patch("moor_cli.models._urlopen_model_catalog_request", return_value=_Resp()) as mock_urlopen:
...
class TestValidateApiFallback:
    ...
    def test_fetch_lmstudio_models_filters_embedding_type(self):
        ...
        with patch("moor_cli.models._urlopen_model_catalog_request", return_value=mock_resp):
    ...
    def test_validate_lmstudio_distinguishes_auth_failure(self):
        ...
        with patch("moor_cli.models._urlopen_model_catalog_request", side_effect=http_error):
...
class TestValidateCodex900kVariants:
    """`-900k` is a Moor picker convention: valid variants come from the
    ...
    def test_catalog_listed_variant_accepted(self):
        with patch("moor_cli.models.provider_model_ids", return_value=self._CATALOG):
    ...
    def test_valid_variant_missing_from_catalog_still_accepted(self):
        ...
        with patch("moor_cli.models.provider_model_ids", return_value=["gpt-5.6-sol"]):
            result = validate_requested_model("gpt-5.6-sol-900k", "openai-codex")
        assert result["accepted"] is True
...
class TestProbeApiModelsUserAgent:
    """Probing custom /v1/models must send a Moor User-Agent.
    ...
    def test_probe_sends_moor_user_agent(self):
        ...
        assert ua.startswith("moor-cli/"), (
            f"User-Agent must advertise moor-cli, got {ua!r}"
        )
    ...
    def test_probe_user_agent_sent_without_api_key(self):
        ...
        assert ua and ua.startswith("moor-cli/")
...
class TestValidateOpenRouterVariantSuffixes:
    ...
    def test_static_catalog_fallback_accepts_variant(self):
        ...
        with patch("moor_cli.models.fetch_api_models", return_value=None), \
             patch(
                 "moor_cli.models.provider_model_ids",
...
class TestValidateRequestedModelMoorPortalRecommendations:
    """Regression tests for issue #71312: the Moor Telegram picker (and any
    other messaging-platform /model validation, since they all share
    validate_requested_model()) rejected models that are live Moor Portal
    recommendations (/api/moor/recommended-models) but not yet in the
    hardcoded curated catalog -- even though `moor chat` already accepts
    ...
    def _validate_moor(self, model, api_models=None, portal_payload=None, portal_raises=False):
        ...
        with patch("moor_cli.models.fetch_api_models", return_value=api_models), \
             patch("moor_cli.models.probe_api_models", return_value=probe_payload), \
             patch("moor_cli.models.fetch_moor_recommended_models", side_effect=_fetch_portal), \
             patch("moor_cli.models._resolve_moor_portal_url", return_value="https://portal.nousresearch.com"), \
             patch("moor_cli.models._model_in_provider_catalog", return_value=False):
            return validate_requested_model(model, "moor")
    ...
    def test_free_portal_recommendation_accepted(self):
        ...
        result = self._validate_moor("inclusionai/ling-3.0-flash:free")
        assert result["accepted"] is True
        assert result["persist"] is True
        assert "Portal recommendation" in (result["message"] or "")

    def test_paid_portal_recommendation_accepted(self):
        result = self._validate_moor("inclusionai/ling-3.0-pro")
        assert result["accepted"] is True
    ...
    def test_model_absent_from_portal_and_catalog_still_rejected(self):
        ...
        result = self._validate_moor("totally-made-up-model-xyz")
        assert result["accepted"] is False
    ...
    def test_non_string_model_name_entries_ignored(self):
        ...
        assert self._validate_moor("5", portal_payload=payload)["accepted"] is False
        result = self._validate_moor(
            "inclusionai/ling-3.0-flash:free", portal_payload=payload
        )
        assert result["accepted"] is True

    def test_non_moor_provider_does_not_consult_portal_feed(self):
        """This fallback tier is Moor-specific; a non-Moor provider must
        not have its rejection changed by (or trigger a call to) the Moor
        Portal feed."""
        probe_payload = {
        ...
        with patch("moor_cli.models.fetch_api_models", return_value=["some/other-model"]), \
             patch("moor_cli.models.probe_api_models", return_value=probe_payload), \
             patch("moor_cli.models.fetch_moor_recommended_models") as mock_portal, \
             patch("moor_cli.models._model_in_provider_catalog", return_value=False):
    ...
    def test_curated_catalog_hit_short_circuits_before_portal_check(self):
        ...
        with patch("moor_cli.models.fetch_api_models", return_value=api_models), \
             patch("moor_cli.models.probe_api_models", return_value=probe_payload), \
             patch("moor_cli.models._model_in_provider_catalog", return_value=True), \
             patch("moor_cli.models.fetch_moor_recommended_models") as mock_portal:
            result = validate_requested_model("inclusionai/ling-2.6-flash", "moor")
        mock_portal.assert_not_called()
        assert result["accepted"] is True
