"""Orphan Desktop-local ``moor serve`` reap at backend start.
...
def test_desktop_local_serve_shape_matches_ephemeral_loopback():
    assert _is_desktop_local_serve_cmdline(
        "python -m moor_cli.main serve --host 127.0.0.1 --port 0"
    )
    assert _is_desktop_local_serve_cmdline(
        "moor serve --isolated --host 127.0.0.1 --port 0 --ssh-owner-nonce abc"
    )
    assert _is_desktop_local_serve_cmdline(
        "/venv/bin/moor serve --host=127.0.0.1 --port=0"
    )


def test_desktop_local_serve_shape_spares_fixed_port_and_non_serve():
    assert not _is_desktop_local_serve_cmdline(
        "moor serve --host 100.106.105.2 --port 9119 --skip-build"
    )
    assert not _is_desktop_local_serve_cmdline(
        "moor serve --host 127.0.0.1 --port 9119"
    )
    assert not _is_desktop_local_serve_cmdline("moor gateway run --replace")
    # "serve" inside another token is not the serve subcommand (token matcher, not substring).
    assert not _is_desktop_local_serve_cmdline(
        "moor kanban --preserve-cache --host 127.0.0.1 --port 0"
    )


def test_reap_only_kills_ppid1_local_serves():
    scanned = [
        (111, "moor serve --host 127.0.0.1 --port 0"),  # orphan local
        (222, "moor serve --host 127.0.0.1 --port 0"),  # still has parent
        (333, "moor serve --host 100.1.2.3 --port 9119"),  # fixed remote
        (444, "moor serve --isolated --host 127.0.0.1 --port 0"),  # orphan isolated
    ]
    ...
    with (
        ...
        patch(
            "moor_cli.dashboard_procs._process_ppid",
...
def _valid_lock_payload(pid: int, ownership_id: str, spawn_nonce: str) -> dict:
    ...
    return {
        ...
        "moorHome": "~/.moor",
        "logPath": f"~/.moor/desktop-ssh/{ownership_id}/{spawn_nonce}.log",
...
def test_valid_lockfile_payload_rejects_wrong_owner_and_shape():
    ...
    bad_log["logPath"] = "~/.moor/desktop-ssh/{oid}/other.log".format(oid=oid)
...
def test_reap_spare_lock_owned_ssh_remote_backend_of_foreign_client():
    ...
    scanned = [
        (555, "moor serve --host 127.0.0.1 --port 0"),  # lock-owned remote
        (666, "moor serve --host 127.0.0.1 --port 0"),  # genuine orphan
    ]
    ppids = {555: 1, 666: 1}
    terms: list[int] = []
    live = {555, 666}
...
def test_reap_spares_young_backend_until_desktop_can_write_lock():
    ...
    scanned = [(777, "moor serve --isolated --host 127.0.0.1 --port 0")]
    terms: list[int] = []
...
def test_reap_spares_backend_when_process_age_is_unknown():
    scanned = [(778, "moor serve --isolated --host 127.0.0.1 --port 0")]
    terms: list[int] = []
...
def test_reap_age_boundary_makes_180_second_orphan_eligible():
    scanned = [
        (779, "moor serve --isolated --host 127.0.0.1 --port 0"),
        (780, "moor serve --isolated --host 127.0.0.1 --port 0"),
    ]
    terms: list[int] = []
    live = {779, 780}
...
def test_reap_spare_lock_owned_backend_even_without_exclude_match(tmp_path):
    ...
    spares a matching orphaned serve even when MOOR_DESKTOP_CHILD_PID is
    ...
    scanned = [(4242, "moor serve --host 127.0.0.1 --port 0")]
    terms: list[int] = []
