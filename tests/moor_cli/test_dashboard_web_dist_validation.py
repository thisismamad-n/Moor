"""Regression tests: `moor dashboard` validates MOOR_WEB_DIST before serving.

A custom MOOR_WEB_DIST without --skip-build previously skipped BOTH the
...
def test_env_dist_without_index_exits(main_mod, monkeypatch, tmp_path, capsys):
    ...
    builds = []
    monkeypatch.setattr(
        "moor_cli.main_web_build._build_web_ui", lambda *a, **k: builds.append(a) or True
    )
    ...
    assert "MOOR_WEB_DIST" in out and str(empty_dist) in out
...
def test_skip_build_missing_dist_attempts_one_recovery_build(
    ...
    _wire_common(main_mod, monkeypatch)
    monkeypatch.delenv("MOOR_WEB_DIST", raising=False)
    project_root = tmp_path / "proj"
    dist = project_root / "moor_cli" / "web_dist"
    ...
    monkeypatch.setattr("moor_cli.main_web_build._build_web_ui", fake_build)
