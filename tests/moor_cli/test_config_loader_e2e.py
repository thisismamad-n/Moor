...
Runs a real subprocess with a temp MOOR_HOME whose config.yaml contains a
``${ENV_VAR}`` reference, plus a managed-scope overlay dir (MOOR_MANAGED_DIR).
...
Subprocess (not in-process monkeypatching) so module-level ``_moor_home``
globals, managed-scope caches, and ``_under_pytest`` guards behave like
production: ``MOOR_MANAGED_DIR`` is set explicitly, which bypasses the
...
def test_behavioral_read_gets_expansion_and_overlay_while_writeback_stays_raw(
    ...
    code = textwrap.dedent(
        ...
        import os
        from pathlib import Path
        saved = Path(server._moor_home, "config.yaml").read_text(encoding="utf-8")
    ...
    out = _run_py(
        ...
        {
            "MOOR_HOME": str(home),
            "MOOR_MANAGED_DIR": str(managed_dir),
...
def test_writeback_roundtrip_byte_identical_when_unchanged(tmp_path):
    ...
    out = _run_py(code, {"MOOR_HOME": str(home)}, tmp_path)
