...
# ---- end-to-end migrate(, expose_moor_tools=False) ----
...
class TestMigrate:
    ...
    def test_plugin_discovery_failure_non_fatal(self, tmp_path, monkeypatch):
        ...
        report = migrate({"mcp_servers": {"x": {"command": "y"}}},
                         codex_home=tmp_path, discover_plugins=True, expose_moor_tools=False)
    ...
    def test_full_migration_round_trip(self, tmp_path):
        ...
        report = migrate(moor_cfg, codex_home=tmp_path, expose_moor_tools=False)
        assert report.written
        text = (tmp_path / "config.toml").read_text()
    ...
    def test_preserves_user_mcp_server_outside_managed_block(self, tmp_path):
        ...
        migrate({"mcp_servers": {"moor-mcp": {"command": "npx"}}},
        ...
        assert "[mcp_servers.moor-mcp]" in final
    ...
    def test_summary_reports_migration_count(self, tmp_path):
        ...
        }, codex_home=tmp_path, expose_moor_tools=False)
...
class TestStripUnmanagedPluginTables:
    ...
    ``moor codex-runtime migrate`` would re-emit them inside the managed
    ...
    def test_migrate_dedups_codex_owned_plugin_tables(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            "moor_cli.codex_runtime_plugin_migration._query_codex_plugins",
            fake_query,
        )
        migrate({}, codex_home=tmp_path, discover_plugins=True, expose_moor_tools=False)
...
# ---- Bug C: MOOR_HOME tempdir leak into ~/.codex/config.toml ----


class TestMoorHomeLeakGuard:
    """Regression tests for issue #26250 Bug C.

    Previously ``_build_moor_tools_mcp_entry()`` read ``MOOR_HOME``
    ...
    once codex spawned the moor-tools MCP subprocess.
    """




    def test_real_moor_home_propagates(self, monkeypatch, tmp_path):
        """A legitimate MOOR_HOME (not a tempdir path) DOES propagate so the
        ...
        real_path = "/Users/alice/.moor"
        monkeypatch.setenv("MOOR_HOME", real_path)
        entry = _build_moor_tools_mcp_entry()
        env = entry.get("env", {})
        assert env.get("MOOR_HOME") == real_path

    def test_unset_moor_home_omits_env_key(self, monkeypatch):
        """When MOOR_HOME is unset in the environment, the MCP entry MUST
        NOT bake in a resolved-default path. The codex subprocess should
        inherit whatever MOOR_HOME its launcher (systemd, gateway, shell)
        ...
        entry = _build_moor_tools_mcp_entry()
        env = entry.get("env", {})
        assert "MOOR_HOME" not in env, (
            f"MOOR_HOME should not be set when env var is unset, got: "
            f"{env.get('MOOR_HOME')!r}"
        )
