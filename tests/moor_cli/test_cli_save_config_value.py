...
class TestSaveConfigValueAtomic:
    ...
    def config_env(self, tmp_path, monkeypatch):
        ...
        # save_config_value resolves the target live via get_moor_home(), so
        # point MOOR_HOME at the temp dir (the _moor_home import-time
...
class TestSaveConfigValueTargetsUserConfig:
    """Regression: persisted runtime settings must land in MOOR_HOME/config.yaml
    ...
    install whose MOOR_HOME/config.yaml did not exist yet, save_config_value
    fell back to the checked-in cli-config.yaml. The toggle reported success, but
    startup read MOOR_HOME/config.yaml and never saw the setting."""

    def test_creates_user_config_when_absent(self, tmp_path, monkeypatch):
        # Fresh MOOR_HOME with NO config.yaml (managed/desktop first launch).
        moor_home = tmp_path / ".moor"
        moor_home.mkdir()
        monkeypatch.setenv("MOOR_HOME", str(moor_home))
