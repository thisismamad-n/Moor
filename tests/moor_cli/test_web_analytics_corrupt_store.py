...
from moor_cli.web_routers import _common, analytics
...
def test_corrupt_store_polls_return_status_and_warn_once_per_interval(tmp_path, monkeypatch, caplog):
    ...
    with caplog.at_level(logging.DEBUG, logger="moor_cli.web_server"):
    ...
    for resp in (first, second, third):
        ...
        assert "moor doctor" in resp.json()["detail"]["message"]
    ...
    warnings = [
        ...
        if r.levelno >= logging.WARNING and r.name.startswith("moor_cli.web_server")
    ]
    ...
    assert sum(
        r.levelno >= logging.WARNING and r.name.startswith("moor_cli.web_server")
