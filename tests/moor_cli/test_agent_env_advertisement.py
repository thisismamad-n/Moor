"""Tests for the AI_AGENT / MOOR_AGENT harness-attribution env vars.
...
cross-session leak guard that strips ``MOOR_SESSION_*`` from subprocess
...
class TestWrapCommandAdvertisesHarness:
    ...
    def _wrap(self, command: str) -> str:
        ...
        env._cwd_marker = "__MOOR_CWD_testsession0__"
        env._snapshot_path = "/tmp/moor-snap-testsession0.sh"
    ...
    def test_wrap_command_contains_export(self):
        ...
        assert 'MOOR_AGENT="${MOOR_AGENT:-true}"' in wrapped
    ...
    def test_shell_sets_default_and_preserves_outer(self):
        ...
        wrapped = self._wrap('echo "AI=$AI_AGENT MOOR=$MOOR_AGENT"')

        clean_env = {k: v for k, v in os.environ.items()
                     if k not in ("AI_AGENT", "MOOR_AGENT")}
        out = subprocess.run(
        ...
        outer_env = dict(clean_env, AI_AGENT="pi", MOOR_AGENT="false")
        out = subprocess.run(
            ["bash", "-c", wrapped], capture_output=True, text=True,
