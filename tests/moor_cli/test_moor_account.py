"""Tests for normalized Moor Portal account entitlement helpers."""
...
from moor_cli.moor_account import (
    ...
    reset_moor_portal_account_info_cache,
)
...
def test_fresh_account_payload_normalization(monkeypatch, payload, expected_paid):
    ...
    monkeypatch.setattr("moor_cli.auth.get_provider_auth_state", lambda provider: _state(token))
    monkeypatch.setattr("moor_cli.auth.resolve_moor_access_token", lambda: "fresh-token")
    monkeypatch.setattr("moor_cli.moor_account._fetch_moor_account_info", lambda *a, **kw: payload)

    info = get_moor_portal_account_info(force_fresh=True)

    assert isinstance(info, MoorPortalAccountInfo)
...
def test_no_oauth_token_reports_inference_key_present(monkeypatch):
    monkeypatch.setattr("moor_cli.auth.get_provider_auth_state", lambda provider: {})
    ...
    assert info.credential_source == "pool:manual-moor"
...
def test_member_spend_cap_exceeded_message(monkeypatch):
    ...
    msg = format_moor_portal_entitlement_message(info, capability="Moor model access")
