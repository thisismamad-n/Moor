...
def test_termux_skips_bundled_skill_sync_when_stamp_fresh(monkeypatch, tmp_path, main_mod):
    ...
    monkeypatch.setattr(main_mod, "get_moor_home", lambda: tmp_path)
...
def test_oneshot_subprocess_exits_without_teardown_abort():
    program = textwrap.dedent(
        ...
        from moor_cli.main import _exit_after_oneshot
...
def test_oneshot_wires_session_db_for_recall(monkeypatch):
    """moor -z bypasses MoorCLI, but recall still needs SessionDB."""
    from moor_cli.oneshot import _run_agent
    ...
    monkeypatch.setitem(sys.modules, "moor_state", mod("moor_state", SessionDB=FakeSessionDB))
    monkeypatch.setitem(
        sys.modules,
        "moor_cli.config",
        mod("moor_cli.config", load_config=lambda: {"model": {"default": "m"}}),
    )
    monkeypatch.setitem(
        sys.modules,
        "moor_cli.models",
        mod("moor_cli.models", detect_provider_for_model=lambda *_args, **_kwargs: None),
    )
    monkeypatch.setitem(
        sys.modules,
    ...
    monkeypatch.setitem(
        ...
        mod("moor_cli.tools_config", _get_platform_tools=lambda *_args, **_kwargs: {"session_search"}),
    )
...
def test_launch_tui_exports_model_provider_and_toolsets(monkeypatch, main_mod):
    ...
    def fake_call(argv, cwd=None, env=None):
        ...
        active_path_during_call = Path(env["MOOR_TUI_ACTIVE_SESSION_FILE"])
    ...
    with pytest.raises(SystemExit):
        main_mod._launch_tui(
            model="moor/moor-test", provider="moor", toolsets="web, terminal"
        )

    env = captured["env"]
    assert env["MOOR_MODEL"] == "moor/moor-test"
    assert env["MOOR_INFERENCE_MODEL"] == "moor/moor-test"
    assert env["MOOR_TUI_PROVIDER"] == "moor"
    assert env["MOOR_INFERENCE_PROVIDER"] == "moor"
    assert env["MOOR_TUI_TOOLSETS"] == "web,terminal"
    active_path = Path(env["MOOR_TUI_ACTIVE_SESSION_FILE"])
    assert active_path.name.startswith("moor-tui-active-session-")
...
def test_make_tui_argv_dev_prebuilds_moor_ink(monkeypatch, main_mod, tmp_path):
