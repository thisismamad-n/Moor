...
def test_suffix_scopes_default_home_and_profiles(tmp_path, suffix):
    ...
    env.update(
        ...
        MOOR_DATA_DIR_SUFFIX=suffix,
    )
    script = """
import json
import os
from pathlib import Path
from moor_constants import get_moor_home, get_process_moor_home, get_default_moor_root
from moor_cli.profiles import _get_profiles_root, _get_active_profile_path, resolve_profile_env
root = get_default_moor_root()
profile = root / 'profiles' / 'coder'
profile.mkdir(parents=True)
(profile / 'config.yaml').write_text('', encoding='utf-8')
result = [str(get_moor_home()), str(get_process_moor_home()), str(root),
          str(_get_profiles_root()), str(_get_active_profile_path()), resolve_profile_env('coder')]
os.environ['MOOR_HOME'] = str(profile)
result.extend([str(get_moor_home()), str(get_default_moor_root())])
os.environ.pop('MOOR_HOME')
os.environ['MOOR_DATA_DIR_SUFFIX'] += '-changed'
result.append(str(get_default_moor_root()))
...
"""
    ...
    base = tmp_path / "AppData" / "Local" / "moor" if sys.platform == "win32" else tmp_path / ".moor"
...
def test_startup_readers_use_the_suffixed_home(tmp_path, monkeypatch):
    ...
    from moor_cli.dashboard_procs import _moor_home_dir
    from moor_cli.env_loader import load_moor_dotenv
    from moor_startup_watchdog import get_startup_watchdog_dump_path

    monkeypatch.setattr(Path, "home", lambda: tmp_path)
    monkeypatch.setenv("LOCALAPPDATA", str(tmp_path / "AppData" / "Local"))
    monkeypatch.delenv("MOOR_HOME", raising=False)
    monkeypatch.setenv("MOOR_DATA_DIR_SUFFIX", "magic-test")
    monkeypatch.delenv("SUFFIX_TEST_CREDENTIAL", raising=False)
    home = get_process_moor_home()
    home.mkdir(parents=True)
    (home / ".env").write_text("SUFFIX_TEST_CREDENTIAL=suffixed\n", encoding="utf-8")

    load_moor_dotenv(project_env=tmp_path / "absent.env", load_external_secrets=False)
    assert os.environ.get("SUFFIX_TEST_CREDENTIAL") == "suffixed"
    assert _moor_home_dir() == home
...
def test_suffix_does_not_change_explicit_or_context_home(tmp_path, monkeypatch):
    from moor_constants import (
        get_default_moor_root, get_moor_home, get_process_moor_home,
    ...
    monkeypatch.setenv("MOOR_HOME", str(explicit))
    monkeypatch.setenv("MOOR_DATA_DIR_SUFFIX", "magic-test")
    assert get_moor_home() == get_process_moor_home() == get_default_moor_root() == explicit
    token = set_moor_home_override(scoped)
    try:
        assert get_moor_home() == scoped
        assert get_process_moor_home() == get_default_moor_root() == explicit
    finally:
