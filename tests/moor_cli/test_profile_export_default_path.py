...
def profiles():
    ...
    Sibling test files reload ``moor_cli.profiles`` / ``moor_cli.main``;
    a top-level ``from moor_cli.profiles import ...`` here would bind the
    ...
    """
    return importlib.import_module("moor_cli.profiles")
...
def test_default_export_path_is_managed_and_outside_named_profiles(
    ...
    default_home.mkdir()
    monkeypatch.setattr(profiles, "_get_default_moor_home", lambda: default_home)
...
def test_custom_moor_home_inside_a_checkout_uses_a_sibling_store(
    tmp_path, monkeypatch, profiles
):
    ...
    monkeypatch.setattr(profiles, "_get_default_moor_home", lambda: checkout)
...
def test_checkout_detection_does_not_depend_on_cwd(tmp_path, monkeypatch, profiles):
    """MOOR_HOME inside a checkout must be detected even when cwd is elsewhere.
    ...
    checkout.mkdir()
    (checkout / ".git").write_text("gitdir: ../git\n", encoding="utf-8")
    moor_home = checkout / "data_home"
    moor_home.mkdir()
    elsewhere = tmp_path / "elsewhere"
    elsewhere.mkdir()
    monkeypatch.chdir(elsewhere)  # cwd has no .git ancestor inside tmp_path
    monkeypatch.setattr(Path, "home", lambda: tmp_path / "home")
    monkeypatch.setattr(profiles, "_get_default_moor_home", lambda: moor_home)
...
def test_cli_export_rejects_bad_profile_name_without_traceback(
    ...
    main_mod = importlib.import_module("moor_cli.main")
    default_home = tmp_path / ".moor"
    default_home.mkdir()
    monkeypatch.setattr(profiles, "_get_default_moor_home", lambda: default_home)
...
def test_slash_export_uses_the_same_managed_destination(
    ...
    mixin_mod = importlib.import_module("moor_cli.cli_commands_mixin")
    default_home = tmp_path / ".moor"
    default_home.mkdir()
    monkeypatch.setattr(profiles, "_get_default_moor_home", lambda: default_home)
...
async def test_profile_export_api_uses_the_shared_managed_destination(
    ...
    from moor_cli.web_models import ProfileExport

    router_mod = importlib.import_module("moor_cli.web_routers.profiles")
...
def test_cwd_in_unrelated_checkout_does_not_prove_safety(
    ...
    of a MOOR_HOME inside checkout B."""
    ...
    monkeypatch.setattr(Path, "home", lambda: tmp_path / "home")
    monkeypatch.setattr(profiles, "_get_default_moor_home", lambda: checkout_b)
