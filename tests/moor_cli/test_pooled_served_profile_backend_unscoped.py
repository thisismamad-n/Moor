"""A pooled Desktop backend (``moor --profile X serve``, MOOR_HOME=<root>/profiles/X) answers its
REST without ``?profile=``. Those UNSCOPED reads/verbs are about X — a profile the live default
multiplexer serves — and must agree with the scoped ``?profile=X`` answer and with ``moor -p X status``:
...
restart`` under X's MOOR_HOME that exited 78 while the UI reported success.
"""
...
def pooled_served_process(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("MOOR_HOME", str(root / "profiles" / "alpha"))
...
def test_unscoped_lifecycle_verbs_in_a_served_profile_process_address_the_multiplexer(pooled_served_process):
    ...
    assert _profile_action_environment(restart)["MOOR_HOME"] == str(pooled_served_process)
