"""OAuth-shaped model-provider plugins are first-class in `moor auth` (#116408).

The seam: ``ProviderProfile.auth_handler``. These tests drive the real ``moor auth`` argparse
surface against a plugin discovered from an isolated MOOR_HOME, so they fail if the dispatch is
...
def _rediscover() -> None:
    """Point the next profile lookup at the (new) MOOR_HOME user plugin dir.
...
def install_provider(tmp_path, monkeypatch):
    """Write a model-provider plugin into an isolated MOOR_HOME and discover it."""
    installed: list[str] = []
...
def _parse_auth_args(argv: list[str]) -> argparse.Namespace:
    """Parse `moor auth <argv>` through the real subcommand parser."""
...
def test_oauth_plugin_owns_every_auth_action(tmp_path, install_provider, capsys, argv, expected):
    """An oauth_external plugin registers and `moor auth <action>` reaches its handler, args included,
    ...
    assert not (tmp_path / "moor" / "auth.json").exists()
...
def test_oauth_plugin_without_handler_fails_loud_and_builtins_are_untouched(tmp_path, install_provider):
    ...
    pool = json.loads((tmp_path / "moor" / "auth.json").read_text(encoding="utf-8"))["credential_pool"]
