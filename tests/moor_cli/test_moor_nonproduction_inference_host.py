"""Operator authority over Moor routing overrides and Portal-returned inference hosts.
...
* Profile authority — ``MOOR_INFERENCE_BASE_URL`` / ``MOOR_PORTAL_BASE_URL`` are per-profile
...
* Service authority — a network-provided inference host outside the production set is accepted
  only when it equals the host the operator named in ``MOOR_INFERENCE_BASE_URL``. Sharing the
...
OTHER_MOOR_HOST = "https://portal.nousresearch.com/v1"  # Moor-owned, but not an inference gateway
NONPROD_PORTAL = "https://portal.example-env.nousresearch.com"


@pytest.mark.parametrize("helper, var", [
    (auth_moor._moor_inference_env_override, "MOOR_INFERENCE_BASE_URL"),
    (auth_moor._moor_portal_env_override, "MOOR_PORTAL_BASE_URL"),
])
...
@pytest.mark.parametrize(
    ...
    [
        ...
        # Another Moor-owned host is not the named recipient — refused, DNS suffix or not.
        (ENV_INFERENCE, OTHER_MOOR_HOST, None),
...
def test_network_inference_host_needs_the_operator_named_recipient(monkeypatch, inference_override, url, expected):
    if inference_override is None:
        ...
        monkeypatch.setenv("MOOR_INFERENCE_BASE_URL", inference_override)
    assert auth_moor._validate_moor_inference_url_from_network(url) == expected
...
def test_portal_override_alone_grants_nothing(monkeypatch):
    ...
    monkeypatch.setenv("MOOR_PORTAL_BASE_URL", NONPROD_PORTAL)
    monkeypatch.delenv("MOOR_INFERENCE_BASE_URL", raising=False)
    state = {"portal_base_url": NONPROD_PORTAL, "inference_base_url": ENV_INFERENCE, "client_id": "cid"}
    portal, stored_inference, _effective, _ = auth_moor._moor_effective_routing(state)
    assert portal == NONPROD_PORTAL and stored_inference == auth_moor.DEFAULT_MOOR_INFERENCE_URL
    assert auth_moor._healed_moor_inference_url({"inference_base_url": ENV_INFERENCE}) == (
        auth_moor.DEFAULT_MOOR_INFERENCE_URL)
    monkeypatch.setenv("MOOR_INFERENCE_BASE_URL", ENV_INFERENCE)
    assert auth_moor._healed_moor_inference_url({"inference_base_url": ENV_INFERENCE}) == ENV_INFERENCE
    assert auth_moor._validate_moor_inference_url_from_network(ENV_INFERENCE.replace("https", "http", 1)) is None
...
def test_recipient_match_follows_the_profile_scope_under_multiplex(monkeypatch):
    ...
    try:
        token = ss.set_secret_scope({})
        try:
            assert auth_moor._validate_moor_inference_url_from_network(ENV_INFERENCE) is None
        finally:
            ss.reset_secret_scope(token)
        token = ss.set_secret_scope({"MOOR_INFERENCE_BASE_URL": ENV_INFERENCE})
        try:
            assert auth_moor._validate_moor_inference_url_from_network(ENV_INFERENCE) == ENV_INFERENCE
        finally:
            ss.reset_secret_scope(token)
    finally:
        ss.set_multiplex_active(False)
