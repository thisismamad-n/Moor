"""Tests for the post-pull HEAD-movement gate in ``moor update``.
...
def _patch_update_deps(monkeypatch, tmp_path, run_side_effect):
    """Patch the moor_cli.main helpers ``_cmd_update_impl`` touches.

    ``_m()`` in update_cmd.py lazily returns moor_cli.main, so patching
    attributes on that module is the canonical test surface (matches
    tests/moor_cli/test_cmd_update.py).
    """
    ...
    monkeypatch.setattr(
        moor_gateway, "find_gateway_pids", lambda all_profiles=False: []
    )
    monkeypatch.setattr(
        moor_gateway, "supports_systemd_services", lambda: False
    )
    monkeypatch.setattr(
...
def test_update_success_when_head_moves(monkeypatch, tmp_path, capsys):
    ...
    moor_main.cmd_update(args)  # completes normally (no SystemExit)
