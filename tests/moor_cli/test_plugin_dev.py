...
def test_resolve_rejects_directory_without_manifest(tmp_path: Path) -> None:
    ...
    from moor_cli.plugin_dev import resolve_plugin_path
...
def test_doctor_default_target_does_not_copy_cwd(
    ...
    """``moor plugins doctor`` with no argument defaults to ``.``.

    Before the manifest guard, that copied the whole working directory into
    a temporary MOOR_HOME — running it from ``$HOME`` copied the home
...
def test_resolve_prefers_installed_id_over_unrelated_local_dir(
    ...
    installed = moor_home / "plugins" / "sample"
    ...
    monkeypatch.chdir(workdir)
    monkeypatch.setattr(plugin_dev, "get_moor_home", lambda: moor_home)
...
def test_doctor_removes_temp_home_when_staging_copy_fails(
    ...
    """A copy failure (e.g. ENOSPC) must not strand a moor-plugin-doctor-* dir."""
    import errno
    import shutil
    import tempfile
    ...
    # exact window where a stranded moor-plugin-doctor-* dir was observed.
    leftovers = list(scratch.glob("moor-plugin-doctor-*"))
