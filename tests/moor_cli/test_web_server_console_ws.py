"""Dashboard Moor Console websocket tests."""
...
@pytest.fixture
def console_client(monkeypatch, _isolate_moor_home):
...
def test_console_ws_cancel_returns_to_prompt(console_client, monkeypatch):
    from moor_cli.console_engine import ConsoleResult, MoorConsoleEngine

    def slow_execute(self, line: str, *, confirmed: bool = False):
        time.sleep(0.2)
        return ConsoleResult("ok", output="late", command=line)

    monkeypatch.setattr(MoorConsoleEngine, "execute", slow_execute)
...
def test_console_cancel_stops_forked_agent_request_before_reporting(console_client, monkeypatch, blocking_provider):
    ...
    skill_dir = get_moor_home() / "skills" / "console-cancel-probe"
