"""Regression for #41403 — ``moor update`` must restart ALL macOS launchd gateways.
...
Sibling ``ai.moor.gateway-<profile>`` services kept running pre-update
...
class TestLaunchdGatewayLabelsForInstall:
    def test_labels_derive_from_this_installs_profiles(self, monkeypatch):
        ...
        the shared per-user LaunchAgents dir. A sandboxed MOOR_HOME (tests,
        ...
        assert launchd_gateway_labels_for_install() == [
            "ai.moor.gateway",
            "ai.moor.gateway-merit-ops",
            "ai.moor.gateway-tfl-wiki",
        ]

    def test_no_profiles_means_no_fleet(self, monkeypatch):
        monkeypatch.setattr(moor_cli.profiles, "list_profiles", lambda: [])
        assert launchd_gateway_labels_for_install() == []
...
class TestLocateLaunchdGatewayService:
    def test_domains_resolve_per_label_not_from_cache(self, monkeypatch):
        ...
        assert _locate_launchd_gateway_service("ai.moor.gateway-a") == (
            f"gui/{UID}",
            4242,
        )
        assert _locate_launchd_gateway_service("ai.moor.gateway-b") == (
            f"user/{UID}",
            4242,
        )
    ...
    def test_loaded_without_live_process(self, monkeypatch):
        ...
        assert _locate_launchd_gateway_service("ai.moor.gateway-x") == (
            f"gui/{UID}",
            None,
        )
    ...
    def test_not_loaded_in_either_domain(self, monkeypatch):
        ...
        assert _locate_launchd_gateway_service("ai.moor.gateway-x") == (None, None)
    ...
    def test_timeout_propagates_to_caller(self, monkeypatch):
        ...
        with pytest.raises(subprocess.TimeoutExpired):
            _locate_launchd_gateway_service("ai.moor.gateway-x")
...
class TestProbeLaunchdDomainForLabel:
    def test_unloaded_label_falls_back_to_managername(self, monkeypatch):
        ...
        monkeypatch.setattr(gw.subprocess, "run", fake_run)
        assert _probe_launchd_domain_for_label("ai.moor.gateway-x") == f"gui/{UID}"
    ...
    def test_unloaded_label_defaults_to_user_domain(self, monkeypatch):
        ...
        monkeypatch.setattr(gw.subprocess, "run", fake_run)
        assert _probe_launchd_domain_for_label("ai.moor.gateway-x") == f"user/{UID}"
...
class TestGetServicePidsScoping:
    def _wire(self, monkeypatch):
        ...
        monkeypatch.setattr(
            gw,
            "launchd_gateway_labels_for_install",
            lambda: ["ai.moor.gateway", "ai.moor.gateway-a", "ai.moor.gateway-b"],
        )
        located = {
            "ai.moor.gateway": (f"gui/{UID}", 100),
            "ai.moor.gateway-a": (f"gui/{UID}", 200),
            "ai.moor.gateway-b": (None, None),  # not bootstrapped
        }
        monkeypatch.setattr(
...
def _fleet(monkeypatch, tmp_path, *, current, labels, located,
    ...
    """Wire a fake launchd fleet through moor_cli.gateway seams.
...
class TestRestartMacosLaunchdGateways:
    def test_current_delegates_and_siblings_kickstart_in_own_domains(
        ...
        current = "ai.moor.gateway-merit-ops"
        rec = _fleet(
            monkeypatch,
            tmp_path,
            current=current,
            labels=["ai.moor.gateway", current, "ai.moor.gateway-user-scoped"],
            located={
                "ai.moor.gateway": (f"gui/{UID}", 100),
                current: (f"gui/{UID}", 200),
                "ai.moor.gateway-user-scoped": (f"user/{UID}", 300),
            },
        )
        restarted: list[str] = []
        failed: list[str] = []
        ...
        assert rec.kickstarts == [
            f"gui/{UID}/ai.moor.gateway",
            f"user/{UID}/ai.moor.gateway-user-scoped",
        ]
        assert rec.waits == [
            f"gui/{UID}/ai.moor.gateway",
            f"user/{UID}/ai.moor.gateway-user-scoped",
    ...
    def test_current_profile_without_plist_makes_no_launchctl_calls(
        ...
        still processed."""
        current = "ai.moor.gateway"
        rec = _fleet(
            monkeypatch,
            tmp_path,
            current=current,
            labels=[current, "ai.moor.gateway-a"],
            located={"ai.moor.gateway-a": (f"gui/{UID}", 200)},
            plist_exists=False,
        )
        restarted: list[str] = []
        failed: list[str] = []
        ...
        assert restarted == ["ai.moor.gateway-a"]
        assert failed == []
    ...
    def test_unbootstrapped_sibling_is_skipped_not_failed(
        ...
        rec = _fleet(
            ...
            tmp_path,
            current="ai.moor.gateway",
            labels=["ai.moor.gateway", "ai.moor.gateway-idle"],
            located={
                "ai.moor.gateway": (f"gui/{UID}", 100),
                "ai.moor.gateway-idle": (None, None),
            },
        )
        restarted: list[str] = []
        failed: list[str] = []
        ...
        assert rec.kickstarts == []
        assert restarted == ["ai.moor.gateway"]
        assert failed == []
    ...
    def test_loaded_but_not_running_sibling_is_kickstarted(
        ...
        rec = _fleet(
            ...
            tmp_path,
            current="ai.moor.gateway",
            labels=["ai.moor.gateway", "ai.moor.gateway-dormant"],
            located={
                "ai.moor.gateway": (f"gui/{UID}", 100),
                "ai.moor.gateway-dormant": (f"gui/{UID}", None),
            },
        )
        restarted: list[str] = []
        failed: list[str] = []
        ...
        assert rec.drains == []
        assert rec.kickstarts == [f"gui/{UID}/ai.moor.gateway-dormant"]
        assert restarted == ["ai.moor.gateway", "ai.moor.gateway-dormant"]
        assert failed == []
    ...
    def test_graceful_drain_with_keepalive_respawn_skips_kickstart(
        ...
        rec = _fleet(
            ...
            tmp_path,
            current="ai.moor.gateway",
            labels=["ai.moor.gateway", "ai.moor.gateway-a"],
            located={
                "ai.moor.gateway": (f"gui/{UID}", 100),
                "ai.moor.gateway-a": (f"gui/{UID}", 200),
            },
        ...
        assert rec.kickstarts == []
        assert rec.waits == [f"gui/{UID}/ai.moor.gateway-a"]
        assert restarted == ["ai.moor.gateway", "ai.moor.gateway-a"]
        assert failed == []
    ...
    def test_kickstart_failure_is_recorded_and_rest_continue(
        ...
        rec = _fleet(
            ...
            labels=[
                "ai.moor.gateway",
                "ai.moor.gateway-bad",
                "ai.moor.gateway-good",
            ],
            located={
                "ai.moor.gateway": (f"gui/{UID}", 100),
                "ai.moor.gateway-bad": (f"gui/{UID}", 200),
                "ai.moor.gateway-good": (f"gui/{UID}", 300),
            },
            kick_errors={
                "ai.moor.gateway-bad": subprocess.CalledProcessError(
        ...
        assert failed == ["ai.moor.gateway-bad"]
        assert rec.kickstarts == [f"gui/{UID}/ai.moor.gateway-good"]
        assert restarted == ["ai.moor.gateway", "ai.moor.gateway-good"]
    ...
    def test_timeout_during_discovery_is_failed_and_rest_continue(
        ...
        rec = _fleet(
            ...
            labels=[
                "ai.moor.gateway",
                "ai.moor.gateway-wedged",
                "ai.moor.gateway-after",
            ],
            located={
                "ai.moor.gateway": (f"gui/{UID}", 100),
                "ai.moor.gateway-wedged": subprocess.TimeoutExpired(
                    cmd=["launchctl", "print"], timeout=5
                ),
                "ai.moor.gateway-after": (f"gui/{UID}", 300),
            },
        )
        restarted: list[str] = []
        failed: list[str] = []
        ...
        assert failed == ["ai.moor.gateway-wedged"]
        assert rec.kickstarts == [f"gui/{UID}/ai.moor.gateway-after"]
        assert restarted == ["ai.moor.gateway", "ai.moor.gateway-after"]
    ...
    def test_timeout_during_kickstart_is_failed_and_rest_continue(
        ...
        rec = _fleet(
            ...
            located={
                "ai.moor.gateway": (f"gui/{UID}", 100),
                "ai.moor.gateway-wedged": (f"gui/{UID}", 200),
                "ai.moor.gateway-after": (f"gui/{UID}", 300),
            },
            kick_errors={
                "ai.moor.gateway-wedged": subprocess.TimeoutExpired(
    ...
    def test_sibling_that_never_comes_back_is_failed(self, monkeypatch, tmp_path):
        rec = _fleet(
            ...
            tmp_path,
            current="ai.moor.gateway",
            labels=["ai.moor.gateway", "ai.moor.gateway-zombie"],
            located={
                "ai.moor.gateway": (f"gui/{UID}", 100),
                "ai.moor.gateway-zombie": (f"gui/{UID}", 200),
            },
            wait_results={"ai.moor.gateway-zombie": False},
        )
        restarted: list[str] = []
        failed: list[str] = []
        ...
        assert restarted == ["ai.moor.gateway"]
        assert failed == ["ai.moor.gateway-zombie"]


def _write_launchd_plist(agents_dir: Path, label: str, *, argv=(), moor_home=None, raw=None):
    ...
    if moor_home is not None:
        data["EnvironmentVariables"] = {"MOOR_HOME": str(moor_home)}
...
class TestLegacyLaunchdLabelsForInstall:
    ...
    from the plist's pinned MOOR_HOME, fail-closed, so another install's fleet stays untouched
    (the #41403 boundary) and a sandboxed MOOR_HOME never enumerates the account's real units."""
    ...
    def test_only_units_pinned_to_this_installs_homes_are_credited(self, monkeypatch, tmp_path):
        ...
        _write_launchd_plist(agents, "ai.moor.gateway-398559f7", argv=[venv_python], moor_home=root / "profiles" / "gopod")
        _write_launchd_plist(agents, "ai.moor.gateway-1a2b3c4d", argv=[venv_python], moor_home=root)
        _write_launchd_plist(agents, "ai.moor.gateway-gopod", argv=[venv_python], moor_home=root / "profiles" / "gopod")
        # Another install's unit: same venv on disk, but its home lives under a different root.
        _write_launchd_plist(agents, "ai.moor.gateway-5db8084b", argv=[venv_python], moor_home=tmp_path / "other-root" / "profiles" / "gopod")
        _write_launchd_plist(agents, "ai.moor.gateway-nohome", argv=[venv_python])  # no pinned home: unattributable
        _write_launchd_plist(agents, "ai.moor.gateway-broken", raw="not a plist at all")
        assert gw.legacy_launchd_labels_for_install(exclude={"ai.moor.gateway-gopod", "ai.moor.gateway"}) == [
            "ai.moor.gateway-1a2b3c4d", "ai.moor.gateway-398559f7",
        ]
...
class TestRestartMacosLaunchdGatewaysLegacyUnits:
    def test_legacy_labelled_sibling_is_restarted_like_any_other(self, monkeypatch, tmp_path, capsys):
        ...
        current = "ai.moor.gateway"
        legacy = "ai.moor.gateway-398559f7"
        rec = _fleet(
            monkeypatch,
            tmp_path,
            current=current,
            labels=[current, "ai.moor.gateway-gopod"],
            legacy_labels=[legacy],
            located={
                current: (f"gui/{UID}", 100),
                "ai.moor.gateway-gopod": (f"gui/{UID}", 200),
        ...
        assert rec.kickstarts == [
            f"gui/{UID}/ai.moor.gateway-gopod",
            f"user/{UID}/{legacy}",
        ]
        assert restarted == [current, "ai.moor.gateway-gopod", legacy]
        assert failed == []
...
class TestWaitForLaunchdServicePid:
    def test_returns_true_once_pid_changes(self, monkeypatch):
        ...
        assert gw._wait_for_launchd_service_pid(
            "ai.moor.gateway-x", old_pid=200, timeout=5.0, domain=f"gui/{UID}"
        )
    ...
    def test_returns_false_when_pid_never_changes(self, monkeypatch):
        ...
        assert not gw._wait_for_launchd_service_pid(
            "ai.moor.gateway-x", old_pid=200, timeout=3.0, domain=f"gui/{UID}"
        )
