"""Tests for moor_cli.tools_config platform tool persistence."""
...
from moor_cli.moor_account import MoorPortalAccountInfo, MoorToolAccessInfo
...
def test_all_invalid_platform_toolsets_logs_runtime_warning(caplog):
    ...
    (e.g. 'moor' instead of 'moor-cli') must warn at resolve time so an
    already-corrupted config is caught at runtime, not just during migration."""
    import moor_cli.tools_config as _tc
    ...
    config = {"platform_toolsets": {"cli": ["moor"]}}

    with caplog.at_level(logging.WARNING, logger="moor_cli.tools_config"):
        _get_platform_tools(config, "cli")

    warnings = [r.getMessage() for r in caplog.records if r.levelno >= logging.WARNING]
    assert any("#38798" in m and "moor" in m for m in warnings), warnings
...
def test_valid_platform_toolsets_no_runtime_warning(caplog):
    ...
    config = {"platform_toolsets": {"cli": ["moor-cli"]}}
...
def test_partially_valid_platform_toolsets_no_runtime_warning(caplog):
    ...
    config = {"platform_toolsets": {"cli": ["moor-cli", "bogus"]}}
...
def test_enable_on_string_platform_toolsets_keeps_listed_entries():
    """#115866: `moor tools enable` must operate on the selection a list-literal
...
def test_malformed_list_string_platform_toolsets_warns_then_falls_back(caplog):
    ...
    with caplog.at_level("WARNING", logger="moor_cli.tools_config"):
        enabled = _get_platform_tools(config, "cli", include_default_mcp_servers=False)
...
# via the platform's native composite. The composite ``moor-discord``
...
def test_discord_toolsets_do_not_leak_to_other_platforms():
    ...
    config = {"platform_toolsets": {"telegram": ["moor-telegram", "discord"]}}
...
def test_save_platform_tools_preserves_mcp_server_names():
    ...
    Regression test for https://github.com/thisismamad-n/Moor/issues/1247
    """
    config = {
        "platform_toolsets": {
...
def test_first_install_moor_auto_configures_video_gen(monkeypatch):
    """When a Moor subscriber checks video_gen in the toolset checklist,
    apply_moor_managed_defaults must write video_gen.provider and
    ...
    monkeypatch.setattr(
        "moor_cli.tools_config._prompt_toolset_checklist",
        lambda *args, **kwargs: {"video_gen"},
    )
    monkeypatch.setattr("moor_cli.tools_config.save_config", lambda config: None)
    monkeypatch.setattr(
        "moor_cli.tools_config._get_enabled_platforms",
        lambda: ["cli"],
    )
    monkeypatch.setattr(
        "moor_cli.moor_subscription.get_moor_portal_account_info",
        lambda *args, **kwargs: MoorPortalAccountInfo(
            logged_in=True,
            source="jwt",
            fresh=False,
    ...
    configured = []
    monkeypatch.setattr(
        "moor_cli.tools_config._configure_toolset",
...
class TestPlatformToolsetConsistency:
    ...
    def test_all_platforms_have_toolset_definitions(self):
        ...
        from moor_cli.tools_config import PLATFORMS
    ...
    def test_gateway_toolset_includes_all_messaging_platforms(self):
        """moor-gateway includes list should cover all messaging platforms."""
        from moor_cli.tools_config import PLATFORMS
        from toolsets import TOOLSETS

        gateway_includes = set(TOOLSETS["moor-gateway"]["includes"])
    ...
    def test_skills_config_covers_tools_config_platforms(self):
        ...
        from moor_cli.tools_config import PLATFORMS as TOOLS_PLATFORMS
        from moor_cli.skills_config import PLATFORMS as SKILLS_PLATFORMS
...
def test_numeric_mcp_server_name_does_not_crash_sorted():
    ...
    Regression test for https://github.com/thisismamad-n/Moor/issues/6901
    """
    config = {
...
class TestAgentBrowserPostSetup:
    ...
    def _stub_browser_use_install(self):
        with patch("moor_cli.tools_config_post_setup._ensure_browser_use_cli") as stub:
            yield stub
    ...
    def test_install_failure_reports_error(self, failure):
        ...
        ), patch("moor_cli.tools_config_post_setup._print_warning") as warn, patch(
            "moor_cli.tools_config_post_setup._print_info"
        ) as info:
            _run_post_setup("agent_browser")

        assert any(str(error) in c.args[0] for c in warn.call_args_list)
        assert any("moor tools post-setup agent_browser" in c.args[0] for c in info.call_args_list)
...
class TestBrowserUseCliInstalledForAllNonCamofoxBackends:
    ...
    def test_browser_post_setup_attempts_cli_install(self, key):
        with patch("moor_cli.tools_config_post_setup._ensure_browser_use_cli") as ensure, patch(
    ...
    def test_camofox_post_setup_never_touches_browser_use(self):
        ...
        the CLI in."""
        with patch("moor_cli.tools_config_post_setup._ensure_browser_use_cli") as ensure, patch(
            "moor_constants.find_node_executable", return_value=None
    ...
    def test_ensure_helper_always_delegates_to_install_cli(self):
        ...
        $MOOR_HOME/bin when only side installs exist."""
        with patch(
            "moor_cli.tools_config_post_setup.shutil.which", return_value="/usr/bin/browser-use"
        ), patch(
            "tools.browser_use_cli.install_cli",
            return_value=(True, "browser-use CLI already installed (/managed/bin/browser-use)"),
        ) as install:
            from moor_cli.tools_config import _ensure_browser_use_cli
    ...
    def test_ensure_helper_install_failure_is_non_fatal(self):
        ...
        with patch(
            "moor_cli.tools_config_post_setup.shutil.which", return_value=None
        ), patch(
            "tools.browser_use_cli.install_cli",
            return_value=(False, "`uv tool install browser-use` failed:\nboom"),
        ), patch("moor_cli.tools_config_post_setup._print_warning") as warn:
...
class TestImagegenBackendRegistry:
    ...
    def test_image_gen_providers_tagged_with_registered_backend(self):
        ...
        from moor_cli.tools_config import IMAGEGEN_BACKENDS, TOOL_CATEGORIES
...
class TestImagegenModelPicker:
    ...
    def test_picker_writes_chosen_model_to_config(self):
        from moor_cli.tools_config import _configure_imagegen_model
        config = {}
        # Force _prompt_choice to pick index 1 (second-in-ordered-list).
        with patch("moor_cli.tools_config._prompt_choice", return_value=1):
            _configure_imagegen_model("fal", config)
        # ordered[0] == current (default klein), ordered[1] == first non-default
        from moor_cli.tools_config import IMAGEGEN_BACKENDS
    ...
    def test_picker_with_gpt_image_does_not_prompt_quality(self):
        ...
        config = {}
        with patch("moor_cli.tools_config._prompt_choice", side_effect=fake_prompt):
            _configure_imagegen_model("fal", config)
    ...
    def test_picker_repairs_corrupt_config_section(self):
        ...
        from moor_cli.tools_config import IMAGEGEN_BACKENDS, _configure_imagegen_model
        config = {"image_gen": "some-garbage-string"}
        with patch("moor_cli.tools_config._prompt_choice", return_value=0):
            _configure_imagegen_model("fal", config)
...
def test_get_effective_configurable_toolsets_dedupes_bundled_plugins():
    ...
    them twice — otherwise `moor tools` → "reconfigure existing" shows
    the same toolset two rows in a row.
    """
    from moor_cli.tools_config import _get_effective_configurable_toolsets
...
def test_visible_providers_reuses_logged_out_feature_snapshot(monkeypatch):
    ...
    assert any(
        provider.get("managed_moor_feature") == "image_gen"
...
def test_visible_providers_reuses_pool_video_feature_snapshot(monkeypatch):
    ...
    account = MoorPortalAccountInfo(
        ...
        tool_access=MoorToolAccessInfo(
            enabled=True,
    ...
    assert not any(
        provider.get("managed_moor_feature") == "video_gen"
        for provider in providers
    )
...
#
# FAL, Krea and Portal models all live behind the single "Moor Subscription" row; the stored
# model id picks the gateway. Before, the Portal plugin rendered its own row that also wrote
# `provider: moor`, so two rows read active at once and the Portal pick generated on FAL.


def _managed_image_row() -> dict:
    return next(p for p in TOOL_CATEGORIES["image_gen"]["providers"] if p.get("managed_moor_feature") == "image_gen")


def test_exactly_one_image_row_is_active_for_a_managed_selection(monkeypatch):
    import moor_cli.tools_config as tools_config
    from moor_cli.tools_config_providers import _plugin_image_gen_providers

    monkeypatch.setattr(
        tools_config, "get_moor_subscription_features",
        lambda config, **kwargs: SimpleNamespace(features={"image_gen": SimpleNamespace(managed_by_moor=True)}),
    )
    ...
    for model in ("fal-ai/flux-2/klein/9b", "krea-2-medium", "google/gemini-3-pro-image"):
        config = {"image_gen": {"provider": "moor", "model": model}}
...
def test_gui_model_catalog_for_the_managed_row_spans_every_managed_gateway(monkeypatch):
    import moor_cli.tools_config as tools_config
    from moor_cli.web_routers.tools import _resolve_toolset_model_plugin, _toolset_model_catalog
    from plugins.image_gen.krea import KREA_MODEL_IDS
    from tools.image_generation_catalog import FAL_MODELS

    paid = MoorPortalAccountInfo(logged_in=True, source="jwt", fresh=False, paid_service_access=True)
    monkeypatch.setattr(
        tools_config, "get_moor_subscription_features",
...
def test_pool_only_account_is_offered_fal_models_only(monkeypatch):
    import moor_cli.tools_config as tools_config
    from moor_cli.tools_config_providers import _managed_image_catalog

    pool = MoorPortalAccountInfo(
        logged_in=True, source="jwt", fresh=False, paid_service_access=False,
        tool_access=MoorToolAccessInfo(enabled=True, coverage={"fal": True, "krea": False}))
    monkeypatch.setattr(
        tools_config, "get_moor_subscription_features",
...
# ── Toolsets that shipped after a platform's last `moor tools` save ────────
#
# Saving the picker (or one toggle in the desktop Toolsets UI) replaces a
# platform's composite (``[moor-cli]``) with a frozen explicit list, and
...
def test_agent_disabled_toolsets_json_array_string_form_still_wins():
    ...
    `moor config set agent.disabled_toolsets '["memory"]'`). It must be
...
def test_disabled_composite_debugging_prunes_constituent_platform_toolsets():
    ...
    toolsets on ``moor tools --summary``, not only strip them at runtime."""
    config = {
        "platform_toolsets": {"cli": ["moor-cli"]},
...
def test_platforms_whose_composite_excludes_it_are_left_narrow():
    ...
    composite (moor-acp, moor-webhook) that never carried the toolset."""
    from toolsets import TOOLSETS, resolve_toolset

    narrow = [
        platform
        for platform in ("acp", "webhook")
        if f"moor-{platform}" in TOOLSETS
        and not any(
            set(resolve_toolset(ts, include_registry=False))
            <= set(resolve_toolset(f"moor-{platform}"))
...
def test_explicit_plugin_toolset_admitted_in_platform_toolsets(monkeypatch):
    ...
    ``moor-cli``), it MUST be admitted as a configurable key instead of
    ...
    import moor_cli.tools_config as _tc_mod
    ...
    # standard composite — exactly the "I want moor-cli AND a2a in my CLI
    # session" config the issue's user was trying to write.
    config = {"platform_toolsets": {"cli": ["moor-cli", "dplat_client"]}}
...
def test_explicit_plugin_toolset_admitted_against_real_a2a_plugin(monkeypatch):
    ...
    a real config like ``platform_toolsets.cli: [moor-cli, a2a]``, ``a2a``
    ...
    config = {"platform_toolsets": {"cli": ["moor-cli", "a2a"]}}
    enabled = _get_platform_tools(config, "cli")
...
class TestLightpandaPostSetup:
    ...
    def test_reports_binary_when_found(self, _stub_browser_use_install):
        ...
        with patch("tools.browser_lightpanda.find_lightpanda_binary", return_value="/opt/lightpanda"), \
             patch("moor_cli.tools_config_post_setup._print_success") as ok, \
             patch("moor_cli.tools_config_post_setup._print_warning") as warn:
    ...
    def test_prints_install_hint_when_missing(self):
        ...
        with patch("tools.browser_lightpanda.find_lightpanda_binary", return_value=None), \
             patch("moor_cli.tools_config_post_setup._print_warning") as warn, \
             patch("moor_cli.tools_config_post_setup._print_info") as info:
            _run_post_setup("lightpanda")
