...
class TestAgentBrowserPostSetup:
    ...
    tools.browser_tool_install._find_agent_browser (PATH -> Homebrew/moor-managed
    ...
    def test_warns_when_neither_npx_nor_agent_browser_on_path(self):
        with patch("shutil.which", return_value=None), patch(
            "subprocess.run"
        ) as run, patch("moor_cli.tools_config_post_setup._print_warning") as warn:
    ...
    def test_chromium_already_installed_skips_subprocess(self):
        with patch("shutil.which", return_value="/usr/bin/npx"), patch(
            ...
            "moor_cli.tools_config_post_setup._print_success"
        ) as success:
            _run_post_setup("agent_browser")
    ...
    def test_docker_with_missing_chromium_warns_instead_of_installing(self):
        with patch("shutil.which", return_value="/usr/bin/npx"), patch(
            ...
            "moor_cli.tools_config_post_setup._print_warning"
        ) as warn:
            _run_post_setup("agent_browser")
    ...
    def test_installs_chromium_via_npx_resolved_only_through_extended_path(self):
        ...
        moor_npx = "/home/user/.moor/node/bin/npx"
        with patch("shutil.which", return_value=None), patch(
            "subprocess.run"
        ) as run, patch(
            "tools.browser_tool_install._chromium_installed", return_value=False
            ...
            "tools.browser_tool_install._resolve_npx_bin", return_value=moor_npx
        ), patch(
            "moor_cli.tools_config_post_setup._print_success"
        ):
            run.return_value = SimpleNamespace(returncode=0, stdout="", stderr="")
        ...
        run.assert_called_once()
        assert run.call_args.args[0] == [
            moor_npx, "--ignore-scripts", "-y", AGENT_BROWSER_NPX_SPEC, "install", "--with-deps",
        ]
...
class TestImagegenBackendRegistry:
    ...
    def test_image_gen_providers_tagged_with_fal_backend(self):
        """Both Moor Subscription and FAL.ai providers must carry the
...
# ---------------------------------------------------------------------------
# Inline Moor Portal login gate on managed-provider selection
# ---------------------------------------------------------------------------
...
#    reported as added/removed by `moor tools` ──────────────────────────




def test_kanban_not_reported_as_removed_in_diff():
    """Reproduces the false-signal bug: `moor tools` printed ``- kanban``
...
# Ready for every zero-env-var provider row, including logged-out Moor
...
def _fake_features(*, logged_in: bool, paid: bool = True):
    account = (
        ...
        if logged_in
        else MoorPortalAccountInfo(
            logged_in=False, source="none", fresh=False, paid_service_access=None
        )
    )
    return SimpleNamespace(moor_auth_present=logged_in, account_info=account)
...
# `moor tools post-setup <key>` child. On Windows each console child (npm,
