"""Tests for the resolve_moor_access_token startup-burst memo (PR #66016).
...
def test_repeat_calls_within_ttl_hit_memo(monkeypatch, tmp_path):
    ...
    first = auth.resolve_moor_access_token()
    second = auth.resolve_moor_access_token()
    third = auth.resolve_moor_access_token()
...
def test_insecure_callers_bypass_memo(monkeypatch, tmp_path):
    ...
    auth.resolve_moor_access_token()
    auth.resolve_moor_access_token(insecure=True)
...
def test_memo_does_not_leak_across_multiplex_profile_contexts(tmp_path):
    ...
    moor_constants.set_moor_home_override (gateway/run.py,
    tui_gateway/server.py), not the MOOR_HOME env var — the memo must key
    on that same resolved home, or one profile's context can read another
    profile's already-cached Moor access token for up to the TTL window.
    """
    ...
    reset_token = moor_constants.set_moor_home_override(str(profile_a))
    try:
        token_a = auth.resolve_moor_access_token()
    finally:
        moor_constants.reset_moor_home_override(reset_token)

    # Still well within the 5s TTL — this is exactly the race window: profile
    # B's context calls resolve_moor_access_token() shortly after profile A's.
    reset_token = moor_constants.set_moor_home_override(str(profile_b))
    try:
        token_b = auth.resolve_moor_access_token()
    finally:
        moor_constants.reset_moor_home_override(reset_token)
