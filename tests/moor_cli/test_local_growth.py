...
def test_overrides_roundtrip_and_clear(moor_home):
    from moor_cli.local_runtime.growth import (
...
def test_corrupt_overrides_read_as_empty(moor_home):
    from moor_cli.local_runtime.growth import (
        load_window_overrides,
...
def test_growth_declines_foreign_endpoints(moor_home):
...
def test_occupancy_confirmed_skips_gate_one():
    ...
    # Moor' threshold (e.g. 80% of window) can sit BELOW the ladder's 85%
...
def _tiny_profile(model_id: str):
    from moor_cli.local_runtime.estimator import LayerKind, ModelProfile
...
def test_preset_generation_for_catalog_model_with_mmproj(moor_home, tmp_path, monkeypatch):
    ...
    import moor_cli.local_runtime.presets as presets_mod
...
def test_preset_restores_grown_window_capped_at_native(moor_home, tmp_path, monkeypatch):
...
def test_preset_ignores_override_below_launch_window(moor_home, tmp_path, monkeypatch):
...
def test_preset_restores_grown_window_midladder(moor_home, tmp_path, monkeypatch):
    """The real growth shape: launch at a lower rung, override to a middle
    rung -> the preset window follows the override."""
    import moor_cli.local_runtime.presets as presets_mod

    from moor_cli.local_runtime.estimator import HardwareBudget, LayerKind, ModelProfile
    from moor_cli.local_runtime.growth import save_window_override
