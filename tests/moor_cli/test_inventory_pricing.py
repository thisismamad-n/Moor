...
columns + Free/Pro badges and gate paid models on free Moor accounts, the
same way the `moor model` CLI picker does.
"""

from threading import Event
from time import monotonic

import moor_cli.inventory as inv
...
def _patch_pricing(monkeypatch, *, free_tier, pricing, unavailable=None):
    ...
    monkeypatch.setattr(models_mod, "check_moor_free_tier", lambda *, force_fresh=False: free_tier)
    monkeypatch.setattr(
...
def test_apply_pricing_free_models_get_flat_100_percent_sale(monkeypatch):
    ...
    rows = [{"slug": "moor", "models": ["a/free", "b/natively-free"]}]
    inv._apply_pricing(rows)
...
def test_cold_moor_entitlement_keeps_models_unselectable(monkeypatch):
    ...
    monkeypatch.setattr(models_mod, "get_cached_moor_free_tier", lambda: None)
    rows = [{"slug": "moor", "models": ["free/model", "paid/model"]}]
    ...
    rows = [{"slug": "moor", "models": ["m"], "warning": "paste NOUS_API_KEY to activate"}]
    inv._apply_pricing(rows, cached_only=True)
...
def test_prewarm_preserves_context_and_runs_once_per_profile(tmp_path, monkeypatch):
    ...
    def capture_context(_rows):
        ...
        observed[label] = (moor_home_key(), dict(scope))
        started[label].set()
    ...
    try:
        ...
        assert observed["a"] == (
            moor_home_key(tmp_path / "a"),
            {"PROFILE_MARKER": "a"},
        )
        assert observed["b"] == (
            moor_home_key(tmp_path / "b"),
            {"PROFILE_MARKER": "b"},
        )
    finally:
        release.set()
...
def test_prewarm_endpoint_rotation_starts_a_new_worker(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        inv,
        "_apply_pricing",
        lambda _rows: models_pricing.get_pricing_for_provider("moor"),
    )

    token = set_moor_home_override(str(tmp_path / "profile"))
    threads = []
    try:
        threads.append(
            inv._prewarm_pricing_async(
                [{"slug": "moor", "models": ["a/model"]}],
...
def test_prewarm_moor_rotation_when_another_provider_is_current(tmp_path, monkeypatch):
    """Moor endpoint identity must not depend on Moor being selected."""
    from moor_constants import (
        reset_moor_home_override,
        set_moor_home_override,
    )
    ...
    monkeypatch.setattr(models_pricing, "_pricing_cache_retry_after", {})
    monkeypatch.setattr(models_pricing, "_pricing_provider_cache_keys", {})
    monkeypatch.setattr(models_pricing, "get_cached_moor_inference_base_url",
        lambda: active_endpoint["value"],
    )
    monkeypatch.setattr(models_pricing, "_resolve_moor_pricing_credentials",
        lambda: ("", active_endpoint["value"]),
