...
board into `MOOR_KANBAN_BOARD` at boot so subprocess shell-outs (e.g.
`moor kanban …`) inherit the same board the in-process kanban tools resolve.
Without this, a concurrent `moor kanban boards switch` from another session
...
def _isolate_kanban_board_env():
    """Snapshot `MOOR_KANBAN_BOARD` and restore it after the test.
    ...
    """
    prev = os.environ.get("MOOR_KANBAN_BOARD")
...
def test_pin_does_not_overwrite_existing_env(monkeypatch):
    ...
    assert main_mod.os.environ.get("MOOR_KANBAN_BOARD") == "preset"
