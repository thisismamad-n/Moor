...
The Windows ``moor`` command is a launcher derived from the venv console
script. Its canonical home is the managed binary dir ``MOOR_HOME\\bin`` —
OUTSIDE the git checkout — because the earlier in-checkout home
(``moor-agent\\bin``) was swept by ``moor update``'s autostash
(``git stash push --include-untracked``) and, with the desktop updater's
``--keep-stash``, never restored: ``moor`` stopped resolving in every new
...
existing install's PATH to the canonical layout from the ``moor update``
tail. Platform verdict, PATH values, and registry I/O are injected
parameters (same pattern as ``moor_constants.venv_bin_dir``), so these
...
def _make_managed(tmp_path, monkeypatch, *, relocatable: bool = False):
    """Fake managed layout: MOOR_HOME/moor-agent/venv/Scripts + launchers."""
    home = tmp_path / "moor"
...
# ---------------------------------------------------------------------------
# migrate_windows_bin_path — the `moor update` tail migration
# ---------------------------------------------------------------------------
...
def test_migration_moves_path_to_home_bin_and_strips_legacy(managed_install):
    ...
    (root / "bin").mkdir()
    (root / "bin" / "moor.exe").write_bytes(b"legacy copy")
    ...
    assert (root / "bin" / "moor.exe").read_bytes() == b"legacy copy"
...
def test_normalize_windows_path_equivalences():
    assert (
        _normalize_windows_path(r"C:\Users\Me\AppData\Local\moor\bin")
        == _normalize_windows_path("c:/users/me/appdata/local/MOOR/BIN/")
    )
...
def test_repo_gitignores_the_legacy_bin_dir():
    ...
    launchers at ``<checkout>/bin``. ``moor update`` autostashes with
    ...
    assert result.returncode == 0, (
        "bin/moor.exe is not gitignored — moor update's autostash "
