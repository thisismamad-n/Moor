"""Regression tests for the OAuth dispatcher in moor_cli.web_server.
...
Moor must not mint subscription OAuth tokens from an unattended HTTP
...
import moor_cli.web_routers.oauth as _rt_oauth
import moor_cli.web_server_oauth as _web_server_oauth
...
def test_oauth_provider_status_uses_profile_query(tmp_path, monkeypatch):
    from moor_cli import web_server as ws
    ...
    fake_catalog = ({
        ...
        "name": "Fake OAuth",
        "flow": "pkce",
        "cli_command": "moor auth add fake-oauth",
...
def test_codex_dashboard_start_rewords_device_authorization_error(monkeypatch):
    ...
    try:
        ...
        assert "moor auth" not in detail
    finally:
...
def test_moor_dashboard_poller_preserves_effective_scope_when_token_omits_scope(monkeypatch):
    ...
    def fake_refresh_moor_oauth_from_state(state, **kwargs):
    ...
    monkeypatch.setattr(
        ...
        fake_refresh_moor_oauth_from_state,
    )
    monkeypatch.setattr(auth_mod, "persist_moor_credentials", lambda state: None)

    try:
        _web_server_oauth._moor_poller(session_id)
...
def test_anthropic_dashboard_oauth_is_removed_and_external():
    ...
    assert providers["anthropic"]["cli_command"] == "moor auth add anthropic"
...
def test_accounts_offers_every_oauth_provider_from_catalog():
    ...
    `moor model` universe) must be offered by /api/providers/oauth. This keeps
    ...
    for d in provider_catalog():
        if d.tab == "accounts":
            assert d.slug in offered, (
                f"{d.slug} is an accounts-tab provider in `moor model` but is "
...
def test_oauth_catalog_marks_external_providers_not_disconnectable():
    """External CLI credentials are visible in Accounts but cannot be removed by Moor."""
    resp = client.get("/api/providers/oauth", headers=HEADERS)
    assert resp.status_code == 200, resp.text
    providers = {p["id"]: p for p in resp.json()["providers"]}
...
def test_xai_dashboard_poller_seeds_single_entry_and_clears_suppression(tmp_path, monkeypatch):
    ...
    # Prior `moor auth remove xai-oauth` left the source suppressed.
...
def test_status_falls_through_to_generic_dispatcher_for_catalog_only_provider():
    ...
    logged-out; now they dispatch to moor_cli.auth.get_auth_status (the
    ...
    with patch("moor_cli.auth.get_auth_status", return_value=fake_status):
