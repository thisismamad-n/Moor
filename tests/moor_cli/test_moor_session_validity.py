"""Tests for the local-only Moor session classifier exposed on /api/status."""
...
# ── get_moor_auth_status_local — refresh-free display snapshot ──
...
def test_local_status_not_logged_in_after_terminal_quarantine(monkeypatch):
    ...
    status = auth.get_moor_auth_status_local()
...
def test_local_status_repeated_polling_never_uses_live_auth(monkeypatch):
    ...
    assert all(
        auth.get_moor_auth_status_local()["logged_in"] for _ in range(10)
    )
