...
import pytest
import yaml
from moor_cli import main_provider_setup
...
def test_aux_pickers_route_through_the_shared_substrate(configured_home):
    ...
    with (
        patch("moor_cli.model_switch.list_authenticated_providers", _direct),
        patch("moor_cli.inventory.build_aux_picker_rows", _substrate),
        patch("moor_cli.main_provider_setup._prompt_provider_choice", return_value=None),
    ):
    ...
    assert direct_calls == [], (
        ...
        "route through moor_cli.inventory.build_aux_picker_rows so custom "
