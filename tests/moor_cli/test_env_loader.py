...
def test_recovered_update_retry_skips_external_secret_sources(tmp_path, monkeypatch):
    ...
    loaded = load_moor_dotenv(moor_home=home)
...
def test_utf32_le_bom_leaves_file_untouched(tmp_path, caplog):
    ...
    from moor_cli.env_loader import _sanitize_env_file_if_needed

    env_file = tmp_path / ".env"
    content = "MOOR_TEST_KEY=hello_utf32\nSECOND_KEY=world\n"
    raw = codecs.BOM_UTF32_LE + content.encode("utf-32-le")
    env_file.write_bytes(raw)

    with caplog.at_level(logging.WARNING, logger="moor_cli.env_loader"):
...
def test_known_keys_absent_from_user_env_are_cleared(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("MOOR_ACP_AUTH_METHOD", "cursor_login")
    ...
    # MOOR_ACP_AUTH_METHOD and COPILOT_CLI_PATH are NOT in the profile .env → cleared
    assert "MOOR_ACP_AUTH_METHOD" not in os.environ
...
def test_empty_assignment_in_user_env_is_preserved(tmp_path, monkeypatch):
    ...
    Empty ``MOOR_ACP_AUTH_METHOD=`` tells the ACP adapter to skip
    ...
    home = tmp_path / "moor"
    home.mkdir()
    (home / ".env").write_text("MOOR_ACP_AUTH_METHOD=\n", encoding="utf-8")
    ...
    assert "MOOR_ACP_AUTH_METHOD" in os.environ
    assert os.environ["MOOR_ACP_AUTH_METHOD"] == ""
    # COPILOT_CLI_PATH is absent from .env → cleared
    assert "COPILOT_CLI_PATH" not in os.environ


def test_no_user_env_does_not_clear_anything(tmp_path, monkeypatch):
    """When no profile .env exists (bare profile), load_moor_dotenv must not
    ...
    assert os.getenv("MOOR_ACP_AUTH_METHOD") == "cursor_login"
...
def test_known_key_explicitly_set_in_user_env_is_kept(tmp_path, monkeypatch):
    ...
    home.mkdir()
    (home / ".env").write_text(
        "MOOR_ACP_AUTH_METHOD=claude_code_cli\n", encoding="utf-8"
    )
    ...
    assert os.getenv("MOOR_ACP_AUTH_METHOD") == "claude_code_cli"
...
def test_export_prefixed_known_key_in_user_env_is_kept(tmp_path, monkeypatch):
    ...
    home.mkdir()
    (home / ".env").write_text(
        "export MOOR_ACP_AUTH_METHOD=claude_code_cli\n", encoding="utf-8"
    )
    monkeypatch.setenv("MOOR_ACP_AUTH_METHOD", "cursor_login")
    load_moor_dotenv(moor_home=home)
    assert os.getenv("MOOR_ACP_AUTH_METHOD") == "claude_code_cli"
...
def test_cleanup_scope_is_the_profile_managed_set():
    ...
    """
    from moor_cli.env_loader import _PROFILE_MANAGED_ENV_KEYS
...
#
# load_moor_dotenv loads .env with override=True, so a stale
...
# the shared moor_cli.config.apply_terminal_config_to_env helper.
# ---------------------------------------------------------------------------
...
def _seed_terminal_home(tmp_path, monkeypatch, *, config_yaml=None, env_text=None):
    ...
    # The bridge is scoped to the process MOOR_HOME (a different profile's
...
def test_config_yaml_terminal_backend_overrides_stale_env(tmp_path, monkeypatch):
    """Regression for #29186: a leftover TERMINAL_ENV=docker in ~/.moor/.env
...
def test_other_profile_home_does_not_bridge_process_config(tmp_path, monkeypatch):
    ...
    load_moor_dotenv(moor_home=other_home)
...
def test_parent_injected_dashboard_session_token_survives_dotenv(tmp_path, monkeypatch):
    """A parent that spawns `moor dashboard` mints MOOR_DASHBOARD_SESSION_TOKEN and keeps it for
    its own /api probes; a persisted token in ~/.moor/.env must not replace it, or the parent gets
    HTTP 401 from its own child (#115955). Ordinary keys keep the documented .env-wins precedence."""
    home = tmp_path / "moor"
    home.mkdir()
    (home / ".env").write_text(
        "MOOR_DASHBOARD_SESSION_TOKEN=persisted-token\nMOOR_DASHBOARD_PUBLIC_URL=http://127.0.0.1:1\n",
        encoding="utf-8",
    )
    monkeypatch.setenv("MOOR_DASHBOARD_SESSION_TOKEN", "link-token")
    monkeypatch.setenv("MOOR_DASHBOARD_PUBLIC_URL", "http://127.0.0.1:43123")

    load_moor_dotenv(moor_home=home)
    assert os.environ["MOOR_DASHBOARD_SESSION_TOKEN"] == "link-token"
    assert os.environ["MOOR_DASHBOARD_PUBLIC_URL"] == "http://127.0.0.1:1"  # control: .env still wins
...
def test_dotenv_published_dashboard_session_token_still_reloads(tmp_path, monkeypatch):
    ...
    (home / ".env").write_text("MOOR_DASHBOARD_SESSION_TOKEN=first\n", encoding="utf-8")
    load_moor_dotenv(moor_home=home)
    assert os.environ["MOOR_DASHBOARD_SESSION_TOKEN"] == "first"

    (home / ".env").write_text("MOOR_DASHBOARD_SESSION_TOKEN=second\n", encoding="utf-8")
    load_moor_dotenv(moor_home=home)
    assert os.environ["MOOR_DASHBOARD_SESSION_TOKEN"] == "second"
