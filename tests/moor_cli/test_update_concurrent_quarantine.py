...
# ---------------------------------------------------------------------------
# _quarantine_running_moor_exe — retry + reboot-deferred fallback
# ---------------------------------------------------------------------------
...
def test_quarantine_succeeds_first_attempt(_winp, tmp_path):
    ...
    pairs = cli_main._quarantine_running_moor_exe(tmp_path)
...
def test_quarantine_falls_back_to_reboot_schedule(_winp, tmp_path, capsys, monkeypatch):
    ...
    monkeypatch.setattr(cli_main, "_moor_exe_shims", lambda d: [shim])
