...
def _complete_stub_login(client) -> None:
    ...
    client's cookie jar holds ``moor_session_at`` / ``moor_session_rt``
...
class _UnreachableProvider(StubAuthProvider):
    ...
    Models the real-world bug — a self-hosted-OIDC session hits the ``moor``
    provider first, which tries to reach Moor Portal's JWKS; if that's
    unreachable ``moor`` raises ProviderError. The gate must keep trying the
...
def _mint_stub_at(stub: StubAuthProvider) -> str:
    ...
    state = dict(
        seg.split("=", 1)
        for seg in ls.cookie_payload["moor_session_pkce"].split(";")
        if "=" in seg
    )["state"]
    verifier = dict(
        seg.split("=", 1)
