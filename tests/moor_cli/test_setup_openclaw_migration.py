...
from moor_cli import setup_migration
...
class TestOfferOpenclawMigration:
    ...
    def test_skips_when_user_declines(self, tmp_path):
        ...
        openclaw_dir.mkdir()
        script = tmp_path / "openclaw_to_moor.py"
        script.write_text("# placeholder")
        with (
            patch("moor_cli.setup.Path.home", return_value=tmp_path),
            patch.object(setup_migration, "_OPENCLAW_SCRIPT", script),
            patch.object(setup_mod, "prompt_yes_no", return_value=False),
        ):
            assert setup_mod._offer_openclaw_migration(tmp_path / ".moor") is False
    ...
    def test_runs_migration_when_user_accepts(self, tmp_path):
        ...
        # Create a fake moor home with config
        ...
        fake_mod = ModuleType("openclaw_to_moor")
        ...
        fake_migrator.migrate.return_value = {
            ...
            "output_dir": str(moor_home / "migration"),
        }
        ...
        with (
            ...
            result = setup_mod._offer_openclaw_migration(moor_home)
...
class TestSetupWizardOpenclawIntegration:
    ...
    def test_migration_offered_during_first_time_setup(self, tmp_path):
        ...
        with (
            patch.object(setup_mod, "ensure_moor_home"),
            patch.object(setup_mod, "load_config", return_value={}),
            patch.object(setup_mod, "get_moor_home", return_value=tmp_path),
            patch.object(setup_mod, "get_env_value", return_value=""),
            patch.object(setup_mod, "is_interactive_stdin", return_value=True),
            patch("moor_cli.auth.get_active_provider", return_value=None),
