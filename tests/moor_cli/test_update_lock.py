"""Cross-process update mutual exclusion (``moor_cli.update_lock``).

Three surfaces can start an update of one install tree: a terminal ``moor
...
dashboard-spawned ``moor update`` was mid-``npm install``, which then failed
...
@pytest.fixture
def marker(tmp_path):
    return tmp_path / ".moor-update-in-progress"
...
def test_marker_path_follows_process_moor_home(tmp_path, monkeypatch):
    ...
    assert update_marker_path() == tmp_path / ".moor-update-in-progress"
...
class TestHandoffFromOrchestratingUpdater:
    """The Tauri updater holds the marker, then spawns ``moor update``.
...
class TestAncestryHandoff:
    ...
    ``moor-setup`` under ``~/.moor`` is only refreshed by a full installer
    ...
    def _liveness_pinned_true(self, monkeypatch):
        monkeypatch.setattr("moor_cli.update_lock._pid_alive", lambda pid: True)
