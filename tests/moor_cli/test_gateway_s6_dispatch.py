"""Tests for the Phase 4 s6 dispatch helper in moor_cli.gateway.

`_dispatch_via_service_manager_if_s6` decides whether a
`moor gateway start/stop/restart` invocation should be routed to
...
def test_dispatch_all_handles_partial_failure(
    ...
    monkeypatch.setattr(
        "moor_cli.service_manager.detect_service_manager", lambda: "s6",
    )
    monkeypatch.setattr(
        "moor_cli.service_manager.get_service_manager", lambda: rec,
    )
...
def _stub_s6(monkeypatch: pytest.MonkeyPatch, *, on_s6: bool) -> _CallRecorder:
    ...
    rec = _CallRecorder()
    monkeypatch.setattr(
        "moor_cli.service_manager.detect_service_manager",
...
def test_redirect_falls_back_when_sleep_missing(
    ...
    monkeypatch.setattr("moor_cli.gateway._profile_suffix", lambda: "")

    monkeypatch.setattr("moor_cli.gateway.os.execvp", _raise_missing_sleep)
    block_calls: list[bool] = []
    monkeypatch.setattr(
        "moor_cli.gateway._block_until_terminated",
        lambda: block_calls.append(True),
    )
    monkeypatch.delenv("MOOR_S6_SUPERVISED_CHILD", raising=False)
    monkeypatch.delenv("MOOR_GATEWAY_NO_SUPERVISE", raising=False)
...
def _armed_watchdog(monkeypatch: pytest.MonkeyPatch):
    """Arm the real watchdog as moor_cli.main's argv fast-path does; the long timeout keeps the
