"""Regression tests for TOCTOU-safe credential file writers in ``moor_cli.auth``.
...
# ---------------------------------------------------------------------------
# _save_auth_store  (~/.moor/auth.json — every native OAuth provider)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Moor shared-credential store write (inside _write_shared_moor_state)
# ---------------------------------------------------------------------------


def test_shared_moor_store_writes_0o600_with_0o700_parent(tmp_path, monkeypatch):
    """The Moor shared-credential store must land at 0o600 / parent 0o700."""
    monkeypatch.setenv("MOOR_HOME", str(tmp_path))
    # _moor_shared_store_path() refuses to touch the real shared store during
    ...
    # home" reference — which currently tracks MOOR_HOME via
    # get_default_moor_root() — can't collide with our override and
    # falsely claim we're writing to the real user's shared store.
    monkeypatch.setenv("MOOR_SHARED_AUTH_DIR", str(tmp_path / "shared_override"))
    old_umask = os.umask(0o022)
    try:
        from moor_cli import auth as auth_mod

        state = {
            "access_token": "moor-access-xxx",
            "refresh_token": "moor-refresh-xxx",
            "token_type": "Bearer",
        ...
        auth_mod._write_shared_moor_state(state)
        path = auth_mod._moor_shared_store_path()
    finally:
        os.umask(old_umask)

    assert path.exists(), "shared Moor store was not written"
    mode = stat.S_IMODE(path.stat().st_mode)
    parent_mode = stat.S_IMODE(path.parent.stat().st_mode)

    assert mode == 0o600, (
        f"Moor shared store mode 0o{mode:o} != 0o600 — TOCTOU race regressed"
    )
    assert parent_mode == 0o700, (
        f"Moor shared store parent dir mode 0o{parent_mode:o} != 0o700"
    )
