...
def test_curated_codex_fallback_excludes_chatgpt_rejected_pro_slugs(monkeypatch):
    ...
    monkeypatch.setattr(
        "moor_cli.codex_models._fetch_models_from_api",
...
def test_setup_wizard_codex_import_resolves():
    ...
    # This mirrors the exact import used in moor_cli/setup.py line 873.
    # A prior bug had 'get_codex_models' (wrong) instead of 'get_codex_model_ids'.
    from moor_cli.codex_models import get_codex_model_ids as setup_import
...
def test_model_command_prompts_to_reuse_or_reauthenticate_codex_session(monkeypatch, capsys):
    from moor_cli.model_setup_flows import _model_flow_openai_codex
    ...
    monkeypatch.setattr(
        "moor_cli.auth.get_codex_auth_status",
        lambda: {"logged_in": True, "source": "moor-auth-store"},
    )
    monkeypatch.setattr(
        "moor_cli.auth.resolve_codex_runtime_credentials",
...
def _make_cli(model="anthropic/claude-opus-4.6", **kwargs):
    """Create a MoorCLI with minimal mocking."""
    ...
    with (
        ...
        cli = MoorCLI(model=model, **kwargs)
    return cli
...
class TestNormalizeModelForProvider:
    ...
    def test_default_model_replaced(self):
        ...
        with (
            patch("cli.get_tool_definitions", return_value=[]),
            patch.dict("os.environ", {"LLM_MODEL": "", "MOOR_MAX_ITERATIONS": ""}, clear=False),
            patch.dict(_cli_mod.__dict__, {"CLI_CONFIG": _clean_config}),
        ):
            from cli import MoorCLI
            cli = MoorCLI()
