...
from moor_cli import main as moor_main, update_cmd
from tests.moor_cli.test_update_target_identity import git, update_tree  # noqa: F401
...
def test_update_preserves_local_work_and_rescues_orphan_before_reset(
    ...
    monkeypatch.setattr(moor_main, '_sync_with_upstream_if_needed', update_cmd._sync_with_upstream_if_needed)
    ...
    def fault(command, *args, **kwargs):
        if 'reset' in command and '--hard' in command:
            refs = original(['git', 'for-each-ref', '--format=%(objectname)',
                             'refs/moor-update-backups/'], cwd=t.clone,
    ...
    if failure not in {'ref', 'head'}:
        ...
        assert f'refs/moor-update-backups/{kind}-main-' in output
        if kind == 'diverged':
...
def test_rescue_retention_uses_real_refs(tmp_path, monkeypatch, mode):
    ...
    prefix = 'refs/moor-update-backups/orphan-main-'
    if mode == 'count':
    ...
    assert set(git(tmp_path, 'for-each-ref', '--format=%(refname)', 'refs/moor-update-backups/').splitlines()) == set(refs) - removed


@pytest.mark.parametrize('fault,body,modules,message', [
    ('syntax', '<<<<<<< Updated upstream\nVALUE = 2\n', (), 'made the Moor agent unexecutable'),
...
def test_restore_validates_real_stash_and_each_import(probe_root, monkeypatch, capsys, fault, body, modules, message):
    import moor_cli.update_cmd_stash as stash
    ...
    git(tmp_path, 'add', 'first.py', 'consumer.py', 'moor_bootstrap.py')
    git(tmp_path, '-c', 'user.name=Fixture', '-c', 'user.email=fixture@example.invalid',
        '-c', 'commit.gpgsign=false', 'commit', '-qm', 'base')
    source.write_text(body, encoding='utf-8')
    ref = moor_main._stash_local_changes_if_needed(['git'], tmp_path)
    assert ref
    ...
    if message is None:
        assert moor_main._restore_stashed_changes(['git'], tmp_path, ref, prompt_user=False)
        assert source.read_text(encoding='utf-8') == body
        assert not git(tmp_path, 'stash', 'list')
    else:
        with pytest.raises(SystemExit) as error:
            moor_main._restore_stashed_changes(['git'], tmp_path, ref, prompt_user=False)
        assert error.value.code == 1
...
def test_unreadable_stash_prompt_keeps_work(tmp_path, monkeypatch, capsys, error):
    ...
    assert moor_main._restore_stashed_changes(['git'], tmp_path, 'stash@{0}', prompt_user=True) is False
...
def test_stash_selector_is_a_bare_index_never_a_brace_selector(tmp_path):
    ...
    import moor_cli.update_cmd_stash as stash_mod
...
def test_autostash_survives_intent_to_add_entries(tmp_path):
    ...
    Reported: `moor update` aborted with "Entry 'tests/...' not uptodate. Cannot merge." because
    ...
    assert moor_main._restore_stashed_changes(["git"], tmp_path, stash_ref, prompt_user=False)
...
def test_conflicted_restore_records_parked_step_in_receipt(monkeypatch, tmp_path):
    ...
    with _active_receipt(probe):
        restored = moor_main._restore_stashed_changes(["git"], tmp_path, stash_ref, prompt_user=False)
