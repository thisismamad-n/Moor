...
from moor_cli import config as moor_config
...
def _patch_managed_uv(request):
    ...
    with patch("moor_cli.managed_uv.resolve_uv", side_effect=_fake_resolve_uv), \
         patch("moor_cli.managed_uv.ensure_uv", side_effect=_fake_ensure_uv), \
         patch("moor_cli.managed_uv.update_managed_uv", side_effect=_fake_update_managed_uv):
        yield
...
def _patch_gateway_discovery():
    ...
    ``_purge_stale_moor_modules`` must also be stubbed: it evicts
    ``moor_cli.gateway`` from ``sys.modules`` mid-update, and the restart
    phase's fresh ``from moor_cli.gateway import ...`` then loads an
    ...
    it ``ai.moor.gateway`` and the verify step exits 1 (#111866, #110701).
    """
    with patch("moor_cli.gateway.find_gateway_pids", return_value=[]), \
         patch("moor_cli.gateway.supports_systemd_services", return_value=False), \
         patch("moor_cli.update_cmd_fleet._restart_macos_launchd_gateways", lambda *a, **k: None), \
         patch("moor_cli.gateway.find_profile_gateway_processes", return_value=[]), \
         patch("moor_cli.update_inventory.collect_runtime_inventory", return_value=None), \
         patch("moor_cli.update_inventory.report_unaccounted_runtimes", return_value=False), \
         patch.object(moor_main, "_fleet_probe_expected_runtimes", lambda *a, **kw: False), \
         patch.object(moor_main, "_purge_stale_moor_modules", lambda *a, **kw: None), \
         patch("moor_cli.update_receipt.collect_fleet_versions", return_value=[]):
        yield
...
def _setup_update_mocks(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(moor_main, "_stash_local_changes_if_needed", lambda *a, **kw: None)
    monkeypatch.setattr(moor_main, "_restore_stashed_changes", lambda *a, **kw: True)
    monkeypatch.setattr(moor_config, "get_missing_env_vars", lambda required_only=True: [])
    monkeypatch.setattr(moor_config, "get_missing_config_fields", lambda: [])
    monkeypatch.setattr(moor_config, "check_config_version", lambda **_kwargs: (5, 5))
    monkeypatch.setattr(moor_config, "migrate_config", lambda **kw: {"env_added": [], "config_added": []})
    monkeypatch.setattr(moor_main, "_upgrade_pip_before_lazy_refresh", lambda *a, **kw: None)
    monkeypatch.setattr(moor_main, "_refresh_active_lazy_features", lambda *a, **kw: True)
...
def test_refresh_active_memory_provider_dependencies_reinstalls_active_provider(monkeypatch):
    ...
    monkeypatch.setattr(
        "moor_cli.memory_setup._install_dependencies",
        lambda provider_name, force=False: recorded.append((provider_name, force)),
    )

    moor_main._refresh_active_memory_provider_dependencies()
...
def test_reload_updated_runtime_modules_restores_new_moor_constants_symbol(monkeypatch):
    """A pre-pull module object missing a new helper is repaired by reload."""
    import moor_constants

    monkeypatch.delattr(moor_constants, "apply_subprocess_home_env", raising=False)
    assert not hasattr(moor_constants, "apply_subprocess_home_env")

    moor_main._reload_updated_runtime_modules()

    assert callable(moor_constants.apply_subprocess_home_env)
...
def _make_update_side_effect(
    ...
    ``refs/moor-update-backups/orphan-<branch>-*`` (oldest first) so the
...
def test_cmd_update_skips_stash_restore_when_reset_fails(monkeypatch, tmp_path, capsys):
    ...
    monkeypatch.setattr(
        moor_main, "_stash_local_changes_if_needed",
        lambda *a, **kw: "abc123deadbeef",
    )
    restore_calls = []
    monkeypatch.setattr(
        moor_main, "_restore_stashed_changes",
        lambda *a, **kw: restore_calls.append(1) or True,
    )

    side_effect, _ = _make_update_side_effect(ff_only_fails=True, reset_fails=True)
    monkeypatch.setattr(moor_main.subprocess, "run", side_effect)

    with pytest.raises(SystemExit, match="1"):
        moor_main.cmd_update(SimpleNamespace())
...
def test_cmd_update_orphan_history_backs_up_before_reset(monkeypatch, tmp_path, capsys):
    ...
    ``refs/moor-update-backups/orphan-*`` ref before the reset proceeds."""
    _setup_update_mocks(monkeypatch, tmp_path)
    ...
    assert ref_name.startswith("refs/moor-update-backups/orphan-main-")
...
def test_cmd_update_orphan_rescue_refs_pruned_beyond_keep_limit(monkeypatch, tmp_path, capsys):
    ...
    stale_refs = [
        "refs/moor-update-backups/orphan-main-"
...
def test_prune_orphan_rescue_refs_leaves_unparseable_names_alone():
    ...
    weird = "refs/moor-update-backups/orphan-main-not-a-timestamp-xyz"
    calls = []
    ...
    with mock_patch.object(moor_main.subprocess, "run", side_effect=fake_run):
...
def _setup_setting_test(monkeypatch, tmp_path, mode):
    ...
    monkeypatch.setattr(
        moor_main, "_discard_stashed_changes",
...
def _setup_keep_stash_test(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        moor_main, "_park_stashed_changes",
    ...
    monkeypatch.setattr(
        "moor_cli.gateway.find_gateway_pids", lambda **kw: [], raising=False
    )
...
def test_update_keep_stash_parks_instead_of_restoring(monkeypatch, tmp_path):
    ...
    moor_main.cmd_update(SimpleNamespace(yes=True, keep_stash=True))
...
def test_update_without_keep_stash_still_restores(monkeypatch, tmp_path):
    ...
    moor_main.cmd_update(SimpleNamespace(yes=True, keep_stash=False))
...
def test_bootstrap_marker_not_autostashed_by_update(tmp_path):
    ...
    ``moor update``'s ``git stash push --include-untracked`` does not sweep it
    ...
    repo_gitignore = Path(moor_main.__file__).resolve().parents[1] / ".gitignore"
    ...
    marker = tmp_path / ".moor-bootstrap-complete"
    marker.write_text("")

    # Exact flags used by moor update (moor_cli/main.py).
    git("stash", "push", "--include-untracked", "-m", "moor-update-autostash")

    assert marker.exists(), (
        ".moor-bootstrap-complete was swept into the update autostash — it must "
    ...
    assert ".moor-bootstrap-complete" not in status
...
def test_restore_rejects_invalid_python_and_keeps_clean_updated_tree(
    ...
    import subprocess
    from moor_cli import update_cmd
    import moor_cli.update_cmd_deps as update_cmd_deps
    ...
    assert "made the Moor agent unexecutable" in output
...
def test_restore_allows_preexisting_import_time_failure(monkeypatch, tmp_path):
    ...
    assert moor_main._restore_stashed_changes(
        ["git"], tmp_path, stash_ref, prompt_user=False
    )
...
def test_restore_stays_parked_when_untracked_baseline_is_unknown(
    ...
    from moor_cli import update_cmd
    import moor_cli.update_cmd_stash as update_cmd_stash
...
def test_prune_orphan_rescue_refs_with_real_git_unpins_objects(tmp_path):
    ...
    old_ref = "refs/moor-update-backups/orphan-main-20200101-000000-" + snap_sha[:12]
    ...
    remaining = git("for-each-ref", "refs/moor-update-backups/").stdout
