"""
Tests for --yes / --force flag separation in `moor skills install`.
...
def test_cli_skills_install_yes_sets_skip_confirm(monkeypatch):
    ...
    monkeypatch.setattr("moor_cli.skills_hub.skills_command", fake_skills_command)
    monkeypatch.setattr(
        sys,
        "argv",
        ["moor", "skills", "install", "official/email/agentmail", "--yes"],
    )
...
def test_cli_skills_install_no_flags(monkeypatch):
    ...
    monkeypatch.setattr(
        sys,
        "argv",
        ["moor", "skills", "install", "test/skill"],
    )
