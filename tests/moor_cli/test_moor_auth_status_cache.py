"""Tests for the get_moor_auth_status() process-level cache.

The cache avoids re-validating Moor credentials on every menu paint —
`moor tools` → "All Platforms" used to fire ~31 OAuth refresh POSTs
...
also call invalidate_moor_auth_status_cache().
"""
...
def test_get_moor_auth_status_caches_consecutive_calls(tmp_path, monkeypatch):
    ...
    with patch.object(auth_mod, "_compute_moor_auth_status", side_effect=fake_compute):
        first = auth_mod.get_moor_auth_status()
        second = auth_mod.get_moor_auth_status()
        third = auth_mod.get_moor_auth_status()

    assert call_count["n"] == 1, (
        f"_compute_moor_auth_status was called {call_count['n']}× — "
