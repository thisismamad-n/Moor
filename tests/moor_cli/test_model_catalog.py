"""Tests for moor_cli.model_catalog — remote manifest fetch + cache + fallback."""
...
def isolated_home(tmp_path, monkeypatch):
    """Isolate MOOR_HOME + reset any module-level catalog cache per test."""
    home = tmp_path / ".moor"
    home.mkdir()
    ...
    import importlib
    from moor_cli import model_catalog
...
def _valid_manifest() -> dict:
    return {
        ...
        "providers": {
            ...
            "moor": {
                "metadata": {"display_name": "Moor Portal"},
                "models": [
...
class TestValidation:
    def test_accepts_well_formed_manifest(self, isolated_home):
        from moor_cli.model_catalog import _validate_manifest
...
class TestCuratedAccessors:
    ...
    def test_moor_returns_none_when_catalog_empty(self, isolated_home):
        from moor_cli import model_catalog
        with patch.object(model_catalog, "_fetch_manifest", return_value=None):
            assert model_catalog.get_curated_moor_models() is None
...
class TestDefaultModelFromCache:
    ...
    def _manifest_with_default(self) -> dict:
        ...
        m["providers"]["moor"]["models"][1]["default"] = True  # kimi-k2.6
        return m
    ...
    def test_reads_label_from_disk_cache(self, isolated_home):
        ...
        with patch.object(model_catalog, "_fetch_manifest") as fetch:
            ...
            assert (
                model_catalog.get_default_model_from_cache("moor")
    ...
    def test_shipped_manifest_labels_glm52_default(self, isolated_home):
        ...
        import moor_cli.model_catalog as model_catalog
        from moor_cli.models import PREFERRED_SILENT_DEFAULT_MODEL

        repo_root = Path(model_catalog.__file__).resolve().parent.parent
        manifest = json.loads(
            (repo_root / "website" / "static" / "api" / "model-catalog.json").read_text()
        )
        for provider in ("openrouter", "moor"):
...
class TestRefreshCadence:
    def test_default_ttl_is_twenty_minutes_and_legacy_hours_honoured(self):
        ...
        with patch("moor_cli.config.load_config", return_value={"model_catalog": {"ttl_minutes": 20}}):
        ...
        with patch("moor_cli.config.load_config", return_value={"model_catalog": {"ttl_minutes": 20, "ttl_hours": 3}}):
    ...
    def test_refresh_catalogs_forces_every_source(self):
        ...
        with patch.object(model_catalog, "_load_catalog_config", return_value={
            "enabled": True, "url": "http://master", "ttl_hours": 1.0, "providers": {},
             ...
             patch("moor_cli.models.fetch_openrouter_models") as orm, \
             patch("moor_cli.models.fetch_moor_recommended_models") as moor:
        ...
        moor.assert_called_once_with(force_refresh=True)


class TestIntegrationWithModelsModule:
    """Exercise the fallback paths via the real callers in moor_cli.models."""



    def test_picker_moor_row_uses_curated_list(self, tmp_path, monkeypatch):
        """The /model picker surfaces the curated ``_PROVIDER_MODELS["moor"]``
        list in curated order — matching the ``moor model`` CLI — not the live
        ...
        # ``MOOR_HOME / auth.json`` then resolves to the same path the
        # seat-belt thinks is the "real" user store. Use the autouse
        # ``_hermetic_environment`` MOOR_HOME directly instead.
        import importlib
        from moor_cli import model_catalog
        from moor_cli.models import get_curated_moor_model_ids
        importlib.reload(model_catalog)
        try:
            from moor_cli.model_switch_providers import list_picker_providers

            active_home = Path(os.environ["MOOR_HOME"])
            (active_home / "auth.json").write_text(
                json.dumps(
                    {
                        "providers": {"moor": {"access_token": "fake"}},
                        "credential_pool": {},
                    }
                )
            )
            ...
            # (``curated["moor"] = get_curated_moor_model_ids()``), so the test
            # stays an invariant — it can't rot as the curated/manifest list grows.
            with patch.object(
                model_catalog, "_fetch_manifest", return_value=_valid_manifest()
            ), patch("moor_cli.models.check_moor_free_tier", return_value=False), patch(
                "moor_cli.models.union_with_portal_free_recommendations",
                side_effect=lambda ids, *a, **k: (ids, {}),
            ), patch(
                "moor_cli.models.union_with_portal_paid_recommendations",
                side_effect=lambda ids, *a, **k: (ids, {}),
            ):
                expected = get_curated_moor_model_ids()
                picker = list_picker_providers(
                    current_provider="moor", max_models=99
                )
        finally:
            model_catalog.reset_cache()

        moor_row = next((r for r in picker if r["slug"] == "moor"), None)
        assert moor_row is not None, "moor row must appear when authed"
        assert moor_row["models"] == expected
    ...
    def test_picker_max_models_cap_semantics(self, tmp_path, monkeypatch):
        ...
        try:
            ...
            with patch.object(
                ...
                expected = get_curated_moor_model_ids()
                full = list_picker_providers(current_provider="moor", max_models=None)
                one = list_picker_providers(current_provider="moor", max_models=1)
                ...
                zero = list_authenticated_providers(
                    current_provider="moor", max_models=0
                )
        finally:
            model_catalog.reset_cache()

        def _moor(rows):
            return next((r for r in rows if r["slug"] == "moor"), None)

        # Only meaningful when the curated list actually exceeds 1 entry.
        assert len(expected) > 1, "test needs a multi-model curated moor list"
...
#
# History: qwen/qwen3.6-plus was added to _PROVIDER_MODELS["moor"] in commit
...
class TestManifestMatchesInRepoLists:
    ...
    def test_in_repo_lists_match_manifest(self):
        ...
        assert self._strip_volatile(actual) == self._strip_volatile(expected), (
            ...
            "_PROVIDER_MODELS['moor'] / OPENROUTER_MODELS. "
