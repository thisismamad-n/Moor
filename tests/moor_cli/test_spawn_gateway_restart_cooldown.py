...
class TestRepeatRestartWithinCooldown:
    ...
    @patch(
        "moor_cli.web_server._gateway_subcommand",
        return_value=["gateway", "restart"],
    )
    @patch("moor_cli.web_server._spawn_moor_action")
    @patch("moor_cli.web_server._ACTION_PROCS", {})
...
class TestExistingBehaviourIsPreserved:
    ...
    def test_live_child_is_still_reused_without_consulting_the_clock(
        ...
        with patch(
            "moor_cli.web_server._ACTION_PROCS", {"gateway-restart": live}
        ), patch(
            "moor_cli.web_server._ACTION_COMMANDS",
