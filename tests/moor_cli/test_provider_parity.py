...
the SAME provider universe as ``moor model`` (the CLI/TUI picker).
...
def test_every_moor_model_provider_is_configurable_in_desktop():
    """PARITY CONTRACT: GUI (keys ∪ accounts) ⊇ `moor model` universe."""
    ...
    assert not missing, (
        "providers shown in `moor model` but not configurable in the desktop "
