...
profile switcher can target any profile's MOOR_HOME. These tests pin:
reads/writes land in the REQUESTED profile, the dashboard's own profile
stays untouched, and the chat PTY env is scoped via MOOR_HOME.
"""
import json
...
@pytest.fixture
def isolated_profiles(tmp_path, monkeypatch, _isolate_moor_home):
...
class TestProfileScopedModel:
    ...
    def test_model_options_uses_config_only_scope_for_selected_profile(
        ...
        monkeypatch.setattr(
            "moor_cli.inventory.load_picker_context", lambda: object()
        )
        monkeypatch.setattr(
            "moor_cli.inventory.build_model_options_payload",
...
class TestProfileScopedPostSetup:
    def test_post_setup_spawns_with_profile_flag(
        ...
        config / write per-profile state see the same MOOR_HOME the rest
        ...
        monkeypatch.setattr(
            "moor_cli.tools_config.valid_post_setup_keys",
...
class TestProfileScopedGateway:
    ...
    def test_status_reads_requested_profile_home(
        ...
        def fake_get_running_pid(*args, **kwargs):
            ...
            seen_homes.append(str(get_moor_home()))
            return None
        ...
        assert resp.json()["moor_home"] == str(isolated_profiles["worker_beta"])
...
class TestProfileScopedChatPty:
    def test_chat_argv_scopes_moor_home(self, isolated_profiles, monkeypatch):
        ...
        assert env["MOOR_HOME"] == str(isolated_profiles["worker_beta"])
        # Scoped chat must NOT attach to the dashboard's in-memory gateway.
        assert "MOOR_TUI_GATEWAY_URL" not in env
