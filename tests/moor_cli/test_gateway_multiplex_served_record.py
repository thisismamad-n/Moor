...
- Probe: ``named_profile_served_by_running_multiplexer`` reads the pid-verified default
  ``gateway_state.json`` first (env-only opt-in on the default profile is invisible to ``moor -p X``);
...
- ``moor status`` / ``cron status`` on a satellite say running-via-multiplexer; the default lists served.
"""
...
def served_root(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("MOOR_HOME", str(root / "profiles" / "coder"))
    ...
    monkeypatch.setattr(status, "_read_process_cmdline", lambda pid: (
        "python -m moor_cli.main gateway run" if pid == os.getpid() else real_cmdline(pid)))
    return root
...
def test_probe_falls_back_to_config_only_without_recorded_key(served_root):
    ...
    (served_root / "gateway_state.json").write_text(json.dumps(
        {"pid": os.getpid(), "moor_home": str(served_root), "gateway_state": "running"}))
...
def test_probe_survives_a_missing_default_pid_file(served_root):
    ...
    (``moor -p X status``, ``cron list``, the dashboard ladder) say "not running" about the gateway
...
def test_setup_gateway_service_step_skips_install_for_served_profile(served_root, monkeypatch, capsys):
    ...
    monkeypatch.setenv("MOOR_HOME", str(served_root / "profiles" / "other"))  # not in the live record
    assert gw.ensure_gateway_service(context="setup") is True
    assert calls == []
...
def test_recycled_pid_does_not_lend_a_stale_record_its_served_profiles(served_root):
    ...
    once did, so `moor -p coder gateway start` exited 78 for a multiplexer that was long gone."""
    import subprocess
    import gateway.status as status
    from moor_cli.gateway import named_profile_served_by_running_multiplexer
    ...
    try:
        ...
        for name in ("gateway.pid", "gateway_state.json"):
            (served_root / name).write_text(json.dumps({
                "pid": child.pid, "moor_home": str(served_root), "gateway_state": "running",
...
def test_dashboard_liveness_ladder_reports_served_profile_running(served_root):
    ...
    "stopped" while `moor -p X status` said running. Alpha's `<X>:<platform>` entries project as its own."""
