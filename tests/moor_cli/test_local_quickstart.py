...
def test_quickstart_runs_all_three_legs(client, monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        "moor_cli.local_runtime.binaries.installed_tags", lambda: [])
    monkeypatch.setattr(
    ...
    monkeypatch.setattr(
        "moor_cli.web_routers.local_models.download_file", _fake_download)
...
def test_quickstart_skips_satisfied_legs(client, monkeypatch):
    ...
    monkeypatch.setattr(
        "moor_cli.local_runtime.binaries.installed_tags", lambda: ["b10362"])
    monkeypatch.setattr(
        "moor_cli.local_runtime.binaries.ensure_runtime_installed",
        lambda tag, backend, progress=None: calls.append("install"))
    ...
    monkeypatch.setattr(
        "moor_cli.local_runtime.bootstrap.staged_model_ids", lambda: all_ids)
    monkeypatch.setattr(
        "moor_cli.web_routers.local_models.download_file",
