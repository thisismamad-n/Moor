...
def test_quickstart_refuses_when_nothing_fits(client, monkeypatch):
    ...
    monkeypatch.setattr(
        "moor_cli.local_runtime.catalog.select_variant", lambda *a, **k: None)
...
def test_quickstart_runs_all_three_legs(client, capable_hardware, monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        "moor_cli.local_runtime.binaries.installed_engine", lambda *args, **kwargs: None)
    monkeypatch.setattr(
        "moor_cli.local_runtime.binaries.ensure_engine",
    ...
    monkeypatch.setattr(
        "moor_cli.web_routers.local_models._download_job", _fake_download)

    # Leg 3: activation — stub the server start and the model assignment.
    monkeypatch.setattr(
        "moor_cli.local_runtime.bootstrap.ensure_local_runtime",
        lambda config, force=False: calls.append("server") or None)
    monkeypatch.setattr(
        "moor_cli.web_routers.local_models._state_endpoint",
...
def test_quickstart_skips_satisfied_legs(client, capable_hardware, monkeypatch):
    ...
    monkeypatch.setattr(
        "moor_cli.local_runtime.binaries.installed_engine", lambda *args, **kwargs: Engine("cpu", "b10362", Path("unused")))
    monkeypatch.setattr(
        "moor_cli.local_runtime.binaries.ensure_engine",
        lambda backend, **kwargs: calls.append("install"))
    ...
    monkeypatch.setattr(
        "moor_cli.web_routers.local_models._download_job",
...
def quickstart_ready(monkeypatch):
    ...
    from moor_cli.local_runtime.catalog import VariantChoice

    monkeypatch.setattr(
        "moor_cli.local_runtime.binaries.installed_engine", lambda *args, **kwargs: Engine("cpu", "b10362", Path("unused")))
    monkeypatch.setattr(
        "moor_cli.local_runtime.catalog.select_variant",
        lambda entry, budget: VariantChoice(variant=entry.variants[0],
                                            zero_spill=True,
                                            reason_key="best-fits"))
    monkeypatch.setattr(
        "moor_cli.web_routers.local_models._engine_too_old",
