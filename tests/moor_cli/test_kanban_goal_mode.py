...
2. Spawn: _default_spawn sets the MOOR_KANBAN_GOAL_MODE env vars only
...
# ---------------------------------------------------------------------------
# CLI judge gate tests (moor kanban complete bypass fix)
# ---------------------------------------------------------------------------

class TestCLIJudgeGate:
    """moor kanban complete must apply the same goal_mode judge gate as the
    ...
    def _run(self, monkeypatch, *, goal_mode=True, judge_available=True,
        ...
        from moor_cli.kanban import _cmd_complete
        ...
        monkeypatch.setattr("moor_cli.kanban.kb.get_task", lambda conn, tid: fake_task)
        monkeypatch.setattr("moor_cli.kanban.kb.complete_task", fake_complete_task)
        monkeypatch.setattr("moor_cli.kanban.kbc.connect_closing", fake_connect_closing)
        monkeypatch.setattr("moor_cli.kanban._worker_run_id_for", lambda _: None)
