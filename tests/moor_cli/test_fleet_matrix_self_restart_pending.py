...
def _fleet_homes(monkeypatch, tmp_path, records: dict[str, dict]) -> None:
    ...
    monkeypatch.setattr("moor_cli.version_info.get_code_identity", lambda refresh=False: {"sha": HEAD, "version": "1.0"})
    monkeypatch.setattr("moor_cli.profiles._get_default_moor_home", lambda: root)
    monkeypatch.setattr("moor_cli.profiles._get_profiles_root", lambda: root / "profiles")
...
def test_restart_phase_records_accepted_self_restart_and_verify_exits_clean(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("moor_cli.update_cmd_maint._refresh_dashboard_after_update", lambda *a, **k: None)
    ...
    restart = fleet_mod._GatewayRestartOutcome(
        incomplete=False, phase_errors=[], pre_restart_gateway_pids=[ancestor], restarted_services=["moor-gateway"],
