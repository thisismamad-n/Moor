...
def _parse_slack_args(argv):
    """Build the real `moor slack` parser and parse argv against it."""
...
def _run_console_entrypoint(*argv: str) -> subprocess.CompletedProcess[str]:
    ...
    return subprocess.run(
        [
            sys.executable,
            "-c",
            "from moor_cli.main import main; raise SystemExit(main())",
            *argv,
        ],
...
def test_slack_dispatcher_propagates_manifest_failure(monkeypatch):
    from moor_cli import main as main_module
    from moor_cli import slack_cli
...
class TestSlackManifestArgparse:
    ...
    def test_long_description_file_reports_tilde_expansion_errors(
        ...
        source = "~moor-user-that-does-not-exist-20260716/AGENTS.md"
