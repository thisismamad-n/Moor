...
def test_write_source_stamp_records_live_checkout_identity_atomically(tmp_path):
    ...
    repo.mkdir()
    _git(repo, "init", "-q")
    _git(repo, "config", "user.name", "Moor Test")
    _git(repo, "config", "user.email", "moor@example.invalid")
...
def test_stale_source_stamp_defers_to_live_checkout(tmp_path, monkeypatch):
    from moor_cli.source_stamp import write_source_stamp
    from moor_cli.version_info import _reset_version_info_cache, get_version_info
