...
def test_cmd_setup_generic_choice_cancel_writes_nothing(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(memory_setup, "get_moor_home", lambda: tmp_path)
    monkeypatch.setattr("moor_cli.config.load_config", lambda: {"memory": {}})
...
# behavior is covered by tests/moor_cli/test_memory_setup_env_denylist.py,
# which exercises the current save_env_value-routed signature
# (env_writes, moor_home=None) \u2014 these three tests pinned the prior direct
...
def test_install_dependencies_force_reinstalls_versioned_specs(tmp_path, monkeypatch):
    ...
    so a downgraded/stripped bridge package is restored on moor update."""
...
def test_cmd_status_memory_tool_gate_disabled(capsys, monkeypatch):
    ...
    monkeypatch.setattr("moor_cli.config.load_config", lambda: _cfg)
    # check_memory_requirements() reads the readonly loader, not load_config.
    monkeypatch.setattr(
        "moor_cli.config.load_config_readonly", lambda: _cfg, raising=False
    )
