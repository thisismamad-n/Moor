"""Tests for moor_cli._early_recovery — the dependency-light bootstrap
repair that runs BEFORE moor_cli.main's third-party imports (#57828 / #58004).

Covers:
- entry-point lifecycle: a broken core import (dotenv) crashes the import of
  moor_cli.main WITHOUT early recovery, and imports fine when recovery runs
...
def _run_lifecycle_subprocess(tmp_path: Path, *, repair: bool) -> subprocess.CompletedProcess:
    ...
    script.write_text(
        textwrap.dedent(
            ...
            import moor_cli._early_recovery as er
            ...
            import moor_cli.main  # noqa: F401
...
def test_broken_dotenv_crashes_main_import_without_repair(tmp_path):
    """Negative control: the shadow really breaks importing moor_cli.main,
...
def test_early_recovery_module_is_stdlib_only(tmp_path):
    ...
    script.write_text(
        textwrap.dedent(
            ...
            STDLIB = set(sys.stdlib_module_names) | {"moor_cli"}
            ...
            import moor_cli._early_recovery  # noqa: F401
...
def test_core_marker_triggers_install_before_any_native_import(
    ...
    import moor_cli._install_repair  # noqa: F401  (pre-import for patch)
...
def test_core_marker_marks_attempts_and_keeps_marker_on_install_failure(
    ...
    import moor_cli._install_repair  # noqa: F401
...
def test_core_marker_from_dead_updater_is_recovered_on_update_retry(
    ...
    """Retrying ``moor update`` must consume a prior deferral marker.
