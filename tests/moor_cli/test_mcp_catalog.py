"""Tests for moor_cli.mcp_catalog and moor_cli.mcp_picker.
...
def _default_mock_probe(monkeypatch):
    ...
    ``moor_cli.mcp_catalog._probe_tools`` themselves.
    """
...
def catalog_dir(tmp_path, monkeypatch):
    ...
    cat.mkdir()
    monkeypatch.setenv("MOOR_OPTIONAL_MCPS", str(cat))
    return cat


@pytest.fixture(autouse=True)
def _isolate_moor_home(tmp_path, monkeypatch):
    """Redirect all config I/O to a temp MOOR_HOME."""
    hh = tmp_path / "moor-home"
    hh.mkdir()
    monkeypatch.setenv("MOOR_HOME", str(hh))
    monkeypatch.setattr(
        "moor_cli.config.get_moor_home", lambda: hh
    )
    monkeypatch.setattr(
        "moor_cli.config.get_config_path", lambda: hh / "config.yaml"
    )
    monkeypatch.setattr(
        "moor_cli.config.get_env_path", lambda: hh / ".env"
    )
    # mcp_catalog grabs get_moor_home() lazily through moor_constants
    monkeypatch.setattr(
        "moor_constants.get_moor_home", lambda: hh
    )
    return hh
...
class TestManifestParsing:
    ...
    def test_suggest_empty_block_rejected(self, catalog_dir):
        ...
        from moor_cli.mcp_catalog import list_catalog, catalog_diagnostics
    ...
    def test_http_api_key_builds_bearer_headers_template(self, catalog_dir):
        ...
        _write_manifest(catalog_dir, "demo", body)
        from moor_cli.mcp_catalog import _build_server_config
...
class TestInstall:
    def test_install_simple_stdio_writes_config(self, catalog_dir):
        _write_manifest(catalog_dir, "demo", _basic_manifest())
        from moor_cli.mcp_catalog import install_entry
    ...
    def test_install_rejects_exfil_shaped_stdio_manifest(self, catalog_dir):
        ...
        from moor_cli.config import load_config
        from moor_cli.mcp_catalog import CatalogError, install_entry
...
class TestUninstall:
    def test_uninstall_removes_server_block(self, catalog_dir):
        _write_manifest(catalog_dir, "demo", _basic_manifest())
        from moor_cli.mcp_catalog import install_entry, uninstall_entry
        from moor_cli.config import load_config
    ...
    def test_uninstall_missing_returns_false(self):
        from moor_cli.mcp_catalog import uninstall_entry
...
class TestGitInstallShaRef:
    def test_sha_ref_skips_branch_attempt(self, catalog_dir, monkeypatch, tmp_path):
        ...
        from moor_cli import mcp_catalog
        from moor_cli.mcp_catalog import _do_git_install
...
class TestToolsConfigIncludeMode:
    def test_configure_mcp_writes_include_not_exclude(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr(
            "moor_cli.curses_ui.curses_checklist",
...
class TestShippedCatalog:
    def test_all_shipped_manifests_parse(self, monkeypatch):
        ...
        """
        # Use the actual repo's optional-mcps directory (no MOOR_OPTIONAL_MCPS
        # override) so this test catches real manifests.
        monkeypatch.delenv("MOOR_OPTIONAL_MCPS", raising=False)
        from moor_cli.mcp_catalog import _catalog_root, _parse_manifest
