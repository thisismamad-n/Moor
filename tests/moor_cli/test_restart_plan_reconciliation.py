...
def test_mechanism_ids_are_machine_readable_and_described():
    ...
    assert describe_restart_mechanism("manual", "default") == "moor gateway restart"
    assert "sc.exe" in describe_restart_mechanism("windows-service", "default")


def test_windows_service_supervisor_classification():
    from moor_cli.update_inventory import _detect_supervisor_for_pid
...
def test_windows_service_runtime_reconciles_via_service_profiles():
    ...
    outcomes = match_runtime_outcomes(
        ...
        restarted_services=["moor-gateway"], relaunched_profiles=["default"],
...
def test_failed_unit_is_failed():
    outcomes = match_runtime_outcomes(
        ...
        failed_units=["moor-gateway-work.service"],
    )
...
def test_restarted_service_unit_matches_profile():
    outcomes = match_runtime_outcomes(
        ...
        restarted_services=["moor-gateway.service"], relaunched_profiles=[],
        externally_supervised_profiles=[], killed_pids=set(), failed_units=[],
    )
    assert outcomes[0]["outcome"] == "restarted"


def test_launchd_default_gateway_restarted_via_ai_moor_label():
    """macOS restart bookkeeping records ``ai.moor.gateway``, which does not
    contain the substring ``moor-gateway``. The default-profile gateway must
    ...
    outcomes = match_runtime_outcomes(
        ...
        restarted_services=["ai.moor.gateway"], relaunched_profiles=[],
        externally_supervised_profiles=[], killed_pids=set(), failed_units=[],
    )
    assert outcomes[0]["outcome"] == "restarted"
    assert report_unaccounted_runtimes(outcomes) is False
...
def test_launchd_named_profile_and_failed_label():
    restarted = match_runtime_outcomes(
        ...
        restarted_services=["ai.moor.gateway-work"], relaunched_profiles=[],
        externally_supervised_profiles=[], killed_pids=set(), failed_units=[],
    )
...
def test_named_profile_serve_does_not_match_gateway_profile_unit():
    """``moor-gateway-work.service`` restarted must not credit the ``work``
    serve — the old substring match (``"work" in unit``) did exactly that."""
    outcomes = match_runtime_outcomes(
        _plan(_rt("work", 101, supervisor="systemd"), _serve("work", 901)),
        restarted_services=["moor-gateway-work.service"], relaunched_profiles=[],
        externally_supervised_profiles=[], killed_pids=set(), failed_units=[],
    )
    by_pid = {o["pid"]: o["outcome"] for o in outcomes}
...
def test_serve_reconciles_against_its_own_unit_vocabulary():
    """A serve IS covered when a ``moor-serve*`` unit for its profile was
    ...
    outcomes = match_runtime_outcomes(
        ...
        restarted_services=["moor-gateway", "user/moor-serve",
                            "moor-serve-work.service", "moor-dashboard-ops"],
        relaunched_profiles=[], externally_supervised_profiles=[],
        killed_pids=set(), failed_units=["moor-serve-qa.service"],
    )
    by_pid = {o["pid"]: o["outcome"] for o in outcomes}
    assert by_pid == {900: "restarted", 901: "restarted", 902: "restarted", 903: "failed"}
    # exact names: ``work`` must not claim ``moor-serve-workbench``
    outcomes = match_runtime_outcomes(
        _plan(_serve("work", 901)),
        restarted_services=["moor-serve-workbench.service"], relaunched_profiles=[],
        externally_supervised_profiles=[], killed_pids=set(), failed_units=[],
    )
    assert outcomes[0]["outcome"] == "unaccounted"
...
def test_serve_outcome_follows_incarnation_probe_when_provided():
    ...
    alive is unaccounted — even when a moor-serve unit was restarted."""
    plan = _plan(_serve("default", 900), _serve("default", 901, kind="dashboard"))
    outcomes = match_runtime_outcomes(
        plan, restarted_services=["moor-serve.service"], relaunched_profiles=[],
        externally_supervised_profiles=[], killed_pids=set(), failed_units=[],
...
def test_unaccounted_serve_report_names_serve_remedy_not_gateway_restart(capsys):
    ...
    assert "moor-serve.service" in out
