...
One gateway per host serves every profile. ``moor -p X gateway install|start`` used to refuse only
...
``--force`` and pointed at ``moor gateway install`` (default) / ``moor gateway migrate --multiplex``;
...
def test_named_profile_install_and_start_refuse_without_force_when_no_multiplexer_runs(quiet_host):
    ...
    for profile in ("coder", "ops", "coder"):
        ...
        for verb in ("install", "start"):
            ...
            assert "moor gateway install" in out and "moor gateway migrate --multiplex" in out
            assert f"moor -p {profile} gateway install --force" in out
