...
subparser, placing the flag BEFORE the subcommand (e.g. 'moor --yolo chat')
...
def _build_parser():
    """Build the moor argument parser from the real code.
...
class TestChatVerboseArg:
    ...
    def test_cmd_chat_forwards_none_when_verbose_is_absent(self, monkeypatch):
        ...
        fake_banner = types.ModuleType("moor_cli.banner")
        ...
        monkeypatch.setitem(sys.modules, "moor_cli.banner", fake_banner)
...
class TestYoloEnvVar:
    """Verify --yolo sets MOOR_YOLO_MODE regardless of flag position.
...
class TestAcceptHooksOnAgentSubparsers:
    ...
    parser and `chat`, so `moor gateway run --accept-hooks` failed
    ...
    # One driver subprocess parses ALL argvs: moor_cli.main is a very heavy
    # import (previously 11 separate `python -m moor_cli.main` spawns with a
...
"""
    ...
    def test_accepted_at_every_position(self):
        """Every `moor <argv>` must exit 0 (help) rather than failing
        with `unrecognized arguments`."""
        import json
        import subprocess
...
class TestChatSubparserInheritedValueFlags:
    ...
    Regression test for #28780: `moor -t web chat` silently dropped the
    ...
    Uses the real `moor_cli._parser.build_top_level_parser()` rather than
    ...
    def test_chat_subparser_inherited_value_flags_use_suppress(self):
        ...
        assert not offenders, (
            ...
            "top-level value when used as `moor <flag> <value> chat`:\n  "
