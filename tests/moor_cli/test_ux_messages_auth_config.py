...
class TestConfigParseWarning:
    ...
    def test_warning_names_repair_commands_and_line(self, tmp_path, monkeypatch, capsys, fallback):
        ...
        assert "line " in err
        assert "`moor config edit`" in err and "`moor config check`" in err
...
class TestConfigGuardCopy:
    def test_require_parseable_user_config_says_what_happened_and_how_to_fix(self, tmp_path, monkeypatch):
        ...
        assert msg.startswith("Moor stopped because your settings file")
        assert "`moor config edit`" in msg and "--ignore-user-config" in msg
    ...
    def test_refuse_write_names_repair_commands_and_absolute_backup_dir(self, tmp_path, monkeypatch):
        ...
        assert "`moor config edit`" in msg and "`moor config check`" in msg
        assert f"{display_moor_home()}/backups/config/" in msg
...
def test_no_provider_configured_points_at_model_login_and_auth_add(monkeypatch, tmp_path):
    ...
    assert "`moor model`" in msg and "`/login`" in msg and "`moor auth add <provider>`" in msg
    assert "~/.moor/.env" not in msg or display_moor_home() == "~/.moor"
    assert f"{display_moor_home()}/.env" in msg
    assert "inference provider" not in msg


# ── cli-07: Moor device-code sign-in failures ────────────────────────────────────────────────


class TestSignInFailureCopy:
    @pytest.mark.parametrize("code, pointer", [
        ("expired_token", "`moor portal`"),
        ("access_denied", "`moor model`"),
    ])
    def test_poll_error_codes_map_to_plain_copy(self, code, pointer):
        ...
        with pytest.raises(RuntimeError) as excinfo:
            auth_device_flow._poll_for_token(
                client=_Client(), portal_base_url="https://portal.nousresearch.com", client_id="moor-cli",
                device_code="device", expires_in=5, poll_interval=1)

        msg = str(excinfo.value)
        assert not msg.startswith(code)
        assert "`moor portal`" in msg and pointer in msg
    ...
    def test_network_failure_names_host_and_retry_command(self):
        ...
        assert "portal.nousresearch.com" in lines[0] and "`moor portal`" in lines[0]
    ...
    def test_run_login_prints_plain_copy_not_raw_exception(self, capsys):
        from moor_cli.model_setup_flows_common import _run_login
    ...
    def test_login_moor_prints_plain_copy(self, monkeypatch, capsys):
        from moor_cli import auth_moor

        def _boom(**_k):
            raise httpx.ConnectError("[Errno -2] Name or service not known")

        monkeypatch.setattr(auth_moor, "_offer_shared_moor_import", lambda *_a, **_k: None, raising=False)
        monkeypatch.setattr("moor_cli.auth._moor_device_code_login", _boom)
        ...
        with pytest.raises(SystemExit) as excinfo:
            auth_moor._login_moor(args, PROVIDER_REGISTRY["moor"])

        assert excinfo.value.code == 1
        out = capsys.readouterr().out
        assert "Could not sign in" in out and "`moor portal`" in out
...
class TestAuthCommandsCopy:
    def test_unknown_provider_offers_did_you_mean_and_pointers(self, monkeypatch):
        ...
        assert "`moor auth`" in msg and "`moor model`" in msg
    ...
    def test_refresh_failure_names_relogin_command(self, monkeypatch):
        ...
        matched = SimpleNamespace(id="abc", label="device-code", source="device_code", provider="moor",
        ...
        with pytest.raises(SystemExit) as excinfo:
            auth_commands.auth_refresh_command(SimpleNamespace(provider="moor", target=None))

        msg = str(excinfo.value)
        assert "refresh_failed" not in msg and "status now" not in msg
        assert "`moor auth add moor --type oauth`" in msg
...
def test_creds_for_target_provider_names_auth_add_and_model(monkeypatch):
    ...
    assert "`moor auth add kimi-coding`" in msg and "/model" in msg
...
class TestProviderSetupFailureCopy:
    def test_setup_provider_step_reports_reason_and_that_nothing_changed(self, monkeypatch, capsys, tmp_path):
        ...
        monkeypatch.setattr("moor_cli.main.select_provider_and_model", _boom)
    ...
    def test_quick_setup_error_hook_uses_same_copy(self, capsys):
        ...
        lines = provider_setup_failure_lines(RuntimeError("keyring locked"), retry_command="moor portal")
        assert lines[0].startswith("Could not finish connecting a provider")
        assert "moor portal" in " ".join(lines)
...
class TestRunLoginProviderAwareCopy:
    def test_minimax_failure_names_minimax_retry_not_portal(self, capsys):
        ...
        out = capsys.readouterr().out
        assert "moor auth add minimax-oauth" in out
        assert "moor portal" not in out and "portal.nousresearch.com" not in out
        assert "moor login" not in out

    def test_moor_failure_keeps_portal_retry(self, capsys):
        from moor_cli.auth import PROVIDER_REGISTRY
        from moor_cli.model_setup_flows_common import _run_login
        ...
        assert _run_login(_boom, SimpleNamespace(), PROVIDER_REGISTRY["moor"]) is False
        out = capsys.readouterr().out
        assert "moor portal" in out and "portal.nousresearch.com" in out
    ...
    def test_silent_nonzero_system_exit_still_tells_user_how_to_retry(self, capsys):
        ...
        out = capsys.readouterr().out
        assert "Sign-in did not complete" in out and "moor auth add openai-codex" in out
    ...
    def test_system_exit_with_message_surfaces_the_message(self, capsys):
        ...
        out = capsys.readouterr().out
        assert "token endpoint returned 400" in out and "moor model" in out
