...
def moor_home(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv(
        "MOOR_BUNDLED_PLUGINS", str(tmp_path / "empty-bundled")
    )
...
class TestV1Regression:
    def test_v1_manifest_parses_with_defaults(self, moor_home):
        _write_plugin(moor_home / "plugins", "oldie")
        _enable(moor_home, ["oldie"])
        mgr = PluginManager()
    ...
    def test_v1_unknown_fields_do_not_warn_loudly(self, moor_home, caplog):
        _write_plugin(
            moor_home / "plugins", "oldie",
...
class TestV2Parsing:
    def test_v2_fields_parse(self, moor_home):
        _write_plugin(
            moor_home / "plugins", "modern",
            manifest_extra={
                "manifest_version": 2,
                "api_version": 1,
                "license": "MIT",
    ...
    def test_unknown_field_in_v2_warns_but_loads(self, moor_home, caplog):
        _write_plugin(
            moor_home / "plugins", "modern",
    ...
    def test_future_manifest_version_warns_but_loads(self, moor_home, caplog):
        _write_plugin(
            moor_home / "plugins", "fromfuture",
            manifest_extra={
                "manifest_version": SUPPORTED_MANIFEST_VERSION + 5,
            },
        )
        _enable(moor_home, ["fromfuture"])
        with caplog.at_level(logging.WARNING):
            mgr = PluginManager()
            mgr.discover_and_load()
    ...
    def test_malformed_v2_fields_warn_and_degrade(self, moor_home, caplog):
        _write_plugin(
            moor_home / "plugins", "sloppy",
            manifest_extra={
                "manifest_version": 2,
                "api_version": "banana",
...
class TestDependencyOrder:
    def test_dep_registers_before_dependent(self, moor_home):
        ...
        _write_plugin(
            moor_home / "plugins", "aaa-consumer",
            manifest_extra={
                "manifest_version": 2,
        ...
        _write_plugin(
            moor_home / "plugins", "zzz-base",
            register_body="import sys; sys._m2_order.append('zzz-base')",
        )
        _enable(moor_home, ["aaa-consumer", "zzz-base"])
        import sys
    ...
    def test_missing_dep_warns_but_loads(self, moor_home, caplog):
        _write_plugin(
            moor_home / "plugins", "needy",
            manifest_extra={
                "manifest_version": 2,
                "requires_plugins": [{"id": "ghost-plugin"}],
            },
        )
        _enable(moor_home, ["needy"])
        with caplog.at_level(logging.WARNING):
            mgr = PluginManager()
            mgr.discover_and_load()
    ...
    def test_cycle_warns_and_falls_back_alpha(self, moor_home, caplog):
        _write_plugin(
            moor_home / "plugins", "cyc-a",
            manifest_extra={
                "manifest_version": 2,
...
class TestConfigSchema:
    def test_type_mismatch_warns_but_loads(self, moor_home, caplog):
        _write_plugin(
            moor_home / "plugins", "cfgd",
            manifest_extra={
                "manifest_version": 2,
                "config_schema": {
                    "api_url": {"type": "str"},
    ...
    def test_unknown_declared_type_skips_check(self, moor_home, caplog):
        _write_plugin(
            moor_home / "plugins", "weird",
            manifest_extra={
                "manifest_version": 2,
...
class TestPythonDependenciesSeam:
    def test_missing_pip_dep_surfaced_with_hint_not_installed(
        self, moor_home, caplog, monkeypatch
    ):
        calls = []
        import subprocess
    ...
    def test_satisfied_pip_dep_is_quiet(self, moor_home, caplog):
        _write_plugin(
            moor_home / "plugins", "pipok",
            manifest_extra={
                "manifest_version": 2,
                "python_dependencies": ["pyyaml>=5,<7"],
            },
        )
        _enable(moor_home, ["pipok"])
        with caplog.at_level(logging.WARNING):
            mgr = PluginManager()
            mgr.discover_and_load()
...
class TestCtxHasPlugin:
    def test_has_plugin_probe(self, moor_home):
        _write_plugin(moor_home / "plugins", "probe-target")
        _write_plugin(
            moor_home / "plugins", "prober",
            manifest_extra={
                "manifest_version": 2,
        ...
        _enable(moor_home, ["probe-target", "prober"])
        import sys
...
class TestRequiresMoor:
    def test_unsatisfied_requires_moor_skips_without_importing(self, moor_home, monkeypatch):
        ...
        _write_plugin(moor_home / "plugins", "future", manifest_extra={"requires_moor": ">=99.0"},
                      register_body="import sys; sys._rh_future = True")
        _write_plugin(moor_home / "plugins", "current", manifest_extra={"requires_moor": ">=1.2,<2"},
                      register_body="import sys; sys._rh_current = True")
        _enable(moor_home, ["future", "current"])
        try:
            mgr = PluginManager()
            mgr.discover_and_load()
            assert not hasattr(sys, "_rh_future")
            assert "requires moor >=99.0" in (mgr._plugins["future"].error or "")
