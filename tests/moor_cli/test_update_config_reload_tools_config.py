"""``moor update`` runs config migrations in the PRE-pull updater process. A migration step
that imports a helper at call time (``_migrate_to_45`` → ``moor_cli.tools_config``) resolves
...
The class fix: the migration step evicts every cached Moor module (``_purge_stale_moor_modules``,
...
def _restore_sys_modules():
    """The purge under test evicts real Moor modules; put the originals back for later tests."""
...
def test_reload_config_modules_restores_missing_tools_config_symbol():
    ...
    tools_config = sys.modules.get("moor_cli.tools_config")
    if tools_config is None:
        import moor_cli.tools_config as tools_config  # noqa: F811
    assert hasattr(tools_config, "_configurable_keys")

    del tools_config._configurable_keys
    try:
        from moor_cli.update_cmd_config import _reload_config_modules
        _reload_config_modules()
        assert hasattr(sys.modules["moor_cli.tools_config"], "_configurable_keys")
    finally:
        reloaded = sys.modules["moor_cli.tools_config"]
...
def test_update_migration_import_failure_after_purge_prints_fallback(tmp_path, monkeypatch, capsys):
    """The purge protects ``moor_constants`` (identity-bearing ContextVar state), so the
    post-purge ``from moor_cli.config import ...`` re-executes NEW config.py against that OLD
    root module and can raise ImportError. That must print the 'run moor config migrate' fallback and
    ...
    monkeypatch.setenv("MOOR_HOME", str(home))
    import moor_cli.config  # noqa: F401  (pre-pull world has config cached)
    import moor_constants
    # A pre-pull root module lacking a symbol the post-pull moor_cli/config.py imports at module level.
    monkeypatch.delattr(moor_constants, "get_process_moor_home")
    ...
    assert "moor config migrate" in out, out
