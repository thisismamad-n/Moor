...
from moor_cli.models_pricing import compute_sale_discount, fetch_models_with_pricing
...
def test_free_model_gets_flat_100_percent_discount():
    ...
    # "0.0000000000" strings (Moor portal shape) count as free too.
...
def test_fetch_models_with_pricing_copies_nested_original(monkeypatch):
    ...
    # Moor Portal opts in via include_sale_original=True.
...
def test_resolve_moor_pricing_credentials_honors_inference_env_override(monkeypatch):
    """Staging profiles set MOOR_INFERENCE_BASE_URL — pricing must follow it.
    ...
    api_key, base_url = models_pricing._resolve_moor_pricing_credentials()
    assert api_key == ""
...
def test_resolve_moor_pricing_credentials_normalizes_either_suffix(monkeypatch):
    """``/v1`` on the override is optional and must not change the result."""
    monkeypatch.setattr(
        "moor_cli.auth.resolve_moor_runtime_credentials", lambda: None
    )
    for override in (
        ...
        monkeypatch.setenv("MOOR_INFERENCE_BASE_URL", override)
        assert models_pricing._resolve_moor_pricing_credentials()[1] == (
            "https://stg-inference-api.nousresearch.com"
        )
