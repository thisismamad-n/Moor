...
A ``moor update`` killed after git pull advanced HEAD but before the
...
- next ``moor update`` with git already up to date still runs the
...
def _patch_update_deps(monkeypatch, tmp_path, run_side_effect):
    ...
    monkeypatch.setattr(moor_main, "_run_pre_update_backup", lambda *a, **k: None)
    monkeypatch.setattr(
        moor_main, "_pause_windows_gateways_for_update", lambda: None
    )
    monkeypatch.setattr(
        moor_main, "_resume_windows_gateways_after_update", lambda *a, **k: None
    )
    ...
    monkeypatch.setattr(moor_main, "_build_web_ui", lambda *a, **k: None)
    ...
    monkeypatch.setattr(
        moor_gateway, "find_gateway_pids", lambda **_kwargs: []
    )
    monkeypatch.setattr(moor_gateway, "supports_systemd_services", lambda: False)
    monkeypatch.setattr(
        moor_gateway, "find_profile_gateway_processes", lambda *a, **k: []
    )
    monkeypatch.setattr(
    ...
    monkeypatch.setattr(
        "moor_cli.update_inventory.collect_runtime_inventory",
...
def test_marker_round_trip_under_moor_home():
    path = update_cmd._fleet_restart_pending_marker_path()
    assert path.parent == get_moor_home()
...
def test_run_pending_restart_true_when_no_gateways(monkeypatch, capsys):
    monkeypatch.setattr(
        "moor_cli.gateway.find_gateway_pids", lambda **k: []
    )
    monkeypatch.setattr(moor_main, "_purge_stale_moor_modules", lambda: None)
...
def test_clean_update_escalates_surviving_serve_as_unaccounted(
    ...
    ``moor-gateway.service``) and an unmanaged ``serve`` on the same
...
def test_interrupt_between_pull_and_restart_leaves_marker(
    ...
    monkeypatch.setattr(moor_main, "_clear_bytecode_cache", _interrupt)
...
def test_startup_warn_prints_when_marker_present(capsys):
    ...
    assert "moor gateway restart" in err
