...
def _write_auth_store(moor_home, payload):
...
def test_resolver_recovers_when_probe_confirms_reset(tmp_path, monkeypatch):
    ...
    _write_auth_store(moor_home, _pool_only_rate_limited_store())
    ...
    store = json.loads((moor_home / "auth.json").read_text())
...
def test_pool_probe_not_fired_for_non_quota_exhaustion(tmp_path, monkeypatch):
    ...
    _write_auth_store(tmp_path / "moor", store)
