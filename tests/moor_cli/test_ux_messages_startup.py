"""Invariant tests for the plain-language startup messages of the ``moor`` CLI.

Contracts (not snapshots): a mistyped subcommand names the typo, offers the closest
match and ``moor --help`` without dumping the 70-entry choice list; a rejected
profile name explains the rule; missing optional dependencies point at ``moor update``.
"""
...
def test_unknown_subcommand_names_typo_suggests_closest_and_hides_choice_list():
    ...
    assert "'sesions' is not a `moor` command" in text
    assert "Did you mean: sessions" in text
    assert "moor --help" in text
...
def test_unknown_subcommand_without_close_match_still_points_at_help():
    ...
    assert "'zzqx' is not a `moor` command" in text
...
def test_nested_group_typo_names_the_group_and_suggests():
    ...
    assert "'stat' is not a `moor gateway` command" in text
...
def test_non_choice_errors_keep_argparse_usage_line():
    ...
    assert text.startswith("usage: moor")
...
def test_invalid_profile_flag_value_explains_rule_and_exits(monkeypatch):
    ...
    monkeypatch.setattr(sys, "argv", ["moor", "-p", "Work Bot", "status"])
    err = io.StringIO()
    with redirect_stderr(err), pytest.raises(SystemExit) as exc:
    ...
    assert "moor profile list" in text
...
def test_option_looking_dash_p_value_is_a_silent_skip_even_under_moor(monkeypatch):
    ...
    assert _main._scan_profile_flag(sys.argv[1:]) == (None, 0, None)
    monkeypatch.setattr(sys, "argv", ["moor", "-p", "--flag"])
    assert _main._scan_profile_flag(sys.argv[1:]) == (None, 0, None)
...
def test_invalid_dash_p_after_a_subcommand_is_left_to_that_subcommand(monkeypatch):
    ...
    # A plugin/subcommand flag such as `moor kanban serve -p "Work Bot"` is not our profile selector.
    monkeypatch.setattr(sys, "argv", ["moor", "kanban", "serve", "-p", "Work Bot"])
    assert _main._scan_profile_flag(sys.argv[1:]) == (None, 0, None)
...
def test_bare_continue_with_no_session_names_the_next_step(monkeypatch):
    ...
    monkeypatch.setattr(
        "moor_cli.terminal_breadcrumbs.resolve_breadcrumb_session", lambda: None, raising=False
    )
    out = io.StringIO()
    ...
    text = out.getvalue()
    assert "moor sessions list" in text
...
def test_model_picker_does_not_warn_about_missing_provider(monkeypatch, capsys):
    ...
    monkeypatch.setattr("moor_cli.auth.resolve_provider", _raise)
...
def test_missing_dashboard_deps_point_at_moor_update():
...
def test_missing_node_message_names_cli_fallback(monkeypatch, capsys):
    from moor_cli import main_tui_launch as _tui

    monkeypatch.setattr("moor_constants.find_node_executable", lambda _b: None)
    monkeypatch.setattr("moor_cli.dep_ensure.ensure_dependency", lambda _n: False, raising=False)
    monkeypatch.delenv("MOOR_NODE", raising=False)
    with pytest.raises(SystemExit):
        _tui._tui_node_bin("node")
    text = capsys.readouterr().out
    assert "moor --cli" in text
