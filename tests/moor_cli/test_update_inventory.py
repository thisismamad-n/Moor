"""Tests for moor_cli.update_inventory — the plan phase (#91277 Phase 2)."""
...
def fleet(monkeypatch, tmp_path):
    ...
    import re
    monkeypatch.setattr("moor_cli.profiles._get_default_moor_home", lambda: default_home)
    monkeypatch.setattr("moor_cli.profiles._get_profiles_root", lambda: default_home / "profiles")
    monkeypatch.setattr("moor_cli.profiles._PROFILE_ID_RE", re.compile(r"^[a-z0-9][a-z0-9_-]*$"), raising=False)
    ...
    monkeypatch.setattr("moor_cli.config.detect_install_method", lambda *a, **k: "git")
    monkeypatch.setattr("moor_cli.config.get_managed_system", lambda: None)
    return tmp_path
...
class TestCollectInventory:
    def test_two_profile_fleet(self, fleet):
        ...
        from moor_cli.update_inventory import describe_restart_mechanism

        assert "moor -p work gateway restart" in describe_restart_mechanism(
            by_profile["work"].restart_via, "work"
        )

    def test_docker_install_not_updatable_in_place(self, fleet, monkeypatch):
        monkeypatch.setattr("moor_cli.config.detect_install_method", lambda *a, **k: "docker")
        monkeypatch.setattr(
            "moor_cli.config.recommended_update_command_for_method",
    ...
    def test_pid_file_fallback_covers_unstamped_profiles(self, fleet, monkeypatch):
        ...
        from moor_cli.gateway import ProfileGatewayProcess
    ...
    def test_never_raises_when_everything_fails(self, monkeypatch):
        ...
        for target in (
            "moor_cli.config.detect_install_method",
            "moor_cli.build_info.get_code_identity",
            "moor_cli.profiles._get_default_moor_home",
...
class TestReceiptIntegration:
    def test_plan_recorded_into_active_receipt(self, fleet, monkeypatch, tmp_path):
        ...
        home.mkdir()
        monkeypatch.setattr("moor_cli.config.get_moor_home", lambda: home, raising=False)
        ur._current = None
