...
Regression: the old implementation wrote ``moor_conversation_<ts>.json``
to the current working directory (CWD). Users who ran /save expected the
file to be discoverable via ``moor sessions browse``, but CWD-resident
snapshots are not indexed in the state DB and are generally invisible.
The fix writes snapshots under ``~/.moor/sessions/saved/`` and prints
...
def moor_home(tmp_path, monkeypatch):
    ...
    # Clear any cached moor_home computation
    import moor_constants
    if hasattr(moor_constants, "_moor_home_cache"):
        moor_constants._moor_home_cache = None
    return home
...
def test_save_conversation_writes_under_moor_home(moor_home, tmp_path, monkeypatch, capsys):
    """Snapshot must land under ~/.moor/sessions/saved/, not CWD."""
    # Change CWD to a different directory to prove the file does NOT go there.
    work = tmp_path / "somewhere-else"
    work.mkdir()
    monkeypatch.chdir(work)

    # Import fresh to pick up the MOOR_HOME fixture
    ...
    cli.MoorCLI.save_conversation(stub, "/save json")

    # File must NOT be in CWD
    cwd_leak = list(work.glob("moor_conversation_*.json"))
    assert not cwd_leak, f"snapshot leaked to CWD: {cwd_leak}"

    # File MUST be under ~/.moor/sessions/saved/
    saved_dir = moor_home / "sessions" / "saved"
    assert saved_dir.is_dir(), "expected saved/ subdirectory to be created"
    files = list(saved_dir.glob("moor_conversation_*.json"))
    ...
    assert "moor --resume 20260101_120000_abc123" in out, out


def test_save_conversation_empty_history_does_nothing(moor_home, capsys):
    for mod in [m for m in sys.modules if m.startswith("cli") or m == "moor_constants"]:
        sys.modules.pop(mod, None)
    import cli
...
def test_save_conversation_bad_format_shows_usage(moor_home, capsys):
    ...
    cli.MoorCLI.save_conversation(stub, "/save pdf")
