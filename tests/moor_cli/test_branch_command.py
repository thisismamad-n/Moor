...
def session_db(tmp_path):
    ...
    os.environ["MOOR_HOME"] = str(tmp_path / ".moor")
    os.makedirs(tmp_path / ".moor", exist_ok=True)
    from moor_state import SessionDB
    db = SessionDB(db_path=tmp_path / ".moor" / "test_sessions.db")
    yield db
    db.close()
...
def cli_instance(tmp_path, session_db):
    """Create a minimal MoorCLI-like object for testing _handle_branch_command."""
...
class TestBranchCommandCLI:
    ...
    def test_branch_with_custom_name(self, cli_instance, session_db):
        ...
        MoorCLI._handle_branch_command(cli_instance, "/branch refactor approach")
