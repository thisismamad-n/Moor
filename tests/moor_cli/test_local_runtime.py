"""Contract tests for moor_cli.local_runtime — Rollouts 1+2.

Per the design's verification plan: relationships and contracts, no
change-detector tests, real imports against temp MOOR_HOME (the autouse
...
def test_llamacpp_endpoint_stale_state_falls_through(tmp_path, monkeypatch):
    ...
    with socket.socket() as s:
        ...
        dead_port = s.getsockname()[1]
    from moor_cli.local_runtime import endpoint as ep
    from moor_cli.local_runtime.detect import DEFAULT_PROBE_PORTS
    from moor_cli.local_runtime.supervisor import state_path
...
def test_resolution_kicks_boot_when_no_thread_is_booting(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("MOOR_HOME", str(tmp_path / ".moor"))
    from moor_cli.local_runtime import bootstrap as bs
    from moor_cli.local_runtime import endpoint as ep
    from moor_cli.local_runtime.supervisor import state_path
...
def test_endpoint_identity_stable_across_supervisor_instances(tmp_path, monkeypatch):
    ...
    key_file = tmp_path / ".moor" / "runtimes" / "llamacpp" / ".api_key"
