...
def test_env_var_names_the_managing_system(moor_home, monkeypatch, env_value, expected):
    if env_value is None:
        monkeypatch.delenv("MOOR_MANAGED", raising=False)
    else:
        monkeypatch.setenv("MOOR_MANAGED", env_value)
...
def test_marker_file_names_the_managing_system(
    moor_home, monkeypatch, marker_text, expected
):
    """An interactive shell reads .managed, not the MOOR_MANAGED of the service."""
    (moor_home / ".managed").write_text(marker_text, encoding="utf-8")
    monkeypatch.delenv("MOOR_MANAGED", raising=False)
...
def test_env_var_wins_over_the_marker(moor_home, monkeypatch):
    (moor_home / ".managed").write_text("nixos", encoding="utf-8")
    monkeypatch.setenv("MOOR_MANAGED", "home-manager")
...
def test_managed_install_names_its_system_and_offers_an_update(
    moor_home, monkeypatch, tmp_path, managed_value
):
    """The message names the system, so the user knows what owns the install."""
    monkeypatch.setenv("MOOR_MANAGED", managed_value)
    ...
    # `moor update` cannot run on a managed install, so the advice must not
    # name it.
    assert config_mod.recommended_update_command() != "moor update"
...
def test_unmanaged_install_offers_no_update_command(moor_home, monkeypatch):
    monkeypatch.delenv("MOOR_MANAGED", raising=False)
    assert config_mod.get_managed_update_command() is None


def test_unreadable_marker_still_reports_managed(moor_home, monkeypatch):
    """A marker we cannot read is still a marker. Fail closed, not open."""
    marker = moor_home / ".managed"
