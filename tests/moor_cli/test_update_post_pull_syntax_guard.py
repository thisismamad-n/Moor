"""Tests for the post-pull syntax guard in ``moor update``.

When a bad commit lands on ``main`` with a syntax error in a critical file
(e.g. orphan merge-conflict markers in ``moor_cli/config.py``), the CLI
becomes unbootable — every ``moor`` invocation imports those files at
...
markers in ``moor_cli/config.py``; users who ran ``moor update`` in
the 7-minute window before #28458 landed could not run any ``moor``
command afterward.
"""
...
# release; if a future ``moor update`` would brick users, this test fails
# in CI first.
# ---------------------------------------------------------------------------
