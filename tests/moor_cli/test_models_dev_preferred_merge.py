...
These guard the contract:
  ...
  * OpenRouter and Moor Portal are NEVER merged — they keep their curated
    (OpenRouter) or live-Portal (Moor) semantics.
...
appear in ``/model`` without a Moor release.
"""
...
class TestProviderModelIdsPreferred:
    ...
    def test_k3_live_discovery_is_scoped_to_kimi_coding_endpoint(self):
        ...
        with patch("moor_cli.urllib_security.open_credentialed_url", side_effect=fake_open):
            with patch(
    ...
    def test_kimi_setup_flow_uses_same_coding_plan_catalog(self):
        ...
        from moor_cli.model_setup_flows import _model_flow_kimi
...
class TestOpenRouterAndMoorUnchanged:
    """Per Teknium: openrouter and moor are NEVER merged with models.dev."""


    def test_openrouter_does_not_call_merge(self):
        """openrouter takes its own live path — merge helper must NOT run."""
        with patch(
            "moor_cli.models._merge_with_models_dev",
