"""Tests for the stale-dashboard handling run at the end of ``moor update``.

``moor update`` detects ``moor dashboard`` processes left over from the
...
from moor_cli.update_cmd import _finish_dashboard_update_cleanup
from moor_cli.main_dashboard import _restart_managed_dashboard_service
from moor_cli.dashboard_procs import _kill_stale_dashboard_processes as _warn_stale_dashboard_processes
...
def _write_valid_ssh_backend_lock(tmp_path, monkeypatch) -> int:
    ...
    (lock_dir / "backend.lock.json").write_text(json.dumps({
        ...
        "port": 46369,
        "profile": "default",
        "moorPath": "/opt/moor/bin/moor",
        "moorHome": str(tmp_path),
        "logPath": f"{tmp_path}/desktop-ssh/{ownership_id}/{spawn_nonce}.log",
        "startedAt": "2026-08-21T15:27:39Z",
    }))
    monkeypatch.setenv("MOOR_HOME", str(tmp_path))
    monkeypatch.delenv("MOOR_DESKTOP_CHILD_PID", raising=False)
    return pid
...
def test_update_cleanup_spares_backend_owned_by_valid_ssh_lock(tmp_path, monkeypatch):
    ...
    with patch(
        "moor_cli.main_dashboard._find_stale_dashboard_pids",
    ...
    def test_self_pid_excluded(self):
        with patch("subprocess.run") as mock_run:
            mock_run.return_value = MagicMock(
                returncode=0,
                stdout="\n".join([
                    _ps_line(os.getpid(), "python3 -m moor_cli.main dashboard"),
                    _ps_line(12345, "moor dashboard --port 9119"),
                ]) + "\n",
                stderr="",
            )
...
class TestKillStaleDashboardPosix:
    ...
    def test_sigterm_graceful_exit(self, capsys):
        ...
        with patch("moor_cli.main_dashboard._find_stale_dashboard_pids",
    ...
    def test_user_scope_restart_never_falls_back_to_system_or_sudo(self, capsys):
        ...
        def fake_run(args, *a, **kw):
            ...
            if args == ["systemctl", "--user", "list-unit-files", "moor-dashboard.service", "--no-legend", "--no-pager"]:
                return MagicMock(returncode=0, stdout="moor-dashboard.service enabled enabled\n", stderr="")
            if args == ["systemctl", "--user", "is-active", "moor-dashboard.service"]:
                return MagicMock(returncode=0, stdout="active\n", stderr="")
            if args == ["systemctl", "--user", "is-enabled", "moor-dashboard.service"]:
                return MagicMock(returncode=0, stdout="enabled\n", stderr="")
            if args == ["systemctl", "--user", "restart", "moor-dashboard.service"]:
                return MagicMock(returncode=0, stdout="", stderr="")
            raise AssertionError(f"unexpected subprocess.run call: {args}")

        with patch("subprocess.run", side_effect=fake_run), \
             patch("moor_cli.main_dashboard._find_stale_dashboard_pids", return_value=[]) as find_pids, \
             patch("os.kill") as kill:
            _kill_stale_dashboard_processes(restart_managed=True)

        assert calls == [
            ["systemctl", "--user", "list-unit-files", "moor-dashboard.service", "--no-legend", "--no-pager"],
            ["systemctl", "--user", "is-active", "moor-dashboard.service"],
            ["systemctl", "--user", "is-enabled", "moor-dashboard.service"],
            ["systemctl", "--user", "restart", "moor-dashboard.service"],
        ]
        ...
        assert "✓ restarted moor-dashboard.service" in capsys.readouterr().out
...
class TestKillStaleDashboardWindows:
    ...
    def test_taskkill_invoked_for_each_pid(self, capsys):
        ...
        with patch("moor_cli.main_dashboard._find_stale_dashboard_pids",
             ...
             patch("moor_cli._subprocess_compat.pid_is_moor", return_value=True), \
...
class TestWindowsWmicEncoding:
    ...
    `moor update` on non-UTF-8 system locales (e.g. cp936 on zh-CN).
    """
    ...
    def test_wmic_routed_through_bounded_probe_run_with_ignore_errors(self):
        ...
        with patch("sys.platform", "win32"), \
             patch("moor_cli._subprocess_compat.bounded_probe_run") as mock_probe:
            mock_probe.return_value = subprocess.CompletedProcess(
                args=["wmic"],
                returncode=0,
                stdout=(
                    "CommandLine=python -m moor_cli.main dashboard\n"
                    "ProcessId=12345\n"
                ),
                stderr="",
            )
    ...
    def test_probe_failure_fails_open_to_empty_list(self):
        ...
        with patch("sys.platform", "win32"), \
             patch("moor_cli._subprocess_compat.bounded_probe_run",
                   return_value=None):
            assert _find_stale_dashboard_pids() == []
...
class TestSupervisedBackendRestart:
    ...
    def test_supervised_pid_restarts_owning_unit(self, capsys):
        ...
        with patch.object(main_dashboard, "_restart_managed_dashboard_service", return_value=False), \
             ...
             patch.object(main_dashboard, "_get_pid_cgroup_path",
                          return_value="/system.slice/moor-serve.service"), \
             patch.object(main_dashboard, "_get_systemd_service_for_pid",
                          return_value="moor-serve.service"), \
        ...
        restart.assert_called_once_with(
            "moor-serve.service", "/system.slice/moor-serve.service"
        )
        out = capsys.readouterr().out
        assert "✓ restarted systemd service moor-serve.service" in out
    ...
    def test_already_restarted_unit_is_left_untouched(self):
        """Review on #83595: moor update's systemd fleet-restart loop may
        ...
        with patch.object(main_dashboard, "_restart_managed_dashboard_service", return_value=False), \
             ...
             patch("time.sleep"):
            result = _kill_stale_dashboard_processes(
                restart_managed=True, already_restarted_units={"moor-serve"}
            )
    ...
    def test_respawn_adds_no_open_to_dashboard_commands(self, tmp_path, monkeypatch):
        ...
        with patch.object(live.subprocess, "Popen", _FakePopen):
            failed = live._respawn_dashboard_processes([
                ["moor", "dashboard", "--port", "8300"],
                ["moor", "serve", "--host", "0.0.0.0"],
            ])

        assert failed == []
        assert spawned[0] == ["moor", "dashboard", "--port", "8300", "--no-open"]
        assert spawned[1] == ["moor", "serve", "--host", "0.0.0.0"]
...
class TestFilterDashboardRespawnCandidates:
    ...
    def test_skips_serve_port_zero(self):
        ...
        assert _filter_dashboard_respawn_candidates([
            (42, argv, "/home/u/.moor/profiles/mini-cat"),
        ]) == []
    ...
    def test_keeps_ppid1_fixed_port_for_repeat_update(self):
        ...
        argv = ["moor", "dashboard", "--port", "9119"]
        assert _filter_dashboard_respawn_candidates([(10, argv, None)]) == [argv]

    def test_dedupes_identical_normalized_cmdlines(self):
        from moor_cli.dashboard_procs import _filter_dashboard_respawn_candidates

        a = ["/usr/bin/python3", "-m", "moor_cli.main", "dashboard", "--port", "8300"]
        b = ["/other/python", "-m", "moor_cli.main", "dashboard", "--port", "8300"]
    ...
    def test_caps_one_per_profile(self):
        ...
        a = ["moor", "--profile", "coder", "dashboard", "--port", "8300"]
        b = ["moor", "--profile", "coder", "dashboard", "--port", "8301"]
        c = ["moor", "--profile", "writer", "dashboard", "--port", "8302"]
        out = _filter_dashboard_respawn_candidates([
            (1, a, None),
            (2, b, None),
            (3, c, None),
    ...
    def test_caps_one_per_moor_home(self):
        from moor_cli.dashboard_procs import _filter_dashboard_respawn_candidates

        home = "/tmp/moor-home-a"
        a = ["moor", "dashboard", "--port", "8300"]
        b = ["moor", "dashboard", "--port", "8301"]
        out = _filter_dashboard_respawn_candidates(
            [
                (1, a, home),
                (2, b, home),
    ...
    def test_profile_flag_and_profiles_home_share_cap(self):
        ...
        out = _filter_dashboard_respawn_candidates([
            (1, a, None),
            (2, b, "/home/u/.moor/profiles/coder"),
        ])
        assert out == [a]
    ...
    def test_default_profile_same_root_home_caps(self):
        ...
        a = ["moor", "--profile", "default", "dashboard", "--port", "8300"]
        b = ["moor", "dashboard", "--port", "8301"]
        home = "/home/u/.moor"
        out = _filter_dashboard_respawn_candidates(
            [
    ...
    def test_foreign_home_backend_is_not_replayed(self):
        """A backend from another MOOR_HOME is never respawned (#94030).
        ...
        out = _filter_dashboard_respawn_candidates(
            [
                (1, a, "/home/u/.moor"),
                (2, b, "/work/project/.moor"),
            ],
            own_home="/home/u/.moor",
        )
        assert out == [a]
    ...
    def test_skips_sidecar_fixed_port_serve_on_foreign_home(self):
        ...
        argv = [
            "/Users/u/.moor-sidecar/moor-agent/venv/bin/python",
            "-m", "moor_cli.main",
            "serve", "--host", "127.0.0.1", "--port", "9118", "--skip-build",
        ]
        out = _filter_dashboard_respawn_candidates(
            [(15364, argv, "/Users/u/.moor-lifeos")],
            own_home="/Users/u/.moor",
        )
        assert out == []

    def test_matching_moor_home_is_kept(self):
        from moor_cli.dashboard_procs import _filter_dashboard_respawn_candidates

        argv = ["moor", "serve", "--host", "127.0.0.1", "--port", "9118"]
        out = _filter_dashboard_respawn_candidates(
            [(15364, argv, "/Users/u/.moor")],
            own_home="/Users/u/.moor",
        )
        assert out == [argv]

    def test_symlinked_moor_home_compares_equal(self, tmp_path):
        from moor_cli.dashboard_procs import _filter_dashboard_respawn_candidates
    ...
    def test_unknown_home_stays_eligible(self):
        """Unreadable MOOR_HOME (env probe failed) keeps pre-#94030 behaviour."""
        from moor_cli.dashboard_procs import _filter_dashboard_respawn_candidates
    ...
    def test_own_home_defaults_to_get_moor_home(self, monkeypatch):
        ...
        monkeypatch.setattr(
            moor_constants, "get_moor_home", lambda: Path("/home/u/.moor")
        )
        argv = ["moor", "serve", "--port", "9118"]
        foreign = _filter_dashboard_respawn_candidates([
            (1, argv, "/Users/u/.moor-lifeos"),
        ])
        assert foreign == []
    ...
    def test_keeps_fixed_port_serve(self):
        ...
        argv = ["moor", "serve", "--host", "0.0.0.0", "--port", "9119"]
        assert _filter_dashboard_respawn_candidates([
            (9, argv, None),
        ]) == [argv]
...
class TestCmdlineCapture:
    ...
    def test_reads_proc_cmdline_when_available(self, tmp_path, monkeypatch):
        ...
        proc_file.write_bytes(b"/usr/bin/python3\x00-m\x00moor_cli.main\x00serve\x00")
        ...
        assert argv == ["/usr/bin/python3", "-m", "moor_cli.main", "serve"]
...
class TestPostUpdateStaleModuleReload:
    ...
    ``moor update`` runs in the PRE-pull Python process. When the update
    adds a new symbol to ``moor_cli._subprocess_compat`` (as #87134 added
    ``bounded_probe_run``), the post-update dashboard cleanup's lazy
    ``from moor_cli._subprocess_compat import bounded_probe_run`` hits the
    ...
    def test_node_failures_skip_reload_and_kill(self):
        ...
        with patch.object(update_cmd, "_reload_process_scan_modules") as mock_reload, \
             patch("moor_cli.main._kill_stale_dashboard_processes") as mock_kill:
    ...
    def test_reload_restores_missing_symbol(self):
        ...
        import moor_cli._subprocess_compat as compat
        from moor_cli import update_cmd
        ...
        try:
            ...
            stale = sys.modules["moor_cli._subprocess_compat"]
            assert hasattr(stale, "bounded_probe_run")
        finally:
            importlib.reload(sys.modules["moor_cli._subprocess_compat"])
            importlib.reload(sys.modules["moor_cli.dashboard_procs"])
