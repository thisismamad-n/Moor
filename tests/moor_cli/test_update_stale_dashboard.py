...
def _refresh_bindings_against_live_module():
    """Rebind module-level names to the *current* ``moor_cli.main``.

    Other tests in the suite (notably ``test_env_loader.py`` and
    ``test_skills_subparser.py``) reload or delete ``moor_cli.main`` from
    ``sys.modules``.  When that happens on the same xdist worker before we
    run, our top-of-file ``from moor_cli.main import ...`` bindings end
    up pointing at the *old* module object.  ``patch(\"moor_cli.main.X\")``
    ...
    live = sys.modules.get("moor_cli.main")
    if live is None:
        live = importlib.import_module("moor_cli.main")
...
class TestKillStaleDashboardPosix:
    ...
    def test_user_scope_restart_never_falls_back_to_system_or_sudo(self, capsys):
        ...
        with patch("subprocess.run", side_effect=fake_run), \
             patch("moor_cli.main._find_stale_dashboard_pids", return_value=[12345]) as find_pids, \
...
class TestSupervisedBackendRestart:
    ...
    def _live(self):
        return sys.modules["moor_cli.main"]
...
class TestFilterDashboardRespawnCandidates:
    ...
    def test_distinct_dot_moor_homes_do_not_share_cap(self):
        from moor_cli.dashboard_procs import _filter_dashboard_respawn_candidates
