...
where moor's shared picker expects ``prompt`` / ``completion``; these tests
...
def test_fetch_ai_gateway_models_filters_against_live_catalog():
    ...
    with patch("moor_cli.models._urlopen_model_catalog_request", return_value=_mock_urlopen(payload)):
...
def test_fetch_ai_gateway_models_falls_back_on_error():
    _reset_caches()
    with patch("moor_cli.models._urlopen_model_catalog_request", side_effect=OSError("network")):
        result = fetch_ai_gateway_models(force_refresh=True)
...
def test_fetch_ai_gateway_models_routes_bearer_through_catalog_opener(monkeypatch):
    ...
    with patch("moor_cli.models._urlopen_model_catalog_request", return_value=_mock_urlopen({"data": []})) as opener:
