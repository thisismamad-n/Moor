...
def test_dry_run_prints_plan_without_mutating(monkeypatch, tmp_path, capsys):
    ...
    monkeypatch.setattr(uninstall, "get_moor_home", lambda: moor_home)
    monkeypatch.setattr(uninstall, "_is_default_moor_home", lambda home: False)
    ...
    assert str(moor_home) in output
    assert project_root.exists()
    assert moor_home.exists()
