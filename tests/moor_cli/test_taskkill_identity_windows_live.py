...
- ``gateway.status.terminate_pid(force=True)`` requires a matching
  ...
  mismatched identity.
- ``moor_cli._subprocess_compat.pid_is_moor`` fails closed on foreign
  processes and identity mismatches.
- ``moor_cli.update_cmd._refuse_gateway_ancestor_tree_kill`` refuses to
...
class TestPidIsMoorLive:
    def test_foreign_real_process_is_refused(self):
        """A live non-Moor process (bare python sleeper in a temp-ish argv)
        ...
        try:
            ...
            # no 'moor' token; if the checkout path itself contains one this
            # assertion is environment-dependent, so guard for it.
            if "moor" in sys.executable.lower():
                pytest.skip("interpreter path names moor; probe would match")
            assert pid_is_moor(proc.pid) is False
        finally:
            _cleanup(proc)

    def test_stale_fingerprint_is_refused_even_for_moor_argv(self):
        ...
        try:
            ...
            assert (
                pid_is_moor(proc.pid, expected_start_time=live + 12345) is False
            )
        finally:
            _cleanup(proc)
...
class TestAncestorRefusalLive:
    def test_real_parent_chain_is_refused(self, capsys):
        ...
        from moor_cli.update_cmd import _refuse_gateway_ancestor_tree_kill
