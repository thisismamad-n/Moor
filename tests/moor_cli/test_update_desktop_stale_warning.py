...
#88251: ``moor update`` treated a failed desktop pack as non-fatal, printed
...
import moor_cli.update_cmd_maint as update_cmd_maint
...
def desktop_env(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "moor_constants.with_moor_node_path", lambda: {}, raising=False
    )
    monkeypatch.setattr(
        "moor_constants.display_moor_home", lambda: str(tmp_path), raising=False
    )
...
def test_gateway_exit_code_file_tracks_desktop_rebuild(tmp_path, monkeypatch):
    monkeypatch.setattr(update_cmd, "get_moor_home", lambda: tmp_path)
