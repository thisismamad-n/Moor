...
from moor_cli.local_runtime.hf_browse import (
    HFFileGroup,
    HFModelHit,
    repo_files,
    rough_fit,
...
def test_search_parses_hf_hits(monkeypatch):
    ...
    monkeypatch.setattr("moor_cli.local_runtime.hf_browse._get_json",
                        lambda url: canned)
...
def test_search_route_requires_query_and_maps_errors(client, monkeypatch):
    ...
    monkeypatch.setattr("moor_cli.local_runtime.hf_browse.search_models", boom)
...
def test_browsed_download_stages_and_bounces(client, tmp_path, monkeypatch):
    ...
    from tests.moor_cli.test_local_models_routes import _FakeRangeOpener
