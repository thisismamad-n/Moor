"""Tests for moor_cli.web_server and related config utilities."""
...
import moor_cli.web_routers.status as _rt_status
...
@pytest.fixture
def _install_example_plugin(_isolate_moor_home):
    """Drop the example-dashboard fixture into the per-test MOOR_HOME
    ...
    ``MOOR_BUNDLED_PLUGINS`` override because the bundled dir is
    resolved per-call (other tests in the suite implicitly rely on the
    real bundled plugins — kanban, moor-achievements, model providers
    ...
    user_plugins_dir = get_moor_home() / "plugins"
    ...
    # plugin in exactly as a real operator would with `moor plugins
    # enable example`.
    ...
    #      and start serving requests against a torn-down MOOR_HOME.
    app = web_server.app
...
class TestSessionTokenInjection:
    """The desktop shell mints MOOR_DASHBOARD_SESSION_TOKEN and signs its
    ...
    def test_honors_injected_token(self, monkeypatch):
        ...
        monkeypatch.setenv("MOOR_DASHBOARD_SESSION_TOKEN", "desktop-seeded-token")
...
class TestWebServerEndpoints:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_moor_home):
        """Create a TestClient and isolate the state DB under the test MOOR_HOME."""
        try:
            from starlette.testclient import TestClient
        except ImportError:
    ...
    def test_profiles_sidebar_heals_stale_schema_store(self):
        ...
        array — the desktop rendered "No sessions yet" after `moor update`
    ...
    def test_startup_eager_reconcile_heals_stale_store(self):
        ...
        #79531/#80037: after `moor update` an old-schema state.db used to
    ...
    def test_startup_eager_reconcile_never_raises(self, monkeypatch):
        ...
        monkeypatch.setattr(moor_state, "SessionDB", boom)
    ...
    def test_heal_gives_up_when_reconcile_cannot_fix_the_store(self, monkeypatch):
        ...
        original_init = moor_state.SessionDB.__init__
        ...
        # the class on moor_state covers every open the helper makes.
        monkeypatch.setattr(moor_state.SessionDB, "__init__", counting_init)
    ...
    def test_generic_corruption_does_not_trigger_writable_heal(
        ...
        monkeypatch.setattr(moor_state, "SessionDB", corrupt_open)
    ...
    def test_decode_error_triggers_writable_heal(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(moor_state, "SessionDB", scripted_open)
    ...
    def test_messaging_platforms_profile_scopes_gateway_reads(self, monkeypatch):
        ...
        MOOR_HOME contextvar override (#56986), so /api/messaging/platforms
    ...
    def test_declared_surface_put_writes_config_and_secret(self):
        ...
        config_path = get_moor_home() / "hindsight" / "config.json"
    ...
    def _isolate_honcho_config(self):
        # Honcho tests write the suite-wide MOOR_HOME honcho.json; snapshot and
        # restore it so provider status/config state never leaks across tests.
        from moor_constants import get_moor_home

        path = get_moor_home() / "honcho.json"
    ...
    def test_put_honcho_writes_host_block_root_and_secret(self, monkeypatch, tmp_path):
        ...
        cfg = json.loads((get_moor_home() / "honcho.json").read_text(encoding="utf-8"))
        # baseUrl is root-scoped; the rest live in the active host block.
        assert cfg["baseUrl"] == "https://honcho.example.dev"
        assert cfg["hosts"]["moor"]["workspace"] == "myws"
        assert cfg["hosts"]["moor"]["peerName"] == "eri"
        assert cfg["hosts"]["moor"]["environment"] == "local"
        assert cfg["hosts"]["moor"]["sessionStrategy"] == "per-repo"
        # The key lands where the client reads first; GET keeps it write-only.
        assert cfg["hosts"]["moor"]["apiKey"] == "hch-test-key"
    ...
    def test_get_media_requires_auth(self):
        from moor_cli.web_server import _SESSION_HEADER_NAME
    ...
    def test_update_moor_returns_docker_guidance_without_spawning(self, monkeypatch):
        ...
        def fail_spawn(*_args, **_kwargs):
            nonlocal spawned
            spawned = True
            raise AssertionError("docker update guard should not spawn moor update")
        ...
        # method through moor_cli.config directly.
        monkeypatch.setattr(
            "moor_cli.config.detect_install_method", lambda *_a, **_k: "docker"
        )
        monkeypatch.setattr(_cfg_mod, "detect_install_method", lambda _root: "docker")
        monkeypatch.setattr(_web_server_gateway, "_spawn_moor_action", fail_spawn)
        _web_server_gateway._ACTION_PROCS.pop("moor-update", None)
        _web_server_gateway._ACTION_RESULTS.pop("moor-update", None)

        resp = self.client.post("/api/moor/update")

        assert resp.status_code == 200
        data = resp.json()
        assert data["ok"] is False
        assert data["name"] == "moor-update"
        assert data["pid"] is None
        assert data["error"] == "docker_update_unsupported"
        assert "docker pull nousresearch/hermes-agent:latest" in data["message"]
        assert spawned is False

        status = self.client.get("/api/actions/moor-update/status")
    ...
    def test_update_moor_returns_apt_guidance_without_spawning(self, monkeypatch):
        ...
        def fail_spawn(*_args, **_kwargs):
            nonlocal spawned
            spawned = True
            raise AssertionError("APT-managed update guard should not spawn moor update")

        monkeypatch.setattr(_web_server_files, "_dashboard_local_update_managed_externally", lambda: False)
        # The shared admission gate (#91277 Phase 3) resolves the install
        # method through moor_cli.config directly, so patch it there (the
        # web_server module alias only feeds the /update/check endpoint).
        monkeypatch.setattr(
            "moor_cli.config.detect_install_method", lambda *_a, **_k: "apt"
        )
        ...
        assert data["update_command"] == "pkg upgrade moor-agent"
        assert spawned is False

        check = self.client.get("/api/moor/update/check")
        assert check.status_code == 200
        check_data = check.json()
        assert check_data["install_method"] == "apt"
        assert check_data["can_apply"] is False
        assert check_data["update_command"] == "pkg upgrade moor-agent"
    ...
    def test_update_status_recovers_completed_result_after_dashboard_restart(self, monkeypatch, tmp_path):
        ...
        action_id = "c" * 32
        (tmp_path / "moor-update.log").write_text(
            "=== moor-update started 2026-08-17 11:19:34 ===\n"
            "pulling updates...\n",
            encoding="utf-8",
        )
        (tmp_path / "update.log").write_text(
            "=== moor update started 2026-08-17T11:19:35 ===\n"
            "✓ Update complete!\n"
            f"=== moor-update completed {action_id} ===\n",
            encoding="utf-8",
        )
        ...
        status = self.client.get("/api/actions/moor-update/status?lines=2000")
        ...
        assert f"=== moor-update completed {action_id} ===" in data["lines"]

    def test_update_moor_spawns_with_action_id(self, monkeypatch):
        import moor_cli.web_server as web_server
        ...
        monkeypatch.setattr(_web_server_gateway, "_spawn_moor_action", fake_spawn)
        _web_server_gateway._ACTION_PROCS.pop("moor-update", None)
        _web_server_gateway._ACTION_RESULTS.pop("moor-update", None)
    ...
    def test_update_moor_reuses_running_action(self, monkeypatch):
        ...
        _web_server_gateway._ACTION_PROCS["moor-update"] = Proc()
        _web_server_gateway._ACTION_IDS["moor-update"] = "b" * 32

        try:
            resp = self.client.post("/api/moor/update")
        finally:
            _web_server_gateway._ACTION_PROCS.pop("moor-update", None)
            _web_server_gateway._ACTION_IDS.pop("moor-update", None)
    ...
    def test_platform_scoped_messaging_env_vars_are_channel_managed(self):
        ...
        tests/moor_cli/test_setup_hidden_env.py.
        """
        from moor_cli.web_server_messaging import _MESSAGING_KEYS_PAGE_KEYS, _build_catalog_entry, _channel_managed_env_keys
    ...
    def test_reveal_env_var(self, tmp_path):
        ...
        from moor_cli.web_server import _SESSION_HEADER_NAME, _SESSION_TOKEN
    ...
    def test_reveal_env_var_legacy_authorization_header_still_works(self, tmp_path):
        ...
        from moor_cli.config import save_env_value
        from moor_cli.web_server import _SESSION_TOKEN
    ...
    def test_telegram_onboarding_apply_reports_restart_failure_after_save(
        ...
        def fake_request(method, path, *, body=None, bearer_token=None):
            if method == "POST":
                return {
                    ...
                    "suggested_username": "moor_pair_restart_fails_bot",
                    "deep_link": "https://t.me/newbot/MoorSetupBot/moor_pair_restart_fails_bot",
                    "qr_payload": "https://t.me/newbot/MoorSetupBot/moor_pair_restart_fails_bot",
            ...
            return {
                "status": "ready",
                "bot_username": "moor_pair_restart_fails_bot",
        ...
        monkeypatch.setattr(_web_server_gateway, "_spawn_moor_action", fail_spawn_action)
    ...
    def test_parse_model_ids_handles_openai_and_bare_shapes(self):
        ...
        from moor_cli.web_server_profiles import _parse_model_ids
    ...
    def test_set_model_main_custom_persists_api_key_and_registers_provider(self):
        ...
        ``moor model`` custom flow. Regression for the desktop loop where a
    ...
    def test_deleting_the_active_custom_endpoint_clears_its_model_mirror(self):
        ...
        """
        from moor_cli.config import custom_endpoint_key_env, get_env_value, load_config
    ...
    def test_custom_endpoint_save_scopes_to_the_requested_profile(self):
        ...
        """
        from moor_cli import profiles as profiles_mod
        from moor_cli.config import custom_endpoint_key_env
        from moor_constants import get_moor_home

        default_home = get_moor_home()
        worker_home = profiles_mod.get_profile_dir("worker")
        worker_home.mkdir(parents=True)
    ...
    def test_two_endpoints_on_one_host_keep_separate_credentials(self):
        ...
        the first's key.
        """
        from moor_cli.config import custom_endpoint_key_env, get_env_value
...
class TestBuildSchemaFromConfig:
    ...
    def test_overrides_applied(self):
        from moor_cli.web_server_config import CONFIG_SCHEMA
    ...
    def test_timezone_field_is_searchable_select(self):
        ...
        """
        from moor_cli.web_server_config import CONFIG_SCHEMA, _timezone_options
...
class TestConfigRoundTrip:
    ...
    def test_round_trip_preserves_schema_invisible_nested_keys(self):
        ...
        from moor_cli.config import load_config, read_raw_config, save_config
...
class TestNewEndpoints:
    ...
    @pytest.fixture(autouse=True)
    def _setup(self, monkeypatch, _isolate_moor_home):
        try:
            from starlette.testclient import TestClient
        except ImportError:
            pytest.skip("fastapi/starlette not installed")
    ...
    def test_toolsets_resolve_subscription_features_once(self, monkeypatch):
        import moor_cli.tools_config as tools_config
        from moor_cli.moor_subscription import MoorSubscriptionFeatures

        calls = 0
        features = MoorSubscriptionFeatures(
            subscribed=False,
            moor_auth_present=False,
            provider_is_moor=False,
            features={},
            account_info=None,
        )
    ...
    def test_get_toolset_config_reports_truthful_provider_status(self, monkeypatch):
        ...
        logged-out Moor Subscription rows, xAI TTS without Grok OAuth, and
        ...
        # Logged out of Moor Portal → managed subscription rows need sign-in.
        monkeypatch.setattr(
            "moor_cli.moor_subscription.get_moor_portal_account_info",
            lambda *a, **k: MoorPortalAccountInfo(
        ...
        assert by_name["Moor Subscription"]["status"] == "needs_auth"
    ...
    def test_select_managed_moor_provider_reports_needs_moor_auth(self, monkeypatch):
        ...
        (ensure_moor_portal_access) — so the row never activated and nothing
        told the user to sign in. The endpoint now reports the entitlement
        gap so the client can drive the existing Moor OAuth flow.
        """
        from moor_cli.moor_account import MoorPortalAccountInfo
        ...
        resp = self.client.put(
            ...
            json={"provider": "Moor Subscription (Browser Use cloud)"},
        )
        assert resp.status_code == 200
        data = resp.json()
        assert data["ok"] is True
        assert data["needs_moor_auth"] is True
        assert data["feature"] == "browser"
        # The selection is still persisted — activation is what's gated.
        # Managed rows store the single 'moor' provider string (the runtime
        # maps it to the Browser Use cloud through the Moor Tool Gateway).
        from moor_cli.config import load_config
        cfg = load_config()
        assert cfg["browser"]["cloud_provider"] == "moor"
    ...
    def test_terminal_ssh_probe_ready_when_configured(self, monkeypatch):
        ...
        assert "moor@devbox.example.com" in ssh["detail"]
...
class TestDesktopLoopbackAuthExemption:
    ...
    def test_exempt_with_desktop_env_and_session_token_on_loopback(self, monkeypatch):
        ...
        monkeypatch.setenv("MOOR_DASHBOARD_SESSION_TOKEN", "desktop-minted")
    ...
    def test_not_exempt_without_desktop_env(self, monkeypatch):
        ...
        monkeypatch.setenv("MOOR_DASHBOARD_SESSION_TOKEN", "tok")
    ...
    def test_not_exempt_without_any_credential(self, monkeypatch):
        ...
        # MOOR_DESKTOP=1 alone is not enough: a plain serve with the env var
...
class TestModelContextLength:
    ...
    def test_normalize_extracts_context_length_from_dict(self):
        ...
        from moor_cli.web_server_config import _normalize_config_for_web
...
class TestStatusInstallId:
    ...
    Moor home (not the profile home), survives a fresh process-cache read,
    ...
    def test_status_reports_persistent_install_id(self, monkeypatch):
        ...
        id_file = get_default_moor_root() / "install_id"
    ...
    def test_all_profiles_of_one_install_share_the_id(self, monkeypatch, tmp_path):
        """MOOR_HOME=<root> and MOOR_HOME=<root>/profiles/<name> resolve to
...
class TestNormaliseThemeDefinition:
    ...
    def test_rejects_non_dict(self):
        from moor_cli.web_server_dashboard import _normalise_theme_definition
...
class TestThemeBootstrapCSS:
    ...
    @staticmethod
    def _write_theme(moor_home, name="ocean"):
        themes_dir = moor_home / "dashboard-themes"
    ...
    def test_user_theme_renders_bundle_vars(self, tmp_path, monkeypatch):
        ...
        assert css.startswith('<style id="moor-theme-bootstrap">')
...
class TestDeleteSessionEndpoint:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_moor_home):
        ...
        monkeypatch.setattr(
            moor_state, "DEFAULT_DB_PATH", get_moor_home() / "state.db"
        )
...
class TestBulkDeleteSessionsEndpoint:
    ...
    1. Route-ordering: ``/api/sessions/bulk-delete`` must shadow the
       ...
       the block comment in ``moor_cli/web_server.py``).
    ...
    def test_route_order_not_shadowed_by_session_id(self):
        ...
        assert "deleted" in body, (
            ...
            "registration order in moor_cli/web_server.py."
        )
...
class TestDeleteEmptySessionsEndpoint:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_moor_home):
        ...
        # Pin the SessionDB to the isolated MOOR_HOME so each test
...
class TestPluginAPIAuth:
    ...
    @pytest.fixture(autouse=True)
    def _setup_test_client(self, monkeypatch, _isolate_moor_home, _install_example_plugin):
        ...
        installs it into the per-test ``MOOR_HOME``.
        """
        try:
            from starlette.testclient import TestClient
        except ImportError:
    ...
    def test_plugin_route_allows_auth(self):
        ...
        test fixture (installed into MOOR_HOME by the class-level
    ...
    def test_non_kanban_plugin_route_requires_auth(self):
        ...
        so any plugin's API surface — kanban, moor-achievements, future
        plugins — must require the session token. Hit a non-kanban plugin
        path to lock that in.
        """
        # Real plugin path (moor-achievements is loaded by default).
        resp = self.client.get("/api/plugins/moor-achievements/overview")
        assert resp.status_code == 401
...
class TestDashboardPluginManifestExtensions:
    ...
    def test_user_plugins_ignore_profile_home_override(self, tmp_path, monkeypatch):
        ...
        MOOR_HOME override scopes a request to another profile."""
        from moor_constants import (
            reset_moor_home_override,
            set_moor_home_override,
        )
    ...
    def test_user_plugins_found_under_profile_scoped_process(self, tmp_path, monkeypatch):
        ...
        sets MOOR_HOME=<root>/profiles/<name>) must still discover user
        plugins installed in the moor root's plugins/ directory."""
        root = tmp_path / "moor-root"
...
# or ``sh -c 'printf …'``) instead of the real ``moor --tui`` binary.  The
...
class TestPtyWebSocket:
    ...
    def test_tui_python_command_uses_child_path(self, tmp_path):
        ...
        command = f"moor-review-python{Path(sys.executable).suffix}"
        ...
        assert env["MOOR_PYTHON"] == command
    ...
    def test_unavailable_platform_closes_with_message(self, monkeypatch):
        from moor_cli.pty_bridge import PtyUnavailableError
    ...
    def test_pub_broadcasts_to_events_subscribers(self):
        ...
        """
        import asyncio
        from moor_cli import web_server as ws_mod
...
def test_resolve_chat_argv_injects_gateway_ws_url(monkeypatch):
    ...
    gateway_url = env.get("MOOR_TUI_GATEWAY_URL", "")
...
class TestDashboardPluginStaticAssetAllowlist:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_moor_home, _install_example_plugin):
        ...
        the per-test ``MOOR_HOME`` user-plugins dir.
        """
        try:
            from starlette.testclient import TestClient
        except ImportError:
...
class TestDesktopCronTicker:
    ...
    def test_ticker_runs_when_desktop(self, monkeypatch, _isolate_moor_home):
        ...
        with self._client():
            assert called.wait(3.0), "expected cron tick under MOOR_DESKTOP=1"
...
class TestServeIndexMissingIndex:
    ...
    def test_index_uses_ssh_token_applied_after_spa_mount(
        ...
        assert resp.status_code == 200
        assert 'window.__MOOR_SESSION_TOKEN__="after-mount"' in resp.text


class TestHeadlessServeTokenPage:
    """Headless `moor serve` must serve the Desktop token handshake page
    at `/` when the dashboard auth gate is off (#94227).

    The Electron renderer boots by fetching `/` and extracting
    ``window.__MOOR_SESSION_TOKEN__`` for WebSocket auth. Headless serve
    ...
    def test_root_serves_token_page_when_not_gated(self, monkeypatch):
        ...
        match = re.search(
            r'window\.__MOOR_SESSION_TOKEN__\s*=\s*("(?:\\.|[^"\\])*")',
            resp.text,
        )
        ...
        assert "window.__MOOR_AUTH_REQUIRED__=false" in resp.text
...
class TestHashedAssetCacheHeaders:
    ...
    def test_serve_css_is_immutable_and_keeps_prefix_rewrites(
        ...
        prefixed = client.get(
            ...
            headers={"X-Forwarded-Prefix": "/moor"},
        )
        assert prefixed.status_code == 200
        assert prefixed.headers["cache-control"] == self._IMMUTABLE
        assert "url(/moor/ds-assets/bg.png)" in prefixed.text
        assert "url(/moor/fonts-terminal/x.woff2)" in prefixed.text
