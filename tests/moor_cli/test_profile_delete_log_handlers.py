...
A long-lived multiplex process (the Desktop's ``moor serve`` backend) routes records to every
...
def routed_profile(tmp_path, monkeypatch):
    ...
    moor_logging.setup_logging(moor_home=home, force=True)
    try:
        yield home, profile
    finally:
...
def _log_into(profile: Path, *, adopt: bool) -> None:
    ...
    ``setup_logging(moor_home=<profile>)`` adoption path ``agent_init`` takes."""
    logger = logging.getLogger("agent.tests.profile-delete")
    token = set_moor_home_override(profile)
    try:
        if adopt:
            moor_logging.setup_logging(moor_home=profile)
        else:
            assert moor_logging.enable_profile_log_routing([Path(profile).parent.parent, profile]) is True
...
def _routers() -> list:
    return [h for h in moor_logging._queued_file_handlers
            if isinstance(h, moor_logging._ProfileRoutingFileHandler)]
