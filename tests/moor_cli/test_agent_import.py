"""Tests for moor_cli.agent_import — ``moor import-agent``.
...
Uses the profile_env fixture pattern from tests/moor_cli/test_profiles.py:
Path.home() and MOOR_HOME are redirected to tmp_path so nothing touches
the real ~/.moor.
"""
...
# ---------------------------------------------------------------------------
# Shared fixture: redirect Path.home() and MOOR_HOME (profile_env pattern)
# ---------------------------------------------------------------------------

@pytest.fixture()
def profile_env(tmp_path, monkeypatch):
    """Isolated environment: Path.home() -> tmp_path, MOOR_HOME -> tmp/.moor."""
...
@pytest.fixture()
def moor_home(profile_env):
    return profile_env / ".moor"
...
def run_import(agent, source, moor_home, execute, overwrite=False):
    return AgentImporter(
        agent=agent,
        source_root=source,
        target_root=moor_home,
        execute=execute,
        overwrite=overwrite,
    ).run()
...
class TestDetection:
    ...
    def test_unsupported_agent_raises(self, moor_home, tmp_path):
        with pytest.raises(ValueError):
            AgentImporter("cursor", tmp_path, moor_home)
...
class TestDryRun:
    def test_claude_dry_run_writes_nothing(self, claude_tree, moor_home):
        before = snapshot_tree(moor_home)
        report = run_import("claude-code", claude_tree, moor_home, execute=False)
        assert snapshot_tree(moor_home) == before
        assert report["dry_run"] is True
        assert report["summary"]["imported"] > 0


    def test_dry_run_and_real_run_plan_same_items(self, claude_tree, moor_home):
        preview = run_import("claude-code", claude_tree, moor_home, execute=False)
        applied = run_import("claude-code", claude_tree, moor_home, execute=True)
...
class TestClaudeCodeImport:
    @pytest.fixture()
    def report(self, claude_tree, moor_home):
        return run_import("claude-code", claude_tree, moor_home, execute=True)


    def test_allowlist_lands_in_config_yaml(self, report, moor_home):
        config = yaml.safe_load((moor_home / "config.yaml").read_text(encoding="utf-8"))
...
class TestCodexImport:
    @pytest.fixture()
    def report(self, codex_tree, moor_home):
        return run_import("codex", codex_tree, moor_home, execute=True)


    def test_mcp_servers_from_config_toml(self, report, moor_home):
        config = yaml.safe_load((moor_home / "config.yaml").read_text(encoding="utf-8"))
    ...
    def test_skill_copied(self, report, moor_home):
        assert (moor_home / "skills" / "codex-imports" / "db-migrate" / "SKILL.md").exists()
...
class TestSecretsNeverImported:
    def test_no_secret_values_anywhere_claude(self, claude_tree, moor_home):
        run_import("claude-code", claude_tree, moor_home, execute=True)
        blob = "".join(
            p.read_text(encoding="utf-8", errors="replace")
            for p in moor_home.rglob("*") if p.is_file()
        )
        assert "ghp_SECRET123" not in blob
        assert "Bearer abc123" not in blob
        assert "sk-ant-SUPERSECRET" not in blob

    def test_no_secret_values_anywhere_codex(self, codex_tree, moor_home):
        run_import("codex", codex_tree, moor_home, execute=True)
        blob = "".join(
            p.read_text(encoding="utf-8", errors="replace")
            for p in moor_home.rglob("*") if p.is_file()
        )
    ...
    def test_stripped_secrets_reported(self, claude_tree, moor_home):
    ...
    def test_non_secret_header_kept(self, claude_tree, moor_home):
        run_import("claude-code", claude_tree, moor_home, execute=True)
        config = yaml.safe_load((moor_home / "config.yaml").read_text(encoding="utf-8"))
...
class TestMalformedInputs:
    def test_bad_settings_json_reports_error(self, profile_env, moor_home):
        root = profile_env / ".claude"
        root.mkdir()
        (root / "settings.json").write_text("{not json!!", encoding="utf-8")
        (root / "CLAUDE.md").write_text("- still importable\n", encoding="utf-8")
        report = run_import("claude-code", root, moor_home, execute=True)
        ...
        assert "still importable" in (
            moor_home / "memories" / "MEMORY.md").read_text(encoding="utf-8")


    def test_empty_tree_all_skipped(self, profile_env, moor_home):
        root = profile_env / ".codex"
        root.mkdir()
        report = run_import("codex", root, moor_home, execute=True)
        assert report["summary"]["imported"] == 0
...
class TestMergeSemantics:
    ...
    def test_existing_skill_conflicts_without_overwrite(
            self, claude_tree, moor_home):
        dest = moor_home / "skills" / "claude-code-imports" / "deploy-helper"
    ...
    def test_reimport_is_idempotent_for_memory(self, claude_tree, moor_home):
        run_import("claude-code", claude_tree, moor_home, execute=True)
        first = (moor_home / "memories" / "MEMORY.md").read_text(encoding="utf-8")
        report = run_import("claude-code", claude_tree, moor_home, execute=True)
        assert (moor_home / "memories" / "MEMORY.md").read_text(encoding="utf-8") == first
...
class TestExistingMemoryStorePreserved:
    ...
    @pytest.fixture()
    def seeded_home(self, moor_home):
        memory = moor_home / "memories" / "MEMORY.md"
...
class TestExistingConfigPreserved:
    ...
    @pytest.fixture()
    def config_path(self, moor_home):
        return moor_home / "config.yaml"
    ...
    def test_unreadable_config_is_left_byte_identical(
        ...
        try:
            report = run_import("claude-code", claude_tree, moor_home,
                                execute=True)
            statuses = {
                i["status"]
                for kind in CONFIG_WRITING_KINDS
    ...
    def test_failed_write_does_not_truncate_the_existing_config(
            self, moor_home, config_path, monkeypatch):
        """An interrupted dump leaves the previous file complete."""
        from moor_cli import agent_import
...
class TestCliWiring:
    ...
    def test_command_dry_run_via_cli_writes_nothing(
        ...
        assert not (moor_home / "skills" / "claude-code-imports").exists()
        config_text = (moor_home / "config.yaml").read_text(encoding="utf-8") \
            if (moor_home / "config.yaml").exists() else ""
...
class TestSyncManifest:
    ...
    def test_sync_reimports_changed_source_but_never_clobbers_user_skill(
        ...
        user_skill = moor_home / "skills" / "claude-code-imports" / "hand-rolled"
        ...
        imports = moor_home / "skills" / "claude-code-imports"
        ...
        # An imported skill the user then EDITED locally is no longer moor-owned: the next sync
