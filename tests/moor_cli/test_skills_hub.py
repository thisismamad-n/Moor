...
from moor_cli.skills_hub import do_check, do_install, do_list, do_update, handle_skills_slash
...
def test_do_list_platform_env_is_ignored(three_source_env, monkeypatch):
    ...
    MOOR_HOME (swapped by -p), so it must NOT pass a platform arg to
    ``get_disabled_skill_names`` — otherwise per-platform overrides
    would silently leak in from MOOR_PLATFORM env."""
...
def test_do_search_json_flag_emits_full_identifiers(capsys):
    ...
    from moor_cli.skills_hub import do_search
