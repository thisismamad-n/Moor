...
def _write_auth_store(tmp_path, payload: dict) -> None:
    ...
    (moor_home / "auth.json").write_text(json.dumps(payload, indent=2))
...
def test_auth_add_migrates_display_name_derived_legacy_pool_key(
    ...
    with patch("moor_cli.models.clear_provider_models_cache") as clear_cache:
        auth_add_command(_Args())
...
def test_auth_add_moor_oauth_persists_pool_entry(tmp_path, monkeypatch):
    monkeypatch.setenv("MOOR_HOME", str(tmp_path / "moor"))
    _write_auth_store(tmp_path, {"version": 1, "providers": {}})
    token = _jwt_with_email("moor@example.com")
    monkeypatch.setattr(
        "moor_cli.auth._moor_device_code_login",
        lambda **kwargs: {
    ...
    entries = payload["credential_pool"]["moor"]
    device_code_entries = [
    ...
    # `moor auth add moor` must also populate providers.moor so the
    # 401-recovery path (resolve_moor_runtime_credentials) can refresh an
    # invoke JWT when the token expires. If this mirror is missing, recovery
    # raises "Moor is not logged into Moor Portal" and the agent dies.
...
def test_auth_add_moor_oauth_honors_custom_label(tmp_path, monkeypatch):
    """`moor auth add moor --type oauth --label <name>` must preserve the
    custom label end-to-end — it was silently dropped in the first cut of the
    persist_moor_credentials helper because `--label` wasn't threaded through.
    """
    monkeypatch.setenv("MOOR_HOME", str(tmp_path / "moor"))
    _write_auth_store(tmp_path, {"version": 1, "providers": {}})
    token = _jwt_with_email("moor@example.com")
    ...
    pool_entry = payload["credential_pool"]["moor"][0]
    assert pool_entry["source"] == "device_code"
    assert pool_entry["label"] == "my-moor"

    # … and survives in providers.moor so a subsequent load_pool() re-seeds
    # it without reverting to the auto-derived fingerprint.
    assert payload["providers"]["moor"]["label"] == "my-moor"


def test_auth_add_codex_oauth_keeps_distinct_pool_accounts(tmp_path, monkeypatch):
    """Two ``moor auth add openai-codex`` runs for different ChatGPT
    ...
    second independent one. ``moor auth list`` showed two labels sharing
...
def test_codex_runtime_pool_only_rate_limit_is_not_missing_auth(tmp_path, monkeypatch):
    ...
    from moor_cli.auth import AuthError, CODEX_RATE_LIMITED_CODE, resolve_codex_runtime_credentials
...
def test_auth_add_xai_oauth_sets_active_provider(tmp_path, monkeypatch):
    """moor auth add xai-oauth must set active_provider and write a pool entry.
...
def test_auth_add_xai_oauth_keeps_distinct_pool_accounts(tmp_path, monkeypatch):
    """Two ``moor auth add xai-oauth`` runs must produce independent pool entries.
...
def test_auth_remove_codex_migrates_legacy_dict_suppression(tmp_path, monkeypatch):
    ...
    payload = json.loads((tmp_path / "moor" / "auth.json").read_text(encoding="utf-8"))
...
def test_logout_resets_codex_config_when_auth_state_already_cleared(tmp_path, monkeypatch, capsys):
    """`moor logout --provider openai-codex` must still clear model.provider.
...
def test_unsuppress_credential_source_clears_marker(tmp_path, monkeypatch):
    ...
    from moor_cli.auth import suppress_credential_source, unsuppress_credential_source, is_source_suppressed
...
# =============================================================================
# Unified credential-source stickiness — every source Moor reads from has a
...
def test_seed_from_singletons_respects_moor_pkce_suppression(tmp_path, monkeypatch):
    """anthropic moor_pkce must not re-seed from ~/.moor/.anthropic_oauth.json when suppressed."""
    moor_home = tmp_path / "moor"
    moor_home.mkdir(parents=True, exist_ok=True)
    monkeypatch.setenv("MOOR_HOME", str(moor_home))

    import yaml
    (moor_home / "config.yaml").write_text(yaml.dump({"model": {"provider": "anthropic", "model": "claude"}}))
    ...
    # Stub the readers so only moor_pkce is "available"; claude_code returns None
    import agent.anthropic_credentials as aa
    monkeypatch.setattr(aa, "read_moor_oauth_credentials", lambda: {
    ...
    # moor_pkce suppressed, claude_code returns None → nothing should be seeded
    assert entries == []
    assert "moor_pkce" not in active
...
def test_credential_sources_registry_has_expected_steps():
    ...
    required = {
        ...
        "~/.moor/.anthropic_oauth.json",
        "auth.json providers.moor",
...
def test_qwen_oauth_login_marks_active_through_moved_owner(monkeypatch):
    ...
    import moor_cli.auth_commands as auth_commands
    import moor_cli.auth_qwen as auth_qwen
