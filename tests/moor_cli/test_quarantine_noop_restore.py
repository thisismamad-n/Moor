...
``moor*.exe`` shims aside (``moor.exe.old.<ms>``) before invoking the
...
quarantined shims stayed renamed aside and ``moor`` vanished from PATH
...
def _make_scripts_dir(tmp_path: Path) -> Path:
    ...
    for name in ("moor", "moor-agent", "moor-acp", "moor-gateway"):
...
# ---------------------------------------------------------------------------
# moor_cli.main._run_quarantined_install
# ---------------------------------------------------------------------------
...
def test_main_noop_success_restores_shims(tmp_path):
    ...
    assert "moor.exe" in names, "moor.exe must be restored after a no-op install"
    assert "moor-acp.exe" in names
    assert "moor-gateway.exe" in names
    assert (scripts / "moor.exe").read_bytes() == b"MZ-old-moor"
...
def test_main_rewriting_success_keeps_fresh_shims(tmp_path):
    ...
    assert (scripts / "moor.exe").read_bytes() == b"MZ-new-moor"
...
# ---------------------------------------------------------------------------
# moor_cli._install_repair._run_install_cmd (the deferred-recovery path)
# ---------------------------------------------------------------------------
...
def test_repair_noop_success_restores_shims(tmp_path):
    ...
    names = _shim_names(scripts)
    assert "moor.exe" in names, "moor.exe must be restored after a no-op recovery install"
    assert (scripts / "moor.exe").read_bytes() == b"MZ-old-moor"
...
def test_repair_rewriting_success_keeps_fresh_shims(tmp_path):
    ...
    def fake_run(cmd, cwd=None, check=None, env=None):
        (scripts / "moor.exe").write_bytes(b"MZ-new-moor")
