...
def test_validate_source_rejects_hermes_origin():
    with pytest.raises(ValueError, match="Moor"):
        validate_source(HERMES_URL, "master")
...
def test_validate_update_tree_rejects_hermes_pyproject(tmp_path):
    (tmp_path / "pyproject.toml").write_text(HERMES_PYPROJECT, encoding="utf-8")
    _write_component_files(tmp_path)
...
def test_validate_git_target_rejects_hermes_remote_tree(tmp_path):
    _git("init", "-q", cwd=tmp_path)
    (tmp_path / "pyproject.toml").write_text(HERMES_PYPROJECT, encoding="utf-8")
    _write_component_files(tmp_path)
    _git("add", "-A", cwd=tmp_path)
    _git("-c", "user.name=t", "-c", "user.email=t@t", "commit", "-qm", "hermes", cwd=tmp_path)
    _git("branch", "-M", "master", cwd=tmp_path)
    _git("remote", "add", "origin", CANONICAL_URL, cwd=tmp_path)
    _git("update-ref", "refs/remotes/origin/master", "HEAD", cwd=tmp_path)
