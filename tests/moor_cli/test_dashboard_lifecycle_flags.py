"""Tests for ``moor dashboard --stop`` / ``--status`` flags.

These flags share the detection + kill path with the post-``moor update``
...
class TestDashboardStatus:
    def test_status_no_processes(self, capsys):
        ...
        assert "No moor dashboard or serve processes running" in out
    ...
    def test_status_with_processes(self, capsys):
        ...
        # couldn't see (#81564).
        processes = [
            (12345, "moor dashboard --port 9119"),
            (12346, "python -m moor_cli.main dashboard --host 0.0.0.0 --port 9120"),
            (12347, "moor serve --host 100.94.65.93 --port 9119"),
        ]
        with patch("moor_cli.dashboard_procs._scan_dashboard_processes", return_value=processes), \
             patch("gateway.status._pid_exists", return_value=True), \
             patch("moor_cli.main_dashboard._dashboard_listening", return_value=True), \
             pytest.raises(SystemExit) as exc:
            cmd_dashboard(_ns(status=True))
...
class TestDashboardStop:
    ...
    def test_stop_kills_and_exits_zero_when_all_killed(self, capsys):
        ...
        # for the post-`moor update` path).
        kwargs = mock_kill.call_args.kwargs
        assert "reason" in kwargs
        assert "stop" in kwargs["reason"].lower()
        assert exc.value.code == 0

    def test_stop_scopes_scan_and_kill_to_the_invoking_moor_home(self, tmp_path, monkeypatch, capsys):
        ...
        with patch("moor_cli.main._find_stale_dashboard_pids", return_value=[12345]) as scan, \
             patch("moor_cli.dashboard_procs._kill_stale_dashboard_processes",
        ...
        with patch("moor_cli.main._find_stale_dashboard_pids", return_value=[]), \
             patch("moor_cli.dashboard_procs._kill_stale_dashboard_processes") as kill, \
             pytest.raises(SystemExit) as exc:
            cmd_dashboard(_ns(stop=True))
...
class TestLifecycleFlagsTakePrecedence:
    ...
    def test_stop_does_not_fall_through_to_server_start(self):
        ...
        with patch("moor_cli.main._find_stale_dashboard_pids",
                   return_value=[]), \
             patch.dict(sys.modules, {"moor_cli.web_server": fake_ws}), \
             pytest.raises(SystemExit):
            cmd_dashboard(_ns(stop=True))
