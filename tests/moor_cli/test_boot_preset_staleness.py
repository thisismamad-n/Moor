...
def test_presets_stale_when_a_staged_model_has_no_section(moor_home):
    from moor_cli.local_runtime.bootstrap import _presets_stale

    _stage(moor_home, "model-a")
    _stage(moor_home, "model-b")
    _write_presets(moor_home, "model-a")
    assert _presets_stale() is True


def test_presets_current_when_every_staged_model_is_covered(moor_home):
    from moor_cli.local_runtime.bootstrap import _presets_stale
...
def test_boot_replaces_incumbent_with_stale_presets(moor_home, monkeypatch):
    ...
    import moor_cli.local_runtime.bootstrap as boot
    ...
    stopped = {}
    monkeypatch.setattr(
        "moor_cli.local_runtime.endpoint._state_endpoint",
    ...
    monkeypatch.setattr(
        "moor_cli.local_runtime.binaries.ensure_runtime_installed", fake_boot)
...
def test_refresh_bounces_an_adopted_server(moor_home, monkeypatch):
...
def test_refresh_no_server_anywhere_is_a_noop(moor_home, monkeypatch):
    import moor_cli.local_runtime.bootstrap as boot

    monkeypatch.setattr(boot, "_SUPERVISOR", None)
    monkeypatch.setattr(
        "moor_cli.local_runtime.endpoint._state_endpoint", lambda: None)
