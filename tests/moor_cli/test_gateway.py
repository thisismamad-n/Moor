...
def _install_fake_gateway_run(monkeypatch, start_gateway):
    ...
    # /moor-gateway.service"`` and runs ``systemctl --user daemon-reload``
    # — both target the *real* user environment because the conftest only
    # sandboxes ``MOOR_HOME``, not ``HOME``. Tests that drive
    ...
    # unit (baking in the test's pytest-tmp ``MOOR_HOME`` value, which
...
def _run_native_windows_gateway_start_diag(
    ...
    script = textwrap.dedent(
        ...
        diag_path = pathlib.Path(os.environ["MOOR_HOME"]) / "logs" / "gateway-exit-diag.log"
    ...
    env.update(
        {
            "MOOR_HOME": str(tmp_path),
            "MOOR_GATEWAY_DETACHED": "1",
            "MOOR_GATEWAY_EXIT_DIAG": "1",
    ...
    from moor_cli._subprocess_compat import windows_detach_flags_without_breakaway
...
def test_gateway_run_subprocess_preserves_daemon_exit_codes(
    ...
    script = textwrap.dedent(
        ...
        outcome = os.environ["MOOR_TEST_GATEWAY_OUTCOME"]
    ...
    env = {
        **os.environ,
        "MOOR_HOME": str(tmp_path),
        "MOOR_GATEWAY_EXIT_DIAG": "0",
        "MOOR_TEST_GATEWAY_OUTCOME": outcome,
...
def _clear_supervisor_markers(monkeypatch):
    ...
    monkeypatch.delenv("MOOR_S6_SUPERVISED_CHILD", raising=False)
...
def test_s6_runtime_snapshot_reports_supervised_service(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("moor_constants.is_container", lambda: True)
    monkeypatch.setattr("moor_cli.service_manager.detect_service_manager", lambda: "s6")
    monkeypatch.setattr("moor_cli.service_manager.get_service_manager", lambda: FakeS6Manager())
...
def test_spawn_detached_gateway_timestamps_stderr(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(gateway, "get_moor_home", lambda: tmp_path)
...
def test_systemd_install_checks_linger_status(monkeypatch, tmp_path, capsys):
    ...
    # hermetic test MOOR_HOME (a tmp dir), which the temp-home write
    # guard correctly refuses.
    monkeypatch.setattr(
        gateway,
        "generate_systemd_unit",
        lambda system=False, run_as_user=None: (
            '[Service]\nEnvironment="MOOR_HOME=/home/alice/.moor"\n'
        ),
    )
...
def test_gateway_install_noninteractive_skips_legacy_unit_prompt(monkeypatch, tmp_path):
    ...
    """
    monkeypatch.setattr(gateway, "has_legacy_moor_units", lambda: True)
    ...
    monkeypatch.setattr(gateway, "remove_legacy_moor_units", lambda interactive=False: calls.append(("remove_legacy",)))
    monkeypatch.setattr(gateway, "print_legacy_unit_warning", lambda: None)

    fake_path = tmp_path / "moor-gateway.service"
...
class TestWaitForGatewayExit:
    ...
    def test_kill_gateway_processes_force_uses_helper(self, monkeypatch):
        ...
        monkeypatch.setattr(
            gateway, "_capture_gateway_argv", lambda pid: ["python", "-m", "moor_cli.main", "gateway", "run"]
        )
        monkeypatch.setattr(
            gateway,
            "terminate_pid",
    ...
    def test_kill_gateway_processes_force_refuses_recycled_pid(self, monkeypatch):
        ...
        monkeypatch.setattr(
            ...
            lambda pid: None if pid == 11 else ["python", "-m", "moor_cli.main", "gateway", "run"],
        )
        monkeypatch.setattr(
            gateway,
            "terminate_pid",
...
class TestReapUnsupervisedGatewayOrphansMacOS:
    ...
    gateway every time Moor Desktop opens (``moor serve`` calls
...
class TestReapUnsupervisedGatewayOrphansWindows:
    ...
    (``moor serve`` calls ``_reap_unsupervised_gateway_orphans`` during
...
class TestReaperCandidateIsSupervisorOwned:
    ...
    def test_windows_scheduled_task_gateway_spared_without_pidfile(self, monkeypatch):
        ...
        gateway_pid = 52615
        bootstrap_pid = 52616   # Task-launched `moor gateway run` bootstrap
        ...
        bootstrap = SimpleNamespace(
            pid=bootstrap_pid, parent=lambda: services, name=lambda: "moor-gateway.exe"
        )
        gw = SimpleNamespace(
            pid=gateway_pid, parent=lambda: bootstrap, name=lambda: "moor-gateway.exe"
        )
        ...
        orphan = SimpleNamespace(pid=orphan_pid, parent=lambda: None, name=lambda: "moor-gateway.exe")
    ...
    def test_windows_backstop_fails_open_when_bootstrap_exited(self, monkeypatch):
        ...
        monkeypatch.setattr(gateway, "is_windows", lambda: True)
        stranded = SimpleNamespace(pid=4242, parent=lambda: None, name=lambda: "moor-gateway.exe")
...
def test_module_has_logger():
    ...
    assert gateway.logger.name == "moor_cli.gateway"
...
class TestWindowsScheduledTaskSupervisorGuard:
    ...
    def test_running_task_skips_reap(self, monkeypatch):
        """Moor_Gateway_* is Running => reaper returns False, kills nothing."""
        monkeypatch.setattr(gateway, "is_windows", lambda: True)
        monkeypatch.setattr(gateway, "is_macos", lambda: False)
        monkeypatch.setattr(gateway, "supports_systemd_services", lambda: False)
        ...
        # hardcoded "MoorGateway" would leave the guard dormant on every
        # standard install (task name is Moor_Gateway / Moor_Gateway_<p>).
        import moor_cli.gateway_windows as gateway_windows

        monkeypatch.setattr(
            gateway_windows, "get_task_name", lambda: "Moor_Gateway_testprof"
        )
        queried = []
        ...
        assert result is False
        assert killed_pids == []
        assert queried == ["Moor_Gateway_testprof"]
    ...
    def test_windows_scheduled_task_running_returns_false_off_windows(self, monkeypatch):
        ...
        assert gateway._windows_scheduled_task_supervises("MoorGateway") is False
        assert gateway._windows_scheduled_task_state("MoorGateway") is None
    ...
    def test_supervises_ready_and_queued_but_not_disabled(self, monkeypatch):
        ...
        for state, expected in states.items():
            ...
            assert gateway._windows_scheduled_task_supervises("Moor_Gateway") is expected, state
...
def test_find_windows_gateway_services_rejects_transitional_ancestor(monkeypatch):
    ...
    class FakeService:
        def as_dict(self):
            return {"name": "MoorGateway", "pid": 100, "status": "stop_pending"}
