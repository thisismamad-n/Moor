"""Tests for the Moor plugin system (moor_cli.plugins)."""
...
def _make_plugin_dir(base: Path, name: str, *, register_body: str = "pass",
    ...
    ``<moor_home>/config.yaml`` under ``plugins.enabled``. Plugins are
    ...
    *base* is expected to be ``<moor_home>/plugins/``; we derive
    ``<moor_home>`` from it by walking one level up unless *home* is
    given explicitly.

    Pass *home* explicitly whenever the target Moor home for this
    plugin isn't necessarily the current ``MOOR_HOME`` env var — e.g.
    when writing fixtures for two profiles up front and only switching
    ``MOOR_HOME``/``set_moor_home_override()`` per-profile afterwards
    ...
    if auto_enable:
        # Write/merge plugins.enabled in <MOOR_HOME>/config.yaml.
        # Config is always read from MOOR_HOME (not from the project
        # dir for project plugins), so that's where we opt in.
        if home is not None:
            moor_home = Path(home)
        else:
            import os
            moor_home_str = os.environ.get("MOOR_HOME")
            if moor_home_str:
                moor_home = Path(moor_home_str)
            else:
                moor_home = base.parent
...
class TestPluginDiscovery:
    ...
    def test_removed_relay_plugin_identity_cannot_be_reloaded(
        ...
        assert "Relay lifecycle is owned by Moor core" in state.error
        assert RELAY_PLUGINS_CONFIG_ENV in state.error
        assert "Refusing to load removed Moor Relay plugin" in caplog.text
    ...
    def test_plugin_can_register_and_invoke_middleware(self, tmp_path, monkeypatch):
        plugins_dir = tmp_path / "moor_test" / "plugins"
        _make_plugin_dir(
            plugins_dir,
            "mw_plugin",
            register_body=(
        ...
        monkeypatch.setenv("MOOR_HOME", str(tmp_path / "moor_test"))
...
class TestPluginLoading:
    ...
    def test_load_registers_namespace_module(self, tmp_path, monkeypatch):
        """Directory plugins are importable under moor_plugins.<name>."""
        plugins_dir = tmp_path / "moor_test" / "plugins"
        _make_plugin_dir(plugins_dir, "ns_plugin")
        monkeypatch.setenv("MOOR_HOME", str(tmp_path / "moor_test"))

        # Clean up any prior namespace module
        sys.modules.pop("moor_plugins.ns_plugin", None)

        mgr = PluginManager()
        mgr.discover_and_load()

        assert "moor_plugins.ns_plugin" in sys.modules
    ...
    def test_entrypoint_memory_provider_auto_coerced_to_exclusive(
        ...
        ``moor_agent.plugins`` entry point but exposing
        ...
        monkeypatch.setattr(
            "moor_cli.plugins.importlib.metadata.entry_points",
    ...
    def test_entrypoint_duplicate_does_not_block_directory_provider_activation(
        ...
        # Same-name directory provider under $MOOR_HOME/plugins/.
        moor_home = tmp_path / "moor_test"
...
class TestForceReloadSymmetry:
    ...
    def test_re_register_config_hooks_clears_idempotence_set(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "moor_cli.config.load_config", lambda: {"hooks": {}}
        )
        home_key = str(get_moor_home().expanduser().resolve())
    ...
    def test_hook_timeout_does_not_block_caller(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "moor_cli.plugins._resolve_hook_callback_timeout", lambda: 0.15
        )
...
class TestPreToolCallDirective:
    ...
    def test_first_party_observer_receives_pre_tool_call(self, monkeypatch):
        from moor_cli import observability
        from moor_cli.plugins import get_pre_tool_call_directive
...
class TestGetPreVerifyContinueMessage:
    ...
    def test_none_when_no_hooks(self, monkeypatch):
        monkeypatch.setattr("moor_cli.plugins.invoke_hook", lambda hook_name, **kwargs: [])
    ...
    def test_forwards_scope_signals_to_hooks(self, monkeypatch):
        ...
        monkeypatch.setattr("moor_cli.plugins.invoke_hook", capture)
...
class TestPluginContext:
    ...
    def test_register_tool_override_blocked_without_operator_opt_in(self, tmp_path, monkeypatch):
        ...
        from moor_cli.plugins import PluginToolOverrideError
        ...
        try:
            ...
            from moor_cli.plugins import PluginContext, PluginManifest
    ...
    def test_register_tool_override_blocked_via_delayed_callback(self, tmp_path, monkeypatch):
        ...
        try:
            ...
            import sys as _sys
            mod = _sys.modules.get("moor_plugins.delayed_override_plugin")
...
class TestPluginManagerList:
    ...
    def test_shared_hook_name_credited_to_every_plugin(self, tmp_path, monkeypatch):
        ...
        `moor plugins list`. Attribution now counts what each plugin's own
...
class TestPluginCommands:
    ...
    def test_plugin_manager_scoped_by_moor_home_override(self, tmp_path):
        """set_moor_home_override() must get its own manager per profile.

        This is the production path used by the gateway multiplexer
        (``gateway/run.py``'s ``_profile_scope`` context manager) and by
        subagent/embedded callers: it swaps ``MOOR_HOME`` via a
        context-local ContextVar, which — per
        ``moor_constants.set_moor_home_override`` — deliberately does
        NOT touch ``os.environ``. A regression test that only flips the
        ``MOOR_HOME`` env var never exercises this path.
        """
        ...
        def write_engine_plugin(home: Path) -> None:
            _make_plugin_dir(
                ...
                register_body=(
                    ...
                    "        def __init__(self):\n"
                    "            self.home = os.environ.get('MOOR_HOME')\n\n"
                    "        @property\n"
                    "        def name(self):\n"
                    "            return 'home-engine'\n\n"
                    "        def update_from_response(self, usage):\n"
        ...
        # Note: HomeEngine reads os.environ['MOOR_HOME'] itself (simulating
        # a real plugin like moor-lcm capturing its home at registration),
        ...
        # not last-write-wins.
        token_a2 = set_moor_home_override(str(home_a))
        try:
            manager_a2 = plugins_mod.get_plugin_manager()
        finally:
            reset_moor_home_override(token_a2)
    ...
    def test_relative_import_not_leaked_across_home_switch(self, tmp_path):
        ...
        ``moor_plugins.<slug>``, but a relative import inside the
        plugin's ``__init__.py`` (``from .state import STATE``) is cached
        separately in ``sys.modules`` under
        ``moor_plugins.<slug>.state``. If a profile switch replaces only
...
class TestPluginCommandResultResolution:
    ...
    def test_awaits_async_result_with_running_loop(self, monkeypatch):
        ...
        monkeypatch.setattr("moor_cli.plugins.asyncio.get_running_loop", lambda: _Loop())
    ...
    def test_running_loop_timeout_does_not_hang_forever(self, monkeypatch):
        ...
        monkeypatch.setattr("moor_cli.plugins.asyncio.get_running_loop", lambda: _Loop())
        monkeypatch.setattr("moor_cli.plugins._PLUGIN_COMMAND_AWAIT_TIMEOUT_SECS", 0.1)
...
class TestPluginDispatchTool:
    ...
    def test_dispatch_tool_calls_registry(self):
        ...
        with patch("moor_cli.plugins.PluginContext.dispatch_tool.__module__", "moor_cli.plugins"):
...
class TestPluginDebugLogging:
    """MOOR_PLUGINS_DEBUG opt-in stderr handler for plugin developers."""
    ...
    def test_debug_handler_not_installed_when_env_var_absent(self, monkeypatch):
        ...
        monkeypatch.delenv("MOOR_PLUGINS_DEBUG", raising=False)
        from moor_cli import plugins as plugins_mod
...
class TestPluginContextProfileName:
    """ctx.profile_name resolves from MOOR_HOME in every context."""
    ...
    def test_default_profile(self, tmp_path, monkeypatch):
        """MOOR_HOME at the root resolves to 'default'."""
        home = tmp_path / ".moor"
        home.mkdir()
    ...
    def test_named_profile(self, tmp_path, monkeypatch):
        """MOOR_HOME under profiles/<name> resolves to that name."""
        prof = tmp_path / ".moor" / "profiles" / "coder"
        prof.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("MOOR_HOME", str(prof))
