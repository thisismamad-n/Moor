"""Tests for Moor subscription feature detection."""

import shutil
import sys

from moor_cli.moor_account import MoorPortalAccountInfo, MoorToolAccessInfo
from moor_cli import moor_subscription as ns
...
def _account(*, logged_in: bool, paid: bool | None = None) -> MoorPortalAccountInfo:
...
def _pool_account() -> MoorPortalAccountInfo:
    ...
    return MoorPortalAccountInfo(
        ...
        tool_access=MoorToolAccessInfo(enabled=True, coverage=_POOL_COVERAGE),
    )


def test_get_moor_subscription_features_recognizes_direct_exa_backend(monkeypatch):
    env = {"EXA_API_KEY": "exa-test"}

    monkeypatch.setattr(ns, "get_env_value", lambda name: env.get(name, ""))
    monkeypatch.setattr(
        ns, "get_moor_portal_account_info", lambda: _account(logged_in=False)
    )
    ...
    features = ns.get_moor_subscription_features({"web": {"backend": "exa"}})

    assert features.web.available is True
    assert features.web.active is True
    assert features.web.managed_by_moor is False
    assert features.web.direct_override is True
    assert features.web.current_provider == "exa"


def test_get_moor_subscription_features_recognizes_keyless_tavily_backend(monkeypatch):
    ...
    features = ns.get_moor_subscription_features({"web": {"backend": "tavily"}})
...
def test_keyless_tavily_search_backend_without_shared_backend(monkeypatch):
    ...
    features = ns.get_moor_subscription_features(
...
def test_local_browser_unavailable_without_chromium(monkeypatch):
    ...
    assert features.browser.managed_by_moor is False
...
def _capture_checklist(monkeypatch, *, selected_idx):
    ...
    monkeypatch.setattr(
        "moor_cli.config.save_config", lambda cfg: None, raising=False
    )
    return captured
...
def test_logged_in_entitled_account_yields_a_state_for_every_feature(monkeypatch):
    ...
    crashing `moor status`, `moor tools`, and the dashboard toolsets API."""
    monkeypatch.setattr(ns, "get_moor_portal_account_info", lambda **kw: _pool_account())
    ...
    result = ns.get_moor_subscription_features({"model": {"provider": "moor"}})
...
def test_get_gateway_eligible_tools_treats_explicit_backend_as_configured(monkeypatch):
    ...
    but an explicit non-moor selection must still keep it out of
    'unconfigured' — regression for #92647, where it was pre-checked and a
    single Enter during `moor model` overwrote it to `web.backend: moor`.
    """
    monkeypatch.setattr(ns, "get_moor_portal_account_info", lambda **kw: _account(logged_in=True, paid=True))
    monkeypatch.setattr(
        ns,
        "_get_gateway_direct_credentials",
        lambda: {"web": False, "image_gen": False, "video_gen": False, "tts": False, "browser": False},
    ...
    config = {"model": {"provider": "moor"}, "web": {"backend": "searxng"}}
...
def test_get_gateway_eligible_tools_treats_browser_use_selection_as_explicit(monkeypatch):
    ...
    managed selection, which is always stored as `cloud_provider: moor`
    ...
    config = {"model": {"provider": "moor"}, "browser": {"cloud_provider": "browser-use"}}
    unconfigured, has_direct, explicit_configured, already_managed = ns.get_gateway_eligible_tools(config)
...
def test_get_gateway_eligible_tools_not_entitled_returns_four_empty_lists(monkeypatch):
    """A logged-in Moor account with no paid access and no free tool pool
    ...
    monkeypatch.setattr(ns, "get_moor_portal_account_info", lambda **kw: _account(logged_in=True, paid=False))
...
def test_prompt_enable_tool_gateway_not_entitled_does_not_crash(monkeypatch):
    ...
    not raise when a Moor account is logged in but not entitled to the Tool
...
def test_prompt_enable_tool_gateway_never_offers_explicit_backend(monkeypatch):
    ...
    an explicit non-moor selection, so it can never be silently overwritten
...
def test_prompt_enable_tool_gateway_persists_decline(monkeypatch):
    ...
    captured = _capture_checklist(monkeypatch, selected_idx=[])
    monkeypatch.setattr(
        "moor_cli.config.save_config", lambda cfg: saved.append(dict(cfg)), raising=False
    )
...
def test_apply_moor_managed_defaults_writes_video_gen_config(monkeypatch):
    """apply_moor_managed_defaults must store the managed 'moor' selection
    when a Moor subscriber selects video_gen without a direct FAL_KEY."""
    monkeypatch.setattr(tool_backend_helpers, "managed_moor_tools_enabled", lambda **kw: True)
    monkeypatch.delenv("FAL_KEY", raising=False)
    monkeypatch.setattr(ns, "fal_key_is_configured", lambda: False)
    monkeypatch.setattr(
        ns, "get_moor_portal_account_info",
        lambda **kw: _account(logged_in=True, paid=True),
    )

    config = {"model": {"provider": "moor"}}
    changed = ns.apply_moor_managed_defaults(
...
# ---------------------------------------------------------------------------
# ensure_moor_portal_access — inline login gate for `moor tools`
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# STT — managed-by-Moor detection (Phase 4 follow-up)
# ---------------------------------------------------------------------------
...
def _stt_features_stub(*, account_info):
    return ns.MoorSubscriptionFeatures(
        subscribed=True,
        moor_auth_present=True,
        provider_is_moor=True,
        account_info=account_info,
        features={
            key: ns.MoorFeatureState(
                key=key, label=key, included_by_default=True,
                available=False, active=False, managed_by_moor=False,
...
def _block_legacy_agent_browser_checks(monkeypatch):
    ...
    monkeypatch.setattr("moor_constants.agent_browser_runnable", lambda path: False)
...
def test_has_agent_browser_import_failure_falls_back_to_path_check(monkeypatch):
    ...
    monkeypatch.setattr(
        "moor_constants.agent_browser_runnable",
...
def test_has_agent_browser_import_failure_falls_back_to_moor_managed_node_path(
    ...
    monkeypatch.setattr(
        "moor_constants.with_moor_node_path", lambda: {"PATH": str(managed_dir)}
    )
    monkeypatch.setattr(
        "moor_constants.agent_browser_runnable",
