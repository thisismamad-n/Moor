"""Narrowing the Moor model lists to an org's policy.
...
class TestRestrictToMoorPolicy:
    ...
    def test_preserves_curated_order(self):
        ...
        assert restrict_to_moor_policy(curated, allowed) == curated

    def test_keeps_a_free_sibling_when_its_base_is_reachable(self):
        """Portal free recommendations are ``:free`` ids."""
        assert restrict_to_moor_policy(["vendor/model:free"], {"vendor/model"}) == [
            "vendor/model:free"
        ]
    ...
    def test_drops_a_free_sibling_whose_base_is_blocked(self):
        assert restrict_to_moor_policy(["vendor/model:free"], {"other/model"}) == []


class TestMoorPolicyAllowedIds:
    ...
    def _patch(self, monkeypatch, *, policy_present, api_key="sk-test", pricing=None):
        calls = []
        monkeypatch.setattr(
            account_mod, "moor_policy_present", lambda: policy_present
        )
        monkeypatch.setattr(models_pricing, "_resolve_moor_pricing_credentials",
    ...
    def test_returns_the_authenticated_catalog_keys(self, monkeypatch):
        ...
        assert moor_policy_allowed_ids() == {"a/one", "b/two"}
        assert len(calls) == 1
    ...
    def test_declines_to_filter_an_unrestricted_org(self, monkeypatch):
        ...
        assert moor_policy_allowed_ids() is None
...
class TestMoorPolicyNotice:

    def _patch(self, monkeypatch, present):
        monkeypatch.setattr(account_mod, "moor_policy_present", lambda: present)

    def test_shows_a_line_for_a_governed_org(self, monkeypatch):
        self._patch(monkeypatch, True)
        assert "restricts which models" in account_mod.moor_policy_notice(removed=True)
    ...
    def test_silent_otherwise(self, monkeypatch, present):
        ...
        assert account_mod.moor_policy_notice(removed=True) == ""
    ...
    def test_silent_when_the_filter_removed_nothing(self, monkeypatch):
        ...
        self._patch(monkeypatch, True)
        assert account_mod.moor_policy_notice(removed=False) == ""

    def test_names_no_models(self, monkeypatch):
        """The blocked set is most of the catalog under an allowlist."""
        self._patch(monkeypatch, True)
        notice = account_mod.moor_policy_notice(removed=True)
...
class TestAllowlistOutsideTheCuratedList:
    ...
    def test_does_not_rescue_a_catalog_sized_allowed_set(self):
        ...
        oversized = {f"cn/model-{i}" for i in range(_MOOR_POLICY_APPEND_MAX + 1)}
        assert restrict_to_moor_policy(["vendor/one"], oversized, rescue_empty=True) == []
...
class TestRescueIsOptIn:
    ...
    def test_rescue_only_when_asked(self):
        assert restrict_to_moor_policy([], {"a/one"}, rescue_empty=True) == ["a/one"]
    ...
    def test_an_already_empty_unavailable_list_is_never_filled(self):
        ...
        assert restrict_to_moor_policy([], reachable) == []
...
class TestPolicyRunsBeforeTierSplit:
    ...
    def test_a_rescued_paid_model_stays_unavailable_for_a_free_tier_user(self):
        from moor_cli.models import partition_moor_models_by_tier

        pricing = {
            "vendor/free": {"prompt": "0", "completion": "0"},
            "vendor/paid": {"prompt": "0.000002", "completion": "0.00001"},
        }
        narrowed = restrict_to_moor_policy(
            list(pricing), {"vendor/paid"}, rescue_empty=True
        )
        selectable, unavailable = partition_moor_models_by_tier(
