"""Tests for kanban board export / import (``moor_cli.kanban_transfer``).
...
from moor_cli import kanban_transfer as kt
from moor_cli.archive_safe import normalize_archive_parts, safe_extract_targz
...
def kanban_root(tmp_path, monkeypatch):
    ...
    def _use(name: str) -> Path:
        ...
        monkeypatch.setenv("MOOR_KANBAN_HOME", str(root))
        for var in ("MOOR_KANBAN_DB", "MOOR_KANBAN_WORKSPACES_ROOT",
                    "MOOR_KANBAN_ATTACHMENTS_ROOT", "MOOR_KANBAN_BOARD"):
...
def test_import_rejects_a_future_format_version(kanban_root, tmp_path):
    ...
    kanban_root("target")
    with pytest.raises(ValueError, match="newer than this Moor"):
