...
loader so ``moor chat`` doesn't import ~20 gateway SDKs. The a2a plugin ships
...
``moor tools`` checklist. The same tools worked in gateway/web processes only
...
def _write_platform_plugin(
    ...
    *,
    ...
    """
    from moor_cli.plugins import PluginManifest
...
def clean_registry():
    ...
    ``probeplat`` name, so the imported ``moor_plugins.*`` modules have to go
    ...
    for name in set(sys.modules) - before_modules:
        if name.startswith("moor_plugins."):
...
class TestA2AClientToolsInCliProcess:
    ...
    def test_a2a_appears_in_the_moor_tools_checklist(self):
    ...
    def test_platform_bundle_includes_the_client_tools(self):
        ...
        assert A2A_CLIENT_TOOLS.issubset(set(resolve_toolset("moor-a2a")))
...
class TestDeferredPlatformToolPreregistration:
    ...
    def test_tools_stay_attributed_after_materialization(
        ...
        """`moor plugins list` must still credit the pre-registered tools.
    ...
    def test_partially_registered_tools_are_still_attributed(
        ...
        `moor plugins list` under-report what the process is carrying, and
    ...
    def test_failed_materialization_tears_down_pre_registered_tools(
        ...
        adapter. Attribution and the registry therefore agree at zero: `moor
