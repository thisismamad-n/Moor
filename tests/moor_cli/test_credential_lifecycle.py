...
MOOR_HOME (tests/conftest.py isolation) and assert every store agrees
afterwards.
...
@pytest.fixture
def moor_home(monkeypatch, tmp_path):
    """Fresh MOOR_HOME with .env + auth.json + config.yaml fixtures."""
...
def test_delete_clears_provider_models_cache(moor_home):
    _write_env(moor_home, ZAI_API_KEY=FAKE_ZAI_KEY)
    _write_auth(moor_home, {"zai": [_zai_pool_fixture()["zai"][0]]})
    cache_path = moor_home / "provider_models_cache.json"
...
def test_update_rotates_config_yaml_model_mirror(moor_home):
    old = "sk-oe-" + "f" * 24
    new = "sk-oe-" + "g" * 24
    _write_env(moor_home, OPENAI_API_KEY=old)
    _write_config(
        moor_home,
        "model:\n"
        "  provider: custom\n"
    ...
    cfg_text = moor_home.joinpath("config.yaml").read_text(encoding="utf-8")
...
# load_pool() or a separate `moor auth add`.
# ---------------------------------------------------------------------------
...
def test_put_api_env_materializes_credential_pool_entry(moor_home):
    ...
    the old cached credential until a separate ``moor auth add opencode-go``
...
def test_put_api_env_writes_auth_json_for_provider(moor_home):
    """Sentinel for #96058: PUT /api/env must modify auth.json on disk.

    The reported symptom was ``stat -c '%y' ~/.moor/auth.json`` returning
...
def test_update_rotates_keyed_providers_mirror(moor_home):
    old = "sk-kp-" + "n" * 24
    new = "sk-kp-" + "o" * 24
    _write_env(moor_home, OPENAI_API_KEY=old)
    _write_config(
...
def test_delete_scrubs_keyed_providers_mirror(moor_home):
    old = "sk-kp-" + "p" * 24
    _write_env(moor_home, OPENAI_API_KEY=old)
    _write_config(
        moor_home,
...
def test_scrub_never_touches_providers_base_url_alias(moor_home):
