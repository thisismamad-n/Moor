"""Tests for moor_cli.uninstall.remove_node_symlinks.

Regression for #34536: the POSIX installer drops node/npm/npx symlinks in
~/.local/bin pointing into $MOOR_HOME/node and prepends ~/.local/bin to
...
def _make_moor_node(moor_home: Path) -> Path:
    """Create a fake $MOOR_HOME/node/bin/{node,npm,npx} tree."""
    node_bin = moor_home / "node" / "bin"
...
def test_leaves_unrelated_symlinks_untouched(fake_home):
    ...
    moor_home = fake_home / ".moor"
    _make_moor_node(moor_home)
    ...
    removed = uninstall.remove_node_symlinks(moor_home)
...
def test_removes_fhs_symlinks_in_usr_local_bin(fake_home, tmp_path, monkeypatch):
    ...
    """
    moor_home = fake_home / ".moor"
    node_bin = _make_moor_node(moor_home)
