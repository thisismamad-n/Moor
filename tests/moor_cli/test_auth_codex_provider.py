"""Tests for Codex auth — tokens stored in Moor auth store (~/.moor/auth.json)."""
...
def _setup_moor_auth(moor_home: Path, *, access_token: str = "access", refresh_token: str = "refresh"):
...
def test_resolve_codex_runtime_credentials_missing_access_token(tmp_path, monkeypatch):
    ...
    _setup_moor_auth(moor_home, access_token="")
...
def test_resolve_codex_runtime_credentials_falls_back_to_pool_when_singleton_empty(tmp_path, monkeypatch):
    ...
    (moor_home / "auth.json").write_text(json.dumps(auth_store))
    monkeypatch.setenv("MOOR_HOME", str(moor_home))
...
def test_save_codex_tokens_syncs_credential_pool(tmp_path, monkeypatch):
    ...
    auth = json.loads((moor_home / "auth.json").read_text())
...
def test_save_codex_tokens_syncs_manual_device_code_entries(tmp_path, monkeypatch):
    ...
    fix landed would have run ``moor auth add openai-codex`` as a
    ...
    clobbered independent accounts added via ``moor auth add openai-codex``.
    ...
    (moor_home / "auth.json").write_text(json.dumps({
        ...
        "credential_pool": {
            "openai-codex": [
                ...
                # Independent account from `moor auth add openai-codex` —
...
def test_save_codex_tokens_does_not_overwrite_independent_manual_entries(tmp_path, monkeypatch):
    ...
    Regression for #39236: ``moor auth add openai-codex`` for accounts B and C
    ...
    (moor_home / "auth.json").write_text(json.dumps({
        ...
        "credential_pool": {
            "openai-codex": [
                ...
                # ``moor auth add openai-codex`` (account B and account C).
...
def test_codex_tokens_not_written_to_shared_file(tmp_path, monkeypatch):
    ...
    _save_codex_tokens({"access_token": "moor-at", "refresh_token": "moor-rt"})
    ...
    assert data["tokens"]["access_token"] == "moor-at"


def test_resolve_returns_moor_auth_store_source(tmp_path, monkeypatch):
    moor_home = tmp_path / "moor"
    _setup_moor_auth(moor_home)
    monkeypatch.setenv("MOOR_HOME", str(moor_home))

    creds = resolve_codex_runtime_credentials()
    assert creds["source"] == "moor-auth-store"
...
def test_refresh_429_classified_as_quota_not_auth_failure(monkeypatch):
    ...
    than a misleading "run moor auth".
    """
    from moor_cli.auth import (
    ...
    assert "moor auth" not in rendered
...
def test_refresh_429_without_retry_after_header(monkeypatch):
    ...
    from moor_cli.auth import CODEX_RATE_LIMITED_CODE
...
def test_is_rate_limited_auth_error_distinguishes_credential_errors():
    ...
    from moor_cli.auth import CODEX_RATE_LIMITED_CODE, is_rate_limited_auth_error
...
def _patch_httpx_post(monkeypatch, responses):
    """Patch moor_cli.auth.httpx.Client so .post() returns queued responses."""
    seq = iter(responses)
    ...
    monkeypatch.setattr("moor_cli.auth.httpx.Client", lambda *a, **k: _FakeClient())
