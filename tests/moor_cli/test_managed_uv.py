"""Tests for moor_cli.managed_uv — one path, no guessing."""
...
def _runtime_info(
    ...
    from moor_cli.sqlite_runtime import SQLiteRuntimeInfo
...
def _RRR(status):
    ...
    """
    from moor_cli.managed_uv import RuntimeRepairResult
...
class TestManagedUvPath:
    ...
    def test_posix(self, tmp_path):
        with patch("moor_cli.managed_uv.get_moor_home", return_value=tmp_path):
            from moor_cli.managed_uv import managed_uv_path
...
class TestMacOSManagedPythonSigning:
    def test_signs_with_stable_identifier_and_verifies(self, tmp_path, monkeypatch):
        ...
        assert calls[0][0] == [
            ...
            "--timestamp=none",
            "--identifier",
            "com.moorinc.moor.managed-python",
            "--requirements",
            '=designated => identifier "com.moorinc.moor.managed-python"',
            str(python),
        ]
        assert calls[1][0] == [
            "/usr/bin/codesign",
...
class TestEnsureUv:
    ...
    def test_install_reports_runtime_repair_to_observer(self, tmp_path):
        ...
        observed = []
        with patch(
            "moor_cli.managed_uv.get_moor_home",
...
class TestEnsureUvUpdateBoundary:
    ...
    ``moor update`` runs the call site from the old, already-imported
    ``moor_cli.main`` against the freshly pulled ``managed_uv``. A release
    ...
    def test_failure_unpacks_without_raising(self, tmp_path):
        with patch("moor_cli.managed_uv.get_moor_home", return_value=tmp_path), \
             patch("moor_cli.managed_uv.repair_vulnerable_runtime", return_value=_RRR("not-applicable")), \
             patch("moor_cli.managed_uv._install_uv", side_effect=RuntimeError("network down")):
            from moor_cli.managed_uv import ensure_uv
...
class TestEnsureUvWindowsSafe:
    ...
    def test_uvresult_would_break_windows_list2cmdline(self):
        ...
        import subprocess
        from moor_cli.managed_uv import _UvResult
        with pytest.raises(TypeError):
            subprocess.list2cmdline([_UvResult("C:\\moor\\uv.exe"), "pip"])
    ...
    def test_windows_returns_plain_str_safe_for_subprocess(self, tmp_path):
        ...
        with patch("moor_cli.managed_uv.get_moor_home", return_value=tmp_path), \
             patch("moor_cli.managed_uv.repair_vulnerable_runtime", return_value=_RRR("not-applicable")):
            from moor_cli.managed_uv import _UvResult, ensure_uv
            uv_bin = ensure_uv()
...
class TestManagedPythonStore:
    def test_store_is_checkout_scoped_across_profiles(self, tmp_path, monkeypatch):
        from moor_cli.managed_uv import managed_python_install_dir

        checkout = tmp_path / "checkout"
        monkeypatch.setenv("MOOR_HOME", str(tmp_path / "profiles" / "alpha"))
        ...
        expected = checkout / ".moor-runtime" / "python"
...
class TestRuntimeRepair:
    def test_safe_runtime_is_a_noop(self, tmp_path):
        from moor_cli.managed_uv import repair_vulnerable_runtime

        root, live, sentinel = _make_runtime_install(tmp_path)
        current = _runtime_info(live / "bin" / "python", (3, 53, 1))
        with patch(
                 "moor_cli.managed_uv.probe_sqlite_runtime",
                 return_value=current,
             ), \
             patch(
                 "moor_cli.managed_uv._install_safe_python_generation"
             ) as mock_install:
    ...
    def test_failed_candidate_preserves_live_venv(self, tmp_path):
        ...
        generation = root / ".moor-runtime" / "python" / "generation-test"
        ...
        reacquired = _acquire_repair_lock(root / ".moor-runtime")
    ...
    def test_successful_repair_removes_parked_backup(self, tmp_path):
        ...
        candidate_python.write_text("candidate interpreter", encoding="utf-8")
        fixed = _runtime_info(candidate_python, (3, 53, 1))
        candidate_venv = root / ".moor-runtime" / "venv-candidate"
...
class TestRuntimeCutover:
    def test_os_lock_blocks_concurrent_repair_and_releases(self, tmp_path):
        ...
        runtime_root = tmp_path / ".moor-runtime"
    ...
    def test_post_swap_smoke_failure_rolls_back_live_venv(self, tmp_path):
        from moor_cli.managed_uv import _cut_over_candidate

        root, live, sentinel = _make_runtime_install(tmp_path)
        runtime_root = root / ".moor-runtime"
...
class TestRuntimeRequestMinorLine:
    ...
    def test_requests_minor_line(self):
        from moor_cli.managed_uv import _runtime_request
...
class TestMinorLineFallForward:
    ...
    `moor update` with no path to a fixed runtime.
    """
...
class TestRefreshManagedUvCatalog:
    ...
    def test_version_change_reports_true(self, tmp_path):
        ...
        with patch("moor_cli.managed_uv.get_moor_home", return_value=tmp_path), \
             patch("moor_cli.managed_uv._install_uv"), \
             patch(
                 "moor_cli.managed_uv._uv_version_string",
...
class TestRepairRetriesAfterUvRefresh:
    def _run_repair(self, tmp_path, *, refresh_result, second_attempt):
        ...
        with patch(
             ...
             patch(
                 "moor_cli.managed_uv._refresh_managed_uv_catalog",
    ...
    def test_retry_success_proceeds_to_staging(self, tmp_path):
        def second_attempt(root):
            generation = root / ".moor-runtime" / "python" / "generation-retry"
            candidate_python = generation / "bin" / "python"
            candidate_python.parent.mkdir(parents=True)
...
class TestDefaultLiveVenv:
    ...
    'not-applicable' on every moor update and stayed on journal_mode=DELETE
    ...
    def test_dot_venv_only_is_targeted(self, tmp_path):
        from moor_cli.managed_uv import _default_live_venv
...
class TestVenvPythonUpdateBoundary:
    """``_venv_python`` must survive a moor_constants predating its symbol.

    ``moor update`` imports moor_constants from the OLD checkout, ``git
    pull`` replaces that file, and the freshly-pulled managed_uv then runs its
    lazy ``from moor_constants import venv_python_path`` against the module
    ...
    plainly contains it, which is what made the error so confusing:
        ...
        cannot import name 'venv_python_path' from 'moor_constants'
        (~/.moor/moor-agent/moor_constants.py)
    ...
    def test_recovers_when_the_cached_module_predates_the_symbol(self, monkeypatch):
        ...
        from moor_cli.managed_uv import _venv_python
        ...
        monkeypatch.delattr(moor_constants, "venv_python_path", raising=False)
        ...
        expected = Path("/opt/moor/venv/Scripts/python.exe") \
            if sys.platform == "win32" else Path("/opt/moor/venv/bin/python")
        assert _venv_python(Path("/opt/moor/venv")) == expected

    def test_recovery_uses_the_shared_helper_not_a_second_copy(self, monkeypatch):
        """The reload must resolve through moor_constants, not open-code it.

        Hand-rolling `Scripts`/`bin` here is what #76105 deduped away and what
        `test_no_open_coded_venv_layout_remains_in_moor_cli` bans.
        """
        import moor_constants
        ...
        assert _venv_python(Path("/opt/moor/venv")) == sentinel
...
class TestWindowsRuntimeSelfLock:
    ...
    A CLI ``moor update`` runs from the venv's own python, and
    ...
    def test_self_lock_defers_repair_before_provisioning(
        ...
        with patch(
                 "moor_cli.managed_uv._windows_runtime_holders",
                 return_value=(False, ""),
             ), \
             patch(
                 "moor_cli.managed_uv.probe_sqlite_runtime",
    ...
    def test_venv_launcher_ancestor_is_a_self_lock(self, tmp_path, monkeypatch):
        r"""The venv\Scripts\moor.exe shim stays mapped while it waits for
