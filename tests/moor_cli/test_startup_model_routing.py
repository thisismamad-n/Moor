...
def test_startup_route_uses_configured_moor_provider(monkeypatch):
    ...
    route = model_switch.resolve_startup_model_route(
        "moor/deepseek-v4-pro",
        user_providers={"moor": {"base_url": "https://inference.example/v1"}},
    )
    assert route == model_switch.StartupModelRoute("deepseek-v4-pro", "moor", "")
...
def test_startup_route_aggregator_native_slug_stays_on_aggregator(monkeypatch):
    ...
    monkeypatch.setattr(model_switch, "DIRECT_ALIASES", {})
    monkeypatch.setattr(
        "moor_cli.models._find_openrouter_slug",
