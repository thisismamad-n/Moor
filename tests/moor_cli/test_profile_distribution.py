"""Tests for moor_cli.profile_distribution — git-based profile installs.
...
# ---------------------------------------------------------------------------
# Isolated profile env (matches tests/moor_cli/test_profiles.py)
# ---------------------------------------------------------------------------
...
class TestInstall:
    ...
    def test_update_respects_distribution_owned_allowlist(self, profile_env):
        ...
        from moor_cli.profile_distribution import read_manifest as _read
    ...
    def test_install_enforces_moor_requires(self, profile_env, monkeypatch):
        ...
        import moor_cli
        monkeypatch.setattr(moor_cli, "__version__", "0.1.0", raising=False)

        mf = DistributionManifest(
            name="future",
            version="1.0.0",
            moor_requires=">=99.0.0",
        )
...
class TestDescribe:
    ...
    def test_describe_missing_profile_raises(self, profile_env):
        with pytest.raises(DistributionError, match="No profile named .*moor profile list"):
...
class TestInstalledAtStamp:
    ...
    def test_update_refreshes_installed_at(self, profile_env, monkeypatch):
        ...
        monkeypatch.setattr(
            "moor_cli.profile_distribution.datetime", _FakeDT, raising=True
        )

        from moor_cli.profile_distribution import update_distribution
...
class TestProfileInfoDistribution:
    ...
    def test_malformed_manifest_does_not_break_list(self, profile_env):
        from moor_cli.profiles import create_profile, list_profiles, get_profile_dir
