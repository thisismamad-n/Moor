"""Tests that provider selection via `moor model` always persists correctly.
...
def config_home(tmp_path, monkeypatch):
    """Isolated MOOR_HOME with a minimal string-format config."""
    home = tmp_path / "moor"
    home.mkdir()
...
class TestProviderPersistsAfterModelSave:
    ...
    def test_api_key_provider_saved_when_model_was_string(self, config_home, monkeypatch):
        ...
        with patch("moor_cli.auth._prompt_model_selection", return_value="kimi-k2.5"), \
...
class TestZaiEndpointPicker:
    ...
    def test_current_endpoint_is_default_choice(self, config_home, monkeypatch):
        ...
        from moor_cli.auth import ZAI_ENDPOINTS
        from moor_cli.model_setup_flows import _select_zai_endpoint
