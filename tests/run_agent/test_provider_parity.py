...
class TestBuildApiKwargsMoorPortal:
    def test_includes_moor_product_tags(self, monkeypatch):
        ...
        kwargs = agent._build_api_kwargs(messages)
        extra = kwargs.get("extra_body", {})
        assert extra.get("tags") == moor_portal_tags(session_id=agent.session_id)
...
# ── Chat completions response handling (OpenRouter/Moor) ─────────────────────
...
class TestAuxiliaryClientProviderPriority:
    ...
    def test_moor_when_no_openrouter(self, monkeypatch):
        monkeypatch.delenv("OPENROUTER_API_KEY", raising=False)
        from agent.auxiliary_client import _MOOR_MODEL, get_text_auxiliary_client
        moor_auth = {
            "access_token": _fake_invoke_jwt(),
            "scope": "inference:invoke",
        }
        with patch("agent.auxiliary_client._read_moor_auth", return_value=moor_auth), \
             patch("agent.auxiliary_client.OpenAI") as mock, \
             patch("moor_cli.models.get_moor_recommended_aux_model", return_value=None):
            client, model = get_text_auxiliary_client()
        assert model == _MOOR_MODEL

    def test_custom_endpoint_when_no_moor(self, monkeypatch):
        """Custom endpoint is used when no OpenRouter/Moor keys are available.
