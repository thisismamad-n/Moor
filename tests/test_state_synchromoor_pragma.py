...
durability one, and `_enforce_macos_synchromoor_full()` returned early off
...
def _config(monkeypatch, database_section):
    ...
    It imports `moor_cli.config` lazily inside the function body, so the
...
class TestResolveSynchronousLevel:
    ...
    def test_accepts_documented_spellings(self, raw, expected):
        assert resolve_synchromoor_level(raw) == expected
    ...
    def test_rejects_everything_else(self, raw):
        assert resolve_synchromoor_level(raw) is None
    ...
    def test_yaml_off_is_a_level_but_yaml_on_is_not(self):
        ...
        """
        assert resolve_synchromoor_level(False) == 0
        assert resolve_synchromoor_level(True) is None
...
class TestMacOSFloor:
    ...
    `_enforce_macos_synchromoor_full()` runs inside `apply_wal_with_fallback()`,
...
def test_example_config_calls_the_darwin_rule_a_floor_not_a_pin():
    """The macOS wording has to match _apply_synchromoor_pragma's actual rule.
