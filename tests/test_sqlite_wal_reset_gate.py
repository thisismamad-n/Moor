...
from moor_state_wal import apply_wal_with_fallback, is_sqlite_wal_reset_vulnerable, sqlite_source_id
...
class TestApplyWalWalResetGate:
    def test_fresh_db_uses_delete_when_vulnerable(self, tmp_path, monkeypatch, caplog):
        monkeypatch.setattr(
            moor_state_wal, "is_sqlite_wal_reset_vulnerable", lambda version_info=None: True
        )
...
def test_doctor_warns_without_adding_issues(monkeypatch, tmp_path, capsys):
    ...
    monkeypatch.setattr(moor_state_wal, "sqlite_source_id", lambda: "testid-abc")
