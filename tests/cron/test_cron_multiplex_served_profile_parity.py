...
builds must resolve exactly as it would under a standalone ``moor -p <name> gateway run``.
"""
...
def two_homes(tmp_path, monkeypatch):
    ...
    for key, val in (("MOOR_CRON_TIMEOUT", "111"), ("MOOR_MODEL", "default-model"),
                     ("TERMINAL_ENV", "docker"), ("TERMINAL_DOCKER_IMAGE", "default-only-image"),
                     ("MOOR_LANGUAGE", "en")):
        monkeypatch.setenv(key, val)
    set_multiplex_active(True)
    home_token = set_moor_home_override(str(alpha))
    try:
        yield root, alpha
    finally:
...
def test_cron_env_settings_resolve_from_the_served_profile(two_homes):
    """``MOOR_CRON_TIMEOUT`` / ``MOOR_MODEL`` are alpha's (or absent) both with the fire-time
    ...
    try:
        ...
        try:
            ...
            assert captured["target_model"] == ""  # alpha has no MOOR_MODEL; default's must not leak
...
def test_child_env_for_served_profile_drops_launch_profile_settings(two_homes):
    ...
    try:
        ...
        reset_secret_scope(token)
    for key in ("MOOR_MODEL", "TERMINAL_ENV", "TERMINAL_DOCKER_IMAGE", "MOOR_LANGUAGE", "MOOR_CRON_TIMEOUT"):
        assert key not in env, key
    assert env["MOOR_HOME"] == str(alpha)
    assert "PATH" in env

    # No-op when the scope IS the launch profile (standalone / default profile's own children).
    reset_moor_home_override(set_moor_home_override(None))
    home_token = set_moor_home_override(str(root))
    try:
        env = strip_launch_profile_env(build_subprocess_env(scrub_secrets=True, inherit_profile_home=True))
    finally:
        reset_moor_home_override(home_token)
    assert env["TERMINAL_ENV"] == "docker"
    assert env["MOOR_MODEL"] == "default-model"
