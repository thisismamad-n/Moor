"""
Tests for timezone support (moor_time module + integration points).
...
# =========================================================================
# moor_time.now() — core helper
# =========================================================================

class TestMoorTimeNow:
...
class TestGetTimezone:
    ...
    def test_returns_zoneinfo_for_valid(self):
        os.environ["MOOR_TIMEZONE"] = "Europe/London"
    ...
    def test_multiplex_prefers_routed_profile_config_over_env(self, tmp_path, monkeypatch):
        """Under the multiplexed gateway MOOR_TIMEZONE holds only the DEFAULT profile's value
        ...
        assert moor_time.get_timezone_name() == "America/New_York"

        set_multiplex_active(True)
        try:
            assert moor_time.get_timezone_name() == "America/New_York"  # default profile turn
            token = set_moor_home_override(str(routed_home))
            try:
                assert moor_time.get_timezone_name() == "Asia/Tokyo"
                assert str(moor_time.get_timezone()) == "Asia/Tokyo"
            finally:
...
class TestCodeExecutionTZ:
    ...
    def test_tz_injected_when_configured(self):
        """When MOOR_TIMEZONE is set, child process sees TZ env var.
        ...
        #   2) MOOR_TIMEZONE itself does NOT leak into the child env
        probe = (
            'import os; '
            'print("TZ=" + os.environ.get("TZ", "NOT_SET")); '
            'print("MOOR_TIMEZONE=" + os.environ.get("MOOR_TIMEZONE", "NOT_SET"))'
        )
        ...
        assert "MOOR_TIMEZONE=NOT_SET" in result["output"], (
            "MOOR_TIMEZONE should not leak into child env (only TZ)"
        )

    def test_tz_not_injected_when_empty(self):
        """When MOOR_TIMEZONE is not set, child process has no TZ."""
        import json as _json
        os.environ.pop("MOOR_TIMEZONE", None)
...
class TestCronTimezone:
    ...
    def test_ensure_aware_naive_preserves_absolute_time(self):
        ...
        Regression: the old code used replace(tzinfo=moor_tz) which shifted
    ...
    def test_get_due_jobs_naive_cross_timezone(self, tmp_path, monkeypatch):
        ...
        # of the naive timestamp exceeds _moor_now's wall time — this would
        # have caused a false "not due" with the old replace(tzinfo=...) approach.
        os.environ["MOOR_TIMEZONE"] = "Pacific/Midway"  # UTC-11
        _reset_moor_time_cache()
    ...
    def test_create_job_stores_tz_aware_timestamps(self, tmp_path, monkeypatch):
        ...
        os.environ["MOOR_TIMEZONE"] = "US/Eastern"
        _reset_moor_time_cache()
