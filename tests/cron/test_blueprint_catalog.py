...
the shared /blueprint command handler (moor_cli/blueprint_cmd.py), and
the docs generator. Uses an isolated MOOR_HOME for anything that touches the
cron job store.
"""
...
class TestRenderers:
    ...
    def test_deeplink_shape(self):
        ...
        assert url.startswith("moor://blueprint/morning-brief?")
