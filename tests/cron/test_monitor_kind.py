...
def moor_env(tmp_path, monkeypatch):
    """Isolate MOOR_HOME for each test so jobs/scripts/snapshots don't leak."""
    home = tmp_path / ".moor"
    home.mkdir()
    (home / "scripts").mkdir()
    (home / "cron").mkdir()
...
def _install_agent_stubs(monkeypatch, observed: dict):
    ...
    from moor_cli import runtime_provider as _rtp
    monkeypatch.setattr(
        _rtp,
    ...
    monkeypatch.setenv("MOOR_CRON_TIMEOUT", "0")
...
def test_create_job_stores_monitor_script(moor_env):
    from cron.jobs import create_job, get_job

    _write_script(moor_env, "mon.sh", "echo stable\n")
    job = create_job(
...
def test_create_job_monitor_script_and_url_mutually_exclusive(moor_env):
...
def test_create_job_monitor_rejected_with_no_agent(moor_env):
    from cron.jobs import create_job

    _write_script(moor_env, "w.sh", "echo hi\n")
...
def test_update_job_rejects_no_agent_on_monitor_job(moor_env):
...
def test_update_job_rejects_adding_monitor_to_no_agent_job(moor_env):
    from cron.jobs import create_job, update_job
...
def test_update_job_rejects_second_monitor_source(moor_env):
    from cron.jobs import create_job, update_job
...
def test_update_job_allows_clearing_monitor_then_no_agent(moor_env):
...
def test_update_job_unrelated_fields_skip_mode_validation(moor_env):
...
def test_hash_is_exact_bytes(moor_env):
...
def test_unified_diff_is_capped(moor_env):
...
def _make_monitor_job(moor_env, script_body: str):
    from cron.jobs import create_job

    _write_script(moor_env, "mon.sh", script_body)
    return create_job(
...
def test_first_run_always_runs_agent(moor_env, monkeypatch):
    from cron.scheduler import run_job

    job = _make_monitor_job(moor_env, "echo 'state A'\n")
    observed: dict = {}
...
def test_bidi_monitor_output_is_sanitized_before_agent(moor_env, monkeypatch):
    """Runtime monitor data with a WhatsApp-style bidi marker must not block the job (#111523)."""
    from cron.scheduler import run_job

    job = _make_monitor_job(moor_env, "printf 'Alice\\342\\200\\252 Work\\n'\n")
    observed: dict = {}
    _install_agent_stubs(monkeypatch, observed)
...
def test_unchanged_output_suppresses_agent_run(moor_env, monkeypatch):
...
def test_changed_output_injects_diff(moor_env, monkeypatch):
    from cron.jobs import get_job
    from cron.scheduler import run_job
    ...
    _write_script(moor_env, "mon.sh", "echo 'state B'\n")
    job = get_job(job["id"])
    success, doc, final, error = run_job(job)
    assert success is True
...
def test_hash_persists_across_scheduler_restart(moor_env, monkeypatch):
...
def test_monitor_script_failure_is_error_not_change(moor_env, monkeypatch):
    from cron.jobs import get_job
    from cron.scheduler import run_job
    ...
    _write_script(moor_env, "mon.sh", "echo boom >&2\nexit 3\n")
    job = get_job(job["id"])
    success, doc, final, error = run_job(job)
...
def test_cronjob_tool_create_with_monitor_script(moor_env):
    from cron.jobs import get_job
    from tools.cronjob_tools import cronjob

    _write_script(moor_env, "mon.sh", "echo hi\n")
    result = json.loads(
        cronjob(
            action="create",
            prompt="React",
...
def test_cronjob_tool_rejects_monitor_script_path_escape(moor_env):
    from tools.cronjob_tools import cronjob
...
def test_cronjob_tool_update_clears_monitor_script(moor_env):
    from cron.jobs import get_job
    from tools.cronjob_tools import cronjob
