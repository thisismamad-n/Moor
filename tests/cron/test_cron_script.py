...
- Path resolution (absolute, relative to MOOR_HOME/scripts/)
"""
...
def cron_env(tmp_path, monkeypatch):
    """Isolated cron environment with temp MOOR_HOME."""
    moor_home = tmp_path / ".moor"
    moor_home.mkdir()
    (moor_home / "cron").mkdir()
    (moor_home / "cron" / "output").mkdir()
    (moor_home / "scripts").mkdir()
    monkeypatch.setenv("MOOR_HOME", str(moor_home))

    # Clear cached module-level paths
    import cron.jobs as jobs_mod
    monkeypatch.setattr(jobs_mod, "MOOR_DIR", moor_home)
    monkeypatch.setattr(jobs_mod, "CRON_DIR", moor_home / "cron")
    monkeypatch.setattr(jobs_mod, "JOBS_FILE", moor_home / "cron" / "jobs.json")
    monkeypatch.setattr(jobs_mod, "OUTPUT_DIR", moor_home / "cron" / "output")
...
class TestRunJobScript:
    ...
    def test_script_subprocess_env_sanitized(self, cron_env, monkeypatch):
        ...
        from tools.environments.local import _MOOR_PROVIDER_ENV_BLOCKLIST
        from cron.scheduler import _run_job_script

        # sorted() so the probed var is deterministic across runs
        # (frozenset iteration order varies with PYTHONHASHSEED).
        blocked_var = sorted(_MOOR_PROVIDER_ENV_BLOCKLIST)[0]
...
class TestScriptPathContainment:
    ...
    def test_absolute_path_outside_scripts_dir_blocked(self, cron_env):
        """Absolute paths outside ~/.moor/scripts/ must be rejected."""
        from cron.scheduler import _run_job_script
...
class TestRunJobEnvVarCleanup:
    ...
    def test_env_vars_cleaned_on_early_error(self, cron_env, monkeypatch):
        ...
        assert os.environ.get("MOOR_SESSION_PLATFORM") is None
        assert os.environ.get("MOOR_SESSION_CHAT_ID") is None
        assert os.environ.get("MOOR_SESSION_CHAT_NAME") is None
