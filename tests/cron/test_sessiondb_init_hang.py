...
which is already bounded by MOOR_CRON_TIMEOUT — the worker thread submitted
...
timeout (MOOR_CRON_SESSION_DB_TIMEOUT, default 10s) so a hang there can
...
class TestSessionDbInitTimeout:
    def test_sessiondb_init_preserves_multiplex_profile_context(
        ...
        profile_token = set_moor_home_override(profile_home)
        try:
            with patch("cron.scheduler._moor_home", None), \
                 patch("cron.scheduler_delivery._resolve_origin", return_value=None), \
                 patch("moor_cli.env_loader.load_moor_dotenv"), \
                 patch("moor_cli.env_loader.reset_secret_source_cache"), \
                 patch("moor_state_registry.acquire", side_effect=make_session_db), \
                 patch(
    ...
    def test_run_job_does_not_hang_when_sessiondb_init_wedges(self, tmp_path, monkeypatch):
        ...
        with patch("cron.scheduler._moor_home", tmp_path), \
             ...
             patch("moor_cli.env_loader.load_moor_dotenv"), \
             patch("moor_cli.env_loader.reset_secret_source_cache"), \
             patch("moor_state_registry.acquire"), \
             patch(
                 "moor_cli.runtime_provider.resolve_runtime_provider",
                 return_value=_RUNTIME,
             ), \
    ...
    def test_invalid_timeout_env_falls_back_to_default(self, tmp_path, monkeypatch, caplog):
        """A malformed MOOR_CRON_SESSION_DB_TIMEOUT logs a warning and still
        bounds the call (mirrors MOOR_CRON_TIMEOUT's own fallback)."""
        monkeypatch.setenv("MOOR_CRON_SESSION_DB_TIMEOUT", "not-a-number")
        fake_db = MagicMock()
        ...
        assert any(
            "MOOR_CRON_SESSION_DB_TIMEOUT" in rec.message
    ...
    def test_late_session_db_result_is_closed(self, tmp_path, monkeypatch):
        ...
        try:
            with patch("cron.scheduler._moor_home", tmp_path), \
                 ...
                 patch("moor_cli.env_loader.load_moor_dotenv"), \
                 patch("moor_cli.env_loader.reset_secret_source_cache"), \
                 patch("moor_state_registry.acquire", side_effect=_hanging_then_capture), \
                 patch(
                     "moor_cli.runtime_provider.resolve_runtime_provider",
                     return_value={
                         "api_key": "test-key",
...
class TestSessionDbInitAfterEarlyReturns:
    ...
    def test_wake_gate_false_never_opens_session_db(self, tmp_path, monkeypatch):
        ...
        with patch("cron.scheduler._moor_home", tmp_path), \
             ...
             patch("moor_cli.env_loader.load_moor_dotenv"), \
             patch("moor_cli.env_loader.reset_secret_source_cache"), \
             patch("moor_state_registry.acquire") as mock_db_cls, \
             patch(
