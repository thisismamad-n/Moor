...
def test_standalone_fallback_pool_keeps_profile_scope(tmp_path, monkeypatch):
    ...
    from moor_constants import get_moor_home, set_moor_home_override
    ...
    async def _inside_running_loop():
        ...
        set_moor_home_override(str(sec_home))
...
def test_desktop_ticker_gates_on_profile_gateway_running(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "moor_cli.profiles._check_gateway_running", lambda home: home.name == "ops"
    )
    captured = {}
    ...
    monkeypatch.setattr("moor_logging.enable_profile_log_routing", lambda homes: None)
