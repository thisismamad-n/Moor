...
def multiplex_homes(tmp_path, monkeypatch):
    ...
    ``get_default_moor_root`` is patched so the primary config, the profiles
    ...
    monkeypatch.setattr(
        "moor_constants.get_default_moor_root", lambda: root
    )
    token = set_moor_home_override(str(grant_home))
