...
profile-scoped agent session writes to ~/.moor/profiles/<p>/cron/jobs.json,
while the profile-less gateway reads only ~/.moor/cron/jobs.json — the job
...
def test_cron_storage_anchors_at_root_under_profile(tmp_path, monkeypatch):
    ...
    # MOOR_HOME is a profile under it (the #32091 scenario).
    import moor_constants
    monkeypatch.setattr(moor_constants, "_get_platform_default_moor_home",
                        lambda: root)
    monkeypatch.setenv("MOOR_HOME", str(profile_home))

    # get_default_moor_root must return the ROOT, not the profile dir.
    assert moor_constants.get_default_moor_root().resolve() == root.resolve()
    # ...while get_moor_home (used elsewhere) follows the profile override.
    assert moor_constants.get_moor_home().resolve() == profile_home.resolve()

    # cron/jobs.py computes MOOR_DIR from get_default_moor_root at import,
    # so a fresh import under this env anchors the store at <root>/cron.
    import cron.jobs as jobs
    importlib.reload(jobs)
    try:
        assert jobs.MOOR_DIR.resolve() == root.resolve()
...
def test_cron_storage_unaffected_when_no_profile(tmp_path, monkeypatch):
    """With no profile (MOOR_HOME == root), behavior is unchanged: store at
    <root>/cron."""
    root = tmp_path / "moor_home"
...
def test_tick_lock_anchors_at_root_under_profile(tmp_path, monkeypatch):
    ...
    try:
        # _moor_home override is None -> uses get_default_moor_root()
        sched._moor_home = None
...
def test_get_default_moor_root_docker_layouts(tmp_path, monkeypatch):
    """get_default_moor_root resolves the root for Docker/custom MOOR_HOME
    (outside ~/.moor), so cron storage works in containers."""
    import moor_constants
    native = tmp_path / "native_home"
    monkeypatch.setattr(moor_constants, "_get_platform_default_moor_home", lambda: native)

    # Docker custom root (outside native): MOOR_HOME itself IS the root.
    monkeypatch.setenv("MOOR_HOME", "/opt/data")
    assert moor_constants.get_default_moor_root() == Path("/opt/data")

    # Docker profile layout: <custom>/profiles/<name> -> <custom>.
    monkeypatch.setenv("MOOR_HOME", "/opt/data/profiles/coder")
    assert moor_constants.get_default_moor_root() == Path("/opt/data")
