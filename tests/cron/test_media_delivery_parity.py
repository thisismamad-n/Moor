...
text-only on manual ``moor cron run <job-id>``. Same box, same token, same
...
def _install_fake_slack_sender(monkeypatch, result_factory):
    ...
    if entry is None:
        ...
        import moor_cli.plugins as hp_boot
...
def slack_platform_config(monkeypatch, tmp_path):
    ...
    # Config caches are process-global; clear them so the temp MOOR_HOME wins.
    try:
...
class TestMediaPolicyEnvBridge:
    ...
    def test_bridge_helper_exists_and_applies_config(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setenv("MOOR_HOME", str(home))
        for var in (
            "MOOR_MEDIA_DELIVERY_STRICT",
            "MOOR_MEDIA_ALLOW_DIRS",
            "MOOR_MEDIA_TRUST_RECENT_FILES",
        ):
        ...
        assert os.environ.get("MOOR_MEDIA_DELIVERY_STRICT") == "1"
        assert str(allow_dir) in os.environ.get("MOOR_MEDIA_ALLOW_DIRS", "")
        assert os.environ.get("MOOR_MEDIA_TRUST_RECENT_FILES") == "0"
    ...
    def test_deliver_result_runs_bridge(self, monkeypatch, tmp_path, media_file):
        ...
        Realistic enterprise shape: MOOR_MEDIA_DELIVERY_STRICT=1 arrives via
