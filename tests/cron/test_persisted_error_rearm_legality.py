...
def cron_store(tmp_path, monkeypatch):
    ...
    (moor_home / "cron").mkdir(parents=True)
    monkeypatch.setenv("MOOR_HOME", str(moor_home))
    monkeypatch.setattr(J, "MOOR_DIR", moor_home)
    monkeypatch.setattr(J, "CRON_DIR", moor_home / "cron")
    monkeypatch.setattr(J, "JOBS_FILE", moor_home / "cron" / "jobs.json")
    monkeypatch.setattr(J, "OUTPUT_DIR", moor_home / "cron" / "output")
...
class TestCronRearmRespectsScheduleLegality:
    def test_weekday_job_errored_friday_does_not_fire_on_saturday(self, cron_store):
        ...
        with mock.patch.object(J, "_moor_now", lambda: now):
            due = J.get_due_jobs()
