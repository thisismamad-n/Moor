...
def test_roster_roundtrip_and_validation(root):
    rows = _rows() + [
        ...
        "not-a-dict",
        {"profile": "default", "handle": "moor", "connection_id": "cloud-1"},  # dupe
    ]
...
def test_resolve_remote_target_forms(root):
    ...
    assert bot_relay.resolve_remote_target("@moor", roster)["profile"] == "default"
    ...
    assert bot_relay.resolve_remote_target("moor@cloud-1", roster)["profile"] == "default"
    ...
    assert bot_relay.resolve_remote_target("moor@nope", roster) is None
...
def test_resolve_ambiguous_handle_across_connections(root):
    ...
    assert "moor" in forms  # unique handle stays bare
...
def test_relay_route_queues_envelope_and_spawns_waiter(tmp_path, monkeypatch):
    ...
    bot_relay.write_remote_roster(home, [
        {"profile": "default", "handle": "moor", "connection_id": "cloud-1",
    ...
    assert pending[0]["message"].startswith("Message from 🤖 moor (@moor): ping")
...
def test_protocol_section_lists_remote_teammates(tmp_path):
    ...
    assert "`@moor` — on Moor Cloud — Moxie" in section
...
def test_cleanup_bot_relay_artifacts_missing_dir_is_zero(tmp_path, monkeypatch):
    monkeypatch.setenv("MOOR_HOME", str(tmp_path / "nope"))
...
def test_message_agent_surfaces_runtime_offline_refusal(tmp_path, monkeypatch):
    ...
    bot_relay.write_remote_roster(home, [
        {"profile": "default", "handle": "moor", "connection_id": "cloud-1",
         "connection_label": "Moor Cloud", "online": False},
    ])
    monkeypatch.setattr(
        "tools.bot_mode_dm._spawn_delivery",
        lambda *a, **k: json.dumps({"status": "sent"}),
