...
from tools.skills_hub_search import (
    MOOR_INDEX_TTL, _load_moor_index, create_source_router, parallel_search_sources, unified_search,
)
...
class TestParseFrontmatterQuick:
    def test_valid_frontmatter_including_nested_yaml(self):
        ...
        nested = "---\nname: test\nmetadata:\n  moor:\n    tags: [a, b]\n---\n\nBody.\n"
        assert GitHubSource._parse_frontmatter_quick(nested)["metadata"]["moor"]["tags"] == ["a", "b"]
...
def _make_index_source(skills):
    """Build a MoorIndexSource pre-loaded with a fixed skill list."""
    from tools.skills_hub_official import MoorIndexSource
    src = MoorIndexSource(auth=GitHubAuth())
...
class TestProviderFilter:
    ...
    def test_unified_search_provider_filter_keeps_index_source(self):
        ...
        src = MagicMock()
        src.source_id.return_value = "moor-index"
...
class TestOptionalSkillSourceLiveRepoFallback:
    ...
    searchable and installable without `moor update` (live-repo fallback)."""
...
class TestInstallPathSafety:
    ...
    def test_install_from_quarantine_rejects_category_bucket_overwrite(self, tmp_path):
        ...
        Regression test for GitHub issue #75983: ``moor skills install … --name
...
class TestLoadMoorIndex:
    ...
    def _isolate_cache(monkeypatch, tmp_path):
        ...
        cache_file = tmp_path / "moor-index.json"
        monkeypatch.setattr("tools.skills_hub_search._moor_index_cache_file", lambda: cache_file)
        return cache_file
    ...
    def test_persistent_decoding_error_falls_back_to_stale_cache(
        ...
        old = time.time() - (MOOR_INDEX_TTL + 100)
        import os
