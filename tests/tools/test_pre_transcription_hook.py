...
PROMPT = "Moor, Teknium, Moor inc., kanban"
...
def _fake_hooks(monkeypatch, results):
    ...
    monkeypatch.setattr("moor_cli.plugins.invoke_hook", _invoke)
    return captured
...
def _no_hooks(monkeypatch):
    ...
    monkeypatch.setattr("moor_cli.plugins.has_hook", lambda name: False)
    monkeypatch.setattr("moor_cli.plugins.invoke_hook", _boom)
...
class TestUnsupportedBackends:
    def test_xai_logs_debug_and_proceeds_without_prompt(
        ...
        monkeypatch.setattr(
            "tools.xai_http.moor_xai_user_agent", lambda: "test-ua",
        )
