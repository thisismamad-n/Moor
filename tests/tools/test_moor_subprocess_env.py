"""Tests for moor_subprocess_env() — the centralized credential-safe env
...
full credential environment. Two tiers:
  ...
  * Tier 2 (_MOOR_PROVIDER_ENV_BLOCKLIST): LLM provider/tool keys — stripped
...
from tools.environments.local_env_policy import _ALWAYS_STRIP_KEYS, _MOOR_PROVIDER_ENV_FORCE_PREFIX
...
_TIER1_SAMPLE = {
    ...
    "MOOR_DASHBOARD_SESSION_TOKEN": "dash-secret",
}
...
def _build(extra=None, *, inherit_credentials=False):
    ...
    with patch.dict(os.environ, env, clear=True):
        return moor_subprocess_env(inherit_credentials=inherit_credentials)
...
class TestStripByDefault:
    ...
    def test_buzz_platform_vars_stripped_by_default(self):
        ...
        for var in buzz_sample:
            assert var not in result, f"{var} leaked via moor_subprocess_env"
...
class TestDelegatedChildMarker:
    def test_delegated_child_context_scrubs_parent_kanban_keys_and_sets_marker(self):
        ...
        with patch.dict(
            os.environ,
            {
                **_SAFE_SAMPLE,
                ...
                "MOOR_KANBAN_RUN_ID": "123",
                "MOOR_KANBAN_DB": "/tmp/parent-kanban.db",
                "MOOR_KANBAN_WORKSPACE": "/tmp/parent-workspace",
            },
            clear=True,
        ):
        ...
        assert env["MOOR_KANBAN_DB"] == "/tmp/parent-kanban.db"
        assert env["MOOR_KANBAN_WORKSPACE"] == "/tmp/parent-workspace"
