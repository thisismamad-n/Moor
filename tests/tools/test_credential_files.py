...
class TestRegisterCredentialFiles:
    def test_dict_with_path_key(self, tmp_path):
        ...
        moor_home.mkdir()
        (moor_home / "token.json").write_text("{}")

        with patch.dict(os.environ, {"MOOR_HOME": str(moor_home)}):
            missing = register_credential_files([{"path": "token.json"}])

        assert missing == []
        mounts = get_credential_file_mounts()
        assert len(mounts) == 1
        assert mounts[0]["host_path"] == str(moor_home / "token.json")
        assert mounts[0]["container_path"] == "/root/.moor/token.json"
    ...
    def test_path_takes_precedence_over_name(self, tmp_path):
        ...
        moor_home = tmp_path / ".moor"
        moor_home.mkdir()
        (moor_home / "real.json").write_text("{}")
...
class TestSkillsDirectoryMount:
    def test_returns_mount_when_skills_dir_exists(self, tmp_path):
        ...
        assert mounts[0]["container_path"] == "/root/.moor/skills"


    def test_custom_container_base(self, tmp_path):
        moor_home = tmp_path / ".moor"
        (moor_home / "skills").mkdir(parents=True)

        with patch.dict(os.environ, {"MOOR_HOME": str(moor_home)}):
            mounts = get_skills_directory_mount(container_base="/home/user/.moor")

        assert mounts[0]["container_path"] == "/home/user/.moor/skills"
...
class TestIterSkillsFiles:
    def test_returns_files_skipping_symlinks(self, tmp_path):
        ...
        assert "/root/.moor/skills/cat/myskill/SKILL.md" in paths
        assert "/root/.moor/skills/cat/myskill/scripts/run.sh" in paths
    ...
    def test_skips_excluded_bookkeeping_dirs(self, tmp_path):
        ...
        paths = {f["container_path"] for f in files}
        assert "/root/.moor/skills/cat/myskill/SKILL.md" in paths
        assert "/root/.moor/skills/cat/myskill/references/api.md" in paths
        for excluded in (
            ".hub",
            ".archive",
            ".curator_backups",
...
class TestPathTraversalSecurity:
    ...
    def test_dotdot_traversal_rejected(self, tmp_path, monkeypatch):
        """'../sensitive' must not escape MOOR_HOME."""
        monkeypatch.setenv("MOOR_HOME", str(tmp_path / ".moor"))
        (tmp_path / ".moor").mkdir()

        # Create a sensitive file one level above moor_home
    ...
    def test_deep_traversal_rejected(self, tmp_path, monkeypatch):
        ...
        # Create a fake sensitive file outside moor_home
    ...
    def test_nested_subdir_inside_moor_home_allowed(self, tmp_path, monkeypatch):
        """Files in subdirectories of MOOR_HOME must be allowed."""
        moor_home = tmp_path / ".moor"
        moor_home.mkdir()
        subdir = moor_home / "creds"
        subdir.mkdir()
    ...
    def test_symlink_traversal_rejected(self, tmp_path, monkeypatch):
        """A symlink inside MOOR_HOME pointing outside must be rejected."""
        moor_home = tmp_path / ".moor"
        moor_home.mkdir()
        monkeypatch.setenv("MOOR_HOME", str(moor_home))

        # Create a sensitive file outside moor_home
        sensitive = tmp_path / "sensitive.json"
        sensitive.write_text('{"secret": "value"}')

        # Create a symlink inside moor_home pointing outside
        symlink = moor_home / "evil_link.json"
        try:
        ...
        # The resolved path escapes MOOR_HOME — must be rejected
        assert result is False
        assert get_credential_file_mounts() == []
...
class TestConfigPathTraversal:
    ...
    def test_config_traversal_rejected(self, tmp_path, monkeypatch):
        """'../secret' in config.yaml must not escape MOOR_HOME."""
        moor_home = tmp_path / ".moor"
        moor_home.mkdir()
        monkeypatch.setenv("MOOR_HOME", str(moor_home))
        ...
        sensitive.write_text("{}")
        self._write_config(moor_home, ["../secret.json"])
    ...
    def test_config_absolute_path_rejected(self, tmp_path, monkeypatch):
        ...
        sensitive.write_text("{}")
        self._write_config(moor_home, [str(sensitive)])

        mounts = get_credential_file_mounts()
        assert mounts == []

    def test_config_legitimate_file_works(self, tmp_path, monkeypatch):
        """Normal files inside MOOR_HOME via config must still mount."""
        moor_home = tmp_path / ".moor"
        moor_home.mkdir()
        monkeypatch.setenv("MOOR_HOME", str(moor_home))

        (moor_home / "oauth.json").write_text("{}")
        self._write_config(moor_home, ["oauth.json"])
...
class TestCacheDirectoryMounts:
    ...
    def test_returns_existing_cache_dirs(self, tmp_path, monkeypatch):
        ...
        moor_home = tmp_path / ".moor"
        moor_home.mkdir()
        (moor_home / "cache" / "documents").mkdir(parents=True)
        (moor_home / "cache" / "audio").mkdir(parents=True)
        (moor_home / "cache" / "videos").mkdir(parents=True)
        monkeypatch.setenv("MOOR_HOME", str(moor_home))

        mounts = get_cache_directory_mounts()
        paths = {m["container_path"] for m in mounts}
        assert "/root/.moor/cache/documents" in paths
        assert "/root/.moor/cache/audio" in paths
        assert "/root/.moor/cache/videos" in paths
    ...
    def test_legacy_dir_names_resolved(self, tmp_path, monkeypatch):
        ...
        ``has content`` for ``get_moor_dir``'s populated-legacy check
        (see #27602 — empty legacy stubs are no longer honoured).
        """
        moor_home = tmp_path / ".moor"
        moor_home.mkdir()
        # Use legacy dir name with content — get_moor_dir prefers
        # populated old over new.
        legacy_doc = moor_home / "document_cache"
        legacy_img = moor_home / "image_cache"
        legacy_doc.mkdir()
        legacy_img.mkdir()
        ...
        mounts = get_cache_directory_mounts()
        host_paths = {m["host_path"] for m in mounts}
        assert str(moor_home / "document_cache") in host_paths
        assert str(moor_home / "image_cache") in host_paths
        # Container paths always use the new layout
        container_paths = {m["container_path"] for m in mounts}
        assert "/root/.moor/cache/documents" in container_paths
        assert "/root/.moor/cache/images" in container_paths
    ...
    def test_empty_moor_home(self, tmp_path, monkeypatch):
        ...
        mounts = get_cache_directory_mounts()
        container_paths = {m["container_path"] for m in mounts}
        assert "/root/.moor/attachments" in container_paths
        assert "/root/.moor/images" in container_paths
        assert "/root/.moor/cache/images" in container_paths
        for mount in mounts:
    ...
    def test_images_upload_dir_is_mounted(self, tmp_path, monkeypatch):
        ...
        Desktop / clipboard / PDF uploads land in ``MOOR_HOME/images``, not
        ...
        """
        moor_home = tmp_path / ".moor"
        (moor_home / "images").mkdir(parents=True)
        monkeypatch.setenv("MOOR_HOME", str(moor_home))

        mounts = get_cache_directory_mounts()
        by_container = {m["container_path"]: m["host_path"] for m in mounts}
        assert "/root/.moor/images" in by_container
        assert by_container["/root/.moor/images"] == str(moor_home / "images")
    ...
    def test_images_upload_file_maps_into_container(self, tmp_path, monkeypatch):
        ...
        moor_home = tmp_path / ".moor"
        (moor_home / "images").mkdir(parents=True)
        upload = moor_home / "images" / "upload_20260722_181019_1.png"
        upload.write_bytes(bytes.fromhex("89504e470d0a1a0a"))
        monkeypatch.setenv("MOOR_HOME", str(moor_home))

        assert (
            map_cache_path_to_container(str(upload))
            == "/root/.moor/images/upload_20260722_181019_1.png"
        )
...
class TestMapCachePathToContainer:
    ...
    def test_maps_path_under_cache_dir(self, tmp_path, monkeypatch):
        moor_home = tmp_path / ".moor"
        img_dir = moor_home / "cache" / "images"
        ...
        assert (
            ...
            == "/root/.moor/cache/images/generated.png"
        )
    ...
    def test_maps_path_even_when_cache_dir_missing(self, tmp_path, monkeypatch):
        ...
        mapped = map_cache_path_to_container(str(moor_home / "cache" / "images" / "x.png"))
        assert mapped == "/root/.moor/cache/images/x.png"
        assert (moor_home / "cache" / "images").is_dir()
...
class TestToAgentVisiblePathPerBackend:
    ...
    def _staged(self, tmp_path, monkeypatch):
        moor_home = tmp_path / ".moor"
        (moor_home / "attachments").mkdir(parents=True)
        monkeypatch.setenv("MOOR_HOME", str(moor_home))
        return str(moor_home / "attachments" / "drop.zip")

    def test_docker_maps_to_root_moor(self, tmp_path, monkeypatch):
        staged = self._staged(tmp_path, monkeypatch)
        monkeypatch.setenv("TERMINAL_ENV", "docker")
        from tools.credential_files import to_agent_visible_cache_path
        assert to_agent_visible_cache_path(staged) == "/root/.moor/attachments/drop.zip"

    def test_ssh_maps_to_tilde_moor(self, tmp_path, monkeypatch):
        staged = self._staged(tmp_path, monkeypatch)
        monkeypatch.setenv("TERMINAL_ENV", "ssh")
        from tools.credential_files import to_agent_visible_cache_path
        assert to_agent_visible_cache_path(staged) == "~/.moor/attachments/drop.zip"
...
class TestIterCacheFiles:
    ...
    def test_enumerates_files(self, tmp_path, monkeypatch):
        ...
        moor_home = tmp_path / ".moor"
        doc_dir = moor_home / "cache" / "documents"
...
class TestMasterCredentialStoresAreNeverMountable:
    """Containment is not enough — MOOR_HOME *is* where the keys live.
    ...
    ``../../.ssh/id_rsa`` from escaping MOOR_HOME, but every master
    ...
    def test_refused_entry_does_not_block_the_rest_of_the_batch(self, tmp_path):
        ...
        paths = [m["container_path"] for m in mounts]
        assert "/root/.moor/google_token.json" in paths
        assert "/root/.moor/.env" not in paths
