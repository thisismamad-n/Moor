...
Older images made /opt/moor/ui-tui and /opt/moor/node_modules writable so a
...
def test_dockerfile_uses_prebuilt_tui_instead_of_writable_node_modules() -> None:
    ...
    assert "ENV MOOR_TUI_DIR=/opt/moor/ui-tui" in text
    assert "cd ../ui-tui && npm run build" in text
    assert "chown -R moor:moor /opt/moor/ui-tui" not in text
    assert "chown -R moor:moor /opt/moor/node_modules" not in text
