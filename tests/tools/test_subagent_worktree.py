...
class SubagentWorktreeTests(unittest.TestCase):
    def setUp(self):
        self.tmp = Path(tempfile.mkdtemp(prefix="moor-sw-test-"))
    ...
    def test_create_makes_isolated_worktree(self):
        ...
        self.assertEqual(info["branch"], "moor-subagent/subagent-abc123")
...
class WorktreePayloadSchemaTests(unittest.TestCase):
    ...
    def setUp(self):
        self.tmp = Path(tempfile.mkdtemp(prefix="moor-sw-schema-"))
        self.addCleanup(shutil.rmtree, self.tmp, True)
