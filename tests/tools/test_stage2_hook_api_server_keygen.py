...
was gated on ``[ -f "$MOOR_HOME/.env" ]`` while the first-boot seed that
...
def _run_keygen(
    ...
    script = (
        ...
        f"{env_setup}"
        f'MOOR_HOME="{home}"\n'
        # In tests we run unprivileged; as_moor is a passthrough then.
        'as_moor() { "$@"; }\n'
...
def test_keygen_skips_when_container_env_provides_key(
    ...
    Moor loads $MOOR_HOME/.env with override=True, so a key generated
...
def test_dockerignore_keeps_env_example_template() -> None:
    """The first-boot seed copies /opt/moor/.env.example -> $MOOR_HOME/.env.
    ...
    assert verdict == "included", (
        ...
        "seeds $MOOR_HOME/.env from it on first boot"
    )
