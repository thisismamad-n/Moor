...
# boundary, blocking `moor send` message bodies, multi-line
# `git commit -m` messages, and heredoc text that merely MENTION
# shutdown/reboot commands.
_QUOTED_NEWLINE_DATA_ALLOW = [
    # moor send with a multi-line message body (the reported symptom)
    'moor send -t telegram -s "spark1" "console output:\nsudo reboot\ndone"',
    'moor send -t telegram "line1\nshutdown -h now\nline3"',
...
_QUOTED_NEWLINE_THREATS_BLOCK = [
    ...
    'moor send -t telegram "$(sudo reboot)"',
    'echo "`shutdown -h now`"',
...
def test_quoted_newline_data_not_blocked_by_full_guard_chain(clean_session):
    ...
    command = 'moor send -t telegram "status:\nsudo reboot happened at 3am"'
...
def test_yolo_env_var_cannot_bypass_hardline(clean_session, monkeypatch):
    """MOOR_YOLO_MODE=1 must not bypass the hardline floor."""
