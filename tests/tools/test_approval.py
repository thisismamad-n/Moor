...
class TestApprovalModeParsing:
    ...
    def test_config_bool_false_maps_to_off(self):
        with mock_patch("moor_cli.config.load_config_readonly", return_value={"approvals": {"mode": False}}):
...
class TestDetectDangerousRm:
    ...
    def test_nonrecursive_verification_artifact_cleanup_is_not_dangerous(self):
        with mock_patch("tempfile.gettempdir", return_value="/tmp"):
            for prefix in ("moor-verify-", "moor-ad-hoc-"):
    ...
    def test_symlinked_temp_dir_only_exempts_canonical_target(self, tmp_path):
        ...
        basename = "moor-verify-example.py"
    ...
    def test_verification_cleanup_exemption_rejects_broader_deletions(self):
        commands = (
            "rm -rf /tmp/moor-verify-example.py",
            "rm -f /tmp/moor-verify-example.py /tmp/other.py",
            "rm -f /tmp/nested/../moor-verify-example.py",
            "rm -f /tmp/a/../../tmp/moor-verify-example.py",
            "rm -f /var/tmp/moor-verify-example.py",
            "rm -f /tmp/moor-verify-*",
            "rm -f /tmp/moor-verify-$(touch>/tmp/pwned).py",
            "rm -f /tmp/moor-ad-hoc-`touch>/tmp/pwned`.py",
            "rm -f /tmp/moor-verify-example.py; touch /tmp/pwned",
        )
        with mock_patch("tempfile.gettempdir", return_value="/tmp"):
...
class TestWindowsShellDestructiveCommands:
    def test_windows_destructive_requires_approval(self):
        cases = [
            (r"cmd /c del /f /q C:\tmp\moor-victim\file.txt", "Windows cmd destructive delete"),
            (r"cmd.exe /k rmdir /s /q C:\tmp\moor-victim", "Windows cmd destructive delete"),
            ...
            (r"powershell Remove-Item -Recurse -Force C:\tmp\moor-victim",
...
class TestSessionKeyContext:
    def test_context_session_key_overrides_process_env(self):
        ...
        try:
            with mock_patch.dict("os.environ", {"MOOR_SESSION_KEY": "bob"}, clear=False):
...
class TestMoorConfigWriteProtection:
    """Terminal-side pairing for the file_tools write_file/patch deny on
    ~/.moor/config.yaml (#14639). config.yaml IS the security policy
    ...
    def test_write_idioms_against_config(self):
        for command in (
            "echo 'approvals:' > ~/.moor/config.yaml",
            "echo '  mode: off' >> ~/.moor/config.yaml",
            "echo x | tee ~/.moor/config.yaml",
            "echo x | tee $MOOR_HOME/config.yaml",
            "cp /tmp/evil.yaml ~/.moor/config.yaml",
        ):
            dangerous, key, desc = detect_dangerous_command(command)
            assert dangerous is True, command
            assert key is not None, command
...
class TestSensitiveCopyMovePattern:
    ...
    shell rc files, or ~/.moor/config.yaml/.env must require approval — the
...
class TestWindowsAbsolutePathFolding:
    ...
    ~/.moor/ in dangerous-command detection.
    ...
    The OS-specific ``Path.home()`` / ``get_moor_home()`` tests above only
    exercise this branch on a Windows host; these monkeypatch a Windows-style
    HOME/MOOR_HOME so the fold is verified on the POSIX CI runner too."""
...
class TestSmartDeniedPrompt:
    ...
    def test_smart_deny_uses_locale_specific_once_deny_choices(self, monkeypatch, capsys):
        monkeypatch.setenv("MOOR_LANGUAGE", "tr")
...
class TestGatewayProtection:
    ...
    def test_gateway_run_backgrounded_detected(self):
        cmd = "kill 1605 && cd ~/.moor/moor-agent && source venv/bin/activate && python -m moor_cli.main gateway run --replace &disown; echo done"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "systemctl" in desc
        for variant in (
            "python -m moor_cli.main gateway run --replace &",
            "nohup python -m moor_cli.main gateway run --replace",
        ):
    ...
    def test_systemctl_restart_flagged(self):
        ...
        cmd = "systemctl --user restart moor-gateway"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
...
class TestWebhookApprovalExclusion:
    ...
    def test_webhook_platform_returns_false(self, monkeypatch):
        ...
        monkeypatch.setenv("MOOR_SESSION_PLATFORM", "webhook")
    ...
    def test_webhook_dangerous_command_denies_by_default(self, monkeypatch):
        ...
        monkeypatch.setenv("MOOR_SESSION_PLATFORM", "webhook")
        monkeypatch.setenv("MOOR_SESSION_KEY", "test-webhook-session")
    ...
    def test_api_server_dangerous_command_denies_by_default(self, monkeypatch):
        ...
        monkeypatch.delenv("MOOR_GATEWAY_SESSION", raising=False)
        monkeypatch.delenv("MOOR_INTERACTIVE", raising=False)
        monkeypatch.setenv("MOOR_SESSION_PLATFORM", "api_server")
        monkeypatch.setenv("MOOR_SESSION_KEY", "test-api-session")
...
class TestPgrepKillExpansion:
    """kill -9 $(pgrep moor) bypasses the pkill/killall name-matching
    ...
    def test_kill_pidof_expansion_detected(self):
        """`kill $(pidof moor)` is the BSD/Linux equivalent of the
        pgrep expansion and bypasses the pkill/killall name pattern
        in the same way. See issue #33071."""
        dangerous, _, desc = detect_dangerous_command("kill -TERM $(pidof moor_cli.main)")
        assert dangerous is True
        assert "pidof" in desc.lower() or "pgrep" in desc.lower()
        assert detect_dangerous_command("kill -9 `pidof moor`")[0] is True
...
class TestLaunchctlGatewayLifecycle:
    ...
    label achieves the same effect as `moor gateway stop|restart` and
    must require the same approval. See issue #33071.
    """

    def test_launchctl_against_moor_label_detected(self):
        for cmd in (
            "launchctl stop ai.moor.gateway",
            "launchctl kickstart -k system/ai.moor.gateway",
            "launchctl bootout system/ai.moor.gateway",
            "launchctl unload ~/Library/LaunchAgents/ai.moor.gateway.plist",
        ):
            dangerous, _, desc = detect_dangerous_command(cmd)
            assert dangerous is True, cmd
    ...
    def test_spliced_detection_does_not_flag_prose_or_other_services(self):
        ...
        a moor-gateway identifier, so quoted prose and non-gateway moor
    ...
    def test_label_built_before_verb_detected(self):
        ...
        "moor"/"ai.moor" to appear AFTER the verb and missed this
...
class TestApprovalTimeoutIsNotConsent:
    ...
    def setup_method(self):
        ...
        self._saved_env = {
            k: os.environ.get(k)
            for k in ("MOOR_GATEWAY_SESSION", "MOOR_CRON_SESSION",
                      "MOOR_YOLO_MODE",
                      "MOOR_SESSION_KEY", "MOOR_INTERACTIVE")
        }
        os.environ.pop("MOOR_YOLO_MODE", None)
        os.environ.pop("MOOR_INTERACTIVE", None)
        # MOOR_CRON_SESSION takes priority over MOOR_GATEWAY_SESSION in
        ...
        os.environ.pop("MOOR_CRON_SESSION", None)
        os.environ["MOOR_GATEWAY_SESSION"] = "1"
        os.environ["MOOR_SESSION_KEY"] = self.SESSION_KEY
    ...
    * ``tirith_fail_open: false`` → surface a Tirith-style warning through
    ...
    def test_import_error_allows_when_fail_open_or_disabled(self, enabled, fail_open):
        ...
        with _patch("builtins.__import__", side_effect=self._make_failing_import(real_import)):
            with _patch("moor_cli.config.load_config_readonly", return_value=cfg):
                with _patch("tools.approval.detect_dangerous_command", return_value=(False, None, None)):
                    with mock_patch.dict("os.environ", {"MOOR_INTERACTIVE": "1"}, clear=False):
...
GATEWAY_LIFECYCLE_LAUNCHCTL = (
    ...
    "launchctl unload ~/Library/LaunchAgents/ai.moor.gateway.plist",
    "launchctl load ~/Library/LaunchAgents/ai.moor.gateway.plist",
    "launchctl stop ai.moor.gateway",
    "launchctl restart ai.moor.gateway",
    "launchctl bootout gui/501/ai.moor.gateway",
    "launchctl remove ai.moor.gateway",
    "launchctl disable gui/501/ai.moor.gateway",
)
...
class TestLifecycleGuardLaunchctlParity:
    ...
    under ``_MOOR_GATEWAY == "1"``, the ``cron.lifecycle_guard`` block is
    ...
    def test_unrelated_labels_are_not_blocked(self):
        ...
        services, including other Moor ones, stay runnable."""
        from cron.lifecycle_guard import contains_gateway_lifecycle_command

        for cmd in (
            "launchctl bootout gui/501/com.example.unrelated",
            "launchctl remove ai.moor.update-checker",
