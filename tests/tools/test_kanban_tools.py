...
Verifies:
  - Tools are gated on MOOR_KANBAN_TASK: a normal chat session sees
...
def test_kanban_tools_hidden_without_env_var(monkeypatch, tmp_path):
    """Normal `moor chat` sessions (no MOOR_KANBAN_TASK) must have
    ...
    schema = registry.get_definitions(set(resolve_toolset("moor-cli")), quiet=True)
...
def worker_env(monkeypatch, tmp_path):
    """Simulate being a worker: MOOR_HOME isolated, MOOR_KANBAN_TASK set
...
def _make_goal_mode_worker_env(monkeypatch, tmp_path):
    """Set up an isolated MOOR_HOME with one claimed goal_mode task,
...
def test_comment_happy_path(worker_env):
    ...
    try:
        ...
        # Author defaults to MOOR_PROFILE env we set in the fixture
...
def test_comment_ignores_caller_supplied_author(worker_env):
    ...
    derived from ``MOOR_PROFILE`` so a worker can't forge a comment
    under an authoritative-looking name like ``moor-system`` and
    ...
    try:
        comments = kb.list_comments(conn, worker_env)
        # Author comes from MOOR_PROFILE in the fixture, not the
        # caller-supplied "moor-system" override.
        assert comments[0].author == "test-worker"
    finally:
        conn.close()
...
def test_unblock_with_pending_parents_returns_todo(monkeypatch, tmp_path):
    ...
    home.mkdir()
    monkeypatch.setenv("MOOR_HOME", str(home))
    monkeypatch.setenv("MOOR_PROFILE", "orchestrator")
    from pathlib import Path as _Path
    monkeypatch.setattr(_Path, "home", lambda: tmp_path)
...
# ---------------------------------------------------------------------------
#
# A worker process has MOOR_KANBAN_TASK set to its own task id. The
...
# are unrestricted.
#
# Orchestrator profiles (no MOOR_KANBAN_TASK in env) are intentionally
...
def test_worker_can_comment_on_foreign_task(worker_env):
    ...
    # MOOR_PROFILE — never to a caller-controlled string.
    conn = kbc.connect()
    try:
...
def test_orchestrator_complete_any_task_allowed(monkeypatch, tmp_path):
    """Orchestrator profiles (no MOOR_KANBAN_TASK) can still complete
...
# ---------------------------------------------------------------------------
#
# The dispatcher pins the active board via MOOR_KANBAN_BOARD env var,
...
# restarting Moor. These tests pin that ``board=<slug>`` argument
...
def multi_board_env(monkeypatch, tmp_path):
    """Isolated Moor home with two distinct kanban boards seeded.

    Returns ``("default", "alt")`` slugs. The default board has one
    pre-existing task ``seed_default``; ``alt`` has ``seed_alt``. No
    MOOR_KANBAN_TASK is pinned (orchestrator context) — workers test
    the env-task case via the existing ``worker_env`` fixture.
    """
    home = tmp_path / ".moor"
    home.mkdir()
    monkeypatch.setenv("MOOR_HOME", str(home))
    # Make sure neither MOOR_KANBAN_DB nor MOOR_KANBAN_BOARD pin a
    ...
    monkeypatch.delenv("MOOR_KANBAN_TASK", raising=False)
    monkeypatch.setenv("MOOR_PROFILE", "test-orchestrator")
    from pathlib import Path as _Path
    monkeypatch.setattr(_Path, "home", lambda: tmp_path)
...
# - Gateway sessions: MOOR_SESSION_PLATFORM + MOOR_SESSION_CHAT_ID set.
# - TUI sessions: MOOR_SESSION_KEY (or MOOR_SESSION_ID) set, with
...
def test_create_subscribes_gateway_session(monkeypatch, worker_env):
    ...
    monkeypatch.setenv("MOOR_SESSION_THREAD_ID", "thread-7")
    monkeypatch.setenv("MOOR_SESSION_USER_ID", "user-9")
    monkeypatch.setenv("MOOR_SESSION_USER_ID_ALT", "alt-user-9")
    monkeypatch.setenv("MOOR_SESSION_CHAT_TYPE", "forum")
...
def test_create_subscribes_tui_session_via_session_key(monkeypatch, worker_env):
    ...
    local channel), but the parent process exports MOOR_SESSION_KEY.
    ...
    monkeypatch.delenv("MOOR_SESSION_THREAD_ID", raising=False)
    monkeypatch.delenv("MOOR_SESSION_USER_ID", raising=False)
    monkeypatch.setenv("MOOR_SESSION_KEY", "tui-session-abc")
    monkeypatch.delenv("MOOR_SESSION_ID", raising=False)
...
def test_create_respects_auto_subscribe_on_create_false(monkeypatch, worker_env, tmp_path):
    ...
    # worker_env already created <tmp>/.moor; use a fresh sibling
    # home to avoid mkdir() colliding with the worker's directory.
    home = tmp_path / "gate-home" / ".moor"
    home.mkdir(parents=True)
    (home / "config.yaml").write_text(
        "kanban:\n  auto_subscribe_on_create: false\n"
    )
    monkeypatch.setenv("MOOR_HOME", str(home))
    monkeypatch.setenv("MOOR_SESSION_PLATFORM", "discord")
    monkeypatch.setenv("MOOR_SESSION_CHAT_ID", "channel-1")
...
def allow_private_urls(monkeypatch):
    ...
    Mirrors a user setting MOOR_ALLOW_PRIVATE_URLS on a private network.
...
def default_url_guard(monkeypatch):
    ...
    Clears MOOR_ALLOW_PRIVATE_URLS and resets url_safety's process-lifetime
