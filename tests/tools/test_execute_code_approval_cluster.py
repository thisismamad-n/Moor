...
Covers the canonical fix for issues #4146, #27303, #30882, #33057:
  ...
  4. tools.code_execution_tool._scrub_child_env — broad MOOR_ prefix dropped,
...
def gw_session(monkeypatch):
    """A clean gateway session: MOOR_GATEWAY_SESSION set, a bound session
...
def test_env_scrub_moor_allowlist_and_secret_blocks():
    ...
    env = {
        ...
        "MOOR_KANBAN_DB": "postgres://u:p@h/db",
    ...
    for kept in (
        ...
        "MOOR_DELEGATED_CHILD_CONTEXT", "PATH",
    ):
        assert kept in out, f"{kept} should be kept"
    for dropped in (
        "MOOR_BASE_URL", "MOOR_INTERACTIVE", "MOOR_KANBAN_DB",
...
def test_env_scrub_no_log_when_nothing_dropped(caplog):
    """No diagnostic noise when there are no dropped MOOR_* vars."""
    import logging
