"""Cross-session MOOR_SESSION_ID leak via the shared bash snapshot.
...
``MOOR_SESSION_ID`` into the snapshot, so every LATER session ``source``d that
stale value and its ``echo $MOOR_SESSION_ID`` reported a FOREIGN session's id
— overriding the correct per-command Popen env injected by
``_inject_session_context_env``.

The fix strips the per-session bridged vars (MOOR_SESSION_* / UI /
...
def test_export_snippet_shape():
    snippet = _export_dump_excluding_session_vars('"$__moor_snap_tmp"')
    ...
    assert snippet.rstrip().endswith('> "$__moor_snap_tmp"')
    ...
    try:
        def run_as(sid):
            ...
            def worker():
                ...
                out["r"] = env.execute('echo "[$MOOR_SESSION_ID]"')
        ...
        if os.path.exists(snap):
            with open(snap) as f:
                assert "MOOR_SESSION_ID" not in f.read()
    finally:
        env.cleanup()
