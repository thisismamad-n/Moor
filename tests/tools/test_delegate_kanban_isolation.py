...
# cwd. Without an explicit PYTHONPATH the child resolves ``moor_cli`` /
...
def _make_running_kanban_task(monkeypatch, tmp_path):
    ...
    monkeypatch.setenv("MOOR_PROFILE", "parent-worker")
    monkeypatch.setenv("MOOR_KANBAN_WORKSPACE", str(workspace))
    monkeypatch.setenv("MOOR_KANBAN_ATTACHMENTS_ROOT", str(attachments_root))
...
def test_delegated_child_context_suppresses_env_gated_kanban_tools(monkeypatch, tmp_path):
    ...
    The parent process may be a dispatcher worker with MOOR_KANBAN_TASK set;
    the child is only a subagent, not the run owner.
    """
    monkeypatch.setenv("MOOR_KANBAN_TASK", "t_parent")
    monkeypatch.setenv("MOOR_KANBAN_RUN_ID", "123")
    home = tmp_path / ".moor"
    home.mkdir()
    monkeypatch.setenv("MOOR_HOME", str(home))
...
def test_delegate_child_execute_code_env_bridges_contextvar_and_scrubs_kanban(
    ...
    ``os.environ`` and therefore never wrote MOOR_DELEGATED_CHILD_CONTEXT into
    the sandbox env.
    """
    home = tmp_path / ".moor"
    home.mkdir()
    ...
    monkeypatch.setenv("MOOR_KANBAN_WORKSPACE", str(tmp_path / "parent-workspace"))
    monkeypatch.setenv("MOOR_KANBAN_CLAIM_LOCK", "lock")
    ...
    with delegated_child_context():
        env = _scrub_child_env(
            dict(os.environ),
            is_passthrough=lambda k: k.startswith("MOOR_KANBAN_"),
            is_windows=False,
        )

    assert os.environ.get("MOOR_DELEGATED_CHILD_CONTEXT") is None
    assert env["MOOR_HOME"] == str(home)
    assert env["MOOR_DELEGATED_CHILD_CONTEXT"]  # fenced board root (path), not a bare flag
    ...
    assert env["MOOR_KANBAN_DB"] == str(home / "kanban.db")
    assert env["MOOR_KANBAN_WORKSPACE"] == str(tmp_path / "parent-workspace")
