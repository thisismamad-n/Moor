...
def fake_moor(tmp_path, monkeypatch):
    """Build a two-profile Moor layout and point MOOR_HOME at
    the moor-security profile (matching the original-incident shape).
    """
    ...
    monkeypatch.setenv("MOOR_HOME", str(sec_home))

    import moor_constants
    monkeypatch.setattr(moor_constants, "get_default_moor_root", lambda: root)

    import agent.file_safety as fs
    monkeypatch.setattr(fs, "_moor_home_path", lambda: sec_home)
...
class TestWriteFileCrossProfileGuard:
    def test_in_profile_write_allowed(self, fake_moor):
        from tools.file_tools import write_file_tool
        target = fake_moor["sec_home"] / "skills" / "new-skill" / "SKILL.md"
    ...
    def test_cross_profile_write_blocked_by_default(self, fake_moor):
        ...
        assert "moor-security" in result["error"]
        # File untouched.
        assert target.read_text() == original


    def test_non_moor_path_unaffected(self, fake_moor, tmp_path):
        from tools.file_tools import write_file_tool
...
class TestPatchCrossProfileGuard:
    def test_cross_profile_patch_blocked(self, fake_moor):
    ...
    def test_cross_profile_patch_bypass(self, fake_moor):
        from tools.file_tools import patch_tool
        target = fake_moor["root"] / "skills" / "shared-skill" / "SKILL.md"
        result_json = patch_tool(
            mode="replace",
    ...
    def test_v4a_patch_extracts_path_for_guard(self, fake_moor):
...
class TestSkillManageCrossProfileErrorUX:
    ...
    def test_error_names_other_profile_when_skill_lives_there(
        self, fake_moor, monkeypatch
    ):
        """The original incident shape — model expects 'foo' in active
        profile, but 'foo' lives in default. Error must point at default."""
        self._make_skill_in_profile(fake_moor["root"], "default-only-skill")

        # Re-import the module so SKILLS_DIR picks up MOOR_HOME (set in
        ...
        assert "not found in active profile 'moor-security'" in err
...
class TestSystemPromptActiveProfile:
    def test_default_profile_line_in_prompt(self, tmp_path, monkeypatch):
        ...
        about ~/.moor/profiles/<name>/."""
        # Don't set MOOR_HOME — falls back to default.
        import agent.file_safety as fs
        monkeypatch.setattr(fs, "_moor_home_path", lambda: tmp_path / "fake")
        monkeypatch.setattr(fs, "_moor_root_path", lambda: tmp_path / "fake")
    ...
    def test_named_profile_line_in_prompt_text(self, fake_moor):
        """When active profile is 'moor-security', the prompt warns
        ...
        assert "Active Moor profile" in src
        assert "cross_profile=True" in src
        assert "~/.moor/profiles/" in src
        # Both branches present (default and named profile).
        assert "Active Moor profile: default" in src
        assert "Active Moor profile: {active_profile}" in src
