"""Strict tool-provider selection: the `moor tools` choice always wins.

Policy (owner decision): the provider string stored in config.yaml is what
runs at call time. "moor" → managed Moor Tool Gateway only; a vendor name →
...
selection and pointing at `moor tools`.
...
class TestReadSelection:
    ...
    def test_moor_provider_returned(self):
        with self._with_raw({"image_gen": {"provider": "moor"}}):
            assert tbh.read_selection("image_gen") == "moor"

    def test_legacy_use_gateway_true_maps_to_moor(self):
        """Old configs stored use_gateway: true beside a vendor name — only
        the managed picker row ever wrote it, so it means 'moor'."""
        with self._with_raw({"video_gen": {"provider": "fal", "use_gateway": True}}):
            assert tbh.read_selection("video_gen") == "moor"
...
class TestImageFalStrictSelection:
    def test_moor_selection_routes_managed_even_with_fal_key(self):
        from tools import image_generation_tool as it

        with patch.object(it, "read_selection", return_value="moor"), \
    ...
    def test_moor_selection_unentitled_raises_selection_error(self):
        from tools import image_generation_tool as it
        ...
        assert "image_gen is configured to use moor" in str(exc.value)
        assert "moor tools" in str(exc.value)
...
class TestVideoFalStrictSelection:
    def test_moor_selection_routes_managed_even_with_fal_key(self):
        ...
        with patch("tools.tool_backend_helpers.read_selection", return_value="moor"), \
...
class TestSttStrictSelection:
    def test_moor_selection_beats_direct_openai_key(self):
        from tools import transcription_tools as tt

        with patch.object(tt, "_load_stt_config", return_value={"openai": {"api_key": "sk-direct"}}), \
             patch("tools.tool_backend_helpers.read_selection", return_value="moor"), \
             patch("tools.managed_tool_gateway.resolve_managed_tool_gateway", return_value=MANAGED):
...
class TestBrowserUseStrictSelection:
    ...
    def test_moor_selection_routes_managed_even_with_direct_key(self):
...
class TestWriteProviderConfig:
    def test_managed_row_writes_moor_and_clears_legacy_flag(self):
        from moor_cli.tools_config import _write_provider_config

        config = {"tts": {"provider": "edge", "use_gateway": False}}
        provider = {"name": "Moor Subscription", "tts_provider": "openai"}
        _write_provider_config(provider, config, managed_feature="tts")
        assert config["tts"]["provider"] == "moor"
        assert "use_gateway" not in config["tts"]

    def test_byok_row_writes_vendor_and_clears_legacy_flag(self):
        from moor_cli.tools_config import _write_provider_config

        config = {"web": {"backend": "moor", "use_gateway": True}}
    ...
    def test_managed_image_row_persists_moor_provider(self):
        from moor_cli.tools_config import _write_provider_config

        config = {}
        provider = {"name": "Moor Subscription", "imagegen_backend": "fal"}
        _write_provider_config(provider, config, managed_feature="image_gen")
        assert config["image_gen"]["provider"] == "moor"
    ...
    def test_plugin_injected_byok_row_clears_stale_use_gateway(self):
        ...
        config = {"stt": {"provider": "moor", "use_gateway": True}}
