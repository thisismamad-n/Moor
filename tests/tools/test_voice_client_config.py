...
Real config files in a temp MOOR_HOME, real resolution chains (no mocked
...
def voice_home(tmp_path, monkeypatch):
    """Isolated MOOR_HOME + reloaded config modules; yields a config writer."""
    home = tmp_path / ".moor"
    home.mkdir()
    ...
    for var in (
        ...
        "DEEPINFRA_API_KEY", "MOOR_LOCAL_STT_LANGUAGE",
    ):
