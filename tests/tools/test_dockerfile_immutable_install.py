"""Contract tests for the Docker image's immutable /opt/moor install tree."""
...
def test_dockerfile_makes_opt_moor_readonly_for_moor_user() -> None:
    ...
    assert "chown -R root:root /opt/moor" not in text
    assert "chmod -R a+rX /opt/moor" not in text
    assert "chmod -R a-w /opt/moor" not in text


def test_dockerfile_does_not_chown_install_trees_to_moor() -> None:
    text = _dockerfile_text()
    forbidden_patterns = (
        r"chown\s+-R\s+moor:moor\s+/opt/moor/\.venv",
        r"chown\s+-R\s+moor:moor\s+/opt/moor/ui-tui",
        r"chown\s+-R\s+moor:moor\s+/opt/moor/gateway",
        r"chown\s+-R\s+moor:moor\s+/opt/moor/node_modules",
    )
    for pattern in forbidden_patterns:
        assert not re.search(pattern, text), (
            "runtime install trees under /opt/moor must stay immutable; "
...
def test_dockerfile_bakes_code_scoped_install_method_stamp() -> None:
    ...
    (/opt/moor/.install_method) first; baking it at build time keeps the
    published image self-identifying as 'docker' WITHOUT writing into the
    shared $MOOR_HOME data volume (which a host install may also use).
    The stamp is created by root in the shim-wiring RUN block; the moor
    user can't modify it (go-w from the --chmod on the source COPY).
    """
    text = _dockerfile_text()
    assert "printf 'docker\\n' > /opt/moor/.install_method" in text

    # The stamp must be in the RUN block that wires the exec shim.
    shim_block = re.search(
        r"RUN mkdir -p /opt/moor/bin && \\\n"
        r"(?:.*\\\n)+?"
...
def test_dockerfile_redirects_lazy_installs_to_durable_target() -> None:
    ...
    # never under the immutable /opt/moor tree.
    assert f"ENV MOOR_LAZY_INSTALL_TARGET={target}" in text
    assert target.startswith("/opt/data/"), "target must be on the durable volume"
    assert "ENV MOOR_LAZY_INSTALL_TARGET=/opt/moor" not in text

    # The seal flag must still be present — the redirect rides on top of it,
    # it does not replace it.
    assert "ENV MOOR_DISABLE_LAZY_INSTALLS=1" in text

    # stage2-hook must seed + chown the target dir so first-use installs
    # succeed as the unprivileged moor runtime user.
    stage2 = (REPO_ROOT / "docker" / "stage2-hook.sh").read_text()
    assert '"$MOOR_HOME/lazy-packages"' in stage2, (
...
def test_dockerfile_bakes_photon_sidecar_deps() -> None:
    ...
    assert not re.search(
        r"chown\s+-R\s+moor:moor\s+/opt/moor/plugins", text
    )
