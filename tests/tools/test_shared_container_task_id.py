...
tests -- see `moor-agent-dev` skill, "Why do subagents get their own
...
def test_distinct_session_keys_get_distinct_slots(monkeypatch):
    monkeypatch.setenv("MOOR_SESSION_KEY", "sess-A")
    a = terminal_tool._resolve_container_task_id(None)
    monkeypatch.setenv("MOOR_SESSION_KEY", "sess-B")
...
#
# The tests above set MOOR_SESSION_KEY through os.environ, which only
...
# MOOR_SESSION_KEY absent from os.environ.


def test_session_key_from_contextvar_without_environ(monkeypatch):
    # Prove the fix works on the gateway path: MOOR_SESSION_KEY is NOT in
...
def test_contextvar_session_key_wins_over_environ(monkeypatch):
    ...
    monkeypatch.setenv("MOOR_SESSION_KEY", "sess-ENV")
    tokens = set_session_vars(session_key="sess-CTX")
    try:
        assert (
            terminal_tool._resolve_container_task_id(None) == "session:sess-CTX"
...
# long-lived container per Moor profile, shared by every session of that
