...
def _restore_tool_modules():
    original_moor_home = os.environ.get("MOOR_HOME")
    original_modules = {
        name: module
    ...
    try:
        ...
        if original_moor_home is None:
            os.environ.pop("MOOR_HOME", None)
        else:
            os.environ["MOOR_HOME"] = original_moor_home
...
def _install_modal_test_modules(
    ...
    *,
    ...
    moor_cli.__path__ = [str(REPO_ROOT / "moor_cli")]  # type: ignore[attr-defined]
    ...
    async def _lookup_aio(_name: str, create_if_missing: bool = False):
        return types.SimpleNamespace(name="moor-agent", create_if_missing=create_if_missing)
    ...
    return {
        "snapshot_store": moor_home / "modal_snapshots.json",
