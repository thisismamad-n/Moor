...
class TestBlocksMutationsInSourceRepo:
    ...
    def test_relative_cd_into_repo(self, repo):
        hit, _ = _detect("cd moor-agent && git pull", repo.parent, repo)
        assert hit is True
...
class TestBlockMessageGuidance:
    ...
    def test_scratch_hint_honors_moor_home(self, repo, monkeypatch):
        monkeypatch.setenv("MOOR_HOME", "/custom/moor-home")
        hit, msg = _detect("git rebase origin/main", repo, repo)
        assert hit is True
        assert "/custom/moor-home/scratch" in msg
