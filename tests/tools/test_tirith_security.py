...
class TestInstallArchiveMemberValidation:
    ...
    def test_install_extracts_regular_tirith_member(self, mock_target, mock_which,
        ...
        assert reason == ""
        assert path == str(moor_home / "bin" / "tirith")
    ...
    def test_install_rejects_non_regular_tirith_member(self, mock_target, mock_which,
        ...
        assert not os.path.lexists(moor_home / "bin" / "tirith")
    ...
    def test_ensure_installed_non_blocking(self):
        ...
        with patch("tools.tirith_security._load_security_config",
                   return_value={"tirith_enabled": True, "tirith_path": "tirith",
                                 "tirith_timeout": 5, "tirith_fail_open": True}), \
             patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._moor_bin_dir", return_value="/nonexistent"), \
             patch("tools.tirith_security._is_install_failed_on_disk", return_value=False), \
...
class TestMoorHomeIsolation:
    def test_moor_bin_dir_respects_moor_home(self):
        """_moor_bin_dir must use MOOR_HOME, not hardcoded ~/.moor."""
        from tools.tirith_security import _moor_bin_dir
        import tempfile
        tmpdir = tempfile.mkdtemp()
        with patch.dict(os.environ, {"MOOR_HOME": tmpdir}):
            result = _moor_bin_dir()
