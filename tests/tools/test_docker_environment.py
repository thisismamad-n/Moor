...
def test_non_persistent_cleanup_removes_container(monkeypatch):
    ...
    docker rm so containers don't leak across moor processes.
...
def _make_execute_only_env(forward_env=None):
    ...
    env._snapshot_path = "/tmp/moor-snap-test123.sh"
    env._cwd_file = "/tmp/moor-cwd-test123.txt"
    env._cwd_marker = "__MOOR_CWD_test123__"
...
def test_init_env_args_uses_moor_dotenv_for_allowlisted_env(monkeypatch):
    """_build_init_env_args picks up forwarded env vars from .env file at init time."""
    # Use a var that is NOT in _MOOR_PROVIDER_ENV_BLOCKLIST (GITHUB_TOKEN
    # is in the copilot provider's api_key_env_vars and gets stripped).
    env = _make_execute_only_env(["DATABASE_URL"])

    monkeypatch.delenv("DATABASE_URL", raising=False)
    monkeypatch.setattr(docker_env, "_load_moor_env_vars", lambda: {"DATABASE_URL": "value_from_dotenv"})
...
def test_init_env_args_prefers_shell_env_over_moor_dotenv(monkeypatch):
...
def test_init_env_args_uses_moor_dotenv_for_empty_shell_env(monkeypatch):
    ...
    MY_SECRET=`, clobbering the correct value sitting in ~/.moor/.env.
    """
    env = _make_execute_only_env(["MY_SECRET"])

    monkeypatch.setenv("MY_SECRET", "")
    monkeypatch.setattr(docker_env, "_load_moor_env_vars", lambda: {"MY_SECRET": "value_from_dotenv"})
...
def test_init_env_args_uses_active_profile_for_forwarded_env(monkeypatch):
    ...
    monkeypatch.setattr(docker_env, "_load_moor_env_vars", lambda: {})
...
def test_egress_node_options_overrides_conflicting_ca_flag(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda: ([], {"_MOOR_EGRESS_NODE_OPTIONS_APPEND": "--use-openssl-ca"}, []),
    )
    calls = _mock_subprocess_run(monkeypatch)
...
def test_snap_compat_drops_only_init_and_no_new_privileges(monkeypatch):
    ...
    def strip(argv):  # everything except the two flags and the random container name
        return [a for a in argv if a not in ("--init", "--security-opt", "no-new-privileges") and not a.startswith("moor-")]
...
def test_run_command_tags_moor_agent_label(monkeypatch):
    """Every container moor-agent starts must carry the moor-agent=1 label
    so the orphan reaper (and external operators) can identify them with a
    single ``docker ps --filter label=moor-agent=1`` call. Regression test
    ...
    assert "moor-agent=1" in labels, (
        f"moor-agent=1 label missing; got labels: {sorted(labels)}"
    )
...
def test_run_command_sanitizes_unsafe_task_id(monkeypatch):
    ...
    assert "moor-task-id=task_with_weird_chars" in labels, (
...
def test_shared_container_key_replaces_profile_identity(monkeypatch):
    ...
    assert a._labels["moor-profile"] == b._labels["moor-profile"]
    assert a._labels["moor-profile"] != "research"
    assert a._labels["moor-profile"].startswith("team_workspace-")
...
def test_distinct_shared_keys_never_collide(monkeypatch):
    ...
    assert c._labels["moor-profile"] != d._labels["moor-profile"]
    # Both stay within Docker's 63-char label-value bound.
    assert len(c._labels["moor-profile"]) <= 63
    assert len(d._labels["moor-profile"]) <= 63
...
def test_empty_shared_container_key_preserves_profile_isolation(monkeypatch):
    ...
    assert env._labels["moor-profile"] == "research"
...
def test_egress_enabled_does_not_reuse_pre_egress_container(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda: (
            ["-v", "/tmp/ca:/etc/ssl/certs/moor-egress-ca.crt:ro"],
    ...
    def _run(cmd, **kwargs):
        ...
        if isinstance(cmd, list) and len(cmd) >= 2:
            ...
            if sub == "ps":
                ...
                assert any(str(part).startswith("label=moor-egress=") for part in cmd)
                return subprocess.CompletedProcess(cmd, 0, stdout="", stderr="")
            if sub == "run":
                return subprocess.CompletedProcess(cmd, 0, stdout="fresh-cid\n", stderr="")
        return subprocess.CompletedProcess(cmd, 0, stdout="", stderr="")
        ...
        if isinstance(cmd, list) and len(cmd) >= 2:
            ...
            if sub == "ps":
                if "--format" in cmd:
                    ...
                    assert any(
                        str(part) == "label=moor-egress=off" for part in cmd
...
def test_failed_docker_run_cleans_up_orphaned_container(monkeypatch):
    ...
    assert rm_cmd[3].startswith("moor-"), "should remove the container by its generated name"
...
def test_credential_mount_skipped_when_source_is_directory(monkeypatch, tmp_path, caplog):
    ...
    fake_mounts = [
        {"host_path": str(corrupted_dir), "container_path": "/root/.moor/google_token.json"},
    ]
    monkeypatch.setattr(
...
def test_credential_mount_skipped_when_source_missing(monkeypatch, tmp_path, caplog):
    ...
    fake_mounts = [
        {"host_path": str(missing_path), "container_path": "/root/.moor/deleted_token.json"},
    ]
    monkeypatch.setattr(
        "tools.credential_files.get_credential_file_mounts",
        lambda: fake_mounts,
...
def test_s6_image_skips_docker_init_and_mounts_run_exec(monkeypatch):
    ...
    _make_dummy_env(image="moor-agent:latest")
