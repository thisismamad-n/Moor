...
@pytest.fixture(autouse=True)
def _enable_managed_moor_tools(monkeypatch):
    """Patch the source modules so managed_moor_tools_enabled() returns True
    even after tool modules are dynamically reloaded."""
    monkeypatch.setattr(
        "moor_cli.moor_account.get_moor_portal_account_info",
        lambda: MoorPortalAccountInfo(
            logged_in=True,
            source="jwt",
            fresh=False,
...
def test_managed_fal_submit_uses_gateway_origin_and_moor_token(monkeypatch):
    captured = {}
    ...
    assert captured["client_key"] == "moor-token"
...
def test_openai_tts_uses_managed_audio_gateway_when_direct_key_absent(monkeypatch, tmp_path):
    ...
    assert captured["api_key"] == "moor-token"
...
def test_transcription_uses_model_specific_response_formats(monkeypatch, tmp_path):
    ...
    # The managed audio route is the stored "moor" selection (strict model);
    ...
    transcription_tools._load_stt_config = lambda: {"provider": "moor"}
