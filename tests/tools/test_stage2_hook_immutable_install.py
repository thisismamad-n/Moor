...
Hosted/container Moor keeps user-writable state under MOOR_HOME
(/opt/data). The installed source, venv, TUI bundle, and node_modules under
/opt/moor must remain root-owned/non-writable by the runtime moor user so
...
def test_stage2_does_not_chown_install_tree_to_moor(stage2_text: str) -> None:
    ...
    for install_tree in (
        ...
        assert install_tree not in stage2_text, (
            f"stage2 must not chown {install_tree} back to moor; "
            "the Dockerfile keeps /opt/moor immutable and writable state "
            "belongs under MOOR_HOME"
        )
...
def test_stage2_documents_immutable_install_contract(stage2_text: str) -> None:
    ...
    assert "MOOR_DISABLE_LAZY_INSTALLS=1" in stage2_text
    assert "/opt/moor" in stage2_text
