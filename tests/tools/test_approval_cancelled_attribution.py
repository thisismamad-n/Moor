...
def gateway_session(monkeypatch):
    ...
    for k in ("MOOR_CRON_SESSION", "MOOR_YOLO_MODE", "MOOR_INTERACTIVE"):
        monkeypatch.delenv(k, raising=False)
    monkeypatch.setenv("MOOR_GATEWAY_SESSION", "1")
    monkeypatch.setenv("MOOR_SESSION_KEY", SESSION_KEY)
    # ``--yolo`` is frozen at import from MOOR_YOLO_MODE; a host shell running yolo must not
...
def cli_session(monkeypatch):
    mod._session_approved.clear()
    for k in ("MOOR_CRON_SESSION", "MOOR_YOLO_MODE", "MOOR_GATEWAY_SESSION", "MOOR_EXEC_ASK"):
        monkeypatch.delenv(k, raising=False)
