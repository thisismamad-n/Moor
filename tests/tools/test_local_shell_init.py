...
class TestSnapshotEndToEnd:
    ...
    def test_exported_env_changes_persist_between_commands(self, tmp_path):
        ...
        try:
            first = env.execute(
                'export MOOR_STICKY_ENV_PROBE="sticky"; '
                'export PATH="/tmp/moor-session-bin:$PATH"; '
                'echo "first=$MOOR_STICKY_ENV_PROBE"'
            )
            second = env.execute(
                'echo "second=$MOOR_STICKY_ENV_PROBE"; echo "PATH=$PATH"'
            )
        finally:
            env.cleanup()
    ...
    def test_snapshot_picks_up_init_file_exports(self, tmp_path, monkeypatch):
        ...
        with patch(
            ...
            try:
                result = env.execute(
                    'echo "PROBE=$MOOR_SHELL_INIT_PROBE"; echo "PATH=$PATH"'
                )
            finally:
                env.cleanup()
