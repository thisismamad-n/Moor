...
``~/.moor/browser-profile/<browser>/`` and launches the user's real binary
on the copy with a devtools port (see moor_cli.browser_connect). These tests
...
class TestSnapshotRealProfile:
    ...
    def test_fresh_snapshot_copies_auth_and_skips_caches(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(bc, "get_moor_home", lambda: home)
...
class TestRealProfileCdpLaunch:
    ...
    def test_non_chromium_default_fails_closed(self):
        ...
        with patch.object(bt_cloud, "_use_real_profile", return_value=True), \
             patch("moor_cli.browser_connect.detect_default_chromium", return_value=None):
            cdp, err = bt_real_profile._real_profile_cdp()
        assert cdp is None
    ...
    def test_snapshot_failure_fails_closed(self):
        self._reset()
        with patch.object(bt_cloud, "_use_real_profile", return_value=True), \
             patch("moor_cli.browser_connect.detect_default_chromium", return_value="chrome"), \
             patch("moor_cli.browser_connect.snapshot_real_profile", return_value=(None, "boom")):
            cdp, err = bt_real_profile._real_profile_cdp()
        assert cdp is None
    ...
    def test_launch_returns_http_cdp(self, tmp_path):
        ...
        with patch.object(bt_cloud, "_use_real_profile", return_value=True), \
             patch("moor_cli.browser_connect.detect_default_chromium", return_value="chrome"), \
             patch("moor_cli.browser_connect.snapshot_real_profile", return_value=(str(tmp_path), None)), \
             patch("moor_cli.browser_connect.chromium_executable", return_value="/usr/bin/chrome"), \
    ...
    def test_reattaches_to_surviving_chrome_instead_of_overlaying_its_profile(self, tmp_path, live_browser_id):
        """The attach daemon of a crashed owner gets reaped, but its Chrome (moor-launched,
        ...
        with patch.object(bt_cloud, "_use_real_profile", return_value=True), \
             patch("moor_cli.browser_connect.detect_default_chromium", return_value="chrome"), \
             patch("moor_cli.browser_connect.real_profile_copy_dir", return_value=str(tmp_path)), \
             patch("moor_cli.browser_connect.snapshot_real_profile", return_value=(None, "boom")) as snapshot, \
...
class TestConsentConfigRead:
    ...
    def test_consent_read_from_config(self, tmp_path, monkeypatch):
        ...
        with patch("moor_cli.config.read_raw_config",
...
class TestChannelIdentity:
    ...
    def test_channel_sentinel_fails_closed_in_cdp(self):
        ...
        bt._real_profile_cdp_cache.clear()
        with patch.object(bt_cloud, "_use_real_profile", return_value=True), \
             patch("moor_cli.browser_connect.detect_default_chromium",
                   return_value=bc.UNSUPPORTED_CHANNEL), \
             patch("moor_cli.browser_connect.snapshot_real_profile") as snap:
            cdp, err = bt_real_profile._real_profile_cdp()
        assert cdp is None
...
class TestReviewBugFixes:
    ...
    def test_permissions_secured_on_refresh(self, tmp_path, monkeypatch):
        ...
        bc.snapshot_real_profile("chrome", src=str(src))  # fresh
        secured = []
        with patch("moor_cli.config._secure_dir", side_effect=secured.append):
    ...
    def test_lightpanda_engine_fails_actionably(self):
        ...
        with patch.object(bt_cloud, "_use_real_profile", return_value=True), \
             ...
             patch("moor_cli.browser_connect.detect_default_chromium") as det:
            cdp, err = bt_real_profile._real_profile_cdp()
        assert cdp is None
...
class TestReviewRound3:
    ...
    def test_snapshot_blocks_when_locked_even_with_autoclose(self, tmp_path, monkeypatch):
        ...
        message offers the close (mentions Moor can close it)."""
        import moor_cli.browser_connect as bc
        src = self._multi(tmp_path / "real")
        home = tmp_path / "hh"
        monkeypatch.setattr(bc, "get_moor_home", lambda: home)
    ...
    def test_consent_off_triggers_cleanup(self, tmp_path, monkeypatch):
        called = {"n": 0}
        with patch.object(bt_cloud, "_use_real_profile", return_value=False), \
             patch("moor_cli.browser_connect.cleanup_real_profile_snapshots",
    ...
    def test_relaunch_path_does_snapshot(self, tmp_path):
        ...
        with patch.object(bt_cloud, "_use_real_profile", return_value=True), \
             ...
             patch("moor_cli.browser_connect.detect_default_chromium", return_value="chrome"), \
             patch("moor_cli.browser_connect.real_profile_copy_dir", return_value=str(tmp_path)), \
             patch("moor_cli.browser_connect.snapshot_real_profile",
...
class TestWindowsLockedProfileCopy:
    ...
    def _locked_src(self, root):
        ...
        con.executemany("insert into cookies values(?,?)",
                        [("moor.ai", f"c{i}") for i in range(42)])
        con.commit()
