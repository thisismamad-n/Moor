...
from tools.environments.local_env_policy import (
    ...
    _MOOR_PROVIDER_ENV_FORCE_PREFIX,
)
...
class TestProviderEnvBlocklist:
    """Provider env vars loaded from ~/.moor/.env must not leak."""
    ...
    def test_general_aws_credential_chain_is_preserved(self):
        ...
        refuses to re-allow anything in _MOOR_PROVIDER_ENV_BLOCKLIST
    ...
    def test_tool_and_gateway_vars_are_stripped(self):
        ...
        leaked_vars = {
            ...
            "MOOR_DASHBOARD_SESSION_TOKEN": "dashboard-session-secret",
    ...
    def test_bare_moor_resolves_from_sanitized_subprocess_path(self):
        """Cron children can resolve Moor even when the gateway PATH cannot."""
        from tools.environments.local import _sanitize_subprocess_env

        with patch(
            "tools.environments.local._resolve_moor_bin_dir",
    ...
    def test_bare_moor_path_does_not_duplicate_existing_install_dir(self):
...
class TestTerminalFirstPartyPlatformEnv:
    ...
    (and the other BUZZ_* vars) are stripped by _MOOR_PROVIDER_ENV_BLOCKLIST
    ...
    children then get the BUZZ_* vars; execute_code, moor_subprocess_env,
    ...
    def test_buzz_vars_stay_in_blocklist(self):
        ...
        moor_subprocess_env, env_passthrough registration)."""
...
class TestTerminalFirstPartySnapshotIsolation:
    ...
    an exclusion, profile A's BUZZ_PRIVATE_KEY lands in moor-snap-<id>.sh
...
class TestForceEnvOptIn:
    """Callers can opt in to passing a blocked var via _MOOR_FORCE_ prefix."""

    def test_force_prefix_passes_blocked_var(self):
        """_MOOR_FORCE_OPENAI_API_KEY in self.env should inject OPENAI_API_KEY."""
        result_env = _run_with_env(self_env={
            f"{_MOOR_PROVIDER_ENV_FORCE_PREFIX}OPENAI_API_KEY": "sk-explicit",
        })
        ...
        assert f"{_MOOR_PROVIDER_ENV_FORCE_PREFIX}OPENAI_API_KEY" not in result_env
    ...
    def test_force_prefix_overrides_os_environ_block(self):
        ...
        result_env = _run_with_env(
            ...
            self_env={f"{_MOOR_PROVIDER_ENV_FORCE_PREFIX}OPENAI_BASE_URL": "http://intended/v1"},
        )
...
class TestActiveVenvMarkerStripping:
    ...
    into the Moor venv path instead of the project's own ``.venv`` —
    silently clobbering the Moor environment (and, when the other project
    pins a different Python, breaking the gateway outright). The Moor venv
    stays reachable via PATH, so stripping the markers is safe.
    """

    def test_virtualenv_marker_stripped_end_to_end(self):
        result_env = _run_with_env(extra_os_env={
            "VIRTUAL_ENV": "/home/user/.moor/moor-agent/venv",
        })
        assert "VIRTUAL_ENV" not in result_env

    def test_conda_prefix_marker_stripped_end_to_end(self):
        result_env = _run_with_env(extra_os_env={
            "CONDA_PREFIX": "/opt/conda/envs/moor",
        })
...
class TestPythonpathSelectiveStrip:
    ...
    ``_strip_moor_owned_pythonpath`` surgically removes only the
    ...
    def test_owned_entries_stripped_matrix(self):
        ...
        independently via parents[2] so an off-by-one in _moor_repo_root
        ...
        for input_entries, expected in cases:
            ...
            _strip_moor_owned_pythonpath(env)
            if expected is None:
    ...
    def test_non_owned_runtime_shaped_entries_preserved(self):
        ...
        off-by-one in _moor_repo_root cannot silently pass; no launcher
    ...
    def test_windows_backslash_paths(self):
        ...
        pyver = f"python{sys.version_info[0]}.{sys.version_info[1]}"
        moor_win = f"C:\\\\Users\\\\u\\\\.moor\\\\moor-agent\\\\venv\\\\lib\\\\{pyver}\\\\site-packages"
        user_win = "D:\\\\user\\\\lib"
        env = {
            "PYTHONPATH": ";".join([moor_win, user_win]),
        }
    ...
    def test_windows_moor_owned_paths_stripped(self):
        ...
        _strip_moor_owned_pythonpath(env)
        entries = env["PYTHONPATH"].split(";")
        assert moor_win not in entries
        assert user_win in entries
    ...
    def test_base_python_sanitizer_uses_validated_separate_runtime_venv(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(local, "_moor_repo_root_aliases", (repo_root,))
    ...
    def test_unrelated_virtual_env_is_not_runtime_provenance(self, tmp_path, monkeypatch):
        ...
        local_pythonpath._strip_moor_owned_pythonpath(env)
    ...
    def test_builders_strip_moor_venv_pythonpath(self, builder):
        ...
        with patch.dict(os.environ, seed, clear=True):
            if builder == "_make_run_env":
                ...
                result = local_mod.moor_subprocess_env()
    ...
    def test_scrub_child_env_strips_moor_venv_pythonpath(self):
        """execute_code's _scrub_child_env path: after scrubbing, Moor venv
        site-packages entries should be stripped when
        _strip_moor_owned_pythonpath is applied (as the spawn path does),
        ...
        _strip_moor_owned_pythonpath(scrubbed)
    ...
    def test_execute_code_composition_strips_inherited_moor_entries(self, same_env):
        ...
        * user entries survive after the controlled entries.
        ...
        moor_root = str(Path(cet.__file__).resolve().parents[1])
        venv_sp = str(_running_venv_site_packages())
        ...
        with patch("tools.code_execution_tool._load_config",
             ...
             patch("tools.code_execution_env._uses_moor_python_environment",
                   return_value=same_env), \
             patch("subprocess.Popen", side_effect=_fake_popen), \
             patch.dict(os.environ, {
                 "PYTHONPATH": os.pathsep.join(
                     [moor_root, venv_sp, user_a, user_b]),
             }):
        ...
        norm_root = os.path.normcase(moor_root)
    ...
    def test_configured_home_alias_matches_launcher_output(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("MOOR_HOME", str(configured_home))

        launcher_entry = Path(_preserve_moor_home_path(physical_root))
        aliases = local_pythonpath._build_moor_repo_root_aliases(
            physical_root.resolve(),
            physical_root,
            configured_home,
        )

        assert launcher_entry == configured_home / "moor-agent"
    ...
    def test_profile_rehome_keeps_junction_lexical_alias(self, tmp_path, monkeypatch):
        ...
        The desktop/CLI spawn children with MOOR_HOME and PYTHONPATH in the
        configured (junction) spelling, but --profile / sticky active_profile
        re-home MOOR_HOME through resolve_profile_env() before the
        ...
        monkeypatch.setenv("MOOR_HOME", str(configured_home))
        lexical_root = configured_home / "moor-agent"
        ...
        # The sanitizer now runs under the re-homed (profile) MOOR_HOME.
        aliases = local_pythonpath._build_moor_repo_root_aliases(
            physical_root.resolve(),
            physical_root,
    ...
    def test_repo_level_junction_recovers_lexical_alias(self, tmp_path, monkeypatch):
        ...
        (e.g. D:\\moor\\moor-agent -> C:\\...\\moor-agent) while the
        ...
        # repo-level link: <configured-home>/moor-agent -> physical repo
        try:
            _make_directory_link(configured_home / "moor-agent", physical_root)
        except OSError as exc:
            pytest.skip(f"directory link unavailable on this host: {exc}")
    ...
    def test_same_named_non_owned_directories_preserved(self, tmp_path, monkeypatch):
        ...
        physical_root = _physical_repo_root(tmp_path)
        configured_home = tmp_path / "configured-home"
        (configured_home / "moor-agent").mkdir(parents=True)
        unrelated = tmp_path / "user-tools" / "moor-agent"
        ...
        for lookalike in (configured_home / "moor-agent", unrelated):
    ...
    def test_profile_home_with_repo_level_junction(self, tmp_path, monkeypatch):
        ...
        is <root>/profiles/<name> while the repo is a link at <root>/moor-agent.
        ...
        try:
            _make_directory_link(configured_root / "moor-agent", physical_root)
        except OSError as exc:
            pytest.skip(f"directory link unavailable on this host: {exc}")

        configured_home = configured_root / "profiles" / "coder"
        lexical_root = configured_root / "moor-agent"
        aliases = local_pythonpath._build_moor_repo_root_aliases(
            physical_root.resolve(),
            physical_root,
            configured_home,
        ...
        assert any(local_pythonpath._same_path(a, lexical_root) for a in aliases)
        assert not any(local_pythonpath._same_path(a, configured_home / "moor-agent") for a in aliases)
    ...
    def test_validated_runtime_venv_lexical_after_repo_recovery(self, tmp_path, monkeypatch):
        ...
        local._moor_site_packages = None
...
class TestPythonhomeSanitized:
    ...
    def test_builders_strip_pythonhome(self, builder):
        ...
        with patch.dict(os.environ, seed, clear=True):
            if builder == "_make_run_env":
                ...
                result = local_mod._sanitize_subprocess_env(dict(os.environ))
            elif builder == "moor_subprocess_env":
                result = local_mod.moor_subprocess_env()
            else:
    ...
    def test_build_subprocess_env_no_scrub_preserves_pythonhome(self):
        ...
        base = {
            ...
            "HOME": "/home/user",
            "PYTHONHOME": "/opt/moor-venv",
            "VIRTUAL_ENV": "/opt/moor-venv",
            "SERVICE_TOKEN": "s3cr3t",
        }
        result = build_subprocess_env(base, scrub_secrets=False)
        assert result.get("PYTHONHOME") == "/opt/moor-venv"
        assert result.get("VIRTUAL_ENV") == "/opt/moor-venv"
...
class TestBlocklistCoverage:
    ...
    def test_issue_1002_offenders(self):
        ...
        assert must_block.issubset(_MOOR_PROVIDER_ENV_BLOCKLIST)
    ...
    def test_registry_vars_are_in_blocklist(self):
        ...
        by the user's Claude Code install, not Moor (#55878).
        """
        ...
        for pconfig in PROVIDER_REGISTRY.values():
            for var in pconfig.api_key_env_vars:
                if var in exempt:
                    continue
                assert var in _MOOR_PROVIDER_ENV_BLOCKLIST, (
                    f"Registry var {var} (provider={pconfig.id}) missing from blocklist"
                )
            if pconfig.base_url_env_var:
                assert pconfig.base_url_env_var in _MOOR_PROVIDER_ENV_BLOCKLIST, (
    ...
    def test_bedrock_bearer_token_is_in_blocklist(self):
        ...
        assert "AWS_BEARER_TOKEN_BEDROCK" in _MOOR_PROVIDER_ENV_BLOCKLIST
    ...
    def test_general_aws_chain_not_in_blocklist(self):
        ...
        leaked_block = general_chain & _MOOR_PROVIDER_ENV_BLOCKLIST
    ...
    def test_claude_code_oauth_token_is_inheritable(self):
        ...
        assert "CLAUDE_CODE_OAUTH_TOKEN" not in _MOOR_PROVIDER_ENV_BLOCKLIST
    ...
    def test_optional_tool_and_messaging_vars_are_in_blocklist(self):
        ...
        for name, metadata in OPTIONAL_ENV_VARS.items():
            ...
            if category in {"tool", "messaging"}:
                assert name in _MOOR_PROVIDER_ENV_BLOCKLIST, (
...
class TestSanePathIncludesHomebrew:
    ...
    def _disable_moor_bin_injection(self):
        """These tests assert the sane-path merge in isolation. Disable the
        moor-install-dir prepend (a separate concern, covered by
        TestMoorBinDirOnPath) so a real ``moor`` on the test runner's PATH
        doesn't shift the asserted PATH layout."""
        from tools.environments import local as local_mod
        saved = local_mod._MOOR_BIN_DIR
        local_mod._MOOR_BIN_DIR = None  # resolved -> no dir to inject
        yield
        local_mod._MOOR_BIN_DIR = saved
...
class TestMoorBinDirOnPath:
    """The moor install dir is reachable in the terminal subshell PATH.

    Plugins shelling out to bare ``moor`` via the terminal tool must work
    even when the gateway was launched without the moor install dir on
    PATH (systemd, service managers, cron). See the discussion that motivated
    _resolve_moor_bin_dir / _prepend_moor_bin_dir.
    """

    def _reset_cache(self):
        from tools.environments import local as local_mod
        local_mod._MOOR_BIN_DIR = local_mod._SENTINEL

    def test_resolves_via_which(self, monkeypatch):
        from tools.environments import local as local_mod
        self._reset_cache()
        monkeypatch.setattr(local_mod.shutil, "which",
                            lambda name: "/opt/moor/bin/moor" if name == "moor" else None)
        monkeypatch.setattr(local_mod.os.path, "isdir", lambda p: p == "/opt/moor/bin")
        assert local_mod._resolve_moor_bin_dir() == "/opt/moor/bin"


    def test_prepend_noop_when_unresolved(self, monkeypatch):
        from tools.environments import local as local_mod
        self._reset_cache()
        local_mod._MOOR_BIN_DIR = None
        assert local_mod._prepend_moor_bin_dir("/usr/bin:/bin") == "/usr/bin:/bin"

    def test_make_run_env_injects_moor_bin_dir(self):
        """A gateway env missing the moor dir gets it back in the subshell PATH.

        Platform-agnostic: ``_prepend_moor_bin_dir`` uses ``os.pathsep`` on
        every host, so no platform flag is faked here."""
        from tools.environments import local as local_mod
        from tools.environments.local import _make_run_env
        self._reset_cache()
        local_mod._MOOR_BIN_DIR = "/opt/moor/bin"
        with patch.dict(
            os.environ,
            {"PATH": os.pathsep.join(["/usr/bin", "/bin"])},
            clear=True,
        ...
        assert entries[0] == "/opt/moor/bin"
        assert "/usr/bin" in entries


class TestMoorInternalDynamicSecrets:
    """Dynamically-named Moor secrets injected at gateway/CLI startup must
    not leak into terminal subprocesses.

    The static ``_MOOR_PROVIDER_ENV_BLOCKLIST`` is name-based and derived
    ...
    ``_is_moor_internal_secret`` is the single source of truth; every spawn
    path (``_sanitize_subprocess_env``, ``_make_run_env``,
    ``moor_subprocess_env``, Docker forward filter, ``env_passthrough``)
    consults it. These tests exercise the terminal execute path + predicate.
    """

    def test_predicate_matches_auxiliary_api_key(self):
        from tools.environments.local_env_policy import _is_moor_internal_secret
        assert _is_moor_internal_secret("AUXILIARY_VISION_API_KEY")
        assert _is_moor_internal_secret("AUXILIARY_WEB_EXTRACT_API_KEY")
        assert _is_moor_internal_secret("AUXILIARY_APPROVAL_API_KEY")
        # plugin-registered task names are covered by the pattern
        assert _is_moor_internal_secret("AUXILIARY_MY_PLUGIN_TASK_API_KEY")

    def test_predicate_matches_auxiliary_base_url(self):
        from tools.environments.local_env_policy import _is_moor_internal_secret
        assert _is_moor_internal_secret("AUXILIARY_VISION_BASE_URL")
        assert _is_moor_internal_secret("AUXILIARY_COMPRESSION_BASE_URL")

    def test_predicate_matches_gateway_relay_auth(self):
        from tools.environments.local_env_policy import _is_moor_internal_secret
        assert _is_moor_internal_secret("GATEWAY_RELAY_SECRET")
        assert _is_moor_internal_secret("GATEWAY_RELAY_DELIVERY_KEY")
        assert _is_moor_internal_secret("GATEWAY_RELAY_SESSION_TOKEN")
    ...
    def test_predicate_allows_auxiliary_non_secrets(self):
        ...
        from tools.environments.local_env_policy import _is_moor_internal_secret
        assert not _is_moor_internal_secret("AUXILIARY_VISION_PROVIDER")
        assert not _is_moor_internal_secret("AUXILIARY_VISION_MODEL")
        assert not _is_moor_internal_secret("GATEWAY_RELAY_URL")
        assert not _is_moor_internal_secret("GATEWAY_RELAY_PLATFORMS")
        assert not _is_moor_internal_secret("GATEWAY_RELAY_ID")  # not a secret suffix
        # unrelated vars pass through
        assert not _is_moor_internal_secret("PATH")
        assert not _is_moor_internal_secret("MY_APP_KEY")
    ...
    def test_gateway_relay_static_names_in_blocklist(self):
        ...
        assert "GATEWAY_RELAY_SECRET" in _MOOR_PROVIDER_ENV_BLOCKLIST
        assert "GATEWAY_RELAY_DELIVERY_KEY" in _MOOR_PROVIDER_ENV_BLOCKLIST
        assert "GATEWAY_RELAY_ID" in _MOOR_PROVIDER_ENV_BLOCKLIST
