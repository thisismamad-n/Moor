...
class TestHonchoClientConfigDefaults:
    def test_default_values(self):
        ...
        assert config.host == "moor"
        assert config.workspace_id == "moor"
...
class TestFromGlobalConfig:
    ...
    def test_base_url_host_block_overrides_root_and_env(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            ...
            "hosts": {"moor": {"baseUrl": "http://host-block:9001"}},
        }))
    ...
    def test_base_url_full_precedence_chain(self, tmp_path):
        ...
        config_file = tmp_path / "config.json"
        layers = {
            "hosts": {"moor": {"baseUrl": "http://host:1"}},
...
class TestResolveSessionName:
    ...
    def test_per_repo_uses_git_root(self):
        ...
        with patch.object(
            HonchoClientConfig, "_git_repo_name", return_value="moor-agent"
        ):
            result = config.resolve_session_name("/home/user/moor-agent/subdir")
        assert result == "moor-agent"


class TestResolveConfigPath:
    def test_prefers_moor_home_when_exists(self, tmp_path):
        moor_home = tmp_path / "moor"
        moor_home.mkdir()
        local_cfg = moor_home / "honcho.json"
    ...
    def test_falls_back_to_default_profile_when_no_local(self, tmp_path, monkeypatch):
        # Profile mode: MOOR_HOME points at ~/.moor/profiles/<name>, so
        # _get_default_moor_home() must resolve back to ~/.moor — that's
        # the bug the HOME-anchored helper fixes (vs. blindly using Path.home()).
        fake_home = tmp_path / "fakehome"
        fake_home.mkdir()
        default_home = fake_home / ".moor"
        ...
        assert _get_default_moor_home() == default_home
        assert result == default_cfg


class TestResolveActiveHost:
    def test_profile_host_key_uses_honcho_safe_separator(self):
        assert profile_host_key("coder") == "moor_coder"
        assert profile_host_key("default") == "moor"


    def test_explicit_env_var_wins(self):
        with patch.dict(os.environ, {"MOOR_HONCHO_HOST": "moor.coder"}):
            assert resolve_active_host() == "moor.coder"
    ...
    def test_profiles_import_failure_falls_back(self):
        ...
        with patch.dict(os.environ, {}, clear=False), patch(
            ...
            os.environ.pop("MOOR_HONCHO_HOST", None)
            # Temporarily remove moor_cli.profiles to simulate import failure
            saved = sys.modules.get("moor_cli.profiles")
            sys.modules["moor_cli.profiles"] = None  # type: ignore
            try:
                assert resolve_active_host() == "moor"
            finally:
                if saved is not None:
                    sys.modules["moor_cli.profiles"] = saved
                else:
                    sys.modules.pop("moor_cli.profiles", None)


class TestProfileScopedConfig:
    def test_from_env_uses_profile_host(self):
        with patch.dict(os.environ, {"HONCHO_API_KEY": "key"}):
            config = HonchoClientConfig.from_env(host="moor_coder")
        assert config.host == "moor_coder"
        assert config.workspace_id == "moor"  # shared workspace
        assert config.ai_peer == "moor_coder"
...
class TestObservationModeMigration:
    ...
    def test_existing_config_defaults_to_unified(self, tmp_path):
        ...
        cfg_file.write_text(json.dumps({
            "apiKey": "k",
            "hosts": {"moor": {"enabled": True, "aiPeer": "moor"}},
        }))
...
class TestGetHonchoClient:
    ...
    def test_dot_form_legacy_host_key_keeps_local_api_key(self):
        """Regression for #37436: a legacy dot-form host block (moor.work)
        ...
        cfg = HonchoClientConfig(
            ...
            host="moor_work",
            workspace_id="moor",
            raw={"hosts": {"moor.work": {"apiKey": "explicit-local-key"}}},
        )
    ...
    def test_timeout_change_triggers_client_rebuild(self):
        ...
        cfg_yaml = get_moor_home() / "config.yaml"
    ...
    def test_managed_config_timeout_does_not_thrash_singleton(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("MOOR_MANAGED_DIR", str(managed_dir))
...
class TestGetHonchoClientBaseUrlDoublePrefixFix:
    ...
    def test_lan_default_host_empty_key_uses_local_placeholder(self, tmp_path):
        ...
        with patch.dict(os.environ, {}, clear=True), \
             patch("moor_cli.profiles.get_active_profile_name", return_value="default"), \
