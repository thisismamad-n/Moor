...
Under a gateway (Telegram, Discord, Slack, ...) Moor passes the
...
class TestCrossPlatformMemoryUnification:
    """The same physical user talking to Moor via Telegram AND Discord
...
class TestProfilePeerUniqueness:
    """Each Moor profile can pin to its own unique peerName.

    Profile cloning copies host blocks, but operators routinely diverge them
    afterwards (e.g. `moor -p partner` pinned to a different person's peer).
