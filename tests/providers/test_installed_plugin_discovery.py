"""A provider installed by ``moor plugins install`` must actually be found.

The installer clones into ``$MOOR_HOME/plugins/<name>/`` (flat), provider
...
def _clear_provider_caches():
    ...
    for mod in list(sys.modules):
        if mod.startswith(("plugins.model_providers", "_moor_user_provider")):
            del sys.modules[mod]
...
def test_flat_installed_model_provider_plugins_are_discovered_alongside_nested_ones(moor_home):
    _write_plugin(moor_home / "plugins" / "installed-acp", name="installed-acp",
                  manifest='name: installed-acp\nkind: "model-provider"\n')
    _write_plugin(moor_home / "plugins" / "model-providers" / "nested-acp", name="nested-acp",
...
def test_other_plugins_in_the_flat_directory_are_left_to_the_plugin_manager(moor_home):
    _write_plugin(moor_home / "plugins" / "other-standalone", name="other-standalone",
                  manifest="name: other-standalone\nkind: standalone\n")
    _write_plugin(moor_home / "plugins" / "manifestless", name="manifestless", manifest=None)
    _write_plugin(moor_home / "plugins" / "broken-manifest", name="broken-manifest",
