...
class TestOpenRouterProfile:
    ...
    def test_speed_tier_slugs_pin_endpoints_and_rewrite_wire_model(self):
        """Moor-style ``-fast``/``-flex`` slugs are OpenRouter ENDPOINTS of the base model: the wire
...
class TestMoorProfile:
    ...
    def test_extra_body_ignores_provider_preferences(self):
        """Moor Portal rejects caller-supplied provider routing prefs (HTTP 400)."""
        p = get_provider_profile("moor")
        body = p.build_extra_body(
...
class TestAlibabaRegionalAndTokenPlanProfiles:
    ...
    def test_cn_variants_resolve_in_auth_registry(self, monkeypatch):
        ...
        moor_cli/auth.py:461-490) and raised
        "Unknown provider 'alibaba-coding-plan-cn'" (moor_cli/auth.py:1937)
