...
def test_compat_resolution_warns_once_per_name_with_the_new_location():
    from moor_cli.plugin_compat import MoorPluginCompatWarning, _seen
    ...
    with warnings.catch_warnings(record=True) as rec:
        ...
        getattr(mod, name)
        getattr(mod, name)
    ours = [w for w in rec if issubclass(w.category, MoorPluginCompatWarning)]
...
def test_importing_the_facade_itself_does_not_warn():
    ...
    from moor_cli.plugin_compat import MoorPluginCompatWarning
    import sys
