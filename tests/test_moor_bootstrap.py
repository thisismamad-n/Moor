"""Tests for moor_bootstrap — Windows UTF-8 stdio shim.

The bootstrap module is imported at the top of every Moor entry point
(moor, moor-agent, moor-acp, gateway, batch_runner, cli.py).  It
...
Key invariants covered by these tests:
  ...
  5. Load order: every Moor entry point imports moor_bootstrap as its
...
def _fresh_import():
    """Return a freshly-imported moor_bootstrap module.

    Drops any cached copy from sys.modules first so module-level code
    runs again and the platform check re-evaluates.
    """
    sys.modules.pop("moor_bootstrap", None)
    import moor_bootstrap  # noqa: WPS433
    return moor_bootstrap
...
class TestWindowsBehavior:
    ...
    def test_stdout_reconfigured_to_utf8_on_windows(self):
        # The live process's stdout should now be UTF-8 (the Moor CLI
...
class TestEntryPointsImportBootstrap:
    """Every Moor entry point must import moor_bootstrap as its
    ...
    # Entry points that invoke Moor as a process.  Each one must
    # import moor_bootstrap before doing any file I/O or stdout writes.
    ENTRY_POINTS = [
        "moor_cli/main.py",   # moor CLI (console_script)
        "run_agent.py",          # moor-agent (console_script)
        "acp_adapter/entry.py",  # moor-acp (console_script)
    ...
    def test_entry_point_imports_bootstrap(self, path):
        """The file must contain 'import moor_bootstrap' and that
        ...
        half-finished ``moor update`` (git-reset landed new code but
        ``uv pip install -e .`` didn't finish re-registering
        ``moor_bootstrap`` as a top-level module) leaves moor
        ...
        """
        # Resolve relative to the moor-agent repo root.  Tests live
        # at tests/test_moor_bootstrap.py, so go up one dir.
        import pathlib
        ...
        for node in ast.iter_child_nodes(tree):
            ...
            # moor start even when moor_bootstrap hasn't been
        ...
        assert first_import_name == "moor_bootstrap", (
            f"{path}: first top-level import is {first_import_name!r}, "
            f"but it must be 'moor_bootstrap' so UTF-8 stdio is "
            f"configured before anything else initializes.  Move the "
            f"'import moor_bootstrap' line to be the first import."
        )


class TestHardenImportPath:
    """harden_import_path() must keep a same-named package in the launch
    directory from shadowing Moor's own top-level modules — covering both
    the relative ('' / '.') and absolute-path forms the cwd can take on
    sys.path (issue #51286)."""

    def _run(self, hb, path_seed, env=None):
        original = sys.path[:]
        original_env = os.environ.get("MOOR_PYTHON_SRC_ROOT")
        try:
            sys.path[:] = path_seed
            if env is not None:
                os.environ["MOOR_PYTHON_SRC_ROOT"] = env
            elif "MOOR_PYTHON_SRC_ROOT" in os.environ:
                del os.environ["MOOR_PYTHON_SRC_ROOT"]
            hb.harden_import_path(src_root="/opt/moor")
            return sys.path[:]
        finally:
            sys.path[:] = original
            if original_env is None:
                os.environ.pop("MOOR_PYTHON_SRC_ROOT", None)
            else:
                os.environ["MOOR_PYTHON_SRC_ROOT"] = original_env

    def test_relative_cwd_forms_removed(self):
        hb = _fresh_import()
        result = self._run(hb, ["", ".", "/opt/moor", "/usr/lib/python"])
    ...
    def test_absolute_cwd_path_loses_to_src_root(self):
        ...
        # Moor root.  The guard must relocate Moor to the front.
        hb = _fresh_import()
        result = self._run(hb, ["/home/user/tg-ws-proxy", "/opt/moor"])
        assert result[0] == "/opt/moor"
        # The cwd absolute path may still appear (it can hold legit deps),
        # but only AFTER the Moor root.
        assert result.index("/opt/moor") < result.index("/home/user/tg-ws-proxy")
    ...
    def test_env_var_used_when_no_arg(self):
        ...
        try:
            ...
            os.environ["MOOR_PYTHON_SRC_ROOT"] = "/env/moor"
            hb.harden_import_path()
            assert sys.path[0] == "/env/moor"
        finally:
            sys.path[:] = original
            if original_env is None:
                os.environ.pop("MOOR_PYTHON_SRC_ROOT", None)
