...
``Invoke-MoorStep``, which starts the step with ``RedirectStandardOutput`` /
...
last of them to close it. ``moor update`` deliberately runs build steps with
stdout inherited (the tee-stderr runner in ``moor_cli/main.py``), so the
...
closes and ``Invoke-MoorStep`` blocks for the life of the gateway.

Everything the hand-off owes the Desktop is downstream of that call:
``.moor-update-result.json`` is never written, ``.moor-update-in-progress``
is never cleared, and the Desktop is never relaunched -- so the app sits on
"Updating Moor" until the user kills the gateway by hand, and the stale marker
...
step writing to both pipes took 18.3s vs 0.29s. ``moor update`` is exactly this
...
class TestIdleWatchdogCountsUpdateLogGrowth:
    ...
    Real updates are stdout-silent for 40+ minutes: ``moor update`` captures
    the (very loud) Electron/vite build into ``logs/update.log`` — NOT the
    child's stdout (``moor_cli/update_cmd.py``, the update-log tee) — so the
    ...
    the ``MOOR_UPDATE_STEP_IDLE_SECONDS`` override each fails a test here.
    """
...
def test_update_step_survives_pipe_leak_flood_and_live_child_stall(
    ...
    *leak* -- a step that spawns a grandchild with ``UseShellExecute = $false``
    ...
    asserts the grandchild was **still alive** when ``Invoke-MoorStep``
    ...
    *logstall* -- a step that is silent on its pipes but appends to the
    ...
    This is the shape of every real ``moor update`` build: output streams to
    ...
    env = {
        **os.environ,
        ...
        "MOOR_UPDATE_STEP_IDLE_SECONDS": "3",
    ...
    assert "PIPE-DRAIN SELF-TEST: PASS" in result.stdout, (
        ...
        "Moor' forever) or metered a chatty step (backpressure on the running "
