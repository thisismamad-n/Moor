...
Real incident (gateway session 1518671026962174144)::
    ...
    cannot import name 'env_float' from 'utils' (~/.moor/moor-agent/utils.py)
...
2. ``moor update`` ran ``git pull``, updating ``utils.py`` (now defining
...
``moor_cli/main.py`` (the ``moor update`` flow, ~line 9326) already
acknowledges this exact hazard -- "source files on disk are newer than cached
Python modules in this process" -- and reloads ``moor_constants`` after the
