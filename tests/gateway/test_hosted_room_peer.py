...
def test_room_link_endpoint_reads_supported_config_with_env_override(
    ...
    (home / "config.yaml").write_text(
        "gateway:\n  room_link_url: https://configured.example.test/moor\n",
        encoding="utf-8",
    )
    monkeypatch.setenv("MOOR_HOME", str(home))
    ...
    assert local_room_link_endpoint() == {
        "available": True,
        "url": "https://configured.example.test/moor",
        "transport_security": "tls",
    }

    monkeypatch.setenv(
        "MOOR_ROOM_LINK_URL", "https://override.example.test/moor"
    )
    assert local_room_link_endpoint()["url"] == (
        "https://override.example.test/moor"
    )
...
def test_named_profile_inherits_gateway_room_link_endpoint(tmp_path, monkeypatch):
    ...
    (root / "config.yaml").write_text(
        "gateway:\n  room_link_url: https://gateway.example.test/moor\n",
        encoding="utf-8",
    )
    ...
    try:
        assert local_room_link_endpoint() == {
            "available": True,
            "url": "https://gateway.example.test/moor",
            "transport_security": "tls",
        }
    finally:
        reset_moor_home_override(token)
...
def test_named_profile_room_link_override_wins_over_gateway_root(
    ...
    (profile / "config.yaml").write_text(
        "gateway:\n  room_link_url: https://profile.example.test/moor\n",
        encoding="utf-8",
    )
    monkeypatch.setenv("MOOR_HOME", str(root))
    monkeypatch.delenv("MOOR_ROOM_LINK_URL", raising=False)
    ...
    try:
        assert local_room_link_endpoint()["url"] == (
            "https://profile.example.test/moor"
        )
    finally:
        reset_moor_home_override(token)
