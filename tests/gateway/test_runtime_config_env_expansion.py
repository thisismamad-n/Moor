...
def gateway_home(monkeypatch, tmp_path):
    ...
    monkeypatch.delenv("MOOR_PREFILL_MESSAGES_FILE", raising=False)
    monkeypatch.delenv("MOOR_EPHEMERAL_SYSTEM_PROMPT", raising=False)
    monkeypatch.delenv("MOOR_GATEWAY_BUSY_INPUT_MODE", raising=False)
    monkeypatch.delenv("MOOR_RESTART_DRAIN_TIMEOUT", raising=False)
    monkeypatch.delenv("MOOR_BACKGROUND_NOTIFICATIONS", raising=False)
    return tmp_path
