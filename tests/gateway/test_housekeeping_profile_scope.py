...
``gateway.multiplex_profiles`` the skills-sync pulls resolved Moor credentials through the
...
def _profile(home: Path, base_url: str) -> None:
    ...
    (home / ".env").write_text(f"MOOR_INFERENCE_BASE_URL={base_url}\n", encoding="utf-8")
    (home / "auth.json").write_text(json.dumps({"version": 1, "providers": {"moor": {
...
def two_homes(tmp_path, monkeypatch):
    ...
    # The hermetic conftest pins ``moor_state.DEFAULT_DB_PATH`` at one sandbox store whenever
    # moor_state is already imported, and that pin WINS over ``get_moor_home()`` inside
    ...
    resolved = Path(moor_state._default_db_path())
...
def _record_credential_chores(monkeypatch):
    """Replace the three credential-reading chores with recorders of (home, Moor override) they see."""
    import agent.curator as curator
    import tools.skills_sync_client as ssc
    import tools.skills_sync_client_org as sso
    from moor_cli.auth_moor import _moor_inference_env_override
    from moor_constants import get_moor_home

    seen: dict = {"sync": [], "org": [], "curator": []}

    def _rec(key):
        return lambda *a, **k: seen[key].append((get_moor_home().name, _moor_inference_env_override()))
...
def test_multiplexed_auto_archive_tick_sweeps_every_served_profile_store(two_homes, monkeypatch):
    ...
    ``acquire()`` resolves through ``get_moor_home()``, so an unscoped tick archived the
    launch profile's store only — and `moor serve`/the dashboard defer to the gateway for
...
def test_profile_scope_setup_failure_restores_the_callers_home(two_homes, monkeypatch):
    ...
    resolving ``get_moor_home()`` to the failed profile for every later unscoped read.
    """
...
def test_multiplexed_plugin_update_check_visits_every_served_profiles_plugins(two_homes, monkeypatch):
    ...
    import moor_cli.plugins_updates as updates
