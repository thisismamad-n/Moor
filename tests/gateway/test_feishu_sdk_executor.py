...
async def test_run_blocking_executes_on_owned_pool():
    ...
    assert captured["thread"].startswith("moor-feishu-sdk")
    adapter._shutdown_sdk_executor()
...
async def test_run_blocking_propagates_caller_contextvars(tmp_path, monkeypatch):
    ...
    multiplexed profile's MOOR_HOME override is a contextvar, and a worker that lost it
    would flush dedup state / look up threads under the wrong profile."""
    monkeypatch.setenv("MOOR_HOME", str(tmp_path))
    from gateway.config import PlatformConfig
    from moor_constants import get_moor_home_override, reset_moor_home_override, set_moor_home_override

    adapter = FeishuAdapter(PlatformConfig())
    token = set_moor_home_override(str(tmp_path / "profiles" / "secondary"))
    try:
        assert await adapter._run_blocking(get_moor_home_override) == str(tmp_path / "profiles" / "secondary")
    finally:
