...
``X-moor-session-Key``.  Every conversation-affinity hint Moor sends is
derived from that physical id — ``prompt_cache_key`` on both OpenAI-wire
transports, the OpenRouter/Moor sticky ``session_id``, and xAI's
...
def _headers(session_key=None):
    ...
    if session_key:
        h["X-moor-session-Key"] = session_key
    return h
