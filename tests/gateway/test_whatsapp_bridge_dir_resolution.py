...
(/opt/moor/scripts/whatsapp-bridge) is read-only, so `npm install` fails
with EACCES. The resolver must detect the read-only install dir and mirror the
bridge source into a writable MOOR_HOME location instead.
"""
import importlib
...
def test_readonly_install_mirrors_to_moor_home(tmp_path, monkeypatch):
    """A read-only install tree is mirrored into a writable MOOR_HOME."""
    ...
    monkeypatch.setattr(
        "moor_constants.get_moor_home", lambda: moor_home
    )
    ...
    expected = moor_home / "scripts" / "whatsapp-bridge"
