...
def store_factory(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(moor_state, "SessionDB", _raise)
...
def test_codex_override_never_runs_on_the_default_providers_endpoint(store_factory, codex_on_turn):
    ...
    DEFAULT provider's runtime (Moor URL + Moor key + chat_completions). The turn runs on ONE coherent
    ...
    moor = {"provider": "moor", "api_key": "moor-key", "api_mode": "chat_completions",
            "base_url": "https://inference-api.nousresearch.com/v1"}
    ...
    with patch("gateway.run._resolve_runtime_agent_kwargs_for_provider", side_effect=_for_provider), \
         patch("gateway.run._resolve_runtime_agent_kwargs", return_value=dict(moor)):
        model, runtime = runner._resolve_session_agent_runtime(
            session_key=session_key, user_config={"model": {"default": "openai/gpt-6-luna", "provider": "moor"}})

    expected_model, expected = ("gpt-6-luna-900k", codex) if codex_on_turn == "recovers" else ("openai/gpt-6-luna", moor)
