...
class TestMentionPatterns:
    ...
    def test_pattern_matches_text(self, monkeypatch):
        adapter = _make_gating_adapter(
            monkeypatch, extra={"mention_patterns": ["^moor"]}
        )
        assert adapter._message_matches_mention_patterns("moor please help") is True
        assert adapter._message_matches_mention_patterns("please moor help") is False
