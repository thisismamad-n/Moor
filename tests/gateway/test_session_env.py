...
def test_set_session_env_sets_contextvars(monkeypatch):
    ...
    monkeypatch.delenv("MOOR_SESSION_CHAT_NAME", raising=False)
    monkeypatch.delenv("MOOR_SESSION_CHAT_TYPE", raising=False)
    monkeypatch.delenv("MOOR_SESSION_USER_ID", raising=False)
    monkeypatch.delenv("MOOR_SESSION_USER_NAME", raising=False)
    ...
    assert get_session_env("MOOR_SESSION_PLATFORM") == "telegram"
    assert get_session_env("MOOR_SESSION_SOURCE") == ""
    assert get_session_env("MOOR_SESSION_CHAT_ID") == "-1001"
    assert get_session_env("MOOR_SESSION_CHAT_NAME") == "Group"
    assert get_session_env("MOOR_SESSION_CHAT_TYPE") == "group"
    assert get_session_env("MOOR_SESSION_USER_ID") == "123456"
    assert get_session_env("MOOR_SESSION_USER_NAME") == "alice"
    assert get_session_env("MOOR_SESSION_THREAD_ID") == "17585"

    # os.environ should NOT be touched
    assert os.getenv("MOOR_SESSION_PLATFORM") is None
    assert os.getenv("MOOR_SESSION_SOURCE") is None
    assert os.getenv("MOOR_SESSION_CHAT_TYPE") is None
    assert os.getenv("MOOR_SESSION_THREAD_ID") is None
...
def test_clear_session_env_restores_previous_state(monkeypatch):
    ...
    assert get_session_env("MOOR_SESSION_PLATFORM") == ""
    assert get_session_env("MOOR_SESSION_CHAT_ID") == ""
    assert get_session_env("MOOR_SESSION_CHAT_NAME") == ""
    assert get_session_env("MOOR_SESSION_CHAT_TYPE") == ""
    assert get_session_env("MOOR_SESSION_USER_ID") == ""
    assert get_session_env("MOOR_SESSION_USER_NAME") == ""
    assert get_session_env("MOOR_SESSION_THREAD_ID") == ""
...
def test_get_session_env_falls_back_to_os_environ(monkeypatch):
    ...
    assert get_session_env("MOOR_SESSION_PLATFORM") == "discord"
...
def test_session_key_falls_back_to_os_environ(monkeypatch):
    ...
    monkeypatch.setenv("MOOR_SESSION_KEY", "env-session-123")

    # No contextvar set — should read from os.environ
    assert get_session_env("MOOR_SESSION_KEY") == "env-session-123"

    # Set contextvar — should prefer it
    tokens = set_session_vars(session_key="ctx-session-456")
    assert get_session_env("MOOR_SESSION_KEY") == "ctx-session-456"
...
def test_session_key_no_race_condition_with_contextvars(monkeypatch):
    ...
    async def handler(key: str, delay: float):
        ...
        try:
            ...
            read_back = get_session_env("MOOR_SESSION_KEY")
    ...
    try:
        result = await runner._run_in_executor_with_context(
            lambda: {
                ...
                "user_id": get_session_env("MOOR_SESSION_USER_ID"),
...
def test_cron_session_contextvar_preserves_legacy_env_fallback(monkeypatch):
    ...
    assert get_session_env("MOOR_CRON_SESSION") == "1"
...
def test_cron_session_explicit_blank_masks_leaked_env(monkeypatch):
    ...
    try:
        assert get_session_env("MOOR_CRON_SESSION") == ""
    finally:
        clear_session_vars(tokens)
