...
def test_recover_closes_owned_db_when_unexpected_exception_escapes(
    ...
    db = InterruptingDB()
    monkeypatch.setattr("moor_state_registry.acquire", lambda: db)
    monkeypatch.setattr(
        "moor_state_registry.release_or_close", lambda _: setattr(db, "released", True)
    )
...
def test_get_flush_dir_uses_get_moor_home(tmp_path, monkeypatch):
    """Flush dir must use get_moor_home(), not hardcoded Path.home()."""
    ...
    monkeypatch.setattr(
        "moor_constants.get_moor_home", fake_get_moor_home
    )
