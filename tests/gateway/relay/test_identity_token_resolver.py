"""Unit tests for the generic-OIDC / Moor-Portal caller-identity token resolver.

Covers gateway.relay._resolve_relay_identity_token() — the canonical resolver
shared by the runtime self-provision path and the `moor gateway enroll` CLI.
...
Three modes:
  ...
  2. Moor Portal (resolve_moor_access_token) otherwise — the default.

The HTTP calls and the Moor resolver are monkeypatched; these prove the mode
