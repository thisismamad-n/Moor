...
# abstract (NUL-prefixed) AF_UNIX names are a Linux kernel feature
def test_notify_supports_systemd_abstract_socket(monkeypatch):
    name = "\0moor-test-notify"
    receiver = socket.socket(socket.AF_UNIX, socket.SOCK_DGRAM)
    receiver.bind(name)
    receiver.settimeout(1.0)
    monkeypatch.setenv("NOTIFY_SOCKET", "@moor-test-notify")
...
def test_notify_uses_nonblocking_datagram_send(monkeypatch):
    ...
    monkeypatch.setenv("NOTIFY_SOCKET", "/tmp/moor-test-notify")
