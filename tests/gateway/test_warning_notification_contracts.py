...
def test_real_retry_producers_keep_final_failure_and_persistence(tmp_path, monkeypatch, caplog, enabled):
    ...
    sent.clear()
    with patch("agent.moor_rate_guard.moor_rate_limit_remaining", return_value=60), \
         patch("moor_cli.anon_auth.apply_model_switch"), \
         patch("moor_cli.anon_auth.route_is_welcome_host", return_value=False):
        verdict = moor_rate_limit_guard(agent, _retry=SimpleNamespace(), api_messages=[], messages=[],
