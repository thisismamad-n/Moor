...
redirected ``get_moor_home()`` for that turn.  The rows carried the correct
...
def multiplex_homes(tmp_path, monkeypatch):
    """A root home plus a named profile home, with MOOR_HOME on the root.
    ...
    # the hatch is closed and resolution goes through ``get_moor_home()``,
    # which is what production does.  ``MOOR_HOME`` above still keeps that
...
def test_primary_handler_enters_routed_profile_scope_before_dispatch(multiplex_homes):
    ...
    async def capture_scope(event):
        seen.append(Path(get_moor_home()))
...
def test_primary_handler_rejected_route_falls_back_and_marks_sentinel(multiplex_homes):
    ...
    async def capture_scope(event):
        seen.append((Path(get_moor_home()), event.source.profile_route_rejected))
