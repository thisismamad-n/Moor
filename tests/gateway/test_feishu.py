...
class TestAdapterBehavior(unittest.TestCase):
    ...
    @patch.dict(
        os.environ,
        {
            "MOOR_FEISHU_TEXT_BATCH_MAX_MESSAGES": "2",
        },
        clear=True,
    )
    ...
    def test_download_remote_document_reads_response_before_httpx_client_closes(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            with patch.dict(os.environ, {"MOOR_HOME": tmp}, clear=False):
                adapter = FeishuAdapter(PlatformConfig())
    ...
    def test_dedup_state_persists_across_adapter_restart(self):
        ...
        with tempfile.TemporaryDirectory() as temp_home:
            with patch.dict(os.environ, {"MOOR_HOME": temp_home}, clear=False):
    ...
    def test_send_splits_fenced_code_blocks_into_separate_post_rows(self):
        ...
        content = (
            "确认已入库 ✓\n"
            "文件路径：`/root/.moor/profiles/agent_cto/cron/jobs.json`\n"
            "**解码后的内容：**\n"
            "```json\n"
            '{"cron": "list"}\n'
            "```\n"
...
class TestHydrateBotIdentity(unittest.TestCase):
    ...
    def test_hydration_populates_open_id_from_bot_info(self):
        ...
        self.assertEqual(adapter._bot_open_id, "ou_moor_hydrated")
...
class TestWebhookSecurity(unittest.TestCase):
    ...
    def test_webhook_request_rejects_oversized_chunked_body_while_reading(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            token = set_moor_home_override(tmpdir)
            try:
                adapter = FeishuAdapter(PlatformConfig())
            finally:
...
class TestDedupTTL(unittest.TestCase):
    ...
    def test_load_tolerates_malformed_timestamp_values(self):
        ...
        with tempfile.TemporaryDirectory() as temp_home:
            with patch.dict(os.environ, {"MOOR_HOME": temp_home}, clear=True):
                adapter = FeishuAdapter(PlatformConfig())
    ...
    def test_concurrent_dedup_persists_land_in_order(self):
        ...
        # The class-level env wipe drops the per-test MOOR_HOME, so the adapter resolves
        # its dedup store under the operator's real ~/.moor and every id a live gateway
        ...
        with tempfile.TemporaryDirectory() as scratch:
            with patch("plugins.platforms.feishu.adapter.get_moor_home", return_value=Path(scratch)):
                adapter = FeishuAdapter(PlatformConfig())
            writes = []
            calls = [0]
