...
def _clear_env(monkeypatch):
    monkeypatch.delenv("MOOR_GATEWAY_HTTPX_KEEPALIVE_EXPIRY", raising=False)
    monkeypatch.delenv("MOOR_GATEWAY_HTTPX_MAX_KEEPALIVE", raising=False)

def test_env_override_rejects_garbage(monkeypatch):
    """Malformed env values fall back to defaults rather than raising."""
    monkeypatch.setenv("MOOR_GATEWAY_HTTPX_KEEPALIVE_EXPIRY", "not-a-number")
    monkeypatch.setenv("MOOR_GATEWAY_HTTPX_MAX_KEEPALIVE", "-3")
