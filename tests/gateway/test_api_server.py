    ...
    async def test_health_detailed_returns_ok(self, adapter):
        ...
        with patch("gateway.status.read_runtime_status", return_value={
            ...
            async with TestClient(TestServer(app)) as cli:
                ...
                assert data["platform"] == "moor-agent"
...
class TestModelsEndpoint:
    @pytest.mark.asyncio
    async def test_models_returns_moor_agent(self, adapter):
        app = _create_app(adapter)
        async with TestClient(TestServer(app)) as cli:
            ...
            assert data["data"][0]["id"] == "moor-agent"
            assert data["data"][0]["owned_by"] == "moor"
    ...
    def test_resolve_model_name_default_profile(self):
        """Default profile falls back to 'moor-agent'."""
        with patch("moor_cli.profiles.get_active_profile_name", return_value="default"):
            assert APIServerAdapter._resolve_model_name("") == "moor-agent"
    ...
    async def test_model_options_returns_shared_inventory(self, adapter, monkeypatch):
        ...
        ctx = object()
        payload = {
            "providers": [{"slug": "moor", "name": "Moor Portal", "models": ["gpt-5.5"]}],
            "model": "gpt-5.5",
            "provider": "moor",
        }
        seen = {"thread_calls": 0}
...
class TestCapabilitiesEndpoint:
    ...
    async def test_capabilities_advertises_plugin_safe_contract(self, adapter):
        ...
        async with TestClient(TestServer(app)) as cli:
            ...
            assert resp.status == 200
            data = await resp.json()
            assert data["object"] == "moor.api_server.capabilities"
            assert data["platform"] == "moor-agent"
            assert data["model"] == "moor-agent"
...
class TestToolsetsEndpoint:
    ...
    async def test_toolsets_returns_resolved_tools(self, adapter):
        ...
        with patch(
            "moor_cli.tools_config._get_effective_configurable_toolsets",
            return_value=fake_toolsets,
        ), patch(
            "moor_cli.tools_config._get_platform_tools",
            return_value={"default"},
        ), patch(
            "moor_cli.tools_config.get_moor_subscription_features",
            return_value=feature_snapshot,
        ) as resolve_features, patch(
            "moor_cli.tools_config._toolset_has_keys",
            return_value=True,
        ) as has_keys, patch(
...
class TestChatCompletionsEndpoint:
    ...
    async def test_stream_includes_tool_progress(self, adapter):
        ...
        async with TestClient(TestServer(app)) as cli:
            ...
            with patch.object(adapter, "_run_agent", side_effect=_mock_run_agent):
                ...
                assert "event: moor.tool.progress" in body
    ...
    async def test_stream_emits_tool_lifecycle_with_call_id(self, adapter):
        ...
        ``tool.started``-style ``moor.tool.progress`` event; clients
        ...
        of the lifecycle pair on the same ``event: moor.tool.progress``
        ...
        async with TestClient(TestServer(app)) as cli:
            ...
            for i, line in enumerate(lines):
                if line.strip() != "event: moor.tool.progress":
                    continue
    ...
    async def test_result_error_fallback_is_redacted(self, adapter):
        ...
        async with TestClient(TestServer(app)) as cli:
            with patch.object(adapter, "_run_agent", new_callable=AsyncMock) as mock_run:
                ...
                resp = await cli.post(
                    "/v1/responses",
                    json={"model": "moor-agent", "input": "Hello"},
                )
    ...
    async def test_stream_task_done_callback_enqueues_eos_for_responses(self, adapter):
        ...
        async with TestClient(TestServer(app)) as cli:
            ...
            with (
                ...
                resp = await cli.post(
                    "/v1/responses",
                    json={"model": "moor-agent", "input": "hi", "stream": True},
                )
                assert resp.status == 200
    ...
    async def test_get_stored_response(self, adapter):
        ...
        async with TestClient(TestServer(app)) as cli:
            ...
            with patch.object(adapter, "_run_agent", new_callable=AsyncMock) as mock_run:
                ...
                resp = await cli.post(
                    "/v1/responses",
                    json={"model": "moor-agent", "input": "Hi"},
                )
    ...
    async def test_tool_calls_in_output(self, adapter):
        ...
        async with TestClient(TestServer(app)) as cli:
            with patch.object(adapter, "_run_agent", new_callable=AsyncMock) as mock_run:
                ...
                resp = await cli.post(
                    "/v1/responses",
                    json={"model": "moor-agent", "input": "What is 6*7?"},
                )
    ...
    async def test_hard_failure_redacts_secret_like_error_text(self, adapter):
        ...
        async with TestClient(TestServer(app)) as cli:
            with patch.object(adapter, "_run_agent", new_callable=AsyncMock) as mock_run:
                ...
                resp = await cli.post(
                    "/v1/chat/completions",
                    json={"model": "moor-agent", "messages": [{"role": "user", "content": "hello"}]},
                )
            ...
            assert "OPENAI_API_KEY=" in body
            assert data["error"]["moor"]["failed"] is True
            ...
            with patch.object(auth_adapter, "_run_agent", new_callable=AsyncMock) as mock_run:
                for bad in ("../../../../etc/pwned", "/abs/path", "..\\win"):
                    resp = await cli.post(
                        ...
                        json={"model": "moor-agent", "messages": [{"role": "user", "content": "hi"}]},
                    )
        ...
        async with TestClient(TestServer(app)) as cli:
            with patch.object(auth_adapter, "_run_agent", new_callable=AsyncMock) as mock_run:
                ...
                resp = await cli.post(
                    ...
                    json={"model": "moor-agent", "input": "hello", "store": False},
                )
            assert resp.status == 200
...
class TestStoredSessionModelFilter:
    ...
    "no stored model" — replaying "moor-agent" upstream 400s. Found live
    (Aug 2026): the first cross-gateway `moor peer dm` against a fresh
    api_server failed every turn with "moor-agent is not a valid model ID".
    """
...
class TestSessionDbOffEventLoop:
    ...
    async def test_create_session_without_model_does_not_persist_virtual_alias(self, auth_adapter):
        ...
        virtual model alias (self._model_name, e.g. "moor-agent") as if it
        ...
        "moor-agent" as an invalid model identifier on every turn.
        """
        app = _create_app(auth_adapter)
    ...
    async def test_create_session_with_explicit_virtual_alias_does_not_persist_it(self, auth_adapter):
        """Sending ``model: "moor-agent"`` explicitly (the virtual alias
    ...
    async def test_create_session_with_provider_prefixed_virtual_alias_does_not_persist_it(self, auth_adapter):
        ...
        "openrouter::moor-agent" never matched self._model_name and leaked
...
class TestApiKeyStartupGuardFailsClosed:
    ...
    def _blocking_auth_import():
        ...
        def _blocked(name, *args, **kwargs):
            if name == "moor_cli.auth":
                raise ImportError("simulated: moor_cli.auth unavailable")
...
class TestCreateAgentModelRecovery:
    def test_create_agent_defaults_to_provider_catalog_model_when_empty(self, monkeypatch):
        ...
        `moor auth add openai-codex` without `moor model`) —
        ...
        monkeypatch.setattr(
            "moor_cli.models.get_default_model_for_provider",
    ...
    def test_create_agent_does_not_cache_virtual_alias(self, monkeypatch):
        """Write-side guard: the advertised virtual model (``moor-agent``)
