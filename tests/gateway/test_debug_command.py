...
class TestHandleDebugCommand:
    ...
    async def test_debug_sweeps_expired_pastes_before_upload(self):
        ...
        with patch("moor_cli.debug._sweep_expired_pastes", return_value=(0, 0)) as mock_sweep, \
             patch("moor_cli.debug._capture_dump", return_value="dump"), \
             patch("moor_cli.debug.collect_debug_report", return_value="report"), \
             patch("moor_cli.debug.upload_to_pastebin", return_value="https://paste.rs/report"), \
