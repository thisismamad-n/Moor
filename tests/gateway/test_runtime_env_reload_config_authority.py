...
MOOR_MAX_ITERATIONS, but a later per-turn load_dotenv(..., override=True)
can restore a stale .env MOOR_MAX_ITERATIONS value before the next turn.
"""
...
def test_reload_runtime_env_preserves_config_terminal_backend(
    ...
    ``moor config get terminal.backend`` still says local.
    """
    moor_home = tmp_path / ".moor"
    moor_home.mkdir()
    (moor_home / "config.yaml").write_text(
    ...
    (moor_home / ".env").write_text("TERMINAL_ENV=docker\n", encoding="utf-8")
