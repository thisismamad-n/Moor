...
from moor_state_common import FTS_REBUILD_DEFERRAL_KEY, FTS_STALE_KEY, LEGACY_FTS_SQL, LEGACY_FTS_TRIGRAM_SQL, SCHEMA_SQL, _FTS_TRIGGERS
from moor_state_dbfile import _concrete_state_db_holder_pids, _is_inactive_orphan_desktop_holder
...
class TestRuntimeFtsRebuild:
    def test_reap_candidates_exclude_uninspectable_holder_suspicions(
        ...
        assert _concrete_state_db_holder_pids(
            db_path,
            [
                (222, "uninspectable holder: python -m moor_cli.main serve --port 0"),
    ...
    @pytest.mark.parametrize(
        "argv",
        (
            ("journalctl", "-u", "moor-agent.service"),
            ("grep", "moor-agent", "/var/log/syslog"),
            (
                "/usr/sbin/tailscaled",
                "be-child",
                "ssh",
                "--cmd=python -m moor_cli.main gateway",
            ),
            ("tmux", "new-session", "/opt/moor-agent/.venv/bin/moor gateway"),
            ("python3", "/opt/moor-agent/tools/check_state.py"),
            ("moor-monitor", "gateway"),
            ("moorctl", "serve"),
            ("python3", "worker.py", "moor_cli.main"),
            ("python3", "-m", "other.module", "moor_cli.main"),
            ("python3", "-c", "moor_cli.main"),
            ("python3", "-Icprint('moor_cli.main')", "moor_cli/main.py"),
        ),
    )
    def test_uninspectable_non_moor_process_is_not_a_holder(self, argv):
        assert not moor_state_holders._looks_like_moor(argv)

    @pytest.mark.parametrize(
        "argv",
        (
            ("/usr/local/bin/moor", "gateway"),
            ("/usr/local/bin/moor-agent", "serve"),
            ("/usr/local/bin/moor-acp", "--stdio"),
            ("/usr/bin/python3", "-m", "moor_cli.main", "gateway"),
            ("/usr/bin/python3", "-m", "acp_adapter"),
            ("/usr/bin/python3", "-Im", "moor_cli.main", "gateway"),
            ("/usr/bin/python3", "-mmoor_cli.main", "gateway"),
            ("/usr/bin/python3", "-W", "ignore", "-m", "moor_cli.main"),
            ("/usr/bin/python3", "-Xdev", "-m", "moor_cli.main"),
            (
                "/opt/moor-agent/.venv/bin/python",
                "/opt/moor-agent/moor_cli/main.py",
                "gateway",
            ),
            ("python.exe", "--", "moor_cli/main.py", "gateway"),
            ("python3", "/opt/moor-agent/run_agent.py", "--query", "hello"),
        ),
    )
    def test_uninspectable_moor_process_remains_a_holder(self, argv):
        assert moor_state_holders._looks_like_moor(argv)
    ...
    def test_foreign_holder_uninspectable_process_cmdline_fallback(
        ...
        flagged when /proc/<pid>/cmdline identifies it as a Moor process."""
        db_path = tmp_path / "state.db"
        ...
        # PID 222's cmdline is world-readable and looks like Moor
        cmdline_path = proc_root / "222" / "cmdline"
        cmdline_path.write_bytes(
            b"python3\x00-m\x00moor_cli.main\x00chat\x00"
        )
        ...
        def _fake_argv(pid):
            ...
            try:
                ...
                return None
        monkeypatch.setattr(moor_state_holders, "_read_proc_argv", _fake_argv)

        holders = moor_state_holders.foreign_state_db_holders(db_path)
        # Should include PID 222 with the cmdline info
        assert len(holders) == 1
        assert holders[0][0] == 222
        assert "moor_cli.main" in holders[0][1]
    ...
    def test_structural_corruption_propagates_without_live_fts_mutation(
        ...
        from moor_state import StateDbCorruptError
    ...
    def test_repeated_deferrals_reap_inactive_orphan_then_rebuild(
        ...
        monkeypatch.setattr(moor_state_schema.time, "time", lambda: 120.0)
...
class TestPhysicalCorruptionAcceptance:
    ...
    def test_canonical_btree_corruption_fails_closed(
        ...
        try:
            caplog.clear()
            with caplog.at_level("WARNING", logger="moor_state"):
                with pytest.raises(sqlite3.DatabaseError) as caught:
