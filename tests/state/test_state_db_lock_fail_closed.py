...
work on a file several Moor processes share (gateway service, the Desktop
app's `moor serve` backend, CLI sessions, the TUI slash worker):

* `moor_state_common.fts_rebuild_admission` — full structural FTS rebuilds
* `moor_state_repair._cross_process_repair_lock`   — writable_schema surgery / VACUUM
