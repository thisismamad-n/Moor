...
def test_xai_oauth_authorize_url_includes_referrer_hermes_agent():
    """Attribution: xAI's OAuth server can identify Moor-originated logins
...
def test_xai_callback_handler_returns_400_when_callback_url_lacks_code_and_error():
    ...
    try:
        ...
        assert "moor auth add xai-oauth" in body
...
def test_credential_pool_seed_respects_suppression(tmp_path, monkeypatch):
    """`moor auth remove xai-oauth <N>` for the seeded entry suppresses
    ...
    # Suppress the source — mimic `moor auth remove`.
...
def test_pool_exhausted_xai_entry_recovers_after_singleton_refresh(tmp_path, monkeypatch):
    ...
    user runs ``moor model`` -> xAI Grok OAuth (or another process
    ...
    # Simulate the user re-running `moor model` -> xAI Grok OAuth: the
...
def test_pool_manual_xai_entry_not_synced_from_singleton(tmp_path, monkeypatch):
    ...
    ``moor auth add xai-oauth``) own their own refresh-token lifecycle
