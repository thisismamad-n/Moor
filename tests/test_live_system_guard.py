...
def test_argv_arguments_are_not_treated_as_executables(tmp_path):
    ...
    guard (the path contains "moor" via the pytest tmp root)."""
...
def test_direct_killer_argv_is_still_blocked():
    with pytest.raises(RuntimeError, match="live-system guard"):
        subprocess.run(["pkill", "-f", "moor-guard-regression-nomatch"])
...
def test_wrapped_killer_command_is_still_blocked():
    ...
    shell wrapper."""
    with pytest.raises(RuntimeError, match="live-system guard"):
        subprocess.run(["bash", "-c", "pkill -f moor-guard-regression-nomatch"])


def test_env_wrapped_killer_command_is_still_blocked():
    with pytest.raises(RuntimeError, match="live-system guard"):
        subprocess.run(["env", "GUARD_TEST=1", "pkill", "-f", "moor-guard-regression-nomatch"])


def test_gateway_start_inside_a_container_exec_is_not_blocked():
    """``docker exec <ctr> moor gateway start`` launches the gateway INSIDE the container,
    ...
    stub_dir = os.path.join(os.environ["MOOR_HOME"], "stub-bin")
    ...
    result = subprocess.run(
        [stub, "exec", "-u", "moor", "ctr", "sh", "-c", "moor -p prof gateway start"],
        capture_output=True,
        text=True,
    )
    assert result.returncode == 0
