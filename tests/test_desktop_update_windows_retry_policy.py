...
def test_retry_policy_distinguishes_self_lock_deferral(tmp_path: Path) -> None:
    ...
    command = f"""
        . '{policy}'
        $withoutMarker = @(
            (Test-MoorUpdateShouldRetry -ExitCode 0 -InstallRoot '{root}'),
            (Test-MoorUpdateShouldRetry -ExitCode 1 -InstallRoot '{root}'),
            (Test-MoorUpdateShouldRetry -ExitCode 2 -InstallRoot '{root}')
        )
        New-Item -ItemType File -Path (Join-Path '{root}' '.update-incomplete') | Out-Null
        $withMarker = Test-MoorUpdateShouldRetry -ExitCode 2 -InstallRoot '{root}'
