"""`moor profile install` / `moor profile update` against a real distribution with cron jobs.

A distribution author schedules jobs with the real CLI (``moor -p author cron create``) and
publishes the profile directory; a user installs it, schedules a job of their own, and later
pulls the author's next version with ``moor profile update``. Every step is a fresh sandboxed
...
class World:
    def __init__(self, root: Path):
        ...
        self.moor_home = Path(self.env["MOOR_HOME"])
    ...
    def cli(self, *args: str):
        cp = H.run([PY, "-m", "moor_cli.main", *args], env=self.env, cwd=self.root, writable=[self.root], timeout=300)
    ...
    def installed(self) -> Path:
        return self.moor_home / "profiles" / "shipped-bot"
...
def world(tmp_path_factory) -> World:
    ...
    author = w.moor_home / "profiles" / "author"
