...
Every Moor process these suites spawn runs:

* with a HOME/MOOR_HOME under the test's tmp dir and an environment built from an
  allowlist (no inherited ``*_API_KEY`` / ``MOOR_*``), so only the fake provider is
  configured;
* inside a ``bwrap`` sandbox when bubblewrap is usable: its own PID namespace (the
  ...
  ``~/.moor`` bind-mounted read-only, and ``--die-with-parent`` so killing the
...
def isolated_env(
    ...
    *,
    ...
    """Allowlisted environment with HOME/MOOR_HOME under ``root``."""
    home = root / "home"
    moor_home = home / ".moor"
    ...
    env.update(
        ...
        DBUS_SESSION_BUS_ADDRESS="unix:path=/nonexistent/moor-test-bus",
        NO_COLOR="1",
        TERM="dumb",
        PYTHONUNBUFFERED="1",
        PYTHONHASHSEED="0",
    ...
    uv = shutil.which("uv") or (str(REAL_HOME / ".moor" / "bin" / "uv") if (REAL_HOME / ".moor" / "bin" / "uv").exists() else None)
...
def sandbox_argv(argv: Sequence[str], *, writable: Iterable[Path]) -> list[str]:
    ...
    real_moor = REAL_HOME / ".moor"
    if real_moor.is_dir():
        cmd += ["--ro-bind", str(real_moor), str(real_moor)]
    for w in writable:
        w = Path(w)
