"""Session-attach handshake: ``moor --tui --resume SID`` must hand the TUI only the REAL owner's socket.
...
reply's ``websocket_url`` as ``MOOR_TUI_GATEWAY_URL`` for the TUI child. Whoever answers on the
...
* the client is the real ``moor --tui --resume SID`` CLI (argparse, profile-home resolution, the
...
* the TUI child is a recording stand-in bundle selected through the supported prebuilt-bundle override
  (``MOOR_TUI_DIR/dist/entry.js``). It writes the environment the launcher handed it and exits, so the
...
Why the owner side is a test process: no shipped Moor runtime advertises ``shared_runtime_url`` or
serves ``/api/session-attach`` yet (``moor serve`` and the dashboard do not register one), so the
...
db.close()
home = str(Path(get_moor_home()).resolve())
state = {}
...
class Owner(BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        if "nonce" in q:
            reply["nonce"] = q["nonce"]
            msg = f"moor-session-attach:{sid}:{q['nonce']}".encode()
...
# Recording TUI bundle: the launcher runs ``node $MOOR_TUI_DIR/dist/entry.js`` with the env it built.
PROBE_ENTRY = """
const fs = require("fs");
fs.writeFileSync(process.env.ATTACH_PROBE_OUT, JSON.stringify({
  gateway_url: process.env.MOOR_TUI_GATEWAY_URL || null,
  resume: process.env.MOOR_TUI_RESUME || null,
}));
"""
...
rows = [e for e in active_session_registry_snapshot(get_moor_home(), strict=True)
...
def home(tmp_path: Path) -> Path:
    ...
    write_home(home / ".moor", "http://127.0.0.1:9/v1", api_key=canary("unused-key"))
...
def _resume(home: Path, sid: str) -> tuple[subprocess.CompletedProcess, dict | None]:
    """Run ``moor --tui --resume SID``; return the process result and what the TUI child received."""
    out = home.parent / f"handoff-{uuid.uuid4().hex}.json"
    result = run_moor(
        ["--tui", "--resume", sid], home, timeout=90,
        extra_env={"MOOR_TUI_DIR": str(home.parent / "tui-probe"), "MOOR_SKIP_NODE_BOOTSTRAP": "1",
