...
Moor runs for real in child processes with HOME=<tmp>/home and MOOR_HOME=<tmp>/home/.moor
...
_STRIP_PREFIXES = ("MOOR_", "OPENAI", "ANTHROPIC", "OPENROUTER", "AWS_", "AZURE_", "GOOGLE_", "GEMINI",
                   "PYTEST_", "MOOR_", "XAI_", "LLM_", "CUSTOM_", "TERMINAL_", "TENANT_", "API_SERVER_")
...
def hermetic_env(home: Path, extra: dict[str, str] | None = None) -> dict[str, str]:
    ...
    home = home.resolve()
    assert home != real_user_home() and home / ".moor" != real_user_home() / ".moor", home
    ...
    for var in ("HTTP_PROXY", "HTTPS_PROXY", "ALL_PROXY", "http_proxy", "https_proxy", "all_proxy", "XDG_STATE_HOME",
                ...
                # live moor-gateway unit, whatever it decides about service management.
...
def make_tenants(root: Path, names: Iterable[str], launch: str = "default") -> dict[str, Tenant]:
    """Build tenant homes on disk: launch profile at HOME/.moor, the rest under profiles/<name>."""
    moor_home = root / "home" / ".moor"
    tenants: dict[str, Tenant] = {}
    for name in names:
        home = moor_home if name == launch else moor_home / "profiles" / name
...
def seed_due_cron_job(t: Tenant, home: Path) -> str:
    ...
    r = subprocess.run([sys.executable, "-c", _SEED_CRON, t.cron_prompt], cwd=str(home),
                       env=hermetic_env(home, {"MOOR_HOME": str(t.home)}), capture_output=True, text=True,
...
def run_moor(argv: list[str], home: Path, timeout: float = 120.0,
               extra_env: dict[str, str] | None = None) -> subprocess.CompletedProcess:
    proc = subprocess.Popen(
        [sys.executable, "-m", "moor_cli.main", *argv], cwd=str(home), env=hermetic_env(home, extra_env),
...
class ServeBackend(TuiBackend):
    """The real Desktop backend: ``moor serve --port 0`` with the JSON-RPC surface on ``/api/ws``.

    ``MOOR_DESKTOP=1`` marks it app-spawned, which is also what runs the cron ticker in-process.
    """
    ...
    def __init__(self, home: Path, log_path: Path, extra_env: dict[str, str] | None = None) -> None:
        ...
        self._log = open(log_path, "a", encoding="utf-8")  # noqa: SIM115 - closed in close()
        env = {"MOOR_DASHBOARD_SESSION_TOKEN": self.token, "MOOR_DESKTOP": "1", **(extra_env or {})}
        self.proc = subprocess.Popen(
        ...
        self.port = poll(lambda: port_box[0] if port_box else (self.proc.poll() is not None and -1), 120,
                         "moor serve to report its port")
        assert self.port > 0, f"moor serve exited rc={self.proc.returncode}"
