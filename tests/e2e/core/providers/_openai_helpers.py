...
Every test drives REAL Moor processes (``moor -z`` oneshot, ``--resume``, the
``tui_gateway`` stdio server) against a loopback fake of the vendor HTTP API
(``tests/fakes/providers``) and asserts on the next wire request Moor sends, the
user-visible answer, and persisted ``state.db`` rows. Homes are hermetic: a fake HOME
whose ``.moor`` is the MOOR_HOME, an allowlisted env with no credentials.
"""
...
# One tool Moor always offers on the CLI toolset and that has an observable,
...
class Home:
    ...
    def env(self, extra: dict[str, str] | None = None) -> dict[str, str]:
        """Allowlisted env: the runner may itself be a Moor process whose MOOR_* or
        credential env would silently reroute the child."""
        import pwd  # the suite is Linux-gated

        real_root = Path(pwd.getpwuid(os.getuid()).pw_dir, ".moor").resolve()  # windows-footgun: ok — every file here is skipif(not linux)
    ...
    def write(self, config: dict[str, Any], dotenv: dict[str, str] | None = None,
        ...
        (self.moor_home / ".env").write_text(
            "".join(f"{k}={v}\n" for k, v in (dotenv or {}).items()), encoding="utf-8")
        if auth is not None:
            (self.moor_home / "auth.json").write_text(json.dumps(auth), encoding="utf-8")
        return self
...
def oneshot(h: Home, prompt: str, *args: str, resume: str | None = None, timeout: float = TURN_TIMEOUT,
    ...
    """One real ``moor -z`` turn; the usage file names the session for ``--resume``."""
    usage_file = h.root / f"usage-{time.monotonic_ns()}.json"
    argv = ["-z", prompt, "--usage-file", str(usage_file), *args]
    if resume:
        argv += ["--resume", resume]
    proc = subprocess.run(moor_argv(*argv), cwd=h.project, env=h.env(env), capture_output=True,
