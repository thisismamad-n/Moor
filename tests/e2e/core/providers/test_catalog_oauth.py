...
Every cell drives the real ``python -m moor_cli.main`` in a hermetic HOME (fake HOME, MOOR_HOME
...
from tests.fakes.providers.catalog_oauth import MOOR_INVOKE_SCOPE, OAuthFake, make_jwt

pytestmark = pytest.mark.skipif(sys.platform == "win32", reason="POSIX harness")

REPO_ROOT = Path(__file__).resolve().parents[4]
PROD_MOOR_INFERENCE = "https://inference-api.nousresearch.com/v1"
TURN_TIMEOUT = 120.0
...
KNOWN: dict[str, Known] = {
    "device_interval": Known(
        ...
        "#121163 Moor device-code login polls at 1s, ignoring the server's interval"),
    "moor_401_retry_route": Known(
        r"^401 recovery retry left MOOR_INFERENCE_BASE_URL: egress to \[[^\]]*'inference-api\.nousresearch\.com:443'",
        "#121323 Moor 401 pool recovery retries on the stored production host, "
        "dropping the MOOR_INFERENCE_BASE_URL override"),
}
...
class Home:
    ...
    def env(self, extra: dict[str, str] | None = None) -> dict[str, str]:
        ...
        env.update({
            ...
            "TMPDIR": str(self.home), "MOOR_SHARED_AUTH_DIR": str(self.home / "shared"),
    ...
    def run(self, argv: list[str], extra_env: dict[str, str] | None = None,
        ...
        return subprocess.run(
            [sys.executable, "-m", "moor_cli.main", *argv], cwd=str(self.home), env=self.env(extra_env),
...
def _moor_state(fake: OAuthFake, access: str, refresh: str, ttl_s: int) -> dict[str, Any]:
    return {"access_token": access, "refresh_token": refresh, "token_type": "Bearer",
            "scope": MOOR_INVOKE_SCOPE, "client_id": "moor-cli", "portal_base_url": fake.origin,
            "inference_base_url": PROD_MOOR_INFERENCE, "obtained_at": _iso(-60), "expires_at": _iso(ttl_s),
            "agent_key": access, "agent_key_expires_at": _iso(ttl_s),
            "tls": {"insecure": False, "ca_bundle": None}}


OTHER_MOOR_ROW = "moor-manual-2"
...
def _moor_store(fake: OAuthFake, access: str, refresh: str, ttl_s: int) -> dict[str, Any]:
    state = _moor_state(fake, access, refresh, ttl_s)
    other_access = make_jwt("other-row", ttl_s=7200)
    other = {**_moor_state(fake, other_access, "rt-other-row-untouched", 7200), "id": OTHER_MOOR_ROW,
             "label": "second-login", "auth_type": "oauth", "priority": 1, "source": "manual:device_code"}
    return {"version": 1, "active_provider": "moor", "providers": {"moor": state},
            "credential_pool": {
                "moor": [{**state, "id": "moor-dc-1", "label": "seed", "auth_type": "oauth", "priority": 0,
...
def _assert_untouched(before: dict[str, Any], after: dict[str, Any]) -> None:
    for provider, row_id in (("moor", OTHER_MOOR_ROW), ("openrouter", "or-1")):
...
def device_login(tmp_path_factory):
    """One real ``moor auth add moor --type oauth`` device-code login against the fake Portal."""
    root = tmp_path_factory.mktemp("moor-device")
    with CatalogFake() as sentinel, OAuthFake(device_interval=DEVICE_INTERVAL, poll_script=DEVICE_SCRIPT) as fake:
        home = Home(root, sentinel, "moor", "oauth-e2e/model")
        seed = {"version": 1, "credential_pool": {"openrouter": [OPENROUTER_ROW]}}
        home.seed_auth(seed)
        proc = home.run(["auth", "add", "moor", "--type", "oauth", "--no-browser", "--portal-url", fake.origin],
                        extra_env={"MOOR_INFERENCE_BASE_URL": f"{fake.origin}/v1"}, timeout=90)
...
def test_moor_device_login_persists_tokens(device_login) -> None:
    ...
    store = home.auth()
    state = store.get("providers", {}).get("moor") or {}
    assert state.get("refresh_token") == issued["refresh_token"], (
        f"providers.moor did not persist the login's refresh token: {state.get('refresh_token')!r}\n{info}")
    assert state.get("access_token") == issued["access_token"], f"providers.moor access token not persisted\n{info}"
    pooled = [r.get("refresh_token") for r in store.get("credential_pool", {}).get("moor", [])]
    assert issued["refresh_token"] in pooled, f"credential_pool.moor missing the login: {pooled}\n{info}"
...
def test_moor_device_login_slow_down_grows_interval(device_login) -> None:
...
def test_moor_device_login_honors_server_interval(device_login) -> None:
    gaps = device_login["gaps"]
    assert len(gaps) == len(DEVICE_SCRIPT), f"unexpected poll count, gaps={gaps}"
...
def _moor_turn(tmp_path: Path, sentinel: CatalogFake, fake: OAuthFake, access: str, ttl_s: int):
    home = Home(tmp_path, sentinel, "moor", "oauth-e2e/model", base_url=f"{fake.origin}/v1")
    seed = _moor_store(fake, access, "rt-seed-0", ttl_s)
    home.seed_auth(seed)
    proc = home.run(["-z", "Say hi", "--provider", "moor", "-m", "oauth-e2e/model"],
                    extra_env={"MOOR_INFERENCE_BASE_URL": f"{fake.origin}/v1"})
...
def _assert_rotation_persisted(home: Home, seed: dict[str, Any], fake: OAuthFake, info: str) -> str:
    ...
    assert refreshes[0].headers.get("x-moor-refresh-token") == "rt-seed-0", (
        f"refresh redeemed the wrong token: {refreshes[0].headers.get('x-moor-refresh-token')!r}\n{info}")
    rotated = fake.issued[-1]
    store = home.auth()
    assert store["providers"]["moor"].get("refresh_token") == rotated["refresh_token"], (
        f"providers.moor kept a spent refresh token {store['providers']['moor'].get('refresh_token')!r} "
        f"instead of the rotation {rotated['refresh_token']!r}\n{info}")
    assert _row(store, "moor", "moor-dc-1").get("refresh_token") == rotated["refresh_token"], (
        f"credential_pool.moor[moor-dc-1] kept a spent refresh token "
        f"{_row(store, 'moor', 'moor-dc-1').get('refresh_token')!r}\n{info}")
    _assert_untouched(seed, store)
    return rotated["access_token"]


def test_moor_expired_access_token_refreshes_before_turn(tmp_path, sentinel) -> None:
    stale = make_jwt("expired", ttl_s=-60)
    with OAuthFake(valid_refresh={"rt-seed-0"}, revoked={stale}) as fake:
        home, seed, proc = _moor_turn(tmp_path, sentinel, fake, stale, ttl_s=-60)
...
def test_moor_inference_401_refreshes_rotates_and_retries(tmp_path, sentinel) -> None:
    revoked = make_jwt("revoked-by-server", ttl_s=7200)
    with OAuthFake(valid_refresh={"rt-seed-0"}, revoked={revoked}) as fake:
        home, seed, proc = _moor_turn(tmp_path, sentinel, fake, revoked, ttl_s=7200)
        info = _describe(proc, fake, sentinel)
        assert any(r.bearer == revoked for r in fake.inference()), f"the stale bearer was never tried\n{info}"
        fresh = _assert_rotation_persisted(home, seed, fake, info)
        with known_gate(KNOWN["moor_401_retry_route"]):
            gate(not _vendor_egress(sentinel),
                 f"401 recovery retry left MOOR_INFERENCE_BASE_URL: egress to {_vendor_egress(sentinel)}\n{info}")
