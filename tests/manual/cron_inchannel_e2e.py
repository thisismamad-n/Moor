...
Uses a throwaway MOOR_HOME so it never touches ~/.moor. No real names.
"""
...
def _fresh_home():
    """Point MOOR_HOME at a throwaway dir BEFORE importing gateway modules
    (mirror.py binds _SESSIONS_INDEX from get_moor_home() at import time)."""
    d = tempfile.mkdtemp(prefix="cron_inchannel_e2e_")
    os.environ["MOOR_HOME"] = d
    return Path(d)


HOME = _fresh_home()

# Import AFTER MOOR_HOME is set.
...
# a different get_moor_home() at import if something imported it earlier).
...
def main():
    ...
    print(f"MOOR_HOME (throwaway): {HOME}")
