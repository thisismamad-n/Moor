"""display_moor_home() renders POSIX separators on every platform.

Maintainer catch (#95681 arc): on Windows with a custom MOOR_HOME under
the user profile (e.g. AppData/Local/moor), ``"~/" +
str(home.relative_to(Path.home()))`` produced the mixed-separator chimera
``~/AppData\\Local\\moor`` — which then leaked into every consumer that
appends sub-paths (the skill_manage schema showed the agent
``~/AppData\\Local\\moor/skills/``). The ``~/`` shorthand implies POSIX
...
class TestDisplayMoorHomePosix(unittest.TestCase):
    def test_nested_home_renders_forward_slashes(self):
        ...
        nested = fake_userhome / "AppData" / "Local" / "moor"
        with patch.object(hc, "get_moor_home", return_value=nested):
            out = hc.display_moor_home()
        self.assertEqual(out, "~/AppData/Local/moor")
        self.assertNotIn("\\", out)

    def test_default_home_unchanged(self):
        import moor_constants as hc

        with patch.object(hc, "get_moor_home",
                          return_value=Path.home() / ".moor"):
            out = hc.display_moor_home()
        self.assertEqual(out, "~/.moor")

    def test_outside_home_falls_back_to_absolute(self):
        import moor_constants as hc

        outside = Path("/opt/moor-custom") if os.name != "nt" else Path("C:/opt/moor-custom")
        with patch.object(hc, "get_moor_home", return_value=outside):
            out = hc.display_moor_home()
