"""Shared fixtures for the moor-agent test suite.
...
2. **Isolated MOOR_HOME.** MOOR_HOME points to a per-test tempdir so
   code reading ``~/.moor/*`` via ``get_moor_home()`` can't see the
   real one. (We do NOT also redirect HOME — that broke subprocesses in
   CI. Code using ``Path.home() / ".moor"`` instead of the canonical
   ``get_moor_home()`` is a bug to fix at the callsite.)
...
# ── Sandbox MOOR_HOME before ANY test module is imported ──────────────────
# `moor_cli/main.py` calls `setup_logging()` at MODULE level, which resolves
# `get_moor_home()` and attaches rotating file handlers to the ROOT logger.
...
# real `~/.moor/logs/agent.log` and `errors.log`.
#
# The `_isolate_env` fixture below also sandboxes MOOR_HOME, but fixtures run
...
# the REAL Moor root — capture it BEFORE the sandbox rewires MOOR_HOME,
# otherwise the deny-list would point at the throwaway tempdir and the guard
# would silently stop protecting the operator's actual ~/.moor (#69385).
_PRE_SANDBOX_KANBAN_OVERRIDE = os.environ.get("MOOR_KANBAN_HOME", "").strip()
_PRE_SANDBOX_MOOR_HOME = os.environ.get("MOOR_HOME", "")


def _moor_home_points_at_production(value: str) -> bool:
    """True when a pre-set MOOR_HOME resolves to the real production root.

    Gateway-launched shells (and developer shells that ``export
    MOOR_HOME=~/.moor``) hand pytest the PRODUCTION home. Historically
    the session sandbox below honored any pre-set value, so collection-time
    imports (logging handlers, ``moor_state.DEFAULT_DB_PATH``) froze paths
    inside the real ``~/.moor`` — the escape vector that landed pytest
    ...
    MOOR_HOME is honored now.
    """
    if not value:
        return True
    try:
        resolved = Path(value).expanduser().resolve()
        real_root = (Path.home() / ".moor").resolve()
    except Exception:
        return True
...
# like a real CLI") — which used to disarm moor_state's live-DB guard in
# the child at the same moment the child lost the MOOR_HOME redirect.
# MOOR_TEST_ISOLATION is OUR marker: exported here (before any test module
# imports), inherited by every child by default, and honored by
# moor_state_guard._running_under_pytest() as a test-context signal. A child
...
# open a real DB must export MOOR_STATE_DB_GUARD_BYPASS=1 in that child's
# env instead of stripping markers.
os.environ["MOOR_TEST_ISOLATION"] = os.environ.get("MOOR_HOME", "") or "1"
...
MOOR_HOME_AT_CONFTEST_IMPORT = os.environ.get("MOOR_HOME", "")
...
# MOOR_* vars that change test behavior by being set. Unset all of these
# unconditionally — individual tests that need them set do so explicitly.
_MOOR_BEHAVIORAL_VARS = frozenset({
    ...
    # final response text to ``moor_cli.voice.speak_text`` — real synthesis,
    ...
    "MOOR_SESSION_CHAT_ID",
    "MOOR_SESSION_CHAT_NAME",
    "MOOR_SESSION_CHAT_TYPE",
    "MOOR_SESSION_THREAD_ID",
    "MOOR_SESSION_SOURCE",
    "MOOR_SESSION_KEY",
    ...
    "MOOR_PLATFORM",
    "MOOR_MODEL",
    "MOOR_INFERENCE_MODEL",
    "MOOR_INFERENCE_PROVIDER",
    "MOOR_TUI_PROVIDER",
    "MOOR_MANAGED",
    "MOOR_MANAGED_DIR",
    "MOOR_DEV",
    ...
    "MOOR_TIMEZONE",
    "MOOR_REDACT_SECRETS",
    "MOOR_BACKGROUND_NOTIFICATIONS",
    "MOOR_EXEC_ASK",
    "MOOR_HOME_MODE",
    "MOOR_AGENT_USE_LEGACY_SESSION_KEYS",
    # Kanban path/board pins must never leak from a developer shell or
    # dispatched worker into tests; otherwise tests can write fake tasks to
    # the real ~/.moor/kanban.db instead of the per-test MOOR_HOME.
    "MOOR_KANBAN_DB",
    "MOOR_KANBAN_BOARD",
    "MOOR_KANBAN_HOME",
    "MOOR_KANBAN_WORKSPACES_ROOT",
    "MOOR_KANBAN_LOGS_ROOT",
    "MOOR_KANBAN_TASK",
    "MOOR_KANBAN_WORKSPACE",
    "MOOR_KANBAN_RUN_ID",
    "MOOR_KANBAN_CLAIM_LOCK",
    "MOOR_KANBAN_DISPATCH_IN_GATEWAY",
    ...
    # otherwise-unrelated config tests away from the default "moor" host.
    "MOOR_HONCHO_HOST",
    # Dashboard OAuth auth gate (PR #30156). When set, the bundled
    # dashboard-auth `moor` plugin auto-registers itself on plugin discovery,
    ...
    "MOOR_DASHBOARD_OAUTH_CLIENT_ID",
    "MOOR_DASHBOARD_PORTAL_URL",
    "TERMINAL_CWD",
    "TERMINAL_ENV",
...
def _hermetic_environment(tmp_path, monkeypatch):
    ...
    Also redirects HOME and MOOR_HOME to per-test tempdirs so code that
    reads ``~/.moor/*`` can't touch the real one, and pins TZ/LANG so
    ...
    # 2. Blank behavioral MOOR_* vars that could change test semantics.
    for name in _MOOR_BEHAVIORAL_VARS:
        monkeypatch.delenv(name, raising=False)
    ...
    # 3. Redirect MOOR_HOME to a per-test tempdir. Code that reads
    #    ``~/.moor/*`` via ``get_moor_home()`` now gets the tempdir.
    #
    #    NOTE: We do NOT also redirect HOME. Doing so broke CI because
    #    some tests (and their transitive deps) spawn subprocesses that
    #    inherit HOME and expect it to be stable. If a test genuinely
    #    needs HOME isolated, it should set it explicitly in its own
    #    fixture. Any code in the codebase reading ``~/.moor/*`` via
    #    ``Path.home() / ".moor"`` instead of ``get_moor_home()``
    ...
    monkeypatch.setenv("MOOR_TEST_ISOLATION", str(fake_moor_home))
    # And never let a developer-shell (or leaked child) bypass disarm the
    # guard for in-process code under test.
    monkeypatch.delenv("MOOR_STATE_DB_GUARD_BYPASS", raising=False)

    # 3b. moor_state computes ``DEFAULT_DB_PATH = get_moor_home() / "state.db"``
    #     at import time. When the module is first imported at collection (any
    #     test file with a top-level ``from moor_state import ...``) that
    ...
    try:
        import moor_cli.plugins as _plugins_mod
        ...
        # previous test's tmp_path MOOR_HOME can't leak forward. Paths
...
@pytest.fixture(autouse=True)
def _isolate_moor_home(_hermetic_environment):
...
def _neutralize_kanban_memory_guard(request, monkeypatch):
    ...
    if request.node.get_closest_marker("real_memory_guard"):
        return
    try:
        from moor_cli import kanban_db_dispatch as _kbd_mod
    except Exception:
        return
...
# invocation), kanban writes silently pollute the real ~/.moor. This autouse
...
# allow-list because test-level fixtures legitimately move MOOR_HOME to
...
def _capture_real_kanban_root() -> Path:
    ...
    file (before the session MOOR_HOME sandbox rewired the env), so the
    deny-list keeps pointing at the operator's actual root. Mirrors
    ``kanban_db.kanban_home()`` resolution order:
    1. ``MOOR_KANBAN_HOME`` env var when set and non-empty
    ...
    if _PRE_SANDBOX_MOOR_HOME and not _moor_home_points_at_production(
        ...
        # MOOR_HOME was genuinely set to a CUSTOM root before the sandbox
        ...
        return get_default_moor_root().resolve()
    # No pre-existing MOOR_HOME: the real root is the platform default,
    # NOT the sandbox tempdir now sitting in the env.
    return (Path.home() / ".moor").resolve()
...
def _kanban_write_guard(_hermetic_environment, monkeypatch):
    ...
    ``~/.moor`` captured at import time. Hermetic tests that legitimately
    move MOOR_HOME to sibling tempdirs are unaffected.

    Only patches when ``moor_cli.kanban_db_connect`` is *already imported*
    ...
    """
    _kdb = sys.modules.get("moor_cli.kanban_db")
    _kdbc = sys.modules.get("moor_cli.kanban_db_connect")
    if _kdb is None or _kdbc is None:
        return

    # The sys.modules probe can observe the module MID-IMPORT: a fixture
    # boundary firing while another test's lazy `import moor_cli.kanban_db`
...
# ``moor_state._ensure_test_isolation`` (the single choke point every
...
#     installs where MOOR_HOME is not ~/.moor) into the guard's
...
# so subprocess children that import moor_state directly are covered even
# without this fixture.


@pytest.fixture(autouse=True)
def _state_db_write_guard(request, monkeypatch):
    _hs = sys.modules.get("moor_state")
    ...
    if _PRE_SANDBOX_MOOR_HOME and not _moor_home_points_at_production(
        ...
        extra_roots.append(
            Path(_PRE_SANDBOX_MOOR_HOME).expanduser().resolve()
        )
    monkeypatch.setattr(
...
def _reset_tui_gateway_server_state():
    ...
    # ``get_moor_home()`` call (active-session registry, config paths)
    # to a stale per-test tmpdir. Force the main-thread ContextVar back
    # to its default.
    try:
        from moor_constants import get_moor_home_override, set_moor_home_override

        if get_moor_home_override() is not None:
            set_moor_home_override(None)
    except Exception:
        pass
...
def _wal_is_usable() -> bool:
    ...
    IMPORTANT: this must NOT import ``moor_state``. That module computes
    ``DEFAULT_DB_PATH`` from ``get_moor_home()`` at import time, so importing
    it during collection — before the per-test ``_isolate_moor_home`` fixture
    redirects ``MOOR_HOME`` — permanently caches the DEVELOPER'S REAL
    ``~/.moor/state.db`` for the whole session. Tests then read live
    production sessions instead of a tempdir. The version predicate is
    duplicated from ``moor_state._is_sqlite_wal_reset_vulnerable`` (upstream
...
#      process environment: ``os.environ["MOOR_VOICE_TTS"] = "1"``. The
...
#      ``moor_cli.voice.speak_text(final_response)`` on a daemon thread.
...
# to mock it:
#
#  • ``moor_cli.voice.speak_text`` — the synth+playback entry point both
#    gateway call sites late-import, so patching the module attribute catches
#    them wherever they import it from.
#  • ``moor_cli.voice.play_audio_file`` — the module-level binding
#    ``speak_text`` actually plays through. Patching the binding inside
#    ``moor_cli.voice`` (not ``tools.voice_mode``) keeps the real function
...
def _live_system_guard(request, monkeypatch):
    ...
    targeting moor/python patterns are also blocked.
    """
...
def _audio_playback_guard(request, monkeypatch):
    ...
    depth behind ``_MOOR_BEHAVIORAL_VARS``: the env blanking stops the flag
    ...
    try:
        import moor_cli.voice as _voice
    except Exception:
