"""Tests for moor_state.py — SessionDB SQLite CRUD, FTS5 search, export."""
...
from moor_state_common import FTS_SQL, FTS_STORAGE_VERSION, SCHEMA_SQL, SCHEMA_VERSION
...
class TestConnectionLifecycle:
    ...
    def test_failed_writable_open_does_not_leak_tracked_connection(
        ...
        monkeypatch.setattr(moor_state, "_connect_tracked_db", capture_connect)
        monkeypatch.setattr(
            SessionDB,
            "_init_schema",
    ...
    def test_failed_wal_read_open_does_not_leak_tracked_connection(
        ...
        from moor_cli import sqlite_safe_read

        db_path = tmp_path / "state.db"
        db = SessionDB(db_path=db_path)
        opened = []
        real_connect = moor_state._connect_tracked_db
        real_pragmas = moor_state.apply_database_pragmas
        ...
        monkeypatch.setattr(moor_state, "_connect_tracked_db", capture_connect)
        monkeypatch.setattr(moor_state, "apply_database_pragmas", fail_pragmas)
        ...
        try:
            ...
            monkeypatch.setattr(
                moor_state, "apply_database_pragmas", real_pragmas
            )
            for conn in opened:
                try:
                    conn.close()
    ...
    def test_read_only_open_exhausts_retry_budget_for_persistent_ioerr(
        ...
        monkeypatch.setattr(moor_state, "_connect_tracked_db", bad_connect)
        monkeypatch.setattr(moor_state, "_READ_ONLY_IOERR_RETRY_BACKOFF_S", 0.0)
        budget = moor_state._READ_ONLY_IOERR_RETRY_ATTEMPTS
...
class TestSessionLifecycle:
    ...
    def test_cjk_search_falls_back_to_like_when_trigram_unavailable(
        ...
        monkeypatch.setattr("moor_state.sqlite3.connect", connect_without_trigram)
        db = SessionDB(db_path=db_path)
        try:
            db.create_session(session_id="s1", source="cli")
...
class TestSessionTitleLineage:
    ...
    def test_projected_tip_inherits_root_title_when_untitled(self, db):
        ...
        `moor peer dm` -> canonical "Bot Chat", #106165). A titled tip keeps its own title."""
        import time as _time
        self._make_compression_chain(db, _time.time() - 3600)
...
class TestFtsRebuildLoopWithoutTrigram:
    ...
    interrupted ``moor sessions optimize-storage`` silently lost its place
    ...
    def _trace(monkeypatch, statements, *, trigram):
        ...
        monkeypatch.setattr("moor_state.sqlite3.connect", connect)
    ...
    def test_fts_trigger_subsets_match_the_ddl(self):
        ...
        from moor_state_schema import _FTS_BASE_TRIGGERS, _FTS_TRIGRAM_TRIGGERS
...
class TestListSessionsRich:
    ...
    def test_rich_list_keeps_legacy_reset_children_visible(self, db, end_reason):
        from moor_state_common import _ephemeral_child_sql
    ...
    # tests/moor_state/test_resolve_resume_session_id.py
...
class TestVacuum:
    ...
    def test_auto_maintenance_freelist_ratio_exactly_at_threshold_skips(self, db, monkeypatch):
        ...
        from moor_state_common import AUTO_VACUUM_MIN_FREELIST_RATIO
...
class TestFTS5ToolCallMigration:
    ...
    def test_v10_to_v11_upgrade_backfills_tool_fields(self, tmp_path):
        ...
        try:
            ...
            # `moor db optimize` performs the v23 transition; afterwards the
...
class TestFTSExternalContentMigration:
    ...
    def _simulate_pre_fix_demote_crash_window(self, db):
        ...
        """
        from moor_state_common import FTS_SQL, FTS_TRIGRAM_SQL
    ...
    def test_optimize_resume_after_demote_crash_window_restores_search(
        ...
        try:
            ...
            assert db.get_meta("fts_storage_version") == str(
                moor_state_common.FTS_STORAGE_VERSION
            )
            assert db._conn.execute(
    ...
    def test_optimize_heals_premature_stamp_with_empty_index(self, tmp_path):
        ...
        try:
            ...
            db._conn.execute(
                ...
                "('fts_storage_version', ?) "
                "ON CONFLICT(key) DO UPDATE SET value = excluded.value",
                (str(moor_state_common.FTS_STORAGE_VERSION),),
            )
            db._conn.commit()
...
def test_refresh_compression_lock_requires_holder_and_preserves_reclaimability(db, monkeypatch):
    ...
    monkeypatch.setattr(moor_state.time, "time", lambda: 1000.0)
    ...
    monkeypatch.setattr(moor_state.time, "time", lambda: 1005.0)
    ...
    monkeypatch.setattr(moor_state.time, "time", lambda: 1016.0)
...
def test_refresh_cannot_resurrect_a_lock_already_reclaimed(db, monkeypatch):
    ...
    monkeypatch.setattr(moor_state.time, "time", lambda: 1020.0)
    assert db.try_acquire_compression_lock("s1", "holder-b", ttl_seconds=10.0) is True
...
class TestGetMessagesPagination:
    ...
    def test_resume_safety_counts_active_rows_across_lineage(self, db):
        ...
        with pytest.raises(moor_state.SessionResumeTooLargeError) as exc_info:
    ...
    def test_resume_safety_tip_only_counts_the_tip_segment(self, db):
        ...
        with pytest.raises(moor_state.SessionResumeTooLargeError) as full:
        ...
        with pytest.raises(moor_state.SessionResumeTooLargeError) as tip:
    ...
    def test_export_safety_is_bounded_to_the_requested_active_segment(self, db):
        ...
        with pytest.raises(moor_state.SessionExportTooLargeError) as exc_info:
    ...
    def test_zero_limit_disables_resume_and_export_guards(self, db, monkeypatch):
        ...
        with pytest.raises(moor_state.SessionResumeTooLargeError):
            db.assert_resume_safe("big", max_messages=2)
        with pytest.raises(moor_state.SessionExportTooLargeError):
        ...
        # effect only).
        monkeypatch.setattr(moor_state, "resolved_max_resume_messages", lambda: 0)
        monkeypatch.setattr(moor_state, "resolved_max_export_messages", lambda: 0)
    ...
    def test_guard_limits_resolve_from_config_at_call_time(self, db, monkeypatch):
        ...
        monkeypatch.setattr(moor_state, "resolved_max_resume_messages", lambda: 3)
        monkeypatch.setattr(moor_state, "resolved_max_export_messages", lambda: 3)
        with pytest.raises(moor_state.SessionResumeTooLargeError) as resume_exc:
            db.assert_resume_safe("cfg")
        assert resume_exc.value.limit == 3
        with pytest.raises(moor_state.SessionExportTooLargeError) as export_exc:
...
class TestGatewayRoutingPkHeal:
    ...
    def test_legacy_pk_rebuilt_to_composite(self, tmp_path):
        db_path = self._make_legacy_db(
            tmp_path, rows=[("/home/u/.moor/sessions", "agent:main:telegram:dm:1", "{}", 1.0)]
        )
        db = SessionDB(db_path=db_path)
        try:
            assert self._pk_cols(db) == ["scope", "session_key"]
            # Existing rows survive the rebuild.
            entries = db.load_gateway_routing_entries(scope="/home/u/.moor/sessions")
...
class TestFtsRebuildFinishWithoutTrigram:
    ...
    def test_rebuild_finish_skips_trigram_when_unavailable(
        ...
        monkeypatch.setattr(
            "moor_state.sqlite3.connect", connect_without_trigram
        )
        db = SessionDB(db_path=db_path)
        try:
            assert db._trigram_available is False
