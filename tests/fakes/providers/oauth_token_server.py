...
flows Moor drives but does not own:
...
Both the Anthropic (``/v1/oauth/token``) and Moor Portal (``/api/oauth/token``)
...
def make_test_ca(directory: Path, hosts: Iterable[str]) -> TestCA:
    ...
    ca_name = x509.Name([x509.NameAttribute(NameOID.COMMON_NAME, "moor e2e test CA")])
    ca_cert = (
