...
binary download are exercised manually by `moor secrets bitwarden
...
def moor_home(tmp_path, monkeypatch):
    """Point Moor at an isolated home directory."""
    home = tmp_path / ".moor"
    home.mkdir()
    monkeypatch.setenv("MOOR_HOME", str(home))
    # Some modules cache get_moor_home; clear if needed.
    import moor_constants
    if hasattr(moor_constants, "_MOOR_HOME_CACHE"):
        moor_constants._MOOR_HOME_CACHE = None  # type: ignore[attr-defined]
    return home
...
def test_install_bws_happy_path(moor_home, monkeypatch):
...
def test_env_loader_calls_bsm_when_enabled(tmp_path, monkeypatch):
    ...
    from moor_cli.env_loader import _apply_external_secret_sources
