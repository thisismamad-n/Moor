...
import moor_state
from moor_state import SessionDB, preflight_db_writability
...
def moor_home(tmp_path, monkeypatch):
    """Isolated MOOR_HOME so the repair scope covers tmp DBs."""
    home = tmp_path / ".moor"
    home.mkdir()
...
class TestRepairScope:
    def test_repairs_readonly_db_inside_home(self, moor_home):
    ...
    def test_repairs_readonly_sidecars(self, moor_home):
        db = moor_home / "state.db"
        _make_wal_db(db)
    ...
    def test_repairs_readonly_parent_directory(self, moor_home):
        sub = moor_home / "kanban"
        sub.mkdir()
        db = sub / "kanban.db"
        _make_db(db)
        os.chmod(sub, 0o555)
...
class TestRefusalOutsideScope:
    def test_actionable_error_names_file_and_chmod(self, moor_home, tmp_path):
    ...
    def test_wal_error_warns_against_deletion(self, moor_home, tmp_path):
        outside = tmp_path / "elsewhere"
        outside.mkdir()
        db = outside / "custom.db"
        _make_wal_db(db)
...
class TestSkips:
    ...
    def test_healthy_db_untouched(self, moor_home):
        db = moor_home / "state.db"
        _make_db(db)
...
class TestSessionDBIntegration:
    def test_sessiondb_selfheals_readonly_db_in_home(self, moor_home):
