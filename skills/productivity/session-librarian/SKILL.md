...
| List/filter by metadata (age, source, cost, tokens, workspace) | `moor sessions list` / `stats` via terminal |
| Rename | `moor sessions rename <session_id> <title...>` |
| Bulk soft-hide (reversible) | `moor sessions archive <filters>` |
| Delete (destructive) | `moor sessions delete` / `moor sessions prune <filters>` |
| Export before deleting anything valuable | `moor sessions export --session-id <id> --format md` |
...
`moor sessions list --source telegram --limit 50` instead.
...
- Before deleting anything with meaningful content, offer
  `moor sessions export --format md` as a backup.
...
After a cleanup pass, re-run the discovery query and `moor sessions list`
