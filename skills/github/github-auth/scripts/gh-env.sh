#!/usr/bin/env bash
# GitHub environment detection helper for Moor Agent skills.
#
...
if command -v gh &>/dev/null && gh auth status &>/dev/null 2>&1; then
    ...
    GH_AUTH_METHOD="curl"
elif _moor_env="${MOOR_HOME:-$HOME/.moor}/.env"; [ -f "$_moor_env" ] && grep -q "^GITHUB_TOKEN=" "$_moor_env" 2>/dev/null; then
