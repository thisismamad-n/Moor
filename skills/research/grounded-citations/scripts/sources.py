...
Ledger path resolution (first wins):
  --ledger PATH
  $MOOR_CITATION_LEDGER
  $MOOR_HOME/cache/citations/ledger.json
"""
...
from _moor_home import get_moor_home  # noqa: E402
...
def resolve_ledger_path(explicit: str | None = None) -> Path:
    ...
    env = os.environ.get("MOOR_CITATION_LEDGER", "").strip()
    if env:
        return Path(env).expanduser()
    return get_moor_home() / "cache" / "citations" / "ledger.json"
