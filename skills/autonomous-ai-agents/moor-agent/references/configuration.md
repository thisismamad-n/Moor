...
`moor config check` reports sections missing from an older config.

### Toolsets

Enable/disable via `moor tools` (interactive) or `moor tools enable/disable NAME`.
Full enumeration: `TOOLSETS` dict in `toolsets.py` (`_MOOR_CORE_TOOLS` is the default bundle most platforms inherit).
