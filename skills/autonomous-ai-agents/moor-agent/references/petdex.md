...
the Moor CLI, TUI, and desktop app. This skill drives the `moor pets` CLI
...
Use the `terminal` tool to run `moor pets <subcommand>`.

## Quick Reference

| Goal | Command |
| --- | --- |
| Browse the gallery | `moor pets list` (add a substring to filter: `moor pets list cat`) |
| List installed pets | `moor pets list --installed` |
| Install a pet | `moor pets install <slug>` (add `--select` to make it active) |
...
| Preview/animate in terminal | `moor pets show [slug] [--cycle] [--state run]` |
| Disable the pet | `moor pets off` |
| Remove a pet | `moor pets remove <slug>` |
| Diagnose setup | `moor pets doctor` |

## Procedure

1. Find a pet: `moor pets list <query>` and note its `slug`.
2. Install + activate: `moor pets install <slug> --select`.
3. Preview it: `moor pets show` (Ctrl+C to stop).
4. Confirm setup: `moor pets doctor` — shows the resolved pet, configured
   render mode, detected terminal graphics protocol, and effective mode.

Pets install into `<MOOR_HOME>/pets/<slug>/` (profile-aware). Selecting a pet
...
- `scale` (float) — on-screen size of the native 192×208 frames (default 0.33,
  ...
  `moor pets scale <factor>`, the `/pet scale` slash command, or the desktop
  Appearance slider.
...
- `moor pets doctor` reports `✓ ready` when a pet is installed, selected,
