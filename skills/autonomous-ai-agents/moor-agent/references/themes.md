...
desktop GUI at once. The skin engine (`moor_cli/skin_engine.py`) resolves the
active skin and the gateway pushes it to every surface, so a file dropped in
`~/.moor/skins/` is the theme analogue of a plugin: no code, all surfaces. This
...
- Write access to the Moor home dir — `~/.moor` by default, or `$MOOR_HOME`
  / the active profile's dir. Skins live in `<moor-home>/skins/`.
- Native tools: `write_file` (create the YAML), `read_file` / `search_files`
  (inspect existing skins), `terminal` (activate via `moor config set`).
...
3. `write_file` it to `<moor-home>/skins/<name>.yaml`.
...
4. **Confirm the new look landed** and tell the user how to revert: run
   `moor config set display.skin default` (or they can `/skin default`).
...
```
moor skin set <key> <hex>      # e.g. moor skin set ui_tool "#00FFFF"
```
...
palette and resets the background. `moor skin set background "#08201f"` changes
only the background; `moor skin use <name>` / `moor skin list` switch and
enumerate.

## Pitfalls

- **Don't hardcode `~/.moor`** when a profile is active — resolve the real home
  from `$MOOR_HOME` first, falling back to `~/.moor`.
...
- **Name collisions**: a skin named like a desktop built-in (`mono`, `slate`,
  `cyberpunk`, `moor`, `midnight`, `ember`) won't override that built-in on the
  GUI. Pick a fresh name.
- **Never hand-edit `config.yaml` to activate.** Use `moor config set
...
- **You apply it, not the user.** `moor config set display.skin <name>` is
...
- `read_file` the written `<moor-home>/skins/<name>.yaml` and confirm valid
  YAML with the intended `name` and `colors`.
- Run `moor config get display.skin` and confirm it reports `<name>`.
