...
Registry of record: `moor_cli/commands.py` (`COMMAND_REGISTRY`) — every
...
/snapshot [sub]          Create/restore Moor config+state snapshots (CLI)
...
/subscription (/upgrade) Moor plan info (CLI)
/topup                   Moor balance / billing
/copy [N]                Copy last response to clipboard (CLI)
/paste                   Attach clipboard image (CLI)
/image <path>            Attach a local image file (CLI)
/update                  Update Moor to latest
/version (/v)            Show version
/debug [moor|local]      Upload debug report, get shareable links
```
