...
- **CLI:** `moor curator <verb>` — `status`, `usage`, `run`, `pause`,
...
- **Cost:** the deterministic inactivity/prune sweep runs for free. The
  ...
  **off by default** — opt in with `curator.consolidate: true` or
  `moor curator run --consolidate`. Routine background curation costs
  zero tokens.
- **Telemetry:** sidecar at `~/.moor/skills/.usage.json` holds
...
see a focused `kanban_*` toolset gated by `MOOR_KANBAN_TASK`, and
...
- **Isolation:** board is the hard boundary (workers get
  `MOOR_KANBAN_BOARD` pinned in env); tenant is a soft namespace
