...
Author widget apps for the Moor TUI (`moor --tui`): glanceable ambient
...
- The TUI must be in use (`moor --tui`). Widgets do not render in the
...
1. Use `write_file` to create `~/.moor/tui-widgets/<name>.mjs` (see
...
- A thrown `register()` is logged and skipped; check
  `~/.moor/logs/tui_gateway_crash.log` if a widget never appears.
