...
moor config get delegation.max_concurrent_children

# 2. Did Moor' truncator or rejector actually fire?
grep -E "Truncated.*delegate_task|Too many tasks" ~/.moor/logs/agent.log | tail
# If neither line appears, neither cap path executed.

# 3. Confirm the resolver returns what config says (in venv with moor on path)
