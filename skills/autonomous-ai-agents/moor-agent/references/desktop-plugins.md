...
system (`~/.moor/plugins/`) is otherwise documented separately.

There are TWO on-disk doors, same contract and hot reload:

- `$MOOR_HOME/desktop-plugins/<id>/plugin.js` — standalone desktop plugin.
  Loads enabled by default.
- `$MOOR_HOME/plugins/<id>/desktop/plugin.js` — the desktop HALF of a
  ...
  `.moor-package.json` marker) — the ONLY root the renderer loads from — so
...
- Write access to `$MOOR_HOME/desktop-plugins/` (usually
  `~/.moor/desktop-plugins/`).

## How to Run

1. Create `$MOOR_HOME/desktop-plugins/<name>/plugin.js` from
   `templates/plugin.js` (in this skill directory) — that's
   `~/.moor/...` by default, or `~/.moor/profiles/<profile>/...` under a
...
The ONLY import surface is `@moor/plugin-sdk` (plus `react` /
...
- `ctx.os` — the curated OS door, attributed to your plugin:
  ...
  posts a native OS notification. Fires only while the user is away from Moor
  ...
  (`moor://index-network/intent/1`), a hash path (`/index-network/intent/1`),
...
- Backend: if the plugin ships a Python `plugin_api.py` (under
  `~/.moor/plugins/<id>/dashboard/`, manifest `"api": "plugin_api.py"`), reach
...
- Do not import anything except `@moor/plugin-sdk`, `react`, and
