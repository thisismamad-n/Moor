# Moor Portal — authenticating third-party apps against the subscription

Recurring user question: "Can app X (Karakeep, OpenWebUI, LibreChat, OpenViking,
LangChain pipeline, n8n flow, etc.) use my Moor Portal subscription without me
...
"Sign in with Moor Portal" SSO that third-party apps can register as clients
...
1. Read Moor's existing Portal credential out of `~/.moor/auth.json`.
...
Only when the consumer is *another first-party Moor thing* (a future SDK, a
Moor-branded extension, a Discord-bot integration that needs per-user
