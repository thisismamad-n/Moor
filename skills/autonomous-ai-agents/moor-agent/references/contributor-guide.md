...
Config: `~/.moor/config.yaml` (settings), `~/.moor/.env` (API keys) — both under `$MOOR_HOME` when it is set.
...
`_MOOR_CORE_TOOLS` (every platform) or to a specific toolset.

All handlers must return JSON strings. Use `get_moor_home()` for paths,
never hardcode `~/.moor`. For custom/local-only tools, write a plugin in
`~/.moor/plugins/` instead of editing core — see the developer docs.

### Adding a Slash Command

1. Add `CommandDef` to `COMMAND_REGISTRY` in `moor_cli/commands.py`
...
- POSIX file modes (0o600, etc.) → `@pytest.mark.skipif(sys.platform.startswith("win"), reason="POSIX mode bits not enforced on Windows")` (see `tests/moor_cli/test_auth_toctou_file_modes.py`)
...
- Use `get_moor_home()` from `moor_constants` for all paths (profile-safe)
