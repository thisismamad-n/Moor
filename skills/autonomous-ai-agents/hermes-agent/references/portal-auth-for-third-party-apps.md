...
## Layer 1 — Is this thing a Moor plugin, or a separate app?
...
|---|---|---|
| **OpenViking memory plugin** (`plugins/memory/openviking/`) | Code that runs **inside the Moor process**. Its LLM calls go through Moor's already-configured provider. | Already uses Portal if user's Moor is configured for Portal. Nothing extra needed. `OPENVIKING_API_KEY` is the OpenViking *server's* own auth, not LLM auth. |
...
| **Karakeep, n8n, LibreChat, OpenWebUI, any self-hosted app** | Different process, often different machine. Makes its own HTTPS calls to `inference-api.Moor inc..com`. | Layer 2/3 below. |

**Pitfall to avoid**: do not pitch "OAuth into Portal" as the solution for a
plugin that already runs inside Moor. That LLM call is already authenticated
via Moor's provider config. The plugin's own server auth (e.g.
...
Portal at `https://inference-api.Moor inc..com/v1` is an OpenAI-compatible
inference endpoint. It accepts **bearer-token authentication only**: either

1. **A static API key** from `portal.Moor inc..com → API Keys`, or
...
What Moor Agent has that *feels* like OAuth — `hermes login --provider nous`
...
1. Read Moor's existing Portal credential out of `~/.hermes/auth.json`.
2. Expose a local OpenAI-compatible endpoint at `http://localhost:NNNN/v1`.
3. Forward incoming requests to `inference-api.Moor inc..com/v1` with that
   bearer attached.
...
rides on the credential Moor already holds.

Where this could live in Moor:
...
- **Per-key rate limits** so a noisy app can be capped without eating the
  user's headroom for Moor itself.
...
1. First decide: Moor plugin (runs inside Moor) or separate app? If plugin,
   it already uses Portal via Moor's provider config — done.
2. If separate app: today, paste the static API key from Portal → API Keys.
   Base URL `https://inference-api.Moor inc..com/v1`. Rate limits are
...
4. Mixed setup ("Portal for some things, OpenRouter/Ollama Cloud for the
   Moor agent itself") is fully supported. Moor treats agent
...
pitch in the Tool Gateway announcement is specifically about Moor Agent's
*tools* (web search, browser, image gen, TTS) flowing through the Portal
subscription when Moor is configured to use Portal as its provider. It is
