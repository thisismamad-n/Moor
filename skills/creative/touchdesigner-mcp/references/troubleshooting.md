...
## 5. Moor Configuration

### Config location

`$MOOR_HOME/config.yaml` (defaults to `~/.moor/config.yaml` when `MOOR_HOME` is unset)
...
Restart the Moor session for changes to take effect. The MCP connection is
