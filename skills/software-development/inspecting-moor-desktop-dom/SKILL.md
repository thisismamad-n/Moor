---
name: inspecting-moor-desktop-dom
...
`debugging-moor-desktop`), or anything where the real question is "does this
look right".
...
- **no `MOOR_DESKTOP_DEV_SERVER`** — an unpackaged `electron .` against
  `dist/` is how the packaged app gets smoke tested, so it behaves like one.

`MOOR_DESKTOP_CDP_PORT` moves the port (`=9333`) or disables it (`=off`).

Check before doing anything else:

```bash
curl -s --max-time 3 http://127.0.0.1:${MOOR_DESKTOP_CDP_PORT:-9222}/json/version
```
...
cannot collide with a running `hgui`; the separate `MOOR_HOME` keeps it away
from real sessions. Pick a port other than 9222 for the same reason. Run it in
the background and kill it when done.

`npm run perf:serve` does the same with a temp `MOOR_HOME` baked in, if you
...
- **A throwaway `MOOR_HOME` has no backend.** The app logs `ECONNREFUSED` for
  `moor:api` and may exit on its own. The renderer still mounts and the DOM is
