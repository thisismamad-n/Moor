...
> **Docker HOME pitfall:** In the official Moor Docker layout, `/opt/data` is `MOOR_HOME`, but Moor tool subprocesses use `/opt/data/home` as `HOME`. That means `~/.xurl` resolves to `/opt/data/home/.xurl` for moor-run `xurl` commands, not `/opt/data/.xurl`. Run the user setup with the same HOME:
> ```bash
