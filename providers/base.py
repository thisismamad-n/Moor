...
def _profile_user_agent() -> str:
    """Return a ``moor-cli/<version>`` UA string, with a stable fallback.
    ...
    """
    try:
        from moor_cli import __version__ as _ver  # lazy: avoid layer cycle at import time
        return f"moor-cli/{_ver}"
    except Exception:
        return "moor-cli"
...
class ProviderProfile:
    ...
    # describe how to launch it; moor_cli/auth.py's
    ...
    def create_client(self, **client_kwargs: Any) -> Any | None:
        ...
        a profile registered from ``~/.moor/plugins/model-providers/`` or a
    ...
    def fetch_models(
        ...
        *,
        ...
        # moor-cli UA so the catalog endpoint is reachable.
