...
1. Bundled plugins: ``plugins/model-providers/<name>/`` (shipped with moor-agent)
2. User plugins: ``$MOOR_HOME/plugins/model-providers/<name>/``
3. Pip-installed plugins: distributions exposing a ``moor_agent.plugins``
...
def _user_plugins_dir() -> Path | None:
    """Return ``$MOOR_HOME/plugins/model-providers/`` if it exists."""
    try:
        from moor_constants import get_moor_home

        d = get_moor_home() / "plugins" / "model-providers"
...
def _installed_plugins_dir() -> Path | None:
    """Return ``$MOOR_HOME/plugins/`` if it exists.

    This is where ``moor plugins install`` clones a plugin — flat, one
...
def _import_plugin_dir(plugin_dir: Path, source: str) -> None:
    ...
    if source == "bundled":
        ...
        module_name = f"_moor_user_provider_{safe_name}"
...
def _discover_entry_point_providers() -> None:
    """Import pip-installed provider plugins via the ``moor_agent.plugins``
    ...
    * **Provider targets only.** The ``moor_agent.plugins`` group is shared
    ...
    first, so filesystem plugins (bundled + ``$MOOR_HOME``) keep their
    ...
    try:
        from moor_cli.plugins import _get_disabled_plugins, _get_enabled_plugins
...
def _discover_providers() -> None:
    ...
    Order:
      ...
      2. User plugins at ``$MOOR_HOME/plugins/model-providers/<name>/``
      2b. Plugins installed by ``moor plugins install`` at
          ``$MOOR_HOME/plugins/<name>/`` that declare ``kind: model-provider``
    ...
    #    filesystem steps means a bundled or ``$MOOR_HOME`` profile of the
    ...
    # 1. Bundled plugins — shipped with moor-agent.
    ...
    # 2. User plugins — under $MOOR_HOME/plugins/model-providers/<name>/.
    ...
    # 2b. Plugins installed by ``moor plugins install`` / the plugin index.
    #     Those clone into $MOOR_HOME/plugins/<name>/ — flat, NOT under
