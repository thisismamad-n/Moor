...
```bash
moor              # CLI interactiva — inicia una conversación
moor model        # Elige tu proveedor y modelo LLM
moor tools        # Configura qué herramientas están habilitadas
moor config set   # Establece valores de configuración individuales
moor gateway      # Inicia el gateway de mensajería (Telegram, Discord, etc.)
moor setup        # Ejecuta el asistente de configuración completo
moor claw migrate # Migra desde OpenClaw (si vienes de OpenClaw)
moor update       # Actualiza a la última versión
moor doctor       # Diagnostica cualquier problema
```
...
## Evita la colección de claves API — Moor Portal
...
Esto te autentica vía OAuth, establece Moor como tu proveedor y activa el Tool Gateway. Comprueba qué está conectado en cualquier momento con `moor portal info`. Detalles completos en la [página de documentación del Tool Gateway](https://hermes-agent.nousresearch.com/docs/user-guide/features/tool-gateway).
...
Moor tiene dos puntos de entrada: inicia la interfaz de terminal con `moor`, o ejecuta el gateway y habla con él desde Telegram, Discord, Slack, WhatsApp, Signal o Email. Una vez en una conversación, muchos comandos de barra son compartidos entre ambas interfaces.

| Acción                              | CLI                                           | Plataformas de mensajería                                                         |
| ----------------------------------- | --------------------------------------------- | --------------------------------------------------------------------------------- |
| Empezar a chatear                   | `moor`                                      | Ejecuta `moor gateway setup` + `moor gateway start`, luego envía un mensaje al bot |
...
**Durante la configuración inicial:** El asistente de configuración (`moor setup`) detecta automáticamente `~/.openclaw` y ofrece migrar antes de que comience la configuración.

**En cualquier momento después de instalar:**

```bash
moor claw migrate              # Migración interactiva (preset completo)
moor claw migrate --dry-run    # Vista previa de qué se migraría
moor claw migrate --preset user-data   # Migrar sin secretos
moor claw migrate --overwrite  # Sobreescribir conflictos existentes
```
...
- **Habilidades** — habilidades creadas por el usuario → `~/.moor/skills/openclaw-imports/`
...
Consulta `moor claw migrate --help` para todas las opciones, o usa la habilidad `openclaw-migration` para una migración guiada interactiva por el agente con vistas previas de dry-run.
...
- 🔌 [MoorClaw](https://github.com/AaronWong1999/hermesclaw) — Puente WeChat comunitario: Ejecuta Moor Agent y OpenClaw en la misma cuenta de WeChat.
...
Creado por [Moor inc.](https://nousresearch.com).
