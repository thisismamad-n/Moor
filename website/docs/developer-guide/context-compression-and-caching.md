...
Moor Agent uses a dual compression system and Anthropic prompt caching to
...
Moor has two separate compression layers that operate independently:
...
value is kept per `model@host` in `~/.moor/cache/image_token_costs.json` and
...
excluded because they already unlock the wider window), Moor raises the
...
`gpt-5.4-900k`). These are moor-side aliases: the suffix is stripped before
...
thread context, so Moor' auxiliary summarizer cannot shrink it — rewriting the
...
Moor' local transcript is never rewritten on this runtime — state.db records
the compaction boundary while the visible transcript stays intact. All other
routes (including Codex OAuth chat sessions) keep Moor' summary compressor.
...
Anthropic allows a maximum of 4 `cache_control` breakpoints per request. Moor
...
5. **Model identity is part of the cache key**: Provider-side caches are scoped
   ...
   Moor can avoid; user-facing docs for `/model`, fallback providers, and
