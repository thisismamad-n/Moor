...
capture is a separate plane served by Moor's native NeMo Relay SDK
...
| --- | --- | --- |
| Gateway gauges | `/v1/metrics` | `moor.gateway.up/state/busy/drainable/active_agents/background_work/background_delegations/restart_requested`, `moor.platform.up/degraded` with bounded `error_code` attributes |
...
`moor.gateway.active_agents`, `moor.gateway.background_work`, and
`moor.gateway.background_delegations` are complementary. `active_agents`
...
a plain `moor gateway run`: the exporter lives in the gateway process, so
...
`moor.gateway.up`, `moor.platform.up`, and `moor.platform.degraded`.
...
absent_over_time(moor_gateway_up[5m])

# Locally owned bridge is explicitly down.
moor_platform_up == 0

# Scheduler thread is stale even though the gateway may still be alive.
moor_cron_scheduler_heartbeat_age_seconds > 180

# Ticker loops but has not completed a successful tick recently.
moor_cron_scheduler_last_success_age_seconds > 300

# One or more jobs are beyond their existing scheduler grace window.
moor_cron_jobs_overdue > 0

# Catch-up counter increased, proving at least one stale occurrence was
# collapsed and run once after a delay.
increase(moor_cron_scheduler_catch_up_occurrences[15m]) > 0
```

Cron execution lifecycle records arrive as `moor.cron_execution` spans.
Alert or derive events from bounded attributes such as:

```text
moor.status = failed|unknown
moor.delivery_outcome = failed|not_configured
moor.error_class = auth_failed|rate_limited|timeout|network_error|
...
3. a cron lifecycle feed keyed only by opaque `moor.job_key`;
...
python scripts/observability/otel_capture_collector.py \
  --host 127.0.0.1 --port 4318 --log ~/.moor/cache/scratch/moor_otel_capture.jsonl
...
python scripts/observability/gateway_health_export_probe.py \
  ...
  --log ~/.moor/cache/scratch/moor_otel_capture.jsonl --wait 8
...
`moor.error_class = ...` list in this file's alert section and the enum's unit
...
```bash
moor monitoring status                 # posture
python scripts/observability/gateway_health_export_probe.py \
  --endpoint http://127.0.0.1:4318/v1/traces \
  --log ~/.moor/cache/scratch/cap.jsonl --wait 8          # drive the real exporter
```
...
The `moor monitoring` CLI intentionally exposes `status` only. This first
