...
Moor runs shell commands through a pluggable set of **terminal backends**.
...
installed under `~/.moor/plugins/`, registering a backend the user selects
...
| `moor setup` backend picker | `display_name`, `description`, `setup_instructions()`, `post_setup()` |
| Dashboard terminal-backend picker (probe status) | `probe()` |
| `moor status` / `moor doctor` | `doctor_checks()` |
...
```python title="~/.moor/plugins/acmebox/__init__.py"
...
class AcmeBoxProvider(TerminalEnvironmentProvider):
    ...
    def cache_path_base(self):
        return "~/.moor"    # where synced cache files land, or None
...
```yaml title="~/.moor/plugins/acmebox/plugin.yaml"
name: acmebox
version: 0.1.0
...
```bash
moor plugins enable acmebox
moor config set terminal.backend acmebox
```
