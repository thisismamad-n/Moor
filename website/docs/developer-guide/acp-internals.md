...
The ACP adapter wraps Moor' synchronous `AIAgent` in an async JSON-RPC stdio server.
...
```text
moor acp / moor-acp / python -m acp_adapter
  -> acp_adapter.entry.main()
  -> parse --version / --check / --setup before server startup
  -> load ~/.moor/.env
  -> configure stderr logging
  -> construct MoorACPAgent
...
- `allow_once` -> Moor `once`
- `allow_always` -> Moor `always`
- reject options -> Moor `deny`

Timeouts and bridge failures deny by default.

### Tool rendering helpers

`acp_adapter/tools.py` maps Moor tools to ACP tool kinds and builds editor-facing content.
...
new_session(cwd)
  ...
  -> create AIAgent(platform="acp", enabled_toolsets=<_get_platform_tools(config, "acp"), as on the
                   gateway: platform_toolsets.acp (default moor-acp) plus the resolver's extras
...
retries, interrupt before any reply) is closed by the core loop with a moor-authored
...
Instead it reuses Moor' runtime resolver:

- `acp_adapter/auth.py`
- `moor_cli/runtime_provider.py`

So ACP advertises and uses the currently configured Moor provider/credentials. It also always advertises a terminal setup auth method (`moor-setup`, args `--setup`) so first-run ACP clients can open Moor' interactive model/provider configuration before starting a normal ACP session.
...
- ACP sessions are persisted to the shared `~/.moor/state.db` (SessionDB) and transparently restored across process restarts; they appear in `session_search`
...
- `toolsets.py` — `moor-acp` toolset definition
- `moor_cli/main.py` — `moor acp` CLI subcommand
- `pyproject.toml` — `[acp]` optional dependency + `moor-acp` script
