...
description: "How to build an image-generation backend plugin for Moor Agent"
---
...
Moor scans for image-gen backends in three places:

1. **Bundled** — `<repo>/plugins/image_gen/<name>/` (auto-loaded with `kind: backend`, always available)
2. **User** — `~/.moor/plugins/image_gen/<name>/` (opt-in via `plugins.enabled`)
...
A bundled plugin is complete at this point. User plugins at `~/.moor/plugins/image_gen/<name>/` need to be added to `plugins.enabled` in `config.yaml` (or run `moor plugins enable <name>`).
...
`kind: backend` is what routes the plugin to the image-gen registration path. `requires_env` is prompted during `moor plugins install`.
