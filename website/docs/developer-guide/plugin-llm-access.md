...
moor-agent itself). For the async surface (`acomplete()` /
...
using Moor built-in auxiliary tasks; it does not permit another plugin's
...
* **Auth.** Pulls API keys, OAuth tokens, or refresh tokens from
  `~/.moor/auth.json` / env, including the credential pool when
...
* Implementation: [`agent/plugin_llm.py`](https://github.com/thisismamad-n/Moor/blob/main/agent/plugin_llm.py)
* Tests: [`tests/agent/test_plugin_llm.py`](https://github.com/thisismamad-n/Moor/blob/main/tests/agent/test_plugin_llm.py)
