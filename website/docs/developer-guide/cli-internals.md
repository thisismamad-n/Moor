...
sidebar_position: 15
title: "CLI Internals"
description: "How moor_cli is shaped: slash dispatch, config loaders, the skin engine, the transactional update pipeline, and process-identity rules"
---

# CLI Internals

Companion to `moor_cli/AGENTS.md` (the rules) — this page holds the longer explanations.
...
- Use the canonical matchers: `gateway.status.looks_like_gateway_command_line` (gateway run),
  `moor_cli.update_cmd._moor_holder_subcommand` (top-level subcommand of any Moor argv). Never
...
Built-in skins (`_BUILTIN_SKINS` in `moor_cli/skin_engine.py`): `default` (classic gold/kawaii),
...
User skins are `~/.moor/skins/<name>.yaml` with the same keys, activated with `/skin <name>` or
...
keys, memory, sessions, skills, gateway). For single-profile commands (`moor -p x <cmd>`),
`_apply_profile_override()` in `moor_cli/main.py` sets `MOOR_HOME` before any module imports, so
every `get_moor_home()` reference scopes to the active profile. The multiplex gateway and the
Desktop/dashboard `serve` backend serve several profiles from one process instead: the active
profile is a contextvar override bound per activity, `os.environ["MOOR_HOME"]` stays the launch
