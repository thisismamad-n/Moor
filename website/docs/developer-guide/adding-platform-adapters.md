...
Plugin metadata. The `requires_env` and `optional_env` blocks auto-populate `moor config` UI entries (see [Surfacing Env Vars](#surfacing-env-vars-in-hermes-config) below).
...
Most users set up a platform by dropping env vars into `~/.hermes/.env` rather than editing `config.yaml`. The `env_enablement_fn` hook lets your plugin pick those env vars up **before** the adapter is constructed, so `moor gateway status`, `get_connected_platforms()`, and cron delivery see the correct state without instantiating the platform SDK.
