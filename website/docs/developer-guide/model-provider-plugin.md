...
2. **User plugins** — `$MOOR_HOME/plugins/model-providers/<name>/` — drop in any directory; no restart required for subsequent sessions
...
Four values are recognized. Moor picks one based on:
...
`auth_type` gates which codepaths treat your provider as a "simple api-key provider" — if it's not `api_key`, the PluginManager still records the manifest but Moor' CLI-level automation (doctor checks, `--provider` flag, setup wizard delegation) may skip over it.
...
```bash
export MOOR_HOME=/tmp/moor-plugin-test
...
See [Building a Moor Plugin](/developer-guide/plugins#distribute-via-pip) for the full entry-points setup.
...
- [Building a Moor Plugin](/developer-guide/plugins) — general plugin authoring
