...
| An **STT backend** (custom whisper / ASR CLI) | [Voice Message Transcription](/user-guide/features/tts#voice-message-transcription-stt) — set `MOOR_LOCAL_STT_COMMAND` to an argv-tokenized template |
...
| **Gateway event hooks** (fire on startup, session events, commands) | [Event Hooks](/user-guide/features/hooks#gateway-event-hooks) — drop `HOOK.yaml` + `handler.py` into `~/.moor/hooks/<name>/` |
...
| **Additional skill sources** (custom GitHub repos, private skill indexes) | [Skills](/user-guide/features/skills) — `moor skills tap add <repo>` · [Publishing a tap](/user-guide/features/skills#publishing-a-custom-skill-tap) |
...
```bash
moor plugins install owner/repository --no-enable
moor plugins list
moor plugins enable <plugin-name>
```
...
from an isolated `MOOR_HOME`. Those tests load and invoke the plugin through
...
Moor's internals were split into `<stem>_<topic>` sibling modules in Sep 2026 (PR #102117). **Internal
...
- **Check your plugin:** `moor plugins compat /path/to/your/plugin` lists every `file:line` with the
  old path and the new one, and exits 1 while any remain. `COMPAT_MANIFEST.md` in the repo is the full map.
- **What users see:** a notice under the CLI banner, in `moor doctor` and after `moor update`, and a
  one-time Desktop dialog naming the plugin. Each resolution through an old path also emits a
  `MoorPluginCompatWarning` once per process.
- **From 2026-09-14:** plugins that still import old paths are **not loaded** (the reason shows in
  `moor plugins list`). Users can force-load with `plugins.allow_deprecated_imports: true` until the
...
```bash
mkdir -p ~/.moor/plugins/calculator
cd ~/.moor/plugins/calculator
```

### Validate with Plugin Doctor

`moor plugins doctor [path-or-id]` runs the same directory discovery,
...
```bash
moor plugins doctor . --ci
```

Doctor uses a temporary `MOOR_HOME`, restores plugin registration state after
...
`llm.task_override` (see `moor_cli/plugin_capabilities.py` for the
...
| `python_dependencies` | list of str | Declared pip requirements (e.g. `"requests>=2.0,<3"`). **Declaration seam only** — Moor validates them, and `moor plugins install` / `moor plugins doctor` surface missing ones with a `pip install` hint, but Moor **never auto-installs** them. Pin upper bounds. |
...
- `ctx.register_cli_command()` registers a CLI subcommand (e.g. `moor my-plugin <subcommand>`)
...
`<MOOR_HOME>/plugin-data/`. Neither API exposes another plugin's namespace.
...
If your plugin doesn't show up — or shows up but isn't loading — set `MOOR_PLUGINS_DEBUG=1` to get verbose discovery logs on stderr:

```bash
MOOR_PLUGINS_DEBUG=1 moor plugins list
```
...
The same logs are always written to `~/.moor/logs/agent.log` at WARNING level (failures only) and DEBUG level (everything) when the env var is set. So if you can't run with the env var (e.g. from inside the gateway), tail the log file instead:
...
- **Not enabled in config** — plugins are opt-in. Run `moor plugins enable <name>` (the name comes from the `plugins list` output, which can be `<category>/<plugin>` for nested layouts).
- **Wrong directory layout:** Native packages use `~/.moor/plugins/<plugin-name>/plugin.yaml` (flat) or one category level. Portable packages use root `plugin.json` in the same locations. Anything deeper is ignored.
...
tree, and `moor plugins update` / `remove` git-pull or delete it — your
...
# <moor home>/plugin-data/<name>/ — created on first use
...
**Key properties:**
- Plugin skills are **read-only** — they don't enter `~/.moor/skills/` and can't be edited via `skill_manage`.
...
:::tip Legacy pattern
The old `shutil.copy2` pattern (copying a skill into `~/.moor/skills/`) still works but creates name collision risk with built-in skills. Prefer `ctx.register_skill()` for new plugins.
:::
...
When users run `moor plugins install`, they're **prompted interactively** for any missing `requires_env` variables. Values are saved to `.env` automatically.
...
auditable in `~/.moor/logs/agent.log`. Plugins load after built-in
...
plugins:
  entries:
    my-plugin:                    # the plugin's registry key from `moor plugins list`
...
Per-hook context is capped at `10,000` characters by default. Anything above the cap is written to `$MOOR_HOME/hook_outputs/<session_id>/<uuid>.txt` and replaced with a head/tail preview plus the saved path. The model can read the full content via `read_file` or `terminal` if it genuinely needs it. This keeps a runaway plugin from inflating every subsequent turn's prompt and blowing out the prompt cache prefix. Tune in `config.yaml`:
...
hooks:
  output_spill:
    ...
    # directory: null      # default: $MOOR_HOME/hook_outputs
```
...
The canonical list of kinds is `VALID_MIDDLEWARE` in `moor_cli/middleware.py`:
...
- Middleware payloads carry `middleware_schema_version` (`moor.middleware.v1`) alongside the observer telemetry fields.
...
Plugins can add their own `moor <plugin>` subcommand tree:
...
After registration, users can run `moor my-plugin status`, `moor my-plugin config`, etc.
...
|---|---|---|
| Invoked as | `/name` in a session | `moor name` in a terminal |
...
`ctx._cli_ref` is only populated in an **interactive CLI** session. It is `None` in the gateway, in non-interactive `moor chat -q` runs, and in **kanban-spawned worker sessions** — so any plugin logic that reaches through `_cli_ref` silently no-ops in exactly those contexts. Two stable, session-agnostic APIs cover what hooks actually need:

- **`ctx.profile_name`** — the active profile name (e.g. `"default"`, or the assignee profile in a kanban worker). Derived from `MOOR_HOME`, so it works everywhere with no `_cli_ref` dependency.
...
For running a full `moor <subcommand>` (e.g. `moor kanban show`), shell out with the `terminal` tool via `ctx.dispatch_tool("terminal", {"command": "moor kanban show ..."})` — there is no in-process slash-command bridge for headless worker sessions, and tools are the supported way to drive Moor from a hook.
...
Moor has five specialized plugin types beyond the general surface. Each ships as a directory under `plugins/<category>/<name>/` (bundled) or `~/.moor/plugins/<category>/<name>/` (user). The contract differs by category — pick the one you need, then read its full guide.
...
def register(ctx):
    ctx.register_platform(
        ...
        # show up in `moor gateway status` without SDK instantiation.
...
Model Context Protocol (MCP) servers register their own tools into Moor without any Python plugin. Declare them in `~/.moor/config.yaml`:
...
Drop a manifest + handler into `~/.moor/hooks/<name>/`:
...
```bash
pip install moor-plugin-calculator
# Plugin auto-discovered on next moor startup
```
