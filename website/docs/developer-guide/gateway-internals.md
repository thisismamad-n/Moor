...
The messaging gateway is the long-running process that connects Moor to 20+ external messaging platforms through a unified architecture.
...
1. `resolve_command()` from `moor_cli/commands.py` maps input to canonical name (handles aliases, prefix matching)
...
|--------|-----------------|
| `~/.moor/.env` | API keys, bot tokens, platform credentials |
| `~/.moor/config.yaml` | Model settings, tool configuration, display options |
...
- **Explicit target delivery** — the send engine specifying `telegram:-1001234567890`, exposed via the [`moor send` CLI](../guides/pipe-script-output.md) for shell scripts and via cron `deliver:` targets
...
- **`reload-plugins` control verb** — other processes (`moor plugins install`, `moor serve`) ask the
