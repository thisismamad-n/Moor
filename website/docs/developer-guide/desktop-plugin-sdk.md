...
title: "Desktop Plugin SDK (@moor/plugin-sdk)"
...
A **desktop plugin** is a single ESM file that default-exports a `MoorPlugin`.
It imports one module — `@moor/plugin-sdk` — and gets everything: the app's
...
`$MOOR_HOME/desktop-plugins/<id>/plugin.js` and the app loads it within seconds
...
desktop app** (`moor desktop`) SDK — the `@moor/plugin-sdk` module and
`$MOOR_HOME/desktop-plugins/`. The **web dashboard** (`moor dashboard`) has
...
|------|-------|-----|------------|
| **Disk** (recommended) | `$MOOR_HOME/desktop-plugins/<id>/plugin.js` | users, agents | none — plain ESM, loaded uncompiled |
...
All three take the same `MoorPlugin` contract, appear in **Settings → Plugins**,
...
by default, or `~/.moor/profiles/<name>/...` under a named profile). The folder
name must equal the plugin `id`.

```javascript
// ~/.moor/desktop-plugins/hello/plugin.js
import { host, haptic, useValue } from '@moor/plugin-sdk'
...
The only importable specifiers are `@moor/plugin-sdk`, `react`, and
`react/jsx-runtime` — everything else fails to resolve, on purpose.
:::

## The plugin contract

A plugin default-exports a `MoorPlugin`:

```ts
interface MoorPlugin {
...
interface PluginContext {
  ...
  os: PluginOs
  /** Plugin-scoped JSON persistence (keys live under `moor.plugin.<id>.`). */
...
```javascript
import { ROUTES_AREA, SIDEBAR_NAV_AREA } from '@moor/plugin-sdk'
...
```javascript
import { STATUSBAR_AREAS, TITLEBAR_AREAS } from '@moor/plugin-sdk'
...
```javascript
import { PALETTE_AREA, KEYBINDS_AREA } from '@moor/plugin-sdk'
...
```javascript
import { THEMES_AREA } from '@moor/plugin-sdk'
...
```javascript
import { Button, useTheme } from '@moor/plugin-sdk'
...
```javascript
import { host, requestTheme } from '@moor/plugin-sdk'
...
```javascript
import { TRANSCRIPT_DIRECTIVE_AREA } from '@moor/plugin-sdk'
...
`window.moor.send('get-price eth')` (or a declarative
`<button data-moor-send="get-price eth">` — no script needed) hands that
...
```javascript
import { Contribute, TITLEBAR_AREAS } from '@moor/plugin-sdk'
...
ctx.os.notify({
  ...
  // Body click → focus Moor + navigate. Same vocabulary as OS deep links:
  activate: 'moor://index-network/intent/1',
  ...
  actions: [
    { id: 'open', label: 'Open', activate: 'moor://index-network/intent/1' },
    { id: 'dismiss', label: 'Dismiss', onAction: () => dismiss('1') },
  ],
})
```

`activate` is deeplink-compatible: `moor://index-network/intent/1` and the
hash path `/index-network/intent/1` resolve to the same in-app route (and the
same `moor://…` URL works as an OS deep link). Action buttons only render on
...
```javascript
import { useQuery, useMutation, useQueryClient, atom, computed, useValue } from '@moor/plugin-sdk'
...
```javascript
import { queryClient } from '@moor/plugin-sdk'
...
a package into `~/.moor/plugins` is inert on every surface until the user
...
the `moor://` scheme — a plain anchor on your website or README:
...
`config.yaml` (and not in `plugins.disabled`). Project plugins (`./.moor/`)
...
(`moor.plugin.<id>.*`) so plugins can't read or clobber each other:
...
export a `MoorPlugin`). It's discovered by `discoverBundledPlugins()` at boot —
...
1. It goes through the app's Vite build, so you can write **real JSX** and import
   the SDK by its `@moor/plugin-sdk` alias.
2. It's still lint-fenced to `@moor/plugin-sdk` + `react` only — no `@/…` app
   internals.
...
- **Only three specifiers resolve:** `@moor/plugin-sdk`, `react`,
...
### Agents: the `moor-desktop-plugins` skill

When an agent writes a desktop plugin, it should load the bundled
**`moor-desktop-plugins`** skill — it carries the same contract as this page in
...
`$MOOR_HOME/desktop-plugins/<id>/plugin.js` and the folder name matches the
export `id`. Run ⌘K → **Reload desktop plugins**. Check the app for an error
toast naming the failure, and tail `moor logs gui -f`.

**"unsupported import" on load.** A disk plugin may only import
`@moor/plugin-sdk`, `react`, and `react/jsx-runtime`. Remove any other import.
...
`~/.moor/plugins/<id>/dashboard/manifest.json` has `"api": "plugin_api.py"`,
that the plugin is in `plugins.enabled` in `config.yaml`, and restart the gateway
(backend routes mount at startup). Tail `~/.moor/logs/errors.log` for
