...
description: "How to add a new inference provider to Moor Agent — auth, runtime resolution, CLI flows, adapters, tests, and docs"
---

# Adding Providers

Moor can already talk to any OpenAI-compatible endpoint through the custom provider path. Do not add a built-in provider unless you want first-class UX for that service:

- provider-specific auth or token refresh
- a curated model catalog
- setup / `moor model` menu entries
...
1. `moor_cli/auth.py` decides how credentials are found.
2. `moor_cli/runtime_provider.py` turns that into runtime data:
   - `provider`
   - `api_mode`
   - `base_url`
   - `api_key`
...
4. `moor_cli/models.py` and `moor_cli/main.py` make the provider show up in the CLI. (`moor_cli/setup.py` delegates to `main.py` automatically — no changes needed there.)
...
1. `moor_cli/auth.py`
2. `moor_cli/models.py`
3. `moor_cli/runtime_provider.py`
4. `moor_cli/main.py`
...
:::tip
`moor_cli/setup.py` does **not** need changes. The setup wizard delegates provider/model selection to `select_provider_and_model()` in `main.py` — any provider added there is automatically available in `moor setup`.
:::
...
2. That's it. Provider plugins auto-load the first time anything calls `get_provider_profile()` or `list_providers()` — bundled plugins (this repo) and user plugins at `$MOOR_HOME/plugins/model-providers/` both get picked up.
...
7. `moor model` menu includes the provider
8. `moor setup` wizard delegates to `main.py` automatically
...
User plugins at `$MOOR_HOME/plugins/model-providers/<name>/` override bundled plugins of the same name (last-writer-wins in `register_provider()`) — so third parties can monkey-patch or replace any built-in profile without editing the repo.
...
- OAuth or token refresh (Moor Portal, Codex, Qwen Portal, Copilot)
...
- Provider-specific `moor model` menu entries with bespoke auth flows
...
- `PROVIDER_REGISTRY` in `moor_cli/auth.py`
- `_PROVIDER_LABELS` in `moor_cli/models_catalog_static.py` (re-exported by `moor_cli/models.py`)
- `_PROVIDER_ALIASES` in both `moor_cli/auth.py` and `moor_cli/models_catalog_static.py`
- CLI `--provider` choices in `moor_cli/main.py`
...
## Step 2: Add auth metadata in `moor_cli/auth.py`
...
- OAuth / auth-store path: Moor, OpenAI Codex

Questions to answer here:

- What env vars should Moor check, and in what priority order?
...
## Step 3: Add model catalog and aliases in `moor_cli/models.py`
...
## Step 4: Resolve runtime data in `moor_cli/runtime_provider.py`
...
Be careful with API-key precedence. Moor already contains logic to avoid leaking an OpenRouter key to unrelated endpoints. A new provider should be equally explicit about which key goes to which base URL.

## Step 5: Wire the CLI in `moor_cli/main.py`

A provider is not discoverable until it shows up in the interactive `moor model` flow.

Update these in `moor_cli/main.py`:
...
:::tip
`moor_cli/setup.py` does not need changes — it calls `select_provider_and_model()` from `main.py`, so your new provider appears in both `moor model` and `moor setup` automatically.
:::
...
When you add a native provider, double-check that Moor is only sending fields that provider actually understands.
...
- `tests/moor_cli/test_runtime_provider_resolution.py`
- `tests/moor_cli/test_cli_provider_resolution.py`
- `tests/moor_cli/test_model_switch_custom_providers.py` (and adjacent `tests/moor_cli/test_model_switch_*.py`)
- `tests/moor_cli/test_setup_model_provider.py`
...
python -m pytest tests/moor_cli/test_runtime_provider_resolution.py tests/moor_cli/test_cli_provider_resolution.py tests/moor_cli/test_setup_model_provider.py tests/agent/test_provider_parity.py -q
```
...
- [ ] `ProviderConfig` added in `moor_cli/auth.py`
- [ ] aliases added in `moor_cli/auth.py` and `moor_cli/models.py`
- [ ] model catalog added in `moor_cli/models.py`
- [ ] runtime branch added in `moor_cli/runtime_provider.py`
- [ ] CLI wiring added in `moor_cli/main.py` (setup.py inherits automatically)
...
### 7. Updating `moor model` but not `moor setup`
