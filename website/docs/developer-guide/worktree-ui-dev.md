...
| Variable | Meaning |
|----------|---------|
| `MOOR_MAIN_CHECKOUT` | The deps checkout — where `node_modules` really lives, and whose `.venv/bin/python` runs the backend. |
| `MOOR_GUI_DEPS_CHECKOUT` | Where the desktop deps (`apps/desktop/node_modules`) live. Defaults to `MOOR_MAIN_CHECKOUT`; override only if you keep desktop deps elsewhere. |

Neither is read by Moor itself — they're private to these helpers. The variables Moor *does* read are covered in [Environment Variables](../reference/environment-variables.md).

## `htui` — TUI from the worktree

The Ink TUI has a dev path already: `moor --tui --dev` runs the TypeScript sources via `tsx` instead of the prebuilt bundle. `htui` is a one-liner over it that also points the run at the current worktree's `ui-tui/`:

```bash
htui() {
  local root
  root="$(_moor_root)" || { echo "htui: not in a Moor checkout" >&2; return 1; }
  ( cd "$root" && PYTHONPATH="$root" \
      "$MOOR_MAIN_CHECKOUT/.venv/bin/python" -m moor_cli.main --tui --dev "$@" )
}
```

`--dev` compiles from source, so it links `ui-tui/node_modules` from `MOOR_MAIN_CHECKOUT` when the root lockfile matches and installs locally otherwise (see [`_moor_root` / linking helpers](#shared-helpers)).

:::warning `--dev` and `MOOR_TUI_DIR` are mutually exclusive
`MOOR_TUI_DIR` points Moor at a *prebuilt* bundle (Nix, system packages), which has no source to hot-reload. If it's set in your shell, `moor --tui --dev` exits with an error. Run `unset MOOR_TUI_DIR` before `htui`.
:::
...
hgui() {
  ...
  root="$(_moor_root)" || { echo "hgui: not in a Moor checkout" >&2; return 1; }
  ...
  if cmp -s "$root/package-lock.json" "$deps/package-lock.json"; then
    _moor_link_deps "$desktop" "$deps/apps/desktop"
    _moor_link_deps "$root" "$deps"
  else
  ...
  trap '_moor_gui_cleanup "$root"' INT TERM EXIT

  ( cd "$desktop"
    export PATH="$root/node_modules/.bin:$PATH"
    MOOR_DESKTOP_MOOR_ROOT="$root" \
    MOOR_DESKTOP_PYTHON="$MOOR_MAIN_CHECKOUT/.venv/bin/python" \
    MOOR_DESKTOP_IGNORE_EXISTING=1 \
    MOOR_DESKTOP_CWD="$root" \
    npm run dev )
}
```
...
|----------|----------------|
| `MOOR_DESKTOP_MOOR_ROOT` | Runs the backend from **this worktree**, not the packaged/PATH `moor`. |
| `MOOR_DESKTOP_PYTHON` | Reuses the deps checkout's venv instead of re-resolving a Python. |
| `MOOR_DESKTOP_IGNORE_EXISTING` | Ignores any `moor` on `PATH` so it can't shadow the worktree. |
| `MOOR_DESKTOP_CWD` | Opens the desktop chat rooted at the worktree. |
...
```bash
# The enclosing worktree, verified as a real Moor checkout.
_moor_root() {
  local root
  root="$(git rev-parse --show-toplevel 2>/dev/null)" || return 1
  [[ -f "$root/moor_cli/main.py" && -d "$root/ui-tui" ]] && print -r "$root"
}
...
_moor_gui_cleanup() {
  ...
  lsof -t -i:5174 >/dev/null 2>&1 && killport 5174
  pgrep -f 'moor_cli\.main.*dashboard.*--port 0' 2>/dev/null | xargs -r kill -TERM 2>/dev/null
}
```
