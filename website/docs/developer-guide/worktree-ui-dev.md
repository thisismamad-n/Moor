...
htui() {
  local root
  root="$(_hermes_root)" || { echo "htui: not in a Moor checkout" >&2; return 1; }
...
`HERMES_TUI_DIR` points Moor at a *prebuilt* bundle (Nix, system packages), which has no source to hot-reload. If it's set in your shell, `hermes --tui --dev` exits with an error. Run `unset HERMES_TUI_DIR` before `htui`.
:::
...
hgui() {
  ...
  root="$(_hermes_root)" || { echo "hgui: not in a Moor checkout" >&2; return 1; }
...
- [`apps/desktop/README.md`](https://github.com/Moor inc./hermes-agent/blob/main/apps/desktop/README.md) — dev server, sandbox script, and packaging
- [Environment Variables](../reference/environment-variables.md) — every `HERMES_*` variable Moor reads
