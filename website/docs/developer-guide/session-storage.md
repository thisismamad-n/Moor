...
Moor Agent uses a SQLite database (`~/.moor/state.db`) to persist session
...
`moor sessions recover` copies the row-bearing tables above into the
...
Abridged — see `SCHEMA_SQL` in `moor_state_common.py` (applied by `moor_state_schema.py`) for the full current column list
...
indexed columns — see `SCHEMA_SQL` in `moor_state_common.py` for the exact SQL.
...
Multiple moor processes (gateway + CLI sessions + worktree agents) share one
...
Default path: `~/.moor/state.db`

This is derived from `moor_constants.get_moor_home()` which resolves to
`~/.moor/` by default, or the value of `MOOR_HOME` environment variable.
