...
:::tip
Memory providers are one of two **provider plugin** types. The other is [Context Engine Plugins](./context-engine-plugin.md), which replace the built-in context compressor. Both follow the same pattern: single-select, config-driven, managed via `moor plugins`.
:::
...
|---|---|---|
| Bundled | `plugins/memory/<name>/` | Ships with Moor. Closed to new providers — see [CONTRIBUTING](https://github.com/thisismamad-n/Moor/blob/main/CONTRIBUTING.md). |
...
| Package | `moor_agent.memory_providers` entry point | Distribution supplied by the installation owner; nothing to copy. |
...
Moor's selected environment with pip. On PM-managed installations, ship a
directory provider with declared Python dependencies; plugin admission and
`moor memory setup` prepare them through PM before use. Owner-managed builds
...
anything. A successful preparation may require restarting Moor before the
...
Moor, in `$MOOR_HOME/plugins/<name>/` when installed by a user, or in
`./.moor/plugins/<name>/` for a project-local one:
...
`moor_agent.memory_providers` group. The entry-point name is the provider
name users select in `memory.provider`; its value points to the provider's
`register(ctx)` function:

```toml title="pyproject.toml"
[project.entry-points."moor_agent.memory_providers"]
...
package layout. No copy under `$MOOR_HOME/plugins/` is required.
...
(the dashboard config panel) and `cli.py` (your `moor <provider>`
...
class MyMemoryProvider(MemoryProvider):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs always includes:
          moor_home (str): Active MOOR_HOME path. Use for storage.
        """
...
| Keyword | Meaning |
|---|---|
| `moor_home` | Active profile's storage directory. |
...
| `session_title_source` | Stored title provenance, when available: `derived`, `llm`, or `user`. Automatic sources must not be mistaken for explicit identity overrides; missing provenance retains a provider's legacy behavior. Shared constants live in `moor_state_common.py`. |
...
| `agent_workspace`, `agent_context` | Runtime agent scope. `agent_workspace` is `moor`; `agent_context` is `cron` for scheduler runs, `subagent` for `delegate_task` children, else `primary` — skip automatic writes for the non-primary values. |
...
|--------|---------|-----------------|
| `get_config_schema()` | Declare config fields for `moor memory setup` | **Yes** |
| `save_config(values, moor_home)` | Write non-secret config to native location | **Yes** (unless env-var-only) |
...
Older Moor versions can omit `previous_content`. Providers that require exact
...
None of the providers bundled with Moor advertise checkpoint API v2 — the
...
required checkpoint cannot be guaranteed there. The checkpoint-aware Moor
...
Every setup surface Moor gives a bundled provider is driven by files in the provider's
...
| `moor memory setup` wizard | `get_config_schema()` declares the fields the wizard prompts for, `save_config(config, moor_home)` persists them, `post_setup(moor_home, config)` runs afterwards for anything interactive (OAuth, first sync); `get_status_config()` feeds `moor memory status` |
| `moor <provider> …` subcommands | `cli.py` with `register_cli(subparser)` ([Adding CLI Commands](#adding-cli-commands)) |
| Python dependencies | `pyproject.toml` `[project] dependencies` (or `python_dependencies` in `plugin.yaml`); installed under Moor' own pins at install time and re-applied across `moor update` |

Your provider's name, `memory.<name>` config section, data directory and tool names are the
contract with existing users. A provider that moves out of core keeps all four; Moor then
installs the catalog plugin automatically for anyone whose `memory.provider` still names it
(on `moor update`, and once at agent start when `security.allow_lazy_installs` is on).

## Config Schema

`get_config_schema()` returns a list of field descriptors used by `moor memory setup`:
...
Every field in `get_config_schema()` is prompted during `moor memory setup`. Providers with many options should keep the schema minimal — only include fields the user **must** configure (API key, required credentials). Document optional settings in a config file reference (e.g. `$MOOR_HOME/myprovider.json`) rather than prompting for them all during setup. This keeps the setup wizard fast while still supporting advanced configuration. See the Supermemory provider for an example — it only prompts for the API key; all other options live in `supermemory.json`.
:::

## Save Config

```python
def save_config(self, values: dict, moor_home: str) -> None:
    """Write non-secret config to your native location."""
    import json
    from pathlib import Path
    config_path = Path(moor_home) / "my-provider.json"
...
**`sync_turn()` MUST be non-blocking.** If your backend has latency (API calls, LLM processing), run the work in a daemon thread — spawned with `agent.memory_provider.spawn_context_thread`, never a bare `threading.Thread`. Profile isolation (the active `MOOR_HOME`, the per-turn secret scope) lives in `contextvars`, and a plain thread starts with an empty context: under multiplexed profiles it would silently write into the *default* profile's store, and `get_secret()` fails closed there.
...
The same applies to prefetch and writer threads. Small JSON config sidecars (`$MOOR_HOME/<provider>.json`) are read with `utils.read_json_or_empty` and written with `utils.atomic_json_write`; anything under `config.yaml` goes through `moor_cli.config.save_config(..., merge_existing=True)`.
...
All storage paths **must** use the `moor_home` kwarg from `initialize()`, not hardcoded `~/.moor`:

```python
# CORRECT — profile-scoped
from moor_constants import get_moor_home
data_dir = get_moor_home() / "my-provider"

# WRONG — shared across all profiles
data_dir = Path("~/.moor/my-provider").expanduser()
```
...
Memory provider plugins can register their own CLI subcommand tree (e.g. `moor my-provider status`, `moor my-provider config`). This uses a convention-based discovery system — no changes to core files needed.
...
4. Your commands appear under `moor <provider-name> <subcommand>`

**Active-provider gating:** Your CLI commands only appear when your provider is the active `memory.provider` in config. If a user hasn't configured your provider, your commands won't show in `moor --help`.
...
def my_command(args):
    ...
    if sub == "status":
        ...
        print("Usage: moor my-provider <status|config>")

def register_cli(subparser) -> None:
    """Build the moor my-provider argparse tree.
...
## `MOOR_HOME` survival contract (what wrappers can rely on)

For wrapper-style providers that keep their runtime in a sidecar venv outside moor-managed Python (no dependency surface — no `pyproject.toml`, `pip_dependencies`, or `python_dependencies` — at the scanned plugin root; a `pyproject.toml` belonging solely to an external or nested sidecar is not scanned):

- **Location.** `$MOOR_HOME/plugins/<name>/` is the profile-scoped plugin location, and `MOOR_HOME` follows the active context override, then `$MOOR_HOME`, then the platform default. Propagate `MOOR_HOME` when launching the wrapper or sidecar so profile isolation holds; `MemoryManager.initialize_all` injects the active `moor_home` into every provider.
- **Survival.** Ordinary Moor updates — including managed-venv rebuild/replacement by pm — do not delete or rewrite `$MOOR_HOME/plugins/**`. An installed wrapper directory and its marker file (e.g. `mnemosyne-wrapper.json`) survive. Explicit plugin updates and deletion flows (`moor uninstall`, `moor plugins remove`, profile deletion, user deletion) are excluded from this guarantee.
