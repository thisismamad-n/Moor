...
has to be thrown away or summarized. Moor has always done this in one batch:
...
Moor folds the single oldest un-absorbed exchange into a running summary. The
...
The in-memory splice alone isn't enough. Moor's normal session flush is
...
With `micro_compact` unset or `false` Moor behaves exactly as it always has:
...
Defaults to `$MOOR_HOME/logs/agent.log`. It reports passes, outcome mix, net
