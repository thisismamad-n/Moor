...
selection, or handoff to runtimes such as NeMo Relay without changing Moor'
planner, model provider adapters, tool registry, memory, or CLI UX.

With middleware enabled, plugins can:

- Rewrite LLM provider request kwargs before Moor calls the provider.
- Rewrite tool arguments before guardrails, approval checks, hooks, and tool
  execution see them.
- Wrap the actual LLM execution callback while preserving Moor retry,
  streaming, interrupt, and hook behavior.
- Wrap the actual tool execution callback while preserving Moor guardrails,
...
- `telemetry_schema_version`: currently `moor.observer.v1`
- `middleware_schema_version`: currently `moor.middleware.v1`
...
Moor stores those trace entries in later observer hook payloads as
`middleware_trace`.
...
If multiple plugins register the same execution middleware kind, Moor runs
them as a nested chain in registration order. Middleware failures are fail-open:
Moor logs a warning and continues with the next middleware or the base
...
For each provider request, Moor applies middleware in this order:
...
For each tool call, Moor applies middleware in this order:

1. Parse and coerce model-provided tool arguments.
2. Apply `tool_request` middleware.
3. Run the normal Moor pre-execution path against the effective arguments:
...
For isolated local testing, use one `MOOR_HOME` for plugin enablement and the
agent run:

```bash
export MOOR_HOME=$HOME/.moor/cache/scratch/moor-middleware-test
mkdir -p "$MOOR_HOME"
moor plugins enable <plugin-name>
...
```bash
export MOOR_HOME="$HOME/moor-middleware-test"
export MOOR_RUNTIME_DIR="$MOOR_HOME/tools"
source ./activate
python moor plugins enable <plugin-name>
python moor chat --query 'Reply exactly ok'
```
...
def tag_llm_request(**kwargs):
    ...
    extra_body.setdefault("metadata", {})["moor_middleware_demo"] = True
...
def normalize_terminal_workdir(**kwargs):
    ...
    args.setdefault("workdir", str(Path.home() / ".moor" / "cache" / "scratch" / "moor-middleware-demo"))
    return {
        "args": args,
        "source": "middleware-demo",
...
Return the same response shape Moor expects from the provider adapter. Do not
...
- If execution middleware raises before calling `next_call(...)`, Moor treats
...
- If execution middleware calls `next_call(...)` successfully and then raises
  during post-processing, Moor preserves the downstream result and does not
  run the provider or tool a second time.
- If downstream provider or tool execution fails, middleware may let that error
  propagate or translate it deliberately. Moor does not convert downstream
