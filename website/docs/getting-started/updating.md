...
description: "How to update Moor Agent to the latest version or uninstall it"
---
...
If the source checkout was left sitting on a feature branch (by tooling, a worktree experiment, or a manual checkout), `moor update` switches it back to the update target automatically whenever the working tree is clean:
...
- `discard` — auto-stash and drop the stash after the pull, so the update always lands on a clean tree. Use this only on machines where you never intend to keep local edits to the Moor source. It stash-drops (not `git reset --hard` + `git clean -fd`), so ignored paths like `node_modules`, `venv`, and build outputs are never touched.

In the desktop app this is **Settings → Advanced → In-App Update Local Changes**.

**Desktop updates never auto-restore.** The desktop updater invokes `moor update --keep-stash`: local source edits are still stashed so the update can proceed, but they are **not** re-applied afterward — they stay parked in `git stash` and the update log prints the exact `git stash apply <ref>` command to bring them back. This prevents local edits from silently riding along across desktop updates and breaking the freshly updated install. (`non_interactive_local_changes: discard` still wins if you've opted into discarding.) To restore parked changes manually:

```bash
cd ~/.moor/moor-agent   # or your install root
git stash list --format='%gd %H %s'   # find the moor-update-autostash entry
git stash apply stash@{0}
```

You can pass `--keep-stash` to a terminal `moor update` too if you want the same never-reapply behavior interactively.

### Preview-only: `moor update --check`

Want to know if an update is available before pulling? Run `moor update --check` — it fetches and compares commits against `origin/main`. No files are modified, no gateway is restarted. Useful in scripts and cron jobs that gate on "is there an update".

### Fleet preview: `moor update --plan`

Before updating a machine that runs several profiles or services, `moor update --plan` prints the full update plan without changing anything: the install kind (git checkout, Docker image, Nix/apt managed), every running Moor service across all profiles with its supervisor (systemd, launchd, manual) and the code version it is actually serving, and the restart mechanism each one will get. Manually-launched `moor serve` / `moor dashboard` backends appear too (from the spawn ledger), with their recorded bind endpoint and a "stop before code swap, relaunch with recorded launch args" restart mechanism. On image- or package-managed installs the plan reports that the install is not updatable in place and names the right update command instead. Read-only and safe on a live fleet.
...
For high-value profiles (production gateways, shared team installs) you can opt into a full pre-pull backup of `MOOR_HOME` (config, auth, sessions, skills, pairing):
...
`updates.pre_update_backup` is a single knob with three modes: `quick` (default — the lightweight state snapshot described above), `full` (the quick snapshot plus a complete `MOOR_HOME` zip; can add minutes on large homes), and `off` (no pre-update backup at all — `--no-backup` does the same for a single run). Legacy boolean values still work: `true` means `full`, `false` means `off`.

:::tip Moving to a new machine instead?
Update backups protect an in-place update. If you're migrating your whole setup to different hardware, use `moor backup` + `moor import` instead — see [Exporting Moor to another machine](/reference/faq#exporting-moor-to-another-machine) and [`moor backup` vs `moor profile export`](/reference/faq#moor-backup-vs-moor-profile-export).
:::

### Windows: another `moor.exe` is running

On Windows, `moor update` will refuse to run if it detects another `moor.exe` process holding the venv's entry-point executable open — most commonly the Moor Desktop app's spawned backend, an open `moor` REPL in another terminal, or a running gateway:

```
$ moor update
✗ Another moor.exe is running:
    PID 12345  moor.exe

  Updating now would fail to overwrite ...\venv\Scripts\moor.exe because
...
🔄 Restarting gateways...
✅ Gateway restarted
✅ Moor Agent updated successfully!
```
...
- All output is mirrored to `~/.moor/logs/update.log` while the update runs. If your terminal disappears, reconnect and inspect the log to see whether the update finished and whether the gateway restart succeeded:

```bash
tail -f ~/.moor/logs/update.log
```
...
nix flake update moor-agent

# Or rebuild with the latest
nix profile upgrade moor-agent
```
...
The uninstaller gives you the option to keep your configuration files (`~/.moor/`) for a future reinstall.

:::tip Moving to a new machine rather than leaving?
Take your setup with you before removing anything: `moor backup` captures the entire `~/.moor` directory including credentials, while `moor profile export` packs a single profile with credentials excluded by design (so an export alone is not a full backup). See [`moor backup` vs `moor profile export`](/reference/faq#moor-backup-vs-moor-profile-export).
:::
...
```bash
rm -f ~/.local/bin/moor
rm -rf /path/to/moor-agent
rm -rf ~/.moor            # Optional — keep if you plan to reinstall
```
...
```bash
moor gateway stop
# Linux: systemctl --user disable moor-gateway
# macOS: launchctl remove ai.moor.gateway
```
:::
