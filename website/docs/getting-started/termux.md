...
description: "Run Moor Agent directly on an Android phone with Termux"
---
...
Moor Agent can run directly on an Android phone through [Termux](https://termux.dev/).
...
That does not stop Moor from working well as a phone-native CLI agent — it just means the recommended mobile install is intentionally narrower than the desktop/server install.
...
- links `moor` into `$PREFIX/bin` so it stays on your Termux PATH
...
Moor requires **Python >=3.11,&lt;3.14**. Current Termux ships `python`
...
### 5. Put `moor` on your Termux PATH

```bash
ln -sf "$PWD/venv/bin/moor" "$PREFIX/bin/moor"
```

`$PREFIX/bin` is already on PATH in Termux, so this makes the `moor` command persist across new shells without re-activating the venv every time.
...
Or set keys directly in `~/.moor/.env`.
