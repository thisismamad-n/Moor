...
**Onchain AI Garage** put together a Masterclass walkthrough of installation, setup, and basic commands — a good companion to this page if you'd rather follow along on video. For more, see the full [Moor Agent Tutorials & Use Cases](https://www.youtube.com/playlist?list=PLmpUb_PWAkDxewld5ZYyKifuHxgIbiq2d) playlist.

<div style={{position: 'relative', paddingBottom: '56.25%', height: 0, overflow: 'hidden', maxWidth: '100%', marginBottom: '1.5rem'}}>
  <iframe
    style={{position: 'absolute', top: 0, left: 0, width: '100%', height: '100%'}}
    src="https://www.youtube-nocookie.com/embed/R3YOGfTBcQg"
    title="Moor Agent Masterclass: Installation, Setup, Basic Commands"
    frameBorder="0"
...
- Setting up Moor for a team, bot, or always-on workflow
...
|---|---|---|
| I just want Moor working on my machine | `moor setup` | Run a real chat and verify it responds |
| I already know my provider | `moor model` | Save the config, then start chatting |
| I want a bot or always-on setup | `moor gateway setup` after CLI works | Connect Telegram, Discord, Slack, or another platform |
| I want a local or self-hosted model | `moor model` → custom endpoint | Verify the endpoint, model name, and context length |
| I want multi-provider fallback | `moor model` first | Add routing and fallback only after the base chat works |

**Rule of thumb:** if Moor cannot complete a normal chat, do not add more features yet. Get one clean conversation working first, then layer on gateway, cron, skills, voice, or routing.

---

## 1. Install Moor Agent
...
The single most important setup step. Use `moor model` to walk through the choice interactively:

```bash
moor model
```

:::tip Easiest path: Moor Portal
...
- **Quick Setup (Moor Portal)** — OAuth login, no API keys to manage; sets up a model plus the Tool Gateway tools, billed to your [Moor Portal subscription](/integrations/moor-portal). The recommended fast path.
...
Blank Slate writes an explicit `platform_toolsets.cli` list plus `agent.disabled_toolsets`, so nothing you didn't choose ever loads — not even after `moor update`. Re-enable anything later with `moor tools`, seed skills with `moor skills opt-in --sync`, or tune settings with `moor setup agent`.
:::
...
|----------|-----------|---------------|
| **Moor Portal** | Subscription-based, zero-config | OAuth login via `moor model` |
| **OpenAI Codex** | ChatGPT or Codex subscription, uses Codex models | Device code auth via `moor model` → **ChatGPT or Codex Subscription** |
| **Anthropic** | Claude models directly — Max plan + extra usage credits (OAuth), or API key for pay-per-token | `moor model` → OAuth login (requires Max + extra credits), or an Anthropic API key |
...
| **MiniMax (OAuth)** | MiniMax frontier model via browser OAuth — no API key needed (model name in `moor_cli/models.py` may change between releases) | `moor model` → MiniMax (OAuth) |
...
| **xAI Grok OAuth** | SuperGrok / Premium+ subscription, no API key needed | `moor model` → xAI Grok OAuth |
...
| **Qwen OAuth** | Qwen Portal browser OAuth — no API key needed | `moor model` → Qwen OAuth |
...
| **GitHub Copilot** | GitHub Copilot subscription (GPT-5.x, Claude, Gemini, etc.) | OAuth via `moor model`, or `COPILOT_GITHUB_TOKEN` / `GH_TOKEN` |
| **GitHub Copilot ACP** | Copilot ACP agent backend (spawns local `copilot` CLI) | `moor model` (requires `copilot` CLI + `copilot login`) |
...
Moor Agent requires a model with at least **64,000 tokens** of context. Models with smaller windows cannot maintain enough working memory for multi-step tool-calling workflows and will be rejected at startup. Most hosted models (Claude, GPT, Gemini, Qwen, DeepSeek) meet this easily. If you're running a local model, set its context size to at least 64K (e.g. `--ctx-size 65536` for llama.cpp or `-c 65536` for Ollama).
:::

:::tip
You can switch providers at any time with `moor model` — no lock-in. For a full list of all supported providers and setup details, see [AI Providers](../integrations/providers.md).
:::

### How settings are stored

Moor separates secrets from normal config:

- **Secrets and tokens** → `~/.moor/.env`
- **Non-secret settings** → `~/.moor/config.yaml`

The easiest way to set values correctly is through the CLI:

```bash
moor config set model anthropic/claude-opus-4.6
...
```bash
moor            # classic CLI
moor --tui      # modern TUI (recommended)
```

You'll see a welcome banner with your model, available tools, and skills. Use a prompt that's specific and easy to verify:

:::tip Pick your interface
Moor ships with two terminal interfaces: the classic `prompt_toolkit` CLI and a newer [TUI](../user-guide/tui.md) with modal overlays, mouse selection, and non-blocking input. Both share the same sessions, slash commands, and config — try each with `moor` vs `moor --tui`.
:::
...
```bash
moor --continue    # Resume the most recent session
moor -c            # Short form
```
...
```bash
moor gateway setup    # Interactive platform configuration
```
...
- `moor tools` — tune tool access per platform
- `moor skills` — browse and install reusable workflows
...
```bash
moor config set terminal.backend docker    # Docker isolation
moor config set terminal.backend ssh       # Remote server
```

For Docker sandboxes, you can also enable the **egress credential-injection proxy** so the sandbox never sees your real API keys — only opaque proxy tokens that work exclusively from behind a local TLS-intercepting daemon. See [Egress proxy](../user-guide/egress/iron-proxy.md). Setup is `moor egress setup && moor egress start`; `moor setup terminal` also points Docker users at it. Modal, SSH, Daytona, and Singularity are not wired yet.

### Voice mode

```bash
# From the Moor install directory (the curl installer placed it at
# ~/.moor/moor-agent on Linux/macOS or %LOCALAPPDATA%\moor\moor-agent on Windows):
cd ~/.moor/moor-agent
...
Skills are on-demand instruction documents that teach Moor how to do a specific task — deploy to Kubernetes, open a GitHub PR, fine-tune a model, search for GIFs. Each is a `SKILL.md` file with a name, a description, and a step-by-step procedure. The agent reads the short descriptions for free and only loads a skill's full content when a task actually calls for it, so adding skills doesn't bloat every request.

Moor ships with a catalog of bundled skills already installed in `~/.moor/skills/`. You can add more from the Skills Hub, or write your own.

**Browse and install from the hub:**

```bash
moor skills browse                      # list everything available
moor skills search kubernetes           # find skills by keyword
moor skills install openai/skills/k8s   # install one (runs a security scan first)
```

The install argument is a `source/path` slug from the hub — `openai/skills/k8s` means the `k8s` skill from OpenAI's catalog. `moor skills browse` shows the exact slugs to use.
...
/k8s                                       # load it and let Moor ask what you need
```
...
(If you installed without `[all]`, run `cd ~/.moor/moor-agent && uv pip install -e ".[acp]"` first.)
...
|---|---|---|
| Moor opens but gives empty or broken replies | Provider auth or model selection is wrong | Run `moor model` again and confirm provider, model, and auth |
...
| Gateway starts but nobody can message it | Bot token, allowlist, or platform setup is incomplete | Re-run `moor gateway setup` and check `moor gateway status` |
| `moor --continue` can't find old session | Switched profiles or session never saved | Check `moor sessions list` and confirm you're in the right profile |
...
| `moor doctor` flags config problems | Config values are missing or stale | Fix the config, retest a plain chat before adding features |
...
2. `moor model`
3. `moor setup`
4. `moor sessions list`
5. `moor --continue`
6. `moor gateway status`
...
| `moor tools` | Configure which tools are enabled per platform |
| `moor setup` | Full setup wizard (configures everything at once) |
| `moor doctor` | Diagnose issues |
| `moor update` | Update to latest version |
| `moor gateway` | Start the messaging gateway |
| `moor --continue` | Resume last session |
...
- **[Moving to another machine](/reference/faq#exporting-moor-to-another-machine)** — `moor backup` migrates your whole setup (or [a single profile](/reference/faq#moving-a-single-profile-to-another-machine)); no need to rebuild from scratch
