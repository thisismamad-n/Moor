...
description: "Install and deploy Moor Agent with Nix — from quick `nix run` to fully declarative NixOS module with container mode"
---
...
**For non-NixOS users**, this only changes the install step. Everything after (`moor setup`, `moor gateway install`, config editing) works identically to the standard install.
...
These outputs unpack the pinned archives. They are not the complete Moor
...
After `nix profile install`, `moor`, `moor-agent`, and `moor-acp` are on your PATH. From here, the workflow is identical to the [standard installation](./installation.md) — `moor setup` walks you through provider selection, `moor gateway install` sets up a launchd (macOS) or systemd user service, and config lives in `~/.moor/`.

:::warning Messaging platforms (Discord, Telegram, Slack)
The default package includes ALL libraries moor-agent might need. if you want a smaller variant, check the other flake outputs. 
...
# /etc/nixos/flake.nix (or your system flake)
{
  ...
  outputs = { nixpkgs, moor-agent, ... }: {
    nixosConfigurations.your-host = nixpkgs.lib.nixosSystem {
      system = "x86_64-linux";
      modules = [
        moor-agent.nixosModules.default
        ./configuration.nix
      ];
    };
  };
...
That's it. `nixos-rebuild switch` creates the `moor` user, generates `config.yaml`, wires up secrets, and starts the gateway — a long-running service that connects the agent to messaging platforms (Telegram, Discord, etc.) and listens for incoming messages.
...
```bash
echo "OPENROUTER_API_KEY=sk-or-your-key" | sudo install -m 0600 -o moor /dev/stdin /var/lib/moor/env
```

```nix
services.moor-agent.environmentFiles = [ "/var/lib/moor/env" ];
```
:::

:::tip addToSystemPackages
Setting `addToSystemPackages = true` does two things: puts the `moor` CLI on your system PATH **and** sets `MOOR_HOME` system-wide so the interactive CLI shares state (sessions, skills, cron) with the gateway service. Without it, running `moor` in your shell creates a separate `~/.moor/` directory.
:::
...
Set `container.hostUsers` to create a `~/.moor` symlink to the service state directory, so the host CLI and the container share sessions, config, and memories:
...
Users listed in `hostUsers` are automatically added to the `moor` group for file permission access.
...
The CLI auto-detects when sudo is needed and uses it transparently. Without this, you'll need to run `sudo moor chat` manually.
:::
...
:::note Model naming
`settings.model.default` uses the model identifier your provider expects. With [OpenRouter](https://openrouter.ai) (the default), these look like `"anthropic/claude-sonnet-4"` or `"google/gemini-3-flash"`. If you're using a provider directly (Anthropic, OpenAI), set `settings.model.base_url` to point at their API and use their native model IDs (e.g., `"claude-sonnet-4-20250514"`). When no `base_url` is set, Moor defaults to OpenRouter.
:::
...
```nix
services.moor-agent.configFile = /etc/moor/config.yaml;
```

This bypasses `settings` entirely — no merge, no generation. The file is copied as-is to `$MOOR_HOME/config.yaml` on each activation.
...
| Override the moor package | `package` | `inputs.moor-agent.packages.${system}.default.override { ... }` |
| Change state directory | `stateDir` | `"/opt/moor"` |
...
Both `environment` (non-secret vars) and `environmentFiles` (secret files) are merged into `$MOOR_HOME/.env` at activation time (`nixos-rebuild switch`). Moor reads this file on every startup, so changes take effect with a `systemctl restart moor-agent` — no container recreation needed.
...
{
  sops = {
    defaultSopsFile = ./secrets/moor.yaml;
    age.keyFile = "/home/user/.config/sops/age/keys.txt";
    secrets."moor-env" = { format = "yaml"; };
  };

  services.moor-agent.environmentFiles = [
    config.sops.secrets."moor-env".path
  ];
}
```
...
```yaml
# secrets/moor.yaml (encrypted with sops)
moor-env: |
...
```nix
{
  age.secrets.moor-env.file = ./secrets/moor-env.age;

  services.moor-agent.environmentFiles = [
    config.age.secrets.moor-env.path
  ];
}
```
...
{
  services.moor-agent = {
    authFile = config.sops.secrets."moor/auth.json".path;
...
```nix
{
  services.moor-agent.mcpServers = {
    filesystem = {
      command = "npx";
      args = [ "-y" "@modelcontextprotocol/server-filesystem" "/data/workspace" ];
    };
...
:::tip
Environment variables in `env` values are resolved from `$MOOR_HOME/.env` at runtime. Use `environmentFiles` to inject secrets — never put tokens directly in Nix config.
:::
...
```nix
{
  services.moor-agent.mcpServers.remote-api = {
...
Set `auth = "oauth"` for servers using OAuth 2.1. Moor implements the full PKCE flow — metadata discovery, dynamic client registration, token exchange, and automatic refresh.

```nix
{
  services.moor-agent.mcpServers.my-oauth-server = {
...
Tokens are stored in `$MOOR_HOME/mcp-tokens/<server-name>.json` and persist across restarts and rebuilds.

<details>
<summary><strong>Initial OAuth authorization on headless servers</strong></summary>

The first OAuth authorization requires a browser-based consent flow. In a headless deployment, Moor prints the authorization URL to stdout/logs instead of opening a browser.

**Option A: Interactive bootstrap** — run the flow once via `docker exec` (container) or `sudo -u moor` (native):

```bash
# Container mode
docker exec -it moor-agent \
  moor mcp add my-oauth-server --url https://mcp.example.com/mcp --auth oauth

# Native mode
sudo -u moor MOOR_HOME=/var/lib/moor/.moor \
  moor mcp add my-oauth-server --url https://mcp.example.com/mcp --auth oauth
```
...
```bash
moor mcp add my-oauth-server --url https://mcp.example.com/mcp --auth oauth
scp ~/.moor/mcp-tokens/my-oauth-server{,.client}.json \
    server:/var/lib/moor/.moor/mcp-tokens/
# Ensure: chown moor:moor, chmod 0600
```
...
```nix
{
  services.moor-agent.mcpServers.analysis = {
    command = "npx";
...
When moor runs via the NixOS module, the following CLI commands are **blocked** with a descriptive error pointing you to `configuration.nix`:

| Blocked command | Why |
|---|---|
| `moor setup` | Config is declarative — edit `settings` in your Nix config |
| `moor config edit` | Config is generated from `settings` |
| `moor config set <key> <value>` | Config is generated from `settings` |
| `moor gateway install` | The systemd service is managed by NixOS |
| `moor gateway uninstall` | The systemd service is managed by NixOS |
...
When container mode is enabled, moor runs inside a persistent Ubuntu container with the Nix-built binary bind-mounted read-only from the host:

```
Host                                    Container
────                                    ─────────
/nix/store/...-moor-agent-0.1.0  ──►  /nix/store/... (ro)
~/.moor -> /var/lib/moor/.moor       (symlink bridge, per hostUsers)
/var/lib/moor/                    ──►  /data/          (rw)
...
The container is only recreated when its **identity hash** changes. The hash covers: schema version, image, `extraVolumes`, `extraOptions`, and the entrypoint script. Changes to environment variables, settings, documents, or the moor package itself do **not** trigger recreation.

:::warning Writable layer loss
When the identity hash changes (image upgrade, new volumes, new container options), the container is destroyed and recreated from a fresh pull of `container.image`. Any `apt install`, `pip install`, or `npm install` packages in the writable layer are lost. State in `/data` and `/home/moor` is preserved (these are bind mounts).

If the agent relies on specific packages, consider baking them into a custom image (`container.image = "my-registry/moor-base:latest"`) or scripting their installation in the agent's SOUL.md.
:::

### GC Root Protection

The `preStart` script creates a GC root at `${stateDir}/.gc-root` pointing to the current moor package. This prevents `nix-collect-garbage` from removing the running binary. If the GC root somehow breaks, restarting the service recreates it.

---

## Plugins

The NixOS module supports declarative plugin installation — no imperative `moor plugins install` needed.

### Directory Plugins (`extraPlugins`)

For plugins that are just a source tree with `plugin.yaml` + `__init__.py` (e.g., [moor-lcm](https://github.com/stephenschoettler/moor-lcm)):

```nix
services.moor-agent.extraPlugins = [
...
Plugins are symlinked into `$MOOR_HOME/plugins/` at activation time. Moor discovers them via its normal directory scan. Removing a plugin from the list and running `nixos-rebuild switch` removes the symlink.
...
The package's `site-packages` is added to PYTHONPATH in the moor wrapper. `importlib.metadata` discovers the entry point at session start.

### Optional Dependency Groups (`extraDependencyGroups`)

For optional extras declared in moor-agent's `pyproject.toml`, use `extraDependencyGroups` to include them in the sealed venv at build time. This is required for any extra not in the default `[all]` set — on Nix, runtime installation into the read-only store is not possible.

```nix
# Enable Discord, Telegram, Slack
services.moor-agent.extraDependencyGroups = [ "messaging" ];
```
...
Memory providers that live in the [plugin catalog](../user-guide/features/plugins.md) rather than in the Moor tree (e.g. Hindsight) are not extras. Install them like any catalog plugin with `moor plugins install hindsight`, or declaratively via [`extraPlugins`](#directory-plugins-extraplugins) pointing at the plugin's source tree.
...
services.moor-agent = {
  ...
  extraPythonPackages = [ config.services.moor-agent.package.python.pkgs.redis ];  # its Python dep
...
```nix
{
  inputs.moor-agent.url = "github:NousResearch/hermes-agent";
...
```nix
services.moor-agent.settings.plugins.enabled = [
  "moor-lcm"
  "rtk-rewrite"
];
```
...
:::note
A build-time collision check prevents plugin packages from shadowing core moor dependencies. If a plugin provides a package already in the sealed venv, `nixos-rebuild` fails with a clear error.
:::
...
and the `dev` dependency group. `MOOR_PYTHON` points to its interpreter. It does not install
...
nix build .#checks.x86_64-linux.managed-guard       # MOOR_MANAGED blocks mutation
...
| `bundled-skills` | Skills directory exists, contains SKILL.md files, `MOOR_BUNDLED_SKILLS` is set in wrapper |
...
|---|---|---|---|
| `enable` | `bool` | `false` | Enable the moor-agent service |
| `package` | `package` | `moor-agent` | The moor-agent package to use |
| `user` | `str` | `"moor"` | System user |
| `group` | `str` | `"moor"` | System group |
| `createUser` | `bool` | `true` | Auto-create user/group |
| `stateDir` | `str` | `"/var/lib/moor"` | State directory (`MOOR_HOME` parent) |
| `workingDirectory` | `str` | `"${stateDir}/workspace"` | Agent working directory |
| `addToSystemPackages` | `bool` | `false` | Add `moor` CLI to system PATH and set `MOOR_HOME` system-wide |
...
| Option | Type | Default | Description |
|---|---|---|---|
| `environmentFiles` | `listOf str` | `[]` | Paths to env files with secrets. Merged into `$MOOR_HOME/.env` at activation time |
...
| `container.hostUsers` | `listOf str` | `[]` | Interactive users who get a `~/.moor` symlink to the service stateDir and are auto-added to the `moor` group |
...
```bash
systemctl status moor-agent
docker ps -a --filter name=moor-agent
docker inspect moor-agent --format='{{.State.Status}}'
docker exec -it moor-agent bash
docker exec moor-agent readlink /data/current-package
docker exec moor-agent cat /data/.container-identity
```
...
```bash
sudo systemctl stop moor-agent
docker rm -f moor-agent
sudo rm /var/lib/moor/.container-identity
sudo systemctl start moor-agent
```
...
```bash
# Native mode
sudo -u moor cat /var/lib/moor/.moor/.env

# Container mode
docker exec moor-agent cat /data/.moor/.env
```

### GC Root Verification

```bash
nix-store --query --roots $(docker exec moor-agent readlink /data/current-package)
```
...
| `No adapter available for discord` (or telegram/slack) | Messaging deps missing from the sealed Nix venv | Install `#messaging` variant: `nix profile install ...#messaging`. For NixOS module: `extraDependencyGroups = [ "messaging" ]`. Read `journalctl -u moor-agent` for `InstallError` or `requirements not met` and the underlying cause. |
...
| `unable to find user moor` | Container still starting (entrypoint hasn't created user yet) | Wait a few seconds and retry — the CLI retries automatically |
| Tool added via `extraPackages` not found in terminal | Requires `nixos-rebuild switch` to update the per-user profile | Rebuild and restart: `nixos-rebuild switch && systemctl restart moor-agent` |
