...
- **Model & provider pin** — give the Bot its own model. Any provider/model pair Moor knows about works, and different Bots can run on different models side by side. Leave it unset to inherit from the launch profile.
...
moor peer list
moor peer dm spark < /tmp/dm.txt        # message body from a file (nothing shell-interpreted)
moor peer dm spark/researcher < /tmp/dm.txt   # named profile on a multiplexed peer
moor peer run spark --idempotency-key ticket-123 < /tmp/long-task.txt
