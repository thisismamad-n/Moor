...
sidebar_position: 4
title: "Slack"
description: "Set up Moor Agent as a Slack bot using Socket Mode"
---

# Slack Setup

Connect Moor Agent to Slack as a bot using Socket Mode. Socket Mode uses WebSockets instead of
public HTTP endpoints, so your Moor instance doesn't need to be publicly accessible — it works
behind firewalls, on your laptop, or on a private server.

:::warning Classic Slack Apps Deprecated
Classic Slack apps (using RTM API) were **fully deprecated in March 2025**. Moor uses the modern
...
The fastest path is to paste a manifest Moor generates for you. It
...
### Option A: From a moor-generated manifest (recommended)
...
4. Enter an app name (e.g., "Moor Agent") and select your workspace
5. Click **Create App**
...
it will only work in DMs. Without `files:read`, Moor can chat but **cannot reliably read user-uploaded attachments**.
...
Even with all the correct scopes and event subscriptions, Slack will not allow users to send direct messages to the bot unless the Messages Tab is enabled. This is a Slack platform requirement, not a Moor configuration issue.
:::
...
Moor uses Slack **Member IDs** (not usernames or display names) for the allowlist.
...
can spend the entire turn in hidden reasoning and never produce visible assistant text; Moor now
...
Every Moor command (`/btw`, `/stop`, `/new`, `/model`, `/help`, ...)
...
Moor command with its description.
...
New Slack apps use Slack's **Agent** messaging experience. Existing Moor
...
to `message.im`, `app_home_opened`, and `app_context_changed`, so Moor can
identify a Messages-tab DM and receive the user's active Slack context with a
turn. Moor only supplies that context as a label; it does not read the viewed
channel's history.
...
`/moor bg run the tests` — Moor routes it the same way as `/bg
...
Slack never delivers them to Moor.

As a workaround, Moor recognises a leading `!` as an alternate
command prefix that works in threads (and anywhere else). Type
`!queue`, `!stop`, `!model gpt-5.4`, etc. as a regular thread reply —
Moor treats it identically to the slash form and replies in the same
thread.
...
Moor falls back to a text prompt, the prompt instructs you to reply
...
Understanding how Moor behaves in different contexts:

| Context | Behavior |
|---------|----------|
| **DMs** | Bot responds to every message — no @mention needed |
| **Channels** | Bot **only responds when @mentioned** (e.g., `@Moor Agent what time is it?`). In channels, Moor replies in a thread attached to that message. |
| **Threads** | If you @mention Moor inside an existing thread, it replies in that same thread. Once the bot has an active session in a thread, **subsequent replies in that thread do not require @mention** — the bot follows the conversation naturally. |
...
| `platforms.slack.extra.native_task_cards` | `false` | When `true`, renders live tool calls as Slack-native plan/task cards. Cards work with Slack's built-in default `tool_progress: off`; an explicitly configured `display.tool_progress: off` (global or `display.platforms.slack`; `/verbose` writes the same key) disables cards too. Cards need a thread: when the card lane is active and the chat has no thread to anchor on (a top-level DM with `reply_in_thread: false`), Moor shows no tool progress instead of text bubbles, unless you explicitly set `tool_progress: new`/`all`, which falls back to editable text progress there. Recoverable native API failures fall back to one continuously edited text update. |
...
- Moor checks thread eligibility before attempting publication, so a
...
When `true` (the default), each user in a shared channel gets their own isolated conversation session. Two people talking to Moor in `#general` will have separate histories and contexts.
...
slack:
  ...
  # must @mention the bot before Moor will respond.
...
`client_msg_id` — the same signature Moor uses to recognise app posts — so it
is dropped as bot traffic. This blocks a common pattern: a custom front-end (an
internal dashboard, a mobile shell, a kiosk) that sends messages to Moor *as*
the logged-in user.
...
Set `SLACK_HOME_CHANNEL` to a channel ID where Moor will deliver scheduled messages,
...
Make sure the bot has been **invited to the channel** (`/invite @Moor Agent`).
...
Moor can connect to **multiple Slack workspaces** simultaneously using a single gateway instance. Each workspace is authenticated independently with its own bot user ID.
...
- When a message arrives, Moor uses the correct workspace-specific client to respond.
...
| Bot works in DMs but not in channels | **Most common issue.** Add `message.channels` and `message.groups` to event subscriptions, reinstall the app, and invite the bot to the channel with `/invite @Moor Agent` |
...
| Bot responds but can't post in a channel | Invite the bot to the channel with `/invite @Moor Agent` |
| Bot can chat but can't read uploaded images/files | Add `files:read`, then **reinstall** the app. Moor now surfaces attachment access diagnostics in-chat when Slack returns scope/auth/permission failures. |
...
7. ✅ Bot was **invited** to the channel (`/invite @Moor Agent`)
