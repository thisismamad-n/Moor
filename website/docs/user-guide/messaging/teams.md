...
description: "Set up Moor Agent as a Microsoft Teams bot"
---
...
Teams delivers @mentions as regular messages with `<at>BotName</at>` tags, which Moor strips automatically before processing.
...
```bash
# devtunnel (Microsoft)
devtunnel create moor-bot --allow-anonymous
devtunnel port create moor-bot -p 3978 --protocol http  # replace 3978 with TEAMS_PORT if changed
devtunnel host moor-bot
...
The public tunnel URL uses HTTPS, but Moor' local webhook listener uses plain HTTP. The tunnel terminates TLS and forwards HTTP to port `3978`; do not configure the local tunnel port as HTTPS.
...
**Docker** (must run from the directory that contains `docker-compose.yml` — usually your cloned `moor-agent` repo, not `~`):

```bash
cd /path/to/moor-agent
MOOR_UID=$(id -u) MOOR_GID=$(id -g) docker compose up -d gateway
```

**Native / systemd install** (typical `moor` one-liner installer under `~/.moor/moor-agent`):

```bash
moor gateway restart
# or foreground: moor gateway run
```

The Teams SDK is optional; when Teams is enabled, the gateway lazy-installs it into Moor' own venv on first start (do **not** use system `pip install` on Ubuntu 24.04 — that hits PEP 668 `externally-managed-environment`). To install manually into the Moor venv:

```bash
~/.moor/moor-agent/venv/bin/pip install microsoft-teams-apps aiohttp
...
Alternatively, configure via `~/.moor/config.yaml`:
...
For a permanent server, terminate TLS at a reverse proxy and forward requests to the plain HTTP Moor listener, normally `http://127.0.0.1:3978`. Register the proxy's public HTTPS endpoint with Teams:
...
Make sure the public HTTPS endpoint is reachable from the internet and uses a valid TLS certificate. Teams rejects self-signed certificates. Keep the Moor listener behind the proxy; port `3978` does not serve HTTPS itself.
...
| `requirements not met` / `Teams SDK missing` / `No adapter available for teams` | Restart gateway so lazy-install can run, or install into the **Moor venv**: `~/.moor/moor-agent/venv/bin/pip install microsoft-teams-apps aiohttp`. System `pip` fails on Ubuntu 24.04 (PEP 668) and would not affect the service anyway |
...
| `No inference provider configured` | Check that `ANTHROPIC_API_KEY` (or another provider key) is set in `~/.moor/.env` |
...
| Tunnel URL changes on restart | devtunnel URLs are persistent if you use a named tunnel (`devtunnel create moor-bot`). ngrok and cloudflared generate a new URL each run unless you have a paid plan — update the bot endpoint with `teams app update` when it changes |
| Teams shows "This bot is not responding" | The webhook returned an error. Check `docker logs moor` / `moor gateway status -l` for tracebacks |
...
- Store credentials in `~/.moor/.env` with permissions `600` (`chmod 600 ~/.moor/.env`)
