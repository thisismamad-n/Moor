...
moor photon setup --phone +15551234567
```
...
6. **Runs `npm install`** inside the plugin's sidecar directory. On
   ...
   `~/.moor/photon/sidecar`; set `PHOTON_SIDECAR_DIR` to pin an
   explicit location.

Runtime credentials are written to `~/.moor/.env`
...
(device token, dashboard project id) lives in `~/.moor/auth.json` under
...
```bash
moor pairing approve photon <CODE>
```

Use `moor pairing list` to see pending codes and approved users.

**Pre-authorize specific numbers** (in `~/.moor/.env`):

```bash
PHOTON_ALLOWED_USERS=+15551234567,+15559876543
```

**Open access** (dev only, in `~/.moor/.env`):
...
- **`sidecar deps : ✗ run moor photon install-sidecar`** — Node is
  installed but `spectrum-ts` isn't. Run the suggested command.
- **`device token : ✗ missing`** — run `moor photon setup` to log in.
- **`No iMessage line assigned yet`** — Spectrum is enabled but no line
  has been provisioned; re-run `moor photon setup` or check the
  [dashboard][app].
- **Sidecar won't start** — confirm `node --version` is 18.17+ and that
  `moor photon install-sidecar` completed without errors.
...
- **Read receipts are supported.** The sidecar marks an inbound iMessage
  read after forwarding it to Moor, so the sender sees `Read` without
  waiting for a model/tool turn. Inbound receipts for moor-sent messages
...
- **Cron and standalone sends need the gateway running.** Out-of-process
  senders (cron jobs, `moor send`, the dashboard) reuse the sidecar the
  gateway spawned — they read its port/token from
  `<moor-home>/runtime/photon-sidecar.json`, written once the sidecar
