...
sidebar_position: 12
title: "Google Chat"
description: "Set up Moor Agent as a Google Chat bot using Cloud Pub/Sub"
---

# Google Chat Setup

Connect Moor Agent to Google Chat as a bot. The integration uses Cloud Pub/Sub
pull subscriptions for inbound events and the Chat REST API for outbound messages.
Equivalent ergonomics to Slack Socket Mode or Telegram long-polling: your Moor
...
> Run `moor gateway setup` and pick **Google Chat** for a guided walk-through.
...
download the file. Save it somewhere only Moor can read (e.g.,
`~/.moor/google-chat-sa.json`, `chmod 600`).
...
- Delivery type: **Pull**
- Message retention: **7 days** (so backlog survives a moor restart)
...
- Principal: `moor-chat-bot@<your-project>.iam.gserviceaccount.com`
- Role: `Pub/Sub Subscriber`

Also grant `Pub/Sub Viewer` on the same subscription — Moor calls
...
- **App name**: whatever you want users to see ("Moor" is reasonable).
...
- **Connection settings**: select **Cloud Pub/Sub**, enter the topic name
  `projects/<your-project>/topics/moor-chat-events`.
...
`ADDED_TO_SPACE` event that Moor uses to cache the bot's own `users/{id}` for
self-message filtering.

---

## Step 9: Configure Moor

Add the Google Chat section to `~/.moor/.env`:
...
On Docker / hosted images `/opt/moor/.venv` is read-only. That installer
routes through `tools.lazy_deps` into `MOOR_LAZY_INSTALL_TARGET`
...
Send "hola" in the test DM. The bot posts a "Moor is thinking…" marker, then
...
`~/.moor/config.yaml` — e.g. a kitten assistant named Ada:

```yaml
platforms:
  google_chat:
    # Custom working-state marker text (default: "Moor is thinking…").
...
Thread support: when a user replies inside a thread, Moor detects the
`thread.name` and posts its reply in the same thread, so each thread gets a
separate Moor session.
...
3. Download the JSON. Move it onto the host that runs Moor.
4. Register the client with Moor (run under the profile you want it scoped to):
...
moor -p <profile> python -m plugins.platforms.google_chat.oauth \
    --client-secret /path/to/client_secret.json
```

That writes the client secret into the active profile's Moor home (e.g.
`~/.moor/google_chat_user_client_secret.json` for the default profile). The
...
The token lands at `~/.moor/google_chat_user_tokens/<sanitized_email>.json`.
...
single-user token at `~/.moor/google_chat_user_token.json` (if present from
...
1. Check the Pub/Sub subscription has undelivered messages in the console.
   If it does, Moor isn't authenticated — verify `GOOGLE_CHAT_SERVICE_ACCOUNT_JSON`
...
3. Check `moor gateway` logs for `[GoogleChat] Connected`. If you see
...
- **Attachment download protection**: Moor will only attach the SA bearer
...
- **User OAuth scope**: the per-user attachment flow requests *only*
  ...
  `~/.moor/google_chat_user_tokens/<sanitized_email>.json` (filesystem
