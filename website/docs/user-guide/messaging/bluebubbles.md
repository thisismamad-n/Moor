...
Connect Moor to Apple iMessage via [BlueBubbles](https://bluebubbles.app/) — a free, open-source macOS server that bridges iMessage to any device.
...
- Network connectivity between Moor and the BlueBubbles server
...
Or set environment variables directly in `~/.moor/.env`:
...
By default, Moor responds to every authorized BlueBubbles/iMessage DM or group message. To make group chats opt-in, enable mention gating:
...
With `require_mention: true`, DMs still work normally, but group-chat messages are ignored unless they match a mention pattern. If you do not configure custom patterns, Moor uses conservative defaults for `Moor` and `@Moor agent` variants.
...
When someone messages your iMessage, Moor automatically sends them a pairing code. Approve it with:
```bash
moor pairing approve bluebubbles <CODE>
```
Use `moor pairing list` to see pending codes and approved users.

**Pre-authorize specific users** (in `~/.moor/.env`):
```bash
BLUEBUBBLES_ALLOWED_USERS=user@icloud.com,+15551234567
```

**Open access** (in `~/.moor/.env`):
```bash
...
```
iMessage → Messages.app → BlueBubbles Server → Webhook → Moor
Moor → BlueBubbles REST API → Messages.app → iMessage
```

- **Inbound:** BlueBubbles sends webhook events to a local listener when new messages arrive. No polling — instant delivery.
- **Outbound:** Moor sends messages via the BlueBubbles REST API.
...
| `BLUEBUBBLES_MENTION_PATTERNS` | No | Moor wake words | JSON array, newline-separated, or comma-separated regex patterns for group mention matching |

Auto-marking messages as read is controlled by the `send_read_receipts` key under `platforms.bluebubbles.extra` in `~/.moor/config.yaml` (default: `true`). There is no corresponding environment variable.
...
### Chat Addressing
You can address chats by email or phone number — Moor resolves them to BlueBubbles chat GUIDs automatically. No need to use raw GUID format.
