...
sidebar_position: 3
title: "Discord"
description: "Set up Moor Agent as a Discord bot"
---

# Discord Setup

Moor Agent integrates with Discord as a bot, letting you chat with your AI assistant through direct messages or server channels. The bot receives your messages, processes them through the Moor Agent pipeline (including tool use, memory, and reasoning), and responds in real time. It supports text, voice messages, file attachments, and slash commands.

Before setup, here's the part most people want to know: how Moor behaves once it's in your server.

## How Moor Behaves

| Context | Behavior |
|---------|----------|
| **DMs** | Moor responds to every message. No `@mention` needed. Each DM has its own session. |
| **Server channels** | By default, Moor only responds when you `@mention` it. If you post in a channel without mentioning it, Moor ignores the message. |
...
| **Threads** | Moor replies in the same thread. Mention rules still apply unless that thread or its parent channel is configured as free-response. Threads stay isolated from the parent channel for session history. |
| **Shared channels with multiple users** | By default, Moor isolates session history per user inside the channel for safety and clarity. Two people talking in the same channel do not share one transcript unless you explicitly disable that. |
| **Messages mentioning other users** | When `DISCORD_IGNORE_NO_MENTION` is `true` (the default), Moor stays silent if a message @mentions other users but does **not** mention the bot. This prevents the bot from jumping into conversations directed at other people. Set to `false` if you want the bot to respond to all messages regardless of who is mentioned. This only applies in server channels, not DMs. |

:::tip
If you want a normal bot-help channel where people can talk to Moor without tagging it every time, add that channel to `DISCORD_FREE_RESPONSE_CHANNELS`.
:::

### Discord Gateway Model

Moor on Discord is not a webhook that replies statelessly. It runs through the full messaging gateway, which means each incoming message goes through:

1. authorization (`DISCORD_ALLOWED_USERS`)
2. mention / free-response checks
3. session lookup
4. session transcript loading
5. normal Moor agent execution, including tools, memory, and slash commands
6. response delivery back to Discord

That matters because behavior in a busy server depends on both Discord routing and Moor session policy.
...
So if Alice and Bob both talk to Moor in `#research`, Moor treats those as separate conversations by default even though they are using the same visible Discord channel.
...
Moor tracks running agents by session key.
...
After the configured number of consecutive unhealthy samples, the adapter emits one retryable fatal event. A closed transport (`socket_closed` / `client_closed`) is a confirmed death and forces the reconnect on the first unhealthy sample; the threshold applies to soft signals only (stale heartbeat ACK, latency, event silence) — see [#118487](https://github.com/thisismamad-n/Moor/issues/118487). The existing gateway reconnect watcher creates a fresh adapter; the Discord adapter does not start a second unbounded reconnect loop.
...
3. Enter a name for your application (e.g., "Moor Agent") and accept the Developer Terms of Service.
4. Click **Create**.
...
The bot token is the credential Moor Agent uses to log in as your bot. Still on the **Bot** page:
...
After authorizing, the bot will appear in your server's member list (it will show as offline until you start the Moor gateway).

## Step 7: Find Your Discord User ID

Moor Agent uses your Discord User ID to control who can interact with the bot. To find it:
...
| `DISCORD_ALLOW_BOTS` | No | `"none"` | Controls how the bot handles messages from other Discord bots. `"none"` — ignore all other bots. `"mentions"` — only accept bot messages that `@mention` Moor. `"all"` — accept all bot messages. By default, either enabled mode still requires a literal inline mention; see the next setting. |
...
| `MOOR_DISCORD_TEXT_BATCH_SPLIT_DELAY_SECONDS` | No | `2.0` | Longer quiet period for near-limit Discord splits and continuation chunks from a recently tagged bot in the same channel. |
...
- If a message arrives inside a thread or forum post and that thread has no explicit entry, Moor falls back to the parent channel/forum ID.
...
When `true`: Alice and Bob talking in `#research` each have their own separate conversation with Moor. When `false`: the entire channel shares one conversation transcript and one running-agent slot.
...
Moor automatically registers installed skills as **native Discord Application Commands**. This means skills appear in Discord's autocomplete `/` menu alongside built-in commands.
...
If you run multiple Moor gateways against the same Discord application (e.g. staging + production), only one of them should own the global slash-command registration — otherwise the last startup wins and the registrations flap. Turn slash registration off on the "follower" gateway:
...
Discord's per-upload size limit depends on the server's boost tier (25 MB free, up to 500 MB). If Moor gets an HTTP 413, the adapter falls back to a link pointing at the local cache path rather than failing silently.
...
Moor Agent supports Discord voice messages:
...
- **Discord voice channels**: Moor can also join a voice channel, listen to users speaking, and talk back in the channel.

For the full setup and operational guide, see:
- [Voice Mode](../features/voice-mode.md)
- [Use Voice Mode with Moor](../../guides/use-voice-mode-with-moor.md)
...
discord.py plays only one audio stream per connection, so Moor installs a software mixer on the outgoing stream that sums an ambient loop, acknowledgements, and TTS replies into that single stream — they overlap instead of cutting each other off.
...
For more information on securing your Moor Agent deployment, see the [Security Guide](../security.md).
