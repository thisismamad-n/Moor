...
2. **Add** → name (e.g. `moor-bot`) → role: **Admin**.
...
6. Copy the token → update `WHATSAPP_CLOUD_ACCESS_TOKEN` in `~/.moor/.env` → restart the gateway.
...
2. Generate a **Verify Token** — the wizard does this for you with `secrets.token_urlsafe(32)`; if you're configuring manually, run:
   ...
   ```
   Save it as `WHATSAPP_CLOUD_VERIFY_TOKEN` in `~/.moor/.env`.
3. Start the Moor gateway: `moor gateway`.
...
The `moor whatsapp-cloud` wizard prints these links at the end of setup. None of this is required for the bot to work — it's pure polish for how your bot appears to users.

---

## Configuration reference

All settings live in `~/.moor/.env`.  Required values are in **bold**.
...
- **Voice notes** — auto-downloaded as `.ogg`, transcribed via your configured STT provider (local faster-whisper, OpenAI/Moor, Groq, etc.), then handed to the agent as text.
...
- **Verify token mismatch** — the token in `~/.moor/.env`'s `WHATSAPP_CLOUD_VERIFY_TOKEN` must match exactly what you typed into Meta's dashboard.  Run the curl probe above to confirm the gateway's verify handshake works locally first.
- **Gateway not running** — check `moor gateway` is up.
...
Common cause: the toolset configured for `whatsapp_cloud` is missing the tools the agent wants to call.  Check `moor tools list` and verify the platform is using `moor-whatsapp` (the default Cloud adapter toolset, same as Baileys).

If the model emits tool-call-shaped text instead of a structured call, it usually means the toolset was effectively empty.  See `moor_cli/platforms.py` for the platform → default toolset mapping.
...
| | Baileys (`moor whatsapp`) | Cloud API (`moor whatsapp-cloud`) |
|---|---|---|
