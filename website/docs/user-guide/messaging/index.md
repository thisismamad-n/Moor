...
description: "Chat with Moor from Telegram, Discord, Slack, WhatsApp, Signal, SMS, Email, Home Assistant, Mattermost, Matrix, DingTalk, Yuanbao, Microsoft Teams, LINE, Raft, Webhooks, or any OpenAI-compatible frontend via the API server — architecture and setup overview"
---
...
For the full voice feature set — including CLI microphone mode, spoken replies in messaging, and Discord voice-channel conversations — see [Voice Mode](/user-guide/features/voice-mode) and [Use Voice Mode with Moor](/guides/use-voice-mode-with-moor).

:::tip
Bots need both a model provider and tool providers (TTS, web). A [Moor Portal](/integrations/moor-portal) subscription bundles all of them.
:::
...
For group chats, hooks, and automation flows, Moor supports explicit silence tokens. If the agent's final response is exactly one supported token, the gateway suppresses outbound delivery and sends nothing to the chat.
...
Silence is a delivery decision only. Moor keeps the assistant silence turn in the session transcript, so the conversation still alternates normally:
...
Failed turns still surface as errors; Moor does not hide failures just because the text resembles a silence token.
...
```bash
moor gateway setup        # Interactive setup for all messaging platforms
```
...
moor gateway setup        # Configure messaging platforms interactively
moor gateway install      # Install as a user service (Linux) / launchd service (macOS)
sudo moor gateway install --system   # Linux only: install a boot-time system service
moor gateway start        # Start the default service
moor gateway stop         # Stop the default service
moor gateway status       # Check default service status
moor gateway status --system         # Linux only: inspect the system service explicitly
```
...
with the `session_reset` section in `~/.moor/config.yaml`:
...
Configure per-platform overrides in `~/.moor/gateway.json`:
...
Different channels can run different models and personas from a **single gateway** — e.g. a cheap fast model in `#daily` and a frontier model with a specialist prompt in `#dev`. Configure `channel_overrides` under the platform in `~/.moor/gateway-config.yaml`:
...
moor pairing approve telegram XKGH5N7P

# Other pairing commands:
moor pairing list          # View pending + approved users
moor pairing revoke telegram 123456789  # Remove access
```
...
Control how much tool activity is displayed in `~/.moor/config.yaml`:
...
Setting `display.tool_progress: log` sends **no** progress bubbles to chat. Instead, each tool call is appended as a line to `~/.moor/logs/tool_calls.log` — a rotating audit file (5 MB × 3 backups) run through the same secret-redacting formatter as regular logs, so credentials never land on disk. Use it when you want a full tool-call trail without any chat noise.

### Configurable status phrases

Long-running gateway status lines ("still working…"-style heartbeats) draw from a phrase catalog. Built-in defaults ship in `gateway/assets/status_phrases.yaml`; you can add your own with profile-portable files under `MOOR_HOME`:

- `~/.moor/status_phrases.yaml` or any `*.yaml` in `~/.moor/status_phrases/` (conventional paths, auto-loaded), or
...
Off by default. When enabled, Moor prepends a human-readable timestamp
...
When the agent running a background session uses `terminal(background=true)` to start long-running processes (servers, builds, etc.), the gateway can push status updates to your chat. Control this with `display.background_process_notifications` in `~/.moor/config.yaml`:
...
```bash
MOOR_BACKGROUND_NOTIFICATIONS=result
```
...
moor gateway start                 # Start the service
moor gateway stop                  # Stop the service
moor gateway status                # Check status
journalctl --user -u moor-gateway -f  # View logs

# Enable lingering (keeps running after logout)
sudo loginctl enable-linger $USER

# Or install a boot-time system service that still runs as your user
sudo moor gateway install --system
sudo moor gateway start --system
sudo moor gateway status --system
journalctl -u moor-gateway -f
```

Use the user service on laptops and dev boxes. Use the system service on VPS or headless hosts that should come back at boot without relying on systemd linger.

:::danger Don't add a custom `ExecStopPost` kill drop-in
The unit Moor installs already shuts the gateway down cleanly with `KillMode=mixed` + `KillSignal=SIGTERM`, and uses `Restart=always` with `RestartForceExitStatus` so updates and `/restart` respawn correctly. Do **not** add a systemd drop-in such as `ExecStopPost=/bin/kill -9 $MAINPID` — `ExecStopPost` fires on *every* stop, including clean restarts, so it `SIGKILL`s the freshly spawned instance before it stabilizes and `Restart=always` immediately respawns it. The result is an infinite restart loop (and, on Telegram, a flood of restart messages). If you've added such a drop-in, remove it: `systemctl --user edit moor-gateway` (or `sudo systemctl edit moor-gateway` for a system service) and delete the `ExecStopPost` line, then `systemctl --user daemon-reload`.
:::
...
```bash
moor gateway install          # user service
sudo loginctl enable-linger $USER   # one-time: start at boot, survive logout
```

After that, `moor update` can restart the gateway without any privileges. If you prefer to keep the system service, either run updates with `sudo moor update`, or grant the service account passwordless sudo for systemctl, e.g. in `sudo visudo -f /etc/sudoers.d/moor-gateway`:

```
moor ALL=(root) NOPASSWD: /usr/bin/systemctl --no-ask-password reset-failed moor-gateway*, /usr/bin/systemctl --no-ask-password start moor-gateway*, /usr/bin/systemctl --no-ask-password restart moor-gateway*
```
:::
...
If you run multiple Moor installations on the same machine (with different `MOOR_HOME` directories), each gets its own systemd service name. The default `~/.moor` uses `moor-gateway`; other installations use `moor-gateway-<hash>`. The `moor gateway` commands automatically target the correct service for your current `MOOR_HOME`.
:::

### macOS (launchd)

```bash
moor gateway install               # Install as launchd agent
moor gateway start                 # Start the service
moor gateway stop                  # Stop the service
moor gateway status                # Check status
tail -f ~/.moor/logs/gateway.log   # View logs
```

The generated plist lives at `~/Library/LaunchAgents/ai.moor.gateway.plist`. It includes three environment variables:
...
- **MOOR_HOME** — scopes the gateway to your Moor installation.

:::tip PATH changes after install
launchd plists are static — if you install new tools (e.g. a new Node.js version via nvm, or ffmpeg via Homebrew) after setting up the gateway, run `moor gateway install` again to capture the updated PATH. The gateway will detect the stale plist and reload automatically.
:::

:::info Multiple installations
Like the Linux systemd service, each `MOOR_HOME` directory gets its own launchd label. The default `~/.moor` uses `ai.moor.gateway`; other installations use `ai.moor.gateway-<suffix>`.
:::
...
|----------|---------|--------------|
| CLI | `moor-cli` | Full access |
| Telegram | `moor-telegram` | Full tools including terminal |
| Discord | `moor-discord` | Full tools including terminal |
| WhatsApp | `moor-whatsapp` | Full tools including terminal |
| WhatsApp Cloud API | `moor-whatsapp` | Full tools including terminal (shares toolset with the Baileys bridge) |
| Slack | `moor-slack` | Full tools including terminal |
| Google Chat | `moor-google_chat` | Full tools including terminal |
| Signal | `moor-signal` | Full tools including terminal |
| SMS | `moor-sms` | Full tools including terminal |
| Email | `moor-email` | Full tools including terminal |
| Home Assistant | `moor-homeassistant` | Full tools + HA device control (ha_list_entities, ha_get_state, ha_call_service, ha_list_services) |
| Mattermost | `moor-mattermost` | Full tools including terminal |
| Matrix | `moor-matrix` | Full tools including terminal |
| DingTalk | `moor-dingtalk` | Full tools including terminal |
| Feishu/Lark | `moor-feishu` | Full tools including terminal |
| WeCom | `moor-wecom` | Full tools including terminal |
| WeCom Callback | `moor-wecom-callback` | Full tools including terminal |
| Weixin | `moor-weixin` | Full tools including terminal |
| BlueBubbles | `moor-bluebubbles` | Full tools including terminal |
| QQBot | `moor-qqbot` | Full tools including terminal |
| Yuanbao | `moor-yuanbao` | Full tools including terminal |
| Microsoft Teams | `moor-teams` | Full tools including terminal |
| API Server | `moor-api-server` | Full tools (drops `clarify`, `text_to_speech` — programmatic access doesn't have an interactive user) |
| Webhooks | `moor-webhook` | Full tools including terminal |
| Raft | `moor-raft` | Wake-only channel; agent uses Raft CLI for message I/O |
...
`platforms.<name>.enabled: false` in `~/.moor/config.yaml` is authoritative.
...
1. **Gateway log** (`~/.moor/logs/gateway.log` or the systemd / launchd unit log). Search for the platform name and `circuit breaker`, `paused`, or `disabled`. The trip event includes the failure count and the last error.
