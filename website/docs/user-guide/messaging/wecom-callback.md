...
Connect Moor to WeCom (Enterprise WeChat) as a self-built enterprise application using the callback/webhook model.

:::info WeCom Bot vs WeCom Callback
Moor supports two WeCom integration modes:
...
> Run `moor gateway setup` and pick **WeCom Callback** for a guided walk-through.
...
3. Moor decrypts the message, queues it for the agent
...
(Use `moor gateway start` only after `moor gateway install` has registered the systemd/launchd service.)
...
console. A mismatch between the token configured in Moor and the token the
...
in `~/.moor/.env` values around `=` will also break signature checks. After
fixing, restart `moor gateway run`.
...
Check `moor gateway run` logs for the bound host/port. If the adapter bound to
