...
`~/.moor/config.yaml`, `~/.moor/.env`, and even the shell environment — for
...
Managed scope is read from a system-level directory, default `/etc/moor`:

```text
/etc/moor/
├── config.yaml     # managed config layer (wins over ~/.moor/config.yaml)
└── .env            # managed env layer (wins over ~/.moor/.env + shell)
```
...
The location can be relocated with the `MOOR_MANAGED_DIR` environment variable
(for containers or non-`/etc` deployments). This is a deployment/bootstrap path
knob — like `MOOR_HOME` — set by the same administrator who owns the managed
files. It is **never persisted** to any `.env` by Moor.

```bash
# Point managed scope at a custom directory (set by IT / the deployment, not the user)
export MOOR_MANAGED_DIR=/opt/org/moor-policy
```

:::warning
A user who can set `MOOR_MANAGED_DIR` can repoint managed scope at a directory
...
left user-settable. `moor doctor` reports the *resolved* managed directory so
...
|---|---|---|
| 1 | `/etc/moor/config.yaml` (managed) | `/etc/moor/.env` (managed) |
| 2 | `~/.moor/config.yaml` (user) | `~/.moor/.env` (user) |
...
```bash
moor config        # shows a header naming the managed source + the pinned keys
moor doctor        # reports the resolved managed dir + pinned key counts
```

If you try to change a managed value, Moor refuses and names the source:

```bash
$ moor config set model.default my/model
Cannot set 'model.default': it is managed by your administrator
(/etc/moor/config.yaml) and cannot be changed.
```

The same applies to managed secrets — `moor config set` / setup will not write
a user value for an env key pinned by the managed `.env`.

## Setting up a managed scope (administrators)

```bash
sudo mkdir -p /etc/moor

# Pin some config values for every user on this machine
sudo tee /etc/moor/config.yaml >/dev/null <<'YAML'
model:
  provider: moor
security:
  redact_secrets: true
...
sudo tee /etc/moor/.env >/dev/null <<'ENV'
OPENAI_API_BASE=https://inference.example.com/v1
ENV

sudo chmod 0755 /etc/moor
sudo chmod 0644 /etc/moor/config.yaml /etc/moor/.env
```

Changes take effect on the next Moor start (a malformed managed file is logged
loudly and ignored — it never blocks startup, but the admin should check
`moor doctor` to confirm the policy is being applied).

## Security model and limitations (v1)

- **Enforcement is filesystem permissions only.** If a user has write access to
  the managed directory (or runs Moor as `root`), managed scope is advisory.
