...
Moor Agent automatically saves every conversation as a session. Sessions enable conversation resume, cross-session search, and full conversation history management.
...
1. **SQLite database** (`~/.moor/state.db`) — structured session metadata with FTS5 full-text search, plus full message history
...
Moor stores session history so it can resume conversations, but it does not
...
Moor explicitly injects for that turn.
...
For example, if a user sends an image and asks Moor to make a meme from it,
Moor may inspect that image once with vision and run an image-processing
...
A bare `-c` is terminal-aware: each CLI session drops a small breadcrumb file under `~/.moor/terminal-sessions/` keyed by the terminal it runs in (tty device, tmux pane, kitty window, wezterm pane, Zellij pane, Windows Terminal session, ...). When you run `moor -c` again in the *same* terminal, Moor resumes that terminal's own session — so two panes side by side each continue their own conversation instead of both grabbing the globally most-recent one. If there's no breadcrumb for the terminal (first use, deleted session, or a stale breadcrumb older than 30 days), `-c` falls back to the most-recent-session behavior. `-c "name"` and `--resume` are unaffected. Disable with `session.terminal_continue: false` in `config.yaml`.
...
moor -c "my project"   # → resumes "my project #3"
```
...
moor --resume 20250305_091523_a1b2c3d4
moor -r 20250305_091523_a1b2c3d4

# Resume by title
moor --resume "refactoring auth"

# Resume the most recent session — same lookup as -c
moor --resume latest

# Or with the chat subcommand
moor chat --resume 20250305_091523_a1b2c3d4
```

Session IDs are shown when you exit a CLI session, and can be found with `moor sessions list`.
...
moor --resume latest --in ./my-project

# Works with the TUI too
moor --tui --resume latest --in ./my-project
```
...
```bash
moor --resume 20250305_091523_a1b2c3 --no-restore-cwd
```
...
`moor sessions list` accepts `--workspace <needle>` to show only sessions whose workspace key (git repo root, else cwd) matches — by path substring or exact directory basename:

```bash
moor sessions list --workspace my-project
moor sessions list --workspace ~/code/moor-agent
```

### Conversation Recap on Resume

When you resume a session, Moor displays a compact recap of the previous conversation in a styled panel before the input prompt:

<img className="docs-terminal-figure" src={useBaseUrl('/img/docs/session-recap.svg')} alt="Stylized preview of the Previous Conversation recap panel shown when resuming a Moor session." />
...
To disable the recap and keep the minimal one-liner behavior, set in `~/.moor/config.yaml`:
...
**Resume back to CLI:** when you want to come back to a desktop, just run `/resume <title>` (or `moor -r "<title>"` from the shell) and pick up where the platform left off.
...
Moor automatically generates a short descriptive title (3–7 words) for each session after the first exchange. This runs in a background thread using a fast auxiliary model, so it adds no latency. You'll see auto-generated titles when browsing sessions with `moor sessions list` or `moor sessions browse`.
...
```bash
moor sessions rename 20250305_091523_a1b2c3d4 "refactoring auth module"
```
...
When a session's context is compressed (manually via `/compress` or automatically), Moor creates a new continuation session. If the original had a title, the new session automatically gets a numbered title:

```
"my project" → "my project #2" → "my project #3"
```

When you resume by name (`moor -c "my project"`), it automatically picks the most recent session in the lineage.
...
Moor provides a full set of session management commands via `moor sessions`:
...
# Filter by platform
moor sessions list --source telegram
...
`moor sessions export` is one surface for every export format, selected with `--format`:
...
moor sessions export backup.jsonl

# Export sessions from a specific platform
moor sessions export telegram-history.jsonl --source telegram
...
moor sessions export --format html --session-id 20250305_091523_a1b2c3d4 transcript.html

# All Telegram sessions from the last week in one file, secrets redacted
moor sessions export --format html --newer-than 1w --source telegram --redact archive.html
```
...
moor sessions export prompts.jsonl --session-id 20250305_091523_a1b2c3d4 --only user-prompts

# Markdown, straight to stdout
moor sessions export - --session-id 20250305_091523_a1b2c3d4 --only user-prompts --format md
```
...
`--format trace` emits Claude Code JSONL — the transcript shape the Hugging Face Hub auto-detects for its [Agent Trace Viewer](https://huggingface.co/docs/hub/agent-traces). Write it locally, or add `--upload` to push it to your own private `moor-traces` dataset (reads `HF_TOKEN`):

```bash
# Trace of the most recent session, to stdout
moor sessions export --format trace

# One session to a local trace file
moor sessions export --format trace --session-id 20250305_091523_a1b2c3d4 trace.jsonl

# Upload straight to your private HF traces dataset
moor sessions export --format trace --session-id 20250305_091523_a1b2c3d4 --upload
```
...
Pass `--format md` or `--format qmd` when you want a readable, file-based archive before hiding or deleting old sessions. Markdown/QMD exports write one file per session into a directory (default: `~/.moor/session-exports`).

```bash
# Export one session to Markdown
moor sessions export --format md --session-id 20250305_091523_a1b2c3d4

# Export a compression lineage as one logical document
moor sessions export --format md --session-id 20250305_091523_a1b2c3d4 --lineage logical

# Preview ended sessions older than 90 days without writing files
moor sessions export --format md --older-than 90 --dry-run

# Export ended Telegram sessions older than 2 weeks to QMD files
moor sessions export --format qmd --older-than 2w --source telegram

# Export long Claude sessions, secrets redacted
moor sessions export --format md --model sonnet --min-messages 50 --redact

# Only after verification, export and delete one explicitly named session
moor sessions export --format md --session-id 20250305_091523_a1b2c3d4 --delete-after-verified --yes
```
...
moor sessions delete 20250305_091523_a1b2c3d4

# Delete without confirmation
moor sessions delete 20250305_091523_a1b2c3d4 --yes
```
...
moor sessions rename 20250305_091523_a1b2c3d4 "debugging auth flow"
...
moor sessions pin 20250305_091523_a1b2c3d4
moor sessions pin 20250305 20250306

# Remove the pin
moor sessions unpin 20250305_091523_a1b2c3d4

# List pinned sessions
moor sessions pinned

# Machine-readable output, e.g. for a nightly backup of your pin set
moor sessions pinned --json > pinned-sessions.json
```
...
moor sessions prune --older-than 30

# Durations work too: 5h, 30m, 2d, 1w
moor sessions prune --older-than 12h

# Delete only a specific time window (e.g. a batch of test sessions
# created in the last 5 hours)
moor sessions prune --newer-than 5h

# Explicit window with absolute timestamps
moor sessions prune --after "2026-07-05 09:00" --before "2026-07-05 14:30"

# Only prune sessions from a specific platform (all ages — any filter
# disables the implicit 90-day default)
moor sessions prune --source telegram
moor sessions prune --source cron --older-than 60   # add a time flag to narrow

# More filters — all AND together
moor sessions prune --newer-than 5h --title "smoke test"   # title substring
moor sessions prune --older-than 30 --max-messages 3        # tiny sessions
moor sessions prune --cwd ~/scratch --end-reason done       # by cwd / end reason
moor sessions prune --model gpt-5 --older-than 1w           # by model (substring)
moor sessions prune --provider openrouter --older-than 60   # by billing provider
moor sessions prune --branch feature/old-experiment         # by git branch
moor sessions prune --user 12345678 --chat-type group       # by messaging origin
moor sessions prune --max-tokens 500 --older-than 7         # by token usage
moor sessions prune --max-cost 0.01 --max-tool-calls 0      # cheap, tool-less runs

# Preview what would be deleted, without deleting anything
moor sessions prune --newer-than 5h --dry-run

# Skip confirmation
moor sessions prune --older-than 30 --yes
```
...
the implicit 90-day default, so `moor sessions prune --source cron` or
`--model gpt-4o` matches all ages — add a time flag to narrow it. Only a
completely bare `moor sessions prune` keeps the 90-day cutoff. Every
...
`moor sessions archive` takes the same filters as `prune` but soft-hides
...
moor sessions archive --newer-than 5h

# Archive by title substring, preview first
moor sessions archive --title "dry run" --dry-run
...
For deeper analytics — token usage, cost estimates, tool breakdown, and activity patterns — use [`moor insights`](/reference/cli-commands#moor-insights).
...
`moor sessions repair-routing` finds message-bearing session rows with no
...
moor sessions repair-routing --apply

# Widen/narrow the contiguity window (default 900 seconds)
moor sessions repair-routing --max-gap-seconds 300
```
...
(`cp ~/.moor/state.db ~/.moor/state.db.bak`).
...
moor sessions import

# Limit to one tool, or point at a specific file
moor sessions import --from claude
moor sessions import --from codex ~/.codex/sessions/2026/08/15/rollout-....jsonl
...
`moor sessions import` creates a new Moor session titled
`Imported from Claude Code: <first user message>` (or Codex CLI) and prints
the id plus a ready-to-paste `moor --resume <id>` command.
...
When Moor cannot get a participant identifier for a shared chat, it falls back to one shared session for that room.

### Shared vs Isolated Group Sessions

By default, Moor uses `group_sessions_per_user: true` in `config.yaml`. That means:

- Alice and Bob can both talk to Moor in the same Discord channel without sharing transcript history
...
|------|------|-------------|
| SQLite database | `~/.moor/state.db` | All session metadata + messages with FTS5 |
| Gateway messages    | `~/.moor/state.db`   | SQLite — canonical store for all session messages |
| Gateway routing index | `gateway_routing` table in `~/.moor/state.db` | Maps session keys to active session IDs (origin metadata, expiry flags) |
| Legacy routing mirror | `~/.moor/sessions/sessions.json` | Backward-compat mirror of the routing index, written when `gateway.write_sessions_json: true` (the default) |
...
`state.db`; `~/.moor/sessions/sessions.json` is a **legacy mirror** of it,
...
`moor sessions list`, `/sessions`, and the dashboard all read `state.db`,
which holds **every** session (CLI, TUI, and gateway). The `/save` snapshots
under `~/.moor/sessions/saved/*.json` are convenience exports, not the index.

If CLI sessions genuinely don't appear in `moor sessions list`, the cause is
`state.db` not receiving them — run `moor sessions repair` and watch for a
...
`*.jsonl` files in `~/.moor/sessions/`. They are no longer written or
read by Moor. Safe to delete after verifying the corresponding session
exists in state.db.
:::
...
- Pruning runs at most once per `sessions.min_interval_hours` (default 24); the last-run timestamp is tracked inside `state.db` itself so it's shared across every Moor process in the same `MOOR_HOME`

Without pruning, `state.db` grows without bound — multi-GB files within weeks were reported on gateway + cron installs. If you would rather keep every ended session forever (the pre-#54189 behavior), turn it off in `~/.moor/config.yaml`:
...
`in its tip segment`). `moor sessions export` still works for such sessions.
...
moor sessions delete <session_id>
