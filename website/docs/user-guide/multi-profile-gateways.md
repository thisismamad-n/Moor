...
(`ai.moor.gateway-<name>.plist`) or systemd user service
(`moor-gateway-<name>.service`). This guide adds the patterns for managing
them collectively.
...
With a multiplexer running, a named-profile `moor gateway start` / `run` is a
...
- `/p/coder/...` API-server requests must use `API_SERVER_KEY` from
  `~/.moor/profiles/coder/.env`; the default listener key is rejected.
...
home). `moor status` reports the multiplexer and the profiles it serves;
`moor status -p <name>` slices to one profile. Each profile still writes its
