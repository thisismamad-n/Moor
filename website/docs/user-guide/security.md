...
The approval system supports three modes, configured via `approvals.mode` in `~/.moor/config.yaml`:
...
| `destructive_slash_confirm` | `true` | When true, destructive session slash commands (`/clear`, `/new`, `/reset`, `/undo`) prompt before discarding conversation state. Three-option dialog (Approve Once / Always Approve / Cancel) routed through native yes/no buttons on Telegram, Discord, and Slack; text fallback elsewhere. Users who click **Always Approve** flip this key to `false`. The TUI also honors this setting for its `/clear`, `/new`, and `/reset` modal; `MOOR_TUI_NO_CONFIRM=1` force-skips that modal regardless of the configured value. |
...
1. **CLI flag**: Start a session with `moor --yolo` or `moor chat --yolo`
2. **Slash command**: Type `/yolo` during a session to toggle it on/off
3. **Environment variable**: Set `MOOR_YOLO_MODE=1`
...
YOLO mode is available in both CLI and gateway sessions. Internally, it sets the `MOOR_YOLO_MODE` environment variable which is checked before every command execution.
...
| `tee` to `/etc/`, `~/.ssh/`, `~/.moor/.env` | Overwrite sensitive file via tee |
| `>` / `>>` to `/etc/`, `~/.ssh/`, `~/.moor/.env` | Overwrite sensitive file via redirection |
...
| `pkill`/`killall` moor/gateway | Self-termination prevention |
...
The `MOOR_EXEC_ASK=1` environment variable is automatically set when running the gateway.

### Permanent Allowlist

Commands approved with "always" are saved to `~/.moor/config.yaml`:
...
:::tip
Use `moor config edit` to review or remove patterns from your permanent allowlist.
:::
...
```bash
moor approvals suggest            # dry run — prints a numbered proposal
moor approvals suggest --apply 1,3  # merge picks into command_allowlist
moor approvals suggest --json     # machine-readable output
```

The command scans the session database (`~/.moor/state.db`) for
...
These categories are always denied, even when `MOOR_WRITE_SAFE_ROOT` is unset:

| Category | Examples |
|----------|----------|
| OS credential stores | `~/.ssh/` (keys, `authorized_keys`), `~/.aws/`, `~/.kube/`, `/etc/sudoers`, `~/.netrc` |
| Moor credential stores | `auth.json`, `.env`, `.anthropic_oauth.json`, `mcp-tokens/`, `pairing/` under MOOR_HOME (active profile and global root) |
...
### MOOR_WRITE_SAFE_ROOT (optional sandbox)
...
Unset the variable to restore unrestricted writes (subject to the protected-path denylist). Full reference: [MOOR_WRITE_SAFE_ROOT](../reference/environment-variables.md#moor_write_safe_root).

### Cron and other Moor state

Do not ask the agent to `patch` `~/.moor/cron/jobs.json` directly. Use the `cronjob` tool, [`moor cron`](./features/cron.md), or `/cron` — they update the job store through the supported API. The same applies to other Moor control files when write safety blocks direct edits.
...
Set allowed user IDs as comma-separated values in `~/.moor/.env`:
...
Set GATEWAY_ALLOW_ALL_USERS=true in ~/.moor/.env to allow open access,
...
3. The bot owner runs `moor pairing approve <platform> <code>` on the CLI
4. The user is permanently approved for that platform

Control how unauthorized direct messages are handled in `~/.moor/config.yaml`:
...
moor pairing approve telegram ABC12DEF

# Revoke a user's access
moor pairing revoke telegram 123456789

# Clear all pending codes
moor pairing clear-pending
```

:::tip Docker users: run pairing commands as the `moor` user
The official Docker image runs the gateway as the unprivileged `moor` user
...
```bash
docker exec -u moor moor-agent moor pairing approve telegram ABC12DEF
```
...
Container resources are configurable in `~/.moor/config.yaml`:
...
- **Persistent mode** (`container_persistent: true`): Bind-mounts `/workspace` and `/root` from `~/.moor/sandboxes/docker/<task_id>/`
...
Some skills need **files** (not just env vars) in the sandbox — for example, Google Workspace stores OAuth tokens as `google_token.json` under the active profile's `MOOR_HOME`. Skills declare these in frontmatter:
...
When loaded, Moor checks if these files exist in the active profile's `MOOR_HOME` and registers them for mounting:
...
4. **Store secrets securely** — keep API keys in `~/.moor/.env` with proper file permissions
...
9. **Monitor logs** — check `~/.moor/logs/` for unauthorized access attempts
10. **Keep updated** — run `moor update` regularly for security patches
...
For maximum security, run the gateway on a separate machine or VM. Set `terminal.backend: ssh` in `config.yaml`, then provide host details via environment variables in `~/.moor/.env`:
...
TERMINAL_SSH_USER=moor
TERMINAL_SSH_KEY=~/.ssh/moor_agent_key
```
...
Moor ships with a built-in advisory scanner that flags Python packages in the active venv that match a curated catalog of known-compromised versions (supply-chain worms like the May 2026 `mistralai 2.4.6` poisoning). Implementation lives in `moor_cli/security_advisories.py`.

How it runs:

- **CLI startup banner.** A one-line warning is printed if any advisory matches, with a pointer to `moor doctor` for the full remediation.
- **`moor doctor`.** Surfaces every active advisory with version specifics and 2-4 step remediation instructions.
...
Many features (Mistral TTS, ElevenLabs, Honcho memory, Bedrock, Slack, Matrix, …) depend on Python packages that not every user needs. Moor installs these **lazily** on first use rather than eagerly under `moor-agent[all]`. The implementation lives in `tools/lazy_deps.py`.
...
3. If the install fails or the user has disabled lazy installs, the call raises `FeatureUnavailable` with the actual pip stderr and a pointer at `moor tools`.
...
When disabled, backends that need optional deps will tell the user to run the install manually (`pip install …`) or pick a different backend via `moor tools`.
