...
list of Moor gateways. Its intro says it plainly: *"Manage this device and
every Moor gateway it can reach through remote, SSH, or Cloud connections."*
...
- **Duplicates are rejected when you save**: there is only ever one **local**
  ...
  **SSH** entries are deduplicated on the normalized `user@host:port` plus
  remote profile.
- Cloud entries normally come from the Moor Cloud sign-in/discovery flow at
  the top of the Gateways page — the **Moor Cloud** kind in the add-connection
   ...
   - *Remote gateway only:*
     ...
     - **Authentication** — choose **Session token** or **OAuth**:
       - **Session token** — paste the dashboard session token from the
         ...
         token."*
       - **OAuth** — sign in through the Moor Portal browser flow; no token
         to paste.
   - *SSH only:*
...
Gateways are machines or hosted backends; profiles are isolated Moor agents
...
connection is registered) dispatches `moor update` to every eligible
...
panel, ⌘K **Update Moor**, the update-ready toast) run the same fan-out
...
- **"Connection test failed"** — the backend isn't reachable at that URL from
  this machine. Check that `moor serve` is running on the remote host, the
