...
| | |
|---|---|
| Source | Optional — install with `moor skills install official/creative/touchdesigner-mcp` |
...
```
Moor Agent -> MCP (Streamable HTTP) -> twozero.tox (port 40404) -> TD Python
```
...
```bash
bash "${MOOR_HOME:-$HOME/.moor}/skills/creative/touchdesigner-mcp/scripts/setup.sh"
```

The script will:
1. Check if TD is running
2. Download twozero.tox if not already cached
3. Add `twozero_td` MCP server to Moor config (if missing)
...
3. **Restart Moor session** to pick up the new MCP server
