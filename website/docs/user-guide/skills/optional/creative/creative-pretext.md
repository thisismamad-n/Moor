...
| | |
|---|---|
| Source | Optional — install with `moor skills install official/creative/pretext` |
...
This skill exists so Moor can make **cool demos** with it — the kind people post to X. See `pretext.cool` and `chenglou.me/pretext` for the community demo corpus.
...
2. **Start from a template**:
   ...
   - `write_file` to a new `.html` in `~/.moor/cache/scratch/` or the user's workspace.
