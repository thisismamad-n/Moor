...
| Related skills | [`moor-agent`](../../bundled/autonomous-ai-agents/autonomous-ai-agents-moor-agent.md), [`mcporter`](../../optional/mcp/mcp-mcporter.md), [`fastmcp`](../../optional/mcp/mcp-fastmcp.md) |
...
# MCP OAuth on a Remote Moor Gateway

## Overview

Moor' built-in MCP OAuth client runs a one-shot HTTP listener on `127.0.0.1:<port>`
inside the Moor process and registers that loopback address as the OAuth
`redirect_uri`. That works perfectly for a local CLI on the user's own machine.
It breaks completely when Moor runs as a remote gateway (container, VPS,
messaging bot), because the user's browser resolves `127.0.0.1` to the user's own
laptop, not the remote container — so the authorization code never reaches Moor.

This skill does the OAuth dance by hand and writes the resulting tokens into the
exact files Moor' token storage expects, so a subsequent `/reload-mcp` finds
...
2. Moor is running as a **remote gateway** (container, VPS, Docker, managed service) — NOT a local CLI on the user's laptop.
...
Do NOT use this for:
- **Local CLI Moor** — just set `auth: oauth` in `mcp_servers.<name>` and `/reload-mcp`. The built-in flow opens a browser and captures the callback on localhost. Works perfectly.
...
already cover the deployment. When Moor detects a remote session it prints two
...
Both require an interactive terminal to the Moor host. The rest of this skill
is for when there is NO interactive TTY — Moor running purely as a messaging
...
```bash
env | grep -iE "MOOR|RAILWAY|CONTAINER"
...
```json
{
  "client_name": "Moor Agent (manual OAuth)",
...
**Stash `code_verifier` and `state` to disk** (e.g. `~/.moor/cache/scratch/.mcp-oauth-work/<server>.json`,
...
UA** — Cloudflare will 403 you even though Moor (which uses httpx) will succeed.
...
1. **Do not assume "headless" means "OAuth impossible."** The built-in flow works fine for local CLI; the issue is strictly remote deployments where the user's browser and the Moor process are on different machines. Check the execution environment before claiming OAuth isn't an option.
...
11. **Client info file is NOT optional.** Moor needs `<server>.client.json` to know the `client_id` for refresh grants. Skipping it means the first refresh fails and the user has to re-auth — writing both files is the whole point of this skill.
...
13. **Security: the stash file contains the `code_verifier`.** Delete `~/.moor/cache/scratch/.mcp-oauth-work/<server>.json` immediately after successful token exchange. There's no reason to keep a proof-of-identity secret around once it's consumed.
...
- **Don't use `mcp-remote` as a fallback.** It runs an npx subprocess whose OAuth callback server ALSO sits on the remote container's localhost — same problem. `mcp-remote` only helps when the MCP client doesn't speak remote HTTP at all (Moor does natively).
...
- **Don't claim Moor doesn't support a feature without reading the source.** Grep the source tree before making capability claims.
...
- `native-mcp` — general guide to configuring MCP in Moor. Authoritative config reference lives there.
- `mcporter` — the external CLI bridge, for ad-hoc MCP calls outside of Moor' config.
