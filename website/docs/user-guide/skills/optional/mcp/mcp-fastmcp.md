...
| Related skills | [`moor-agent`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-moor-agent), [`mcporter`](/docs/user-guide/skills/optional/mcp/mcp-mcporter) |
...
- smoke-test a server with the FastMCP CLI before wiring it into Moor or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to Moor. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.moor/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.moor/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is Moor integration, either:

- configure the server in `~/.moor/config.yaml` using the `native-mcp` skill, or
...
### Moor cannot see the deployed server

The server-building part may be correct while the Moor config is not. Load the `native-mcp` skill and configure the server in `~/.moor/config.yaml`, then restart Moor.
