...
| | |
|---|---|
| Source | Optional — install with `moor skills install official/security/1password` |
...
Set `OP_SERVICE_ACCOUNT_TOKEN` in `${MOOR_HOME:-~/.moor}/.env` (the skill will prompt for this on first load).
