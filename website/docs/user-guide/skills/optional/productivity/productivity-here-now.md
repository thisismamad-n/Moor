...
- Skill helper paths:
  - `${MOOR_SKILL_DIR}/scripts/publish.sh` for publishing sites
  - `${MOOR_SKILL_DIR}/scripts/drive.sh` for private Drive storage
...
| `--client {name}`      | Agent name for attribution (e.g. `moor`)    |
