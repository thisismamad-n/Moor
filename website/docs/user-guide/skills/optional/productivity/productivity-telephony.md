...
- save provider credentials into `${MOOR_HOME:-~/.moor}/.env`
...
```bash
SCRIPT="$(find ~/.moor/skills -path '*/telephony/scripts/telephony.py' -print -quit)"
```
...
3. Buy it and save it into `${MOOR_HOME:-~/.moor}/.env` + state:
```bash
python3 "$SCRIPT" twilio-buy "+17025551234" --save-env
```
...
3. persist that number to `${MOOR_HOME:-~/.moor}/.env`
