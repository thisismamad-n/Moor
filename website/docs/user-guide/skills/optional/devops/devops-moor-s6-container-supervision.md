---
title: "Moor S6 Container Supervision — Modify or debug s6 services in the Moor Docker image"
sidebar_label: "Moor S6 Container Supervision"
description: "Modify or debug s6 services in the Moor Docker image"
---

{/* This page is auto-generated from the skill's SKILL.md by website/scripts/generate-skill-docs.py. Edit the source SKILL.md, not this page. */}

# Moor S6 Container Supervision

Modify or debug s6 services in the Moor Docker image.
...
# Moor s6-overlay Container Supervision

## When to use this skill

Load this skill when you're working on:
- Adding or removing a static service in the Moor Docker image (something that should be supervised at every container start, like the dashboard)
- Diagnosing why a per-profile gateway isn't starting, restarting, or surviving `docker restart`
- Understanding why the container's CMD is `/opt/moor/docker/main-wrapper.sh` and how leading-dash args reach the user's program
...
If you're just running the Moor Agent and want to use Docker, see `website/docs/user-guide/docker.md` instead.
...
│   ├── 01-moor-setup                ← docker/stage2-hook.sh
│   │   ├── UID/GID remap
│   │   ├── chown /opt/data
│   │   ├── chown /opt/data/profiles (every boot)
│   │   ├── seed .env / config.yaml / SOUL.md
│   │   └── skills_sync.py
│   └── 02-reconcile-profiles          ← moor_cli.container_boot
│       ├── chown /run/service (moor-writable for runtime register)
│       └── walk $MOOR_HOME/profiles/<name>/gateway_state.json
│           → recreate /run/service/gateway-<name>/
│           → auto-start only those with prior_state == "running"
│
├── s6-rc.d (static services, in /etc/s6-overlay/s6-rc.d/)
│   ├── main-moor/run                ← exec sleep infinity (no-op slot)
│   └── dashboard/run                  ← if MOOR_DASHBOARD=1, runs `moor dashboard`
│
├── /run/service (s6-svscan watches; tmpfs)
│   ├── gateway-coder/                 ← runtime-registered per-profile
│   │   ├── type        ("longrun")
│   │   ├── run         ("#!/command/with-contenv sh ... exec s6-setuidgid moor moor -p coder gateway run")
│   │   ├── down        (marker — present means "registered but don't auto-start")
│   │   └── log/run     (s6-log → $MOOR_HOME/logs/gateways/coder/current)
│   └── ...
│
└── CMD ("main program")               ← /opt/moor/docker/main-wrapper.sh
    └── routes user args: bare exec | moor subcommand | moor (no args)
...
| `docker/s6-rc.d/main-moor/run` | No-op `sleep infinity` — slot exists so the s6-rc user bundle is valid; main moor runs as the CMD, not as a supervised service. |
| `docker/s6-rc.d/dashboard/run` | Conditional service — `exec sleep infinity` unless `MOOR_DASHBOARD` is truthy. |
...
| `moor_cli/service_manager.py` | `S6ServiceManager`: `register_profile_gateway`, `unregister_profile_gateway`, `start/stop/restart/is_running`, `list_profile_gateways`. |
| `moor_cli/container_boot.py` | `reconcile_profile_gateways()` — walks persistent profiles, regenerates s6 slots, emits `container-boot.log`. |
| `moor_cli/gateway.py::_dispatch_via_service_manager_if_s6` | Intercepts `moor gateway start/stop/restart` and routes to s6 when running in a container. |

## Why Architecture B (CMD as main program, not s6-supervised)

The original plan (v1–v3) called for main moor to run as a supervised s6-rc service. Two real s6-overlay v3 mechanics blocked that:
...
2. Drop to moor via `s6-setuidgid moor` at the top of run (unless you specifically need root).
...
Edit `S6ServiceManager._render_run_script` in `moor_cli/service_manager.py`. The function is also called by `moor_cli/container_boot.py::_register_service` during boot reconciliation, so it's the single source of truth. Update the corresponding assertion in `tests/moor_cli/test_service_manager.py::test_s6_register_creates_service_dir_and_triggers_scan`.

### Run the docker test harness

```sh
docker build -t moor-agent-harness:latest .
MOOR_TEST_IMAGE=moor-agent-harness:latest scripts/run_tests.sh tests/docker/ -v
...
`/command/` (where s6-overlay puts its binaries) is on PATH only for processes spawned by the supervision tree — services, cont-init.d, main-wrapper.sh. `docker exec <c> s6-svstat …` will fail with "command not found"; always use the absolute path `/command/s6-svstat`. The `moor` binary works because the Dockerfile adds `/opt/moor/.venv/bin` to the runtime `ENV PATH`.

### Profile directory ownership

The cont-init reconciler runs as moor (`s6-setuidgid moor` in `02-reconcile-profiles`). If a profile dir ends up root-owned (e.g. because `docker exec <c> moor profile create …` ran as root by default), the reconciler can't read SOUL.md and fails with `PermissionError`. Mitigation: `stage2-hook.sh` chowns `$MOOR_HOME/profiles` to moor on **every** boot, idempotently. Don't remove that block.

### Files written by `docker exec` are root-owned

`docker exec` defaults to root. Either pass `--user moor` or rely on the stage2 chown sweep next reboot. Don't write files under `$MOOR_HOME/profiles/<name>/` as root manually — the next reconcile pass will sweep them but in-flight operations may hit perm errors.
...
Most likely the profile has no model or auth configured. The service slot is correct — the gateway itself is unconfigured. Run `moor -p <profile> setup` first. The s6 supervisor will keep restarting it; that's the desired behavior (when you fix the config, the next attempt succeeds and stays up).

### Reconciler skipped a profile

The reconciler keys on the **presence of `SOUL.md`** as the "real profile" marker. `moor profile create` always seeds it. If a profile dir is missing SOUL.md (stray directory, partial restore, backup-in-progress), the reconciler skips it intentionally. Add a `SOUL.md` (even empty) to opt back in.
