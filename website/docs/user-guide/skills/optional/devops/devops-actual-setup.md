---
title: "Actual Setup — Set up Actual Computer (actual.inc) inference in Moor"
sidebar_label: "Actual Setup"
description: "Set up Actual Computer (actual.inc) inference in Moor"
---
...
Set up Actual Computer (actual.inc) inference in Moor.
...
Sets up [actual.inc](https://actual.inc) (Actual Computer) as a Moor inference
...
- User has an `ac_` key and wants Moor routed through their Actual cluster.
...
- Moor has **first-class `actual` provider support** (provider id `actual`,
  aliases `actual-computer`, `actualcomputer`, `aci`). Do NOT configure Actual
  as a `custom_providers` / `providers.actual.*` entry on current Moor — the
...
1. **reasoning_effort trap (handled by Moor since the first-class provider).**
   ...
   request still 400s this way on an old Moor, set a per-model cap:
   `agent.reasoning_overrides.<model>: high` in config.yaml.
2. **Context-window overflow on small local models.** Moor' default toolset
   ...
   servers emit a bare `data: [DONE]` — Moor reports
...
5. **Do not create a custom provider named `actual`.** Older setup guides
   ...
   current Moor the built-in provider wins the name; stale custom blocks
