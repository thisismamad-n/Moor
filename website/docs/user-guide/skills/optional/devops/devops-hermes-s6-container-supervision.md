---
title: "Moor S6 Container Supervision"
sidebar_label: "Moor S6 Container Supervision"
description: "Modify, debug, or extend the s6-overlay supervision tree inside the Moor Agent Docker image — adding new services, debugging profile gateways, understandin..."
---
...
Modify, debug, or extend the s6-overlay supervision tree inside the Moor Agent Docker image — adding new services, debugging profile gateways, understanding the Architecture B main-program pattern.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `moor skills install official/devops/hermes-s6-container-supervision` |
...
│   └── dashboard/run                  ← if HERMES_DASHBOARD=1, runs `moor dashboard`
│
...
| `hermes_cli/gateway.py::_dispatch_via_service_manager_if_s6` | Intercepts `moor gateway start/stop/restart` and routes to s6 when running in a container. |
...
- `hermes-tool-quirks`: Specific Moor-tool workarounds (sed/grep/etc.) — load when debugging the s6 stack's interaction with hermes built-in tools.
