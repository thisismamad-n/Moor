...
2. Clone the relevant repo (shallow clone to save time):
   ...
   git clone --depth 1 https://github.com/GPTomics/bioSkills.git ~/.moor/cache/scratch/bioSkills

   # ClawBio (runnable pipelines)
   git clone --depth 1 https://github.com/ClawBio/ClawBio.git ~/.moor/cache/scratch/ClawBio
   ```
3. Read the specific skill:
   ```bash
   # bioSkills — each skill is at: <category>/<skill-name>/SKILL.md
   cat ~/.moor/cache/scratch/bioSkills/variant-calling/gatk-variant-calling/SKILL.md

   # ClawBio — each skill is at: skills/<skill-name>/
   cat ~/.moor/cache/scratch/ClawBio/skills/pharmgx-reporter/README.md
   ```
...
- The fetched skills are NOT in Moor SKILL.md format. They use their own structure (bioSkills: code pattern cookbooks; ClawBio: README + Python scripts). Read them as expert reference material.
