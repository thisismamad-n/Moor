...
| | |
|---|---|
| Source | Optional — install with `moor skills install official/research/darwinian-evolver` |
| Path | `optional-skills/research/darwinian-evolver` |
| Version | `0.1.0` |
| Author | Bihruze (Asahi0x), Moor Agent |
| License | MIT |
...
import upstream classes into Moor itself.
...
```bash
mkdir -p ~/.moor/cache/darwinian-evolver && cd ~/.moor/cache/darwinian-evolver
...
```bash
cd ~/.moor/cache/darwinian-evolver/darwinian_evolver \
...
uv run darwinian_evolver parrot \
  ...
  --output_dir ~/.moor/cache/scratch/parrot_demo
```

Outputs:
- `~/.moor/cache/scratch/parrot_demo/snapshots/iteration_N.pkl` — pickled population per iteration
- `~/.moor/cache/scratch/parrot_demo/<jsonl>` — per-iteration JSON log (path printed at end)

Open `~/.moor/cache/darwinian-evolver/darwinian_evolver/darwinian_evolver/lineage_visualizer.html`
...
SKILL_DIR=~/.moor/skills/research/darwinian-evolver
DE_DIR=~/.moor/cache/darwinian-evolver/darwinian_evolver

cd "$DE_DIR" && \
  EVOLVER_MODEL='openai/gpt-4o-mini' \
  uv run --with openai python "$SKILL_DIR/scripts/parrot_openrouter.py" \
    --num_iterations 3 --num_parents_per_iteration 2 \
    --output_dir ~/.moor/cache/scratch/parrot_or
```
...
uv run --with openai python "$SKILL_DIR/scripts/show_snapshot.py" \
  ~/.moor/cache/scratch/parrot_or/snapshots/iteration_3.pkl
```
...
7. **AGPL.** Never `from darwinian_evolver import ...` inside Moor core.
   Custom driver scripts under `~/.moor/skills/...` are user-side and fine.
