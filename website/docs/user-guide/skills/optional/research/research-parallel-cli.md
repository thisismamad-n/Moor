...
| | |
|---|---|
| Source | Optional — install with `moor skills install official/research/parallel-cli` |
...
This is an optional third-party workflow, not a Moor core capability.

Important expectations:
- Parallel is a paid service with a free tier, not a fully free local tool.
- It overlaps with Moor native `web_search` / `web_extract`, so do not prefer it by default for ordinary lookups.
...
Prefer Moor native `web_search` / `web_extract` for quick one-off lookups when Parallel is not specifically requested.
...
7. Prefer Moor native tools unless the user wants Parallel specifically or needs Parallel-only workflows.
...
```bash
parallel-cli search "latest React 19 changes" --json -o ~/.moor/cache/scratch/react-19-search.json
```
...
## Recommended Moor usage patterns
...
- For large result sets, save JSON to `~/.moor/cache/scratch/*.json` (the Moor scratch dir) instead of stuffing everything into context.
- Do not silently choose Parallel when Moor native tools are already sufficient.
