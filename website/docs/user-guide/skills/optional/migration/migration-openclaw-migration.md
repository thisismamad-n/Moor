<<<<<<< HEAD
---
title: "Openclaw Migration — Migrate a user's OpenClaw customization footprint into Moor Agent"
sidebar_label: "Openclaw Migration"
description: "Migrate a user's OpenClaw customization footprint into Moor Agent"
---
...
Migrate a user's OpenClaw customization footprint into Moor Agent. Imports Moor-compatible memories, SOUL.md, command allowlists, user skills, and selected workspace assets from ~/.openclaw, then reports exactly what could not be migrated and why.
...
# OpenClaw -> Moor Migration

Use this skill when a user wants to move their OpenClaw setup into Moor Agent with minimal manual cleanup.
...
- archive non-secret docs that do not have a direct Moor destination
...
Moor CLI supports the `clarify` tool for interactive prompts, but it is limited to:
...
- `{"question":"One or more imported OpenClaw skills already exist in Moor. How should I handle those skill conflicts?","choices":["keep existing skills","overwrite conflicting skills with backup","import conflicting skills under renamed folders"]}`
...
- `{"question":"Do you want to copy the OpenClaw workspace instructions file into a Moor workspace?","choices":["skip workspace instructions","copy to a workspace path","decide later"]}`
...
13. If a renamed imported skill has an empty `details.backup`, do not imply the existing Moor skill was renamed or backed up. Say only that the imported copy was placed in the new destination and reference `details.renamed_from` as the pre-existing folder that remained in place.
...
2. Do not migrate secrets by default. Tokens, auth blobs, device credentials, and raw gateway config should stay out of Moor unless the user explicitly asks for secret migration.
3. Do not silently overwrite non-empty Moor targets unless the user explicitly wants that. The helper script will preserve backups when overwriting is enabled.
...
6. Even in secret-migration mode, only migrate secrets with a clean Moor destination. Unsupported auth blobs must still be reported as skipped.
...
- Moor persona state imported
- Moor memory files populated with converted OpenClaw knowledge
...
=======
---
title: "Openclaw Migration — Import an OpenClaw setup (memories, skills) into Moor"
sidebar_label: "Openclaw Migration"
description: "Import an OpenClaw setup (memories, skills) into Moor"
---
...
Import an OpenClaw setup (memories, skills) into Moor.
