...
Add to `~/.moor/config.yaml` (paste your actual key — MCP env vars are not expanded from .env):
```yaml
mcp_servers:
  agentmail:
    command: "npx"
...
1. Create a dedicated inbox:
   - Use `create_inbox` with a username (e.g. `moor-agent`)
   - The agent gets address: `moor-agent@agentmail.to`
2. Send an email:
