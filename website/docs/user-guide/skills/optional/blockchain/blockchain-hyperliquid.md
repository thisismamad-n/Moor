...
The script reads `${MOOR_HOME:-~/.moor}/.env` for two optional defaults:
...
Helper script: `~/.moor/skills/blockchain/hyperliquid/scripts/hyperliquid_client.py`
...
optional when `HYPERLIQUID_USER_ADDRESS` is set in `${MOOR_HOME:-~/.moor}/.env`.
