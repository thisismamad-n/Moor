---
title: "Honcho — Configure and troubleshoot Honcho memory for Moor"
sidebar_label: "Honcho"
description: "Configure and troubleshoot Honcho memory for Moor"
---
...
Configure and troubleshoot Honcho memory for Moor.
...
# Honcho Memory for Moor

Honcho provides AI-native cross-session user modeling. It learns who the user is across conversations and gives every Moor profile its own peer identity while sharing a unified view of the user.
...
See: https://docs.honcho.dev/v3/guides/integrations/moor#running-honcho-locally-with-moor
...
3. **AI peer card** -- the identity card for this Moor profile's AI peer
...
Honcho models conversations as interactions between **peers**. Moor creates two peers per session:

- **User peer** (`peerName`): represents the human. Honcho builds a user representation from observed messages.
- **AI peer** (`aiPeer`): represents this Moor instance. Each profile gets its own AI peer so agents develop independent views.
...
| `per-session` | New Honcho session each Moor run |
| `global` | Single session across all directories |

Manual override: `moor honcho map my-project-name`
...
Each Moor profile gets its own Honcho AI peer while sharing the same workspace (user context). This means:
...
1. Creates a `moor.coder` host block in `honcho.json`
...
Guidelines for Moor when Honcho memory is active.
...
Config file: `$MOOR_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global).
...
Run `moor honcho setup`. Ensure `memory.provider: honcho` is in `~/.moor/config.yaml`.

### Memory not persisting across sessions
Check `moor honcho status` -- verify `saveMessages: true` and `writeFrequency` isn't `session` (which only writes on exit).

### Profile not getting its own peer
Use `--clone` when creating: `moor profile create <name> --clone`. For existing profiles: `moor honcho sync`.
...
| Command | Description |
|---------|-------------|
| `moor honcho setup` | Interactive setup wizard (cloud/local, identity, observation, recall, sessions) |
| `moor honcho status` | Show resolved config, connection test, peer info for active profile |
| `moor honcho enable` | Enable Honcho for the active profile (creates host block if needed) |
| `moor honcho disable` | Disable Honcho for the active profile |
| `moor honcho peer` | Show or update peer names (`--user <name>`, `--ai <name>`, `--reasoning <level>`) |
| `moor honcho peers` | Show peer identities across all profiles |
| `moor honcho mode` | Show or set recall mode (`hybrid`, `context`, `tools`) |
| `moor honcho tokens` | Show or set token budgets (`--context <N>`, `--dialectic <N>`) |
| `moor honcho sessions` | List known directory-to-session-name mappings |
| `moor honcho map <name>` | Map current working directory to a Honcho session name |
| `moor honcho identity` | Seed AI peer identity or show both peer representations |
| `moor honcho sync` | Create host blocks for all Moor profiles that don't have one yet |
| `moor honcho migrate` | Step-by-step migration guide from OpenClaw native memory to Moor + Honcho |
