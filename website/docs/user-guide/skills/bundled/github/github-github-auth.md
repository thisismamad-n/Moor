...
- Give it a name like "moor-agent"
- Select scopes:
...
- Give it a title like "moor-agent-&lt;machine-name>"
...
uv run python3 "${MOOR_HOME:-$HOME/.moor}/skills/github/github-auth/scripts/git-credential-token.py"
```
...
elif _moor_env="${MOOR_HOME:-$HOME/.moor}/.env"; [ -f "$_moor_env" ] && grep -q "^GITHUB_TOKEN=" "$_moor_env"; then
  export GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" "$_moor_env" | head -1 | cut -d= -f2 | tr -d '\n\r')
  echo "AUTH_METHOD=curl"
elif grep -q "github.com" ~/.git-credentials 2>/dev/null; then
  export GITHUB_TOKEN=$(uv run python3 "${MOOR_HOME:-$HOME/.moor}/skills/github/github-auth/scripts/git-credential-token.py")
  echo "AUTH_METHOD=curl"
else
