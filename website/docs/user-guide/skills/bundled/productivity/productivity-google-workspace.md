...
- `scripts/google_api.py` — compatibility wrapper CLI. It prefers `gws` for operations when available, while preserving Moor' existing JSON output contract.
...
>
> Important Moor CLI note: if the file path starts with `/`, do NOT send only the bare path as its own message in the CLI, because it can be mistaken for a slash command. Send it in a sentence instead, like:
...
explicit (for example `~/Downloads/moor-google-client-secret.json`), then run
...
`~/.moor/google_oauth_last_url.txt`.
...
- Token is stored at `~/.moor/google_token.json` and auto-refreshes.
- Pending OAuth session state/verifier are stored temporarily at `~/.moor/google_oauth_pending.json` until exchange completes.
- If `gws` is installed, `google_api.py` points it at the same `~/.moor/google_token.json` credentials file. Users do not need to run a separate `gws auth login` flow.
