...
- A long-running process (moor gateway, tui_gateway) misbehaves and you can't restart it
...
The moor test runner and pytest both support this:
...
For long-lived processes: Moor gateway, tui_gateway, a daemon, a process that's already misbehaving and can't be restarted clean.

### Setup

```bash
source <moor-agent-repo>/.venv/bin/activate
pip install debugpy
```
...
The easiest terminal-side DAP client is VS Code CLI or a small script. From inside Moor you have two practical options:
...
Easiest: add `breakpoint()` near the suspect line, then run `moor` normally. Control returns to your terminal at the pause point.
...
debugpy.wait_for_client()
```
Start `moor --tui`. The TUI will appear frozen (its backend is waiting). Attach a client; execution resumes when you `continue`.
...
9. **Forking / multiprocessing.** pdb does not follow forks. Each child needs its own `breakpoint()` or `set_trace()`. For Moor subagents, debug one process at a time.
