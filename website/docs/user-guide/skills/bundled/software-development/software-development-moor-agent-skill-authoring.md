...
| Path | `skills/software-development\moor-agent-skill-authoring` |
| Version | `2.0.0` |
| Author | Moor Agent |
| License | MIT |
...
1. **User-local:** `~/.moor/skills/<maybe-category>/<name>/SKILL.md` — personal, not shared. Created via `skill_manage(action='create')`.
2. **In-repo (this skill is about this case):** `skills/<category>/<name>/SKILL.md` or `optional-skills/<category>/<name>/SKILL.md` inside the moor-agent repo — committed, shipped with the package. Use `write_file` + `git add`. `skill_manage(action='create')` does NOT target this tree.
...
- You're committing a reusable workflow that should ship with moor-agent
...
- Don't use for: personal skills in `~/.moor/skills/` (just use `skill_manage`)
...
- **Optional (`optional-skills/<category>/`)** — niche, vertical-specific (blockchain, gaming, finance, one app), recurring-job/task skills, or anything heavy. Installed via `moor skills install official/<category>/<skill>`.
...
- Every entry must resolve to an existing **in-repo** skill in the same tree state as your PR. Do not reference skills that were only planned, live in another PR, or exist only in `~/.moor/skills/`.
...
1. **Using `skill_manage(action='create')` for an in-repo skill.** It writes to `~/.moor/skills/`, not the repo tree. Use `write_file`.
...
- [ ] `name`, `description`, `version`, `author`, `license`, `platforms`, `metadata.moor.{tags, related_skills}` all present
