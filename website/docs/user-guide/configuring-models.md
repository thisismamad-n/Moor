...
Moor uses two kinds of model slots:
...
[Moor Portal](/user-guide/features/tool-gateway) provides 300+ models under one subscription. On a fresh install, run `moor setup --portal` to log in and set Moor as your provider in one command. Inspect what's wired up with `moor portal info`.

- Portal subscribers also get **10% off token-billed providers**.
:::

:::note `model:` schema — empty string vs. mapping
On a brand-new install the bundled default config has `model: ""` (an empty string sentinel meaning "not configured yet"). The first time you run `moor setup` or `moor model`, that key is upgraded in-place to a mapping with `provider`, `default`, `base_url`, and `api_mode` sub-keys — the shape shown throughout this page and in [`profiles.md`](./profiles.md) / [`configuration.md`](./configuration.md). If you ever see an empty string in `config.yaml`, run `moor model` (or click **Change** in the dashboard) and Moor will write the dict form for you.
:::
...
- **Right** — the curated model list for the selected provider. These are the agentic models Moor recommends for that provider, not the raw `/models` dump (which on OpenRouter includes 400+ models including TTS, image generators, and rerankers).

Type in the filter box to narrow by provider name, slug, or model ID.

Pick a model, hit **Switch**, and Moor writes it to `~/.moor/config.yaml` under the `model` section. **This applies to new sessions only** — any chat tab you already have open keeps running whatever model it started with. To hot-swap the current chat, use the `/model` slash command inside it.

### Mid-session switches and context warnings

When you switch models **inside an active session** (Herm TUI model picker, `moor` CLI, or `/model` on Telegram/Discord), Moor estimates whether your **next message** will run **preflight context compression** against the new model's window. If the session is already near or above that model's compression threshold (see [Context Compression](./configuration.md#context-compression)), the switch reply includes a warning — the same `warning_message` path used for expensive-model notices. The switch still applies immediately; compression runs on the **first user message after the switch**, before the model answers.
...
| **Skills Hub** | `moor skills search` uses this. Usually fine at `auto`. |
| **MCP** | MCP tool routing. Usually fine at `auto`. |
| **Triage Specifier** | Routes the Kanban triage specifier (`moor kanban specify`) that expands a rough one-liner into a concrete spec. A cheap, capable model works well. |
...
| **Profile Describer** | Routes profile-description generation (`moor profile describe --auto` / the dashboard auto-generate button). Short, cheap call. |
...
When you save via the dashboard, Moor writes to `~/.moor/config.yaml`:
...
`provider: auto` with `model: ''` tells Moor to use the main model for that task, while still honoring fallback policy if the main route cannot serve the auxiliary call.
...
Provider entries (`providers.<name>` in the `providers:` dict, or items in the legacy `custom_providers` list) accept knobs that shape how Moor talks to the endpoint:
...
### Moor Portal: which wire carries Claude

Moor Portal serves its `anthropic/*` models on two routes: OpenAI-compatible `/v1/chat/completions` and the native Anthropic Messages wire `/v1/messages`. `moor.anthropic_wire` picks one:
...
Moor lists a provider only if it has a working credential. Check **Keys** in the sidebar — you should see one of: an API key, a successful OAuth, or a custom endpoint URL. If the provider you want isn't there, run `moor setup` to wire it up, or go to **Keys** and add the env var.
...
### I picked a model but Moor switched providers on me
...
Define your own short names for models you reach for often, then use `/model <alias>` in a running session or `moor chat --model <alias>` at startup. There are two equivalent formats — pick whichever fits your workflow.
...
**Short string form (`model.aliases.<name>: provider/model`)** — convenient from the shell because `moor config set` writes scalars and now also parses inline list/mapping literals, though this short alias form still can't carry a custom `base_url`:
...
> `moor config set` also accepts inline **list/mapping literals** (JSON/YAML flow style). Quote them so your shell passes them through intact:
>
> ```bash
> moor config set platform_toolsets.line '["clarify", "file", "web"]'
> moor config set display.tool_progress_overrides '{"terminal": "off"}'
> ```

Both paths feed the same loader (`moor_cli/model_switch.py`). Entries declared in `model_aliases:` take precedence over `model.aliases:` entries with the same name.
...
### `moor model` subcommand

```bash
moor model            # Interactive provider + model picker (the canonical way to switch defaults)
```

`moor model` walks you through picking a provider, authenticating (OAuth flows open a browser; API-key providers prompt for the key), and then choosing a specific model from that provider's curated catalog. The choice is written to `model.provider` and `model.default` in `~/.moor/config.yaml`.

To list providers/models without launching the picker, use the dashboard or the REST endpoints below. To inspect what the CLI will actually use right now: `moor config get model --json` and `moor status`.

### Direct config edit

Edit `~/.moor/config.yaml` and restart whatever reads it. See the [Configuration reference](./configuration.md) for the full schema.
...
The session token is injected into the dashboard HTML at startup and rotates on every server restart. Grab it from the browser devtools (`window.__MOOR_SESSION_TOKEN__`) if you're scripting against a running dashboard.
