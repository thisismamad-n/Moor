...
sidebar_position: 1
title: "CLI Interface"
description: "Master the Moor Agent terminal interface — commands, keybindings, personalities, and more"
---
...
One command — `moor setup --portal` — and you're ready to `moor chat`. See [Moor Portal](/integrations/moor-portal).
:::

:::tip
Moor also ships a modern TUI with modal overlays, mouse selection, and non-blocking input. Launch it with `moor --tui` — see the [TUI](tui.md) guide.
:::
...
moor chat --query-file prompt.txt
moor chat --query-file - < prompt.txt
...
moor chat --provider moor        # Use Moor Portal
...
moor --continue             # Resume the most recent CLI session (-c)
moor --resume <session_id>  # Resume a specific session by ID (-r)
moor --resume latest        # Resume the most recent session (same as -c)
moor --resume latest --in ./dir  # Resume ./dir's latest session, staying in ./dir

# Verbose mode (debug output)
moor chat --verbose

# Isolated git worktree (for running multiple agents in parallel)
moor -w                         # Interactive mode in worktree
moor -w -z "Fix issue #123"     # Single query in worktree
```

### Worktree cleanup

`moor -w` sessions create disposable worktrees under `<repo>/.worktrees/`.
...
- Trees **in use by a running moor session** are never touched.
- **Untracked-only scratch** (PR body drafts, notes) is archived to
  `~/.moor/archive/worktree-prune/` before its tree is removed — never
  destroyed.
...
launches `moor -w` for days — no longer accumulate merged scratch trees
...
The `moor plugins` commands manage native Moor plugins and portable Agent
...
moor plugins disable <plugin-name>
moor plugins update <plugin-name>
moor plugins remove <plugin-name>
```
...
<img className="docs-terminal-figure" src="/docs/img/docs/cli-layout.svg" alt="Stylized preview of the Moor CLI layout showing the banner, conversation area, and fixed input prompt." />
<p className="docs-figure-caption">The Moor CLI banner, conversation stream, and fixed input prompt rendered as a stable docs figure instead of fragile text art.</p>
...
| ⚠ YOLO | **YOLO mode warning** — shown whenever `MOOR_YOLO_MODE` is on (either `moor --yolo` at launch or `/yolo` toggled mid-session). Mirrors the banner-line warning so you can't forget you're in auto-approve mode. |
...
On the `openai-codex` provider, `/usage` also shows any banked usage-limit resets on your ChatGPT account ("You have N resets banked - use /usage reset to activate"). `/usage reset` redeems one banked reset, fully restoring your 5-hour and weekly limits. Moor refuses to redeem while your limits aren't exhausted (a banked reset restores the full allowance, so spending it early wastes it) — pass `/usage reset --force` to redeem anyway.

### Session Resume Display

When resuming a previous session (`moor -c` or `moor --resume <id>`), a "Previous Conversation" panel appears between the banner and the input prompt, showing a compact recap of the conversation history. See [Sessions — Conversation Recap on Resume](sessions.md#conversation-recap-on-resume) for details and configuration.
...
| `Ctrl+D` | Exit |
| `Ctrl+Z` | Suspend Moor to background (Unix only). Run `fg` in the shell to resume. |
...
Type `/` to see the autocomplete dropdown. Moor supports a large set of CLI slash commands, dynamic skill commands, and user-defined quick commands.
...
| `/voice tts` | Toggle spoken playback for Moor replies |
...
Every installed skill in `~/.moor/skills/` is automatically registered as a slash command. The skill name becomes the command:
...
You can also define custom personalities in `~/.moor/config.yaml`:

```yaml
personalities:
  helpful: "You are a helpful, friendly AI assistant."
  kawaii: "You are a kawaii assistant! Use cute expressions..."
  pirate: "Arrr! Ye be talkin' to Captain Moor..."
  # Add your own!
```
...
On Unix systems, press **`Ctrl+Z`** to suspend Moor to the background — just like any terminal process. The shell prints a confirmation:

```
Moor Agent has been suspended. Run `fg` to bring Moor Agent back.
```
...
Resume this session with:
  moor --resume 20260225_143052_a1b2c3
...
```bash
moor --continue                          # Resume the most recent CLI session
moor -c                                  # Short form
moor -c "my project"                     # Resume a named session (latest in lineage)
moor --resume 20260225_143052_a1b2c3     # Resume a specific session by ID
moor --resume "refactoring auth"         # Resume by title
moor --resume latest                     # Resume the most recent session (same as -c)
moor --resume latest --in ./my-project   # Latest session for ./my-project's workspace
moor -r 20260225_143052_a1b2c3           # Short form
```
...
CLI sessions are stored in Moor's SQLite state database under `~/.moor/state.db`. The database keeps:
...
Moor immediately confirms the task and gives you back the prompt:
