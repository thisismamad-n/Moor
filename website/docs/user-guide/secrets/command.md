...
secrets:
  command:
    enabled: true
    command: "cat /run/user/1000/moor-secrets.env"
    # or any vault CLI that dumps KEY=VALUE lines:
    # command: "pass show moor/env"
    # command: "secret-tool lookup service moor-env"
```
...
- The helper's **stderr is discarded** — vault CLI diagnostics can carry secret material, so they never reach Moor' output. Failures log structured fields only (exit code / signal / errno), never the command string.
...
| `helper command failed` | Non-zero exit, timeout, spawn failure | Run the helper manually in a shell to see its real error (Moor discards its stderr on purpose) |
