...
Resolve provider API keys from [1Password](https://1password.com/) at process startup instead of storing them in plaintext inside `~/.moor/.env`. You keep your keys as 1Password items and reference them by `op://vault/item/field`; rotating a credential becomes a single change in 1Password.
...
2. You map environment-variable names to `op://` references in `~/.moor/config.yaml`.
3. Every time `moor` (or the gateway, or a cron job) starts, after `~/.moor/.env` has loaded, Moor runs `op read` for each reference and sets the resolved values into `os.environ`.
...
- **Service accounts** (recommended for servers/CI): create a service account in 1Password, grant it read access to the relevant vault, and export its token as `OP_SERVICE_ACCOUNT_TOKEN` in `~/.moor/.env`. The token is the credential — treat it like any other bearer token.
...
1. **In `~/.moor/.env` (recommended).** `moor secrets onepassword setup --token <token>` writes the token to `~/.moor/.env`, exactly like Bitwarden's `BWS_ACCESS_TOKEN`. Because `load_moor_dotenv()` always loads `.env`, the token is available everywhere with zero extra setup. This is the simplest reliable option.

2. **In `~/.moor/.op.env` (gitignored).** If you'd rather keep the service-account token out of `.env` — for example so `.env` can be checked into a private dotfiles repo while the token stays out of version control — place it in `~/.moor/.op.env`:

   ```bash
   echo 'OP_SERVICE_ACCOUNT_TOKEN=ops_...' > ~/.moor/.op.env
   chmod 600 ~/.moor/.op.env
   ```
...
3. **Via systemd `EnvironmentFile` (Linux gateway).** If you run the gateway under systemd, you can inject the token directly into the service environment:
   ...
   ```ini
   [Service]
   EnvironmentFile=-/home/youruser/.moor/.op.env
   ```
...
```bash
moor secrets onepassword setup
```
...
```bash
moor secrets onepassword set OPENAI_API_KEY    "op://Private/OpenAI/api key"
moor secrets onepassword set ANTHROPIC_API_KEY "op://Private/Anthropic/credential"
```

### 4. Preview and confirm

```bash
moor secrets onepassword sync     # dry-run: resolve now, show what would apply
moor secrets onepassword status   # config + binary + references + auth
```

From now on, every `moor` invocation resolves the references at startup. You'll see a one-line summary in stderr the first time secrets are applied in a process.

## CLI

| Command | What it does |
|---|---|
| `moor secrets onepassword setup` | Verify `op`, set account / token env var, enable |
| `moor secrets onepassword status` | Show config, binary, auth, and configured references |
| `moor secrets onepassword token` | Rotate the service-account token: validate with `op whoami`, then store it in `.env` |
| `moor secrets onepassword set ENV_VAR "op://…"` | Map an env var to a reference (stored stripped + validated) |
| `moor secrets onepassword remove ENV_VAR` | Drop a mapping |
| `moor secrets onepassword sync` | Dry-run: resolve references now and show what would apply |
| `moor secrets onepassword sync --apply` | Resolve and export into the current shell's environment |
| `moor secrets onepassword disable` | Flip `enabled: false`; leaves mappings in place |

`op` and `1password` are accepted as aliases for `onepassword`.

## Configuration

Defaults in `~/.moor/config.yaml`:
...
| `op read failed for 'op://…': …` | Locked session, expired token, or no vault access | `op signin`, run `moor secrets onepassword token` to rotate the service-account token, or grant the service account access |
...
Successful, complete pulls are cached in-process and on disk under `<moor_home>/cache/op_cache.json` (written atomically, mode `0600`), so back-to-back short-lived `moor` invocations don't re-shell `op` for every reference. The cache:
...
- A 1Password service-account token can read every secret the account has access to. Store it in `~/.moor/.env` (not `config.yaml`), and revoke + regenerate from 1Password if it leaks.
- Moor refuses to let a resolved value overwrite the token env var itself, even with `override_existing: true`.
- The `op` child process gets a minimal allowlisted environment (auth/session vars + `PATH`/`HOME` and the `op` config-location vars `OP_CONFIG_DIR`/`XDG_CONFIG_HOME`), not a copy of the full `os.environ`, so post-dotenv provider credentials aren't all inherited by the child. Set `OP_CONFIG_DIR` when `~/.config` is not writable by the Moor user (common in containers).
...
- **Single-machine personal setups** where `~/.moor/.env` is fine.
