...
Pull API keys from [Bitwarden Secrets Manager](https://bitwarden.com/products/secrets-manager/) at process startup instead of storing them in plaintext inside `~/.moor/.env`. One bootstrap secret (a machine-account access token) replaces N per-provider keys, and rotating a credential becomes a single change in the Bitwarden web app.
...
2. Moor stores that single token in `~/.moor/.env` as `BWS_ACCESS_TOKEN`.
...
The `bws` binary is auto-downloaded into `~/.moor/bin/` on first use — no `apt`, no `brew`, no `sudo`.
...
2. Create or pick a **Project** (e.g. "Moor keys").
...
4. **Machine accounts → New machine account → My Moor machine** → **Projects** tab → grant Read access to your project.
...
1. Download and verify `bws v2.0.0` into `~/.moor/bin/bws`.
...
From now on, every `moor` invocation pulls fresh secrets at startup. You'll see a one-line summary in stderr the first time secrets are applied in a process.
...
| `moor secrets bitwarden install` | Just download the pinned `bws` binary (no auth required) |
...
Bitwarden Secrets Manager: → Run `moor secrets bitwarden token` to paste a fresh access token ...
```

Fix it without re-running the whole wizard:

```bash
moor secrets bitwarden token                     # masked prompt
moor secrets bitwarden token --access-token 0.…  # non-interactive
```
...
| `encrypted_cache.enabled` | `false` | Store the last successful fetch in an AES-GCM encrypted cache at `~/.moor/cache/bws_cache.enc.json`. |
...
| `auto_install` | `true` | When true, `bws` is auto-downloaded into `~/.moor/bin/` on first use. |

## Failure modes

Bitwarden never blocks Moor startup. If anything goes wrong, you'll see a one-line warning in stderr and Moor continues with whatever credentials `.env` already had:

| Symptom | Cause | Fix |
|---|---|---|
| `BWS_ACCESS_TOKEN is not set` | Enabled in config but token cleared from `.env` | Re-run `moor secrets bitwarden setup` |
...
- The pinned version (`bws v2.0.0` at time of writing) is updated through PRs to this repo — Moor does not auto-upgrade `bws` to "latest" because upstream release shapes can change.

## When NOT to use this

- **Single-machine personal setups** where `~/.moor/.env` is fine. You're trading one credential for another and adding a network dependency at startup.
...
The good case for this is multi-machine fleets, shared dev boxes, gateway VPSes, or any setup where you want centralized rotation and revocation across multiple Moor installations.
