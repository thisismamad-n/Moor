...
A **profile distribution** packages a complete Moor agent — personality, skills, cron jobs, MCP connections, config — as a git repository. Anyone with access to the repo can install the whole agent with one command, update it in place, and keep their own memories, sessions, and API keys untouched.

If a [profile](./profiles.md) is a local agent, a distribution is that agent made shareable.

## Two ways to share a profile

Moor has two sharing paths, and they answer different questions. Distributions are the durable one; export files are the quick one.

| | **Distribution** (git repo) | **Export file** (`.tar.gz`) |
|---|---|---|
| Ship it by | `moor profile install <repo>` | Send a file — chat, AirDrop, USB, email |
| Recipient needs | git, and access to the repo | The file |
| Updates | `moor profile update` pulls new versions | Re-send the file |
...
| Made with | `moor profile install` / `update` | `/export` and `/import`, or `moor profile export` / `import` |
...
Before distributions, sharing a Moor agent meant sending someone:
...
```bash
moor profile install github.com/you/my-research-agent --alias
```

…and they now have the whole agent. They fill in their own API keys (`.env.EXAMPLE` → `.env`), and they can run `my-research-agent chat` or address it through Telegram / Discord / Slack / any gateway platform. When you push a new version, they run `moor profile update my-research-agent` and pull your changes — their memories and sessions stay put.
...
The tradeoff: recipients need git installed. On any machine running Moor in 2026, that's already true.
...
:::caution
**Moor does not control git.** The file exclusions described on this page are applied by the **installer** when someone runs `moor profile install` or `moor profile update`. They are **not** applied when you run `git add` or `git commit`.
:::
...
```bash
moor profile create research-bot
research-bot setup                    # configure model, API keys
# Edit ~/.moor/profiles/research-bot/SOUL.md
...
Create `~/.moor/profiles/research-bot/distribution.yaml`:
...
Create `~/.moor/profiles/research-bot/.gitignore` with at minimum:
...
Recipients who run `moor profile update research-bot` will pull the latest.
...
5. Copies distribution-owned files into `~/.moor/profiles/research-bot/` (or wherever the manifest's `name` resolves). The [hard-excluded paths](#whats-not-in-a-distribution-ever) are stripped during this copy, even if the author accidentally left them in the repo.
...
```bash
# GitHub shorthand
moor profile install github.com/you/research-bot

# Full HTTPS
moor profile install https://github.com/you/research-bot.git

# SSH
moor profile install git@github.com:you/research-bot.git

# Self-hosted, GitLab, Gitea, Forgejo — any Git host
moor profile install https://git.example.com/team/research-bot.git

# Private repo using your configured git auth
moor profile install git@github.com:your-org/internal-bot.git

# Local directory during development (no git push needed)
moor profile install ~/my-profile-in-progress/
```
...
```bash
# Alice
moor profile install github.com/acme/support-bot --name support-us --alias
# Bob (same distribution, different local name)
moor profile install github.com/acme/support-bot --name support-eu --alias
```
...
```bash
cp ~/.moor/profiles/research-bot/.env.EXAMPLE ~/.moor/profiles/research-bot/.env
...
```bash
moor profile info research-bot
```
...
`moor profile list` also shows a `Distribution` column so at a glance you can see which of your profiles came from repos and which you hand-built:
...
```bash
moor profile delete research-bot
```
...
Path:    ~/.moor/profiles/research-bot
...
Any iteration on the laptop (`git commit && push`) pulls onto the workstation with `moor profile update research-bot`. Memories stay per-machine — the laptop remembers its own conversations, the workstation remembers its own, they don't collide.
...
# Each engineer
moor profile install git@github.com:your-org/pr-reviewer.git --alias
# Fill in .env with their own API key (billed to them), .env.EXAMPLE points at what's required
pr-reviewer chat
```

When the lead ships v1.1 (better SOUL, new skill), engineers run `moor profile update pr-reviewer` and everyone's on the new version within minutes.
...
cd ~/.moor/profiles/polymarket-trader
# Write a solid README.md at the repo root — GitHub shows it on the repo page
git init && git add . && git status   # confirm no secrets staged
git commit -m "v1.0"
git tag v1.0.0
# Publish to a public GitHub repo
git remote add origin https://github.com/you/moor-polymarket-trader.git
git push -u origin main --tags

# Anyone
moor profile install github.com/you/moor-polymarket-trader --alias
```
...
# Each on-call
moor profile install git@github.com:your-org/incident-2026-q2.git --alias

# Incident resolved — tear it down
moor profile delete incident-2026-q2
```
...
:::note
Git ref pinning (`#v1.2.0`) is planned but not in the initial release — install currently tracks the default branch. Track your installed version via `moor profile info <name>` and hold off on updates until you're ready.
:::
...
moor profile info research-bot | grep Version
...
```yaml
# ~/.moor/profiles/research-bot/local/my-overrides.yaml
...
moor profile delete research-bot --yes
moor profile install github.com/you/research-bot --alias

# Update to current main but reset config.yaml to the distribution's default
moor profile update research-bot --force-config --yes
```
...
moor profile install github.com/yourname/forked-research-bot --alias

# Iterate locally in ~/.moor/profiles/forked-research-bot/
...
moor profile install ~/.moor/profiles/research-bot --name research-bot-test --alias

# Tweak, delete, re-install until it's right
moor profile delete research-bot-test --yes
moor profile install ~/.moor/profiles/research-bot --name research-bot-test
```
...
```bash
moor profile export research-bot
moor profile export research-bot -o ./research-bot.tar.gz
```
...
```bash
moor profile import ./research-bot.tar.gz
moor profile import ./research-bot.tar.gz --name research-bot-2
```

The profile name is inferred from the archive unless you pass `--name`. Importing over an existing profile is refused — rename or delete the old one first. A shell wrapper (`research-bot` → `moor -p research-bot`) is created when the name doesn't collide with an existing command.
...
:::note
You cannot import as `default` — that name is the built-in root profile (`~/.moor`). Pass `--name something-else`.
:::
...
**The default profile** (`~/.moor`) is exported through an allow-list — only known Moor artifacts, so an unrelated file sitting in your home directory can't get swept in:
...
**A named profile** (`~/.moor/profiles/<name>`) copies the whole directory minus `auth.json` / `.env`. That's broader — if the profile has `state.db`, logs, or caches, they go in the archive too, and the file gets big.
...
Cron jobs from a distribution are **not auto-scheduled** — the installer prints `moor -p <name> cron list` and you enable them explicitly. SOUL.md and skills ARE active as soon as you start chatting with the profile, so read them before your first run if you're installing from someone you don't know.
...
- `install`, `update`, `info` live inside `moor profile` — not a parallel command tree.
...
- Reserved profile names (`moor`, `test`, `tmp`, `root`, `sudo`) are rejected at install time to avoid collisions with common binaries.
...
- [`moor profile export` / `import`](../reference/profile-commands.md#moor-profile-export) — the CLI form of [export files](#export-and-import-a-profile-file)
- [Slash Commands reference](../reference/slash-commands.md) — `/export`, `/import`, and every other in-chat command
- [Using SOUL with Moor](../guides/use-soul-with-moor.md) — authoring personalities
