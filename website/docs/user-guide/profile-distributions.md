...
:::caution
**Moor does not control git.** The file exclusions described on this page are applied by the **installer** when someone runs `hermes profile install` or `hermes profile update`. They are **not** applied when you run `git add` or `git commit`.
:::
...
**The default profile** (`~/.hermes`) is exported through an allow-list — only known Moor artifacts, so an unrelated file sitting in your home directory can't get swept in:
