...
:::tip
Hosted-mode auth uses Moor Portal OAuth; if you also want the dashboard to talk to a real backend, `moor setup --portal` wires up the model and tool gateway too. See [Moor Portal](/integrations/moor-portal).
:::
...
```bash
# Custom port
moor dashboard --port 8080

# Bind to all interfaces (use with caution on shared networks)
moor dashboard --host 0.0.0.0

# Start without opening browser
moor dashboard --no-open
```
...
child with the selected profile's `MOOR_HOME`, so the conversation runs
...
processes (manage them via `moor -p <name> gateway …`), each profile's
session database, and cron schedulers (the Cron page already aggregates
across profiles with its own filter).

## Prerequisites

The default `moor-agent` install does not ship the HTTP stack or PTY helper — those are optional extras. The **web dashboard** needs FastAPI and Uvicorn (`web` extra). The **Chat** tab also needs `ptyprocess` to spawn the embedded TUI behind a pseudo-terminal (`pty` extra on POSIX). Install both with:

```bash
cd ~/.moor/moor-agent && uv pip install -e ".[web,pty]"
```

The `web` extra pulls in FastAPI/Uvicorn; `pty` pulls in `ptyprocess` (POSIX) or `pywinpty` (native Windows — note that the embedded TUI itself still requires WSL). `cd ~/.moor/moor-agent && uv pip install -e ".[all]"` includes both extras and is the easiest path if you also want messaging/voice/etc.

When you run `moor dashboard` without the dependencies, it will tell you what to install. If the frontend hasn't been built yet and `npm` is available, it builds automatically on first launch.

The Chat tab is part of every `moor dashboard` launch — the embedded browser chat pane (running the TUI over PTY/WebSocket) is always available, with no extra flag required.
...
- **Disk warnings** — the volume holding `~/.moor` is nearly full
...
- `ptyprocess` — installed by the `pty` extra (`cd ~/.moor/moor-agent && uv pip install -e ".[web,pty]"`, or `[all]` covers both)
...
```ini
[Service]
EnvironmentFile=%h/.moor/.env
ExecStart=/path/to/venv/bin/python -m moor_cli.main dashboard \
...
MOOR_DASHBOARD_BASIC_AUTH_PASSWORD=choose-a-strong-password
MOOR_DASHBOARD_BASIC_AUTH_SECRET=<32+ random bytes; openssl rand -base64 32>
```
...
:::tip
Config changes take effect on the next agent session or gateway restart. The web dashboard edits the same `config.yaml` file that `moor config set` and the gateway read from.
:::
...
Create and manage [profiles](../profiles.md) — isolated Moor instances with their own config, skills, and sessions.
...
- **Set as active** — flips the sticky default that **future CLI/gateway runs** pick up (same as `moor profile use`). This does *not* change what the dashboard manages — that's the profile switcher's job
- **Edit model / description / SOUL** — inline editors writing into that profile
- **Rename / Delete** — named profiles only

### Skills

Browse, search, and toggle installed skills and toolsets, and install new ones from the hub. Skills are loaded from `~/.moor/skills/` and grouped by category.
...
- **Browse hub** — a third view searches the skill hub across all sources (the same as `moor skills search`), installs any result by identifier with a live install log, and offers an "Update all" button to refresh installed skills.
...
block in `config.yaml` that `moor mcp` reads from.
...
**Catalog:** browse the Moor-approved MCP servers (the bundled `optional-mcps/`
...
`moor mcp catalog` / `moor mcp install` use.
...
Connect Moor to any messaging platform from the browser — full parity with
`moor setup gateway`. The page lists every supported channel (Telegram,
...
- **Restart gateway** — credentials are written to `~/.moor/.env` and the enabled flag to `config.yaml`; the gateway connects each enabled channel on its next restart, which you can trigger right from the page. On a system-scope install (`moor gateway install --system`) the dashboard runs the restart under `sudo -n`, so the dashboard user needs passwordless sudo; without it the request fails immediately instead of reporting a restart that the CLI then refuses.
...
- **Host** — live system stats: OS / kernel, architecture, hostname, Python and Moor versions, CPU core count + utilization, memory, disk usage of the Moor home, uptime, and load average. (CPU/memory/disk come from `psutil` when installed; identity fields are always shown.) The Moor version shows an **update-status badge** (up to date / N commits behind) and a **Check for updates** button. When an update is available on a git install, an **Update now** button opens a confirmation dialog — showing how many commits you'll pull — before running `moor update` in the background. On Docker/Nix installs the dashboard can't apply the update in place, so it shows the correct out-of-band command instead.
- **Moor Portal** — login status, the active inference provider, and the Tool Gateway routing table (which tools run via the Portal vs. locally), with a link to manage your subscription. Read-only mirror of `moor portal`.
- **Skill curator** — the background skill-maintenance status (active / paused, interval, last run) with pause/resume and a run-now button. Mirrors `moor curator`.
...
![System admin page — host stats and Moor Portal status](/img/dashboard/admin-system-top.png)
...
This re-reads `~/.moor/.env` into the running process's environment. Useful when you've added a new provider key via the dashboard and want to use it immediately.
...
`MOOR_HOME`. Omitted = the dashboard's own profile. Unknown profile names
...
- **`disk`** — a live `shutil.disk_usage()` sample of the volume holding
  `~/.moor`. Fields: `pressure`, `free_mb`, `total_mb`, `used_percent`,
...
| `GET /api/mcp/catalog` | Browse the Moor-approved MCP catalog |
...
| `GET /api/moor/update/check` | Report update availability (commits behind, install method) without applying. For git installs that are behind, also returns a `commits` list (`sha`, `summary`, `author`, `at`) of what's changed. `?force=1` busts the 24h cache (the check goes through the GitHub API, never `git fetch`) |
| `GET /api/curator` · `PUT .../paused` · `POST .../run` | Skill-curator status + pause/resume + run |
| `GET /api/portal` | Moor Portal auth + Tool Gateway routing (read-only) |
...
When the dashboard is bound to a public or non-loopback address — anything other than `127.0.0.1` / `localhost` — Moor Agent engages an auth gate. Every request must carry a verified session cookie or it's bounced to the login page. Three providers ship in the box:
...
- **[OAuth (Moor Portal)](#default-provider-moor-research)** — for hosted deployments and any dashboard reachable over the public internet, and the recommended path for a [remote Moor Desktop connection](#connecting-moor-desktop-to-a-remote-backend). Every login is verified against your Moor account, so this is the provider suitable for internet-facing use.
- **[Self-hosted OIDC](#self-hosted-oidc-provider)** — for bringing your own identity provider via standard OpenID Connect (Keycloak, Auth0, Okta, Google, GitHub via an OIDC bridge, etc.). No Moor Portal involved; suitable for public-internet exposure when fronted by a conformant OIDC server.
...
|-------|-----------|----------|
| `moor dashboard` (default — binds to `127.0.0.1`) | OFF | Local development |
| `moor dashboard --host 0.0.0.0` | **ON** | Remote / production — protect with the username/password provider or OAuth |
...
If the gate would engage but **no** `DashboardAuthProvider` is registered (no Moor plugin, no custom plugin), `moor dashboard` refuses to bind with an explicit error message. There is no "default-deny but accept everything" fallback — a misconfigured gated dashboard never starts.

When you run `moor dashboard --host 0.0.0.0` **interactively** (a real terminal) and no provider is configured yet, Moor doesn't just fail — it offers to set one up on the spot: pick **username & password** (writes `dashboard.basic_auth` to `config.yaml` and you're running in seconds) or **OAuth** (points you at `moor dashboard register`). Non-interactive callers — Docker/s6, CI, piped runs — skip the prompt and hit the fail-closed error above, so an unattended deploy still never starts without auth.

### Default provider: Moor inc.

The bundled `plugins/dashboard_auth/moor` plugin is **always installed** and auto-loaded. It auto-registers a `DashboardAuthProvider` named `moor` when a client ID is configured.

Because every login is verified against Moor Portal and protected by your Moor account, **the Moor provider is the one suitable for exposing a dashboard to the public internet.**

#### Registering a dashboard

To use the Moor provider you need an OAuth client ID (shape `agent:{id}`). There are two ways to get one:

- **CLI — `moor dashboard register`.** Run it on the host where the dashboard lives. It resolves your existing Moor login (run `moor setup` first if you're not logged in), registers a self-hosted OAuth client with the Portal, and writes `MOOR_DASHBOARD_OAUTH_CLIENT_ID` into `~/.moor/.env` for you. Optional flags: `--name` (a human-readable label, otherwise auto-generated) and `--redirect-uri` (a public HTTPS callback URL for an internet-facing host).

  ```bash
  moor dashboard register
  # ✓ Registered dashboard "swift_falcon"
  # …writes MOOR_DASHBOARD_OAUTH_CLIENT_ID to ~/.moor/.env
  ```

- **GUI — the Local Dashboards page.** Open [`/local-dashboards`](https://portal.nousresearch.com/local-dashboards) in the Moor Portal to register, name, manage, and revoke self-hosted dashboards from the browser. Copy the resulting `agent:{id}` client ID into `MOOR_DASHBOARD_OAUTH_CLIENT_ID` (env) or `dashboard.oauth.client_id` (config.yaml). This is also where you revoke a dashboard registered via the CLI.
...
|---------|-----------|--------|----------------|
| `MOOR_DASHBOARD_OAUTH_CLIENT_ID` | `dashboard.oauth.client_id` | `agent:{instance_id}` | `moor dashboard register` |

Per the Moor Agent convention (`~/.moor/.env` is for API keys / secrets only), **`config.yaml` is the recommended place to set these values** for local dev, on-prem, and any deployment you control directly. The environment-variable path exists so a hosting platform's secret injection can push per-deploy `client_id`s without anyone having to edit `config.yaml` inside the image — that's its primary purpose.
...
Bundled providers reported these issues:
  • moor: MOOR_DASHBOARD_OAUTH_CLIENT_ID is not set (and
...
Configure an auth provider before exposing the dashboard:
  ...
  • OAuth: run `moor dashboard register` (Moor Portal) or install a
...
#### Worked example: Moor inc.

From a logged-in Moor install to a Moor-gated dashboard in three steps.

**1. Log in and register the dashboard.** `moor dashboard register` uses your existing Moor login to provision an OAuth client and writes `MOOR_DASHBOARD_OAUTH_CLIENT_ID` into `~/.moor/.env` for you:

```bash
moor setup            # if you're not already logged into Moor Portal
moor dashboard register
# ✓ Registered dashboard "swift_falcon"
# …writes MOOR_DASHBOARD_OAUTH_CLIENT_ID to ~/.moor/.env
```

**2. Run the dashboard on a reachable address.** A non-loopback bind engages the OAuth gate, and the `client_id` just written activates the `moor` provider:

```bash
moor dashboard --host 0.0.0.0 --port 9119 --no-open
```

**3. Log in.** Open `http://<host>:9119/`, you'll be bounced to `/login`. Click **Sign in with Moor inc.** → authenticate at the Portal → land back on the authenticated dashboard. Verify the gate from any machine:
...
`GET /api/auth/me` then returns the verified session (`provider: moor`). For an internet-facing host, register with `--redirect-uri https://moor.example.com/auth/callback` and set `MOOR_DASHBOARD_PUBLIC_URL` so the OAuth callback resolves to your public URL (see [Public URL override](#public-url-override)).
...
The username/password provider is intended for self-hosted / on-prem / homelab dashboards on a **trusted network**, or reachable only over a **VPN**. It protects a single shared credential with no external identity provider, MFA, or per-user accounts behind it, so it is **not suitable for exposing a dashboard directly to the public internet**. For an internet-facing dashboard, use the [Moor inc. provider](#default-provider-moor-research) (or your own [self-hosted OIDC](#self-hosted-oidc-provider) / [custom OAuth](#custom-providers) provider) instead.
:::

#### Configuration

Like the Moor provider, it reads from `config.yaml` (canonical) with environment variables winning when set non-empty. It activates only when `username` plus either `password_hash` (preferred) or `password` are configured — otherwise it's a no-op, so OAuth users and loopback operators are unaffected.
...
|---------|-----------|-------|
| `MOOR_DASHBOARD_BASIC_AUTH_USERNAME` | `dashboard.basic_auth.username` | required to activate |
| `MOOR_DASHBOARD_BASIC_AUTH_PASSWORD_HASH` | `dashboard.basic_auth.password_hash` | preferred (no plaintext at rest) |
| `MOOR_DASHBOARD_BASIC_AUTH_PASSWORD` | `dashboard.basic_auth.password` | plaintext; **wins over a config `password_hash`** so you can rotate via env |
| `MOOR_DASHBOARD_BASIC_AUTH_SECRET` | `dashboard.basic_auth.secret` | token-signing key |
| `MOOR_DASHBOARD_BASIC_AUTH_TTL_SECONDS` | `dashboard.basic_auth.session_ttl_seconds` | access-token lifetime |
...
**1. Set credentials in `~/.moor/.env`.** Hash the password so no plaintext sits at rest, and set a stable signing secret so sessions survive restarts:
...
cat >> ~/.moor/.env <<EOF
MOOR_DASHBOARD_BASIC_AUTH_USERNAME=admin
MOOR_DASHBOARD_BASIC_AUTH_PASSWORD_HASH=$HASH
MOOR_DASHBOARD_BASIC_AUTH_SECRET=$(openssl rand -base64 32)
EOF
chmod 600 ~/.moor/.env
```
...
`GET /api/auth/me` then returns the verified session (`provider: basic`). Keep this behind a VPN — see the warning above; for a public host use the [Moor inc.](#default-provider-moor-research) or [self-hosted OIDC](#self-hosted-oidc-provider) provider instead.
...
If you run your own identity provider, the bundled `plugins/dashboard_auth/self_hosted` plugin authenticates the dashboard against it using **standard OpenID Connect** — no per-IDP code, no Moor Portal involved. It's verified against and works with any conformant OIDC server:

> **Authentik · Keycloak · Zitadel · Authelia · Auth0 · Okta · Google · …**

Like the Moor provider, it auto-loads and only registers itself once it's configured, so it's a no-op for loopback dashboards.
...
dashboard:
  oauth:
    ...
    self_hosted:
      issuer: https://auth.example.com/application/o/moor/   # required
      client_id: moor-dashboard                              # required
...
| Env var | Overrides | Notes |
|---------|-----------|-------|
| `MOOR_DASHBOARD_OIDC_ISSUER` | `dashboard.oauth.self_hosted.issuer` | OIDC issuer URL — required |
| `MOOR_DASHBOARD_OIDC_CLIENT_ID` | `dashboard.oauth.self_hosted.client_id` | Public client id — required |
| `MOOR_DASHBOARD_OIDC_SCOPES` | `dashboard.oauth.self_hosted.scopes` | Defaults to `openid profile email` |
...
**1. Run Keycloak with a pre-configured realm.** Save this realm export as `realm-moor.json` — it defines a `moor` realm, a **public PKCE client** (`moor-dashboard`), and a test user, all imported on boot so there's nothing to click in the admin UI:
...
```json
{
  ...
  "clients": [
    {
      "clientId": "moor-dashboard",
      "name": "Moor Agent Dashboard",
      "enabled": true,
      "publicClient": true,
...
docker run --rm -p 8080:8080 \
  ...
  -v "$PWD/realm-moor.json:/opt/keycloak/data/import/realm-moor.json:ro" \
  quay.io/keycloak/keycloak:26.0 \
  start-dev --import-realm
```

Once it's up, the realm advertises standard OIDC discovery at
`http://localhost:8080/realms/moor/.well-known/openid-configuration` (issuer
`http://localhost:8080/realms/moor`). The admin console is at
...
```bash
export MOOR_DASHBOARD_OIDC_ISSUER="http://localhost:8080/realms/moor"
export MOOR_DASHBOARD_OIDC_CLIENT_ID="moor-dashboard"
export MOOR_DASHBOARD_PUBLIC_URL="http://localhost:9119"
moor dashboard --host 0.0.0.0 --port 9119 --no-open
```

`MOOR_DASHBOARD_PUBLIC_URL` tells the dashboard its OAuth callback is
...
> admin console (Clients → moor-dashboard → Settings). The same pattern works
...
For deploys behind reverse proxies that don't reliably forward those headers (manual nginx setups, on-prem ingresses, custom-domain deploys with partial proxy chains), set `dashboard.public_url` (or `MOOR_DASHBOARD_PUBLIC_URL`) to the **complete public URL** the dashboard is reached at:

```yaml
dashboard:
  public_url: "https://dashboard.example.com/moor"
  trusted_proxies:
    - "172.20.0.5"
```
...
first; without one, Moor fails closed at startup. This prevents the local SPA
...
Moor always preserves loopback trust and rejects `*`, `0.0.0.0/0`, and
...
moor dashboard --host 127.0.0.1 --port 9119 --no-open
```
...
|---------|---------------|-------------|
| `dashboard.public_url` in `config.yaml` | `MOOR_DASHBOARD_PUBLIC_URL` | Local dev / on-prem (canonical) |
| `MOOR_DASHBOARD_PUBLIC_URL` env var | — | Hosting-platform secrets / CI |
...
The provider implements the [Moor Portal OAuth contract v1](https://github.com/NousResearch/nous-account-service/blob/main/docs/agent-dashboard-oauth-contract.md) — authorization-code grant with PKCE (S256):

1. User hits `/` without a session cookie → gate redirects to `/login`.
2. Login page shows a "Continue with Moor inc." button → `/auth/login?provider=moor`.
...
5. Server exchanges the code for an access token at `POST /api/oauth/token`, verifies the JWT signature against the Portal's JWKS (`/.well-known/jwks.json`), and sets the `moor_session_at` cookie.
...
| Name | Lifetime | Notes |
|------|----------|-------|
| `moor_session_at` | Token TTL (15 min) | HttpOnly, SameSite=Lax, Secure-when-HTTPS |
| `moor_session_pkce` | 10 min | HttpOnly; holds the PKCE verifier + provider hint during the round trip. SameSite=None + Secure over HTTPS (must survive the cross-site IDP redirect chain — Chromium drops SameSite=Lax cookies set on a 302 in a cross-site chain); SameSite=Lax on loopback HTTP |
| `moor_session_rt` | unused in v1 | Reserved for forward-compat; not written when `refresh_token` is empty |
...
The sidebar widget shows `Logged in as <user_id…> via moor` with a logout icon. Clicking it POSTs `/auth/logout`, which clears all dashboard-auth cookies and redirects back to `/login`.

### Audit log

Every login start, success, failure, and session-verify failure is written as a JSON line to `$MOOR_HOME/logs/dashboard-auth.log`. Sensitive fields (`access_token`, `refresh_token`, `code`, `code_verifier`, `state`, `Authorization` header) are redacted before logging.

### Custom providers

To plug a non-Moor OAuth provider (e.g. Google, GitHub, custom OIDC), create a plugin that registers a `DashboardAuthProvider`:

```python
# ~/.moor/plugins/dashboard-auth-myidp/__init__.py
from moor_cli.dashboard_auth import DashboardAuthProvider, Session, LoginStart
...
The bundled first consumer is the **drain** provider (`plugins/dashboard_auth/drain`): `moor-account-service` provisions a per-agent secret via `MOOR_DASHBOARD_DRAIN_SECRET`, and the provider verifies inbound bearer tokens against it with a constant-time compare, registering `/api/gateway/drain` as token-authable. It **fails closed** — a weak/short secret (< 256 bits) is rejected at registration and the endpoint stays disabled; it's a no-op when the env var is unset. Behavioural knobs (`scope`, `min_secret_chars`) live under `dashboard.drain_auth` in `config.yaml`.
...
```bash
# Quick env-var path.
MOOR_DASHBOARD_OAUTH_CLIENT_ID=agent:test \
  moor dashboard --host 0.0.0.0
...
The recipe below uses the username/password path because it's the quickest to stand up on a trusted network; for the OAuth path see [Default provider: Moor inc.](#default-provider-moor-research).

### On the backend (the remote machine)

```bash
# 1. Set the dashboard login credentials in ~/.moor/.env (secrets file, 0600).
cat >> ~/.moor/.env <<'EOF'
MOOR_DASHBOARD_BASIC_AUTH_USERNAME=admin
MOOR_DASHBOARD_BASIC_AUTH_PASSWORD=choose-a-strong-password
...
- **Remote URL** — `http://<backend-host>:9119` (path prefixes like `/moor` are supported if you front it with a reverse proxy)
...
The session refreshes automatically and survives restarts when `MOOR_DASHBOARD_BASIC_AUTH_SECRET` is set on the backend.

### Environment-variable override

Instead of the in-app setting, you can point the desktop at a backend with an env var before launching it. When `MOOR_DESKTOP_REMOTE_URL` is set, it overrides the saved in-app URL (the Gateway settings panel shows an "env override" badge and disables editing); you still **Sign in** with your username and password from the panel.

| Env var | Value |
|---------|-------|
| `MOOR_DESKTOP_REMOTE_URL` | `http://<backend-host>:9119` |

### Troubleshooting

- **"Remote gateway incomplete"** — you haven't entered a remote URL.
- **Sign-in fails with 401 / "Invalid credentials"** — the username or password doesn't match the backend's `MOOR_DASHBOARD_BASIC_AUTH_USERNAME` / `MOOR_DASHBOARD_BASIC_AUTH_PASSWORD`. The backend returns the same generic error for unknown user and wrong password, so check both. Confirm the gate with `curl -s http://<host>:9119/api/status | jq '.auth_required, .auth_providers'` — it should report `true` and include `"basic"`.
...
- **Signed out on every restart** — set `MOOR_DASHBOARD_BASIC_AUTH_SECRET` to a stable value; otherwise the signing key is regenerated per boot.
...
The frontend is built with React 19, TypeScript, Tailwind CSS v4, and shadcn/ui-style components. Production builds output to `moor_cli/web_dist/` which the FastAPI server serves as a static SPA.

## Automatic Build on Update

When you run `moor update`, the web frontend is automatically rebuilt if `npm` is available. This keeps the dashboard in sync with code updates. If `npm` isn't installed, the update skips the frontend build and `moor dashboard` will build it on first launch.
...
| Theme | Character |
|-------|-----------|
| **Moor Teal** (`default`) | Dark teal + cream, system fonts, comfortable spacing |
| **Moor Teal (Large)** (`default-large`) | Same as default with 18px text and roomier spacing |
| **Moor Blue** (`moor-blue`) | Moor-branded blue accents with airy spacing |
