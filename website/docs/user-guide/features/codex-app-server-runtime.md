...
Moor can optionally hand `openai/*`, `openai-codex/*` and [named custom provider](#named-custom-providers) turns to the [Codex CLI app-server](https://github.com/openai/codex) instead of running its own tool loop. When enabled, terminal commands, file edits, sandboxing, and MCP tool calls all execute inside Codex's runtime — Moor becomes the shell around it (sessions DB, slash commands, gateway, memory and skill review).
...
:::tip
Not using OpenAI Codex? `moor setup --portal` configures a non-Codex backend with Claude/Gemini/etc. in one step. See [Moor Portal](../../integrations/moor-portal.md).
:::
...
- **Native Codex plugins** — Linear, GitHub, Gmail, Calendar, Canva, etc. — installed via `codex plugin` are auto-migrated and active in your Moor session.
- **Moor' richer tools come along** — web_search, web_extract, browser automation, vision, image generation, skills, and TTS work via an MCP callback. Codex calls back into Moor for tools it doesn't have built in.
- **Memory and skill nudges keep working** — Codex's events are projected into Moor' message shape so the self-improvement loop sees a normal-looking transcript.
- **Your Moor persona rides along** — the composed system prompt (SOUL.md, MEMORY.md/USER.md, per-channel `system_prompt` overrides) is sent to the codex thread once as developer instructions when the thread starts, and Codex's built-in personality is disabled so it cannot compete with yours.
...
These ship with `codex app-server` itself — no Moor involvement, no MCP, no plugins. All five are available the moment the runtime starts:
...
- **`update_plan`** — codex's internal todo / plan tracker. Equivalent of Moor' `todo` tool, but managed entirely inside codex's runtime.
- **`view_image`** — load a local image file into the conversation so the model can see it.
- **`web_search`** — codex has its own built-in web search when configured. Moor also exposes `web_search` (Firecrawl-backed) via the callback below; the model picks whichever it prefers.
...
When you enable the runtime, Moor queries codex's `plugin/list` RPC and writes a `[plugins."<name>@openai-curated"]` entry for every plugin you have installed. The plugins themselves are managed by codex and authorized once via codex's own UI.
...
### 3. Moor tool callback (MCP server, registered in `~/.codex/config.toml`)

Moor registers itself as an MCP server so codex can call back for tools codex doesn't ship with. Available via the callback:
...
- **`image_generate`** — image generation through Moor' image_gen plugin chain.
...
These four Moor tools require the running AIAgent context (mid-loop state) to dispatch, and a stateless MCP callback can't drive them. Switch back to the default runtime (`/codex-runtime auto`) when you need any of them:

- **`delegate_task`** — spawn subagents
- **`memory`** — Moor' persistent memory store
- **`session_search`** — cross-session search
- **`todo`** — Moor' todo store (codex's `update_plan` is the in-runtime equivalent)
...
|  | Moor default runtime | Codex app-server (opt-in) |
|---|---|---|
...
2. **Codex OAuth login.** The codex subprocess reads `~/.codex/auth.json`. Two ways to populate it:
   ...
   <a id="named-custom-providers"></a>**Or: a named custom provider.** A `providers.<name>` entry in Moor config can use this runtime when the **same name** is defined as a Codex provider. Moor config:
   ...
   Moor sends only `model` and `modelProvider = "my-gateway"` on `thread/start`; codex resolves `base_url` and reads the key from `env_key` in its own environment. **Moor never forwards the API key**, so `MY_GATEWAY_API_KEY` must be present in the process environment Moor runs in — `~/.moor/.env` is loaded at startup and provider credentials are inherited by the codex subprocess. Auxiliary calls (titles, compression, memory review) still use Moor' own `providers.my-gateway` entry.

   Caveats: the name after `custom:` is the `providers:` config key and must match the `[model_providers.<name>]` table name exactly — if it does not exist on the codex side, codex reports an unknown provider rather than silently using the Moor endpoint. Anonymous `provider: custom` (a bare `base_url`) is not eligible: it has no stable name to hand to codex, so it stays on Moor' standard runtime.
...
3. **(Optional) Install the Codex plugins you want.** When you enable the runtime, Moor auto-migrates whichever curated plugins you've already installed via Codex CLI:
   ...
   ```
   Moor will discover them and write `[plugins."<name>@openai-curated"]` entries to `~/.codex/config.toml` automatically.
...
If the Moor process cannot resolve `codex` from `PATH` — typical for gateway services,
...
`model.codex_bin` is used everywhere Moor spawns codex: the `/codex-runtime` availability
...
Moor' background self-improvement fires on counter thresholds:
...
**Both keep working on the codex runtime.** The codex path projects each completed `commandExecution` / `fileChange` / `mcpToolCall` / `dynamicToolCall` item into a synthetic `assistant tool_call` + `tool` result message, so by the time the review runs it sees the same shape it sees on the default Moor runtime.
...
One detail: the review fork itself needs to call Moor' agent-loop tools (`memory`, `skill_manage`), which require Moor' own dispatch. So when the parent agent is on `codex_app_server`, the review fork is **downgraded to `codex_responses`** — same OAuth credentials, same `openai-codex` provider, but talks to OpenAI's Responses API directly so Moor owns the loop and the agent-loop tools work. This is invisible to the user.

Net effect: enable the codex runtime and your memory + skill nudges keep firing exactly as they would otherwise.

## How approvals work

Codex requests approval before executing commands or applying patches. These get translated into Moor' standard "Dangerous Command" prompt:
...
For `apply_patch` (file edit) approvals, Moor shows a summary of what changed (`1 add, 1 update: src/new.py, src/old.py`) when codex provides the data via the corresponding `fileChange` item.

## Permission profiles

Codex has three built-in permission profiles:
- `:read-only` — no writes; every shell command requires approval
- `:workspace` — writes within the current workspace allowed without prompts (Moor' default when you enable the runtime)
- `:danger-no-sandbox` — no sandbox at all (don't use this unless you understand it)

You can override the default in `~/.codex/config.toml` outside Moor' managed block:
...
When this runtime is on with the `openai-codex` provider, **auxiliary tasks (title generation, context compression, vision auto-detect, the background self-improvement review fork) also flow through your ChatGPT subscription by default**, because Moor' auxiliary client uses the main provider/model when no per-task override is set.
...
The self-improvement review fork inherits the main runtime via `_current_main_runtime()` and Moor downgrades it from `codex_app_server` to `codex_responses` automatically (so the fork can actually call `memory` and `skill_manage` — Moor' own agent-loop tools). That fork still uses your subscription auth unless you've routed aux tasks elsewhere.
...
Anything **outside** that block is yours. Re-running migration (via `/codex-runtime codex_app_server`, whenever you toggle the runtime on, or `moor codex-runtime migrate`) replaces the managed block in place but preserves user content above and below it verbatim. This means you can:

- Add your own MCP servers Moor doesn't know about
...
**Same-name servers.** If your own `[mcp_servers.<name>]` table (outside the block) uses the same name as a server in Moor' `mcp_servers`, your table wins: Moor skips its projection for that name instead of emitting a second `[mcp_servers.<name>]` header (which is invalid TOML and would stop codex from starting). The migration report lists such names under "Kept N user-owned MCP server(s)". To let Moor manage the server, delete your table and re-run the migration. The rendered file is parsed as TOML before it replaces `config.toml`; an unparsable result is reported and the existing file is left untouched.

### Running the migration from a script

```bash
moor codex-runtime migrate            # rewrite the managed block for the active profile
moor codex-runtime migrate --dry-run  # report only, no write
moor codex-runtime migrate --json     # machine-readable report (migrated, preserved_user_servers, errors, …)
moor -p work codex-runtime migrate    # a named profile's mcp_servers
```
...
Moor does NOT rewrite `HOME` when spawning the codex app-server subprocess (we use `os.environ.copy()` and only overlay `CODEX_HOME` and `RUST_LOG`). This means:
...
Moor' `mcp_servers` config is auto-translated to the TOML format Codex expects. The migration runs every time you enable the runtime and is idempotent — re-runs replace the managed section but preserve any user-edited Codex config.

What translates:

| Moor (`config.yaml`) | Codex (`config.toml`) |
|---|---|
...
Plugins installed via `codex plugin` (Linear, GitHub, Gmail, Calendar, Canva, etc.) are discovered through Codex's `plugin/list` RPC. For each plugin where `installed: true`, Moor writes a `[plugins."<name>@openai-curated"]` block enabling it in your Moor session.

This means: when your friend says "I have Calendar and GitHub set up in my Codex CLI" and they enable Moor' codex runtime, Moor activates those automatically. No re-configuration needed.
...
- Plugin OAuth — you authorize each plugin once in Codex itself; Moor doesn't touch credentials.

## Moor tool callback (the new MCP server)
...
**Tools NOT available:** `delegate_task`, `memory`, `session_search`, `todo`. These need the running AIAgent context to dispatch (mid-loop state) and a stateless MCP callback can't drive them. Use the default Moor runtime (`/codex-runtime auto`) when you need these.
...
This runtime is **opt-in beta**. Working as of Moor Agent 2026.5 + Codex CLI 0.130.0:
...
- Moor web_search through codex (verified live: "OpenAI Codex CLI – Getting Started" returned end-to-end)
...
- **`fallback_providers` fail over only on quota and rate-limit failures.** When a codex app-server turn fails with a billing / usage-limit / rate-limit error, Moor switches to the configured [fallback provider](./fallback-providers.md) and retries the same turn on it; auth failures (`codex login` expired), turn timeouts and unknown-model errors do not fail over on this runtime and surface as the turn's error instead.
- **Prior Moor history is seeded only into a thread codex starts from scratch.** A codex thread that codex hands back via `thread/resume` already holds the conversation. When no resumable thread exists — the session ran on another provider before `/model` switched to openai-codex, codex could not resume the stored thread, or the running thread was retired — the new thread's `developerInstructions` carry Moor' system prompt followed by the session's prior turns (user and assistant text, tool names, tool-result previews; the most recent ~32K characters). When the composed prompt changes mid-session (for example `/personality` in the TUI or Desktop), the next turn retires the running thread and starts a new one carrying the updated prompt plus that same history seed.
- **The codex thread itself does survive a restart.** After each committed turn Moor stores the codex thread id on the session row (`codex_thread_id` in the session's `model_config`, `moor sessions` / `state.db`). The next agent built for that same Moor session — a later `/api/sessions/{id}/chat` request, or the first turn after the API server or gateway restarts — issues `thread/resume` for the stored id before `turn/start`, so the model keeps its own memory of the earlier turns (that is why no history seed is sent on resume). When codex cannot hand the thread back (its rollout was deleted, `CODEX_HOME` changed, the previous app-server was killed while still writing it), Moor fails closed: it drops the stored id, starts a fresh thread and shows one line — `Codex thread could not be resumed; starting a new one.` — on the status rail of the surface you are on (CLI, TUI/Desktop, messaging gateway). A `/new` session never resumes an older thread.
...
If you find a bug, [open an issue](https://github.com/thisismamad-n/Moor/issues) with the output of `moor logs --since 5m`. Mention `codex-runtime` in the title so it's easy to triage.

## Architecture

```
                ┌─── Moor shell (CLI / TUI / gateway) ───┐
...
For implementation details, see [PR #24182](https://github.com/thisismamad-n/Moor/pull/24182) and the [Codex app-server protocol README](https://github.com/openai/codex/blob/main/codex-rs/app-server/README.md).
