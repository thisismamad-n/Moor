...
When a top-level agent provides a `tasks` array, Moor returns one background handle and runs the subagents in parallel. Results come back **per completion unit**, not once at the end:
