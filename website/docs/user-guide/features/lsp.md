...
2. Download the latest release zip from
   ...
   and extract it.
3. Point Moor at the extracted bundle — the directory that contains
   `PowerShellEditorServices/Start-EditorServices.ps1`. Either:
   - set `lsp.servers.powershell.command: ["/path/to/bundle"]` in
     `config.yaml`, or
   - extract it to `<MOOR_HOME>/lsp/PowerShellEditorServices`, or
   - export `PSES_BUNDLE_PATH=/path/to/bundle`.

`moor lsp status` reports `installed` once `pwsh` is found; if the
...
run `moor lsp install typescript` or auto-install fires on first
use.

## CLI

```
moor lsp status          # service state + per-server install status
moor lsp list            # registry, optionally --installed-only
moor lsp install <id>    # eagerly install one server
moor lsp install-all     # try every server with a known recipe
moor lsp restart         # tear down running clients
moor lsp which <id>      # print resolved binary path
```

`moor lsp status` is the best starting point — it shows which
...
lsp:
  ...
  #   auto    — install via npm/pip/go install into <MOOR_HOME>/lsp/bin
...
`<MOOR_HOME>/lsp/bin/`. NPM packages land in
`<MOOR_HOME>/lsp/node_modules/` with bin symlinks one level up.
...
**`moor lsp status` shows a server as "missing"**

The binary isn't on PATH and isn't in `<MOOR_HOME>/lsp/bin/`. Run
`moor lsp install <server_id>` to attempt an auto-install, or
install the binary manually through the language's normal toolchain.

**`Backend warnings` section in `moor lsp status`**
...
When `moor lsp status` shows a `Backend warnings` section, install
...
Check `~/.moor/logs/agent.log` for `[agent.lsp.client]` entries —
...
the rest of the session. Run `moor lsp restart` to clear the set;
