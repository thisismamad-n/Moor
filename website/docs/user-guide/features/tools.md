...
description: "Overview of Moor Agent's tools — what's available, how toolsets work, and terminal backends"
---
...
Moor ships with a broad built-in tool registry covering web search, browser automation, terminal execution, file editing, memory, delegation, scheduled tasks, Home Assistant, and more.
...
| **X Search** | `x_search` | Search X (Twitter) posts and threads via xAI's built-in `x_search` Responses tool — gated on xAI credentials (SuperGrok OAuth or `XAI_API_KEY`); off by default, opt in via `moor tools` → 🐦 X (Twitter) Search. |
...
moor chat --toolsets "web,terminal"
...
See [Toolsets Reference](../../reference/toolsets-reference.md) for the full set, including platform presets such as `moor-cli`, `moor-telegram`, and dynamic MCP toolsets like `mcp-<server>`.
...
# Configure
moor config set terminal.backend singularity
moor config set terminal.singularity_image ~/python.sif
```
...
:::warning
On messaging platforms, if sudo fails, the output includes a tip to add `SUDO_PASSWORD` to `~/.moor/.env`.
:::
