...
# Optional: only if a browser must call Moor directly
...
Use this endpoint when integrating dashboards, browser UIs, or control planes so they can discover whether the running Moor version supports runs, streaming, cancellation, and session continuity without depending on private Python internals.

## Browser-extension control

Moor can route browser tools through an authenticated extension that controls
the browser session associated with the current Moor session. The feature is
...
register only for an existing server session. Moor derives the controller
...
Moor preserves the existing browser backend. Once the gateway binds a
...
Moor never retries the same action through another backend.
...
2. Moor returns a single-use ticket with a 30-second TTL and the filtered,
   server-bound controller scope.
3. Open `GET /v1/browser-control/ws` with both WebSocket subprotocols:
   `moor-browser-control-v1` and
   `moor-browser-control-ticket.<ticket>`.
...
Moor sends `browser.controller.command` frames containing `command_id`,
...
Interrupt a running agent turn. The endpoint returns immediately with `{"status": "stopping"}` while Moor asks the active agent to stop at the next safe interruption point.
...
External UIs can manage Moor sessions over REST without standing up the dashboard. All endpoints are gated by `API_SERVER_KEY` and live under `/api/sessions/*`.
...
Configure the key via `API_SERVER_KEY` env var. If you need a browser to call Moor directly, also set `API_SERVER_CORS_ORIGINS` to an explicit allowlist.
...
To give multiple users their own isolated Moor instance (separate config, memory, skills), use [profiles](/user-guide/profiles):
...
In Open WebUI, add each as a separate connection. The model dropdown shows `alice` and `bob` as distinct models, each backed by a fully isolated Moor instance. See the [Open WebUI guide](/user-guide/messaging/open-webui#multi-user-setup-with-profiles) for details.
