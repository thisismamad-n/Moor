...
description: "Use Moor Agent inside ACP-compatible editors such as VS Code, Zed, and JetBrains"
---

# ACP Editor Integration

Moor Agent can run as an ACP server, letting ACP-compatible editors talk to Moor over stdio and render:
...
ACP is a good fit when you want Moor to behave like an editor-native coding agent instead of a standalone CLI or messaging bot.

## What Moor exposes in ACP mode

Moor runs with a curated `hermes-acp` toolset designed for editor workflows. It includes:
...
Install Moor normally, then add the ACP extra:
...
For Zed registry installs, Zed launches Moor through the official ACP Registry entry. That entry uses a `uvx` distribution that runs:
...
3. Search for **Moor Agent**.
4. Install it and start a new Moor external-agent thread.
...
The source copy of Moor' official ACP Registry metadata lives at:
...
Provider resolution uses Moor' normal runtime resolver, so ACP inherits the currently configured provider and credentials. Moor also advertises a terminal auth method (`--setup`) for first-run registry clients; this opens Moor' interactive model/provider setup.
...
- In Zed, open the ACP Registry with `zed: acp registry` and search for **Moor Agent**.
...
Install `uv` from the official uv installation docs, then retry the Moor Agent thread from Zed.
...
change your Moor-wide default — use `hermes model` for that.
