...
description: "Use Moor Agent inside ACP-compatible editors and collaboration platforms"
---
...
Moor. ACP is a good fit when you want Moor to keep its existing identity,
provider setup, memory, skills, and tools while another application owns the
conversation transport.

## What Moor exposes in ACP mode

Moor runs with a curated `moor-acp` toolset designed for editor workflows. It includes:
...
Any of the following starts Moor in ACP mode:
...
Moor logs to stderr so stdout remains reserved for ACP JSON-RPC traffic.
...
```bash
moor acp --setup-browser           # interactive (prompts before ~400 MB download)
moor acp --setup-browser --yes     # accept the download non-interactively
```

This is the standalone command. The terminal-auth flow (`moor acp --setup`) also offers the browser bootstrap as a follow-up question after model selection, so most users never need to run `--setup-browser` directly.
...
This is a transport integration, not a second Moor installation. The
subprocess launched by `buzz-acp` uses the same Moor configuration,
credentials, memory, skills, and state as `moor` on that host.

(This is distinct from [Buzz Desktop's managed runtime](#buzz-desktop), which
spawns Moor locally as a preset harness. The relay bridge is for joining Buzz
*channels* as an agent identity, typically on a server.)

Prerequisites:

- Complete the ACP installation and `moor acp --check` above.
- Build `buzz-acp` and the `buzz` CLI from the
  [Buzz repository](https://github.com/block/buzz)
  (`cargo build --release -p buzz-acp`).
- Mint a dedicated Nostr keypair for Moor (`buzz-admin generate-key`) and
...
export BUZZ_ACP_AGENT_COMMAND="moor"
...
the same operating-system user that owns the intended Moor home. Setup,
...
The bridge discovers every Buzz channel where the Moor identity is a member
and automatically subscribes when it is added to another channel. Buzz channel
membership therefore remains the access boundary; Moor does not need a
separate channel list in its own configuration.

To expose Moor ACP activity in the owner's Buzz Desktop, add:
...
where Moor is expected to work.
...
2. Select **Moor Agent** from the built-in agent list.
3. Connect and start chatting.

If you want to define Moor manually, add it through VS Code settings under `acp.agents`:
...
Configure Moor as a custom agent server in Zed settings:
...
3. Start a new Moor external-agent thread.

Prerequisites:

- Configure Moor provider credentials first with `moor model`, or set them in `~/.moor/.env` / `~/.moor/config.yaml`.

### JetBrains

Use an ACP-compatible plugin and point it at `moor acp` or `moor-acp`.

### Buzz Desktop

[Buzz](https://github.com/block/buzz) ships Moor Agent as a preset runtime.
With Moor installed the normal way, Buzz discovers it automatically —
open **Settings → Runtimes** and Moor appears under your runtimes.

If discovery fails (older installs), make sure the ACP launcher resolves on a
login-shell PATH:

```bash
command -v moor-acp || command -v moor
```

Recent installs write both `moor` and `moor-acp` launchers into
`~/.local/bin`; running `moor update` adds the `moor-acp` launcher to
older installs. As a manual fallback, configure Buzz's agent command as
`moor` with args `["acp"]`.

#### Model picker

Buzz Desktop (v0.5.1+) renders Moor' full model menu in the agent's runtime
settings. The list comes from Moor itself over ACP: it shows every model
from providers you have authenticated in Moor (the same inventory behind
`moor model` and the `/model` command), so a model missing from the menu
means its provider has no credentials configured on the Moor side.
...
Leave it there when the runtime is Moor.

Two behaviors combine on this path. The `moor-acp` toolset includes `terminal`
and `execute_code`, and Buzz's ACP bridge answers Moor' permission requests
itself with `allow_once` rather than surfacing them. A Moor agent in Buzz
...
ACP mode uses the same Moor configuration as the CLI:
...
Provider resolution uses Moor' normal runtime resolver, so ACP inherits the currently configured provider and credentials. Moor also advertises a terminal auth method (`--setup`) for first-run ACP clients; this opens Moor' interactive model/provider setup.

## Host integration

These variables are set by an **ACP host process** (an editor or another agent
harness) on the Moor subprocess it spawns. They are not user configuration —
do not set them by hand in `.env` or `config.yaml`.

| Variable | Value | Effect |
|----------|-------|--------|
| `MOOR_ACP_SKIP_CONFIGURED_MCP` | `1` | Skip starting the **globally configured** MCP servers from `config.yaml` before the ACP JSON-RPC loop begins. |

Moor normally starts every MCP server configured in `config.yaml` before it
...
The underlying `AIAgent` still uses Moor' normal persistence/logging paths, but ACP `list/load/resume/fork` are scoped to the currently running ACP server process.

## Working directory behavior

ACP sessions bind the editor's cwd to the Moor task ID so file and terminal tools run relative to the editor workspace, not the server process cwd.
...
| `allow_always` | Allow always | All future sessions | Yes (written to the Moor permanent allowlist) |
| `deny` | Deny | This one tool call | No |

`allow_session` is the right default for an editor workflow where you trust an agent for the duration of a task but don't want to grant a long-lived allowlist entry. The safety trade-off is straightforward: the broader the scope, the less the editor will interrupt you, and the more damage a misbehaving agent (or prompt injection) can do before you notice. Start with `allow_once` for unfamiliar commands; promote to `allow_session` once you've seen the agent run the same pattern correctly a few times; reserve `allow_always` for truly idempotent commands you trust forever (e.g. `git status`).

The ACP bridge maps these options onto Moor' internal approval semantics — `allow_always` writes a permanent allowlist entry the same way the CLI does, while `allow_session` only affects the in-process approval cache for the current ACP session.
...
or by editing `~/.moor/.env`. The terminal auth flow (`moor acp --setup`) can also trigger the interactive provider/model setup.
