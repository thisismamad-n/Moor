...
The bundled `plan` skill is a good example. Running `/plan [request]` loads the skill's instructions, telling Moor to inspect context if needed, write a markdown implementation plan instead of executing the task, and save the result under `.moor/plans/` relative to the active workspace/backend working directory.
...
- **Create locally, update in place**: New agent-created skills are written to `~/.moor/skills/`. Existing skills are modified where they are found, including skills under `external_dirs`, when the agent uses `skill_manage` actions such as `patch`, `edit`, `write_file`, `remove_file`, or `delete`.
