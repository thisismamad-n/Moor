...
All skills live in **`~/.moor/skills/`** — the primary directory and source of truth. On fresh install, bundled skills are copied from the repo. Hub-installed and agent-created skills also go here. The agent can modify or delete any skill.

You can also point Moor at **external skill directories** — additional folders scanned alongside the local one. See [External Skill Directories](#external-skill-directories) below.
...
**At install time** (applies to the default `~/.moor` profile): the
...
run `moor skills opt-out` afterwards if you want the profile empty.

**At profile-create time** (named profiles):

```bash
moor profile create research --no-skills
```

**On an already-installed profile** (default or named), toggle it at runtime:

```bash
moor skills opt-out            # stop future seeding — nothing on disk is touched
moor skills opt-out --remove   # also delete UNMODIFIED bundled skills (confirms first)
moor skills opt-in --sync      # undo: remove the marker and re-seed now
```

All of these paths write a `.no-bundled-skills` marker into the profile directory. While the marker is present, the installer, `moor update`, and any skill sync all skip bundled-skill seeding for that profile. Delete the marker (or run `moor skills opt-in`) to re-enable.
...
```bash
/ocr-and-documents ~/.moor/cache/scratch/scan.pdf extract the tables   # loads one skill; ~/.moor/cache/scratch/scan.pdf is the argument
```

For combinations you use repeatedly, prefer a [skill bundle](#skill-bundles) —
same effect under one short command.

(Plan mode works the same way but is a built-in command now: `/plan [request]` tells Moor to inspect context if needed, write a markdown implementation plan instead of executing the task, and save the result under `.moor/plans/` relative to the active workspace/backend working directory.)

You can also interact with skills through natural conversation:

```bash
moor chat --toolsets skills -q "What skills do you have?"
moor chat --toolsets skills -q "Show me the axolotl skill"
```
...
description, the standard section order, moor-tool framing, no invented
commands).
...
/home/user/.moor/cache/chart-q4-2025.png
...
```text
~/.moor/skills/                  # Single source of truth
...
and `examples/`. Unreferenced repository files are not copied. Moor scans the
...
policy above), Moor can run [NVIDIA SkillEvaluator](https://github.com/NVIDIA/SkillEvaluator)
...
If you maintain skills outside of Moor — for example, a shared `~/.agents/skills/` directory used by multiple AI tools — you can tell Moor to scan those directories too.

Add `external_dirs` under the `skills` section in `~/.moor/config.yaml`:
...
- **Create locally, update in place**: New agent-created skills are written to `~/.moor/skills/` (or `skills.create_dir` when configured — see below). Existing skills are modified where they are found, including skills under `external_dirs`, when the agent uses `skill_manage` actions such as `patch` (targeted or full rewrite), `write_file`, `remove_file`, or `delete`.
...
```text
~/.moor/skills/               # Local (primary, read-write)
├── devops/deploy-k8s/
│   └── SKILL.md
└── mlops/axolotl/
    └── SKILL.md
...
By default the agent writes new skills to the profile-local `~/.moor/skills/`. If you want agent-created skills to land somewhere else — a shared "brain" directory, a git-tracked repo, or a fleet-wide skills volume — set `create_dir` under the `skills` section:
...
Paths support `~` expansion and `${VAR}` substitution; relative paths resolve against your Moor home. Setting `create_dir` to the local skills dir is the same as leaving it unset.


## Project-Local Skills

Repos can carry their own skills, active only for sessions started inside that project — the same pattern other agent harnesses use for repo-local configuration. When you launch Moor inside a git checkout, it looks for skills in:

```text
<project-root>/.moor/skills/    # moor-native location
...
Skills are procedure documents the agent follows, so Moor does **not** auto-load them from arbitrary cloned repos. The first time you run Moor in a repo with project skills, the banner shows a notice:

```text
◆ 3 project skill(s) found in /home/you/myproject but not loaded — run `moor skills trust` to enable them.
```

Trust the repo once (from inside it, or by passing the path):

```bash
moor skills trust             # trust the current repo
moor skills trust ~/myproject # or explicitly
moor skills untrust           # revoke
```

Trusted roots are stored in `skills.trusted_project_dirs` in `~/.moor/config.yaml`. Set `skills.project_discovery: false` to turn the feature off entirely (no scanning, no notices).

### Precedence

Project skills are the **highest-precedence tier**: `project → local (~/.moor/skills/) → external_dirs`. A project skill named `deploy` overrides a same-named profile or bundled skill for sessions inside that repo — that's the point: vendored repo skills win on their home turf, without touching your global profile. Project skills are tagged `[project]` in the agent's skill index so provenance stays visible.

Like external dirs, project skill directories are treated as repo-owned: autonomous skill maintenance (the curator) never modifies them, and new agent-created skills always go to `~/.moor/skills/`.

### Scan-time quarantine

Trust is a repo-level decision, but a repo's skill content changes with every `git pull`. To close that gap, every project skill is scanned with the same security scanner used for Skills Hub installs before it enters the index. A skill whose scan verdict is **dangerous** (prompt-injection directives, credential-exfiltration commands, hidden-text tricks) is quarantined: it does not appear in the skill index, `skills_list`, slash commands, and refuses to load by name with an explanatory error. Scans are content-hash cached under `~/.moor/cache/project_skill_scans/` (never inside your repo) and re-run automatically when the skill's content changes.
...
In the TUI and Desktop the project root follows each **session's workspace** (the directory shown in the sidebar / set with the workspace picker), so starting `moor --tui` inside a trusted repo registers its project skills as slash commands even when `terminal.cwd` is left at the default placeholder `.`; two sessions open in two repos each see their own.
...
Bundles live in **`~/.moor/skill-bundles/<slug>.yaml`** and look like this:
...
- `description` (optional) — short text shown in `/bundles` and `moor bundles list`.
...
moor bundles create research

# Overwrite an existing bundle
moor bundles create backend-dev --skill ... --force

# Delete a bundle
moor bundles delete backend-dev

# Re-scan ~/.moor/skill-bundles/ and report changes
moor bundles reload
```
...
- You want to ship a team-wide "task profile" by checking the bundle YAML into a shared dotfiles repo and symlinking it into `~/.moor/skill-bundles/`.

A bundle is just a YAML alias — it doesn't install skills for you. The skills themselves must already be present (in `~/.moor/skills/` or an external skill directory). Otherwise the bundle invocation just skips the missing ones.
...
Staged writes survive restarts under `~/.moor/pending/skills/` and are
...
Unfiltered searches (CLI, TUI, and the dashboard) are answered from a cached centralized index that covers the external registries. That index is rebuilt periodically, so when it has no match for your query Moor also asks `skills.sh`, ClawHub, LobeHub and well-known endpoints directly — a skill published minutes ago still shows up. That extra pass gets at most 8 seconds of the search budget, so a slow registry cannot turn a miss into a long wait. Custom GitHub taps are not part of that fallback (search them with `--source github`, or via the index once it catches up), and provider filters such as `--source nvidia` do not trigger it (those registries carry no provider data).

### Common commands

```bash
moor skills browse                              # Browse all hub skills (official first)
moor skills browse --source official            # Browse only official optional skills
moor skills search kubernetes                   # Search all sources
moor skills search react --source skills-sh     # Search the skills.sh directory
moor skills search https://mintlify.com/docs --source well-known
moor skills inspect openai/skills/k8s           # Preview before installing
moor skills install openai/skills/k8s           # Install with security scan
moor skills install official/security/1password
moor skills install skills-sh/vercel-labs/json-render/json-render-react --force
moor skills install well-known:https://mintlify.com/docs/.well-known/skills/mintlify
...
moor skills list --source hub                   # List hub-installed skills
moor skills check                               # Check installed hub skills for upstream updates
moor skills update                              # Reinstall hub skills with upstream changes when needed
moor skills audit                               # Re-scan all hub skills for security
moor skills uninstall k8s                       # Remove a hub skill
moor skills reset google-workspace              # Un-stick a bundled skill from "user-modified" (see below)
moor skills reset google-workspace --restore    # Also restore the bundled version, deleting your local edits
moor skills publish skills/my-skill --to github --repo owner/repo
moor skills snapshot export setup.json          # Export skill config
moor skills tap add myorg/skills-repo           # Add a custom GitHub source
```
...
|--------|---------|-------|
| `official` | `official/security/1password` | Optional skills shipped with Moor. |
| `skills-sh` | `skills-sh/vercel-labs/agent-skills/vercel-react-best-practices` | Searchable via `moor skills search <query> --source skills-sh`. Moor resolves alias-style skills when the skills.sh slug differs from the repo folder. |
...
Moor currently integrates with these skills ecosystems and discovery sources:

#### 1. Official optional skills (`official`)

These are maintained in the Moor repository itself and install with built-in trust.
...
This is Vercel's public skills directory. Moor can search it directly, inspect skill detail pages, resolve alias-style slugs, and install from the underlying source repo.
...
```bash
moor skills search https://mintlify.com/docs --source well-known
moor skills inspect well-known:https://mintlify.com/docs/.well-known/skills/mintlify
moor skills install well-known:https://mintlify.com/docs/.well-known/skills/mintlify
```

#### 4. Direct GitHub skills (`github`)

Moor can install directly from GitHub repositories and GitHub-based taps. This is useful when you already know the repo/path or want to add your own custom source repo.
...
```bash
moor skills install openai/skills/k8s
...
categorization, no moor-side changes required.
...
- Moor source id: `browse-sh`
- Trust level: `community`

```bash
moor skills search airbnb --source browse-sh
moor skills inspect browse-sh/airbnb.com/search-listings-ddgioa
moor skills install browse-sh/airbnb.com/search-listings-ddgioa
```
...
- Scope: `SKILL.md` plus exact referenced support files in the allowlisted directories. Moor does not enumerate or copy unrelated files from the host.

```bash
moor skills install https://sharethis.chat/SKILL.md
moor skills install https://example.com/my-skill/SKILL.md --category productivity
```
...
moor skills install https://example.com/SKILL.md --name sharethis-chat

# Or inside a chat session:
/skills install https://example.com/SKILL.md --name sharethis-chat
```

Trust level is always `community` — the same security scan runs as for every other source. The URL is stored as the install identifier, so `moor skills update` re-fetches from the same URL automatically when you want to refresh.
...
`moor skills inspect ...` now also surfaces upstream metadata when available:
- repo URL
...
| Level | Source | Policy |
|-------|--------|--------|
| `builtin` | Ships with Moor | Always trusted |
...
```bash
moor skills check          # Report which installed hub skills changed upstream
moor skills update         # Reinstall only the skills with updates available
moor skills update react   # Update one specific installed hub skill
moor skills update react --force   # Overwrite a skill you've edited locally
```
...
If you want to share a curated set of skills — for your team, your org, or publicly — you can publish them as a **tap**: a GitHub repository other Moor users add with `moor skills tap add <owner/repo>`. No server, no registry sign-up, no release pipeline. Just a directory of `SKILL.md` files.
...
- Each skill directory must contain a `SKILL.md` with standard [SKILL.md frontmatter](#skillmd-format) (`name`, `description`, plus optional `metadata.moor.tags`, `version`, `author`, `platforms`, `metadata.moor.config`).
...
Moor discovers skills by listing every subdirectory of the tap path and probing each for `SKILL.md`.
...
After pushing that to GitHub, any Moor user can subscribe and install:

```bash
moor skills tap add my-org/moor-skills
moor skills search deploy
moor skills install my-org/moor-skills/deploy-runbook
```

#### Non-default paths

If your skills don't live under `skills/` (common when you're adding a `skills/` subtree to an existing project), edit the tap entry in `~/.moor/skills/.hub/taps.json`:
...
The `moor skills tap add` CLI defaults new taps to `path: "skills/"`; edit the file directly if you need a different path. `moor skills tap list` shows the effective path per tap.
...
```bash
moor skills install owner/repo/skills/my-workflow
```
...
Taps are stored in `~/.moor/skills/.hub/taps.json` (created on demand).

## Bundled skill updates (`moor skills reset`)

Moor ships with a set of bundled skills in `skills/` inside the repo. On install and on every `moor update`, a sync pass copies those into `~/.moor/skills/` and records a manifest at `~/.moor/skills/.bundled_manifest` mapping each skill name to the content hash at the time it was synced (the **origin hash**).

On each sync, Moor recomputes the hash of your local copy and compares it to the origin hash:
...
Generated runtime caches inside a skill (`__pycache__/`, `.pytest_cache/`, `.mypy_cache/`, `.ruff_cache/`, and a `.pyc` sitting next to its `.py`) are not part of the hash, so running a skill's helper script never marks it user-modified or hides it from `moor skills list-modified` / `diff`.

The protection is good, but it has one sharp edge. If you edit a bundled skill and then later want to abandon your changes and go back to the bundled version by just copy-pasting from `~/.moor/moor-agent/skills/`, the manifest still holds the *old* origin hash from whenever the last successful sync ran. Your fresh copy-paste contents (current bundled hash) won't match that stale origin hash, so sync keeps flagging it as user-modified.

`moor skills reset` is the escape hatch:
...
moor skills reset google-workspace --restore
...
:::note Profiles
Each profile has its own `.bundled_manifest` under its own `MOOR_HOME`, so `moor -p coder skills reset <name>` only affects that profile.
:::
