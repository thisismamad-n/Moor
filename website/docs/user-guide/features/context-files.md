...
sidebar_position: 8
title: "Context Files"
description: "Project context files — .moor.md, AGENTS.md, CLAUDE.md, global SOUL.md, and .cursorrules — automatically injected into every conversation"
---

# Context Files

Moor Agent automatically discovers and loads context files that shape how it behaves. Some are project-local and discovered from your working directory. `SOUL.md` is now global to the Moor instance and is loaded from `MOOR_HOME` only.

## Supported Context Files

| File | Purpose | Discovery |
|------|---------|-----------| 
| **.moor.md** / **MOOR.md** | Project instructions (highest priority) | Walks to git root |
...
| **SOUL.md** | Global personality and tone customization for this Moor instance | `MOOR_HOME/SOUL.md` only |
| **.cursorrules** | Cursor IDE coding conventions | CWD only |
| **.cursor/rules/*.mdc** | Cursor IDE rule modules | CWD only |

:::info Priority system
Only **one** project context type is loaded per session (first match wins): `.moor.md` → `AGENTS.override.md` → `AGENTS.md` → `CLAUDE.md` → `.cursorrules`. **SOUL.md** is always loaded independently as the agent identity (slot #1).
...
- `~/.moor/SOUL.md`
- or `$MOOR_HOME/SOUL.md` if you run Moor with a custom home directory
...
Moor is compatible with Cursor IDE's `.cursorrules` file and `.cursor/rules/*.mdc` rule modules. If these files exist in your project root and no higher-priority context file (`.moor.md`, `AGENTS.md`, or `CLAUDE.md`) is found, they're loaded as the project context.
...
1. **Scan working directory** — checks for `.moor.md` → `AGENTS.md` → `CLAUDE.md` → `.cursorrules` (first match wins)
...
If any threat pattern is detected in a project context file (`.moor.md`, `AGENTS.md`, `CLAUDE.md`,
