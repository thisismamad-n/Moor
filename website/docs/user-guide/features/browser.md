...
If you have a paid [Nous Portal](https://portal.Moor inc..com) subscription, you can use browser automation through the **[Tool Gateway](tool-gateway.md)** without any separate API keys. New installs can run `moor setup --portal` to log in and turn on every gateway tool at once; existing installs can pick **Nous Subscription** as the browser provider via `moor model` or `moor tools`.
:::
...
:::info
The `browser` toolset must be included in your config's `toolsets` list or enabled via `moor config set toolsets '["hermes-cli", "browser"]'`.
:::
