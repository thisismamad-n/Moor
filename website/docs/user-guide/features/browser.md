...
Moor Agent includes a full browser automation toolset with multiple backend options:
...
- **Lightpanda local engine** via [Lightpanda](https://lightpanda.io) — a headless browser built from scratch in Zig for machines; instant start up, 16x lower memory and 9x faster than Chrome. Works in Browser Use mode (Moor spawns it, no Chromium or Node needed) and with the built-in tools (automatic Chrome fallback for actions it doesn't support yet)
...
If you have a paid [Moor Portal](https://portal.nousresearch.com) subscription, you can use browser automation through the **[Tool Gateway](tool-gateway.md)** without any separate API keys. New installs can run `moor setup --portal` to log in and turn on every gateway tool at once; existing installs can pick **Moor Subscription** as the browser provider via `moor model` or `moor tools`.
:::
...
The `.env` keys above supply **credentials only**. The active cloud browser is chosen by the `browser.cloud_provider` selection written by `moor tools` → Browser Automation (`browserbase`, `browser-use`, `camofox`, or `moor` for the Moor Subscription). Once a selection exists, adding or removing a key does not switch providers — and a selected provider with a missing key errors with guidance to run `moor tools` instead of silently rerouting. Never-configured setups still autodetect from available credentials.
:::
...
When a cloud provider is configured, Moor auto-spawns a **local Chromium sidecar**
...
mode, so you need it installed (`moor setup tools → Browser Automation`
...
When enabled, Moor copies your default browser's **active** profile — the one
...
`~/.moor/browser-profile/<browser>/`, then launches your **real browser
...
When you turn the toggle back off, Moor deletes the snapshot store
(`~/.moor/browser-profile/`) on the next browser use, so the copied
...
an exclusive (deny-all) lock, so Moor cannot copy them while the browser is
...
browser for you** when it's holding the profile. Even with this on, Moor never
...
(e.g. a background/tray instance relaunched), Moor stays blocked and tells you
...
Or configure via `moor tools` → Browser Automation → Camofox.

Camofox is selected like any other browser backend: pick **Camofox** in `moor tools` → Browser Automation, which writes `browser.cloud_provider: camofox` to `config.yaml`. `CAMOFOX_URL` is only the server address — setting it no longer selects the backend by itself once a browser selection exists (never-configured setups still autodetect it).

#### Persistent browser sessions

By default, each Camofox session gets a random identity — cookies and logins don't survive across agent restarts. To enable persistent browser sessions, add the following to `~/.moor/config.yaml`:
...
Then fully restart Moor so the new config is picked up.

:::warning Nested path matters
Moor reads `browser.camofox.managed_persistence`, **not** a top-level `managed_persistence`. A common mistake is writing:

```yaml
# ❌ Wrong — Moor ignores this
managed_persistence: true
```

If the flag is placed at the wrong path, Moor silently falls back to a random ephemeral `userId` and your login state will be lost on every session.
:::
...
- Scopes the `userId` to the active Moor profile, so different Moor profiles get different browser profiles (profile isolation).

##### What Moor does not do
- It does not force persistence on the Camofox server. Moor only sends a stable `userId`; the server must honor it by mapping that `userId` to a persistent Firefox profile directory.
- If your Camofox server build treats every request as ephemeral (e.g. always calls `browser.newContext()` without loading a stored profile), Moor cannot make those sessions persist. Make sure you are running a Camofox build that implements userId-based profile persistence.

##### Verify it's working

1. Start Moor and your Camofox server.
...
If step 5 logs you out, the Camofox server isn't honoring the stable `userId`. Double-check your config path, confirm you fully restarted Moor after editing `config.yaml`, and verify your Camofox server version supports persistent per-user profiles.

##### Where state lives

Moor derives the stable `userId` from the profile-scoped directory `~/.moor/browser_auth/camofox/` (or the equivalent under `$MOOR_HOME` for non-default profiles). The actual browser profile data lives on the Camofox server side, keyed by that `userId`. To fully reset a persistent profile, clear it on the Camofox server and remove the corresponding Moor profile's state directory.

#### Externally managed Camofox sessions

When another app drives the visible Camofox browser (a desktop assistant, a custom integration, another agent), configure Moor to operate inside that same identity instead of spawning its own isolated profile.

Three knobs control the behavior:

| Setting | Env var | Effect |
|---------|---------|--------|
| `browser.camofox.user_id` | `CAMOFOX_USER_ID` | Camofox `userId` Moor uses when creating tabs. Setting this opts the session into "externally managed" mode. |
...
| `browser.camofox.adopt_existing_tab` | `CAMOFOX_ADOPT_EXISTING_TAB` | When true, Moor calls `GET /tabs?userId=<user_id>` on first use and reuses an existing tab before creating a new one. |
...
- Moor skips destructive cleanup at task end (same as `managed_persistence: true`). The other app's tab/cookies/profile survive.
- Moor does **not** call `DELETE /sessions/<user_id>` — that endpoint wipes all user data, so it would nuke the external app's session if it fired.

**How tab adoption works (when `adopt_existing_tab: true`):**

1. On the first browser tool call after a process start, Moor issues `GET /tabs?userId=<user_id>` (5-second timeout).
2. If any tab in the response has `listItemId == session_key`, Moor adopts the most recently created one in that group.
3. Otherwise, Moor adopts the most recently created tab for the user (any `listItemId`).
4. If no tabs exist or the request fails, Moor falls back to creating a new tab on the next operation.

Adoption only fires until `tab_id` is populated for the session. If the external app closes the adopted tab mid-run, the next browser tool call will surface a Camofox error — Moor does not re-poll for a fresh tab on every call.

**Picking `session_key`:** if you want Moor to reliably attach to a *specific* existing tab, set `session_key` to the `listItemId` the external app used when creating it. If you leave `session_key` unset and only set `user_id`, Moor generates a per-task `session_key` (`task_<id>`) — Moor will share cookies and the profile with the external app, but will open its own tab alongside instead of reusing one.

**Concurrency note:** the external app and Moor can drive the same Camofox `userId` simultaneously, but Camofox does not coordinate per-tab focus between clients. Coordinate ownership at the application layer (e.g. the external app pauses while Moor runs).

#### VNC live view

When Camofox runs in headed mode (with a visible browser window), it exposes a VNC port in its health check response. Moor automatically discovers this and includes the VNC URL in navigation responses, so the agent can share a link for you to watch the browser live.
...
Lightpanda is a **local engine** (a browser source, like "Local Browser"), not a cloud provider. Install the binary and put it on your `PATH` (see the [Lightpanda installation guide](https://lightpanda.io/docs/run-locally/installation/one-liner)), then pick **Lightpanda** in `moor tools` → Browser Automation, or set:
...
- **Browser Use mode (the default).** Moor launches `lightpanda serve --host 127.0.0.1 --port <free>` itself — one process per `browser_exec` session name (or per task) — and points the Browser Use CLI at it. No Chromium, Playwright or Node.js is needed. The process is reaped after `browser.inactivity_timeout`, on exit, and by the orphan sweep if Moor crashes. All of these processes share one on-disk HTTP cache at `$MOOR_HOME/cache/browser-use/lightpanda/http-cache`, so repeat visits skip re-downloading assets. Moor passes the cache flag only when the installed Lightpanda supports it (0.3.x+); older binaries simply run without a cache. To clear it, stop your Lightpanda sessions first, then delete that directory. Lightpanda has no graphical renderer, so `capture_screenshot()` is unavailable and the tool description tells the model to work text-first; it also holds one page per session, so the model is told to call `new_tab()` once and `goto_url()` afterwards (tracked upstream in [lightpanda-io/browser#1962](https://github.com/lightpanda-io/browser/issues/1962)).
- **Built-in browser tools** (`/browser use off`). Moor drives Lightpanda through `agent-browser --engine lightpanda` over CDP, the same way it drives local Chrome, with **automatic Chrome fallback**: Lightpanda handles the actions it supports (navigate, snapshot, click, type, scroll, back, press, eval) and Moor transparently retries on Chrome for anything it doesn't. Screenshots and `browser_vision` are routed straight to Chrome.

**When the engine is ignored.** `browser.engine` is the lowest-precedence browser setting: a cloud provider (including the Moor subscription browser — and on never-configured setups, any `BROWSERBASE_API_KEY` / `BROWSER_USE_API_KEY` in `~/.moor/.env` auto-selects one), Camofox, a `browser.cdp_url` / `/browser connect` override, or `browser.use_real_profile` all take precedence. Picking Lightpanda in `moor tools` writes `cloud_provider: local` for you; `/browser status` and `moor doctor` report when the engine is configured but shadowed, and by what.

### Local Chromium-family browser via CDP (`/browser connect`)

Instead of a cloud provider, you can attach Moor browser tools to your own running Chrome, Brave, Chromium, or Edge instance via the Chrome DevTools Protocol (CDP). This is useful when you want to see what the agent is doing in real-time, interact with pages that require your own cookies/sessions, or avoid cloud browser costs.

:::note
`/browser connect` is an **interactive-CLI slash command** — it is not dispatched by the gateway. If you try to run it inside a WebUI, Telegram, Discord, or other gateway chat, the message will be sent to the agent as plain text and the command will not execute. Start Moor from the terminal (`moor` or `moor chat`) and issue `/browser connect` there.
:::
...
Then launch the Moor CLI and run `/browser connect`.
...
If Moor runs inside WSL2 but the Chrome window you want to control runs on the Windows host, `/browser connect` is often not the best path.

Why:

- `/browser connect` expects Moor itself to reach a usable CDP endpoint
...
- even when Windows Chrome is debuggable, the cleanest integration is often to let a Windows-side browser MCP server attach to Chrome and let Moor talk to that MCP server

For that setup, prefer `chrome-devtools-mcp` through Moor MCP support.
...
If you do **not** set any cloud credentials and don't use `/browser connect`, Moor can still use the browser tools through a local Chromium install driven by `agent-browser`.
...
# Browser Use mode (default) "lightpanda" makes Moor spawn `lightpanda serve`;
...
# Moor doesn't add automatically; setting it disables the auto-injection.
...
:::info
The `browser` toolset must be included in your config's `toolsets` list or enabled via `moor config set toolsets '["moor-cli", "browser"]'`.
:::
...
You can also run `moor config set browser.snapshot_threshold 30000`. The setting applies to both explicit `browser_snapshot` calls and the automatic snapshot returned after navigation, including the Camofox backend (minimum 1000). Restart the current Moor session after changing it so the browser config cache reloads.
...
Screenshots are stored in `~/.moor/cache/screenshots/` and automatically cleaned up after 24 hours.
...
When enabled, recording starts automatically on the first `browser_navigate` and saves to `~/.moor/browser_recordings/` when the session closes. Works in both local and cloud (Browserbase) modes. Recordings older than 72 hours are automatically cleaned up.
...
:::note
If paid features aren't available on your plan, Moor automatically falls back — first disabling `keepAlive`, then proxies — so browsing still works on free plans.
:::
...
- **Snapshot size** — large pages are truncated at `browser.snapshot_threshold` (default 15,000 characters, matching `web_extract`; no LLM summarization); the complete snapshot is saved to `~/.moor/cache/web/` and the output points at it for `read_file` paging
