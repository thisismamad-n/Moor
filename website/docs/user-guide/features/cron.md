...
Schedule tasks to run automatically with natural language or cron expressions. Moor exposes cron management through a single `cronjob` tool with action-style operations instead of separate schedule/list/remove tools.
...
`moor setup --portal` is the lowest-friction option for unattended runs since OAuth refresh is automatic. See [Moor Portal](/integrations/moor-portal).
:::

:::tip
**Per-job reasoning effort.** A job can pin its own thinking level, independent of the model pin: one of `none`, `minimal`, `low`, `medium`, `high`, `xhigh`, `max`, `ultra`. When set, it overrides both the global `agent.reasoning_effort` and per-model `agent.reasoning_overrides` for that job's runs (`none` disables thinking). Set it via `moor cron create/edit --reasoning-effort high`; pass an empty string on edit to clear the pin and follow config again. (It is deliberately not exposed on the agent's `cronjob` tool — model configuration stays a user decision.) Levels a model doesn't support are clamped or omitted by the provider at request time — pinning `xhigh` on a model that caps at `high` runs at `high`. The pin has no effect on `no_agent` jobs (there is no LLM call to tune). Use it to run heavy scheduled analyses at `high` while cheap recurring jobs run at `minimal`, without touching your global default.
:::

:::warning
Cron-run sessions cannot recursively create more cron jobs. Moor disables cron management tools inside cron executions to prevent runaway scheduling loops.
:::
...
Moor will use the unified `cronjob` tool internally.
...
Or: `moor config set cron.preflight false`
...
```bash
moor config set cron.model_drift_guard false
```
...
1. loads jobs from `~/.moor/cron/jobs.json`
...
A file lock at `~/.moor/cron/.tick.lock` prevents overlapping scheduler ticks from double-running the same job batch.

### Execution history

Moor records each claimed cron attempt in the profile-local
...
`completed`, `failed`, or `unknown`. After restart, Moor marks an abandoned
...
Inspect recent attempts with `moor cron runs [job-id] --limit 20` (alias:
...
in a row and suggesting you fix, pause (`moor cron pause <job>`), or remove
it. Any successful run resets the streak, and `moor cron list` shows the
...
moor cron incidents --state alerted # filter: detected | alerted | closed
...
`moor cron doctor` is a read-only health check over every active job. It
...
- script missing, not a file, or resolving outside `MOOR_HOME/scripts`,
...
`moor cron incidents` (durable failure records) and `moor cron runs`
...
| `"local"` | Save to local files only (`~/.moor/cron/output/`) | Default on CLI |
...
`last_delivery_error`. `moor cron list` shows it in yellow as
`delivery_failed: <reason>`, `moor cron doctor` reports it as a delivery
...
and in `moor cron doctor` as `last delivery unverified (...)`. The marker is
...
If the agent's final response contains `[SILENT]`, delivery is suppressed entirely. The output is still saved locally for audit (in `~/.moor/cron/output/`), but no message is sent to the delivery target.
...
Or set the `MOOR_CRON_SCRIPT_TIMEOUT` environment variable. The resolution order is: env var → config.yaml → 3600s default.
...
Or set the `MOOR_CRON_MEDIA_SEND_TIMEOUT` environment variable. The resolution order is: env var → config.yaml → 300s default. A timed-out attachment is recorded in the job's run status as a partial delivery failure (the text still delivers).
...
`.sh` / `.bash` files run under `bash` from `PATH` when available, otherwise `/bin/bash` (important on Windows Git Bash). Anything else runs under the current Python interpreter (`sys.executable`). Scripts must resolve inside `$MOOR_HOME/scripts/` — relative names, absolute paths, and `~`-prefixed paths are accepted when the resolved target stays in that directory; paths that escape it are rejected. Subprocess env is sanitized (`_sanitize_subprocess_env`): provider API credentials and other moor-managed secrets are **not** inherited by cron scripts.

### The agent sets these up for you

The `cronjob` tool's schema exposes `no_agent` to Moor directly, so you can describe a watchdog in chat and let the agent wire it up:

```text
Ping me on Telegram if RAM is over 85%, every 5 minutes.
```

Moor will write the check script to `~/.moor/scripts/` via `write_file`, then call:
...
cronjob(
    action="create",
    prompt="Fetch the top 10 AI/ML stories from Hacker News. Save them to ~/.moor/data/briefs/raw.md in markdown format with title, URL, and score.",
    schedule="0 7 * * *",
...
cronjob(
    action="create",
    prompt="Read ~/.moor/data/briefs/raw.md. Score each story 1–10 for engagement potential and novelty. Output the top 5 to ~/.moor/data/briefs/ranked.md.",
    schedule="30 7 * * *",
...
cronjob(
    action="create",
    prompt="Read ~/.moor/data/briefs/ranked.md. Write 3 tweet drafts (hook + body + hashtags). Deliver to telegram:7976161601.",
    schedule="0 8 * * *",
...
- When Job 2 fires, Moor reads Job 1's most recent output from `~/.moor/cron/output/{job1_id}/*.md`
...
From the CLI: `moor cron create "every 6h" "Scan for news" --continuity`, and `moor cron edit <job_id> --continuity` / `--no-continuity` to toggle it on an existing job. The same toggle appears in the dashboard's cron editor and the desktop Bot Mode routine dialog.
...
- `moor cron list` — a red `⚠ Missed scheduled fire:` line under the job
...
Runtimes that can't receive detached results (one-shot `moor -z`, `moor
...
When `enabled_toolsets` is set on a job it wins; otherwise the `moor tools` cron-platform config wins; otherwise Moor falls back to the built-in defaults. This matters for cost control: carrying `browser`, `delegation` into every tiny "fetch news" job bloats the tool-schema prompt on every LLM call.

### Skipping the agent entirely: `wakeAgent`

If your cron job attaches a pre-check script (via `script=`), the script can decide at runtime whether Moor should even invoke the agent. Emit a final stdout line of the form:
...
Jobs are stored in `~/.moor/cron/jobs.json`. Output from job runs is saved to `~/.moor/cron/output/{job_id}/{timestamp}.md`.
