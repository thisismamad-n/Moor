...
| CLI | Saved to `~/.moor/audio_cache/` | MP3 |
...
tts:
  ...
  piper:
    ...
    # voices_dir: ''                            # default: ~/.moor/cache/piper-voices/
...
tts:
  ...
  gemini:
    voice: Algieba
    persona_prompt_file: ~/.moor/tts/butler-voice.md
```
...
On the first TTS call for a voice that isn't cached locally, Moor runs `python -m piper.download_voices <name>` and downloads the model (~20-90MB depending on quality tier) into `~/.moor/cache/piper-voices/`. Subsequent calls reuse the cached model.
...
Local engines (Piper, KittenTTS) load their model lazily, so without help the *first* spoken reply after you turn speech on pays the whole model load — and on a fresh install the voice download — as silence before the first word. Moor treats the speech-output toggles as the signal that TTS is about to be needed:
...
| A voice-listing API used by `moor setup` | **Plugin** (override `list_voices()`) |
...
Drop this in `~/.moor/plugins/my-tts/`:
...
Enable it (`moor plugins enable my-tts`), point `tts.provider` at it (`tts.provider: my-tts` in `config.yaml`), and the `text_to_speech` tool will route through your plugin.

#### Optional hooks

Override these on your provider class for richer integration:

- `list_voices()` → list of `{id, display, language, gender, preview_url}` dicts shown in `moor tools`.
- `list_models()` → list of `{id, display, languages, max_text_length}` dicts.
- `get_setup_schema()` → return `{name, badge, tag, env_vars: [{key, prompt, url}]}` to power the picker row in `moor tools` / `moor setup`. Without this, the plugin still works but its row in the picker is minimal.
...
:::info Zero Config
Local transcription works out of the box when `faster-whisper` is installed. If that's unavailable, Moor can also use a local `whisper` CLI from common install locations (like `/opt/homebrew/bin`) or a custom command via `MOOR_LOCAL_STT_COMMAND`.
:::
...
stt:
  ...
  xai:
    ...
    language: ""              # optional ISO-639-1 hint; blank = use MOOR_LOCAL_STT_LANGUAGE if set, else "en"
```
...
The first use downloads the selected model from `huggingface.co`; later loads prefer the local cache and do not require an online revision check. On networks where the Hub is unavailable, export an accessible mirror in the shell or service that starts Moor:
...
**Groq API** — Requires `GROQ_API_KEY`. Good cloud fallback when you want a free hosted STT option. Set `stt.groq.language` (or the global `MOOR_LOCAL_STT_LANGUAGE` env var) to skip Whisper's auto-detect and reduce latency on known-language audio.
...
**Custom local CLI fallback** — Set `MOOR_LOCAL_STT_COMMAND` if you want Moor to call a local transcription command directly. The command template supports `{input_path}`, `{output_dir}`, `{language}`, and `{model}` placeholders. Moor tokenizes the rendered template into an argument list and executes it without a shell, so operators such as `|`, `>`, `&&`, and `;` are passed as literal arguments. Your command must write a `.txt` transcript somewhere under `{output_dir}`.
...
export VOLCENGINE_APP_ID="your-app-id"
export VOLCENGINE_ACCESS_TOKEN="your-access-token"
export MOOR_LOCAL_STT_COMMAND='doubao-speech transcribe {input_path} --out {output_dir}/transcript.txt'
```
...
```bash
export MOOR_LOCAL_STT_COMMAND='sh -c '\''whisper "$1" --output_format txt --output_dir "$2" | tee "$2/whisper.log"'\'' _ {input_path} {output_dir}'
```
...
This complements the legacy `MOOR_LOCAL_STT_COMMAND` escape hatch via the built-in `local_command` path. Unlike the shell-driven command-provider registry, the legacy template is tokenized into argv and runs without implicit shell interpretation. Use `stt.providers.<name>` when you want **multiple** shell-driven STT engines, a name you can pick via `stt.provider`, or anything that needs per-provider `language` / `model` / `timeout`.
...
The shell command runs under the same user as Moor with full filesystem access — same trust model as `tts.providers.<name>: type: command` and `MOOR_LOCAL_STT_COMMAND`. Only declare command providers from sources you trust.
...
| Only the legacy single-command escape hatch is wanted        | `MOOR_LOCAL_STT_COMMAND` env var (argv-tokenized; no implicit shell) |
...
Drop this in `~/.moor/plugins/my-stt/`:
...
Enable it (`moor plugins enable my-stt`), set `stt.provider: my-stt` in `config.yaml`, and voice-message transcription will route through your plugin.
...
- `get_setup_schema()` → return `{name, badge, tag, env_vars: [{key, prompt, url}]}` to power picker rows in `moor tools` / `moor setup` (the picker category for STT is not yet shipped — this metadata is available to plugins for forward compatibility).
