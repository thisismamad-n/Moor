...
description: "Customize Moor Agent's personality with a global SOUL.md, built-in personalities, and custom persona definitions"
---

# Personality & SOUL.md

Moor Agent's personality is fully customizable. `SOUL.md` is the **primary identity** — it's the first thing in the system prompt and defines who the agent is.

- `SOUL.md` — a durable persona file that lives in `MOOR_HOME` and serves as the agent's identity (slot #1 in the system prompt)
- built-in or custom `/personality` presets — session-level system-prompt overlays

If you want to change who Moor is — or replace it with an entirely different agent persona — edit `SOUL.md`.

## How SOUL.md works now

Moor now seeds a default `SOUL.md` automatically in:

```text
~/.moor/SOUL.md
```

More precisely, it uses the current instance's `MOOR_HOME`, so if you run Moor with a custom home directory, it will use:
...
- Moor creates a starter `SOUL.md` automatically if one does not exist yet
- Existing user `SOUL.md` files are never overwritten
- Moor loads `SOUL.md` only from `MOOR_HOME`
- Moor does not look in the current working directory for `SOUL.md`
- If `SOUL.md` exists but is empty, or cannot be loaded, Moor falls back to a built-in default identity
...
If Moor loaded `SOUL.md` from whatever directory you happened to launch it in, your personality could change unexpectedly between projects. By loading only from `MOOR_HOME`, the personality belongs to the Moor instance itself.

That also makes it easier to teach users:
- "Edit `~/.moor/SOUL.md` to change Moor' default personality."
...
- how Moor should handle uncertainty, disagreement, or ambiguity
...
## What Moor injects into the prompt
...
If the file is empty, whitespace-only, or cannot be read, Moor falls back to a built-in default identity ("You are Moor Agent, built by Moor inc.. Be direct: match the length of your reply to the weight of the ask..."). This fallback also applies when `skip_context_files` is set (e.g., in subagent/delegation contexts).
...
Moor ships with built-in personalities you can switch to with `/personality`.
...
| **pirate** | Captain Moor, tech-savvy buccaneer |
...
These are convenient overlays, but your global `SOUL.md` still gives Moor its persistent default personality unless the overlay meaningfully changes it.

## Custom personalities in config

Built-in personalities are always available on every surface (CLI, messaging platforms, TUI, and the desktop app). You can add your own — or override a built-in by reusing its name — in `~/.moor/config.yaml` under `agent.personalities` (a top-level `personalities:` block works too; `agent.personalities` wins if the same name appears in both).
...
1. Keep a thoughtful global `SOUL.md` in `~/.moor/SOUL.md`
...
- [SOUL.md Guide](../../guides/use-soul-with-moor.md)

## CLI appearance vs conversational personality

Conversational personality and CLI appearance are separate:

- `SOUL.md`, `agent.system_prompt`, and `/personality` affect how Moor speaks
- `display.skin` and `/skin` affect how Moor looks in the terminal
