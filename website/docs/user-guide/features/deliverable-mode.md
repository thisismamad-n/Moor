...
2. **The gateway scans agent responses for file paths.** Any absolute path
   (`~/.moor/cache/scratch/...`) or home-relative path (`~/...`) ending in a supported
...
persona in `~/.moor/SOUL.md`, or as a named preset under
`agent.personalities` in `~/.moor/config.yaml` (switchable per session
via `/personality`).

The mechanic the agent has to use is simple: render the file to an
absolute path (e.g. `~/.moor/cache/scratch/q3-revenue.png`) and mention that path as
...
kanban_complete(
    ...
    artifacts=[
        "~/.moor/cache/scratch/q3-revenue.png",
        "~/.moor/cache/scratch/q3-report.pdf",
    ],
)
```
...
Install MCP servers via `~/.moor/config.yaml` under the `mcp_servers`
