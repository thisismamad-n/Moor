...
User plugins at `~/.moor/plugins/model-providers/<name>/` and `~/.moor/plugins/memory/<name>/` override bundled plugins of the same name — last-writer-wins in `register_provider()` / `register_memory_provider()`. Drop a directory in, and it replaces the built-in without any repo edits.
...
Plugins can register the 26 lifecycle events currently accepted by `moor_cli.plugins.VALID_HOOKS`. The **[Event Hooks catalog](/user-guide/features/hooks#shipped-plugin-hook-catalog)** is canonical for exact timing, return handling, payload fields, and privacy notes.
...
moor plugins search <term>                 # search the community plugin index
moor plugins install <name>                # install by index name (resolved to repo @ pinned ref)
...
Install from Git**. Legacy `moor://plugin-agent/…` and
...
`moor plugins search <term>` searches the **community plugin index** — a
static, machine-readable JSON catalog of community plugins. Matching is fuzzy
across name, description, and tags:

```bash
moor plugins search telegram               # fuzzy search
moor plugins search                        # browse the whole index
moor plugins search --capability platform  # filter by declared capability
moor plugins search media --json           # machine-readable output
moor plugins search --refresh              # bypass the 24h local cache
```
...
```bash
moor plugins install moor-media-studio
```
...
overridable via `moor config set plugins.index_url <url>`). Fetches are
cached under `~/.moor/cache/plugin_index.json` for 24 hours; when the
...
(`moor_cli/data/plugin_index.json` in the repo) is the format reference.

**Submitting a plugin.** The index is maintained as a plain JSON file —
submit a pull request to the
[moor-plugin-index](https://github.com/NousResearch/hermes-plugin-index)
...
version: 1.0.0
plugins:
  - name: moor-media-studio            # bare community-index name…
