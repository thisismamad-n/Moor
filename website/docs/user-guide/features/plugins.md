...
[Adding Tools](/developer-guide/adding-tools) page is for built-in Moor
core tools that live in `tools/` and `toolsets.py`.

**→ [Build a Moor Plugin](/developer-guide/plugins)** — step-by-step guide with a complete working example.
...
|--------|------|----------|
| Bundled | `<repo>/plugins/` | Ships with Moor — see [Built-in Plugins](/user-guide/features/built-in-plugins) |
...
| Nix | `services.moor-agent.extraPlugins` / `extraPythonPackages` | NixOS declarative installs — see [Nix Setup](/getting-started/nix-setup#plugins) |
...
The same agent-plugin pin is available in Moor Desktop: **Capabilities →
...
install. `moor plugins list` prints
...
Plugins can register the 27 lifecycle events currently accepted by `moor_cli.plugins.VALID_HOOKS`. The **[Event Hooks catalog](/user-guide/features/hooks#shipped-plugin-hook-catalog)** is canonical for exact timing, return handling, payload fields, and privacy notes.
...
|---|---|---|
| A **tool** the LLM can call | Python plugin — `ctx.register_tool()` | [Build a Moor Plugin](/developer-guide/plugins) · [Adding Tools](/developer-guide/adding-tools) |
| A **lifecycle hook** (pre/post LLM, session start/end, tool filter) | Python plugin — `ctx.register_hook()` | [Hooks reference](/user-guide/features/hooks) · [Build a Moor Plugin](/developer-guide/plugins) |
| A **slash command** for the CLI / gateway | Python plugin — `ctx.register_command()` | [Build a Moor Plugin](/developer-guide/plugins) · [Extending the CLI](/developer-guide/extending-the-cli) |
| A **subcommand** for `moor <thing>` | Python plugin — `ctx.register_cli_command()` | [Extending the CLI](/developer-guide/extending-the-cli) |
...
| **External tools via MCP** (filesystem, GitHub, Linear, Notion, any MCP server) | Config-driven — declare `mcp_servers.<name>` with `command:` / `url:` in `config.yaml`. Moor auto-discovers the server's tools and registers them alongside built-ins. | [MCP](/user-guide/features/mcp) |
| **Additional skill sources** (custom GitHub repos, private skill indexes) | CLI — `moor skills tap add <repo>` | [Skills Hub](/user-guide/features/skills#skills-hub) · [Publishing a custom tap](/user-guide/features/skills#publishing-a-custom-skill-tap) |
| **Gateway event hooks** (fire on `gateway:startup`, `session:start`, `agent:end`, `command:*`) | Drop `HOOK.yaml` + `handler.py` into `~/.moor/hooks/<name>/` | [Event Hooks](/user-guide/features/hooks#gateway-event-hooks) |
...
On NixOS, plugins can be installed declaratively via the module options — no `moor plugins install` needed. See the **[Nix Setup guide](/getting-started/nix-setup#plugins)** for full details.
...
moor plugins list                          # table: enabled / disabled / not enabled
...
moor plugins install user/repo --enable    # install AND enable (no prompt)
...
moor plugins remove my-plugin              # uninstall
moor plugins enable my-plugin              # add to allow-list
moor plugins disable my-plugin             # remove from allow-list + add to disabled
...
```
moor://plugin/install?repo=owner/repo            # main install link
...
Clicking one opens Moor and shows a **confirmation dialog** — the repo id,
...
The public [Plugin Catalog](/plugins) includes **Install in Moor** on every
card. Catalog links carry `catalog_name`, a URL-encoded `repo` (including
`#subdir` when present), and `sha`:

```text
moor://plugin/install?repo=owner%2Frepo&catalog_name=example-plugin&sha=0123456789abcdef0123456789abcdef01234567
```
...
`moor plugins install <catalog-name>` command in the expanded card to retain
catalog resolution.
...
[Add to Moor link](/reference/mcp-config-reference#add-to-moor-link).
