...
description: "Connect Moor Agent to external tool servers via MCP — and control exactly which MCP tools Moor loads"
---
...
The `mcpServers` block in your `~/.claude.json` maps to `mcp_servers` in Moor' `config.yaml` — and `moor import-agent claude-code` migrates it (along with skills and instructions) automatically. See [Import from Other Agents](../import-from-other-agents.md).
:::

## What MCP gives you

- Access to external tool ecosystems without writing a native Moor tool first
- Local stdio servers and remote HTTP MCP servers in the same config
- Automatic tool discovery and registration at startup
- Utility wrappers for MCP resources and prompts when supported by the server
- Per-server filtering so you can expose only the MCP tools you actually want Moor to see

## Quick start

1. MCP support ships with the standard install — no extra step needed.

2. Add an MCP server to `~/.moor/config.yaml`:
...
4. Ask Moor to use the MCP-backed capability.
...
Moor will discover the MCP server's tools and use them like any other tool.

## Catalog: one-click install for Moor-approved MCPs

Moor ships a curated catalog of MCP servers that Moor staff has reviewed
...
moor mcp install n8n    # install a catalog entry by name
```

The picker shows each entry with its current status:

```
n8n          available              Manage and inspect n8n workflows from Moor
...
`optional-mcps/` in the moor-agent repo — presence in that directory means
Moor approval. There is no community submission tier; entries are added by
merging a PR.

Catalog entries can require:

- **API key** — Moor prompts at install time and writes the value to
  `~/.moor/.env`. Non-secret values (base URLs) go to the same file.
- **OAuth** (remote MCP) — written as `auth: oauth` in your config; the MCP
  client opens a browser on first connection.
- **OAuth** (third-party provider like Google/GitHub) — Moor points you at
  `moor auth <provider>` if you haven't authenticated already.

### Tool selection at install time

After credentials are configured, Moor probes the MCP server to list every
...
written (if not). Re-run `moor mcp configure <name>` once the server is
reachable to refine.
...
the moor-agent repo, so Moor has reviewed each entry before it shipped —
...
PR adds an entry with a newer `manifest_version` than your installed Moor
understands, the picker will surface a warning (`⚠ '<name>' requires a newer
Moor`) for that entry instead of silently hiding it. Run `moor update`
to install the latest Moor when you see that.
...
from environment variables (which include everything in `~/.moor/.env`).
...
MCPs are never auto-updated. Re-run `moor mcp install <name>` to refresh
...
HTTP MCP servers are remote endpoints Moor connects to directly.
...
- you do not want Moor spawning a local subprocess for that integration
...
Most hosted MCP servers (Cloudflare, Linear, Sentry, Atlassian, Asana, Figma, Stripe, …) require OAuth 2.1 instead of a static bearer token. Set `auth: oauth` and Moor handles discovery, client identification, PKCE, token exchange, refresh, and step-up auth via the MCP Python SDK.

Moor identifies itself with a [Client ID Metadata Document](../../reference/mcp-config-reference.md#client-identification-cimd-and-dcr) on servers that support one, and falls back to Dynamic Client Registration on those that don't. Both are automatic; there is nothing to configure.
...
Or: `moor mcp install figma`, then `moor mcp login figma`.
:::
...
- **Device-code login (no callback at all):** if the server's authorization server advertises a device authorization endpoint, run `moor mcp login <server> --flow device` on the machine running Moor. It prints a verification URL and a short code; open the URL on any device, enter the code, and Moor polls for approval. No browser is launched on the host and no callback listener is needed. Set `oauth.flow: device` on the server to make `login` and `reauth` use it by default. Details: [Device-code login](../../reference/mcp-config-reference.md#device-code-login-rfc-8628).
- **SSH port forward:** `ssh -N -L <port>:127.0.0.1:<port> user@host` in a separate terminal, then let the redirect flow normally.
- **Proxied callback (`redirect_uri`):** when a public HTTPS endpoint forwards to the host (e.g. a Tailscale Funnel or reverse proxy pointed at the callback port), set `oauth.redirect_uri` and the browser redirect reaches Moor on its own — no tunnel or paste needed:
...
For fully headless gateways (messaging bot, no interactive terminal at all), the optional [`mcp-oauth-remote-gateway` skill](../skills/optional/mcp/mcp-mcp-oauth-remote-gateway.md) walks the agent through completing the flow manually and writing tokens where Moor expects them.
...
See [OAuth over SSH / Remote Hosts](../../guides/oauth-over-ssh.md#mcp-servers) for the full walkthrough, including DCR-less servers (e.g. Slack), pre-registered `client_id`/`client_secret`, scope customization, and re-auth via `moor mcp login <server>`.

**Pitfall — providers that don't support automatic registration (Google Drive, Atlassian).** Some servers reject the dynamic client registration step (RFC 7591) that bare `auth: oauth` relies on — Google's official Drive server (`https://drivemcp.googleapis.com/mcp/v1`) returns a `400 Bad Request`, so no OAuth client is created and no token is acquired. The symptom is subtle: these servers also serve `tools/list` *without* auth, so `moor mcp login` can list the tools and look like it worked, but every real tool call later times out. `moor mcp login` now detects this (it checks that a token actually landed on disk) and tells you to supply your own OAuth client. Create one in the provider's console and add it to config:
...
Then run `moor mcp login googledrive` — with the pre-registered client, Moor skips registration and runs the normal browser authorization flow.
...
Remote HTTP MCP servers that require mutual TLS (client-certificate authentication) are supported via `client_cert` / `client_key`. Moor passes the resolved certificate to the underlying HTTP client for the TLS handshake.
...
Moor reads MCP config from `~/.moor/config.yaml` under `mcp_servers`.
...
| `enabled` | bool | If `false`, Moor skips the server entirely |
...
For well-known MCP servers, `moor mcp add` accepts a `--preset` flag that fills in the transport details so you don't have to look up the command and args. The preset only supplies defaults — anything else (env vars, headers, filtering) you pass on the same command line still wins.
...
moor mcp add codex --preset codex
```
...
You can pick any local name (`moor mcp add my-codex --preset codex` is fine); the preset only provides the `command`/`args` defaults.

## How Moor registers MCP tools

Moor prefixes MCP tools so they do not collide with built-in names:
...
In practice, you usually do not need to call the prefixed name manually — Moor sees the tool and chooses it during normal reasoning.
...
When supported, Moor also registers utility tools around MCP resources and prompts:
...
- Moor only registers resource utilities if the MCP session actually supports resource operations
- Moor only registers prompt utilities if the MCP session actually supports prompt operations

So a server that exposes callable tools but no resources/prompts will not get those extra wrappers.

## Per-server filtering

You can control which tools each MCP server contributes to Moor, allowing fine-grained management of your tool namespace.
...
If `enabled: false`, Moor skips the server completely and does not even attempt a connection.
...
You can also separately disable moor-added utility wrappers:
...
If your config filters out all callable tools and disables or omits all supported utilities, Moor does not create an empty runtime MCP toolset for that server.
...
Moor discovers MCP servers at startup and registers their tools into the normal tool registry.
...
MCP servers can notify Moor when their available tools change at runtime by sending a `notifications/tools/list_changed` notification. When Moor receives this notification, it automatically re-fetches the server's tool list and updates the registry — no manual `/reload-mcp` required.
...
A running messaging gateway (`moor gateway run`) also watches `config.yaml` on its own: within about a minute of you removing an `mcp_servers` entry or setting `enabled: false`, that server's connection is torn down; a newly added entry is connected. No restart or `/reload-mcp` needed for the edit to take effect.
...
For stdio servers, Moor does not blindly pass your full shell environment.
...
Then verify your config and restart Moor.
...
Because Moor now only registers those wrappers when both are true:
...
When `supports_parallel_tool_calls` is `true`, Moor may execute multiple tools from that server at the same time within a single tool-call batch, just like it does for built-in read-only tools (web_search, read_file, etc.).
...
MCP servers can request LLM inference from Moor via the `sampling/createMessage` protocol. This allows an MCP server to ask Moor to generate text on its behalf — useful for servers that need LLM capabilities but don't have their own model access.
...
- You want Claude Code, Cursor, or another coding agent to send and read Telegram/Discord/Slack messages through Moor
- You want a single MCP server that bridges to all of Moor's connected messaging platforms at once
- You already have a running Moor gateway with connected platforms
...
Add Moor to your MCP client config. For example, in Claude Code's `~/.claude/claude_desktop_config.json`:
...
Or if you installed Moor in a specific location:

```json
{
  "mcpServers": {
    "moor": {
      "command": "/home/user/.moor/moor-agent/venv/bin/moor",
      "args": ["mcp", "serve"]
    }
  }
}
...
The MCP server exposes 10 tools, matching OpenClaw's channel bridge surface plus a moor-specific channel browser:
...
The MCP server includes a live event bridge that polls Moor's session database for new messages. This gives MCP clients near-real-time awareness of incoming conversations:
...
```bash
moor mcp serve              # Normal mode
moor mcp serve --verbose    # Debug logging on stderr
```

### How it works

The MCP server reads conversation data directly from Moor's session store — `~/.moor/state.db` is the primary source, with `sessions.json` kept only as a legacy fallback. A background thread polls the database for new messages and maintains an in-memory event queue. For sending messages, it uses the same internal send engine (`tools/send_message_tool.py`) that powers cron delivery and the `moor send` CLI.
...
- The embedded `moor mcp serve` exposes a **stdio-only** MCP server today. If you need an HTTP MCP server, run a separate adapter — or, much more commonly, use the MCP **client** side of Moor, which already speaks both stdio and HTTP (`url` + `headers` in `mcp_servers.yaml` / `config.yaml`; see [HTTP servers](#http-servers) above).
