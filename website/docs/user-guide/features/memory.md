...
description: "How Moor Agent remembers across sessions — MEMORY.md, USER.md, and session search"
---

# Persistent Memory

Moor Agent has bounded, curated memory that persists across sessions. This lets it remember your preferences, your projects, your environment, and things it has learned.
...
- All CLI and messaging sessions are stored in SQLite (`~/.moor/state.db`) with FTS5 full-text search
...
```bash
moor sessions list    # Browse past sessions
```
...
- **Classic CLI / standalone** — `moor journey` (aliases: `moor learning`, `moor memory-graph`) renders the timeline in the terminal. Flags: `--play` animates the build-up (`--fps` to tune it), `--width`/`--height` override the render size, `--no-color` disables color, and `--json` dumps the raw graph payload.
...
|---------|--------------|
| `moor journey list` | List node ids — skill names and `memory:<source>:<index>` ids for memory chunks. |
...
or update a skill. This is Moor' consent-aware learning loop: repeated
...
For deeper, persistent memory that goes beyond MEMORY.md and USER.md, Moor ships with 8 external memory provider plugins — including Honcho, OpenViking, Mem0, Hindsight, Holographic, RetainDB, ByteRover, and Supermemory.

External providers run **alongside** built-in memory (never replacing it) and add capabilities like knowledge graphs, semantic search, automatic fact extraction, and cross-session user modeling.

```bash
moor memory setup      # pick a provider and configure it
