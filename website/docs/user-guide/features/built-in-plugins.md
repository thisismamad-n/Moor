...
description: "Plugins shipped with Moor Agent that run automatically via lifecycle hooks — disk-cleanup and friends"
---
...
This is the same mechanism user-installed plugins use. Bundled plugins are never auto-enabled — not on fresh install, not for existing users upgrading to a newer Moor. You always opt in explicitly.
...
NeMo Relay is no longer a bundled Moor plugin. Do not run `moor plugins enable observability/nemo_relay`; Moor core now owns the Relay session, turn, LLM, and tool lifecycles.
...
The old `MOOR_NEMO_RELAY_ATOF_*` and `MOOR_NEMO_RELAY_ATIF_*` settings no longer activate exporters. `moor doctor` reports these stale settings when no replacement `plugins.toml` is selected.
...
Both defaults preserve one session scope for the full session. Rotated spans retain the same `session_id` and add `moor.session.segment` plus `moor.session.segment_reason` (`compaction` or `max_turns`).
...
| File | Contents |
|---|---|
| `state.json` | Unlock history: which badges you've earned and when. Stable across Moor updates. |
