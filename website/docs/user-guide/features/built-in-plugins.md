...
See the [Plugins](/user-guide/features/plugins) page for the general plugin system, and [Build a Moor Plugin](/guides/build-a-hermes-plugin) to write your own.
...
Bundled plugins are written exactly like any other Moor plugin — see [Build a Moor Plugin](/guides/build-a-hermes-plugin). The only differences are:
