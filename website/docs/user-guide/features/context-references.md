...
Type `@` followed by a reference to inject content directly into your message. Moor expands the reference inline and appends the content under an `--- Attached Context ---` section.
...
- Moor env: `$MOOR_HOME/.env`

These directories are fully blocked (any file inside):
- `~/.ssh/`, `~/.aws/`, `~/.gnupg/`, `~/.kube/`, `$MOOR_HOME/skills/.hub/`
