...
Moor Agent can drive your desktop — clicking, typing, scrolling,
...
or add `computer_use` to your enabled toolsets in `~/.moor/config.yaml`.
...
| Moor session | cua-driver mode | Human intervention |
|---|---|---|
| Manual or smart approvals (default) | `standard` | Normal Moor approvals; Cua stops at its protected boundary |
...
| `--yolo`, `/yolo`, or `approvals.mode: off` | private `unrestricted` daemon | One explicit Moor risk acceptance; no runtime Cua prompts |
...
computer_use:
  ...
  capability_manifest: ~/.moor/cua-manifest.yaml
```
...
identity instead of resetting with every Moor build), and Moor verifies
...
## `moor computer-use doctor` — your first triage stop

`moor computer-use doctor` runs cua-driver's structured
`health_report` MCP tool and prints a per-check matrix. It's the single
fastest way to find out *why* an action isn't working.

```
$ moor computer-use doctor
...
without it. Moor disables it automatically where it is a known failure
...
These are **platform deep dives, not duplicates of the Moor skill** —
...
Code, Codex, OpenCode, OpenClaw, and Antigravity; **Moor
...
image capture also saves a bounded, shareable copy under Moor' image cache
...
Moor applies multi-layer guardrails:
...
Pair with `approvals.mode: manual` in `~/.moor/config.yaml` if you
...
- **Windows: elevated (admin) windows can't be driven from a normal
  ...
  Moor agent) cannot enumerate the UIA tree of, or inject mouse input
...
- **Platform-specific deployment gotchas:**
  ...
  - **Windows** SSH sessions run in **Session 0**, which has no
    interactive desktop. Drive Moor from inside the RDP / console
...
On Linux, native Wayland support remains an explicit opt-in. Moor passes the
...
```
MOOR_CUA_DRIVER_CMD=/path/to/your/cua-driver
```
...
upstream. **Moor disables it for you** — on every cua-driver invocation
(the MCP backend, `status`, `doctor`, and install) Moor sets
...
When it's on, `moor computer-use doctor` reports `telemetry: enabled`;
...
unreleased fix — point Moor at a binary you built from source instead
of the published release. Moor resolves the driver with
`shutil.which("cua-driver")` and **does not enforce
`MOOR_CUA_DRIVER_VERSION`**, so a local build (reported as
...
- `-NoAutoStart` skips registering the `cua-driver-serve` logon daemon
  — you don't need it for Moor testing (see notes).
...
### Option B — point Moor straight at the built binary (fastest loop)

Skip the install ceremony entirely: `cargo build` and set
`MOOR_CUA_DRIVER_CMD` to the resulting binary. Best for rapid
edit/build/test.
...
```
# Windows (.env)
MOOR_CUA_DRIVER_CMD=C:\path\to\cua\libs\cua-driver\rust\target\debug\cua-driver.exe
# macOS / Linux (.env)
MOOR_CUA_DRIVER_CMD=/path/to/cua/libs/cua-driver/rust/target/debug/cua-driver
```

### Confirm Moor is using your build

- `moor computer-use status` prints the resolved binary path and
  version.
- `moor computer-use doctor` confirms the binary is reachable and
...
- **Rebuild loop.** After editing cua-driver source, re-run
  ...
  for Option A, or just re-`cargo build` for Option B — no Moor
  change needed either way.
- **Local builds skip the version check.** Moor warns when the
...
**First action when anything's off: run `moor computer-use doctor`.**
...
Run `moor computer-use install` to fetch the cua-driver binary, or
run `moor tools` and enable the Computer Use toolset.
...
Wayland without an XWayland bridge. `moor computer-use doctor` will
...
- **cua.ai/docs** — the cua-driver project's documentation:
  ...
  - [Connect your agent](https://cua.ai/docs/how-to-guides/driver/connect-your-agent) — register cua-driver with various harnesses (Moor among them)
