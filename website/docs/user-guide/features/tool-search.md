...
**Tool Search** is Moor' opt-in progressive-disclosure layer for that
...
:::info Built-in Moor tools never defer
The tools that make up Moor' core capability set (`terminal`,
`read_file`, `write_file`, `patch`, `search_files`, `todo`, `memory`,
`browser_*`, `web_search`, `web_extract`, `clarify`, `execute_code`,
`delegate_task`, `session_search`, and the rest of
`_MOOR_CORE_TOOLS`) are *always* loaded directly. Only MCP tools and
...
When the model invokes `tool_call`, Moor **unwraps the bridge** and
...
- **No provider-native validation for deferred schemas.** `tool_describe`
  ...
  only the generic `tool_call.arguments` object. Moor therefore coerces and
  validates the underlying arguments locally before dispatch; the concrete
  tool or MCP server remains responsible for schemas Moor cannot safely
...
- **No JS sandbox.** Moor uses the simpler "structured tools" mode
