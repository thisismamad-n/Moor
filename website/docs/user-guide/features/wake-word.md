...
sidebar_position: 11
title: "Wake Word"
description: "Hands-free 'Hey Moor' wake word — start a voice session by speaking, the 'Hey Siri' way"
---
...
When the desktop app connects to a **remote** Moor backend (for example a
...
Moor supports **client capture** for that case:
...
|--------|------|---------|-------|
| **openWakeWord** (default) | Free | None | Local ONNX models. Ships a bundled **"hey moor"** model (default); also supports `hey_jarvis`, `alexa`, `hey_mycroft`, … and custom models |
...
By default the phrase is **"hey moor"** — a model for it ships with Moor, so
...
```bash
cd ~/.moor/moor-agent && uv pip install -e ".[wake]"
```

## Quick start

```bash
# In an interactive `moor` session:
...
desktop ear button — also writes `wake_word.enabled` to `~/.moor/config.yaml`,
...
wake_word:
  ...
  openwakeword:
    model: hey_moor         # bundled default; OR a built-in name OR a path to a custom .onnx/.tflite
...
- **`confirmation_frames`** (default `3`, openWakeWord only) — how many
  *consecutive* over-threshold frames are required before the wake fires. A real
  "hey moor" holds a high score across several frames; an ambient blip spikes
...
- **`sensitivity`** (default `0.6`) — the detection threshold, `0.0`–`1.0`.
  ...
  **all** engines — for openWakeWord it's the raw per-frame score threshold, for
  ...
  if real "hey moor" utterances are missed.
...
default) to let Moor choose per platform: **tflite on Apple Silicon**, onnx
...
fire. The tflite backend needs `ai-edge-litert` on macOS, which Moor installs
on demand alongside the other wake-word deps.

### Surfaces (CLI, TUI, GUI)

The wake word works in all three Moor surfaces, and `surface` picks which one
...
"Hey Moor" works out of the box — the bundled openWakeWord model
(`model: hey_moor`) is the default. To wake on something else, the easiest
...
command (`moor -p <profile>`) instead of routing.
...
wake_word:
  ...
  phrase: "computer"
  openwakeword:
    model: ~/.moor/wakewords/computer.onnx   # or a built-in name like hey_jarvis
```
...
syllables with an uncommon word ("moor" qualifies) beat common words like
"hello" or "stop".
:::

### Option C — Porcupine (custom keyword in seconds)

Create a "Hey Moor" keyword in the [Picovoice Console](https://console.picovoice.ai/),
...
wake_word:
  ...
  phrase: "hey moor"
  porcupine:
    keyword: ~/.moor/wakewords/hey_moor.ppn
```

Set your access key in `~/.moor/.env`:
...
- A TTS provider for speaking the reply (the default `edge-tts` works with no
  ...
  both STT and TTS are ready — `moor tools` (Voice section) sets them up.
- The wake engine deps (auto-installed, or `moor-agent[wake]`).
...
```bash
moor config set wake_word.input_device "Microphone Array"
```

Use `null` to return to the process default:

```bash
moor config set wake_word.input_device null
```
