...
When running Moor inside Docker, the default `network_mode: host` gives the
...
The Moor [SECURITY.md](https://github.com/thisismamad-n/Moor/blob/main/SECURITY.md) §2 defines the trust model. The
...
#
# Usage:
#   MOOR_UID=$(id -u) MOOR_GID=$(id -g) docker compose up -d
#
...
services:
  gateway:
    ...
    environment:
      ...
      - NO_PROXY=moor,moor-dashboard,localhost
...
docker compose exec gateway \
  curl -sf --max-time 5 http://moor-dashboard:9119/health && echo "OK: internal reachable" || echo "FAIL"
...
- [SECURITY.md](https://github.com/thisismamad-n/Moor/blob/main/SECURITY.md) — Moor trust model and vulnerability reporting
- [Docker](../docker.md) — running Moor in a container
- [Egress proxy](iron-proxy.md) — credential-injection firewall for the sandbox
- [docker-compose.yml](https://github.com/thisismamad-n/Moor/blob/main/docker-compose.yml) — default compose configuration
