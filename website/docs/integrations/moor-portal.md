...
That single command runs the Portal OAuth, lets you pick a Moor model, sets Moor as your inference provider in `config.yaml`, and turns on the Tool Gateway. You're ready to `moor chat` immediately after.
...
The Portal proxies a curated catalog of agentic models from across the ecosystem — billed against your Moor subscription instead of one credit balance per lab.
...
The same subscription unlocks the [Tool Gateway](/user-guide/features/tool-gateway), which routes Moor Agent's tool calls through Moor-managed infrastructure. Five backends, one login:
...
Because everything routes through one OAuth-authenticated Portal session, you don't accumulate a `.env` file with a dozen long-lived API keys. The refresh token at `~/.moor/auth.json` is the only credential on disk, and Moor mints short-lived JWTs from it per request — see [Token handling](#token-handling) below.
...
2. Stores the refresh token at `~/.moor/auth.json`
3. Lets you pick a Moor model from the curated list (or skip to keep your current one)
4. Sets Moor as your inference provider in `~/.moor/config.yaml` (when you pick a model)
5. Turns on the Tool Gateway (web, image, TTS, browser routing)
6. Returns you to your terminal ready to `moor chat`
...
If you already have Moor configured with OpenRouter, Anthropic, or any other provider and you want to add the Portal alongside them:

```bash
moor model
# pick "Moor Portal" from the provider list
# browser opens, sign in, done
```

Your existing providers stay configured. You can switch between them with `/model` mid-session or `moor model` between sessions — the Portal becomes one of your available providers, not your only one.

### Headless / SSH / remote setup

OAuth needs a browser, but the loopback callback runs on the machine where Moor is running. For remote hosts, see [OAuth over SSH / Remote Hosts](/guides/oauth-over-ssh) — the same patterns work for the Portal as for any other OAuth-based provider (`ssh -L` port forwarding).

### Profile setup

If you use [Moor profiles](/user-guide/profiles), the Portal refresh token is automatically shared across all profiles via a shared token store. Sign in once on any profile, and the rest pick it up automatically — no need to repeat the OAuth flow per profile.
...
```bash
moor portal            # log in to Moor Portal + set it up (one-shot onboarding)
moor portal info       # login status, subscription info, model + gateway routing
moor portal status     # alias for `portal info`
moor portal tools      # detailed Tool Gateway catalog with per-tool routing
moor portal open       # open the subscription management page in your browser
```

`moor portal` (with no subcommand) is the human-readable alias for `moor auth add moor --type oauth` — it logs you in, lets you pick a Moor model, sets Moor as your inference provider, and offers the Tool Gateway opt-in (identical to `moor setup --portal`, and the same Moor flow as the first-time quick setup).

`moor portal info` gives you the high-level overview:
...
```
  ...
  Model:   ✓ using Moor as inference provider

  Tool Gateway
  ────────────
  Web search & extract  via Moor Portal
  Image generation      via Moor Portal
  Text-to-speech        via Moor Portal
  Browser automation    via Moor Portal
...
If you already have, say, a Browserbase account and want to keep using it while routing web search and image generation through Moor, that's supported. Use `moor tools` to pick backends per tool:

```bash
moor tools
# → Web search       → "Moor Subscription"
# → Image generation → "Moor Subscription"
...
- **CLI shortcut:** `moor portal open` (opens the same page in your default browser)

## Configuration reference

After `moor setup --portal`, `~/.moor/config.yaml` will look like:
...
Moor mints a short-lived JWT from your stored Portal refresh token on each inference call rather than reusing a long-lived API key. The token lifecycle is fully automatic — refresh, mint, retry on transient 401 — and you never see it.
...
If the Portal invalidates the refresh token (password change, manual revoke, session expiry), the invalid refresh token is **quarantined locally** so Moor stops replaying it and you don't see a stream of identical 401s. The next call surfaces a clear "re-authentication required" message. Run `moor auth add moor` to log in again; the quarantine clears on the next successful login.

## Troubleshooting

### `moor portal info` shows "not logged in"
...
or use `moor model` and re-select Moor Portal.

### Got a "re-authentication required" message mid-session

Your Portal refresh token was invalidated (password change, manual revoke, or session expiry). Run `moor auth add moor` and your next request will use the new credentials. Any quarantine on the old token clears automatically on successful re-login.
...
Check `moor portal info` first — if it shows you're using a different provider (`Model: currently openrouter` instead of `using Moor as inference provider`), your local config has drifted. Run `moor model`, pick Moor Portal, and the next request will route through your subscription.

## See also

- **[Tool Gateway](/user-guide/features/tool-gateway)** — Full details on every gateway tool, per-tool config, and pricing
- **[Subscription proxy](/user-guide/features/subscription-proxy)** — Use your Portal subscription from non-Moor tools (other agents, scripts, third-party clients)
...
- **[Profiles](/user-guide/profiles)** — Multiple Moor configurations sharing one Portal login
