...
| Provider | Setup |
|----------|-------|
| **Nous Portal** | `moor model` (OAuth, subscription-based) |
| **OpenAI Codex** | `moor model` (ChatGPT OAuth, uses Codex models) |
...
| **Google Gemini (OAuth)** | `moor model` → "Google Gemini (OAuth)" (provider: `google-gemini-cli`, free tier supported, browser PKCE login) |
...
moor model              # existing install — pick "Nous Portal" from the list
moor portal info        # inspect login + routing at any time
```

Don't have a subscription yet? Get one at [portal.Moor inc..com/manage-subscription](https://portal.Moor inc..com/manage-subscription).

**For full details:** see the dedicated [Nous Portal integration page](/integrations/nous-portal) (what's in the subscription, model catalog, troubleshooting) and the step-by-step [Run Moor Agent with Nous Portal guide](/guides/run-hermes-with-nous-portal).

**Client identification.** Every Portal request from Moor Agent carries a `client=hermes-client-v<version>` tag (e.g. `client=hermes-client-v0.13.0`) auto-aligned to your installed release. This is sent on all Portal pathways — main chat loop, auxiliary calls, compression summarizer, web extraction — and lets Portal-side telemetry distinguish Moor traffic from other clients. No config required; the tag updates automatically when you `moor update`.
...
:::info Codex Note
The OpenAI Codex provider authenticates via device code (open a URL, enter a code). Moor stores the resulting credentials in its own auth store under `~/.hermes/auth.json` and can import existing Codex CLI credentials from `~/.codex/auth.json` when present. No Codex CLI installation is required.

If a token refresh fails with a terminal error (HTTP 4xx, `invalid_grant`, revoked grant, etc.), Moor marks the refresh token as dead and stops replaying it so you don't see a flood of identical auth failures. The next request surfaces a typed re-auth message instead. Run `moor auth add codex-oauth` (or `moor model` → OpenAI Codex) to start a fresh device-code login; the quarantine clears on the next successful exchange.
:::
...
Paid Nous Portal subscribers also get access to the **[Tool Gateway](/user-guide/features/tool-gateway)** — web search, image generation, TTS, and browser automation routed through your subscription. No extra API keys needed. On a fresh install, `moor setup --portal` logs you in, sets Nous as your provider, and turns the gateway on in one command. Existing users can enable it from `moor model` or per-tool from `moor tools`. Inspect routing at any time with `moor portal info`.
:::
...
When you choose Anthropic OAuth through `moor model`, Moor prefers Claude Code's own credential store over copying the token into `~/.hermes/.env`. That keeps refreshable Claude credentials refreshable.
...
xAI is wired through the Responses API (`codex_responses` transport) for automatic reasoning support on Grok 4 models — no `reasoning_effort` parameter needed, the server reasons by default. Set `XAI_API_KEY` in `~/.hermes/.env` and pick xAI in `moor model`, or drop `grok` as a shortcut into `/model grok-4-fast-reasoning`.
...
**Retired xAI model migration (May 15, 2026):** xAI is retiring `grok-4*`, `grok-3`, `grok-code-fast-1`, and `grok-imagine-image-pro` on 2026-05-15. `moor doctor` and `moor chat` startup both detect any config still pointing at a retired ref and print the recommended replacement. Use `hermes migrate xai` for a one-shot config rewrite — dry-run by default, add `--apply` to write changes (a timestamped `config.yaml.bak-pre-migrate-xai-*` backup is created automatically).
...
# → done — Moor auto-provisions your free tier on first request
```

Moor ships Google's **public** `gemini-cli` desktop OAuth client by default —
...
- Responses translated back to OpenAI shape so the rest of Moor works unchanged
...
| VPC-SC-protected org | Moor detects `SECURITY_POLICY_VIOLATED` and forces `standard-tier` automatically |
...
experience, use your own API key via the `gemini` provider instead. Moor shows
...
Supported parsers: `hermes` (Qwen 2.5, Moor 2/3), `llama3_json` (Llama 3.x), `mistral`, `deepseek_v3`, `deepseek_v31`, `xlam`, `pythonic`. Without these flags, tool calls won't work — the model will output tool calls as text.
...
Native tool calling support (best performance): Llama 3.x, Qwen 2.5 (including Coder), Moor 2/3, Mistral, DeepSeek, Functionary. All other models use a generic handler that works but may be less efficient. See the [llama.cpp function calling docs](https://github.com/ggml-org/llama.cpp/blob/master/docs/function-calling.md) for the full list.
...
Moor will automatically load a LM Studio model with 64K context length
...
custom_providers:
  ...
  - name: work
    ...
    api_mode: chat_completions   # set explicitly by `moor model` → Custom Endpoint wizard; auto-detection still happens as a fallback
...
The `moor model` → Custom Endpoint wizard now prompts for `api_mode` explicitly and persists your answer to `config.yaml`. URL-based auto-detection (e.g. `/anthropic` paths → `anthropic_messages`) still happens as a fallback when the field is left blank.
