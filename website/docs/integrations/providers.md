...
| **Qwen Cloud (Alibaba DashScope)** | `DASHSCOPE_API_KEY` in `~/.moor/.env` (provider: `alibaba`) |
...
| **OpenCode Free** | Keyless — no API key or account needed (provider: `opencode-free`, aliases: `free`, `opencode_free`). Select via `moor model` or `/model free`; requests are sent anonymously |
...
moor chat --provider tencent-tokenhub --model hy3-preview
