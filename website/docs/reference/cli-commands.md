...
| `moor auth` | Manage credentials — add, list, remove, reset, set strategy. Handles OAuth flows for Codex/Nous/Anthropic. |
| `hermes login` / `logout` | **Deprecated** — use `moor auth` instead. |
...
| `moor checkpoints` | Inspect / prune / clear `~/.hermes/checkpoints/` (the shadow store used by `/rollback`). Run with no args for a status overview. |
...
| `moor memory` | Configure external memory provider. Plugin-specific subcommands (e.g. `hermes honcho`) register automatically when their provider is active. |
...
| `moor portal` | Nous Portal status, subscription link, and Tool Gateway routing. See [Tool Gateway](../user-guide/features/tool-gateway.md). |
...
| `moor update` | Pull latest code and reinstall dependencies (git installs), or check PyPI and `pip install --upgrade` (pip installs). `--check` previews without installing; `--backup` takes a pre-pull `HERMES_HOME` snapshot. |
...
| Option | Description |
|--------|-------------|
| `--all` | On `start` / `restart` / `stop`: act on **every profile's** gateway, not just the active `HERMES_HOME`. Useful if you run multiple profiles side-by-side and want to restart them all after `moor update`. |
...
:::tip WSL users
Use `moor gateway run` instead of `moor gateway start` — WSL's systemd support is unreliable. Wrap it in tmux for persistence: `tmux new -s hermes 'moor gateway run'`. See [WSL FAQ](/reference/faq#wsl-gateway-keeps-disconnecting-or-hermes-gateway-start-fails) for details.
:::
...
**Easiest path:** `moor setup --portal` — OAuth into Nous Portal and opt into the [Tool Gateway](../user-guide/features/tool-gateway.md) in one shot.
...
moor slack manifest --write      # write to ~/.hermes/slack-manifest.json
...
> Not to be confused with `moor claw migrate` (one-shot import of OpenClaw configuration into Moor) — `hermes migrate` is the top-level config-rewrite command.
...
:::caution
`hermes login` has been removed. Use `moor auth` to manage OAuth credentials, `moor model` to select a provider, or `moor setup` for full interactive setup.
:::
...
:::tip
`moor dump` is specifically designed for sharing. For interactive diagnostics, use `moor doctor`. For a visual overview, use `hermes status`.
:::
...
The report includes system info (OS, Python version, Moor version), recent agent and gateway logs (512 KB limit per file), and redacted API key status. Keys are always redacted — no secrets are uploaded.
...
### Examples
```bash
moor import ~/hermes-backup-20260423.zip           # Prompts before overwriting existing config
moor import ~/hermes-backup-20260423.zip --force   # Overwrite without prompting
```
...
moor skills install https://sharethis.chat/SKILL.md                     # Direct URL (single-file SKILL.md)
...
On a fresh install the first scheduled pass is deferred by one full `interval_hours` (7 days by default) — the gateway will not curate immediately on the first tick after `moor update`. Use `hermes curator run --dry-run` to preview before that happens.
...
See [Plugins](../user-guide/features/plugins.md) and [Build a Moor Plugin](../guides/build-a-hermes-plugin.md).
...
| `--no-backup` | Skip the pre-migration zip snapshot of `~/.hermes/` (by default a single restore-point archive is written to `~/.hermes/backups/pre-migration-*.zip` before apply; restorable with `moor import`). |
...
# Fish
moor completion fish > ~/.config/fish/completions/hermes.fish
```
...
**pip installs:** `moor update` detects pip-based installations automatically — it queries PyPI for the latest release and runs `pip install --upgrade hermes-agent` instead of `git pull`. PyPI releases track tagged versions (major/minor releases), not every commit on `main`. Use `--check` to see if a newer PyPI release is available without installing.

**git installs:** `moor update` pulls the configured update branch (default: `main`). If your checkout is on another branch, Moor may check out the update branch before pulling. Commit branch work before updating when you want to keep it outside the update autostash flow.
...
- **Pairing data snapshot.** Even when `--backup` is off, `moor update` takes a lightweight snapshot of `~/.hermes/pairing/` and the Feishu comment rules before `git pull`. You can roll it back with `hermes backup restore --state pre-update` if a pull rewrites a file you were editing.
...
| `hermes postinstall` | Internal bootstrap. Runs once after `pip install hermes-agent` (or `moor update` on pip installs) to install non-Python dependencies that pip cannot provide — Node.js runtime, headless browser, ripgrep, ffmpeg — and then trigger `moor setup` if the profile has not been configured yet. Safe to re-run idempotently. |
| `moor uninstall [--full] [--gui] [--yes]` | Remove Moor, optionally deleting all config/data. `--gui` removes only the desktop Chat GUI, leaving the agent intact; `--full` also deletes config/data; `--yes` skips prompts. |
