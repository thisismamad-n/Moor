...
```bash
moor [global-options] <command> [subcommand/options]
```
...
| `--profile <name>`, `-p <name>` | Select which Moor profile to use for this invocation. Overrides the sticky default set by `moor profile use`. |
...
| `--ignore-user-config` | Ignore `~/.moor/config.yaml` and fall back to built-in defaults. Credentials in `.env` are still loaded. |
| `--ignore-rules` | Skip auto-injection of `AGENTS.md`, `SOUL.md`, `.cursorrules`, memory, and preloaded skills. |
| `--tui` | Launch the [TUI](../user-guide/tui.md) instead of the classic CLI. Equivalent to `MOOR_TUI=1`. Always wins over `display.interface`. |
...
### `moor-agent` (legacy single-query runner)

The install also ships `moor-agent`, a minimal runner that sends one query and exits: `moor-agent --query "summarize README.md"` (or `moor-agent "summarize README.md"`). `moor-agent --help` lists its options (`--model`, `--base-url`, `--max-turns`, `--enabled-toolsets`, `--disabled-toolsets`, `--list-tools`, `--save-trajectories`, …) and `moor-agent --version` prints the version; neither starts the agent. Run with no query, it prints the same help and exits. For anything else use `moor` (`moor -z <prompt>` is the scripted one-shot).
...
| `moor codex-runtime` | Noninteractive counterpart of `/codex-runtime`: `migrate [--dry-run] [--json]` regenerates the moor-managed block in `~/.codex/config.toml` for the selected profile. See [Codex app-server runtime](../user-guide/features/codex-app-server-runtime.md#running-the-migration-from-a-script). |
| `moor status` | Show agent, auth, and platform status. |
| `moor usage` | Show the configured account's rate-limit windows (the `/usage` block) without a session; `--json` for scripts. |
...
```bash
moor
moor chat -q "Summarize the latest PRs"          # seeds an interactive session
moor chat --oneshot -q "Summarize the latest PRs"  # answer and exit
moor chat --provider openrouter --model anthropic/claude-sonnet-4.6
moor chat --toolsets web,terminal,skills
moor chat --quiet -q "Return only JSON"
moor chat -q "Inspect this repository" --format stream-json
moor chat --worktree -q "Review this repo and open a PR"
moor chat --ignore-user-config --ignore-rules -q "Repro without my personal setup"
moor chat --safe-mode -q "Is this bug mine or Moor'?"
```
...
```bash
moor chat -q "Summarize this repository" --format stream-json
```
...
(`MOOR_KANBAN_TASK` set) whose turn failed only because the provider was
...
### `moor -z <prompt>` — scripted one-shot

For programmatic callers (shell scripts, CI, cron, parent processes piping in a prompt), `moor -z` is the purest one-shot entry point: **single prompt in, final response text out, nothing else on stdout or stderr.** No banner, no spinner, no tool previews, no `Session:` line — just the agent's final reply as plain text.

```bash
moor -z "What's the capital of France?"
# → Paris.

# Parent scripts can cleanly capture the response:
answer=$(moor -z "summarize this" < /path/to/file.txt)
```

Per-run overrides (no mutation to `~/.moor/config.yaml`):

| Flag | Equivalent env var | Purpose |
|---|---|---|
| `-m` / `--model <model>` | `MOOR_INFERENCE_MODEL` | Override the model for this run |
...
```bash
moor -z "…" --provider openrouter --model openai/gpt-5.5
# or:
MOOR_INFERENCE_MODEL=anthropic/claude-sonnet-4.6 moor -z "…"
```
...
`moor -z "…" --usage-file /path/report.json` writes a machine-readable usage report after the run: `estimated_cost_usd`, `input_tokens` / `output_tokens` / `cache_read_tokens` / `cache_write_tokens` / `reasoning_tokens` / `total_tokens`, `api_calls`, `model`, `provider`, `session_id`, `service_tier`, the `completed` / `failed` / `partial` / `interrupted` flags and `turn_exit_reason` (why `completed` is false, e.g. `max_iterations_reached(3/3)`). Those top-level counters cover the **main agent loop** only. Auxiliary LLM calls made on the same run (title generation, vision, context compression, `web_extract`, background review, …) are reported separately under `auxiliary` — the same totals plus a per-task `by_task` map — and `total_including_auxiliary` (`estimated_cost_usd`, `total_tokens`, `api_calls`) is the grand total to bill on. The report is written **even when the run fails**, so batch pipelines can always account for spend. It has no effect outside `-z`/`--oneshot`, and a broken usage write never masks the run's own outcome.

```bash
moor -z "summarize this repo" --usage-file ~/.moor/cache/scratch/usage.json
jq .total_including_auxiliary.estimated_cost_usd ~/.moor/cache/scratch/usage.json
jq .auxiliary.by_task ~/.moor/cache/scratch/usage.json      # what did title generation / vision cost?
```
...
- log into OAuth-backed providers (Anthropic, Copilot, Codex, Moor Portal)
- enter or update API keys
- pick from provider-specific model lists
- configure a custom/self-hosted endpoint
- save the new default into config

:::warning moor model vs /model — know the difference
**`moor model`** (run from your terminal, outside any Moor session) is the **full provider setup wizard**. It can add new providers, run OAuth flows, prompt for API keys, and configure endpoints.
...
**If you need to add a new provider:** Exit your Moor session first (`Ctrl+C` or `/quit`), then run `moor model` from your terminal prompt.
:::
...
If you've only configured OpenRouter, `/model` will only show OpenRouter models. To add another provider (Anthropic, DeepSeek, Copilot, etc.), exit your session and run `moor model` from the terminal.
:::
...
| `enroll` | Experimental: enroll this gateway with a relay connector and save relay credentials for connector-backed platforms. See [Moor Relay](../user-guide/messaging/relay.md). |

Options:

| Option | Description |
|--------|-------------|
| `--all` | On `start` / `restart` / `stop`: act on **every profile's** gateway, not just the active `MOOR_HOME`. Useful if you run multiple profiles side-by-side and want to restart them all after `moor update`. |
| `--no-supervise` | On `run`: inside the s6-overlay Docker image, opt out of auto-supervision and use pre-s6 foreground semantics — gateway runs as the container's main process with no auto-restart. No-op outside the s6 image. Equivalent to setting `MOOR_GATEWAY_NO_SUPERVISE=1`. |
...
`moor gateway restart`, or service-restart update exits with status `75`
...
:::tip WSL users
Use `moor gateway run` instead of `moor gateway start` — WSL's systemd support is unreliable. Wrap it in tmux for persistence: `tmux new -s moor 'moor gateway run'`. See [WSL FAQ](./faq.md#wsl-gateway-keeps-disconnecting-or-moor-gateway-start-fails) for details.
:::
...
```bash
moor setup [model|tts|terminal|gateway|tools|agent] [--non-interactive] [--reset] [--quick] [--reconfigure] [--portal]
```

**Easiest path:** `moor setup --portal` — OAuth into Moor Portal and opt into the [Tool Gateway](../user-guide/features/tool-gateway.md) in one shot.
...
| `--reconfigure` | Backwards-compat alias — bare `moor setup` on an existing install now does this by default. |
| `--portal` | One-shot Moor Portal setup: log in via OAuth, set Moor as the inference provider, and opt into the [Tool Gateway](../user-guide/features/tool-gateway.md). Skips the rest of the wizard. |

## `moor portal`

```bash
moor portal [status|open|tools]
```

Inspect Moor Portal auth, Tool Gateway routing, and reach the subscription page. Subcommand-less invocation runs `status`.
...
| `tools` | List every Tool Gateway partner (Firecrawl, FAL, OpenAI TTS, Browser Use, Modal) and which are routed via Moor. |

For configuration of the gateway itself, see [Tool Gateway](../user-guide/features/tool-gateway.md). For the one-shot setup path, see `moor setup --portal` above.
...
```bash
moor slack manifest              # print manifest to stdout
moor slack manifest --write      # write to ~/.moor/slack-manifest.json
moor slack manifest --long-description-file AGENTS.md --write
moor slack manifest --slashes-only  # just the features.slash_commands array
```
...
|------|---------|---------|
| `--write [PATH]` | stdout | Write to a file instead of stdout. Bare `--write` writes `$MOOR_HOME/slack-manifest.json`. |
...
Run `moor slack manifest --write` again after `moor update` to pick
up any new commands.
...
```bash
moor send --to <target> "message text"
moor send --to <target> --file <path>
echo "message" | moor send --to <target>
moor send --list [platform]
```

Send a one-shot message to a configured messaging platform without spinning up an agent or gateway loop. Reuses the gateway's already-configured credentials (`~/.moor/.env` + `~/.moor/config.yaml`) so ops scripts, cron jobs, CI hooks, and monitoring daemons can post status updates without reimplementing each platform's REST client.

For bot-token platforms (Telegram, Discord, Slack, Signal, SMS, WhatsApp-CloudAPI) no running gateway is required — `moor send` talks directly to the platform's REST endpoint. Plugin platforms that need a persistent adapter still require a live gateway.
...
If neither a positional `message` argument nor `--file` is provided, `moor send` reads from stdin when it is not a TTY. Exit codes: `0` on success, `1` on delivery/backend failure, `2` on usage errors.
...
```bash
moor send --to telegram "MEDIA:~/.moor/cache/scratch/screenshot.png"
moor send --to telegram "Build chart for today MEDIA:~/.moor/cache/scratch/chart.png"   # with caption
moor send --to discord:#ops "MEDIA:~/.moor/cache/scratch/report.pdf"
```
...
```bash
moor send --to telegram "[[as_document]] MEDIA:~/.moor/cache/scratch/screenshot.png"
```
...
moor peer run <peer>[/<agent>] --idempotency-key <key> "message"
moor peer status <peer>[/<agent>] <run_id>
moor peer stop <peer>[/<agent>] <run_id>
...
```bash
moor secrets bitwarden <subcommand>
moor secrets bw <subcommand>          # short alias
```

Pull API keys from an external secret manager at process startup instead of storing them in `~/.moor/.env`. Currently supports **Bitwarden Secrets Manager**. See the full guide: [Bitwarden integration](../user-guide/secrets/bitwarden.md).
...
```bash
moor codex-runtime migrate [--dry-run] [--json]
```

Runs the `~/.codex/config.toml` migration that `/codex-runtime codex_app_server` triggers, without a chat session: Moor' `mcp_servers` (plus installed codex plugins and the `default_permissions` default) are projected into the managed block for the selected profile (`moor -p <name> codex-runtime migrate`). User text outside the block is kept verbatim; a user-owned `[mcp_servers.<name>]` with the same name as a Moor server is preserved and the Moor projection for that name skipped (reported as `preserved_user_servers`). The result is validated as TOML before an atomic write; exit code is 1 when the report contains errors.
...
```bash
moor security <subcommand>
```

On-demand vulnerability scan against [OSV.dev](https://osv.dev). Covers the Moor venv (installed PyPI distributions), Python dependencies declared by plugins under `~/.moor/plugins/`, and pinned `npx`/`uvx` MCP servers in `config.yaml`. Does NOT scan globally-installed packages or editor/browser extensions.
...
## `moor login` / `moor logout` *(Deprecated)*

:::caution
`moor login` has been removed. Use `moor auth` to manage OAuth credentials, `moor model` to select a provider, or `moor setup` for full interactive setup.
:::
...
moor auth add openai-codex --browser                   # Codex: browser auth-code + PKCE on localhost:1455 instead of device code
...
```bash
moor usage                          # configured model provider, human-readable block
moor usage --provider openai-codex  # a specific provider
moor usage --json                   # one JSON document on stdout
```
...
```bash
moor cron <list|create|edit|pause|resume|run|remove|status|runs|incidents|doctor|tick>
```
...
`$MOOR_HOME/plugins/<name>/`. An unknown or unavailable provider falls back to
...
```bash
moor kanban [--board <slug>] <action> [options]
```

Multi-profile, multi-project collaboration board. Each install can host many boards (one per project, repo, or domain); each board is a standalone queue with its own SQLite DB and dispatcher scope. New installs start with one board called `default`, whose DB is `~/.moor/kanban.db` for back-compat; additional boards live at `~/.moor/kanban/boards/<slug>/kanban.db`. The gateway-embedded dispatcher sweeps every board per tick.
...
| `boards switch <slug>` / `boards use` | Persist `<slug>` as the active board (writes `~/.moor/kanban/current`). |
...
moor kanban boards create atm10-server --name "ATM10 Server" --icon 🎮
moor kanban --board atm10-server create "Restart server" --assignee ops

# Switch the active board for subsequent calls.
moor kanban boards switch atm10-server
moor kanban list                  # shows atm10-server tasks
...
Board resolution order (highest precedence first): `--board <slug>` flag → `MOOR_KANBAN_BOARD` env var → `~/.moor/kanban/current` file → `default`.
...
moor egress install --force          # check and repair the managed copy
...
moor egress setup --from-bitwarden   # use Bitwarden Secrets Manager as credential source
moor egress setup --no-bitwarden     # explicitly switch back to env-based credentials
moor egress setup --rotate-tokens    # mint fresh proxy tokens (default preserves existing)

moor egress start                    # spawn the managed proxy daemon
moor egress stop                     # SIGTERM (then SIGKILL after 5s grace)
moor egress restart                  # stop (if running) then start — needed for secret changes
moor egress reload                   # hot-reload the ruleset in-place (no restart, no dropped
                                       #   connections) via the loopback management API

moor egress status                   # binary + config + pid + listening + mappings
moor egress status --show-tokens     # print proxy tokens in full (default: redacted)

moor egress disable                  # flip proxy.enabled = false (does not stop a running proxy)
moor egress config                   # print the path to proxy.yaml for inspection
```
...
moor egress setup && moor egress start
moor config set terminal.backend docker   # if not already

# Switching credential source after the fact
moor egress setup --from-bitwarden       # env → bitwarden
moor egress setup --no-bitwarden         # bitwarden → env
# (just `setup` without either flag preserves the existing mode)

# Rotating all tokens (e.g. after a suspected token leak)
moor egress setup --rotate-tokens    # setup offers to restart the running daemon for you
# (running sandboxes still hold old tokens; restart them too)

# Adding a new upstream
# Edit ~/.moor/config.yaml proxy.extra_allowed_hosts: [api.example.com]
moor egress setup
moor egress restart                  # one-command apply (stop + start)
```

### Diagnostic shortcuts

```bash
moor egress status                     # current state in one view
cat ~/.moor/proxy/proxy.yaml           # the rendered iron-proxy config
tail -20 ~/.moor/proxy/iron-proxy.log  # daemon-level diagnostics
tail -f ~/.moor/proxy/iron-proxy.log | jq  # daemon + per-request log (line-delimited JSON; v0.39 combines both streams)
```
...
```bash
moor project <create|list|show|add-folder|remove-folder|rename|set-primary|use|archive|restore|bind-board>
```
...
```bash
moor webhook <subscribe|list|remove|test>
```
...
### `moor webhook subscribe`

```bash
moor webhook subscribe <name> [options]
```
...
Exit status: `0` when the report lists no unresolved problems, `1` when at least one remains (including problems `--fix` could not repair), so a health gate or CI step can trust `moor doctor` as a check.
...
| **Identity** | Active profile name, MOOR_HOME path |
...
:::tip
`moor dump` is specifically designed for sharing. For interactive diagnostics, use `moor doctor`. For a visual overview, use `moor status`.
:::

## `moor debug`

```bash
moor debug share [options]
```
...
| `--moor` | Upload to Moor-internal diagnostics storage instead of a public paste service. Use this when Moor support asks for a private diagnostic bundle. |
...
Default uploads use public paste services tried in order: paste.rs, dpaste.com. `--moor` uploads the same debug bundle to private Moor diagnostics storage instead; the returned viewer link is for the Moor team and auto-deletes after 14 days.

### Examples

```bash
moor debug share              # Upload debug report, print URL
moor debug share --lines 500  # Include more log lines
moor debug share --expire 30  # Keep paste for 30 days
moor debug share --moor       # Upload a private diagnostics bundle for Moor support
moor debug share --local      # Print report to terminal (no upload)
```
...
| Option | Description |
|--------|-------------|
| `-o`, `--output <path>` | Output path for the zip file (default: `~/moor-backup-<timestamp>.zip`). |
| `-q`, `--quick` | Quick snapshot: only critical state files (config.yaml, state.db, .env, auth, cron jobs). Much faster than a full backup. |
| `-l`, `--label <name>` | Label for the snapshot (only used with `--quick`). |
| `-k`, `--keep <N>` | After a full backup, delete older `moor-backup-*.zip` files in the output directory beyond the newest N (default 3; `0` keeps everything). Custom-named zips are never touched. |
...
moor backup -o ~/backups/moor.zip   # Full backup to specific path
...
```bash
moor checkpoints [COMMAND]
```

Inspect and manage the shadow git store at `~/.moor/checkpoints/` — the storage layer behind the in-session `/rollback` command. Safe to run any time; does not require the agent to be running.

| Subcommand | Description |
|------------|-------------|
| `status` (default) | Show total size, project count, and per-project breakdown. Bare `moor checkpoints` is equivalent. |
...
```bash
moor checkpoints                                  # status overview
moor checkpoints prune --retention-days 3         # aggressive cleanup
moor checkpoints prune --max-size-mb 200          # tighten size cap once
moor checkpoints clear-legacy -f                  # drop v1 archive dirs
moor checkpoints clear -f                         # wipe everything
```
...
```
  ⚠ Session data replaced by older backup contents:
    ...
    Recover from a newer backup or snapshot: moor snapshot list
```

### Examples
```bash
moor import ~/moor-backup-20260423.zip           # Prompts before overwriting existing config
moor import ~/moor-backup-20260423.zip --force   # Overwrite without prompting
```

## `moor logs`

```bash
moor logs [log_name] [options]
```

View, tail, and filter Moor log files. All logs are stored in `~/.moor/logs/` (or `<profile>/logs/` for non-default profiles).
...
moor logs gateway -n 100

# Show only warnings and errors from the last hour
moor logs --level WARNING --since 1h

# Filter by a specific session
moor logs --session abc123

# Follow errors.log, starting from 30 minutes ago
moor logs errors --since 30m -f
...
moor logs --level WARNING --since 2h --session tg-12345
```
...
Moor uses Python's `RotatingFileHandler`. Old logs are rotated automatically — look for `agent.log.1`, `agent.log.2`, etc. The `moor logs list` subcommand shows all log files including rotated ones.


## `moor prompt-size`

```bash
moor prompt-size [--platform <name>] [--json]
```
...
moor prompt-size --platform telegram

# Machine-readable output for scripts
moor prompt-size --json
```

:::tip
The skills index and tool schemas scale with how many skills and tools you have
enabled. To shrink the prompt, disable unused toolsets (`moor tools`) or
uninstall skills you don't need (`moor skills`). Context files (AGENTS.md,
...
`moor config set/get/unset` use `.` as the nesting separator, but many real
...
- **Existing keys just work.** When navigating an existing mapping, an
  ...
  splitting. `moor config set providers.p.models.grok-4.6.supports_vision true`
...
- **Creating a new dotted key requires escaping.** Escape literal dots with a
  backslash: `moor config set 'providers.p.models.grok-4\.7.context_length' 128000`
...
```bash
moor pairing <list|approve|revoke|clear-pending>
```
...
```bash
moor skills browse
moor skills browse --source official
moor skills search react --source skills-sh
moor skills search https://mintlify.com/docs --source well-known
moor skills inspect official/security/1password
moor skills inspect skills-sh/vercel-labs/json-render/json-render-react
moor skills install official/migration/openclaw-migration
moor skills install skills-sh/anthropics/skills/pdf --force
moor skills install https://sharethis.chat/SKILL.md                     # Direct URL (+ referenced support files)
moor skills install https://example.com/SKILL.md --name my-skill        # Override name when frontmatter has none
moor skills check
moor skills update
moor skills config
moor skills reset google-workspace
moor skills reset google-workspace --restore --yes
moor skills opt-out                  # stop future bundled-skill seeding (nothing deleted)
moor skills opt-out --remove --yes   # also delete UNMODIFIED bundled skills
moor skills opt-in --sync            # undo: remove marker and re-seed now
```
...
Skill bundles group several skills under one `/<bundle-name>` slash command. Invoking the bundle loads every referenced skill into a single combined user message. Storage: `~/.moor/skill-bundles/<slug>.yaml`. See [Skill Bundles](../user-guide/features/skills.md#skill-bundles) for the YAML schema and behavior.
...
| `reload` | Re-scan `~/.moor/skill-bundles/` and report added/removed bundles |

Examples:

```bash
moor bundles create backend-dev \
...
moor bundles list
moor bundles show backend-dev
moor bundles delete backend-dev
```
...
| `backup` | Take a manual tar.gz snapshot of `~/.moor/skills/` (curator also snapshots automatically before every real run) |
| `rollback` | Restore `~/.moor/skills/` from a snapshot (defaults to newest) |
...
On a fresh install the first scheduled pass is deferred by one full `interval_hours` (7 days by default) — the gateway will not curate immediately on the first tick after `moor update`. Use `moor curator run --dry-run` to preview before that happens.
...
moor moa delete <name>
```

`moor moa configure` reuses Moor' provider → model picker for each reference model and the aggregator. A preset is an execution-mode configuration, not a primary model or provider.

## `moor fallback`

```bash
moor fallback <subcommand>
```
...
| `add` | Pick a provider + model (same picker as `moor model`) and append to the chain |
...
```bash
moor hooks <subcommand>
```

Inspect shell-script hooks declared in `~/.moor/config.yaml`, test them against synthetic payloads, and manage the first-use consent allowlist at `~/.moor/shell-hooks-allowlist.json`.
...
Set up and manage external memory provider plugins. Bundled providers: honcho, openviking, mem0, holographic, retaindb, byterover, supermemory; hindsight (plugin catalog) after `moor plugins install hindsight`. Only one external provider can be active at a time. Built-in memory (MEMORY.md/USER.md) is always active.
...
When an external memory provider is active, it may register its own top-level `moor <provider>` command for provider-specific management (e.g. `moor honcho` when Honcho is active). Inactive providers do not expose their subcommands. Run `moor --help` to see what's currently wired in.
:::
...
| `install <name>` | Install a catalog entry (e.g. `moor mcp install deepwiki`). |
...
See [MCP Config Reference](./mcp-config-reference.md), [Use MCP with Moor](../guides/use-mcp-with-moor.md), and [MCP Server Mode](../user-guide/features/mcp.md#running-moor-as-an-mcp-server).

## `moor plugins`

```bash
moor plugins [subcommand]
```

Unified plugin management — general plugins, memory providers, and context engines in one place. Running `moor plugins` with no subcommand opens a composite interactive screen with two sections:
...
`moor computer-use install` is the stable entry point for installing the
[cua-driver](https://github.com/trycua/cua) binary used by the
`computer_use` toolset. It runs the same upstream installer that
`moor tools` invokes when you first enable Computer Use, so it's safe
...
`MOOR_CUA_DRIVER_CMD`; update that binary directly or remove the override.
`moor computer-use status` reports when repair is required.
...
`moor update` automatically re-runs the upstream installer at the end
...
```bash
moor pets <list|install|select|show|off|scale|remove|doctor>
```
...
```bash
moor insights [--days N] [--source platform]
```
...
```bash
moor claw migrate [options]
```

Migrate your OpenClaw setup to Moor. Reads from `~/.openclaw` (or a custom path) and writes to `~/.moor`. Automatically detects legacy directory names (`~/.clawdbot`, `~/.moltbot`) and config filenames (`clawdbot.json`, `moltbot.json`).
...
| `--no-backup` | Skip the pre-migration zip snapshot of `~/.moor/` (by default a single restore-point archive is written to `~/.moor/backups/pre-migration-*.zip` before apply; restorable with `moor import`). |
...
moor claw migrate --preset full

# Full migration including API keys
moor claw migrate --preset full --migrate-secrets

# Migrate user data only (no secrets), overwrite conflicts
moor claw migrate --preset user-data --overwrite

# Migrate from a custom OpenClaw path
moor claw migrate --source /home/user/old-openclaw
```

## `moor import-agent`

```bash
moor import-agent [claude-code|codex] [options]
```

Import a **Claude Code** (`~/.claude`) or **OpenAI Codex CLI** (`~/.codex`) setup into Moor. Maps `CLAUDE.md`/`AGENTS.md` instructions to memory entries, `Bash(...)` permission allow/deny rules to `command_allowlist`/`approvals.deny`, MCP servers to `mcp_servers` in `config.yaml`, and skill directories into `~/.moor/skills/`. Always previews before applying; API keys and credentials are never imported.
...
Launch the web dashboard to manage configuration, API keys, and sessions. For a headless backend, use [`moor serve`](#moor-serve). FastAPI, Uvicorn, and the platform PTY helper are core dependencies. The `web` extra adds exact HTTP-stack constraints and is selected by standard PM setup through `all`. If dependencies are damaged, run `moor pm repair`. The embedded Chat tab requires a POSIX PTY environment, such as Linux, macOS, or WSL2. See [Web Dashboard](../user-guide/features/web-dashboard.md).
...
| `--stop` | — | Stop the running `moor dashboard` / `moor serve` backend **of this Moor home** and exit (`-p <profile>` / `MOOR_HOME` select which; backends of other profiles, other installs on the machine, and the shell you typed the command into are never touched — a backend whose owner cannot be read is left alone). SIGTERM, a 10s grace, then SIGKILL; a hosted Chat TUI that outlives its backend is stopped too (it would otherwise keep the deleted `state.db-wal` open and block the next start). Messaging-gateway bots started from the dashboard are not touched. |
| `--status` | — | List running `moor dashboard` processes and exit. |

### `moor dashboard register`

Register this install as a self-hosted dashboard with your Moor Portal account. Creates an OAuth client, writes `MOOR_DASHBOARD_OAUTH_CLIENT_ID` into `~/.moor/.env`, and prints how to engage the login gate. Requires being logged in (`moor setup`).

| Option | Description |
|--------|-------------|
| `--name` | Human-readable label for the dashboard (default: auto-generated). |
| `--redirect-uri` | Public HTTPS OAuth redirect URI (e.g. `https://moor.example.com/auth/callback`). Omit for localhost-only use. |
| `--portal-url` | Override the Moor Portal base URL for registration (default: the portal you logged into). Also settable via `MOOR_DASHBOARD_PORTAL_URL`. |
...
moor dashboard --port 8080 --no-open
...
moor profile create work --clone --sync-imports   # also carry over the import-agent sync manifest
moor profile use work
moor profile alias work --name h-work
moor profile export work -o work-backup.tar.gz
moor profile import work-backup.tar.gz --name restored
moor profile install github.com/user/my-distro --alias
moor profile update work
moor -p work chat -q "Hello from work profile"
```
...
This command does not update the Moor application itself.
...
- **Gateway restart.** After a successful update, Moor attempts to restart all running gateway profiles of the home being updated (its root and every `profiles/<name>` under it) automatically so they pick up the new code. Gateways and `moor-gateway*` services that belong to a different `MOOR_HOME` on the same machine — another install, or a scratch home running `moor update` — are named in the output and left alone. Use `moor gateway restart` when you want to restart a gateway without applying an update.
...
- **Legacy `moor.service` warning.** If Moor detects a pre-rename `moor.service` systemd unit (instead of the current `moor-gateway.service`), it prints a one-time migration hint so you can avoid flap-loop issues.
