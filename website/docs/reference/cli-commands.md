...
```bash
moor
moor chat -q "Summarize the latest PRs"
...
Same agent, same tools, same skills — just strips every interactive / cosmetic layer. If you need tool output in the transcript too, use `moor chat -q` instead; `-z` is explicitly for "I only want the final answer".
...
```bash
moor cron <list|create|edit|pause|resume|run|remove|status|tick>
```
