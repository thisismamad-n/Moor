...
of fields, and Moor schedules it as a cron job — no cron syntax required.
...
- **CLI, TUI, and messengers** — type `/blueprint <name>` (e.g.
  `/blueprint morning-brief`) and Moor asks you for what it needs, one
...
A blueprint is just a skill with a `metadata.moor.blueprint` block in its
