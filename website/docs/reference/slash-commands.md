...
Moor has two slash-command surfaces, both driven by a central `COMMAND_REGISTRY` in `moor_cli/commands.py`:
...
| `/goal <text>` | Set a standing goal Moor works toward across turns — our take on the Ralph loop. After each turn an auxiliary judge model decides whether the goal is done; if not, Moor auto-continues. Subcommands: `/goal status`, `/goal pause`, `/goal resume`, `/goal clear`. Budget defaults to 20 turns (`goals.max_turns`); any real user message preempts the continuation loop, and state survives `/resume`. See [Persistent Goals](../user-guide/features/goals.md) for the full walkthrough. |
...
| `/export [profile] [-o out.tar.gz]` | **CLI only.** Pack a profile into a shareable `.tar.gz` — skills, memory, persona, crons, plugins, settings, and (from the desktop) themes and layout. Credentials (`auth.json`, `.env`) are stripped. Defaults to the active profile and `<name>.tar.gz` in the current directory. Same archive as `moor profile export`; for a versioned, updatable share use a [profile distribution](../user-guide/profile-distributions.md) instead. |
...
| `/busy [queue\|steer\|interrupt\|status]` | Control what happens when you message while Moor is working — queue the new message, steer mid-turn, or interrupt immediately. Works in the CLI and messaging gateway. |
...
| `/bundles` | List configured skill bundles — `/<name>` slash aliases that preload several skills at once. Configure under `bundles:` in `~/.moor/config.yaml`. See [Skill Bundles](../user-guide/features/skills.md#skill-bundles). |
...
| `/plan [task]` | Write a markdown implementation plan to `.moor/plans/` in the active workspace — planning only, no execution. Empty argument infers the task from the conversation. (Formerly the bundled `plan` skill; now built-in so it survives the Telegram/Discord command-menu caps.) |
...
| `/reload-skills` (alias: `/reload_skills`) | Re-scan `~/.moor/skills/` for newly installed or removed skills |
...
| `/hatch <description>` (alias: `/generate-pet`) | Generate a brand-new petdex pet from a text description, using the configured image backend (OpenRouter / Moor Portal). See [Pets](../user-guide/features/pets.md). |
...
| `/topup` | Show your Moor balance and manage billing on the portal (replaces the old `/credits` and `/billing` commands). |
| `/subscription` (alias: `/upgrade`) | **CLI only.** View your Moor plan and change it in the browser. |
| `/login` | Sign in with a Moor account. Runs off-turn: the consent link and code arrive in the session, and the sign-in settles when you approve it in the browser. |
...
User-defined quick commands map a short slash command to either a shell command or another slash command. Configure them in `~/.moor/config.yaml`:
...
Define your own short names for models you use often, then reach them with `/model <alias>` in a running session, `moor chat --model <alias>` at startup, or any messaging platform. Aliases work identically in these paths, on session-only (default) and `--global` switches.

Two config formats are supported:

**Full form** — pin an exact model, provider, and optionally a base URL. Put this in `~/.moor/config.yaml`:
...
```bash
moor config set model.aliases.fav anthropic/claude-opus-4.6
moor config set model.aliases.grok x-ai/grok-4
```
...
| `/topup` | Show your Moor balance and manage billing on the portal. |
| `/login` | Sign in with a Moor account. **Paired direct messages only** — in a group, channel, or broadcast-shaped platform Moor refuses. On Slack use `/moor login`. |
| `/whoami` | Show your slash command access level (admin / user). |
...
| `/heartbeat every <interval> <prompt>` (alias: `/hb`) | Set a recurring prompt that re-enters this session when idle. Subcommands: `status`, `pause`, `resume`, `clear`. On Slack use `/moor heartbeat …`. |
...
| `/refine [focus]` | Run the memory/skill self-improvement review now, optionally with focus instructions. On Slack use `/moor refine …`. |
| `/review [instructions]` | Spawn an independent reviewer subagent for the work just discussed (PR, code, docs); its review re-enters this chat when done. On Slack use `/moor review …`. |
...
| `/plan [task]` | Write a markdown implementation plan to `.moor/plans/`; no execution. |
...
| `/skills [pending\|approve\|reject\|diff\|approval]` | Review pending **skill** writes staged by the write-approval gate (`skills.write_approval`). Shows a one-line gist per staged write; `/skills diff <id>` is truncated for chat — read the full diff on the CLI or in `~/.moor/pending/skills/<id>.json`. Only appears when the gate is on (or staged writes remain); search/install stay CLI-only. |
...
- In the TUI, `/sessions` shows live sessions in the current TUI process. Use `/resume [name]` or `moor --tui --resume <id-or-title>` for saved or closed transcripts.
...
**Inline skip:** append `now`, `--yes`, or `-y` to bypass the modal for a single invocation — e.g. `/reset now`, `/new --yes my-session`, `/clear -y`, `/undo -y`. Useful when the modal doesn't render correctly on your terminal (see [issue #30768](https://github.com/thisismamad-n/Moor/issues/30768) for native Windows PowerShell) or when scripting against the CLI.

Set `approvals.destructive_slash_confirm: false` in `~/.moor/config.yaml` to disable the prompts globally; set it back to `true` to re-enable. See [Security — Destructive slash command confirmation](../user-guide/security.md#dangerous-command-approval) for context.
