...
- **[Moor Portal](/integrations/moor-portal)** — Moor inc.'s subscription gateway — 300+ models plus web/image/TTS/browser through one OAuth login (recommended for newcomers)
...
**Cause:** Moor requires Python 3.11 or newer.
...
See [Security](/user-guide/security) for Moor' documented execution controls and [Providers](/integrations/providers) for provider configuration.
...
# Set a valid model
moor config set MOOR_MODEL anthropic/claude-opus-4.7
...
cd ~/.moor/moor-agent && uv pip install -e ".[messaging]"  # Telegram, Discord, Slack, and shared gateway deps
...
If the baseline looks high before you've typed anything, that's the fixed prompt budget — the system prompt plus tool schemas sent on every call. Run [`moor prompt-size`](/reference/cli-commands#moor-prompt-size) to measure it, then trim: disable toolsets you don't use (`moor tools`) and uninstall or disable skills you don't need (`moor skills`).
...
cd ~/.moor/moor-agent && uv pip install -e ".[mcp]"
...
There is no hard limit. Each profile is just a directory under `~/.moor/profiles/`. The practical limit depends on your disk space and how many concurrent gateways your system can handle (each gateway is a lightweight Python process). Running dozens of profiles is fine; each idle profile uses no resources.
...
2. On the **source machine**, create a full backup:
   ...
   moor backup
   ```
   This creates a zip of your entire `~/.moor/` directory — config, API keys, memories, skills, sessions, and profiles — saved to your home directory as `~/moor-backup-<timestamp>.zip`.
