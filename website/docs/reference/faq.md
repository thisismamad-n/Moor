...
description: "Frequently asked questions and solutions to common issues with Moor Agent"
---
...
### What LLM providers work with Moor?

Moor Agent works with any OpenAI-compatible API. Supported providers include:

- **[OpenRouter](https://openrouter.ai/)** — access hundreds of models through one API key (recommended for flexibility)
- **[Moor Portal](../integrations/moor-portal.md)** — Moor inc.'s subscription gateway — 300+ models plus web/image/TTS/browser through one OAuth login (recommended for newcomers)
- **OpenAI** — GPT-5.4, GPT-5-codex, GPT-4.1, GPT-4o, etc.
- **Anthropic** — Claude models (direct API, OAuth via `moor auth add anthropic`, OpenRouter, or any compatible proxy)
...
Set your provider with `moor model` or by editing `~/.moor/.env`. See the [Environment Variables](./environment-variables.md) reference for all provider keys.
...
- let Moor use the resulting MCP browser tools

This is more reliable than trying to force Moor core browser transport to attach directly across the WSL2/Windows boundary.

See:

- [Use MCP with Moor](../guides/use-mcp-with-moor.md#wsl2-bridge-moor-in-wsl-to-windows-chrome)
- [Browser Automation](../user-guide/features/browser.md#wsl2--windows-chrome-prefer-mcp-over-browser-connect)

### Is my data sent anywhere?

API calls go **only to the LLM provider you configure** (e.g., OpenRouter, your local Ollama instance). Moor Agent does not collect telemetry, usage data, or analytics. Your conversations, memory, and skills are stored locally in `~/.moor/`.

### Can I use it offline / with local models?

Yes. Run `moor model`, select **Custom endpoint**, and enter your server's URL:
...
# Context length: 64000   ← Moor minimum; set this to match your server's actual context window
```
...
Moor persists the endpoint, provider, and base URL in `config.yaml` so it survives restarts. If your local server has exactly one model loaded, `/model custom` auto-detects it. You can also set `provider: custom` in config.yaml — it's a first-class provider, not an alias for anything else.
...
:::tip Ollama users
If you set a custom `num_ctx` in Ollama (e.g., `ollama run --num_ctx 64000`), make sure to set the matching context length in Moor — Ollama's `/api/show` reports the model's *maximum* context, not the effective `num_ctx` you configured.
:::

:::tip Timeouts with local models
Moor auto-detects local endpoints and relaxes streaming timeouts (read timeout raised from 120s to 1800s, stale stream detection disabled). If you still hit timeouts on very large contexts, set `MOOR_STREAM_READ_TIMEOUT=1800` in your `.env`. See the [Local LLM guide](../guides/local-llm-on-mac.md#timeouts) for details.
:::

### How much does it cost?

Moor Agent itself is **free and open-source** (MIT license). You pay only for the LLM API usage from your chosen provider. Local models are completely free to run.

### Can multiple people use one instance?

Yes. The [messaging gateway](../user-guide/messaging/index.md) lets multiple users interact with the same Moor Agent instance via Telegram, Discord, Slack, WhatsApp, or Home Assistant. Access is controlled through allowlists (specific user IDs) and DM pairing (first user to message claims access).
...
Yes. Import the `AIAgent` class and use Moor programmatically:
...
#### `moor: command not found` after installation
...
For a managed-install error, run `moor doctor` and use that installation's
[update method](../getting-started/updating.md).

#### Terminal commands say `node: command not found` (or `nvm`, `pyenv`, `asdf`, …)

**Cause:** Moor builds a per-session environment snapshot by running `bash -l` once at startup. A bash login shell reads `/etc/profile`, `~/.bash_profile`, and `~/.profile`, but **does not source `~/.bashrc`** — so tools that install themselves there (`nvm`, `asdf`, `pyenv`, `cargo`, custom `PATH` exports) stay invisible to the snapshot. This most commonly happens when Moor runs under systemd or in a minimal shell where nothing has pre-loaded the interactive shell profile.

**Solution:** Moor auto-sources `~/.bashrc` by default. If that's not enough — e.g. you're a zsh user whose PATH lives in `~/.zshrc`, or you init `nvm` from a standalone file — list the extra files to source in `~/.moor/config.yaml`:
...
#### The agent says "Moor policy" or "Moor guardrails" refused my request

A model cannot reliably identify why it refused a request. If the refusal appears only in the assistant's prose, its claim that a hidden Moor runtime policy caused it may be a hallucinated explanation or a restriction applied by the selected model or provider.

Moor enforcement is explicit: a blocked tool action returns a tool error naming the denied command or path, and an approval-required action shows an approval prompt. Moor does not silently turn those execution controls into a general content-refusal layer. Provider-level controls can still apply when configured, such as Amazon Bedrock Guardrails.

To isolate the source:

1. Run `/status` to confirm the active model and provider.
2. Check whether the refusal includes an actual Moor tool error or approval prompt. If it is prose only, do not treat the model's attribution as runtime evidence.
3. Retry in a fresh session with another configured model or provider. A refusal that changes with the model is model/provider behavior, not a Moor execution control.
4. If an explicit tool error appears, use its exact text when reporting the problem.

See [Security](../user-guide/security.md) for Moor' documented execution controls and [Providers](../integrations/providers.md) for provider configuration.

#### "…refused this request because of a policy on your account"

**Meaning:** the provider rejected the request for an account-level reason that retrying cannot change — an aggregator's data/privacy settings excluded every endpoint for the model, or the model's upstream provider has blocked the account (for example `this user has been blocked for a previous policy violation`, which OpenRouter can relay inside an otherwise successful HTTP 200 stream). Moor sends the request once, does not retry it or rotate credentials, and moves to your fallback chain if one is configured.

**Solution:** check the account's status and data/privacy settings with the provider named in the reply, or switch to another model or provider with `/model`. `moor fallback add` routes future blocks to a backup automatically.
...
| "The AI model service isn't reachable right now — the configured model endpoint is **not running or is unreachable**." | Nothing accepted the connection (`Connection refused`, no route to host, DNS failure). | Start the model server / check `base_url`, then `/retry`; `moor doctor` on the host. |
| "Moor **could not reach** the AI model service (no further detail from the SDK)." | The SDK reported a generic `APIConnectionError` and kept no cause; neither of the above is certain. | `/retry`; `moor doctor` if it persists. The raw exception is in `moor logs`. |
...
**Solution:** Exit your session and use `moor model` from your terminal to add new providers:

```bash
# Exit the Moor chat session first (Ctrl+C or /quit)
...
After adding a new provider via `moor model`, start a new chat session — `/model` will now show all your configured providers.

:::tip Quick reference
| Want to... | Use |
|-----------|-----|
| Add a new provider | `moor model` (from terminal) |
| Enter/change API keys | `moor model` (from terminal) |
...
# Or set directly
moor config set OPENROUTER_API_KEY sk-or-v1-xxxxxxxxxxxx
```

:::warning
Make sure the key matches the provider. An OpenAI key won't work with OpenRouter and vice versa. Check `~/.moor/.env` for conflicting entries.
:::
...
- Using `moor chat --provider <alternative>` to route to a different backend

#### Context length exceeded

**Cause:** The conversation has grown too long for the model's context window, or Moor detected the wrong context length for your model.
...
moor chat --model openrouter/google/gemini-3-flash-preview
```

If this happens on the first long conversation, Moor may have the wrong context length for your model. Check what it detected:
...
**"The model server rejected this request as too large, but this conversation is only about N tokens…":** a local server (localhost, LAN, Tailscale) said "context exceeded" without quoting any measurement, while Moor's own estimate of the request is far below the window it knows for the model — so it does **not** compress or blame the conversation, and the turn stays retryable. On single-slot local servers (LM Studio, Ollama) this is almost always another request holding the server's context at that moment — typically a background memory review from an earlier session (`thread=bg-review` in `logs/agent.log`). Wait a moment and `/retry`. If it recurs with no other Moor process running, the server is loading the model with a smaller window than Moor assumes: raise the server's context setting or lower `model.context_length` to match it. Hosted providers never get this message: they have no shared slot to wait out, so the same rejection there means the route's real window is smaller than Moor assumes, and Moor compresses and retries instead.
...
**Cause:** Moor detected a potentially destructive command (e.g., `rm -rf`, `DROP TABLE`). This is a safety feature.
...
:::tip
This is working as intended — Moor never silently runs destructive commands. The approval prompt shows you exactly what will execute.
:::
...
- Or switch to the terminal interface for administrative tasks: `moor chat`
...
cat ~/.moor/logs/gateway.log | tail -50
```
...
**Solution:**
- Verify your bot token is valid with `moor gateway setup`
- Check gateway logs: `cat ~/.moor/logs/gateway.log | tail -50`
...
Configure in `~/.moor/config.yaml` under your gateway's settings. See the [Messaging docs](../user-guide/messaging/index.md).
...
cd ~/.moor/moor-agent && python -c "import pm; pm.sync_venv(['messaging'], explicit=True)"  # Telegram, Discord, Slack, and shared gateway deps
...
#### WSL: Gateway keeps disconnecting or `moor gateway start` fails
...
tmux new -s moor 'moor gateway run'
# Reattach later: tmux attach -t moor

# Option 3: Background via nohup
nohup moor gateway run > ~/.moor/logs/gateway.log 2>&1 &
```
...
1. Create a task that runs `wsl -d Ubuntu -- bash -lc 'moor gateway run'`
...
**Solution:** The gateway captures your shell PATH when you run `moor gateway install`. If you installed tools after setting up the gateway, re-run the install to capture the updated PATH:

```bash
moor gateway install    # Re-snapshots your current PATH
moor gateway start      # Detects the updated plist and reloads
```

You can verify the plist has the correct PATH:
```bash
/usr/libexec/PlistBuddy -c "Print :EnvironmentVariables:PATH" \
  ~/Library/LaunchAgents/ai.moor.gateway.plist
```
...
**Solution:**
- Try a faster/smaller model: `moor chat --model openrouter/meta-llama/llama-3.1-8b-instruct`
- Reduce active toolsets: `moor chat -t "terminal"`
...
If the baseline looks high before you've typed anything, that's the fixed prompt budget — the system prompt plus tool schemas sent on every call. Run [`moor prompt-size`](./cli-commands.md#moor-prompt-size) to measure it, then trim: disable toolsets you don't use (`moor tools`) and uninstall or disable skills you don't need (`moor skills`).
...
Verify your `~/.moor/config.yaml` MCP configuration:
```yaml
mcp_servers:
  filesystem:
    command: "npx"
...
moor config show | grep -A 12 mcp_servers

# Restart Moor or reload MCP after config changes
moor chat
```
...
:::warning
If an MCP server crashes mid-request, Moor will report a timeout. Check the server's own logs (not just Moor logs) to diagnose the root cause.
:::
...
No. Each profile has its own memory store, session database, and skills directory. They are completely isolated. If you want to start a new profile with existing memories and sessions, use `moor profile create newname --clone-all` to copy everything from the current profile, or add `--clone-from <profile>` to copy from a specific source profile.

This isolation is also the reason to never run two agents against the *same* profile or Moor home: both write memory automatically and each loads the other's writes at session start, so their stored state degrades with every session. One agent per profile; for genuinely shared memory across agents, use an [external memory provider](../user-guide/features/memory-providers.md).

### What happens when I run `moor update`?

`moor update` pulls the latest code and reinstalls dependencies **once** (not per-profile). It then syncs updated skills to all profiles automatically. You only need to run `moor update` once — it covers every profile on the machine.


### How many profiles can I run?

There is no hard limit. Each profile is a directory under `~/.moor/profiles/` that carries at least one identity file (`config.yaml`, `.env`, `SOUL.md`, `profile.yaml`, `auth.json` or `state.db`); a bare directory without one (a leftover from a log rotation or cron tick) is not a profile — it is not listed or served, `-p <name>` reports it as missing, and `moor profile create <name>` refuses to overwrite it until you move or remove it. The practical limit depends on your disk space and how many concurrent gateways your system can handle (each gateway is a lightweight Python process). Running dozens of profiles is fine; each idle profile uses no resources.
...
**Solution: Delegation config.** Moor can route subagents to a different model automatically. Set this in `~/.moor/config.yaml`:
...
Now when you tell Moor "write me a Twitter thread about X" and it spawns a `delegate_task` subagent, that subagent runs on Gemini instead of your main model. Your primary conversation stays on GPT-5.4.
...
**Scenario:** In OpenClaw, you had multiple independent agents bound to specific WhatsApp chats — one for a family shopping list group, another for your private chat. Can Moor do this?

**Current limitation:** Moor profiles each require their own WhatsApp number/session. You cannot bind multiple profiles to different chats on the same WhatsApp number — the WhatsApp bridge (Baileys) uses one authenticated session per number.
...
**Scenario:** You see gateway exec logs, Moor reasoning, and tool call details in Telegram instead of just the final output.
...
**Scenario:** Telegram has a 100 slash command limit, and your skills are pushing past it. You want to disable skills you don't need on Telegram, but `moor skills config` settings don't seem to take effect.

**Solution:** Use `moor skills config` to disable skills per-platform. This writes to `config.yaml`:
...
After changing this, **restart the gateway** (`moor gateway restart` or kill and relaunch). The Telegram bot command menu rebuilds on startup.
...
**Current behavior:** Moor creates sessions keyed by user ID on most platforms, so each person gets their own conversation context. This is by design for privacy and context isolation.
...
### Exporting Moor to another machine
...
2. On the **source machine**, create a full backup:
   ...
   moor backup
   ```
   This saves a zip archive at `~/moor-backup-<timestamp>.zip`.
   The full backup covers configuration, credentials, memories, skills, sessions,
   and profiles under the Moor data root. It is not an application or runtime image.

3. Copy the zip to the new machine and import it:
   ```bash
   # On the source machine
   scp ~/moor-backup-<timestamp>.zip newmachine:~/

   # On the new machine
   moor import ~/moor-backup-<timestamp>.zip
   ```

4. On the new machine, run `moor setup` to verify API keys and provider config are working.
...
```bash
# On the source machine
moor profile export work ./work-backup.tar.gz

# Copy the file to the target machine, then:
moor profile import ./work-backup.tar.gz work
```
...
### `moor backup` vs `moor profile export` {#moor-backup-vs-moor-profile-export}

| Feature | `moor backup` | `moor profile export` |
| :--- | :--- | :--- |
| **Use Case** | **Full machine migration** | **Porting/sharing a specific profile** |
| **Scope** | Moor data root, with the exclusions listed below | Single profile directory |
...
`moor backup --quick` saves selected state files instead of a full archive.
...
```bash
rsync -av --exclude='moor-agent' ~/.moor/ newmachine:~/.moor/
```

:::tip
`moor backup` produces a consistent snapshot even while Moor is actively running. The restored archive excludes machine-local runtime files like `gateway.pid` and `cron.pid`.
:::
...
moor config show | head -20

# Re-run model selection
moor model

# Or test with a known-good model
moor chat -q "hello" --model anthropic/claude-opus-4.7
```
...
1. **Search existing issues:** [GitHub Issues](https://github.com/thisismamad-n/Moor/issues)
