...
description: "Find, install, use, and create skills — on-demand knowledge that teaches Moor new workflows"
---

# Working with Skills

Skills are on-demand knowledge documents that teach Moor how to handle specific tasks — from generating ASCII art to managing GitHub PRs. This guide walks you through using them day to day.
...
Every Moor installation ships with bundled skills. See what's available:
...
You can also trigger skills through natural conversation — ask Moor to use a specific skill, and it will load it via the `skill_view` tool.
...
Official optional skills ship with Moor but aren't active by default. Install them explicitly:
...
For how to ship skills in your own plugin, see [Build a Moor Plugin → Bundle skills](/guides/build-a-hermes-plugin#bundle-skills).
...
When a skill with config is first loaded, Moor prompts you for the values. They're stored in `config.yaml` under `skills.config.*`.
...
moor config show | grep '^skills\.config'
```
