...
description: "Complete guide to migrating your OpenClaw / Clawdbot setup to Moor Agent — what gets migrated, how config maps, and what to check after."
---
...
:::note
Coming from **Claude Code** or **OpenAI Codex CLI** instead? Use [`moor import-agent`](../user-guide/import-from-other-agents.md).
:::

:::tip
If your OpenClaw setup was multi-provider, `moor setup --portal` collapses it to one OAuth — 300+ models plus the Tool Gateway in a single login. See [Moor Portal](/integrations/moor-portal).
:::
...
| `--no-backup` | Skip the pre-migration zip snapshot of `~/.moor/` (by default a single restore-point archive is written before apply, under `~/.moor/backups/pre-migration-*.zip`; restorable with `moor import`). |
...
| What | OpenClaw source | Moor destination | Notes |
|------|----------------|-------------------|-------|
| Persona | `workspace/SOUL.md` | `~/.moor/SOUL.md` | Direct copy |
| Workspace instructions | `workspace/AGENTS.md` | `AGENTS.md` in `--workspace-target` | Requires `--workspace-target` flag |
| Long-term memory | `workspace/MEMORY.md` | `~/.moor/memories/MEMORY.md` | Parsed into entries, merged with existing, deduped. Uses `§` delimiter. |
| User profile | `workspace/USER.md` | `~/.moor/memories/USER.md` | Same entry-merge logic as memory. |
| Daily memory files | `workspace/memory/*.md` | `~/.moor/memories/MEMORY.md` | All daily files merged into main memory. |
...
| Source | OpenClaw location | Moor destination |
|--------|------------------|-------------------|
| Workspace skills | `workspace/skills/` | `~/.moor/skills/openclaw-imports/` |
| Managed/shared skills | `~/.openclaw/skills/` | `~/.moor/skills/openclaw-imports/` |
| Personal cross-project | `~/.agents/skills/` | `~/.moor/skills/openclaw-imports/` |
| Project-level shared | `workspace/.agents/skills/` | `~/.moor/skills/openclaw-imports/` |

Skill conflicts are handled by `--skill-conflict`: `skip` leaves the existing Moor skill, `overwrite` replaces it, `rename` creates a `-imported` copy.

### Model and provider configuration

| What | OpenClaw config path | Moor destination | Notes |
|------|---------------------|-------------------|-------|
| Default model | `agents.defaults.model` | `config.yaml` → `model` | Can be a string or `{primary, fallbacks}` object |
| Custom providers | `models.providers.*` | `config.yaml` → `custom_providers` (auto-migrated to the canonical `providers:` dict on the next `moor update` config migration) | Maps `baseUrl`, `apiType`/`api` — handles both short ("openai", "anthropic") and hyphenated ("openai-completions", "anthropic-messages", "google-generative-ai") values |
| Provider API keys | `models.providers.*.apiKey` | `~/.moor/.env` | Requires `--migrate-secrets`. See [API key resolution](#api-key-resolution) below. |

### Agent behavior

| What | OpenClaw config path | Moor config path | Mapping |
|------|---------------------|-------------------|---------|
...
| OpenClaw config path | Moor config path | Notes |
|---------------------|-------------------|-------|
...
| OpenClaw field | Moor field | Notes |
|----------------|-------------|-------|
...
| TTS assets | `~/.moor/tts/` (file copy) |

### Messaging platforms

| Platform | OpenClaw config path | Moor `.env` variable | Notes |
|----------|---------------------|----------------------|-------|
...
| What | OpenClaw path | Moor path | Notes |
|------|-------------|-------------|-------|
...
| Gateway auth token | `gateway.auth.token` | `.env` → `MOOR_GATEWAY_TOKEN` | Requires `--migrate-secrets` |
...
### Archived (no direct Moor equivalent)

These are saved to `~/.moor/migration/openclaw/<timestamp>/archive/` for manual review:

| What | Archive file | How to recreate in Moor |
|------|-------------|--------------------------|
...
2. **Review archived files** — anything in `~/.moor/migration/openclaw/<timestamp>/archive/` needs manual attention.

3. **Start a new session** — imported skills and memory entries take effect in new sessions, not the current one.

4. **Verify API keys** — run `moor status` to check provider authentication.

5. **Test messaging** — if you migrated platform tokens, restart the gateway: `systemctl --user restart moor-gateway`
...
Imported skills land in `~/.moor/skills/openclaw-imports/`. Start a new session for them to take effect, or run `/skills` to verify they're loaded.
