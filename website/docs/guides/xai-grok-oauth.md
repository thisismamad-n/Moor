...
On servers, containers, or SSH sessions where no browser is available, Moor detects the remote environment and prints the authorization URL instead of opening a browser.
...
If the consent page renders the authorization code directly on the page (xAI's current behavior on browser-based consoles) instead of redirecting to your `127.0.0.1:56121/callback`, paste **just the bare code value** at the `Callback URL:` prompt — Moor accepts the full URL, a bare `?code=...&state=...` query fragment, or a bare code interchangeably.
...
Moor detected that the `state` value returned by the authorization server doesn't match what it sent.
...
On SSH or container sessions Moor prints the authorization URL instead of opening a browser. The loopback callback listener still binds `127.0.0.1:56121` on the remote host — your laptop's browser can't reach it without an SSH local-forward:
...
- [OAuth over SSH / Remote Hosts](./oauth-over-ssh.md) — required reading if Moor is on a different machine than your browser
