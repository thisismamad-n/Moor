...
description: "Sign in with your SuperGrok or X Premium+ subscription to use Grok models in Moor Agent — no API key required"
---

# xAI Grok OAuth (SuperGrok / X Premium+)

Moor Agent supports xAI Grok through a browser-based OAuth device-code login flow against [accounts.x.ai](https://accounts.x.ai), using either a **SuperGrok subscription** ([grok.com](https://x.ai/grok)) or an **X Premium+ subscription** (linked X account). No `XAI_API_KEY` is required — log in once and Moor automatically refreshes your session in the background.
...
The same OAuth bearer token is also reused by every direct-to-xAI surface in Moor — TTS, image generation, video generation, and transcription — so a single login covers all four.
...
# → Moor opens or prints an accounts.x.ai verification URL
...
After the first login, credentials are stored under `~/.moor/auth.json` and refreshed automatically before they expire.
...
On servers, containers, browser-only consoles (Cloud Shell, Codespaces, EC2 Instance Connect), or SSH sessions where Moor cannot open a browser locally, Moor prints the xAI verification URL and user code. Open the URL in any browser on your laptop or in the cloud console, enter the code if prompted, and Moor will keep polling until xAI approves the login. No SSH tunnel or local callback listener is required.
...
The same device-code flow applies when you sign in from the web dashboard or the desktop app: Moor shows the verification URL and user code, then polls in the background until you approve access.

## How the Login Works

1. Moor requests a device code from `auth.x.ai`.
2. You open the verification URL, sign in, enter the displayed code if prompted, and approve access.
3. Moor polls xAI until approval, then saves tokens to `~/.moor/auth.json`.
4. From then on, Moor refreshes the access token in the background — you stay signed in until you `moor auth logout xai-oauth` or revoke access from your xAI account settings.
...
```bash
moor config set model.default grok-4.6
moor config set model.provider xai-oauth
```

## Configuration Reference

After login, `~/.moor/config.yaml` will contain:
...
```bash
moor --provider xai-oauth        # canonical
moor --provider grok-oauth       # alias
moor --provider x-ai-oauth       # alias
moor --provider xai-grok-oauth   # alias
```
...
Moor refreshes the token before each session and again reactively on a 401. If refresh fails with `invalid_grant` (the refresh token was revoked, or the account was rotated), Moor surfaces a typed re-auth message instead of crashing.
...
Device-code approval has a finite expiry window (xAI sets `expires_in` on the device-code response, typically on the order of tens of minutes). If you do not approve the login in time, Moor raises a timeout error.

**Fix:** re-run `moor auth add xai-oauth` (or `moor model`). The flow starts fresh.

### Logging in from a remote server

On SSH or container sessions Moor prints the verification URL and user code instead of opening a browser. Open that URL in a browser on your laptop or in a cloud console — no SSH port forward is needed for xAI Grok OAuth.
...
This is **not** a stale-token problem — re-running `moor model` won't change it. xAI's backend has been seen to restrict OAuth API access to specific SuperGrok tiers despite the in-app subscription being active (issue [#26847](https://github.com/thisismamad-n/Moor/issues/26847)).
