...
sidebar_position: 16
title: "Google Gemini"
description: "Use Moor Agent with Google Gemini — native AI Studio API, API-key setup, tool calling, streaming, and quota guidance"
---
...
doesn't end with an API version segment (`v1beta`, `v1alpha`, `v1`, ...), Moor
...
prefix no longer identifies its surface. Moor never reroutes by key shape: the
...
express-mode key, and Moor completes it to the `publishers/google` form. Each
surface only accepts its own keys — a `403 PERMISSION_DENIED` usually means the
key/host pairing is crossed, and Moor appends guidance naming the other surface.
...
Earlier Moor releases detected the `AQ.` prefix and rerouted such keys to
...
`moor doctor`, TTS — now goes to the AI Studio host and a Vertex express key
gets `403 PERMISSION_DENIED` there. Add `GEMINI_BASE_URL=https://aiplatform.googleapis.com`
to `~/.moor/.env` (or set `base_url` on the provider) once and restart.
:::
...
Google publishes moving aliases for the Pro and Flash Gemini families. `gemini-pro-latest` and `gemini-flash-latest` are useful when you want Google to advance the model automatically without changing your Moor config. Note that your usage charges may be affected if newer models introduce different rates.
...
Moor could not find a usable API key. Add one of these to `~/.moor/.env`:
...
Moor may hide low-throughput Gemma models from the picker by default. If you intentionally want to evaluate one, set the model ID directly in `~/.moor/config.yaml`.
