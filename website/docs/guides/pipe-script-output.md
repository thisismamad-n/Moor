...
description: "Send text from any shell script, cron job, CI hook, or monitoring daemon to Telegram, Discord, Slack, Signal, and other platforms using `moor send`."
---

# Pipe Script Output to Messaging Platforms

`moor send` is a small, scriptable CLI that pushes a message to any
messaging platform Moor is already configured for. Think of it as a
...
- Piping any tool's output anywhere (`make | moor send --to slack:#builds`)

The command reuses the same credentials and platform adapters that `moor
...
moor send --to telegram:-1001234567890:17585 "threaded reply"

# List every configured target
moor send --list

# Filter by platform
moor send --list telegram
```
...
Any platform Moor ships adapters for works as a target:
...
`moor send` resolves the message body in this order:

1. **Positional argument** — `moor send --to telegram "hi"`
2. **`--file PATH`** — `moor send --to telegram --file msg.txt`
3. **Piped stdin** — `echo hi | moor send --to telegram`

When stdin is a TTY (no pipe), Moor does **not** wait for input — you'll
...
if [ "$ram_pct" -ge 85 ]; then
  moor send --to telegram --subject "⚠ MEMORY WARNING" \
    "RAM ${ram_pct}% on $(hostname)"
fi
```

Because `moor send` reuses your Moor config, the same script works on
any host where Moor is installed — no need to export bot tokens into
...
`moor send`. If the Python interpreter can't load because the box is
...
if ./scripts/deploy.sh; then
  moor send --to slack:#deploys "✅ ${CI_COMMIT_SHA:0:7} deployed"
else
  tail -n 100 deploy.log | moor send \
...
```bash
./train.py --epochs 200 && \
  moor send --to telegram "training done" || \
  moor send --to telegram "training failed (exit $?)"
```
...
moor send --to telegram --quiet "keepalive" || {
  echo "Telegram delivery failed" >&2
  exit 1
}

# Capture the message ID for later editing / threading
msg_id=$(moor send --to discord:#ops --json "build started" \
  | jq -r .message_id)
```

---

## Does `moor send` Need the Gateway Running?
...
`~/.moor/.env` and `~/.moor/config.yaml`. It's a standalone subprocess
...
pointing at the gateway; start it with `moor gateway start` and retry.
...
The listing is built from `~/.moor/channel_directory.json`, which the
gateway refreshes every few minutes while it's running. If you see
"no channels discovered yet", start the gateway once (`moor gateway
...
| Approach | Multi-platform | Reuses Moor creds | Needs gateway | Best for |
|----------|----------------|---------------------|---------------|----------|
| `moor send` | ✅ | ✅ | No (bot-token) | Everything below |
...
`moor send` is intentionally the simplest possible surface. If you need
...
If you just need to pipe a raw string, reach for `moor send`.
