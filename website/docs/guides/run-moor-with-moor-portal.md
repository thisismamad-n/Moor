---
sidebar_position: 1
title: "Run Moor Agent with Moor Portal"
...
3. Sets `model.provider: moor` in `~/.moor/config.yaml`
...
OAuth needs a browser, but the loopback callback runs on the machine where Moor is running. Two options:

```bash
# Option A: SSH port forwarding (preferred)
ssh -N -L 8642:127.0.0.1:8642 user@remote-host    # in a local terminal
moor setup --portal                              # on the remote, open the printed URL in your local browser

# Option B: device-code login (works from Cloud Shell, Codespaces, EC2 Instance Connect)
moor auth add moor --type oauth
...
If any line shows something other than "via Moor Portal" or the auth line says "not logged in", jump to [Troubleshooting](#troubleshooting) below.
...
`moor setup --portal` lets you pick a model during setup, but the whole point of the subscription is access to the full catalog — switch any time with `/model` mid-session:
...
moor config set model.default anthropic/claude-sonnet-4.6
```
...
The gateway is opt-in per tool, not all-or-nothing. If you already have a Browserbase account and want to keep using it while routing web search and image generation through Moor, that's supported:

```bash
moor tools
# → Web search       → "Moor Subscription"     (recommended)
# → Image generation → "Moor Subscription"     (recommended)
# → Browser          → "Browserbase"           (your existing key)
# → TTS              → "Moor Subscription"     (recommended)
```

These rows appear in `moor tools` even before you've logged into Moor Portal — if you pick "Moor Subscription" without an active session, Moor runs the Portal login inline (without changing your inference provider or your other tools).
...
You'll see per-tool routing — `via Moor Portal` for the ones routed through the subscription, and the partner name (`browserbase`, `firecrawl`, etc.) for the ones using your own keys.
...
```bash
moor setup tts
# → pick "Moor Subscription" for TTS
# → pick a speech-to-text backend (local faster-whisper is free, no setup)
```

Then in any messaging-platform session (Telegram, Discord, Signal, etc.), send a voice message and Moor will transcribe it, respond, and reply with synthesized voice — all on your Portal subscription.
...
### `moor portal info` shows "not logged in" after `moor setup --portal`
...
```bash
moor config set model.provider moor
```
...
Re-verify with `moor portal info`.

### Tool Gateway tools showing partner names instead of "via Moor Portal"

Per-tool config is overriding the gateway. Run:

```bash
moor tools
# pick "Moor Subscription" for any tool you want gateway-routed
```

Some users intentionally mix — e.g. routing web through Moor but using their own Browserbase key for browser. If that's intentional, leave it alone. If not, this command fixes it.

### "Re-authentication required" mid-session

Your Portal refresh token was invalidated (password change, manual revoke, session expiry). The token is now quarantined locally so Moor doesn't replay it endlessly. Just log in again:
...
`moor portal info` will tell you whether you're actually routing through the Portal or some other provider. Common causes:

- `model.provider` set to `openrouter`/`anthropic`/etc. instead of `moor`
- An OAuth refresh failure that fell back to a different configured provider
- Multiple Moor profiles where you're using the wrong one (check `moor profile list`)

### Want to revoke and start clean

```bash
moor auth logout moor       # wipes the local refresh token
