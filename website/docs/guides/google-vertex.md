...
- **`google-auth`** — installed automatically the first time you select Vertex (lazy install). Run `moor setup` to repair a managed install if that fails.
...
- The **credential path** is a pointer to a secret and lives in `~/.moor/.env`.
- **Project ID and region** are non-secret routing settings and live in `~/.moor/config.yaml`.
...
Vertex requires the `google/` vendor prefix on model IDs. The `moor model` picker offers:
...
`/model` switches among already-configured providers and models; it does not collect new credentials. Configure Vertex with `moor model` first.
...
Moor found neither a service-account JSON nor working ADC. Either set `VERTEX_CREDENTIALS_PATH` in `~/.moor/.env`, or run `gcloud auth application-default login`. If your project isn't embedded in the credentials, set `vertex.project_id` in `config.yaml`.

### `google-auth` not installed

Moor lazy-installs it the first time you select the Vertex provider. If that fails, run `moor setup` to repair the managed install.
