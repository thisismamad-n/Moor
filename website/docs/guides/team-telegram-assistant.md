...
This tutorial walks you through setting up a Telegram bot powered by Moor Agent that multiple team members can use. By the end, your team will have a shared AI assistant they can message for help with code, research, system administration, and anything else — secured with per-user authorization.
...
- **Moor Agent installed** on a server or VPS (not your laptop — the bot needs to stay running). Follow the [installation guide](/getting-started/installation) if you haven't yet.
...
:::tip
A $5/month VPS is plenty for running the gateway. Moor itself is lightweight — the LLM API calls are what cost money, and those happen remotely.
:::
...
2. **Send `/newbot`** — BotFather will ask you two things:
   - **Display name** — what users see (e.g., `Team Moor Assistant`)
   - **Username** — must end in `bot` (e.g., `myteam_moor_bot`)
...
4. **Set a description** (optional but recommended):
   ...
   ```
   Team AI assistant powered by Moor Agent. DM me for help with code, research, debugging, and more.
   ```
...
# View live logs
journalctl --user -u moor-gateway -f
...
moor pairing revoke telegram 987654321
...
**Option 2:** Set it manually in `~/.moor/.env`:
...
Control how much detail the bot shows when using tools. In `~/.moor/config.yaml`:
...
For a full guide, see [Use SOUL.md with Moor](/guides/use-soul-with-moor).
...
| What | Location |
|------|----------|
| Gateway logs | `journalctl --user -u moor-gateway` (Linux) or `~/.moor/logs/gateway.log` (macOS) |
| Cron job output | `~/.moor/cron/output/{job_id}/{timestamp}.md` |
| Cron job definitions | `~/.moor/cron/jobs.json` |
| Pairing data | `~/.moor/pairing/` |
| Session history | `~/.moor/sessions/` |
