---
sidebar_position: 7
title: "Use SOUL.md with Moor"
description: "How to use SOUL.md to shape Moor Agent's default voice, what belongs there, and how it differs from AGENTS.md and /personality"
---

# Use SOUL.md with Moor

`SOUL.md` is the **primary identity** for your Moor instance. It's the first thing in the system prompt — it defines who the agent is, how it speaks, and what it avoids.

If you want Moor to feel like the same assistant every time you talk to it — or if you want to replace the Moor persona entirely with your own — this is the file to use.
...
- personality
- communication style
- how direct or warm Moor should be
- what Moor should avoid stylistically
- how Moor should relate to uncertainty, disagreement, and ambiguity

In short:
- `SOUL.md` is about who Moor is and how Moor speaks
...
Moor now uses only the global SOUL file for the current instance:

```text
~/.hermes/SOUL.md
```

If you run Moor with a custom home directory, it becomes:
...
Moor automatically seeds a starter `SOUL.md` for you if one does not already exist.

That means most users now begin with a real file they can read and edit immediately.

Important:
- if you already have a `SOUL.md`, Moor does not overwrite it
- if the file exists but is empty, Moor adds nothing from it to the prompt

## How Moor uses it

When Moor starts a session, it reads `SOUL.md` from `HERMES_HOME`, scans it for prompt-injection patterns, truncates it if needed, and uses it as the **agent identity** — slot #1 in the system prompt. This means SOUL.md completely replaces the built-in default identity text.
...
That alone can noticeably change how Moor feels.
...
Moor already tries to be helpful and clear. `SOUL.md` should add real personality and style, not restate obvious defaults.
...
# Style
How Moor should sound.

# Avoid
What Moor should not do.

# Defaults
How Moor should behave when ambiguity appears.
```
...
Then restart Moor or start a new session.
...
### I edited SOUL.md but Moor still sounds the same
...
### Moor is ignoring parts of my SOUL.md
