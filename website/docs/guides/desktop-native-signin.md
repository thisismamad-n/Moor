...
description: "How the Moor Desktop app signs in to a gated gateway using your system browser and PKCE — no embedded webview, no session cookies"
---

# Desktop Native Sign-In (RFC 8252)

When the Moor Desktop app connects to a **gated gateway** (a hosted or
...
For Moor specifically, native sign-in means:
...
```
Desktop app                Gateway (/auth/native/*)          Moor Portal (IDP)
...
desktop app* and an OAuth client *to the upstream identity provider* (Moor
...
dashboard-auth subsystem. OAuth providers (e.g. the bundled **Moor** provider)
...
- [Run Moor with Moor Portal](./run-moor-with-moor-portal.md)
