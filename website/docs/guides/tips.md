...
Run `moor setup --portal` — you get 300+ models including Claude, GPT-5, and Gemini under one subscription. See [Moor Portal](../integrations/moor-portal.md).
:::
...
For a full walkthrough, see [Use SOUL.md with Moor](./use-soul-with-moor.md).
...
:::tip
Run `/usage` periodically to see your token consumption. Run `/insights` for a broader view of usage patterns over the last 30 days. To see what your *fixed* per-message cost is before any conversation — system prompt, skills index, memory, tool schemas — run [`moor prompt-size`](../reference/cli-commands.md#moor-prompt-size) (works offline).
:::
