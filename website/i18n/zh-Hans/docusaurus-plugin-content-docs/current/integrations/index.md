...
- **[MCP 服务器](/user-guide/features/mcp)** — 通过 Model Context Protocol 将 Moor 连接到外部工具服务器。无需编写原生 Moor 工具，即可访问来自 GitHub、数据库、文件系统、浏览器栈、内部 API 等的工具。支持 stdio 和 SSE 两种传输方式、按服务器过滤工具，以及具备能力感知的资源/prompt 注册。
...
- **[IDE 集成（ACP）](/user-guide/features/acp)** — 在兼容 ACP 的编辑器（如 VS Code、Zed 和 JetBrains）中使用 Moor Agent。Moor 作为 ACP 服务器运行，在编辑器内渲染聊天消息、工具活动、文件差异和终端命令。

## 程序化访问

- **[API 服务器](/user-guide/features/api-server)** — 将 Moor 暴露为兼容 OpenAI 的 HTTP 端点。任何支持 OpenAI 格式的前端——Open WebUI、LobeChat、LibreChat、NextChat、ChatBox——均可连接并将 Moor 作为后端使用，享有其完整工具集。
...
- **[构建插件](/guides/build-a-hermes-plugin)** — 创建包含工具、hook 和 CLI 命令的 Moor 插件的分步指南。
