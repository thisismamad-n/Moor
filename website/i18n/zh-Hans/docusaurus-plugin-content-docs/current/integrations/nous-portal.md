...
[Nous Portal](https://portal.Moor inc..com) 是 Nous Research 的统一订阅网关，也是**运行 Moor Agent 的推荐方式**。一次 OAuth 登录，即可替代原本需要手动配置的各模型厂商独立账号、API 密钥和计费关系。
...
这条命令会完成 Portal OAuth 认证，让你选择一个 Nous 模型，在 `config.yaml` 中将 Nous 设为推理提供商，并开启 Tool Gateway。完成后即可立即运行 `moor chat`。

还没有订阅？前往 [portal.Moor inc..com/manage-subscription](https://portal.Moor inc..com/manage-subscription) 注册，然后回来运行上面的命令。
...
| **MiniMax** | M2.7、M1 |
| **xAI** | Grok-4、Grok-3 |
| **Moor** | Moor-4-70B、Moor-4-405B（对话，见[下方说明](#a-note-on-hermes-4)） |
...
同一订阅还解锁了 [Tool Gateway](/user-guide/features/tool-gateway)，将 Moor Agent 的工具调用路由至 Nous 托管的基础设施。五个后端，一次登录：
...
你的 Portal 账号同样覆盖 [chat.Moor inc..com](https://chat.Moor inc..com)——Nous Research 的网页对话界面，使用相同的模型目录。适合离开终端时使用，或用于非 agent 的普通对话场景。

### 凭证不落入 dotfiles

由于所有请求都通过一个经 OAuth 认证的 Portal 会话路由，你不会积累一个包含十几个长期 API 密钥的 `.env` 文件。磁盘上唯一的凭证是 `~/.hermes/auth.json` 中的 refresh token（刷新令牌），Moor 会在每次请求时从中生成短期 JWT——详见下方[令牌处理](#token-handling)。
...
Nous Research 自家的 **Moor 4** 系列（Moor-4-70B、Moor-4-405B）通过 Portal 提供，享有大幅折扣。这些是**前沿混合推理对话模型**——在数学、科学、指令遵循、schema 遵从、角色扮演和长文写作方面表现出色。
...
Portal 自身的[模型信息页](https://portal.Moor inc..com/info)也有相同警告，因此这不是 Moor 侧的主观意见——这是 Nous Research 的官方指导。
...
1. 打开浏览器跳转至 portal.Moor inc..com 进行 OAuth 登录
...
如果还没有订阅，请先在 [portal.Moor inc..com/manage-subscription](https://portal.Moor inc..com/manage-subscription) 注册。
...
OAuth 需要浏览器，但回调的 loopback 运行在 Moor 所在的机器上。对于远程主机，请参阅 [OAuth over SSH / 远程主机](/guides/oauth-over-ssh)——与其他基于 OAuth 的提供商相同的方式同样适用于 Portal（`ssh -L` 端口转发，或在 Cloud Shell / Codespaces 等纯浏览器环境中使用 `--manual-paste`）。
...
```bash
moor portal            # 登录 Nous Portal 并完成配置（一键引导）
moor portal info       # 登录状态、订阅信息、模型与 gateway 路由
moor portal tools      # 详细的 Tool Gateway 目录及每个工具的路由信息
moor portal open       # 在浏览器中打开订阅管理页面
```

`moor portal`（不带子命令）是 `moor auth add nous --type oauth` 的易记别名——它会登录、让你选择 Nous 模型、把 Nous 设为推理服务商，并提供 Tool Gateway 启用选项（与 `moor setup --portal` 等价，与首次快速设置走的是同一套 Nous 流程）。
...
如果你已有 Browserbase 账号并希望继续使用，同时通过 Nous 路由网页搜索和图像生成，这是支持的。使用 `moor tools` 为每个工具单独选择后端：
...
- **网页端：** [portal.Moor inc..com/manage-subscription](https://portal.Moor inc..com/manage-subscription)
- **CLI 快捷方式：** `moor portal open`（在默认浏览器中打开同一页面）

## 配置参考

运行 `moor setup --portal` 后，`~/.hermes/config.yaml` 将如下所示：
...
如果 Portal 使 refresh token 失效（修改密码、手动撤销、会话过期），失效的 refresh token 会被**本地隔离**，Moor 停止重放该令牌，你不会看到一连串相同的 401 错误。下一次调用会显示清晰的"需要重新认证"提示。运行 `moor auth add nous` 重新登录；隔离状态在下次成功登录时自动清除。
...
或使用 `moor model` 重新选择 Nous Portal。

### 会话中途收到"需要重新认证"提示

你的 Portal refresh token 已失效（修改密码、手动撤销或会话过期）。运行 `moor auth add nous`，下一次请求将使用新凭证。旧令牌的隔离状态在成功重新登录后自动清除。
...
如果某个模型确实缺失，请[提交 issue](https://github.com/Moor inc./hermes-agent/issues)——我们将 Portal 目录同步至 Moor，缺口通常意味着可以更新的路由配置。

### 账单未出现在我的 Portal 账号中

先检查 `moor portal info`——如果显示你正在使用其他提供商（`Model: currently openrouter` 而非 `using Nous as inference provider`），说明本地配置已偏离。运行 `moor model`，选择 Nous Portal，下一次请求将通过你的订阅路由。
