---
title: "Openclaw Migration — 将用户的 OpenClaw 自定义配置迁移到 Moor Agent"
sidebar_label: "Openclaw Migration"
description: "将用户的 OpenClaw 自定义配置迁移到 Moor Agent"
---
...
将用户的 OpenClaw 自定义配置迁移到 Moor Agent。从 `~/.openclaw` 导入 Moor 兼容的记忆、`SOUL.md`、命令白名单、用户技能及所选工作区资产，并精确报告无法迁移的内容及原因。
...
当用户希望以最少的手动清理将其 OpenClaw 配置迁移到 Moor Agent 时，使用此 skill。
...
使用 `--migrate-secrets` 时，还将导入一小组 Moor 兼容的白名单 secret，目前包括：
...
Moor CLI 支持 `clarify` 工具进行交互式提示，但有以下限制：
...
13. 如果重命名的已导入 skill 的 `details.backup` 为空，不得暗示现有 Moor skill 已被重命名或备份。仅说明已导入的副本被放置在新目标位置，并将 `details.renamed_from` 作为保持原位的已有文件夹引用。
...
2. 默认不迁移 secret。Token、认证 blob、设备凭据和原始 gateway 配置应保留在 Moor 之外，除非用户明确要求迁移 secret。
3. 除非用户明确要求，否则不得静默覆盖非空的 Moor 目标。辅助脚本在启用覆盖时会保留备份。
4. 始终向用户提供跳过项报告。该报告是迁移的一部分，而非可选附加内容。
5. 优先使用主 OpenClaw 工作区（`~/.openclaw/workspace/`）而非 `workspace.default/`。仅在主文件缺失时才使用默认工作区作为备用。
6. 即使在 secret 迁移模式下，也只迁移具有干净 Moor 目标的 secret。不支持的认证 blob 仍须报告为已跳过。
