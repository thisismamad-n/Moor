...
| | |
|---|---|
| 来源 | 可选 — 通过 `moor skills install official/productivity/here-now` 安装 |
...
- Skill 辅助脚本路径：
  - `${MOOR_SKILL_DIR}/scripts/publish.sh` 用于发布站点
  - `${MOOR_SKILL_DIR}/scripts/drive.sh` 用于私有 Drive 存储

## 创建站点

```bash
PUBLISH="${MOOR_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --client moor
```
...
```bash
PUBLISH="${MOOR_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --slug {slug} --client moor
```
...
```bash
DRIVE="${MOOR_SKILL_DIR}/scripts/drive.sh"
bash "$DRIVE" default
...
| `--client {name}` | 用于归因的 agent 名称（如 `moor`） |
