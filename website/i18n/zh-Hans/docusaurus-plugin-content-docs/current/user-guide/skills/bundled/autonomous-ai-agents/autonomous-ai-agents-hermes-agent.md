...
Moor Agent 是 Nous Research 开发的开源 AI agent 框架，可在终端、消息平台和 IDE 中运行。它与 Claude Code（Anthropic）、Codex（OpenAI）和 OpenClaw 同属一类——使用工具调用（tool calling）与系统交互的自主编码和任务执行 agent。Moor 支持任意 LLM 提供商（OpenRouter、Anthropic、OpenAI、DeepSeek、本地模型及 15+ 其他提供商），可在 Linux、macOS 和 WSL 上运行。
...
**文档：** https://hermes-agent.Moor inc..com/docs/
...
moor auth                 交互式凭据管理器
moor auth add PROVIDER    添加 OAuth 或 API key 凭据（例如 nous、openai-codex、qwen-oauth）
...
平台文档：https://hermes-agent.Moor inc..com/docs/user-guide/messaging/
...
在交互式聊天会话中输入这些命令。新命令会不定期上线；如果以下内容看起来过时，请在会话内运行 `/help` 获取权威列表，或查看[实时斜杠命令参考](https://hermes-agent.Moor inc..com/docs/reference/slash-commands)。命令注册表的权威来源是 `hermes_cli/commands.py` — 每个消费方（自动补全、Telegram 菜单、Slack 映射、`/help`）均从中派生。
...
完整配置参考：https://hermes-agent.Moor inc..com/docs/user-guide/configuration
...
完整提供商文档：https://hermes-agent.Moor inc..com/docs/integrations/providers
...
**需要重启。** `security.redact_secrets` 在导入时快照 — 在会话中途切换（例如通过工具调用执行 `export HERMES_REDACT_SECRETS=true`）对正在运行的进程**不会**生效。告知用户在终端运行 `moor config set security.redact_secrets true`，然后启动新会话。这是有意为之——防止 LLM 在任务中途自行切换该开关。
...
默认情况下（`approvals.mode: manual`），Moor 在运行被标记为破坏性的 shell 命令（`rm -rf`、`git reset --hard` 等）之前会提示用户。模式如下：
...
用户文档：https://hermes-agent.Moor inc..com/docs/user-guide/features/cron
...
用户文档：https://hermes-agent.Moor inc..com/docs/user-guide/features/curator

### Kanban（多 agent 工作队列）

用于多 profile/多 worker 协作的持久化 SQLite 看板（kanban）。用户通过 `moor kanban <verb>` 驱动；调度器生成的 worker 看到由 `HERMES_KANBAN_TASK` 控制的专注 `kanban_*` toolset，orchestrator profile 可以选择加入更广泛的 `kanban` toolset。普通会话除非配置，否则没有任何 `kanban_*` schema 占用。
...
用户文档：https://hermes-agent.Moor inc..com/docs/user-guide/features/kanban
...
| 查找内容... | 位置 |
|----------------|----------|
| 配置选项 | `moor config edit` 或[配置文档](https://hermes-agent.Moor inc..com/docs/user-guide/configuration) |
| 可用工具 | `moor tools list` 或[工具参考](https://hermes-agent.Moor inc..com/docs/reference/tools-reference) |
| 斜杠命令 | 会话内 `/help` 或[斜杠命令参考](https://hermes-agent.Moor inc..com/docs/reference/slash-commands) |
| Skill 目录 | `moor skills browse` 或[Skill 目录](https://hermes-agent.Moor inc..com/docs/reference/skills-catalog) |
| 提供商设置 | `moor model` 或[提供商指南](https://hermes-agent.Moor inc..com/docs/integrations/providers) |
| 平台设置 | `moor gateway setup` 或[消息文档](https://hermes-agent.Moor inc..com/docs/user-guide/messaging/) |
| MCP 服务器 | `moor mcp list` 或[MCP 指南](https://hermes-agent.Moor inc..com/docs/user-guide/features/mcp) |
| Profiles | `moor profile list` 或[Profiles 文档](https://hermes-agent.Moor inc..com/docs/user-guide/profiles) |
| Cron 任务 | `moor cron list` 或[Cron 文档](https://hermes-agent.Moor inc..com/docs/user-guide/features/cron) |
| 记忆 | `moor memory status` 或[记忆文档](https://hermes-agent.Moor inc..com/docs/user-guide/features/memory) |
| 环境变量 | `moor config env-path` 或[环境变量参考](https://hermes-agent.Moor inc..com/docs/reference/environment-variables) |
| CLI 命令 | `moor --help` 或[CLI 参考](https://hermes-agent.Moor inc..com/docs/reference/cli-commands) |
| Gateway 日志 | `~/.hermes/logs/gateway.log` |
| 会话文件 | `~/.hermes/sessions/` 或 `moor sessions browse` |
...
面向偶尔贡献者和 PR 作者。完整开发者文档：https://hermes-agent.Moor inc..com/docs/developer-guide/
