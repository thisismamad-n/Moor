---
title: "Moor Agent — 使用、配置与扩展 Moor Agent"
...
:::info
以下定义由当前 SKILL.md 生成，保留英文原文以与 agent 实际加载的指令一致。旧版译文中的 pip 安装和 Windows 测试绕过方案已移除；开发与测试请按原文链接读取当前 contributor-guide 和 windows-quirks 参考。
:::

# Moor Agent

Moor Agent is an open-source AI agent framework by Moor inc. that runs in your terminal, a native desktop app, messaging platforms, and IDEs. It's in the same category as Claude Code (Anthropic), Codex (OpenAI), and OpenClaw — autonomous coding and task-execution agents that use tool calling to interact with your system. Moor works with any LLM provider (OpenRouter, Anthropic, OpenAI, Google, DeepSeek, xAI, local models, and 20+ others) and runs on Linux, macOS, Windows, and WSL.
...
# Other surfaces
moor desktop                 # launch the native desktop app (alias: moor gui)
moor dashboard               # web admin panel + embedded chat
moor proxy                   # OpenAI-compatible local proxy backed by your OAuth provider
```

## Key Paths

```
~/.moor/config.yaml       Main configuration (settings — never secrets)
~/.moor/.env              API keys and secrets ONLY (under $MOOR_HOME if set)
$MOOR_HOME/skills/        Installed skills
~/.moor/skins/            Custom themes (see references/themes.md)
~/.moor/desktop-plugins/  Desktop app UI plugins (see references/desktop-plugins.md)
~/.moor/tui-widgets/      TUI widget apps (see references/tui-widgets.md)
~/.moor/pets/             Installed pet mascots (see references/petdex.md)
~/.moor/state.db          Canonical session store (SQLite + FTS5)
~/.moor/sessions/         Gateway routing index, request dumps, *.jsonl transcripts
...
Profiles use `~/.moor/profiles/<name>/` with the same layout. When a profile is active, resolve the real home from `$MOOR_HOME` — never hardcode `~/.moor`.

## Routing Table — load the reference for the task

| User wants... | Load |
|---|---|
| **Anything not listed below — "can Moor do X?", "how do I set up X?"** | **https://hermes-agent.nousresearch.com/docs/llms.txt** |
...
| AGENTS.md / .moor.md / CLAUDE.md project rules | `references/project-context-files.md` |
...
| MCP servers (add, catalog, `moor mcp`) | `references/native-mcp.md` |
...
| "Can app X use my Moor Portal subscription/OAuth?" | `references/portal-auth-for-third-party-apps.md` |
...
need more than their docs page. For everything else Moor ships, fetch
`llms.txt` and it maps the question to the page that answers it.

Two theming rules that hold even without loading the reference: **you apply skins yourself** (`moor config set display.skin <name>` — every surface repaints live within ~a second; don't tell the user to run `/skin`), and **to tweak one color, edit the ACTIVE skin** (`moor skin set <key> <hex>`) — never fork `default`, which drops the palette and resets the background.
...
| | `delegate_task` | Spawning `moor` process |
|-|-----------------|--------------------------|
...
- **"Can $external_app use my Moor Portal subscription / OAuth?"** — see `references/portal-auth-for-third-party-apps.md`. Walk the user through three layers (plugin-vs-app, what Portal actually exposes, local-broker-proxy option).

## Surfaces (quick orientation)

- **Desktop app** (`moor desktop` / `moor gui`) — native Electron app for macOS/Linux/Windows: streaming chat, session list, Cmd+K palette, drag-and-drop files, native notifications, per-profile remote-gateway login. Extend it with UI plugins — `references/desktop-plugins.md`.
- **Web dashboard** (`moor dashboard`) — full admin panel: messaging channels, MCP catalog, webhooks, memory, profile builder, plus an embedded `moor --tui` chat. Secured behind an OAuth/token gate.
- **Ink TUI** (`moor --tui` or `display.interface: tui`) — terminal UI with docked widget apps — `references/tui-widgets.md`.
- **OpenAI-compatible proxy** (`moor proxy`) — a local OpenAI API backed by whichever OAuth provider you're signed into. Point Codex CLI, Aider, Cline, or any script at it — no API key.
...
- **Profile-safe paths** — `get_moor_home()` in code, `$MOOR_HOME` when resolving paths in a session.
- **Never hand-edit `config.yaml` for the user** — use `moor config set KEY VAL`; a stray indent can corrupt the file and break the live gateway.
