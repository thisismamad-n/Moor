...
devtunnel port create moor-bot -p 3978 --protocol http  # 如已修改 TEAMS_PORT，请替换 3978
...
| `No inference provider configured` | 检查 `~/.moor/.env` 中是否设置了 `ANTHROPIC_API_KEY`（或其他提供商密钥） |
| 机器人收到消息但忽略它们 | 你的 AAD 对象 ID 可能不在 `TEAMS_ALLOWED_USERS` 中。运行 `teams status --verbose` 查找 |
| 隧道 URL 在重启后变更 | 使用命名隧道（`devtunnel create moor-bot`）时，devtunnel URL 是持久的。ngrok 和 cloudflared 每次运行都会生成新 URL（除非你有付费计划）——URL 变更时请用 `teams app update` 更新机器人端点 |
...
- 将凭据存储在权限为 `600` 的 `~/.moor/.env` 中（`chmod 600 ~/.moor/.env`）
