...
sidebar_position: 9
title: "Matrix"
description: "将 Moor Agent 设置为 Matrix 机器人"
---
...
在开始设置之前，先了解大多数人最想知道的：Moor 连接后的行为方式。

## Moor 的行为方式

| 场景 | 行为 |
|---------|----------|
| **私聊（DM）** | Moor 响应每条消息，无需 `@提及`。每个 DM 有独立的会话。设置 `MATRIX_DM_MENTION_THREADS=true` 可在 DM 中被 `@提及` 时创建线程。 |
| **房间** | 默认情况下，Moor 需要 `@提及` 才会响应。设置 `MATRIX_REQUIRE_MENTION=false` 或将房间 ID 添加到 `MATRIX_FREE_RESPONSE_ROOMS` 可开启自由响应模式。房间邀请会被自动接受。 |
| **线程** | Moor 支持 Matrix 线程（MSC3440）。在线程中回复时，Moor 会将线程上下文与主房间时间线隔离。机器人已参与的线程无需提及即可响应。 |
| **自动线程** | 默认情况下，Moor 会为其在房间中响应的每条消息自动创建线程，以保持对话隔离。设置 `MATRIX_AUTO_THREAD=false` 可禁用此功能。设置 `MATRIX_DM_AUTO_THREAD=true`（默认 false）可同时为私聊消息自动创建线程——这与 `MATRIX_DM_MENTION_THREADS` 不同，后者仅在私聊中 @提及 Bot 时才创建线程。 |
| **多用户共享房间** | 默认情况下，Moor 在房间内按用户隔离会话历史。同一房间中的两个人不会共享同一对话记录，除非你明确禁用该功能。 |
...
2. 选择一个用户名，例如 `moor`——完整的用户 ID 将是 `@hermes:your-server.org`。
...
你也可以以自己的用户身份运行 Moor。这意味着机器人以你的名义发帖——适合个人助手场景。

## 第二步：获取访问令牌

Moor 需要访问令牌（access token）来向 homeserver 进行身份验证。有两种方式：
...
Moor Agent 使用你的 Matrix 用户 ID 来控制谁可以与机器人交互。Matrix 用户 ID 的格式为 `@username:server`。
...
Moor 支持 Matrix 端对端加密，你可以在加密房间中与机器人聊天。
...
如果你之前使用 `MATRIX_ENCRYPTION=true` 运行 Moor，并正在升级到使用新的基于 SQLite 的加密存储的版本，机器人的加密身份已发生变化。你的 Matrix 客户端（Element）可能缓存了旧的设备密钥，并拒绝与机器人共享加密会话。
...
:::tip
**新安装不受影响。** 此迁移仅在你之前使用旧版 Moor 配置了可用的 E2EE 并正在升级时才需要。
...
| 设置 | 说明 |
|---------|-------------|
| `GATEWAY_PROXY_URL` | 远程 Moor API 服务器的 URL（例如 `http://192.168.1.100:8642`） |
...
:::tip
会话连续性通过 `X-Moor-Session-Id` 请求头维护。主机的 API 服务器按此 ID 跟踪会话，因此对话在消息之间持续存在，就像使用本地 agent 一样。
:::
...
**解决方法**：同步循环在出错时每 5 秒自动重试。检查 Moor 日志中与同步相关的警告。如果机器人持续落后，请确保你的 homeserver 有足够的资源。
...
- **媒体支持**：Moor 可以发送和接收图片、音频、视频和文件附件。媒体通过 Matrix 内容仓库 API 上传到你的 homeserver。
