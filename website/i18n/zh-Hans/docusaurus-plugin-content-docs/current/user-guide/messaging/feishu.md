...
sidebar_position: 11
title: "飞书 / Lark"
description: "将 Moor Agent 配置为飞书或 Lark 机器人"
---
...
Moor Agent 可作为全功能机器人与飞书和 Lark 集成。连接后，你可以在私信或群聊中与 Agent 对话，在 home chat 中接收 cron job 结果，并通过标准 gateway 流程发送文本、图片、音频和文件附件。

该集成支持两种连接模式：

- `websocket` — 推荐；Moor 主动建立出站连接，无需公开 webhook 端点
- `webhook` — 适用于已将 Moor 部署在可访问 HTTP 端点后的场景
...
当 Moor 运行在你的笔记本、工作站或私有服务器上时，使用 WebSocket 模式。无需公开 URL。官方 Lark SDK 会建立并维护一个持久的出站 WebSocket 连接，并支持自动重连。
...
仅当 Moor 已部署在可访问的 HTTP 端点后时，才使用 webhook 模式。

```bash
FEISHU_CONNECTION_MODE=webhook
```

在 webhook 模式下，Moor 启动一个 HTTP 服务器（通过 `aiohttp`），并在以下路径提供飞书端点：
...
`FEISHU_GROUP_POLICY` 环境变量控制 Moor 是否以及如何在群聊中响应：
...
| 值 | 行为 |
|-------|----------|
| `open` | Moor 响应任意群中任意用户的 @提及。 |
| `allowlist` | Moor 仅响应 `FEISHU_ALLOWED_USERS` 中列出的用户的 @提及。 |
| `disabled` | Moor 完全忽略所有群消息。 |

在所有模式下，消息处理前机器人必须被明确 @提及（或 @all）。私信始终绕过此限制。

设置 `FEISHU_REQUIRE_MENTION=false` 可让 Moor 读取所有群消息而无需 @提及：
...
Moor 在启动时自动检测机器人的 `open_id` 和显示名称。仅当自动检测无法访问飞书 API，或你的应用使用租户范围用户 ID 时，才需要手动设置：
...
默认情况下，Moor 忽略其他机器人发送的消息。当你希望 Moor 参与 A2A 编排或接收同一群中其他机器人的通知时，可启用机器人间消息传递。
...
2. **设置回调的接收方式：**
   在同一标签页中，Moor 以 `websocket` 模式运行时选择 **长连接**（Lark SDK 通过现有连接接收回调）；webhook 模式则填写请求地址（与事件 webhook 相同的端点，例如 `https://your-server:8765/feishu/webhook`）。飞书必须能够访问并解析该地址，否则点击会返回 200342/200343。
...
:::warning
没有已发布的卡片回调时，飞书仍会成功*发送*交互式卡片（发送仅需 `im:message:send` 权限），但点击任意按钮将返回错误 200340。卡片看起来正常——错误仅在用户与其交互时才会出现，并且点击不会到达 Moor（`gateway.log` 中没有任何记录），因为飞书在投递回调之前就拒绝了它。

错误码 200672 / 200673 表示回调*确实*到达了 Moor 而飞书拒绝了响应；如果遇到，请附上对应的 `gateway.log` 日志提交 issue。
:::

## 文档评论智能回复

除聊天外，适配器还可以回复**飞书/Lark 文档**中的 `@` 提及。当用户在文档中评论（局部文本选区或全文评论）并 @提及机器人时，Moor 读取文档内容及周围的评论线程，并在线程中内联发布 LLM 回复。
...
| `websockets not installed; websocket mode unavailable` | 安装 websockets：`moor pm repair` |
...
| 启用 `FEISHU_ALLOW_BOTS` 后对端机器人消息仍被忽略 | Moor 尚无法识别自身——请设置 `FEISHU_BOT_OPEN_ID`（若应用使用 `sender_id_type=user_id` 则同时设置 `FEISHU_BOT_USER_ID`）。 |
