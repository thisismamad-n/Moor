...
创建完成后，打开该 SA，进入 **Keys → Add Key → Create new key → JSON**，下载文件。将其保存到只有 Moor 可读的位置（例如 `~/.hermes/google-chat-sa.json`，`chmod 600`）。
...
安装 Google Chat 适配器所需的依赖（目前没有发布 Moor extra，请直接安装）：
