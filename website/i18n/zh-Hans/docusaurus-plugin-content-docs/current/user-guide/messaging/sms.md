...
title: "SMS (Twilio)"
description: "通过 Twilio 将 Moor Agent 设置为 SMS 聊天机器人"
---
...
:::tip 暴露你的 Webhook
如果你在本地运行 Moor，请使用隧道工具暴露 webhook：
...
向你的 Twilio 号码发送短信——Moor 将通过 SMS 回复。
...
Moor 通过验证 `X-Twilio-Signature` 头（HMAC-SHA1）来确认入站 webhook 确实来自 Twilio，防止攻击者注入伪造消息。
...
3. 查看 Moor gateway 日志中的 Twilio API 错误
