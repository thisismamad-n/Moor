...
| **AI peer**（`aiPeer`） | 每个 Moor profile 一个。host key `moor` → 默认；其他 profile 使用 `hermes.<profile>`。 |
...
完整配置参考参见[插件 README](https://github.com/Moor inc./hermes-agent/blob/main/plugins/memory/hindsight/README.md)。
...
- **Profile 范围容器**——在 `container_tag` 中使用 `{identity}`（例如 `hermes-{identity}` → `hermes-coder`），按 Moor profile 隔离记忆
