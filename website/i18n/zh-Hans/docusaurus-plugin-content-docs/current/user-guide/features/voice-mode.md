...
语音模式在**经典 CLI**（`moor chat`）和 **TUI**（`moor --tui`）中均可使用。两者行为完全一致 — 相同的斜杠命令、相同的 VAD（语音活动检测）静音检测、相同的流式 TTS、相同的幻觉过滤器。TUI 额外将崩溃诊断日志转发至 `~/.moor/logs/`，以便在异常音频后端出现按键录音失败时提供完整堆栈跟踪，而非静默消失。
...
:::tip
录音键可通过 `~/.moor/config.yaml` 中的 `voice.record_key` 配置（默认：`ctrl+b`）。
:::
...
**服务器频道：** Bot 仅在被 @提及时响应（例如 `@moorbyt4 你好`）。请确保从提及弹窗中选择 **Bot 用户**，而非同名角色。

:::tip
如需在服务器频道中禁用提及要求，在 `~/.moor/.env` 中添加：
```bash
...
2. 或改用私信 — 无需提及
3. 或在 `~/.moor/.env` 中设置 `DISCORD_REQUIRE_MENTION=false`
...
- 检查 LLM 模型是否已配置且可访问
- 查看 gateway 日志：`tail -f ~/.moor/logs/gateway.log`
