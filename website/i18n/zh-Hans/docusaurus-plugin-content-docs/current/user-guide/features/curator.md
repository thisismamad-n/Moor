...
跟踪 [issue #7816](https://github.com/Moor inc./hermes-agent/issues/7816)。
...
moor curator backup         # take a manual snapshot of ~/.hermes/skills/
...
在每次真正的 curator pass 之前，Moor 会在 `~/.hermes/skills/.curator_backups/<utc-iso>/skills.tar.gz` 处对 `~/.hermes/skills/` 进行 tar.gz 快照。如果某次 pass 归档或合并了你不希望被触碰的内容，可以用一条命令撤销整次运行：
...
- `~/.hermes/skills/.hub/lock.json`（通过 `moor skills install` 安装的技能）。
...
- [Issue #7816](https://github.com/Moor inc./hermes-agent/issues/7816)——原始提案与设计讨论