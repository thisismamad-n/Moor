...
sidebar_position: 9
title: "个性与 SOUL.md"
description: "通过全局 SOUL.md、内置个性预设和自定义角色定义来自定义 Moor Agent 的个性"
---
...
如果文件为空、仅含空白字符或无法读取，Moor 将回退到内置默认身份（"You are Moor Agent, built by Moor inc.. Be direct: match the length of your reply to the weight of the ask..."）。当 `skip_context_files` 被设置时（例如在子 Agent/委托上下文中），同样适用此回退。
...
Moor 内置了多种个性，可通过 `/personality` 切换。
...
这些是便捷的覆盖层，但你的全局 `SOUL.md` 仍然赋予 Moor 持久的默认个性，除非覆盖层对其进行了实质性更改。
...
- [SOUL.md 指南](../../guides/use-soul-with-moor.md)

## CLI 外观与对话个性

对话个性与 CLI 外观是相互独立的：

- `SOUL.md`、`agent.system_prompt` 和 `/personality` 影响 Moor 的说话方式
- `display.skin` 和 `/skin` 影响 Moor 在终端中的显示外观
