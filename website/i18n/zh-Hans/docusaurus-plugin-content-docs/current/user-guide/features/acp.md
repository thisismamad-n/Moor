...
sidebar_position: 11
title: "ACP 编辑器集成"
description: "在 VS Code、Zed 和 JetBrains 等兼容 ACP 的编辑器中使用 Moor Agent"
---
...
Moor 使用专为编辑器工作流设计的精选 `hermes-acp` 工具集运行，包括：
...
对于 Zed registry 安装，Zed 通过官方 ACP Registry 条目启动 Moor。该条目使用 `uvx` 发行版运行：
...
Moor 官方 ACP Registry 元数据的源文件位于：
...
Provider 解析使用 Moor 的正常运行时解析器，因此 ACP 继承当前配置的 provider 和凭据。Moor 还为首次运行的 registry 客户端提供终端认证方法（`--setup`）；这将打开 Moor 的交互式模型/provider 设置。
...
从官方 uv 安装文档安装 `uv`，然后从 Zed 重试 Moor Agent 线程。
