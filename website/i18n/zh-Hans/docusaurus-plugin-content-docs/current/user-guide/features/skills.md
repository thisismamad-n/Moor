...
捆绑的 `plan` skill 是一个很好的示例。运行 `/plan [request]` 会加载该 skill 的指令，告知 Moor 在需要时检查上下文、编写 markdown 实现计划而非直接执行任务，并将结果保存在相对于当前工作区/后端工作目录的 `.hermes/plans/` 下。
...
当遇到缺失的值时，Moor 仅在本地 CLI 中实际加载 skill 时才会安全地请求输入。你可以跳过设置并继续使用该 skill。消息平台不会在聊天中请求密钥——它们会告诉你改用本地的 `moor setup` 或 `~/.hermes/.env`。
...
Moor 支持发布 Claude 兼容插件/市场清单的市场仓库。

已知集成来源包括：
- [anthropics/skills](https://github.com/anthropics/skills)
- [aiskillstore/marketplace](https://github.com/aiskillstore/marketplace)

Moor 来源 id：`claude-marketplace`
...
直接从任何 HTTP(S) URL 安装单文件 `SKILL.md`——当作者在自己的站点上托管 skill 时非常有用（无 hub 列表，无需输入 GitHub 路径）。Moor 获取 URL，解析 YAML frontmatter，进行安全扫描并安装。
...
Moor 在仓库的 `skills/` 中附带一组捆绑 skills。在安装时以及每次 `moor update` 时，同步过程会将这些 skills 复制到 `~/.hermes/skills/` 中，并在 `~/.hermes/skills/.bundled_manifest` 记录一个清单，将每个 skill 名称映射到同步时的内容哈希（**origin hash**）。
...
:::note Profiles
每个 profile 在其自己的 `HERMES_HOME` 下有自己的 `.bundled_manifest`，因此 `moor -p coder skills reset <name>` 只影响该 profile。
:::
