...
每个 hook 是 `~/.moor/hooks/` 下的一个目录，包含两个文件：
...
这是社区中流行的一种模式：在 `~/.moor/BOOT.md` 放置一个 Markdown 检查清单，让 agent 在每次 gateway 启动时执行一次。适用于"每次启动时检查隔夜 cron 失败情况，若有失败则在 Discord 上通知我"，或"汇总过去 24 小时的 deploy.log 并发布到 Slack #ops"等场景。

本教程展示如何以用户自定义 hook 的方式自行构建。Moor 不内置 BOOT.md hook——你可以精确配置自己想要的行为。

#### 我们要构建什么

1. 在 `~/.moor/BOOT.md` 放置一个包含自然语言启动指令的文件。
...
创建 `~/.moor/BOOT.md`。像给人类助手下达指令一样编写：
...
删除 `~/.moor/BOOT.md` 即可禁用检查清单——hook 保持加载状态，但在文件不存在时会静默跳过。
...
Moor 早期版本将此作为内置 hook 发布，每次 gateway 启动时都会静默生成一个使用裸默认值的 agent。这让使用自定义端点的用户感到意外，也让不知道它在运行的用户无从察觉。将其作为文档化模式保留——由你在 hooks 目录中构建——意味着你能清楚地看到它的行为，并通过编写文件来选择启用。

### 工作原理

1. Gateway 启动时，`HookRegistry.discover_and_load()` 扫描 `~/.moor/hooks/`
...
- 运行时事件名以 `moor_cli.plugins.VALID_HOOKS` 为准。`moor hooks list` 只列出已配置的 shell/outbound hook，并非可用事件目录；只有 `moor hooks test <event>` 收到无效事件时才会打印有效集合。

### 已发布的 plugin-hook 目录

下表列出每个 call site 实际传入的事件专属字段。为保持向后兼容，`PluginManager` 还会向每个 plugin-hook 回调加入 `telemetry_schema_version="moor.observer.v1"`。这个旧版 envelope 标记并不表示所有 hook payload 共用同一套语义 schema；新的版本化 contract 应归属于具体事件或 capability family。
...
在 `cli-config.yaml` 中声明 shell 脚本 hook，Moor 会在对应的插件 hook 事件触发时将其作为子进程运行——在 CLI 和 gateway 会话中均适用。无需编写 Python 插件。
...
Shell hooks 通过在 CLI 启动（`moor_cli/main.py`）和 gateway 启动（`gateway/run.py`）时调用 `agent.shell_hooks.register_from_config(cfg)` 来注册。它们与 Python 插件 hook 自然组合——两者都流经同一个分发器。

### 对比一览

| 维度 | Shell hooks | [Plugin hooks](#plugin-hooks) | [Gateway hooks](#gateway-event-hooks) |
|------|-------------|-------------------------------|---------------------------------------|
| 声明位置 | `~/.moor/config.yaml` 中的 `hooks:` 块 | 插件 `plugin.yaml` 中的 `register()` | `HOOK.yaml` + `handler.py` 目录 |
| 存放位置 | `~/.moor/agent-hooks/`（约定） | `~/.moor/plugins/<name>/` | `~/.moor/hooks/<name>/` |
...
每次事件触发时，Moor 为每个匹配的 hook（在 matcher 允许的情况下）生成一个子进程，将 JSON 载荷通过 **stdin** 传入，并从 **stdout** 读取 JSON 响应。
...
Claude Code 的 `UserPromptSubmit` 事件在 Moor 中没有对应的独立事件——`pre_llm_call` 在相同位置触发，且已支持上下文注入。在此使用即可。
...
每个唯一的 `(event, command)` 对在 Moor 首次遇到时会提示用户审批，然后将决定持久化到 `~/.moor/shell-hooks-allowlist.json`。后续运行（CLI 或 gateway）跳过提示。

三种方式可绕过交互式提示——满足其一即可：

1. CLI 上的 `--accept-hooks` 标志（如 `moor --accept-hooks chat`）
2. `MOOR_ACCEPT_HOOKS=1` 环境变量
...
- 只引用你自己编写或完整审查过的脚本。
- 将脚本保存在 `~/.moor/agent-hooks/` 内，便于审计路径。
...
hooks:
  outbound:
    - url: "https://example.com/moor-events"
      events: [post_tool_call, on_session_end]
      secret_env: MOOR_WEBHOOK_SECRET
```

配置中的事件名同样以 `moor_cli.plugins.VALID_HOOKS` 为准。`moor hooks list` 会列出这些已配置 target，但不会输出完整的可用事件目录。