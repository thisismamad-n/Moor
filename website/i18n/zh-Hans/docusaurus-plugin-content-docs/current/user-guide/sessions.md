...
1. **SQLite 数据库**（`~/.moor/state.db`）——包含 FTS5 全文搜索的结构化 session 元数据，以及完整消息历史
...
:::tip
当 session 变长时使用 `/compress`，用 `/new` 开启新线程，仅在需要从存储中删除旧的已结束 session 时才使用 `moor sessions prune`。压缩会减少活跃上下文，而不是隐私删除。向 `/new` 传入名称（例如 `/new payments-refactor`）可以预先设置新 session 的初始标题——便于之后通过 `/resume <name>` 或 `/sessions` 选择器找到它。
:::
...
# 会自动恢复最新的一个
moor -c "my project"   # → 恢复 "my project #3"
```
...
Session ID 在退出 CLI session 时显示，也可通过 `moor sessions list` 查找。
...
要禁用摘要并保留最简单的单行行为，在 `~/.moor/config.yaml` 中设置：
...
**恢复到 CLI：** 当你想回到桌面时，只需运行 `/resume <title>`（或在 shell 中运行 `moor -r "<title>"`），从平台停止的地方继续。
...
Moor 在第一次交换后自动为每个 session 生成简短的描述性标题（3–7 个词）。这在后台线程中使用快速辅助模型运行，不增加延迟。浏览 `moor sessions list` 或 `moor sessions browse` 时可以看到自动生成的标题。
...
按名称恢复时（`moor -c "my project"`），会自动选取谱系中最新的 session。
...
Moor 通过 `moor sessions` 提供完整的 session 管理命令集：
...
`moor sessions export` 是所有导出格式的统一入口，用 `--format` 选择：
...
`--format trace` 生成 Claude Code JSONL — Hugging Face Hub 的 [Agent Trace Viewer](https://huggingface.co/docs/hub/agent-traces) 可自动识别的转录格式。可以写入本地文件，或加 `--upload` 推送到你自己的私有 `moor-traces` 数据集（读取 `HF_TOKEN`）：
...
当你想在隐藏或删除旧 session 之前保留一份可读的文件归档时，传入 `--format md` 或 `--format qmd`。Markdown/QMD 导出会为每个 session 写入一个文件到目录中（默认：`~/.moor/session-exports`）。
...
# 仅清理特定平台的 session
moor sessions prune --source telegram --older-than 60
...
如需更深入的分析——token 用量、费用估算、工具分解和活动模式——请使用 [`moor insights`](../reference/cli-commands.md#moor-insights)。
...
| 内容 | 路径 | 描述 |
|------|------|-------------|
| SQLite 数据库 | `~/.moor/state.db` | 所有 session 元数据 + 带 FTS5 的消息 |
| Gateway 消息 | `~/.moor/state.db` | SQLite——所有 session 消息的权威存储 |
| Gateway 路由索引 | `~/.moor/sessions/sessions.json` | 将 session 键映射到活跃 session ID（来源元数据、过期标志） |

SQLite 数据库使用 WAL 模式支持并发读取和单写入，非常适合 gateway 的多平台架构。

:::note 遗留 JSONL 对话记录
在 state.db 成为权威存储之前创建的 session 可能在 `~/.moor/sessions/` 中留有
...
- 清理最多每 `sessions.min_interval_hours`（默认 24）小时运行一次；上次运行时间戳记录在 `state.db` 内部，因此在同一 `MOOR_HOME` 下的所有 Moor 进程间共享

默认为**关闭**——session 历史对 `session_search` 召回很有价值，静默删除可能会让用户感到意外。在 `~/.moor/config.yaml` 中启用：
...
:::tip
数据库增长缓慢（典型情况：数百个 session 约 10–15 MB），session 历史为跨历史对话的 `session_search` 召回提供支持，因此自动清理默认关闭。如果你运行繁重的 gateway/cron 工作负载且 `state.db` 明显影响性能（已观察到的故障模式：约 1000 个 session 的 384 MB state.db 导致 FTS5 插入和 `/resume` 列表变慢），则启用它。使用 `moor sessions prune` 进行一次性清理，无需开启自动清理。
:::