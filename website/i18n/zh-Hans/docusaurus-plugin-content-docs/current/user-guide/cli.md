...
# 使用指定提供商
moor chat --provider nous        # 使用 Nous Portal
...
# 启动时预加载一个或多个 skill
moor -s hermes-agent-dev,github-auth
...
| 时长 | 会话已用时间 |
| ⚠ YOLO | **YOLO 模式警告**——当 `HERMES_YOLO_MODE` 开启时显示（通过启动时的 `moor --yolo` 或会话中的 `/yolo` 切换）。与横幅行警告保持同步，确保你不会忘记自己处于自动批准模式。 |
...
CLI 会话存储在 Moor 的 SQLite 状态数据库 `~/.hermes/state.db` 中。数据库保存：
