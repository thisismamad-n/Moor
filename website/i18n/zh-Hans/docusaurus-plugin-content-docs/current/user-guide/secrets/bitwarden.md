...
2. Moor 将该单一令牌以 `BWS_ACCESS_TOKEN` 的形式存储在 `~/.hermes/.env` 中。
3. 每次 `moor`（或 gateway，或 cron 任务）启动时，在加载 `~/.hermes/.env` 之后，Moor 会调用 `bws secret list <project_id>` 并将返回的密钥写入 `os.environ`。
...
| `moor secrets bitwarden status` | 显示配置、二进制版本及令牌是否存在 |
