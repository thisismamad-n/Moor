...
sidebar_position: 16
title: "Google Gemini"
description: "将 Moor Agent 与 Google Gemini 配合使用——原生 AI Studio API、API 密钥配置、工具调用、流式传输及配额说明"
---
...
运行 `moor model` 后，`~/.moor/config.yaml` 将包含：
...
Moor 找不到可用的 API 密钥。请将以下任一项添加到 `~/.moor/.env`：
...
Moor 默认可能会在选择器中隐藏低吞吐量的 Gemma 模型。如果你有意评估某个模型，请直接在 `~/.moor/config.yaml` 中设置模型 ID。
