...
常见自动化模式的复制粘贴蓝图。每个蓝图使用 Moor 内置的 [cron 调度器](../user-guide/features/cron.md) 实现基于时间的触发，使用 [webhook 平台](../user-guide/messaging/webhooks.md) 实现事件驱动触发。
...
| **GitHub 事件** | PR 开启、推送、issue、CI 结果时触发 | Webhook 平台（`moor webhook subscribe`） |
| **API 调用** | 外部服务向你的端点 POST JSON | Webhook 平台（config.yaml 路由或 `moor webhook subscribe`） |
...
```bash
moor webhook subscribe github-pr-review \
  --events "pull_request" \
...
3. For each merged PR, check if it modified:
   ...
   - CLI commands (moor_cli/commands.py, moor_cli/main.py) — may need docs/reference/cli-commands.md update
   - Config options (moor_cli/config.py) — may need docs/user-guide/configuration.md update
...
1. Locate the moor-agent checkout and its pyproject.toml and uv.lock. Do not activate or mutate Moor's dependency environment.
2. Scan uv.lock with an independently installed scanner that supports that lock format (check its --help). Preserve the complete findings and errors. If no scanner is available, report the blocker; do not install one into Moor.
...
```bash
moor webhook subscribe deploy-verify \
  --events "deployment" \
...
```python title="~/.moor/scripts/check-uptime.py"
...
for ep in ENDPOINTS:
    try:
        start = time.time()
        req = urllib.request.Request(ep["url"], headers={"User-Agent": "moor-monitor/1.0"})
...
moor cron create "every 30m" \
  ...
  --script ~/.moor/scripts/check-uptime.py \
  --name "Uptime monitor" \
  --deliver telegram
```
...
```bash
moor cron create "0 8 * * *" \
  "Search arXiv for the 3 most interesting papers on 'language model reasoning' OR 'tool-use agents' from the past day. For each paper, create an Obsidian note with the title, authors, abstract summary, key contribution, and potential relevance to Moor Agent development." \
...
```bash
moor webhook subscribe auto-port \
  --events "pull_request" \
...
```bash
moor webhook subscribe stripe-payments \
...
```bash
moor cron create "0 3 * * 0" \
  "Run a comprehensive security audit of the moor-agent codebase.
