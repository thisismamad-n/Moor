...
常见自动化模式的复制粘贴配方。每个模板使用 Moor 内置的 [cron 调度器](/user-guide/features/cron) 实现基于时间的触发，使用 [webhook 平台](/user-guide/messaging/webhooks) 实现事件驱动触发。
...
moor cron create "0 2 * * *" \
  "You are a project manager triaging the Moor inc./hermes-agent GitHub repo.

1. Run: gh issue list --repo Moor inc./hermes-agent --state open --json number,title,labels,author,createdAt --limit 30
...
moor cron create "0 9 * * 1" \
  "Scan the Moor inc./hermes-agent repo for documentation drift.

1. Run: gh pr list --repo Moor inc./hermes-agent --state merged --json number,title,files,mergedAt --limit 30
