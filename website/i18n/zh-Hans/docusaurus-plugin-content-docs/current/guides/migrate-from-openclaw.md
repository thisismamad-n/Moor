...
sidebar_position: 10
title: "从 OpenClaw 迁移"
description: "将 OpenClaw / Clawdbot 配置迁移到 Moor Agent 的完整指南——包括迁移内容、配置键映射及迁移后的检查事项。"
---
...
| `--no-backup` | 跳过迁移前对 `~/.moor/` 的 zip 快照备份（默认在执行前写入单个还原点归档，位于 `~/.moor/backups/pre-migration-*.zip`；可通过 `moor import` 还原）。 |
...
| 内容 | OpenClaw 来源 | Moor 目标 | 备注 |
|------|----------------|-------------------|-------|
| Persona | `workspace/SOUL.md` | `~/.moor/SOUL.md` | 直接复制 |
| 工作区指令 | `workspace/AGENTS.md` | `--workspace-target` 中的 `AGENTS.md` | 需要 `--workspace-target` 标志 |
| 长期记忆 | `workspace/MEMORY.md` | `~/.moor/memories/MEMORY.md` | 解析为条目，与现有内容合并并去重，使用 `§` 分隔符 |
| 用户档案 | `workspace/USER.md` | `~/.moor/memories/USER.md` | 与记忆相同的条目合并逻辑 |
| 每日记忆文件 | `workspace/memory/*.md` | `~/.moor/memories/MEMORY.md` | 所有每日文件合并至主记忆 |
...
| 来源 | OpenClaw 位置 | Moor 目标 |
|--------|------------------|-------------------|
| 工作区 skills | `workspace/skills/` | `~/.moor/skills/openclaw-imports/` |
| 托管/共享 skills | `~/.openclaw/skills/` | `~/.moor/skills/openclaw-imports/` |
| 个人跨项目 skills | `~/.agents/skills/` | `~/.moor/skills/openclaw-imports/` |
| 项目级共享 skills | `workspace/.agents/skills/` | `~/.moor/skills/openclaw-imports/` |

Skill 冲突由 `--skill-conflict` 处理：`skip` 保留现有 Moor skill，`overwrite` 替换，`rename` 创建带 `-imported` 后缀的副本。

### 模型与 Provider 配置

| 内容 | OpenClaw 配置路径 | Moor 目标 | 备注 |
|------|---------------------|-------------------|-------|
...
| Provider API 密钥 | `models.providers.*.apiKey` | `~/.moor/.env` | 需要 `--migrate-secrets`。参见下方 [API 密钥解析](#api-key-resolution) |

### Agent 行为

| 内容 | OpenClaw 配置路径 | Moor 配置路径 | 映射规则 |
|------|---------------------|-------------------|---------|
...
不会导入空闲或每日重置计时器。Moor 会话会持续保留，直到显式使用 `/new` 或 `/reset`。身份关联、线程绑定、维护、作用域和发送策略等高级设置仍会归档以供参考。

### MCP 服务器

| OpenClaw 字段 | Moor 字段 | 备注 |
|----------------|-------------|-------|
...
| TTS 资源文件 | `~/.moor/tts/`（文件复制） |

### 消息平台

| 平台 | OpenClaw 配置路径 | Moor `.env` 变量 | 备注 |
|----------|---------------------|----------------------|-------|
...
| Gateway 认证 token | `gateway.auth.token` | `.env` → `MOOR_GATEWAY_TOKEN` | 需要 `--migrate-secrets` |
| 工作目录 | `agents.defaults.workspace` | `.env` → `MESSAGING_CWD` | |

### 已归档（无对应 Moor 等效项）

以下内容保存至 `~/.moor/migration/openclaw/<timestamp>/archive/` 供人工审查：

| 内容 | 归档文件 | 在 Moor 中的重建方式 |
|------|-------------|--------------------------|
...
2. **审查归档文件** — `~/.moor/migration/openclaw/<timestamp>/archive/` 中的所有内容需要人工处理。

3. **开启新会话** — 导入的 skills 和记忆条目在新会话中生效，当前会话不受影响。

4. **验证 API 密钥** — 运行 `moor status` 检查 provider 认证状态。

5. **测试消息平台** — 若迁移了平台 token，重启 gateway：`systemctl --user restart moor-gateway`
...
导入的 skills 位于 `~/.moor/skills/openclaw-imports/`。开启新会话后生效，或运行 `/skills` 验证是否已加载。
