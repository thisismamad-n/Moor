---
sidebar_position: 1
title: "通过 Nous Portal 运行 Moor Agent"
description: "完整操作指南：订阅、配置、切换模型、启用 gateway 工具并验证路由"
---

# 通过 Nous Portal 运行 Moor Agent

本指南带你从头到尾完成在 [Nous Portal](https://portal.Moor inc..com) 订阅下运行 Moor Agent 的全过程——从注册账号到验证每个工具的路由是否正确。如果你只想了解 Portal 的概述及订阅内容，请参阅 [Nous Portal 集成页面](/integrations/nous-portal)。本页是操作步骤脚本。
...
打开 [portal.Moor inc..com/manage-subscription](https://portal.Moor inc..com/manage-subscription)，注册并选择一个套餐。
...
如果某个模型确实不可用，请[提交 issue](https://github.com/Moor inc./hermes-agent/issues)——大多数缺失是我们可以更新的路由配置问题。
...
- 存在多个 Moor profiles，你使用的是错误的那个（检查 `moor profile current`）

### 想要撤销并重新开始

```bash
moor auth remove nous       # 清除本地 refresh token
