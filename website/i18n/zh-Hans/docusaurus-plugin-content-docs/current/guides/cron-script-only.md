...
Moor 将此称为**无 agent 模式**。这是去掉 LLM 的 cron 系统。
...
无 agent 模式的真正优势在于：agent 本身可以为你设置看门狗——无需编辑器、无需 shell、无需记忆 CLI 参数。你描述需求，Moor 编写脚本、安排计划，并告知你何时触发。
...
当你的请求类似"当 X 时告警我"或"每 N 分钟检查 Y，如果 Z 则通知我"，Moor 的 `cronjob` 工具描述会指示它在消息内容完全由脚本决定时使用 `no_agent=True`。当请求需要推理时（*"总结新 issue"*、*"挑选最有趣的头条"*、*"起草一条友好提醒"*），则回退到普通的 LLM 驱动路径。
...
> **你：** 改成每 15 分钟一次
>
> **Moor：** *（调用 `cronjob(action='update', job_id='abc123', schedule='every 15m')`）*
...
| OS cron + `curl` 到 [webhook 订阅](/user-guide/messaging/webhooks) | 你的脚本，由 OS 调度 | 当 Moor 本身可能不健康时（即被监控对象） |
