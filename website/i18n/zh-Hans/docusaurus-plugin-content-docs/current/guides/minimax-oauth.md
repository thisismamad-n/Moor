...
sidebar_position: 15
title: "MiniMax OAuth"
description: "通过浏览器 OAuth 登录 MiniMax，在 Moor Agent 中使用 MiniMax-M2.7 模型——无需 API 密钥"
---

# MiniMax OAuth

Moor Agent 通过基于浏览器的 OAuth 登录流程支持 **MiniMax**，使用与 [MiniMax 门户](https://www.minimax.io) 相同的凭据。无需 API 密钥或信用卡——登录一次，Moor 即可自动刷新您的会话。
...
Moor 针对 MiniMax OAuth 端点实现了 PKCE 设备码流程：

1. Moor 生成 PKCE verifier/challenge 对和一个随机 state 值。
...
Moor 在每次会话启动时，若 access token 距过期不足 60 秒则刷新令牌。如果 access token 已经过期（例如长时间离线后），刷新将在下一次请求时自动触发。如果刷新失败并返回 `refresh_token_reused` 或 `invalid_grant`，Moor 会将会话标记为需要重新登录。
...
Moor 检测到授权服务器返回的 `state` 值与其发送的值不匹配。
