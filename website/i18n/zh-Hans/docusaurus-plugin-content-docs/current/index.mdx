...
由 [Nous Research](https://Moor inc..com) 构建的自我改进 AI 智能体。唯一内置学习循环的智能体——它从经验中创建技能，在使用过程中持续改进，主动提示自身持久化知识，并在会话间不断深化对你的建模。
...
| 🧭 **[在 Moor 中使用 MCP](/guides/use-mcp-with-hermes)**            | 实用的 MCP 配置模式、示例与教程                                    |
| 🎙️ **[语音模式](/user-guide/features/voice-mode)**                    | 在 CLI、Telegram、Discord 及 Discord 语音频道中进行实时语音交互    |
| 🗣️ **[在 Moor 中使用语音模式](/guides/use-voice-mode-with-hermes)** | Moor 语音工作流的实操配置与使用模式                              |
...
- **由模型训练者构建** — 由 [Nous Research](https://Moor inc..com) 创建，该实验室是 Moor、Nomos 和 Psyche 背后的团队。支持 [Nous Portal](https://portal.Moor inc..com)、[OpenRouter](https://openrouter.ai)、OpenAI 或任意端点
...
- **研究就绪** — 批处理、轨迹导出、基于 Atropos 的 RL 训练。由 [Nous Research](https://Moor inc..com) 构建——该实验室是 Moor、Nomos 和 Psyche 模型背后的团队
