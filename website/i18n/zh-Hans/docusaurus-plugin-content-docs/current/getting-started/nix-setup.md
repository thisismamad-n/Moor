...
```bash
# 直接运行（首次使用时构建，之后使用缓存）
nix run github:Moor inc./hermes-agent -- setup
nix run github:Moor inc./hermes-agent -- chat

# 或持久化安装
nix profile install github:Moor inc./hermes-agent
moor setup
moor chat
```

执行 `nix profile install` 后，`moor`、`hermes-agent` 和 `hermes-acp` 将出现在你的 PATH 中。之后的工作流与[标准安装](./installation.md)完全相同——`moor setup` 引导你完成提供商选择，`moor gateway install` 设置 launchd（macOS）或 systemd 用户服务，配置存放在 `~/.hermes/`。
...
# /etc/nixos/flake.nix（或你的系统 flake）
{
  inputs = {
    ...
    hermes-agent.url = "github:Moor inc./hermes-agent";
  };
...
```bash
echo "OPENROUTER_API_KEY=sk-or-your-key" | sudo install -m 0600 -o moor /dev/stdin /var/lib/hermes/env
```
...
设置 `addToSystemPackages = true` 有两个作用：将 `moor` CLI 添加到系统 PATH，**并**在系统范围内设置 `HERMES_HOME`，使交互式 CLI 与 gateway 服务共享状态（会话、技能、cron）。不设置此项时，在 shell 中运行 `moor` 会创建独立的 `~/.hermes/` 目录。
:::
...
- 对于在 moor 代码库上工作的开发者，设置 `HERMES_DEV=1` 可绕过容器路由，直接运行本地检出版本
...
| 覆盖 moor 包 | `package` | `inputs.hermes-agent.packages.${system}.default.override { ... }` |
...
`environment`（非密钥变量）和 `environmentFiles`（密钥文件）在激活时（`nixos-rebuild switch`）都会合并到 `$HERMES_HOME/.env` 中。Moor 在每次启动时读取此文件，因此更改在 `systemctl restart hermes-agent` 后生效——无需重建容器。
...
Agent 身份文件是独立的：Moor 从 `$HERMES_HOME/SOUL.md` 加载其主要 `SOUL.md`，在 NixOS 模块中对应 `${services.hermes-agent.stateDir}/.hermes/SOUL.md`。将 `SOUL.md` 放入 `documents` 只会创建一个工作区文件，不会替换主角色文件。
...
# 原生模式
sudo -u moor HERMES_HOME=/var/lib/hermes/.hermes \
  moor mcp add my-oauth-server --url https://mcp.example.com/mcp --auth oauth
```
...
# 确保：chown moor:hermes，chmod 0600
```
...
2. **`.managed` 标记文件**，位于 `HERMES_HOME` 中——由激活脚本设置，对交互式 shell 可见（例如 `docker exec -it hermes-agent moor config set ...` 也会被屏蔽）
...
插件在激活时以符号链接方式安装到 `$HERMES_HOME/plugins/`。Moor 通过其正常的目录扫描发现它们。从列表中移除插件并运行 `nixos-rebuild switch` 会删除符号链接。
...
```nix
{
  inputs.hermes-agent.url = "github:Moor inc./hermes-agent";
...
| 检查 | 测试内容 |
|---|---|
| `package-contents` | `moor` 和 `hermes-agent` 二进制文件存在且 `moor version` 可运行 |
...
| `managed-guard` | `HERMES_MANAGED=true moor config set ...` 打印 NixOS 错误 |
...
| `addToSystemPackages` | `bool` | `false` | 将 `moor` CLI 添加到系统 PATH 并在系统范围内设置 `HERMES_HOME` |
...
| `container.hostUsers` | `listOf str` | `[]` | 获得 `~/.hermes` 符号链接（指向服务 stateDir）的交互式用户，自动加入 `moor` 组 |
...
```
/var/lib/hermes/                     # stateDir（归 moor:hermes 所有，权限 0750）
├── .hermes/                         # HERMES_HOME
...
```bash
# 原生模式
sudo -u moor cat /var/lib/hermes/.hermes/.env
...
| `moor version` 显示旧版本 | 容器未重启 | `systemctl restart hermes-agent` |
| `/var/lib/hermes` 权限拒绝 | 状态目录为 `0750 moor:hermes` | 使用 `docker exec` 或 `sudo -u moor` |
