...
description: "通过签名 APT 仓库安装适用于 aarch64 Android 的 Moor 预发布软件包"
---

# 在 Android 上使用 Termux

Moor 的 Termux 软件包适用于 aarch64（arm64-v8a）设备，目前处于预发布测试阶段。
...
4. 添加 canary 仓库：
   ...
   ```bash
   printf '%s\n' \
     "deb [signed-by=$PREFIX/etc/apt/keyrings/moor-agent.asc] https://hermes-assets.nousresearch.com/releases/termux/canary moor-canary main" \
...
软件包位于 `$PREFIX/lib/moor-agent/`。
`$PREFIX/bin/` 中的 `moor`、`moor-agent` 和 `moor-acp` 链接使用包内运行时。
它们不依赖 Termux 的 Python 或 Node.js 软件包。
数据位于 `~/.moor/`，或所选 `MOOR_HOME`。
...
`moor update` 拒绝修改 APT 所有的代码，改由包管理器更新。
...
使用 `moor gateway run` 在 Termux 会话中运行 gateway。
...
故障排查时运行 `moor doctor`，并提供 `moor --version` 和完整错误。
