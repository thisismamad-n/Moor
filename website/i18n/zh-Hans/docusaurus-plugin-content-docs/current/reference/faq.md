...
:::tip
`hermes backup` 即使在 Moor 正在运行时也能生成一致的快照。还原的归档文件不包含机器本地的运行时文件，如 `gateway.pid` 和 `cron.pid`。
:::
...
3. **提交 bug 报告：** 请包含您的操作系统、Python 版本（`python3 --version`）、Moor 版本（`hermes --version`）以及完整的错误信息