...
|--------|-------------|
| `--version`, `-V` | 显示版本并退出。 |
| `--profile <name>`, `-p <name>` | 选择本次调用使用的 Moor profile（配置文件）。覆盖 `moor profile use` 设置的粘性默认值。 |
...
| `--ignore-user-config` | 忽略 `~/.moor/config.yaml`，回退到内置默认值。`.env` 中的凭据仍会加载。 |
| `--ignore-rules` | 跳过 `AGENTS.md`、`SOUL.md`、`.cursorrules`、memory（记忆）和预加载 skill 的自动注入。 |
| `--tui` | 启动 [TUI](../user-guide/tui.md) 而非经典 CLI。等同于 `MOOR_TUI=1`。 |
...
| `moor bundles` | 将多个 skill 归组到单个 `/<name>` 斜杠命令下。参见 [Skill Bundles](../user-guide/features/skills.md#skill-bundles)。 |
...
| `--ignore-user-config` | 忽略 `~/.moor/config.yaml`，使用内置默认值。`.env` 中的凭据仍会加载。适用于隔离的 CI 运行、可复现的 bug 报告和第三方集成。 |
...
### `moor -z <prompt>` — 脚本化单次调用

对于程序化调用方（shell 脚本、CI、cron、通过管道传入 prompt 的父进程），`moor -z` 是最纯粹的单次入口：**单个 prompt 输入，最终响应文本输出，stdout 和 stderr 上不输出任何其他内容。** 无横幅、无 spinner、无工具预览、无 `Session:` 行——只有 agent 的最终回复纯文本。
...
单次运行覆盖（不修改 `~/.moor/config.yaml`）：

| 标志 | 等效环境变量 | 用途 |
|---|---|---|
| `-m` / `--model <model>` | `MOOR_INFERENCE_MODEL` | 覆盖本次运行的模型 |
...
相同的 agent、相同的工具、相同的 skill——只是剥离了所有交互式/装饰性层。如果你还需要在记录中包含工具输出，请改用 `moor chat -q`；`-z` 专门用于"我只需要最终答案"的场景。
...
- 登录基于 OAuth 的 provider（Anthropic、Copilot、Codex、Moor Portal）
- 输入或更新 API 密钥
- 从 provider 特定的模型列表中选择
- 配置自定义/自托管端点
- 将新默认值保存到 config

:::warning moor model 与 /model——了解区别
**`moor model`**（从终端运行，在任何 Moor 会话外部）是**完整的 provider 设置向导**。它可以添加新 provider、运行 OAuth 流程、提示输入 API 密钥并配置端点。

**`/model`**（在活跃的 Moor 聊天会话中输入）只能**在已设置好的 provider 和模型之间切换**。它无法添加新 provider、运行 OAuth 或提示输入 API 密钥。

**如果需要添加新 provider：** 先退出 Moor 会话（`Ctrl+C` 或 `/quit`），然后从终端提示符运行 `moor model`。
:::
...
如果你只配置了 OpenRouter，`/model` 将只显示 OpenRouter 模型。要添加其他 provider（Anthropic、DeepSeek、Copilot 等），请退出会话并从终端运行 `moor model`。
:::
...
| 选项 | 说明 |
|--------|-------------|
| `--all` | 在 `start` / `restart` / `stop` 时：对**每个 profile** 的 gateway 执行操作，而不仅限于活跃的 `MOOR_HOME`。当你并行运行多个 profile 并希望在 `moor update` 后全部重启时很有用。 |
...
:::tip WSL 用户
使用 `moor gateway run` 而非 `moor gateway start`——WSL 的 systemd 支持不稳定。用 tmux 包裹以保持持久运行：`tmux new -s moor 'moor gateway run'`。详见 [WSL FAQ](/reference/faq#wsl-gateway-keeps-disconnecting-or-moor-gateway-start-fails)。
:::
...
| `--reset` | 在设置前将配置重置为默认值。 |
| `--reconfigure` | 向后兼容别名——在已有安装上裸运行 `moor setup` 现在默认执行此操作。 |
| `--portal` | 一键 Moor Portal 设置：通过 OAuth 登录，将 Moor 设为推理 provider，并选择加入 [Tool Gateway](../user-guide/features/tool-gateway.md)。跳过向导其余部分。 |
...
检查 Moor Portal 认证、Tool Gateway 路由，并访问订阅页面。不带子命令时运行 `status`。

| 子命令 | 说明 |
|------------|-------------|
| `status`（默认） | Portal 认证状态 + 每个工具的 Tool Gateway 路由摘要。不带子命令时也会显示。 |
| `open` | 在默认浏览器中打开 `portal.nousresearch.com/manage-subscription`。 |
| `tools` | 列出每个 Tool Gateway 合作伙伴（Firecrawl、FAL、OpenAI TTS、Browser Use、Modal）及哪些通过 Moor 路由。 |

关于 gateway 本身的配置，请参阅 [Tool Gateway](../user-guide/features/tool-gateway.md)。关于一键设置路径，请参阅上方的 `moor setup --portal`。
...
```bash
moor slack manifest              # 将 manifest 打印到 stdout
moor slack manifest --write      # 写入 ~/.moor/slack-manifest.json
moor slack manifest --slashes-only  # 仅输出 features.slash_commands 数组
```
...
| 标志 | 默认值 | 用途 |
|------|---------|---------|
| `--write [PATH]` | stdout | 写入文件而非 stdout。裸 `--write` 写入 `$MOOR_HOME/slack-manifest.json`。 |
...
`moor update` 后重新运行 `moor slack manifest --write` 以获取新增命令。


## `moor login` / `moor logout` *（已弃用）*

:::caution
`moor login` 已被移除。请使用 `moor auth` 管理 OAuth 凭据，使用 `moor model` 选择 provider，或使用 `moor setup` 进行完整的交互式设置。
:::
...
```bash
moor auth                                              # 交互式向导
moor auth list                                         # 显示所有池
moor auth list openrouter                              # 显示特定 provider
moor auth add openrouter --api-key sk-or-v1-xxx        # 添加 API 密钥
moor auth add anthropic --type oauth                   # 添加 OAuth 凭据
moor auth remove openrouter 2                          # 按索引删除
moor auth reset openrouter                             # 清除冷却时间
moor auth status anthropic                             # 显示某 provider 的认证状态
moor auth logout anthropic                             # 登出并清除已存储的认证状态
moor auth spotify                                      # 通过 PKCE 将 Moor 与 Spotify 认证
```
...
多 profile、多项目协作看板。每个安装可托管多个看板（每个项目、仓库或领域一个）；每个看板是独立的队列，拥有自己的 SQLite 数据库和调度器作用域。新安装从名为 `default` 的单个看板开始，其数据库为 `~/.moor/kanban.db`（向后兼容）；其他看板位于 `~/.moor/kanban/boards/<slug>/kanban.db`。嵌入在 gateway 中的调度器每次 tick 扫描所有看板。
...
| `boards switch <slug>` / `boards use` | 将 `<slug>` 持久化为活跃看板（写入 `~/.moor/kanban/current`）。 |
...
moor kanban list                  # 显示 atm10-server 的任务
...
看板解析顺序（优先级从高到低）：`--board <slug>` 标志 → `MOOR_KANBAN_BOARD` 环境变量 → `~/.moor/kanban/current` 文件 → `default`。

所有操作也可作为 gateway 中的斜杠命令使用（`/kanban …`），参数界面相同——包括 `boards` 子命令和 `--board` 标志。

完整设计——与 Cline Kanban / Paperclip / NanoClaw / Gemini Enterprise 的对比、八种协作模式、四个用户故事、并发正确性证明——请参阅仓库中的 `docs/moor-kanban-v1-spec.pdf` 或 [Kanban 用户指南](/user-guide/features/kanban)。
...
| `--script` | 位于 `~/.moor/scripts/` 下的过滤/转换脚本。webhook payload 以 JSON 形式通过 stdin 传入；JSON stdout 会替换 payload，空 stdout、`[SILENT]` 或非零退出码会忽略该 webhook。参见[脚本过滤与转换](../user-guide/messaging/webhooks.md#script-filters-and-transforms)。 |

订阅持久化到 `~/.moor/webhook_subscriptions.json`，webhook 适配器无需重启 gateway 即可热重载。
...
| **Identity** | 活跃 profile 名称、MOOR_HOME 路径 |
...
:::tip
`moor dump` 专为分享而设计。交互式诊断请使用 `moor doctor`。可视化概览请使用 `moor status`。
:::
...
```bash
moor debug share              # 上传调试报告，打印 URL
moor debug share --lines 500  # 包含更多日志行
moor debug share --expire 30  # 粘贴保留 30 天
moor debug share --local      # 在终端打印报告（不上传）
```
...
创建 Moor 配置、skill、会话和数据的 zip 归档。备份不包含 moor-agent 代码库本身。

| 选项 | 说明 |
|--------|-------------|
| `-o`, `--output <path>` | zip 文件的输出路径（默认：`~/moor-backup-<timestamp>.zip`）。 |
...
- `moor-agent` 代码本身（这是用户数据备份，不是仓库快照）。

### 示例

```bash
moor backup                           # 完整备份到 ~/moor-backup-*.zip
moor backup -o /tmp/moor.zip        # 完整备份到指定路径
moor backup --quick                   # 仅状态快速快照
moor backup --quick --label "pre-upgrade"  # 带标签的快速快照
```
...
检查和管理 `~/.moor/checkpoints/` 处的影子 git 存储——会话内 `/rollback` 命令的存储层。可随时安全运行；不需要 agent 正在运行。

| 子命令 | 说明 |
|------------|-------------|
| `status`（默认） | 显示总大小、项目数量和每个项目的详情。裸 `moor checkpoints` 等同于此。 |
| `list` | `status` 的别名。 |
...
```bash
moor checkpoints                                  # 状态概览
moor checkpoints prune --retention-days 3         # 激进清理
moor checkpoints prune --max-size-mb 200          # 一次性收紧大小上限
moor checkpoints clear-legacy -f                  # 删除 v1 归档目录
moor checkpoints clear -f                         # 清除所有内容
```
...
### 示例
```bash
moor import ~/moor-backup-20260423.zip           # 覆盖现有配置前提示确认
moor import ~/moor-backup-20260423.zip --force   # 不提示直接覆盖
```
...
查看、跟踪和过滤 Moor 日志文件。所有日志存储在 `~/.moor/logs/`（非默认 profile 存储在 `<profile>/logs/`）。
...
Moor 使用 Python 的 `RotatingFileHandler`。旧日志会自动轮转——查找 `agent.log.1`、`agent.log.2` 等。`moor logs list` 子命令显示所有日志文件，包括已轮转的。
...
moor skills install https://sharethis.chat/SKILL.md                     # 直接 URL（含引用的支持文件）
moor skills install https://example.com/SKILL.md --name my-skill        # frontmatter 无名称时覆盖名称
moor skills check
moor skills update
moor skills config
...
Skill bundle 将多个 skill 归组到一个 `/<bundle-name>` 斜杠命令下。调用 bundle 会将每个引用的 skill 加载到单个合并的用户消息中。存储位置：`~/.moor/skill-bundles/<slug>.yaml`。YAML schema 和行为请参阅 [Skill Bundles](../user-guide/features/skills.md#skill-bundles)。
...
| `reload` | 重新扫描 `~/.moor/skill-bundles/` 并报告新增/删除的 bundle |
...
| `backup` | 手动对 `~/.moor/skills/` 进行 tar.gz 快照（curator 在每次真实运行前也会自动快照） |
| `rollback` | 从快照恢复 `~/.moor/skills/`（默认使用最新快照） |
...
在全新安装时，第一次计划运行会延迟一个完整的 `interval_hours`（默认 7 天）——gateway 不会在 `moor update` 后的第一次 tick 时立即执行 curator。使用 `moor curator run --dry-run` 在此之前预览。
...
| `add` | 选择 provider + 模型（与 `moor model` 相同的选择器）并追加到链末尾 |
...
检查 `~/.moor/config.yaml` 中声明的 shell 脚本 hook，针对合成 payload 测试它们，并管理 `~/.moor/shell-hooks-allowlist.json` 处的首次使用同意许可名单。
...
:::info Provider 特定子命令
当外部 memory provider 处于活跃状态时，它可能会注册自己的顶级 `moor <provider>` 命令用于 provider 特定管理（例如 Honcho 激活时的 `moor honcho`）。未激活的 provider 不暴露其子命令。运行 `moor --help` 查看当前已连接的命令。
:::
...
参见 [MCP 配置参考](./mcp-config-reference.md)、[在 Moor 中使用 MCP](../guides/use-mcp-with-moor.md) 和 [MCP 服务器模式](../user-guide/features/mcp.md#running-moor-as-an-mcp-server)。
...
统一的 plugin 管理——通用 plugin、memory provider 和 context engine 集于一处。不带子命令运行 `moor plugins` 会打开包含两个部分的复合交互界面：
...
参见 [Plugins](../user-guide/features/plugins.md) 和 [构建 Moor Plugin](../developer-guide/plugins/index.md)。
...
`moor computer-use install` 是安装 `computer_use` toolset 使用的 [cua-driver](https://github.com/trycua/cua) 二进制文件的稳定入口。它运行与首次启用 Computer Use 时 `moor tools` 调用的相同上游安装程序，因此如果 toolset 切换未触发安装（例如在已配置用户的设置中），可以安全地用于重新运行安装。

`moor update` 在更新结束时，如果 cua-driver 在 PATH 中，会自动重新运行上游安装程序，因此大多数用户不需要手动调用 `--upgrade`。当上游发布了你现在就想要的修复，而不想等待下次 Moor 更新时，使用此选项。
...
将 OpenClaw 设置迁移到 Moor。从 `~/.openclaw`（或自定义路径）读取并写入 `~/.moor`。自动检测旧版目录名（`~/.clawdbot`、`~/.moltbot`）和配置文件名（`clawdbot.json`、`moltbot.json`）。
...
| `--no-backup` | 跳过迁移前对 `~/.moor/` 的 zip 快照（默认情况下，在应用前会将单个还原点归档写入 `~/.moor/backups/pre-migration-*.zip`；可用 `moor import` 恢复）。 |
...
启动 Web 控制台——基于浏览器的界面，用于管理配置、API 密钥和监控会话。需要 `cd ~/.moor/moor-agent && uv pip install -e ".[web]"`（FastAPI + Uvicorn）。内嵌浏览器 Chat 标签页始终可用，但额外需要 `pty` extra（`cd ~/.moor/moor-agent && uv pip install -e ".[web,pty]"`）以及 POSIX PTY 环境（如 Linux、macOS 或 WSL2）。完整文档请参阅 [Web 控制台](/user-guide/features/web-dashboard)。
...
| `--stop` | — | 停止正在运行的 `moor dashboard` 进程并退出。 |
| `--status` | — | 列出正在运行的 `moor dashboard` 进程并退出。 |
...
拉取最新的 `moor-agent` 代码并在受管理的 venv 中重新安装依赖，然后重新运行安装后 hook（MCP 服务器、skill 同步、补全安装）。可在运行中的安装上安全执行。使用 `--check` 查看你的检出是否落后于 `origin/main`，而不安装。

| 选项 | 说明 |
|--------|-------------|
| `--check` | 并排打印当前 commit 和最新 `origin/main` commit，同步时退出码为 0，落后时为 1。不拉取、不安装、不重启任何内容。 |
| `--backup` | 在拉取前创建 `MOOR_HOME` 的带标签预更新快照（config、auth、会话、skill、配对数据）。默认**关闭**——之前的始终备份行为在大型主目录上每次更新会增加数分钟。通过 `config.yaml` 中的 `update.backup: true` 永久开启。 |
| `--restart-gateway` | 成功更新后重启正在运行的 gateway 服务。如果安装了多个 profile，隐含 `--all` 语义。 |

附加行为：

- **配对数据快照。** 即使 `--backup` 关闭，`moor update` 也会在 `git pull` 前对 `~/.moor/pairing/` 和 Feishu 评论规则进行轻量快照。如果拉取覆盖了你正在编辑的文件，可以用 `moor backup restore --state pre-update` 回滚。
- **旧版 `moor.service` 警告。** 如果 Moor 检测到预重命名的 `moor.service` systemd 单元（而非当前的 `moor-gateway.service`），会打印一次性迁移提示，帮助你避免循环重启问题。
...
|---------|-------------|
| `moor --version` | 打印版本信息。 |
| `moor update` | 拉取最新变更并重新安装依赖。 |
| `moor uninstall [--full] [--yes]` | 删除 Moor，可选择删除所有 config/数据。 |
