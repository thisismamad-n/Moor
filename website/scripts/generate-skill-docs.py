...
def render_skill_page(
    ...
    moor_meta = (fm.get("metadata") or {}).get("moor") or {}
    tags = moor_meta.get("tags") or []
    related = moor_meta.get("related_skills") or []
    ...
    if meta["source_kind"] == "bundled":
        ...
        info_rows.append(
            (
                "Source",
                "Optional — install with `moor skills install official/"
                + meta["category"]
                + "/"
                + meta["slug"]
                + "`",
...
def build_catalog_md_bundled(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Catalog of bundled skills that ship with Moor Agent"',
        "---",
        "",
        "# Bundled Skills Catalog",
        "",
        "Moor ships with a large built-in skill library copied into `~/.moor/skills/` on install. Each skill below links to a dedicated page with its full definition, setup, and usage.",
        "",
        "Moor also syncs bundled skills on `moor update`, but the sync manifest respects local deletions and user edits. If a skill listed here is missing from your profile's `~/.moor/skills/` tree, it is still shipped with Moor; restore it with `moor skills reset <name> --restore`.",
        "",
...
def build_catalog_md_optional(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Official optional skills shipped with moor-agent — install via moor skills install official/<category>/<skill>"',
        "---",
        "",
        "# Optional Skills Catalog",
        "",
        "Optional skills ship with moor-agent under `optional-skills/` but are **not active by default**. Install them explicitly:",
        "",
        "```bash",
