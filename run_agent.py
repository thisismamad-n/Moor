...
def _launch_cwd_for_session(source: str) -> Optional[str]:
    ...
    launched from is meaningful for ``moor -c`` / ``--resume`` (relaunch
...
def _session_source_for_agent(platform: Optional[str]) -> str:
    try:
        ...
        source = get_session_env("MOOR_SESSION_SOURCE", "")
    except Exception:
        source = os.environ.get("MOOR_SESSION_SOURCE", "")
...
_loaded_env_paths = load_moor_dotenv(moor_home=_moor_home, project_env=_project_env)
...
from agent.prompt_builder import (  # noqa: F401  # re-exported via _ra() / mock.patch("run_agent.<name>") / from run_agent import <name>
    ...
    build_moor_subscription_prompt,
    load_soul_md,
)
...
# (moor_state._rows_to_conversation), so any code that mutates a loaded or
...
def _safe_session_filename_component(session_id: str) -> str:
    ...
    ``X-moor-session-Id`` API header) and are otherwise interpolated raw
    into on-disk artifact filenames under ``~/.moor/sessions/``.  Without
...
class AIAgent:
    ...
    _TOOL_CALL_ARGUMENTS_CORRUPTION_MARKER = (
        "[moor-agent: tool call arguments were corrupted in this session and "
    ...
    def _ensure_db_session(self) -> None:
        ...
        try:
            ...
            # `moor --resume`. set_session_yolo() no-ops on a missing row, so
    ...
    def _ensure_lmstudio_runtime_loaded(
        ...
        from moor_cli.models import ensure_lmstudio_model_loaded
    ...
    def _vprint(self, *args, force: bool = False, **kwargs):
        ...
        parseable single-query flows such as ``moor chat -q``. In that mode,
    ...
    def _should_emit_quiet_tool_messages(self) -> bool:
        ...
        ``moor chat -Q``) always wins: those flows neutralize the rendering
    ...
    def _resolved_api_call_timeout(self) -> float:
        ...
        Priority:
          ...
          3. ``MOOR_API_TIMEOUT`` env var (legacy escape hatch)
          4. 1800.0s default
        ...
        helper, the hardcoded ``MOOR_API_TIMEOUT`` fallback would always be
        ...
        if cfg is not None:
            return cfg
        return env_float("MOOR_API_TIMEOUT", 1800.0)
    ...
    def _resolved_api_call_stale_timeout_base(self) -> tuple[float, bool]:
        ...
        Priority:
          ...
          3. ``MOOR_API_CALL_STALE_TIMEOUT`` env var
        ...
        env_timeout = os.getenv("MOOR_API_CALL_STALE_TIMEOUT")
    ...
    def _stale_timeout_is_explicit(self) -> bool:
        ...
        the ``MOOR_API_CALL_STALE_TIMEOUT`` env var. Reasoning-model floors
        ...
        if get_provider_stale_timeout(self.provider, self.model) is not None:
            return True
        return os.getenv("MOOR_API_CALL_STALE_TIMEOUT") is not None
    ...
    def _codex_silent_hang_hint(self, model: Optional[str] = None) -> Optional[str]:
        ...
        moor-agent #21444 for the symptom history.  The upstream backend
        ...
        return (
            ...
            "See moor-agent#21444 for symptom history."
        )
    ...
    def _provider_model_requires_responses_api(
        ...
        *,
        ...
        # Moor serves GPT-5.x models via its OpenAI-compatible chat
        # completions endpoint; its /v1/responses endpoint returns 404.
        if normalized_provider == "moor":
            return False
    ...
    def _decorate_xai_entitlement_error(detail: str) -> str:
        ...
        SuperGrok subscribers can use Moor against xai-oauth.  Lots of
    ...
    def _hook_payload_max_chars() -> int:
        raw = os.getenv("MOOR_PLUGIN_PAYLOAD_MAX_CHARS", "50000")
        try:
    ...
    def _invoke_api_request_error_hook(
        ...
        *,
        ...
        try:
            from moor_cli import lifecycle as _lifecycle
    ...
    def _redact_message_content(content):
        ...
        Respects ``MOOR_REDACT_SECRETS`` via ``redact_sensitive_text`` —
    ...
    def _save_session_log(self, messages: List[Dict[str, Any]] = None):
        ...
        whose external tooling consumes ``~/.moor/sessions/session_{sid}.json``
        ...
        # untrusted input (X-moor-session-Id header) and must not escape
        ...
        try:
            ...
            for msg in messages:
                ...
                # output, or user paste. Respects MOOR_REDACT_SECRETS via
    ...
    def _file_mutation_verifier_enabled(self) -> bool:
        ...
        ``MOOR_FILE_MUTATION_VERIFIER`` env var overrides config.  Exposed
        ...
        try:
            import os as _os
            env = _os.environ.get("MOOR_FILE_MUTATION_VERIFIER")
            if env is not None:
    ...
    def _format_file_mutation_failure_footer(cls, failed: Dict[str, Dict[str, Any]]) -> str:
        ...
        (e.g. ``~/.moor/config.yaml``) to a messaging channel (#35584).
        """
        if not failed:
            return ""
        lines = [
    ...
    def _turn_completion_explainer_enabled(self) -> bool:
        ...
        True).  ``MOOR_TURN_COMPLETION_EXPLAINER`` env var overrides
        ...
        try:
            import os as _os
            env = _os.environ.get("MOOR_TURN_COMPLETION_EXPLAINER")
            if env is not None:
                return env.strip().lower() not in {"0", "false", "no", "off"}
        ...
        if reason == "session_persistence_failed":
            ...
            if cause == "corrupt":
                return (
                    ...
                    "1. Run `moor doctor --fix`\n"
                    "2. Salvage with: sqlite3 ~/.moor/state.db \".recover\" "
                    "(then replace state.db)\n"
                    "3. Restore from a backup in ~/.moor/backups/\n"
            ...
            return (
                ...
                "Check the state database health (`moor doctor`), then "
    ...
    def _touch_activity(
        ...
        *,
        ...
        process is a dispatcher-spawned worker (MOOR_KANBAN_TASK set),
    ...
    def _capture_credits(self, http_response: Any) -> None:
        """Parse x-moor-credits-* headers, cache CreditsState, fire threshold notices.
        ...
        """
        # Dev test fixture (MOOR_DEV_CREDITS_FIXTURE): inject a chosen notice state
        ...
        if _fixture is not None:
            ...
            logger.info(
                ...
                "(real headers bypassed — `echo clear` / unset MOOR_DEV_CREDITS_FIXTURE to restore)",
        ...
        if not headers:
            return
        _dev = is_truthy_value(os.environ.get("MOOR_DEV_CREDITS"))
        ...
        if state is None:
            if _dev:
                logger.info(
                    "credits ▸ response had no valid x-moor-credits-* headers "
                    "(miss — producer off / non-Moor path / >TTL stale)"
                )
            return
        ...
        if _dev:
            # MOOR_DEV_CREDITS: stream each capture to agent.log — watch live with
            # `moor logs -f` (grep 'credits ▸'). Dev-only; silent for normal users.
    ...
    def _try_refresh_codex_client_credentials(self, *, force: bool = True) -> bool:
        ...
        # pool entry (``moor auth add xai-oauth``) whose tokens belong to
    ...
    def _try_refresh_moor_client_credentials(
        self,
        *,
        force: bool = True,
    ) -> bool:
        ...
        try:
            ...
            logger.debug("Moor credential refresh failed: %s", exc)
            return False
        ...
        self._client_kwargs["api_key"] = self.api_key
        self._client_kwargs["base_url"] = self.base_url
        # Moor requests should not inherit OpenRouter-only attribution headers.
        self._client_kwargs.pop("default_headers", None)

        if not self._replace_primary_openai_client(reason="moor_credential_refresh"):
            return False

        return True

    def _try_refresh_env_client_credentials(self) -> bool:
        """Adopt ~/.moor/.env credential/base-url edits at the turn boundary.

        A Settings save (desktop ``PUT /api/env``, ``moor setup``) updates
        ...
        if (
            ...
            if self.provider == "kimi-coding":
                from moor_cli.auth import _resolve_kimi_base_url

                base_url = _resolve_kimi_base_url(
                    api_key, pconfig.inference_base_url, env_url
                ).rstrip("/")
            elif self.provider == "zai":
                from moor_cli.auth import _resolve_zai_base_url
    ...
    def _apply_client_headers_for_base_url(
        ...
        *,
        ...
        if base_url_host_matches(base_url, "openrouter.ai"):
            ...
            self._client_kwargs["default_headers"] = moor_xai_default_headers()
        else:
    ...
    def _anthropic_messages_create(self, api_kwargs: dict, *, client: Any = None):
        ...
        return create_anthropic_message(
            ...
            # matching header families (x-ratelimit-* / x-moor-credits-*).
    ...
    def _supports_reasoning_extra_body(self) -> bool:
        ...
        known reasoning-capable model families and direct Moor Portal.
        """
    ...
    def _lmstudio_reasoning_options_cached(self) -> list[str]:
        ...
        if cached is not None:
            ...
            if opts or (_time.monotonic() - ts) < 60:
                return opts
        try:
            from moor_cli.models import lmstudio_model_reasoning_options
    ...
    def _ollama_supports_thinking_cached(self) -> bool:
        ...
        try:
            from moor_cli.models import ollama_model_supports_thinking
    ...
    def _compress_context(
        ...
        *,
        ...
        try:
            ...
            # id rotation updated moor_logging._session_context (a
            ...
            # MOOR_SESSION_ID to the child id after an out-of-place
    ...
    def run_conversation(
        ...
        from moor_cli.observability.relay_shared_metrics import (
            finish_task_run,
            start_task_run,
        )
        ...
        try:
            ...
            if getattr(relay_turn, "relay_enabled", True):
                ...
                task_started = True
            # Publish the conversation id for ambient Moor Portal tagging. Every
