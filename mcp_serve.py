"""
Moor MCP Server — expose messaging conversations as MCP tools.
...
Plus: channels_list (Moor-specific extra)

Usage:
    moor mcp serve
    moor mcp serve --verbose
...
logger = logging.getLogger("moor.mcp_serve")
...
def _get_sessions_dir() -> Path:
    """Return the sessions directory using MOOR_HOME."""
    try:
        from moor_constants import get_moor_home
        return get_moor_home() / "sessions"
    except ImportError:
        return Path(os.environ.get("MOOR_HOME", Path.home() / ".moor")) / "sessions"
...
def _load_channel_directory() -> dict:
    ...
    try:
        from moor_constants import get_moor_home
        directory_file = get_moor_home() / "channel_directory.json"
    except ImportError:
        directory_file = Path(
            os.environ.get("MOOR_HOME", Path.home() / ".moor")
...
class EventBridge:
    ...
    This is the Moor equivalent of OpenClaw's WebSocket gateway bridge.
    ...
    def _establish_baseline_with_db(self, db) -> None:
        ...
        try:
            from moor_constants import get_moor_home
            db_file = get_moor_home() / "state.db"
        except ImportError:
            db_file = Path(os.environ.get("MOOR_HOME", Path.home() / ".moor")) / "state.db"
        try:
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "MCPServer":
    ...
    mcp = MCPServer(
        "moor",
        instructions=(
            "Moor Agent messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the Moor MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
        print(
