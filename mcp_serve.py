"""
Moor MCP Server — expose messaging conversations as MCP tools (`moor mcp serve`).
...
Matches OpenClaw's 9-tool channel bridge surface plus the moor-specific
channels_list. Client config: {"mcpServers": {"moor": {"command": "moor", "args": ["mcp", "serve"]}}}
"""
...
def _moor_home() -> Path:
    try:
        ...
        return Path(os.environ.get("MOOR_HOME", Path.home() / ".moor"))


def _get_sessions_dir() -> Path:
    return _moor_home() / "sessions"


def _read_state_db_mtime() -> float:
    try:
        return (_moor_home() / "state.db").stat().st_mtime
...
def _load_channel_directory() -> dict:
    ...
    return _read_json(_moor_home() / "channel_directory.json")
...
class EventBridge:
    ...
    event queue with waiter support (the Moor analogue of OpenClaw's WebSocket
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "MCPServer":
    ...
    mcp = MCPServer("moor", instructions=(
