# nix/moor-agent.nix — Overridable Moor Agent package
#
...
let
  ...
  mkHermesVenv =
    ...
    callPackage ./python.nix {
      ...
      pythonSrc = moorNpmLib.pythonSrc;
  ...
  # lib.nix) — the wrapper exposes them via MOOR_OPTIONAL_SKILLS, the
  ...
  # to a dev checkout — the loader reads them from MOOR_BUNDLED_PLUGINS.
  ...
  # at by MOOR_BUNDLED_LOCALES so the wrapped binary always resolves human
  ...
  # exposed via MOOR_OPTIONAL_MCPS.
  ...
  checkPackageCollisions = ''
    ...
    core = set()
    venv_sp = pathlib.Path('${moorVenv}/${sitePackagesPath}')
        ...
        for di in sp.glob('*.dist-info'):
            ...
            for line in meta.read_text().splitlines():
                if line.startswith('Name:'):
                    ...
                    if pkg in core:
                        print(f'ERROR: plugin package \"{pkg}\" collides with a package in moor sealed venv', file=sys.stderr)
...
stdenv.mkDerivation (finalAttrs: {
  ...
  installPhase = ''
    ...
    ${lib.optionalString (extraPythonPackages != [ ]) ''
      ...
      ${moorVenv}/bin/python3 -c "${checkPackageCollisions}"
      echo "=== No collisions ==="
    ''}
  ...
  passthru =
    let
      devPython = (mkMoorVenv (extraDependencyGroups ++ [ "dev" ])).editableVenv;
    in
    {
      inherit
        hermesTui
      ...
      devShellHook = ''
        export MOOR_PYTHON=${devPython}/bin/python3
      '';
