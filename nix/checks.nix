...
{ inputs, ... }: {
  perSystem = { pkgs, lib, self', ... }:
    let
      moor-agent = self'.packages.default;
      moorVenv = moor-agent.moorVenv;
      ...
      evalHomeSplit =
        ...
        inputs.home-manager.lib.homeManagerConfiguration {
          ...
          modules = [
            ...
            {
              programs.moor-agent = programs;
              services.moor-agent = services;
            }
          ];
        };

      # The option names that each module defines under
      # services.moor-agent. The internal names that the module system adds
      # are not in the list.
      moduleOptionNames =
        eval: lib.attrNames (lib.filterAttrs (n: _: !lib.hasPrefix "_" n) eval.options.services.moor-agent);
      ...
      configKeys = pkgs.runCommand "moor-config-keys" {} ''
        set -euo pipefail
        export HOME=$TMPDIR
        ${moorVenv}/bin/python3 -c '
import json, sys
...
' > $out
    ...
    in {
      ...
      checks = {
        ...
        in pkgs.runCommand "moor-cross-eval" { } (
          if failures != [] then
        ...
        build-package = pkgs.runCommand "moor-build-package" { } ''
          echo "PASS: package built at ${moor-agent}"
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify the devShell builds successfully (cross-platform).
        build-devshell = pkgs.runCommand "moor-build-devshell" { } ''
          ...
          let
            enabled = evalHomeSplit {
              ...
              services = {
                ...
                environment.MOOR_TEST = "1";
                environmentFiles = [ "/run/secrets/moor-env" ];
                moorHomeFiles."SOUL.md" = "test soul";
            ...
            processes =
              if pkgs.stdenv.hostPlatform.isDarwin then
                ...
                }) (lib.filterAttrs (n: _: lib.hasPrefix "moor" n) cfg.launchd.agents)
              else
                lib.mapAttrs (_: unit: {
                  argv = [ unit.Service.ExecStart ];
                  env = unit.Service.Environment;
                }) (lib.filterAttrs (n: _: lib.hasPrefix "moor" n) cfg.systemd.user.services);
            ...
            activation = cfg.home.activation.moorAgentSetup.data;

            failures =
              lib.optional (names != [
                "moor-agent"
                "moor-backend"
              ]) "expected moor-agent + moor-backend processes, got: ${toString names}"
              ++ lib.optional (
                !lib.hasInfix "bin/moor gateway" (argvOf "moor-agent")
              ) "gateway process does not run `moor gateway`: ${argvOf "moor-agent"}"
              ++ lib.optional (
                !lib.hasInfix "bin/moor serve" (argvOf "moor-backend")
              ) "backend process does not run `moor serve`: ${argvOf "moor-backend"}"
              ++ lib.optional (
                !lib.hasInfix "--no-open" (argvOf "moor-backend")
              ) "backend must pass --no-open so a service never opens a browser"
              ++ lib.optional (
                lib.any (n: !lib.hasInfix "/home/moor-check/.moor" (envOf n)) names
              ) "gateway and backend must share one MOOR_HOME"
              ++ lib.optional (
                cfg.home.sessionVariables.MOOR_HOME or null != "/home/moor-check/.moor"
              ) "programs.moor-agent.enable must export MOOR_HOME for interactive shells"
              ++ lib.optional (
                !lib.hasInfix "moor-config-merge" activation
              ) "activation must deep-merge config.yaml, not overwrite it"
              ++ lib.optional (
                !lib.hasInfix "/home/moor-check/.moor/SOUL.md" activation
              ) "moorHomeFiles must install into MOOR_HOME"
              ++ lib.optional (
                !lib.hasInfix "/home/test-user/workspace/AGENTS.md" activation
              ) "documents must install into workingDirectory"
              # The CLI reads MOOR_MANAGED to name the rebuild command when
              # it refuses to write the configuration. A Home Manager install
              # has no nixos-rebuild command. Thus it must not report NixOS.
              ++ lib.optional (
                !lib.any (n: lib.hasInfix "MOOR_MANAGED=home-manager" (envOf n)) names
              ) "processes must report MOOR_MANAGED=home-manager"
              ++ lib.optional (
                !lib.hasInfix "moor-managed" activation
              ) "activation must write a .managed marker naming the managing system";
          in
          pkgs.runCommand "moor-home-manager-module" { } (
            if failures != [ ] then
        ...
        workspace-files-need-a-directory =
          let
            ...
            cases = [
              ...
              {
                name = "moorHomeFiles needs no directory";
                ok = accepts { moorHomeFiles."SOUL.md" = "x"; };
              }
              {
          ...
          in
          pkgs.runCommand "moor-workspace-files-need-a-directory" { } (
            if failed != [ ] then
        ...
        # ── The desktop application shares one MOOR_HOME ───────────────
        # `programs.enable` exports MOOR_HOME with home.sessionVariables,
        ...
        # no shell profile. Thus the desktop application would open ~/.moor
        # while the services use the MOOR_HOME of the module, and the user
        ...
        home-manager-desktop =
          let
            tokenFile = "/run/secrets/moor-desktop-token";
            ...
            enabled = evalHomeSplit {
              ...
              services = {
                enable = true;
                moorHome = "/home/moor-check/.moor-work";
            ...
            desktopPackages = builtins.filter (p: (p.pname or "") == "moor-desktop") cfg.home.packages;
            ...
            # second Moor runtime beside the services is the fault that
            # `programs.enable` plus a plain desktop package would give.
            agentPackages = builtins.filter (p: (p.pname or "") == "moor-agent") cfg.home.packages;
            ...
            backendScript =
              let
                argv =
                  if pkgs.stdenv.hostPlatform.isDarwin then
                    cfg.launchd.agents.moor-backend.config.ProgramArguments
                  else
                    [ cfg.systemd.user.services.moor-backend.Service.ExecStart ];
            ...
            failures =
              ...
              ) "programs.desktop.enable must install exactly one moor-desktop package, got ${toString (lib.length desktopPackages)}"
              ++ lib.optional (
                setValue "MOOR_HOME" != "/home/moor-check/.moor-work"
              ) "the launcher must carry MOOR_HOME: a GUI launcher reads no shell profile, so home.sessionVariables never reaches it (got: ${toString (setValue "MOOR_HOME")})"
              ++ lib.optional (
                setValue "MOOR_MANAGED" != "home-manager"
              ) "the launcher must report MOOR_MANAGED=home-manager while the services own the configuration (got: ${toString (setValue "MOOR_MANAGED")})"
              ++ lib.optional (
                lib.length agentPackages == 1
                && setValue "MOOR_DESKTOP_MOOR" != "${lib.head agentPackages}/bin/moor"
              ) "the launcher must pin the agent package that programs.enable installs, and not a second runtime: ${toString (setValue "MOOR_DESKTOP_MOOR")}"

              # ── The application reaches the backend of the service ──────
              ++ lib.optional (
                setValue "MOOR_DESKTOP_REMOTE_URL" != "http://127.0.0.1:9231"
              ) "the launcher must name the backend of the service, or the application starts a second one (got: ${toString (setValue "MOOR_DESKTOP_REMOTE_URL")})"
              ++ lib.optional (
                !lib.hasInfix "MOOR_DESKTOP_REMOTE_TOKEN" wrapper
              ) "the launcher must give a token with the URL: the desktop resolver throws when the URL is set alone"
              ++ lib.optional (
                !lib.hasInfix "MOOR_DASHBOARD_SESSION_TOKEN" backendScript
              ...
              ++ lib.optional (
                setValue "MOOR_DESKTOP_REMOTE_TOKEN" != null
              ) "the token must never be a --set value: makeWrapper writes it into the world-readable Nix store";
          in
          pkgs.runCommand "moor-home-manager-desktop" { } (
            if failures != [ ] then
              throw "Home Manager desktop check failed:\n${lib.concatMapStringsSep "\n" (f: "  - ${f}") failures}"
            else
              ''
                echo "PASS: the desktop launcher shares MOOR_HOME, the runtime and the backend of the service"
                mkdir -p $out
                echo "ok" > $out/result
              ''
          );
        ...
        home-manager-desktop-standalone =
          let
            ...
            failures =
              ...
              ++ lib.optional (
                !lib.hasInfix "--set MOOR_HOME" wrapper
              ) "the launcher must carry MOOR_HOME even with no services"
              ++ lib.optional (
                lib.hasInfix "MOOR_MANAGED" wrapper
              ) "the launcher must not claim a managed install when no activation writes one"
              ++ lib.optional (
                lib.hasInfix "MOOR_DESKTOP_REMOTE_URL" wrapper
              ) "the launcher must not name a backend when the services run none"
              ++ lib.optional (
                cfg.systemd.user.services ? moor-backend || cfg.launchd.agents ? moor-backend
              ) "programs.enable alone must start no service";
          in
          pkgs.runCommand "moor-home-manager-desktop-standalone" { } (
            if failures != [ ] then
        ...
        # command line. A silent removal thus leaves them with no `moor`
        ...
        home-manager-install-package-removed =
          let
            ...
            cases = [
              {
                value = true;
                expect = "programs.moor-agent.enable = true;";
              }
              {
                value = false;
                expect = "programs.moor-agent.enable = false;";
              }
            ];
          ...
          in
          pkgs.runCommand "moor-home-manager-install-package-removed" { } (
            if failures != [ ] then
        ...
        module-option-parity =
          ...
          in
          pkgs.runCommand "moor-module-option-parity" { } (
            if failures != [ ] then
        ...
        nixos-module =
          let
            ...
            units = lib.filterAttrs (n: _: lib.hasPrefix "moor" n) cfg.systemd.services;
            names = lib.attrNames units;
            execOf = name: units.${name}.serviceConfig.ExecStart;
            activation = cfg.system.activationScripts."moor-agent-setup".text;

            failures =
              lib.optional (names != [
                "moor-agent"
                "moor-backend"
              ]) "expected moor-agent + moor-backend units, got: ${toString names}"
              ++ lib.optional (
                !lib.hasInfix "bin/moor gateway" (execOf "moor-agent")
              ) "gateway unit does not run `moor gateway`: ${execOf "moor-agent"}"
              ++ lib.optional (
                !lib.hasInfix "bin/moor dashboard" (execOf "moor-backend")
              ) "backend unit does not run `moor dashboard`: ${execOf "moor-backend"}"
              ++ lib.optional (
                units.moor-agent.environment.MOOR_HOME != units.moor-backend.environment.MOOR_HOME
              ) "gateway and backend must share one MOOR_HOME"
              ++ lib.optional (
                !lib.hasInfix "/var/lib/moor/.moor/SOUL.md" activation
              ) "moorHomeFiles must install into MOOR_HOME";
          ...
          in
          pkgs.runCommand "moor-nixos-module" { } (
            if failures != [ ] then
        ...
        # polls and then execs moor, and the assertions reject a
        # configuration that cannot work.
        backend-bind-wait =
          let
            execOf =
              settings:
              (evalNixosModule ({ enable = true; } // settings)).config.systemd.services.moor-backend.serviceConfig.ExecStart;
            ...
            failures =
              ...
              lib.optional (!lib.hasInfix "bin/moor serve --host 127.0.0.1" direct)
                "without waitFor the backend must exec moor directly, got: ${direct}"
              ++ lib.optional (lib.hasInfix "moor-backend-launch" direct)
                "without waitFor the backend must not use the launcher"

              # The hostname mode polls the resolver, then binds the name.
              ++ lib.optional (!lib.hasInfix "moor-backend-launch" hostnameWait)
              ...
              ++ lib.optional (!lib.hasInfix "exec " hostnameScript)
                "the launcher must exec moor, so that it keeps the MainPID"
              ...
              ++ lib.optional (!lib.hasInfix "bin/moor dashboard" interfaceScript)
          ...
          in
          pkgs.runCommand "moor-backend-bind-wait" { } (
            if failures != [ ] then
        ...
        # $MOOR_HOME/.env. The important property is that a second run
        ...
        env-file-assembly =
          ...
          in
          pkgs.runCommand "moor-env-file-assembly" { } ''
            set -e
            workdir=$(mktemp -d)
            ...
            grep -qx 'MOOR_PUBLIC=visible' "$workdir/.env" || \
        ...
        service-argv =
          let
            ...
            sentinel = "--moor-nix-argv-probe";
            probe = argv: lib.escapeShellArgs (argv ++ [ sentinel ]);
          in
          pkgs.runCommand "moor-service-argv" { } ''
            set -e
        ...
        package-contents = pkgs.runCommand "moor-package-contents" { } ''
          set -e
          echo "=== Checking binaries ==="
          test -x ${moor-agent}/bin/moor || (echo "FAIL: moor binary missing"; exit 1)
          test -x ${moor-agent}/bin/moor-agent || (echo "FAIL: moor-agent binary missing"; exit 1)
          echo "PASS: All binaries present"

          echo "=== Checking version ==="
          ${moor-agent}/bin/moor --version 2>&1 | grep -qi "moor" || (echo "FAIL: version check"; exit 1)
        ...
        entry-points-sync = pkgs.runCommand "moor-entry-points-sync" { } ''
          ...
          for bin in moor moor-agent moor-acp; do
            test -x ${moor-agent}/bin/$bin || (echo "FAIL: $bin binary missing from Nix package"; exit 1)
        ...
        cli-commands = pkgs.runCommand "moor-cli-commands" { } ''
          set -e
          export HOME=$(mktemp -d)

          echo "=== Checking moor --help ==="
          ${moor-agent}/bin/moor --help 2>&1 | grep -q "gateway" || (echo "FAIL: gateway subcommand missing"; exit 1)
          ${moor-agent}/bin/moor --help 2>&1 | grep -q "config" || (echo "FAIL: config subcommand missing"; exit 1)
        ...
        bundled-skills = pkgs.runCommand "moor-bundled-skills" { } ''
          set -e
          echo "=== Checking bundled skills ==="
          test -d ${moor-agent}/share/moor-agent/skills || (echo "FAIL: skills directory missing"; exit 1)
          echo "PASS: skills directory exists"

          # -L: skills/ is a symlink to the filtered source store path
          SKILL_COUNT=$(find -L ${moor-agent}/share/moor-agent/skills -name "SKILL.md" | wc -l)
          test "$SKILL_COUNT" -gt 0 || (echo "FAIL: no SKILL.md files found in skills directory"; exit 1)
          echo "PASS: $SKILL_COUNT bundled skills found"

          grep -q "MOOR_BUNDLED_SKILLS" ${moor-agent}/bin/moor || \
            (echo "FAIL: MOOR_BUNDLED_SKILLS not in wrapper"; exit 1)
          echo "PASS: MOOR_BUNDLED_SKILLS set in wrapper"

          # Optional skills ship via the wrapper too (pythonSrc excludes
          # them from the wheel, so the env var is the only path in nix).
          test -d ${moor-agent}/share/moor-agent/optional-skills || \
            (echo "FAIL: optional-skills directory missing"; exit 1)
          OPT_COUNT=$(find -L ${moor-agent}/share/moor-agent/optional-skills -name "SKILL.md" | wc -l)
          test "$OPT_COUNT" -gt 0 || (echo "FAIL: no SKILL.md files in optional-skills"; exit 1)
          grep -q "MOOR_OPTIONAL_SKILLS" ${moor-agent}/bin/moor || \
            (echo "FAIL: MOOR_OPTIONAL_SKILLS not in wrapper"; exit 1)
          echo "PASS: $OPT_COUNT optional skills found, MOOR_OPTIONAL_SKILLS set in wrapper"
        ...
        bundled-plugins = pkgs.runCommand "moor-bundled-plugins" { } ''
          set -e
          echo "=== Checking bundled plugins ==="
          test -d ${moor-agent}/share/moor-agent/plugins || (echo "FAIL: plugins directory missing"; exit 1)
          echo "PASS: plugins directory exists"

          test -f ${moor-agent}/share/moor-agent/plugins/platforms/irc/plugin.yaml || \
            (echo "FAIL: irc plugin manifest missing"; exit 1)
          echo "PASS: irc plugin manifest present"

          grep -q "MOOR_BUNDLED_PLUGINS" ${moor-agent}/bin/moor || \
            (echo "FAIL: MOOR_BUNDLED_PLUGINS not in wrapper"; exit 1)
          echo "PASS: MOOR_BUNDLED_PLUGINS set in wrapper"
        ...
        bundled-locales = pkgs.runCommand "moor-bundled-locales" { } ''
          set -e
          echo "=== Checking bundled locales ==="
          test -d ${moor-agent}/share/moor-agent/locales || (echo "FAIL: locales directory missing"; exit 1)
          echo "PASS: locales directory exists"

          # -L: locales/ is a symlink to the source store path
          LOC_COUNT=$(find -L ${moor-agent}/share/moor-agent/locales -name "*.yaml" | wc -l)
          test "$LOC_COUNT" -ge 16 || (echo "FAIL: expected >=16 catalogs, found $LOC_COUNT"; exit 1)
          echo "PASS: $LOC_COUNT locale catalogs found"

          test -f ${moor-agent}/share/moor-agent/locales/en.yaml || (echo "FAIL: en.yaml missing"; exit 1)
          echo "PASS: en.yaml present"

          grep -q "MOOR_BUNDLED_LOCALES" ${moor-agent}/bin/moor || \
            (echo "FAIL: MOOR_BUNDLED_LOCALES not in wrapper"; exit 1)
          echo "PASS: MOOR_BUNDLED_LOCALES set in wrapper"

          # locales/ is a bare data dir (no __init__.py), shipped via a
          # symlink + MOOR_BUNDLED_LOCALES (not via wheel data-files).
          # Verify the wrapper override resolves real strings.
          export HOME=$(mktemp -d)
          RENDERED=$(cd "$HOME" && MOOR_BUNDLED_LOCALES=${moor-agent}/share/moor-agent/locales \
            ${moorVenv}/bin/python3 -c "from agent import i18n; print(i18n.t('gateway.reset.header_default', lang='en'))")
          echo "rendered: $RENDERED"
          test "$RENDERED" != "gateway.reset.header_default" || (echo "FAIL: i18n returned the raw key with MOOR_BUNDLED_LOCALES set"; exit 1)
        ...
        # MOOR_OPTIONAL_MCPS (not via wheel data-files).
        bundled-mcps = pkgs.runCommand "moor-bundled-mcps" { } ''
          set -e
          echo "=== Checking bundled optional-mcps ==="
          test -d ${moor-agent}/share/moor-agent/optional-mcps || (echo "FAIL: optional-mcps directory missing"; exit 1)
          echo "PASS: optional-mcps directory exists"

          MANIFEST_COUNT=$(find -L ${moor-agent}/share/moor-agent/optional-mcps -name "manifest.yaml" | wc -l)
          test "$MANIFEST_COUNT" -gt 0 || (echo "FAIL: no manifest.yaml files found"; exit 1)
          echo "PASS: $MANIFEST_COUNT catalog manifests found"

          grep -q "MOOR_OPTIONAL_MCPS" ${moor-agent}/bin/moor || \
            (echo "FAIL: MOOR_OPTIONAL_MCPS not in wrapper"; exit 1)
          echo "PASS: MOOR_OPTIONAL_MCPS set in wrapper"

          export HOME=$(mktemp -d)
          CATALOG=$(cd "$HOME" && ${moor-agent}/bin/moor mcp catalog 2>/dev/null || true)
          echo "catalog output: $CATALOG"
          test -n "$CATALOG" || (echo "FAIL: moor mcp catalog returned empty"; exit 1)
        ...
        bundled-tui = pkgs.runCommand "moor-bundled-tui" { } ''
          set -e
          echo "=== Checking bundled TUI ==="
          test -d ${moor-agent}/ui-tui || (echo "FAIL: ui-tui directory missing"; exit 1)
          echo "PASS: ui-tui directory exists"

          test -f ${moor-agent}/ui-tui/dist/entry.js || (echo "FAIL: compiled entry.js missing"; exit 1)
          echo "PASS: compiled entry.js present"

          # self-contained bundle; no runtime node_modules expected

          grep -q "MOOR_TUI_DIR" ${moor-agent}/bin/moor || \
            (echo "FAIL: MOOR_TUI_DIR not in wrapper"; exit 1)
          echo "PASS: MOOR_TUI_DIR set in wrapper"
        ...
        # Verify MOOR_NODE is set in wrapper and points to Node 26+
        # (Moor pins its toolchain to Node 26 everywhere)
        moor-node = pkgs.runCommand "moor-node-version" { } ''
          set -e
          echo "=== Checking MOOR_NODE in wrapper ==="
          grep -q "MOOR_NODE" ${moor-agent}/bin/moor || \
            (echo "FAIL: MOOR_NODE not set in wrapper"; exit 1)
          echo "PASS: MOOR_NODE present in wrapper"

          MOOR_NODE=$(sed -n "s/^export MOOR_NODE='\(.*\)'/\1/p" ${moor-agent}/bin/moor)
          test -x "$MOOR_NODE" || (echo "FAIL: MOOR_NODE=$MOOR_NODE not executable"; exit 1)
          echo "PASS: MOOR_NODE executable at $MOOR_NODE"

          NODE_MAJOR=$("$MOOR_NODE" --version | sed 's/^v//' | cut -d. -f1)
          test "$NODE_MAJOR" -ge 26 || \
            (echo "FAIL: Node v$NODE_MAJOR < 26, Moor requires Node 26"; exit 1)
          echo "PASS: Node v$NODE_MAJOR >= 26"

          echo "=== All MOOR_NODE checks passed ==="
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify MOOR_MANAGED guard works on all mutation commands
        managed-guard = pkgs.runCommand "moor-managed-guard" { } ''
          set -e
          export HOME=$(mktemp -d)
          ...
          check_blocked() {
            local label="$1"
            shift
            OUTPUT=$(MOOR_MANAGED=true "$@" 2>&1 || true)
          ...
          echo "=== Checking MOOR_MANAGED guards ==="
          check_blocked "config set" ${moor-agent}/bin/moor config set model foo
          check_blocked "config edit" ${moor-agent}/bin/moor config edit
        ...
        in pkgs.runCommand "moor-extra-python-packages" { } ''
          ...
          if grep -q "PYTHONPATH" ${moor-agent}/bin/moor; then
        ...
        extra-dependency-groups = let
          moorWithGroups = moor-agent.override {
            extraDependencyGroups = [ "honcho" ];
          };
        in pkgs.runCommand "moor-extra-dependency-groups" { } ''
          set -e
          ...
          echo "derivation: ${moorWithGroups}"
          echo "venv: ${moorWithGroups.moorVenv}"
        ...
        messaging-variant = pkgs.runCommand "moor-messaging-variant" { } ''
          set -e
          echo "=== Checking discord.py importable from messaging variant ==="
          ${self'.packages.messaging.moorVenv}/bin/python3 -c \
        ...
        in pkgs.runCommand "moor-config-roundtrip" {
          ...
          merge_and_load() {
            local moor_home="$1"
            export MOOR_HOME="$moor_home"
            ${configMergeScript} ${nixSettings} "$moor_home/config.yaml"
            ${moorVenv}/bin/python3 -c '
import json, sys
