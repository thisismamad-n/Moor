# nix/nixosModules.nix — the NixOS module for moor-agent
#
...
{ inputs, ... }:
{
  flake.nixosModules.default =
    ...
    let
      ...
      containerModeFile = pkgs.writeText "moor-container-mode" ''
      ...
      // common.processEnvironment { inherit moorHome; };
      ...
      config = lib.mkIf cfg.enable (
        lib.mkMerge [
          ...
          (lib.mkIf cfg.addToSystemPackages {
            ...
            environment.variables.MOOR_HOME = moorHome;
          })
              ...
              ++ [
                {
                  ...
                  message = "services.moor-agent: backend.mode is not supported together with container.enable — the container runs the gateway only.";
                }
              ];
          }
          ...
          # ── Directories ───────────────────────────────────────────────────
          {
            systemd.tmpfiles.rules = [
              ...
              "d ${moorHome}                  2770 ${cfg.user} ${cfg.group} - -"
              "d ${cfg.stateDir}/home           0750 ${cfg.user} ${cfg.group} - -"
              "d ${cfg.workingDirectory}        2770 ${cfg.user} ${cfg.group} - -"
            ]
            ++ map (d: "d ${moorHome}/${d} 2770 ${cfg.user} ${cfg.group} - -") common.stateSubdirs;
          }

          # ── Activation: link config + auth + documents ────────────────────
          {
            system.activationScripts."moor-agent-setup" =
              lib.stringAfter
                (
                ...
                ''
                  ...
                  chown ${cfg.user}:${cfg.group} ${cfg.stateDir} ${moorHome} ${cfg.stateDir}/home ${cfg.workingDirectory}
                  chmod 2770 ${cfg.stateDir} ${moorHome} ${cfg.workingDirectory}
                  ...
                  find ${moorHome} -maxdepth 1 \
                    \( -name "*.db" -o -name "*.db-wal" -o -name "*.db-shm" -o -name "SOUL.md" \) \
                    -exec chmod g+rw {} + 2>/dev/null || true
                  for _subdir in ${lib.concatStringsSep " " common.stateSubdirs}; do
                    mkdir -p "${moorHome}/$_subdir"
                    chown ${cfg.user}:${cfg.group} "${moorHome}/$_subdir"
                    chmod 2770 "${moorHome}/$_subdir"
                    find "${moorHome}/$_subdir" -type f \
                      -exec chmod g+rw {} + 2>/dev/null || true
                  done

                  ${common.mkStateScript {
                    inherit pkgs cfg moorHome;
                    workingDirectory = cfg.workingDirectory;
                  ...
                  chown -h ${cfg.user}:${cfg.group} ${moorHome}/plugins/nix-managed-* 2>/dev/null || true
                  ...
                  ${
                    if cfg.container.enable then
                      ''
                        install -o ${cfg.user} -g ${cfg.group} -m 0644 ${containerModeFile} ${moorHome}/.container-mode
                      ''
                    else
                      ''
                        rm -f ${moorHome}/.container-mode
                        ...
                        ${lib.concatStringsSep "\n" (
                          map (
                            ...
                            in
                            ''
                              if [ -L "${symlinkPath}" ] && [ "$(readlink "${symlinkPath}")" = "${moorHome}" ]; then
                                rm -f "${symlinkPath}"
