# nix/nixosModules.nix — NixOS module for moor-agent
#
# Two modes:
#   container.enable = false (default) → native systemd service
#   container.enable = true            → OCI container (persistent writable layer)
#
# Container mode: moor runs from /nix/store bind-mounted read-only into a
...
# container recreation. Environment variables are written to $MOOR_HOME/.env
# and read by moor at startup — no container recreation needed for env changes.
#
# Tool resolution: the moor wrapper uses --suffix PATH for nix store tools,
...
{ inputs, ... }: {
  ...
  let
    ...
    # Deep-merge config type (from 0xrsydn/nix-moor-agent)
    deepConfigType = lib.types.mkOptionType {
      name = "moor-config-attrs";
    ...
    # terminal.cwd replaces the deprecated MESSAGING_CWD env var — moor
    ...
    generatedConfigFile = pkgs.writeText "moor-config.yaml" configJson;
    configFile = if cfg.configFile != null then cfg.configFile else generatedConfigFile;

    configMergeScript = pkgs.callPackage ./configMergeScript.nix { };

    # config.yaml mode: group-writable (0660) when interactive users share this
    # MOOR_HOME via addToSystemPackages, so they can save settings through the
    ...
    documentDerivation = pkgs.runCommand "moor-documents" { } (
      ''
        mkdir -p $out
      '' + lib.concatStringsSep "\n" (
        lib.mapAttrsToList (name: value:
          ...
          else "cat > $out/${name} <<'MOOR_DOC_EOF'\n${value}\nMOOR_DOC_EOF"
        ) cfg.documents
      )
    );

    containerName = "moor-agent";
    containerDataDir = "/data";     # stateDir mount point inside container
    containerHomeDir = "/home/moor";
    ...
    # moor user + sudo on first boot (writable layer persists), then
    # drops privileges. Supports arbitrary base images (Debian, Alpine, etc).
    containerEntrypoint = pkgs.writeShellScript "moor-container-entrypoint" ''
      set -eu

      MOOR_UID="''${MOOR_UID:?MOOR_UID must be set}"
      MOOR_GID="''${MOOR_GID:?MOOR_GID must be set}"

      # ── Group: ensure a group with GID=$MOOR_GID exists ──
      # Check by GID (not name) to avoid collisions with pre-existing groups
      # (e.g. GID 100 = "users" on Ubuntu)
      EXISTING_GROUP=$(getent group "$MOOR_GID" 2>/dev/null | cut -d: -f1 || true)
      if [ -n "$EXISTING_GROUP" ]; then
        GROUP_NAME="$EXISTING_GROUP"
      else
        GROUP_NAME="moor"
        if command -v groupadd >/dev/null 2>&1; then
          groupadd -g "$MOOR_GID" "$GROUP_NAME"
        elif command -v addgroup >/dev/null 2>&1; then
          addgroup -g "$MOOR_GID" "$GROUP_NAME" 2>/dev/null || true
        fi
      fi

      # ── User: ensure a user with UID=$MOOR_UID exists ──
      PASSWD_ENTRY=$(getent passwd "$MOOR_UID" 2>/dev/null || true)
      if [ -n "$PASSWD_ENTRY" ]; then
        TARGET_USER=$(echo "$PASSWD_ENTRY" | cut -d: -f1)
        TARGET_HOME=$(echo "$PASSWD_ENTRY" | cut -d: -f6)
      else
        TARGET_USER="moor"
        TARGET_HOME="/home/moor"
        if command -v useradd >/dev/null 2>&1; then
          useradd -u "$MOOR_UID" -g "$MOOR_GID" -m -d "$TARGET_HOME" -s /bin/bash "$TARGET_USER"
        elif command -v adduser >/dev/null 2>&1; then
          adduser -u "$MOOR_UID" -D -h "$TARGET_HOME" -s /bin/sh -G "$GROUP_NAME" "$TARGET_USER" 2>/dev/null || true
        fi
      fi
      mkdir -p "$TARGET_HOME"
      chown "$MOOR_UID:$MOOR_GID" "$TARGET_HOME"
      chmod 0750 "$TARGET_HOME"

      # Ensure MOOR_HOME is owned by the target user.
      ...
      if [ -n "''${MOOR_HOME:-}" ] && [ -d "$MOOR_HOME" ]; then
        find "$MOOR_HOME" \! -user "$MOOR_UID" -exec chown "$MOOR_UID:$MOOR_GID" {} +
      fi
      ...
      if [ ! -f /var/lib/moor-tools-provisioned ] && command -v apt-get >/dev/null 2>&1; then
        ...
        touch /var/lib/moor-tools-provisioned
      fi

      if command -v sudo >/dev/null 2>&1 && [ ! -f /etc/sudoers.d/moor ]; then
        mkdir -p /etc/sudoers.d
        echo "$TARGET_USER ALL=(ALL) NOPASSWD:ALL" > /etc/sudoers.d/moor
        chmod 0440 /etc/sudoers.d/moor
      fi
      ...
      if command -v setpriv >/dev/null 2>&1; then
        exec setpriv --reuid="$MOOR_UID" --regid="$MOOR_GID" --init-groups "$@"
    ...
    # so they can update without recreation. Env vars go through $MOOR_HOME/.env.
    ...
    # Default: /var/lib/moor/workspace → /data/workspace.
  ...
  in {
    options.services.moor-agent = with lib; {
      ...
      package = mkOption {
        type = types.package;
        default = moor-agent;
        description = "The moor-agent package to use.";
      };
      ...
      stateDir = mkOption {
        type = types.str;
        default = "/var/lib/moor";
        description = "State directory. Contains .moor/ subdir (MOOR_HOME).";
      };
      ...
      environmentFiles = mkOption {
        ...
        description = ''
          ...
          Contents are merged into $MOOR_HOME/.env at activation time.
          Moor reads this file on every startup via load_moor_dotenv().
        '';
      };
      ...
      environment = mkOption {
        ...
        default = { };
        description = ''
          Non-secret environment variables. Merged into $MOOR_HOME/.env
          ...
          options = {
            ...
            auth = mkOption {
              ...
              description = ''
                ...
                (remote MCP servers). Tokens are stored in $MOOR_HOME/mcp-tokens/.
              '';
            };
      ...
      extraPackages = mkOption {
        ...
        description = ''
          ...
          Implemented via the moor user's per-user profile
      ...
      extraPlugins = mkOption {
        ...
        default = [ ];
        description = ''
          Directory-based plugin packages to symlink into the moor plugins
      ...
      addToSystemPackages = mkOption {
        ...
        default = false;
        description = ''
          Add the moor CLI to environment.systemPackages and export
          MOOR_HOME system-wide (via environment.variables) so interactive
      ...
      container = {
        ...
        hostUsers = mkOption {
          ...
          default = [ ];
          description = ''
            Interactive users who get a ~/.moor symlink to the service
            stateDir. These users are automatically added to the moor group.
          '';
          example = [ "sidbin" ];
        };
      };
    ...
    config = lib.mkIf cfg.enable (lib.mkMerge [
      ...
      (lib.mkIf (cfg.mcpServers != { }) {
        services.moor-agent.settings.mcp_servers = lib.mapAttrs (_name: srv:
          # Stdio transport
      ...
      # ── Host CLI ──────────────────────────────────────────────────────
      # Add the moor CLI to system PATH and export MOOR_HOME system-wide
      # so interactive shells share state (sessions, skills, cron) with the
      # gateway service instead of creating a separate ~/.moor/.
      (lib.mkIf cfg.addToSystemPackages {
        environment.systemPackages = [ effectivePackage ];
        environment.variables.MOOR_HOME = "${cfg.stateDir}/.moor";
      })
      ...
      # ── Assertions ─────────────────────────────────────────────────────
      {
        ...
        in [{
          ...
          message = "services.moor-agent.extraPlugins: duplicate plugin names detected: ${toString names}. If using fetchFromGitHub, set name = \"plugin-name\" to disambiguate.";
        }];
      }

      # ── Warnings ──────────────────────────────────────────────────────
      # ── Per-user profile for extraPackages ───────────────────────────
      # Wire extraPackages into the moor user's per-user profile so the
      ...
      (lib.mkIf (cfg.container.enable && !cfg.addToSystemPackages && cfg.container.hostUsers != []) {
        warnings = [
          ''
            services.moor-agent: container.enable is true and container.hostUsers
            is set, but addToSystemPackages is false. Without a host-installed moor
            binary, container routing will not work for interactive users.
            Set addToSystemPackages = true or ensure moor is on PATH.
          ''
        ];
      })
      ...
      # ── Directories ───────────────────────────────────────────────────
      {
        systemd.tmpfiles.rules = [
          ...
          "d ${cfg.stateDir}/.moor        2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.moor/cron   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.moor/sessions 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.moor/logs   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.moor/memories 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.moor/plugins 2770 ${cfg.user} ${cfg.group} - -"
      ...
      {
        system.activationScripts."moor-agent-setup" = lib.stringAfter ([ "users" ] ++ lib.optional (config.system.activationScripts ? setupSecrets) "setupSecrets") ''
          # Ensure directories exist (activation runs before tmpfiles)
          mkdir -p ${cfg.stateDir}/.moor
          mkdir -p ${cfg.stateDir}/home
          mkdir -p ${cfg.workingDirectory}
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir} ${cfg.stateDir}/.moor ${cfg.stateDir}/home ${cfg.workingDirectory}
          chmod 2770 ${cfg.stateDir} ${cfg.stateDir}/.moor ${cfg.workingDirectory}
          ...
          find ${cfg.stateDir}/.moor -maxdepth 1 \
            \( -name "*.db" -o -name "*.db-wal" -o -name "*.db-shm" -o -name "SOUL.md" \) \
            -exec chmod g+rw {} + 2>/dev/null || true
          for _subdir in cron sessions logs memories plugins; do
            mkdir -p "${cfg.stateDir}/.moor/$_subdir"
            chown ${cfg.user}:${cfg.group} "${cfg.stateDir}/.moor/$_subdir"
            chmod 2770 "${cfg.stateDir}/.moor/$_subdir"
            find "${cfg.stateDir}/.moor/$_subdir" -type f \
              -exec chmod g+rw {} + 2>/dev/null || true
          done
          ...
          # moor-group users can save settings via the CLI/TUI, else 0640).
          ${if cfg.configFile != null then ''
            install -o ${cfg.user} -g ${cfg.group} -m ${configYamlMode} -D ${configFile} ${cfg.stateDir}/.moor/config.yaml
          '' else ''
            ${configMergeScript} ${generatedConfigFile} ${cfg.stateDir}/.moor/config.yaml
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.moor/config.yaml
            chmod ${configYamlMode} ${cfg.stateDir}/.moor/config.yaml
          ''}

          # Managed mode marker (so interactive shells also detect NixOS management)
          touch ${cfg.stateDir}/.moor/.managed
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.moor/.managed
          chmod 0644 ${cfg.stateDir}/.moor/.managed
          ...
          ${if cfg.container.enable then ''
            cat > ${cfg.stateDir}/.moor/.container-mode <<'MOOR_CONTAINER_MODE_EOF'
    # Written by NixOS activation script. Do not edit manually.
    backend=${cfg.container.backend}
    container_name=${containerName}
    exec_user=${cfg.user}
    moor_bin=${containerDataDir}/current-package/bin/moor
    MOOR_CONTAINER_MODE_EOF
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.moor/.container-mode
            chmod 0644 ${cfg.stateDir}/.moor/.container-mode
          '' else ''
            rm -f ${cfg.stateDir}/.moor/.container-mode

            # Remove symlink bridge for hostUsers
            ${lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.moor";
              in ''
                if [ -L "${symlinkPath}" ] && [ "$(readlink "${symlinkPath}")" = "${cfg.stateDir}/.moor" ]; then
                  rm -f "${symlinkPath}"
                  echo "moor-agent: removed symlink ${symlinkPath}"
                fi
              '') cfg.container.hostUsers)}
          ''}

          # ── Symlink bridge for interactive users ───────────────────────
          # Create ~/.moor -> stateDir/.moor for each hostUser so the
          ...
          ${lib.optionalString cfg.container.enable
            (lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.moor";
                target = "${cfg.stateDir}/.moor";
              in ''
                if [ -d "${symlinkPath}" ] && [ ! -L "${symlinkPath}" ]; then
                  # Real directory — back it up, then create symlink.
                  # (ln -sfn cannot atomically replace a directory.)
                  _backup="${symlinkPath}.bak.$(date +%s)"
                  echo "moor-agent: backing up existing ${symlinkPath} to $_backup"
          ...
          ${lib.optionalString (cfg.authFile != null) ''
            ${if cfg.authFileForceOverwrite then ''
              install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.moor/auth.json
            '' else ''
              if [ ! -f ${cfg.stateDir}/.moor/auth.json ]; then
                install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.moor/auth.json
              fi
            ''}
          ''}
          ...
          # Moor reads $MOOR_HOME/.env at startup via load_moor_dotenv(),
          # so this is the single source of truth for both native and container mode.
          ${lib.optionalString (cfg.environment != {} || cfg.environmentFiles != []) ''
            ENV_FILE="${cfg.stateDir}/.moor/.env"
            install -o ${cfg.user} -g ${cfg.group} -m 0640 /dev/null "$ENV_FILE"
            cat > "$ENV_FILE" <<'MOOR_NIX_ENV_EOF'
    ${envFileContent}
    MOOR_NIX_ENV_EOF
            ${lib.concatStringsSep "\n" (map (f: ''
              if [ -f "${f}" ]; then
        ...
        find ${cfg.stateDir}/.moor/plugins -maxdepth 1 -type l -name 'nix-managed-*' -delete 2>/dev/null || true
        ...
        ${lib.concatStringsSep "\n" (map (plugin:
          ...
          in ''
            ...
            ln -sfn ${plugin} ${cfg.stateDir}/.moor/plugins/nix-managed-${name}
            chown -h ${cfg.user}:${cfg.group} ${cfg.stateDir}/.moor/plugins/nix-managed-${name}
          '') cfg.extraPlugins)}
        '';
      }
      ...
      (lib.mkIf (!cfg.container.enable) {
        systemd.services.moor-agent = {
          ...
          serviceConfig = {
            ...
            # $MOOR_HOME/.env by the activation script. load_moor_dotenv()
            # reads them at Python startup — no systemd EnvironmentFile needed.

            ExecStart = lib.concatStringsSep " " ([
              "${effectivePackage}/bin/moor"
              "gateway"
            ] ++ cfg.extraArgs);
            ...
            # so interactive users in the moor group can read/write them.
            UMask = "0007";
        ...
        systemd.services.moor-agent = {
          ...
          preStart = ''
            ...
            if [ "$NEED_CREATE" = "true" ]; then
              ...
              MOOR_UID=$(${pkgs.coreutils}/bin/id -u ${cfg.user})
              MOOR_GID=$(${pkgs.coreutils}/bin/id -g ${cfg.user})
              ...
              ${containerBin} create \
                ...
                --env MOOR_UID="$MOOR_UID" \
                --env MOOR_GID="$MOOR_GID" \
                --env MOOR_HOME=${containerDataDir}/.moor \
                --env MOOR_MANAGED=true \
                --env HOME=${containerHomeDir} \
                ${lib.concatStringsSep " " cfg.container.extraOptions} \
                ${cfg.container.image} \
                ${containerDataDir}/current-package/bin/moor gateway run --replace ${lib.concatStringsSep " " cfg.extraArgs}
