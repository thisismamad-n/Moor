...
#
# `services.moor-agent` is the same option set on both modules. Both modules
...
#   homeManagerModules.nix  moorHome, programs.moor-agent (the CLI and
...
let
  ...
  sharedOptions =
    ...
    {
      enable = lib.mkEnableOption "Moor Agent";
      ...
      settings = mkOption {
        ...
        default = { };
        description = ''
          The Moor configuration, as an attribute set. The module joins the
          ...
          which includes the keys that `moor config set` and the settings
      ...
      environmentFiles = mkOption {
        ...
        description = ''
          ...
          $MOOR_HOME/.env. Moor reads that file at each start, with
          load_moor_dotenv().

          Each activation writes .env again from the start. Thus a secret
          file cannot go into .env two times.
        '';
        example = literalExpression ''[ config.sops.secrets."moor/env".path ]'';
      };
      ...
      authFile = mkOption {
        ...
        description = ''
          ...
          does not exist. Thus a token that Moor refreshes at runtime stays
          after an activation.
        '';
      };
      ...
      documents = mkOption {
        ...
        description = ''
          ...
          Moor reads SOUL.md and memories/ from MOOR_HOME, so put those
          files in `moorHomeFiles`.
      ...
      moorHomeFiles = mkOption {
        type = documentsType;
        default = { };
        description = ''
          Files that the module installs into MOOR_HOME. Each key is a path
          relative to that directory, and the module makes the necessary
          subdirectories. Each value is a string or a path.

          Moor reads SOUL.md and the memory files from MOOR_HOME and not
          from the working directory. Declare those files here, or Moor
          does not load them.
        '';
        example = literalExpression ''
          {
      ...
      extraPythonPackages = mkOption {
        ...
        description = ''
          ...
          with the same Python interpreter as moor. The interpreter
          major.minor is derived from pm/lock.json by nix/pythonLock.nix —
          take packages from config.services.moor-agent.package.python.pkgs so the set always
          matches the interpreter moor was built with.
        '';
        example = literalExpression ''
          [
      ...
      # ── The backend: `moor serve` or `moor dashboard` ──────────────
      # `moor serve` and `moor dashboard` are the same entry point,
      # moor_cli.main:cmd_dashboard, with one flag of difference. serve runs
      # without a user interface. dashboard also serves the web application.
      # Both give the /api/ws and /api/pty sockets that Moor Desktop
      ...
      # controls an external gateway, with `moor gateway restart`. It does
      ...
      backend = {
        mode = mkOption {
          ...
          description = ''
            ...
            - "serve"     — the backend without a user interface. It gives
                            the /api/ws and /api/pty sockets that Moor
                            Desktop connects to.
        ...
        sessionTokenFile = mkOption {
          ...
          description = ''
            ...
            MOOR_DASHBOARD_SESSION_TOKEN. That token authorizes the /api
            routes and the /api/ws socket. Moor Desktop presents the same
          ...
          '';
          example = literalExpression ''config.sops.secrets."moor/desktop-token".path'';
        };
      };
    };
  ...
  # removal leaves them with no `moor` on the PATH and no message. The
  ...
  installPackageRemovedMessage =
    value:
    ''
      services.moor-agent.installPackage was removed. Moor now
      separates the installation from the services, which is the
      Home Manager convention:

        programs.moor-agent.enable = ${lib.boolToString (value != false)};  # the moor CLI, and MOOR_HOME for your shells
        programs.moor-agent.desktop.enable = true;  # the desktop application

      `services.moor-agent` keeps the state, the configuration and
  ...
  mkConfigFiles =
    ...
    let
      generated = pkgs.writeText "moor-config.yaml" (
  ...
  mkDocumentTree =
    { pkgs, documents }:
    pkgs.runCommand "moor-documents" { } (
      ''
        mkdir -p $out
      ''
  ...
  mkEnvScript =
    { pkgs, environment }:
    let
      base = pkgs.writeText "moor-env-base" (
        lib.concatStringsSep "\n" (lib.mapAttrsToList (k: v: "${k}=${v}") environment)
        + lib.optionalString (environment != { }) "\n"
      );
    in
    pkgs.writeShellScript "moor-env-merge" ''
      set -eu
      ...
      for file in "$@"; do
        if [ -r "$file" ]; then
          ...
          echo "moor-agent: WARNING cannot read environmentFile $file" >&2
        fi
      done
    '';
  ...
  mkStateScript =
    {
      ...
      # MOOR_MANAGED variable of the service. The value tells the shell
    ...
    in
    ''
      # Directories. The service units and Moor make most of these
      ...
      ${run}mkdir -p ${
        lib.escapeShellArgs (
          ...
          ++ map (d: "${moorHome}/${d}") stateDirs
        )
      }

      # config.yaml: merge the Nix settings into the file on disk. Moor
      ...
      ${
        if cfg.configFile != null then
          "${inst} -m ${modes.config} -D ${configFiles.effective} ${moorHome}/config.yaml"
        else
          ''
            ${run}${configFiles.mergeScript} ${configFiles.generated} ${moorHome}/config.yaml
            ${run}chmod ${modes.config} ${moorHome}/config.yaml
          ''
      }

      # The managed-mode marker. It makes an interactive shell also refuse to
      # change the configuration that Nix owns.
      ${inst} -m ${modes.managed} ${pkgs.writeText "moor-managed" managedSystem} ${moorHome}/.managed

      ${lib.optionalString (cfg.environment != { } || cfg.environmentFiles != [ ]) ''
        ${run}${envScript} ${moorHome}/.env ${modes.env} ${lib.escapeShellArgs cfg.environmentFiles}
        ${lib.optionalString (owner != null) "${run}chown ${owner} ${moorHome}/.env"}
      ''}

      ${lib.optionalString (cfg.authFile != null) (
        if cfg.authFileForceOverwrite then
          "${inst} -m ${modes.auth} ${cfg.authFile} ${moorHome}/auth.json"
        else
          ''
            if [ ! -e ${moorHome}/auth.json ]; then
              ${inst} -m ${modes.auth} ${cfg.authFile} ${moorHome}/auth.json
            fi
          ''
      )}
      ...
      ${installDocuments homeDocumentTree moorHome cfg.moorHomeFiles}
      ...
      ${run}find ${moorHome}/plugins -maxdepth 1 -type l -name 'nix-managed-*' -delete 2>/dev/null || true
      ${lib.concatMapStringsSep "\n" (plugin: ''
        if [ ! -f ${plugin}/plugin.yaml ]; then
          echo "moor-agent: ERROR extraPlugins entry '${plugin}' has no plugin.yaml" >&2
          exit 1
        fi
        ${run}ln -sfn ${plugin} ${moorHome}/plugins/nix-managed-${lib.getName plugin}
      '') cfg.extraPlugins}
    '';
  ...
  #
  # `exec` on the last line keeps moor as the MainPID of the unit. No shell
  ...
  backendLauncher =
    ...
    pkgs.writeShellScript "moor-backend-launch" (
      builtins.replaceStrings [ "@HOST@" ] [ ''"$_target"'' ] ''
        ...
        ${lib.optionalString (cfg.backend.sessionTokenFile != null) ''
          ...
          if [ ! -r "$_token_file" ]; then
            echo "moor-backend: cannot read the session token file '$_token_file'. The unit stops." >&2
            echo "moor-backend: backend.sessionTokenFile must name a runtime path that this user can read." >&2
            exit 1
          fi

          MOOR_DASHBOARD_SESSION_TOKEN="$(${pkgs.coreutils}/bin/tr -d '\r\n' < "$_token_file")"
          export MOOR_DASHBOARD_SESSION_TOKEN

          if [ -z "$MOOR_DASHBOARD_SESSION_TOKEN" ]; then
            echo "moor-backend: the session token file '$_token_file' is empty. The unit stops." >&2
            exit 1
          fi
        ''}
        ${
          if cfg.backend.waitFor == null then
            ...
            ''
              ...
              while :; do
                ...
                if [ "$_waited" -ge "$_timeout" ]; then
                  echo "moor-backend: '$_target' did not resolve after ''${_timeout}s. The unit stops." >&2
                  exit 1
                fi

                if [ "$_waited" = 0 ]; then
                  echo "moor-backend: waits for '$_target' to resolve..." >&2
                fi
            ...
            ''
              ...
              while :; do
                ...
                if [ "$_waited" -ge "$_timeout" ]; then
                  echo "moor-backend: interface '$_iface' had no IPv4 address after ''${_timeout}s. The unit stops." >&2
                  echo "moor-backend: a fallback address can expose the backend more widely than you intend." >&2
                  exit 1
                fi

                if [ "$_waited" = 0 ]; then
                  echo "moor-backend: waits for an IPv4 address on '$_iface'..." >&2
                fi
                ${pkgs.coreutils}/bin/sleep 2
                _waited=$(( _waited + 2 ))
              done
        ...
        echo "moor-backend: binds to $_target:${toString cfg.backend.port} (from $_how)" >&2
  ...
  backendDescription =
    ...
    if cfg.backend.mode == "dashboard" then
      "Moor Agent web dashboard and desktop backend"
    else
      "Moor Agent backend for Moor Desktop";

  # The environment that each Moor process needs, from either module.
  #
  # managedSystem gives the value of MOOR_MANAGED. The CLI reads that
    ...
    [
      {
        ...
        message = ''
          ...
          To give Moor an identity and a memory, use
          ${optionPath}.moorHomeFiles instead. Those files go to
          MOOR_HOME. Moor reads SOUL.md and memories/ only from there.
        '';
      }
    ];
  ...
  # The subdirectories of MOOR_HOME that both modules make.
  stateSubdirs = [
    "cron"
    "sessions"
    "logs"
