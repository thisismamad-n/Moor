...
let
    ...
    (callPackage pyproject-nix.build.packages {
      ...
      (
        lib.composeManyExtensions [
          ...
          # must remain blocked.
          (final: prev: {
            moor-agent = prev.moor-agent.overrideAttrs (_old: {
              MOOR_NIX_BUILD = "1";
            });
          })
        ]
  ...
  # $MOOR_PYTHON_SRC_ROOT at runtime.
  workspaceRoot = ./..;
  ...
  editableOverlay = editableWorkspace.mkEditablePyprojectOverlay {
    root = "$MOOR_PYTHON_SRC_ROOT"; # resolved at shellHook time
  };
  ...
  editableSet = pythonSet.overrideScope (
    lib.composeManyExtensions [
      editableOverlay
      (final: prev: {
        moor-agent = prev.moor-agent.overrideAttrs (old: {
