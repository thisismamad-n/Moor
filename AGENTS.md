...
Moor is a personal AI agent that runs the same agent core across a CLI, a messaging
...
Read the balance right: Moor ships a **lot**. Most merges are bug fixes to reported
...
- **Expand reach at the edges.** New adapters, channels, providers, models, desktop/TUI/
  ...
  the existing setup/config UX (`moor tools`, `moor setup`, auto-install) rather than
...
- **E2E validation, not just green unit mocks.** Anything touching resolution chains, config
  ...
  real path with real imports against a temp `MOOR_HOME`. Mocks hide integration bugs.
...
- **New `MOOR_*` env vars for non-secret config.** `.env` is for secrets only. Behavioral
...
- **Outbound telemetry / usage attribution without opt-in gating.** No analytics,
  ...
  (config gate + setup prompt + `moor tools` toggle) exists. Park behind a label.
...
- **Third-party products integrated into the core tree.** Observability backends, vendor
  ...
  backend we don't own. Ship as a **standalone plugin repo** (`~/.moor/plugins/` or pip
  entry point), promoted in the Moor inc. Discord `#plugins-skills-and-skins`. This is a
...
2. **CLI command + skill** — config/state/infra expressible as shell commands; the agent runs
   `moor <subcommand>` guided by a skill. Default for subscriptions, scheduled tasks,
   service setup (`moor webhook`, `moor cron`, `moor tools`).
...
drive a locally spawned backend, one over SSH, one behind URL + token, or Moor Cloud, and
only the first two carry `MOOR_DESKTOP=1`. An env-keyed gate is a silent no-op on the other
topologies — the tool is stripped from the schema while the platform hint tells the model it
is "inside the Moor desktop app". The pattern:

- **The toolset is the surface gate.** Keep such tools off `_MOOR_CORE_TOOLS` and in a named
...
- **Ask which identity you mean.** `MOOR_DESKTOP=1` legitimately means "this backend was
  spawned by the app" (cron ticker, web-dist handling). It does NOT mean "a GUI is watching";
  the embedded terminal pane (`moor --tui` against that backend) is the counterexample.
...
├── toolsets.py           # TOOLSETS dict, _MOOR_CORE_TOOLS
├── cli.py                # MoorCLI (REPL, slash dispatch) + moor_cli/cli_*_mixin.py
├── moor_state.py       # SessionDB facade; moor_state_*.py siblings
├── moor_constants.py   # get_moor_home(), display_moor_home() — profile-aware paths
├── moor_logging.py     # agent.log / errors.log / gateway.log (profile-aware)
├── batch_runner.py       # Parallel batch processing
├── agent/                # turn_*.py loop phases, providers, memory, compression, prompt builder
├── moor_cli/           # CLI subcommands, setup, config, plugins loader, skins, updater
...
**User state:** `~/.moor/config.yaml` (settings), `~/.moor/.env` (secrets only),
`~/.moor/logs/` (`agent.log` INFO+, `errors.log` WARNING+, `gateway.log`); all
profile-aware via `get_moor_home()`. Browse logs with `moor logs [--follow] [--level] [--session]`.
...
families: `moor_state.py` (21), `gateway/run.py` (15), `tools/mcp_tool.py` (15),
`moor_cli/kanban.py` (14), `moor_cli/web_server.py` (13 + 24 routers), `moor_cli/auth.py`
(12), `tools/browser_tool.py` (11), `cli.py` (12 `moor_cli/cli_*_mixin.py`), `run_agent.py`
...
- **Compat pointers are OFF LIMITS in-tree.** Old import paths kept alive for external plugins
  ...
  `-W error::moor_cli.plugin_compat.MoorPluginCompatWarning` catches them in the suite.
...
- **Never hardcode `~/.moor`.** `get_moor_home()` for code paths, `display_moor_home()`
  ...
  sets `MOOR_HOME`. Profile operations themselves are HOME-anchored
...
- **Don't wire in dead code without E2E validation.** Unshipped code was dead for a reason;
  E2E the real resolution chain with real imports against a temp `MOOR_HOME` first.
...
vars unset, `TZ=UTC`, `LANG=C.UTF-8`, `MOOR_HOME` → temp dir, and per-file subprocess
...
- **Tests must not write to `~/.moor/`.** The autouse `_isolate_moor_home` fixture in
  `tests/conftest.py` redirects `MOOR_HOME`; never hardcode `~/.moor/` in tests. Profile
  tests also mock `Path.home()` so `_get_profiles_root()` / `_get_default_moor_home()` stay
  in the temp dir (pattern: `tests/moor_cli/test_profiles.py`):
  ```python
  @pytest.fixture
  def profile_env(tmp_path, monkeypatch):
      home = tmp_path / ".moor"; home.mkdir()
      monkeypatch.setattr(Path, "home", lambda: tmp_path)
      monkeypatch.setenv("MOOR_HOME", str(home))
      return home
  ```
  Tests that `patch.object(Path, "home", ...)` must ALSO set `MOOR_HOME` — code reads the
  env var, not `Path.home()/.moor`.
...
`tests/moor_cli/test_venv_holder_windows_live.py` on a real `windows-latest` runner (real
...
| `cli.py`, `moor_cli/`, `main.py` | `moor_cli/AGENTS.md` | CLI mixins, `_SLASH_DISPATCH`, slash registry, config system + loaders, skins, `moor update` pipeline, profiles / multiplex |
...
| `plugins/`, `moor_cli/plugins*.py` | `plugins/AGENTS.md` | Plugin kinds, native compat contract, in-tree policy, Sep-2026 compat window |
| `tui_gateway/`, `ui-tui/` | `tui_gateway/AGENTS.md` | Process model, JSON-RPC transport, key surfaces, slash flow, dev commands |
| `web/`, `moor_cli/web_routers/` | `web/AGENTS.md` | Dashboard embeds the real TUI; what React may and may not rebuild |
...
| `cron/`, kanban (`moor_cli/kanban*.py`, `tools/kanban_tools.py`, `plugins/kanban/`) | `cron/AGENTS.md` | Scheduler invariants, job fields, kanban board/dispatcher |
