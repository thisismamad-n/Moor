...
> **FORK NOTICE — READ FIRST.** This repo is the Moor fork of upstream
> `NousResearch/hermes-agent`. Brand handling is mechanized: never
> hand-edit brand terms in tracked files. Read
> **`MOOR_REBRAND_PLAYBOOK.md`** (agent handoff: policies, engine
> architecture, war stories, verification gates, decision framework) and
> **`MOOR_WORKFLOW.md`** (user workflow) before any work that touches
> naming, upstream syncs, or `scripts/rebrand.py`. After any upstream
> merge, run `python scripts/rebrand.py` — it heals, protects live
> endpoints/model IDs, renames, and self-verifies (second run must report
> 0 changes).
