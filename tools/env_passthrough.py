...
def _is_moor_provider_credential(name: str) -> bool:
    """True if ``name`` is a moor-managed provider credential per
    ``_MOOR_PROVIDER_ENV_BLOCKLIST`` or a dynamic moor-internal secret
    ...
    in the ``execute_code`` child); non-Moor keys (TENOR_API_KEY, …) stay
    registerable. Fails closed when the blocklist cannot be imported."""
    try:
        from tools.environments.local_env_policy import (
            _MOOR_PROVIDER_ENV_BLOCKLIST, _is_moor_internal_secret)
        ...
        return True
    return _is_moor_internal_secret(name) or name in _MOOR_PROVIDER_ENV_BLOCKLIST


def register_env_passthrough(var_names: Iterable[str]) -> None:
    """Register env var names as allowed in sandboxed environments (typically a
    skill's ``required_environment_variables``). moor-managed provider credentials
...
def _accepted(names, refusal_msg: str):
    """Yield non-empty *names* that are not Moor provider credentials; refused
