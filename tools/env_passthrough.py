...
def _is_hermes_provider_credential(name: str) -> bool:
    ...
    Non-Moor API keys (TENOR_API_KEY, NOTION_TOKEN, etc.) are NOT
    ...
    let a skill tunnel a Moor credential into the execute_code child.
    """
    try:
        ...
        return True
    # Dynamically-generated Moor-internal secrets (AUXILIARY_*_API_KEY /
...
def register_env_passthrough(var_names: Iterable[str]) -> None:
    ...
    Non-Moor third-party API keys (TENOR_API_KEY, NOTION_TOKEN, etc.)
    ...
    try:
        ...
        if isinstance(passthrough, list):
            for item in passthrough:
                ...
                # Moor-managed provider credentials must not be passed
