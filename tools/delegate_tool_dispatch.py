...
def _capture_origin() -> tuple[str, str, Any, Any]:
    ...
    clobbers the MOOR_SESSION_ID ContextVar/os.environ with the subagent's id."""
...
def _resolve_async_wake_sid(origin_wake_sid: str) -> Optional[str]:
    ...
    wake. Uses the origin captured BEFORE child construction — MOOR_SESSION_ID here would be the subagent's internal
    id.
    """
    try:
...
def _resolve_async_session_key(parent_agent: Any, origin_ui_session_id: str) -> tuple[str, str]:
    ...
    platform conversation key (agent:main:...). The CLI has no bound approval contextvar and no MOOR_SESSION_KEY, so
    ...
    with _quiet(None):
        ...
        origin_ui_session_id = get_session_env("MOOR_UI_SESSION_ID", "") or origin_ui_session_id
