...
``moor mcp install <name>`` in the terminal.
"""
...
def setup_mcp_tool(
    ...
    if callback is None:
        return tool_error(
            "setup_mcp is only available in the Moor desktop app. Use the "
            "terminal instead: `moor mcp install <name>` for catalog entries, "
            "`moor mcp login <name>` for OAuth."
        )
...
SETUP_MCP_SCHEMA = {
    ...
    "description": (
        ...
        "Moor desktop chat. The card lets them install a catalog entry, "
        ...
        "continue without the server. Catalog names: run `moor mcp catalog` "
