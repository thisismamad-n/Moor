...
* **openwakeword** (default, free, no API key) — loads an ONNX model. Defaults
  to the bundled "hey moor" model (``tools/wakewords/``) so the wake word
...
# Minimum gap between two consecutive wake fires, so one "hey moor" can't
...
# Bundled "hey moor" model (tools/wakewords/) — the default, so the wake word
# works out of the box. Config names in _ALIASES resolve to it, not a built-in.
_BUNDLED_MODEL_NAME = "hey_moor"
...
_BUNDLED_MODEL_ALIASES = frozenset({"", "hey_moor", "hey moor", "moor"})


def _bundled_wakeword_path(framework: str = "onnx") -> str:
    """Path to the shipped hey_moor model (.onnx/.tflite) for ``framework``."""
...
def wake_phrase(cfg: Optional[Dict[str, Any]] = None) -> str:
    ...
    return str(_get(cfg, "phrase")) or "hey moor"
...
def enrolled_profile_phrases() -> Dict[str, str]:
    ...
    try:
        ...
        from moor_cli.profiles import get_profile_dir, list_profiles
...
class _OpenWakeWordEngine(_Engine):
    ...
    def __init__(self, cfg: Dict[str, Any]):
        ...
        # Default (or explicit "hey_moor") → the bundled model; a built-in name
...
# MOOR_HOME. Keywords are typed phrases tokenized at RUNTIME — no
...
def _sherpa_model_root() -> Path:
    ...
    return get_moor_home() / "cache" / "wakewords"
...
class _SherpaKwsEngine(_Engine):
    ...
    model's vocabulary, so "hey moor", "hey coder", or any other phrase works
    ...
    def __init__(self, cfg: Dict[str, Any]):
        ...
        # wake any profile ("hey moor" / "hey coder" / ...). display-name →
        # profile is kept for routing the match back.
        phrase = str(_get(cfg, "phrase") or "hey moor").strip()
        ...
        kw = tempfile.NamedTemporaryFile(
            mode="w", suffix=".txt", prefix="moor-kws-", delete=False, encoding="utf-8"
        )
...
# ---------------------------------------------------------------------------
# Process-wide singleton (mirrors moor_cli.voice's continuous API)
# ---------------------------------------------------------------------------
...
def _lock_path() -> Path:
    ...
    return get_default_moor_root() / "runtime" / "wake-word.lock"
