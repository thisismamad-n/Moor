...
Registered only under the dispatcher (``MOOR_KANBAN_TASK`` set) or when the profile
enables the ``kanban`` toolset. Tools rather than ``moor kanban`` shell-outs: they run
...
def _visible(*, to_env_worker: bool) -> bool:
    ...
    (MOOR_KANBAN_TASK) per flag; else the profile toolset decides."""
...
def _reject_delegated_child_mutation(tool_name: str) -> None:
    """A delegate_task child shares the parent's process, so inherited MOOR_KANBAN_*
...
def _default_task_id(arg: Optional[str]) -> Optional[str]:
    ...
    if _is_delegated_child_context() or not _is_dispatcher_owned_worker():
        return None
    return os.environ.get("MOOR_KANBAN_TASK") or None


def _require_task_id(args: dict) -> str:
    tid = _default_task_id(args.get("task_id"))
    _check(tid, "task_id is required (or set MOOR_KANBAN_TASK in the env)")
    return tid


def _own_task_env(task_id: str, var: str) -> Optional[str]:
    """``$var`` only when this worker is scoped to ``task_id``; else None."""
    return os.environ.get(var) if os.environ.get("MOOR_KANBAN_TASK") == task_id else None


def _worker_run_id(task_id: str) -> Optional[int]:
    """This worker's dispatcher run id when it is scoped to task_id."""
    raw = _own_task_env(task_id, "MOOR_KANBAN_RUN_ID")
    try:
...
def _stamp_worker_session_metadata(task_id: str, metadata: Optional[dict]) -> Optional[dict]:
    ...
    session_id = _own_task_env(task_id, "MOOR_SESSION_ID")
...
def _enforce_worker_task_ownership(tid: str) -> None:
    """A dispatcher-spawned worker may only mutate its own MOOR_KANBAN_TASK; a
...
# worker context (no ``MOOR_KANBAN_TASK``). - No durable note on these auto-heartbeats; that's reserved
...
def heartbeat_current_worker_from_env() -> bool:
    ...
    attempted. ``MOOR_KANBAN_RUN_ID`` pins the run row so a reclaimed stale run is not
    ...
    try:
        ...
        with _board(None, quiet_close=True) as (kb, conn):
            ops = ((kb.heartbeat_claim, {"claimer": os.environ.get("MOOR_KANBAN_CLAIM_LOCK")}),
...
def inject_new_comments_from_env(agent: Any) -> bool:
    ...
    steer was injected; never raises. Own comments (``MOOR_PROFILE``) are skipped."""
...
def _handle_heartbeat(args: dict, **kw) -> str:
    ...
    from moor_cli import kanban_db_dispatch as kbd
    with _board(args.get("board")) as (kb, conn):
        # The dispatcher pins MOOR_KANBAN_CLAIM_LOCK at spawn; the default
        # claimer covers locally-driven workers that bypassed the dispatcher.
        kb.heartbeat_claim(conn, tid, claimer=os.environ.get("MOOR_KANBAN_CLAIM_LOCK"))
...
def _handle_comment(args: dict, **kw) -> str:
    ...
    # forge a directive from ``moor-system``. Cross-task commenting stays unrestricted —
    ...
    # comment from an authoritative-looking name like ``moor-system`` and poison the future-worker context
...
def _download_url_with_cap(url: str, max_bytes: int) -> tuple[bytes, Optional[str]]:
    ...
    for _ in range(_MAX_ATTACH_URL_REDIRECTS + 1):
        ...
        chunks: list[bytes] = []
        total = 0
        with httpx.stream("GET", current_url, headers={"User-Agent": "moor-kanban/attach"},
...
def _handle_create(args: dict, **kw) -> str:
    ...
    # Prefer the request-scoped api_server origin binding over MOOR_SESSION_ID: the env
    ...
    with _board(args.get("board")) as (kb, conn):
        ...
        new_tid = kb.create_task(
            ...
            parents=tuple(parents), tenant=args.get("tenant") or os.environ.get("MOOR_TENANT"),
            ...
            created_by=os.environ.get("MOOR_PROFILE") or "worker", session_id=session_id)
...
def _resolve_notify_target() -> Optional[dict[str, Any]]:
    ...
    Gateway sessions: ``MOOR_SESSION_PLATFORM``/``CHAT_ID`` ContextVars. TUI/desktop:
    those are cleared but the subprocess inherits ``MOOR_SESSION_KEY`` -> ``platform="tui"``
    for the TUI poller. ``MOOR_SESSION_ID`` is deliberately NOT a fallback: it is set for
    ...
    platform, chat_id = env("MOOR_SESSION_PLATFORM", ""), env("MOOR_SESSION_CHAT_ID", "")
    if not platform or not chat_id:
        session_key = env("MOOR_SESSION_KEY", "") or os.environ.get("MOOR_SESSION_KEY", "")
        if not session_key:
            return None
    ...
    notifier_profile = env("MOOR_SESSION_PROFILE", "") or os.environ.get("MOOR_PROFILE")
    ...
    return dict(
        ...
        user_id=env("MOOR_SESSION_USER_ID", "") or None,
        user_id_alt=env("MOOR_SESSION_USER_ID_ALT", "") or None,
