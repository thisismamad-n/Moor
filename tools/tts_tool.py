...
def _get_provider(tts_config: Dict[str, Any]) -> str:
    ...
    The managed "Moor Subscription" selection (``tts.provider: moor``) is
    ...
    if provider == MOOR_MANAGED_PROVIDER:
        return "openai"
    return provider
...
# custom voice-cloning scripts, etc.) into Moor without any Python code
...
#           output_format: wav
#
# Moor writes the input text to a temp UTF-8 file, runs the command with
...
def _generate_xai_tts(text: str, output_path: str, tts_config: Dict[str, Any]) -> str:
    ...
    # output_format when Moor actually needs a non-default format/override.
...
def _resolve_openai_audio_client_config() -> tuple[str, str, bool]:
    ...
    string):
    - ``"moor"`` (or legacy ``use_gateway: true``) → managed gateway ONLY;
