...
def _default_output_dir() -> str:
    ...
    MOOR_HOME and later scope requests to a different profile via
    ``moor_constants.set_moor_home_override()`` — a frozen module constant keeps writing synthesized
...
def _get_provider(tts_config: Dict[str, Any]) -> str:
    ...
    speech); ``moor`` is serviced by the OpenAI path through the managed openai-audio gateway."""
    provider = (tts_config.get("provider") or DEFAULT_PROVIDER).lower().strip()
    return "openai" if provider == MOOR_MANAGED_PROVIDER else provider
...
_PLUGIN_COMPAT_LAZY = {
    ...
    'moor_xai_user_agent': ('tools.xai_http', 'moor_xai_user_agent'),
