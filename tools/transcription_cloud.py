...
local server > env > managed Moor gateway). Facade-owned state and helpers
...
def _transcribe_xai(
    ...
    if not api_key:
        return _error_result("No xAI credentials found. Configure xAI OAuth in `moor model` or set XAI_API_KEY")
    ...
    def _post() -> Any:
        ...
        def _post_transcription(bearer: str, endpoint_base_url: str):
            headers = {"Authorization": f"Bearer {bearer}", "User-Agent": moor_xai_user_agent()}
...
def _transcribe_deepinfra(
    ...
    """Resolve DeepInfra credentials/model (shared ``moor_cli.models`` helpers), then delegate to :func:`_transcribe_openai`."""
...
def _resolve_openai_audio_client_config() -> tuple[str, str]:
    ...
    ``"moor"`` -> managed gateway ONLY (a direct OPENAI_API_KEY must NOT override it); any other
    ...
    from tools.tool_backend_helpers import (
        MOOR_MANAGED_PROVIDER, managed_moor_tools_enabled, moor_tool_gateway_unavailable_message,
    ...
    def _managed() -> Optional[tuple[str, str]]:
        ...
        if gateway is None:
            return None
        return gateway.moor_user_token, urljoin(f"{gateway.gateway_origin.rstrip('/')}/", "v1")

    if selected == MOOR_MANAGED_PROVIDER:
        managed = _managed()
        if managed is None:
            raise ValueError(selection_error("stt", MOOR_MANAGED_PROVIDER,
    ...
    if managed is None:
        ...
        if managed_moor_tools_enabled():
            message += ". " + moor_tool_gateway_unavailable_message("managed OpenAI audio for transcription")
