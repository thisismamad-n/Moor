...
# Where the vendor's done page sends the browser after consent; the dev desktop registers moor-dev://.
ConnectorReturnTarget = Literal["moor-desktop", "moor-desktop-dev", "portal"]

# Moor dispatch caps batches lower, so client-side chunking is deliberately absent.
WIRE_BATCH_MAX = 25
...
class ConnectorExecuteCall(_Wire):
    ...
    # On the wire for the multi-account switch; never sent by moor today, because the vendor answers
...
class ConnectorConnectionsRequest(_Wire):
    ...
    return_to: Optional[ConnectorReturnTarget] = Field(default=None, alias="returnTo")
    # The caller's operation id, echoed on the moor://connections/done link.
    op: Optional[str] = Field(default=None, min_length=1, max_length=128)
