...
def _node_fallback(command: str) -> str:
    ...
    moor_home = os.path.expanduser(os.getenv("MOOR_HOME", os.path.join(home, ".moor")))
    ...
    candidates = (os.path.join(moor_home, "node", "bin", command), os.path.join(home, ".local", "bin", command),
...
def _npx_bin_candidates(bin_dir: str, name: str, *, windows: Optional[bool] = None) -> list:
    ...
    extension (same precedence as ``moor_constants._candidate_node_command_names``). ``windows``
...
def _npx_cached_bin(args: list) -> Optional[tuple]:
    ...
    for nothing (~48 MB private memory per MCP server, measured); Moor already supervises the
