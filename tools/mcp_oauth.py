...
refresh; this module supplies ``MoorTokenStorage`` (on-disk persistence), the localhost callback
listener and ``build_oauth_auth()`` (legacy entry point). client_id is Moor' Client ID Metadata
...
def _get_token_dir(moor_home: str | Path | None = None) -> Path:
    """``MOOR_HOME/mcp-tokens/`` — per-profile token directory."""
    from moor_constants import get_moor_home

    return Path(moor_home if moor_home is not None else get_moor_home()) / "mcp-tokens"
...
def _cached_client_info(storage: "MoorTokenStorage | None") -> dict | None:
...
def _cached_redirect(storage: "MoorTokenStorage | None") -> "tuple[str | None, int | None]":
...
def _raise_if_non_interactive(lead: str) -> None:
    ...
    ``lead`` is the boundary-specific first sentence; this helper appends the shared, actionable ``moor
    mcp login`` next-step so the guidance wording lives in one place across every non-interactive OAuth
    boundary (#57836).
    """
    if not _is_interactive():
        raise OAuthNonInteractiveError(
            f"{lead} Run `moor mcp login <server>` interactively to (re)authorize, then restart or reload the gateway."
        )
...
def _write_json(path: Path, data: dict) -> None:
    ...
    # secure_parent_dir refuses to chmod /, top-level dirs, or the moor-agent install tree (#25821,
    # #93050).
...
class MoorTokenStorage:
    """Persist OAuth state as ``MOOR_HOME/mcp-tokens/<server_name>`` + ``.json`` (tokens),
    ...
    def _path(self, suffix: str) -> Path:
        return _get_token_dir(self._moor_home) / f"{self._server_name}{suffix}"
...
def _paste_callback_reader(result: dict) -> None:
    ...
    if line.lower() in _SKIP_TOKENS:
        ...
        print(
            "  OAuth skipped. Run `moor mcp login <server>` later to authenticate, "
...
def _callback_outcome(result: dict, cimd_url: str | None):
    ...
    if result["auth_code"] is None:
        hint = (
            ...
            f"server rejected Moor' Client ID Metadata Document ({cimd_url}); set ``cimd: false`` under that "
...
def remove_oauth_tokens(server_name: str, *, moor_home: str | Path | None = None) -> None:
    """Delete stored OAuth tokens and client info for a server."""
    MoorTokenStorage(server_name, moor_home=moor_home).remove()
    logger.info("OAuth tokens removed for '%s'", server_name)


# CIMD (OAuth Client ID Metadata Documents): the client_id IS an HTTPS URL the server fetches for our
# name/logo/redirect URIs, replacing per-install DCR. The SDK does the protocol; Moor only decides
...
def _server_declined_cimd(storage: "MoorTokenStorage | None") -> bool:
    ...
    in its 401 branch — after Moor must fix the redirect URI — so cached metadata closes the gap;
...
def _maybe_use_cimd(cfg: dict, storage: "MoorTokenStorage | None" = None) -> "tuple[str, int] | None":
...
def _invalidate_tokens_on_client_change(
    storage: "MoorTokenStorage", new_client_id: str, new_client_secret: str | None) -> None:
    ...
    if removed:
        logger.warning(
            ...
            "the previous client. Re-authorize with: moor mcp login %s",
            storage._server_name, old_client_id, new_client_id, storage._server_name)


def _maybe_preregister_client(storage: "MoorTokenStorage", cfg: dict, client_metadata: "OAuthClientMetadata") -> None:
...
def humanize_oauth_registration_error(
    ...
    when the user overrode it or an older Moor is running."""
    msg = str(exc)
    lowered = msg.lower()
...
def build_oauth_auth(server_name: str, server_url: str, oauth_config: dict | None = None) -> "OAuthClientProvider | None":
    ...
    if not _is_interactive() and not storage.has_cached_tokens():
        raise OAuthNonInteractiveError(
            ...
            f"requires browser authorization. Run `moor mcp login {server_name}` interactively first to complete "
            "initial authorization, then cached tokens will be reused.")
    kwargs = build_provider_kwargs(cfg, storage, ssh_proxy_hint=True)
    if MoorOAuthClientProvider is None:
        from tools.mcp_oauth_provider import MoorProviderMixin

        MoorOAuthClientProvider = type("MoorOAuthClientProvider", (MoorProviderMixin, _sdk_class("OAuthClientProvider")), {
            "__doc__": "SDK provider plus Moor' token-endpoint fixes (see ``MoorProviderMixin``).",
            "__module__": __name__, "_moor_logger": logger})
    return MoorOAuthClientProvider(server_url=server_url, **kwargs)
