...
URL of Moor' published Client ID Metadata Document (CIMD) as the
...
This module provides the glue:
    - ``MoorTokenStorage``: persists tokens/client-info to disk so they
...
_USER_SKIPPED_SENTINEL = "__moor_user_skipped__"
...
def _get_token_dir(moor_home: str | Path | None = None) -> Path:
    """Return the directory for MCP OAuth token files.

    Uses MOOR_HOME so each profile gets its own OAuth tokens.
    Layout: ``MOOR_HOME/mcp-tokens/``
    """
    from moor_constants import get_moor_home

    base = Path(moor_home) if moor_home is not None else Path(get_moor_home())
...
def _cached_redirect_port(storage: "MoorTokenStorage | None") -> int | None:
...
def _cached_redirect_uri(storage: "MoorTokenStorage | None") -> str | None:
...
def _raise_if_non_interactive(lead: str) -> None:
    ...
    shared, actionable ``moor mcp login`` next-step so the guidance wording
    lives in one place across every non-interactive OAuth boundary (#57836).
    """
    if not _is_interactive():
        raise OAuthNonInteractiveError(
            f"{lead} "
            "Run `moor mcp login <server>` interactively to (re)authorize, "
...
# ---------------------------------------------------------------------------
# MoorTokenStorage -- persistent token/client-info on disk
# ---------------------------------------------------------------------------


class MoorTokenStorage:
    """Persist OAuth tokens and client registration to JSON files.

    File layout::

        MOOR_HOME/mcp-tokens/<server_name>.json         -- tokens
        MOOR_HOME/mcp-tokens/<server_name>.client.json   -- client info
        MOOR_HOME/mcp-tokens/<server_name>.meta.json     -- oauth server metadata
        MOOR_HOME/mcp-tokens/<server_name>.cimd-off      -- CIMD refused here
    """
    ...
    def _tokens_path(self) -> Path:
        return _get_token_dir(self._moor_home) / f"{self._server_name}.json"

    def _client_info_path(self) -> Path:
        return _get_token_dir(self._moor_home) / f"{self._server_name}.client.json"

    def _meta_path(self) -> Path:
        return _get_token_dir(self._moor_home) / f"{self._server_name}.meta.json"

    def _cimd_rejected_path(self) -> Path:
        return _get_token_dir(self._moor_home) / f"{self._server_name}.cimd-off"
    ...
    def mark_cimd_rejected(self) -> None:
        ...
        refused. Cleared by ``remove()``, i.e. by ``moor mcp login`` /
        ``moor mcp remove``, so a fixed document gets another chance.
        """
        ...
        if result["auth_code"] is None:
            ...
            if cimd_url:
                hint = (
                    ...
                    f"Moor' Client ID Metadata Document ({cimd_url}); set "
...
def _paste_callback_reader(result: dict) -> None:
    ...
    if line.lower() in _SKIP_TOKENS:
        ...
        print(
            "  OAuth skipped. Run `moor mcp login <server>` later to "
...
MoorOAuthClientProvider: Any = None


def _get_moor_oauth_provider_class() -> type | None:
    global MoorOAuthClientProvider
    if MoorOAuthClientProvider is not None:
        return MoorOAuthClientProvider
    if not _ensure_sdk_loaded():
        return None

    class _MoorOAuthClientProvider(OAuthClientProvider):
        ...
        def _stamp_token_user_agent(self, request):
            ua = getattr(self, "_moor_token_user_agent", None)
            if ua:
    ...
    _MoorOAuthClientProvider.__name__ = "MoorOAuthClientProvider"
    _MoorOAuthClientProvider.__qualname__ = "MoorOAuthClientProvider"
    MoorOAuthClientProvider = _MoorOAuthClientProvider
    return MoorOAuthClientProvider
...
def remove_oauth_tokens(
    ...
    *,
    ...
    storage = MoorTokenStorage(server_name, moor_home=moor_home)
    storage.remove()
...
# 2026-07-28. The SDK does the protocol work; Moor only decides whether a
...
# (section 4.2), so a CIMD flow cannot use the ephemeral port Moor picks
...
def _has_cached_client_info(storage: "MoorTokenStorage | None") -> bool:
...
def _server_declined_cimd(storage: "MoorTokenStorage | None") -> bool:
    ...
    Moor has to fix the redirect URI. Cached authorization-server metadata
...
def _maybe_use_cimd(
    cfg: dict,
    storage: "MoorTokenStorage | None" = None,
) -> "tuple[str, int] | None":
    """Return ``(client_id URL, pinned callback port)``, or None to use DCR.

    Every early return below is a case where the redirect URI Moor would
...
def _invalidate_tokens_on_client_change(
    ...
    if removed:
        logger.warning(
            ...
            "Re-authorize with: moor mcp login %s",
...
def build_oauth_auth(
    ...
    if not _is_interactive() and not storage.has_cached_tokens():
        raise OAuthNonInteractiveError(
            ...
            f"`moor mcp login {server_name}` interactively first to complete "
    ...
    provider_class = _get_moor_oauth_provider_class()
