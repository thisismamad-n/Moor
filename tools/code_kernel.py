...
"""Auto-generated Moor session-kernel runner. One exec cell per request."""
import contextlib
import io
import json
import os
...
_SENTINEL = os.environ["MOOR_KERNEL_SENTINEL"]
_CAPTURE_LIMIT = {capture_limit}
_SPILL_DIR = os.environ.get("MOOR_KERNEL_SPILL_DIR", "")
_SPILL_CAP = {spill_cap}
_PARENT_PROCESS_HANDLE = os.environ.pop("MOOR_KERNEL_PARENT_PROCESS_HANDLE", "")
_PARENT_DEATH_FD = os.environ.pop("MOOR_KERNEL_PARENT_DEATH_FD", "")
...
def _start_parent_death_pipe_watchdog():
    ...
    threading.Thread(target=_wait, name="moor-parent-watchdog", daemon=True).start()
...
def _rpc_forever(kernel: SessionKernel, max_tool_calls: int,
    ...
    (the client stub reconnects: MOOR_RPC_PERSISTENT). The serving thread carries NO frozen
...
def _parent_process_handle(child_env: Dict[str, str]):
    ...
    try:
        ...
        if handle:
            child_env["MOOR_KERNEL_PARENT_PROCESS_HANDLE"] = str(int(handle))
            startupinfo = subprocess.STARTUPINFO()
            startupinfo.lpAttributeList = {"handle_list": [int(handle)]}
    except (AttributeError, ImportError, OSError, TypeError, ValueError):
        if handle and close is not None:
            close(handle)
        child_env.pop("MOOR_KERNEL_PARENT_PROCESS_HANDLE", None)
        handle = close = startupinfo = None
...
def _spawn(kernel: SessionKernel, *, child_python: str, child_cwd: str,
    ...
    kernel.tmpdir = tempfile.mkdtemp(prefix="moor_kernel_")
    kernel.rpc_token = secrets.token_urlsafe(32)
    kernel.sentinel = "@@MOOR-KERNEL-" + secrets.token_urlsafe(16) + "@@"
    rpc_endpoint = _bind_rpc_socket(kernel)
    for name, src in (("moor_tools.py", generate_moor_tools_module(list(sandbox_tools))),
                      ("moor_kernel_runner.py", KERNEL_RUNNER_SOURCE)):
    ...
    if not _IS_WINDOWS:
        ...
        child_env["MOOR_KERNEL_PARENT_DEATH_FD"] = str(death_r)
        pass_fds = (death_r,)
    try:
        kernel.proc = subprocess.Popen(
            [child_python, os.path.join(kernel.tmpdir, "moor_kernel_runner.py")],
