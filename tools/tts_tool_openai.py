...
Also owns the managed-gateway (Moor portal ``openai-audio`` proxy) route selection that
...
# DeepInfra base URL is resolved via moor_cli.models.deepinfra_base_url (shared).
...
def _managed_openai_audio_route() -> Optional[tuple]:
    ...
    if gateway is None:
        return None
    return gateway.moor_user_token, urljoin(f"{gateway.gateway_origin.rstrip('/')}/", "v1"), True
...
def _resolve_openai_audio_client_config() -> tuple[str, str, bool]:
    ...
    Moor proxy, so callers coerce the request). Strict on the stored ``tts`` selection: ``"moor"``
    ...
    if route is None:
        ...
        if managed_moor_tools_enabled():
            message += ". " + moor_tool_gateway_unavailable_message("managed OpenAI audio for TTS")
...
def _generate_deepinfra_tts(text: str, output_path: str, tts_config: Dict[str, Any]) -> str:
    """Resolve DeepInfra credentials/model (live ``moor_cli.models`` catalog, no hardcoded ids), then
    delegate to the OpenAI-compatible handler."""
    api_key = _origin()._resolve_provider_key("DEEPINFRA_API_KEY", "deepinfra")
    if not api_key:
        raise ValueError("DEEPINFRA_API_KEY not set. Run `moor setup` to configure, or set the env var directly.")
