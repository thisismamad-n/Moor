...
Independent of local skill state (no ``~/.moor`` reads); ``tools/skills_sync_client.py``
...
def _check_version(caps: Dict[str, Any]) -> None:
    ...
    if ver.split(".", 1)[0] != WIRE_VERSION:
        raise SyncError(f"this server speaks sync version {ver!r}, but this Moor speaks "
...
class SyncClient:
    """Sync client bound to a base URL + Moor bearer. Org refs/objects live behind SEPARATE ``org/``
