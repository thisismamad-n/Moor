...
* Core tools defined in ``toolsets._MOOR_CORE_TOOLS`` are *never* deferred.
...
def _core_tool_names() -> frozenset[str]:
    ...
    try:
        ...
        return frozenset(_MOOR_CORE_TOOLS)
    except Exception:
        return frozenset()


# Session-gated GUI toolsets. Off ``_MOOR_CORE_TOOLS`` so non-GUI clients
...
def is_deferrable_tool_name(name: str) -> bool:
    ...
    OR it is neither in ``_MOOR_CORE_TOOLS`` nor a session-gated GUI
