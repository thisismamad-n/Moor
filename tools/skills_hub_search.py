"""Skills Hub discovery: the centralized Moor index fetch (cached, stale-
...
from tools.skills_hub_official import MoorIndexSource, OptionalSkillSource
...
def _load_moor_index() -> Optional[dict]:
    ...
    cache_file = _moor_index_cache_file()
    cached = _read_json_if_fresh(cache_file, MOOR_INDEX_TTL)
...
def _load_stale_index_cache() -> Optional[dict]:
    ...
    from tools.skills_hub import _read_json_if_fresh
    return _read_json_if_fresh(_moor_index_cache_file(), float("inf"))
...
def _select_active_sources(sources: List[SkillSource], source_filter: str) -> List[SkillSource]:
    ...
    index_available = effective == "all" and any(
        src.source_id() == "moor-index" and getattr(src, "is_available", False) for src in sources
    )
