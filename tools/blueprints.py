...
`moor skills publish` for sharing. No new source type, no new store, no new
...
@dataclass
class BlueprintSpec:
    """Parsed ``metadata.moor.blueprint`` automation spec for a skill."""
...
def parse_blueprint(skill_md_text: str) -> Optional[BlueprintSpec]:
    ...
    A skill is a blueprint iff ``metadata.moor.blueprint`` is a mapping containing
    ...
    moor = meta.get("moor") if isinstance(meta, dict) else None
    blueprint = moor.get("blueprint") if isinstance(moor, dict) else None
    if blueprint is None:
        return None
    if not isinstance(blueprint, dict):
        raise BlueprintError("metadata.moor.blueprint must be a mapping")
...
def export_blueprint(job: Dict[str, Any], body: str, *, blueprint_name: Optional[str] = None) -> str:
    ...
    and emit a SKILL.md (with a ``metadata.moor.blueprint`` block) they can hand
    to ``moor skills publish`` to share. ``body`` is the plain-language
