...
class ProcessRegistry:
    ...
    def wait_for_pending_completions(
        ...
        *,
        ...
        One-shot CLI runs (``moor -q/-Q/-z``) exit as soon as their single
        ...
        (#90879): a recipient invoked as ``moor -p <bot> chat -Q
    ...
    def request_close_terminal(self, session_id: str) -> dict:
        ...
        if sink is None:
            return {
                "status": "error",
                "error": "close_terminal is only available in the Moor desktop app.",
            }
...
def format_process_notification(evt: dict) -> "str | None":
    ...
    if _exit in {-15, 143, "-15", "143"}:
        _signal = ", SIGTERM"
    if _reason == "killed":
        _status = f"terminated by {_source or 'Moor'}"
    elif _reason == "lost":
