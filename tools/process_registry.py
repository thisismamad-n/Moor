...
def _is_supervised_gateway_process() -> bool:
    """Whether this process is the live, supervised Moor gateway itself.
...
def _build_systemd_scope_argv(shell_argv: List[str], unit_suffix: str) -> List[str]:
    ...
    if binary is None:
        ...
        return shell_argv
    return _systemd_scope_argv(binary, f"moor-worker-{unit_suffix}", *shell_argv)
...
class ProcessRegistry:
    ...
    def _config_value(section: str, key: str, fallback):
        ...
        from moor_cli.config import DEFAULT_CONFIG, cfg_get, read_raw_config
    ...
    def spawn_via_env(
        ...
        log_path, pid_path, exit_path = (f"{temp_dir}/moor_bg_{session.id}.{ext}" for ext in ("log", "pid", "exit"))
        q = shlex.quote
        bg_command = (
    ...
    def is_session_waiting(self, session_id: str) -> bool:
        """Whether a goal loop (``moor_cli.goals`` wait barrier) should stay parked on
    ...
    def wait_for_pending_completions(
        ...
        A one-shot CLI run (``moor -q/-Q/-z``) exits when its turn ends; a background
        ...
        Bot Mode handoff REPLIES are the visible casualty (#90879): a recipient invoked as ``moor -p <bot>
    ...
    def _reconcile_local_exit(self, session: "ProcessSession") -> None:
        ...
        descendant (e.g. a daemon from ``moor update``) holds the pipe open, poll()
        ...
        process (e.g. a daemon spawned by `moor update` restarting the gateway) is still holding the
    ...
    def request_close_terminal(self, session_id: str) -> dict:
        ...
        if self.on_close is None:
            return {"status": "error", "error": "close_terminal is only available in the Moor desktop app."}
