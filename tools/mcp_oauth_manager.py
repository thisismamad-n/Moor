...
`mcp_tool.py`, and `moor_cli/mcp_config.py`. This module is the ONLY place
...
# ---------------------------------------------------------------------------
# MoorMCPOAuthProvider — OAuthClientProvider subclass with disk-watch
# ---------------------------------------------------------------------------


def _make_moor_provider_class() -> Optional[type]:
    ...
    class MoorMCPOAuthProvider(OAuthClientProvider):
        ...
        async def _initialize(self) -> None:
            ...
            Paired with :class:`MoorTokenStorage` persisting an absolute
            ...
            if (
                isinstance(storage, MoorTokenStorage)
            ...
            async with httpx.AsyncClient(timeout=10.0) as client:
                ...
                for url in build_protected_resource_metadata_discovery_urls(
                    ...
                    try:
                        ...
                        logger.debug(
                            ...
                            self._moor_server_name, url, exc,
                        )
                        continue
                ...
                for url in build_oauth_authorization_server_metadata_discovery_urls(
                    ...
                    if asm:
                        ...
                        storage = self.context.storage
                        from tools.mcp_oauth import MoorTokenStorage
                        if isinstance(storage, MoorTokenStorage):
        ...
        def _persist_oauth_metadata_if_changed(self) -> None:
            ...
            storage = self.context.storage
            from tools.mcp_oauth import MoorTokenStorage
            if not isinstance(storage, MoorTokenStorage):
                return
        ...
        async def _maybe_flag_poisoned_client(self, response: Any) -> None:
            ...
            and is handled by ``moor mcp reauth``.
            ...
            back to ``moor mcp reauth``.
            """
            try:
                if self._moor_preregistered:
                    return
                ...
                # walking back into the same refusal. `moor mcp login`
    ...
    return MoorMCPOAuthProvider


# Cached at import time. Tested and used by :class:`MCPOAuthManager`.
_MOOR_PROVIDER_CLS: Optional[type] = _make_moor_provider_class()
...
class MCPOAuthManager:
    ...
    def _build_provider(
        ...
        Constructs :class:`MoorMCPOAuthProvider` directly using the helpers
        ...
        if (
            ...
            raise OAuthNonInteractiveError(
                ...
                "cached tokens found. Run `moor mcp login "
    ...
    def remove(
        ...
        *,
        ...
        """
        with self._entries_lock:
            entry = self._entries.pop(self._key(server_name, moor_home), None)
    ...
    def evict(
        ...
        *,
        ...
        with self._entries_lock:
            self._entries.pop(self._key(server_name, moor_home), None)
    ...
    async def invalidate_if_disk_changed(
        ...
        *,
        ...
        async with entry.lock:
            tokens_path = _get_token_dir(moor_home) / f"{_safe_filename(server_name)}.json"
            try:
