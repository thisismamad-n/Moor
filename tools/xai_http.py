...
def get_env_value(name: str, default=None):
    ...
    Wraps :func:`moor_cli.config.get_env_value` so tests can patch ``tools.xai_http.get_env_value``.
    """
    try:
...
def resolve_xai_http_credentials(
    *, force_refresh: bool = False, api_key_hint: Optional[str] = None, prefer_api_key: bool = False,
    ...
    Default order: moor-managed xAI OAuth, then ``XAI_API_KEY`` (via ``get_env_value`` so
    ``~/.moor/.env`` keys count). ``prefer_api_key=True`` inverts that for API-metered
    endpoints where the subscription OAuth bearer authorizes but misbehaves (x_search answers
    without citations, TTS 403s). Both branches honor ``MOOR_XAI_BASE_URL``/``XAI_BASE_URL``
    ...
    Prefers moor-managed xAI OAuth credentials when available, then falls back to ``XAI_API_KEY`` resolved
    via ``moor_cli.config.get_env_value`` so keys stored in ``~/.moor/.env`` (the standard Moor
    ...
    scoping is identical to the fallback branch, and the base URL honors ``MOOR_XAI_BASE_URL`` /
