...
_YOLO_MODE_FROZEN: bool = is_truthy_value(os.getenv("MOOR_YOLO_MODE", ""))
...
# os.environ["MOOR_INTERACTIVE"] races: one session's restore in `finally`
...
# single-threaded CLI callers that still export MOOR_INTERACTIVE.
_moor_interactive_ctx: contextvars.ContextVar[Optional[str]] = contextvars.ContextVar(
    "moor_interactive",
    default=None,
)


def set_moor_interactive_context(interactive: bool) -> contextvars.Token:
    """Bind interactive mode for the current context (thread or asyncio task).

    Use this instead of mutating ``os.environ["MOOR_INTERACTIVE"]`` from
    concurrent executor threads. When unset (default), interactive detection
    falls back to the ``MOOR_INTERACTIVE`` env var for legacy callers.
    """
    return _moor_interactive_ctx.set("1" if interactive else "")


def reset_moor_interactive_context(token: contextvars.Token) -> None:
    """Restore the prior value from :func:`set_moor_interactive_context`."""
    _moor_interactive_ctx.reset(token)
...
def _is_interactive_cli() -> bool:
    ...
    back to the ``MOOR_INTERACTIVE`` env var for single-threaded callers.
    """
    ctx_val = _moor_interactive_ctx.get()
    if ctx_val is not None:
        return is_truthy_value(ctx_val)
    return env_var_enabled("MOOR_INTERACTIVE")
...
def get_current_session_key(default: str = "default") -> str:
    ...
    return get_session_env("MOOR_SESSION_KEY", default)
...
def _get_session_platform() -> str:
    ...
    try:
        ...
        return get_session_env("MOOR_SESSION_PLATFORM", "") or ""
    except Exception:
        return os.getenv("MOOR_SESSION_PLATFORM", "") or ""
...
def _is_cron_approval_context() -> bool:
    ...
    try:
        ...
        return is_truthy_value(get_session_env("MOOR_CRON_SESSION", ""))
    except Exception:
        return env_var_enabled("MOOR_CRON_SESSION")


def _is_single_query_approval_context() -> bool:
    """True when the current approval decision is from a single-query (-q) session.

    ``moor chat -q "..."`` runs one turn and exits with no user waiting to
    answer approval prompts, but it still exports ``MOOR_INTERACTIVE=1`` so
    ...
    try:
        ...
        return is_truthy_value(get_session_env("MOOR_SINGLE_QUERY_SESSION", ""))
    except Exception:
        return env_var_enabled("MOOR_SINGLE_QUERY_SESSION")


def _is_gateway_approval_context() -> bool:
    """True when this call is inside a gateway/API session.

    Legacy gateway integrations set MOOR_GATEWAY_SESSION in process env.
    Newer concurrent gateway paths bind MOOR_SESSION_PLATFORM via
    ...
    from a gateway platform (cron binds MOOR_SESSION_PLATFORM via
    ...
    if _is_cron_approval_context():
        return False
    if env_var_enabled("MOOR_GATEWAY_SESSION"):
        return True
...
def _should_fall_through_to_cli_approval(
    *,
    ...
    ``MOOR_EXEC_ASK`` (and sometimes a session platform marker) can leak into
...
# via shell expansions like $HOME or $MOOR_HOME, or by the resolved absolute
# active profile home path such as /home/moor/.moor/config.yaml. The
# resolved-absolute form is folded into the ~/.moor/ patterns at detection
...
# go stale when MOOR_HOME is set after this module is imported, e.g. under the
# hermetic test conftest or any deferred-profile-resolution path).
_SSH_SENSITIVE_PATH = r'(?:~|\$home|\$\{home\})/\.ssh(?:/|$)'
_MOOR_ENV_PATH = (
    r'(?:~\/\.moor/|'
    r'(?:\$home|\$\{home\})/\.moor/|'
    r'(?:\$moor_home|\$\{moor_home\})/)'
    r'\.env\b'
)
# ~/.moor/config.yaml IS the security policy: approvals.mode, yolo, and the
...
# theater. Mirrors _MOOR_ENV_PATH; matches the MOOR_HOME override form as
# well as ~/.moor/.
_MOOR_CONFIG_PATH = (
    r'(?:~\/\.moor/|'
    r'(?:\$home|\$\{home\})/\.moor/|'
...
def _save_blocked_payload(command: str) -> Optional[str]:
    ...
    try:
        ...
        script_dir = get_moor_home() / "cache" / "blocked-scripts"
...
DANGEROUS_PATTERNS = [
    ...
    # `moor` and `gateway` (e.g. `moor -p ade gateway restart`) so a
    ...
    (r'\bmoor\s+update\b', "moor update (restarts gateway, kills running agents)"),
    # Docker container lifecycle — any user with docker.sock mounted (a common
    # Docker Compose pattern) gives the agent the ability to restart/stop/kill
    # containers without approval.  These are agent-initiated lifecycle operations
    # that should always require user consent, just like `moor gateway restart`
    ...
    # a lifecycle command past the guard — same treatment as the `moor ...
    ...
    (r'gateway\s+run\b.*(&\s*$|&\s*;|\bdisown\b|\bsetsid\b)', "start gateway outside systemd (use 'systemctl --user restart moor-gateway')"),
    (r'\bnohup\b.*gateway\s+run\b', "start gateway outside systemd (use 'systemctl --user restart moor-gateway')"),
    # Self-termination protection: prevent agent from killing its own process
    (r'\b(pkill|killall)\b.*\b(moor|gateway|cli\.py)\b', "kill moor/gateway process (self-termination)"),
    # Self-termination via kill + command substitution (pgrep/pidof).
    # The name-based pattern above catches `pkill moor` but not
    # `kill -9 $(pgrep -f moor)` because the substitution is opaque
    ...
    # the `moor gateway stop|restart` pattern above by driving launchd
    # directly against the service label (commonly `ai.moor.gateway`).
    # Catch the operations that stop, restart, or unload it.
    #
    # Order-independent (2026-08-02 incident): the previous version required
    # "moor"/"ai.moor" to appear AFTER the launchctl verb in the same
    ...
    # 'ai.moor.gateway-apollo:...' ...; do label=${item%%:*}; launchctl
    # bootout "$label"; done` — never has the literal text "moor" appear
    ...
    # an unrelated "moor" mention now also matches) — for an approval gate
    # that's the correct direction to err: an extra approval prompt is
    # cheap, a missed one took down the whole gateway fleet.
    (r'(?=[\s\S]*\blaunchctl\s+(?:stop|kickstart|bootout|unload|kill|disable|remove)\b)(?=[\s\S]*\b(?:moor|ai\.moor)\b)', "stop/restart moor launchd service (kills running agents)"),
    # File copy/move/edit into sensitive system paths (/etc/ and macOS
    # /private/etc/ mirror).
    (rf'\b(cp|mv|install)\b.*\s{_SYSTEM_CONFIG_PATH}', "copy/move file into system config path"),
    (rf'\b(cp|mv|install)\b.*\s["\']?{_PROJECT_SENSITIVE_WRITE_TARGET}["\']?{_COMMAND_TAIL}', "overwrite project env/config file"),
    # cp/mv/install OVERWRITING a sensitive credential/SSH/shell-rc/Moor file.
    # The tee/redirection patterns above already gate _SENSITIVE_WRITE_TARGET
    # (~/.ssh/*, ~/.netrc/.pgpass/.npmrc/.pypirc, shell rc files,
    # ~/.moor/config.yaml/.env), but cp/mv/install was only paired for /etc and
    ...
    # In-place edit of a moor-managed security file (~/.moor/config.yaml or
    ...
    (rf'\bsed\s+-[^\s]*i.*(?:{_MOOR_CONFIG_PATH}|{_MOOR_ENV_PATH})', "in-place edit of Moor config/env"),
    (rf'\bsed\s+--in-place\b.*(?:{_MOOR_CONFIG_PATH}|{_MOOR_ENV_PATH})', "in-place edit of Moor config/env (long flag)"),
    ...
    (rf'\b(?:perl|ruby)\b.*(?:^|\s)-[^\s]*i\b.*(?:{_MOOR_CONFIG_PATH}|{_MOOR_ENV_PATH})', "in-place edit of Moor config/env (perl/ruby)"),
...
def _normalize_command_for_detection(command: str) -> str:
    ...
    # ~/ and ~/.moor/ forms so static user-sensitive patterns catch
    ...
    # it tracks HOME / MOOR_HOME even when those are set after this module is
    ...
    #
    # Fold the (more specific) Moor home first: on Windows it nests under the
    # user home (C:\Users\alice\AppData\...\moor), so folding the user home
    # first would eat the prefix the moor-home fold needs.
    command = _rewrite_resolved_moor_home(command)
...
def _home_prefix_fold_regex(path: str):
    ...
    components below the root — so a stray HOME / MOOR_HOME such as ``/``,
...
def _fold_home_prefixes(command: str, paths, replacement: str) -> str:
    ...
    *replacement* has no trailing separator (``~`` / ``~/.moor``); the matched
...
def _rewrite_resolved_moor_home(command: str) -> str:
    """Rewrite the resolved absolute Moor home prefix to ``~/.moor/``.

    Resolves the active ``MOOR_HOME`` at call time (and its symlink-resolved
    form) and folds an occurrence of ``<home>/`` in *command* into
    ``~/.moor/`` so the static ``_MOOR_CONFIG_PATH`` / ``_MOOR_ENV_PATH``
    patterns match. In Docker and gateway deployments the agent often references
    the resolved absolute path directly (e.g. ``sed -i ...
    /home/moor/.moor/config.yaml``) rather than ``~``, ``$HOME``, or
    ``$MOOR_HOME``. Matches both POSIX and Windows separators. No-op when the
    ...
    try:
        ...
        return command
    return _fold_home_prefixes(command, candidates, "~/.moor")
...
def _mask_quoted_newlines(command: str) -> str:
    ...
    start. That made any multi-line quoted argument (``moor send`` message
...
def _is_verification_artifact_cleanup(command: str) -> bool:
    """Return whether *command* only removes one Moor ad-hoc temp script."""
    try:
    ...
    if os.path.dirname(target) != temp_dir:
        return False
    return re.fullmatch(r"moor-(?:verify|ad-hoc)-[A-Za-z0-9_.-]+", basename) is not None


_GATEWAY_LIFECYCLE_SPLICE_DESCRIPTION = (
    "stop/restart moor gateway via shell-spliced verb (kills running agents)"
)
...
def _is_shell_token_spliced_gateway_lifecycle(command: str) -> bool:
    ...
    ``launchctl kick"start" -k gui/501/ai.moor.gateway`` auto-approved
    ...
    anchored on a moor-gateway identifier — reusing its prose
...
def is_approval_bypass_active_for_session(session_key: str) -> bool:
    ...
    into one place:
      - process-scoped ``--yolo`` / ``MOOR_YOLO_MODE`` (frozen at import time
    ...
    *,
    ...
    Args:
        ...
        fail_closed_when_no_human: When True, a non-interactive non-gateway
            ...
            MOOR_INTERACTIVE unset) BLOCKS instead of auto-approving. The
    ...
    # Single-query (-q) sessions export MOOR_INTERACTIVE=1 but have no user
    ...
    if not is_cli and not is_gateway:
        ...
        if _is_cron_approval_context():
            ...
            logger.warning(
                ...
                "BLOCKED (fail-closed). Set MOOR_INTERACTIVE or "
                "MOOR_GATEWAY_SESSION to answer the prompt.",
                autoapprove_log_prefix, pattern_key, description,
            )
            return {
                "approved": False,
        ...
        logger.warning(
            "%s (pattern: %s): %s — set MOOR_INTERACTIVE or "
            "MOOR_GATEWAY_SESSION to require approval.",
            autoapprove_log_prefix, pattern_key, description,
        )
        return {"approved": True, "message": None}

    if is_gateway or env_var_enabled("MOOR_EXEC_ASK"):
        ...
        # can see (MOOR_EXEC_ASK / platform-marker leaks into CLI).
...
def request_tool_approval(
    ...
    *,
    ...
    (a bare script with no ``MOOR_INTERACTIVE``) fails CLOSED — a plugin-
...
def get_plugin_manager():
    ...
    from moor_cli.plugins import discover_plugins, get_plugin_manager as _get_manager
...
def _present_with_selected_transport(
    *,
    ...
    try:
        from agent.redact import redact_sensitive_text
        from moor_cli.approval_transport import ApprovalRequest, invoke_approval_transport
...
def check_all_command_guards(command: str, env_type: str,
    ...
    if _is_single_query_approval_context():
        is_cli = False
        is_gateway = False
        # MOOR_EXEC_ASK routes through the gateway decision loop (no human
        ...
        if _is_single_query_approval_context():
            if _get_single_query_approval_mode() == "deny":
                ...
                try:
                    ...
                    try:
                        from moor_cli.config import load_config_readonly as _load_cfg
...
def check_execute_code_guard(code: str, env_type: str,
    ...
    # Ask-mode (MOOR_EXEC_ASK) still takes this path even when INTERACTIVE
    ...
    if notify_cb is None:
        # MOOR_EXEC_ASK (and sometimes a session platform marker) can leak
