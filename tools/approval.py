...
def _release_permission_mode_dependents(session_key: str) -> None:
    """Drop resources whose immutable mode derives from Moor YOLO. Lazy import so approval-only
...
def _unattended_contexts() -> list[_Unattended]:
    """Active unattended contexts in evaluation order: single-query first (``moor chat -q``
    exports MOOR_INTERACTIVE=1 but nobody answers); cron beats a platform marker because
...
def _presence(approval_callback=None) -> tuple:
    ...
    (-q) exports MOOR_INTERACTIVE=1 but nobody answers prompts, and MOOR_EXEC_ASK has no
        ...
        for ctx in _unattended_contexts():
            ...
            if fail_closed_when_no_human:
                logger.warning("%s (pattern: %s): %s — no interactive user/gateway present; "
                               ...
                               "MOOR_GATEWAY_SESSION to answer the prompt.", *log_args)
                return _blocked(no_human_block_message or (
                    ...
                    pattern_key=pattern_key, description=description)
        logger.warning("%s (pattern: %s): %s — set MOOR_INTERACTIVE or "
                       "MOOR_GATEWAY_SESSION to require approval.", *log_args)
        return _approved()
...
_PLUGIN_COMPAT_LAZY = {
    ...
    'reset_moor_interactive_context': ('tools.approval_context', 'reset_moor_interactive_context'),
    ...
    'set_moor_interactive_context': ('tools.approval_context', 'set_moor_interactive_context'),
}
