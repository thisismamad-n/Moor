"""`moor computer-use doctor` — thin client for cua-driver's `health_report` MCP tool. cua-driver owns the health
...
def run_doctor(driver_cmd: Optional[str] = None, *, include: Sequence[str] = (), skip: Sequence[str] = (), json_output: bool = False,
    ...
    try:  # prefer real health_report; on denial/non-schema, synthesize via probes
        ...
        print(f"cua-driver could not be started from {binary!r}: {e}\n"
              "  The Moor runtime interpreter cannot execute this binary; the tool may still work because the\n"
              "  shell resolves a different copy on PATH. Fix: install cua-driver outside the protected directory\n"
              "  (e.g. the upstream installer's default under your user profile) or point MOOR_CUA_DRIVER_CMD at\n"
              "  a copy the runtime can execute, then re-run `moor computer-use doctor`.", file=sys.stderr)
        return 2
    except RuntimeError as e:
    ...
    if json_output:
        # Additive envelope: upstream keys preserved, identity under moor_identity (and environment under
        # moor_environment when present) so overall/checks parsers keep working.
        payload = {**report, "moor_identity": identity}
        if environment:
            payload["moor_environment"] = environment
