...
etc.) surface as specific blocked checks via `moor computer-use doctor`
...
# fetches the latest release, so a `MOOR_CUA_DRIVER_VERSION` env var would
# only have *looked* like it pinned. For a reproducible version, point
# `MOOR_CUA_DRIVER_CMD` at a specific binary instead.
...
def _standard_runtime_launch_args(
    ...
    *,
    ...
    private_socket = socket_path or os.path.join(
        tempfile.gettempdir(), f"moor-cua-standard-{uuid.uuid4().hex[:12]}.sock"
    )
...
def _empty_discovery_reason() -> str:
    ...
    return (
        "window discovery returned no windows; run `moor computer-use "
    ...
    * ``bounded`` — a user-reviewed capability manifest
    ...
    def __init__(
        ...
        if sys.platform == "win32":
            self.socket_path = rf"\\.\pipe\moor-cua-{token}"
        else:
...
def _candidate_cua_driver_commands(override: Optional[str] = None) -> List[str]:
    ...
    ``MOOR_CUA_DRIVER_CMD`` is authoritative; only when neither is set do we
    ...
    tool even though `moor computer-use doctor` succeeds from a login shell.
    """
...
def resolve_cua_driver_cmd(override: Optional[str] = None) -> Optional[str]:
    ...
    A supplied override (or ``MOOR_CUA_DRIVER_CMD``) is never silently
...
def cua_driver_update_nudge() -> Optional[str]:
    ...
    return (
        ...
        f"update with `moor computer-use install --upgrade`."
    )
...
def _maybe_repair_runtime_contract(contract: Dict[str, Any]) -> Dict[str, Any]:
    ...
    ``MOOR_CUA_DRIVER_CMD`` override is authoritative even when broken, and
    ...
    def _start_lifecycle_locked(self) -> None:
        ...
        if not self._ready_event.wait(timeout=30.0):
            ...
            raise RuntimeError(
                ...
                "Run `moor computer-use doctor` and check "
                f"{display_moor_home()}/logs/agent.log for the phase timings."
            )
...
class CuaDriverBackend(ComputerUseBackend):
    ...
    def __init__(self, permission_mode: str = "standard") -> None:
        ...
        # backend startup.
        self._session_id: str = f"moor-{uuid.uuid4().hex[:12]}"
    ...
    def start(self) -> None:
        ...
        if not contract.get("ready"):
            ...
            if os.environ.get(_CUA_DRIVER_CMD_ENV, "").strip():
                repair = (
                    "Update the binary selected by MOOR_CUA_DRIVER_CMD or "
                    "remove that override."
                )
            else:
                repair = "Run `moor computer-use install` to repair it."
    ...
    def capture(
        ...
        Maps moor `capture(mode, app)` → cua-driver `list_windows` +
