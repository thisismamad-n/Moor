...
def sanitized_cua_driver_env() -> Dict[str, str]:
    """``cua_driver_child_env()`` with Moor provider secrets stripped — cua-driver is a third-party binary and must
...
def _empty_discovery_reason() -> str:
    ...
    if sys.platform == "darwin":  # headless Mac / asleep panel: ScreenCaptureKit has 0 shareable displays while TCC looks fine
        return ("window discovery returned no windows; on macOS this usually means no shareable display (headless Mac or "
                "panel asleep) — wake the display or attach a monitor/HDMI dummy, then run `moor computer-use doctor`")
    return "window discovery returned no windows; run `moor computer-use doctor` (display reachability, AX capability)"
...
def _maybe_repair_runtime_contract(contract: Dict[str, Any]) -> Dict[str, Any]:
    ...
    attempted / it failed). Never raises. An explicit ``MOOR_CUA_DRIVER_CMD`` override is authoritative even
...
class CuaDriverBackend(_CaptureMixin, _InputMixin, ComputerUseBackend):
    ...
    def __init__(self, permission_mode: str = "standard") -> None:
        ...
        # Public session label (one per Moor run) sent as `session` on every call: owns the cursor color and
