...
The LLM writes a Python script that calls Moor tools via RPC, collapsing
...
# Loopback TCP replaces AF_UNIX on Windows, so execute_code runs on every platform Moor does.
...
def _spill_full_stdout(stdout_text: str) -> Optional[str]:
    ...
    try:
        ...
        cache_dir = get_moor_dir("cache/exec", "exec_spill")
...
# these as the top execute_code failure classes: moor_tools import misuse, importing the built-in
...
def generate_moor_tools_module(enabled_tools: List[str],
    ...
    """Source of the moor_tools.py stub module for SANDBOX_ALLOWED_TOOLS ∩ *enabled_tools*.
...
def _call(tool_name, args):
    ...
    # re-accepts (MOOR_RPC_PERSISTENT=1); retry once on a fresh socket.
    _attempts = 2 if os.environ.get("MOOR_RPC_PERSISTENT") == "1" else 1
    with _call_lock:
...
def _run_remote_per_call(env, env_type: str, code: str, effective_task_id: str,
    ...
    """Per-call script ship: stage moor_tools.py + script.py in a fresh remote sandbox dir,
    serve file-RPC from a polling thread, run, clean up."""
    sandbox_dir = f"{_env_temp_dir(env)}/moor_exec_{uuid.uuid4().hex[:12]}"
    ...
    try:
        ...
        _ship_file_to_remote(env, f"{sandbox_dir}/moor_tools.py",
                             generate_moor_tools_module(list(sandbox_tools), transport="file"))
        ...
        rpc_thread.start()
        env_prefix = (f"MOOR_RPC_DIR={quoted_rpc_dir} MOOR_RPC_TOKEN={shlex.quote(rpc_token)} "
...
def _execute_remote(code: str, task_id: Optional[str], enabled_tools: Optional[List[str]],
    ...
    try:
        ...
        try:
            # --- Session-kernel path (moor-agent#96873) ------------------- Same always-on model as
...
def execute_code(
    ...
    with RPC access to a subset of Moor tools; returns the JSON result string. "Sandbox" means
    ...
    # execute_code is a straight bypass — the terminal() path refuses `launchctl bootout ai.moor.gateway`,
...
def build_execute_code_schema(enabled_sandbox_tools: set = None,
    ...
    if mode == "strict":
        ...
        cwd_note = (
            ...
            "Moor's own python (the common case — stdlib plus Moor's "
    ...
    description = (
        "Run Python that calls Moor tools programmatically. Use when you "
