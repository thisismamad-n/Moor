...
Lets the LLM write a Python script that calls Moor tools via RPC,
collapsing multi-step tool chains into a single inference turn.

Architecture (two transports):

  **Local backend (UDS):**
  1. Parent generates a `moor_tools.py` stub module with UDS RPC functions
  ...
  **Remote backends (file-based RPC):**
  1. Parent generates `moor_tools.py` with file-based RPC stubs
...
# available on every platform Moor itself runs on.
...
# match a safe prefix, the operational MOOR_ allowlist, or (on Windows) an
...
# MOOR_HOME.
#
# NB: the broad "MOOR_" prefix was deliberately removed (#27303) — it leaked
# MOOR_*-named config that lacks a secret substring (e.g. MOOR_BASE_URL,
# MOOR_KANBAN_DB, MOOR_*_WEBHOOK).  The child only needs the few
# location/profile vars in _MOOR_CHILD_ALLOWED below; MOOR_RPC_SOCKET /
# MOOR_RPC_DIR / TZ / HOME are injected explicitly after scrubbing.
...
# Operational MOOR_* vars the child legitimately needs by exact name — these
# are non-secret runtime-location flags (the same set moor_cli treats as the
# runtime location) that repo-root modules a sandbox script imports may read at
# import time.  None match _SECRET_SUBSTRINGS.
_MOOR_CHILD_ALLOWED = frozenset({
    "MOOR_HOME",
    "MOOR_PROFILE",
    "MOOR_CONFIG",
    "MOOR_ENV",
...
def _scrub_child_env(source_env, is_passthrough=None, is_windows=None):
    ...
    Rules (order matters):
      ...
      4. Operational MOOR_* vars (_MOOR_CHILD_ALLOWED) pass by exact name.
    ...
    scrubbed = {}
    # Non-secret MOOR_* vars dropped by the tightened allowlist (#27303). The
    # broad "MOOR_" prefix used to pass these through; now only the
    # operational set does. The drop is intentional (those vars can carry
    # config like MOOR_KANBAN_DB / MOOR_BASE_URL), but a sandbox script
    ...
    for k, v in source_env.items():
        ...
        if any(k.startswith(p) for p in _SAFE_ENV_PREFIXES):
            scrubbed[k] = v
            continue
        if k in _MOOR_CHILD_ALLOWED:
            scrubbed[k] = v
            continue
        if is_windows and k.upper() in _WINDOWS_ESSENTIAL_ENV_VARS:
            scrubbed[k] = v
            continue
        if k.startswith("MOOR_"):
            # Non-secret (secrets were already dropped above) and not in any
            # allowlist — a deliberately-dropped MOOR_* var.
            _dropped_moor.append(k)
    if _dropped_moor:
        logger.debug(
            "execute_code: dropped %d non-allowlisted MOOR_* var(s) from the "
...
_TOOL_STUBS = {
    ...
    "write_file": (
        ...
        '"""Write content to a file (always overwrites). Returns dict with status. cross_profile=True opts out of the cross-Moor-profile soft guard."""',
    ...
    "patch": (
        ...
        '"""Targeted find-and-replace (mode="replace") or V4A multi-file patches (mode="patch"). Returns dict with status. cross_profile=True opts out of the cross-Moor-profile soft guard."""',
...
def _sandbox_failure_hint(stderr_text: str, enabled_tools=None) -> Optional[str]:
    ...
    moor_tools import misuse (importing tools that aren't in the sandbox,
    ...
    try:
        m = re.search(
            r"cannot import name '(\w+)' from 'moor_tools'", window
        )
        if m:
            missing = m.group(1)
...
def generate_moor_tools_module(enabled_tools: List[str],
                                 transport: str = "uds") -> str:
    """
    Build the source code for the moor_tools.py stub module.
...
"""Auto-generated Moor tools RPC stubs."""
...
def _connect():
    ...
    global _sock
    if _sock is None:
        endpoint = os.environ["MOOR_RPC_SOCKET"]
...
def _call(tool_name, args):
    ...
    request = json.dumps({
        "tool": tool_name,
        "args": args,
        "token": os.environ.get("MOOR_RPC_TOKEN", ""),
    }) + "\\n"
    with _call_lock:
        conn = _connect()
...
"""Auto-generated Moor tools RPC stubs (file-based transport)."""
import json, os, shlex, tempfile, threading, time

_RPC_DIR = os.environ.get("MOOR_RPC_DIR") or os.path.join(tempfile.gettempdir(), "moor_rpc")
_seq = 0
...
def _execute_remote(
    ...
    The script and the generated moor_tools.py module are shipped to
    ...
    sandbox_dir = f"{temp_dir}/moor_exec_{sandbox_id}"
    ...
    try:
        ...
        tools_src = generate_moor_tools_module(
            list(sandbox_tools), transport="file",
        )
        _ship_file_to_remote(env, f"{sandbox_dir}/moor_tools.py", tools_src)
        ...
        env_prefix = (
            f"MOOR_RPC_DIR={shlex.quote(f'{sandbox_dir}/rpc')} "
            f"MOOR_RPC_TOKEN={shlex.quote(rpc_token)} "
            f"PYTHONDONTWRITEBYTECODE=1"
        )
        tz = os.getenv("MOOR_TIMEZONE", "").strip()
        if tz:
...
def execute_code(
    ...
    # terminal() path refuses `launchctl bootout ai.moor.gateway`, but the
    ...
    # --- Set up temp directory with moor_tools.py and script.py ---
    tmpdir = tempfile.mkdtemp(prefix="moor_sandbox_")
    ...
    # selector from MOOR_RPC_SOCKET (path vs. ``tcp://host:port``).
    ...
    if _use_tcp_rpc:
        ...
        sock_path = os.path.join(_sock_tmpdir, f"moor_rpc_{uuid.uuid4().hex}.sock")
    ...
    try:
        # Write the auto-generated moor_tools module.
        ...
        tools_src = generate_moor_tools_module(list(sandbox_tools))
        with open(os.path.join(tmpdir, "moor_tools.py"), "w", encoding="utf-8") as f:
            f.write(tools_src)
        ...
        #   connect.  MOOR_RPC_SOCKET is set to ``tcp://127.0.0.1:<port>``
        ...
        # MOOR_TIMEZONE is an internal Moor setting and must not leak
        # into child processes.
        _tz_name = os.getenv("MOOR_TIMEZONE", "").strip()
        if _tz_name:
        ...
        # ``moor_tools.py`` always lives in the staging directory, so that
        ...
        _strip_moor_owned_pythonpath(child_env)
        _moor_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        _existing_pp = child_env.get("PYTHONPATH", "")
        _pp_parts = [tmpdir]
        if _uses_moor_python_environment(_child_python):
            _pp_parts.append(_moor_root)
        elif _child_python not in _external_env_logged:
            # Import behavior changes silently otherwise — surface it (once
            # per interpreter path) so "import moor_constants suddenly
            ...
            logger.info(
                ...
                "environment; moor root omitted from PYTHONPATH",
                _child_python,
            )
        if _existing_pp:
        ...
        # but scripts can still read secrets from disk (e.g. open('~/.moor/.env')).
...
def _load_config() -> dict:
    ...
    ``moor --tui`` where it is never used.  Read the lightweight raw config
...
def _get_execution_mode() -> str:
    ...
    Mode semantics:
      ...
      - ``strict``: scripts run in an isolated temp directory with
        ``sys.executable`` (moor-agent's python). Reproducible and the
...
def _python_environment_prefix(python_path: str) -> str:
    ...
    sticky empty result would silently drop the moor root from every
...
def _uses_moor_python_environment(python_path: str) -> bool:
...
def build_execute_code_schema(enabled_sandbox_tools: set = None,
    ...
    When tools are disabled via ``moor tools`` (e.g. web is turned off),
    ...
    # temp-dir staging and moor-agent's own python.
    if mode == "strict":
        cwd_note = (
    ...
    description = (
        ...
        f"Available via `from moor_tools import ...`:\n\n"
        f"{tool_lines}\n\n"
            ...
            "properties": {
                "code": {
                    ...
                    "description": (
                        ...
                        f"`from moor_tools import {import_str}` "
