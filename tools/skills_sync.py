...
~/.moor/skills/, tracking each synced skill's origin hash in .bundled_manifest (v2 "name:hash"
...
# retarget MOOR_HOME after import, and frozen constants would resolve (and for
...
# once under the launch MOOR_HOME and later scope requests to a different profile via
# set_moor_home_override(). See #65828.
...
def _moor_home() -> Path:
    return _live(MOOR_HOME, _MOOR_HOME_AT_IMPORT, get_moor_home)


def _skills_dir() -> Path:
    return _live(SKILLS_DIR, _SKILLS_DIR_AT_IMPORT, lambda: _moor_home() / "skills")
...
# Written by `moor profile create --no-skills` / installer `--no-skills`: sync seeds only
# essential skills. Mirrors moor_cli.profiles.NO_BUNDLED_SKILLS_MARKER (no CLI import here).
NO_BUNDLED_SKILLS_MARKER = ".no-bundled-skills"


def _get_bundled_dir() -> Path:  # MOOR_BUNDLED_SKILLS env first, then repo-relative
...
def _write_manifest(entries: Dict[str, str]):
    ...
    mkdir_under_moor_home(_manifest_file().parent)
    try:
    ...
    for candidate in st.active_index.get(skill_name, []):
        ...
        if not _matches_origin_hash(candidate, origin_hash):  # moving a customized copy would edit user work
            st.say(
                ...
                f"Run `moor skills reset {skill_name} --restore` to move to the new location.")
            continue
        try:
        ...
        if dest.exists():
            ...
            if _dir_hash(dest) == bundled_hash:
                ...
                st.say(
                    ...
                    f"by this name — yours was kept. Run `moor skills reset {skill_name}` to "
...
def sync_skills(quiet: bool = False) -> dict:
    """Sync bundled skills into ~/.moor/skills/ using the manifest; returns the per-category
    result dict. Opted-out profiles seed ONLY ESSENTIAL_SKILLS (the system prompt always
    points at ``moor-agent``)."""
...
def _rmtree_writable(path: Path) -> None:
    ...
    child of the active skills root (bad join / missing MOOR_HOME / malicious manifest entry).
