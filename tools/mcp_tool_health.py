...
class MCPServerHealthMixin:
    ...
    def _make_logging_callback(self):
        """``logging_callback`` forwarding server ``notifications/message`` into Moor logging (SDK default drops them).

        Routes MCP ``notifications/message`` log notifications from the server into Moor' logging
        (agent.log via moor_logging), tagged with the server name. Without this, the SDK's default
