...
Backend is selected during ``moor tools`` (``web.backend`` in config.yaml; per
...
def _env_value(name: str) -> str:
    """Resolve ``name`` via the config-aware env layer (``moor config set`` values), then process env.

    Mirrors the SearXNG provider's ``_searxng_url()`` so that values set through Moor' config/.env layer
    (``moor config set``, ``moor tools``) are honored here too — not just raw process-env exports.
...
def _web_requires_env() -> list[str]:
    ...
    on ``managed_moor_tools_enabled()`` cost a synchronous portal HTTP refresh at every CLI startup.
...
def web_search_tool(query: str, limit: int = 5) -> str:
    ...
    try:
        ...
        if provider is None:
            fallback = "No web search provider configured. Run `moor tools` to set one up."
...
def _provider_is_ready(provider) -> bool:
    ...
    False (so dispatch can emit a precise error), so readiness gates (tool check_fn, ``moor doctor``)
...
_PLUGIN_COMPAT_LAZY = {
    ...
    'moor_tool_gateway_unavailable_message': ('tools.tool_backend_helpers', 'moor_tool_gateway_unavailable_message'),
