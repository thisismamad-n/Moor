...
Backend is selected during ``moor tools`` setup (web.backend in config.yaml).
When available, Moor can route Firecrawl calls through a Moor-hosted tool-gateway
for Moor Subscribers only.
...
- Firecrawl: https://docs.firecrawl.dev/introduction (search, extract; direct or derived firecrawl-gateway.<domain> for Moor Subscribers)
...
from tools.managed_tool_gateway import (  # noqa: F401 — backward-compat names for tests
    ...
    peek_moor_access_token as _peek_moor_access_token,
    read_moor_access_token as _read_moor_access_token,
...
def _env_value(name: str) -> str:
    """Resolve ``name`` via Moor config-aware env, falling back to process env.

    Mirrors the SearXNG provider's ``_searxng_url()`` so that values set
    through Moor' config/.env layer (``moor config set``, ``moor tools``)
...
def _load_web_config() -> dict:
    """Load the ``web:`` section from ~/.moor/config.yaml."""
    try:
...
def _get_backend() -> str:
    ...
    Reads ``web.backend`` from config.yaml (set by ``moor tools``). A
    ...
    if configured:
        ...
        # The managed "Moor Subscription" selection ("moor") is serviced by
        ...
        if configured == MOOR_MANAGED_PROVIDER:
            return "firecrawl"
        return configured
    ...
    # pre-empted by a Moor OAuth token whose subscription tier may not
...
def _is_backend_available(backend: str) -> bool:
    ...
    if backend == "xai":
        ...
        # runs on every web_search dispatch + every `moor tools` repaint.
        try:
...
def _web_requires_env() -> list[str]:
    ...
    set.  Gating them on ``managed_moor_tools_enabled()`` only saved
    string noise in the metadata list, but cost a synchronous HTTP
    refresh against the Moor portal on every CLI startup (invoked at
            ...
            if provider is None and backend and selection_exists("web"):
                ...
                if disabled_key:
                    ...
                    error_text = (
                        ...
                        f"Re-enable it with `moor plugins enable {disabled_key}` "
                ...
                if backend and selection_exists("web"):
                    ...
                    if disabled_key:
                        ...
                        error_text = (
                            ...
                            f"config. Re-enable it with `moor plugins "
                        ...
                        return json.dumps(
                            {
                                ...
                                "error": (
                                    ...
                                    f"`moor plugins enable {disabled_key}` "
                                    "(or remove it from plugins.disabled)."
                                ),
                            },
                            ensure_ascii=False,
...
def _provider_is_ready(provider) -> bool:
    ...
    require a true availability probe — otherwise ``moor doctor`` paints a
...
if __name__ == "__main__":
    ...
    from moor_cli.config import get_env_value as _gev
