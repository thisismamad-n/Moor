...
# `cronjob(action="run")` executes in-process (comfortably below MOOR_AGENT_TIMEOUT).
# Mirrors the 10s cadence of tools/environments/base.py::touch_activity_if_due (delegate_task's heartbeat
# uses 30s) — comfortably below the 1800s default MOOR_AGENT_TIMEOUT. See #76502.
_CRON_RUN_HEARTBEAT_INTERVAL = 10.0
# Hard ceiling: with MOOR_CRON_TIMEOUT=0 a truly hung run would otherwise mask the
# gateway watchdog forever; past this the heartbeat stops and the watchdog regains authority.
# The child cron run has its own inactivity watchdog (MOOR_CRON_TIMEOUT, default 600s) that bounds a
# wedged job, but with MOOR_CRON_TIMEOUT=0 (explicit "unlimited") a truly hung run_one_job would otherwise
...
def _reap_stale_executions(job_name: str) -> None:
    ...
    one-shot `moor cron run` that died mid-run). The ticker does this at startup; one-shot
...
def _try_dispatch_background_run(
    ...
    # completion, so don't claim-and-dispatch (direct callers like `moor cron run` exit right after).
...
Jobs run in a fresh session with no current-chat context, so prompts must be self-contained, and the agent's FINAL RESPONSE is what gets delivered — cron runs are autonomous and cannot ask questions. Prefer updating an existing job over creating near-duplicates.""",
    "parameters": {
        ...
        "properties": {
            ...
            "script": {
                "type": "string",
                "description": f"Optional script run each tick; stdout is injected into the agent's prompt as context (with no_agent=True the script IS the job). Relative paths resolve under {display_moor_home()}/scripts/; .sh/.bash via bash, else Python. On update, '' clears."
            },
            "monitor": {
                "type": "string",
...
# intentionally NOT here: per-job inference pins are user-owned (dashboard, `moor cron
