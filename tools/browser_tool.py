...
# dependency could read every Moor secret from process.env.
...
# than a concrete path (also compared in moor_cli/tools_config.py and doctor.py).
...
_REAL_PROFILE_SESSION = "moor-real-profile"
...
def _socket_safe_tmpdir() -> str:
    """Short temp dir for Unix sockets: macOS ``TMPDIR`` + ``agent-browser-moor_…``
...
# Owner Moor home per session: the janitor is one process-global thread, so each
...
_PLUGIN_COMPAT_LAZY = {
    ...
    'get_moor_home_override': ('moor_constants', 'get_moor_home_override'),
    'moor_home_key': ('moor_constants', 'moor_home_key'),
