#!/usr/bin/env python3
"""Image generation via FAL.ai (model picked in ``moor tools``, persisted to ``image_gen.model``).
...
from tools.tool_backend_helpers import (
    MOOR_MANAGED_PROVIDER, fal_key_is_configured, managed_moor_tools_enabled,
    moor_tool_gateway_unavailable_message, read_selection, selection_error)
...
def _resolve_managed_fal_gateway():
    """Managed gateway config for the stored `moor tools` selection, or ``None`` for direct FAL.

    ``"moor"`` (or legacy ``use_gateway: true``) → managed ONLY (unreachable = selection-naming
    ...
    if selected == MOOR_MANAGED_PROVIDER:
        ...
        if gateway is None:
            raise ValueError(selection_error(
                "image_gen", MOOR_MANAGED_PROVIDER,
                "the Moor Tool Gateway is not available (not entitled or unreachable)"))
        return gateway
...
def _get_managed_fal_client(managed_gateway):
    ...
    client_config = (managed_gateway.gateway_origin.rstrip("/"), managed_gateway.moor_user_token)
    with _managed_fal_client_lock:
        if _managed_fal_client is None or _managed_fal_client_config != client_config:
            ...
            _managed_fal_client = _ManagedFalSyncClient(
                _load_fal_client(), key=managed_gateway.moor_user_token,
...
def _submit_fal_request(model: str, arguments: Dict[str, Any]):
    ...
    try:
        ...
        if status is not None and 400 <= status < 500:
            ...
            raise ValueError(
                f"Moor Subscription gateway rejected model '{model}' (HTTP {status}). This model "
                f"may not yet be enabled on the Moor Portal's FAL proxy. Either:\n"
                f"  • Set FAL_KEY in your environment to use FAL.ai directly, or\n"
                f"  • Pick a different model via `moor tools` → Image Generation."
...
def _plugin_provider_name() -> Optional[str]:
    """Configured provider that must go through the plugin registry; None for unset/fal/moor."""
    configured = _read_configured_image_provider()
    if not configured or configured in ("fal", MOOR_MANAGED_PROVIDER):
        return None
    return configured
...
_CACHE_BASE_BY_BACKEND = {"docker": "/root/.moor", "singularity": "/root/.moor",
                          "modal": "/root/.moor", "ssh": "~/.moor"}
...
def _prepare_fal_request(model_id, meta, prompt, aspect_ratio, seed, overrides, source_images):
    ...
    if source_images and not edit_endpoint:
        raise ValueError(
            ...
            f"via `moor tools` → Image Generation.")
...
def _build_no_backend_setup_message() -> str:
    ...
    managed = managed_moor_tools_enabled()
    ...
    if managed:
        ...
        if gateway_message := moor_tool_gateway_unavailable_message("managed FAL image generation"):
    ...
    if managed:
        lines.append("  2. Sign in to a Moor account that has the managed FAL gateway enabled "
                     "(`moor setup`)")
    lines.append("  3. Configure a different image_gen provider via `moor tools` → Image Generation "
                 "(run `moor plugins list` to see installed backends)")
    return "\n".join(lines)
...
def _dispatch_to_plugin_provider(
    ...
    (provider unset / ``"fal"`` / ``"moor"``). Providers without ``upscale`` ignore it via ``**kwargs``."""
    configured = _plugin_provider_name()
    if configured is None:
        return None
    try:
    ...
    if provider is None:
        return _provider_error(
            ...
            f"Run `moor plugins list` to see available image gen backends.", "provider_not_registered")
    ...
    try:
        ...
        if is_type_error and ("image_url" in kwargs or "reference_image_urls" in kwargs):
            ...
            return _provider_error(
                ...
                f"text-to-image, or pick a backend that supports editing via `moor tools` → "
...
def _maybe_route_managed_krea(
    ...
    ``"moor"`` stored (a picker choice dispatches normally) and a resolvable Krea gateway.
    """
    configured_provider = _read_configured_image_provider()
    if configured_provider is not None and configured_provider != MOOR_MANAGED_PROVIDER:
        return None
