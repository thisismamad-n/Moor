...
selectable via ``moor tools`` → Image Generation; the active model is
...
FAL_MODELS: Dict[str, Dict[str, Any]] = {
    ...
    "fal-ai/nano-banana-pro": {
        ...
        "defaults": {
            ...
            # Users on Moor Subscription should stay at 1K for predictable billing.
            "resolution": "1K",
        },
        "supports": {
    ...
    "fal-ai/gpt-image-2": {
        ...
        "defaults": {
            # Same quality pinning as gpt-image-1.5: medium keeps Moor
...
def _resolve_managed_fal_gateway():
    """Resolve the FAL route from the stored `moor tools` selection.

    Dispatch is a plain switch on the stored ``image_gen`` provider string:
    - ``"moor"`` (or legacy ``use_gateway: true``) → managed fal-queue
    ...
    if selected == MOOR_MANAGED_PROVIDER:
        ...
        if gateway is None:
            raise ValueError(selection_error(
                "image_gen",
                MOOR_MANAGED_PROVIDER,
                "the Moor Tool Gateway is not available (not entitled or "
                "unreachable)",
            ))
        return gateway
...
def _get_managed_fal_client(managed_gateway):
    ...
    with _managed_fal_client_lock:
        ...
        _managed_fal_client = _ManagedFalSyncClient(
            fal_client,
            key=managed_gateway.moor_user_token,
...
def _submit_fal_request(model: str, arguments: Dict[str, Any]):
    ...
    try:
        ...
        if status is not None and 400 <= status < 500:
            ...
            raise ValueError(
                f"Moor Subscription gateway rejected model '{model}' "
                f"(HTTP {status}). This model may not yet be enabled on "
                f"the Moor Portal's FAL proxy. Either:\n"
...
def _agent_cache_base_for_env(env: Any) -> str | None:
    ...
    # Moor cache roots can be translated without side effects. SSH can still
...
def check_fal_api_key() -> bool:
    """True if the FAL backend selected via `moor tools` (or, on a
...
def _build_no_backend_setup_message() -> str:
    ...
    Used by the in-tree FAL path. Mentions:
      - FAL_KEY signup link
      - managed-gateway status (if Moor tools are enabled)
    ...
    if managed_moor_tools_enabled():
        ...
        gateway_message = moor_tool_gateway_unavailable_message(
            "managed FAL image generation",
        )
        if gateway_message:
    ...
    if managed_moor_tools_enabled():
        lines.append(
            "  2. Sign in to a Moor account that has the managed FAL "
            "gateway enabled (`moor setup`)"
        )
    lines.append(
        "  3. Configure a different image_gen provider via `moor tools` "
        "→ Image Generation (run `moor plugins list` to see installed "
        "backends)"
    )
    return "\n".join(lines)
...
def _dispatch_to_plugin_provider(
    ...
    configured = _read_configured_image_provider()
    if not configured or configured in ("fal", MOOR_MANAGED_PROVIDER):
        # Unset/explicit FAL keeps the legacy FAL path; "moor" (managed
        # Moor Subscription selection) also runs the legacy pipeline, which
    ...
    if provider is None:
        return json.dumps({
            ...
            "error": (
                ...
                f"registered that name. Run `moor plugins list` to see "
        ...
        if "image_url" in kwargs or "reference_image_urls" in kwargs:
            ...
            return json.dumps({
                ...
                "error": (
                    ...
                    f"supports editing via `moor tools` → Image Generation."
                ),
...
def _maybe_route_managed_krea(
    ...
    # (other than the managed "moor" selection, which IS a managed-mode
...
# model/provider via `moor tools` or `/skills`.
