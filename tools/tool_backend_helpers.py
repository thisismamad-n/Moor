...
def managed_moor_tools_enabled(*, force_fresh: bool = False) -> bool:
    """Coarse gate: entitled to the Moor Tool Gateway (paid Portal access OR a live free
    ...
    try:
        ...
        account_info = (get_moor_portal_account_info(force_fresh=True) if force_fresh
                        else get_moor_portal_account_info())
        return bool(account_info.logged_in) and account_info.tool_gateway_entitled
    except Exception:
        return False


def moor_tool_gateway_unavailable_message(capability: str = "the Moor Tool Gateway", *,
    ...
    try:
        ...
        pass
    return (f"{capability} is unavailable. Run `moor model` to refresh your "
...
def _dotenv_value(env_var: str) -> str:
    """``.env`` value via ``moor_cli.config.get_env_value`` (``""`` when unavailable)."""
    try:
...
def resolve_provider_secret(env_var: str, provider_id: str, config_value: str = "",
    ...
    ``moor_cli.config.get_env_value``) -> credential pool for ``provider_id``. Under an
    ...
    Resolution order (fixes #68003 — keys added via ``moor auth add <provider>`` were invisible to the
...
def resolve_openai_audio_api_key() -> str:
    ...
    Outside a multiplexed turn, ``OPENAI_API_KEY`` additionally falls back to the credential pool (``moor
...
# Provider value the managed "Moor Subscription" picker rows write for every category;
# any other name = that vendor direct; no key = legacy autodetect.
MOOR_MANAGED_PROVIDER = "moor"
...
def read_selection(section: str) -> str | None:
    """THE single runtime read of the persisted `moor tools` selection: ``"moor"`` (managed
    ...
    to ``"moor"`` regardless of the name key. Never raises."""
    raw = _raw_section(section)
    if raw is None:
        return None
    if is_truthy_value(raw.get("use_gateway")):
        return MOOR_MANAGED_PROVIDER
...
# (moor_cli.config.validate_config_structure) and selection_error() consult this map so the user learns
...
def selection_error(section: str, selection_name: str, failure: str) -> str:
    ...
    return (f"{section} is configured to use {selection_name} (set via moor "
            f"tools), but {failure}. Run 'moor tools' to change it.")
