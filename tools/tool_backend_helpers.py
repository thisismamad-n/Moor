...
# The provider value the managed "Moor Subscription" picker rows write for
# every category (image_gen.provider: moor, web.backend: moor,
# browser.cloud_provider: moor, ...). Runtime dispatch is a plain switch on
# the stored string: "moor" → managed gateway client; any vendor name → that
...
def read_selection(section: str) -> str | None:
    """Return the stored `moor tools` provider string for a config section.

    THE single runtime read of the persisted selection. Returns:
    - ``"moor"`` — the managed Moor Tool Gateway row was selected,
    ...
    key. ``use_gateway: true`` was only ever written by the managed "Moor
    Subscription" row, so it maps to ``"moor"`` regardless of the name key;
...
def selection_error(section: str, selection_name: str, failure: str) -> str:
    ...
    return (
        f"{section} is configured to use {selection_name} (set via moor "
