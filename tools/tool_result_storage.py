...
2. **Per-result persistence** (maybe_persist_tool_result): After a tool
   ...
   ``$MOOR_HOME/cache/spillover/{tool_use_id}.txt`` — alongside the other
   moor-owned caches (images, audio, documents, ...) instead of littering
...
HEREDOC_MARKER = "MOOR_PERSIST_EOF"
...
def get_spillover_dir():
    """Return $MOOR_HOME/cache/spillover as a Path (not created)."""
    from moor_constants import get_moor_home

    return get_moor_home() / SPILLOVER_SUBDIR
...
def _write_to_spillover(content: str, filename: str):
    """Write content host-side to $MOOR_HOME/cache/spillover.
        ...
        if callable(get_temp_dir):
            try:
                ...
                if temp_dir:
                    ...
                    return f"{temp_dir}/moor-results"
    return STORAGE_DIR
...
def _heredoc_marker(content: str) -> str:
    ...
    return f"MOOR_PERSIST_{uuid.uuid4().hex[:8]}"
...
def maybe_persist_tool_result(
    ...
    # Always persist host-side first: $MOOR_HOME/cache/spillover is the
    # single canonical home for spilled results (with the other moor-owned
