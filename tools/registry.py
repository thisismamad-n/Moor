"""Central registry for all moor-agent tools.
...
def _discovery_cache_path() -> Optional[Path]:
    ...
    try:
        ...
        # import time (moor_constants itself is stdlib-only, so no cycle).
...
# timescales. Cache results for ~30 s so env-var flips via ``moor tools``
...
def check_fn_cache_scope() -> Optional[str]:
    ...
    try:
        ...
        browser_identity = (
            get_session_env("MOOR_SESSION_ID", ""),
            get_session_env("MOOR_BROWSER_CONTROL_PRINCIPAL", ""),
            get_session_env("MOOR_BROWSER_CONTROL_TRANSPORT_FAMILY", ""),
        )
...
def invalidate_check_fn_cache() -> None:
    ...
    affect tool availability (e.g. ``moor tools enable``)."""
    with _check_fn_cache_lock:
...
class ToolRegistry:
    ...
    def __init__(self):
        ...
        # Plugin registrations are overlays keyed by resolved MOOR_HOME. A
    ...
    def deregister(self, name: str) -> None:
        ...
        with self._lock:
            ...
            if not entry.toolset.startswith("mcp-"):
                ...
                # (``moor_plugins.{name}``), not the exact module string.
                # A handler defined in ``moor_plugins.pkg.handlers`` is
                # still owned by the ``moor_plugins.pkg`` package — exact
    ...
    def get_definitions(self, tool_names: Set[str], quiet: bool = False) -> List[dict]:
        ...
        etc.); TTL chosen so env-var changes (``moor tools enable foo``)
