...
def _deepinfra_model(section: Dict[str, Any], kind: str) -> Optional[str]:
    ...
    from moor_cli.models import deepinfra_model_ids
...
def _resolve_stt_client_config() -> Dict[str, Any]:
    ...
    if provider == "openai":
        # Covers the Moor-managed selection too: the resolver returns the user's
...
def _resolve_tts_client_config() -> Dict[str, Any]:
    ...
    if provider == "openai":
        # Covers the direct-key, custom-base_url, and Moor-managed selections.
        try:
...
def resolve_client_voice_config() -> Dict[str, Any]:
    ...
    Callers scope the profile via ``moor_constants.set_moor_home_override``
