...
def _moor_tags(fm: dict) -> Any:
    """``metadata.moor.tags`` from parsed frontmatter, or ``[]`` (unvalidated type)."""
    metadata = fm.get("metadata", {})
    moor_meta = metadata.get("moor", {}) if isinstance(metadata, dict) else None
    return moor_meta.get("tags", []) if isinstance(moor_meta, dict) else []
