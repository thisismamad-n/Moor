...
Moor Cloud, docker) is a persistent line. This module is the gateway-side
...
file plumbing on the gateway's own MOOR root — no network. The public
...
# ``bot_mode.envelope_ttl_seconds`` default in moor_cli/config_defaults.py.
...
def _normalize_roster_row(row: Any) -> Optional[dict]:
    ...
    if not profile or not connection_id:
        return None
    if not handle:
        handle = "moor" if profile == "default" else profile
    if (
...
def _moor_cli() -> str:
    """Resolve the moor CLI beside this gateway's own interpreter.

    The deliver RPC runs on the target gateway, whose process is the venv
    python — its bin/Scripts directory holds the matching ``moor``
    entrypoint. A bare ``"moor"`` relies on PATH, which is exactly what
    ...
    sibling = exe.parent / ("moor.exe" if sys.platform == "win32" else "moor")
    if sibling.is_file():
        return str(sibling)
    found = shutil.which("moor")
    if found:
        return found
    return "moor"
...
# turns concurrently: deliveries spawn separate ``moor`` subprocesses, so
