...
# One turn in a profile's canonical Bot Chat: ``moor -p <profile> *BOT_CHAT_TURN_ARGS``.
...
def _normalize_roster_row(row: Any) -> Optional[dict]:
    ...
    handle = str(row.get("handle") or "").strip().lstrip("@") or ("moor" if profile == "default" else profile)
...
def cleanup_bot_relay_artifacts(max_age_hours: float | None = None) -> int:
    ...
    del max_age_hours
    try:
        base = relay_root(_moor_root(Path(_default_home())))
...
def _moor_cli() -> str:
    """moor CLI beside this interpreter, then ``shutil.which``, then the bare name
    (service contexts lack PATH, so a bare "moor" died with ENOENT).

    The deliver RPC runs on the target gateway, whose process is the venv python — its bin/Scripts directory
    holds the matching ``moor`` entrypoint. A bare ``"moor"`` relies on PATH, which is exactly what
    ...
    """
    sibling = Path(sys.executable or "").parent / ("moor.exe" if sys.platform == "win32" else "moor")
    return str(sibling) if sibling.is_file() else shutil.which("moor") or "moor"
...
def local_delivery_command(profile: str, query_file: str) -> list[str]:
    ...
    return [_moor_cli(), "-p", profile, *BOT_CHAT_TURN_ARGS, "--query-file", query_file]


# Two deliveries into the SAME profile must never run Bot Chat turns concurrently.
# Deliveries are separate ``moor`` subprocesses, so the lock is a per-profile
...
# target profile must never run their Bot Chat turns concurrently: deliveries spawn separate ``moor``
