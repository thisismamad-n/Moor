...
``~/.moor/.env`` (Docker: see ``CAMOFOX_REWRITE_LOOPBACK_URLS`` below).
"""
...
def _config_cdp_url() -> str:
    ...
    try:
        from moor_cli.config import read_raw_config  # late-bound: tests patch the source module
...
def _camofox_identity_override(task_id: Optional[str], camofox_cfg: Dict[str, Any]) -> Optional[Dict[str, str]]:
    ...
    share a user ID so Moor uses the same profile), or None."""
...
def _get_session(task_id: Optional[str]) -> Dict[str, Any]:
    ...
    with _sessions_lock:
        ...
        if identity is None:
            identity = {"user_id": f"moor_{uuid.uuid4().hex[:10]}", "session_key": f"task_{task_id[:16]}"}
...
def _save_screenshot(content: bytes) -> str:
    """Write PNG bytes under ``$MOOR_HOME/browser_screenshots`` and return the path."""
