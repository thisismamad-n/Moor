"""
Lazy dependency installer for opt-in Moor Agent backends.
...
* **Durable-target mode (immutable images).** When the deployment seals the
  ...
  installed package cannot brick Moor, which is what made it safe to seal
...
def _unsupported_feature_reason(feature: str) -> Optional[str]:
    ...
    if sys.platform == "win32" and feature == "platform.matrix":
        return (
            ...
            "from sdist. Run Moor under WSL to use Matrix on Windows."
        )
    return None
