"""Lazy dependency installer for opt-in Moor backends.
...
(never system Python); durable-target mode (``MOOR_LAZY_INSTALL_TARGET``, sealed images)
...
LAZY_DEPS: dict[str, tuple[str, ...]] = {
    ...
    # ─── Wake word ("Hey Moor") engines (sync with the `wake` extra) ──────
    ...
    # on mere presence, so `moor update` re-asserts this pin everywhere hub exists. MUST stay
    # inside transformers' window and match uv.lock (tests/test_project_metadata.py enforces).
    # HF Agent Trace Viewer upload (moor trace upload / /upload-trace). huggingface-hub is a SHARED
    ...
    # feature active from mere package presence, the `moor update` lazy-refresh pass re-asserts THIS pin
...
def _allow_lazy_installs() -> bool:
    ...
    modes; (2) the sealed venv (``MOOR_DISABLE_LAZY_INSTALLS=1``) blocks only without a durable
...
def _unsupported_feature_reason(feature: str) -> Optional[str]:
    ...
    if sys.platform == "win32" and feature == "platform.matrix":
        return ("unsupported on Windows: Matrix E2EE depends on python-olm, which has no Windows wheel and "
                "requires make + libolm to build from sdist. Run Moor under WSL to use Matrix on Windows.")
    return None
...
def _is_satisfied(spec: str) -> bool:
    """Present AND inside the spec's version range, so ``moor update`` propagates pin bumps to
...
def _uv_binary() -> Optional[str]:
    """Managed uv first ($MOOR_HOME/bin is never on PATH), then PATH. A lookup, not ensure_uv():
...
def _venv_pip_install(specs: tuple[str, ...], *, timeout: int = 300) -> _InstallResult:
    ...
    ``moor_cli.tools_config._pip_install`` (no CLI dependency)."""
    if not specs:
    ...
    if _lazy_install_target() is None:
        ...
        if managed_by:
            raise FeatureUnavailable(
                ...
                f"so Moor cannot install them at runtime. Add the dependencies for {feature!r} via "
...
def install_specs(specs: list[str] | tuple[str, ...], *, timeout: int = 300) -> InstallSpecsResult:
    ...
    target = _lazy_install_target()
    if not _allow_lazy_installs():
        sealed = os.environ.get("MOOR_DISABLE_LAZY_INSTALLS") == "1" and target is None
        reason = ("runtime installs are disabled on this deployment: the agent environment is immutable "
                  "and no writable install target is configured (MOOR_LAZY_INSTALL_TARGET)"
...
def active_features() -> list[str]:
    ...
    asyncpg are deliberately not proof a backend was enabled. Drives ``moor update``."""
...
def refresh_active_features(*, prompt: bool = False) -> dict[str, str]:
    """Re-run ``ensure`` for every active feature (``moor update``); returns
