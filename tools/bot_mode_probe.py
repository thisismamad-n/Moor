...
def is_bot_mode_managed(home: str | os.PathLike | None = None) -> bool:
    ...
    try:
        resolved = Path(
            str(home) if home else (os.getenv("MOOR_HOME") or os.path.expanduser("~/.moor"))
        )
...
def _profile_role(profile_dir: Path) -> str:
    ...
    The ui_meta['moor-bots'].title is the name the user gave the bot in
    ...
    try:
        ...
        if isinstance(ui_meta, dict) and isinstance(ui_meta.get("moor-bots"), dict):
            title = str(ui_meta["moor-bots"].get("title") or "").strip()
            if title:
                parts.append(title)
...
def _peers(root: Path) -> list[str]:
    """Registered peer gateway names (``moor peer``), for the protocol text.
...
def _remote_paragraph(root: Path) -> str:
    ...
    connected to the user's Desktop (local, remote URL, SSH, Moor Cloud,
...
def capability_fingerprint(home: str | os.PathLike | None = None) -> str:
    ...
    try:
        ...
        surface["peers"] = _peers(_moor_root(resolved))
    except Exception:
        surface["peers"] = []
    try:
        ...
        surface["remote_roster"] = sorted(
            ...
            for r in read_remote_roster(_moor_root(resolved))
        )
    except Exception:
