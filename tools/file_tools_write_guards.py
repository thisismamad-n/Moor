...
def _get_moor_config_resolved() -> str | None:
    ...
    return _cached_lookup("_moor_config_resolved", "_moor_config_resolved_loaded", _config_path_resolved,
                          lambda: str(Path(_expand_tilde("~/.moor/config.yaml")).resolve()))


def _get_real_moor_home() -> str | None:
    """Return the realpath of the authoritative Moor home (cached)."""
    return _cached_lookup("_real_moor_home_cached", "_real_moor_home_loaded", _moor_home_real,
                          lambda: os.path.realpath(_expand_tilde("~/.moor")))
...
def _check_sensitive_path(filepath: str, task_id: str = "default") -> str | None:
    ...
    if moor_config and moor_config in candidates:
        return (
...
# vector (AGENTS.md / CLAUDE.md / SOUL.md / .cursorrules / project .moor tree).
...
def _protected_instruction_reason(filepath: str, task_id: str = "default",
    ...
    for candidate in (normalized, resolved):
        ...
        if base_lower in _PROTECTED_INSTRUCTION_BASENAMES or any(
            ...
            return base
        # Project-local .moor config dirs (<repo>/.moor/config.yaml) steer
        # behavior too. Only the IMMEDIATE parent counts — matching any ancestor
        # would gate every write inside a checkout living under ~/.moor.
...
def _get_container_mirror_prefix_for_task(task_id: str = "default") -> str | None:
    """Return the container-side Moor mirror prefix for persistent Docker file tools."""
    try:
        ...
        if env is not None:
            ...
            return "/root/.moor" if persistent_docker else None
...
def _check_cross_profile_path(filepath: str, task_id: str = "default") -> str | None:
    ...
    Moor state (a write the host never reads). Not profile isolation — that guard was
