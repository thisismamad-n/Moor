...
teammate agent (another Moor profile on this install, or an agent on a
...
DM each other via a prompt-injected ``moor -p <bot> chat ...`` shellout.
...
- Everything here is additive. The legacy protocol transports
  (``moor -p`` / ``moor peer dm``) keep working for older prompts.

The transports themselves are unchanged and proven:
- local teammate  → ``moor -p <name> chat --in ~ -c "Bot Chat"
  --create-if-missing -Q --query-file <tmp>`` (one turn, reply on stdout)
- peer teammate   → ``moor peer dm <peer>[/<name>] < <tmp>``
...
def cleanup_bot_dm_cache(
    ...
    locations: list[tuple[Path, str]] = [
        (temp_root, "moor-dm-*.txt"),
        (temp_root, "moor-relay-dm-*.txt"),
    ]
    try:
...
def _delivery_lock(argv: list[str], *, stdin_file: bool):
    ...
    Local deliveries (``moor -p <profile> chat …``) collide with relay
    ...
    # resolves the venv-relative moor next to this gateway's interpreter
    ...
    if (
        stdin_file
        or len(argv) < 3
        or cli not in ("moor", "moor.exe")
        or argv[1] != "-p"
    ):
        return contextlib.nullcontext()
    from tools.bot_relay import acquire_turn_lock

    home = Path(os.getenv("MOOR_HOME") or os.path.expanduser("~/.moor"))
    return acquire_turn_lock(_moor_root(home), argv[2])
