...
processes survive cleanup, idle reaping, or Moor exit.
...
from tools.tool_backend_helpers import coerce_modal_mode, managed_moor_tools_enabled
...
def _current_session_profile() -> str:
    """Active session's Moor profile name, or "" (same lookup discipline as
...
def _maybe_reap_docker_orphans(container_config: Dict[str, Any]) -> None:
    ...
    Sweeps Exited containers labeled ``moor-agent=1`` for the current
    profile — leftovers of Moor processes that died without firing
    ...
    Moor processes sharing a profile) and the once-per-interpreter flag so
...
def _ensure_terminal_env_bridged() -> None:
    ...
    at startup; processes that skip those paths (``moor serve``, Desktop
    ...
    env values (possibly stale from ``moor setup``); env values for omitted keys
