...
def _current_session_key() -> str:
    ...
    Single lookup point for the ``MOOR_SESSION_KEY`` ContextVar with the
    ...
    return get_session_env("MOOR_SESSION_KEY", "")
...
def _in_delegated_child_context() -> bool:
    ...
    inherit process-wide interactivity signals (``MOOR_INTERACTIVE=1`` set
...
def _transform_sudo_command(command: str | None) -> tuple[str | None, str | None]:
    ...
    # MOOR_INTERACTIVE=1 (and, on a recycled worker thread, potentially a
...
def _docker_persistent_profile_scoped() -> bool:
    ...
    ``container_persistent: true`` is ONE long-lived container per Moor
...
def _current_session_profile() -> str:
    """Return the active session's Moor profile name, or "" when unset.
    ...
    return get_session_env("MOOR_SESSION_PROFILE", "")
...
def _create_environment(env_type: str, image: str, cwd: str, timeout: int,
    ...
    if env_type == "local":
        ...
        if provider is not None:
            ...
            try:
                env_obj._moor_backend_name = provider.name.strip().lower()
            except AttributeError:
...
def terminal_tool(
    ...
    try:
        ...
        # restart|stop|uninstall targeting moor-gateway) must never run inside the
        ...
        # Gate on the SUPERVISED-gateway probe, not the raw _MOOR_GATEWAY
        # marker: gateway.run sets it at import time, so it leaks into every
        # process that merely imports gateway.run (moor serve --isolated,
        # CLI, web server) which are NOT the gateway and must be able to
        # restart it. A plain foreground `moor gateway run` (env set, PID
        ...
        if background:
            ...
            if sudo_cache_cleared:
                has_sudo_prompt_callback = _get_sudo_password_callback() is not None
                can_reprompt = (
                    has_sudo_prompt_callback or env_var_enabled("MOOR_INTERACTIVE")
                ) and not _in_delegated_child_context()
                if can_reprompt:
                    output += (
