"""Shared ``OAuthClientProvider`` customizations for Moor MCP OAuth.
...
class MoorProviderMixin:
    """Token-endpoint fixes layered over the SDK's ``OAuthClientProvider`` (must precede it in
    the MRO; subclasses set ``_moor_logger`` to keep their own logger name).
    ...
    _moor_logger: logging.Logger = logger
    ...
    def _coerce_client_secret_post(self) -> None:
        """Same rule as ``MoorTokenStorage._coerce_secret_auth_method``, applied to the
        ...
        if MoorTokenStorage._coerce_secret_auth_method(data):
    ...
    async def _handle_refresh_response(self, response) -> bool:
        ...
        try:
            ...
            self._moor_logger.warning("Invalid refresh response: %s", response.status_code)
            self.context.clear_tokens()
            return False
        await self._store_tokens(token_response)
        return True
...
def prepare_oauth_config(server_name: str, server_url: str, oauth_config: dict | None) -> tuple[dict, "MoorTokenStorage"]:
    ...
    return cfg, mo.MoorTokenStorage(server_name)


def build_provider_kwargs(cfg: dict, storage: "MoorTokenStorage", *, ssh_proxy_hint: bool) -> dict[str, Any]:
