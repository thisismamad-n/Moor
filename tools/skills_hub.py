#!/usr/bin/env python3
"""
Skills Hub — Source adapters and hub state management for the Moor Skills Hub.
...
Used by moor_cli/skills_hub.py for CLI commands and the /skills slash command.
"""
...
def _skills_dir() -> Path:
    ...
    return Path(forced) if forced is not None else _moor_home() / "skills"
...
class GitHubSource(SkillSource):
    ...
    def inspect(self, identifier: str) -> Optional[SkillMeta]:
        ...
        if isinstance(metadata, dict):
            moor_meta = metadata.get("moor", {})
            if isinstance(moor_meta, dict):
                tags = moor_meta.get("tags", [])
        if not tags:
...
class OptionalSkillSource(SkillSource):
    ...
    These skills are official (maintained by Moor inc.) but not activated
    by default — they don't appear in the system prompt and aren't copied to
    ~/.moor/skills/ during setup.  They are discoverable via the Skills Hub
    ...
    def _fetch_from_live_repo(self, rel: str) -> Optional[SkillBundle]:
        ...
        ``moor update``. Rather than telling them to update first, resolve
...
def _category_skill_dirs(directory: Path) -> List[str]:
    ...
    ``moor_cli.skills_hub._existing_categories``.
    """
    skill_dirs: List[str] = []
...
def _moor_index_cache_file() -> Path:
    return _index_cache_dir() / "moor-index.json"
...
def _load_moor_index() -> Optional[dict]:
    ...
    We cache it locally for MOOR_INDEX_TTL seconds to avoid repeated
    downloads within a session.
    """
    # Check local cache
    moor_index_cache_file = _moor_index_cache_file()
    if moor_index_cache_file.exists():
        try:
            age = time.time() - moor_index_cache_file.stat().st_mtime
            if age < MOOR_INDEX_TTL:
                return json.loads(moor_index_cache_file.read_text(encoding="utf-8"))
        except (OSError, json.JSONDecodeError):
            pass
    ...
    for accept_encoding in ("gzip, deflate", "identity"):
        try:
            ...
            logger.debug(
                "Moor index decode failed (Accept-Encoding=%s): %s",
                accept_encoding,
                e,
            )
            continue
    ...
    # Cache locally
    try:
        moor_index_cache_file.parent.mkdir(parents=True, exist_ok=True)
        moor_index_cache_file.write_text(json.dumps(data), encoding="utf-8")
    except OSError:
        pass
...
class MoorIndexSource(SkillSource):
    """Skill source backed by the centralized Moor Skills Index.
    ...
    def fetch(self, identifier: str) -> Optional[SkillBundle]:
        ...
        if resolved:
            ...
            if bundle:
                bundle.source = entry.get("source", "moor-index")
    ...
    def _to_meta(entry: dict) -> SkillMeta:
        return SkillMeta(
            name=entry.get("name", ""),
            description=entry.get("description", ""),
            source=entry.get("source", "moor-index"),
            identifier=entry.get("identifier", ""),
...
def create_source_router(auth: Optional[GitHubAuth] = None) -> List[SkillSource]:
    ...
    sources: List[SkillSource] = [
        ...
        MoorIndexSource(auth=auth), # Centralized index (search + resolved install paths)
