...
from moor_cli.fallback_config import scoped_fallback_chain

logger = logging.getLogger("tools.delegate_tool")  # log-record parity with the origin module

# Runtime-provider sentinel for providers that are not natively known; must
# match moor_cli.runtime_provider.RUNTIME_PROVIDER_TYPE_CUSTOM.
...
def _load_config() -> dict:
    ...
    (follows MOOR_HOME/profile; no deepcopy, since this runs on every get_definitions() rebuild) over the legacy
    ``cli.CLI_CONFIG``, which can hide user-set keys — except that ``MOOR_IGNORE_USER_CONFIG=1`` is only honored
    by the legacy loader, so it stays authoritative when that flag is set."""
    if os.environ.get("MOOR_IGNORE_USER_CONFIG") != "1":
        try:
...
_MOOR_PROVIDERS = frozenset({"moor", "moor-portal", "nousresearch"})
...
def _resolve_child_runtime(
    ...
    or is Moor Portal (dual-wire); a pinned ``delegation.command`` must exist on PATH or the spawn fails loudly;
    ...
    # otherwise). Moor Portal is dual-wire within one provider (anthropic/* → Messages, else chat_completions), so
    ...
    # would pin a child Moor/Qwen subagent onto the parent's Claude Messages wire (or the reverse).
    # agent_init honors an explicit api_mode above its moor branch, so re-derive here before construction.
    _parent_provider = getattr(parent_agent, "provider", None) or ""
    if override_api_mode is not None:
        effective_api_mode = override_api_mode
    elif (effective_provider or "").strip().lower() in _MOOR_PROVIDERS:
