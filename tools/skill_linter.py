...
def _check_frontmatter(frontmatter: Dict[str, Any], skill_dir: Optional[Path]) -> Iterator[LintFinding]:
    ...
    if not isinstance(moor_meta, dict):
        yield _warn("missing-metadata", "frontmatter is missing metadata.moor.{tags, related_skills}.")
    elif "tags" not in moor_meta:
        yield _warn("missing-metadata", "metadata.moor.tags is missing.")
    author = str(frontmatter.get("author", ""))
    if author and author.strip().lower() in ("moor", "agent", "moor agent") and (
        author != "Moor Agent"):
        yield _warn("author-caps", f"author '{author}' should be 'Moor Agent' (proper caps) "
