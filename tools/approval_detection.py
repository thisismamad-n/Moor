...
# Sensitive write targets, matched via ~ / $HOME / $MOOR_HOME spellings. The resolved absolute
# home is folded into these forms at detection time by _normalize_command_for_detection(), so no
# import-time path snapshot (stale once MOOR_HOME is set after import) lives in the patterns.
_SSH_SENSITIVE_PATH = r'(?:~|\$home|\$\{home\})/\.ssh(?:/|$)'
_MOOR_ENV_PATH = (
    r'(?:~\/\.moor/|(?:\$home|\$\{home\})/\.moor/|(?:\$moor_home|\$\{moor_home\})/)' r'\.env\b'
)
# ~/.moor/config.yaml IS the security policy (approvals.mode, yolo, allowlist) and the config cache is mtime-keyed,
# so a write takes effect mid-session. Terminal-side coverage (sed -i, tee, >, cp) pairs the file_tools deny.
_MOOR_CONFIG_PATH = (
    r'(?:~\/\.moor/|(?:\$home|\$\{home\})/\.moor/|(?:\$moor_home|\$\{moor_home\})/)' r'config\.yaml\b'
)
...
_SENSITIVE_WRITE_TARGET = (
    rf'(?:{_SYSTEM_CONFIG_PATH}|/dev/sd|{_SSH_SENSITIVE_PATH}|{_MOOR_ENV_PATH}|{_MOOR_CONFIG_PATH}|'
...
DANGEROUS_PATTERNS = [
    ...
    # launchctl-driven gateway stop/restart on macOS (label `ai.moor.gateway`). Two independent lookaheads, NOT a
    # sequential match: a for-loop building the label from a list defined EARLIER (`for item in 'ai.moor...'; do
    # launchctl bootout "$label"`) never has "moor" after the verb, and that slipped past and restarted 4 gateways
    ...
    # ~/.netrc/.pgpass/.npmrc/.pypirc, shell rc files, ~/.moor/config.yaml/.env), but cp/mv/install was
    ...
    # sed -i on Moor config/.env bypasses the redirection/tee rules; pairs the file_tools
    # write_file/patch deny so the terminal side is not an open door.
    # In-place edit of a moor-managed security file (~/.moor/config.yaml or .env). sed -i bypasses the
...
def _normalize_command_for_detection(command: str) -> str:
    ...
    command = re.sub(r'\\\r?\n', '', command)
    # Fold absolute user/Moor home prefixes to ~/ and ~/.moor/ so the static patterns catch /home/alice/.bashrc
    # and C:\Users\alice\.bashrc. Resolved at detection time (not import time) so it tracks HOME/MOOR_HOME set
    # later. MUST run before the backslash strip (which would dissolve C:\Users\alice to C:Usersalice). Moor home
...
def _rewrite_resolved_moor_home(command: str) -> str:
    """Resolved MOOR_HOME (and its realpath) -> ``~/.moor/`` so the _MOOR_CONFIG_PATH /
    _MOOR_ENV_PATH rules match Docker/gateway deployments that spell the absolute path."""
    try:
        ...
        return command
    return _fold_home_prefixes(command, paths, "~/.moor")
...
_GATEWAY_LIFECYCLE_SPLICE_DESCRIPTION = "stop/restart moor gateway via shell-spliced verb (kills running agents)"
...
def _is_verification_artifact_cleanup(command: str) -> bool:
    ...
    return (
        ...
        and re.fullmatch(r"moor-(?:verify|ad-hoc)-[A-Za-z0-9_.-]+", basename) is not None
    )
...
def _is_shell_token_spliced_gateway_lifecycle(command: str) -> bool:
    ...
    verb is an ARGUMENT, so ``launchctl kick"start" -k gui/501/ai.moor.gateway`` auto-approved.
    Delegates to ``cron.lifecycle_guard`` (shlex-tokenized, anchored on a moor-gateway
