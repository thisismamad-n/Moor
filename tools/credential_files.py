...
def register_credential_file(
    relative_path: str,
    container_base: str = "/root/.moor",
) -> bool:
    """Register a credential file for mounting into remote sandboxes.

    *relative_path* is relative to ``MOOR_HOME`` (e.g. ``google_token.json``).
    ...
    The resolved host path must remain inside MOOR_HOME so that a malicious
    skill cannot declare ``required_credential_files: ['../../.ssh/id_rsa']``
    and exfiltrate sensitive host files into a container sandbox.

    Containment alone is not sufficient, because MOOR_HOME is exactly where
    ...
    """
    moor_home = _resolve_moor_home()

    # Reject absolute paths — they bypass the MOOR_HOME sandbox entirely.
    ...
    host_path = moor_home / relative_path

    # Resolve symlinks and normalise ``..`` before the containment check so
    # that traversal like ``../. ssh/id_rsa`` cannot escape MOOR_HOME.
    from tools.path_security import validate_within_dir

    containment_error = validate_within_dir(host_path, moor_home)
    ...
    # inside MOOR_HOME and therefore pass the containment check above.
            ...
            for item in cred_files:
                if isinstance(item, str) and item.strip():
                    ...
                    if resolved_path.is_file():
                        container_path = f"/root/.moor/{rel}"
                        result.append({
...
def _safe_skills_path(skills_dir: Path) -> str:
    ...
    safe_dir = Path(tempfile.mkdtemp(prefix="moor-skills-safe-"))
...
# Each tuple is (new_subpath, old_name) matching moor_constants.get_moor_dir().
...
def get_cache_directory_mounts(
    ...
    ``get_moor_dir()`` for backward compatibility with old directory layouts.
    """
    ...
    for new_subpath, old_name in _CACHE_DIRS:
        ...
        if not host_dir.is_dir():
            ...
            # mount costs the feature. get_moor_dir() already resolved
...
def map_cache_path_to_container(
    ...
    caller decides which ``container_base`` applies (Docker ``/root/.moor``,
    SSH ``<remote_home>/.moor``, etc.) and whether translation is wanted.
...
def to_agent_visible_cache_path(
    ...
    * docker / modal — bind-mounted (docker) or per-file-synced (modal) at
      ``/root/.moor`` (the *container_base* default).
    * ssh / daytona / vercel_sandbox — file-synced under the remote user's
      home; ``~/.moor`` is shell-expanded by the remote shell, so tool
