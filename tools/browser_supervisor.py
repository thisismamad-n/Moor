...
One ``CDPSupervisor`` runs per Moor ``task_id`` that has a reachable CDP
...
def _redact_cdp_error_text(exc: object) -> str:
    ...
    those credentials never reach Moor logs or tracebacks. Falls back to a
...
DIALOG_BRIDGE_HOST = "moor-dialog-bridge.invalid"
...
(() => {
  if (window.__moorDialogBridgeInstalled) return;
  window.__moorDialogBridgeInstalled = true;
  const ENDPOINT = "http://moor-dialog-bridge.invalid/";
