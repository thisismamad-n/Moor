...
# Must match moor_cli.runtime_provider.RUNTIME_PROVIDER_TYPE_CUSTOM.
...
def _expand_parent_toolsets(parent_toolsets: set) -> set:
    ...
    When a parent uses a composite toolset like ``moor-cli`` (which bundles
...
def _blocked_toolsets_for_role(role: str) -> List[str]:
    ...
    mixed platform bundles such as ``moor-cli`` because those also contain
...
def _build_child_agent(
    ...
    model on OpenRouter while the parent runs on Moor Portal).
    """
    ...
    if toolsets:
        ...
        # Expand composite toolsets (e.g. moor-cli) so that individual
    ...
    # Blocked tools also live inside mixed platform bundles (moor-cli,
    # moor-telegram, etc.) that _strip_blocked_tools must keep because they
    ...
    # Extract parent's API key so subagents inherit auth (e.g. Moor Portal).
    ...
    #
    # Moor Portal is dual-wire within a single provider: anthropic/* → Messages,
    ...
    # reverse). agent_init honors an explicit api_mode above its moor branch, so
    # re-derive here before construction.
    _parent_provider = getattr(parent_agent, "provider", None) or ""
    _effective_provider_norm = (effective_provider or "").strip().lower()
    if override_api_mode is not None:
        effective_api_mode = override_api_mode
    elif _effective_provider_norm in {"moor", "moor-portal", "Moor inc."}:
        from moor_cli.providers import moor_api_mode

        effective_api_mode = moor_api_mode(effective_model)
...
def _dump_subagent_timeout_diagnostic(
    *,
    ...
    writes a dedicated log under ``~/.moor/logs/subagent-<sid>-<ts>.log``
...
def _spill_summary_to_file(task_index: int, summary: str) -> Optional[str]:
    ...
    try:
        ...
        cache_dir = get_moor_dir("cache/delegation", "delegation_cache")
    ...
    if _subagent_id:
        if owner_session_id is None:
            try:
                ...
                owner_session_id = get_session_env("MOOR_UI_SESSION_ID", "") or None
            except Exception:
...
def _finalize_child_results(
    ...
    with _parent_finalization_lock(parent_agent):
        ...
        parent_session_id = getattr(parent_agent, "session_id", None)
        try:
            from moor_cli.plugins import invoke_hook as invoke_hook
        except Exception:
            invoke_hook = None
...
def delegate_task(
    ...
    # MOOR_SESSION_ID ContextVar and os.environ with the subagent's internal
    ...
    try:
        ...
        _origin_ui_session_id = get_session_env("MOOR_UI_SESSION_ID", "")
    except Exception:
    ...
    if background:
        ...
        if not _async_ok:
            ...
            # MOOR_SESSION_ID here would return the subagent's internal id.
        ...
        if not _async_ok:
            ...
            if isinstance(_sync_result, dict):
                _sync_result["note"] = (
                    ...
                    "one-shot runner such as `moor -z`, a cron job, a Kanban "
        ...
        try:
            ...
            _source = get_session_env("MOOR_SESSION_SOURCE", "")
            ...
            _origin_ui_session_id = (
                get_session_env("MOOR_UI_SESSION_ID", "") or _origin_ui_session_id
            )
        ...
        if not _session_key:
            ...
            # during gateway/TUI turns and MOOR_SESSION_KEY is not in the CLI
...
def _resolve_delegation_credentials(cfg: dict, parent_agent) -> dict:
    ...
    try:
        ...
        raise ValueError(
            ...
            f"Available providers: openrouter, moor, zai, kimi-coding, minimax."
        ) from exc
    ...
    if not api_key:
        raise ValueError(
            ...
            f"Set the appropriate environment variable or run 'moor auth'."
        )
...
def _load_config() -> dict:
    ...
    MOOR_HOME/profile. ``cli.CLI_CONFIG`` is a legacy fallback for entry
    ...
    ``MOOR_IGNORE_USER_CONFIG=1`` (``moor chat --ignore-user-config``) is
    ...
    """
    prefer_legacy = os.environ.get("MOOR_IGNORE_USER_CONFIG") == "1"
