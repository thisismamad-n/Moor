...
def _check_logging_callback_support() -> bool:
    ...
    server emits, so server-side diagnostics never reach Moor' logs.
    """
    if not _MCP_AVAILABLE:
        return False
    try:
    ...
    if os.sep not in resolved_command:
        ...
        if which_hit:
            ...
            candidates = [
                ...
                # image (which the Moor Docker image copies node + npm +
...
# ---------------------------------------------------------------------------
# MCP ImageContent block → Moor MEDIA tag
# ---------------------------------------------------------------------------
...
def _cache_mcp_image_block(block) -> str:
    ...
    return a ``MEDIA:<path>`` tag that Moor gateways know how to render.
...
def _render_mcp_resource_block(block, server_name: str = "") -> str:
    ...
    - ``EmbeddedResource`` with blob contents → bytes are decoded (size-capped)
      and materialized into the Moor document cache; returns a marker with
...
class ElicitationHandler:
    ...
    Form-mode elicitations are routed through Moor' existing approval
...
class MCPServerTask:
    ...
    async def _run_stdio(self, config: dict):
        ...
        # ungraceful exit of this Moor process (kill -9, crash, force-quit)
    ...
    async def run(self, config: dict):
        ...
        # routes those requests through Moor' approval system.
        ...
        while True:
            try:
                ...
                # and the MCP server would stay dead until Moor is fully
...
def _load_mcp_config() -> Dict[str, dict]:
    """Read ``mcp_servers`` from the Moor config file.
...
def _make_tool_handler(server_name: str, tool_name: str, tool_timeout: float):
    ...
    def _handler(args: dict, **kwargs) -> str:
        ...
        async def _call():
            ...
            # flow through Moor' MEDIA: tag convention and out to messaging
            ...
            # Moor' MEDIA tag + cache_image_from_bytes) was the cleaner of
...
def sanitize_mcp_name_component(value: str) -> str:
    ...
    Preserves Moor's historical behavior of converting hyphens to
...
# Native MCP tool-name prefix. Moor uses the ``mcp__<server>__<tool>``
...
def _convert_mcp_schema(server_name: str, mcp_tool) -> dict:
    """Convert an MCP tool listing to the Moor registry schema format.
