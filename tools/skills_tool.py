...
MOOR_HOME = get_moor_home()  # all skills live in ~/.moor/skills/ (seeded from bundled)
...
def _skills_dir() -> Path:
    ...
    it, else live profile-scoped MOOR_HOME (long-lived runtimes may import before profile set)."""
    configured = Path(SKILLS_DIR)
    return configured if configured != _SKILLS_DIR_AT_IMPORT else get_moor_home() / "skills"
...
def _get_category_from_path(skill_path: Path) -> Optional[str]:
    """``~/.moor/skills/mlops/axolotl/SKILL.md`` -> ``"mlops"``; active profile dir first
...
def _is_skill_disabled(name: str, platform: str = None) -> bool:
    """Disabled in config? Platform precedence: explicit arg, ``MOOR_PLATFORM``, session
    ``MOOR_SESSION_PLATFORM``. A globally-disabled skill stays disabled on every platform
    (keep in sync with agent.skill_utils.get_disabled_skill_names)."""
    try:
        from moor_cli.config import load_config
        skills_cfg = load_config().get("skills", {})
        resolved_platform = platform or os.getenv("MOOR_PLATFORM")
        if not resolved_platform:
            with suppress(Exception):
                from gateway.session_context import get_session_env
                resolved_platform = get_session_env("MOOR_SESSION_PLATFORM") or ""
...
def _org_provenance_header(skill_dir: Path, active_skills_dir: Path):
    ...
    header = (
        ...
        "> `moor sync propose` (or automatically, if your org enables it).\n\n")
...
def _log_security_warnings(name: str, skill_md: Path, content: str, all_dirs, active_skills_dir):
    ...
    if not _under_any(skill_md, trusted_dirs):
        warnings.append(f"skill file is outside the trusted skills directory (~/.moor/skills/): {skill_md}")
...
def skill_view(
    ...
    try:
        ...
        if _is_skill_disabled(resolved_name):
            return _fail(f"Skill '{resolved_name}' is disabled. Enable it with `moor skills` or inspect the files directly on disk.")
        if file_path and skill_dir:
            return _serve_skill_file(
                skill_dir, file_path, name, list_available=True, mark_read=True,
                hint="Use a relative path within the skill directory")
        # tags/related_skills: metadata.moor.* (agentskills.io) first, then top-level.
        metadata = frontmatter.get("metadata")
        moor_meta = (metadata.get("moor", {}) or {}) if isinstance(metadata, dict) else {}
        tags, related_skills = (
            _parse_tags(moor_meta.get(k) or frontmatter.get(k, "")) for k in ("tags", "related_skills"))
