...
_MOOR_CONFIG_FILES = r'\.moor/(?:config\.yaml|SOUL\.md)'
...
THREAT_PATTERNS = [
    ...
    (r'\$HOME/\.moor/\.env|\~/\.moor/\.env',
     "moor_env_access", "critical", "exfiltration", "directly references Moor secrets file"),
    ...
    # quarantine only acts on "dangerous") but high for Moor/other config (setup docs routinely say
    ...
    (_prose_modify_re(_MOOR_CONFIG_FILES),
     "moor_config_mod", "high", "persistence", "modification language aimed at Moor configuration files (verify intent)"),
    (_shell_write_re(_MOOR_CONFIG_FILES),
     "moor_config_mod_shell", "critical", "persistence", "shell write (redirect/sed -i/tee/cp/mv) targeting Moor configuration files"),
    (r'\.moor/config\.yaml|\.moor/SOUL\.md',
     "moor_config_ref", "low", "persistence", "references Moor configuration files (informational; only modification intent is scored)"),
...
# `.skillignore` is moor-native; `.clawhubignore` is honored for skills published through ClawHub.
