...
# prefix, the exact-name MOOR_ allowlist, or (Windows) an OS-essential name.
# The broad "MOOR_" prefix is deliberately NOT safe — it leaked config vars
# without a secret substring (MOOR_BASE_URL, MOOR_KANBAN_DB, *_WEBHOOK).
# MOOR_RPC_SOCKET / MOOR_RPC_DIR / TZ / HOME are injected after scrubbing.
...
# imports may read at import time. MOOR_DELEGATED_CHILD_CONTEXT must ride
# along or a child that imports Moor code loses the Kanban mutation guard
# while still inheriting MOOR_HOME.
_MOOR_CHILD_ALLOWED = frozenset({
    "MOOR_HOME", "MOOR_PROFILE", "MOOR_CONFIG", "MOOR_ENV", "MOOR_DELEGATED_CHILD_CONTEXT",
})
...
def _scrub_child_env(source_env, is_passthrough=None, is_windows=None):
    ...
    (4) operational MOOR_* pass by exact name; (5) on Windows the
    ...
    scrubbed = {}
    # Non-secret MOOR_* vars no allowlist admits are dropped on purpose; a script importing a
    ...
    if _dropped_moor:
        logger.debug(
            ...
            len(_dropped_moor), ", ".join(sorted(_dropped_moor)),
        )
...
def _build_child_env(*, rpc_endpoint: str, rpc_token: str, tmpdir: str,
    ...
    # Only TZ reaches the child; MOOR_TIMEZONE is an internal setting.
    ...
    _pp_parts = [tmpdir]
    if _uses_moor_python_environment(child_python):
        _pp_parts.append(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
    elif child_python not in _external_env_logged:
        # Surface once per interpreter so "import moor_constants fails" is diagnosable.
        _external_env_logged.add(child_python)
        logger.info("execute_code: child interpreter %s is outside the Moor "
                    "environment; moor root omitted from PYTHONPATH", child_python)
    if _existing_pp:
...
def _uses_moor_python_environment(python_path: str) -> bool:
    """Whether *python_path* belongs to Moor's active Python environment. Short-circuits when
    it IS the running interpreter (by path or realpath — covers ``uv run`` venvs) so no probe
    runs on the default strict path and a flaky probe can never drop the moor root."""
