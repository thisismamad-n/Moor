...
it is automatically downloaded from GitHub releases to $MOOR_HOME/bin/tirith.
...
def _get_moor_home() -> str:
    """Return the Moor home directory, respecting MOOR_HOME env var."""
    return str(get_moor_home())


def _failure_marker_path() -> str:
    """Return the path to the install-failure marker file."""
    return os.path.join(_get_moor_home(), ".tirith-install-failed")
...
def _moor_bin_dir() -> str:
    """Return $MOOR_HOME/bin, creating it if needed."""
    d = os.path.join(_get_moor_home(), "bin")
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $MOOR_HOME/bin/tirith.
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    2. $MOOR_HOME/bin/tirith (previously auto-installed)
    3. Auto-install from GitHub releases → $MOOR_HOME/bin/tirith
    ...
    if os.path.isfile(moor_bin) and os.access(moor_bin, os.X_OK):
        _resolved_path = moor_bin
        _install_failure_reason = ""
        _clear_install_failed()
        return moor_bin
