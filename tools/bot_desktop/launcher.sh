#!/usr/bin/env bash
# Moor Bot Desktop — one headless Xfce desktop per Moor profile, served over RFB.
#
# Spawned by tools/bot_desktop/runtime.py with MOOR_BD_* variables set. Runs TigerVNC's Xvnc
...
: "${MOOR_BD_PROFILE:?}"      # profile name (display name in the VNC title)
: "${MOOR_BD_DISPLAY_NUM:?}"  # allocated by runtime.py
: "${MOOR_BD_SOCKET:?}"       # RFB unix socket path
: "${MOOR_BD_XAUTH:?}"        # Xauthority path
: "${MOOR_BD_ENV_FILE:?}"     # where to publish DISPLAY/XAUTHORITY/DBUS_SESSION_BUS_ADDRESS
: "${MOOR_BD_CONFIG_HOME:?}"  # per-profile XDG_CONFIG_HOME (xfconf lives here)
GEOM="${MOOR_BD_GEOMETRY:-1440x900}"
DEPTH=24

export XDG_CONFIG_HOME="$MOOR_BD_CONFIG_HOME"
export XDG_CACHE_HOME="${MOOR_BD_CACHE_HOME:-$MOOR_BD_CONFIG_HOME/.cache}"
export XDG_DATA_HOME="${MOOR_BD_DATA_HOME:-$MOOR_BD_CONFIG_HOME/.local-share}"
...
mkdir -p "$XDG_CONFIG_HOME/xfce4/xfconf/xfce-perchannel-xml" "$XDG_CONFIG_HOME/autostart" \
         "$XDG_CACHE_HOME" "$XDG_DATA_HOME" "$(dirname "$MOOR_BD_SOCKET")"

export DISPLAY=":$MOOR_BD_DISPLAY_NUM"
export XAUTHORITY="$MOOR_BD_XAUTH"
...
xlock="/tmp/.X${MOOR_BD_DISPLAY_NUM}-lock"
if [[ -e "$xlock" ]] && ! kill -0 "$(tr -d ' ' < "$xlock" 2>/dev/null)" 2>/dev/null; then
  rm -f "$xlock" "/tmp/.X11-unix/X${MOOR_BD_DISPLAY_NUM}"  # no-tmp: ok — X11 display socket, fixed by the protocol
fi
...
# ---- look: dark theme from whatever the host ships (first match wins), Moor wallpaper ----
...
: "${MOOR_BD_WALLPAPER:="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)/wallpaper.png"}"
...
<channel name="xfwm4" version="1.0">
  <property name="general" type="empty">
    ...
    <property name="theme" type="string" value="MOOR_BD_WM_THEME"/>
    <property name="title_font" type="string" value="DejaVu Sans Bold 9"/>
  </property>
</channel>
EOF
sed -i "s|MOOR_BD_WM_THEME|$WM_THEME_NAME|" "$X/xfwm4.xml"
...
<channel name="xsettings" version="1.0">
  <property name="Net" type="empty">
    ...
    <property name="ThemeName" type="string" value="MOOR_BD_GTK_THEME"/>
    <property name="IconThemeName" type="string" value="MOOR_BD_ICON_THEME"/>
  </property>
...
EOF
sed -i "s|MOOR_BD_GTK_THEME|$GTK_THEME_NAME|; s|MOOR_BD_ICON_THEME|$ICON_THEME_NAME|" "$X/xsettings.xml"
    ...
    <property name="screen0" type="empty">
      <property name="monitorVNC-0" type="empty">
        <property name="workspace0" type="empty">
          ...
          <property name="last-image" type="string" value="MOOR_BD_WALLPAPER_PLACEHOLDER"/>
...
EOF
sed -i "s|MOOR_BD_WALLPAPER_PLACEHOLDER|$MOOR_BD_WALLPAPER|" "$X/xfce4-desktop.xml"
...
if [[ ! -e "$X/xfce4-panel.xml" ]]; then
  ...
  add_launcher() {  # name icon executable [Exec= line] — skipped when the executable is missing.
    ...
    printf '[Desktop Entry]\nVersion=1.0\nType=Application\nName=%s\nIcon=%s\n%s\nTerminal=false\nStartupNotify=false\n' \
      "$1" "$2" "${4:-Exec=$3}" > "$L/launcher-$n/moor.desktop"
    dock_ids+=("$n")
  }
  ...
  [[ -n "${MOOR_BD_BROWSER_EXEC:-}" ]] && \
    add_launcher "Browser" internet-web-browser "$MOOR_BD_BROWSER_EXEC" "${MOOR_BD_BROWSER_EXEC_LINE:-}" && \
    echo "$n" > "$L/.moor-browser-launcher"
  ...
  for id in "${dock_ids[@]}"; do
    ...
    dock_items+="<property name=\"plugin-$id\" type=\"string\" value=\"launcher\"><property name=\"items\" type=\"array\"><value type=\"string\" value=\"moor.desktop\"/></property></property>"
  done
...
# after `moor profile rename` the old path would open a browser with an empty, unshared cookie jar.
L="$XDG_CONFIG_HOME/xfce4/panel"
# Profiles seeded before the marker existed have the launcher but no `.moor-browser-launcher`; recover
# it from our own Browser entry (moor.desktop is ours by name) so their Exec= follows a rename too.
if [[ -n "${MOOR_BD_BROWSER_EXEC:-}" && -n "${MOOR_BD_BROWSER_EXEC_LINE:-}" && ! -r "$L/.moor-browser-launcher" ]]; then
  for d in "$L"/launcher-*/moor.desktop; do
    [[ -f "$d" ]] || continue
    while IFS= read -r line; do
      if [[ "$line" == "Name=Browser" ]]; then
        bn="${d%/moor.desktop}"; bn="${bn##*/launcher-}"
        printf '%s\n' "$bn" > "$L/.moor-browser-launcher"; break 2
      fi
    done < "$d"
  done
fi
if [[ -n "${MOOR_BD_BROWSER_EXEC:-}" && -n "${MOOR_BD_BROWSER_EXEC_LINE:-}" && -r "$L/.moor-browser-launcher" ]]; then
  bn="$(cat "$L/.moor-browser-launcher")"
  d="$L/launcher-$bn/moor.desktop"
  [[ -f "$d" ]] && sed -i "s|^Exec=.*|$(printf '%s' "$MOOR_BD_BROWSER_EXEC_LINE" | sed 's/[&|\\]/\\&/g')|" "$d"
fi

# Tests seed the config tree on a fake PATH and stop here (no X server needed).
[[ -n "${MOOR_BD_SEED_ONLY:-}" ]] && exit 0
...
Xvnc "$DISPLAY" -geometry "$GEOM" -depth "$DEPTH" -dpi 96 \
  -rfbport -1 -rfbunixpath "$MOOR_BD_SOCKET" -rfbunixmode 0600 \
  -SecurityTypes None -AlwaysShared -AcceptSetDesktopSize -FrameRate 30 -SendCutText=0 -MaxCutText 262144 \
  -desktop "moor:$MOOR_BD_PROFILE" -auth "$XAUTHORITY" -nolisten tcp \
...
dbus-run-session -- bash -c '
  ...
  printf "DISPLAY=%s\nXAUTHORITY=%s\nDBUS_SESSION_BUS_ADDRESS=%s\nXDG_CONFIG_HOME=%s\nXDG_CACHE_HOME=%s\nXDG_DATA_HOME=%s\n" \
    ...
    > "$MOOR_BD_ENV_FILE.tmp" && mv -f "$MOOR_BD_ENV_FILE.tmp" "$MOOR_BD_ENV_FILE"
