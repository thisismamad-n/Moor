...
_VISION_DOWNLOAD_TIMEOUT = _read_vision_setting("MOOR_VISION_DOWNLOAD_TIMEOUT", "download_timeout", float)
...
def _resolve_vision_cpu_workers() -> int:
    """MOOR_VISION_MAX_CONCURRENCY → ``auxiliary.vision.max_concurrency`` → host cores (< 1 ignored)."""
    val = _read_vision_setting("MOOR_VISION_MAX_CONCURRENCY", "max_concurrency", int, minimum=1)
...
_TOOL_RESULT_MEDIA_PROVIDERS = frozenset({
    "openrouter", "moor", "vertex", "bedrock", "anthropic-vertex", "google-vertex",
...
async def vision_analyze_tool(
    ...
    is pre-formatted by the caller. Temp images live under $MOOR_HOME/cache/vision/."""
...
def check_vision_requirements() -> bool:
    ...
    provider → openrouter → moor) — without the auto step the tool would vanish whenever
...
async def _materialize_video(video_url: str, task_id: Optional[str], temp_paths: list) -> Path:
    ...
    if await _validate_image_url_async(video_url):
        ...
        if blocked:
            raise PermissionError(blocked["message"])
        path = get_moor_dir("cache/video", "temp_video_files") / f"temp_video_{uuid.uuid4()}.mp4"
        temp_paths.append(path)
