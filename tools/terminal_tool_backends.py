...
from tools.tool_backend_helpers import (has_direct_modal_credentials, managed_moor_tools_enabled,
                                        moor_tool_gateway_unavailable_message, resolve_modal_backend_state)
...
def _modal_unavailable_reason(modal_state: Dict[str, Any]) -> tuple[str, str]:
    ...
    gateway = moor_tool_gateway_unavailable_message("managed Modal execution")
    if modal_state["managed_mode_blocked"] or modal_state["mode"] == "managed":
        tail = (("Moor Tool Gateway access is not currently available and no direct Modal credentials/config "
...
def _build_docker_env(*, image, cwd, timeout, cc, task_id, host_cwd, **_):
    ...
    # One-shot reaper for labeled containers orphaned by prior Moor processes that died before
