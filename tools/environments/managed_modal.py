...
class ManagedModalEnvironment(BaseEnvironment):
    """Gateway-owned Modal sandbox with moor-compatible execute/cleanup."""
    ...
    def __init__(self, image: str, cwd: str = "/root", timeout: int = 60,
        ...
        if gateway is None:
            raise ValueError("Managed Modal requires a configured tool gateway and Moor user token")
        self._gateway_origin = gateway.gateway_origin.rstrip("/")
        self._moor_user_token = gateway.moor_user_token
    ...
    def _request(self, method: str, path: str, *, json: Dict[str, Any] | None = None, timeout: int = 30,
        ...
        headers = {"Authorization": f"Bearer {self._moor_user_token}", "Content-Type": "application/json",
                   **(extra_headers or {})}
