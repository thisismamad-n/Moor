"""Secret-scrub policy for Moor child processes: pure data + predicates for which env
names are moor-managed credentials. The env *builders* applying it (``_make_run_env``,
``_sanitize_subprocess_env``, ``moor_subprocess_env``) live in ``tools.environments.local``."""
...
# moor-managed AWS *inference* credentials for ``auth_type="aws_sdk"`` (Bedrock):
...
_STATIC_PROVIDER_ENV_BLOCKLIST = frozenset({
    ...
    "EMAIL_HOME_ADDRESS", "EMAIL_HOME_ADDRESS_NAME", "MOOR_DASHBOARD_SESSION_TOKEN",
...
def _build_provider_env_blocklist() -> frozenset:
    ...
    # Claude Code install, not Moor: stripping it made agent-spawned ``claude`` CLIs
    ...
    # It is set and owned by the user's Claude Code install (subscription OAuth), not a moor-managed
    # inference credential — Claude subscription auth is not a working Moor provider path. It arrives via
...
# ``_sanitize_subprocess_env``); execute_code, moor_subprocess_env, docker and
...
# ``buzz`` (the gateway's ``MOOR_SESSION_PLATFORM`` ContextVar; concurrency safe under a multi-session
...
# scope. Every other surface stays sealed — execute_code scrubbing, :func:`moor_subprocess_env` (browser /
...
# the blocklist entirely because it is NOT a Moor credential; these ARE moor-managed first-party
...
def _buzz_terminal_context_active() -> bool:
    ...
    try:
        ...
        return get_session_env("MOOR_SESSION_PLATFORM", "").strip().lower() == "buzz"
    except Exception:
        return False
...
# ANOTHER project's deps into the Moor venv (still reachable via PATH, so stripping
# is safe); PYTHONHOME redirects a child interpreter's stdlib to the Moor venv
# (version-mismatch crashes). PYTHONPATH is handled separately (moor-owned entries only).
...
# dependencies into the Moor venv path instead of the project's own ``.venv`` — silently clobbering the
# Moor environment (e.g. a project pinned to a different Python version overwrites it and breaks the
# gateway). PYTHONHOME is included because a gateway-inherited value redirects the standard-library search
# of ANY child interpreter — including unrelated system/venv Pythons — to the Moor venv's stdlib, which
# crashes with version-mismatch errors before a child script even imports a package (#75018). Moor itself
...
# _strip_moor_owned_pythonpath() which removes only moor-owned entries, preserving user-set paths.
_ACTIVE_VENV_MARKER_VARS = ("VIRTUAL_ENV", "CONDA_PREFIX", "PYTHONHOME")


def _is_moor_internal_secret(key: str) -> bool:
    """True for moor-internal secrets injected under *dynamic* names the static
...
_ALWAYS_STRIP_KEYS: frozenset[str] = frozenset({
    ...
    "HASS_TOKEN", "EMAIL_PASSWORD", "MOOR_DASHBOARD_SESSION_TOKEN",
