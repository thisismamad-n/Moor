...
# Module-level binding: tests patch ``ssh._load_moor_env_vars`` to fake the .env file.
...
class SSHEnvironment(BaseEnvironment):
    ...
    def __init__(self, host: str, user: str, cwd: str = "~",
        ...
        self.control_dir = Path(tempfile.gettempdir()) / "moor-ssh"
        ...
        self._sync_manager = FileSyncManager(
            get_files_fn=lambda: iter_sync_files(f"{self._remote_home}/.moor"),
    ...
    def _ensure_remote_dirs(self) -> None:
        """Create base ~/.moor directory tree on remote in one SSH call."""
        base = f"{self._remote_home}/.moor"
    ...
    def _ssh_bulk_upload(self, files: list[tuple[str, str]]) -> None:
        ...
        with tempfile.TemporaryDirectory(prefix="moor-ssh-bulk-") as staging:
    ...
    def _run_bash(self, cmd_string: str, *, login: bool = False, timeout: int = 120,
        ...
        values, unset_names = resolve_passthrough_env(moor_env_loader=_load_moor_env_vars)
