...
from tools.environments.local_env_policy import _MOOR_PROVIDER_ENV_BLOCKLIST, _is_moor_internal_secret


def load_moor_env_vars() -> dict[str, str]:
    """``~/.moor/.env`` values, or ``{}`` — a broken .env must not fail command execution."""
    try:
...
def resolve_passthrough_env(explicit_forward: Iterable[str] = (),
                            moor_env_loader: Callable[[], dict[str, str]] = load_moor_env_vars,
    ...
    is filtered through the Moor provider-credential blocklist and the dynamic internal-secret
    ...
    forward_keys = set(explicit_forward) | (implicit_forward - _MOOR_PROVIDER_ENV_BLOCKLIST)
