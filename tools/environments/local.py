...
def _resolve_local_initial_cwd(cwd: str) -> str:
    ...
    directory Moor was already launched from (for example ``moor-agent``
    while the process cwd is ``~/.moor/moor-agent``), passing it through
    unchanged makes the wrapper run ``cd moor-agent`` *inside* the project
    ...
    # Common recovery for config values like ``moor-agent`` when Moor was
    # launched from that directory already.  ``os.path.abspath`` would point at
    # a nonexistent nested ``./moor-agent``; use the current directory instead.
...
_MOOR_PROVIDER_ENV_BLOCKLIST = _build_provider_env_blocklist()
...
# project's dependencies into the Moor venv path instead of the project's own
# ``.venv`` — silently clobbering the Moor environment (e.g. a project pinned
# to a different Python version overwrites it and breaks the gateway). The
# Moor venv stays reachable via PATH (its bin dir is first), so stripping
...
def _inject_session_context_env(env: dict) -> None:
    ...
    (MOOR_SESSION_*) are bridged onto the child env here.
...
def _sanitize_subprocess_env(base_env: dict | None, extra_env: dict | None = None) -> dict:
    ...
    for key, value in (extra_env or {}).items():
        if key.startswith(_MOOR_PROVIDER_ENV_FORCE_PREFIX):
            real_key = key[len(_MOOR_PROVIDER_ENV_FORCE_PREFIX):]
            if _is_moor_internal_secret(real_key):
                continue
    ...
    _inject_context_moor_home(sanitized)

    from moor_constants import apply_subprocess_home_env
...
# legitimate child Moor spawns needs them, and they are the highest-value
...
# narrow subset of _MOOR_PROVIDER_ENV_BLOCKLIST; provider keys are handled by
# the conditional Tier-2 strip in moor_subprocess_env().
_ALWAYS_STRIP_KEYS: frozenset[str] = frozenset({
    ...
    # _is_moor_internal_secret, but _ID has no secret suffix, so it must be
...
def moor_subprocess_env(*, inherit_credentials: bool = False) -> dict[str, str]:
    ...
    single source of truth (``_MOOR_PROVIDER_ENV_BLOCKLIST``).  The terminal
    ...
    * **Tier 1 (always):** ``_ALWAYS_STRIP_KEYS`` — gateway bot tokens, GitHub
      ...
      ``inherit_credentials``.  No child Moor spawns legitimately needs them.
    * **Tier 2 (conditional):** the rest of ``_MOOR_PROVIDER_ENV_BLOCKLIST``
    ...
    # legitimate use for them. See :func:`_is_moor_internal_secret`.
    for key in list(env):
        if key.startswith(_MOOR_PROVIDER_ENV_FORCE_PREFIX):
            env.pop(key, None)
        elif _is_moor_internal_secret(key):
    ...
    if not inherit_credentials:
        ...
        for key in _MOOR_PROVIDER_ENV_BLOCKLIST:
            env.pop(key, None)

    # Windows UTF-8 safety for spawned processes (#31420).
    env.setdefault("PYTHONUTF8", "1")

    _inject_context_moor_home(env)
    from moor_constants import apply_subprocess_home_env
    apply_subprocess_home_env(env)

    _strip_moor_owned_pythonpath_and_runtime_markers(env)

    _apply_windows_msys_bash_env_defaults(env)

    # Cross-session leak guard, same as the terminal spawn paths: this helper
    # copies os.environ, whose MOOR_SESSION_* mirror is a last-writer-wins
    ...
    # still see the parent's MOOR_HOME but lose the DB mutation guard.
...
def build_subprocess_env(
    ...
    *,
    ...
    function (or :func:`moor_subprocess_env` for the model-driving-CLI
    surface) instead of copying ``os.environ`` directly, so profile-home
    propagation (``MOOR_HOME`` / subprocess ``HOME`` contract) and the
    ...
    * ``scrub_secrets=True`` (default) — delegate to
      ...
      list (provider blocklist + ``_is_moor_internal_secret`` dynamic
      patterns + kanban/venv-marker/session-context guards) **and** of
      ``MOOR_HOME`` / subprocess-HOME propagation.  On this path profile
    ...
    * ``inherit_profile_home`` — on the non-scrub path, when True, bridge the
      context-local Moor home override into ``MOOR_HOME`` and apply the
      subprocess HOME contract (``moor_constants.apply_subprocess_home_env``).
    ...
    * ``extra`` — applied **last** on the non-scrub path so explicit caller
      overrides (e.g. a session-scoped ``MOOR_HOME``) always win.  On the
      scrub path it is forwarded as ``_sanitize_subprocess_env``'s
      ``extra_env`` (same force-prefix / blocklist handling as today).
    """
    if scrub_secrets:
        # _sanitize_subprocess_env already performs MOOR_HOME override
...
def _find_bash() -> str:
    ...
    # system Git (or a stale MOOR_GIT_BASH_PATH pointing at one) must not
    ...
    #   PortableGit: %LOCALAPPDATA%\moor\git\bin\bash.exe   (primary)
    #   MinGit:      %LOCALAPPDATA%\moor\git\usr\bin\bash.exe (legacy/32-bit fallback)
    _local_appdata = os.environ.get("LOCALAPPDATA", "")
    _moor_portable_git = os.path.join(_local_appdata, "moor", "git") if _local_appdata else ""
    if _moor_portable_git:
        for candidate in (
            os.path.join(_moor_portable_git, "bin", "bash.exe"),        # PortableGit (primary)
            os.path.join(_moor_portable_git, "usr", "bin", "bash.exe"), # MinGit fallback
        ):
    ...
    # MOOR_GIT_BASH_PATH pointing at a broken Git-for-Windows install
    # (``Directory \\drivers\\etc does not exist``) must not win over a
    # healthy portable Git under %LOCALAPPDATA%\\moor\\git.
    for candidate in candidates:
        if _bash_starts(candidate):
            if candidate != custom and custom and os.path.isfile(custom):
                logger.warning(
                    "MOOR_GIT_BASH_PATH=%s fails to start; using %s instead",
                    custom,
                    candidate,
                )
            return candidate
...
def _git_bash_aslr_help(bash: str, details: str = "") -> str:
    ...
    return (
        ...
        "Then restart Moor. If the override is blocked or later re-applied, "
...
# Cached directory containing the ``moor`` console-script.
# ``_SENTINEL`` distinguishes "not resolved yet" from a resolved ``None``.
_SENTINEL = object()
_MOOR_BIN_DIR: "str | None | object" = _SENTINEL


def _resolve_moor_bin_dir() -> str | None:
    """Return the directory holding the ``moor`` console-script, or None.
    ...
    moor install dir (``~/.local/bin``, the venv ``bin``/``Scripts``, pipx,
    nix) is absent from that PATH, so plugins shelling out to bare ``moor``
    via the terminal tool hit ``command not found`` (exit 127) even though
    ``moor`` works fine in the user's own interactive terminal.

    We resolve the install dir once (it never changes within a process) and
    prepend-if-missing it to the subshell PATH so bare ``moor`` resolves
    regardless of how the gateway was started.

    Resolution order (cheap, no heavy imports):
      1. ``shutil.which("moor")`` — normal PATH-installed shim.
      2. The directory of ``sys.argv[0]`` when it's an absolute path to a
         real ``moor`` executable (covers nix-store / venv wrappers).
      3. The directory of ``sys.executable`` — the running interpreter's
         venv ``bin``/``Scripts`` is where its console-scripts live.
    """
    global _MOOR_BIN_DIR
    if _MOOR_BIN_DIR is not _SENTINEL:
        return _MOOR_BIN_DIR  # type: ignore[return-value]

    candidate: str | None = None

    which = shutil.which("moor")
    if which:
    ...
    if candidate is None:
        ...
        if (
            os.path.isabs(argv0)
            and (base == "moor" or base.startswith("moor."))
    ...
    if candidate is None:
        ...
        if exe_dir:
            shim = "moor.exe" if _IS_WINDOWS else "moor"
    ...
    _MOOR_BIN_DIR = candidate
    return candidate


def _prepend_moor_bin_dir(existing_path: str) -> str:
    """Prepend the moor install dir to ``existing_path`` if it's missing.
...
def _managed_runtime_path_entries() -> list[str]:
    ...
    - ``$MOOR_HOME/node`` (+ ``/bin``) — installed to satisfy the desktop and
    ...
    ``get_moor_home()`` is profile-scoped and a managed tree can appear
    mid-process (``heal_moor_managed_node``, a first browser install).
    """
    try:
        from moor_constants import get_moor_home, iter_moor_node_dirs

        candidates = [*iter_moor_node_dirs(), get_moor_home() / "bin"]
...
def _apply_windows_msys_bash_env_defaults(env: dict) -> None:
    ...
    Windows commands such as ``tasklist``, ``schtasks``, and ``wmic``.  Moor
...
def _make_run_env(env: dict) -> dict:
    ...
    for k, v in merged.items():
        if k.startswith(_MOOR_PROVIDER_ENV_FORCE_PREFIX):
            real_key = k[len(_MOOR_PROVIDER_ENV_FORCE_PREFIX):]
            if _is_moor_internal_secret(real_key):
                continue
            run_env[real_key] = v
        elif _is_moor_internal_secret(k):
            continue
        else:
            passthrough = _is_passthrough(k)
            if k in _MOOR_PROVIDER_ENV_BLOCKLIST and not passthrough:
                continue
    ...
    if path_key is not None:
        ...
        # Ensure the moor install dir is reachable so plugins can shell out
        # to bare ``moor`` via the terminal tool even when the gateway was
        # launched without it on PATH (systemd, service managers, cron, etc.).
        run_env[path_key] = _prepend_moor_bin_dir(new_path)

    _inject_context_moor_home(run_env)
    ...
    _strip_moor_owned_pythonpath_and_runtime_markers(run_env)
...
def _build_moor_repo_root_aliases(
    resolved_root: Path,
    lexical_root: Path,
    ...
    # lexically the same way get_default_moor_root() does (parent of a
    ...
    # while still never matching arbitrary descendants of MOOR_HOME.
    ...
    # junction/symlink under the configured root (e.g. D:\moor\moor-agent
    # -> C:\...\moor-agent) while the import spelling (editable install)
...
#: PYTHONPATH so the backend can do ``import tools``, ``import moor_cli``,
#: etc.  Subprocesses that are NOT the Moor backend don't need it and it
#: can shadow local packages.
_moor_repo_root: Path = Path(__file__).resolve().parents[2]
...
_moor_repo_root_aliases: tuple[Path, ...] = _build_moor_repo_root_aliases(
    _moor_repo_root,
...
#: subprocess spawn.
_moor_site_packages: list[Path] | None = None
...
def _validated_runtime_venv(env: dict) -> Path | None:
    ...
    if not any(_same_path(candidate, repo_root / "venv") for repo_root in _moor_repo_root_aliases):
        return None
...
def _get_moor_site_packages(env: dict) -> list[Path]:
    ...
    """
    global _moor_site_packages
    if _moor_site_packages is not None:
        result = list(_moor_site_packages)
    else:
        result = []
        if _in_venv:
            try:
        ...
        _moor_site_packages = list(result)
...
def _strip_moor_owned_pythonpath(env: dict) -> None:
    ...
    moor_site_packages = _get_moor_site_packages(env)
    ...
    for entry in pp.split(os.pathsep):
        ...
        if not should_strip:
            should_strip = any(
                ...
                for repo_root in _moor_repo_root_aliases
            )
...
class LocalEnvironment(BaseEnvironment):
    ...
    def get_temp_dir(self) -> str:
        ...
        **Windows:** hardcoded ``/tmp`` is wrong in two ways — native Python
        ...
        ``MOOR_HOME`` instead — single-word path, guaranteed to exist, same
        string resolves in both Git Bash and native Python.
        """
        if _IS_WINDOWS:
            # Derive a Windows-safe temp dir under MOOR_HOME.  Using
            ...
            try:
                ...
                cache_dir = get_moor_home() / "cache" / "terminal"
            except Exception:
                cache_dir = Path(tempfile.gettempdir()) / "moor_terminal"
    ...
    def _run_bash(self, cmd_string: str, *, login: bool = False,
        ...
        if not _IS_WINDOWS:
            try:
                proc._moor_pgid = os.getpgid(proc.pid)
        ...
        try:
            if _IS_WINDOWS:
                ...
                try:
                    ...
                    pgid = getattr(proc, "_moor_pgid", None)
                    if pgid is None:
                        raise
