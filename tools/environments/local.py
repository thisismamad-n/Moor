...
_BG_GROUP_RE = re.compile(r"^(moor_bg_[A-Za-z0-9_-]+)\.(log|pid|exit)$")


def _default_terminal_temp_dir() -> "Path | None":
    """Return MOOR_HOME/cache/terminal, or None if unresolvable."""
    try:
        from moor_constants import get_moor_home
        return get_moor_home() / "cache" / "terminal"
    except Exception:
        return None
...
def _apply_profile_home(env: dict) -> None:
    """Bridge the context-local MOOR_HOME override, then the subprocess HOME contract."""
    from moor_constants import apply_subprocess_home_env, get_moor_home_override
    try:
        if value := get_moor_home_override():
            env["MOOR_HOME"] = value
    except Exception:
        pass
...
def _inject_session_context_env(env: dict) -> None:
    """Bridge gateway session ContextVars (MOOR_SESSION_*) into a child env.
...
def _filter_secret_env(
    ...
    """Copy *items* into *out*, dropping moor-managed secrets. ``_MOOR_FORCE_<NAME>``
    ...
    for key, value in items.items():
        if key.startswith(_MOOR_PROVIDER_ENV_FORCE_PREFIX):
            if not unwrap_force:
                continue
            key = key[len(_MOOR_PROVIDER_ENV_FORCE_PREFIX):]
...
def _finalize_child_env(env: dict) -> dict:
    ...
    bridging, moor-owned PYTHONPATH + venv-marker strip, MSYS defaults, delegate_task
...
def _scrubbed_env(parts, plugin_strip: frozenset, fix_path) -> dict:
    ...
    *fix_path* (always prepending the moor install dir so bare ``moor`` resolves
    ...
    if path_key is not None:
        out[path_key] = _prepend_moor_bin_dir(fix_path(out.get(path_key, "")))
    return _finalize_child_env(out)


def _sanitize_subprocess_env(base_env: dict | None, extra_env: dict | None = None) -> dict:
    """Filter moor-managed secrets from a subprocess environment (background/PTY
...
def _resolve_moor_bin_dir() -> str | None:
    """Directory holding the ``moor`` console-script, or None (cached). A gateway
    launched by systemd/cron/a desktop launcher lacks the install dir on PATH and bare
    ``moor`` exits 127. Order: ``which``; absolute ``sys.argv[0]`` naming a real
    moor executable; ``sys.executable``'s dir if it holds the shim."""
    ...
    elif (os.path.isabs(argv0) and (base == "moor" or base.startswith("moor."))
    ...
    _MOOR_BIN_DIR = candidate if candidate and os.path.isdir(candidate) else None
    return _MOOR_BIN_DIR


def _prepend_moor_bin_dir(existing_path: str) -> str:
    """Prepend the moor install dir to ``existing_path`` if missing."""
...
def _apply_windows_msys_bash_env_defaults(env: dict) -> None:
    ...
    and ``wmic``. Moor runs terminal commands through bash on Windows, so set the standard MSYS opt-out by
    default. Refs #56700.
...
# spellings — the Windows gateway launcher renders moor-owned paths under the
# configured MOOR_HOME spelling (possibly a junction to another drive).
_moor_repo_root: Path = Path(__file__).resolve().parents[2]
_moor_repo_root_aliases: tuple[Path, ...] = _build_moor_repo_root_aliases(
    _moor_repo_root, Path(__file__).absolute().parents[2], get_process_moor_home())
_in_venv: bool = (getattr(sys, "base_prefix", sys.prefix) != sys.prefix
                  or hasattr(sys, "real_prefix"))  # real_prefix: virtualenv<20
_moor_site_packages: list[Path] | None = None  # lazily cached by local_pythonpath
...
def _kill_process_group_posix(proc) -> None:
    ...
    try:
        ...
        if (pgid := getattr(proc, "_moor_pgid", None)) is None:
            raise
...
class LocalEnvironment(BaseEnvironment):
    ...
    # Commands run on the Moor host itself — controller-side platform behavior
