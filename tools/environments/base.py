...
def get_sandbox_dir() -> Path:
    ...
    overlays/SIF cache). ``TERMINAL_SANDBOX_DIR`` overrides ``{MOOR_HOME}/sandboxes``."""
    custom = os.getenv("TERMINAL_SANDBOX_DIR")
    p = Path(custom) if custom else get_moor_home() / "sandboxes"
...
class BaseEnvironment(ABC):
    """Common interface and unified execution flow for all Moor backends. Subclasses
    ...
    # True only when commands execute on the SAME host as the Moor process
    ...
    def fetch_file(self, remote_path: str, local_dest: Path, *, max_bytes: int) -> None:
        ...
        import base64
        import binascii
        marker = f"__MOOR_FETCH_{uuid.uuid4().hex[:12]}__"
    ...
    def _wait_for_process(
        ...
        if stdin_errors := getattr(proc, "_moor_stdin_errors", None):
    ...
    def _extract_cwd_from_output(self, result: dict):
        """Parse the ``__MOOR_CWD_{session}__`` marker from ``result["output"]``, update
