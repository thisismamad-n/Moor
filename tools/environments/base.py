"""Base class for all Moor execution environment backends.
...
# MOOR_DEBUG_INTERRUPT=1 to log loop entry/exit, periodic heartbeats, and
# every is_interrupted() state change from _wait_for_process.  Off by default
# to avoid flooding production gateway logs.
_DEBUG_INTERRUPT = bool(os.getenv("MOOR_DEBUG_INTERRUPT"))
...
def get_sandbox_dir() -> Path:
    ...
    Configurable via TERMINAL_SANDBOX_DIR. Defaults to {MOOR_HOME}/sandboxes/.
    """
    custom = os.getenv("TERMINAL_SANDBOX_DIR")
    if custom:
        p = Path(custom)
    else:
        p = get_moor_home() / "sandboxes"
...
def _pipe_stdin(proc: subprocess.Popen, data: str) -> None:
    ...
    recorded on ``proc._moor_stdin_errors`` while stdin is still closed
    ...
    proc._moor_stdin_errors = errors
    ...
    proc._moor_stdin_thread = thread
    thread.start()
...
def _cwd_marker(session_id: str) -> str:
    return f"__MOOR_CWD_{session_id}__"
...
# the FIRST session's MOOR_SESSION_ID into the snapshot makes every LATER
...
# MOOR_SESSION_ID leak via the shared snapshot). Stripping them from the
...
# with one of these prefixes (or is MOOR_UI_SESSION_ID). Used by unit tests
# as the Python-side contract for the exclusion set; the dump path unsets by
# name/prefix instead of grepping declare lines (see below / issue #71296).
_SNAPSHOT_EXCLUDED_ENV_REGEX = (
    "^declare -x (MOOR_SESSION_|MOOR_UI_SESSION_ID|MOOR_CRON_AUTO_DELIVER_|MOOR_CRON_SESSION)"
)
...
def _export_dump_excluding_session_vars(
    ...
    Matrix room/display name via ``MOOR_SESSION_CHAT_NAME``) land in the
    ...
    return (
        "{ ( "
        "unset ${!MOOR_SESSION_*} ${!MOOR_CRON_AUTO_DELIVER_*} "
        # AI_AGENT / MOOR_AGENT are per-command attribution markers
...
class BaseEnvironment(ABC):
    """Common interface and unified execution flow for all Moor backends.
    ...
    def __init__(self, cwd: str, timeout: int, env: dict = None):
        ...
        self._snapshot_path = f"{temp_dir}/moor-snap-{self._session_id}.sh"
        self._cwd_file = f"{temp_dir}/moor-cwd-{self._session_id}.txt"
    ...
    def init_session(self):
        ...
        _snap_tmp = '"$__moor_snap_tmp"'
        snapshot_excluded = self._snapshot_excluded_passthrough_names()
        bootstrap = (
            f"umask 077\n"
            f"__moor_snap_tmp=$(mktemp {_snap_tmp_template}) || exit 1\n"
            ...
            f"__moor_fns=$(declare -F | awk '{{print $3}}' | grep -vE '^_[^_]') || true\n"
            f"[ -n \"$__moor_fns\" ] && declare -f $__moor_fns "
    ...
    def _wrap_command(self, command: str, cwd: str) -> str:
        ...
        for name in passthrough_names:
            marker = f"_MOOR_RUNTIME_PASSTHROUGH_{name}"
        ...
        # ``MOOR_AGENT`` marker.  The value MUST equal our id in the public
        # agent-harness registry (``moor-agent`` — see huggingface.js
        ...
        parts.append(
            'export AI_AGENT="${AI_AGENT:-moor-agent}" '
            'MOOR_AGENT="${MOOR_AGENT:-true}"'
        )
        ...
        parts.append("__moor_ec=$?")
        # Restrict Moor metadata files without changing the user's command
        ...
        if self._snapshot_ready:
            parts.append(
                f"__moor_snap_tmp=$(mktemp {_snap_tmp_template}) && "
    ...
    def _embed_stdin_heredoc(command: str, stdin_data: str) -> str:
        ...
        delimiter = f"MOOR_STDIN_{uuid.uuid4().hex[:12]}"
    ...
    def _wait_for_process(
        ...
        if bounded_capture:
            ...
            try:
                spill_dir = get_moor_home() / "cache" / "terminal-output"
        ...
        # --- Debug tracing (opt-in via MOOR_DEBUG_INTERRUPT=1) -------------
        ...
        stdin_thread = getattr(proc, "_moor_stdin_thread", None)
        if stdin_thread is not None:
            stdin_thread.join(timeout=5)
        rendered = output.render()
        result = self._finalize_wait_result(output, rendered, proc.returncode)
        stdin_errors = getattr(proc, "_moor_stdin_errors", None)
        if stdin_errors:
    ...
    def _extract_cwd_from_output(self, result: dict):
        """Parse the __MOOR_CWD_{session}__ marker from stdout output.
