...
<url>``) is spawned as a direct child of the Moor process. Moor's own
...
Moor process dies hard — ``kill -9``, an OS-level crash, a force-quit of
...
parent dies, so nothing reaps these until the next Moor startup's opt-in
