...
from moor_state_errors import STORAGE_RECOVERY_DOCS_URL, classify_persistence_error, is_disk_full_error
...
_DOCTOR = "Run `moor {profile_arg}doctor --fix` to diagnose and repair."

# cause -> (code, gloss, action). "disk" is split by is_disk_full_error at lookup time.
_STORAGE_FAILURES: dict[str, tuple[str, str, str]] = {
    "locked": (
        "storage_locked",
        "the session database is locked by another Moor process",
        "Wait a moment and try again; if it persists, stop the other Moor process "
        "(`moor {profile_arg}gateway stop`).",
    ),
    "disk_full": (
        "disk_full",
        "the disk is full",
    ...
    "corrupt": (
        ...
        _DOCTOR + " Recovery: `moor {profile_arg}sessions recover --source <state.db> --inspect-only`.",
    ),
    "fts_index": (
        "storage_index_corrupt",
        "the session search index is damaged (the messages themselves are intact)",
        "Run `moor {profile_arg}doctor --fix` (or `moor {profile_arg}sessions repair`) to rebuild it.",
    ),
    "replaced": (
        "storage_replaced",
        "the session database file was replaced while Moor was running",
        "Stop Moor (`moor {profile_arg}gateway stop`), run `moor {profile_arg}doctor`, then start it again.",
    ),
    ...
    "deleted_wal": (
        ...
        "Nothing is lost. Quit every Moor process on this profile (Desktop app, "
        "`moor {profile_arg}gateway stop`, dashboard, cron), run `moor {profile_arg}doctor` — it names "
        "any process still holding the log — then start Moor again. Do not run `doctor --fix` or delete "
    ...
    "turn_lease": (
        "storage_busy",
        "another Moor process took over this session",
