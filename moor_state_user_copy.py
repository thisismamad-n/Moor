...
from moor_state_errors import classify_persistence_error, is_disk_full_error
...
_DOCTOR = "Run `moor doctor --fix` to diagnose and repair."
...
_STORAGE_FAILURES: dict[str, tuple[str, str, str]] = {
    "locked": (
        ...
        "Wait a moment and try again; if it persists, stop the other Moor process (`moor gateway stop`).",
    ),
    "disk_full": (
        "disk_full",
        "the disk is full",
    ...
    "corrupt": (
        ...
        f"{_DOCTOR} Recovery: `moor sessions recover --source <state.db> --inspect-only`.",
    ),
    "fts_index": (
        "storage_index_corrupt",
        "the session search index is damaged (the messages themselves are intact)",
        "Run `moor doctor --fix` (or `moor sessions repair`) to rebuild it.",
    ),
    "replaced": (
        "storage_replaced",
        "the session database file was replaced while Moor was running",
        "Stop Moor (`moor gateway stop`), run `moor doctor`, then start it again.",
    ),
    "deleted_wal": (
        "storage_replaced",
        "the session database file was changed or replaced while Moor was running",
        "Stop Moor (`moor gateway stop`), run `moor doctor`, then start it again.",
    ),
    "compression": (
        "storage_busy",
