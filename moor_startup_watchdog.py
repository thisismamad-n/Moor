...
timeout. Config is env-only (``MOOR_STARTUP_WATCHDOG=0``,
``MOOR_STARTUP_WATCHDOG_TIMEOUT_S``) because config.yaml parsing is itself in scope.
...
ENV_STARTUP_WATCHDOG = "MOOR_STARTUP_WATCHDOG"
ENV_STARTUP_WATCHDOG_TIMEOUT_S = "MOOR_STARTUP_WATCHDOG_TIMEOUT_S"
...
# Module singleton: the arm sites (moor_cli.main / moor_cli.gateway /
...
def get_startup_watchdog_dump_path(home: Optional[Path] = None) -> Path:
    """Return ``<MOOR_HOME>/logs/gateway-startup-watchdog.log``."""
    base = home if home is not None else _process_moor_home()
    return base.joinpath(*_DUMP_RELATIVE)


def startup_watchdog_disabled() -> bool:
    """True when ``MOOR_STARTUP_WATCHDOG`` opts out explicitly."""
...
def arm_startup_watchdog(
    ...
    *,
    ...
    ``MOOR_STARTUP_WATCHDOG=0`` or when the thread could not be started.
    """
    global _handle
    try:
