...
/// desktop app handing off via `moor-setup.exe --update`).
///
...
pub fn run() {
    // Tracing → bootstrap-installer.log under MOOR_HOME/logs/ so install
    ...
    // Moor is already installed, so users can re-run setup to repair a broken
    ...
    tracing::info!(?mode, force_setup, "Moor installer starting");
    ...
    tauri::Builder::default()
        ...
        .setup(move |app| {
            ...
            // Moor is already installed should NOT show the installer or
            // rebuild — it should just open the app, so the /Applications
            // "Moor" doubles as a normal launcher (first run installs, every
            ...
            // Menu/Desktop "Moor" shortcuts that install.ps1 creates, and a
            // reliable detached relaunch there needs the DETACHED_PROCESS +
            // startup-grace handling used by launch_moor_desktop — out of
                ...
                if bootstrap::moor_is_installed(&install_root) {
                    match bootstrap::spawn_installed_desktop(&install_root) {
                        Ok(()) => {
                            ...
                            // before we exit (mirrors launch_moor_desktop).
        ...
        .expect("error while running Moor Setup");
}
