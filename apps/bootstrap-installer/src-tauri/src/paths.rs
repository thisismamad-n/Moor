...
//!
//! Mirrors `moor_constants.get_moor_home()` from the Python CLI:
//!   Windows: %LOCALAPPDATA%\moor
//!   macOS:   ~/.moor
//!   Linux:   ~/.moor  (override via $MOOR_HOME)
//!
//! NOTE (macOS): Python's get_moor_home(), scripts/install.sh, and the
//! Electron desktop's resolveMoorHome() ALL use ~/.moor on macOS — there
...
/// Returns the canonical Moor home directory, respecting $MOOR_HOME if set.
pub fn moor_home() -> PathBuf {
    if let Ok(override_path) = std::env::var("MOOR_HOME") {
    ...
    {
        // %LOCALAPPDATA%\moor — matches scripts/install.ps1's $MoorHome.
        if let Some(local_app_data) = dirs::data_local_dir() {
            return local_app_data.join("moor");
        }
    }

    // macOS + Linux + fallback: ~/.moor (matches Python get_moor_home(),
    // install.sh, and the Electron desktop's resolveMoorHome()).
    if let Some(home) = dirs::home_dir() {
        return home.join(".moor");
    }
...
pub fn bootstrap_cache_dir() -> PathBuf {
    moor_home().join("bootstrap-cache")
}
...
/// MOOR_HOME so it survives repo checkout deletion (unlike anything under
/// moor-agent/).
///
/// On Windows this is `%LOCALAPPDATA%\moor\moor-setup.exe`; on other
...
/// mid-update re-locks the venv shim and triggers `force_kill_other_moor`,
/// which then kills that legitimate backend in a respawn loop (#50238).
///
/// Lives directly under MOOR_HOME (same rationale as `installer_dest`) so the
/// Electron desktop — which resolves MOOR_HOME identically and pins it into
/// the updater's env — agrees on the exact path.
pub fn update_in_progress_marker() -> PathBuf {
    moor_home().join(".moor-update-in-progress")
}
...
pub fn copy_self_to_moor_home() -> std::io::Result<()> {
    ...
    tracing::info!(?src, ?dest, "copied installer to MOOR_HOME");
    Ok(())
}
...
/// We don't always know ACTIVE_MOOR_ROOT until install.ps1 reports it, so
/// this is a probe helper, not a definitive path.
pub fn likely_bootstrap_marker(install_root: &Path) -> PathBuf {
    install_root.join(".moor-bootstrap-complete")
}

/// Initializes tracing to bootstrap-installer.log under MOOR_HOME/logs/.
...
pub fn init_logging() -> Option<WorkerGuard> {
    ...
    if let Err(err) = std::fs::create_dir_all(&dir) {
        ...
        eprintln!("[moor-setup] could not create log dir {dir:?}: {err}");
        return None;
    }
    ...
    let env_filter = tracing_subscriber::EnvFilter::try_from_env("MOOR_BOOTSTRAP_LOG")
...
#[tauri::command]
pub fn get_moor_home() -> String {
    moor_home().to_string_lossy().into_owned()
}
