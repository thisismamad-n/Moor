...
//!      venv shim and packaged app.asar are free; otherwise `moor update`
//!      or repair bootstrap can race locked files),
//!   2. run `moor update --yes --gateway` (Python/repo update; this does NOT
//!      rebuild apps/desktop by design — see cmd_update in moor_cli/main.py),
//!   3. run `moor desktop --build-only` (the rebuild step update skips),
...
//!
//! Cross-platform note: `moor update` already handles macOS/Linux (git/pip).
//! The only OS-specific bits here are the venv shim path (resolve_moor) and
...
/// `moor update` exit code meaning "another moor process is holding the
/// venv shim open / dirty precondition" — see _cmd_update_impl in
/// moor_cli/main.py (sys.exit(2)). We surface a targeted message for this.
const UPDATE_EXIT_CONCURRENT: i32 = 2;

/// How long to wait for the old desktop process to release files under the
/// install tree before giving up and letting `moor update`'s own guard decide.
...
///
/// The marker is also the cross-process update lock: `moor update` claims
/// the same file (see `moor_cli/update_lock.py`) so a dashboard-spawned
...
/// overwriting — the pre-fix clobber is what let a dashboard `moor update`
...
/// UPDATE_MARKER_MAX_AGE_SECONDS in moor_cli/update_lock.py — all three read
...
/// a dashboard-spawned `moor update`) still blocks.
...
/// the marker name our PID — because a `moor update` child that
...
async fn run_update(app: AppHandle) -> Result<()> {
    let moor_home = crate::paths::moor_home();
    let install_root = moor_home.join("moor-agent");
    ...
    // it, that backend re-locks the venv shim, our `force_kill_other_moor`
    ...
    //
    // The same marker is the cross-process update lock (moor_cli/
    // update_lock.py claims it too), so a live foreign owner means another
    // updater — most often a dashboard-spawned `moor update` — is already
    ...
    let moor = resolve_moor(&install_root).ok_or_else(|| {
        let msg = format!(
    ...
    // async on Windows. If it still holds the venv shim, `moor update`
    ...
    // ---- stage 2: moor update -----------------------------------------
    // Pass --branch so `moor update` targets the branch this installer was
    ...
    // without --branch, `moor update` switches the checkout to `main` (a
    ...
    // --force skips `moor update`'s Windows running-exe guard (which would
    ...
    // time `moor update` runs there is no legitimate moor.exe to protect,
    ...
    // probe only checks the moor.exe shim and app.asar, so an external venv
    ...
    // below surfaces the correct "close all Moor windows" message.
    ...
    // Retry-once for the update-boundary crash. `moor update` lazily imports
    ...
    // second `moor update` runs clean because the now-current module is loaded
    ...
    // checkout predating the MOOR_UPDATE_HANDOFF_PID env fix (8c76fe19f)
    ...
    match update.exit_code {
        ...
        other => {
            let msg = format!(
                "moor update failed (exit {:?}). See {} for details.",
                other,
                crate::paths::moor_home()
                    .join("logs")
                    .join("update.log")
                    .display()
            );
    ...
    // ---- stage 3: moor desktop --build-only ----------------------------
    // `moor update` deliberately does NOT build apps/desktop (it installs
    ...
    if let Some(target_app) = launch_target {
        ...
        crate::bootstrap::launch_moor_desktop(app.clone(), install_root.to_string_lossy().into_owned()).await
    {
...
pub(crate) async fn wait_for_install_locks_free(install_root: &Path, app: &AppHandle, stage: &str) {
    ...
    loop {
        ...
        if Instant::now() >= deadline {
            ...
            // Moor.exe, so an image-name kill would tear down the app itself.
            emit_log(
                app,
                Some(stage),
                LogStream::Stdout,
                &format!(
                    "[handoff] Moor still holding install files ({}); locating backend shims…",
                    format_locked_paths(&locked)
                ),
            );
            let shim = venv_moor(install_root);
...
fn install_lock_probe_paths(install_root: &Path) -> Vec<PathBuf> {
    let mut paths = vec![venv_moor(install_root)];
...
/// Find processes running the exact `venv\Scripts\moor.exe` shim for this
/// installation. Windows image names are case-insensitive and the desktop is
/// also Moor.exe, so matching by image name alone is unsafe.
#[cfg(windows)]
...
async fn run_streamed(
    ...
    // the same rule about pipe EOF: `moor update` is precisely the shape that
...
/// Path to the venv moor shim under an install root, regardless of existence.
fn venv_moor(install_root: &Path) -> PathBuf {
    if cfg!(target_os = "windows") {
        install_root.join("venv").join("Scripts").join("moor.exe")
    } else {
        install_root.join("venv").join("bin").join("moor")
    }
}

/// Resolve the moor CLI to drive. Prefer the venv shim in the install we
/// just updated; fall back to `moor` on PATH.
fn resolve_moor(install_root: &Path) -> Option<PathBuf> {
    let shim = venv_moor(install_root);
    if shim.exists() {
        return Some(shim);
    }
    // PATH fallback. which-style probe via env, kept dependency-free.
    let exe = if cfg!(target_os = "windows") { "moor.exe" } else { "moor" };
...
fn update_child_env(install_root: &Path) -> Vec<(String, OsString)> {
    ...
    let mut envs = vec![(
        "MOOR_HOME".to_string(),
        moor_home.as_os_str().to_os_string(),
    )];
    // `moor update` is a Python CLI writing to a pipe here, so CPython
    ...
    // `moor update` child claims that SAME lock (moor_cli/update_lock.py).
    ...
    // sync with HANDOFF_PID_ENV in moor_cli/update_lock.py.
    envs.push((
        "MOOR_UPDATE_HANDOFF_PID".to_string(),
        OsString::from(std::process::id().to_string()),
    ));
    if let Some(path) = path_with_prepended_entries(&[
        moor_home.join("node").join("bin"),
...
async fn install_macos_app_update(
    ...
    let rebuilt_app = crate::bootstrap::resolve_moor_desktop_app(install_root).ok_or_else(|| {
        anyhow!(
    ...
    let tmp = PathBuf::from(format!("{}.moor-update-new", target_app.display()));
    let old = PathBuf::from(format!("{}.moor-update-old", target_app.display()));
    ...
    let ditto = Command::new("/usr/bin/ditto")
        .arg(&rebuilt_app)
        .arg(&tmp)
        .current_dir(crate::paths::moor_home())
        .status()
        .await
...
mod tests {
    ...
    #[test]
    fn venv_moor_is_under_install_root() {
        let root = Path::new("/x/moor-agent");
        let shim = venv_moor(root);
    ...
    fn update_child_env_forces_unbuffered_python() {
        let envs = update_child_env(Path::new("/x/moor-agent"));
        assert!(
            envs.iter()
    ...
    fn update_child_env_names_our_pid_for_the_lock_handoff() {
        let envs = update_child_env(Path::new("/x/moor-agent"));
        assert!(
            envs.iter().any(|(k, v)| k == "MOOR_UPDATE_HANDOFF_PID"
                && v.to_str() == Some(std::process::id().to_string().as_str())),
            "the moor update child claims the same marker we hold; without our pid \
    ...
    fn lock_probe_paths_include_desktop_app_payload() {
        ...
        assert!(
            probes.iter().any(|p| p == &venv_moor(root)),
    ...
    fn locked_paths_ignores_missing_payloads() {
        let root = Path::new("/nonexistent/moor-agent");
        let probes = install_lock_probe_paths(root);
    ...
    fn same_windows_path_accepts_case_only_difference() {
        assert!(same_windows_path(
            Path::new(r"C:\Users\tester\.moor\moor-agent\venv\scripts\MOOR.EXE"),
            Path::new(r"c:\users\tester\.moor\moor-agent\venv\Scripts\moor.exe"),
        ));
    }
    ...
    fn same_windows_path_rejects_desktop_binary() {
        assert!(!same_windows_path(
            Path::new(r"C:\Users\tester\.moor\moor-agent\apps\desktop\Moor.exe"),
            Path::new(r"C:\Users\tester\.moor\moor-agent\venv\Scripts\moor.exe"),
        ));
    }
    ...
    fn update_marker_guard_writes_then_removes_on_drop() {
        ...
        let marker = dir.join(".moor-update-in-progress");
    ...
    fn acquire_refuses_while_a_live_updater_owns_the_marker() {
        ...
        // dashboard `moor update` and install-mode bootstrap mutate one tree
    ...
    fn acquire_adopts_a_marker_prewritten_with_our_own_pid() {
        // #74761: desktop writeUpdateMarker(moorHome, child.pid) races ahead
    ...
    // checkout's `moor update` reads it as a live foreign update and exits
    ...
    fn missing_or_garbage_marker_never_heals() {
        ...
        let garbage = dir.join(".moor-update-in-progress");
    ...
    fn heal_end_to_end_marker_lifecycle() {
        ...
        // own exit 2, e.g. a genuinely still-running Moor, stays terminal).
        assert!(!should_heal_self_marker_refusal(
            Some(UPDATE_EXIT_CONCURRENT),
            &marker
        ));
    ...
    async fn swap_installs_new_bundle_and_cleans_up() {
        ...
        let tmp = base.join("Moor.app.moor-update-new");
        let old = base.join("Moor.app.moor-update-old");
    ...
    async fn swap_failure_never_leaves_target_missing() {
        ...
        let target = base.join("Moor.app");
        let tmp = base.join("Moor.app.moor-update-new"); // intentionally absent
        let old = base.join("Moor.app.moor-update-old");
        write_marker(&target, "OLD");
    ...
    async fn swap_rolls_back_when_install_step_fails() {
        ...
        let target = base.join("Moor.app");
        let tmp = base.join("Moor.app.moor-update-new"); // absent
        let old = base.join("Moor.app.moor-update-old");
        write_marker(&target, "OLD");
