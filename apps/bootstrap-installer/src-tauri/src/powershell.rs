...
/// read side does not see EOF until the last of them closes it. `moor update`
...
/// stranded with it.
///
/// Same bound `Invoke-MoorStep` grew in `scripts/desktop-update/windows.ps1`
...
///
/// `moor_home_override` propagates to the child as $MOOR_HOME so the
...
pub async fn run_script(
    ...
    if let Some(cwd) = stable_script_cwd(script_path, moor_home_override) {
        cmd.current_dir(cwd);
    }

    if let Some(home) = moor_home_override {
...
fn stable_script_cwd<'a>(script_path: &'a Path, moor_home_override: Option<&'a str>) -> Option<&'a Path> {
    if let Some(home) = moor_home_override {
...
"#;
    ...
    #[test]
    fn stable_script_cwd_prefers_existing_moor_home() {
    ...
    /// child. `moor update` is this shape -- the Electron build alone is
    /// megabytes.
    #[cfg(unix)]
    #[tokio::test]
