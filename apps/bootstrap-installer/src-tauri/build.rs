...
fn main() {
    ...
    // local checkout hasn't pushed). Set MOOR_BUILD_PIN_COMMIT to bake an
    // immutable commit pin for reproducible/release installers.
    //
    // Commit pin resolution:
    //   - MOOR_BUILD_PIN_COMMIT, if set and non-empty. Accepts a SHA, tag,
    ...
    //   1. MOOR_BUILD_PIN_BRANCH, if set and non-empty.
    ...
    if let Some(c) = &commit {
        ...
        println!(
            "cargo:warning=moor-bootstrap: pinning to commit {}",
            short(c)
        );
    }
    if let Some(b) = &branch {
        ...
        match &commit {
            Some(_) => println!("cargo:warning=moor-bootstrap: pinning to branch {b}"),
            None => println!(
                "cargo:warning=moor-bootstrap: following branch {b} HEAD (no commit pin; \
    ...
    if commit.is_none() && branch.is_none() {
        ...
        // build environment.
        println!(
            "cargo:warning=moor-bootstrap: no pin resolved at build time; binary will fail at runtime without MOOR_SETUP_DEV_REPO_ROOT or runtime args"
        );
    }
    ...
    // MOOR_BUILD_PIN_COMMIT resolves a moving ref (tag/branch) to a SHA, a
    ...
    let attrs = {
        let manifest = include_str!("moor-setup.manifest");
...
fn resolve_commit_pin() -> Option<String> {
    ...
    // asks for one via MOOR_BUILD_PIN_COMMIT. With no env var, we return
    // None and the installer follows the branch HEAD at install time.
    let requested = std::env::var("MOOR_BUILD_PIN_COMMIT").ok()?;
    ...
    panic!(
        "MOOR_BUILD_PIN_COMMIT={requested:?} could not be resolved to a commit \
...
fn resolve_branch_pin() -> Option<String> {
    if let Ok(v) = std::env::var("MOOR_BUILD_PIN_BRANCH") {
        if !v.trim().is_empty() {
