...
export const $moorHome = atom<string | null>(null)
...
export async function initialize(): Promise<void> {
  ...
  if (fake) {
    unlisten = () => {}
    $logPath.set('~/.moor/logs/bootstrap-installer.log')
    $moorHome.set('~/.moor')
  ...
  try {
    const [logPath, moorHome, mode] = await Promise.all([
      invoke<string>('get_log_path'),
      invoke<string>('get_moor_home'),
      invoke<AppMode>('get_mode')
    ])

    $logPath.set(logPath)
    $moorHome.set(moorHome)
    $mode.set(mode)
  } catch (err) {
  ...
  unlisten = await listen<BootstrapEvent>('bootstrap', (event) => {
    ...
    switch (payload.type) {
      ...
      case 'complete':
        ...
        // Install: show the "launch Moor" success screen. Update: this is a
...
export async function startUpdate(): Promise<void> {
  ...
  // Update is driven by the desktop handing off (moor-setup.exe --update);
...
export async function launchMoorDesktop(): Promise<void> {
  ...
  await invoke('launch_moor_desktop', { installRoot })
}
...
const FAKE_INSTALL_STAGES: FakeStage[] = [
  ...
  { name: 'repo', title: 'Moor repository' },
