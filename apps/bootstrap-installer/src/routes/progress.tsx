...
export default function ProgressScreen({ bootstrap }: ProgressProps) {
  ...
  const title = bootstrap.status === 'completed' ? 'Done' : isUpdate ? 'Updating Moor' : 'Setting up Moor Agent'

  const description = isUpdate
    ? 'Moor is updating to the latest version — this only takes a moment.'
  ...
  return (
    <div className="moor-fade-in flex h-full flex-col">
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        {showLogs && (
          <div className="flex w-1/2 flex-col border-l border-(--stroke-moor)">
            <div className="flex shrink-0 items-center justify-between border-b border-(--stroke-moor) px-3 py-2 text-xs">
      ...
      <div className="flex shrink-0 items-center justify-between border-t border-(--stroke-moor) px-6 py-3">
        <button
