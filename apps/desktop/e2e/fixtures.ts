 ...
 *     (electron → moor serve → provider → inference → renderer) is
 ...
 * `electron-builder --dir` step and matches `moor desktop --source`. The
...
export function createSandbox(prefix: string): Sandbox {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), `moor-e2e-${prefix}-${Math.random()}`))
  const moorHome = path.join(root, 'moor-home')
 ...
 */
export function writeEnvFile(moorHome: string, apiKey = 'e2e-mock-key'): void {
 ...
 */
function writeEmptyConfig(moorHome: string): void {
  const configPath = path.join(moorHome, 'config.yaml')
 ...
 *
 * Key env vars:
 *  - MOOR_HOME → sandbox moor-home (isolated config/sessions)
 *  - MOOR_DESKTOP_USER_DATA_DIR → sandbox electron-user-data
 *  - MOOR_DESKTOP_IGNORE_EXISTING=1 → don't pick up `moor` from PATH
 *    (we want the dev checkout at REPO_ROOT)
 *  - MOOR_DESKTOP_MOOR_ROOT → REPO_ROOT (dev checkout resolution)
 *  - MOOR_DESKTOP_APP_NAME → unique-ish per test (avoids single-instance lock)
 ...
 *
 * @param sandbox  - isolated MOOR_HOME + userData
 * @param env      - the process environment (already has MOOR_HOME etc.)
...
export async function setupNoProvider(): Promise<NoProviderFixture> {
  ...
  writeEmptyConfig(sandbox.moorHome)
...
export interface DeadBackendOptions {
  /**
   * When true, inject a fake boot error via MOOR_DESKTOP_BOOT_FAKE_ERROR
 ...
 * nothing listens on). By default the backend still boots (`moor serve`
...
export async function setupDeadBackend(options: DeadBackendOptions = {}): Promise<DeadBackendFixture> {
  ...
  const configPath = path.join(sandbox.moorHome, 'config.yaml')
  fs.writeFileSync(
    configPath,
...
function resolvePackagedBinaryPath(): string {
  ...
  return path.join(RELEASE_ROOT, 'linux-unpacked', 'moor')
}
 ...
 * MOOR_HOME + userData, unique app name) as the dev-mode fixtures.
 *
...
export async function setupPackagedApp(): Promise<PackagedAppFixture> {
  ...
  const env = buildAppEnv(sandbox, {
    ...
    MOOR_DESKTOP_BOOT_FAKE: '1',
    MOOR_DESKTOP_BOOT_FAKE_STEP_MS: '120',
  })

  // Clear dev-server + moor-root overrides — the packaged binary
  // should use its own bundled renderer, not the dev checkout.
  delete (env as Record<string, string | undefined>).MOOR_DESKTOP_DEV_SERVER
  delete (env as Record<string, string | undefined>).MOOR_DESKTOP_MOOR
  delete (env as Record<string, string | undefined>).MOOR_DESKTOP_MOOR_ROOT
