 ...
 * packaged binary) with a real `moor serve` backend pointed at a mock
 * inference server. The full chain is exercised:
 *
 *   electron → moor serve (python) → mock provider → renderer
 *
...
test.describe('dev-mode boot with mock backend', () => {
  test('window opens with Moor title', async () => {
    const title = await fixture!.page.title()
    expect(title).toContain('Moor')
  })
  ...
  test('the preload bridge reaches the renderer', async () => {
    const bridge = await fixture!.page.evaluate(() => {
      const desktop = (window as unknown as { moorDesktop?: Record<string, unknown> }).moorDesktop
  ...
  test('backend boots and app becomes ready', async () => {
    ...
    // electron starts → moor serve is spawned → WS connects → config
