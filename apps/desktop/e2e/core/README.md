...
- **C2 transcript integrity** — `transcript-integrity.spec.ts`: one real app +
  one real `moor serve`, only the LLM faked (`provider.ts`, scripted per turn
...
- **C5 boot / process lifecycle** — `boot-lifecycle.spec.ts`: interactive
  ...
  child → zero sandbox processes (/proc census on the sandbox `MOOR_HOME`,
...
- **Session lineage** — `lineage-sidebar.spec.ts`: a branch child is born
  ...
  compression rows (`moor chat -q` with `compression.in_place: false` on the
  same `MOOR_HOME`, compacting against the fake provider), then the Desktop
...
- **Remote topology** — `remote-topology.spec.ts`: the whole app on a remote
  `moor serve` (`MOOR_DESKTOP_REMOTE_URL` + token): a client-only image is
...
sandboxed `HOME`/`MOOR_HOME`/user-data per test, all `MOOR_*` and
...
`MOOR_E2E_CORE_ROOT` picks the sandbox parent dir (default: OS tmpdir);
`MOOR_E2E_CORE_KEEP=1` keeps sandboxes for post-mortem.
