...
// not reachable in this single-local-source rig: `window.moorDesktop` is a
...
// `src/plugins/moor-bots/screen-connection.test.ts`; the second test here
...
async function seedBot(moorHome: string, mockUrl: string, name: string): Promise<void> {
  const dir = path.join(moorHome, 'profiles', name)
  fs.mkdirSync(dir, { recursive: true })
...
const PROFILE_KEY = '/e2e/alpha/.moor'
...
test.beforeAll(async () => {
  ...
  await seedBot(sandbox.moorHome, mock.url, 'alpha')
