 ...
 * AIAgent loop, using the E2E mock provider configured in `moorHome`.
 *
...
export class RealSessionBuilder {
  ...
  private constructor(moorHome: string) {
  ...
  static async start(moorHome: string): Promise<RealSessionBuilder> {
    const builder = new RealSessionBuilder(moorHome)
