 ...
 * second gateway, "Homelab", is a REAL second `moor serve` this spec spawns
 * with its own MOOR_HOME, profiles and session token, registered in the v2
...
function findMoorBinary(): string {
  const venv = path.join(REPO_ROOT, '.venv', 'bin', 'moor')

  if (fs.existsSync(venv)) {
    return venv
  }

  const result = spawnSync('which', ['moor'], { encoding: 'utf8' })

  if (result.status === 0 && result.stdout.trim()) {
    return result.stdout.trim()
  }

  throw new Error('moor binary not found: create the repo venv (uv sync) or put moor on PATH')
}
...
/**
 * Spawn a second, fully real `moor serve` as the remote gateway. Its
 * session token is pinned through MOOR_DASHBOARD_SESSION_TOKEN so the
...
async function startRemoteGateway(root: string, mockUrl: string, profiles: string[]): Promise<RemoteGateway> {
  ...
  const child: ChildProcess = spawn(
    ...
    ['serve', '--host', '127.0.0.1', '--port', String(port), '--skip-build'],
    {
      ...
      env: {
        ...process.env,
        MOOR_HOME: home,
        MOOR_DASHBOARD_SESSION_TOKEN: REMOTE_TOKEN,
      },
  ...
  while (Date.now() < deadline) {
    if (child.exitCode !== null) {
      throw new Error(`remote moor serve exited early (${child.exitCode}):\n${log}`)
    }
  ...
  if (Date.now() >= deadline) {
    throw new Error(`remote moor serve never became ready:\n${log}`)
  }
...
test.describe('fleet profile rail — two registered gateways', () => {
  ...
  test.beforeAll(async () => {
    ...
    seedProfiles(sandbox.moorHome, ['research'])
