...
async function setupSeededMockBackend(): Promise<MockBackendFixture> {
  ...
  fs.appendFileSync(
    path.join(sandbox.moorHome, 'config.yaml'),
    '\ncompression:\n  threshold_tokens: 1\n',
    'utf8',
  )
  writeEnvFile(sandbox.moorHome)
  const builder = await RealSessionBuilder.start(sandbox.moorHome)
...
test('live verify-on-stop continuations stay out of the transcript', async ({}, testInfo) => {
  ...
  writeMockProviderConfig(sandbox.moorHome, mock.url)
  fs.appendFileSync(path.join(sandbox.moorHome, 'config.yaml'), '\nagent:\n  verify_on_stop: true\n', 'utf8')
