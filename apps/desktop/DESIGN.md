...
2. **Borderless elevation for floating panels.** Overlays float on
   `shadow-moor` + a `--stroke-moor` hairline, not thick framed boxes. In-panel
...
3. **One primitive per concern.** One `Button`, one set of control variants,
   ...
   fork.
4. **Tokens, not literals.** Reference CSS vars (`--ui-*`, `--shadow-moor`,
...
```
shadow-moor           /* downward-weighted, layered contact→ambient falloff */
border-(--stroke-moor) /* currentColor hairline, theme-adaptive */
```
...
normal behavior. The tray's **Show Moor** restores hidden windows;
**Quit Moor** keeps the ordinary active-work confirmation and teardown.
...
| `--stroke-moor` | the overlay hairline (pairs with `shadow-moor`) |
...
- **`BrandMark`** (`src/components/brand-mark.tsx`) is the brand glyph — the
  `moor-girl` mark on a fixed tile, theme-aware: black girl on white in light
  ...
  + squircle backgrounds (`assets/moor-girl-*.svg` × `assets/backgrounds/`).
...
- [ ] Reuse a primitive (`Button`, `SearchField`, `SegmentedControl`,
      ...
      forking one?
- [ ] Tokens (`--ui-*`, `shadow-moor`, `--stroke-moor`) — zero raw colors /
      one-off shadows?
...
- [ ] Overlay uses `shadow-moor` + `border-(--stroke-moor)`, no hard border?
