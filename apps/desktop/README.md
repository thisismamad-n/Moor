...
Prebuilt installers are built and distributed via [the Moor Desktop website.](https://hermes-agent.nousresearch.com/).
...
- **Moor Agent** runs as a headless `moor serve` process and exposes the
...
Boot logs land in `MOOR_HOME/logs/desktop.log` (includes backend output and recent Python tracebacks) — check it first if the app reports a boot failure.
...
```powershell
# Force a clean first-launch setup
Remove-Item "$env:LOCALAPPDATA\moor\moor-agent\.moor-bootstrap-complete"
# Rebuild a broken Python venv
Remove-Item -Recurse -Force "$env:LOCALAPPDATA\moor\moor-agent\venv"
```

> The default Moor home on Windows is `%LOCALAPPDATA%\moor`. Set the `MOOR_HOME` env var if you've relocated it.
