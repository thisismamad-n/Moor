...
test('chooseUpdaterArgs: gentle --update when both updater runtime files exist', () => {
  assert.deepEqual(chooseUpdaterArgs({ hasBootstrapMarker: true, hasVenvMoor: true, hasVenvPython: true }, 'main'), [
    '--update',
    '--branch',
    'main'
  ])
...
test('chooseUpdaterArgs: marker-only install uses --repair when the venv is gone', () => {
  assert.deepEqual(
    chooseUpdaterArgs({ hasBootstrapMarker: true, hasVenvMoor: false, hasVenvPython: false }, 'main'),
    ['--repair', '--branch', 'main']
  )
})

test('chooseUpdaterArgs: partial updater runtimes use --repair', () => {
  assert.deepEqual(chooseUpdaterArgs({ hasBootstrapMarker: true, hasVenvMoor: false, hasVenvPython: true }, 'main'), [
    '--repair',
    '--branch',
    'main'
  ])
  assert.deepEqual(chooseUpdaterArgs({ hasBootstrapMarker: true, hasVenvMoor: true, hasVenvPython: false }, 'main'), [
    '--repair',
    '--branch',
    'main'
  ])
...
test('chooseUpdaterArgs: passes the branch through unchanged in both modes', () => {
  assert.deepEqual(
    chooseUpdaterArgs({ hasBootstrapMarker: false, hasVenvMoor: true, hasVenvPython: true }, 'release/1.2'),
    ['--update', '--branch', 'release/1.2']
  )
  assert.deepEqual(
    chooseUpdaterArgs({ hasBootstrapMarker: false, hasVenvMoor: false, hasVenvPython: false }, 'release/1.2'),
...
test('resolveVenvMoorCommand: returns null off Windows', () => {
  const deps = makeDeps({ isWindows: false })

  assert.equal(resolveVenvMoorCommand('/root/venv/Scripts/moor.exe', [], deps), null)
})

test('resolveVenvMoorCommand: returns null for a .cmd/.bat script command', () => {
  const deps = makeDeps({ isCommandScript: () => true })

  assert.equal(resolveVenvMoorCommand('/root/venv/Scripts/moor.cmd', [], deps), null)
})

test('resolveVenvMoorCommand: returns null when the basename is not moor/moor.exe', () => {
  const deps = makeDeps()

  assert.equal(resolveVenvMoorCommand('/root/venv/Scripts/python.exe', [], deps), null)
})

test('resolveVenvMoorCommand: returns null when the parent dir is not Scripts', () => {
  const deps = makeDeps()

  assert.equal(resolveVenvMoorCommand('/root/venv/bin/moor.exe', [], deps), null)
})

test('resolveVenvMoorCommand: returns null when the venv python does not exist on disk', () => {
  const deps = makeDeps({ fileExists: () => false })

  assert.equal(resolveVenvMoorCommand('/root/venv/Scripts/moor.exe', [], deps), null)
})

test('resolveVenvMoorCommand: probes the venv python before trusting it (returns null on failed probe)', () => {
  let probed = false

  const deps = makeDeps({
    canImportMoorCli: (python: string) => {
      probed = true
  ...
  const result = resolveVenvMoorCommand('/root/venv/Scripts/moor.exe', ['serve'], deps)
...
test('resolveVenvMoorCommand: returns the resolved python backend descriptor when the probe passes', () => {
  const deps = makeDeps()
  const result = resolveVenvMoorCommand('/root/venv/Scripts/moor.exe', ['serve', '--port', '0'], deps)
...
test('resolveVenvMoorCommand: is case-insensitive on moor.exe and the Scripts dir name', () => {
  const deps = makeDeps()

  assert.ok(resolveVenvMoorCommand('/root/venv/Scripts/MOOR.EXE', [], deps))
  assert.ok(resolveVenvMoorCommand('/root/venv/SCRIPTS/moor.exe', [], deps))
})
