...
export const ZOOM_STORAGE_KEY = 'moor:desktop:zoomLevel'
 ...
 * but forgot to emit 'moor:zoom:changed', leaving the control stuck at 100%.
...
export function applyZoomLevel(webContents, level) {
  ...
  webContents.send('moor:zoom:changed', { level: clamped, percent: zoomLevelToPercent(clamped) })
 ...
 * only learns of changes through `moor:zoom:changed`, which never fired —
