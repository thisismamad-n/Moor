 ...
 *   - the browser sees only a minimal "return to Moor" page.
 */
...
const DONE_HTML =
  ...
  '<p>You can close this window and return to Moor.</p>' +
...
export function registerMcpOauthCallbackIpc() {
  ...
  ipcMain.handle('moor:mcp-oauth:listen', async () => {
  ...
  ipcMain.handle('moor:mcp-oauth:wait', async (_event, id, timeoutMs) => {
  ...
  ipcMain.handle('moor:mcp-oauth:cancel', (_event, id) => {
    dispose(String(id || ''))
