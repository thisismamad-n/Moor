/**
 * windows-moor-path.ts
 *
 * Pure, dependency-injected pieces of Windows `moor` resolution pulled out
 * of main.ts's findOnPath(), handOffWindowsBootstrapRecovery(), and
 * unwrapWindowsVenvMoorCommand(). Each of the three functions here pins one
 ...
 *      FIRST, so an extensionless Git-Bash `moor` shim shadowed the real
 *      moor.cmd/moor.exe; the shim then failed the --version probe and
 ...
 *      venv\Scripts\moor.exe (the console-script shim, written at the END
 ...
 *   3. resolveVenvMoorCommand() — unwrapWindowsVenvMoorCommand() returned
 ...
 * extensionless file (e.g. a Git-Bash shell-script shim named `moor`) must
 * not shadow `moor.cmd`/`moor.exe`. The empty entry is kept LAST so
 ...
 * signals (venv python interpreter, venv moor shim, bootstrap-complete
 * marker) — gating on just the moor.exe console-script shim alone is the
...
export interface ResolveVenvMoorCommandDeps {
  isWindows: boolean
...
/**
 * If `command` is a Windows venv `moor`/`moor.exe` console-script shim
 * (i.e. `<venvRoot>/Scripts/moor(.exe)`), resolve it to the underlying
 * venv python invoked as `python -m moor_cli.main <backendArgs>` — but
 * ONLY after smoke-testing that interpreter with canImportMoorCli(). A
 * venv whose update died mid-`pip install` still has python.exe + moor.exe
 ...
 *
 * Returns null when `command` is not a venv moor shim, the underlying
...
export function resolveVenvMoorCommand(
  ...
  if (!/^moor(?:\.exe)?$/i.test(basename(resolved))) {
    return null
  }
  ...
  return {
    label: `existing Moor Python at ${python}`,
    command: python,
