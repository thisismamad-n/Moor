 ...
 *      and Moor launcher. Marker-only or partial runtimes must use --repair;
 ...
 * venv Python interpreter and the Moor launcher that drives `moor update`.
...
export function chooseUpdaterArgs(signals: BootstrapRecoverySignals, branch: string): string[] {
  const canRunUpdater = signals.hasVenvMoor && signals.hasVenvPython
