...
test('the native preload exposes routed peer opening and default preference events with unsubscribe', async () => {
  ...
  assert.deepEqual(electron.ipcRenderer.invoke.mock.lastCall, ['moor:window:openInstance', route])
  await bridge.openWindow()
  assert.deepEqual(electron.ipcRenderer.invoke.mock.lastCall, ['moor:window:openInstance', undefined])
  await bridge.profile.setDefault(route)
  assert.deepEqual(electron.ipcRenderer.invoke.mock.lastCall, ['moor:profile:default:set', route])
  await bridge.profile.getDefault()
  assert.deepEqual(electron.ipcRenderer.invoke.mock.lastCall, ['moor:profile:default:get'])
  bridge.setF12ShortcutActive(true)
  assert.equal(electron.ipcRenderer.send.mock.lastCall?.[0], 'moor:f12ShortcutActive')
  ...
  assert.equal(shortcutChannel, 'moor:f12-shortcut')
  ...
  assert.equal(channel, 'moor:profile:default:changed')
  listener({}, route)
  listener({}, null)
