...
export function registerHudIpc({
  ...
  ipcMain.on('moor:hud:native-drag', event => {
    event.returnValue = hudWindowing().move === 'native-drag'
  })

  ipcMain.on('moor:hud:windowing', event => {
  ...
  ipcMain.on('moor:hud:workspace-transfer', (event, transferring) => {
  ...
  ipcMain.handle('moor:hud:frost', (_event, showing) => {
  ...
  ipcMain.on('moor:hud:begin-move', event => {
    const hudWindow = getHudWindow()
  ...
  ipcMain.on('moor:hud:end-move', event => {
    const hudWindow = getHudWindow()
  ...
  ipcMain.handle('moor:hud:reset-layout', event => {
    const hudWindow = getHudWindow()
