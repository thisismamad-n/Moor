// Backend subcommand routing for the desktop-managed Moor process.
//
// The desktop app launches its own headless backend via `moor serve` — it
...
// app hasn't updated yet, or an older `moor` resolved from PATH) only knows
 ...
 * @param {string} [profile] optional Moor profile to pin via `--profile`.
 */
 ...
 * `-m moor_cli.main` and any `--profile <name>`). Returns a copy; if there is
...
/**
 * True when a runtime's `moor_cli/subcommands/dashboard.py` source registers
