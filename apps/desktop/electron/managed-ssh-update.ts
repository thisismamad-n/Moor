 ...
 * wire protocol:
 *
 *   gate dials -> drain every captured scope -> detached `moor update`
...
function buildPosixManagedUpdateLaunch(target: RemoteUpdateTarget, correlationId: string): string {
  ...
  const home = validateRemoteValue(target.moorHome, 'Moor home')
  const moorPath = validateRemoteValue(target.moorPath, 'launcher path')
  ...
  const launcherWord = expandRemotePath(moorPath)

  const updateCommand =
    `env MOOR_HOME=${homeWord} ` +
    `MOOR_UPDATE_CORRELATION_ID=${shq(correlation)} ` +
    'MOOR_UPDATE_ORIGIN_PROFILE=default ' +
    `MOOR_UPDATE_ORIGIN_HOME=${homeWord} ` +
    `MOOR_UPDATE_OUTPUT_PATH=${outputWord} ` +
...
function buildWindowsManagedUpdateLaunch(target: RemoteUpdateTarget, correlationId: string): string {
  ...
  const wrapper = [
    ...
    `$env:MOOR_HOME=${psLiteral(home)}`,
    `$env:MOOR_UPDATE_CORRELATION_ID=${psLiteral(correlation)}`,
    '$env:MOOR_UPDATE_ORIGIN_PROFILE="default"',
    `$env:MOOR_UPDATE_ORIGIN_HOME=${psLiteral(home)}`,
    `$env:MOOR_UPDATE_OUTPUT_PATH=${psLiteral(outputPath)}`,
    ...
    `$env:MOOR_UPDATE_WINDOWS_DETACHED=${psLiteral(correlation)}`,
    `$env:MOOR_UPDATE_TAURI_OUTCOME_PATH=${psLiteral(statusPath)}`,
    `$env:MOOR_UPDATE_TAURI_READY_PATH=${psLiteral(readyPath)}`,
    ...
    `& ${psLiteral(moorPath)} update --yes *>> ${psLiteral(outputPath)}`,
    '$rc=$LASTEXITCODE',
...
marker_path=install_root/'.moor-update-in-progress'
...
async function runManagedSshUpdate<TScope extends ManagedSshScope>(
  ...
  return {
    ...
    message:
      outcome === 'updated'
        ? 'Remote Moor updated and every managed SSH profile is ready.'
        : restoreOk
