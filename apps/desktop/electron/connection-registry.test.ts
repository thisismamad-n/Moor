...
test('primary SSH reuse rejects a descriptor with a different remote Moor path', async () => {
  const registry = migrateV1ToRegistry({
    mode: 'ssh',
    remote: { mode: 'ssh', host: 'build-host', remoteMoorPath: '/srv/moor', user: 'alice' },
    profiles: {}
  })
...
test('registry primary reuses a matching primary backend descriptor', () => {
  const registry = normalizeRegistry({
    ...
    connections: [
      ...
      { id: 'moor-vps', kind: 'ssh', label: 'Moor VPS', host: 'moor-vps' }
    ]
  })

  const descriptor = {
    connectionId: 'moor-vps',
    mode: 'remote' as const,
    remoteKind: 'ssh' as const,
    ssh: { host: 'moor-vps' }
  }

  assert.equal(registrySourceOwnsPrimaryBackend(registry, 'moor-vps', descriptor), true)
...
test('resolvedConnectionId keeps same-host SSH routes distinct by port, key, path, and profile', () => {
  ...
  const registry: ConnectionRegistry = {
    ...
    connections: [
      ...
      { ...base, id: 'ssh-path', label: 'SSH path', remoteMoorPath: '/opt/moor' },
  ...
  assert.equal(resolve({ ...base, remoteMoorPath: '/opt/moor' }), 'ssh-path')
...
test('roster: source profile metadata follows the connection-qualified row', () => {
  ...
  const vpsMeta = {
    display_name: 'Emma',
    ui_meta: { 'moor-bots': { title: 'Emma', shape: 'blobatar::sun', color: '#8b5cf6' } },
    has_avatar: true
  }
...
test('Apply remote preserves an existing URL identity and label without duplicates', () => {
  ...
  assert.equal(matches[0].id, 'moor-alex')
  assert.equal(matches[0].label, 'Existing gateway')
  assert.equal(matches[0].authMode, 'oauth')
  assert.equal(applied.primary, 'moor-alex')
  assert.equal(applied.lastUsed, 'moor-alex')
})
