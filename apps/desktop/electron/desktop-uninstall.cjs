 ...
 *
 *   macOS:   …/Moor.app/Contents/MacOS/Moor  → …/Moor.app
 *   Windows: …\Moor\Moor.exe                 → …\Moor  (install dir)
...
function resolveRemovableAppPath(execPath, platform, env = {}) {
  ...
  if (platform === 'darwin') {
    // …/Moor.app/Contents/MacOS/Moor → strip 3 segments to the .app
    const macOsDir = p.dirname(exe) // …/Contents/MacOS
    const contents = p.dirname(macOsDir) // …/Contents
    const appBundle = p.dirname(contents) // …/Moor.app
  ...
  if (platform === 'win32') {
    ...
    if (/[\\/]Moor$/i.test(dir) || /[\\/]hermes-desktop$/i.test(dir)) return dir
    return null
  }
...
function buildWindowsCleanupScript({ desktopPid, pythonExe, pythonPath, agentRoot, uninstallArgs, appPath, hermesHome }) {
  ...
  // a problem, but Moor install paths don't use them.
  const q = s => `"${String(s).replace(/"/g, '')}"`
  const lines = [
    '@echo off',
