...
const _REDACTIONS: Array<[RegExp, string]> = [
  [/(MOOR_DASHBOARD_SESSION_TOKEN=)(\S+)/g, '$1<redacted>'],
...
// root under a short per-user base (`~/.moor/desktop-ssh`) so even worst case
...
function defaultControlDir() {
  ...
  return path.join(os.homedir(), '.moor', 'desktop-ssh')
}
