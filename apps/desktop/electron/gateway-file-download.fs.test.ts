...
beforeEach(async () => {
  dir = await fs.promises.mkdtemp(path.join(os.tmpdir(), 'moor-download-fs-'))
})
...
test('a pre-open EEXIST collision leaves the seeded temp file and the destination untouched', async () => {
  const dest = path.join(dir, 'report.bin')
  const pinnedTemp = path.join(dir, '.moor-download-pinned.part')
  ...
  assert.deepEqual(await listing(), ['.moor-download-pinned.part', 'report.bin'])
})
