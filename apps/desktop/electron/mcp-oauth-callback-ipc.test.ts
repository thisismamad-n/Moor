...
test('listen binds a loopback listener and wait resolves with the redirect params', async () => {
  ...
  assert.match(await res.text(), /return to Moor/)
...
test('non-callback noise (favicon) does not settle the listener', async () => {
  ...
  const waitPromise = invoke('moor:mcp-oauth:wait', id, 5000) as Promise<{ code: null | string }>
...
test('cancel tears the listener down and wait reports listener not found afterwards', async () => {
  ...
  assert.equal(await invoke('moor:mcp-oauth:cancel', id), true)

  await assert.rejects(fetch(`${redirectUri}?code=x&state=s`))

  const result = (await invoke('moor:mcp-oauth:wait', id, 100)) as { error: null | string }

  assert.equal(result.error, 'listener not found')
})

test('wait times out when no callback arrives', async () => {
  const { id } = (await invoke('moor:mcp-oauth:listen')) as { id: string }

  const result = (await invoke('moor:mcp-oauth:wait', id, 1000)) as { code: null | string; error: null | string }
