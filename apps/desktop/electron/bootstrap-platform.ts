 ...
 * null to keep it enabled. `MOOR_DESKTOP_DISABLE_GPU` overrides detection
...
function detectRemoteDisplay(options: { env?: NodeJS.ProcessEnv; platform?: NodeJS.Platform } = {}) {
  ...
  if (GPU_OVERRIDE_ON.has(override)) {
    return 'override (MOOR_DESKTOP_DISABLE_GPU)'
  }
 ...
 * gateway tokens. The `moor desktop` launcher detects the session keychain
 * (or reads `desktop.password_store` from config.yaml) and bridges the value
 * in via MOOR_DESKTOP_PASSWORD_STORE.
 *
...
function resolveLinuxPasswordStore(options: { env?: NodeJS.ProcessEnv; platform?: NodeJS.Platform } = {}) {
  ...
  const requested = String(env.MOOR_DESKTOP_PASSWORD_STORE || '').trim()
  ...
  if (!LINUX_PASSWORD_STORES.has(requested)) {
    return { store: null, warning: `ignoring unknown MOOR_DESKTOP_PASSWORD_STORE value: ${requested}` }
  }
