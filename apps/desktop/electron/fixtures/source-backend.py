...
def main() -> None:
    ...
    root = Path(os.environ["MOOR_HOME"]) / "moor-agent"
    root.mkdir(parents=True)
    for name in (
        "moor_cli", "moor_platform", "pm", "agent", "tools", "gateway", "tui_gateway", "cron", "plugins",
    ):
    ...
    assert sites, "run with the prepared Moor Python dependency environment"
    wheels = temp / "wheels"
    wheels.mkdir()
    ...
    assert selected and selected.is_relative_to(Path(os.environ["MOOR_HOME"]))
    ...
    launchers = ensure_install_launchers(root, root / ".moor" / "bin")
    assert launchers
    ...
    print(json.dumps({"root": str(root), "launcher": str(root / ".moor/bin/moor"),
