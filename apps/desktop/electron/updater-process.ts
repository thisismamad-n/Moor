 ...
 *
 * The staged Tauri `moor-setup.exe` has no self-update path, so every
 ...
 * checkout instead: every `moor update` refreshes the code that drives the
 ...
 * and QUITS, the script waits it out, runs `moor update`, swaps/relaunches
 * the app, and writes .moor-update-result.json. With the app gone before
 * the update starts, the MOOR_DESKTOP_CHILD_PID reaper-exclusion dance is
 ...
 * `moor update` card — old checkouts pull the script on their next update).
 */
 ...
 *
 * The Tauri installer self-copies into MOOR_HOME on *every* platform
 * (`moor-setup.exe` on Windows, `moor-setup` elsewhere — see
 * apps/bootstrap-installer `paths::installer_dest` and
 * `bootstrap::copy_self_to_moor_home`), so finding that binary on macOS or
 ...
 * than the hand-off protocol holds the update marker, spawns `moor update`,
...
export function resolveStagedUpdaterBinary(
  ...
  const candidate = path.join(moorHome, 'moor-setup.exe')
 ...
 *
 * `copy_self_to_moor_home` deliberately no-ops during `--update`
 ...
 * for those binaries lets them acquire cleanly and run `moor update`, which
 ...
 * the installer is written to MOOR_HOME at install/repair time, making mtime
