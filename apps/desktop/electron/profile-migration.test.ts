 ...
 *      top of startMoor() in main.ts, before primaryProfileKey() is read); the
...
function baseDeps(overrides: Record<string, unknown> = {}) {
  ...
  return {
    legacyActivePath: '/home/u/.moor/active_profile',
    moorHome: '/home/u/.moor',
    profilesRoot: '/home/u/.moor/profiles',
...
test('findRunningGatewayProfiles returns [] when no pid files exist', () => {
  const fs = makeFs({ '/home/u/.moor/profiles/coder': { dir: true } })
  assert.deepEqual(
    findRunningGatewayProfiles('/home/u/.moor/profiles', ['coder'], {
      ...fs,
      isMoorProcess: () => true
    }),
    []
...
test('findRunningGatewayProfiles drops stale (non-moor) recycled PIDs', () => {
  // Two profiles have pid files, but only coder's pid is a live moor process.
  // The recycled PID at 5678 belongs to an unrelated process (e.g. Chrome).
  const fs = makeFs({
    '/home/u/.moor/profiles/coder/gateway.pid': { content: '{"pid":1234}' },
    '/home/u/.moor/profiles/writer/gateway.pid': { content: '{"pid":5678}' }
  })

  const deps = {
    ...fs,
    isMoorProcess: (pid: number) => pid === 1234
  }

  assert.deepEqual(findRunningGatewayProfiles('/home/u/.moor/profiles', ['coder', 'writer'], deps), ['coder'])
})

test('findRunningGatewayProfiles tolerates malformed pid files', () => {
  const fs = makeFs({
    '/home/u/.moor/profiles/coder/gateway.pid': { content: '{not json' }
  })
...
test('findRunningGatewayProfiles drops non-integer and non-positive PIDs', () => {
  const fs = makeFs({
    '/home/u/.moor/profiles/coder/gateway.pid': { content: '{"pid":-1}' },
    '/home/u/.moor/profiles/writer/gateway.pid': { content: '{"pid":1.5}' },
    '/home/u/.moor/profiles/extra/gateway.pid': { content: '{"pid":0}' }
  })

  assert.deepEqual(
    findRunningGatewayProfiles('/home/u/.moor/profiles', ['coder', 'writer', 'extra'], {
      ...fs,
      isMoorProcess: () => true
    }),
    []
...
test('findRunningGatewayProfiles preserves order of allProfiles', () => {
  const fs = makeFs({
    '/home/u/.moor/profiles/coder/gateway.pid': { content: '{"pid":1}' },
    '/home/u/.moor/profiles/writer/gateway.pid': { content: '{"pid":2}' }
  })

  const deps = { ...fs, isMoorProcess: () => true }
  assert.deepEqual(findRunningGatewayProfiles('/home/u/.moor/profiles', ['coder', 'writer'], deps), [
    'coder',
    'writer'
  ])
})
...
test('listProfileDirs includes default and any name passing the regex', () => {
  const fs = makeFs({
    '/home/u/.moor/profiles/default': { dir: true },
    '/home/u/.moor/profiles/coder': { dir: true },
    '/home/u/.moor/profiles/writer': { dir: true }
  })
...
test('listProfileDirs skips files (not directories) and invalid names', () => {
  const fs = makeFs({
    '/home/u/.moor/profiles/default': { dir: true },
    '/home/u/.moor/profiles/some-file.txt': { dir: false },
    '/home/u/.moor/profiles/UPPERCASE': { dir: true } // regex rejects
  })
...
test('decideMigration suppresses write when best is default (single-profile fallback)', () => {
  ...
  // default-primary and we leave it alone. Default's DB is $MOOR_HOME/state.db,
  // not profiles/default/state.db.
  const deps = baseDeps()

  const d = decideMigration(null, [], ['default', 'coder'], deps, p => (p.endsWith('/.moor/state.db') ? 99 : 50))
...
test('migrateActiveProfileIfMissing is a no-op when a user-selected preference file exists', () => {
  ...
  const fs = makeFs({
    ...
    '/home/u/.moor/profiles/coder': { dir: true },
    '/home/u/.moor/profiles/writer': { dir: true },
    '/home/u/.moor/profiles/writer/state.db': { size: 400 * 1024 * 1024, mtime: NOW - 86_400_000 }
  })
...
test('migrateActiveProfileIfMissing writes legacy choice with no _migrated flag', () => {
  ...
  const fs = makeFs({
    '/home/u/.moor/active_profile': { content: 'coder' },
    '/home/u/.moor/profiles/coder': { dir: true },
    '/home/u/.moor/profiles/writer': { dir: true }
  })
...
test('migrateActiveProfileIfMissing writes heuristic choice with _migrated=true', () => {
  ...
  const fs = makeFs({
    '/home/u/.moor/profiles/coder/state.db': { size: 50 * 1024 * 1024, mtime: NOW - 86_400_000 },
    '/home/u/.moor/profiles/writer/state.db': { size: 200 * 1024 * 1024, mtime: NOW - 86_400_000 }
  })
...
test('migrateActiveProfileIfMissing is a no-op for single-profile (default-only) installs', () => {
  ...
  // Production default DB is ~/.moor/state.db, not profiles/default/state.db.
  let written: unknown = null

  const fs = makeFs({
    '/home/u/.moor/state.db': { size: 10 * 1024 * 1024, mtime: NOW - 86_400_000 }
  })
...
test('migrateActiveProfileIfMissing prefers a single running gateway over heuristics', () => {
  ...
  const fs = makeFs({
    '/home/u/.moor/profiles/coder/gateway.pid': { content: '{"pid":42}' },
    '/home/u/.moor/profiles/writer/state.db': { size: 500 * 1024 * 1024, mtime: NOW - 86_400_000 }
  })

  const deps = baseDeps({
    ...fs,
    isMoorProcess: (pid: number) => pid === 42,
    writeJson: (_p: string, payload: unknown) => {
      written = payload
    }
  })
...
// ---------------------------------------------------------------------------
// Production layout: default is ~/.moor, not ~/.moor/profiles/default
// ---------------------------------------------------------------------------

test('profileStateDbPath puts default at moorHome, named under profilesRoot', () => {
  assert.equal(profileStateDbPath('default', '/home/u/.moor', '/home/u/.moor/profiles'), '/home/u/.moor/state.db')
  assert.equal(
    profileStateDbPath('conduit', '/home/u/.moor', '/home/u/.moor/profiles'),
    '/home/u/.moor/profiles/conduit/state.db'
  )
})

test('profileGatewayPidPath puts default at moorHome', () => {
  assert.equal(
    profileGatewayPidPath('default', '/home/u/.moor', '/home/u/.moor/profiles'),
    '/home/u/.moor/gateway.pid'
  )
  assert.equal(
    profileGatewayPidPath('coder', '/home/u/.moor', '/home/u/.moor/profiles'),
    '/home/u/.moor/profiles/coder/gateway.pid'
  )
})
...
test('findRunningGatewayProfiles sees default gateway.pid at moorHome', () => {
  const fs = makeFs({
    '/home/u/.moor/gateway.pid': { content: '{"pid":99}' },
    '/home/u/.moor/profiles/coder/gateway.pid': { content: '{"pid":11}' }
  })

  assert.deepEqual(
    findRunningGatewayProfiles('/home/u/.moor/profiles', ['default', 'coder'], {
      ...fs,
      moorHome: '/home/u/.moor',
      isMoorProcess: pid => pid === 99
    }),
    ['default']
  )
})
...
test('migrateActiveProfileIfMissing does not pin a tiny named profile over a large default DB', () => {
  ...
  // ~/.moor/profiles/<name>, never scored ~/.moor/state.db, and wrote
  // { profile: named, _migrated: true }.
  let written: unknown = null

  const fs = makeFs({
    '/home/u/.moor/profiles/conduit': { dir: true },
    '/home/u/.moor/state.db': { size: 409 * 1024 * 1024, mtime: NOW - 86_400_000 },
    '/home/u/.moor/profiles/conduit/state.db': { size: 2 * 1024 * 1024, mtime: NOW - 60_000 }
  })
...
test('migrateActiveProfileIfMissing still pins a named profile that actually beats default', () => {
  ...
  const fs = makeFs({
    '/home/u/.moor/profiles/work': { dir: true },
    '/home/u/.moor/state.db': { size: 5 * 1024 * 1024, mtime: NOW - 86_400_000 },
    '/home/u/.moor/profiles/work/state.db': { size: 200 * 1024 * 1024, mtime: NOW - 86_400_000 }
  })
...
test('migrateActiveProfileIfMissing does not pin default when only default gateway is running', () => {
  ...
  const fs = makeFs({
    '/home/u/.moor/gateway.pid': { content: '{"pid":7}' },
    '/home/u/.moor/state.db': { size: 10 * 1024 * 1024, mtime: NOW - 86_400_000 }
  })

  const deps = baseDeps({
    ...fs,
    isMoorProcess: (pid: number) => pid === 7,
    writeJson: (_p: string, payload: unknown) => {
      written = payload
    }
  })
