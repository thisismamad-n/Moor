 ...
 *
 * The Python side (moor_cli/plugin_compat.py) statically scans the user's enabled external plugins on
 * every CLI/gateway/TUI start and writes MOOR_HOME/.plugin-compat-report.json when any plugin imports a
...
export function readReport(moorHome: string): PluginCompatReport | null {
  try {
    const raw = fs.readFileSync(path.join(moorHome, REPORT_FILE), 'utf8')
...
export function pendingNotice(moorHome: string, userData: string): PendingNotice | null {
  const report = readReport(moorHome)
