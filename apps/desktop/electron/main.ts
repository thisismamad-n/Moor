...
import {
  ...
  resolveVenvMoorCommand
} from './windows-moor-path'
import {
...
const USER_DATA_OVERRIDE = process.env.MOOR_DESKTOP_USER_DATA_DIR
...
const DEV_SERVER = process.env.MOOR_DESKTOP_DEV_SERVER
const IS_PACKAGED = app.isPackaged || Boolean(process.env.MOOR_DESKTOP_IS_PACKAGED)
...
if (REMOTE_DISPLAY_REASON) {
  ...
  console.log(
    `[moor] remote display detected (${REMOTE_DISPLAY_REASON}); disabling GPU hardware acceleration to prevent flicker`
  )
}
...
if (IS_WSL && !REMOTE_DISPLAY_REASON && fs.existsSync('/dev/dxg')) {
  ...
  console.log('[moor] WSL GPU passthrough (/dev/dxg) detected; enabling GPU acceleration')
}
...
if (PASSWORD_STORE.warning) {
  console.warn(`[moor] ${PASSWORD_STORE.warning}`)
}

if (PASSWORD_STORE.store) {
  app.commandLine.appendSwitch('password-store', PASSWORD_STORE.store)
  console.log(`[moor] using password-store backend: ${PASSWORD_STORE.store}`)
}
...
// never go through `moor desktop`; it is version-scoped so an app update
...
if (IS_WINDOWS) {
  ...
  if (shouldAttemptAclRepair(priorMarker)) {
    ...
    if (acl.ok) {
      console.log(`[moor] granted ALL APPLICATION PACKAGES RX on ${exeDir} (#38216)`)
    } else if (acl.error && acl.error !== 'missing-target-or-exec') {
      console.warn(`[moor] AppContainer ACL grant failed on ${exeDir}: ${acl.error}`)
    }
  }
  ...
  if (sandboxDecision.enable && sandboxDecision.reason !== 'already-enabled') {
    ...
    console.log(
      `[moor] Windows sandbox fallback enabled (${sandboxDecision.reason}); launching with --no-sandbox (#38216)`
    )
  }
  ...
  app.on('child-process-gone', (_event, details) => {
    ...
    console.warn(
      `[moor] Windows GPU sandbox crashed (exit=${details?.exitCode}); relaunching once with --no-sandbox (#38216)`
    )
    ...
    try {
      ...
      console.error(`[moor] --no-sandbox relaunch failed: ${error?.message || error}`)
    }
  })
}
    ...
    try {
      ...
      if (parsed && typeof parsed === 'object' && typeof parsed.commit === 'string' && parsed.commit.length >= 7) {
        if (parsed.schemaVersion !== INSTALL_STAMP_SCHEMA_VERSION) {
          console.warn(
            `[moor] install-stamp.json schemaVersion ${parsed.schemaVersion} != expected ${INSTALL_STAMP_SCHEMA_VERSION}; ignoring`
          )
      ...
      console.warn(`[moor] install-stamp.json found at ${p} , but parsing failed with ${e}`)
...
if (INSTALL_STAMP) {
  console.log(
    `[moor] install stamp: ${INSTALL_STAMP.commit.slice(0, 12)}${INSTALL_STAMP.branch ? ` (${INSTALL_STAMP.branch})` : ''}${INSTALL_STAMP.dirty ? ' [DIRTY]' : ''} from ${INSTALL_STAMP.source || 'unknown'}`
  )
} else if (IS_PACKAGED) {
  ...
  console.error(
    '[moor] WARNING: no install-stamp.json found in packaged build. First-launch bootstrap will not have a pinned ref to install.'
  )
}
...
//
// Defaults:
//   Windows: %LOCALAPPDATA%\moor (matches install.ps1)
//   macOS / Linux: ~/.moor (matches install.sh)
//
// Special case for Windows: if the user has a legacy ~/.moor directory
// (e.g., from a prior pip install or a manual setup) AND no
// %LOCALAPPDATA%\moor yet, prefer the legacy path so we don't orphan their
// existing config / sessions / .env. New installs go to %LOCALAPPDATA%.
//
// MOOR_DESKTOP_USER_DATA_DIR (used by test:desktop:fresh) puts the sandbox
// MOOR_HOME beneath the throwaway userData dir so a fresh-install run never
// touches the user's real ~/.moor / %LOCALAPPDATA%\moor.
function resolveMoorHome() {
  if (process.env.MOOR_HOME) {
    return normalizeMoorHomeRoot(process.env.MOOR_HOME)
  }

  if (USER_DATA_OVERRIDE) {
    return path.join(path.resolve(USER_DATA_OVERRIDE), 'moor-home')
  }

  if (IS_WINDOWS) {
    // A GUI app launched from Explorer inherits the environment block captured
    // at login, so a MOOR_HOME set via `setx` AFTER login is invisible in
    // process.env even though the CLI (a fresh shell) sees it. Without this the
    // backend silently falls back to %LOCALAPPDATA%\moor and reports "No
    // inference provider configured" despite a valid configured home (#45471).
    // Consult the live User-scoped registry value before the default below.
    const fromRegistry = readWindowsUserEnvVar('MOOR_HOME')

    if (fromRegistry) {
      return normalizeMoorHomeRoot(fromRegistry)
    }
  }

  if (IS_WINDOWS && process.env.LOCALAPPDATA) {
    const localappdata = path.join(process.env.LOCALAPPDATA, 'moor')
    const legacy = path.join(app.getPath('home'), '.moor')

    // Migrate transparently to LOCALAPPDATA, but honour an existing legacy
    // ~/.moor setup (no LOCALAPPDATA install yet) so users don't lose state.
  ...
  return path.join(app.getPath('home'), '.moor')
}

const MOOR_HOME = resolveMoorHome()

function pathWithMoorManagedNode(...entries) {
  const managed = moorManagedNodePathEntries(MOOR_HOME).filter(directoryExists)
...
// boots, going straight to `resolveMoorBackend()`. Missing or stale marker
...
//
// We deliberately put the marker INSIDE ACTIVE_MOOR_ROOT (not alongside)
...
const BOOTSTRAP_COMPLETE_MARKER = path.join(ACTIVE_MOOR_ROOT, '.moor-bootstrap-complete')
...
const APP_NAME = process.env.MOOR_DESKTOP_APP_NAME || 'Moor'
...
if (IS_WINDOWS) {
  app.setAppUserModelId('com.moorinc.moor')
}

// Seed the native About panel with the live Moor version. This is refreshed
// on every open via the explicit "About" menu handler (refreshAboutPanel), so
// an in-place `moor update` mid-session is reflected without an app restart;
...
// startMoor(); this pool only holds EXTRA profile
...
const POOL_MAX_BACKENDS = Math.max(1, Number(process.env.MOOR_DESKTOP_POOL_MAX) || 3)
const POOL_IDLE_MS = Math.max(60_000, Number(process.env.MOOR_DESKTOP_POOL_IDLE_MS) || 10 * 60_000)
...
// onto the error so subsequent startMoor() calls (e.g. the renderer's
...
function rememberLog(chunk) {
  ...
  if (moorLog.length > 300) {
    moorLog.splice(0, moorLog.length - 300)
  }
...
function ensureWslWindowsFonts() {
  ...
  try {
    ...
    const confPath = path.join(confDir, '99-moor-wsl-windows-fonts.conf')
    let existing = ''
...
function broadcastBootProgress() {
  ...
  webContents.send('moor:boot-progress', bootProgressState)
}
...
//
// The snapshot is queryable via the moor:bootstrap:get IPC handler so a
...
function broadcastBootstrapEvent(ev) {
  ...
  webContents.send('moor:bootstrap:event', ev)
}
...
async function waitForUpdateToFinish() {
  ...
  // "nothing happened").
  try {
    const result = readAndConsumeHandoffResult(MOOR_HOME)
...
function isMoorSourceRoot(root) {
...
// normal MOOR_DASHBOARD_READY stdout line and no ready-file side channel is
// needed.
...
function resolveGitBinary() {
  ...
  if (localAppData) {
    candidates.push(path.join(localAppData, 'moor', 'git', 'cmd', 'git.exe'))
    candidates.push(path.join(localAppData, 'moor', 'git', 'bin', 'git.exe'))
  }
...
function resolveUpdateRoot() {
  const candidates = [
    ...
    !IS_PACKAGED && isMoorSourceRoot(SOURCE_REPO_ROOT) ? SOURCE_REPO_ROOT : null,
    isMoorSourceRoot(ACTIVE_MOOR_ROOT) ? ACTIVE_MOOR_ROOT : null
  ].filter(Boolean)

  return candidates.find(c => directoryExists(path.join(c, '.git'))) || candidates[0] || ACTIVE_MOOR_ROOT
}
...
function emitUpdateProgress(payload) {
  ...
  for (const window of BrowserWindow.getAllWindows()) {
    window.webContents.send('moor:updates:progress', merged)
  }
}
...
function resolveUpdaterBinary() {
  return resolveStagedUpdaterBinary(MOOR_HOME, { fileExists, isWindows: IS_WINDOWS })
}
...
function venvMoorShimPath(updateRoot) {
  return IS_WINDOWS
    ? path.join(updateRoot, 'venv', 'Scripts', 'moor.exe')
    : path.join(updateRoot, 'venv', 'bin', 'moor')
}
...
// only signals the direct child, so on Windows a backend `moor.exe` that
// spawned its own grandchildren (a `moor` REPL, a pty terminal session, the
...
async function claimBackendChild(child, command, profile, nonce) {
  try {
    ...
    child.moorBackendIdentity = identity
...
function releaseBackendChild(child) {
  const identity = child?.moorBackendIdentity
...
// `moorProcess.kill('SIGTERM')` + `app.quit()` fire-and-forget: SIGTERM on
...
async function releaseBackendLock(updateRoot, tag) {
  ...
  const moorProcess = backendConnectionState.getProcess()

  stopBackendTreesForUpdate(moorProcess, {
  ...
  while (Date.now() < deadlineMs) {
    ...
    const currentMoorProcess = backendConnectionState.getProcess()

    if (currentMoorProcess && Number.isInteger(currentMoorProcess.pid)) {
      stragglers.push(currentMoorProcess.pid)
    }
...
// `moor update`). Instead we spawn the staged Moor-Setup binary with
// --update and quit, so it can run `moor update` (which refuses while we
...
async function applyUpdates(opts: { stopSafeBlockers?: boolean } = {}) {
  ...
  try {
      ...
      if (!resolveUpdateScriptHandoff(updateRoot)) {
        ...
        try {
          ...
          if (head.code === 0 && current && current !== 'HEAD') {
            ...
            if (branch !== 'main') {
              command = `moor update --branch ${branch}`
            }
          }
        } catch {
          // Best-effort: fall back to bare `moor update` if branch detection fails.
        }
    ...
    // moor.exe (held by the backend child / its grandchildren) and the update
    ...
    if (!lock.unlocked) {
      ...
      const message =
        'Update aborted: another process is holding the Moor install open ' +
        '(a second Moor window or a terminal running moor?). Close it and retry.'
    ...
    if (scriptHandoff) {
      ...
      if (Number.isInteger(child.pid)) {
        writeUpdateMarker(MOOR_HOME, child.pid)
      }
...
async function handOffWindowsBootstrapRecovery(reason) {
  ...
  const venvBin = path.join(updateRoot, 'venv', IS_WINDOWS ? 'Scripts' : 'bin')
  const venvMoor = path.join(venvBin, IS_WINDOWS ? 'moor.exe' : 'moor')
  ...
  // not just the `moor.exe` console-script shim. That shim is generated at the
  ...
  const haveRealInstall =
    fileExists(venvPython) || fileExists(venvMoor) || fileExists(path.join(updateRoot, '.moor-bootstrap-complete'))
...
function preflightStateDb(moorHome, rememberLog) {
  ...
  try {
    ...
    if (stat.size > 100) {
      ...
      const emergencyPath = path.join(moorHome, `state.db.pre-update-emergency-${ts}.bak`)
      ...
      try {
        ...
        try {
          const homeDir = fs.readdirSync(moorHome)
          ...
          for (const old of backups.slice(2)) {
            try {
              fs.unlinkSync(path.join(moorHome, old))
            } catch {
              void 0
            }
          }
...
async function applyUpdatesPosixHandoff(opts: any) {
  ...
  if (!handoff) {
    emitUpdateProgress({ stage: 'manual', message: 'moor update', percent: null })

    return { ok: true, manual: true, command: 'moor update', moorRoot: updateRoot }
  }
...
function resolveWebDist() {
  const override = process.env.MOOR_DESKTOP_WEB_DIST
  ...
  if (IS_PACKAGED && /app\.asar(?=$|[\\/])/.test(fallback) && !directoryExists(fallback)) {
    rememberLog(
      `[web-dist] dashboard frontend dir resolved to an asar-internal path that ` +
        ...
        `Ensure dist/** is unpacked (asarUnpack) or set MOOR_DESKTOP_WEB_DIST.`
    )
  }
...
function createPythonBackend(root, label, backendArgs, options: any = {}) {
  ...
  return {
    ...
    label,
    command,
    args: ['-m', 'moor_cli.main', ...backendArgs],
...
function createActiveBackend(backendArgs) {
  ...
  return {
    kind: 'python',
    label: `Moor at ${ACTIVE_MOOR_ROOT}`,
    command,
    args: ['-m', 'moor_cli.main', ...backendArgs],
    env: buildDesktopBackendEnv({
      moorHome: MOOR_HOME,
      pythonPathEntries: [ACTIVE_MOOR_ROOT, ...getVenvSitePackagesEntries(VENV_ROOT)],
      venvRoot: VENV_ROOT
    }),
    root: ACTIVE_MOOR_ROOT,
    bootstrap: true,
    shell: false
  }
}
...
function resolveMoorBackend(backendArgs) {
  ...
  //    MOOR_DESKTOP_IGNORE_EXISTING=1 forces the bootstrap path for testing.
  if (process.env.MOOR_DESKTOP_IGNORE_EXISTING !== '1') {
    let moorCommand = null
    const moorOverride = process.env.MOOR_DESKTOP_MOOR

    if (moorOverride) {
      const resolvedOverride = findOnPath(moorOverride)
    ...
    if (moorCommand) {
      ...
      // Smoke-test the candidate before trusting it. A `moor` shim
      ...
      const shellForProbe = isCommandScript(moorCommand)

      // MOOR_DESKTOP_MOOR is an explicit deployment override (used by
  ...
  if (python) {
    ...
    // having moor_cli installed -- common on dev boxes that have
    ...
    rememberLog(`Ignoring system Python ${python}: moor_cli is not importable; falling through to bootstrap.`)
  }
  ...
  //    resolveMoorBackend was the old "no payload" path and forced the
...
async function ensureRuntime(backend) {
  ...
  // backend.kind === 'bootstrap-needed' means resolveMoorBackend couldn't
  ...
  if (backend.kind === 'bootstrap-needed') {
    ...
    if (await handOffWindowsBootstrapRecovery('bootstrap-needed')) {
      const handoffError: Error & { isBootstrapFailure?: boolean; bootstrapHandedOff?: boolean } = new Error(
        'Moor recovery was handed off to Moor Setup. The desktop will restart when recovery completes.'
      )
    ...
    if (!bootstrapResult.ok) {
      ...
      // Latch the failure so subsequent startMoor() calls return this
      // same error without re-running install.ps1.  Cleared by the
      // moor:bootstrap:reset IPC (renderer's "Reload and retry").
  ...
  // %LOCALAPPDATA%\moor\git\, which findGitBash() picks up, so for any
  ...
  if (!fileExists(venvPython)) {
    ...
    // normal flow because activeRuntimeState() requires isMoorSourceRoot()
  ...
  backend.label = `Moor at ${ACTIVE_MOOR_ROOT} (venv: ${VENV_ROOT})`
  updateBootProgress({
    phase: 'runtime.ready',
...
function getLinkTitleSession() {
  ...
  linkTitleSession = session.fromPartition('moor:link-titles', { cache: false })
...
async function previewFileTarget(rawTarget, baseDir) {
  ...
  const base = baseDir ? path.resolve(expandUserPath(baseDir)) : resolveMoorCwd()
...
function sendPreviewFileChanged(payload) {
  ...
  webContents.send('moor:preview-file-changed', payload)
}
...
async function waitForMoor(baseUrl, token, signal?, authMode?, headers = {}) {
  const { probeHealth, probeIsCredentialed } = await buildReadinessHealthProbe(baseUrl, authMode, token)

  return waitForMoorReady(baseUrl, {
    token,
    signal,
    fetchPublicJson,
...
function sendBackendExit(payload) {
  ...
  webContents.send('moor:backend-exit', payload)
}
...
function sendClosePreviewRequested() {
  ...
  webContents.send('moor:close-preview-requested')
}
...
function sendOpenFolderRequested() {
  ...
  webContents.send('moor:open-folder-requested')
}
...
function sendPowerResume() {
  ...
  webContents.send('moor:power-resume')
}
...
// 'moor:power-battery' push below.
ipcMain.handle('moor:power-battery:get', () => onBatteryPower === true)
...
function broadcastBatteryState(next: boolean) {
  ...
  for (const win of BrowserWindow.getAllWindows()) {
    ...
    if (webContents && !webContents.isDestroyed()) {
      webContents.send('moor:power-battery', next)
    }
  }
}
...
function sendOpenUpdatesRequested() {
  ...
  webContents.send('moor:open-updates')
...
function sendWindowStateChanged(nextIsFullscreen?: boolean, target = mainWindow) {
  ...
  webContents.send('moor:window-state-changed', state)
}
...
// Moor inc.) instead of a static session token. The auth model is
...
//   * Moor Portal now issues a 24h ROTATING, reuse-detected refresh token
//     alongside the ~15-min access token (Portal NAS #293 / moor #37247).
...
const OAUTH_SESSION_PARTITION = 'persist:moor-remote-oauth'
...
// feature; the server half lives in moor_cli/dashboard_auth/native_flow.py.
// ---------------------------------------------------------------------------
...
// --- Moor Cloud discovery + silent per-agent sign-in (cloud-auto-discovery
// Phase 3) ---------------------------------------------------------------
//
// The "cloud" connection mode lets a user sign in to the Moor portal ONCE in
...
// hasLiveOauthSession, which looks for moor_session_at/rt that the portal
...
function openPortalLoginWindow() {
  ...
  return new Promise((resolve, reject) => {
    if (!app.isReady()) {
      reject(new Error('Desktop is not ready to start a Moor Cloud sign-in.'))
...
async function cloudAgentSilentSignIn(dashboardUrl) {
  ...
  if (!(await hasLivePortalSession())) {
    const err = new Error('Your Moor Cloud session has expired. Sign in to Moor Cloud again.') as any
    err.needsCloudLogin = true
    throw err
  }
...
function readDesktopConnectionsRegistry() {
  ...
  if (mtime === null) {
    ...
    try {
      ...
      // hard-failing every moor:connections:* call.
...
// a valid stored value (pins the root MOOR_HOME explicitly); null means "no
...
async function sanitizeDesktopConnectionConfig(config = readDesktopConnectionConfig(), profile = null) {
  ...
  const envOverride = key ? false : Boolean(process.env.MOOR_DESKTOP_REMOTE_URL)
  ...
  const remoteUrl = envOverride ? String(process.env.MOOR_DESKTOP_REMOTE_URL || '') : String(block.url || '')
  ...
  return {
    ...
    sshRemoteMoorPath: (ssh || savedSsh)?.remoteMoorPath || '',
    sshRemoteProfile: (ssh || savedSsh)?.remoteProfile || '',
    // The env override only forces the global/primary connection; a per-profile
    // scope is never overridden by MOOR_DESKTOP_REMOTE_URL.
    envOverride
  }
}
...
function coerceDesktopConnectionConfig(input: any = {}, existing = readDesktopConnectionConfig(), options: any = {}) {
  ...
  // describe a discovered Moor Cloud instance, NOT a user-owned remote gateway,
...
function buildSshBlock(input: any, existingBlock: any = {}) {
  ...
  const merged = normalizeSshConfig({
    ...
    remoteMoorPath: input.sshRemoteMoorPath ?? existingBlock.remoteMoorPath,
...
async function bootstrapSshConnectionInner(profile, sshConfig, reuseToken, source, fingerprint, lease) {
  ...
  try {
    const platform = await detectRemotePlatform(ssh, sshConfig.remoteMoorPath || '')
    ...
    result = await lifecycle({
      ...
      pickLocalPort,
      waitForMoor: (baseUrl, token) => waitForMoor(baseUrl, token, lease.signal, 'token'),
  ...
  sshConnections.set(scope, {
    ...
    host: sshConfig.host,
    hostLabel,
    moorVersion: result.moorVersion || '',
  ...
  sshRememberLog(
    `[ssh] connection ${result.reused ? 'REUSED' : 'spawned'} dashboard: ` +
      `${result.moorVersion || 'moor (version unknown)'} at ${result.moorPath || '?'}`
  )
  ...
  return { ...connection, remoteMoorVersion: result.moorVersion || '' }
}
...
async function resolveRemoteBackend(profile) {
  ...
  const rawEnvUrl = process.env.MOOR_DESKTOP_REMOTE_URL
  const rawEnvToken = process.env.MOOR_DESKTOP_REMOTE_TOKEN

  if (rawEnvUrl) {
    if (!rawEnvToken) {
      throw new Error(
        'MOOR_DESKTOP_REMOTE_URL is set but MOOR_DESKTOP_REMOTE_TOKEN is not. ' +
...
async function probeRemoteAuthMode(rawUrl) {
  ...
  if (authRequired) {
    ...
    // button can read "Sign in with Moor inc." instead of a generic
...
async function testDesktopConnectionConfig(input: any = {}) {
  if (input.mode === 'ssh') {
    const sshConfig = normalizeSshConfig({
      ...
      remoteMoorPath: input.sshRemoteMoorPath
    })
    ...
    try {
      ...
      for (;;) {
        try {
          await ssh.open()
          const platform: any = await detectRemotePlatform(ssh, sshConfig.remoteMoorPath || '')
          let moorPath
          let moorVersion
          let supported
          ...
          if (platform.os === 'Windows') {
            ...
            moorPath = runtime.moorPath
            const inspection = await helper(ssh, runtime, 'inspect', [runtime.moorPath])
            moorVersion = inspection.version
            supported = inspection.supported
          } else {
            moorPath = await remoteLifecycle.locateMoor(ssh, sshConfig.remoteMoorPath || '')
            moorVersion = await remoteLifecycle.probeMoorVersion(ssh, moorPath)
            supported = await remoteLifecycle.remoteSupportsSshOwnership(ssh, moorPath)
          }
          ...
          return {
            ...
            remoteMoorPath: moorPath,
            remoteMoorVersion: moorVersion,
  ...
  if (wantRemote && block?.url) {
    ...
    const remote = (await resolveRemoteBackend(key)) || (await startMoor())
...
function resetMoorConnection({ soft = false } = {}) {
...
// startMoor() spawns fresh instead of racing the dying one. Shared by the
// connection-config and profile switch flows.
async function teardownPrimaryBackendAndWait({ soft = false } = {}) {
  // Capture the reference before resetMoorConnection() invalidates it.
  const moorProcess = backendConnectionState.getProcess()
  const dying = moorProcess && !moorProcess.killed ? moorProcess : null
...
function sendConnectionApplied() {
  ...
  webContents.send('moor:connection:applied')
}
...
function broadcastConnectionsChanged(payload: { connectionId: string; reason: 'removed' | 'updated' }) {
  for (const win of BrowserWindow.getAllWindows()) {
    ...
    if (webContents && !webContents.isDestroyed()) {
      webContents.send('moor:connections:changed', payload)
    }
  }
}
...
async function ensureBackend(profile) {
  ...
  if (route.backend === 'primary') {
    const connection = await startMoor()
...
async function connectRegistryBackend(source, profile, key, poolEntry) {
  ...
  if (source.kind === 'ssh') {
    ...
    const sshConfig = normalizeSshConfig({
      ...
      remoteMoorPath: source.remoteMoorPath,
    ...
    return {
      ...
      // is only the routing label. moor:api uses it to translate explicit
  ...
  await waitForMoor(connection.baseUrl, connection.token, undefined, connection.authMode, connection.headers)
  poolEntry.remoteBaseUrl = connection.baseUrl
...
// local-spawn portion of startMoor() but without the boot-progress UI,
  ...
  const connectionPromise = (async () => {
    const connectRemote = async remote => {
      ...
      return {
        ...
        remoteMoorVersion: remote.remoteMoorVersion,
        token: remote.token,
        wsUrl: remote.wsUrl,
        logs: moorLog.slice(-80),
        ...getWindowState()
    ...
    // deterministic (it wins over the sticky ~/.moor/active_profile file) and
    // resolves MOOR_HOME the same way `moor -p <name>` does on the CLI. An
    ...
    const setup = await runPrimaryBackendStartup({
      ...
      prepareLocalBackend: async () => {
        ...
        return resolveMoorBackend(backendArgs)
      },
      resolveRemote: () => {
    ...
    await claimBackendChild(moorProcess, `${backend.command} ${backend.args.join(' ')}`, profile, backendNonce)
    const processOwner = backendConnectionState.attachProcess(connectionAttempt, moorProcess)

    if (!processOwner) {
      stopBackendChild(moorProcess)
      await waitForBackendExit(moorProcess)
    ...
    moorProcess.once('exit', (code, signal) => {
      releaseBackendChild(moorProcess)

      if (!backendConnectionState.clearForCurrentProcess(processOwner)) {
        rememberLog(`Ignoring stale Moor backend exit (${signal || code})`)
    ...
    const port = await Promise.race([
      waitForDashboardPortAnnouncement(moorProcess, { readyFile }),
      backendStartFailed
    ])
    ...
    const authToken = await adoptServedDashboardToken(baseUrl, token, {
      childAlive: () => moorProcess.exitCode === null && !moorProcess.killed,
      rememberLog
    })
...
function spawnPetOverlayWindow(bounds) {
  const win = new BrowserWindow({
    ...
    // instead, leaving the main Moor app as the Dock/cmd-tab anchor.
    skipTaskbar: !IS_MAC,
    hasShadow: false,
    alwaysOnTop: true,
  ...
  try {
    ...
    // which briefly hides the Dock/cmd-tab presence. Keep Moor in the normal
  ...
  win.on('closed', () => {
    ...
    if (mainWindow && !mainWindow.isDestroyed()) {
      mainWindow.webContents.send('moor:pet-overlay:control', { type: 'pop-in' })
    }
  })
...
function startHudCursorFeed(win: BrowserWindow) {
  ...
  const timer = setInterval(() => {
    ...
    last = key
    win.webContents.send('moor:hud:cursor', point)
  }, HUD_CURSOR_POLL_MS)
...
function broadcastHudState(open) {
  ...
  for (const win of BrowserWindow.getAllWindows()) {
    if (!win.isDestroyed()) {
      win.webContents.send('moor:hud:changed', payload)
    }
  }
}
...
function openHudWindow(sessionId, profile) {
  ...
  if (hudWindow && !hudWindow.isDestroyed()) {
    ...
    if (sessionId && sessionId !== hudSessionId) {
      ...
      hudWindow.webContents.send('moor:hud:goto', sessionId)
...
function spawnQuickEntryWindow() {
  ...
  win.webContents.on('did-finish-load', () => {
    if (!win.isDestroyed() && quickEntryLastState) {
      win.webContents.send('moor:quick-entry:state', quickEntryLastState)
    }
  })
...
function showQuickEntryWindow() {
  ...
  quickEntryWindow.webContents.send('moor:quick-entry:shown')
}
...
function createWindow() {
  ...
  startMoor().catch(error => rememberLog(error.stack || error.message))
...
ipcMain.handle('moor:connection', async (_event, profile) => ensureBackend(profile))
...
ipcMain.handle('moor:connection:revalidate', async () => {
  ...
  return remoteRevalidation.run(connectionPromise, async () => {
    const [result] = await Promise.all([
      revalidateRemoteConnection({
        ...
        resetConnection: resetMoorConnection,
...
ipcMain.handle('moor:backend:touch', async (_event, profile) => {
  touchPoolBackend(profile)

  return { ok: true }
})
ipcMain.handle('moor:gateway:ws-url', async (_event, profile) => {
  return gatewayWsUrlIpcResult(() => freshGatewayWsUrl(profile))
})
ipcMain.handle('moor:window:openSession', async (_event, sessionId, opts) => {
...
ipcMain.handle('moor:window:openInstance', async () => {
...
// it (`moor --tui --resume <id>`). Not the in-app terminal pane: the point is
...
ipcMain.handle('moor:window:openInTerminal', async (_event, sessionId, opts) => {
  ...
  try {
    ...
    const scriptPath = path.join(
      scriptDir,
      `moor-${crypto.randomBytes(6).toString('hex')}${terminalScriptExtension()}`
    )
    ...
    fs.writeFileSync(
      ...
      buildTerminalScript({
        ...
        cwd,
        env: terminalScriptEnv(backend.env, MOOR_HOME)
      }),
      { mode: 0o700 }
    )
...
ipcMain.handle('moor:wake-indicator:get', () => wakeIndicatorController.getState())
ipcMain.on('moor:wake-indicator:set', (_event, state) => {
...
ipcMain.handle('moor:zoom:get', event => {
...
ipcMain.on('moor:zoom:set-percent', (event, percent) => {
  const window = BrowserWindow.fromWebContents(event.sender)
...
ipcMain.handle('moor:pet-overlay:open', async (_event, request) => {
...
ipcMain.handle('moor:pet-overlay:close', async () => {
  closePetOverlay()
...
ipcMain.on('moor:pet-overlay:set-bounds', (_event, bounds) => {
...
ipcMain.on('moor:pet-overlay:ignore-mouse', (_event, ignore) => {
  if (petOverlayWindow && !petOverlayWindow.isDestroyed()) {
...
ipcMain.on('moor:pet-overlay:set-focusable', (_event, focusable) => {
  if (!petOverlayWindow || petOverlayWindow.isDestroyed()) {
    return
  }
...
ipcMain.on('moor:pet-overlay:state', (_event, payload) => {
  if (petOverlayWindow && !petOverlayWindow.isDestroyed()) {
    petOverlayWindow.webContents.send('moor:pet-overlay:state', payload)
  }
})
// Overlay → main renderer: control messages (pop back in, composer submit).
ipcMain.on('moor:pet-overlay:control', (_event, payload) => {
  if (!mainWindow || mainWindow.isDestroyed()) {
    return
  }
...
ipcMain.handle('moor:hud:open', async (_event, request) => {
  openHudWindow(
...
ipcMain.handle('moor:hud:vibrancy', (_event, on) => {
...
// reaches the bar.
ipcMain.on('moor:hud:ignore-mouse', (_event, ignore) => {
  if (hudWindow && !hudWindow.isDestroyed()) {
    hudWindow.setIgnoreMouseEvents(Boolean(ignore), { forward: true })
  }
})

ipcMain.on('moor:hud:move-by', (event, delta) => {
...
ipcMain.on('moor:hud:set-bounds', (event, bounds) => {
...
ipcMain.on('moor:hud:session', (event, sessionId) => {
  if (hudWindow && !hudWindow.isDestroyed() && event.sender === hudWindow.webContents) {
    hudSessionId = typeof sessionId === 'string' && sessionId ? sessionId : null
  }
})

ipcMain.handle('moor:hud:close', async () => {
  closeHudWindow()

  return { ok: true }
})
ipcMain.handle('moor:bootstrap:reset', async () => {
...
ipcMain.handle('moor:bootstrap:repair', async () => {
...
ipcMain.handle('moor:ssh-config:hosts', async () => ({ hosts: collectSshConfigHosts() }))
ipcMain.handle('moor:ssh-config:resolve', async (_event, host) => {
...
ipcMain.handle('moor:connections:list', async () => sanitizeConnectionsRegistry())
ipcMain.handle('moor:connections:save', async (_event, payload) => {
  const saved = await saveRegistryConnection(payload)

  return { ok: true, connection: saved, registry: sanitizeConnectionsRegistry() }
})
ipcMain.handle('moor:connections:remove', async (_event, id) => {
  const key = String(id || '')
...
ipcMain.handle('moor:connections:set-primary', async (_event, id) => {
  const registry = setPrimaryConnection(readDesktopConnectionsRegistry(), String(id || ''))
  writeDesktopConnectionsRegistry(registry)

  return { ok: true, registry: sanitizeConnectionsRegistry(registry) }
})
ipcMain.handle('moor:connections:test', async (_event, id) => {
  const registry = readDesktopConnectionsRegistry()
  ...
  if (entry.kind === 'ssh') {
    return testDesktopConnectionConfig({
      ...
      sshRemoteMoorPath: entry.remoteMoorPath
    })
  }
  ...
  return Promise.all(
    registry.connections.map(async connection => {
      try {
        ...
        // The root MOOR_HOME is an agent too; enumerations that omit it
...
ipcMain.handle('moor:agents:roster', async () => {
...
// moor:gateway:ws-url. Same single-use-ticket discipline for OAuth sources.
ipcMain.handle('moor:gateway:ws-url-for', async (_event, payload) => {
  const { connectionId, profile } = payload && typeof payload === 'object' ? (payload as any) : ({} as any)
...
// Fan out `moor update` to every eligible registered connection at once.
...
// /api/moor/update endpoint (the dashboard updater), which runs
// `moor update` on THAT machine.
ipcMain.handle('moor:connections:update-all', async () => {
  const registry = readDesktopConnectionsRegistry()
...
ipcMain.handle('moor:connection-config:probe', async (_event, rawUrl) => probeRemoteAuthMode(rawUrl))
...
// --- Moor Cloud (cloud-auto-discovery Phase 3) ---
...
ipcMain.handle('moor:cloud:agent-sign-in', async (_event, dashboardUrl) => {
...
ipcMain.handle('moor:connection-config:save', async (_event, payload) => {
  const config = coerceDesktopConnectionConfig(payload)
  writeDesktopConnectionConfig(config)

  return sanitizeDesktopConnectionConfig(config, payload?.profile)
})
ipcMain.handle('moor:connection-config:apply', async (_event, payload) => {
  const config = coerceDesktopConnectionConfig(payload)
  writeDesktopConnectionConfig(config)
...
ipcMain.handle('moor:profile:set', async (_event, name) => {
  ...
  // new MOOR_HOME. Pool backends keep their own homes, so only the primary
  // is torn down.
  await teardownPrimaryBackendAndWait()
  mainWindow?.reload()
...
ipcMain.on('moor:previewShortcutActive', (_event, active) => {
...
ipcMain.handle('moor:api', async (_event, request) => {
  ...
  if (!deletingProfile) {
    return handleMoorApiRequest(request)
  }
...
ipcMain.handle('moor:data-url-read-max:get', () => ({
...
ipcMain.handle('moor:data-url-read-max:set', (_event, maxMb) => {
...
ipcMain.handle('moor:readFileDataUrl', async (_event, filePath) => {
...
ipcMain.handle('moor:readFileDataUrlForAttach', async (_event, filePath) => {
  return readFileDataUrlForIpc(filePath, {
...
ipcMain.handle('moor:readFileText', async (_event, filePath) => {
...
ipcMain.handle('moor:writeClipboard', (_event, text) => {
...
ipcMain.handle('moor:readClipboard', () => clipboard.readText())

ipcMain.handle('moor:saveGatewayFile', (_event, payload) => saveGatewayFile(payload))

ipcMain.handle('moor:saveImageFromUrl', (_event, url) => saveImageFromUrl(String(url || '')))

ipcMain.handle('moor:saveImageBuffer', async (_event, payload) => {
...
ipcMain.handle('moor:normalizePreviewTarget', (_event, target, baseDir) =>
  normalizePreviewTarget(String(target || ''), baseDir ? String(baseDir) : '')
)

ipcMain.handle('moor:watchPreviewFile', (_event, url) => watchPreviewFile(String(url || '')))

ipcMain.handle('moor:watchDirectory', (_event, dir) => watchDirectory(String(dir || '')))

ipcMain.handle('moor:stopPreviewFileWatch', (_event, id) => stopPreviewFileWatch(String(id || '')))
...
ipcMain.on('moor:active-work', (event, payload) => {
...
ipcMain.on('moor:titlebar-theme', (_event, payload) => {
...
ipcMain.on('moor:native-theme', (_event, mode) => {
...
ipcMain.on('moor:translucency', (_event, payload) => {
...
ipcMain.on('moor:keep-awake', (_event, on) => {
...
ipcMain.handle('moor:quick-entry:settings:set', async (_event, patch) => {
...
ipcMain.on('moor:quick-entry:state', (_event, payload) => {
  ...
  if (quickEntryWindow && !quickEntryWindow.isDestroyed()) {
    quickEntryWindow.webContents.send('moor:quick-entry:state', payload)
  }
})

ipcMain.on('moor:quick-entry:dismiss', () => hideQuickEntryWindow())
...
ipcMain.on('moor:devtools:disable-f12', (_event, on) => {
...
ipcMain.handle('moor:openExternal', (_event, url) => {
...
ipcMain.handle('moor:find-in-page', async (event, query, options) => {
  const win = BrowserWindow.fromWebContents(event.sender)
...
ipcMain.handle('moor:stop-find-in-page', event => {
  const win = BrowserWindow.fromWebContents(event.sender)
...
ipcMain.handle('moor:openPreviewInBrowser', async (_event, url) => {
...
// it via writeDefaultProjectDir so resolveMoorCwd picks it up on the next
// session spawn (no app restart needed).
ipcMain.handle('moor:setting:defaultProjectDir:get', async () => ({
  dir: readDefaultProjectDir(),
  defaultLabel: app.getPath('home'),
  resolvedCwd: resolveMoorCwd()
}))

ipcMain.handle('moor:workspace:sanitize', async (_event, cwd) => sanitizeWorkspaceCwd(cwd))

ipcMain.handle('moor:setting:defaultProjectDir:set', async (_event, dir) => {
...
ipcMain.handle('moor:setting:defaultProjectDir:pick', async () => {
...
ipcMain.handle('moor:fetchLinkTitle', (_event, url) => fetchLinkTitle(url))

ipcMain.handle('moor:logs:reveal', async () => {
  try {
    await fs.promises.mkdir(path.dirname(DESKTOP_LOG_PATH), { recursive: true })
...
ipcMain.handle('moor:logs:recent', async () => ({ path: DESKTOP_LOG_PATH, lines: moorLog.slice(-200) }))
...
function terminalShellCommand() {
  // MOOR_DESKTOP_SHELL is the cross-platform escape hatch (a path or a bare
  ...
  const override = (process.env.MOOR_DESKTOP_SHELL || (IS_WINDOWS ? '' : process.env.SHELL) || '').trim()
...
function terminalShellEnv() {
  ...
  // Let a moor/--tui launched in this pane know it's embedded in the desktop
  // GUI (build_environment_hints surfaces this). Distinct from MOOR_DESKTOP,
  // which marks the agent *backend* and gates cron/gateway behavior.
  env.MOOR_DESKTOP_TERMINAL = '1'

  return env
}

function terminalChannel(id, suffix) {
  return `moor:terminal:${id}:${suffix}`
}
...
ipcMain.handle('moor:fs:readDir', async (_event, dirPath) => readDirForIpc(dirPath))

ipcMain.handle('moor:fs:gitRoot', async (_event, startPath) => gitRootForIpc(startPath))
...
ipcMain.handle('moor:fs:openDir', async (_event, dirPath) => {
...
// The LOCAL Desktop runtime-plugin root: `<MOOR_HOME>/desktop-plugins`,
// resolved from the main-process MOOR_HOME (see resolveMoorHome) — NOT from
// the connected backend. A remote backend reports its own `moor_home` over
...
async function localPluginsRoot(dirName: string): Promise<string> {
  ...
  // profiles/<name>/, matching the profile-scoped moor_home the backend
  // reported before this resolver existed. 'default'/unset pins the global root.
  const profile = readActiveDesktopProfile()
  const base = profile && profile !== 'default' ? path.join(MOOR_HOME, 'profiles', profile) : MOOR_HOME
...
ipcMain.handle('moor:fs:desktopPluginsRoot', async () => localPluginsRoot('desktop-plugins'))

// The LOCAL agent-plugin root (`<MOOR_HOME>/plugins`), same Electron-local
...
ipcMain.handle('moor:fs:agentPluginsRoot', async () => localPluginsRoot('plugins'))

// Rename a file/folder in place. The renderer passes the existing path + a new
// base name; the destination is resolved in the SAME parent dir so a rename can
// never move the item elsewhere or traverse out. Rejects on a name collision.
ipcMain.handle('moor:fs:rename', async (_event, targetPath, newName) => {
...
ipcMain.handle('moor:fs:writeText', async (_event, filePath, content) => {
...
ipcMain.handle('moor:fs:trash', async (_event, targetPath) => {
  const target = String(targetPath || '').trim()
...
ipcMain.handle('moor:git:worktreeList', async (_event, repoPath) => listWorktrees(repoPath, resolveGitBinary()))

ipcMain.handle('moor:git:worktreeAdd', async (_event, repoPath, options) =>
  addWorktree(repoPath, options || {}, resolveGitBinary())
)

ipcMain.handle('moor:git:worktreeRemove', async (_event, repoPath, worktreePath, options) =>
  removeWorktree(repoPath, worktreePath, options || {}, resolveGitBinary())
)

ipcMain.handle('moor:git:branchSwitch', async (_event, repoPath, branch) =>
  switchBranch(repoPath, branch, resolveGitBinary())
)

ipcMain.handle('moor:git:branchList', async (_event, repoPath) => listBranches(repoPath, resolveGitBinary()))

ipcMain.handle('moor:git:baseBranchList', async (_event, repoPath) => listBaseBranches(repoPath, resolveGitBinary()))
...
ipcMain.handle('moor:git:repoStatus', async (_event, repoPath) => repoStatus(repoPath, resolveGitBinary()))
...
ipcMain.handle('moor:git:review:list', async (_event, repoPath, scope, baseRef) =>
  reviewList(repoPath, scope, baseRef, resolveGitBinary())
)
ipcMain.handle('moor:git:review:diff', async (_event, repoPath, filePath, scope, baseRef, staged) =>
...
ipcMain.handle('moor:git:fileDiff', async (_event, repoPath, filePath) =>
  fileDiffVsHead(repoPath, filePath, resolveGitBinary())
)
ipcMain.handle('moor:git:review:stage', async (_event, repoPath, filePath) =>
  reviewStage(repoPath, filePath ?? null, resolveGitBinary())
)
ipcMain.handle('moor:git:review:unstage', async (_event, repoPath, filePath) =>
  reviewUnstage(repoPath, filePath ?? null, resolveGitBinary())
)
ipcMain.handle('moor:git:review:revert', async (_event, repoPath, filePath) =>
  reviewRevert(repoPath, filePath ?? null, resolveGitBinary())
)
ipcMain.handle('moor:git:review:revParse', async (_event, repoPath, ref) =>
  reviewRevParse(repoPath, ref, resolveGitBinary())
)
ipcMain.handle('moor:git:review:commit', async (_event, repoPath, message, push) =>
  reviewCommit(repoPath, message, Boolean(push), resolveGitBinary())
)
ipcMain.handle('moor:git:review:commitContext', async (_event, repoPath) =>
  reviewCommitContext(repoPath, resolveGitBinary())
)
ipcMain.handle('moor:git:review:push', async (_event, repoPath) => reviewPush(repoPath, resolveGitBinary()))
ipcMain.handle('moor:git:review:shipInfo', async (_event, repoPath) => reviewShipInfo(repoPath, resolveGhBinary()))
...
ipcMain.handle('moor:git:scanRepos', async (_event, roots, options) => {
  try {
...
ipcMain.handle('moor:terminal:start', async (event, payload = {}) => {
...
ipcMain.handle('moor:terminal:write', (_event, id, data) => {
...
ipcMain.handle('moor:terminal:resize', (_event, id, size = {}) => {
  const sessionInfo = terminalSessions.get(String(id || ''))
...
ipcMain.handle('moor:terminal:cwd', async (_event, id) => {
  const sessionInfo = terminalSessions.get(String(id || ''))
...
ipcMain.handle('moor:terminal:dispose', (_event, id) => disposeTerminalSession(String(id || '')))
...
function resolveMoorVersion() {
  try {
    const root = resolveUpdateRoot()
    const initPath = path.join(root, 'moor_cli', '__init__.py')
...
ipcMain.handle('moor:version', async () => ({
  appVersion: resolveMoorVersion(),
...
async function getUninstallSummary() {
  ...
  return new Promise(resolve => {
    ...
    try {
      const child = spawn(
        py,
        ['-m', 'moor_cli.main', 'uninstall', '--gui-summary'],
        hiddenWindowsChildOptions({
          cwd: agentRoot,
          env: { ...process.env, MOOR_HOME, NO_COLOR: '1' },
          stdio: ['ignore', 'pipe', 'ignore']
        })
      )
...
async function runDesktopUninstall(mode) {
  ...
  try {
    await releaseBackendLock(ACTIVE_MOOR_ROOT, 'uninstall')
  } catch (error) {
  ...
  try {
    if (IS_WINDOWS) {
      scriptPath = path.join(app.getPath('temp'), `moor-uninstall-${Date.now()}.cmd`)
      ...
      scriptPath = path.join(app.getPath('temp'), `moor-uninstall-${Date.now()}.sh`)
...
ipcMain.handle('moor:uninstall:summary', async () => getUninstallSummary())
ipcMain.handle('moor:uninstall:run', async (_event, payload) => {
...
ipcMain.handle('moor:vscode-theme:search', async (_event, query) => searchMarketplaceThemes(String(query || ''), 20))

// ---------------------------------------------------------------------------
// moor:// deep links (e.g. moor://blueprint/morning-brief?time=08:00, or
...
function _extractDeepLink(argv) {
  ...
  return argv.find(a => typeof a === 'string' && a.startsWith(`${MOOR_PROTOCOL}://`)) || null
}
...
function handleDeepLink(url) {
  ...
  // moor://blueprint/<key>?slot=val  -> host="blueprint", path="/<key>"
  ...
  try {
    ...
    mainWindow.focus()
    mainWindow.webContents.send('moor:deep-link', payload)
...
ipcMain.handle('moor:deep-link-ready', () => {
  ...
  if (_pendingDeepLink) {
    ...
    handleDeepLink(
      `${MOOR_PROTOCOL}://${queued.kind}/${encodeURIComponent(queued.name)}` +
...
function registerDeepLinkProtocol() {
  try {
    if (process.defaultApp && process.argv.length >= 2) {
      ...
      app.setAsDefaultProtocolClient(MOOR_PROTOCOL, process.execPath, [path.resolve(process.argv[1])])
    } else {
      app.setAsDefaultProtocolClient(MOOR_PROTOCOL)
    }
  } catch (err) {
...
// second-instance argv. Without the lock a second `moor://` launch spawns a
...
app.whenReady().then(() => {
  ...
  // Win/Linux cold start: the launching moor:// URL is in our own argv.
