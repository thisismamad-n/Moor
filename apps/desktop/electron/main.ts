...
async function ensureBackend(profile) {
  ...
  entry.connectionPromise = spawnPoolBackend(key, entry).catch(async error => {
    ...
    rememberLog(
      `Moor backend for profile "${key}" failed to start: ${error instanceof Error ? error.message : String(error)}`
    )
...
async function ensureRegistryBackend(connectionId, profile) {
  ...
  if (source.kind === 'local') {
    ...
    localEntry.connectionPromise = spawnPoolBackend(profileKey, localEntry, {
      ...
      rememberLog(
        `Moor backend for profile "${profileKey}" (forced-local) failed to start: ${error instanceof Error ? error.message : String(error)}`
      )
...
async function startMoor() {
  ...
  const connectionPromise = (async () => {
    ...
    // Discover the ephemeral port the child bound to
    const port = await Promise.race([
      waitForDashboardPortAnnouncement(moorProcess, {
...
ipcMain.handle('moor:connection', async (_event, profile) => {
  const connection = await ensureBackend(profile)
...
ipcMain.handle('moor:preview:reach', async (_event, url) => reachablePreviewUrl(String(url || '')))
