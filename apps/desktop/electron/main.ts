...
function readPersistedPoolLimits() {
  try {
    ...
    const fromEnv = clampPoolLimits({
      maxBackends: Number(process.env.MOOR_DESKTOP_POOL_MAX) || undefined,
      idleMs: Number(process.env.MOOR_DESKTOP_POOL_IDLE_MS) || undefined
    })
...
//   * WSL2 IPC stall  = the renderer's `moor:backend:touch` roundtrips
...
const POOL_KEEPALIVE_FRESH_MS = Math.max(
  120_000,
  Number(process.env.MOOR_DESKTOP_POOL_KEEPALIVE_FRESH_MS) || 4 * 60_000
)
...
const BUNDLE_SWAP_RELAUNCH_FLAG = '--moor-bundle-swap-relaunched'
...
function killMoorOwnedVenvDaemons(updateRoot) {
  if (!IS_WINDOWS) {
    return
  }
  ...
  try {
    ...
    holders = (Array.isArray(parsed) ? parsed : [parsed]).filter(p =>
      isMoorOwnedVenvDaemon(p?.ExecutablePath, p?.CommandLine, scriptsDir)
    )
  } catch {
...
async function claimBackendChild(child, command, profile, nonce, outputTail: BackendOutputTail | null = null) {
  ...
  if (decision.action === 'fail') {
    ...
    throw new Error(
      `Moor backend (PID ${child.pid}) died before its identity could be recorded: ${decision.reason}${outputTail?.describe() ?? ''}`
    )
  }
  ...
  if (decision.action === 'degrade') {
    ...
    rememberLog(
      `WARNING: process start marker probe failed for live Moor backend PID ${child.pid}; ` +
  ...
  try {
    ...
    await waitForBackendExit(child)
    throw new Error(
      `Could not persist ownership for the Moor backend: ${error.message}${outputTail?.describe() ?? ''}`
    )
  }
}
...
async function releaseBackendLock(updateRoot, tag) {
  ...
  if (moorProcess && Number.isInteger(moorProcess.pid)) {
    initialPids.push(moorProcess.pid)
  }
  ...
  try {
    ...
    if (scriptHandoff) {
      ...
      if (Number.isInteger(child.pid)) {
        writeUpdateMarker(MOOR_HOME, child.pid, { startedAt: updateStartedAt })
      }
...
async function handOffWindowsBootstrapRecovery(reason) {
  ...
  // The updater invokes the venv's Moor launcher, which in turn requires the
  ...
  // marker-only install through --update dead-ends at "Could not find the moor
  ...
  const updaterArgs = chooseUpdaterArgs(
    {
      hasBootstrapMarker: fileExists(path.join(updateRoot, '.moor-bootstrap-complete')),
      hasVenvMoor: fileExists(venvMoor),
...
function sendPreviewNavCommand(command: 'back' | 'forward' | 'reload') {
  ...
  webContents.send('moor:preview-nav', command)
}
...
async function showPluginCompatNoticeOnce() {
  ...
  try {
    notice = pendingPluginCompatNotice(MOOR_HOME, app.getPath('userData'))
  } catch (err) {
...
function installContextMenuBridge(window: BrowserWindow) {
  window.webContents.on('context-menu', (_event, params) => {
    ...
    if (params.isEditable && params.misspelledWord) {
      window.webContents.send('moor:context-menu-spellcheck', {
...
// contains "moor", avoiding false positives from stale gateway.pid files
// whose PID was recycled by the OS to an unrelated process.
function isMoorProcess(pid) {
  try {
...
function migrateActiveProfileIfMissing() {
  migrateActiveProfileIfMissingPure(DESKTOP_PROFILE_CONFIG_PATH, {
    legacyActivePath: path.join(MOOR_HOME, 'active_profile'),
    moorHome: MOOR_HOME,
    profilesRoot: path.join(MOOR_HOME, 'profiles'),
...
async function rollbackSshBootstrapResult(ssh, result, profile, sshConfig, boundaryError) {
  ...
  try {
    const expected = {
      ...
      moorPath: result.moorPath,
      moorHome: result.moorHome,
    ...
    if (result.platform?.os === 'Windows') {
      await terminateOwnedWindowsDashboardForUpdate(
        ssh,
        { moorPath: result.moorPath, moorHome: result.moorHome, python: result.pythonPath },
        expected
      )
...
async function bootstrapSshConnectionInner(profile, sshConfig, reuseToken, source, metadata, fingerprint, lease) {
  ...
  await fenceManagedSshBootstrapPublication({
    ...
    publish: () => {
      ...
      sshConnections.set(scope, {
        ...
        // per-profile pools also use bare keys. Only startMoor' explicit call
  ...
  return {
    ...connection,
    remoteMoorVersion: result.moorVersion || '',
    ssh: {
      effectiveConfigFingerprint: sshConfig.effectiveConfigFingerprint,
      host: sshConfig.host,
...
async function ensureBackend(profile, opts: { spawnPriority?: LocalBackendSpawnPriority } = {}) {
  ...
  // spawning, so two children never share one profile's MOOR_HOME.
...
function remoteUpdateTargetFromState(state): RemoteUpdateTarget {
  if (!state?.ssh || !state?.moorPath || !state?.moorHome) {
  ...
  return {
    ...
    moorPath: state.moorPath,
    moorHome: state.moorHome,
...
async function openManagedSshUpdateTransport(
  ...
  try {
    const platform: any = await detectRemotePlatform(ssh, config.remoteMoorPath || '')

    if (platform.os === 'Windows') {
      const runtime = platform.moorPath ? platform : await probeWindowsRemote(ssh, config.remoteMoorPath || '')
      ...
      return {
        ...
        target: {
          ...
          platform: 'Windows',
          moorPath: runtime.moorPath,
          moorHome: runtime.moorHome,
          pythonPath: runtime.python
        }
      }
    }

    const moorPath = await remoteLifecycle.locateMoor(ssh, config.remoteMoorPath || '')
    const moorHome = await remoteLifecycle.probeRemoteMoorHome(ssh)

    return {
      close: () => ssh.close(),
      target: { ssh, platform: platform.os, moorPath, moorHome }
    }
  } catch (error) {
    await ssh.close()
    throw error
...
async function drainManagedSshScope(scope) {
  ...
  try {
    ...
    if (state.remotePlatform === 'Windows') {
      await terminateOwnedWindowsDashboardForUpdate(
        state.ssh,
        { moorPath: state.moorPath, moorHome: state.moorHome, python: state.pythonPath },
        expected
      )
    ...
    if (state && state.localPort && state.remotePort && scope.reuseToken) {
      try {
        ...
        if (!forwardClosed) {
          await waitForMoor(`http://127.0.0.1:${state.localPort}`, scope.reuseToken, undefined, 'token')
        } else {
...
async function startMoor() {
  ...
  const connectionPromise = (async () => {
    const connectRemote = async remote => {
      ...
      return createPrimaryRemoteConnection(remote, moorLog.slice(-80), getWindowState())
    }
    ...
    primaryOutputTail.attach(moorProcess)
    ...
    moorProcess.once('exit', (code, signal) => {
      ...
      if (!backendReady) {
        const message = `Moor backend exited before it became ready (${signal || code}).${primaryOutputTail.describe()}`
        updateBootProgress(
          {
            error: message,
            message,
    ...
    // The backend's plugin discovery just ran and refreshed MOOR_HOME/.plugin-compat-report.json.
...
function wireCommonWindowHandlers(win, { zoom = true }: { zoom?: boolean } = {}) {
  ...
  // links arrive via `moor:openExternal`, not here. See window-open-policy.ts.
...
function notifyBrowserPopoutClosed(tabId) {
  ...
  for (const other of BrowserWindow.getAllWindows()) {
    if (!other.isDestroyed()) {
      other.webContents.send('moor:browser-popout:closed', tabId)
    }
  }
}
...
function startHudGameOverlayFeed(win: BrowserWindow) {
  ...
  const push = (state: { active: boolean; app: string }) => {
    if (!win.isDestroyed()) {
      win.webContents.send('moor:hud:game-overlay', state)
    }
  }
  ...
  const enumerate = async () => {
    ...
    if (!reported) {
      reported = true
      console.warn(`[moor] HUD cannot enumerate windows: ${windows.reason}`)
    }
...
function spawnHudWindow(sessionId, profile) {
  ...
  // grab, moor:hud:workspace-transfer temporarily makes the X11 window
...
ipcMain.handle('moor:connection', async (_event, profile, extra) => {
...
ipcMain.handle('moor:connection:for', async (_event, payload) => {
  ...
  // Same single-owner claim as 'moor:connection', keyed by the composite
...
ipcMain.on('moor:connection:active-route', (event, route) => {
  ...
  return remoteRevalidation.run(connectionPromise, async () => {
    const [result] = await Promise.all([
      revalidateRemoteConnection({
        ...
        resetConnection: () => resetMoorConnection({ soft: true }),
...
ipcMain.handle('moor:pool-limits:set', async (_event, raw) => {
...
ipcMain.handle('moor:backend:recycle', async (_event, profile) => {
...
ipcMain.handle('moor:connections:set-launch-mode', async (_event, mode) => {
  assertCanMutateManagedPrimaryRouting()
...
ipcMain.handle('moor:connections:set-last-used', async (_event, id) => {
...
async function probeSshProfileInventory(connection) {
  ...
  const sshConfig = normalizeSshConfig({
    ...
    remoteMoorPath: connection.remoteMoorPath
  })
  ...
  try {
    await ssh.open()
    const profiles = await remoteLifecycle.listRemoteMoorProfiles(ssh)
...
ipcMain.handle('moor:connections:update-managed', async (_event, rawId) => requestManagedSshUpdate(rawId))
...
ipcMain.handle('moor:connections:update-all', async (_event, payload) => {
  const registry = readDesktopConnectionsRegistry()
...
ipcMain.handle('moor:readPluginSource', async (_event: unknown, filePath: unknown) => {
  const { resolvedPath, stat } = await resolveReadableFileForIpc(filePath, {
...
ipcMain.handle('moor:context-menu:edit', (event, command) => {
...
ipcMain.handle('moor:context-menu:copy-image', event => {
...
ipcMain.handle('moor:context-menu:spellcheck', (event, action) => {
...
ipcMain.handle('moor:context-menu:guest-add-word', (_event, payload) => {
...
ipcMain.handle('moor:capturePreview', async (_event, payload) => {
  const guest = electronWebContents.fromId(Number(payload?.webContentsId))
...
// it arrives from `moor desktop --local` or directly on Moor.exe (a
...
ipcMain.handle('moor:resolveFavicon', (_event, url) => resolveFaviconCached(url))
...
// The About page's "Restart Moor" button (shown when bundleSwapPending):
...
ipcMain.handle('moor:app:relaunch', async () => {
...
// ---------------------------------------------------------------------------
const MOOR_PROTOCOL = DEV_SERVER ? 'moor-dev' : 'moor'
...
function registerDeepLinkProtocol() {
  try {
    ...
    rememberLog(`[deeplink] registered ${MOOR_PROTOCOL}:// handler`)
  } catch (err) {
...
app.whenReady().then(() => {
  ...
  // startMoor() finishes resolving the configured backend.
  const primaryProfile = primaryProfileKey()
