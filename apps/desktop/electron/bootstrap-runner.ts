...
function readExistingPinnedCommit(activeRoot: string | null | undefined): string | null {
  ...
  try {
    const raw = fs.readFileSync(path.join(activeRoot, '.moor-bootstrap-complete'), 'utf8')
...
// checkout under ~/.moor/moor-agent. Used as a last-resort fallback when
// the pinned commit can't be fetched from GitHub (e.g. a locally-built desktop
// app stamped to an unpushed HEAD).
function installedAgentInstallScript(moorHome) {
  if (!moorHome) {
    return null
  }

  const candidate = path.join(moorHome, 'moor-agent', 'scripts', installScriptName())
...
function cachedScriptPath(moorHome, commit) {
...
async function resolveInstallScript({
  ...
  try {
    ...
    const installed = installedAgentInstallScript(moorHome)
...
function spawnBash(scriptPath, args, { emit, stageName, abortSignal, moorHome }: any = {}) {
  return new Promise<any>((resolve, reject) => {
...
function buildPosixPinArgs({ installStamp, activeRoot, moorHome, pinCommit = true }) {
  const args = ['--dir', activeRoot, '--moor-home', moorHome]
...
async function fetchManifest({ scriptPath, installerKind, emit, moorHome, activeRoot, installStamp, pinCommit }) {
  const isPosix = installerKind === 'posix'

  const args = isPosix
    ? ['--manifest', ...buildPosixPinArgs({ installStamp, activeRoot, moorHome, pinCommit })]
...
async function runStage({
  ...
  const args = isPosix
    ? [
        ...
        '--non-interactive',
        '--json',
        ...buildPosixPinArgs({ installStamp, activeRoot, moorHome, pinCommit })
      ]
...
async function runBootstrap(opts) {
  ...
  const runLog = openRunLog(logRoot || path.join(moorHome, 'logs'))
  ...
  try {
    ...
    const scriptInfo = await resolveInstallScript({ installStamp, sourceRepoRoot, moorHome, emit })
