...
test('Windows relaunch gate refuses live and uncertain markers before executing the remote runtime', async () => {
  for (const observation of ['LIVE:4242', 'UNCERTAIN']) {
    ...
    const ssh = sshWith(async command => {
      ...
      if (script.includes('Get-Command moor.exe')) {
        return JSON.stringify({
          os: 'Windows',
          arch: 'AMD64',
          moorHome: 'C:\\Users\\alice\\.moor',
          moorPath: 'C:\\Moor\\moor.exe',
          python: 'C:\\Moor\\python.exe'
        })
      }

      if (script.includes('.moor-update-in-progress')) {
        return observation
      }
    ...
    assert.equal(
      scripts.some(script => script.includes('moor_cli.windows_ssh_runtime')),
      false
    )
  }
})
...
test('Windows relaunch gate uses strict install-wide marker parsing and fail-closed PID probing', async () => {
  ...
  await assertWindowsRemoteInstallUpdateClear(ssh, 'C:\\Users\\alice\\.moor\\profiles\\research')
  assert.match(script, /\.moor-update-in-progress/)
...
test('Windows probe validates Moor and Python topology before selection', async () => {
  let script = ''
  ...
  const fallbackJoin = script.indexOf('Join-Path $moorHome')
  ...
  await assert.rejects(
    detectRemotePlatform(
      sshWith(async command => {
        ...
        throw new Error('Moor is not installed on the remote Windows host.')
      })
    ),
    (err: any) => err.kind === 'unsupported-platform' && /Moor is not installed/.test(err.message)
  )
})

test('helper command uses the fixed remote Python entry point and quotes path data', () => {
  const command = helperCommand({ python: "C:\\Program Files\\Moor's\\python.exe" }, 'inspect', [
    'C:\\x y\\moor.exe'
  ])

  const encoded = command.split(' ').pop()!
  const script = Buffer.from(encoded, 'base64').toString('utf16le')
  assert.match(script, /-m' 'moor_cli\.windows_ssh_runtime' 'inspect'/)
  assert.match(script, /Moor''s/)
  assert.match(script, /C:\\x y\\moor\.exe/)
})
...
test('Windows SSH reuse requires the requested remote profile to match the lock', () => {
  ...
  const runtime = { moorPath: lock.moorPath, moorHome: lock.moorHome }
