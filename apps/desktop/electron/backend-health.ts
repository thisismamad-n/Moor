...
export interface MoorReadyOptions {
   ...
   * two very different meanings of a 401 (see `waitForMoorReady`).
   */
...
/**
 * The one factory for the actionable Moor Cloud agent-is-down error, shared by
 ...
 *  - readiness-probe exhaustion in waitForMoorReady().
 *
 ...
 */
export function makeMoorCloudBackendDownError(baseUrl: string, error: unknown): Error | null {
  if (!isMoorCloudAgentUrl(baseUrl)) {
    return null
  }
  ...
  try {
    ...
    // baseUrl is known to parse (isMoorCloudAgentUrl already did); keep the raw
...
/**
 * True when the backend URL points at a Moor-managed Moor Cloud instance
 ...
 */
export function isMoorCloudAgentUrl(baseUrl: string): boolean {
  try {
 ...
 * `isReauthRequired` so startMoor latches; `needsOauthLogin` alone only
...
export async function waitForMoorReady(baseUrl: string, options: MoorReadyOptions): Promise<void> {
  ...
  // When a Moor-managed cloud agent returns a server-side HTTP error
  ...
  const cloudError = makeMoorCloudBackendDownError(baseUrl, lastError)

  if (cloudError !== null) {
    throw cloudError
  }

  throw new Error(`Moor backend did not become ready: ${detail}`)
}
