...
export function markerPath(moorHome) {
  return path.join(moorHome, '.moor-update-in-progress')
}
...
export function readLiveUpdateMarker(
  ...
  const file = markerPath(moorHome)
  let raw
 ...
 *
 * The Tauri-based moor-setup.exe takes several seconds to initialise its
...
export function writeUpdateMarker(
  ...
  const file = markerPath(moorHome)
  const nowMs = now()
  const owner = readLiveUpdateMarker(moorHome, { kill, maxAgeMs, now: () => nowMs })
...
export function updateHandoffConflict(
  ...
  const owner = readLiveUpdateMarker(moorHome, opts)
