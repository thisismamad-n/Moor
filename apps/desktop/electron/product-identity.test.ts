...
afterEach((): void => {
  delete process.env.MOOR_DESKTOP_VARIANT
  delete process.env.MOOR_PAYLOAD_TAG
  delete process.env.MOOR_BUILD_COMMIT
  delete process.env.MOOR_PAYLOAD_VERSION
  vi.resetModules()
})
  ...
  try {
    ...
    const actual: unknown = JSON.parse(
      execFileSync(process.execPath, [file], {
        encoding: 'utf8',
        env: { ...process.env, MOOR_DESKTOP_VARIANT: 'store', MOOR_BUILD_COMMIT: 'a'.repeat(40) }
      })
    )
...
test('nonstable runtime pins userData before the app name can change', async (): Promise<void> => {
  ...
  process.env.MOOR_PAYLOAD_TAG = 'v0.28.0+canary.20260818T000000Z'
  ...
  const paths: Record<string, string> = { appData: root, userData: path.join(root, 'Moor') }
  let name: string = 'Moor'
  ...
  try {
    ...
    assert.equal(paths.userData, path.join(root, 'Moor'))
...
test.each([
  [undefined, 'Moor', 'moor', 'latest', 'canary'],
  ['bundled', 'Moor Agent', 'moor', 'latest', 'canary'],
  ['light', 'Moor Light', 'moor-light', 'light', 'light-canary']
] as const)(
  ...
  async (
    ...
    for (const [tag, commit, expectedCli, expectedChannel] of [
      ...
      process.env.MOOR_BUILD_COMMIT = commit
    ...
    process.env.MOOR_BUILD_COMMIT = 'not-a-sha'
    process.env.MOOR_PAYLOAD_TAG = 'v1.2.3'
...
test('packaging isolates boot metadata and executable names without renaming release artifacts', async (): Promise<void> => {
  ...
  process.env.MOOR_PAYLOAD_TAG = 'v0.28.0'
  process.env.MOOR_BUILD_COMMIT = ''
  ...
  for (const build of ['canary', 'abcdef1234567890abcdef1234567890abcdef12']) {
    process.env.MOOR_PAYLOAD_TAG = build === 'canary' ? 'v0.28.0+canary.20260818T000000Z' : ''
    process.env.MOOR_BUILD_COMMIT = build === 'canary' ? '' : build
    const identity: ProductIdentity = await identityForVariant('bundled')
    ...
    process.env.MOOR_PAYLOAD_VERSION = '0.28.0'
    ...
    assert.deepEqual(config.protocols[0].schemes, ['moor'])
...
test('nonstable builds cannot claim the official Store package', async (): Promise<void> => {
  ...
  delete process.env.MOOR_PAYLOAD_TAG
  process.env.MOOR_BUILD_COMMIT = 'abcdef1234567890abcdef1234567890abcdef12'
  await assert.rejects(identityForVariant('store'), /Store.*stable/)
})
