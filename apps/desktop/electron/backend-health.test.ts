...
test('uses lightweight /api/health for current backends', async () => {
  ...
  await waitForMoorReady('http://127.0.0.1:9000/', {
    token: 'secret-token',
...
test('does not fall back to heavyweight /api/status for transient health failures', async () => {
  ...
  await assert.rejects(
    waitForMoorReady('http://127.0.0.1:9000', {
      fetchPublicJson: async url => {
        calls.push(['public', url])
        throw new Error('Timed out connecting to Moor backend after 15000ms')
      },
      fetchJson: async url => {
        calls.push(['token', url])
      },
...
test('recognizes missing-route shapes only', () => {
  ...
  assert.equal(
    isMissingHealthEndpointError(
      new Error('Expected JSON from /api/health but got HTML. The endpoint is likely missing on the Moor backend.')
    ),
    true
  )
  assert.equal(isMissingHealthEndpointError(new Error('Timed out connecting to Moor backend after 15000ms')), false)
...
test('anonymous gate-shaped 401 falls back to /api/status (backend predates /api/health)', async () => {
  ...
  await waitForMoorReady('http://192.168.1.132:9119', {
    token: null,
    fetchPublicJson: async url => {
      calls.push(['public', url])
...
test('a credentialed 401 fails fast for reauth instead of reporting a dead session ready', async () => {
  ...
  await assert.rejects(
    waitForMoorReady('https://gateway.example', {
      token: 'session-token',
      fetchPublicJson: async () => {
...
test('unsigned OAuth is a terminal reauth failure; a bare needsOauthLogin hint is not', () => {
  ...
  assert.equal(isReauthRequiredError(new Error('Could not reach the remote Moor gateway')), false)
})
...
test('a credentialed probe still uses the 404 fallback for a genuinely missing route', async () => {
  ...
  await waitForMoorReady('https://gateway.example', {
    token: 'session-token',
    fetchPublicJson: async () => {
      throw new Error('public probe must not be used when credentialed')
    },
...
test('isMoorCloudAgentUrl detects cloud agent hosts', () => {
  // Positive cases
  assert.equal(isMoorCloudAgentUrl('https://ares-3009.agents.nousresearch.com'), true)
  assert.equal(isMoorCloudAgentUrl('https://ares-3009.agents.nousresearch.com/api/health'), true)
  assert.equal(isMoorCloudAgentUrl('http://test.agents.nousresearch.com'), true)

  // Negative cases
  assert.equal(isMoorCloudAgentUrl('http://127.0.0.1:9000'), false)
  assert.equal(isMoorCloudAgentUrl('https://gateway.example.com'), false)
  assert.equal(isMoorCloudAgentUrl('https://nousresearch.com'), false)
  assert.equal(isMoorCloudAgentUrl('not-a-url'), false)
})

test('waitForMoorReady surfaces actionable error for cloud agent 503', async () => {
  let attempts = 0
  const currentTime = { value: 0 }

  try {
    await waitForMoorReady('https://ares-3009.agents.nousresearch.com', {
      fetchPublicJson: async () => {
        attempts++
        // Always return 503
    ...
    assert.ok(error.message.includes('Moor Cloud agent'), `unexpected message: ${error.message}`)
...
test('waitForMoorReady does not cloud-wrap non-cloud 503 errors', async () => {
  const currentTime = { value: 0 }
...
test('makeMoorCloudBackendDownError produces the Cloud shape and preserves cause', () => {
  const err = new Error('upstream unavailable') as any
  err.statusCode = 503
  const result = makeMoorCloudBackendDownError('https://ares-3009.agents.nousresearch.com', err)
  assert.ok(result)
  ...
  assert.ok(result?.message.includes('Moor Cloud agent ares-3009.agents.nousresearch.com is down'))
})

test('makeMoorCloudBackendDownError returns null for a Cloud 401 (routes to reauth)', () => {
  const err = new Error('Unauthorized') as any
  err.statusCode = 401
  assert.equal(makeMoorCloudBackendDownError('https://ares-3009.agents.nousresearch.com', err), null)
})

test('makeMoorCloudBackendDownError returns null for a non-Cloud 503 (generic remote failure)', () => {
  const err = new Error('Service Unavailable') as any
  err.statusCode = 503
  assert.equal(makeMoorCloudBackendDownError('https://gateway.example.com', err), null)
  assert.equal(makeMoorCloudBackendDownError('http://127.0.0.1:9000', err), null)
})

test('makeMoorCloudBackendDownError preserves legacy string-prefix compatibility', () => {
  const result = makeMoorCloudBackendDownError(
