...
function sandbox(tag: string) {
  const home = fs.mkdtempSync(path.join(os.tmpdir(), `moor-handoff-marker-${tag}-`))
  const installRoot = path.join(home, 'moor-agent')
...
function markerStartedAt(home: string): number {
  const [, startedAt] = fs.readFileSync(path.join(home, '.moor-update-in-progress'), 'utf8').split('\n')
...
function runPosix(installRoot: string, startedAt?: string) {
  ...
  if (startedAt === undefined) {
    ...
    env.MOOR_UPDATE_STARTED_AT = startedAt
  }
...
function assertScriptHandoff(run: (installRoot: string, startedAt?: string) => ReturnType<typeof spawnSync>) {
  ...
  fs.writeFileSync(path.join(refreshed.home, '.moor-update-in-progress'), '999999\n1\n')
