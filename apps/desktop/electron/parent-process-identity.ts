export type ParentWatchdogEnv = {
  ...
  /** Spawn tag consumed by moor_cli.process_identity (`v1:<install>:<purpose>:<spawner_pid>:<spawner_create_s>`).
...
export function parentWatchdogEnv(pid: number, startMarker: string | null, nonce: string): ParentWatchdogEnv {
  ...
  env.MOOR_SPAWN = spawnTag(pid, startMarker)
...
/** Build the `MOOR_SPAWN` tag mirrored by moor_cli/process_identity.py.
