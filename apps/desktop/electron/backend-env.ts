 ...
 * straight into `%LOCALAPPDATA%\moor\node` (node.exe at the root, no `bin\`);
 * `scripts/install.sh` and the node-bootstrap helper use the POSIX
 * `$MOOR_HOME/node/bin`. Emit BOTH on every platform so mixed and migrated
 ...
 * `iter_moor_node_dirs()` in moor_constants.py, which the Electron main
 * process cannot import.
 */
function moorManagedNodePathEntries(
  moorHome,
  ...
  const root = pathModule.join(moorHome, 'node')
...
function buildDesktopBackendPath({
  ...
  const moorNodeDirs = moorManagedNodePathEntries(moorHome, { platform, pathModule })
  const venvBin = venvRoot ? pathModule.join(venvRoot, platform === 'win32' ? 'Scripts' : 'bin') : null
  const saneEntries = platform === 'win32' ? [] : POSIX_SANE_PATH_ENTRIES

  return appendUniquePathEntries([moorNodeDirs, venvBin, currentPath, saneEntries], { delimiter })
}

function normalizeMoorHomeRoot(moorHome, { pathModule = pathModuleForPlatform(process.platform) }: any = {}) {
  if (!moorHome) {
    return moorHome
  }
