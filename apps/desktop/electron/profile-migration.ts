...
export interface MigrationDeps {
  ...
  /** Default profile home (`~/.moor`). Default's state.db and gateway.pid live here. */
  moorHome: string
  /** Named-profile root (`~/.moor/profiles`). Does not contain `default`. */
  profilesRoot: string
  ...
  isMoorProcess: (pid: number) => boolean
  now: () => number
...
/**
 * Production layout: default IS `moorHome`; named profiles are children of
 * `profilesRoot`. There is no `profiles/default` directory on a normal install.
 */
export function profileStateDbPath(name: string, moorHome: string, profilesRoot: string): string {
  return name === 'default' ? `${moorHome}/state.db` : `${profilesRoot}/${name}/state.db`
}

export function profileGatewayPidPath(name: string, moorHome: string, profilesRoot: string): string {
  return name === 'default' ? `${moorHome}/gateway.pid` : `${profilesRoot}/${name}/gateway.pid`
}

function resolveMoorHome(profilesRoot: string, moorHome?: string): string {
  if (moorHome) {
    return moorHome
  }

  // Tests that predate moorHome pass only profilesRoot.
...
/**
 * Return the profile names whose gateway.pid file points to a live moor process.
 ...
 *
 * `moorHome` is optional so existing call sites that only pass `profilesRoot`
 * still work: it is derived as the parent of `…/profiles`.
 */
export function findRunningGatewayProfiles(
  profilesRoot: string,
  allProfiles: string[],
  deps: Pick<MigrationDeps, 'existsSync' | 'readFileSync' | 'isMoorProcess'> & { moorHome?: string }
): string[] {
  const moorHome = resolveMoorHome(profilesRoot, deps.moorHome)
  const running: string[] = []

  for (const name of allProfiles) {
    const pidFile = profileGatewayPidPath(name, moorHome, profilesRoot)
...
export function decideMigration(
  ...
  for (const name of candidates) {
    const s = score(profileStateDbPath(name, deps.moorHome, deps.profilesRoot))
...
/** Default is always a candidate; it is `$MOOR_HOME`, not `$MOOR_HOME/profiles/default`. */
...
export function migrateActiveProfileIfMissing(desktopProfileConfigPath: string, deps: MigrationDeps): boolean {
  ...
  // launches `moor --profile default` and is worse than writing nothing
