...
function ownershipEntry(overrides: Partial<BackendIdentity> = {}) {
  return { command: 'moor serve --port 0', ...identity(overrides) }
}
...
test('backend identity check matches only serve and dashboard invocation shapes', () => {
  assert.equal(backendCommandMatches('/venv/bin/moor serve --port 0'), true)
  assert.equal(backendCommandMatches('python -m moor_cli.main dashboard --no-open'), true)
  assert.equal(backendCommandMatches('/venv/bin/moor --profile work serve --port 0'), true)
  assert.equal(backendCommandMatches('"C:\\Moor Runtime\\moor.exe" dashboard --no-open'), true)
  assert.equal(backendCommandMatches('moor chat --query serve'), false)
