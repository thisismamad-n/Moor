 ...
 * (`persist:moor-remote-oauth`) — the jar was keyed on the auth *mode*, not
 ...
 * setup: one box, two dashboards) fought over the same `moor_session*`
 ...
 *     cascade deliberately shares one jar with the Moor Portal session.
...
export const LEGACY_OAUTH_PARTITION = 'persist:moor-remote-oauth'
