 ...
 *   1. MOOR_GIT_BASH_PATH env var override
 *   2. PortableGit under %LOCALAPPDATA%\moor\git\ (install.ps1)
...
export function findGitBash(opts: GitBashOptions): string | null {
  ...
  // Respect MOOR_GIT_BASH_PATH if set (mirrors tools/environments/local.py:_find_bash).
  const gitBashPath = env.MOOR_GIT_BASH_PATH
  ...
  if (localAppData) {
    candidates.push(joinWin(localAppData, 'moor', 'git', 'bin', 'bash.exe'))
    candidates.push(joinWin(localAppData, 'moor', 'git', 'usr', 'bin', 'bash.exe'))
  }
