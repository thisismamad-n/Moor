...
async function probeWindowsRemote(ssh, explicitMoorPath = '') {
  ...
  const script = [
    ...
    '$candidates+=(Join-Path $moorHome "moor-agent\\venv\\Scripts\\moor.exe")',
    '$candidates+=(Join-Path $HOME "moor-agent\\.venv\\Scripts\\moor.exe")',
    '$moor=$candidates|Where-Object{Test-Path -LiteralPath $_ -PathType Leaf}|Select-Object -First 1',
    ...
    '$python=Join-Path (Split-Path $moor) "python.exe"',
