...
const READY_RE = /^MOOR_(?:BACKEND|DASHBOARD)_READY port=(\d+)/gm
...
async function probeWindowsRemote(ssh, explicitMoorPath = '') {
  const explicit = psLiteral(explicitMoorPath)
  ...
  const script = [
    ...
    'if(-not $moorHome){$moorHome=Join-Path $env:LOCALAPPDATA "moor"}',
    'Assert-NoReparse $moorHome $true',
    '$candidate=[IO.Path]::Combine($moorHome, "moor-agent\\venv\\Scripts\\moor.exe")',
    ...
    '$profileCandidate=[IO.Path]::Combine($HOME, "moor-agent\\.venv\\Scripts\\moor.exe")',
    ...
    '$fallbackHomeCandidate=Join-Path $moorHome "moor-agent\\venv\\Scripts\\moor.exe"',
    '$fallbackProfileCandidate=Join-Path $HOME "moor-agent\\.venv\\Scripts\\moor.exe"',
    '$candidates=@()',
    'if($explicit){$candidates+=$explicit}',
    '$cmd=Get-Command moor.exe -ErrorAction SilentlyContinue',
    ...
    '$moor=$null',
    'foreach($candidate in $candidates){Assert-NoReparse $candidate $true;$candidatePython=[IO.Path]::Combine([IO.Path]::GetDirectoryName($candidate), "python.exe");Assert-NoReparse $candidatePython $true;try{$item=Get-Item -LiteralPath $candidate -Force -ErrorAction Stop;if(($item.Attributes -band [IO.FileAttributes]::ReparsePoint) -eq 0 -and -not $item.PSIsContainer){$moor=$item.FullName;break}}catch [Management.Automation.ItemNotFoundException]{continue}}',
    'if(-not $moor){throw "Moor is not installed on the remote Windows host."}',
    'Assert-NoReparse $moor $false',
    'if($explicit -and $moor -ne $explicit){throw "The configured Moor path is not an executable file."}',
    '$python=[IO.Path]::Combine([IO.Path]::GetDirectoryName($moor), "python.exe")',
    'Assert-NoReparse $python $false',
    '[ordered]@{os="Windows";arch=$env:PROCESSOR_ARCHITECTURE;moorHome=$moorHome;moorPath=$moor;python=$python}|ConvertTo-Json -Compress'
  ].join(';')

  return JSON.parse((await ssh.exec(powerShellCommand(script))).trim())
}

function windowsUpdateMarkerProbeCommand(moorHome) {
  const script = [
    '$ErrorActionPreference="Stop"',
...
public static class MoorMarkerNoFollow {
...
`,
    ...
    '}',
    '}',
    `$moorHome=${psLiteral(moorHome)}`,
    '$installRoot=$moorHome',
    '$parent=Split-Path -Parent $moorHome',
    'if((Split-Path -Leaf $parent) -ieq "profiles"){$installRoot=Split-Path -Parent $parent}',
    '$marker=Join-Path $installRoot ".moor-update-in-progress"',
    '$result="UNCERTAIN"',
    '$stream=$null;$memory=$null',
    'try{',
    'Assert-NoReparse $marker $true',
    'if(-not (Test-Path -LiteralPath $marker -PathType Leaf)){$result="CLEAR"}else{$stream=[MoorMarkerNoFollow]::OpenRead($marker)',
 ...
 */
async function assertWindowsRemoteInstallUpdateClear(ssh, moorHome) {
  let observation = ''

  try {
    observation =
      String(await ssh.exec(windowsUpdateMarkerProbeCommand(moorHome)))
        .replace(/^\uFEFF/, '')
        .trim()
        .split(/\r?\n/)
        .pop() || ''
  } catch (cause) {
    const error: any = new Error('Could not prove that the remote Moor install is clear for SSH startup.')
  ...
  const error: any = new Error(
    live
      ? `Remote Moor update process ${live[1]} is still running; SSH startup is paused.`
      : 'The remote Moor update marker is unreadable or malformed; refusing SSH startup.'
  )
...
async function detectRemotePlatform(ssh, explicitMoorPath = '') {
  try {
  ...
  try {
    return await probeWindowsRemote(ssh, explicitMoorPath)
  } catch (cause: any) {
...
function helperCommand(runtime, operation, args = []) {
  const argv = [runtime.python, '-m', 'moor_cli.windows_ssh_runtime', operation, ...args]
...
function atomicWindowsSpawnCommand(runtime, reservation: any = {}) {
  const argv = [runtime.python, '-m', 'moor_cli.windows_ssh_runtime', 'spawn']
  const helper = operation => [runtime.python, '-m', 'moor_cli.windows_ssh_runtime', operation]

  const script = [
    '$ErrorActionPreference="Stop"',
    `$moorHome=${psLiteral(runtime.moorHome)}`,
    '$installRoot=$moorHome',
    '$parent=Split-Path -Parent $moorHome',
    'if((Split-Path -Leaf $parent) -ieq "profiles"){$installRoot=Split-Path -Parent $parent}',
    '$marker=Join-Path $installRoot ".moor-update-in-progress"',
    ...
    reservation.ownershipId
      ...
      : '  if($LASTEXITCODE -ne 0){exit $LASTEXITCODE}',
    reservation.ownershipId
      ? `  $spawned=$spawnLines[-1]|ConvertFrom-Json; $lock=[ordered]@{schemaVersion=2;protocolVersion=1;ownershipId=${psLiteral(reservation.ownershipId)};spawnNonce=${psLiteral(reservation.spawnNonce)};pid=[int]$spawned.pid;creationTimeNs=[string]$spawned.creationTimeNs;port=0;profile=${psLiteral(reservation.profile)};moorPath=${psLiteral(reservation.moorPath)};moorHome=${psLiteral(reservation.moorHome)};tokenFingerprint=${psLiteral(reservation.tokenFingerprint)};startedAt=${psLiteral(reservation.startedAt)}}|ConvertTo-Json -Compress; ` +
...
function validLock(lock, ownershipId) {
  ...
  return Boolean(
    ...
    typeof lock.moorPath === 'string' &&
    typeof lock.moorHome === 'string'
  )
}
...
function reusableWindowsLock(lock, state, profile, reuseToken, runtime) {
  return Boolean(
    ...
    lock.moorPath === runtime.moorPath &&
    lock.moorHome === runtime.moorHome
  )
}
...
function windowsLockMatchesManagedUpdateScope(lock, expected) {
  return Boolean(
    ...
    lock.moorPath === expected.moorPath &&
    lock.moorHome === expected.moorHome
  )
}
...
async function connectWindowsRemote(deps) {
  ...
  assertBootstrapNotSuperseded(signal)
  const runtime = await probeWindowsRemote(ssh, remoteMoorPath)
  await assertWindowsRemoteInstallUpdateClear(ssh, runtime.moorHome)
  ...
  runtime.moorPath = inspection.path
  const moorVersion = inspection.version || ''
  rememberLog(`[ssh-lifecycle] remote platform Windows/${runtime.arch}`)
  rememberLog(`[ssh-lifecycle] located moor at ${runtime.moorPath}`)
  ...
  try {
    ...
    spawned = await atomicWindowsSpawn(
      ssh,
      runtime,
      JSON.stringify({ ownershipId, spawnNonce, profile, moorPath: runtime.moorPath }),
      {
        ownershipId,
        spawnNonce,
        profile,
  ...
  try {
    ...
    const baseUrl = `http://127.0.0.1:${localPort}`
    await waitForMoor(baseUrl, token)
    assertBootstrapNotSuperseded(signal)
