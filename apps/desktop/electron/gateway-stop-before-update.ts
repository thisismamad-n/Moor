 ...
 *
 * So we delegate to `moor gateway stop --all`: the CLI discovers every
 ...
 * force-kills survivors — the same logic `moor update`'s
 ...
 */
export function startGatewaysAfterUpdateAbort(moorCliPath: string, deps: StopGatewayBeforeUpdateDeps = {}): boolean {
  return runGatewayLifecycleCommand(moorCliPath, ['gateway', 'start', '--all'], deps)
}
...
function runGatewayLifecycleCommand(moorCliPath: string, args: string[], deps: StopGatewayBeforeUpdateDeps): boolean {
  ...
  if (deps.spy) {
    deps.spy(moorCliPath, args)
  }

  if (!existsSync(moorCliPath)) {
    return false
  }

  try {
    exec(moorCliPath, args, {
