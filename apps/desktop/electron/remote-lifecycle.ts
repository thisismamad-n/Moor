 ...
 *     `MOOR_DASHBOARD_READY port=<n>` readiness line,
 ...
 * the real SshConnection, fetch, adoptServedDashboardToken, and waitForMoor in.
 *
 * The minted MOOR_DASHBOARD_SESSION_TOKEN is the SPAWN credential. After
...
// Resolve the remote moor executable. An EXPLICIT path is honored strictly
...
async function locateMoor(ssh, remoteMoorPath) {
  const resolveLauncher = async (candidate: string) => {
    // Return the candidate path directly. The moor binary or wrapper script
  ...
  if (remoteMoorPath) {
    if (await isExecutable(remoteMoorPath)) {
      return resolveLauncher(remoteMoorPath)
    }

    const err: any = new Error(
      `The Moor path you set is not an executable on the remote host: "${remoteMoorPath}". ` +
        'Check the path (it must be the full path to the `moor` binary on the remote, e.g. ' +
        '~/moor-agent/.venv/bin/moor), or clear it to auto-detect.'
    )

    err.kind = 'moor-not-found'
    throw err
  }
  ...
  try {
    const found = (await ssh.exec(`bash -lc ${shq('command -v moor')}`)).trim()
  ...
  candidates.push('~/.local/bin/moor')
  candidates.push('/usr/local/bin/moor')
  candidates.push('~/.moor/moor-agent/venv/bin/moor')
...
// Probe the resolved binary's version string (first line of `<moor> --version`,
// e.g. "Moor Agent v0.18.2 ..."), or '' on failure. Surfaces WHICH moor a
...
// The MOOR_HOME the remote dashboard will use (explicit env wins, else
// ~/.moor). Recorded in the lockfile so a future reuse can tell it's the same
// state store; best-effort.
async function probeRemoteMoorHome(ssh) {
  try {
    const out = (await ssh.exec('echo "${MOOR_HOME:-$HOME/.moor}"')).trim().split('\n').pop()
...
marker=home/'.moor-update-in-progress'
try:
 ...
 * bytes; it never imports or executes code from the changing Moor checkout.
 ...
 */
async function assertRemoteInstallUpdateClear(ssh, moorHome) {
  const home = assertSafeRemoteHome(moorHome)
  let observation = ''
...
async function listRemoteMoorProfiles(ssh) {
  const home = assertSafeRemoteHome(await probeRemoteMoorHome(ssh))
  ...
  try {
    ...
    const error: any = new Error('Could not list remote Moor profiles.')
    error.kind = 'transient-transport-error'
    error.cause = cause
    throw error
  }
...
function assertSafeRemoteHome(home) {
  ...
  if (!/^(\/|~\/)[A-Za-z0-9._/+-]+$/.test(value) || value.includes('..')) {
    const error: any = new Error('Unsafe remote Moor home.')
...
async function readLockfile(ssh, ownershipId) {
  ...
  for (const field of ['profile', 'moorPath', 'moorHome', 'logPath', 'startedAt']) {
...
async function pidIsOurDashboard(
  ...
  if (!pid || !/^[0-9a-f]{16}$/.test(String(spawnNonce || '')) || !moorPath) {
    return false
  }
...
function buildOwnedStaleTerminationCommand(lock, ownershipId) {
  ...
  const expectedPath = expandRemotePath(lock.moorPath)
  const expectedHome = lock.moorHome ? expandRemotePath(lock.moorHome) : "''"
  ...
  const executableMatch = lock.moorHome
...
expected_path=os.path.expanduser(${py(lock.moorPath)})
moor_home=os.path.expanduser(${py(lock.moorHome)})
expected_entries={expected_path,os.path.join(moor_home,"moor-agent","venv","bin","moor")}
...
// execing Moor. mutexPath is expandRemotePath() output — a complete shell
...
result=None
try:
 result=subprocess.run(["sh","-c",payload,"moor-update-mutex",str(fd)],pass_fds=(fd,),check=False)
finally:
 os.close(fd)
...
function buildSpawnCommand(moorPath, profile, opts: any = {}) {
  ...
  const marker = expandRemotePath(`${remoteInstallRoot(opts.moorHome || '~/.moor')}/.moor-update-in-progress`)

  const updateMutex = expandRemotePath(
    `${remoteInstallRoot(opts.moorHome || '~/.moor')}/.moor-update-in-progress.mutex`
  )
...
async function spawnRemoteDashboard(
  ...
  try {
    ...
    out = await ssh.exec(
      buildSpawnCommand(moorPath, profile, {
        spawnNonce,
        tokenFilePath,
        logPath,
        moorHome,
 ...
 * opened SshConnection, forward/pickLocalPort/waitForMoor, a token-gated
...
async function connect(deps) {
  ...
  const moorPath = await locateMoor(ssh, remoteMoorPath)
  log(`located moor at ${moorPath}`)
  const moorVersion = await probeMoorVersion(ssh, moorPath)

  if (moorVersion) {
    log(`remote moor version: ${moorVersion}`)
  }
  ...
  if (isLockfileSkew(lock)) {
    ...
    const error: any = new Error(
      `The remote ownership record ${lpath} does not match this Moor Desktop build (${lock.reason}). ` +
  ...
  if (lock) {
    ...
    const reusable =
      ...
      lock.moorPath === moorPath &&
      lock.moorHome === moorHome
  ...
  try {
    ...
    assertBootstrapNotSuperseded(signal)
    const baseUrl = `http://127.0.0.1:${localPort}`
    await waitForMoor(baseUrl, spawnToken)
    assertBootstrapNotSuperseded(signal)
