...
async function spawnRemoteDashboard(ssh, { moorPath, profile, token, ownershipId }) {
  ...
  try {
    out = await ssh.exec(buildSpawnCommand(moorPath, profile, { spawnNonce, tokenFilePath, logPath }))
  } catch (error) {
    try {
      await ssh.exec(`rm -f ${expandRemotePath(tokenFilePath)}`)
    } catch {
