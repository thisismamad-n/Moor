 ...
 *
 * The pre-update handoff kills moor-owned venv daemons (the memory plugin's
 * hindsight daemon) so the updater never races a mapped shim. External
 * holders (a user terminal running `moor`, unrelated scripts) must NOT be
...
/**
 * True when a process is a moor-owned venv daemon: its exe lives under
 ...
 * spawns DETACHED — it outlives Moor and holds venv shims mapped).
 */
export function isMoorOwnedVenvDaemon(
