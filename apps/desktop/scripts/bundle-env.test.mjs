...
test('explicit clears beat inherited homes and prevent Windows registry fallback before spawning', async () => {
  const root = mkdtempSync(join(tmpdir(), 'moor-bundle-clear-'))
  const paths = fileURLToPath(new URL('../electron/data-paths.ts', import.meta.url))
  const defaults = { MOOR_HOME: null, MOOR_DATA_DIR_SUFFIX: 'magic-test' }
  try {
    const entry = join(root, 'entry.mjs')
    writeFileSync(entry, `
      import {resolveDesktopMoorHome} from ${JSON.stringify(paths)};
      import {execFileSync} from 'node:child_process';
      let registryReads = 0;
      const home = resolveDesktopMoorHome({
        home: 'C:/Users/test', platform: 'win32', env: process.env,
        readWindowsHome: () => { registryReads++; return 'C:/old-moor'; }
      });
      console.log(JSON.stringify({cleared: process.env.MOOR_HOME, home, registryReads,
        child: execFileSync(process.execPath, ['-p', 'process.env.MOOR_HOME'], {
          env: {...process.env, MOOR_HOME: home}, encoding: 'utf8'
        }).trim()}));
    `)
    ...
    const env = { ...process.env, MOOR_HOME: 'C:/old-moor', LOCALAPPDATA: 'C:/Users/test/AppData/Local' }
    delete env.MOOR_DATA_DIR_SUFFIX
    delete env.MOOR_DESKTOP_USER_DATA_DIR
    const actual = JSON.parse(execFileSync(process.execPath, [outfile], { env, encoding: 'utf8' }))
    expect(actual).toEqual({ cleared: '', home: 'C:\\Users\\test\\AppData\\Local\\moormagic-test',
      child: 'C:\\Users\\test\\AppData\\Local\\moormagic-test', registryReads: 0 })
  } finally {
...
test('applyBundleEnvironment replays the banner semantics for defaults, runtime overrides and clears', async () => {
  const root = mkdtempSync(join(tmpdir(), 'moor-bundle-pure-'))
  const defaults = { MOOR_HOME: null, MOOR_DATA_DIR_SUFFIX: 'baked', MOOR_GUEST_ONBOARDING: '1' }
  const keys = Object.keys(defaults)
  const base = { MOOR_HOME: 'runtime', MOOR_DATA_DIR_SUFFIX: 'explicit' }
  try {
    ...
    delete env.MOOR_GUEST_ONBOARDING
    ...
    expect(viaFunction).toEqual({
      MOOR_HOME: '', MOOR_DATA_DIR_SUFFIX: 'explicit', MOOR_GUEST_ONBOARDING: '1',
    })
  } finally {
    rmSync(root, { recursive: true, force: true })
  }
...
test('baked defaults precede imported module initialization and reach children without overriding explicit env', async () => {
  const root = mkdtempSync(join(tmpdir(), 'moor-bundle-env-'))
  const defaults = { MOOR_GUEST_ONBOARDING: '1', MOOR_DATA_DIR_SUFFIX: 'magic-test', MOOR_SHARED_AUTH_DIR: 'a=b "q"\n$(no)', MOOR_SKIP_INTRO: '' }
  ...
  try {
    ...
    const entry = join(root, 'entry.mjs')
    writeFileSync(entry, `import {values} from './reader.mjs'; import {execFileSync} from 'node:child_process'; console.log(JSON.stringify({values, child: execFileSync(process.execPath, ['-p', 'process.env.MOOR_DATA_DIR_SUFFIX'], {encoding:'utf8'}).trim()}));`, 'utf8')
    const outfile = join(root, 'bundle.mjs')
    await build({ entryPoints: [entry], bundle: true, platform: 'node', format: 'esm', outfile, banner: { js: environmentDefaultsBanner(JSON.stringify(defaults)) } })
    const run = extra => JSON.parse(execFileSync(process.execPath, [outfile], { env: { ...env, ...extra }, encoding: 'utf8' }))
    expect(run({})).toEqual({ values: defaults, child: defaults.MOOR_DATA_DIR_SUFFIX })
    expect(run({ MOOR_DATA_DIR_SUFFIX: '-explicit', MOOR_GUEST_ONBOARDING: '' })).toEqual({ values: { ...defaults, MOOR_DATA_DIR_SUFFIX: '-explicit', MOOR_GUEST_ONBOARDING: '' }, child: '-explicit' })
    for (const bad of ['[]', 'null', '{"BAD-NAME":"x"}', '{"MOOR_HOME":1}', '{"MOOR_HOME":"\\u0000"}',
      '{"NODE_OPTIONS":"--require=evil"}', '{"PATH":null}', '{"MOOR_PYTHON":"/untrusted/python"}']) {
