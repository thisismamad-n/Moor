...
const UPSTREAM_URL = 'https://github.com/thisismamad-n/Moor.git'
...
const LEGACY_STORAGE_KEY = 'moor.desktop.inflightTurnJournal.v1'
const LEGACY_MIGRATION_KEY = 'moor.desktop.inflightTurnJournal.v2.migrated'
...
async function verifyInteractiveSurfaces(cdp, timed, measure, label) {
  ...
  const version = await timed(`version.ipc.${label}`, () => cdp.eval('window.moorDesktop.getVersion()'))
...
async function withTemporarySandbox(label, body) {
  const sandbox = mkdtempSync(join(tmpdir(), `moor-short-session-${label}-`))
...
async function executeRunInSandboxAttempt(target, index, warmup, mock, output, sandbox, attempt) {
  ...
  const moorHome = join(sandbox, 'moor-home')
  const userData = join(sandbox, 'electron-user-data')
  const desktopLog = join(moorHome, 'logs', 'desktop.log')
  ...
  writeSandboxConfig(moorHome, mock.url)
  ...
  const child = spawn(
    ...
    [target.targetDesktop, `--user-data-dir=${userData}`, `--remote-debugging-port=${port}`],
    {
      ...
      env: sanitizedEnv({
        MOOR_DESKTOP_APP_NAME: `MoorShortSession-${target.label}-${process.pid}-${index}-${warmup ? 'w' : 'm'}-${attempt}`,
        MOOR_DESKTOP_MOOR_ROOT: target.targetRoot,
        MOOR_DESKTOP_IGNORE_EXISTING: '1',
        MOOR_DESKTOP_USER_DATA_DIR: userData,
        MOOR_HOME: moorHome,
...
async function main() {
  ...
  const ephemeralRoot = mkdtempSync(join(tmpdir(), 'moor-short-session-ab-'))
  const prepared = []
