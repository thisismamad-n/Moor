 ...
 *
 * Stamps the Moor icon + identity onto the packed Windows Moor.exe via
 * rcedit (delegated to set-exe-identity.mjs). This runs for EVERY packed build
 * — first install, `moor desktop`, the installer's --update rebuild, and a
 ...
 *   - packager.appInfo.productFilename: the exe basename (e.g. 'Moor')
 */
...
export default async function afterPack(context) {
  ...
  const productName = context.packager?.appInfo?.productFilename || 'Moor'
  ...
  try {
    ...
    console.warn(`[after-pack] exe identity stamp failed (${err.message}); Moor.exe keeps the stock Electron icon`)
  }
}
