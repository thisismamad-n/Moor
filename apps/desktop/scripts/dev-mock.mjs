...
function createSandbox() {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), `moor-dev-mock-${Date.now()}`))
  ...
  return { root, moorHome, userDataDir, cleanup: () => fs.rmSync(root, { recursive: true, force: true }) }
}

function writeMockConfig(moorHome, mockUrl) {
  fs.writeFileSync(
    path.join(moorHome, 'config.yaml'),
...
`,
  ...
  fs.writeFileSync(path.join(moorHome, '.env'), 'MOCK_API_KEY=e2e-mock-key\n', 'utf8')
}
...
async function main() {
  ...
  const env = {
    ...
    MOOR_DESKTOP_APP_NAME: `MoorDevMock-${Date.now()}`,
  }
