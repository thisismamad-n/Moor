#!/usr/bin/env node
// set-exe-identity.cjs — stamp the Moor icon + version metadata onto the
// built Moor.exe using rcedit, completely decoupled from electron-builder's
// signing path.
//
// WHY THIS EXISTS
// ---------------
...
// skips rcedit, so the unpacked Moor.exe keeps the stock Electron icon and
...
//   node scripts/set-exe-identity.cjs <path-to-Moor.exe>
//
...
async function stampExeIdentity(exe, desktopRoot = path.resolve(__dirname, '..')) {
  ...
  console.log('[set-exe-identity] done — Moor icon + identity stamped')
}
