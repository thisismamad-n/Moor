...
// no longer bundles the Moor Agent Python payload (that's fetched at first
// launch via install.ps1 / install.sh, per the Phase 1 thin-installer flow).
const APP = (() => {
  if (PLATFORM === 'darwin') {
    const appPath = path.join(RELEASE_ROOT, `mac-${ARCH}`, 'Moor.app')
    return {
      appPath,
      binary: path.join(appPath, 'Contents', 'MacOS', 'Moor'),
  ...
  if (PLATFORM === 'win32') {
    ...
    return {
      appPath: unpacked,
      binary: path.join(unpacked, 'Moor.exe'),
  ...
  return {
    appPath: unpacked,
    binary: path.join(unpacked, 'Moor'),
    resourcesPath: path.join(unpacked, 'resources'),
    asarPath: path.join(unpacked, 'resources', 'app.asar'),
    unpackedDistIndex: path.join(unpacked, 'resources', 'app.asar.unpacked', 'dist', 'index.html')
  }
...
// Default MOOR_HOME for non-sandboxed runs -- matches main.ts's
// resolveMoorHome(). On Windows it's %LOCALAPPDATA%\moor; elsewhere
// it's ~/.moor. The fresh-install sandbox launchFresh() sets its own
// MOOR_HOME and never touches this.
const DEFAULT_MOOR_HOME = (() => {
  if (PLATFORM === 'win32' && process.env.LOCALAPPDATA) {
    return path.join(process.env.LOCALAPPDATA, 'moor')
  }
  return path.join(os.homedir(), '.moor')
})()
const VENV_ROOT = path.join(DEFAULT_MOOR_HOME, 'moor-agent', 'venv')
...
function ensurePackagedApp() {
  if (process.env.MOOR_DESKTOP_SKIP_BUILD === '1' && exists(APP.binary)) {
    return
  }
...
function resolveDmgPath() {
  if (!exists(RELEASE_ROOT)) {
    return path.join(RELEASE_ROOT, `Moor-${PACKAGE_JSON.version}-${ARCH}.dmg`)
  }

  const prefix = `Moor-${PACKAGE_JSON.version}`
  const candidates = fs
  ...
  return candidates.length > 0
    ...
    : path.join(RELEASE_ROOT, `Moor-${PACKAGE_JSON.version}-${ARCH}.dmg`)
}

function resolveNsisPath() {
  // electron-builder NSIS artifactName template is 'Moor-${version}-${os}-${arch}.${ext}'
...
function ensureDmg() {
  ...
  if (process.env.MOOR_DESKTOP_SKIP_BUILD === '1' && exists(resolveDmgPath())) {
    return
  }
...
function ensureNsis() {
  ...
  if (process.env.MOOR_DESKTOP_SKIP_BUILD === '1' && resolveNsisPath()) {
    return
  }
...
function launchFresh() {
  ...
  console.log(`  MOOR_HOME: ${moorHome}`)
  console.log(`  cwd: ${cwd}`)

  return { runtimeRoot: path.join(moorHome, 'moor-agent', 'venv') }
}

// Validate the packaged bundle matches the thin-installer architecture:
//   - The Moor Agent Python payload is NOT shipped (it's fetched at first
...
function validateBundle() {
  ...
  // present anymore. If a stray ship of moor_cli sneaks back in we want
  // to fail loudly rather than re-introduce the 400MB delta we just removed.
  const staleFactoryMarker = path.join(APP.resourcesPath, 'moor-agent', 'moor_cli', 'main.py')
...
function help() {
  console.log(`Usage:
  npm run test:desktop:existing  # build packaged app, launch with normal PATH/existing Moor
  npm run test:desktop:fresh     # build packaged app, launch with temp userData + MOOR_HOME
