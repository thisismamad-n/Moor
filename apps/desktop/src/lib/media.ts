...
export async function resolveMediaDisplaySrc(path: string): Promise<string> {
  ...
  if (!window.moorDesktop?.readFileDataUrl) {
...
export async function resolveMediaPlaybackSrc(path: string): Promise<string> {
  ...
  if (window.moorDesktop && ['audio', 'video'].includes(mediaKind(path))) {
...
export function mediaGatewayStreamUrl(path: string): string {
  ...
  if (isRemoteGateway()) {
    ...
    return `moor-media://remote/${file}${scope ? `?${scope}` : ''}`
  }
...
export function mediaStreamUrl(path: string): string {
  return `moor-media://stream/${encodeURIComponent(filePathFromMediaPath(path))}`
}
...
// (workspace, skills, ~/.moor/cache, etc.); /api/media is intentionally
...
export async function downloadGatewayMediaFile(
  ...
  if (!window.moorDesktop?.saveGatewayFile) {
