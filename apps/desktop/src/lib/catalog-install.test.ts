...
function route(href: string) {
  ...
  return resolveDeepLinkAction({ kind: url.hostname, name: url.pathname.slice(1), params: Object.fromEntries(url.searchParams) })
}
...
describe('public catalog install links', () => {
  ...
  it('forwards catalog identity, display pin and encoded subdirectory to the plugin confirmation', () => {
    const plugin = { name: 'weather', repo: 'https://github.com/owner/plugins', subdir: 'packages/weather', sha: 'a'.repeat(40) }

    expect(route(pluginCatalogInstallUrl(plugin))).toEqual({
      type: 'plugin-install', repo: `${plugin.repo}#${plugin.subdir}`, catalogName: plugin.name,
      sha: plugin.sha, enable: true, force: false, legacyHint: null
    })
  })
})
