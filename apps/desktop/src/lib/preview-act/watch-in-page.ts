...
export function watchInPage(doc: Document, holder: WatchHolder, stage: WatchStage, label?: string): void {
  const win = doc.defaultView as (Window & { __moorWatch?: Record<string, unknown> }) | null
  ...
  const busy = 25
  const state = win.__moorWatch || (win.__moorWatch = {})
  ...
  const build = () => {
    ...
    const stale = doc.querySelectorAll('moor-watch')

    for (let i = 0; i < stale.length; i++) {
      stale[i].remove()
    }

    const host = doc.createElement('moor-watch')
  ...
  // this up" in dev.
  const stamp = (win as unknown as { __moorWatchTag?: number }).__moorWatchTag
