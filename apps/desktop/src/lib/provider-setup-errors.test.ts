...
describe('isProviderSetupErrorMessage', () => {
  it('matches generic missing-provider copy', () => {
    ...
    expect(isProviderSetupErrorMessage('No Moor provider is configured.')).toBe(true)
    expect(isProviderSetupErrorMessage('set an API key (OPENROUTER_API_KEY) in ~/.moor/.env')).toBe(true)
  })
  ...
  it('does not match bare env var mentions from auxiliary warnings', () => {
    ...
    expect(isProviderSetupErrorMessage('Run `moor setup` or set OPENROUTER_API_KEY.')).toBe(false)
    expect(
      isProviderSetupErrorMessage(
        '⚠ No auxiliary LLM provider configured — context compression will drop middle turns without a summary. Run `moor setup` or set OPENROUTER_API_KEY.'
      )
    ).toBe(false)
