 ...
 * `moor://` deep links and `host.navigate('/path?…')`.
 *
 ...
 *  - `moor://index-network/intent/1` → `/index-network/intent/1` (plugin-scoped)
 *  - `moor://open/my-page?item=x` → `/my-page?item=x` (generic open)
 *  - `/my-page?item=x` / `#/my-page?item=x` (hash-router paths)
 */

export type MoorOpenTarget = string | { href: string } | { path: string; params?: Record<string, string> }
...
export function normalizeMoorOpenString(raw: string): string | null {
  ...
  if (trimmed.startsWith('moor://') || trimmed.startsWith(`${MOOR_PROTOCOL}//`)) {
    try {
      ...
      // moor://index-network/intent/1 → /index-network/intent/1
...
export function resolveMoorOpenPath(target: MoorOpenTarget | null | undefined): string | null {
  if (target == null) {
    return null
  }

  if (typeof target === 'string') {
    return normalizeMoorOpenString(target)
  }
  ...
  if ('href' in target && typeof target.href === 'string') {
    return normalizeMoorOpenString(target.href)
  }

  if ('path' in target && typeof target.path === 'string') {
    const base = normalizeMoorOpenString(target.path)
 ...
 * (`moor://<kind>/<name>?…` → kind/name/params).
 */
export function pathFromMoorDeepLink(kind: string, name: string, params: Record<string, string> = {}): string | null {
  if (!kind || !name) {
    return null
  }

  if (kind === 'open') {
    return resolveMoorOpenPath({ path: `/${name.replace(/^\//, '')}`, params })
  }
  ...
  return resolveMoorOpenPath({ path: `/${kind}/${name.replace(/^\//, '')}`, params })
}

/** Convenience for `moor://open/<name>?…` payloads. */
export function pathFromOpenDeepLink(name: string, params: Record<string, string> = {}): string | null {
  return pathFromMoorDeepLink('open', name, params)
}
