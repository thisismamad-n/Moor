...
// Moor is rendering, and when the two are opposite schemes it takes over
...
function renderedMode(doc: Document): RenderedMode {
  const attr = doc.documentElement.dataset.moorMode
 ...
 * the only carriers of Moor' palette — are dropped so the paste target's
