import { moorApi } from '@/api/client'
import type {
  MoorConnection,
  MoorReadDirResult,
  MoorReadFileTextResult,
  MoorSelectPathsOptions
} from '@/global'
...
function connectionCacheKey(connection: MoorConnection | null) {
  if (!connection) {
    return 'local:'
  }
...
export function desktopFsCacheKey(connection: MoorConnection | null = $connection.get()) {
...
export async function readDesktopDir(path: string): Promise<MoorReadDirResult> {
  if (!isDesktopFsRemoteMode()) {
    return bridge().readDir(path)
  }

  return remoteFsApi<MoorReadDirResult>(fsPath('list', path))
}

export async function readDesktopFileText(path: string): Promise<MoorReadFileTextResult> {
  if (!isDesktopFsRemoteMode()) {
    return bridge().readFileText(path)
  }

  return remoteFsApi<MoorReadFileTextResult>(fsPath('read-text', path))
}
...
export async function readDesktopFileDataUrlLocalFirst(path: string): Promise<string> {
  try {
    const local = await window.moorDesktop?.readFileDataUrl?.(path)
...
export async function selectDesktopPaths(options?: MoorSelectPathsOptions): Promise<string[]> {
  const desktop = bridge()
