...
const emptyMoorFlags = (): MoorAddFlags => ({
  args: [],
  auth: null,
  command: null,
  env: {},
...
function readMoorToken(flags: MoorAddFlags, tokens: string[], index: number): number {
...
function moorEntry(flags: MoorAddFlags): McpImportEntry | null {
...
function fromMoorAdd(tokens: string[]): McpImportEntry | null {
  const flags = emptyMoorFlags()

  for (let i = 3; i < tokens.length; i++) {
    i = readMoorToken(flags, tokens, i)
...
function parseLine(line: string): McpImportEntry[] | null {
  ...
  if (tokens[0] === 'moor' && tokens[1] === 'mcp' && tokens[2] === 'add') {
    const entry = fromMoorAdd(tokens)
