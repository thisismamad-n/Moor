...
describe('free-tier refusals', () => {
  const surface = parseErrorSurface({
    ...
    layer: 'provider',
    message: '  Using Moor without signing in is switched off right now. To sign in: /login. ',
    provider: 'moor',
    retryable: false
  })!

  it('carries the backend sentence and offers the free sign-in, never an OAuth re-login', () => {
    expect(surface.message).toBe('Using Moor without signing in is switched off right now. To sign in: /login.')
    const plan = errorRecoveryPlan(surface)
