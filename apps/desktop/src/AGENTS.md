...
## The desktop is its own chat surface on a `moor serve` backend
...
(`@moor/shared`: `JsonRpcGatewayClient` + WS URL helpers), which the web dashboard also consumes.
The desktop has **no build/runtime dependency on the dashboard frontend**: it spawns a headless
`moor serve` (`headless_backend=True` → `cmd_dashboard` skips `_build_web_ui` and exports
`MOOR_SERVE_HEADLESS=1` so `mount_spa()` disables the SPA even if a stray `web_dist/` exists).
`dashboard` and `serve` share `cmd_dashboard`/`start_server` but neither launches the other. It does
NOT embed `moor --tui` — own composer, transcript, slash pipeline.
...
and ONLY when it does not (older managed install / PATH `moor` not yet updated) rewrites argv to
...
The backend the app spawns is a **pooled `moor serve --port 0` per (connection, profile)**: its
launch home is that profile, `MOOR_DESKTOP=1` is set, and its in-process cron ticker stands down
...
- `src/lib/desktop-slash-commands.ts` is the load-bearing file: `DESKTOP_COMMAND_SPECS` (built-ins
  ...
  `CommandDef` in `moor_cli/commands.py`; the live `commands.catalog` carries it, and
  `src/lib/desktop-slash-registry.json` (regenerate with `scripts/dump_desktop_slash_registry.py`;
  `tests/moor_cli/test_desktop_slash_registry.py` + the vitest file fail on drift) is the offline
...
## Bot Mode (`src/plugins/moor-bots/`) — one bot = ONE canonical forever-chat, identified by NAME

Each bot is a Moor **profile** with a persistent identity. This invariant regressed repeatedly,
...
**There is NO session-id pin.** The old design stored a pointer in `ui_meta['moor-bots'].chat`;
...
in `src/plugins/moor-bots/`: `canonical-chat-registry.test.ts` (tripwire: the open path never
