...
const MOOR_BLUE = '#0053FD'

describe('themeHue', () => {
  it('reads the accent hue that ships', () => {
    // Moor blue. Both palettes sit at this hue — light seeds `#0053fd` and dark
...
describe('the shipped moor accents', () => {
  const cases = [
...
describe('retintTheme', () => {
  ...
  // The real motivating case: Moor blue is legible on GitHub's light sidebar
  ...
  describe('a seed that only works in one mode', () => {
    const blue = retintTheme(nousTheme, MOOR_BLUE)

    it('keeps the picked color where it already passes', () => {
      expect(blue.colors.primary.toLowerCase()).toBe(MOOR_BLUE.toLowerCase())
    })

    it('lightens it for the mode where it does not', () => {
      const dark = blue.darkColors!.primary

      expect(dark.toLowerCase()).not.toBe(MOOR_BLUE.toLowerCase())
    ...
    it('adapts by lightness, holding the hue — so it still reads as the brand', () => {
      const picked = hexToOklch(MOOR_BLUE)!
