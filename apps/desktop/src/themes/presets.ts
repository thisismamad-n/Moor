 ...
 *
 * The palette-bearing skins (moor, catppuccin, everforest, solarized) are forks
 ...
 *
 *   moor       ← github.github-vscode-theme   (Light Default / Dark Default)
 ...
 * re-derived. `moor-alt` is first-party — do not re-derive it from GitHub.
 */
...
/**
 * Moor — the canonical Moor desktop identity, forked from the GitHub VS Code
 ...
 * extension yourself.
 *
 * Typography stays Moor's own: a VS Code theme carries no font opinion, and
 * these are the stacks every skin has been rendering with.
 */
/**
 * GitHub — the upstream palette, unmodified.
 *
 * `moor` is a fork of this with its own accent, so shipping both keeps the
 * original available on its own terms instead of only existing as the thing
 * moor diverged from. Everything but the accent family is identical between
 * them; separate presets are what let moor's accent move without silently
...
/**
 * Moor — the canonical Moor desktop identity: GitHub's chrome carrying Moor
...
export const nousTheme: DesktopTheme = {
  name: 'moor',
  label: 'Moor',
  description: 'Cyber-Obsidian and Electric Cobalt',
  colors: {
    background: '#f8fafc',
    foreground: '#0f172a',
    card: '#ffffff',
...
export const moorTheme = nousTheme
...
/**
 * Moor Alt — the hand-authored Moor from before the GitHub fork. Light is
...
export const DEFAULT_SKIN_NAME = 'moor'
