...
const SKIN_KEY = 'moor-desktop-theme-v2'
const MODE_KEY = 'moor-desktop-mode-v1'
...
const PROFILE_SKINS_KEY = 'moor-desktop-profile-themes-v1'
const PROFILE_MODES_KEY = 'moor-desktop-profile-modes-v1'
// Last active profile, recorded so the boot-time paint can pick that profile's
// theme before the gateway reports which profile actually launched.
const LAST_PROFILE_KEY = 'moor-desktop-active-profile-v1'
...
const RETIRED_SKINS = new Set(['moor-light', 'default', 'gold'])
...
function applyTheme(theme: DesktopTheme, mode: 'light' | 'dark') {
  ...
  root.dataset.moorTheme = skinName
  root.dataset.moorMode = rendered
  ...
  window.moorDesktop?.setTitleBarTheme?.({
    background: chromeBg,
  ...
  try {
    window.localStorage.setItem('moor-boot-background', chromeBg)
    window.localStorage.setItem('moor-boot-color-scheme', rendered)
  } catch {
  ...
  if (typo.fontUrl && !INJECTED_FONT_URLS.has(typo.fontUrl)) {
    ...
    link.dataset.moorThemeFont = 'true'
...
const syncNativeTheme = (pref: ThemeMode, rendered: 'light' | 'dark') =>
  window.moorDesktop?.setNativeTheme?.(pref === 'system' ? 'system' : rendered)
