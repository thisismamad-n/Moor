...
// Moor Portal OAuth), ahead of OpenRouter and the rest of the key catalog.
const API_KEY_OPTIONS: ApiKeyOption[] = [
  ...
  },
  {
    ...
    docsUrl: 'https://github.com/thisismamad-n/Moor#bring-your-own-endpoint',
    placeholder: 'http://127.0.0.1:8000/v1'
  }
]

// Build the FULL API-key provider catalog from the backend model options so the
// onboarding / Providers key form lists every `api_key` provider `moor model`
    ...
    >
      <div
        className={cn(
          ...
          bare
            ? ''
            : 'overflow-hidden rounded-xl border border-(--stroke-moor) bg-(--ui-chat-bubble-background) shadow-moor',
...
export const FEATURED_ID = 'moor'
const SHOW_ALL_KEY = 'moor-onboarding-show-all-v1'
...
export function Picker({ ctx }: { ctx: OnboardingContext }) {
  ...
  // Collapse the secondary providers behind a disclosure whenever Moor Portal
        ...
        {showRest ? (
          <>
            {/* Fireworks leads the expanded list, matching CANONICAL_PROVIDERS
                (Moor → Fireworks), but stays hidden until the user opens it. */}
