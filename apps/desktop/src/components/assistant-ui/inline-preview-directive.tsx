 ...
 * own design.
 *
 * WIDGETS TALK BACK OFF-SCREEN. `window.moor.send(prompt)` (or declarative
 * `data-moor-send` on any clickable element) routes the prompt through the
...
const SIZE_MESSAGE_TYPE = 'moor-inline-preview-size'
const INTENT_MESSAGE_TYPE = 'moor-inline-preview-intent'
...
/** The script that gives the widget its ONE voice: `moor.send(prompt)`.
 ...
 *  wires `data-moor-send` so declarative HTML works with zero script:
 *  `<button data-moor-send="get-price eth">ETH</button>`. */
export function intentScript(token: string): string {
  return (
    ...
    ')},"*");return true}' +
    'window.moor={send:send};' +
    'addEventListener("click",function(e){var el=e.target&&e.target.closest?' +
    'e.target.closest("[data-moor-send]"):null;' +
    'if(el)send(el.getAttribute("data-moor-send")||"")},true)})()</script>'
  )
}
...
export function InlinePreviewDirective({
  ...
  // URL connection — including a same-machine `moor serve` — renders live.)
