...
const ErrorRecoveryActions: FC = () => {
  ...
  // Open Logs reveals the LOCAL Electron profile's MOOR_HOME/logs. On a
  ...
  const openLogs = useCallback(async () => {
    try {
      const root = await window.moorDesktop?.logsRoot?.()
      ...
      const result = await window.moorDesktop?.openDir?.(root)
