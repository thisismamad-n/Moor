...
function sessionsBotsTree() {
  ...
  registerPane('moor-bots:pane', { placement: 'left', hideOnly: true })
  registerPane('workspace', { placement: 'main', uncloseable: true })
  $layoutTree.set(
    split('row', [
      group(['sessions', 'moor-bots:pane'], { active: 'sessions', id: 'g-side' }),
...
describe('hide-only strip tabs', () => {
  it('labels the zone menu rows from the pane string localizer, not the register-time title', () => {
    ...
    expect(hideOnlyZoneTabs('g-side').map(tab => tab.title)).toEqual(['Сеансы', 'moor-bots:pane'])
  })

  it('hides and shows a chrome tab, keeping the pane in the tree', () => {
    sessionsBotsTree()

    expect(setStripTabHidden('moor-bots:pane', true)).toBe(true)
    expect($hiddenTreePanes.get()).toContain('moor-bots:pane')
    expect($hiddenStripTabs.get()).toContain('moor-bots:pane')
    // Hidden, not dismissed: the pane stays in the layout tree.
    expect(allPaneIds($layoutTree.get()!)).toContain('moor-bots:pane')

    expect(setStripTabHidden('moor-bots:pane', false)).toBe(true)
    expect($hiddenTreePanes.get()).not.toContain('moor-bots:pane')
    expect($hiddenStripTabs.get()).not.toContain('moor-bots:pane')
  })

  it('refuses to hide the zone last visible tab', () => {
    sessionsBotsTree()
    setStripTabHidden('moor-bots:pane', true)
  ...
  it('persists hides and clears them on reveal', () => {
    ...
    const persisted = JSON.parse(window.localStorage.getItem('moor.desktop.hiddenStripTabs.v1') ?? '[]')

    expect(persisted).toContain('moor-bots:pane')
    ...
    revealTreePane('moor-bots:pane')
    expect($hiddenTreePanes.get()).not.toContain('moor-bots:pane')
    expect(window.localStorage.getItem('moor.desktop.hiddenStripTabs.v1')).toBeNull()
  })
  ...
  it('lists the zone hide-only tabs with live hidden state for the menu', () => {
    ...
    expect(hideOnlyZoneTabs('g-side')).toEqual([
      ...
      { hidden: true, id: 'moor-bots:pane', title: 'moor-bots:pane' }
    ])
  ...
  it('excludes hide-only tabs from every close verb', () => {
    ...
    $layoutTree.set(group(['sessions', 'moor-bots:pane', 'session-tile:x'], { active: 'sessions', id: 'g-mixed' }))
