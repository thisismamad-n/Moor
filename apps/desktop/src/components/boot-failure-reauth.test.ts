...
describe('isRemoteReauthError', () => {
  ...
  it('ignores non-auth boot errors and nullish', () => {
    expect(isRemoteReauthError('Moor background process exited during startup.')).toBe(false)
...
describe('deriveProviderShape', () => {
  ...
  it('OAuth shape when the provider is a redirect IDP', () => {
    expect(deriveProviderShape([{ name: 'moor', displayName: 'Moor inc.', supportsPassword: false }])).toEqual({
      isPassword: false,
  ...
  it('mixed deployment keeps generic OAuth copy (not every provider is password)', () => {
    ...
    expect(shape.providerLabel).toBe('Username & Password / Moor inc.')
  })
...
describe('signInLabel', () => {
  ...
  it('OAuth gateway names the provider', () => {
    expect(signInLabel({ url: 'x', isPassword: false, providerLabel: 'Moor inc.' })).toBe(
      'Sign in with Moor inc.'
    )
  })
