...
function failBoot() {
  $desktopBoot.set({
    error: 'Could not connect to Moor gateway',
    fakeMode: false,
    message: 'boot failed',
...
function stubDesktop(config: Record<string, unknown>, overrides: Record<string, unknown> = {}) {
  ...
  return () => Object.defineProperty(window, 'moorDesktop', { configurable: true, value: original })
}
...
describe('BootFailureOverlay', () => {
  ...
  it('recovers a cloud connection through the portal cascade instead of native OAuth', async () => {
    ...
    const restore = stubDesktop(
      ...
      },
      {
        ...
        oauthLogoutConnectionConfig: logout,
        probeConnectionConfig: vi.fn().mockResolvedValue({ providers: [{ id: 'moor', type: 'oauth' }] })
      }
    )
  ...
  it('shows the Moor Cloud down recovery when the backend flags isCloudBackendDown', async () => {
    const restore = stubDesktop(remoteToken)
    $desktopBoot.set({
      error: 'Moor Cloud agent ares-3009.agents.nousresearch.com is down (HTTP 503: server-side fault).',
      fakeMode: false,
    ...
    try {
      ...
      // remote-failure copy.
      expect(await screen.findByText(/Moor Cloud agent is down/i)).toBeTruthy()
