...
import type { MoorGitWorktree, MoorRepoStatus } from '@/global'
...
const EMPTY_WORKTREES: MoorGitWorktree[] = []
...
export const $repoStatusByCwd = atom<Record<string, MoorRepoStatus | null>>({})
export const $repoWorktreesByCwd = atom<Record<string, MoorGitWorktree[]>>({})
...
export const $repoStatus: ReadableAtom<MoorRepoStatus | null> = computed(
...
export const $repoWorktrees: ReadableAtom<MoorGitWorktree[]> = computed(
...
const statusAtomByCwd = new Map<string, ReadableAtom<MoorRepoStatus | null>>()
const worktreesAtomByCwd = new Map<string, ReadableAtom<MoorGitWorktree[]>>()
const $noRepoStatus = atom<MoorRepoStatus | null>(null)
const $noWorktrees = atom<MoorGitWorktree[]>(EMPTY_WORKTREES)

/** Reactive status for one repo cwd (a tile's worktree). Stable per cwd. */
export function repoStatusForCwd(cwd?: null | string): ReadableAtom<MoorRepoStatus | null> {
...
export function repoWorktreesForCwd(cwd?: null | string): ReadableAtom<MoorGitWorktree[]> {
  const key = normalizeCwd(cwd)
...
function setRepoStatusEntry(target: string, status: MoorRepoStatus | null): void {
...
function setRepoWorktreesEntry(target: string, worktrees: MoorGitWorktree[]): void {
...
interface RepoStatusRefreshRequest {
  probe: (cwd: string) => Promise<MoorRepoStatus | null>
  seq: number
  target: string
}
