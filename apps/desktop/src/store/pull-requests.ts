...
import type { MoorBranchPullRequest } from '@/global'
import { scanSessionPullRequests, type SessionInfo } from '@/moor'
...
export const $pullRequestsByBranch = atom<Record<string, MoorBranchPullRequest>>({})
...
export const $prBranchBySession = persistentAtom<Record<string, string>>(
  'moor.desktop.prBranchBySession',
  {},
  Codecs.stringRecord
)
...
const $prScannedSessions = persistentAtom<string[]>('moor.desktop.prScannedSessions', [], Codecs.stringArray)
...
export function pullRequestBucket(pr: MoorBranchPullRequest | undefined): PullRequestBucket {
  if (!pr) {
    return 'none'
  }
