...
// "Backgrounded" = the user isn't on Moor. `document.hidden` only flips when
...
export interface NativeNotificationInput {
   ...
   * `sessionId` (plugins). Same vocabulary as `moor://index-network/intent/1`.
   */
  activate?: string
...
export interface PluginNotificationAction {
  id: string
  label: string
  /** Navigate here on button press (path or `moor://index-network/intent/1`). */
  activate?: MoorOpenTarget
 ...
 *  user is away from Moor — the in-app toast (`host.notify`) covers the
 *  foreground case. */
export function dispatchPluginNativeNotification(pluginId: string, input: PluginNativeNotificationInput): void {
  const activate = resolveMoorOpenPath(input.activate) ?? undefined
  ...
  const actions: NativeNotificationAction[] | undefined = input.actions?.map(action => ({
    activate: resolveMoorOpenPath(action.activate) ?? undefined,
    id: action.id,
    text: action.label
  }))
