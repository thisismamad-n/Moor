 ...
 *  Working in `~/www/moor-agent`, the draft says "moor-agent" constantly
 *  — a "Use skill: moor-agent" pill on every mention is noise (the reported
 *  annoyance that prompted this guard). Boundary containment, not exact
 *  segment equality, so worktree dirs (`moor-agent-suggest`) suppress too. */
 ...
 *  segment of a qualified form (`github/moor-agent-dev`, `plugin:skill`). */
