...
import type { MoorGitBaseBranch, MoorGitBranch } from '@/global'
import { getMoorConfig, type MoorGateway } from '@/moor'
...
import type { ProjectInfo, ProjectsPayload } from '@/types/moor'
...
const PROJECT_SCOPE_KEY = 'moor.desktop.projectScope'
...
async function refreshProjectTreeOn(gateway: MoorGateway): Promise<void> {
...
async function refreshProjectTreeAcrossProfiles(): Promise<void> {
  ...
  try {
    const res = await window.moorDesktop.api<ProjectTreePayload>({
...
const repoScanStates = new WeakMap<MoorGateway, RepoScanState>()
const scanningGatewayGenerations = new WeakMap<MoorGateway, number>()
...
export async function scanAndRecordRepos(force = false): Promise<void> {
  ...
  try {
    const policy = repoDiscoveryPolicyFromConfig(await getMoorConfig(context.profile))
...
// repo, returning where Moor should start working. Git is the source of
...
export async function listRepoBranches(repoPath: string): Promise<MoorGitBranch[]> {
  const git = desktopGit()
...
export async function listBaseBranches(repoPath: string): Promise<MoorGitBaseBranch[]> {
  const git = desktopGit()
...
export async function revealPath(path: null | string): Promise<void> {
  if (path) {
    await window.moorDesktop?.revealPath?.(path)
  }
}
...
export async function copyPath(path: null | string): Promise<void> {
  if (path) {
    await window.moorDesktop?.writeClipboard?.(path)
  }
}
