...
const { getProfiles } = await import('@/moor')
...
const remoteConn = (over: Partial<MoorConnection> = {}): MoorConnection =>
  ({ baseUrl: 'https://moor-roy.tail.ts.net', mode: 'remote', profile: 'vps-remote', ...over }) as MoorConnection
...
beforeEach(() => {
  ...
  $profiles.set([])
  vi.stubGlobal('window', { moorDesktop: { getConnection } })
