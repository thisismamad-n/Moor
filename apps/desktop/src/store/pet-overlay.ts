...
const OVERLAY_ACTIVE_KEY = 'moor.desktop.pet-overlay-active.v1'
const OVERLAY_BOUNDS_KEY = 'moor.desktop.pet-overlay-bounds.v1'
...
function pushNow(): void {
  window.moorDesktop?.petOverlay?.pushState(currentPayload())
}
...
function openOverlay(request: PetOverlayOpenRequest): void {
  const api = window.moorDesktop?.petOverlay
...
export function restorePetOverlay(): void {
  if (!window.moorDesktop?.petOverlay || !$petOverlayActive.get() || stateUnsubs.length) {
    return
  }
...
export function popInPet(): void {
  ...
  stateUnsubs = []
  $petOverlayActive.set(false)
  void window.moorDesktop?.petOverlay?.close()
}
