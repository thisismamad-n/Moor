...
function dialProfile(
  desktop: NonNullable<typeof window.moorDesktop>,
  profile: string,
...
export function setPrimaryGatewayConnection(connection: Pick<MoorConnection, 'connectionId'> | null): void {
...
export async function ensureActiveGatewayOpen(): Promise<MoorGateway | null> {
  ...
  if (!isOpen(entry.gateway)) {
    ...
    // "Moor gateway is not connected" on the Sessions `+` action (#88880).
