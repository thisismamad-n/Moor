 ...
 * composer with the reply scrolling above it, so Moor can be driven while
...
export const canUseHud = (): boolean =>
  typeof window !== 'undefined' && typeof window.moorDesktop?.hud?.open === 'function'

export function openHud(sessionId?: null | string): void {
  const api = window.moorDesktop?.hud
...
export function resetHudLayout(): void {
  void window.moorDesktop?.hud?.resetLayout?.()
}
...
export const reportHudSession = (sessionId: null | string): void => window.moorDesktop?.hud?.setSession?.(sessionId)
...
export function watchHudState(onClosed?: (sessionId: null | string) => void): () => void {
  const off = window.moorDesktop?.hud?.onChanged?.(({ open, sessionId }) => {
    $hudActive.set(open)
