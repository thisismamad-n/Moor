 ...
 *
 * Local models ship on main behind `--local` (either `moor desktop --local`
 * or the flag on Moor.exe itself). The flag is strict: without it the GUI
...
export const $localModelsEnabled = atom<boolean>(
  typeof window !== 'undefined' && window.moorDesktop?.localModelsEnabled === true
)
