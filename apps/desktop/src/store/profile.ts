...
// /api/profiles/active `current`). "default" is the root ~/.moor. This is the
...
const PROFILE_ORDER_STORAGE_KEY = 'moor.desktop.profileOrder'
...
const PROFILE_COLORS_STORAGE_KEY = 'moor.desktop.profileColors'
...
export async function refreshActiveProfile(): Promise<void> {
  ...
  try {
    const res = await moorApi<ActiveProfileResponse>({
...
// Persist the choice and relaunch the backend under the new MOOR_HOME. The
...
export async function switchProfile(name: string): Promise<void> {
  ...
  await window.moorDesktop.profile.set(name)
}
...
async function resolveConnectionForProfile(profile: string): Promise<MoorConnection | null> {
  const getConnection = window.moorDesktop?.getConnection
...
// previous descriptor.
async function resolveConnectionForAgent(connectionId: string, profile: string): Promise<MoorConnection | null> {
...
const SHOW_ALL_PROFILES_STORAGE_KEY = 'moor.desktop.showAllProfiles'
...
export function selectProfile(name: string): void {
  ...
  void Promise.all([activateOnCurrentSource(target), shouldRememberStartupProfile])
    .then(([, shouldRemember]) => {
      if (shouldRemember) {
        return window.moorDesktop?.profile?.remember(target)
      }
...
async function isLocalDesktopProfile(target: string): Promise<boolean> {
  const getConnectionConfig = window.moorDesktop?.getConnectionConfig
...
// Switch to the default (root ~/.moor) profile — bound to ⌘1.
...
export function touchActiveGatewayBackend(): void {
  ...
  void window.moorDesktop?.touchBackend?.(target).catch(() => undefined)
}
