...
import { type GatewayEvent, LOCAL_CONNECTION_ID, registryBackendScopeKey } from '@moor/shared'
...
const BOT_CHAT_SCOPE_KEY = 'moor.desktop.botChatSessions.v1'
 ...
 *  (moor-agent#90102). The tile bucket is a Local Storage cache, and a
 ...
 * restore re-dials the profile's backend, whose ensure_moor_home() re-creates
 * the profile directory the delete just removed (moor-agent#94235). Same
...
export function dropTilesForProfile(
  ...
  const ownerMatches = (owner: SessionProfileRoute | undefined): boolean => {
    ...
    // would orphan a live conversation (moor-agent#94235). Tiles persisted
