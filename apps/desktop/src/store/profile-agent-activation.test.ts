...
const agentConn = (over: Partial<MoorConnection> = {}): MoorConnection =>
  ({ baseUrl: 'https://homelab.invalid', mode: 'remote', profile: 'research', ...over }) as MoorConnection

const localConn = (over: Partial<MoorConnection> = {}): MoorConnection =>
  ({ baseUrl: '', mode: 'local', profile: 'default', ...over }) as MoorConnection

const getConnection = vi.fn<(profile?: string | null) => Promise<MoorConnection>>()

const getConnectionFor =
  vi.fn<(payload: { connectionId?: null | string; profile?: null | string }) => Promise<MoorConnection>>()
...
beforeEach(() => {
  ...
  vi.stubGlobal('window', { moorDesktop: { getConnection, getConnectionFor }, localStorage })
...
describe('ensureGatewayAgent commit hook (beforeActivate) — the Sessions switcher door (#93937)', () => {
  ...
  it('a descriptor lookup that never answers is bounded, fails open, and releases the mutex', async () => {
    ...
    try {
      getConnectionFor.mockImplementationOnce(() => new Promise<MoorConnection>(() => undefined))
