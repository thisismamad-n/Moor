 ...
 * (`moor:data-url-read-max:*`) and the default/clamp in electron/hardening.ts.
 */
...
export async function refreshDataUrlReadMaxMb(): Promise<number> {
  const api = window.moorDesktop?.dataUrlReadMax
...
if (typeof window !== 'undefined' && window.moorDesktop?.dataUrlReadMax) {
