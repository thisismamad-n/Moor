 ...
 *
 * The archive is the CLI's own `moor profile export` tar.gz (config, skills,
...
import { exportProfileArchive, importProfileArchive } from '@/moor'
...
import type { ProfileDesktopOverlay } from '@/types/moor'
...
const ARCHIVE_FILTERS = [{ extensions: ['tar.gz', 'tgz'], name: 'Moor profile' }]
...
export async function runExportProfileFlow(profile?: string): Promise<null | string> {
  ...
  const pick = window.moorDesktop?.selectSavePath
...
export async function runImportProfileFlow(): Promise<null | string> {
  const paths = await window.moorDesktop?.selectPaths?.({
