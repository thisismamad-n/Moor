    ...
    $desktopOnboarding.set(
      baseState({
        flow: {
          ...
          provider: makeOAuthProvider('moor', 'Moor Portal'),
          start: {
...
describe('device-code poll expiry', () => {
  ...
  function deviceCodeProvider() {
    ...
    return { ...makeOAuthProvider('moor', 'Moor Portal'), flow: 'device_code' as const }
  }
  ...
  it('lapses to an error with actionable guidance when the window expires still pending', async () => {
    vi.useFakeTimers()
    installApiMock(async ({ path }: { path: string }) => {
      if (path === '/api/providers/oauth/moor/start') {
        return deviceStart(2)
      }

      if (path === '/api/providers/oauth/moor/poll/device-sess-1') {
