...
const checkMoorUpdateSpy = vi.fn()
const updateMoorSpy = vi.fn()
const getActionStatusSpy = vi.fn()

vi.mock('@/moor', () => ({
  checkMoorUpdate: (...args: unknown[]) => checkMoorUpdateSpy(...args),
  updateMoor: (...args: unknown[]) => updateMoorSpy(...args),
...
describe('checkBackendUpdates', () => {
  ...
  it('maps the backend /update/check onto the backend status, including commits', async () => {
    setRemote(true)
    checkMoorUpdateSpy.mockResolvedValue({
      install_method: 'git',
      current_version: '0.16.0',
      behind: 2,
      update_available: true,
      can_apply: true,
      update_command: 'moor update',
      message: null,
    ...
    expect(checkMoorUpdateSpy).toHaveBeenCalled()
  ...
  it('is a no-op in local mode (backend check only runs when remote)', async () => {
    ...
    expect(checkMoorUpdateSpy).not.toHaveBeenCalled()
  })
})
...
describe('requestActiveUpdate', () => {
  ...
  beforeEach(() => {
    ...
    updateMoorSpy.mockReset().mockResolvedValue({ ok: true, name: 'update' })
    checkMoorUpdateSpy.mockReset().mockResolvedValue({
      install_method: 'git',
    ...
    ;(globalThis as unknown as { window: unknown }).window = {
      moorDesktop: { updates: { apply: applyClientMock, check: checkClientMock } }
    }
    vi.useRealTimers()
  })
  ...
  it('applies the CLIENT update in local mode, never the backend', async () => {
    ...
    expect(updateMoorSpy).not.toHaveBeenCalled()
  ...
  it('applies the BACKEND update in remote mode', async () => {
    ...
    requestActiveUpdate()
    await vi.waitFor(() => expect(updateMoorSpy).toHaveBeenCalled())
...
describe('explicit update targets', () => {
  ...
  // for updating THIS app — routes here via `moor:open-updates`.
  ...
  it('still defaults to the connected machine when no target is named', async () => {
    ...
    expect($updateOverlayTarget.get()).toBe('backend')
    await vi.waitFor(() => expect(checkMoorUpdateSpy).toHaveBeenCalled())
  ...
  it('keeps the everything-flow for the generic, target-less apply', async () => {
    ...
    await vi.waitFor(() => expect(updateMoorSpy).toHaveBeenCalled(), { timeout: 5000 })
  })
...
describe('applyEverythingUpdate', () => {
  ...
  it('remote mode: updates the backend, then the still-behind client — the stale-GUI gap', async () => {
    ...
    expect(updateMoorSpy).toHaveBeenCalledTimes(1)
  ...
  it('a failed backend leg does not strand the fan-out or the client', async () => {
    setRemote(true)
    $mockConnectionsRegistry.set(registryOf(['local', 'vps']))
    updateMoorSpy.mockRejectedValue(new Error('backend gone'))
...
describe('applyUpdates terminal state', () => {
  ...
  beforeEach(() => {
    ...
    ;(globalThis as unknown as { window: unknown }).window = {
      moorDesktop: { updates: { apply: applyMock } }
    }
    vi.useRealTimers()
  })
  ...
  it('keeps the manual command state for CLI installs with no staged updater', async () => {
    applyMock.mockResolvedValue({ ok: true, manual: true, command: 'moor update' })

    await applyUpdates()

    expect($updateApply.get().stage).toBe('manual')
    expect($updateApply.get().command).toBe('moor update')
    expect($updateOverlayOpen.get()).toBe(true)
    expect(notifySpy).not.toHaveBeenCalled()
  })
  ...
  it('lands on a closeable manual-restart state when the rebuilt sandbox blocks auto-relaunch', async () => {
    ...
    applyMock.mockResolvedValue({
      ...
      manualRestart: true,
      sandboxBlocked: true,
      message: 'Backend updated. Quit and reopen Moor to finish.'
    })
...
describe('applyBackendUpdate recovery', () => {
  ...
  it('waits for the backend to return after the restart drops the connection, then clears the overlay', async () => {
    ...
    updateMoorSpy.mockResolvedValue({ action_id: actionId, ok: true, name: 'update', pid: 1 })
    getActionStatusSpy.mockRejectedValueOnce(new Error('ECONNREFUSED')).mockResolvedValueOnce({
      exit_code: null,
      lines: [`=== moor-update completed ${actionId} ===`],
      name: 'update',
      pid: null,
      running: false
    })
  ...
  it('keeps waiting past the old 45-second cutoff while the update action is running', async () => {
    ...
    updateMoorSpy.mockResolvedValue({ action_id: actionId, ok: true, name: 'moor-update', pid: 1 })

    for (let attempt = 0; attempt < 31; attempt += 1) {
      getActionStatusSpy.mockResolvedValueOnce({
        exit_code: null,
        lines: ['=== moor-update started now ===', `step ${attempt}`],
        name: 'moor-update',
        pid: 1,
        running: true
      })
  ...
  it('treats a successful no-op as complete without waiting for a restart', async () => {
    updateMoorSpy.mockResolvedValue({ ok: true, name: 'moor-update', pid: 1 })
    getActionStatusSpy.mockResolvedValue({
      exit_code: 0,
      lines: ['stale output from another run', '=== moor-update started now ===', '✓ Already up to date!'],
      name: 'moor-update',
      pid: 1,
      running: false
    })
  ...
  it('treats a successful dependency repair as complete without waiting for a restart', async () => {
    ...
    getActionStatusSpy.mockResolvedValue({
      exit_code: 0,
      lines: ['=== moor-update started now ===', '✓ Dependencies repaired!', '✓ Update complete!'],
      name: 'moor-update',
      pid: 1,
      running: false
    })
  ...
  it('waits for current-action completion proof after the backend restarts', async () => {
    ...
    getActionStatusSpy
      ...
      .mockResolvedValueOnce({
        exit_code: null,
        lines: ['Update complete!', `=== moor-update completed ${'c'.repeat(32)} ===`],
        name: 'moor-update',
        pid: null,
        running: false
      })
      .mockResolvedValueOnce({
        exit_code: null,
        lines: ['Update complete!', `=== moor-update completed ${actionId} ===`],
        name: 'moor-update',
        pid: null,
        running: false
      })
  ...
  it('accepts its terminal receipt when a verbose update pushes the start marker out of the log tail', async () => {
    ...
    getActionStatusSpy.mockRejectedValueOnce(new Error('ECONNREFUSED')).mockResolvedValueOnce({
      exit_code: null,
      lines: ['final build output', 'Update complete!', `=== moor-update completed ${actionId} ===`],
      name: 'moor-update',
      pid: null,
      running: false
    })
    ...
    await expect(promise).resolves.toMatchObject({ ok: true })
    expect(getActionStatusSpy).toHaveBeenCalledWith('moor-update', 2000)
  })

  it('proves a pre-action-ID backend reached its requested commit after restart', async () => {
    $backendUpdateStatus.set({
      behind: 2,
      commits: [{ at: 1, author: 'Moor', sha: 'requested-target', summary: 'target' }],
      fetchedAt: 1,
      supported: true,
    ...
    checkMoorUpdateSpy
      ...
      .mockResolvedValueOnce({
        behind: 1,
        can_apply: true,
        commits: [{ at: 2, author: 'Moor', sha: 'newer-commit', summary: 'newer' }],
        current_version: '0.18.2',
        install_method: 'git',
        message: null,
        update_available: true,
    ...
    await expect(promise).resolves.toMatchObject({ ok: true })
    expect(checkMoorUpdateSpy).toHaveBeenCalledTimes(2)
  })
  ...
  it('proves a fast pre-action-ID packaged update by its changed version', async () => {
    ...
    await expect(promise).resolves.toMatchObject({ ok: true })
    expect(checkMoorUpdateSpy).toHaveBeenCalledWith(true)
  })
  ...
  it('resumes action polling after a transient status failure', async () => {
    ...
    getActionStatusSpy
      ...
      .mockResolvedValueOnce({
        exit_code: null,
        lines: ['=== moor-update started now ===', 'still running'],
        name: 'moor-update',
        pid: 1,
        running: true
      })
      .mockResolvedValueOnce({
        exit_code: 0,
        lines: ['=== moor-update started now ===', 'Update complete!'],
        name: 'moor-update',
        pid: 1,
        running: false
      })
  ...
  it('restores the fixed action deadline after reconnecting', async () => {
    updateMoorSpy.mockResolvedValue({ action_id: 'a'.repeat(32), ok: true, name: 'moor-update', pid: 1 })
  ...
  it('shares one in-flight update between concurrent apply requests', async () => {
    ...
    getActionStatusSpy.mockResolvedValue({
      exit_code: 0,
      lines: ['=== moor-update started now ===', '✓ Already up to date!'],
      name: 'moor-update',
      pid: 1,
      running: false
    })
  ...
  it('fails immediately when the update action exits nonzero', async () => {
    ...
    getActionStatusSpy.mockResolvedValue({
      exit_code: 1,
      lines: ['=== moor-update started now ===', 'update failed'],
      name: 'moor-update',
      pid: 1,
      running: false
    })
  ...
  it('surfaces an error when the backend never comes back after the restart', async () => {
    updateMoorSpy.mockResolvedValue({ ok: true, name: 'update', pid: 1 })
    getActionStatusSpy.mockRejectedValue(new Error('ECONNREFUSED'))
    checkMoorUpdateSpy.mockRejectedValue(new Error('ECONNREFUSED'))
...
describe('startUpdatePoller', () => {
  ...
  beforeEach(() => {
    ...
    $updateStatus.set(null)
    ;(globalThis as unknown as { window: unknown }).window = {
      moorDesktop: { updates: { check: checkMock, onProgress: onProgressMock } },
