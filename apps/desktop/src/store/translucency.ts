 ...
 * `@moor/shared/translucency`, which owns the mapping both this store and the
 * main process read.
 *
 ...
 * material underneath the web contents to read (see the `[data-moor-glass]`
 * block in styles.css).
 */
 ...
 * sets `moorDesktop.glassSupported` from `os.release()` so Win10 cannot
...
export const GLASS_SUPPORTED =
  typeof window !== 'undefined' && typeof window.moorDesktop?.glassSupported === 'boolean'
    ? window.moorDesktop.glassSupported
...
export const TRANSLUCENCY_SUPPORTED =
  typeof window !== 'undefined' && typeof window.moorDesktop?.translucencySupported === 'boolean'
    ? window.moorDesktop.translucencySupported
    : isMacPlatform() || isWindowsPlatform()
...
/* Peek: while the user is actively adjusting translucency from Settings, the
   ...
   [data-moor-translucency-peek] rules in styles.css). A counter rather
   than a boolean: a held slider drag and a timed pulse from a picker click
   can overlap. */
const PEEK_ATTR = 'data-moor-translucency-peek'
...
const applyGlassSurfaces = ({ intensity, mode, scope }: TranslucencyState): void => {
  ...
  root.toggleAttribute('data-moor-glass-on', glassLive)
  root.toggleAttribute('data-moor-glass', glassOn)
  root.toggleAttribute('data-moor-clear', clearOn)
  ...
  if (glassOn) {
    root.setAttribute('data-moor-glass-scope', scope)
  } else {
    root.removeAttribute('data-moor-glass-scope')
  }
...
if (typeof window !== 'undefined') {
  ...
  $translucency.subscribe(state => {
    ...
    window.moorDesktop?.setTranslucency?.(state)
  })
