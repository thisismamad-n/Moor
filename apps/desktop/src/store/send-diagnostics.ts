...
// user explicitly clicks Upload → diagnostics.share_moor runs backend-side
// (collect + force-redact + Moor-S3 upload) → the modal shows the private
// view link plus the support handoff (GitHub Issues · Moor Portal Support ·
// Discord).
//
// Consent is per-upload and explicit — no "always allow", mirroring the CLI's
// `moor debug share --moor` confirmation contract. On a remote connection
...
async function collectLocalExtras(): Promise<Record<string, string>> {
  try {
    const logs = await window.moorDesktop?.getRecentLogs?.()
...
export async function confirmSendDiagnostics(): Promise<void> {
  ...
  try {
    ...
    const response = await gateway.request<ShareMoorResponse>(
