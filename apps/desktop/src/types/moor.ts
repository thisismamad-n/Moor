...
export interface OAuthProvider {
   ...
   *  embedded terminal. Null when Moor doesn't know how to remove it. */
...
export interface EnvVarInfo {
  ...
  // in moor_cli/provider_catalog.py). When present, the Keys tab groups by
  // this provider identity — the SAME one `moor model` uses — instead of
...
export type MoorConfigRecord = Record<string, unknown>
...
export interface ModelOptionProvider {
   ...
   *  the `moor model` CLI / onboarding OAuth flow. */
  auth_type?: string
  ...
  /** Moor only: whether the current account is on the free tier. */
  free_tier?: boolean
  /** Moor only: paid models a free-tier user cannot select (shown disabled). */
...
export interface ProfileDesktopOverlay {
  ...
  /** Layout tree (moor.desktop.layoutTree.v2 shape). */
  layoutTree?: unknown
...
// folders. Mirrors moor_cli/projects_db.Project.to_dict().
...
export interface SkillInfo {
  ...
  usage?: number
  /** 'agent' = learned/local (editable), 'bundled' = ships with Moor, 'hub' = installed. */
  provenance?: 'agent' | 'bundled' | 'hub'
}

/** One entry of the built-in optional-skills catalog (optional-skills/ in the
 *  repo) — official skills that ship with Moor but install on demand. */
...
export interface ToolProvider {
   ...
   *  CLI `moor tools` active-provider detection). */
  is_active: boolean
  /** Honest readiness computed server-side (keys ∧ Moor entitlement ∧
 ...
 *  not Moor); elsewhere, driver health from `cua-driver doctor`. `null`
...
export interface ActionStatusResponse {
  ...
  pid: number | null
  running: boolean
  /** moor-update only: durable completion identity recovered from update.log. */
  action_id?: string
  /** moor-update only: summary of the durable update receipt (#91277 bullet 3) —
...
/** Shape of `GET /api/moor/update/check` — the backend's own update state.
...
/** One Moor-approved MCP catalog entry from `GET /api/mcp/catalog`. */
...
export interface ModelAssignmentResponse {
  ...
  base_url?: string
  /** Toolset keys auto-routed through the Moor Tool Gateway as a result of
   *  switching the main provider to Moor. Empty unless provider === 'moor'
