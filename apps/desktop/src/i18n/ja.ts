...
export const ja = defineLocale({
  sessionImport: {
    title: '別のアプリから続ける',
    subtitle: '会話をMoorに取り込み、続きを始めましょう。',
    action: 'セッションを取り込む',
    readingFrom: '読み込み元',
  ...
  boot: {
    ...
    steps: {
      ...
      startingMoorDesktop: 'Moor Desktop を起動中…'
    },
    errors: {
    ...
    failure: {
      ...
      cloudDownTitle: 'Moor Cloud エージェントが停止しています',
      cloudDownDescription:
        'このゲートウェイが接続している Moor 管理のクラウドエージェントがサーバーエラーを返しています。ここから再起動することはできません。ステータスを確認するか、ローカルゲートウェイに切り替えるか、サポートに連絡してください。',
      cloudDownHint:
        '下のボタンから Moor Portal（インスタンスの状態と操作）を開くか、Discord でサポートを受けられます。',
  ...
  sendDiagnostics: {
    title: 'Moor に診断情報を送信',
    privacyNotice:
      'デバッグバンドルを Moor 内部ストレージにアップロードします（公開ペーストではありません）。システム情報（OS、バージョン、プロバイダー、設定済み API キーの種類 — キー自体は含まれません）と、エージェント/ゲートウェイ/デスクトップの完全なログ（各最大 512 KB。会話内容、ツール出力、ファイルパスを含む可能性が高い）が含まれます。シークレットはアップロード前にマスクされます。閲覧できるのは Moor スタッフと許可された Discord モデレーターのみで、14 日後に自動削除されます。',
    upload: 'アップロード',
    uploading: 'アップロード中…',
    cancel: 'キャンセル',
    close: '閉じる',
    ...
    failedHint:
      'ターミナルから `moor debug share --moor` を実行するか、`moor debug share --local` でアップロードせずにレポートを表示することもできます。',
    handoffLead: '続きは次の場所で:',
    links: {
      github: 'GitHub Issues',
      portal: 'Moor Portal サポート',
      discord: 'Discord'
    }
  },
  ...
  settings: {
    ...
    appearance: {
      ...
      tipsTitle: 'アプリ内ヒント',
      tipsDesc:
        'アプリの一部を指す小さな吹き出し。待機中にときどき、また役に立つときは Moor からも表示します。閉じたヒントは二度と表示されません。',
      tipsReset: (count: number) => `閉じた${count}件のヒントを元に戻す`,
      toursTitle: 'ガイドツアー',
      toursDesc: '画面を暗くして各ステップを強調しながら、Moor がアプリを案内します。',
      ...
      pet: {
        title: 'ペット',
        intro:
          'アプリ上に浮かぶ petdex のアニメーションマスコットを採用しましょう。ツール実行中は走り、成功すると喜び、エラーでしょんぼりと、Moor の状態に反応します。',
        restartHint:
          'ペット機能には再起動が必要です。この機能が追加される前に起動したアプリが動作中です。Moor を終了して再度開き、このページに戻ってください。',
        scaleTitle: 'サイズ',
    ...
    gateway: {
      ...
      intro:
        'Moor Desktop はデフォルトで独自のローカルゲートウェイを起動します。別のマシンや信頼できるプロキシの背後で既に動作している Moor バックエンドをこのアプリで制御する場合は、リモートゲートウェイを使用してください。ゲートウェイ接続はマシン単位の設定で、プロファイルは接続したゲートウェイから検出されます。',
      envOverrideTitle: '環境変数がこのデスクトップセッションを制御しています。',
      envOverrideDesc:
        '保存された設定を使用するには MOOR_DESKTOP_REMOTE_URL と MOOR_DESKTOP_REMOTE_TOKEN の設定を解除してください。',
      ...
      remoteUrlDesc:
        'リモートダッシュボードバックエンドのベース URL。/moor などのパスプレフィックスもサポートしています。',
      ...
      sshMoorPathTitle: 'Moor パス（任意）',
      sshMoorPathDesc: 'リモートの moor バイナリへのフルパス。空欄 = 自動検出。',
      sshMoorPathPlaceholder: '自動検出',
      ...
      sshErrNotInstalled:
        'リモートホストに Moor がインストールされていません。リモートでインストールする（curl -fsSL https://hermes-agent.nousresearch.com/install.sh | sh）か、Moor パスを設定してください。',
      sshErrPlatform:
    ...
    localModels: {
      ...
      runtimeInstalledDetail: (tag, backend) =>
        `ビルド ${tag}、${backend} バックエンド。サーバーは Moor が起動・管理します。`,
      ...
      upToDateDetail: (tag, backend) => `llama.cpp ${tag}（${backend}）で動作中——Moor が提供する最新ビルドです。`,
      updateToast: next =>
    ...
    toolsets: {
      ...
      nousIncluded: 'Moor サブスクリプションに含まれています。有効にするには Moor Portal にサインインしてください。',
      nousAuthNeededTitle: 'Moor Portal にサインイン',
      nousAuthNeededMessage: provider =>
        `${provider} は保存されましたが、Moor Portal にサインインするまで有効になりません。`,
      nousAuthSignIn: 'サインイン',
      nousAuthDoneTitle: 'Moor Portal に接続しました',
      nousAuthDoneMessage: 'サブスクリプションのバックエンドが有効になりました。',
      nousAuthFailed: 'Moor Portal のサインインが完了しませんでした',
      ...
      browserRealProfile: {
        ...
        prompt: {
          ...
          body: 'Moor が既定ブラウザプロファイルのスナップショットでブラウジングできるようにすると、サイトはログイン済みの状態で開きます。',
  ...
  skills: {
    ...
    skillArchivedMessage: 'moor curator restore で復元できます。',
  ...
  starmap: {
    ...
    emptyDesc: 'Moor がスキルやメモリを蓄積すると、ここに表示されます。'
  },
  agents: {
    close: 'エージェントを閉じる',
    title: 'スポーンツリー',
  ...
  commandCenter: {
    ...
    pets: {
      ...
      staleBackend: 'ペット機能を使うには Moor を再起動してください。',
      empty: '一致するペットがありません。',
      turnOff: 'オフ',
      turnOn: 'オン',
      installed: 'インストール済み',
    ...
    generatePet: {
      ...
      staleBackend: 'ペットを生成するには Moor を更新してください。',
      backgroundHint: 'このウィンドウは閉じても大丈夫です。完了したら Moor が通知します。',
    ...
    moorActiveSessions: (version, count) => `Moor ${version} · アクティブセッション ${count}`,
  ...
  messaging: {
    ...
    fieldCopy: {
      ...
      MATRIX_USER_ID: { label: 'ボットユーザー ID', placeholder: '@moor:example.org' },
  ...
  profiles: {
    ...
    remoteOverride: {
      ...
      description: 'このプロファイルのセッションは、このパソコンではなく指定したリモートの Moor で実行されます。',
      urlLabel: 'リモートアドレス',
  ...
  sidebar: {
    ...
    projects: {
      ...
      staleBackend:
        'プロジェクトを作成するには Moor バックエンドを更新してください。バックエンドがこのデスクトップアプリより古いです（設定 → 更新 → バックエンド）。',
      deleteConfirm:
        'Moor から保存済みプロジェクトを削除します。ファイル・git リポジトリ・ワークツリーはそのまま残ります。',
      startWork: '新しいワークツリー',
  ...
  updates: {
    ...
    applyingBody:
      'Moor アップデーターが独自のウィンドウで引き継ぎ、完了後に自動的に Moor を再度開きます。更新中はご自分で Moor を開き直さないでください。',
  ...
  modelPicker: {
    ...
    pro: 'Pro',
    proNeedsSubscription: 'Pro モデルには有料の Moor サブスクリプションが必要です。',
    free: '無料',
    freeTier: '無料プラン',
  ...
  preview: {
    ...
    web: {
      ...
      blankPageBody: '上のアドレス欄に入力するか、Moor にページを開くよう頼んでください。',
  ...
  assistant: {
    ...
    approval: {
      ...
      alwaysDescription: pattern =>
        `これにより "${pattern}" パターンが永続的な許可リスト (~/.moor/config.yaml) に追加されます。Moor はこのセッションや将来のセッションで、このようなコマンドについて再度尋ねません。`,
      alwaysAllow: '常に許可'
    },
    clarify: {
  ...
  desktop: {
    ...
    handoff: {
      ...
      timedOut: 'ゲートウェイの待機がタイムアウトしました。`moor gateway` は起動していますか？'
    }
  },
  ...
  tips: {
    ...
    items: {
      ...
      skills: {
        title: '一度教えれば覚えます',
        text: 'スキルは手順書のフォルダで、必要な場面で Moor が自分で読み込みます。'
      },
      messaging: {
        title: 'デスクを離れても Moor',
      ...
      profiles: {
        ...
        text: 'それぞれが独自のキー・メモリ・セッションを持つ、別の Moor です。'
      },
      'composer-mentions': {
        title: 'ファイルとコマンド',
