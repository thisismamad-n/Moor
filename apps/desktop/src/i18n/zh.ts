...
export const zh: Translations = {
  ...
  boot: {
    ...
    steps: {
      ...
      startingMoorDesktop: '正在启动 Moor 桌面版…'
    },
    errors: {
      backgroundExited: 'Moor 后台进程已退出。',
      backgroundExitedDuringStartup: 'Moor 后台进程在启动期间退出。',
    ...
    failure: {
      ...
      cloudDownTitle: 'Moor Cloud 代理已宕机',
      cloudDownDescription:
        '此网关连接的 Moor 托管云代理正在返回服务器错误。无法在此处重启——请检查其状态、切换到本地网关或获取支持。',
      cloudDownHint: '使用下方按钮打开 Moor Portal（查看实例状态与操作）或加入 Discord 获取支持。',
  ...
  notifications: {
    ...
    backendOutOfDateMessage: '你的 Moor 后端早于当前桌面构建，可能无法正常工作。请更新以保持一致。',
  ...
  sendDiagnostics: {
    title: '向 Moor 发送诊断信息',
    privacyNotice:
      '这会将调试包上传到 Moor 内部存储（并非公开粘贴板）。内容包括系统信息（操作系统、版本、服务商、已配置的 API 密钥种类 — 绝不包含密钥本身）以及完整的 agent、gateway 和桌面端日志（每个最多 512 KB，很可能包含对话内容、工具输出与文件路径）。上传前会先脱敏。仅 Moor 员工与获准的 Discord 版主可查看，14 天后自动删除。',
    upload: '上传',
    uploading: '上传中…',
    cancel: '取消',
    close: '关闭',
    ...
    failedTitle: '上传失败',
    failedHint:
      '您也可以在终端运行 `moor debug share --moor`，或运行 `moor debug share --local` 在不上传的情况下查看报告。',
    handoffLead: '在以下位置继续讨论:',
    links: {
      github: 'GitHub Issues',
      portal: 'Moor Portal 支持',
      discord: 'Discord'
    }
  },
  ...
  settings: {
    ...
    resetConfirm: '将所有设置恢复为 Moor 默认值？',
    exportFailed: '导出失败',
    resetFailed: '重置失败',
    nav: {
      providers: '提供方',
    ...
    plugins: {
      title: '桌面插件',
      blurb:
        '加载到此应用中的界面扩展——随构建捆绑，或放入 desktop-plugins 文件夹（包括 Moor 编写的插件）。禁用会即时卸载插件并在重启后保持。',
      count: n => `已安装 ${n} 个`,
      openFolder: '打开插件文件夹',
      rescan: '重新扫描',
      reveal: '在文件管理器中显示',
      ...
      agent: {
        title: '智能体插件',
        blurb:
          '你安装到 Moor 后端的插件——工具、技能、MCP 服务器、钩子和斜杠命令。「便携」插件是 Agent Plugins 标准包（技能 + MCP 组合，也可在其他智能体中使用）。开关在新会话中生效。',
        appliesTo: '应用于：',
        empty: '尚未安装智能体插件。',
      ...
      installModal: {
        ...
        agentLabel: '智能体插件',
        desktopLabel: '桌面 UI',
        agentTargetLocal: profile => `安装到 ${profile} 后端（~/.moor/plugins/）`,
    ...
    notifications: {
      ...
      kinds: {
        ...
        input: {
          label: '需要输入',
          description: 'Moor 提出了问题，或需要密码或密钥。'
        },
        turnDone: {
          label: '回复就绪',
          description: 'Moor 在后台时完成了一轮对话。'
        },
        turnError: {
          label: '本轮失败',
          description: '后台回合错误。'
    ...
    appearance: {
      ...
      colorMode: '颜色模式',
      colorModeDesc: '选择固定模式，或让 Moor 跟随系统设置。',
      ...
      tipsTitle: '应用内提示',
      tipsDesc: '指向应用某处的小气泡：空闲时偶尔出现，需要时 Moor 也会给你一条。关掉一条就不再出现。',
      tipsReset: (count: number) => `恢复 ${count} 条已关闭的提示`,
      toursTitle: '引导导览',
      toursDesc: '让 Moor 带你熟悉应用：调暗界面并逐步高亮每个位置。',
      ...
      pet: {
        title: '宠物',
        intro:
          '领养一只悬浮在应用上的 petdex 动画宠物，它会根据 Moor 的状态做出反应——工具执行时奔跑、成功时欢呼、出错时沮丧。',
        restartHint: '宠物功能需要重启——当前运行的应用在此功能加入前启动。请退出并重新打开 Moor，然后回到此处。',
        scaleTitle: '大小',
    ...
    fieldDescriptions: defineFieldCopy({
      ...
      agent: {
        ...
        maxTurns: 'Moor 停止一次运行前工具调用轮次的上限。'
      },
      terminal: {
        cwd: '工具与终端操作的默认项目目录。',
      ...
      fileReadMaxChars: 'Moor 单次文件读取可读取的最大字符数。',
      approvals: {
        mode: 'Moor 如何处理需要显式审批的命令。',
      ...
      updates: {
        nonInteractiveLocalChanges:
          'Moor 从应用内更新时（无终端提示），保留本地源码修改（暂存）或丢弃（放弃）。通过终端更新时始终会询问。'
      }
    }),
    about: {
      ...
      bundleOutOfSyncDesc:
        'Moor 运行时已更新,但桌面应用本身仍是旧构建——在应用更新之前,新的界面功能(如 Bot Mode)不会显示。请运行下方的更新以重新构建应用。如果此警告仍未消除,请从最新的桌面安装程序重新安装。',
      ...
      automaticUpdatesDesc: 'Moor 会在后台自动检查更新，并在有可用更新时通知你。',
    ...
    connections: {
      title: '已注册网关',
      intro: '管理本机以及通过远程、SSH 或 Moor Cloud 连接可访问的每个 Moor 网关。',
      stagedNote:
      ...
      cloudAddHint: '提示：在上方登录 Moor Cloud 可自动发现你的智能体——此表单仅用于手动注册已知的实例 URL。',
      save: '保存连接',
      saving: '保存中…',
      cancel: '取消',
      empty: '尚未注册任何连接。'
    ...
    gateway: {
      ...
      envOverride: '环境变量覆盖',
      intro:
        'Moor Desktop 默认会启动自己的本地网关。当你希望此应用控制另一台机器上或可信代理后的现有 Moor 后端时，可以使用远程网关。网关连接属于本机级设置；profile 是从所连接的网关中发现的。',
      envOverrideTitle: '环境变量正在控制此桌面会话。',
      envOverrideDesc: '取消设置 MOOR_DESKTOP_REMOTE_URL 和 MOOR_DESKTOP_REMOTE_TOKEN 后才会使用下面保存的设置。',
      modeTitle: '连接模式',
      localTitle: '本地网关',
      localDesc: '在 localhost 启动私有 Moor 后端。这是默认方式，并且可离线工作。',
      remoteTitle: '远程网关',
      remoteDesc: '将此桌面外壳连接到远程 Moor 后端。',
      remoteAuthHint: '托管网关使用 OAuth 或用户名密码；自托管网关也可能使用会话 token。',
      cloudTitle: 'Moor Cloud',
      cloudDesc: '只需登录 Moor Cloud 一次，即可从你账户下的智能体中选择——无需粘贴 URL。',
      cloudSignInTitle: 'Moor Cloud',
      cloudSignIn: '登录 Moor Cloud',
      cloudSignedIn: '已登录 Moor Cloud',
      cloudNeedsSignIn: '登录 Moor Cloud 以发现你账户下的智能体。',
      ...
      cloudDiscoverFailed: '无法加载你的 Moor Cloud 智能体',
      cloudConnectFailed: '无法连接到该智能体',
      cloudSignInFailed: 'Moor Cloud 登录失败',
      cloudSignedOutTitle: '已退出 Moor Cloud',
      cloudSignedOutMessage: '已清除 Moor Cloud 会话。',
      ...
      remoteUrlDesc: '远程 dashboard 后端的基础 URL。支持路径前缀，例如 /moor。',
      ...
      savedTitle: '网关设置已保存',
      restartingMessage: 'Moor Desktop 将使用已保存设置重新连接（界面保持打开）。',
      savedMessage: '已保存，下一次重启生效。',
      connectedTo: (baseUrl, version) => `已连接到 ${baseUrl}${version ? ` · Moor ${version}` : ''}`,
      ...
      sshMoorPathTitle: 'Moor 路径（可选）',
      sshMoorPathDesc: '远程 moor 可执行文件的完整路径。留空 = 自动检测。',
      sshMoorPathPlaceholder: '自动检测',
      ...
      sshErrNotInstalled:
        '远程主机上未安装 Moor。请在远程安装（curl -fsSL https://hermes-agent.nousresearch.com/install.sh | sh）或设置 Moor 路径。',
    ...
    localModels: {
      ...
      runtimeInstalledDetail: (tag, backend) => `构建 ${tag}，${backend} 后端。Moor 会为您启动并管理服务器。`,
      ...
      upToDateDetail: (tag, backend) => `正在运行 llama.cpp ${tag}（${backend}）——Moor 提供的最新构建。`,
    ...
    providers: {
      ...
      intro: '使用订阅登录，无需复制 API 密钥。Moor 会在应用中为你完成浏览器登录。',
      connected: '已连接',
      collapse: '收起',
      ...
      localEndpoint: {
        title: '本地 / 自定义端点',
        description: '将 Moor 指向任意 OpenAI 兼容端点（Zyphra、vLLM、llama.cpp、Ollama 等）。'
      },
      loading: '正在加载提供方...'
    },
    sessions: {
    ...
    toolsets: {
      ...
      useBackend: '使用此后端',
      nousIncluded: '包含在 Moor 订阅中；登录 Moor Portal 即可激活。',
      nousAuthNeededTitle: '登录 Moor Portal',
      nousAuthNeededMessage: provider => `已保存 ${provider}，但在登录 Moor Portal 之前不会激活。`,
      nousAuthSignIn: '登录',
      nousAuthDoneTitle: '已连接 Moor Portal',
      nousAuthDoneMessage: '订阅后端现已激活。',
      nousAuthFailed: 'Moor Portal 登录未完成',
      ...
      browserRealProfile: {
        ...
        prompt: {
          title: '让网站保持登录状态',
          body: '让 Moor 使用默认浏览器配置文件的快照进行浏览，网站打开时即已登录。',
  ...
  starmap: {
    ...
    loading: '加载中…',
    emptyTitle: '尚无学习内容',
    emptyDesc: '当 Moor 为你的工作构建技能和记忆时，会显示在这里。',
    share: '分享图谱',
  ...
  commandCenter: {
    ...
    pets: {
      ...
      staleBackend: '请重启 Moor 以使用宠物功能——当前后端版本过旧。',
      empty: '没有匹配的宠物。',
      turnOff: '关闭',
      turnOn: '开启',
      installed: '已安装',
    ...
    generatePet: {
      ...
      staleBackend: '请更新 Moor 以生成宠物。',
      backgroundHint: '你可以关闭此窗口——完成后 Moor 会通知你。',
    ...
    moorActiveSessions: (version, count) => `Moor ${version} · 活跃会话 ${count}`,
    restartGateway: '重启网关',
    openBrowser: '打开浏览器',
  ...
  messaging: {
    ...
    fieldCopy: {
      ...
      MATRIX_USER_ID: { label: 'Bot 用户 ID', placeholder: '@moor:example.org' },
    ...
    platformIntro: {
      ...
      signal: '在可访问的位置运行 signal-cli REST 桥接，然后把 Moor 指向该 URL 和已注册的电话号码。',
      whatsapp: '启动 Moor 自带的 WhatsApp 桥接，首次运行时扫描二维码，然后启用该平台。',
      bluebubbles:
        '在装有 iMessage 的 Mac 上运行 BlueBubbles Server，暴露其 API，然后用服务器密码把 Moor 指向该 URL。',
      ...
      weixin:
        '运行 `moor gateway setup`，选择 Weixin，然后使用个人微信账号扫描并确认二维码。Moor 会通过腾讯 iLink Bot API 连接并保存凭据。',
      qqbot: '在 QQ 开放平台 (q.qq.com) 注册一个应用，复制 App ID 和 Client Secret。',
      api_server:
        '把 Moor 暴露为兼容 OpenAI 的 API。设置一个鉴权密钥，然后把 Open WebUI / LobeChat 等指向 host:port。',
  ...
  profiles: {
    ...
    remoteOverride: {
      ...
      description: '此配置档案中的会话将在你指定的远程 Moor 上运行，而不是这台电脑。',
      urlLabel: '远程地址',
    ...
    deleting: '删除中…',
    createDesc: '配置档案是相互独立的 Moor 环境：各自拥有独立的配置、技能和 SOUL.md。',
    nameLabel: '名称',
    cloneFrom: '克隆来源',
    cloneFromNone: '无（空白）',
  ...
  cron: {
    ...
    newCron: '新建定时任务',
    emptyDescNew: '按 cron 表达式排程一个提示词。Moor 会运行它，并把结果发送到你选择的目的地。',
  ...
  sidebar: {
    ...
    projects: {
      ...
      createFailed: '无法创建项目',
      staleBackend: '请更新 Moor 后端以创建项目——当前后端比桌面应用旧（设置 → 更新 → 后端）。',
      deleteConfirm: '这会从 Moor 中移除已保存的项目。文件、git 仓库和工作树保持不变。',
      startWork: '新建工作树',
  ...
  composer: {
    ...
    placeholderStarting: '正在启动 Moor…',
    placeholderReconnecting: '正在重新连接 Moor…',
    ...
    attachUrlDesc: 'Moor 将抓取该页面并作为本回合的上下文。',
  ...
  updates: {
    ...
    manualTitle: '从终端更新',
    manualBody: '你是从命令行安装的 Moor，因此更新也需要在那里运行。请将此命令粘贴到终端：',
    manualPickedUp: '下次启动 Moor 时会使用新版本。',
    guiSkewTitle: '请更新桌面应用',
    guiSkewBody:
      '后端已更新，但此桌面应用包未更改。请更新或重新安装 Moor 桌面应用（你的 AppImage / .deb / .rpm）以保持一致。',
    copy: '复制',
    copied: '已复制',
    done: '完成',
    applyingBody:
  ...
  install: {
    ...
    backToSetup: '返回',
    failedTitle: '安装失败',
    settingUpTitle: '正在设置 Moor Agent',
    finishingTitle: '正在收尾',
    failedDesc:
      '某个安装步骤失败。在 Windows 上，如果另一个 Moor CLI 或桌面实例正在运行，可能会出现这种情况。请停止正在运行的 Moor 实例后重试。可查看下面的详情或 desktop 日志中的完整记录。',
    activeDesc: '这是一次性设置。Moor 安装器正在下载依赖并配置你的机器。之后启动会跳过此步骤。',
  ...
  onboarding: {
    headerTitle: '开始设置 Moor Agent',
    headerDesc: '连接模型提供方即可开始对话。大多数选项只需一次点击。',
    preparingInstall: 'Moor 正在完成安装。首次运行通常不到一分钟。',
    starting: '正在启动 Moor…',
    ...
    recommended: '推荐',
    connected: '已连接',
    featuredPitch: '一个订阅，300+ 前沿模型 — 运行 Moor 的推荐方式',
    ...
    apiKeyOptions: {
      ...
      local: {
        short: '自托管',
        description: '将 Moor 指向本地或自托管的 OpenAI 兼容端点 (vLLM、llama.cpp、Ollama 等)。'
      }
    },
    backToSignIn: '返回登录',
    getKey: '获取密钥',
    ...
    flowSubtitles: {
      ...
      device_code: '在浏览器中打开验证页面 — Moor 会自动连接',
    ...
    authorizeThere: '请在那里授权 Moor。',
    copyAuthCode: '复制授权码并粘贴到下面。',
    pasteAuthCode: '粘贴授权码',
    reopenAuthPage: '重新打开授权页面',
    autoBrowser: provider => `已在浏览器中打开 ${provider}。请在那里授权 Moor，连接会自动完成，无需复制或粘贴。`,
  ...
  modelPicker: {
    ...
    pro: 'Pro',
    proNeedsSubscription: 'Pro 模型需要付费 Moor 订阅。',
    free: '免费',
    freeTier: '免费层',
  ...
  preview: {
    ...
    largeTitle: '此文件较大',
    largeBody: (label, size) => `${label} 大小为 ${size}。Moor 只会显示前 512 KB。`,
    previewAnyway: '仍然预览',
    ...
    web: {
      ...
      failedToLoad: '预览加载失败',
      tryAgain: '重试',
      restarting: 'Moor 正在重启...',
      askRestart: '让 Moor 重启服务器',
      lookingRestart: taskId => `Moor 正在查找要重启的预览服务器 (${taskId})`,
      restartingTitle: '正在重启预览服务器',
      restartingMessage: 'Moor 正在后台工作。可在预览控制台查看进度。',
      ...
      blankPageBody: '在上方输入地址开始浏览，或让 Moor 打开一个页面。',
      finishedRestarting: message => `Moor 已完成预览服务器重启${message ? `: ${message}` : ''}`,
      failedRestarting: message => `服务器重启失败：${message}`,
      unknownError: '未知错误',
      restartedTitle: '预览服务器已重启',
      reloadingNow: '正在重新加载预览。',
      restartFailedTitle: '预览重启失败',
      restartFailedMessage: 'Moor 无法重启服务器。',
      stillWorking: 'Moor 仍在工作，但还没有收到重启结果。服务器命令可能正在前台运行。',
  ...
  assistant: {
    ...
    approval: {
      ...
      alwaysDescription: pattern =>
        `这会将“${pattern}”模式加入永久允许列表 (~/.moor/config.yaml)。Moor 对类似命令将不再询问，包括当前会话和未来会话。`,
      alwaysAllow: '始终允许'
    },
    clarify: {
      notReady: '澄清请求尚未就绪',
  ...
  prompts: {
    ...
    sudoTitle: '管理员密码',
    sudoDesc: 'Moor 需要你的 sudo 密码来运行特权命令。它只会发送给你的本地 agent。',
  ...
  desktop: {
    ...
    restartToUseSaveImage: '重启 Moor 桌面版后可使用保存图片。',
    restartToSaveImages: '重启 Moor 桌面版以保存图片',
    ...
    handoff: {
      ...
      timedOut: '等待网关超时。`moor gateway` 是否正在运行？'
    }
  },
