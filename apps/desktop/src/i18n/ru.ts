...
export const ru = defineLocale({
  sessionImport: {
    ...
    subtitle: 'Перенесите разговор в Moor и продолжите с того места, где остановились.',
    ...
    chooseHelp: 'Выберите сессию, чтобы прочитать историю перед импортом в Moor.',
    ...
    you: 'Вы',
    snapshot: 'Этот разговор уже есть в Moor. Откройте существующую копию, чтобы продолжить.',
    copyNotice:
      'Копируется текст разговора. Исходные файлы не меняются. Вывод инструментов и рассуждения не переносятся.',
    importing: 'Импорт…',
    open: 'Открыть в Moor',
    continue: 'Продолжить в Moor',
  ...
  boot: {
    ready: 'Moor Desktop готов',
    desktopBootFailedWithMessage: message => `Не удалось запустить приложение: ${message}`,
    steps: {
      connectingGateway: 'Подключение к шлюзу',
      loadingSettings: 'Загрузка настроек Moor',
      loadingSessions: 'Загрузка последних сеансов',
      retryingRemoteBackend: 'Переподключение к удалённому бэкенду Moor…',
      startingDesktopConnection: 'Запуск подключения приложения',
      startingMoorDesktop: 'Запуск Moor Desktop…'
    },
    errors: {
      backgroundExited: 'Фоновый процесс Moor завершён.',
      backgroundExitedDuringStartup: 'Фоновый процесс Moor завершился при запуске.',
    ...
    failure: {
      title: 'Moor не удалось запустить',
      description:
  ...
  notifications: {
    sharedProfileWarning:
      'Другая установка Moor использует этот профиль. Обе установки используют общие настройки и данные, поэтому изменения могут конфликтовать. Можно продолжить работу или закрыть другую установку перед внесением изменений.',
    region: 'Уведомления',
    hide: 'Скрыть',
    show: 'Показать',
    more: count =>
    ...
    backendOutOfDateMessage:
      'Ваш бэкенд Moor старше этой сборки приложения и может работать некорректно. Обновите их, чтобы они совпали.',
    ...
    errors: {
      ...
      methodNotAllowed:
        'Бэкенд приложения отклонил запрос (405 Method Not Allowed). Попробуйте перезапустить Moor Desktop.',
    ...
    native: {
      ...
      inputBody: 'Moor ожидает ваш ответ.',
      turnDoneTitle: 'Moor завершил',
      turnDoneBody: '',
  ...
  billingBlock: {
    titleMoor: 'Кредиты Moor закончились',
  ...
  settings: {
    ...
    resetConfirm: 'Сбросить все настройки к значениям Moor по умолчанию?',
    ...
    plugins: {
      ...
      installModal: {
        ...
        installUncertain:
          'Moor перестал ждать результат установки, но плагин может всё ещё устанавливаться. Закройте это окно и обновите список плагинов перед повторной установкой.',
    ...
    notifications: {
      ...
      focusedHint: 'Оповещения о завершении срабатывают только пока Moor в фоне.',
      kinds: {
        approval: {
        ...
        input: {
          label: 'Нужен ввод',
          description: 'Moor задал вопрос или требует пароль/секрет.'
        },
        turnDone: {
          label: 'Ответ готов',
          description: 'Ход завершился, пока Moor был в фоне.'
        },
        turnError: {
          label: 'Ход не удался',
        ...
        plugin: {
          ...
          description: 'Плагин приложения отправил уведомление, пока Moor был в фоне.'
        }
      },
    ...
    appearance: {
      ...
      colorModeDesc: 'Выберите фиксированный режим или позвольте Moor следовать настройкам системы.',
      ...
      reactionsDesc: 'Эмодзи-тапбеки в стиле iMessage — реагируйте на сообщения, и Moor сможет реагировать на ваши.',
      ...
      pet: {
        title: 'Питомец',
        intro:
          'Заберите анимированного питомца из petdex, который парит над приложением и реагирует на действия Moor — «бегает», пока выполняются инструменты, радуется успеху и хмурится при ошибках.',
        restartHint:
          'Питомцам нужен быстрый перезапуск — текущее приложение запустилось до появления этой функции. Выйдите из Moor и откройте снова, затем вернитесь сюда.',
        scaleTitle: 'Размер',
    ...
    fieldDescriptions: defineFieldCopy({
      ...
      agent: {
        ...
        maxTurns: 'Верхний предел ходов с вызовами инструментов, после которого Moor останавливает запуск.'
      },
      terminal: {
      ...
      fileReadMaxChars: 'Максимальное число символов, которые Moor может прочитать из одного запроса к файлу.',
      approvals: {
        mode: 'Как Moor обрабатывает команды, требующие явного подтверждения.',
      ...
      updates: {
        nonInteractiveLocalChanges:
          'Когда Moor обновляет себя из приложения (без запроса в терминале), сохранять локальные правки исходников (stash) или выбрасывать (discard). Обновления из терминала всегда спрашивают.'
      }
    }),
    about: {
      updates: 'Обновления'
    ...
    config: {
      ...
      minimizeToTrayDesc:
        'Сворачивание окон или закрытие главного окна скрывает их в системном трее (строке меню macOS), оставляя Moor работать. Для выхода выберите «Выйти из Moor» в меню трея или нажмите Cmd+Q. По умолчанию выключено; действует только на этом устройстве.',
      ...
      loading: 'Загрузка конфигурации Moor…',
    ...
    hudModifier: {
      ...
      permission:
        'Разрешите Moor в Системных настройках → Конфиденциальность и безопасность → Мониторинг ввода, затем повторите попытку. Жест не записывает нажатия клавиш и не снимает экран.',
      unavailable:
        'Вспомогательная программа жеста HUD не запустилась или неожиданно остановилась. Повторите попытку или перезапустите Moor. Обычное сочетание HUD по-прежнему работает внутри Moor.',
      missingHelper:
        'В этой установке Moor отсутствует вспомогательная программа жеста HUD. Обновите или переустановите Moor и повторите попытку.',
      unsupportedSession:
    ...
    screenshot: {
      ...
      enabledDesc:
        'Нажмите обе клавиши Command одновременно в любом приложении, чтобы снять его переднее окно и прикрепить снимок к текущему черновику Moor. Автоматической отправки нет. По умолчанию выключено; действует только на этом Mac. Окно может содержать конфиденциальные данные — проверьте вложение перед отправкой.',
      ...
      inputPermission:
        'Разрешение на мониторинг ввода позволяет Moor распознавать обе клавиши Command, когда активно другое приложение. Разрешите Moor доступ в Системных настройках → Конфиденциальность и безопасность → Мониторинг ввода, затем вернитесь сюда и повторите попытку.',
      screenPermission:
        'Разрешение на запись экрана позволяет Moor снимать переднее окно приложения при использовании этого сочетания. Разрешите Moor доступ в Системных настройках → Конфиденциальность и безопасность → Запись экрана, затем вернитесь сюда и повторите попытку. Перезапустите Moor, если macOS попросит.',
    ...
    quickEntry: {
      ...
      enabledDesc:
        'Глобальным горячим ключом вызывайте маленький композер откуда угодно и отправляйте запрос, не открывая Moor.',
    ...
    connections: {
      ...
      intro:
        'Управляйте этим устройством и всеми шлюзами Moor, до которых можно дотянуться через удалённые, SSH или Cloud-соединения.',
      stagedNote:
      ...
      updateSkippedCloud: 'Управляется Moor Cloud',
      kindLocal: 'Локальный',
      kindRemote: 'Удалённый шлюз',
      kindCloud: 'Moor Cloud',
      kindSsh: 'SSH',
      kindLocalDesc: 'Рантайм Moor, которым управляет это приложение.',
      kindRemoteDesc: 'Шлюз Moor, доступный по HTTP(S) — LAN, Tailscale или интернет.',
      kindCloudDesc: 'Хостинговая инстанция, обнаруженная через ваш аккаунт Moor Cloud.',
      kindSshDesc: 'Установка Moor, доступная по SSH.',
      labelTitle: 'Имя',
      labelDesc:
      ...
      headersDesc:
        'Отправляются с каждым HTTP- и WebSocket-запросом к этому шлюзу — например, для прокси доступа вроде Cloudflare Access (CF-Access-Client-Id / CF-Access-Client-Secret). Значения хранятся зашифрованными. Заголовки, которыми управляет Moor (Authorization, Cookie, Host…), игнорируются.',
      ...
      cloudAddHint:
        'Подсказка: вход в Moor Cloud выше автоматически обнаруживает ваших агентов — эту форму используйте только для ручной регистрации известного URL инстанса.',
    ...
    gateway: {
      ...
      intro:
        'По умолчанию — локальный. Используйте удалённый, когда приложение должно управлять бэкендом Moor в другом месте. Соединения шлюзов — на уровне машины; профили обнаруживаются из подключённых шлюзов.',
      envOverrideTitle: 'Переменные окружения управляют этой сессией приложения.',
      envOverrideDesc:
        'Сбросьте MOOR_DESKTOP_REMOTE_URL и MOOR_DESKTOP_REMOTE_TOKEN, чтобы использовать сохранённую настройку ниже.',
      modeTitle: 'Режим подключения',
      localTitle: 'Локальный шлюз',
      localDesc: 'Запускает приватный бэкенд Moor на localhost. Это значение по умолчанию, работает офлайн.',
      remoteTitle: 'Удалённый шлюз',
      remoteDesc: 'Подключает это приложение к удалённому бэкенду Moor.',
      remoteAuthHint:
        'Хостинговые шлюзы используют OAuth или логин/пароль; самохостинговые могут использовать токен сессии.',
      cloudTitle: 'Moor Cloud',
      cloudDesc: 'Войдите в Moor Cloud один раз и выбирайте агентов из своего аккаунта — без вставки URL.',
      cloudSignInTitle: 'Moor Cloud',
      cloudSignIn: 'Войти в Moor Cloud',
      cloudSignedIn: 'Вы вошли в Moor Cloud',
      cloudNeedsSignIn: 'Войдите в Moor Cloud, чтобы обнаружить агентов в вашем аккаунте.',
      ...
      cloudDiscoverFailed: 'Не удалось загрузить агентов Moor Cloud',
      cloudConnectFailed: 'Не удалось подключиться к этому агенту',
      cloudSignInFailed: 'Не удалось войти в Moor Cloud',
      cloudSignedOutTitle: 'Выход из Moor Cloud',
      cloudSignedOutMessage: 'Сессия Moor Cloud сброшена.',
      ...
      remoteUrlDesc: 'Базовый URL удалённого бэкенда дашборда. Поддерживаются префиксы пути, например /moor.',
      ...
      restartingMessage: 'Moor Desktop переподключится с сохранёнными настройками — оболочка останется открытой.',
      savedMessage: 'Сохранено для следующего перезапуска.',
      connectedTo: (baseUrl, version) => `Подключено к ${baseUrl}${version ? ` · Moor ${version}` : ''}`,
      ...
      sshDesc:
        'Moor запускается на удалённой машине по SSH и туннелируется в это приложение — ничего не нужно запускать или открывать самим. Требуется рабочая SSH-аутентификация по ключу на хост.',
      sshTrustHint:
      ...
      sshMoorPathTitle: 'Путь к Moor (необязательно)',
      sshMoorPathDesc: 'Полный путь к бинарнику moor на удалённой машине. Пусто = автоопределение.',
      sshMoorPathPlaceholder: 'автоопределение',
      ...
      sshReachable: (host, platform) => `Доступен: ${host} (${platform}) — Moor найден`,
      ...
      sshErrAuth:
        'SSH-аутентификация не удалась. Загрузите ключ в ssh-agent (ssh-add) или задайте IdentityFile в ~/.ssh/config — Moor запускает ssh в неинтерактивном режиме.',
      sshErrHostKey:
      ...
      sshErrNotInstalled:
        'Moor не установлен на удалённой машине. Установите его там (curl -fsSL https://raw.githubusercontent.com/thisismamad-n/Moor/master/scripts/install.sh | bash) или задайте путь к Moor.',
      sshErrPlatform:
        'Неподдерживаемая удалённая платформа. SSH-режим Moor Desktop поддерживает удалённые хосты Linux, macOS и Windows.',
      sshErrTimeout: 'SSH-соединение истекло. Хост может быть недоступен или «спит».',
      sshErrUpdateRequired: 'Перед подключением через SSH обновите Moor на удалённой машине.',
    ...
    mcp: {
      ...
      deepLinkDescription:
        'Ссылка запросила добавить этот MCP-сервер в Moor. Проверьте конфигурацию ниже — она пришла из ссылки, а не из Moor.',
    ...
    providers: {
      ...
      intro:
        'Войдите по подписке — копировать API-ключ не нужно. Moor проведёт вход в браузере прямо здесь, в приложении.',
      ...
      localEndpoint: {
        ...
        description: 'Направьте Moor на любой OpenAI-совместимый эндпоинт (Zyphra, vLLM, llama.cpp, Ollama и т. д.).'
      },
    ...
    toolsets: {
      ...
      nousIncluded: 'Входит в подписку Moor — войдите в Moor Portal, чтобы активировать.',
      nousAuthNeededTitle: 'Войдите в Moor Portal',
      nousAuthNeededMessage: provider => `${provider} сохранён, но не активируется, пока вы не войдёте в Moor Portal.`,
      nousAuthSignIn: 'Войти',
      nousAuthDoneTitle: 'Moor Portal подключён',
      nousAuthDoneMessage: 'Ваши бэкенды по подписке теперь активны.',
      nousAuthFailed: 'Вход в Moor Portal не завершён',
  ...
  skills: {
    ...
    skillArchivedMessage: 'Восстановить через moor curator restore.',
    hub: {
  ...
  starmap: {
    ...
    emptyDesc: 'По мере того как Moor создаёт навыки и память для вашей работы, они появятся здесь.',
  ...
  commandCenter: {
    ...
    pets: {
      ...
      staleBackend: 'Перезапустите Moor, чтобы использовать питомцев — бэкенд старше этой функции.',
    ...
    generatePet: {
      ...
      staleBackend: 'Обновите Moor, чтобы генерировать питомцев.',
      backgroundHint: 'Можно закрыть — Moor уведомит, когда будет готово.',
    ...
    nav: {
      ...
      settings: { title: 'Настройки', detail: 'Настройка Moor desktop' },
    ...
    moorActiveSessions: (version, count) => `Moor ${version} · Активные сеансы: ${count}`,
  ...
  messaging: {
    ...
    fieldCopy: {
      ...
      MATRIX_USER_ID: { label: 'ID пользователя бота', placeholder: '@moor:example.org' },
  ...
  profiles: {
    ...
    fleet: {
      localDevice: 'Это устройство (локальный бэкенд — установит Moor, если его нет, иначе откроет новую сессию)',
      ...
      installDeviceDesc:
        'Moor будет установлен локально, затем откроется новая сессия на этом компьютере. Установка не начнётся, пока вы не подтвердите.',
    ...
    defaultDescription: 'Используется при запуске Moor и для новых чатов. Профили существующих сессий не меняются.',
    ...
    deleting: 'Удаление...',
    createDesc: 'Профили — это независимые среды Moor: отдельные настройки, навыки и SOUL.md.',
    nameLabel: 'Имя',
  ...
  modelAssignment: {
    saveFailed: 'Moor не сохранил это изменение модели.'
  },
  ...
  cron: {
    ...
    newCron: 'Новый cron',
    emptyDescNew:
      'Запланируйте промпт, который будет выполняться по cron-выражению. Moor выполнит его и доставит результаты в выбранное вами место.',
  ...
  sidebar: {
    ...
    projects: {
      ...
      staleBackend:
        'Обновите бэкенд Moor, чтобы создавать проекты — ваш бэкенд старше этого desktop-приложения (Настройки → Обновления → Бэкенд).',
      deleteConfirm: 'Это удалит сохранённый проект из Moor. Файлы, git-репозитории и worktrees не пострадают.',
      ...
      worktreeStaleBackend:
        'Обновите бэкенд Moor, чтобы создавать worktrees по этому удалённому соединению — он старше git worktree API.',
  ...
  composer: {
    ...
    placeholderStarting: 'Запуск Moor...',
    placeholderReconnecting: 'Переподключение к Moor…',
    ...
    commandDescs: {
      ...
      '/goal': 'Задать постоянную цель, над которой Moor работает до завершения',
      ...
      '/plan': 'Записать план реализации в Markdown в .moor/plans/ без выполнения',
      ...
      '/profile': 'Переключить активный профиль Moor',
      ...
      '/subscription': 'Показать план Moor и изменить его в браузере',
      '/topup': 'Показать баланс Moor и управлять оплатой',
      '/platform': 'Приостановить, возобновить или перечислить сбоящие платформы шлюза',
      '/version': 'Показать версию Moor Agent',
    ...
    attachUrlDesc: 'Moor загрузит страницу и добавит её как контекст для этого хода.',
  ...
  statusStack: {
    ...
    coding: {
      ...
      agentShip: 'Попросить Moor открыть PR',
  ...
  updates: {
    discontinuedTitle: 'Эта сборка Moor больше не поддерживается',
    discontinuedBody: 'Эта сборка Moor больше не поддерживается и может перестать работать — удалите её. Ваши данные останутся на диске.',
    channels: { stable: 'Стабильный', canary: 'Тестовый' },
    bundleSwapPending: 'Перезапустите, чтобы завершить обновление',
    bundleSwapPendingDesc:
      'Обновлённое приложение уже установлено — достаточно перезапустить Moor, чтобы загрузить его. Чаты и настройки не пострадают.',
    bundleSwapPendingAction: 'Перезапустить Moor',
    stages: {
      idle: 'Готовимся…',
      prepare: 'Готовимся…',
      fetch: 'Скачиваем…',
      pull: 'Почти готово…',
      pydeps: 'Завершаем…',
      update: 'Обновляем Moor…',
      rebuild: 'Пересобираем desktop-приложение…',
      restart: 'Перезапускаем Moor…',
    ...
    unsupportedMessage: 'Эта версия Moor не может обновлять себя из приложения.',
    connectionRetry: 'Проверьте соединение и попробуйте снова.',
    gitUnusable: 'Moor не удалось запустить Git на этом компьютере, поэтому проверить обновления не получилось.',
    ...
    availableBody: 'Новая версия Moor готова к установке.',
    availableTitleBackend: 'Доступно обновление бэкенда',
    availableBodyBackend: 'Новая версия подключённого бэкенда Moor готова к установке.',
    ...
    manualBody:
      'Moor установлен из командной строки, поэтому обновления тоже выполняются там. Вставьте это в терминал:',
    manualPickedUp: 'Moor подхватит новую версию при следующем запуске.',
    guiSkewTitle: 'Обновите desktop-приложение',
    guiSkewBody:
      'Бэкенд обновлён, но пакет этого desktop-приложения не изменился. Обновите или переустановите desktop-приложение Moor (ваш AppImage / .deb / .rpm), чтобы версии совпали.',
    copy: 'Копировать',
    copied: 'Скопировано',
    done: 'Готово',
    applyingBody:
      'Обновлятор Moor возьмёт процесс на себя в своём окне и автоматически перезапустит Moor, когда закончит. Пожалуйста, не открывайте Moor вручную во время обновления.',
    applyingBodyBackend:
      'Удалённый бэкенд применяет обновление и перезапустится. Moor переподключится автоматически, когда он снова заработает.',
    applyingClose: 'Это окно закроется во время обновления, затем Moor откроется сам.',
    errorTitle: 'Обновление не завершилось',
    errorBody: 'Не переживайте — ничего не потеряно. Можно попробовать снова прямо сейчас.',
    blockerTitle: 'Закрыть локальные предпросмотры, чтобы обновить Moor?',
    blockerBody:
      'Moor нужно остановить эти локальные предпросмотры перед обновлением. Ваши файлы не будут изменены или удалены.',
    foreignBlockerTitle: 'Закрыть другие процессы, чтобы обновить Moor',
    foreignBlockerBody:
      'Moor не может безопасно закрыть эти процессы автоматически. Закройте приложение, терминал или службу, которой принадлежит каждый из них, и повторите обновление.',
    mixedBlockerBody:
      'Moor может закрыть локальные предпросмотры, перечисленные ниже. Остальные процессы нужно закрыть вручную, прежде чем обновление сможет продолжиться.',
    ...
    bundleOutOfSyncDesc:
      'Рантайм Moor обновлён, но само приложение — ещё старая сборка: новые функции интерфейса (например, Bot Mode) не появятся до обновления. Запустите обновление ниже, чтобы пересобрать приложение. Если предупреждение не исчезнет, переустановите с последнего установщика.',
  ...
  install: {
    ...
    oneTimeTitle: 'Moor требует одноразовой установки',
    ...
    setupChoiceTitle: 'Настройка Moor Desktop',
    setupChoiceDesc:
      'Подключите это приложение к уже работающему шлюзу Moor или установите Moor локально на этот компьютер.',
    connectExistingTitle: 'Подключиться к существующему Moor',
    ...
    installLocalTitle: 'Установить Moor локально',
    installLocalDesc: 'Скачайте Moor, создайте его Python-окружение и запустите бэкенд на этом компьютере.',
    localStartUnavailable: 'Не удалось начать локальную установку. Перезапустите Moor Desktop и попробуйте снова.',
    remoteSetupTitle: 'Подключиться к существующему Moor',
    remoteSetupDesc: 'Введите URL вашего шлюза. Moor Desktop определит, нужен токен или вход через браузер.',
    remoteUrlTitle: 'URL шлюза',
    remoteUrlDesc: 'Используйте базовый URL шлюза Moor, включая https:// для удалённых.',
    remoteUrlPlaceholder: 'https://gateway.example.com/moor',
    probing: 'Определяем аутентификацию шлюза...',
    probeError: 'Не удалось подключиться к этому шлюзу Moor.',
    ...
    settingUpTitle: 'Настройка Moor Agent',
    finishingTitle: 'Завершаем',
    failedDesc:
      'Один из шагов установки завершился ошибкой. На Windows это может произойти, если запущена другая инстанция Moor CLI или desktop. Остановите все работающие инстанции Moor и повторите. Подробности — ниже или в журнале desktop.',
    activeDesc:
      'Это одноразовая настройка. Установщик Moor скачивает зависимости и настраивает вашу машину. При следующих запусках этот шаг будет пропущен.',
  ...
  onboarding: {
    headerTitle: 'Настроим для вас Moor Agent',
    headerDesc: 'Подключите провайдера модели, чтобы начать общение. Большинство вариантов — в один клик.',
    preparingInstall: 'Moor завершает установку. Обычно это занимает меньше минуты при первом запуске.',
    starting: 'Запускаем Moor…',
    ...
    recommended: 'Рекомендуется',
    connected: 'Подключено',
    featuredPitch: 'Одна подписка, 300+ передовых моделей — рекомендуемый способ запускать Moor',
    ...
    apiKeyOptions: {
      ...
      local: {
        short: 'self-hosted',
        description:
          'Укажите Moor локальный или self-hosted OpenAI-совместимый endpoint (vLLM, llama.cpp, Ollama и т.д.).'
      }
    },
    ...
    flowSubtitles: {
      ...
      device_code: 'Откроет страницу подтверждения в браузере — Moor подключится автоматически',
    ...
    authorizeThere: 'Авторизуйте Moor там.',
    ...
    autoBrowser: provider =>
      `Мы открыли ${provider} в вашем браузере. Авторизуйте Moor там, и подключение произойдёт автоматически — ничего копировать и вставлять не нужно.`,
  ...
  modelPicker: {
    ...
    pro: 'Pro',
    proNeedsSubscription: 'Модели Pro требуют платной подписки Moor.',
    free: 'Free',
    freeTier: 'Бесплатный тариф',
  ...
  preview: {
    ...
    largeBody: (label, size) => `${label} — ${size}. Moor покажет только первые 512 КБ.`,
    ...
    web: {
      ...
      tryAgain: 'Попробовать снова',
      restarting: 'Moor перезапускается...',
      askRestart: 'Попросить Moor перезапустить сервер',
      lookingRestart: taskId => `Moor ищет сервер предпросмотра для перезапуска (${taskId})`,
      restartingTitle: 'Перезапуск сервера предпросмотра',
      restartingMessage: 'Moor работает в фоне. Следите за прогрессом в консоли предпросмотра.',
      ...
      blankPageBody: 'Введите адрес выше, чтобы просматривать, или попросите Moor открыть страницу.',
      finishedRestarting: message => `Moor завершил перезапуск сервера предпросмотра${message ? `: ${message}` : ''}`,
      ...
      restartFailedMessage: 'Moor не смог перезапустить сервер.',
      stillWorking:
        'Moor всё ещё работает, но результата перезапуска пока нет. Команда сервера может выполняться в foreground.',
  ...
  interfaceMode: {
    ...
    hint: 'Меняет то, что показано, а не то, что умеет Moor.',
    sessionNote:
      'Задано простым режимом. Изменение здесь действует до конца сеанса; переключитесь в расширенный, чтобы сделать его своим.',
    simple: {
      label: 'Простой',
      description: 'Для общения с Moor. Боковая панель и чат; без терминала, файлов и панелей diff.'
    },
    advanced: {
      label: 'Расширенный',
  ...
  assistant: {
    thread: {
      ...
      loadingResponse: 'Moor загружает ответ',
    ...
    approval: {
      gatewayDisconnected: 'Шлюз Moor не подключён',
      ...
      alwaysDescription: pattern =>
        `Это добавит паттерн «${pattern}» в ваш постоянный список разрешений (~/.moor/config.yaml). Moor больше не будет спрашивать о подобных командах — ни в этом сеансе, ни в будущем.`,
  ...
  prompts: {
    ...
    sudoInstallDesc:
      'Moor нужен ваш пароль sudo, чтобы установить пакеты Bot Screen (TigerVNC + Xfce) на хосте шлюза. Он отправляется только на этот хост.',
    sudoPlaceholder: 'пароль sudo',
    secretTitle: 'Требуется секрет',
    secretDesc: 'Moor нужны учётные данные, чтобы продолжить.',
  ...
  desktop: {
    ...
    restartToUseSaveImage: 'Перезапустите Moor Desktop, чтобы использовать «Сохранить изображение».',
    restartToSaveImages: 'Перезапустите Moor Desktop, чтобы сохранять изображения',
    ...
    handoff: {
      ...
      timedOut: 'Превышено время ожидания шлюза. Выполняется ли `moor gateway`?'
    }
  },
  errors: {
