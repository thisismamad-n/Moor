/**
 * @moor/plugin-sdk — THE plugin language. The vscode-module model: plugin
 ...
 *    object as `window.__MOOR_PLUGIN_SDK__` and maps the import to it, so a
...
import type { PaginatedSessions, UsageStats } from '@/types/moor'
...
export interface PluginProfileRoute {
  ...
  profile: string
  /** Backend Moor profile served by that route. */
...
async function requestPluginProfile<T>(
  ...
  const getAgentRoster = window.moorDesktop?.getAgentRoster
...
async function pluginRouteStillRegistered(route: PluginProfileRoute): Promise<boolean> {
  const getProfileRoutes = window.moorDesktop?.getProfileRoutes
...
//
// ensureGatewayProfile awaits the store's dial, and MoorGateway.connect() has
...
export const host = {
  ...
  deleteProfile: async (profile: string | PluginProfileRoute): Promise<void> => {
    ...
    // restores on relaunch and re-creates the deleted profile (moor-agent#94235).
    dropTilesForProfile(
  ...
  connections: async () => {
    ...
    if (!bridge) {
      throw new Error('This Desktop build has no connection registry. Update Moor Desktop.')
    }
  ...
  agents: async () => {
    const roster = window.moorDesktop?.getAgentRoster

    if (!roster) {
      throw new Error('This Desktop build cannot enumerate multi-source agents. Update Moor Desktop.')
    }
  ...
  newChat: (profile?: null | string | PluginProfileRoute, options: PluginNewChatOptions = {}): void => {
    if (options.workspaceMode === 'bots') {
      ...
      if (!openTab) {
        notify({ kind: 'error', message: 'Update Moor Desktop to open another Bot chat.' })
   ...
   *
   *  `isStaleTile` (moor-agent#90102): the caller's reconciliation probe
  ...
  profileRoutes: async () => {
    ...
    if (!getProfileRoutes) {
      throw new Error('Moor Desktop connection routing unavailable')
    }
  ...
  listPersistedSessions: async (
    ...
    return moorApi<PaginatedSessions>({
  ...
  setPersistedSessionHidden: async (
    ...
    return moorApi<{ ok: boolean; hidden: boolean }>({
      ...(route ? { connectionId: route.connectionId } : {}),
   ...
   *  components that take a `MoorGateway` prop directly (e.g. `McpTab`),
 ...
 *  "MOOR AGENT" with it, a `chat.empty` contribution writes its own name. */
...
export type { MoorGateway } from '@/moor'
...
export type { MoorOpenTarget } from '@/lib/moor-open-target'
...
/** Moor' reasoning levels + their compact labels, so a plugin surfacing a
...
export type { RpcEvent, StatusResponse } from '@/types/moor'
