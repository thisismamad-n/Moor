...
vi.mock('@/moor', () => ({ deleteProfile: vi.fn(), getLogs: vi.fn(), getStatus: vi.fn(), moorApi: vi.fn() }))
...
const { deleteProfile, moorApi } = await import('@/moor')
...
describe('connection-aware plugin host APIs', () => {
  it('retires a profile gateway before deleting it', async () => {
    ...
    // profile's backend, re-creating its MOOR_HOME (#94235).
  ...
  it('refreshes the profile inventory before asking Electron for routes', async () => {
    ...
    ;(window as unknown as { moorDesktop: unknown }).moorDesktop = { getProfileRoutes }
  ...
  it('reads and hides persisted sessions through the source primary without activating the profile', async () => {
    ...
    expect(moorApi).toHaveBeenNthCalledWith(
      1,
    ...
    expect(moorApi).toHaveBeenNthCalledWith(2, {
      connectionId: 'source-a',
      path: '/api/sessions/bot-chat',
      method: 'PATCH',
      body: { hidden: true, profile: 'backend-worker' }
    })
    expect(vi.mocked(moorApi).mock.calls.every(([request]) => !('profile' in request))).toBe(true)
