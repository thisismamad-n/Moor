...
function isMissingIpcHandler(error: unknown): boolean {
  ...
  return message.includes("No handler registered for 'moor:saveImageFromUrl'")
}
...
export function useImageDownload(src?: string) {
  ...
  const download = useCallback(async () => {
    ...
    try {
      if (window.moorDesktop?.saveImageFromUrl) {
        if (await window.moorDesktop.saveImageFromUrl(src)) {
