...
declare global {
  interface Window {
    moorDesktop: {
      ...
      getConnection: (profile?: string | null) => Promise<MoorConnection>
      ...
      // Resume this session in the user's own terminal emulator (`moor --tui
      ...
      // Moor Cloud: one portal login powers discovery + silent per-agent
      ...
      api: <T>(request: MoorApiRequest) => Promise<T>
      notify: (payload: MoorNotification) => Promise<boolean>
      ...
      readFileText: (filePath: string) => Promise<MoorReadFileTextResult>
      ...
      readPluginSource?: (filePath: string) => Promise<MoorReadFileTextResult>
      selectPaths: (options?: MoorSelectPathsOptions) => Promise<string[]>
      ...
      normalizePreviewTarget: (target: string, baseDir?: string) => Promise<MoorPreviewTarget | null>
      watchPreviewFile: (url: string) => Promise<MoorPreviewWatch>
      ...
      watchDirectory?: (dir: string) => Promise<MoorPreviewWatch>
      stopPreviewFileWatch: (id: string) => Promise<boolean>
      setActiveWork?: (payload: MoorActiveWork) => void
      setTitleBarTheme?: (payload: MoorTitleBarTheme) => void
      ...
      // Local Desktop runtime-plugin root (<MOOR_HOME>/desktop-plugins),
      ...
      git?: {
        worktreeList: (repoPath: string) => Promise<MoorGitWorktree[]>
        worktreeAdd: (
          repoPath: string,
        ...
        branchList: (repoPath: string) => Promise<MoorGitBranch[]>
        ...
        baseBranchList: (repoPath: string) => Promise<MoorGitBaseBranch[]>
        ...
        repoStatus: (repoPath: string) => Promise<MoorRepoStatus | null>
        ...
        review: {
          list: (repoPath: string, scope: MoorReviewScope, baseRef?: null | string) => Promise<MoorReviewList>
          diff: (
            repoPath: string,
            filePath: string,
            scope: MoorReviewScope,
            baseRef?: null | string,
            staged?: boolean
          ) => Promise<string>
          ...
          shipInfo: (repoPath: string) => Promise<MoorReviewShipInfo>
      ...
      terminal: {
        ...
        onExit: (id: string, callback: (payload: MoorTerminalExit) => void) => () => void
        resize: (id: string, size: { cols: number; rows: number }) => Promise<boolean>
        start: (options?: { cols?: number; cwd?: string; rows?: number }) => Promise<MoorTerminalSession>
...
export interface MoorTerminalSession {
  cwd: string
  id: string
  shell: string
}

export interface MoorTerminalExit {
  code: number | null
  signal: string | null
}
...
export interface DesktopConnectionConfig {
  envOverride: boolean
  // The saved connection mode. 'cloud' is a Moor Cloud connection: it carries
...
export interface DesktopConnectionConfigInput {
  ...
  remoteUrl?: string
  // For a 'cloud' connection: the selected Moor Cloud org (slug or id) to
...
// A discovered Moor Cloud agent — the trimmed DTO from NAS GET /api/agents.
...
export interface DesktopBootProgress {
  error: string | null
  fakeMode: boolean
  /** True when the boot failure is a Moor Cloud agent that is down (HTTP 502/503/504). */
...
export interface MoorApiRequest {
  path: string
  method?: string
  body?: unknown
...
export interface MoorPreviewTarget {
  binary?: boolean
  byteSize?: number
  kind: 'file' | 'url'
  label: string
...
export interface MoorReadFileTextResult {
  binary?: boolean
  byteSize?: number
  language?: string
  mimeType?: string
...
export interface MoorPreviewWatch {
  id: string
  path: string
}
...
export interface MoorGitWorktree {
  path: string
  branch: null | string
  isMain: boolean
  detached: boolean
...
export interface MoorGitBranch {
  name: string
  checkedOut: boolean
  isDefault: boolean
  isRemote: boolean
...
// preselect it.
export interface MoorGitBaseBranch {
  name: string
  isRemote: boolean
  isDefault: boolean
}
...
export interface MoorRepoStatusFile {
  path: string
  staged: boolean
  unstaged: boolean
  untracked: boolean
...
export interface MoorRepoStatus {
  branch: null | string
...
// turn began.
export type MoorReviewScope = 'branch' | 'lastTurn' | 'uncommitted'

// One changed file in the review pane (status letter, +/- lines, staged flag).
export interface MoorReviewFile {
  path: string
  added: number
  removed: number
...
export interface MoorReviewList {
  files: MoorReviewFile[]
...
export interface MoorBranchPullRequest {
  branch: string
  draft: boolean
  number: number
...
export interface MoorRepoPullRequests {
  ghReady: boolean
...
export interface MoorReviewShipInfo {
  ghReady: boolean
  pr: MoorReviewPr | null
}

export interface MoorReadDirEntry {
  name: string
  path: string
  isDirectory: boolean
}

export interface MoorReadDirResult {
  entries: MoorReadDirEntry[]
  error?: string
}

export interface MoorPreviewFileChanged {
  id: string
  path: string
  url: string
}

export interface MoorSelectPathsOptions {
  title?: string
  defaultPath?: string
  directories?: boolean
  multiple?: boolean
