...
// "Timed out connecting to Moor backend" that hangs the UI (#48504).
//
...
// the call; each pooled backend already has its own MOOR_HOME, so no backend
...
// process to route them to that host (moor:api's registry branch). Null /
...
async function activeConnection(): Promise<MoorConnection> {
  const getConnectionFor = window.moorDesktop.getConnectionFor

  if (_apiConnectionId && getConnectionFor) {
    return getConnectionFor({ connectionId: _apiConnectionId, profile: _apiProfile })
  }

  return window.moorDesktop.getConnection(_apiProfile)
}

/** Options for a plugin REST call — mirrors the app's own `moorDesktop.api`
 *  shape, minus the path (which is namespace-derived). */
export interface PluginRestOptions {
  method?: string
  body?: unknown
  /** Single-file multipart upload (see MoorApiRequest.upload). */
...
export async function pluginRest<T>(pluginId: string, path: string, opts: PluginRestOptions = {}): Promise<T> {
  if (!window.moorDesktop?.api) {
    throw new Error('Moor desktop bridge unavailable')
  }

  const suffix = pluginPathSuffix('pluginRest', path)

  return window.moorDesktop.api<T>({
...
export async function listSessions(
  ...
  const result = await window.moorDesktop.api<PaginatedSessions>({
    path:
...
// "Error invoking remote method 'moor:api': Error: 404: ..." through the
...
export async function listSidebarSessions(req: SidebarSessionsRequest): Promise<SidebarSessionsResponse> {
  ...
  try {
    result = await window.moorDesktop.api<SidebarSessionsResponse>({
...
export function searchSessions(query: string): Promise<SessionSearchResponse> {
  return window.moorDesktop.api<SessionSearchResponse>({
...
export function getSession(id: string, profile?: string | null): Promise<SessionInfo> {
  ...
  return window.moorDesktop.api<SessionInfo>({
    ...(profile ? { profile } : {}),
...
export function getSessionMessages(
  ...
  return window.moorDesktop.api<SessionMessagesResponse>({
    ...(profile ? { profile } : {}),
...
export function renameSession(
  ...
  return window.moorDesktop.api<{ ok: boolean; title: string }>({
    ...(profile ? { profile } : {}),
    path: `/api/sessions/${encodeURIComponent(id)}`,
    method: 'PATCH',
...
export function getGlobalModelInfo(): Promise<ModelInfoResponse> {
  return window.moorDesktop.api<ModelInfoResponse>({
    ...profileScoped(),
    path: '/api/model/info',
...
export function getStatus(): Promise<StatusResponse> {
  return window.moorDesktop.api<StatusResponse>({
    ...profileScoped(),
    path: '/api/status'
  })
}
...
export function getLogs(params: {
  ...
  return window.moorDesktop.api<LogsResponse>({
    ...profileScoped(),
    path: suffix ? `/api/logs?${suffix}` : '/api/logs'
  })
}

export function getMoorConfig(profile?: string): Promise<MoorConfig> {
  return window.moorDesktop.api<MoorConfig>({
...
export function getMoorConfigRecord(profile?: null | string): Promise<MoorConfigRecord> {
...
export function getMoorConfigSchema(): Promise<ConfigSchemaResponse> {
  return window.moorDesktop.api<ConfigSchemaResponse>({
    ...profileScoped(),
    path: '/api/config/schema'
  })
}

export function saveMoorConfig(config: MoorConfigRecord): Promise<{ ok: boolean }> {
  return window.moorDesktop.api<{ ok: boolean }>({
    ...profileScoped(),
    path: '/api/config',
    method: 'PUT',
...
export function getMemoryProviderConfig(provider: string): Promise<MemoryProviderConfig> {
  return window.moorDesktop.api<MemoryProviderConfig>({
    ...profileScoped(),
...
export function getEnvVars(): Promise<Record<string, EnvVarInfo>> {
  return window.moorDesktop.api<Record<string, EnvVarInfo>>({
    ...profileScoped(),
    path: '/api/env'
  })
}
...
export function validateProviderCredential(
  ...
  return window.moorDesktop.api<{ ok: boolean; reachable: boolean; message: string; models?: string[] }>({
    ...profileScoped(),
...
export function getCustomEndpoints(): Promise<CustomEndpointsResponse> {
  return window.moorDesktop.api<CustomEndpointsResponse>({
    ...profileScoped(),
...
export function validateCustomEndpoint(endpoint: CustomEndpointUpdate): Promise<CustomEndpointValidationResponse> {
  return window.moorDesktop.api<CustomEndpointValidationResponse>({
...
export function activateCustomEndpoint(id: string): Promise<{ ok: boolean; provider: string; model: string }> {
  return window.moorDesktop.api<{ ok: boolean; provider: string; model: string }>({
    ...profileScoped(),
...
export function disconnectOAuthProvider(providerId: string): Promise<{ ok: boolean; provider: string }> {
  return window.moorDesktop.api<{ ok: boolean; provider: string }>({
    ...profileScoped(),
...
export function startMemoryProviderOAuth(provider: string): Promise<MemoryProviderOAuthStatus> {
  return window.moorDesktop.api<MemoryProviderOAuthStatus>({
    ...profileScoped(),
...
export function getStarmapGraph(): Promise<StarmapGraph> {
  return window.moorDesktop.api<StarmapGraph>({
    ...profileScoped(),
...
export function testMcpServer(name: string, profile?: null | string): Promise<McpTestResult> {
  return window.moorDesktop.api<McpTestResult>({
    ...profileScoped(profile),
 ...
 *  `saveMoorConfig`, this REPLACES rather than deep-merges, so deletes,
...
export function authMcpServer(name: string, profile?: null | string): Promise<McpOAuthFlow> {
  return window.moorDesktop.api<McpOAuthFlow>({
    ...profileScoped(profile),
...
export function cancelMcpOAuthFlow(flowId: string, profile?: null | string): Promise<{ ok: boolean; status: string }> {
  return window.moorDesktop.api<{ ok: boolean; status: string }>({
    ...profileScoped(profile),
    path: `/api/mcp/oauth/flows/${encodeURIComponent(flowId)}`,
    method: 'DELETE'
  })
...
export function getToolsets(profile?: null | string): Promise<ToolsetInfo[]> {
  return window.moorDesktop.api<ToolsetInfo[]>({
    ...profileScoped(profile),
...
export function getToolsetConfig(name: string, profile?: null | string): Promise<ToolsetConfig> {
  return window.moorDesktop.api<ToolsetConfig>({
    ...profileScoped(profile),
...
export function getToolsetModels(
  ...
  return window.moorDesktop.api<ToolsetModelsResponse>({
    ...profileScoped(profile),
...
export function selectToolsetModel(
  ...
  return window.moorDesktop.api<{ ok: boolean; name: string; model: string }>({
    ...profileScoped(profile),
...
export interface SelectToolsetProviderResponse {
  ...
  capability?: string
  /** Present (true) when a managed Moor row was selected but the Portal
   *  entitlement is missing — the row won't activate until the user signs
   *  in to Moor Portal. */
  needs_moor_auth?: boolean
  /** The managed feature key (e.g. "browser") when needs_moor_auth is set. */
  feature?: string
}
...
export function selectToolsetProvider(
  ...
  return window.moorDesktop.api<SelectToolsetProviderResponse>({
    ...profileScoped(profile),
...
export function runToolsetPostSetup(
  ...
  return window.moorDesktop.api<ActionResponse & { key: string }>({
    ...profileScoped(profile),
...
export function getTerminalBackends(): Promise<TerminalBackendsResponse> {
  return window.moorDesktop.api<TerminalBackendsResponse>({
    ...profileScoped(),
...
export function selectTerminalBackend(backend: string): Promise<{ ok: boolean; backend: string }> {
  return window.moorDesktop.api<{ ok: boolean; backend: string }>({
    ...profileScoped(),
...
export function getComputerUseStatus(): Promise<ComputerUseStatus> {
  return window.moorDesktop.api<ComputerUseStatus>({
    ...profileScoped(),
...
export function getMessagingPlatforms(): Promise<MessagingPlatformsResponse> {
  return window.moorDesktop.api<MessagingPlatformsResponse>({
...
export function updateMessagingPlatform(
  ...
  return window.moorDesktop.api<{ ok: boolean; platform: string }>({
...
export function testMessagingPlatform(platformId: string): Promise<MessagingPlatformTestResponse> {
  return window.moorDesktop.api<MessagingPlatformTestResponse>({
...
export function getPairing(): Promise<PairingResponse> {
  return window.moorDesktop.api<PairingResponse>({
    ...profileScoped(),
    path: '/api/pairing'
  })
}
...
export function approvePairing(platform: string, requestId: string): Promise<{ ok: boolean; user: PairingUser }> {
  return window.moorDesktop.api<{ ok: boolean; user: PairingUser }>({
    ...profileScoped(),
...
export function getWebhooks(): Promise<WebhooksResponse> {
  return window.moorDesktop.api<WebhooksResponse>({
    ...profileScoped(),
    path: '/api/webhooks'
  })
}
...
export function enableWebhooks(): Promise<WebhookEnableResponse> {
  return window.moorDesktop.api<WebhookEnableResponse>({
    ...profileScoped(),
...
export function createWebhook(body: WebhookCreatePayload): Promise<WebhookCreateResponse> {
  return window.moorDesktop.api<WebhookCreateResponse>({
    ...profileScoped(),
    path: '/api/webhooks',
    method: 'POST',
    body
...
export function setWebhookEnabled(
  ...
  return window.moorDesktop.api<{ enabled: boolean; name: string; ok: boolean }>({
    ...profileScoped(),
...
// Cron jobs are stored per-profile (<MOOR_HOME>/cron/jobs.json), and the
...
export function getCronJobs(profile?: string): Promise<CronJob[]> {
  ...
  return window.moorDesktop.api<CronJob[]>({
    ...profileScoped(),
    ...connectionScoped(),
...
export async function getCronJobRuns(jobId: string, limit = 20): Promise<SessionInfo[]> {
  const { runs } = await window.moorDesktop.api<{ runs: SessionInfo[] }>({
    ...profileScoped(),
    ...connectionScoped(),
...
export async function getCronDeliveryTargets(): Promise<CronDeliveryTarget[]> {
  const { targets } = await window.moorDesktop.api<{ targets: CronDeliveryTarget[] }>({
    ...profileScoped(),
    ...connectionScoped(),
...
export function getAutomationBlueprints(): Promise<{ blueprints: AutomationBlueprint[] }> {
  return window.moorDesktop.api<{ blueprints: AutomationBlueprint[] }>({
    ...profileScoped(),
    ...connectionScoped(),
...
export function getProfiles(): Promise<ProfilesResponse> {
  return window.moorDesktop.api<ProfilesResponse>({
    path: '/api/profiles',
    timeoutMs: STARTUP_REQUEST_TIMEOUT_MS
  })
}
...
export function createProfile(body: ProfileCreatePayload): Promise<{ name: string; ok: boolean; path: string }> {
  return window.moorDesktop.api<{ name: string; ok: boolean; path: string }>({
    path: '/api/profiles',
    method: 'POST',
    body
  })
...
export function deleteProfile(name: string): Promise<{ ok: boolean; path: string }> {
  return window.moorDesktop.api<{ ok: boolean; path: string }>({
    path: `/api/profiles/${encodeURIComponent(name)}`,
    method: 'DELETE'
  })
}
...
export function getProfileSoul(name: string): Promise<ProfileSoul> {
  return window.moorDesktop.api<ProfileSoul>({
...
export function getProfileSetupCommand(name: string): Promise<ProfileSetupCommand> {
  return window.moorDesktop.api<ProfileSetupCommand>({
...
export function exportProfileArchive(
  ...
  return window.moorDesktop.api<{ archive: string; ok: boolean }>({
...
export function importProfileArchive(
  ...
  return window.moorDesktop.api<{ desktop: null | ProfileDesktopOverlay; name: string; ok: boolean; path: string }>({
...
export function getUsageAnalytics(days = 30, profile?: null | string): Promise<AnalyticsResponse> {
  return window.moorDesktop.api<AnalyticsResponse>({
    ...profileScoped(profile),
...
export function getGlobalModelOptions(opts?: {
  ...
  return window.moorDesktop.api<ModelOptionsResponse>({
    ...profileScoped(),
...
export interface RecommendedDefaultModel {
  provider: string
  model: string
  /** True/false for Moor (free vs paid tier); null for other providers. */
...
export function getAuxiliaryModels(): Promise<AuxiliaryModelsResponse> {
  return window.moorDesktop.api<AuxiliaryModelsResponse>({
    ...profileScoped(),
...
export function getMoaModels(): Promise<MoaConfigResponse> {
  return window.moorDesktop.api<MoaConfigResponse>({
    ...profileScoped(),
    path: '/api/model/moa'
  })
}
...
export function saveMoaModels(body: MoaConfigResponse): Promise<MoaConfigResponse & { ok: boolean }> {
  return window.moorDesktop.api<MoaConfigResponse & { ok: boolean }>({
    ...profileScoped(),
    path: '/api/model/moa',
    method: 'PUT',
    body
...
export function updateMoor(): Promise<ActionResponse> {
  return window.moorDesktop.api<ActionResponse>({
    ...profileScoped(),
    path: '/api/moor/update',
    method: 'POST'
  })
}
...
export function checkMoorUpdate(force = false): Promise<BackendUpdateCheckResponse> {
  return window.moorDesktop.api<BackendUpdateCheckResponse>({
    ...profileScoped(),
    path: `/api/moor/update/check${force ? '?force=true' : ''}`
  })
}

export function getActionStatus(name: string, lines = 200, profile?: null | string): Promise<ActionStatusResponse> {
  return window.moorDesktop.api<ActionStatusResponse>({
    ...profileScoped(profile),
...
export function transcribeAudio(dataUrl: string, mimeType?: string): Promise<AudioTranscriptionResponse> {
  return window.moorDesktop.api<AudioTranscriptionResponse>({
...
export function speakText(text: string): Promise<AudioSpeakResponse> {
  return window.moorDesktop.api<AudioSpeakResponse>({
    ...profileScoped(),
...
export function getElevenLabsVoices(): Promise<ElevenLabsVoicesResponse> {
  return window.moorDesktop.api<ElevenLabsVoicesResponse>({
...
export function getSkillHubSources(profile?: null | string): Promise<SkillHubSourcesResponse> {
  return window.moorDesktop.api<SkillHubSourcesResponse>({
    ...profileScoped(profile),
...
export function searchSkillsHub(
  ...
  return window.moorDesktop.api<SkillHubSearchResponse>({
    ...profileScoped(profile),
...
export function scanSkillHub(identifier: string, profile?: null | string): Promise<SkillHubScanResult> {
  return window.moorDesktop.api<SkillHubScanResult>({
    ...profileScoped(profile),
...
// `moor mcp` and the dashboard MCP page). Raw JSON editing stays in
// config.yaml via saveMoorConfig.
// ---------------------------------------------------------------------------

export function listMcpServers(): Promise<{ servers: McpServerSummary[] }> {
  return window.moorDesktop.api<{ servers: McpServerSummary[] }>({
    ...profileScoped(),
    path: '/api/mcp/servers'
  })
}
...
export function getGhAuthStatus(refresh = false): Promise<{ available: boolean; authenticated: boolean }> {
  return window.moorDesktop.api<{ available: boolean; authenticated: boolean }>({
    ...profileScoped(),
...
export function installMcpCatalogEntry(
  ...
  return window.moorDesktop.api<{ ok: boolean; name?: string; pid?: number; action?: string; background?: boolean }>({
    ...profileScoped(profile),
...
// ---------------------------------------------------------------------------
// Memory data + curator (parity with `moor memory` / `moor curator`).
// ---------------------------------------------------------------------------

export function getMemoryStatus(): Promise<MemoryStatusResponse> {
  return window.moorDesktop.api<MemoryStatusResponse>({
    ...profileScoped(),
    path: '/api/memory'
  })
}
...
export function resetMemory(target: 'all' | 'memory' | 'user'): Promise<{ ok: boolean; deleted: string[] }> {
  return window.moorDesktop.api<{ ok: boolean; deleted: string[] }>({
    ...profileScoped(),
...
export function getCuratorStatus(): Promise<CuratorStatusResponse> {
  return window.moorDesktop.api<CuratorStatusResponse>({
    ...profileScoped(),
    path: '/api/curator'
  })
}
...
export function setCuratorPaused(paused: boolean): Promise<{ ok: boolean; paused: boolean }> {
  return window.moorDesktop.api<{ ok: boolean; paused: boolean }>({
    ...profileScoped(),
...
// ---------------------------------------------------------------------------
// Maintenance operations (parity with `moor doctor` / `moor security
// audit` / `moor backup` / `moor debug share` and the dashboard System
...
export function runDoctor(): Promise<ActionResponse> {
  return window.moorDesktop.api<ActionResponse>({ path: '/api/ops/doctor', method: 'POST', body: {} })
}

export function runSecurityAudit(): Promise<ActionResponse> {
  return window.moorDesktop.api<ActionResponse>({ path: '/api/ops/security-audit', method: 'POST', body: {} })
}

export function runBackup(): Promise<ActionResponse & { archive?: string }> {
  return window.moorDesktop.api<ActionResponse & { archive?: string }>({
    path: '/api/ops/backup',
    method: 'POST',
    body: {}
  })
...
export function runDebugShare(): Promise<DebugShareResponse> {
  return window.moorDesktop.api<DebugShareResponse>({
