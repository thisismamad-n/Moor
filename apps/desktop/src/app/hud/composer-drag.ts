...
function setWorkspaceTransfer(transferring: boolean): void {
  window.moorDesktop?.hud?.setWorkspaceTransfer?.(transferring)
}

function moveHud(state: PressState): void {
  window.moorDesktop?.hud?.moveBy?.({
    width: state.originW,
...
function armGrab(state: PressState, workspaceTransfer: boolean): void {
  ...
  window.moorDesktop?.hud?.beginMove?.()
}
 ...
 * (see moor:hud:move-by — a transparent frameless window drifts wider on
...
export function useHudComposerDrag(
  ...
  const reset = useCallback(() => {
    ...
    if (state) {
      if (state.armed) {
        window.moorDesktop?.hud?.endMove?.()
      }
