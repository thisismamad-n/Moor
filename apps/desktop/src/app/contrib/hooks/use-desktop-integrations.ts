...
import { pathFromMoorDeepLink, resolveMoorOpenPath } from '@/lib/moor-open-target'
...
export function useDesktopIntegrations({
  ...
  useEffect(() => {
    ...
    // "About Moor" — it is the OS-standard affordance for updating THIS app,
    ...
    const unsubscribe = window.moorDesktop?.onOpenUpdatesRequested?.(() => openUpdatesWindow('client'))
  ...
  // moor://index-network/intent/1 vocabulary with deep links.
  useEffect(() => {
    const unsubscribe = window.moorDesktop?.onNotificationActivate?.(payload => {
      if (!payload) {
        return
      }
      ...
      if (payload.activate) {
        ...
        // the pre-IPC validation — any future moorDesktop.notify caller gets
        // funneled through the same resolver.
        const path = resolveMoorOpenPath(payload.activate)
  ...
  useEffect(() => {
    const unsubscribe = window.moorDesktop?.onDeepLink?.(payload => {
      ...
      // link (moor://index-network/intent/1, moor://open/…). The resolver
      // rejects reserved kinds and unsafe paths.
      const path = pathFromMoorDeepLink(payload.kind, payload.name || '', payload.params || {})
  ...
  useEffect(() => {
    const unsubscribe = window.moorDesktop?.onPreviewNav?.(command => {
