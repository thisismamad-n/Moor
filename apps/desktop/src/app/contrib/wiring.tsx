...
import { useMoorConfigRecord } from '../hooks/use-config-record'
...
import { useMoorConfig } from '../session/hooks/use-moor-config'
...
export function ContribWiring({ children }: { children: ReactNode }) {
  ...
  const { refreshMoorConfig, sttEnabled, voiceMaxRecordingSeconds } = useMoorConfig({ activeSessionIdRef })
  ...
  useEffect(() => {
    ...
    window.addEventListener('moor:open-keybinds', onOpenKeybinds)

    return () => window.removeEventListener('moor:open-keybinds', onOpenKeybinds)
  }, [navigate])
  ...
  useEffect(() => {
    ...
    void refreshMoorConfig(true)
    void refreshActiveProfile()
    resetProjectTreeState()
  }, [gatewayScope, refreshCurrentModel, refreshMoorConfig])
  ...
  const configRecord = useMoorConfigRecord()
