...
function ResumeLastSessionSetting() {
  ...
  const configQuery = useMoorConfigRecord()
  ...
  const update = (on: boolean) => {
    ...
    void saveMoorConfig(setNested({}, 'display.resume_last_session', on), writeScope)
      .then(result => {
        if (!result.ok) {
...
function MarketplaceThemeResults({
  ...
  const search = useQuery({
    ...
    queryFn: () => window.moorDesktop?.themes?.searchMarketplace(debounced) ?? Promise.resolve([]),
