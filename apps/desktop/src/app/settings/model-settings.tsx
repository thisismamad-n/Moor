...
import { moorConfigCacheWriter, invalidateMoorConfig, useMoorConfigRecord } from '../hooks/use-config-record'
...
// backend now surfaces the full `moor model` universe (every canonical
...
// Mirrors `_AUX_TASK_SLOTS` in moor_cli/web_server.py. Friendly labels and
...
export function ModelSettings({ onMainModelChanged, scopeProfile }: ModelSettingsProps) {
  ...
  const { data: config } = useMoorConfigRecord(scopeProfile)
  ...
  const refresh = useCallback(
    async ({ replaceSelection = false }: { replaceSelection?: boolean } = {}) => {
      ...
      try {
        ...
        void invalidateMoorConfig(scopeProfile)
      } catch (err) {
        if (profileEpoch.current === epoch) {
  ...
  const writeAgentDefault = useCallback(
    async (key: string, value: string) => {
      ...
      try {
        await saveMoorConfig(next, scopeProfile)
      } catch (err) {
        setConfig(prev)
  ...
  const activateApiKeyProvider = useCallback(async () => {
    ...
    try {
      ...
      // `moor model` curation). Best-effort — fall through to the refreshed
  ...
  const recycleStaleBackend = useCallback(async () => {
    ...
    try {
      await window.moorDesktop?.recycleBackend?.(scopeProfile)
