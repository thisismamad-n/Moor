...
function ConfigSettingsInner({
  ...
  const { data: loadedConfig, isError: configLoadFailed, refetch: refetchConfig } = useMoorConfigRecord(scopeProfile)
  ...
  const writeConfigCache = useMemo(() => moorConfigCacheWriter(scopeProfile), [scopeProfile])
  ...
  const {
    ...
    queryKey:
      scopeProfile == null ? ['moor-config-schema'] : ['moor-config-schema', normalizeProfileKey(scopeProfile)],
    queryFn: () => getMoorConfigSchema(scopeProfile),
    staleTime: 5 * 60 * 1000
  })
  ...
  // never touched — possibly changed out-of-band by `moor config set`
  // while this page sat open — is never resent with its stale value.
  const configBaselineRef = useRef<MoorConfigRecord | null>(null)
    ...
    const t = window.setTimeout(() => {
      ...
      saveQueueRef.current = saveQueueRef.current.then(async () => {
        try {
          ...
          const result = await saveMoorConfig(patch, scopeProfile)
  ...
  const applyConfig = (next: MoorConfigRecord) => {
