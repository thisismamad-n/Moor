...
export function GatewaySettings({ embedded = false }: { embedded?: boolean } = {}) {
  ...
  const setKeychainEncryption = async (on: boolean) => {
    ...
    try {
      const res = await window.moorDesktop.setSecretStorageEncryption(on)
  ...
  useEffect(() => {
    if (state.mode !== 'ssh' || !window.moorDesktop?.sshConfigHosts) {
      return
    }
  ...
  const performSave = async (apply: boolean, allowPlainTextToken: boolean) => {
    ...
    try {
      const next = apply
        ? await window.moorDesktop.applyConnectionConfig(payload(allowPlainTextToken))
        : await window.moorDesktop.saveConnectionConfig(payload(allowPlainTextToken))
      ...
      const errors = {
        ...
        'moor-not-found': g.sshErrNotInstalled,
  ...
  const signIn = async () => {
    ...
    try {
      ...
      const saved = await window.moorDesktop.saveConnectionConfig({
        mode: state.mode,
      ...
      const result = await window.moorDesktop.oauthLoginConnectionConfig(trimmedUrl)
      ...
      if (result.connected) {
        const refreshed = await window.moorDesktop.getConnectionConfig(null)
  ...
  const signOut = async () => {
    ...
    try {
      await window.moorDesktop.oauthLogoutConnectionConfig(trimmedUrl)
      const refreshed = await window.moorDesktop.getConnectionConfig(null)
  ...
  const resolveSshHost = async (host: string) => {
    if (!host || !window.moorDesktop?.sshResolveHost) {
      return
    }
    ...
    try {
      const resolved = await window.moorDesktop.sshResolveHost(host)
  ...
  const testRemote = async () => {
    ...
    try {
      const result = await window.moorDesktop.testConnectionConfig({
        mode: 'remote',
        remoteAuthMode: authMode,
        remoteToken: authMode === 'token' ? remoteToken.trim() || undefined : undefined,
        remoteUrl: trimmedUrl
  ...
  if (!window.moorDesktop?.getConnectionConfig) {
    ...
    <SettingsContent bare={embedded}>
          ...
          <ListRow
            action={
              <Input
                ...
                placeholder="https://gateway.example.com/moor"
        ...
        <div className="mt-5 grid gap-1">
          ...
          <ListRow
            action={
              <Input
                className={cn('h-8 font-mono', CONTROL_TEXT)}
                onChange={event => setState(current => ({ ...current, sshRemoteMoorPath: event.target.value }))}
                placeholder={g.sshMoorPathPlaceholder}
                value={state.sshRemoteMoorPath}
              />
            }
            description={g.sshMoorPathDesc}
            title={g.sshMoorPathTitle}
          />
        </div>
      ) : null}
      ...
      {embedded ? null : (
        <div className="mt-6 grid gap-1">
          ...
          <ListRow
            action={
              <Button onClick={() => void window.moorDesktop?.revealLogs()} size="sm" variant="textStrong">
                <FileText />
                {g.openLogs}
              </Button>
            }
