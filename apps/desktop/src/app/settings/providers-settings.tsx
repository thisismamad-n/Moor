...
function OAuthPicker({
  ...
  // The free tier holds a token but no account: it is never "connected"; the featured Moor row
  // names it (Moor · free tier) and offers the sign-in that keeps its connectors.
...
function ConnectedProviderRow({
  ...
  // Moor can clear this provider's creds via the API.
  const canDisconnect = provider.disconnectable ?? provider.flow !== 'external'
  // External (CLI-managed) provider Moor can't clear via the API, but ships a
...
export function ProvidersSettings({
  ...
  // Moor never deletes creds another tool owns behind a silent API call.
