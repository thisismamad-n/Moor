...
beforeEach(() => {
  ...
  getMoorConfigSchema.mockResolvedValue({ fields: {} })
...
describe('ConfigSettings autosave', () => {
  it('sends a later revert instead of diffing it away against the stale page-load baseline', async () => {
    getMoorConfigRecord.mockResolvedValue({ checkpoints: { enabled: false }, other: 'untouched' })
    ...
    try {
      ...
      await waitFor(() => expect(saveMoorConfig).toHaveBeenCalledTimes(1))
      ...
      await waitFor(() => expect(saveMoorConfig).toHaveBeenCalledTimes(2))
      ...
      expect(saveMoorConfig.mock.calls[1][0]).toEqual({ checkpoints: { enabled: false } })
    } finally {
      vi.useRealTimers()
    }
  })
