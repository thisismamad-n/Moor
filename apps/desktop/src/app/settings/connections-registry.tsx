 ...
 * agent sources (local runtime + any number of remote gateways / Moor Cloud
...
export function ConnectionsRegistrySection() {
  ...
  useEffect(() => {
    if (!editorWantsOauth || !editorUrl || !window.moorDesktop?.probeConnectionConfig) {
      setAuthProbe(null)
  ...
  const signInOauth = useCallback(async () => {
    ...
    try {
      const result = await window.moorDesktop.oauthLoginConnectionConfig(editorUrl)
