...
import type { MoorConfigRecord, SessionInfo } from '@/types/moor'
  ...
  const persist = useCallback(
    async (autoArchive: boolean, archiveDays: number) => {
      ...
      try {
        ...
        await saveMoorConfig({ sessions: { auto_archive: autoArchive, auto_archive_days: archiveDays } })
      } catch (err) {
...
// / Program Files), which buried any files Moor wrote there.
function DefaultProjectDirSetting() {
  ...
  useEffect(() => {
    ...
    // / Storybook / non-Electron context `window.moorDesktop` is
    ...
    const settings = window.moorDesktop?.settings
