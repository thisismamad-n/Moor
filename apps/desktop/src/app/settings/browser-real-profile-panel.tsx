...
import { type ProfileScope, saveMoorConfigRecord } from '@/moor'
 ...
 * the default browser's profile (cookies/logins) into a moor-owned copy;
...
export function BrowserRealProfilePanel({ profile }: BrowserRealProfilePanelProps) {
  ...
  const { data: config } = useMoorConfigRecord(profile)
  ...
  const toggle = useCallback(
    async (on: boolean) => {
      ...
      try {
        ...
        await saveMoorConfigRecord({ browser: { use_real_profile: on } }, profile)
        notify({
          kind: 'info',
