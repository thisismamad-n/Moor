...
function provider(id: string, loggedIn: boolean, patch: Partial<OAuthProvider> = {}): OAuthProvider {
  return {
    cli_command: `moor auth add ${id}`,
    disconnectable: true,
    docs_url: '',
    flow: 'device_code',
    id,
    name: id === 'moor' ? 'Moor Portal' : 'MiniMax',
    status: {
      logged_in: loggedIn
    },
    ...patch
...
beforeEach(() => {
  ...
  disconnectOAuthProvider.mockResolvedValue({ ok: true, provider: 'moor' })
  listOAuthProviders.mockResolvedValue({
    providers: [provider('moor', true), provider('minimax-oauth', false)]
  })
})
...
describe('ProvidersSettings', () => {
  it('disconnects a connected provider account and refreshes the accounts list', async () => {
    ...
    const remove = await screen.findByRole('button', { name: 'Remove Moor Portal' })
    await act(async () => {
    ...
    await waitFor(() => expect(disconnectOAuthProvider).toHaveBeenCalledWith('moor'))
  ...
  it('keeps provider selection separate from account removal', async () => {
    ...
    expect(startManualProviderOAuth).toHaveBeenCalledWith('moor')
    expect(disconnectOAuthProvider).not.toHaveBeenCalled()
  })
    ...
    listOAuthProviders.mockResolvedValue({
      providers: [
        provider('qwen-oauth', true, {
          ...
          disconnect_hint: "Use `moor auth add qwen-oauth` or that provider's CLI to remove it.",
