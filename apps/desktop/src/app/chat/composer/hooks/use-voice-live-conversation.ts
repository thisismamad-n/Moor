...
interface VoiceLiveConversationOptions {
  ...
  enabled: boolean
  onFatalError?: () => void
  /** Interrupt the in-flight Moor turn (Stop-button seam). Fired when a new
   *  delegation supersedes one still running. */
  onInterrupt?: () => Promise<void> | void
  onStopWord?: () => void
  /** Submit a Moor turn: `text` is the user's last words (the bubble and the
...
/** Turn transcript fragments into the Moor turn: `prompt` is what the user
 ...
 * Moor. There is no `transcribing` phase: the voice model owns speech.
 */
export function useVoiceLiveConversation({
  ...
  // Drive the reply back into the voice: stream commentary as Moor writes
  ...
  useEffect(() => {
    ...
    const tick = () => {
      ...
      if (tool && tool !== lastToolLabelRef.current) {
        ...
        session.think(delegationId, `Moor is working: ${tool}. Not done yet.`)
      }
      ...
      if (
        ...
        if (spokenLengthRef.current === 0) {
          session.think(delegationId, 'Moor finished that request without a spoken result.')
        }
