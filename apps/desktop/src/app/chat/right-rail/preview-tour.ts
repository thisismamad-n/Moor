...
function buildTourScript(action: TourAction): string {
  return `(function () {
  var w = window;
  if (!w.__moorTourEngine) {
    ${driverIife}
    w.__moorTourHolder = {};
    w.__moorTourCollect = (${collectTourTargets.toString()});
    w.__moorTourEngine = (${runTourEngine.toString()});
  }
  if (!document.getElementById('__moor-tour-style')) {
    var style = document.createElement('style');
    style.id = '__moor-tour-style';
    style.textContent = ${JSON.stringify(driverCss)};
    (document.head || document.documentElement).appendChild(style);
  }
  return JSON.stringify(w.__moorTourEngine(
    w.driver.js.driver,
    w.__moorTourHolder,
