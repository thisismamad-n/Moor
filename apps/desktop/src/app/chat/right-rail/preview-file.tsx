...
import { MOOR_PATHS_MIME } from '@/app/chat/hooks/use-composer-actions'
...
async function readTextPreview(filePath: string) {
  try {
    ...
    if (!message.includes("No handler registered for 'moor:readFileText'")) {
      throw error
    }
  }
  ...
  const dataUrl = await window.moorDesktop.readFileDataUrl(filePath)
...
function startLineDrag(event: ReactDragEvent<HTMLElement>, filePath: string, { end, start }: LineSelection) {
  ...
  event.dataTransfer.setData(MOOR_PATHS_MIME, JSON.stringify([{ line: start, lineEnd, path: filePath }]))
