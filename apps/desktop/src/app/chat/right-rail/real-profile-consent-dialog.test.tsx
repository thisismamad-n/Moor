...
vi.mock('@/moor', () => ({
  saveMoorConfigRecord: (config: Record<string, unknown>, profile?: unknown) => mocks.save(config, profile)
}))

const promptCopy = {
  title: 'Stay signed in to your sites',
  body: 'Let Moor browse with a snapshot of your default browser profile.',
...
vi.mock('../../hooks/use-config-record', () => ({
  moorConfigCacheWriter: () => (config: Record<string, unknown>) => mocks.cache(config),
  useMoorConfigRecord: () => ({ data: mocks.loadedConfig })
}))

describe('RealProfileConsentDialog', () => {
  beforeEach(() => {
    mocks.loadedConfig = { browser: { allow_private_urls: false }, model: { provider: 'moor' } }
