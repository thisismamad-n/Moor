...
const preamble = () => `  var w = window;
  ...
  w.__moorActHolder = w.__moorActHolder || {};
  w.__moorAct = ${actEngineSource()};
  w.__moorWatch_fn = (${watchInPage.toString()});
  w.__moorWatchTag = ${WATCH_TAG};
  var holder = w.__moorActHolder;
  var act = function (a) { return w.__moorAct(document, holder, a); };
  var watch = function (stage, label) {
    try { w.__moorWatch_fn(document, holder, stage, label); } catch (err) {}
  };
...
${preamble()}
  ...
  document.addEventListener('pointerdown', function (e) {
    w.__moorHit = { tag: e.target ? e.target.tagName : '?', trusted: e.isTrusted === true };
...
function hitNote(hit?: { tag: string; trusted: boolean } | null): string | undefined {
  return hit && hit.tag === 'MOOR-WATCH' ? 'The action overlay intercepted the click instead of the page.' : undefined
}
