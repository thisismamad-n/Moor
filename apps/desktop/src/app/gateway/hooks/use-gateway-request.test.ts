...
vi.mock('@/moor', async importOriginal => {
  ...
  return { ...actual, MoorGateway: FakeMoorGateway }
})
...
const fakeGateway = { connectionState: 'open' } as unknown as MoorGateway
...
async function activateRemoteGateway() {
  ...
  setPrimaryGateway(primary as unknown as MoorGateway, 'default')
...
describe('useGatewayRequest', () => {
  ...
  it('surfaces a missing optional scoped mint bridge without falling back to a stale ticket or local lookup', async () => {
    const { desktop, gateway } = await activateRemoteGateway()
    Reflect.deleteProperty(window.moorDesktop, 'getGatewayWsUrlFor')
  ...
  it('rejects instead of hanging forever when the reconnect getConnection() wedges (#93454)', async () => {
    ...
    } as unknown as MoorGateway

    const getConnection = vi.fn(() => new Promise(() => undefined))

    ;(window as unknown as { moorDesktop: unknown }).moorDesktop = { getConnection }
    $gateway.set(dropped)
