...
export function useGatewayRequest() {
  ...
  const gateway = useStore($gateway) as MoorGateway | null
  const gatewayRef = useRef<MoorGateway | null>(null)

  const connectionRef = useRef<Awaited<ReturnType<NonNullable<typeof window.moorDesktop>['getConnection']>> | null>(
    null
  )

  const gatewayStateRef = useRef(gatewayState)
  const reconnectingRef = useRef<Promise<MoorGateway | null> | null>(null)
  ...
  useEffect(
    () =>
      $gateway.subscribe(gateway => {
        gatewayRef.current = gateway as MoorGateway | null
      }),
    []
  )
