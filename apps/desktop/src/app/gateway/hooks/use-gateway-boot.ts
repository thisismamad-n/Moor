import { isGatewayReauthRequired, JsonRpcGatewayError, resolveGatewayWsUrl } from '@moor/shared'
...
export function primaryRuntimeConnectionId(connection: Pick<MoorConnection, 'connectionId' | 'mode'>): null | string {
...
interface GatewayBootOptions {
  ...
  onConnectionReady: (
    connection: Awaited<ReturnType<NonNullable<typeof window.moorDesktop>['getConnection']>> | null
  ) => void
  onGatewayReady: (gateway: MoorGateway | null) => void
...
export function useGatewayBoot({
  ...
  useEffect(() => {
    ...
    // composer stayed disabled forever on "Starting Moor...". Once the
    ...
    const attemptReconnect = async () => {
      ...
      try {
        ...
        // "Starting Moor…". The probe is a no-op for a healthy or local backend.
    ...
    const softSwitch = async () => {
      ...
      try {
        ...
        await Promise.all([
          ...
          callbacksRef.current.refreshMoorConfig(false, ownsSwitch).catch(() => undefined),
    ...
    const gateway = adoptedFromHmr ? survivor!.gateway : new MoorGateway()
    ...
    async function boot() {
      try {
        ...
        await Promise.all([
          ...
          callbacksRef.current.refreshMoorConfig(),
