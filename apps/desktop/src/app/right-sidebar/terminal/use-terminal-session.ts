...
const MOOR_PATHS_MIME = 'application/x-moor-paths'
...
function transferHasDropCandidates(t: DataTransfer): boolean {
  if (t.types?.includes(MOOR_PATHS_MIME)) {
    return true
  }
...
function collectDroppedPaths(t: DataTransfer): string[] {
  ...
  try {
    const raw = t.getData(MOOR_PATHS_MIME)
...
export function useTerminalSession({
  ...
  useEffect(() => {
    ...
    const terminalApi = window.moorDesktop?.terminal
    ...
    const term = new Terminal({
      ...
      // Full-screen TUIs (moor --tui, vim) grab the mouse, so a plain drag
    ...
    term.attachCustomKeyEventHandler(event => {
      ...
      void (async () => {
        const text = (await window.moorDesktop?.readClipboard?.()) ?? ''
    ...
    const mount = () => {
      ...
      try {
        ...
        console.warn('[moor-terminal] WebGL unavailable; falling back to DOM', err)
      }
  ...
  useEffect(() => {
    ...
    return $terminalInjection.subscribe(command => {
      ...
      hasSessionActivityRef.current = true
      void window.moorDesktop?.terminal?.write(sessionId, `${command}\r`)
