...
beforeEach(() => {
  ...
  readDir.mockReset()
  ;(window as unknown as { moorDesktop: { readDir: typeof readDir } }).moorDesktop = { readDir }
})
...
describe('useProjectTree', () => {
  ...
  it('dedupes concurrent loadChildren calls for the same id', async () => {
    ...
    let resolveChildren: ((value: MoorReadDirResult) => void) | undefined
    readDir.mockImplementationOnce(
      () =>
        new Promise<MoorReadDirResult>(resolve => {
  ...
  it('discards a stale live refresh after the active registered connection changes', async () => {
    let resolveRefreshFromA: ((result: MoorReadDirResult) => void) | undefined
  ...
  it('discards a stale child read after the active registered connection changes', async () => {
    let resolveChildFromA: ((result: MoorReadDirResult) => void) | undefined
  ...
  it('discards a stale root read after the active registered connection changes', async () => {
    let resolveFirst: ((result: MoorReadDirResult) => void) | undefined
    readDir.mockImplementationOnce(
      () =>
        new Promise<MoorReadDirResult>(resolve => {
  ...
  it('returns no-bridge gracefully when window.moorDesktop is missing', async () => {
    delete (window as unknown as { moorDesktop?: unknown }).moorDesktop
  ...
  it('keeps an unreadable root on screen while it re-probes', async () => {
    ...
    let releaseProbe: ((value: MoorReadDirResult) => void) | undefined
  ...
  it('keeps loaded rows on screen while the root refreshes', async () => {
    ...
    let releaseRefresh: ((value: MoorReadDirResult) => void) | undefined
  ...
  it('clears the rows when the same path is re-read from another backend', async () => {
    ...
    let releaseFromB: ((value: MoorReadDirResult) => void) | undefined
