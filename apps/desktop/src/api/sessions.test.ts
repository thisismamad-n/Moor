...
const moorApi = vi.mocked(client.moorApi)
...
describe('deleteSession profile scoping', () => {
  it('scopes the DELETE to the owning profile in the URL (object owner)', async () => {
    ...
    vi.mocked(client.capabilityScoped).mockReturnValue({ profile: 'tommy', connectionId: 'moor-pi' })

    await deleteSession('sess-1', { connectionId: 'moor-pi', profile: 'tommy' })

    expect(moorApi.mock.calls[0][0]).toMatchObject({
      method: 'DELETE',
      path: '/api/sessions/sess-1?profile=tommy',
      connectionId: 'moor-pi',
      profile: 'tommy'
    })
  })
  ...
  it('omits the profile query when no owner is known', async () => {
    ...
    expect((moorApi.mock.calls[0][0] as { path: string }).path).not.toContain('profile=')
  })
