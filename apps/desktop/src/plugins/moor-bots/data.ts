...
import { atom, host, queryClient, useQuery, useValue } from '@moor/plugin-sdk'
...
export function newBotChat(bot: RosterRow) {
  if (typeof host.newChat !== 'function') {
    host.notify?.({
      kind: 'error',
      message:
        getPluginCtx()?.i18n?.t('bot.openAnotherChatUnsupported') ?? 'Update Moor Desktop to open another Bot chat.'
    })
