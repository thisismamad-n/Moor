 ...
 *  click path's tile-staleness probe (moor-agent#90102), which must
...
async function openStoredBotChat(
  ...
  // until an app restart (moor-agent#93604). A resume is cheap and
  ...
  if (route && typeof host.requestProfile !== 'function') {
    throw new Error(
      getPluginCtx()?.i18n?.t('bot.remoteConnectionsUnsupported') ??
        'Update Moor Desktop to chat with bots on other connections.'
    )
  }
