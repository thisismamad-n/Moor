 ...
 * bot-initiated sends use `moor -p <bot> chat --in ~ -c "Bot Chat"`.
 */
...
const BOT_ATTENTION_HINTS = {
  ...
  missing_config: 'Provider not configured — run moor model',
...
/** Mirrors host.paneVisibility('moor-bots:pane') — wired in register(). */
...
const GROUP_CHAT_SYNC_META_KEY = 'moor-bots-groups'
 ...
 *  clients (Moor Go, headless backends) see rooms regardless of which
...
// to (local, remote URL, SSH, Moor Cloud, docker) must be able to find
  ...
  try {
    ...
    return profiles
      .map(profile => ({
        ...
        title: String(profile?.ui_meta?.['moor-bots']?.title || profile?.display_name || ''),
...
async function saveBotMeta(owner, patch) {
  ...
  try {
    ...
    const request = route ? requestForBot(bot, 'profiles.configure', { name, ui_meta: { 'moor-bots': rest } }) :
      host.request('profiles.configure', { name, ui_meta: { 'moor-bots': rest } })
...
// handoffs (`moor -p <bot> chat --in ~ -c "Bot Chat" --create-if-missing`)
  ...
  for (const bot of roster) {
    ...
    if (image && typeof image === 'string' && image.startsWith('data:')) {
      ...
      Promise.resolve(request)
        .then(() => queryClient.invalidateQueries({ queryKey: ['moor-bots', 'roster'] }))
...
function mergeServerMeta(roster, fetchedAt = 0) {
  ...
  for (const bot of roster) {
    const server = bot.ui_meta?.['moor-bots']
 ...
 * directory (moor-agent#52279). That is the "can't delete a bot" error. */
...
if (typeof document !== 'undefined' && !document.getElementById('moor-bots-roster-css')) {
  const style = document.createElement('style')
  style.id = 'moor-bots-roster-css'
  style.textContent =
    '.moor-bots-roster [data-radix-scroll-area-viewport] > div {' +
    ' display: block !important; width: 100%; min-width: 0; }' +
    '.moor-scroll-cap > [data-radix-scroll-area-viewport] { max-height: inherit; }' +
    '@keyframes moor-bots-pulse { 0%,100% { opacity: 1; } 50% { opacity: 0.35; } }' +
    '.moor-bots-pulse { animation: moor-bots-pulse 1.2s ease-in-out infinite; }'
...
// and the row falls back to the "run moor mcp / Settings" hint. profile is
...
function McpSetupButton({ profile, entry, onDone, ensureProfile }) {
  ...
  const beginOAuth = async () => {
    ...
    try {
      ...
      } else if (typeof window !== 'undefined' && window.moorDesktop && window.moorDesktop.openExternal) {
        window.moorDesktop.openExternal(authUrl)
      } else {
...
function PetTab({ image, onImage }) {
  ...
  if (!pets.length) {
    return jsx('div', {
      ...
      children: 'No pets in the petdex gallery. Run `moor pets` to explore.'
    })
  }
 ...
 *  protocol into the system prompt itself (moor-agent bot_mode_probe).
 ...
 *  callable alias is 'moor' — the mention middleware resolves it back to
...
function botHandle(name, bot) {
  ...
  return (name || '').trim().toLowerCase() === 'default' ? 'moor' : name
}

/** Taggable @-forms derived from a bot's friendly names — the core profile
 *  display name (`moor profile rename`) and the Bot Mode title. Free text
 ...
 *  (researchbuddy). Reserved tokens are dropped so a bot renamed "Moor"
 *  can never hijack the primary profile's @moor alias. */
function mentionNameForms(value) {
  ...
  return [...new Set([slug, collapsed])].filter(
    form => /^[a-z0-9][a-z0-9_-]*$/.test(form) && !['all', 'everyone', 'user', 'default', 'moor'].includes(form)
  )
}
...
function botFriendlyNames(bot) {
  ...
  return [bot?.ui_meta?.['moor-bots']?.title, localTitle, aliasTitle, bot?.title, bot?.display_name]
}
...
function newBotChat(bot) {
  if (typeof host.newChat !== 'function') {
    host.notify?.({ kind: 'error', message: 'Update Moor Desktop to open another Bot chat.' })
...
function resolveRosterMentions(text, roster, active = {}) {
  ...
  for (const match of prose.matchAll(/(^|\s)@([a-z0-9][a-z0-9_-]*)/gi)) {
    ...
    if (token === 'moor') {
      token = byForm.has('moor') ? 'moor' : token
    }
...
// raw Cloud hostname, or to generic 'Moor' in Cloud-only mode.
//
...
// grandfather, or recover. Stored-id pins (ui_meta['moor-bots'].chat) were
...
async function openStoredBotChat(owner, storedId, summary) {
  ...
  // even though the session is fine (moor-agent#89617) — clicking Retry
...
async function prepareBotSource(bot) {
  ...
  if (route && typeof host.requestProfile !== 'function') {
    throw new Error('Update Moor Desktop to chat with agents on other connections.')
  }
...
async function ensureBotMetadata(bot) {
  ...
  const server = row?.ui_meta?.['moor-bots']
...
function displayName(bot, meta) {
  ...
  // "Moor". Annotated active rows carry sourceScoped too, and keying this
  ...
  // Core-profile display name (profile.yaml, set via `moor profile rename
  ...
  // never generic "Moor" or a hostname-derived label.
  if (alias) {
...
function durableGroupChatMembers(bots) {
  return (bots || []).map(bot => {
    ...
    const title = String(botRosterMeta(bot, $botMeta.get())?.title || bot.ui_meta?.['moor-bots']?.title || bot.title || '').trim()
...
function groupSpeakerLabel(name) {
  ...
  // Core profile display_name (`moor profile rename …` / dashboard) from
  ...
  return trimmed.toLowerCase() === 'default' ? 'Moor' : trimmed
}
...
function messagingProtocolSection(name, roster) {
  ...
  return [
    ...
    '',
    '```',
    'moor -p <agent-name> chat --in ~ -c "Bot Chat" --create-if-missing -Q -q "Message from \uD83E\uDD16 ' + handle + ' (@' + handle + '): your message"',
    '',
    ...
    '',
    'The roster grows over time — run `moor profile list` for the LIVE',
...
function previewKind(preview) {
  ...
  if (match) {
    ...
    // profile reads @moor, never @default (#89484).
 ...
 *  (moor-agent#90268). Older gateways omit worker_session — always false. */
    ...
    return jsxs('div', {
      ...
      children: [
        labeled(
          'Provider',
          jsx(Input, {
            placeholder: 'omnirouter / 9router / moor \u2026',
...
function AdvancedProfileConfig({ bot, state, setState }) {
  ...
  // (moor-agent#87317): Skills (installed list + one-click hub installs +
...
// {type: 'moor-skill-pick', ...} to us (moor-agent#86243). We validate
...
function HubSkillsSection({ forProfile, onInstalled }) {
  ...
  useEffect(() => {
    ...
    const onMessage = event => {
      ...
      if (!data || data.type !== 'moor-skill-pick' || !data.name) {
        return
      }
  ...
  const ensureAgentCreated = () => {
    ...
    const flight = singleFlight(flightRef, async () => {
      ...
      if (remoteTarget) {
        ...
        try {
          void requestForTarget('profiles.configure', { name: slug, ui_meta: { 'moor-bots': look } }).catch(() => undefined)
...
// moor -p <bot> chat delegation wrapper so the run reaches that bot's
...
function routinePrompt(bot, title, instruction, activeProfile) {
  ...
  return (
    ...
    `moor -p ${shellQuote(bot)} chat -c ${shellQuote(`Routine: ${title}`)} -q ${shellQuote(`[Scheduled routine] ${instruction}`)}\n\n` +
...
function GroupChatWorkspace({ group, members, onBack, visible = true }) {
  ...
  const renderEntry = (entry, index) => {
                  ...
                  // primary "default" profile renders as Moor, remote dupes
...
function BotsHomeView() {
  ...
  const gateway = bot.connectionLabel || (bot.connectionId === 'local' ? 'This device' : 'Moor gateway')
...
function GroupRow({ active, group, members, needsYou, onOpen, onDisband }) {
  ...
  // and the group prompt: the primary profile is @moor, not @default.
          ...
          : roster.length === 0
            ...
            : allBotsHidden && !hiddenExpanded
              ...
              : rosterRows.length === 0 && matchingHiddenBots.length === 0
                ...
                : jsx(ScrollArea, {
                    className: 'moor-bots-roster min-h-0 flex-1',
                    children: jsx('div', {
...
export default {
  ...
  register(ctx) {
    ...
    // excluded (a bot doesn't @ itself); 'default' surfaces as @moor.
    ctx.register({
    ...
    if (!document.getElementById('moor-bots-keyframes')) {
      const style = document.createElement('style')
      style.id = 'moor-bots-keyframes'
      style.textContent = '@keyframes moor-bots-bob { from { transform: translateY(0); } to { transform: translateY(-3px); } }'
      document.head.appendChild(style)
    }
            ...
            try {
              const registry =
                typeof window !== 'undefined'
                  ? await Promise.resolve(window.moorDesktop?.connections?.list?.()).catch(() => null)
                  : null
    ...
    try {
      if (typeof window !== 'undefined') {
        unbindConnectionsChanged =
          window.moorDesktop?.connections?.onChanged?.(payload => {
