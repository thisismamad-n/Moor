...
async function saveBotMeta(name, patch) {
  ...
  try {
    ...
    serverRequest = Promise.resolve(host.request('profiles.configure', { name, ui_meta: { 'moor-bots': rest } }))
  } catch {
        ...
        children: [
          jsxs('div', {
            ...
            children: [
              ...
              activeNow
                ? jsx('span', {
                    className: 'moor-bots-pulse size-1.5 shrink-0 rounded-full bg-(--ui-accent,#4f9cf9)',
...
function BotsPane() {
  ...
  const activityOf = bot => {
    const created = allMeta[bot.name]?.created || bot.ui_meta?.['moor-bots']?.created || 0
      ...
      data: {
        handler: async draft => {
          ...
          for (const match of prose.matchAll(/(^|\s)@([a-z0-9][a-z0-9_-]*)/gi)) {
            ...
            if (name === 'moor' && !names.includes('moor') && names.includes('default')) {
              name = 'default'
            }
          ...
          const note =
            ...
            mentioned.map(n => '`moor -p ' + shellQuote(n) + ' chat --in ~ -c "Bot Chat" --create-if-missing -Q -q "Message from \uD83E\uDD16 ' + shellDoubleQuote(senderName) + ' (@' + shellDoubleQuote(botHandle(active)) + '): <your composed message>"`').join('\n') +
