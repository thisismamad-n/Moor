...
function published(room: Room) {
  ...
  return (configure?.params.ui_meta as Record<string, Record<string, unknown>>)?.['moor-bots-groups']
}
...
describe('speaker labels', () => {
  it('the default profile speaks as Moor in transcripts, not @default', async () => {
    ...
    expect(line).toBe('Moor: hello room')
  ...
  it('honor friendly identity: Bot Mode title, then display_name, never a stale Moor', async () => {
    ...
    expect(chat.groupSpeakerLabel('default')).toBe('Moor')
...
describe('sync worker', () => {
  it('retries a gateway CAS conflict and publishes the merged room', async () => {
    ...
    await drain(() => (room.gateway.uiMetaRevisions['moor-bots-groups'] || 0) < 2, 60)

    const stored = room.gateway.uiMeta['moor-bots-groups'] as {
      rooms: Record<string, { log: GroupMessage[] }>
    }

    expect(room.gateway.uiMetaRevisions['moor-bots-groups']).toBe(2)
