 ...
 * `{type:'moor-skill-pick'}` back and the plugin installs via skills.manage.
 *
...
describe('hub pick messages', () => {
  it('installs the picked skill when it comes from our own frame', () => {
    ...
    expect(installCalls()).toEqual([['skills.manage', { action: 'install', query: 'moor/web-research' }]])
  })
  ...
  it('regression: refuses identifiers outside the slug charset', () => {
    ...
    for (const identifier of ['../../etc/passwd', 'skill; rm -rf /', '-flag', 'name with spaces', '']) {
      postPick({ identifier, name: 'Web Research', type: 'moor-skill-pick' }, { source: frame.contentWindow })
    }
  ...
  it('ignores messages that are not a skill pick', () => {
    ...
    postPick({ identifier: 'moor/web-research', type: 'oauth-callback' }, { source: frame.contentWindow })
    postPick({ identifier: 'moor/web-research', type: 'moor-skill-pick' }, { source: frame.contentWindow })
