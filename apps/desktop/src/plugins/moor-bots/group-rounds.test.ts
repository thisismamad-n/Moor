...
describe('routing', () => {
  ...
  it('resolves @moor to the default member', async () => {
    const { rounds } = await loadRoom()
    ...
    const parsed = rounds.parseGroupChatMentions('@moor take a look', members)
...
describe('turn prompt', () => {
  it('addresses the default profile as @moor', async () => {
    const { rounds } = await loadRoom()
    ...
    expect(own).toMatch(/You are @moor,/)
    ...
    expect(peer).toMatch(/group chat with @moor/)
  })
