 ...
 * job for a different profile keeps the `moor -p <bot> chat` delegation
...
/** Run the delegation command under a `moor` stub that prints its argv, so
...
function argvOf(prompt: string): string[] {
  ...
  const result = spawnSync('sh', ['-c', `moor() { printf '%s\\037' "$@"; }\n${command}`], { encoding: 'utf8' })
...
describe('direct vs delegated execution', () => {
  ...
  it('keeps the delegation wrapper for a different active profile', () => {
    ...
    expect(prompt).toMatch(/moor -p 'research' chat/)
