    ...
    host: {
      ...
      requestProfile: async (_route, method) =>
        method === 'profiles.list'
          ? { profiles: [{ name, ui_meta: { 'moor-bots': { chat: 'owner-chat' } } }] }
          : { sessions: [] },
...
test('behavior: remote default never opens the same-name local chat', async () => {
  ...
  const context = {
    ...
    host: {
      ...
      request: async () => ({ profiles: [{ name: 'default', ui_meta: { 'moor-bots': { chat: 'this-device-chat' } } }] }),
      notify: () => undefined,
