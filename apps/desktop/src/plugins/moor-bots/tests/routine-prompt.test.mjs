...
test('integration: a different active profile retains the delegated routine wrapper', () => {
  ...
  assert.match(prompt, /moor -p 'research' chat/)
...
test('security: delegated routine arguments remain literal shell values', () => {
  ...
  const command = prompt.slice(prompt.indexOf('moor '), prompt.lastIndexOf('\n\nIf the command'))
  const script = `moor() { printf '%s\\037' "$@"; }\n${command}`
...
test('security: upgrade pauses persisted delegated routines before they can execute', async () => {
  const titleSentinel = `/tmp/moor-bot-mode-title-${process.pid}`
  const taskSentinel = `/tmp/moor-bot-mode-task-${process.pid}`
  ...
  const oldPrompt =
    ...
    `moor -p research chat -c "Routine: ${title}" -q ${JSON.stringify(`[Scheduled routine] ${instruction}`)}\n\n` +
  ...
  const command = recreated.slice(recreated.indexOf('moor '), recreated.lastIndexOf('\n\nIf the command'))
  const script = `moor() { printf '%s\\037' "$@"; }\n${command}`
...
test('regression: Create Cronjob passes the active profile to routinePrompt', () => {
  ...
  assert.doesNotMatch(__routines.routinePrompt('ops', 'Check', instruction, 'ops'), /moor -p/)
})
