...
// The @mention middleware appends a handoff note whose moor command the
...
/** Run the note's first moor command under a stub that echoes each argv
 *  element — proves the shell received the interpolations as LITERALS. */
function runHandoffCommand(noteText) {
  const command = noteText.match(/`moor -p [^`]*`/)[0].slice(1, -1)
...
test('security: a poisoned bot title stays literal in the handoff command', async () => {
  const quoteSentinel = `/tmp/moor-bot-mode-mention-quote-${process.pid}`
  const subSentinel = `/tmp/moor-bot-mode-mention-sub-${process.pid}`
...
test('regression: the handoff command quotes the recipient argument', async () => {
  ...
  assert.match(result.text, /`moor -p 'ops' chat --in ~/)
})
