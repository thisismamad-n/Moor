...
function runtime() {
  ...
  const code = source
    .replace(/^import \* as sdk from '@moor\/plugin-sdk'\r?\n/m, '')
...
test('default rows use source identity without borrowing another source title', () => {
  ...
  // label. Titled: the title wins. Untitled: it stays "Moor". Regression:
  ...
  assert.equal(name(active, undefined), 'Moor')
})
...
test('botHandle: precomputed multi-source handle wins; default stays moor', () => {
  ...
  assert.equal(botHandle('default'), 'moor')
})
...
test('displayName: local default stays Moor; remote default uses the device label', () => {
...
test('resolveRosterMentions: @moor in this chat is not a handoff to yourself', () => {
  const { __resolveRosterMentions: resolve } = runtime()
  const roster = [
    { name: 'default', connectionId: 'local', handle: 'moor' },
    {
      name: 'default',
      connectionId: 'mac-mini',
      connectionLabel: 'Mac Mini',
  ...
  const hits = resolve('ask @moor and @default-mac-mini', roster, {
    name: 'default',
    connectionId: 'local'
  })
