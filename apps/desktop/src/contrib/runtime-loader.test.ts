...
describe('scanDiskPlugins (#66899)', () => {
  ...
  it('treats a package without a Desktop half as metadata, not a throwing file read', async () => {
    ...
    readDir.mockImplementation(async dir => {
      if (dir === '/local/.moor/plugins') {
        return { entries: [{ isDirectory: true, name: 'my-feature', path: '/local/.moor/plugins/my-feature' }] }
      }

      if (dir === '/local/.moor/plugins/my-feature') {
        return {
          entries: [{ isDirectory: false, name: 'plugin.yaml', path: '/local/.moor/plugins/my-feature/plugin.yaml' }]
        }
      }
    ...
    expect(readDir).toHaveBeenCalledWith('/local/.moor/plugins/my-feature')
    expect(readDir).not.toHaveBeenCalledWith('/local/.moor/plugins/my-feature/desktop')
  ...
  it('a DIRECTORY named plugin.js is not a plugin entry (metadata walk rejects it)', async () => {
    ...
    agentPluginsRoot.mockResolvedValue('')
    readDir.mockImplementation(async dir => {
      if (dir === '/local/.moor/desktop-plugins') {
        return { entries: [{ isDirectory: true, name: 'odd', path: '/local/.moor/desktop-plugins/odd' }] }
      }

      if (dir === '/local/.moor/desktop-plugins/odd') {
        // A folder literally named plugin.js — must resolve to "no entry".
        return {
          entries: [{ isDirectory: true, name: 'plugin.js', path: '/local/.moor/desktop-plugins/odd/plugin.js' }]
        }
      }
  ...
  it('loads a unified desktop half OPT-IN: inventoried but not activated by default', async () => {
    ...
    readDir.mockImplementation(async dir => {
      if (dir === '/local/.moor/plugins') {
        return { entries: [{ isDirectory: true, name: 'uni', path: '/local/.moor/plugins/uni' }] }
      }

      if (dir === '/local/.moor/plugins/uni') {
        return {
          entries: [{ isDirectory: true, name: 'desktop', path: '/local/.moor/plugins/uni/desktop' }]
        }
      }

      if (dir === '/local/.moor/plugins/uni/desktop') {
        return {
            ...
            ? [
                {
                  isDirectory: false,
                  name: 'plugin.js',
                  path: '/local/.moor/plugins/uni/desktop/plugin.js'
                }
              ]
            : []
        }
...
describe('plugin source reads (512 KiB preview-cap bug)', () => {
  ...
  const standaloneRootWith = (name: string) => {
    const folder = `/local/.moor/desktop-plugins/${name}`
  ...
  it('loads the full source via readPluginSource when the shell offers it', async () => {
    ;(window.moorDesktop as unknown as { readPluginSource: unknown }).readPluginSource = readPluginSource
    desktopPluginsRoot.mockResolvedValue('/local/.moor/desktop-plugins')
    agentPluginsRoot.mockResolvedValue('')
    ...
    try {
      ...
      expect(readPluginSource).toHaveBeenCalledWith('/local/.moor/desktop-plugins/big/plugin.js')
      expect(register).toHaveBeenCalledTimes(1)
  ...
  it('older shell without readPluginSource: a truncated preview read fails LOUDLY, never evaluates', async () => {
    ...
    try {
      ...
      expect($pluginRecords.get().huge).toMatchObject({
        kind: 'disk',
        status: 'error',
        file: '/local/.moor/desktop-plugins/huge/plugin.js'
      })
...
describe('bundled-shadowed disk copies', () => {
  it('skips a disk copy of a bundled plugin but publishes a visible inventory row', async () => {
    ...
    publishPlugin({ id: 'moor-bots', name: 'Bot Mode', kind: 'bundled', status: 'loaded' })
    ...
    try {
      const id = await loadRuntimePlugin(
        'export default { id: "moor-bots", name: "Bot Mode", register() {} }',
        'moor-bots',
        { file: '/local/.moor/desktop-plugins/moor-bots/plugin.js' }
      )

      // Skipped — the bundled copy stays the only live registration...
      expect(id).toBeNull()
      expect($pluginRecords.get()['moor-bots']).toMatchObject({ kind: 'bundled', status: 'loaded' })
      ...
      expect($pluginRecords.get()['moor-bots:disk-shadowed']).toMatchObject({
        kind: 'disk',
        status: 'disabled',
        file: '/local/.moor/desktop-plugins/moor-bots/plugin.js'
      })
