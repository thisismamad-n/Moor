...
/**
 * User widget apps — Moor authors its own TUI widgets, mirroring the
 * Python plugin contract: drop `<name>.mjs` into `$MOOR_HOME/tui-widgets/`,
 ...
 * React.createElement.
 *
 * Trust model matches `~/.moor/plugins/`: files under MOOR_HOME execute
...
const widgetsDir = () => join(process.env.MOOR_HOME?.trim() || join(homedir(), '.moor'), 'tui-widgets')
 ...
 *  every change, so a widget Moor writes appears within ~a second — no
...
export function watchUserWidgets(dir = widgetsDir()): void {
  ...
  if (!attach()) {
    ...
    // user (or Moor) ever writes must hot-load too — a 10s poll here read
