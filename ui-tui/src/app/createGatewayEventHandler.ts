...
const commitTheme = (theme: Theme) => {
  ...
  const pin = configPinnedTheme ? process.env.MOOR_TUI_THEME : undefined

  writeBootTheme(theme, process.env.MOOR_TUI_BACKGROUND, pin === 'light' || pin === 'dark' ? pin : undefined)
 ...
 * to MOOR_TUI_THEME — the priority-2 signal `detectLightMode` already
 * honors (only an explicit MOOR_TUI_LIGHT env var outranks it); 'auto'
 ...
 */
// True once CONFIG (via light/dark) owns the MOOR_TUI_THEME env pin, so an
...
export function applyConfiguredTuiTheme(raw: unknown): void {
  ...
  const current = process.env.MOOR_TUI_THEME ?? ''
  ...
  if (mode === 'light' || mode === 'dark') {
    ...
    process.env.MOOR_TUI_THEME = mode
  } else {
    // 'auto' clears only a pin CONFIG set — never a MOOR_TUI_THEME the user
    ...
    delete process.env.MOOR_TUI_THEME
  }
 ...
 * is cached into MOOR_TUI_BACKGROUND — the slot `detectLightMode` already
 ...
 * MOOR_TUI_LIGHT / MOOR_TUI_THEME overrides still win inside
...
export function syncThemeToTerminalBackground(): void {
  ...
  onTerminalBackground(hex => {
    ...
    resolved = true
    process.env.MOOR_TUI_BACKGROUND = hex
    reapplyTheme()
  })
  ...
  onTerminalForeground(hex => {
    if (resolved || process.env.MOOR_TUI_THEME || process.env.MOOR_TUI_LIGHT) {
      return
    }
    ...
    resolved = true
    process.env.MOOR_TUI_BACKGROUND = inferred
    reapplyTheme()
  })
  ...
  setTimeout(() => {
    if (
      ...
      process.env.MOOR_TUI_BACKGROUND ||
      process.env.MOOR_TUI_THEME ||
      process.env.MOOR_TUI_LIGHT ||
    ...
    execFile('defaults', ['read', '-g', 'AppleInterfaceStyle'], (error, stdout) => {
      if (resolved || process.env.MOOR_TUI_BACKGROUND || process.env.MOOR_TUI_THEME) {
        return
      }
      ...
      // inference).
      resolved = true
      process.env.MOOR_TUI_BACKGROUND = dark ? '#1e1e1e' : '#ffffff'
      reapplyTheme()
    })
  }, 1500).unref?.()
}
...
export function createGatewayEventHandler(ctx: GatewayEventHandlerContext): (ev: GatewayEvent) => void {
  ...
  const scheduleStartupPrompt = () => {
    ...
    setTimeout(async () => {
      ...
      // launches, `moor --tui -q "…"`) — submit LITERALLY, bypassing the
  ...
  const handleReady = (skin?: GatewaySkin) => {
    ...
    // Arm "Hey Moor" if this surface owns it (server gates on config).
    ...
    // forging a brand-new one.  Mirrors classic CLI's `moor -c` /
    // `moor --tui` muscle memory and addresses the audit's "session
  ...
  return (ev: GatewayEvent) => {
    ...
    switch (ev.type) {
      ...
      case 'wake.detected': {
        // "Hey Moor": optionally open a fresh session (start_new_session),
        ...
        void (async () => {
          ...
          if (wakeProfile && wakeProfile !== ownProfile) {
            sys(`wake phrase for profile '${wakeProfile}' — run: moor -p ${wakeProfile} --tui`)
