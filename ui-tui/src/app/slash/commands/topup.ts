import { driveChargeSettlement, type SettlementOutcome } from '@moor/shared/charge-settlement'
...
const renderBillingError = (
  ...
  switch (env.error) {
    ...
    case 'remote_spending_disabled':
      ...
      // turn it on from the portal's Moor Agent page; this is NOT a per-terminal stop.
      sys(
      ...
      ctx.gateway
        ...
        .then(
          ctx.guarded<BillingStateResponse>(s => {
            if (!s.logged_in) {
              sys('💳 Not logged into Moor Portal — run /portal to log in, then /topup.')
