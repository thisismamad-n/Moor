...
import { terminalBackgroundHex } from '@moor/ink'
...
export const debugCommands: SlashCommand[] = [
  ...
  {
    help: 'rescan $MOOR_HOME/tui-widgets and (re)register user widget apps',
    name: 'widgets-reload',
    run: (_arg, ctx) => {
  ...
  {
    help: 'write a V8 heap snapshot + memory diagnostics (see MOOR_HEAPDUMP_DIR)',
    name: 'heapdump',
    run: (_arg, ctx) => {
      ...
      ctx.transcript.panel('Theme', [
        {
          rows: [
            ...
            ['MOOR_TUI_BACKGROUND', process.env.MOOR_TUI_BACKGROUND ?? '(unset)'],
            ['MOOR_TUI_THEME', process.env.MOOR_TUI_THEME ?? '(unset)'],
