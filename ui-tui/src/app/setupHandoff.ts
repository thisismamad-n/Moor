import type { RunExternalProcess } from '@moor/ink'
...
export async function runExternalSetup({ args, ctx, done, launcher, suspend }: RunExternalSetupOptions) {
  ...
  transcript.sys(`launching \`moor ${args.join(' ')}\`…`)
  ...
  if (result.error) {
    transcript.sys(`error launching moor: ${result.error}`)
  ...
  if (result.code !== 0) {
    transcript.sys(`moor ${args[0]} exited with code ${result.code}`)
    patchUiState({ status: 'setup required' })
