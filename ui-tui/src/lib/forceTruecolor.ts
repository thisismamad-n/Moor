...
export function shouldForceTruecolor(env: NodeJS.ProcessEnv = process.env): boolean {
  const override = (env.MOOR_TUI_TRUECOLOR ?? '').trim()
...
if (shouldForceTruecolor()) {
  ...
  // incorrectly. Keep Moor on the safer TERM-driven 256-color path unless
  // users explicitly opt back in via MOOR_TUI_TRUECOLOR=1.
