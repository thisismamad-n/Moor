 ...
 * `moor-boot-background` / `moor-boot-color-scheme` localStorage keys.
 *
...
// Profile-aware: the Python launcher exports MOOR_HOME (set by
// _apply_profile_override) before spawning the TUI. Falling back to
// ~/.moor matches get_moor_home()'s default.
const bootFilePath = () => join(process.env.MOOR_HOME ?? join(homedir(), '.moor'), 'tui-theme-boot.json')

// Never touch the user's real ~/.moor from test runs (the TS suite has no
// MOOR_HOME isolation fixture).
...
export interface BootSeedResult {
  ...
  /** True when the cached config pin was seeded into MOOR_TUI_THEME. */
  seededPin: boolean
}
...
export function seedBootEnvironment(boot: BootTheme | null, env: NodeJS.ProcessEnv): BootSeedResult {
  ...
  if (!boot || env.MOOR_TUI_THEME || env.MOOR_TUI_LIGHT) {
    return result
  }
  ...
  if (
    ...
    !env.MOOR_TUI_BACKGROUND
  ) {
    env.MOOR_TUI_BACKGROUND = boot.background
...
export function invalidateBootBackground(env: NodeJS.ProcessEnv = process.env): boolean {
  if (!seeded.seededBackground || env.MOOR_TUI_BACKGROUND !== seeded.seededBackground) {
    return false
  }

  delete env.MOOR_TUI_BACKGROUND
...
/** True when this boot replayed a cached config pin into MOOR_TUI_THEME.
