...
const logDir = join(process.env.MOOR_HOME?.trim() || join(homedir(), '.moor'), 'logs')
...
// a real ~/.moor (tests must stay hermetic — see AGENTS.md).
...
export function recordParentLifecycle(line: string): void {
  ...
  try {
    ...
    if (!warned) {
      warned = true
      process.stderr.write('moor-tui: parent lifecycle log unavailable\n')
    }
  }
}
