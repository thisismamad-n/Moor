...
const ENV_KEYS = ['MOOR_AUTO_HEAPDUMP', 'MOOR_HEAPDUMP_DIR', 'MOOR_HEAPDUMP_MAX_BYTES'] as const
...
describe('performHeapDump auto opt-in gate (#21767)', () => {
  ...
  beforeEach(() => {
    ...
    dir = mkdtempSync(join(tmpdir(), 'moor-heapdump-test-'))
    process.env.MOOR_HEAPDUMP_DIR = dir
  })
  ...
  it('writes diagnostics only for auto-high without MOOR_AUTO_HEAPDUMP', async () => {
  ...
  it('writes diagnostics only for auto-critical without MOOR_AUTO_HEAPDUMP', async () => {
  ...
  it('accepts truthy spellings (true|yes|on, case-insensitive) as opt-in', async () => {
    for (const value of ['true', 'YES', 'On']) {
      process.env.MOOR_AUTO_HEAPDUMP = value
      const result = await performHeapDump('auto-high')
  ...
  it('writes both for manual triggers regardless of MOOR_AUTO_HEAPDUMP', async () => {
...
describe('heapdump retention guard (#21767)', () => {
  ...
  beforeEach(() => {
    savedDir = process.env.MOOR_HEAPDUMP_DIR
    savedMax = process.env.MOOR_HEAPDUMP_MAX_BYTES
    delete process.env.MOOR_AUTO_HEAPDUMP
    dir = mkdtempSync(join(tmpdir(), 'moor-heapdump-prune-'))
    process.env.MOOR_HEAPDUMP_DIR = dir
  })

  afterEach(() => {
    if (savedDir === undefined) {
      delete process.env.MOOR_HEAPDUMP_DIR
    } else {
      process.env.MOOR_HEAPDUMP_DIR = savedDir
    }

    if (savedMax === undefined) {
      delete process.env.MOOR_HEAPDUMP_MAX_BYTES
    } else {
      process.env.MOOR_HEAPDUMP_MAX_BYTES = savedMax
    }
  ...
  it('evicts oldest files when total bytes exceed the cap, retaining the newest', async () => {
    ...
    process.env.MOOR_HEAPDUMP_MAX_BYTES = String(2 * 1024)
