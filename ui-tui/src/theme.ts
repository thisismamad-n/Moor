...
// Mirrors @moor/ink's colorize.ts. Keep local: app code compiles from
// ui-tui/src, while @moor/ink is bundled separately from packages/.
...
export const DARK_SEEDS: ThemeSeeds = {
  accent: '#FFBF00',
  // The classic Moor navy surfaces are IDENTITY, not derivation drift —
...
// The classic light-mode Moor look was never hand-authored: for years the
 ...
 *  answer when known (cached in MOOR_TUI_BACKGROUND), else the mode's
 *  assumed pole. */
function referenceBackground(isLight: boolean, env: NodeJS.ProcessEnv = process.env): string {
  const cached = (env.MOOR_TUI_BACKGROUND ?? '').trim()
...
// Terminal. Explicit MOOR_TUI_THEME / COLORFGBG signals above still win,
...
// `MOOR_TUI_BACKGROUND` is intentionally generic so a future OSC11
...
//
//   1. `MOOR_TUI_LIGHT` boolean — `1`/`true`/`yes`/`on` → light;
//      `0`/`false`/`no`/`off` → dark.  Either explicit value wins
//      regardless of any later signal.
//   2. `MOOR_TUI_THEME` named override — `light` / `dark` win over
//      every signal below.
//   3. `MOOR_TUI_BACKGROUND` hex hint (3- or 6-digit) — luminance
...
//
// Anything we can't decide stays dark — the default Moor palette
// is the dark one.
export function detectLightMode(
  ...
  const lightFlag = (env.MOOR_TUI_LIGHT ?? '').trim().toLowerCase()
  ...
  const themeFlag = (env.MOOR_TUI_THEME ?? '').trim().toLowerCase()
  ...
  const bgHint = backgroundLuminance(env.MOOR_TUI_BACKGROUND ?? '')
 ...
 * the OSC-11 background answer cached into MOOR_TUI_BACKGROUND after
...
export function fromSkin(
  ...
  // itself into MOOR_TUI_BACKGROUND. See #applySkin / syncThemeToTerminalBackground).
  ...
  //    base seeds for this polarity. The base's IDENTITY FILLS (Moor navy
  ...
  // the derived value already carries the identity seeds (e.g. Moor navy).
  const activeRow =
