"""SQLite journal-mode and PRAGMA policy for state.db (split from moor_state).

Patchable helpers are looked up as module globals at call time, so tests patch ``moor_state_wal.<name>``.
"""
...
from moor_cli.sqlite_runtime import is_sqlite_wal_reset_vulnerable as _is_sqlite_wal_reset_vulnerable
from moor_state_errors import is_sqlite_lock_error

# Log-record parity with the origin module (caplog tests pin "moor_state").
...
# line would repeat per connection). Tests clear these via ``moor_state_wal.<name>``.
...
def _wal_reset_repair_hint() -> str:
    """Repair hint matching what ``moor update`` can actually do for this install type.

    See #75153.
    """
    try:
        from moor_cli.config import detect_install_method, get_project_root, recommended_update_command_for_method
        ...
        return "install a Python build bundled with SQLite 3.51.3+ (or backports 3.50.7 / 3.44.6) and restart Moor"
...
_ONCE_LOGS = {
    "wal_reset_bug": (_wal_reset_bug_warned_lock, "_wal_reset_bug_warned_paths", logging.WARNING,
        ...
        "%s. See `moor doctor`. This warning fires once per process per database."),
    ...
    "delete_overridden": (_delete_overridden_warned_lock, "_delete_overridden_warned_paths", logging.ERROR,
        ...
        "downgrade under open connections can corrupt the DB). To apply journal_mode=DELETE, stop every Moor "
        "process using this database and run `moor sessions set-journal-mode delete` (add `--db PATH` for a "
    ...
    "cross_vm_fs_existing_wal": (_cross_vm_existing_wal_warned_lock, "_cross_vm_existing_wal_warned_paths", logging.ERROR,
        ...
        "Fix one of two ways: stop every Moor process using this database and run `moor sessions "
...
_SYNCHROMOOR_LEVELS: Dict[str, int] = {"OFF": 0, "NORMAL": 1, "FULL": 2, "EXTRA": 3}
_SYNCHROMOOR_NAMES: Dict[int, str] = {v: k for k, v in _SYNCHROMOOR_LEVELS.items()}
_SYNCHROMOOR_FULL = 2


def resolve_synchromoor_level(raw_value: Any) -> Optional[int]:
    ...
    if isinstance(raw_value, int):
        return raw_value if raw_value in _SYNCHROMOOR_NAMES else None
    text = str(raw_value).strip()
    if text.upper() in _SYNCHROMOOR_LEVELS:
        return _SYNCHROMOOR_LEVELS[text.upper()]
    with contextlib.suppress(TypeError, ValueError):
        return int(text) if int(text) in _SYNCHROMOOR_NAMES else None
    return None


def _apply_synchromoor_pragma(conn: sqlite3.Connection, raw_value: Any, *, db_label: str) -> None:
    ...
    ``cache_size`` can. Darwin floor: this runs after :func:`_enforce_macos_synchromoor_full`, so a configured
    ``NORMAL`` would silently undo the btree protection — raising is allowed, lowering is refused out loud."""
    if (level := resolve_synchromoor_level(raw_value)) is None:
        logger.warning("%s: ignoring unrecognized database.synchronous=%r (expected OFF, NORMAL, FULL, EXTRA, or 0-3)",
                       db_label, raw_value)
        return
    if sys.platform == "darwin" and level < _SYNCHROMOOR_FULL:
        logger.warning("%s: refusing database.synchronous=%s on macOS; keeping FULL. Darwin's fsync() does not "
                       ...
                       "to prevent.", db_label, _SYNCHROMOOR_NAMES[level])
        return
    with contextlib.suppress(sqlite3.OperationalError):
...
def apply_database_pragmas(conn: sqlite3.Connection, *, db_label: str = "state.db") -> None:
    ...
    try:
        from moor_cli.config import cfg_get, load_config_readonly  # local: avoids a circular import
    ...
    # Last: sizing pragmas cannot change durability (see _apply_synchromoor_pragma).
    if (raw_synchronous := cfg_get(cfg, "database", "synchronous", default=None)) is not None:
        _apply_synchromoor_pragma(conn, raw_synchronous, db_label=db_label)
