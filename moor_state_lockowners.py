...
open-descriptor scan (``moor_state_holders``) cannot tell a reader from the one writer: every
Moor process has the DB open. SQLite's unix VFS takes ``fcntl`` byte-range locks whose offsets
