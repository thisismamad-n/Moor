...
Runs tool-calling agent tasks in moor-agent's execution environments (local,
docker, modal) and writes trajectories in Moor format (from/value pairs with
...
class MiniSWERunner:
    """Tool-calling agent loop over a Moor execution environment, emitting Moor trajectories."""
    ...
    def _convert_to_moor_format(self, messages: List[Dict[str, Any]], user_query: str) -> List[Dict[str, Any]]:
        """Convert the OpenAI-style message list to the Moor trajectory format used by batch_runner.py."""
        system_msg = MOOR_SYSTEM_PREFIX + f"<tools>\n{self._format_tools_for_system_message()}\n</tools>\n" + MOOR_SYSTEM_SUFFIX
    ...
    def run_task(self, task: str) -> Dict[str, Any]:
        ...
        try:
            ...
            self._cleanup_env()
        return {"conversations": self._convert_to_moor_format(messages, task), "completed": completed, "api_calls": api_call_count,
