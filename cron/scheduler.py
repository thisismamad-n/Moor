...
A file lock (~/.moor/cron/.tick.lock) keeps overlapping processes to one tick at a time.
"""
...
class CronTickYielded(RuntimeError):
    ...
    open). Raised, not returned, so ``record_ticker_error`` sees it and ``moor cron status``
    isn't green.
    """
...
def _summarize_cron_failure_for_delivery(job: dict, error: str | None) -> str:
    ...
    # (interrupted `moor update`, manual git pull) serves MIXED modules — old entries frozen in
    ...
    if provider_reachable and re.search(
        ...
        if skew is not None:
            ...
            message += (
                ...
                "`moor gateway restart` to fix it."
            )
...
def _merge_mcp_into_per_job_toolsets(per_job: list[str], cfg: dict) -> list[str]:
    ...
    if "no_mcp" in per_job:
        return result
    # lazy: avoid heavy moor_cli import at module load; shares MCP-membership with gateway/CLI
...
# Per-fire usage audit log; resolves via _get_moor_home() so profile-scoped paths work.
...
def _interpreter_shutting_down(exc: Optional[BaseException] = None) -> bool:
    ...
    A cron tick can fire while the gateway is tearing down — SIGTERM from ``moor update`` / ``moor
...
def _get_moor_home() -> Path:
    """Moor home at call time (honouring the test override). Cron is per-profile: never freeze
    ...
    resolving the active MOOR_HOME at call time means a profile's jobs are stored AND executed under that
...
def _load_cron_job_config(job: dict, job_id: str, job_name: str) -> _CronJobConfig:
    ...
    MOOR_MODEL > config ``model:``. Re-read every tick (no cache) so ``moor cron edit --model``
...
def _resolve_job_runtime(
    ...
    try:
        # Do NOT pass MOOR_INFERENCE_PROVIDER as `requested`: it would override persisted config
...
def _check_model_drift(
    ...
    if _finite_oneshot:
        ...
        _remediation = (
            "To run on the new config, on the host running Moor pin it explicitly: "
            f"`moor cron edit {job_id} --provider <provider> "
...
def _open_cron_session_db(job: dict):
    """Open the SQLite session store under its own timeout (MOOR_CRON_TIMEOUT only watches
    ...
    # handle (#96290). Bounded with its own timeout (separate from MOOR_CRON_TIMEOUT, which only watches
...
def _run_agent_with_watchdog(
    ...
    watchdog: default 600s, override MOOR_CRON_TIMEOUT, 0 = unlimited."""
...
def _final_response_from_result(result: dict, job_id: str, job_name: str, AIAgent) -> str:
    ...
    if final_response.strip() and turn_exit_reason:
        ...
        _explainer_variants = []
        try:
            from moor_state_errors import PERSISTENCE_ERROR_CAUSES as _causes
        except Exception:
...
def _finalize_cron_session(session_db, agent, job_id: str, job_name: str, cron_session_id: str) -> None:
    ...
    try:
        ...
        with contextlib.suppress((Exception, KeyboardInterrupt)):
            ...
            # MOOR_SESSION_KEY is not in the CLI environment, so the key resolves empty here. Since #64240
    ...
    # vocabulary in moor_state's session_lifecycle_statuses docstring — keep the tuple below in sync when
...
def _prepare_job_prompt(
    ...
    # user never chose. no_agent jobs are exempt. Escape hatch: MOOR_IGNORE_USER_CONFIG=1.
...
class _CronRunScope:
    ...
    MOOR_SESSION_* are deliberately NOT seeded from job["origin"]: it is delivery metadata, not
    a sender, and terminal/tts/skills/send_message tools would act as if the origin user were
    driving the agent. Delivery reads job["origin"] / MOOR_CRON_AUTO_DELIVER_* directly.
    """
    ...
    def __init__(self, job: dict, job_id: str, execution_id: Optional[str]):
        ...
        self._ctx_tokens = set_session_vars(
            ...
            # We clear the MOOR_SESSION_* routing keys just below, so an async delegation's completion
            ...
            # MOOR_SESSION_KEY, which risks routing a cron subagent's output into an unrelated user chat.)
        ...
        self._cron_session_var = _VAR_MAP["MOOR_CRON_SESSION"]
    ...
    def enter(self) -> None:
        ...
        # MOOR_KANBAN_TASK in env, and an unrelated job could close the worker's task. Must be a
...
def run_one_job(
    ...
    external_owner = os.environ.get("_MOOR_CRON_EXTERNAL_WORKER") == execution_id
...
def _run_one_job_body(
    ...
    try:
        ...
        _scope_token = set_secret_scope(build_profile_secret_scope(_get_moor_home()))
        ...
        _terminal_scope_token = install_profile_terminal_scope(_get_moor_home())
...
def _launch_external_cron_worker(job: dict) -> bool:
    ...
    execution_id = str(job["execution_id"])
    job_id = str(job["id"])
    handoff_dir = _get_moor_home() / "cron" / "external-workers"
    ...
    try:
        with os.fdopen(fd, "w", encoding="utf-8") as payload_file:
            json.dump(
                {
                    "job": job,
                    "profile_home": str(_get_moor_home().resolve()),
    ...
    worker_env = build_subprocess_env(
        ...
        extra={"MOOR_HOME": str(_get_moor_home().resolve())},
    )
    try:
        process = subprocess.Popen(
            scoped_command,
    ...
    try:
        with use_cron_store(profile_home):
            ...
            try:
                ...
                return False
            old_external_execution = os.environ.get("_MOOR_CRON_EXTERNAL_WORKER")
            os.environ["_MOOR_CRON_EXTERNAL_WORKER"] = execution_id
            try:
...
def _worktree_maintenance_repos() -> List[str]:
    """Repos whose ``.worktrees/`` to keep pruned: the moor checkout plus job workdir repo roots,
    filtered to those that actually have a ``.worktrees/`` dir."""
    repos: set = set()

    # Moor source checkout (git installs only; wheel installs have no .git).
    with contextlib.suppress(Exception):
...
def _maybe_run_worktree_maintenance() -> None:
    ...
    git. Same conservative pruner as ``moor -w`` startup (dirty/unpushed/locked trees untouched).
...
def _maybe_reap_dead_owners() -> None:
    ...
    # recovery previously ran only at scheduler STARTUP. A one-shot `moor cron run` that claimed a job and
...
if __name__ == "__main__":
    if "--external-worker-file" in sys.argv:
        ...
        try:
            ...
            setup_logging(moor_home=_get_moor_home(), mode="cron")
        except Exception:
            pass
        raise SystemExit(
