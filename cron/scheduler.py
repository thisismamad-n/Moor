...
def cron_delivery_targets() -> list[dict]:
    ...
    try:
        from moor_cli.profiles import list_profile_names
...
def _deliver_to_bot_chat(job: dict, content: str, profile: str) -> Optional[str]:
    ...
    Runs ``moor [-p <profile>] chat --in ~ -c "Bot Chat" --create-if-missing
    ...
    scheduler's MOOR_HOME) or a validated local profile name.  Returns None
    ...
    if moor_bin:
        ...
        try:
            ...
            if _ilu.find_spec("moor_cli") is not None:
                argv = [sys.executable, "-m", "moor_cli.main"]
            else:
                return "bot-chat delivery failed: moor CLI not resolvable"
        except Exception:
            return "bot-chat delivery failed: moor CLI not resolvable"
    ...
    if profile:
        argv += ["-p", profile]
        # -p owns profile resolution in the child; a leftover MOOR_HOME
    ...
    try:
        with tempfile.NamedTemporaryFile(
            "w", encoding="utf-8", suffix=".txt", prefix="moor-cron-botchat-",
            delete=False,
        ) as fh:
            fh.write(message)
            query_file = fh.name
...
def _resolve_bot_chat_target(job: dict, profile_arg: str) -> Optional[dict]:
    ...
    ``profile_arg`` is ``""`` for the job's own profile (the MOOR_HOME
    ...
    ``~/.moor/profiles/`` tree, so same-named profiles on other gateways
    can never be targeted by accident.
    """
    if not profile_arg:
        # Own profile: chat subprocess inherits MOOR_HOME, no name needed.
    ...
    for media_path, _is_voice in media_files:
        try:
            ...
            try:
                ...
                # MOOR_CRON_MEDIA_SEND_TIMEOUT env override).
...
def _deliver_result(job: dict, content: str, adapters=None, loop=None) -> Optional[str]:
    ...
    # at boot; a standalone process (manual `moor cron run` from the CLI,
...
def _get_media_send_timeout() -> int:
    ...
    MOOR_CRON_MEDIA_SEND_TIMEOUT env var wins, then
    ...
    """
    env_value = os.getenv("MOOR_CRON_MEDIA_SEND_TIMEOUT", "").strip()
    if env_value:
        try:
            timeout = int(float(env_value))
            if timeout > 0:
            ...
            logger.warning(
                "Invalid MOOR_CRON_MEDIA_SEND_TIMEOUT=%r; using config/default",
                env_value,
            )
...
def _run_job_script(
    ...
    * anything else — run with the current Python interpreter
    ...
    provider credentials and other moor-managed secrets are not inherited
