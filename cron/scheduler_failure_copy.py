...
Every notice says WHAT happened and WHAT TO DO, and names the exact ``moor cron`` command plus
...
def cron_output_dir_display(job_id: str) -> str:
    ...
    return f"{display_moor_home()}/cron/output/{job_id}/"
...
def generic_failure_notice(job_name: str, job_id: str, cleaned_error: str) -> str:
    ...
    return (
        ...
        f"See the full run with `moor cron runs {job_id}` (output saved under "
        f"{cron_output_dir_display(job_id)}); run it again with `moor cron run {job_id}`, "
        f"edit it with `moor cron edit {job_id}`, or pause it with `moor cron pause {job_id}`."
    )
...
def script_timeout_notice(job_name: str, job_id: str) -> str:
    return (
        ...
        f"Check the script's output under {cron_output_dir_display(job_id)} or `moor cron runs {job_id}`, "
        f"then run it again with `moor cron run {job_id}`."
    )
...
def inactivity_notice(job_name: str, job_id: str) -> str:
    return (
        ...
        f"{cron_output_dir_display(job_id)} (`moor cron runs {job_id}`), then run it again with "
        f"`moor cron run {job_id}`."
    )
