...
schedule via the `cronjob` tool; users via `moor cron list|add|edit|pause|resume|run|remove` or
...
- File lock `~/.moor/cron/.tick.lock` prevents duplicate ticks across processes.
...
Durable SQLite-backed board letting multiple profiles/workers collaborate. Users: `moor kanban
...
- **Dispatcher:** long-lived loop (default 60s) that reclaims stale claims, promotes ready tasks,
  ...
  (`kanban.dispatch_in_gateway: true`). Standalone: `plugins/kanban/systemd/moor-kanban-dispatcher.service`.
...
Isolation: **board** is the hard boundary — workers get `MOOR_KANBAN_BOARD` pinned in their env and
...
`tests/cron/`, `tests/moor_cli/test_kanban*.py`, `tests/tools/test_kanban*.py`. Schedule parsing
