...
def _existing_profile_homes(profile_homes: list) -> list:
    ...
    back on disk and in ``moor profile list`` (#47368). Filtering on directory existence leaves a deleted
...
def fire_overdue_jobs(
    ...
    from cron.jobs import (
        ONESHOT_GRACE_SECONDS, _ensure_aware, _moor_now, is_job_runnable, load_jobs,
    )
...
class InProcessCronScheduler(CronScheduler):
    ...
    def start(
        ...
        # Heartbeat before the first sleep so `moor cron status` sees a live ticker immediately.
