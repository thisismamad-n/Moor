...
(``moor cron notepad <job_id> set ...``) via the terminal tool; no model tool is added.
"""
...
# ticks (set_moor_home_override) cannot leak one profile's notepad rows into the import-time home
...
def _current_notepad_file() -> Path:
    return NOTEPAD_FILE or (get_moor_home().resolve() / "cron" / "notepad.db")
...
def _initialize_schema(conn: sqlite3.Connection) -> None:
    prepare_ledger(conn, db_label="cron/notepad.db", synchromoor_full=False)
    conn.execute(
...
def render_notepad_section(job_id: str) -> str:
    ...
    return (
        ...
        f"`moor cron notepad {job_id} set <key> <value>` "
        f"(also: get/delete/list; `moor cron notepad {job_id} delete "
