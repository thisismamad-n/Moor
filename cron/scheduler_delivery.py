...
def _open_continuable_cron_thread(job: dict, adapter, chat_id: str, loop) -> Optional[str]:
    ...
    if not callable(create_thread) or loop is None:
        return None
    thread_name = f"Moor — {job.get('name') or job.get('id', 'cron')}"
    try:
...
def _deliver_to_bot_chat(job: dict, content: str, profile: str) -> Optional[str]:
    ...
    ``moor [-p <profile>] chat --in ~ -c "Bot Chat" --create-if-missing -Q --query-file`` — the
    ...
    if profile:
        argv += ["-p", profile]
        # -p owns profile resolution; this scheduler's MOOR_HOME must not shadow it.
    ...
    try:
        with tempfile.NamedTemporaryFile(
            "w", encoding="utf-8", suffix=".txt", prefix="moor-cron-botchat-", delete=False,
        ) as fh:
            fh.write(message)
            query_file = fh.name
...
def _deliver_result(
    ...
    # job's own attempt: a worker's script may dispatch another job in-process (`moor cron run`),
    # and that nested delivery must not be keyed under the outer execution id.
    external_execution = os.environ.get("_MOOR_CRON_EXTERNAL_WORKER", "")
    ...
    # persisted as ``last_delivery_unverified`` so `moor cron list` shows it.
    ...
    # at boot; standalone runs (`moor cron run`) did not, silently dropping files. Idempotent.
