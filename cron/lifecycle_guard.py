...
A cron job that restarts/stops the gateway from inside the gateway (``moor gateway restart``,
``launchctl kickstart ai.moor.gateway``, ``systemctl restart moor-gateway``) kills the process,
...
_GATEWAY_LIFECYCLE_PATTERN = re.compile(
    r"(?i)"
    # Branch A: destructive `moor gateway` ops. `start` is excluded: starting from inside a
    # gateway is benign and a job may legitimately start a sibling profile. The lookbehind keeps
    # `moor` from being a path component or word tail (`/docs/moor gateway restart-notes.md`)
    ...
    # -l ai.moor.gateway-<suffix> -- <helper-script>` (or `launchctl bootstrap gui/<uid> <plist>`) creates
...
# split across continuation lines (e.g. `launchctl submit \` / `  -l ai.moor.gateway-... \` / `  -- ...`,
...
# Branch A2: `moor -p <profile> gateway restart|stop` (also `--profile <name>` /
...
# `moor -p venus gateway restart` operates on a sibling profile's gateway and is a legitimate fleet
...
# `$label` next to the verb. These verbs act on an EXISTING job, so the moor-gateway label anchor
...
def _current_profile_name() -> Optional[str]:
    """Profile running the guard: ``MOOR_PROFILE_NAME``/``MOOR_PROFILE`` env first, then
    ``moor_cli.profiles.get_active_profile_name`` (from ``MOOR_HOME``); ``None`` if neither."""
...
def contains_gateway_lifecycle_command(text: str) -> bool:
    ...
    ``moor``/``gateway`` token) — this function is the single choke point
    ...
    # <<'EOF'`) are masked before scanning (#88336): a runbook line "a human can run: moor gateway
    ...
    # Profile-flag form (#78028): `moor -p <profile> gateway restart|stop` bypasses Branch A because the
    # selector sits between `moor` and `gateway`. It is only the same foot-gun when the named profile IS
...
def _resolve_script_path(script_path: str) -> Optional[Path]:
    ...
    ``<MOOR_HOME>/scripts/``) so the guard scans the file that will actually run."""
    ...
    try:
        ...
        # get_moor_home() falls back to Path.home(), which raises when neither MOOR_HOME nor
...
def check_gateway_lifecycle(prompt: Optional[str], script: Optional[str] = None) -> None:
    ...
    if python_script:
        ...
        # The direct command regex below still scans the full text, so a literal `moor gateway restart`
