...
``cron.scheduler``, not ``moor_cli.main``, so nothing bootstraps the gateway's checkout
...
The shared subprocess sanitizer strips moor-owned PYTHONPATH entries because user
children must not see our tree. This child IS Moor, so the pin is applied *after* the
...
def pin_moor_tree_on_pythonpath(worker_env: dict, repo_root: Path) -> dict:
