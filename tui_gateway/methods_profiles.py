...
only have the ws door), on the same `moor_cli.profiles` primitives. Bodies are rebound onto
...
def _pin_profile_model(profile_dir, provider, model) -> None:
    _lazy("moor_cli.web_routers.profiles", "_write_profile_model")(profile_dir, provider, model)
...
def _resurrect_recoverable_canonical(db, profile_path, session_id):
    ...
    try:
        ...
        try:
            ...
            _best_effort(lambda: _lazy("moor_state_registry", "release_or_close")(wdb))
    except Exception:
        return False
...
def _canonical_session_row(db, profile_path):
    ...
    and its click target describe the same session (moor-agent#88200) with no client-side pointer
    involved.
    """
    try:
...
def _latest_profile_session_rows(db):
    ...
    (moor-agent#90268). Workers heartbeat ``last_activity_at`` every ≤60s while running (#72016), so a
...
def _(rid, params: dict) -> dict:
    """List Moor profiles. ``include_sessions`` (default true) adds ``last_session`` /
...
def _mirror_voice_sections(path) -> bool:
    ...
    try:
        from moor_cli.config import load_config_readonly, read_user_config_raw, save_config
...
def _inherit_launch_model(path) -> bool:
    ...
    from moor_cli.config import load_config_readonly, read_user_config_raw
    with _moor_home_scope(path):
...
def _mirror_launch_credentials(path, params: dict) -> dict:
    ...
    if not share_auth:  # a copy forks token state: the first refresh in either store strands the other
        ...
        if mirrored["auth"]:
            ...
            _best_effort(lambda: _lazy("moor_cli.auth", "strip_cloned_single_use_oauth_grants")(path))
...
def _describe_toolsets(cfg):
    ...
    default_off = _try(lambda: _lazy("moor_cli.tools_config", "_DEFAULT_OFF_TOOLSETS"), set())
    toolsets_out = []
...
def _configure_model(profile_dir, params, applied):
    ...
    if not is_truthy_value(params.get("confirm_expensive_model", False)):
        warn = _lazy("moor_cli.model_selection_guards", "combined_selection_warning")
...
def _configure_cfg_sections(profile_dir, params, applied) -> None:
    ...
    if want_mcp:  # launch catalog read BEFORE the home override flips config resolution
        load_launch = _lazy("moor_cli.config", "load_config_readonly")
    ...
    with _moor_home_scope(profile_dir):
        ...
        if isinstance(params.get("disabled_skills"), list):
            try:
                from moor_cli.skills_config import save_disabled_skills
