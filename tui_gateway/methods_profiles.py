...
handlers delegate to the same `moor_cli.profiles` primitives the REST
endpoints use.
...
def _(rid, params: dict) -> dict:
    ...
    def _canonical_session_row(profile_path):
        ...
        describe the same session (moor-agent#88200) with no client-side
        pointer involved.
    ...
    def _latest_profile_session_rows(profile_path):
        ...
        sessions never surface in conversation lists (moor-agent#90268).
    ...
    try:
        ...
        # append the protocol to SOUL.md (the desktop's moor-bots plugin)
...
def _(rid, params: dict) -> dict:
    ...
    ``moor setup`` in that flow to recover. A profile spawned as an
    ...
    # instead (moor_cli.auth: profile reads fall back to the global
    ...
    def _mirror_voice_sections() -> bool:
        ...
        profile via the context-local MOOR_HOME override — the same
    ...
    if model and provider:
        try:
            from moor_cli.web_routers.profiles import _write_profile_model
...
def _(rid, params: dict) -> dict:
    ...
    All reads are scoped to the profile via the MOOR_HOME override.
    """
    name = str(params.get("name") or "").strip()
    if not name:
    ...
    try:
        ...
        try:
            ...
            # Toolsets: the same filtered universe the `moor tools`
            ...
            # composites (moor-discord, feishu_drive, ...) and reports
            ...
            try:
                ...
                platform_enabled = set()
            try:
                from moor_cli.tools_config import _DEFAULT_OFF_TOOLSETS
            except Exception:
            ...
            for ts_name, ts_label, ts_desc in _get_effective_configurable_toolsets():
                ...
                # a per-profile editor — `moor tools` / Settings is where
