...
def _profile_build_scope(profile_home):
    """Bind MOOR_HOME + secret scope for an agent build (home alone leaves get_secret() on the LAUNCH .env)."""
    if not profile_home:
        yield
        return
...
def _resume_guard(ctx: _Resume) -> dict | None:
    ...
    from moor_state import SessionResumeTooLargeError, resolved_max_resume_messages
...
def _(rid, params: dict) -> dict:
    ...
    # the WS-detached sentinel: detached is attachable until grace-reap, and ``moor --tui`` rides stdio.
...
def _(rid, params: dict) -> dict:
    ...
    with _profile_db(params) as db:
        ...
        try:
            home = Path(profile_home) if profile_home is not None else get_moor_home()
...
def _(rid, params: dict, session: dict) -> dict:
    ...
    # Moor credits are agent-independent (portal fetch); fail-open when absent.
    with contextlib.suppress(Exception):
        from agent.account_usage import moor_credits_lines
        if credits := moor_credits_lines():
...
def _pet_config_followup(what: str, fn, *args) -> None:
    """Best-effort ``moor_cli.pets`` active-slug update after a store op that already succeeded."""
    try:
        fn(*args)
    except Exception as exc:  # noqa: BLE001
...
def _(rid, params: dict) -> dict:
    ...
    from moor_cli.moor_billing import post_subscription_preview
...
def _billing_route(name: str, call, *, invalid=None, message: str = "", error: str = "invalid_request",
    ...
    """Portal write route on ``moor_cli.moor_billing`` (lazy; tests patch its functions): ``invalid(params)``
...
def _(rid, params: dict, session: dict) -> dict:
    ...
    lines = [
        "Moor TUI Status", "", f"Session ID: {key}", f"Path: {display_moor_home()}",
...
def _(rid, params: dict, session: dict) -> dict:
    ...
    path = saved_dir / f"moor_conversation_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    with session["history_lock"]:
        messages = list(session.get("history", []))
