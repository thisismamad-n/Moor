...
# both (apps/desktop/src/plugins/moor-bots/relay.ts: RELAY_TURN_ATTEMPT_MS / RELAY_TURN_MAX_ATTEMPTS)
...
def _(rid, params: dict, _root=_relay_root, _run=_run_delivery) -> dict:
    ...
    GATEWAY via the one-turn ``moor -p <profile> chat -c "Bot Chat"`` transport local DMs use →
