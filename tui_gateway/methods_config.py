    ...
    try:
        with _profile_db(params) as db:
            ...
            with pdb.connect_closing() as conn:
                ...
                # zero Moor sessions still surface. The desktop's native scan
...
def _(rid, params: dict) -> dict:
    ...
    if key == "profile":
        ...
        return _ok(rid, {"home": str(_moor_home), "display": display_moor_home()})
    if key == "project":
...
def _(rid, params: dict) -> dict:
    """Upload a redacted debug bundle to Moor-internal diagnostics storage.

    Desktop's "Send Diagnostics" action (error card / diagnostics UI). Same
    collection + force-redaction pipeline as ``moor debug share --moor``
    (collect_share_bundle → build_moor_bundle → share_to_moor); redaction is
    ...
    try:
        ...
        blob = build_moor_bundle(bundle, redact=True)
