...
from moor_constants import display_moor_home as _display_moor_home
...
def _cfg_get_provider(params):
    from moor_cli.models import list_available_providers, normalize_provider
...
def _cfg_get_mtime(params):
    cfg_path = _moor_home / "config.yaml"
    try:
...
_CONFIG_GETTERS = {
    ...
    "profile": lambda params: {"home": str(_moor_home), "display": _display_moor_home()},
...
def _readiness_check(rid, params, probe):
    ...
    optional ``profile`` param's MOOR_HOME + ``.env`` secret scope (ContextVars: concurrent checks
    ...
    try:
        ...
        def probe(profile, scoped):
            ...
            if (not provider_configured and provider == "bedrock"
                ...
                return fail("No Moor provider is configured.", source)
...
@method("diagnostics.share_moor")
def _(rid, params: dict) -> dict:
    """Upload a redacted debug bundle to Moor-internal diagnostics storage — same collection +
    force-redaction pipeline as ``moor debug share --moor``; redaction is NOT client-controllable
    ...
    try:
        from moor_cli.debug import _redact_log_text, build_moor_bundle, collect_share_bundle
        ...
        res = share_to_moor(build_moor_bundle(bundle, redact=True))
