...
method("reload.env", params=ReloadEnvParams, result=ReloadEnvResult,
       doc="Re-read ~/.moor/.env (CLI /reload parity); built agents keep their pool until /new.")
...
class PluginsManageResult(Result):
    ...
    ``install`` → ``moor_cli.plugins_cmd.dashboard_install_plugin``'s ok payload; ``update`` →
    ``ok``/``unchanged``/``sha``."""
