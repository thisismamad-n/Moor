...
load_moor_dotenv(
    moor_home=_moor_home, project_env=Path(__file__).parent.parent / ".env"
)
...
# appends every unhandled exception to ~/.moor/logs/tui_gateway_crash.log
...
_CRASH_LOG = os.path.join(_moor_home, "logs", "tui_gateway_crash.log")
...
try:
    from moor_cli.banner import prefetch_update_check
...
try:
    _slash_timeout = float(os.environ.get("MOOR_TUI_SLASH_TIMEOUT_S") or "45")
...
def _resolve_ws_orphan_reap_grace() -> float:
    ...
    ``MOOR_TUI_WS_ORPHAN_REAP_GRACE_S`` env var is kept as an internal
    override for backward compatibility and wins when set.
    """
    raw = os.environ.get("MOOR_TUI_WS_ORPHAN_REAP_GRACE_S")
...
_LONG_HANDLERS = frozenset(
    {
        ...
        # pool checks, pricing fetch, Moor tier check, optional custom-provider
...
try:
    _rpc_pool_workers = max(
        2, int(os.environ.get("MOOR_TUI_RPC_POOL_WORKERS") or "8")
    )
except (ValueError, TypeError):
    _rpc_pool_workers = 8
...
_current_runtime_session_record: contextvars.ContextVar[dict | None] = (
    contextvars.ContextVar("moor_gateway_runtime_session_record", default=None)
)
...
def _prepend_tool_paths(env: dict[str, str]) -> dict[str, str]:
    ...
    try:
        ...
        managed_bin = str(Path(get_moor_home()) / "bin")
    except Exception:
        pass
...
class _SlashWorker:
    """Persistent MoorCLI subprocess for slash commands."""
    ...
    def __init__(self, session_key: str, model: str, profile_home: str | None = None):
        ...
        # gateway's launch MOOR_HOME (#40677). The override goes through the
        # build_subprocess_env factory's `extra` (applied last, always wins)
        # instead of a hand-rolled env["MOOR_HOME"] assignment.
        from tools.environments.local import build_subprocess_env
        env = build_subprocess_env(
            moor_subprocess_env(inherit_credentials=True),
            scrub_secrets=False,
            inherit_profile_home=False,  # base already carries the HOME contract
            extra={"MOOR_HOME": str(profile_home)} if profile_home else None,
        )
...
def _close_sessions_for_transport(
    ...
    for sid, session in owned:
        if session.get("close_on_disconnect"):
            ...
            # actually fire; a standalone `moor --tui` keeps real _stdio.
...
try:
    _SESSION_TTL_S = float(os.environ.get("MOOR_TUI_SESSION_TTL_S") or 6 * 3600)
...
def _transport_is_dead(transport) -> bool:
    ...
    # for a standalone `moor --tui`, so it must NOT count as dead here (doing
...
def _reclaim_orphaned_leases() -> None:
    ...
    try:
        from moor_cli.active_sessions import release_orphaned_leases
...
def _max_live_sessions() -> int:
    try:
        from moor_cli.active_sessions import coerce_max_concurrent_sessions
...
def _sweep_orphaned_session_rows() -> list[str]:
    ...
    ago (``MOOR_TUI_SESSION_TTL_S``). A freshly created row that copied
...
def _schedule_startup_orphan_sweep() -> None:
    ...
    reads the DB. ``MOOR_TUI_WS_ORPHAN_REAP_GRACE_S=0`` (park forever)
    and ``MOOR_TUI_SESSION_TTL_S=0`` both suppress the sweep; so does
...
# ``profile`` on those calls; we open that profile's db and bind its MOOR_HOME
...
def _profile_scoped(handler):
    """Bind ``params['profile']``'s MOOR_HOME around a handler.

    Pets (config + sprites) and projects (projects.db, discovery policy) both
    resolve via ``get_moor_home``. The desktop sends ``profile`` so a single
...
def _launch_configured_cwd() -> str | None:
    ...
    (wherever ``moor dashboard`` was launched) instead of the configured
...
def _inside_compute_host_child() -> bool:
    return os.environ.get("MOOR_COMPUTE_HOST_CHILD") == "1"
...
def _start_agent_build(sid: str, session: dict) -> None:
    ...
    Classic `moor` shows the prompt before constructing AIAgent; the TUI used
    ...
    def _build() -> None:
        ...
        try:
            ...
            # MOOR_HOME so config/skills/model resolve to it, and hand the
...
def _ensure_session_db_row(session: dict) -> None:
    ...
    * A terminal session (``moor`` / ``moor --tui`` / CLI) is started from a
...
def _load_dashboard_process_isolation_config(cfg: dict | None = None) -> dict[str, Any]:
    ...
    ``moor_cli.config.DEFAULT_CONFIG``. Keep
...
def _load_cfg_raw() -> dict:
    ...
    try:
        ...
        # launch profile's _moor_home. Cache is keyed on the resolved path, so
        # profiles don't clobber each other.
        override = get_moor_home_override()
        home = override if isinstance(override, str) and override else _moor_home
...
def _load_cfg() -> dict:
    ...
    ``moor_cli.config.load_config_readonly`` — managed-scope overlay and
...
def _apply_managed(cfg: dict) -> dict:
    ...
    moor_cli.config.load_config, so without this a managed skin / reasoning_effort
...
def _set_session_context(
    ...
    *,
    ...
    try:
        ...
        # can read MOOR_SESSION_ID. Without this, set_session_vars leaves the
        ...
        # MOOR_SESSION_ID even though agent_init set it via
...
def _enable_gateway_prompts() -> None:
    ...
    os.environ["MOOR_GATEWAY_SESSION"] = "1"
    os.environ["MOOR_EXEC_ASK"] = "1"
...
_TOUR_BRIDGE_UNAVAILABLE = json.dumps(
    {
        "success": False,
        "error": (
            "No Moor Desktop window answered the tour request. The tour is "
            ...
            "nothing listening. Update the Moor Desktop app and start a new "
...
def resolve_skin() -> dict:
    try:
        from moor_cli.skin_engine import init_skin_from_config, get_active_skin
...
def _broadcast_skin_if_changed() -> None:
    ...
    (``moor config set display.skin``) OR edited the active skin's colors in
...
def _watcher_home() -> Path:
    ...
    override = get_moor_home_override()
    return Path(override if isinstance(override, str) and override else _moor_home)
...
def _ensure_skin_watcher() -> None:
    ...
    threading.Thread(target=_loop, name="moor-change-watcher", daemon=True).start()


def _resolve_model() -> str:
    env = (
        os.environ.get("MOOR_MODEL", "")
        or os.environ.get("MOOR_INFERENCE_MODEL", "")
    ).strip()
    if env:
        return env
    ...
    try:
        from moor_cli.models import get_preferred_silent_default_model
...
def _resolve_session_platform() -> str:
    ...
    Resolution:
      * ``MOOR_DESKTOP=1`` and ``MOOR_DESKTOP_TERMINAL`` unset → "desktop"
        (the chat-panel backend — a graphical React surface, not a terminal).
      * ``MOOR_DESKTOP_TERMINAL=1`` → "tui"
        (``moor --tui`` running in the desktop's embedded terminal pane;
        it IS a TUI, just embedded. The clarifier attached to the tui hint
        in system_prompt.py tells the agent about the embedding.)
      * neither set → "tui"
        (standalone ``moor --tui``.)
    """
    if is_truthy_value(os.environ.get("MOOR_DESKTOP")) and not is_truthy_value(
        os.environ.get("MOOR_DESKTOP_TERMINAL")
    ):
        return "desktop"
    return "tui"
...
def _config_model_target() -> tuple[str, str]:
    ...
    Unlike `_resolve_model()`, this never reads MOOR_MODEL /
    MOOR_INFERENCE_MODEL. Those env vars are a launch-scoped seed
    (`moor --tui -m <model>`, hosted-instance provisioning); if they
    ...
    # No fallback to _resolve_model() here: that reads MOOR_MODEL /
    # MOOR_INFERENCE_MODEL, which `moor --tui -m <model>` sets as a
...
def _resolve_startup_runtime() -> tuple[str, str | None]:
    ...
    explicit_provider = os.environ.get("MOOR_TUI_PROVIDER", "").strip()
    ...
    try:
        ...
        current_provider = (
            ...
            or os.environ.get("MOOR_INFERENCE_PROVIDER", "").strip().lower()
            or "auto"
        )
...
from moor_state import _BARE_BILLING_PROVIDERS
...
def _persist_live_session_system_prompt(session: dict | None) -> None:
    ...
    # Re-bind MOOR_HOME to the session's profile so load_soul_md() and
    ...
    profile_home = session.get("profile_home")
    home_token = (
        set_moor_home_override(profile_home) if profile_home else None
    )
    try:
...
def _load_approval_mode() -> str:
    ...
    canonical ``moor_cli.config.load_config`` path applies managed-scope
...
def _load_reasoning_config(model: str = "") -> dict | None:
    ...
    :func:`moor_constants.resolve_reasoning_config` (per-model override >
...
def _load_tool_progress_mode() -> str:
    env = os.environ.get("MOOR_TUI_TOOL_PROGRESS", "").strip().lower()
...
def _gui_surface_toolsets(platform: str) -> set[str]:
    ...
    Both entries are deliberately off ``_MOOR_CORE_TOOLS`` — every other
    ...
    paths run with ``MOOR_DESKTOP=1``. Keying GUI capability off that env var
...
def _load_enabled_toolsets(platform: str | None = None) -> list[str] | None:
    ...
    explicit = [
        item.strip()
        for item in os.environ.get("MOOR_TUI_TOOLSETS", "").split(",")
        if item.strip()
    ]
    cfg = None
    ...
    # The desktop app and `moor --tui` both land here. See
    ...
    if explicit and validate_toolset is not None:
        ...
        if any(name in {"all", "*"} for name in built_in):
            ...
            if ignored:
                print(
                    "[tui] MOOR_TUI_TOOLSETS=all enables every toolset; "
        ...
        if unknown:
            print(
                f"[tui] ignoring unknown MOOR_TUI_TOOLSETS entries: {', '.join(unknown)}",
                file=sys.stderr,
                flush=True,
            )
        if disabled:
            print(
                "[tui] ignoring disabled MCP servers in MOOR_TUI_TOOLSETS "
        ...
        fallback_notice = (
            "[tui] no valid MOOR_TUI_TOOLSETS entries; using configured CLI toolsets"
        )
    ...
    try:
        ...
        if not enabled:
            return None
        # The client-surface toolsets are off _MOOR_CORE_TOOLS (every other
        # platform would carry their schema for nothing), so the platform
        # recovery above — which keys off moor-cli's tool universe — can't
        ...
        if fallback_notice is not None:
            print(
                "[tui] no valid MOOR_TUI_TOOLSETS entries and configured CLI toolsets could not be loaded; enabling all toolsets",
                file=sys.stderr,
                flush=True,
            )
        return None
...
def _apply_model_switch(
    ...
    *,
    ...
    #
    # We deliberately do NOT write process-global env vars (MOOR_MODEL /
    # MOOR_INFERENCE_MODEL / MOOR_TUI_PROVIDER / MOOR_INFERENCE_PROVIDER)
...
def _sync_agent_model_with_config(sid: str, session: dict) -> None:
    ...
    try:
        _apply_model_switch(
            ...
            # target the sync computed — the path that leaked `moor --tui -m`
...
def _get_usage(agent) -> dict:
    ...
    # MOOR_DEV_CREDITS so the payload stays clean when the flag is off.
    if is_truthy_value(os.environ.get("MOOR_DEV_CREDITS")):
        try:
...
def _session_info(agent, session: dict | None = None) -> dict:
    ...
    if agent is not None and not (session or {}).get("_compute_host_active"):
        try:
            ...
            pass
        try:
            from moor_cli.banner import get_available_skills
    ...
    try:
        ...
        pass
    try:
        from moor_cli.banner import get_update_result
...
def _wire_callbacks(sid: str):
    ...
    def secret_cb(env_var, prompt, metadata=None):
        ...
        from moor_cli.config import save_env_value_secure
...
def _available_personalities(cfg: dict | None = None) -> dict:
    """Built-ins + user overrides, via moor_cli.personality (single owner)."""
    from moor_cli.personality import available_personalities
...
def _validate_personality(value: str, cfg: dict | None = None) -> tuple[str, str]:
    ...
    Same contract as moor_cli.personality.resolve_personality — (name,
    ...
    """
    from moor_cli.personality import normalize_personality_name
...
def _prompt_text(value) -> str:
    ...
    Delegates to moor_cli.personality (single owner).
    """
...
def _cfg_max_turns(cfg: dict, default: int) -> int:
    ...
    env_val = os.environ.get("MOOR_TUI_MAX_TURNS")
    if env_val:
...
def _parse_tui_skills_env() -> list[str]:
    raw = os.environ.get("MOOR_TUI_SKILLS", "")
    skills: list[str] = []
    seen: set[str] = set()
...
def _load_fallback_model():
    ...
    stays in parity with ``MoorCLI.__init__`` and ``gateway/run.py``:
...
def _make_agent(
    ...
    # block. Dashboard /api/ws uses moor_cli.mcp_startup; TUI stdio keeps
    ...
    cfg = _load_cfg()
    from moor_cli.config import resolve_ephemeral_system_prompt_from_config
    ...
    return AIAgent(
        ...
        checkpoints_enabled=is_truthy_value(os.environ.get("MOOR_TUI_CHECKPOINTS")),
        pass_session_id=is_truthy_value(os.environ.get("MOOR_TUI_PASS_SESSION_ID")),
        skip_context_files=is_truthy_value(os.environ.get("MOOR_IGNORE_RULES")),
        skip_memory=is_truthy_value(os.environ.get("MOOR_IGNORE_RULES")),
...
def _init_session(
    ...
    with _sessions_lock:
        _sessions[sid] = {
            ...
            "edit_snapshots": {},
            "tool_started_at": {},
            # Profile-scoped MOOR_HOME for app-global remote mode; None =
...
def _build_persist_message_with_image_refs(user_text: str, image_paths: list[str]) -> str:
    ...
    format the desktop client (directive-text.tsx / MOOR_DIRECTIVE_RE)
...
def _session_home(session: dict) -> Path:
    """The MOOR_HOME the session's durable state lives in (profile-aware)."""
    profile_home = session.get("profile_home")
    return Path(profile_home) if profile_home else Path(_moor_home)
...
def _finalize_superseded_runtimes(stale: list[tuple[str, dict]]) -> None:
    ...
    moor_state_common._RECOVERABLE_END_REASONS so canonical Bot Chat
...
def _pet_gen_root():
    ...
    root = get_moor_home() / "cache" / "pet-gen"
...
try:
    _PET_REFERENCE_MAX_BYTES = max(
        1,
        int(os.environ.get("MOOR_PET_REFERENCE_MAX_BYTES") or str(16 * 1024 * 1024)),
    )
except (TypeError, ValueError):
...
#
# Layout:  $MOOR_HOME/spawn-trees/<session_id>/<timestamp>.json
...
def _spawn_trees_root():
    ...
    root = get_moor_home() / "spawn-trees"
    root.mkdir(parents=True, exist_ok=True)
    return root
...
def _collect_kanban_notifications(session: dict) -> list:
    ...
    ``platform="tui"`` and ``chat_id=MOOR_SESSION_KEY`` (see
    ...
    # DB when MOOR_KANBAN_DB pins the board path (same guard as the gateway
    # notifier).
...
def _wire_desktop_ui() -> None:
    ...
    Idempotent. The tool hands back the turn's ``MOOR_UI_SESSION_ID`` as
...
def _run_prompt_submit(
    ...
    *,
    ...
    def run():
        ...
        approval_token = None
        session_tokens = []
        home_token = None  # per-turn MOOR_HOME override for a resumed remote profile
        secret_token = None
        ...
        try:
            ...
            if _profile_home_str:
                home_token = set_moor_home_override(_profile_home_str)
            ...
            if images:
                try:
                    ...
                    from moor_cli.config import load_config as _tui_load_config
                ...
                try:
                    ...
                    def _thinking_should_play() -> bool:
                        if is_audio_output_active():
                            return False
                        try:
                            from moor_cli.voice import is_continuous_active
            ...
            # billing_url, is_moor, message) so the TUI/desktop render a
            ...
            if (
                ...
                try:
                    ...
                    logger.warning("voice TTS skipped: moor_cli.voice unavailable")
                except Exception as e:
            ...
            # Run while any profile-specific MOOR_HOME override is still active
...
def _session_images_dir(session: dict) -> Path:
    ...
    run BEFORE ``prompt.submit`` installs the session's profile MOOR_HOME
    override, so ``get_moor_home()`` here would return the gateway's launch
    ...
    """
    profile_home = session.get("profile_home")
    base = Path(profile_home) if profile_home else _moor_home
    return base / "images"
...
def _desktop_attachment_dir(session: dict) -> Path:
    ...
    installs the session's profile MOOR_HOME override, while the docker/ssh
...
def _(rid, params: dict) -> dict:
    ...
    if key == "model":
        try:
            ...
            if session:
                from moor_cli.model_switch import parse_model_switch_args
    ...
    if key == "yolo":
        ...
        try:
            ...
            if session:
                ...
                current = is_truthy_value(os.environ.get("MOOR_YOLO_MODE"))
                enable = _resolve_toggle(current)
                if enable:
                    os.environ["MOOR_YOLO_MODE"] = "1"
                    nv = "1"
                else:
                    os.environ.pop("MOOR_YOLO_MODE", None)
                    nv = "0"
    ...
    if key in {"prompt", "personality", "skin"}:
        try:
            ...
            if key == "prompt":
                ...
                # through moor_cli.personality (single owner) and never
...
def _is_repo_junk(root: str) -> bool:
    ...
    :func:`_non_workspace_dirs`) or anything under MOOR_HOME (~/.moor by
    ...
    moor_home = os.path.realpath(str(get_moor_home()))

    return (
        os.path.normcase(real) in _non_workspace_dirs()
        or real == moor_home
        or real.startswith(moor_home + os.sep)
    )
...
def _is_session_cwd_junk(cwd: str) -> bool:
    ...
    MOOR_HOME may be an intentional prose/data workspace. The pre-Projects
    desktop surfaced every such cwd, so exclude only the broad defaults that
    would create catch-all projects: MOOR_HOME itself and the dirs in
    ...
    moor_home = os.path.normcase(os.path.realpath(str(get_moor_home())))
    return real in _non_workspace_dirs() or real == moor_home
...
def _scan_discovered_repos_remote(conn, policy: dict) -> bool:
    ...
    zero Moor sessions still appear in the sidebar (#81723). Mirrors the
        ...
        try:
            for dirpath, dirnames, _filenames in os.walk(root):
                ...
                if ".git" in dirnames:
                    ...
                    # Not a repo: skip hidden dirs (e.g. .moor) and node_modules.
...
def _discover_repos_payload(
    ...
    repos even with zero moor sessions. Session-derived roots cover repos
    outside the scan roots. Both are junk-filtered (moor home subtree + bare
...
def _cli_exec_blocked(argv: list[str]) -> str | None:
    ...
    if not argv:
        return "bare `moor` is interactive — use `/moor chat -q …` or run `moor` in another terminal"
    a0 = argv[0].lower()
    if a0 == "setup":
        return "`moor setup` needs a full terminal — run it outside the TUI"
    if a0 == "gateway":
        return "`moor gateway` is long-running — run it in another terminal"
    if a0 == "sessions" and len(argv) > 1 and argv[1].lower() == "browse":
        return "`moor sessions browse` is interactive — use /resume here, or run browse in another terminal"
    if a0 == "config" and len(argv) > 1 and argv[1].lower() == "edit":
        return "`moor config edit` needs $EDITOR in a real terminal"
    return None
...
def _model_picker_context(agent):
    ...
    from moor_cli.inventory import load_picker_context
...
def _voice_mode_enabled() -> bool:
    ...
    """
    return os.environ.get("MOOR_VOICE", "").strip() == "1"


def _voice_tts_enabled() -> bool:
    """Whether agent replies should be spoken back via TTS (runtime only)."""
    return os.environ.get("MOOR_VOICE_TTS", "").strip() == "1"


def _any_session_running() -> bool:
    """True while any session's agent turn is in flight.

    Registered as the voice busy-probe (``moor_cli.voice.set_voice_busy_probe``)
...
def _speak_text_with_barge(text: str) -> None:
    """Speak *text* via moor_cli.voice.speak_text with spoken barge-in.
    ...
    """
    from moor_cli.voice import speak_text
...
def _(rid, params: dict) -> dict:
    ...
    * ``tts`` — toggle speech-output of agent replies. Requires mode on
    ...
    if action in {"on", "off"}:
        ...
        os.environ["MOOR_VOICE"] = "1" if enabled else "0"
    ...
    if action == "tts":
        ...
        os.environ["MOOR_VOICE_TTS"] = "1" if new_value else "0"
        if not new_value:
            _tts_stream_stop(user_barge=False)
...
def _(rid, params: dict) -> dict:
    ...
    try:
        if action == "start":
            ...
            from moor_cli.voice import start_continuous
            ...
            # are tolerated.
            try:
                from moor_cli.voice import set_voice_busy_probe
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        import moor_cli.voice  # noqa: F401
...
def _failure_messages(url: str, port: int, system: str) -> list[str]:
    from moor_cli.browser_connect import manual_chrome_debug_command
