...
from moor_constants import (
    get_moor_home, get_moor_home_override, reset_moor_home_override, set_moor_home_override)
...
_SLASH_WORKER_TIMEOUT_S = max(5.0, env_float("MOOR_TUI_SLASH_TIMEOUT_S", 45.0))
...
def _resolve_ws_orphan_reap_grace() -> float:
    ...
    return _ws_orphan_setting("MOOR_TUI_WS_ORPHAN_REAP_GRACE_S", "ws_orphan_reap_grace_s", 20.0)
...
_WS_ORPHAN_ACTIVITY_STALE_S = _ws_orphan_setting("MOOR_TUI_WS_ORPHAN_ACTIVITY_STALE_S", "ws_orphan_activity_stale_s", 600.0)
...
_rpc_pool_workers = max(2, env_int("MOOR_TUI_RPC_POOL_WORKERS", 8))
...
_current_runtime_session_record: contextvars.ContextVar[dict | None] = contextvars.ContextVar(
    "moor_gateway_runtime_session_record", default=None)
...
_current_rpc_method: contextvars.ContextVar[str] = contextvars.ContextVar("moor_gateway_rpc_method", default="")
...
def _prepend_tool_paths(env: dict[str, str]) -> dict[str, str]:
    ...
    ~/.local/bin to PATH so slash_worker children resolve moor-managed CLIs under the Desktop's minimal PATH."""
    managed_bin = ""
    with contextlib.suppress(Exception):
...
class _SlashWorker:
    ...
    def __init__(self, session_key: str, model: str, profile_home: str | None = None):
        ...
        # launch MOOR_HOME (#40677). The override goes through the build_subprocess_env factory's `extra`
        # (applied last, always wins) instead of a hand-rolled env["MOOR_HOME"] assignment.
        ...
        env = _prepend_tool_paths(build_subprocess_env(
            moor_subprocess_env(inherit_credentials=True), scrub_secrets=False,
            inherit_profile_home=False, extra={"MOOR_HOME": str(profile_home)} if profile_home else None))
        ...
        # Prepend the Moor venv bin dir and the user-local bin dir to PATH so slash_worker child processes
        # can resolve moor-managed CLIs (browser-use, uvx) even when the parent gateway was launched with
...
_SESSION_TTL_S = max(0.0, env_float("MOOR_TUI_SESSION_TTL_S", float(6 * 3600)))
...
_EXIT_FLUSH_BUDGET_S = max(0.0, env_float("MOOR_TUI_EXIT_FLUSH_BUDGET_S", 5.0))
_INCREMENTAL_FLUSH_INTERVAL_S = max(0.0, env_float("MOOR_TUI_SESSION_FLUSH_INTERVAL_S", _REAPER_SCAN_S))
...
# backend, so calls carry ``profile`` → open that profile's db and bind its MOOR_HOME (ContextVar
...
def _profile_home(profile: str | None) -> Path | None:
    ...
    try:
        ...
        return None
    if home.resolve() == Path(_moor_home).resolve() or not home.exists():
...
def _profile_scoped(handler):
    """Bind ``params['profile']``'s MOOR_HOME around a handler (pets/projects resolve via
    ``get_moor_home``, so app-global remote mode still hits the focused profile). No-op for launch.
...
def _bind_build_profile_scopes(profile_home: str) -> "_TurnScopes":
    """Bind a session profile's MOOR_HOME / secret / terminal scopes for an agent build. Fail-open per
...
def _release_build_profile_scopes(scopes: "_TurnScopes") -> None:
    if scopes.home is not None:
        reset_moor_home_override(scopes.home)
...
def _active_config_path() -> Path:
    ...
    return Path(override if isinstance(override, str) and override else _moor_home) / "config.yaml"
...
def _set_session_context(session_key: str, cwd: str | None = None, *, ui_session_id: str = "") -> list:
    with contextlib.suppress(Exception):
        ...
        # Live conversation id for subprocess MOOR_SESSION_ID: an explicitly empty contextvar is authoritative
...
def _enable_gateway_prompts() -> None:
    ...
    os.environ.update(MOOR_GATEWAY_SESSION="1", MOOR_EXEC_ASK="1", MOOR_INTERACTIVE="1")
...
_TOUR_BRIDGE_UNAVAILABLE = json.dumps({
    "success": False,
    "error": ("No Moor Desktop window answered the tour request. The tour is driven by the desktop app's "
              "renderer, which updates separately from this backend, so an app build older than the tour tool "
              "has nothing listening. Update the Moor Desktop app and start a new session. Do not retry tour "
              "in this session.")})
...
def _env_model_seed() -> str:
    """The launch-scoped model seed (``moor --tui -m``, hosted provisioning); "" when unset."""
    return (os.environ.get("MOOR_MODEL", "") or os.environ.get("MOOR_INFERENCE_MODEL", "")).strip()
...
def _resolve_session_platform() -> str:
    """``MOOR_DESKTOP=1`` without ``MOOR_DESKTOP_TERMINAL`` → "desktop" (chat panel; the agent then
    suggests TUI-only slash commands), else "tui" (embedded terminal pane or standalone ``moor --tui``)."""
    desktop = is_truthy_value(os.environ.get("MOOR_DESKTOP"))
    return "desktop" if desktop and not is_truthy_value(os.environ.get("MOOR_DESKTOP_TERMINAL")) else "tui"
...
def _config_model_target() -> tuple[str, str]:
    """(model, provider) selected by config.yaml — and ONLY config: the MOOR_MODEL launch seed fed into
...
def _resolve_startup_runtime() -> tuple[str, str | None]:
    ...
    if explicit_provider := os.environ.get("MOOR_TUI_PROVIDER", "").strip():
...
def _is_routable_provider(provider: str) -> bool:
    with contextlib.suppress(Exception):
        from moor_cli.runtime_provider import is_routable_provider
...
def _stored_session_runtime_overrides(row: dict | None) -> dict:
    ...
    room bots "out of Moor credits" after a profile switch); signals: ``room_plumbing`` /
...
def _persist_live_session_system_prompt(session: dict | None) -> None:
    ...
    if live is None or not hasattr(live[0], "_build_system_prompt") or not hasattr(live[2], "update_system_prompt"):
        return
    agent, session_key, db = live
    # Re-bind the session's profile MOOR_HOME (the build's finally reset it → root profile's SOUL.md/skills)
    ...
    home_token = set_moor_home_override(profile_home) if profile_home else None
...
def _load_reasoning_config(model: str = "") -> dict | None:
    """Via the shared chokepoint :func:`moor_constants.resolve_reasoning_config` (per-model override >
...
def _resolve_explicit_toolsets(explicit: list[str], validate_toolset) -> list[str] | None | bool:
    """Resolve a MOOR_TUI_TOOLSETS pin: list, None for "all", False when nothing was valid."""
    ...
    if any(name in {"all", "*"} for name in built_in):
        if ignored := [name for name in explicit if name not in {"all", "*"}]:
            _tui_notice(f"[tui] MOOR_TUI_TOOLSETS=all enables every toolset; ignoring additional entries: {', '.join(ignored)}")
        return None
    if not unresolved:
        return built_in
    ...
    if unknown:
        _tui_notice(f"[tui] ignoring unknown MOOR_TUI_TOOLSETS entries: {', '.join(unknown)}")
    if disabled:
        _tui_notice("[tui] ignoring disabled MCP servers in MOOR_TUI_TOOLSETS "
...
def _load_enabled_toolsets(platform: str | None = None) -> list[str] | None:
    """The agent's toolsets for this session (None = all): an explicit MOOR_TUI_TOOLSETS pin; else the
    ...
    explicit = [item.strip() for item in os.environ.get("MOOR_TUI_TOOLSETS", "").split(",") if item.strip()]
    ...
    try:
        ...
        if fallback_notice is not None:
            _tui_notice("[tui] no valid MOOR_TUI_TOOLSETS entries and configured CLI toolsets could not be loaded; enabling all toolsets")
        return None
...
def _get_usage(agent) -> dict:
    ...
    # Dev-only live credits-spent readout, gated on MOOR_DEV_CREDITS so the payload stays clean otherwise.
...
def _startup_system_prompt(cfg: dict, task_id: str) -> str:
    """Config ephemeral system prompt + MOOR_TUI_SKILLS preload block. Hard-fails only when EVERY requested
    ...
    if missing_skills:
        ...
        logger.warning("Unknown skill(s) requested, skipping: %s. Continuing with: %s. "
                       "List available skills with `moor skills list`.", missing_display, ", ".join(loaded_skills))
    if skills_prompt:
...
def _make_agent(
    ...
    for _mod in ("moor_cli.mcp_startup", "tui_gateway.entry"):
        with contextlib.suppress(Exception):
...
def _init_session(
    ...
    with _sessions_lock:
        _sessions[sid] = {
            ...
            # Profile-scoped MOOR_HOME (None = launch); SessionBranch copies the parent's (same state.db).
...
try:
    _PET_REFERENCE_MAX_BYTES = max(1, int(os.environ.get("MOOR_PET_REFERENCE_MAX_BYTES") or str(16 * 1024 * 1024)))
except (TypeError, ValueError):
...
_CLI_EXEC_BLOCKED = {
    ("setup",): "`moor setup` needs a full terminal — run it outside the TUI",
    ("gateway",): "`moor gateway` is long-running — run it in another terminal",
    ("sessions", "browse"): "`moor sessions browse` is interactive — use /resume here, or run browse in another terminal",
    ("config", "edit"): "`moor config edit` needs $EDITOR in a real terminal",
}
