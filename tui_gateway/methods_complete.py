...
def _profile_mention_items(prefix: str) -> list[dict]:
    ...
    is also offered as 'moor' when no real profile claims that name."""
    out: list[dict] = []
    try:
        ...
        if "moor".startswith(prefix.lower()) and "moor" not in seen:
            out.append(_item("@moor", "agent profile (primary)"))
    except Exception:
        return []
    return out
...
@_catch(5033)
def _(rid, params: dict) -> dict:
    from moor_cli.inventory import build_model_options_payload
...
def _(rid, params: dict) -> dict:
    ...
    if pconfig.auth_type != "api_key":
        return _err(rid, 4003, f"{pconfig.name} uses {pconfig.auth_type} auth — run `moor model` to configure")
...
def _(rid, params: dict) -> dict:
    ...
    from moor_cli.auth import PROVIDER_REGISTRY, clear_provider_auth
