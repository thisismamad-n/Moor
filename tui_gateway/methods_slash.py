...
def _format_live_review_output(sid: str, session: Optional[dict], arg: str) -> str:
    ...
    # (MOOR_UI_SESSION_ID + steer authority), or delegate_task registers the reviewer with no owner
...
def _format_live_history_output(sid: str, session: dict, arg: str) -> str:
    ...
    for idx, message in enumerate(messages, start=1):
        ...
        label = {"user": "You", "assistant": "Moor"}.get(role, role.title())
...
def _mirror_personality(sid, session, agent, arg) -> None:
    if arg and agent:
        ...
        from moor_cli.personality import persist_personality  # single owner: no surface drift
