...
@method("reload.env")
def _(rid, params: dict) -> dict:
    """Re-read ``~/.moor/.env`` into the gateway process via
    ``moor_cli.config.reload_env``, matching classic CLI's ``/reload``
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        r = subprocess.run(
            ...
            cwd=os.getcwd(),
            # cli.exec runs `python -m moor_cli.main` (can drive the agent) →
...
def _(rid, params: dict) -> dict:
    ...
    if name == "moa":
        ...
        try:
            from moor_cli.moa_config import moa_usage, normalize_moa_config
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        # Re-bind MOOR_HOME to the session's profile so get_skill_commands()
        ...
        _home_token = (
            set_moor_home_override(_profile_home) if _profile_home else None
        )
        try:
        ...
        sections = [
            ...
            },
            {
                ...
                "rows": [
                    ...
                    ["Config File", str(_moor_home / "config.yaml")],
                ],
            },
        ]
...
def _(rid, params: dict) -> dict:
    ...
    # Optional profile scoping: cronjob() keys off MOOR_HOME, so scoping the
...
def _(rid, params: dict) -> dict:
    ...
    gateway. Shares its renderer with the ``moor journey`` CLI.
    """
    try:
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        if action == "browse":
            from moor_cli.skills_hub import browse_skills
        ...
        if action == "inspect":
            from moor_cli.skills_hub import inspect_skill
...
def _(rid, params: dict) -> dict:
    ...
    keys], transport}]}`` — the same catalog `moor mcp` offers, so
...
# (moor_cli/web_routers/mcp.py) so a desktop plugin can manage MCP servers for
# ANY profile, not just the launch profile. Each accepts an optional ``profile``
# param that scopes MOOR_HOME via set_moor_home_override (omitted/None = the
# launch profile) in a try/finally, exactly like ``skills.manage`` / ``mcp.catalog``.
# All persistence reuses moor_cli/mcp_config.py helpers — no logic is duplicated.
...
def _(rid, params: dict) -> dict:
    ...
    if err:
        return err
    try:
        from moor_cli.config import load_config, save_config, save_env_value
        from moor_cli.mcp_config import (
...
def _(rid, params: dict) -> dict:
    ...
    if err:
        return err
    try:
        from moor_cli.mcp_config import _remove_mcp_server
...
def _(rid, params: dict) -> dict:
    ...
    (``window.moorDesktop.openExternal``) and then polls
    ...
    the SAME interactive MCP OAuth machinery ``moor mcp login`` uses
    ...
    try:
        ...
        result = mcp_oauth_sessions.start_flow(moor_home, name, cfg)
        return _ok(
            rid,
            {
                "ok": True,
...
def _(rid, params: dict) -> dict:
    ...
    primitives as ``moor plugins`` / the dashboard, so the three surfaces
    ...
    Actions:
      ...
      - ``install`` → git-clone into ``~/.moor/plugins/`` (non-interactive).
    ...
    plugins live under each profile's MOOR_HOME, so a client can list or
