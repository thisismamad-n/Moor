...
def _profile_scoped_rpc(
    ...
    """Wrap a handler body with the optional ``profile`` MOOR_HOME scope. Order: ``required``
    ...
    def deco(body):
        def handler(rid, params: dict) -> dict:
            ...
            if profile := _str_arg(params, "profile") if scoped else "":
                try:
                    profile_dir = _tools_mod("moor_cli.profiles").get_profile_dir(profile)
                    if not profile_dir or not profile_dir.is_dir():
                        return _err(rid, 4064, f"profile '{profile}' not found")
                    token = _tools_mod("moor_constants").set_moor_home_override(str(profile_dir))
...
def _scoped_rpc(name: str, fail_code: int = 5024, **kw):
    """``@method(name)`` + ``_profile_scoped_rpc`` (optional ``profile`` MOOR_HOME scope)."""
...
def _mcp_named_server(rid, params):
    ...
    name, servers = _str_arg(params, "name"), _tools_mod("moor_cli.mcp_config")._get_mcp_servers()
...
def _capture_run_kwargs(timeout: int) -> dict:
    ...
    return dict(
        ...
        stdin=subprocess.DEVNULL, creationflags=_tools_mod("moor_cli._subprocess_compat").windows_hide_flags())
...
_SIMPLE_RPCS = {
    ...
    # Re-read ``~/.moor/.env`` (CLI ``/reload`` parity); built agents keep their pool, ``/new`` resolves fresh.
    "reload.env": (5015, lambda params: {"updated": int(_tools_mod("moor_cli.config").reload_env())}),
    "plugins.list": (5032, lambda params: {"plugins": [
        {"name": n, "version": getattr(i, "version", "?"), "enabled": getattr(i, "enabled", True)}
        for n, i in _tools_mod("moor_cli.plugins").get_plugin_manager()._plugins.items()]}),
...
def _mcp_reload_confirm_required() -> bool:
    ...
    try:
        cfg = _tools_mod("moor_cli.config").load_config()
...
def _catalog_registry(cat: _Catalog) -> None:
    commands = _tools_mod("moor_cli.commands")
...
def _catalog_plugin_commands(cat: _Catalog) -> None:
    plugin_cmds = _tools_mod("moor_cli.plugins").get_plugin_commands() or {}
    if plugin_cmds:
...
def _(rid, params: dict) -> dict:
    ...
    return _ok(rid, {
        "pairs": cat.pairs, "sub": {k: v[:] for k, v in _tools_mod("moor_cli.commands").SUBCOMMANDS.items()},
        "canon": cat.canon,
...
@method("cli.exec")
def _(rid, params: dict) -> dict:
    """Run `python -m moor_cli.main` with argv; capture stdout/stderr (non-interactive only)."""
    ...
    return _captured_exec(
        ...
        env=moor_subprocess_env(inherit_credentials=True))


@_rpc("command.resolve", 5012)
def _(rid, params: dict) -> dict:
    r = _tools_mod("moor_cli.commands").resolve_command(params.get("name", ""))
    if r:
...
def _plugin_command_handler(name: str):
    try:
        return _tools_mod("moor_cli.plugins").get_plugin_command_handler(name)
    except Exception:
        return None


def _run_plugin_command(handler, arg: str) -> str:
    return str(_tools_mod("moor_cli.plugins").resolve_plugin_command_result(handler(arg)) or "")


def _is_profile_skill_command(session: dict, base: str) -> bool:
    """True when ``/base`` is a skill command of the session's profile (MOOR_HOME bound to it so
...
def _bundle_key_for(name: str):
    ...
    try:
        if _tools_mod("moor_cli.commands").resolve_command(name) is None:
...
_cmd_init = _prompt_builtin("moor_cli.init_command", "build_init_prompt_for_cwd", kw="extra")
...
def _cmd_moa(rid, params, session, name, arg):
    ...
    try:
        moa = _tools_mod("moor_cli.moa_config")
        if not arg:
...
def _cmd_focus(rid, params, session, name, arg):
    ...
    fv = _tools_mod("moor_cli.focus_view")
...
def _cmd_goal(rid, params, session, name, arg):
    ...
    from moor_cli.goals import goal_kick_prompt, last_user_message_from_db
...
def _cmd_loop(rid, params, session, name, arg):
    sid_key, loops, err = _session_key_or_err(rid, session, "moor_cli.loops", "loops")
    if err:
        return err
...
def _(rid, params: dict) -> dict:
    cfg = _load_cfg()
    api_key = _tools_mod("agent.secret_scope").get_secret("MOOR_API_KEY", "") or cfg.get("api_key", "")
    masked = f"****{api_key[-4:]}" if len(api_key) > 4 else "(not set)"
    base_url = os.environ.get("MOOR_BASE_URL", "") or cfg.get("base_url", "")
    sections = [
        {"title": "Model", "rows": [
        ...
        {"title": "Environment", "rows": [["Working Dir", os.getcwd()], ["Config File", str(_moor_home / "config.yaml")]]},
    ]
...
def _(rid, params: dict) -> dict:
    ...
    hc, tc = _tools_mod("moor_cli.config"), _tools_mod("moor_cli.tools_config")
    cfg = hc.load_config()
...
def _(rid, params: dict) -> dict:
    """cronjob() keys off MOOR_HOME, so ``profile`` reaches a per-profile cron store."""
...
def _skills_install(rid, params, query):
    ...
    _tools_mod("moor_cli.skills_hub").do_install(query, skip_confirm=True, console=quiet)
    return _ok(rid, {"installed": True, "name": query})


def _skills_browse(rid, params, query):
    pg = int(params.get("page", 0) or 0) or (int(query) if query.isdigit() else 1)
    browse = _tools_mod("moor_cli.skills_hub").browse_skills
    return _ok(rid, browse(page=pg, page_size=int(params.get("page_size", 20))))


_SKILLS_ACTIONS = {
    "list": lambda rid, params, query: _ok(rid, {"skills": _tools_mod("moor_cli.banner").get_available_skills()}),
    "search": _skills_search, "install": _skills_install, "browse": _skills_browse,
    "inspect": lambda rid, params, query: _ok(
        rid, {"info": _tools_mod("moor_cli.skills_hub").inspect_skill(query) or {}})}
...
# Gateway mirrors of the dashboard REST surface (moor_cli/web_routers/mcp.py) so a
# desktop plugin can manage MCP servers for ANY profile. Persistence: moor_cli/mcp_config.py.
@_scoped_rpc("mcp.catalog")
def _(rid, params: dict) -> dict:
    """``{servers: [{name, description, installed, enabled, requires: [env keys], transport}]}`` per profile."""
    mcp_catalog = _tools_mod("moor_cli.mcp_catalog")
    out = []
...
def _(rid, params: dict) -> dict:
    ...
    enabled, tools}]}``"""
    servers = _tools_mod("moor_cli.mcp_config")._get_mcp_servers()
...
def _(rid, params: dict) -> dict:
    ...
    import time
    hc = _tools_mod("moor_constants")
    configured = _tools_mod("moor_cli.mcp_config")._get_mcp_servers()
...
def _(rid, params: dict) -> dict:
    ...
    mc = _tools_mod("moor_cli.mcp_config")
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        cfg["auth"] = "oauth"
        moor_home = str(_tools_mod("moor_constants").get_moor_home().expanduser().resolve(strict=False))
        result = _tools_mod("tui_gateway.mcp_oauth_sessions").start_flow(
            moor_home, name, cfg, client_redirect_uri=client_redirect_uri)
...
def _(rid, params: dict) -> dict:
    """TUI Plugins Hub backend (shares primitives with ``moor plugins`` / the dashboard):
