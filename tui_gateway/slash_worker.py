...
# Stop a ``utils/`` (or ``proxy/``, ``ui/``) package in the launch directory from shadowing Moor's own
...
# retry loop. ``moor_bootstrap`` lives at the repo root (no collision risk), so importing it first is safe.
# ``moor_bootstrap`` lives at the repo root, so importing it is safe before the guard runs (its name won't
...
_WATCHDOG_POLL_S = max(0.05, env_float("MOOR_SLASH_WATCHDOG_POLL_S", 2.0))
_ORPHAN_GRACE_S = max(0.0, env_float("MOOR_SLASH_WATCHDOG_GRACE_S", 5.0))
...
def _prepare_slash_worker_runtime() -> None:
    """Start bounded MCP discovery before MoorCLI snapshots tools: each slash_worker child is its
    own process — the parent ``moor serve`` discovery thread does not populate this registry.

    See #61891.
    """
    from moor_cli.mcp_startup import start_background_mcp_discovery, wait_for_mcp_discovery
...
def main():
    ...
    # Start before the (hundreds-of-ms) MoorCLI build — that window is itself an orphan risk if the
