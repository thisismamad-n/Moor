...
activate with `moor --tui` or `MOOR_TUI=1`. `tui_gateway` is ALSO the backend the Desktop app
...
(`4000` + key path) and, under `MOOR_TEST_ISOLATION=1`, raises `ContractViolation` when a handler's
...
the launch home, never `~/.moor`. The first non-launch home hosted flips
...
**A method that sets only `get_moor_home_override()` is half-bound**: config paths resolve to the
...
(`moor -p X` workers, `key_cmd` helpers, browser drivers), never `dict(os.environ)`. Grep for
...
themselves come from `moor_cli/commands.py` (`moor_cli/AGENTS.md`).
