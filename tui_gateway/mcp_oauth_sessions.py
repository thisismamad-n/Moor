...
land. Reuses ``moor mcp login``'s probe under ``force_interactive_oauth`` plus
...
def _probe_with_rollback(
    server_name: str, cfg: dict, moor_home: str, flow, reconnect_live: bool) -> None:
...
def _worker(
        session_id: str, moor_home: str, server_name: str, cfg: dict, reconnect_live: bool) -> None:
    ...
    try:
        ...
        try:
            with force_interactive_oauth(), dashboard_oauth_flow(flow):
                _probe_with_rollback(server_name, cfg, moor_home, flow, reconnect_live)
        finally:
...
def start_flow(
    moor_home: str, server_name: str, cfg: dict, *, reconnect_live: bool = False,
    ...
    with _sessions_lock:
        ...
        if any(r["server_name"] == server_name and r["moor_home"] == moor_home for r in active):
    ...
    rec = {
        "session_id": session_id, "server_name": server_name, "moor_home": moor_home,
        "flow": flow, "httpd": httpd, "created_at": time.time()}
    with _sessions_lock:
        _sessions[session_id] = rec
    threading.Thread(
        target=_worker, args=(session_id, moor_home, server_name, dict(cfg), reconnect_live),
