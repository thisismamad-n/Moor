...
def _flush_sessions_before_exit(budget_s: float | None = None) -> int:
    ...
    hung SQLite write can't block exit past ``MOOR_TUI_EXIT_FLUSH_BUDGET_S`` (default 5s). Running sessions
    ...
    worker = threading.Thread(target=_run, daemon=True, name="moor-exit-flush")
    worker.start()
    worker.join(budget)
...
_HEARTBEAT_REFRESH_S = max(0.0, env_float("MOOR_GATEWAY_HEARTBEAT_REFRESH_S", 60.0))
...
def _start_backend_heartbeat_refresher() -> None:
    ...
    threading.Thread(target=_loop, name="moor-gateway-heartbeat", daemon=True).start()
