...
def _is_repo_junk(root: str) -> bool:
    ...
    MOOR_HOME. User-created projects pointing there are still honored."""
    if not root:
        return True
...
def _is_session_cwd_junk(cwd: str) -> bool:
    """A non-git cwd that stays in flat Recents. A DESCENDANT of MOOR_HOME may be an
    intentional prose/data workspace, so only MOOR_HOME itself is excluded here."""
    if not cwd:
        return True
    from moor_constants import get_moor_home
...
def _scan_discovered_repos_remote(conn, policy: dict) -> bool:
    ...
    host must scan its own disk so repos with zero Moor sessions still appear in the sidebar (#81723).
