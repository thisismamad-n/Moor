...
def _set_model(rid, params, key, value, session):
    ...
    if session:
        ...
        if parse_model_switch_args(str(value)).is_once:
            ...
            # One string for every client: the Ink TUI (dashboard /chat, `moor --tui`) has no
...
def _set_focus(rid, params, key, value, session):
    ...
    from moor_cli.focus_view import FOCUS_TOOL_PROGRESS_MODE, normalize_tool_progress_mode, resolve_focus_arg
    d_f = _display_cfg()
...
def _set_yolo(rid, params, key, value, session):
    ...
    if scope == "global":
        ...
        enable = _BOOL_WORDS.get(raw, not is_truthy_value(os.environ.get("MOOR_YOLO_MODE")))
        if enable:
...
def _set_cwd(rid, params, key, value, session):
    ...
    if Path(get_moor_home()).resolve() == Path(_moor_home).resolve():
...
def _set_personality(rid, params, key, value, session):
    ...
    # Persists via moor_cli.personality (single owner), never the user-owned system prompt.
