...
documents (stop Moor, recover or restore, start again). No marker is written to disk:
...
def mark_storage_corrupt(db_path, reason: object) -> None:
    ...
    logger.error(
        ...
        "until Moor restarts on a recovered or restored file. Stop Moor, then run "
        "`moor sessions recover --source %s --inspect-only` or restore a snapshot.",
