# Moor Agent Security Policy

This document describes Moor Agent's trust model, names the one
security boundary the project treats as load-bearing, and defines the
scope for vulnerability reports.

## 1. Reporting a Vulnerability

Report privately via [GitHub Security Advisories](https://github.com/thisismamad-n/Moor/security/advisories/new)
...
- Environment details (`moor --version`, commit SHA, OS, Python
  version).
...
- **Trust envelope.** The set of resources an operator has implicitly
  granted Moor Agent access to by running it — typically, whatever
  the operator's own user account can reach on the host.
- **Stance.** An explicit statement in Moor Agent's documentation
...
Moor Agent supports two OS-level isolation postures. They address
...
Moor Agent supports this in two ways:

- **Moor Agent's own Docker image and Compose setup.** Lighter-
...
Under a whole-process wrapper, Moor Agent's in-process heuristics
...
Moor Agent filters the environment it passes to its lower-trust
...
A malicious or buggy plugin is not a vulnerability in Moor Agent
itself. Bugs in Moor Agent's plugin-install or plugin-discovery
...
4. **Within the authorized set, all callers are equally trusted.**
   Moor Agent does not model per-caller capabilities inside a
...
- Trust-model documentation violations: code behaving contrary to
  what this policy, Moor Agent's own documentation, or reasonable
  operator expectations would predict — including cases where
  Moor Agent has documented a stance about how its output should
...
- **Documented break-glass settings.** Operator-selected trade-offs
  ...
  moor-home security, and similar. Reports against those
...
- **Community-contributed skills and plugins.** Third-party skills
  ...
  plugins are in the operator's review surface, not Moor Agent's
  trust surface (§2.4, §2.5). A skill or plugin doing something
  malicious is the expected failure mode of one that wasn't
  reviewed, not a vulnerability in Moor Agent. Bugs in Moor
...
- Moor Agent includes supply-chain guards for MCP server
