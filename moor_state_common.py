...
Extracted verbatim from moor_state.py so the SessionDB mixin modules
(moor_state_search / moor_state_schema / moor_state_portability) can
reference them without importing moor_state (which would be a cycle).
moor_state re-imports every name here for backward compatibility.
"""
...
# `moor sessions optimize-storage` rebuilds it on a capable host.
...
# service, the Desktop app's `moor serve` backend, interactive CLI sessions,
...
#
# Semantics mirror `moor_state._cross_process_repair_lock` (the schema-
...
# rebuilder cannot wedge future rebuilds. It lives here (not moor_state)
# because the search/schema mixins cannot import moor_state (cycle).
#
