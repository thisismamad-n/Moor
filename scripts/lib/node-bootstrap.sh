...
#   if [ "$MOOR_NODE_AVAILABLE" = true ]; then ...; fi
#
# Env inputs (set before sourcing to override defaults):
#   MOOR_NODE_MIN_VERSION   (default: 20)   — accepted on PATH
#   MOOR_NODE_TARGET_MAJOR  (default: 22)   — installed when we install
#   MOOR_HOME               (default: $HOME/.moor)
# ============================================================================

MOOR_NODE_MIN_VERSION="${MOOR_NODE_MIN_VERSION:-20}"
MOOR_NODE_TARGET_MAJOR="${MOOR_NODE_TARGET_MAJOR:-22}"
MOOR_HOME="${MOOR_HOME:-$HOME/.moor}"
MOOR_NODE_AVAILABLE=false
...
_nb_configure_npm_prefix() {
    ...
    printf 'prefix=%s\n' "$(dirname "$_link_dir")" > "$MOOR_HOME/node/etc/npmrc"
}
...
_nb_npm_range() {
    if [ -n "${MOOR_NPM_TARGET_RANGE:-}" ]; then
        printf '%s\n' "$MOOR_NPM_TARGET_RANGE"
        return 0
    fi
...
# The Python side recovers through moor_cli/npm_engine.py; the installer path
...
_nb_ensure_bundled_npm_range() {
    local npm_bin="$MOOR_HOME/node/bin/npm"
    [ -x "$npm_bin" ] || return 0
    ...
    _nb_warn "Fix manually: npm install -g --prefix \"$MOOR_HOME/node\" npm@\"$range\""
    return 1
}
...
_nb_try_fnm() {
    ...
    _nb_log "fnm detected — installing Node $MOOR_NODE_TARGET_MAJOR..."
    eval "$(fnm env 2>/dev/null)" || true
    fnm install "$MOOR_NODE_TARGET_MAJOR" >/dev/null 2>&1 || return 1
    fnm use     "$MOOR_NODE_TARGET_MAJOR" >/dev/null 2>&1 || return 1
...
_nb_try_proto() {
    ...
    _nb_log "proto detected — installing Node $MOOR_NODE_TARGET_MAJOR..."
    proto install node "$MOOR_NODE_TARGET_MAJOR" >/dev/null 2>&1 || return 1
    _nb_have_modern_node || return 1
...
_nb_try_nvm() {
    ...
    _nb_log "nvm detected — installing Node $MOOR_NODE_TARGET_MAJOR..."
    nvm install "$MOOR_NODE_TARGET_MAJOR" >/dev/null 2>&1 || return 1
    nvm use     "$MOOR_NODE_TARGET_MAJOR" >/dev/null 2>&1 || return 1
    _nb_have_modern_node || return 1
...
_nb_try_brew() {
    ...
    brew install "node@${MOOR_NODE_TARGET_MAJOR}" >/dev/null 2>&1 \
        || brew install node >/dev/null 2>&1 \
        || return 1
    brew link --overwrite --force "node@${MOOR_NODE_TARGET_MAJOR}" >/dev/null 2>&1 || true
    _nb_have_modern_node || return 1
...
_nb_install_bundled_node() {
    ...
    local index_url="https://nodejs.org/dist/latest-v${MOOR_NODE_TARGET_MAJOR}.x/"
    local tarball
    tarball=$(curl -fsSL "$index_url" \
        | grep -oE "node-v${MOOR_NODE_TARGET_MAJOR}\.[0-9]+\.[0-9]+-${node_os}-${node_arch}\.tar\.xz" \
        | head -1)
    if [ -z "$tarball" ]; then
        tarball=$(curl -fsSL "$index_url" \
            | grep -oE "node-v${MOOR_NODE_TARGET_MAJOR}\.[0-9]+\.[0-9]+-${node_os}-${node_arch}\.tar\.gz" \
            | head -1)
    fi
    if [ -z "$tarball" ]; then
        _nb_warn "Could not resolve Node $MOOR_NODE_TARGET_MAJOR binary for $node_os-$node_arch"
        return 1
    fi
    ...
    _nb_log "Extracting to $MOOR_HOME/node/..."
    ...
    local _link_dir
    _link_dir="$(_nb_get_link_dir)"
    # MOOR_NODE_SKIP_LINKS=1: the caller only wants the private managed tree
    ...
    if [ "${MOOR_NODE_SKIP_LINKS:-0}" != "1" ]; then
        mkdir -p "$_link_dir"
        ln -sf "$MOOR_HOME/node/bin/node" "$_link_dir/node"
        ln -sf "$MOOR_HOME/node/bin/npm"  "$_link_dir/npm"
        ln -sf "$MOOR_HOME/node/bin/npx"  "$_link_dir/npx"
    fi
    ...
    _nb_have_modern_node || return 1
    _nb_ok "Node $(node --version) installed to $MOOR_HOME/node/"
...
_nb_managed_tool_broken() {
    ...
    local probe
    for probe in \
        "$MOOR_HOME/node/bin/$tool" \
        "$MOOR_HOME/node/${tool}.exe" \
        "$MOOR_HOME/node/$tool"; do
...
# The managed node runs but is below MOOR_NODE_TARGET_MAJOR — an old tree
...
_nb_managed_node_outdated() {
    ...
    for probe in "$MOOR_HOME/node/bin/node" "$MOOR_HOME/node/node"; do
        [ -x "$probe" ] || continue
        ver="$("$probe" --version 2>/dev/null)" || return 1
        major="${ver#v}"; major="${major%%.*}"
        case "$major" in ''|*[!0-9]*) return 1 ;; esac
        [ "$major" -lt "$MOOR_NODE_TARGET_MAJOR" ] && return 0
        return 1
    done
    return 1
}
...
# absent. Used by moor_constants.find_moor_node_executable() and safe
# to call from install reruns.
heal_managed_node() {
    [ -d "$MOOR_HOME/node" ] || return 1
...
ensure_node() {
    ...
    _nb_try_fnm   && { MOOR_NODE_AVAILABLE=true; return 0; }
    _nb_try_proto && { MOOR_NODE_AVAILABLE=true; return 0; }
    _nb_try_nvm   && { MOOR_NODE_AVAILABLE=true; return 0; }

    # Platform package managers.
    _nb_try_termux_pkg && { MOOR_NODE_AVAILABLE=true; return 0; }
    _nb_try_brew       && { MOOR_NODE_AVAILABLE=true; return 0; }

    # Last resort: pinned nodejs.org tarball.
    _nb_install_bundled_node && { MOOR_NODE_AVAILABLE=true; return 0; }

    _nb_warn "Node.js install failed — TUI and browser tools will be unavailable."
    _nb_warn "Install manually: https://nodejs.org/en/download/  (or: \`brew install node\`, \`fnm install $MOOR_NODE_TARGET_MAJOR\`, etc.)"
    return 1
}
