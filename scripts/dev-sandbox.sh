#!/usr/bin/env bash
# Run a Moor instance in an isolated sandbox — separate MOOR_HOME,
...
# .moor-sandbox/ in the worktree git root).
#
# Usage:
#   scripts/dev-sandbox.sh python -m moor_cli.main
#   scripts/dev-sandbox.sh moor desktop
#   scripts/dev-sandbox.sh electron .
#   scripts/dev-sandbox.sh -- npm run dev   # from apps/desktop/
#   scripts/dev-sandbox.sh --persistent moor desktop
#   scripts/dev-sandbox.sh --persistent -- npm run dev
#
# Seed the sandbox MOOR_HOME from an existing directory (e.g. your main
# ~/.moor) so config, sessions, skills, etc. are pre-populated:
#   scripts/dev-sandbox.sh --from ~/.moor moor desktop
#
# Override the app name (default: MoorSandbox):
#   MOOR_DEV_SANDBOX_NAME=Staging scripts/dev-sandbox.sh moor desktop
#
# Override the persistent sandbox dir name (default: .moor-sandbox):
#   MOOR_DEV_SANDBOX_DIR=.staging-sandbox scripts/dev-sandbox.sh --persistent moor desktop
...
export MOOR_DESKTOP_USER_DATA_DIR="$SANDBOX_ROOT/user-data"
export MOOR_DESKTOP_APP_NAME="$SANDBOX_NAME"

mkdir -p "$MOOR_HOME" "$MOOR_DESKTOP_USER_DATA_DIR"

if [ -n "$SEED_DIR" ]; then
  # Only seed when the sandbox MOOR_HOME is empty — avoids clobbering an
  # existing persistent sandbox on re-run.
  if [ -z "$(ls -A "$MOOR_HOME" 2>/dev/null)" ]; then
    echo "[sandbox] seeding MOOR_HOME from $SEED_DIR" >&2
    cp -a "$SEED_DIR/." "$MOOR_HOME/"
  else
    echo "[sandbox] --from ignored: $MOOR_HOME already has content" >&2
  fi
fi

echo "[sandbox] MOOR_HOME=$MOOR_HOME" >&2
echo "[sandbox] userData=$MOOR_DESKTOP_USER_DATA_DIR" >&2
echo "[sandbox] appName=$MOOR_DESKTOP_APP_NAME" >&2
if [ "$PERSISTENT" = true ]; then
