...
def setup_isolated_home(enabled: bool, listing: str = "off",
    ...
    """Create a fresh ~/.moor/ for one test, copying minimal credentials.

    Also reads OPENROUTER_API_KEY from the user's real ``~/.moor/.env`` so
    the agent can authenticate against OpenRouter inside the isolated home.
    """
    home_dir = Path(tempfile.mkdtemp(prefix="moor_ts_live_"))
    moor_home = home_dir / ".moor"
    moor_home.mkdir(parents=True)

    if ORIGINAL_AUTH.exists():
        shutil.copy(ORIGINAL_AUTH, moor_home / "auth.json")

    # Copy .env so OPENROUTER_API_KEY (or others) are visible to the agent
    # running inside the isolated home.
    real_env_file = Path.home() / ".moor" / ".env"
    if real_env_file.exists():
        shutil.copy(real_env_file, moor_home / ".env")
        ...
        load_moor_dotenv(moor_home=str(Path.home() / ".moor"))
    ...
    (moor_home / "config.yaml").write_text(_yaml_dump(cfg), encoding="utf-8")
    return moor_home
...
def reset_module_state():
    """Drop cached modules so the new MOOR_HOME takes effect."""
    keys = [k for k in sys.modules.keys()
            if k.startswith(("tools.", "model_tools", "toolsets",
                             "moor_cli", "agent.", "run_agent"))]
    for k in keys:
        del sys.modules[k]
...
def main():
    ...
    # Restore original MOOR_HOME
    if ORIGINAL_HOME is not None:
        os.environ["MOOR_HOME"] = ORIGINAL_HOME
    else:
