#!/usr/bin/env bash
# moor-update-rehearsal.sh — for an EXISTING Moor install.
#
# Two steps:
#   pre   take a `moor backup` of your data, clone MOOR_HOME and the desktop
#         app's Electron userData into the backup dir, then point the install's
#         update source at a fork so `moor update` pulls that fork's main.
...
# when the backup dir is on the same disk. MOOR_HOME/cache is left as it is.
#
#   ./moor-update-rehearsal.sh pre  --source <git-url>
#   # ... run `moor update`, use Moor, test whatever you need ...
#   ./moor-update-rehearsal.sh post
#
# Options:
#   --source URL   repo to pull the update from; updates follow its main
#                  (default: the rehearsal fork)
#   --backup-root DIR   where the backup lives (default ~/moor-update-rehearsal)
#   --yes          post: skip the confirmation
#
# Requires: git. `pre` and `moor update` need network access to --source.

set -euo pipefail

OFFICIAL_HTTPS="https://github.com/thisismamad-n/Moor.git"
OFFICIAL_SSH="git@github.com:NousResearch/hermes-agent.git"
DEFAULT_SOURCE="https://github.com/ethernet8023/moor-agent.git"

SUBCMD=""
BACKUP_ROOT="${HOME}/moor-update-rehearsal"
...
usage() {
  ...
  cat <<'EOF'
moor-update-rehearsal.sh -- run against an EXISTING Moor install.
...
resolve_paths() {
  local suffix="${MOOR_DATA_DIR_SUFFIX:-}"
  if [ -n "${MOOR_HOME:-}" ]; then
    MOOR_HOME="$(cd "$MOOR_HOME" 2>/dev/null && pwd || printf '%s' "$MOOR_HOME")"
  else
    MOOR_HOME="$HOME/.moor${suffix}"
  fi
  INSTALL_DIR="$MOOR_HOME/moor-agent"
  if [ -n "${MOOR_DESKTOP_USER_DATA_DIR:-}" ]; then
    USERDATA_DIR="$MOOR_DESKTOP_USER_DATA_DIR"; USERDATA_SOURCE="env"
  else
    USERDATA_DIR="$HOME/Library/Application Support/Moor${suffix}"; USERDATA_SOURCE="default"
  fi
  BACKUP_ROOT="$(native_path "$BACKUP_ROOT")"
  MOOR_HOME="$(native_path "$MOOR_HOME")"
...
load_snapshot() {
  ...
  local recorded_path="$SNAP/moor-home.txt"
  [ -f "$recorded_path" ] || die "$SNAP is not a rehearsal backup (no moor-home.txt)"
  [ -d "$SNAP/home" ] || die "$SNAP has no clone of MOOR_HOME (made by an older version of this script, or already restored)"
  ...
  current="$(printf '%s' "$MOOR_HOME" | tr '\\' '/')"
  [ "$(printf '%s' "$recorded" | tr '\\' '/')" = "$current" ] \
    || die "that backup belongs to MOOR_HOME=$recorded, not $MOOR_HOME; pass --backup-root to pick the right one"
}
...
# The install's own moor: pre-branch installs keep it in the venv.
resolve_moor_exe() {
  local c
  for c in "$INSTALL_DIR/venv/bin/moor" "$INSTALL_DIR/.moor/bin/moor" \
           "$INSTALL_DIR/venv/Scripts/moor.exe" "$MOOR_HOME/bin/moor"; do
...
cmd_pre() {
  resolve_paths
  step "your install"
  say "MOOR_HOME   $MOOR_HOME"
  say "install       $INSTALL_DIR"
  say "desktop data  $USERDATA_DIR ($USERDATA_SOURCE)"
  say "backup to     $BACKUP_ROOT"
  [ -d "$MOOR_HOME" ] || die "no MOOR_HOME at $MOOR_HOME"
  [ -d "$INSTALL_DIR/.git" ] || die "no git checkout at $INSTALL_DIR — this tool covers source installs"
  local moor_exe
  moor_exe="$(resolve_moor_exe)" || die "no moor executable found in $INSTALL_DIR (venv/bin, .moor/bin) or $MOOR_HOME/bin"

  step "before we start (nothing here is a pass/fail, just read it)"
  if command -v pgrep >/dev/null 2>&1; then
    local procs
    procs="$(pgrep -fl hermes 2>/dev/null | grep -v 'moor-update-rehearsal' || true)"
    if [ -n "$procs" ]; then
      warn "Moor looks like it is running — close the desktop app and the gateway"
      warn "before you run 'moor update', or the dependency sync may fail:"
      printf '    %s\n' "$procs"
    else
      ok "no Moor processes running"
    fi
  fi
  local n
  ...
  step "backing up your data (moor backup)"
  ...
  started="$SECONDS"
  out="$(MOOR_HOME="$MOOR_HOME" "$moor_exe" backup -o "$SNAP/moor-backup.zip" 2>&1)" || code=$?
  if [ "$code" = 1 ] && [ -f "$SNAP/moor-backup.zip" ]; then
    printf '    %s\n' "$out"
    warn "moor backup finished INCOMPLETE ($((SECONDS - started))s): the files listed above are not in $SNAP/moor-backup.zip"
  elif [ "$code" != 0 ] || [ ! -f "$SNAP/moor-backup.zip" ]; then
    printf '    %s\n' "$out"
    die "moor backup failed (exit $code) — nothing else was done"
  else
    ok "moor-backup.zip ($(du -h "$SNAP/moor-backup.zip" | cut -f1), $((SECONDS - started))s)"
  fi

  step "cloning MOOR_HOME and the desktop app's data"
  detect_clone_mode
  case "$CLONE_MODE" in
  ...
  clone_tree "$MOOR_HOME" "$SNAP/home" cache || die "cloning MOOR_HOME failed"
  ok "MOOR_HOME -> $SNAP/home ($((SECONDS - started))s, cache/ left out)"
...
EOF
  ...
  ok "created $MOOR_HOME/.skip_upstream_prompt (stops the 'add upstream remote?' prompt)"
...
cmd_status() {
  ...
  say "data backup   $([ -f "$SNAP/moor-backup.zip" ] && echo moor-backup.zip || echo none)"
  say "marker        $([ -f "$MOOR_HOME/.skip_upstream_prompt" ] && echo present || echo absent)"
  local n=0
...
cmd_post() {
  ...
  say "  $MOOR_HOME  (everything except cache/, including the checkout)"
  say "  $USERDATA_DIR"
  ...
  local moor_exe
  moor_exe="$(resolve_moor_exe)" || die "no Moor launcher for $INSTALL_DIR; stop this home's gateway before restoring"
  MOOR_HOME="$MOOR_HOME" "$moor_exe" gateway stop \
  ...
  step "restoring your MOOR_HOME"
  swap_tree "$MOOR_HOME" "$SNAP/home" "$aside/home" cache \
  ...
  step "cleaning up"
  # Only the whole trees the update left behind. moor-backup.zip stays.
  rm -rf "$aside"
  ok "deleted the post-update trees ($aside)"

  step "done"
  say "Your MOOR_HOME and the desktop app's data are back exactly as they were."
  say "Open the desktop app once and run 'moor doctor' to confirm."
  say "Nothing was judged or changed by this script; the backup at $SNAP"
  say "(including moor-backup.zip) is yours to keep or delete."
}
