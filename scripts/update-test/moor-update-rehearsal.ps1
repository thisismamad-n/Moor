<#
.SYNOPSIS
  moor-update-rehearsal.ps1 -- for an EXISTING Moor install (Windows).

.DESCRIPTION
  Two steps:

    pre   take a `moor backup` of your data, snapshot the disk that holds
          MOOR_HOME and the desktop app's Electron userData (Volume Shadow Copy),
          then point the install's update source at a fork so `moor update`
  ...
  copied, and files it added are deleted. MOOR_HOME\cache is left as it is.

  If Windows drops the snapshot (it can when the disk's shadow storage fills),
  your files are untouched but post cannot roll back; the `moor backup` zip
...
.PARAMETER BackupRoot
  Where the backup lives (default: $HOME\moor-update-rehearsal).

.PARAMETER Yes
  post: skip the confirmation.

.EXAMPLE
  ./moor-update-rehearsal.ps1 pre -Source <git-url>
  # ... run `moor update`, use Moor, test ...
  ./moor-update-rehearsal.ps1 post

.NOTES
  `pre` and `moor update` need network access to -Source, and a usable git on PATH.
#>
[CmdletBinding()]
param(
  ...
  [string]$Source = 'https://github.com/ethernet8023/moor-agent.git',
  [string]$BackupRoot,
  [switch]$Yes
)
...
$OfficialHttps = 'https://github.com/thisismamad-n/Moor.git'
...
function Get-ResolvedPaths {
  $suffix = if ($env:MOOR_DATA_DIR_SUFFIX) { $env:MOOR_DATA_DIR_SUFFIX } else { '' }
  ...
  if ($env:MOOR_HOME) {
    ...
    $home_ = "$(Join-Path $base 'moor')$suffix"
  }
  $install = Join-Path $home_ 'moor-agent'
  if ($env:MOOR_DESKTOP_USER_DATA_DIR) {
    $ud = $env:MOOR_DESKTOP_USER_DATA_DIR
    ...
    $userData = Join-Path $appData "Moor$suffix"
...
function Load-Snapshot {
  ...
  $script:Snap = $snap
  $recordedPath = Join-Path $snap 'moor-home.txt'
  if (-not (Test-Path -LiteralPath $recordedPath)) { Fail "$snap is not a rehearsal backup (no moor-home.txt)" }
  ...
  if ($recorded -ne $P.Home) {
    Fail "that backup belongs to MOOR_HOME=$recorded, not $($P.Home); pass -BackupRoot to pick the right one"
  }
}
...
function Resolve-MoorExe {
  param($P)
  foreach ($c in @(
      (Join-Path $P.Install 'venv\Scripts\moor.exe'),
      (Join-Path $P.Home 'bin\moor.exe'),
      (Join-Path $P.Home 'bin\moor.cmd'))) {
...
function Invoke-Pre {
  ...
  $procs = @(Get-Process -Name 'Moor', 'moor' -ErrorAction SilentlyContinue)
  if ($procs.Count) {
  ...
  else { Ok 'no Moor processes running' }
  ...
  $zip = Join-Path $script:Snap 'moor-backup.zip'
  $started = Get-Date
  $prevHome = $env:MOOR_HOME
  $env:MOOR_HOME = $P.Home
  $prev = $ErrorActionPreference
  $ErrorActionPreference = 'Continue'
  try { $out = & $moorExe backup -o $zip 2>&1; $code = $LASTEXITCODE }
  finally { $ErrorActionPreference = $prev; $env:MOOR_HOME = $prevHome }
  $elapsed = [int]((Get-Date) - $started).TotalSeconds
  if ($code -eq 1 -and (Test-Path -LiteralPath $zip)) {
    $out | ForEach-Object { Write-Host "    $_" }
    Warn "moor backup finished INCOMPLETE (${elapsed}s): the files listed above are not in $zip"
  }
  elseif ($code -ne 0 -or -not (Test-Path -LiteralPath $zip)) {
    $out | ForEach-Object { Write-Host "    $_" }
    Fail "moor backup failed (exit $code) -- nothing else was done"
  }
  else { Ok "moor-backup.zip ($([math]::Round((Get-Item $zip).Length / 1MB, 1)) MB, ${elapsed}s)" }
  ...
  Set-Content -LiteralPath (Join-Path $script:Snap 'moor-home.txt') -Encoding utf8 -Value @($P.Home)
  $manifest = [ordered]@{
    schema              = 4
  ...
  # URL, which matters: `moor update` resolves its channel from the archive and
...
function Invoke-Status {
  ...
  Say "data backup   $(if (Test-Path -LiteralPath (Join-Path $snap 'moor-backup.zip')) { 'moor-backup.zip' } else { 'none' })"
...
function Invoke-Post {
  ...
  if ($gone.Count) {
    ...
    Say "  moor import `"$zip`""
    Say 'or, if moor itself no longer starts, extract the zip over your home:'
    Say "  tar -xf `"$zip`" -C `"$($P.Home)`""
    Say 'then reinstall Moor to get the program back.'
  ...
  Step 'stopping Moor'
  foreach ($name in @('Moor', 'moor')) {
    Get-Process -Name $name -ErrorAction SilentlyContinue | Stop-Process -Force -ErrorAction SilentlyContinue
  }
  Ok 'asked Moor to stop (if anything was running)'

  $mounts = @{}
  try {
    foreach ($s in $shadows) { $mounts[$s.Volume] = Mount-Shadow $s }
    $trees = @([pscustomobject]@{ Label = 'MOOR_HOME'; Live = $P.Home; IsHome = $true })
...
if (-not $BackupRoot) { $BackupRoot = Join-Path $env:USERPROFILE 'moor-update-rehearsal' }
...
switch ($Command) {
  ...
  default {
    ...
    if ($PSCommandPath) {
      ...
      Write-Host 'moor-update-rehearsal.ps1 -- run against an EXISTING Moor install.'
      Write-Host ''
