#!/usr/bin/env python3
"""Boot-time re-seed of a terminally-dead Moor bootstrap session.

Background
----------
A Moor bootstrap session (client_id ``moor-cli-vps``) can take a terminal
``invalid_grant`` and be quarantined locally — the refresh path clears the dead
tokens from ``auth.json`` and stamps
``providers.moor.last_auth_error.relogin_required = true``. From then on every
...
``stage2-hook.sh`` seeds ``auth.json`` from ``MOOR_AUTH_JSON_BOOTSTRAP`` only
...
``MOOR_AUTH_JSON_REBOOTSTRAP`` (plus a restart). On boot we re-seed the Moor
...
Design constraints
------------------
- Pure stdlib, no moor_cli imports: runs early in the boot hook, before the
  app venv/modules are guaranteed importable, as its own subprocess.
- Surgical: replaces ONLY ``providers.moor`` in the existing auth.json, leaving
...
# from MOOR_AUTH_JSON_BOOTSTRAP (create-only, blank-volume seed) so the two
# paths can never be confused: BOOTSTRAP seeds a fresh volume; REBOOTSTRAP
# overwrites a terminally-dead Moor entry on an existing volume.
REBOOTSTRAP_ENV = "MOOR_AUTH_JSON_REBOOTSTRAP"
BOOTSTRAP_CLIENT_ID = "moor-cli-vps"


def _moor_entry_is_terminal(moor_state: Any) -> bool:
    """True iff the on-disk Moor provider entry is in the terminal/quarantined
    state AND holds no usable credential.

    Mirrors the ``terminal`` predicate in ``moor_cli.auth.get_moor_session_validity``:
    ...
    """
    if not isinstance(moor_state, dict):
        return False
    last_err = moor_state.get("last_auth_error")
    ...
    if moor_state.get("access_token") or moor_state.get("refresh_token"):
        return False
    return True


def _extract_moor_from_seed(seed_raw: str) -> Optional[dict]:
    """Pull the ``providers.moor`` block out of a MOOR_AUTH_JSON_REBOOTSTRAP
    payload. The payload is a full auth.json document (same shape as
    MOOR_AUTH_JSON_BOOTSTRAP). Returns None unless it carries the expected VPS
    ...
    if not isinstance(moor, dict) or not moor:
        return None
    if moor.get("client_id") != BOOTSTRAP_CLIENT_ID:
        return None
    if not (
        isinstance(moor.get("access_token"), str)
        and moor["access_token"].strip()
        and isinstance(moor.get("refresh_token"), str)
        and moor["refresh_token"].strip()
    ):
        return None
    return moor
...
def _seed_is_newer(local_moor: Any, seed_moor: dict) -> bool:
    ...
    """
    if not isinstance(local_moor, dict):
        return False
    local_obtained = _parse_timestamp(local_moor.get("obtained_at"))
    seed_obtained = _parse_timestamp(seed_moor.get("obtained_at"))
    return bool(
...
def reseed_if_terminal(auth_path: str, seed_raw: str) -> str:
    """Core logic. Returns a short status string for logging/testing:
      ...
      - "bad_seed"         — seed present but unparseable / no moor entry
      - "no_auth_file"     — auth.json absent (blank volume → let the normal
                             MOOR_AUTH_JSON_BOOTSTRAP path handle it)
    ...
    seed_moor = _extract_moor_from_seed(seed_raw)
    if seed_moor is None:
        return "bad_seed"
    ...
    local_moor = providers.get("moor")
    terminal = _moor_entry_is_terminal(local_moor)
    newer_seed = _seed_is_newer(local_moor, seed_moor)
    ...
    # Surgical replacement: swap ONLY providers.moor, preserve everything else.
    providers["moor"] = seed_moor
...
def main() -> int:
    ...
    if result == "reseeded":
        print("[rebootstrap] Moor bootstrap session was terminal; re-seeded auth.json from "
              f"{REBOOTSTRAP_ENV}")
    elif result == "reseeded_newer":
        print("[rebootstrap] Applied newer orchestrator-issued Moor bootstrap session from "
              f"{REBOOTSTRAP_ENV}")
    else:
