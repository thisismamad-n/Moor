...
When Moor runs in TUI mode, the gateway child process communicates with
...
# ``get_moor_home() / "plugins"`` (user) and ``./.moor/plugins/`` (project,
# gated behind ``MOOR_ENABLE_PROJECT_PLUGINS``) — see
# ``moor_cli/plugins.py:10-12``.  The guard only checked the bundled
...
# this script runs as a standalone subprocess — ``moor_constants`` isn't
...
def main() -> int:
    ...
    # Add repo root to sys.path so we can import moor_constants (this script
    ...
    # plugins in get_moor_home()/plugins/ and project plugins in
    # ./.moor/plugins/, where code like ori/hooks.py can spawn
    # subprocesses with inherited stdin — #67639).
    plugin_roots: list[Path] = [get_moor_home() / "plugins"]
    if os.environ.get("MOOR_ENABLE_PROJECT_PLUGINS"):
        plugin_roots.append(Path.cwd() / ".moor" / "plugins")
