...
MOOR_* placeholders (see the constants below), and hands the result to
...
(plan: pm-clean):
  ...
  * put the payload repo snapshot FIRST and the venv site-packages second
    ...
    moor_cli wins over anything stale in site-packages — the sealed
    ...
    Win32Locker → concurrent-log-handler → moor_logging.py's files),
  ...
  * default sys.pycache_prefix to the user-level cache (%LOCALAPPDATA%
    \\moor\\pycache on win32, ~/.cache/moor-pycache elsewhere) when the
...
MOOR_ENTRY_MODULE = "__MOOR_ENTRY_MODULE__"
MOOR_ENTRY_FUNC = "__MOOR_ENTRY_FUNC__"
MOOR_REPO_REL = "__MOOR_REPO_REL__"
MOOR_SITE_REL = "__MOOR_SITE_REL__"
...
def payload_sys_paths(here):
    ...
    return [_join_rel(here, MOOR_REPO_REL), _join_rel(here, MOOR_SITE_REL)]
...
def default_pycache_dir(environ):
    ...
    if sys.platform == "win32":
        ...
        return os.path.join(base, "moor", "pycache") if base else None
    base = environ.get("HOME")
    return os.path.join(base, ".cache", "moor-pycache") if base else None
...
def configure(here, environ=None):
    ...
    # Repo snapshot first — its moor_cli wins over anything stale in
    ...
    if not environ.get("PYTHONPYCACHEPREFIX"):
        ...
        if not os.path.exists(os.path.join(os.path.dirname(repo_entry), ".moor-baked-pycache")):
...
def main(argv=None):
    ...
    configure(here)
    importlib.import_module("moor_bootstrap")
    module = importlib.import_module(MOOR_ENTRY_MODULE)
    target = getattr(module, MOOR_ENTRY_FUNC)
