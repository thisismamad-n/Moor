...
Each compiler publishes `moor-build.json` inside its output (inside `dist/`
...
Existing arbitrary output directories require the builder's `.moor-product`
marker. Files, symlinks, and source directories are rejected. The exact npm
destinations (`ui-tui/dist`, `moor_cli/web_dist`, `apps/desktop/dist`, and
...
Run the generator with a Moor runtime Python (Pillow and resvg-py are core
dependencies):
...
The convenience wrapper runs the same generator on `MOOR_PYTHON` (else
...
| Web build | `moor_cli/web_dist/` | npm `prebuild` prepares icons |
...
cache. `scripts.bundles.stage --cache PATH` (or `moor pm bundle --cache PATH`)
...
| `repo` | Yes | Code/resource location beneath `OUT`, such as `moor-agent`, `.`, or `share/moor-agent` |
...
a Moor wheel. It also writes `site_packages/moor-agent.pth` with a relative
...
`OUT/repo/moor_cli/tui_dist/` and web files at
`OUT/repo/moor_cli/web_dist/`. Reference placement links TUI at `OUT/ui-tui`
