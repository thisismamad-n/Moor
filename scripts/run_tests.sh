#!/usr/bin/env bash
# Canonical test runner for moor-agent. Run this instead of calling
...
#   * Proper venv activation (probes .venv, venv, then ~/.moor/...)
#
# Usage:
...
# (MOOR_PYTHON is exported by the devShell hook and ships [dev] extras:
# pytest, pytest-asyncio, pytest-timeout, ruff, ty).
#
# A candidate must have pytest INSTALLED, not merely exist. The release venv
# at ~/.moor/moor-agent/venv has bin/activate but no pytest, so an
...
VENV_PYTHON=""
SKIPPED_VENVS=""
for candidate in "$REPO_ROOT/.venv" "$REPO_ROOT/venv" "$HOME/.moor/moor-agent/venv"; do
...
if [ -n "$VENV" ]; then
  PYTHON="$VENV_PYTHON"
elif [ -n "${MOOR_PYTHON:-}" ] && [ -x "$MOOR_PYTHON" ] \
    && "$MOOR_PYTHON" -c 'import pytest' 2>/dev/null; then
  # Guard with an import check: MOOR_PYTHON may point at the RELEASE
  # venv (no pytest) when inherited from a wrapped `moor` binary rather
  # than the devShell hook.
  PYTHON="$MOOR_PYTHON"
  echo "▶ no local venv — using Nix dev venv via MOOR_PYTHON: $PYTHON"
else
  echo "error: no virtualenv with pytest found in $REPO_ROOT/.venv or $REPO_ROOT/venv," >&2
  echo "       and MOOR_PYTHON is not a python with pytest (enter the Nix devShell or create a venv)" >&2
  if [ -n "$SKIPPED_VENVS" ]; then
...
if [ -f "$HOME/.moor/pytest_live_guard.py" ]; then
  EXTRA_PYTHONPATH="$HOME/.moor"
...
#
#   * MOOR_TEST_WORKERS / PATHS / FILE_TIMEOUT / FILE_RETRIES / SLICE are
#     read by run_tests_parallel.py at argparse-default time — inside the
#     stripped environment.
#   * MOOR_TEST_IMAGE is read by tests/docker/conftest.py to skip its
...
# MOOR_RUN_SLOW_PET_TESTS / MOOR_E2E_BROWSER opt-ins already forwarded.
...
for _test_var in MOOR_TEST_IMAGE MOOR_TEST_WORKERS MOOR_TEST_PATHS \
  MOOR_TEST_FILE_TIMEOUT MOOR_TEST_FILE_RETRIES MOOR_TEST_SLICE; do
  if [ -n "${!_test_var:-}" ]; then
