...
distributions it is a RAM-backed tmpfs that fills under Moor load. Moor resolves scratch
space through one helper (``moor_constants.get_scratch_dir()`` → ``MOOR_HOME/cache/scratch``,
which every Moor process also exports as ``TMPDIR``/``TMP``/``TEMP``), and prompts + skills
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(
        ...
        f"moor_constants.get_scratch_dir() (or $TMPDIR / tempfile, which Moor points there), tell the "
