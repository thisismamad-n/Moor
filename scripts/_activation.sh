...
# scripts/_moor-python (the shebang prologue) and scripts/run_tests.sh.
#
# pm records, beside the installed-state file named by __MOOR_ACTIVATED, one
...
# moor_activation_current REPO: status 0 when the inherited environment
# matches REPO's current inputs.
moor_activation_current() {
    local repo="$1" sentinel stamps stamp input stamped=0
    [ -n "${__MOOR_ACTIVATED:-}" ] && [ -e "${__MOOR_ACTIVATED}" ] || return 1
    # activate.ps1 records a Windows path; Git Bash accepts it with slashes.
    sentinel="${__MOOR_ACTIVATED//\\//}"
...
# moor_ensure_activated REPO: source REPO/activate unless the inherited
# environment is current. `--` passes no options: `source` without arguments
# would hand activate the CALLER's positional parameters.
moor_ensure_activated() {
    moor_activation_current "$1" && return 0
