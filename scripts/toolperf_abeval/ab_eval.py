...
measured in the production session DB) through `moor chat` twice — once per
...
Environment:
  ...
  ABEVAL_HOME    MOOR_HOME for runs   (default: $ABEVAL_ROOT/home)
    ...
    for rep in range(reps):
        for name in TASKS:
            ...
            env.update({
                ...
                "MOOR_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY": str(atof.parent),
                "MOOR_NEMO_RELAY_ATOF_FILENAME": atof.name,
                "MOOR_NEMO_RELAY_ATOF_MODE": "overwrite",
            })
            q = TASKS[name].replace("{WORK}", str(work))
            t0 = time.time()
            try:
                p = subprocess.run(
                    [sys.executable, "-m", "moor_cli.main", "chat", "--query", q,
