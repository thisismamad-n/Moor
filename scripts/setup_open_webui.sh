...
# Bootstrap Open WebUI against Moor Agent's OpenAI-compatible API server.
#
# Idempotent by design:
# - ensures ~/.moor/.env has API server settings
# - installs Open WebUI into ~/.local/open-webui-venv
# - writes a reusable launcher at ~/.local/bin/start-open-webui-moor.sh
...
#   MOOR_API_PORT=8642
#   MOOR_API_HOST=127.0.0.1
#   MOOR_API_MODEL_NAME='Moor Agent'

OPEN_WEBUI_PORT="${OPEN_WEBUI_PORT:-8080}"
OPEN_WEBUI_HOST="${OPEN_WEBUI_HOST:-127.0.0.1}"
OPEN_WEBUI_NAME="${OPEN_WEBUI_NAME:-Moor Agent WebUI}"
...
MOOR_ENV_FILE="${MOOR_ENV_FILE:-$HOME/.moor/.env}"
MOOR_API_PORT="${MOOR_API_PORT:-8642}"
MOOR_API_HOST="${MOOR_API_HOST:-127.0.0.1}"
MOOR_API_CONNECT_HOST="${MOOR_API_CONNECT_HOST:-127.0.0.1}"
MOOR_API_MODEL_NAME="${MOOR_API_MODEL_NAME:-Moor Agent}"
MOOR_API_BASE_URL="http://${MOOR_API_CONNECT_HOST}:${MOOR_API_PORT}/v1"
LAUNCHER_PATH="$HOME/.local/bin/start-open-webui-moor.sh"
LOG_DIR="$HOME/.moor/logs"
...
write_launcher() {
  ...
  quoted_base_url="$(shell_quote "$MOOR_API_BASE_URL")"
...
ensure_env_permissions() {
  chmod 600 "$MOOR_ENV_FILE" 2>/dev/null || true
}

install_launchd_service() {
  local plist="$HOME/Library/LaunchAgents/ai.openwebui.moor.plist"
...
<dict>
  <key>Label</key>
  <string>ai.openwebui.moor</string>
...
EOF
  ...
  launchctl enable "gui/$(id -u)/ai.openwebui.moor"
  launchctl kickstart -k "gui/$(id -u)/ai.openwebui.moor"
}

install_systemd_user_service() {
  require_cmd systemctl
  local unit_dir="$HOME/.config/systemd/user"
  local unit="$unit_dir/openwebui-moor.service"
  mkdir -p "$unit_dir"
  cat > "$unit" <<EOF
[Unit]
Description=Open WebUI connected to Moor Agent
After=default.target

[Service]
Type=simple
ExecStart=/bin/bash %h/.local/bin/start-open-webui-moor.sh
Restart=always
RestartSec=3
WorkingDirectory=%h
StandardOutput=append:%h/.moor/logs/openwebui.log
StandardError=append:%h/.moor/logs/openwebui.error.log

[Install]
WantedBy=default.target
EOF
  systemctl --user daemon-reload
  systemctl --user enable --now openwebui-moor.service
}
...
main() {
  ...
  local api_key
  api_key="$(get_env_value API_SERVER_KEY "$MOOR_ENV_FILE")"
  if [[ -z "$api_key" ]]; then
    api_key="$(generate_secret)"
  fi

  log 'Ensuring Moor API server is configured...'
  upsert_env API_SERVER_ENABLED true "$MOOR_ENV_FILE"
  upsert_env API_SERVER_HOST "$MOOR_API_HOST" "$MOOR_ENV_FILE"
  upsert_env API_SERVER_PORT "$MOOR_API_PORT" "$MOOR_ENV_FILE"
  upsert_env API_SERVER_MODEL_NAME "$MOOR_API_MODEL_NAME" "$MOOR_ENV_FILE"
  upsert_env API_SERVER_KEY "$api_key" "$MOOR_ENV_FILE"
  ensure_env_permissions

  log 'Restarting Moor gateway so API server settings take effect...'
  moor gateway restart >/dev/null 2>&1 || true
  sleep 4
  if ! curl -fsS "http://${MOOR_API_CONNECT_HOST}:${MOOR_API_PORT}/health" >/dev/null; then
    log 'Moor API server did not answer on the first check. Trying to start gateway in the background...'
    nohup moor gateway run >/dev/null 2>&1 &
    sleep 6
  fi
  curl -fsS "http://${MOOR_API_CONNECT_HOST}:${MOOR_API_PORT}/health" >/dev/null
  ...
  log "Moor API endpoint: ${MOOR_API_BASE_URL}"
