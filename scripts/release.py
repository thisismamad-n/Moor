...
VERSION_FILE = REPO_ROOT / "moor_cli" / "__init__.py"
...
LEGACY_AUTHOR_MAP = {
    ...
    "pi@moor.local": "Elektrofussel",  # PR #61675 salvage (honcho: defaultHost + private-range local URL detection)
    ...
    "75556242+webtecnica@users.noreply.github.com": "webtecnica",  # PR #63360 salvage (moor: restore inference-api base_url)
    ...
    "kavi@local.moor": "kavioavio",  # Issue #46544 / PR #47705 evolution (smart DENY exact-operation owner override)
    ...
    "sonxi@moor.local": "17324393074",  # PR #53196 salvage (tools_config: known_plugin_toolsets null guard; commit under unlinked local identity)
    ...
    "sunsky.lau@gmail.com": "liuhao1024",  # PR #56993 salvage (gateway: process-level MOOR_HOME for pid/lock/status identity files; #56986)
    ...
    "wyuebei@gmail.com": "wyuebei-cloud",  # PR #56640 salvage (moor journey: replace GNU-only %-d strftime with dt.day for Windows)
    ...
    "moor.wanderer@yahoo.com": "trismegistus-wanderer",  # PR #31856 salvage (gateway: defer idle-TTL agent-cache eviction until the session store says the session actually expired, so the expiry watcher can still fire MemoryProvider.on_session_end with the live transcript; #11205)
    ...
    "me@keslerm.com": "keslerm",  # PR #3459 salvage (gateway: 'log' tool_progress mode — silent in chat, tool calls appended to ~/.moor/logs/tool_calls.log via rotating handler; duplicate of #3458 by @dlkakbs who submitted 4 min earlier — both credited)
    ...
    "hodlclone@gmail.com": "HODLCLONE",  # PR #49351 salvage (Moor Portal token resilience: rotate refresh tokens write-through to the source auth store in profile mode, skip Moor fallback when no local token, sync gateway session model after fallback)
    ...
    "121278003+Cossackx@users.noreply.github.com": "Cossackx",  # PR #52528 salvage (Windows moor-shim resolution + prefer --update on recovery; #52378)
    ...
    "annguyenMoor@users.noreply.github.com": "annguyenMoor",
    ...
    "annguyenMoor@users.noreply.github.com": "annguyenMoor",
    "285874597+annguyenMoor@users.noreply.github.com": "annguyenMoor",
    "perkintahmaz50@gmail.com": "devatnull",
    ...
    "274096618+moor-agent-dhabibi@users.noreply.github.com": "dhabibi",
    ...
    "mind-dragon@moor.research": "Mind-Dragon",
    ...
    "moor@nousresearch.com": "Moor inc.",
    ...
    "sberan@gmail.com": "sberan",  # PR #54494 salvage (--connect-timeout flag on moor mcp add)
    ...
    "moor@example.com": "shellybotmoyer",  # PR #18915 (bot-committed)
    ...
    "github@nadyamoor.anonaddy.com": "ruangraung",  # PR #42308
    ...
    "sylw3st3rr@gmail.com": "Sylw3ster",  # salvage of #23252 (MOOR_KANBAN_BOARD restore)
    ...
    "58116817+TurgutKural@users.noreply.github.com": "TurgutKural",  # salvage of #23356 (MOOR_HOME inject)
    ...
    "sauravsejal40@gmail.com": "Saurav0989",  # PR #27071 (docs: moor-eval community link)
    ...
    "vigo@moor": "hawknewton",  # PR #26294 (bedrock boto3 lazy_deps)
    ...
    "ethie@moor": "ethernet8023",  # PR #29342 (TUI clipboard copy on linux/wayland)
    ...
    "2663402852@qq.com": "x1am1",  # PR #35098 (chown root-owned top-level MOOR_HOME state files)
    ...
    "annguyen@nousresearch.com": "annguyenMoor",  # PR #52844 co-author
    ...
    "zhchl@moor-agent.local": "8294",  # PR #50572 salvage (honor config context_length on banner)
...
def parse_coauthors(body: str) -> list:
    ...
    _ignored_emails = {"noreply@anthropic.com", "noreply@github.com",
                       "cursoragent@cursor.com", "moor@nousresearch.com"}
