...
# changing PATH, installing software, or touching the user's Moor home.

$repoRoot = Split-Path -Parent (Split-Path -Parent (Split-Path -Parent $MyInvocation.MyCommand.Path))
$installScript = Join-Path $repoRoot 'scripts\install.ps1'
$testRoot = Join-Path $env:TEMP ("moor-node-compatibility-test-" + [Guid]::NewGuid().ToString('N'))
$MoorHome = Join-Path $testRoot 'home'
$InstallDir = Join-Path $testRoot 'missing-checkout'
. $installScript -MoorHome $MoorHome -InstallDir $InstallDir
