...
function Invoke-Normalization {
    ...
    try {
        ...
        # its diagnostics are all `[moor] `-prefixed, so the two separate
        ...
        $stderr = ($raw | Where-Object { $_ -like '`[moor`]*' }) -join "`n"
        $stdout = ($raw | Where-Object { $_ -notlike '`[moor`]*' }) -join "`n"
    } finally {
    ...
    return @{
        ...
        MoorHome = $(if ($paths) { $paths.moor_home } else { $null })
...
$expectedInstallDir = "$($longRoot)${sep}AppData${sep}Local" + '\moor\moor-agent'
...
$result = Invoke-Normalization -Environment @{ TEMP = $shortTemp; TMP = $shortTemp } `
    -ExtraArgs @('-InstallDir', (Join-Path $shortProfile 'custom-moor'))
Assert-Equal -Expected (Join-Path $longRoot 'custom-moor') -Actual $result.InstallDir -Label "explicit -InstallDir keeps the caller's directory, on the long root"
