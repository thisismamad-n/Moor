...
find_browser() {
  ...
  # email included — inside this window that is titled "Moor" (#88410).
...
stop_ui() { # error/manual outcomes keep the window up briefly so a watching
  ...
  if [ "${1:-}" = "leave-window" ]; then
    sleep "${MOOR_UPDATE_SHIM_GRACE_SECONDS:-15}"
  fi
...
linux_gate() {
  ...
  # _desktop_linux_userns_sandbox_available() probe in moor_cli/main.py).
...
# (`venv/bin/moor` has a `python3` shebang), so no Python-side heal —
...
# (moor_cli/macos_tcc_anchor.ensure_tcc_anchor, which re-anchors whenever
...
# touched; and when it does restore symlinks, the very `moor update` run it
...
tcc_pick_update_invoke() { # sets UPDATE_INVOKE; safety net past a failed heal
  ...
  # launchd gateway proves `venv/bin/python -m moor_cli.main` works when
  # every alias entrypoint is bricked — drive the update the same way.
  local bin="$1"
  UPDATE_INVOKE=("$bin/moor")
  if ! tcc_probe_python "$bin/python3" && tcc_probe_python "$bin/python"; then
    UPDATE_INVOKE=("$bin/python" -m moor_cli.main)
  fi
}
...
NOW="$(date +%s)"
STARTED_AT="${MOOR_UPDATE_STARTED_AT:-$NOW}"
...
# venv/bin/moor execs venv/bin/python3, so a dead alias kills every attempt
...
tcc_pick_update_invoke "$INSTALL_ROOT/venv/bin"
if [ "${UPDATE_INVOKE[0]}" != "$MOOR_BIN" ]; then
...
# "close all Moor windows" sentinel.
KEEP_STASH=""
if "${UPDATE_INVOKE[@]}" update --help 2>/dev/null | grep -q -- '--keep-stash'; then
  KEEP_STASH="--keep-stash"
else
  log "installed moor predates --keep-stash; running without it"
fi
...
if [ "$CODE" -ne 0 ] && [ "$CODE" -ne 2 ]; then
  ...
  if printf '%s' "$OUT" | grep -q "CODE UPDATE SKIPPED"; then
    log "moor update skipped (checkout parked on a non-target branch); not retrying"
    FINAL_CODE=8
...
if [ "$CODE" -eq 0 ]; then FINAL_CODE=0 FINAL_MSG="Update complete."
  ...
  FINAL_CODE="$CODE" FINAL_MSG="Update failed (exit $CODE). Run moor debug share in a terminal to send a report."
  ...
  if ! tcc_probe_python "$INSTALL_ROOT/venv/bin/python3" \
    ...
    FINAL_MSG="Update failed: the Python interpreter inside $INSTALL_ROOT/venv cannot start (heal state: $TCC_HEAL_STATE). Reinstall the runtime with the Moor installer, or run moor doctor --fix from a terminal if any moor command still works."
  fi
fi
exit "$FINAL_CODE"
