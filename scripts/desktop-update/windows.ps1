...
# (%MOOR_HOME%\moor-setup.exe). That binary has no self-update path --
# copy_self_to_moor_home deliberately no-ops during --update -- so every
...
#
# This script lives in the repo checkout, so EVERY `moor update` refreshes
...
#     [-NoMarkerCleanup]    leave .moor-update-in-progress in place (tests)
#
...
# .moor-update-result.json for the relaunched Desktop to surface, and
# relaunches the Desktop so the user is never left stranded.
#
# Marker: we claim MOOR_HOME\.moor-update-in-progress with OUR pid as
# step 0 (the wrapper cmd.exe pid the Desktop saw is useless -- it exits
# immediately), retaining MOOR_UPDATE_STARTED_AT from the Desktop hand-off.
# moor_cli/update_lock.py's ancestry rule lets our
# `moor update` child adopt the claim; electron/update-marker.ts parks a
...
try {
    Add-Type -Namespace MoorHandoff -Name Win32 -MemberDefinition @'
...
$MoorHome = if ($InstallRoot) { Split-Path -Parent $InstallRoot } else { $TempDir }
...
$script:Ui = $null
$script:UiStage = "Moor will open once done."   # until the first gate; matches ui.html
...
function Stop-UiServer([switch]$LeaveWindow) {
    ...
    # profile plus any stale moor-update-ui-* leftovers from interrupted
    ...
    # update marker (.moor-update-in-progress) serialises hand-offs, so no
    # other run's profile can be in active use here.
    try {
        $profileDirs = @()
        if ($script:UiServer.Profile) { $profileDirs += $script:UiServer.Profile }
        Get-ChildItem -LiteralPath $TempDir -Directory -Filter "moor-update-ui-*" -ErrorAction SilentlyContinue |
...
function Publish-UiProgress([string]$Message) {
    ...
    # stderr remain asynchronously drained in Invoke-MoorStep and are never
...
function Show-ProgressWindow {
    ...
    if ($htmlPath -and $browser) {
        ...
        if ($server) {
            try {
                ...
                $browserProfile = Join-Path $TempDir ("moor-update-ui-{0}" -f $PID)
                $browserArgs = @(
    ...
    try {
        ...
        try {
            $form.Activate()
            if ($script:Win32) { [MoorHandoff.Win32]::SetForegroundWindow($form.Handle) | Out-Null }
        } catch {}
        [System.Windows.Forms.Application]::DoEvents()
...
function Show-ErrorFinale([string]$Message) {
    ...
    # log tail -- `moor debug share` uploads the real evidence and the
    ...
    try {
        ...
        $ui.Sub.Text = "Run `"moor debug share`" in a terminal to send a report."
        ...
        try {
            $ui.Form.Activate()
            if ($script:Win32) { [MoorHandoff.Win32]::SetForegroundWindow($ui.Form.Handle) | Out-Null }
        } catch {}
...
function Start-DesktopRelaunch {
    ...
    try {
        ...
        if ($r -and $r.ReturnValue -eq 0) {
            ...
            try {
                if ($script:Win32) {
                    [MoorHandoff.Win32]::AllowSetForegroundWindow([int]$r.ProcessId) | Out-Null
                    ...
                    while ((Get-Date) -lt $deadline) {
                        ...
                        if ($hwnd -ne [System.IntPtr]::Zero) {
                            [MoorHandoff.Win32]::ShowWindow($hwnd, 9) | Out-Null  # SW_RESTORE
                            [MoorHandoff.Win32]::SetForegroundWindow($hwnd) | Out-Null
    ...
    if (-not $spawned) {
        ...
        # verify by watching for a fresh Moor process.
        try {
            ...
            while ((Get-Date) -lt $explorerDeadline) {
                ...
                if ($fresh.Count -gt 0) {
                    ...
                    try {
                        if ($script:Win32) {
                            [MoorHandoff.Win32]::AllowSetForegroundWindow([int]$fresh[0].Id) | Out-Null
...
# closes it. `moor update` deliberately runs its build steps with stdout
# inherited (moor_cli/main.py, the tee-stderr runner), so the tree under a
...
if ($env:MOOR_UPDATE_PIPE_DRAIN_SECONDS) {
    $parsedGrace = 0
    if ([int]::TryParse($env:MOOR_UPDATE_PIPE_DRAIN_SECONDS, [ref]$parsedGrace) -and $parsedGrace -ge 0) {
...
if ($env:MOOR_UPDATE_STEP_IDLE_SECONDS) {
    $parsedIdle = 0
    if ([int]::TryParse($env:MOOR_UPDATE_STEP_IDLE_SECONDS, [ref]$parsedIdle) -and $parsedIdle -gt 0) {
        $script:StepIdleTimeoutSeconds = $parsedIdle
    }
}

# Silence on the pipes is NOT silence in the update. `moor update` captures
# the (very loud) Electron/vite build into logs/update.log instead of its own
# stdout (moor_cli/update_cmd.py, the update-log tee), so a real update is
...
if ($env:MOOR_UPDATE_PROGRESS_LOG) {
    $script:StepProgressLogPath = $env:MOOR_UPDATE_PROGRESS_LOG
}
...
public static class MoorUpdateJob {
...
function Invoke-MoorStep([string]$Exe, [string[]]$MoorArgs, [string]$Tag) {
    ...
    # afterwards, where `moor debug share` picks it up.
    #
    ...
    # .moor-update-result.json, clearing .moor-update-in-progress,
    ...
    # Moor" until the user killed something by hand. Losing the tail of a
    ...
    $arguments = ($MoorArgs | ForEach-Object { '"{0}"' -f ($_ -replace '"', '\"') }) -join ' '
    ...
    try {
        ...
        $started = [MoorUpdateJob]::StartAssigned($Exe, $arguments)
    } finally {
    ...
    while ($true) {
        ...
        if ($proc.HasExited) {
            ...
            # Quiet pipes are how a healthy `moor update` looks for 40+
            ...
            if ($currentLogStamp -ne $progressLogStamp) {
                ...
                $stalled = [MoorUpdateJob]::TerminateAndWait($job, 124, 10000)
                if (-not $stalled) {
...
# the page can be QA'd in any browser; MOOR_SELFTEST_FAIL=1 exercises the
# error state, MOOR_SELFTEST_HOLD_SECONDS delays the terminal event.
if ($SelfTestUi) {
    ...
    $hold = 6
    if ($env:MOOR_SELFTEST_HOLD_SECONDS) { $hold = [int]$env:MOOR_SELFTEST_HOLD_SECONDS }
    Publish-UiProgress "Testing quiet update"
    Start-Sleep -Seconds $hold
    if ($env:MOOR_SELFTEST_FAIL) {
...
# -SelfTestPipeDrain: prove Invoke-MoorStep survives a leaked pipe ------
# The #90455 deadlock needs no update, no checkout and no Moor install to
...
#            update log, the shape of every real `moor update` build (output
...
if ($SelfTestPipeDrain) {
    ...
    if ($env:MOOR_SELFTEST_HOLD_SECONDS) { $hold = [int]$env:MOOR_SELFTEST_HOLD_SECONDS }
    $floodKb = 8192
    if ($env:MOOR_SELFTEST_FLOOD_KB) { $floodKb = [int]$env:MOOR_SELFTEST_FLOOD_KB }
    # $PSHOME is this interpreter's own directory -- no hardcoded system path.
    $powershell = Join-Path $PSHOME "powershell.exe"
    $stamp = [Guid]::NewGuid().ToString("N")
    $childPs1 = Join-Path $TempDir "moor-pipe-drain-$stamp.ps1"
    $floodPs1 = Join-Path $TempDir "moor-pipe-flood-$stamp.ps1"
    $pidFile = Join-Path $TempDir "moor-pipe-drain-$stamp.pid"
    $stallPs1 = Join-Path $TempDir "moor-step-stall-$stamp.ps1"
    $stallPidFile = Join-Path $TempDir "moor-step-stall-$stamp.pid"
    $stallGrandchildPidFile = Join-Path $TempDir "moor-step-stall-grandchild-$stamp.pid"
    $logStallPs1 = Join-Path $TempDir "moor-step-logstall-$stamp.ps1"
    $logStallProgress = Join-Path $TempDir "moor-step-logstall-$stamp.update.log"
...
'@
    ...
    # merely loud. `moor update` is this shape -- the Electron/vite build
...
'@
    ...
    $flood = Invoke-MoorStep $powershell @(
    ...
    $stall = Invoke-MoorStep $powershell @(
    ...
    try {
        $logstall = Invoke-MoorStep $powershell @(
    ...
    if ($stallAlive) { $problems += "stalled child pid $stallPid remained alive after Invoke-MoorStep returned" }
    if ($stallGrandchildAlive) { $problems += "stalled descendant pid $stallGrandchildPid remained alive after Invoke-MoorStep returned" }
...
try {
    ...
    try {
        ...
        $startedAt = 0L
        $hasStartedAt = [int64]::TryParse($env:MOOR_UPDATE_STARTED_AT, [ref]$startedAt)
    ...
    Publish-UiProgress "Waiting for Moor to close"
    if ($DesktopPid -gt 0) {
    ...
    Publish-UiProgress "Preparing Moor files"
    $shim = Join-Path $InstallRoot "venv\Scripts\moor.exe"
    if (Test-Path -LiteralPath $shim) {
        ...
        if (-not $unlocked) {
            ...
            $finalCode = 5
            $finalMsg = "Update aborted: another process is still holding the Moor install open (venv\Scripts\moor.exe locked after 20s). Nothing was changed. Close other Moor windows/terminals and try again."
            Write-HandoffLog $finalMsg
            exit $finalCode
        }
    ...
    # --force skips only the moor.exe shim guard, which step 2 just PROVED
    ...
    #
    # DRIVE THE UPDATE THROUGH venv\Scripts\python.exe, NOT venv\Scripts\moor.exe.
    # `uv pip install -e .` has to replace the console-script shims, so
    # _quarantine_running_moor_exe must first rename the running moor.exe
    # out of the way. On Windows that rename fails whenever ANY child process
    # spawned from that moor.exe is still alive: a child inherits a handle on
    ...
    # install whose Start Menu shortcut points at a Moor.exe that no longer
    ...
    #
    # Running the same code as `python.exe -m moor_cli.main update` puts the
    ...
    if (-not (Test-Path -LiteralPath $pythonExe)) {
        $finalCode = 3
        $finalMsg = "Update aborted: $pythonExe is missing. The install needs repair (run the Moor installer or `moor doctor`)."
        Write-HandoffLog $finalMsg
        exit $finalCode
    }
    $updateArgs = @("-m", "moor_cli.main", "update", "--yes", "--gateway", "--force", "--branch", $Branch)
    ...
    # collides with the "close all Moor windows" sentinel.
    try {
        $updateHelp = & $pythonExe -m moor_cli.main update --help 2>$null | Out-String
        if ($updateHelp -match "--keep-stash") {
            $updateArgs += "--keep-stash"
        } else {
            Write-HandoffLog "installed moor predates --keep-stash; running without it"
        }
    } catch {
    ...
    Write-HandoffLog "moor update exit code: $($res.Code)"

    $retryPolicyPath = Join-Path $PSScriptRoot "retry-policy.ps1"
    if (Test-Path -LiteralPath $retryPolicyPath) {
        . $retryPolicyPath
        $shouldRetry = Test-MoorUpdateShouldRetry -ExitCode $res.Code -InstallRoot $InstallRoot
    } else {
    ...
    # `moor update` treats a Desktop GUI build failure as NON-fatal (prints
    ...
    if ($res.Code -eq 0 -and $res.Output -match "Desktop build failed") {
        ...
        $rebuild = Invoke-MoorStep $pythonExe @("-m", "moor_cli.main", "desktop", "--force-build", "--build-only") "rebuild"
    ...
    if ($res.Code -eq 0 -and -not $desktopBuildFailed) {
        ...
        $finalCode = 6
        $finalMsg = "Code and dependencies updated, but the Desktop app REBUILD FAILED - you are running the previous build. Run `moor desktop --force-build` from a terminal to retry."
    } else {
        $finalCode = $res.Code
        $finalMsg = "Update failed (exit $($res.Code)). Run `moor debug share` in a terminal to send a report."
    }
    exit $finalCode
} finally {
    ...
    if (-not $script:TreeSafeToFinalize) {
        ...
        $finalCode = 7
        $finalMsg = "Update recovery could not stop every updater process. Moor was not restarted to avoid overlapping the active install. Wait for it to finish or restart Windows, then reopen Moor."
        ...
        if ($finalCode -ne 0) {
            ...
            Publish-UiProgress "Opening Moor"
