function Get-MoorRuntimeCommand {
    param(
        [Parameter(Mandatory = $true)][string]$InstallRoot,
        [string]$Module = 'moor_cli.main'
    )
    ...
    foreach ($name in @('moor.exe', 'moor.cmd')) {
        $launcher = Join-Path $InstallRoot ".moor\bin\$name"
    ...
    if (Test-Path -LiteralPath (Join-Path $InstallRoot 'moor_cli/_launchers.py') -PathType Leaf) {
        $directories = @(
            (Join-Path $env:MOOR_HOME 'bin'),
            (Join-Path (Split-Path -Parent $InstallRoot) 'bin')
        )
        if ($env:LOCALAPPDATA) { $directories += Join-Path $env:LOCALAPPDATA 'moor/bin' }
    ...
    throw "Installation launcher is missing under $InstallRoot\.moor\bin. Repair this installation."
}