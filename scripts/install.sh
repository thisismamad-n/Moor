...
# Python-driven tool session (e.g. Moor terminal tool). A pre-set PYTHONPATH
...
# Configuration
REPO_URL_SSH="git@github.com:thisismamad-n/Moor.git"
REPO_URL_HTTPS="https://github.com/thisismamad-n/Moor.git"
MOOR_HOME="${MOOR_HOME:-$HOME/.moor}"
# INSTALL_DIR is resolved AFTER arg parsing and OS detection so we can pick an
# FHS-style layout for root installs.  Track whether the user gave us an
# explicit directory — if so we never override it.
if [ -n "${MOOR_INSTALL_DIR:-}" ]; then
...
#   code at /usr/local/lib/moor-agent, command at /usr/local/bin/moor,
#   data still at /root/.moor (MOOR_HOME).  Matches Claude Code / Codex CLI
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "                   write \$MOOR_HOME/.no-bundled-skills so future"
            echo "                   'moor update' runs never inject bundled skills either"
            ...
            echo "  --include-desktop  Also build the desktop app (apps/desktop -> Moor.app)"
            ...
            echo ""
            echo "Notes:"
            echo "  When running as root on Linux, Moor installs the code under"
            echo "  /usr/local/lib/moor-agent and links the command into"
            echo "  /usr/local/bin/moor (FHS layout — matches Claude Code / Codex CLI)."
            echo "  Data, config, sessions, and logs still live in \$MOOR_HOME"
            echo "  (default /root/.moor).  This keeps Docker bind-mounted volumes"
            echo "  small and ensures the command is on PATH for all shells."
            echo "  Existing installs at \$MOOR_HOME/moor-agent are preserved in-place."
...
print_banner() {
    ...
    echo "│             ⚕ Moor Agent Installer                    │"
    echo "├─────────────────────────────────────────────────────────┤"
    echo "│  An open source AI agent by Moor inc..              │"
    echo "└─────────────────────────────────────────────────────────┘"
    echo -e "${NC}"
}
...
# intentional, but they leave the checkout dirty — which forces `moor update`
...
emit_manifest() {
    ...
    printf '%s' '{"protocol_version":1,"stages":[{"name":"prerequisites","title":"System prerequisites","category":"runtime","needs_user_input":false},{"name":"repository","title":"Download Moor Agent","category":"runtime","needs_user_input":false},{"name":"venv","title":"Create Python virtual environment","category":"runtime","needs_user_input":false},{"name":"python-deps","title":"Install Python dependencies","category":"runtime","needs_user_input":false},{"name":"node-deps","title":"Install browser-tool dependencies","category":"runtime","needs_user_input":false},{"name":"path","title":"Install moor command","category":"runtime","needs_user_input":false},{"name":"config","title":"Prepare config and skills","category":"configuration","needs_user_input":false},{"name":"setup","title":"Configure API keys and settings","category":"configuration","needs_user_input":true},{"name":"gateway","title":"Configure gateway service","category":"configuration","needs_user_input":true},'"$desktop_stage"'{"name":"complete","title":"Finish install","category":"runtime","needs_user_input":false}]}'
    printf '\n'
}
...
# Decide where the repo checkout + venv live, and where the `moor` command
# symlink goes.  Called after detect_os so $OS/$DISTRO are known.
#
# Defaults:
#   - Non-root, any OS:       INSTALL_DIR = $MOOR_HOME/moor-agent
#                             command link in $HOME/.local/bin
#   - Termux (any uid):       INSTALL_DIR = $MOOR_HOME/moor-agent
#                             command link in $PREFIX/bin (already on PATH)
#   - Root on Linux (new):    INSTALL_DIR = /usr/local/lib/moor-agent
#                             command link in /usr/local/bin
#                             (unless a legacy install already exists at
#                              $MOOR_HOME/moor-agent — then preserve it)
#
# Always no-op when the user set --dir or $MOOR_INSTALL_DIR.
resolve_install_layout() {
    ...
    # Termux: package manager manages /data/data/..., keep code in MOOR_HOME.
    if is_termux; then
    ...
    if [ "$OS" = "linux" ] && [ "$(id -u)" -eq 0 ]; then
        if [ -d "$MOOR_HOME/moor-agent/.git" ]; then
            INSTALL_DIR="$MOOR_HOME/moor-agent"
            log_info "Existing install detected at $INSTALL_DIR — keeping legacy layout"
            log_info "  (new root installs use /usr/local/lib/moor-agent)"
            return 0
        fi
        INSTALL_DIR="/usr/local/lib/moor-agent"
        ...
        # /usr/local/bin/moor wrapper unable to exec the bad-interpreter venv
        # python.  See #21457.
        ...
        log_info "  Command: /usr/local/bin/moor"
        log_info "  Data:    $MOOR_HOME (unchanged)"
...
configure_managed_node_npm_prefix() {
    ...
    mkdir -p "$MOOR_HOME/node/etc"
    printf 'prefix=%s\n' "$(dirname "$link_dir")" > "$MOOR_HOME/node/etc/npmrc"
}

get_moor_command_path() {
    local link_dir
    link_dir="$(get_command_link_dir)"
    if [ -x "$link_dir/moor" ]; then
        echo "$link_dir/moor"
    else
        echo "moor"
    fi
...
install_uv() {
    ...
    # Moor owns its own uv at $MOOR_HOME/bin/uv.  Always install there —
    # no PATH probing, no conda guards, no multi-location resolution chains.
    # The runtime update path (moor_cli/managed_uv.py) looks in the same
    # place, so install.sh and `moor update` stay in sync.
    local _managed_uv="$MOOR_HOME/bin/uv"
    ...
    log_info "Installing managed uv into $MOOR_HOME/bin ..."
    mkdir -p "$MOOR_HOME/bin"

    # Two-stage: download the installer, then run it.  Piping
    # `curl | sh` masks curl failures (sh exits 0 on empty stdin)
    # and conflates network errors with installer errors.
    local _uv_install_log _uv_installer
    _uv_install_log="$(mktemp 2>/dev/null || echo "/tmp/moor-uv-install.$$.log")"
    _uv_installer="$(mktemp 2>/dev/null || echo "/tmp/moor-uv-installer.$$.sh")"
    ...
    # directly into $MOOR_HOME/bin instead of ~/.local/bin.
    if UV_UNMANAGED_INSTALL="$MOOR_HOME/bin" sh "$_uv_installer" >>"$_uv_install_log" 2>&1; then
        rm -f "$_uv_installer"
        if [ -x "$_managed_uv" ]; then
            UV_CMD="$_managed_uv"
        else
...
check_network_prerequisites() {
    ...
    if [ "$DISTRO" = "termux" ]; then
        ...
        log_warn "Network checks failed. Moor install may complete, but web search and dependency downloads can fail."
    ...
    if [ -n "$pkg_install" ]; then
        ...
        # sudo needs password — ask once for everything
            ...
            if [ "$IS_INTERACTIVE" = true ]; then
                ...
                log_info "Moor Agent itself does not require or retain root access."
    ...
    if [ -d "$INSTALL_DIR" ]; then
        if [ -d "$INSTALL_DIR/.git" ]; then
            ...
            if [ -n "$(git status --porcelain)" ]; then
                ...
                # index-level conflict state. Mirrors the `moor update` path
                # (#4735).
                ...
                local stash_name
                stash_name="moor-install-autostash-$(date -u +%Y%m%d-%H%M%S)"
            ...
            if [ -n "$autostash_ref" ]; then
                ...
                if [ "$restore_now" = "yes" ]; then
                    ...
                    if [ "$restore_ok" = "yes" ] && [ -z "$conflicted_files" ]; then
                        ...
                        log_warn "Review git diff / git status if Moor behaves unexpectedly."
                    else
...
setup_path() {
    log_info "Setting up moor command..."
    ...
    if [ ! -x "$MOOR_BIN" ] || { [ "$USE_VENV" = true ] && [ ! -f "$MOOR_ENTRYPOINT" ]; }; then
    ...
    # Create a user-facing shim for the moor command.
    # We intentionally clear PYTHONPATH/PYTHONHOME here so inherited env vars
    # can't make this launcher import modules from another checkout.
    mkdir -p "$command_link_dir"
    # Older installs created this path as a symlink to $MOOR_BIN. Without
    # the rm, `cat >` follows the symlink and overwrites the venv pip entry
    # point with this shim — making `exec "$MOOR_BIN"` self-recurse. (#21454)
    rm -f "$command_link_dir/moor"
    if [ "$USE_VENV" = true ]; then
        ...
        cat > "$command_link_dir/moor" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$MOOR_BIN" "$MOOR_ENTRYPOINT" "\$@"
EOF
    else
        cat > "$command_link_dir/moor" <<EOF
#!/usr/bin/env bash
...
EOF
    ...
    chmod +x "$command_link_dir/moor"
    log_success "Installed moor launcher → $command_link_display_dir/moor"

    # Also expose `moor-agent`. The `moor-agent` console script declared in
    ...
    rm -f "$command_link_dir/moor-agent"
    if [ "$USE_VENV" = true ]; then
        cat > "$command_link_dir/moor-agent" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$MOOR_BIN" "$INSTALL_DIR/run_agent.py" "\$@"
EOF
    else
        cat > "$command_link_dir/moor-agent" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$MOOR_BIN" run_agent.py "\$@"
EOF
    fi
    chmod +x "$command_link_dir/moor-agent"
    log_success "Installed moor-agent launcher → $command_link_display_dir/moor-agent"

    # Also expose `moor-acp`. ACP hosts (Zed, JetBrains, Buzz) resolve the
    # agent by command name on the login-shell PATH, and the `moor-acp`
    ...
    rm -f "$command_link_dir/moor-acp"
    if [ "$USE_VENV" = true ]; then
        cat > "$command_link_dir/moor-acp" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$MOOR_BIN" "$MOOR_ENTRYPOINT" acp "\$@"
EOF
    else
        cat > "$command_link_dir/moor-acp" <<EOF
#!/usr/bin/env bash
...
EOF
    ...
    chmod +x "$command_link_dir/moor-acp"
    log_success "Installed moor-acp launcher → $command_link_display_dir/moor-acp"

    if [ "$DISTRO" = "termux" ]; then
        export PATH="$command_link_dir:$PATH"
        log_info "$command_link_display_dir is the native Termux command path"
        log_success "moor command ready"
        return 0
    fi
    ...
    if [ "$ROOT_FHS_LAYOUT" = true ]; then
        ...
        if env -i HOME="$HOME" TERM="${TERM:-dumb}" bash -i -c 'command -v moor' \
                >/dev/null 2>&1; then
        ...
        log_info "moor not on PATH in non-login shells (common on RHEL-family)"
        PATH_LINE='export PATH="/usr/local/bin:$PATH"'
        PATH_COMMENT='# Moor Agent — ensure /usr/local/bin is on PATH (RHEL non-login shells)'
    ...
    if ! echo "$PATH" | tr ':' '\n' | grep -q "^$command_link_dir$"; then
        ...
        if [ "$IS_FISH" = "true" ]; then
            if ! grep -q 'fish_add_path.*\.local/bin' "$FISH_CONFIG" 2>/dev/null; then
                echo "" >> "$FISH_CONFIG"
                echo "# Moor Agent — ensure ~/.local/bin is on PATH" >> "$FISH_CONFIG"
    ...
    # Export for current session so moor works immediately
    export PATH="$command_link_dir:$PATH"
...
copy_config_templates() {
    ...
    # Create ~/.moor directory structure (config at top level, code in subdir)
    mkdir -p "$MOOR_HOME"/{cron,sessions,logs,pairing,hooks,image_cache,audio_cache,memories,skills}

    # Create .env at ~/.moor/.env (top level, easy to find)
    if [ ! -f "$MOOR_HOME/.env" ]; then
        if [ -f "$INSTALL_DIR/.env.example" ]; then
            cp "$INSTALL_DIR/.env.example" "$MOOR_HOME/.env"
            log_success "Created ~/.moor/.env from template"
        else
            touch "$MOOR_HOME/.env"
            log_success "Created ~/.moor/.env"
        fi
    else
        log_info "~/.moor/.env already exists, keeping it"
    fi
    ...
    # Create config.yaml at ~/.moor/config.yaml (top level, easy to find)
    if [ ! -f "$MOOR_HOME/config.yaml" ]; then
        if [ -f "$INSTALL_DIR/cli-config.yaml.example" ]; then
            cp "$INSTALL_DIR/cli-config.yaml.example" "$MOOR_HOME/config.yaml"
            log_success "Created ~/.moor/config.yaml from template"
        fi
    else
        log_info "~/.moor/config.yaml already exists, keeping it"
    fi

    # Create SOUL.md if it doesn't exist (global persona file).
    # This MUST match DEFAULT_SOUL_MD in moor_cli/default_soul.py — the
...
SOUL_EOF
    ...
    log_success "Configuration directory ready: ~/.moor/"

    # Seed bundled skills into ~/.moor/skills/ (manifest-based, one-time per skill)
    if [ "$NO_SKILLS" = true ]; then
        # Blank-slate install: write the opt-out marker and skip seeding.
        # skills_sync.py and `moor update` both honor this marker, so the
        ...
        printf '%s\n' \
            ...
            "Delete this file to re-enable sync on the next 'moor update'." \
            > "$MOOR_HOME/.no-bundled-skills" 2>/dev/null || true
        log_info "Skipping bundled skills (--no-skills). Wrote $MOOR_HOME/.no-bundled-skills"
        log_info "  Future 'moor update' runs will not inject bundled skills. Delete the marker to opt back in."
    else
        log_info "Syncing bundled skills to ~/.moor/skills/ ..."
        if "$INSTALL_DIR/venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" 2>/dev/null; then
            log_success "Skills synced to ~/.moor/skills/"
        else
            # Fallback: simple directory copy if Python sync fails
            if [ -d "$INSTALL_DIR/skills" ] && [ ! "$(ls -A "$MOOR_HOME/skills/" 2>/dev/null | grep -v '.bundled_manifest')" ]; then
                cp -r "$INSTALL_DIR/skills/"* "$MOOR_HOME/skills/" 2>/dev/null || true
                log_success "Skills copied to ~/.moor/skills/"
            fi
        fi
    fi
}
...
strip_snap_browser_override() {
    ...
    local tmp
    tmp="$(mktemp)" || return 0
    if grep -Ev '^AGENT_BROWSER_EXECUTABLE_PATH=/snap/|^# Moor Agent browser tools' "$env_file" > "$tmp"; then
        mv "$tmp" "$env_file"
        log_warn "Removed stale Snap browser override (AGENT_BROWSER_EXECUTABLE_PATH=/snap/...) from $env_file"
        log_info "Moor will use the bundled Chromium instead."
...
configure_browser_env_from_system_browser() {
    ...
    {
        echo ""
        echo "# Moor Agent browser tools — explicit browser override."
...
install_node_deps() {
    ...
    # Keep the checkout clean so `moor update` doesn't autostash every run.
    restore_dirty_lockfiles "$INSTALL_DIR"
}
...
install_browser_use_cli() {
    ...
    # `moor tools` can install it later.
    if [ "$SKIP_BROWSER" = true ]; then
    ...
    if [ -x "$MOOR_HOME/bin/browser-use" ]; then
    ...
    if run_with_timeout 600 env UV_NO_CONFIG=1 UV_TOOL_BIN_DIR="$MOOR_HOME/bin" \
        ...
        log_info "Install later with: $UV_CMD tool install browser-use  (or via 'moor tools')"
    fi
}
...
install_computer_use_driver() {
    ...
    # `moor tools`, the dashboard, or the desktop app — is a config flip,
    # not a surprise multi-minute binary fetch (the confusion this fixes:
    # users had to discover `moor computer-use install` on their own).
    ...
    # Same upstream installer `moor computer-use install` runs; time-boxed
    ...
    if run_with_timeout 660 /bin/bash -c \
        ...
        >"$cua_log" 2>&1; then
        log_success "Computer Use driver installed (enable via 'moor tools' → Computer Use)"
    else
        log_warn "Computer Use driver install failed — it will install on demand when you enable the tool."
        log_info "Install later with: moor computer-use install"
...
run_setup_wizard() {
    ...
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Setup wizard skipped (no terminal available). Run 'moor setup' after install."
        return 0
    fi
    ...
    # Run moor setup using the venv Python directly (no activation needed).
    # Redirect stdin from /dev/tty so interactive prompts work when piped from curl.
    if [ "$USE_VENV" = true ]; then
        "$INSTALL_DIR/venv/bin/python" -m moor_cli.main setup < /dev/tty
    else
        python -m moor_cli.main setup < /dev/tty
    fi
}

maybe_start_gateway() {
    # Check if any messaging platform tokens were configured
    ENV_FILE="$MOOR_HOME/.env"
    if [ ! -f "$ENV_FILE" ]; then
        return 0
    fi
    ...
    log_info "The gateway needs to be running for Moor to send/receive messages."

    # If WhatsApp is enabled and no session exists yet, run foreground first for QR scan
    WHATSAPP_VAL=$(grep "^WHATSAPP_ENABLED=" "$ENV_FILE" 2>/dev/null | cut -d'=' -f2-)
    WHATSAPP_SESSION="$MOOR_HOME/whatsapp/session/creds.json"
    if [ "$WHATSAPP_VAL" = "true" ] && [ ! -f "$WHATSAPP_SESSION" ]; then
        if [ "$IS_INTERACTIVE" = true ]; then
            echo ""
            log_info "WhatsApp is enabled but not yet paired."
            log_info "Running 'moor whatsapp' to pair via QR code..."
            echo ""
            if prompt_yes_no "Pair WhatsApp now?" "yes"; then
                MOOR_CMD="$(get_moor_command_path)"
                $MOOR_CMD whatsapp || true
            fi
        else
            log_info "WhatsApp pairing skipped (non-interactive). Run 'moor whatsapp' to pair."
        fi
    fi
    ...
    # but opening fails with ENXIO. See #16746.
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Gateway setup skipped (no terminal available). Run 'moor gateway install' later."
        return 0
    fi
    ...
    if [ "$should_install_gateway" = true ]; then
        ...
        if [ "$DISTRO" != "termux" ] && command -v systemctl &> /dev/null; then
            ...
            if $MOOR_CMD gateway install 2>/dev/null; then
                log_success "Gateway service installed"
                if $MOOR_CMD gateway start 2>/dev/null; then
                    log_success "Gateway started! Your bot is now online."
                else
                    log_warn "Service installed but failed to start. Try: moor gateway start"
                fi
            else
                log_warn "Systemd install failed. You can start manually: moor gateway"
            fi
        else
            if [ "$DISTRO" = "termux" ]; then
            ...
            nohup $MOOR_CMD gateway > "$MOOR_HOME/logs/gateway.log" 2>&1 &
            GATEWAY_PID=$!
            log_success "Gateway started (PID $GATEWAY_PID). Logs: ~/.moor/logs/gateway.log"
            log_info "To stop: kill $GATEWAY_PID"
            log_info "To restart later: moor gateway"
            if [ "$DISTRO" = "termux" ]; then
                log_warn "Android may stop background processes when Termux is suspended or the system reclaims resources."
            fi
        fi
    else
        log_info "Skipped. Start the gateway later with: moor gateway"
    fi
}

write_bootstrap_marker() {
    # Writes $INSTALL_DIR/.moor-bootstrap-complete, which tells the Moor
    ...
    local marker_path="$INSTALL_DIR/.moor-bootstrap-complete"
...
print_success() {
    ...
    echo ""
    echo -e "   ${YELLOW}Config:${NC}    $MOOR_HOME/config.yaml"
    echo -e "   ${YELLOW}API Keys:${NC}  $MOOR_HOME/.env"
    echo -e "   ${YELLOW}Data:${NC}      $MOOR_HOME/cron/, sessions/, logs/"
    ...
    echo ""
    echo -e "   ${GREEN}moor${NC}              Start chatting"
    echo -e "   ${GREEN}moor setup${NC}        Configure API keys & settings"
    echo -e "   ${GREEN}moor config${NC}       View/edit configuration"
    echo -e "   ${GREEN}moor config edit${NC}  Open config in editor"
    echo -e "   ${GREEN}moor gateway install${NC} Install gateway service (messaging + cron)"
    echo -e "   ${GREEN}moor update${NC}       Update to latest version"
    echo ""

    echo -e "${CYAN}─────────────────────────────────────────────────────────${NC}"
    echo ""
    if [ "$DISTRO" = "termux" ]; then
        echo -e "${YELLOW}⚡ 'moor' was linked into $(get_command_link_display_dir), which is already on PATH in Termux.${NC}"
        echo ""
    elif [ "$ROOT_FHS_LAYOUT" = true ]; then
        echo -e "${YELLOW}⚡ 'moor' was linked into /usr/local/bin and is ready to use — no shell reload needed.${NC}"
        echo ""
    else
        echo -e "${YELLOW}⚡ Reload your shell to use 'moor' command:${NC}"
        echo ""
        LOGIN_SHELL="$(basename "${SHELL:-/bin/bash}")"
...
ensure_browser() {
    if ! command -v node >/dev/null 2>&1; then
        local node_bin="$MOOR_HOME/node/bin/node"
        if [ -x "$node_bin" ]; then
            export PATH="$MOOR_HOME/node/bin:$PATH"
        else
    ...
    local npm_bin
    npm_bin="$(command -v npm 2>/dev/null || echo "$MOOR_HOME/node/bin/npm")"
    if [ ! -x "$npm_bin" ]; then
    ...
    # which every consumer (tools/browser_tool.py, `moor update`'s npx
    ...
    if ! run_with_timeout "$NODE_DEPS_TIMEOUT" "$npm_bin" install -g --prefix "$MOOR_HOME/node" --silent --ignore-scripts \
...
# extract a tree MISSING the electron binary, so the `electron`->`Moor` rename
...
# _purge_electron_build_cache() used by `moor desktop`; install.sh was the only
...
install_desktop() {
    ...
    #         retry (matches install.ps1 / `moor desktop`),
    ...
    local app=""
    if [ "$OS" = "linux" ]; then
        if [ -x "$desktop_dir/release/linux-unpacked/Moor" ]; then
            app="$desktop_dir/release/linux-unpacked/Moor"
        elif [ -x "$desktop_dir/release/linux-unpacked/moor" ]; then
            app="$desktop_dir/release/linux-unpacked/moor"
        fi
    else
        local cand
        for cand in \
            "$desktop_dir/release/mac-arm64/Moor.app" \
            "$desktop_dir/release/mac/Moor.app"; do
            if [ -d "$cand" ]; then
                app="$cand"
                break
            fi
    ...
    # `moor desktop`, so install/repair and self-update agree about the app's
    ...
    # importing Python cannot reverse it by loading MOOR_HOME/.env. If the
    ...
    if [ "$OS" = "macos" ] && [ -z "${CSC_LINK:-}" ] && [ -z "${APPLE_SIGNING_IDENTITY:-}" ] && command -v codesign >/dev/null 2>&1; then
        ...
        if [ -x "$config_python" ]; then
            if MOOR_HOME="$MOOR_HOME" "$config_python" - "$desktop_dir" <<'PYEOF'
import sys
from pathlib import Path
from moor_cli.main import _desktop_macos_relaunchable_fixup
...
PYEOF
    ...
    # checkout stays clean for the next `moor update`.
    restore_dirty_lockfiles "$INSTALL_DIR"
}
...
main() {
    ...
    # Code-scoped stamp: write next to the install tree, not into $MOOR_HOME.
    # $MOOR_HOME is a shared data dir (it can be bind-mounted into a Docker
    # gateway too), so a stamp there gets clobbered by the container's 'docker'
    # stamp and wrongly blocks 'moor update' on this host install.
