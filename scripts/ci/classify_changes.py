...
# tree generates llms.txt — the index every LLM (Moor included, via the
# moor-agent skill) reads to learn what Moor can do — and
...
_MCP_CATALOG_FILES = {"moor_cli/mcp_catalog.py"}
...
# Rust crates — currently just the Tauri bootstrap installer (moor-setup).
...
def _py_test_only(p: str) -> bool:
    ...
    Product jobs (Desktop E2E's ``moor serve`` backend, the Docker image)
