# Behavioral test for install.ps1's moor launcher staging (PR #92092,
...
#
# The test lifts the real Install-MoorCommandLaunchers function from the
...
# the caller (Set-PathVariable passes $MoorHome\bin -- the managed binary
...
$fn = $ast.Find({
    ...
    $n.Name -eq 'Install-MoorCommandLaunchers'
}, $true)

if (-not $fn) {
    throw "Install-MoorCommandLaunchers not found in $installPs1"
}
...
$caseRoot = [System.IO.Path]::GetFullPath((Join-Path $tempBase (
    'moor-cli-launcher-test-' + [guid]::NewGuid().ToString('N')
)))
...
try {
    $installRoot = Join-Path $caseRoot 'moor-agent'
    ...
    $missingThrew = $false
    try {
        Install-MoorCommandLaunchers -Root $installRoot -Destination $binDir | Out-Null
    } catch {
        $missingThrew = $_.Exception.Message -like '*required launcher not found*'
    }
    Assert-True $missingThrew 'missing moor.exe fails the launcher stage'
    ...
    $moorV2 = [byte[]](77, 90, 2)
    $acp = [byte[]](77, 90, 3)
    [System.IO.File]::WriteAllBytes((Join-Path $scriptsDir 'moor.exe'), $moorV1)
    Set-Content -Path (Join-Path $installRoot 'venv\pyvenv.cfg') `
        -Value "home = X" -Encoding Ascii

    $staged = Install-MoorCommandLaunchers -Root $installRoot -Destination $binDir
    Assert-True ($staged -eq $binDir) 'returns the destination it staged into'
    Assert-BytesEqual $moorV1 `
        ([System.IO.File]::ReadAllBytes((Join-Path $binDir 'moor.exe'))) `
        'normal venv: exe copy lands in the destination'
    Assert-True (-not (Test-Path -LiteralPath (Join-Path $binDir 'moor-acp.exe'))) `
        'optional ACP launcher may be absent'

    [System.IO.File]::WriteAllBytes((Join-Path $scriptsDir 'moor.exe'), $moorV2)
    [System.IO.File]::WriteAllBytes((Join-Path $scriptsDir 'moor-acp.exe'), $acp)
    Install-MoorCommandLaunchers -Root $installRoot -Destination $binDir | Out-Null
    Assert-BytesEqual $moorV2 `
        ([System.IO.File]::ReadAllBytes((Join-Path $binDir 'moor.exe'))) `
        'installer refreshes an existing Moor launcher'
    Assert-BytesEqual $acp `
        ([System.IO.File]::ReadAllBytes((Join-Path $binDir 'moor-acp.exe'))) `
    ...
    Install-MoorCommandLaunchers -Root $installRoot -Destination $binDir | Out-Null
    Assert-True (Test-Path -LiteralPath (Join-Path $binDir 'moor.cmd')) `
        'relocatable venv: .cmd delegator staged'
    Assert-True (-not (Test-Path -LiteralPath (Join-Path $binDir 'moor.exe'))) `
        'relocatable venv: stale exe copy removed'
    $cmdBody = [System.IO.File]::ReadAllText((Join-Path $binDir 'moor.cmd'))
    Assert-True ($cmdBody.Contains((Join-Path $scriptsDir 'moor.exe')) -and $cmdBody.Contains('%*')) `
