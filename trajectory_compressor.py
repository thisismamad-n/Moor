...
from moor_constants import OPENROUTER_BASE_URL, get_moor_home
from agent.retry_utils import jittered_backoff
from moor_cli.env_loader import load_moor_dotenv

# Load .env from MOOR_HOME first, then project root as a dev fallback.
load_moor_dotenv(moor_home=get_moor_home(), project_env=Path(__file__).parent / ".env")
...
_PROVIDER_HOSTS: Tuple[Tuple[str, str], ...] = (
    ("openrouter.ai", "openrouter"), ("nousresearch.com", "moor"), ("z.ai", "zai"), ("moonshot.ai", "kimi-coding"),
...
class TrajectoryCompressor:
    ...
    def _init_summarizer(self):
        ...
        if provider:
            ...
            if client is None:
                raise RuntimeError(f"Provider '{provider}' is not configured. Check your API key or run: moor setup")
