"""state.db file-level health helpers, split out of ``moor_state.py``.
...
holder-process classification.  Helpers that moor_state itself imports and
calls (``_connect_tracked_db`` & co) are looked up lazily from ``moor_state`` at
call time, so tests that monkeypatch ``moor_state.<name>`` keep intercepting.
"""
...
# shared lock (see moor_cli/sqlite_safe_read.py) so another process can treat this writer as
...
def refuse_deleted_wal_generation(db_path) -> None:
    ...
    *before* ``sqlite3.connect`` so a second opener cannot mint a replacement WAL inode."""
    from moor_state import DeletedWalGenerationError
    from moor_state_errors import _DELETED_WAL_GENERATION_MSG
...
def _connect_tracked_db(path, tracking_path=None, **kwargs):
    ...
    without moor_cli); a real connection failure must propagate — a silent untracked retry
    would disable the guard for that connection."""
    try:
        from moor_cli.sqlite_safe_read import connect_tracked
    except ImportError:
        logger.debug("moor_cli.sqlite_safe_read unavailable; opening %s untracked "
                     "(byte-probe guard inactive in this install)", path)
        return sqlite3.connect(str(path), **kwargs)
    # Open through THIS module's sqlite3.connect so tests patching moor_state.sqlite3.connect keep control.
...
def is_zeroed_state_db(path: Path, *, probe_bytes: int = 100, force: bool = False) -> bool:
    ...
    snapshots).  Prefers ``moor_cli.backup.is_zeroed_sqlite_file``; this copy keeps SessionDB
    ...
    with quarantine_cross_process_lock(path) as acquired:
        if not acquired:
            logger.error("quarantine lock for %s not acquired within 5s — refusing to "
                         ...
                         "state-snapshots via `moor snapshot list` / `moor snapshot restore <id>`.",
                         path)
            return None
        return _do_quarantine()
...
def collect_state_db_stats(db_path: Path) -> Dict[str, Any]:
    ...
    from moor_state import _connect_tracked_db
