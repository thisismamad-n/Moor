"""ACP auth helpers — detect and advertise Moor authentication methods."""
...
TERMINAL_SETUP_AUTH_METHOD_ID = "moor-setup"


def detect_provider() -> Optional[str]:
    """Resolve the active Moor runtime provider, or None if unavailable.
...
def build_auth_methods() -> list[Any]:
    """Return registry-compatible ACP auth methods for Moor.

    The ACP registry requires at least one usable auth method in the initial
    handshake. A fresh Zed install may have no Moor credentials yet, so the
    ...
    if provider:
        methods.append(AuthMethodAgent(
            ...
            description=f"Authenticate Moor using the currently configured {provider} runtime credentials.",
        ))
    methods.append(TerminalAuthMethod(
        id=TERMINAL_SETUP_AUTH_METHOD_ID, name="Configure Moor provider", type="terminal", args=["--setup"],
        description=("Open Moor' interactive model/provider setup in a terminal. "
...
def has_provider() -> bool:
    """Return True if Moor can resolve any runtime provider credentials."""
