...
Loads environment variables from ``~/.moor/.env``, configures logging
...
try:
    ...
    # shadowing Moor's own modules — ``moor acp`` can be started from any
...
def _load_env() -> None:
    ...
    if loaded:
        ...
        logging.getLogger(__name__).info(
            "No .env found at %s, using system env", moor_home / ".env"
        )
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--setup-browser",
        action="store_true",
        help="Install agent-browser + Playwright Chromium into ~/.moor/node/ "
