"""ACP agent server — exposes Moor Agent via the Agent Client Protocol."""
...
from tools.approval import (
    reset_moor_interactive_context,
    set_moor_interactive_context,
)
...
try:
    from moor_cli import __version__ as MOOR_VERSION
except Exception:
    MOOR_VERSION = "0.0.0"
...
# already uses (`moor_cli/moa_cmd.py`). This bounds each provider's row, not
...
def _path_from_file_uri(uri: str) -> Path | None:
    ...
    /mnt/<drive>/... so Moor running in WSL can read it.
    """
    raw = (uri or "").strip()
    if not raw:
        return None
    ...
    if path is None:
        return [{
            ...
            "text": _format_resource_text(
                ...
                name=name,
                title=title,
                body="[Resource link only; Moor cannot read non-file ACP resource URIs directly.]",
            ),
        }]
...
def _content_blocks_to_openai_user_content(
    ...
    """Convert ACP prompt blocks into a Moor/OpenAI-compatible user content payload."""
...
class MoorACPAgent(acp.Agent):
    """ACP Agent implementation wrapping Moor AIAgent."""
    ...
    def _session_modes(self, state: SessionState) -> SessionModeState:
        ...
        modes, which coexist with the model picker, so Moor maps edit approval
    ...
    def _build_model_state(self, state: SessionState) -> SessionModelState | None:
        ...
        The shared Moor inventory is also used by ``moor model``, the TUI,
        ...
        try:
            ...
            from moor_cli.models import normalize_provider, provider_label
            ...
            def choice_provider(model_id: str) -> str:
                ...
                if parts[:1] == ["custom"] and len(parts) > 1:
                    from moor_cli.models import _configured_custom_provider_ids
    ...
    def _resolve_model_selection(raw_model: str, current_provider: str) -> tuple[str, str]:
        ...
        try:
            from moor_cli.models import detect_provider_for_model, parse_model_input
    ...
    def _build_usage_update(state: SessionState) -> UsageUpdate | None:
        ...
        the current request pressure.  Moor estimates ``used`` from the same
    ...
    def _provenance_meta(
        ...
        """Best-effort ``_meta.moor.sessionProvenance`` for an ACP session."""
        try:
    ...
    async def _send_session_info_update(
        ...
        session_id: str,
        *,
        current_moor_session_id: Optional[str] = None,
        previous_moor_session_id: Optional[str] = None,
    ) -> None:
        """Send ACP native session metadata after Moor changes it.

        When the internal Moor head rotated (e.g. compression-driven session
        split during a turn), pass ``previous_moor_session_id`` so the
        attached ``_meta.moor.sessionProvenance`` flags the rotation reason.
        """
        if not self._conn:
            return
        try:
        ...
        # moor_state.py schema — only started_at/ended_at). Use "now" as
        ...
        meta = self._provenance_meta(
            session_id,
            current_moor_session_id or session_id,
    ...
    async def _register_session_mcp_servers(
        ...
        try:
            ...
            enabled_toolsets = _expand_acp_enabled_toolsets(
                getattr(state.agent, "enabled_toolsets", None) or ["moor-acp"],
    ...
    def _schedule_mcp_late_refresh(self, state: SessionState) -> None:
        ...
        """
        try:
            from moor_cli.mcp_startup import mcp_discovery_in_flight
        except Exception:
            return
        ...
        def _wait_then_refresh() -> None:
            try:
                from moor_cli.mcp_startup import join_mcp_discovery
    ...
    async def initialize(
        ...
        **kwargs: Any,
        ...
        return InitializeResponse(
            ...
            agent_info=Implementation(name="moor-agent", version=MOOR_VERSION),
    ...
    async def authenticate(self, method_id: str, **kwargs: Any) -> AuthenticateResponse | None:
        ...
        # Moor' threat model (ACP is stdio-only, local-trust), but poor
        ...
        if normalized_method == TERMINAL_SETUP_AUTH_METHOD_ID:
            # Terminal auth launches Moor setup/model selection out-of-band.
    ...
    def _history_summary_meta(message: dict[str, Any], text: str) -> dict[str, Any] | None:
        ...
        Two distinct keys under ``_meta.moor`` (ACP's extensibility
        ...
        * ``containsCompactionSummary: true`` — a merged-tail message: real
        ...
        if kind is None and message.get(COMPRESSED_SUMMARY_METADATA_KEY):
            ...
            kind = "standalone"
        if kind == "standalone":
            return {"moor": {"compactionSummary": True}}
        if kind == "merged":
            return {"moor": {"containsCompactionSummary": True}}
        return None
    ...
    async def _replay_session_history(self, state: SessionState) -> None:
        ...
        notifications. Merely restoring server-side state makes Moor
    ...
    async def prompt(
        ...
        **kwargs: Any,
        ...
        # ACP thought panes should not receive Moor' local kawaii waiting/status
        ...
        # tools.approval (set_moor_interactive_context) rather than
        # os.environ["MOOR_INTERACTIVE"], so concurrent executor workers can't
        ...
        # callback shape — not the gateway-queue MOOR_EXEC_ASK path,
        ...
        def _run_agent() -> dict:
            ...
            # Bind MOOR_SESSION_KEY for this session so per-session caches
            ...
            try:
                ...
                # model emits absolute paths under ~/.moor/workspace and the
                # edit silently lands outside the editor's workspace.
                # cron_session="" explicitly marks this as a non-cron context,
                # masking any leaked process-global MOOR_CRON_SESSION (#37968).
            ...
            interactive_token = set_moor_interactive_context(True)
            ...
            previous_session_id = os.environ.get("MOOR_SESSION_ID")
            ...
            try:
                ...
                if interactive_token is not None:
                    reset_moor_interactive_context(interactive_token)
                # Restore MOOR_SESSION_ID symmetrically.
                if previous_session_id is None:
                    os.environ.pop("MOOR_SESSION_ID", None)
                else:
                    os.environ["MOOR_SESSION_ID"] = previous_session_id
                if approval_cb:
                    try:
                        from tools import terminal_tool as _terminal_tool
        ...
        try:
            # Snapshot the internal Moor DB session id before the turn so we
            ...
            pre_turn_moor_id = getattr(state.agent, "session_id", None)
            # Wrap the executor call in a fresh copy of the current context so
            # concurrent ACP sessions on the shared ThreadPoolExecutor don't
            # stomp on each other's ContextVar writes (MOOR_SESSION_KEY in
        ...
        # _meta.moor.sessionProvenance so ACP clients can render the boundary
        # and keep old/new ids in lineage. The ACP session_id is unchanged.
        post_turn_moor_id = getattr(state.agent, "session_id", None)
        if (
            conn
            and post_turn_moor_id
            and pre_turn_moor_id
            and post_turn_moor_id != pre_turn_moor_id
        ):
            try:
                await self._send_session_info_update(
                    session_id,
                    current_moor_session_id=post_turn_moor_id,
        ...
        # Moor' local "waiting for model response" interrupt status is metadata,
    ...
    def _cmd_version(self, args: str, state: SessionState) -> str:
        return f"Moor Agent v{MOOR_VERSION}"
