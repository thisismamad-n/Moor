...
from acp_adapter.commands import MOOR_VERSION, SlashCommandsMixin, _estimate_tokens
...
from tools.approval_context import reset_moor_interactive_context, set_moor_interactive_context
...
class MoorACPAgent(SlashCommandsMixin, acp.Agent):
    ...
    def _build_model_state(self, state: SessionState) -> SessionModelState | None:
        """Authenticated providers + models, from the shared Moor inventory (same substrate
        as ``moor model``/TUI/dashboard) so the selector isn't just the current curated list."""
    ...
    def _provenance_meta(
        self, acp_session_id: str, current_moor_session_id: str, previous_moor_session_id: Optional[str] = None
    ) -> Optional[dict]:
        """Best-effort ``_meta.moor.sessionProvenance`` for an ACP session."""
        try:
            return session_provenance_meta(
                self.session_manager._get_db(), acp_session_id, current_moor_session_id,
    ...
    async def _send_session_info_update(
        self, session_id: str, *,
        current_moor_session_id: Optional[str] = None, previous_moor_session_id: Optional[str] = None,
    ) -> None:
        """Session metadata update; pass ``previous_moor_session_id`` when the internal head
        ...
        update = SessionInfoUpdate(
            ...
            field_meta=self._provenance_meta(
                session_id, current_moor_session_id or session_id, previous_moor_session_id
            ),
        )
        await self._send(
    ...
    async def _register_session_mcp_servers(
        ...
        try:
            ...
            agent.enabled_toolsets = _expand_acp_enabled_toolsets(
                getattr(agent, "enabled_toolsets", None) or ["moor-acp"],
    ...
    def _run_agent_turn(
        ...
        Interactive routing is a ``tools.approval`` contextvar, not ``MOOR_INTERACTIVE`` in
        ...
        with contextlib.ExitStack() as stack:
            # MOOR_SESSION_KEY scopes per-session caches (interactive sudo password) to this
            # session, not the reused thread. ``cwd`` pins what the system prompt reports as the
            # working directory — otherwise it advertises the Moor workspace while tools are
            # rooted at the client's project and edits land outside it. ``cron_session=""`` masks
            # any leaked process-global MOOR_CRON_SESSION.
            ...
            stack.callback(reset_moor_interactive_context, set_moor_interactive_context(True))
            # Tools tag side-effects with the ACP session (``kanban_create``); save/restore it.
            stack.callback(_restore_env, "MOOR_SESSION_ID", os.environ.get("MOOR_SESSION_ID"))
    ...
    async def prompt(self, prompt: list[PromptBlock], session_id: str, **kwargs: Any) -> PromptResponse:
        ...
        return await self._finish_turn(state, session_id, conn, result, pre_turn_moor_id, cbs.streamed)
    ...
    async def _finish_turn(
        self, state: SessionState, session_id: str, conn: Any, result: dict, pre_turn_moor_id: Any,
