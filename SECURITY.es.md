# Política de Seguridad de Moor Agent

Este documento describe el modelo de confianza de Moor Agent, identifica el
...
Reporta de forma privada a través de [GitHub Security Advisories](https://github.com/Moor inc./hermes-agent/security/advisories/new)
o **security@Moor inc..com**. No abras issues públicos para
...
Moor Agent es un agente personal de un solo inquilino. Su postura es
...
- **Proceso del agente.** El intérprete Python que ejecuta Moor Agent,
...
- **Envolvente de confianza.** El conjunto de recursos a los que un operador ha otorgado
  implícitamente acceso a Moor Agent al ejecutarlo — típicamente, todo lo que
  la propia cuenta de usuario del operador puede alcanzar en el host.
- **Postura.** Una declaración explícita en la documentación o código de Moor Agent
...
Moor Agent admite dos posturas de aislamiento a nivel de SO. Abordan
...
Moor Agent admite esto de dos maneras:

- **La propia imagen Docker de Moor Agent y la configuración de Compose.** Más
...
Bajo una envoltura de proceso completo, las heurísticas en proceso de Moor Agent
...
Moor Agent filtra el entorno que pasa a sus componentes en proceso de
...
Un plugin malicioso o con errores no es una vulnerabilidad en Moor Agent
en sí mismo. Los errores en la ruta de instalación o descubrimiento de plugins de Moor Agent
...
4. **Dentro del conjunto autorizado, todos los llamadores tienen la misma confianza.**
   Moor Agent no modela capacidades por llamador dentro de un único adaptador.
...
- Violaciones de la documentación del modelo de confianza: código que se comporta
  contrariamente a lo que esta política, la propia documentación de Moor Agent o
  las expectativas razonables del operador predecirían — incluyendo casos donde
  Moor Agent ha documentado una postura sobre cómo su salida debe ser
...
- **Habilidades y plugins contribuidos por la comunidad.** Las habilidades de terceros
  ...
  están en la superficie de revisión del operador, no en la superficie de confianza de Moor Agent
  (§2.4, §2.5). Una habilidad o plugin que haga algo
  malicioso es el modo de falla esperado de uno que no fue
  revisado, no una vulnerabilidad en Moor Agent. Los errores en la ruta de
  instalación de habilidades o plugins de Moor Agent que impidan al
...
- Moor Agent incluye guardias de cadena de suministro para lanzamientos de servidores
