...
an unlinked SQLite DB/WAL/SHM generation.  ``moor_state`` supplies only the
...
_MOOR_EXECUTABLES = frozenset({"moor", "moor-agent", "moor-acp"})
_MOOR_PYTHON_MODULES = frozenset({"acp_adapter", "moor_cli.main"})
_MOOR_PYTHON_SCRIPTS = frozenset({"moor_cli/main.py", "run_agent.py"})
...
def _looks_like_moor(argv: Sequence[str]) -> bool:
    """Return whether argv identifies a supported Moor execution target."""
    if not argv:
        return False
    program = os.path.basename(argv[0]).lower().removesuffix(".exe")
    if program in _MOOR_EXECUTABLES:
        return True
    ...
    if kind == "module":
        return normalized in _MOOR_PYTHON_MODULES
    return any(
        normalized == script or normalized.endswith(f"/{script}")
        for script in _MOOR_PYTHON_SCRIPTS
    )
        ...
        try:
            ...
            for pid_str in os.listdir("/proc"):
                ...
                try:
                    ...
                    if argv is not None and _looks_like_moor(argv):
                        cmdline = " ".join(argv)
