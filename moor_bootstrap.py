"""Windows UTF-8 bootstrap for Moor entry points.
...
should be imported at the very top of every Moor entry point
(``moor``, ``moor-agent``, ``moor-acp``, ``python -m gateway.run``,
...
def suppress_platform_ver_console() -> None:
    ...
    Mirrors ``moor_cli._subprocess_compat.suppress_platform_ver_console``
    ...
    ``moor_bootstrap``, never ``moor_cli.main``.
    """
    if not _IS_WINDOWS:
        return
    try:
...
def harden_import_path(src_root: str | None = None) -> None:
    ...
    Moor ships top-level modules with common names (``utils``, ``proxy``,
    ...
    We drop the relative forms outright, then force the real Moor source root
...
def activate_durable_lazy_target() -> None:
    ...
    (``MOOR_LAZY_INSTALL_TARGET``, e.g. ``/opt/data/lazy-packages``).
