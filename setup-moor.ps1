# ============================================================================
# Moor Agent Setup Script (Windows) — THE dev-environment entry point.
# ============================================================================
...
Write-Host ''
Write-Host 'Moor Agent Setup' -ForegroundColor Cyan
Write-Host ''
...
$store = if ($env:MOOR_RUNTIME_DIR) { $env:MOOR_RUNTIME_DIR } else { Join-Path $HOME '.moor/tools' }
...
if (Test-Path $uv) {
    ...
    $tmp = Join-Path ([System.IO.Path]::GetTempPath()) ("moor-setup-" + [guid]::NewGuid().ToString('n'))
...
# ---------------------------------------------------------------------------
$skillsDir = if ($env:MOOR_HOME) { Join-Path $env:MOOR_HOME 'skills' } else { Join-Path $HOME '.moor/skills' }
...
Write-Host '     moor setup'
Write-Host ''
Write-Host '  3. Start chatting:'
Write-Host '     moor'
Write-Host ''
Write-Host 'Other commands:'
Write-Host '  moor pm install     # Re-run the tool + dependency install'
Write-Host '  moor status         # Check configuration'
Write-Host '  moor doctor         # Diagnose issues'
