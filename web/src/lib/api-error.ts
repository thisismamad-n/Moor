...
export const API_UNREACHABLE_MESSAGE =
  "Moor dashboard cannot reach the Moor service. Is `moor dashboard` still running?";
...
const STATUS_COPY: Record<number, string> = {
  ...
  500: "The Moor service hit an internal error.",
  502: "The dashboard proxy could not reach the Moor service.",
  503: "The Moor service is not ready yet. Try again in a moment.",
  504: "The Moor service took too long to respond.",
};

export function humanizeStatus(status: number): string {
  return STATUS_COPY[status] ?? `The Moor service answered with an unexpected error (${status}).`;
}
