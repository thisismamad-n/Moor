...
import { api, MOOR_BASE_PATH } from "@/lib/api";
...
export const MANIFEST_CACHE_KEY = "moor:plugin-manifests";
...
export function usePlugins() {
  ...
  useEffect(() => {
    ...
    for (const manifest of manifests) {
      ...
      if (manifest.css) {
        const cssUrl = `${MOOR_BASE_PATH}/dashboard-plugins/${manifest.name}/${manifest.css}`;
      ...
      const baseUrl = `${MOOR_BASE_PATH}/dashboard-plugins/${manifest.name}/${manifest.entry}`;
      const scriptSrc = import.meta.env.DEV
        ? `${baseUrl}?moor_dv=${Date.now()}`
        : baseUrl;
      ...
      script.setAttribute("data-moor-plugin", manifest.name);
