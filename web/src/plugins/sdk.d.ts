/**
 * Moor Dashboard Plugin SDK — typed contract (SPIKE)
 * ====================================================
 *
 * This is the public type surface for ``window.__MOOR_PLUGIN_SDK__`` and
 * ``window.__MOOR_PLUGINS__``, the globals the dashboard host exposes to
 ...
 * window.__MOOR_PLUGIN_SDK__`` because:
 *   1. The runtime object is assembled from many internal modules
 *      (``@/lib/api``, ``@moor-research/ui``, …). Deriving the type would
 ...
 *
 * Versioning: bump ``MoorPluginSDK["sdkVersion"]`` (and the
 ...
 *   - Ship as a published ``@moor/dashboard-plugin-sdk`` types package, or
 ...
 *     (pulls @moor-research/ui types into the contract) or is the loose
 ...
 * of calling ``fetch`` with a hand-read ``window.__MOOR_SESSION_TOKEN__``.
 */
...
// ---------------------------------------------------------------------------
// Registry surface (window.__MOOR_PLUGINS__)
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// SDK surface (window.__MOOR_PLUGIN_SDK__)
// ---------------------------------------------------------------------------

export interface MoorPluginSDK {
  ...
  /**
   * Shared UI primitives (Moor DS / shadcn). Typed permissively at the
...
declare global {
  interface Window {
    __MOOR_PLUGIN_SDK__?: MoorPluginSDK;
    __MOOR_PLUGINS__?: PluginRegistry;
  }
}
