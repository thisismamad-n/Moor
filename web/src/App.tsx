...
import { SelectionSwitcher } from "@moor-research/ui/ui/components/selection-switcher";
...
import { useBelowBreakpoint } from "@moor-research/ui/hooks/use-below-breakpoint";
...
const SIDEBAR_COLLAPSED_KEY = "moor-sidebar-collapsed";
          ...
          >
            ...
            >
              ...
              {sidebarNav.pluginItems.length > 0 && (
                <div
                  aria-labelledby="moor-sidebar-plugin-nav-heading"
                ...
                >
                  <span
                    ...
                    id="moor-sidebar-plugin-nav-heading"
                  >
...
function SidebarSystemActions({
  ...
  const updateConfirmDescription = useMemo(() => {
    if (updateConfirmInfo?.behind && updateConfirmInfo.behind > 0) {
      ...
      return `This will run 'moor update' (${cmd}) and pull ${n} new commit${n === 1 ? "" : "s"}. The gateway restarts when the update finishes; the current session keeps its prompt cache until then.`;
    }
    const cmd = updateConfirmInfo?.update_command ?? "moor update";
    return (
      t.status.updateMoorConfirmMessage ??
      `This will run 'moor update' (${cmd}) and restart the gateway when it finishes.`
    );
  }, [t.status.updateMoorConfirmMessage, updateConfirmInfo]);
  ...
  if (canUpdateMoor) {
    items.push({
      action: "update",
      icon: Download,
      label: t.status.updateMoor,
      runningLabel: t.status.updatingMoor,
      spin: false,
    });
  }
  ...
  return (
    ...
    <ConfirmDialog
      cancelLabel={t.common.cancel}
      confirmLabel={t.status.updateMoorConfirmNow ?? "Update now"}
      description={
      ...
      title={t.status.updateMoorConfirmTitle ?? `${t.status.updateMoor}?`}
    />
    </>
  );
}
