...
interface MoorConsoleModalProps {
  open: boolean;
  onClose: () => void;
}
...
export function MoorConsoleModal({ open, onClose }: MoorConsoleModalProps) {
  const modalRef = useModalBehavior({ open, onClose });
  const hostRef = useRef<HTMLDivElement | null>(null);
  const termRef = useRef<XtermTerminal | null>(null);
  const wsRef = useRef<WebSocket | null>(null);
  const lineRef = useRef("");
  const promptRef = useRef("moor> ");
  const inputPromptRef = useRef("moor> ");
  ...
  const handleFrame = useCallback(
    (frame: ConsoleFrame) => {
      ...
      if (frame.type === "ready") {
        const nextPrompt = frame.prompt || "moor> ";
  ...
  useEffect(() => {
    ...
    writeLine(term, "\x1b[2mConnecting to Moor Console...\x1b[0m");
  ...
  return createPortal(
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="moor-console-title"
    >
      <div
        className={cn(
          themedBody,
