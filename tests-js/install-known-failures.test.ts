...
const lockedLog = [
  ...
  'File "C:/install/venv/Scripts/moor.exe/__main__.py", line 10, in <module>',
...
const base = {
  ...
  installMethod: 'installer-script', updateMethod: 'moor-update',
  error: 'E2E ASSERTION FAILED: moor update exited 1 (expected 0)', logs: { update: lockedLog },
}
...
describe('known install failures', () => {
  it('recognizes the released launcher self-lock, not generic access denied', () => {
    ...
    expect(matchKnownFailure({ ...base, logs: { update: lockedLog.replaceAll('moor.exe', 'other.exe') } })).toBeNull()
  ...
  it('matches manual-only app updates only for the three proven July script cases', () => {
    const sample = {
      ...
      updateMethod: 'moor-desktop-app-update',
      error: 'E2E ASSERTION FAILED: app driven via captured moor desktop spec; update completed',
      logs: { desktop: '[moor] [updates] no staged updater; surfacing manual `moor update` for CLI install at C:/install\n[moor] [updates] manual: moor update\n' },
    }
    ...
    expect(matchKnownFailure({ ...sample, logs: { desktop: '[updates] manual: moor update' } })).toBeNull()
  })
...
it('renders known receipts as footnotes, without suppressing a red job', async () => {
  ...
  const name = 'windows: installer-script -> moor-update (v2026.3.12 -> HEAD)'
