...
test('the update window carries Moor connection state without cloning Chromium state', (): void => {
  ...
  const captured = {
    MOOR_HOME: path.join(root, 'moor-home'),
  ...
  try {
    ...
    const isolatedUserData = isolated.MOOR_DESKTOP_USER_DATA_DIR

    expect(isolatedUserData).not.toBe(userData)
    expect(captured.MOOR_DESKTOP_USER_DATA_DIR).toBe(userData)
    expect(isolated.MOOR_HOME).toBe(captured.MOOR_HOME)
