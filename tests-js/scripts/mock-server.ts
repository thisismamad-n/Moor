 ...
 * `E2E_SAY(code-farmer)[{at}moor Reply with B.] E2E_SAY(moor)[B]`.
...
/** Create an isolated MOOR_HOME + Electron user-data dir in the OS temp dir. */
function createDevSandbox(): DevSandbox {
  const root = fs.mkdtempSync(nodePath.join(os.tmpdir(), `moor-dev-mock-${Date.now()}`))
  const moorHome = nodePath.join(root, 'moor-home')
...
async function runDevLaunch(): Promise<void> {
  ...
  const env: Record<string, string> = {
    ...
    MOOR_DESKTOP_MOOR_ROOT: repoRoot,
