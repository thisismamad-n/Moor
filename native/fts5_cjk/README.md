...
storage discipline as the other indexes). On a populated database, run
    ...
    moor sessions optimize-storage

to backfill it; new messages are indexed live either way. Set
`sessions.cjk_fts: false` in `~/.moor/config.yaml` to disable. Override
the .so location with `MOOR_FTS5_CJK_SO`.
