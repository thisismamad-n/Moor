#!/bin/sh
# shellcheck shell=sh
# /opt/moor/docker/main-wrapper.sh — wraps the container's CMD with
...
# with-contenv before touching MOOR_HOME / PATH. On the non-PID-1
# fallback path the Dockerfile env is still intact, so we skip the
# re-exec and continue directly.
#
# Routing:
#   no args                       → exec `moor` (the default)
#   first arg is an executable    → exec it directly (sleep, bash, sh, …)
#   first arg is anything else    → exec `moor <args>` (subcommand passthrough)
#
# Drop to moor via s6-setuidgid, but skip it when already non-root.
set -e

if [ -z "${MOOR_MAIN_WRAPPER_ENV_READY:-}" ] && \
   [ -z "${MOOR_HOME:-}" ] && \
   [ -x /command/with-contenv ]; then
    export MOOR_MAIN_WRAPPER_ENV_READY=1
    exec /command/with-contenv sh "$0" "$@"
fi
unset MOOR_MAIN_WRAPPER_ENV_READY

drop() { [ "$(id -u)" = 0 ] && set -- s6-setuidgid moor "$@"; exec "$@"; }
...
# an arbitrary non-root, non-moor UID, the bootstrap was skipped and the
# baked image dirs (owned by the moor build UID) are unwritable, so fail
...
if [ "$cur_uid" != 0 ] && [ "$cur_uid" != "$(id -u moor)" ]; then
    cat >&2 <<EOF
[moor] ERROR: container started with --user $cur_uid (an arbitrary, non-moor UID) — not supported.
...
# to the moor user's home before dropping privileges so libraries that
...
_moor_orig_cwd="${MOOR_ORIG_CWD:-$PWD}"
...
# the user's command so `moor chat` starts in the Docker -w
