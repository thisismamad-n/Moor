...
#
# Wired into the image as /etc/cont-init.d/01-moor-setup by the
...
MOOR_HOME="${MOOR_HOME:-/opt/data}"
INSTALL_DIR="/opt/moor"

# Drop to moor via s6-setuidgid, but skip it when already non-root.
as_moor() { [ "$(id -u)" = 0 ] || { "$@"; return; }; s6-setuidgid moor "$@"; }
...
# starts non-root. The baked install tree under /opt/moor is intentionally
# root-owned and non-writable; mutable runtime state must live under
# $MOOR_HOME. An arbitrary `--user` UID therefore cannot repair or populate
...
# default) and pass MOOR_UID/MOOR_GID — or the PUID/PGID aliases — which the
...
# `id -u moor` is the unremapped build UID (10000) because no root-only remap
# could run. root starts (id -u = 0) and the normal supervised drop to the
# moor UID are both unaffected.
cur_uid="$(id -u)"
if [ "$cur_uid" != 0 ] && [ "$cur_uid" != "$(id -u moor)" ]; then
    cat >&2 <<EOF
[stage2] ERROR: container started with --user $cur_uid (an arbitrary, non-moor UID).
...
and the baked /opt/moor install tree is intentionally root-owned and
...
Start the container as root (the default) and pass your host UID/GID instead:
    ...
    docker run -e MOOR_UID=\$(id -u) -e MOOR_GID=\$(id -g) ...

NAS users (Synology / unRAID / UGOS) can use the PUID/PGID aliases:

    docker run -e PUID=\$(id -u) -e PGID=\$(id -g) ...

The image remaps the moor user to that UID/GID at boot and chowns the data
...
# --- Bootstrap MOOR_HOME as root ---
# Create the directory (and any missing parents) while we still have root
# privileges so the chown checks below see real metadata and the later
# `s6-setuidgid moor mkdir -p` block doesn't EACCES on root-owned
# ancestors. Without this, custom MOOR_HOME paths whose parents only
# root can create (e.g. `MOOR_HOME=/home/moor/.moor` in a Compose
...
# --- UID/GID remap ---
# Accept PUID/PGID as aliases for MOOR_UID/MOOR_GID.  NAS users (UGOS,
...
# UID 10000 leaves the runtime unable to read the volume.  MOOR_UID/
# MOOR_GID still win when both are set.  See #15290, salvages #25872.
MOOR_UID="${MOOR_UID:-${PUID:-}}"
MOOR_GID="${MOOR_GID:-${PGID:-}}"

if [ -n "${MOOR_UID:-}" ] && validate_uid_gid "$MOOR_UID" && [ "$MOOR_UID" != "$(id -u moor)" ]; then
    echo "[stage2] Changing moor UID to $MOOR_UID"
    usermod -u "$MOOR_UID" moor
fi
if [ -n "${MOOR_GID:-}" ] && validate_uid_gid "$MOOR_GID" && [ "$MOOR_GID" != "$(id -g moor)" ]; then
    echo "[stage2] Changing moor GID to $MOOR_GID"
    # -o allows non-unique GID (e.g. macOS GID 20 "staff" may already
    # exist as "dialout" in the Debian-based container image).
    groupmod -o -g "$MOOR_GID" hermes 2>/dev/null || true
fi
...
# host's `docker` group (or root). The supervised moor user (UID 10000)
...
# moor process with `Groups: 10000` (998 gone); after this hook adds
...
# matching entry that includes moor. Idempotent across container
...
#   - moor is already a member of the right group (idempotent restart)
...
for sock in /var/run/docker.sock /run/docker.sock; do
    ...
    if id -G hermes 2>/dev/null | tr ' ' '\n' | grep -qx "$sock_gid"; then
        echo "[stage2] moor already in group $sock_gid for $sock"
        break
    fi
    ...
    if usermod -aG "$sock_group" hermes 2>/dev/null; then
        echo "[stage2] Added moor to group $sock_group (GID $sock_gid) for $sock"
    else
        echo "[stage2] Warning: usermod -aG $sock_group moor failed; docker backend may fail with EACCES"
    fi
    break
done

# --- Fix ownership of data volume ---
# When MOOR_UID is remapped or the top-level $MOOR_HOME isn't owned by
# the runtime moor UID, restore ownership to moor — but ONLY for the
# directories moor actually writes to. The full $MOOR_HOME may be a
# host-mounted bind containing unrelated user files; `chown -R` would
# silently destroy host ownership of those (see issue #19788).
#
# The canonical list of moor-owned subdirs is the same one the s6-setuidgid
# mkdir -p block below seeds. Keep them in sync if the seed list changes.
actual_moor_uid=$(id -u moor)
...
chown_moor_tree() {
    ...
    chown -R moor:moor "$target" 2>/dev/null || \
        echo "[stage2] Warning: chown $target failed (rootless container?) — continuing"
}

tree_has_non_moor_owner() {
    target="$1"
    find "$target" \( ! -user moor -o ! -group moor \) -print -quit 2>/dev/null | grep -q .
}

needs_chown=false
if [ "$(stat -c %u "$MOOR_HOME" 2>/dev/null)" != "$actual_moor_uid" ]; then
    needs_chown=true
fi
if [ "$needs_chown" = true ]; then
    echo "[stage2] Fixing ownership of $MOOR_HOME (targeted) to moor ($actual_moor_uid)"
    # In rootless Podman the container's "root" is mapped to an
    # unprivileged host UID — chown will fail. That's fine: the volume
    # is already owned by the mapped user on the host side.
    #
    # Top-level $MOOR_HOME: chown the directory itself (not its contents)
    # so moor can mkdir new subdirs but bind-mounted host files keep
    # their existing ownership.
    if refuse_symlinked_path "chown" "$MOOR_HOME"; then
        :
    else
        chown moor:moor "$MOOR_HOME" 2>/dev/null || \
            echo "[stage2] Warning: chown $MOOR_HOME failed (rootless container?) — continuing"
    fi
    # moor-owned subdirs: recursive chown is safe here because these are
...
# $MOOR_HOME (/opt/data) and run with PYTHONDONTWRITEBYTECODE plus
# MOOR_DISABLE_LAZY_INSTALLS=1. Keeping /opt/moor root-owned and
...
# $MOOR_HOME/lazy-packages dir on the data volume (Dockerfile sets
# MOOR_LAZY_INSTALL_TARGET). That dir is appended to the END of sys.path,
...
# even though installs are re-enabled. The dir is seeded + chowned to moor
...
# are invoked via `docker exec <container> moor …` (which defaults
# to root unless `-u` is passed), and that breaks the cont-init
# reconciler (02-reconcile-profiles) which runs as moor and walks
...
if [ -d "$MOOR_HOME/profiles" ] && tree_has_non_moor_owner "$MOOR_HOME/profiles"; then
    chown_moor_tree "$MOOR_HOME/profiles"
fi

# Always reset ownership of $MOOR_HOME/cron on every boot for the same
# docker-exec/root-write reason as profiles/. The cron scheduler state
# (jobs.json) must stay readable by the unprivileged moor runtime even
...
# gate as profiles/).
if [ -d "$MOOR_HOME/cron" ] && tree_has_non_moor_owner "$MOOR_HOME/cron"; then
    chown_moor_tree "$MOOR_HOME/cron"
fi

# Always ensure logs/gateways is moor-owned (#45258). Formerly healed by
...
# top-level $MOOR_HOME is mis-owned, so a warm volume with moor-owned
# MOOR_HOME but root-owned logs/gateways would otherwise leave
# s6-setuidgid moor mkdir failing with Permission denied. Non-recursive:
# profile leaf dirs are each created/owned by their own log/run as moor.
if [ -d "$MOOR_HOME/logs/gateways" ]; then
    if refuse_symlinked_path "chown" "$MOOR_HOME/logs/gateways"; then
        :
    else
        chown moor:moor "$MOOR_HOME/logs/gateways" 2>/dev/null || true
    fi
fi
...
# moor pairing approve …` defaults to uid=0 and writes 0600 root-owned
# approval files that the unprivileged moor gateway cannot read,
# silently leaving the approved user unauthorized (#10270). The targeted
# data-volume chown above only runs when the top-level $MOOR_HOME is
...
# and cron/.
if [ -d "$MOOR_HOME/platforms/pairing" ] && tree_has_non_moor_owner "$MOOR_HOME/platforms/pairing"; then
    chown_moor_tree "$MOOR_HOME/platforms/pairing"
fi
# Legacy location (pre-consolidated layout).
if [ -d "$MOOR_HOME/pairing" ] && tree_has_non_moor_owner "$MOOR_HOME/pairing"; then
    chown_moor_tree "$MOOR_HOME/pairing"
fi

# Reset ownership of moor-owned top-level state files on every boot.
# The targeted data-volume chown above only covers moor-owned
# *subdirectories*; loose state files living directly under $MOOR_HOME
# are missed. When those files are created or rewritten by
# `docker exec <container> moor …` (root unless `-u` is passed) they
# land root-owned, and the unprivileged moor runtime then hits
...
# sweep so host-owned files in a bind-mounted $MOOR_HOME are never
...
# entries of moor_cli.profile_distribution.USER_OWNED_EXCLUDE plus the
...
for f in \
    ...
    active_profile; do
    if [ -e "$MOOR_HOME/$f" ]; then
        if refuse_symlinked_path "chown" "$MOOR_HOME/$f"; then
            :
        else
            chown moor:moor "$MOOR_HOME/$f" 2>/dev/null || true
        fi
    fi
done

# --- config.yaml permissions ---
# Ensure config.yaml is readable by the moor runtime user even if it
# was edited on the host after initial ownership setup.
if [ -f "$MOOR_HOME/config.yaml" ]; then
    if refuse_symlinked_path "chown/chmod" "$MOOR_HOME/config.yaml"; then
        :
    else
        chown moor:moor "$MOOR_HOME/config.yaml" 2>/dev/null || true
        chmod 640 "$MOOR_HOME/config.yaml" 2>/dev/null || true
    fi
fi

# --- Seed directory structure as moor user ---
# Run as moor via s6-setuidgid so dirs end up owned correctly (matters
...
as_moor mkdir -p \
    ...
    "$MOOR_HOME/lazy-packages"

# --- Install-method stamp ---
# The 'docker' stamp is baked into the immutable install tree at
# /opt/moor/.install_method (see Dockerfile), NOT written here into
# $MOOR_HOME. detect_install_method() reads the code-scoped stamp first.
#
# Why we no longer stamp $MOOR_HOME: it is a shared DATA volume, commonly
# bind-mounted from the host (~/.moor:/opt/data) and sometimes shared with a
...
# 'moor update'. To heal homes already poisoned by older images, remove a
# stale 'docker' stamp from $MOOR_HOME if one is present (the host install's
# own installer re-creates its code-scoped stamp; a genuine container relies on
# the baked /opt/moor stamp, so deleting the data-dir copy is safe).
if [ -f "$MOOR_HOME/.install_method" ]; then
    stamped="$(tr -d '[:space:]' < "$MOOR_HOME/.install_method" 2>/dev/null || true)"
    if [ "$stamped" = "docker" ]; then
        rm -f "$MOOR_HOME/.install_method" 2>/dev/null || true
    fi
fi
...
seed_one() {
    dest=$1
    src=$2
    if [ ! -f "$MOOR_HOME/$dest" ] && [ -f "$INSTALL_DIR/$src" ]; then
        if refuse_symlinked_path "seed" "$MOOR_HOME/$dest"; then
            :
        else
            as_moor cp "$INSTALL_DIR/$src" "$MOOR_HOME/$dest"
        fi
    fi
}
...
# the first-boot seed above depends on /opt/moor/.env.example being present
...
# not generate one. Moor loads $MOOR_HOME/.env with override=True, so
...
if [ -n "${API_SERVER_KEY:-}" ]; then
    if [ -f "$MOOR_HOME/.env" ] && grep -q '^API_SERVER_KEY=..*' "$MOOR_HOME/.env" 2>/dev/null; then
        echo "[stage2] Warning: API_SERVER_KEY is set in both the container environment and $MOOR_HOME/.env — the .env value wins at runtime (loaded with override=True)"
    else
        ...
        if [ -f "$MOOR_HOME/.env" ] && ! refuse_symlinked_path "clean" "$MOOR_HOME/.env"; then
            sed -i '/^API_SERVER_KEY=$/d' "$MOOR_HOME/.env" 2>/dev/null || true
        fi
        echo "[stage2] API_SERVER_KEY provided via container environment — skipping generation"
    fi
elif ! grep -q '^API_SERVER_KEY=..*' "$MOOR_HOME/.env" 2>/dev/null; then
    if refuse_symlinked_path "append" "$MOOR_HOME/.env"; then
        :
    else
        if [ ! -f "$MOOR_HOME/.env" ]; then
            ...
            (umask 077 && as_moor touch "$MOOR_HOME/.env") 2>/dev/null || true
        fi
        if [ -f "$MOOR_HOME/.env" ]; then
            ...
            if [ -n "$_gen_key" ]; then
                ...
                sed -i '/^API_SERVER_KEY=$/d' "$MOOR_HOME/.env" 2>/dev/null || true
                if printf 'API_SERVER_KEY=%s\n' "$_gen_key" >> "$MOOR_HOME/.env" 2>/dev/null; then
                    echo "[stage2] Generated API_SERVER_KEY for the loopback gateway api_server"
                else
                    echo "[stage2] Warning: could not write API_SERVER_KEY to $MOOR_HOME/.env (read-only volume?) — gateway api_server (cron fires) will be unavailable"
                fi
            fi
            unset _gen_key
        else
            echo "[stage2] Warning: could not create $MOOR_HOME/.env — gateway api_server (cron fires) will be unavailable"
        fi
    fi
fi
...
if [ -f "$MOOR_HOME/.env" ]; then
    if refuse_symlinked_path "chown/chmod" "$MOOR_HOME/.env"; then
        :
    else
        chown moor:moor "$MOOR_HOME/.env" 2>/dev/null || true
        chmod 600 "$MOOR_HOME/.env" 2>/dev/null || true
    fi
fi
...
# as the unprivileged `moor` user — without this it gets EACCES on every
...
# moor group (any group member could e.g. stop/suspend this machine), not
...
if [ -S /.fly/api ]; then
    if refuse_symlinked_path "chgrp/chmod" /.fly/api; then
        :
    elif chgrp moor /.fly/api 2>/dev/null && chmod g+w /.fly/api 2>/dev/null; then
        echo "[stage2] Granted moor group access to the Fly Machines API socket"
    else
...
# $MOOR_HOME on the mounted volume. Run the same safe, non-interactive
# config-schema migrations that `moor update` runs for non-Docker installs,
# after first-boot seeding and before supervised gateway services start.
# Set MOOR_SKIP_CONFIG_MIGRATION=1 for controlled/manual migrations.
if [ -f "$MOOR_HOME/config.yaml" ]; then
    s6-setuidgid moor "$INSTALL_DIR/.venv/bin/python" "$INSTALL_DIR/scripts/docker_config_migrate.py" \
...
if [ ! -f "$MOOR_HOME/auth.json" ] && [ -n "${MOOR_AUTH_JSON_BOOTSTRAP:-}" ]; then
    if refuse_symlinked_path "seed" "$MOOR_HOME/auth.json"; then
        :
    else
        printf '%s' "$MOOR_AUTH_JSON_BOOTSTRAP" > "$MOOR_HOME/auth.json"
        chown moor:moor "$MOOR_HOME/auth.json" 2>/dev/null || true
        chmod 600 "$MOOR_HOME/auth.json"
    fi
fi

# auth.json: re-seed a TERMINALLY-DEAD Moor bootstrap session (self-heal).
#
# The [ ! -f ] guard above deliberately refuses to clobber an existing
# auth.json, so a container whose Moor bootstrap session took a terminal
# invalid_grant (tokens cleared, providers.moor.last_auth_error.relogin_required
...
# supply a freshly-issued session via MOOR_AUTH_JSON_REBOOTSTRAP (distinct
# from the create-only *_BOOTSTRAP var); this helper swaps ONLY the
# providers.moor entry when the on-disk entry is provably terminal OR the
...
if [ -f "$MOOR_HOME/auth.json" ] && [ -n "${MOOR_AUTH_JSON_REBOOTSTRAP:-}" ]; then
    if refuse_symlinked_path "reseed" "$MOOR_HOME/auth.json"; then
        :
    else
        s6-setuidgid moor "$INSTALL_DIR/.venv/bin/python" \
            "$INSTALL_DIR/scripts/docker_rebootstrap_moor_session.py" \
            "$MOOR_HOME/auth.json" \
            || echo "[stage2] Warning: docker_rebootstrap_moor_session.py failed; continuing"
    fi
fi
...
# can set MOOR_GATEWAY_BOOTSTRAP_STATE=running; we seed the state file
...
# MOOR_AUTH_JSON_BOOTSTRAP seeds auth.json. The [ ! -f ] guard is the
...
if [ ! -f "$MOOR_HOME/gateway_state.json" ] && \
        [ "${MOOR_GATEWAY_BOOTSTRAP_STATE:-}" = "running" ]; then
    if refuse_symlinked_path "seed" "$MOOR_HOME/gateway_state.json"; then
        :
    else
        printf '{"gateway_state":"running"}\n' > "$MOOR_HOME/gateway_state.json"
        chown moor:moor "$MOOR_HOME/gateway_state.json" 2>/dev/null || true
        chmod 644 "$MOOR_HOME/gateway_state.json"
    fi
fi
...
if [ -d "$INSTALL_DIR/skills" ]; then
    as_moor "$INSTALL_DIR/.venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" \
        || echo "[stage2] Warning: skills_sync.py failed; continuing"
fi

# --- Discover agent-browser's Chromium binary ---
# The image's Dockerfile runs `npx playwright install chromium`, which
# populates ``$PLAYWRIGHT_BROWSERS_PATH`` (=/opt/moor/.playwright) with
# a ``chromium_headless_shell-<build>/chrome-headless-shell-linux64/``
# directory. agent-browser (the runtime CLI Moor spawns for the
...
# main-wrapper.sh propagates it into the supervised ``moor`` process
...
if [ -z "${AGENT_BROWSER_EXECUTABLE_PATH:-}" ] && \
    ...
    if [ -n "$browser_bin" ]; then
        ...
        # up for all supervised services (main-moor, dashboard, etc.).
