...
# Moor WebUI, NAS "update" flows that preserve an old entrypoint, etc.)
...
#
# Test hook: MOOR_TINI_SHIM_TARGET overrides the `/init` path (and
# MOOR_TINI_SHIM_WRAPPER the main-wrapper path) so unit tests can
# record argv without a real s6 tree.

set -e

INIT_TARGET="${MOOR_TINI_SHIM_TARGET:-/init}"
WRAPPER="${MOOR_TINI_SHIM_WRAPPER:-/opt/moor/docker/main-wrapper.sh}"
